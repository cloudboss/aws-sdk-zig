...
test "CreateUser returns successfully" {
    ...
    const create = try shared_client.createUser(
        arena.allocator(),
        .{ .user_name = user_name },
        .{},
    );
    ...
    const del = try shared_client.deleteUser(
        arena.allocator(),
        .{ .user_name = user_name },
        .{},
    );
...
test "GetUser returns created user with correct name" {
    ...
    const get = try shared_client.getUser(
        arena.allocator(),
        .{ .user_name = user_name },
        .{},
    );
...
test "ListUsers includes created user" {
    ...
    const list = try shared_client.listUsers(arena.allocator(), .{}, .{});
...
test "DeleteUser removes user" {
    ...
    const result = shared_client.getUser(
        arena.allocator(),
        .{ .user_name = user_name },
        .{},
    );
...
test "CreateRole returns role with correct name" {
    ...
    const create = try shared_client.createRole(
        arena.allocator(),
        .{
            .role_name = role_name,
    ...
    const del = try shared_client.deleteRole(
        arena.allocator(),
        .{ .role_name = role_name },
        .{},
    );
...
test "GetRole returns created role" {
    ...
    const get = try shared_client.getRole(
        arena.allocator(),
        .{ .role_name = role_name },
        .{},
    );
...
test "CreatePolicy returns policy ARN" {
    ...
    const create = try shared_client.createPolicy(
        arena.allocator(),
        .{
    ...
    const del = try shared_client.deletePolicy(
        arena.allocator(),
        .{ .policy_arn = arn },
        .{},
    );
...
test "AttachRolePolicy links policy to role" {
    ...
    const cr = try shared_client.createRole(
        arena.allocator(),
        .{
            .role_name = role_name,
            .assume_role_policy_document = trust_policy,
    ...
    const cp = try shared_client.createPolicy(
        arena.allocator(),
        .{
            .policy_name = policy_name,
            .policy_document = policy_document,
    ...
    const attach = try shared_client.attachRolePolicy(
        arena.allocator(),
    ...
    const detach = try shared_client.detachRolePolicy(
        arena.allocator(),
        .{ .role_name = role_name, .policy_arn = arn },
        .{},
    );
    ...
    const dr = try shared_client.deleteRole(
        arena.allocator(),
        .{ .role_name = role_name },
        .{},
    );
    ...
    const dp = try shared_client.deletePolicy(
        arena.allocator(),
        .{ .policy_arn = arn },
        .{},
    );
...
test "ListAttachedRolePolicies includes attached policy" {
    ...
    const list = try shared_client.listAttachedRolePolicies(
        arena.allocator(),
        .{ .role_name = role_name },
        .{},
    );
...
test "ListUsers paginator collects all users across pages" {
    ...
    for (user_names) |name| {
        const result = try shared_client.deleteUser(
            arena.allocator(),
            .{ .user_name = name },
            .{},
        );
...
test "ListRoles includes created role" {
    ...
    const list = try shared_client.listRoles(arena.allocator(), .{}, .{});
...
test "ListPolicies returns policies" {
    ...
    const list = try shared_client.listPolicies(arena.allocator(), .{}, .{});
...
test "CreateUser with duplicate name returns EntityAlreadyExists" {
    ...
    var diagnostic: iam.ServiceError = undefined;
    const dup = shared_client.createUser(
        arena.allocator(),
        .{ .user_name = user_name },
        .{ .diagnostic = &diagnostic },
    );
...
test "GetRole returns error for nonexistent role" {
    ...
    const result = shared_client.getRole(
        arena.allocator(),
