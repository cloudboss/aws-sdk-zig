...
if [[ -n "${LIVE_SCENARIO}" ]]; then
    SCENARIO_DIRS=("${SCENARIOS_DIR}/${LIVE_SCENARIO}/")
else
...
cleanup() {
    if [[ -f "${SCRIPT_DIR}/cleanup-live.sh" ]]; then
        echo "Running live cleanup..."
        bash "${SCRIPT_DIR}/cleanup-live.sh" || true
    fi
}
trap cleanup EXIT
...
for scenario_dir in "${SCENARIO_DIRS[@]}"; do
    ...
    if /bin/sh -c "${env_cmd} && zig build 'live-test-${scenario}' ${ZIG_BUILD_FLAGS}" 2>&1; then
        echo "  PASS: ${scenario}"
        PASS=$((PASS + 1))
    else
        echo "  FAIL: ${scenario}"
