...
test "kms listKeys" {
    ...
    defer arena.deinit();
    var client = kms.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
...
test "acm listCertificates" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var client = acm.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
...
test "wafv2 listWebAcLs" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var client = wafv2.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
...
test "guardduty listDetectors" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var client = guardduty.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
...
test "inspector2 listFindings" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var client = inspector2.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
...
test "accessanalyzer listAnalyzers" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var client = accessanalyzer.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
...
test "detective listGraphs" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var client = detective.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
...
test "macie2 listFindings" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var client = macie2.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
    _ = client.listFindings(
        arena.allocator(),
        .{},
...
test "securityhub describeHub" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var client = securityhub.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
...
test "shield describeSubscription" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var client = shield.Client.init(std.testing.allocator, &shared_cfg.?);
    defer client.deinit();
