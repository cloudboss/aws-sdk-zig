...
class RestJsonProtocol : ProtocolGenerator {
    ...
    private fun writeJsonKeyValue(writer: ZigWriter, jsonKey: String, zigType: String, accessor: String) {
        when (zigType) {
            "[]const u8" -> {
    ...
    private fun writeJsonKeyValueForOptional(writer: ZigWriter, jsonKey: String, zigType: String, varName: String) {
        when (zigType) {
            "[]const u8" -> {
                writer.write("try body_buf.appendSlice(alloc, \"\\\"\$L\\\":\\\"\");", jsonKey)
    ...
    override fun writeDeserializeResponse(writer: ZigWriter, ctx: OperationContext) {
        ...
        // REST-JSON deserializer also takes status for @httpResponseCode
        writer.openBlock(
