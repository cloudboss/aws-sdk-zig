...
class WaiterGenerator(
    ...
    private fun writeWaiterStruct(writer: ZigWriter, waiter: WaiterInfo) {
        ...
        writer.write("const io = self.client.config.io;")
        writer.write("const start = std.Io.Clock.real.now(io).toSeconds();")
        ...
        writer.closeBlock("}")
        writer.blankLine()
        writer.write("const elapsed: u32 = @intCast(std.Io.Clock.real.now(io).toSeconds() - start);")
        writer.openBlock("if (elapsed >= self.config.max_wait_time_s) {")
        writer.write("return error.WaiterTimedOut;")
        writer.closeBlock("}")
        writer.blankLine()
        writer.write("const jittered = aws.waiter.jitteredDelay(io, self.config.min_delay_s, delay_s);")
        writer.write("io.sleep(.fromSeconds(@intCast(jittered)), .awake) catch {};")
