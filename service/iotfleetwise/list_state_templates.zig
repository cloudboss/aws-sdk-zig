...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStateTemplatesInput, options: Options) !ListStateTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStateTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.ListStateTemplates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStateTemplatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListStateTemplatesOutput, body, alloc);
}
