...
pub const ListDecoderManifestSignalsInput = struct {
...
pub const ListDecoderManifestSignalsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDecoderManifestSignalsInput, options: CallOptions) !ListDecoderManifestSignalsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDecoderManifestSignalsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.ListDecoderManifestSignals");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDecoderManifestSignalsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDecoderManifestSignalsOutput, body, allocator);
}
