...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEncryptionConfigurationInput, options: Options) !GetEncryptionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEncryptionConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.GetEncryptionConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEncryptionConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEncryptionConfigurationOutput, body, alloc);
}
