...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreateVehicleInput, options: Options) !BatchCreateVehicleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateVehicleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.BatchCreateVehicle");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateVehicleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchCreateVehicleOutput, body, alloc);
}
