...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFleetInput, options: Options) !GetFleetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.GetFleet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFleetOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetFleetOutput, body, alloc);
}
