...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVehicleInput, options: Options) !DeleteVehicleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVehicleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.DeleteVehicle");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVehicleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteVehicleOutput, body, alloc);
}
