...
pub const AssociateVehicleFleetInput = struct {
    /// The ID of a fleet.
    fleet_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateVehicleFleetInput, options: Options) !AssociateVehicleFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateVehicleFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.AssociateVehicleFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateVehicleFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
