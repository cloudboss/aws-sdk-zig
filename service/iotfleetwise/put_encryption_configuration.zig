...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEncryptionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.PutEncryptionConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEncryptionConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutEncryptionConfigurationOutput, body, alloc);
}
