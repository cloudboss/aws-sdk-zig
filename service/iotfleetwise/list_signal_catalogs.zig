...
const SignalCatalogSummary = @import("signal_catalog_summary.zig").SignalCatalogSummary;

pub const ListSignalCatalogsInput = struct {
...
pub const ListSignalCatalogsOutput = struct {
    /// The token to retrieve the next set of results, or `null` if there are no
    /// more results.
    next_token: ?[]const u8 = null,

    /// A list of information about each signal catalog.
    summaries: ?[]const SignalCatalogSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSignalCatalogsInput, options: CallOptions) !ListSignalCatalogsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSignalCatalogsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.ListSignalCatalogs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSignalCatalogsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSignalCatalogsOutput, body, allocator);
}
