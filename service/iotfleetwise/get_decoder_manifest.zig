...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDecoderManifestInput, options: Options) !GetDecoderManifestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDecoderManifestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.GetDecoderManifest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDecoderManifestOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetDecoderManifestOutput, body, alloc);
}
