...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVehicleInput, options: Options) !CreateVehicleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVehicleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.CreateVehicle");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVehicleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateVehicleOutput, body, alloc);
}
