...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStateTemplateInput, options: Options) !DeleteStateTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteStateTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.DeleteStateTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteStateTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteStateTemplateOutput, body, alloc);
}
