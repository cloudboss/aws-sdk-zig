...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStateTemplateInput, options: Options) !GetStateTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStateTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.GetStateTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStateTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetStateTemplateOutput, body, alloc);
}
