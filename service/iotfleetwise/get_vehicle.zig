...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVehicleInput, options: Options) !GetVehicleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVehicleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.GetVehicle");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVehicleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetVehicleOutput, body, alloc);
}
