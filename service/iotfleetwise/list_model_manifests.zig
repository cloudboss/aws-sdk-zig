...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelManifestsInput, options: Options) !ListModelManifestsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelManifestsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.ListModelManifests");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelManifestsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListModelManifestsOutput, body, alloc);
}
