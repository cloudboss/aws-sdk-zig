...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDecoderManifestNetworkInterfacesInput, options: Options) !ListDecoderManifestNetworkInterfacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDecoderManifestNetworkInterfacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.ListDecoderManifestNetworkInterfaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDecoderManifestNetworkInterfacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDecoderManifestNetworkInterfacesOutput, body, alloc);
}
