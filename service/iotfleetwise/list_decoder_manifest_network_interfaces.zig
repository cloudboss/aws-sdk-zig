...
pub const ListDecoderManifestNetworkInterfacesInput = struct {
...
pub const ListDecoderManifestNetworkInterfacesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDecoderManifestNetworkInterfacesInput, options: CallOptions) !ListDecoderManifestNetworkInterfacesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDecoderManifestNetworkInterfacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.ListDecoderManifestNetworkInterfaces");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDecoderManifestNetworkInterfacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDecoderManifestNetworkInterfacesOutput, body, allocator);
}
