...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLoggingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.GetLoggingOptions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLoggingOptionsOutput {
    ...
    return aws.json.parseJsonObject(GetLoggingOptionsOutput, body, alloc);
}
