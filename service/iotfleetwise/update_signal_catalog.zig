...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSignalCatalogInput, options: Options) !UpdateSignalCatalogOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSignalCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.UpdateSignalCatalog");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSignalCatalogOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateSignalCatalogOutput, body, alloc);
}
