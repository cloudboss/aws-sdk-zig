...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVehiclesInput, options: Options) !ListVehiclesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVehiclesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.ListVehicles");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVehiclesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListVehiclesOutput, body, alloc);
}
