...
pub const ListModelManifestNodesInput = struct {
...
pub const ListModelManifestNodesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelManifestNodesInput, options: CallOptions) !ListModelManifestNodesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListModelManifestNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.ListModelManifestNodes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListModelManifestNodesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListModelManifestNodesOutput, body, allocator);
}
