...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetModelManifestInput, options: Options) !GetModelManifestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetModelManifestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.GetModelManifest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetModelManifestOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetModelManifestOutput, body, alloc);
}
