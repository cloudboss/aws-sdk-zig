...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDecoderManifestInput, options: Options) !CreateDecoderManifestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDecoderManifestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.CreateDecoderManifest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDecoderManifestOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateDecoderManifestOutput, body, alloc);
}
