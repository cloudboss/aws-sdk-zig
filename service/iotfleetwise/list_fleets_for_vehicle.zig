...
pub const ListFleetsForVehicleInput = struct {
...
pub const ListFleetsForVehicleOutput = struct {
    /// A list of fleet IDs that the vehicle is associated with.
    fleets: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFleetsForVehicleInput, options: CallOptions) !ListFleetsForVehicleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFleetsForVehicleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.ListFleetsForVehicle");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFleetsForVehicleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFleetsForVehicleOutput, body, allocator);
}
