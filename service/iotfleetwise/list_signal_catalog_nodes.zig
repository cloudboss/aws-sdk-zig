...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSignalCatalogNodesInput, options: Options) !ListSignalCatalogNodesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSignalCatalogNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.ListSignalCatalogNodes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSignalCatalogNodesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSignalCatalogNodesOutput, body, alloc);
}
