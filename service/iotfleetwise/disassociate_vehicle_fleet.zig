...
pub const DisassociateVehicleFleetInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateVehicleFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.DisassociateVehicleFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateVehicleFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
