...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegisterAccountStatusInput, options: Options) !GetRegisterAccountStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRegisterAccountStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.GetRegisterAccountStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRegisterAccountStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRegisterAccountStatusOutput, body, alloc);
}
