...
const VehicleStatus = @import("vehicle_status.zig").VehicleStatus;

pub const GetVehicleStatusInput = struct {
    ...
    /// next
    /// set of results, reissue the search request and include the returned token.
    /// When all results
...
pub const GetVehicleStatusOutput = struct {
    /// Lists information about the state of the vehicle with deployed campaigns.
    campaigns: ?[]const VehicleStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVehicleStatusInput, options: CallOptions) !GetVehicleStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetVehicleStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.GetVehicleStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetVehicleStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetVehicleStatusOutput, body, allocator);
}
