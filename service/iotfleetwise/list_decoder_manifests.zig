...
pub const ListDecoderManifestsInput = struct {
    /// When you set the `listResponseScope` parameter to `METADATA_ONLY`, the list
    /// response includes: decoder manifest name, Amazon Resource Name (ARN),
...
pub const ListDecoderManifestsOutput = struct {
    /// The token to retrieve the next set of results, or `null` if there are no
    /// more results.
    next_token: ?[]const u8 = null,

    /// A list of information about each decoder manifest.
    summaries: ?[]const DecoderManifestSummary = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDecoderManifestsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.ListDecoderManifests");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDecoderManifestsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDecoderManifestsOutput, body, allocator);
}
