...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCampaignInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.DeleteCampaign");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCampaignOutput {
    ...
    return aws.json.parseJsonObject(DeleteCampaignOutput, body, allocator);
}
