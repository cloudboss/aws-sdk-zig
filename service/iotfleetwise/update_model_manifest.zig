...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateModelManifestInput, options: Options) !UpdateModelManifestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateModelManifestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.UpdateModelManifest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateModelManifestOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateModelManifestOutput, body, alloc);
}
