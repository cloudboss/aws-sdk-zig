...
const CreateFleetInput = @import("create_fleet_request.zig").CreateFleetRequest;

const CreateFleetOutput = @import("create_fleet_response.zig").CreateFleetResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.CreateFleet");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFleetOutput {
    ...
    return aws.json.parseJsonObject(CreateFleetOutput, body, alloc);
}
