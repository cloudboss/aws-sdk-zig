...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVehicleInput, options: Options) !UpdateVehicleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVehicleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.UpdateVehicle");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVehicleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateVehicleOutput, body, alloc);
}
