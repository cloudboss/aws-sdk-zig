...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSignalCatalogInput, options: Options) !GetSignalCatalogOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSignalCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.GetSignalCatalog");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSignalCatalogOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetSignalCatalogOutput, body, alloc);
}
