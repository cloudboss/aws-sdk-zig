...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSignalCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.CreateSignalCatalog");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSignalCatalogOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateSignalCatalogOutput, body, alloc);
}
