...
pub const CreateSignalCatalogInput = struct {
...
pub const CreateSignalCatalogOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSignalCatalogInput, options: CallOptions) !CreateSignalCatalogOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSignalCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.CreateSignalCatalog");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSignalCatalogOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateSignalCatalogOutput, body, allocator);
}
