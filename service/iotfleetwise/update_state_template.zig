...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStateTemplateInput, options: Options) !UpdateStateTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStateTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IoTAutobahnControlPlane.UpdateStateTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStateTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateStateTemplateOutput, body, alloc);
}
