...
pub const ListVehiclesInFleetInput = struct {
    /// The ID of a fleet.
    fleet_id: []const u8,
...
pub const ListVehiclesInFleetOutput = struct {
    /// The token to retrieve the next set of results, or `null` if there are no
    /// more results.
    next_token: ?[]const u8 = null,

    /// A list of vehicles associated with the fleet.
    vehicles: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVehiclesInFleetInput, options: CallOptions) !ListVehiclesInFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVehiclesInFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "IoTAutobahnControlPlane.ListVehiclesInFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVehiclesInFleetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListVehiclesInFleetOutput, body, allocator);
}
