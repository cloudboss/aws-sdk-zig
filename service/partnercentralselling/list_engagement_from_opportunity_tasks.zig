...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEngagementFromOpportunityTasksInput, options: Options) !ListEngagementFromOpportunityTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEngagementFromOpportunityTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.ListEngagementFromOpportunityTasks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEngagementFromOpportunityTasksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEngagementFromOpportunityTasksOutput, body, alloc);
}
