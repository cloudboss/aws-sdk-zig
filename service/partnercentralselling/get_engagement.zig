...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEngagementInput, options: Options) !GetEngagementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEngagementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.GetEngagement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEngagementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEngagementOutput, body, alloc);
}
