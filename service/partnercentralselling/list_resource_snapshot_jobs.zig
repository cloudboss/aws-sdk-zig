...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceSnapshotJobsInput, options: Options) !ListResourceSnapshotJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceSnapshotJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.ListResourceSnapshotJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceSnapshotJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceSnapshotJobsOutput, body, alloc);
}
