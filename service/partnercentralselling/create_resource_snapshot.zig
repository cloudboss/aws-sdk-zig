...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResourceSnapshotInput, options: Options) !CreateResourceSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourceSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.CreateResourceSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourceSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateResourceSnapshotOutput, body, alloc);
}
