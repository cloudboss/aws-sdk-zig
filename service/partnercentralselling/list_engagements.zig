...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEngagementsInput, options: Options) !ListEngagementsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEngagementsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.ListEngagements");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEngagementsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEngagementsOutput, body, alloc);
}
