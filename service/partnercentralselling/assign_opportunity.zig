...
const AssignOpportunityOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssignOpportunityInput, options: Options) !AssignOpportunityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssignOpportunityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.AssignOpportunity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssignOpportunityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
