...
const RejectEngagementInvitationOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectEngagementInvitationInput, options: Options) !RejectEngagementInvitationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectEngagementInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.RejectEngagementInvitation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectEngagementInvitationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
