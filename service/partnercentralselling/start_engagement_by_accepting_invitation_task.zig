...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartEngagementByAcceptingInvitationTaskInput, options: Options) !StartEngagementByAcceptingInvitationTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartEngagementByAcceptingInvitationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.StartEngagementByAcceptingInvitationTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartEngagementByAcceptingInvitationTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartEngagementByAcceptingInvitationTaskOutput, body, alloc);
}
