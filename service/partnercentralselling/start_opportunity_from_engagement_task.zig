...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartOpportunityFromEngagementTaskInput, options: Options) !StartOpportunityFromEngagementTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartOpportunityFromEngagementTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.StartOpportunityFromEngagementTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartOpportunityFromEngagementTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartOpportunityFromEngagementTaskOutput, body, alloc);
}
