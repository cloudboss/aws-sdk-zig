...
const DisassociateOpportunityOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateOpportunityInput, options: Options) !DisassociateOpportunityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateOpportunityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.DisassociateOpportunity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateOpportunityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
