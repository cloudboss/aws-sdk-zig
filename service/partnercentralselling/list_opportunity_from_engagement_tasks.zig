...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOpportunityFromEngagementTasksInput, options: Options) !ListOpportunityFromEngagementTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOpportunityFromEngagementTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.ListOpportunityFromEngagementTasks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOpportunityFromEngagementTasksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOpportunityFromEngagementTasksOutput, body, alloc);
}
