...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceSnapshotJobInput, options: Options) !GetResourceSnapshotJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceSnapshotJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.GetResourceSnapshotJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceSnapshotJobOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetResourceSnapshotJobOutput, body, alloc);
}
