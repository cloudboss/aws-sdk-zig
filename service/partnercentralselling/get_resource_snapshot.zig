...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceSnapshotInput, options: CallOptions) !GetResourceSnapshotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResourceSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSPartnerCentralSelling.GetResourceSnapshot");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResourceSnapshotOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetResourceSnapshotOutput, body, allocator);
}
