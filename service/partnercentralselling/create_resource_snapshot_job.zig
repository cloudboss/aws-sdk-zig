...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResourceSnapshotJobInput, options: Options) !CreateResourceSnapshotJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourceSnapshotJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.CreateResourceSnapshotJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourceSnapshotJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateResourceSnapshotJobOutput, body, alloc);
}
