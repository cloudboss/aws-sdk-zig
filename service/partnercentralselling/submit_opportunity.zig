...
fn serializeRequest(alloc: std.mem.Allocator, input: SubmitOpportunityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.SubmitOpportunity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SubmitOpportunityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
