...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEngagementInvitationInput, options: Options) !CreateEngagementInvitationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEngagementInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.CreateEngagementInvitation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEngagementInvitationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateEngagementInvitationOutput, body, alloc);
}
