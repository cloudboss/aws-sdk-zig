...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEngagementResourceAssociationsInput, options: Options) !ListEngagementResourceAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEngagementResourceAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.ListEngagementResourceAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEngagementResourceAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEngagementResourceAssociationsOutput, body, alloc);
}
