...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAwsOpportunitySummaryInput, options: Options) !GetAwsOpportunitySummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAwsOpportunitySummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.GetAwsOpportunitySummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAwsOpportunitySummaryOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetAwsOpportunitySummaryOutput, body, alloc);
}
