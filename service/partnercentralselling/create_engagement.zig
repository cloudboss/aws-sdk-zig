...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEngagementInput, options: Options) !CreateEngagementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEngagementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.CreateEngagement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEngagementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEngagementOutput, body, alloc);
}
