...
const AssociateOpportunityOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateOpportunityInput, options: Options) !AssociateOpportunityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateOpportunityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.AssociateOpportunity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateOpportunityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
