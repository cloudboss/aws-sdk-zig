...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEngagementMembersInput, options: Options) !ListEngagementMembersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEngagementMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.ListEngagementMembers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEngagementMembersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEngagementMembersOutput, body, alloc);
}
