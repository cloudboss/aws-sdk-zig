...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOpportunitiesInput, options: Options) !ListOpportunitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOpportunitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.ListOpportunities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOpportunitiesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListOpportunitiesOutput, body, alloc);
}
