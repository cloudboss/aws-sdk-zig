...
const StartResourceSnapshotJobOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartResourceSnapshotJobInput, options: Options) !StartResourceSnapshotJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartResourceSnapshotJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPartnerCentralSelling.StartResourceSnapshotJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartResourceSnapshotJobOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
