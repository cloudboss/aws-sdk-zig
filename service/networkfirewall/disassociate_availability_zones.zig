...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateAvailabilityZonesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.DisassociateAvailabilityZones");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateAvailabilityZonesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateAvailabilityZonesOutput, body, alloc);
}
