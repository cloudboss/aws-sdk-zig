...
pub const CreateProxyRulesInput = struct {
...
pub const CreateProxyRulesOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateProxyRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.CreateProxyRules");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateProxyRulesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateProxyRulesOutput, body, allocator);
}
