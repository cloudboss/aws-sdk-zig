...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTLSInspectionConfigurationInput, options: Options) !DeleteTLSInspectionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTLSInspectionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.DeleteTLSInspectionConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTLSInspectionConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteTLSInspectionConfigurationOutput, body, alloc);
}
