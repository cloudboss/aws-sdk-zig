...
const DescribeProxyResource = @import("describe_proxy_resource.zig").DescribeProxyResource;

pub const DescribeProxyInput = struct {
...
pub const DescribeProxyOutput = struct {
    /// Proxy attached to a NAT gateway.
    proxy: ?DescribeProxyResource = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProxyInput, options: CallOptions) !DescribeProxyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeProxyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.DescribeProxy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeProxyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProxyOutput, body, allocator);
}
