...
pub const UpdateFirewallPolicyChangeProtectionInput = struct {
...
pub const UpdateFirewallPolicyChangeProtectionOutput = struct {
    /// The Amazon Resource Name (ARN) of the firewall.
    firewall_arn: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFirewallPolicyChangeProtectionInput, options: CallOptions) !UpdateFirewallPolicyChangeProtectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFirewallPolicyChangeProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.UpdateFirewallPolicyChangeProtection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFirewallPolicyChangeProtectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFirewallPolicyChangeProtectionOutput, body, allocator);
}
