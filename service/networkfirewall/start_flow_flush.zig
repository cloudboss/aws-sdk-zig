...
pub const StartFlowFlushInput = struct {
...
pub const StartFlowFlushOutput = struct {
    /// The Amazon Resource Name (ARN) of the firewall.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFlowFlushInput, options: CallOptions) !StartFlowFlushOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartFlowFlushInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.StartFlowFlush");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartFlowFlushOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartFlowFlushOutput, body, allocator);
}
