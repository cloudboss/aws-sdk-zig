...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFirewallAnalysisSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.UpdateFirewallAnalysisSettings");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFirewallAnalysisSettingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFirewallAnalysisSettingsOutput, body, alloc);
}
