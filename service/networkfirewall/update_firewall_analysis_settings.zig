...
pub const UpdateFirewallAnalysisSettingsInput = struct {
...
pub const UpdateFirewallAnalysisSettingsOutput = struct {
    /// An optional setting indicating the specific traffic analysis types to enable
    /// on the firewall.
    enabled_analysis_types: ?[]const EnabledAnalysisType = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFirewallAnalysisSettingsInput, options: CallOptions) !UpdateFirewallAnalysisSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFirewallAnalysisSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.UpdateFirewallAnalysisSettings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFirewallAnalysisSettingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFirewallAnalysisSettingsOutput, body, allocator);
}
