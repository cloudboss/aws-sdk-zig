...
const ProxyRuleGroupAttachment = @import("proxy_rule_group_attachment.zig").ProxyRuleGroupAttachment;
const ProxyConfiguration = @import("proxy_configuration.zig").ProxyConfiguration;

pub const AttachRuleGroupsToProxyConfigurationInput = struct {
    ...
    rule_groups: []const ProxyRuleGroupAttachment,
...
pub const AttachRuleGroupsToProxyConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachRuleGroupsToProxyConfigurationInput, options: CallOptions) !AttachRuleGroupsToProxyConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AttachRuleGroupsToProxyConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.AttachRuleGroupsToProxyConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AttachRuleGroupsToProxyConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AttachRuleGroupsToProxyConfigurationOutput, body, allocator);
}
