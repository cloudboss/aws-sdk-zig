...
const ProxyMetadata = @import("proxy_metadata.zig").ProxyMetadata;

pub const ListProxiesInput = struct {
...
pub const ListProxiesOutput = struct {
    /// When you request a list of objects with a `MaxResults` setting, if the
    /// number of objects that are still available
    /// for retrieval exceeds the maximum you requested, Network Firewall returns a
    /// `NextToken`
    ...
    proxies: ?[]const ProxyMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProxiesInput, options: CallOptions) !ListProxiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProxiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.ListProxies");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListProxiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProxiesOutput, body, allocator);
}
