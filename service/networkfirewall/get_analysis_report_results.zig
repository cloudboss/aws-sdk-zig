...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAnalysisReportResultsInput, options: Options) !GetAnalysisReportResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAnalysisReportResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.GetAnalysisReportResults");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAnalysisReportResultsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAnalysisReportResultsOutput, body, alloc);
}
