...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpcEndpointAssociationInput, options: Options) !DescribeVpcEndpointAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpcEndpointAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.DescribeVpcEndpointAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVpcEndpointAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeVpcEndpointAssociationOutput, body, alloc);
}
