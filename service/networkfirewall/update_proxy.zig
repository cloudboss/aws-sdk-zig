...
pub const UpdateProxyInput = struct {
    /// Listener properties for HTTP and HTTPS traffic to add.
    listener_properties_to_add: ?[]const ListenerPropertyRequest = null,

    /// Listener properties for HTTP and HTTPS traffic to remove.
    listener_properties_to_remove: ?[]const ListenerPropertyRequest = null,
    ...
    tls_intercept_properties: ?TlsInterceptPropertiesRequest = null,
    ...
    pub const json_field_names = .{
        .listener_properties_to_add = "ListenerPropertiesToAdd",
        .listener_properties_to_remove = "ListenerPropertiesToRemove",
...
pub const UpdateProxyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProxyInput, options: CallOptions) !UpdateProxyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateProxyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.UpdateProxy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateProxyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateProxyOutput, body, allocator);
}
