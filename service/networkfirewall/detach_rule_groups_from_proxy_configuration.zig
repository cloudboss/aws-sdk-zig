...
pub const DetachRuleGroupsFromProxyConfigurationInput = struct {
...
pub const DetachRuleGroupsFromProxyConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachRuleGroupsFromProxyConfigurationInput, options: CallOptions) !DetachRuleGroupsFromProxyConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DetachRuleGroupsFromProxyConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.DetachRuleGroupsFromProxyConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DetachRuleGroupsFromProxyConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetachRuleGroupsFromProxyConfigurationOutput, body, allocator);
}
