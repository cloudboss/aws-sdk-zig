...
pub const AssociateFirewallPolicyInput = struct {
...
pub const AssociateFirewallPolicyOutput = struct {
    /// The Amazon Resource Name (ARN) of the firewall.
    firewall_arn: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateFirewallPolicyInput, options: CallOptions) !AssociateFirewallPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateFirewallPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.AssociateFirewallPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateFirewallPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateFirewallPolicyOutput, body, allocator);
}
