...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFirewallPoliciesInput, options: Options) !ListFirewallPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFirewallPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.ListFirewallPolicies");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFirewallPoliciesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFirewallPoliciesOutput, body, alloc);
}
