...
pub const DescribeFirewallInput = struct {
...
pub const DescribeFirewallOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFirewallInput, options: CallOptions) !DescribeFirewallOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFirewallInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.DescribeFirewall");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFirewallOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFirewallOutput, body, allocator);
}
