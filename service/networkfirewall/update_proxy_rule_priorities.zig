...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProxyRulePrioritiesInput, options: Options) !UpdateProxyRulePrioritiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProxyRulePrioritiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.UpdateProxyRulePriorities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProxyRulePrioritiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateProxyRulePrioritiesOutput, body, alloc);
}
