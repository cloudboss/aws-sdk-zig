...
pub const UpdateFirewallDescriptionInput = struct {
...
pub const UpdateFirewallDescriptionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFirewallDescriptionInput, options: CallOptions) !UpdateFirewallDescriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFirewallDescriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.UpdateFirewallDescription");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFirewallDescriptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFirewallDescriptionOutput, body, allocator);
}
