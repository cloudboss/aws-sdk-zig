...
pub const UpdateFirewallPolicyInput = struct {
...
pub const UpdateFirewallPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFirewallPolicyInput, options: Options) !UpdateFirewallPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFirewallPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.UpdateFirewallPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFirewallPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFirewallPolicyOutput, body, alloc);
}
