...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFirewallPolicyInput, options: CallOptions) !UpdateFirewallPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFirewallPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.UpdateFirewallPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFirewallPolicyOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateFirewallPolicyOutput, body, allocator);
}
