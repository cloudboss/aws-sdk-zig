...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVpcEndpointAssociationInput, options: Options) !CreateVpcEndpointAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVpcEndpointAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.CreateVpcEndpointAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVpcEndpointAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateVpcEndpointAssociationOutput, body, alloc);
}
