...
pub const UpdateAvailabilityZoneChangeProtectionInput = struct {
...
pub const UpdateAvailabilityZoneChangeProtectionOutput = struct {
    /// A setting indicating whether the firewall is protected against changes to
    /// the subnet associations.
    /// Use this setting to protect against
    /// accidentally modifying the subnet associations for a firewall that is in
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAvailabilityZoneChangeProtectionInput, options: CallOptions) !UpdateAvailabilityZoneChangeProtectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAvailabilityZoneChangeProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.UpdateAvailabilityZoneChangeProtection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAvailabilityZoneChangeProtectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateAvailabilityZoneChangeProtectionOutput, body, allocator);
}
