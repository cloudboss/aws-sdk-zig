...
pub const DescribeProxyRuleGroupInput = struct {
...
pub const DescribeProxyRuleGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProxyRuleGroupInput, options: CallOptions) !DescribeProxyRuleGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeProxyRuleGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.DescribeProxyRuleGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeProxyRuleGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProxyRuleGroupOutput, body, allocator);
}
