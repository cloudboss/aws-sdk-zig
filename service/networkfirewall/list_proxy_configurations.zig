...
const ProxyConfigurationMetadata = @import("proxy_configuration_metadata.zig").ProxyConfigurationMetadata;

pub const ListProxyConfigurationsInput = struct {
...
pub const ListProxyConfigurationsOutput = struct {
    /// When you request a list of objects with a `MaxResults` setting, if the
    /// number of objects that are still available
    /// for retrieval exceeds the maximum you requested, Network Firewall returns a
    /// `NextToken`
    ...
    /// results and
    /// the number of proxy configurations that you have, this might not be the full
    /// list.
    proxy_configurations: ?[]const ProxyConfigurationMetadata = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .proxy_configurations = "ProxyConfigurations",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProxyConfigurationsInput, options: CallOptions) !ListProxyConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProxyConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.ListProxyConfigurations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListProxyConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProxyConfigurationsOutput, body, allocator);
}
