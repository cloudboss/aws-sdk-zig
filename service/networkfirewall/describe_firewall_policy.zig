...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFirewallPolicyInput, options: Options) !DescribeFirewallPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFirewallPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.DescribeFirewallPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFirewallPolicyOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeFirewallPolicyOutput, body, alloc);
}
