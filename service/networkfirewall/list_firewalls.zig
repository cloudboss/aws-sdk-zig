...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFirewallsInput, options: Options) !ListFirewallsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFirewallsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.ListFirewalls");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFirewallsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFirewallsOutput, body, alloc);
}
