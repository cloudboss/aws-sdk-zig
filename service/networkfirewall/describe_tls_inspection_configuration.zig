...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTLSInspectionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.DescribeTLSInspectionConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTLSInspectionConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeTLSInspectionConfigurationOutput, body, alloc);
}
