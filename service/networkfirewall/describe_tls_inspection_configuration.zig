...
pub const DescribeTLSInspectionConfigurationInput = struct {
...
pub const DescribeTLSInspectionConfigurationOutput = struct {
    ...
    tls_inspection_configuration: ?TLSInspectionConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTLSInspectionConfigurationInput, options: CallOptions) !DescribeTLSInspectionConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTLSInspectionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.DescribeTLSInspectionConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTLSInspectionConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeTLSInspectionConfigurationOutput, body, allocator);
}
