...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFirewallInput, options: Options) !CreateFirewallOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFirewallInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.CreateFirewall");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFirewallOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateFirewallOutput, body, alloc);
}
