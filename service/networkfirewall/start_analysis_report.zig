...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAnalysisReportInput, options: Options) !StartAnalysisReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartAnalysisReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "NetworkFirewall_20201112.StartAnalysisReport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartAnalysisReportOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StartAnalysisReportOutput, body, alloc);
}
