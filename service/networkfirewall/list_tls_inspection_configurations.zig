...
const TLSInspectionConfigurationMetadata = @import("tls_inspection_configuration_metadata.zig").TLSInspectionConfigurationMetadata;

pub const ListTLSInspectionConfigurationsInput = struct {
...
pub const ListTLSInspectionConfigurationsOutput = struct {
    /// When you request a list of objects with a `MaxResults` setting, if the
    /// number of objects that are still available
    /// for retrieval exceeds the maximum you requested, Network Firewall returns a
    /// `NextToken`
    ...
    tls_inspection_configurations: ?[]const TLSInspectionConfigurationMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTLSInspectionConfigurationsInput, options: CallOptions) !ListTLSInspectionConfigurationsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "network-firewall");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTLSInspectionConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.ListTLSInspectionConfigurations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTLSInspectionConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTLSInspectionConfigurationsOutput, body, allocator);
}
