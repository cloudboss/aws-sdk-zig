const MatchAttributes = @import("match_attributes.zig").MatchAttributes;

/// The inspection criteria and action for a single stateless rule. Network
/// Firewall inspects each packet for the specified matching
/// criteria. When a packet matches the criteria, Network Firewall performs the
/// rule's actions on
/// the packet.
pub const RuleDefinition = struct {
    ...
    ///
    /// * **aws:forward_to_sfe** - Discontinues
    /// stateless inspection of the packet and forwards it to the stateful rule
    /// engine for
    /// inspection.
    ///
    /// Additionally, you can specify a custom action.
    /// To
    /// do this, you define a custom action by name and type, then provide the name
    /// you've assigned
    /// to the action in this `Actions` setting. For information about the options,
    /// see
    /// CustomAction.
    ///
    ...
    /// `aws:pass` and the custom action with `[“aws:pass”,
    /// “MyMetricsAction”]`.
