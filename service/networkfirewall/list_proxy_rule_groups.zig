...
const ProxyRuleGroupMetadata = @import("proxy_rule_group_metadata.zig").ProxyRuleGroupMetadata;

pub const ListProxyRuleGroupsInput = struct {
...
pub const ListProxyRuleGroupsOutput = struct {
    /// When you request a list of objects with a `MaxResults` setting, if the
    /// number of objects that are still available
    /// for retrieval exceeds the maximum you requested, Network Firewall returns a
    /// `NextToken`
    ...
    /// results and
    /// the number of proxy rule groups that you have, this might not be the full
    /// list.
    proxy_rule_groups: ?[]const ProxyRuleGroupMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProxyRuleGroupsInput, options: CallOptions) !ListProxyRuleGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProxyRuleGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "NetworkFirewall_20201112.ListProxyRuleGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListProxyRuleGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProxyRuleGroupsOutput, body, allocator);
}
