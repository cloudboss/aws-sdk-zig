...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAttributeGroupInput, options: Options) !GetAttributeGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAttributeGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAttributeGroupOutput {
    var result: GetAttributeGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAttributeGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
