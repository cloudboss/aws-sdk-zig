...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAttributeGroupInput, options: Options) !CreateAttributeGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAttributeGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAttributeGroupOutput {
    var result: CreateAttributeGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAttributeGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
