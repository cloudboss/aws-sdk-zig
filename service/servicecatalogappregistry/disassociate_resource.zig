...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateResourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateResourceOutput {
    var result: DisassociateResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
