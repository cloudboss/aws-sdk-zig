...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssociatedResourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociatedResourcesOutput {
    var result: ListAssociatedResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssociatedResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
