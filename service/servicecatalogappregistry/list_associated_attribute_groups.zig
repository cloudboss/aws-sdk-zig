...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssociatedAttributeGroupsInput, options: Options) !ListAssociatedAttributeGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssociatedAttributeGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociatedAttributeGroupsOutput {
    var result: ListAssociatedAttributeGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssociatedAttributeGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
