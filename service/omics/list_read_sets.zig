...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReadSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/readsets");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReadSetsOutput {
    var result: ListReadSetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReadSetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
