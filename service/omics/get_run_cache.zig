...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRunCacheInput, options: Options) !GetRunCacheOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRunCacheInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRunCacheOutput {
    var result: GetRunCacheOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRunCacheOutput, body, alloc);
    }
    _ = status;
    _ = headers;
