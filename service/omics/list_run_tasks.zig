...
const TaskListItem = @import("task_list_item.zig").TaskListItem;

pub const ListRunTasksInput = struct {
    /// The run's ID.
    id: []const u8,
...
pub const ListRunTasksOutput = struct {
    /// A list of tasks.
    items: ?[]const TaskListItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRunTasksInput, options: Options) !ListRunTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRunTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/task");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRunTasksOutput {
    var result: ListRunTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRunTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
