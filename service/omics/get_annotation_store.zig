...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAnnotationStoreInput, options: Options) !GetAnnotationStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAnnotationStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAnnotationStoreOutput {
    var result: GetAnnotationStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAnnotationStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
