...
pub const StartReadSetActivationJobInput = struct {
    ...
    sources: []const StartReadSetActivationJobSourceItem,
...
pub const StartReadSetActivationJobOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartReadSetActivationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/activationjob");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartReadSetActivationJobOutput {
    var result: StartReadSetActivationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReadSetActivationJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
