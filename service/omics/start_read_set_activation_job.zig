...
fn serializeRequest(alloc: std.mem.Allocator, input: StartReadSetActivationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/activationjob");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartReadSetActivationJobOutput {
    var result: StartReadSetActivationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReadSetActivationJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
