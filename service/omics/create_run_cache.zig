...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRunCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cache_bucket_owner_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"cacheBucketOwnerId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"cacheS3Location\":");
    try aws.json.writeValue(@TypeOf(input.cache_s3_location), input.cache_s3_location, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRunCacheOutput {
    var result: CreateRunCacheOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRunCacheOutput, body, alloc);
    }
    _ = status;
    _ = headers;
