...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.run_group_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "runGroupId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRunsOutput {
    var result: ListRunsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRunsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
