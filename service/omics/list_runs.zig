...
pub const ListRunsOutput = struct {
    /// A list of runs.
    items: ?[]const RunListItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRunsInput, options: CallOptions) !ListRunsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRunsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRunsOutput {
    var result: ListRunsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRunsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
