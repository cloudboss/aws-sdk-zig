...
const ActivateReadSetFilter = @import("activate_read_set_filter.zig").ActivateReadSetFilter;
const ActivateReadSetJobItem = @import("activate_read_set_job_item.zig").ActivateReadSetJobItem;

pub const ListReadSetActivationJobsInput = struct {
    /// A filter to apply to the list.
    filter: ?ActivateReadSetFilter = null,
...
pub const ListReadSetActivationJobsOutput = struct {
    /// A list of jobs.
    activation_jobs: ?[]const ActivateReadSetJobItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReadSetActivationJobsInput, options: Options) !ListReadSetActivationJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReadSetActivationJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/activationjobs");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReadSetActivationJobsOutput {
    var result: ListReadSetActivationJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReadSetActivationJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
