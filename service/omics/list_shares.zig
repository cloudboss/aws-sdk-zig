...
pub const ListSharesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSharesInput, options: Options) !ListSharesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSharesInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.resource_owner), input.resource_owner, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSharesOutput {
    var result: ListSharesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSharesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
