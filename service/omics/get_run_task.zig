...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRunTaskInput, options: Options) !GetRunTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRunTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRunTaskOutput {
    var result: GetRunTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRunTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
