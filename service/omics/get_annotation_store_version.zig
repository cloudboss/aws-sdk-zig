...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAnnotationStoreVersionInput, options: Options) !GetAnnotationStoreVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAnnotationStoreVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAnnotationStoreVersionOutput {
    var result: GetAnnotationStoreVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAnnotationStoreVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
