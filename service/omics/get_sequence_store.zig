...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSequenceStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSequenceStoreOutput {
    var result: GetSequenceStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSequenceStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
