...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CompleteMultipartReadSetUploadInput, options: Options) !CompleteMultipartReadSetUploadOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CompleteMultipartReadSetUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/complete");
    ...
    try body_buf.appendSlice(alloc, "\"parts\":");
    try aws.json.writeValue(@TypeOf(input.parts), input.parts, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CompleteMultipartReadSetUploadOutput {
    var result: CompleteMultipartReadSetUploadOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CompleteMultipartReadSetUploadOutput, body, alloc);
    }
    _ = status;
    _ = headers;
