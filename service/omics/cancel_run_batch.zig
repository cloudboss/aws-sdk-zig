...
pub const CancelRunBatchInput = struct {
...
pub const CancelRunBatchOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelRunBatchInput, options: CallOptions) !CancelRunBatchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelRunBatchInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelRunBatchOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: CancelRunBatchOutput = .{};
