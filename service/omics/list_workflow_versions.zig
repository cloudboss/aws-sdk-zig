...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkflowVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkflowVersionsOutput {
    var result: ListWorkflowVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkflowVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
