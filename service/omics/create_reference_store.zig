...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReferenceStoreInput, options: Options) !CreateReferenceStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReferenceStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReferenceStoreOutput {
    var result: CreateReferenceStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateReferenceStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
