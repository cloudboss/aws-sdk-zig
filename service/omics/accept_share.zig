...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptShareInput, options: CallOptions) !AcceptShareOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptShareInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptShareOutput {
    var result: AcceptShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptShareOutput, body, allocator);
    }
    _ = status;
    _ = headers;
