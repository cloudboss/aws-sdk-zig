...
pub const UpdateVariantStoreInput = struct {
...
pub const UpdateVariantStoreOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVariantStoreInput, options: Options) !UpdateVariantStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVariantStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/variantStore/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVariantStoreOutput {
    var result: UpdateVariantStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVariantStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
