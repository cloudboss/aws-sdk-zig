...
fn serializeRequest(allocator: std.mem.Allocator, input: UploadReadSetPartInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(allocator, &query_buf, @tagName(input.part_source));
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UploadReadSetPartOutput {
    var result: UploadReadSetPartOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UploadReadSetPartOutput, body, allocator);
    }
    _ = status;
    _ = headers;
