...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRunGroupInput, options: Options) !GetRunGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRunGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRunGroupOutput {
    var result: GetRunGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRunGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
