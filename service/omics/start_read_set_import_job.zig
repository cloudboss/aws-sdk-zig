...
const StartReadSetImportJobSourceItem = @import("start_read_set_import_job_source_item.zig").StartReadSetImportJobSourceItem;
const ReadSetImportJobStatus = @import("read_set_import_job_status.zig").ReadSetImportJobStatus;

pub const StartReadSetImportJobInput = struct {
    ...
    sources: []const StartReadSetImportJobSourceItem,
...
pub const StartReadSetImportJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartReadSetImportJobInput, options: Options) !StartReadSetImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartReadSetImportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartReadSetImportJobOutput {
    var result: StartReadSetImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReadSetImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
