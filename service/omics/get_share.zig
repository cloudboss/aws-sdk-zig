...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetShareInput, options: Options) !GetShareOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetShareInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetShareOutput {
    var result: GetShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetShareOutput, body, alloc);
    }
    _ = status;
    _ = headers;
