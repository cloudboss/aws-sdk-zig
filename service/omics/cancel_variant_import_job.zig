...
pub const CancelVariantImportJobInput = struct {
    /// The job's ID.
    job_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelVariantImportJobInput, options: Options) !CancelVariantImportJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelVariantImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/import/variant/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelVariantImportJobOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: CancelVariantImportJobOutput = .{};
