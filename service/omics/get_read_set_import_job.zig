...
const ImportReadSetSourceItem = @import("import_read_set_source_item.zig").ImportReadSetSourceItem;
const ReadSetImportJobStatus = @import("read_set_import_job_status.zig").ReadSetImportJobStatus;

pub const GetReadSetImportJobInput = struct {
    /// The job's ID.
    id: []const u8,
...
pub const GetReadSetImportJobOutput = struct {
    ...
    sources: ?[]const ImportReadSetSourceItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReadSetImportJobInput, options: Options) !GetReadSetImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReadSetImportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReadSetImportJobOutput {
    var result: GetReadSetImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReadSetImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
