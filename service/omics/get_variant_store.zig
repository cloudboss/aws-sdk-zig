...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVariantStoreInput, options: Options) !GetVariantStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVariantStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVariantStoreOutput {
    var result: GetVariantStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVariantStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
