...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartReferenceImportJobInput, options: CallOptions) !StartReferenceImportJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartReferenceImportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartReferenceImportJobOutput {
    var result: StartReferenceImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReferenceImportJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
