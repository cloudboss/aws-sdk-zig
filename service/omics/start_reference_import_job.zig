...
const StartReferenceImportJobSourceItem = @import("start_reference_import_job_source_item.zig").StartReferenceImportJobSourceItem;
const ReferenceImportJobStatus = @import("reference_import_job_status.zig").ReferenceImportJobStatus;

pub const StartReferenceImportJobInput = struct {
    ...
    sources: []const StartReferenceImportJobSourceItem,
...
pub const StartReferenceImportJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartReferenceImportJobInput, options: Options) !StartReferenceImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartReferenceImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/referencestore/");
    try path_buf.appendSlice(alloc, input.reference_store_id);
    try path_buf.appendSlice(alloc, "/importjob");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartReferenceImportJobOutput {
    var result: StartReferenceImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReferenceImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
