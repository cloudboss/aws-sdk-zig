...
pub const CancelAnnotationImportJobInput = struct {
    /// The job's ID.
    job_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelAnnotationImportJobInput, options: CallOptions) !CancelAnnotationImportJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelAnnotationImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/import/annotation/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelAnnotationImportJobOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: CancelAnnotationImportJobOutput = .{};
