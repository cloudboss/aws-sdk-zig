...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReferenceMetadataInput, options: Options) !GetReferenceMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReferenceMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReferenceMetadataOutput {
    var result: GetReferenceMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReferenceMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
