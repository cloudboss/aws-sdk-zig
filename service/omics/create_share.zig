...
pub const CreateShareOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateShareInput, options: CallOptions) !CreateShareOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"principalSubscriber\":");
    try aws.json.writeValue(@TypeOf(input.principal_subscriber), input.principal_subscriber, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    has_prev = true;
    if (input.share_name) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"shareName\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateShareOutput {
    var result: CreateShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateShareOutput, body, allocator);
    }
    _ = status;
    _ = headers;
