...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"principalSubscriber\":");
    try aws.json.writeValue(@TypeOf(input.principal_subscriber), input.principal_subscriber, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.share_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"shareName\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateShareOutput {
    var result: CreateShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateShareOutput, body, alloc);
    }
    _ = status;
    _ = headers;
