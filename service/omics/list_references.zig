...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReferencesInput, options: Options) !ListReferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReferencesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReferencesOutput {
    var result: ListReferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReferencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
