...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVariantStoreInput, options: Options) !CreateVariantStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVariantStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"reference\":");
    try aws.json.writeValue(@TypeOf(input.reference), input.reference, alloc, &body_buf);
    has_prev = true;
    if (input.sse_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sseConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVariantStoreOutput {
    var result: CreateVariantStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVariantStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
