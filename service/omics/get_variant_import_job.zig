...
fn serializeRequest(allocator: std.mem.Allocator, input: GetVariantImportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetVariantImportJobOutput {
    var result: GetVariantImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVariantImportJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
