...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAnnotationStoreInput, options: Options) !CreateAnnotationStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAnnotationStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"storeFormat\":");
    try aws.json.writeValue(@TypeOf(input.store_format), input.store_format, alloc, &body_buf);
    has_prev = true;
    if (input.store_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"storeOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAnnotationStoreOutput {
    var result: CreateAnnotationStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAnnotationStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
