...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReadSetUploadPartsInput, options: Options) !ListReadSetUploadPartsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReadSetUploadPartsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/parts");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"partSource\":");
    try aws.json.writeValue(@TypeOf(input.part_source), input.part_source, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReadSetUploadPartsOutput {
    var result: ListReadSetUploadPartsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReadSetUploadPartsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
