...
const ActivateReadSetSourceItem = @import("activate_read_set_source_item.zig").ActivateReadSetSourceItem;
const ReadSetActivationJobStatus = @import("read_set_activation_job_status.zig").ReadSetActivationJobStatus;

pub const GetReadSetActivationJobInput = struct {
    /// The job's ID.
    id: []const u8,
...
pub const GetReadSetActivationJobOutput = struct {
    ...
    sources: ?[]const ActivateReadSetSourceItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReadSetActivationJobInput, options: CallOptions) !GetReadSetActivationJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetReadSetActivationJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetReadSetActivationJobOutput {
    var result: GetReadSetActivationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReadSetActivationJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
