...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReadSetActivationJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReadSetActivationJobOutput {
    var result: GetReadSetActivationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReadSetActivationJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
