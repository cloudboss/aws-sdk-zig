...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkflowVersionInput, options: Options) !GetWorkflowVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkflowVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkflowVersionOutput {
    var result: GetWorkflowVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkflowVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
