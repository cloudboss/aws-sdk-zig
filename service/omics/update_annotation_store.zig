...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAnnotationStoreInput, options: Options) !UpdateAnnotationStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAnnotationStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAnnotationStoreOutput {
    var result: UpdateAnnotationStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAnnotationStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
