...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAnnotationStoreVersionInput, options: Options) !UpdateAnnotationStoreVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAnnotationStoreVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAnnotationStoreVersionOutput {
    var result: UpdateAnnotationStoreVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAnnotationStoreVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
