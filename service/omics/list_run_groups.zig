...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRunGroupsInput, options: CallOptions) !ListRunGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRunGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRunGroupsOutput {
    var result: ListRunGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRunGroupsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
