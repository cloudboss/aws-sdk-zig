...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReadSetExportJobInput, options: Options) !GetReadSetExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReadSetExportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReadSetExportJobOutput {
    var result: GetReadSetExportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReadSetExportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
