...
pub const DeleteVariantStoreInput = struct {
...
pub const DeleteVariantStoreOutput = struct {
    /// The store's status.
    status: StoreStatus,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVariantStoreInput, options: Options) !DeleteVariantStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVariantStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVariantStoreOutput {
    var result: DeleteVariantStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteVariantStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
