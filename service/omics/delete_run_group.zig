...
pub const DeleteRunGroupInput = struct {
    /// The run group's ID.
    id: []const u8,
...
const DeleteRunGroupOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRunGroupInput, options: Options) !DeleteRunGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRunGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/runGroup/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRunGroupOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteRunGroupOutput = .{};
