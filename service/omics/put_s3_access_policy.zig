...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutS3AccessPolicyInput, options: Options) !PutS3AccessPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutS3AccessPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/s3accesspolicy/");
    try path_buf.appendSlice(alloc, input.s_3_access_point_arn);
    ...
    try body_buf.appendSlice(alloc, "\"s3AccessPolicy\":");
    try aws.json.writeValue(@TypeOf(input.s_3_access_policy), input.s_3_access_policy, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutS3AccessPolicyOutput {
    var result: PutS3AccessPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutS3AccessPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
