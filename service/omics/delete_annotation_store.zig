...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAnnotationStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAnnotationStoreOutput {
    var result: DeleteAnnotationStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAnnotationStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
