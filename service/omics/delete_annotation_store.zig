...
pub const DeleteAnnotationStoreOutput = struct {
    /// The store's status.
    status: StoreStatus,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAnnotationStoreInput, options: CallOptions) !DeleteAnnotationStoreOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAnnotationStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAnnotationStoreOutput {
    var result: DeleteAnnotationStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAnnotationStoreOutput, body, allocator);
    }
    _ = status;
    _ = headers;
