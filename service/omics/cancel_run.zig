...
pub const CancelRunInput = struct {
    /// The run's ID.
    id: []const u8,
...
pub const CancelRunOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelRunInput, options: CallOptions) !CancelRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelRunInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelRunOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: CancelRunOutput = .{};
