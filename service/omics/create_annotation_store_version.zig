...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAnnotationStoreVersionInput, options: Options) !CreateAnnotationStoreVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAnnotationStoreVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/annotationStore/");
    ...
    if (input.version_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"versionOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAnnotationStoreVersionOutput {
    var result: CreateAnnotationStoreVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAnnotationStoreVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
