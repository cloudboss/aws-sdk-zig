...
const RunCacheListItem = @import("run_cache_list_item.zig").RunCacheListItem;

pub const ListRunCachesInput = struct {
    /// The maximum number of results to return.
    max_results: ?i32 = null,

    /// Optional pagination token returned from a prior call to the `ListRunCaches`
    /// API operation.
...
pub const ListRunCachesOutput = struct {
    /// Details about each run cache in the response.
    items: ?[]const RunCacheListItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRunCachesInput, options: Options) !ListRunCachesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRunCachesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRunCachesOutput {
    var result: ListRunCachesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRunCachesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
