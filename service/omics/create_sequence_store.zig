...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSequenceStoreInput, options: Options) !CreateSequenceStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSequenceStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.e_tag_algorithm_family) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"eTagAlgorithmFamily\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSequenceStoreOutput {
    var result: CreateSequenceStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSequenceStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
