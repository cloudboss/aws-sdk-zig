...
pub const GetReferenceStoreInput = struct {
    /// The store's ID.
    id: []const u8,
...
pub const GetReferenceStoreOutput = struct {
    /// The store's ARN.
    arn: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReferenceStoreInput, options: CallOptions) !GetReferenceStoreOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetReferenceStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetReferenceStoreOutput {
    var result: GetReferenceStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReferenceStoreOutput, body, allocator);
    }
    _ = status;
    _ = headers;
