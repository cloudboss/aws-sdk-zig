...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReferenceStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReferenceStoreOutput {
    var result: GetReferenceStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReferenceStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
