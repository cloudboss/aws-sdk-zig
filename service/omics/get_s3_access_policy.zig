...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetS3AccessPolicyInput, options: Options) !GetS3AccessPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetS3AccessPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetS3AccessPolicyOutput {
    var result: GetS3AccessPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetS3AccessPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
