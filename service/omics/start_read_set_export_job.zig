...
const ExportReadSet = @import("export_read_set.zig").ExportReadSet;
const ReadSetExportJobStatus = @import("read_set_export_job_status.zig").ReadSetExportJobStatus;

pub const StartReadSetExportJobInput = struct {
...
pub const StartReadSetExportJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartReadSetExportJobInput, options: Options) !StartReadSetExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartReadSetExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/exportjob");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartReadSetExportJobOutput {
    var result: StartReadSetExportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReadSetExportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
