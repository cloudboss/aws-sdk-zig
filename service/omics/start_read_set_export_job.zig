...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartReadSetExportJobInput, options: CallOptions) !StartReadSetExportJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartReadSetExportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartReadSetExportJobOutput {
    var result: StartReadSetExportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReadSetExportJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
