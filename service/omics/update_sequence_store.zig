...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSequenceStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.fallback_location) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"fallbackLocation\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
    ...
    if (input.propagated_set_level_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"propagatedSetLevelTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.s_3_access_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"s3AccessConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSequenceStoreOutput {
    var result: UpdateSequenceStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSequenceStoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
