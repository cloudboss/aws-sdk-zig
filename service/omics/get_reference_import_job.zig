...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReferenceImportJobInput, options: Options) !GetReferenceImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReferenceImportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReferenceImportJobOutput {
    var result: GetReferenceImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReferenceImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
