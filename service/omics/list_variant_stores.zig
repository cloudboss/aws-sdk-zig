...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVariantStoresInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVariantStoresOutput {
    var result: ListVariantStoresOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVariantStoresOutput, body, alloc);
    }
    _ = status;
    _ = headers;
