...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteShareInput, options: CallOptions) !DeleteShareOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteShareInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteShareOutput {
    var result: DeleteShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteShareOutput, body, allocator);
    }
    _ = status;
    _ = headers;
