...
pub const DeleteShareInput = struct {
...
pub const DeleteShareOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteShareInput, options: Options) !DeleteShareOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteShareInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteShareOutput {
    var result: DeleteShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteShareOutput, body, alloc);
    }
    _ = status;
    _ = headers;
