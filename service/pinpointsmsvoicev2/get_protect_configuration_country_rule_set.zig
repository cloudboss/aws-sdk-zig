...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProtectConfigurationCountryRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.GetProtectConfigurationCountryRuleSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProtectConfigurationCountryRuleSetOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetProtectConfigurationCountryRuleSetOutput, body, alloc);
}
