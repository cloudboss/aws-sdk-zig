...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetDefaultMessageFeedbackEnabledInput, options: Options) !SetDefaultMessageFeedbackEnabledOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetDefaultMessageFeedbackEnabledInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.SetDefaultMessageFeedbackEnabled");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetDefaultMessageFeedbackEnabledOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SetDefaultMessageFeedbackEnabledOutput, body, alloc);
}
