...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetDefaultMessageTypeInput, options: Options) !SetDefaultMessageTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetDefaultMessageTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.SetDefaultMessageType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetDefaultMessageTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SetDefaultMessageTypeOutput, body, alloc);
}
