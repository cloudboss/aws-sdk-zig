...
pub const DiscardRegistrationVersionInput = struct {
...
pub const DiscardRegistrationVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DiscardRegistrationVersionInput, options: CallOptions) !DiscardRegistrationVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DiscardRegistrationVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PinpointSMSVoiceV2.DiscardRegistrationVersion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DiscardRegistrationVersionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DiscardRegistrationVersionOutput, body, allocator);
}
