...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePoolInput, options: Options) !DeletePoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeletePool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeletePoolOutput, body, alloc);
}
