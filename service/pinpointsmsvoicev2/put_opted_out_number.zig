...
fn serializeRequest(alloc: std.mem.Allocator, input: PutOptedOutNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.PutOptedOutNumber");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutOptedOutNumberOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutOptedOutNumberOutput, body, alloc);
}
