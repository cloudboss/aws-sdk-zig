...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateOriginationIdentityInput, options: Options) !AssociateOriginationIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateOriginationIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.AssociateOriginationIdentity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateOriginationIdentityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateOriginationIdentityOutput, body, alloc);
}
