...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSenderIdInput, options: Options) !UpdateSenderIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSenderIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.UpdateSenderId");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSenderIdOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSenderIdOutput, body, alloc);
}
