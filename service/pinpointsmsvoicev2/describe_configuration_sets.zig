...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConfigurationSetsInput, options: Options) !DescribeConfigurationSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConfigurationSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DescribeConfigurationSets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConfigurationSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConfigurationSetsOutput, body, alloc);
}
