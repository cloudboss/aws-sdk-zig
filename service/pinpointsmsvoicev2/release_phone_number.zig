...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReleasePhoneNumberInput, options: Options) !ReleasePhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReleasePhoneNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.ReleasePhoneNumber");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReleasePhoneNumberOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ReleasePhoneNumberOutput, body, alloc);
}
