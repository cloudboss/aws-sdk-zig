...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVerifiedDestinationNumberInput, options: Options) !DeleteVerifiedDestinationNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVerifiedDestinationNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteVerifiedDestinationNumber");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVerifiedDestinationNumberOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteVerifiedDestinationNumberOutput, body, alloc);
}
