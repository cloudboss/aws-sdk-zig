...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProtectConfigurationCountryRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.UpdateProtectConfigurationCountryRuleSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProtectConfigurationCountryRuleSetOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateProtectConfigurationCountryRuleSetOutput, body, alloc);
}
