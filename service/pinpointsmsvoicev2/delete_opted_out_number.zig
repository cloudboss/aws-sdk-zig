...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOptedOutNumberInput, options: Options) !DeleteOptedOutNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteOptedOutNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteOptedOutNumber");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteOptedOutNumberOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteOptedOutNumberOutput, body, alloc);
}
