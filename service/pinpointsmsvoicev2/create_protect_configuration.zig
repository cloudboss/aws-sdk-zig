...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProtectConfigurationInput, options: CallOptions) !CreateProtectConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateProtectConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PinpointSMSVoiceV2.CreateProtectConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateProtectConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateProtectConfigurationOutput, body, allocator);
}
