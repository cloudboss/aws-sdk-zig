...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVerifiedDestinationNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.CreateVerifiedDestinationNumber");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVerifiedDestinationNumberOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateVerifiedDestinationNumberOutput, body, alloc);
}
