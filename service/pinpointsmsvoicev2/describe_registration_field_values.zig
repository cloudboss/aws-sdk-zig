...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRegistrationFieldValuesInput, options: Options) !DescribeRegistrationFieldValuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRegistrationFieldValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DescribeRegistrationFieldValues");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRegistrationFieldValuesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRegistrationFieldValuesOutput, body, alloc);
}
