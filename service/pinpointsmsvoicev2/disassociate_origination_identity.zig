...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateOriginationIdentityInput, options: Options) !DisassociateOriginationIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateOriginationIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DisassociateOriginationIdentity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateOriginationIdentityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateOriginationIdentityOutput, body, alloc);
}
