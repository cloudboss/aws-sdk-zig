...
fn serializeRequest(alloc: std.mem.Allocator, input: PutProtectConfigurationRuleSetNumberOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.PutProtectConfigurationRuleSetNumberOverride");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutProtectConfigurationRuleSetNumberOverrideOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutProtectConfigurationRuleSetNumberOverrideOutput, body, alloc);
}
