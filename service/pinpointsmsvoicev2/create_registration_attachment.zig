...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRegistrationAttachmentInput, options: Options) !CreateRegistrationAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRegistrationAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.CreateRegistrationAttachment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRegistrationAttachmentOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateRegistrationAttachmentOutput, body, alloc);
}
