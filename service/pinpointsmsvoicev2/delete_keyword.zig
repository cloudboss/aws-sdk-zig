...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteKeywordInput, options: Options) !DeleteKeywordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteKeywordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteKeyword");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteKeywordOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteKeywordOutput, body, alloc);
}
