...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRegistrationFieldValueInput, options: Options) !DeleteRegistrationFieldValueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRegistrationFieldValueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteRegistrationFieldValue");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRegistrationFieldValueOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteRegistrationFieldValueOutput, body, alloc);
}
