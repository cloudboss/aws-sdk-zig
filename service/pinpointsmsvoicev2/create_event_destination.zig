...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEventDestinationInput, options: Options) !CreateEventDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEventDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.CreateEventDestination");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEventDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEventDestinationOutput, body, alloc);
}
