...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAccountDefaultProtectConfigurationInput, options: Options) !DeleteAccountDefaultProtectConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAccountDefaultProtectConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteAccountDefaultProtectConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAccountDefaultProtectConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteAccountDefaultProtectConfigurationOutput, body, alloc);
}
