...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePoolInput, options: Options) !UpdatePoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.UpdatePool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdatePoolOutput, body, alloc);
}
