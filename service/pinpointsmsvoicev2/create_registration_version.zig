...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRegistrationVersionInput, options: Options) !CreateRegistrationVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRegistrationVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.CreateRegistrationVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRegistrationVersionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateRegistrationVersionOutput, body, alloc);
}
