...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRegistrationFieldDefinitionsInput, options: Options) !DescribeRegistrationFieldDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRegistrationFieldDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DescribeRegistrationFieldDefinitions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRegistrationFieldDefinitionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeRegistrationFieldDefinitionsOutput, body, alloc);
}
