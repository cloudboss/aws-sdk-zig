...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePhoneNumbersInput, options: Options) !DescribePhoneNumbersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePhoneNumbersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DescribePhoneNumbers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePhoneNumbersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePhoneNumbersOutput, body, alloc);
}
