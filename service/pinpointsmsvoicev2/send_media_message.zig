...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendMediaMessageInput, options: Options) !SendMediaMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendMediaMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.SendMediaMessage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendMediaMessageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SendMediaMessageOutput, body, alloc);
}
