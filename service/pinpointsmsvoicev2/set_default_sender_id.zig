...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetDefaultSenderIdInput, options: Options) !SetDefaultSenderIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetDefaultSenderIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.SetDefaultSenderId");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetDefaultSenderIdOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SetDefaultSenderIdOutput, body, alloc);
}
