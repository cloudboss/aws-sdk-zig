...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRegistrationAssociationInput, options: Options) !CreateRegistrationAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRegistrationAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.CreateRegistrationAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRegistrationAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateRegistrationAssociationOutput, body, alloc);
}
