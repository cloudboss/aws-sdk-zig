...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOptOutListsInput, options: Options) !DescribeOptOutListsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOptOutListsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DescribeOptOutLists");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOptOutListsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeOptOutListsOutput, body, alloc);
}
