...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CarrierLookupInput, options: Options) !CarrierLookupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CarrierLookupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.CarrierLookup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CarrierLookupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CarrierLookupOutput, body, alloc);
}
