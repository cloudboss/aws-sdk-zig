...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDefaultMessageTypeInput, options: Options) !DeleteDefaultMessageTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDefaultMessageTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteDefaultMessageType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDefaultMessageTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteDefaultMessageTypeOutput, body, alloc);
}
