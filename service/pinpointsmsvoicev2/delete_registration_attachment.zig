...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRegistrationAttachmentInput, options: Options) !DeleteRegistrationAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRegistrationAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteRegistrationAttachment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRegistrationAttachmentOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteRegistrationAttachmentOutput, body, alloc);
}
