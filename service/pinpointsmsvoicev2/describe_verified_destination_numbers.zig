...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVerifiedDestinationNumbersInput, options: Options) !DescribeVerifiedDestinationNumbersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVerifiedDestinationNumbersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DescribeVerifiedDestinationNumbers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVerifiedDestinationNumbersOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeVerifiedDestinationNumbersOutput, body, alloc);
}
