...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEventDestinationInput, options: Options) !DeleteEventDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteEventDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteEventDestination");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteEventDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteEventDestinationOutput, body, alloc);
}
