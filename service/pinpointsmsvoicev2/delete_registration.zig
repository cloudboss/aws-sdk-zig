...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteRegistration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRegistrationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteRegistrationOutput, body, alloc);
}
