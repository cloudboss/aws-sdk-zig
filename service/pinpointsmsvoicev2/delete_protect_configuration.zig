...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteProtectConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PinpointSMSVoiceV2.DeleteProtectConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteProtectConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteProtectConfigurationOutput, body, alloc);
}
