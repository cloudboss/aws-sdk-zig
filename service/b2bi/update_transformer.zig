...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTransformerInput, options: Options) !UpdateTransformerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTransformerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.UpdateTransformer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTransformerOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateTransformerOutput, body, alloc);
}
