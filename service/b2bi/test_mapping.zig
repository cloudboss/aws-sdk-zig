...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestMappingInput, options: Options) !TestMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.TestMapping");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestMappingOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(TestMappingOutput, body, alloc);
}
