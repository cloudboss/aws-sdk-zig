...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTransformerJobInput, options: Options) !StartTransformerJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartTransformerJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.StartTransformerJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartTransformerJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartTransformerJobOutput, body, alloc);
}
