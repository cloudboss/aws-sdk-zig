...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePartnershipInput, options: Options) !CreatePartnershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePartnershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.CreatePartnership");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePartnershipOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreatePartnershipOutput, body, alloc);
}
