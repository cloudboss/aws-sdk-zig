...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestConversionInput, options: Options) !TestConversionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestConversionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.TestConversion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestConversionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(TestConversionOutput, body, alloc);
}
