...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePartnershipInput, options: Options) !UpdatePartnershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePartnershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.UpdatePartnership");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePartnershipOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdatePartnershipOutput, body, alloc);
}
