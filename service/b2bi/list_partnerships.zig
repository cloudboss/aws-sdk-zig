...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPartnershipsInput, options: Options) !ListPartnershipsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPartnershipsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.ListPartnerships");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPartnershipsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPartnershipsOutput, body, alloc);
}
