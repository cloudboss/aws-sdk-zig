...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCapabilitiesInput, options: Options) !ListCapabilitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCapabilitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.ListCapabilities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCapabilitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCapabilitiesOutput, body, alloc);
}
