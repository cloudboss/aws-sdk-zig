...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPartnershipInput, options: Options) !GetPartnershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPartnershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.GetPartnership");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPartnershipOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetPartnershipOutput, body, alloc);
}
