...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestParsingInput, options: Options) !TestParsingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestParsingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.TestParsing");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestParsingOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(TestParsingOutput, body, alloc);
}
