...
pub const ListProfilesOutput = struct {
    ...
    profiles: ?[]const ProfileSummary = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.ListProfiles");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProfilesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListProfilesOutput, body, alloc);
}
