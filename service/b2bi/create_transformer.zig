...
pub const CreateTransformerInput = struct {
    ...
    /// Specify the `InputConversion` object, which contains the format options for
...
pub const CreateTransformerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTransformerInput, options: CallOptions) !CreateTransformerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTransformerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "B2BI.CreateTransformer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTransformerOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateTransformerOutput, body, allocator);
}
