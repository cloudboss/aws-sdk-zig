...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStarterMappingTemplateInput, options: Options) !CreateStarterMappingTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStarterMappingTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "B2BI.CreateStarterMappingTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStarterMappingTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateStarterMappingTemplateOutput, body, alloc);
}
