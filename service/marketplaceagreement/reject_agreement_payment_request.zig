...
pub const RejectAgreementPaymentRequestInput = struct {
...
pub const RejectAgreementPaymentRequestOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectAgreementPaymentRequestInput, options: CallOptions) !RejectAgreementPaymentRequestOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RejectAgreementPaymentRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMPCommerceService_v20200301.RejectAgreementPaymentRequest");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RejectAgreementPaymentRequestOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RejectAgreementPaymentRequestOutput, body, allocator);
}
