...
pub const CancelAgreementPaymentRequestInput = struct {
...
pub const CancelAgreementPaymentRequestOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelAgreementPaymentRequestInput, options: CallOptions) !CancelAgreementPaymentRequestOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "awsmpcommerceservice_v20200301");
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelAgreementPaymentRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMPCommerceService_v20200301.CancelAgreementPaymentRequest");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelAgreementPaymentRequestOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelAgreementPaymentRequestOutput, body, allocator);
}
