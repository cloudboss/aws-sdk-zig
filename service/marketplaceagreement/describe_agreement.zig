...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAgreementInput, options: Options) !DescribeAgreementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAgreementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMPCommerceService_v20200301.DescribeAgreement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAgreementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAgreementOutput, body, alloc);
}
