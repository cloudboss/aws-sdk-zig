...
pub const UpdatePurchaseOrdersInput = struct {
    /// Contains information about purchase order associations.
    purchase_orders: []const PurchaseOrder,
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePurchaseOrdersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMPCommerceService_v20200301.UpdatePurchaseOrders");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePurchaseOrdersOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
