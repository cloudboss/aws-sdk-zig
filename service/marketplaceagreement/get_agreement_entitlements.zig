...
const AgreementEntitlement = @import("agreement_entitlement.zig").AgreementEntitlement;

pub const GetAgreementEntitlementsInput = struct {
...
pub const GetAgreementEntitlementsOutput = struct {
    /// A list of agreement entitlements which are part of the latest agreement.
    agreement_entitlements: ?[]const AgreementEntitlement = null,
    ...
    pub const json_field_names = .{
        .agreement_entitlements = "agreementEntitlements",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAgreementEntitlementsInput, options: CallOptions) !GetAgreementEntitlementsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAgreementEntitlementsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMPCommerceService_v20200301.GetAgreementEntitlements");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAgreementEntitlementsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAgreementEntitlementsOutput, body, allocator);
}
