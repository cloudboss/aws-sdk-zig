...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchAgreementsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMPCommerceService_v20200301.SearchAgreements");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchAgreementsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchAgreementsOutput, body, alloc);
}
