...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBillingViewInput, options: Options) !UpdateBillingViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBillingViewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBilling.UpdateBillingView");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBillingViewOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateBillingViewOutput, body, alloc);
}
