...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBillingViewInput, options: Options) !GetBillingViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBillingViewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBilling.GetBillingView");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBillingViewOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetBillingViewOutput, body, alloc);
}
