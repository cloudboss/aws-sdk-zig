...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBillingViewsInput, options: Options) !ListBillingViewsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBillingViewsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBilling.ListBillingViews");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBillingViewsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListBillingViewsOutput, body, alloc);
}
