...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBillingViewInput, options: Options) !CreateBillingViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBillingViewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBilling.CreateBillingView");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBillingViewOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateBillingViewOutput, body, alloc);
}
