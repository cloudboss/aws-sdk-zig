...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnrollmentStatusesInput, options: Options) !ListEnrollmentStatusesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnrollmentStatusesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CostOptimizationHubService.ListEnrollmentStatuses");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnrollmentStatusesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEnrollmentStatusesOutput, body, alloc);
}
