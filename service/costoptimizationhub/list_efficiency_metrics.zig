...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEfficiencyMetricsInput, options: Options) !ListEfficiencyMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEfficiencyMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CostOptimizationHubService.ListEfficiencyMetrics");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEfficiencyMetricsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEfficiencyMetricsOutput, body, alloc);
}
