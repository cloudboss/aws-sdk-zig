...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecommendationSummariesInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("costoptimizationhub", "Cost Optimization Hub", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CostOptimizationHubService.ListRecommendationSummaries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecommendationSummariesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRecommendationSummariesOutput, body, alloc);
}
