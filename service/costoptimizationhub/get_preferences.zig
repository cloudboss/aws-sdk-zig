...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPreferencesInput, options: Options) !GetPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPreferencesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CostOptimizationHubService.GetPreferences");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPreferencesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPreferencesOutput, body, alloc);
}
