...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SuggestInput, options: Options) !SuggestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SuggestInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "format=sdk&pretty=true");
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "q=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.query);
    query_has_prev = true;
    if (input.size) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "size=");
        {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "suggester=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.suggester);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SuggestOutput {
    var result: SuggestOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SuggestOutput, body, alloc);
    }
    _ = status;
    _ = headers;
