...
pub const StopReplicationToReplicaInput = struct {
...
pub const StopReplicationToReplicaOutput = struct {
    ...
    pub fn deinit(self: *StopReplicationToReplicaOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: StopReplicationToReplicaInput, options: Options) !StopReplicationToReplicaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopReplicationToReplicaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "secretsmanager.StopReplicationToReplica");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopReplicationToReplicaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopReplicationToReplicaOutput, body, alloc);
}
