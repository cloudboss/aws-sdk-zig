...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopReplicationToReplicaInput, options: CallOptions) !StopReplicationToReplicaOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopReplicationToReplicaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "secretsmanager.StopReplicationToReplica");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopReplicationToReplicaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopReplicationToReplicaOutput, body, allocator);
}
