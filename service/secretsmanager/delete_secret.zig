...
pub const DeleteSecretOutput = struct {
    ...
    pub fn deinit(self: *DeleteSecretOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DeleteSecretInput, options: Options) !DeleteSecretOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSecretInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "secretsmanager.DeleteSecret");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSecretOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteSecretOutput, body, alloc);
}
