...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetSecretValueInput, options: CallOptions) !BatchGetSecretValueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchGetSecretValueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "secretsmanager.BatchGetSecretValue");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchGetSecretValueOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetSecretValueOutput, body, allocator);
}
