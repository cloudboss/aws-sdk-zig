...
pub const ListSecretVersionIdsOutput = struct {
    ...
    pub fn deinit(self: *ListSecretVersionIdsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListSecretVersionIdsInput, options: Options) !ListSecretVersionIdsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecretVersionIdsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "secretsmanager.ListSecretVersionIds");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecretVersionIdsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSecretVersionIdsOutput, body, alloc);
}
