...
const SecretVersionsListEntry = @import("secret_versions_list_entry.zig").SecretVersionsListEntry;

pub const ListSecretVersionIdsInput = struct {
    /// Specifies whether to include versions of secrets that don't have any staging
    /// labels
    /// attached to them. Versions without staging labels are considered deprecated
    /// and are
    /// subject to deletion by Secrets Manager. By default, versions without staging
    /// labels aren't
    /// included.
...
pub const ListSecretVersionIdsOutput = struct {
    ...
    versions: ?[]const SecretVersionsListEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecretVersionIdsInput, options: CallOptions) !ListSecretVersionIdsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "secretsmanager");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSecretVersionIdsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "secretsmanager.ListSecretVersionIds");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSecretVersionIdsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSecretVersionIdsOutput, body, allocator);
}
