...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSecretInput, options: CallOptions) !UpdateSecretOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSecretInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "secretsmanager.UpdateSecret");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateSecretOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSecretOutput, body, allocator);
}
