...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSecretValueInput, options: CallOptions) !PutSecretValueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutSecretValueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "secretsmanager.PutSecretValue");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutSecretValueOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutSecretValueOutput, body, allocator);
}
