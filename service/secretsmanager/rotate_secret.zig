...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RotateSecretInput, options: CallOptions) !RotateSecretOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RotateSecretInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "secretsmanager.RotateSecret");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RotateSecretOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RotateSecretOutput, body, allocator);
}
