...
pub const CancelRotateSecretOutput = struct {
    ...
    pub fn deinit(self: *CancelRotateSecretOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: CancelRotateSecretInput, options: Options) !CancelRotateSecretOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelRotateSecretInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "secretsmanager.CancelRotateSecret");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelRotateSecretOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelRotateSecretOutput, body, alloc);
}
