...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListKeyspacesInput, options: Options) !ListKeyspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListKeyspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KeyspacesService.ListKeyspaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListKeyspacesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListKeyspacesOutput, body, alloc);
}
