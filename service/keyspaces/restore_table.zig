...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreTableInput, options: Options) !RestoreTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KeyspacesService.RestoreTable");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreTableOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(RestoreTableOutput, body, alloc);
}
