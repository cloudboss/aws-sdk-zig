...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKeyspaceInput, options: Options) !CreateKeyspaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKeyspaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KeyspacesService.CreateKeyspace");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKeyspaceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateKeyspaceOutput, body, alloc);
}
