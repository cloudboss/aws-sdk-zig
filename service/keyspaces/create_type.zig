...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTypeInput, options: Options) !CreateTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KeyspacesService.CreateType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTypeOutput, body, alloc);
}
