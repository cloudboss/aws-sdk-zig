...
pub const GetTableAutoScalingSettingsInput = struct {
...
pub const GetTableAutoScalingSettingsOutput = struct {
    ...
    replica_specifications: ?[]const ReplicaAutoScalingSpecification = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableAutoScalingSettingsInput, options: CallOptions) !GetTableAutoScalingSettingsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "cassandra");
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTableAutoScalingSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "KeyspacesService.GetTableAutoScalingSettings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTableAutoScalingSettingsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetTableAutoScalingSettingsOutput, body, allocator);
}
