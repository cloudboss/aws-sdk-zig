...
pub const GetKeyspaceInput = struct {
...
pub const GetKeyspaceOutput = struct {
    ...
    pub const json_field_names = .{
        .keyspace_name = "keyspaceName",
        .replication_group_statuses = "replicationGroupStatuses",
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetKeyspaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "KeyspacesService.GetKeyspace");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetKeyspaceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetKeyspaceOutput, body, allocator);
}
