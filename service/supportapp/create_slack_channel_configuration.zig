...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSlackChannelConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.channel_role_arn), input.channel_role_arn, allocator, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.notify_on_case_severity), input.notify_on_case_severity, allocator, &body_buf);
    has_prev = true;
