...
pub const CreateSlackChannelConfigurationInput = struct {
    ...
    ///
    /// If you specify `high` or `all`, you must specify `true`
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSlackChannelConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/control/create-slack-channel-configuration";
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"channelRoleArn\":");
    try aws.json.writeValue(@TypeOf(input.channel_role_arn), input.channel_role_arn, alloc, &body_buf);
    has_prev = true;
    if (input.notify_on_add_correspondence_to_case) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"notifyOnAddCorrespondenceToCase\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"notifyOnCaseSeverity\":");
    try aws.json.writeValue(@TypeOf(input.notify_on_case_severity), input.notify_on_case_severity, alloc, &body_buf);
    has_prev = true;
    if (input.notify_on_create_or_reopen_case) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"notifyOnCreateOrReopenCase\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.notify_on_resolve_case) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"notifyOnResolveCase\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSlackChannelConfigurationOutput {
    ...
    _ = status;
    _ = headers;
    const result: CreateSlackChannelConfigurationOutput = .{};
