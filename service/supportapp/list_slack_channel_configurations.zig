...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSlackChannelConfigurationsInput, options: Options) !ListSlackChannelConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSlackChannelConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSlackChannelConfigurationsOutput {
    var result: ListSlackChannelConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSlackChannelConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
