...
const ListQuickSetupTypesInput = struct {};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQuickSetupTypesInput, options: Options) !ListQuickSetupTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQuickSetupTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQuickSetupTypesOutput {
    var result: ListQuickSetupTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQuickSetupTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
