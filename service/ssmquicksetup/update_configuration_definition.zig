...
const UpdateConfigurationDefinitionOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfigurationDefinitionInput, options: Options) !UpdateConfigurationDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfigurationDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/configurationDefinition/");
    ...
    if (input.local_deployment_administration_role_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"LocalDeploymentAdministrationRoleArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.local_deployment_execution_role_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LocalDeploymentExecutionRoleName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.type_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TypeVersion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfigurationDefinitionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
