...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConfigurationManagerInput, options: Options) !CreateConfigurationManagerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConfigurationManagerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ConfigurationDefinitions\":");
    try aws.json.writeValue(@TypeOf(input.configuration_definitions), input.configuration_definitions, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConfigurationManagerOutput {
    var result: CreateConfigurationManagerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConfigurationManagerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
