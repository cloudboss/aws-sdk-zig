...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfigurationManagerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfigurationManagerOutput {
    var result: GetConfigurationManagerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfigurationManagerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
