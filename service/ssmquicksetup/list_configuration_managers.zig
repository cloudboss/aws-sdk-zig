...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConfigurationManagersInput, options: Options) !ListConfigurationManagersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConfigurationManagersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConfigurationManagersOutput {
    var result: ListConfigurationManagersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConfigurationManagersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
