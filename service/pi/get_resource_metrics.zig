...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceMetricsInput, options: Options) !GetResourceMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PerformanceInsightsv20180227.GetResourceMetrics");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceMetricsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResourceMetricsOutput, body, alloc);
}
