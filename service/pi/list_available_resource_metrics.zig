...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAvailableResourceMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PerformanceInsightsv20180227.ListAvailableResourceMetrics");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAvailableResourceMetricsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAvailableResourceMetricsOutput, body, allocator);
}
