...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAvailableResourceDimensionsInput, options: CallOptions) !ListAvailableResourceDimensionsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "pi");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAvailableResourceDimensionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PerformanceInsightsv20180227.ListAvailableResourceDimensions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAvailableResourceDimensionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAvailableResourceDimensionsOutput, body, allocator);
}
