...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDimensionKeysInput, options: Options) !DescribeDimensionKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDimensionKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PerformanceInsightsv20180227.DescribeDimensionKeys");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDimensionKeysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDimensionKeysOutput, body, alloc);
}
