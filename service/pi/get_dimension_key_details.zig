...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDimensionKeyDetailsInput, options: Options) !GetDimensionKeyDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDimensionKeyDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PerformanceInsightsv20180227.GetDimensionKeyDetails");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDimensionKeyDetailsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDimensionKeyDetailsOutput, body, alloc);
}
