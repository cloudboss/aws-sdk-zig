...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePerformanceAnalysisReportInput, options: Options) !CreatePerformanceAnalysisReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePerformanceAnalysisReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PerformanceInsightsv20180227.CreatePerformanceAnalysisReport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePerformanceAnalysisReportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePerformanceAnalysisReportOutput, body, alloc);
}
