...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPerformanceAnalysisReportsInput, options: CallOptions) !ListPerformanceAnalysisReportsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPerformanceAnalysisReportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PerformanceInsightsv20180227.ListPerformanceAnalysisReports");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPerformanceAnalysisReportsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPerformanceAnalysisReportsOutput, body, allocator);
}
