...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConnectorRegistrationInput, options: Options) !UpdateConnectorRegistrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConnectorRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.connector_label), input.connector_label, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConnectorRegistrationOutput {
    var result: UpdateConnectorRegistrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConnectorRegistrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
