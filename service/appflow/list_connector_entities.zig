...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConnectorEntitiesInput, options: Options) !ListConnectorEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConnectorEntitiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConnectorEntitiesOutput {
    var result: ListConnectorEntitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConnectorEntitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
