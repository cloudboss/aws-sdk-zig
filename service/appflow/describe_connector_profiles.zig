...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConnectorProfilesInput, options: Options) !DescribeConnectorProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConnectorProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connector_profile_names) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"connectorProfileNames\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConnectorProfilesOutput {
    var result: DescribeConnectorProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeConnectorProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
