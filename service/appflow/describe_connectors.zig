...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConnectorsInput, options: Options) !DescribeConnectorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connector_types) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"connectorTypes\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConnectorsOutput {
    var result: DescribeConnectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeConnectorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
