...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConnectorProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"connectorType\":");
    try aws.json.writeValue(@TypeOf(input.connector_type), input.connector_type, alloc, &body_buf);
    has_prev = true;
    if (input.kms_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"kmsArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConnectorProfileOutput {
    var result: CreateConnectorProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConnectorProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
