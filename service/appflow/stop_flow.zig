...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopFlowInput, options: CallOptions) !StopFlowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopFlowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopFlowOutput {
    var result: StopFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopFlowOutput, body, allocator);
    }
    _ = status;
    _ = headers;
