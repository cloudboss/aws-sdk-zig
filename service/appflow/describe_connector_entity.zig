...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConnectorEntityInput, options: Options) !DescribeConnectorEntityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConnectorEntityInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.connector_entity_name), input.connector_entity_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConnectorEntityOutput {
    var result: DescribeConnectorEntityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeConnectorEntityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
