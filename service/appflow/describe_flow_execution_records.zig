...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFlowExecutionRecordsInput, options: Options) !DescribeFlowExecutionRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFlowExecutionRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"flowName\":");
    try aws.json.writeValue(@TypeOf(input.flow_name), input.flow_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFlowExecutionRecordsOutput {
    var result: DescribeFlowExecutionRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFlowExecutionRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
