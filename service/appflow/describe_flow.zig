...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFlowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFlowOutput {
    var result: DescribeFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
