...
pub const CancelFlowExecutionsInput = struct {
...
pub const CancelFlowExecutionsOutput = struct {
    ...
    invalid_executions: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelFlowExecutionsInput, options: CallOptions) !CancelFlowExecutionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelFlowExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/cancel-flow-executions";
    ...
    if (input.execution_ids) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"executionIds\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelFlowExecutionsOutput {
    var result: CancelFlowExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelFlowExecutionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
