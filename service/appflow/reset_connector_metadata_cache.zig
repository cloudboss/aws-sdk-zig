...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetConnectorMetadataCacheInput, options: Options) !ResetConnectorMetadataCacheOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetConnectorMetadataCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.api_version) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"apiVersion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.connector_entity_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"connectorEntityName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.entities_path) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"entitiesPath\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetConnectorMetadataCacheOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
