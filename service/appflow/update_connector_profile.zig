...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConnectorProfileInput, options: Options) !UpdateConnectorProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConnectorProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"connectionMode\":");
    try aws.json.writeValue(@TypeOf(input.connection_mode), input.connection_mode, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"connectorProfileConfig\":");
    try aws.json.writeValue(@TypeOf(input.connector_profile_config), input.connector_profile_config, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"connectorProfileName\":");
    try aws.json.writeValue(@TypeOf(input.connector_profile_name), input.connector_profile_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConnectorProfileOutput {
    var result: UpdateConnectorProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConnectorProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
