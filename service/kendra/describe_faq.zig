...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFaqInput, options: Options) !DescribeFaqOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFaqInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.DescribeFaq");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFaqOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFaqOutput, body, alloc);
}
