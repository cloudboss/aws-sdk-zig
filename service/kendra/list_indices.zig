...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIndicesInput, options: Options) !ListIndicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIndicesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.ListIndices");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIndicesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListIndicesOutput, body, alloc);
}
