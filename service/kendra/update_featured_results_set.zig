...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFeaturedResultsSetInput, options: Options) !UpdateFeaturedResultsSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFeaturedResultsSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.UpdateFeaturedResultsSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFeaturedResultsSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFeaturedResultsSetOutput, body, alloc);
}
