...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePrincipalMappingInput, options: Options) !DescribePrincipalMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePrincipalMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.DescribePrincipalMapping");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePrincipalMappingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePrincipalMappingOutput, body, alloc);
}
