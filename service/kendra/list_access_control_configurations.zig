...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessControlConfigurationsInput, options: Options) !ListAccessControlConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessControlConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.ListAccessControlConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessControlConfigurationsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListAccessControlConfigurationsOutput, body, alloc);
}
