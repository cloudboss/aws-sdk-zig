...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetDocumentStatusInput, options: Options) !BatchGetDocumentStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetDocumentStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.BatchGetDocumentStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetDocumentStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetDocumentStatusOutput, body, alloc);
}
