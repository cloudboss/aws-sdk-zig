...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataSourceInput, options: Options) !DescribeDataSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.DescribeDataSource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataSourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDataSourceOutput, body, alloc);
}
