...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeThesaurusInput, options: Options) !DescribeThesaurusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeThesaurusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.DescribeThesaurus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeThesaurusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeThesaurusOutput, body, alloc);
}
