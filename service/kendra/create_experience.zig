...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateExperienceInput, options: Options) !CreateExperienceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateExperienceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.CreateExperience");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateExperienceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateExperienceOutput, body, alloc);
}
