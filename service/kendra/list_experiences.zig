...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExperiencesInput, options: Options) !ListExperiencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExperiencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.ListExperiences");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExperiencesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListExperiencesOutput, body, alloc);
}
