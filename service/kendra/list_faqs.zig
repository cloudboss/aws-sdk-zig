...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFaqsInput, options: Options) !ListFaqsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFaqsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.ListFaqs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFaqsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFaqsOutput, body, alloc);
}
