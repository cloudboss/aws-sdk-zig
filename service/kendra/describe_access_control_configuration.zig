...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccessControlConfigurationInput, options: Options) !DescribeAccessControlConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccessControlConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.DescribeAccessControlConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccessControlConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAccessControlConfigurationOutput, body, alloc);
}
