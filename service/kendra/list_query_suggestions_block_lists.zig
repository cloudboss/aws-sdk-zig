...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQuerySuggestionsBlockListsInput, options: Options) !ListQuerySuggestionsBlockListsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQuerySuggestionsBlockListsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.ListQuerySuggestionsBlockLists");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQuerySuggestionsBlockListsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListQuerySuggestionsBlockListsOutput, body, alloc);
}
