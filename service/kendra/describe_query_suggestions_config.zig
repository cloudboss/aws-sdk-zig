...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeQuerySuggestionsConfigInput, options: Options) !DescribeQuerySuggestionsConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeQuerySuggestionsConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.DescribeQuerySuggestionsConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeQuerySuggestionsConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeQuerySuggestionsConfigOutput, body, alloc);
}
