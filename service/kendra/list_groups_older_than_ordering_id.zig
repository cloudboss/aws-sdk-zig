...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGroupsOlderThanOrderingIdInput, options: Options) !ListGroupsOlderThanOrderingIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupsOlderThanOrderingIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.ListGroupsOlderThanOrderingId");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupsOlderThanOrderingIdOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListGroupsOlderThanOrderingIdOutput, body, alloc);
}
