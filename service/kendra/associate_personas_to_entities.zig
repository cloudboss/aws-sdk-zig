...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociatePersonasToEntitiesInput, options: Options) !AssociatePersonasToEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociatePersonasToEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.AssociatePersonasToEntities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociatePersonasToEntitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociatePersonasToEntitiesOutput, body, alloc);
}
