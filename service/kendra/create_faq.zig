...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFaqInput, options: Options) !CreateFaqOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFaqInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.CreateFaq");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFaqOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateFaqOutput, body, alloc);
}
