...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExperienceEntitiesInput, options: Options) !ListExperienceEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExperienceEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.ListExperienceEntities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExperienceEntitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListExperienceEntitiesOutput, body, alloc);
}
