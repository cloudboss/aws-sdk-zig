...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateQuerySuggestionsBlockListInput, options: Options) !CreateQuerySuggestionsBlockListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateQuerySuggestionsBlockListInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.CreateQuerySuggestionsBlockList");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateQuerySuggestionsBlockListOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateQuerySuggestionsBlockListOutput, body, alloc);
}
