...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFeaturedResultsSetsInput, options: Options) !ListFeaturedResultsSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFeaturedResultsSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.ListFeaturedResultsSets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFeaturedResultsSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFeaturedResultsSetsOutput, body, alloc);
}
