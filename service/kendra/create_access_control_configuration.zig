...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccessControlConfigurationInput, options: Options) !CreateAccessControlConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccessControlConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.CreateAccessControlConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccessControlConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateAccessControlConfigurationOutput, body, alloc);
}
