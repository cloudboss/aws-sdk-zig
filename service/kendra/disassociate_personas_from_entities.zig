...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociatePersonasFromEntitiesInput, options: Options) !DisassociatePersonasFromEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociatePersonasFromEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.DisassociatePersonasFromEntities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociatePersonasFromEntitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociatePersonasFromEntitiesOutput, body, alloc);
}
