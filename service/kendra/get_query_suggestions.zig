...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQuerySuggestionsInput, options: Options) !GetQuerySuggestionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQuerySuggestionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.GetQuerySuggestions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQuerySuggestionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetQuerySuggestionsOutput, body, alloc);
}
