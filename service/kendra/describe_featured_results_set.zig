...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFeaturedResultsSetInput, options: Options) !DescribeFeaturedResultsSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFeaturedResultsSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.DescribeFeaturedResultsSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFeaturedResultsSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFeaturedResultsSetOutput, body, alloc);
}
