...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteFeaturedResultsSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.BatchDeleteFeaturedResultsSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteFeaturedResultsSetOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDeleteFeaturedResultsSetOutput, body, alloc);
}
