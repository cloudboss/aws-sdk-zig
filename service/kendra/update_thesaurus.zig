...
const UpdateThesaurusOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateThesaurusInput, options: Options) !UpdateThesaurusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateThesaurusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.UpdateThesaurus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateThesaurusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
