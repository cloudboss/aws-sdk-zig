...
pub const CreateThesaurusInput = struct {
    ...
    /// to access your S3 bucket that contains the thesaurus file. For more
    /// information,
    /// see [IAM
    ...
    /// also use tags to help control access to the thesaurus. Tag keys and values
    /// can
...
pub const CreateThesaurusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateThesaurusInput, options: CallOptions) !CreateThesaurusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateThesaurusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSKendraFrontendService.CreateThesaurus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateThesaurusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateThesaurusOutput, body, allocator);
}
