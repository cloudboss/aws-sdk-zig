...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccessControlConfigurationInput, options: Options) !UpdateAccessControlConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccessControlConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraFrontendService.UpdateAccessControlConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccessControlConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
