...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProductSubscriptionsInput, options: Options) !ListProductSubscriptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProductSubscriptionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProductSubscriptionsOutput {
    var result: ListProductSubscriptionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProductSubscriptionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
