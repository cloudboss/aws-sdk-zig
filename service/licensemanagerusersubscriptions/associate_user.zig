...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateUserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateUserOutput {
    var result: AssociateUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
