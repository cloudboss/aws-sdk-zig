...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterIdentityProviderInput, options: Options) !DeregisterIdentityProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterIdentityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.identity_provider_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IdentityProviderArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.product) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterIdentityProviderOutput {
    var result: DeregisterIdentityProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeregisterIdentityProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
