...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterIdentityProviderInput, options: Options) !RegisterIdentityProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterIdentityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"IdentityProvider\":");
    try aws.json.writeValue(@TypeOf(input.identity_provider), input.identity_provider, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Product\":");
    try aws.json.writeValue(@TypeOf(input.product), input.product, alloc, &body_buf);
    has_prev = true;
    if (input.settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterIdentityProviderOutput {
    var result: RegisterIdentityProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterIdentityProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
