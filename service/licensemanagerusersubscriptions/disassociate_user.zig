...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateUserInput, options: Options) !DisassociateUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateUserInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.instance_user_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InstanceUserArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.username) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateUserOutput {
    var result: DisassociateUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
