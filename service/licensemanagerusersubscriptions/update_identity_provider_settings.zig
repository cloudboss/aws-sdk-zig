...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIdentityProviderSettingsInput, options: Options) !UpdateIdentityProviderSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIdentityProviderSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UpdateSettings\":");
    try aws.json.writeValue(@TypeOf(input.update_settings), input.update_settings, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIdentityProviderSettingsOutput {
    var result: UpdateIdentityProviderSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIdentityProviderSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
