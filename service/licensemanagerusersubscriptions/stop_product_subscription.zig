...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopProductSubscriptionInput, options: Options) !StopProductSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopProductSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.product_user_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ProductUserArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.username) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopProductSubscriptionOutput {
    var result: StopProductSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopProductSubscriptionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
