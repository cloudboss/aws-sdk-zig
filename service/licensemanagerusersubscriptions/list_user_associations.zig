...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUserAssociationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUserAssociationsOutput {
    var result: ListUserAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUserAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
