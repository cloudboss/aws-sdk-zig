...
pub const ListOriginationNumbersOutput = struct {
    ...
    pub fn deinit(self: *ListOriginationNumbersOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListOriginationNumbersInput, options: Options) !ListOriginationNumbersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOriginationNumbersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListOriginationNumbers&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOriginationNumbersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListOriginationNumbersResult")) break;
            },
            else => {},
        }
    }

    var result: ListOriginationNumbersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.phone_numbers = try serde.deserializePhoneNumberInformationList(&reader, alloc, "member");
                } else {
