...
pub const OptInPhoneNumberOutput = struct {
    ...
    pub fn deinit(self: *OptInPhoneNumberOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: OptInPhoneNumberInput, options: Options) !OptInPhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: OptInPhoneNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=OptInPhoneNumber&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !OptInPhoneNumberOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: OptInPhoneNumberOutput = .{};
