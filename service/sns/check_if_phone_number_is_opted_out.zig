...
pub const CheckIfPhoneNumberIsOptedOutOutput = struct {
    ...
    pub fn deinit(self: *CheckIfPhoneNumberIsOptedOutOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: CheckIfPhoneNumberIsOptedOutInput, options: Options) !CheckIfPhoneNumberIsOptedOutOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CheckIfPhoneNumberIsOptedOutInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CheckIfPhoneNumberIsOptedOut&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CheckIfPhoneNumberIsOptedOutOutput {
    _ = status;
    _ = headers;
    _ = alloc;
