...
pub const CreateSMSSandboxPhoneNumberOutput = struct {
    ...
    pub fn deinit(self: *CreateSMSSandboxPhoneNumberOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: CreateSMSSandboxPhoneNumberInput, options: Options) !CreateSMSSandboxPhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSMSSandboxPhoneNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateSMSSandboxPhoneNumber&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSMSSandboxPhoneNumberOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
