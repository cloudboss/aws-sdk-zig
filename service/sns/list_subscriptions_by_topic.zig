...
pub const ListSubscriptionsByTopicOutput = struct {
    ...
    pub fn deinit(self: *ListSubscriptionsByTopicOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListSubscriptionsByTopicInput, options: Options) !ListSubscriptionsByTopicOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSubscriptionsByTopicInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListSubscriptionsByTopic&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSubscriptionsByTopicOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListSubscriptionsByTopicResult")) break;
            },
            else => {},
        }
    }

    var result: ListSubscriptionsByTopicOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "Subscriptions")) {
                    result.subscriptions = try serde.deserializeSubscriptionsList(&reader, alloc, "member");
                } else {
