...
pub const ListTopicsOutput = struct {
    ...
    pub fn deinit(self: *ListTopicsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListTopicsInput, options: Options) !ListTopicsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTopicsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListTopics&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTopicsOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListTopicsResult")) break;
            },
            else => {},
        }
    }
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                ...
                } else if (std.mem.eql(u8, e.local, "Topics")) {
                    result.topics = try serde.deserializeTopicsList(&reader, alloc, "member");
                } else {
