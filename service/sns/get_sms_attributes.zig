...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSMSAttributesInput, options: CallOptions) !GetSMSAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSMSAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetSMSAttributes&Version=2010-03-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSMSAttributesOutput {
    _ = status;
    _ = headers;
