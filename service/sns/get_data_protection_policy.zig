...
pub const GetDataProtectionPolicyOutput = struct {
    /// Retrieves the `DataProtectionPolicy` in JSON string format.
    data_protection_policy: ?[]const u8 = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *GetDataProtectionPolicyOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetDataProtectionPolicyInput, options: Options) !GetDataProtectionPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataProtectionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetDataProtectionPolicy&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataProtectionPolicyOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetDataProtectionPolicyResult")) break;
            },
            else => {},
        }
    }

    var result: GetDataProtectionPolicyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DataProtectionPolicy")) {
                    result.data_protection_policy = try alloc.dupe(u8, try reader.readElementText());
                } else {
