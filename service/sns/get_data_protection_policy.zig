...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataProtectionPolicyInput, options: CallOptions) !GetDataProtectionPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDataProtectionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetDataProtectionPolicy&Version=2010-03-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDataProtectionPolicyOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DataProtectionPolicy")) {
                    result.data_protection_policy = try allocator.dupe(u8, try reader.readElementText());
                } else {
