...
fn serializeRequest(allocator: std.mem.Allocator, input: PutDataProtectionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=PutDataProtectionPolicy&Version=2010-03-31");
    try body_buf.appendSlice(allocator, "&DataProtectionPolicy=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.data_protection_policy);
