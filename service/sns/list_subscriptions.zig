...
pub const ListSubscriptionsOutput = struct {
    ...
    pub fn deinit(self: *ListSubscriptionsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListSubscriptionsInput, options: Options) !ListSubscriptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSubscriptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListSubscriptions&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSubscriptionsOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListSubscriptionsResult")) break;
            },
            else => {},
        }
    }
