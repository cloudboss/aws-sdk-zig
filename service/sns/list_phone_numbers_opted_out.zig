...
pub const ListPhoneNumbersOptedOutInput = struct {
...
pub const ListPhoneNumbersOptedOutOutput = struct {
    ...
    phone_numbers: ?[]const []const u8 = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *ListPhoneNumbersOptedOutOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListPhoneNumbersOptedOutInput, options: Options) !ListPhoneNumbersOptedOutOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPhoneNumbersOptedOutInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListPhoneNumbersOptedOut&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPhoneNumbersOptedOutOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListPhoneNumbersOptedOutResult")) break;
            },
            else => {},
        }
    }

    var result: ListPhoneNumbersOptedOutOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "phoneNumbers")) {
                    result.phone_numbers = try serde.deserializePhoneNumberList(&reader, alloc, "member");
                } else {
