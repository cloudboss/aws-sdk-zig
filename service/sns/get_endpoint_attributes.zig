...
pub const GetEndpointAttributesOutput = struct {
    ...
    pub fn deinit(self: *GetEndpointAttributesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetEndpointAttributesInput, options: Options) !GetEndpointAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEndpointAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetEndpointAttributes&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEndpointAttributesOutput {
    _ = status;
    _ = headers;
