...
pub const ListPlatformApplicationsInput = struct {
...
pub const ListPlatformApplicationsOutput = struct {
    ...
    /// action.
    platform_applications: ?[]const PlatformApplication = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *ListPlatformApplicationsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListPlatformApplicationsInput, options: Options) !ListPlatformApplicationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPlatformApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListPlatformApplications&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPlatformApplicationsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListPlatformApplicationsResult")) break;
            },
            else => {},
        }
    }

    var result: ListPlatformApplicationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "PlatformApplications")) {
                    result.platform_applications = try serde.deserializeListOfPlatformApplications(&reader, alloc, "member");
                } else {
