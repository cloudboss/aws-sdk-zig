...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemovePermissionInput, options: Options) !RemovePermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemovePermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RemovePermission&Version=2010-03-31");
    try body_buf.appendSlice(alloc, "&Label=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.label);
