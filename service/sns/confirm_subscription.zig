...
pub const ConfirmSubscriptionOutput = struct {
    ...
    pub fn deinit(self: *ConfirmSubscriptionOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ConfirmSubscriptionInput, options: Options) !ConfirmSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ConfirmSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ConfirmSubscription&Version=2010-03-31");
    if (input.authenticate_on_unsubscribe) |v| {
        try body_buf.appendSlice(alloc, "&AuthenticateOnUnsubscribe=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConfirmSubscriptionOutput {
    _ = status;
    _ = headers;
