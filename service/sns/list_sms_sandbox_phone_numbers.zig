...
pub const ListSMSSandboxPhoneNumbersInput = struct {
...
pub const ListSMSSandboxPhoneNumbersOutput = struct {
    ...
    phone_numbers: ?[]const SMSSandboxPhoneNumber = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *ListSMSSandboxPhoneNumbersOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListSMSSandboxPhoneNumbersInput, options: Options) !ListSMSSandboxPhoneNumbersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSMSSandboxPhoneNumbersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListSMSSandboxPhoneNumbers&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSMSSandboxPhoneNumbersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListSMSSandboxPhoneNumbersResult")) break;
            },
            else => {},
        }
    }

    var result: ListSMSSandboxPhoneNumbersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.phone_numbers = try serde.deserializeSMSSandboxPhoneNumberList(&reader, alloc, "member");
                } else {
