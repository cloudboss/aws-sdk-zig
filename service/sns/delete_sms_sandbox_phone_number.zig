...
pub const DeleteSMSSandboxPhoneNumberOutput = struct {
    ...
    pub fn deinit(self: *DeleteSMSSandboxPhoneNumberOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteSMSSandboxPhoneNumberInput, options: Options) !DeleteSMSSandboxPhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSMSSandboxPhoneNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteSMSSandboxPhoneNumber&Version=2010-03-31");
    try body_buf.appendSlice(alloc, "&PhoneNumber=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.phone_number);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSMSSandboxPhoneNumberOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: DeleteSMSSandboxPhoneNumberOutput = .{};
