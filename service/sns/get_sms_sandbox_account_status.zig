...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSMSSandboxAccountStatusInput, options: CallOptions) !GetSMSSandboxAccountStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSMSSandboxAccountStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=GetSMSSandboxAccountStatus&Version=2010-03-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSMSSandboxAccountStatusOutput {
    _ = status;
    _ = headers;
    _ = allocator;
