...
pub const GetSMSSandboxAccountStatusOutput = struct {
    ...
    pub fn deinit(self: *GetSMSSandboxAccountStatusOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetSMSSandboxAccountStatusInput, options: Options) !GetSMSSandboxAccountStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSMSSandboxAccountStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=GetSMSSandboxAccountStatus&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSMSSandboxAccountStatusOutput {
    _ = status;
    _ = headers;
    _ = alloc;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetSMSSandboxAccountStatusResult")) break;
            },
            else => {},
        }
    }

    var result: GetSMSSandboxAccountStatusOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsInSandbox")) {
                    result.is_in_sandbox = std.mem.eql(u8, try reader.readElementText(), "true");
                } else {
