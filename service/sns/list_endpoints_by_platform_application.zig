...
pub const ListEndpointsByPlatformApplicationOutput = struct {
    ...
    pub fn deinit(self: *ListEndpointsByPlatformApplicationOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListEndpointsByPlatformApplicationInput, options: Options) !ListEndpointsByPlatformApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEndpointsByPlatformApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListEndpointsByPlatformApplication&Version=2010-03-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEndpointsByPlatformApplicationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Endpoints")) {
                    result.endpoints = try serde.deserializeListOfEndpoints(&reader, alloc, "member");
