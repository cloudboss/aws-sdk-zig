...
pub const VerifySMSSandboxPhoneNumberOutput = struct {
    ...
    pub fn deinit(self: *VerifySMSSandboxPhoneNumberOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: VerifySMSSandboxPhoneNumberInput, options: Options) !VerifySMSSandboxPhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifySMSSandboxPhoneNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=VerifySMSSandboxPhoneNumber&Version=2010-03-31");
    try body_buf.appendSlice(alloc, "&OneTimePassword=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.one_time_password);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifySMSSandboxPhoneNumberOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
