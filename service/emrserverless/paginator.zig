...
pub const ListJobRunAttemptsPaginator = struct {
    client: *Client,
    params: list_job_run_attempts.ListJobRunAttemptsInput,
    next_token: ?[]const u8 = null,
    done: bool = false,
    allocator: std.mem.Allocator,
    ...
    pub fn next(self: *Self, allocator: std.mem.Allocator, options: list_job_run_attempts.Options) !list_job_run_attempts.ListJobRunAttemptsOutput {
        if (self.done) {
            return error.EndOfPagination;
        }
        ...
        const output = try list_job_run_attempts.execute(self.client, allocator, self.params, options);
