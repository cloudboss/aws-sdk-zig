...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDashboardForJobRunInput, options: Options) !GetDashboardForJobRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDashboardForJobRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dashboard");
    ...
    if (input.access_system_profile_logs) |v| {
        ...
        try query_buf.appendSlice(alloc, "accessSystemProfileLogs=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDashboardForJobRunOutput {
    var result: GetDashboardForJobRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDashboardForJobRunOutput, body, alloc);
    }
    _ = status;
    _ = headers;
