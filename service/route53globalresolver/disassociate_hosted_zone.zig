...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateHostedZoneInput, options: Options) !DisassociateHostedZoneOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateHostedZoneInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/hosted-zone-associations/hosted-zone/");
    try path_buf.appendSlice(alloc, input.hosted_zone_id);
    try path_buf.appendSlice(alloc, "/resource-arn/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateHostedZoneOutput {
    var result: DisassociateHostedZoneOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateHostedZoneOutput, body, alloc);
    }
    _ = status;
    _ = headers;
