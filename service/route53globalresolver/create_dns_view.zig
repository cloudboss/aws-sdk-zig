...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDNSViewInput, options: CallOptions) !CreateDNSViewOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDNSViewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateDNSViewOutput {
    var result: CreateDNSViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDNSViewOutput, body, allocator);
    }
    _ = status;
    _ = headers;
