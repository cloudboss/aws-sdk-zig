...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGlobalResolverInput, options: Options) !GetGlobalResolverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGlobalResolverInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGlobalResolverOutput {
    var result: GetGlobalResolverOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGlobalResolverOutput, body, alloc);
    }
    _ = status;
    _ = headers;
