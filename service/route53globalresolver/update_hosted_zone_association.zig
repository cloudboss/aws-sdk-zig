...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateHostedZoneAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateHostedZoneAssociationOutput {
    var result: UpdateHostedZoneAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateHostedZoneAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
