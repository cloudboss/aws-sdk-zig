...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreateFirewallRuleInput, options: Options) !BatchCreateFirewallRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateFirewallRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateFirewallRuleOutput {
    var result: BatchCreateFirewallRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchCreateFirewallRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
