...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccessSourceInput, options: Options) !UpdateAccessSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccessSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/access-sources/");
    try path_buf.appendSlice(alloc, input.access_source_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccessSourceOutput {
    var result: UpdateAccessSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccessSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
