...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAccessSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAccessSourceOutput {
    var result: DeleteAccessSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAccessSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
