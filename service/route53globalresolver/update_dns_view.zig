...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDNSViewInput, options: Options) !UpdateDNSViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDNSViewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDNSViewOutput {
    var result: UpdateDNSViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDNSViewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
