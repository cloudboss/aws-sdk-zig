...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableDNSViewInput, options: Options) !EnableDNSViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableDNSViewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableDNSViewOutput {
    var result: EnableDNSViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EnableDNSViewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
