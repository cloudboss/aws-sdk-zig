...
const AccessSourcesItem = @import("access_sources_item.zig").AccessSourcesItem;

pub const ListAccessSourcesInput = struct {
...
pub const ListAccessSourcesOutput = struct {
    /// An array containing information about the access sources, such as the ID,
    /// CIDR etc.
    access_sources: ?[]const AccessSourcesItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessSourcesInput, options: CallOptions) !ListAccessSourcesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAccessSourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAccessSourcesOutput {
    var result: ListAccessSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccessSourcesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
