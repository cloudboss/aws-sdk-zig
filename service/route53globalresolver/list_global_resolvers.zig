...
const GlobalResolversItem = @import("global_resolvers_item.zig").GlobalResolversItem;

pub const ListGlobalResolversInput = struct {
...
pub const ListGlobalResolversOutput = struct {
    /// Paginated list of Global Resolvers.
    global_resolvers: ?[]const GlobalResolversItem = null,
    ...
    pub const json_field_names = .{
        .global_resolvers = "globalResolvers",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGlobalResolversInput, options: CallOptions) !ListGlobalResolversOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListGlobalResolversInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListGlobalResolversOutput {
    var result: ListGlobalResolversOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGlobalResolversOutput, body, allocator);
    }
    _ = status;
    _ = headers;
