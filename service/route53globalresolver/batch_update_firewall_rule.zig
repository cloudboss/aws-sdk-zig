...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateFirewallRuleInput, options: Options) !BatchUpdateFirewallRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateFirewallRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateFirewallRuleOutput {
    var result: BatchUpdateFirewallRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateFirewallRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
