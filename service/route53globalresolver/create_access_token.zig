...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccessTokenInput, options: Options) !CreateAccessTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccessTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.expires_at) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"expiresAt\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccessTokenOutput {
    var result: CreateAccessTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAccessTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
