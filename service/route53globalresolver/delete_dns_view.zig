...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDNSViewInput, options: Options) !DeleteDNSViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDNSViewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDNSViewOutput {
    var result: DeleteDNSViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDNSViewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
