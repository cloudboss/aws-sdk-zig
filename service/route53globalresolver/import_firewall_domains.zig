...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportFirewallDomainsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/domains/s3_file_url");
    ...
    try body_buf.appendSlice(alloc, "\"domainFileUrl\":");
    try aws.json.writeValue(@TypeOf(input.domain_file_url), input.domain_file_url, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportFirewallDomainsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportFirewallDomainsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
