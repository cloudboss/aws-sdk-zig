...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccessSourceInput, options: Options) !GetAccessSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessSourceOutput {
    var result: GetAccessSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAccessSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
