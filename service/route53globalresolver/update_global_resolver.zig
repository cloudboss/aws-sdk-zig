...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGlobalResolverInput, options: Options) !UpdateGlobalResolverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGlobalResolverInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/global-resolver/");
    ...
    if (input.observability_region) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"observabilityRegion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGlobalResolverOutput {
    var result: UpdateGlobalResolverOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGlobalResolverOutput, body, alloc);
    }
    _ = status;
    _ = headers;
