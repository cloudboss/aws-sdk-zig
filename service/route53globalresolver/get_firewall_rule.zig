...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFirewallRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFirewallRuleOutput {
    var result: GetFirewallRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFirewallRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
