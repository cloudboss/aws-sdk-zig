...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFirewallDomainsInput, options: Options) !UpdateFirewallDomainsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFirewallDomainsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/firewall-domain-lists/");
    try path_buf.appendSlice(alloc, input.firewall_domain_list_id);
    ...
    try aws.json.writeValue(@TypeOf(input.domains), input.domains, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"operation\":");
    try aws.json.writeValue(@TypeOf(input.operation), input.operation, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFirewallDomainsOutput {
    var result: UpdateFirewallDomainsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFirewallDomainsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
