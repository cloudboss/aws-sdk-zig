...
pub const DisableDNSViewInput = struct {
...
pub const DisableDNSViewOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableDNSViewInput, options: CallOptions) !DisableDNSViewOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableDNSViewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableDNSViewOutput {
    var result: DisableDNSViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisableDNSViewOutput, body, allocator);
    }
    _ = status;
    _ = headers;
