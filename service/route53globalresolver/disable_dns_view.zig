...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableDNSViewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableDNSViewOutput {
    var result: DisableDNSViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisableDNSViewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
