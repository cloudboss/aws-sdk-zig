...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetHostedZoneAssociationInput, options: Options) !GetHostedZoneAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetHostedZoneAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.hosted_zone_association_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetHostedZoneAssociationOutput {
    var result: GetHostedZoneAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetHostedZoneAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
