...
const DNSViewSummary = @import("dns_view_summary.zig").DNSViewSummary;

pub const ListDNSViewsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDNSViewsInput, options: CallOptions) !ListDNSViewsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDNSViewsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/dns-views/resolver/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDNSViewsOutput {
    var result: ListDNSViewsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDNSViewsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
