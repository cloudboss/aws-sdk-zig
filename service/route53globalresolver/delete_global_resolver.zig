...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGlobalResolverInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGlobalResolverOutput {
    var result: DeleteGlobalResolverOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteGlobalResolverOutput, body, alloc);
    }
    _ = status;
    _ = headers;
