...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAccessTokenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAccessTokenOutput {
    var result: DeleteAccessTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAccessTokenOutput, body, allocator);
    }
    _ = status;
    _ = headers;
