...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteFirewallRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"firewallRules\":");
    try aws.json.writeValue(@TypeOf(input.firewall_rules), input.firewall_rules, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteFirewallRuleOutput {
    var result: BatchDeleteFirewallRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteFirewallRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
