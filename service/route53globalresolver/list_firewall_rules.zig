...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFirewallRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "dnsview_id=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.dns_view_id);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFirewallRulesOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFirewallRulesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
