...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHostedZoneAssociationsInput, options: Options) !ListHostedZoneAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHostedZoneAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/hosted-zone-associations/resource-arn/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHostedZoneAssociationsOutput {
    var result: ListHostedZoneAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListHostedZoneAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
