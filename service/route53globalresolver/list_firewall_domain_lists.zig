...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFirewallDomainListsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.global_resolver_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "global_resolver_id=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFirewallDomainListsOutput {
    var result: ListFirewallDomainListsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFirewallDomainListsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
