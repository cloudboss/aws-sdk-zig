...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessTokenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessTokenOutput {
    var result: GetAccessTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAccessTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
