...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMediaCapturePipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.chime_sdk_meeting_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ChimeSdkMeetingConfiguration\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SinkArn\":");
    try aws.json.writeValue(@TypeOf(input.sink_arn), input.sink_arn, alloc, &body_buf);
    has_prev = true;
    if (input.sink_iam_role_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SinkIamRoleArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SinkType\":");
    try aws.json.writeValue(@TypeOf(input.sink_type), input.sink_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceArn\":");
    try aws.json.writeValue(@TypeOf(input.source_arn), input.source_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceType\":");
    try aws.json.writeValue(@TypeOf(input.source_type), input.source_type, alloc, &body_buf);
    has_prev = true;
    if (input.sse_aws_key_management_params) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SseAwsKeyManagementParams\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMediaCapturePipelineOutput {
    var result: CreateMediaCapturePipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMediaCapturePipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
