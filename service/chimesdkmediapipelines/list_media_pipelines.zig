...
const MediaPipelineSummary = @import("media_pipeline_summary.zig").MediaPipelineSummary;

pub const ListMediaPipelinesInput = struct {
...
pub const ListMediaPipelinesOutput = struct {
    /// The media pipeline objects in the list.
    media_pipelines: ?[]const MediaPipelineSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMediaPipelinesInput, options: Options) !ListMediaPipelinesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMediaPipelinesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/sdk-media-pipelines";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMediaPipelinesOutput {
    var result: ListMediaPipelinesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMediaPipelinesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
