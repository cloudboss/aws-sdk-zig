...
fn serializeRequest(allocator: std.mem.Allocator, input: StartSpeakerSearchTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"VoiceProfileDomainArn\":");
    try aws.json.writeValue(@TypeOf(input.voice_profile_domain_arn), input.voice_profile_domain_arn, allocator, &body_buf);
    has_prev = true;
