...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMediaCapturePipelineInput, options: Options) !GetMediaCapturePipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMediaCapturePipelineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMediaCapturePipelineOutput {
    var result: GetMediaCapturePipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMediaCapturePipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
