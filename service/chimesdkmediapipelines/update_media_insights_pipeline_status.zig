...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMediaInsightsPipelineStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/media-insights-pipeline-status/");
    ...
    try body_buf.appendSlice(alloc, "\"UpdateStatus\":");
    try aws.json.writeValue(@TypeOf(input.update_status), input.update_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMediaInsightsPipelineStatusOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
