...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMediaPipelineKinesisVideoStreamPoolInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMediaPipelineKinesisVideoStreamPoolOutput {
    var result: GetMediaPipelineKinesisVideoStreamPoolOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMediaPipelineKinesisVideoStreamPoolOutput, body, alloc);
    }
    _ = status;
    _ = headers;
