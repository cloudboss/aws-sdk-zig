...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMediaPipelineKinesisVideoStreamPoolInput, options: Options) !CreateMediaPipelineKinesisVideoStreamPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMediaPipelineKinesisVideoStreamPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.stream_configuration), input.stream_configuration, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMediaPipelineKinesisVideoStreamPoolOutput {
    var result: CreateMediaPipelineKinesisVideoStreamPoolOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMediaPipelineKinesisVideoStreamPoolOutput, body, alloc);
    }
    _ = status;
    _ = headers;
