...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMediaInsightsPipelineInput, options: Options) !CreateMediaInsightsPipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMediaInsightsPipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.kinesis_video_stream_recording_source_runtime_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"KinesisVideoStreamRecordingSourceRuntimeConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.kinesis_video_stream_source_runtime_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"KinesisVideoStreamSourceRuntimeConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MediaInsightsPipelineConfigurationArn\":");
    try aws.json.writeValue(@TypeOf(input.media_insights_pipeline_configuration_arn), input.media_insights_pipeline_configuration_arn, alloc, &body_buf);
    has_prev = true;
    if (input.media_insights_runtime_metadata) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MediaInsightsRuntimeMetadata\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.s3_recording_sink_runtime_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"S3RecordingSinkRuntimeConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMediaInsightsPipelineOutput {
    var result: CreateMediaInsightsPipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMediaInsightsPipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
