...
pub const CreateMediaStreamPipelineInput = struct {
...
pub const CreateMediaStreamPipelineOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMediaStreamPipelineInput, options: CallOptions) !CreateMediaStreamPipelineOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateMediaStreamPipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/sdk-media-stream-pipelines";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateMediaStreamPipelineOutput {
    var result: CreateMediaStreamPipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMediaStreamPipelineOutput, body, allocator);
    }
    _ = status;
    _ = headers;
