...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMediaStreamPipelineInput, options: Options) !CreateMediaStreamPipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMediaStreamPipelineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMediaStreamPipelineOutput {
    var result: CreateMediaStreamPipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMediaStreamPipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
