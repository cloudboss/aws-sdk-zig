...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMediaPipelineKinesisVideoStreamPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/media-pipeline-kinesis-video-stream-pools/");
    ...
    if (input.stream_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"StreamConfiguration\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMediaPipelineKinesisVideoStreamPoolOutput {
    var result: UpdateMediaPipelineKinesisVideoStreamPoolOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMediaPipelineKinesisVideoStreamPoolOutput, body, alloc);
    }
    _ = status;
    _ = headers;
