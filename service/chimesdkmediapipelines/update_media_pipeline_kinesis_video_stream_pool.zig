...
pub const UpdateMediaPipelineKinesisVideoStreamPoolInput = struct {
    ...
    stream_configuration: ?KinesisVideoStreamConfigurationUpdate = null,
...
pub const UpdateMediaPipelineKinesisVideoStreamPoolOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateMediaPipelineKinesisVideoStreamPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/media-pipeline-kinesis-video-stream-pools/");
    ...
    if (input.stream_configuration) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateMediaPipelineKinesisVideoStreamPoolOutput {
    var result: UpdateMediaPipelineKinesisVideoStreamPoolOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMediaPipelineKinesisVideoStreamPoolOutput, body, allocator);
    }
    _ = status;
    _ = headers;
