...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMediaInsightsPipelineConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/media-insights-pipeline-configurations/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMediaInsightsPipelineConfigurationOutput {
    var result: GetMediaInsightsPipelineConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMediaInsightsPipelineConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
