...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMediaConcatenationPipelineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMediaConcatenationPipelineOutput {
    var result: CreateMediaConcatenationPipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMediaConcatenationPipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
