...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMediaInsightsPipelineConfigurationInput, options: Options) !CreateMediaInsightsPipelineConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMediaInsightsPipelineConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Elements\":");
    try aws.json.writeValue(@TypeOf(input.elements), input.elements, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MediaInsightsPipelineConfigurationName\":");
    try aws.json.writeValue(@TypeOf(input.media_insights_pipeline_configuration_name), input.media_insights_pipeline_configuration_name, alloc, &body_buf);
    has_prev = true;
    if (input.real_time_alert_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RealTimeAlertConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ResourceAccessRoleArn\":");
    try aws.json.writeValue(@TypeOf(input.resource_access_role_arn), input.resource_access_role_arn, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMediaInsightsPipelineConfigurationOutput {
    var result: CreateMediaInsightsPipelineConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMediaInsightsPipelineConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
