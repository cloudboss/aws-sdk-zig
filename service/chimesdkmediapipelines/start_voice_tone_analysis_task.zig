...
fn serializeRequest(alloc: std.mem.Allocator, input: StartVoiceToneAnalysisTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.language_code), input.language_code, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartVoiceToneAnalysisTaskOutput {
    var result: StartVoiceToneAnalysisTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartVoiceToneAnalysisTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
