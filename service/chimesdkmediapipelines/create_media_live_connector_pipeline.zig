...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMediaLiveConnectorPipelineInput, options: Options) !CreateMediaLiveConnectorPipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMediaLiveConnectorPipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Sinks\":");
    try aws.json.writeValue(@TypeOf(input.sinks), input.sinks, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMediaLiveConnectorPipelineOutput {
    var result: CreateMediaLiveConnectorPipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMediaLiveConnectorPipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
