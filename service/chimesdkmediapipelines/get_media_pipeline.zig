...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMediaPipelineInput, options: Options) !GetMediaPipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMediaPipelineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMediaPipelineOutput {
    var result: GetMediaPipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMediaPipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
