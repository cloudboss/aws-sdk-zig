...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDevicePoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.DeleteDevicePool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDevicePoolOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
