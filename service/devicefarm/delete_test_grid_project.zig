...
pub const DeleteTestGridProjectInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTestGridProjectInput, options: Options) !DeleteTestGridProjectOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTestGridProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.DeleteTestGridProject");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTestGridProjectOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
