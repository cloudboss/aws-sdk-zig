...
const GetProjectInput = @import("get_project_request.zig").GetProjectRequest;

const GetProjectOutput = @import("get_project_result.zig").GetProjectResult;
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.GetProject");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetProjectOutput {
    ...
    return aws.json.parseJsonObject(GetProjectOutput, body, allocator);
}
