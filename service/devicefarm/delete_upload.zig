...
pub const DeleteUploadInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUploadInput, options: CallOptions) !DeleteUploadOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.DeleteUpload");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUploadOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
