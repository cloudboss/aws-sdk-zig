...
pub const ListSuitesInput = struct {
...
pub const ListSuitesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSuitesInput, options: CallOptions) !ListSuitesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSuitesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.ListSuites");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSuitesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSuitesOutput, body, allocator);
}
