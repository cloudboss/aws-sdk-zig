...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDevicePoolInput, options: Options) !UpdateDevicePoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDevicePoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.UpdateDevicePool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDevicePoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateDevicePoolOutput, body, alloc);
}
