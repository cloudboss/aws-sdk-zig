...
pub const GetVPCEConfigurationInput = struct {
...
pub const GetVPCEConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVPCEConfigurationInput, options: CallOptions) !GetVPCEConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetVPCEConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.GetVPCEConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetVPCEConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetVPCEConfigurationOutput, body, allocator);
}
