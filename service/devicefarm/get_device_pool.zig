...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDevicePoolInput, options: Options) !GetDevicePoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDevicePoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.GetDevicePool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDevicePoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDevicePoolOutput, body, alloc);
}
