...
pub const CreateTestGridProjectInput = struct {
...
pub const CreateTestGridProjectOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTestGridProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.CreateTestGridProject");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTestGridProjectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTestGridProjectOutput, body, allocator);
}
