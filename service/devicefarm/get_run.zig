...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRunInput, options: CallOptions) !GetRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.GetRun");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRunOutput {
    ...
    return aws.json.parseJsonObject(GetRunOutput, body, allocator);
}
