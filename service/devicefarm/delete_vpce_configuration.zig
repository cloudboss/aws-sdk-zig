...
pub const DeleteVPCEConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVPCEConfigurationInput, options: Options) !DeleteVPCEConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteVPCEConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.DeleteVPCEConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteVPCEConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
