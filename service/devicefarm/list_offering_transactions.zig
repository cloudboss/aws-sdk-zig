...
pub const ListOfferingTransactionsInput = struct {
...
pub const ListOfferingTransactionsOutput = struct {
    /// An identifier that was returned from the previous call to this operation,
    /// which can
    /// be used to return the next set of items in the list.
    next_token: ?[]const u8 = null,
    ...
    /// Device
    /// Farm.
    offering_transactions: ?[]const OfferingTransaction = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOfferingTransactionsInput, options: CallOptions) !ListOfferingTransactionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOfferingTransactionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.ListOfferingTransactions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOfferingTransactionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOfferingTransactionsOutput, body, allocator);
}
