...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeviceInput, options: Options) !GetDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.GetDevice");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeviceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeviceOutput, body, alloc);
}
