...
pub const StopRemoteAccessSessionInput = struct {
...
pub const StopRemoteAccessSessionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopRemoteAccessSessionInput, options: CallOptions) !StopRemoteAccessSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopRemoteAccessSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.StopRemoteAccessSession");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopRemoteAccessSessionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopRemoteAccessSessionOutput, body, allocator);
}
