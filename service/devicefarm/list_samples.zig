...
pub const ListSamplesInput = struct {
...
pub const ListSamplesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSamplesInput, options: CallOptions) !ListSamplesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSamplesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.ListSamples");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSamplesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSamplesOutput, body, allocator);
}
