...
pub const ListDeviceInstancesInput = struct {
...
pub const ListDeviceInstancesOutput = struct {
    /// An object that contains information about your device instances.
    device_instances: ?[]const DeviceInstance = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeviceInstancesInput, options: CallOptions) !ListDeviceInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDeviceInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.ListDeviceInstances");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDeviceInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDeviceInstancesOutput, body, allocator);
}
