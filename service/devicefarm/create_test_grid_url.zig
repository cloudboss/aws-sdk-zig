...
pub const CreateTestGridUrlInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTestGridUrlInput, options: CallOptions) !CreateTestGridUrlOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTestGridUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.CreateTestGridUrl");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTestGridUrlOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTestGridUrlOutput, body, allocator);
}
