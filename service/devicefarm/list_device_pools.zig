...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDevicePoolsInput, options: Options) !ListDevicePoolsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDevicePoolsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.ListDevicePools");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDevicePoolsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDevicePoolsOutput, body, alloc);
}
