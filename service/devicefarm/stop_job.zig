...
fn serializeRequest(allocator: std.mem.Allocator, input: StopJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.StopJob");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopJobOutput {
    ...
    return aws.json.parseJsonObject(StopJobOutput, body, allocator);
}
