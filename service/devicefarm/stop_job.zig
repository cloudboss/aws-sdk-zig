...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopJobInput, options: Options) !StopJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.StopJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopJobOutput, body, alloc);
}
