...
pub const GetRemoteAccessSessionInput = struct {
...
pub const GetRemoteAccessSessionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRemoteAccessSessionInput, options: CallOptions) !GetRemoteAccessSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRemoteAccessSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.GetRemoteAccessSession");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRemoteAccessSessionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRemoteAccessSessionOutput, body, allocator);
}
