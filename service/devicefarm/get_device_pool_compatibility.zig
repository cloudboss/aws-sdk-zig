...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDevicePoolCompatibilityInput, options: Options) !GetDevicePoolCompatibilityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDevicePoolCompatibilityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.GetDevicePoolCompatibility");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDevicePoolCompatibilityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDevicePoolCompatibilityOutput, body, alloc);
}
