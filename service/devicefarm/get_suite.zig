...
pub const GetSuiteInput = struct {
    /// The suite's ARN.
    arn: []const u8,
...
pub const GetSuiteOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSuiteInput, options: CallOptions) !GetSuiteOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSuiteInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.GetSuite");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSuiteOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSuiteOutput, body, allocator);
}
