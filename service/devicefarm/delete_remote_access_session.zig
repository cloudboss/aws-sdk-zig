...
pub const DeleteRemoteAccessSessionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRemoteAccessSessionInput, options: Options) !DeleteRemoteAccessSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRemoteAccessSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.DeleteRemoteAccessSession");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRemoteAccessSessionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
