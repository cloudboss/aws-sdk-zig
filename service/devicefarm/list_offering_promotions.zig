...
const OfferingPromotion = @import("offering_promotion.zig").OfferingPromotion;

pub const ListOfferingPromotionsInput = struct {
...
pub const ListOfferingPromotionsOutput = struct {
    ...
    offering_promotions: ?[]const OfferingPromotion = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOfferingPromotionsInput, options: CallOptions) !ListOfferingPromotionsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "devicefarm");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOfferingPromotionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.ListOfferingPromotions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOfferingPromotionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOfferingPromotionsOutput, body, allocator);
}
