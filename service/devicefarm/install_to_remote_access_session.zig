...
pub const InstallToRemoteAccessSessionInput = struct {
    ...
    remote_access_session_arn: []const u8,
...
pub const InstallToRemoteAccessSessionOutput = struct {
    /// An app to upload or that has been uploaded.
    app_upload: ?Upload = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InstallToRemoteAccessSessionInput, options: CallOptions) !InstallToRemoteAccessSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: InstallToRemoteAccessSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.InstallToRemoteAccessSession");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !InstallToRemoteAccessSessionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(InstallToRemoteAccessSessionOutput, body, allocator);
}
