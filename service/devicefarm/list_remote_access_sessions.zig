...
pub const ListRemoteAccessSessionsInput = struct {
...
pub const ListRemoteAccessSessionsOutput = struct {
    /// An identifier that was returned from the previous call to this operation,
    /// which can
    /// be used to return the next set of items in the list.
    next_token: ?[]const u8 = null,
    ...
    /// access session you are
    /// requesting.
    remote_access_sessions: ?[]const RemoteAccessSession = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRemoteAccessSessionsInput, options: CallOptions) !ListRemoteAccessSessionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRemoteAccessSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.ListRemoteAccessSessions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRemoteAccessSessionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRemoteAccessSessionsOutput, body, allocator);
}
