...
pub const RenewOfferingInput = struct {
...
pub const RenewOfferingOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RenewOfferingInput, options: CallOptions) !RenewOfferingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RenewOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.RenewOffering");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RenewOfferingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RenewOfferingOutput, body, allocator);
}
