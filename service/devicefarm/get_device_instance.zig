...
pub const GetDeviceInstanceInput = struct {
...
pub const GetDeviceInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeviceInstanceInput, options: CallOptions) !GetDeviceInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDeviceInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.GetDeviceInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDeviceInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeviceInstanceOutput, body, allocator);
}
