...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNetworkProfileInput, options: Options) !UpdateNetworkProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNetworkProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.UpdateNetworkProfile");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNetworkProfileOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateNetworkProfileOutput, body, alloc);
}
