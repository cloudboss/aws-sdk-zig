...
pub const ListTestGridProjectsInput = struct {
...
pub const ListTestGridProjectsOutput = struct {
    ...
    test_grid_projects: ?[]const TestGridProject = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTestGridProjectsInput, options: CallOptions) !ListTestGridProjectsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTestGridProjectsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.ListTestGridProjects");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTestGridProjectsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTestGridProjectsOutput, body, allocator);
}
