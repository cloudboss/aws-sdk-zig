...
pub const StopRunOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopRunInput, options: CallOptions) !StopRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.StopRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopRunOutput, body, allocator);
}
