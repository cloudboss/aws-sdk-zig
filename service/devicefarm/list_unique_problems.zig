...
const UniqueProblem = @import("unique_problem.zig").UniqueProblem;

pub const ListUniqueProblemsInput = struct {
...
pub const ListUniqueProblemsOutput = struct {
    ...
    ///
    /// * STOPPED
    unique_problems: ?[]const aws.map.MapEntry([]const UniqueProblem) = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUniqueProblemsInput, options: CallOptions) !ListUniqueProblemsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUniqueProblemsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.ListUniqueProblems");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUniqueProblemsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUniqueProblemsOutput, body, allocator);
}
