...
pub const GetNetworkProfileInput = struct {
...
pub const GetNetworkProfileOutput = struct {
    /// The network profile.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNetworkProfileInput, options: CallOptions) !GetNetworkProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetNetworkProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.GetNetworkProfile");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetNetworkProfileOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetNetworkProfileOutput, body, allocator);
}
