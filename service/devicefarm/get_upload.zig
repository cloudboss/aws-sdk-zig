...
pub const GetUploadInput = struct {
    /// The upload's ARN.
    arn: []const u8,
...
pub const GetUploadOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUploadInput, options: CallOptions) !GetUploadOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.GetUpload");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetUploadOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUploadOutput, body, allocator);
}
