...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOfferingStatusInput, options: Options) !GetOfferingStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOfferingStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.GetOfferingStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOfferingStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOfferingStatusOutput, body, alloc);
}
