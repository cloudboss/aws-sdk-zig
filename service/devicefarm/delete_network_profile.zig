...
pub const DeleteNetworkProfileInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteNetworkProfileInput, options: Options) !DeleteNetworkProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteNetworkProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.DeleteNetworkProfile");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteNetworkProfileOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
