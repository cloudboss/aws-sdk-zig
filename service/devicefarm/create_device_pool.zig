...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDevicePoolInput, options: Options) !CreateDevicePoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDevicePoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DeviceFarm_20150623.CreateDevicePool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDevicePoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDevicePoolOutput, body, alloc);
}
