...
pub const ListNetworkProfilesInput = struct {
...
pub const ListNetworkProfilesOutput = struct {
    /// A list of the available network profiles.
    network_profiles: ?[]const NetworkProfile = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListNetworkProfilesInput, options: CallOptions) !ListNetworkProfilesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListNetworkProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DeviceFarm_20150623.ListNetworkProfiles");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListNetworkProfilesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListNetworkProfilesOutput, body, allocator);
}
