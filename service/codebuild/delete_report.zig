...
pub const DeleteReportInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteReportInput, options: CallOptions) !DeleteReportOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.DeleteReport");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteReportOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
