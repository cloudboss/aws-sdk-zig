...
pub const ListSandboxesForProjectInput = struct {
...
pub const ListSandboxesForProjectOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSandboxesForProjectInput, options: CallOptions) !ListSandboxesForProjectOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSandboxesForProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.ListSandboxesForProject");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSandboxesForProjectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSandboxesForProjectOutput, body, allocator);
}
