...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSandboxesForProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.ListSandboxesForProject");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSandboxesForProjectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSandboxesForProjectOutput, body, alloc);
}
