...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReportGroupsInput, options: Options) !ListReportGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReportGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.ListReportGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReportGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListReportGroupsOutput, body, alloc);
}
