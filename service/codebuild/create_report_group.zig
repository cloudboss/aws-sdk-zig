...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReportGroupInput, options: Options) !CreateReportGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReportGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.CreateReportGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReportGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateReportGroupOutput, body, alloc);
}
