...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartBuildBatchInput, options: CallOptions) !StartBuildBatchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartBuildBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.StartBuildBatch");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartBuildBatchOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartBuildBatchOutput, body, allocator);
}
