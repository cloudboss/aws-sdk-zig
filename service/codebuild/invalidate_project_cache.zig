...
pub const InvalidateProjectCacheInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InvalidateProjectCacheInput, options: Options) !InvalidateProjectCacheOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: InvalidateProjectCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.InvalidateProjectCache");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !InvalidateProjectCacheOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
