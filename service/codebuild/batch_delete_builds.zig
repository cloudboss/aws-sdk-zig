...
pub const BatchDeleteBuildsInput = struct {
...
pub const BatchDeleteBuildsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteBuildsInput, options: CallOptions) !BatchDeleteBuildsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDeleteBuildsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.BatchDeleteBuilds");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDeleteBuildsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDeleteBuildsOutput, body, allocator);
}
