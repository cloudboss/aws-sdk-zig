...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSourceCredentialsInput, options: Options) !ListSourceCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSourceCredentialsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.ListSourceCredentials");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSourceCredentialsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSourceCredentialsOutput, body, alloc);
}
