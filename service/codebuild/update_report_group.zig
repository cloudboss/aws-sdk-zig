...
pub const UpdateReportGroupInput = struct {
...
pub const UpdateReportGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateReportGroupInput, options: CallOptions) !UpdateReportGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateReportGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.UpdateReportGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateReportGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateReportGroupOutput, body, allocator);
}
