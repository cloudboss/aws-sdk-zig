...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteReportGroupInput, options: Options) !DeleteReportGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteReportGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.DeleteReportGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteReportGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
