...
fn serializeRequest(alloc: std.mem.Allocator, input: StartCommandExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.StartCommandExecution");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartCommandExecutionOutput {
    ...
    return aws.json.parseJsonObject(StartCommandExecutionOutput, body, alloc);
}
