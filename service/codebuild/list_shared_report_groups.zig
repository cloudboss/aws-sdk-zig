...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSharedReportGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.ListSharedReportGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSharedReportGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSharedReportGroupsOutput, body, alloc);
}
