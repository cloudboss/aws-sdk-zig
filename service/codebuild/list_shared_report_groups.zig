...
pub const ListSharedReportGroupsInput = struct {
...
pub const ListSharedReportGroupsOutput = struct {
    /// During a previous call, the maximum number of items that can be returned is
    /// the value
    /// specified in `maxResults`. If there more items in the list, then a unique
    /// string called a *nextToken* is returned. To get the next batch of
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSharedReportGroupsInput, options: CallOptions) !ListSharedReportGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSharedReportGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.ListSharedReportGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSharedReportGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSharedReportGroupsOutput, body, allocator);
}
