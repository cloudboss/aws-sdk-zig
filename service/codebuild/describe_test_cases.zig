...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTestCasesInput, options: CallOptions) !DescribeTestCasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTestCasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.DescribeTestCases");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTestCasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTestCasesOutput, body, allocator);
}
