...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetProjectsInput, options: Options) !BatchGetProjectsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetProjectsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.BatchGetProjects");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetProjectsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetProjectsOutput, body, alloc);
}
