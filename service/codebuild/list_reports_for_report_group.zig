...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReportsForReportGroupInput, options: Options) !ListReportsForReportGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReportsForReportGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.ListReportsForReportGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReportsForReportGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListReportsForReportGroupOutput, body, alloc);
}
