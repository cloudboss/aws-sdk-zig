...
pub const ListBuildsForProjectInput = struct {
    ...
    /// not the build
    /// identifier. If this is not specified, the results are sorted in descending
    /// order.
    ///
...
pub const ListBuildsForProjectOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBuildsForProjectInput, options: CallOptions) !ListBuildsForProjectOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBuildsForProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.ListBuildsForProject");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBuildsForProjectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBuildsForProjectOutput, body, allocator);
}
