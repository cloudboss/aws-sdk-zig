...
pub const StopBuildBatchInput = struct {
...
pub const StopBuildBatchOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopBuildBatchInput, options: CallOptions) !StopBuildBatchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopBuildBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeBuild_20161006.StopBuildBatch");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopBuildBatchOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopBuildBatchOutput, body, allocator);
}
