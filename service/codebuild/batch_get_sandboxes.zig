...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetSandboxesInput, options: Options) !BatchGetSandboxesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetSandboxesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.BatchGetSandboxes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetSandboxesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetSandboxesOutput, body, alloc);
}
