...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportSourceCredentialsInput, options: Options) !ImportSourceCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportSourceCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.ImportSourceCredentials");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportSourceCredentialsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ImportSourceCredentialsOutput, body, alloc);
}
