...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCommandExecutionsForSandboxInput, options: Options) !ListCommandExecutionsForSandboxOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCommandExecutionsForSandboxInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeBuild_20161006.ListCommandExecutionsForSandbox");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCommandExecutionsForSandboxOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCommandExecutionsForSandboxOutput, body, alloc);
}
