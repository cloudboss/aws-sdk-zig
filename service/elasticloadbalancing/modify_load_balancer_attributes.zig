...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyLoadBalancerAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyLoadBalancerAttributes&Version=2012-06-01");
    if (input.load_balancer_attributes.access_log) |sv| {
        if (sv.emit_interval) |sv2| {
            try body_buf.appendSlice(alloc, "&LoadBalancerAttributes.AccessLog.EmitInterval=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{sv2}) catch "");
        }
        try body_buf.appendSlice(alloc, "&LoadBalancerAttributes.AccessLog.Enabled=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (sv.enabled) "true" else "false");
        if (sv.s3_bucket_name) |sv2| {
            try body_buf.appendSlice(alloc, "&LoadBalancerAttributes.AccessLog.S3BucketName=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv2);
        }
        if (sv.s3_bucket_prefix) |sv2| {
            try body_buf.appendSlice(alloc, "&LoadBalancerAttributes.AccessLog.S3BucketPrefix=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv2);
        }
    }
    ...
    if (input.load_balancer_attributes.connection_draining) |sv| {
        try body_buf.appendSlice(alloc, "&LoadBalancerAttributes.ConnectionDraining.Enabled=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (sv.enabled) "true" else "false");
        if (sv.timeout) |sv2| {
            try body_buf.appendSlice(alloc, "&LoadBalancerAttributes.ConnectionDraining.Timeout=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{sv2}) catch "");
        }
    }
    if (input.load_balancer_attributes.connection_settings) |sv| {
        try body_buf.appendSlice(alloc, "&LoadBalancerAttributes.ConnectionSettings.IdleTimeout=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{sv.idle_timeout}) catch "");
    }
    if (input.load_balancer_attributes.cross_zone_load_balancing) |sv| {
        try body_buf.appendSlice(alloc, "&LoadBalancerAttributes.CrossZoneLoadBalancing.Enabled=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (sv.enabled) "true" else "false");
    }
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LoadBalancerAttributes")) {
                    result.load_balancer_attributes = try serde.deserializeLoadBalancerAttributes(&reader, alloc);
