...
pub const AttachLoadBalancerToSubnetsInput = struct {
...
pub const AttachLoadBalancerToSubnetsOutput = struct {
    /// The IDs of the subnets attached to the load balancer.
    subnets: ?[]const []const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachLoadBalancerToSubnetsInput, options: CallOptions) !AttachLoadBalancerToSubnetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AttachLoadBalancerToSubnetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AttachLoadBalancerToSubnets&Version=2012-06-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AttachLoadBalancerToSubnetsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AttachLoadBalancerToSubnetsResult")) break;
            },
            else => {},
        }
    }

    var result: AttachLoadBalancerToSubnetsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
