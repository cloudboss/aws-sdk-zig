...
pub const DescribeLoadBalancerPoliciesInput = struct {
...
pub const DescribeLoadBalancerPoliciesOutput = struct {
    /// Information about the policies.
    policy_descriptions: ?[]const PolicyDescription = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLoadBalancerPoliciesInput, options: CallOptions) !DescribeLoadBalancerPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeLoadBalancerPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeLoadBalancerPolicies&Version=2012-06-01");
    if (input.load_balancer_name) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeLoadBalancerPoliciesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeLoadBalancerPoliciesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeLoadBalancerPoliciesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PolicyDescriptions")) {
                    result.policy_descriptions = try serde.deserializePolicyDescriptions(allocator, &reader, "member");
                } else {
