...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLoadBalancerPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeLoadBalancerPolicies&Version=2012-06-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLoadBalancerPoliciesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PolicyDescriptions")) {
                    result.policy_descriptions = try serde.deserializePolicyDescriptions(&reader, alloc, "member");
                } else {
