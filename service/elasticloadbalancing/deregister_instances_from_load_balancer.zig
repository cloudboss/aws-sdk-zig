...
pub const DeregisterInstancesFromLoadBalancerInput = struct {
...
pub const DeregisterInstancesFromLoadBalancerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterInstancesFromLoadBalancerInput, options: Options) !DeregisterInstancesFromLoadBalancerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterInstancesFromLoadBalancerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeregisterInstancesFromLoadBalancer&Version=2012-06-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterInstancesFromLoadBalancerOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeregisterInstancesFromLoadBalancerResult")) break;
            },
            else => {},
        }
    }

    var result: DeregisterInstancesFromLoadBalancerOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Instances")) {
                    result.instances = try serde.deserializeInstances(&reader, alloc, "member");
                } else {
