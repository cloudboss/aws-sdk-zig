...
pub const SetLoadBalancerPoliciesForBackendServerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetLoadBalancerPoliciesForBackendServerInput, options: Options) !SetLoadBalancerPoliciesForBackendServerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetLoadBalancerPoliciesForBackendServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=SetLoadBalancerPoliciesForBackendServer&Version=2012-06-01");
    try body_buf.appendSlice(allocator, "&InstancePort=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, std.fmt.allocPrint(allocator, "{d}", .{input.instance_port}) catch "");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetLoadBalancerPoliciesForBackendServerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: SetLoadBalancerPoliciesForBackendServerOutput = .{};
