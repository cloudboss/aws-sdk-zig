...
fn serializeRequest(alloc: std.mem.Allocator, input: SetLoadBalancerPoliciesForBackendServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetLoadBalancerPoliciesForBackendServer&Version=2012-06-01");
    try body_buf.appendSlice(alloc, "&InstancePort=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.instance_port}) catch "");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetLoadBalancerPoliciesForBackendServerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
