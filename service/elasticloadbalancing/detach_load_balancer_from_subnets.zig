...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachLoadBalancerFromSubnetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DetachLoadBalancerFromSubnets&Version=2012-06-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachLoadBalancerFromSubnetsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Subnets")) {
                    result.subnets = try serde.deserializeSubnets(&reader, alloc, "member");
                } else {
