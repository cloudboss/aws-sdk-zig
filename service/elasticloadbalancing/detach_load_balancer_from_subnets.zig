...
pub const DetachLoadBalancerFromSubnetsInput = struct {
...
pub const DetachLoadBalancerFromSubnetsOutput = struct {
    /// The IDs of the remaining subnets for the load balancer.
    subnets: ?[]const []const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachLoadBalancerFromSubnetsInput, options: CallOptions) !DetachLoadBalancerFromSubnetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DetachLoadBalancerFromSubnetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DetachLoadBalancerFromSubnets&Version=2012-06-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DetachLoadBalancerFromSubnetsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DetachLoadBalancerFromSubnetsResult")) break;
            },
            else => {},
        }
    }

    var result: DetachLoadBalancerFromSubnetsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Subnets")) {
                    result.subnets = try serde.deserializeSubnets(allocator, &reader, "member");
                } else {
