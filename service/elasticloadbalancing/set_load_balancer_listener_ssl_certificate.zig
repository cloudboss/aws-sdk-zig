...
pub const SetLoadBalancerListenerSSLCertificateInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetLoadBalancerListenerSSLCertificateInput, options: Options) !SetLoadBalancerListenerSSLCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetLoadBalancerListenerSSLCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=SetLoadBalancerListenerSSLCertificate&Version=2012-06-01");
    ...
    try body_buf.appendSlice(allocator, "&LoadBalancerPort=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, std.fmt.allocPrint(allocator, "{d}", .{input.load_balancer_port}) catch "");
    try body_buf.appendSlice(allocator, "&SSLCertificateId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.ssl_certificate_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetLoadBalancerListenerSSLCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: SetLoadBalancerListenerSSLCertificateOutput = .{};
