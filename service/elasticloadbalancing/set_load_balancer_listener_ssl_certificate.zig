...
fn serializeRequest(alloc: std.mem.Allocator, input: SetLoadBalancerListenerSSLCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetLoadBalancerListenerSSLCertificate&Version=2012-06-01");
    try body_buf.appendSlice(alloc, "&LoadBalancerName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.load_balancer_name);
    try body_buf.appendSlice(alloc, "&LoadBalancerPort=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.load_balancer_port}) catch "");
    try body_buf.appendSlice(alloc, "&SSLCertificateId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.ssl_certificate_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetLoadBalancerListenerSSLCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
