...
pub const DescribeLoadBalancerPolicyTypesInput = struct {
    /// The names of the policy types. If no names are specified, describes all
    /// policy types defined by Elastic Load Balancing.
    policy_type_names: ?[]const []const u8 = null,
};

pub const DescribeLoadBalancerPolicyTypesOutput = struct {
    /// Information about the policy types.
    policy_type_descriptions: ?[]const PolicyTypeDescription = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLoadBalancerPolicyTypesInput, options: CallOptions) !DescribeLoadBalancerPolicyTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeLoadBalancerPolicyTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeLoadBalancerPolicyTypes&Version=2012-06-01");
    if (input.policy_type_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&PolicyTypeNames.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeLoadBalancerPolicyTypesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeLoadBalancerPolicyTypesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeLoadBalancerPolicyTypesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PolicyTypeDescriptions")) {
                    result.policy_type_descriptions = try serde.deserializePolicyTypeDescriptions(allocator, &reader, "member");
                } else {
