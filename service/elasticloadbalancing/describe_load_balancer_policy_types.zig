...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLoadBalancerPolicyTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeLoadBalancerPolicyTypes&Version=2012-06-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLoadBalancerPolicyTypesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PolicyTypeDescriptions")) {
                    result.policy_type_descriptions = try serde.deserializePolicyTypeDescriptions(&reader, alloc, "member");
                } else {
