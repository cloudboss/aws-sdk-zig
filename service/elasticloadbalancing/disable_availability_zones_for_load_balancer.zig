...
pub const DisableAvailabilityZonesForLoadBalancerInput = struct {
...
pub const DisableAvailabilityZonesForLoadBalancerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableAvailabilityZonesForLoadBalancerInput, options: Options) !DisableAvailabilityZonesForLoadBalancerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableAvailabilityZonesForLoadBalancerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisableAvailabilityZonesForLoadBalancer&Version=2012-06-01");
    for (input.availability_zones, 0..) |item, idx| {
        const n = idx + 1;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableAvailabilityZonesForLoadBalancerOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DisableAvailabilityZonesForLoadBalancerResult")) break;
            },
            else => {},
        }
    }

    var result: DisableAvailabilityZonesForLoadBalancerOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AvailabilityZones")) {
                    result.availability_zones = try serde.deserializeAvailabilityZones(&reader, alloc, "member");
                } else {
