...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInstanceHealth&Version=2012-06-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceHealthOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InstanceStates")) {
                    result.instance_states = try serde.deserializeInstanceStates(&reader, alloc, "member");
                } else {
