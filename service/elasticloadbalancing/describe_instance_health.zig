...
pub const DescribeInstanceHealthInput = struct {
...
pub const DescribeInstanceHealthOutput = struct {
    /// Information about the health of the instances.
    instance_states: ?[]const InstanceState = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceHealthInput, options: CallOptions) !DescribeInstanceHealthOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstanceHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstanceHealth&Version=2012-06-01");
    if (input.instances) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstanceHealthOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeInstanceHealthResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeInstanceHealthOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InstanceStates")) {
                    result.instance_states = try serde.deserializeInstanceStates(allocator, &reader, "member");
                } else {
