...
pub const ConfigureHealthCheckInput = struct {
...
pub const ConfigureHealthCheckOutput = struct {
    /// The updated health check.
    health_check: ?HealthCheck = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfigureHealthCheckInput, options: CallOptions) !ConfigureHealthCheckOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ConfigureHealthCheckOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ConfigureHealthCheckResult")) break;
            },
            else => {},
        }
    }

    var result: ConfigureHealthCheckOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
