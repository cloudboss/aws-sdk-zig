...
pub const DeleteLoadBalancerListenersInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLoadBalancerListenersInput, options: Options) !DeleteLoadBalancerListenersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteLoadBalancerListenersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteLoadBalancerListeners&Version=2012-06-01");
    ...
    for (input.load_balancer_ports, 0..) |item, idx| {
        const n = idx + 1;
        var prefix_buf: [256]u8 = undefined;
        const field_prefix = std.fmt.bufPrint(&prefix_buf, "&LoadBalancerPorts.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteLoadBalancerListenersOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteLoadBalancerListenersOutput = .{};
