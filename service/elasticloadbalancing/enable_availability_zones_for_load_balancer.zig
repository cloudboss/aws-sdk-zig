...
pub const EnableAvailabilityZonesForLoadBalancerInput = struct {
...
pub const EnableAvailabilityZonesForLoadBalancerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableAvailabilityZonesForLoadBalancerInput, options: Options) !EnableAvailabilityZonesForLoadBalancerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableAvailabilityZonesForLoadBalancerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableAvailabilityZonesForLoadBalancer&Version=2012-06-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableAvailabilityZonesForLoadBalancerOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EnableAvailabilityZonesForLoadBalancerResult")) break;
            },
            else => {},
        }
    }

    var result: EnableAvailabilityZonesForLoadBalancerOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
