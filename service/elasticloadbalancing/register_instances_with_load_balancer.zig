...
pub const RegisterInstancesWithLoadBalancerInput = struct {
...
pub const RegisterInstancesWithLoadBalancerOutput = struct {
    /// The updated list of instances for the load balancer.
    instances: ?[]const Instance = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterInstancesWithLoadBalancerInput, options: CallOptions) !RegisterInstancesWithLoadBalancerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterInstancesWithLoadBalancerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RegisterInstancesWithLoadBalancer&Version=2012-06-01");
    for (input.instances, 0..) |item, idx| {
        const n = idx + 1;
        {
            var prefix_buf: [256]u8 = undefined;
            if (item.instance_id) |fv_1| {
                const field_prefix = std.fmt.bufPrint(&prefix_buf, "&Instances.member.{d}.InstanceId=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterInstancesWithLoadBalancerOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RegisterInstancesWithLoadBalancerResult")) break;
            },
            else => {},
        }
    }

    var result: RegisterInstancesWithLoadBalancerOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Instances")) {
                    result.instances = try serde.deserializeInstances(allocator, &reader, "member");
                } else {
