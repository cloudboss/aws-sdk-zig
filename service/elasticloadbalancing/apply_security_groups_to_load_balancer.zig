...
fn serializeRequest(alloc: std.mem.Allocator, input: ApplySecurityGroupsToLoadBalancerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ApplySecurityGroupsToLoadBalancer&Version=2012-06-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ApplySecurityGroupsToLoadBalancerOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SecurityGroups")) {
                    result.security_groups = try serde.deserializeSecurityGroups(&reader, alloc, "member");
                } else {
