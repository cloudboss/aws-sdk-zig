...
pub const ApplySecurityGroupsToLoadBalancerInput = struct {
...
pub const ApplySecurityGroupsToLoadBalancerOutput = struct {
    /// The IDs of the security groups associated with the load balancer.
    security_groups: ?[]const []const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ApplySecurityGroupsToLoadBalancerInput, options: CallOptions) !ApplySecurityGroupsToLoadBalancerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ApplySecurityGroupsToLoadBalancerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ApplySecurityGroupsToLoadBalancer&Version=2012-06-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ApplySecurityGroupsToLoadBalancerOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ApplySecurityGroupsToLoadBalancerResult")) break;
            },
            else => {},
        }
    }

    var result: ApplySecurityGroupsToLoadBalancerOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SecurityGroups")) {
                    result.security_groups = try serde.deserializeSecurityGroups(allocator, &reader, "member");
                } else {
