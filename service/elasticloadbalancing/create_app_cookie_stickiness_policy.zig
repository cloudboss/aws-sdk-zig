...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAppCookieStickinessPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateAppCookieStickinessPolicy&Version=2012-06-01");
    try body_buf.appendSlice(alloc, "&CookieName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cookie_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAppCookieStickinessPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
