...
const UpdateBackendAuthResourceConfig = @import("update_backend_auth_resource_config.zig").UpdateBackendAuthResourceConfig;

pub const UpdateBackendAuthInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const UpdateBackendAuthOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBackendAuthInput, options: CallOptions) !UpdateBackendAuthOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateBackendAuthInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateBackendAuthOutput {
    var result: UpdateBackendAuthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBackendAuthOutput, body, allocator);
    }
    _ = status;
    _ = headers;
