...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBackendConfigInput, options: Options) !UpdateBackendConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBackendConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/config/update");
    ...
    if (input.login_auth_config) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"LoginAuthConfig\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBackendConfigOutput {
    var result: UpdateBackendConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBackendConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
