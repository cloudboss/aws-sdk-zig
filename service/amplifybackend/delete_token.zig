...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTokenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTokenOutput {
    var result: DeleteTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteTokenOutput, body, allocator);
    }
    _ = status;
    _ = headers;
