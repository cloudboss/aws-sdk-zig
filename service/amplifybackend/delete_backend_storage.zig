...
pub const DeleteBackendStorageInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const DeleteBackendStorageOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBackendStorageInput, options: CallOptions) !DeleteBackendStorageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBackendStorageInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.service_name), input.service_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBackendStorageOutput {
    var result: DeleteBackendStorageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBackendStorageOutput, body, allocator);
    }
    _ = status;
    _ = headers;
