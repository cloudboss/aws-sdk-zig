...
pub const DeleteBackendInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const DeleteBackendOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBackendInput, options: CallOptions) !DeleteBackendOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBackendInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBackendOutput {
    var result: DeleteBackendOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBackendOutput, body, allocator);
    }
    _ = status;
    _ = headers;
