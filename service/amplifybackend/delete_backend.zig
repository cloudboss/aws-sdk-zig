...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBackendInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBackendOutput {
    var result: DeleteBackendOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBackendOutput, body, alloc);
    }
    _ = status;
    _ = headers;
