...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBackendInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"AppName\":");
    try aws.json.writeValue(@TypeOf(input.app_name), input.app_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBackendOutput {
    var result: CreateBackendOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBackendOutput, body, alloc);
    }
    _ = status;
    _ = headers;
