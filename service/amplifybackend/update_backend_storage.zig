...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBackendStorageInput, options: CallOptions) !UpdateBackendStorageOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "amplifybackend");
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateBackendStorageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateBackendStorageOutput {
    var result: UpdateBackendStorageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBackendStorageOutput, body, allocator);
    }
    _ = status;
    _ = headers;
