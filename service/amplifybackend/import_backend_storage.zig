...
pub const ImportBackendStorageInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const ImportBackendStorageOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportBackendStorageInput, options: CallOptions) !ImportBackendStorageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ImportBackendStorageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ImportBackendStorageOutput {
    var result: ImportBackendStorageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportBackendStorageOutput, body, allocator);
    }
    _ = status;
    _ = headers;
