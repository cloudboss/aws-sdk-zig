...
pub const GetBackendAPIInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const GetBackendAPIOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackendAPIInput, options: CallOptions) !GetBackendAPIOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBackendAPIInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBackendAPIOutput {
    var result: GetBackendAPIOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackendAPIOutput, body, allocator);
    }
    _ = status;
    _ = headers;
