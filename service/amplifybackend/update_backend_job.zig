...
pub const UpdateBackendJobInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const UpdateBackendJobOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBackendJobInput, options: Options) !UpdateBackendJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBackendJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBackendJobOutput {
    var result: UpdateBackendJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBackendJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
