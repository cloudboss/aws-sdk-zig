...
pub const CreateBackendAPIInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const CreateBackendAPIOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBackendAPIInput, options: CallOptions) !CreateBackendAPIOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateBackendAPIInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateBackendAPIOutput {
    var result: CreateBackendAPIOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBackendAPIOutput, body, allocator);
    }
    _ = status;
    _ = headers;
