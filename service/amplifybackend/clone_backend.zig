...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CloneBackendInput, options: CallOptions) !CloneBackendOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CloneBackendInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/clone");
    ...
    try body_buf.appendSlice(allocator, "\"TargetEnvironmentName\":");
    try aws.json.writeValue(@TypeOf(input.target_environment_name), input.target_environment_name, allocator, &body_buf);
    has_prev = true;
