...
const CreateBackendAuthResourceConfig = @import("create_backend_auth_resource_config.zig").CreateBackendAuthResourceConfig;

pub const GetBackendAuthInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const GetBackendAuthOutput = struct {
    /// The app ID.
    ...
    /// Amplify project.
    resource_config: ?CreateBackendAuthResourceConfig = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackendAuthInput, options: CallOptions) !GetBackendAuthOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBackendAuthInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBackendAuthOutput {
    var result: GetBackendAuthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackendAuthOutput, body, allocator);
    }
    _ = status;
    _ = headers;
