...
const BackendAPIResourceConfig = @import("backend_api_resource_config.zig").BackendAPIResourceConfig;

pub const DeleteBackendAPIInput = struct {
    /// The app ID.
    app_id: []const u8,
    ...
    /// project.
    resource_config: ?BackendAPIResourceConfig = null,
...
pub const DeleteBackendAPIOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBackendAPIInput, options: CallOptions) !DeleteBackendAPIOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBackendAPIInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBackendAPIOutput {
    var result: DeleteBackendAPIOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBackendAPIOutput, body, allocator);
    }
    _ = status;
    _ = headers;
