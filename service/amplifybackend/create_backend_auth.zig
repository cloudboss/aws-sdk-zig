...
pub const CreateBackendAuthInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const CreateBackendAuthOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBackendAuthInput, options: CallOptions) !CreateBackendAuthOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateBackendAuthInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateBackendAuthOutput {
    var result: CreateBackendAuthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBackendAuthOutput, body, allocator);
    }
    _ = status;
    _ = headers;
