...
pub const UpdateBackendAPIInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const UpdateBackendAPIOutput = struct {
    /// The app ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBackendAPIInput, options: CallOptions) !UpdateBackendAPIOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateBackendAPIInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateBackendAPIOutput {
    var result: UpdateBackendAPIOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBackendAPIOutput, body, allocator);
    }
    _ = status;
    _ = headers;
