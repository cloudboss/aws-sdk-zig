...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTokenInput, options: Options) !GetTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTokenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTokenOutput {
    var result: GetTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
