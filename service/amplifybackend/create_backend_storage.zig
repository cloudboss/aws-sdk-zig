...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBackendStorageInput, options: Options) !CreateBackendStorageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBackendStorageInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BackendEnvironmentName\":");
    try aws.json.writeValue(@TypeOf(input.backend_environment_name), input.backend_environment_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBackendStorageOutput {
    var result: CreateBackendStorageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBackendStorageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
