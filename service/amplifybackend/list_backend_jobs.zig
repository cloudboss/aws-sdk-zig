...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBackendJobsInput, options: Options) !ListBackendJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBackendJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBackendJobsOutput {
    var result: ListBackendJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBackendJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
