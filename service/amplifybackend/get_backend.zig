...
pub const GetBackendInput = struct {
    /// The app ID.
    app_id: []const u8,
...
pub const GetBackendOutput = struct {
    /// A stringified version of the cli.json file for your Amplify project.
    amplify_feature_flags: ?[]const u8 = null,

    /// A stringified version of the current configs for your Amplify project.
    amplify_meta_config: ?[]const u8 = null,
    ...
    backend_environment_list: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackendInput, options: CallOptions) !GetBackendOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBackendInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.backend_environment_name) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBackendOutput {
    var result: GetBackendOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackendOutput, body, allocator);
    }
    _ = status;
    _ = headers;
