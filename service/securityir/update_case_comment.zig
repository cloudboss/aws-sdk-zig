...
pub const UpdateCaseCommentInput = struct {
...
pub const UpdateCaseCommentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCaseCommentInput, options: CallOptions) !UpdateCaseCommentOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "security-ir");
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateCaseCommentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/update-case-comment/");
    try path_buf.appendSlice(allocator, input.comment_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateCaseCommentOutput {
    var result: UpdateCaseCommentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCaseCommentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
