...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCaseCommentInput, options: Options) !UpdateCaseCommentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCaseCommentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/update-case-comment/");
    try path_buf.appendSlice(alloc, input.comment_id);
    ...
    try body_buf.appendSlice(alloc, "\"body\":");
    try aws.json.writeValue(@TypeOf(input.body), input.body, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCaseCommentOutput {
    var result: UpdateCaseCommentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCaseCommentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
