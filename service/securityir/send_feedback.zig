...
fn serializeRequest(alloc: std.mem.Allocator, input: SendFeedbackInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.result_id);
    try path_buf.appendSlice(alloc, "/send-feedback");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"usefulness\":");
    try aws.json.writeValue(@TypeOf(input.usefulness), input.usefulness, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendFeedbackOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
