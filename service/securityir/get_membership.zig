...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMembershipInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMembershipOutput {
    var result: GetMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMembershipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
