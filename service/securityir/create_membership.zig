...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cover_entire_organization) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"coverEntireOrganization\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"incidentResponseTeam\":");
    try aws.json.writeValue(@TypeOf(input.incident_response_team), input.incident_response_team, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"membershipName\":");
    try aws.json.writeValue(@TypeOf(input.membership_name), input.membership_name, allocator, &body_buf);
    has_prev = true;
