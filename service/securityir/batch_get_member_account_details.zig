...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetMemberAccountDetailsInput, options: Options) !BatchGetMemberAccountDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetMemberAccountDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batch-member-details");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetMemberAccountDetailsOutput {
    var result: BatchGetMemberAccountDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetMemberAccountDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
