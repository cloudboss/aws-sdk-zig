...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResolverTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/update-resolver-type");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResolverTypeOutput {
    var result: UpdateResolverTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResolverTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
