...
pub const UpdateResolverTypeInput = struct {
...
pub const UpdateResolverTypeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResolverTypeInput, options: CallOptions) !UpdateResolverTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateResolverTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/update-resolver-type");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateResolverTypeOutput {
    var result: UpdateResolverTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResolverTypeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
