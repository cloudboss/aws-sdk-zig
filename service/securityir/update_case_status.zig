...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCaseStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/update-case-status");
    ...
    try body_buf.appendSlice(alloc, "\"caseStatus\":");
    try aws.json.writeValue(@TypeOf(input.case_status), input.case_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCaseStatusOutput {
    var result: UpdateCaseStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCaseStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
