...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCasesInput, options: Options) !ListCasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCasesOutput {
    var result: ListCasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
