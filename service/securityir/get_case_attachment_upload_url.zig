...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCaseAttachmentUploadUrlInput, options: Options) !GetCaseAttachmentUploadUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCaseAttachmentUploadUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/get-presigned-url");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"contentLength\":");
    try aws.json.writeValue(@TypeOf(input.content_length), input.content_length, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCaseAttachmentUploadUrlOutput {
    var result: GetCaseAttachmentUploadUrlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCaseAttachmentUploadUrlOutput, body, alloc);
    }
    _ = status;
    _ = headers;
