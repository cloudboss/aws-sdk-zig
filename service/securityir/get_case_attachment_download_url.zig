...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCaseAttachmentDownloadUrlInput, options: Options) !GetCaseAttachmentDownloadUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCaseAttachmentDownloadUrlInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCaseAttachmentDownloadUrlOutput {
    var result: GetCaseAttachmentDownloadUrlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCaseAttachmentDownloadUrlOutput, body, alloc);
    }
    _ = status;
    _ = headers;
