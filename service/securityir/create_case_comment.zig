...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCaseCommentInput, options: Options) !CreateCaseCommentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCaseCommentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/create-comment");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCaseCommentOutput {
    var result: CreateCaseCommentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCaseCommentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
