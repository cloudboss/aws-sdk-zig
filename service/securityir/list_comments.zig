...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCommentsInput, options: Options) !ListCommentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCommentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/list-comments");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCommentsOutput {
    var result: ListCommentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCommentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
