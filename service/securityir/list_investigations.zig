...
const InvestigationAction = @import("investigation_action.zig").InvestigationAction;
...
pub const ListInvestigationsOutput = struct {
    ...
    investigation_actions: ?[]const InvestigationAction = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInvestigationsInput, options: Options) !ListInvestigationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInvestigationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/list-investigations");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInvestigationsOutput {
    var result: ListInvestigationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInvestigationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
