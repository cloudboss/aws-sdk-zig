...
pub const CancelMembershipInput = struct {
...
pub const CancelMembershipOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMembershipInput, options: Options) !CancelMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelMembershipInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelMembershipOutput {
    var result: CancelMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelMembershipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
