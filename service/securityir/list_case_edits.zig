...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCaseEditsInput, options: Options) !ListCaseEditsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCaseEditsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/list-case-edits");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCaseEditsOutput {
    var result: ListCaseEditsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCaseEditsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
