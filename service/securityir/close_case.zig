...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CloseCaseInput, options: Options) !CloseCaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CloseCaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/close-case");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CloseCaseOutput {
    var result: CloseCaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CloseCaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
