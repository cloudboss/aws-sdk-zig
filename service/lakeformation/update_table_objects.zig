...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTableObjectsInput, options: Options) !UpdateTableObjectsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTableObjectsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.transaction_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TransactionId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"WriteOperations\":");
    try aws.json.writeValue(@TypeOf(input.write_operations), input.write_operations, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTableObjectsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
