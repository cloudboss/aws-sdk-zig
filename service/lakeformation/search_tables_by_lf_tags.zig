...
const TaggedTable = @import("tagged_table.zig").TaggedTable;

pub const SearchTablesByLFTagsInput = struct {
...
pub const SearchTablesByLFTagsOutput = struct {
    ...
    table_list: ?[]const TaggedTable = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchTablesByLFTagsInput, options: CallOptions) !SearchTablesByLFTagsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchTablesByLFTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/SearchTablesByLFTags";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchTablesByLFTagsOutput {
    var result: SearchTablesByLFTagsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchTablesByLFTagsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
