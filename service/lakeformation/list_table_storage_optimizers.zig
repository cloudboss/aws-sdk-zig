...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTableStorageOptimizersInput, options: Options) !ListTableStorageOptimizersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTableStorageOptimizersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.storage_optimizer_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"StorageOptimizerType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTableStorageOptimizersOutput {
    var result: ListTableStorageOptimizersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTableStorageOptimizersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
