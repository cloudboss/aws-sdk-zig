...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTransactionInput, options: Options) !StartTransactionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartTransactionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.transaction_type) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"TransactionType\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartTransactionOutput {
    var result: StartTransactionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartTransactionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
