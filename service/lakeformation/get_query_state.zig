...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQueryStateInput, options: Options) !GetQueryStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQueryStateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQueryStateOutput {
    var result: GetQueryStateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQueryStateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
