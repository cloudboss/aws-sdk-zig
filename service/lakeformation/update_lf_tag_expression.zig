...
pub const UpdateLFTagExpressionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLFTagExpressionInput, options: Options) !UpdateLFTagExpressionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLFTagExpressionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/UpdateLFTagExpression";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLFTagExpressionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateLFTagExpressionOutput = .{};
