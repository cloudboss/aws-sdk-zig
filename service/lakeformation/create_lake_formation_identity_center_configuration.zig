...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLakeFormationIdentityCenterConfigurationInput, options: Options) !CreateLakeFormationIdentityCenterConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLakeFormationIdentityCenterConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.instance_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InstanceArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLakeFormationIdentityCenterConfigurationOutput {
    var result: CreateLakeFormationIdentityCenterConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLakeFormationIdentityCenterConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
