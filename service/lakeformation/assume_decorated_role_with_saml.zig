...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssumeDecoratedRoleWithSAMLInput, options: Options) !AssumeDecoratedRoleWithSAMLOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssumeDecoratedRoleWithSAMLInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.saml_assertion), input.saml_assertion, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssumeDecoratedRoleWithSAMLOutput {
    var result: AssumeDecoratedRoleWithSAMLOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssumeDecoratedRoleWithSAMLOutput, body, alloc);
    }
    _ = status;
    _ = headers;
