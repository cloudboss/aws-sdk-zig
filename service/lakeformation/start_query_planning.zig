...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartQueryPlanningInput, options: Options) !StartQueryPlanningOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartQueryPlanningInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"QueryPlanningContext\":");
    try aws.json.writeValue(@TypeOf(input.query_planning_context), input.query_planning_context, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartQueryPlanningOutput {
    var result: StartQueryPlanningOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartQueryPlanningOutput, body, alloc);
    }
    _ = status;
    _ = headers;
