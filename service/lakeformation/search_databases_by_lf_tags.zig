...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchDatabasesByLFTagsInput, options: Options) !SearchDatabasesByLFTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchDatabasesByLFTagsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchDatabasesByLFTagsOutput {
    var result: SearchDatabasesByLFTagsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchDatabasesByLFTagsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
