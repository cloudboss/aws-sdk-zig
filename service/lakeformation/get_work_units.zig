...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkUnitsInput, options: Options) !GetWorkUnitsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkUnitsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkUnitsOutput {
    var result: GetWorkUnitsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkUnitsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
