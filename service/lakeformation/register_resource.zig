...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterResourceInput, options: Options) !RegisterResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.expected_resource_owner_account) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ExpectedResourceOwnerAccount\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.hybrid_access_enabled) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"HybridAccessEnabled\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.use_service_linked_role) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UseServiceLinkedRole\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.with_federation) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"WithFederation\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.with_privileged_access) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"WithPrivilegedAccess\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterResourceOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
