...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLFTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.resource_share_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ResourceShareType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLFTagsOutput {
    var result: ListLFTagsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLFTagsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
