...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLFTagExpressionInput, options: Options) !GetLFTagExpressionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLFTagExpressionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLFTagExpressionOutput {
    var result: GetLFTagExpressionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLFTagExpressionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
