...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataCellsFilterInput, options: Options) !GetDataCellsFilterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataCellsFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TableCatalogId\":");
    try aws.json.writeValue(@TypeOf(input.table_catalog_id), input.table_catalog_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataCellsFilterOutput {
    var result: GetDataCellsFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataCellsFilterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
