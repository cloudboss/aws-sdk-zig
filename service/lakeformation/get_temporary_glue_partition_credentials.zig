...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTemporaryGluePartitionCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Partition\":");
    try aws.json.writeValue(@TypeOf(input.partition), input.partition, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTemporaryGluePartitionCredentialsOutput {
    var result: GetTemporaryGluePartitionCredentialsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTemporaryGluePartitionCredentialsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
