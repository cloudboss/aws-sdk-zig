...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQueryStatisticsInput, options: Options) !GetQueryStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQueryStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQueryStatisticsOutput {
    var result: GetQueryStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQueryStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
