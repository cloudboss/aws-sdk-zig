...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataLakePrincipalInput, options: Options) !GetDataLakePrincipalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataLakePrincipalInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    const path = "/GetDataLakePrincipal";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataLakePrincipalOutput {
    var result: GetDataLakePrincipalOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataLakePrincipalOutput, body, alloc);
    }
    _ = status;
    _ = headers;
