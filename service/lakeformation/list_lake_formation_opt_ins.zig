...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLakeFormationOptInsInput, options: Options) !ListLakeFormationOptInsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLakeFormationOptInsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLakeFormationOptInsOutput {
    var result: ListLakeFormationOptInsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLakeFormationOptInsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
