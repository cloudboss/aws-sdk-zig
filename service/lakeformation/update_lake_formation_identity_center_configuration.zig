...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLakeFormationIdentityCenterConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.application_status) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ApplicationStatus\":");
    ...
    if (input.external_filtering) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ExternalFiltering\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.service_integrations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ServiceIntegrations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.share_recipients) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ShareRecipients\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLakeFormationIdentityCenterConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
