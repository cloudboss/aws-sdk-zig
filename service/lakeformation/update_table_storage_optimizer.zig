...
pub const UpdateTableStorageOptimizerInput = struct {
    ...
    storage_optimizer_config: []const aws.map.MapEntry([]const aws.map.StringMapEntry),
...
pub const UpdateTableStorageOptimizerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTableStorageOptimizerInput, options: Options) !UpdateTableStorageOptimizerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTableStorageOptimizerInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/UpdateTableStorageOptimizer";
    ...
    if (input.catalog_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"CatalogId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DatabaseName\":");
    try aws.json.writeValue(@TypeOf(input.database_name), input.database_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"StorageOptimizerConfig\":");
    try aws.json.writeValue(@TypeOf(input.storage_optimizer_config), input.storage_optimizer_config, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TableName\":");
    try aws.json.writeValue(@TypeOf(input.table_name), input.table_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTableStorageOptimizerOutput {
    var result: UpdateTableStorageOptimizerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTableStorageOptimizerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
