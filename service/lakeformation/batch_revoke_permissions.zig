...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchRevokePermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchRevokePermissionsOutput {
    var result: BatchRevokePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchRevokePermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
