...
pub const UpdateDataCellsFilterInput = struct {
    /// A `DataCellsFilter` structure containing information about the data cells
    /// filter.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDataCellsFilterInput, options: Options) !UpdateDataCellsFilterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDataCellsFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/UpdateDataCellsFilter";
    ...
    try body_buf.appendSlice(allocator, "\"TableData\":");
    try aws.json.writeValue(@TypeOf(input.table_data), input.table_data, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDataCellsFilterOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateDataCellsFilterOutput = .{};
