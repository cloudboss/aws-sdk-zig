...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataCellsFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"TableData\":");
    try aws.json.writeValue(@TypeOf(input.table_data), input.table_data, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataCellsFilterOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
