...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GrantPermissionsInput, options: Options) !GrantPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GrantPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.condition) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Condition\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Permissions\":");
    try aws.json.writeValue(@TypeOf(input.permissions), input.permissions, alloc, &body_buf);
    has_prev = true;
    if (input.permissions_with_grant_option) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PermissionsWithGrantOption\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GrantPermissionsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
