...
pub const PutDataLakeSettingsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDataLakeSettingsInput, options: Options) !PutDataLakeSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutDataLakeSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/PutDataLakeSettings";
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"DataLakeSettings\":");
    try aws.json.writeValue(@TypeOf(input.data_lake_settings), input.data_lake_settings, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutDataLakeSettingsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutDataLakeSettingsOutput = .{};
