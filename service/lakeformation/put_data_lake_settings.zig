...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDataLakeSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DataLakeSettings\":");
    try aws.json.writeValue(@TypeOf(input.data_lake_settings), input.data_lake_settings, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDataLakeSettingsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
