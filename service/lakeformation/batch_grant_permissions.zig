...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGrantPermissionsInput, options: CallOptions) !BatchGrantPermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchGrantPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchGrantPermissionsOutput {
    var result: BatchGrantPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGrantPermissionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
