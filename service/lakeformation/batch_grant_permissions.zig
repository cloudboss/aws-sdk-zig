...
const BatchPermissionsFailureEntry = @import("batch_permissions_failure_entry.zig").BatchPermissionsFailureEntry;

pub const BatchGrantPermissionsInput = struct {
    ...
    entries: []const BatchPermissionsRequestEntry,
...
pub const BatchGrantPermissionsOutput = struct {
    /// A list of failures to grant permissions to the resources.
    failures: ?[]const BatchPermissionsFailureEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGrantPermissionsInput, options: Options) !BatchGrantPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGrantPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/BatchGrantPermissions";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGrantPermissionsOutput {
    var result: BatchGrantPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGrantPermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
