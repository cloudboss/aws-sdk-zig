...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataCellsFilterInput, options: Options) !ListDataCellsFilterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataCellsFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.table) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Table\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataCellsFilterOutput {
    var result: ListDataCellsFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataCellsFilterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
