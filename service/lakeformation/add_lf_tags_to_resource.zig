...
fn serializeRequest(alloc: std.mem.Allocator, input: AddLFTagsToResourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddLFTagsToResourceOutput {
    var result: AddLFTagsToResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddLFTagsToResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
