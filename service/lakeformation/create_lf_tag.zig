...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLFTagInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"TagValues\":");
    try aws.json.writeValue(@TypeOf(input.tag_values), input.tag_values, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLFTagOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
