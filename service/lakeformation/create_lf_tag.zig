...
pub const CreateLFTagInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLFTagInput, options: Options) !CreateLFTagOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLFTagInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.tag_key), input.tag_key, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TagValues\":");
    try aws.json.writeValue(@TypeOf(input.tag_values), input.tag_values, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLFTagOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateLFTagOutput = .{};
