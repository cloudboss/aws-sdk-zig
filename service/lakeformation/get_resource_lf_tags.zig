...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceLFTagsInput, options: Options) !GetResourceLFTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceLFTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.show_assigned_lf_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ShowAssignedLFTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceLFTagsOutput {
    var result: GetResourceLFTagsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceLFTagsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
