...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTemporaryGlueTableCredentialsInput, options: CallOptions) !GetTemporaryGlueTableCredentialsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTemporaryGlueTableCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.query_session_context) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"QuerySessionContext\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.s3_path) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"S3Path\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTemporaryGlueTableCredentialsOutput {
    var result: GetTemporaryGlueTableCredentialsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTemporaryGlueTableCredentialsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
