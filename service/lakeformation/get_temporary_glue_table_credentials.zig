...
pub const GetTemporaryGlueTableCredentialsInput = struct {
...
pub const GetTemporaryGlueTableCredentialsOutput = struct {
    ...
    vended_s3_path: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTemporaryGlueTableCredentialsInput, options: Options) !GetTemporaryGlueTableCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTemporaryGlueTableCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetTemporaryGlueTableCredentials";
    ...
    if (input.audit_context) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AuditContext\":");
    ...
    if (input.query_session_context) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"QuerySessionContext\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.s3_path) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"S3Path\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.supported_permission_types) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SupportedPermissionTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TableArn\":");
    try aws.json.writeValue(@TypeOf(input.table_arn), input.table_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTemporaryGlueTableCredentialsOutput {
    var result: GetTemporaryGlueTableCredentialsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTemporaryGlueTableCredentialsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
