...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLFTagExpressionsInput, options: Options) !ListLFTagExpressionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLFTagExpressionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLFTagExpressionsOutput {
    var result: ListLFTagExpressionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLFTagExpressionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
