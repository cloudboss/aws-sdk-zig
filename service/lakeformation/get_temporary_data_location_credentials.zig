...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTemporaryDataLocationCredentialsInput, options: Options) !GetTemporaryDataLocationCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTemporaryDataLocationCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.credentials_scope) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"CredentialsScope\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.data_locations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DataLocations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTemporaryDataLocationCredentialsOutput {
    var result: GetTemporaryDataLocationCredentialsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTemporaryDataLocationCredentialsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
