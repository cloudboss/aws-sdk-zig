...
const DataLakeSettings = @import("data_lake_settings.zig").DataLakeSettings;

pub const GetDataLakeSettingsInput = struct {
...
pub const GetDataLakeSettingsOutput = struct {
    /// A structure representing a list of Lake Formation principals designated as
    /// data lake administrators.
    data_lake_settings: ?DataLakeSettings = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataLakeSettingsInput, options: Options) !GetDataLakeSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataLakeSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetDataLakeSettings";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataLakeSettingsOutput {
    var result: GetDataLakeSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataLakeSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
