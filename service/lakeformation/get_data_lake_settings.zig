...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataLakeSettingsInput, options: CallOptions) !GetDataLakeSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDataLakeSettingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDataLakeSettingsOutput {
    var result: GetDataLakeSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataLakeSettingsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
