...
pub const GetEffectivePermissionsForPathInput = struct {
...
pub const GetEffectivePermissionsForPathOutput = struct {
    /// A continuation token, if this is not the first call to retrieve this list.
    next_token: ?[]const u8 = null,

    /// A list of the permissions for the specified table or database resource
    /// located at the path in Amazon S3.
    permissions: ?[]const PrincipalResourcePermissions = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEffectivePermissionsForPathInput, options: Options) !GetEffectivePermissionsForPathOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEffectivePermissionsForPathInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetEffectivePermissionsForPath";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEffectivePermissionsForPathOutput {
    var result: GetEffectivePermissionsForPathOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEffectivePermissionsForPathOutput, body, alloc);
    }
    _ = status;
    _ = headers;
