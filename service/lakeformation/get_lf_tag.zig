...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLFTagInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLFTagOutput {
    var result: GetLFTagOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLFTagOutput, body, alloc);
    }
    _ = status;
    _ = headers;
