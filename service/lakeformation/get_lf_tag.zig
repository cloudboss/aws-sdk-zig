...
pub const GetLFTagInput = struct {
...
pub const GetLFTagOutput = struct {
    /// The identifier for the Data Catalog. By default, the account ID. The Data
    /// Catalog is the persistent metadata store. It contains database definitions,
    /// table definitions, and other control information to manage your Lake
    /// Formation environment.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLFTagInput, options: CallOptions) !GetLFTagOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLFTagInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLFTagOutput {
    var result: GetLFTagOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLFTagOutput, body, allocator);
    }
    _ = status;
    _ = headers;
