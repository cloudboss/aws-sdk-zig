...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLFTagInput, options: Options) !UpdateLFTagOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLFTagInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.tag_values_to_add) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TagValuesToAdd\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tag_values_to_delete) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TagValuesToDelete\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLFTagOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
