...
pub const DescribeTransactionInput = struct {
...
pub const DescribeTransactionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTransactionInput, options: Options) !DescribeTransactionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTransactionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/DescribeTransaction";
    ...
    try aws.json.writeValue(@TypeOf(input.transaction_id), input.transaction_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTransactionOutput {
    var result: DescribeTransactionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTransactionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
