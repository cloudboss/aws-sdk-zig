...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveLFTagsFromResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"LFTags\":");
    try aws.json.writeValue(@TypeOf(input.lf_tags), input.lf_tags, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveLFTagsFromResourceOutput {
    var result: RemoveLFTagsFromResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveLFTagsFromResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
