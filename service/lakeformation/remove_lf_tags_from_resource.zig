...
const LFTagError = @import("lf_tag_error.zig").LFTagError;

pub const RemoveLFTagsFromResourceInput = struct {
...
pub const RemoveLFTagsFromResourceOutput = struct {
    /// A list of failures to untag a resource.
    failures: ?[]const LFTagError = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveLFTagsFromResourceInput, options: CallOptions) !RemoveLFTagsFromResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveLFTagsFromResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/RemoveLFTagsFromResource";
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"LFTags\":");
    try aws.json.writeValue(@TypeOf(input.lf_tags), input.lf_tags, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveLFTagsFromResourceOutput {
    var result: RemoveLFTagsFromResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveLFTagsFromResourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
