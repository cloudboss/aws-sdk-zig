...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePackageImportJobInput, options: Options) !DescribePackageImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePackageImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/packages/import-jobs/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePackageImportJobOutput {
    var result: DescribePackageImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePackageImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
