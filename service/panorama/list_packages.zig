...
const PackageListItem = @import("package_list_item.zig").PackageListItem;
...
pub const ListPackagesOutput = struct {
    ...
    /// A list of packages.
    packages: ?[]const PackageListItem = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPackagesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPackagesOutput {
    var result: ListPackagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPackagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
