...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ProvisionDeviceInput, options: Options) !ProvisionDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ProvisionDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.networking_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"NetworkingConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ProvisionDeviceOutput {
    var result: ProvisionDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ProvisionDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
