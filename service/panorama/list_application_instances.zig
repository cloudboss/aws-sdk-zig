...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApplicationInstancesInput, options: Options) !ListApplicationInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApplicationInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.status_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "statusFilter=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApplicationInstancesOutput {
    var result: ListApplicationInstancesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListApplicationInstancesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
