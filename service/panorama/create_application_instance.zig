...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApplicationInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.application_instance_id_to_replace) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ApplicationInstanceIdToReplace\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DefaultRuntimeContextDevice\":");
    try aws.json.writeValue(@TypeOf(input.default_runtime_context_device), input.default_runtime_context_device, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.manifest_overrides_payload) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ManifestOverridesPayload\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ManifestPayload\":");
    try aws.json.writeValue(@TypeOf(input.manifest_payload), input.manifest_payload, alloc, &body_buf);
    has_prev = true;
    if (input.name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.runtime_role_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RuntimeRoleArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateApplicationInstanceOutput {
    var result: CreateApplicationInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateApplicationInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
