...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDevicesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.device_aggregated_status_filter) |v| {
        ...
        try query_buf.appendSlice(alloc, "DeviceAggregatedStatusFilter=");
    ...
    if (input.name_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "NameFilter=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDevicesOutput {
    var result: ListDevicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevicesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
