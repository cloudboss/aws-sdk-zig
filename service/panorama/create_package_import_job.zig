...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePackageImportJobInput, options: Options) !CreatePackageImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePackageImportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePackageImportJobOutput {
    var result: CreatePackageImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePackageImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
