...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeApplicationInstanceInput, options: Options) !DescribeApplicationInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeApplicationInstanceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeApplicationInstanceOutput {
    var result: DescribeApplicationInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeApplicationInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
