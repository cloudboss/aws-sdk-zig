...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateJobForDevicesInput, options: Options) !CreateJobForDevicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateJobForDevicesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.device_job_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DeviceJobConfig\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateJobForDevicesOutput {
    var result: CreateJobForDevicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateJobForDevicesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
