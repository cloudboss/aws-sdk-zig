...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePackageVersionInput, options: Options) !DescribePackageVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePackageVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/packages/metadata/");
    try path_buf.appendSlice(alloc, input.package_id);
    try path_buf.appendSlice(alloc, "/versions/");
    try path_buf.appendSlice(alloc, input.package_version);
    ...
    if (input.owner_account) |v| {
        ...
        try query_buf.appendSlice(alloc, "OwnerAccount=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.patch_version) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "PatchVersion=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePackageVersionOutput {
    var result: DescribePackageVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePackageVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
