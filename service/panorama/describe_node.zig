...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNodeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNodeOutput {
    var result: DescribeNodeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeNodeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
