...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDeviceJobInput, options: Options) !DescribeDeviceJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDeviceJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDeviceJobOutput {
    var result: DescribeDeviceJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDeviceJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
