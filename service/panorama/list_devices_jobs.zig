...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDevicesJobsInput, options: Options) !ListDevicesJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDevicesJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDevicesJobsOutput {
    var result: ListDevicesJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevicesJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
