...
const ExecutionReference = @import("execution_reference.zig").ExecutionReference;
...
pub const ListExecutionsOutput = struct {
    ...
    executions: ?[]const ExecutionReference = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExecutionsInput, options: CallOptions) !ListExecutionsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "bcm-data-exports");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSBillingAndCostManagementDataExports.ListExecutions");
