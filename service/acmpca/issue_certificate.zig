...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: IssueCertificateInput, options: Options) !IssueCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: IssueCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.IssueCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !IssueCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(IssueCertificateOutput, body, alloc);
}
