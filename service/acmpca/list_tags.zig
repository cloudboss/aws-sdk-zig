...
const ListTagsInput = @import("list_tags_request.zig").ListTagsRequest;

const ListTagsOutput = @import("list_tags_response.zig").ListTagsResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTagsInput, options: Options) !ListTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.ListTags");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTagsOutput {
    ...
    return aws.json.parseJsonObject(ListTagsOutput, body, alloc);
}
