...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCertificateAuthorityCertificateInput, options: Options) !GetCertificateAuthorityCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCertificateAuthorityCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.GetCertificateAuthorityCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCertificateAuthorityCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCertificateAuthorityCertificateOutput, body, alloc);
}
