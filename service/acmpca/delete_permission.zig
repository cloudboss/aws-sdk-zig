...
const DeletePermissionOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePermissionInput, options: Options) !DeletePermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.DeletePermission");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePermissionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
