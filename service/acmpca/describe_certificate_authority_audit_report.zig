...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCertificateAuthorityAuditReportInput, options: Options) !DescribeCertificateAuthorityAuditReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCertificateAuthorityAuditReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.DescribeCertificateAuthorityAuditReport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCertificateAuthorityAuditReportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCertificateAuthorityAuditReportOutput, body, alloc);
}
