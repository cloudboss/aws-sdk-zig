...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCertificateAuthorityInput, options: Options) !DescribeCertificateAuthorityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCertificateAuthorityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.DescribeCertificateAuthority");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCertificateAuthorityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCertificateAuthorityOutput, body, alloc);
}
