...
pub const TagCertificateAuthorityOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TagCertificateAuthorityInput, options: CallOptions) !TagCertificateAuthorityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TagCertificateAuthorityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ACMPrivateCA.TagCertificateAuthority");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TagCertificateAuthorityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
