...
pub const TagCertificateAuthorityInput = struct {
...
const TagCertificateAuthorityOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TagCertificateAuthorityInput, options: Options) !TagCertificateAuthorityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TagCertificateAuthorityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.TagCertificateAuthority");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TagCertificateAuthorityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
