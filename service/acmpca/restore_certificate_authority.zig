...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreCertificateAuthorityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.RestoreCertificateAuthority");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreCertificateAuthorityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
