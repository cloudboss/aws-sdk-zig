...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCertificateAuthorityCsrInput, options: CallOptions) !GetCertificateAuthorityCsrOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCertificateAuthorityCsrInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ACMPrivateCA.GetCertificateAuthorityCsr");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCertificateAuthorityCsrOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCertificateAuthorityCsrOutput, body, allocator);
}
