...
pub const GetCertificateAuthorityCsrInput = struct {
...
pub const GetCertificateAuthorityCsrOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCertificateAuthorityCsrInput, options: Options) !GetCertificateAuthorityCsrOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCertificateAuthorityCsrInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.GetCertificateAuthorityCsr");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCertificateAuthorityCsrOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCertificateAuthorityCsrOutput, body, alloc);
}
