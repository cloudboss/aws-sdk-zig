...
const CreatePermissionOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePermissionInput, options: Options) !CreatePermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.CreatePermission");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePermissionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
