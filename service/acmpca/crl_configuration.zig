const CrlDistributionPointExtensionConfiguration = @import("crl_distribution_point_extension_configuration.zig").CrlDistributionPointExtensionConfiguration;
const CrlType = @import("crl_type.zig").CrlType;
const S3ObjectAcl = @import("s3_object_acl.zig").S3ObjectAcl;
...
/// parameter. Your private CA by default copies the CNAME or the S3 bucket name
/// to the **CRL Distribution Points** extension of each certificate it issues.
/// If you want to configure this default behavior to be something different,
/// you can set the **CrlDistributionPointExtensionConfiguration** parameter.
/// Your S3 bucket policy must give write permission to Amazon Web Services
/// Private CA.
///
...
/// calculate the **nextUpdate** field in the CRL. The CRL is refreshed prior to
/// a certificate's expiration date or when a certificate is revoked. When a
...
/// (CRL)](https://docs.aws.amazon.com/privateca/latest/userguide/crl-planning.html) in the *Amazon Web Services Private Certificate Authority User Guide*
pub const CrlConfiguration = struct {
    ...
    ///   been revoked for any reason. Each certiﬁcate that Amazon Web Services
    ///   Private CA issues is bound to a speciﬁc CRL through its CRL distribution
    ///   point (CDP) extension, deﬁned in [ RFC
    ///   5280](https://datatracker.ietf.org/doc/html/rfc5280#section-4.2.1.9).
    ...
    /// CA when you call the
    /// [CreateCertificateAuthority](https://docs.aws.amazon.com/privateca/latest/APIReference/API_CreateCertificateAuthority.html) action or for an existing CA when you call the [UpdateCertificateAuthority](https://docs.aws.amazon.com/privateca/latest/APIReference/API_UpdateCertificateAuthority.html) action.
    enabled: bool,
    ...
    /// you have have enabled the Block Public Access (BPA) feature in your S3
