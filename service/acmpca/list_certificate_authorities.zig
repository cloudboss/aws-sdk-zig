...
pub const ListCertificateAuthoritiesInput = struct {
...
pub const ListCertificateAuthoritiesOutput = struct {
    /// Summary information about each certificate authority you have created.
    certificate_authorities: ?[]const CertificateAuthority = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCertificateAuthoritiesInput, options: Options) !ListCertificateAuthoritiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCertificateAuthoritiesInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("acmpca", "ACM PCA", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ACMPrivateCA.ListCertificateAuthorities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCertificateAuthoritiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCertificateAuthoritiesOutput, body, alloc);
}
