...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCertificateAuthoritiesInput, options: CallOptions) !ListCertificateAuthoritiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCertificateAuthoritiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ACMPrivateCA.ListCertificateAuthorities");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCertificateAuthoritiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCertificateAuthoritiesOutput, body, allocator);
}
