...
pub const UntagCertificateAuthorityOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UntagCertificateAuthorityInput, options: CallOptions) !UntagCertificateAuthorityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UntagCertificateAuthorityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ACMPrivateCA.UntagCertificateAuthority");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UntagCertificateAuthorityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
