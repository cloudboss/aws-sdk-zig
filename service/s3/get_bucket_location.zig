...
pub const GetBucketLocationInput = struct {
...
pub const GetBucketLocationOutput = struct {
    /// Specifies the Region where the bucket resides. For a list of all the Amazon
    /// S3 supported location
    /// constraints by Region, see [Regions and
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBucketLocationInput, options: CallOptions) !GetBucketLocationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBucketLocationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBucketLocationOutput {
    _ = allocator;
    var result: GetBucketLocationOutput = .{};
    _ = status;
