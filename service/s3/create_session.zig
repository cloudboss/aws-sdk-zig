...
pub const CreateSessionInput = struct {
    ...
    /// or
    /// `ReadOnly`. If no session mode is specified, the default behavior attempts
    /// to create
    /// a session with the maximum allowable privilege. It will first attempt to
    /// create a
    /// `ReadWrite` session, and if that is not allowed by permissions, it will
    /// attempt to create a
    /// `ReadOnly` session. If neither session type is allowed, the request will
    /// return an Access Denied error. A
