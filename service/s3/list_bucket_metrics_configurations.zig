...
pub const ListBucketMetricsConfigurationsInput = struct {
...
pub const ListBucketMetricsConfigurationsOutput = struct {
    /// The marker that is used as a starting point for this metrics configuration
    /// list response. This value
    ...
    /// that the list is not complete and the NextContinuationToken will be provided
    /// for a subsequent
    /// request.
    is_truncated: ?bool = null,

    /// The list of metrics configurations for a bucket.
    metrics_configuration_list: ?[]const MetricsConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBucketMetricsConfigurationsInput, options: CallOptions) !ListBucketMetricsConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBucketMetricsConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "metrics&x-id=ListBucketMetricsConfigurations");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBucketMetricsConfigurationsOutput {
    var result: ListBucketMetricsConfigurationsOutput = .{};
    _ = status;
    ...
    var metrics_configuration_list_list: std.ArrayList(MetricsConfiguration) = .empty;
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ContinuationToken")) {
                    ...
                    try metrics_configuration_list_list.append(allocator, try serde.deserializeMetricsConfiguration(allocator, &reader));
    ...
    result.metrics_configuration_list = if (metrics_configuration_list_list.items.len > 0) try metrics_configuration_list_list.toOwnedSlice(allocator) else null;
    _ = headers;
