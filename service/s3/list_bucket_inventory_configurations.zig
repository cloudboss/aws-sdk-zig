...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBucketInventoryConfigurationsInput, options: CallOptions) !ListBucketInventoryConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBucketInventoryConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "inventory&x-id=ListBucketInventoryConfigurations");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBucketInventoryConfigurationsOutput {
    ...
    var inventory_configuration_list_list: std.ArrayList(InventoryConfiguration) = .empty;
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ContinuationToken")) {
                    ...
                    try inventory_configuration_list_list.append(allocator, try serde.deserializeInventoryConfiguration(allocator, &reader));
    ...
    result.inventory_configuration_list = if (inventory_configuration_list_list.items.len > 0) try inventory_configuration_list_list.toOwnedSlice(allocator) else null;
    _ = headers;
