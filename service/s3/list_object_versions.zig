...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObjectVersionsInput, options: CallOptions) !ListObjectVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListObjectVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "versions");
    query_has_prev = true;
    ...
    if (input.version_id_marker) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "version-id-marker=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListObjectVersionsOutput {
    ...
    var delete_markers_list: std.ArrayList(DeleteMarkerEntry) = .empty;
    var versions_list: std.ArrayList(ObjectVersion) = .empty;
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CommonPrefixes")) {
                    ...
                    try delete_markers_list.append(allocator, try serde.deserializeDeleteMarkerEntry(allocator, &reader));
                    ...
                    result.next_version_id_marker = try allocator.dupe(u8, try reader.readElementText());
                    ...
                    result.version_id_marker = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "Version")) {
                    try versions_list.append(allocator, try serde.deserializeObjectVersion(allocator, &reader));
                } else {
    ...
    result.delete_markers = if (delete_markers_list.items.len > 0) try delete_markers_list.toOwnedSlice(allocator) else null;
    result.versions = if (versions_list.items.len > 0) try versions_list.toOwnedSlice(allocator) else null;
