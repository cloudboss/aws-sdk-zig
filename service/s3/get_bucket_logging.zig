...
pub const GetBucketLoggingOutput = struct {
    ...
    pub fn deinit(self: *GetBucketLoggingOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetBucketLoggingInput, options: Options) !GetBucketLoggingOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LoggingEnabled")) {
                    result.logging_enabled = try serde.deserializeLoggingEnabled(&reader, alloc);
                } else {
