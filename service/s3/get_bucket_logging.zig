...
pub const GetBucketLoggingOutput = struct {
    logging_enabled: ?LoggingEnabled = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBucketLoggingInput, options: CallOptions) !GetBucketLoggingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBucketLoggingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBucketLoggingOutput {
    var result: GetBucketLoggingOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LoggingEnabled")) {
                    result.logging_enabled = try serde.deserializeLoggingEnabled(allocator, &reader);
                } else {
