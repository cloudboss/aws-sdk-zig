...
pub const PutPublicAccessBlockInput = struct {
    ...
    /// more information
    /// about when Amazon S3 considers a bucket or object public, see [The Meaning
    /// of
    /// "Public"](https://docs.aws.amazon.com/AmazonS3/latest/dev/access-control-block-public-access.html#access-control-block-public-access-policy-status) in the *Amazon S3 User Guide*.
...
pub const PutPublicAccessBlockOutput = struct {
    ...
    pub fn deinit(self: *PutPublicAccessBlockOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: PutPublicAccessBlockInput, options: Options) !PutPublicAccessBlockOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutPublicAccessBlockInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "publicAccessBlock");
    query_has_prev = true;
    const query = try query_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<PublicAccessBlockConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
