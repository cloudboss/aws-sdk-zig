...
fn serializeRequest(allocator: std.mem.Allocator, input: PutPublicAccessBlockInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "publicAccessBlock");
    query_has_prev = true;
    const query = try query_buf.toOwnedSlice(allocator);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(allocator, "<PublicAccessBlockConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
