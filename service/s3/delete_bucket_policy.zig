...
pub const DeleteBucketPolicyOutput = struct {
    ...
    pub fn deinit(self: *const DeleteBucketPolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteBucketPolicyInput, options: Options) !DeleteBucketPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBucketPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBucketPolicyOutput {
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteBucketPolicyOutput = .{ .allocator = alloc };
