...
pub const DeleteBucketPolicyInput = struct {
    /// The bucket name.
    ///
...
pub const DeleteBucketPolicyOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBucketPolicyInput, options: CallOptions) !DeleteBucketPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBucketPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "policy");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBucketPolicyOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteBucketPolicyOutput = .{};
