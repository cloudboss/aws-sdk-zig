...
pub const AbortMultipartUploadInput = struct {
    ...
    /// provided timestamp. If the initiated timestamp of the multipart upload does
    /// not match the provided
    /// value, the operation returns a `412 Precondition Failed` error. If the
    /// initiated timestamp
    ...
    if_match_initiated_time: ?i64 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AbortMultipartUploadInput, options: CallOptions) !AbortMultipartUploadOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AbortMultipartUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "x-id=AbortMultipartUpload");
    query_has_prev = true;
    ...
    if (input.if_match_initiated_time) |v| {
        {
            const num_str = std.fmt.allocPrint(allocator, "{d}", .{v}) catch "";
            try request.headers.put(allocator, "x-amz-if-match-initiated-time", num_str);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AbortMultipartUploadOutput {
    _ = allocator;
