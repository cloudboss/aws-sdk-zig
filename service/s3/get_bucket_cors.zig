...
pub const GetBucketCorsOutput = struct {
    ...
    pub fn deinit(self: *const GetBucketCorsOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: GetBucketCorsInput, options: Options) !GetBucketCorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBucketCorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBucketCorsOutput {
    _ = body;
    _ = status;
    _ = headers;
    const result: GetBucketCorsOutput = .{ .allocator = alloc };
