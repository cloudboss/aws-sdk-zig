...
pub const GetBucketCorsInput = struct {
...
pub const GetBucketCorsOutput = struct {
    /// A set of origins and methods (cross-origin access that you want to allow).
    /// You can add up to 100
    /// rules to the configuration.
    cors_rules: ?[]const CORSRule = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBucketCorsInput, options: CallOptions) !GetBucketCorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBucketCorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "cors");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBucketCorsOutput {
    var result: GetBucketCorsOutput = .{};
    _ = status;
    ...
    var cors_rules_list: std.ArrayList(CORSRule) = .empty;
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CORSRule")) {
                    try cors_rules_list.append(allocator, try serde.deserializeCORSRule(allocator, &reader));
                } else {
    ...
    result.cors_rules = if (cors_rules_list.items.len > 0) try cors_rules_list.toOwnedSlice(allocator) else null;
    _ = headers;
