...
pub const GetBucketRequestPaymentInput = struct {
...
pub const GetBucketRequestPaymentOutput = struct {
    /// Specifies who pays for the download and request fees.
    payer: ?Payer = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBucketRequestPaymentInput, options: CallOptions) !GetBucketRequestPaymentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBucketRequestPaymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "requestPayment");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBucketRequestPaymentOutput {
    _ = allocator;
    var result: GetBucketRequestPaymentOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Payer")) {
                    result.payer = std.meta.stringToEnum(Payer, try reader.readElementText());
                } else {
