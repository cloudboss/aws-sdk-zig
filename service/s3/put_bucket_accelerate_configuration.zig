...
fn serializeRequest(allocator: std.mem.Allocator, input: PutBucketAccelerateConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "<AccelerateConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeAccelerateConfiguration(allocator, &body_buf, input.accelerate_configuration);
    try body_buf.appendSlice(allocator, "</AccelerateConfiguration>");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutBucketAccelerateConfigurationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
