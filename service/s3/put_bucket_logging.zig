...
pub const PutBucketLoggingInput = struct {
...
const PutBucketLoggingOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBucketLoggingInput, options: Options) !PutBucketLoggingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutBucketLoggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "logging");
    query_has_prev = true;
    const query = try query_buf.toOwnedSlice(allocator);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(allocator, "<BucketLoggingStatus xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeBucketLoggingStatus(allocator, &body_buf, input.bucket_logging_status);
    try body_buf.appendSlice(allocator, "</BucketLoggingStatus>");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutBucketLoggingOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutBucketLoggingOutput = .{};
