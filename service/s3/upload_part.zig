...
fn serializeRequest(allocator: std.mem.Allocator, input: UploadPartInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "x-id=UploadPart");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UploadPartOutput {
