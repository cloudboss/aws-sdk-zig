...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteObjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "x-id=DeleteObject");
    query_has_prev = true;
    ...
    if (input.if_match_last_modified_time) |v| {
        {
            ...
            try request.headers.put(allocator, "x-amz-if-match-last-modified-time", num_str);
        }
    }
    if (input.if_match_size) |v| {
        {
            const num_str = std.fmt.allocPrint(allocator, "{d}", .{v}) catch "";
            try request.headers.put(allocator, "x-amz-if-match-size", num_str);
        }
    }
    if (input.mfa) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteObjectOutput {
