...
pub const GetObjectAclInput = struct {
...
pub const GetObjectAclOutput = struct {
    /// A list of grants.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetObjectAclInput, options: CallOptions) !GetObjectAclOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetObjectAclInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetObjectAclOutput {
    var result: GetObjectAclOutput = .{};
    _ = status;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccessControlList")) {
                    result.grants = try serde.deserializeGrants(allocator, &reader, "Grant");
