...
pub const UpdateBucketMetadataInventoryTableConfigurationOutput = struct {
    ...
    pub fn deinit(self: *UpdateBucketMetadataInventoryTableConfigurationOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UpdateBucketMetadataInventoryTableConfigurationInput, options: Options) !UpdateBucketMetadataInventoryTableConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBucketMetadataInventoryTableConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "metadataInventoryTable");
    query_has_prev = true;
    const query = try query_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<InventoryTableConfigurationUpdates xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeInventoryTableConfigurationUpdates(alloc, &body_buf, input.inventory_table_configuration);
    try body_buf.appendSlice(alloc, "</InventoryTableConfigurationUpdates>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBucketMetadataInventoryTableConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
