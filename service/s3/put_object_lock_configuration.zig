...
pub const PutObjectLockConfigurationOutput = struct {
    request_charged: ?RequestCharged = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *PutObjectLockConfigurationOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: PutObjectLockConfigurationInput, options: Options) !PutObjectLockConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutObjectLockConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "object-lock");
    query_has_prev = true;
    ...
    const body: ?[]const u8 = blk: {
        if (input.object_lock_configuration) |payload| {
            ...
            try body_buf.appendSlice(alloc, "<ObjectLockConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
            try serde.serializeObjectLockConfiguration(alloc, &body_buf, payload);
            try body_buf.appendSlice(alloc, "</ObjectLockConfiguration>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutObjectLockConfigurationOutput {
    _ = alloc;
    var result: PutObjectLockConfigurationOutput = .{};
    _ = status;
    _ = body;
