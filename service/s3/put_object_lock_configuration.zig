...
fn serializeRequest(allocator: std.mem.Allocator, input: PutObjectLockConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const body: ?[]const u8 = blk: {
        if (input.object_lock_configuration) |payload| {
            ...
            try body_buf.appendSlice(allocator, "<ObjectLockConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    ...
    if (input.token) |v| {
        try request.headers.put(allocator, "x-amz-bucket-object-lock-token", v);
    }

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutObjectLockConfigurationOutput {
    _ = allocator;
