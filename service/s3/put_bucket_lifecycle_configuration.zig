...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBucketLifecycleConfigurationInput, options: Options) !PutBucketLifecycleConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutBucketLifecycleConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const body: ?[]const u8 = blk: {
        if (input.lifecycle_configuration) |payload| {
            ...
            try body_buf.appendSlice(allocator, "<BucketLifecycleConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
            try serde.serializeBucketLifecycleConfiguration(allocator, &body_buf, payload);
            try body_buf.appendSlice(allocator, "</BucketLifecycleConfiguration>");
