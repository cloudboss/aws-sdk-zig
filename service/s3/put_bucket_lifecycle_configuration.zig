...
pub fn execute(client: *Client, input: PutBucketLifecycleConfigurationInput, options: Options) !PutBucketLifecycleConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBucketLifecycleConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "lifecycle");
    query_has_prev = true;
    ...
    if (input.transition_default_minimum_object_size) |v| {
        try request.headers.put(alloc, "x-amz-transition-default-minimum-object-size", @tagName(v));
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !PutBucketLifecycleConfigurationOutput {
    _ = body;
    _ = status;
    const result: PutBucketLifecycleConfigurationOutput = .{ .allocator = alloc };
