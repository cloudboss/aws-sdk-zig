...
pub fn execute(client: *Client, input: GetBucketInventoryConfigurationInput, options: Options) !GetBucketInventoryConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBucketInventoryConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "inventory&x-id=GetBucketInventoryConfiguration");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetBucketInventoryConfigurationOutput {
    _ = body;
    _ = status;
