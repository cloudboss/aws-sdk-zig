...
const PutBucketOwnershipControlsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBucketOwnershipControlsInput, options: Options) !PutBucketOwnershipControlsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBucketOwnershipControlsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "ownershipControls");
    query_has_prev = true;
    const query = try query_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<OwnershipControls xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeOwnershipControls(alloc, &body_buf, input.ownership_controls);
    try body_buf.appendSlice(alloc, "</OwnershipControls>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutBucketOwnershipControlsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
