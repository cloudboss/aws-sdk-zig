...
/// account the first time that you add an object encrypted with SSE-KMS to a
/// bucket. By default, Amazon S3
...
pub const ServerSideEncryptionByDefault = struct {
    ...
    /// service operations, you must use a fully
    ...
    /// owner. Also, if you use a key ID, you can run into a LogDestination
    /// undeliverable error when
