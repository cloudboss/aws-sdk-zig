...
pub const ListMultipartUploadsOutput = struct {
    ...
    pub fn deinit(self: *const ListMultipartUploadsOutput) void {
        if (self.bucket) |v| {
        ...
        if (self.next_upload_id_marker) |v| {
            self.allocator.free(v);
        }
        if (self.prefix) |v| {
            self.allocator.free(v);
        }
        if (self.upload_id_marker) |v| {
            self.allocator.free(v);
        }
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ListMultipartUploadsOutput {
    ...
    if (findElement(body, "MaxUploads")) |content| {
        result.max_uploads = std.fmt.parseInt(i32, content, 10) catch null;
    }
    if (findElement(body, "NextKeyMarker")) |content| {
        result.next_key_marker = try alloc.dupe(u8, content);
    }
    if (findElement(body, "NextUploadIdMarker")) |content| {
        result.next_upload_id_marker = try alloc.dupe(u8, content);
    }
