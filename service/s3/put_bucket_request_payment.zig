...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBucketRequestPaymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<RequestPaymentConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeRequestPaymentConfiguration(alloc, &body_buf, input.request_payment_configuration);
    try body_buf.appendSlice(alloc, "</RequestPaymentConfiguration>");
