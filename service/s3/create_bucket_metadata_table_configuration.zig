...
const CreateBucketMetadataTableConfigurationOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBucketMetadataTableConfigurationInput, options: Options) !CreateBucketMetadataTableConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateBucketMetadataTableConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "<MetadataTableConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeMetadataTableConfiguration(allocator, &body_buf, input.metadata_table_configuration);
    try body_buf.appendSlice(allocator, "</MetadataTableConfiguration>");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateBucketMetadataTableConfigurationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
