...
pub fn execute(client: *Client, input: PutBucketNotificationConfigurationInput, options: Options) !PutBucketNotificationConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBucketNotificationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "notification");
    query_has_prev = true;
    ...
    if (input.skip_destination_validation) |v| {
        try request.headers.put(alloc, "x-amz-skip-destination-validation", if (v) "true" else "false");
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !PutBucketNotificationConfigurationOutput {
    _ = body;
    _ = status;
