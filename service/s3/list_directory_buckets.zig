...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDirectoryBucketsInput, options: CallOptions) !ListDirectoryBucketsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDirectoryBucketsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "x-id=ListDirectoryBuckets");
    query_has_prev = true;
    ...
    if (input.max_directory_buckets) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "max-directory-buckets=");
        {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDirectoryBucketsOutput {
