...
pub const GetObjectLegalHoldOutput = struct {
    /// The current legal hold status for the specified object.
    legal_hold: ?ObjectLockLegalHold = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetObjectLegalHoldInput, options: CallOptions) !GetObjectLegalHoldOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetObjectLegalHoldInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "legal-hold");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetObjectLegalHoldOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
