...
const UpdateBucketMetadataJournalTableConfigurationOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBucketMetadataJournalTableConfigurationInput, options: Options) !UpdateBucketMetadataJournalTableConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBucketMetadataJournalTableConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "metadataJournalTable");
    query_has_prev = true;
    const query = try query_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<JournalTableConfigurationUpdates xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeJournalTableConfigurationUpdates(alloc, &body_buf, input.journal_table_configuration);
    try body_buf.appendSlice(alloc, "</JournalTableConfigurationUpdates>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBucketMetadataJournalTableConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
