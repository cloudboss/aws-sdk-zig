...
pub const PutObjectRetentionOutput = struct {
    request_charged: ?RequestCharged = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutObjectRetentionInput, options: CallOptions) !PutObjectRetentionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutObjectRetentionInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "retention");
    query_has_prev = true;
    ...
    const body: ?[]const u8 = blk: {
        if (input.retention) |payload| {
            ...
            try body_buf.appendSlice(allocator, "<ObjectLockRetention xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
            try serde.serializeObjectLockRetention(allocator, &body_buf, payload);
            try body_buf.appendSlice(allocator, "</ObjectLockRetention>");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutObjectRetentionOutput {
    _ = allocator;
    var result: PutObjectRetentionOutput = .{};
    _ = status;
    _ = body;
