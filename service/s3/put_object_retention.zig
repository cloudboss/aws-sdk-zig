...
pub const PutObjectRetentionOutput = struct {
    ...
    pub fn deinit(self: *const PutObjectRetentionOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: PutObjectRetentionInput, options: Options) !PutObjectRetentionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutObjectRetentionInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "retention");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutObjectRetentionOutput {
    var result: PutObjectRetentionOutput = .{ .allocator = alloc };
    _ = status;
    _ = body;
