...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBucketMetricsConfigurationInput, options: CallOptions) !GetBucketMetricsConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBucketMetricsConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "metrics&x-id=GetBucketMetricsConfiguration");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBucketMetricsConfigurationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
