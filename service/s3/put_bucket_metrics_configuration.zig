...
pub const PutBucketMetricsConfigurationOutput = struct {
    ...
    pub fn deinit(self: *PutBucketMetricsConfigurationOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: PutBucketMetricsConfigurationInput, options: Options) !PutBucketMetricsConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBucketMetricsConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<MetricsConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeMetricsConfiguration(alloc, &body_buf, input.metrics_configuration);
    try body_buf.appendSlice(alloc, "</MetricsConfiguration>");
