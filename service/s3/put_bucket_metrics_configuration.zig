...
pub const PutBucketMetricsConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBucketMetricsConfigurationInput, options: CallOptions) !PutBucketMetricsConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutBucketMetricsConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "metrics");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(allocator, "<MetricsConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeMetricsConfiguration(allocator, &body_buf, input.metrics_configuration);
    try body_buf.appendSlice(allocator, "</MetricsConfiguration>");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutBucketMetricsConfigurationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutBucketMetricsConfigurationOutput = .{};
