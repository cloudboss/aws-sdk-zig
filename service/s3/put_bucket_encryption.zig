...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBucketEncryptionInput, options: CallOptions) !PutBucketEncryptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutBucketEncryptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "encryption");
    query_has_prev = true;
    const query = try query_buf.toOwnedSlice(allocator);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(allocator, "<ServerSideEncryptionConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeServerSideEncryptionConfiguration(allocator, &body_buf, input.server_side_encryption_configuration);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutBucketEncryptionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
