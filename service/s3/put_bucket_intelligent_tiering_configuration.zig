...
pub const PutBucketIntelligentTieringConfigurationOutput = struct {
    ...
    pub fn deinit(self: *PutBucketIntelligentTieringConfigurationOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: PutBucketIntelligentTieringConfigurationInput, options: Options) !PutBucketIntelligentTieringConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBucketIntelligentTieringConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "intelligent-tiering");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "<IntelligentTieringConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://s3.amazonaws.com/doc/2006-03-01/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeIntelligentTieringConfiguration(alloc, &body_buf, input.intelligent_tiering_configuration);
    try body_buf.appendSlice(alloc, "</IntelligentTieringConfiguration>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutBucketIntelligentTieringConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
