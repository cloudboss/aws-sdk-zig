...
pub const UpdateVocabularyInput = struct {
    ...
    /// option for
    /// updating your custom vocabulary is to save your entries in a text file and
    /// upload them
...
pub const UpdateVocabularyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVocabularyInput, options: CallOptions) !UpdateVocabularyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateVocabularyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Transcribe.UpdateVocabulary");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateVocabularyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateVocabularyOutput, body, allocator);
}
