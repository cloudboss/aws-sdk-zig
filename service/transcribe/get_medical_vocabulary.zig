...
pub const GetMedicalVocabularyInput = struct {
...
pub const GetMedicalVocabularyOutput = struct {
    ...
    /// contains information about why the custom medical vocabulary request failed.
    /// See also:
    /// [Common
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMedicalVocabularyInput, options: CallOptions) !GetMedicalVocabularyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMedicalVocabularyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Transcribe.GetMedicalVocabulary");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMedicalVocabularyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMedicalVocabularyOutput, body, allocator);
}
