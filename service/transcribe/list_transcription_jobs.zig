...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTranscriptionJobsInput, options: Options) !ListTranscriptionJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTranscriptionJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.ListTranscriptionJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTranscriptionJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTranscriptionJobsOutput, body, alloc);
}
