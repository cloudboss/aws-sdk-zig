...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVocabularyFilterInput, options: CallOptions) !UpdateVocabularyFilterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateVocabularyFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Transcribe.UpdateVocabularyFilter");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateVocabularyFilterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateVocabularyFilterOutput, body, allocator);
}
