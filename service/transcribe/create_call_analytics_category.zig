...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCallAnalyticsCategoryInput, options: Options) !CreateCallAnalyticsCategoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCallAnalyticsCategoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.CreateCallAnalyticsCategory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCallAnalyticsCategoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCallAnalyticsCategoryOutput, body, alloc);
}
