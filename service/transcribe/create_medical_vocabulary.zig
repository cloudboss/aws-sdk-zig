...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMedicalVocabularyInput, options: Options) !CreateMedicalVocabularyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMedicalVocabularyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.CreateMedicalVocabulary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMedicalVocabularyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateMedicalVocabularyOutput, body, alloc);
}
