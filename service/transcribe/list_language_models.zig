...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLanguageModelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.ListLanguageModels");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLanguageModelsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLanguageModelsOutput, body, alloc);
}
