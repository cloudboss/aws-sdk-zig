...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCallAnalyticsJobInput, options: Options) !GetCallAnalyticsJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCallAnalyticsJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.GetCallAnalyticsJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCallAnalyticsJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCallAnalyticsJobOutput, body, alloc);
}
