...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVocabularyFilterInput, options: Options) !GetVocabularyFilterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVocabularyFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.GetVocabularyFilter");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVocabularyFilterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetVocabularyFilterOutput, body, alloc);
}
