...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLanguageModelInput, options: Options) !DescribeLanguageModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLanguageModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.DescribeLanguageModel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLanguageModelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeLanguageModelOutput, body, alloc);
}
