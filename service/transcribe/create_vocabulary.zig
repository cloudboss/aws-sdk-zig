...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVocabularyInput, options: Options) !CreateVocabularyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVocabularyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.CreateVocabulary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVocabularyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateVocabularyOutput, body, alloc);
}
