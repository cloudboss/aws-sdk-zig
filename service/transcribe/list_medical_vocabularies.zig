...
pub const ListMedicalVocabulariesInput = struct {
...
pub const ListMedicalVocabulariesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMedicalVocabulariesInput, options: CallOptions) !ListMedicalVocabulariesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMedicalVocabulariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Transcribe.ListMedicalVocabularies");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMedicalVocabulariesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMedicalVocabulariesOutput, body, allocator);
}
