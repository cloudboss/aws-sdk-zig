...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMedicalTranscriptionJobsInput, options: Options) !ListMedicalTranscriptionJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMedicalTranscriptionJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.ListMedicalTranscriptionJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMedicalTranscriptionJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMedicalTranscriptionJobsOutput, body, alloc);
}
