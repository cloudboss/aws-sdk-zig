...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTranscriptionJobInput, options: Options) !GetTranscriptionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTranscriptionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.GetTranscriptionJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTranscriptionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTranscriptionJobOutput, body, alloc);
}
