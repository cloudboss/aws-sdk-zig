...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMedicalScribeJobsInput, options: Options) !ListMedicalScribeJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMedicalScribeJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Transcribe.ListMedicalScribeJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMedicalScribeJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMedicalScribeJobsOutput, body, alloc);
}
