...
const CallAnalyticsJobSummary = @import("call_analytics_job_summary.zig").CallAnalyticsJobSummary;

pub const ListCallAnalyticsJobsInput = struct {
...
pub const ListCallAnalyticsJobsOutput = struct {
    /// Provides a summary of information about each result.
    call_analytics_job_summaries: ?[]const CallAnalyticsJobSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCallAnalyticsJobsInput, options: CallOptions) !ListCallAnalyticsJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCallAnalyticsJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Transcribe.ListCallAnalyticsJobs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCallAnalyticsJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCallAnalyticsJobsOutput, body, allocator);
}
