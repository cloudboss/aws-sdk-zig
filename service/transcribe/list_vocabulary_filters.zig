...
const VocabularyFilterInfo = @import("vocabulary_filter_info.zig").VocabularyFilterInfo;

pub const ListVocabularyFiltersInput = struct {
...
pub const ListVocabularyFiltersOutput = struct {
    ...
    vocabulary_filters: ?[]const VocabularyFilterInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVocabularyFiltersInput, options: CallOptions) !ListVocabularyFiltersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVocabularyFiltersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Transcribe.ListVocabularyFilters");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVocabularyFiltersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListVocabularyFiltersOutput, body, allocator);
}
