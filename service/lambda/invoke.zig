...
pub const InvokeOutput = struct {
    ...
    pub fn deinit(self: *const InvokeOutput) void {
        if (self.durable_execution_arn) |v| {
            self.allocator.free(v);
        }
        if (self.executed_version) |v| {
            self.allocator.free(v);
        }
        if (self.function_error) |v| {
            self.allocator.free(v);
        }
        if (self.log_result) |v| {
            self.allocator.free(v);
        }
        if (self.payload) |v| {
            self.allocator.free(v);
...
pub fn execute(client: *Client, input: InvokeInput, options: Options) !InvokeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InvokeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.durable_execution_name) |v| {
        try request.headers.put(alloc, "X-Amz-Durable-Execution-Name", v);
    }
