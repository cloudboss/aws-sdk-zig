...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InvokeAsyncInput, options: CallOptions) !InvokeAsyncOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: InvokeAsyncInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/2014-11-13/functions/");
    try path_buf.appendSlice(allocator, input.function_name);
    try path_buf.appendSlice(allocator, "/invoke-async");
    const path = try path_buf.toOwnedSlice(allocator);

    const body = input.invoke_args;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !InvokeAsyncOutput {
    _ = allocator;
    var result: InvokeAsyncOutput = .{};
    _ = body;
