...
pub const CreateAliasOutput = struct {
    ...
    pub fn deinit(self: *const CreateAliasOutput) void {
        if (self.alias_arn) |v| {
...
pub fn execute(client: *Client, input: CreateAliasInput, options: Options) !CreateAliasOutput {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateAliasOutput {
    var result: CreateAliasOutput = .{ .allocator = alloc };
    _ = status;
