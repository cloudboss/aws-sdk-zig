...
pub const UpdateEventSourceMappingInput = struct {
...
const UpdateEventSourceMappingOutput = @import("event_source_mapping_configuration.zig").EventSourceMappingConfiguration;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEventSourceMappingInput, options: Options) !UpdateEventSourceMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEventSourceMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.amazon_managed_kafka_event_source_config) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AmazonManagedKafkaEventSourceConfig\":");
    ...
    if (input.document_db_event_source_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DocumentDBEventSourceConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.enabled) |v| {
    ...
    if (input.function_response_types) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FunctionResponseTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.provisioned_poller_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ProvisionedPollerConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.self_managed_kafka_event_source_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SelfManagedKafkaEventSourceConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_access_configurations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceAccessConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEventSourceMappingOutput {
    var result: UpdateEventSourceMappingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEventSourceMappingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
