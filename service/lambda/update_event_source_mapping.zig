...
pub const UpdateEventSourceMappingOutput = @import("event_source_mapping_configuration.zig").EventSourceMappingConfiguration;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEventSourceMappingInput, options: CallOptions) !UpdateEventSourceMappingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateEventSourceMappingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateEventSourceMappingOutput {
    var result: UpdateEventSourceMappingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEventSourceMappingOutput, body, allocator);
    }
    _ = status;
    _ = headers;
