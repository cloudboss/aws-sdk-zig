...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLayerVersionInput, options: CallOptions) !GetLayerVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLayerVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLayerVersionOutput {
    var result: GetLayerVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLayerVersionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
