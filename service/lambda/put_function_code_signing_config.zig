...
pub const PutFunctionCodeSigningConfigOutput = struct {
    /// The The Amazon Resource Name (ARN) of the code signing configuration.
    code_signing_config_arn: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutFunctionCodeSigningConfigInput, options: CallOptions) !PutFunctionCodeSigningConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutFunctionCodeSigningConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.code_signing_config_arn), input.code_signing_config_arn, allocator, &body_buf);
    has_prev = true;
