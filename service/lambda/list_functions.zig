...
pub const ListFunctionsInput = struct {
    ...
    /// function. For example, `us-east-1` filters the list of functions to include
    /// only Lambda@Edge functions replicated from a master function in US East (N.
...
pub const ListFunctionsOutput = struct {
    ...
    pub fn deinit(self: *const ListFunctionsOutput) void {
...
pub fn execute(client: *Client, input: ListFunctionsInput, options: Options) !ListFunctionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFunctionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.master_region) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "MasterRegion=");
        try appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ListFunctionsOutput {
    var result: ListFunctionsOutput = .{ .allocator = alloc };
    _ = status;
