...
pub const ListLayerVersionsOutput = struct {
    ...
    pub fn deinit(self: *ListLayerVersionsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListLayerVersionsInput, options: Options) !ListLayerVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLayerVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.compatible_architecture) |v| {
        ...
        try query_buf.appendSlice(alloc, "CompatibleArchitecture=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.compatible_runtime) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "CompatibleRuntime=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLayerVersionsOutput {
    var result: ListLayerVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLayerVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
