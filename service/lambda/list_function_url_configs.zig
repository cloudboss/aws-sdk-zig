...
pub const ListFunctionUrlConfigsOutput = struct {
    ...
    pub fn deinit(self: *const ListFunctionUrlConfigsOutput) void {
...
pub fn execute(client: *Client, input: ListFunctionUrlConfigsInput, options: Options) !ListFunctionUrlConfigsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFunctionUrlConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/urls");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFunctionUrlConfigsOutput {
    var result: ListFunctionUrlConfigsOutput = .{ .allocator = alloc };
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFunctionUrlConfigsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
