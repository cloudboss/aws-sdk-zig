...
pub const ListAliasesOutput = struct {
    /// A list of aliases.
    aliases: ?[]const AliasConfiguration = null,
    ...
    pub fn deinit(self: *const ListAliasesOutput) void {
...
pub fn execute(client: *Client, input: ListAliasesInput, options: Options) !ListAliasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.function_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "FunctionVersion=");
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ListAliasesOutput {
