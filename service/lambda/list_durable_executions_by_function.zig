...
pub fn execute(client: *Client, input: ListDurableExecutionsByFunctionInput, options: Options) !ListDurableExecutionsByFunctionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDurableExecutionsByFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2025-12-01/functions/");
    try path_buf.appendSlice(alloc, input.function_name);
    try path_buf.appendSlice(alloc, "/durable-executions");
    ...
    if (input.durable_execution_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "DurableExecutionName=");
    ...
    if (input.reverse_order) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ReverseOrder=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.started_after) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "StartedAfter=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.started_before) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "StartedBefore=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.statuses) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "Statuses=");
        try appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ListDurableExecutionsByFunctionOutput {
