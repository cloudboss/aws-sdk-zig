...
pub fn execute(client: *Client, input: GetFunctionConcurrencyInput, options: Options) !GetFunctionConcurrencyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFunctionConcurrencyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFunctionConcurrencyOutput {
    var result: GetFunctionConcurrencyOutput = .{ .allocator = alloc };
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFunctionConcurrencyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
