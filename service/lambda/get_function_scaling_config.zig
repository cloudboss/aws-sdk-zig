...
pub const GetFunctionScalingConfigOutput = struct {
    ...
    applied_function_scaling_config: ?FunctionScalingConfig = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFunctionScalingConfigInput, options: CallOptions) !GetFunctionScalingConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFunctionScalingConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFunctionScalingConfigOutput {
    var result: GetFunctionScalingConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFunctionScalingConfigOutput, body, allocator);
    }
    _ = status;
    _ = headers;
