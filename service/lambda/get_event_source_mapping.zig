...
pub const GetEventSourceMappingOutput = @import("event_source_mapping_configuration.zig").EventSourceMappingConfiguration;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEventSourceMappingInput, options: CallOptions) !GetEventSourceMappingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetEventSourceMappingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetEventSourceMappingOutput {
    var result: GetEventSourceMappingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEventSourceMappingOutput, body, allocator);
    }
    _ = status;
    _ = headers;
