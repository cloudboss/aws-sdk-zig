...
pub const ListFunctionEventInvokeConfigsOutput = struct {
    ...
    function_event_invoke_configs: ?[]const FunctionEventInvokeConfig = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFunctionEventInvokeConfigsInput, options: CallOptions) !ListFunctionEventInvokeConfigsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFunctionEventInvokeConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/event-invoke-config/list");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFunctionEventInvokeConfigsOutput {
    var result: ListFunctionEventInvokeConfigsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFunctionEventInvokeConfigsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
