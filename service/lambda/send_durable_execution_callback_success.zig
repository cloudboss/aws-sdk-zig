...
pub const SendDurableExecutionCallbackSuccessOutput = struct {
    ...
    pub fn deinit(self: *const SendDurableExecutionCallbackSuccessOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: SendDurableExecutionCallbackSuccessInput, options: Options) !SendDurableExecutionCallbackSuccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendDurableExecutionCallbackSuccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2025-12-01/durable-execution-callbacks/");
    try path_buf.appendSlice(alloc, input.callback_id);
    try path_buf.appendSlice(alloc, "/succeed");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendDurableExecutionCallbackSuccessOutput {
    _ = body;
    _ = status;
    _ = headers;
    const result: SendDurableExecutionCallbackSuccessOutput = .{ .allocator = alloc };
