...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventSourceMappingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.event_source_arn) |v| {
        ...
        try query_buf.appendSlice(alloc, "EventSourceArn=");
        try appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.function_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "FunctionName=");
        try appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ListEventSourceMappingsOutput {
