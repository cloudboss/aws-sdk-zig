...
pub const ListVersionsByFunctionOutput = struct {
    ...
    pub fn deinit(self: *const ListVersionsByFunctionOutput) void {
...
pub fn execute(client: *Client, input: ListVersionsByFunctionInput, options: Options) !ListVersionsByFunctionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVersionsByFunctionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVersionsByFunctionOutput {
    var result: ListVersionsByFunctionOutput = .{ .allocator = alloc };
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVersionsByFunctionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
