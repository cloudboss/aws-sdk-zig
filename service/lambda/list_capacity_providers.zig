...
pub const ListCapacityProvidersOutput = struct {
    ...
    pub fn deinit(self: *const ListCapacityProvidersOutput) void {
...
pub fn execute(client: *Client, input: ListCapacityProvidersInput, options: Options) !ListCapacityProvidersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCapacityProvidersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCapacityProvidersOutput {
    var result: ListCapacityProvidersOutput = .{ .allocator = alloc };
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCapacityProvidersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
