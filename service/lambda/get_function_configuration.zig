...
pub const GetFunctionConfigurationOutput = struct {
    ...
    pub fn deinit(self: *const GetFunctionConfigurationOutput) void {
...
pub fn execute(client: *Client, input: GetFunctionConfigurationInput, options: Options) !GetFunctionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFunctionConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFunctionConfigurationOutput {
    var result: GetFunctionConfigurationOutput = .{ .allocator = alloc };
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFunctionConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
