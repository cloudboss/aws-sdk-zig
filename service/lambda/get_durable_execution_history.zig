...
pub const GetDurableExecutionHistoryOutput = struct {
    ...
    pub fn deinit(self: *const GetDurableExecutionHistoryOutput) void {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDurableExecutionHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_execution_data) |v| {
        ...
        try query_buf.appendSlice(alloc, "IncludeExecutionData=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDurableExecutionHistoryOutput {
    var result: GetDurableExecutionHistoryOutput = .{ .allocator = alloc };
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDurableExecutionHistoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
