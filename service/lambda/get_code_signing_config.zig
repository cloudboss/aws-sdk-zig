...
pub const GetCodeSigningConfigOutput = struct {
    ...
    pub fn deinit(self: *GetCodeSigningConfigOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetCodeSigningConfigInput, options: Options) !GetCodeSigningConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCodeSigningConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCodeSigningConfigOutput {
    var result: GetCodeSigningConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCodeSigningConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
