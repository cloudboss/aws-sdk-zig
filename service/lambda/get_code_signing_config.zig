...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCodeSigningConfigInput, options: CallOptions) !GetCodeSigningConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCodeSigningConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCodeSigningConfigOutput {
    var result: GetCodeSigningConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCodeSigningConfigOutput, body, allocator);
    }
    _ = status;
    _ = headers;
