...
const UpdateAliasOutput = @import("alias_configuration.zig").AliasConfiguration;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAliasInput, options: Options) !UpdateAliasOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAliasOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAliasOutput, body, allocator);
    }
    _ = status;
    _ = headers;
