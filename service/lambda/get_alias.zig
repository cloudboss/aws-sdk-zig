...
pub const GetAliasOutput = @import("alias_configuration.zig").AliasConfiguration;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAliasOutput {
    var result: GetAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAliasOutput, body, allocator);
    }
    _ = status;
    _ = headers;
