...
pub const PutFunctionScalingConfigOutput = struct {
    ...
    pub fn deinit(self: *PutFunctionScalingConfigOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: PutFunctionScalingConfigInput, options: Options) !PutFunctionScalingConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutFunctionScalingConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2025-11-30/functions/");
    try path_buf.appendSlice(alloc, input.function_name);
    try path_buf.appendSlice(alloc, "/function-scaling-config");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.qualifier);
    query_has_prev = true;
    ...
    if (input.function_scaling_config) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"FunctionScalingConfig\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutFunctionScalingConfigOutput {
    var result: PutFunctionScalingConfigOutput = .{};
    if (body.len > 0) {
