...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFunctionUrlConfigInput, options: CallOptions) !CreateFunctionUrlConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateFunctionUrlConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateFunctionUrlConfigOutput {
    var result: CreateFunctionUrlConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFunctionUrlConfigOutput, body, allocator);
    }
    _ = status;
    _ = headers;
