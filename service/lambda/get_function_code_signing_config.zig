...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFunctionCodeSigningConfigInput, options: CallOptions) !GetFunctionCodeSigningConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFunctionCodeSigningConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFunctionCodeSigningConfigOutput {
    var result: GetFunctionCodeSigningConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFunctionCodeSigningConfigOutput, body, allocator);
    }
    _ = status;
    _ = headers;
