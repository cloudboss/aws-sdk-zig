...
pub const GetLayerVersionPolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLayerVersionPolicyInput, options: CallOptions) !GetLayerVersionPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLayerVersionPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLayerVersionPolicyOutput {
    var result: GetLayerVersionPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLayerVersionPolicyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
