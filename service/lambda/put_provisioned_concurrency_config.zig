...
pub const PutProvisionedConcurrencyConfigOutput = struct {
    ...
    pub fn deinit(self: *const PutProvisionedConcurrencyConfigOutput) void {
        if (self.last_modified) |v| {
            self.allocator.free(v);
        }
        if (self.status_reason) |v| {
            self.allocator.free(v);
        }
    }
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutProvisionedConcurrencyConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ProvisionedConcurrentExecutions\":");
    try aws.json.writeValue(@TypeOf(input.provisioned_concurrent_executions), input.provisioned_concurrent_executions, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutProvisionedConcurrencyConfigOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutProvisionedConcurrencyConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
