...
fn serializeRequest(alloc: std.mem.Allocator, input: PublishLayerVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.compatible_architectures) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"CompatibleArchitectures\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.compatible_runtimes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"CompatibleRuntimes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PublishLayerVersionOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PublishLayerVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
