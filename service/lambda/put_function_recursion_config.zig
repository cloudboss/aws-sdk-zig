...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutFunctionRecursionConfigInput, options: Options) !PutFunctionRecursionConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutFunctionRecursionConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2024-08-31/functions/");
    try path_buf.appendSlice(alloc, input.function_name);
    try path_buf.appendSlice(alloc, "/recursion-config");
    ...
    try aws.json.writeValue(@TypeOf(input.recursive_loop), input.recursive_loop, alloc, &body_buf);
    has_prev = true;
