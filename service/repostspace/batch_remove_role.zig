...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchRemoveRoleInput, options: CallOptions) !BatchRemoveRoleOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "repostspace");
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchRemoveRoleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchRemoveRoleOutput {
    var result: BatchRemoveRoleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchRemoveRoleOutput, body, allocator);
    }
    _ = status;
    _ = headers;
