...
pub const BatchAddChannelRoleToAccessorsInput = struct {
...
pub const BatchAddChannelRoleToAccessorsOutput = struct {
    /// An array of successfully updated identifiers.
    added_accessor_ids: ?[]const []const u8 = null,

    /// An array of errors that occurred when roles were added.
    errors: ?[]const BatchError = null,

    pub const json_field_names = .{
        .added_accessor_ids = "addedAccessorIds",
        .errors = "errors",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchAddChannelRoleToAccessorsInput, options: CallOptions) !BatchAddChannelRoleToAccessorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchAddChannelRoleToAccessorsInput, config: *aws.Config) !aws.http.Request {
