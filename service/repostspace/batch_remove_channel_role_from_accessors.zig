...
pub const BatchRemoveChannelRoleFromAccessorsInput = struct {
...
pub const BatchRemoveChannelRoleFromAccessorsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchRemoveChannelRoleFromAccessorsInput, options: CallOptions) !BatchRemoveChannelRoleFromAccessorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchRemoveChannelRoleFromAccessorsInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"channelRole\":");
    try aws.json.writeValue(@TypeOf(input.channel_role), input.channel_role, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchRemoveChannelRoleFromAccessorsOutput {
    var result: BatchRemoveChannelRoleFromAccessorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchRemoveChannelRoleFromAccessorsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
