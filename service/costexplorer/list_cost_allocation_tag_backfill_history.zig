...
pub const ListCostAllocationTagBackfillHistoryInput = struct {
...
pub const ListCostAllocationTagBackfillHistoryOutput = struct {
    /// The list of historical cost allocation tag backfill requests.
    backfill_requests: ?[]const CostAllocationTagBackfillRequest = null,
    ...
    pub const json_field_names = .{
        .backfill_requests = "BackfillRequests",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCostAllocationTagBackfillHistoryInput, options: CallOptions) !ListCostAllocationTagBackfillHistoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCostAllocationTagBackfillHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSInsightsIndexService.ListCostAllocationTagBackfillHistory");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCostAllocationTagBackfillHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCostAllocationTagBackfillHistoryOutput, body, allocator);
}
