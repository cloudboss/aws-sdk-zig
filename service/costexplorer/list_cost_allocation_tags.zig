...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCostAllocationTagsInput, options: Options) !ListCostAllocationTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCostAllocationTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.ListCostAllocationTags");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCostAllocationTagsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCostAllocationTagsOutput, body, alloc);
}
