...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCostAndUsageInput, options: Options) !GetCostAndUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCostAndUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetCostAndUsage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCostAndUsageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCostAndUsageOutput, body, alloc);
}
