...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCostAndUsageComparisonsInput, options: Options) !GetCostAndUsageComparisonsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCostAndUsageComparisonsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetCostAndUsageComparisons");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCostAndUsageComparisonsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCostAndUsageComparisonsOutput, body, alloc);
}
