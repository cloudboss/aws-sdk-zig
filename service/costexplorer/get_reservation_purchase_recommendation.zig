...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReservationPurchaseRecommendationInput, options: Options) !GetReservationPurchaseRecommendationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReservationPurchaseRecommendationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetReservationPurchaseRecommendation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReservationPurchaseRecommendationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetReservationPurchaseRecommendationOutput, body, alloc);
}
