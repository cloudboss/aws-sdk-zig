...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAnomalyMonitorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.CreateAnomalyMonitor");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAnomalyMonitorOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateAnomalyMonitorOutput, body, alloc);
}
