...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartCommitmentPurchaseAnalysisInput, options: Options) !StartCommitmentPurchaseAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartCommitmentPurchaseAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.StartCommitmentPurchaseAnalysis");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartCommitmentPurchaseAnalysisOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartCommitmentPurchaseAnalysisOutput, body, alloc);
}
