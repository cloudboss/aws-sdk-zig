...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCostCategoryDefinitionInput, options: Options) !DeleteCostCategoryDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCostCategoryDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.DeleteCostCategoryDefinition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCostCategoryDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteCostCategoryDefinitionOutput, body, alloc);
}
