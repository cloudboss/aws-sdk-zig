...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCostForecastInput, options: Options) !GetCostForecastOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCostForecastInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetCostForecast");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCostForecastOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCostForecastOutput, body, alloc);
}
