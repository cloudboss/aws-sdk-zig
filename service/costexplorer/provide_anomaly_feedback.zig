...
pub const ProvideAnomalyFeedbackInput = struct {
    /// A cost anomaly ID.
    anomaly_id: []const u8,
...
pub const ProvideAnomalyFeedbackOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ProvideAnomalyFeedbackInput, options: CallOptions) !ProvideAnomalyFeedbackOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ProvideAnomalyFeedbackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSInsightsIndexService.ProvideAnomalyFeedback");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ProvideAnomalyFeedbackOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ProvideAnomalyFeedbackOutput, body, allocator);
}
