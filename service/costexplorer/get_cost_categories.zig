...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCostCategoriesInput, options: Options) !GetCostCategoriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCostCategoriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetCostCategories");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCostCategoriesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetCostCategoriesOutput, body, alloc);
}
