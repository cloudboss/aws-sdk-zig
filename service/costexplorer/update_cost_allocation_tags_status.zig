...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCostAllocationTagsStatusInput, options: Options) !UpdateCostAllocationTagsStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCostAllocationTagsStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.UpdateCostAllocationTagsStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCostAllocationTagsStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCostAllocationTagsStatusOutput, body, alloc);
}
