...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCostCategoryDefinitionInput, options: Options) !UpdateCostCategoryDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCostCategoryDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.UpdateCostCategoryDefinition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCostCategoryDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCostCategoryDefinitionOutput, body, alloc);
}
