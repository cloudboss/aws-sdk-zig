...
pub const ListSavingsPlansPurchaseRecommendationGenerationInput = struct {
...
pub const ListSavingsPlansPurchaseRecommendationGenerationOutput = struct {
    /// The list of historical recommendation generations.
    generation_summary_list: ?[]const GenerationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSavingsPlansPurchaseRecommendationGenerationInput, options: CallOptions) !ListSavingsPlansPurchaseRecommendationGenerationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSavingsPlansPurchaseRecommendationGenerationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSInsightsIndexService.ListSavingsPlansPurchaseRecommendationGeneration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSavingsPlansPurchaseRecommendationGenerationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSavingsPlansPurchaseRecommendationGenerationOutput, body, allocator);
}
