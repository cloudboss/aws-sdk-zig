...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCostAndUsageWithResourcesInput, options: Options) !GetCostAndUsageWithResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCostAndUsageWithResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetCostAndUsageWithResources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCostAndUsageWithResourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCostAndUsageWithResourcesOutput, body, alloc);
}
