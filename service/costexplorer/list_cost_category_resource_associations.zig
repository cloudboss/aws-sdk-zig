...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCostCategoryResourceAssociationsInput, options: Options) !ListCostCategoryResourceAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCostCategoryResourceAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.ListCostCategoryResourceAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCostCategoryResourceAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCostCategoryResourceAssociationsOutput, body, alloc);
}
