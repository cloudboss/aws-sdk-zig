...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApproximateUsageRecordsInput, options: Options) !GetApproximateUsageRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApproximateUsageRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetApproximateUsageRecords");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApproximateUsageRecordsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetApproximateUsageRecordsOutput, body, alloc);
}
