...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRightsizingRecommendationInput, options: Options) !GetRightsizingRecommendationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRightsizingRecommendationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetRightsizingRecommendation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRightsizingRecommendationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRightsizingRecommendationOutput, body, alloc);
}
