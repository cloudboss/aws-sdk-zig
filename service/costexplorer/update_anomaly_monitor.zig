...
pub const UpdateAnomalyMonitorInput = struct {
...
pub const UpdateAnomalyMonitorOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAnomalyMonitorInput, options: CallOptions) !UpdateAnomalyMonitorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAnomalyMonitorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSInsightsIndexService.UpdateAnomalyMonitor");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAnomalyMonitorOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateAnomalyMonitorOutput, body, allocator);
}
