...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAnomalyMonitorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.UpdateAnomalyMonitor");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAnomalyMonitorOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateAnomalyMonitorOutput, body, alloc);
}
