...
const CostCategory = @import("cost_category.zig").CostCategory;

pub const DescribeCostCategoryDefinitionInput = struct {
...
pub const DescribeCostCategoryDefinitionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCostCategoryDefinitionInput, options: CallOptions) !DescribeCostCategoryDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeCostCategoryDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSInsightsIndexService.DescribeCostCategoryDefinition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeCostCategoryDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCostCategoryDefinitionOutput, body, allocator);
}
