...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReservationCoverageInput, options: CallOptions) !GetReservationCoverageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetReservationCoverageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSInsightsIndexService.GetReservationCoverage");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetReservationCoverageOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetReservationCoverageOutput, body, allocator);
}
