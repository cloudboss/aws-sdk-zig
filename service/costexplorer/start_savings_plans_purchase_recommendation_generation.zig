...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSavingsPlansPurchaseRecommendationGenerationInput, options: Options) !StartSavingsPlansPurchaseRecommendationGenerationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSavingsPlansPurchaseRecommendationGenerationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.StartSavingsPlansPurchaseRecommendationGeneration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSavingsPlansPurchaseRecommendationGenerationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartSavingsPlansPurchaseRecommendationGenerationOutput, body, alloc);
}
