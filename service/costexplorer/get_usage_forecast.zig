...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUsageForecastInput, options: Options) !GetUsageForecastOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUsageForecastInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetUsageForecast");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUsageForecastOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUsageForecastOutput, body, alloc);
}
