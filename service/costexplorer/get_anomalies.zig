...
const AnomalyDateInterval = @import("anomaly_date_interval.zig").AnomalyDateInterval;
const AnomalyFeedbackType = @import("anomaly_feedback_type.zig").AnomalyFeedbackType;
const TotalImpactFilter = @import("total_impact_filter.zig").TotalImpactFilter;
const Anomaly = @import("anomaly.zig").Anomaly;

pub const GetAnomaliesInput = struct {
    ...
    /// example, you
    /// can filter anomalies `GREATER_THAN 200.00` to retrieve anomalies, with an
    /// estimated
    /// dollar impact greater than 200.
    total_impact: ?TotalImpactFilter = null,
...
pub const GetAnomaliesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAnomaliesInput, options: CallOptions) !GetAnomaliesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAnomaliesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSInsightsIndexService.GetAnomalies");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAnomaliesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetAnomaliesOutput, body, allocator);
}
