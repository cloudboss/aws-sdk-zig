...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCostComparisonDriversInput, options: CallOptions) !GetCostComparisonDriversOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCostComparisonDriversInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSInsightsIndexService.GetCostComparisonDrivers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCostComparisonDriversOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCostComparisonDriversOutput, body, allocator);
}
