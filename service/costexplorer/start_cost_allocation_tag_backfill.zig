...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartCostAllocationTagBackfillInput, options: Options) !StartCostAllocationTagBackfillOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartCostAllocationTagBackfillInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.StartCostAllocationTagBackfill");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartCostAllocationTagBackfillOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartCostAllocationTagBackfillOutput, body, alloc);
}
