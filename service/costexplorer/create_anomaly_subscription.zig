...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAnomalySubscriptionInput, options: Options) !CreateAnomalySubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAnomalySubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.CreateAnomalySubscription");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAnomalySubscriptionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateAnomalySubscriptionOutput, body, alloc);
}
