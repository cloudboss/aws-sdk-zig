...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCommitmentPurchaseAnalysisInput, options: Options) !GetCommitmentPurchaseAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCommitmentPurchaseAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetCommitmentPurchaseAnalysis");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCommitmentPurchaseAnalysisOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetCommitmentPurchaseAnalysisOutput, body, alloc);
}
