...
pub const DeleteAnomalySubscriptionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAnomalySubscriptionInput, options: Options) !DeleteAnomalySubscriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAnomalySubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSInsightsIndexService.DeleteAnomalySubscription");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAnomalySubscriptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
