...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSavingsPlansUtilizationInput, options: Options) !GetSavingsPlansUtilizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSavingsPlansUtilizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.GetSavingsPlansUtilization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSavingsPlansUtilizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSavingsPlansUtilizationOutput, body, alloc);
}
