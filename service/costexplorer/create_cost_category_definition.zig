...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCostCategoryDefinitionInput, options: Options) !CreateCostCategoryDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCostCategoryDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.CreateCostCategoryDefinition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCostCategoryDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCostCategoryDefinitionOutput, body, alloc);
}
