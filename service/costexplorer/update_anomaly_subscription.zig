...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAnomalySubscriptionInput, options: Options) !UpdateAnomalySubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAnomalySubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSInsightsIndexService.UpdateAnomalySubscription");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAnomalySubscriptionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateAnomalySubscriptionOutput, body, alloc);
}
