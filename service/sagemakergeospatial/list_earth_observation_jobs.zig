...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEarthObservationJobsInput, options: CallOptions) !ListEarthObservationJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListEarthObservationJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListEarthObservationJobsOutput {
    var result: ListEarthObservationJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEarthObservationJobsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
