...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchRasterDataCollectionInput, options: Options) !SearchRasterDataCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchRasterDataCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RasterDataCollectionQuery\":");
    try aws.json.writeValue(@TypeOf(input.raster_data_collection_query), input.raster_data_collection_query, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchRasterDataCollectionOutput {
    var result: SearchRasterDataCollectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchRasterDataCollectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
