...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartEarthObservationJobInput, options: Options) !StartEarthObservationJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartEarthObservationJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartEarthObservationJobOutput {
    var result: StartEarthObservationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartEarthObservationJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
