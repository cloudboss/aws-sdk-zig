...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVectorEnrichmentJobInput, options: Options) !GetVectorEnrichmentJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVectorEnrichmentJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVectorEnrichmentJobOutput {
    var result: GetVectorEnrichmentJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVectorEnrichmentJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
