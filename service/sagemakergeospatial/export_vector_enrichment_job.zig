...
fn serializeRequest(alloc: std.mem.Allocator, input: ExportVectorEnrichmentJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExportVectorEnrichmentJobOutput {
    var result: ExportVectorEnrichmentJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ExportVectorEnrichmentJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
