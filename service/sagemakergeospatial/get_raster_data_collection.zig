...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRasterDataCollectionInput, options: Options) !GetRasterDataCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRasterDataCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/raster-data-collection/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRasterDataCollectionOutput {
    var result: GetRasterDataCollectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRasterDataCollectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
