...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDatastoreInput, options: Options) !GetDatastoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDatastoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDatastoreOutput {
    var result: GetDatastoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDatastoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
