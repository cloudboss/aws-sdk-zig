...
const ImageSetProperties = @import("image_set_properties.zig").ImageSetProperties;

pub const ListImageSetVersionsInput = struct {
...
pub const ListImageSetVersionsOutput = struct {
    /// Lists all properties associated with an image set.
    image_set_properties_list: ?[]const ImageSetProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImageSetVersionsInput, options: Options) !ListImageSetVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImageSetVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/listImageSetVersions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImageSetVersionsOutput {
    var result: ListImageSetVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImageSetVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
