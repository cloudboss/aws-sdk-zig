...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchImageSetsInput, options: Options) !SearchImageSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchImageSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/searchImageSets");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchImageSetsOutput {
    var result: SearchImageSetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchImageSetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
