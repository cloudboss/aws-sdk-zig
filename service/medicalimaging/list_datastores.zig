...
const DatastoreSummary = @import("datastore_summary.zig").DatastoreSummary;

pub const ListDatastoresInput = struct {
...
pub const ListDatastoresOutput = struct {
    /// The list of summaries of data stores.
    datastore_summaries: ?[]const DatastoreSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDatastoresInput, options: Options) !ListDatastoresOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDatastoresInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.datastore_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "datastoreStatus=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDatastoresOutput {
    var result: ListDatastoresOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDatastoresOutput, body, alloc);
    }
    _ = status;
    _ = headers;
