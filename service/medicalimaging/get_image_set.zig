...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetImageSetInput, options: Options) !GetImageSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImageSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/getImageSet");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImageSetOutput {
    var result: GetImageSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImageSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
