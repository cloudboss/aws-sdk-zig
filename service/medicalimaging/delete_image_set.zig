...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteImageSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/deleteImageSet");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteImageSetOutput {
    var result: DeleteImageSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteImageSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
