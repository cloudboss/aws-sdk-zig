...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateImageSetMetadataInput, options: CallOptions) !UpdateImageSetMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateImageSetMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/updateImageSetMetadata");
    ...
    const body = try aws.json.jsonStringify(input.update_image_set_metadata_updates, allocator);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateImageSetMetadataOutput {
    var result: UpdateImageSetMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateImageSetMetadataOutput, body, allocator);
    }
    _ = status;
    _ = headers;
