...
pub const GetPolicyVersionOutput = struct {
    ...
    pub fn deinit(self: *const GetPolicyVersionOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: GetPolicyVersionInput, options: Options) !GetPolicyVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPolicyVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetPolicyVersion&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetPolicyVersionOutput {
    _ = status;
    _ = body;
    const result: GetPolicyVersionOutput = .{ .allocator = alloc };
