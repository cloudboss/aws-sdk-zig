...
pub const DeleteSSHPublicKeyOutput = struct {
    ...
    pub fn deinit(self: *const DeleteSSHPublicKeyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteSSHPublicKeyInput, options: Options) !DeleteSSHPublicKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSSHPublicKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteSSHPublicKey&Version=2010-05-08");
    try body_buf.appendSlice(alloc, "&SSHPublicKeyId=");
    try appendUrlEncoded(alloc, &body_buf, input.ssh_public_key_id);
    try body_buf.appendSlice(alloc, "&UserName=");
    try appendUrlEncoded(alloc, &body_buf, input.user_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSSHPublicKeyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeleteSSHPublicKeyOutput = .{ .allocator = alloc };
