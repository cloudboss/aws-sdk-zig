...
pub const AttachRolePolicyInput = struct {
...
pub const AttachRolePolicyOutput = struct {
    ...
    pub fn deinit(self: *const AttachRolePolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: AttachRolePolicyInput, options: Options) !AttachRolePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachRolePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AttachRolePolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachRolePolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: AttachRolePolicyOutput = .{ .allocator = alloc };
