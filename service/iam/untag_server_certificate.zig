...
pub const UntagServerCertificateOutput = struct {
    ...
    pub fn deinit(self: *UntagServerCertificateOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UntagServerCertificateInput, options: Options) !UntagServerCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagServerCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UntagServerCertificate&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UntagServerCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: UntagServerCertificateOutput = .{};
