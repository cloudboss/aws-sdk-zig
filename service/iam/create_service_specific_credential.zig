...
pub const CreateServiceSpecificCredentialOutput = struct {
    ...
    pub fn deinit(self: *const CreateServiceSpecificCredentialOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceSpecificCredentialInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.service_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateServiceSpecificCredentialOutput {
    ...
    _ = headers;
    _ = body;
    const result: CreateServiceSpecificCredentialOutput = .{ .allocator = alloc };
