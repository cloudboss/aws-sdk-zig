...
pub const CreateServiceSpecificCredentialOutput = struct {
    ...
    pub fn deinit(self: *CreateServiceSpecificCredentialOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: CreateServiceSpecificCredentialInput, options: Options) !CreateServiceSpecificCredentialOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceSpecificCredentialInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateServiceSpecificCredential&Version=2010-05-08");
    if (input.credential_age_days) |v| {
        try body_buf.appendSlice(alloc, "&CredentialAgeDays=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    try body_buf.appendSlice(alloc, "&ServiceName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.service_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateServiceSpecificCredentialOutput {
    _ = status;
    _ = headers;
