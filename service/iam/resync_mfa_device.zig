...
pub const ResyncMFADeviceOutput = struct {
    ...
    pub fn deinit(self: *const ResyncMFADeviceOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: ResyncMFADeviceInput, options: Options) !ResyncMFADeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResyncMFADeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ResyncMFADevice&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResyncMFADeviceOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: ResyncMFADeviceOutput = .{ .allocator = alloc };
