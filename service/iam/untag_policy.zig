...
/// the
/// *IAM User Guide*.
pub const UntagPolicyInput = struct {
...
pub const UntagPolicyOutput = struct {
    ...
    pub fn deinit(self: *const UntagPolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UntagPolicyInput, options: Options) !UntagPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UntagPolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UntagPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UntagPolicyOutput = .{ .allocator = alloc };
