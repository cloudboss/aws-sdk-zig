...
const EnableMFADeviceOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableMFADeviceInput, options: Options) !EnableMFADeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableMFADeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.authentication_code_1);
    try body_buf.appendSlice(alloc, "&AuthenticationCode2=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.authentication_code_2);
