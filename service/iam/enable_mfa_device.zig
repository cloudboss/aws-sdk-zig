...
pub const EnableMFADeviceOutput = struct {
    ...
    pub fn deinit(self: *const EnableMFADeviceOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: EnableMFADeviceInput, options: Options) !EnableMFADeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableMFADeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableMFADevice&Version=2010-05-08");
    try body_buf.appendSlice(alloc, "&AuthenticationCode1=");
    try appendUrlEncoded(alloc, &body_buf, input.authentication_code_1);
    try body_buf.appendSlice(alloc, "&AuthenticationCode2=");
    try appendUrlEncoded(alloc, &body_buf, input.authentication_code_2);
