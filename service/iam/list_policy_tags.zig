...
pub const ListPolicyTagsOutput = struct {
    ...
    pub fn deinit(self: *ListPolicyTagsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListPolicyTagsInput, options: Options) !ListPolicyTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPolicyTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListPolicyTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPolicyTagsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListPolicyTagsResult")) break;
            },
            else => {},
        }
    }

    var result: ListPolicyTagsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
