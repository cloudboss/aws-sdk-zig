...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPolicyTagsInput, options: CallOptions) !ListPolicyTagsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPolicyTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListPolicyTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPolicyTagsOutput {
    _ = status;
    _ = headers;
