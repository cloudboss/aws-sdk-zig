...
pub const ListSAMLProvidersOutput = struct {
    ...
    /// Web Services
    /// account.
    saml_provider_list: ?[]const SAMLProviderListEntry = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSAMLProvidersInput, options: CallOptions) !ListSAMLProvidersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSAMLProvidersInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=ListSAMLProviders&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSAMLProvidersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListSAMLProvidersResult")) break;
            },
            else => {},
        }
    }

    var result: ListSAMLProvidersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SAMLProviderList")) {
                    result.saml_provider_list = try serde.deserializeSAMLProviderListType(allocator, &reader, "member");
                } else {
