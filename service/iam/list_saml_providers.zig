...
pub const ListSAMLProvidersOutput = struct {
    ...
    pub fn deinit(self: *ListSAMLProvidersOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListSAMLProvidersInput, options: Options) !ListSAMLProvidersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSAMLProvidersInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=ListSAMLProviders&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSAMLProvidersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SAMLProviderList")) {
                    result.saml_provider_list = try serde.deserializeSAMLProviderListType(&reader, alloc, "member");
                } else {
