...
fn serializeRequest(allocator: std.mem.Allocator, input: AddClientIDToOpenIDConnectProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AddClientIDToOpenIDConnectProvider&Version=2010-05-08");
    try body_buf.appendSlice(allocator, "&ClientID=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.client_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AddClientIDToOpenIDConnectProviderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
