...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceLastAccessedDetailsWithEntitiesInput, options: CallOptions) !GetServiceLastAccessedDetailsWithEntitiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetServiceLastAccessedDetailsWithEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetServiceLastAccessedDetailsWithEntities&Version=2010-05-08");
    ...
    try body_buf.appendSlice(allocator, "&ServiceNamespace=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.service_namespace);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetServiceLastAccessedDetailsWithEntitiesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EntityDetailsList")) {
                    result.entity_details_list = try serde.deserializeentityDetailsListType(allocator, &reader, "member");
                } else if (std.mem.eql(u8, e.local, "Error")) {
                    result.@"error" = try serde.deserializeErrorDetails(allocator, &reader);
