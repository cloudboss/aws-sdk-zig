...
pub const UpdateRoleOutput = struct {
    ...
    pub fn deinit(self: *const UpdateRoleOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UpdateRoleInput, options: Options) !UpdateRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateRole&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoleOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UpdateRoleOutput = .{ .allocator = alloc };
