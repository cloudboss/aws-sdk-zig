...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRolePermissionsBoundaryInput, options: CallOptions) !PutRolePermissionsBoundaryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutRolePermissionsBoundaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=PutRolePermissionsBoundary&Version=2010-05-08");
    try body_buf.appendSlice(allocator, "&PermissionsBoundary=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.permissions_boundary);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutRolePermissionsBoundaryOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
