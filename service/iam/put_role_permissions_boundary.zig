...
pub const PutRolePermissionsBoundaryOutput = struct {
    ...
    pub fn deinit(self: *PutRolePermissionsBoundaryOutput) void {
        self._arena.deinit();
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRolePermissionsBoundaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.permissions_boundary);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRolePermissionsBoundaryOutput {
    ...
    _ = body;
    _ = alloc;
    const result: PutRolePermissionsBoundaryOutput = .{};
