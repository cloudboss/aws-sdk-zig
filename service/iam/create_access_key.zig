...
pub const CreateAccessKeyInput = struct {
...
pub const CreateAccessKeyOutput = struct {
    /// A structure with details about the access key.
    access_key: ?AccessKey = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccessKeyInput, options: CallOptions) !CreateAccessKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAccessKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateAccessKey&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAccessKeyOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccessKey")) {
                    result.access_key = try serde.deserializeAccessKey(allocator, &reader);
                } else {
