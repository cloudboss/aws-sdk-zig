...
pub const TagInstanceProfileInput = struct {
...
pub const TagInstanceProfileOutput = struct {
    ...
    pub fn deinit(self: *const TagInstanceProfileOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: TagInstanceProfileInput, options: Options) !TagInstanceProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TagInstanceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=TagInstanceProfile&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TagInstanceProfileOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: TagInstanceProfileOutput = .{ .allocator = alloc };
