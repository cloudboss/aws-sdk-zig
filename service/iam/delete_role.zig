...
pub const DeleteRoleOutput = struct {
    ...
    pub fn deinit(self: *const DeleteRoleOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteRoleInput, options: Options) !DeleteRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteRole&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRoleOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeleteRoleOutput = .{ .allocator = alloc };
