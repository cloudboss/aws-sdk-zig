...
pub const UntagSAMLProviderOutput = struct {
    ...
    pub fn deinit(self: *const UntagSAMLProviderOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UntagSAMLProviderInput, options: Options) !UntagSAMLProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagSAMLProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UntagSAMLProvider&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UntagSAMLProviderOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UntagSAMLProviderOutput = .{ .allocator = alloc };
