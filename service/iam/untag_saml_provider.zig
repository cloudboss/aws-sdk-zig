...
const UntagSAMLProviderOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UntagSAMLProviderInput, options: Options) !UntagSAMLProviderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UntagSAMLProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UntagSAMLProvider&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UntagSAMLProviderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: UntagSAMLProviderOutput = .{};
