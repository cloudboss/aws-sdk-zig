...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAccountSummaryInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=GetAccountSummary&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAccountSummaryOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SummaryMap")) {
                    result.summary_map = try serde.deserializesummaryMapType(allocator, &reader, "entry");
                } else {
