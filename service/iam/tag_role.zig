...
pub const TagRoleOutput = struct {
    ...
    pub fn deinit(self: *const TagRoleOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: TagRoleInput, options: Options) !TagRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TagRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=TagRole&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TagRoleOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: TagRoleOutput = .{ .allocator = alloc };
