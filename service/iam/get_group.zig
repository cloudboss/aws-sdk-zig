...
pub const GetGroupOutput = struct {
    ...
    pub fn deinit(self: *const GetGroupOutput) void {
        if (self.marker) |v| {
...
pub fn execute(client: *Client, input: GetGroupInput, options: Options) !GetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetGroup&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetGroupOutput {
    _ = status;
    var result: GetGroupOutput = .{ .allocator = alloc };
