...
pub const DeleteVirtualMFADeviceOutput = struct {
    ...
    pub fn deinit(self: *const DeleteVirtualMFADeviceOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteVirtualMFADeviceInput, options: Options) !DeleteVirtualMFADeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVirtualMFADeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteVirtualMFADevice&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVirtualMFADeviceOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeleteVirtualMFADeviceOutput = .{ .allocator = alloc };
