...
pub const ListRolesOutput = struct {
    ...
    pub fn deinit(self: *ListRolesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListRolesInput, options: Options) !ListRolesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRolesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListRoles&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRolesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListRolesResult")) break;
            },
            else => {},
        }
    }

    var result: ListRolesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.roles = try serde.deserializeroleListType(&reader, alloc, "member");
                } else {
