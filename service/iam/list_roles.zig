...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRolesInput, options: CallOptions) !ListRolesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRolesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRolesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.roles = try serde.deserializeroleListType(allocator, &reader, "member");
                } else {
