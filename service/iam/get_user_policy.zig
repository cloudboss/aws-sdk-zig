...
pub const GetUserPolicyInput = struct {
...
pub const GetUserPolicyOutput = struct {
    /// The policy document.
    ///
    ...
    pub fn deinit(self: *GetUserPolicyOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetUserPolicyInput, options: Options) !GetUserPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUserPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetUserPolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUserPolicyOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetUserPolicyResult")) break;
            },
            else => {},
        }
    }

    var result: GetUserPolicyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
