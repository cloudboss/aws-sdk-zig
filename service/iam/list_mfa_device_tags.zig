...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMFADeviceTagsInput, options: CallOptions) !ListMFADeviceTagsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMFADeviceTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListMFADeviceTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMFADeviceTagsOutput {
    _ = status;
    _ = headers;
