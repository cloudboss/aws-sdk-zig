...
pub const ListMFADeviceTagsInput = struct {
...
pub const ListMFADeviceTagsOutput = struct {
    ...
    pub fn deinit(self: *ListMFADeviceTagsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListMFADeviceTagsInput, options: Options) !ListMFADeviceTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMFADeviceTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListMFADeviceTags&Version=2010-05-08");
    if (input.marker) |v| {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.serial_number);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMFADeviceTagsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListMFADeviceTagsResult")) break;
            },
            else => {},
        }
    }

    var result: ListMFADeviceTagsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
