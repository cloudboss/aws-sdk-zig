...
pub const SetDefaultPolicyVersionOutput = struct {
    ...
    pub fn deinit(self: *const SetDefaultPolicyVersionOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: SetDefaultPolicyVersionInput, options: Options) !SetDefaultPolicyVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetDefaultPolicyVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetDefaultPolicyVersion&Version=2010-05-08");
    ...
    try appendUrlEncoded(alloc, &body_buf, input.version_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !SetDefaultPolicyVersionOutput {
    _ = status;
    _ = body;
    const result: SetDefaultPolicyVersionOutput = .{ .allocator = alloc };
