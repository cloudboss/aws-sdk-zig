...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UploadServerCertificateInput, options: CallOptions) !UploadServerCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UploadServerCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UploadServerCertificate&Version=2010-05-08");
    try body_buf.appendSlice(allocator, "&CertificateBody=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.certificate_body);
    ...
    try body_buf.appendSlice(allocator, "&PrivateKey=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.private_key);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UploadServerCertificateOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ServerCertificateMetadata")) {
                    result.server_certificate_metadata = try serde.deserializeServerCertificateMetadata(allocator, &reader);
