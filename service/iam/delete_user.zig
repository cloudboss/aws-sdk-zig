...
pub const DeleteUserOutput = struct {
    ...
    pub fn deinit(self: *const DeleteUserOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteUserInput, options: Options) !DeleteUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteUser&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteUserOutput {
    ...
    _ = headers;
    _ = body;
    const result: DeleteUserOutput = .{ .allocator = alloc };
