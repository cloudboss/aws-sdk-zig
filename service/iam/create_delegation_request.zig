...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDelegationRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.notification_channel);
    ...
    try appendUrlEncoded(alloc, &body_buf, input.requestor_workflow_id);
    try body_buf.appendSlice(alloc, "&SessionDuration=");
    try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.session_duration}) catch "");
