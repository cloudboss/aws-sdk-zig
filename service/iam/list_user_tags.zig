...
pub const ListUserTagsOutput = struct {
    ...
    pub fn deinit(self: *const ListUserTagsOutput) void {
        if (self.marker) |v| {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUserTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListUserTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUserTagsOutput {
    _ = status;
    _ = headers;
    var result: ListUserTagsOutput = .{ .allocator = alloc };
