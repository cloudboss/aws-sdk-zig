...
pub const ListUserTagsInput = struct {
...
pub const ListUserTagsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUserTagsInput, options: CallOptions) !ListUserTagsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUserTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListUserTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUserTagsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListUserTagsResult")) break;
            },
            else => {},
        }
    }

    var result: ListUserTagsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
