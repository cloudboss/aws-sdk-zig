...
pub const UpdateSigningCertificateInput = struct {
    ...
    /// certificate can be used for programmatic calls to Amazon Web Services
    /// `Inactive` means that
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSigningCertificateInput, options: CallOptions) !UpdateSigningCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSigningCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UpdateSigningCertificate&Version=2010-05-08");
    try body_buf.appendSlice(allocator, "&CertificateId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.certificate_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateSigningCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
