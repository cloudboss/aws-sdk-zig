...
pub fn execute(client: *Client, input: UpdateSigningCertificateInput, options: Options) !UpdateSigningCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSigningCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateSigningCertificate&Version=2010-05-08");
    try body_buf.appendSlice(alloc, "&CertificateId=");
    try appendUrlEncoded(alloc, &body_buf, input.certificate_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSigningCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
