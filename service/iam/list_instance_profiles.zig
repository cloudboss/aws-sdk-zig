...
pub const ListInstanceProfilesInput = struct {
    ...
    /// (/), listing
    /// all instance profiles. This parameter allows (through its [regex
...
pub const ListInstanceProfilesOutput = struct {
    ...
    pub fn deinit(self: *ListInstanceProfilesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListInstanceProfilesInput, options: Options) !ListInstanceProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstanceProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListInstanceProfiles&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstanceProfilesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListInstanceProfilesResult")) break;
            },
            else => {},
        }
    }

    var result: ListInstanceProfilesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
