...
pub const DeleteAccountAliasOutput = struct {
    ...
    pub fn deinit(self: *const DeleteAccountAliasOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteAccountAliasInput, options: Options) !DeleteAccountAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAccountAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteAccountAlias&Version=2010-05-08");
    try body_buf.appendSlice(alloc, "&AccountAlias=");
    try appendUrlEncoded(alloc, &body_buf, input.account_alias);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAccountAliasOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeleteAccountAliasOutput = .{ .allocator = alloc };
