...
pub const GetRolePolicyInput = struct {
...
pub const GetRolePolicyOutput = struct {
    /// The policy document.
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRolePolicyInput, options: CallOptions) !GetRolePolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRolePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetRolePolicy&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRolePolicyOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetRolePolicyResult")) break;
            },
            else => {},
        }
    }

    var result: GetRolePolicyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
