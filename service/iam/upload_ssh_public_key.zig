...
pub const UploadSSHPublicKeyOutput = struct {
    ...
    pub fn deinit(self: *const UploadSSHPublicKeyOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: UploadSSHPublicKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.ssh_public_key_body);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !UploadSSHPublicKeyOutput {
    _ = status;
    _ = body;
    const result: UploadSSHPublicKeyOutput = .{ .allocator = alloc };
