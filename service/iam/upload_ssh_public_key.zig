...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UploadSSHPublicKeyInput, options: CallOptions) !UploadSSHPublicKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UploadSSHPublicKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UploadSSHPublicKey&Version=2010-05-08");
    try body_buf.appendSlice(allocator, "&SSHPublicKeyBody=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.ssh_public_key_body);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UploadSSHPublicKeyOutput {
    _ = status;
    _ = headers;
