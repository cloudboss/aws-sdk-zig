...
pub const ListRoleTagsInput = struct {
...
pub const ListRoleTagsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoleTagsInput, options: CallOptions) !ListRoleTagsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRoleTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListRoleTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRoleTagsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListRoleTagsResult")) break;
            },
            else => {},
        }
    }

    var result: ListRoleTagsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
