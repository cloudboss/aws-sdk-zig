...
pub const ListRoleTagsOutput = struct {
    ...
    pub fn deinit(self: *ListRoleTagsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListRoleTagsInput, options: Options) !ListRoleTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoleTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListRoleTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoleTagsOutput {
    _ = status;
    _ = headers;
