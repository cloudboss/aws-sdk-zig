...
pub const GetContextKeysForCustomPolicyInput = struct {
    /// A list of policies for which you want the list of context keys referenced in
    /// those
    /// policies. Each document is specified as a string containing the complete,
    /// valid JSON
...
pub const GetContextKeysForCustomPolicyOutput = struct {
    /// The list of context keys that are referenced in the input policies.
    context_key_names: ?[]const []const u8 = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *GetContextKeysForCustomPolicyOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetContextKeysForCustomPolicyInput, options: Options) !GetContextKeysForCustomPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContextKeysForCustomPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetContextKeysForCustomPolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContextKeysForCustomPolicyOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetContextKeysForCustomPolicyResult")) break;
            },
            else => {},
        }
    }

    var result: GetContextKeysForCustomPolicyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
