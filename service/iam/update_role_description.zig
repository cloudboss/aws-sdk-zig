...
pub const UpdateRoleDescriptionOutput = struct {
    /// A structure that contains details about the modified role.
    role: ?Role = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *UpdateRoleDescriptionOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UpdateRoleDescriptionInput, options: Options) !UpdateRoleDescriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoleDescriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateRoleDescription&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoleDescriptionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "UpdateRoleDescriptionResult")) break;
            },
            else => {},
        }
    }

    var result: UpdateRoleDescriptionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
