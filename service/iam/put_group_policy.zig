...
pub const PutGroupPolicyOutput = struct {
    ...
    pub fn deinit(self: *const PutGroupPolicyOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutGroupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PutGroupPolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutGroupPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: PutGroupPolicyOutput = .{ .allocator = alloc };
