...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPolicyVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListPolicyVersions&Version=2010-05-08");
    if (input.marker) |v| {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.versions = try serde.deserializepolicyDocumentVersionListType(allocator, &reader, "member");
                } else {
