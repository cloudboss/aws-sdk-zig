...
pub const ListUsersInput = struct {
    ...
    /// (/), listing
    /// all user names. This parameter allows (through its [regex
...
pub const ListUsersOutput = struct {
    ...
    pub fn deinit(self: *const ListUsersOutput) void {
        if (self.marker) |v| {
...
pub fn execute(client: *Client, input: ListUsersInput, options: Options) !ListUsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUsersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListUsers&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ListUsersOutput {
    _ = status;
    var result: ListUsersOutput = .{ .allocator = alloc };
