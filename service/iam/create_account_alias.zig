...
pub const CreateAccountAliasOutput = struct {
    ...
    pub fn deinit(self: *const CreateAccountAliasOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: CreateAccountAliasInput, options: Options) !CreateAccountAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccountAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateAccountAlias&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccountAliasOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: CreateAccountAliasOutput = .{ .allocator = alloc };
