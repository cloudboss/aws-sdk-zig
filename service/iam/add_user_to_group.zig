...
pub const AddUserToGroupOutput = struct {
    ...
    pub fn deinit(self: *const AddUserToGroupOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: AddUserToGroupInput, options: Options) !AddUserToGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddUserToGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AddUserToGroup&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddUserToGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: AddUserToGroupOutput = .{ .allocator = alloc };
