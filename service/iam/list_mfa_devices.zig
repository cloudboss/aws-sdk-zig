...
pub const ListMFADevicesOutput = struct {
    ...
    pub fn deinit(self: *ListMFADevicesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListMFADevicesInput, options: Options) !ListMFADevicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMFADevicesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListMFADevices&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMFADevicesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.mfa_devices = try serde.deserializemfaDeviceListType(&reader, alloc, "member");
                } else {
