...
pub const ListMFADevicesOutput = struct {
    ...
    mfa_devices: ?[]const MFADevice = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMFADevicesInput, options: CallOptions) !ListMFADevicesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMFADevicesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListMFADevices&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMFADevicesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListMFADevicesResult")) break;
            },
            else => {},
        }
    }

    var result: ListMFADevicesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                ...
                } else if (std.mem.eql(u8, e.local, "MFADevices")) {
                    result.mfa_devices = try serde.deserializemfaDeviceListType(allocator, &reader, "member");
                } else {
