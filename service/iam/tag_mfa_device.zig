...
pub const TagMFADeviceOutput = struct {
    ...
    pub fn deinit(self: *const TagMFADeviceOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: TagMFADeviceInput, options: Options) !TagMFADeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TagMFADeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=TagMFADevice&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TagMFADeviceOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: TagMFADeviceOutput = .{ .allocator = alloc };
