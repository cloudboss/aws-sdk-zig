...
pub const GetLoginProfileInput = struct {
...
pub const GetLoginProfileOutput = struct {
    /// A structure containing the user name and the profile creation date for the
    /// user.
    login_profile: ?LoginProfile = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLoginProfileInput, options: CallOptions) !GetLoginProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLoginProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetLoginProfile&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLoginProfileOutput {
    _ = status;
    _ = headers;
