...
pub const UpdateAccessKeyInput = struct {
...
pub const UpdateAccessKeyOutput = struct {
    ...
    pub fn deinit(self: *const UpdateAccessKeyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UpdateAccessKeyInput, options: Options) !UpdateAccessKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccessKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateAccessKey&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccessKeyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UpdateAccessKeyOutput = .{ .allocator = alloc };
