...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRoleInput, options: CallOptions) !GetRoleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetRole&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRoleOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetRoleResult")) break;
            },
            else => {},
        }
    }

    var result: GetRoleOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
