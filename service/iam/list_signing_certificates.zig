...
pub fn execute(client: *Client, input: ListSigningCertificatesInput, options: Options) !ListSigningCertificatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSigningCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListSigningCertificates&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ListSigningCertificatesOutput {
    _ = status;
