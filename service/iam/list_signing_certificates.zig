...
pub const ListSigningCertificatesInput = struct {
...
pub const ListSigningCertificatesOutput = struct {
    /// A list of the user's signing certificate information.
    certificates: ?[]const SigningCertificate = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSigningCertificatesInput, options: CallOptions) !ListSigningCertificatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSigningCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListSigningCertificates&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSigningCertificatesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Certificates")) {
                    result.certificates = try serde.deserializecertificateListType(allocator, &reader, "member");
