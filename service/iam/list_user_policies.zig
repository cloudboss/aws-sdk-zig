...
pub const ListUserPoliciesOutput = struct {
    ...
    pub fn deinit(self: *ListUserPoliciesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListUserPoliciesInput, options: Options) !ListUserPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUserPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListUserPolicies&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUserPoliciesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListUserPoliciesResult")) break;
            },
            else => {},
        }
    }

    var result: ListUserPoliciesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
