...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUserPoliciesInput, options: CallOptions) !ListUserPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUserPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListUserPolicies&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUserPoliciesOutput {
    _ = status;
    _ = headers;
