...
pub const DisableOrganizationsRootSessionsOutput = struct {
    ...
    pub fn deinit(self: *DisableOrganizationsRootSessionsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DisableOrganizationsRootSessionsInput, options: Options) !DisableOrganizationsRootSessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableOrganizationsRootSessionsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=DisableOrganizationsRootSessions&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableOrganizationsRootSessionsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DisableOrganizationsRootSessionsResult")) break;
            },
            else => {},
        }
    }

    var result: DisableOrganizationsRootSessionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
