...
pub const ListGroupsForUserInput = struct {
...
pub const ListGroupsForUserOutput = struct {
    /// A list of groups.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGroupsForUserInput, options: CallOptions) !ListGroupsForUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListGroupsForUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListGroupsForUser&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListGroupsForUserOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListGroupsForUserResult")) break;
            },
            else => {},
        }
    }

    var result: ListGroupsForUserOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
