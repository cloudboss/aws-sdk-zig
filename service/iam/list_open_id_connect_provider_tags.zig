...
pub const ListOpenIDConnectProviderTagsInput = struct {
...
pub const ListOpenIDConnectProviderTagsOutput = struct {
    ...
    /// identity
    /// provider. Each tag consists of a key name and an associated value. If no
    /// tags are attached to the specified resource, the response contains an empty
    /// list.
    tags: ?[]const Tag = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *ListOpenIDConnectProviderTagsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListOpenIDConnectProviderTagsInput, options: Options) !ListOpenIDConnectProviderTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOpenIDConnectProviderTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListOpenIDConnectProviderTags&Version=2010-05-08");
    if (input.marker) |v| {
    ...
    try body_buf.appendSlice(alloc, "&OpenIDConnectProviderArn=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.open_id_connect_provider_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOpenIDConnectProviderTagsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListOpenIDConnectProviderTagsResult")) break;
            },
            else => {},
        }
    }

    var result: ListOpenIDConnectProviderTagsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
