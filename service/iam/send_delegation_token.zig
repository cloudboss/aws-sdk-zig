...
pub const SendDelegationTokenOutput = struct {
    ...
    pub fn deinit(self: *const SendDelegationTokenOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: SendDelegationTokenInput, options: Options) !SendDelegationTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendDelegationTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SendDelegationToken&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendDelegationTokenOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: SendDelegationTokenOutput = .{ .allocator = alloc };
