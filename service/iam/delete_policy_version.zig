...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePolicyVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeletePolicyVersion&Version=2010-05-08");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.version_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePolicyVersionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
