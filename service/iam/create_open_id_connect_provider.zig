...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOpenIDConnectProviderInput, options: Options) !CreateOpenIDConnectProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOpenIDConnectProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.url);
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "OpenIDConnectProviderArn")) {
                    result.open_id_connect_provider_arn = try alloc.dupe(u8, try reader.readElementText());
