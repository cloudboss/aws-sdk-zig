...
pub const CreatePolicyVersionOutput = struct {
    ...
    pub fn deinit(self: *const CreatePolicyVersionOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreatePolicyVersionOutput {
    _ = status;
    _ = body;
    const result: CreatePolicyVersionOutput = .{ .allocator = alloc };
