...
pub fn execute(client: *Client, input: GetServiceLinkedRoleDeletionStatusInput, options: Options) !GetServiceLinkedRoleDeletionStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceLinkedRoleDeletionStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetServiceLinkedRoleDeletionStatus&Version=2010-05-08");
    try body_buf.appendSlice(alloc, "&DeletionTaskId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.deletion_task_id);
