...
pub const SetSecurityTokenServicePreferencesOutput = struct {
    ...
    pub fn deinit(self: *SetSecurityTokenServicePreferencesOutput) void {
        self._arena.deinit();
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetSecurityTokenServicePreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.global_endpoint_token_version));
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetSecurityTokenServicePreferencesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
