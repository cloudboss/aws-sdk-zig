...
pub const DeleteGroupPolicyOutput = struct {
    ...
    pub fn deinit(self: *const DeleteGroupPolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteGroupPolicyInput, options: Options) !DeleteGroupPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGroupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteGroupPolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGroupPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeleteGroupPolicyOutput = .{ .allocator = alloc };
