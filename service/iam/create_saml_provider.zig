...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSAMLProviderInput, options: CallOptions) !CreateSAMLProviderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSAMLProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateSAMLProvider&Version=2010-05-08");
    if (input.add_private_key) |v| {
        try body_buf.appendSlice(allocator, "&AddPrivateKey=");
    ...
    try body_buf.appendSlice(allocator, "&SAMLMetadataDocument=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.saml_metadata_document);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSAMLProviderOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SAMLProviderArn")) {
                    result.saml_provider_arn = try allocator.dupe(u8, try reader.readElementText());
