...
pub const ListSAMLProviderTagsInput = struct {
...
pub const ListSAMLProviderTagsOutput = struct {
    ...
    /// Markup Language
    /// (SAML) identity provider. Each tag consists of a key name and an associated
    /// value. If no tags are attached to the specified resource, the response
    /// contains an empty list.
    tags: ?[]const Tag = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSAMLProviderTagsInput, options: CallOptions) !ListSAMLProviderTagsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSAMLProviderTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListSAMLProviderTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSAMLProviderTagsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListSAMLProviderTagsResult")) break;
            },
            else => {},
        }
    }

    var result: ListSAMLProviderTagsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
