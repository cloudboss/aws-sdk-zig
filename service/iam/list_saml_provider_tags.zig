...
pub const ListSAMLProviderTagsOutput = struct {
    ...
    pub fn deinit(self: *ListSAMLProviderTagsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListSAMLProviderTagsInput, options: Options) !ListSAMLProviderTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSAMLProviderTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListSAMLProviderTags&Version=2010-05-08");
    if (input.marker) |v| {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.saml_provider_arn);
