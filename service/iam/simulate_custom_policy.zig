...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SimulateCustomPolicyInput, options: CallOptions) !SimulateCustomPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SimulateCustomPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=SimulateCustomPolicy&Version=2010-05-08");
    ...
    if (input.caller_arn) |v| {
        try body_buf.appendSlice(allocator, "&CallerArn=");
    ...
    if (input.resource_handling_option) |v| {
        try body_buf.appendSlice(allocator, "&ResourceHandlingOption=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    ...
    if (input.resource_policy) |v| {
        try body_buf.appendSlice(allocator, "&ResourcePolicy=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SimulateCustomPolicyOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EvaluationResults")) {
                    result.evaluation_results = try serde.deserializeEvaluationResultsListType(allocator, &reader, "member");
