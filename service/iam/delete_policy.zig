...
pub const DeletePolicyOutput = struct {
    ...
    pub fn deinit(self: *const DeletePolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeletePolicyInput, options: Options) !DeletePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeletePolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeletePolicyOutput {
    _ = status;
    _ = body;
    const result: DeletePolicyOutput = .{ .allocator = alloc };
