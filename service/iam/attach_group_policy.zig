...
pub const AttachGroupPolicyOutput = struct {
    ...
    pub fn deinit(self: *const AttachGroupPolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: AttachGroupPolicyInput, options: Options) !AttachGroupPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachGroupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AttachGroupPolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachGroupPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: AttachGroupPolicyOutput = .{ .allocator = alloc };
