...
pub const EnableOutboundWebIdentityFederationOutput = struct {
    ...
    /// OpenID Connect (OIDC) discovery endpoints
    /// at `/.well-known/openid-configuration and /.well-known/jwks.json`. The
    /// OpenID Connect (OIDC) discovery endpoints contain verification keys and
    /// metadata necessary
    ...
    pub fn deinit(self: *EnableOutboundWebIdentityFederationOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: EnableOutboundWebIdentityFederationInput, options: Options) !EnableOutboundWebIdentityFederationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableOutboundWebIdentityFederationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=EnableOutboundWebIdentityFederation&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableOutboundWebIdentityFederationOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EnableOutboundWebIdentityFederationResult")) break;
            },
            else => {},
        }
    }

    var result: EnableOutboundWebIdentityFederationOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IssuerIdentifier")) {
                    result.issuer_identifier = try alloc.dupe(u8, try reader.readElementText());
                } else {
