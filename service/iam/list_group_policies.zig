...
pub const ListGroupPoliciesInput = struct {
...
pub const ListGroupPoliciesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGroupPoliciesInput, options: CallOptions) !ListGroupPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListGroupPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListGroupPolicies&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListGroupPoliciesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListGroupPoliciesResult")) break;
            },
            else => {},
        }
    }

    var result: ListGroupPoliciesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.policy_names = try serde.deserializepolicyNameListType(allocator, &reader, "member");
                } else {
