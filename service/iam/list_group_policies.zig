...
pub const ListGroupPoliciesOutput = struct {
    ...
    pub fn deinit(self: *ListGroupPoliciesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListGroupPoliciesInput, options: Options) !ListGroupPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListGroupPolicies&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupPoliciesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.policy_names = try serde.deserializepolicyNameListType(&reader, alloc, "member");
                } else {
