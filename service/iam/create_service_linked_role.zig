...
pub const CreateServiceLinkedRoleOutput = struct {
    ...
    pub fn deinit(self: *const CreateServiceLinkedRoleOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceLinkedRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.aws_service_name);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateServiceLinkedRoleOutput {
    _ = status;
    _ = body;
    const result: CreateServiceLinkedRoleOutput = .{ .allocator = alloc };
