...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableOrganizationsRootCredentialsManagementInput, options: CallOptions) !EnableOrganizationsRootCredentialsManagementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableOrganizationsRootCredentialsManagementInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=EnableOrganizationsRootCredentialsManagement&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableOrganizationsRootCredentialsManagementOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EnableOrganizationsRootCredentialsManagementResult")) break;
            },
            else => {},
        }
    }

    var result: EnableOrganizationsRootCredentialsManagementOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
