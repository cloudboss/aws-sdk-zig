...
pub const DeleteServiceLinkedRoleInput = struct {
    /// The name of the service-linked role to be deleted.
    role_name: []const u8,
};

pub const DeleteServiceLinkedRoleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteServiceLinkedRoleInput, options: CallOptions) !DeleteServiceLinkedRoleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteServiceLinkedRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteServiceLinkedRole&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteServiceLinkedRoleOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteServiceLinkedRoleResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteServiceLinkedRoleOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeletionTaskId")) {
                    result.deletion_task_id = try allocator.dupe(u8, try reader.readElementText());
                } else {
