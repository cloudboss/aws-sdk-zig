...
pub const UpdateAssumeRolePolicyOutput = struct {
    ...
    pub fn deinit(self: *const UpdateAssumeRolePolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UpdateAssumeRolePolicyInput, options: Options) !UpdateAssumeRolePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssumeRolePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateAssumeRolePolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssumeRolePolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UpdateAssumeRolePolicyOutput = .{ .allocator = alloc };
