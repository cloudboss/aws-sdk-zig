...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListPolicies&Version=2010-05-08");
    if (input.marker) |v| {
    ...
    if (input.only_attached) |v| {
        try body_buf.appendSlice(allocator, "&OnlyAttached=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPoliciesOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListPoliciesResult")) break;
            },
            else => {},
        }
    }
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.policies = try serde.deserializepolicyListType(allocator, &reader, "member");
                } else {
