...
pub const UpdateUserOutput = struct {
    ...
    pub fn deinit(self: *const UpdateUserOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UpdateUserInput, options: Options) !UpdateUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateUser&Version=2010-05-08");
    if (input.new_path) |v| {
        try body_buf.appendSlice(alloc, "&NewPath=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.new_user_name) |v| {
        try body_buf.appendSlice(alloc, "&NewUserName=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !UpdateUserOutput {
    _ = status;
    _ = body;
    const result: UpdateUserOutput = .{ .allocator = alloc };
