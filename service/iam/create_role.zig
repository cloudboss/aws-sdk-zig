...
pub const CreateRoleOutput = struct {
    ...
    pub fn deinit(self: *const CreateRoleOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: CreateRoleInput, options: Options) !CreateRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.assume_role_policy_document);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateRoleOutput {
    _ = status;
    _ = body;
    const result: CreateRoleOutput = .{ .allocator = alloc };
