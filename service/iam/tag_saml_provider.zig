...
pub const TagSAMLProviderOutput = struct {
    ...
    pub fn deinit(self: *const TagSAMLProviderOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !TagSAMLProviderOutput {
    _ = status;
    _ = body;
    const result: TagSAMLProviderOutput = .{ .allocator = alloc };
