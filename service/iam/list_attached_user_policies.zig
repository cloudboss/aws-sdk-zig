...
pub const ListAttachedUserPoliciesOutput = struct {
    ...
    pub fn deinit(self: *ListAttachedUserPoliciesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListAttachedUserPoliciesInput, options: Options) !ListAttachedUserPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAttachedUserPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListAttachedUserPolicies&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAttachedUserPoliciesOutput {
    _ = status;
    _ = headers;
