...
pub const ListAttachedUserPoliciesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAttachedUserPoliciesInput, options: CallOptions) !ListAttachedUserPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAttachedUserPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListAttachedUserPolicies&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAttachedUserPoliciesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListAttachedUserPoliciesResult")) break;
            },
            else => {},
        }
    }

    var result: ListAttachedUserPoliciesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
