...
pub const DeleteInstanceProfileOutput = struct {
    ...
    pub fn deinit(self: *const DeleteInstanceProfileOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteInstanceProfileInput, options: Options) !DeleteInstanceProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteInstanceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.instance_profile_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteInstanceProfileOutput {
    ...
    _ = headers;
    _ = body;
    const result: DeleteInstanceProfileOutput = .{ .allocator = alloc };
