...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteInstanceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteInstanceProfile&Version=2010-05-08");
    try body_buf.appendSlice(allocator, "&InstanceProfileName=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.instance_profile_name);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteInstanceProfileOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
