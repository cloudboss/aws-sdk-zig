...
pub const UpdateServerCertificateOutput = struct {
    ...
    pub fn deinit(self: *const UpdateServerCertificateOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UpdateServerCertificateInput, options: Options) !UpdateServerCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServerCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateServerCertificate&Version=2010-05-08");
    if (input.new_path) |v| {
        try body_buf.appendSlice(alloc, "&NewPath=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.new_server_certificate_name) |v| {
        try body_buf.appendSlice(alloc, "&NewServerCertificateName=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServerCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UpdateServerCertificateOutput = .{ .allocator = alloc };
