...
pub const ListAccountAliasesOutput = struct {
    /// A list of aliases associated with the account. Amazon Web Services supports
    /// only one alias per
    /// account.
    account_aliases: ?[]const []const u8 = null,
    ...
    pub fn deinit(self: *ListAccountAliasesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListAccountAliasesInput, options: Options) !ListAccountAliasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListAccountAliases&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountAliasesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListAccountAliasesResult")) break;
            },
            else => {},
        }
    }

    var result: ListAccountAliasesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccountAliases")) {
                    result.account_aliases = try serde.deserializeaccountAliasListType(&reader, alloc, "member");
