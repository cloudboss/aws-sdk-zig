...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountAliasesInput, options: CallOptions) !ListAccountAliasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAccountAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListAccountAliases&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAccountAliasesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccountAliases")) {
                    result.account_aliases = try serde.deserializeaccountAliasListType(allocator, &reader, "member");
