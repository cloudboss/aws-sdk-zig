...
pub const UpdateGroupOutput = struct {
    ...
    pub fn deinit(self: *const UpdateGroupOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UpdateGroupInput, options: Options) !UpdateGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateGroup&Version=2010-05-08");
    try body_buf.appendSlice(alloc, "&GroupName=");
    try appendUrlEncoded(alloc, &body_buf, input.group_name);
    if (input.new_group_name) |v| {
        try body_buf.appendSlice(alloc, "&NewGroupName=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.new_path) |v| {
        try body_buf.appendSlice(alloc, "&NewPath=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UpdateGroupOutput = .{ .allocator = alloc };
