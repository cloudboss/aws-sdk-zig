...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableOrganizationsRootSessionsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=EnableOrganizationsRootSessions&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableOrganizationsRootSessionsOutput {
    _ = status;
    _ = headers;
