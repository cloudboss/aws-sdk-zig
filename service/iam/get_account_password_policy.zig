...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountPasswordPolicyInput, options: CallOptions) !GetAccountPasswordPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAccountPasswordPolicyInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=GetAccountPasswordPolicy&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAccountPasswordPolicyOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PasswordPolicy")) {
                    result.password_policy = try serde.deserializePasswordPolicy(allocator, &reader);
                } else {
