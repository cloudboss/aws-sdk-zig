...
pub const ListRolePoliciesInput = struct {
...
pub const ListRolePoliciesOutput = struct {
    ...
    pub fn deinit(self: *ListRolePoliciesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListRolePoliciesInput, options: Options) !ListRolePoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRolePoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListRolePolicies&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRolePoliciesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListRolePoliciesResult")) break;
            },
            else => {},
        }
    }

    var result: ListRolePoliciesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
