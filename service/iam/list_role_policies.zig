...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRolePoliciesInput, options: CallOptions) !ListRolePoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRolePoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListRolePolicies&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRolePoliciesOutput {
    _ = status;
    _ = headers;
