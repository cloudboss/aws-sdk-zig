...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOpenIDConnectProvidersInput, options: CallOptions) !ListOpenIDConnectProvidersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOpenIDConnectProvidersInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=ListOpenIDConnectProviders&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOpenIDConnectProvidersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "OpenIDConnectProviderList")) {
                    result.open_id_connect_provider_list = try serde.deserializeOpenIDConnectProviderListType(allocator, &reader, "member");
                } else {
