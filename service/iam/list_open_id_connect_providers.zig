...
pub const ListOpenIDConnectProvidersOutput = struct {
    ...
    /// Services account.
    open_id_connect_provider_list: ?[]const OpenIDConnectProviderListEntry = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *ListOpenIDConnectProvidersOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListOpenIDConnectProvidersInput, options: Options) !ListOpenIDConnectProvidersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOpenIDConnectProvidersInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=ListOpenIDConnectProviders&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOpenIDConnectProvidersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListOpenIDConnectProvidersResult")) break;
            },
            else => {},
        }
    }

    var result: ListOpenIDConnectProvidersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "OpenIDConnectProviderList")) {
                    result.open_id_connect_provider_list = try serde.deserializeOpenIDConnectProviderListType(&reader, alloc, "member");
                } else {
