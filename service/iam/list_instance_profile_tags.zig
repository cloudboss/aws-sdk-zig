...
pub const ListInstanceProfileTagsInput = struct {
...
pub const ListInstanceProfileTagsOutput = struct {
    ...
    pub fn deinit(self: *ListInstanceProfileTagsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListInstanceProfileTagsInput, options: Options) !ListInstanceProfileTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstanceProfileTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListInstanceProfileTags&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstanceProfileTagsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListInstanceProfileTagsResult")) break;
            },
            else => {},
        }
    }

    var result: ListInstanceProfileTagsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
