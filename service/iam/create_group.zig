...
pub const CreateGroupOutput = struct {
    ...
    pub fn deinit(self: *CreateGroupOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: CreateGroupInput, options: Options) !CreateGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateGroup&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGroupOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateGroupResult")) break;
            },
            else => {},
        }
    }
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Group")) {
                    result.group = try serde.deserializeGroup(&reader, alloc);
                } else {
