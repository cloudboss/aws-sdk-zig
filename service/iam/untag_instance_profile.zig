...
/// the
/// *IAM User Guide*.
pub const UntagInstanceProfileInput = struct {
...
pub const UntagInstanceProfileOutput = struct {
    ...
    pub fn deinit(self: *const UntagInstanceProfileOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UntagInstanceProfileInput, options: Options) !UntagInstanceProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagInstanceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UntagInstanceProfile&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UntagInstanceProfileOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UntagInstanceProfileOutput = .{ .allocator = alloc };
