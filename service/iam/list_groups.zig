...
pub const ListGroupsInput = struct {
    ...
    /// (/), listing
    /// all groups. This parameter allows (through its [regex
...
pub const ListGroupsOutput = struct {
    ...
    pub fn deinit(self: *ListGroupsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListGroupsInput, options: Options) !ListGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListGroups&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupsOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListGroupsResult")) break;
            },
            else => {},
        }
    }
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Groups")) {
                    result.groups = try serde.deserializegroupListType(&reader, alloc, "member");
