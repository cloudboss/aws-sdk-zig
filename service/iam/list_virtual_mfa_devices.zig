...
pub const ListVirtualMFADevicesInput = struct {
    ...
    /// devices.,
    assignment_status: ?assignmentStatusType = null,
...
pub const ListVirtualMFADevicesOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVirtualMFADevicesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVirtualMFADevicesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.virtual_mfa_devices = try serde.deserializevirtualMFADeviceListType(allocator, &reader, "member");
                } else {
