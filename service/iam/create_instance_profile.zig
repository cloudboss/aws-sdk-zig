...
pub const CreateInstanceProfileOutput = struct {
    ...
    pub fn deinit(self: *const CreateInstanceProfileOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: CreateInstanceProfileInput, options: Options) !CreateInstanceProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInstanceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateInstanceProfile&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateInstanceProfileOutput {
    _ = status;
    _ = body;
    const result: CreateInstanceProfileOutput = .{ .allocator = alloc };
