...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInstanceProfilesForRoleInput, options: CallOptions) !ListInstanceProfilesForRoleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListInstanceProfilesForRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListInstanceProfilesForRole&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListInstanceProfilesForRoleOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InstanceProfiles")) {
                    result.instance_profiles = try serde.deserializeinstanceProfileListType(allocator, &reader, "member");
