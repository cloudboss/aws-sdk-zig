...
pub const ListInstanceProfilesForRoleOutput = struct {
    ...
    pub fn deinit(self: *ListInstanceProfilesForRoleOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListInstanceProfilesForRoleInput, options: Options) !ListInstanceProfilesForRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstanceProfilesForRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListInstanceProfilesForRole&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstanceProfilesForRoleOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListInstanceProfilesForRoleResult")) break;
            },
            else => {},
        }
    }

    var result: ListInstanceProfilesForRoleOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InstanceProfiles")) {
                    result.instance_profiles = try serde.deserializeinstanceProfileListType(&reader, alloc, "member");
