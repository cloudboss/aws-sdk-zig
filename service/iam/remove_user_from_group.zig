...
pub const RemoveUserFromGroupOutput = struct {
    ...
    pub fn deinit(self: *const RemoveUserFromGroupOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: RemoveUserFromGroupInput, options: Options) !RemoveUserFromGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveUserFromGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RemoveUserFromGroup&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveUserFromGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: RemoveUserFromGroupOutput = .{ .allocator = alloc };
