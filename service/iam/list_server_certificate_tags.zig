...
pub const ListServerCertificateTagsOutput = struct {
    ...
    pub fn deinit(self: *ListServerCertificateTagsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListServerCertificateTagsInput, options: Options) !ListServerCertificateTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServerCertificateTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListServerCertificateTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServerCertificateTagsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListServerCertificateTagsResult")) break;
            },
            else => {},
        }
    }

    var result: ListServerCertificateTagsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
