...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServerCertificateTagsInput, options: CallOptions) !ListServerCertificateTagsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListServerCertificateTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListServerCertificateTags&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListServerCertificateTagsOutput {
    _ = status;
    _ = headers;
