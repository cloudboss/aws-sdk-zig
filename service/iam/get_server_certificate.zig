...
pub const GetServerCertificateOutput = struct {
    /// A structure containing details about the server certificate.
    server_certificate: ?ServerCertificate = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *GetServerCertificateOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetServerCertificateInput, options: Options) !GetServerCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServerCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetServerCertificate&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServerCertificateOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetServerCertificateResult")) break;
            },
            else => {},
        }
    }

    var result: GetServerCertificateOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ServerCertificate")) {
                    result.server_certificate = try serde.deserializeServerCertificate(&reader, alloc);
                } else {
