...
pub fn execute(client: *Client, input: ListServiceSpecificCredentialsInput, options: Options) !ListServiceSpecificCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServiceSpecificCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListServiceSpecificCredentials&Version=2010-05-08");
    if (input.all_users) |v| {
        try body_buf.appendSlice(alloc, "&AllUsers=");
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ListServiceSpecificCredentialsOutput {
    _ = status;
