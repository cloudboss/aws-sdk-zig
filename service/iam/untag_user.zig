...
pub const UntagUserOutput = struct {

    allocator: std.mem.Allocator,

    pub fn deinit(self: *const UntagUserOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UntagUserInput, options: Options) !UntagUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UntagUser&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UntagUserOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UntagUserOutput = .{ .allocator = alloc };
