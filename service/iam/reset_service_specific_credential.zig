...
pub const ResetServiceSpecificCredentialInput = struct {
    ...
    /// this
    /// value is not specified, then the operation assumes the user whose
...
pub const ResetServiceSpecificCredentialOutput = struct {
    ...
    pub fn deinit(self: *ResetServiceSpecificCredentialOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ResetServiceSpecificCredentialInput, options: Options) !ResetServiceSpecificCredentialOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetServiceSpecificCredentialInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ResetServiceSpecificCredential&Version=2010-05-08");
    try body_buf.appendSlice(alloc, "&ServiceSpecificCredentialId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.service_specific_credential_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetServiceSpecificCredentialOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ResetServiceSpecificCredentialResult")) break;
            },
            else => {},
        }
    }

    var result: ResetServiceSpecificCredentialOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ServiceSpecificCredential")) {
                    result.service_specific_credential = try serde.deserializeServiceSpecificCredential(&reader, alloc);
                } else {
