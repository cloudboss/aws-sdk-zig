...
pub const AttachUserPolicyOutput = struct {
    ...
    pub fn deinit(self: *const AttachUserPolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: AttachUserPolicyInput, options: Options) !AttachUserPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachUserPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AttachUserPolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachUserPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: AttachUserPolicyOutput = .{ .allocator = alloc };
