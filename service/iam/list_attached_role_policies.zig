...
pub const ListAttachedRolePoliciesOutput = struct {
    ...
    pub fn deinit(self: *ListAttachedRolePoliciesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListAttachedRolePoliciesInput, options: Options) !ListAttachedRolePoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAttachedRolePoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListAttachedRolePolicies&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAttachedRolePoliciesOutput {
    _ = status;
    _ = headers;
