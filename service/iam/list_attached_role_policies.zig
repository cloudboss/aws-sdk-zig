...
pub const ListAttachedRolePoliciesInput = struct {
...
pub const ListAttachedRolePoliciesOutput = struct {
    /// A list of the attached policies.
    attached_policies: ?[]const AttachedPolicy = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAttachedRolePoliciesInput, options: CallOptions) !ListAttachedRolePoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAttachedRolePoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListAttachedRolePolicies&Version=2010-05-08");
    if (input.marker) |v| {
    ...
    if (input.path_prefix) |v| {
        try body_buf.appendSlice(allocator, "&PathPrefix=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAttachedRolePoliciesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListAttachedRolePoliciesResult")) break;
            },
            else => {},
        }
    }

    var result: ListAttachedRolePoliciesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AttachedPolicies")) {
                    result.attached_policies = try serde.deserializeattachedPoliciesListType(allocator, &reader, "member");
