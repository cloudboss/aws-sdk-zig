...
pub fn execute(client: *Client, input: UpdateSAMLProviderInput, options: Options) !UpdateSAMLProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSAMLProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateSAMLProvider&Version=2010-05-08");
    ...
    if (input.remove_private_key) |v| {
        try body_buf.appendSlice(alloc, "&RemovePrivateKey=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !UpdateSAMLProviderOutput {
    _ = status;
