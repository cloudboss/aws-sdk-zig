...
fn serializeRequest(allocator: std.mem.Allocator, input: ListServerCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListServerCertificates&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListServerCertificatesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.server_certificate_metadata_list = try serde.deserializeserverCertificateMetadataListType(allocator, &reader, "member");
                } else {
