...
pub const DetachGroupPolicyOutput = struct {
    ...
    pub fn deinit(self: *const DetachGroupPolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DetachGroupPolicyInput, options: Options) !DetachGroupPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachGroupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DetachGroupPolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachGroupPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DetachGroupPolicyOutput = .{ .allocator = alloc };
