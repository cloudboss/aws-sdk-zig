...
pub const UpdateLoginProfileOutput = struct {
    ...
    pub fn deinit(self: *const UpdateLoginProfileOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UpdateLoginProfileInput, options: Options) !UpdateLoginProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLoginProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateLoginProfile&Version=2010-05-08");
    if (input.password) |v| {
        try body_buf.appendSlice(alloc, "&Password=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.password_reset_required) |v| {
        try body_buf.appendSlice(alloc, "&PasswordResetRequired=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLoginProfileOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UpdateLoginProfileOutput = .{ .allocator = alloc };
