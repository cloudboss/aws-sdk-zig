...
pub const DeactivateMFADeviceOutput = struct {
    ...
    pub fn deinit(self: *const DeactivateMFADeviceOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeactivateMFADeviceInput, options: Options) !DeactivateMFADeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeactivateMFADeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeactivateMFADevice&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeactivateMFADeviceOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeactivateMFADeviceOutput = .{ .allocator = alloc };
