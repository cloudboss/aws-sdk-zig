...
/// *IAM User Guide*.
pub const DetachUserPolicyInput = struct {
...
pub const DetachUserPolicyOutput = struct {
    ...
    pub fn deinit(self: *const DetachUserPolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DetachUserPolicyInput, options: Options) !DetachUserPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachUserPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DetachUserPolicy&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachUserPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DetachUserPolicyOutput = .{ .allocator = alloc };
