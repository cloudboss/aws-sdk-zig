...
pub const ListSSHPublicKeysInput = struct {
    ...
    /// the
    /// `UserName` field is determined implicitly based on the Amazon Web Services
    /// access key
...
pub const ListSSHPublicKeysOutput = struct {
    ...
    ssh_public_keys: ?[]const SSHPublicKeyMetadata = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSSHPublicKeysInput, options: CallOptions) !ListSSHPublicKeysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSSHPublicKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListSSHPublicKeys&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSSHPublicKeysOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListSSHPublicKeysResult")) break;
            },
            else => {},
        }
    }

    var result: ListSSHPublicKeysOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                ...
                } else if (std.mem.eql(u8, e.local, "SSHPublicKeys")) {
                    result.ssh_public_keys = try serde.deserializeSSHPublicKeyListType(allocator, &reader, "member");
                } else {
