...
pub const ListSSHPublicKeysOutput = struct {
    ...
    pub fn deinit(self: *ListSSHPublicKeysOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListSSHPublicKeysInput, options: Options) !ListSSHPublicKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSSHPublicKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListSSHPublicKeys&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSSHPublicKeysOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.ssh_public_keys = try serde.deserializeSSHPublicKeyListType(&reader, alloc, "member");
                } else {
