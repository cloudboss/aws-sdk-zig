...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UntagMFADeviceInput, options: CallOptions) !UntagMFADeviceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UntagMFADeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UntagMFADevice&Version=2010-05-08");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UntagMFADeviceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: UntagMFADeviceOutput = .{};
