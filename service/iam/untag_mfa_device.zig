...
pub const UntagMFADeviceOutput = struct {
    ...
    pub fn deinit(self: *const UntagMFADeviceOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UntagMFADeviceInput, options: Options) !UntagMFADeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagMFADeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UntagMFADevice&Version=2010-05-08");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UntagMFADeviceOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: UntagMFADeviceOutput = .{ .allocator = alloc };
