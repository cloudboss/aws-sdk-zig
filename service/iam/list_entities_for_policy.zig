...
pub const ListEntitiesForPolicyOutput = struct {
    ...
    policy_groups: ?[]const PolicyGroup = null,

    /// A list of IAM roles that the policy is attached to.
    policy_roles: ?[]const PolicyRole = null,

    /// A list of IAM users that the policy is attached to.
    policy_users: ?[]const PolicyUser = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEntitiesForPolicyInput, options: CallOptions) !ListEntitiesForPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListEntitiesForPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListEntitiesForPolicyOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.policy_groups = try serde.deserializePolicyGroupListType(allocator, &reader, "member");
                } else if (std.mem.eql(u8, e.local, "PolicyRoles")) {
                    result.policy_roles = try serde.deserializePolicyRoleListType(allocator, &reader, "member");
                } else if (std.mem.eql(u8, e.local, "PolicyUsers")) {
                    result.policy_users = try serde.deserializePolicyUserListType(allocator, &reader, "member");
                } else {
