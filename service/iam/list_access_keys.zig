...
pub const ListAccessKeysOutput = struct {
    ...
    pub fn deinit(self: *ListAccessKeysOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ListAccessKeysInput, options: Options) !ListAccessKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListAccessKeys&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessKeysOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccessKeyMetadata")) {
                    result.access_key_metadata = try serde.deserializeaccessKeyMetadataListType(&reader, alloc, "member");
