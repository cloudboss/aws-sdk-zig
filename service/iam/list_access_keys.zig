...
pub const ListAccessKeysInput = struct {
...
pub const ListAccessKeysOutput = struct {
    /// A list of objects containing metadata about the access keys.
    access_key_metadata: ?[]const AccessKeyMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessKeysInput, options: CallOptions) !ListAccessKeysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAccessKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListAccessKeys&Version=2010-05-08");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAccessKeysOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListAccessKeysResult")) break;
            },
            else => {},
        }
    }
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccessKeyMetadata")) {
                    result.access_key_metadata = try serde.deserializeaccessKeyMetadataListType(allocator, &reader, "member");
