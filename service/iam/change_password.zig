...
pub const ChangePasswordOutput = struct {
    ...
    pub fn deinit(self: *const ChangePasswordOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: ChangePasswordInput, options: Options) !ChangePasswordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ChangePasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ChangePassword&Version=2010-05-08");
    try body_buf.appendSlice(alloc, "&NewPassword=");
    try appendUrlEncoded(alloc, &body_buf, input.new_password);
    try body_buf.appendSlice(alloc, "&OldPassword=");
    try appendUrlEncoded(alloc, &body_buf, input.old_password);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ChangePasswordOutput {
    _ = status;
    _ = body;
    const result: ChangePasswordOutput = .{ .allocator = alloc };
