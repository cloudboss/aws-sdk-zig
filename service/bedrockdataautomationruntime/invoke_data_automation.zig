...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InvokeDataAutomationInput, options: CallOptions) !InvokeDataAutomationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: InvokeDataAutomationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonBedrockKeystoneRuntimeService.InvokeDataAutomation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !InvokeDataAutomationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(InvokeDataAutomationOutput, body, allocator);
}
