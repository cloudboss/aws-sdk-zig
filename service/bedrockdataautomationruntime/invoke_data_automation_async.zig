...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InvokeDataAutomationAsyncInput, options: CallOptions) !InvokeDataAutomationAsyncOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: InvokeDataAutomationAsyncInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonBedrockKeystoneRuntimeService.InvokeDataAutomationAsync");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !InvokeDataAutomationAsyncOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(InvokeDataAutomationAsyncOutput, body, allocator);
}
