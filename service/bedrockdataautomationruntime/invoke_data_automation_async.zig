...
pub const InvokeDataAutomationAsyncInput = struct {
    /// Blueprint list.
...
pub const InvokeDataAutomationAsyncOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InvokeDataAutomationAsyncInput, options: Options) !InvokeDataAutomationAsyncOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InvokeDataAutomationAsyncInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonBedrockKeystoneRuntimeService.InvokeDataAutomationAsync");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InvokeDataAutomationAsyncOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(InvokeDataAutomationAsyncOutput, body, alloc);
}
