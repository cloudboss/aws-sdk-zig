...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppAssessmentInput, options: Options) !DescribeAppAssessmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAppAssessmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAppAssessmentOutput {
    var result: DescribeAppAssessmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAppAssessmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
