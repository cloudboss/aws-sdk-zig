...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMetricsExportInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMetricsExportOutput {
    var result: StartMetricsExportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMetricsExportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
