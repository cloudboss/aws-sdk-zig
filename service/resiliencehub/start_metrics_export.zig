...
pub const StartMetricsExportInput = struct {
...
pub const StartMetricsExportOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMetricsExportInput, options: CallOptions) !StartMetricsExportOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartMetricsExportInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/start-metrics-export";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartMetricsExportOutput {
    var result: StartMetricsExportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMetricsExportOutput, body, allocator);
    }
    _ = status;
    _ = headers;
