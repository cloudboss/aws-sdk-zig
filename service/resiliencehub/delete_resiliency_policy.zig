...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResiliencyPolicyInput, options: Options) !DeleteResiliencyPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteResiliencyPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteResiliencyPolicyOutput {
    var result: DeleteResiliencyPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteResiliencyPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
