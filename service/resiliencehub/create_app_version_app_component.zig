...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAppVersionAppComponentInput, options: Options) !CreateAppVersionAppComponentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAppVersionAppComponentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAppVersionAppComponentOutput {
    var result: CreateAppVersionAppComponentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppVersionAppComponentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
