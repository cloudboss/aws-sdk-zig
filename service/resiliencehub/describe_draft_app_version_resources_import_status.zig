...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDraftAppVersionResourcesImportStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDraftAppVersionResourcesImportStatusOutput {
    var result: DescribeDraftAppVersionResourcesImportStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDraftAppVersionResourcesImportStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
