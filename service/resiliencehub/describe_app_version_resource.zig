...
pub const DescribeAppVersionResourceInput = struct {
...
pub const DescribeAppVersionResourceOutput = struct {
    /// Amazon Resource Name (ARN) of the Resilience Hub application. The format for
    /// this ARN is:
    /// arn:`partition`:resiliencehub:`region`:`account`:app/`app-id`. For more
    /// information about ARNs,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppVersionResourceInput, options: CallOptions) !DescribeAppVersionResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAppVersionResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/describe-app-version-resource";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAppVersionResourceOutput {
    var result: DescribeAppVersionResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAppVersionResourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
