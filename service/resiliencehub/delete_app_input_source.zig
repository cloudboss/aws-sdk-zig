...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAppInputSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.eks_source_cluster_namespace) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"eksSourceClusterNamespace\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.terraform_source) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"terraformSource\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAppInputSourceOutput {
    var result: DeleteAppInputSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAppInputSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
