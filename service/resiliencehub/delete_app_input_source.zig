...
pub const DeleteAppInputSourceInput = struct {
...
pub const DeleteAppInputSourceOutput = struct {
    /// Amazon Resource Name (ARN) of the Resilience Hub application. The format for
    /// this ARN is:
    /// arn:`partition`:resiliencehub:`region`:`account`:app/`app-id`. For more
    /// information about ARNs,
    ...
    /// from.
    app_input_source: ?AppInputSource = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAppInputSourceInput, options: CallOptions) !DeleteAppInputSourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAppInputSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/delete-app-input-source";
    ...
    if (input.eks_source_cluster_namespace) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"eksSourceClusterNamespace\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    ...
    if (input.terraform_source) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"terraformSource\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAppInputSourceOutput {
    var result: DeleteAppInputSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAppInputSourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
