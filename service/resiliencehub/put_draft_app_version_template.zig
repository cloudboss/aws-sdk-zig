...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDraftAppVersionTemplateInput, options: CallOptions) !PutDraftAppVersionTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutDraftAppVersionTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"appTemplateBody\":");
    try aws.json.writeValue(@TypeOf(input.app_template_body), input.app_template_body, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutDraftAppVersionTemplateOutput {
    var result: PutDraftAppVersionTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutDraftAppVersionTemplateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
