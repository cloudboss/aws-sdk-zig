...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResiliencyPolicyInput, options: Options) !UpdateResiliencyPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResiliencyPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResiliencyPolicyOutput {
    var result: UpdateResiliencyPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResiliencyPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
