...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppVersionAppComponentInput, options: Options) !UpdateAppVersionAppComponentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAppVersionAppComponentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAppVersionAppComponentOutput {
    var result: UpdateAppVersionAppComponentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAppVersionAppComponentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
