...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAppVersionResourceInput, options: CallOptions) !DeleteAppVersionResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAppVersionResourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAppVersionResourceOutput {
    var result: DeleteAppVersionResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAppVersionResourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
