...
pub const DeleteAppVersionResourceInput = struct {
...
pub const DeleteAppVersionResourceOutput = struct {
    /// Amazon Resource Name (ARN) of the Resilience Hub application. The format for
    /// this ARN is:
    /// arn:`partition`:resiliencehub:`region`:`account`:app/`app-id`. For more
    /// information about ARNs,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAppVersionResourceInput, options: Options) !DeleteAppVersionResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAppVersionResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/delete-app-version-resource";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAppVersionResourceOutput {
    var result: DeleteAppVersionResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAppVersionResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
