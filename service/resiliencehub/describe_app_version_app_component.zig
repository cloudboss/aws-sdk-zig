...
pub const DescribeAppVersionAppComponentInput = struct {
...
pub const DescribeAppVersionAppComponentOutput = struct {
    /// Amazon Resource Name (ARN) of the Resilience Hub application. The format for
    /// this ARN is:
    /// arn:`partition`:resiliencehub:`region`:`account`:app/`app-id`. For more
    /// information about ARNs,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppVersionAppComponentInput, options: CallOptions) !DescribeAppVersionAppComponentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAppVersionAppComponentInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/describe-app-version-app-component";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAppVersionAppComponentOutput {
    var result: DescribeAppVersionAppComponentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAppVersionAppComponentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
