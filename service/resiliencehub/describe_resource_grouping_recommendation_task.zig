...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeResourceGroupingRecommendationTaskInput, options: Options) !DescribeResourceGroupingRecommendationTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeResourceGroupingRecommendationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.grouping_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"groupingId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeResourceGroupingRecommendationTaskOutput {
    var result: DescribeResourceGroupingRecommendationTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeResourceGroupingRecommendationTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
