...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAppVersionResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.app_components), input.app_components, alloc, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.logical_resource_id), input.logical_resource_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"physicalResourceId\":");
    try aws.json.writeValue(@TypeOf(input.physical_resource_id), input.physical_resource_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAppVersionResourceOutput {
    var result: CreateAppVersionResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppVersionResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
