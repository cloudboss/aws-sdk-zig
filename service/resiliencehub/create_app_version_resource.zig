...
pub const CreateAppVersionResourceInput = struct {
...
pub const CreateAppVersionResourceOutput = struct {
    /// Amazon Resource Name (ARN) of the Resilience Hub application. The format for
    /// this ARN is:
    /// arn:`partition`:resiliencehub:`region`:`account`:app/`app-id`. For more
    /// information about ARNs,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAppVersionResourceInput, options: CallOptions) !CreateAppVersionResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAppVersionResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/create-app-version-resource";
    ...
    try aws.json.writeValue(@TypeOf(input.app_components), input.app_components, allocator, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.logical_resource_id), input.logical_resource_id, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"physicalResourceId\":");
    try aws.json.writeValue(@TypeOf(input.physical_resource_id), input.physical_resource_id, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAppVersionResourceOutput {
    var result: CreateAppVersionResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppVersionResourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
