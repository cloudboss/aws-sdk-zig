...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddDraftAppVersionResourceMappingsInput, options: Options) !AddDraftAppVersionResourceMappingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddDraftAppVersionResourceMappingsInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"resourceMappings\":");
    try aws.json.writeValue(@TypeOf(input.resource_mappings), input.resource_mappings, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddDraftAppVersionResourceMappingsOutput {
    var result: AddDraftAppVersionResourceMappingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddDraftAppVersionResourceMappingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
