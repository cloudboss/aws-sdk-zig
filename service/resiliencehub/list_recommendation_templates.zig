...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecommendationTemplatesInput, options: Options) !ListRecommendationTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecommendationTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.assessment_arn) |v| {
        ...
        try query_buf.appendSlice(alloc, "assessmentArn=");
    ...
    if (input.recommendation_template_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "recommendationTemplateArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecommendationTemplatesOutput {
    var result: ListRecommendationTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRecommendationTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
