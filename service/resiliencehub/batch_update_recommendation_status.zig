...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateRecommendationStatusInput, options: Options) !BatchUpdateRecommendationStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateRecommendationStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"requestEntries\":");
    try aws.json.writeValue(@TypeOf(input.request_entries), input.request_entries, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateRecommendationStatusOutput {
    var result: BatchUpdateRecommendationStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateRecommendationStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
