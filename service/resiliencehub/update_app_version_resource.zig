...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppVersionResourceInput, options: CallOptions) !UpdateAppVersionResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAppVersionResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.app_components) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"appComponents\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    ...
    if (input.excluded) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"excluded\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAppVersionResourceOutput {
    var result: UpdateAppVersionResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAppVersionResourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
