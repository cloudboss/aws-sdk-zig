...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRecommendationTemplateInput, options: CallOptions) !CreateRecommendationTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRecommendationTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.recommendation_types) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"recommendationTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateRecommendationTemplateOutput {
    var result: CreateRecommendationTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRecommendationTemplateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
