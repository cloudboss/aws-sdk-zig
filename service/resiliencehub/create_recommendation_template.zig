...
pub const CreateRecommendationTemplateInput = struct {
    ...
    recommendation_types: ?[]const RenderRecommendationType = null,
...
pub const CreateRecommendationTemplateOutput = struct {
    ...
    recommendation_template: ?RecommendationTemplate = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRecommendationTemplateInput, options: Options) !CreateRecommendationTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRecommendationTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/create-recommendation-template";
    ...
    try body_buf.appendSlice(alloc, "\"assessmentArn\":");
    try aws.json.writeValue(@TypeOf(input.assessment_arn), input.assessment_arn, alloc, &body_buf);
    has_prev = true;
    if (input.bucket_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"bucketName\":");
    ...
    if (input.recommendation_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"recommendationIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.recommendation_types) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"recommendationTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRecommendationTemplateOutput {
    var result: CreateRecommendationTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRecommendationTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
