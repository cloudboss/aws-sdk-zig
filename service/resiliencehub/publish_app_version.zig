...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PublishAppVersionInput, options: Options) !PublishAppVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PublishAppVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PublishAppVersionOutput {
    var result: PublishAppVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PublishAppVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
