...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMetricsExportInput, options: Options) !DescribeMetricsExportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMetricsExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"metricsExportId\":");
    try aws.json.writeValue(@TypeOf(input.metrics_export_id), input.metrics_export_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMetricsExportOutput {
    var result: DescribeMetricsExportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeMetricsExportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
