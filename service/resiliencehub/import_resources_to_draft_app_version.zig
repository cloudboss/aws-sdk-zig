...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportResourcesToDraftAppVersionInput, options: Options) !ImportResourcesToDraftAppVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportResourcesToDraftAppVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.eks_sources) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"eksSources\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.import_strategy) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"importStrategy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_arns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sourceArns\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.terraform_sources) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"terraformSources\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportResourcesToDraftAppVersionOutput {
    var result: ImportResourcesToDraftAppVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportResourcesToDraftAppVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
