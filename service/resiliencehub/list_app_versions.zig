...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppVersionsInput, options: Options) !ListAppVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppVersionsOutput {
    var result: ListAppVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
