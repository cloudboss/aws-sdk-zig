...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppAssessmentComplianceDriftsInput, options: Options) !ListAppAssessmentComplianceDriftsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppAssessmentComplianceDriftsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppAssessmentComplianceDriftsOutput {
    var result: ListAppAssessmentComplianceDriftsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppAssessmentComplianceDriftsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
