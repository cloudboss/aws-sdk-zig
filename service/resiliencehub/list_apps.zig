...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.aws_application_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "awsApplicationArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.from_last_assessment_time) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "fromLastAssessmentTime=");
        {
    ...
    if (input.to_last_assessment_time) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "toLastAssessmentTime=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
