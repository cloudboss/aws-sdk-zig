...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppVersionResourceMappingsInput, options: Options) !ListAppVersionResourceMappingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppVersionResourceMappingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppVersionResourceMappingsOutput {
    var result: ListAppVersionResourceMappingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppVersionResourceMappingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
