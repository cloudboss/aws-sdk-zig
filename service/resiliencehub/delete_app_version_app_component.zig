...
pub const DeleteAppVersionAppComponentInput = struct {
...
pub const DeleteAppVersionAppComponentOutput = struct {
    /// Amazon Resource Name (ARN) of the Resilience Hub application. The format for
    /// this ARN is:
    /// arn:`partition`:resiliencehub:`region`:`account`:app/`app-id`. For more
    /// information about ARNs,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAppVersionAppComponentInput, options: Options) !DeleteAppVersionAppComponentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAppVersionAppComponentInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/delete-app-version-app-component";
    ...
    try aws.json.writeValue(@TypeOf(input.id), input.id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAppVersionAppComponentOutput {
    var result: DeleteAppVersionAppComponentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAppVersionAppComponentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
