...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResiliencyPolicyInput, options: Options) !CreateResiliencyPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResiliencyPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.data_location_constraint) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"dataLocationConstraint\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.policy_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"policyDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"policyName\":");
    try aws.json.writeValue(@TypeOf(input.policy_name), input.policy_name, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResiliencyPolicyOutput {
    var result: CreateResiliencyPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateResiliencyPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
