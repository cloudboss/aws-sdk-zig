...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppVersionInput, options: Options) !UpdateAppVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAppVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAppVersionOutput {
    var result: UpdateAppVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAppVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
