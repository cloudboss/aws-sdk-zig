...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppAssessmentsInput, options: Options) !ListAppAssessmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppAssessmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.assessment_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "assessmentName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.assessment_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "assessmentStatus=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.compliance_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "complianceStatus=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.invoker) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "invoker=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppAssessmentsOutput {
    var result: ListAppAssessmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppAssessmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
