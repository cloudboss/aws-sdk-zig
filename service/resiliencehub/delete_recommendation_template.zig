...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRecommendationTemplateInput, options: Options) !DeleteRecommendationTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRecommendationTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"recommendationTemplateArn\":");
    try aws.json.writeValue(@TypeOf(input.recommendation_template_arn), input.recommendation_template_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRecommendationTemplateOutput {
    var result: DeleteRecommendationTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRecommendationTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
