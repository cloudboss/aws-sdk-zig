...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppVersionTemplateInput, options: Options) !DescribeAppVersionTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAppVersionTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAppVersionTemplateOutput {
    var result: DescribeAppVersionTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAppVersionTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
