...
pub const ListAppVersionResourcesInput = struct {
...
pub const ListAppVersionResourcesOutput = struct {
    /// Token for the next set of results, or null if there are no more results.
    next_token: ?[]const u8 = null,

    /// The physical resources in the application version.
    physical_resources: ?[]const PhysicalResource = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppVersionResourcesInput, options: CallOptions) !ListAppVersionResourcesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAppVersionResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/list-app-version-resources";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAppVersionResourcesOutput {
    var result: ListAppVersionResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppVersionResourcesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
