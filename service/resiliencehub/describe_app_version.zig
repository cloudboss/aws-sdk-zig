...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAppVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAppVersionOutput {
    var result: DescribeAppVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAppVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
