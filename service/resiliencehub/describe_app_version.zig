...
pub const DescribeAppVersionInput = struct {
...
pub const DescribeAppVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppVersionInput, options: CallOptions) !DescribeAppVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAppVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/describe-app-version";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAppVersionOutput {
    var result: DescribeAppVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAppVersionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
