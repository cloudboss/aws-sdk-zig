...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSolNetworkPackageInput, options: Options) !CreateSolNetworkPackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSolNetworkPackageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSolNetworkPackageOutput {
    var result: CreateSolNetworkPackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSolNetworkPackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
