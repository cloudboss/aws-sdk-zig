...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSolNetworkPackagesInput, options: Options) !ListSolNetworkPackagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSolNetworkPackagesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSolNetworkPackagesOutput {
    var result: ListSolNetworkPackagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSolNetworkPackagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
