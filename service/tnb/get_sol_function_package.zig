...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSolFunctionPackageInput, options: Options) !GetSolFunctionPackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSolFunctionPackageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSolFunctionPackageOutput {
    var result: GetSolFunctionPackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSolFunctionPackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
