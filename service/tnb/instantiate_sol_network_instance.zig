...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InstantiateSolNetworkInstanceInput, options: Options) !InstantiateSolNetworkInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InstantiateSolNetworkInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/instantiate");
    ...
    if (input.dry_run) |v| {
        ...
        try query_buf.appendSlice(alloc, "dry_run=");
    ...
    if (input.additional_params_for_ns) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"additionalParamsForNs\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InstantiateSolNetworkInstanceOutput {
    var result: InstantiateSolNetworkInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(InstantiateSolNetworkInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
