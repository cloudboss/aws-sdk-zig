...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ValidateSolFunctionPackageContentInput, options: CallOptions) !ValidateSolFunctionPackageContentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ValidateSolFunctionPackageContentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/package_content/validate");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ValidateSolFunctionPackageContentOutput {
    var result: ValidateSolFunctionPackageContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ValidateSolFunctionPackageContentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
