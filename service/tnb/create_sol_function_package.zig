...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSolFunctionPackageInput, options: Options) !CreateSolFunctionPackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSolFunctionPackageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSolFunctionPackageOutput {
    var result: CreateSolFunctionPackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSolFunctionPackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
