...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSolNetworkInstanceInput, options: Options) !UpdateSolNetworkInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSolNetworkInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.modify_vnf_info_data) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"modifyVnfInfoData\":");
    ...
    if (input.update_ns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"updateNs\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"updateType\":");
    try aws.json.writeValue(@TypeOf(input.update_type), input.update_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSolNetworkInstanceOutput {
    var result: UpdateSolNetworkInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSolNetworkInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
