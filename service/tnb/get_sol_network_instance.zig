...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSolNetworkInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sol/nslcm/v1/ns_instances/");
    try path_buf.appendSlice(alloc, input.ns_instance_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSolNetworkInstanceOutput {
    var result: GetSolNetworkInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSolNetworkInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
