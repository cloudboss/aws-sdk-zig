...
pub const GetSolNetworkPackageContentInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSolNetworkPackageContentInput, options: Options) !GetSolNetworkPackageContentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSolNetworkPackageContentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/nsd_content");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSolNetworkPackageContentOutput {
    var result: GetSolNetworkPackageContentOutput = .{};
    if (body.len > 0) {
        result.nsd_content = try alloc.dupe(u8, body);
    }
    _ = status;
