...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSolNetworkPackageContentInput, options: Options) !PutSolNetworkPackageContentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutSolNetworkPackageContentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutSolNetworkPackageContentOutput {
    var result: PutSolNetworkPackageContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutSolNetworkPackageContentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
