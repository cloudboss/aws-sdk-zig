...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSolFunctionPackageContentInput, options: Options) !PutSolFunctionPackageContentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutSolFunctionPackageContentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutSolFunctionPackageContentOutput {
    var result: PutSolFunctionPackageContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutSolFunctionPackageContentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
