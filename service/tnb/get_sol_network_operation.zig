...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSolNetworkOperationInput, options: Options) !GetSolNetworkOperationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSolNetworkOperationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSolNetworkOperationOutput {
    var result: GetSolNetworkOperationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSolNetworkOperationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
