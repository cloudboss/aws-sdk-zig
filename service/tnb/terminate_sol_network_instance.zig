...
fn serializeRequest(alloc: std.mem.Allocator, input: TerminateSolNetworkInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/terminate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TerminateSolNetworkInstanceOutput {
    var result: TerminateSolNetworkInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TerminateSolNetworkInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
