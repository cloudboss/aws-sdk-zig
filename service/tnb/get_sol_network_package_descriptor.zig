...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSolNetworkPackageDescriptorInput, options: Options) !GetSolNetworkPackageDescriptorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSolNetworkPackageDescriptorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/nsd");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSolNetworkPackageDescriptorOutput {
    var result: GetSolNetworkPackageDescriptorOutput = .{};
    if (body.len > 0) {
        result.nsd = try alloc.dupe(u8, body);
    }
    _ = status;
