...
fn serializeRequest(alloc: std.mem.Allocator, input: DecreaseReplicaCountInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DecreaseReplicaCount&Version=2015-02-02");
    ...
    if (input.new_replica_count) |v| {
        try body_buf.appendSlice(alloc, "&NewReplicaCount=");
    ...
    if (input.replica_configuration) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
                ...
                try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{item.new_replica_count}) catch "");
            }
            {
                var prefix_buf: [256]u8 = undefined;
                const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ReplicaConfiguration.ConfigureShard.{d}.NodeGroupId=", .{n}) catch continue;
                try body_buf.appendSlice(alloc, field_prefix);
                try aws.url.appendUrlEncoded(alloc, &body_buf, item.node_group_id);
            }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DecreaseReplicaCountOutput {
    _ = status;
    _ = headers;
