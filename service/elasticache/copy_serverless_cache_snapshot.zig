...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyServerlessCacheSnapshotInput, options: Options) !CopyServerlessCacheSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyServerlessCacheSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CopyServerlessCacheSnapshot&Version=2015-02-02");
    if (input.kms_key_id) |v| {
        try body_buf.appendSlice(alloc, "&KmsKeyId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&SourceServerlessCacheSnapshotName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_serverless_cache_snapshot_name);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    try body_buf.appendSlice(alloc, "&TargetServerlessCacheSnapshotName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_serverless_cache_snapshot_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyServerlessCacheSnapshotOutput {
    _ = status;
    _ = headers;
