...
pub const DescribeCacheParameterGroupsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCacheParameterGroupsInput, options: Options) !DescribeCacheParameterGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCacheParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCacheParameterGroups&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCacheParameterGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeCacheParameterGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeCacheParameterGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheParameterGroups")) {
                    result.cache_parameter_groups = try serde.deserializeCacheParameterGroupList(&reader, alloc, "CacheParameterGroup");
