...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeServiceUpdatesInput, options: Options) !DescribeServiceUpdatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeServiceUpdatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeServiceUpdates&Version=2015-02-02");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeServiceUpdatesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.service_updates = try serde.deserializeServiceUpdateList(&reader, alloc, "ServiceUpdate");
                } else {
