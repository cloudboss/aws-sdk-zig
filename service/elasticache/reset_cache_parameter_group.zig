...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetCacheParameterGroupInput, options: Options) !ResetCacheParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetCacheParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ResetCacheParameterGroup&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetCacheParameterGroupOutput {
    _ = status;
    _ = headers;
