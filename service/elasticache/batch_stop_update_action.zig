...
pub const BatchStopUpdateActionInput = struct {
...
pub const BatchStopUpdateActionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchStopUpdateActionInput, options: Options) !BatchStopUpdateActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchStopUpdateActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=BatchStopUpdateAction&Version=2015-02-02");
    ...
    try body_buf.appendSlice(alloc, "&ServiceUpdateName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.service_update_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchStopUpdateActionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "BatchStopUpdateActionResult")) break;
            },
            else => {},
        }
    }

    var result: BatchStopUpdateActionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ProcessedUpdateActions")) {
                    result.processed_update_actions = try serde.deserializeProcessedUpdateActionList(&reader, alloc, "ProcessedUpdateAction");
                } else if (std.mem.eql(u8, e.local, "UnprocessedUpdateActions")) {
                    result.unprocessed_update_actions = try serde.deserializeUnprocessedUpdateActionList(&reader, alloc, "UnprocessedUpdateAction");
                } else {
