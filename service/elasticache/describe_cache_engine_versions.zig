...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCacheEngineVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCacheEngineVersions&Version=2015-02-02");
    ...
    if (input.default_only) |v| {
        try body_buf.appendSlice(alloc, "&DefaultOnly=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCacheEngineVersionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheEngineVersions")) {
                    result.cache_engine_versions = try serde.deserializeCacheEngineVersionList(&reader, alloc, "CacheEngineVersion");
