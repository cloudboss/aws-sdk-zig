...
fn serializeRequest(alloc: std.mem.Allocator, input: CopySnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CopySnapshot&Version=2015-02-02");
    if (input.kms_key_id) |v| {
        try body_buf.appendSlice(alloc, "&KmsKeyId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&SourceSnapshotName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_snapshot_name);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    if (input.target_bucket) |v| {
        try body_buf.appendSlice(alloc, "&TargetBucket=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&TargetSnapshotName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_snapshot_name);
