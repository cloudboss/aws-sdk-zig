...
pub const DeleteGlobalReplicationGroupInput = struct {
...
pub const DeleteGlobalReplicationGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGlobalReplicationGroupInput, options: Options) !DeleteGlobalReplicationGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteGlobalReplicationGroup&Version=2015-02-02");
    ...
    try body_buf.appendSlice(alloc, "&RetainPrimaryReplicationGroup=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, if (input.retain_primary_replication_group) "true" else "false");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteGlobalReplicationGroupResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteGlobalReplicationGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
