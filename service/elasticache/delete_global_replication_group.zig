...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGlobalReplicationGroupInput, options: CallOptions) !DeleteGlobalReplicationGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteGlobalReplicationGroup&Version=2015-02-02");
    ...
    try body_buf.appendSlice(allocator, "&RetainPrimaryReplicationGroup=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, if (input.retain_primary_replication_group) "true" else "false");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
