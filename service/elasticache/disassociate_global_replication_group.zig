...
pub const DisassociateGlobalReplicationGroupInput = struct {
...
pub const DisassociateGlobalReplicationGroupOutput = struct {
    global_replication_group: ?GlobalReplicationGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateGlobalReplicationGroupInput, options: CallOptions) !DisassociateGlobalReplicationGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DisassociateGlobalReplicationGroup&Version=2015-02-02");
    ...
    try body_buf.appendSlice(allocator, "&ReplicationGroupRegion=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.replication_group_region);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DisassociateGlobalReplicationGroupResult")) break;
            },
            else => {},
        }
    }

    var result: DisassociateGlobalReplicationGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
