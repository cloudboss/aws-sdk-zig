...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisassociateGlobalReplicationGroup&Version=2015-02-02");
    ...
    try body_buf.appendSlice(alloc, "&ReplicationGroupRegion=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.replication_group_region);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
