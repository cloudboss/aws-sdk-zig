...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeSnapshots&Version=2015-02-02");
    ...
    if (input.show_node_group_config) |v| {
        try body_buf.appendSlice(alloc, "&ShowNodeGroupConfig=");
    ...
    if (input.snapshot_source) |v| {
        try body_buf.appendSlice(alloc, "&SnapshotSource=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
