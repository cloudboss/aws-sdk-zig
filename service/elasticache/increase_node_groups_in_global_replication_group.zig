...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: IncreaseNodeGroupsInGlobalReplicationGroupInput, options: Options) !IncreaseNodeGroupsInGlobalReplicationGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: IncreaseNodeGroupsInGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=IncreaseNodeGroupsInGlobalReplicationGroup&Version=2015-02-02");
    ...
    if (input.regional_configurations) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
                ...
                try aws.url.appendUrlEncoded(alloc, &body_buf, item.replication_group_id);
            }
            {
                var prefix_buf: [256]u8 = undefined;
                ...
                try body_buf.appendSlice(alloc, field_prefix);
                try aws.url.appendUrlEncoded(alloc, &body_buf, item.replication_group_region);
            }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !IncreaseNodeGroupsInGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
