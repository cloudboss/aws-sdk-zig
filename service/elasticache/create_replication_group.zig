...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReplicationGroupInput, options: Options) !CreateReplicationGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateReplicationGroup&Version=2015-02-02");
    if (input.at_rest_encryption_enabled) |v| {
        try body_buf.appendSlice(alloc, "&AtRestEncryptionEnabled=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.auth_token) |v| {
        try body_buf.appendSlice(alloc, "&AuthToken=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.automatic_failover_enabled) |v| {
        try body_buf.appendSlice(alloc, "&AutomaticFailoverEnabled=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    ...
    if (input.cache_node_type) |v| {
        try body_buf.appendSlice(alloc, "&CacheNodeType=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.cache_parameter_group_name) |v| {
        try body_buf.appendSlice(alloc, "&CacheParameterGroupName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    ...
    if (input.cache_subnet_group_name) |v| {
        try body_buf.appendSlice(alloc, "&CacheSubnetGroupName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.cluster_mode) |v| {
        try body_buf.appendSlice(alloc, "&ClusterMode=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
    if (input.data_tiering_enabled) |v| {
        try body_buf.appendSlice(alloc, "&DataTieringEnabled=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.engine) |v| {
    ...
    if (input.global_replication_group_id) |v| {
        try body_buf.appendSlice(alloc, "&GlobalReplicationGroupId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.ip_discovery) |v| {
        try body_buf.appendSlice(alloc, "&IpDiscovery=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
    ...
    if (input.multi_az_enabled) |v| {
        try body_buf.appendSlice(alloc, "&MultiAZEnabled=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    ...
    if (input.notification_topic_arn) |v| {
        try body_buf.appendSlice(alloc, "&NotificationTopicArn=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.num_cache_clusters) |v| {
        try body_buf.appendSlice(alloc, "&NumCacheClusters=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.num_node_groups) |v| {
        try body_buf.appendSlice(alloc, "&NumNodeGroups=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.port) |v| {
    ...
    if (input.primary_cluster_id) |v| {
        try body_buf.appendSlice(alloc, "&PrimaryClusterId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.replicas_per_node_group) |v| {
        try body_buf.appendSlice(alloc, "&ReplicasPerNodeGroup=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    try body_buf.appendSlice(alloc, "&ReplicationGroupDescription=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.replication_group_description);
    ...
    if (input.serverless_cache_snapshot_name) |v| {
        try body_buf.appendSlice(alloc, "&ServerlessCacheSnapshotName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    ...
    if (input.snapshot_name) |v| {
        try body_buf.appendSlice(alloc, "&SnapshotName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    ...
    if (input.snapshot_window) |v| {
        try body_buf.appendSlice(alloc, "&SnapshotWindow=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
    ...
    if (input.transit_encryption_enabled) |v| {
        try body_buf.appendSlice(alloc, "&TransitEncryptionEnabled=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.transit_encryption_mode) |v| {
        try body_buf.appendSlice(alloc, "&TransitEncryptionMode=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReplicationGroupOutput {
    _ = status;
    _ = headers;
