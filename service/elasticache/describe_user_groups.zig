...
pub const DescribeUserGroupsInput = struct {
...
pub const DescribeUserGroupsOutput = struct {
    /// An optional marker returned from a prior request. Use this marker for
    /// pagination of
    /// results from this operation. If this parameter is specified, the response
    /// includes only
    ...
    user_groups: ?[]const UserGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUserGroupsInput, options: CallOptions) !DescribeUserGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeUserGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeUserGroups&Version=2015-02-02");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeUserGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeUserGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeUserGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.user_groups = try serde.deserializeUserGroupList(allocator, &reader, "member");
                } else {
