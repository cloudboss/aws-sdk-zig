...
pub const AuthorizeCacheSecurityGroupIngressInput = struct {
...
pub const AuthorizeCacheSecurityGroupIngressOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AuthorizeCacheSecurityGroupIngressInput, options: Options) !AuthorizeCacheSecurityGroupIngressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AuthorizeCacheSecurityGroupIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AuthorizeCacheSecurityGroupIngress&Version=2015-02-02");
    try body_buf.appendSlice(alloc, "&CacheSecurityGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cache_security_group_name);
    try body_buf.appendSlice(alloc, "&EC2SecurityGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.ec2_security_group_name);
    try body_buf.appendSlice(alloc, "&EC2SecurityGroupOwnerId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.ec2_security_group_owner_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AuthorizeCacheSecurityGroupIngressOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AuthorizeCacheSecurityGroupIngressResult")) break;
            },
            else => {},
        }
    }

    var result: AuthorizeCacheSecurityGroupIngressOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheSecurityGroup")) {
                    result.cache_security_group = try serde.deserializeCacheSecurityGroup(&reader, alloc);
                } else {
