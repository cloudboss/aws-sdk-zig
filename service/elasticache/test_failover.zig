...
pub const TestFailoverInput = struct {
    /// The name of the node group (called shard in the console) in this replication
    /// group on
    /// which automatic failover is to be tested. You may test automatic failover on
    /// up to 15
...
pub const TestFailoverOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestFailoverInput, options: Options) !TestFailoverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestFailoverInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=TestFailover&Version=2015-02-02");
    try body_buf.appendSlice(alloc, "&NodeGroupId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.node_group_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestFailoverOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TestFailoverResult")) break;
            },
            else => {},
        }
    }

    var result: TestFailoverOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
