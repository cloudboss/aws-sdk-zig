...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGlobalReplicationGroupInput, options: Options) !CreateGlobalReplicationGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateGlobalReplicationGroup&Version=2015-02-02");
    if (input.global_replication_group_description) |v| {
        try body_buf.appendSlice(alloc, "&GlobalReplicationGroupDescription=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&GlobalReplicationGroupIdSuffix=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.global_replication_group_id_suffix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
