...
pub const CacheClusterDeletedWaiter = struct {
    client: *Client,
    ...
    fn poll(self: *Self) aws.waiter.AcceptorState {
        ...
        _ = self.client.describeCacheClusters(arena.allocator(), self.params, .{ .diagnostic = &diagnostic }) catch |err| {
            if (err == error.ServiceError) {
                defer diagnostic.deinit();
                if (std.mem.eql(u8, diagnostic.code(), "CacheClusterNotFound")) {
                    return .success;
                }
            }
            return .retry;
...
pub const ReplicationGroupDeletedWaiter = struct {
    client: *Client,
    ...
    fn poll(self: *Self) aws.waiter.AcceptorState {
        ...
        var diagnostic: @import("errors.zig").ServiceError = undefined;
        _ = self.client.describeReplicationGroups(arena.allocator(), self.params, .{ .diagnostic = &diagnostic }) catch |err| {
            if (err == error.ServiceError) {
                defer diagnostic.deinit();
                if (std.mem.eql(u8, diagnostic.code(), "ReplicationGroupNotFoundFault")) {
                    return .success;
                }
            }
            return .retry;
