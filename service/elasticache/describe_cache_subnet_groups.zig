...
pub const DescribeCacheSubnetGroupsInput = struct {
...
pub const DescribeCacheSubnetGroupsOutput = struct {
    /// A list of cache subnet groups. Each element in the list contains detailed
    /// information
    /// about one group.
    cache_subnet_groups: ?[]const CacheSubnetGroup = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCacheSubnetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCacheSubnetGroups&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCacheSubnetGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeCacheSubnetGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeCacheSubnetGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheSubnetGroups")) {
                    result.cache_subnet_groups = try serde.deserializeCacheSubnetGroups(&reader, alloc, "CacheSubnetGroup");
