...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeCacheSubnetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeCacheSubnetGroups&Version=2015-02-02");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeCacheSubnetGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheSubnetGroups")) {
                    result.cache_subnet_groups = try serde.deserializeCacheSubnetGroups(allocator, &reader, "CacheSubnetGroup");
