...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteReplicationGroupInput, options: Options) !DeleteReplicationGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteReplicationGroup&Version=2015-02-02");
    if (input.final_snapshot_identifier) |v| {
        try body_buf.appendSlice(alloc, "&FinalSnapshotIdentifier=");
    ...
    if (input.retain_primary_cluster) |v| {
        try body_buf.appendSlice(alloc, "&RetainPrimaryCluster=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteReplicationGroupOutput {
    _ = status;
    _ = headers;
