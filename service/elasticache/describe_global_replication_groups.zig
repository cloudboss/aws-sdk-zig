...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGlobalReplicationGroupsInput, options: Options) !DescribeGlobalReplicationGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGlobalReplicationGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeGlobalReplicationGroups&Version=2015-02-02");
    ...
    if (input.show_member_info) |v| {
        try body_buf.appendSlice(alloc, "&ShowMemberInfo=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGlobalReplicationGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GlobalReplicationGroups")) {
                    result.global_replication_groups = try serde.deserializeGlobalReplicationGroupList(&reader, alloc, "GlobalReplicationGroup");
