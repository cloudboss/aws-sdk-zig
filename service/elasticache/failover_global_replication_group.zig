...
pub const FailoverGlobalReplicationGroupInput = struct {
...
pub const FailoverGlobalReplicationGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: FailoverGlobalReplicationGroupInput, options: Options) !FailoverGlobalReplicationGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: FailoverGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=FailoverGlobalReplicationGroup&Version=2015-02-02");
    try body_buf.appendSlice(alloc, "&GlobalReplicationGroupId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.global_replication_group_id);
    try body_buf.appendSlice(alloc, "&PrimaryRegion=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.primary_region);
    try body_buf.appendSlice(alloc, "&PrimaryReplicationGroupId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.primary_replication_group_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !FailoverGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "FailoverGlobalReplicationGroupResult")) break;
            },
            else => {},
        }
    }

    var result: FailoverGlobalReplicationGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GlobalReplicationGroup")) {
                    result.global_replication_group = try serde.deserializeGlobalReplicationGroup(&reader, alloc);
                } else {
