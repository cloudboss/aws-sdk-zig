...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: FailoverGlobalReplicationGroupInput, options: CallOptions) !FailoverGlobalReplicationGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: FailoverGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=FailoverGlobalReplicationGroup&Version=2015-02-02");
    ...
    try body_buf.appendSlice(allocator, "&PrimaryRegion=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.primary_region);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !FailoverGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
