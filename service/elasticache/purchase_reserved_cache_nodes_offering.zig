...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurchaseReservedCacheNodesOfferingInput, options: Options) !PurchaseReservedCacheNodesOfferingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseReservedCacheNodesOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PurchaseReservedCacheNodesOffering&Version=2015-02-02");
    if (input.cache_node_count) |v| {
        try body_buf.appendSlice(alloc, "&CacheNodeCount=");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.reserved_cache_nodes_offering_id);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PurchaseReservedCacheNodesOfferingOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ReservedCacheNode")) {
                    result.reserved_cache_node = try serde.deserializeReservedCacheNode(&reader, alloc);
                } else {
