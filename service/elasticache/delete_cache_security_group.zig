...
pub const DeleteCacheSecurityGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCacheSecurityGroupInput, options: Options) !DeleteCacheSecurityGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCacheSecurityGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteCacheSecurityGroup&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCacheSecurityGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: DeleteCacheSecurityGroupOutput = .{};
