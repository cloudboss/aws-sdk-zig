...
pub const CreateCacheSubnetGroupInput = struct {
...
pub const CreateCacheSubnetGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCacheSubnetGroupInput, options: Options) !CreateCacheSubnetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCacheSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateCacheSubnetGroup&Version=2015-02-02");
    try body_buf.appendSlice(alloc, "&CacheSubnetGroupDescription=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cache_subnet_group_description);
    try body_buf.appendSlice(alloc, "&CacheSubnetGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cache_subnet_group_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCacheSubnetGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateCacheSubnetGroupResult")) break;
            },
            else => {},
        }
    }

    var result: CreateCacheSubnetGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheSubnetGroup")) {
                    result.cache_subnet_group = try serde.deserializeCacheSubnetGroup(&reader, alloc);
                } else {
