...
pub const CreateUserGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUserGroupInput, options: Options) !CreateUserGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUserGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateUserGroup&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUserGroupOutput {
    _ = status;
    _ = headers;
