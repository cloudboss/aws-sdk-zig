...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationGroupsInput, options: Options) !DescribeReplicationGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReplicationGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeReplicationGroups&Version=2015-02-02");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReplicationGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.replication_groups = try serde.deserializeReplicationGroupList(&reader, alloc, "ReplicationGroup");
                } else {
