...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteServerlessCacheInput, options: Options) !DeleteServerlessCacheOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteServerlessCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteServerlessCache&Version=2015-02-02");
    if (input.final_snapshot_name) |v| {
        try body_buf.appendSlice(alloc, "&FinalSnapshotName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteServerlessCacheOutput {
    _ = status;
    _ = headers;
