...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUpdateActionsInput, options: Options) !DescribeUpdateActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUpdateActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeUpdateActions&Version=2015-02-02");
    ...
    if (input.service_update_time_range) |v| {
        if (v.end_time) |sv| {
            try body_buf.appendSlice(alloc, "&ServiceUpdateTimeRange.EndTime=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{sv}) catch "");
        }
        if (v.start_time) |sv| {
            try body_buf.appendSlice(alloc, "&ServiceUpdateTimeRange.StartTime=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{sv}) catch "");
        }
    }
    if (input.show_node_level_update_status) |v| {
        try body_buf.appendSlice(alloc, "&ShowNodeLevelUpdateStatus=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUpdateActionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.update_actions = try serde.deserializeUpdateActionList(&reader, alloc, "UpdateAction");
                } else {
