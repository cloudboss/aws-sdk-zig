...
pub const DescribeCacheSecurityGroupsInput = struct {
...
pub const DescribeCacheSecurityGroupsOutput = struct {
    /// A list of cache security groups. Each element in the list contains detailed
    /// information about one group.
    cache_security_groups: ?[]const CacheSecurityGroup = null,

    /// Provides an identifier to allow retrieval of paginated results.
    marker: ?[]const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCacheSecurityGroupsInput, options: CallOptions) !DescribeCacheSecurityGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeCacheSecurityGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeCacheSecurityGroups&Version=2015-02-02");
    if (input.cache_security_group_name) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeCacheSecurityGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeCacheSecurityGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeCacheSecurityGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheSecurityGroups")) {
                    result.cache_security_groups = try serde.deserializeCacheSecurityGroups(allocator, &reader, "CacheSecurityGroup");
