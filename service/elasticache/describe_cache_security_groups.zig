...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCacheSecurityGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCacheSecurityGroups&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCacheSecurityGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheSecurityGroups")) {
                    result.cache_security_groups = try serde.deserializeCacheSecurityGroups(&reader, alloc, "CacheSecurityGroup");
