...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCacheClustersInput, options: Options) !DescribeCacheClustersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCacheClustersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCacheClusters&Version=2015-02-02");
    ...
    if (input.show_cache_clusters_not_in_replication_groups) |v| {
        try body_buf.appendSlice(alloc, "&ShowCacheClustersNotInReplicationGroups=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.show_cache_node_info) |v| {
        try body_buf.appendSlice(alloc, "&ShowCacheNodeInfo=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCacheClustersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheClusters")) {
                    result.cache_clusters = try serde.deserializeCacheClusterList(&reader, alloc, "CacheCluster");
