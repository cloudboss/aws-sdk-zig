...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEngineDefaultParametersInput, options: Options) !DescribeEngineDefaultParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEngineDefaultParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEngineDefaultParameters&Version=2015-02-02");
    try body_buf.appendSlice(alloc, "&CacheParameterGroupFamily=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cache_parameter_group_family);
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEngineDefaultParametersOutput {
    _ = status;
    _ = headers;
