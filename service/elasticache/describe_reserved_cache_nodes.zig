...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReservedCacheNodesInput, options: Options) !DescribeReservedCacheNodesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedCacheNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeReservedCacheNodes&Version=2015-02-02");
    ...
    if (input.reserved_cache_node_id) |v| {
        try body_buf.appendSlice(alloc, "&ReservedCacheNodeId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.reserved_cache_nodes_offering_id) |v| {
        try body_buf.appendSlice(alloc, "&ReservedCacheNodesOfferingId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReservedCacheNodesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.reserved_cache_nodes = try serde.deserializeReservedCacheNodeList(&reader, alloc, "ReservedCacheNode");
                } else {
