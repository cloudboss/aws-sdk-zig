...
pub const DecreaseNodeGroupsInGlobalReplicationGroupInput = struct {
    ...
    /// GlobalNodeGroupsToRemove is a list of NodeGroupIds to remove from the
    /// cluster.
    /// ElastiCache will attempt to remove all node groups listed by
    /// GlobalNodeGroupsToRemove from the cluster.
    global_node_groups_to_remove: ?[]const []const u8 = null,

    /// If the value of NodeGroupCount is less than the current number of node
    /// groups
    /// (shards), then either NodeGroupsToRemove or NodeGroupsToRetain is required.
    /// GlobalNodeGroupsToRetain is a list of NodeGroupIds to retain from the
    /// cluster.
    /// ElastiCache will attempt to retain all node groups listed by
    /// GlobalNodeGroupsToRetain from the cluster.
    global_node_groups_to_retain: ?[]const []const u8 = null,
...
pub const DecreaseNodeGroupsInGlobalReplicationGroupOutput = struct {
    global_replication_group: ?GlobalReplicationGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DecreaseNodeGroupsInGlobalReplicationGroupInput, options: CallOptions) !DecreaseNodeGroupsInGlobalReplicationGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DecreaseNodeGroupsInGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DecreaseNodeGroupsInGlobalReplicationGroup&Version=2015-02-02");
    ...
    if (input.global_node_groups_to_remove) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&GlobalNodeGroupsToRemove.GlobalNodeGroupId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item);
        }
    }
    if (input.global_node_groups_to_retain) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&GlobalNodeGroupsToRetain.GlobalNodeGroupId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DecreaseNodeGroupsInGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DecreaseNodeGroupsInGlobalReplicationGroupResult")) break;
            },
            else => {},
        }
    }

    var result: DecreaseNodeGroupsInGlobalReplicationGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
