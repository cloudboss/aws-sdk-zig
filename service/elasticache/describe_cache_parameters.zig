...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCacheParametersInput, options: Options) !DescribeCacheParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCacheParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCacheParameters&Version=2015-02-02");
    try body_buf.appendSlice(alloc, "&CacheParameterGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cache_parameter_group_name);
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCacheParametersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheNodeTypeSpecificParameters")) {
                    result.cache_node_type_specific_parameters = try serde.deserializeCacheNodeTypeSpecificParametersList(&reader, alloc, "CacheNodeTypeSpecificParameter");
