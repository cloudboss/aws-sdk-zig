...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeServerlessCachesInput, options: Options) !DescribeServerlessCachesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeServerlessCachesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeServerlessCaches&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeServerlessCachesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.serverless_caches = try serde.deserializeServerlessCacheList(&reader, alloc, "member");
                } else {
