...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAllowedNodeTypeModificationsInput, options: Options) !ListAllowedNodeTypeModificationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAllowedNodeTypeModificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListAllowedNodeTypeModifications&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAllowedNodeTypeModificationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ScaleDownModifications")) {
                    result.scale_down_modifications = try serde.deserializeNodeTypeList(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "ScaleUpModifications")) {
                    result.scale_up_modifications = try serde.deserializeNodeTypeList(&reader, alloc, "member");
                } else {
