...
pub const DeleteUserGroupInput = struct {
    /// The ID of the user group.
    user_group_id: []const u8,
};

const DeleteUserGroupOutput = @import("user_group.zig").DeleteUserGroupOutput;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUserGroupInput, options: Options) !DeleteUserGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteUserGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteUserGroup&Version=2015-02-02");
    try body_buf.appendSlice(alloc, "&UserGroupId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.user_group_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteUserGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteUserGroupResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteUserGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ARN")) {
                    ...
                    result.pending_changes = try serde.deserializeUserGroupPendingChanges(&reader, alloc);
                } else if (std.mem.eql(u8, e.local, "ReplicationGroups")) {
                    result.replication_groups = try serde.deserializeUGReplicationGroupIdList(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "ServerlessCaches")) {
                    result.serverless_caches = try serde.deserializeUGServerlessCacheIdList(&reader, alloc, "member");
                    ...
                    result.user_ids = try serde.deserializeUserIdList(&reader, alloc, "member");
                } else {
