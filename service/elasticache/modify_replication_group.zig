...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyReplicationGroupInput, options: Options) !ModifyReplicationGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyReplicationGroup&Version=2015-02-02");
    ...
    if (input.remove_user_groups) |v| {
        try body_buf.appendSlice(alloc, "&RemoveUserGroups=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    ...
    if (input.snapshotting_cluster_id) |v| {
        try body_buf.appendSlice(alloc, "&SnapshottingClusterId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyReplicationGroupOutput {
    _ = status;
    _ = headers;
