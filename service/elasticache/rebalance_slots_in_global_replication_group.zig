...
pub const RebalanceSlotsInGlobalReplicationGroupInput = struct {
...
pub const RebalanceSlotsInGlobalReplicationGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebalanceSlotsInGlobalReplicationGroupInput, options: Options) !RebalanceSlotsInGlobalReplicationGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RebalanceSlotsInGlobalReplicationGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RebalanceSlotsInGlobalReplicationGroup&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RebalanceSlotsInGlobalReplicationGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RebalanceSlotsInGlobalReplicationGroupResult")) break;
            },
            else => {},
        }
    }

    var result: RebalanceSlotsInGlobalReplicationGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
