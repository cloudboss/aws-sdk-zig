...
pub const CreateCacheSecurityGroupInput = struct {
...
pub const CreateCacheSecurityGroupOutput = struct {
    cache_security_group: ?CacheSecurityGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCacheSecurityGroupInput, options: CallOptions) !CreateCacheSecurityGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateCacheSecurityGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateCacheSecurityGroup&Version=2015-02-02");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateCacheSecurityGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateCacheSecurityGroupResult")) break;
            },
            else => {},
        }
    }

    var result: CreateCacheSecurityGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
