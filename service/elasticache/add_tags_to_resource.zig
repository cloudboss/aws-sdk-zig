...
fn serializeRequest(alloc: std.mem.Allocator, input: AddTagsToResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AddTagsToResource&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddTagsToResourceOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AddTagsToResourceResult")) break;
            },
            else => {},
        }
    }

    var result: AddTagsToResourceOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
