...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedCacheNodesOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeReservedCacheNodesOfferings&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReservedCacheNodesOfferingsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.reserved_cache_nodes_offerings = try serde.deserializeReservedCacheNodesOfferingList(&reader, alloc, "ReservedCacheNodesOffering");
                } else {
