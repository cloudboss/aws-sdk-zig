...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCacheParameterGroupInput, options: Options) !CreateCacheParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCacheParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateCacheParameterGroup&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCacheParameterGroupOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CacheParameterGroup")) {
                    result.cache_parameter_group = try serde.deserializeCacheParameterGroup(&reader, alloc);
                } else {
