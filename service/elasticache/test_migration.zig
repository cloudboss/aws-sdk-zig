...
fn serializeRequest(alloc: std.mem.Allocator, input: TestMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=TestMigration&Version=2015-02-02");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestMigrationOutput {
    _ = status;
    _ = headers;
