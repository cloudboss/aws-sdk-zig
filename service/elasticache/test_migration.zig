...
pub const TestMigrationInput = struct {
    /// List of endpoints from which data should be migrated. List should have only
    /// one
    /// element.
    customer_node_endpoint_list: []const CustomerNodeEndpoint,

    /// The ID of the replication group to which data is to be migrated.
    replication_group_id: []const u8,
};

pub const TestMigrationOutput = struct {
    replication_group: ?ReplicationGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestMigrationInput, options: CallOptions) !TestMigrationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TestMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=TestMigration&Version=2015-02-02");
    for (input.customer_node_endpoint_list, 0..) |item, idx| {
        const n = idx + 1;
        {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TestMigrationOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TestMigrationResult")) break;
            },
            else => {},
        }
    }

    var result: TestMigrationOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
