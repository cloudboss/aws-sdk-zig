...
pub const ModifyUserGroupOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyUserGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyUserGroup&Version=2015-02-02");
    if (input.engine) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyUserGroupOutput {
    _ = status;
    _ = headers;
