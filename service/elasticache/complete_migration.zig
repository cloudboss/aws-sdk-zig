...
pub const CompleteMigrationInput = struct {
    /// Forces the migration to stop without ensuring that data is in sync. It is
    /// recommended
    /// to use this option only to abort the migration and not recommended when
    /// application
...
pub const CompleteMigrationOutput = struct {
    replication_group: ?ReplicationGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CompleteMigrationInput, options: CallOptions) !CompleteMigrationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CompleteMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CompleteMigration&Version=2015-02-02");
    if (input.force) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CompleteMigrationOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CompleteMigrationResult")) break;
            },
            else => {},
        }
    }

    var result: CompleteMigrationOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
