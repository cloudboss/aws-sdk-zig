...
fn serializeRequest(alloc: std.mem.Allocator, input: CompleteMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CompleteMigration&Version=2015-02-02");
    if (input.force) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CompleteMigrationOutput {
    _ = status;
    _ = headers;
