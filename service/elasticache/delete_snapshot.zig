...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteSnapshot&Version=2015-02-02");
    try body_buf.appendSlice(allocator, "&SnapshotName=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.snapshot_name);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSnapshotOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteSnapshotResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteSnapshotOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
