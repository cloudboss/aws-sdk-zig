...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHomeRegionControlInput, options: Options) !CreateHomeRegionControlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHomeRegionControlInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHubMultiAccountService.CreateHomeRegionControl");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHomeRegionControlOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateHomeRegionControlOutput, body, alloc);
}
