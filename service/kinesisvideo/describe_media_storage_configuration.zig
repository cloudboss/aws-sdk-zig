...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMediaStorageConfigurationInput, options: Options) !DescribeMediaStorageConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMediaStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMediaStorageConfigurationOutput {
    var result: DescribeMediaStorageConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeMediaStorageConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
