...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStreamStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.stream_storage_configuration), input.stream_storage_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStreamStorageConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
