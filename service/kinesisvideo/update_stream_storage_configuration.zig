...
pub const UpdateStreamStorageConfigurationInput = struct {
    /// The version of the stream whose storage configuration you want to change. To
    /// get the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStreamStorageConfigurationInput, options: CallOptions) !UpdateStreamStorageConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateStreamStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/updateStreamStorageConfiguration";
    ...
    try aws.json.writeValue(@TypeOf(input.stream_storage_configuration), input.stream_storage_configuration, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateStreamStorageConfigurationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateStreamStorageConfigurationOutput = .{};
