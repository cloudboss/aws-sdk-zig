...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMediaStorageConfigurationInput, options: Options) !UpdateMediaStorageConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMediaStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MediaStorageConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.media_storage_configuration), input.media_storage_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMediaStorageConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
