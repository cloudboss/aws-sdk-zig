...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataEndpointInput, options: Options) !GetDataEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"APIName\":");
    try aws.json.writeValue(@TypeOf(input.api_name), input.api_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataEndpointOutput {
    var result: GetDataEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
