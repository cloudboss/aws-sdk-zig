...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEdgeAgentConfigurationsInput, options: Options) !ListEdgeAgentConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEdgeAgentConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"HubDeviceArn\":");
    try aws.json.writeValue(@TypeOf(input.hub_device_arn), input.hub_device_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEdgeAgentConfigurationsOutput {
    var result: ListEdgeAgentConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEdgeAgentConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
