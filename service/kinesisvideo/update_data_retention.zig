...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDataRetentionInput, options: Options) !UpdateDataRetentionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataRetentionInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DataRetentionChangeInHours\":");
    try aws.json.writeValue(@TypeOf(input.data_retention_change_in_hours), input.data_retention_change_in_hours, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataRetentionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
