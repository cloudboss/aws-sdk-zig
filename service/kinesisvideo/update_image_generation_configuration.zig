...
pub const UpdateImageGenerationConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateImageGenerationConfigurationInput, options: Options) !UpdateImageGenerationConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateImageGenerationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/updateImageGenerationConfiguration";

    var body_buf: std.ArrayList(u8) = .{};
    var has_prev = false;
    try body_buf.appendSlice(allocator, "{");

    if (input.image_generation_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ImageGenerationConfiguration\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateImageGenerationConfigurationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateImageGenerationConfigurationOutput = .{};
