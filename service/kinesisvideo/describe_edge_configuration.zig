...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEdgeConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEdgeConfigurationOutput {
    var result: DescribeEdgeConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeEdgeConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
