...
pub const DescribeEdgeConfigurationInput = struct {
...
pub const DescribeEdgeConfigurationOutput = struct {
    ...
    edge_agent_status: ?EdgeAgentStatus = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEdgeConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/describeEdgeConfiguration";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEdgeConfigurationOutput {
    var result: DescribeEdgeConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeEdgeConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
