...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSignalingChannelEndpointInput, options: Options) !GetSignalingChannelEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSignalingChannelEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.single_master_channel_endpoint_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SingleMasterChannelEndpointConfiguration\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSignalingChannelEndpointOutput {
    var result: GetSignalingChannelEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSignalingChannelEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
