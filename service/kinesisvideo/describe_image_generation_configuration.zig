...
const ImageGenerationConfiguration = @import("image_generation_configuration.zig").ImageGenerationConfiguration;

pub const DescribeImageGenerationConfigurationInput = struct {
...
pub const DescribeImageGenerationConfigurationOutput = struct {
    ...
    image_generation_configuration: ?ImageGenerationConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageGenerationConfigurationInput, options: CallOptions) !DescribeImageGenerationConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeImageGenerationConfigurationInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("kinesisvideo", "Kinesis Video", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/describeImageGenerationConfiguration";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeImageGenerationConfigurationOutput {
    var result: DescribeImageGenerationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeImageGenerationConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
