...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeImageGenerationConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeImageGenerationConfigurationOutput {
    var result: DescribeImageGenerationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeImageGenerationConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
