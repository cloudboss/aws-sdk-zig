...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNotificationConfigurationInput, options: Options) !DescribeNotificationConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNotificationConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNotificationConfigurationOutput {
    var result: DescribeNotificationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeNotificationConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
