...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSignalingChannelInput, options: Options) !CreateSignalingChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSignalingChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.channel_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ChannelType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.single_master_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SingleMasterConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSignalingChannelOutput {
    var result: CreateSignalingChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSignalingChannelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
