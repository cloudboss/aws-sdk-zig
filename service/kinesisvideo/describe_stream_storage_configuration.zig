...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStreamStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStreamStorageConfigurationOutput {
    var result: DescribeStreamStorageConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeStreamStorageConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
