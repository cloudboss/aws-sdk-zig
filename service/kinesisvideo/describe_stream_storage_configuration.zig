...
pub const DescribeStreamStorageConfigurationInput = struct {
    /// The Amazon Resource Name (ARN) of the stream for which you want to retrieve
...
pub const DescribeStreamStorageConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStreamStorageConfigurationInput, options: CallOptions) !DescribeStreamStorageConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeStreamStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("kinesisvideo", "Kinesis Video", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/describeStreamStorageConfiguration";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeStreamStorageConfigurationOutput {
    var result: DescribeStreamStorageConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeStreamStorageConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
