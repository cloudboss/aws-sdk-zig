...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartEdgeConfigurationUpdateInput, options: Options) !StartEdgeConfigurationUpdateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartEdgeConfigurationUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"EdgeConfig\":");
    try aws.json.writeValue(@TypeOf(input.edge_config), input.edge_config, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartEdgeConfigurationUpdateOutput {
    var result: StartEdgeConfigurationUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartEdgeConfigurationUpdateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
