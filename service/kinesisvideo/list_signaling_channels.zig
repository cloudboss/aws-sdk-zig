...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSignalingChannelsInput, options: Options) !ListSignalingChannelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSignalingChannelsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.channel_name_condition) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ChannelNameCondition\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSignalingChannelsOutput {
    var result: ListSignalingChannelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSignalingChannelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
