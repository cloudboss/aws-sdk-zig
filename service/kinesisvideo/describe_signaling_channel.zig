...
pub const DescribeSignalingChannelInput = struct {
...
pub const DescribeSignalingChannelOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSignalingChannelInput, options: Options) !DescribeSignalingChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSignalingChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/describeSignalingChannel";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSignalingChannelOutput {
    var result: DescribeSignalingChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSignalingChannelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
