...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDbClusterInput, options: Options) !GetDbClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDbClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonTimestreamInfluxDB.GetDbCluster");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDbClusterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDbClusterOutput, body, alloc);
}
