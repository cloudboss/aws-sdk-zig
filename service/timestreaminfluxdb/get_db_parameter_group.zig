...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDbParameterGroupInput, options: Options) !GetDbParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDbParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonTimestreamInfluxDB.GetDbParameterGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDbParameterGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetDbParameterGroupOutput, body, alloc);
}
