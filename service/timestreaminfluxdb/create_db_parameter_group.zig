...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDbParameterGroupInput, options: Options) !CreateDbParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDbParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonTimestreamInfluxDB.CreateDbParameterGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDbParameterGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateDbParameterGroupOutput, body, alloc);
}
