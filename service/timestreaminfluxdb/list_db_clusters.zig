...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDbClustersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonTimestreamInfluxDB.ListDbClusters");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDbClustersOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListDbClustersOutput, body, alloc);
}
