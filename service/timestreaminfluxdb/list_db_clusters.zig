...
const DbClusterSummary = @import("db_cluster_summary.zig").DbClusterSummary;

pub const ListDbClustersInput = struct {
...
pub const ListDbClustersOutput = struct {
    /// A list of Timestream for InfluxDB cluster summaries.
    items: ?[]const DbClusterSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDbClustersInput, options: CallOptions) !ListDbClustersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDbClustersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonTimestreamInfluxDB.ListDbClusters");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDbClustersOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListDbClustersOutput, body, allocator);
}
