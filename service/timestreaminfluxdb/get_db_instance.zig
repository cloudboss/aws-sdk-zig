...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDbInstanceInput, options: Options) !GetDbInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDbInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonTimestreamInfluxDB.GetDbInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDbInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDbInstanceOutput, body, alloc);
}
