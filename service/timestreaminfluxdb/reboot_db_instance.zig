...
fn serializeRequest(alloc: std.mem.Allocator, input: RebootDbInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonTimestreamInfluxDB.RebootDbInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RebootDbInstanceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(RebootDbInstanceOutput, body, alloc);
}
