...
pub const UpdateDbInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDbInstanceInput, options: CallOptions) !UpdateDbInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDbInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonTimestreamInfluxDB.UpdateDbInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDbInstanceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateDbInstanceOutput, body, allocator);
}
