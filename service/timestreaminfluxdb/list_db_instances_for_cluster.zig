...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDbInstancesForClusterInput, options: Options) !ListDbInstancesForClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDbInstancesForClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonTimestreamInfluxDB.ListDbInstancesForCluster");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDbInstancesForClusterOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListDbInstancesForClusterOutput, body, alloc);
}
