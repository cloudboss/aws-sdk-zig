...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDbInstanceInput, options: Options) !DeleteDbInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDbInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonTimestreamInfluxDB.DeleteDbInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDbInstanceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteDbInstanceOutput, body, alloc);
}
