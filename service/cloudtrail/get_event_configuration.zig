...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEventConfigurationInput, options: Options) !GetEventConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEventConfigurationInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("cloudtrail", "CloudTrail", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.GetEventConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEventConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEventConfigurationOutput, body, alloc);
}
