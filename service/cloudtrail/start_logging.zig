...
pub const StartLoggingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartLoggingInput, options: Options) !StartLoggingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartLoggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.StartLogging");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartLoggingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
