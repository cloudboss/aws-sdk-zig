...
fn serializeRequest(allocator: std.mem.Allocator, input: StartLoggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudTrail_20131101.StartLogging");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartLoggingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
