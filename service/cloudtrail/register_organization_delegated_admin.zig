...
pub const RegisterOrganizationDelegatedAdminInput = struct {
    /// An organization member account ID that you want to designate as a delegated
    /// administrator.
    member_account_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterOrganizationDelegatedAdminInput, options: Options) !RegisterOrganizationDelegatedAdminOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterOrganizationDelegatedAdminInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudTrail_20131101.RegisterOrganizationDelegatedAdmin");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterOrganizationDelegatedAdminOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
