...
/// logging of all
/// data events for a general purpose bucket named `amzn-s3-demo-bucket1`. In
...
/// user specified an S3 bucket with an empty prefix, events that occur on any
/// object in
...
pub const DataResource = struct {
    ...
    ///   an
    /// empty object prefix such as `arn:aws:s3:::amzn-s3-demo-bucket1/`. The trail
    /// logs data
    /// events for all objects in this S3 bucket.
    ///
    /// * To log data events for specific objects, specify the S3 bucket and object
    ///   prefix
    /// such as `arn:aws:s3:::amzn-s3-demo-bucket1/example-images`. The trail logs
    /// data events
