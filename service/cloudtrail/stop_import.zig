...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopImportInput, options: Options) !StopImportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopImportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.StopImport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopImportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopImportOutput, body, alloc);
}
