...
pub const StopImportInput = struct {
...
pub const StopImportOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopImportInput, options: CallOptions) !StopImportOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopImportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudTrail_20131101.StopImport");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopImportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopImportOutput, body, allocator);
}
