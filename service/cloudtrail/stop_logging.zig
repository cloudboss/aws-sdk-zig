...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopLoggingInput, options: Options) !StopLoggingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopLoggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.StopLogging");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopLoggingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
