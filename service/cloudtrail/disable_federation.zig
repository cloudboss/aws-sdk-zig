...
pub const DisableFederationInput = struct {
...
pub const DisableFederationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableFederationInput, options: CallOptions) !DisableFederationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableFederationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudTrail_20131101.DisableFederation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableFederationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisableFederationOutput, body, allocator);
}
