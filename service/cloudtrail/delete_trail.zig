...
pub const DeleteTrailInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTrailInput, options: Options) !DeleteTrailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTrailInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.DeleteTrail");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTrailOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
