...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTrailInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudTrail_20131101.DeleteTrail");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTrailOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
