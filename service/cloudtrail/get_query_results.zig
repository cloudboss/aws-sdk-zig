...
pub const GetQueryResultsOutput = struct {
    ...
    query_result_rows: ?[]const []const []const aws.map.StringMapEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQueryResultsInput, options: Options) !GetQueryResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQueryResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.GetQueryResults");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQueryResultsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetQueryResultsOutput, body, alloc);
}
