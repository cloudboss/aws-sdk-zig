...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEventDataStoreInput, options: Options) !UpdateEventDataStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEventDataStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.UpdateEventDataStore");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEventDataStoreOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateEventDataStoreOutput, body, alloc);
}
