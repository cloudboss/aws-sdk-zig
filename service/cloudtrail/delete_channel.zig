...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteChannelInput, options: Options) !DeleteChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.DeleteChannel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteChannelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
