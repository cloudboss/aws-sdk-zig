...
pub const StartEventDataStoreIngestionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartEventDataStoreIngestionInput, options: CallOptions) !StartEventDataStoreIngestionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartEventDataStoreIngestionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudTrail_20131101.StartEventDataStoreIngestion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartEventDataStoreIngestionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
