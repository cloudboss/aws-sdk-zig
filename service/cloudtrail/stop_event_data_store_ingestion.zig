...
pub const StopEventDataStoreIngestionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopEventDataStoreIngestionInput, options: Options) !StopEventDataStoreIngestionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopEventDataStoreIngestionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.StopEventDataStoreIngestion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopEventDataStoreIngestionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
