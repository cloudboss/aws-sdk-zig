...
const SourceConfig = @import("source_config.zig").SourceConfig;
...
pub const GetChannelOutput = struct {
    ...
    /// the channel.
    /// For service-linked channels, the destination is the Amazon Web Services
    ...
    /// created the channel and `custom-suffix` represents the suffix generated by
    /// the
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.GetChannel");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChannelOutput {
    ...
    return aws.json.parseJsonObject(GetChannelOutput, body, alloc);
}
