...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImportFailuresInput, options: Options) !ListImportFailuresOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImportFailuresInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.ListImportFailures");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImportFailuresOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListImportFailuresOutput, body, alloc);
}
