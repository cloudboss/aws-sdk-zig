...
pub const DeleteEventDataStoreInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEventDataStoreInput, options: Options) !DeleteEventDataStoreOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteEventDataStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudTrail_20131101.DeleteEventDataStore");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteEventDataStoreOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
