...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQueriesInput, options: Options) !ListQueriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQueriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.ListQueries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQueriesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListQueriesOutput, body, alloc);
}
