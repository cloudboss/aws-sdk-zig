...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrailInput, options: Options) !CreateTrailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrailInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.CreateTrail");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrailOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTrailOutput, body, alloc);
}
