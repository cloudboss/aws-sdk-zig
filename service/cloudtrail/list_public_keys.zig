...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPublicKeysInput, options: Options) !ListPublicKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPublicKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.ListPublicKeys");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPublicKeysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPublicKeysOutput, body, alloc);
}
