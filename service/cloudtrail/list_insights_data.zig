...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInsightsDataInput, options: Options) !ListInsightsDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInsightsDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudTrail_20131101.ListInsightsData");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInsightsDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInsightsDataOutput, body, alloc);
}
