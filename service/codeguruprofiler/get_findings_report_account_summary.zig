...
pub const GetFindingsReportAccountSummaryInput = struct {
    ...
    /// in paginated output.
    /// When this parameter is used, `GetFindingsReportAccountSummary` only returns
    /// `maxResults`
...
pub const GetFindingsReportAccountSummaryOutput = struct {
    /// The `nextToken` value to include in a future
    /// `GetFindingsReportAccountSummary` request.
    /// When the results of a `GetFindingsReportAccountSummary` request exceed
    /// `maxResults`, this
    ...
    /// in your AWS account.
    report_summaries: ?[]const FindingsReportSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingsReportAccountSummaryInput, options: Options) !GetFindingsReportAccountSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingsReportAccountSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/internal/findingsReports";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.daily_reports_only) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "dailyReportsOnly=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingsReportAccountSummaryOutput {
    var result: GetFindingsReportAccountSummaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingsReportAccountSummaryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
