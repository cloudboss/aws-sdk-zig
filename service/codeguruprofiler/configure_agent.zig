...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfigureAgentInput, options: Options) !ConfigureAgentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ConfigureAgentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/configureAgent");
    ...
    if (input.fleet_instance_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"fleetInstanceId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConfigureAgentOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
