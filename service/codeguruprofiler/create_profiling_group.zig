...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProfilingGroupInput, options: Options) !CreateProfilingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProfilingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.client_token);
    query_has_prev = true;
    ...
    if (input.agent_orchestration_config) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"agentOrchestrationConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.compute_platform) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"computePlatform\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"profilingGroupName\":");
    try aws.json.writeValue(@TypeOf(input.profiling_group_name), input.profiling_group_name, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProfilingGroupOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
