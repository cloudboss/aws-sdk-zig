...
pub const PostAgentProfileInput = struct {
    ...
    /// prevent the
    /// accidental submission of duplicate profiling data if there are failures and
    /// retries.
    profile_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PostAgentProfileInput, options: Options) !PostAgentProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PostAgentProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/agentProfile");
    const path = try path_buf.toOwnedSlice(allocator);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.profile_token) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "profileToken=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
    const query = try query_buf.toOwnedSlice(allocator);

    const body = input.agent_profile;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PostAgentProfileOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PostAgentProfileOutput = .{};
