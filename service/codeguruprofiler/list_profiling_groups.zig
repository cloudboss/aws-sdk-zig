...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProfilingGroupsInput, options: Options) !ListProfilingGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProfilingGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_description) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeDescription=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProfilingGroupsOutput {
    var result: ListProfilingGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProfilingGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
