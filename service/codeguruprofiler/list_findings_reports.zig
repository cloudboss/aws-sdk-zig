...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFindingsReportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/findingsReports");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFindingsReportsOutput {
    var result: ListFindingsReportsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFindingsReportsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
