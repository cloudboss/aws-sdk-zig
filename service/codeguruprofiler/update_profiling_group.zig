...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProfilingGroupInput, options: Options) !UpdateProfilingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProfilingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.agent_orchestration_config), input.agent_orchestration_config, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProfilingGroupOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
