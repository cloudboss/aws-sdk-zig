...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetFrameMetricDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/frames/-/metrics");
    ...
    if (input.target_resolution) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "targetResolution=");
    ...
    if (input.frame_metrics) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"frameMetrics\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetFrameMetricDataOutput {
    var result: BatchGetFrameMetricDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetFrameMetricDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
