...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProfileTimesInput, options: Options) !ListProfileTimesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProfileTimesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/profileTimes");
    ...
    if (input.order_by) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "orderBy=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "period=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.period));
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProfileTimesOutput {
    var result: ListProfileTimesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProfileTimesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
