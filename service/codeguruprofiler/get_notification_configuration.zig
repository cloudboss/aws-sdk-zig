...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNotificationConfigurationInput, options: Options) !GetNotificationConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNotificationConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNotificationConfigurationOutput {
    var result: GetNotificationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNotificationConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
