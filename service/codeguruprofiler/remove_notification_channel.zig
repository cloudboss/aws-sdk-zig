...
pub const RemoveNotificationChannelInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveNotificationChannelInput, options: Options) !RemoveNotificationChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveNotificationChannelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveNotificationChannelOutput {
    var result: RemoveNotificationChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveNotificationChannelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
