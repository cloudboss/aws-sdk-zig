...
pub const GetRecordsOutput = struct {
    ...
    /// stream records. If
    /// set to `null`, the shard has been closed and the requested iterator will not
    /// return
    /// any more data.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRecordsInput, options: Options) !GetRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDBStreams_20120810.GetRecords");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRecordsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRecordsOutput, body, alloc);
}
