...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStreamsInput, options: Options) !ListStreamsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStreamsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDBStreams_20120810.ListStreams");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStreamsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListStreamsOutput, body, alloc);
}
