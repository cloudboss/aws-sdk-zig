...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartWorkflowInput, options: Options) !StartWorkflowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartWorkflowInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/migrationworkflow/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartWorkflowOutput {
    var result: StartWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartWorkflowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
