...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTemplateStepGroupsInput, options: Options) !ListTemplateStepGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTemplateStepGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/templatestepgroups/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTemplateStepGroupsOutput {
    var result: ListTemplateStepGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTemplateStepGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
