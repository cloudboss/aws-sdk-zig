...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkflowStepGroupsInput, options: Options) !ListWorkflowStepGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkflowStepGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkflowStepGroupsOutput {
    var result: ListWorkflowStepGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkflowStepGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
