...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkflowStepInput, options: Options) !GetWorkflowStepOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkflowStepInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkflowStepOutput {
    var result: GetWorkflowStepOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkflowStepOutput, body, alloc);
    }
    _ = status;
    _ = headers;
