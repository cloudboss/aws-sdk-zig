...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RetryWorkflowStepInput, options: Options) !RetryWorkflowStepOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RetryWorkflowStepInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/retryworkflowstep/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RetryWorkflowStepOutput {
    var result: RetryWorkflowStepOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RetryWorkflowStepOutput, body, alloc);
    }
    _ = status;
    _ = headers;
