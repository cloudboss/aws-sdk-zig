...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkflowStepInput, options: CallOptions) !CreateWorkflowStepOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateWorkflowStepInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.step_action_type), input.step_action_type, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
