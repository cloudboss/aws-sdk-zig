...
pub const CreateWorkflowStepInput = struct {
...
pub const CreateWorkflowStepOutput = struct {
    /// The ID of the step.
    id: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkflowStepInput, options: Options) !CreateWorkflowStepOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkflowStepInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"stepActionType\":");
    try aws.json.writeValue(@TypeOf(input.step_action_type), input.step_action_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"stepGroupId\":");
    try aws.json.writeValue(@TypeOf(input.step_group_id), input.step_group_id, alloc, &body_buf);
    has_prev = true;
    if (input.step_target) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"stepTarget\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.workflow_step_automation_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"workflowStepAutomationConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkflowStepOutput {
    var result: CreateWorkflowStepOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWorkflowStepOutput, body, alloc);
    }
    _ = status;
    _ = headers;
