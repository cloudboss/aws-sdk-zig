...
pub const StopWorkflowInput = struct {
...
pub const StopWorkflowOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopWorkflowInput, options: CallOptions) !StopWorkflowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopWorkflowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopWorkflowOutput {
    var result: StopWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopWorkflowOutput, body, allocator);
    }
    _ = status;
    _ = headers;
