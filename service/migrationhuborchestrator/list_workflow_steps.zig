...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkflowStepsInput, options: Options) !ListWorkflowStepsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkflowStepsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/workflowstepgroups/");
    try path_buf.appendSlice(alloc, input.step_group_id);
    try path_buf.appendSlice(alloc, "/workflowsteps");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkflowStepsOutput {
    var result: ListWorkflowStepsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkflowStepsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
