...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkflowStepInput, options: Options) !UpdateWorkflowStepOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkflowStepInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkflowStepOutput {
    var result: UpdateWorkflowStepOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateWorkflowStepOutput, body, alloc);
    }
    _ = status;
    _ = headers;
