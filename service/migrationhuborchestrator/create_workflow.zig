...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkflowInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.application_configuration_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"applicationConfigurationId\":");
    ...
    try aws.json.writeValue(@TypeOf(input.input_parameters), input.input_parameters, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"templateId\":");
    try aws.json.writeValue(@TypeOf(input.template_id), input.template_id, alloc, &body_buf);
    has_prev = true;
