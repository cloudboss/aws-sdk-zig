...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.template_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"templateDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"templateSource\":");
    try aws.json.writeValue(@TypeOf(input.template_source), input.template_source, alloc, &body_buf);
    has_prev = true;
