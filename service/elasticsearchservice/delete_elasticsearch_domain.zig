...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteElasticsearchDomainInput, options: Options) !DeleteElasticsearchDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteElasticsearchDomainInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteElasticsearchDomainOutput {
    var result: DeleteElasticsearchDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteElasticsearchDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
