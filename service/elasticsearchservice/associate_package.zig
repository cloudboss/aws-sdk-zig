...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociatePackageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2015-01-01/packages/associate/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociatePackageOutput {
    var result: AssociatePackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociatePackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
