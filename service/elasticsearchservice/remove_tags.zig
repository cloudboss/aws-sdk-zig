...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveTagsInput, options: Options) !RemoveTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2015-01-01/tags-removal";
    ...
    try aws.json.writeValue(@TypeOf(input.tag_keys), input.tag_keys, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveTagsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: RemoveTagsOutput = .{};
