...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUpgradeStatusInput, options: Options) !GetUpgradeStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUpgradeStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2015-01-01/es/upgradeDomain/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUpgradeStatusOutput {
    var result: GetUpgradeStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUpgradeStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
