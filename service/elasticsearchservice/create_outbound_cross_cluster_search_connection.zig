...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOutboundCrossClusterSearchConnectionInput, options: Options) !CreateOutboundCrossClusterSearchConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOutboundCrossClusterSearchConnectionInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("elasticsearchservice", "Elasticsearch Service", alloc);
    ...
    try body_buf.appendSlice(alloc, "\"ConnectionAlias\":");
    try aws.json.writeValue(@TypeOf(input.connection_alias), input.connection_alias, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DestinationDomainInfo\":");
    try aws.json.writeValue(@TypeOf(input.destination_domain_info), input.destination_domain_info, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceDomainInfo\":");
    try aws.json.writeValue(@TypeOf(input.source_domain_info), input.source_domain_info, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOutboundCrossClusterSearchConnectionOutput {
    var result: CreateOutboundCrossClusterSearchConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateOutboundCrossClusterSearchConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
