...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpgradeElasticsearchDomainInput, options: Options) !UpgradeElasticsearchDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpgradeElasticsearchDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.perform_check_only) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PerformCheckOnly\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TargetVersion\":");
    try aws.json.writeValue(@TypeOf(input.target_version), input.target_version, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpgradeElasticsearchDomainOutput {
    var result: UpgradeElasticsearchDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpgradeElasticsearchDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
