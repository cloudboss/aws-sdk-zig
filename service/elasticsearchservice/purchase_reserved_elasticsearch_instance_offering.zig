...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurchaseReservedElasticsearchInstanceOfferingInput, options: Options) !PurchaseReservedElasticsearchInstanceOfferingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseReservedElasticsearchInstanceOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReservationName\":");
    try aws.json.writeValue(@TypeOf(input.reservation_name), input.reservation_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReservedElasticsearchInstanceOfferingId\":");
    try aws.json.writeValue(@TypeOf(input.reserved_elasticsearch_instance_offering_id), input.reserved_elasticsearch_instance_offering_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PurchaseReservedElasticsearchInstanceOfferingOutput {
    var result: PurchaseReservedElasticsearchInstanceOfferingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PurchaseReservedElasticsearchInstanceOfferingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
