...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePackageInput, options: Options) !DeletePackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePackageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePackageOutput {
    var result: DeletePackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
