...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCompatibleElasticsearchVersionsInput, options: Options) !GetCompatibleElasticsearchVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCompatibleElasticsearchVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCompatibleElasticsearchVersionsOutput {
    var result: GetCompatibleElasticsearchVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCompatibleElasticsearchVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
