...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeElasticsearchDomainConfigInput, options: Options) !DescribeElasticsearchDomainConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeElasticsearchDomainConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeElasticsearchDomainConfigOutput {
    var result: DescribeElasticsearchDomainConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeElasticsearchDomainConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
