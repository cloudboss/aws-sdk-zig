...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AuthorizeVpcEndpointAccessInput, options: Options) !AuthorizeVpcEndpointAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AuthorizeVpcEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/authorizeVpcEndpointAccess");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AuthorizeVpcEndpointAccessOutput {
    var result: AuthorizeVpcEndpointAccessOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AuthorizeVpcEndpointAccessOutput, body, alloc);
    }
    _ = status;
    _ = headers;
