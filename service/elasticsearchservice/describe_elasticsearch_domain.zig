...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeElasticsearchDomainInput, options: Options) !DescribeElasticsearchDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeElasticsearchDomainInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeElasticsearchDomainOutput {
    var result: DescribeElasticsearchDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeElasticsearchDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
