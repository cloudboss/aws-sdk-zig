...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListElasticsearchInstanceTypesInput, options: Options) !ListElasticsearchInstanceTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListElasticsearchInstanceTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2015-01-01/es/instanceTypes/");
    try path_buf.appendSlice(alloc, input.elasticsearch_version);
    ...
    if (input.domain_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "domainName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListElasticsearchInstanceTypesOutput {
    var result: ListElasticsearchInstanceTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListElasticsearchInstanceTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
