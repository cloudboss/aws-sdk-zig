...
fn serializeRequest(alloc: std.mem.Allocator, input: AddTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.tag_list), input.tag_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddTagsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
