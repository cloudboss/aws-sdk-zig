...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReservedElasticsearchInstanceOfferingsInput, options: Options) !DescribeReservedElasticsearchInstanceOfferingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedElasticsearchInstanceOfferingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReservedElasticsearchInstanceOfferingsOutput {
    var result: DescribeReservedElasticsearchInstanceOfferingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeReservedElasticsearchInstanceOfferingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
