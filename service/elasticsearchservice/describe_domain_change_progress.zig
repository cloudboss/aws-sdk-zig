...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDomainChangeProgressInput, options: Options) !DescribeDomainChangeProgressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDomainChangeProgressInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2015-01-01/es/domain/");
    try path_buf.appendSlice(alloc, input.domain_name);
    try path_buf.appendSlice(alloc, "/progress");
    ...
    if (input.change_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "changeid=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDomainChangeProgressOutput {
    var result: DescribeDomainChangeProgressOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDomainChangeProgressOutput, body, alloc);
    }
    _ = status;
    _ = headers;
