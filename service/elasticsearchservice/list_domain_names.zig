...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDomainNamesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDomainNamesOutput {
    var result: ListDomainNamesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDomainNamesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
