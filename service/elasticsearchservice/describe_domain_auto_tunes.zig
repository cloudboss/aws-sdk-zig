...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDomainAutoTunesInput, options: Options) !DescribeDomainAutoTunesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDomainAutoTunesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/autoTunes");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDomainAutoTunesOutput {
    var result: DescribeDomainAutoTunesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDomainAutoTunesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
