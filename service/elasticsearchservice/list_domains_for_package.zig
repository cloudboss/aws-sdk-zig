...
pub const ListDomainsForPackageOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDomainsForPackageInput, options: Options) !ListDomainsForPackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDomainsForPackageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2015-01-01/packages/");
    try path_buf.appendSlice(alloc, input.package_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDomainsForPackageOutput {
    var result: ListDomainsForPackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDomainsForPackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
