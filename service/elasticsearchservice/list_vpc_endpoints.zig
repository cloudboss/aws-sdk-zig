...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVpcEndpointsInput, options: Options) !ListVpcEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVpcEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVpcEndpointsOutput {
    var result: ListVpcEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVpcEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
