...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeElasticsearchInstanceTypeLimitsInput, options: Options) !DescribeElasticsearchInstanceTypeLimitsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeElasticsearchInstanceTypeLimitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2015-01-01/es/instanceTypeLimits/");
    try path_buf.appendSlice(alloc, input.elasticsearch_version);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.instance_type);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeElasticsearchInstanceTypeLimitsOutput {
    var result: DescribeElasticsearchInstanceTypeLimitsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeElasticsearchInstanceTypeLimitsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
