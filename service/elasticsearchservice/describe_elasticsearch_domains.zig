...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeElasticsearchDomainsInput, options: CallOptions) !DescribeElasticsearchDomainsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeElasticsearchDomainsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeElasticsearchDomainsOutput {
    var result: DescribeElasticsearchDomainsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeElasticsearchDomainsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
