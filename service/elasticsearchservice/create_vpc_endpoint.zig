...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVpcEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DomainArn\":");
    try aws.json.writeValue(@TypeOf(input.domain_arn), input.domain_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVpcEndpointOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVpcEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
