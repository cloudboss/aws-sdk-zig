...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateElasticsearchDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.elasticsearch_cluster_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ElasticsearchClusterConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.elasticsearch_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ElasticsearchVersion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateElasticsearchDomainOutput {
    var result: CreateElasticsearchDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateElasticsearchDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
