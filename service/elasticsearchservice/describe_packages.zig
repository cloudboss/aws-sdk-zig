...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePackagesInput, options: Options) !DescribePackagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePackagesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2015-01-01/packages/describe";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePackagesOutput {
    var result: DescribePackagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePackagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
