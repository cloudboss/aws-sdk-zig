...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPrefetchScheduleInput, options: Options) !GetPrefetchScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPrefetchScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPrefetchScheduleOutput {
    var result: GetPrefetchScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPrefetchScheduleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
