...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetChannelScheduleInput, options: Options) !GetChannelScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChannelScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.audience) |v| {
        ...
        try query_buf.appendSlice(alloc, "audience=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.duration_minutes) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "durationMinutes=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChannelScheduleOutput {
    var result: GetChannelScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChannelScheduleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
