...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVodSourceInput, options: Options) !CreateVodSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVodSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVodSourceOutput {
    var result: CreateVodSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVodSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
