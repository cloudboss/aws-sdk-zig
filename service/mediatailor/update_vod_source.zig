...
pub const UpdateVodSourceInput = struct {
...
pub const UpdateVodSourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVodSourceInput, options: Options) !UpdateVodSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVodSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/vodSource/");
    try path_buf.appendSlice(alloc, input.vod_source_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVodSourceOutput {
    var result: UpdateVodSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVodSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
