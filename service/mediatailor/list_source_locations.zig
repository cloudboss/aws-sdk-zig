...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSourceLocationsInput, options: Options) !ListSourceLocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSourceLocationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSourceLocationsOutput {
    var result: ListSourceLocationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSourceLocationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
