...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVodSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVodSourceOutput {
    var result: DescribeVodSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeVodSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
