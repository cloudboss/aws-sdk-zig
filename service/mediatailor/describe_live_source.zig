...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLiveSourceInput, options: Options) !DescribeLiveSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLiveSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLiveSourceOutput {
    var result: DescribeLiveSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeLiveSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
