...
pub const DescribeSourceLocationInput = struct {
...
pub const DescribeSourceLocationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSourceLocationInput, options: CallOptions) !DescribeSourceLocationOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "mediatailor");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSourceLocationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSourceLocationOutput {
    var result: DescribeSourceLocationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSourceLocationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
