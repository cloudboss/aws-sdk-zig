...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSourceLocationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSourceLocationOutput {
    var result: DescribeSourceLocationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSourceLocationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
