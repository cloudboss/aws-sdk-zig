...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVodSourcesInput, options: Options) !ListVodSourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVodSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/vodSources");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVodSourcesOutput {
    var result: ListVodSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVodSourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
