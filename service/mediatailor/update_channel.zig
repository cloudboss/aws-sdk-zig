...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.audiences) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"Audiences\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.filler_slate) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FillerSlate\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Outputs\":");
    try aws.json.writeValue(@TypeOf(input.outputs), input.outputs, alloc, &body_buf);
    has_prev = true;
    if (input.time_shift_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TimeShiftConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
