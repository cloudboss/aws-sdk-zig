...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProgramInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProgramOutput {
    var result: UpdateProgramOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProgramOutput, body, alloc);
    }
    _ = status;
    _ = headers;
