...
pub const UpdateLiveSourceInput = struct {
...
pub const UpdateLiveSourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLiveSourceInput, options: CallOptions) !UpdateLiveSourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateLiveSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateLiveSourceOutput {
    var result: UpdateLiveSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateLiveSourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
