...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLiveSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLiveSourceOutput {
    var result: UpdateLiveSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateLiveSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
