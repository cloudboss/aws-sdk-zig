...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPrefetchSchedulesInput, options: Options) !ListPrefetchSchedulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPrefetchSchedulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPrefetchSchedulesOutput {
    var result: ListPrefetchSchedulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrefetchSchedulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
