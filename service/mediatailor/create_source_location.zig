...
pub const CreateSourceLocationInput = struct {
...
pub const CreateSourceLocationOutput = struct {
    /// Access configuration parameters. Configures the type of authentication used
    /// to access content from your source location.
    access_configuration: ?AccessConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSourceLocationInput, options: CallOptions) !CreateSourceLocationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSourceLocationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSourceLocationOutput {
    var result: CreateSourceLocationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSourceLocationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
