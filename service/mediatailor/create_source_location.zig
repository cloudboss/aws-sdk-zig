...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSourceLocationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSourceLocationOutput {
    var result: CreateSourceLocationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSourceLocationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
