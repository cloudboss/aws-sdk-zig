...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePrefetchScheduleInput, options: Options) !CreatePrefetchScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePrefetchScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/prefetchSchedule/");
    try path_buf.appendSlice(alloc, input.playback_configuration_name);
    ...
    if (input.consumption) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"Consumption\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.recurring_prefetch_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecurringPrefetchConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.retrieval) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Retrieval\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.schedule_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ScheduleType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePrefetchScheduleOutput {
    var result: CreatePrefetchScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePrefetchScheduleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
