...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLiveSourceInput, options: CallOptions) !CreateLiveSourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLiveSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLiveSourceOutput {
    var result: CreateLiveSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLiveSourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
