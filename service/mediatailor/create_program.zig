...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProgramInput, options: CallOptions) !CreateProgramOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateProgramInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.live_source_name) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"LiveSourceName\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
