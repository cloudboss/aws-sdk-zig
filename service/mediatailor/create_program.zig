...
pub const CreateProgramInput = struct {
...
pub const CreateProgramOutput = struct {
    /// The ad break configuration settings.
    ad_breaks: ?[]const AdBreak = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProgramInput, options: Options) !CreateProgramOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProgramInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ad_breaks) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AdBreaks\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.audience_media) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"AudienceMedia\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.live_source_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LiveSourceName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceLocationName\":");
    try aws.json.writeValue(@TypeOf(input.source_location_name), input.source_location_name, alloc, &body_buf);
    has_prev = true;
    if (input.vod_source_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"VodSourceName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProgramOutput {
    var result: CreateProgramOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProgramOutput, body, alloc);
    }
    _ = status;
    _ = headers;
