...
const LiveSource = @import("live_source.zig").LiveSource;

pub const ListLiveSourcesInput = struct {
...
pub const ListLiveSourcesOutput = struct {
    /// Lists the live sources.
    items: ?[]const LiveSource = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLiveSourcesInput, options: CallOptions) !ListLiveSourcesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLiveSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/liveSources");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLiveSourcesOutput {
    var result: ListLiveSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLiveSourcesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
