...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLiveSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/liveSources");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLiveSourcesOutput {
    var result: ListLiveSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLiveSourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
