...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPlaybackConfigurationsInput, options: Options) !ListPlaybackConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPlaybackConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPlaybackConfigurationsOutput {
    var result: ListPlaybackConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPlaybackConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
