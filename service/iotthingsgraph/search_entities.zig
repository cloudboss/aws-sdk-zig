...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchEntitiesInput, options: Options) !SearchEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IotThingsGraphFrontEndService.SearchEntities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchEntitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchEntitiesOutput, body, alloc);
}
