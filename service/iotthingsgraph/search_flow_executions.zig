...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchFlowExecutionsInput, options: Options) !SearchFlowExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchFlowExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IotThingsGraphFrontEndService.SearchFlowExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchFlowExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchFlowExecutionsOutput, body, alloc);
}
