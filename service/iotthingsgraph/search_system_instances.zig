...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchSystemInstancesInput, options: Options) !SearchSystemInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchSystemInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IotThingsGraphFrontEndService.SearchSystemInstances");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchSystemInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchSystemInstancesOutput, body, alloc);
}
