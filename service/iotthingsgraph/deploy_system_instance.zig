...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeploySystemInstanceInput, options: Options) !DeploySystemInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeploySystemInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IotThingsGraphFrontEndService.DeploySystemInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeploySystemInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeploySystemInstanceOutput, body, alloc);
}
