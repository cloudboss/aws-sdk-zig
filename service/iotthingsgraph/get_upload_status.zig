...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUploadStatusInput, options: Options) !GetUploadStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUploadStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IotThingsGraphFrontEndService.GetUploadStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUploadStatusOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetUploadStatusOutput, body, alloc);
}
