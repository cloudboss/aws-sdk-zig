...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNamespaceDeletionStatusInput, options: Options) !GetNamespaceDeletionStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNamespaceDeletionStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IotThingsGraphFrontEndService.GetNamespaceDeletionStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNamespaceDeletionStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetNamespaceDeletionStatusOutput, body, alloc);
}
