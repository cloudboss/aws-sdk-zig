...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchSystemTemplatesInput, options: Options) !SearchSystemTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchSystemTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IotThingsGraphFrontEndService.SearchSystemTemplates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchSystemTemplatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchSystemTemplatesOutput, body, alloc);
}
