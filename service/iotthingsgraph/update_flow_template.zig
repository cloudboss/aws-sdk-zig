...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFlowTemplateInput, options: Options) !UpdateFlowTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFlowTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IotThingsGraphFrontEndService.UpdateFlowTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFlowTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFlowTemplateOutput, body, alloc);
}
