...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSystemTemplateInput, options: Options) !CreateSystemTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSystemTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "IotThingsGraphFrontEndService.CreateSystemTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSystemTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateSystemTemplateOutput, body, alloc);
}
