...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStackResourcesInput, options: Options) !DescribeStackResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStackResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeStackResources&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStackResourcesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackResources")) {
                    result.stack_resources = try serde.deserializeStackResources(&reader, alloc, "member");
                } else {
