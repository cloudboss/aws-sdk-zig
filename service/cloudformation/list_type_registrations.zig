...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTypeRegistrationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListTypeRegistrations&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTypeRegistrationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.registration_token_list = try serde.deserializeRegistrationTokenList(&reader, alloc, "member");
                } else {
