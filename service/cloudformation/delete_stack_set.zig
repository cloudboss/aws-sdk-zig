...
pub const DeleteStackSetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStackSetInput, options: Options) !DeleteStackSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteStackSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteStackSet&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteStackSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteStackSetOutput = .{};
