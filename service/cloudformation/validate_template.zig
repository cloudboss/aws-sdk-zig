...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ValidateTemplateInput, options: Options) !ValidateTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ValidateTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ValidateTemplate&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ValidateTemplateOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Capabilities")) {
                    result.capabilities = try serde.deserializeCapabilities(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "CapabilitiesReason")) {
                    result.capabilities_reason = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "DeclaredTransforms")) {
                    result.declared_transforms = try serde.deserializeTransformsList(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "Description")) {
                    result.description = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "Parameters")) {
                    result.parameters = try serde.deserializeTemplateParameters(&reader, alloc, "member");
                } else {
