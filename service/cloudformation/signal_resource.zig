...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SignalResourceInput, options: Options) !SignalResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SignalResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SignalResource&Version=2010-05-15");
    try body_buf.appendSlice(alloc, "&LogicalResourceId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.logical_resource_id);
    ...
    try body_buf.appendSlice(alloc, "&UniqueId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.unique_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SignalResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
