...
pub const SetTypeDefaultVersionInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetTypeDefaultVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=SetTypeDefaultVersion&Version=2010-05-15");
    if (input.arn) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetTypeDefaultVersionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: SetTypeDefaultVersionOutput = .{};
