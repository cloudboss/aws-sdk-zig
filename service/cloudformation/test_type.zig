...
fn serializeRequest(alloc: std.mem.Allocator, input: TestTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=TestType&Version=2010-05-15");
    if (input.arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestTypeOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TypeVersionArn")) {
                    result.type_version_arn = try alloc.dupe(u8, try reader.readElementText());
                } else {
