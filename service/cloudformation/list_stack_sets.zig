...
pub const ListStackSetsInput = struct {
...
pub const ListStackSetsOutput = struct {
    ...
    /// user's StackSets.
    summaries: ?[]const StackSetSummary = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStackSetsInput, options: Options) !ListStackSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStackSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListStackSets&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStackSetsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListStackSetsResult")) break;
            },
            else => {},
        }
    }

    var result: ListStackSetsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.summaries = try serde.deserializeStackSetSummaries(&reader, alloc, "member");
                } else {
