...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStackInstancesInput, options: Options) !DeleteStackInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteStackInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteStackInstances&Version=2010-05-15");
    ...
    try body_buf.appendSlice(alloc, "&RetainStacks=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, if (input.retain_stacks) "true" else "false");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteStackInstancesOutput {
    _ = status;
    _ = headers;
