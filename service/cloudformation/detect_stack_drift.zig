...
pub const DetectStackDriftInput = struct {
...
pub const DetectStackDriftOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectStackDriftInput, options: Options) !DetectStackDriftOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectStackDriftInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DetectStackDrift&Version=2010-05-15");
    if (input.logical_resource_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&LogicalResourceIds.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectStackDriftOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DetectStackDriftResult")) break;
            },
            else => {},
        }
    }

    var result: DetectStackDriftOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackDriftDetectionId")) {
                    result.stack_drift_detection_id = try alloc.dupe(u8, try reader.readElementText());
                } else {
