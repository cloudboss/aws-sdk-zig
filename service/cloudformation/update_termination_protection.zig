...
pub const UpdateTerminationProtectionInput = struct {
...
pub const UpdateTerminationProtectionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTerminationProtectionInput, options: Options) !UpdateTerminationProtectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTerminationProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateTerminationProtection&Version=2010-05-15");
    try body_buf.appendSlice(alloc, "&EnableTerminationProtection=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, if (input.enable_termination_protection) "true" else "false");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTerminationProtectionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "UpdateTerminationProtectionResult")) break;
            },
            else => {},
        }
    }

    var result: UpdateTerminationProtectionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackId")) {
