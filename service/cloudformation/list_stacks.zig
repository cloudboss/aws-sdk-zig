...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStacksInput, options: Options) !ListStacksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStacksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListStacks&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStacksOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.stack_summaries = try serde.deserializeStackSummaries(&reader, alloc, "member");
                } else {
