...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStackInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateStackInstances&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStackInstancesOutput {
    _ = status;
    _ = headers;
