...
pub const CreateStackInstancesInput = struct {
    /// [Self-managed permissions] The account IDs of one or more Amazon Web
    /// Services accounts that you want to
    ...
    /// create stack instances using the
...
pub const CreateStackInstancesOutput = struct {
    /// The unique identifier for this StackSet operation.
    operation_id: ?[]const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStackInstancesInput, options: CallOptions) !CreateStackInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateStackInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateStackInstances&Version=2010-05-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateStackInstancesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateStackInstancesResult")) break;
            },
            else => {},
        }
    }

    var result: CreateStackInstancesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
