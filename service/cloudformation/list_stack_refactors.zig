...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStackRefactorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListStackRefactors&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStackRefactorsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.stack_refactor_summaries = try serde.deserializeStackRefactorSummaries(&reader, alloc, "member");
                } else {
