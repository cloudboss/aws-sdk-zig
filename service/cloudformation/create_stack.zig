...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateStack&Version=2010-05-15");
    ...
    if (input.on_failure) |v| {
        try body_buf.appendSlice(alloc, "&OnFailure=");
    ...
    if (input.timeout_in_minutes) |v| {
        try body_buf.appendSlice(alloc, "&TimeoutInMinutes=");
