...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStackSetOperationResultsInput, options: Options) !ListStackSetOperationResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStackSetOperationResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListStackSetOperationResults&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStackSetOperationResultsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.summaries = try serde.deserializeStackSetOperationResultSummaries(&reader, alloc, "member");
                } else {
