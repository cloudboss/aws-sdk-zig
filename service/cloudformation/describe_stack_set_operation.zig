...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStackSetOperationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeStackSetOperation&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStackSetOperationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackSetOperation")) {
                    result.stack_set_operation = try serde.deserializeStackSetOperation(&reader, alloc);
                } else {
