...
pub const DescribeStackSetOperationInput = struct {
...
pub const DescribeStackSetOperationOutput = struct {
    /// The specified StackSet operation.
    stack_set_operation: ?StackSetOperation = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStackSetOperationInput, options: CallOptions) !DescribeStackSetOperationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeStackSetOperationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeStackSetOperation&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeStackSetOperationOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeStackSetOperationResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeStackSetOperationOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackSetOperation")) {
                    result.stack_set_operation = try serde.deserializeStackSetOperation(allocator, &reader);
                } else {
