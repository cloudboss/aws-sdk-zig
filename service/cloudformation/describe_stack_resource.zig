...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStackResourceInput, options: Options) !DescribeStackResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStackResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeStackResource&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStackResourceOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackResourceDetail")) {
                    result.stack_resource_detail = try serde.deserializeStackResourceDetail(&reader, alloc);
                } else {
