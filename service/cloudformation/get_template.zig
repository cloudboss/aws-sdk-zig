...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetTemplate&Version=2010-05-15");
    ...
    if (input.template_stage) |v| {
        try body_buf.appendSlice(alloc, "&TemplateStage=");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StagesAvailable")) {
                    result.stages_available = try serde.deserializeStageList(&reader, alloc, "member");
