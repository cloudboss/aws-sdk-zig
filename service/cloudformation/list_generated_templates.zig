...
pub const ListGeneratedTemplatesInput = struct {
    ...
    /// get the next set of results. By default the `ListGeneratedTemplates` API
    /// action
...
pub const ListGeneratedTemplatesOutput = struct {
    ...
    summaries: ?[]const TemplateSummary = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGeneratedTemplatesInput, options: CallOptions) !ListGeneratedTemplatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListGeneratedTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListGeneratedTemplates&Version=2010-05-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListGeneratedTemplatesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListGeneratedTemplatesResult")) break;
            },
            else => {},
        }
    }

    var result: ListGeneratedTemplatesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.summaries = try serde.deserializeTemplateSummaries(allocator, &reader, "member");
                } else {
