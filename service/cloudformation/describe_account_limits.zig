...
pub const DescribeAccountLimitsOutput = struct {
    ...
    /// limits and their
    /// values.
    account_limits: ?[]const AccountLimit = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAccountLimitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAccountLimits&Version=2010-05-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAccountLimitsOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccountLimits")) {
                    result.account_limits = try serde.deserializeAccountLimitList(allocator, &reader, "member");
