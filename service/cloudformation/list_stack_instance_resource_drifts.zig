...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStackInstanceResourceDriftsInput, options: Options) !ListStackInstanceResourceDriftsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStackInstanceResourceDriftsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListStackInstanceResourceDrifts&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStackInstanceResourceDriftsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.summaries = try serde.deserializeStackInstanceResourceDriftsSummaries(&reader, alloc, "member");
                } else {
