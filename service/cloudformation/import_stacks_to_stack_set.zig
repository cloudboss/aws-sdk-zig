...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportStacksToStackSetInput, options: Options) !ImportStacksToStackSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportStacksToStackSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ImportStacksToStackSet&Version=2010-05-15");
    if (input.call_as) |v| {
    ...
    if (input.stack_ids_url) |v| {
        try body_buf.appendSlice(alloc, "&StackIdsUrl=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportStacksToStackSetOutput {
    _ = status;
    _ = headers;
