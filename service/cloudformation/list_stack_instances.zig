...
pub const ListStackInstancesInput = struct {
    ...
    filters: ?[]const StackInstanceFilter = null,
    ...
    /// instances for.
    stack_instance_account: ?[]const u8 = null,

    /// The name of the Region where you want to list stack instances.
    stack_instance_region: ?[]const u8 = null,

    /// The name or unique ID of the StackSet that you want to list stack instances
    /// for.
    stack_set_name: []const u8,
};

pub const ListStackInstancesOutput = struct {
    ...
    summaries: ?[]const StackInstanceSummary = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStackInstancesInput, options: Options) !ListStackInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStackInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListStackInstances&Version=2010-05-15");
    if (input.call_as) |v| {
    ...
    if (input.stack_instance_account) |v| {
        try body_buf.appendSlice(alloc, "&StackInstanceAccount=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.stack_instance_region) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStackInstancesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListStackInstancesResult")) break;
            },
            else => {},
        }
    }

    var result: ListStackInstancesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.summaries = try serde.deserializeStackInstanceSummaries(&reader, alloc, "member");
                } else {
