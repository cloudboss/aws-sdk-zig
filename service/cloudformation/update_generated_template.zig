...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGeneratedTemplateInput, options: Options) !UpdateGeneratedTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGeneratedTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateGeneratedTemplate&Version=2010-05-15");
    ...
    if (input.new_generated_template_name) |v| {
        try body_buf.appendSlice(alloc, "&NewGeneratedTemplateName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.refresh_all_resources) |v| {
        try body_buf.appendSlice(alloc, "&RefreshAllResources=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGeneratedTemplateOutput {
    _ = status;
    _ = headers;
