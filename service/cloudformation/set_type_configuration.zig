...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetTypeConfigurationInput, options: Options) !SetTypeConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetTypeConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetTypeConfiguration&Version=2010-05-15");
    try body_buf.appendSlice(alloc, "&Configuration=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.configuration);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetTypeConfigurationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ConfigurationArn")) {
                    result.configuration_arn = try alloc.dupe(u8, try reader.readElementText());
                } else {
