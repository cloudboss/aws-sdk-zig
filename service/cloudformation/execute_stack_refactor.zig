...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteStackRefactorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ExecuteStackRefactor&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteStackRefactorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: ExecuteStackRefactorOutput = .{};
