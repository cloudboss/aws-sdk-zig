...
/// stack creation and updating, and to roll back that operation if the
/// application breaches the
/// threshold of any of the alarms you've specified. For more information, see
/// [Roll back your CloudFormation stack on alarm breach with rollback
...
pub const RollbackConfiguration = struct {
    ...
    /// all the rollback
    /// triggers after the stack creation or update operation deploys all necessary
    /// resources.
    ///
    /// The default is 0 minutes.
    ///
    /// If you specify a monitoring period but don't specify any rollback triggers,
    /// CloudFormation still
    /// waits the specified period of time before cleaning up old resources after
    /// update operations. You
    /// can use this monitoring period to perform any manual stack validation
    /// desired, and manually
    /// cancel the stack creation or update (using
    /// [CancelUpdateStack](https://docs.aws.amazon.com/AWSCloudFormation/latest/APIReference/API_CancelUpdateStack.html), for example) as necessary.
    ///
    ...
    /// and applies them to
    /// any subsequent update operations for the stack, unless you specify
    /// otherwise. If you do specify
    /// rollback triggers for this parameter, those triggers replace any list of
    /// triggers previously
    ...
    /// before (for example,
    /// when creating the stack or during a previous stack update). Any triggers
