...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStackRefactorInput, options: Options) !DescribeStackRefactorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStackRefactorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeStackRefactor&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStackRefactorOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Description")) {
                    ...
                    result.stack_ids = try serde.deserializeStackIds(&reader, alloc, "member");
