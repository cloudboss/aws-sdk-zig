...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStackPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetStackPolicy&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStackPolicyOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackPolicyBody")) {
                    result.stack_policy_body = try alloc.dupe(u8, try reader.readElementText());
                } else {
