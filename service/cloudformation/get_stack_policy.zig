...
pub const GetStackPolicyInput = struct {
...
pub const GetStackPolicyOutput = struct {
    /// Structure that contains the stack policy body. For more information, see
    /// [Prevent updates to stack
    /// resources](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/protect-stack-resources.html) in the
    /// *CloudFormation User Guide*.
    stack_policy_body: ?[]const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStackPolicyInput, options: CallOptions) !GetStackPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetStackPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetStackPolicy&Version=2010-05-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetStackPolicyOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetStackPolicyResult")) break;
            },
            else => {},
        }
    }

    var result: GetStackPolicyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackPolicyBody")) {
                    result.stack_policy_body = try allocator.dupe(u8, try reader.readElementText());
                } else {
