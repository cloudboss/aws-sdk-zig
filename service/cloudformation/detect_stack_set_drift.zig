...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectStackSetDriftInput, options: Options) !DetectStackSetDriftOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectStackSetDriftInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DetectStackSetDrift&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectStackSetDriftOutput {
    _ = status;
    _ = headers;
