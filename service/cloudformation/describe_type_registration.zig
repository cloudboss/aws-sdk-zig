...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTypeRegistrationInput, options: CallOptions) !DescribeTypeRegistrationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTypeRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTypeRegistration&Version=2010-05-15");
    try body_buf.appendSlice(allocator, "&RegistrationToken=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.registration_token);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTypeRegistrationOutput {
    _ = status;
    _ = headers;
