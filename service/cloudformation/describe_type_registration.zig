...
pub const DescribeTypeRegistrationInput = struct {
...
pub const DescribeTypeRegistrationOutput = struct {
    /// The description of the extension registration request.
    description: ?[]const u8 = null,

    /// The current status of the extension registration request.
    progress_status: ?RegistrationStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTypeRegistrationInput, options: Options) !DescribeTypeRegistrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTypeRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTypeRegistration&Version=2010-05-15");
    try body_buf.appendSlice(alloc, "&RegistrationToken=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.registration_token);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTypeRegistrationOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeTypeRegistrationResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeTypeRegistrationOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
