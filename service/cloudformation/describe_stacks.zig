...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStacksInput, options: Options) !DescribeStacksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStacksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeStacks&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStacksOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.stacks = try serde.deserializeStacks(&reader, alloc, "member");
                } else {
