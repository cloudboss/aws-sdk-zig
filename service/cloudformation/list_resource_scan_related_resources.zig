...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceScanRelatedResourcesInput, options: Options) !ListResourceScanRelatedResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceScanRelatedResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListResourceScanRelatedResources&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceScanRelatedResourcesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.related_resources = try serde.deserializeRelatedResources(&reader, alloc, "member");
                } else {
