...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetStackPolicyInput, options: Options) !SetStackPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetStackPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetStackPolicy&Version=2010-05-15");
    ...
    if (input.stack_policy_body) |v| {
        try body_buf.appendSlice(alloc, "&StackPolicyBody=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.stack_policy_url) |v| {
        try body_buf.appendSlice(alloc, "&StackPolicyURL=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetStackPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
