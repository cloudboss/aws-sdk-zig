...
pub const ListStackSetAutoDeploymentTargetsInput = struct {
...
pub const ListStackSetAutoDeploymentTargetsOutput = struct {
    /// If the request doesn't return all the remaining results, `NextToken` is set
    /// to
    /// a token. To retrieve the next set of results, call
    /// [ListStackSetAutoDeploymentTargets](https://docs.aws.amazon.com/AWSCloudFormation/latest/APIReference/API_ListStackSetAutoDeploymentTargets.html) again and use that value for the
    ...
    summaries: ?[]const StackSetAutoDeploymentTargetSummary = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStackSetAutoDeploymentTargetsInput, options: Options) !ListStackSetAutoDeploymentTargetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStackSetAutoDeploymentTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListStackSetAutoDeploymentTargets&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStackSetAutoDeploymentTargetsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListStackSetAutoDeploymentTargetsResult")) break;
            },
            else => {},
        }
    }

    var result: ListStackSetAutoDeploymentTargetsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.summaries = try serde.deserializeStackSetAutoDeploymentTargetSummaries(&reader, alloc, "member");
                } else {
