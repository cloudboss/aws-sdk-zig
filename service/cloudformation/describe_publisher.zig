...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePublisherInput, options: Options) !DescribePublisherOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePublisherInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribePublisher&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePublisherOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IdentityProvider")) {
                    ...
                    result.publisher_profile = try alloc.dupe(u8, try reader.readElementText());
