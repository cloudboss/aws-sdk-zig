...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTypesInput, options: Options) !ListTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListTypes&Version=2010-05-15");
    if (input.deprecated_status) |v| {
        try body_buf.appendSlice(alloc, "&DeprecatedStatus=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
    if (input.filters) |v| {
        if (v.category) |sv| {
            try body_buf.appendSlice(alloc, "&Filters.Category=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(sv));
        }
        if (v.publisher_id) |sv| {
            try body_buf.appendSlice(alloc, "&Filters.PublisherId=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
        }
        if (v.type_name_prefix) |sv| {
            try body_buf.appendSlice(alloc, "&Filters.TypeNamePrefix=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTypesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.type_summaries = try serde.deserializeTypeSummaries(&reader, alloc, "member");
                } else {
