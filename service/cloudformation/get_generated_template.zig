...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGeneratedTemplateInput, options: Options) !GetGeneratedTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGeneratedTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetGeneratedTemplate&Version=2010-05-15");
    if (input.format) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGeneratedTemplateOutput {
    _ = status;
    _ = headers;
