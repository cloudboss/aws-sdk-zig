...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UpdateStack&Version=2010-05-15");
    ...
    if (input.stack_policy_during_update_body) |v| {
        try body_buf.appendSlice(allocator, "&StackPolicyDuringUpdateBody=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.stack_policy_during_update_url) |v| {
        try body_buf.appendSlice(allocator, "&StackPolicyDuringUpdateURL=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
