...
pub const UpdateStackInput = struct {
    ...
    /// CloudFormation
    /// associates with the stack. Specify an empty list to remove all notification
    /// topics.
    ...
    /// updating, the stack
    /// update fails. By default, CloudFormation grants permissions to all resource
    /// types. IAM uses this
    /// parameter for CloudFormation-specific condition keys in IAM policies. For
    ...
    /// stack policy
    /// during this update. If you don't specify a stack policy, the current policy
    /// that is associated
    /// with the stack will be used.
    stack_policy_during_update_body: ?[]const u8 = null,

    /// Location of a file that contains the temporary overriding stack policy. The
    /// URL must point
    ...
    /// that is associated
    /// with the stack will be used.
    stack_policy_during_update_url: ?[]const u8 = null,
    ...
    /// The location for an
    /// Amazon S3 bucket must start with `https://`. URLs from S3 static websites
    /// are not
    /// supported.
    ///
    ...
    /// these tags to
    /// supported resources in the stack. You can specify a maximum number of 50
    /// tags.
    ///
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateStack&Version=2010-05-15");
    ...
    if (input.disable_rollback) |v| {
        try body_buf.appendSlice(alloc, "&DisableRollback=");
    ...
    if (input.retain_except_on_create) |v| {
        try body_buf.appendSlice(alloc, "&RetainExceptOnCreate=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.role_arn) |v| {
    ...
    if (input.stack_policy_during_update_body) |v| {
        try body_buf.appendSlice(alloc, "&StackPolicyDuringUpdateBody=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.stack_policy_during_update_url) |v| {
        try body_buf.appendSlice(alloc, "&StackPolicyDuringUpdateURL=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStackOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "UpdateStackResult")) break;
            },
            else => {},
        }
    }

    var result: UpdateStackOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
