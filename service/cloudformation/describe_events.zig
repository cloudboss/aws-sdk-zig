...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEvents&Version=2010-05-15");
    ...
    if (input.filters) |v| {
        if (v.failed_events) |sv| {
            try body_buf.appendSlice(alloc, "&Filters.FailedEvents=");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.operation_events = try serde.deserializeOperationEvents(&reader, alloc, "member");
                } else {
