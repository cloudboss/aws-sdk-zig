...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PublishTypeInput, options: Options) !PublishTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PublishTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PublishType&Version=2010-05-15");
    if (input.arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PublishTypeOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PublicTypeArn")) {
                    result.public_type_arn = try alloc.dupe(u8, try reader.readElementText());
                } else {
