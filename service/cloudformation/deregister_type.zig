...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterTypeInput, options: Options) !DeregisterTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeregisterType&Version=2010-05-15");
    if (input.arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
