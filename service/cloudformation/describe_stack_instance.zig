...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStackInstanceInput, options: Options) !DescribeStackInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStackInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeStackInstance&Version=2010-05-15");
    if (input.call_as) |v| {
        try body_buf.appendSlice(alloc, "&CallAs=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
    try body_buf.appendSlice(alloc, "&StackInstanceAccount=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.stack_instance_account);
    try body_buf.appendSlice(alloc, "&StackInstanceRegion=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.stack_instance_region);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStackInstanceOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackInstance")) {
                    result.stack_instance = try serde.deserializeStackInstance(&reader, alloc);
                } else {
