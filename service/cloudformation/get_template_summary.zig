...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTemplateSummaryInput, options: Options) !GetTemplateSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTemplateSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetTemplateSummary&Version=2010-05-15");
    if (input.call_as) |v| {
    ...
    if (input.template_summary_config) |v| {
        if (v.treat_unrecognized_resource_types_as_warnings) |sv| {
            try body_buf.appendSlice(alloc, "&TemplateSummaryConfig.TreatUnrecognizedResourceTypesAsWarnings=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTemplateSummaryOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Capabilities")) {
                    ...
                    result.parameters = try serde.deserializeParameterDeclarations(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "ResourceIdentifierSummaries")) {
                    result.resource_identifier_summaries = try serde.deserializeResourceIdentifierSummaries(&reader, alloc, "member");
                    ...
                    result.warnings = try serde.deserializeWarnings(&reader, alloc);
                } else {
