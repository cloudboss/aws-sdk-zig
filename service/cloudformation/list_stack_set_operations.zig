...
pub const ListStackSetOperationsInput = struct {
...
pub const ListStackSetOperationsOutput = struct {
    ...
    summaries: ?[]const StackSetOperationSummary = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStackSetOperationsInput, options: CallOptions) !ListStackSetOperationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListStackSetOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListStackSetOperations&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListStackSetOperationsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListStackSetOperationsResult")) break;
            },
            else => {},
        }
    }

    var result: ListStackSetOperationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.summaries = try serde.deserializeStackSetOperationSummaries(allocator, &reader, "member");
                } else {
