...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHookResultsInput, options: Options) !ListHookResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHookResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListHookResults&Version=2010-05-15");
    ...
    if (input.type_arn) |v| {
        try body_buf.appendSlice(alloc, "&TypeArn=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHookResultsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HookResults")) {
                    result.hook_results = try serde.deserializeHookResultSummaries(&reader, alloc, "member");
