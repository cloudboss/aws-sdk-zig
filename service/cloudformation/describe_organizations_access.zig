...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrganizationsAccessInput, options: Options) !DescribeOrganizationsAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOrganizationsAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeOrganizationsAccess&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOrganizationsAccessOutput {
    _ = status;
    _ = headers;
    _ = alloc;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Status")) {
                    result.status = std.meta.stringToEnum(OrganizationStatus, try reader.readElementText());
                } else {
