...
pub const DescribeStackSetInput = struct {
...
pub const DescribeStackSetOutput = struct {
    /// The specified StackSet.
    stack_set: ?StackSet = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStackSetInput, options: CallOptions) !DescribeStackSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeStackSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeStackSet&Version=2010-05-15");
    if (input.call_as) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeStackSetOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeStackSetResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeStackSetOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StackSet")) {
                    result.stack_set = try serde.deserializeStackSet(allocator, &reader);
                } else {
