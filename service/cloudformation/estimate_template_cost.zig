...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EstimateTemplateCostInput, options: Options) !EstimateTemplateCostOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EstimateTemplateCostInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EstimateTemplateCost&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EstimateTemplateCostOutput {
    _ = status;
    _ = headers;
