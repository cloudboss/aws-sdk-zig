...
pub const ListStackRefactorActionsInput = struct {
...
pub const ListStackRefactorActionsOutput = struct {
    ...
    stack_refactor_actions: ?[]const StackRefactorAction = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStackRefactorActionsInput, options: Options) !ListStackRefactorActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStackRefactorActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListStackRefactorActions&Version=2010-05-15");
    ...
    try body_buf.appendSlice(alloc, "&StackRefactorId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.stack_refactor_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStackRefactorActionsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListStackRefactorActionsResult")) break;
            },
            else => {},
        }
    }

    var result: ListStackRefactorActionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "StackRefactorActions")) {
                    result.stack_refactor_actions = try serde.deserializeStackRefactorActions(&reader, alloc, "member");
                } else {
