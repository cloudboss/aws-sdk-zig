...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceScansInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListResourceScans&Version=2010-05-15");
    ...
    if (input.scan_type_filter) |v| {
        try body_buf.appendSlice(alloc, "&ScanTypeFilter=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceScansOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.resource_scan_summaries = try serde.deserializeResourceScanSummaries(&reader, alloc, "member");
                } else {
