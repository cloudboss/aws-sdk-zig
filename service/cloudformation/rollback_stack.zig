...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RollbackStackInput, options: Options) !RollbackStackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RollbackStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RollbackStack&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RollbackStackOutput {
    _ = status;
    _ = headers;
