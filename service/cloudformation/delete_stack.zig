...
pub const DeleteStackInput = struct {
    ...
    retain_resources: ?[]const []const u8 = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteStack&Version=2010-05-15");
    if (input.client_request_token) |v| {
        try body_buf.appendSlice(alloc, "&ClientRequestToken=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.deletion_mode) |v| {
        try body_buf.appendSlice(alloc, "&DeletionMode=");
    ...
    if (input.retain_resources) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&RetainResources.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteStackOutput {
    ...
    _ = body;
    _ = alloc;
    const result: DeleteStackOutput = .{};
