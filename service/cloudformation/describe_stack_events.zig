...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStackEventsInput, options: Options) !DescribeStackEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStackEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeStackEvents&Version=2010-05-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStackEventsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.stack_events = try serde.deserializeStackEvents(&reader, alloc, "member");
                } else {
