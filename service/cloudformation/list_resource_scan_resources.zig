...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceScanResourcesInput, options: Options) !ListResourceScanResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceScanResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListResourceScanResources&Version=2010-05-15");
    ...
    if (input.resource_type_prefix) |v| {
        try body_buf.appendSlice(alloc, "&ResourceTypePrefix=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.tag_key) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceScanResourcesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.resources = try serde.deserializeScannedResources(&reader, alloc, "member");
                } else {
