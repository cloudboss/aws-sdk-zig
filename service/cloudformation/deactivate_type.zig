...
pub const DeactivateTypeInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeactivateTypeInput, options: Options) !DeactivateTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeactivateTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeactivateType&Version=2010-05-15");
    if (input.arn) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeactivateTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeactivateTypeOutput = .{};
