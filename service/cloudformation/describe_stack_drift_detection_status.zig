...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStackDriftDetectionStatusInput, options: Options) !DescribeStackDriftDetectionStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStackDriftDetectionStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeStackDriftDetectionStatus&Version=2010-05-15");
    try body_buf.appendSlice(alloc, "&StackDriftDetectionId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.stack_drift_detection_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStackDriftDetectionStatusOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DetectionStatus")) {
                    ...
                    result.detection_status_reason = try alloc.dupe(u8, try reader.readElementText());
