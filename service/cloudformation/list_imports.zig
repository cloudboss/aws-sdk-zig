...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListImports&Version=2010-05-15");
    try body_buf.appendSlice(alloc, "&ExportName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.export_name);
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Imports")) {
                    result.imports = try serde.deserializeImports(&reader, alloc, "member");
