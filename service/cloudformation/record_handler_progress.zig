...
fn serializeRequest(alloc: std.mem.Allocator, input: RecordHandlerProgressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RecordHandlerProgress&Version=2010-05-15");
    try body_buf.appendSlice(alloc, "&BearerToken=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.bearer_token);
    if (input.client_request_token) |v| {
        try body_buf.appendSlice(alloc, "&ClientRequestToken=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.current_operation_status) |v| {
        try body_buf.appendSlice(alloc, "&CurrentOperationStatus=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
    if (input.error_code) |v| {
        try body_buf.appendSlice(alloc, "&ErrorCode=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
    try body_buf.appendSlice(alloc, "&OperationStatus=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.operation_status));
    if (input.resource_model) |v| {
        try body_buf.appendSlice(alloc, "&ResourceModel=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.status_message) |v| {
        try body_buf.appendSlice(alloc, "&StatusMessage=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RecordHandlerProgressOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
