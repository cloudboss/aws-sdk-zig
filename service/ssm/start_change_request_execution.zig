...
pub const StartChangeRequestExecutionOutput = struct {
    ...
    pub fn deinit(self: *StartChangeRequestExecutionOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: StartChangeRequestExecutionInput, options: Options) !StartChangeRequestExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartChangeRequestExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.StartChangeRequestExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartChangeRequestExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartChangeRequestExecutionOutput, body, alloc);
}
