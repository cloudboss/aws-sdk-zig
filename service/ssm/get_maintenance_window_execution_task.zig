...
pub const GetMaintenanceWindowExecutionTaskOutput = struct {
    ...
    pub fn deinit(self: *GetMaintenanceWindowExecutionTaskOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetMaintenanceWindowExecutionTaskInput, options: Options) !GetMaintenanceWindowExecutionTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMaintenanceWindowExecutionTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetMaintenanceWindowExecutionTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMaintenanceWindowExecutionTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMaintenanceWindowExecutionTaskOutput, body, alloc);
}
