...
pub const GetPatchBaselineForPatchGroupOutput = struct {
    ...
    pub fn deinit(self: *GetPatchBaselineForPatchGroupOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetPatchBaselineForPatchGroupInput, options: Options) !GetPatchBaselineForPatchGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPatchBaselineForPatchGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetPatchBaselineForPatchGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPatchBaselineForPatchGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPatchBaselineForPatchGroupOutput, body, alloc);
}
