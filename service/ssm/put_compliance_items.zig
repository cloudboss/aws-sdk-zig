...
pub const PutComplianceItemsOutput = struct {
    ...
    pub fn deinit(self: *PutComplianceItemsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: PutComplianceItemsInput, options: Options) !PutComplianceItemsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutComplianceItemsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.PutComplianceItems");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutComplianceItemsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
