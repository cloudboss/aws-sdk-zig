...
pub const GetParametersByPathOutput = struct {
    ...
    pub fn deinit(self: *GetParametersByPathOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetParametersByPathInput, options: Options) !GetParametersByPathOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetParametersByPathInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetParametersByPath");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetParametersByPathOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetParametersByPathOutput, body, alloc);
}
