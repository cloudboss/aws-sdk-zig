...
pub const DescribeAssociationExecutionTargetsOutput = struct {
    ...
    pub fn deinit(self: *DescribeAssociationExecutionTargetsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeAssociationExecutionTargetsInput, options: Options) !DescribeAssociationExecutionTargetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssociationExecutionTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeAssociationExecutionTargets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssociationExecutionTargetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAssociationExecutionTargetsOutput, body, alloc);
}
