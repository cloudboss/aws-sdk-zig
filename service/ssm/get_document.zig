...
pub const GetDocumentOutput = struct {
    ...
    pub fn deinit(self: *GetDocumentOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetDocumentInput, options: Options) !GetDocumentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDocumentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetDocument");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDocumentOutput {
    ...
    return aws.json.parseJsonObject(GetDocumentOutput, body, alloc);
}
