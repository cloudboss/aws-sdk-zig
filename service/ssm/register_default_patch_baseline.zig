...
pub const RegisterDefaultPatchBaselineInput = struct {
...
pub const RegisterDefaultPatchBaselineOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterDefaultPatchBaselineInput, options: CallOptions) !RegisterDefaultPatchBaselineOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterDefaultPatchBaselineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.RegisterDefaultPatchBaseline");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterDefaultPatchBaselineOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterDefaultPatchBaselineOutput, body, allocator);
}
