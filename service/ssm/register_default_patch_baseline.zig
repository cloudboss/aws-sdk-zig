...
pub const RegisterDefaultPatchBaselineOutput = struct {
    ...
    pub fn deinit(self: *RegisterDefaultPatchBaselineOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: RegisterDefaultPatchBaselineInput, options: Options) !RegisterDefaultPatchBaselineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterDefaultPatchBaselineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.RegisterDefaultPatchBaseline");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterDefaultPatchBaselineOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterDefaultPatchBaselineOutput, body, alloc);
}
