...
pub const CreateOpsMetadataInput = struct {
    ...
    /// of five tags for
    /// an OpsMetadata object. Tags enable you to categorize a resource in different
    /// ways, such as by
    /// purpose, owner, or environment. For example, you might want to tag an
    /// OpsMetadata object to
    /// identify an environment or target Amazon Web Services Region. In this case,
...
pub const CreateOpsMetadataOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOpsMetadataInput, options: CallOptions) !CreateOpsMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateOpsMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.CreateOpsMetadata");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateOpsMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateOpsMetadataOutput, body, allocator);
}
