...
pub const CreateOpsMetadataOutput = struct {
    ...
    pub fn deinit(self: *CreateOpsMetadataOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: CreateOpsMetadataInput, options: Options) !CreateOpsMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOpsMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.CreateOpsMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOpsMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateOpsMetadataOutput, body, alloc);
}
