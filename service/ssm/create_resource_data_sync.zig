...
pub const CreateResourceDataSyncInput = struct {
    ...
    s_3_destination: ?ResourceDataSyncS3Destination = null,
...
pub const CreateResourceDataSyncOutput = struct {
    ...
    pub fn deinit(self: *CreateResourceDataSyncOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: CreateResourceDataSyncInput, options: Options) !CreateResourceDataSyncOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourceDataSyncInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.CreateResourceDataSync");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourceDataSyncOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
