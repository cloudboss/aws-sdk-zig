...
pub const UpdateAssociationStatusOutput = struct {
    ...
    pub fn deinit(self: *UpdateAssociationStatusOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: UpdateAssociationStatusInput, options: Options) !UpdateAssociationStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssociationStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateAssociationStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssociationStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateAssociationStatusOutput, body, alloc);
}
