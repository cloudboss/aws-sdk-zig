...
pub const GetMaintenanceWindowOutput = struct {
    ...
    pub fn deinit(self: *GetMaintenanceWindowOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetMaintenanceWindowInput, options: Options) !GetMaintenanceWindowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMaintenanceWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetMaintenanceWindow");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMaintenanceWindowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMaintenanceWindowOutput, body, alloc);
}
