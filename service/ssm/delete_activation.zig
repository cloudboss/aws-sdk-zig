...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteActivationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DeleteActivation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteActivationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
