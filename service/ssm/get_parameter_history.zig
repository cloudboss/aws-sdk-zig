...
const ParameterHistory = @import("parameter_history.zig").ParameterHistory;

pub const GetParameterHistoryInput = struct {
    ...
    /// to review
    /// history. For parameters shared with you from another account, you must use
    /// the full ARN.
    name: []const u8,
...
pub const GetParameterHistoryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetParameterHistoryInput, options: CallOptions) !GetParameterHistoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetParameterHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.GetParameterHistory");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetParameterHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetParameterHistoryOutput, body, allocator);
}
