...
pub const ListOpsItemEventsOutput = struct {
    ...
    pub fn deinit(self: *ListOpsItemEventsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListOpsItemEventsInput, options: Options) !ListOpsItemEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOpsItemEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListOpsItemEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOpsItemEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOpsItemEventsOutput, body, alloc);
}
