...
pub const ListOpsItemEventsInput = struct {
    ...
    /// results.
    filters: ?[]const OpsItemEventFilter = null,
...
pub const ListOpsItemEventsOutput = struct {
    ...
    summaries: ?[]const OpsItemEventSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOpsItemEventsInput, options: CallOptions) !ListOpsItemEventsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOpsItemEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.ListOpsItemEvents");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOpsItemEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOpsItemEventsOutput, body, allocator);
}
