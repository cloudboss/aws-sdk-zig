...
pub const DescribeInventoryDeletionsOutput = struct {
    /// A list of status items for deleted inventory.
    inventory_deletions: ?[]const InventoryDeletionStatusItem = null,
    ...
    pub fn deinit(self: *DescribeInventoryDeletionsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeInventoryDeletionsInput, options: Options) !DescribeInventoryDeletionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInventoryDeletionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeInventoryDeletions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInventoryDeletionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInventoryDeletionsOutput, body, alloc);
}
