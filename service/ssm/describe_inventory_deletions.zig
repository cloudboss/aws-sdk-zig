...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInventoryDeletionsInput, options: CallOptions) !DescribeInventoryDeletionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInventoryDeletionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DescribeInventoryDeletions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInventoryDeletionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInventoryDeletionsOutput, body, allocator);
}
