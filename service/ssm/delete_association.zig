...
pub const DeleteAssociationOutput = struct {
    ...
    pub fn deinit(self: *DeleteAssociationOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteAssociationInput, options: Options) !DeleteAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeleteAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAssociationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
