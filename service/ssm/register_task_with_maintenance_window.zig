...
pub const RegisterTaskWithMaintenanceWindowOutput = struct {
    ...
    pub fn deinit(self: *RegisterTaskWithMaintenanceWindowOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: RegisterTaskWithMaintenanceWindowInput, options: Options) !RegisterTaskWithMaintenanceWindowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterTaskWithMaintenanceWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.RegisterTaskWithMaintenanceWindow");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterTaskWithMaintenanceWindowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterTaskWithMaintenanceWindowOutput, body, alloc);
}
