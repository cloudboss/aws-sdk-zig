...
pub const DescribePatchGroupsOutput = struct {
    ...
    pub fn deinit(self: *DescribePatchGroupsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribePatchGroupsInput, options: Options) !DescribePatchGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePatchGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribePatchGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePatchGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePatchGroupsOutput, body, alloc);
}
