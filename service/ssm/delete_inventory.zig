...
pub const DeleteInventoryOutput = struct {
    ...
    pub fn deinit(self: *DeleteInventoryOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DeleteInventoryInput, options: Options) !DeleteInventoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteInventoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeleteInventory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteInventoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteInventoryOutput, body, alloc);
}
