...
pub const DescribeInstancePatchesOutput = struct {
    ...
    pub fn deinit(self: *DescribeInstancePatchesOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeInstancePatchesInput, options: Options) !DescribeInstancePatchesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstancePatchesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeInstancePatches");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstancePatchesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstancePatchesOutput, body, alloc);
}
