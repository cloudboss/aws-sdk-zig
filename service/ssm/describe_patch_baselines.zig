...
pub const DescribePatchBaselinesOutput = struct {
    ...
    pub fn deinit(self: *DescribePatchBaselinesOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribePatchBaselinesInput, options: Options) !DescribePatchBaselinesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePatchBaselinesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribePatchBaselines");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePatchBaselinesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePatchBaselinesOutput, body, alloc);
}
