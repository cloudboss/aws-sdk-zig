...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelMaintenanceWindowExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.CancelMaintenanceWindowExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelMaintenanceWindowExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelMaintenanceWindowExecutionOutput, body, alloc);
}
