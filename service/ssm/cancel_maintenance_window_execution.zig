...
pub const CancelMaintenanceWindowExecutionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMaintenanceWindowExecutionInput, options: CallOptions) !CancelMaintenanceWindowExecutionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelMaintenanceWindowExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.CancelMaintenanceWindowExecution");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelMaintenanceWindowExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelMaintenanceWindowExecutionOutput, body, allocator);
}
