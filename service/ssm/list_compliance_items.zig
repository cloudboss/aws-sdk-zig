...
pub const ListComplianceItemsOutput = struct {
    ...
    pub fn deinit(self: *ListComplianceItemsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListComplianceItemsInput, options: Options) !ListComplianceItemsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListComplianceItemsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListComplianceItems");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListComplianceItemsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListComplianceItemsOutput, body, alloc);
}
