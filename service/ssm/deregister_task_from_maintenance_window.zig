...
pub const DeregisterTaskFromMaintenanceWindowOutput = struct {
    ...
    pub fn deinit(self: *DeregisterTaskFromMaintenanceWindowOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DeregisterTaskFromMaintenanceWindowInput, options: Options) !DeregisterTaskFromMaintenanceWindowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterTaskFromMaintenanceWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeregisterTaskFromMaintenanceWindow");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterTaskFromMaintenanceWindowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeregisterTaskFromMaintenanceWindowOutput, body, alloc);
}
