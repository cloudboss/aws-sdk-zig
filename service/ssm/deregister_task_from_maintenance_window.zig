...
pub const DeregisterTaskFromMaintenanceWindowOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterTaskFromMaintenanceWindowInput, options: CallOptions) !DeregisterTaskFromMaintenanceWindowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterTaskFromMaintenanceWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DeregisterTaskFromMaintenanceWindow");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterTaskFromMaintenanceWindowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeregisterTaskFromMaintenanceWindowOutput, body, allocator);
}
