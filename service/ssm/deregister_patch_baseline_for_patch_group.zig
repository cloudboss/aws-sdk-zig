...
pub const DeregisterPatchBaselineForPatchGroupOutput = struct {
    ...
    pub fn deinit(self: *DeregisterPatchBaselineForPatchGroupOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DeregisterPatchBaselineForPatchGroupInput, options: Options) !DeregisterPatchBaselineForPatchGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterPatchBaselineForPatchGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeregisterPatchBaselineForPatchGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterPatchBaselineForPatchGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeregisterPatchBaselineForPatchGroupOutput, body, alloc);
}
