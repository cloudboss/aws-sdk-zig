...
pub const ListAssociationsOutput = struct {
    ...
    pub fn deinit(self: *ListAssociationsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListAssociationsInput, options: Options) !ListAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssociationsOutput, body, alloc);
}
