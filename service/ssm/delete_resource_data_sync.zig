...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResourceDataSyncInput, options: Options) !DeleteResourceDataSyncOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteResourceDataSyncInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DeleteResourceDataSync");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteResourceDataSyncOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
