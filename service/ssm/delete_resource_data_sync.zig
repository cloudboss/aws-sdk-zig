...
pub const DeleteResourceDataSyncOutput = struct {
    ...
    pub fn deinit(self: *DeleteResourceDataSyncOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteResourceDataSyncInput, options: Options) !DeleteResourceDataSyncOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteResourceDataSyncInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeleteResourceDataSync");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteResourceDataSyncOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
