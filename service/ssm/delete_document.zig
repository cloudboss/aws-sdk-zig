...
pub const DeleteDocumentOutput = struct {
    ...
    pub fn deinit(self: *DeleteDocumentOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteDocumentInput, options: Options) !DeleteDocumentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDocumentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeleteDocument");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDocumentOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
