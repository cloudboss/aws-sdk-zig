...
pub const GetOpsSummaryOutput = struct {
    ...
    pub fn deinit(self: *GetOpsSummaryOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetOpsSummaryInput, options: Options) !GetOpsSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOpsSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetOpsSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOpsSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOpsSummaryOutput, body, alloc);
}
