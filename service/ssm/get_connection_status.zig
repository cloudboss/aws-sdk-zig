...
pub const GetConnectionStatusOutput = struct {
    ...
    pub fn deinit(self: *GetConnectionStatusOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetConnectionStatusInput, options: Options) !GetConnectionStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConnectionStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetConnectionStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConnectionStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetConnectionStatusOutput, body, alloc);
}
