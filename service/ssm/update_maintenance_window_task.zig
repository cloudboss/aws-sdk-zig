...
pub const UpdateMaintenanceWindowTaskOutput = struct {
    ...
    pub fn deinit(self: *UpdateMaintenanceWindowTaskOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: UpdateMaintenanceWindowTaskInput, options: Options) !UpdateMaintenanceWindowTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMaintenanceWindowTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateMaintenanceWindowTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMaintenanceWindowTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateMaintenanceWindowTaskOutput, body, alloc);
}
