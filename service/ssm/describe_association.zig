...
pub const DescribeAssociationOutput = struct {
    ...
    pub fn deinit(self: *DescribeAssociationOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeAssociationInput, options: Options) !DescribeAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAssociationOutput, body, alloc);
}
