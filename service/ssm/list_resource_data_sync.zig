...
pub const ListResourceDataSyncOutput = struct {
    ...
    pub fn deinit(self: *ListResourceDataSyncOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListResourceDataSyncInput, options: Options) !ListResourceDataSyncOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceDataSyncInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListResourceDataSync");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceDataSyncOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceDataSyncOutput, body, alloc);
}
