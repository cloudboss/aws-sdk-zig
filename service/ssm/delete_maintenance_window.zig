...
pub const DeleteMaintenanceWindowOutput = struct {
    ...
    pub fn deinit(self: *DeleteMaintenanceWindowOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DeleteMaintenanceWindowInput, options: Options) !DeleteMaintenanceWindowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMaintenanceWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeleteMaintenanceWindow");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMaintenanceWindowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteMaintenanceWindowOutput, body, alloc);
}
