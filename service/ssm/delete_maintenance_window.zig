...
pub const DeleteMaintenanceWindowOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMaintenanceWindowInput, options: CallOptions) !DeleteMaintenanceWindowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMaintenanceWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DeleteMaintenanceWindow");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMaintenanceWindowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteMaintenanceWindowOutput, body, allocator);
}
