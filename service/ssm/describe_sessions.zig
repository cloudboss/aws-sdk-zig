...
pub const DescribeSessionsOutput = struct {
    ...
    pub fn deinit(self: *DescribeSessionsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeSessionsInput, options: Options) !DescribeSessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeSessions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSessionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSessionsOutput, body, alloc);
}
