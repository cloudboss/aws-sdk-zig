...
pub const GetDeployablePatchSnapshotForInstanceOutput = struct {
    ...
    pub fn deinit(self: *GetDeployablePatchSnapshotForInstanceOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetDeployablePatchSnapshotForInstanceInput, options: Options) !GetDeployablePatchSnapshotForInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeployablePatchSnapshotForInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetDeployablePatchSnapshotForInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeployablePatchSnapshotForInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeployablePatchSnapshotForInstanceOutput, body, alloc);
}
