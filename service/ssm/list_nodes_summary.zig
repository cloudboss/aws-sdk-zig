...
pub const ListNodesSummaryOutput = struct {
    ...
    summary: ?[]const []const aws.map.StringMapEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListNodesSummaryInput, options: CallOptions) !ListNodesSummaryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListNodesSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.ListNodesSummary");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListNodesSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListNodesSummaryOutput, body, allocator);
}
