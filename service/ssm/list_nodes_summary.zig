...
pub const ListNodesSummaryOutput = struct {
    ...
    pub fn deinit(self: *ListNodesSummaryOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListNodesSummaryInput, options: Options) !ListNodesSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListNodesSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListNodesSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListNodesSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListNodesSummaryOutput, body, alloc);
}
