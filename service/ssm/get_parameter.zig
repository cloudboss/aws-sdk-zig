...
pub const GetParameterOutput = struct {
    ...
    pub fn deinit(self: *GetParameterOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetParameterInput, options: Options) !GetParameterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetParameterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetParameter");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetParameterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetParameterOutput, body, alloc);
}
