...
pub const DescribeMaintenanceWindowsForTargetOutput = struct {
    ...
    pub fn deinit(self: *DescribeMaintenanceWindowsForTargetOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeMaintenanceWindowsForTargetInput, options: Options) !DescribeMaintenanceWindowsForTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMaintenanceWindowsForTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeMaintenanceWindowsForTarget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMaintenanceWindowsForTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMaintenanceWindowsForTargetOutput, body, alloc);
}
