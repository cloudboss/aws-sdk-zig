...
pub const GetMaintenanceWindowExecutionOutput = struct {
    ...
    pub fn deinit(self: *GetMaintenanceWindowExecutionOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetMaintenanceWindowExecutionInput, options: Options) !GetMaintenanceWindowExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMaintenanceWindowExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetMaintenanceWindowExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMaintenanceWindowExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMaintenanceWindowExecutionOutput, body, alloc);
}
