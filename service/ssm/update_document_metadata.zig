...
const DocumentReviews = @import("document_reviews.zig").DocumentReviews;

pub const UpdateDocumentMetadataInput = struct {
...
pub const UpdateDocumentMetadataOutput = struct {
    ...
    pub fn deinit(self: *UpdateDocumentMetadataOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UpdateDocumentMetadataInput, options: Options) !UpdateDocumentMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDocumentMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateDocumentMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDocumentMetadataOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
