...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDocumentMetadataInput, options: Options) !UpdateDocumentMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDocumentMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.UpdateDocumentMetadata");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDocumentMetadataOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
