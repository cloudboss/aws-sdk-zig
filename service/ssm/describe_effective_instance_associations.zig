...
pub const DescribeEffectiveInstanceAssociationsOutput = struct {
    /// The associations for the requested managed node.
    associations: ?[]const InstanceAssociation = null,
    ...
    pub fn deinit(self: *DescribeEffectiveInstanceAssociationsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeEffectiveInstanceAssociationsInput, options: Options) !DescribeEffectiveInstanceAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEffectiveInstanceAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeEffectiveInstanceAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEffectiveInstanceAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEffectiveInstanceAssociationsOutput, body, alloc);
}
