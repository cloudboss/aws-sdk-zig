...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEffectiveInstanceAssociationsInput, options: CallOptions) !DescribeEffectiveInstanceAssociationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEffectiveInstanceAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DescribeEffectiveInstanceAssociations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEffectiveInstanceAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEffectiveInstanceAssociationsOutput, body, allocator);
}
