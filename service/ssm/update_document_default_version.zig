...
pub const UpdateDocumentDefaultVersionOutput = struct {
    ...
    pub fn deinit(self: *UpdateDocumentDefaultVersionOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: UpdateDocumentDefaultVersionInput, options: Options) !UpdateDocumentDefaultVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDocumentDefaultVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateDocumentDefaultVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDocumentDefaultVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateDocumentDefaultVersionOutput, body, alloc);
}
