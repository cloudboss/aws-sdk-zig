...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDocumentDefaultVersionInput, options: CallOptions) !UpdateDocumentDefaultVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDocumentDefaultVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.UpdateDocumentDefaultVersion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDocumentDefaultVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateDocumentDefaultVersionOutput, body, allocator);
}
