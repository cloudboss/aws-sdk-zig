...
pub const DescribeInstancePatchStatesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstancePatchStatesInput, options: CallOptions) !DescribeInstancePatchStatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstancePatchStatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DescribeInstancePatchStates");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstancePatchStatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstancePatchStatesOutput, body, allocator);
}
