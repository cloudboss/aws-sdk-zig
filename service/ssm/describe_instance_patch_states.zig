...
pub const DescribeInstancePatchStatesOutput = struct {
    ...
    pub fn deinit(self: *DescribeInstancePatchStatesOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeInstancePatchStatesInput, options: Options) !DescribeInstancePatchStatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstancePatchStatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeInstancePatchStates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstancePatchStatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstancePatchStatesOutput, body, alloc);
}
