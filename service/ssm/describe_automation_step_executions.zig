...
pub const DescribeAutomationStepExecutionsOutput = struct {
    ...
    pub fn deinit(self: *DescribeAutomationStepExecutionsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeAutomationStepExecutionsInput, options: Options) !DescribeAutomationStepExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAutomationStepExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeAutomationStepExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAutomationStepExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAutomationStepExecutionsOutput, body, alloc);
}
