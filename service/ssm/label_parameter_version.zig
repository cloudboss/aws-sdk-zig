...
pub const LabelParameterVersionOutput = struct {
    ...
    pub fn deinit(self: *LabelParameterVersionOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: LabelParameterVersionInput, options: Options) !LabelParameterVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: LabelParameterVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.LabelParameterVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !LabelParameterVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(LabelParameterVersionOutput, body, alloc);
}
