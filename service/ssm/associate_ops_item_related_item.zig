...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateOpsItemRelatedItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.AssociateOpsItemRelatedItem");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateOpsItemRelatedItemOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateOpsItemRelatedItemOutput, body, alloc);
}
