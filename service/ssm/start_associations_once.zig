...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAssociationsOnceInput, options: CallOptions) !StartAssociationsOnceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartAssociationsOnceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.StartAssociationsOnce");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartAssociationsOnceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
