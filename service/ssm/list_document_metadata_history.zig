...
const DocumentMetadataEnum = @import("document_metadata_enum.zig").DocumentMetadataEnum;
const DocumentMetadataResponseInfo = @import("document_metadata_response_info.zig").DocumentMetadataResponseInfo;

pub const ListDocumentMetadataHistoryInput = struct {
...
pub const ListDocumentMetadataHistoryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDocumentMetadataHistoryInput, options: CallOptions) !ListDocumentMetadataHistoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDocumentMetadataHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.ListDocumentMetadataHistory");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDocumentMetadataHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDocumentMetadataHistoryOutput, body, allocator);
}
