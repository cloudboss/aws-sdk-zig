...
pub const DescribeDocumentPermissionOutput = struct {
    ...
    pub fn deinit(self: *DescribeDocumentPermissionOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeDocumentPermissionInput, options: Options) !DescribeDocumentPermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDocumentPermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeDocumentPermission");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDocumentPermissionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDocumentPermissionOutput, body, alloc);
}
