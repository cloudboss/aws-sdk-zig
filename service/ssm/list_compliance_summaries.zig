...
pub const ListComplianceSummariesOutput = struct {
    ...
    pub fn deinit(self: *ListComplianceSummariesOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListComplianceSummariesInput, options: Options) !ListComplianceSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListComplianceSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListComplianceSummaries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListComplianceSummariesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListComplianceSummariesOutput, body, alloc);
}
