...
const DocumentFilter = @import("document_filter.zig").DocumentFilter;
...
pub const ListDocumentsInput = struct {
    ...
    filters: ?[]const DocumentKeyValuesFilter = null,
...
pub const ListDocumentsOutput = struct {
    ...
    document_identifiers: ?[]const DocumentIdentifier = null,
    ...
    pub fn deinit(self: *ListDocumentsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListDocumentsInput, options: Options) !ListDocumentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDocumentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListDocuments");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDocumentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDocumentsOutput, body, alloc);
}
