...
pub const DeleteParameterInput = struct {
...
pub const DeleteParameterOutput = struct {
    ...
    pub fn deinit(self: *DeleteParameterOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteParameterInput, options: Options) !DeleteParameterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteParameterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeleteParameter");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteParameterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
