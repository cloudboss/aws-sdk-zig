...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteParameterInput, options: Options) !DeleteParameterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteParameterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DeleteParameter");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteParameterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
