...
pub const GetAutomationExecutionOutput = struct {
    ...
    pub fn deinit(self: *GetAutomationExecutionOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetAutomationExecutionInput, options: Options) !GetAutomationExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAutomationExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetAutomationExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAutomationExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAutomationExecutionOutput, body, alloc);
}
