...
pub const DescribeMaintenanceWindowsOutput = struct {
    ...
    pub fn deinit(self: *DescribeMaintenanceWindowsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeMaintenanceWindowsInput, options: Options) !DescribeMaintenanceWindowsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMaintenanceWindowsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeMaintenanceWindows");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMaintenanceWindowsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMaintenanceWindowsOutput, body, alloc);
}
