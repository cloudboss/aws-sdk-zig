...
pub const GetMaintenanceWindowExecutionTaskInvocationOutput = struct {
    ...
    pub fn deinit(self: *GetMaintenanceWindowExecutionTaskInvocationOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetMaintenanceWindowExecutionTaskInvocationInput, options: Options) !GetMaintenanceWindowExecutionTaskInvocationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMaintenanceWindowExecutionTaskInvocationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetMaintenanceWindowExecutionTaskInvocation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMaintenanceWindowExecutionTaskInvocationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMaintenanceWindowExecutionTaskInvocationOutput, body, alloc);
}
