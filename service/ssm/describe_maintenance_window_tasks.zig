...
pub const DescribeMaintenanceWindowTasksOutput = struct {
    ...
    pub fn deinit(self: *DescribeMaintenanceWindowTasksOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeMaintenanceWindowTasksInput, options: Options) !DescribeMaintenanceWindowTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMaintenanceWindowTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeMaintenanceWindowTasks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMaintenanceWindowTasksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMaintenanceWindowTasksOutput, body, alloc);
}
