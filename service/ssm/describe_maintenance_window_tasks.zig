...
pub const DescribeMaintenanceWindowTasksOutput = struct {
    ...
    tasks: ?[]const MaintenanceWindowTask = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMaintenanceWindowTasksInput, options: CallOptions) !DescribeMaintenanceWindowTasksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMaintenanceWindowTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DescribeMaintenanceWindowTasks");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMaintenanceWindowTasksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMaintenanceWindowTasksOutput, body, allocator);
}
