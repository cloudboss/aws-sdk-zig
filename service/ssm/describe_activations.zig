...
pub const DescribeActivationsOutput = struct {
    /// A list of activations for your Amazon Web Services account.
    activation_list: ?[]const Activation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeActivationsInput, options: CallOptions) !DescribeActivationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeActivationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DescribeActivations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeActivationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeActivationsOutput, body, allocator);
}
