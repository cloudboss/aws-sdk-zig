...
pub const DescribeActivationsOutput = struct {
    ...
    pub fn deinit(self: *DescribeActivationsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeActivationsInput, options: Options) !DescribeActivationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeActivationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeActivations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeActivationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeActivationsOutput, body, alloc);
}
