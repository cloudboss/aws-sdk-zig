...
pub const DeregisterManagedInstanceOutput = struct {
    ...
    pub fn deinit(self: *DeregisterManagedInstanceOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeregisterManagedInstanceInput, options: Options) !DeregisterManagedInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterManagedInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeregisterManagedInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterManagedInstanceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
