...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterManagedInstanceInput, options: Options) !DeregisterManagedInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterManagedInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DeregisterManagedInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterManagedInstanceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
