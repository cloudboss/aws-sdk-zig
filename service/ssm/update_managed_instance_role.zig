...
pub const UpdateManagedInstanceRoleOutput = struct {
    ...
    pub fn deinit(self: *UpdateManagedInstanceRoleOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UpdateManagedInstanceRoleInput, options: Options) !UpdateManagedInstanceRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateManagedInstanceRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateManagedInstanceRole");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateManagedInstanceRoleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
