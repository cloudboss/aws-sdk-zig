...
pub const UpdatePatchBaselineOutput = struct {
    ...
    pub fn deinit(self: *UpdatePatchBaselineOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: UpdatePatchBaselineInput, options: Options) !UpdatePatchBaselineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePatchBaselineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdatePatchBaseline");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePatchBaselineOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdatePatchBaselineOutput, body, alloc);
}
