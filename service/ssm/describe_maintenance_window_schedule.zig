...
pub const DescribeMaintenanceWindowScheduleOutput = struct {
    ...
    pub fn deinit(self: *DescribeMaintenanceWindowScheduleOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeMaintenanceWindowScheduleInput, options: Options) !DescribeMaintenanceWindowScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMaintenanceWindowScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeMaintenanceWindowSchedule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMaintenanceWindowScheduleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMaintenanceWindowScheduleOutput, body, alloc);
}
