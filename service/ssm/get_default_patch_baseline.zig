...
pub const GetDefaultPatchBaselineOutput = struct {
    ...
    pub fn deinit(self: *GetDefaultPatchBaselineOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetDefaultPatchBaselineInput, options: Options) !GetDefaultPatchBaselineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDefaultPatchBaselineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetDefaultPatchBaseline");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDefaultPatchBaselineOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDefaultPatchBaselineOutput, body, alloc);
}
