...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDefaultPatchBaselineInput, options: CallOptions) !GetDefaultPatchBaselineOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDefaultPatchBaselineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.GetDefaultPatchBaseline");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDefaultPatchBaselineOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDefaultPatchBaselineOutput, body, allocator);
}
