...
pub const CreateMaintenanceWindowOutput = struct {
    ...
    pub fn deinit(self: *CreateMaintenanceWindowOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: CreateMaintenanceWindowInput, options: Options) !CreateMaintenanceWindowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMaintenanceWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.CreateMaintenanceWindow");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMaintenanceWindowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateMaintenanceWindowOutput, body, alloc);
}
