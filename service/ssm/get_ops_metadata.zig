...
pub const GetOpsMetadataOutput = struct {
    ...
    pub fn deinit(self: *GetOpsMetadataOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetOpsMetadataInput, options: Options) !GetOpsMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOpsMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetOpsMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOpsMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOpsMetadataOutput, body, alloc);
}
