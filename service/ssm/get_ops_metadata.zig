...
pub const GetOpsMetadataOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOpsMetadataInput, options: CallOptions) !GetOpsMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetOpsMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.GetOpsMetadata");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetOpsMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOpsMetadataOutput, body, allocator);
}
