...
pub const DescribeMaintenanceWindowExecutionTaskInvocationsOutput = struct {
    ...
    pub fn deinit(self: *DescribeMaintenanceWindowExecutionTaskInvocationsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeMaintenanceWindowExecutionTaskInvocationsInput, options: Options) !DescribeMaintenanceWindowExecutionTaskInvocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMaintenanceWindowExecutionTaskInvocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeMaintenanceWindowExecutionTaskInvocations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMaintenanceWindowExecutionTaskInvocationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMaintenanceWindowExecutionTaskInvocationsOutput, body, alloc);
}
