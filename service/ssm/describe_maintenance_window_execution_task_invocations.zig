...
pub const DescribeMaintenanceWindowExecutionTaskInvocationsOutput = struct {
    ...
    window_execution_task_invocation_identities: ?[]const MaintenanceWindowExecutionTaskInvocationIdentity = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMaintenanceWindowExecutionTaskInvocationsInput, options: CallOptions) !DescribeMaintenanceWindowExecutionTaskInvocationsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "ssm");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMaintenanceWindowExecutionTaskInvocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DescribeMaintenanceWindowExecutionTaskInvocations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMaintenanceWindowExecutionTaskInvocationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMaintenanceWindowExecutionTaskInvocationsOutput, body, allocator);
}
