...
pub const DescribeAutomationExecutionsOutput = struct {
    /// The list of details about each automation execution which has occurred which
    /// matches the
    /// filter specification, if any.
    automation_execution_metadata_list: ?[]const AutomationExecutionMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAutomationExecutionsInput, options: CallOptions) !DescribeAutomationExecutionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAutomationExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DescribeAutomationExecutions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAutomationExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAutomationExecutionsOutput, body, allocator);
}
