...
pub const DescribeAutomationExecutionsOutput = struct {
    ...
    pub fn deinit(self: *DescribeAutomationExecutionsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeAutomationExecutionsInput, options: Options) !DescribeAutomationExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAutomationExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeAutomationExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAutomationExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAutomationExecutionsOutput, body, alloc);
}
