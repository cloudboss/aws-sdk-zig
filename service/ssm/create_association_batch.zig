...
pub const CreateAssociationBatchOutput = struct {
    ...
    pub fn deinit(self: *CreateAssociationBatchOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: CreateAssociationBatchInput, options: Options) !CreateAssociationBatchOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssociationBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.CreateAssociationBatch");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssociationBatchOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAssociationBatchOutput, body, alloc);
}
