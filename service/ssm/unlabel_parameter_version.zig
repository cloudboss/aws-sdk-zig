...
pub const UnlabelParameterVersionOutput = struct {
    ...
    pub fn deinit(self: *UnlabelParameterVersionOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: UnlabelParameterVersionInput, options: Options) !UnlabelParameterVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UnlabelParameterVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UnlabelParameterVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UnlabelParameterVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UnlabelParameterVersionOutput, body, alloc);
}
