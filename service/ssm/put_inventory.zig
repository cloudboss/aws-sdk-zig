...
pub const PutInventoryOutput = struct {
    ...
    pub fn deinit(self: *PutInventoryOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: PutInventoryInput, options: Options) !PutInventoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutInventoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.PutInventory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutInventoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutInventoryOutput, body, alloc);
}
