...
pub const UpdateOpsItemOutput = struct {
    ...
    pub fn deinit(self: *UpdateOpsItemOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UpdateOpsItemInput, options: Options) !UpdateOpsItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateOpsItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateOpsItem");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateOpsItemOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
