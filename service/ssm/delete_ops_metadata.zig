...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOpsMetadataInput, options: Options) !DeleteOpsMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteOpsMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DeleteOpsMetadata");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteOpsMetadataOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
