...
pub const DeleteOpsMetadataOutput = struct {
    ...
    pub fn deinit(self: *DeleteOpsMetadataOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteOpsMetadataInput, options: Options) !DeleteOpsMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteOpsMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeleteOpsMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteOpsMetadataOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
