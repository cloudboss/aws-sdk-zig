...
pub const UpdateMaintenanceWindowOutput = struct {
    ...
    pub fn deinit(self: *UpdateMaintenanceWindowOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: UpdateMaintenanceWindowInput, options: Options) !UpdateMaintenanceWindowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMaintenanceWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateMaintenanceWindow");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMaintenanceWindowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateMaintenanceWindowOutput, body, alloc);
}
