...
pub const DeletePatchBaselineOutput = struct {
    /// The ID of the deleted patch baseline.
    baseline_id: ?[]const u8 = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *DeletePatchBaselineOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DeletePatchBaselineInput, options: Options) !DeletePatchBaselineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePatchBaselineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeletePatchBaseline");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePatchBaselineOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeletePatchBaselineOutput, body, alloc);
}
