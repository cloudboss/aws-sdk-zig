...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePatchBaselineInput, options: CallOptions) !DeletePatchBaselineOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePatchBaselineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DeletePatchBaseline");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePatchBaselineOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeletePatchBaselineOutput, body, allocator);
}
