...
pub const CreateOpsItemOutput = struct {
    ...
    pub fn deinit(self: *CreateOpsItemOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: CreateOpsItemInput, options: Options) !CreateOpsItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOpsItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.CreateOpsItem");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOpsItemOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateOpsItemOutput, body, alloc);
}
