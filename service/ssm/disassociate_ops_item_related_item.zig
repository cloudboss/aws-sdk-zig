...
pub const DisassociateOpsItemRelatedItemOutput = struct {
    ...
    pub fn deinit(self: *DisassociateOpsItemRelatedItemOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DisassociateOpsItemRelatedItemInput, options: Options) !DisassociateOpsItemRelatedItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateOpsItemRelatedItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DisassociateOpsItemRelatedItem");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateOpsItemRelatedItemOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
