...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateOpsItemRelatedItemInput, options: Options) !DisassociateOpsItemRelatedItemOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateOpsItemRelatedItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DisassociateOpsItemRelatedItem");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateOpsItemRelatedItemOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
