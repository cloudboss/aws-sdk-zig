...
pub const SendAutomationSignalOutput = struct {
    ...
    pub fn deinit(self: *SendAutomationSignalOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: SendAutomationSignalInput, options: Options) !SendAutomationSignalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendAutomationSignalInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.SendAutomationSignal");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendAutomationSignalOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
