...
pub const UpdateResourceDataSyncOutput = struct {
    ...
    pub fn deinit(self: *UpdateResourceDataSyncOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UpdateResourceDataSyncInput, options: Options) !UpdateResourceDataSyncOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResourceDataSyncInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateResourceDataSync");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResourceDataSyncOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
