...
pub const GetInventoryOutput = struct {
    ...
    pub fn deinit(self: *GetInventoryOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetInventoryInput, options: Options) !GetInventoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInventoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetInventory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInventoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInventoryOutput, body, alloc);
}
