...
pub const TerminateSessionOutput = struct {
    ...
    pub fn deinit(self: *TerminateSessionOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: TerminateSessionInput, options: Options) !TerminateSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TerminateSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.TerminateSession");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TerminateSessionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TerminateSessionOutput, body, alloc);
}
