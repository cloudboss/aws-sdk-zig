...
pub const GetOpsItemOutput = struct {
    ...
    pub fn deinit(self: *GetOpsItemOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetOpsItemInput, options: Options) !GetOpsItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOpsItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetOpsItem");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOpsItemOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOpsItemOutput, body, alloc);
}
