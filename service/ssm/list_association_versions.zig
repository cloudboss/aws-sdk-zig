...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssociationVersionsInput, options: CallOptions) !ListAssociationVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssociationVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.ListAssociationVersions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssociationVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssociationVersionsOutput, body, allocator);
}
