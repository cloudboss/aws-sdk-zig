...
pub const ListAssociationVersionsOutput = struct {
    /// Information about all versions of the association for the specified
    /// association ID.
    association_versions: ?[]const AssociationVersionInfo = null,
    ...
    pub fn deinit(self: *ListAssociationVersionsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListAssociationVersionsInput, options: Options) !ListAssociationVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssociationVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListAssociationVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociationVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssociationVersionsOutput, body, alloc);
}
