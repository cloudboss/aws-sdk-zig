...
pub const CancelCommandOutput = struct {
    ...
    pub fn deinit(self: *CancelCommandOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: CancelCommandInput, options: Options) !CancelCommandOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelCommandInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.CancelCommand");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelCommandOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
