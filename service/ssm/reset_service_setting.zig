...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetServiceSettingInput, options: CallOptions) !ResetServiceSettingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResetServiceSettingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.ResetServiceSetting");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResetServiceSettingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResetServiceSettingOutput, body, allocator);
}
