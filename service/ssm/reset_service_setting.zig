...
pub const ResetServiceSettingInput = struct {
...
pub const ResetServiceSettingOutput = struct {
    ...
    pub fn deinit(self: *ResetServiceSettingOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ResetServiceSettingInput, options: Options) !ResetServiceSettingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetServiceSettingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ResetServiceSetting");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetServiceSettingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResetServiceSettingOutput, body, alloc);
}
