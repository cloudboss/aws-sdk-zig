...
pub const DescribeAssociationExecutionsOutput = struct {
    ...
    pub fn deinit(self: *DescribeAssociationExecutionsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeAssociationExecutionsInput, options: Options) !DescribeAssociationExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssociationExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeAssociationExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssociationExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAssociationExecutionsOutput, body, alloc);
}
