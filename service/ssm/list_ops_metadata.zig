...
pub const ListOpsMetadataInput = struct {
    ...
    /// the call.
    filters: ?[]const OpsMetadataFilter = null,
...
pub const ListOpsMetadataOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOpsMetadataInput, options: CallOptions) !ListOpsMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOpsMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.ListOpsMetadata");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOpsMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOpsMetadataOutput, body, allocator);
}
