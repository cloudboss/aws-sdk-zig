...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCommandInvocationsInput, options: CallOptions) !ListCommandInvocationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCommandInvocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.ListCommandInvocations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCommandInvocationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCommandInvocationsOutput, body, allocator);
}
