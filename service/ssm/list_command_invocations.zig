...
pub const ListCommandInvocationsOutput = struct {
    /// (Optional) A list of all invocations.
    command_invocations: ?[]const CommandInvocation = null,
    ...
    pub fn deinit(self: *ListCommandInvocationsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListCommandInvocationsInput, options: Options) !ListCommandInvocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCommandInvocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListCommandInvocations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCommandInvocationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCommandInvocationsOutput, body, alloc);
}
