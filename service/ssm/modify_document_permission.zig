...
pub const ModifyDocumentPermissionOutput = struct {
    ...
    pub fn deinit(self: *ModifyDocumentPermissionOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ModifyDocumentPermissionInput, options: Options) !ModifyDocumentPermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDocumentPermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ModifyDocumentPermission");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDocumentPermissionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
