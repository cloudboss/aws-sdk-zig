...
pub const CreateAssociationOutput = struct {
    ...
    pub fn deinit(self: *CreateAssociationOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: CreateAssociationInput, options: Options) !CreateAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.CreateAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAssociationOutput, body, alloc);
}
