...
pub const ListResourceComplianceSummariesOutput = struct {
    ...
    pub fn deinit(self: *ListResourceComplianceSummariesOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListResourceComplianceSummariesInput, options: Options) !ListResourceComplianceSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceComplianceSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListResourceComplianceSummaries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceComplianceSummariesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceComplianceSummariesOutput, body, alloc);
}
