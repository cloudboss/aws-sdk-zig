...
pub const DeregisterTargetFromMaintenanceWindowOutput = struct {
    ...
    pub fn deinit(self: *DeregisterTargetFromMaintenanceWindowOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DeregisterTargetFromMaintenanceWindowInput, options: Options) !DeregisterTargetFromMaintenanceWindowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterTargetFromMaintenanceWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeregisterTargetFromMaintenanceWindow");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterTargetFromMaintenanceWindowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeregisterTargetFromMaintenanceWindowOutput, body, alloc);
}
