...
pub const GetInventorySchemaOutput = struct {
    ...
    pub fn deinit(self: *GetInventorySchemaOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetInventorySchemaInput, options: Options) !GetInventorySchemaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInventorySchemaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.GetInventorySchema");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInventorySchemaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInventorySchemaOutput, body, alloc);
}
