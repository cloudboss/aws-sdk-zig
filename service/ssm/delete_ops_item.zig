...
pub const DeleteOpsItemInput = struct {
...
pub const DeleteOpsItemOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOpsItemInput, options: CallOptions) !DeleteOpsItemOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteOpsItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DeleteOpsItem");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteOpsItemOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
