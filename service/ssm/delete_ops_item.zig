...
pub const DeleteOpsItemOutput = struct {
    ...
    pub fn deinit(self: *DeleteOpsItemOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteOpsItemInput, options: Options) !DeleteOpsItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteOpsItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeleteOpsItem");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteOpsItemOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
