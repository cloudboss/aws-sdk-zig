...
pub const UpdateMaintenanceWindowTargetOutput = struct {
    ...
    pub fn deinit(self: *UpdateMaintenanceWindowTargetOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: UpdateMaintenanceWindowTargetInput, options: Options) !UpdateMaintenanceWindowTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMaintenanceWindowTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateMaintenanceWindowTarget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMaintenanceWindowTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateMaintenanceWindowTargetOutput, body, alloc);
}
