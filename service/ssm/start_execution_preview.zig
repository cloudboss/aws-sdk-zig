...
pub const StartExecutionPreviewInput = struct {
    ...
    /// operation.
    execution_inputs: ?ExecutionInputs = null,
...
pub fn execute(client: *Client, input: StartExecutionPreviewInput, options: Options) !StartExecutionPreviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartExecutionPreviewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.StartExecutionPreview");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartExecutionPreviewOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartExecutionPreviewOutput, body, alloc);
}
