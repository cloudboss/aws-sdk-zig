...
pub const ListCommandsOutput = struct {
    ...
    pub fn deinit(self: *ListCommandsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListCommandsInput, options: Options) !ListCommandsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCommandsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListCommands");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCommandsOutput {
    ...
    return aws.json.parseJsonObject(ListCommandsOutput, body, alloc);
}
