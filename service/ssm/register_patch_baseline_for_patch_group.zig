...
pub const RegisterPatchBaselineForPatchGroupOutput = struct {
    ...
    pub fn deinit(self: *RegisterPatchBaselineForPatchGroupOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: RegisterPatchBaselineForPatchGroupInput, options: Options) !RegisterPatchBaselineForPatchGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterPatchBaselineForPatchGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.RegisterPatchBaselineForPatchGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterPatchBaselineForPatchGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterPatchBaselineForPatchGroupOutput, body, alloc);
}
