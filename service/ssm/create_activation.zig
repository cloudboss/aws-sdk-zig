...
pub const CreateActivationInput = struct {
    ...
    /// format, such as
    /// "2024-07-07T00:00:00". You can specify a date up to 30 days in advance. If
    /// you don't provide an
    ...
    registration_metadata: ?[]const RegistrationMetadataItem = null,
    ...
    /// activation to identify which servers or virtual machines (VMs) in your
    ...
    /// an activation ID
    /// and code. When you specify the activation ID and code, tags assigned to the
    /// activation are
    ...
    /// tag your
    /// on-premises servers, edge devices, and VMs after they connect to Systems
    /// Manager for the first time and are
    /// assigned a managed node ID. This means they are listed in the Amazon Web
    /// Services Systems Manager console with an ID that
    /// is prefixed with "mi-". For information about how to add tags to your
    /// managed nodes, see AddTagsToResource. For information about how to remove
    /// tags from your managed nodes,
...
pub const CreateActivationOutput = struct {
    ...
    pub fn deinit(self: *CreateActivationOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: CreateActivationInput, options: Options) !CreateActivationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateActivationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.CreateActivation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateActivationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateActivationOutput, body, alloc);
}
