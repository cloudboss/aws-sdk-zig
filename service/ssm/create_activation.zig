...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateActivationInput, options: CallOptions) !CreateActivationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateActivationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.CreateActivation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateActivationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateActivationOutput, body, allocator);
}
