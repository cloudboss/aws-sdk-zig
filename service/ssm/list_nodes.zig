...
pub const ListNodesOutput = struct {
    ...
    pub fn deinit(self: *ListNodesOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListNodesInput, options: Options) !ListNodesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListNodes");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListNodesOutput {
    ...
    return aws.json.parseJsonObject(ListNodesOutput, body, alloc);
}
