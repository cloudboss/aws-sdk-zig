...
pub const DescribeInstancePropertiesOutput = struct {
    ...
    pub fn deinit(self: *DescribeInstancePropertiesOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeInstancePropertiesInput, options: Options) !DescribeInstancePropertiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstancePropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeInstanceProperties");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstancePropertiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstancePropertiesOutput, body, alloc);
}
