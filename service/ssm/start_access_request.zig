...
pub const StartAccessRequestOutput = struct {
    ...
    pub fn deinit(self: *StartAccessRequestOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: StartAccessRequestInput, options: Options) !StartAccessRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartAccessRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.StartAccessRequest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartAccessRequestOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartAccessRequestOutput, body, alloc);
}
