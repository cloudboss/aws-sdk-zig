...
pub const DeleteParametersOutput = struct {
    ...
    pub fn deinit(self: *DeleteParametersOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DeleteParametersInput, options: Options) !DeleteParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DeleteParameters");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteParametersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteParametersOutput, body, alloc);
}
