...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInventoryEntriesInput, options: CallOptions) !ListInventoryEntriesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListInventoryEntriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.ListInventoryEntries");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListInventoryEntriesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInventoryEntriesOutput, body, allocator);
}
