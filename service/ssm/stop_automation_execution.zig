...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopAutomationExecutionInput, options: Options) !StopAutomationExecutionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopAutomationExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.StopAutomationExecution");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopAutomationExecutionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
