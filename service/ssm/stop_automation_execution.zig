...
pub const StopAutomationExecutionOutput = struct {
    ...
    pub fn deinit(self: *StopAutomationExecutionOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: StopAutomationExecutionInput, options: Options) !StopAutomationExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopAutomationExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.StopAutomationExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopAutomationExecutionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
