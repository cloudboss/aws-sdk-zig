...
pub const DescribeDocumentOutput = struct {
    ...
    pub fn deinit(self: *DescribeDocumentOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeDocumentInput, options: Options) !DescribeDocumentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDocumentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeDocument");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDocumentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDocumentOutput, body, alloc);
}
