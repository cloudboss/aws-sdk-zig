...
pub const ListOpsItemRelatedItemsInput = struct {
    ...
    /// results.
    filters: ?[]const OpsItemRelatedItemsFilter = null,
...
pub const ListOpsItemRelatedItemsOutput = struct {
    ...
    summaries: ?[]const OpsItemRelatedItemSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOpsItemRelatedItemsInput, options: CallOptions) !ListOpsItemRelatedItemsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOpsItemRelatedItemsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.ListOpsItemRelatedItems");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOpsItemRelatedItemsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOpsItemRelatedItemsOutput, body, allocator);
}
