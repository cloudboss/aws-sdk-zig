...
pub const ListOpsItemRelatedItemsOutput = struct {
    ...
    pub fn deinit(self: *ListOpsItemRelatedItemsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListOpsItemRelatedItemsInput, options: Options) !ListOpsItemRelatedItemsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOpsItemRelatedItemsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListOpsItemRelatedItems");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOpsItemRelatedItemsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOpsItemRelatedItemsOutput, body, alloc);
}
