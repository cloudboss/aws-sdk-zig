...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceAssociationsStatusInput, options: CallOptions) !DescribeInstanceAssociationsStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstanceAssociationsStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSSM.DescribeInstanceAssociationsStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstanceAssociationsStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstanceAssociationsStatusOutput, body, allocator);
}
