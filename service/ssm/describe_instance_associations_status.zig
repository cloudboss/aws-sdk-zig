...
pub const DescribeInstanceAssociationsStatusOutput = struct {
    /// Status information about the association.
    instance_association_status_infos: ?[]const InstanceAssociationStatusInfo = null,
    ...
    pub fn deinit(self: *DescribeInstanceAssociationsStatusOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DescribeInstanceAssociationsStatusInput, options: Options) !DescribeInstanceAssociationsStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceAssociationsStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.DescribeInstanceAssociationsStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceAssociationsStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstanceAssociationsStatusOutput, body, alloc);
}
