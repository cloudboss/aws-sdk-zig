...
pub const ListDocumentVersionsOutput = struct {
    ...
    pub fn deinit(self: *ListDocumentVersionsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListDocumentVersionsInput, options: Options) !ListDocumentVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDocumentVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.ListDocumentVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDocumentVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDocumentVersionsOutput, body, alloc);
}
