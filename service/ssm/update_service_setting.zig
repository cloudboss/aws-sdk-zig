...
pub const UpdateServiceSettingOutput = struct {
    ...
    pub fn deinit(self: *UpdateServiceSettingOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UpdateServiceSettingInput, options: Options) !UpdateServiceSettingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceSettingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSSM.UpdateServiceSetting");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceSettingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
