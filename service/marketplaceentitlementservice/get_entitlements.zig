...
pub const GetEntitlementsInput = struct {
    /// Filter is used to return entitlements for a specific customer or for a
    /// specific
    /// dimension. Filters are described as keys mapped to a lists of values.
    /// Filtered requests are
...
pub const GetEntitlementsOutput = struct {
    /// The set of entitlements found through the GetEntitlements operation. If the
    /// result
    /// contains an empty set of entitlements, NextToken might still be present and
    /// should be
    /// used.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEntitlementsInput, options: CallOptions) !GetEntitlementsOutput {
    ...
    try aws.signing.signRequest(alloc, &request, creds, client.config.region, "marketplaceentitlementservice");
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetEntitlementsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("marketplaceentitlementservice", "Marketplace Entitlement Service", allocator);
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMPEntitlementService.GetEntitlements");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetEntitlementsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEntitlementsOutput, body, allocator);
}
