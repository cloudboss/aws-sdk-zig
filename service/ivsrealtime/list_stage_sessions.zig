...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStageSessionsInput, options: Options) !ListStageSessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStageSessionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStageSessionsOutput {
    var result: ListStageSessionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStageSessionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
