...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListParticipantsInput, options: Options) !ListParticipantsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListParticipantsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.filter_by_published) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"filterByPublished\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.filter_by_recording_state) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"filterByRecordingState\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.filter_by_user_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"filterByUserId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListParticipantsOutput {
    var result: ListParticipantsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListParticipantsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
