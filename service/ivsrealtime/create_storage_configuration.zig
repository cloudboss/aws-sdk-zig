...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.s_3), input.s_3, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStorageConfigurationOutput {
    var result: CreateStorageConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateStorageConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
