...
pub const CreateStorageConfigurationInput = struct {
...
pub const CreateStorageConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStorageConfigurationInput, options: CallOptions) !CreateStorageConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/CreateStorageConfiguration";
    ...
    try aws.json.writeValue(@TypeOf(input.s_3), input.s_3, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateStorageConfigurationOutput {
    var result: CreateStorageConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateStorageConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
