...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListParticipantReplicasInput, options: Options) !ListParticipantReplicasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListParticipantReplicasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListParticipantReplicasOutput {
    var result: ListParticipantReplicasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListParticipantReplicasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
