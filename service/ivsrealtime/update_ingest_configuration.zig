...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIngestConfigurationInput, options: Options) !UpdateIngestConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIngestConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIngestConfigurationOutput {
    var result: UpdateIngestConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIngestConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
