...
pub const GetEncoderConfigurationInput = struct {
...
pub const GetEncoderConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEncoderConfigurationInput, options: Options) !GetEncoderConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEncoderConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetEncoderConfiguration";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEncoderConfigurationOutput {
    var result: GetEncoderConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEncoderConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
