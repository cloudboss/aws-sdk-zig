...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateParticipantTokenInput, options: Options) !CreateParticipantTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateParticipantTokenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateParticipantTokenOutput {
    var result: CreateParticipantTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateParticipantTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
