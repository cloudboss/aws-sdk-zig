...
pub const ImportPublicKeyInput = struct {
...
pub const ImportPublicKeyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportPublicKeyInput, options: Options) !ImportPublicKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportPublicKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.public_key_material), input.public_key_material, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportPublicKeyOutput {
    var result: ImportPublicKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportPublicKeyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
