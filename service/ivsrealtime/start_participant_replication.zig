...
pub const StartParticipantReplicationInput = struct {
    ...
    pub const json_field_names = .{
        ...
        .reconnect_window_seconds = "reconnectWindowSeconds",
        .source_stage_arn = "sourceStageArn",
    };
};

pub const StartParticipantReplicationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartParticipantReplicationInput, options: Options) !StartParticipantReplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartParticipantReplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/StartParticipantReplication";
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"destinationStageArn\":");
    try aws.json.writeValue(@TypeOf(input.destination_stage_arn), input.destination_stage_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"participantId\":");
    try aws.json.writeValue(@TypeOf(input.participant_id), input.participant_id, alloc, &body_buf);
    has_prev = true;
    if (input.reconnect_window_seconds) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"reconnectWindowSeconds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sourceStageArn\":");
    try aws.json.writeValue(@TypeOf(input.source_stage_arn), input.source_stage_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartParticipantReplicationOutput {
    var result: StartParticipantReplicationOutput = .{};
    _ = body;
    _ = status;
    if (headers.get("access-control-allow-origin")) |value| {
        result.access_control_allow_origin = try alloc.dupe(u8, value);
    }
    if (headers.get("access-control-expose-headers")) |value| {
        result.access_control_expose_headers = try alloc.dupe(u8, value);
    }
    ...
    if (headers.get("content-security-policy")) |value| {
        result.content_security_policy = try alloc.dupe(u8, value);
    }
    if (headers.get("strict-transport-security")) |value| {
        result.strict_transport_security = try alloc.dupe(u8, value);
    }
    if (headers.get("x-content-type-options")) |value| {
        result.x_content_type_options = try alloc.dupe(u8, value);
    }
    if (headers.get("x-frame-options")) |value| {
        result.x_frame_options = try alloc.dupe(u8, value);
    }
