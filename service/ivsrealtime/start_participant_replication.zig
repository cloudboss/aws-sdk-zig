...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartParticipantReplicationInput, options: CallOptions) !StartParticipantReplicationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartParticipantReplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.reconnect_window_seconds) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"reconnectWindowSeconds\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
