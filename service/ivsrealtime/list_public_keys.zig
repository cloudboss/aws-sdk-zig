...
pub const ListPublicKeysOutput = struct {
    /// If there are more public keys than `maxResults`, use `nextToken` in the
    /// request to get the next set.
    next_token: ?[]const u8 = null,

    /// List of the matching public keys (summary information only).
    public_keys: ?[]const PublicKeySummary = null,
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPublicKeysOutput {
    var result: ListPublicKeysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPublicKeysOutput, body, allocator);
    }
    _ = status;
    _ = headers;
