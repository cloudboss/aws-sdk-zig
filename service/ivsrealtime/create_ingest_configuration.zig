...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIngestConfigurationInput, options: Options) !CreateIngestConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIngestConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ingestProtocol\":");
    try aws.json.writeValue(@TypeOf(input.ingest_protocol), input.ingest_protocol, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIngestConfigurationOutput {
    var result: CreateIngestConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIngestConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
