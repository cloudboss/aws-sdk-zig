...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartCompositionInput, options: Options) !StartCompositionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartCompositionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartCompositionOutput {
    var result: StartCompositionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartCompositionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
