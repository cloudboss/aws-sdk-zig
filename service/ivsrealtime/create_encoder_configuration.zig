...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEncoderConfigurationInput, options: Options) !CreateEncoderConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEncoderConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.video) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"video\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEncoderConfigurationOutput {
    var result: CreateEncoderConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEncoderConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
