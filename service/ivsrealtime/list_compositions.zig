...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCompositionsInput, options: Options) !ListCompositionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCompositionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.filter_by_encoder_configuration_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"filterByEncoderConfigurationArn\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCompositionsOutput {
    var result: ListCompositionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCompositionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
