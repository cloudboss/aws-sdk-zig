...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStorageConfigurationInput, options: Options) !GetStorageConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStorageConfigurationOutput {
    var result: GetStorageConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetStorageConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
