...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInstanceInput, options: Options) !CreateInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"IdentityManagementType\":");
    try aws.json.writeValue(@TypeOf(input.identity_management_type), input.identity_management_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"InboundCallsEnabled\":");
    try aws.json.writeValue(@TypeOf(input.inbound_calls_enabled), input.inbound_calls_enabled, alloc, &body_buf);
    has_prev = true;
    if (input.instance_alias) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InstanceAlias\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OutboundCallsEnabled\":");
    try aws.json.writeValue(@TypeOf(input.outbound_calls_enabled), input.outbound_calls_enabled, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInstanceOutput {
    var result: CreateInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
