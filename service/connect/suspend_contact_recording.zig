...
pub const SuspendContactRecordingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SuspendContactRecordingInput, options: Options) !SuspendContactRecordingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SuspendContactRecordingInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/contact/suspend-recording";
    ...
    if (input.contact_recording_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ContactRecordingType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"InitialContactId\":");
    try aws.json.writeValue(@TypeOf(input.initial_contact_id), input.initial_contact_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SuspendContactRecordingOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: SuspendContactRecordingOutput = .{};
