...
pub const ListEvaluationFormsOutput = struct {
    /// Provides details about a list of evaluation forms belonging to an instance.
    evaluation_form_summary_list: ?[]const EvaluationFormSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEvaluationFormsInput, options: Options) !ListEvaluationFormsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEvaluationFormsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEvaluationFormsOutput {
    var result: ListEvaluationFormsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEvaluationFormsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
