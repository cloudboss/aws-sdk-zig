...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVocabularyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VocabularyName\":");
    try aws.json.writeValue(@TypeOf(input.vocabulary_name), input.vocabulary_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVocabularyOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVocabularyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
