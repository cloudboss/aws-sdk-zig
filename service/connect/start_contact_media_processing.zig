...
fn serializeRequest(alloc: std.mem.Allocator, input: StartContactMediaProcessingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.failure_mode) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FailureMode\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.processor_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ProcessorArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartContactMediaProcessingOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
