...
const ContactMediaProcessingFailureMode = @import("contact_media_processing_failure_mode.zig").ContactMediaProcessingFailureMode;

pub const StartContactMediaProcessingInput = struct {
    /// The identifier of the contact.
    contact_id: ?[]const u8 = null,

    /// The desired behavior for failed message processing.
    failure_mode: ?ContactMediaProcessingFailureMode = null,
    ...
    /// Amazon Resource Name of the lambda in
    /// the lambda console.
    processor_arn: ?[]const u8 = null,

    pub const json_field_names = .{
        .contact_id = "ContactId",
        .failure_mode = "FailureMode",
        .instance_id = "InstanceId",
        .processor_arn = "ProcessorArn",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartContactMediaProcessingInput, options: Options) !StartContactMediaProcessingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartContactMediaProcessingInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/contact/start-contact-media-processing";
    ...
    if (input.failure_mode) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"FailureMode\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    ...
    if (input.processor_arn) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ProcessorArn\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartContactMediaProcessingOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: StartContactMediaProcessingOutput = .{};
