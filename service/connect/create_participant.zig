...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateParticipantInput, options: Options) !CreateParticipantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateParticipantInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateParticipantOutput {
    var result: CreateParticipantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateParticipantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
