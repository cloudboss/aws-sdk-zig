...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchAssociateAnalyticsDataSetInput, options: Options) !BatchAssociateAnalyticsDataSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchAssociateAnalyticsDataSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchAssociateAnalyticsDataSetOutput {
    var result: BatchAssociateAnalyticsDataSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchAssociateAnalyticsDataSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
