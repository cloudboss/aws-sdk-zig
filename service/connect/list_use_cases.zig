...
pub const ListUseCasesInput = struct {
...
pub const ListUseCasesOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// The use cases.
    use_case_summary_list: ?[]const UseCase = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUseCasesInput, options: CallOptions) !ListUseCasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUseCasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUseCasesOutput {
    var result: ListUseCasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUseCasesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
