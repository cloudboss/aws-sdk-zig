...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUseCasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUseCasesOutput {
    var result: ListUseCasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUseCasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
