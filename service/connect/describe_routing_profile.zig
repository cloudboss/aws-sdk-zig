...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRoutingProfileInput, options: CallOptions) !DescribeRoutingProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRoutingProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeRoutingProfileOutput {
    var result: DescribeRoutingProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRoutingProfileOutput, body, allocator);
    }
    _ = status;
    _ = headers;
