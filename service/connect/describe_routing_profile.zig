...
const RoutingProfile = @import("routing_profile.zig").RoutingProfile;

pub const DescribeRoutingProfileInput = struct {
...
pub const DescribeRoutingProfileOutput = struct {
    /// The routing profile.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRoutingProfileInput, options: Options) !DescribeRoutingProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRoutingProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRoutingProfileOutput {
    var result: DescribeRoutingProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRoutingProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
