...
pub const DescribeAuthenticationProfileInput = struct {
...
pub const DescribeAuthenticationProfileOutput = struct {
    /// The authentication profile object being described.
    authentication_profile: ?AuthenticationProfile = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAuthenticationProfileInput, options: CallOptions) !DescribeAuthenticationProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAuthenticationProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAuthenticationProfileOutput {
    var result: DescribeAuthenticationProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAuthenticationProfileOutput, body, allocator);
    }
    _ = status;
    _ = headers;
