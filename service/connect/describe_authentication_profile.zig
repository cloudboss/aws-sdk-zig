...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAuthenticationProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAuthenticationProfileOutput {
    var result: DescribeAuthenticationProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAuthenticationProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
