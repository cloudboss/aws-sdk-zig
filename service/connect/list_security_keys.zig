...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecurityKeysInput, options: CallOptions) !ListSecurityKeysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSecurityKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/security-keys");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSecurityKeysOutput {
    var result: ListSecurityKeysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityKeysOutput, body, allocator);
    }
    _ = status;
    _ = headers;
