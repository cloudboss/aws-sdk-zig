...
const SecurityKey = @import("security_key.zig").SecurityKey;

pub const ListSecurityKeysInput = struct {
...
pub const ListSecurityKeysOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// The security keys.
    security_keys: ?[]const SecurityKey = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecurityKeysInput, options: Options) !ListSecurityKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/security-keys");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecurityKeysOutput {
    var result: ListSecurityKeysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityKeysOutput, body, alloc);
    }
    _ = status;
    _ = headers;
