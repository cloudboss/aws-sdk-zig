...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContactEvaluationInput, options: Options) !UpdateContactEvaluationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContactEvaluationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.updated_by) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UpdatedBy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContactEvaluationOutput {
    var result: UpdateContactEvaluationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateContactEvaluationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
