...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendChatIntegrationEventInput, options: Options) !SendChatIntegrationEventOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendChatIntegrationEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"DestinationId\":");
    try aws.json.writeValue(@TypeOf(input.destination_id), input.destination_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Event\":");
    try aws.json.writeValue(@TypeOf(input.event), input.event, alloc, &body_buf);
    has_prev = true;
    if (input.new_session_details) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"NewSessionDetails\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceId\":");
    try aws.json.writeValue(@TypeOf(input.source_id), input.source_id, alloc, &body_buf);
    has_prev = true;
    if (input.subtype) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Subtype\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendChatIntegrationEventOutput {
    var result: SendChatIntegrationEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendChatIntegrationEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
