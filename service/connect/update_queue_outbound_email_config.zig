...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQueueOutboundEmailConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/outbound-email-config");
    ...
    try aws.json.writeValue(@TypeOf(input.outbound_email_config), input.outbound_email_config, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQueueOutboundEmailConfigOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
