...
pub const UpdateQueueOutboundEmailConfigInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateQueueOutboundEmailConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/outbound-email-config");
    ...
    try aws.json.writeValue(@TypeOf(input.outbound_email_config), input.outbound_email_config, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateQueueOutboundEmailConfigOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateQueueOutboundEmailConfigOutput = .{};
