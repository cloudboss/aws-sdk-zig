...
fn serializeRequest(alloc: std.mem.Allocator, input: StartContactRecordingInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VoiceRecordingConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.voice_recording_configuration), input.voice_recording_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartContactRecordingOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
