...
const VoiceRecordingConfiguration = @import("voice_recording_configuration.zig").VoiceRecordingConfiguration;

pub const StartContactRecordingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartContactRecordingInput, options: Options) !StartContactRecordingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartContactRecordingInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/contact/start-recording";
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"VoiceRecordingConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.voice_recording_configuration), input.voice_recording_configuration, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartContactRecordingOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: StartContactRecordingOutput = .{};
