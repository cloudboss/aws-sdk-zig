...
pub const ListQuickConnectsInput = struct {
    ...
    quick_connect_types: ?[]const QuickConnectType = null,
...
pub const ListQuickConnectsOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// Information about the quick connects.
    quick_connect_summary_list: ?[]const QuickConnectSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQuickConnectsInput, options: Options) !ListQuickConnectsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQuickConnectsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.quick_connect_types) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "QuickConnectTypes=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQuickConnectsOutput {
    var result: ListQuickConnectsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQuickConnectsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
