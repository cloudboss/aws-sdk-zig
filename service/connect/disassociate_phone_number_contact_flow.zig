...
pub const DisassociatePhoneNumberContactFlowInput = struct {
...
const DisassociatePhoneNumberContactFlowOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociatePhoneNumberContactFlowInput, options: Options) !DisassociatePhoneNumberContactFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociatePhoneNumberContactFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/phone-number/");
    try path_buf.appendSlice(alloc, input.phone_number_id);
    try path_buf.appendSlice(alloc, "/contact-flow");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "instanceId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.instance_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociatePhoneNumberContactFlowOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociatePhoneNumberContactFlowOutput = .{};
