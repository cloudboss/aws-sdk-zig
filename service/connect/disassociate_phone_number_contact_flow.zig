...
pub const DisassociatePhoneNumberContactFlowOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociatePhoneNumberContactFlowInput, options: CallOptions) !DisassociatePhoneNumberContactFlowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociatePhoneNumberContactFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/contact-flow");
    const path = try path_buf.toOwnedSlice(allocator);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "instanceId=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.instance_id);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociatePhoneNumberContactFlowOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
