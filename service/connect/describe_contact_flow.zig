...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContactFlowInput, options: Options) !DescribeContactFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeContactFlowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeContactFlowOutput {
    var result: DescribeContactFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeContactFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
