...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePredefinedAttributeInput, options: CallOptions) !DescribePredefinedAttributeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribePredefinedAttributeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribePredefinedAttributeOutput {
    var result: DescribePredefinedAttributeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePredefinedAttributeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
