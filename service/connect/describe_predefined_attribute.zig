...
pub const DescribePredefinedAttributeInput = struct {
...
pub const DescribePredefinedAttributeOutput = struct {
    /// Information about the predefined attribute.
    predefined_attribute: ?PredefinedAttribute = null,

    pub const json_field_names = .{
        .predefined_attribute = "PredefinedAttribute",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePredefinedAttributeInput, options: Options) !DescribePredefinedAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePredefinedAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/predefined-attributes/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePredefinedAttributeOutput {
    var result: DescribePredefinedAttributeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePredefinedAttributeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
