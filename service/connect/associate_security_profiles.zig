...
pub const AssociateSecurityProfilesInput = struct {
...
const AssociateSecurityProfilesOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateSecurityProfilesInput, options: Options) !AssociateSecurityProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateSecurityProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/associate-security-profiles/");
    ...
    try aws.json.writeValue(@TypeOf(input.entity_arn), input.entity_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EntityType\":");
    try aws.json.writeValue(@TypeOf(input.entity_type), input.entity_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SecurityProfiles\":");
    try aws.json.writeValue(@TypeOf(input.security_profiles), input.security_profiles, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateSecurityProfilesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateSecurityProfilesOutput = .{};
