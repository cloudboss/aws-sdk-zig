...
pub const GetPromptFileInput = struct {
...
pub const GetPromptFileOutput = struct {
    ...
    /// they can access the prompt in
    /// S3.
    prompt_presigned_url: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPromptFileInput, options: Options) !GetPromptFileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPromptFileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/prompts/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.prompt_id);
    try path_buf.appendSlice(alloc, "/file");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPromptFileOutput {
    var result: GetPromptFileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPromptFileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
