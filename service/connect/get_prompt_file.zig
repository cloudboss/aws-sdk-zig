...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPromptFileInput, options: CallOptions) !GetPromptFileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPromptFileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/file");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPromptFileOutput {
    var result: GetPromptFileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPromptFileOutput, body, allocator);
    }
    _ = status;
    _ = headers;
