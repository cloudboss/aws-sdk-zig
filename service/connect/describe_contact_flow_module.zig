...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeContactFlowModuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeContactFlowModuleOutput {
    var result: DescribeContactFlowModuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeContactFlowModuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
