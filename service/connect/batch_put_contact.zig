...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchPutContactInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact/batch/");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ContactDataRequestList\":");
    try aws.json.writeValue(@TypeOf(input.contact_data_request_list), input.contact_data_request_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchPutContactOutput {
    var result: BatchPutContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchPutContactOutput, body, alloc);
    }
    _ = status;
    _ = headers;
