...
const ContactDataRequest = @import("contact_data_request.zig").ContactDataRequest;
...
pub const BatchPutContactInput = struct {
    ...
    contact_data_request_list: []const ContactDataRequest,
    ...
    pub const json_field_names = .{
        ...
        .contact_data_request_list = "ContactDataRequestList",
        .instance_id = "InstanceId",
    };
};

pub const BatchPutContactOutput = struct {
    /// List of requests for which contact creation failed.
    failed_request_list: ?[]const FailedRequest = null,

    /// List of requests for which contact was successfully created.
    successful_request_list: ?[]const SuccessfulRequest = null,

    pub const json_field_names = .{
        .failed_request_list = "FailedRequestList",
        .successful_request_list = "SuccessfulRequestList",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchPutContactInput, options: CallOptions) !BatchPutContactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchPutContactInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/contact/batch/");
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"ContactDataRequestList\":");
    try aws.json.writeValue(@TypeOf(input.contact_data_request_list), input.contact_data_request_list, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchPutContactOutput {
    var result: BatchPutContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchPutContactOutput, body, allocator);
    }
    _ = status;
    _ = headers;
