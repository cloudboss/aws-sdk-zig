...
pub const UpdateContactScheduleInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContactScheduleInput, options: Options) !UpdateContactScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContactScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.scheduled_time), input.scheduled_time, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContactScheduleOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateContactScheduleOutput = .{};
