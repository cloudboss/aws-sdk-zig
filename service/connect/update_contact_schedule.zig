...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateContactScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.scheduled_time), input.scheduled_time, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateContactScheduleOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
