...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRealtimeContactAnalysisSegmentsV2Input, options: Options) !ListRealtimeContactAnalysisSegmentsV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRealtimeContactAnalysisSegmentsV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact/list-real-time-analysis-segments-v2/");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OutputType\":");
    try aws.json.writeValue(@TypeOf(input.output_type), input.output_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SegmentTypes\":");
    try aws.json.writeValue(@TypeOf(input.segment_types), input.segment_types, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRealtimeContactAnalysisSegmentsV2Output {
    var result: ListRealtimeContactAnalysisSegmentsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRealtimeContactAnalysisSegmentsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
