...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApprovedOriginsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/approved-origins");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApprovedOriginsOutput {
    var result: ListApprovedOriginsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListApprovedOriginsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
