...
pub const ListApprovedOriginsInput = struct {
...
pub const ListApprovedOriginsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApprovedOriginsInput, options: CallOptions) !ListApprovedOriginsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListApprovedOriginsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/approved-origins");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListApprovedOriginsOutput {
    var result: ListApprovedOriginsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListApprovedOriginsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
