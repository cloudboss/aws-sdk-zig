...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHoursOfOperationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHoursOfOperationOutput {
    var result: DescribeHoursOfOperationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeHoursOfOperationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
