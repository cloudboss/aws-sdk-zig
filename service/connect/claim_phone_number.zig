...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ClaimPhoneNumberInput, options: Options) !ClaimPhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ClaimPhoneNumberInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ClaimPhoneNumberOutput {
    var result: ClaimPhoneNumberOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ClaimPhoneNumberOutput, body, alloc);
    }
    _ = status;
    _ = headers;
