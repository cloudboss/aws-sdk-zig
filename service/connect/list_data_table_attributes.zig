...
pub const ListDataTableAttributesInput = struct {
    /// Optional list of specific attribute IDs to retrieve. Used for CloudFormation
    /// to effectively describe attributes
    /// by ID. If NextToken is provided, this parameter is ignored.
    attribute_ids: ?[]const []const u8 = null,
...
pub const ListDataTableAttributesOutput = struct {
    /// A list of data table attributes with their complete configuration and
    /// metadata.
    attributes: ?[]const DataTableAttribute = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataTableAttributesInput, options: Options) !ListDataTableAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataTableAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.attribute_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"AttributeIds\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataTableAttributesOutput {
    var result: ListDataTableAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataTableAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
