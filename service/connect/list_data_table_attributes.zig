...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataTableAttributesInput, options: CallOptions) !ListDataTableAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDataTableAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.attribute_ids) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"AttributeIds\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDataTableAttributesOutput {
    var result: ListDataTableAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataTableAttributesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
