...
fn serializeRequest(alloc: std.mem.Allocator, input: PutUserStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AgentStatusId\":");
    try aws.json.writeValue(@TypeOf(input.agent_status_id), input.agent_status_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutUserStatusOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
