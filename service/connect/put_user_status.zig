...
pub const PutUserStatusInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutUserStatusInput, options: Options) !PutUserStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutUserStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"AgentStatusId\":");
    try aws.json.writeValue(@TypeOf(input.agent_status_id), input.agent_status_id, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutUserStatusOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutUserStatusOutput = .{};
