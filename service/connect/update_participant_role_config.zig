...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateParticipantRoleConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact/participant-role-config/");
    ...
    try body_buf.appendSlice(alloc, "\"ChannelConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.channel_configuration), input.channel_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateParticipantRoleConfigOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
