...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateParticipantRoleConfigInput, options: Options) !UpdateParticipantRoleConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateParticipantRoleConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/contact/participant-role-config/");
    ...
    try body_buf.appendSlice(allocator, "\"ChannelConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.channel_configuration), input.channel_configuration, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateParticipantRoleConfigOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateParticipantRoleConfigOutput = .{};
