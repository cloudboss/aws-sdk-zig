...
const SecurityProfileSearchCriteria = @import("security_profile_search_criteria.zig").SecurityProfileSearchCriteria;
const SecurityProfilesSearchFilter = @import("security_profiles_search_filter.zig").SecurityProfilesSearchFilter;
const SecurityProfileSearchSummary = @import("security_profile_search_summary.zig").SecurityProfileSearchSummary;

pub const SearchSecurityProfilesInput = struct {
    ...
    search_criteria: ?SecurityProfileSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?SecurityProfilesSearchFilter = null,
...
pub const SearchSecurityProfilesOutput = struct {
    ...
    security_profiles: ?[]const SecurityProfileSearchSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchSecurityProfilesInput, options: Options) !SearchSecurityProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchSecurityProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-security-profiles";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchSecurityProfilesOutput {
    var result: SearchSecurityProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchSecurityProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
