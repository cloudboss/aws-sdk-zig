...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchSecurityProfilesInput, options: CallOptions) !SearchSecurityProfilesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchSecurityProfilesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchSecurityProfilesOutput {
    var result: SearchSecurityProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchSecurityProfilesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
