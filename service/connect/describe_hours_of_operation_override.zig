...
const HoursOfOperationOverride = @import("hours_of_operation_override.zig").HoursOfOperationOverride;

pub const DescribeHoursOfOperationOverrideInput = struct {
...
pub const DescribeHoursOfOperationOverrideOutput = struct {
    /// Information about the hours of operations override.
    hours_of_operation_override: ?HoursOfOperationOverride = null,

    pub const json_field_names = .{
        .hours_of_operation_override = "HoursOfOperationOverride",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHoursOfOperationOverrideInput, options: Options) !DescribeHoursOfOperationOverrideOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHoursOfOperationOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/hours-of-operations/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.hours_of_operation_id);
    try path_buf.appendSlice(alloc, "/overrides/");
    try path_buf.appendSlice(alloc, input.hours_of_operation_override_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHoursOfOperationOverrideOutput {
    var result: DescribeHoursOfOperationOverrideOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeHoursOfOperationOverrideOutput, body, alloc);
    }
    _ = status;
    _ = headers;
