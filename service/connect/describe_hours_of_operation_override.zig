...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHoursOfOperationOverrideInput, options: CallOptions) !DescribeHoursOfOperationOverrideOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHoursOfOperationOverrideInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHoursOfOperationOverrideOutput {
    var result: DescribeHoursOfOperationOverrideOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeHoursOfOperationOverrideOutput, body, allocator);
    }
    _ = status;
    _ = headers;
