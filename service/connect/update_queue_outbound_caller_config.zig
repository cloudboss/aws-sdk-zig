...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQueueOutboundCallerConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/outbound-caller-config");
    ...
    try aws.json.writeValue(@TypeOf(input.outbound_caller_config), input.outbound_caller_config, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQueueOutboundCallerConfigOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
