...
pub const DeleteContactFlowModuleVersionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteContactFlowModuleVersionInput, options: Options) !DeleteContactFlowModuleVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteContactFlowModuleVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.contact_flow_module_version);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteContactFlowModuleVersionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteContactFlowModuleVersionOutput = .{};
