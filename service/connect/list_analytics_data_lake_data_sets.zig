...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnalyticsDataLakeDataSetsInput, options: CallOptions) !ListAnalyticsDataLakeDataSetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAnalyticsDataLakeDataSetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAnalyticsDataLakeDataSetsOutput {
    var result: ListAnalyticsDataLakeDataSetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAnalyticsDataLakeDataSetsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
