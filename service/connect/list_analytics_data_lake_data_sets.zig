...
const AnalyticsDataSetsResult = @import("analytics_data_sets_result.zig").AnalyticsDataSetsResult;

pub const ListAnalyticsDataLakeDataSetsInput = struct {
...
pub const ListAnalyticsDataLakeDataSetsOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// An array of successful results: `DataSetId`, `DataSetName`. This is a
    /// paginated API, so
    /// `nextToken` is given if there are more results to be returned.
    results: ?[]const AnalyticsDataSetsResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnalyticsDataLakeDataSetsInput, options: Options) !ListAnalyticsDataLakeDataSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAnalyticsDataLakeDataSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/analytics-data/instance/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAnalyticsDataLakeDataSetsOutput {
    var result: ListAnalyticsDataLakeDataSetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAnalyticsDataLakeDataSetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
