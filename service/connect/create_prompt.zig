...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePromptInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.s3_uri), input.s3_uri, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePromptOutput {
    var result: CreatePromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePromptOutput, body, alloc);
    }
    _ = status;
    _ = headers;
