...
pub const ListAssociatedContactsOutput = struct {
    /// List of the contact summary for all the contacts in contact tree associated
    /// with unique identifier.
    contact_summary_list: ?[]const AssociatedContactSummary = null,
    ...
    pub const json_field_names = .{
        .contact_summary_list = "ContactSummaryList",
        .next_token = "NextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssociatedContactsInput, options: Options) !ListAssociatedContactsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssociatedContactsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact/associated/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociatedContactsOutput {
    var result: ListAssociatedContactsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssociatedContactsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
