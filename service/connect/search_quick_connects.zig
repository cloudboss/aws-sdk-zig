...
const QuickConnectSearchCriteria = @import("quick_connect_search_criteria.zig").QuickConnectSearchCriteria;
const QuickConnectSearchFilter = @import("quick_connect_search_filter.zig").QuickConnectSearchFilter;
const QuickConnect = @import("quick_connect.zig").QuickConnect;

pub const SearchQuickConnectsInput = struct {
    ...
    search_criteria: ?QuickConnectSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?QuickConnectSearchFilter = null,
...
pub const SearchQuickConnectsOutput = struct {
    ...
    quick_connects: ?[]const QuickConnect = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchQuickConnectsInput, options: Options) !SearchQuickConnectsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchQuickConnectsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-quick-connects";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchQuickConnectsOutput {
    var result: SearchQuickConnectsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchQuickConnectsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
