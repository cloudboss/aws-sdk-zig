...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchQuickConnectsInput, options: CallOptions) !SearchQuickConnectsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchQuickConnectsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchQuickConnectsOutput {
    var result: SearchQuickConnectsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchQuickConnectsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
