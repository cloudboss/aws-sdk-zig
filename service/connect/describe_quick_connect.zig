...
pub const DescribeQuickConnectInput = struct {
...
pub const DescribeQuickConnectOutput = struct {
    /// Information about the quick connect.
    quick_connect: ?QuickConnect = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeQuickConnectInput, options: Options) !DescribeQuickConnectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeQuickConnectInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeQuickConnectOutput {
    var result: DescribeQuickConnectOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeQuickConnectOutput, body, alloc);
    }
    _ = status;
    _ = headers;
