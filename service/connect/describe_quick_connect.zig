...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeQuickConnectInput, options: CallOptions) !DescribeQuickConnectOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeQuickConnectInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeQuickConnectOutput {
    var result: DescribeQuickConnectOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeQuickConnectOutput, body, allocator);
    }
    _ = status;
    _ = headers;
