...
pub const TagContactInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TagContactInput, options: Options) !TagContactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TagContactInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TagContactOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: TagContactOutput = .{};
