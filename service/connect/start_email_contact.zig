...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartEmailContactInput, options: Options) !StartEmailContactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartEmailContactInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.from_email_address), input.from_email_address, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartEmailContactOutput {
    var result: StartEmailContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartEmailContactOutput, body, alloc);
    }
    _ = status;
    _ = headers;
