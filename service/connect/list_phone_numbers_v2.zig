...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPhoneNumbersV2Input, config: *aws.Config) !aws.http.Request {
    ...
    if (input.phone_number_country_codes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PhoneNumberCountryCodes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.phone_number_types) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PhoneNumberTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPhoneNumbersV2Output {
    var result: ListPhoneNumbersV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPhoneNumbersV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
