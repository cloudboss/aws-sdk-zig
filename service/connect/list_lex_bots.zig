...
pub const ListLexBotsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLexBotsInput, options: CallOptions) !ListLexBotsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLexBotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/lex-bots");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLexBotsOutput {
    var result: ListLexBotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLexBotsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
