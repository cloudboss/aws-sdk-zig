...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLexBotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/lex-bots");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLexBotsOutput {
    var result: ListLexBotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLexBotsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
