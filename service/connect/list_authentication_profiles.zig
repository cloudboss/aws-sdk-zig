...
const AuthenticationProfileSummary = @import("authentication_profile_summary.zig").AuthenticationProfileSummary;

pub const ListAuthenticationProfilesInput = struct {
...
pub const ListAuthenticationProfilesOutput = struct {
    /// A summary of a given authentication profile.
    authentication_profile_summary_list: ?[]const AuthenticationProfileSummary = null,
    ...
    pub const json_field_names = .{
        .authentication_profile_summary_list = "AuthenticationProfileSummaryList",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAuthenticationProfilesInput, options: CallOptions) !ListAuthenticationProfilesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAuthenticationProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/authentication-profiles-summary/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAuthenticationProfilesOutput {
    var result: ListAuthenticationProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAuthenticationProfilesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
