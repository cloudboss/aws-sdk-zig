...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAuthenticationProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/authentication-profiles-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAuthenticationProfilesOutput {
    var result: ListAuthenticationProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAuthenticationProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
