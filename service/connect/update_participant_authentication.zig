...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateParticipantAuthenticationInput, options: Options) !UpdateParticipantAuthenticationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateParticipantAuthenticationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.error_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ErrorDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateParticipantAuthenticationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
