...
const ViewInputContent = @import("view_input_content.zig").ViewInputContent;
const ViewStatus = @import("view_status.zig").ViewStatus;
const View = @import("view.zig").View;

pub const CreateViewInput = struct {
    /// A unique Id for each create view request to avoid duplicate view creation.
    /// For example, the view is idempotent
    ...
    /// version).These tags can be used to organize,
    /// track, or control access for this resource. For example, { "tags":
...
pub const CreateViewOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateViewInput, options: Options) !CreateViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateViewInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/views/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateViewOutput {
    var result: CreateViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateViewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
