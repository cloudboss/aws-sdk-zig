...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDescribeDataTableValueInput, options: Options) !BatchDescribeDataTableValueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDescribeDataTableValueInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/values/describe");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDescribeDataTableValueOutput {
    var result: BatchDescribeDataTableValueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDescribeDataTableValueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
