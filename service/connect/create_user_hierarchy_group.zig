...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUserHierarchyGroupInput, options: CallOptions) !CreateUserHierarchyGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateUserHierarchyGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateUserHierarchyGroupOutput {
    var result: CreateUserHierarchyGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateUserHierarchyGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
