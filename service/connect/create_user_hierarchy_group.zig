...
pub const CreateUserHierarchyGroupInput = struct {
    ...
    /// at level one if the parent group ID
    /// is null.
...
pub const CreateUserHierarchyGroupOutput = struct {
    /// The Amazon Resource Name (ARN) of the hierarchy group.
    hierarchy_group_arn: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUserHierarchyGroupInput, options: Options) !CreateUserHierarchyGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUserHierarchyGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.parent_group_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ParentGroupId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUserHierarchyGroupOutput {
    var result: CreateUserHierarchyGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateUserHierarchyGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
