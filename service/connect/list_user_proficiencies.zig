...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUserProficienciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/proficiencies");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUserProficienciesOutput {
    var result: ListUserProficienciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUserProficienciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
