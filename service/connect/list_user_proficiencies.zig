...
const UserProficiency = @import("user_proficiency.zig").UserProficiency;

pub const ListUserProficienciesInput = struct {
...
pub const ListUserProficienciesOutput = struct {
    ...
    user_proficiency_list: ?[]const UserProficiency = null,

    pub const json_field_names = .{
        .last_modified_region = "LastModifiedRegion",
        .last_modified_time = "LastModifiedTime",
        .next_token = "NextToken",
        .user_proficiency_list = "UserProficiencyList",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUserProficienciesInput, options: CallOptions) !ListUserProficienciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUserProficienciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/proficiencies");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUserProficienciesOutput {
    var result: ListUserProficienciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUserProficienciesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
