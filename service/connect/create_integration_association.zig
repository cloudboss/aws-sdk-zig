...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIntegrationAssociationInput, options: Options) !CreateIntegrationAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIntegrationAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"IntegrationArn\":");
    try aws.json.writeValue(@TypeOf(input.integration_arn), input.integration_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"IntegrationType\":");
    try aws.json.writeValue(@TypeOf(input.integration_type), input.integration_type, alloc, &body_buf);
    has_prev = true;
    if (input.source_application_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceApplicationName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_application_url) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceApplicationUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIntegrationAssociationOutput {
    var result: CreateIntegrationAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIntegrationAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
