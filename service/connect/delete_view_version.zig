...
pub const DeleteViewVersionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteViewVersionInput, options: Options) !DeleteViewVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteViewVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.view_version);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteViewVersionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteViewVersionOutput = .{};
