...
pub const GetContactAttributesInput = struct {
...
pub const GetContactAttributesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContactAttributesInput, options: Options) !GetContactAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContactAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact/attributes/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.initial_contact_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContactAttributesOutput {
    var result: GetContactAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContactAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
