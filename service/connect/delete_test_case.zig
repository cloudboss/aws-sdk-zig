...
pub const DeleteTestCaseInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTestCaseInput, options: Options) !DeleteTestCaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTestCaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTestCaseOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteTestCaseOutput = .{};
