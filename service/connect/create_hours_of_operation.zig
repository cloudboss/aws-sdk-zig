...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHoursOfOperationInput, options: Options) !CreateHoursOfOperationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHoursOfOperationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHoursOfOperationOutput {
    var result: CreateHoursOfOperationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateHoursOfOperationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
