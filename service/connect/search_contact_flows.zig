...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchContactFlowsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchContactFlowsOutput {
    var result: SearchContactFlowsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchContactFlowsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
