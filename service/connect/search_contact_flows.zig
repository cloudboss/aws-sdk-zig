...
const ContactFlowSearchCriteria = @import("contact_flow_search_criteria.zig").ContactFlowSearchCriteria;
const ContactFlowSearchFilter = @import("contact_flow_search_filter.zig").ContactFlowSearchFilter;
const ContactFlow = @import("contact_flow.zig").ContactFlow;

pub const SearchContactFlowsInput = struct {
    ...
    /// invalid results.
    search_criteria: ?ContactFlowSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?ContactFlowSearchFilter = null,
...
pub const SearchContactFlowsOutput = struct {
    ...
    pub const json_field_names = .{
        ...
        .contact_flows = "ContactFlows",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchContactFlowsInput, options: CallOptions) !SearchContactFlowsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchContactFlowsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-contact-flows";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchContactFlowsOutput {
    var result: SearchContactFlowsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchContactFlowsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
