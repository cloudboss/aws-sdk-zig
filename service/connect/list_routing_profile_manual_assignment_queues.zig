...
const RoutingProfileManualAssignmentQueueConfigSummary = @import("routing_profile_manual_assignment_queue_config_summary.zig").RoutingProfileManualAssignmentQueueConfigSummary;

pub const ListRoutingProfileManualAssignmentQueuesInput = struct {
...
pub const ListRoutingProfileManualAssignmentQueuesOutput = struct {
    ...
    /// profile.
    routing_profile_manual_assignment_queue_config_summary_list: ?[]const RoutingProfileManualAssignmentQueueConfigSummary = null,

    pub const json_field_names = .{
        .last_modified_region = "LastModifiedRegion",
        .last_modified_time = "LastModifiedTime",
        .next_token = "NextToken",
        .routing_profile_manual_assignment_queue_config_summary_list = "RoutingProfileManualAssignmentQueueConfigSummaryList",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoutingProfileManualAssignmentQueuesInput, options: CallOptions) !ListRoutingProfileManualAssignmentQueuesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRoutingProfileManualAssignmentQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/manual-assignment-queues");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRoutingProfileManualAssignmentQueuesOutput {
    var result: ListRoutingProfileManualAssignmentQueuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoutingProfileManualAssignmentQueuesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
