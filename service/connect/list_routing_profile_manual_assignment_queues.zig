...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoutingProfileManualAssignmentQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/manual-assignment-queues");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoutingProfileManualAssignmentQueuesOutput {
    var result: ListRoutingProfileManualAssignmentQueuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoutingProfileManualAssignmentQueuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
