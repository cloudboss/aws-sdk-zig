...
pub const UpdateContactFlowMetadataInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContactFlowMetadataInput, options: Options) !UpdateContactFlowMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateContactFlowMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.contact_flow_state) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ContactFlowState\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateContactFlowMetadataOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateContactFlowMetadataOutput = .{};
