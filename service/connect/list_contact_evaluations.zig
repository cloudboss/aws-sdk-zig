...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContactEvaluationsInput, options: CallOptions) !ListContactEvaluationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListContactEvaluationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "contactId=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.contact_id);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListContactEvaluationsOutput {
    var result: ListContactEvaluationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactEvaluationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
