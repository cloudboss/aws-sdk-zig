...
pub const ListContactEvaluationsInput = struct {
...
pub const ListContactEvaluationsOutput = struct {
    /// Provides details about a list of contact evaluations belonging to an
    /// instance.
    evaluation_summary_list: ?[]const EvaluationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContactEvaluationsInput, options: Options) !ListContactEvaluationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContactEvaluationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact-evaluations/");
    ...
    try query_buf.appendSlice(alloc, "contactId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.contact_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContactEvaluationsOutput {
    var result: ListContactEvaluationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactEvaluationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
