...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTestCaseInput, options: Options) !CreateTestCaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTestCaseInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.entry_point) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EntryPoint\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.initialization_data) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InitializationData\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.last_modified_region) |v| {
        try request.headers.put(alloc, "x-amz-last-modified-region", v);
    }
    if (input.last_modified_time) |v| {
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try request.headers.put(alloc, "x-amz-last-modified-time", num_str);
        }
    }
    if (input.test_case_id) |v| {
        try request.headers.put(alloc, "x-amz-resource-id", v);
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTestCaseOutput {
    var result: CreateTestCaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTestCaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
