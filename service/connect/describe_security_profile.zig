...
const SecurityProfile = @import("security_profile.zig").SecurityProfile;
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSecurityProfileInput, options: Options) !DescribeSecurityProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSecurityProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSecurityProfileOutput {
    var result: DescribeSecurityProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSecurityProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
