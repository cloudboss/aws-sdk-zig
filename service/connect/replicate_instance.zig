...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReplicateInstanceInput, options: Options) !ReplicateInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReplicateInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/replicate");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReplicaAlias\":");
    try aws.json.writeValue(@TypeOf(input.replica_alias), input.replica_alias, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReplicaRegion\":");
    try aws.json.writeValue(@TypeOf(input.replica_region), input.replica_region, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReplicateInstanceOutput {
    var result: ReplicateInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ReplicateInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
