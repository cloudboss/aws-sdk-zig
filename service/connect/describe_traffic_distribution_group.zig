...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrafficDistributionGroupInput, options: Options) !DescribeTrafficDistributionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrafficDistributionGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrafficDistributionGroupOutput {
    var result: DescribeTrafficDistributionGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTrafficDistributionGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
