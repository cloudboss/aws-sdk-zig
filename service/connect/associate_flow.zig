...
pub const AssociateFlowInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateFlowInput, options: Options) !AssociateFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"FlowId\":");
    try aws.json.writeValue(@TypeOf(input.flow_id), input.flow_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateFlowOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateFlowOutput = .{};
