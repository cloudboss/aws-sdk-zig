...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateApprovedOriginInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/approved-origin");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.origin);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateApprovedOriginOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
