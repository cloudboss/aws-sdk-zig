...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateAnalyticsDataSetInput, options: Options) !AssociateAnalyticsDataSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateAnalyticsDataSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateAnalyticsDataSetOutput {
    var result: AssociateAnalyticsDataSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateAnalyticsDataSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
