...
pub const StartScreenSharingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartScreenSharingInput, options: Options) !StartScreenSharingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartScreenSharingInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/contact/screen-sharing";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartScreenSharingOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: StartScreenSharingOutput = .{};
