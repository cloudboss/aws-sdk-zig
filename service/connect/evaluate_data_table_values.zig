...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EvaluateDataTableValuesInput, options: Options) !EvaluateDataTableValuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EvaluateDataTableValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/values/evaluate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EvaluateDataTableValuesOutput {
    var result: EvaluateDataTableValuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EvaluateDataTableValuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
