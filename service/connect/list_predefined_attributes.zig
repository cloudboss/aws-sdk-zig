...
pub const ListPredefinedAttributesOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// Summary of the predefined attributes.
    predefined_attribute_summary_list: ?[]const PredefinedAttributeSummary = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .predefined_attribute_summary_list = "PredefinedAttributeSummaryList",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPredefinedAttributesInput, options: Options) !ListPredefinedAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPredefinedAttributesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPredefinedAttributesOutput {
    var result: ListPredefinedAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPredefinedAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
