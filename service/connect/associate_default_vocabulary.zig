...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateDefaultVocabularyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/default-vocabulary/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.language_code);
    ...
    if (input.vocabulary_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"VocabularyId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateDefaultVocabularyOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
