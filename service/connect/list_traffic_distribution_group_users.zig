...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrafficDistributionGroupUsersInput, options: Options) !ListTrafficDistributionGroupUsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrafficDistributionGroupUsersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrafficDistributionGroupUsersOutput {
    var result: ListTrafficDistributionGroupUsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTrafficDistributionGroupUsersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
