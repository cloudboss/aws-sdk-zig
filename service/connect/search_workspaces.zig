...
const WorkspaceSearchCriteria = @import("workspace_search_criteria.zig").WorkspaceSearchCriteria;
const WorkspaceSearchFilter = @import("workspace_search_filter.zig").WorkspaceSearchFilter;
const WorkspaceSearchSummary = @import("workspace_search_summary.zig").WorkspaceSearchSummary;

pub const SearchWorkspacesInput = struct {
    ...
    search_criteria: ?WorkspaceSearchCriteria = null,

    /// Filters to apply to the search, such as tag-based filters.
    search_filter: ?WorkspaceSearchFilter = null,
...
pub const SearchWorkspacesOutput = struct {
    ...
    workspaces: ?[]const WorkspaceSearchSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchWorkspacesInput, options: Options) !SearchWorkspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchWorkspacesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchWorkspacesOutput {
    var result: SearchWorkspacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchWorkspacesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
