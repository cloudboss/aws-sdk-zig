...
pub const DescribePromptInput = struct {
...
pub const DescribePromptOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePromptInput, options: CallOptions) !DescribePromptOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribePromptInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribePromptOutput {
    var result: DescribePromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePromptOutput, body, allocator);
    }
    _ = status;
    _ = headers;
