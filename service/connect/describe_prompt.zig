...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePromptInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePromptOutput {
    var result: DescribePromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePromptOutput, body, alloc);
    }
    _ = status;
    _ = headers;
