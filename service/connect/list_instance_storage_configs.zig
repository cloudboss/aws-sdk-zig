...
pub const ListInstanceStorageConfigsInput = struct {
...
pub const ListInstanceStorageConfigsOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// A valid storage type.
    storage_configs: ?[]const InstanceStorageConfig = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInstanceStorageConfigsInput, options: CallOptions) !ListInstanceStorageConfigsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListInstanceStorageConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/storage-configs");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListInstanceStorageConfigsOutput {
    var result: ListInstanceStorageConfigsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInstanceStorageConfigsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
