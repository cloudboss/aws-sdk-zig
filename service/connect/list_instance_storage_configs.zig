...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstanceStorageConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/storage-configs");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstanceStorageConfigsOutput {
    var result: ListInstanceStorageConfigsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInstanceStorageConfigsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
