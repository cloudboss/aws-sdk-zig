...
const LexVersion = @import("lex_version.zig").LexVersion;
const LexBotConfig = @import("lex_bot_config.zig").LexBotConfig;
...
pub const ListBotsOutput = struct {
    ...
    /// bots associated with the specified
    /// instance.
    lex_bots: ?[]const LexBotConfig = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "lexVersion=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.lex_version));
    query_has_prev = true;
