...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataTablePrimaryValuesInput, options: Options) !ListDataTablePrimaryValuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataTablePrimaryValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/values/list-primary");
    ...
    if (input.primary_attribute_values) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"PrimaryAttributeValues\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.record_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecordIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataTablePrimaryValuesOutput {
    var result: ListDataTablePrimaryValuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataTablePrimaryValuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
