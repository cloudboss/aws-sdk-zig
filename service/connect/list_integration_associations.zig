...
const IntegrationAssociationSummary = @import("integration_association_summary.zig").IntegrationAssociationSummary;

pub const ListIntegrationAssociationsInput = struct {
...
pub const ListIntegrationAssociationsOutput = struct {
    /// The associations.
    integration_association_summary_list: ?[]const IntegrationAssociationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIntegrationAssociationsInput, options: Options) !ListIntegrationAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIntegrationAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/instance/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/integration-associations");
    ...
    if (input.integration_arn) |v| {
        ...
        try query_buf.appendSlice(alloc, "integrationArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.integration_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "integrationType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIntegrationAssociationsOutput {
    var result: ListIntegrationAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIntegrationAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
