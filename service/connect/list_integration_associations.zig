...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIntegrationAssociationsInput, options: CallOptions) !ListIntegrationAssociationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListIntegrationAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.integration_arn) |v| {
        ...
        try query_buf.appendSlice(allocator, "integrationArn=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
    if (input.integration_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "integrationType=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListIntegrationAssociationsOutput {
    var result: ListIntegrationAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIntegrationAssociationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
