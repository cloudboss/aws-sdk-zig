...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateViewVersionInput, options: Options) !CreateViewVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateViewVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.view_content_sha_256) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ViewContentSha256\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateViewVersionOutput {
    var result: CreateViewVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateViewVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
