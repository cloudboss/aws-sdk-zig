...
const EmailAddressSearchCriteria = @import("email_address_search_criteria.zig").EmailAddressSearchCriteria;
const EmailAddressSearchFilter = @import("email_address_search_filter.zig").EmailAddressSearchFilter;
...
pub const SearchEmailAddressesInput = struct {
    ...
    search_criteria: ?EmailAddressSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?EmailAddressSearchFilter = null,
...
pub const SearchEmailAddressesOutput = struct {
    ...
    email_addresses: ?[]const EmailAddressMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchEmailAddressesInput, options: Options) !SearchEmailAddressesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchEmailAddressesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-email-addresses";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchEmailAddressesOutput {
    var result: SearchEmailAddressesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchEmailAddressesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
