...
pub const ListSecurityProfileFlowModulesInput = struct {
...
pub const ListSecurityProfileFlowModulesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecurityProfileFlowModulesInput, options: CallOptions) !ListSecurityProfileFlowModulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSecurityProfileFlowModulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/security-profiles-flow-modules/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSecurityProfileFlowModulesOutput {
    var result: ListSecurityProfileFlowModulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityProfileFlowModulesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
