...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityProfileFlowModulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/security-profiles-flow-modules/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecurityProfileFlowModulesOutput {
    var result: ListSecurityProfileFlowModulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityProfileFlowModulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
