...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetricDataV2Input, config: *aws.Config) !aws.http.Request {
    ...
    if (input.interval) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Interval\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetricDataV2Output {
    var result: GetMetricDataV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMetricDataV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
