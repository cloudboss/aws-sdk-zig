...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHoursOfOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/hours-of-operations-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHoursOfOperationsOutput {
    var result: ListHoursOfOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListHoursOfOperationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
