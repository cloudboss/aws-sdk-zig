...
const HoursOfOperationSummary = @import("hours_of_operation_summary.zig").HoursOfOperationSummary;

pub const ListHoursOfOperationsInput = struct {
...
pub const ListHoursOfOperationsOutput = struct {
    /// Information about the hours of operation.
    hours_of_operation_summary_list: ?[]const HoursOfOperationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHoursOfOperationsInput, options: CallOptions) !ListHoursOfOperationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListHoursOfOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/hours-of-operations-summary/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListHoursOfOperationsOutput {
    var result: ListHoursOfOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListHoursOfOperationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
