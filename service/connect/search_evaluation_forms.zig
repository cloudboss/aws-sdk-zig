...
const EvaluationFormSearchCriteria = @import("evaluation_form_search_criteria.zig").EvaluationFormSearchCriteria;
const EvaluationFormSearchFilter = @import("evaluation_form_search_filter.zig").EvaluationFormSearchFilter;
...
pub const SearchEvaluationFormsInput = struct {
    ...
    search_criteria: ?EvaluationFormSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?EvaluationFormSearchFilter = null,
...
pub const SearchEvaluationFormsOutput = struct {
    ...
    evaluation_form_search_summary_list: ?[]const EvaluationFormSearchSummary = null,
    ...
    pub const json_field_names = .{
        ...
        .evaluation_form_search_summary_list = "EvaluationFormSearchSummaryList",
        .next_token = "NextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchEvaluationFormsInput, options: Options) !SearchEvaluationFormsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchEvaluationFormsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-evaluation-forms";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchEvaluationFormsOutput {
    var result: SearchEvaluationFormsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchEvaluationFormsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
