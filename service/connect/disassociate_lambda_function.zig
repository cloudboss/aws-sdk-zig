...
pub const DisassociateLambdaFunctionInput = struct {
...
const DisassociateLambdaFunctionOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateLambdaFunctionInput, options: Options) !DisassociateLambdaFunctionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateLambdaFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/lambda-function");
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "functionArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.function_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateLambdaFunctionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateLambdaFunctionOutput = .{};
