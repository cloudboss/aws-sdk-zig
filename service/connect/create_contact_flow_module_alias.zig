...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContactFlowModuleAliasInput, options: Options) !CreateContactFlowModuleAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContactFlowModuleAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.alias_name), input.alias_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ContactFlowModuleVersion\":");
    try aws.json.writeValue(@TypeOf(input.contact_flow_module_version), input.contact_flow_module_version, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContactFlowModuleAliasOutput {
    var result: CreateContactFlowModuleAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateContactFlowModuleAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
