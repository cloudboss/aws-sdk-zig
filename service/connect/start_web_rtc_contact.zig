...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartWebRTCContactInput, options: Options) !StartWebRTCContactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartWebRTCContactInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allowed_capabilities) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AllowedCapabilities\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartWebRTCContactOutput {
    var result: StartWebRTCContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartWebRTCContactOutput, body, alloc);
    }
    _ = status;
    _ = headers;
