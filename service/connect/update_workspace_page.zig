...
pub const UpdateWorkspacePageInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkspacePageInput, options: Options) !UpdateWorkspacePageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspacePageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.page);
    ...
    if (input.new_page) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"NewPage\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspacePageOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateWorkspacePageOutput = .{};
