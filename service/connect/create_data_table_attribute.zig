...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataTableAttributeInput, options: Options) !CreateDataTableAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataTableAttributeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataTableAttributeOutput {
    var result: CreateDataTableAttributeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataTableAttributeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
