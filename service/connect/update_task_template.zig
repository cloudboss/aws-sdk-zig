...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTaskTemplateInput, options: Options) !UpdateTaskTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTaskTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/task/template/");
    try path_buf.appendSlice(alloc, input.task_template_id);
    ...
    if (input.constraints) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"Constraints\":");
    ...
    if (input.defaults) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Defaults\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.self_assign_flow_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SelfAssignFlowId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.status) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTaskTemplateOutput {
    var result: UpdateTaskTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTaskTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
