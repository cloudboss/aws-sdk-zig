...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTaskTemplatesInput, options: Options) !ListTaskTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTaskTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTaskTemplatesOutput {
    var result: ListTaskTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTaskTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
