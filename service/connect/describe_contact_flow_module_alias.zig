...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContactFlowModuleAliasInput, options: Options) !DescribeContactFlowModuleAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeContactFlowModuleAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeContactFlowModuleAliasOutput {
    var result: DescribeContactFlowModuleAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeContactFlowModuleAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
