...
pub const TransferContactInput = struct {
...
pub const TransferContactOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TransferContactInput, options: Options) !TransferContactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TransferContactInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.queue_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"QueueId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.user_id) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TransferContactOutput {
    var result: TransferContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TransferContactOutput, body, alloc);
    }
    _ = status;
    _ = headers;
