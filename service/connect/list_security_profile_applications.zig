...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityProfileApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/security-profiles-applications/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecurityProfileApplicationsOutput {
    var result: ListSecurityProfileApplicationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityProfileApplicationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
