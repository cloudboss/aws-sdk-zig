...
pub const CreateQuickConnectInput = struct {
...
pub const CreateQuickConnectOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateQuickConnectInput, options: Options) !CreateQuickConnectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateQuickConnectInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateQuickConnectOutput {
    var result: CreateQuickConnectOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateQuickConnectOutput, body, alloc);
    }
    _ = status;
    _ = headers;
