...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateQuickConnectInput, options: CallOptions) !CreateQuickConnectOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateQuickConnectInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"QuickConnectConfig\":");
    try aws.json.writeValue(@TypeOf(input.quick_connect_config), input.quick_connect_config, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateQuickConnectOutput {
    var result: CreateQuickConnectOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateQuickConnectOutput, body, allocator);
    }
    _ = status;
    _ = headers;
