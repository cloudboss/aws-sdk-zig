...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCurrentUserDataInput, options: Options) !GetCurrentUserDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCurrentUserDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/metrics/userdata/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCurrentUserDataOutput {
    var result: GetCurrentUserDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCurrentUserDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
