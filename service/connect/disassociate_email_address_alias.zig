...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateEmailAddressAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/disassociate-alias");
    ...
    try body_buf.appendSlice(allocator, "\"AliasConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.alias_configuration), input.alias_configuration, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateEmailAddressAliasOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
