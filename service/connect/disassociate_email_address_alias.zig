...
pub const DisassociateEmailAddressAliasInput = struct {
    /// Configuration object that specifies which alias relationship to remove. The
    /// alias association must currently
    /// exist between the primary email address and the specified alias email
    /// address.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateEmailAddressAliasInput, options: Options) !DisassociateEmailAddressAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateEmailAddressAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/email-addresses/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.email_address_id);
    try path_buf.appendSlice(alloc, "/disassociate-alias");
    ...
    try body_buf.appendSlice(alloc, "\"AliasConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.alias_configuration), input.alias_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateEmailAddressAliasOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateEmailAddressAliasOutput = .{};
