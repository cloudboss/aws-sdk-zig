...
pub const ListWorkspacesOutput = struct {
    ...
    workspace_summary_list: ?[]const WorkspaceSummary = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .workspace_summary_list = "WorkspaceSummaryList",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkspacesInput, options: Options) !ListWorkspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkspacesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkspacesOutput {
    var result: ListWorkspacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkspacesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
