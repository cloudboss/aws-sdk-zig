...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAgentStatusesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.agent_status_types) |v| {
        ...
        try query_buf.appendSlice(alloc, "AgentStatusTypes=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAgentStatusesOutput {
    var result: ListAgentStatusesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentStatusesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
