...
const AgentStatusSummary = @import("agent_status_summary.zig").AgentStatusSummary;

pub const ListAgentStatusesInput = struct {
    /// Available agent status types.
    agent_status_types: ?[]const AgentStatusType = null,
...
pub const ListAgentStatusesOutput = struct {
    /// A summary of agent statuses.
    agent_status_summary_list: ?[]const AgentStatusSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAgentStatusesInput, options: CallOptions) !ListAgentStatusesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAgentStatusesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.agent_status_types) |v| {
        for (v) |item| {
            if (query_has_prev) try query_buf.appendSlice(allocator, "&");
            try query_buf.appendSlice(allocator, "AgentStatusTypes=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAgentStatusesOutput {
    var result: ListAgentStatusesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentStatusesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
