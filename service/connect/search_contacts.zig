...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchContactsInput, options: Options) !SearchContactsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchContactsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchContactsOutput {
    var result: SearchContactsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchContactsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
