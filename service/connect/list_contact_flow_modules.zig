...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContactFlowModulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact-flow-modules-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContactFlowModulesOutput {
    var result: ListContactFlowModulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactFlowModulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
