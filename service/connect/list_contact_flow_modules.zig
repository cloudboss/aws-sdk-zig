...
const ContactFlowModuleSummary = @import("contact_flow_module_summary.zig").ContactFlowModuleSummary;

pub const ListContactFlowModulesInput = struct {
    /// The state of the flow module.
    contact_flow_module_state: ?ContactFlowModuleState = null,
...
pub const ListContactFlowModulesOutput = struct {
    /// Information about the flow module.
    contact_flow_modules_summary_list: ?[]const ContactFlowModuleSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContactFlowModulesInput, options: CallOptions) !ListContactFlowModulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListContactFlowModulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/contact-flow-modules-summary/");
    try path_buf.appendSlice(allocator, input.instance_id);
    const path = try path_buf.toOwnedSlice(allocator);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.contact_flow_module_state) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListContactFlowModulesOutput {
    var result: ListContactFlowModulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactFlowModulesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
