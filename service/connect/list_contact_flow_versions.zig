...
const ContactFlowVersionSummary = @import("contact_flow_version_summary.zig").ContactFlowVersionSummary;

pub const ListContactFlowVersionsInput = struct {
...
pub const ListContactFlowVersionsOutput = struct {
    /// A list of flow version summaries.
    contact_flow_version_summary_list: ?[]const ContactFlowVersionSummary = null,
    ...
    pub const json_field_names = .{
        .contact_flow_version_summary_list = "ContactFlowVersionSummaryList",
        .next_token = "NextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContactFlowVersionsInput, options: Options) !ListContactFlowVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContactFlowVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContactFlowVersionsOutput {
    var result: ListContactFlowVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactFlowVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
