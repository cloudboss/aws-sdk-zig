...
pub const UntagContactInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UntagContactInput, options: Options) !UntagContactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UntagContactInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/contact/tags/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UntagContactOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UntagContactOutput = .{};
