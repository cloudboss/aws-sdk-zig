...
const DataTableSummary = @import("data_table_summary.zig").DataTableSummary;

pub const ListDataTablesInput = struct {
    /// The unique identifier for the Amazon Connect instance whose data tables
    /// should be listed.
...
pub const ListDataTablesOutput = struct {
    /// A list of data table summaries containing basic information about each table
    /// including ID, ARN, name, and
    /// modification details.
    data_table_summary_list: ?[]const DataTableSummary = null,
    ...
    pub const json_field_names = .{
        .data_table_summary_list = "DataTableSummaryList",
        .next_token = "NextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataTablesInput, options: Options) !ListDataTablesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataTablesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataTablesOutput {
    var result: ListDataTablesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataTablesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
