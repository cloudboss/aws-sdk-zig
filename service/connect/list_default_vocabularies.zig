...
pub const ListDefaultVocabulariesOutput = struct {
    /// A list of default vocabularies.
    default_vocabulary_list: ?[]const DefaultVocabulary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDefaultVocabulariesInput, options: Options) !ListDefaultVocabulariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDefaultVocabulariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/default-vocabulary-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDefaultVocabulariesOutput {
    var result: ListDefaultVocabulariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDefaultVocabulariesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
