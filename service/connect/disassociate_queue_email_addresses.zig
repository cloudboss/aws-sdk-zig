...
pub const DisassociateQueueEmailAddressesInput = struct {
    ...
    /// List of email address identifiers to disassociate from the queue. These are
    /// the unique identifiers of email addresses that should no longer be routed to
    /// this queue.
    email_addresses_id: []const []const u8,
    ...
    pub const json_field_names = .{
        ...
        .email_addresses_id = "EmailAddressesId",
        .instance_id = "InstanceId",
        .queue_id = "QueueId",
    };
};

pub const DisassociateQueueEmailAddressesOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateQueueEmailAddressesInput, options: CallOptions) !DisassociateQueueEmailAddressesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateQueueEmailAddressesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/disassociate-email-addresses");
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"EmailAddressesId\":");
    try aws.json.writeValue(@TypeOf(input.email_addresses_id), input.email_addresses_id, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateQueueEmailAddressesOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateQueueEmailAddressesOutput = .{};
