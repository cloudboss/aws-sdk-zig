...
const AttachedFilesConfigurationSummary = @import("attached_files_configuration_summary.zig").AttachedFilesConfigurationSummary;

pub const ListAttachedFilesConfigurationsInput = struct {
...
pub const ListAttachedFilesConfigurationsOutput = struct {
    /// Information about the attached files configurations.
    attached_files_configurations: ?[]const AttachedFilesConfigurationSummary = null,
    ...
    pub const json_field_names = .{
        .attached_files_configurations = "AttachedFilesConfigurations",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAttachedFilesConfigurationsInput, options: CallOptions) !ListAttachedFilesConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAttachedFilesConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAttachedFilesConfigurationsOutput {
    var result: ListAttachedFilesConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAttachedFilesConfigurationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
