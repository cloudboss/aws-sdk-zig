...
pub const CreateContactFlowModuleInput = struct {
...
pub const CreateContactFlowModuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContactFlowModuleInput, options: Options) !CreateContactFlowModuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContactFlowModuleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.external_invocation_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ExternalInvocationConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContactFlowModuleOutput {
    var result: CreateContactFlowModuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateContactFlowModuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
