...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContactFlowModuleInput, options: CallOptions) !CreateContactFlowModuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateContactFlowModuleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.external_invocation_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ExternalInvocationConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
