...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SubmitContactEvaluationInput, options: Options) !SubmitContactEvaluationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SubmitContactEvaluationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.evaluation_id);
    try path_buf.appendSlice(alloc, "/submit");
    ...
    if (input.submitted_by) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SubmittedBy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SubmitContactEvaluationOutput {
    var result: SubmitContactEvaluationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SubmitContactEvaluationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
