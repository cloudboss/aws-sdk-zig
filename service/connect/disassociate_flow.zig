...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateFlowInput, options: Options) !DisassociateFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/flow-associations/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateFlowOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
