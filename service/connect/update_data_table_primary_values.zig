...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDataTablePrimaryValuesInput, options: Options) !UpdateDataTablePrimaryValuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataTablePrimaryValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/values/update-primary");
    ...
    try body_buf.appendSlice(alloc, "\"LockVersion\":");
    try aws.json.writeValue(@TypeOf(input.lock_version), input.lock_version, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"NewPrimaryValues\":");
    try aws.json.writeValue(@TypeOf(input.new_primary_values), input.new_primary_values, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PrimaryValues\":");
    try aws.json.writeValue(@TypeOf(input.primary_values), input.primary_values, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataTablePrimaryValuesOutput {
    var result: UpdateDataTablePrimaryValuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDataTablePrimaryValuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
