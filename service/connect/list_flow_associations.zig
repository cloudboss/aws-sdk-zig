...
pub const ListFlowAssociationsInput = struct {
...
pub const ListFlowAssociationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFlowAssociationsInput, options: CallOptions) !ListFlowAssociationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFlowAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/flow-associations-summary/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFlowAssociationsOutput {
    var result: ListFlowAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFlowAssociationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
