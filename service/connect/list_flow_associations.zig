...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFlowAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/flow-associations-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFlowAssociationsOutput {
    var result: ListFlowAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFlowAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
