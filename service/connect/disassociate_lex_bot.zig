...
pub const DisassociateLexBotInput = struct {
...
const DisassociateLexBotOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateLexBotInput, options: Options) !DisassociateLexBotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateLexBotInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/lex-bot");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "botName=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.bot_name);
    query_has_prev = true;
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "lexRegion=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.lex_region);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateLexBotOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateLexBotOutput = .{};
