...
pub const DisassociateLexBotOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateLexBotInput, options: CallOptions) !DisassociateLexBotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateLexBotInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/lex-bot");
    const path = try path_buf.toOwnedSlice(allocator);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "botName=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.bot_name);
    query_has_prev = true;
    ...
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "lexRegion=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.lex_region);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateLexBotOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
