...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePhoneNumberInput, options: Options) !DescribePhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePhoneNumberInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePhoneNumberOutput {
    var result: DescribePhoneNumberOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePhoneNumberOutput, body, alloc);
    }
    _ = status;
    _ = headers;
