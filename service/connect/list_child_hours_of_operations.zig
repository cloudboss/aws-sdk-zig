...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChildHoursOfOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/hours");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChildHoursOfOperationsOutput {
    var result: ListChildHoursOfOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChildHoursOfOperationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
