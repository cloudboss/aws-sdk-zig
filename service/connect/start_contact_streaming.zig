...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartContactStreamingInput, options: CallOptions) !StartContactStreamingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartContactStreamingInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.chat_streaming_configuration), input.chat_streaming_configuration, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartContactStreamingOutput {
    var result: StartContactStreamingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartContactStreamingOutput, body, allocator);
    }
    _ = status;
    _ = headers;
