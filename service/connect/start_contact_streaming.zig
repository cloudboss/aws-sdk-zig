...
pub const StartContactStreamingInput = struct {
...
pub const StartContactStreamingOutput = struct {
    /// The identifier of the streaming configuration enabled.
    streaming_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartContactStreamingInput, options: Options) !StartContactStreamingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartContactStreamingInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/contact/start-streaming";
    ...
    try body_buf.appendSlice(alloc, "\"ChatStreamingConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.chat_streaming_configuration), input.chat_streaming_configuration, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartContactStreamingOutput {
    var result: StartContactStreamingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartContactStreamingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
