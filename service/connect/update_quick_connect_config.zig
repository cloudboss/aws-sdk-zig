...
pub const UpdateQuickConnectConfigInput = struct {
...
const UpdateQuickConnectConfigOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateQuickConnectConfigInput, options: Options) !UpdateQuickConnectConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQuickConnectConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/quick-connects/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.quick_connect_id);
    ...
    try body_buf.appendSlice(alloc, "\"QuickConnectConfig\":");
    try aws.json.writeValue(@TypeOf(input.quick_connect_config), input.quick_connect_config, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQuickConnectConfigOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateQuickConnectConfigOutput = .{};
