...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFederationTokenInput, options: Options) !GetFederationTokenOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFederationTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/user/federate/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFederationTokenOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFederationTokenOutput, body, allocator);
    }
    _ = status;
    _ = headers;
