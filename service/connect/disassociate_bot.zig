...
pub const DisassociateBotInput = struct {
...
const DisassociateBotOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateBotInput, options: Options) !DisassociateBotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateBotInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.lex_bot) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LexBot\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.lex_v2_bot) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LexV2Bot\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateBotOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateBotOutput = .{};
