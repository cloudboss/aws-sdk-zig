...
pub const DisassociateBotOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateBotInput, options: CallOptions) !DisassociateBotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateBotInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.lex_bot) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"LexBot\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.lex_v2_bot) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"LexV2Bot\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateBotOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
