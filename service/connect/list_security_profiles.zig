...
const SecurityProfileSummary = @import("security_profile_summary.zig").SecurityProfileSummary;

pub const ListSecurityProfilesInput = struct {
...
pub const ListSecurityProfilesOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// Information about the security profiles.
    security_profile_summary_list: ?[]const SecurityProfileSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecurityProfilesInput, options: Options) !ListSecurityProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/security-profiles-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecurityProfilesOutput {
    var result: ListSecurityProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
