...
const PromptSearchCriteria = @import("prompt_search_criteria.zig").PromptSearchCriteria;
const PromptSearchFilter = @import("prompt_search_filter.zig").PromptSearchFilter;
const Prompt = @import("prompt.zig").Prompt;

pub const SearchPromptsInput = struct {
    ...
    search_criteria: ?PromptSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?PromptSearchFilter = null,
...
pub const SearchPromptsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchPromptsInput, options: Options) !SearchPromptsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchPromptsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchPromptsOutput {
    var result: SearchPromptsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchPromptsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
