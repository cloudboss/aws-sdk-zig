...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchPromptsInput, options: CallOptions) !SearchPromptsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchPromptsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchPromptsOutput {
    var result: SearchPromptsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchPromptsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
