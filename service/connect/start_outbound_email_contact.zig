...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartOutboundEmailContactInput, options: Options) !StartOutboundEmailContactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartOutboundEmailContactInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.additional_recipients) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AdditionalRecipients\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DestinationEmailAddress\":");
    try aws.json.writeValue(@TypeOf(input.destination_email_address), input.destination_email_address, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EmailMessage\":");
    try aws.json.writeValue(@TypeOf(input.email_message), input.email_message, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartOutboundEmailContactOutput {
    var result: StartOutboundEmailContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartOutboundEmailContactOutput, body, alloc);
    }
    _ = status;
    _ = headers;
