...
const UpdateAgentStatusOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAgentStatusInput, options: Options) !UpdateAgentStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAgentStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/agent-status/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.agent_status_id);
    ...
    if (input.reset_order_number) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ResetOrderNumber\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.state) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAgentStatusOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateAgentStatusOutput = .{};
