...
pub const DescribeAgentStatusInput = struct {
    /// The identifier for the agent status.
    agent_status_id: []const u8,
...
pub const DescribeAgentStatusOutput = struct {
    /// The agent status.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAgentStatusInput, options: CallOptions) !DescribeAgentStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAgentStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/agent-status/");
    try path_buf.appendSlice(allocator, input.instance_id);
    try path_buf.appendSlice(allocator, "/");
    try path_buf.appendSlice(allocator, input.agent_status_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAgentStatusOutput {
    var result: DescribeAgentStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAgentStatusOutput, body, allocator);
    }
    _ = status;
    _ = headers;
