...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAgentStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAgentStatusOutput {
    var result: DescribeAgentStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAgentStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
