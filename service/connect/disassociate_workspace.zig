...
pub const DisassociateWorkspaceInput = struct {
...
pub const DisassociateWorkspaceOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateWorkspaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/disassociate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateWorkspaceOutput {
    var result: DisassociateWorkspaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateWorkspaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
