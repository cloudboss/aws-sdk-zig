...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportWorkspaceMediaInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"MediaSource\":");
    try aws.json.writeValue(@TypeOf(input.media_source), input.media_source, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MediaType\":");
    try aws.json.writeValue(@TypeOf(input.media_type), input.media_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportWorkspaceMediaOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
