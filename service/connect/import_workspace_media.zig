...
pub const ImportWorkspaceMediaInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportWorkspaceMediaInput, options: Options) !ImportWorkspaceMediaOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ImportWorkspaceMediaInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"MediaSource\":");
    try aws.json.writeValue(@TypeOf(input.media_source), input.media_source, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"MediaType\":");
    try aws.json.writeValue(@TypeOf(input.media_type), input.media_type, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ImportWorkspaceMediaOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ImportWorkspaceMediaOutput = .{};
