...
pub const ListHoursOfOperationOverridesInput = struct {
...
pub const ListHoursOfOperationOverridesOutput = struct {
    /// Information about the hours of operation override.
    hours_of_operation_override_list: ?[]const HoursOfOperationOverride = null,
    ...
    pub const json_field_names = .{
        .hours_of_operation_override_list = "HoursOfOperationOverrideList",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHoursOfOperationOverridesInput, options: Options) !ListHoursOfOperationOverridesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHoursOfOperationOverridesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHoursOfOperationOverridesOutput {
    var result: ListHoursOfOperationOverridesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListHoursOfOperationOverridesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
