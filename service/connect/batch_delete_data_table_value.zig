...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteDataTableValueInput, options: Options) !BatchDeleteDataTableValueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteDataTableValueInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/values/delete");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteDataTableValueOutput {
    var result: BatchDeleteDataTableValueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteDataTableValueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
