...
pub const ListLambdaFunctionsInput = struct {
...
pub const ListLambdaFunctionsOutput = struct {
    /// The Lambdafunction ARNs associated with the specified instance.
    lambda_functions: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLambdaFunctionsInput, options: CallOptions) !ListLambdaFunctionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLambdaFunctionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/lambda-functions");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLambdaFunctionsOutput {
    var result: ListLambdaFunctionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLambdaFunctionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
