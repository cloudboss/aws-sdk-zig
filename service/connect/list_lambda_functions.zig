...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLambdaFunctionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/lambda-functions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLambdaFunctionsOutput {
    var result: ListLambdaFunctionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLambdaFunctionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
