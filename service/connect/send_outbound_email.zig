...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendOutboundEmailInput, options: Options) !SendOutboundEmailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendOutboundEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/outbound-email");
    ...
    if (input.source_campaign) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceCampaign\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TrafficType\":");
    try aws.json.writeValue(@TypeOf(input.traffic_type), input.traffic_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendOutboundEmailOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
