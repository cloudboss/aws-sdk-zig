...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Function\":");
    try aws.json.writeValue(@TypeOf(input.function), input.function, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PublishStatus\":");
    try aws.json.writeValue(@TypeOf(input.publish_status), input.publish_status, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TriggerEventSource\":");
    try aws.json.writeValue(@TypeOf(input.trigger_event_source), input.trigger_event_source, alloc, &body_buf);
    has_prev = true;
