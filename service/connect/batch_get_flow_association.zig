...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetFlowAssociationInput, options: Options) !BatchGetFlowAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetFlowAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/flow-associations-batch/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetFlowAssociationOutput {
    var result: BatchGetFlowAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetFlowAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
