...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTrafficDistributionInput, options: Options) !UpdateTrafficDistributionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTrafficDistributionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/traffic-distribution/");
    ...
    if (input.agent_config) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AgentConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.sign_in_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SignInConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.telephony_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TelephonyConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTrafficDistributionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
