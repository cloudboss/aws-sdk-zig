...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspaceVisibilityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/visibility");
    ...
    try aws.json.writeValue(@TypeOf(input.visibility), input.visibility, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspaceVisibilityOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
