...
pub const UpdateWorkspaceVisibilityInput = struct {
    /// The identifier of the Amazon Connect instance. You can [find the instance
    /// ID](https://docs.aws.amazon.com/connect/latest/adminguide/find-instance-arn.html) in
    /// the Amazon Resource Name (ARN) of the instance.
    instance_id: []const u8,

    /// The visibility setting for the workspace. Valid values are: `ALL` (available
    /// to all users),
    /// `ASSIGNED` (available only to assigned users and routing profiles), and
    /// `NONE` (not visible to
    /// any users).
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkspaceVisibilityInput, options: Options) !UpdateWorkspaceVisibilityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateWorkspaceVisibilityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/visibility");
    ...
    try aws.json.writeValue(@TypeOf(input.visibility), input.visibility, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateWorkspaceVisibilityOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateWorkspaceVisibilityOutput = .{};
