...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreateDataTableValueInput, options: Options) !BatchCreateDataTableValueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateDataTableValueInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/values/create");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateDataTableValueOutput {
    var result: BatchCreateDataTableValueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchCreateDataTableValueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
