...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataTableMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataTableMetadataOutput {
    var result: UpdateDataTableMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDataTableMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
