...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTestCaseExecutionSummaryInput, options: Options) !GetTestCaseExecutionSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTestCaseExecutionSummaryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTestCaseExecutionSummaryOutput {
    var result: GetTestCaseExecutionSummaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTestCaseExecutionSummaryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
