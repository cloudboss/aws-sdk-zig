...
pub const UpdateViewContentInput = struct {
...
pub const UpdateViewContentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateViewContentInput, options: Options) !UpdateViewContentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateViewContentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateViewContentOutput {
    var result: UpdateViewContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateViewContentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
