...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstanceAttributesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstanceAttributesOutput {
    var result: ListInstanceAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInstanceAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
