...
pub const ListInstanceAttributesInput = struct {
...
pub const ListInstanceAttributesOutput = struct {
    /// The attribute types.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInstanceAttributesInput, options: CallOptions) !ListInstanceAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListInstanceAttributesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListInstanceAttributesOutput {
    var result: ListInstanceAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInstanceAttributesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
