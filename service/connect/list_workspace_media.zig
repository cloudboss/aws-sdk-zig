...
const MediaItem = @import("media_item.zig").MediaItem;

pub const ListWorkspaceMediaInput = struct {
...
pub const ListWorkspaceMediaOutput = struct {
    /// A list of media assets for the workspace.
    media: ?[]const MediaItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkspaceMediaInput, options: CallOptions) !ListWorkspaceMediaOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListWorkspaceMediaInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListWorkspaceMediaOutput {
    var result: ListWorkspaceMediaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkspaceMediaOutput, body, allocator);
    }
    _ = status;
    _ = headers;
