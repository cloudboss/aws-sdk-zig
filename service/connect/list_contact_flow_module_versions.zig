...
const ContactFlowModuleVersionSummary = @import("contact_flow_module_version_summary.zig").ContactFlowModuleVersionSummary;

pub const ListContactFlowModuleVersionsInput = struct {
...
pub const ListContactFlowModuleVersionsOutput = struct {
    /// Information about the flow module versions.
    contact_flow_module_version_summary_list: ?[]const ContactFlowModuleVersionSummary = null,
    ...
    pub const json_field_names = .{
        .contact_flow_module_version_summary_list = "ContactFlowModuleVersionSummaryList",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContactFlowModuleVersionsInput, options: CallOptions) !ListContactFlowModuleVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListContactFlowModuleVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListContactFlowModuleVersionsOutput {
    var result: ListContactFlowModuleVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactFlowModuleVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
