...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContactFlowModuleVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContactFlowModuleVersionsOutput {
    var result: ListContactFlowModuleVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactFlowModuleVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
