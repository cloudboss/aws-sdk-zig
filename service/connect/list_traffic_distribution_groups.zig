...
const TrafficDistributionGroupSummary = @import("traffic_distribution_group_summary.zig").TrafficDistributionGroupSummary;

pub const ListTrafficDistributionGroupsInput = struct {
...
pub const ListTrafficDistributionGroupsOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// A list of traffic distribution groups.
    traffic_distribution_group_summary_list: ?[]const TrafficDistributionGroupSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrafficDistributionGroupsInput, options: CallOptions) !ListTrafficDistributionGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTrafficDistributionGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/traffic-distribution-groups";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTrafficDistributionGroupsOutput {
    var result: ListTrafficDistributionGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTrafficDistributionGroupsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
