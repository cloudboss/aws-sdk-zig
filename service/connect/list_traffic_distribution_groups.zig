...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrafficDistributionGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrafficDistributionGroupsOutput {
    var result: ListTrafficDistributionGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTrafficDistributionGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
