...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataTableAttributeInput, options: Options) !DescribeDataTableAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataTableAttributeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataTableAttributeOutput {
    var result: DescribeDataTableAttributeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDataTableAttributeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
