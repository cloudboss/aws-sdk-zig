...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataTableInput, options: Options) !DescribeDataTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataTableInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataTableOutput {
    var result: DescribeDataTableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDataTableOutput, body, alloc);
    }
    _ = status;
    _ = headers;
