...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContactRoutingDataInput, options: Options) !UpdateContactRoutingDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContactRoutingDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/routing-data");
    ...
    if (input.queue_priority) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"QueuePriority\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.queue_time_adjustment_seconds) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"QueueTimeAdjustmentSeconds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.routing_criteria) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RoutingCriteria\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContactRoutingDataOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
