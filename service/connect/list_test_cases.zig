...
const TestCaseSummary = @import("test_case_summary.zig").TestCaseSummary;

pub const ListTestCasesInput = struct {
...
pub const ListTestCasesOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// Information about the tests.
    test_case_summary_list: ?[]const TestCaseSummary = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .test_case_summary_list = "TestCaseSummaryList",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTestCasesInput, options: CallOptions) !ListTestCasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTestCasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/test-cases-summary/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTestCasesOutput {
    var result: ListTestCasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTestCasesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
