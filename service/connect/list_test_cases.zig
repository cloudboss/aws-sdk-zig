...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTestCasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/test-cases-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTestCasesOutput {
    var result: ListTestCasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTestCasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
