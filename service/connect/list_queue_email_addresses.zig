...
const EmailAddressSummary = @import("email_address_summary.zig").EmailAddressSummary;

pub const ListQueueEmailAddressesInput = struct {
...
pub const ListQueueEmailAddressesOutput = struct {
    ...
    email_address_metadata_list: ?[]const EmailAddressSummary = null,
    ...
    pub const json_field_names = .{
        .email_address_metadata_list = "EmailAddressMetadataList",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQueueEmailAddressesInput, options: CallOptions) !ListQueueEmailAddressesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListQueueEmailAddressesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListQueueEmailAddressesOutput {
    var result: ListQueueEmailAddressesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQueueEmailAddressesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
