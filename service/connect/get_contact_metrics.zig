...
const ContactMetricInfo = @import("contact_metric_info.zig").ContactMetricInfo;
...
pub const GetContactMetricsInput = struct {
    ...
    /// POSITION_IN_QUEUE (the contact's
    /// current position in the queue) and ESTIMATED_WAIT_TIME (the predicted time
    /// in seconds until the contact is
    /// connected to an agent)
    metrics: []const ContactMetricInfo,
...
pub const GetContactMetricsOutput = struct {
    ...
    /// matches the ContactId provided
    /// in the request.
    id: ?[]const u8 = null,

    /// A list of metric results containing the calculated values for each requested
    /// metric. Each result includes
    /// the metric name and its corresponding value. For example, POSITION_IN_QUEUE
    /// returns a numeric value representing
    /// the contact's position in queue, and ESTIMATED_WAIT_TIME returns the
    /// predicted wait time in seconds.
    metric_results: ?[]const ContactMetricResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContactMetricsInput, options: Options) !GetContactMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContactMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.contact_id), input.contact_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContactMetricsOutput {
    var result: GetContactMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContactMetricsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
