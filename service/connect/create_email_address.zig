...
pub const CreateEmailAddressInput = struct {
...
pub const CreateEmailAddressOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEmailAddressInput, options: Options) !CreateEmailAddressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEmailAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.email_address), input.email_address, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEmailAddressOutput {
    var result: CreateEmailAddressOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEmailAddressOutput, body, alloc);
    }
    _ = status;
    _ = headers;
