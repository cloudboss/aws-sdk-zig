...
pub const StopContactStreamingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopContactStreamingInput, options: Options) !StopContactStreamingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopContactStreamingInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/contact/stop-streaming";
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"StreamingId\":");
    try aws.json.writeValue(@TypeOf(input.streaming_id), input.streaming_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopContactStreamingOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: StopContactStreamingOutput = .{};
