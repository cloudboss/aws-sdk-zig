...
fn serializeRequest(allocator: std.mem.Allocator, input: StopContactStreamingInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"StreamingId\":");
    try aws.json.writeValue(@TypeOf(input.streaming_id), input.streaming_id, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopContactStreamingOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
