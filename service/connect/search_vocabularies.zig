...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchVocabulariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/vocabulary-summary/");
    ...
    if (input.name_starts_with) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"NameStartsWith\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchVocabulariesOutput {
    var result: SearchVocabulariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchVocabulariesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
