...
fn serializeRequest(alloc: std.mem.Allocator, input: MonitorContactInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allowed_monitor_capabilities) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AllowedMonitorCapabilities\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MonitorContactOutput {
    var result: MonitorContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(MonitorContactOutput, body, alloc);
    }
    _ = status;
    _ = headers;
