...
const MonitorCapability = @import("monitor_capability.zig").MonitorCapability;

pub const MonitorContactInput = struct {
    /// Specify which monitoring actions the user is allowed to take. For example,
    /// whether the user is allowed to
    /// escalate from silent monitoring to barge. AllowedMonitorCapabilities is
    /// required if barge is enabled.
    allowed_monitor_capabilities: ?[]const MonitorCapability = null,
...
pub const MonitorContactOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MonitorContactInput, options: CallOptions) !MonitorContactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: MonitorContactInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/contact/monitor";

    var body_buf: std.ArrayList(u8) = .{};
    var has_prev = false;
    try body_buf.appendSlice(allocator, "{");

    if (input.allowed_monitor_capabilities) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"AllowedMonitorCapabilities\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !MonitorContactOutput {
    var result: MonitorContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(MonitorContactOutput, body, allocator);
    }
    _ = status;
    _ = headers;
