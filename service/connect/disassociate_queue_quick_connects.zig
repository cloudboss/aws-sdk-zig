...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateQueueQuickConnectsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/disassociate-quick-connects");
    ...
    try aws.json.writeValue(@TypeOf(input.quick_connect_ids), input.quick_connect_ids, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateQueueQuickConnectsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
