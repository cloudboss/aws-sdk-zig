...
pub const DisassociateQueueQuickConnectsInput = struct {
    ...
    quick_connect_ids: []const []const u8,
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateQueueQuickConnectsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/disassociate-quick-connects");
    ...
    try body_buf.appendSlice(alloc, "\"QuickConnectIds\":");
    try aws.json.writeValue(@TypeOf(input.quick_connect_ids), input.quick_connect_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateQueueQuickConnectsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateQueueQuickConnectsOutput = .{};
