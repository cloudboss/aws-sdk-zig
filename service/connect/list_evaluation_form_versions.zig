...
const EvaluationFormVersionSummary = @import("evaluation_form_version_summary.zig").EvaluationFormVersionSummary;

pub const ListEvaluationFormVersionsInput = struct {
...
pub const ListEvaluationFormVersionsOutput = struct {
    /// Provides details about a list of evaluation forms belonging to an instance.
    evaluation_form_version_summary_list: ?[]const EvaluationFormVersionSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEvaluationFormVersionsInput, options: Options) !ListEvaluationFormVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEvaluationFormVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/evaluation-forms/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.evaluation_form_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEvaluationFormVersionsOutput {
    var result: ListEvaluationFormVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEvaluationFormVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
