...
pub const CreateWorkspacePageInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkspacePageInput, options: Options) !CreateWorkspacePageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkspacePageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/pages");
    ...
    if (input.input_data) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InputData\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Page\":");
    try aws.json.writeValue(@TypeOf(input.page), input.page, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.slug) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Slug\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkspacePageOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateWorkspacePageOutput = .{};
