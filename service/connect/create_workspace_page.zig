...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateWorkspacePageInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.input_data) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"InputData\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"Page\":");
    try aws.json.writeValue(@TypeOf(input.page), input.page, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    if (input.slug) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"Slug\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateWorkspacePageOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
