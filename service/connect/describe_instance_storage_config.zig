...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceStorageConfigInput, options: Options) !DescribeInstanceStorageConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceStorageConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/storage-config/");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.resource_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceStorageConfigOutput {
    var result: DescribeInstanceStorageConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeInstanceStorageConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
