...
pub const UpdateContactFlowModuleAliasInput = struct {
    ...
    contact_flow_module_version: ?i64 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContactFlowModuleAliasInput, options: Options) !UpdateContactFlowModuleAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContactFlowModuleAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact-flow-modules/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.contact_flow_module_id);
    try path_buf.appendSlice(alloc, "/alias/");
    try path_buf.appendSlice(alloc, input.alias_id);
    ...
    if (input.contact_flow_module_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ContactFlowModuleVersion\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContactFlowModuleAliasOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateContactFlowModuleAliasOutput = .{};
