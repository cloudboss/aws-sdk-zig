...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchResourceTagsInput, options: Options) !SearchResourceTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchResourceTagsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchResourceTagsOutput {
    var result: SearchResourceTagsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchResourceTagsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
