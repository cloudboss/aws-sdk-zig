...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateEmailAddressAliasInput, options: Options) !AssociateEmailAddressAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateEmailAddressAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/associate-alias");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateEmailAddressAliasOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateEmailAddressAliasOutput = .{};
