...
pub const UpdateUserSecurityProfilesInput = struct {
...
const UpdateUserSecurityProfilesOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserSecurityProfilesInput, options: Options) !UpdateUserSecurityProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserSecurityProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"SecurityProfileIds\":");
    try aws.json.writeValue(@TypeOf(input.security_profile_ids), input.security_profile_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserSecurityProfilesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateUserSecurityProfilesOutput = .{};
