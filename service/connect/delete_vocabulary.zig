...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteVocabularyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/vocabulary-remove/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteVocabularyOutput {
    var result: DeleteVocabularyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteVocabularyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
