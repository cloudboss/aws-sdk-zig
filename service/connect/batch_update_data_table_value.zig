...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateDataTableValueInput, options: CallOptions) !BatchUpdateDataTableValueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchUpdateDataTableValueInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/values/update");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchUpdateDataTableValueOutput {
    var result: BatchUpdateDataTableValueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateDataTableValueOutput, body, allocator);
    }
    _ = status;
    _ = headers;
