...
const BatchUpdateDataTableValueFailureResult = @import("batch_update_data_table_value_failure_result.zig").BatchUpdateDataTableValueFailureResult;
const BatchUpdateDataTableValueSuccessResult = @import("batch_update_data_table_value_success_result.zig").BatchUpdateDataTableValueSuccessResult;

pub const BatchUpdateDataTableValueInput = struct {
...
pub const BatchUpdateDataTableValueOutput = struct {
    /// A list of values that failed to be updated with error messages explaining
    /// the failure reason.
    failed: ?[]const BatchUpdateDataTableValueFailureResult = null,

    /// A list of successfully updated values with their new lock versions and
    /// identifiers.
    successful: ?[]const BatchUpdateDataTableValueSuccessResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateDataTableValueInput, options: Options) !BatchUpdateDataTableValueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateDataTableValueInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/data-tables/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.data_table_id);
    try path_buf.appendSlice(alloc, "/values/update");
    ...
    try aws.json.writeValue(@TypeOf(input.values), input.values, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateDataTableValueOutput {
    var result: BatchUpdateDataTableValueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateDataTableValueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
