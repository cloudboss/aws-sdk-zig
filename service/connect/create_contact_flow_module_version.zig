...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContactFlowModuleVersionInput, options: Options) !CreateContactFlowModuleVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContactFlowModuleVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.flow_module_content_sha_256) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FlowModuleContentSha256\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContactFlowModuleVersionOutput {
    var result: CreateContactFlowModuleVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateContactFlowModuleVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
