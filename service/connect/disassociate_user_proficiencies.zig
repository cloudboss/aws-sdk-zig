...
const DisassociateUserProficienciesOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateUserProficienciesInput, options: Options) !DisassociateUserProficienciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateUserProficienciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/disassociate-proficiencies");
    ...
    try body_buf.appendSlice(alloc, "\"UserProficiencies\":");
    try aws.json.writeValue(@TypeOf(input.user_proficiencies), input.user_proficiencies, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateUserProficienciesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
