...
pub const UpdateUserRoutingProfileOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserRoutingProfileInput, options: CallOptions) !UpdateUserRoutingProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateUserRoutingProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/routing-profile");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateUserRoutingProfileOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
