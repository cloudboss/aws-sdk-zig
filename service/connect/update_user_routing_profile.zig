...
pub const UpdateUserRoutingProfileInput = struct {
...
const UpdateUserRoutingProfileOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserRoutingProfileInput, options: Options) !UpdateUserRoutingProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserRoutingProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/routing-profile");
    ...
    try body_buf.appendSlice(alloc, "\"RoutingProfileId\":");
    try aws.json.writeValue(@TypeOf(input.routing_profile_id), input.routing_profile_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserRoutingProfileOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateUserRoutingProfileOutput = .{};
