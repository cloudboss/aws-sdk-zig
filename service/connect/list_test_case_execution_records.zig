...
pub const ListTestCaseExecutionRecordsInput = struct {
...
pub const ListTestCaseExecutionRecordsOutput = struct {
    /// An array of test case execution record objects.
    execution_records: ?[]const ExecutionRecord = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTestCaseExecutionRecordsInput, options: Options) !ListTestCaseExecutionRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTestCaseExecutionRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.test_case_execution_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTestCaseExecutionRecordsOutput {
    var result: ListTestCaseExecutionRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTestCaseExecutionRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
