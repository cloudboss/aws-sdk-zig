...
pub const UpdateRoutingProfileAgentAvailabilityTimerOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoutingProfileAgentAvailabilityTimerInput, options: CallOptions) !UpdateRoutingProfileAgentAvailabilityTimerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRoutingProfileAgentAvailabilityTimerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/agent-availability-timer");
    ...
    try aws.json.writeValue(@TypeOf(input.agent_availability_timer), input.agent_availability_timer, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRoutingProfileAgentAvailabilityTimerOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
