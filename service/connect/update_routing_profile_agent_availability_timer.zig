...
pub const UpdateRoutingProfileAgentAvailabilityTimerInput = struct {
...
const UpdateRoutingProfileAgentAvailabilityTimerOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoutingProfileAgentAvailabilityTimerInput, options: Options) !UpdateRoutingProfileAgentAvailabilityTimerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoutingProfileAgentAvailabilityTimerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/agent-availability-timer");
    ...
    try body_buf.appendSlice(alloc, "\"AgentAvailabilityTimer\":");
    try aws.json.writeValue(@TypeOf(input.agent_availability_timer), input.agent_availability_timer, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoutingProfileAgentAvailabilityTimerOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateRoutingProfileAgentAvailabilityTimerOutput = .{};
