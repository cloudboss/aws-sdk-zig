...
const UpdatePredefinedAttributeOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePredefinedAttributeInput, options: Options) !UpdatePredefinedAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePredefinedAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.attribute_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AttributeConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.purposes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Purposes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.values) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePredefinedAttributeOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
