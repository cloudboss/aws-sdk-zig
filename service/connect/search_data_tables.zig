...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchDataTablesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchDataTablesOutput {
    var result: SearchDataTablesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchDataTablesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
