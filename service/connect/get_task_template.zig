...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTaskTemplateInput, options: Options) !GetTaskTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTaskTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.snapshot_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "snapshotVersion=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTaskTemplateOutput {
    var result: GetTaskTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTaskTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
