...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAttachedFileUploadInput, options: Options) !StartAttachedFileUploadOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartAttachedFileUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/attached-files/");
    ...
    try query_buf.appendSlice(alloc, "associatedResourceArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.associated_resource_arn);
    query_has_prev = true;
    ...
    if (input.created_by) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"CreatedBy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FileName\":");
    try aws.json.writeValue(@TypeOf(input.file_name), input.file_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FileSizeInBytes\":");
    try aws.json.writeValue(@TypeOf(input.file_size_in_bytes), input.file_size_in_bytes, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FileUseCaseType\":");
    try aws.json.writeValue(@TypeOf(input.file_use_case_type), input.file_use_case_type, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartAttachedFileUploadOutput {
    var result: StartAttachedFileUploadOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartAttachedFileUploadOutput, body, alloc);
    }
    _ = status;
    _ = headers;
