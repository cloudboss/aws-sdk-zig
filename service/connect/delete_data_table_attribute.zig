...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDataTableAttributeInput, options: Options) !DeleteDataTableAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDataTableAttributeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDataTableAttributeOutput {
    var result: DeleteDataTableAttributeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDataTableAttributeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
