...
pub const AgentInfo = struct {
    ...
    /// for
    /// `AfterContactWorkEndTimestamp`.
    after_contact_work_end_timestamp: ?i64,

    /// The date and time when the agent started doing After Contact Work for the
    /// contact, in UTC time.
    after_contact_work_start_timestamp: ?i64,

    /// The total hold duration in seconds initiated by the agent.
    agent_initiated_hold_duration: ?i32,

    /// Agent pause duration for a contact in seconds.
    agent_pause_duration_in_seconds: ?i32,
    ...
    preview_end_timestamp: ?i64,

    /// List of `StateTransition` for a supervisor.
    state_transitions: ?[]const StateTransition,
