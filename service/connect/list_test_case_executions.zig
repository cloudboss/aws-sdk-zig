...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTestCaseExecutionsInput, options: Options) !ListTestCaseExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTestCaseExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/test-case-executions/");
    ...
    if (input.test_case_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "testCaseId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.test_case_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "testCaseName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTestCaseExecutionsOutput {
    var result: ListTestCaseExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTestCaseExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
