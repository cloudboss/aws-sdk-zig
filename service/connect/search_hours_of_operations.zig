...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchHoursOfOperationsInput, options: CallOptions) !SearchHoursOfOperationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchHoursOfOperationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchHoursOfOperationsOutput {
    var result: SearchHoursOfOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchHoursOfOperationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
