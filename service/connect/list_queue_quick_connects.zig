...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQueueQuickConnectsInput, options: CallOptions) !ListQueueQuickConnectsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListQueueQuickConnectsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListQueueQuickConnectsOutput {
    var result: ListQueueQuickConnectsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQueueQuickConnectsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
