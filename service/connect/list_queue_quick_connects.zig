...
pub const ListQueueQuickConnectsInput = struct {
...
pub const ListQueueQuickConnectsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQueueQuickConnectsInput, options: Options) !ListQueueQuickConnectsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQueueQuickConnectsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQueueQuickConnectsOutput {
    var result: ListQueueQuickConnectsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQueueQuickConnectsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
