...
const EvaluationSearchCriteria = @import("evaluation_search_criteria.zig").EvaluationSearchCriteria;
const EvaluationSearchFilter = @import("evaluation_search_filter.zig").EvaluationSearchFilter;
const EvaluationSearchSummary = @import("evaluation_search_summary.zig").EvaluationSearchSummary;

pub const SearchContactEvaluationsInput = struct {
    ...
    search_criteria: ?EvaluationSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?EvaluationSearchFilter = null,
...
pub const SearchContactEvaluationsOutput = struct {
    ...
    evaluation_search_summary_list: ?[]const EvaluationSearchSummary = null,
    ...
    pub const json_field_names = .{
        ...
        .evaluation_search_summary_list = "EvaluationSearchSummaryList",
        .next_token = "NextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchContactEvaluationsInput, options: Options) !SearchContactEvaluationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchContactEvaluationsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-contact-evaluations";
    ...
    if (input.search_criteria) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SearchCriteria\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.search_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SearchFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchContactEvaluationsOutput {
    var result: SearchContactEvaluationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchContactEvaluationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
