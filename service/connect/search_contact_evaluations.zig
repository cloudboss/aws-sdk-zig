...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchContactEvaluationsInput, options: CallOptions) !SearchContactEvaluationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchContactEvaluationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchContactEvaluationsOutput {
    var result: SearchContactEvaluationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchContactEvaluationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
