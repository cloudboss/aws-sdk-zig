...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoutingProfileQueuesInput, options: Options) !ListRoutingProfileQueuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoutingProfileQueuesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoutingProfileQueuesOutput {
    var result: ListRoutingProfileQueuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoutingProfileQueuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
