...
pub const UpdateWorkspaceThemeInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkspaceThemeInput, options: Options) !UpdateWorkspaceThemeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspaceThemeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/theme");
    ...
    if (input.theme) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"Theme\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspaceThemeOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateWorkspaceThemeOutput = .{};
