...
pub const ListViewVersionsOutput = struct {
    /// The token for the next set of results. Use the value returned in the
    /// previous response in the next request to
    /// retrieve the next set of results.
    next_token: ?[]const u8 = null,
    ...
    view_version_summary_list: ?[]const ViewVersionSummary = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .view_version_summary_list = "ViewVersionSummaryList",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListViewVersionsInput, options: Options) !ListViewVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListViewVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.view_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListViewVersionsOutput {
    var result: ListViewVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListViewVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
