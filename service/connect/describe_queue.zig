...
pub const DescribeQueueInput = struct {
...
pub const DescribeQueueOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeQueueInput, options: CallOptions) !DescribeQueueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeQueueInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeQueueOutput {
    var result: DescribeQueueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeQueueOutput, body, allocator);
    }
    _ = status;
    _ = headers;
