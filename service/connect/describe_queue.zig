...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeQueueInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeQueueOutput {
    var result: DescribeQueueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeQueueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
