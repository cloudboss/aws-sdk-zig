...
const UpdateUserPhoneConfigOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserPhoneConfigInput, options: Options) !UpdateUserPhoneConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserPhoneConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/phone-config");
    ...
    try body_buf.appendSlice(alloc, "\"PhoneConfig\":");
    try aws.json.writeValue(@TypeOf(input.phone_config), input.phone_config, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserPhoneConfigOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateUserPhoneConfigOutput = .{};
