...
const NotificationSearchCriteria = @import("notification_search_criteria.zig").NotificationSearchCriteria;
const NotificationSearchFilter = @import("notification_search_filter.zig").NotificationSearchFilter;
const NotificationSearchSummary = @import("notification_search_summary.zig").NotificationSearchSummary;

pub const SearchNotificationsInput = struct {
    ...
    /// filtering by notification ID and message content using comparison types such
    /// as STARTS_WITH, CONTAINS, and EXACT.
    search_criteria: ?NotificationSearchCriteria = null,

    /// Filters to apply to the search results, such as tag-based filters.
    search_filter: ?NotificationSearchFilter = null,
...
pub const SearchNotificationsOutput = struct {
    ...
    notifications: ?[]const NotificationSearchSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchNotificationsInput, options: CallOptions) !SearchNotificationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchNotificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-notifications";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchNotificationsOutput {
    var result: SearchNotificationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchNotificationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
