...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateWorkspaceInput, options: CallOptions) !AssociateWorkspaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateWorkspaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateWorkspaceOutput {
    var result: AssociateWorkspaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateWorkspaceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
