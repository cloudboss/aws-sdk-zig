...
pub const AssociateWorkspaceInput = struct {
    ...
    /// workspace. Valid resource types are
    /// users and routing profiles.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateWorkspaceInput, options: Options) !AssociateWorkspaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateWorkspaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/associate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateWorkspaceOutput {
    var result: AssociateWorkspaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateWorkspaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
