...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateLexBotInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.lex_bot), input.lex_bot, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateLexBotOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
