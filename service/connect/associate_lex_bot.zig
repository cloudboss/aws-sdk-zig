...
pub const AssociateLexBotInput = struct {
...
pub const AssociateLexBotOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateLexBotInput, options: CallOptions) !AssociateLexBotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateLexBotInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.lex_bot), input.lex_bot, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateLexBotOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateLexBotOutput = .{};
