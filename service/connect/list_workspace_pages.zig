...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkspacePagesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkspacePagesOutput {
    var result: ListWorkspacePagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkspacePagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
