...
pub const ListWorkspacePagesInput = struct {
...
pub const ListWorkspacePagesOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// A list of page configurations in the workspace.
    workspace_page_list: ?[]const WorkspacePage = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .workspace_page_list = "WorkspacePageList",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkspacePagesInput, options: CallOptions) !ListWorkspacePagesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListWorkspacePagesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListWorkspacePagesOutput {
    var result: ListWorkspacePagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkspacePagesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
