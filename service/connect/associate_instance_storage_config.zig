...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateInstanceStorageConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.storage_config), input.storage_config, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateInstanceStorageConfigOutput {
    var result: AssociateInstanceStorageConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateInstanceStorageConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
