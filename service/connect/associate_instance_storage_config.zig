...
pub const AssociateInstanceStorageConfigInput = struct {
    ...
    /// of
    /// contacts](https://docs.aws.amazon.com/connect/latest/adminguide/enable-contact-analysis-segment-streams.html), use the following types:
    ///
    ...
    /// supported and will apply only
    /// to VOICE channel contacts. Use `REAL_TIME_CONTACT_ANALYSIS_VOICE_SEGMENTS`
...
pub const AssociateInstanceStorageConfigOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateInstanceStorageConfigInput, options: CallOptions) !AssociateInstanceStorageConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateInstanceStorageConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.storage_config), input.storage_config, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateInstanceStorageConfigOutput {
    var result: AssociateInstanceStorageConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateInstanceStorageConfigOutput, body, allocator);
    }
    _ = status;
    _ = headers;
