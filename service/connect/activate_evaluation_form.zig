...
pub const ActivateEvaluationFormOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ActivateEvaluationFormInput, options: Options) !ActivateEvaluationFormOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ActivateEvaluationFormInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ActivateEvaluationFormOutput {
    var result: ActivateEvaluationFormOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ActivateEvaluationFormOutput, body, alloc);
    }
    _ = status;
    _ = headers;
