...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEntitySecurityProfilesInput, options: Options) !ListEntitySecurityProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEntitySecurityProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/entity-security-profiles-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEntitySecurityProfilesOutput {
    var result: ListEntitySecurityProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEntitySecurityProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
