...
pub const ListAnalyticsDataAssociationsInput = struct {
...
pub const ListAnalyticsDataAssociationsOutput = struct {
    ...
    results: ?[]const AnalyticsDataAssociationResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnalyticsDataAssociationsInput, options: Options) !ListAnalyticsDataAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAnalyticsDataAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/association");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.data_set_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "DataSetId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAnalyticsDataAssociationsOutput {
    var result: ListAnalyticsDataAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAnalyticsDataAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
