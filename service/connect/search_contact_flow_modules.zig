...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchContactFlowModulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchContactFlowModulesOutput {
    var result: SearchContactFlowModulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchContactFlowModulesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
