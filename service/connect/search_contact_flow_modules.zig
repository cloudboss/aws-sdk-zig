...
const ContactFlowModule = @import("contact_flow_module.zig").ContactFlowModule;

pub const SearchContactFlowModulesInput = struct {
    ...
    /// minimum of 2
    /// characters and a maximum of 25 characters. Any queries with character
    /// lengths outside of this range will result in
    /// invalid results.
    search_criteria: ?ContactFlowModuleSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?ContactFlowModuleSearchFilter = null,
...
pub const SearchContactFlowModulesOutput = struct {
    /// The total number of flows which matched your search query.
    approximate_total_count: ?i64 = null,

    /// The search criteria to be used to return flow modules.
    contact_flow_modules: ?[]const ContactFlowModule = null,
    ...
    pub const json_field_names = .{
        ...
        .contact_flow_modules = "ContactFlowModules",
        .next_token = "NextToken",
    };
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchContactFlowModulesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-contact-flow-modules";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchContactFlowModulesOutput {
    var result: SearchContactFlowModulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchContactFlowModulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
