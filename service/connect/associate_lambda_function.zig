...
pub const AssociateLambdaFunctionOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateLambdaFunctionInput, options: CallOptions) !AssociateLambdaFunctionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateLambdaFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"FunctionArn\":");
    try aws.json.writeValue(@TypeOf(input.function_arn), input.function_arn, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateLambdaFunctionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
