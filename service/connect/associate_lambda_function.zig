...
pub const AssociateLambdaFunctionInput = struct {
...
const AssociateLambdaFunctionOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateLambdaFunctionInput, options: Options) !AssociateLambdaFunctionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateLambdaFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FunctionArn\":");
    try aws.json.writeValue(@TypeOf(input.function_arn), input.function_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateLambdaFunctionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateLambdaFunctionOutput = .{};
