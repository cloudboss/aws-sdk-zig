...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataTableInput, options: Options) !CreateDataTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.time_zone), input.time_zone, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ValueLockLevel\":");
    try aws.json.writeValue(@TypeOf(input.value_lock_level), input.value_lock_level, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataTableOutput {
    var result: CreateDataTableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataTableOutput, body, alloc);
    }
    _ = status;
    _ = headers;
