...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContactFlowInput, options: Options) !CreateContactFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContactFlowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContactFlowOutput {
    var result: CreateContactFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateContactFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
