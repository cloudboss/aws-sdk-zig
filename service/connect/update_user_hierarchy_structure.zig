...
const HierarchyStructureUpdate = @import("hierarchy_structure_update.zig").HierarchyStructureUpdate;

pub const UpdateUserHierarchyStructureInput = struct {
...
pub const UpdateUserHierarchyStructureOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserHierarchyStructureInput, options: CallOptions) !UpdateUserHierarchyStructureOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateUserHierarchyStructureInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/user-hierarchy-structure/");
    ...
    try body_buf.appendSlice(allocator, "\"HierarchyStructure\":");
    try aws.json.writeValue(@TypeOf(input.hierarchy_structure), input.hierarchy_structure, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateUserHierarchyStructureOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateUserHierarchyStructureOutput = .{};
