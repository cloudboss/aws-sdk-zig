...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserHierarchyStructureInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/user-hierarchy-structure/");
    ...
    try body_buf.appendSlice(alloc, "\"HierarchyStructure\":");
    try aws.json.writeValue(@TypeOf(input.hierarchy_structure), input.hierarchy_structure, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserHierarchyStructureOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
