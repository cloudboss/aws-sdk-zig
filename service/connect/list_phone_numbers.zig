...
const PhoneNumberSummary = @import("phone_number_summary.zig").PhoneNumberSummary;
...
pub const ListPhoneNumbersInput = struct {
    ...
    ///
    /// We recommend using
    /// [ListPhoneNumbersV2](https://docs.aws.amazon.com/connect/latest/APIReference/API_ListPhoneNumbersV2.html) to return phone number types. While ListPhoneNumbers returns number types
...
pub const ListPhoneNumbersOutput = struct {
    ...
    phone_number_summary_list: ?[]const PhoneNumberSummary = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPhoneNumbersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/phone-numbers-summary/");
    ...
    if (input.phone_number_country_codes) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "phoneNumberCountryCodes=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.phone_number_types) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "phoneNumberTypes=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
