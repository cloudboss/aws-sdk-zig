...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCurrentMetricDataInput, options: Options) !GetCurrentMetricDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCurrentMetricDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/metrics/current/");
    ...
    try body_buf.appendSlice(alloc, "\"CurrentMetrics\":");
    try aws.json.writeValue(@TypeOf(input.current_metrics), input.current_metrics, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.groupings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Groupings\":");
    ...
    if (input.sort_criteria) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SortCriteria\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCurrentMetricDataOutput {
    var result: GetCurrentMetricDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCurrentMetricDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
