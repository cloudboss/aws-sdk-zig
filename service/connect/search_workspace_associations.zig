...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchWorkspaceAssociationsInput, options: Options) !SearchWorkspaceAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchWorkspaceAssociationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchWorkspaceAssociationsOutput {
    var result: SearchWorkspaceAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchWorkspaceAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
