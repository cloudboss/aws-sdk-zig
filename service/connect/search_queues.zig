...
pub const SearchQueuesInput = struct {
    ...
    search_criteria: ?QueueSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?QueueSearchFilter = null,
...
pub const SearchQueuesOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchQueuesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchQueuesOutput {
    var result: SearchQueuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchQueuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
