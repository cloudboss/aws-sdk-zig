...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchHoursOfOperationOverridesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchHoursOfOperationOverridesOutput {
    var result: SearchHoursOfOperationOverridesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchHoursOfOperationOverridesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
