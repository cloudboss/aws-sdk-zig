...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataTableValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/values/list");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataTableValuesOutput {
    var result: ListDataTableValuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataTableValuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
