...
const DataTableValueSummary = @import("data_table_value_summary.zig").DataTableValueSummary;

pub const ListDataTableValuesInput = struct {
...
pub const ListDataTableValuesOutput = struct {
    /// Specify the pagination token from a previous request to retrieve the next
    /// page of results.
    next_token: ?[]const u8 = null,
    ...
    values: ?[]const DataTableValueSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataTableValuesInput, options: CallOptions) !ListDataTableValuesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDataTableValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/values/list");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDataTableValuesOutput {
    var result: ListDataTableValuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataTableValuesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
