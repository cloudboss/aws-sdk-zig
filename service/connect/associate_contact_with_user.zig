...
pub const AssociateContactWithUserInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateContactWithUserInput, options: Options) !AssociateContactWithUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateContactWithUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/associate-user");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateContactWithUserOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateContactWithUserOutput = .{};
