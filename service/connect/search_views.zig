...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchViewsInput, options: Options) !SearchViewsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchViewsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchViewsOutput {
    var result: SearchViewsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchViewsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
