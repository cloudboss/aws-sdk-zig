...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchTestCasesInput, options: Options) !SearchTestCasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchTestCasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchTestCasesOutput {
    var result: SearchTestCasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchTestCasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
