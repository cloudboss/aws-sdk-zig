...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePersistentContactAssociationInput, options: Options) !CreatePersistentContactAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePersistentContactAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact/persistent-contact-association/");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RehydrationType\":");
    try aws.json.writeValue(@TypeOf(input.rehydration_type), input.rehydration_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceContactId\":");
    try aws.json.writeValue(@TypeOf(input.source_contact_id), input.source_contact_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePersistentContactAssociationOutput {
    var result: CreatePersistentContactAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePersistentContactAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
