...
const AssociateRoutingProfileQueuesOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateRoutingProfileQueuesInput, options: Options) !AssociateRoutingProfileQueuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateRoutingProfileQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/associate-queues");
    ...
    if (input.manual_assignment_queue_configs) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ManualAssignmentQueueConfigs\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.queue_configs) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"QueueConfigs\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateRoutingProfileQueuesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateRoutingProfileQueuesOutput = .{};
