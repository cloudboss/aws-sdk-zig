...
pub const DisassociateHoursOfOperationsInput = struct {
    ...
    /// to disassociate with the child hours of operation resource.
    parent_hours_of_operation_ids: []const []const u8,

    pub const json_field_names = .{
        .hours_of_operation_id = "HoursOfOperationId",
        .instance_id = "InstanceId",
        .parent_hours_of_operation_ids = "ParentHoursOfOperationIds",
    };
};

const DisassociateHoursOfOperationsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateHoursOfOperationsInput, options: Options) !DisassociateHoursOfOperationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateHoursOfOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/disassociate-hours");
    ...
    try body_buf.appendSlice(alloc, "\"ParentHoursOfOperationIds\":");
    try aws.json.writeValue(@TypeOf(input.parent_hours_of_operation_ids), input.parent_hours_of_operation_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateHoursOfOperationsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateHoursOfOperationsOutput = .{};
