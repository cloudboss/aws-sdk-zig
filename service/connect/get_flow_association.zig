...
pub const GetFlowAssociationInput = struct {
...
pub const GetFlowAssociationOutput = struct {
    ...
    /// A valid resource type.
    resource_type: ?FlowAssociationResourceType = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFlowAssociationInput, options: CallOptions) !GetFlowAssociationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFlowAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFlowAssociationOutput {
    var result: GetFlowAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowAssociationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
