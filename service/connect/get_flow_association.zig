...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFlowAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFlowAssociationOutput {
    var result: GetFlowAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
