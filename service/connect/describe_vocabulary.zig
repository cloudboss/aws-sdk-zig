...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVocabularyInput, options: Options) !DescribeVocabularyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVocabularyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/vocabulary/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.vocabulary_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVocabularyOutput {
    var result: DescribeVocabularyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeVocabularyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
