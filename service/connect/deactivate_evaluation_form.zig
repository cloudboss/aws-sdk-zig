...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeactivateEvaluationFormInput, options: CallOptions) !DeactivateEvaluationFormOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeactivateEvaluationFormInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeactivateEvaluationFormOutput {
    var result: DeactivateEvaluationFormOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeactivateEvaluationFormOutput, body, allocator);
    }
    _ = status;
    _ = headers;
