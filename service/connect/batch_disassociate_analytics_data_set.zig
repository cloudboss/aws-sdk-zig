...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDisassociateAnalyticsDataSetInput, options: Options) !BatchDisassociateAnalyticsDataSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDisassociateAnalyticsDataSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"DataSetIds\":");
    try aws.json.writeValue(@TypeOf(input.data_set_ids), input.data_set_ids, alloc, &body_buf);
    has_prev = true;
    if (input.target_account_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TargetAccountId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDisassociateAnalyticsDataSetOutput {
    var result: BatchDisassociateAnalyticsDataSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDisassociateAnalyticsDataSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
