...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAgentStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAgentStatusOutput {
    var result: CreateAgentStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAgentStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
