...
pub const CreateAgentStatusInput = struct {
...
pub const CreateAgentStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAgentStatusInput, options: CallOptions) !CreateAgentStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAgentStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.display_order) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"DisplayOrder\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAgentStatusOutput {
    var result: CreateAgentStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAgentStatusOutput, body, allocator);
    }
    _ = status;
    _ = headers;
