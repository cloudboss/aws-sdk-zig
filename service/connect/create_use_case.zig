...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUseCaseInput, options: CallOptions) !CreateUseCaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateUseCaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.integration_association_id);
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"UseCaseType\":");
    try aws.json.writeValue(@TypeOf(input.use_case_type), input.use_case_type, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateUseCaseOutput {
    var result: CreateUseCaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateUseCaseOutput, body, allocator);
    }
    _ = status;
    _ = headers;
