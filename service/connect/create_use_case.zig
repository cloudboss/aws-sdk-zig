...
pub const CreateUseCaseInput = struct {
...
pub const CreateUseCaseOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUseCaseInput, options: Options) !CreateUseCaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUseCaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.integration_association_id);
    try path_buf.appendSlice(alloc, "/use-cases");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UseCaseType\":");
    try aws.json.writeValue(@TypeOf(input.use_case_type), input.use_case_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUseCaseOutput {
    var result: CreateUseCaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateUseCaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
