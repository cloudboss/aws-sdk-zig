...
const RoutingProfileSearchCriteria = @import("routing_profile_search_criteria.zig").RoutingProfileSearchCriteria;
const RoutingProfileSearchFilter = @import("routing_profile_search_filter.zig").RoutingProfileSearchFilter;
const RoutingProfile = @import("routing_profile.zig").RoutingProfile;

pub const SearchRoutingProfilesInput = struct {
    ...
    search_criteria: ?RoutingProfileSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?RoutingProfileSearchFilter = null,
...
pub const SearchRoutingProfilesOutput = struct {
    ...
    routing_profiles: ?[]const RoutingProfile = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchRoutingProfilesInput, options: CallOptions) !SearchRoutingProfilesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchRoutingProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-routing-profiles";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchRoutingProfilesOutput {
    var result: SearchRoutingProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchRoutingProfilesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
