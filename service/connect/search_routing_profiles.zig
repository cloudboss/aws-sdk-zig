...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchRoutingProfilesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchRoutingProfilesOutput {
    var result: SearchRoutingProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchRoutingProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
