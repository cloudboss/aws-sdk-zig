...
pub const DescribeUserHierarchyGroupInput = struct {
...
pub const DescribeUserHierarchyGroupOutput = struct {
    /// Information about the hierarchy group.
    hierarchy_group: ?HierarchyGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUserHierarchyGroupInput, options: CallOptions) !DescribeUserHierarchyGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeUserHierarchyGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/user-hierarchy-groups/");
    try path_buf.appendSlice(allocator, input.instance_id);
    try path_buf.appendSlice(allocator, "/");
    try path_buf.appendSlice(allocator, input.hierarchy_group_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeUserHierarchyGroupOutput {
    var result: DescribeUserHierarchyGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeUserHierarchyGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
