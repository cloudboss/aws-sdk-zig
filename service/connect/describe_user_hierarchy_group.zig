...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUserHierarchyGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/user-hierarchy-groups/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.hierarchy_group_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUserHierarchyGroupOutput {
    var result: DescribeUserHierarchyGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeUserHierarchyGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
