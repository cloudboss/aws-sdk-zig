...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTestCaseExecutionInput, options: Options) !StartTestCaseExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartTestCaseExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/start-execution");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartTestCaseExecutionOutput {
    var result: StartTestCaseExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartTestCaseExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
