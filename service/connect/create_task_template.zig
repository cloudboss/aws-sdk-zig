...
pub const CreateTaskTemplateInput = struct {
...
pub const CreateTaskTemplateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTaskTemplateInput, options: Options) !CreateTaskTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTaskTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTaskTemplateOutput {
    var result: CreateTaskTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTaskTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
