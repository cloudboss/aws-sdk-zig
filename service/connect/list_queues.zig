...
pub const ListQueuesInput = struct {
    ...
    /// The type of queue.
    queue_types: ?[]const QueueType = null,
...
pub const ListQueuesOutput = struct {
    ...
    queue_summary_list: ?[]const QueueSummary = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/queues-summary/");
    ...
    if (input.queue_types) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "queueTypes=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
