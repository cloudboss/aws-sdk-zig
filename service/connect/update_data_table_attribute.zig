...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDataTableAttributeInput, options: Options) !UpdateDataTableAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataTableAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.primary) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Primary\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.validation) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Validation\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ValueType\":");
    try aws.json.writeValue(@TypeOf(input.value_type), input.value_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataTableAttributeOutput {
    var result: UpdateDataTableAttributeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDataTableAttributeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
