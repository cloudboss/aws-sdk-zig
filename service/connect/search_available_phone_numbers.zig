...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchAvailablePhoneNumbersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PhoneNumberCountryCode\":");
    try aws.json.writeValue(@TypeOf(input.phone_number_country_code), input.phone_number_country_code, alloc, &body_buf);
    has_prev = true;
    if (input.phone_number_prefix) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PhoneNumberPrefix\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PhoneNumberType\":");
    try aws.json.writeValue(@TypeOf(input.phone_number_type), input.phone_number_type, alloc, &body_buf);
    has_prev = true;
