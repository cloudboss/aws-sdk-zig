...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContactReferencesInput, options: Options) !ListContactReferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContactReferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact/references/");
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "referenceTypes=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.reference_types);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContactReferencesOutput {
    var result: ListContactReferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactReferencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
