...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetricDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/metrics/historical/");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"HistoricalMetrics\":");
    try aws.json.writeValue(@TypeOf(input.historical_metrics), input.historical_metrics, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetricDataOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMetricDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
