...
pub const AssociateSecurityKeyInput = struct {
...
pub const AssociateSecurityKeyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateSecurityKeyInput, options: Options) !AssociateSecurityKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateSecurityKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/security-key");
    ...
    try aws.json.writeValue(@TypeOf(input.key), input.key, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateSecurityKeyOutput {
    var result: AssociateSecurityKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateSecurityKeyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
