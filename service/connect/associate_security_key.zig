...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateSecurityKeyInput, options: CallOptions) !AssociateSecurityKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateSecurityKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/security-key");
    ...
    try aws.json.writeValue(@TypeOf(input.key), input.key, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateSecurityKeyOutput {
    var result: AssociateSecurityKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateSecurityKeyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
