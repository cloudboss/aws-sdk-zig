...
pub const ImportPhoneNumberInput = struct {
    ...
    /// as Amazon Web Services End User
    /// Messaging. If it is from Amazon Web Services End User Messaging, it looks
    /// like the ARN of the phone number to import
    /// from Amazon Web Services End User Messaging.
...
pub const ImportPhoneNumberOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportPhoneNumberInput, options: Options) !ImportPhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportPhoneNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/phone-number/import";
    ...
    if (input.phone_number_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PhoneNumberDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourcePhoneNumberArn\":");
    try aws.json.writeValue(@TypeOf(input.source_phone_number_arn), input.source_phone_number_arn, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportPhoneNumberOutput {
    var result: ImportPhoneNumberOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportPhoneNumberOutput, body, alloc);
    }
    _ = status;
    _ = headers;
