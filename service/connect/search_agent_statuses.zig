...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchAgentStatusesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchAgentStatusesOutput {
    var result: SearchAgentStatusesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchAgentStatusesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
