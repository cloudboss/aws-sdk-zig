...
const AgentStatusSearchCriteria = @import("agent_status_search_criteria.zig").AgentStatusSearchCriteria;
const AgentStatusSearchFilter = @import("agent_status_search_filter.zig").AgentStatusSearchFilter;
const AgentStatus = @import("agent_status.zig").AgentStatus;

pub const SearchAgentStatusesInput = struct {
    ...
    search_criteria: ?AgentStatusSearchCriteria = null,

    /// Filters to be applied to search results.
    search_filter: ?AgentStatusSearchFilter = null,
...
pub const SearchAgentStatusesOutput = struct {
    /// The search criteria to be used to return agent statuses.
    agent_statuses: ?[]const AgentStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchAgentStatusesInput, options: CallOptions) !SearchAgentStatusesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchAgentStatusesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-agent-statuses";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchAgentStatusesOutput {
    var result: SearchAgentStatusesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchAgentStatusesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
