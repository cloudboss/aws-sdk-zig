...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartContactEvaluationInput, options: Options) !StartContactEvaluationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartContactEvaluationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.auto_evaluation_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AutoEvaluationConfiguration\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EvaluationFormId\":");
    try aws.json.writeValue(@TypeOf(input.evaluation_form_id), input.evaluation_form_id, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartContactEvaluationOutput {
    var result: StartContactEvaluationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartContactEvaluationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
