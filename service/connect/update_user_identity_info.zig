...
pub const UpdateUserIdentityInfoOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserIdentityInfoInput, options: CallOptions) !UpdateUserIdentityInfoOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateUserIdentityInfoInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/identity-info");
    ...
    try aws.json.writeValue(@TypeOf(input.identity_info), input.identity_info, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateUserIdentityInfoOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
