...
pub const UpdateUserIdentityInfoInput = struct {
...
const UpdateUserIdentityInfoOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserIdentityInfoInput, options: Options) !UpdateUserIdentityInfoOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserIdentityInfoInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/identity-info");
    ...
    try body_buf.appendSlice(alloc, "\"IdentityInfo\":");
    try aws.json.writeValue(@TypeOf(input.identity_info), input.identity_info, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserIdentityInfoOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateUserIdentityInfoOutput = .{};
