...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContactFlowModuleAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContactFlowModuleAliasesOutput {
    var result: ListContactFlowModuleAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactFlowModuleAliasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
