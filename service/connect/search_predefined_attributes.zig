...
const PredefinedAttributeSearchCriteria = @import("predefined_attribute_search_criteria.zig").PredefinedAttributeSearchCriteria;
...
pub const SearchPredefinedAttributesInput = struct {
    ...
    search_criteria: ?PredefinedAttributeSearchCriteria = null,
...
pub const SearchPredefinedAttributesOutput = struct {
    ...
    predefined_attributes: ?[]const PredefinedAttribute = null,

    pub const json_field_names = .{
        .approximate_total_count = "ApproximateTotalCount",
        .next_token = "NextToken",
        .predefined_attributes = "PredefinedAttributes",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchPredefinedAttributesInput, options: Options) !SearchPredefinedAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchPredefinedAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/search-predefined-attributes";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchPredefinedAttributesOutput {
    var result: SearchPredefinedAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchPredefinedAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
