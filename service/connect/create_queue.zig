...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"HoursOfOperationId\":");
    try aws.json.writeValue(@TypeOf(input.hours_of_operation_id), input.hours_of_operation_id, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.outbound_caller_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OutboundCallerConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.outbound_email_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OutboundEmailConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
