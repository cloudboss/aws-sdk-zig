...
const DisassociateRoutingProfileQueuesOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateRoutingProfileQueuesInput, options: Options) !DisassociateRoutingProfileQueuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateRoutingProfileQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/disassociate-queues");
    ...
    if (input.manual_assignment_queue_references) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ManualAssignmentQueueReferences\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.queue_references) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"QueueReferences\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateRoutingProfileQueuesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
