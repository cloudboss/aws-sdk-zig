...
pub const UpdateRoutingProfileDefaultOutboundQueueInput = struct {
...
const UpdateRoutingProfileDefaultOutboundQueueOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoutingProfileDefaultOutboundQueueInput, options: Options) !UpdateRoutingProfileDefaultOutboundQueueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoutingProfileDefaultOutboundQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/routing-profiles/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.routing_profile_id);
    try path_buf.appendSlice(alloc, "/default-outbound-queue");
    ...
    try body_buf.appendSlice(alloc, "\"DefaultOutboundQueueId\":");
    try aws.json.writeValue(@TypeOf(input.default_outbound_queue_id), input.default_outbound_queue_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoutingProfileDefaultOutboundQueueOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateRoutingProfileDefaultOutboundQueueOutput = .{};
