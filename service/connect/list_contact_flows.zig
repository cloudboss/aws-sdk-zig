...
const ContactFlowSummary = @import("contact_flow_summary.zig").ContactFlowSummary;

pub const ListContactFlowsInput = struct {
    /// The type of flow.
    contact_flow_types: ?[]const ContactFlowType = null,
...
pub const ListContactFlowsOutput = struct {
    /// Information about the flows.
    contact_flow_summary_list: ?[]const ContactFlowSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContactFlowsInput, options: CallOptions) !ListContactFlowsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListContactFlowsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/contact-flows-summary/");
    try path_buf.appendSlice(allocator, input.instance_id);
    const path = try path_buf.toOwnedSlice(allocator);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.contact_flow_types) |v| {
        for (v) |item| {
            if (query_has_prev) try query_buf.appendSlice(allocator, "&");
            try query_buf.appendSlice(allocator, "contactFlowTypes=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListContactFlowsOutput {
    var result: ListContactFlowsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactFlowsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
