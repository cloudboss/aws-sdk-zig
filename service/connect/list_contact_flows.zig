...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContactFlowsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contact-flows-summary/");
    ...
    if (input.contact_flow_types) |v| {
        ...
        try query_buf.appendSlice(alloc, "contactFlowTypes=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContactFlowsOutput {
    var result: ListContactFlowsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactFlowsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
