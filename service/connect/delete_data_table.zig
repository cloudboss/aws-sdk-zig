...
pub const DeleteDataTableInput = struct {
    /// The unique identifier for the data table to delete. Must also accept the
    /// table ARN. Fails with an error if the
    /// version is provided and is not $LATEST.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDataTableInput, options: Options) !DeleteDataTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDataTableInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDataTableOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteDataTableOutput = .{};
