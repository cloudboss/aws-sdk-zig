...
pub const StopTestCaseExecutionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopTestCaseExecutionInput, options: Options) !StopTestCaseExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopTestCaseExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/stop-execution");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopTestCaseExecutionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: StopTestCaseExecutionOutput = .{};
