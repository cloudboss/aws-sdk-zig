...
const AssociateHoursOfOperationsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateHoursOfOperationsInput, options: Options) !AssociateHoursOfOperationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateHoursOfOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/associate-hours");
    ...
    try body_buf.appendSlice(alloc, "\"ParentHoursOfOperationConfigs\":");
    try aws.json.writeValue(@TypeOf(input.parent_hours_of_operation_configs), input.parent_hours_of_operation_configs, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateHoursOfOperationsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
