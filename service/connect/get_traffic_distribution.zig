...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTrafficDistributionInput, options: Options) !GetTrafficDistributionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTrafficDistributionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTrafficDistributionOutput {
    var result: GetTrafficDistributionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTrafficDistributionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
