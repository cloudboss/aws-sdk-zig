...
const DisconnectReason = @import("disconnect_reason.zig").DisconnectReason;

pub const StopContactInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopContactInput, options: Options) !StopContactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopContactInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (input.disconnect_reason) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DisconnectReason\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopContactOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: StopContactOutput = .{};
