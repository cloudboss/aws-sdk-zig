...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHoursOfOperationOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"Config\":");
    try aws.json.writeValue(@TypeOf(input.config), input.config, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EffectiveFrom\":");
    try aws.json.writeValue(@TypeOf(input.effective_from), input.effective_from, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EffectiveTill\":");
    try aws.json.writeValue(@TypeOf(input.effective_till), input.effective_till, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.override_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OverrideType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.recurrence_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecurrenceConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHoursOfOperationOverrideOutput {
    var result: CreateHoursOfOperationOverrideOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateHoursOfOperationOverrideOutput, body, alloc);
    }
    _ = status;
    _ = headers;
