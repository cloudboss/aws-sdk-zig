...
pub const CreateTrafficDistributionGroupInput = struct {
...
pub const CreateTrafficDistributionGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrafficDistributionGroupInput, options: CallOptions) !CreateTrafficDistributionGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTrafficDistributionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/traffic-distribution-group";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTrafficDistributionGroupOutput {
    var result: CreateTrafficDistributionGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTrafficDistributionGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
