...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrafficDistributionGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrafficDistributionGroupOutput {
    var result: CreateTrafficDistributionGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTrafficDistributionGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
