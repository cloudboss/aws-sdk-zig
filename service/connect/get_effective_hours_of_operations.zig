...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEffectiveHoursOfOperationsInput, options: Options) !GetEffectiveHoursOfOperationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEffectiveHoursOfOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/effective-hours-of-operations/");
    ...
    try query_buf.appendSlice(alloc, "fromDate=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.from_date);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "toDate=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.to_date);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEffectiveHoursOfOperationsOutput {
    var result: GetEffectiveHoursOfOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEffectiveHoursOfOperationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
