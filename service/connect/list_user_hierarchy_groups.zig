...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUserHierarchyGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/user-hierarchy-groups-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUserHierarchyGroupsOutput {
    var result: ListUserHierarchyGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUserHierarchyGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
