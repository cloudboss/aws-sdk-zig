...
pub const ListUserHierarchyGroupsInput = struct {
...
pub const ListUserHierarchyGroupsOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// Information about the hierarchy groups.
    user_hierarchy_group_summary_list: ?[]const HierarchyGroupSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUserHierarchyGroupsInput, options: CallOptions) !ListUserHierarchyGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUserHierarchyGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/user-hierarchy-groups-summary/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUserHierarchyGroupsOutput {
    var result: ListUserHierarchyGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUserHierarchyGroupsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
