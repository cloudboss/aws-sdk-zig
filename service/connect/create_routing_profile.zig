...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRoutingProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRoutingProfileOutput {
    var result: CreateRoutingProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRoutingProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
