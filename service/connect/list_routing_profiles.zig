...
pub const ListRoutingProfilesOutput = struct {
    /// If there are additional results, this is the token for the next set of
    /// results.
    next_token: ?[]const u8 = null,

    /// Information about the routing profiles.
    routing_profile_summary_list: ?[]const RoutingProfileSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoutingProfilesInput, options: Options) !ListRoutingProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoutingProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/routing-profiles-summary/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoutingProfilesOutput {
    var result: ListRoutingProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoutingProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
