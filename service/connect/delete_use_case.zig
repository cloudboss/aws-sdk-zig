...
pub const DeleteUseCaseInput = struct {
...
pub const DeleteUseCaseOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUseCaseInput, options: CallOptions) !DeleteUseCaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUseCaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.use_case_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUseCaseOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteUseCaseOutput = .{};
