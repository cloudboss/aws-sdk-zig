...
fn serializeRequest(alloc: std.mem.Allocator, input: StartOutboundVoiceContactInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.answer_machine_detection_config) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AnswerMachineDetectionConfig\":");
    ...
    if (input.campaign_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"CampaignId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DestinationPhoneNumber\":");
    try aws.json.writeValue(@TypeOf(input.destination_phone_number), input.destination_phone_number, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.outbound_strategy) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OutboundStrategy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.queue_id) |v| {
    ...
    if (input.ring_timeout_in_seconds) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RingTimeoutInSeconds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_phone_number) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourcePhoneNumber\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartOutboundVoiceContactOutput {
    var result: StartOutboundVoiceContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartOutboundVoiceContactOutput, body, alloc);
    }
    _ = status;
    _ = headers;
