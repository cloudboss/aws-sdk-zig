...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoutingProfileQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.queue_configs), input.queue_configs, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoutingProfileQueuesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
