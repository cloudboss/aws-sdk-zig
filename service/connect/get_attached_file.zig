...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAttachedFileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.file_id);
    ...
    if (input.url_expiry_in_seconds) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "urlExpiryInSeconds=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAttachedFileOutput {
    var result: GetAttachedFileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAttachedFileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
