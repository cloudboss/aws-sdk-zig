...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeContactEvaluationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeContactEvaluationOutput {
    var result: DescribeContactEvaluationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeContactEvaluationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
