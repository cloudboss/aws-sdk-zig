...
const EvaluationFormContent = @import("evaluation_form_content.zig").EvaluationFormContent;

pub const DescribeContactEvaluationInput = struct {
...
pub const DescribeContactEvaluationOutput = struct {
    /// Information about the evaluation form completed for a specific contact.
    evaluation: ?Evaluation = null,

    /// Information about the evaluation form.
    evaluation_form: ?EvaluationFormContent = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContactEvaluationInput, options: CallOptions) !DescribeContactEvaluationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeContactEvaluationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeContactEvaluationOutput {
    var result: DescribeContactEvaluationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeContactEvaluationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
