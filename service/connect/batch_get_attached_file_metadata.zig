...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetAttachedFileMetadataInput, options: Options) !BatchGetAttachedFileMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetAttachedFileMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"FileIds\":");
    try aws.json.writeValue(@TypeOf(input.file_ids), input.file_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetAttachedFileMetadataOutput {
    var result: BatchGetAttachedFileMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetAttachedFileMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
