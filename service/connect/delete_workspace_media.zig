...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteWorkspaceMediaInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(allocator, &query_buf, @tagName(input.media_type));
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteWorkspaceMediaOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
