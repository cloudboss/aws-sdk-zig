...
pub const DeleteWorkspaceMediaInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteWorkspaceMediaInput, options: Options) !DeleteWorkspaceMediaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteWorkspaceMediaInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/media");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "mediaType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.media_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteWorkspaceMediaOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteWorkspaceMediaOutput = .{};
