...
pub const ListSecurityProfilePermissionsInput = struct {
...
pub const ListSecurityProfilePermissionsOutput = struct {
    ...
    /// valid permissions, see [List of security profile
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecurityProfilePermissionsInput, options: CallOptions) !ListSecurityProfilePermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSecurityProfilePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/security-profiles-permissions/");
    try path_buf.appendSlice(allocator, input.instance_id);
    try path_buf.appendSlice(allocator, "/");
    try path_buf.appendSlice(allocator, input.security_profile_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSecurityProfilePermissionsOutput {
    var result: ListSecurityProfilePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityProfilePermissionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
