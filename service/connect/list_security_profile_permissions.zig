...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityProfilePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/security-profiles-permissions/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecurityProfilePermissionsOutput {
    var result: ListSecurityProfilePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityProfilePermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
