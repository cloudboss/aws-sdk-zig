...
pub const PauseContactInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PauseContactInput, options: Options) !PauseContactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PauseContactInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PauseContactOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PauseContactOutput = .{};
