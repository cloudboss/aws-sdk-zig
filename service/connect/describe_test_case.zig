...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTestCaseInput, options: Options) !DescribeTestCaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTestCaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTestCaseOutput {
    var result: DescribeTestCaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTestCaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
