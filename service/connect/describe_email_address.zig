...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEmailAddressInput, options: Options) !DescribeEmailAddressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEmailAddressInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEmailAddressOutput {
    var result: DescribeEmailAddressOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeEmailAddressOutput, body, alloc);
    }
    _ = status;
    _ = headers;
