...
const UnshareTarget = @import("unshare_target.zig").UnshareTarget;

pub const UnshareDirectoryInput = struct {
...
pub const UnshareDirectoryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UnshareDirectoryInput, options: CallOptions) !UnshareDirectoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UnshareDirectoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.UnshareDirectory");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UnshareDirectoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UnshareDirectoryOutput, body, allocator);
}
