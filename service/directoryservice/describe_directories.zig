...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDirectoriesInput, options: Options) !DescribeDirectoriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDirectoriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DescribeDirectories");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDirectoriesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDirectoriesOutput, body, alloc);
}
