...
const HybridUpdateType = @import("hybrid_update_type.zig").HybridUpdateType;
const HybridUpdateActivities = @import("hybrid_update_activities.zig").HybridUpdateActivities;

pub const DescribeHybridADUpdateInput = struct {
    /// The identifier of the hybrid directory for which to retrieve update
    /// information.
    directory_id: []const u8,

    /// The pagination token from a previous request to DescribeHybridADUpdate. Pass
    ...
    update_type: ?HybridUpdateType = null,
...
pub const DescribeHybridADUpdateOutput = struct {
    ...
    /// update
    /// type.
    update_activities: ?HybridUpdateActivities = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHybridADUpdateInput, options: CallOptions) !DescribeHybridADUpdateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHybridADUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DescribeHybridADUpdate");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHybridADUpdateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeHybridADUpdateOutput, body, allocator);
}
