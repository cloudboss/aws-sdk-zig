...
pub const CreateLogSubscriptionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLogSubscriptionInput, options: Options) !CreateLogSubscriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLogSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.CreateLogSubscription");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLogSubscriptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
