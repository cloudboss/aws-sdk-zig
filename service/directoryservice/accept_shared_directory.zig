...
pub const AcceptSharedDirectoryInput = struct {
...
pub const AcceptSharedDirectoryOutput = struct {
    /// The shared directory in the directory consumer account.
    shared_directory: ?SharedDirectory = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptSharedDirectoryInput, options: CallOptions) !AcceptSharedDirectoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptSharedDirectoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.AcceptSharedDirectory");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptSharedDirectoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AcceptSharedDirectoryOutput, body, allocator);
}
