...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterEventTopicInput, options: Options) !RegisterEventTopicOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterEventTopicInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.RegisterEventTopic");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterEventTopicOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
