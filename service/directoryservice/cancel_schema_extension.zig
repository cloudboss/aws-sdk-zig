...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelSchemaExtensionInput, options: Options) !CancelSchemaExtensionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelSchemaExtensionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.CancelSchemaExtension");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelSchemaExtensionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
