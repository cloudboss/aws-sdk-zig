...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveIpRoutesInput, options: Options) !RemoveIpRoutesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveIpRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.RemoveIpRoutes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveIpRoutesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
