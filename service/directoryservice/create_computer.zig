...
pub const CreateComputerInput = struct {
...
pub const CreateComputerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateComputerInput, options: CallOptions) !CreateComputerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateComputerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.CreateComputer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateComputerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateComputerOutput, body, allocator);
}
