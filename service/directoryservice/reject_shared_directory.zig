...
pub const RejectSharedDirectoryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectSharedDirectoryInput, options: CallOptions) !RejectSharedDirectoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RejectSharedDirectoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.RejectSharedDirectory");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RejectSharedDirectoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RejectSharedDirectoryOutput, body, allocator);
}
