...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrustInput, options: Options) !CreateTrustOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrustInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.CreateTrust");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrustOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTrustOutput, body, alloc);
}
