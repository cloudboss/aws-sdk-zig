...
pub const UpdateTrustInput = struct {
...
pub const UpdateTrustOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTrustInput, options: CallOptions) !UpdateTrustOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTrustInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.UpdateTrust");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTrustOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateTrustOutput, body, allocator);
}
