...
pub const DeleteADAssessmentInput = struct {
...
pub const DeleteADAssessmentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteADAssessmentInput, options: CallOptions) !DeleteADAssessmentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteADAssessmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DeleteADAssessment");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteADAssessmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteADAssessmentOutput, body, allocator);
}
