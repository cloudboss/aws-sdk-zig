...
const Trust = @import("trust.zig").Trust;

pub const DescribeTrustsInput = struct {
    ...
    /// information. If
    /// this member is null, all trust relationships that belong to the current
    /// account are
    /// returned.
    ///
    /// An empty list results in an `InvalidParameterException` being thrown.
    trust_ids: ?[]const []const u8 = null,
...
pub const DescribeTrustsOutput = struct {
    /// If not null, more results are available. Pass this value for the
    /// *NextToken* parameter in a subsequent call to DescribeTrusts to retrieve the
    /// next set of items.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustsInput, options: CallOptions) !DescribeTrustsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTrustsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DescribeTrusts");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTrustsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTrustsOutput, body, allocator);
}
