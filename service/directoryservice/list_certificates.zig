...
pub const ListCertificatesOutput = struct {
    ...
    certificates_info: ?[]const CertificateInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCertificatesInput, options: Options) !ListCertificatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.ListCertificates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCertificatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCertificatesOutput, body, alloc);
}
