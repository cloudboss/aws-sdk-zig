...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventTopicsInput, options: Options) !DescribeEventTopicsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEventTopicsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DescribeEventTopics");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEventTopicsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEventTopicsOutput, body, alloc);
}
