...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTrustInput, options: CallOptions) !DeleteTrustOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTrustInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DeleteTrust");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTrustOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteTrustOutput, body, allocator);
}
