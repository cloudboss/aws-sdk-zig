...
pub const DeleteTrustOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTrustInput, options: Options) !DeleteTrustOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTrustInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DeleteTrust");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTrustOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteTrustOutput, body, alloc);
}
