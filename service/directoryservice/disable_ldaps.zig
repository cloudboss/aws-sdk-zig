...
pub const DisableLDAPSInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableLDAPSInput, options: Options) !DisableLDAPSOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableLDAPSInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DisableLDAPS");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableLDAPSOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
