...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLogSubscriptionInput, options: Options) !DeleteLogSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteLogSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DeleteLogSubscription");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteLogSubscriptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
