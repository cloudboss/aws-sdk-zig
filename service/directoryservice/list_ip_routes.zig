...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIpRoutesInput, options: Options) !ListIpRoutesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIpRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.ListIpRoutes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIpRoutesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListIpRoutesOutput, body, alloc);
}
