...
pub const EnableRadiusInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableRadiusInput, options: CallOptions) !EnableRadiusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableRadiusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.EnableRadius");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableRadiusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
