...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConditionalForwardersInput, options: Options) !DescribeConditionalForwardersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConditionalForwardersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DescribeConditionalForwarders");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConditionalForwardersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConditionalForwardersOutput, body, alloc);
}
