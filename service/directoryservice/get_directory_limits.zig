...
pub const GetDirectoryLimitsOutput = struct {
    ...
    /// current Region.
    directory_limits: ?DirectoryLimits = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDirectoryLimitsInput, options: CallOptions) !GetDirectoryLimitsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDirectoryLimitsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.GetDirectoryLimits");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDirectoryLimitsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDirectoryLimitsOutput, body, allocator);
}
