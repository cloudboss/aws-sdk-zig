...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddIpRoutesInput, options: Options) !AddIpRoutesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddIpRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.AddIpRoutes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddIpRoutesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
