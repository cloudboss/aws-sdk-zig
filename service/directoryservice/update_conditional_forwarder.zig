...
pub const UpdateConditionalForwarderInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConditionalForwarderInput, options: Options) !UpdateConditionalForwarderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateConditionalForwarderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.UpdateConditionalForwarder");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateConditionalForwarderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
