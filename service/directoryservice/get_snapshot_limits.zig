...
const SnapshotLimits = @import("snapshot_limits.zig").SnapshotLimits;

pub const GetSnapshotLimitsInput = struct {
...
pub const GetSnapshotLimitsOutput = struct {
    /// A SnapshotLimits object that contains the manual snapshot limits for the
    /// specified
    /// directory.
    snapshot_limits: ?SnapshotLimits = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSnapshotLimitsInput, options: CallOptions) !GetSnapshotLimitsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSnapshotLimitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.GetSnapshotLimits");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSnapshotLimitsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSnapshotLimitsOutput, body, allocator);
}
