...
pub const ListLogSubscriptionsInput = struct {
    ...
    /// log subscriptions associated with your Amazon Web Services account. If there
    /// are no log subscriptions for
    /// the Amazon Web Services account or the directory, an empty list will be
    /// returned.
...
pub const ListLogSubscriptionsOutput = struct {
    /// A list of active LogSubscription objects for calling the
    /// Amazon Web Services account.
    log_subscriptions: ?[]const LogSubscription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLogSubscriptionsInput, options: CallOptions) !ListLogSubscriptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLogSubscriptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.ListLogSubscriptions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLogSubscriptionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLogSubscriptionsOutput, body, allocator);
}
