...
pub const DisableSsoInput = struct {
    ...
    /// is only used
    /// for AD Connector directories. This account must have privileges to remove a
    /// service
    /// principal name.
    ///
    ...
    /// single sign-on and are not stored by the service. The AD Connector service
    /// account is not
    /// changed.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableSsoInput, options: Options) !DisableSsoOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableSsoInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DisableSso");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableSsoOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
