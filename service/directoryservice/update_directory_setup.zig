...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDirectorySetupInput, options: Options) !UpdateDirectorySetupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDirectorySetupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.UpdateDirectorySetup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDirectorySetupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
