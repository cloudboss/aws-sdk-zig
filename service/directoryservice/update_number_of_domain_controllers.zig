...
pub const UpdateNumberOfDomainControllersInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNumberOfDomainControllersInput, options: Options) !UpdateNumberOfDomainControllersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateNumberOfDomainControllersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.UpdateNumberOfDomainControllers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateNumberOfDomainControllersOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
