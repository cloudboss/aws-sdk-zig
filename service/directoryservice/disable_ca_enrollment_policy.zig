...
pub const DisableCAEnrollmentPolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableCAEnrollmentPolicyInput, options: Options) !DisableCAEnrollmentPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableCAEnrollmentPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DisableCAEnrollmentPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableCAEnrollmentPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
