...
pub const EnableLDAPSInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableLDAPSInput, options: Options) !EnableLDAPSOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableLDAPSInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.EnableLDAPS");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableLDAPSOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
