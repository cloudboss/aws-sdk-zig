...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableLDAPSInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.EnableLDAPS");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableLDAPSOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
