...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableClientAuthenticationInput, options: Options) !DisableClientAuthenticationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableClientAuthenticationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DisableClientAuthentication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableClientAuthenticationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
