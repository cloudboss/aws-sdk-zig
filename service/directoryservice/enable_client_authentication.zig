...
pub const EnableClientAuthenticationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableClientAuthenticationInput, options: Options) !EnableClientAuthenticationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableClientAuthenticationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.EnableClientAuthentication");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableClientAuthenticationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
