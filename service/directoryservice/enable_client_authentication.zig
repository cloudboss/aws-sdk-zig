...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableClientAuthenticationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.EnableClientAuthentication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableClientAuthenticationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
