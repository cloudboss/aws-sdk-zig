...
const DataAccessStatus = @import("data_access_status.zig").DataAccessStatus;

pub const DescribeDirectoryDataAccessInput = struct {
...
pub const DescribeDirectoryDataAccessOutput = struct {
    /// The current status of data access through the Directory Service Data API.
    data_access_status: ?DataAccessStatus = null,

    pub const json_field_names = .{
        .data_access_status = "DataAccessStatus",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDirectoryDataAccessInput, options: CallOptions) !DescribeDirectoryDataAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDirectoryDataAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DescribeDirectoryDataAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDirectoryDataAccessOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDirectoryDataAccessOutput, body, allocator);
}
