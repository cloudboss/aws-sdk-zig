...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRadiusInput, options: Options) !UpdateRadiusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRadiusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.UpdateRadius");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRadiusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
