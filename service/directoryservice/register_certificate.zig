...
pub const RegisterCertificateInput = struct {
    ...
    client_cert_auth_settings: ?ClientCertAuthSettings = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.RegisterCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterCertificateOutput, body, alloc);
}
