...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConditionalForwarderInput, options: Options) !CreateConditionalForwarderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConditionalForwarderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.CreateConditionalForwarder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConditionalForwarderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
