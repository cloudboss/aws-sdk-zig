...
pub const DeleteConditionalForwarderInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConditionalForwarderInput, options: CallOptions) !DeleteConditionalForwarderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteConditionalForwarderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DeleteConditionalForwarder");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteConditionalForwarderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
