...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUpdateDirectoryInput, options: Options) !DescribeUpdateDirectoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUpdateDirectoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DescribeUpdateDirectory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUpdateDirectoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeUpdateDirectoryOutput, body, alloc);
}
