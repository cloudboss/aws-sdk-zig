...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveRegionInput, options: Options) !RemoveRegionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveRegionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.RemoveRegion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveRegionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
