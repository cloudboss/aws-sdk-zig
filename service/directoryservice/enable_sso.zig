...
pub const EnableSsoInput = struct {
    ...
    /// is only used
    /// for AD Connector directories. This account must have privileges to add a
    /// service principal
    /// name.
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableSsoInput, options: Options) !EnableSsoOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableSsoInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.EnableSso");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableSsoOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
