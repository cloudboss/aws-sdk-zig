...
pub const VerifyTrustInput = struct {
...
pub const VerifyTrustOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VerifyTrustInput, options: CallOptions) !VerifyTrustOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: VerifyTrustInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.VerifyTrust");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !VerifyTrustOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(VerifyTrustOutput, body, allocator);
}
