...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDomainControllersInput, options: Options) !DescribeDomainControllersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDomainControllersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DescribeDomainControllers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDomainControllersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDomainControllersOutput, body, alloc);
}
