...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterEventTopicInput, options: Options) !DeregisterEventTopicOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterEventTopicInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DeregisterEventTopic");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterEventTopicOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
