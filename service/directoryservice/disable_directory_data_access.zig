...
pub const DisableDirectoryDataAccessInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableDirectoryDataAccessInput, options: Options) !DisableDirectoryDataAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableDirectoryDataAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DisableDirectoryDataAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableDirectoryDataAccessOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
