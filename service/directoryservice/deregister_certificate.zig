...
pub const DeregisterCertificateInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterCertificateInput, options: Options) !DeregisterCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DeregisterCertificate");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
