...
pub const ListADAssessmentsInput = struct {
...
pub const ListADAssessmentsOutput = struct {
    /// A list of assessment summaries containing basic information about each
    /// directory
    /// assessment.
    assessments: ?[]const AssessmentSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListADAssessmentsInput, options: CallOptions) !ListADAssessmentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListADAssessmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.ListADAssessments");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListADAssessmentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListADAssessmentsOutput, body, allocator);
}
