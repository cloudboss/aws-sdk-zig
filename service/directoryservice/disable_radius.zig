...
pub const DisableRadiusInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableRadiusInput, options: Options) !DisableRadiusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableRadiusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.DisableRadius");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableRadiusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
