...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableRadiusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DirectoryService_20150416.DisableRadius");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableRadiusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
