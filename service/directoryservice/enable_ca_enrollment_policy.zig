...
pub const EnableCAEnrollmentPolicyInput = struct {
    ...
    /// connector to use
    /// for automatic certificate enrollment. This connector must be properly
    /// configured and
...
pub const EnableCAEnrollmentPolicyOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableCAEnrollmentPolicyInput, options: CallOptions) !EnableCAEnrollmentPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableCAEnrollmentPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DirectoryService_20150416.EnableCAEnrollmentPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableCAEnrollmentPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
