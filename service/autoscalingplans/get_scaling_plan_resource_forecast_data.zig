...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetScalingPlanResourceForecastDataInput, options: Options) !GetScalingPlanResourceForecastDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetScalingPlanResourceForecastDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AnyScaleScalingPlannerFrontendService.GetScalingPlanResourceForecastData");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetScalingPlanResourceForecastDataOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetScalingPlanResourceForecastDataOutput, body, alloc);
}
