...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateScalingPlanInput, options: Options) !CreateScalingPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateScalingPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AnyScaleScalingPlannerFrontendService.CreateScalingPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateScalingPlanOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateScalingPlanOutput, body, alloc);
}
