...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteScalingPlanInput, options: Options) !DeleteScalingPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteScalingPlanInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("autoscalingplans", "Auto Scaling Plans", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AnyScaleScalingPlannerFrontendService.DeleteScalingPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteScalingPlanOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
