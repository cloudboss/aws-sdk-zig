...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeScalingPlanResourcesInput, options: Options) !DescribeScalingPlanResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScalingPlanResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AnyScaleScalingPlannerFrontendService.DescribeScalingPlanResources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeScalingPlanResourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeScalingPlanResourcesOutput, body, alloc);
}
