...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2022-01-01/osis/updatePipeline/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePipelineOutput {
    var result: UpdatePipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
