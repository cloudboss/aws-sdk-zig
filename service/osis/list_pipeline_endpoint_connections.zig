...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPipelineEndpointConnectionsInput, options: Options) !ListPipelineEndpointConnectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPipelineEndpointConnectionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPipelineEndpointConnectionsOutput {
    var result: ListPipelineEndpointConnectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPipelineEndpointConnectionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
