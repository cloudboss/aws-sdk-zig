...
fn serializeRequest(alloc: std.mem.Allocator, input: RevokePipelineEndpointConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"EndpointIds\":");
    try aws.json.writeValue(@TypeOf(input.endpoint_ids), input.endpoint_ids, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RevokePipelineEndpointConnectionsOutput {
    var result: RevokePipelineEndpointConnectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RevokePipelineEndpointConnectionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
