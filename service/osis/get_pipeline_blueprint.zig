...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPipelineBlueprintInput, options: Options) !GetPipelineBlueprintOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPipelineBlueprintInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2022-01-01/osis/getPipelineBlueprint/");
    try path_buf.appendSlice(alloc, input.blueprint_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPipelineBlueprintOutput {
    var result: GetPipelineBlueprintOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPipelineBlueprintOutput, body, alloc);
    }
    _ = status;
    _ = headers;
