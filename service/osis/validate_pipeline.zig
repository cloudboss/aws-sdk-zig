...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ValidatePipelineInput, options: Options) !ValidatePipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ValidatePipelineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ValidatePipelineOutput {
    var result: ValidatePipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ValidatePipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
