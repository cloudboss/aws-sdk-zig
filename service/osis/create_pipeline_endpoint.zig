...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePipelineEndpointInput, options: Options) !CreatePipelineEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePipelineEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"PipelineArn\":");
    try aws.json.writeValue(@TypeOf(input.pipeline_arn), input.pipeline_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePipelineEndpointOutput {
    var result: CreatePipelineEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePipelineEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
