...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePipelineInput, options: Options) !CreatePipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePipelineInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("osis", "OSIS", alloc);
    ...
    if (input.buffer_options) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"BufferOptions\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MaxUnits\":");
    try aws.json.writeValue(@TypeOf(input.max_units), input.max_units, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MinUnits\":");
    try aws.json.writeValue(@TypeOf(input.min_units), input.min_units, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PipelineConfigurationBody\":");
    try aws.json.writeValue(@TypeOf(input.pipeline_configuration_body), input.pipeline_configuration_body, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PipelineName\":");
    try aws.json.writeValue(@TypeOf(input.pipeline_name), input.pipeline_name, alloc, &body_buf);
    has_prev = true;
    if (input.pipeline_role_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PipelineRoleArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePipelineOutput {
    var result: CreatePipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
