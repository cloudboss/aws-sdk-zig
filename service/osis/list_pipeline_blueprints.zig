...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPipelineBlueprintsInput, options: Options) !ListPipelineBlueprintsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPipelineBlueprintsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPipelineBlueprintsOutput {
    var result: ListPipelineBlueprintsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPipelineBlueprintsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
