...
pub const InvokeModelInput = struct {
...
pub const InvokeModelOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InvokeModelInput, options: CallOptions) !InvokeModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: InvokeModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/invoke");
    ...
    if (input.guardrail_identifier) |v| {
        try request.headers.put(allocator, "X-Amzn-Bedrock-GuardrailIdentifier", v);
    }
