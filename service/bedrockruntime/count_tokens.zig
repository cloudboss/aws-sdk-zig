...
const CountTokensInput = @import("count_tokens_request.zig").CountTokensRequest;

pub const CountTokensOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CountTokensInput, options: Options) !CountTokensOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CountTokensInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/count-tokens");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CountTokensOutput {
    var result: CountTokensOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CountTokensOutput, body, alloc);
    }
    _ = status;
    _ = headers;
