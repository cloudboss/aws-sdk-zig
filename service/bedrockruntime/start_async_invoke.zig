...
pub const StartAsyncInvokeInput = struct {
...
pub const StartAsyncInvokeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAsyncInvokeInput, options: CallOptions) !StartAsyncInvokeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartAsyncInvokeInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"modelInput\":");
    try aws.json.writeValue(@TypeOf(input.model_input), input.model_input, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartAsyncInvokeOutput {
    var result: StartAsyncInvokeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartAsyncInvokeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
