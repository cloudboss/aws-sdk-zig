...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAsyncInvokeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/async-invoke/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAsyncInvokeOutput {
    var result: GetAsyncInvokeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAsyncInvokeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
