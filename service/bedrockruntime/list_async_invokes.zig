...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAsyncInvokesInput, options: Options) !ListAsyncInvokesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAsyncInvokesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAsyncInvokesOutput {
    var result: ListAsyncInvokesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAsyncInvokesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
