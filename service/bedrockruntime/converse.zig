...
fn serializeRequest(alloc: std.mem.Allocator, input: ConverseInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/converse");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConverseOutput {
    var result: ConverseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ConverseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
