...
pub const ConverseInput = struct {
    ...
    guardrail_config: ?GuardrailConfiguration = null,
...
pub const ConverseOutput = @import("converse_response.zig").ConverseResponse;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConverseInput, options: CallOptions) !ConverseOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "amazonbedrockfrontendservice");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ConverseInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/converse");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ConverseOutput {
    var result: ConverseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ConverseOutput, body, allocator);
    }
    _ = status;
    _ = headers;
