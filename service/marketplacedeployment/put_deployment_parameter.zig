...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDeploymentParameterInput, options: Options) !PutDeploymentParameterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDeploymentParameterInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/catalogs/");
    try path_buf.appendSlice(alloc, input.catalog);
    try path_buf.appendSlice(alloc, "/products/");
    try path_buf.appendSlice(alloc, input.product_id);
    try path_buf.appendSlice(alloc, "/deployment-parameters");
    ...
    try body_buf.appendSlice(alloc, "\"agreementId\":");
    try aws.json.writeValue(@TypeOf(input.agreement_id), input.agreement_id, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"deploymentParameter\":");
    try aws.json.writeValue(@TypeOf(input.deployment_parameter), input.deployment_parameter, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDeploymentParameterOutput {
    var result: PutDeploymentParameterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutDeploymentParameterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
