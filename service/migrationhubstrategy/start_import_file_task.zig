...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartImportFileTaskInput, options: Options) !StartImportFileTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartImportFileTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.s3_bucket), input.s3_bucket, alloc, &body_buf);
    has_prev = true;
    if (input.s_3_bucket_for_report_data) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"s3bucketForReportData\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"s3key\":");
    try aws.json.writeValue(@TypeOf(input.s_3_key), input.s_3_key, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartImportFileTaskOutput {
    var result: StartImportFileTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartImportFileTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
