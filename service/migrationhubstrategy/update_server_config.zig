...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServerConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"serverId\":");
    try aws.json.writeValue(@TypeOf(input.server_id), input.server_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServerConfigOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
