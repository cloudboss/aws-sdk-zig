...
pub const UpdateServerConfigInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServerConfigInput, options: Options) !UpdateServerConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateServerConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/update-server-config/";
    ...
    try body_buf.appendSlice(allocator, "\"serverId\":");
    try aws.json.writeValue(@TypeOf(input.server_id), input.server_id, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateServerConfigOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateServerConfigOutput = .{};
