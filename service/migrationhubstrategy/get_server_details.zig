...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServerDetailsInput, options: Options) !GetServerDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServerDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/get-server-details/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServerDetailsOutput {
    var result: GetServerDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetServerDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
