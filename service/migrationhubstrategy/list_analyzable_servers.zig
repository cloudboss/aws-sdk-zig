...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnalyzableServersInput, options: Options) !ListAnalyzableServersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAnalyzableServersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAnalyzableServersOutput {
    var result: ListAnalyzableServersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAnalyzableServersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
