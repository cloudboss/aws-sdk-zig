...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPortfolioPreferencesInput, options: Options) !GetPortfolioPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPortfolioPreferencesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPortfolioPreferencesOutput {
    var result: GetPortfolioPreferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPortfolioPreferencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
