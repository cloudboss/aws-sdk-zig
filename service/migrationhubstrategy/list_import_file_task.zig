...
const ImportFileTaskInformation = @import("import_file_task_information.zig").ImportFileTaskInformation;

pub const ListImportFileTaskInput = struct {
...
pub const ListImportFileTaskOutput = struct {
    /// The token you use to retrieve the next set of results, or null if there are
    /// no more results.
    next_token: ?[]const u8 = null,

    /// Lists information about the files you import.
    task_infos: ?[]const ImportFileTaskInformation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImportFileTaskInput, options: CallOptions) !ListImportFileTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListImportFileTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/list-import-file-task";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListImportFileTaskOutput {
    var result: ListImportFileTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImportFileTaskOutput, body, allocator);
    }
    _ = status;
    _ = headers;
