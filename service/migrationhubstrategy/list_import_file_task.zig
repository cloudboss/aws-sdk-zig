...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImportFileTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImportFileTaskOutput {
    var result: ListImportFileTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImportFileTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
