...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImportFileTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/get-import-file-task/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImportFileTaskOutput {
    var result: GetImportFileTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImportFileTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
