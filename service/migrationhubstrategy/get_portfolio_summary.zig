...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPortfolioSummaryInput, options: Options) !GetPortfolioSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPortfolioSummaryInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPortfolioSummaryOutput {
    var result: GetPortfolioSummaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPortfolioSummaryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
