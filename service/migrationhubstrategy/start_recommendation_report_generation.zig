...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRecommendationReportGenerationInput, options: Options) !StartRecommendationReportGenerationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRecommendationReportGenerationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRecommendationReportGenerationOutput {
    var result: StartRecommendationReportGenerationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartRecommendationReportGenerationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
