...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRecommendationReportDetailsInput, options: Options) !GetRecommendationReportDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRecommendationReportDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/get-recommendation-report-details/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRecommendationReportDetailsOutput {
    var result: GetRecommendationReportDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRecommendationReportDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
