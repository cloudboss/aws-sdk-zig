...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApplicationComponentDetailsInput, options: Options) !GetApplicationComponentDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApplicationComponentDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/get-applicationcomponent-details/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApplicationComponentDetailsOutput {
    var result: GetApplicationComponentDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetApplicationComponentDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
