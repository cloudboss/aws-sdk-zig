...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLatestAssessmentIdInput, options: CallOptions) !GetLatestAssessmentIdOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLatestAssessmentIdInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    const path = "/get-latest-assessment-id";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLatestAssessmentIdOutput {
    var result: GetLatestAssessmentIdOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLatestAssessmentIdOutput, body, allocator);
    }
    _ = status;
    _ = headers;
