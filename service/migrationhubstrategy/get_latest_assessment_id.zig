...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLatestAssessmentIdInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLatestAssessmentIdOutput {
    var result: GetLatestAssessmentIdOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLatestAssessmentIdOutput, body, alloc);
    }
    _ = status;
    _ = headers;
