...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCollectorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCollectorsOutput {
    var result: ListCollectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCollectorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
