...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBrokerInput, options: Options) !UpdateBrokerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBrokerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBrokerOutput {
    var result: UpdateBrokerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBrokerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
