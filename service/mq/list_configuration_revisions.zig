...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConfigurationRevisionsInput, options: Options) !ListConfigurationRevisionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConfigurationRevisionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConfigurationRevisionsOutput {
    var result: ListConfigurationRevisionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConfigurationRevisionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
