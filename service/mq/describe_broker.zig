...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBrokerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBrokerOutput {
    var result: DescribeBrokerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBrokerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
