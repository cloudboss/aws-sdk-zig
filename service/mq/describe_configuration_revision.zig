...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConfigurationRevisionInput, options: Options) !DescribeConfigurationRevisionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConfigurationRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.configuration_revision);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConfigurationRevisionOutput {
    var result: DescribeConfigurationRevisionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeConfigurationRevisionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
