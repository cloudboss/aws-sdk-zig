...
pub const GetIntrospectionSchemaInput = struct {
    /// The API ID.
    api_id: []const u8,
...
pub const GetIntrospectionSchemaOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIntrospectionSchemaInput, options: CallOptions) !GetIntrospectionSchemaOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIntrospectionSchemaInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/schema");
    ...
    query_has_prev = true;
    if (input.include_directives) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "includeDirectives=");
