...
pub const ListTypesByAssociationInput = struct {
...
pub const ListTypesByAssociationOutput = struct {
    /// An identifier that was returned from the previous call to this operation,
    /// which you can
    /// use to return the next set of items in the list.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTypesByAssociationInput, options: Options) !ListTypesByAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTypesByAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTypesByAssociationOutput {
    var result: ListTypesByAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTypesByAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
