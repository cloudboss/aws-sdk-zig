...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutGraphqlApiEnvironmentVariablesInput, options: Options) !PutGraphqlApiEnvironmentVariablesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutGraphqlApiEnvironmentVariablesInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.environment_variables), input.environment_variables, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutGraphqlApiEnvironmentVariablesOutput {
    var result: PutGraphqlApiEnvironmentVariablesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutGraphqlApiEnvironmentVariablesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
