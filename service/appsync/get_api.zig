...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApiInput, options: Options) !GetApiOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApiInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApiOutput {
    var result: GetApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetApiOutput, body, alloc);
    }
    _ = status;
    _ = headers;
