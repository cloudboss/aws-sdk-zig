...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDataSourceIntrospectionInput, options: Options) !StartDataSourceIntrospectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDataSourceIntrospectionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.rds_data_api_config) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"rdsDataApiConfig\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDataSourceIntrospectionOutput {
    var result: StartDataSourceIntrospectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDataSourceIntrospectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
