...
pub const StartSchemaCreationInput = struct {
    /// The API ID.
    api_id: []const u8,
...
pub const StartSchemaCreationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSchemaCreationInput, options: Options) !StartSchemaCreationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSchemaCreationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/schemacreation");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSchemaCreationOutput {
    var result: StartSchemaCreationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartSchemaCreationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
