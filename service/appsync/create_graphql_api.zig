...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGraphqlApiInput, options: CallOptions) !CreateGraphqlApiOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateGraphqlApiInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.api_type) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"apiType\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
