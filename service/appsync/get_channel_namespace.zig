...
pub const GetChannelNamespaceInput = struct {
    /// The `Api` ID.
    api_id: []const u8,
...
pub const GetChannelNamespaceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetChannelNamespaceInput, options: Options) !GetChannelNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChannelNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChannelNamespaceOutput {
    var result: GetChannelNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChannelNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
