...
pub const GetApiAssociationInput = struct {
    /// The domain name.
...
pub const GetApiAssociationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApiAssociationInput, options: Options) !GetApiAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApiAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApiAssociationOutput {
    var result: GetApiAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetApiAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
