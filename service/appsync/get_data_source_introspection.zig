...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataSourceIntrospectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/datasources/introspections/");
    try path_buf.appendSlice(alloc, input.introspection_id);
    ...
    if (input.include_models_sdl) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeModelsSDL=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataSourceIntrospectionOutput {
    var result: GetDataSourceIntrospectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataSourceIntrospectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
