...
pub const ListResolversByFunctionInput = struct {
    /// The API ID.
    api_id: []const u8,
...
pub const ListResolversByFunctionOutput = struct {
    /// An identifier that you can use to return the next set of items in the list.
    next_token: ?[]const u8 = null,

    /// The list of resolvers.
    resolvers: ?[]const Resolver = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResolversByFunctionInput, options: Options) !ListResolversByFunctionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResolversByFunctionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResolversByFunctionOutput {
    var result: ListResolversByFunctionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResolversByFunctionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
