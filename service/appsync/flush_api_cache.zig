...
pub const FlushApiCacheInput = struct {
    /// The API ID.
    api_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: FlushApiCacheInput, options: CallOptions) !FlushApiCacheOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: FlushApiCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/FlushCache");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !FlushApiCacheOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: FlushApiCacheOutput = .{};
