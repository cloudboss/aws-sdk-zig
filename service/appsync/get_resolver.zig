...
pub const GetResolverInput = struct {
    /// The API ID.
    api_id: []const u8,
...
pub const GetResolverOutput = struct {
    /// The `Resolver` object.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResolverInput, options: CallOptions) !GetResolverOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResolverInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResolverOutput {
    var result: GetResolverOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResolverOutput, body, allocator);
    }
    _ = status;
    _ = headers;
