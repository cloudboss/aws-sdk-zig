...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResolverInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResolverOutput {
    var result: GetResolverOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResolverOutput, body, alloc);
    }
    _ = status;
    _ = headers;
