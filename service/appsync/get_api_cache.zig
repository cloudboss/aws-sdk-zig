...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApiCacheInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApiCacheOutput {
    var result: GetApiCacheOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetApiCacheOutput, body, alloc);
    }
    _ = status;
    _ = headers;
