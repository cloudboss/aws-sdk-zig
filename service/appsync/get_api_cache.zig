...
pub const GetApiCacheInput = struct {
    /// The API ID.
    api_id: []const u8,
...
pub const GetApiCacheOutput = struct {
    /// The `ApiCache` object.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApiCacheInput, options: CallOptions) !GetApiCacheOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetApiCacheInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetApiCacheOutput {
    var result: GetApiCacheOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetApiCacheOutput, body, allocator);
    }
    _ = status;
    _ = headers;
