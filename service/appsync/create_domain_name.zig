...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDomainNameInput, options: Options) !CreateDomainNameOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDomainNameInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.certificate_arn), input.certificate_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDomainNameOutput {
    var result: CreateDomainNameOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDomainNameOutput, body, alloc);
    }
    _ = status;
    _ = headers;
