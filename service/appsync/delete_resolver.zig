...
pub const DeleteResolverInput = struct {
    /// The API ID.
    api_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResolverInput, options: Options) !DeleteResolverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteResolverInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.field_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteResolverOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteResolverOutput = .{};
