...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSchemaCreationStatusInput, options: Options) !GetSchemaCreationStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSchemaCreationStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSchemaCreationStatusOutput {
    var result: GetSchemaCreationStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSchemaCreationStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
