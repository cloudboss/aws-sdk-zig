...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGraphqlApiInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGraphqlApiOutput {
    var result: GetGraphqlApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGraphqlApiOutput, body, alloc);
    }
    _ = status;
    _ = headers;
