...
pub const GetGraphqlApiInput = struct {
...
pub const GetGraphqlApiOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGraphqlApiInput, options: CallOptions) !GetGraphqlApiOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetGraphqlApiInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetGraphqlApiOutput {
    var result: GetGraphqlApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGraphqlApiOutput, body, allocator);
    }
    _ = status;
    _ = headers;
