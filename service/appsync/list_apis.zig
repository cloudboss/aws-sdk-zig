...
pub const ListApisOutput = struct {
    /// The `Api` objects.
    apis: ?[]const Api = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApisInput, options: Options) !ListApisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApisInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApisOutput {
    var result: ListApisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListApisOutput, body, alloc);
    }
    _ = status;
    _ = headers;
