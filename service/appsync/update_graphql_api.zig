...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGraphqlApiInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGraphqlApiOutput {
    var result: UpdateGraphqlApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGraphqlApiOutput, body, alloc);
    }
    _ = status;
    _ = headers;
