...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSourceApiAssociationsInput, options: Options) !ListSourceApiAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSourceApiAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sourceApiAssociations");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSourceApiAssociationsOutput {
    var result: ListSourceApiAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSourceApiAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
