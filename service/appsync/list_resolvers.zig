...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResolversInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResolversOutput {
    var result: ListResolversOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResolversOutput, body, alloc);
    }
    _ = status;
    _ = headers;
