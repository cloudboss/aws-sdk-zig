...
pub const ListResolversInput = struct {
    /// The API ID.
    api_id: []const u8,
...
pub const ListResolversOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResolversInput, options: CallOptions) !ListResolversOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListResolversInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListResolversOutput {
    var result: ListResolversOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResolversOutput, body, allocator);
    }
    _ = status;
    _ = headers;
