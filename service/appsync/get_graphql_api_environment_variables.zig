...
pub const GetGraphqlApiEnvironmentVariablesInput = struct {
...
pub const GetGraphqlApiEnvironmentVariablesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGraphqlApiEnvironmentVariablesInput, options: Options) !GetGraphqlApiEnvironmentVariablesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGraphqlApiEnvironmentVariablesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/environmentVariables");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGraphqlApiEnvironmentVariablesOutput {
    var result: GetGraphqlApiEnvironmentVariablesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGraphqlApiEnvironmentVariablesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
