...
pub const DeleteGraphqlApiInput = struct {
    /// The API ID.
    api_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGraphqlApiInput, options: Options) !DeleteGraphqlApiOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteGraphqlApiInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteGraphqlApiOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteGraphqlApiOutput = .{};
