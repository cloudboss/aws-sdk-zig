...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelNamespacesInput, options: CallOptions) !ListChannelNamespacesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListChannelNamespacesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListChannelNamespacesOutput {
    var result: ListChannelNamespacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelNamespacesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
