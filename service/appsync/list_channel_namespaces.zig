...
pub const ListChannelNamespacesInput = struct {
    /// The `Api` ID.
    api_id: []const u8,
...
pub const ListChannelNamespacesOutput = struct {
    /// The `ChannelNamespace` objects.
    channel_namespaces: ?[]const ChannelNamespace = null,
    ...
    pub const json_field_names = .{
        .channel_namespaces = "channelNamespaces",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelNamespacesInput, options: Options) !ListChannelNamespacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChannelNamespacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/channelNamespaces");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChannelNamespacesOutput {
    var result: ListChannelNamespacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelNamespacesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
