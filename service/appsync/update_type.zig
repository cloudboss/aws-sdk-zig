...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTypeInput, options: Options) !UpdateTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTypeOutput {
    var result: UpdateTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
