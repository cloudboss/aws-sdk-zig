...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EvaluateMappingTemplateInput, options: CallOptions) !EvaluateMappingTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EvaluateMappingTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EvaluateMappingTemplateOutput {
    var result: EvaluateMappingTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EvaluateMappingTemplateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
