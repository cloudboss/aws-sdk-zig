...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateApiInput, options: Options) !AssociateApiOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateApiInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/apiassociation");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateApiOutput {
    var result: AssociateApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateApiOutput, body, alloc);
    }
    _ = status;
    _ = headers;
