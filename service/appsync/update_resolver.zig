...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResolverInput, options: Options) !UpdateResolverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResolverInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResolverOutput {
    var result: UpdateResolverOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResolverOutput, body, alloc);
    }
    _ = status;
    _ = headers;
