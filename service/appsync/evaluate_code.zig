...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EvaluateCodeInput, options: Options) !EvaluateCodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EvaluateCodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.context), input.context, alloc, &body_buf);
    has_prev = true;
    if (input.function) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.runtime), input.runtime, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EvaluateCodeOutput {
    var result: EvaluateCodeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EvaluateCodeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
