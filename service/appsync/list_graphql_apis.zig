...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGraphqlApisInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.api_type) |v| {
        ...
        try query_buf.appendSlice(alloc, "apiType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGraphqlApisOutput {
    var result: ListGraphqlApisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGraphqlApisOutput, body, alloc);
    }
    _ = status;
    _ = headers;
