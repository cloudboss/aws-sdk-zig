...
pub const CreateApiCacheInput = struct {
    /// Caching behavior.
    ///
...
pub const CreateApiCacheOutput = struct {
    /// The `ApiCache` object.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateApiCacheInput, options: Options) !CreateApiCacheOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApiCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ApiCaches");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateApiCacheOutput {
    var result: CreateApiCacheOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateApiCacheOutput, body, alloc);
    }
    _ = status;
    _ = headers;
