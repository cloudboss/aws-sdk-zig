...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChannelNamespaceInput, options: Options) !CreateChannelNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateChannelNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateChannelNamespaceOutput {
    var result: CreateChannelNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChannelNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
