...
pub const UpdateChannelNamespaceInput = struct {
    /// The `Api` ID.
    api_id: []const u8,
...
pub const UpdateChannelNamespaceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateChannelNamespaceInput, options: Options) !UpdateChannelNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChannelNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.code_handlers) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"codeHandlers\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.handler_configs) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"handlerConfigs\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.publish_auth_modes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"publishAuthModes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.subscribe_auth_modes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"subscribeAuthModes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChannelNamespaceOutput {
    var result: UpdateChannelNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChannelNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
