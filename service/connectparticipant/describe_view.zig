...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeViewInput, options: Options) !DescribeViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeViewInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/participant/views/");
    try path_buf.appendSlice(alloc, input.view_token);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeViewOutput {
    var result: DescribeViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeViewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
