...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAttachmentUploadInput, options: Options) !StartAttachmentUploadOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartAttachmentUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AttachmentName\":");
    try aws.json.writeValue(@TypeOf(input.attachment_name), input.attachment_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"AttachmentSizeInBytes\":");
    try aws.json.writeValue(@TypeOf(input.attachment_size_in_bytes), input.attachment_size_in_bytes, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartAttachmentUploadOutput {
    var result: StartAttachmentUploadOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartAttachmentUploadOutput, body, alloc);
    }
    _ = status;
    _ = headers;
