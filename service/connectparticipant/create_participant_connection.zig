...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateParticipantConnectionInput, options: Options) !CreateParticipantConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateParticipantConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connect_participant) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ConnectParticipant\":");
    ...
    try request.headers.put(alloc, "X-Amz-Bearer", input.participant_token);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateParticipantConnectionOutput {
    var result: CreateParticipantConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateParticipantConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
