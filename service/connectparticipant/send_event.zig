...
pub const SendEventInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendEventInput, options: Options) !SendEventOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SendEventInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SendEventOutput {
    var result: SendEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendEventOutput, body, allocator);
    }
    _ = status;
    _ = headers;
