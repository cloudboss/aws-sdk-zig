...
fn serializeRequest(alloc: std.mem.Allocator, input: SendEventInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendEventOutput {
    var result: SendEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
