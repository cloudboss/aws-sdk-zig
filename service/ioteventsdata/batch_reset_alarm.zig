...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchResetAlarmInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"resetActionRequests\":");
    try aws.json.writeValue(@TypeOf(input.reset_action_requests), input.reset_action_requests, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchResetAlarmOutput {
    var result: BatchResetAlarmOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchResetAlarmOutput, body, alloc);
    }
    _ = status;
    _ = headers;
