...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: QueryForecastInput, options: Options) !QueryForecastOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: QueryForecastInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("forecastquery", "forecastquery", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecastRuntime.QueryForecast");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !QueryForecastOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(QueryForecastOutput, body, alloc);
}
