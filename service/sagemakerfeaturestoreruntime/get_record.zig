...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRecordInput, options: Options) !GetRecordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRecordInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("sagemakerfeaturestoreruntime", "SageMaker FeatureStore Runtime", alloc);
    ...
    try path_buf.appendSlice(alloc, "/FeatureGroup/");
    try path_buf.appendSlice(alloc, input.feature_group_name);
    ...
    if (input.expiration_time_response) |v| {
        ...
        try query_buf.appendSlice(alloc, "ExpirationTimeResponse=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.feature_names) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "FeatureName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "RecordIdentifierValueAsString=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.record_identifier_value_as_string);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRecordOutput {
    var result: GetRecordOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRecordOutput, body, alloc);
    }
    _ = status;
    _ = headers;
