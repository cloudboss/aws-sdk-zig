...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRecordInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.deletion_mode) |v| {
        ...
        try query_buf.appendSlice(alloc, "DeletionMode=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "EventTime=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.event_time);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    ...
    if (input.target_stores) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "TargetStores=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRecordOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
