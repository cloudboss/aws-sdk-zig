...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetRecordInput, options: Options) !BatchGetRecordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetRecordInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.expiration_time_response) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ExpirationTimeResponse\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetRecordOutput {
    var result: BatchGetRecordOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetRecordOutput, body, alloc);
    }
    _ = status;
    _ = headers;
