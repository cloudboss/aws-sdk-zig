...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetIpAddressTypeInput, options: Options) !SetIpAddressTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetIpAddressTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetIpAddressType&Version=2015-12-01");
    try body_buf.appendSlice(alloc, "&IpAddressType=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.ip_address_type));
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetIpAddressTypeOutput {
    _ = status;
    _ = headers;
    _ = alloc;
