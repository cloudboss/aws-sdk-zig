...
pub const LoadBalancerAvailableWaiter = struct {
    client: *Client,
    ...
    fn poll(self: *Self) aws.waiter.AcceptorState {
        ...
        _ = self.client.describeLoadBalancers(arena.allocator(), self.params, .{ .diagnostic = &diagnostic }) catch |err| {
            if (err == error.ServiceError) {
                defer diagnostic.deinit();
                if (std.mem.eql(u8, diagnostic.code(), "LoadBalancerNotFound")) {
                    return .retry;
                }
            }
            return .retry;
...
pub const LoadBalancerExistsWaiter = struct {
    client: *Client,
    params: describe_load_balancers.DescribeLoadBalancersInput,
    config: aws.waiter.WaiterConfig = .{
        .min_delay_s = 15,
...
pub const LoadBalancersDeletedWaiter = struct {
    client: *Client,
    params: describe_load_balancers.DescribeLoadBalancersInput,
    config: aws.waiter.WaiterConfig = .{
        .min_delay_s = 15,
...
pub const TargetDeregisteredWaiter = struct {
    client: *Client,
    params: describe_target_health.DescribeTargetHealthInput,
    config: aws.waiter.WaiterConfig = .{
        .min_delay_s = 15,
        .max_delay_s = 120,
        .max_wait_time_s = 300,
    ...
    fn poll(self: *Self) aws.waiter.AcceptorState {
        ...
        var diagnostic: @import("errors.zig").ServiceError = undefined;
        _ = self.client.describeTargetHealth(arena.allocator(), self.params, .{ .diagnostic = &diagnostic }) catch |err| {
            if (err == error.ServiceError) {
                defer diagnostic.deinit();
                if (std.mem.eql(u8, diagnostic.code(), "InvalidTarget")) {
                    return .success;
                }
            }
            return .retry;
...
pub const TargetInServiceWaiter = struct {
    client: *Client,
    params: describe_target_health.DescribeTargetHealthInput,
    config: aws.waiter.WaiterConfig = .{
        .min_delay_s = 15,
