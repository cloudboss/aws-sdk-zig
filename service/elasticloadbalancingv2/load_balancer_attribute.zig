...
pub const LoadBalancerAttribute = struct {
    ...
    /// The default for Network Load Balancers and Gateway Load Balancers is
    /// `false`.
    ...
    /// This attribute is required if access logs are enabled. The bucket must exist
    /// in the same
    /// region as the load balancer and have a bucket policy that grants Elastic
    /// Load Balancing
    ...
    ///   logs.
    /// This attribute is required if connection logs are enabled. The bucket must
    /// exist in the same
    /// region as the load balancer and have a bucket policy that grants Elastic
    /// Load Balancing
    /// permissions to write to the bucket.
    ...
    ///   check logs.
    /// This attribute is required if health check logs are enabled. The bucket must
    /// exist in the same
    /// region as the load balancer and have a bucket policy that grants Elastic
    /// Load Balancing
    /// permissions to write to the bucket.
    ...
    /// Application Load Balancer should preserve the `Host` header in the HTTP
    /// request
    ...
    /// TLS version and cipher suite, are added to the client request before sending
    /// it to the
    /// target. The `x-amzn-tls-version` header has information about the TLS
    /// protocol
    ...
    /// `X-Forwarded-For` header should preserve the source port that the client
    /// used
    ...
    /// preserve, or remove the `X-Forwarded-For` header in the HTTP request before
    /// the
    /// Application Load Balancer sends the request to the target. The possible
    /// values are
    ...
    ///   load
    /// balancer using HTTP/2. If `true`, clients can connect using HTTP/2 or
    /// HTTP/1.1.
    /// However, all client requests are subject to the stricter HTTP/2 header
    /// validation rules.
    /// For example, message header names must contain only alphanumeric characters
    /// and hyphens.
