...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTrustStoreRevocationContentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetTrustStoreRevocationContent&Version=2015-12-01");
    try body_buf.appendSlice(alloc, "&RevocationId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.revocation_id}) catch "");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTrustStoreRevocationContentOutput {
    _ = status;
    _ = headers;
