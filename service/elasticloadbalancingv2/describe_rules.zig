...
pub const DescribeRulesInput = struct {
    ...
    rule_arns: ?[]const []const u8 = null,
};

pub const DescribeRulesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRulesInput, options: CallOptions) !DescribeRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeRules&Version=2015-12-01");
    if (input.listener_arn) |v| {
    ...
    if (input.rule_arns) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&RuleArns.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeRulesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeRulesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeRulesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
