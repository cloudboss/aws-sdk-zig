...
pub const DescribeListenerCertificatesInput = struct {
...
pub const DescribeListenerCertificatesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeListenerCertificatesInput, options: Options) !DescribeListenerCertificatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeListenerCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeListenerCertificates&Version=2015-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeListenerCertificatesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeListenerCertificatesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeListenerCertificatesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Certificates")) {
                    result.certificates = try serde.deserializeCertificateList(&reader, alloc, "member");
