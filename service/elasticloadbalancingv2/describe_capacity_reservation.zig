...
pub const DescribeCapacityReservationInput = struct {
    /// The Amazon Resource Name (ARN) of the load balancer.
    load_balancer_arn: []const u8,
};

pub const DescribeCapacityReservationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCapacityReservationInput, options: Options) !DescribeCapacityReservationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCapacityReservationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCapacityReservation&Version=2015-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCapacityReservationOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeCapacityReservationResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeCapacityReservationOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CapacityReservationState")) {
                    result.capacity_reservation_state = try serde.deserializeZonalCapacityReservationStates(&reader, alloc, "member");
                    ...
                    result.minimum_load_balancer_capacity = try serde.deserializeMinimumLoadBalancerCapacity(&reader, alloc);
                } else {
