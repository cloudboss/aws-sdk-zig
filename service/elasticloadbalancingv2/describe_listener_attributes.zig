...
pub const DescribeListenerAttributesInput = struct {
    /// The Amazon Resource Name (ARN) of the listener.
    listener_arn: []const u8,
};

pub const DescribeListenerAttributesOutput = struct {
    /// Information about the listener attributes.
    attributes: ?[]const ListenerAttribute = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeListenerAttributesInput, options: CallOptions) !DescribeListenerAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeListenerAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeListenerAttributes&Version=2015-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeListenerAttributesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeListenerAttributesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeListenerAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Attributes")) {
                    result.attributes = try serde.deserializeListenerAttributes(allocator, &reader, "member");
                } else {
