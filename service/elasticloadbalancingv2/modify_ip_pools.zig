...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyIpPoolsInput, options: Options) !ModifyIpPoolsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyIpPoolsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyIpPools&Version=2015-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyIpPoolsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IpamPools")) {
                    result.ipam_pools = try serde.deserializeIpamPools(&reader, alloc);
                } else {
