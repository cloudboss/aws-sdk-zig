...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustStoreRevocationsInput, options: Options) !DescribeTrustStoreRevocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrustStoreRevocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTrustStoreRevocations&Version=2015-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrustStoreRevocationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextMarker")) {
                    ...
                    result.trust_store_revocations = try serde.deserializeDescribeTrustStoreRevocationResponse(&reader, alloc, "member");
                } else {
