...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyCapacityReservationInput, options: Options) !ModifyCapacityReservationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyCapacityReservationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyCapacityReservation&Version=2015-12-01");
    ...
    if (input.minimum_load_balancer_capacity) |v| {
        if (v.capacity_units) |sv| {
            try body_buf.appendSlice(alloc, "&MinimumLoadBalancerCapacity.CapacityUnits=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{sv}) catch "");
        }
    }
    if (input.reset_capacity_reservation) |v| {
        try body_buf.appendSlice(alloc, "&ResetCapacityReservation=");
