...
pub const DescribeSSLPoliciesInput = struct {
    /// The type of load balancer. The default lists the SSL policies for all load
    /// balancers.
    load_balancer_type: ?LoadBalancerTypeEnum = null,
...
pub const DescribeSSLPoliciesOutput = struct {
    ...
    ssl_policies: ?[]const SslPolicy = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSSLPoliciesInput, options: CallOptions) !DescribeSSLPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSSLPoliciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSSLPoliciesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeSSLPoliciesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeSSLPoliciesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextMarker")) {
                    result.next_marker = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "SslPolicies")) {
                    result.ssl_policies = try serde.deserializeSslPolicies(allocator, &reader, "member");
                } else {
