...
pub const DescribeLoadBalancerAttributesInput = struct {
    /// The Amazon Resource Name (ARN) of the load balancer.
    load_balancer_arn: []const u8,
};

pub const DescribeLoadBalancerAttributesOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLoadBalancerAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeLoadBalancerAttributes&Version=2015-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLoadBalancerAttributesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeLoadBalancerAttributesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeLoadBalancerAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Attributes")) {
                    result.attributes = try serde.deserializeLoadBalancerAttributes(&reader, alloc, "member");
                } else {
