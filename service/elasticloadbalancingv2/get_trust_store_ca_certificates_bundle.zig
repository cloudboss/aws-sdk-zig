...
pub const GetTrustStoreCaCertificatesBundleInput = struct {
    /// The Amazon Resource Name (ARN) of the trust store.
    trust_store_arn: []const u8,
};

pub const GetTrustStoreCaCertificatesBundleOutput = struct {
    /// The ca certificate bundles Amazon S3 URI.
    location: ?[]const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTrustStoreCaCertificatesBundleInput, options: CallOptions) !GetTrustStoreCaCertificatesBundleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTrustStoreCaCertificatesBundleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetTrustStoreCaCertificatesBundle&Version=2015-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTrustStoreCaCertificatesBundleOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetTrustStoreCaCertificatesBundleResult")) break;
            },
            else => {},
        }
    }

    var result: GetTrustStoreCaCertificatesBundleOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
