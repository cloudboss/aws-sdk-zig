...
pub const DescribeTargetGroupAttributesInput = struct {
    /// The Amazon Resource Name (ARN) of the target group.
    target_group_arn: []const u8,
};

pub const DescribeTargetGroupAttributesOutput = struct {
    /// Information about the target group attributes
    attributes: ?[]const TargetGroupAttribute = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTargetGroupAttributesInput, options: CallOptions) !DescribeTargetGroupAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTargetGroupAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTargetGroupAttributes&Version=2015-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTargetGroupAttributesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeTargetGroupAttributesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeTargetGroupAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
