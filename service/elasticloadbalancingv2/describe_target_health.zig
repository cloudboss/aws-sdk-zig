...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTargetHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTargetHealth&Version=2015-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTargetHealthOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TargetHealthDescriptions")) {
                    result.target_health_descriptions = try serde.deserializeTargetHealthDescriptions(&reader, alloc, "member");
                } else {
