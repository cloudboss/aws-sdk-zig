...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetSubnetsInput, options: Options) !SetSubnetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetSubnetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetSubnets&Version=2015-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetSubnetsOutput {
    _ = status;
    _ = headers;
