...
pub const ModifyTargetGroupAttributesInput = struct {
    /// The target group attributes.
    attributes: []const TargetGroupAttribute,

    /// The Amazon Resource Name (ARN) of the target group.
    target_group_arn: []const u8,
};

pub const ModifyTargetGroupAttributesOutput = struct {
    /// Information about the target group attributes.
    attributes: ?[]const TargetGroupAttribute = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyTargetGroupAttributesInput, options: CallOptions) !ModifyTargetGroupAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyTargetGroupAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyTargetGroupAttributes&Version=2015-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyTargetGroupAttributesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ModifyTargetGroupAttributesResult")) break;
            },
            else => {},
        }
    }

    var result: ModifyTargetGroupAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Attributes")) {
                    result.attributes = try serde.deserializeTargetGroupAttributes(allocator, &reader, "member");
                } else {
