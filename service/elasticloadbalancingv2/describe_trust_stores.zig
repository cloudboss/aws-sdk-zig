...
pub const DescribeTrustStoresInput = struct {
    ...
    trust_store_arns: ?[]const []const u8 = null,
};

pub const DescribeTrustStoresOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustStoresInput, options: Options) !DescribeTrustStoresOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrustStoresInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTrustStores&Version=2015-12-01");
    if (input.marker) |v| {
    ...
    if (input.trust_store_arns) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&TrustStoreArns.member.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try aws.url.appendUrlEncoded(alloc, &body_buf, item);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrustStoresOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeTrustStoresResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeTrustStoresOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextMarker")) {
                    result.next_marker = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "TrustStores")) {
