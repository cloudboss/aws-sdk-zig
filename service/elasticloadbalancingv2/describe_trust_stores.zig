...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustStoresInput, options: CallOptions) !DescribeTrustStoresOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTrustStoresInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTrustStores&Version=2015-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTrustStoresOutput {
    _ = status;
    _ = headers;
