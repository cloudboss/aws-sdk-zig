...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLoadBalancersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeLoadBalancers&Version=2015-12-01");
    if (input.load_balancer_arns) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&LoadBalancerArns.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLoadBalancersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LoadBalancers")) {
                    result.load_balancers = try serde.deserializeLoadBalancers(&reader, alloc, "member");
