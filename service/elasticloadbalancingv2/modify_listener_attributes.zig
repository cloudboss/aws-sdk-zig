...
pub const ModifyListenerAttributesInput = struct {
    /// The listener attributes.
    attributes: []const ListenerAttribute,

    /// The Amazon Resource Name (ARN) of the listener.
    listener_arn: []const u8,
};

pub const ModifyListenerAttributesOutput = struct {
    /// Information about the listener attributes.
    attributes: ?[]const ListenerAttribute = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyListenerAttributesInput, options: CallOptions) !ModifyListenerAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyListenerAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyListenerAttributes&Version=2015-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyListenerAttributesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ModifyListenerAttributesResult")) break;
            },
            else => {},
        }
    }

    var result: ModifyListenerAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
