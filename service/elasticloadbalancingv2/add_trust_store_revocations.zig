...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddTrustStoreRevocationsInput, options: Options) !AddTrustStoreRevocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddTrustStoreRevocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AddTrustStoreRevocations&Version=2015-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddTrustStoreRevocationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TrustStoreRevocations")) {
                    result.trust_store_revocations = try serde.deserializeTrustStoreRevocations(&reader, alloc, "member");
                } else {
