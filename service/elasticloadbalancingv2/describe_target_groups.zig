...
pub const DescribeTargetGroupsInput = struct {
...
pub const DescribeTargetGroupsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTargetGroupsInput, options: CallOptions) !DescribeTargetGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTargetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTargetGroups&Version=2015-12-01");
    ...
    if (input.target_group_arns) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTargetGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeTargetGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeTargetGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
