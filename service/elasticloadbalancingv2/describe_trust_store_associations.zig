...
pub const DescribeTrustStoreAssociationsInput = struct {
...
pub const DescribeTrustStoreAssociationsOutput = struct {
    ...
    trust_store_associations: ?[]const TrustStoreAssociation = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustStoreAssociationsInput, options: Options) !DescribeTrustStoreAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrustStoreAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTrustStoreAssociations&Version=2015-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrustStoreAssociationsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeTrustStoreAssociationsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeTrustStoreAssociationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextMarker")) {
                    result.next_marker = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "TrustStoreAssociations")) {
                    result.trust_store_associations = try serde.deserializeTrustStoreAssociations(&reader, alloc, "member");
                } else {
