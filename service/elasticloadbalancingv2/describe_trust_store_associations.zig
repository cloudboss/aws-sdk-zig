...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustStoreAssociationsInput, options: CallOptions) !DescribeTrustStoreAssociationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTrustStoreAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTrustStoreAssociations&Version=2015-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTrustStoreAssociationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextMarker")) {
                    ...
                    result.trust_store_associations = try serde.deserializeTrustStoreAssociations(allocator, &reader, "member");
                } else {
