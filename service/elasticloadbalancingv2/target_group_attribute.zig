...
pub const TargetGroupAttribute = struct {
    /// The name of the attribute.
    ///
    /// The following attributes are supported by all load balancers:
    ///
    /// * `deregistration_delay.timeout_seconds` - The amount of time, in seconds,
    /// for Elastic Load Balancing to wait before changing the state of a
    /// deregistering target
    /// from `draining` to `unused`. The range is 0-3600 seconds. The
    /// default value is 300 seconds. If the target is a Lambda function, this
    /// attribute is not
    /// supported.
    ///
    ...
    /// unhealthy
    /// in DNS, so that traffic is routed only to healthy zones. The possible values
    /// are
    ...
    /// The minimum number of targets that must be healthy.
    /// If the number of healthy targets is below this value, send traffic to all
    /// targets, including unhealthy targets.
    /// The possible values are 1 to the maximum number of targets. The default is
    /// 1.
    ///
    /// *
    ///   `target_group_health.unhealthy_state_routing.minimum_healthy_targets.percentage` -
    /// The minimum percentage of targets that must be healthy.
    /// If the percentage of healthy targets is below this value, send traffic to
    /// all targets, including unhealthy targets.
    ...
    /// `round_robin`, `least_outstanding_requests`, or `weighted_random`. The
    /// default is
    /// `round_robin`.
    ///
    ...
    /// After this time
    /// period expires, the application-based cookie is considered stale. The range
    /// is 1 second to
    ...
    /// during which requests from a client should be routed to the same target.
    /// After this time
    /// period expires, the load balancer-generated cookie is considered stale. The
    /// range is 1
    ...
    /// the target group type is IP address and the target group protocol is TCP or
    /// TLS.
    ...
    ///   whether
    /// the load balancer terminates connections to unhealthy targets. The value is
    /// `true`
    /// or `false`. The default is `true`. This attribute can't be enabled for UDP
    /// and
    /// TCP_UDP target groups.
    ///
