...
fn serializeRequest(alloc: std.mem.Allocator, input: SetSecurityGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetSecurityGroups&Version=2015-12-01");
    if (input.enforce_security_group_inbound_rules_on_private_link_traffic) |v| {
        try body_buf.appendSlice(alloc, "&EnforceSecurityGroupInboundRulesOnPrivateLinkTraffic=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetSecurityGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EnforceSecurityGroupInboundRulesOnPrivateLinkTraffic")) {
                    ...
                    result.security_group_ids = try serde.deserializeSecurityGroups(&reader, alloc, "member");
                } else {
