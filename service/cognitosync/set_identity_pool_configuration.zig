...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetIdentityPoolConfigurationInput, options: CallOptions) !SetIdentityPoolConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetIdentityPoolConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cognito_streams) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"CognitoStreams\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.push_sync) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"PushSync\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
