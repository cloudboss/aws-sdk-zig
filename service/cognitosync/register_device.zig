...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterDeviceInput, options: Options) !RegisterDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/identity/");
    try path_buf.appendSlice(alloc, input.identity_id);
    try path_buf.appendSlice(alloc, "/device");
    ...
    try aws.json.writeValue(@TypeOf(input.token), input.token, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterDeviceOutput {
    var result: RegisterDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
