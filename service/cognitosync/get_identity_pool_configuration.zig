...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentityPoolConfigurationInput, options: Options) !GetIdentityPoolConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityPoolConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityPoolConfigurationOutput {
    var result: GetIdentityPoolConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIdentityPoolConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
