...
pub const SetCognitoEventsInput = struct {
...
pub const SetCognitoEventsOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetCognitoEventsInput, options: CallOptions) !SetCognitoEventsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "cognito-sync");
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetCognitoEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetCognitoEventsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: SetCognitoEventsOutput = .{};
