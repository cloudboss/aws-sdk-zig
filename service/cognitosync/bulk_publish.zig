...
fn serializeRequest(alloc: std.mem.Allocator, input: BulkPublishInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/bulkpublish");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BulkPublishOutput {
    var result: BulkPublishOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BulkPublishOutput, body, alloc);
    }
    _ = status;
    _ = headers;
