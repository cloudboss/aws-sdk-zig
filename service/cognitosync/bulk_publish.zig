...
pub const BulkPublishInput = struct {
...
pub const BulkPublishOutput = struct {
    /// A name-spaced GUID (for example,
    /// us-east-1:23EC4050-6AEA-7089-A2DD-08002EXAMPLE) created by Amazon Cognito.
    /// GUID generation is
    /// unique within a region.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BulkPublishInput, options: CallOptions) !BulkPublishOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BulkPublishInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/bulkpublish");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BulkPublishOutput {
    var result: BulkPublishOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BulkPublishOutput, body, allocator);
    }
    _ = status;
    _ = headers;
