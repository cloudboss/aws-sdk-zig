...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIdentityPoolUsageInput, options: Options) !ListIdentityPoolUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdentityPoolUsageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdentityPoolUsageOutput {
    var result: ListIdentityPoolUsageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIdentityPoolUsageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
