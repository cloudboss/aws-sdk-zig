...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRecordsInput, options: Options) !UpdateRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.record_patches) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecordPatches\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SyncSessionToken\":");
    try aws.json.writeValue(@TypeOf(input.sync_session_token), input.sync_session_token, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRecordsOutput {
    var result: UpdateRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
