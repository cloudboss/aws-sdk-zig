...
pub const GetCognitoEventsInput = struct {
...
pub const GetCognitoEventsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCognitoEventsInput, options: CallOptions) !GetCognitoEventsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCognitoEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCognitoEventsOutput {
    var result: GetCognitoEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCognitoEventsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
