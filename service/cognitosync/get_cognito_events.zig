...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCognitoEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCognitoEventsOutput {
    var result: GetCognitoEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCognitoEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
