...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIdentityUsageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIdentityUsageOutput {
    var result: DescribeIdentityUsageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeIdentityUsageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
