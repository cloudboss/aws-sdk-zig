...
const IdentityUsage = @import("identity_usage.zig").IdentityUsage;

pub const DescribeIdentityUsageInput = struct {
...
pub const DescribeIdentityUsageOutput = struct {
    /// Usage information for the
    /// identity.
    identity_usage: ?IdentityUsage = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIdentityUsageInput, options: CallOptions) !DescribeIdentityUsageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeIdentityUsageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeIdentityUsageOutput {
    var result: DescribeIdentityUsageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeIdentityUsageOutput, body, allocator);
    }
    _ = status;
    _ = headers;
