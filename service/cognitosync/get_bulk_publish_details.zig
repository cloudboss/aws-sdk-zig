...
const BulkPublishStatus = @import("bulk_publish_status.zig").BulkPublishStatus;

pub const GetBulkPublishDetailsInput = struct {
...
pub const GetBulkPublishDetailsOutput = struct {
    /// If BulkPublishStatus is SUCCEEDED, the time the last bulk publish operation
    /// completed.
    bulk_publish_complete_time: ?i64 = null,

    /// The date/time at which the last bulk publish was initiated.
    bulk_publish_start_time: ?i64 = null,
    ...
    bulk_publish_status: ?BulkPublishStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBulkPublishDetailsInput, options: Options) !GetBulkPublishDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBulkPublishDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/getBulkPublishDetails");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBulkPublishDetailsOutput {
    var result: GetBulkPublishDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBulkPublishDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
