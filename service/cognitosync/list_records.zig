...
pub const ListRecordsInput = struct {
    ...
    sync_session_token: ?[]const u8 = null,
...
pub const ListRecordsOutput = struct {
    ...
    /// Names of merged
    /// datasets.
    merged_dataset_names: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecordsInput, options: Options) !ListRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/identities/");
    try path_buf.appendSlice(alloc, input.identity_id);
    try path_buf.appendSlice(alloc, "/datasets/");
    try path_buf.appendSlice(alloc, input.dataset_name);
    try path_buf.appendSlice(alloc, "/records");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.last_sync_count) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "lastSyncCount=");
        {
    ...
    if (input.sync_session_token) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "syncSessionToken=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecordsOutput {
    var result: ListRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
