...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIdentityPoolUsageInput, options: Options) !DescribeIdentityPoolUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIdentityPoolUsageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIdentityPoolUsageOutput {
    var result: DescribeIdentityPoolUsageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeIdentityPoolUsageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
