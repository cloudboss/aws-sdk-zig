...
const bulk_publish = @import("bulk_publish.zig");
const delete_dataset = @import("delete_dataset.zig");
const describe_dataset = @import("describe_dataset.zig");
const describe_identity_pool_usage = @import("describe_identity_pool_usage.zig");
const describe_identity_usage = @import("describe_identity_usage.zig");
const get_bulk_publish_details = @import("get_bulk_publish_details.zig");
const get_cognito_events = @import("get_cognito_events.zig");
const get_identity_pool_configuration = @import("get_identity_pool_configuration.zig");
const list_datasets = @import("list_datasets.zig");
const list_identity_pool_usage = @import("list_identity_pool_usage.zig");
const list_records = @import("list_records.zig");
const register_device = @import("register_device.zig");
const set_cognito_events = @import("set_cognito_events.zig");
const set_identity_pool_configuration = @import("set_identity_pool_configuration.zig");
...
pub const Client = struct {
    ...
    /// per 24 hours. Bulk publish is an asynchronous request, customers can see the
    ...
    /// the action can't
    /// be undone. Datasets that this dataset was merged with will no longer report
    /// the merge. Any
    ...
    /// a dataset and
    /// identity. With Amazon Cognito Sync, each identity has access only to its own
    /// data. Thus,
    ...
    /// will be rejected. To obtain the current sync count for a record, call
    /// ListRecords. On a successful update of the record, the response returns the
    /// new sync count for that record. You should present that sync count the next
    /// time you try to update that same record. When the record does not exist,
