...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteScheduleInput, options: Options) !DeleteScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.group_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "groupName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteScheduleOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
