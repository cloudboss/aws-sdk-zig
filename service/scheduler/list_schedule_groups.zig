...
fn serializeRequest(alloc: std.mem.Allocator, input: ListScheduleGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListScheduleGroupsOutput {
    var result: ListScheduleGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListScheduleGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
