...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSchedulesInput, options: Options) !ListSchedulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSchedulesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.group_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "ScheduleGroup=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSchedulesOutput {
    var result: ListSchedulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSchedulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
