...
fn serializeRequest(alloc: std.mem.Allocator, input: GetScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetScheduleOutput {
    var result: GetScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetScheduleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
