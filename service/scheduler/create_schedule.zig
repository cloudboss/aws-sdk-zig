...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.action_after_completion) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ActionAfterCompletion\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FlexibleTimeWindow\":");
    try aws.json.writeValue(@TypeOf(input.flexible_time_window), input.flexible_time_window, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ScheduleExpression\":");
    try aws.json.writeValue(@TypeOf(input.schedule_expression), input.schedule_expression, alloc, &body_buf);
    has_prev = true;
    if (input.schedule_expression_timezone) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ScheduleExpressionTimezone\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateScheduleOutput {
    var result: CreateScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateScheduleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
