...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchQuantumTasksInput, options: Options) !SearchQuantumTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchQuantumTasksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchQuantumTasksOutput {
    var result: SearchQuantumTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchQuantumTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
