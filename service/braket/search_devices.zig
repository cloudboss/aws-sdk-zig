...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchDevicesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchDevicesOutput {
    var result: SearchDevicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchDevicesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
