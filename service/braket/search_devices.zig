...
const SearchDevicesFilter = @import("search_devices_filter.zig").SearchDevicesFilter;
const DeviceSummary = @import("device_summary.zig").DeviceSummary;

pub const SearchDevicesInput = struct {
    /// Array of SearchDevicesFilter objects to use when searching for devices.
    filters: []const SearchDevicesFilter,
...
pub const SearchDevicesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchDevicesInput, options: CallOptions) !SearchDevicesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchDevicesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchDevicesOutput {
    var result: SearchDevicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchDevicesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
