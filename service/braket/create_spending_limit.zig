...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSpendingLimitInput, options: Options) !CreateSpendingLimitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSpendingLimitInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.spending_limit), input.spending_limit, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSpendingLimitOutput {
    var result: CreateSpendingLimitOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSpendingLimitOutput, body, alloc);
    }
    _ = status;
    _ = headers;
