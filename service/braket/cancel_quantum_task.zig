...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelQuantumTaskInput, options: Options) !CancelQuantumTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelQuantumTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelQuantumTaskOutput {
    var result: CancelQuantumTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelQuantumTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
