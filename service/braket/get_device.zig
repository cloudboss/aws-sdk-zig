...
const DeviceQueueInfo = @import("device_queue_info.zig").DeviceQueueInfo;
...
pub const GetDeviceOutput = struct {
    ...
    pub const json_field_names = .{
        ...
        .device_queue_info = "deviceQueueInfo",
        .device_status = "deviceStatus",
        .device_type = "deviceType",
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.device_arn);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDeviceOutput {
    var result: GetDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDeviceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
