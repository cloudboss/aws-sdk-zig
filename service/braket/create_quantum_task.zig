...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateQuantumTaskInput, options: CallOptions) !CreateQuantumTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateQuantumTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.device_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"deviceParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.experimental_capabilities) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"experimentalCapabilities\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.job_token) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"jobToken\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"outputS3Bucket\":");
    try aws.json.writeValue(@TypeOf(input.output_s3_bucket), input.output_s3_bucket, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"outputS3KeyPrefix\":");
    try aws.json.writeValue(@TypeOf(input.output_s3_key_prefix), input.output_s3_key_prefix, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"shots\":");
    try aws.json.writeValue(@TypeOf(input.shots), input.shots, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
