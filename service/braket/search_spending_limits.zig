...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchSpendingLimitsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchSpendingLimitsOutput {
    var result: SearchSpendingLimitsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchSpendingLimitsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
