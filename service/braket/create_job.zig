...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"algorithmSpecification\":");
    try aws.json.writeValue(@TypeOf(input.algorithm_specification), input.algorithm_specification, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.checkpoint_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"checkpointConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"deviceConfig\":");
    try aws.json.writeValue(@TypeOf(input.device_config), input.device_config, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"instanceConfig\":");
    try aws.json.writeValue(@TypeOf(input.instance_config), input.instance_config, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
