...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPolicyTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.ListPolicyTemplates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPolicyTemplatesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListPolicyTemplatesOutput, body, alloc);
}
