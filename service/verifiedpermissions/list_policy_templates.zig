...
const PolicyTemplateItem = @import("policy_template_item.zig").PolicyTemplateItem;

pub const ListPolicyTemplatesInput = struct {
...
pub const ListPolicyTemplatesOutput = struct {
    ...
    policy_templates: ?[]const PolicyTemplateItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPolicyTemplatesInput, options: CallOptions) !ListPolicyTemplatesOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "verifiedpermissions");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPolicyTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "VerifiedPermissions.ListPolicyTemplates");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPolicyTemplatesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListPolicyTemplatesOutput, body, allocator);
}
