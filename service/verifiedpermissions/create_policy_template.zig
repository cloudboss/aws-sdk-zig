...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePolicyTemplateInput, options: Options) !CreatePolicyTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePolicyTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.CreatePolicyTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePolicyTemplateOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreatePolicyTemplateOutput, body, alloc);
}
