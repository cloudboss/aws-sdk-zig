...
pub const CreatePolicyTemplateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePolicyTemplateInput, options: CallOptions) !CreatePolicyTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreatePolicyTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "VerifiedPermissions.CreatePolicyTemplate");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreatePolicyTemplateOutput {
    _ = status;
    _ = headers;
