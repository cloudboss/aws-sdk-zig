...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: IsAuthorizedInput, options: Options) !IsAuthorizedOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: IsAuthorizedInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.IsAuthorized");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !IsAuthorizedOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(IsAuthorizedOutput, body, alloc);
}
