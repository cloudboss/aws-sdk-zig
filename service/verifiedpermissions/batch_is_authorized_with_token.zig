...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchIsAuthorizedWithTokenInput, options: Options) !BatchIsAuthorizedWithTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchIsAuthorizedWithTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.BatchIsAuthorizedWithToken");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchIsAuthorizedWithTokenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchIsAuthorizedWithTokenOutput, body, alloc);
}
