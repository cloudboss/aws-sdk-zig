...
const PolicyStoreItem = @import("policy_store_item.zig").PolicyStoreItem;

pub const ListPolicyStoresInput = struct {
...
pub const ListPolicyStoresOutput = struct {
    ...
    policy_stores: ?[]const PolicyStoreItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPolicyStoresInput, options: CallOptions) !ListPolicyStoresOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPolicyStoresInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "VerifiedPermissions.ListPolicyStores");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPolicyStoresOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListPolicyStoresOutput, body, allocator);
}
