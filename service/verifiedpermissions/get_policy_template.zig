...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPolicyTemplateInput, options: Options) !GetPolicyTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPolicyTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.GetPolicyTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPolicyTemplateOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetPolicyTemplateOutput, body, alloc);
}
