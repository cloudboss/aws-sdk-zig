...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchIsAuthorizedInput, options: Options) !BatchIsAuthorizedOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchIsAuthorizedInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.BatchIsAuthorized");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchIsAuthorizedOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchIsAuthorizedOutput, body, alloc);
}
