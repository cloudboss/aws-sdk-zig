...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePolicyStoreInput, options: Options) !UpdatePolicyStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePolicyStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.UpdatePolicyStore");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePolicyStoreOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdatePolicyStoreOutput, body, alloc);
}
