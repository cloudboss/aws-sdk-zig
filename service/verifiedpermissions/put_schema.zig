...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSchemaInput, options: Options) !PutSchemaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutSchemaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.PutSchema");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutSchemaOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutSchemaOutput, body, alloc);
}
