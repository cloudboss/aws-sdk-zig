...
const PolicyFilter = @import("policy_filter.zig").PolicyFilter;
const PolicyItem = @import("policy_item.zig").PolicyItem;
...
pub const ListPoliciesOutput = struct {
    ...
    policies: ?[]const PolicyItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPoliciesInput, options: Options) !ListPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.ListPolicies");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPoliciesOutput {
    ...
    return aws.json.parseJsonObject(ListPoliciesOutput, body, alloc);
}
