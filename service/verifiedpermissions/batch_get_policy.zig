...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetPolicyInput, options: Options) !BatchGetPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.BatchGetPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetPolicyOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchGetPolicyOutput, body, alloc);
}
