...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePolicyTemplateInput, options: Options) !UpdatePolicyTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePolicyTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.UpdatePolicyTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePolicyTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdatePolicyTemplateOutput, body, alloc);
}
