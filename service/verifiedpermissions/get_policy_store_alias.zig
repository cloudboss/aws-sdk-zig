...
pub const GetPolicyStoreAliasInput = struct {
...
pub const GetPolicyStoreAliasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPolicyStoreAliasInput, options: CallOptions) !GetPolicyStoreAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPolicyStoreAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "VerifiedPermissions.GetPolicyStoreAlias");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPolicyStoreAliasOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetPolicyStoreAliasOutput, body, allocator);
}
