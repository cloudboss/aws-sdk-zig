...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdentitySourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.ListIdentitySources");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdentitySourcesOutput {
    ...
    return aws.json.parseJsonObject(ListIdentitySourcesOutput, body, alloc);
}
