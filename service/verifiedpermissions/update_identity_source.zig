...
const UpdateConfiguration = @import("update_configuration.zig").UpdateConfiguration;

pub const UpdateIdentitySourceInput = struct {
...
pub const UpdateIdentitySourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIdentitySourceInput, options: CallOptions) !UpdateIdentitySourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateIdentitySourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "VerifiedPermissions.UpdateIdentitySource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateIdentitySourceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateIdentitySourceOutput, body, allocator);
}
