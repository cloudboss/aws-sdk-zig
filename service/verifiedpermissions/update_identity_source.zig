...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIdentitySourceInput, options: Options) !UpdateIdentitySourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIdentitySourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VerifiedPermissions.UpdateIdentitySource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIdentitySourceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateIdentitySourceOutput, body, alloc);
}
