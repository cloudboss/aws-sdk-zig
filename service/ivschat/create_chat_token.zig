...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChatTokenInput, options: Options) !CreateChatTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateChatTokenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateChatTokenOutput {
    var result: CreateChatTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChatTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
