...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableApplicationLayerAutomaticResponseInput, options: Options) !EnableApplicationLayerAutomaticResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableApplicationLayerAutomaticResponseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.EnableApplicationLayerAutomaticResponse");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableApplicationLayerAutomaticResponseOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
