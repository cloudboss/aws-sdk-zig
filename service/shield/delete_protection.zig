...
pub const DeleteProtectionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteProtectionInput, options: Options) !DeleteProtectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.DeleteProtection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteProtectionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
