...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProtectionGroupsInput, options: Options) !ListProtectionGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProtectionGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.ListProtectionGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProtectionGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProtectionGroupsOutput, body, alloc);
}
