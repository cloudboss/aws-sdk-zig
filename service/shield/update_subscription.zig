...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSubscriptionInput, options: Options) !UpdateSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.UpdateSubscription");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSubscriptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
