...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSubscriptionInput, options: Options) !DescribeSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSubscriptionInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.DescribeSubscription");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSubscriptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSubscriptionOutput, body, alloc);
}
