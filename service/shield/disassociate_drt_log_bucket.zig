...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateDRTLogBucketInput, options: Options) !DisassociateDRTLogBucketOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateDRTLogBucketInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.DisassociateDRTLogBucket");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateDRTLogBucketOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
