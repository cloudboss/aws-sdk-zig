...
pub const DisassociateHealthCheckInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateHealthCheckInput, options: CallOptions) !DisassociateHealthCheckOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateHealthCheckInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.DisassociateHealthCheck");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateHealthCheckOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
