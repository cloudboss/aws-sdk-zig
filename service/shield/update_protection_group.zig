...
pub const UpdateProtectionGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProtectionGroupInput, options: Options) !UpdateProtectionGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateProtectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.UpdateProtectionGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateProtectionGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
