...
pub const DescribeDRTAccessOutput = struct {
    ...
    log_bucket_list: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDRTAccessInput, options: CallOptions) !DescribeDRTAccessOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "shield");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDRTAccessInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.DescribeDRTAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDRTAccessOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDRTAccessOutput, body, allocator);
}
