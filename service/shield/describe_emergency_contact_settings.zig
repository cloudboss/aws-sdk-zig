...
pub const DescribeEmergencyContactSettingsOutput = struct {
    ...
    emergency_contact_list: ?[]const EmergencyContact = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEmergencyContactSettingsInput, options: CallOptions) !DescribeEmergencyContactSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEmergencyContactSettingsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.DescribeEmergencyContactSettings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEmergencyContactSettingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEmergencyContactSettingsOutput, body, allocator);
}
