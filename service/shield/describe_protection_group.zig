...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProtectionGroupInput, options: Options) !DescribeProtectionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProtectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.DescribeProtectionGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProtectionGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeProtectionGroupOutput, body, alloc);
}
