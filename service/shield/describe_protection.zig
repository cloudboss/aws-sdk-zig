...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProtectionInput, options: Options) !DescribeProtectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.DescribeProtection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProtectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProtectionOutput, body, alloc);
}
