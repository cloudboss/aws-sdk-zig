...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableProactiveEngagementInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.DisableProactiveEngagement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableProactiveEngagementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
