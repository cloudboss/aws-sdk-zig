...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableProactiveEngagementInput, options: Options) !DisableProactiveEngagementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableProactiveEngagementInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.DisableProactiveEngagement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableProactiveEngagementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
