...
const AttackDetail = @import("attack_detail.zig").AttackDetail;

pub const DescribeAttackInput = struct {
...
pub const DescribeAttackOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAttackInput, options: CallOptions) !DescribeAttackOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAttackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.DescribeAttack");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAttackOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAttackOutput, body, allocator);
}
