...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProtectionsInput, options: Options) !ListProtectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProtectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.ListProtections");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProtectionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProtectionsOutput, body, alloc);
}
