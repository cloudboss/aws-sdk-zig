...
pub const DeleteProtectionGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteProtectionGroupInput, options: Options) !DeleteProtectionGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteProtectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.DeleteProtectionGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteProtectionGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
