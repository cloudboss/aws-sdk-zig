...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProtectionGroupInput, options: Options) !CreateProtectionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProtectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.CreateProtectionGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProtectionGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
