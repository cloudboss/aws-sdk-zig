...
pub const DisassociateDRTRoleInput = struct {
};

pub const DisassociateDRTRoleOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateDRTRoleInput, options: CallOptions) !DisassociateDRTRoleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateDRTRoleInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.DisassociateDRTRole");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateDRTRoleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
