...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableProactiveEngagementInput, options: CallOptions) !EnableProactiveEngagementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableProactiveEngagementInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.EnableProactiveEngagement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableProactiveEngagementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
