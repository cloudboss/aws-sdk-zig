...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateHealthCheckInput, options: Options) !AssociateHealthCheckOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateHealthCheckInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.AssociateHealthCheck");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateHealthCheckOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
