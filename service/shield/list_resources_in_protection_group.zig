...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourcesInProtectionGroupInput, options: Options) !ListResourcesInProtectionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourcesInProtectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.ListResourcesInProtectionGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourcesInProtectionGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListResourcesInProtectionGroupOutput, body, alloc);
}
