...
pub const DeleteSubscriptionInput = struct {
};

pub const DeleteSubscriptionOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSubscriptionInput, options: CallOptions) !DeleteSubscriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSubscriptionInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.DeleteSubscription");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSubscriptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
