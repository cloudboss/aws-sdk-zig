...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableApplicationLayerAutomaticResponseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.DisableApplicationLayerAutomaticResponse");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableApplicationLayerAutomaticResponseOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
