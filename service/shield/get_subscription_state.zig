...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSubscriptionStateInput, options: CallOptions) !GetSubscriptionStateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSubscriptionStateInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShield_20160616.GetSubscriptionState");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSubscriptionStateOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetSubscriptionStateOutput, body, allocator);
}
