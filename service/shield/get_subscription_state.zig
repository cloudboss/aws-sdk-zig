...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSubscriptionStateInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShield_20160616.GetSubscriptionState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSubscriptionStateOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetSubscriptionStateOutput, body, alloc);
}
