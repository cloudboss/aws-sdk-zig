...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreTableFromRecoveryPointInput, options: Options) !RestoreTableFromRecoveryPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreTableFromRecoveryPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.RestoreTableFromRecoveryPoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreTableFromRecoveryPointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RestoreTableFromRecoveryPointOutput, body, alloc);
}
