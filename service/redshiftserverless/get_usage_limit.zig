...
pub const GetUsageLimitInput = struct {
...
pub const GetUsageLimitOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUsageLimitInput, options: CallOptions) !GetUsageLimitOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetUsageLimitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RedshiftServerless.GetUsageLimit");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetUsageLimitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUsageLimitOutput, body, allocator);
}
