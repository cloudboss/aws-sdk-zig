...
pub const GetEndpointAccessInput = struct {
...
pub const GetEndpointAccessOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEndpointAccessInput, options: CallOptions) !GetEndpointAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RedshiftServerless.GetEndpointAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetEndpointAccessOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEndpointAccessOutput, body, allocator);
}
