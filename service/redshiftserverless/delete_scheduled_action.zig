...
pub const DeleteScheduledActionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteScheduledActionInput, options: Options) !DeleteScheduledActionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteScheduledActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RedshiftServerless.DeleteScheduledAction");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteScheduledActionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteScheduledActionOutput, body, allocator);
}
