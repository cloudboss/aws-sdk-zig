...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomDomainAssociationsInput, options: Options) !ListCustomDomainAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCustomDomainAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.ListCustomDomainAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCustomDomainAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCustomDomainAssociationsOutput, body, alloc);
}
