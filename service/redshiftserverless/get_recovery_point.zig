...
pub const GetRecoveryPointInput = struct {
...
pub const GetRecoveryPointOutput = struct {
    /// The returned recovery point object.
    recovery_point: ?RecoveryPoint = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRecoveryPointInput, options: CallOptions) !GetRecoveryPointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRecoveryPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RedshiftServerless.GetRecoveryPoint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRecoveryPointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRecoveryPointOutput, body, allocator);
}
