...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTrackInput, options: Options) !GetTrackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTrackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.GetTrack");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTrackOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTrackOutput, body, alloc);
}
