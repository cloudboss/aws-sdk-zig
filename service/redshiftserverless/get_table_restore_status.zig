...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableRestoreStatusInput, options: Options) !GetTableRestoreStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTableRestoreStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.GetTableRestoreStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTableRestoreStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTableRestoreStatusOutput, body, alloc);
}
