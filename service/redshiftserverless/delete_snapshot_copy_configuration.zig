...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSnapshotCopyConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.DeleteSnapshotCopyConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSnapshotCopyConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteSnapshotCopyConfigurationOutput, body, alloc);
}
