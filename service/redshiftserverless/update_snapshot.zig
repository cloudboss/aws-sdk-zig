...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSnapshotInput, options: Options) !UpdateSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.UpdateSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSnapshotOutput, body, alloc);
}
