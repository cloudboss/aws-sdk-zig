...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreFromRecoveryPointInput, options: Options) !RestoreFromRecoveryPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreFromRecoveryPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.RestoreFromRecoveryPoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreFromRecoveryPointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RestoreFromRecoveryPointOutput, body, alloc);
}
