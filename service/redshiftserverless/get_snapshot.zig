...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSnapshotInput, options: Options) !GetSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.GetSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSnapshotOutput, body, alloc);
}
