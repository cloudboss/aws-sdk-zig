...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReservationOfferingInput, options: Options) !GetReservationOfferingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReservationOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.GetReservationOffering");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReservationOfferingOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetReservationOfferingOutput, body, alloc);
}
