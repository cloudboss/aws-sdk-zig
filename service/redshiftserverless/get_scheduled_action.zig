...
pub const GetScheduledActionInput = struct {
...
pub const GetScheduledActionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetScheduledActionInput, options: CallOptions) !GetScheduledActionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetScheduledActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RedshiftServerless.GetScheduledAction");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetScheduledActionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetScheduledActionOutput, body, allocator);
}
