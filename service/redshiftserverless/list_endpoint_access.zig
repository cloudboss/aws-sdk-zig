...
pub const ListEndpointAccessInput = struct {
    /// An optional parameter that specifies the maximum number of results to
    /// return. You can use `nextToken` to display the next page of results.
    max_results: ?i32 = null,

    /// If your initial `ListEndpointAccess` operation returns a `nextToken`, you
    /// can include the returned `nextToken` in following `ListEndpointAccess`
...
pub const ListEndpointAccessOutput = struct {
    /// The returned VPC endpoints.
    endpoints: ?[]const EndpointAccess = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEndpointAccessInput, options: CallOptions) !ListEndpointAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RedshiftServerless.ListEndpointAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListEndpointAccessOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListEndpointAccessOutput, body, allocator);
}
