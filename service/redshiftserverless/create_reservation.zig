...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReservationInput, options: Options) !CreateReservationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReservationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.CreateReservation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReservationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateReservationOutput, body, alloc);
}
