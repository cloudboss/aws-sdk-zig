...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNamespaceInput, options: Options) !UpdateNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.UpdateNamespace");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNamespaceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateNamespaceOutput, body, alloc);
}
