...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTracksInput, options: Options) !ListTracksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTracksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.ListTracks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTracksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTracksOutput, body, alloc);
}
