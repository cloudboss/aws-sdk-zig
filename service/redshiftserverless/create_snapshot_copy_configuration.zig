...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSnapshotCopyConfigurationInput, options: Options) !CreateSnapshotCopyConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSnapshotCopyConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.CreateSnapshotCopyConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSnapshotCopyConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateSnapshotCopyConfigurationOutput, body, alloc);
}
