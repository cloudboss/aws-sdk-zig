...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReservationInput, options: Options) !GetReservationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReservationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.GetReservation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReservationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetReservationOutput, body, alloc);
}
