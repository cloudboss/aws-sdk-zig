...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.CreateEndpointAccess");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEndpointAccessOutput {
    ...
    return aws.json.parseJsonObject(CreateEndpointAccessOutput, body, alloc);
}
