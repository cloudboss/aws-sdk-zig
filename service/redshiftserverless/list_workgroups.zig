...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkgroupsInput, options: Options) !ListWorkgroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkgroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.ListWorkgroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkgroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListWorkgroupsOutput, body, alloc);
}
