...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateNamespaceInput, options: Options) !CreateNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.CreateNamespace");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateNamespaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateNamespaceOutput, body, alloc);
}
