...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUsageLimitsInput, options: Options) !ListUsageLimitsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUsageLimitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.ListUsageLimits");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUsageLimitsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUsageLimitsOutput, body, alloc);
}
