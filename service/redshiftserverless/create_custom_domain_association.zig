...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomDomainAssociationInput, options: Options) !CreateCustomDomainAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomDomainAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.CreateCustomDomainAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomDomainAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCustomDomainAssociationOutput, body, alloc);
}
