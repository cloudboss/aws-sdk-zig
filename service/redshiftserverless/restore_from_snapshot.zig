...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreFromSnapshotInput, options: Options) !RestoreFromSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreFromSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.RestoreFromSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreFromSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RestoreFromSnapshotOutput, body, alloc);
}
