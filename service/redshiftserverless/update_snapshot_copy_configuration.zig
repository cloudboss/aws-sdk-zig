...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSnapshotCopyConfigurationInput, options: Options) !UpdateSnapshotCopyConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSnapshotCopyConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.UpdateSnapshotCopyConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSnapshotCopyConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateSnapshotCopyConfigurationOutput, body, alloc);
}
