...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSnapshotCopyConfigurationsInput, options: Options) !ListSnapshotCopyConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSnapshotCopyConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.ListSnapshotCopyConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSnapshotCopyConfigurationsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListSnapshotCopyConfigurationsOutput, body, alloc);
}
