...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEndpointAccessInput, options: Options) !UpdateEndpointAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.UpdateEndpointAccess");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEndpointAccessOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateEndpointAccessOutput, body, alloc);
}
