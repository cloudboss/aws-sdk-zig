...
const ManagedWorkgroupListItem = @import("managed_workgroup_list_item.zig").ManagedWorkgroupListItem;

pub const ListManagedWorkgroupsInput = struct {
    /// An optional parameter that specifies the maximum number of results to
    /// return. You can use nextToken to display the next page of results.
    max_results: ?i32 = null,

    /// If your initial ListManagedWorkgroups operation returns a nextToken, you can
    /// include the returned nextToken in following ListManagedWorkgroups
...
pub const ListManagedWorkgroupsOutput = struct {
    /// The returned array of managed workgroups.
    managed_workgroups: ?[]const ManagedWorkgroupListItem = null,
    ...
    pub const json_field_names = .{
        .managed_workgroups = "managedWorkgroups",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedWorkgroupsInput, options: CallOptions) !ListManagedWorkgroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListManagedWorkgroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RedshiftServerless.ListManagedWorkgroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListManagedWorkgroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListManagedWorkgroupsOutput, body, allocator);
}
