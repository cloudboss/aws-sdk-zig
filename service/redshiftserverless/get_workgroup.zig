...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkgroupInput, options: Options) !GetWorkgroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkgroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.GetWorkgroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkgroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetWorkgroupOutput, body, alloc);
}
