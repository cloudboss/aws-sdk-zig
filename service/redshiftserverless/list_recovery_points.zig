...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecoveryPointsInput, options: Options) !ListRecoveryPointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecoveryPointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.ListRecoveryPoints");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecoveryPointsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRecoveryPointsOutput, body, alloc);
}
