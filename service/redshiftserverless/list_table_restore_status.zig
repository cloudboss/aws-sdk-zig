...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTableRestoreStatusInput, options: Options) !ListTableRestoreStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTableRestoreStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.ListTableRestoreStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTableRestoreStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTableRestoreStatusOutput, body, alloc);
}
