...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUsageLimitInput, options: Options) !UpdateUsageLimitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUsageLimitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftServerless.UpdateUsageLimit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUsageLimitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateUsageLimitOutput, body, alloc);
}
