...
pub const StartQueryWorkloadInsightsTopContributorsDataInput = struct {
    /// The destination category for a top contributors. Destination categories can
...
pub const StartQueryWorkloadInsightsTopContributorsDataOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartQueryWorkloadInsightsTopContributorsDataInput, options: Options) !StartQueryWorkloadInsightsTopContributorsDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartQueryWorkloadInsightsTopContributorsDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/workloadInsights/");
    try path_buf.appendSlice(alloc, input.scope_id);
    try path_buf.appendSlice(alloc, "/topContributorsDataQueries");
    ...
    try body_buf.appendSlice(alloc, "\"destinationCategory\":");
    try aws.json.writeValue(@TypeOf(input.destination_category), input.destination_category, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.metric_name), input.metric_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartQueryWorkloadInsightsTopContributorsDataOutput {
    var result: StartQueryWorkloadInsightsTopContributorsDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartQueryWorkloadInsightsTopContributorsDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
