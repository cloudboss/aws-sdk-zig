...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartQueryMonitorTopContributorsInput, options: Options) !StartQueryMonitorTopContributorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartQueryMonitorTopContributorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartQueryMonitorTopContributorsOutput {
    var result: StartQueryMonitorTopContributorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartQueryMonitorTopContributorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
