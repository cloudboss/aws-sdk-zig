...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMonitorInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.local_resources_to_add) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"localResourcesToAdd\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.local_resources_to_remove) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"localResourcesToRemove\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.remote_resources_to_add) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"remoteResourcesToAdd\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.remote_resources_to_remove) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"remoteResourcesToRemove\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMonitorOutput {
    var result: UpdateMonitorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMonitorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
