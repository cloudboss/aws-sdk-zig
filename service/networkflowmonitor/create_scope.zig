...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateScopeInput, options: Options) !CreateScopeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateScopeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateScopeOutput {
    var result: CreateScopeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateScopeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
