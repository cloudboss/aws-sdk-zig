...
fn serializeRequest(alloc: std.mem.Allocator, input: StartQueryWorkloadInsightsTopContributorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartQueryWorkloadInsightsTopContributorsOutput {
    var result: StartQueryWorkloadInsightsTopContributorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartQueryWorkloadInsightsTopContributorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
