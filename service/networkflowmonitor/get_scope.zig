...
pub const GetScopeOutput = struct {
    ...
    targets: ?[]const TargetResource = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetScopeInput, options: Options) !GetScopeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetScopeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetScopeOutput {
    var result: GetScopeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetScopeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
