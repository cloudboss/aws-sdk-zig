...
pub const ListSipMediaApplicationsInput = struct {
...
pub const ListSipMediaApplicationsOutput = struct {
    /// The token used to return the next page of results.
    next_token: ?[]const u8 = null,

    /// The list of SIP media applications and application details.
    sip_media_applications: ?[]const SipMediaApplication = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSipMediaApplicationsInput, options: Options) !ListSipMediaApplicationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSipMediaApplicationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSipMediaApplicationsOutput {
    var result: ListSipMediaApplicationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSipMediaApplicationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
