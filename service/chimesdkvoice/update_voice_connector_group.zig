...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVoiceConnectorGroupInput, options: CallOptions) !UpdateVoiceConnectorGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateVoiceConnectorGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"VoiceConnectorItems\":");
    try aws.json.writeValue(@TypeOf(input.voice_connector_items), input.voice_connector_items, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateVoiceConnectorGroupOutput {
    var result: UpdateVoiceConnectorGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVoiceConnectorGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
