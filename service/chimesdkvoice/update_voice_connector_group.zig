...
pub const UpdateVoiceConnectorGroupInput = struct {
...
pub const UpdateVoiceConnectorGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVoiceConnectorGroupInput, options: Options) !UpdateVoiceConnectorGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVoiceConnectorGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/voice-connector-groups/");
    try path_buf.appendSlice(alloc, input.voice_connector_group_id);
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VoiceConnectorItems\":");
    try aws.json.writeValue(@TypeOf(input.voice_connector_items), input.voice_connector_items, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVoiceConnectorGroupOutput {
    var result: UpdateVoiceConnectorGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVoiceConnectorGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
