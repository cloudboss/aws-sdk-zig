...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceConnectorTerminationHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/termination/health");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceConnectorTerminationHealthOutput {
    var result: GetVoiceConnectorTerminationHealthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceConnectorTerminationHealthOutput, body, alloc);
    }
    _ = status;
    _ = headers;
