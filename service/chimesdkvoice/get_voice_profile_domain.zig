...
pub const GetVoiceProfileDomainInput = struct {
...
pub const GetVoiceProfileDomainOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVoiceProfileDomainInput, options: Options) !GetVoiceProfileDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceProfileDomainInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceProfileDomainOutput {
    var result: GetVoiceProfileDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceProfileDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
