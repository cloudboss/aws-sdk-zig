...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVoiceConnectorTerminationCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/termination/credentials");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    try query_buf.appendSlice(alloc, "operation=delete");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "\"Usernames\":");
    try aws.json.writeValue(@TypeOf(input.usernames), input.usernames, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVoiceConnectorTerminationCredentialsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
