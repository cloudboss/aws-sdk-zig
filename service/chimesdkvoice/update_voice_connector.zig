...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVoiceConnectorInput, options: Options) !UpdateVoiceConnectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVoiceConnectorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVoiceConnectorOutput {
    var result: UpdateVoiceConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVoiceConnectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
