...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVoiceProfileDomainInput, options: Options) !UpdateVoiceProfileDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVoiceProfileDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/voice-profile-domains/");
    try path_buf.appendSlice(alloc, input.voice_profile_domain_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVoiceProfileDomainOutput {
    var result: UpdateVoiceProfileDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVoiceProfileDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
