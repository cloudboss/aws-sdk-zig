...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceConnectorTerminationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceConnectorTerminationOutput {
    var result: GetVoiceConnectorTerminationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceConnectorTerminationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
