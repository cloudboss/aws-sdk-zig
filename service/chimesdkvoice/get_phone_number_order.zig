...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPhoneNumberOrderInput, options: Options) !GetPhoneNumberOrderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPhoneNumberOrderInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/phone-number-orders/");
    try path_buf.appendSlice(alloc, input.phone_number_order_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPhoneNumberOrderOutput {
    var result: GetPhoneNumberOrderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPhoneNumberOrderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
