...
fn serializeRequest(alloc: std.mem.Allocator, input: PutVoiceConnectorStreamingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/streaming-configuration");
    ...
    try body_buf.appendSlice(alloc, "\"StreamingConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.streaming_configuration), input.streaming_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutVoiceConnectorStreamingConfigurationOutput {
    var result: PutVoiceConnectorStreamingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutVoiceConnectorStreamingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
