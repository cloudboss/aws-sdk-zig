...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceConnectorProxyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceConnectorProxyOutput {
    var result: GetVoiceConnectorProxyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceConnectorProxyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
