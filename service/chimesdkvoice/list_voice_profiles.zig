...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVoiceProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "voice-profile-domain-id=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.voice_profile_domain_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVoiceProfilesOutput {
    var result: ListVoiceProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVoiceProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
