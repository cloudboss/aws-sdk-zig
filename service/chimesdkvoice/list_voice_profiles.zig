...
const VoiceProfileSummary = @import("voice_profile_summary.zig").VoiceProfileSummary;

pub const ListVoiceProfilesInput = struct {
...
pub const ListVoiceProfilesOutput = struct {
    /// The token used to retrieve the next page of results.
    next_token: ?[]const u8 = null,

    /// The list of voice profiles.
    voice_profiles: ?[]const VoiceProfileSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVoiceProfilesInput, options: CallOptions) !ListVoiceProfilesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVoiceProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "voice-profile-domain-id=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.voice_profile_domain_id);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVoiceProfilesOutput {
    var result: ListVoiceProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVoiceProfilesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
