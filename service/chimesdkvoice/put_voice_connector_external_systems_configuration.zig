...
fn serializeRequest(alloc: std.mem.Allocator, input: PutVoiceConnectorExternalSystemsConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/external-systems-configuration");
    ...
    if (input.contact_center_system_types) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ContactCenterSystemTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.session_border_controller_types) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SessionBorderControllerTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutVoiceConnectorExternalSystemsConfigurationOutput {
    var result: PutVoiceConnectorExternalSystemsConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutVoiceConnectorExternalSystemsConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
