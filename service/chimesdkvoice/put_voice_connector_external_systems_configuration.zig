...
const ExternalSystemsConfiguration = @import("external_systems_configuration.zig").ExternalSystemsConfiguration;

pub const PutVoiceConnectorExternalSystemsConfigurationInput = struct {
...
pub const PutVoiceConnectorExternalSystemsConfigurationOutput = struct {
    /// An object that contains information about an external systems configuration
    /// for a Voice Connector.
    external_systems_configuration: ?ExternalSystemsConfiguration = null,

    pub const json_field_names = .{
        .external_systems_configuration = "ExternalSystemsConfiguration",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutVoiceConnectorExternalSystemsConfigurationInput, options: CallOptions) !PutVoiceConnectorExternalSystemsConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutVoiceConnectorExternalSystemsConfigurationInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("chimesdkvoice", "Chime SDK Voice", allocator);
    ...
    try path_buf.appendSlice(allocator, "/external-systems-configuration");
    ...
    if (input.contact_center_system_types) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ContactCenterSystemTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.session_border_controller_types) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"SessionBorderControllerTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutVoiceConnectorExternalSystemsConfigurationOutput {
    var result: PutVoiceConnectorExternalSystemsConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutVoiceConnectorExternalSystemsConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
