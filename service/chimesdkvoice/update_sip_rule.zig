...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSipRuleInput, options: Options) !UpdateSipRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSipRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sip-rules/");
    try path_buf.appendSlice(alloc, input.sip_rule_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSipRuleOutput {
    var result: UpdateSipRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSipRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
