...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePhoneNumberInput, options: Options) !UpdatePhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePhoneNumberInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePhoneNumberOutput {
    var result: UpdatePhoneNumberOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePhoneNumberOutput, body, alloc);
    }
    _ = status;
    _ = headers;
