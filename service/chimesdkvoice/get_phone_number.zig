...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPhoneNumberInput, options: Options) !GetPhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPhoneNumberInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPhoneNumberOutput {
    var result: GetPhoneNumberOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPhoneNumberOutput, body, alloc);
    }
    _ = status;
    _ = headers;
