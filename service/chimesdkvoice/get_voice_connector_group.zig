...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVoiceConnectorGroupInput, options: Options) !GetVoiceConnectorGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceConnectorGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceConnectorGroupOutput {
    var result: GetVoiceConnectorGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceConnectorGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
