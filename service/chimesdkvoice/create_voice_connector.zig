...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVoiceConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RequireEncryption\":");
    try aws.json.writeValue(@TypeOf(input.require_encryption), input.require_encryption, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVoiceConnectorOutput {
    var result: CreateVoiceConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVoiceConnectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
