...
pub const PutVoiceConnectorOriginationInput = struct {
...
pub const PutVoiceConnectorOriginationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutVoiceConnectorOriginationInput, options: Options) !PutVoiceConnectorOriginationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutVoiceConnectorOriginationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/origination");
    ...
    try aws.json.writeValue(@TypeOf(input.origination), input.origination, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutVoiceConnectorOriginationOutput {
    var result: PutVoiceConnectorOriginationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutVoiceConnectorOriginationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
