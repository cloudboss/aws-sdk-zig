...
const GetPhoneNumberSettingsInput = struct {};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPhoneNumberSettingsInput, options: Options) !GetPhoneNumberSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPhoneNumberSettingsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPhoneNumberSettingsOutput {
    var result: GetPhoneNumberSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPhoneNumberSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
