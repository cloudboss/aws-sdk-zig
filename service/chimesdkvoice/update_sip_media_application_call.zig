...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSipMediaApplicationCallInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/calls/");
    try path_buf.appendSlice(alloc, input.transaction_id);
    ...
    try aws.json.writeValue(@TypeOf(input.arguments), input.arguments, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSipMediaApplicationCallOutput {
    var result: UpdateSipMediaApplicationCallOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSipMediaApplicationCallOutput, body, alloc);
    }
    _ = status;
    _ = headers;
