...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutVoiceConnectorProxyInput, options: Options) !PutVoiceConnectorProxyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutVoiceConnectorProxyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/programmable-numbers/proxy");
    ...
    try body_buf.appendSlice(alloc, "\"DefaultSessionExpiryMinutes\":");
    try aws.json.writeValue(@TypeOf(input.default_session_expiry_minutes), input.default_session_expiry_minutes, alloc, &body_buf);
    has_prev = true;
    if (input.disabled) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.fall_back_phone_number) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FallBackPhoneNumber\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PhoneNumberPoolCountries\":");
    try aws.json.writeValue(@TypeOf(input.phone_number_pool_countries), input.phone_number_pool_countries, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutVoiceConnectorProxyOutput {
    var result: PutVoiceConnectorProxyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutVoiceConnectorProxyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
