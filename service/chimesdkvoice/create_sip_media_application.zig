...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSipMediaApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.aws_region), input.aws_region, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSipMediaApplicationOutput {
    var result: CreateSipMediaApplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSipMediaApplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
