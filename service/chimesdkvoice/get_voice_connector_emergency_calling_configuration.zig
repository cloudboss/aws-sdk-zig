...
const EmergencyCallingConfiguration = @import("emergency_calling_configuration.zig").EmergencyCallingConfiguration;

pub const GetVoiceConnectorEmergencyCallingConfigurationInput = struct {
...
pub const GetVoiceConnectorEmergencyCallingConfigurationOutput = struct {
    /// The details of the emergency calling configuration.
    emergency_calling_configuration: ?EmergencyCallingConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVoiceConnectorEmergencyCallingConfigurationInput, options: Options) !GetVoiceConnectorEmergencyCallingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceConnectorEmergencyCallingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/emergency-calling-configuration");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceConnectorEmergencyCallingConfigurationOutput {
    var result: GetVoiceConnectorEmergencyCallingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceConnectorEmergencyCallingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
