...
pub const ListVoiceConnectorsInput = struct {
...
pub const ListVoiceConnectorsOutput = struct {
    /// The token used to return the next page of results.
    next_token: ?[]const u8 = null,

    /// The details of the Voice Connectors.
    voice_connectors: ?[]const VoiceConnector = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVoiceConnectorsInput, options: CallOptions) !ListVoiceConnectorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVoiceConnectorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVoiceConnectorsOutput {
    var result: ListVoiceConnectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVoiceConnectorsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
