...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVoiceConnectorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVoiceConnectorsOutput {
    var result: ListVoiceConnectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVoiceConnectorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
