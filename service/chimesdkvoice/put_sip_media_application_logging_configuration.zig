...
pub const PutSipMediaApplicationLoggingConfigurationInput = struct {
...
pub const PutSipMediaApplicationLoggingConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSipMediaApplicationLoggingConfigurationInput, options: Options) !PutSipMediaApplicationLoggingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutSipMediaApplicationLoggingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sip-media-applications/");
    try path_buf.appendSlice(alloc, input.sip_media_application_id);
    try path_buf.appendSlice(alloc, "/logging-configuration");
    ...
    if (input.sip_media_application_logging_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SipMediaApplicationLoggingConfiguration\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutSipMediaApplicationLoggingConfigurationOutput {
    var result: PutSipMediaApplicationLoggingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutSipMediaApplicationLoggingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
