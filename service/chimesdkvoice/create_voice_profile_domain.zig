...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVoiceProfileDomainInput, options: Options) !CreateVoiceProfileDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVoiceProfileDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.server_side_encryption_configuration), input.server_side_encryption_configuration, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVoiceProfileDomainOutput {
    var result: CreateVoiceProfileDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVoiceProfileDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
