...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociatePhoneNumbersFromVoiceConnectorGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociatePhoneNumbersFromVoiceConnectorGroupOutput {
    var result: DisassociatePhoneNumbersFromVoiceConnectorGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociatePhoneNumbersFromVoiceConnectorGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
