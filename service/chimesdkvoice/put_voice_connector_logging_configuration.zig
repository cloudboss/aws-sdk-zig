...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutVoiceConnectorLoggingConfigurationInput, options: Options) !PutVoiceConnectorLoggingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutVoiceConnectorLoggingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.logging_configuration), input.logging_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutVoiceConnectorLoggingConfigurationOutput {
    var result: PutVoiceConnectorLoggingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutVoiceConnectorLoggingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
