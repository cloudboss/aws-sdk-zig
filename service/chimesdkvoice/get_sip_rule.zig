...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSipRuleInput, options: Options) !GetSipRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSipRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSipRuleOutput {
    var result: GetSipRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSipRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
