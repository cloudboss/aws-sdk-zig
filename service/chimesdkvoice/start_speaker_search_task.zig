...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSpeakerSearchTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.call_leg) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"CallLeg\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VoiceProfileDomainId\":");
    try aws.json.writeValue(@TypeOf(input.voice_profile_domain_id), input.voice_profile_domain_id, alloc, &body_buf);
    has_prev = true;
