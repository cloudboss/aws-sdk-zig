...
const CallLegType = @import("call_leg_type.zig").CallLegType;
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSpeakerSearchTaskInput, options: CallOptions) !StartSpeakerSearchTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartSpeakerSearchTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.call_leg) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"CallLeg\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"VoiceProfileDomainId\":");
    try aws.json.writeValue(@TypeOf(input.voice_profile_domain_id), input.voice_profile_domain_id, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartSpeakerSearchTaskOutput {
    var result: StartSpeakerSearchTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartSpeakerSearchTaskOutput, body, allocator);
    }
    _ = status;
    _ = headers;
