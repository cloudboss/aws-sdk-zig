...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVoiceToneAnalysisTaskInput, options: Options) !GetVoiceToneAnalysisTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceToneAnalysisTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "isCaller=");
    try query_buf.appendSlice(alloc, if (input.is_caller) "true" else "false");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceToneAnalysisTaskOutput {
    var result: GetVoiceToneAnalysisTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceToneAnalysisTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
