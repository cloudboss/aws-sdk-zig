...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVoiceProfileInput, options: CallOptions) !UpdateVoiceProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateVoiceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/voice-profiles/");
    try path_buf.appendSlice(allocator, input.voice_profile_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateVoiceProfileOutput {
    var result: UpdateVoiceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVoiceProfileOutput, body, allocator);
    }
    _ = status;
    _ = headers;
