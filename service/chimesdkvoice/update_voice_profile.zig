...
pub const UpdateVoiceProfileInput = struct {
...
pub const UpdateVoiceProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVoiceProfileInput, options: Options) !UpdateVoiceProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVoiceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/voice-profiles/");
    try path_buf.appendSlice(alloc, input.voice_profile_id);
    ...
    try body_buf.appendSlice(alloc, "\"SpeakerSearchTaskId\":");
    try aws.json.writeValue(@TypeOf(input.speaker_search_task_id), input.speaker_search_task_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVoiceProfileOutput {
    var result: UpdateVoiceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVoiceProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
