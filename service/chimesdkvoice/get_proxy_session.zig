...
pub const GetProxySessionInput = struct {
...
pub const GetProxySessionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProxySessionInput, options: Options) !GetProxySessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProxySessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.proxy_session_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProxySessionOutput {
    var result: GetProxySessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProxySessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
