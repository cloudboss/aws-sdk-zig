...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociatePhoneNumbersFromVoiceConnectorInput, options: Options) !DisassociatePhoneNumbersFromVoiceConnectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociatePhoneNumbersFromVoiceConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=disassociate-phone-numbers");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociatePhoneNumbersFromVoiceConnectorOutput {
    var result: DisassociatePhoneNumbersFromVoiceConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociatePhoneNumbersFromVoiceConnectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
