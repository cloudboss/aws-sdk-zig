...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProxySessionsInput, options: Options) !ListProxySessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProxySessionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProxySessionsOutput {
    var result: ListProxySessionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProxySessionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
