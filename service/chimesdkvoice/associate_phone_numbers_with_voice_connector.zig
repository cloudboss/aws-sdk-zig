...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociatePhoneNumbersWithVoiceConnectorInput, options: Options) !AssociatePhoneNumbersWithVoiceConnectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociatePhoneNumbersWithVoiceConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=associate-phone-numbers");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "\"E164PhoneNumbers\":");
    try aws.json.writeValue(@TypeOf(input.e164_phone_numbers), input.e164_phone_numbers, alloc, &body_buf);
    has_prev = true;
    if (input.force_associate) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ForceAssociate\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociatePhoneNumbersWithVoiceConnectorOutput {
    var result: AssociatePhoneNumbersWithVoiceConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociatePhoneNumbersWithVoiceConnectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
