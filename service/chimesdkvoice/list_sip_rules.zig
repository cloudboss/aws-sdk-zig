...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSipRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.sip_media_application_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sip-media-application=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSipRulesOutput {
    var result: ListSipRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSipRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
