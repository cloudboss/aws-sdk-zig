...
pub const ListSipRulesInput = struct {
...
pub const ListSipRulesOutput = struct {
    /// The token used to return the next page of results.
    next_token: ?[]const u8 = null,

    /// The list of SIP rules and details.
    sip_rules: ?[]const SipRule = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSipRulesInput, options: CallOptions) !ListSipRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSipRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.sip_media_application_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "sip-media-application=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSipRulesOutput {
    var result: ListSipRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSipRulesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
