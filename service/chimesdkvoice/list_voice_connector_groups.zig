...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVoiceConnectorGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVoiceConnectorGroupsOutput {
    var result: ListVoiceConnectorGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVoiceConnectorGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
