...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProxySessionInput, options: Options) !UpdateProxySessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProxySessionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProxySessionOutput {
    var result: UpdateProxySessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProxySessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
