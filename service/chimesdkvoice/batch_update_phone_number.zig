...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchUpdatePhoneNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "operation=batch-update");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(allocator, "\"UpdatePhoneNumberRequestItems\":");
    try aws.json.writeValue(@TypeOf(input.update_phone_number_request_items), input.update_phone_number_request_items, allocator, &body_buf);
    has_prev = true;
