...
pub const GetSipMediaApplicationInput = struct {
...
pub const GetSipMediaApplicationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSipMediaApplicationInput, options: Options) !GetSipMediaApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSipMediaApplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSipMediaApplicationOutput {
    var result: GetSipMediaApplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSipMediaApplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
