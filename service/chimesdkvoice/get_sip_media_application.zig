...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSipMediaApplicationInput, options: CallOptions) !GetSipMediaApplicationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSipMediaApplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSipMediaApplicationOutput {
    var result: GetSipMediaApplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSipMediaApplicationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
