...
const Termination = @import("termination.zig").Termination;

pub const PutVoiceConnectorTerminationInput = struct {
...
pub const PutVoiceConnectorTerminationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutVoiceConnectorTerminationInput, options: Options) !PutVoiceConnectorTerminationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutVoiceConnectorTerminationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/termination");
    ...
    try body_buf.appendSlice(alloc, "\"Termination\":");
    try aws.json.writeValue(@TypeOf(input.termination), input.termination, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutVoiceConnectorTerminationOutput {
    var result: PutVoiceConnectorTerminationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutVoiceConnectorTerminationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
