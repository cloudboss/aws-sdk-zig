...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVoiceProfileDomainsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVoiceProfileDomainsOutput {
    var result: ListVoiceProfileDomainsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVoiceProfileDomainsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
