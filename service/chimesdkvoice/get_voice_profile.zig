...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceProfileOutput {
    var result: GetVoiceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
