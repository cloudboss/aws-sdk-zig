...
pub const GetVoiceProfileInput = struct {
...
pub const GetVoiceProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVoiceProfileInput, options: CallOptions) !GetVoiceProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetVoiceProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetVoiceProfileOutput {
    var result: GetVoiceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceProfileOutput, body, allocator);
    }
    _ = status;
    _ = headers;
