...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSipMediaApplicationCallInput, options: Options) !CreateSipMediaApplicationCallOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSipMediaApplicationCallInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.arguments_map) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ArgumentsMap\":");
    ...
    if (input.sip_headers) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SipHeaders\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSipMediaApplicationCallOutput {
    var result: CreateSipMediaApplicationCallOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSipMediaApplicationCallOutput, body, alloc);
    }
    _ = status;
    _ = headers;
