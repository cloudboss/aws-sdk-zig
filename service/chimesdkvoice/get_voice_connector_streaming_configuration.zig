...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceConnectorStreamingConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceConnectorStreamingConfigurationOutput {
    var result: GetVoiceConnectorStreamingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceConnectorStreamingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
