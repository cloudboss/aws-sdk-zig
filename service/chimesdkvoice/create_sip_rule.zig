...
pub const CreateSipRuleInput = struct {
    /// Disables or enables a SIP rule. You must disable SIP rules
    ...
    /// If `TriggerType` is `RequestUriHostname`, the
    /// value can be the outbound host name of a Voice Connector. If
    /// `TriggerType` is `ToPhoneNumber`, the value can
    /// be a customer-owned phone number in the E164 format. The
    /// `SipMediaApplication` specified in the `SipRule` is triggered
...
pub const CreateSipRuleOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSipRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.target_applications) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TargetApplications\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TriggerType\":");
    try aws.json.writeValue(@TypeOf(input.trigger_type), input.trigger_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TriggerValue\":");
    try aws.json.writeValue(@TypeOf(input.trigger_value), input.trigger_value, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSipRuleOutput {
    var result: CreateSipRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSipRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
