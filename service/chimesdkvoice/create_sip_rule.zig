...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSipRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"TriggerValue\":");
    try aws.json.writeValue(@TypeOf(input.trigger_value), input.trigger_value, allocator, &body_buf);
    has_prev = true;
