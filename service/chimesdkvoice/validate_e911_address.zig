...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ValidateE911AddressInput, options: Options) !ValidateE911AddressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ValidateE911AddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.aws_account_id), input.aws_account_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"City\":");
    try aws.json.writeValue(@TypeOf(input.city), input.city, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Country\":");
    try aws.json.writeValue(@TypeOf(input.country), input.country, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PostalCode\":");
    try aws.json.writeValue(@TypeOf(input.postal_code), input.postal_code, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"StreetInfo\":");
    try aws.json.writeValue(@TypeOf(input.street_info), input.street_info, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"StreetNumber\":");
    try aws.json.writeValue(@TypeOf(input.street_number), input.street_number, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ValidateE911AddressOutput {
    var result: ValidateE911AddressOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ValidateE911AddressOutput, body, alloc);
    }
    _ = status;
    _ = headers;
