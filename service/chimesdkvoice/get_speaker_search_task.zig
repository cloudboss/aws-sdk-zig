...
pub const GetSpeakerSearchTaskInput = struct {
...
pub const GetSpeakerSearchTaskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSpeakerSearchTaskInput, options: Options) !GetSpeakerSearchTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSpeakerSearchTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSpeakerSearchTaskOutput {
    var result: GetSpeakerSearchTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSpeakerSearchTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
