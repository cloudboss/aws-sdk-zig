...
pub const CreateVoiceConnectorGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVoiceConnectorGroupInput, options: Options) !CreateVoiceConnectorGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVoiceConnectorGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (input.voice_connector_items) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVoiceConnectorGroupOutput {
    var result: CreateVoiceConnectorGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVoiceConnectorGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
