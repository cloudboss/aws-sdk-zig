...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceConnectorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceConnectorOutput {
    var result: GetVoiceConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceConnectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
