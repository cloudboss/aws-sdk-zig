...
pub const GetVoiceConnectorInput = struct {
...
pub const GetVoiceConnectorOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVoiceConnectorInput, options: CallOptions) !GetVoiceConnectorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetVoiceConnectorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetVoiceConnectorOutput {
    var result: GetVoiceConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceConnectorOutput, body, allocator);
    }
    _ = status;
    _ = headers;
