...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVoiceConnectorOriginationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVoiceConnectorOriginationOutput {
    var result: GetVoiceConnectorOriginationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVoiceConnectorOriginationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
