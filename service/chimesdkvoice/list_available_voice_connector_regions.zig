...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAvailableVoiceConnectorRegionsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAvailableVoiceConnectorRegionsOutput {
    var result: ListAvailableVoiceConnectorRegionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAvailableVoiceConnectorRegionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
