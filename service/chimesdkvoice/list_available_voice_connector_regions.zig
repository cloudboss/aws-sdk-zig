...
pub const ListAvailableVoiceConnectorRegionsOutput = struct {
    ...
    voice_connector_regions: ?[]const VoiceConnectorAwsRegion = null,

    pub const json_field_names = .{
        .voice_connector_regions = "VoiceConnectorRegions",
    };
};
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAvailableVoiceConnectorRegionsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    const path = "/voice-connector-regions";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAvailableVoiceConnectorRegionsOutput {
    var result: ListAvailableVoiceConnectorRegionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAvailableVoiceConnectorRegionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
