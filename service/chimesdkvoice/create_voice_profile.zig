...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVoiceProfileInput, options: Options) !CreateVoiceProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVoiceProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVoiceProfileOutput {
    var result: CreateVoiceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVoiceProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
