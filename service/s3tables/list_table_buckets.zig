...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTableBucketsInput, options: Options) !ListTableBucketsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTableBucketsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.continuation_token) |v| {
        ...
        try query_buf.appendSlice(alloc, "continuationToken=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.max_buckets) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "maxBuckets=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTableBucketsOutput {
    var result: ListTableBucketsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTableBucketsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
