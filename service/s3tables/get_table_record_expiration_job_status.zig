...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableRecordExpirationJobStatusInput, options: Options) !GetTableRecordExpirationJobStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTableRecordExpirationJobStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTableRecordExpirationJobStatusOutput {
    var result: GetTableRecordExpirationJobStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTableRecordExpirationJobStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
