...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutTableBucketReplicationInput, options: Options) !PutTableBucketReplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutTableBucketReplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutTableBucketReplicationOutput {
    var result: PutTableBucketReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutTableBucketReplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
