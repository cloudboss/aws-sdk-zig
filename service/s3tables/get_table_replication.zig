...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableReplicationInput, options: Options) !GetTableReplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTableReplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTableReplicationOutput {
    var result: GetTableReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTableReplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
