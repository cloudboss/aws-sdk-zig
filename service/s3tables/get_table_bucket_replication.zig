...
pub const GetTableBucketReplicationInput = struct {
...
pub const GetTableBucketReplicationOutput = struct {
    /// The replication configuration for the table bucket, including the IAM role
    /// and replication rules.
    configuration: ?TableBucketReplicationConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableBucketReplicationInput, options: Options) !GetTableBucketReplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTableBucketReplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.table_bucket_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTableBucketReplicationOutput {
    var result: GetTableBucketReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTableBucketReplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
