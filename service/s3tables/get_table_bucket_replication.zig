...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableBucketReplicationInput, options: CallOptions) !GetTableBucketReplicationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTableBucketReplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTableBucketReplicationOutput {
    var result: GetTableBucketReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTableBucketReplicationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
