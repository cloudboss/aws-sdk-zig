...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableBucketInput, options: Options) !GetTableBucketOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTableBucketInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTableBucketOutput {
    var result: GetTableBucketOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTableBucketOutput, body, alloc);
    }
    _ = status;
    _ = headers;
