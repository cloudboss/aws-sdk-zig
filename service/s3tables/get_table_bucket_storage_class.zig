...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableBucketStorageClassInput, options: Options) !GetTableBucketStorageClassOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTableBucketStorageClassInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTableBucketStorageClassOutput {
    var result: GetTableBucketStorageClassOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTableBucketStorageClassOutput, body, alloc);
    }
    _ = status;
    _ = headers;
