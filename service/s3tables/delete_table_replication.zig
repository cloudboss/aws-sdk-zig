...
pub const DeleteTableReplicationOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTableReplicationInput, options: CallOptions) !DeleteTableReplicationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTableReplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.version_token);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTableReplicationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
