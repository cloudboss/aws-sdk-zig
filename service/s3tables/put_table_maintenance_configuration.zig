...
const PutTableMaintenanceConfigurationOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutTableMaintenanceConfigurationInput, options: Options) !PutTableMaintenanceConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutTableMaintenanceConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.value), input.value, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutTableMaintenanceConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
