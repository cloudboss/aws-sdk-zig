...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTableMaintenanceJobStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/maintenance-job-status");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTableMaintenanceJobStatusOutput {
    var result: GetTableMaintenanceJobStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTableMaintenanceJobStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
