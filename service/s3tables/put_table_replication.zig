...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutTableReplicationInput, options: Options) !PutTableReplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutTableReplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutTableReplicationOutput {
    var result: PutTableReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutTableReplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
