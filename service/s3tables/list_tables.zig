...
const TableSummary = @import("table_summary.zig").TableSummary;
...
pub const ListTablesOutput = struct {
    ...
    /// A list of tables.
    tables: ?[]const TableSummary = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTablesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.max_tables) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "maxTables=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTablesOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTablesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
