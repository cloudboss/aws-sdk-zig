...
pub const PutTableBucketStorageClassOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutTableBucketStorageClassInput, options: CallOptions) !PutTableBucketStorageClassOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutTableBucketStorageClassInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.storage_class_configuration), input.storage_class_configuration, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutTableBucketStorageClassOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
