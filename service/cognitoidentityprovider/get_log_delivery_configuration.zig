...
const LogDeliveryConfigurationType = @import("log_delivery_configuration_type.zig").LogDeliveryConfigurationType;

pub const GetLogDeliveryConfigurationInput = struct {
...
pub const GetLogDeliveryConfigurationOutput = struct {
    /// The logging configuration of the requested user pool. Includes types of logs
    /// configured and their destinations.
    log_delivery_configuration: ?LogDeliveryConfigurationType = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLogDeliveryConfigurationInput, options: CallOptions) !GetLogDeliveryConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLogDeliveryConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.GetLogDeliveryConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLogDeliveryConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLogDeliveryConfigurationOutput, body, allocator);
}
