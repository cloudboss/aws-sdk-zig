...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentityProviderByIdentifierInput, options: Options) !GetIdentityProviderByIdentifierOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityProviderByIdentifierInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.GetIdentityProviderByIdentifier");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityProviderByIdentifierOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetIdentityProviderByIdentifierOutput, body, alloc);
}
