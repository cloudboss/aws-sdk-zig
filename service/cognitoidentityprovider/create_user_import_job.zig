...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUserImportJobInput, options: Options) !CreateUserImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUserImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.CreateUserImportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUserImportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateUserImportJobOutput, body, alloc);
}
