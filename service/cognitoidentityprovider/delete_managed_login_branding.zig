...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteManagedLoginBrandingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteManagedLoginBranding");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteManagedLoginBrandingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
