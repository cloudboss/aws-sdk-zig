...
pub const DeleteManagedLoginBrandingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteManagedLoginBrandingInput, options: CallOptions) !DeleteManagedLoginBrandingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteManagedLoginBrandingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteManagedLoginBranding");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteManagedLoginBrandingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
