...
pub const ForgotPasswordInput = struct {
    ...
    /// The ID of the user pool app client associated with the current signed-in
    /// user.
    client_id: []const u8,
...
pub const ForgotPasswordOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ForgotPasswordInput, options: CallOptions) !ForgotPasswordOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ForgotPasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.ForgotPassword");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ForgotPasswordOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ForgotPasswordOutput, body, allocator);
}
