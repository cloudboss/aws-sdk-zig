...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteUserAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteUserAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteUserAttributesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
