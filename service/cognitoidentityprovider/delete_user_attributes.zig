...
pub const DeleteUserAttributesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUserAttributesInput, options: Options) !DeleteUserAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUserAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteUserAttributes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUserAttributesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
