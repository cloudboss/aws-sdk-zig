...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateSoftwareTokenInput, options: Options) !AssociateSoftwareTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateSoftwareTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AssociateSoftwareToken");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateSoftwareTokenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateSoftwareTokenOutput, body, alloc);
}
