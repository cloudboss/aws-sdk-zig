...
pub const RevokeTokenInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeTokenInput, options: Options) !RevokeTokenOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RevokeTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.RevokeToken");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RevokeTokenOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
