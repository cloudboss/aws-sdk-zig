...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCSVHeaderInput, options: Options) !GetCSVHeaderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCSVHeaderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.GetCSVHeader");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCSVHeaderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCSVHeaderOutput, body, alloc);
}
