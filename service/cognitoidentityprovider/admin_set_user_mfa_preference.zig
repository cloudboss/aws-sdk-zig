...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminSetUserMFAPreferenceInput, options: Options) !AdminSetUserMFAPreferenceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminSetUserMFAPreferenceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminSetUserMFAPreference");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminSetUserMFAPreferenceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
