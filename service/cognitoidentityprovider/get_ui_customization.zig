...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUICustomizationInput, options: Options) !GetUICustomizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUICustomizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.GetUICustomization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUICustomizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUICustomizationOutput, body, alloc);
}
