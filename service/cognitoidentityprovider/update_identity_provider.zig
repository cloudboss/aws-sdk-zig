...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateIdentityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateIdentityProvider");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateIdentityProviderOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateIdentityProviderOutput, body, allocator);
}
