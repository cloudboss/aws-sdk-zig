...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIdentityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateIdentityProvider");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIdentityProviderOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateIdentityProviderOutput, body, alloc);
}
