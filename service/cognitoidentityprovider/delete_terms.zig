...
pub const DeleteTermsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTermsInput, options: CallOptions) !DeleteTermsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTermsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteTerms");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTermsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
