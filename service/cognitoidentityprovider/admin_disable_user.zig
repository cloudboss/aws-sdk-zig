...
pub const AdminDisableUserInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminDisableUserInput, options: Options) !AdminDisableUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminDisableUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminDisableUser");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminDisableUserOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
