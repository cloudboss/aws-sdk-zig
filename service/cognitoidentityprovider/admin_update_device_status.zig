...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminUpdateDeviceStatusInput, options: Options) !AdminUpdateDeviceStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminUpdateDeviceStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminUpdateDeviceStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminUpdateDeviceStatusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
