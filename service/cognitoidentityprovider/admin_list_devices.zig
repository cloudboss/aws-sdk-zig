...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminListDevicesInput, options: Options) !AdminListDevicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminListDevicesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminListDevices");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminListDevicesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AdminListDevicesOutput, body, alloc);
}
