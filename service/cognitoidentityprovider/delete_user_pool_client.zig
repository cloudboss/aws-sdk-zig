...
pub const DeleteUserPoolClientInput = struct {
...
pub const DeleteUserPoolClientOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUserPoolClientInput, options: CallOptions) !DeleteUserPoolClientOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUserPoolClientInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteUserPoolClient");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUserPoolClientOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
