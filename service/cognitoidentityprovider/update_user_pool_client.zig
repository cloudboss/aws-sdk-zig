...
pub const UpdateUserPoolClientInput = struct {
...
pub const UpdateUserPoolClientOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateUserPoolClientInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateUserPoolClient");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateUserPoolClientOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateUserPoolClientOutput, body, allocator);
}
