...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUsersInGroupInput, options: Options) !ListUsersInGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUsersInGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ListUsersInGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUsersInGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUsersInGroupOutput, body, alloc);
}
