...
pub const DeleteResourceServerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResourceServerInput, options: CallOptions) !DeleteResourceServerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteResourceServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteResourceServer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteResourceServerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
