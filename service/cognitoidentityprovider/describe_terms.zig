...
pub const DescribeTermsInput = struct {
...
pub const DescribeTermsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTermsInput, options: CallOptions) !DescribeTermsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTermsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeTerms");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTermsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTermsOutput, body, allocator);
}
