...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUserPoolMfaConfigInput, options: Options) !GetUserPoolMfaConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUserPoolMfaConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.GetUserPoolMfaConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUserPoolMfaConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUserPoolMfaConfigOutput, body, alloc);
}
