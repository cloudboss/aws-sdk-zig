...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminSetUserSettingsInput, options: Options) !AdminSetUserSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminSetUserSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminSetUserSettings");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminSetUserSettingsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
