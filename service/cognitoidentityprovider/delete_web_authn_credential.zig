...
pub const DeleteWebAuthnCredentialInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteWebAuthnCredentialInput, options: Options) !DeleteWebAuthnCredentialOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteWebAuthnCredentialInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteWebAuthnCredential");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteWebAuthnCredentialOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
