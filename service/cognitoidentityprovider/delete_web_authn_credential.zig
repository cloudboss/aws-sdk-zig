...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteWebAuthnCredentialInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteWebAuthnCredential");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteWebAuthnCredentialOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
