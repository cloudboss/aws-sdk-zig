...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRiskConfigurationInput, options: Options) !DescribeRiskConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRiskConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeRiskConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRiskConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRiskConfigurationOutput, body, alloc);
}
