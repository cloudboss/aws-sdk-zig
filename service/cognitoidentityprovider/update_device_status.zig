...
pub const UpdateDeviceStatusInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDeviceStatusInput, options: Options) !UpdateDeviceStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDeviceStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateDeviceStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDeviceStatusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
