...
pub const AdminRemoveUserFromGroupInput = struct {
    ...
    /// The ID of the user pool that contains the group and the user that you want
    /// to
    /// remove.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminRemoveUserFromGroupInput, options: CallOptions) !AdminRemoveUserFromGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminRemoveUserFromGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminRemoveUserFromGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminRemoveUserFromGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
