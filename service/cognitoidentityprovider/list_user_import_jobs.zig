...
pub const ListUserImportJobsInput = struct {
    /// The maximum number of import jobs that you want Amazon Cognito to return in
    /// the
    /// response.
    max_results: i32,
...
pub const ListUserImportJobsOutput = struct {
    ...
    /// response
    /// includes logging destination, status, and the Amazon S3 pre-signed URL for
    /// CSV upload.
    user_import_jobs: ?[]const UserImportJobType = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUserImportJobsInput, options: CallOptions) !ListUserImportJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUserImportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.ListUserImportJobs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUserImportJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUserImportJobsOutput, body, allocator);
}
