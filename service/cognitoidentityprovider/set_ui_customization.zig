...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetUICustomizationInput, options: Options) !SetUICustomizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetUICustomizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.SetUICustomization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetUICustomizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SetUICustomizationOutput, body, alloc);
}
