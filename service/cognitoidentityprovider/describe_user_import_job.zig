...
pub const DescribeUserImportJobInput = struct {
...
pub const DescribeUserImportJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUserImportJobInput, options: CallOptions) !DescribeUserImportJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeUserImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeUserImportJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeUserImportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeUserImportJobOutput, body, allocator);
}
