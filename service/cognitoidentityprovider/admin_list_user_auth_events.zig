...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminListUserAuthEventsInput, options: Options) !AdminListUserAuthEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminListUserAuthEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminListUserAuthEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminListUserAuthEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AdminListUserAuthEventsOutput, body, alloc);
}
