...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminSetUserPasswordInput, options: Options) !AdminSetUserPasswordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminSetUserPasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminSetUserPassword");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminSetUserPasswordOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
