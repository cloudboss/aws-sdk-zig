...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserPoolDomainInput, options: Options) !UpdateUserPoolDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserPoolDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateUserPoolDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserPoolDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateUserPoolDomainOutput, body, alloc);
}
