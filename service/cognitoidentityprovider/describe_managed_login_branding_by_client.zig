...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeManagedLoginBrandingByClientInput, options: Options) !DescribeManagedLoginBrandingByClientOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeManagedLoginBrandingByClientInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeManagedLoginBrandingByClient");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeManagedLoginBrandingByClientOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeManagedLoginBrandingByClientOutput, body, alloc);
}
