...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminDisableProviderForUserInput, options: Options) !AdminDisableProviderForUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminDisableProviderForUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminDisableProviderForUser");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminDisableProviderForUserOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
