...
pub const DescribeUserPoolClientInput = struct {
...
pub const DescribeUserPoolClientOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUserPoolClientInput, options: CallOptions) !DescribeUserPoolClientOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeUserPoolClientInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeUserPoolClient");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeUserPoolClientOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeUserPoolClientOutput, body, allocator);
}
