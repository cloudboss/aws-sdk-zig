...
pub const StopUserImportJobInput = struct {
...
pub const StopUserImportJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopUserImportJobInput, options: CallOptions) !StopUserImportJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopUserImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.StopUserImportJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopUserImportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopUserImportJobOutput, body, allocator);
}
