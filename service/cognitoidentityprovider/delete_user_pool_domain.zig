...
pub const DeleteUserPoolDomainInput = struct {
    /// The domain that you want to delete. For custom domains, this is the
    /// fully-qualified
    /// domain name like `auth.example.com`. For Amazon Cognito prefix domains, this
    /// is the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUserPoolDomainInput, options: Options) !DeleteUserPoolDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUserPoolDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteUserPoolDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUserPoolDomainOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
