...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdentityProvidersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ListIdentityProviders");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdentityProvidersOutput {
    ...
    return aws.json.parseJsonObject(ListIdentityProvidersOutput, body, alloc);
}
