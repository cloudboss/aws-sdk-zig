...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUserPoolDomainInput, options: Options) !CreateUserPoolDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUserPoolDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.CreateUserPoolDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUserPoolDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateUserPoolDomainOutput, body, alloc);
}
