...
pub const SetLogDeliveryConfigurationInput = struct {
...
pub const SetLogDeliveryConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetLogDeliveryConfigurationInput, options: CallOptions) !SetLogDeliveryConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetLogDeliveryConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.SetLogDeliveryConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetLogDeliveryConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SetLogDeliveryConfigurationOutput, body, allocator);
}
