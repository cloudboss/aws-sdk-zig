...
fn serializeRequest(alloc: std.mem.Allocator, input: SetUserSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.SetUserSettings");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetUserSettingsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
