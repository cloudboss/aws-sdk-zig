...
pub const ForgetDeviceInput = struct {
...
pub const ForgetDeviceOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ForgetDeviceInput, options: CallOptions) !ForgetDeviceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ForgetDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.ForgetDevice");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ForgetDeviceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
