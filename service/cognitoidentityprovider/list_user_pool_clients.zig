...
const UserPoolClientDescription = @import("user_pool_client_description.zig").UserPoolClientDescription;

pub const ListUserPoolClientsInput = struct {
    /// The maximum number of app clients that you want Amazon Cognito to return in
    /// the
    /// response.
...
pub const ListUserPoolClientsOutput = struct {
    ...
    user_pool_clients: ?[]const UserPoolClientDescription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUserPoolClientsInput, options: CallOptions) !ListUserPoolClientsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUserPoolClientsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.ListUserPoolClients");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUserPoolClientsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUserPoolClientsOutput, body, allocator);
}
