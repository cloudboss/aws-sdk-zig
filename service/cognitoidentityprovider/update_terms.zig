...
pub const UpdateTermsInput = struct {
    ...
    terms_name: ?[]const u8 = null,

    /// This parameter is reserved for future use and currently accepts only one
    /// value.
    terms_source: ?TermsSourceType = null,
...
pub const UpdateTermsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTermsInput, options: CallOptions) !UpdateTermsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTermsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateTerms");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTermsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateTermsOutput, body, allocator);
}
