...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminEnableUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminEnableUser");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminEnableUserOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
