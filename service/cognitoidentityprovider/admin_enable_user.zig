...
pub const AdminEnableUserInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminEnableUserInput, options: Options) !AdminEnableUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminEnableUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminEnableUser");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminEnableUserOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
