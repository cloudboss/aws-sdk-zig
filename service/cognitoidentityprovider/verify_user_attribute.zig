...
pub const VerifyUserAttributeInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VerifyUserAttributeInput, options: Options) !VerifyUserAttributeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: VerifyUserAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.VerifyUserAttribute");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !VerifyUserAttributeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
