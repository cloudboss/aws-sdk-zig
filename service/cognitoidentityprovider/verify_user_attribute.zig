...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyUserAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.VerifyUserAttribute");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyUserAttributeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
