...
pub const AdminGetDeviceInput = struct {
...
pub const AdminGetDeviceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminGetDeviceInput, options: CallOptions) !AdminGetDeviceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminGetDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminGetDevice");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminGetDeviceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(AdminGetDeviceOutput, body, allocator);
}
