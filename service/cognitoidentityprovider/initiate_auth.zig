...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InitiateAuthInput, options: Options) !InitiateAuthOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InitiateAuthInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.InitiateAuth");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InitiateAuthOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(InitiateAuthOutput, body, alloc);
}
