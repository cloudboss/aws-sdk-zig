...
pub const UpdateUserAttributesInput = struct {
    ...
    /// triggers. When
    /// you use the UpdateUserAttributes API action, Amazon Cognito invokes the
    /// function that is assigned
    /// to the *custom message* trigger. When Amazon Cognito invokes this function,
    /// it
    ...
    /// function code
    /// in Lambda, you can process the `clientMetadata` value to enhance your
    /// workflow
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserAttributesInput, options: Options) !UpdateUserAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateUserAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateUserAttributesOutput, body, alloc);
}
