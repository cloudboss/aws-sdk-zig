...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminResetUserPasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminResetUserPassword");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminResetUserPasswordOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
