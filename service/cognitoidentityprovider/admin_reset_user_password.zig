...
pub const AdminResetUserPasswordInput = struct {
    ...
    /// invokes
    /// this function, it passes a JSON payload, which the function receives as
    /// input. This
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminResetUserPasswordInput, options: CallOptions) !AdminResetUserPasswordOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminResetUserPasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminResetUserPassword");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminResetUserPasswordOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
