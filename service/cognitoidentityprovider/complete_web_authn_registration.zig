...
pub const CompleteWebAuthnRegistrationInput = struct {
    ...
    /// A
    /// [RegistrationResponseJSON](https://www.w3.org/TR/WebAuthn-3/#dictdef-registrationresponsejson) public-key credential response from the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CompleteWebAuthnRegistrationInput, options: Options) !CompleteWebAuthnRegistrationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CompleteWebAuthnRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.CompleteWebAuthnRegistration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CompleteWebAuthnRegistrationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
