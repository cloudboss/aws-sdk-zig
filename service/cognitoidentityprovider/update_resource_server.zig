...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResourceServerInput, options: Options) !UpdateResourceServerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResourceServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateResourceServer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResourceServerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateResourceServerOutput, body, alloc);
}
