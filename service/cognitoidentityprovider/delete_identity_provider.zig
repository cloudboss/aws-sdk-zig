...
pub const DeleteIdentityProviderInput = struct {
...
const DeleteIdentityProviderOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteIdentityProviderInput, options: Options) !DeleteIdentityProviderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteIdentityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteIdentityProvider");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteIdentityProviderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
