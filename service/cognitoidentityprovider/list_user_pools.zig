...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUserPoolsInput, options: Options) !ListUserPoolsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUserPoolsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ListUserPools");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUserPoolsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUserPoolsOutput, body, alloc);
}
