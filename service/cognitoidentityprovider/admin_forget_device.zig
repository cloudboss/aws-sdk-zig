...
pub const AdminForgetDeviceInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminForgetDeviceInput, options: CallOptions) !AdminForgetDeviceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminForgetDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminForgetDevice");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminForgetDeviceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
