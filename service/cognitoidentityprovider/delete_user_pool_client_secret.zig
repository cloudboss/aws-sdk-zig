...
pub const DeleteUserPoolClientSecretInput = struct {
...
pub const DeleteUserPoolClientSecretOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUserPoolClientSecretInput, options: CallOptions) !DeleteUserPoolClientSecretOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUserPoolClientSecretInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteUserPoolClientSecret");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUserPoolClientSecretOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
