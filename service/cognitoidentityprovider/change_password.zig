...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ChangePasswordInput, options: Options) !ChangePasswordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ChangePasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ChangePassword");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ChangePasswordOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
