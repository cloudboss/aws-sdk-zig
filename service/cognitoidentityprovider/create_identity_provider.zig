...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIdentityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.CreateIdentityProvider");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIdentityProviderOutput {
    ...
    return aws.json.parseJsonObject(CreateIdentityProviderOutput, body, alloc);
}
