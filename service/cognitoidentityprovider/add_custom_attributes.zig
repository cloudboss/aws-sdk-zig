...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddCustomAttributesInput, options: Options) !AddCustomAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddCustomAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AddCustomAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddCustomAttributesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
