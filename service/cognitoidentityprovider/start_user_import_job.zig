...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartUserImportJobInput, options: Options) !StartUserImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartUserImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.StartUserImportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartUserImportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartUserImportJobOutput, body, alloc);
}
