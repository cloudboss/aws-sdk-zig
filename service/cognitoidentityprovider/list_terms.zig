...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTermsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ListTerms");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTermsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListTermsOutput, body, alloc);
}
