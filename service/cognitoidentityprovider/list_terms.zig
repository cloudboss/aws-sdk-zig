...
const TermsDescriptionType = @import("terms_description_type.zig").TermsDescriptionType;

pub const ListTermsInput = struct {
    /// The maximum number of terms documents that you want Amazon Cognito to return
    /// in the
    /// response.
...
pub const ListTermsOutput = struct {
    /// This API operation returns a limited number of results. The pagination token
    /// is
    /// an identifier that you can present in an additional API request with the
    /// same parameters. When
    ...
    terms: ?[]const TermsDescriptionType = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTermsInput, options: CallOptions) !ListTermsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTermsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.ListTerms");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTermsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListTermsOutput, body, allocator);
}
