...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUserPoolClientInput, options: Options) !CreateUserPoolClientOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUserPoolClientInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.CreateUserPoolClient");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUserPoolClientOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateUserPoolClientOutput, body, alloc);
}
