...
pub const AdminUpdateAuthEventFeedbackInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminUpdateAuthEventFeedbackInput, options: Options) !AdminUpdateAuthEventFeedbackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AdminUpdateAuthEventFeedbackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminUpdateAuthEventFeedback");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AdminUpdateAuthEventFeedbackOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
