...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminUpdateAuthEventFeedbackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminUpdateAuthEventFeedback");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminUpdateAuthEventFeedbackOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
