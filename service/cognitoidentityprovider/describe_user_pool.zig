...
pub const DescribeUserPoolInput = struct {
...
pub const DescribeUserPoolOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUserPoolInput, options: CallOptions) !DescribeUserPoolOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeUserPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeUserPool");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeUserPoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeUserPoolOutput, body, allocator);
}
