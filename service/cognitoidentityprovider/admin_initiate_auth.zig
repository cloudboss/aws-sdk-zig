...
const ContextDataType = @import("context_data_type.zig").ContextDataType;
...
pub const AdminInitiateAuthInput = struct {
    ...
    context_data: ?ContextDataType = null,
...
pub const AdminInitiateAuthOutput = struct {
    /// The outcome of successful authentication. This is only returned if the user
    /// pool has
    /// no additional challenges to return. If Amazon Cognito returns another
    ...
    /// challenge parameters as a
    /// guide to the responses your user or application must provide for the
    /// returned
    ...
    /// request results in another authentication challenge, Amazon Cognito returns
    /// a session ID and the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminInitiateAuthInput, options: CallOptions) !AdminInitiateAuthOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminInitiateAuthInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminInitiateAuth");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminInitiateAuthOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AdminInitiateAuthOutput, body, allocator);
}
