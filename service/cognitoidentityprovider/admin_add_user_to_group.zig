...
pub const AdminAddUserToGroupInput = struct {
    ...
    /// The ID of the user pool that contains the group that you want to add the
    /// user
    /// to.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminAddUserToGroupInput, options: CallOptions) !AdminAddUserToGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminAddUserToGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminAddUserToGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminAddUserToGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
