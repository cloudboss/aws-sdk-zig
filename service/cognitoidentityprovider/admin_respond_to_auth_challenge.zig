...
pub const AdminRespondToAuthChallengeInput = struct {
    ...
    /// triggers.
    /// When you use the AdminRespondToAuthChallenge API action, Amazon Cognito
    /// invokes any functions
    ...
    /// parameter in
    /// your AdminRespondToAuthChallenge request. In your function code in Lambda,
    /// you can
...
pub const AdminRespondToAuthChallengeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminRespondToAuthChallengeInput, options: CallOptions) !AdminRespondToAuthChallengeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminRespondToAuthChallengeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminRespondToAuthChallenge");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminRespondToAuthChallengeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AdminRespondToAuthChallengeOutput, body, allocator);
}
