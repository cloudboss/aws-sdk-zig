...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteUserPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteUserPool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteUserPoolOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
