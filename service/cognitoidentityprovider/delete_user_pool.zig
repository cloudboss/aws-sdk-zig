...
pub const DeleteUserPoolInput = struct {
...
pub const DeleteUserPoolOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUserPoolInput, options: CallOptions) !DeleteUserPoolOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUserPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DeleteUserPool");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUserPoolOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
