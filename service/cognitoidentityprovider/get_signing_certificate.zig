...
pub const GetSigningCertificateInput = struct {
...
pub const GetSigningCertificateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSigningCertificateInput, options: CallOptions) !GetSigningCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSigningCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.GetSigningCertificate");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSigningCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSigningCertificateOutput, body, allocator);
}
