...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUserAuthFactorsInput, options: Options) !GetUserAuthFactorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUserAuthFactorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.GetUserAuthFactors");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUserAuthFactorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUserAuthFactorsOutput, body, alloc);
}
