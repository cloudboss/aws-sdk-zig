...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeResourceServerInput, options: Options) !DescribeResourceServerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeResourceServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeResourceServer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeResourceServerOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeResourceServerOutput, body, alloc);
}
