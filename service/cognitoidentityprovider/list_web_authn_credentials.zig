...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWebAuthnCredentialsInput, options: Options) !ListWebAuthnCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWebAuthnCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ListWebAuthnCredentials");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWebAuthnCredentialsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListWebAuthnCredentialsOutput, body, alloc);
}
