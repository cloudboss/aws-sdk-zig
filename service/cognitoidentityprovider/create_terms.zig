...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTermsInput, options: Options) !CreateTermsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTermsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.CreateTerms");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTermsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTermsOutput, body, alloc);
}
