...
pub const AdminUserGlobalSignOutInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminUserGlobalSignOutInput, options: CallOptions) !AdminUserGlobalSignOutOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminUserGlobalSignOutInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminUserGlobalSignOut");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminUserGlobalSignOutOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
