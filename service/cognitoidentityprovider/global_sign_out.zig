...
pub const GlobalSignOutInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GlobalSignOutInput, options: Options) !GlobalSignOutOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GlobalSignOutInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.GlobalSignOut");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GlobalSignOutOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
