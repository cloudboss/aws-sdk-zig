...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUserPoolDomainInput, options: Options) !DescribeUserPoolDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUserPoolDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeUserPoolDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUserPoolDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeUserPoolDomainOutput, body, alloc);
}
