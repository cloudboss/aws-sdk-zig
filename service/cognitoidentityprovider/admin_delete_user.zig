...
pub const AdminDeleteUserInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminDeleteUserInput, options: CallOptions) !AdminDeleteUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminDeleteUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminDeleteUser");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminDeleteUserOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
