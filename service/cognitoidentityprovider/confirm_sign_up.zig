...
pub const ConfirmSignUpInput = struct {
    ...
    /// assigned to the *post confirmation* trigger. When Amazon Cognito invokes
    /// this
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfirmSignUpInput, options: Options) !ConfirmSignUpOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ConfirmSignUpInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ConfirmSignUp");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConfirmSignUpOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ConfirmSignUpOutput, body, alloc);
}
