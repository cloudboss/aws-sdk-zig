...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResendConfirmationCodeInput, options: Options) !ResendConfirmationCodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResendConfirmationCodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ResendConfirmationCode");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResendConfirmationCodeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResendConfirmationCodeOutput, body, alloc);
}
