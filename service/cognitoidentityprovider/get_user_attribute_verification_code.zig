...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUserAttributeVerificationCodeInput, options: Options) !GetUserAttributeVerificationCodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUserAttributeVerificationCodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.GetUserAttributeVerificationCode");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUserAttributeVerificationCodeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUserAttributeVerificationCodeOutput, body, alloc);
}
