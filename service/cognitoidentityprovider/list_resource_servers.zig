...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceServersInput, options: Options) !ListResourceServersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceServersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ListResourceServers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceServersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceServersOutput, body, alloc);
}
