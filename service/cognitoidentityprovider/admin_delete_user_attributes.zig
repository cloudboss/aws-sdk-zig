...
pub const AdminDeleteUserAttributesInput = struct {
    /// An array of strings representing the user attribute names you want to
    /// delete.
    ///
    /// For custom attributes, you must prepend the `custom:` prefix to the
    /// attribute name.
    user_attribute_names: []const []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminDeleteUserAttributesInput, options: Options) !AdminDeleteUserAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminDeleteUserAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminDeleteUserAttributes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminDeleteUserAttributesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
