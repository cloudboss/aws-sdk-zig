...
pub const AdminListGroupsForUserInput = struct {
...
pub const AdminListGroupsForUserOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdminListGroupsForUserInput, options: CallOptions) !AdminListGroupsForUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdminListGroupsForUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.AdminListGroupsForUser");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdminListGroupsForUserOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AdminListGroupsForUserOutput, body, allocator);
}
