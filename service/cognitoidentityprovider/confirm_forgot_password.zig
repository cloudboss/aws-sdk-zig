...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfirmForgotPasswordInput, options: Options) !ConfirmForgotPasswordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ConfirmForgotPasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ConfirmForgotPassword");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConfirmForgotPasswordOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
