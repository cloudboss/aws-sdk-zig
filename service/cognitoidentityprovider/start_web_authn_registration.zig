...
fn serializeRequest(alloc: std.mem.Allocator, input: StartWebAuthnRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.StartWebAuthnRegistration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartWebAuthnRegistrationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StartWebAuthnRegistrationOutput, body, alloc);
}
