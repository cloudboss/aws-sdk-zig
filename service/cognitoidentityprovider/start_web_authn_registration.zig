...
pub const StartWebAuthnRegistrationInput = struct {
...
pub const StartWebAuthnRegistrationOutput = struct {
    /// The information that a user can provide in their request to register with
    /// their
    /// passkey provider.
    credential_creation_options: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartWebAuthnRegistrationInput, options: CallOptions) !StartWebAuthnRegistrationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartWebAuthnRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.StartWebAuthnRegistration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartWebAuthnRegistrationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StartWebAuthnRegistrationOutput, body, allocator);
}
