...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfirmDeviceInput, options: Options) !ConfirmDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ConfirmDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.ConfirmDevice");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConfirmDeviceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ConfirmDeviceOutput, body, alloc);
}
