...
pub const DescribeManagedLoginBrandingInput = struct {
...
pub const DescribeManagedLoginBrandingOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeManagedLoginBrandingInput, options: CallOptions) !DescribeManagedLoginBrandingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeManagedLoginBrandingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeManagedLoginBranding");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeManagedLoginBrandingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeManagedLoginBrandingOutput, body, allocator);
}
