...
pub const UpdateAuthEventFeedbackInput = struct {
    ...
    /// passed to your user in
    /// the notification email message from the event.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAuthEventFeedbackInput, options: Options) !UpdateAuthEventFeedbackOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAuthEventFeedbackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateAuthEventFeedback");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAuthEventFeedbackOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
