...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAuthEventFeedbackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.UpdateAuthEventFeedback");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAuthEventFeedbackOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
