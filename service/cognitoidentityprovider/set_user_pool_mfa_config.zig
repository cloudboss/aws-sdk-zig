...
pub const SetUserPoolMfaConfigInput = struct {
    /// Sets configuration for user pool email message MFA and sign-in with one-time
    /// passwords
    /// (OTPs). Includes the subject and body of the email message template for
    /// sign-in and MFA
    ...
    /// Configures a user pool for time-based one-time password (TOTP) MFA. Enables
    /// or
    /// disables TOTP.
    ...
    /// and
    /// registration. You can set this configuration independent of the MFA
    /// configuration
...
pub const SetUserPoolMfaConfigOutput = struct {
    ...
    /// authenticators like biometric and security-key devices. Includes
    /// relying-party
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetUserPoolMfaConfigInput, options: CallOptions) !SetUserPoolMfaConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetUserPoolMfaConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityProviderService.SetUserPoolMfaConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetUserPoolMfaConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SetUserPoolMfaConfigOutput, body, allocator);
}
