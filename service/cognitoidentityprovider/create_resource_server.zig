...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResourceServerInput, options: Options) !CreateResourceServerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourceServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.CreateResourceServer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourceServerOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateResourceServerOutput, body, alloc);
}
