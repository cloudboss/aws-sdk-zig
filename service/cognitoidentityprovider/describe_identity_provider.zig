...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIdentityProviderInput, options: Options) !DescribeIdentityProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIdentityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.DescribeIdentityProvider");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIdentityProviderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeIdentityProviderOutput, body, alloc);
}
