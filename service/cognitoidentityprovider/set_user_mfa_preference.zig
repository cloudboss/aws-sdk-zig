...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetUserMFAPreferenceInput, options: Options) !SetUserMFAPreferenceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetUserMFAPreferenceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityProviderService.SetUserMFAPreference");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetUserMFAPreferenceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
