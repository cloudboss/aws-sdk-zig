...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDatasetExportJobInput, options: Options) !CreateDatasetExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDatasetExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.CreateDatasetExportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDatasetExportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDatasetExportJobOutput, body, alloc);
}
