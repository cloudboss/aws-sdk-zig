...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFeatureTransformationInput, options: Options) !DescribeFeatureTransformationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFeatureTransformationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.DescribeFeatureTransformation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFeatureTransformationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFeatureTransformationOutput, body, alloc);
}
