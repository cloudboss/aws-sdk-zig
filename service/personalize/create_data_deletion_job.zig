...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataDeletionJobInput, options: Options) !CreateDataDeletionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataDeletionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.CreateDataDeletionJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataDeletionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDataDeletionJobOutput, body, alloc);
}
