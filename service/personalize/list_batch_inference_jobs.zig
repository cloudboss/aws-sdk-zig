...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBatchInferenceJobsInput, options: Options) !ListBatchInferenceJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBatchInferenceJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.ListBatchInferenceJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBatchInferenceJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBatchInferenceJobsOutput, body, alloc);
}
