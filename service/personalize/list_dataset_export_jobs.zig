...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDatasetExportJobsInput, options: Options) !ListDatasetExportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDatasetExportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.ListDatasetExportJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDatasetExportJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDatasetExportJobsOutput, body, alloc);
}
