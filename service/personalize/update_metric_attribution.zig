...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMetricAttributionInput, options: Options) !UpdateMetricAttributionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMetricAttributionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.UpdateMetricAttribution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMetricAttributionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateMetricAttributionOutput, body, alloc);
}
