...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSolutionVersionsInput, options: Options) !ListSolutionVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSolutionVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.ListSolutionVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSolutionVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSolutionVersionsOutput, body, alloc);
}
