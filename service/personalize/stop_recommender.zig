...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopRecommenderInput, options: Options) !StopRecommenderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopRecommenderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.StopRecommender");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopRecommenderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopRecommenderOutput, body, alloc);
}
