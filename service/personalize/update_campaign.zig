...
pub const UpdateCampaignInput = struct {
    ...
    /// latest solution version of your solution,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCampaignInput, options: Options) !UpdateCampaignOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCampaignInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.UpdateCampaign");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCampaignOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCampaignOutput, body, alloc);
}
