...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDatasetImportJobsInput, options: Options) !ListDatasetImportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDatasetImportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.ListDatasetImportJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDatasetImportJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDatasetImportJobsOutput, body, alloc);
}
