...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMetricAttributionMetricsInput, options: Options) !ListMetricAttributionMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMetricAttributionMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.ListMetricAttributionMetrics");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMetricAttributionMetricsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMetricAttributionMetricsOutput, body, alloc);
}
