...
pub const GetSolutionMetricsInput = struct {
...
pub const GetSolutionMetricsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSolutionMetricsInput, options: CallOptions) !GetSolutionMetricsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSolutionMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonPersonalize.GetSolutionMetrics");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSolutionMetricsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSolutionMetricsOutput, body, allocator);
}
