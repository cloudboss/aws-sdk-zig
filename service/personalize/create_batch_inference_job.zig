...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBatchInferenceJobInput, options: Options) !CreateBatchInferenceJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBatchInferenceJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonPersonalize.CreateBatchInferenceJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBatchInferenceJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateBatchInferenceJobOutput, body, alloc);
}
