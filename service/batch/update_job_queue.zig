...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateJobQueueInput, options: CallOptions) !UpdateJobQueueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateJobQueueInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateJobQueueOutput {
    var result: UpdateJobQueueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateJobQueueOutput, body, allocator);
    }
    _ = status;
    _ = headers;
