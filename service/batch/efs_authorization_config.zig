...
pub const EFSAuthorizationConfig = struct {
    ...
    /// `/` which enforces the path set on the EFS access point. If an access point
    /// is used,
    ...
    /// when mounting the
    /// Amazon EFS file system. If enabled, transit encryption must be enabled in
    /// the
    ...
    /// points](https://docs.aws.amazon.com/batch/latest/userguide/efs-volumes.html#efs-volume-accesspoints) in
