...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobQueueSnapshotInput, options: Options) !GetJobQueueSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetJobQueueSnapshotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetJobQueueSnapshotOutput {
    var result: GetJobQueueSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetJobQueueSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
