...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeComputeEnvironmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.compute_environments) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"computeEnvironments\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeComputeEnvironmentsOutput {
    var result: DescribeComputeEnvironmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeComputeEnvironmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
