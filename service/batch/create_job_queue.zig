...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateJobQueueInput, options: CallOptions) !CreateJobQueueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateJobQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.compute_environment_order) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"computeEnvironmentOrder\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"jobQueueName\":");
    try aws.json.writeValue(@TypeOf(input.job_queue_name), input.job_queue_name, allocator, &body_buf);
    has_prev = true;
    if (input.job_queue_type) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"jobQueueType\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.job_state_time_limit_actions) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"jobStateTimeLimitActions\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    if (input.scheduling_policy_arn) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"schedulingPolicyArn\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.service_environment_order) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"serviceEnvironmentOrder\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.state) |v| {
