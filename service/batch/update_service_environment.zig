...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceEnvironmentInput, options: Options) !UpdateServiceEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceEnvironmentOutput {
    var result: UpdateServiceEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateServiceEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
