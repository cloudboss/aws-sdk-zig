...
pub const DescribeQuotaShareInput = struct {
...
pub const DescribeQuotaShareOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeQuotaShareInput, options: CallOptions) !DescribeQuotaShareOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeQuotaShareInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/describequotashare";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeQuotaShareOutput {
    var result: DescribeQuotaShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeQuotaShareOutput, body, allocator);
    }
    _ = status;
    _ = headers;
