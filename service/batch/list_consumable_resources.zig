...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConsumableResourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConsumableResourcesOutput {
    var result: ListConsumableResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConsumableResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
