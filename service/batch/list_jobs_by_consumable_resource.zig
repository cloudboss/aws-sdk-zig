...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListJobsByConsumableResourceInput, options: CallOptions) !ListJobsByConsumableResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListJobsByConsumableResourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListJobsByConsumableResourceOutput {
    var result: ListJobsByConsumableResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListJobsByConsumableResourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
