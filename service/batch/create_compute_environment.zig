...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateComputeEnvironmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"computeEnvironmentName\":");
    try aws.json.writeValue(@TypeOf(input.compute_environment_name), input.compute_environment_name, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.eks_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"eksConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateComputeEnvironmentOutput {
    var result: CreateComputeEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateComputeEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
