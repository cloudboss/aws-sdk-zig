...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConsumableResourceInput, options: Options) !UpdateConsumableResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConsumableResourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConsumableResourceOutput {
    var result: UpdateConsumableResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConsumableResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
