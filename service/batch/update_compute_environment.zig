...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateComputeEnvironmentInput, options: Options) !UpdateComputeEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateComputeEnvironmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"computeEnvironment\":");
    try aws.json.writeValue(@TypeOf(input.compute_environment), input.compute_environment, alloc, &body_buf);
    has_prev = true;
    if (input.compute_resources) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"computeResources\":");
    ...
    if (input.unmanagedv_cpus) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"unmanagedvCpus\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.update_policy) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"updatePolicy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateComputeEnvironmentOutput {
    var result: UpdateComputeEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateComputeEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
