...
/// template name in the request, but not
/// both.
///
...
pub const LaunchTemplateSpecificationOverride = struct {
    ...
    ///   across launch template overrides can't overlap for the same compute
    ...
    /// `true`. During an infrastructure update, if either `$Default` or `$Latest`
