...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSchedulingPolicyInput, options: CallOptions) !CreateSchedulingPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSchedulingPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSchedulingPolicyOutput {
    var result: CreateSchedulingPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSchedulingPolicyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
