...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeServiceEnvironmentsInput, options: Options) !DescribeServiceEnvironmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeServiceEnvironmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.service_environments) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"serviceEnvironments\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeServiceEnvironmentsOutput {
    var result: DescribeServiceEnvironmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeServiceEnvironmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
