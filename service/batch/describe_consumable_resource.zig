...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConsumableResourceInput, options: Options) !DescribeConsumableResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConsumableResourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConsumableResourceOutput {
    var result: DescribeConsumableResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeConsumableResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
