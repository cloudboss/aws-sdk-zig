...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeJobQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.job_queues) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"jobQueues\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeJobQueuesOutput {
    var result: DescribeJobQueuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeJobQueuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
