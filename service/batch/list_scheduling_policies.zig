...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSchedulingPoliciesInput, options: Options) !ListSchedulingPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSchedulingPoliciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSchedulingPoliciesOutput {
    var result: ListSchedulingPoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSchedulingPoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
