...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeJobDefinitionsInput, options: CallOptions) !DescribeJobDefinitionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeJobDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.job_definitions) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"jobDefinitions\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
