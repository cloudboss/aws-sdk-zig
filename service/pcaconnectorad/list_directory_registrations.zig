...
pub const ListDirectoryRegistrationsInput = struct {
...
pub const ListDirectoryRegistrationsOutput = struct {
    /// Summary information about each directory registration you have created.
    directory_registrations: ?[]const DirectoryRegistrationSummary = null,
    ...
    pub const json_field_names = .{
        .directory_registrations = "DirectoryRegistrations",
        .next_token = "NextToken",
    };
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDirectoryRegistrationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDirectoryRegistrationsOutput {
    var result: ListDirectoryRegistrationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDirectoryRegistrationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
