...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"CertificateAuthorityArn\":");
    try aws.json.writeValue(@TypeOf(input.certificate_authority_arn), input.certificate_authority_arn, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VpcInformation\":");
    try aws.json.writeValue(@TypeOf(input.vpc_information), input.vpc_information, alloc, &body_buf);
    has_prev = true;
