...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelConnectionInput, options: Options) !CancelConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.CancelConnection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelConnectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelConnectionOutput, body, alloc);
}
