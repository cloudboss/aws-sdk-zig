...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartVerificationInput, options: Options) !StartVerificationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartVerificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.StartVerification");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartVerificationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StartVerificationOutput, body, alloc);
}
