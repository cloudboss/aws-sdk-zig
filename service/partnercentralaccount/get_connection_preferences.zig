...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConnectionPreferencesInput, options: Options) !GetConnectionPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConnectionPreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.GetConnectionPreferences");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConnectionPreferencesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetConnectionPreferencesOutput, body, alloc);
}
