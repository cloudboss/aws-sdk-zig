...
pub const CancelProfileUpdateTaskInput = struct {
...
pub const CancelProfileUpdateTaskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelProfileUpdateTaskInput, options: CallOptions) !CancelProfileUpdateTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelProfileUpdateTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PartnerCentralAccount.CancelProfileUpdateTask");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelProfileUpdateTaskOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CancelProfileUpdateTaskOutput, body, allocator);
}
