...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelProfileUpdateTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.CancelProfileUpdateTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelProfileUpdateTaskOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CancelProfileUpdateTaskOutput, body, alloc);
}
