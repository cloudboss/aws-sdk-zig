...
pub const GetConnectionInvitationInput = struct {
...
pub const GetConnectionInvitationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConnectionInvitationInput, options: CallOptions) !GetConnectionInvitationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetConnectionInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PartnerCentralAccount.GetConnectionInvitation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetConnectionInvitationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetConnectionInvitationOutput, body, allocator);
}
