...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConnectionInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.GetConnectionInvitation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConnectionInvitationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetConnectionInvitationOutput, body, alloc);
}
