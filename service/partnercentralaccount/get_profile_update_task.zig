...
pub const GetProfileUpdateTaskInput = struct {
...
pub const GetProfileUpdateTaskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProfileUpdateTaskInput, options: CallOptions) !GetProfileUpdateTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetProfileUpdateTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PartnerCentralAccount.GetProfileUpdateTask");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetProfileUpdateTaskOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetProfileUpdateTaskOutput, body, allocator);
}
