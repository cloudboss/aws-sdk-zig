...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProfileUpdateTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.GetProfileUpdateTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProfileUpdateTaskOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetProfileUpdateTaskOutput, body, alloc);
}
