...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVerificationInput, options: Options) !GetVerificationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVerificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.GetVerification");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVerificationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetVerificationOutput, body, alloc);
}
