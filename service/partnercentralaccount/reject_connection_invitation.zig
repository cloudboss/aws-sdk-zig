...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectConnectionInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.RejectConnectionInvitation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectConnectionInvitationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(RejectConnectionInvitationOutput, body, alloc);
}
