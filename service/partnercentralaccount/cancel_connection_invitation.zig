...
pub const CancelConnectionInvitationInput = struct {
...
pub const CancelConnectionInvitationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelConnectionInvitationInput, options: CallOptions) !CancelConnectionInvitationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelConnectionInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PartnerCentralAccount.CancelConnectionInvitation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelConnectionInvitationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CancelConnectionInvitationOutput, body, allocator);
}
