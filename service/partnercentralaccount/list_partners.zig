...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPartnersInput, options: Options) !ListPartnersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPartnersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.ListPartners");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPartnersOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListPartnersOutput, body, alloc);
}
