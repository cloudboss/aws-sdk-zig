...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConnectionPreferencesInput, options: Options) !UpdateConnectionPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConnectionPreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.UpdateConnectionPreferences");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConnectionPreferencesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateConnectionPreferencesOutput, body, alloc);
}
