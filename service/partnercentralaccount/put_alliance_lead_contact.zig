...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAllianceLeadContactInput, options: Options) !PutAllianceLeadContactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAllianceLeadContactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.PutAllianceLeadContact");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAllianceLeadContactOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutAllianceLeadContactOutput, body, alloc);
}
