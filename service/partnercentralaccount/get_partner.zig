...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPartnerInput, options: Options) !GetPartnerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPartnerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralAccount.GetPartner");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPartnerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPartnerOutput, body, alloc);
}
