...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTraceGraphInput, options: CallOptions) !GetTraceGraphOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTraceGraphInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTraceGraphOutput {
    var result: GetTraceGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTraceGraphOutput, body, allocator);
    }
    _ = status;
    _ = headers;
