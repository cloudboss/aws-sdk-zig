...
pub const GetTraceGraphInput = struct {
    /// Pagination token.
...
pub const GetTraceGraphOutput = struct {
    /// Pagination token.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTraceGraphInput, options: Options) !GetTraceGraphOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTraceGraphInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TraceIds\":");
    try aws.json.writeValue(@TypeOf(input.trace_ids), input.trace_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTraceGraphOutput {
    var result: GetTraceGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTraceGraphOutput, body, alloc);
    }
    _ = status;
    _ = headers;
