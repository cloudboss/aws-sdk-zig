...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRetrievedTracesGraphInput, options: Options) !GetRetrievedTracesGraphOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRetrievedTracesGraphInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRetrievedTracesGraphOutput {
    var result: GetRetrievedTracesGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRetrievedTracesGraphOutput, body, alloc);
    }
    _ = status;
    _ = headers;
