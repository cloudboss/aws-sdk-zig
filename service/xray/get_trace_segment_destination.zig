...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTraceSegmentDestinationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTraceSegmentDestinationOutput {
    var result: GetTraceSegmentDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTraceSegmentDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
