...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInsightSummariesInput, options: Options) !GetInsightSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInsightSummariesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInsightSummariesOutput {
    var result: GetInsightSummariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInsightSummariesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
