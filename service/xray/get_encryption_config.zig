...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEncryptionConfigInput, options: Options) !GetEncryptionConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEncryptionConfigInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEncryptionConfigOutput {
    var result: GetEncryptionConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEncryptionConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
