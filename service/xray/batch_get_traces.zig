...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetTracesInput, options: Options) !BatchGetTracesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetTracesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetTracesOutput {
    var result: BatchGetTracesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetTracesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
