...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSamplingTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.sampling_boost_statistics_documents) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"SamplingBoostStatisticsDocuments\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SamplingStatisticsDocuments\":");
    try aws.json.writeValue(@TypeOf(input.sampling_statistics_documents), input.sampling_statistics_documents, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSamplingTargetsOutput {
    var result: GetSamplingTargetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSamplingTargetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
