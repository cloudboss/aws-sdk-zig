...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutEncryptionConfigInput, options: Options) !PutEncryptionConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEncryptionConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEncryptionConfigOutput {
    var result: PutEncryptionConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutEncryptionConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
