...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSamplingRuleInput, options: Options) !DeleteSamplingRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSamplingRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSamplingRuleOutput {
    var result: DeleteSamplingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteSamplingRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
