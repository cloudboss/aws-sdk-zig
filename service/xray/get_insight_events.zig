...
const InsightEvent = @import("insight_event.zig").InsightEvent;

pub const GetInsightEventsInput = struct {
...
pub const GetInsightEventsOutput = struct {
    /// A detailed description of the event. This includes the time of the event,
    /// client and
    /// root cause impact statistics, and the top anomalous service at the time of
    /// the
    /// event.
    insight_events: ?[]const InsightEvent = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInsightEventsInput, options: CallOptions) !GetInsightEventsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetInsightEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetInsightEventsOutput {
    var result: GetInsightEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInsightEventsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
