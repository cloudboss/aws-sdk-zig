...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInsightEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInsightEventsOutput {
    var result: GetInsightEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInsightEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
