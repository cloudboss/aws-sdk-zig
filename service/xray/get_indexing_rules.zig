...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIndexingRulesInput, options: Options) !GetIndexingRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIndexingRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIndexingRulesOutput {
    var result: GetIndexingRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIndexingRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
