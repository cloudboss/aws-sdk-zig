...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutTelemetryRecordsInput, options: Options) !PutTelemetryRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutTelemetryRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ec2_instance_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"EC2InstanceId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.hostname) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Hostname\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TelemetryRecords\":");
    try aws.json.writeValue(@TypeOf(input.telemetry_records), input.telemetry_records, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutTelemetryRecordsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
