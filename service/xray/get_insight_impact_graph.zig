...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInsightImpactGraphInput, options: CallOptions) !GetInsightImpactGraphOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "xray");
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetInsightImpactGraphInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetInsightImpactGraphOutput {
    var result: GetInsightImpactGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInsightImpactGraphOutput, body, allocator);
    }
    _ = status;
    _ = headers;
