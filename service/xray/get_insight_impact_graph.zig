...
const InsightImpactGraphService = @import("insight_impact_graph_service.zig").InsightImpactGraphService;

pub const GetInsightImpactGraphInput = struct {
    /// The estimated end time of the insight, in Unix time seconds. The EndTime is
    /// exclusive of the value provided.
    /// The time range between the start time and end time can't be more than six
    /// hours.
    end_time: i64,
...
pub const GetInsightImpactGraphOutput = struct {
    ...
    service_graph_end_time: ?i64 = null,

    /// The time, in Unix seconds, at which the service graph started.
    service_graph_start_time: ?i64 = null,

    /// The Amazon Web Services instrumented services related to the insight.
    services: ?[]const InsightImpactGraphService = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInsightImpactGraphInput, options: Options) !GetInsightImpactGraphOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInsightImpactGraphInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"InsightId\":");
    try aws.json.writeValue(@TypeOf(input.insight_id), input.insight_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInsightImpactGraphOutput {
    var result: GetInsightImpactGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInsightImpactGraphOutput, body, alloc);
    }
    _ = status;
    _ = headers;
