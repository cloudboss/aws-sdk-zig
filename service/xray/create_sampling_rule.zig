...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSamplingRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"SamplingRule\":");
    try aws.json.writeValue(@TypeOf(input.sampling_rule), input.sampling_rule, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSamplingRuleOutput {
    var result: CreateSamplingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSamplingRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
