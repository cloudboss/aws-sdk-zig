...
pub const CreateSamplingRuleInput = struct {
    /// The rule definition.
...
pub const CreateSamplingRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSamplingRuleInput, options: CallOptions) !CreateSamplingRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSamplingRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"SamplingRule\":");
    try aws.json.writeValue(@TypeOf(input.sampling_rule), input.sampling_rule, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSamplingRuleOutput {
    var result: CreateSamplingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSamplingRuleOutput, body, allocator);
    }
    _ = status;
    _ = headers;
