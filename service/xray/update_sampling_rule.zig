...
const SamplingRuleUpdate = @import("sampling_rule_update.zig").SamplingRuleUpdate;
const SamplingRuleRecord = @import("sampling_rule_record.zig").SamplingRuleRecord;

pub const UpdateSamplingRuleInput = struct {
...
pub const UpdateSamplingRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSamplingRuleInput, options: Options) !UpdateSamplingRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSamplingRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"SamplingRuleUpdate\":");
    try aws.json.writeValue(@TypeOf(input.sampling_rule_update), input.sampling_rule_update, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSamplingRuleOutput {
    var result: UpdateSamplingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSamplingRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
