...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTraceRetrievalInput, options: Options) !StartTraceRetrievalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartTraceRetrievalInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartTraceRetrievalOutput {
    var result: StartTraceRetrievalOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartTraceRetrievalOutput, body, alloc);
    }
    _ = status;
    _ = headers;
