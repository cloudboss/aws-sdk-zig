...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSamplingStatisticSummariesInput, options: Options) !GetSamplingStatisticSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSamplingStatisticSummariesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSamplingStatisticSummariesOutput {
    var result: GetSamplingStatisticSummariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSamplingStatisticSummariesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
