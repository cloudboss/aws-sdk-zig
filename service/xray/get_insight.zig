...
pub const GetInsightInput = struct {
...
pub const GetInsightOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInsightInput, options: Options) !GetInsightOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInsightInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInsightOutput {
    var result: GetInsightOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInsightOutput, body, alloc);
    }
    _ = status;
    _ = headers;
