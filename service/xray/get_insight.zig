...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInsightInput, options: CallOptions) !GetInsightOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetInsightInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetInsightOutput {
    var result: GetInsightOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInsightOutput, body, allocator);
    }
    _ = status;
    _ = headers;
