...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutTraceSegmentsInput, options: CallOptions) !PutTraceSegmentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutTraceSegmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"TraceSegmentDocuments\":");
    try aws.json.writeValue(@TypeOf(input.trace_segment_documents), input.trace_segment_documents, allocator, &body_buf);
    has_prev = true;
