...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRetrievedTracesInput, options: Options) !ListRetrievedTracesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRetrievedTracesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.trace_format) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TraceFormat\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRetrievedTracesOutput {
    var result: ListRetrievedTracesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRetrievedTracesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
