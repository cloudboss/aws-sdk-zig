...
pub const CancelTraceRetrievalInput = struct {
    /// Retrieval token.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelTraceRetrievalInput, options: Options) !CancelTraceRetrievalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelTraceRetrievalInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/CancelTraceRetrieval";
    ...
    try body_buf.appendSlice(alloc, "\"RetrievalToken\":");
    try aws.json.writeValue(@TypeOf(input.retrieval_token), input.retrieval_token, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelTraceRetrievalOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CancelTraceRetrievalOutput = .{};
