...
pub const UpdateTraceSegmentDestinationInput = struct {
    /// The configured destination of trace segments.
    destination: ?TraceSegmentDestination = null,
...
pub const UpdateTraceSegmentDestinationOutput = struct {
    /// The destination of the trace segments.
    destination: ?TraceSegmentDestination = null,

    /// The status of the update.
    status: ?TraceSegmentDestinationStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTraceSegmentDestinationInput, options: Options) !UpdateTraceSegmentDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTraceSegmentDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/UpdateTraceSegmentDestination";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTraceSegmentDestinationOutput {
    var result: UpdateTraceSegmentDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTraceSegmentDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
