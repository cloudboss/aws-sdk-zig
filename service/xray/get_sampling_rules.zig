...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSamplingRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSamplingRulesOutput {
    var result: GetSamplingRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSamplingRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
