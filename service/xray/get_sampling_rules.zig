...
pub const GetSamplingRulesInput = struct {
    /// Pagination token.
...
pub const GetSamplingRulesOutput = struct {
    /// Pagination token.
    next_token: ?[]const u8 = null,

    /// Rule definitions and metadata.
    sampling_rule_records: ?[]const SamplingRuleRecord = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSamplingRulesInput, options: CallOptions) !GetSamplingRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSamplingRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSamplingRulesOutput {
    var result: GetSamplingRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSamplingRulesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
