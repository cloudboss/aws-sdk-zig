...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGroupsOutput {
    var result: GetGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
