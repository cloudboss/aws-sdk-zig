...
pub const GetGroupsInput = struct {
    /// Pagination token.
...
pub const GetGroupsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGroupsInput, options: CallOptions) !GetGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetGroupsOutput {
    var result: GetGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGroupsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
