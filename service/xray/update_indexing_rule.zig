...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIndexingRuleInput, options: Options) !UpdateIndexingRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIndexingRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIndexingRuleOutput {
    var result: UpdateIndexingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIndexingRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
