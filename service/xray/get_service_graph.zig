...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceGraphInput, options: Options) !GetServiceGraphOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceGraphInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceGraphOutput {
    var result: GetServiceGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetServiceGraphOutput, body, alloc);
    }
    _ = status;
    _ = headers;
