...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVolumeInput, options: Options) !CreateVolumeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVolumeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EUCMIFrontendAPIService.CreateVolume");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVolumeOutput {
    ...
    return aws.json.parseJsonObject(CreateVolumeOutput, body, alloc);
}
