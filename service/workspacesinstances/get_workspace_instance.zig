...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkspaceInstanceInput, options: CallOptions) !GetWorkspaceInstanceOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "workspaces-instances");
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetWorkspaceInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "EUCMIFrontendAPIService.GetWorkspaceInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetWorkspaceInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetWorkspaceInstanceOutput, body, allocator);
}
