...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInstanceTypesInput, options: Options) !ListInstanceTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstanceTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EUCMIFrontendAPIService.ListInstanceTypes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstanceTypesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInstanceTypesOutput, body, alloc);
}
