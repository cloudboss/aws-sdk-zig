...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkspaceInstanceInput, options: Options) !CreateWorkspaceInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkspaceInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EUCMIFrontendAPIService.CreateWorkspaceInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkspaceInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateWorkspaceInstanceOutput, body, alloc);
}
