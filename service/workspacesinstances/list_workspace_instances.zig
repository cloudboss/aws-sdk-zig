...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkspaceInstancesInput, options: Options) !ListWorkspaceInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkspaceInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EUCMIFrontendAPIService.ListWorkspaceInstances");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkspaceInstancesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListWorkspaceInstancesOutput, body, alloc);
}
