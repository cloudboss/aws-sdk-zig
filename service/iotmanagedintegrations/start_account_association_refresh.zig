...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAccountAssociationRefreshInput, options: Options) !StartAccountAssociationRefreshOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartAccountAssociationRefreshInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/account-associations/");
    try path_buf.appendSlice(alloc, input.account_association_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartAccountAssociationRefreshOutput {
    var result: StartAccountAssociationRefreshOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartAccountAssociationRefreshOutput, body, alloc);
    }
    _ = status;
    _ = headers;
