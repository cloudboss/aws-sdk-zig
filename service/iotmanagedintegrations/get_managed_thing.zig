...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetManagedThingInput, options: Options) !GetManagedThingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetManagedThingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetManagedThingOutput {
    var result: GetManagedThingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetManagedThingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
