...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetHubConfigurationInput, options: Options) !GetHubConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetHubConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetHubConfigurationOutput {
    var result: GetHubConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetHubConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
