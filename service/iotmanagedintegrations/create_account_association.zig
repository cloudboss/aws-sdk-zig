...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccountAssociationInput, options: Options) !CreateAccountAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccountAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ConnectorDestinationId\":");
    try aws.json.writeValue(@TypeOf(input.connector_destination_id), input.connector_destination_id, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.general_authorization) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"GeneralAuthorization\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccountAssociationOutput {
    var result: CreateAccountAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAccountAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
