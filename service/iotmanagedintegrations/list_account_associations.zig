...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountAssociationsInput, options: Options) !ListAccountAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connector_destination_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "ConnectorDestinationId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountAssociationsOutput {
    var result: ListAccountAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccountAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
