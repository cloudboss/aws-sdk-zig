...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeviceDiscoveryInput, options: Options) !GetDeviceDiscoveryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeviceDiscoveryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeviceDiscoveryOutput {
    var result: GetDeviceDiscoveryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDeviceDiscoveryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
