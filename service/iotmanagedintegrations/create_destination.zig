...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.delivery_destination_arn), input.delivery_destination_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DeliveryDestinationType\":");
    try aws.json.writeValue(@TypeOf(input.delivery_destination_type), input.delivery_destination_type, alloc, &body_buf);
    has_prev = true;
