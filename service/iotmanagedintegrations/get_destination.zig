...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDestinationInput, options: Options) !GetDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDestinationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDestinationOutput {
    var result: GetDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
