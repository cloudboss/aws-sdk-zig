...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountAssociationInput, options: Options) !GetAccountAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccountAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccountAssociationOutput {
    var result: GetAccountAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAccountAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
