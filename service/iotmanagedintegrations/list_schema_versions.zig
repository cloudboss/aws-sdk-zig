...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSchemaVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.namespace) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "NamespaceFilter=");
    ...
    if (input.schema_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "SchemaIdFilter=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
    if (input.semantic_version) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "SemanticVersionFilter=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
    if (input.visibility) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "VisibilityFilter=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSchemaVersionsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSchemaVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
