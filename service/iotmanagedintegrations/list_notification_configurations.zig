...
const NotificationConfigurationSummary = @import("notification_configuration_summary.zig").NotificationConfigurationSummary;
...
pub const ListNotificationConfigurationsOutput = struct {
    ...
    notification_configuration_list: ?[]const NotificationConfigurationSummary = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .notification_configuration_list = "NotificationConfigurationList",
    };
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListNotificationConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListNotificationConfigurationsOutput {
    var result: ListNotificationConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListNotificationConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
