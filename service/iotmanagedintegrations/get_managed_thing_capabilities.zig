...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetManagedThingCapabilitiesInput, options: Options) !GetManagedThingCapabilitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetManagedThingCapabilitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/managed-things-capabilities/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetManagedThingCapabilitiesOutput {
    var result: GetManagedThingCapabilitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetManagedThingCapabilitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
