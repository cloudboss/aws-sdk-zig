...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOtaTaskConfigurationInput, options: Options) !CreateOtaTaskConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOtaTaskConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.push_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PushConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOtaTaskConfigurationOutput {
    var result: CreateOtaTaskConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateOtaTaskConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
