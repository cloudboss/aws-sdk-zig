...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCloudConnectorInput, options: Options) !GetCloudConnectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCloudConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/cloud-connectors/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCloudConnectorOutput {
    var result: GetCloudConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCloudConnectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
