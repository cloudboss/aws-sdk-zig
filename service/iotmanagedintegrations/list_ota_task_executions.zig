...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOtaTaskExecutionsInput, options: Options) !ListOtaTaskExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOtaTaskExecutionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOtaTaskExecutionsOutput {
    var result: ListOtaTaskExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOtaTaskExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
