...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCredentialLockerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCredentialLockerOutput {
    var result: CreateCredentialLockerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCredentialLockerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
