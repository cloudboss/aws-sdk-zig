...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCredentialLockerInput, options: Options) !GetCredentialLockerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCredentialLockerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCredentialLockerOutput {
    var result: GetCredentialLockerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCredentialLockerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
