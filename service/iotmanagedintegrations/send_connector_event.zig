...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendConnectorEventInput, options: Options) !SendConnectorEventOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendConnectorEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/connector-event/");
    ...
    if (input.connector_device_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ConnectorDeviceId\":");
    ...
    if (input.matter_endpoint) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MatterEndpoint\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.message) |v| {
    ...
    if (input.operation_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OperationVersion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.trace_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TraceId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.user_id) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendConnectorEventOutput {
    var result: SendConnectorEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendConnectorEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
