...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateNotificationConfigurationInput, options: Options) !CreateNotificationConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNotificationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DestinationName\":");
    try aws.json.writeValue(@TypeOf(input.destination_name), input.destination_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateNotificationConfigurationOutput {
    var result: CreateNotificationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateNotificationConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
