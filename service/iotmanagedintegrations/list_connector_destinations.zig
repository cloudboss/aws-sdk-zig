...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConnectorDestinationsInput, options: Options) !ListConnectorDestinationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConnectorDestinationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cloud_connector_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "CloudConnectorId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConnectorDestinationsOutput {
    var result: ListConnectorDestinationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConnectorDestinationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
