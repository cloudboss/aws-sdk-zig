...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProvisioningProfileInput, options: Options) !GetProvisioningProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProvisioningProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/provisioning-profiles/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProvisioningProfileOutput {
    var result: GetProvisioningProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProvisioningProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
