...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCloudConnectorsInput, options: Options) !ListCloudConnectorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCloudConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.lambda_arn) |v| {
        ...
        try query_buf.appendSlice(alloc, "LambdaArn=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCloudConnectorsOutput {
    var result: ListCloudConnectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCloudConnectorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
