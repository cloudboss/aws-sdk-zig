...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConnectorDestinationInput, options: Options) !CreateConnectorDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConnectorDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AuthConfig\":");
    try aws.json.writeValue(@TypeOf(input.auth_config), input.auth_config, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"CloudConnectorId\":");
    try aws.json.writeValue(@TypeOf(input.cloud_connector_id), input.cloud_connector_id, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.secrets_manager) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SecretsManager\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConnectorDestinationOutput {
    var result: CreateConnectorDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConnectorDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
