...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedThingAccountAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.account_association_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "AccountAssociationId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.managed_thing_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ManagedThingId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedThingAccountAssociationsOutput {
    var result: ListManagedThingAccountAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedThingAccountAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
