...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOtaTaskInput, options: Options) !CreateOtaTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOtaTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ota_mechanism) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OtaMechanism\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.ota_scheduling_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OtaSchedulingConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.ota_target_query_string) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OtaTargetQueryString\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.ota_task_execution_retry_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OtaTaskExecutionRetryConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OtaType\":");
    try aws.json.writeValue(@TypeOf(input.ota_type), input.ota_type, alloc, &body_buf);
    has_prev = true;
    if (input.protocol) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"S3Url\":");
    try aws.json.writeValue(@TypeOf(input.s3_url), input.s3_url, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.task_configuration_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TaskConfigurationId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOtaTaskOutput {
    var result: CreateOtaTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateOtaTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
