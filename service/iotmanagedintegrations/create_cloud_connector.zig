...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCloudConnectorInput, options: Options) !CreateCloudConnectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCloudConnectorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCloudConnectorOutput {
    var result: CreateCloudConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCloudConnectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
