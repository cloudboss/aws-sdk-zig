...
const UpdateDestinationOutput = struct {};
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.delivery_destination_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DeliveryDestinationArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.delivery_destination_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DeliveryDestinationType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDestinationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
