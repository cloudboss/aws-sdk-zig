...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeviceDiscoveriesInput, options: Options) !ListDeviceDiscoveriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDeviceDiscoveriesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.type_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "TypeFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDeviceDiscoveriesOutput {
    var result: ListDeviceDiscoveriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDeviceDiscoveriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
