...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOtaTaskConfigurationInput, options: Options) !GetOtaTaskConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOtaTaskConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOtaTaskConfigurationOutput {
    var result: GetOtaTaskConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOtaTaskConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
