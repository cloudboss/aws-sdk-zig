...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateManagedThingInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.authentication_material), input.authentication_material, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"AuthenticationMaterialType\":");
    try aws.json.writeValue(@TypeOf(input.authentication_material_type), input.authentication_material_type, alloc, &body_buf);
    has_prev = true;
    if (input.brand) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateManagedThingOutput {
    var result: CreateManagedThingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateManagedThingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
