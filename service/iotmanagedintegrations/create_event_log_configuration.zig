...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEventLogConfigurationInput, options: Options) !CreateEventLogConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEventLogConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EventLogLevel\":");
    try aws.json.writeValue(@TypeOf(input.event_log_level), input.event_log_level, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEventLogConfigurationOutput {
    var result: CreateEventLogConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEventLogConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
