...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterAccountAssociationInput, options: Options) !RegisterAccountAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterAccountAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.account_association_id), input.account_association_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DeviceDiscoveryId\":");
    try aws.json.writeValue(@TypeOf(input.device_discovery_id), input.device_discovery_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ManagedThingId\":");
    try aws.json.writeValue(@TypeOf(input.managed_thing_id), input.managed_thing_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterAccountAssociationOutput {
    var result: RegisterAccountAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterAccountAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
