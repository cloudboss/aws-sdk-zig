...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSchemaVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSchemaVersionOutput {
    var result: GetSchemaVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSchemaVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
