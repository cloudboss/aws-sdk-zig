...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOtaTaskInput, options: Options) !GetOtaTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOtaTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOtaTaskOutput {
    var result: GetOtaTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOtaTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
