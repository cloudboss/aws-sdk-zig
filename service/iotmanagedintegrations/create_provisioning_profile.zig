...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProvisioningProfileInput, options: Options) !CreateProvisioningProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProvisioningProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.claim_certificate) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ClaimCertificate\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    try aws.json.writeValue(@TypeOf(input.provisioning_type), input.provisioning_type, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProvisioningProfileOutput {
    var result: CreateProvisioningProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProvisioningProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
