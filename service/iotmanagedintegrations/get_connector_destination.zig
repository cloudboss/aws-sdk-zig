...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConnectorDestinationInput, options: Options) !GetConnectorDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConnectorDestinationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConnectorDestinationOutput {
    var result: GetConnectorDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConnectorDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
