...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBrowserProfileInput, options: Options) !DeleteBrowserProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBrowserProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBrowserProfileOutput {
    var result: DeleteBrowserProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBrowserProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
