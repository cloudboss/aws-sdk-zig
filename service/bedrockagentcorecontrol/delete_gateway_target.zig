...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGatewayTargetInput, options: Options) !DeleteGatewayTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGatewayTargetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGatewayTargetOutput {
    var result: DeleteGatewayTargetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteGatewayTargetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
