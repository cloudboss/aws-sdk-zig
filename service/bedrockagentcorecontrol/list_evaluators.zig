...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEvaluatorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEvaluatorsOutput {
    var result: ListEvaluatorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEvaluatorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
