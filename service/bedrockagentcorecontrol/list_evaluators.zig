...
pub const ListEvaluatorsInput = struct {
...
pub const ListEvaluatorsOutput = struct {
    /// The list of evaluator summaries containing basic information about each
    /// evaluator.
    evaluators: ?[]const EvaluatorSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEvaluatorsInput, options: CallOptions) !ListEvaluatorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListEvaluatorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListEvaluatorsOutput {
    var result: ListEvaluatorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEvaluatorsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
