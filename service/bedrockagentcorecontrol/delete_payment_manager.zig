...
pub const DeletePaymentManagerInput = struct {
...
pub const DeletePaymentManagerOutput = struct {
    /// The unique identifier of the deleted payment manager.
    payment_manager_id: ?[]const u8 = null,

    /// The current status of the payment manager, set to `DELETING` when deletion
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePaymentManagerInput, options: CallOptions) !DeletePaymentManagerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePaymentManagerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePaymentManagerOutput {
    var result: DeletePaymentManagerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePaymentManagerOutput, body, allocator);
    }
    _ = status;
    _ = headers;
