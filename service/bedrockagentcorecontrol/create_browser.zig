...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBrowserInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.browser_signing) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"browserSigning\":");
    ...
    if (input.recording) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"recording\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBrowserOutput {
    var result: CreateBrowserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBrowserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
