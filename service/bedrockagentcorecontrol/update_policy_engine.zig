...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePolicyEngineInput, options: Options) !UpdatePolicyEngineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePolicyEngineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePolicyEngineOutput {
    var result: UpdatePolicyEngineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePolicyEngineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
