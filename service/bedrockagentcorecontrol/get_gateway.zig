...
const GetGatewayInput = @import("get_gateway_request.zig").GetGatewayRequest;

const GetGatewayOutput = @import("get_gateway_response.zig").GetGatewayResponse;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGatewayOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
