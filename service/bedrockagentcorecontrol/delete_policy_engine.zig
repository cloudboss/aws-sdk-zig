...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePolicyEngineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePolicyEngineOutput {
    var result: DeletePolicyEngineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePolicyEngineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
