...
const PaymentConnectorSummary = @import("payment_connector_summary.zig").PaymentConnectorSummary;

pub const ListPaymentConnectorsInput = struct {
...
pub const ListPaymentConnectorsOutput = struct {
    /// If the total number of results is greater than the `maxResults` value
    ...
    /// each summary, see the `PaymentConnectorSummary` data type.
    payment_connectors: ?[]const PaymentConnectorSummary = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .payment_connectors = "paymentConnectors",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPaymentConnectorsInput, options: CallOptions) !ListPaymentConnectorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPaymentConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/connectors-list");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPaymentConnectorsOutput {
    var result: ListPaymentConnectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPaymentConnectorsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
