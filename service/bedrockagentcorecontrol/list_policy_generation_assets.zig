...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPolicyGenerationAssetsInput, options: Options) !ListPolicyGenerationAssetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPolicyGenerationAssetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPolicyGenerationAssetsOutput {
    var result: ListPolicyGenerationAssetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPolicyGenerationAssetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
