...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAgentRuntimeEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAgentRuntimeEndpointOutput {
    var result: CreateAgentRuntimeEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAgentRuntimeEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
