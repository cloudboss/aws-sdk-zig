...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPolicyGenerationsInput, options: Options) !ListPolicyGenerationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPolicyGenerationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/policy-generations");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPolicyGenerationsOutput {
    var result: ListPolicyGenerationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPolicyGenerationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
