...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBrowserProfileInput, options: Options) !CreateBrowserProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBrowserProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBrowserProfileOutput {
    var result: CreateBrowserProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBrowserProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
