...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEvaluatorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEvaluatorOutput {
    var result: GetEvaluatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEvaluatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
