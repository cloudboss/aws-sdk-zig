...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOnlineEvaluationConfigInput, options: Options) !DeleteOnlineEvaluationConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteOnlineEvaluationConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/online-evaluation-configs/");
    try path_buf.appendSlice(alloc, input.online_evaluation_config_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteOnlineEvaluationConfigOutput {
    var result: DeleteOnlineEvaluationConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteOnlineEvaluationConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
