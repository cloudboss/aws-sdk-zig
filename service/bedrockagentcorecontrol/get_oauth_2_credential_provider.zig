...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOauth2CredentialProviderInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOauth2CredentialProviderOutput {
    var result: GetOauth2CredentialProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOauth2CredentialProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
