...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAgentRuntimeVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAgentRuntimeVersionsOutput {
    var result: ListAgentRuntimeVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentRuntimeVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
