...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCodeInterpreterInput, options: Options) !GetCodeInterpreterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCodeInterpreterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCodeInterpreterOutput {
    var result: GetCodeInterpreterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCodeInterpreterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
