...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPolicyGenerationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.policy_generation_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPolicyGenerationOutput {
    var result: GetPolicyGenerationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPolicyGenerationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
