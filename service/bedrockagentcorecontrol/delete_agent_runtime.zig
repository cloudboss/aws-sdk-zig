...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAgentRuntimeInput, options: Options) !DeleteAgentRuntimeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAgentRuntimeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAgentRuntimeOutput {
    var result: DeleteAgentRuntimeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAgentRuntimeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
