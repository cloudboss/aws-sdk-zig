...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEvaluatorInput, options: Options) !DeleteEvaluatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteEvaluatorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/evaluators/");
    try path_buf.appendSlice(alloc, input.evaluator_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteEvaluatorOutput {
    var result: DeleteEvaluatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteEvaluatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
