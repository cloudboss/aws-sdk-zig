...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTokenVaultInput, options: Options) !GetTokenVaultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTokenVaultInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.token_vault_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"tokenVaultId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTokenVaultOutput {
    var result: GetTokenVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTokenVaultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
