...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOnlineEvaluationConfigInput, options: CallOptions) !CreateOnlineEvaluationConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateOnlineEvaluationConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.enable_on_create), input.enable_on_create, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
