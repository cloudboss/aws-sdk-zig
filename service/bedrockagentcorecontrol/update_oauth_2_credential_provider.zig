...
pub const UpdateOauth2CredentialProviderInput = struct {
...
pub const UpdateOauth2CredentialProviderOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOauth2CredentialProviderInput, options: Options) !UpdateOauth2CredentialProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateOauth2CredentialProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/identities/UpdateOauth2CredentialProvider";
    ...
    try body_buf.appendSlice(alloc, "\"credentialProviderVendor\":");
    try aws.json.writeValue(@TypeOf(input.credential_provider_vendor), input.credential_provider_vendor, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"oauth2ProviderConfigInput\":");
    try aws.json.writeValue(@TypeOf(input.oauth_2_provider_config_input), input.oauth_2_provider_config_input, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateOauth2CredentialProviderOutput {
    var result: UpdateOauth2CredentialProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateOauth2CredentialProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
