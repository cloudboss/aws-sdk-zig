...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOauth2CredentialProviderInput, options: CallOptions) !UpdateOauth2CredentialProviderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateOauth2CredentialProviderInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateOauth2CredentialProviderOutput {
    var result: UpdateOauth2CredentialProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateOauth2CredentialProviderOutput, body, allocator);
    }
    _ = status;
    _ = headers;
