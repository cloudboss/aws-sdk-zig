...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMemoryInput, options: Options) !CreateMemoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMemoryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"eventExpiryDuration\":");
    try aws.json.writeValue(@TypeOf(input.event_expiry_duration), input.event_expiry_duration, alloc, &body_buf);
    has_prev = true;
    if (input.memory_execution_role_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"memoryExecutionRoleArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.memory_strategies) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"memoryStrategies\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMemoryOutput {
    var result: CreateMemoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMemoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
