...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetTokenVaultCMKInput, options: Options) !SetTokenVaultCMKOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetTokenVaultCMKInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"kmsConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.kms_configuration), input.kms_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetTokenVaultCMKOutput {
    var result: SetTokenVaultCMKOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SetTokenVaultCMKOutput, body, alloc);
    }
    _ = status;
    _ = headers;
