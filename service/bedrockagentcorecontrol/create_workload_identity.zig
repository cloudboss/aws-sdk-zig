...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkloadIdentityInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkloadIdentityOutput {
    var result: CreateWorkloadIdentityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWorkloadIdentityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
