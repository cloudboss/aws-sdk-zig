...
pub const CreateWorkloadIdentityInput = struct {
...
pub const CreateWorkloadIdentityOutput = struct {
    /// The list of allowed OAuth2 return URLs for resources associated with this
    /// workload identity.
    allowed_resource_oauth_2_return_urls: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkloadIdentityInput, options: CallOptions) !CreateWorkloadIdentityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateWorkloadIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/identities/CreateWorkloadIdentity";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateWorkloadIdentityOutput {
    var result: CreateWorkloadIdentityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWorkloadIdentityOutput, body, allocator);
    }
    _ = status;
    _ = headers;
