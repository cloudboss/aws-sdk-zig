...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAgentRuntimeEndpointInput, options: Options) !GetAgentRuntimeEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAgentRuntimeEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAgentRuntimeEndpointOutput {
    var result: GetAgentRuntimeEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAgentRuntimeEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
