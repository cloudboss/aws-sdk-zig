...
pub const UpdateRegistryRecordStatusInput = struct {
    /// The identifier of the registry record to update the status for. You can
...
pub const UpdateRegistryRecordStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRegistryRecordStatusInput, options: CallOptions) !UpdateRegistryRecordStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRegistryRecordStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"statusReason\":");
    try aws.json.writeValue(@TypeOf(input.status_reason), input.status_reason, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRegistryRecordStatusOutput {
    var result: UpdateRegistryRecordStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRegistryRecordStatusOutput, body, allocator);
    }
    _ = status;
    _ = headers;
