...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCodeInterpreterInput, options: Options) !CreateCodeInterpreterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCodeInterpreterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCodeInterpreterOutput {
    var result: CreateCodeInterpreterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCodeInterpreterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
