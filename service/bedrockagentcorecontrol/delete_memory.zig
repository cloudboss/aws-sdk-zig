...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMemoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMemoryOutput {
    var result: DeleteMemoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteMemoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
