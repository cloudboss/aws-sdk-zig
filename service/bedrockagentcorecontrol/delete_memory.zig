...
pub const DeleteMemoryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMemoryInput, options: CallOptions) !DeleteMemoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMemoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMemoryOutput {
    var result: DeleteMemoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteMemoryOutput, body, allocator);
    }
    _ = status;
    _ = headers;
