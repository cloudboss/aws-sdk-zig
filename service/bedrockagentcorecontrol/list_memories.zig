...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMemoriesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMemoriesOutput {
    var result: ListMemoriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMemoriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
