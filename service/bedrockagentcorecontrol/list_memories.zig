...
const MemorySummary = @import("memory_summary.zig").MemorySummary;
...
pub const ListMemoriesOutput = struct {
    /// The list of AgentCore Memory resource summaries.
    memories: ?[]const MemorySummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMemoriesInput, options: CallOptions) !ListMemoriesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMemoriesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMemoriesOutput {
    var result: ListMemoriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMemoriesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
