...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAgentRuntimeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAgentRuntimeOutput {
    var result: GetAgentRuntimeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAgentRuntimeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
