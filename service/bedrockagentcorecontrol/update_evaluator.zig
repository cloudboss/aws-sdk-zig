...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEvaluatorInput, options: Options) !UpdateEvaluatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEvaluatorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEvaluatorOutput {
    var result: UpdateEvaluatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEvaluatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
