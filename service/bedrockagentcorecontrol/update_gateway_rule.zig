...
pub const UpdateGatewayRuleInput = struct {
...
pub const UpdateGatewayRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGatewayRuleInput, options: CallOptions) !UpdateGatewayRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateGatewayRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateGatewayRuleOutput {
    var result: UpdateGatewayRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGatewayRuleOutput, body, allocator);
    }
    _ = status;
    _ = headers;
