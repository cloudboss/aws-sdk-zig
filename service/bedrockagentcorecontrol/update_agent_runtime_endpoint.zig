...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAgentRuntimeEndpointInput, options: Options) !UpdateAgentRuntimeEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAgentRuntimeEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.agent_runtime_version) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"agentRuntimeVersion\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAgentRuntimeEndpointOutput {
    var result: UpdateAgentRuntimeEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAgentRuntimeEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
