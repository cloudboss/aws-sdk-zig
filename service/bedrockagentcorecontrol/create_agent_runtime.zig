...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAgentRuntimeInput, options: Options) !CreateAgentRuntimeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAgentRuntimeInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"agentRuntimeName\":");
    try aws.json.writeValue(@TypeOf(input.agent_runtime_name), input.agent_runtime_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAgentRuntimeOutput {
    var result: CreateAgentRuntimeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAgentRuntimeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
