...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBrowserInput, options: CallOptions) !DeleteBrowserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBrowserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBrowserOutput {
    var result: DeleteBrowserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBrowserOutput, body, allocator);
    }
    _ = status;
    _ = headers;
