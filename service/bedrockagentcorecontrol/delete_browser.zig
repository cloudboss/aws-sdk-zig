...
pub const DeleteBrowserInput = struct {
...
pub const DeleteBrowserOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBrowserInput, options: Options) !DeleteBrowserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBrowserInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.browser_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBrowserOutput {
    var result: DeleteBrowserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBrowserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
