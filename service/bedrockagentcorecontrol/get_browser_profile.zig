...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBrowserProfileInput, options: Options) !GetBrowserProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBrowserProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBrowserProfileOutput {
    var result: GetBrowserProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBrowserProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
