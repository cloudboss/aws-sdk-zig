...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOnlineEvaluationConfigInput, options: Options) !GetOnlineEvaluationConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOnlineEvaluationConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOnlineEvaluationConfigOutput {
    var result: GetOnlineEvaluationConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOnlineEvaluationConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
