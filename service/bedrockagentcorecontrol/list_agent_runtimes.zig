...
pub const ListAgentRuntimesInput = struct {
...
pub const ListAgentRuntimesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAgentRuntimesInput, options: Options) !ListAgentRuntimesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAgentRuntimesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAgentRuntimesOutput {
    var result: ListAgentRuntimesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentRuntimesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
