...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMemoryInput, options: Options) !GetMemoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMemoryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.view) |v| {
        ...
        try query_buf.appendSlice(alloc, "view=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMemoryOutput {
    var result: GetMemoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMemoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
