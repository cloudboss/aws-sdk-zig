...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMemoryInput, options: Options) !UpdateMemoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMemoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMemoryOutput {
    var result: UpdateMemoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMemoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
