...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOauth2CredentialProviderInput, options: Options) !CreateOauth2CredentialProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOauth2CredentialProviderInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOauth2CredentialProviderOutput {
    var result: CreateOauth2CredentialProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateOauth2CredentialProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
