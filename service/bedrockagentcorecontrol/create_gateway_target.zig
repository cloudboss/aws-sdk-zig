...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGatewayTargetInput, options: Options) !CreateGatewayTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGatewayTargetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGatewayTargetOutput {
    var result: CreateGatewayTargetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGatewayTargetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
