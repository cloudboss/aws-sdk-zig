...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateApiKeyCredentialProviderInput, options: Options) !CreateApiKeyCredentialProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApiKeyCredentialProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"apiKey\":");
    try aws.json.writeValue(@TypeOf(input.api_key), input.api_key, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateApiKeyCredentialProviderOutput {
    var result: CreateApiKeyCredentialProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateApiKeyCredentialProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
