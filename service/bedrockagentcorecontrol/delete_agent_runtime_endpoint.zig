...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAgentRuntimeEndpointInput, options: Options) !DeleteAgentRuntimeEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAgentRuntimeEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAgentRuntimeEndpointOutput {
    var result: DeleteAgentRuntimeEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAgentRuntimeEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
