...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPolicyEngineInput, options: Options) !GetPolicyEngineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPolicyEngineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPolicyEngineOutput {
    var result: GetPolicyEngineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPolicyEngineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
