...
pub const ListGatewayTargetsInput = struct {
...
pub const ListGatewayTargetsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGatewayTargetsInput, options: CallOptions) !ListGatewayTargetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListGatewayTargetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListGatewayTargetsOutput {
    var result: ListGatewayTargetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGatewayTargetsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
