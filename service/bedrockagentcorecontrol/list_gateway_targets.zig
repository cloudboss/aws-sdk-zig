...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGatewayTargetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGatewayTargetsOutput {
    var result: ListGatewayTargetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGatewayTargetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
