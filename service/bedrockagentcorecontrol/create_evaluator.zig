...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEvaluatorInput, options: Options) !CreateEvaluatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEvaluatorInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"evaluatorConfig\":");
    try aws.json.writeValue(@TypeOf(input.evaluator_config), input.evaluator_config, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"evaluatorName\":");
    try aws.json.writeValue(@TypeOf(input.evaluator_name), input.evaluator_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"level\":");
    try aws.json.writeValue(@TypeOf(input.level), input.level, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEvaluatorOutput {
    var result: CreateEvaluatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEvaluatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
