...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGatewayTargetInput, options: Options) !GetGatewayTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGatewayTargetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGatewayTargetOutput {
    var result: GetGatewayTargetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGatewayTargetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
