...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApiKeyCredentialProviderInput, options: Options) !GetApiKeyCredentialProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApiKeyCredentialProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/identities/GetApiKeyCredentialProvider";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApiKeyCredentialProviderOutput {
    var result: GetApiKeyCredentialProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetApiKeyCredentialProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
