...
pub const ListApiKeyCredentialProvidersInput = struct {
...
pub const ListApiKeyCredentialProvidersOutput = struct {
    /// The list of API key credential providers.
    credential_providers: ?[]const ApiKeyCredentialProviderItem = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListApiKeyCredentialProvidersInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/identities/ListApiKeyCredentialProviders";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListApiKeyCredentialProvidersOutput {
    var result: ListApiKeyCredentialProvidersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListApiKeyCredentialProvidersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
