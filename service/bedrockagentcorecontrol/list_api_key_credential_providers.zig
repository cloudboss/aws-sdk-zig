...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApiKeyCredentialProvidersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApiKeyCredentialProvidersOutput {
    var result: ListApiKeyCredentialProvidersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListApiKeyCredentialProvidersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
