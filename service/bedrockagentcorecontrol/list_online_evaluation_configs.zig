...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOnlineEvaluationConfigsInput, options: Options) !ListOnlineEvaluationConfigsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOnlineEvaluationConfigsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOnlineEvaluationConfigsOutput {
    var result: ListOnlineEvaluationConfigsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOnlineEvaluationConfigsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
