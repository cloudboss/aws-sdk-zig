...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePolicyEngineInput, options: Options) !CreatePolicyEngineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePolicyEngineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePolicyEngineOutput {
    var result: CreatePolicyEngineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePolicyEngineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
