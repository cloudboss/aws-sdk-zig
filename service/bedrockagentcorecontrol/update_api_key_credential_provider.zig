...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateApiKeyCredentialProviderInput, options: Options) !UpdateApiKeyCredentialProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateApiKeyCredentialProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/identities/UpdateApiKeyCredentialProvider";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateApiKeyCredentialProviderOutput {
    var result: UpdateApiKeyCredentialProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateApiKeyCredentialProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
