...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBrowsersInput, options: Options) !ListBrowsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBrowsersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBrowsersOutput {
    var result: ListBrowsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBrowsersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
