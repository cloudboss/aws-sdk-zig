...
pub const UpdateOnlineEvaluationConfigInput = struct {
...
pub const UpdateOnlineEvaluationConfigOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOnlineEvaluationConfigInput, options: Options) !UpdateOnlineEvaluationConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateOnlineEvaluationConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.evaluation_execution_role_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.execution_status) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"executionStatus\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.rule) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateOnlineEvaluationConfigOutput {
    var result: UpdateOnlineEvaluationConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateOnlineEvaluationConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
