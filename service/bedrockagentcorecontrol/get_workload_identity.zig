...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkloadIdentityInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkloadIdentityOutput {
    var result: GetWorkloadIdentityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkloadIdentityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
