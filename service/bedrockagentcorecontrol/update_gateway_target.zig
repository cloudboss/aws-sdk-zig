...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGatewayTargetInput, options: Options) !UpdateGatewayTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGatewayTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.target_id);
    ...
    if (input.credential_provider_configurations) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"credentialProviderConfigurations\":");
    ...
    if (input.metadata_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"metadataConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"targetConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.target_configuration), input.target_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGatewayTargetOutput {
    var result: UpdateGatewayTargetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGatewayTargetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
