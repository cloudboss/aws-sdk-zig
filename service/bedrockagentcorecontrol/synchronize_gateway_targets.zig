...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SynchronizeGatewayTargetsInput, options: Options) !SynchronizeGatewayTargetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SynchronizeGatewayTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/synchronizeTargets");
    ...
    try body_buf.appendSlice(alloc, "\"targetIdList\":");
    try aws.json.writeValue(@TypeOf(input.target_id_list), input.target_id_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SynchronizeGatewayTargetsOutput {
    var result: SynchronizeGatewayTargetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SynchronizeGatewayTargetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
