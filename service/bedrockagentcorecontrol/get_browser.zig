...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBrowserInput, options: Options) !GetBrowserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBrowserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBrowserOutput {
    var result: GetBrowserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBrowserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
