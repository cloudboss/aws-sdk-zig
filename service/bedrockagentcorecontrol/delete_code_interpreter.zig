...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCodeInterpreterInput, options: Options) !DeleteCodeInterpreterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCodeInterpreterInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.code_interpreter_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCodeInterpreterOutput {
    var result: DeleteCodeInterpreterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteCodeInterpreterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
