...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkloadIdentityInput, options: Options) !UpdateWorkloadIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkloadIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allowed_resource_oauth_2_return_urls) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"allowedResourceOauth2ReturnUrls\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkloadIdentityOutput {
    var result: UpdateWorkloadIdentityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateWorkloadIdentityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
