...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreClusterFromSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/cluster-snapshot/");
    try path_buf.appendSlice(alloc, input.snapshot_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreClusterFromSnapshotOutput {
    var result: RestoreClusterFromSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RestoreClusterFromSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
