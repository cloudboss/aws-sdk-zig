...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"adminUserName\":");
    try aws.json.writeValue(@TypeOf(input.admin_user_name), input.admin_user_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"adminUserPassword\":");
    try aws.json.writeValue(@TypeOf(input.admin_user_password), input.admin_user_password, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"authType\":");
    try aws.json.writeValue(@TypeOf(input.auth_type), input.auth_type, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"shardCapacity\":");
    try aws.json.writeValue(@TypeOf(input.shard_capacity), input.shard_capacity, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"shardCount\":");
    try aws.json.writeValue(@TypeOf(input.shard_count), input.shard_count, alloc, &body_buf);
    has_prev = true;
    if (input.shard_instance_count) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"shardInstanceCount\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
