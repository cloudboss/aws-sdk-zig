...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"adminUserName\":");
    try aws.json.writeValue(@TypeOf(input.admin_user_name), input.admin_user_name, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"adminUserPassword\":");
    try aws.json.writeValue(@TypeOf(input.admin_user_password), input.admin_user_password, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.shard_capacity), input.shard_capacity, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"shardCount\":");
    try aws.json.writeValue(@TypeOf(input.shard_count), input.shard_count, allocator, &body_buf);
    has_prev = true;
