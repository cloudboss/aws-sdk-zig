...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListClusterSnapshotsInput, options: Options) !ListClusterSnapshotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListClusterSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.snapshot_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "snapshotType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListClusterSnapshotsOutput {
    var result: ListClusterSnapshotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListClusterSnapshotsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
