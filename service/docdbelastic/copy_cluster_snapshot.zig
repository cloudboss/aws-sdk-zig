...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyClusterSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.target_snapshot_name), input.target_snapshot_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyClusterSnapshotOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CopyClusterSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
