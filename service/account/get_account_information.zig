...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountInformationInput, options: Options) !GetAccountInformationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccountInformationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccountInformationOutput {
    var result: GetAccountInformationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAccountInformationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
