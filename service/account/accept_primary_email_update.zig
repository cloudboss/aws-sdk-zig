...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptPrimaryEmailUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Otp\":");
    try aws.json.writeValue(@TypeOf(input.otp), input.otp, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptPrimaryEmailUpdateOutput {
    var result: AcceptPrimaryEmailUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptPrimaryEmailUpdateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
