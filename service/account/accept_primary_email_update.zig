...
pub const AcceptPrimaryEmailUpdateInput = struct {
...
pub const AcceptPrimaryEmailUpdateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptPrimaryEmailUpdateInput, options: CallOptions) !AcceptPrimaryEmailUpdateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptPrimaryEmailUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/acceptPrimaryEmailUpdate";
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"Otp\":");
    try aws.json.writeValue(@TypeOf(input.otp), input.otp, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptPrimaryEmailUpdateOutput {
    var result: AcceptPrimaryEmailUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptPrimaryEmailUpdateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
