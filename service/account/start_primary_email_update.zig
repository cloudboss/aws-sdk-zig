...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartPrimaryEmailUpdateInput, options: Options) !StartPrimaryEmailUpdateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartPrimaryEmailUpdateInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("account", "Account", alloc);
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PrimaryEmail\":");
    try aws.json.writeValue(@TypeOf(input.primary_email), input.primary_email, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartPrimaryEmailUpdateOutput {
    var result: StartPrimaryEmailUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartPrimaryEmailUpdateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
