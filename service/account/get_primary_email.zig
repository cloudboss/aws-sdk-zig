...
pub const GetPrimaryEmailInput = struct {
...
pub const GetPrimaryEmailOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPrimaryEmailInput, options: CallOptions) !GetPrimaryEmailOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPrimaryEmailInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPrimaryEmailOutput {
    var result: GetPrimaryEmailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPrimaryEmailOutput, body, allocator);
    }
    _ = status;
    _ = headers;
