...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPrimaryEmailInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPrimaryEmailOutput {
    var result: GetPrimaryEmailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPrimaryEmailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
