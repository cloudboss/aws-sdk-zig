...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegionOptStatusInput, options: CallOptions) !GetRegionOptStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRegionOptStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRegionOptStatusOutput {
    var result: GetRegionOptStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRegionOptStatusOutput, body, allocator);
    }
    _ = status;
    _ = headers;
