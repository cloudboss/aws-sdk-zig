...
pub const GetRegionOptStatusInput = struct {
    ...
    /// `af-south-1`). This function will return the status of whatever Region you
    /// pass into this parameter.
...
pub const GetRegionOptStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegionOptStatusInput, options: Options) !GetRegionOptStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRegionOptStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.region_name), input.region_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRegionOptStatusOutput {
    var result: GetRegionOptStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRegionOptStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
