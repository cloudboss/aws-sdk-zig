...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAlternateContactInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"AlternateContactType\":");
    try aws.json.writeValue(@TypeOf(input.alternate_contact_type), input.alternate_contact_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.phone_number), input.phone_number, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAlternateContactOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
