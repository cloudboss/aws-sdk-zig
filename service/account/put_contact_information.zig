...
const PutContactInformationOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutContactInformationInput, options: Options) !PutContactInformationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutContactInformationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ContactInformation\":");
    try aws.json.writeValue(@TypeOf(input.contact_information), input.contact_information, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutContactInformationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
