...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGovCloudAccountInformationInput, options: Options) !GetGovCloudAccountInformationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGovCloudAccountInformationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.standard_account_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"StandardAccountId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGovCloudAccountInformationOutput {
    var result: GetGovCloudAccountInformationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGovCloudAccountInformationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
