...
pub const GetContactInformationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContactInformationInput, options: Options) !GetContactInformationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContactInformationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/getContactInformation";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContactInformationOutput {
    var result: GetContactInformationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContactInformationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
