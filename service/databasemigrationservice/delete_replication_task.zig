...
pub const DeleteReplicationTaskInput = struct {
...
pub const DeleteReplicationTaskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteReplicationTaskInput, options: CallOptions) !DeleteReplicationTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteReplicationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DeleteReplicationTask");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteReplicationTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteReplicationTaskOutput, body, allocator);
}
