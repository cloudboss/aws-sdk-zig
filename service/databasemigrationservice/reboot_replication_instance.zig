...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebootReplicationInstanceInput, options: Options) !RebootReplicationInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RebootReplicationInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.RebootReplicationInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RebootReplicationInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RebootReplicationInstanceOutput, body, alloc);
}
