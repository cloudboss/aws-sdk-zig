...
pub const DeleteFleetAdvisorDatabasesInput = struct {
    /// The IDs of the Fleet Advisor collector databases to delete.
    database_ids: []const []const u8,
...
pub const DeleteFleetAdvisorDatabasesOutput = struct {
    /// The IDs of the databases that the operation deleted.
    database_ids: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteFleetAdvisorDatabasesInput, options: CallOptions) !DeleteFleetAdvisorDatabasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteFleetAdvisorDatabasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DeleteFleetAdvisorDatabases");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteFleetAdvisorDatabasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteFleetAdvisorDatabasesOutput, body, allocator);
}
