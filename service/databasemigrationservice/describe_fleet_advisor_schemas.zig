...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetAdvisorSchemasInput, options: Options) !DescribeFleetAdvisorSchemasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetAdvisorSchemasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeFleetAdvisorSchemas");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetAdvisorSchemasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetAdvisorSchemasOutput, body, alloc);
}
