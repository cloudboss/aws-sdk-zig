...
pub const DescribeReplicationInstancesInput = struct {
...
pub const DescribeReplicationInstancesOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is
    /// specified, the response includes only records beyond the marker, up to the
    /// value specified
    ...
    replication_instances: ?[]const ReplicationInstance = null,

    pub const json_field_names = .{
        .marker = "Marker",
        .replication_instances = "ReplicationInstances",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationInstancesInput, options: CallOptions) !DescribeReplicationInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReplicationInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeReplicationInstances");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReplicationInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplicationInstancesOutput, body, allocator);
}
