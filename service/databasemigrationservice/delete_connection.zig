...
const DeleteConnectionInput = @import("delete_connection_message.zig").DeleteConnectionMessage;

const DeleteConnectionOutput = @import("delete_connection_response.zig").DeleteConnectionResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConnectionInput, options: Options) !DeleteConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DeleteConnection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteConnectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteConnectionOutput, body, alloc);
}
