...
pub const MoveReplicationTaskInput = struct {
...
pub const MoveReplicationTaskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MoveReplicationTaskInput, options: CallOptions) !MoveReplicationTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: MoveReplicationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.MoveReplicationTask");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !MoveReplicationTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(MoveReplicationTaskOutput, body, allocator);
}
