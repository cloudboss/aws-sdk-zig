...
pub const DescribeRefreshSchemasStatusInput = struct {
...
pub const DescribeRefreshSchemasStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRefreshSchemasStatusInput, options: CallOptions) !DescribeRefreshSchemasStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRefreshSchemasStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeRefreshSchemasStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeRefreshSchemasStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRefreshSchemasStatusOutput, body, allocator);
}
