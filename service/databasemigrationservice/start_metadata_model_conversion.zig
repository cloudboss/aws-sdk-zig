...
pub const StartMetadataModelConversionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMetadataModelConversionInput, options: CallOptions) !StartMetadataModelConversionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartMetadataModelConversionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.StartMetadataModelConversion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartMetadataModelConversionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMetadataModelConversionOutput, body, allocator);
}
