...
const ApplyPendingMaintenanceActionInput = @import("apply_pending_maintenance_action_message.zig").ApplyPendingMaintenanceActionMessage;

const ApplyPendingMaintenanceActionOutput = @import("apply_pending_maintenance_action_response.zig").ApplyPendingMaintenanceActionResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: ApplyPendingMaintenanceActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.ApplyPendingMaintenanceAction");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ApplyPendingMaintenanceActionOutput {
    ...
    return aws.json.parseJsonObject(ApplyPendingMaintenanceActionOutput, body, alloc);
}
