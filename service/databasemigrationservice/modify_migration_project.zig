...
pub const ModifyMigrationProjectInput = struct {
    /// A user-friendly description of the migration project.
    description: ?[]const u8 = null,

    /// The name or Amazon Resource Name (ARN) for the instance profile.
    instance_profile_identifier: ?[]const u8 = null,
    ...
    source_data_provider_descriptors: ?[]const DataProviderDescriptorDefinition = null,

    /// Information about the target data provider, including the name, ARN, and
    /// Amazon Web Services Secrets Manager parameters.
    target_data_provider_descriptors: ?[]const DataProviderDescriptorDefinition = null,
...
pub const ModifyMigrationProjectOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyMigrationProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.ModifyMigrationProject");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyMigrationProjectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyMigrationProjectOutput, body, allocator);
}
