...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataProvidersInput, options: Options) !DescribeDataProvidersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataProvidersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeDataProviders");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataProvidersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDataProvidersOutput, body, alloc);
}
