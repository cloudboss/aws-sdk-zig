...
pub const StartMetadataModelImportInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMetadataModelImportInput, options: CallOptions) !StartMetadataModelImportOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartMetadataModelImportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.StartMetadataModelImport");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartMetadataModelImportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMetadataModelImportOutput, body, allocator);
}
