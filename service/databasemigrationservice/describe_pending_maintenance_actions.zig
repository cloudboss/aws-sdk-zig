...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePendingMaintenanceActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribePendingMaintenanceActions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePendingMaintenanceActionsOutput {
    ...
    return aws.json.parseJsonObject(DescribePendingMaintenanceActionsOutput, body, alloc);
}
