...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMigrationProjectInput, options: Options) !CreateMigrationProjectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMigrationProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.CreateMigrationProject");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMigrationProjectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateMigrationProjectOutput, body, alloc);
}
