...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyReplicationConfigInput, options: Options) !ModifyReplicationConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyReplicationConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.ModifyReplicationConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyReplicationConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyReplicationConfigOutput, body, alloc);
}
