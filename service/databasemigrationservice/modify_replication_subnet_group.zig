...
pub const ModifyReplicationSubnetGroupInput = struct {
...
pub const ModifyReplicationSubnetGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyReplicationSubnetGroupInput, options: CallOptions) !ModifyReplicationSubnetGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyReplicationSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.ModifyReplicationSubnetGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyReplicationSubnetGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyReplicationSubnetGroupOutput, body, allocator);
}
