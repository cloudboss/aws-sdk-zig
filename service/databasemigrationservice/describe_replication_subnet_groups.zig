...
pub const DescribeReplicationSubnetGroupsInput = struct {
...
pub const DescribeReplicationSubnetGroupsOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is
    /// specified, the response includes only records beyond the marker, up to the
    /// value specified
    ...
    replication_subnet_groups: ?[]const ReplicationSubnetGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationSubnetGroupsInput, options: CallOptions) !DescribeReplicationSubnetGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReplicationSubnetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeReplicationSubnetGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReplicationSubnetGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplicationSubnetGroupsOutput, body, allocator);
}
