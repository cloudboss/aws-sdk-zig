...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDataMigrationInput, options: Options) !StartDataMigrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDataMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.StartDataMigration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDataMigrationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartDataMigrationOutput, body, alloc);
}
