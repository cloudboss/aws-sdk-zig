...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteReplicationInstanceInput, options: Options) !DeleteReplicationInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteReplicationInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DeleteReplicationInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteReplicationInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteReplicationInstanceOutput, body, alloc);
}
