...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationTaskAssessmentRunsInput, options: Options) !DescribeReplicationTaskAssessmentRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReplicationTaskAssessmentRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeReplicationTaskAssessmentRuns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReplicationTaskAssessmentRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplicationTaskAssessmentRunsOutput, body, alloc);
}
