...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataMigrationsInput, options: Options) !DescribeDataMigrationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataMigrationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeDataMigrations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataMigrationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDataMigrationsOutput, body, alloc);
}
