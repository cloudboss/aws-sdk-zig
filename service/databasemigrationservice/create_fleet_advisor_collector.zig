...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFleetAdvisorCollectorInput, options: Options) !CreateFleetAdvisorCollectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFleetAdvisorCollectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.CreateFleetAdvisorCollector");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFleetAdvisorCollectorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateFleetAdvisorCollectorOutput, body, alloc);
}
