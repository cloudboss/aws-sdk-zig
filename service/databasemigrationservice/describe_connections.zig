...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConnectionsInput, options: Options) !DescribeConnectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeConnections");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConnectionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConnectionsOutput, body, alloc);
}
