...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMetadataModelImportsInput, options: Options) !DescribeMetadataModelImportsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMetadataModelImportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeMetadataModelImports");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMetadataModelImportsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMetadataModelImportsOutput, body, alloc);
}
