...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReplicationTaskInput, options: Options) !CreateReplicationTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReplicationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.CreateReplicationTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReplicationTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateReplicationTaskOutput, body, alloc);
}
