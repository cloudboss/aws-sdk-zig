...
pub const DescribeReplicationConfigsInput = struct {
...
pub const DescribeReplicationConfigsOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is
    /// specified, the response includes only records beyond the marker, up to the
    /// value specified
    ...
    /// Serverless
    /// replication.
    replication_configs: ?[]const ReplicationConfig = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationConfigsInput, options: CallOptions) !DescribeReplicationConfigsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReplicationConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeReplicationConfigs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReplicationConfigsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplicationConfigsOutput, body, allocator);
}
