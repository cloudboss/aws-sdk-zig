...
pub const DescribeTableStatisticsInput = struct {
...
pub const DescribeTableStatisticsOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is
    /// specified, the response includes only records beyond the marker, up to the
    /// value specified
    ...
    table_statistics: ?[]const TableStatistics = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTableStatisticsInput, options: CallOptions) !DescribeTableStatisticsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTableStatisticsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeTableStatistics");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTableStatisticsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTableStatisticsOutput, body, allocator);
}
