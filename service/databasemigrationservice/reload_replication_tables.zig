...
pub const ReloadReplicationTablesInput = struct {
...
pub const ReloadReplicationTablesOutput = struct {
    /// The Amazon Resource Name of the replication config for which to reload
    /// tables.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReloadReplicationTablesInput, options: CallOptions) !ReloadReplicationTablesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ReloadReplicationTablesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.ReloadReplicationTables");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ReloadReplicationTablesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ReloadReplicationTablesOutput, body, allocator);
}
