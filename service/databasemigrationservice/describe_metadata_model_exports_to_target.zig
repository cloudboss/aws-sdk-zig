...
pub const DescribeMetadataModelExportsToTargetInput = struct {
...
pub const DescribeMetadataModelExportsToTargetOutput = struct {
    /// Specifies the unique pagination token that makes it possible to display the
    /// next page of results.
    /// If this parameter is specified, the response includes only records beyond
    /// the marker, up to the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMetadataModelExportsToTargetInput, options: CallOptions) !DescribeMetadataModelExportsToTargetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMetadataModelExportsToTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeMetadataModelExportsToTarget");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMetadataModelExportsToTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMetadataModelExportsToTargetOutput, body, allocator);
}
