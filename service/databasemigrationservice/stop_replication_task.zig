...
pub const StopReplicationTaskInput = struct {
...
pub const StopReplicationTaskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopReplicationTaskInput, options: CallOptions) !StopReplicationTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopReplicationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.StopReplicationTask");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopReplicationTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopReplicationTaskOutput, body, allocator);
}
