...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRecommendationLimitationsInput, options: Options) !DescribeRecommendationLimitationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRecommendationLimitationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeRecommendationLimitations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRecommendationLimitationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRecommendationLimitationsOutput, body, alloc);
}
