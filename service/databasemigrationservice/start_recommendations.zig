...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.StartRecommendations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRecommendationsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
