...
pub const StartRecommendationsInput = struct {
    ...
    /// availability and
    /// durability settings. For target instance sizing, Fleet Advisor supports the
    /// following
    /// two options: total capacity and resource utilization. For availability and
    /// durability,
    /// Fleet Advisor supports the following two options: production (Multi-AZ
    /// deployments) and
...
pub const StartRecommendationsOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRecommendationsInput, options: CallOptions) !StartRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.StartRecommendations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartRecommendationsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
