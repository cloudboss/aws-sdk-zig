...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMetadataModelConversionInput, options: Options) !CancelMetadataModelConversionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelMetadataModelConversionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.CancelMetadataModelConversion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelMetadataModelConversionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelMetadataModelConversionOutput, body, alloc);
}
