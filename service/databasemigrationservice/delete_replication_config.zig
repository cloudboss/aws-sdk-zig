...
pub const DeleteReplicationConfigInput = struct {
...
pub const DeleteReplicationConfigOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteReplicationConfigInput, options: CallOptions) !DeleteReplicationConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteReplicationConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DeleteReplicationConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteReplicationConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteReplicationConfigOutput, body, allocator);
}
