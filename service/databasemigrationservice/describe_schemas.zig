...
pub const DescribeSchemasInput = struct {
...
pub const DescribeSchemasOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is
    /// specified, the response includes only records beyond the marker, up to the
    /// value specified
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSchemasInput, options: CallOptions) !DescribeSchemasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSchemasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeSchemas");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSchemasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSchemasOutput, body, allocator);
}
