...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReplicationConfigInput, options: Options) !CreateReplicationConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReplicationConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.CreateReplicationConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReplicationConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateReplicationConfigOutput, body, alloc);
}
