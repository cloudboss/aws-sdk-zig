...
pub const StopReplicationOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopReplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.StopReplication");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopReplicationOutput {
    ...
    return aws.json.parseJsonObject(StopReplicationOutput, body, allocator);
}
