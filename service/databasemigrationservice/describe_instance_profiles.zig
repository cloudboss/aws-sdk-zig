...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceProfilesInput, options: Options) !DescribeInstanceProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeInstanceProfiles");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceProfilesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstanceProfilesOutput, body, alloc);
}
