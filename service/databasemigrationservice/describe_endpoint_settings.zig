...
const EndpointSetting = @import("endpoint_setting.zig").EndpointSetting;

pub const DescribeEndpointSettingsInput = struct {
...
pub const DescribeEndpointSettingsOutput = struct {
    /// Descriptions of the endpoint settings available for your source or target
    /// database
    /// engine.
    endpoint_settings: ?[]const EndpointSetting = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEndpointSettingsInput, options: CallOptions) !DescribeEndpointSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEndpointSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeEndpointSettings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEndpointSettingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEndpointSettingsOutput, body, allocator);
}
