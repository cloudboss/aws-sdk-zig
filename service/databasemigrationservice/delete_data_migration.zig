...
pub const DeleteDataMigrationInput = struct {
...
pub const DeleteDataMigrationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDataMigrationInput, options: CallOptions) !DeleteDataMigrationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDataMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DeleteDataMigration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDataMigrationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteDataMigrationOutput, body, allocator);
}
