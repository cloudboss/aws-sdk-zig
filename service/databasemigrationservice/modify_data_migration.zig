...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDataMigrationInput, options: Options) !ModifyDataMigrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDataMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.ModifyDataMigration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDataMigrationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyDataMigrationOutput, body, alloc);
}
