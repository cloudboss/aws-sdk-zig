...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeExtensionPackAssociationsInput, options: Options) !DescribeExtensionPackAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeExtensionPackAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeExtensionPackAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeExtensionPackAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeExtensionPackAssociationsOutput, body, alloc);
}
