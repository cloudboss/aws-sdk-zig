...
const RefreshSchemasStatus = @import("refresh_schemas_status.zig").RefreshSchemasStatus;

pub const RefreshSchemasInput = struct {
...
pub const RefreshSchemasOutput = struct {
    /// The status of the refreshed schema.
    refresh_schemas_status: ?RefreshSchemasStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RefreshSchemasInput, options: CallOptions) !RefreshSchemasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RefreshSchemasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.RefreshSchemas");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RefreshSchemasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RefreshSchemasOutput, body, allocator);
}
