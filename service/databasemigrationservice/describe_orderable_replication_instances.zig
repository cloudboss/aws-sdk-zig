...
const OrderableReplicationInstance = @import("orderable_replication_instance.zig").OrderableReplicationInstance;

pub const DescribeOrderableReplicationInstancesInput = struct {
...
pub const DescribeOrderableReplicationInstancesOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is
    /// specified, the response includes only records beyond the marker, up to the
    /// value specified
    ...
    orderable_replication_instances: ?[]const OrderableReplicationInstance = null,

    pub const json_field_names = .{
        .marker = "Marker",
        .orderable_replication_instances = "OrderableReplicationInstances",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrderableReplicationInstancesInput, options: CallOptions) !DescribeOrderableReplicationInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeOrderableReplicationInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeOrderableReplicationInstances");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeOrderableReplicationInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeOrderableReplicationInstancesOutput, body, allocator);
}
