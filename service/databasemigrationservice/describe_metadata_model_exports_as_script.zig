...
pub const DescribeMetadataModelExportsAsScriptOutput = struct {
    /// Specifies the unique pagination token that makes it possible to display the
    /// next page of results.
    /// If this parameter is specified, the response includes only records beyond
    /// the marker, up to the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMetadataModelExportsAsScriptInput, options: CallOptions) !DescribeMetadataModelExportsAsScriptOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMetadataModelExportsAsScriptInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeMetadataModelExportsAsScript");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMetadataModelExportsAsScriptOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMetadataModelExportsAsScriptOutput, body, allocator);
}
