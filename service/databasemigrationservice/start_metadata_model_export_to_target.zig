...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMetadataModelExportToTargetInput, options: Options) !StartMetadataModelExportToTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMetadataModelExportToTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.StartMetadataModelExportToTarget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMetadataModelExportToTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMetadataModelExportToTargetOutput, body, alloc);
}
