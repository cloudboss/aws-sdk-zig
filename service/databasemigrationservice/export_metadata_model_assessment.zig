...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportMetadataModelAssessmentInput, options: Options) !ExportMetadataModelAssessmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExportMetadataModelAssessmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.ExportMetadataModelAssessment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExportMetadataModelAssessmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportMetadataModelAssessmentOutput, body, alloc);
}
