...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMetadataModelExportAsScriptInput, options: Options) !StartMetadataModelExportAsScriptOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMetadataModelExportAsScriptInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.StartMetadataModelExportAsScript");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMetadataModelExportAsScriptOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMetadataModelExportAsScriptOutput, body, alloc);
}
