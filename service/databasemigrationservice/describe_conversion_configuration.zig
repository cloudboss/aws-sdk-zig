...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConversionConfigurationInput, options: Options) !DescribeConversionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConversionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeConversionConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConversionConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConversionConfigurationOutput, body, alloc);
}
