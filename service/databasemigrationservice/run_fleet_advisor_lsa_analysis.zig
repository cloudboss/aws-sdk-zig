...
fn serializeRequest(alloc: std.mem.Allocator, input: RunFleetAdvisorLsaAnalysisInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.RunFleetAdvisorLsaAnalysis");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RunFleetAdvisorLsaAnalysisOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RunFleetAdvisorLsaAnalysisOutput, body, alloc);
}
