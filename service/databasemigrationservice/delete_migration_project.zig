...
pub const DeleteMigrationProjectInput = struct {
...
pub const DeleteMigrationProjectOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMigrationProjectInput, options: CallOptions) !DeleteMigrationProjectOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMigrationProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DeleteMigrationProject");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMigrationProjectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteMigrationProjectOutput, body, allocator);
}
