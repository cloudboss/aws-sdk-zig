...
pub const DescribeRecommendationsInput = struct {
...
pub const DescribeRecommendationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRecommendationsInput, options: CallOptions) !DescribeRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeRecommendations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRecommendationsOutput, body, allocator);
}
