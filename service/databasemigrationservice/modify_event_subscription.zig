...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyEventSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.ModifyEventSubscription");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyEventSubscriptionOutput {
    ...
    return aws.json.parseJsonObject(ModifyEventSubscriptionOutput, body, alloc);
}
