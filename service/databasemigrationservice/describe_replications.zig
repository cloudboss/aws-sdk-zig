...
pub const DescribeReplicationsInput = struct {
...
pub const DescribeReplicationsOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is
    /// specified, the response includes only records beyond the marker, up to the
    /// value specified
    ...
    replications: ?[]const Replication = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationsInput, options: CallOptions) !DescribeReplicationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeReplications");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReplicationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplicationsOutput, body, allocator);
}
