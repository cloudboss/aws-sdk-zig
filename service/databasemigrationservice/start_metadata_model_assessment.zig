...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMetadataModelAssessmentInput, options: Options) !StartMetadataModelAssessmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMetadataModelAssessmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.StartMetadataModelAssessment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMetadataModelAssessmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMetadataModelAssessmentOutput, body, alloc);
}
