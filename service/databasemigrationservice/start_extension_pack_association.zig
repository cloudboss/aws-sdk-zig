...
pub const StartExtensionPackAssociationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartExtensionPackAssociationInput, options: CallOptions) !StartExtensionPackAssociationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartExtensionPackAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.StartExtensionPackAssociation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartExtensionPackAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartExtensionPackAssociationOutput, body, allocator);
}
