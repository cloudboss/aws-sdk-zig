...
fn serializeRequest(alloc: std.mem.Allocator, input: StartExtensionPackAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.StartExtensionPackAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartExtensionPackAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartExtensionPackAssociationOutput, body, alloc);
}
