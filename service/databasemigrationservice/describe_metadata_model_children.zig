...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMetadataModelChildrenInput, options: Options) !DescribeMetadataModelChildrenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMetadataModelChildrenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeMetadataModelChildren");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMetadataModelChildrenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMetadataModelChildrenOutput, body, alloc);
}
