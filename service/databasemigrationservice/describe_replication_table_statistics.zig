...
const TableStatistics = @import("table_statistics.zig").TableStatistics;

pub const DescribeReplicationTableStatisticsInput = struct {
...
pub const DescribeReplicationTableStatisticsOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is
    /// specified, the response includes only records beyond the marker, up to the
    /// value specified
    ...
    replication_table_statistics: ?[]const TableStatistics = null,

    pub const json_field_names = .{
        .marker = "Marker",
        .replication_config_arn = "ReplicationConfigArn",
        .replication_table_statistics = "ReplicationTableStatistics",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationTableStatisticsInput, options: CallOptions) !DescribeReplicationTableStatisticsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReplicationTableStatisticsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeReplicationTableStatistics");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReplicationTableStatisticsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplicationTableStatisticsOutput, body, allocator);
}
