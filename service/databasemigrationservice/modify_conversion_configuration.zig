...
pub const ModifyConversionConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyConversionConfigurationInput, options: CallOptions) !ModifyConversionConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyConversionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.ModifyConversionConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyConversionConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyConversionConfigurationOutput, body, allocator);
}
