...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationInstanceTaskLogsInput, options: Options) !DescribeReplicationInstanceTaskLogsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReplicationInstanceTaskLogsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeReplicationInstanceTaskLogs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReplicationInstanceTaskLogsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplicationInstanceTaskLogsOutput, body, alloc);
}
