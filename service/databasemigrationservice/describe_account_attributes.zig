...
pub const DescribeAccountAttributesOutput = struct {
    ...
    account_quotas: ?[]const AccountQuota = null,

    /// A unique DMS identifier for an account in a particular Amazon Web Services
    /// Region. The value of this
    /// identifier has the following format: `c99999999999`. DMS uses this
    /// identifier to
    /// name artifacts. For example, DMS uses this identifier to name the default
    /// Amazon S3 bucket
    /// for storing task assessment reports in a given Amazon Web Services Region.
    /// The format of this S3 bucket
    ...
    /// and later.
    unique_account_identifier: ?[]const u8 = null,

    pub const json_field_names = .{
        .account_quotas = "AccountQuotas",
        .unique_account_identifier = "UniqueAccountIdentifier",
    };
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeAccountAttributes");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountAttributesOutput {
    ...
    return aws.json.parseJsonObject(DescribeAccountAttributesOutput, body, alloc);
}
