...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataMigrationInput, options: Options) !CreateDataMigrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.CreateDataMigration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataMigrationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDataMigrationOutput, body, alloc);
}
