...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSubscriptionsToEventBridgeInput, options: Options) !UpdateSubscriptionsToEventBridgeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSubscriptionsToEventBridgeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.UpdateSubscriptionsToEventBridge");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSubscriptionsToEventBridgeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSubscriptionsToEventBridgeOutput, body, alloc);
}
