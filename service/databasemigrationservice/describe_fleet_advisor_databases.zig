...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetAdvisorDatabasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeFleetAdvisorDatabases");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetAdvisorDatabasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetAdvisorDatabasesOutput, body, alloc);
}
