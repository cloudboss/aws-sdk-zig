...
pub const GetTargetSelectionRulesInput = struct {
...
pub const GetTargetSelectionRulesOutput = struct {
    /// The JSON string representing the counterpart selection rules in the target.
    target_selection_rules: ?[]const u8 = null,

    pub const json_field_names = .{
        .target_selection_rules = "TargetSelectionRules",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTargetSelectionRulesInput, options: CallOptions) !GetTargetSelectionRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTargetSelectionRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.GetTargetSelectionRules");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTargetSelectionRulesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTargetSelectionRulesOutput, body, allocator);
}
