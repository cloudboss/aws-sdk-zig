...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDataProviderInput, options: Options) !ModifyDataProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDataProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.ModifyDataProvider");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDataProviderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyDataProviderOutput, body, alloc);
}
