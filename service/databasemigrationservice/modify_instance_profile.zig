...
pub const ModifyInstanceProfileInput = struct {
...
pub const ModifyInstanceProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyInstanceProfileInput, options: CallOptions) !ModifyInstanceProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyInstanceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.ModifyInstanceProfile");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyInstanceProfileOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyInstanceProfileOutput, body, allocator);
}
