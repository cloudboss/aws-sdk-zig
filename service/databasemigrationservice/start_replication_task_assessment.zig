...
fn serializeRequest(alloc: std.mem.Allocator, input: StartReplicationTaskAssessmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.StartReplicationTaskAssessment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartReplicationTaskAssessmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartReplicationTaskAssessmentOutput, body, alloc);
}
