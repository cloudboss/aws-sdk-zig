...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationTaskAssessmentResultsInput, options: Options) !DescribeReplicationTaskAssessmentResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReplicationTaskAssessmentResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeReplicationTaskAssessmentResults");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReplicationTaskAssessmentResultsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplicationTaskAssessmentResultsOutput, body, alloc);
}
