...
pub const CreateEventSubscriptionInput = struct {
    ...
    /// Notifications](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Events.html) in the *Database Migration Service User
    /// Guide.*
    ...
    /// if you
    /// specify a database instance ID, then all of the other values must be
    /// database instance
    /// IDs.
    ...
    /// to be
    /// notified of events generated by a replication instance, you set this
    /// parameter to
...
pub const CreateEventSubscriptionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEventSubscriptionInput, options: CallOptions) !CreateEventSubscriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateEventSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.CreateEventSubscription");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateEventSubscriptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEventSubscriptionOutput, body, allocator);
}
