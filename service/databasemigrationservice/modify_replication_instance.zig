...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyReplicationInstanceInput, options: Options) !ModifyReplicationInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyReplicationInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.ModifyReplicationInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyReplicationInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyReplicationInstanceOutput, body, alloc);
}
