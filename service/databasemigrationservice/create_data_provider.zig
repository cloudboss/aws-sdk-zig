...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataProviderInput, options: Options) !CreateDataProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.CreateDataProvider");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataProviderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDataProviderOutput, body, alloc);
}
