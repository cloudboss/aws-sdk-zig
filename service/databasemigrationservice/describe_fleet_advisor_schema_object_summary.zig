...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetAdvisorSchemaObjectSummaryInput, options: Options) !DescribeFleetAdvisorSchemaObjectSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetAdvisorSchemaObjectSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeFleetAdvisorSchemaObjectSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetAdvisorSchemaObjectSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetAdvisorSchemaObjectSummaryOutput, body, alloc);
}
