...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetAdvisorCollectorsInput, options: Options) !DescribeFleetAdvisorCollectorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetAdvisorCollectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeFleetAdvisorCollectors");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetAdvisorCollectorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetAdvisorCollectorsOutput, body, alloc);
}
