...
pub const CancelMetadataModelCreationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMetadataModelCreationInput, options: CallOptions) !CancelMetadataModelCreationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelMetadataModelCreationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.CancelMetadataModelCreation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelMetadataModelCreationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelMetadataModelCreationOutput, body, allocator);
}
