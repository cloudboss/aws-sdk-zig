...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReloadTablesInput, options: Options) !ReloadTablesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReloadTablesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.ReloadTables");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReloadTablesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ReloadTablesOutput, body, alloc);
}
