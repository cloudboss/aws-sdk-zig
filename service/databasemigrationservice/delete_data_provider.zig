...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDataProviderInput, options: Options) !DeleteDataProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDataProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DeleteDataProvider");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDataProviderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteDataProviderOutput, body, alloc);
}
