const CannedAclForObjectsValue = @import("canned_acl_for_objects_value.zig").CannedAclForObjectsValue;
const CompressionTypeValue = @import("compression_type_value.zig").CompressionTypeValue;
const DataFormatValue = @import("data_format_value.zig").DataFormatValue;
const DatePartitionDelimiterValue = @import("date_partition_delimiter_value.zig").DatePartitionDelimiterValue;
const DatePartitionSequenceValue = @import("date_partition_sequence_value.zig").DatePartitionSequenceValue;
const EncodingTypeValue = @import("encoding_type_value.zig").EncodingTypeValue;
const EncryptionModeValue = @import("encryption_mode_value.zig").EncryptionModeValue;
const ParquetVersionValue = @import("parquet_version_value.zig").ParquetVersionValue;

/// Settings for exporting data to Amazon S3.
pub const S3Settings = struct {
    ...
    /// list for
    /// objects created in an Amazon S3 bucket as .csv or .parquet files. For more
    /// information
    /// about Amazon S3 canned ACLs, see [Canned
    /// ACL](http://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html#canned-acl) in the *Amazon S3 Developer Guide.*
    ///
    ...
    /// `IncludeOpForFullLoad` parameter. If `IncludeOpForFullLoad` is set
    /// to `true`, the first field of every CDC record is set to either `I`
    /// or `U` to indicate INSERT and UPDATE operations at the source. But if
    /// `IncludeOpForFullLoad` is set to `false`, CDC records are written
    /// without an indication of INSERT or UPDATE operations at the source. For more
    /// information
    /// about how these settings work together, see [Indicating Source DB Operations
    /// in Migrated S3
    /// Data](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Target.S3.html#CHAP_Target.S3.Configuring.InsertOps) in the *Database Migration Service
    /// User Guide.*.
    ///
    ...
    /// operations to
    /// .csv or columnar storage (.parquet) output files. By default (the `false`
    /// setting), the first field in a .csv or .parquet record contains the letter I
    /// (INSERT), U
    /// (UPDATE), or D (DELETE). These values indicate whether the row was inserted,
    /// updated, or
    /// deleted at the source database for a CDC load to the target.
    ///
    /// If `CdcInsertsOnly` is set to `true` or `y`, only
    /// INSERTs from the source database are migrated to the .csv or .parquet file.
    /// For .csv format
    ...
    /// Amazon S3.
    ///
    /// When `CdcMaxBatchInterval` and `CdcMinFileSize` are both
    /// specified, the file write is triggered by whichever parameter condition is
    /// met first within
    ...
    /// Amazon S3.
    ///
    /// When `CdcMinFileSize` and `CdcMaxBatchInterval` are both
    /// specified, the file write is triggered by whichever parameter condition is
    /// met first within
    /// an DMS CloudFormation template.
    ///
    ...
    /// required if a
    /// task captures change data; otherwise, it's optional. If `CdcPath` is set,
    /// DMS
    /// reads CDC files from this path and replicates the data changes to the target
    /// endpoint. For
    /// an S3 target if you set [
    /// `PreserveTransactions`
    /// ](https://docs.aws.amazon.com/dms/latest/APIReference/API_S3Settings.html#DMS-Type-S3Settings-PreserveTransactions) to `true`,
    /// DMS verifies that you have set this parameter to a folder path on your S3
    /// target where
    /// DMS can save the transaction order for the CDC load. DMS creates this CDC
    /// folder path
    /// in either your S3 target working directory or the S3 target location
    /// specified by [
    /// `BucketFolder`
    /// ](https://docs.aws.amazon.com/dms/latest/APIReference/API_S3Settings.html#DMS-Type-S3Settings-BucketFolder) and [
    /// `BucketName`
    ...
    /// [Capturing data changes (CDC) including transaction order on the S3
    ...
    /// `UseCsvNoSupValue`
    /// ](https://docs.aws.amazon.com/dms/latest/APIReference/API_S3Settings.html#DMS-Type-S3Settings-UseCsvNoSupValue) is set to true, specify a
    /// string value that you want DMS to use for all columns not included in the
    /// supplemental
    ...
    /// handling the
    /// null value, you can use this parameter to pass a user-defined string as null
    /// when writing
    /// to the target. For example, when target columns are nullable, you can use
    /// this option to
    /// differentiate between the empty string value and the null value. So, if you
    /// set this
    /// parameter value to the empty string ("" or ''), DMS treats the empty string
    /// as the null
    ...
    /// zone format is Area/Location. Use this parameter when
    ...
    /// (.csv) or .parquet output files only to indicate how the rows were added to
    /// the source
    /// database.
    ///
    ...
    /// to indicate that
    /// the rows were inserted at the source database. If `IncludeOpForFullLoad` is
    /// set
    /// to `true` or `y`, the INSERT is recorded as an I annotation in the
    /// first field of the .csv file. This allows the format of your target records
    /// from a full
    ...
    /// information about how these settings work together, see [Indicating Source
    ...
    /// `y`, DMS writes all `TIMESTAMP` columns in a .parquet formatted
    ...
    /// `TIMESTAMP` values. Set this parameter to `true` for S3 endpoint
    ...
    /// `CdcPath`
    /// ](https://docs.aws.amazon.com/dms/latest/APIReference/API_S3Settings.html#DMS-Type-S3Settings-CdcPath). For more information, see [Capturing data changes (CDC) including transaction order on the S3
    /// target](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Target.S3.html#CHAP_Target.S3.EndpointSettings.CdcPath).
    ///
    /// This setting is supported in DMS versions 3.4.2 and later.
    ...
    /// quotation mark. This
    /// formatting complies with RFC 4180. When this value is set to `false` or
    /// `n`, string literals are copied to the target as is. In this case, a
    /// delimiter (row or column) signals the end of the field. Thus, you can't use
    /// a
    /// delimiter as part of the string, because it signals the end of the value.
    ///
    /// For an S3 target, an optional parameter used to set behavior to comply with
    /// RFC 4180 for
    /// data migrated to Amazon S3 using .csv file format only. When this value is
    /// set to
    /// `true` or `y` using Amazon S3 as a target, if the data has
    /// quotation marks or newline characters in it, DMS encloses the entire column
    /// with an
    /// additional pair of double quotation marks ("). Every quotation mark within
    /// the data is
    /// repeated twice.
    ///
    ...
    /// reads. But
    /// as the number of row groups grows, the slower writes become. This parameter
    /// defaults to
    ...
    /// microseconds. For
    /// a CDC load, the rounding of the precision depends on the commit timestamp
    /// supported by DMS
    ...
    /// `CsvNoSupValue`
    /// ](https://docs.aws.amazon.com/dms/latest/APIReference/API_S3Settings.html#DMS-Type-S3Settings-CsvNoSupValue). If not set or set to
    ...
    /// `useTaskStartTimeForFullLoadTimestamp` is set to `true`, each row
    /// of the timestamp column contains the task start time. For CDC loads, each
    /// row of the
