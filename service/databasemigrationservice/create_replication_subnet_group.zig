...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReplicationSubnetGroupInput, options: Options) !CreateReplicationSubnetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReplicationSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.CreateReplicationSubnetGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReplicationSubnetGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateReplicationSubnetGroupOutput, body, alloc);
}
