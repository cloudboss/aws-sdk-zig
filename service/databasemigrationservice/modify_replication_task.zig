...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyReplicationTaskInput, options: Options) !ModifyReplicationTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyReplicationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.ModifyReplicationTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyReplicationTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyReplicationTaskOutput, body, alloc);
}
