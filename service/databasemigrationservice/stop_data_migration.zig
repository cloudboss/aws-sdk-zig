...
pub const StopDataMigrationInput = struct {
...
pub const StopDataMigrationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopDataMigrationInput, options: CallOptions) !StopDataMigrationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopDataMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.StopDataMigration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopDataMigrationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopDataMigrationOutput, body, allocator);
}
