...
pub const DescribeMigrationProjectsInput = struct {
...
pub const DescribeMigrationProjectsOutput = struct {
    /// Specifies the unique pagination token that makes it possible to display the
    /// next page of results.
    /// If this parameter is specified, the response includes only records beyond
    /// the marker, up to the
    ...
    migration_projects: ?[]const MigrationProject = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMigrationProjectsInput, options: CallOptions) !DescribeMigrationProjectsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMigrationProjectsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeMigrationProjects");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMigrationProjectsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMigrationProjectsOutput, body, allocator);
}
