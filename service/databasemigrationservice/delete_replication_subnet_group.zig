...
pub const DeleteReplicationSubnetGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteReplicationSubnetGroupInput, options: Options) !DeleteReplicationSubnetGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteReplicationSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DeleteReplicationSubnetGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteReplicationSubnetGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
