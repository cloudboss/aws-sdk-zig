...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMetadataModelCreationsInput, options: CallOptions) !DescribeMetadataModelCreationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMetadataModelCreationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDMSv20160101.DescribeMetadataModelCreations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMetadataModelCreationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMetadataModelCreationsOutput, body, allocator);
}
