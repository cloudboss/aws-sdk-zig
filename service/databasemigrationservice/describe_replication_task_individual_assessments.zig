...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationTaskIndividualAssessmentsInput, options: Options) !DescribeReplicationTaskIndividualAssessmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReplicationTaskIndividualAssessmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDMSv20160101.DescribeReplicationTaskIndividualAssessments");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReplicationTaskIndividualAssessmentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplicationTaskIndividualAssessmentsOutput, body, alloc);
}
