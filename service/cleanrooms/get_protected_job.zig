...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProtectedJobInput, options: Options) !GetProtectedJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProtectedJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.protected_job_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProtectedJobOutput {
    var result: GetProtectedJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProtectedJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
