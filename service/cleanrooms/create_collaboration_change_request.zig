...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCollaborationChangeRequestInput, options: Options) !CreateCollaborationChangeRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCollaborationChangeRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.changes), input.changes, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCollaborationChangeRequestOutput {
    var result: CreateCollaborationChangeRequestOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCollaborationChangeRequestOutput, body, alloc);
    }
    _ = status;
    _ = headers;
