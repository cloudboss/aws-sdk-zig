...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProtectedQueryInput, options: Options) !UpdateProtectedQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProtectedQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.protected_query_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProtectedQueryOutput {
    var result: UpdateProtectedQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProtectedQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
