...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIdNamespaceAssociationInput, options: Options) !CreateIdNamespaceAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIdNamespaceAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/idnamespaceassociations");
    ...
    if (input.id_mapping_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"idMappingConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"inputReferenceConfig\":");
    try aws.json.writeValue(@TypeOf(input.input_reference_config), input.input_reference_config, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIdNamespaceAssociationOutput {
    var result: CreateIdNamespaceAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIdNamespaceAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
