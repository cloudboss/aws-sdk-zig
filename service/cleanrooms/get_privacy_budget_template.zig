...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPrivacyBudgetTemplateInput, options: Options) !GetPrivacyBudgetTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPrivacyBudgetTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPrivacyBudgetTemplateOutput {
    var result: GetPrivacyBudgetTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPrivacyBudgetTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
