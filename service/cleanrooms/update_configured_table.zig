...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfiguredTableInput, options: Options) !UpdateConfiguredTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfiguredTableInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfiguredTableOutput {
    var result: UpdateConfiguredTableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConfiguredTableOutput, body, alloc);
    }
    _ = status;
    _ = headers;
