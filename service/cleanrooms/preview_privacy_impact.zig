...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PreviewPrivacyImpactInput, options: Options) !PreviewPrivacyImpactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PreviewPrivacyImpactInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/previewprivacyimpact");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PreviewPrivacyImpactOutput {
    var result: PreviewPrivacyImpactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PreviewPrivacyImpactOutput, body, alloc);
    }
    _ = status;
    _ = headers;
