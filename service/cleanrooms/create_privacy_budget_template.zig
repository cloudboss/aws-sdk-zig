...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePrivacyBudgetTemplateInput, options: Options) !CreatePrivacyBudgetTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePrivacyBudgetTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.auto_refresh) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"autoRefresh\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePrivacyBudgetTemplateOutput {
    var result: CreatePrivacyBudgetTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePrivacyBudgetTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
