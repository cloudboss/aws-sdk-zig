...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConfiguredTableAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"configuredTableIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.configured_table_identifier), input.configured_table_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConfiguredTableAssociationOutput {
    var result: CreateConfiguredTableAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConfiguredTableAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
