...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCollaborationChangeRequestInput, options: Options) !UpdateCollaborationChangeRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCollaborationChangeRequestInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCollaborationChangeRequestOutput {
    var result: UpdateCollaborationChangeRequestOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCollaborationChangeRequestOutput, body, alloc);
    }
    _ = status;
    _ = headers;
