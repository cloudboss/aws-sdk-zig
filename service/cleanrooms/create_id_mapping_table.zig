...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIdMappingTableInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIdMappingTableOutput {
    var result: CreateIdMappingTableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIdMappingTableOutput, body, alloc);
    }
    _ = status;
    _ = headers;
