...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"collaborationIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.collaboration_identifier), input.collaboration_identifier, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.payment_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"paymentConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
