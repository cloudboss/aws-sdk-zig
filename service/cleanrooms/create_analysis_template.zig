...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAnalysisTemplateInput, options: CallOptions) !CreateAnalysisTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAnalysisTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.analysis_parameters) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"analysisParameters\":");
    ...
    if (input.error_message_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"errorMessageConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    if (input.synthetic_data_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"syntheticDataParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
