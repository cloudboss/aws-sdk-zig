...
pub const CreateAnalysisTemplateInput = struct {
...
pub const CreateAnalysisTemplateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAnalysisTemplateInput, options: Options) !CreateAnalysisTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAnalysisTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.analysis_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"analysisParameters\":");
    ...
    if (input.error_message_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"errorMessageConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (input.synthetic_data_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"syntheticDataParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAnalysisTemplateOutput {
    var result: CreateAnalysisTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAnalysisTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
