...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCollaborationAnalysisTemplateInput, options: Options) !GetCollaborationAnalysisTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCollaborationAnalysisTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.analysis_template_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCollaborationAnalysisTemplateOutput {
    var result: GetCollaborationAnalysisTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCollaborationAnalysisTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
