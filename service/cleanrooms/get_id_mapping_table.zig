...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdMappingTableInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdMappingTableOutput {
    var result: GetIdMappingTableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIdMappingTableOutput, body, alloc);
    }
    _ = status;
    _ = headers;
