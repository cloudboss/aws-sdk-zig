...
pub const GetIdMappingTableInput = struct {
...
pub const GetIdMappingTableOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdMappingTableInput, options: CallOptions) !GetIdMappingTableOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIdMappingTableInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIdMappingTableOutput {
    var result: GetIdMappingTableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIdMappingTableOutput, body, allocator);
    }
    _ = status;
    _ = headers;
