...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetSchemaAnalysisRuleInput, options: Options) !BatchGetSchemaAnalysisRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetSchemaAnalysisRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batch-schema-analysis-rule");
    ...
    try body_buf.appendSlice(alloc, "\"schemaAnalysisRuleRequests\":");
    try aws.json.writeValue(@TypeOf(input.schema_analysis_rule_requests), input.schema_analysis_rule_requests, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetSchemaAnalysisRuleOutput {
    var result: BatchGetSchemaAnalysisRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetSchemaAnalysisRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
