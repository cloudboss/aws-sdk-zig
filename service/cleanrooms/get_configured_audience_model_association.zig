...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfiguredAudienceModelAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfiguredAudienceModelAssociationOutput {
    var result: GetConfiguredAudienceModelAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfiguredAudienceModelAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
