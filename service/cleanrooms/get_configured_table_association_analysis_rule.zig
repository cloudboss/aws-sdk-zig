...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfiguredTableAssociationAnalysisRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfiguredTableAssociationAnalysisRuleOutput {
    var result: GetConfiguredTableAssociationAnalysisRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfiguredTableAssociationAnalysisRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
