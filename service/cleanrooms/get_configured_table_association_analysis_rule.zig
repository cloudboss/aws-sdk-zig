...
pub const GetConfiguredTableAssociationAnalysisRuleInput = struct {
...
pub const GetConfiguredTableAssociationAnalysisRuleOutput = struct {
    ...
    analysis_rule: ?ConfiguredTableAssociationAnalysisRule = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetConfiguredTableAssociationAnalysisRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetConfiguredTableAssociationAnalysisRuleOutput {
    var result: GetConfiguredTableAssociationAnalysisRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfiguredTableAssociationAnalysisRuleOutput, body, allocator);
    }
    _ = status;
    _ = headers;
