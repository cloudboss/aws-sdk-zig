...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConfiguredTableAnalysisRuleInput, options: Options) !CreateConfiguredTableAnalysisRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConfiguredTableAnalysisRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/configuredTables/");
    try path_buf.appendSlice(alloc, input.configured_table_identifier);
    ...
    try body_buf.appendSlice(alloc, "\"analysisRulePolicy\":");
    try aws.json.writeValue(@TypeOf(input.analysis_rule_policy), input.analysis_rule_policy, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"analysisRuleType\":");
    try aws.json.writeValue(@TypeOf(input.analysis_rule_type), input.analysis_rule_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConfiguredTableAnalysisRuleOutput {
    var result: CreateConfiguredTableAnalysisRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConfiguredTableAnalysisRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
