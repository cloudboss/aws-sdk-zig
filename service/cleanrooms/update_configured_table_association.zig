...
pub const UpdateConfiguredTableAssociationInput = struct {
...
pub const UpdateConfiguredTableAssociationOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfiguredTableAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.configured_table_association_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfiguredTableAssociationOutput {
    var result: UpdateConfiguredTableAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConfiguredTableAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
