...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMembershipsInput, options: Options) !ListMembershipsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMembershipsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMembershipsOutput {
    var result: ListMembershipsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMembershipsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
