...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPrivacyBudgetTemplatesInput, options: Options) !ListPrivacyBudgetTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPrivacyBudgetTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPrivacyBudgetTemplatesOutput {
    var result: ListPrivacyBudgetTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrivacyBudgetTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
