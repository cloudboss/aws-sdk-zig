...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetCollaborationAnalysisTemplateInput, options: Options) !BatchGetCollaborationAnalysisTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetCollaborationAnalysisTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batch-analysistemplates");
    ...
    try body_buf.appendSlice(alloc, "\"analysisTemplateArns\":");
    try aws.json.writeValue(@TypeOf(input.analysis_template_arns), input.analysis_template_arns, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetCollaborationAnalysisTemplateOutput {
    var result: BatchGetCollaborationAnalysisTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetCollaborationAnalysisTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
