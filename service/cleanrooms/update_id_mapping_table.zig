...
pub const UpdateIdMappingTableInput = struct {
...
pub const UpdateIdMappingTableOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIdMappingTableInput, options: Options) !UpdateIdMappingTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIdMappingTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/idmappingtables/");
    try path_buf.appendSlice(alloc, input.id_mapping_table_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIdMappingTableOutput {
    var result: UpdateIdMappingTableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIdMappingTableOutput, body, alloc);
    }
    _ = status;
    _ = headers;
