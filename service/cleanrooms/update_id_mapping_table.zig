...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIdMappingTableInput, options: CallOptions) !UpdateIdMappingTableOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateIdMappingTableInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateIdMappingTableOutput {
    var result: UpdateIdMappingTableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIdMappingTableOutput, body, allocator);
    }
    _ = status;
    _ = headers;
