...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSchemaAnalysisRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSchemaAnalysisRuleOutput {
    var result: GetSchemaAnalysisRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSchemaAnalysisRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
