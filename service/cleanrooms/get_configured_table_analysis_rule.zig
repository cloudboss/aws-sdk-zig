...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConfiguredTableAnalysisRuleInput, options: Options) !GetConfiguredTableAnalysisRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfiguredTableAnalysisRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfiguredTableAnalysisRuleOutput {
    var result: GetConfiguredTableAnalysisRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfiguredTableAnalysisRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
