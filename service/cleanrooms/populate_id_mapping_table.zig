...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PopulateIdMappingTableInput, options: Options) !PopulateIdMappingTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PopulateIdMappingTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/populate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PopulateIdMappingTableOutput {
    var result: PopulateIdMappingTableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PopulateIdMappingTableOutput, body, alloc);
    }
    _ = status;
    _ = headers;
