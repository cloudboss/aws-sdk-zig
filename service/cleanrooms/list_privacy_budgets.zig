...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPrivacyBudgetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPrivacyBudgetsOutput {
    var result: ListPrivacyBudgetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrivacyBudgetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
