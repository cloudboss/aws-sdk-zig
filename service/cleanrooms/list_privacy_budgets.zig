...
const PrivacyBudgetSummary = @import("privacy_budget_summary.zig").PrivacyBudgetSummary;

pub const ListPrivacyBudgetsInput = struct {
...
pub const ListPrivacyBudgetsOutput = struct {
    /// The pagination token that's used to fetch the next set of results.
    next_token: ?[]const u8 = null,
    ...
    privacy_budget_summaries: ?[]const PrivacyBudgetSummary = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .privacy_budget_summaries = "privacyBudgetSummaries",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPrivacyBudgetsInput, options: CallOptions) !ListPrivacyBudgetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPrivacyBudgetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPrivacyBudgetsOutput {
    var result: ListPrivacyBudgetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrivacyBudgetsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
