...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProtectedJobInput, options: Options) !UpdateProtectedJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProtectedJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProtectedJobOutput {
    var result: UpdateProtectedJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProtectedJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
