...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCollaborationIdNamespaceAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.id_namespace_association_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCollaborationIdNamespaceAssociationOutput {
    var result: GetCollaborationIdNamespaceAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCollaborationIdNamespaceAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
