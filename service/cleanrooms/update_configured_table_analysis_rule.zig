...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfiguredTableAnalysisRuleInput, options: Options) !UpdateConfiguredTableAnalysisRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfiguredTableAnalysisRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.analysis_rule_type);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfiguredTableAnalysisRuleOutput {
    var result: UpdateConfiguredTableAnalysisRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConfiguredTableAnalysisRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
