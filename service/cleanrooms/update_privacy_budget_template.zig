...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePrivacyBudgetTemplateInput, options: CallOptions) !UpdatePrivacyBudgetTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePrivacyBudgetTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePrivacyBudgetTemplateOutput {
    var result: UpdatePrivacyBudgetTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePrivacyBudgetTemplateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
