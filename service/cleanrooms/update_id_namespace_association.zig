...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIdNamespaceAssociationInput, options: Options) !UpdateIdNamespaceAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIdNamespaceAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIdNamespaceAssociationOutput {
    var result: UpdateIdNamespaceAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIdNamespaceAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
