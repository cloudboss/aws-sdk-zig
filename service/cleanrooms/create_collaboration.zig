...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCollaborationInput, options: Options) !CreateCollaborationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCollaborationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allowed_result_regions) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"allowedResultRegions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.analytics_engine) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"analyticsEngine\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.auto_approved_change_request_types) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"autoApprovedChangeRequestTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"creatorDisplayName\":");
    try aws.json.writeValue(@TypeOf(input.creator_display_name), input.creator_display_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"creatorMemberAbilities\":");
    try aws.json.writeValue(@TypeOf(input.creator_member_abilities), input.creator_member_abilities, alloc, &body_buf);
    has_prev = true;
    if (input.creator_ml_member_abilities) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"creatorMLMemberAbilities\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.creator_payment_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"creatorPaymentConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.data_encryption_metadata) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"dataEncryptionMetadata\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.is_metrics_enabled) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"isMetricsEnabled\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"members\":");
    try aws.json.writeValue(@TypeOf(input.members), input.members, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.query_log_status), input.query_log_status, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCollaborationOutput {
    var result: CreateCollaborationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCollaborationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
