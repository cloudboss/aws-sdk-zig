...
pub const UpdateCollaborationInput = struct {
...
pub const UpdateCollaborationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCollaborationInput, options: CallOptions) !UpdateCollaborationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateCollaborationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateCollaborationOutput {
    var result: UpdateCollaborationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCollaborationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
