...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCollaborationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCollaborationOutput {
    var result: UpdateCollaborationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCollaborationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
