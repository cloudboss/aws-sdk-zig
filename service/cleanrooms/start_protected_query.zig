...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartProtectedQueryInput, options: Options) !StartProtectedQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartProtectedQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.compute_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"computeConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.result_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resultConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sqlParameters\":");
    try aws.json.writeValue(@TypeOf(input.sql_parameters), input.sql_parameters, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartProtectedQueryOutput {
    var result: StartProtectedQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartProtectedQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
