...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAnalysisTemplateInput, options: Options) !UpdateAnalysisTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAnalysisTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAnalysisTemplateOutput {
    var result: UpdateAnalysisTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAnalysisTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
