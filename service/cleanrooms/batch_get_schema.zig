...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetSchemaInput, options: Options) !BatchGetSchemaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetSchemaInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batch-schema");
    ...
    try body_buf.appendSlice(alloc, "\"names\":");
    try aws.json.writeValue(@TypeOf(input.names), input.names, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetSchemaOutput {
    var result: BatchGetSchemaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetSchemaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
