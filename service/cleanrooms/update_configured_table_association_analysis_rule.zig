...
pub const UpdateConfiguredTableAssociationAnalysisRuleInput = struct {
...
pub const UpdateConfiguredTableAssociationAnalysisRuleOutput = struct {
    /// The updated analysis rule for the conﬁgured table association. In the
    /// console, the `ConfiguredTableAssociationAnalysisRule` is referred to as the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfiguredTableAssociationAnalysisRuleInput, options: CallOptions) !UpdateConfiguredTableAssociationAnalysisRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateConfiguredTableAssociationAnalysisRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateConfiguredTableAssociationAnalysisRuleOutput {
    var result: UpdateConfiguredTableAssociationAnalysisRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConfiguredTableAssociationAnalysisRuleOutput, body, allocator);
    }
    _ = status;
    _ = headers;
