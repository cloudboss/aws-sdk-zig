...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfiguredTableAssociationAnalysisRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfiguredTableAssociationAnalysisRuleOutput {
    var result: UpdateConfiguredTableAssociationAnalysisRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConfiguredTableAssociationAnalysisRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
