...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAnalysisTemplateInput, options: CallOptions) !GetAnalysisTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAnalysisTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAnalysisTemplateOutput {
    var result: GetAnalysisTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAnalysisTemplateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
