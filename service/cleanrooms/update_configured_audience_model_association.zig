...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfiguredAudienceModelAssociationInput, options: Options) !UpdateConfiguredAudienceModelAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfiguredAudienceModelAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.configured_audience_model_association_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfiguredAudienceModelAssociationOutput {
    var result: UpdateConfiguredAudienceModelAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConfiguredAudienceModelAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
