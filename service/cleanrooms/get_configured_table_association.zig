...
pub const GetConfiguredTableAssociationInput = struct {
...
pub const GetConfiguredTableAssociationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConfiguredTableAssociationInput, options: CallOptions) !GetConfiguredTableAssociationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetConfiguredTableAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetConfiguredTableAssociationOutput {
    var result: GetConfiguredTableAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfiguredTableAssociationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
