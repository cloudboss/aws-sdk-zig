...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfiguredTableAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfiguredTableAssociationOutput {
    var result: GetConfiguredTableAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfiguredTableAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
