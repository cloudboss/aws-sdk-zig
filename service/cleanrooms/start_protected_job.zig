...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartProtectedJobInput, options: Options) !StartProtectedJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartProtectedJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/protectedJobs");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"jobParameters\":");
    try aws.json.writeValue(@TypeOf(input.job_parameters), input.job_parameters, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartProtectedJobOutput {
    var result: StartProtectedJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartProtectedJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
