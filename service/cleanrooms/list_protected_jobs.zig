...
const ProtectedJobSummary = @import("protected_job_summary.zig").ProtectedJobSummary;

pub const ListProtectedJobsInput = struct {
    ...
    status: ?ProtectedJobStatus = null,
...
pub const ListProtectedJobsOutput = struct {
    /// The pagination token that's used to fetch the next set of results.
    next_token: ?[]const u8 = null,

    /// A list of protected job summaries.
    protected_jobs: ?[]const ProtectedJobSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProtectedJobsInput, options: Options) !ListProtectedJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProtectedJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProtectedJobsOutput {
    var result: ListProtectedJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProtectedJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
