...
pub const GetCollaborationChangeRequestInput = struct {
...
pub const GetCollaborationChangeRequestOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCollaborationChangeRequestInput, options: Options) !GetCollaborationChangeRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCollaborationChangeRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.change_request_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCollaborationChangeRequestOutput {
    var result: GetCollaborationChangeRequestOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCollaborationChangeRequestOutput, body, alloc);
    }
    _ = status;
    _ = headers;
