...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCollaborationChangeRequestInput, options: CallOptions) !GetCollaborationChangeRequestOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCollaborationChangeRequestInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCollaborationChangeRequestOutput {
    var result: GetCollaborationChangeRequestOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCollaborationChangeRequestOutput, body, allocator);
    }
    _ = status;
    _ = headers;
