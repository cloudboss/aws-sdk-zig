...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStatementResultV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftData.GetStatementResultV2");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStatementResultV2Output {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetStatementResultV2Output, body, alloc);
}
