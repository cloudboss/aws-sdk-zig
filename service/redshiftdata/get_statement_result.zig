...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStatementResultInput, options: CallOptions) !GetStatementResultOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetStatementResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RedshiftData.GetStatementResult");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetStatementResultOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetStatementResultOutput, body, allocator);
}
