...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStatementInput, options: Options) !DescribeStatementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStatementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RedshiftData.DescribeStatement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStatementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeStatementOutput, body, alloc);
}
