...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPermissionSetsInput, options: Options) !ListPermissionSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPermissionSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.ListPermissionSets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPermissionSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPermissionSetsOutput, body, alloc);
}
