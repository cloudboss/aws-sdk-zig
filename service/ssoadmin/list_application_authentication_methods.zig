...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApplicationAuthenticationMethodsInput, options: Options) !ListApplicationAuthenticationMethodsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApplicationAuthenticationMethodsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.ListApplicationAuthenticationMethods");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApplicationAuthenticationMethodsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListApplicationAuthenticationMethodsOutput, body, alloc);
}
