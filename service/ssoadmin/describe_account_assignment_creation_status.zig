...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountAssignmentCreationStatusInput, options: Options) !DescribeAccountAssignmentCreationStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountAssignmentCreationStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.DescribeAccountAssignmentCreationStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountAssignmentCreationStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAccountAssignmentCreationStatusOutput, body, alloc);
}
