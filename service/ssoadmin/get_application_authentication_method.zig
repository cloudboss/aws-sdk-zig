...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApplicationAuthenticationMethodInput, options: Options) !GetApplicationAuthenticationMethodOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApplicationAuthenticationMethodInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.GetApplicationAuthenticationMethod");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApplicationAuthenticationMethodOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetApplicationAuthenticationMethodOutput, body, alloc);
}
