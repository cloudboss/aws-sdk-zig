...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeApplicationProviderInput, options: Options) !DescribeApplicationProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeApplicationProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.DescribeApplicationProvider");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeApplicationProviderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeApplicationProviderOutput, body, alloc);
}
