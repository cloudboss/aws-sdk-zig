...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccountAssignmentInput, options: Options) !CreateAccountAssignmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccountAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.CreateAccountAssignment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccountAssignmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAccountAssignmentOutput, body, alloc);
}
