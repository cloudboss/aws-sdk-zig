...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAccountAssignmentInput, options: Options) !DeleteAccountAssignmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAccountAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.DeleteAccountAssignment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAccountAssignmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteAccountAssignmentOutput, body, alloc);
}
