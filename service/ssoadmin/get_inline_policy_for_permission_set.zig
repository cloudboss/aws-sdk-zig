...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInlinePolicyForPermissionSetInput, options: Options) !GetInlinePolicyForPermissionSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInlinePolicyForPermissionSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.GetInlinePolicyForPermissionSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInlinePolicyForPermissionSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInlinePolicyForPermissionSetOutput, body, alloc);
}
