...
const ApplicationAssignment = @import("application_assignment.zig").ApplicationAssignment;

pub const ListApplicationAssignmentsInput = struct {
...
pub const ListApplicationAssignmentsOutput = struct {
    /// The list of users assigned to an application.
    application_assignments: ?[]const ApplicationAssignment = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApplicationAssignmentsInput, options: CallOptions) !ListApplicationAssignmentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListApplicationAssignmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SWBExternalService.ListApplicationAssignments");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListApplicationAssignmentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListApplicationAssignmentsOutput, body, allocator);
}
