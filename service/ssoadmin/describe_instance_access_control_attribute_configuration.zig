...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceAccessControlAttributeConfigurationInput, options: Options) !DescribeInstanceAccessControlAttributeConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceAccessControlAttributeConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.DescribeInstanceAccessControlAttributeConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceAccessControlAttributeConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstanceAccessControlAttributeConfigurationOutput, body, alloc);
}
