...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountAssignmentCreationStatusInput, options: Options) !ListAccountAssignmentCreationStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountAssignmentCreationStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.ListAccountAssignmentCreationStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountAssignmentCreationStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAccountAssignmentCreationStatusOutput, body, alloc);
}
