...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePermissionSetInput, options: Options) !CreatePermissionSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePermissionSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.CreatePermissionSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePermissionSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePermissionSetOutput, body, alloc);
}
