...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApplicationAccessScopesInput, options: CallOptions) !ListApplicationAccessScopesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListApplicationAccessScopesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SWBExternalService.ListApplicationAccessScopes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListApplicationAccessScopesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListApplicationAccessScopesOutput, body, allocator);
}
