...
pub const ListApplicationProvidersInput = struct {
...
pub const ListApplicationProvidersOutput = struct {
    /// An array list of structures that describe application providers.
    application_providers: ?[]const ApplicationProvider = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApplicationProvidersInput, options: CallOptions) !ListApplicationProvidersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListApplicationProvidersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SWBExternalService.ListApplicationProviders");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListApplicationProvidersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListApplicationProvidersOutput, body, allocator);
}
