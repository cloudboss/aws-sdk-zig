...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrustedTokenIssuerInput, options: Options) !CreateTrustedTokenIssuerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrustedTokenIssuerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.CreateTrustedTokenIssuer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrustedTokenIssuerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTrustedTokenIssuerOutput, body, alloc);
}
