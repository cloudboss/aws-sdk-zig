...
const ListAccountAssignmentsFilter = @import("list_account_assignments_filter.zig").ListAccountAssignmentsFilter;
const PrincipalType = @import("principal_type.zig").PrincipalType;
const AccountAssignmentForPrincipal = @import("account_assignment_for_principal.zig").AccountAssignmentForPrincipal;

pub const ListAccountAssignmentsForPrincipalInput = struct {
    /// Specifies an Amazon Web Services account ID number. Results are filtered to
    /// only those that match this ID number.
    filter: ?ListAccountAssignmentsFilter = null,
...
pub const ListAccountAssignmentsForPrincipalOutput = struct {
    /// An array list of the account assignments for the principal.
    account_assignments: ?[]const AccountAssignmentForPrincipal = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountAssignmentsForPrincipalInput, options: CallOptions) !ListAccountAssignmentsForPrincipalOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "sso");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAccountAssignmentsForPrincipalInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SWBExternalService.ListAccountAssignmentsForPrincipal");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAccountAssignmentsForPrincipalOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAccountAssignmentsForPrincipalOutput, body, allocator);
}
