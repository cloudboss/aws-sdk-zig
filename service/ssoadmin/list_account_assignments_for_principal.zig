...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountAssignmentsForPrincipalInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.ListAccountAssignmentsForPrincipal");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountAssignmentsForPrincipalOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAccountAssignmentsForPrincipalOutput, body, alloc);
}
