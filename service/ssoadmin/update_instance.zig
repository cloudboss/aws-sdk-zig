...
pub const UpdateInstanceInput = struct {
    ...
    /// instance. You can use this to configure customer managed KMS keys or Amazon
    /// Web Services owned KMS keys for encrypting your instance data.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateInstanceInput, options: Options) !UpdateInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.UpdateInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateInstanceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
