...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ProvisionPermissionSetInput, options: Options) !ProvisionPermissionSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ProvisionPermissionSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.ProvisionPermissionSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ProvisionPermissionSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ProvisionPermissionSetOutput, body, alloc);
}
