...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApplicationAssignmentsForPrincipalInput, options: Options) !ListApplicationAssignmentsForPrincipalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApplicationAssignmentsForPrincipalInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.ListApplicationAssignmentsForPrincipal");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApplicationAssignmentsForPrincipalOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListApplicationAssignmentsForPrincipalOutput, body, alloc);
}
