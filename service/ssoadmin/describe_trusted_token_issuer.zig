...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustedTokenIssuerInput, options: Options) !DescribeTrustedTokenIssuerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrustedTokenIssuerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.DescribeTrustedTokenIssuer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrustedTokenIssuerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTrustedTokenIssuerOutput, body, alloc);
}
