...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutApplicationSessionConfigurationInput, options: Options) !PutApplicationSessionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutApplicationSessionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.PutApplicationSessionConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutApplicationSessionConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
