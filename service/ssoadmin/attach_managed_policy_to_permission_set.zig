...
pub const AttachManagedPolicyToPermissionSetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachManagedPolicyToPermissionSetInput, options: Options) !AttachManagedPolicyToPermissionSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AttachManagedPolicyToPermissionSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SWBExternalService.AttachManagedPolicyToPermissionSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AttachManagedPolicyToPermissionSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
