...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRegionInput, options: Options) !DescribeRegionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRegionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.DescribeRegion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRegionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRegionOutput, body, alloc);
}
