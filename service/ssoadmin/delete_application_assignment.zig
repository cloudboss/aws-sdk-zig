...
pub const DeleteApplicationAssignmentInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteApplicationAssignmentInput, options: Options) !DeleteApplicationAssignmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteApplicationAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.DeleteApplicationAssignment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteApplicationAssignmentOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
