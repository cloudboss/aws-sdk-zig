...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteApplicationAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SWBExternalService.DeleteApplicationAssignment");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteApplicationAssignmentOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
