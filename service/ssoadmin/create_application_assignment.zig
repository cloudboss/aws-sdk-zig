...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateApplicationAssignmentInput, options: Options) !CreateApplicationAssignmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApplicationAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.CreateApplicationAssignment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateApplicationAssignmentOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
