...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrustedTokenIssuersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.ListTrustedTokenIssuers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrustedTokenIssuersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTrustedTokenIssuersOutput, body, alloc);
}
