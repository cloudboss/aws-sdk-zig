...
const TrustedTokenIssuerMetadata = @import("trusted_token_issuer_metadata.zig").TrustedTokenIssuerMetadata;

pub const ListTrustedTokenIssuersInput = struct {
...
pub const ListTrustedTokenIssuersOutput = struct {
    ...
    trusted_token_issuers: ?[]const TrustedTokenIssuerMetadata = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .trusted_token_issuers = "TrustedTokenIssuers",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrustedTokenIssuersInput, options: CallOptions) !ListTrustedTokenIssuersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTrustedTokenIssuersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SWBExternalService.ListTrustedTokenIssuers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTrustedTokenIssuersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTrustedTokenIssuersOutput, body, allocator);
}
