...
pub const UpdatePermissionSetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePermissionSetInput, options: Options) !UpdatePermissionSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePermissionSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SWBExternalService.UpdatePermissionSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePermissionSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
