...
pub const DeletePermissionSetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePermissionSetInput, options: Options) !DeletePermissionSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePermissionSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SWBExternalService.DeletePermissionSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePermissionSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
