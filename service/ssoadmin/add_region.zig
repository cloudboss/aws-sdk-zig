...
pub const AddRegionInput = struct {
    ...
    /// instance. The Region name must be 1-32 characters long and follow the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddRegionInput, options: Options) !AddRegionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddRegionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.AddRegion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddRegionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AddRegionOutput, body, alloc);
}
