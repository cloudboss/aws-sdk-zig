...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceInput, options: Options) !DescribeInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.DescribeInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstanceOutput, body, alloc);
}
