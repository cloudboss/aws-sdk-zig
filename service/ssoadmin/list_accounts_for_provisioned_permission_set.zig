...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountsForProvisionedPermissionSetInput, options: Options) !ListAccountsForProvisionedPermissionSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountsForProvisionedPermissionSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SWBExternalService.ListAccountsForProvisionedPermissionSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountsForProvisionedPermissionSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAccountsForProvisionedPermissionSetOutput, body, alloc);
}
