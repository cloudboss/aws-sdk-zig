...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSupplementalTaxRegistrationInput, options: Options) !PutSupplementalTaxRegistrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutSupplementalTaxRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"taxRegistrationEntry\":");
    try aws.json.writeValue(@TypeOf(input.tax_registration_entry), input.tax_registration_entry, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutSupplementalTaxRegistrationOutput {
    var result: PutSupplementalTaxRegistrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutSupplementalTaxRegistrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
