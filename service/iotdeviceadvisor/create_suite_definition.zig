...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSuiteDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"suiteDefinitionConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.suite_definition_configuration), input.suite_definition_configuration, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSuiteDefinitionOutput {
    var result: CreateSuiteDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSuiteDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
