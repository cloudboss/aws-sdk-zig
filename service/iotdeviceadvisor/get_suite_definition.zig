...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSuiteDefinitionInput, options: Options) !GetSuiteDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSuiteDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSuiteDefinitionOutput {
    var result: GetSuiteDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSuiteDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
