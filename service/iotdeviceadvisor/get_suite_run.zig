...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSuiteRunInput, options: Options) !GetSuiteRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSuiteRunInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSuiteRunOutput {
    var result: GetSuiteRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSuiteRunOutput, body, alloc);
    }
    _ = status;
    _ = headers;
