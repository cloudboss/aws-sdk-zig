...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReplicationSetInput, options: Options) !GetReplicationSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReplicationSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReplicationSetOutput {
    var result: GetReplicationSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReplicationSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
