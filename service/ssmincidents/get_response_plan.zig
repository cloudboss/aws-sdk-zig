...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResponsePlanInput, options: Options) !GetResponsePlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResponsePlanInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResponsePlanOutput {
    var result: GetResponsePlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResponsePlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
