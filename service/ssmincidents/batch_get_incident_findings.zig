...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetIncidentFindingsInput, options: Options) !BatchGetIncidentFindingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetIncidentFindingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetIncidentFindingsOutput {
    var result: BatchGetIncidentFindingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetIncidentFindingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
