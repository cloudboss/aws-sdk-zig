...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTimelineEventsInput, options: Options) !ListTimelineEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTimelineEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTimelineEventsOutput {
    var result: ListTimelineEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTimelineEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
