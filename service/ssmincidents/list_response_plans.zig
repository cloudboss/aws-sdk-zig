...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResponsePlansInput, options: Options) !ListResponsePlansOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResponsePlansInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResponsePlansOutput {
    var result: ListResponsePlansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResponsePlansOutput, body, alloc);
    }
    _ = status;
    _ = headers;
