...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartIncidentInput, options: Options) !StartIncidentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartIncidentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.related_items) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"relatedItems\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"responsePlanArn\":");
    try aws.json.writeValue(@TypeOf(input.response_plan_arn), input.response_plan_arn, alloc, &body_buf);
    has_prev = true;
    if (input.title) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.trigger_details) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"triggerDetails\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartIncidentOutput {
    var result: StartIncidentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartIncidentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
