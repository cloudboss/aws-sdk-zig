...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReplicationSetInput, options: Options) !CreateReplicationSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReplicationSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReplicationSetOutput {
    var result: CreateReplicationSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateReplicationSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
