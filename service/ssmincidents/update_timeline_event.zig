...
pub const UpdateTimelineEventInput = struct {
    ...
    /// Amazon Web Services resource involved or associated with the incident. To
    /// specify a reference, enter
    ...
    /// resource, use its ARN. You can also specify an Amazon CloudWatch metric
    /// associated with
    /// the DynamoDB table as a related item.
    ///
    /// This update action overrides all existing references. If you want to keep
    /// existing
    /// references, you must specify them in the call. If you don't, this action
    /// removes any
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTimelineEventInput, options: Options) !UpdateTimelineEventOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTimelineEventInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/updateTimelineEvent";
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"eventId\":");
    try aws.json.writeValue(@TypeOf(input.event_id), input.event_id, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTimelineEventOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateTimelineEventOutput = .{};
