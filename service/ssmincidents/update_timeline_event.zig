...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTimelineEventInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"eventId\":");
    try aws.json.writeValue(@TypeOf(input.event_id), input.event_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTimelineEventOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
