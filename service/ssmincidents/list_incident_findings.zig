...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIncidentFindingsInput, options: Options) !ListIncidentFindingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIncidentFindingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIncidentFindingsOutput {
    var result: ListIncidentFindingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIncidentFindingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
