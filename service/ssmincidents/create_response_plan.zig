...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResponsePlanInput, options: Options) !CreateResponsePlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResponsePlanInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"incidentTemplate\":");
    try aws.json.writeValue(@TypeOf(input.incident_template), input.incident_template, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResponsePlanOutput {
    var result: CreateResponsePlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateResponsePlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
