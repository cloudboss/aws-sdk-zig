...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRelatedItemsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRelatedItemsOutput {
    var result: ListRelatedItemsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRelatedItemsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
