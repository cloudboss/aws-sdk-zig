...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIncidentRecordInput, options: Options) !GetIncidentRecordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIncidentRecordInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIncidentRecordOutput {
    var result: GetIncidentRecordOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIncidentRecordOutput, body, alloc);
    }
    _ = status;
    _ = headers;
