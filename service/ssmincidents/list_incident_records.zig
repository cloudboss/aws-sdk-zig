...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIncidentRecordsInput, options: Options) !ListIncidentRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIncidentRecordsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIncidentRecordsOutput {
    var result: ListIncidentRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIncidentRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
