...
pub const GetQueryStatusInput = struct {
...
pub const GetQueryStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQueryStatusInput, options: CallOptions) !GetQueryStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetQueryStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetQueryStatusOutput {
    var result: GetQueryStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQueryStatusOutput, body, allocator);
    }
    _ = status;
    _ = headers;
