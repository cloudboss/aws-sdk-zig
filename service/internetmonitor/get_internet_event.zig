...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInternetEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v20210603/InternetEvents/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInternetEventOutput {
    var result: GetInternetEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInternetEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
