...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHealthEventsInput, options: Options) !ListHealthEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHealthEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHealthEventsOutput {
    var result: ListHealthEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListHealthEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
