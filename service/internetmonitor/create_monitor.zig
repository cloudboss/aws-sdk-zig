...
pub const CreateMonitorInput = struct {
    ...
    /// Monitor creates a
    /// health event when an internet issue that affects your application end users
    /// has a health score percentage that is at or below a
    ...
    /// Gateway attached to it, to make sure that it has
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMonitorInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.health_events_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"HealthEventsConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.internet_measurements_log_delivery) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InternetMeasurementsLogDelivery\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.max_city_networks_to_monitor) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MaxCityNetworksToMonitor\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.traffic_percentage_to_monitor) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TrafficPercentageToMonitor\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
