...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartQueryInput, options: Options) !StartQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/Queries");
    ...
    if (input.filter_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FilterParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.linked_account_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LinkedAccountId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"QueryType\":");
    try aws.json.writeValue(@TypeOf(input.query_type), input.query_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartQueryOutput {
    var result: StartQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
