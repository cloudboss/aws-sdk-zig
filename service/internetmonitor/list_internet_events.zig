...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInternetEventsInput, options: Options) !ListInternetEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInternetEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.event_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "EventStatus=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.event_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "EventType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.max_results) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "InternetEventMaxResults=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInternetEventsOutput {
    var result: ListInternetEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInternetEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
