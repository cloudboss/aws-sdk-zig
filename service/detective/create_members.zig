...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMembersInput, options: Options) !CreateMembersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"Accounts\":");
    try aws.json.writeValue(@TypeOf(input.accounts), input.accounts, alloc, &body_buf);
    has_prev = true;
    if (input.disable_email_notification) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DisableEmailNotification\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMembersOutput {
    var result: CreateMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
