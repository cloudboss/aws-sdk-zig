...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInvestigationInput, options: Options) !GetInvestigationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInvestigationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInvestigationOutput {
    var result: GetInvestigationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInvestigationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
