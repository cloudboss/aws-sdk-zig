...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMembersInput, options: Options) !DeleteMembersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMembersOutput {
    var result: DeleteMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
