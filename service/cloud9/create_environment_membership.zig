...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEnvironmentMembershipInput, options: Options) !CreateEnvironmentMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEnvironmentMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCloud9WorkspaceManagementService.CreateEnvironmentMembership");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEnvironmentMembershipOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateEnvironmentMembershipOutput, body, alloc);
}
