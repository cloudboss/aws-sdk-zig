...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEnvironmentMembershipsInput, options: Options) !DescribeEnvironmentMembershipsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEnvironmentMembershipsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("cloud9", "Cloud9", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCloud9WorkspaceManagementService.DescribeEnvironmentMemberships");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEnvironmentMembershipsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEnvironmentMembershipsOutput, body, alloc);
}
