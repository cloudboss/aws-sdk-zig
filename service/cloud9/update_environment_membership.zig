...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEnvironmentMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCloud9WorkspaceManagementService.UpdateEnvironmentMembership");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEnvironmentMembershipOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateEnvironmentMembershipOutput, body, alloc);
}
