...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEnvironmentEC2Input, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCloud9WorkspaceManagementService.CreateEnvironmentEC2");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEnvironmentEC2Output {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEnvironmentEC2Output, body, alloc);
}
