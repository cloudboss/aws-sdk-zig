...
pub const DeleteEnvironmentMembershipInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteEnvironmentMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCloud9WorkspaceManagementService.DeleteEnvironmentMembership");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteEnvironmentMembershipOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "ConcurrentAccessException")) {
        const parsed_error: ?errors.ConcurrentAccessException = aws.json.parseJsonObject(errors.ConcurrentAccessException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
    ...
    if (std.mem.eql(u8, error_code, "InternalServerErrorException")) {
        const parsed_error: ?errors.InternalServerErrorException = aws.json.parseJsonObject(errors.InternalServerErrorException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
