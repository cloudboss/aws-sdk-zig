...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateResolverRuleInput, options: Options) !AssociateResolverRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateResolverRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.AssociateResolverRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateResolverRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateResolverRuleOutput, body, alloc);
}
