...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResolverEndpointInput, options: Options) !UpdateResolverEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResolverEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.UpdateResolverEndpoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResolverEndpointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateResolverEndpointOutput, body, alloc);
}
