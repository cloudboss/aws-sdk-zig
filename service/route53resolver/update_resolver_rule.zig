...
const ResolverRuleConfig = @import("resolver_rule_config.zig").ResolverRuleConfig;
const ResolverRule = @import("resolver_rule.zig").ResolverRule;

pub const UpdateResolverRuleInput = struct {
...
pub const UpdateResolverRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResolverRuleInput, options: CallOptions) !UpdateResolverRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateResolverRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.UpdateResolverRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateResolverRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateResolverRuleOutput, body, allocator);
}
