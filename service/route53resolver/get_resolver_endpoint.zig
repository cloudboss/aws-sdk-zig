...
pub const GetResolverEndpointInput = struct {
...
pub const GetResolverEndpointOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResolverEndpointInput, options: CallOptions) !GetResolverEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResolverEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.GetResolverEndpoint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResolverEndpointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResolverEndpointOutput, body, allocator);
}
