...
pub const UpdateOutpostResolverInput = struct {
...
pub const UpdateOutpostResolverOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOutpostResolverInput, options: CallOptions) !UpdateOutpostResolverOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateOutpostResolverInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.UpdateOutpostResolver");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateOutpostResolverOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateOutpostResolverOutput, body, allocator);
}
