...
pub const ListResolverEndpointsInput = struct {
    ...
    /// response to a `ListResolverEndpoints` request.
    ...
    /// another `ListResolverEndpoints` request
    /// to get the next group of Resolver endpoints. In the next request, specify
...
pub const ListResolverEndpointsOutput = struct {
    /// The value that you specified for `MaxResults` in the request.
    max_results: ?i32 = null,

    /// If more than `MaxResults` IP addresses match the specified criteria, you can
    /// submit another `ListResolverEndpoint` request
    ...
    resolver_endpoints: ?[]const ResolverEndpoint = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResolverEndpointsInput, options: CallOptions) !ListResolverEndpointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListResolverEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.ListResolverEndpoints");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListResolverEndpointsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResolverEndpointsOutput, body, allocator);
}
