...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOutpostResolverInput, options: Options) !GetOutpostResolverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOutpostResolverInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.GetOutpostResolver");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOutpostResolverOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOutpostResolverOutput, body, alloc);
}
