...
const FirewallRuleGroupMetadata = @import("firewall_rule_group_metadata.zig").FirewallRuleGroupMetadata;

pub const ListFirewallRuleGroupsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFirewallRuleGroupsInput, options: CallOptions) !ListFirewallRuleGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFirewallRuleGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.ListFirewallRuleGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFirewallRuleGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFirewallRuleGroupsOutput, body, allocator);
}
