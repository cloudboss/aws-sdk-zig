...
pub const ListOutpostResolversInput = struct {
...
pub const ListOutpostResolversOutput = struct {
    /// If more than `MaxResults` Resolvers match the specified criteria, you can
    /// submit another
    /// `ListOutpostResolver` request to get the next group of results. In the next
    ...
    outpost_resolvers: ?[]const OutpostResolver = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOutpostResolversInput, options: CallOptions) !ListOutpostResolversOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOutpostResolversInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.ListOutpostResolvers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOutpostResolversOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOutpostResolversOutput, body, allocator);
}
