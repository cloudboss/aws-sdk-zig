...
pub const DeleteOutpostResolverInput = struct {
...
pub const DeleteOutpostResolverOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOutpostResolverInput, options: CallOptions) !DeleteOutpostResolverOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteOutpostResolverInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.DeleteOutpostResolver");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteOutpostResolverOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteOutpostResolverOutput, body, allocator);
}
