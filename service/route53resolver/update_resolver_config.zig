...
const AutodefinedReverseFlag = @import("autodefined_reverse_flag.zig").AutodefinedReverseFlag;
const ResolverConfig = @import("resolver_config.zig").ResolverConfig;

pub const UpdateResolverConfigInput = struct {
    /// Indicates whether or not the Resolver will create autodefined rules for
    /// reverse DNS
    /// lookups. This is enabled by default. Disabling this option will also affect
    /// EC2-Classic
    /// instances using ClassicLink. For more information, see
    /// [ClassicLink](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/vpc-classiclink.html) in the
    /// *Amazon EC2 guide*.
    ///
    ...
    /// in the
    /// *Amazon EC2 guide* and the blog [EC2-Classic Networking is Retiring – Here’s
    /// How to
...
pub const UpdateResolverConfigOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResolverConfigInput, options: CallOptions) !UpdateResolverConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateResolverConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.UpdateResolverConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateResolverConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateResolverConfigOutput, body, allocator);
}
