...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResolverRuleAssociationsInput, options: Options) !ListResolverRuleAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResolverRuleAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.ListResolverRuleAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResolverRuleAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResolverRuleAssociationsOutput, body, alloc);
}
