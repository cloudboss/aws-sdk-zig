...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateFirewallRuleGroupInput, options: Options) !AssociateFirewallRuleGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateFirewallRuleGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.AssociateFirewallRuleGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateFirewallRuleGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateFirewallRuleGroupOutput, body, alloc);
}
