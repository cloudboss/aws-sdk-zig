...
pub const GetResolverRulePolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResolverRulePolicyInput, options: CallOptions) !GetResolverRulePolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResolverRulePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.GetResolverRulePolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResolverRulePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResolverRulePolicyOutput, body, allocator);
}
