...
pub const GetFirewallRuleGroupInput = struct {
...
pub const GetFirewallRuleGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFirewallRuleGroupInput, options: CallOptions) !GetFirewallRuleGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFirewallRuleGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.GetFirewallRuleGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFirewallRuleGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetFirewallRuleGroupOutput, body, allocator);
}
