...
pub const DeleteResolverRuleInput = struct {
...
pub const DeleteResolverRuleOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteResolverRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.DeleteResolverRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteResolverRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteResolverRuleOutput, body, allocator);
}
