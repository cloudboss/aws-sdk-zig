...
pub const UpdateFirewallRuleGroupAssociationInput = struct {
    /// The identifier of the FirewallRuleGroupAssociation.
    firewall_rule_group_association_id: []const u8,
    ...
    /// numbers, for example, use 100, 200, and so on. You
...
pub const UpdateFirewallRuleGroupAssociationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFirewallRuleGroupAssociationInput, options: CallOptions) !UpdateFirewallRuleGroupAssociationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFirewallRuleGroupAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.UpdateFirewallRuleGroupAssociation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFirewallRuleGroupAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFirewallRuleGroupAssociationOutput, body, allocator);
}
