...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFirewallRuleGroupAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.UpdateFirewallRuleGroupAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFirewallRuleGroupAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFirewallRuleGroupAssociationOutput, body, alloc);
}
