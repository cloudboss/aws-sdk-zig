...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResolverQueryLogConfigInput, options: Options) !CreateResolverQueryLogConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResolverQueryLogConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.CreateResolverQueryLogConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResolverQueryLogConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateResolverQueryLogConfigOutput, body, alloc);
}
