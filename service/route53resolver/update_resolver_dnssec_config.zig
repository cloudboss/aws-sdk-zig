...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResolverDnssecConfigInput, options: Options) !UpdateResolverDnssecConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResolverDnssecConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.UpdateResolverDnssecConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResolverDnssecConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateResolverDnssecConfigOutput, body, alloc);
}
