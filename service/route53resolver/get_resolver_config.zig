...
pub const GetResolverConfigInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResolverConfigInput, options: CallOptions) !GetResolverConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResolverConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.GetResolverConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResolverConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResolverConfigOutput, body, allocator);
}
