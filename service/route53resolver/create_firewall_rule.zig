...
const CreateFirewallRuleInput = @import("create_firewall_rule_request.zig").CreateFirewallRuleRequest;

const CreateFirewallRuleOutput = @import("create_firewall_rule_response.zig").CreateFirewallRuleResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFirewallRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.CreateFirewallRule");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFirewallRuleOutput {
    ...
    return aws.json.parseJsonObject(CreateFirewallRuleOutput, body, alloc);
}
