...
const IpAddressUpdate = @import("ip_address_update.zig").IpAddressUpdate;
const ResolverEndpoint = @import("resolver_endpoint.zig").ResolverEndpoint;

pub const AssociateResolverEndpointIpAddressInput = struct {
    /// Either the IPv4 address that you want to add to a Resolver endpoint or a
    /// subnet ID. If you specify a subnet ID,
    /// Resolver chooses an IP address for you from the available IPs in the
    /// specified subnet.
...
pub const AssociateResolverEndpointIpAddressOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateResolverEndpointIpAddressInput, options: CallOptions) !AssociateResolverEndpointIpAddressOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateResolverEndpointIpAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.AssociateResolverEndpointIpAddress");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateResolverEndpointIpAddressOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateResolverEndpointIpAddressOutput, body, allocator);
}
