...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFirewallRuleGroupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.GetFirewallRuleGroupPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFirewallRuleGroupPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetFirewallRuleGroupPolicyOutput, body, alloc);
}
