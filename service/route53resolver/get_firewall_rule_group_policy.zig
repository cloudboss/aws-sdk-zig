...
pub const GetFirewallRuleGroupPolicyInput = struct {
...
pub const GetFirewallRuleGroupPolicyOutput = struct {
    ...
    /// Manager
    /// (RAM).
    firewall_rule_group_policy: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFirewallRuleGroupPolicyInput, options: CallOptions) !GetFirewallRuleGroupPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFirewallRuleGroupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.GetFirewallRuleGroupPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFirewallRuleGroupPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetFirewallRuleGroupPolicyOutput, body, allocator);
}
