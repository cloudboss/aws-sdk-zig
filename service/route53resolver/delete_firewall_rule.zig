...
const DeleteFirewallRuleInput = @import("delete_firewall_rule_request.zig").DeleteFirewallRuleRequest;

const DeleteFirewallRuleOutput = @import("delete_firewall_rule_response.zig").DeleteFirewallRuleResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteFirewallRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.DeleteFirewallRule");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteFirewallRuleOutput {
    ...
    return aws.json.parseJsonObject(DeleteFirewallRuleOutput, body, alloc);
}
