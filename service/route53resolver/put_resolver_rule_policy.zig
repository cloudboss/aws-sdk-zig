...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutResolverRulePolicyInput, options: Options) !PutResolverRulePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutResolverRulePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.PutResolverRulePolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutResolverRulePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutResolverRulePolicyOutput, body, alloc);
}
