...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResolverRulesInput, options: Options) !ListResolverRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResolverRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.ListResolverRules");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResolverRulesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResolverRulesOutput, body, alloc);
}
