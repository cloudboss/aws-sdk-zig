...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateResolverQueryLogConfigInput, options: Options) !AssociateResolverQueryLogConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateResolverQueryLogConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.AssociateResolverQueryLogConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateResolverQueryLogConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateResolverQueryLogConfigOutput, body, alloc);
}
