...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOutpostResolverInput, options: Options) !CreateOutpostResolverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOutpostResolverInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.CreateOutpostResolver");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOutpostResolverOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateOutpostResolverOutput, body, alloc);
}
