...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFirewallConfigInput, options: Options) !UpdateFirewallConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFirewallConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.UpdateFirewallConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFirewallConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFirewallConfigOutput, body, alloc);
}
