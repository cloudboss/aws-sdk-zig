...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResolverQueryLogConfigInput, options: Options) !GetResolverQueryLogConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResolverQueryLogConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.GetResolverQueryLogConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResolverQueryLogConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResolverQueryLogConfigOutput, body, alloc);
}
