...
const ListFirewallRulesInput = @import("list_firewall_rules_request.zig").ListFirewallRulesRequest;

const ListFirewallRulesOutput = @import("list_firewall_rules_response.zig").ListFirewallRulesResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFirewallRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.ListFirewallRules");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFirewallRulesOutput {
    ...
    return aws.json.parseJsonObject(ListFirewallRulesOutput, body, alloc);
}
