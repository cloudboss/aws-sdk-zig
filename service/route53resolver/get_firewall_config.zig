...
pub const GetFirewallConfigInput = struct {
...
pub const GetFirewallConfigOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFirewallConfigInput, options: CallOptions) !GetFirewallConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFirewallConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.GetFirewallConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFirewallConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetFirewallConfigOutput, body, allocator);
}
