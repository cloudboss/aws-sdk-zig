...
const ImportFirewallDomainsInput = @import("import_firewall_domains_request.zig").ImportFirewallDomainsRequest;

const ImportFirewallDomainsOutput = @import("import_firewall_domains_response.zig").ImportFirewallDomainsResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportFirewallDomainsInput, options: Options) !ImportFirewallDomainsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportFirewallDomainsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.ImportFirewallDomains");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportFirewallDomainsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ImportFirewallDomainsOutput, body, alloc);
}
