...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutFirewallRuleGroupPolicyInput, options: CallOptions) !PutFirewallRuleGroupPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutFirewallRuleGroupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.PutFirewallRuleGroupPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutFirewallRuleGroupPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutFirewallRuleGroupPolicyOutput, body, allocator);
}
