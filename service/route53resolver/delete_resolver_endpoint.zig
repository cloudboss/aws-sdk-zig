...
pub const DeleteResolverEndpointInput = struct {
...
pub const DeleteResolverEndpointOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResolverEndpointInput, options: CallOptions) !DeleteResolverEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteResolverEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.DeleteResolverEndpoint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteResolverEndpointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteResolverEndpointOutput, body, allocator);
}
