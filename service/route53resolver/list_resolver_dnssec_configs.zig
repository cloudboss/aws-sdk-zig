...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResolverDnssecConfigsInput, options: Options) !ListResolverDnssecConfigsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResolverDnssecConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.ListResolverDnssecConfigs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResolverDnssecConfigsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResolverDnssecConfigsOutput, body, alloc);
}
