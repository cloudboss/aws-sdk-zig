...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutResolverQueryLogConfigPolicyInput, options: Options) !PutResolverQueryLogConfigPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutResolverQueryLogConfigPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.PutResolverQueryLogConfigPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutResolverQueryLogConfigPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutResolverQueryLogConfigPolicyOutput, body, alloc);
}
