...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResolverQueryLogConfigAssociationsInput, options: Options) !ListResolverQueryLogConfigAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResolverQueryLogConfigAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.ListResolverQueryLogConfigAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResolverQueryLogConfigAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResolverQueryLogConfigAssociationsOutput, body, alloc);
}
