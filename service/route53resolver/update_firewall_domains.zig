...
const UpdateFirewallDomainsInput = @import("update_firewall_domains_request.zig").UpdateFirewallDomainsRequest;

const UpdateFirewallDomainsOutput = @import("update_firewall_domains_response.zig").UpdateFirewallDomainsResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFirewallDomainsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.UpdateFirewallDomains");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFirewallDomainsOutput {
    ...
    return aws.json.parseJsonObject(UpdateFirewallDomainsOutput, body, alloc);
}
