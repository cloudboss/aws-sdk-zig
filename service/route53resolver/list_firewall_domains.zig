...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFirewallDomainsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.ListFirewallDomains");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFirewallDomainsOutput {
    ...
    return aws.json.parseJsonObject(ListFirewallDomainsOutput, body, allocator);
}
