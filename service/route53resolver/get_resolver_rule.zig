...
pub const GetResolverRuleInput = struct {
...
pub const GetResolverRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResolverRuleInput, options: CallOptions) !GetResolverRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResolverRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.GetResolverRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResolverRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResolverRuleOutput, body, allocator);
}
