...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResolverRuleAssociationInput, options: Options) !GetResolverRuleAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResolverRuleAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.GetResolverRuleAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResolverRuleAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResolverRuleAssociationOutput, body, alloc);
}
