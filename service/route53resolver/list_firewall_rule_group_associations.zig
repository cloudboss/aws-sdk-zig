...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFirewallRuleGroupAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.ListFirewallRuleGroupAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFirewallRuleGroupAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFirewallRuleGroupAssociationsOutput, body, alloc);
}
