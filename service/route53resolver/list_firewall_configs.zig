...
pub const ListFirewallConfigsInput = struct {
...
pub const ListFirewallConfigsOutput = struct {
    /// The configurations for the firewall behavior provided by DNS Firewall for
    /// VPCs from
    /// Amazon Virtual Private Cloud (Amazon VPC).
    firewall_configs: ?[]const FirewallConfig = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFirewallConfigsInput, options: CallOptions) !ListFirewallConfigsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFirewallConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.ListFirewallConfigs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFirewallConfigsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFirewallConfigsOutput, body, allocator);
}
