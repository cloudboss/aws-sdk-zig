...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFirewallRuleGroupInput, options: Options) !CreateFirewallRuleGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFirewallRuleGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.CreateFirewallRuleGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFirewallRuleGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateFirewallRuleGroupOutput, body, alloc);
}
