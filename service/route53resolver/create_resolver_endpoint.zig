...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResolverEndpointInput, options: Options) !CreateResolverEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResolverEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Resolver.CreateResolverEndpoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResolverEndpointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateResolverEndpointOutput, body, alloc);
}
