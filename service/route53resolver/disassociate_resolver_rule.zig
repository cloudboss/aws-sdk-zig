...
pub const DisassociateResolverRuleInput = struct {
...
pub const DisassociateResolverRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateResolverRuleInput, options: CallOptions) !DisassociateResolverRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateResolverRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.DisassociateResolverRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateResolverRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateResolverRuleOutput, body, allocator);
}
