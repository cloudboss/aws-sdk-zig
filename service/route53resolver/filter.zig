...
/// `Name` and `Values`. For example,
/// to list only inbound Resolver endpoints, specify `Direction` for `Name` and
...
pub const Filter = struct {
    ...
    /// with underscore (_) delimiters. For example,
    /// `CreatorRequestId` was originally listed as `CREATOR_REQUEST_ID`. Uppercase
    /// values for `Name` are still supported.
    ///
    ...
    /// `AUTO_RECOVERING`, `ACTION_NEEDED`, or `DELETING`. For more information, see
    /// `Status` in
    ...
    ///   to your network. In the value that
    /// you specify for `Values`, include a trailing dot (.) after the domain name.
    ...
    ///   `Name`,
    /// specify one of the following status codes for `Values`: `COMPLETE`,
    /// `DELETING`, `UPDATING`,
    /// or `FAILED`.
    ///
    ...
    ///   created, in Unix time format and
    ...
    ///   logs to. This value can be the ARN for an
    ...
    /// or was shared with the current account by another Amazon Web Services
    /// account. Valid values include: `NOT_SHARED`, `SHARED_WITH_ME`,
    /// or `SHARED_BY_ME`.
    ///
    ...
    /// [Status](https://docs.aws.amazon.com/Route53/latest/APIReference/API_route53resolver_ResolverQueryLogConfig.html#Route53Resolver-Type-route53resolver_ResolverQueryLogConfig-Status).
    ///
    ...
    ///   query logging configuration, in Unix time format and
    /// Coordinated Universal Time (UTC).
    ///
    ...
    ///
    /// * `ResolverQueryLogConfigId`: The ID of the query logging configuration that
    ///   a VPC is associated with.
    ///
