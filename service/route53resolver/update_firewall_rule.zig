...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFirewallRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.UpdateFirewallRule");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFirewallRuleOutput {
    ...
    return aws.json.parseJsonObject(UpdateFirewallRuleOutput, body, allocator);
}
