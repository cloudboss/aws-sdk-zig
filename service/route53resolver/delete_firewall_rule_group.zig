...
pub const DeleteFirewallRuleGroupInput = struct {
...
pub const DeleteFirewallRuleGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteFirewallRuleGroupInput, options: CallOptions) !DeleteFirewallRuleGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteFirewallRuleGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Resolver.DeleteFirewallRuleGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteFirewallRuleGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteFirewallRuleGroupOutput, body, allocator);
}
