...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegenerateSecurityTokenInput, options: Options) !RegenerateSecurityTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegenerateSecurityTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=regenerate-security-token");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegenerateSecurityTokenOutput {
    var result: RegenerateSecurityTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegenerateSecurityTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
