...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreateRoomMembershipInput, options: Options) !BatchCreateRoomMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateRoomMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"MembershipItemList\":");
    try aws.json.writeValue(@TypeOf(input.membership_item_list), input.membership_item_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateRoomMembershipOutput {
    var result: BatchCreateRoomMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchCreateRoomMembershipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
