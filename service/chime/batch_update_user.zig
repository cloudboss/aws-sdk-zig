...
const UpdateUserRequestItem = @import("update_user_request_item.zig").UpdateUserRequestItem;
const UserError = @import("user_error.zig").UserError;

pub const BatchUpdateUserInput = struct {
    /// The Amazon Chime account ID.
    account_id: []const u8,

    /// The request containing the user IDs and details to update.
    update_user_request_items: []const UpdateUserRequestItem,
...
pub const BatchUpdateUserOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateUserInput, options: Options) !BatchUpdateUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"UpdateUserRequestItems\":");
    try aws.json.writeValue(@TypeOf(input.update_user_request_items), input.update_user_request_items, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateUserOutput {
    var result: BatchUpdateUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
