...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRoomInput, options: CallOptions) !GetRoomOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRoomInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRoomOutput {
    var result: GetRoomOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRoomOutput, body, allocator);
    }
    _ = status;
    _ = headers;
