...
pub const GetRoomOutput = struct {
    /// The room details.
    room: ?Room = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRoomInput, options: Options) !GetRoomOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRoomInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRoomOutput {
    var result: GetRoomOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRoomOutput, body, alloc);
    }
    _ = status;
    _ = headers;
