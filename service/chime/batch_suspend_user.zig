...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchSuspendUserInput, options: Options) !BatchSuspendUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchSuspendUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=suspend");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchSuspendUserOutput {
    var result: BatchSuspendUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchSuspendUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
