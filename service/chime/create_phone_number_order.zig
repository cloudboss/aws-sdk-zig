...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePhoneNumberOrderInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.product_type), input.product_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePhoneNumberOrderOutput {
    var result: CreatePhoneNumberOrderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePhoneNumberOrderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
