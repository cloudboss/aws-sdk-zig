...
const UpdateGlobalSettingsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGlobalSettingsInput, options: Options) !UpdateGlobalSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGlobalSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.business_calling) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"BusinessCalling\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.voice_connector) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"VoiceConnector\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGlobalSettingsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
