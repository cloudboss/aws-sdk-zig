...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoomMembershipInput, options: CallOptions) !UpdateRoomMembershipOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRoomMembershipInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRoomMembershipOutput {
    var result: UpdateRoomMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRoomMembershipOutput, body, allocator);
    }
    _ = status;
    _ = headers;
