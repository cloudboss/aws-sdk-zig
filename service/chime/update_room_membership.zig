...
pub const UpdateRoomMembershipInput = struct {
...
pub const UpdateRoomMembershipOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoomMembershipInput, options: Options) !UpdateRoomMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoomMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.member_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoomMembershipOutput {
    var result: UpdateRoomMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRoomMembershipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
