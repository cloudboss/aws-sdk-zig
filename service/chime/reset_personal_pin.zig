...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetPersonalPINInput, options: Options) !ResetPersonalPINOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetPersonalPINInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=reset-personal-pin");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetPersonalPINOutput {
    var result: ResetPersonalPINOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ResetPersonalPINOutput, body, alloc);
    }
    _ = status;
    _ = headers;
