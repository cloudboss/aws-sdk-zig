...
pub const RedactRoomMessageInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RedactRoomMessageInput, options: Options) !RedactRoomMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RedactRoomMessageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RedactRoomMessageOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: RedactRoomMessageOutput = .{};
