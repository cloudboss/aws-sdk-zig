...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutEventsConfigurationInput, options: CallOptions) !PutEventsConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutEventsConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.outbound_events_https_endpoint) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"OutboundEventsHTTPSEndpoint\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
