...
const EventsConfiguration = @import("events_configuration.zig").EventsConfiguration;

pub const PutEventsConfigurationInput = struct {
...
pub const PutEventsConfigurationOutput = struct {
    /// The configuration that allows a bot to receive outgoing events. Can be an
    /// HTTPS endpoint or an
    /// AWS Lambda function ARN.
    events_configuration: ?EventsConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutEventsConfigurationInput, options: Options) !PutEventsConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEventsConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/events-configuration");
    ...
    if (input.lambda_function_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"LambdaFunctionArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.outbound_events_https_endpoint) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OutboundEventsHTTPSEndpoint\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEventsConfigurationOutput {
    var result: PutEventsConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutEventsConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
