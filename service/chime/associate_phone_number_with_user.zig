...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociatePhoneNumberWithUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=associate-phone-number");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "\"E164PhoneNumber\":");
    try aws.json.writeValue(@TypeOf(input.e164_phone_number), input.e164_phone_number, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociatePhoneNumberWithUserOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
