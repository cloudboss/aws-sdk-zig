...
const UpdateUserSettingsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserSettingsInput, options: Options) !UpdateUserSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"UserSettings\":");
    try aws.json.writeValue(@TypeOf(input.user_settings), input.user_settings, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserSettingsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateUserSettingsOutput = .{};
