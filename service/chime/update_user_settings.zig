...
pub const UpdateUserSettingsOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserSettingsInput, options: CallOptions) !UpdateUserSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateUserSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"UserSettings\":");
    try aws.json.writeValue(@TypeOf(input.user_settings), input.user_settings, allocator, &body_buf);
    has_prev = true;
