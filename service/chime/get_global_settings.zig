...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGlobalSettingsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGlobalSettingsOutput {
    var result: GetGlobalSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGlobalSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
