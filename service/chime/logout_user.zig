...
pub const LogoutUserInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: LogoutUserInput, options: CallOptions) !LogoutUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: LogoutUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "operation=logout");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !LogoutUserOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: LogoutUserOutput = .{};
