...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSupportedPhoneNumberCountriesInput, options: Options) !ListSupportedPhoneNumberCountriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSupportedPhoneNumberCountriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.product_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSupportedPhoneNumberCountriesOutput {
    var result: ListSupportedPhoneNumberCountriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSupportedPhoneNumberCountriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
