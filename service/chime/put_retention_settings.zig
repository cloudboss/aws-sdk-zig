...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRetentionSettingsInput, options: Options) !PutRetentionSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRetentionSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"RetentionSettings\":");
    try aws.json.writeValue(@TypeOf(input.retention_settings), input.retention_settings, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRetentionSettingsOutput {
    var result: PutRetentionSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutRetentionSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
