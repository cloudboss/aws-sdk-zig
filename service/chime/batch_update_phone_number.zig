...
pub const BatchUpdatePhoneNumberInput = struct {
    /// The request containing the phone number IDs and product types or calling
    /// names to update.
    update_phone_number_request_items: []const UpdatePhoneNumberRequestItem,
...
pub const BatchUpdatePhoneNumberOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdatePhoneNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=batch-update");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "\"UpdatePhoneNumberRequestItems\":");
    try aws.json.writeValue(@TypeOf(input.update_phone_number_request_items), input.update_phone_number_request_items, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdatePhoneNumberOutput {
    var result: BatchUpdatePhoneNumberOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdatePhoneNumberOutput, body, alloc);
    }
    _ = status;
    _ = headers;
