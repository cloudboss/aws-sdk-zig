...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRoomInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRoomOutput {
    var result: CreateRoomOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRoomOutput, body, alloc);
    }
    _ = status;
    _ = headers;
