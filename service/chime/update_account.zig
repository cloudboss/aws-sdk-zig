...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.default_license) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DefaultLicense\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccountOutput {
    var result: UpdateAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
