...
pub const UpdateAccountInput = struct {
...
pub const UpdateAccountOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccountInput, options: CallOptions) !UpdateAccountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.default_license) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"DefaultLicense\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAccountOutput {
    var result: UpdateAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccountOutput, body, allocator);
    }
    _ = status;
    _ = headers;
