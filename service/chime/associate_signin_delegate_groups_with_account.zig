...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateSigninDelegateGroupsWithAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "operation=associate-signin-delegate-groups");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(allocator, "\"SigninDelegateGroups\":");
    try aws.json.writeValue(@TypeOf(input.signin_delegate_groups), input.signin_delegate_groups, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateSigninDelegateGroupsWithAccountOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
