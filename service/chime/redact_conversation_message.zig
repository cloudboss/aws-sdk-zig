...
pub const RedactConversationMessageInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RedactConversationMessageInput, options: Options) !RedactConversationMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RedactConversationMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/conversations/");
    try path_buf.appendSlice(alloc, input.conversation_id);
    ...
    try query_buf.appendSlice(alloc, "operation=redact");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RedactConversationMessageOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: RedactConversationMessageOutput = .{};
