...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InviteUsersInput, options: Options) !InviteUsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InviteUsersInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=add");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "\"UserEmailList\":");
    try aws.json.writeValue(@TypeOf(input.user_email_list), input.user_email_list, alloc, &body_buf);
    has_prev = true;
    if (input.user_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UserType\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InviteUsersOutput {
    var result: InviteUsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(InviteUsersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
