...
pub const BatchUnsuspendUserInput = struct {
...
pub const BatchUnsuspendUserOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUnsuspendUserInput, options: Options) !BatchUnsuspendUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUnsuspendUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=unsuspend");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "\"UserIdList\":");
    try aws.json.writeValue(@TypeOf(input.user_id_list), input.user_id_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUnsuspendUserOutput {
    var result: BatchUnsuspendUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUnsuspendUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
