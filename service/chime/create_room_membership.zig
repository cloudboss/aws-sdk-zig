...
pub const CreateRoomMembershipInput = struct {
...
pub const CreateRoomMembershipOutput = struct {
    /// The room membership details.
    room_membership: ?RoomMembership = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRoomMembershipInput, options: Options) !CreateRoomMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRoomMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/rooms/");
    try path_buf.appendSlice(alloc, input.room_id);
    ...
    try body_buf.appendSlice(alloc, "\"MemberId\":");
    try aws.json.writeValue(@TypeOf(input.member_id), input.member_id, alloc, &body_buf);
    has_prev = true;
    if (input.role) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRoomMembershipOutput {
    var result: CreateRoomMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRoomMembershipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
