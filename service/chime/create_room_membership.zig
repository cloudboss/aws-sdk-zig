...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRoomMembershipInput, options: CallOptions) !CreateRoomMembershipOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRoomMembershipInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateRoomMembershipOutput {
    var result: CreateRoomMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRoomMembershipOutput, body, allocator);
    }
    _ = status;
    _ = headers;
