...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRetentionSettingsInput, options: CallOptions) !GetRetentionSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRetentionSettingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRetentionSettingsOutput {
    var result: GetRetentionSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRetentionSettingsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
