...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoomsInput, options: Options) !ListRoomsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoomsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.member_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "member-id=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoomsOutput {
    var result: ListRoomsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoomsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
