...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoomInput, options: Options) !UpdateRoomOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoomInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoomOutput {
    var result: UpdateRoomOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRoomOutput, body, alloc);
    }
    _ = status;
    _ = headers;
