...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeletePhoneNumberInput, options: Options) !BatchDeletePhoneNumberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeletePhoneNumberInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=batch-delete");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "\"PhoneNumberIds\":");
    try aws.json.writeValue(@TypeOf(input.phone_number_ids), input.phone_number_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeletePhoneNumberOutput {
    var result: BatchDeletePhoneNumberOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeletePhoneNumberOutput, body, alloc);
    }
    _ = status;
    _ = headers;
