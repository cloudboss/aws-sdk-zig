...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPhoneNumberOrdersInput, options: CallOptions) !ListPhoneNumberOrdersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPhoneNumberOrdersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPhoneNumberOrdersOutput {
    var result: ListPhoneNumberOrdersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPhoneNumberOrdersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
