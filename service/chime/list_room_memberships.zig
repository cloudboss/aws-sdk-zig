...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoomMembershipsInput, options: Options) !ListRoomMembershipsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoomMembershipsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoomMembershipsOutput {
    var result: ListRoomMembershipsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoomMembershipsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
