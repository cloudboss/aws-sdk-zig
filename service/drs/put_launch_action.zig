...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutLaunchActionInput, options: CallOptions) !PutLaunchActionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutLaunchActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"actionCode\":");
    try aws.json.writeValue(@TypeOf(input.action_code), input.action_code, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"actionVersion\":");
    try aws.json.writeValue(@TypeOf(input.action_version), input.action_version, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"active\":");
    try aws.json.writeValue(@TypeOf(input.active), input.active, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"category\":");
    try aws.json.writeValue(@TypeOf(input.category), input.category, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"optional\":");
    try aws.json.writeValue(@TypeOf(input.optional), input.optional, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
