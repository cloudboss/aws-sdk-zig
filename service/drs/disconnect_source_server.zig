...
pub const DisconnectSourceServerInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisconnectSourceServerInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/DisconnectSourceServer";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisconnectSourceServerOutput {
    var result: DisconnectSourceServerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisconnectSourceServerOutput, body, allocator);
    }
    _ = status;
    _ = headers;
