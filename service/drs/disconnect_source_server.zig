...
fn serializeRequest(alloc: std.mem.Allocator, input: DisconnectSourceServerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisconnectSourceServerOutput {
    var result: DisconnectSourceServerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisconnectSourceServerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
