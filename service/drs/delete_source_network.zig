...
pub const DeleteSourceNetworkInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSourceNetworkInput, options: Options) !DeleteSourceNetworkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSourceNetworkInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/DeleteSourceNetwork";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSourceNetworkOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteSourceNetworkOutput = .{};
