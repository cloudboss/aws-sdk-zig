...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRecoveryInput, options: Options) !StartRecoveryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRecoveryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.is_drill) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"isDrill\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sourceServers\":");
    try aws.json.writeValue(@TypeOf(input.source_servers), input.source_servers, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRecoveryOutput {
    var result: StartRecoveryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartRecoveryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
