...
const UpdateFailbackReplicationConfigurationOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFailbackReplicationConfigurationInput, options: Options) !UpdateFailbackReplicationConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFailbackReplicationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.use_private_ip) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"usePrivateIP\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFailbackReplicationConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
