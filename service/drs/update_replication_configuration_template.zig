...
const UpdateReplicationConfigurationTemplateOutput = @import("replication_configuration_template.zig").ReplicationConfigurationTemplate;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateReplicationConfigurationTemplateInput, options: Options) !UpdateReplicationConfigurationTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateReplicationConfigurationTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"replicationConfigurationTemplateID\":");
    try aws.json.writeValue(@TypeOf(input.replication_configuration_template_id), input.replication_configuration_template_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateReplicationConfigurationTemplateOutput {
    var result: UpdateReplicationConfigurationTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateReplicationConfigurationTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
