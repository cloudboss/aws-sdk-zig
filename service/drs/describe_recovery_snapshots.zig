...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRecoverySnapshotsInput, options: Options) !DescribeRecoverySnapshotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRecoverySnapshotsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRecoverySnapshotsOutput {
    var result: DescribeRecoverySnapshotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRecoverySnapshotsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
