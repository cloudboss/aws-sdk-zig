...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateExtendedSourceServerInput, options: Options) !CreateExtendedSourceServerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateExtendedSourceServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"sourceServerArn\":");
    try aws.json.writeValue(@TypeOf(input.source_server_arn), input.source_server_arn, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateExtendedSourceServerOutput {
    var result: CreateExtendedSourceServerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateExtendedSourceServerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
