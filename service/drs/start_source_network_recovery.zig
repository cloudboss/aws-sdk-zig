...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSourceNetworkRecoveryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.deploy_as_new) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"deployAsNew\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sourceNetworks\":");
    try aws.json.writeValue(@TypeOf(input.source_networks), input.source_networks, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSourceNetworkRecoveryOutput {
    var result: StartSourceNetworkRecoveryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartSourceNetworkRecoveryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
