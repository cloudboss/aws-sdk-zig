...
pub const RetryDataReplicationInput = struct {
...
const RetryDataReplicationOutput = @import("source_server.zig").SourceServer;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RetryDataReplicationInput, options: Options) !RetryDataReplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RetryDataReplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/RetryDataReplication";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RetryDataReplicationOutput {
    var result: RetryDataReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RetryDataReplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
