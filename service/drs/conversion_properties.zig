...
pub const ConversionProperties = struct {
    ...
    data_timestamp: ?[]const u8,

    /// Whether the volume being converted uses UEFI or not
    force_uefi: ?bool,

    /// The root volume name of a conversion job
    root_volume_name: ?[]const u8,

    /// A mapping between the volumes being converted and the converted snapshot ids
    volume_to_conversion_map: ?[]const aws.map.MapEntry([]const aws.map.StringMapEntry),

    /// A mapping between the volumes being converted and the product codes
    /// associated with them
    volume_to_product_codes: ?[]const aws.map.MapEntry([]const ProductCode),

    /// A mapping between the volumes and their sizes
    volume_to_volume_size: ?[]const aws.map.MapEntry(i64),
