...
const DaemonDeployment = @import("daemon_deployment.zig").DaemonDeployment;
const Failure = @import("failure.zig").Failure;

pub const DescribeDaemonDeploymentsInput = struct {
    /// The ARN of the daemon deployments to describe. You can specify up to 20
    /// ARNs.
    daemon_deployment_arns: []const []const u8,

    pub const json_field_names = .{
        .daemon_deployment_arns = "daemonDeploymentArns",
    };
};

pub const DescribeDaemonDeploymentsOutput = struct {
    /// The list of daemon deployments.
    daemon_deployments: ?[]const DaemonDeployment = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDaemonDeploymentsInput, options: CallOptions) !DescribeDaemonDeploymentsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "ecs");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDaemonDeploymentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DescribeDaemonDeployments");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDaemonDeploymentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDaemonDeploymentsOutput, body, allocator);
}
