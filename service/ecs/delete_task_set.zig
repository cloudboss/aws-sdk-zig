...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTaskSetInput, options: Options) !DeleteTaskSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTaskSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DeleteTaskSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTaskSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteTaskSetOutput, body, alloc);
}
