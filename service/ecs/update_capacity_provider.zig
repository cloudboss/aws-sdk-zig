...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCapacityProviderInput, options: Options) !UpdateCapacityProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCapacityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.UpdateCapacityProvider");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCapacityProviderOutput {
    ...
    return aws.json.parseJsonObject(UpdateCapacityProviderOutput, body, alloc);
}
