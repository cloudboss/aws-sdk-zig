...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCapacityProviderInput, options: Options) !CreateCapacityProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCapacityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.CreateCapacityProvider");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCapacityProviderOutput {
    ...
    return aws.json.parseJsonObject(CreateCapacityProviderOutput, body, alloc);
}
