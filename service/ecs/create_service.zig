...
pub const CreateServiceInput = struct {
    ...
    /// ECS](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/using-service-linked-roles.html) in the
    ...
    /// Balancer. When
    /// creating an CodeDeploy deployment group, you specify two target groups
    /// (referred to as
    ...
    /// target group with it. Then, it also associates the other target group with
    /// the
    /// replacement task set. The load balancer can also have up to two listeners: a
    /// required
    /// listener for production traffic and an optional listener that you can use to
    /// perform
    ...
    /// contain
    /// the load balancer target group ARN, the container name, and the container
    /// port to access
    /// from the load balancer. The container name must be as it appears in a
    /// container
    /// definition. The load balancer name parameter must be omitted. When a task
    /// from this
    ...
    /// the
    /// container name , and the container port to access from the load balancer.
    /// The container
    ...
    /// Network Load
    /// Balancers. Classic Load Balancers aren't supported. Also, when you create
    /// any target
    ...
    /// Amazon ECS to
    /// make calls to your load balancer on your behalf. This parameter is only
    /// permitted if you
    /// are using a load balancer with your service and your task definition doesn't
    /// use the
    ...
    /// role
    /// is used for your service unless you specify a role here. The service-linked
    /// role is
    ...
    /// task
    /// placement constraints for running tasks and will stop tasks that don't meet
    /// the
    ...
    /// that is
    /// configured at launch time. Currently, the only supported volume type is an
    /// Amazon EBS
    /// volume.
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.CreateService");
