...
pub const ContainerInstance = struct {
    ...
    /// [PutAttributes](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_PutAttributes.html) operation.
    ...
    /// on the host (with the `host` or `bridge` network mode). Any port that's not
    ...
    /// if possible. For more information, see [Container instance
    /// draining](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/container-instance-draining.html) in the *Amazon Elastic Container Service Developer Guide*.
    ...
    /// in CloudWatch Events for the container instance (inside the `detail` object)
