...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContainerInstancesInput, options: Options) !DescribeContainerInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeContainerInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DescribeContainerInstances");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeContainerInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeContainerInstancesOutput, body, alloc);
}
