...
pub const ClusterServiceConnectDefaultsRequest = struct {
    /// The namespace name or full Amazon Resource Name (ARN) of the Cloud Map
    /// namespace that's
    /// used when you create a service and don't specify a Service Connect
    /// configuration. The
    /// namespace name can include up to 1024 characters. The name is
    /// case-sensitive. The name
    /// can't include greater than (>), less than (<), double quotation marks ("),
    /// or
    /// slash (/).
    ///
    /// If you enter an existing namespace name or ARN, then that namespace will be
    /// used. Any
    /// namespace type is supported. The namespace must be in this account and this
    /// Amazon Web Services
    /// Region.
    ///
    /// If you enter a new name, a Cloud Map namespace will be created. Amazon ECS
    /// creates a Cloud Map namespace
    /// with the "API calls" method of instance discovery only. This instance
    /// discovery method
    ...
    /// the cluster configuration for Service Connect is removed. Note that the
    /// namespace will
    /// remain in Cloud
