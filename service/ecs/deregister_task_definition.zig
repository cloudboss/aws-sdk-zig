...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterTaskDefinitionInput, options: CallOptions) !DeregisterTaskDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterTaskDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DeregisterTaskDefinition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterTaskDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeregisterTaskDefinitionOutput, body, allocator);
}
