...
pub const DockerVolumeConfiguration = struct {
    ...
    /// provided
    /// by Docker because it is used for task placement. If the driver was installed
    /// using the
    /// Docker plugin CLI, use `docker plugin ls` to retrieve the driver name from
    /// your container instance. If the driver was installed using another method,
    /// use Docker
    /// plugin discovery to retrieve the driver name. This parameter maps to
    /// `Driver`
