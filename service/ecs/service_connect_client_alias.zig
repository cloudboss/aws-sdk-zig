...
pub const ServiceConnectClientAlias = struct {
    ...
    /// `discoveryName.namespace` is used. If the `discoveryName` isn't specified,
    ...
    /// few common names are `database`, `db`, or the lowercase name of a database,
    /// such as `mysql` or `redis`. For more information, see [Service
    /// Connect](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-connect.html) in the *Amazon Elastic Container Service Developer Guide*.
