const ServiceConnectTestTrafficRules = @import("service_connect_test_traffic_rules.zig").ServiceConnectTestTrafficRules;
...
pub const ServiceConnectClientAlias = struct {
    ...
    /// need to be
    /// fully-qualified. The name can include up to 127 characters. The name can
    /// include
    /// lowercase letters, numbers, underscores (_), hyphens (-), and periods (.).
    /// The name
    ...
    /// to the
    /// same name that the client application uses by default. For example, a few
    /// common names
    ...
    /// To avoid changing your applications in client Amazon ECS services, set this
    /// to the
    /// same port that the client application uses by default. For more information,
    /// see [Service
    /// Connect](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-connect.html) in the *Amazon Elastic Container Service Developer
    /// Guide*.
    port: i32,
