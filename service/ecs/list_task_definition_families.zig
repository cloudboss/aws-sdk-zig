...
const TaskDefinitionFamilyStatus = @import("task_definition_family_status.zig").TaskDefinitionFamilyStatus;
...
pub const ListTaskDefinitionFamiliesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTaskDefinitionFamiliesInput, options: CallOptions) !ListTaskDefinitionFamiliesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTaskDefinitionFamiliesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.ListTaskDefinitionFamilies");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTaskDefinitionFamiliesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTaskDefinitionFamiliesOutput, body, allocator);
}
