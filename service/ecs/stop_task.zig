...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopTaskInput, options: Options) !StopTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.StopTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopTaskOutput, body, alloc);
}
