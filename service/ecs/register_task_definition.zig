...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterTaskDefinitionInput, options: CallOptions) !RegisterTaskDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterTaskDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.RegisterTaskDefinition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterTaskDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterTaskDefinitionOutput, body, allocator);
}
