...
/// specified in the `add` and `drop` request parameters. For information about
/// the latest behavior, see [Docker Compose: order of cap_drop and
/// cap_add](https://forums.docker.com/t/docker-compose-order-of-cap-drop-and-cap-add/97136/1) in the Docker Community Forum.
///
/// * When the container is a privleged container, the container capabilities
///   are all of the default Docker capabilities. The capabilities specified in
///   the `add` request parameter, and the `drop` request parameter are ignored.
/// * When the `add` request parameter is set to ALL, the container capabilities
///   are all of the default Docker capabilities, excluding those specified in
///   the `drop` request parameter.
...
pub const KernelCapabilities = struct {
    ...
    /// in the docker container create command and the `--cap-drop` option to docker
    /// run.
    ///
    /// Valid values: `"ALL" | "AUDIT_CONTROL" | "AUDIT_WRITE" | "BLOCK_SUSPEND" |
    /// "CHOWN" | "DAC_OVERRIDE" | "DAC_READ_SEARCH" | "FOWNER" | "FSETID" |
