...
pub const DeleteExpressGatewayServiceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteExpressGatewayServiceInput, options: CallOptions) !DeleteExpressGatewayServiceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteExpressGatewayServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DeleteExpressGatewayService");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteExpressGatewayServiceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteExpressGatewayServiceOutput, body, allocator);
}
