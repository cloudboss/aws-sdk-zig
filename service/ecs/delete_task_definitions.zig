...
pub const DeleteTaskDefinitionsInput = struct {
    ...
    /// full Amazon Resource Name (ARN) of the task definition to delete. You must
    /// specify a
    /// `revision`.
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTaskDefinitionsInput, options: Options) !DeleteTaskDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTaskDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DeleteTaskDefinitions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTaskDefinitionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteTaskDefinitionsOutput, body, alloc);
}
