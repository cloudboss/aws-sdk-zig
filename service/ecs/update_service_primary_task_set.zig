...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServicePrimaryTaskSetInput, options: Options) !UpdateServicePrimaryTaskSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServicePrimaryTaskSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.UpdateServicePrimaryTaskSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServicePrimaryTaskSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateServicePrimaryTaskSetOutput, body, alloc);
}
