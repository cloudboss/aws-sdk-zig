...
fn serializeRequest(alloc: std.mem.Allocator, input: SubmitAttachmentStateChangesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.SubmitAttachmentStateChanges");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SubmitAttachmentStateChangesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SubmitAttachmentStateChangesOutput, body, alloc);
}
