...
pub const SubmitAttachmentStateChangesInput = struct {
...
pub const SubmitAttachmentStateChangesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SubmitAttachmentStateChangesInput, options: CallOptions) !SubmitAttachmentStateChangesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SubmitAttachmentStateChangesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.SubmitAttachmentStateChanges");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SubmitAttachmentStateChangesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SubmitAttachmentStateChangesOutput, body, allocator);
}
