...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SubmitTaskStateChangeInput, options: Options) !SubmitTaskStateChangeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SubmitTaskStateChangeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.SubmitTaskStateChange");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SubmitTaskStateChangeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SubmitTaskStateChangeOutput, body, alloc);
}
