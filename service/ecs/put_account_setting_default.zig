...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccountSettingDefaultInput, options: Options) !PutAccountSettingDefaultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccountSettingDefaultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.PutAccountSettingDefault");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccountSettingDefaultOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutAccountSettingDefaultOutput, body, alloc);
}
