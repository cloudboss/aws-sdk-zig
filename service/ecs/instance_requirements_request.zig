...
/// Instead of specifying exact instance types, you define requirements such as
/// vCPU count, memory size, network performance, and accelerator
...
pub const InstanceRequirementsRequest = struct {
    ...
    /// `amazon-web-services`, or `xilinx` depending on your accelerator
    /// requirements.
    ...
    /// processing units, `fpga` for field programmable gate arrays, or `inference`
    ...
    /// allow bare metal instances, `excluded` to exclude them, or `required` to use
    /// only bare metal instances.
    ...
    /// T3a, T4g). Set to `included` to allow burstable instances, `excluded` to
    /// exclude them, or `required` to use only burstable instances.
    burstable_performance: ?BurstablePerformance = null,

    /// The CPU manufacturers to include or exclude. You can specify `intel`, `amd`,
    /// or `amazon-web-services` to control which CPU types are used for your
    /// workloads.
    ...
    /// `included` to allow local storage, `excluded` to exclude it, or `required`
    ...
    /// On-Demand price. This provides more precise cost control for Spot instance
    /// selection.
    ...
    /// This is crucial for network-intensive workloads that require high
    /// throughput.
    ...
    /// higher than an identified On-Demand price. The identified On-Demand price is
    ...
    /// type matches your attributes, then the identified price is from either the
    /// lowest priced current generation instance types or, failing that, the lowest
    /// priced previous generation instance types that match your attributes. When
    /// Amazon ECS selects instance types with your attributes, we will exclude
