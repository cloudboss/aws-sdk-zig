...
pub const HostVolumeProperties = struct {
    ...
    /// path on the host container instance that's presented to the container. If
    /// this parameter is empty, then the Docker daemon has assigned a host path for
    /// you. If the `host` parameter contains a `sourcePath` file location, then the
    /// data volume persists at the specified location on the host container
    /// instance until you delete it manually. If the `sourcePath` value doesn't
