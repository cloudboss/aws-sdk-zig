...
const DaemonTaskDefinition = @import("daemon_task_definition.zig").DaemonTaskDefinition;

pub const DescribeDaemonTaskDefinitionInput = struct {
...
pub const DescribeDaemonTaskDefinitionOutput = struct {
    /// The full daemon task definition description.
    daemon_task_definition: ?DaemonTaskDefinition = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDaemonTaskDefinitionInput, options: CallOptions) !DescribeDaemonTaskDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDaemonTaskDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DescribeDaemonTaskDefinition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDaemonTaskDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDaemonTaskDefinitionOutput, body, allocator);
}
