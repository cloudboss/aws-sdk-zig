...
/// exist in the
/// container image (such as those specified in a parent image or from the
/// image's
/// Dockerfile). This configuration maps to the `HEALTHCHECK` parameter of
/// docker
/// run.
///
...
/// definition. Health check parameters that are specified in a container
/// definition
...
/// container health check defined, or Amazon ECS doesn't have the health status
/// of
/// the container.
///
...
/// continue its
/// lifecycle regardless of its health status. For tasks that are part of a
/// service, if the
...
/// lifecycle](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-lifecycle-explanation.html) in the *Amazon Elastic Container Service Developer
/// Guide*.
///
...
/// agent
/// restarts or temporary unavailability. The health check status is the "last
/// heard
...
/// the agent reconnects and another health check occurs. There are no
/// assumptions
...
/// where
/// container dependency is specified, see [Container
/// dependency](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/example_task_definitions.html#example_task_definition-containerdependency) in the *Amazon Elastic Container Service
/// Developer Guide*.
pub const HealthCheck = struct {
    ...
    /// failed health
    /// checks count towards the maximum number of retries. You can specify between
    /// 0 and 300
