...
/// following priority order (high to low):
///
...
///   before stopping the task. For example, for tasks that are part of services
///   configured to use Elastic Load Balancing target groups, target groups will
///   be deregistered in this state.
...
/// lifecycle](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-lifecycle-explanation.html) in the *Amazon Elastic Container Service Developer Guide*.
///
...
///   agent restarts or temporary unavailability. The health check status is the
///   "last heard from" response from the Amazon ECS agent, so if the container
///   was considered `HEALTHY` prior to the disconnect, that status will remain
///   until the agent reconnects and another health check occurs. There are no
///   assumptions made about the status of the container health checks.
...
/// where container dependency is specified, see [Container
/// dependency](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/example_task_definitions.html#example_task_definition-containerdependency) in the *Amazon Elastic Container Service Developer Guide*.
pub const HealthCheck = struct {
    ...
    /// failure. For more information, see `HealthCheck` in the docker container
    /// create command.
    command: []const []const u8,

    /// The time period in seconds between each health check execution. You may
    /// specify between 5 and 300 seconds. The default value is 30 seconds. This
    ...
    /// default value is 5. This value applies only when you specify a `command`.
    timeout: ?i32 = null,
