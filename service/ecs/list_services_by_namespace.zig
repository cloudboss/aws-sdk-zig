...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServicesByNamespaceInput, options: CallOptions) !ListServicesByNamespaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListServicesByNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.ListServicesByNamespace");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListServicesByNamespaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListServicesByNamespaceOutput, body, allocator);
}
