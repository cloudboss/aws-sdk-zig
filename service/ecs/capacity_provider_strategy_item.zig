...
/// can be set
/// when using the
/// [RunTask](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_RunTask.html)or [CreateCluster](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_CreateCluster.html) APIs or as the default capacity provider strategy for a
...
pub const CapacityProviderStrategyItem = struct {
    ...
    /// the specified capacity provider for each service. Only one capacity provider
    /// in a
    ...
    /// used. When multiple capacity providers are specified within a capacity
    /// provider
