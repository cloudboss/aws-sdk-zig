...
pub const PutAttributesInput = struct {
    ...
    /// contains the
    /// resource to apply attributes. If you do not specify a cluster, the default
    /// cluster is
    /// assumed.
...
pub const PutAttributesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAttributesInput, options: CallOptions) !PutAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.PutAttributes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutAttributesOutput, body, allocator);
}
