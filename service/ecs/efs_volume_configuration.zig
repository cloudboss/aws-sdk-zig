...
/// volumes](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/efs-volumes.html) in
...
pub const EFSVolumeConfiguration = struct {
    ...
    /// directory inside
    /// the host. If this parameter is omitted, the root of the Amazon EFS volume
    /// will be used.
    ...
    /// the Amazon
    /// ECS host and the Amazon EFS server. Transit encryption must be turned on if
    /// Amazon EFS
    /// IAM authorization is used. If this parameter is omitted, the default value
    /// of
    /// `DISABLED` is used. For more information, see [Encrypting data in
    /// transit](https://docs.aws.amazon.com/efs/latest/ug/encryption-in-transit.html) in
    ...
    /// the Amazon
    /// EFS server. If you do not specify a transit encryption port, it will use the
    /// port
