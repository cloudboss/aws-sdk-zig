...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCapacityProviderInput, options: CallOptions) !DeleteCapacityProviderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCapacityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DeleteCapacityProvider");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCapacityProviderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteCapacityProviderOutput, body, allocator);
}
