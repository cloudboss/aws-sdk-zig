...
pub const DeploymentConfiguration = struct {
    ...
    /// cluster
    /// resources for starting replacement tasks are available. For more information
    /// about how
    ...
    /// Amazon ECS scheduler to start replacement tasks, the scheduler stops the
    /// unhealthy tasks
    ...
    /// healthy when all essential containers within the task have passed their
    /// health
    /// checks. The amount of time the service scheduler can wait for is determined
    /// by
    ...
    /// * If a task has no essential containers with a health check defined, the
    ///   service
    /// scheduler will wait for the load balancer target group health check to
    /// return a
    ...
    /// balancer target group health check to return a healthy status before
    /// counting
    ...
    /// `DAEMON` service schedule is 0% for the CLI, the Amazon Web Services SDKs,
    /// and the
    ...
    /// used to define the lower limit on the number of the tasks in the service
    /// that remain in
    ...
    /// the currently running tasks with new tasks. The number of tasks that Amazon
    /// ECS adds or
    ...
    /// by running two identical production environments called blue and green. With
    /// Amazon ECS
    /// blue/green deployments, you can validate new service revisions before
    /// directing
    /// production traffic to them. This approach provides a safer way to deploy
    /// changes with
    ...
    /// environment to a new environment in equal percentages over time. With Amazon
    /// ECS linear
    ...
    /// revision first, then shifts the remaining traffic all at once after a
    /// specified
