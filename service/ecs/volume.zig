...
/// configuration may only specify a `name` and one of either
/// `configuredAtLaunch`, `dockerVolumeConfiguration`, `efsVolumeConfiguration`,
/// `s3filesVolumeConfiguration`, `fsxWindowsFileServerVolumeConfiguration`, or
/// `host`. If an empty volume configuration is specified, by default Amazon ECS
/// uses a host volume. For more information, see [Using data volumes in
...
pub const Volume = struct {
    ...
    /// `sourceVolume` parameter of the `mountPoints` object in the container
    /// definition.
    ///
