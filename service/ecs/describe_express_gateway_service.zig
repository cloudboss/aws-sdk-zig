...
const ExpressGatewayServiceInclude = @import("express_gateway_service_include.zig").ExpressGatewayServiceInclude;
const ECSExpressGatewayService = @import("ecs_express_gateway_service.zig").ECSExpressGatewayService;

pub const DescribeExpressGatewayServiceInput = struct {
    /// Specifies additional information to include in the response. Valid values
    /// are `TAGS` to include resource tags associated with the Express service.
    include: ?[]const ExpressGatewayServiceInclude = null,
...
pub const DescribeExpressGatewayServiceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeExpressGatewayServiceInput, options: CallOptions) !DescribeExpressGatewayServiceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeExpressGatewayServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DescribeExpressGatewayService");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeExpressGatewayServiceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeExpressGatewayServiceOutput, body, allocator);
}
