...
pub const TimeoutConfiguration = struct {
    ...
    /// response per request. A value of `0` can be set to disable
    /// `perRequestTimeout`. `perRequestTimeout` can only be set if Service Connect
    /// `appProtocol` isn't `TCP`. Only `idleTimeout` is allowed for `TCP`
    /// `appProtocol`.
