...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTaskProtectionInput, options: Options) !UpdateTaskProtectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTaskProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.UpdateTaskProtection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTaskProtectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateTaskProtectionOutput, body, alloc);
}
