...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContainerInstancesStateInput, options: Options) !UpdateContainerInstancesStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContainerInstancesStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.UpdateContainerInstancesState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContainerInstancesStateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateContainerInstancesStateOutput, body, alloc);
}
