...
pub const ContainerDefinition = struct {
    ...
    /// instance type
    /// by multiplying the vCPUs listed for that instance type on the [Amazon EC2
    /// Instances](http://aws.amazon.com/ec2/instance-types/) detail
    /// page by 1,024.
    ///
    /// Linux containers share unallocated CPU units with other containers on the
    /// container
    /// instance with the same ratio as their allocated amount. For example, if you
    /// run a
    /// single-container task on a single-core instance type with 512 CPU units
    /// specified for
    /// that container, and that's the only task running on the container instance,
    /// that
    /// container could use the full 1,024 CPU unit share at any given time.
    /// However, if you
    /// launched another copy of the same task on that container instance, each task
    /// is
    /// guaranteed a minimum of 512 CPU units when needed. Moreover, each container
    /// could float
    /// to higher CPU usage if the other container was not using it. If both tasks
    /// were 100%
    /// active all of the time, they would be limited to 512 CPU units.
    ///
    /// On Linux container instances, the Docker daemon on the container instance
    /// uses the CPU
    /// value to calculate the relative CPU share ratios for running containers. The
    /// minimum
    /// valid CPU share value that the Linux kernel allows is 2, and the maximum
    /// valid CPU share
    /// value that the Linux kernel allows is 262144. However, the CPU parameter
    /// isn't required,
    /// and you can use CPU values below 2 or above 262144 in your container
    /// definitions. For
    /// CPU values below 2 (including null) or above 262144, the behavior varies
    /// based on your
    /// Amazon ECS container agent version:
    ///
    /// * **Agent versions less than or equal to 1.1.0:**
    /// Null and zero CPU values are passed to Docker as 0, which Docker then
    /// converts
    /// to 1,024 CPU shares. CPU values of 1 are passed to Docker as 1, which the
    /// Linux
    ...
    /// limit, or a
    /// quota. Windows containers only have access to the specified amount of CPU
    /// that's
    /// described in the task definition. A null or zero CPU value is passed to
    /// Docker as
    ...
    /// recommend that you
    /// use this parameter instead of the `dockerSecurityOptions`. The maximum
    /// number
    /// of ARNs is 1.
    ///
    ...
    /// username, password, and the domain to connect to. For better security, the
    /// instance
    /// isn't joined to the domain for domainless authentication. Other applications
    /// on the
    /// instance can't use the domainless credentials. You can use this parameter to
    /// run tasks
    ...
    /// contain
    /// multiple dependencies on other containers in a task definition. When a
    /// dependency is
    ...
    /// the Docker Remote API or greater on your container instance. To check the
    /// Docker Remote
    ...
    /// `. For images using tags (repository-url/image:tag), up to 255 characters
    /// total
    /// are allowed, including letters (uppercase and lowercase), numbers, hyphens,
    /// underscores,
    /// colons, periods, forward slashes, and number signs (#). For images using
    /// digests
    /// (repository-url/image@digest), the 255 character limit applies only to the
    /// repository
    /// URL and image name (everything before the @ sign). The only supported hash
    /// function is
    /// sha256, and the hash value after sha256: must be exactly 64 characters (only
    /// letters
    /// A-F, a-f, and numbers 0-9 are allowed). This parameter maps to `Image` in
    /// the
    ...
    /// the
    /// Docker daemon (shown in the
    /// [LogConfiguration](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_LogConfiguration.html) data type). Additional log drivers may be available in
    ...
    /// attempts
    /// to exceed the memory specified here, the container is killed. The total
    /// amount of memory
    ...
    /// memory is
    /// under heavy contention, Docker attempts to keep the container memory to this
    /// soft limit.
    /// However, your container can consume more memory when it needs to, up to
    /// either the hard
    ...
    /// 300 MiB. This configuration would allow the container to only reserve 128
    /// MiB of memory
    /// from the remaining resources on the container instance, but also allow the
    /// container to
    ...
    /// ECS can
    /// restart the container without needing to replace the task. For more
    /// information, see
    /// [Restart
    ...
    /// the `ecs-init` package. If your container instances are launched from
    /// version
    ...
    /// agent configuration variable are set, then the default values of 30 seconds
    /// for Linux
    /// containers and 30 seconds on Windows containers are used. Your container
    /// instances
    /// require at least version 1.26.0 of the container agent to use a container
    /// stop timeout
    ...
    /// resolve the provided container image tag to a digest and will use the
    /// original image URI
    /// specified in the container definition for deployment. For more information
    /// about
