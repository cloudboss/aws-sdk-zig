...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SubmitContainerStateChangeInput, options: Options) !SubmitContainerStateChangeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SubmitContainerStateChangeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.SubmitContainerStateChange");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SubmitContainerStateChangeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SubmitContainerStateChangeOutput, body, alloc);
}
