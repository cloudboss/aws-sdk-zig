...
/// one volume
/// created for each task in the service. For information about the supported
/// launch types
/// and operating systems, see [Supported operating systems and launch
/// types](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ebs-volumes.html#ebs-volumes-configuration) in the* Amazon
...
pub const ServiceManagedEBSVolumeConfiguration = struct {
    ...
    /// must
    /// specify the same filesystem type that the volume was using when the snapshot
    /// was
    /// created. If there is a filesystem type mismatch, the tasks will fail to
    /// start.
    ///
    ...
    /// Amazon EBS volumes attached to tasks, see [Encrypt data stored in Amazon EBS
    /// volumes attached to Amazon ECS
    ...
    /// EBS volume
    /// to create new volumes for attachment to the tasks maintained by the service.
    /// This
