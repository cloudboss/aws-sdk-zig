...
/// volume, with one volume created for each task in the service. For
/// information about the supported launch types and operating systems, see
/// [Supported operating systems and launch
/// types](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ebs-volumes.html#ebs-volumes-configuration) in the* Amazon Elastic Container Service Developer Guide*.
///
/// Many of these parameters map 1:1 with the Amazon EBS `CreateVolume` API
/// request parameters.
pub const ServiceManagedEBSVolumeConfiguration = struct {
    /// Indicates whether the volume should be encrypted. If you turn on
    /// Region-level Amazon EBS encryption by default but set this value as `false`,
    /// the setting is overridden and the volume is encrypted with the KMS key
    /// specified for Amazon EBS encryption by default. This parameter maps 1:1 with
    /// the `Encrypted` parameter of the [CreateVolume
    /// API](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_CreateVolume.html) in the *Amazon EC2 API Reference*.
    encrypted: ?bool = null,
    ...
    /// fail to start.
    ///
    /// The available Linux filesystem types are `ext3`, `ext4`, and `xfs`. If no
    ...
    /// volumes, this represents the number of IOPS that are provisioned for the
    ...
    /// Management Service key to use for Amazon EBS encryption. When a key is
    /// specified using this parameter, it overrides Amazon EBS default encryption
    /// or any KMS key that you specified for cluster-level managed storage
    /// encryption. This parameter maps 1:1 with the `KmsKeyId` parameter of the
    /// [CreateVolume
    ...
    /// more information, see [Amazon ECS infrastructure IAM
    ...
    /// maintained by the service. You must specify either `snapshotId` or
    /// `sizeInGiB` in your volume configuration. This parameter maps 1:1 with the
    ...
    /// EBS volume to create new volumes for attachment to the tasks maintained by
    /// the service. This property can be specified only if you specify a
    ...
    /// the [CreateVolume
    /// API](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_CreateVolume.html) in the *Amazon EC2 API Reference*. For more information, see [Amazon EBS volume types](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ebs-volume-types.html) in the *Amazon EC2 User Guide*.
    ///
