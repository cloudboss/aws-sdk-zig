...
/// determines if the
/// tasks reach a steady state. The deployment circuit breaker has an option
/// that will
...
/// failures](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/deployment-circuit-breaker.html) in the* Amazon ECS
/// Developer Guide*.
pub const ServiceDeploymentCircuitBreaker = struct {
    ...
    /// the value
    /// to determine when to move the deployment to a FAILED state. The deployment
    /// circuit
    /// breaker has a minimum threshold of 3 and a maximum threshold of 200. and
    /// uses the values
