...
///
/// Most fields of this parameter (`containerPort`, `hostPort`, `protocol`) maps
/// to `PortBindings` in the docker container create command and the `--publish`
...
pub const PortMapping = struct {
    ...
    ///
    /// * You must use either the `bridge` network mode or the `awsvpc` network
    ///   mode.
    ...
    ///   `hostPortRange` is set to the same value as the `containerPortRange`. This
    ///   is a static mapping strategy.
    /// * For containers in a task with the `bridge` network mode, the Amazon ECS
    ///   agent finds open host ports from the default ephemeral range and passes it
    ///   to docker to bind them to the container ports.
    ///
    ...
    /// #11185](https://github.com/moby/moby/issues/11185) on the Github website.
    ///
    /// For information about how to turn off the docker-proxy in the Docker daemon
    /// config file, see [Docker
    /// daemon](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/bootstrap_container_instance.html#bootstrap_docker_daemon) in the *Amazon ECS Developer Guide*.
    ///
    /// You can call [ `DescribeTasks`
    /// ](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DescribeTasks.html) to view the `hostPortRange` which are the host ports that are bound to the container ports.
    ...
    ///   set to the same value as the `containerPort`. This is a static mapping
    ///   strategy.
    /// * For containers in a task with the `bridge` network mode, the Amazon ECS
    ...
    /// omit the `hostPort` (or set it to `0`) while specifying a `containerPort`
    ...
    /// 49153 through 65535 (Linux) or 49152 through 65535 (Windows) is used. Do not
    ...
    /// you use in the `serviceConnectConfiguration` and the
