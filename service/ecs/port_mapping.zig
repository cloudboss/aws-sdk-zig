...
/// network mode of a task definition is set to `host`, host ports must either
/// be
...
pub const PortMapping = struct {
    ...
    /// only applies
    /// to Service Connect. We recommend that you set this parameter to be
    /// consistent with the
    /// protocol that your application uses. If you set this parameter, Amazon ECS
    /// adds
    /// protocol-specific connection handling to the Service Connect proxy. If you
    /// set this
    ...
    /// through 65535
    /// (Linux) or 49152 through 65535 (Windows) is used. Do not attempt to specify
    /// a host port
    ...
    /// and the
    /// Amazon ECS container agent ports 51678-51680. Any host port that was
    /// previously
    /// specified in a running task is also reserved while the task is running. That
    /// is, after a
    /// task stops, the host port is released. The current reserved ports are
    /// displayed in the
    /// `remainingResources` of
    /// [DescribeContainerInstances](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DescribeContainerInstances.html) output. A container instance can have up to 100
