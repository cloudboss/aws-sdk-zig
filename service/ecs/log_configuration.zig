...
const LogDriver = @import("log_driver.zig").LogDriver;
...
/// `awslogs`, `fluentd`, `gelf`, `json-file`, `journald`,`syslog`, `splunk`,
/// and `awsfirelens`.
...
///   Fluentd output aggregators or a remote host running Logstash to send Gelf
///   logs to.
pub const LogConfiguration = struct {
    ...
    /// and `awsfirelens`.
    ///
    /// For more information about using the `awslogs` log driver, see [Send Amazon
    /// ECS logs to
    /// CloudWatch](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/using_awslogs.html) in the *Amazon Elastic Container Service Developer Guide*.
    ///
    ...
    /// you would like to have included. However, we don't currently provide support
    /// for running modified copies of this software.
    log_driver: LogDriver,

    /// The configuration options to send to the log driver.
    ///
    /// The options you can specify depend on the log driver. Some of the options
    /// you can specify when you use the `awslogs` log driver to route logs to
    ...
    /// clusters in different Regions to a single region in CloudWatch Logs. This is
    /// so that they're all visible in one location. Otherwise, you can separate
    /// them by Region for more granularity. Make sure that the specified log group
    ...
    /// specified prefix, the container name, and the ID of the Amazon ECS task that
    /// the container belongs to. If you specify a prefix with this option, then the
    ...
    /// instance. Because it's difficult to trace logs back to the container that
    /// sent them with just the Docker container ID (which is only available on the
    /// container instance), we recommend that you specify a prefix with this
    /// option.
    ///
    ...
    /// log message consists of a line that matches the pattern and any following
    /// lines that don’t match the pattern. The matched line is the delimiter
    ...
    ///
    /// Multiline logging performs regular expression parsing and matching of all
    /// log messages. This might have a negative impact on logging performance.
    ///
    ...
    /// the log driver specified using `logDriver`. The delivery mode you choose
    /// affects application availability when the flow of logs from container is
    /// interrupted.
    ///
    /// If you use the `blocking` mode and the flow of logs is interrupted, calls
    /// from container code to write to the `stdout` and `stderr` streams will
    /// block. The logging thread of the application will block as a result. This
    /// may cause the application to become unresponsive and lead to container
    /// healthcheck failure.
    ///
    /// If you use the `non-blocking` mode, the container's logs are instead stored
    /// in an in-memory intermediate buffer configured with the `max-buffer-size`
    /// option. This prevents the application from becoming unresponsive when logs
    /// cannot be sent. We recommend using this mode if you want to ensure service
    /// availability and are okay with some log loss. For more information, see
    /// [Preventing log loss with non-blocking mode in the `awslogs` container log
    /// driver](http://aws.amazon.com/blogs/containers/preventing-log-loss-with-non-blocking-mode-in-the-awslogs-container-log-driver/).
    ///
    /// You can set a default `mode` for all containers in a specific Amazon Web
    /// Services Region by using the `defaultLogDriverMode` account setting. If you
    /// don't specify the `mode` option or configure the account setting, Amazon ECS
    /// will default to the `non-blocking` mode. For more information about the
    /// account setting, see [Default log driver
    /// mode](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-account-settings.html#default-log-driver-mode) in the *Amazon Elastic Container Service Developer Guide*.
    ///
    ...
    /// Default value: `10m`
    ///
    /// When `non-blocking` mode is used, the `max-buffer-size` log option controls
    /// the size of the buffer that's used for intermediate message storage. Make
    /// sure to specify an adequate buffer size based on your application. When the
    /// buffer fills up, further logs cannot be stored. Logs that cannot be stored
    /// are lost.
    ///
    /// To route logs using the `splunk` log router, you need to specify a
    /// `splunk-token` and a `splunk-url`.
    ///
    /// When you use the `awsfirelens` log router to route logs to an Amazon Web
    /// Services Service or Amazon Web Services Partner Network destination for log
    /// storage and analytics, you can set the `log-driver-buffer-limit` option to
    /// limit the number of events that are buffered in memory, before being sent to
    /// the log router container. It can help to resolve potential log loss issue
    /// because high throughput might result in memory running out for the buffer
    /// inside of Docker.
    ///
    /// Other options you can specify when using `awsfirelens` to route logs depend
    /// on the destination. When you export logs to Amazon Data Firehose, you can
    /// specify the Amazon Web Services Region with `region` and a name for the log
    /// stream with `delivery_stream`.
    ///
    /// When you export logs to Amazon Kinesis Data Streams, you can specify an
    /// Amazon Web Services Region with `region` and a data stream name with
    /// `stream`.
    ///
    /// When you export logs to Amazon OpenSearch Service, you can specify options
    /// like `Name`, `Host` (OpenSearch Service endpoint without protocol), `Port`,
    /// `Index`, `Type`, `Aws_auth`, `Aws_region`, `Suppress_Type_Name`, and `tls`.
    /// For more information, see [Under the hood: FireLens for Amazon ECS
    /// Tasks](http://aws.amazon.com/blogs/containers/under-the-hood-firelens-for-amazon-ecs-tasks/).
    ///
    /// When you export logs to Amazon S3, you can specify the bucket using the
    /// `bucket` option. You can also specify `region`, `total_file_size`,
