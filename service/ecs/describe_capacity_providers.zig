...
pub const DescribeCapacityProvidersInput = struct {
    ...
    /// only
    /// capacity providers associated with this cluster are returned, including
    /// Amazon ECS
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCapacityProvidersInput, options: Options) !DescribeCapacityProvidersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCapacityProvidersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DescribeCapacityProviders");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCapacityProvidersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCapacityProvidersOutput, body, alloc);
}
