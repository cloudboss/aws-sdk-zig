...
pub const DeregisterContainerInstanceInput = struct {
    ...
    /// the
    /// container instance to deregister. If you do not specify a cluster, the
    /// default cluster
    /// is assumed.
    ...
    /// tasks remain running until you terminate the instance or the tasks stop
    /// through some
    ...
    /// service, then
    /// the service scheduler starts another copy of that task, on a different
    /// container
    /// instance if possible.
    ///
    /// Any containers in orphaned service tasks that are registered with a Classic
    /// Load
    /// Balancer or an Application Load Balancer target group are deregistered. They
    /// begin
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterContainerInstanceInput, options: Options) !DeregisterContainerInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterContainerInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DeregisterContainerInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterContainerInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeregisterContainerInstanceOutput, body, alloc);
}
