...
/// the specified `targetCapacity` value as the target value for the metric. For
/// more information, see [Using managed
/// scaling](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/asg-capacity-providers.html#asg-capacity-providers-managed-scaling) in the *Amazon Elastic Container Service Developer Guide*.
///
...
pub const ManagedScaling = struct {
    ...
    /// parameter is omitted, the default value of `300` seconds is used.
    instance_warmup_period: ?i32 = null,

    /// The maximum number of Amazon EC2 instances that Amazon ECS will scale out at
    /// one time. If this parameter is omitted, the default value of `10000` is
    /// used.
    ...
    /// The specified value must be greater than `0` and less than or equal to
    /// `100`. For example, if you want the capacity provider to maintain 10% spare
    /// capacity, then that means the utilization is 90%, so use a `targetCapacity`
    /// of `90`. The default value of `100` percent results in the Amazon EC2
