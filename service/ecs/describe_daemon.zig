...
const DaemonDetail = @import("daemon_detail.zig").DaemonDetail;

pub const DescribeDaemonInput = struct {
...
pub const DescribeDaemonOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDaemonInput, options: CallOptions) !DescribeDaemonOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDaemonInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerServiceV20141113.DescribeDaemon");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDaemonOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDaemonOutput, body, allocator);
}
