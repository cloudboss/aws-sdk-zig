...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEnvironmentActionInput, options: Options) !UpdateEnvironmentActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEnvironmentActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEnvironmentActionOutput {
    var result: UpdateEnvironmentActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEnvironmentActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
