...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccountPoolInput, options: CallOptions) !CreateAccountPoolOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAccountPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.account_source), input.account_source, allocator, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.resolution_strategy), input.resolution_strategy, allocator, &body_buf);
    has_prev = true;
