...
pub const CreateAccountPoolInput = struct {
...
pub const CreateAccountPoolOutput = struct {
    /// The source of accounts for the account pool. In the current release, it's
    /// either a static list of accounts provided by the customer or a custom Amazon
    /// Web Services Lambda handler.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccountPoolInput, options: Options) !CreateAccountPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccountPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"accountSource\":");
    try aws.json.writeValue(@TypeOf(input.account_source), input.account_source, alloc, &body_buf);
    has_prev = true;
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"resolutionStrategy\":");
    try aws.json.writeValue(@TypeOf(input.resolution_strategy), input.resolution_strategy, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccountPoolOutput {
    var result: CreateAccountPoolOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAccountPoolOutput, body, alloc);
    }
    _ = status;
    _ = headers;
