...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDomainUnitInput, options: Options) !UpdateDomainUnitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDomainUnitInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/domain-units/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDomainUnitOutput {
    var result: UpdateDomainUnitOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDomainUnitOutput, body, alloc);
    }
    _ = status;
    _ = headers;
