...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProjectMembershipsInput, options: Options) !ListProjectMembershipsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProjectMembershipsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProjectMembershipsOutput {
    var result: ListProjectMembershipsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProjectMembershipsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
