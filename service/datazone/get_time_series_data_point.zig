...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTimeSeriesDataPointInput, options: Options) !GetTimeSeriesDataPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTimeSeriesDataPointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTimeSeriesDataPointOutput {
    var result: GetTimeSeriesDataPointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTimeSeriesDataPointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
