...
pub const SearchGroupProfilesInput = struct {
    ...
    /// that you can use in a subsequent call to `SearchGroupProfiles` to list the
    /// next set of results.
    ...
    /// in a subsequent call to `SearchGroupProfiles` to list the next set of
    /// results.
...
pub const SearchGroupProfilesOutput = struct {
    /// The results of the `SearchGroupProfiles` action.
    items: ?[]const GroupProfileSummary = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchGroupProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search-group-profiles");
    ...
    try body_buf.appendSlice(alloc, "\"groupType\":");
    try aws.json.writeValue(@TypeOf(input.group_type), input.group_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchGroupProfilesOutput {
    var result: SearchGroupProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchGroupProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
