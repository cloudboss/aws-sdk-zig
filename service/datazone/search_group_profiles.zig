...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchGroupProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/search-group-profiles");
    ...
    try aws.json.writeValue(@TypeOf(input.group_type), input.group_type, allocator, &body_buf);
    has_prev = true;
