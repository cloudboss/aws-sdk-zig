...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEnvironmentCredentialsInput, options: Options) !GetEnvironmentCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEnvironmentCredentialsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEnvironmentCredentialsOutput {
    var result: GetEnvironmentCredentialsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEnvironmentCredentialsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
