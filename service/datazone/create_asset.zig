...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.external_identifier) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"externalIdentifier\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.forms_input) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"formsInput\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.prediction_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"predictionConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"typeIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.type_identifier), input.type_identifier, alloc, &body_buf);
    has_prev = true;
    if (input.type_revision) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"typeRevision\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssetOutput {
    var result: CreateAssetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
