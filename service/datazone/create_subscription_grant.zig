...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSubscriptionGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.asset_target_names) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"assetTargetNames\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"grantedEntity\":");
    try aws.json.writeValue(@TypeOf(input.granted_entity), input.granted_entity, alloc, &body_buf);
    has_prev = true;
    if (input.subscription_target_identifier) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"subscriptionTargetIdentifier\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSubscriptionGrantOutput {
    var result: CreateSubscriptionGrantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSubscriptionGrantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
