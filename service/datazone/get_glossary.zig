...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGlossaryInput, options: Options) !GetGlossaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGlossaryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGlossaryOutput {
    var result: GetGlossaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGlossaryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
