...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGlossaryInput, options: Options) !CreateGlossaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGlossaryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.usage_restrictions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"usageRestrictions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGlossaryOutput {
    var result: CreateGlossaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGlossaryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
