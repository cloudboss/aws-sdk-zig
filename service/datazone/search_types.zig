...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchTypesInput, options: CallOptions) !SearchTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/types-search");
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"managed\":");
    try aws.json.writeValue(@TypeOf(input.managed), input.managed, allocator, &body_buf);
    has_prev = true;
