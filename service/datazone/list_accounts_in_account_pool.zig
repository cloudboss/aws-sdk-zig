...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountsInAccountPoolInput, options: Options) !ListAccountsInAccountPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountsInAccountPoolInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountsInAccountPoolOutput {
    var result: ListAccountsInAccountPoolOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccountsInAccountPoolOutput, body, alloc);
    }
    _ = status;
    _ = headers;
