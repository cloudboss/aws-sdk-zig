...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEnvironmentProfileInput, options: Options) !GetEnvironmentProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEnvironmentProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEnvironmentProfileOutput {
    var result: GetEnvironmentProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEnvironmentProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
