...
const SortFieldAccountPool = @import("sort_field_account_pool.zig").SortFieldAccountPool;
const SortOrder = @import("sort_order.zig").SortOrder;
const AccountPoolSummary = @import("account_pool_summary.zig").AccountPoolSummary;

pub const ListAccountPoolsInput = struct {
    ...
    /// you can use in a subsequent call to ListAccountPools to list the next set of
    /// account pools.
    ...
    sort_by: ?SortFieldAccountPool = null,
...
pub const ListAccountPoolsOutput = struct {
    /// The results of the ListAccountPools operation.
    items: ?[]const AccountPoolSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountPoolsInput, options: Options) !ListAccountPoolsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountPoolsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/account-pools");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountPoolsOutput {
    var result: ListAccountPoolsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccountPoolsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
