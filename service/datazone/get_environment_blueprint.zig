...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEnvironmentBlueprintInput, options: Options) !GetEnvironmentBlueprintOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEnvironmentBlueprintInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEnvironmentBlueprintOutput {
    var result: GetEnvironmentBlueprintOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEnvironmentBlueprintOutput, body, alloc);
    }
    _ = status;
    _ = headers;
