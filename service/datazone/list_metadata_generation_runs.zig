...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMetadataGenerationRunsInput, options: Options) !ListMetadataGenerationRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMetadataGenerationRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.target_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "targetIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.@"type") |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMetadataGenerationRunsOutput {
    var result: ListMetadataGenerationRunsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMetadataGenerationRunsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
