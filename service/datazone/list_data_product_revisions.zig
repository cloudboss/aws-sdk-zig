...
const DataProductRevision = @import("data_product_revision.zig").DataProductRevision;

pub const ListDataProductRevisionsInput = struct {
    ...
    /// `NextToken` value in a subsequent call to `ListDataProductRevisions` to list
    /// the next set of data product revisions.
...
pub const ListDataProductRevisionsOutput = struct {
    /// The results of the `ListDataProductRevisions` action.
    items: ?[]const DataProductRevision = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataProductRevisionsInput, options: Options) !ListDataProductRevisionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataProductRevisionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataProductRevisionsOutput {
    var result: ListDataProductRevisionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataProductRevisionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
