...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataProductInput, options: Options) !CreateDataProductOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataProductInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataProductOutput {
    var result: CreateDataProductOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataProductOutput, body, alloc);
    }
    _ = status;
    _ = headers;
