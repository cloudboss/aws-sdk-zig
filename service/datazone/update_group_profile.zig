...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGroupProfileInput, options: Options) !UpdateGroupProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGroupProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGroupProfileOutput {
    var result: UpdateGroupProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGroupProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
