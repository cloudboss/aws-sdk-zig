...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserProfileInput, options: Options) !UpdateUserProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/user-profiles/");
    try path_buf.appendSlice(alloc, input.user_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserProfileOutput {
    var result: UpdateUserProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateUserProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
