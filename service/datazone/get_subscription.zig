...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSubscriptionInput, options: Options) !GetSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSubscriptionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSubscriptionOutput {
    var result: GetSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSubscriptionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
