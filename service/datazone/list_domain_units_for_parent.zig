...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDomainUnitsForParentInput, options: Options) !ListDomainUnitsForParentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDomainUnitsForParentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "parentDomainUnitIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.parent_domain_unit_identifier);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDomainUnitsForParentOutput {
    var result: ListDomainUnitsForParentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDomainUnitsForParentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
