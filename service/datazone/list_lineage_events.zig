...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLineageEventsInput, options: Options) !ListLineageEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLineageEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.processing_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "processingStatus=");
    ...
    if (input.timestamp_after) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "timestampAfter=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.timestamp_before) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "timestampBefore=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLineageEventsOutput {
    var result: ListLineageEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLineageEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
