...
pub const DisassociateGovernedTermsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateGovernedTermsInput, options: Options) !DisassociateGovernedTermsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateGovernedTermsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/disassociate-governed-terms");
    ...
    try body_buf.appendSlice(alloc, "\"governedGlossaryTerms\":");
    try aws.json.writeValue(@TypeOf(input.governed_glossary_terms), input.governed_glossary_terms, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateGovernedTermsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateGovernedTermsOutput = .{};
