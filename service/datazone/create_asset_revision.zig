...
pub const CreateAssetRevisionInput = struct {
...
pub const CreateAssetRevisionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssetRevisionInput, options: Options) !CreateAssetRevisionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssetRevisionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssetRevisionOutput {
    var result: CreateAssetRevisionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssetRevisionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
