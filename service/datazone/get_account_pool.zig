...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountPoolInput, options: Options) !GetAccountPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccountPoolInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccountPoolOutput {
    var result: GetAccountPoolOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAccountPoolOutput, body, alloc);
    }
    _ = status;
    _ = headers;
