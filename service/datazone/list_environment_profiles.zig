...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentProfilesInput, options: Options) !ListEnvironmentProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnvironmentProfilesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnvironmentProfilesOutput {
    var result: ListEnvironmentProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEnvironmentProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
