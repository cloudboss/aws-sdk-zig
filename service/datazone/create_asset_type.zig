...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssetTypeInput, options: Options) !CreateAssetTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssetTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.forms_input), input.forms_input, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssetTypeOutput {
    var result: CreateAssetTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssetTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
