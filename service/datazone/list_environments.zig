...
pub const ListEnvironmentsInput = struct {
    ...
    /// that you can use in a subsequent call to `ListEnvironments` to list the next
    /// set of environments.
    ...
    /// `NextToken` value in a subsequent call to `ListEnvironments` to list the
...
pub const ListEnvironmentsOutput = struct {
    ...
    items: ?[]const EnvironmentSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentsInput, options: Options) !ListEnvironmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnvironmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.aws_account_region) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "awsAccountRegion=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.environment_blueprint_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "environmentBlueprintIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.environment_profile_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "environmentProfileIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.provider) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "provider=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.status) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnvironmentsOutput {
    var result: ListEnvironmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEnvironmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
