...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEnvironmentActionInput, options: Options) !GetEnvironmentActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEnvironmentActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEnvironmentActionOutput {
    var result: GetEnvironmentActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEnvironmentActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
