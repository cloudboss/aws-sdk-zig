...
fn serializeRequest(alloc: std.mem.Allocator, input: PostLineageEventInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.client_token) |v| {
        try request.headers.put(alloc, "Client-Token", v);
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PostLineageEventOutput {
    var result: PostLineageEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PostLineageEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
