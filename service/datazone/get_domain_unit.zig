...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDomainUnitInput, options: Options) !GetDomainUnitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainUnitInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainUnitOutput {
    var result: GetDomainUnitOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainUnitOutput, body, alloc);
    }
    _ = status;
    _ = headers;
