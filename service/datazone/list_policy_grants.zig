...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPolicyGrantsInput, options: CallOptions) !ListPolicyGrantsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPolicyGrantsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/grants");
    ...
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "policyType=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.policy_type.wireName());
    query_has_prev = true;
