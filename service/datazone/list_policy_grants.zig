...
const PolicyGrantMember = @import("policy_grant_member.zig").PolicyGrantMember;

pub const ListPolicyGrantsInput = struct {
    ...
    /// `ListPolicyGrants`. When the number of grants to be listed is greater than
    /// the value of `MaxResults`, the response contains a `NextToken` value that
    /// you can use in a subsequent call to `ListPolicyGrants` to list the next set
    /// of grants.
    ...
    /// in a subsequent call to `ListPolicyGrants` to list the next set of grants.
...
pub const ListPolicyGrantsOutput = struct {
    /// The results of this action - the listed grants.
    grant_list: ?[]const PolicyGrantMember = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPolicyGrantsInput, options: Options) !ListPolicyGrantsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPolicyGrantsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "policyType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.policy_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPolicyGrantsOutput {
    var result: ListPolicyGrantsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPolicyGrantsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
