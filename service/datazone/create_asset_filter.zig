...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssetFilterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssetFilterOutput {
    var result: CreateAssetFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssetFilterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
