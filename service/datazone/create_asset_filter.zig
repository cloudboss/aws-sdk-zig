...
pub const CreateAssetFilterInput = struct {
...
pub const CreateAssetFilterOutput = struct {
    /// The ID of the asset.
    asset_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssetFilterInput, options: CallOptions) !CreateAssetFilterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAssetFilterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAssetFilterOutput {
    var result: CreateAssetFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssetFilterOutput, body, allocator);
    }
    _ = status;
    _ = headers;
