...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUserProfileInput, options: Options) !GetUserProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUserProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUserProfileOutput {
    var result: GetUserProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUserProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
