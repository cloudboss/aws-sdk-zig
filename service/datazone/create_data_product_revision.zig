...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataProductRevisionInput, options: Options) !CreateDataProductRevisionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataProductRevisionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataProductRevisionOutput {
    var result: CreateDataProductRevisionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataProductRevisionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
