...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSubscriptionRequestInput, options: Options) !CreateSubscriptionRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSubscriptionRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.asset_permissions) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"assetPermissions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.asset_scopes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetScopes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.metadata_forms) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"metadataForms\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"subscribedListings\":");
    try aws.json.writeValue(@TypeOf(input.subscribed_listings), input.subscribed_listings, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"subscribedPrincipals\":");
    try aws.json.writeValue(@TypeOf(input.subscribed_principals), input.subscribed_principals, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSubscriptionRequestOutput {
    var result: CreateSubscriptionRequestOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSubscriptionRequestOutput, body, alloc);
    }
    _ = status;
    _ = headers;
