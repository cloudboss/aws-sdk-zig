...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetadataGenerationRunInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetadataGenerationRunOutput {
    var result: GetMetadataGenerationRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMetadataGenerationRunOutput, body, alloc);
    }
    _ = status;
    _ = headers;
