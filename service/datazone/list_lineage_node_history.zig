...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLineageNodeHistoryInput, options: Options) !ListLineageNodeHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLineageNodeHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/lineage/nodes/");
    try path_buf.appendSlice(alloc, input.identifier);
    try path_buf.appendSlice(alloc, "/history");
    ...
    if (input.direction) |v| {
        ...
        try query_buf.appendSlice(alloc, "direction=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.event_timestamp_gte) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "timestampGTE=");
        {
    ...
    if (input.event_timestamp_lte) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "timestampLTE=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLineageNodeHistoryOutput {
    var result: ListLineageNodeHistoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLineageNodeHistoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
