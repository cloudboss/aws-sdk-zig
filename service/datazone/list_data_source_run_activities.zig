...
pub const ListDataSourceRunActivitiesInput = struct {
    ...
    /// `NextToken` value in a subsequent call to `ListDataSourceRunActivities` to
    /// list the next set of activities.
    next_token: ?[]const u8 = null,

    /// The status of the data source run.
    status: ?DataAssetActivityStatus = null,
...
pub const ListDataSourceRunActivitiesOutput = struct {
    /// The results of the `ListDataSourceRunActivities` action.
    items: ?[]const DataSourceRunActivity = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDataSourceRunActivitiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDataSourceRunActivitiesOutput {
    var result: ListDataSourceRunActivitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataSourceRunActivitiesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
