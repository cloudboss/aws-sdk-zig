...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataSourceRunActivitiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataSourceRunActivitiesOutput {
    var result: ListDataSourceRunActivitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataSourceRunActivitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
