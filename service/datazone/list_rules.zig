...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/list-rules/");
    try path_buf.appendSlice(alloc, input.target_type);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.target_identifier);
    ...
    if (input.project_ids) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "projectIds=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.rule_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ruleType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
