...
const AssetFilterSummary = @import("asset_filter_summary.zig").AssetFilterSummary;

pub const ListAssetFiltersInput = struct {
    ...
    /// that you can use in a subsequent call to `ListAssetFilters` to list the next
    ...
    /// `NextToken` value in a subsequent call to `ListAssetFilters` to list the
...
pub const ListAssetFiltersOutput = struct {
    /// The results of the `ListAssetFilters` action.
    items: ?[]const AssetFilterSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssetFiltersInput, options: Options) !ListAssetFiltersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssetFiltersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.asset_identifier);
    try path_buf.appendSlice(alloc, "/filters");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssetFiltersOutput {
    var result: ListAssetFiltersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetFiltersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
