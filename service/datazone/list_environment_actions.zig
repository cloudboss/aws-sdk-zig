...
const EnvironmentActionSummary = @import("environment_action_summary.zig").EnvironmentActionSummary;

pub const ListEnvironmentActionsInput = struct {
    ...
    /// `ListEnvironmentActions`. When the number of environment actions to be
    ...
    /// `NextToken` value in a subsequent call to `ListEnvironmentActions` to list
...
pub const ListEnvironmentActionsOutput = struct {
    /// The results of `ListEnvironmentActions`.
    items: ?[]const EnvironmentActionSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentActionsInput, options: Options) !ListEnvironmentActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnvironmentActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/actions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnvironmentActionsOutput {
    var result: ListEnvironmentActionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEnvironmentActionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
