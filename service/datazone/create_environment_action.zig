...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEnvironmentActionInput, options: Options) !CreateEnvironmentActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEnvironmentActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEnvironmentActionOutput {
    var result: CreateEnvironmentActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEnvironmentActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
