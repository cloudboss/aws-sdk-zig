...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEnvironmentBlueprintInput, options: Options) !UpdateEnvironmentBlueprintOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEnvironmentBlueprintInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEnvironmentBlueprintOutput {
    var result: UpdateEnvironmentBlueprintOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEnvironmentBlueprintOutput, body, alloc);
    }
    _ = status;
    _ = headers;
