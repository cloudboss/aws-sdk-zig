...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchPutAttributesMetadataInput, options: Options) !BatchPutAttributesMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchPutAttributesMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchPutAttributesMetadataOutput {
    var result: BatchPutAttributesMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchPutAttributesMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
