...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGlossaryTermInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/glossary-terms");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"glossaryIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.glossary_identifier), input.glossary_identifier, alloc, &body_buf);
    has_prev = true;
    if (input.long_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"longDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.short_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"shortDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.status) |v| {
    ...
    if (input.term_relations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"termRelations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGlossaryTermOutput {
    var result: CreateGlossaryTermOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGlossaryTermOutput, body, alloc);
    }
    _ = status;
    _ = headers;
