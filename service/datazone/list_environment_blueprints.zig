...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentBlueprintsInput, options: Options) !ListEnvironmentBlueprintsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnvironmentBlueprintsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.managed) |v| {
        ...
        try query_buf.appendSlice(alloc, "managed=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnvironmentBlueprintsOutput {
    var result: ListEnvironmentBlueprintsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEnvironmentBlueprintsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
