...
const SubscriptionRequestSummary = @import("subscription_request_summary.zig").SubscriptionRequestSummary;

pub const ListSubscriptionRequestsInput = struct {
    ...
    /// `NextToken` value in a subsequent call to `ListSubscriptionRequests` to list
...
pub const ListSubscriptionRequestsOutput = struct {
    /// The results of the `ListSubscriptionRequests` action.
    items: ?[]const SubscriptionRequestSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSubscriptionRequestsInput, options: Options) !ListSubscriptionRequestsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSubscriptionRequestsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSubscriptionRequestsOutput {
    var result: ListSubscriptionRequestsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSubscriptionRequestsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
