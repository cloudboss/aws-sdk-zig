...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddPolicyGrantInput, options: Options) !AddPolicyGrantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddPolicyGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/addGrant");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"detail\":");
    try aws.json.writeValue(@TypeOf(input.detail), input.detail, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddPolicyGrantOutput {
    var result: AddPolicyGrantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddPolicyGrantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
