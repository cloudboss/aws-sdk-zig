...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGlossaryTermInput, options: Options) !UpdateGlossaryTermOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGlossaryTermInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGlossaryTermOutput {
    var result: UpdateGlossaryTermOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGlossaryTermOutput, body, alloc);
    }
    _ = status;
    _ = headers;
