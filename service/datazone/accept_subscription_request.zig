...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptSubscriptionRequestInput, options: Options) !AcceptSubscriptionRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptSubscriptionRequestInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptSubscriptionRequestOutput {
    var result: AcceptSubscriptionRequestOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptSubscriptionRequestOutput, body, alloc);
    }
    _ = status;
    _ = headers;
