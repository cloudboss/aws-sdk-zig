...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSubscriptionTargetInput, options: CallOptions) !UpdateSubscriptionTargetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSubscriptionTargetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateSubscriptionTargetOutput {
    var result: UpdateSubscriptionTargetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSubscriptionTargetOutput, body, allocator);
    }
    _ = status;
    _ = headers;
