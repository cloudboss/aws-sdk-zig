...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSubscriptionGrantInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSubscriptionGrantOutput {
    var result: GetSubscriptionGrantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSubscriptionGrantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
