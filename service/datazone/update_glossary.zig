...
pub const UpdateGlossaryInput = struct {
...
pub const UpdateGlossaryOutput = struct {
    /// The description to be updated as part of the `UpdateGlossary` action.
    description: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGlossaryInput, options: Options) !UpdateGlossaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGlossaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/glossaries/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGlossaryOutput {
    var result: UpdateGlossaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGlossaryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
