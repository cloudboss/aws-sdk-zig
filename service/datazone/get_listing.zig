...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetListingInput, options: Options) !GetListingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetListingInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/listings/");
    ...
    if (input.listing_revision) |v| {
        ...
        try query_buf.appendSlice(alloc, "listingRevision=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetListingOutput {
    var result: GetListingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetListingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
