...
const InventorySearchScope = @import("inventory_search_scope.zig").InventorySearchScope;
const SearchSort = @import("search_sort.zig").SearchSort;
const SearchInventoryResultItem = @import("search_inventory_result_item.zig").SearchInventoryResultItem;
...
pub const SearchInput = struct {
    ...
    /// the number of results to be listed is greater than the value of
    /// `MaxResults`, the response contains a `NextToken` value that you can use in
    /// a subsequent call to `Search` to list the next set of results.
    ...
    /// in a subsequent call to `Search` to list the next set of results.
    next_token: ?[]const u8 = null,

    /// The identifier of the owning project specified for the search.
    owning_project_identifier: ?[]const u8 = null,

    /// The details of the search.
    search_in: ?[]const SearchInItem = null,

    /// The scope of the search.
    search_scope: InventorySearchScope,
...
pub const SearchOutput = struct {
    ...
    items: ?[]const SearchInventoryResultItem = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.additional_attributes) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"additionalAttributes\":");
    ...
    if (input.owning_project_identifier) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchOutput, body, alloc);
    }
    _ = status;
    _ = headers;
