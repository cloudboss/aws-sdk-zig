...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSubscriptionGrantStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.failure_cause) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"failureCause\":");
    ...
    if (input.target_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"targetName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSubscriptionGrantStatusOutput {
    var result: UpdateSubscriptionGrantStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSubscriptionGrantStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
