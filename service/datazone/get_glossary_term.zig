...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGlossaryTermInput, options: Options) !GetGlossaryTermOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGlossaryTermInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGlossaryTermOutput {
    var result: GetGlossaryTermOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGlossaryTermOutput, body, alloc);
    }
    _ = status;
    _ = headers;
