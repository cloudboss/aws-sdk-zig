...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PostTimeSeriesDataPointsInput, options: CallOptions) !PostTimeSeriesDataPointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PostTimeSeriesDataPointsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"forms\":");
    try aws.json.writeValue(@TypeOf(input.forms), input.forms, allocator, &body_buf);
    has_prev = true;
