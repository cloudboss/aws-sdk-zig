...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSubscriptionTargetInput, options: Options) !CreateSubscriptionTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSubscriptionTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.applicable_asset_types), input.applicable_asset_types, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"authorizedPrincipals\":");
    try aws.json.writeValue(@TypeOf(input.authorized_principals), input.authorized_principals, alloc, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.manage_access_role), input.manage_access_role, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.subscription_target_config), input.subscription_target_config, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSubscriptionTargetOutput {
    var result: CreateSubscriptionTargetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSubscriptionTargetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
