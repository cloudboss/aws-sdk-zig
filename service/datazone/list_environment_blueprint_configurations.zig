...
const EnvironmentBlueprintConfigurationItem = @import("environment_blueprint_configuration_item.zig").EnvironmentBlueprintConfigurationItem;

pub const ListEnvironmentBlueprintConfigurationsInput = struct {
    ...
    /// for `MaxResults` that is less than the number of configurations, the
...
pub const ListEnvironmentBlueprintConfigurationsOutput = struct {
    /// The results of the `ListEnvironmentBlueprintConfigurations` action.
    items: ?[]const EnvironmentBlueprintConfigurationItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentBlueprintConfigurationsInput, options: Options) !ListEnvironmentBlueprintConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnvironmentBlueprintConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnvironmentBlueprintConfigurationsOutput {
    var result: ListEnvironmentBlueprintConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEnvironmentBlueprintConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
