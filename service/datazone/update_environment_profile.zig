...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEnvironmentProfileInput, options: Options) !UpdateEnvironmentProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEnvironmentProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEnvironmentProfileOutput {
    var result: UpdateEnvironmentProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEnvironmentProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
