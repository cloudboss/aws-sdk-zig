...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAssetFilterInput, options: Options) !UpdateAssetFilterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssetFilterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssetFilterOutput {
    var result: UpdateAssetFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAssetFilterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
