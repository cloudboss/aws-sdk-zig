...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssetRevisionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssetRevisionsOutput {
    var result: ListAssetRevisionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetRevisionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
