...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveEntityOwnerInput, options: Options) !RemoveEntityOwnerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveEntityOwnerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/removeOwner");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveEntityOwnerOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
