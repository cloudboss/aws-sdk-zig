...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSubscriptionTargetInput, options: Options) !GetSubscriptionTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSubscriptionTargetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSubscriptionTargetOutput {
    var result: GetSubscriptionTargetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSubscriptionTargetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
