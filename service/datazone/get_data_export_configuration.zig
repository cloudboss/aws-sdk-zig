...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataExportConfigurationInput, options: Options) !GetDataExportConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataExportConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/data-export-configuration");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataExportConfigurationOutput {
    var result: GetDataExportConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataExportConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
