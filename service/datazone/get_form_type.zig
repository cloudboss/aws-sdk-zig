...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFormTypeInput, options: Options) !GetFormTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFormTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFormTypeOutput {
    var result: GetFormTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFormTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
