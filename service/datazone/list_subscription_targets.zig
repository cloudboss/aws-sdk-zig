...
const SubscriptionTargetSummary = @import("subscription_target_summary.zig").SubscriptionTargetSummary;

pub const ListSubscriptionTargetsInput = struct {
    ...
    /// `NextToken` value in a subsequent call to `ListSubscriptionTargets` to list
...
pub const ListSubscriptionTargetsOutput = struct {
    /// The results of the `ListSubscriptionTargets` action.
    items: ?[]const SubscriptionTargetSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSubscriptionTargetsInput, options: CallOptions) !ListSubscriptionTargetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSubscriptionTargetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSubscriptionTargetsOutput {
    var result: ListSubscriptionTargetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSubscriptionTargetsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
