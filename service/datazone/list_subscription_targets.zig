...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSubscriptionTargetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSubscriptionTargetsOutput {
    var result: ListSubscriptionTargetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSubscriptionTargetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
