...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGroupProfileInput, options: Options) !CreateGroupProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGroupProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGroupProfileOutput {
    var result: CreateGroupProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGroupProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
