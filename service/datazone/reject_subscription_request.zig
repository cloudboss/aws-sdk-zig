...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectSubscriptionRequestInput, options: Options) !RejectSubscriptionRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectSubscriptionRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.decision_comment) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"decisionComment\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectSubscriptionRequestOutput {
    var result: RejectSubscriptionRequestOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RejectSubscriptionRequestOutput, body, alloc);
    }
    _ = status;
    _ = headers;
