...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssetFilterInput, options: Options) !GetAssetFilterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssetFilterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssetFilterOutput {
    var result: GetAssetFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssetFilterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
