...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssetTypeInput, options: Options) !GetAssetTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssetTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssetTypeOutput {
    var result: GetAssetTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssetTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
