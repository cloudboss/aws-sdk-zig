...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEnvironmentBlueprintInput, options: Options) !CreateEnvironmentBlueprintOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEnvironmentBlueprintInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/environment-blueprints");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"provisioningProperties\":");
    try aws.json.writeValue(@TypeOf(input.provisioning_properties), input.provisioning_properties, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEnvironmentBlueprintOutput {
    var result: CreateEnvironmentBlueprintOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEnvironmentBlueprintOutput, body, alloc);
    }
    _ = status;
    _ = headers;
