...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccountPoolInput, options: Options) !UpdateAccountPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccountPoolInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccountPoolOutput {
    var result: UpdateAccountPoolOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccountPoolOutput, body, alloc);
    }
    _ = status;
    _ = headers;
