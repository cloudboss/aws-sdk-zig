...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemovePolicyGrantInput, options: Options) !RemovePolicyGrantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemovePolicyGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/removeGrant");
    ...
    if (input.grant_identifier) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"grantIdentifier\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemovePolicyGrantOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
