...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptPredictionsInput, options: Options) !AcceptPredictionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptPredictionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/accept-predictions");
    ...
    if (input.accept_choices) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"acceptChoices\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.accept_rule) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"acceptRule\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptPredictionsOutput {
    var result: AcceptPredictionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptPredictionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
