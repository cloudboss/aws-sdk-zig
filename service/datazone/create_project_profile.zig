...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProjectProfileInput, options: Options) !CreateProjectProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProjectProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProjectProfileOutput {
    var result: CreateProjectProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProjectProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
