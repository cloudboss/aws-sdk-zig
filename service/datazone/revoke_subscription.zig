...
fn serializeRequest(alloc: std.mem.Allocator, input: RevokeSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.retain_permissions) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"retainPermissions\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RevokeSubscriptionOutput {
    var result: RevokeSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RevokeSubscriptionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
