...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProjectsInput, options: Options) !ListProjectsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProjectsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.user_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "userIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProjectsOutput {
    var result: ListProjectsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProjectsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
