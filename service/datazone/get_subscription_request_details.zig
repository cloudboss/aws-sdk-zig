...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSubscriptionRequestDetailsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSubscriptionRequestDetailsOutput {
    var result: GetSubscriptionRequestDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSubscriptionRequestDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
