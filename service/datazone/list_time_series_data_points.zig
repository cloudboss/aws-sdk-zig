...
pub const ListTimeSeriesDataPointsInput = struct {
...
pub const ListTimeSeriesDataPointsOutput = struct {
    /// The results of the ListTimeSeriesDataPoints action.
    items: ?[]const TimeSeriesDataPointSummaryFormOutput = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTimeSeriesDataPointsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ended_at) |v| {
        ...
        try query_buf.appendSlice(alloc, "endedAt=");
        {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "formName=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.form_name);
    query_has_prev = true;
    ...
    if (input.started_at) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "startedAt=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTimeSeriesDataPointsOutput {
    var result: ListTimeSeriesDataPointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTimeSeriesDataPointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
