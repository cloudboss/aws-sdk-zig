...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTimeSeriesDataPointsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ended_at) |v| {
        ...
        try query_buf.appendSlice(allocator, "endedAt=");
        {
    ...
    if (input.started_at) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "startedAt=");
        {
            const num_str = std.fmt.allocPrint(allocator, "{d}", .{v}) catch "";
            try query_buf.appendSlice(allocator, num_str);
        }
