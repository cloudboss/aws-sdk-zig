...
pub const AddEntityOwnerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddEntityOwnerInput, options: Options) !AddEntityOwnerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddEntityOwnerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/addOwner");
    ...
    try aws.json.writeValue(@TypeOf(input.owner), input.owner, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddEntityOwnerOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AddEntityOwnerOutput = .{};
