...
const DataSourceRunSummary = @import("data_source_run_summary.zig").DataSourceRunSummary;

pub const ListDataSourceRunsInput = struct {
    ...
    /// `ListDataSourceRuns`. When the number of runs to be listed is greater than
    /// the value of `MaxResults`, the response contains a `NextToken` value that
    /// you can use in a subsequent call to `ListDataSourceRuns` to list the next
    /// set of runs.
    ...
    status: ?DataSourceRunStatus = null,
...
pub const ListDataSourceRunsOutput = struct {
    /// The results of the `ListDataSourceRuns` action.
    items: ?[]const DataSourceRunSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataSourceRunsInput, options: CallOptions) !ListDataSourceRunsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDataSourceRunsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDataSourceRunsOutput {
    var result: ListDataSourceRunsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataSourceRunsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
