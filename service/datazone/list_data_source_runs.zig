...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataSourceRunsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataSourceRunsOutput {
    var result: ListDataSourceRunsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataSourceRunsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
