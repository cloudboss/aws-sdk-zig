...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEntityOwnersInput, options: Options) !ListEntityOwnersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEntityOwnersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/owners");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEntityOwnersOutput {
    var result: ListEntityOwnersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEntityOwnersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
