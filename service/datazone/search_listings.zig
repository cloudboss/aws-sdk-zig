...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchListingsInput, options: Options) !SearchListingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchListingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/listings/search");
    ...
    if (input.aggregations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"aggregations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.filters) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchListingsOutput {
    var result: SearchListingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchListingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
