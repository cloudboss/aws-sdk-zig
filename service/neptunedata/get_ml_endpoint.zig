...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMLEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMLEndpointOutput {
    var result: GetMLEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMLEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
