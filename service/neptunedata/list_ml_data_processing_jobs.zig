...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMLDataProcessingJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMLDataProcessingJobsOutput {
    var result: ListMLDataProcessingJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMLDataProcessingJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
