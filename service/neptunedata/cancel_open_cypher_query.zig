...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelOpenCypherQueryInput, options: Options) !CancelOpenCypherQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelOpenCypherQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.silent) |v| {
        ...
        try query_buf.appendSlice(alloc, "silent=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelOpenCypherQueryOutput {
    var result: CancelOpenCypherQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelOpenCypherQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
