...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMLModelTransformJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMLModelTransformJobsOutput {
    var result: ListMLModelTransformJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMLModelTransformJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
