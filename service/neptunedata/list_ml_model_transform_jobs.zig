...
pub const ListMLModelTransformJobsInput = struct {
...
pub const ListMLModelTransformJobsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMLModelTransformJobsInput, options: CallOptions) !ListMLModelTransformJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMLModelTransformJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMLModelTransformJobsOutput {
    var result: ListMLModelTransformJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMLModelTransformJobsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
