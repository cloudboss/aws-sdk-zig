...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMLDataProcessingJobInput, options: Options) !StartMLDataProcessingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMLDataProcessingJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.config_file_name) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"configFileName\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"inputDataS3Location\":");
    try aws.json.writeValue(@TypeOf(input.input_data_s3_location), input.input_data_s3_location, alloc, &body_buf);
    has_prev = true;
    if (input.model_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"modelType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.previous_data_processing_job_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"previousDataProcessingJobId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"processedDataS3Location\":");
    try aws.json.writeValue(@TypeOf(input.processed_data_s3_location), input.processed_data_s3_location, alloc, &body_buf);
    has_prev = true;
    if (input.processing_instance_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"processingInstanceType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.processing_instance_volume_size_in_gb) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"processingInstanceVolumeSizeInGB\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.processing_time_out_in_seconds) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"processingTimeOutInSeconds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMLDataProcessingJobOutput {
    var result: StartMLDataProcessingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMLDataProcessingJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
