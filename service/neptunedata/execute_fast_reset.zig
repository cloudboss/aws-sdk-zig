...
const FastResetToken = @import("fast_reset_token.zig").FastResetToken;

pub const ExecuteFastResetInput = struct {
    ...
    ///   the `initiateDatabaseReset` action to actually perform the fast reset.
    action: Action,
...
pub const ExecuteFastResetOutput = struct {
    /// The `payload` is only returned by the `initiateDatabaseReset` action, and
    /// contains the unique token to use with the `performDatabaseReset` action to
    /// make the reset occur.
    payload: ?FastResetToken = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteFastResetInput, options: Options) !ExecuteFastResetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteFastResetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteFastResetOutput {
    var result: ExecuteFastResetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ExecuteFastResetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
