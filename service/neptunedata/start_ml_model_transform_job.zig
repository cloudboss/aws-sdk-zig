...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMLModelTransformJobInput, options: CallOptions) !StartMLModelTransformJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartMLModelTransformJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.base_processing_instance_volume_size_in_gb) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"baseProcessingInstanceVolumeSizeInGB\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.custom_model_transform_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"customModelTransformParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"modelTransformOutputS3Location\":");
    try aws.json.writeValue(@TypeOf(input.model_transform_output_s3_location), input.model_transform_output_s3_location, allocator, &body_buf);
    has_prev = true;
    ...
    if (input.training_job_name) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"trainingJobName\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartMLModelTransformJobOutput {
    var result: StartMLModelTransformJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMLModelTransformJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
