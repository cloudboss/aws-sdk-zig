...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteOpenCypherExplainQueryInput, options: Options) !ExecuteOpenCypherExplainQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteOpenCypherExplainQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.explain_mode), input.explain_mode, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteOpenCypherExplainQueryOutput {
    var result: ExecuteOpenCypherExplainQueryOutput = .{};
    result.results = try alloc.dupe(u8, body);
    _ = status;
    _ = headers;
