...
const PropertygraphSummaryValueMap = @import("propertygraph_summary_value_map.zig").PropertygraphSummaryValueMap;
...
pub const GetPropertygraphSummaryOutput = struct {
    /// Payload containing the property graph summary response.
    payload: ?PropertygraphSummaryValueMap = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPropertygraphSummaryInput, options: Options) !GetPropertygraphSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPropertygraphSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/propertygraph/statistics/summary";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPropertygraphSummaryOutput {
    var result: GetPropertygraphSummaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPropertygraphSummaryOutput, body, alloc);
    }
