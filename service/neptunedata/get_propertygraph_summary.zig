...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPropertygraphSummaryInput, options: CallOptions) !GetPropertygraphSummaryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPropertygraphSummaryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPropertygraphSummaryOutput {
    var result: GetPropertygraphSummaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPropertygraphSummaryOutput, body, allocator);
    }
