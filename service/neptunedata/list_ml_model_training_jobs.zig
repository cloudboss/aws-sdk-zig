...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMLModelTrainingJobsInput, options: Options) !ListMLModelTrainingJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMLModelTrainingJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMLModelTrainingJobsOutput {
    var result: ListMLModelTrainingJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMLModelTrainingJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
