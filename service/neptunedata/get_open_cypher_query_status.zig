...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOpenCypherQueryStatusInput, options: CallOptions) !GetOpenCypherQueryStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetOpenCypherQueryStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetOpenCypherQueryStatusOutput {
    var result: GetOpenCypherQueryStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOpenCypherQueryStatusOutput, body, allocator);
    }
    _ = status;
    _ = headers;
