...
pub const GetOpenCypherQueryStatusInput = struct {
...
pub const GetOpenCypherQueryStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOpenCypherQueryStatusInput, options: Options) !GetOpenCypherQueryStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOpenCypherQueryStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/opencypher/status/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOpenCypherQueryStatusOutput {
    var result: GetOpenCypherQueryStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOpenCypherQueryStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
