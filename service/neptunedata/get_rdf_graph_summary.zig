...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRDFGraphSummaryInput, options: Options) !GetRDFGraphSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRDFGraphSummaryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRDFGraphSummaryOutput {
    var result: GetRDFGraphSummaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRDFGraphSummaryOutput, body, alloc);
    }
