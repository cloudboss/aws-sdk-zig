...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteOpenCypherQueryInput, options: Options) !ExecuteOpenCypherQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteOpenCypherQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"openCypherQuery\":");
    try aws.json.writeValue(@TypeOf(input.open_cypher_query), input.open_cypher_query, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteOpenCypherQueryOutput {
    var result: ExecuteOpenCypherQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ExecuteOpenCypherQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
