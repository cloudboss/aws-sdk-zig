...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteGremlinQueryInput, options: Options) !ExecuteGremlinQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteGremlinQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"gremlinQuery\":");
    try aws.json.writeValue(@TypeOf(input.gremlin_query), input.gremlin_query, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteGremlinQueryOutput {
    var result: ExecuteGremlinQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ExecuteGremlinQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
