...
const DeletePropertygraphStatisticsInput = struct {};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePropertygraphStatisticsInput, options: Options) !DeletePropertygraphStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePropertygraphStatisticsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePropertygraphStatisticsOutput {
    var result: DeletePropertygraphStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePropertygraphStatisticsOutput, body, alloc);
    }
