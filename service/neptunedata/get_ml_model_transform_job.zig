...
pub const GetMLModelTransformJobInput = struct {
...
pub const GetMLModelTransformJobOutput = struct {
    /// The base data processing job.
    base_processing_job: ?MlResourceDefinition = null,
    ...
    models: ?[]const MlConfigDefinition = null,

    /// The remote model transform job.
    remote_model_transform_job: ?MlResourceDefinition = null,

    /// The status of the model-transform job.
    status: ?[]const u8 = null,

    pub const json_field_names = .{
        .base_processing_job = "baseProcessingJob",
        .id = "id",
        .models = "models",
        .remote_model_transform_job = "remoteModelTransformJob",
        .status = "status",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMLModelTransformJobInput, options: Options) !GetMLModelTransformJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMLModelTransformJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMLModelTransformJobOutput {
    var result: GetMLModelTransformJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMLModelTransformJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
