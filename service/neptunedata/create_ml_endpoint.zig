...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMLEndpointInput, options: Options) !CreateMLEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMLEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ml_model_transform_job_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"mlModelTransformJobId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMLEndpointOutput {
    var result: CreateMLEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMLEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
