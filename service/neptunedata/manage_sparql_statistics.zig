...
pub const ManageSparqlStatisticsInput = struct {
...
pub const ManageSparqlStatisticsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ManageSparqlStatisticsInput, options: Options) !ManageSparqlStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ManageSparqlStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ManageSparqlStatisticsOutput {
    var result: ManageSparqlStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ManageSparqlStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
