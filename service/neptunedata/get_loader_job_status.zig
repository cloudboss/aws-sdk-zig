...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLoaderJobStatusInput, options: Options) !GetLoaderJobStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLoaderJobStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/loader/");
    try path_buf.appendSlice(alloc, input.load_id);
    ...
    if (input.details) |v| {
        ...
        try query_buf.appendSlice(alloc, "details=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.errors) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "errors=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.errors_per_page) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "errorsPerPage=");
        {
    ...
    if (input.page) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "page=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLoaderJobStatusOutput {
    var result: GetLoaderJobStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLoaderJobStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
