...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartLoaderJobInput, options: Options) !StartLoaderJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartLoaderJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dependencies) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"dependencies\":");
    ...
    if (input.parallelism) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"parallelism\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.parser_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"parserConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.queue_request) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"queueRequest\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"s3BucketRegion\":");
    try aws.json.writeValue(@TypeOf(input.s_3_bucket_region), input.s_3_bucket_region, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.update_single_cardinality_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"updateSingleCardinalityProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.user_provided_edge_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"userProvidedEdgeIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartLoaderJobOutput {
    var result: StartLoaderJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartLoaderJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
