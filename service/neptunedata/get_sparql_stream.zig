...
const SparqlRecord = @import("sparql_record.zig").SparqlRecord;

pub const GetSparqlStreamInput = struct {
...
pub const GetSparqlStreamOutput = struct {
    ...
    records: ?[]const SparqlRecord = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSparqlStreamInput, options: Options) !GetSparqlStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSparqlStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/sparql/stream";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.commit_num) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "commitNum=");
        {
    ...
    if (input.iterator_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "iteratorType=");
    ...
    if (input.op_num) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "opNum=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.encoding) |v| {
        try request.headers.put(alloc, "Accept-Encoding", @tagName(v));
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSparqlStreamOutput {
    var result: GetSparqlStreamOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSparqlStreamOutput, body, alloc);
    }
    _ = status;
    _ = headers;
