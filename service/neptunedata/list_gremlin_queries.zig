...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGremlinQueriesInput, options: Options) !ListGremlinQueriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGremlinQueriesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_waiting) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeWaiting=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGremlinQueriesOutput {
    var result: ListGremlinQueriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGremlinQueriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
