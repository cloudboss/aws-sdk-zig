...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMLEndpointInput, options: CallOptions) !DeleteMLEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMLEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMLEndpointOutput {
    var result: DeleteMLEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteMLEndpointOutput, body, allocator);
    }
    _ = status;
    _ = headers;
