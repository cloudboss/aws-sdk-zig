...
pub const DeleteMLEndpointInput = struct {
...
pub const DeleteMLEndpointOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMLEndpointInput, options: Options) !DeleteMLEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMLEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ml/endpoints/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMLEndpointOutput {
    var result: DeleteMLEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteMLEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
