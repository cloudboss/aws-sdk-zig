...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteGremlinProfileQueryInput, options: Options) !ExecuteGremlinProfileQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteGremlinProfileQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.chop) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"chop\":");
    ...
    if (input.index_ops) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"indexOps\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.results) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"results\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.serializer) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"serializer\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteGremlinProfileQueryOutput {
    var result: ExecuteGremlinProfileQueryOutput = .{};
    if (body.len > 0) {
        result.output = try alloc.dupe(u8, body);
    }
    _ = status;
    _ = headers;
