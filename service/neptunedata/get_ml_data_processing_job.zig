...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMLDataProcessingJobInput, options: Options) !GetMLDataProcessingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMLDataProcessingJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMLDataProcessingJobOutput {
    var result: GetMLDataProcessingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMLDataProcessingJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
