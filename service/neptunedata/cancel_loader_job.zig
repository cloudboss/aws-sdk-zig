...
pub const CancelLoaderJobInput = struct {
...
pub const CancelLoaderJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelLoaderJobInput, options: Options) !CancelLoaderJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelLoaderJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelLoaderJobOutput {
    var result: CancelLoaderJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelLoaderJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
