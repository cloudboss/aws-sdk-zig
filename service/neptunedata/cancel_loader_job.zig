...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelLoaderJobInput, options: CallOptions) !CancelLoaderJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelLoaderJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelLoaderJobOutput {
    var result: CancelLoaderJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelLoaderJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
