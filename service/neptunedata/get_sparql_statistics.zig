...
const GetSparqlStatisticsInput = struct {};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSparqlStatisticsInput, options: Options) !GetSparqlStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSparqlStatisticsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSparqlStatisticsOutput {
    var result: GetSparqlStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSparqlStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
