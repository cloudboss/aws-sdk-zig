...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMLDataProcessingJobInput, options: Options) !CancelMLDataProcessingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelMLDataProcessingJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ml/dataprocessing/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelMLDataProcessingJobOutput {
    var result: CancelMLDataProcessingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelMLDataProcessingJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
