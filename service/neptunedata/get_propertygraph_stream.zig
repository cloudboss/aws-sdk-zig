...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPropertygraphStreamInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPropertygraphStreamOutput {
    var result: GetPropertygraphStreamOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPropertygraphStreamOutput, body, alloc);
    }
    _ = status;
    _ = headers;
