...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOpenCypherQueriesInput, options: Options) !ListOpenCypherQueriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOpenCypherQueriesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOpenCypherQueriesOutput {
    var result: ListOpenCypherQueriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOpenCypherQueriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
