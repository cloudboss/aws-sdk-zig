...
pub const CancelMLModelTransformJobInput = struct {
...
pub const CancelMLModelTransformJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMLModelTransformJobInput, options: Options) !CancelMLModelTransformJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelMLModelTransformJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ml/modeltransform/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelMLModelTransformJobOutput {
    var result: CancelMLModelTransformJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelMLModelTransformJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
