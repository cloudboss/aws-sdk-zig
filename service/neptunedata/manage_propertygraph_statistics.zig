...
const StatisticsAutoGenerationMode = @import("statistics_auto_generation_mode.zig").StatisticsAutoGenerationMode;
const RefreshStatisticsIdMap = @import("refresh_statistics_id_map.zig").RefreshStatisticsIdMap;

pub const ManagePropertygraphStatisticsInput = struct {
    /// The statistics generation mode. One of: `DISABLE_AUTOCOMPUTE`,
    /// `ENABLE_AUTOCOMPUTE`, or `REFRESH`, the last of which manually triggers DFE
    /// statistics generation.
    mode: ?StatisticsAutoGenerationMode = null,
...
pub const ManagePropertygraphStatisticsOutput = struct {
    /// This is only returned for refresh mode.
    payload: ?RefreshStatisticsIdMap = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ManagePropertygraphStatisticsInput, options: CallOptions) !ManagePropertygraphStatisticsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ManagePropertygraphStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ManagePropertygraphStatisticsOutput {
    var result: ManagePropertygraphStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ManagePropertygraphStatisticsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
