...
fn serializeRequest(alloc: std.mem.Allocator, input: ManagePropertygraphStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ManagePropertygraphStatisticsOutput {
    var result: ManagePropertygraphStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ManagePropertygraphStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
