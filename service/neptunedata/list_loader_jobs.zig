...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLoaderJobsInput, options: Options) !ListLoaderJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLoaderJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_queued_loads) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeQueuedLoads=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLoaderJobsOutput {
    var result: ListLoaderJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLoaderJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
