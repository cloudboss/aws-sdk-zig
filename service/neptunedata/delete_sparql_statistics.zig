...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSparqlStatisticsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSparqlStatisticsOutput {
    var result: DeleteSparqlStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteSparqlStatisticsOutput, body, alloc);
    }
