...
const GetPropertygraphStatisticsInput = struct {};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPropertygraphStatisticsInput, options: Options) !GetPropertygraphStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPropertygraphStatisticsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPropertygraphStatisticsOutput {
    var result: GetPropertygraphStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPropertygraphStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
