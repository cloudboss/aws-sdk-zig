...
pub const CancelGremlinQueryInput = struct {
...
pub const CancelGremlinQueryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelGremlinQueryInput, options: Options) !CancelGremlinQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelGremlinQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/gremlin/status/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelGremlinQueryOutput {
    var result: CancelGremlinQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelGremlinQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
