...
pub const CancelMLModelTrainingJobInput = struct {
...
pub const CancelMLModelTrainingJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMLModelTrainingJobInput, options: Options) !CancelMLModelTrainingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelMLModelTrainingJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ml/modeltraining/");
    ...
    if (input.clean) |v| {
        ...
        try query_buf.appendSlice(alloc, "clean=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.neptune_iam_role_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "neptuneIamRoleArn=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelMLModelTrainingJobOutput {
    var result: CancelMLModelTrainingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelMLModelTrainingJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
