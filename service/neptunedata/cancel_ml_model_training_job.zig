...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMLModelTrainingJobInput, options: CallOptions) !CancelMLModelTrainingJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelMLModelTrainingJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelMLModelTrainingJobOutput {
    var result: CancelMLModelTrainingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelMLModelTrainingJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
