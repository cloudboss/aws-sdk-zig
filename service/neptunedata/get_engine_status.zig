...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEngineStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEngineStatusOutput {
    var result: GetEngineStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEngineStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
