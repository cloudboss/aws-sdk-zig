...
pub const ListMLEndpointsInput = struct {
...
pub const ListMLEndpointsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMLEndpointsInput, options: CallOptions) !ListMLEndpointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMLEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMLEndpointsOutput {
    var result: ListMLEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMLEndpointsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
