...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMLEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMLEndpointsOutput {
    var result: ListMLEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMLEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
