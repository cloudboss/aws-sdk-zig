...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGremlinQueryStatusInput, options: Options) !GetGremlinQueryStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGremlinQueryStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGremlinQueryStatusOutput {
    var result: GetGremlinQueryStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGremlinQueryStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
