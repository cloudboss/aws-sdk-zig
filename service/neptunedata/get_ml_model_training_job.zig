...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMLModelTrainingJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMLModelTrainingJobOutput {
    var result: GetMLModelTrainingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMLModelTrainingJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
