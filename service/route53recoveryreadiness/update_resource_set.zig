...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResourceSetInput, options: CallOptions) !UpdateResourceSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateResourceSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateResourceSetOutput {
    var result: UpdateResourceSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResourceSetOutput, body, allocator);
    }
    _ = status;
    _ = headers;
