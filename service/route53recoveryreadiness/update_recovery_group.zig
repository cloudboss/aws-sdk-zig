...
pub const UpdateRecoveryGroupInput = struct {
...
pub const UpdateRecoveryGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRecoveryGroupInput, options: CallOptions) !UpdateRecoveryGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRecoveryGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRecoveryGroupOutput {
    var result: UpdateRecoveryGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRecoveryGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
