...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReadinessCheckInput, options: Options) !GetReadinessCheckOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetReadinessCheckOutput {
    var result: GetReadinessCheckOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReadinessCheckOutput, body, allocator);
    }
    _ = status;
    _ = headers;
