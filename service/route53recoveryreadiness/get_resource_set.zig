...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceSetInput, options: Options) !GetResourceSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceSetOutput {
    var result: GetResourceSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
