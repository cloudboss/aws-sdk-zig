...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProbeInput, options: Options) !CreateProbeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProbeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"probe\":");
    try aws.json.writeValue(@TypeOf(input.probe), input.probe, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProbeOutput {
    var result: CreateProbeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProbeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
