...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProbeInput, options: Options) !GetProbeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProbeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProbeOutput {
    var result: GetProbeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProbeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
