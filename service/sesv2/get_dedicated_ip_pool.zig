...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDedicatedIpPoolInput, options: Options) !GetDedicatedIpPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDedicatedIpPoolInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDedicatedIpPoolOutput {
    var result: GetDedicatedIpPoolOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDedicatedIpPoolOutput, body, alloc);
    }
    _ = status;
    _ = headers;
