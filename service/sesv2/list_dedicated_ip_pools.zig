...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDedicatedIpPoolsInput, options: Options) !ListDedicatedIpPoolsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDedicatedIpPoolsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDedicatedIpPoolsOutput {
    var result: ListDedicatedIpPoolsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDedicatedIpPoolsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
