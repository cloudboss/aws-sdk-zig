...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateReputationEntityPolicyInput, options: Options) !UpdateReputationEntityPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateReputationEntityPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ReputationEntityPolicy\":");
    try aws.json.writeValue(@TypeOf(input.reputation_entity_policy), input.reputation_entity_policy, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateReputationEntityPolicyOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
