...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReputationEntityInput, options: Options) !GetReputationEntityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReputationEntityInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReputationEntityOutput {
    var result: GetReputationEntityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReputationEntityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
