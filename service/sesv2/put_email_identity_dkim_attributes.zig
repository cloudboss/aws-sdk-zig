...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutEmailIdentityDkimAttributesInput, options: Options) !PutEmailIdentityDkimAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEmailIdentityDkimAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dkim");
    ...
    if (input.signing_enabled) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"SigningEnabled\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEmailIdentityDkimAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
