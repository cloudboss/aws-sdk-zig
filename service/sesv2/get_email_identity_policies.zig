...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEmailIdentityPoliciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEmailIdentityPoliciesOutput {
    var result: GetEmailIdentityPoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEmailIdentityPoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
