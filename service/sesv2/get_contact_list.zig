...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContactListInput, options: Options) !GetContactListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContactListInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContactListOutput {
    var result: GetContactListOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContactListOutput, body, alloc);
    }
    _ = status;
    _ = headers;
