...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfigurationSetEventDestinationInput, options: Options) !UpdateConfigurationSetEventDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfigurationSetEventDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/configuration-sets/");
    try path_buf.appendSlice(alloc, input.configuration_set_name);
    try path_buf.appendSlice(alloc, "/event-destinations/");
    try path_buf.appendSlice(alloc, input.event_destination_name);
    ...
    try body_buf.appendSlice(alloc, "\"EventDestination\":");
    try aws.json.writeValue(@TypeOf(input.event_destination), input.event_destination, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfigurationSetEventDestinationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
