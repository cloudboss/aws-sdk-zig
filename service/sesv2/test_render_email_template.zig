...
pub const TestRenderEmailTemplateInput = struct {
...
pub const TestRenderEmailTemplateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestRenderEmailTemplateInput, options: Options) !TestRenderEmailTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestRenderEmailTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/templates/");
    try path_buf.appendSlice(alloc, input.template_name);
    try path_buf.appendSlice(alloc, "/render");
    ...
    try body_buf.appendSlice(alloc, "\"TemplateData\":");
    try aws.json.writeValue(@TypeOf(input.template_data), input.template_data, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestRenderEmailTemplateOutput {
    var result: TestRenderEmailTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TestRenderEmailTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
