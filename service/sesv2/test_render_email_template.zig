...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestRenderEmailTemplateInput, options: CallOptions) !TestRenderEmailTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TestRenderEmailTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.template_data), input.template_data, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TestRenderEmailTemplateOutput {
    var result: TestRenderEmailTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TestRenderEmailTemplateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
