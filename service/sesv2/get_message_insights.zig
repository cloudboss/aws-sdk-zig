...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMessageInsightsInput, options: Options) !GetMessageInsightsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMessageInsightsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/insights/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMessageInsightsOutput {
    var result: GetMessageInsightsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMessageInsightsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
