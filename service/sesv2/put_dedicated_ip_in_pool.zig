...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDedicatedIpInPoolInput, options: Options) !PutDedicatedIpInPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDedicatedIpInPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/dedicated-ips/");
    try path_buf.appendSlice(alloc, input.ip);
    try path_buf.appendSlice(alloc, "/pool");
    ...
    try body_buf.appendSlice(alloc, "\"DestinationPoolName\":");
    try aws.json.writeValue(@TypeOf(input.destination_pool_name), input.destination_pool_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDedicatedIpInPoolOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutDedicatedIpInPoolOutput = .{};
