...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceTenantsInput, options: Options) !ListResourceTenantsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceTenantsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceTenantsOutput {
    var result: ListResourceTenantsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourceTenantsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
