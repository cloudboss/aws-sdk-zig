...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateContactListInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"ContactListName\":");
    try aws.json.writeValue(@TypeOf(input.contact_list_name), input.contact_list_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateContactListOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
