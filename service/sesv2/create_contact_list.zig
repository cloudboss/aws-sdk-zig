...
pub const CreateContactListInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContactListInput, options: Options) !CreateContactListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContactListInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ContactListName\":");
    try aws.json.writeValue(@TypeOf(input.contact_list_name), input.contact_list_name, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.topics) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Topics\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContactListOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateContactListOutput = .{};
