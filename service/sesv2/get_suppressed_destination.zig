...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSuppressedDestinationInput, options: Options) !GetSuppressedDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSuppressedDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/suppression/addresses/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSuppressedDestinationOutput {
    var result: GetSuppressedDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSuppressedDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
