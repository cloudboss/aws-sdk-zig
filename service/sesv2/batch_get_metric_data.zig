...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetMetricDataInput, options: Options) !BatchGetMetricDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetMetricDataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetMetricDataOutput {
    var result: BatchGetMetricDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetMetricDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
