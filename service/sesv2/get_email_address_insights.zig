...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEmailAddressInsightsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEmailAddressInsightsOutput {
    var result: GetEmailAddressInsightsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEmailAddressInsightsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
