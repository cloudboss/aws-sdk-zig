...
const MailboxValidation = @import("mailbox_validation.zig").MailboxValidation;

pub const GetEmailAddressInsightsInput = struct {
...
pub const GetEmailAddressInsightsOutput = struct {
    /// Detailed validation results for the email address.
    mailbox_validation: ?MailboxValidation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEmailAddressInsightsInput, options: CallOptions) !GetEmailAddressInsightsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetEmailAddressInsightsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetEmailAddressInsightsOutput {
    var result: GetEmailAddressInsightsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEmailAddressInsightsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
