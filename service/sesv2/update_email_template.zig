...
pub const UpdateEmailTemplateInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEmailTemplateInput, options: Options) !UpdateEmailTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEmailTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.template_content), input.template_content, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEmailTemplateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateEmailTemplateOutput = .{};
