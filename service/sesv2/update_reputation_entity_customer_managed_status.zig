...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateReputationEntityCustomerManagedStatusInput, options: Options) !UpdateReputationEntityCustomerManagedStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateReputationEntityCustomerManagedStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/reputation/entities/");
    try path_buf.appendSlice(alloc, input.reputation_entity_type);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.reputation_entity_reference);
    try path_buf.appendSlice(alloc, "/customer-managed-status");
    ...
    try body_buf.appendSlice(alloc, "\"SendingStatus\":");
    try aws.json.writeValue(@TypeOf(input.sending_status), input.sending_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateReputationEntityCustomerManagedStatusOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
