...
pub const PutAccountVdmAttributesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccountVdmAttributesInput, options: Options) !PutAccountVdmAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutAccountVdmAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v2/email/account/vdm";
    ...
    try body_buf.appendSlice(allocator, "\"VdmAttributes\":");
    try aws.json.writeValue(@TypeOf(input.vdm_attributes), input.vdm_attributes, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutAccountVdmAttributesOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutAccountVdmAttributesOutput = .{};
