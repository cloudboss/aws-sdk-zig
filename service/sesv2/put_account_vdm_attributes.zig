...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccountVdmAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"VdmAttributes\":");
    try aws.json.writeValue(@TypeOf(input.vdm_attributes), input.vdm_attributes, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccountVdmAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
