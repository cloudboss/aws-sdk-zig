...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMultiRegionEndpointInput, options: Options) !DeleteMultiRegionEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMultiRegionEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/multi-region-endpoints/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMultiRegionEndpointOutput {
    var result: DeleteMultiRegionEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteMultiRegionEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
