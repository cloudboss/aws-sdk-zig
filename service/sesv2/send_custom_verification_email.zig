...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendCustomVerificationEmailInput, options: Options) !SendCustomVerificationEmailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendCustomVerificationEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v2/email/outbound-custom-verification-emails";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendCustomVerificationEmailOutput {
    var result: SendCustomVerificationEmailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendCustomVerificationEmailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
