...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEmailIdentityInput, options: Options) !CreateEmailIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEmailIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dkim_signing_attributes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DkimSigningAttributes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EmailIdentity\":");
    try aws.json.writeValue(@TypeOf(input.email_identity), input.email_identity, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEmailIdentityOutput {
    var result: CreateEmailIdentityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEmailIdentityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
