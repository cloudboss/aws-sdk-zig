...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDomainDeliverabilityCampaignsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/deliverability-dashboard/domains/");
    try path_buf.appendSlice(alloc, input.subscribed_domain);
    ...
    {
        const num_str = std.fmt.allocPrint(alloc, "{d}", .{input.end_date}) catch "";
    ...
    {
        const num_str = std.fmt.allocPrint(alloc, "{d}", .{input.start_date}) catch "";
        try query_buf.appendSlice(alloc, num_str);
    }
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDomainDeliverabilityCampaignsOutput {
    var result: ListDomainDeliverabilityCampaignsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDomainDeliverabilityCampaignsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
