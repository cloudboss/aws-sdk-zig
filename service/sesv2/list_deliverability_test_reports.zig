...
pub const ListDeliverabilityTestReportsInput = struct {
...
pub const ListDeliverabilityTestReportsOutput = struct {
    /// An object that contains a lists of predictive inbox placement tests that
    /// you've performed.
    deliverability_test_reports: ?[]const DeliverabilityTestReport = null,

    /// A token that indicates that there are additional predictive inbox placement
    /// tests to list. To view additional
    /// predictive inbox placement tests, issue another request to
    /// `ListDeliverabilityTestReports`, and pass
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeliverabilityTestReportsInput, options: Options) !ListDeliverabilityTestReportsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDeliverabilityTestReportsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v2/email/deliverability-dashboard/test-reports";
    ...
    if (input.page_size) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "PageSize=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDeliverabilityTestReportsOutput {
    var result: ListDeliverabilityTestReportsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDeliverabilityTestReportsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
