...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeliverabilityTestReportsInput, options: CallOptions) !ListDeliverabilityTestReportsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDeliverabilityTestReportsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDeliverabilityTestReportsOutput {
    var result: ListDeliverabilityTestReportsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDeliverabilityTestReportsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
