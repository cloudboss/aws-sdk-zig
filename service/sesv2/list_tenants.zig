...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTenantsInput, options: Options) !ListTenantsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTenantsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTenantsOutput {
    var result: ListTenantsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTenantsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
