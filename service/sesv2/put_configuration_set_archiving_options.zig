...
pub const PutConfigurationSetArchivingOptionsInput = struct {
    /// The Amazon Resource Name (ARN) of the MailManager archive that the Amazon
    /// SES API v2 sends email
    /// to.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutConfigurationSetArchivingOptionsInput, options: Options) !PutConfigurationSetArchivingOptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutConfigurationSetArchivingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/archiving-options");
    ...
    if (input.archive_arn) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ArchiveArn\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutConfigurationSetArchivingOptionsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutConfigurationSetArchivingOptionsOutput = .{};
