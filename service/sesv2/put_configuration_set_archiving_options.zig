...
fn serializeRequest(alloc: std.mem.Allocator, input: PutConfigurationSetArchivingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/archiving-options");
    ...
    if (input.archive_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ArchiveArn\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutConfigurationSetArchivingOptionsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
