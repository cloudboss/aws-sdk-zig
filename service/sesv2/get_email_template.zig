...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEmailTemplateInput, options: Options) !GetEmailTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEmailTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEmailTemplateOutput {
    var result: GetEmailTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEmailTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
