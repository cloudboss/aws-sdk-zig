...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ExportDataSource\":");
    try aws.json.writeValue(@TypeOf(input.export_data_source), input.export_data_source, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ExportDestination\":");
    try aws.json.writeValue(@TypeOf(input.export_destination), input.export_destination, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateExportJobOutput {
    var result: CreateExportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateExportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
