...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMultiRegionEndpointInput, options: Options) !GetMultiRegionEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMultiRegionEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMultiRegionEndpointOutput {
    var result: GetMultiRegionEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMultiRegionEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
