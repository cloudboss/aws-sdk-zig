...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContactInput, options: Options) !UpdateContactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContactInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/contact-lists/");
    try path_buf.appendSlice(alloc, input.contact_list_name);
    try path_buf.appendSlice(alloc, "/contacts/");
    try path_buf.appendSlice(alloc, input.email_address);
    ...
    if (input.attributes_data) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"AttributesData\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.topic_preferences) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TopicPreferences\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.unsubscribe_all) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UnsubscribeAll\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContactOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
