...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMultiRegionEndpointInput, options: Options) !CreateMultiRegionEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMultiRegionEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMultiRegionEndpointOutput {
    var result: CreateMultiRegionEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMultiRegionEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
