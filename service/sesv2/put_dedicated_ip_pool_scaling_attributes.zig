...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDedicatedIpPoolScalingAttributesInput, options: Options) !PutDedicatedIpPoolScalingAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDedicatedIpPoolScalingAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v2/email/dedicated-ip-pools/");
    try path_buf.appendSlice(alloc, input.pool_name);
    try path_buf.appendSlice(alloc, "/scaling");
    ...
    try body_buf.appendSlice(alloc, "\"ScalingMode\":");
    try aws.json.writeValue(@TypeOf(input.scaling_mode), input.scaling_mode, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDedicatedIpPoolScalingAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
