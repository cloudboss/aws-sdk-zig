...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccountSendingAttributesInput, options: Options) !PutAccountSendingAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccountSendingAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.sending_enabled) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"SendingEnabled\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccountSendingAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
