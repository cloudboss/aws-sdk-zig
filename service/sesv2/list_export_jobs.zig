...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExportJobsInput, options: Options) !ListExportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.export_source_type) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ExportSourceType\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExportJobsOutput {
    var result: ListExportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListExportJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
