...
const CustomVerificationEmailTemplateMetadata = @import("custom_verification_email_template_metadata.zig").CustomVerificationEmailTemplateMetadata;
...
pub const ListCustomVerificationEmailTemplatesOutput = struct {
    ...
    /// account.
    custom_verification_email_templates: ?[]const CustomVerificationEmailTemplateMetadata = null,
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomVerificationEmailTemplatesOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomVerificationEmailTemplatesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
