...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateImportJobInput, options: Options) !CreateImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v2/email/import-jobs";
    ...
    try body_buf.appendSlice(alloc, "\"ImportDataSource\":");
    try aws.json.writeValue(@TypeOf(input.import_data_source), input.import_data_source, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ImportDestination\":");
    try aws.json.writeValue(@TypeOf(input.import_destination), input.import_destination, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateImportJobOutput {
    var result: CreateImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
