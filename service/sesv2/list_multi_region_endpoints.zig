...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMultiRegionEndpointsInput, options: Options) !ListMultiRegionEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMultiRegionEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMultiRegionEndpointsOutput {
    var result: ListMultiRegionEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMultiRegionEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
