...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEmailIdentityDkimSigningAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dkim/signing");
    ...
    if (input.signing_attributes) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"SigningAttributes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SigningAttributesOrigin\":");
    try aws.json.writeValue(@TypeOf(input.signing_attributes_origin), input.signing_attributes_origin, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEmailIdentityDkimSigningAttributesOutput {
    var result: PutEmailIdentityDkimSigningAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutEmailIdentityDkimSigningAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
