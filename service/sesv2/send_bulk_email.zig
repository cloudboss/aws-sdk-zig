...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendBulkEmailInput, options: Options) !SendBulkEmailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendBulkEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BulkEmailEntries\":");
    try aws.json.writeValue(@TypeOf(input.bulk_email_entries), input.bulk_email_entries, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DefaultContent\":");
    try aws.json.writeValue(@TypeOf(input.default_content), input.default_content, alloc, &body_buf);
    has_prev = true;
    if (input.default_email_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DefaultEmailTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.endpoint_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EndpointId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.feedback_forwarding_email_address) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FeedbackForwardingEmailAddress\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.feedback_forwarding_email_address_identity_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FeedbackForwardingEmailAddressIdentityArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.from_email_address_identity_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FromEmailAddressIdentityArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.reply_to_addresses) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ReplyToAddresses\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tenant_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TenantName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendBulkEmailOutput {
    var result: SendBulkEmailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendBulkEmailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
