...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccountSuppressionAttributesInput, options: Options) !PutAccountSuppressionAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccountSuppressionAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.validation_attributes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ValidationAttributes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccountSuppressionAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
