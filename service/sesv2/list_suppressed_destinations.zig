...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSuppressedDestinationsInput, options: Options) !ListSuppressedDestinationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSuppressedDestinationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.reasons) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "Reason=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSuppressedDestinationsOutput {
    var result: ListSuppressedDestinationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSuppressedDestinationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
