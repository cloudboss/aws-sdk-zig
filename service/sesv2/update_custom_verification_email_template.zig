...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCustomVerificationEmailTemplateInput, options: Options) !UpdateCustomVerificationEmailTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCustomVerificationEmailTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.failure_redirection_url), input.failure_redirection_url, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.success_redirection_url), input.success_redirection_url, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.template_subject), input.template_subject, alloc, &body_buf);
    has_prev = true;
