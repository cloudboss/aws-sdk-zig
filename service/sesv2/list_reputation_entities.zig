...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReputationEntitiesInput, options: Options) !ListReputationEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReputationEntitiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReputationEntitiesOutput {
    var result: ListReputationEntitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReputationEntitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
