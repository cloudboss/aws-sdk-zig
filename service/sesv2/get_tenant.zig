...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTenantInput, options: CallOptions) !GetTenantOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTenantInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTenantOutput {
    var result: GetTenantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTenantOutput, body, allocator);
    }
    _ = status;
    _ = headers;
