...
pub const GetTenantInput = struct {
...
pub const GetTenantOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTenantInput, options: Options) !GetTenantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTenantInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v2/email/tenants/get";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTenantOutput {
    var result: GetTenantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTenantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
