...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTenantResourcesInput, options: Options) !ListTenantResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTenantResourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTenantResourcesOutput {
    var result: ListTenantResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTenantResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
