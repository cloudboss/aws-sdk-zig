...
pub const DeleteTenantResourceAssociationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTenantResourceAssociationInput, options: Options) !DeleteTenantResourceAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTenantResourceAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v2/email/tenants/resources/delete";
    ...
    try aws.json.writeValue(@TypeOf(input.tenant_name), input.tenant_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTenantResourceAssociationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteTenantResourceAssociationOutput = .{};
