...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContactListsInput, options: CallOptions) !ListContactListsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListContactListsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListContactListsOutput {
    var result: ListContactListsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactListsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
