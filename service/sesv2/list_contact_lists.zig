...
const ContactList = @import("contact_list.zig").ContactList;

pub const ListContactListsInput = struct {
    /// A string token indicating that there might be additional contact lists
    /// available to be
    /// listed. Use the token provided in the Response to use in the subsequent call
    /// to
...
pub const ListContactListsOutput = struct {
    /// The available contact lists.
    contact_lists: ?[]const ContactList = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContactListsInput, options: Options) !ListContactListsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContactListsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v2/email/contact-lists";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContactListsOutput {
    var result: ListContactListsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContactListsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
