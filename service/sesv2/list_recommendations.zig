...
pub const ListRecommendationsInput = struct {
    ...
    /// filter, or
    /// combinations of `STATUS` and `IMPACT` or
    /// `STATUS` and `TYPE`
...
pub const ListRecommendationsOutput = struct {
    ...
    /// available to be
    /// listed. Use the token provided in the `ListRecommendationsResponse` to use
    /// in the
    /// subsequent call to `ListRecommendations` with the same parameters to
    /// retrieve the
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v2/email/vdm/recommendations";
