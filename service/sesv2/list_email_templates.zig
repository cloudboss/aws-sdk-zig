...
const EmailTemplateMetadata = @import("email_template_metadata.zig").EmailTemplateMetadata;

pub const ListEmailTemplatesInput = struct {
...
pub const ListEmailTemplatesOutput = struct {
    ...
    /// your Amazon SES
    /// account.
    templates_metadata: ?[]const EmailTemplateMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEmailTemplatesInput, options: Options) !ListEmailTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEmailTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEmailTemplatesOutput {
    var result: ListEmailTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEmailTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
