...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutConfigurationSetSuppressionOptionsInput, options: Options) !PutConfigurationSetSuppressionOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutConfigurationSetSuppressionOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/suppression-options");
    ...
    if (input.suppressed_reasons) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"SuppressedReasons\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.validation_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ValidationOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutConfigurationSetSuppressionOptionsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
