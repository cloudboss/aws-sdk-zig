...
fn serializeRequest(alloc: std.mem.Allocator, input: StartInstanceOnboardingJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartInstanceOnboardingJobOutput {
    var result: StartInstanceOnboardingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartInstanceOnboardingJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
