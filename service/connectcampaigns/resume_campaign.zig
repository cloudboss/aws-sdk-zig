...
pub const ResumeCampaignOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResumeCampaignInput, options: CallOptions) !ResumeCampaignOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResumeCampaignInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/resume");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResumeCampaignOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
