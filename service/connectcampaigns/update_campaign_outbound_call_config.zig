...
const UpdateCampaignOutboundCallConfigOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCampaignOutboundCallConfigInput, options: Options) !UpdateCampaignOutboundCallConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCampaignOutboundCallConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/outbound-call-config");
    ...
    if (input.connect_contact_flow_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"connectContactFlowId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.connect_source_phone_number) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"connectSourcePhoneNumber\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCampaignOutboundCallConfigOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
