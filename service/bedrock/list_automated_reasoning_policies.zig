...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutomatedReasoningPoliciesInput, options: Options) !ListAutomatedReasoningPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutomatedReasoningPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.policy_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "policyArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutomatedReasoningPoliciesOutput {
    var result: ListAutomatedReasoningPoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutomatedReasoningPoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
