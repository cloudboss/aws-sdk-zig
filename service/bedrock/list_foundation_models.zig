...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFoundationModelsInput, options: Options) !ListFoundationModelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFoundationModelsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.by_customization_type) |v| {
        ...
        try query_buf.appendSlice(alloc, "byCustomizationType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.by_inference_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "byInferenceType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.by_output_modality) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "byOutputModality=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.by_provider) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "byProvider=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFoundationModelsOutput {
    var result: ListFoundationModelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFoundationModelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
