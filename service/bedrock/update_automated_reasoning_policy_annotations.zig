...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAutomatedReasoningPolicyAnnotationsInput, options: Options) !UpdateAutomatedReasoningPolicyAnnotationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAutomatedReasoningPolicyAnnotationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"annotations\":");
    try aws.json.writeValue(@TypeOf(input.annotations), input.annotations, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"lastUpdatedAnnotationSetHash\":");
    try aws.json.writeValue(@TypeOf(input.last_updated_annotation_set_hash), input.last_updated_annotation_set_hash, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAutomatedReasoningPolicyAnnotationsOutput {
    var result: UpdateAutomatedReasoningPolicyAnnotationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAutomatedReasoningPolicyAnnotationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
