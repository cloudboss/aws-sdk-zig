...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteEvaluationJobInput, options: Options) !BatchDeleteEvaluationJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteEvaluationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"jobIdentifiers\":");
    try aws.json.writeValue(@TypeOf(input.job_identifiers), input.job_identifiers, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteEvaluationJobOutput {
    var result: BatchDeleteEvaluationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteEvaluationJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
