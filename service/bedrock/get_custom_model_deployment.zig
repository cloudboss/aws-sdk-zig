...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCustomModelDeploymentInput, options: Options) !GetCustomModelDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCustomModelDeploymentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCustomModelDeploymentOutput {
    var result: GetCustomModelDeploymentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCustomModelDeploymentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
