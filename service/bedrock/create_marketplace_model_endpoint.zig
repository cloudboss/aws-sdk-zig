...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMarketplaceModelEndpointInput, options: Options) !CreateMarketplaceModelEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMarketplaceModelEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.accept_eula) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"acceptEula\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"endpointConfig\":");
    try aws.json.writeValue(@TypeOf(input.endpoint_config), input.endpoint_config, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"endpointName\":");
    try aws.json.writeValue(@TypeOf(input.endpoint_name), input.endpoint_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"modelSourceIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.model_source_identifier), input.model_source_identifier, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMarketplaceModelEndpointOutput {
    var result: CreateMarketplaceModelEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMarketplaceModelEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
