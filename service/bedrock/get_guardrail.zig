...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGuardrailInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGuardrailOutput {
    var result: GetGuardrailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGuardrailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
