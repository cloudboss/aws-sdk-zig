...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMarketplaceModelEndpointInput, options: Options) !UpdateMarketplaceModelEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMarketplaceModelEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/marketplace-model/endpoints/");
    try path_buf.appendSlice(alloc, input.endpoint_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMarketplaceModelEndpointOutput {
    var result: UpdateMarketplaceModelEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMarketplaceModelEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
