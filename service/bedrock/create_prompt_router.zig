...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePromptRouterInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"fallbackModel\":");
    try aws.json.writeValue(@TypeOf(input.fallback_model), input.fallback_model, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"models\":");
    try aws.json.writeValue(@TypeOf(input.models), input.models, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"promptRouterName\":");
    try aws.json.writeValue(@TypeOf(input.prompt_router_name), input.prompt_router_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"routingCriteria\":");
    try aws.json.writeValue(@TypeOf(input.routing_criteria), input.routing_criteria, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePromptRouterOutput {
    var result: CreatePromptRouterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePromptRouterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
