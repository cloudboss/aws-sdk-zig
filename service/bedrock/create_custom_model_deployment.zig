...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomModelDeploymentInput, options: Options) !CreateCustomModelDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomModelDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"modelArn\":");
    try aws.json.writeValue(@TypeOf(input.model_arn), input.model_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"modelDeploymentName\":");
    try aws.json.writeValue(@TypeOf(input.model_deployment_name), input.model_deployment_name, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomModelDeploymentOutput {
    var result: CreateCustomModelDeploymentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCustomModelDeploymentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
