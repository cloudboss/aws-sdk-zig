...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutomatedReasoningPolicyTestCaseInput, options: Options) !GetAutomatedReasoningPolicyTestCaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAutomatedReasoningPolicyTestCaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAutomatedReasoningPolicyTestCaseOutput {
    var result: GetAutomatedReasoningPolicyTestCaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAutomatedReasoningPolicyTestCaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
