...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomModelInput, options: Options) !CreateCustomModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomModelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.model_kms_key_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"modelKmsKeyArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"modelName\":");
    try aws.json.writeValue(@TypeOf(input.model_name), input.model_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"modelSourceConfig\":");
    try aws.json.writeValue(@TypeOf(input.model_source_config), input.model_source_config, alloc, &body_buf);
    has_prev = true;
    if (input.model_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"modelTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.role_arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomModelOutput {
    var result: CreateCustomModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCustomModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
