...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateModelCopyJobInput, options: Options) !CreateModelCopyJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateModelCopyJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.model_kms_key_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"modelKmsKeyId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sourceModelArn\":");
    try aws.json.writeValue(@TypeOf(input.source_model_arn), input.source_model_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"targetModelName\":");
    try aws.json.writeValue(@TypeOf(input.target_model_name), input.target_model_name, alloc, &body_buf);
    has_prev = true;
    if (input.target_model_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"targetModelTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateModelCopyJobOutput {
    var result: CreateModelCopyJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateModelCopyJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
