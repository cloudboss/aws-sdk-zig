...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEvaluationJobInput, options: CallOptions) !GetEvaluationJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetEvaluationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/evaluation-jobs/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetEvaluationJobOutput {
    var result: GetEvaluationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEvaluationJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
