...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProvisionedModelThroughputsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.model_arn_equals) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "modelArnEquals=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProvisionedModelThroughputsOutput {
    var result: ListProvisionedModelThroughputsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProvisionedModelThroughputsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
