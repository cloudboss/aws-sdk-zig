...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInferenceProfileInput, options: Options) !CreateInferenceProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInferenceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"inferenceProfileName\":");
    try aws.json.writeValue(@TypeOf(input.inference_profile_name), input.inference_profile_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"modelSource\":");
    try aws.json.writeValue(@TypeOf(input.model_source), input.model_source, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInferenceProfileOutput {
    var result: CreateInferenceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateInferenceProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
