...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImportedModelsInput, options: CallOptions) !ListImportedModelsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListImportedModelsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListImportedModelsOutput {
    var result: ListImportedModelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImportedModelsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
