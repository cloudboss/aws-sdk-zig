...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFoundationModelAgreementInput, options: CallOptions) !CreateFoundationModelAgreementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateFoundationModelAgreementInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"offerToken\":");
    try aws.json.writeValue(@TypeOf(input.offer_token), input.offer_token, allocator, &body_buf);
    has_prev = true;
