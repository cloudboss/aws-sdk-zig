...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutomatedReasoningPolicyTestCasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutomatedReasoningPolicyTestCasesOutput {
    var result: ListAutomatedReasoningPolicyTestCasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutomatedReasoningPolicyTestCasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
