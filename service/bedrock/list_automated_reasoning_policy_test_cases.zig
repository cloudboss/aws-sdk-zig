...
pub const ListAutomatedReasoningPolicyTestCasesOutput = struct {
    /// The pagination token to use in a subsequent request to retrieve the next
    /// page of results.
    next_token: ?[]const u8 = null,

    /// A list of tests for the specified policy.
    test_cases: ?[]const AutomatedReasoningPolicyTestCase = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutomatedReasoningPolicyTestCasesInput, options: CallOptions) !ListAutomatedReasoningPolicyTestCasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAutomatedReasoningPolicyTestCasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAutomatedReasoningPolicyTestCasesOutput {
    var result: ListAutomatedReasoningPolicyTestCasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutomatedReasoningPolicyTestCasesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
