...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInferenceProfilesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInferenceProfilesOutput {
    var result: ListInferenceProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInferenceProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
