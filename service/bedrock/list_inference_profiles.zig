...
const InferenceProfileSummary = @import("inference_profile_summary.zig").InferenceProfileSummary;

pub const ListInferenceProfilesInput = struct {
    ...
    type_equals: ?InferenceProfileType = null,
...
pub const ListInferenceProfilesOutput = struct {
    /// A list of information about each inference profile that you can use.
    inference_profile_summaries: ?[]const InferenceProfileSummary = null,
    ...
    pub const json_field_names = .{
        .inference_profile_summaries = "inferenceProfileSummaries",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInferenceProfilesInput, options: CallOptions) !ListInferenceProfilesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListInferenceProfilesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListInferenceProfilesOutput {
    var result: ListInferenceProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInferenceProfilesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
