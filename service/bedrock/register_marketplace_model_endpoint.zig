...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterMarketplaceModelEndpointInput, options: CallOptions) !RegisterMarketplaceModelEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterMarketplaceModelEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/registration");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterMarketplaceModelEndpointOutput {
    var result: RegisterMarketplaceModelEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterMarketplaceModelEndpointOutput, body, allocator);
    }
    _ = status;
    _ = headers;
