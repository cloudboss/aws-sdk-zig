...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGuardrailVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGuardrailVersionOutput {
    var result: CreateGuardrailVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGuardrailVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
