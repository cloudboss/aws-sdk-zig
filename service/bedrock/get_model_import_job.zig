...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetModelImportJobInput, options: Options) !GetModelImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetModelImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/model-import-jobs/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetModelImportJobOutput {
    var result: GetModelImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetModelImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
