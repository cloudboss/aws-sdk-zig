...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelCopyJobsInput, options: Options) !ListModelCopyJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelCopyJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.source_account_equals) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sourceAccountEquals=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.source_model_arn_equals) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sourceModelArnEquals=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.target_model_name_contains) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "outputModelNameContains=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelCopyJobsOutput {
    var result: ListModelCopyJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListModelCopyJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
