...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutomatedReasoningPolicyBuildWorkflowsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutomatedReasoningPolicyBuildWorkflowsOutput {
    var result: ListAutomatedReasoningPolicyBuildWorkflowsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutomatedReasoningPolicyBuildWorkflowsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
