...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAutomatedReasoningPolicyTestCaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAutomatedReasoningPolicyTestCaseOutput {
    var result: CreateAutomatedReasoningPolicyTestCaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAutomatedReasoningPolicyTestCaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
