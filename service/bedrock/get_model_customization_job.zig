...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetModelCustomizationJobInput, options: Options) !GetModelCustomizationJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetModelCustomizationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/model-customization-jobs/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetModelCustomizationJobOutput {
    var result: GetModelCustomizationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetModelCustomizationJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
