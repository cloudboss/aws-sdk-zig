...
pub const UpdateAutomatedReasoningPolicyInput = struct {
    ...
    /// The updated policy definition containing the formal logic rules, variables,
    /// and types.
...
pub const UpdateAutomatedReasoningPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAutomatedReasoningPolicyInput, options: Options) !UpdateAutomatedReasoningPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAutomatedReasoningPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"policyDefinition\":");
    try aws.json.writeValue(@TypeOf(input.policy_definition), input.policy_definition, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAutomatedReasoningPolicyOutput {
    var result: UpdateAutomatedReasoningPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAutomatedReasoningPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
