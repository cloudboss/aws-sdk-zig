...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetModelInvocationJobInput, options: Options) !GetModelInvocationJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetModelInvocationJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetModelInvocationJobOutput {
    var result: GetModelInvocationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetModelInvocationJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
