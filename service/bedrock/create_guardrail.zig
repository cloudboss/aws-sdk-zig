...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGuardrailInput, options: Options) !CreateGuardrailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGuardrailInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGuardrailOutput {
    var result: CreateGuardrailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGuardrailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
