...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomModelDeploymentsInput, options: Options) !ListCustomModelDeploymentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCustomModelDeploymentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCustomModelDeploymentsOutput {
    var result: ListCustomModelDeploymentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomModelDeploymentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
