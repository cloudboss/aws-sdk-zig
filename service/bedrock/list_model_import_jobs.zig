...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelImportJobsInput, options: CallOptions) !ListModelImportJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListModelImportJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListModelImportJobsOutput {
    var result: ListModelImportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListModelImportJobsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
