...
const ModelImportJobSummary = @import("model_import_job_summary.zig").ModelImportJobSummary;

pub const ListModelImportJobsInput = struct {
    ...
    status_equals: ?ModelImportJobStatus = null,
...
pub const ListModelImportJobsOutput = struct {
    /// Import job summaries.
    model_import_job_summaries: ?[]const ModelImportJobSummary = null,
    ...
    pub const json_field_names = .{
        .model_import_job_summaries = "modelImportJobSummaries",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelImportJobsInput, options: Options) !ListModelImportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelImportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.status_equals) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "statusEquals=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelImportJobsOutput {
    var result: ListModelImportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListModelImportJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
