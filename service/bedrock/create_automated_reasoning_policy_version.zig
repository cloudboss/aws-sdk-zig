...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAutomatedReasoningPolicyVersionInput, options: Options) !CreateAutomatedReasoningPolicyVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAutomatedReasoningPolicyVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"lastUpdatedDefinitionHash\":");
    try aws.json.writeValue(@TypeOf(input.last_updated_definition_hash), input.last_updated_definition_hash, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAutomatedReasoningPolicyVersionOutput {
    var result: CreateAutomatedReasoningPolicyVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAutomatedReasoningPolicyVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
