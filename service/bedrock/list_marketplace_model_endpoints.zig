...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMarketplaceModelEndpointsInput, options: Options) !ListMarketplaceModelEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMarketplaceModelEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.model_source_equals) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "modelSourceIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMarketplaceModelEndpointsOutput {
    var result: ListMarketplaceModelEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMarketplaceModelEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
