...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAutomatedReasoningPolicyTestWorkflowInput, options: Options) !StartAutomatedReasoningPolicyTestWorkflowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartAutomatedReasoningPolicyTestWorkflowInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/test-workflows");
    ...
    if (input.test_case_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"testCaseIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartAutomatedReasoningPolicyTestWorkflowOutput {
    var result: StartAutomatedReasoningPolicyTestWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartAutomatedReasoningPolicyTestWorkflowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
