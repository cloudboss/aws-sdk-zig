...
pub const GetImportedModelInput = struct {
...
pub const GetImportedModelOutput = struct {
    ...
    pub const json_field_names = .{
        ...
        .custom_model_units = "customModelUnits",
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImportedModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/imported-models/");
    try path_buf.appendSlice(alloc, input.model_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImportedModelOutput {
    var result: GetImportedModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImportedModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
