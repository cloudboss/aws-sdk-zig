...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutomatedReasoningPolicyTestResultInput, options: Options) !GetAutomatedReasoningPolicyTestResultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAutomatedReasoningPolicyTestResultInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAutomatedReasoningPolicyTestResultOutput {
    var result: GetAutomatedReasoningPolicyTestResultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAutomatedReasoningPolicyTestResultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
