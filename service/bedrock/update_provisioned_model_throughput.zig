...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProvisionedModelThroughputInput, options: Options) !UpdateProvisionedModelThroughputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProvisionedModelThroughputInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.desired_model_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"desiredModelId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.desired_provisioned_model_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"desiredProvisionedModelName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProvisionedModelThroughputOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
