...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCustomModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCustomModelOutput {
    var result: GetCustomModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCustomModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
