...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEvaluationJobsInput, options: Options) !ListEvaluationJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEvaluationJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.application_type_equals) |v| {
        ...
        try query_buf.appendSlice(alloc, "applicationTypeEquals=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEvaluationJobsOutput {
    var result: ListEvaluationJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEvaluationJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
