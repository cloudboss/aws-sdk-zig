...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProvisionedModelThroughputInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"modelUnits\":");
    try aws.json.writeValue(@TypeOf(input.model_units), input.model_units, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"provisionedModelName\":");
    try aws.json.writeValue(@TypeOf(input.provisioned_model_name), input.provisioned_model_name, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProvisionedModelThroughputOutput {
    var result: CreateProvisionedModelThroughputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProvisionedModelThroughputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
