...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateModelCustomizationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"baseModelIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.base_model_identifier), input.base_model_identifier, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.customization_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"customizationConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.customization_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"customizationType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.custom_model_kms_key_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"customModelKmsKeyId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"customModelName\":");
    try aws.json.writeValue(@TypeOf(input.custom_model_name), input.custom_model_name, alloc, &body_buf);
    has_prev = true;
    if (input.custom_model_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"customModelTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.hyper_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"hyperParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"trainingDataConfig\":");
    try aws.json.writeValue(@TypeOf(input.training_data_config), input.training_data_config, alloc, &body_buf);
    has_prev = true;
    if (input.validation_data_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"validationDataConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateModelCustomizationJobOutput {
    var result: CreateModelCustomizationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateModelCustomizationJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
