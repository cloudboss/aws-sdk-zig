...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPromptRouterInput, options: Options) !GetPromptRouterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPromptRouterInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/prompt-routers/");
    try path_buf.appendSlice(alloc, input.prompt_router_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPromptRouterOutput {
    var result: GetPromptRouterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPromptRouterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
