...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutomatedReasoningPolicyTestResultsInput, options: Options) !ListAutomatedReasoningPolicyTestResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutomatedReasoningPolicyTestResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/test-results");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutomatedReasoningPolicyTestResultsOutput {
    var result: ListAutomatedReasoningPolicyTestResultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutomatedReasoningPolicyTestResultsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
