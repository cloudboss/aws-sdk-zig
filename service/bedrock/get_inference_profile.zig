...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInferenceProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInferenceProfileOutput {
    var result: GetInferenceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInferenceProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
