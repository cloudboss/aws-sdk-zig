...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutomatedReasoningPolicyInput, options: Options) !GetAutomatedReasoningPolicyOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAutomatedReasoningPolicyOutput {
    var result: GetAutomatedReasoningPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAutomatedReasoningPolicyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
