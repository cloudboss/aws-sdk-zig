...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAutomatedReasoningPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAutomatedReasoningPolicyOutput {
    var result: GetAutomatedReasoningPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAutomatedReasoningPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
