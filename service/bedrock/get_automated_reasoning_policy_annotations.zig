...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutomatedReasoningPolicyAnnotationsInput, options: CallOptions) !GetAutomatedReasoningPolicyAnnotationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAutomatedReasoningPolicyAnnotationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAutomatedReasoningPolicyAnnotationsOutput {
    var result: GetAutomatedReasoningPolicyAnnotationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAutomatedReasoningPolicyAnnotationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
