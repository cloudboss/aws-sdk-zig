...
pub const GetAutomatedReasoningPolicyAnnotationsInput = struct {
...
pub const GetAutomatedReasoningPolicyAnnotationsOutput = struct {
    ...
    /// the policy.
    annotations: ?[]const AutomatedReasoningPolicyAnnotation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutomatedReasoningPolicyAnnotationsInput, options: Options) !GetAutomatedReasoningPolicyAnnotationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAutomatedReasoningPolicyAnnotationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/build-workflows/");
    try path_buf.appendSlice(alloc, input.build_workflow_id);
    try path_buf.appendSlice(alloc, "/annotations");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAutomatedReasoningPolicyAnnotationsOutput {
    var result: GetAutomatedReasoningPolicyAnnotationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAutomatedReasoningPolicyAnnotationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
