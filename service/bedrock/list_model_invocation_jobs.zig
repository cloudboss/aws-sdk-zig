...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelInvocationJobsInput, options: Options) !ListModelInvocationJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelInvocationJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.submit_time_after) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "submitTimeAfter=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.submit_time_before) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "submitTimeBefore=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelInvocationJobsOutput {
    var result: ListModelInvocationJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListModelInvocationJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
