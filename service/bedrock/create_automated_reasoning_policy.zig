...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAutomatedReasoningPolicyInput, options: Options) !CreateAutomatedReasoningPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAutomatedReasoningPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAutomatedReasoningPolicyOutput {
    var result: CreateAutomatedReasoningPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAutomatedReasoningPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
