...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFHIRDatastoreInput, options: Options) !CreateFHIRDatastoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFHIRDatastoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "HealthLake.CreateFHIRDatastore");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFHIRDatastoreOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateFHIRDatastoreOutput, body, alloc);
}
