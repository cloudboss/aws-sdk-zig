...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFHIRExportJobInput, options: Options) !StartFHIRExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartFHIRExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "HealthLake.StartFHIRExportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartFHIRExportJobOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StartFHIRExportJobOutput, body, alloc);
}
