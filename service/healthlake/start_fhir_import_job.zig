...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFHIRImportJobInput, options: Options) !StartFHIRImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartFHIRImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "HealthLake.StartFHIRImportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartFHIRImportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartFHIRImportJobOutput, body, alloc);
}
