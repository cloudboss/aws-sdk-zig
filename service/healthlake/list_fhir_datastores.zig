...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFHIRDatastoresInput, options: Options) !ListFHIRDatastoresOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFHIRDatastoresInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "HealthLake.ListFHIRDatastores");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFHIRDatastoresOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFHIRDatastoresOutput, body, alloc);
}
