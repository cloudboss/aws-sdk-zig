...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFHIRExportJobsInput, options: Options) !ListFHIRExportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFHIRExportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "HealthLake.ListFHIRExportJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFHIRExportJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFHIRExportJobsOutput, body, alloc);
}
