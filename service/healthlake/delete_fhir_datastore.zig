...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteFHIRDatastoreInput, options: Options) !DeleteFHIRDatastoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteFHIRDatastoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "HealthLake.DeleteFHIRDatastore");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteFHIRDatastoreOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteFHIRDatastoreOutput, body, alloc);
}
