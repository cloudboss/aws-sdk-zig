...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssociationsForLicenseConfigurationInput, options: Options) !ListAssociationsForLicenseConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssociationsForLicenseConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.ListAssociationsForLicenseConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociationsForLicenseConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssociationsForLicenseConfigurationOutput, body, alloc);
}
