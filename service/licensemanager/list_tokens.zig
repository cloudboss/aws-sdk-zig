...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTokensInput, options: Options) !ListTokensOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTokensInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.ListTokens");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTokensOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTokensOutput, body, alloc);
}
