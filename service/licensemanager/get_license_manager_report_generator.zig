...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseManagerReportGeneratorInput, options: Options) !GetLicenseManagerReportGeneratorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLicenseManagerReportGeneratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.GetLicenseManagerReportGenerator");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLicenseManagerReportGeneratorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLicenseManagerReportGeneratorOutput, body, alloc);
}
