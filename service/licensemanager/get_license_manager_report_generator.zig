...
const ReportGenerator = @import("report_generator.zig").ReportGenerator;

pub const GetLicenseManagerReportGeneratorInput = struct {
...
pub const GetLicenseManagerReportGeneratorOutput = struct {
    /// A report generator that creates periodic reports about your license
    /// configurations.
    report_generator: ?ReportGenerator = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseManagerReportGeneratorInput, options: CallOptions) !GetLicenseManagerReportGeneratorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLicenseManagerReportGeneratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.GetLicenseManagerReportGenerator");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLicenseManagerReportGeneratorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLicenseManagerReportGeneratorOutput, body, allocator);
}
