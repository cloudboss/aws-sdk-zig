...
pub const ListLicenseConfigurationsForOrganizationInput = struct {
...
pub const ListLicenseConfigurationsForOrganizationOutput = struct {
    /// License configurations.
    license_configurations: ?[]const LicenseConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLicenseConfigurationsForOrganizationInput, options: CallOptions) !ListLicenseConfigurationsForOrganizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLicenseConfigurationsForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListLicenseConfigurationsForOrganization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLicenseConfigurationsForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLicenseConfigurationsForOrganizationOutput, body, allocator);
}
