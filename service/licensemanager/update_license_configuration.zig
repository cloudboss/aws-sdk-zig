...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLicenseConfigurationInput, options: Options) !UpdateLicenseConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLicenseConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.UpdateLicenseConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLicenseConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
