...
pub const DeleteLicenseAssetGroupInput = struct {
...
pub const DeleteLicenseAssetGroupOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteLicenseAssetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.DeleteLicenseAssetGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteLicenseAssetGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteLicenseAssetGroupOutput, body, allocator);
}
