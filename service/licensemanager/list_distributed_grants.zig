...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDistributedGrantsInput, options: Options) !ListDistributedGrantsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDistributedGrantsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.ListDistributedGrants");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDistributedGrantsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDistributedGrantsOutput, body, alloc);
}
