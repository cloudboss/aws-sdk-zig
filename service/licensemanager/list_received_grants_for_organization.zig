...
pub const ListReceivedGrantsForOrganizationInput = struct {
...
pub const ListReceivedGrantsForOrganizationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReceivedGrantsForOrganizationInput, options: CallOptions) !ListReceivedGrantsForOrganizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListReceivedGrantsForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListReceivedGrantsForOrganization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListReceivedGrantsForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListReceivedGrantsForOrganizationOutput, body, allocator);
}
