...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLicenseInput, options: Options) !CreateLicenseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLicenseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.CreateLicense");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLicenseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLicenseOutput, body, alloc);
}
