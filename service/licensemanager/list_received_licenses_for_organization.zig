...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReceivedLicensesForOrganizationInput, options: Options) !ListReceivedLicensesForOrganizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReceivedLicensesForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.ListReceivedLicensesForOrganization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReceivedLicensesForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListReceivedLicensesForOrganizationOutput, body, alloc);
}
