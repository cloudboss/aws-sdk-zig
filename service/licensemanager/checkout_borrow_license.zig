...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CheckoutBorrowLicenseInput, options: Options) !CheckoutBorrowLicenseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CheckoutBorrowLicenseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.CheckoutBorrowLicense");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CheckoutBorrowLicenseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CheckoutBorrowLicenseOutput, body, alloc);
}
