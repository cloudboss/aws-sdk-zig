...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLicenseConfigurationInput, options: Options) !CreateLicenseConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLicenseConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.CreateLicenseConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLicenseConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLicenseConfigurationOutput, body, alloc);
}
