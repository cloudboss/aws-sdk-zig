...
pub const GetLicenseAssetGroupInput = struct {
...
pub const GetLicenseAssetGroupOutput = struct {
    /// License asset group.
    license_asset_group: ?LicenseAssetGroup = null,

    pub const json_field_names = .{
        .license_asset_group = "LicenseAssetGroup",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseAssetGroupInput, options: CallOptions) !GetLicenseAssetGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLicenseAssetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.GetLicenseAssetGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLicenseAssetGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetLicenseAssetGroupOutput, body, allocator);
}
