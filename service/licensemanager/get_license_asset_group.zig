...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseAssetGroupInput, options: Options) !GetLicenseAssetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLicenseAssetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.GetLicenseAssetGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLicenseAssetGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetLicenseAssetGroupOutput, body, alloc);
}
