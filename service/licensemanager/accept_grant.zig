...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptGrantInput, options: Options) !AcceptGrantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.AcceptGrant");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptGrantOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AcceptGrantOutput, body, alloc);
}
