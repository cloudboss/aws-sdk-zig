...
pub const AcceptGrantInput = struct {
...
pub const AcceptGrantOutput = struct {
    /// Grant ARN.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptGrantInput, options: CallOptions) !AcceptGrantOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.AcceptGrant");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptGrantOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AcceptGrantOutput, body, allocator);
}
