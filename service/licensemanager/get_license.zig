...
pub const GetLicenseInput = struct {
...
pub const GetLicenseOutput = struct {
    /// License details.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseInput, options: CallOptions) !GetLicenseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLicenseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.GetLicense");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLicenseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLicenseOutput, body, allocator);
}
