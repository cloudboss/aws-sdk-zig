...
pub const ListLicenseSpecificationsForResourceInput = struct {
...
pub const ListLicenseSpecificationsForResourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLicenseSpecificationsForResourceInput, options: CallOptions) !ListLicenseSpecificationsForResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLicenseSpecificationsForResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListLicenseSpecificationsForResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLicenseSpecificationsForResourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLicenseSpecificationsForResourceOutput, body, allocator);
}
