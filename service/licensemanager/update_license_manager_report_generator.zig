...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLicenseManagerReportGeneratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.UpdateLicenseManagerReportGenerator");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLicenseManagerReportGeneratorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
