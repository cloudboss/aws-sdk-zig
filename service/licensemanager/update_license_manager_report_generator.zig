...
pub const UpdateLicenseManagerReportGeneratorInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateLicenseManagerReportGeneratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.UpdateLicenseManagerReportGenerator");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateLicenseManagerReportGeneratorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
