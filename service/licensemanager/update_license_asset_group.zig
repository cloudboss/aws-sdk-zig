...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLicenseAssetGroupInput, options: Options) !UpdateLicenseAssetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLicenseAssetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.UpdateLicenseAssetGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLicenseAssetGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateLicenseAssetGroupOutput, body, alloc);
}
