...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReceivedLicensesInput, options: Options) !ListReceivedLicensesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReceivedLicensesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.ListReceivedLicenses");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReceivedLicensesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListReceivedLicensesOutput, body, alloc);
}
