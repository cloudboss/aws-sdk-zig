...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLicenseAssetRulesetInput, options: Options) !CreateLicenseAssetRulesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLicenseAssetRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.CreateLicenseAssetRuleset");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLicenseAssetRulesetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLicenseAssetRulesetOutput, body, alloc);
}
