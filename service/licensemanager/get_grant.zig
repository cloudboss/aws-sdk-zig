...
pub const GetGrantInput = struct {
...
pub const GetGrantOutput = struct {
    /// Grant details.
    grant: ?Grant = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGrantInput, options: CallOptions) !GetGrantOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.GetGrant");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetGrantOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetGrantOutput, body, allocator);
}
