...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGrantInput, options: Options) !GetGrantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.GetGrant");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGrantOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetGrantOutput, body, alloc);
}
