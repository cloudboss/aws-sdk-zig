...
pub const ListAssetsForLicenseAssetGroupInput = struct {
...
pub const ListAssetsForLicenseAssetGroupOutput = struct {
    /// Assets.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssetsForLicenseAssetGroupInput, options: CallOptions) !ListAssetsForLicenseAssetGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssetsForLicenseAssetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListAssetsForLicenseAssetGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssetsForLicenseAssetGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssetsForLicenseAssetGroupOutput, body, allocator);
}
