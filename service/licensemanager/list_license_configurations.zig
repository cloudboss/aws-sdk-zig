...
pub const ListLicenseConfigurationsInput = struct {
...
pub const ListLicenseConfigurationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLicenseConfigurationsInput, options: CallOptions) !ListLicenseConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLicenseConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListLicenseConfigurations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLicenseConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLicenseConfigurationsOutput, body, allocator);
}
