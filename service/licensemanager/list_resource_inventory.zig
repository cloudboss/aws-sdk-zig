...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceInventoryInput, options: Options) !ListResourceInventoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceInventoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.ListResourceInventory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceInventoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceInventoryOutput, body, alloc);
}
