...
pub const ListLicensesInput = struct {
...
pub const ListLicensesOutput = struct {
    /// License details.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLicensesInput, options: CallOptions) !ListLicensesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLicensesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListLicenses");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLicensesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLicensesOutput, body, allocator);
}
