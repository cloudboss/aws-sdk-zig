...
pub const ListLicenseAssetRulesetsInput = struct {
    ...
    pub const json_field_names = .{
        ...
        .show_aws_managed_license_asset_rulesets = "ShowAWSManagedLicenseAssetRulesets",
    };
};

pub const ListLicenseAssetRulesetsOutput = struct {
    /// License asset rulesets.
    license_asset_rulesets: ?[]const LicenseAssetRuleset = null,

    /// Token for the next set of results.
    next_token: ?[]const u8 = null,

    pub const json_field_names = .{
        .license_asset_rulesets = "LicenseAssetRulesets",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLicenseAssetRulesetsInput, options: CallOptions) !ListLicenseAssetRulesetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLicenseAssetRulesetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListLicenseAssetRulesets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLicenseAssetRulesetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLicenseAssetRulesetsOutput, body, allocator);
}
