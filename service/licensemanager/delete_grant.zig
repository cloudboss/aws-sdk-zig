...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGrantInput, options: Options) !DeleteGrantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.DeleteGrant");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGrantOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteGrantOutput, body, alloc);
}
