...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CheckoutLicenseInput, options: Options) !CheckoutLicenseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CheckoutLicenseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.CheckoutLicense");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CheckoutLicenseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CheckoutLicenseOutput, body, alloc);
}
