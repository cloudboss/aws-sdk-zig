...
pub const DeleteLicenseConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLicenseConfigurationInput, options: Options) !DeleteLicenseConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteLicenseConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.DeleteLicenseConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteLicenseConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
