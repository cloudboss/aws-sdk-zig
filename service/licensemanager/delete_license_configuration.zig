...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteLicenseConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.DeleteLicenseConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteLicenseConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
