...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExtendLicenseConsumptionInput, options: Options) !ExtendLicenseConsumptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExtendLicenseConsumptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.ExtendLicenseConsumption");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExtendLicenseConsumptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExtendLicenseConsumptionOutput, body, alloc);
}
