...
pub const DeleteLicenseAssetRulesetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLicenseAssetRulesetInput, options: Options) !DeleteLicenseAssetRulesetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteLicenseAssetRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.DeleteLicenseAssetRuleset");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteLicenseAssetRulesetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
