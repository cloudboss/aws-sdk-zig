...
pub const ListLicenseVersionsInput = struct {
...
pub const ListLicenseVersionsOutput = struct {
    /// License details.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLicenseVersionsInput, options: CallOptions) !ListLicenseVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLicenseVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListLicenseVersions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLicenseVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLicenseVersionsOutput, body, allocator);
}
