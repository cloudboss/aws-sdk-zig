...
fn serializeRequest(alloc: std.mem.Allocator, input: CheckInLicenseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.CheckInLicense");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CheckInLicenseOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
