...
pub const CheckInLicenseInput = struct {
    /// License beneficiary.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CheckInLicenseInput, options: Options) !CheckInLicenseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CheckInLicenseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.CheckInLicense");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CheckInLicenseOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
