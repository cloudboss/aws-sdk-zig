...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLicenseSpecificationsForResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.UpdateLicenseSpecificationsForResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLicenseSpecificationsForResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
