...
pub const ListReceivedGrantsInput = struct {
...
pub const ListReceivedGrantsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReceivedGrantsInput, options: CallOptions) !ListReceivedGrantsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListReceivedGrantsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListReceivedGrants");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListReceivedGrantsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListReceivedGrantsOutput, body, allocator);
}
