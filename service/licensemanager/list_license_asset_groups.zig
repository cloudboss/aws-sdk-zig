...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLicenseAssetGroupsInput, options: Options) !ListLicenseAssetGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLicenseAssetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.ListLicenseAssetGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLicenseAssetGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLicenseAssetGroupsOutput, body, alloc);
}
