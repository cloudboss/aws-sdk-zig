...
const LicenseAssetGroup = @import("license_asset_group.zig").LicenseAssetGroup;

pub const ListLicenseAssetGroupsInput = struct {
...
pub const ListLicenseAssetGroupsOutput = struct {
    /// License asset groups.
    license_asset_groups: ?[]const LicenseAssetGroup = null,

    /// Token for the next set of results.
    next_token: ?[]const u8 = null,

    pub const json_field_names = .{
        .license_asset_groups = "LicenseAssetGroups",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLicenseAssetGroupsInput, options: CallOptions) !ListLicenseAssetGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLicenseAssetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListLicenseAssetGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLicenseAssetGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLicenseAssetGroupsOutput, body, allocator);
}
