...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteLicenseManagerReportGeneratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.DeleteLicenseManagerReportGenerator");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteLicenseManagerReportGeneratorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
