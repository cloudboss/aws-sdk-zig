...
pub const RejectGrantInput = struct {
...
pub const RejectGrantOutput = struct {
    /// Grant ARN.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectGrantInput, options: CallOptions) !RejectGrantOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RejectGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.RejectGrant");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RejectGrantOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RejectGrantOutput, body, allocator);
}
