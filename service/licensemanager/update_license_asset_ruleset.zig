...
pub const UpdateLicenseAssetRulesetInput = struct {
...
pub const UpdateLicenseAssetRulesetOutput = struct {
    /// Amazon Resource Name (ARN) of the license asset ruleset.
    license_asset_ruleset_arn: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLicenseAssetRulesetInput, options: CallOptions) !UpdateLicenseAssetRulesetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateLicenseAssetRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.UpdateLicenseAssetRuleset");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateLicenseAssetRulesetOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateLicenseAssetRulesetOutput, body, allocator);
}
