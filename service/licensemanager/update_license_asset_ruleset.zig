...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLicenseAssetRulesetInput, options: Options) !UpdateLicenseAssetRulesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLicenseAssetRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.UpdateLicenseAssetRuleset");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLicenseAssetRulesetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateLicenseAssetRulesetOutput, body, alloc);
}
