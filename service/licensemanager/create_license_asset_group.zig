...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLicenseAssetGroupInput, options: Options) !CreateLicenseAssetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLicenseAssetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.CreateLicenseAssetGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLicenseAssetGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLicenseAssetGroupOutput, body, alloc);
}
