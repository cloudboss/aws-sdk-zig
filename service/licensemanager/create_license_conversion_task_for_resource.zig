...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLicenseConversionTaskForResourceInput, options: Options) !CreateLicenseConversionTaskForResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLicenseConversionTaskForResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.CreateLicenseConversionTaskForResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLicenseConversionTaskForResourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLicenseConversionTaskForResourceOutput, body, alloc);
}
