...
pub const ListLicenseManagerReportGeneratorsInput = struct {
...
pub const ListLicenseManagerReportGeneratorsOutput = struct {
    /// Token for the next set of results.
    next_token: ?[]const u8 = null,

    /// A report generator that creates periodic reports about your license
    /// configurations.
    report_generators: ?[]const ReportGenerator = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLicenseManagerReportGeneratorsInput, options: CallOptions) !ListLicenseManagerReportGeneratorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLicenseManagerReportGeneratorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListLicenseManagerReportGenerators");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLicenseManagerReportGeneratorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLicenseManagerReportGeneratorsOutput, body, allocator);
}
