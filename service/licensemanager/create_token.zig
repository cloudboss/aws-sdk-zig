...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTokenInput, options: Options) !CreateTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.CreateToken");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTokenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTokenOutput, body, alloc);
}
