...
const LicenseConfigurationUsage = @import("license_configuration_usage.zig").LicenseConfigurationUsage;

pub const ListUsageForLicenseConfigurationInput = struct {
    ...
    ///
    /// * `resourceType` - The resource type (`EC2_INSTANCE` |
    /// `EC2_HOST` | `EC2_AMI` | `SYSTEMS_MANAGER_MANAGED_INSTANCE`).
    ///
...
pub const ListUsageForLicenseConfigurationOutput = struct {
    /// Information about the license configurations.
    license_configuration_usage_list: ?[]const LicenseConfigurationUsage = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUsageForLicenseConfigurationInput, options: CallOptions) !ListUsageForLicenseConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUsageForLicenseConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListUsageForLicenseConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUsageForLicenseConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUsageForLicenseConfigurationOutput, body, allocator);
}
