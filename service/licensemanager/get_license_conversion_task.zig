...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseConversionTaskInput, options: Options) !GetLicenseConversionTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLicenseConversionTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.GetLicenseConversionTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLicenseConversionTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLicenseConversionTaskOutput, body, alloc);
}
