...
const LicenseUsage = @import("license_usage.zig").LicenseUsage;

pub const GetLicenseUsageInput = struct {
...
pub const GetLicenseUsageOutput = struct {
    /// License usage details.
    license_usage: ?LicenseUsage = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseUsageInput, options: CallOptions) !GetLicenseUsageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLicenseUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.GetLicenseUsage");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLicenseUsageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLicenseUsageOutput, body, allocator);
}
