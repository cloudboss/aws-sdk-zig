...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseAssetRulesetInput, options: Options) !GetLicenseAssetRulesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLicenseAssetRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.GetLicenseAssetRuleset");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLicenseAssetRulesetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLicenseAssetRulesetOutput, body, alloc);
}
