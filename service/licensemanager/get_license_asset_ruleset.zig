...
const LicenseAssetRuleset = @import("license_asset_ruleset.zig").LicenseAssetRuleset;

pub const GetLicenseAssetRulesetInput = struct {
...
pub const GetLicenseAssetRulesetOutput = struct {
    /// License asset ruleset.
    license_asset_ruleset: ?LicenseAssetRuleset = null,

    pub const json_field_names = .{
        .license_asset_ruleset = "LicenseAssetRuleset",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseAssetRulesetInput, options: CallOptions) !GetLicenseAssetRulesetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLicenseAssetRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.GetLicenseAssetRuleset");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLicenseAssetRulesetOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetLicenseAssetRulesetOutput, body, allocator);
}
