...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFailuresForLicenseConfigurationOperationsInput, options: Options) !ListFailuresForLicenseConfigurationOperationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFailuresForLicenseConfigurationOperationsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("licensemanager", "License Manager", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.ListFailuresForLicenseConfigurationOperations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFailuresForLicenseConfigurationOperationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFailuresForLicenseConfigurationOperationsOutput, body, alloc);
}
