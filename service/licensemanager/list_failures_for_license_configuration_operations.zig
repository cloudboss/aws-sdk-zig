...
const LicenseOperationFailure = @import("license_operation_failure.zig").LicenseOperationFailure;

pub const ListFailuresForLicenseConfigurationOperationsInput = struct {
...
pub const ListFailuresForLicenseConfigurationOperationsOutput = struct {
    /// License configuration operations that failed.
    license_operation_failure_list: ?[]const LicenseOperationFailure = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFailuresForLicenseConfigurationOperationsInput, options: CallOptions) !ListFailuresForLicenseConfigurationOperationsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "license-manager");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFailuresForLicenseConfigurationOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLicenseManager.ListFailuresForLicenseConfigurationOperations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFailuresForLicenseConfigurationOperationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFailuresForLicenseConfigurationOperationsOutput, body, allocator);
}
