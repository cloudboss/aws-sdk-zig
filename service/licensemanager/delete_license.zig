...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLicenseInput, options: Options) !DeleteLicenseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteLicenseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLicenseManager.DeleteLicense");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteLicenseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteLicenseOutput, body, alloc);
}
