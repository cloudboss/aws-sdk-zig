...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkloadEstimateInput, options: Options) !CreateWorkloadEstimateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkloadEstimateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.CreateWorkloadEstimate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkloadEstimateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateWorkloadEstimateOutput, body, alloc);
}
