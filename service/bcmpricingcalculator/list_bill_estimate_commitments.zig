...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBillEstimateCommitmentsInput, options: Options) !ListBillEstimateCommitmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBillEstimateCommitmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.ListBillEstimateCommitments");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBillEstimateCommitmentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBillEstimateCommitmentsOutput, body, alloc);
}
