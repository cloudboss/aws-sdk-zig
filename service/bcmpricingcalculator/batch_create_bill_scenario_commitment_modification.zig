...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreateBillScenarioCommitmentModificationInput, options: Options) !BatchCreateBillScenarioCommitmentModificationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateBillScenarioCommitmentModificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.BatchCreateBillScenarioCommitmentModification");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateBillScenarioCommitmentModificationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchCreateBillScenarioCommitmentModificationOutput, body, alloc);
}
