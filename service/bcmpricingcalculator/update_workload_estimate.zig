...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkloadEstimateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.UpdateWorkloadEstimate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkloadEstimateOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateWorkloadEstimateOutput, body, alloc);
}
