...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBillScenariosInput, options: Options) !ListBillScenariosOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBillScenariosInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.ListBillScenarios");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBillScenariosOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBillScenariosOutput, body, alloc);
}
