...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBillScenarioCommitmentModificationsInput, options: Options) !ListBillScenarioCommitmentModificationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBillScenarioCommitmentModificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.ListBillScenarioCommitmentModifications");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBillScenarioCommitmentModificationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBillScenarioCommitmentModificationsOutput, body, alloc);
}
