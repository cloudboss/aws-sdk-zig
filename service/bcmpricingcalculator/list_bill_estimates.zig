...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBillEstimatesInput, options: Options) !ListBillEstimatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBillEstimatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.ListBillEstimates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBillEstimatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBillEstimatesOutput, body, alloc);
}
