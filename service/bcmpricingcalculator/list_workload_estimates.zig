...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkloadEstimatesInput, options: Options) !ListWorkloadEstimatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkloadEstimatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.ListWorkloadEstimates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkloadEstimatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListWorkloadEstimatesOutput, body, alloc);
}
