...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBillEstimateInput, options: Options) !CreateBillEstimateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBillEstimateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.CreateBillEstimate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBillEstimateOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateBillEstimateOutput, body, alloc);
}
