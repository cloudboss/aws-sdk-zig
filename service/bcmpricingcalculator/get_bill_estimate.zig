...
pub const GetBillEstimateInput = struct {
...
pub const GetBillEstimateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBillEstimateInput, options: CallOptions) !GetBillEstimateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBillEstimateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSBCMPricingCalculator.GetBillEstimate");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBillEstimateOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetBillEstimateOutput, body, allocator);
}
