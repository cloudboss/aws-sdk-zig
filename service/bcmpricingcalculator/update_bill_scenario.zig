...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBillScenarioInput, options: Options) !UpdateBillScenarioOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBillScenarioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.UpdateBillScenario");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBillScenarioOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateBillScenarioOutput, body, alloc);
}
