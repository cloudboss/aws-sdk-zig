...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteBillScenarioCommitmentModificationInput, options: Options) !BatchDeleteBillScenarioCommitmentModificationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteBillScenarioCommitmentModificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.BatchDeleteBillScenarioCommitmentModification");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteBillScenarioCommitmentModificationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDeleteBillScenarioCommitmentModificationOutput, body, alloc);
}
