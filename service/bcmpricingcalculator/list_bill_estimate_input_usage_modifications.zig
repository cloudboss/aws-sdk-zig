...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBillEstimateInputUsageModificationsInput, options: Options) !ListBillEstimateInputUsageModificationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBillEstimateInputUsageModificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.ListBillEstimateInputUsageModifications");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBillEstimateInputUsageModificationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBillEstimateInputUsageModificationsOutput, body, alloc);
}
