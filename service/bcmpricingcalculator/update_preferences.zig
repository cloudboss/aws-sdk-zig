...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePreferencesInput, options: Options) !UpdatePreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.UpdatePreferences");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePreferencesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdatePreferencesOutput, body, alloc);
}
