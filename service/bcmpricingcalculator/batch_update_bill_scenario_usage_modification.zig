...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateBillScenarioUsageModificationInput, options: Options) !BatchUpdateBillScenarioUsageModificationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateBillScenarioUsageModificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.BatchUpdateBillScenarioUsageModification");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateBillScenarioUsageModificationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchUpdateBillScenarioUsageModificationOutput, body, alloc);
}
