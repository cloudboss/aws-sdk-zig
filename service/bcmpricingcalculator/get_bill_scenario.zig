...
pub const GetBillScenarioInput = struct {
...
pub const GetBillScenarioOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBillScenarioInput, options: CallOptions) !GetBillScenarioOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBillScenarioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSBCMPricingCalculator.GetBillScenario");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBillScenarioOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetBillScenarioOutput, body, allocator);
}
