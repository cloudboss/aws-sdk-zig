...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBillScenarioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBCMPricingCalculator.GetBillScenario");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBillScenarioOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetBillScenarioOutput, body, alloc);
}
