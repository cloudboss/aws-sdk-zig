...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAddonSubscriptionInput, options: Options) !CreateAddonSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAddonSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.CreateAddonSubscription");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAddonSubscriptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAddonSubscriptionOutput, body, alloc);
}
