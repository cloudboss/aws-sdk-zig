...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIngressPointInput, options: Options) !UpdateIngressPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIngressPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.UpdateIngressPoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIngressPointOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
