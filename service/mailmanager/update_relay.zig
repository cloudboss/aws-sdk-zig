...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRelayInput, options: Options) !UpdateRelayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRelayInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.UpdateRelay");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRelayOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
