...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIngressPointInput, options: Options) !CreateIngressPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIngressPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.CreateIngressPoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIngressPointOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateIngressPointOutput, body, alloc);
}
