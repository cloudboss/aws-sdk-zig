...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRelayInput, options: Options) !GetRelayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRelayInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.GetRelay");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRelayOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRelayOutput, body, alloc);
}
