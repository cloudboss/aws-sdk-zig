...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRuleSetInput, options: Options) !GetRuleSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.GetRuleSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRuleSetOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetRuleSetOutput, body, alloc);
}
