...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTrafficPolicyInput, options: Options) !UpdateTrafficPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTrafficPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.UpdateTrafficPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTrafficPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
