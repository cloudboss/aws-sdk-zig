...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRelayInput, options: Options) !CreateRelayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRelayInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.CreateRelay");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRelayOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateRelayOutput, body, alloc);
}
