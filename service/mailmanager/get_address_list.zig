...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAddressListInput, options: Options) !GetAddressListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAddressListInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.GetAddressList");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAddressListOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetAddressListOutput, body, alloc);
}
