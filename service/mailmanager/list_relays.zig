...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRelaysInput, options: Options) !ListRelaysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRelaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.ListRelays");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRelaysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRelaysOutput, body, alloc);
}
