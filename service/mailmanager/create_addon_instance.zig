...
pub const CreateAddonInstanceInput = struct {
...
pub const CreateAddonInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAddonInstanceInput, options: CallOptions) !CreateAddonInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAddonInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MailManagerSvc.CreateAddonInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAddonInstanceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateAddonInstanceOutput, body, allocator);
}
