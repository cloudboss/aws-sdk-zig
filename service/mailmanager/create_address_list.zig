...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAddressListInput, options: Options) !CreateAddressListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAddressListInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.CreateAddressList");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAddressListOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAddressListOutput, body, alloc);
}
