...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAddonSubscriptionsInput, options: Options) !ListAddonSubscriptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAddonSubscriptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.ListAddonSubscriptions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAddonSubscriptionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAddonSubscriptionsOutput, body, alloc);
}
