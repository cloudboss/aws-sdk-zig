...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMembersOfAddressListInput, options: Options) !ListMembersOfAddressListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMembersOfAddressListInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.ListMembersOfAddressList");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMembersOfAddressListOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListMembersOfAddressListOutput, body, alloc);
}
