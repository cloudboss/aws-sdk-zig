...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListArchiveExportsInput, options: Options) !ListArchiveExportsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListArchiveExportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.ListArchiveExports");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListArchiveExportsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListArchiveExportsOutput, body, alloc);
}
