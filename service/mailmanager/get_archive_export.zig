...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetArchiveExportInput, options: Options) !GetArchiveExportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetArchiveExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MailManagerSvc.GetArchiveExport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetArchiveExportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetArchiveExportOutput, body, alloc);
}
