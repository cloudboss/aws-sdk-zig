...
pub const GetArchiveSearchInput = struct {
...
pub const GetArchiveSearchOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetArchiveSearchInput, options: CallOptions) !GetArchiveSearchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetArchiveSearchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MailManagerSvc.GetArchiveSearch");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetArchiveSearchOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetArchiveSearchOutput, body, allocator);
}
