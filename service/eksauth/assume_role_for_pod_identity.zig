...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssumeRoleForPodIdentityInput, options: Options) !AssumeRoleForPodIdentityOutput {
    ...
    try aws.signing.signRequest(alloc, &request, creds, client.config.region, "eksauth");
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssumeRoleForPodIdentityInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("eksauth", "EKS Auth", alloc);
    ...
    try path_buf.appendSlice(alloc, "/assume-role-for-pod-identity");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssumeRoleForPodIdentityOutput {
    var result: AssumeRoleForPodIdentityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssumeRoleForPodIdentityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
