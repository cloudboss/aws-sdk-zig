...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAuditEventsInput, options: Options) !PutAuditEventsOutput {
    ...
    try aws.signing.signRequest(alloc, &request, creds, client.config.region, "cloudtraildata");
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAuditEventsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("cloudtraildata", "CloudTrail Data", alloc);
    ...
    try query_buf.appendSlice(alloc, "channelArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.channel_arn);
    query_has_prev = true;
    if (input.external_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "externalId=");
    ...
    try body_buf.appendSlice(alloc, "\"auditEvents\":");
    try aws.json.writeValue(@TypeOf(input.audit_events), input.audit_events, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAuditEventsOutput {
    var result: PutAuditEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutAuditEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
