...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAttachmentRoutingPolicyLabelInput, options: Options) !PutAttachmentRoutingPolicyLabelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAttachmentRoutingPolicyLabelInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.routing_policy_label), input.routing_policy_label, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAttachmentRoutingPolicyLabelOutput {
    var result: PutAttachmentRoutingPolicyLabelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutAttachmentRoutingPolicyLabelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
