...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNetworkResourceCountsInput, options: CallOptions) !GetNetworkResourceCountsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetNetworkResourceCountsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/network-resource-count");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetNetworkResourceCountsOutput {
    var result: GetNetworkResourceCountsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNetworkResourceCountsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
