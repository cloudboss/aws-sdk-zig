...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterTransitGatewayInput, options: Options) !RegisterTransitGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterTransitGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/transit-gateway-registrations");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterTransitGatewayOutput {
    var result: RegisterTransitGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterTransitGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
