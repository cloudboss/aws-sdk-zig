...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAttachmentsInput, options: Options) !ListAttachmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAttachmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.attachment_type) |v| {
        ...
        try query_buf.appendSlice(alloc, "attachmentType=");
    ...
    if (input.edge_location) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "edgeLocation=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAttachmentsOutput {
    var result: ListAttachmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAttachmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
