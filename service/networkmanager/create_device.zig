...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDeviceInput, options: Options) !CreateDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeviceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDeviceOutput {
    var result: CreateDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
