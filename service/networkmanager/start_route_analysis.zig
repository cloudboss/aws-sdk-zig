...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRouteAnalysisInput, options: Options) !StartRouteAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRouteAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/route-analyses");
    ...
    if (input.include_return_path) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IncludeReturnPath\":");
    ...
    if (input.use_middleboxes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UseMiddleboxes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRouteAnalysisOutput {
    var result: StartRouteAnalysisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartRouteAnalysisOutput, body, alloc);
    }
    _ = status;
    _ = headers;
