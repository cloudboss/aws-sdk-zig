...
pub const DeleteDeviceInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDeviceInput, options: Options) !DeleteDeviceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDeviceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDeviceOutput {
    var result: DeleteDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDeviceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
