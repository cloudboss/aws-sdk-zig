...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteCoreNetworkChangeSetInput, options: Options) !ExecuteCoreNetworkChangeSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteCoreNetworkChangeSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/core-network-change-sets/");
    try path_buf.appendSlice(alloc, input.policy_version_id);
    try path_buf.appendSlice(alloc, "/execute");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteCoreNetworkChangeSetOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
