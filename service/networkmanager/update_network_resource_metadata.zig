...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNetworkResourceMetadataInput, options: Options) !UpdateNetworkResourceMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNetworkResourceMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNetworkResourceMetadataOutput {
    var result: UpdateNetworkResourceMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateNetworkResourceMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
