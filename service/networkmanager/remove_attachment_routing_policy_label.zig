...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveAttachmentRoutingPolicyLabelInput, options: Options) !RemoveAttachmentRoutingPolicyLabelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveAttachmentRoutingPolicyLabelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/attachment/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveAttachmentRoutingPolicyLabelOutput {
    var result: RemoveAttachmentRoutingPolicyLabelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveAttachmentRoutingPolicyLabelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
