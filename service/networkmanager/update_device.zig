...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDeviceInput, options: Options) !UpdateDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.aws_location) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AWSLocation\":");
    ...
    if (input.site_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SiteId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.type) |v| {
    ...
    if (input.vendor) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Vendor\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDeviceOutput {
    var result: UpdateDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
