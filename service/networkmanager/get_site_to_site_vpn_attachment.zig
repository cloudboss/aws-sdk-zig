...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSiteToSiteVpnAttachmentInput, options: Options) !GetSiteToSiteVpnAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSiteToSiteVpnAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/site-to-site-vpn-attachments/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSiteToSiteVpnAttachmentOutput {
    var result: GetSiteToSiteVpnAttachmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSiteToSiteVpnAttachmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
