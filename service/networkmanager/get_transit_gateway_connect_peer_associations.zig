...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTransitGatewayConnectPeerAssociationsInput, options: Options) !GetTransitGatewayConnectPeerAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTransitGatewayConnectPeerAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.transit_gateway_connect_peer_arns) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "transitGatewayConnectPeerArns=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTransitGatewayConnectPeerAssociationsOutput {
    var result: GetTransitGatewayConnectPeerAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTransitGatewayConnectPeerAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
