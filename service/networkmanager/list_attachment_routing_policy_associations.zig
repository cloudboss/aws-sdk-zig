...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAttachmentRoutingPolicyAssociationsInput, options: Options) !ListAttachmentRoutingPolicyAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAttachmentRoutingPolicyAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/routing-policy-label/core-network/");
    ...
    if (input.attachment_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "attachmentId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAttachmentRoutingPolicyAssociationsOutput {
    var result: ListAttachmentRoutingPolicyAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAttachmentRoutingPolicyAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
