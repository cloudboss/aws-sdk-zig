...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDevicesInput, options: Options) !GetDevicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDevicesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.device_ids) |v| {
        ...
        try query_buf.appendSlice(alloc, "deviceIds=");
    ...
    if (input.site_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "siteId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDevicesOutput {
    var result: GetDevicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDevicesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
