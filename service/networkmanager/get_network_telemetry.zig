...
const NetworkTelemetry = @import("network_telemetry.zig").NetworkTelemetry;

pub const GetNetworkTelemetryInput = struct {
...
pub const GetNetworkTelemetryOutput = struct {
    /// The network telemetry.
    network_telemetry: ?[]const NetworkTelemetry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNetworkTelemetryInput, options: Options) !GetNetworkTelemetryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNetworkTelemetryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/network-telemetry");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNetworkTelemetryOutput {
    var result: GetNetworkTelemetryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNetworkTelemetryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
