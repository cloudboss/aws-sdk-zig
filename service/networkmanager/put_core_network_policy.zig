...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutCoreNetworkPolicyInput, options: Options) !PutCoreNetworkPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutCoreNetworkPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/core-network-policy");
    ...
    if (input.latest_version_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LatestVersionId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutCoreNetworkPolicyOutput {
    var result: PutCoreNetworkPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutCoreNetworkPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
