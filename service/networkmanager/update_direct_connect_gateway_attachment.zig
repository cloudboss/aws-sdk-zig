...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDirectConnectGatewayAttachmentInput, options: Options) !UpdateDirectConnectGatewayAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDirectConnectGatewayAttachmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDirectConnectGatewayAttachmentOutput {
    var result: UpdateDirectConnectGatewayAttachmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDirectConnectGatewayAttachmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
