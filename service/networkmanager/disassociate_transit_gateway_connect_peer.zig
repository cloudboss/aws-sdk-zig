...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateTransitGatewayConnectPeerInput, options: Options) !DisassociateTransitGatewayConnectPeerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateTransitGatewayConnectPeerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.transit_gateway_connect_peer_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateTransitGatewayConnectPeerOutput {
    var result: DisassociateTransitGatewayConnectPeerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateTransitGatewayConnectPeerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
