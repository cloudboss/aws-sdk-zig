...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCoreNetworkPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.policy_version_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "policyVersionId=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCoreNetworkPolicyOutput {
    var result: GetCoreNetworkPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCoreNetworkPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
