...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateTransitGatewayConnectPeerInput, options: CallOptions) !AssociateTransitGatewayConnectPeerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateTransitGatewayConnectPeerInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"TransitGatewayConnectPeerArn\":");
    try aws.json.writeValue(@TypeOf(input.transit_gateway_connect_peer_arn), input.transit_gateway_connect_peer_arn, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateTransitGatewayConnectPeerOutput {
    var result: AssociateTransitGatewayConnectPeerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateTransitGatewayConnectPeerOutput, body, allocator);
    }
    _ = status;
    _ = headers;
