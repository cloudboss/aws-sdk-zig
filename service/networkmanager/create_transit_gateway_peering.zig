...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTransitGatewayPeeringInput, options: Options) !CreateTransitGatewayPeeringOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTransitGatewayPeeringInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TransitGatewayArn\":");
    try aws.json.writeValue(@TypeOf(input.transit_gateway_arn), input.transit_gateway_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTransitGatewayPeeringOutput {
    var result: CreateTransitGatewayPeeringOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTransitGatewayPeeringOutput, body, alloc);
    }
    _ = status;
    _ = headers;
