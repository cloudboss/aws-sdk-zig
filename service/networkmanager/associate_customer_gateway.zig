...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateCustomerGatewayInput, options: Options) !AssociateCustomerGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateCustomerGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"CustomerGatewayArn\":");
    try aws.json.writeValue(@TypeOf(input.customer_gateway_arn), input.customer_gateway_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateCustomerGatewayOutput {
    var result: AssociateCustomerGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateCustomerGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
