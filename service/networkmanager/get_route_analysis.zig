...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouteAnalysisInput, options: Options) !GetRouteAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRouteAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.route_analysis_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRouteAnalysisOutput {
    var result: GetRouteAnalysisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouteAnalysisOutput, body, alloc);
    }
    _ = status;
    _ = headers;
