...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCoreNetworkPrefixListAssociationInput, options: Options) !CreateCoreNetworkPrefixListAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCoreNetworkPrefixListAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PrefixListAlias\":");
    try aws.json.writeValue(@TypeOf(input.prefix_list_alias), input.prefix_list_alias, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PrefixListArn\":");
    try aws.json.writeValue(@TypeOf(input.prefix_list_arn), input.prefix_list_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCoreNetworkPrefixListAssociationOutput {
    var result: CreateCoreNetworkPrefixListAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCoreNetworkPrefixListAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
