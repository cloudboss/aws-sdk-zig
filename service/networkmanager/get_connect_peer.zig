...
pub const GetConnectPeerInput = struct {
...
pub const GetConnectPeerOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetConnectPeerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetConnectPeerOutput {
    var result: GetConnectPeerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConnectPeerOutput, body, allocator);
    }
    _ = status;
    _ = headers;
