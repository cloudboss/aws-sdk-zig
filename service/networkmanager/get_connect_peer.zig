...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConnectPeerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConnectPeerOutput {
    var result: GetConnectPeerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConnectPeerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
