...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVpcAttachmentInput, options: Options) !UpdateVpcAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVpcAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/vpc-attachments/");
    ...
    if (input.add_subnet_arns) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AddSubnetArns\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Options\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.remove_subnet_arns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RemoveSubnetArns\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVpcAttachmentOutput {
    var result: UpdateVpcAttachmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVpcAttachmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
