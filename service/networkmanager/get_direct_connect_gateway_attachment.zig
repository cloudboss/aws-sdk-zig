...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDirectConnectGatewayAttachmentInput, options: Options) !GetDirectConnectGatewayAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDirectConnectGatewayAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/direct-connect-gateway-attachments/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDirectConnectGatewayAttachmentOutput {
    var result: GetDirectConnectGatewayAttachmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDirectConnectGatewayAttachmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
