...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConnectAttachmentInput, options: Options) !CreateConnectAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConnectAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TransportAttachmentId\":");
    try aws.json.writeValue(@TypeOf(input.transport_attachment_id), input.transport_attachment_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConnectAttachmentOutput {
    var result: CreateConnectAttachmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConnectAttachmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
