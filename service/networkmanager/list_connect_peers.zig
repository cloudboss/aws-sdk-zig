...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConnectPeersInput, options: Options) !ListConnectPeersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConnectPeersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connect_attachment_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "connectAttachmentId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConnectPeersOutput {
    var result: ListConnectPeersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConnectPeersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
