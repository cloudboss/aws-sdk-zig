...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLinkAssociationsInput, options: Options) !GetLinkAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLinkAssociationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLinkAssociationsOutput {
    var result: GetLinkAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLinkAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
