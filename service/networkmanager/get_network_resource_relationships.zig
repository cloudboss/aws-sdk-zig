...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNetworkResourceRelationshipsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/network-resource-relationships");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNetworkResourceRelationshipsOutput {
    var result: GetNetworkResourceRelationshipsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNetworkResourceRelationshipsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
