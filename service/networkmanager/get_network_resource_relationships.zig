...
pub const GetNetworkResourceRelationshipsInput = struct {
...
pub const GetNetworkResourceRelationshipsOutput = struct {
    /// The token for the next page of results.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNetworkResourceRelationshipsInput, options: CallOptions) !GetNetworkResourceRelationshipsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetNetworkResourceRelationshipsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/network-resource-relationships");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetNetworkResourceRelationshipsOutput {
    var result: GetNetworkResourceRelationshipsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNetworkResourceRelationshipsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
