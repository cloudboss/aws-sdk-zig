...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTransitGatewayRegistrationsInput, options: CallOptions) !GetTransitGatewayRegistrationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTransitGatewayRegistrationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTransitGatewayRegistrationsOutput {
    var result: GetTransitGatewayRegistrationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTransitGatewayRegistrationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
