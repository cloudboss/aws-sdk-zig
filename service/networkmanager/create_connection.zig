...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ConnectedDeviceId\":");
    try aws.json.writeValue(@TypeOf(input.connected_device_id), input.connected_device_id, alloc, &body_buf);
    has_prev = true;
    if (input.connected_link_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ConnectedLinkId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConnectionOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
