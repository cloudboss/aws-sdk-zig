...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLinksInput, options: Options) !GetLinksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLinksInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.link_ids) |v| {
        ...
        try query_buf.appendSlice(alloc, "linkIds=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLinksOutput {
    var result: GetLinksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLinksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
