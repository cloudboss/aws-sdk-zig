...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDirectConnectGatewayAttachmentInput, options: Options) !CreateDirectConnectGatewayAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDirectConnectGatewayAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DirectConnectGatewayArn\":");
    try aws.json.writeValue(@TypeOf(input.direct_connect_gateway_arn), input.direct_connect_gateway_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EdgeLocations\":");
    try aws.json.writeValue(@TypeOf(input.edge_locations), input.edge_locations, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDirectConnectGatewayAttachmentOutput {
    var result: CreateDirectConnectGatewayAttachmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDirectConnectGatewayAttachmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
