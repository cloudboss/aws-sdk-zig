...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCoreNetworkInput, options: Options) !CreateCoreNetworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCoreNetworkInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"GlobalNetworkId\":");
    try aws.json.writeValue(@TypeOf(input.global_network_id), input.global_network_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCoreNetworkOutput {
    var result: CreateCoreNetworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCoreNetworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
