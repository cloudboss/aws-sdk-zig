...
pub const DeleteGlobalNetworkInput = struct {
...
pub const DeleteGlobalNetworkOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGlobalNetworkInput, options: Options) !DeleteGlobalNetworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGlobalNetworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGlobalNetworkOutput {
    var result: DeleteGlobalNetworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteGlobalNetworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
