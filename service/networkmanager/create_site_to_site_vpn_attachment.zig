...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSiteToSiteVpnAttachmentInput, options: Options) !CreateSiteToSiteVpnAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSiteToSiteVpnAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"CoreNetworkId\":");
    try aws.json.writeValue(@TypeOf(input.core_network_id), input.core_network_id, alloc, &body_buf);
    has_prev = true;
    if (input.routing_policy_label) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RoutingPolicyLabel\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VpnConnectionArn\":");
    try aws.json.writeValue(@TypeOf(input.vpn_connection_arn), input.vpn_connection_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSiteToSiteVpnAttachmentOutput {
    var result: CreateSiteToSiteVpnAttachmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSiteToSiteVpnAttachmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
