...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGlobalNetworkInput, options: Options) !UpdateGlobalNetworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGlobalNetworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGlobalNetworkOutput {
    var result: UpdateGlobalNetworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGlobalNetworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
