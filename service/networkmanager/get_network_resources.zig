...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNetworkResourcesInput, options: CallOptions) !GetNetworkResourcesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetNetworkResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.registered_gateway_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "registeredGatewayArn=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetNetworkResourcesOutput {
    var result: GetNetworkResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNetworkResourcesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
