...
const NetworkResource = @import("network_resource.zig").NetworkResource;

pub const GetNetworkResourcesInput = struct {
...
pub const GetNetworkResourcesOutput = struct {
    /// The network resources.
    network_resources: ?[]const NetworkResource = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNetworkResourcesInput, options: Options) !GetNetworkResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNetworkResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/network-resources");
    ...
    if (input.aws_region) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "awsRegion=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.core_network_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "coreNetworkId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.registered_gateway_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "registeredGatewayArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNetworkResourcesOutput {
    var result: GetNetworkResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNetworkResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
