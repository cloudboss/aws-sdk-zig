...
pub const DeregisterTransitGatewayInput = struct {
...
pub const DeregisterTransitGatewayOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterTransitGatewayInput, options: Options) !DeregisterTransitGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterTransitGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.transit_gateway_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterTransitGatewayOutput {
    var result: DeregisterTransitGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeregisterTransitGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
