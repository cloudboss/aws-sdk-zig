...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPeeringsInput, options: Options) !ListPeeringsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPeeringsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.peering_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "peeringType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPeeringsOutput {
    var result: ListPeeringsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPeeringsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
