...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DecryptDataInput, options: Options) !DecryptDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DecryptDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.key_identifier);
    try path_buf.appendSlice(alloc, "/decrypt");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DecryptionAttributes\":");
    try aws.json.writeValue(@TypeOf(input.decryption_attributes), input.decryption_attributes, alloc, &body_buf);
    has_prev = true;
    if (input.wrapped_key) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"WrappedKey\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DecryptDataOutput {
    var result: DecryptDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DecryptDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
