...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReEncryptDataInput, options: Options) !ReEncryptDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReEncryptDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.incoming_key_identifier);
    try path_buf.appendSlice(alloc, "/reencrypt");
    ...
    try body_buf.appendSlice(alloc, "\"CipherText\":");
    try aws.json.writeValue(@TypeOf(input.cipher_text), input.cipher_text, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"IncomingEncryptionAttributes\":");
    try aws.json.writeValue(@TypeOf(input.incoming_encryption_attributes), input.incoming_encryption_attributes, alloc, &body_buf);
    has_prev = true;
    if (input.incoming_wrapped_key) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IncomingWrappedKey\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OutgoingEncryptionAttributes\":");
    try aws.json.writeValue(@TypeOf(input.outgoing_encryption_attributes), input.outgoing_encryption_attributes, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OutgoingKeyIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.outgoing_key_identifier), input.outgoing_key_identifier, alloc, &body_buf);
    has_prev = true;
    if (input.outgoing_wrapped_key) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OutgoingWrappedKey\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReEncryptDataOutput {
    var result: ReEncryptDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ReEncryptDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
