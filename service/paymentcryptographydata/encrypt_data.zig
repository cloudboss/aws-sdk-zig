...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EncryptDataInput, options: Options) !EncryptDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EncryptDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/encrypt");
    ...
    try body_buf.appendSlice(alloc, "\"EncryptionAttributes\":");
    try aws.json.writeValue(@TypeOf(input.encryption_attributes), input.encryption_attributes, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PlainText\":");
    try aws.json.writeValue(@TypeOf(input.plain_text), input.plain_text, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EncryptDataOutput {
    var result: EncryptDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EncryptDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
