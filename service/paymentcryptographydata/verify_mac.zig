...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VerifyMacInput, options: Options) !VerifyMacOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyMacInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Mac\":");
    try aws.json.writeValue(@TypeOf(input.mac), input.mac, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyMacOutput {
    var result: VerifyMacOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(VerifyMacOutput, body, alloc);
    }
    _ = status;
    _ = headers;
