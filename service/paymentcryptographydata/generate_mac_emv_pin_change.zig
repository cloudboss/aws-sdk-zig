...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateMacEmvPinChangeInput, options: Options) !GenerateMacEmvPinChangeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateMacEmvPinChangeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"DerivationMethodAttributes\":");
    try aws.json.writeValue(@TypeOf(input.derivation_method_attributes), input.derivation_method_attributes, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MessageData\":");
    try aws.json.writeValue(@TypeOf(input.message_data), input.message_data, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"NewEncryptedPinBlock\":");
    try aws.json.writeValue(@TypeOf(input.new_encrypted_pin_block), input.new_encrypted_pin_block, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"NewPinPekIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.new_pin_pek_identifier), input.new_pin_pek_identifier, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PinBlockFormat\":");
    try aws.json.writeValue(@TypeOf(input.pin_block_format), input.pin_block_format, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SecureMessagingConfidentialityKeyIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.secure_messaging_confidentiality_key_identifier), input.secure_messaging_confidentiality_key_identifier, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SecureMessagingIntegrityKeyIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.secure_messaging_integrity_key_identifier), input.secure_messaging_integrity_key_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateMacEmvPinChangeOutput {
    var result: GenerateMacEmvPinChangeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateMacEmvPinChangeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
