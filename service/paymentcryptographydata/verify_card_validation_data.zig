...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VerifyCardValidationDataInput, options: Options) !VerifyCardValidationDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyCardValidationDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"KeyIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.key_identifier), input.key_identifier, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PrimaryAccountNumber\":");
    try aws.json.writeValue(@TypeOf(input.primary_account_number), input.primary_account_number, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ValidationData\":");
    try aws.json.writeValue(@TypeOf(input.validation_data), input.validation_data, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VerificationAttributes\":");
    try aws.json.writeValue(@TypeOf(input.verification_attributes), input.verification_attributes, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyCardValidationDataOutput {
    var result: VerifyCardValidationDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(VerifyCardValidationDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
