...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VerifyPinDataInput, options: Options) !VerifyPinDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyPinDataInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dukpt_attributes) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DukptAttributes\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EncryptionKeyIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.encryption_key_identifier), input.encryption_key_identifier, alloc, &body_buf);
    has_prev = true;
    if (input.encryption_wrapped_key) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EncryptionWrappedKey\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.pin_data_length) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PinDataLength\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VerificationKeyIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.verification_key_identifier), input.verification_key_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyPinDataOutput {
    var result: VerifyPinDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(VerifyPinDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
