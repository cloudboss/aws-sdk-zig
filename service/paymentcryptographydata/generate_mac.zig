...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateMacInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"GenerationAttributes\":");
    try aws.json.writeValue(@TypeOf(input.generation_attributes), input.generation_attributes, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.mac_length) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MacLength\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateMacOutput {
    var result: GenerateMacOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateMacOutput, body, alloc);
    }
    _ = status;
    _ = headers;
