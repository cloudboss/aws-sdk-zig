...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VerifyAuthRequestCryptogramInput, options: Options) !VerifyAuthRequestCryptogramOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyAuthRequestCryptogramInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AuthRequestCryptogram\":");
    try aws.json.writeValue(@TypeOf(input.auth_request_cryptogram), input.auth_request_cryptogram, alloc, &body_buf);
    has_prev = true;
    if (input.auth_response_attributes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"AuthResponseAttributes\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MajorKeyDerivationMode\":");
    try aws.json.writeValue(@TypeOf(input.major_key_derivation_mode), input.major_key_derivation_mode, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SessionKeyDerivationAttributes\":");
    try aws.json.writeValue(@TypeOf(input.session_key_derivation_attributes), input.session_key_derivation_attributes, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TransactionData\":");
    try aws.json.writeValue(@TypeOf(input.transaction_data), input.transaction_data, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyAuthRequestCryptogramOutput {
    var result: VerifyAuthRequestCryptogramOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(VerifyAuthRequestCryptogramOutput, body, alloc);
    }
    _ = status;
    _ = headers;
