...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateCardValidationDataInput, options: Options) !GenerateCardValidationDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateCardValidationDataInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.validation_data_length) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ValidationDataLength\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateCardValidationDataOutput {
    var result: GenerateCardValidationDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateCardValidationDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
