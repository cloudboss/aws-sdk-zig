...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateAs2805KekValidationInput, options: CallOptions) !GenerateAs2805KekValidationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GenerateAs2805KekValidationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"KekValidationType\":");
    try aws.json.writeValue(@TypeOf(input.kek_validation_type), input.kek_validation_type, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"RandomKeySendVariantMask\":");
    try aws.json.writeValue(@TypeOf(input.random_key_send_variant_mask), input.random_key_send_variant_mask, allocator, &body_buf);
    has_prev = true;
