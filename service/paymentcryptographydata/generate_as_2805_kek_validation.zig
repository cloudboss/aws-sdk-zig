...
const As2805KekValidationType = @import("as_2805_kek_validation_type.zig").As2805KekValidationType;
...
pub const GenerateAs2805KekValidationInput = struct {
    ...
    /// uses for node-to-node initialization
    ...
    random_key_send_variant_mask: RandomKeySendVariantMask,

    pub const json_field_names = .{
        .kek_validation_type = "KekValidationType",
        .key_identifier = "KeyIdentifier",
        .random_key_send_variant_mask = "RandomKeySendVariantMask",
    };
};

pub const GenerateAs2805KekValidationOutput = struct {
    /// The `keyARN` of sending KEK that Amazon Web Services Payment Cryptography
    ...
    pub const json_field_names = .{
        ...
        .random_key_receive = "RandomKeyReceive",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateAs2805KekValidationInput, options: Options) !GenerateAs2805KekValidationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateAs2805KekValidationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/as2805kekvalidation/generate";
    ...
    try body_buf.appendSlice(alloc, "\"KekValidationType\":");
    try aws.json.writeValue(@TypeOf(input.kek_validation_type), input.kek_validation_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RandomKeySendVariantMask\":");
    try aws.json.writeValue(@TypeOf(input.random_key_send_variant_mask), input.random_key_send_variant_mask, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateAs2805KekValidationOutput {
    var result: GenerateAs2805KekValidationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateAs2805KekValidationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
