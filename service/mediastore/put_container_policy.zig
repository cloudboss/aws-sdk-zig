...
pub const PutContainerPolicyInput = struct {
...
pub const PutContainerPolicyOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutContainerPolicyInput, options: CallOptions) !PutContainerPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutContainerPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MediaStore_20170901.PutContainerPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutContainerPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
