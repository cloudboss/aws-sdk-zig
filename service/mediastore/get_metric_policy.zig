...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMetricPolicyInput, options: Options) !GetMetricPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetricPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MediaStore_20170901.GetMetricPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetricPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMetricPolicyOutput, body, alloc);
}
