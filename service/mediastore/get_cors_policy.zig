...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCorsPolicyInput, options: CallOptions) !GetCorsPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCorsPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MediaStore_20170901.GetCorsPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCorsPolicyOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetCorsPolicyOutput, body, allocator);
}
