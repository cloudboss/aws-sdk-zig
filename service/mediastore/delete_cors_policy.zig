...
pub const DeleteCorsPolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCorsPolicyInput, options: Options) !DeleteCorsPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCorsPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MediaStore_20170901.DeleteCorsPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCorsPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
