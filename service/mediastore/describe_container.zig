...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContainerInput, options: CallOptions) !DescribeContainerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeContainerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MediaStore_20170901.DescribeContainer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeContainerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeContainerOutput, body, allocator);
}
