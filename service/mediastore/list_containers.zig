...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContainersInput, options: Options) !ListContainersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContainersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MediaStore_20170901.ListContainers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContainersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListContainersOutput, body, alloc);
}
