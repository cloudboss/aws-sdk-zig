...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContainerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MediaStore_20170901.CreateContainer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContainerOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateContainerOutput, body, alloc);
}
