...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutMetricPolicyInput, options: Options) !PutMetricPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutMetricPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MediaStore_20170901.PutMetricPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutMetricPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
