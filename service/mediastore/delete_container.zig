...
pub const DeleteContainerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteContainerInput, options: Options) !DeleteContainerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteContainerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MediaStore_20170901.DeleteContainer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteContainerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
