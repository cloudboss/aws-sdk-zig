...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAutoScalingConfigurationInput, options: CallOptions) !CreateAutoScalingConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAutoScalingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AppRunner.CreateAutoScalingConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAutoScalingConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateAutoScalingConfigurationOutput, body, allocator);
}
