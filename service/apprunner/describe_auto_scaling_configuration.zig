...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAutoScalingConfigurationInput, options: Options) !DescribeAutoScalingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAutoScalingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.DescribeAutoScalingConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAutoScalingConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAutoScalingConfigurationOutput, body, alloc);
}
