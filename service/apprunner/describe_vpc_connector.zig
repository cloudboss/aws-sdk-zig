...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpcConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.DescribeVpcConnector");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVpcConnectorOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeVpcConnectorOutput, body, alloc);
}
