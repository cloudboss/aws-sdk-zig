...
pub const UpdateVpcIngressConnectionInput = struct {
...
pub const UpdateVpcIngressConnectionOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateVpcIngressConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AppRunner.UpdateVpcIngressConnection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateVpcIngressConnectionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateVpcIngressConnectionOutput, body, allocator);
}
