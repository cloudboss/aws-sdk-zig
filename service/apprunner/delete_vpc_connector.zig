...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVpcConnectorInput, options: Options) !DeleteVpcConnectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVpcConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.DeleteVpcConnector");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVpcConnectorOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteVpcConnectorOutput, body, alloc);
}
