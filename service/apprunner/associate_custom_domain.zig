...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateCustomDomainInput, options: CallOptions) !AssociateCustomDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateCustomDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AppRunner.AssociateCustomDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateCustomDomainOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(AssociateCustomDomainOutput, body, allocator);
}
