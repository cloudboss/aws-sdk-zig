...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAutoScalingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.DeleteAutoScalingConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAutoScalingConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteAutoScalingConfigurationOutput, body, alloc);
}
