...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteServiceInput, options: Options) !DeleteServiceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.DeleteService");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteServiceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteServiceOutput, body, alloc);
}
