...
pub const ListVpcConnectorsInput = struct {
...
pub const ListVpcConnectorsOutput = struct {
    ...
    /// the request returns up to `MaxResults` records for each
    /// call.
    vpc_connectors: ?[]const VpcConnector = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVpcConnectorsInput, options: CallOptions) !ListVpcConnectorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVpcConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AppRunner.ListVpcConnectors");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVpcConnectorsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListVpcConnectorsOutput, body, allocator);
}
