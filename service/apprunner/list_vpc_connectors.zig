...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVpcConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.ListVpcConnectors");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVpcConnectorsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListVpcConnectorsOutput, body, alloc);
}
