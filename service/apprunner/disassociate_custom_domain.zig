...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateCustomDomainInput, options: Options) !DisassociateCustomDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateCustomDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.DisassociateCustomDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateCustomDomainOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DisassociateCustomDomainOutput, body, alloc);
}
