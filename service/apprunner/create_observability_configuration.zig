...
const ObservabilityConfiguration = @import("observability_configuration.zig").ObservabilityConfiguration;

pub const CreateObservabilityConfigurationInput = struct {
    ...
    /// new observability configuration, and you can't create a
    /// revision of it.
    ///
...
pub const CreateObservabilityConfigurationOutput = struct {
    /// A description of the App Runner observability configuration that's created
    /// by this request.
    observability_configuration: ?ObservabilityConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateObservabilityConfigurationInput, options: CallOptions) !CreateObservabilityConfigurationOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "apprunner");
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateObservabilityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AppRunner.CreateObservabilityConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateObservabilityConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateObservabilityConfigurationOutput, body, allocator);
}
