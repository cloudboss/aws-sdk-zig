...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVpcIngressConnectionsInput, options: Options) !ListVpcIngressConnectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVpcIngressConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.ListVpcIngressConnections");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVpcIngressConnectionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListVpcIngressConnectionsOutput, body, alloc);
}
