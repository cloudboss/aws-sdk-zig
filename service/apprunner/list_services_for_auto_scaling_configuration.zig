...
pub const ListServicesForAutoScalingConfigurationInput = struct {
...
pub const ListServicesForAutoScalingConfigurationOutput = struct {
    ...
    service_arn_list: ?[]const []const u8 = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .service_arn_list = "ServiceArnList",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServicesForAutoScalingConfigurationInput, options: CallOptions) !ListServicesForAutoScalingConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListServicesForAutoScalingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AppRunner.ListServicesForAutoScalingConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListServicesForAutoScalingConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListServicesForAutoScalingConfigurationOutput, body, allocator);
}
