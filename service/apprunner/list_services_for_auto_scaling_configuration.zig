...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServicesForAutoScalingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.ListServicesForAutoScalingConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServicesForAutoScalingConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListServicesForAutoScalingConfigurationOutput, body, alloc);
}
