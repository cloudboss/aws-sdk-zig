...
pub const CreateVpcIngressConnectionInput = struct {
    ...
    /// the active VPC Ingress Connections in your Amazon Web Services account in
...
pub const CreateVpcIngressConnectionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVpcIngressConnectionInput, options: CallOptions) !CreateVpcIngressConnectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVpcIngressConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AppRunner.CreateVpcIngressConnection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVpcIngressConnectionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateVpcIngressConnectionOutput, body, allocator);
}
