...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeServiceInput, options: Options) !DescribeServiceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.DescribeService");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeServiceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeServiceOutput, body, alloc);
}
