...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVpcIngressConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.DeleteVpcIngressConnection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVpcIngressConnectionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteVpcIngressConnectionOutput, body, alloc);
}
