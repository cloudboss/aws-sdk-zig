...
pub const ListAutoScalingConfigurationsInput = struct {
    /// The name of the App Runner auto scaling configuration that you want to list.
    /// If specified, App Runner lists revisions that share this name. If not
    /// specified, App Runner
...
pub const ListAutoScalingConfigurationsOutput = struct {
    ...
    auto_scaling_configuration_summary_list: ?[]const AutoScalingConfigurationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutoScalingConfigurationsInput, options: CallOptions) !ListAutoScalingConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAutoScalingConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AppRunner.ListAutoScalingConfigurations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAutoScalingConfigurationsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListAutoScalingConfigurationsOutput, body, allocator);
}
