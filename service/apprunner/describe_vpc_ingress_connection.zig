...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpcIngressConnectionInput, options: Options) !DescribeVpcIngressConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpcIngressConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.DescribeVpcIngressConnection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVpcIngressConnectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeVpcIngressConnectionOutput, body, alloc);
}
