...
pub const CreateVpcConnectorInput = struct {
    ...
    /// App Runner uses the
    /// default security group of the Amazon VPC. The default security group allows
    ...
    /// specify.
    ///
    /// App Runner only supports subnets of IP address type *IPv4* and *dual stack*
    /// (IPv4 and IPv6).
...
pub const CreateVpcConnectorOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVpcConnectorInput, options: CallOptions) !CreateVpcConnectorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVpcConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AppRunner.CreateVpcConnector");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVpcConnectorOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateVpcConnectorOutput, body, allocator);
}
