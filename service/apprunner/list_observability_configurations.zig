...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObservabilityConfigurationsInput, options: Options) !ListObservabilityConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListObservabilityConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AppRunner.ListObservabilityConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListObservabilityConfigurationsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListObservabilityConfigurationsOutput, body, alloc);
}
