...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartReportCreationInput, options: Options) !StartReportCreationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartReportCreationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ResourceGroupsTaggingAPI_20170126.StartReportCreation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartReportCreationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
