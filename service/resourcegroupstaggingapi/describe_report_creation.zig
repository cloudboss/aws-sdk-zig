...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReportCreationInput, options: Options) !DescribeReportCreationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReportCreationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ResourceGroupsTaggingAPI_20170126.DescribeReportCreation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReportCreationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReportCreationOutput, body, alloc);
}
