...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UntagResourcesInput, options: Options) !UntagResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ResourceGroupsTaggingAPI_20170126.UntagResources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UntagResourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UntagResourcesOutput, body, alloc);
}
