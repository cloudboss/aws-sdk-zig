...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTagValuesInput, options: Options) !GetTagValuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTagValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ResourceGroupsTaggingAPI_20170126.GetTagValues");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTagValuesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTagValuesOutput, body, alloc);
}
