...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetComplianceSummaryInput, options: Options) !GetComplianceSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetComplianceSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ResourceGroupsTaggingAPI_20170126.GetComplianceSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetComplianceSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetComplianceSummaryOutput, body, alloc);
}
