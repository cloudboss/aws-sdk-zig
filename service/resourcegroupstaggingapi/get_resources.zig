...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourcesInput, options: CallOptions) !GetResourcesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ResourceGroupsTaggingAPI_20170126.GetResources");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResourcesOutput, body, allocator);
}
