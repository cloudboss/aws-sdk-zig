...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TagResourcesInput, options: Options) !TagResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TagResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ResourceGroupsTaggingAPI_20170126.TagResources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TagResourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TagResourcesOutput, body, alloc);
}
