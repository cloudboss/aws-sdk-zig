...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTagKeysInput, options: Options) !GetTagKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTagKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ResourceGroupsTaggingAPI_20170126.GetTagKeys");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTagKeysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTagKeysOutput, body, alloc);
}
