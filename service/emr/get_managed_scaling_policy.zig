...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetManagedScalingPolicyInput, options: Options) !GetManagedScalingPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetManagedScalingPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.GetManagedScalingPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetManagedScalingPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetManagedScalingPolicyOutput, body, alloc);
}
