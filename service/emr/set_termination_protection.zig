...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetTerminationProtectionInput, options: Options) !SetTerminationProtectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetTerminationProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.SetTerminationProtection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetTerminationProtectionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
