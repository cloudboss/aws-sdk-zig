...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetClusterSessionCredentialsInput, options: Options) !GetClusterSessionCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetClusterSessionCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.GetClusterSessionCredentials");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetClusterSessionCredentialsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetClusterSessionCredentialsOutput, body, alloc);
}
