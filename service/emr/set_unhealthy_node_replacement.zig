...
fn serializeRequest(alloc: std.mem.Allocator, input: SetUnhealthyNodeReplacementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.SetUnhealthyNodeReplacement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetUnhealthyNodeReplacementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
