...
pub const PutAutoScalingPolicyInput = struct {
...
pub const PutAutoScalingPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAutoScalingPolicyInput, options: CallOptions) !PutAutoScalingPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutAutoScalingPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ElasticMapReduce.PutAutoScalingPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutAutoScalingPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutAutoScalingPolicyOutput, body, allocator);
}
