...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStudioSessionMappingInput, options: Options) !DeleteStudioSessionMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteStudioSessionMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.DeleteStudioSessionMapping");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteStudioSessionMappingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
