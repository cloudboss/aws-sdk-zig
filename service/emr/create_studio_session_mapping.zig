...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStudioSessionMappingInput, options: Options) !CreateStudioSessionMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStudioSessionMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.CreateStudioSessionMapping");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStudioSessionMappingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
