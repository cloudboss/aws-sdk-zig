...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBlockPublicAccessConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.GetBlockPublicAccessConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBlockPublicAccessConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetBlockPublicAccessConfigurationOutput, body, alloc);
}
