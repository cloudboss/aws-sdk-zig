...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListNotebookExecutionsInput, options: Options) !ListNotebookExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListNotebookExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.ListNotebookExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListNotebookExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListNotebookExecutionsOutput, body, alloc);
}
