...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSupportedInstanceTypesInput, options: Options) !ListSupportedInstanceTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSupportedInstanceTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.ListSupportedInstanceTypes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSupportedInstanceTypesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSupportedInstanceTypesOutput, body, alloc);
}
