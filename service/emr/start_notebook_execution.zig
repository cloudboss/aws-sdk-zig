...
fn serializeRequest(allocator: std.mem.Allocator, input: StartNotebookExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ElasticMapReduce.StartNotebookExecution");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartNotebookExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartNotebookExecutionOutput, body, allocator);
}
