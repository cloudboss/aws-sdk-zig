...
pub const GetPersistentAppUIPresignedURLInput = struct {
    ...
    /// URL.
    persistent_app_ui_type: ?PersistentAppUIType = null,

    pub const json_field_names = .{
        .application_id = "ApplicationId",
        .auth_proxy_call = "AuthProxyCall",
...
pub const GetPersistentAppUIPresignedURLOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPersistentAppUIPresignedURLInput, options: CallOptions) !GetPersistentAppUIPresignedURLOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPersistentAppUIPresignedURLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ElasticMapReduce.GetPersistentAppUIPresignedURL");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPersistentAppUIPresignedURLOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPersistentAppUIPresignedURLOutput, body, allocator);
}
