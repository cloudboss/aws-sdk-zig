...
pub const ModifyClusterInput = struct {
    ...
    /// minimum of 1
    /// step and a maximum of 256 steps. We recommend that you do not change this
    /// parameter while
    /// steps are running or the `ActionOnFailure` setting may not behave as
    /// expected.
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ElasticMapReduce.ModifyCluster");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyClusterOutput {
    ...
    return aws.json.parseJsonObject(ModifyClusterOutput, body, allocator);
}
