...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RunJobFlowInput, options: Options) !RunJobFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RunJobFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.RunJobFlow");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RunJobFlowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RunJobFlowOutput, body, alloc);
}
