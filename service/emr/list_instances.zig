...
pub const ListInstancesInput = struct {
    ...
    instance_group_types: ?[]const InstanceGroupType = null,

    /// A list of instance states that will filter the instances returned with this
    /// request.
    instance_states: ?[]const InstanceState = null,
    ...
    pub const json_field_names = .{
        ...
        .instance_group_types = "InstanceGroupTypes",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInstancesInput, options: Options) !ListInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.ListInstances");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInstancesOutput, body, alloc);
}
