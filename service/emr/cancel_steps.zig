...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelStepsInput, options: Options) !CancelStepsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelStepsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.CancelSteps");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelStepsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelStepsOutput, body, alloc);
}
