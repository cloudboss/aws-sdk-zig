...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetVisibleToAllUsersInput, options: Options) !SetVisibleToAllUsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetVisibleToAllUsersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.SetVisibleToAllUsers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetVisibleToAllUsersOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
