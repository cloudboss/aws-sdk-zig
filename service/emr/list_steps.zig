...
pub const ListStepsInput = struct {
    ...
    /// To return a longer list of steps, use multiple `ListSteps` actions along
    /// with
    ...
    /// can specify
    /// a maximum of ten Step IDs. The character constraint applies to the overall
    /// length of the
    /// array.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStepsInput, options: Options) !ListStepsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListStepsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ElasticMapReduce.ListSteps");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListStepsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListStepsOutput, body, allocator);
}
