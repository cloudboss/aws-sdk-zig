...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddJobFlowStepsInput, options: Options) !AddJobFlowStepsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddJobFlowStepsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.AddJobFlowSteps");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddJobFlowStepsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AddJobFlowStepsOutput, body, alloc);
}
