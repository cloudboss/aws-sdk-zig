...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStudioSessionMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.UpdateStudioSessionMapping");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStudioSessionMappingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
