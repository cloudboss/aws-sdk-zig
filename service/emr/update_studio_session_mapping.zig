...
pub const UpdateStudioSessionMappingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStudioSessionMappingInput, options: CallOptions) !UpdateStudioSessionMappingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateStudioSessionMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ElasticMapReduce.UpdateStudioSessionMapping");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateStudioSessionMappingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
