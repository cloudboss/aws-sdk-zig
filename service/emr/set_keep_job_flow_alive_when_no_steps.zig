...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetKeepJobFlowAliveWhenNoStepsInput, options: Options) !SetKeepJobFlowAliveWhenNoStepsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetKeepJobFlowAliveWhenNoStepsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.SetKeepJobFlowAliveWhenNoSteps");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetKeepJobFlowAliveWhenNoStepsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
