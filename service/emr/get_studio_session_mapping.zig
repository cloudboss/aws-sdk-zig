...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStudioSessionMappingInput, options: Options) !GetStudioSessionMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStudioSessionMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.GetStudioSessionMapping");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStudioSessionMappingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetStudioSessionMappingOutput, body, alloc);
}
