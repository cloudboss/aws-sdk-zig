...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddInstanceGroupsInput, options: Options) !AddInstanceGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddInstanceGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ElasticMapReduce.AddInstanceGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddInstanceGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AddInstanceGroupsOutput, body, alloc);
}
