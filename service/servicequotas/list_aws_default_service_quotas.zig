...
pub const ListAWSDefaultServiceQuotasInput = struct {
...
pub const ListAWSDefaultServiceQuotasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAWSDefaultServiceQuotasInput, options: CallOptions) !ListAWSDefaultServiceQuotasOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "servicequotas");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAWSDefaultServiceQuotasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ServiceQuotasV20190624.ListAWSDefaultServiceQuotas");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAWSDefaultServiceQuotasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAWSDefaultServiceQuotasOutput, body, allocator);
}
