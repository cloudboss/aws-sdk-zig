...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAWSDefaultServiceQuotasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.ListAWSDefaultServiceQuotas");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAWSDefaultServiceQuotasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAWSDefaultServiceQuotasOutput, body, alloc);
}
