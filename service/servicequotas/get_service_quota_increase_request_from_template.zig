...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceQuotaIncreaseRequestFromTemplateInput, options: Options) !GetServiceQuotaIncreaseRequestFromTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceQuotaIncreaseRequestFromTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.GetServiceQuotaIncreaseRequestFromTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceQuotaIncreaseRequestFromTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetServiceQuotaIncreaseRequestFromTemplateOutput, body, alloc);
}
