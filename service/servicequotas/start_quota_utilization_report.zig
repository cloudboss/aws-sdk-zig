...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartQuotaUtilizationReportInput, options: Options) !StartQuotaUtilizationReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartQuotaUtilizationReportInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.StartQuotaUtilizationReport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartQuotaUtilizationReportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartQuotaUtilizationReportOutput, body, alloc);
}
