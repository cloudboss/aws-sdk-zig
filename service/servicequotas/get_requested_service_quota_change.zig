...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRequestedServiceQuotaChangeInput, options: Options) !GetRequestedServiceQuotaChangeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRequestedServiceQuotaChangeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.GetRequestedServiceQuotaChange");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRequestedServiceQuotaChangeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRequestedServiceQuotaChangeOutput, body, alloc);
}
