...
pub const StopAutoManagementInput = struct {
};

pub const StopAutoManagementOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopAutoManagementInput, options: CallOptions) !StopAutoManagementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopAutoManagementInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ServiceQuotasV20190624.StopAutoManagement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopAutoManagementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
