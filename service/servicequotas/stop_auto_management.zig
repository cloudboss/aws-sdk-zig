...
fn serializeRequest(alloc: std.mem.Allocator, input: StopAutoManagementInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.StopAutoManagement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopAutoManagementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
