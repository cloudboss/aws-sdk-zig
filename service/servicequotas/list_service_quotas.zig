...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceQuotasInput, options: Options) !ListServiceQuotasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServiceQuotasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.ListServiceQuotas");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServiceQuotasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListServiceQuotasOutput, body, alloc);
}
