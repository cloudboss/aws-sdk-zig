...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssociationForServiceQuotaTemplateInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.GetAssociationForServiceQuotaTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssociationForServiceQuotaTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAssociationForServiceQuotaTemplateOutput, body, alloc);
}
