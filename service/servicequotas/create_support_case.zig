...
pub const CreateSupportCaseInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSupportCaseInput, options: CallOptions) !CreateSupportCaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSupportCaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ServiceQuotasV20190624.CreateSupportCase");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSupportCaseOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
