...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSupportCaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.CreateSupportCase");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSupportCaseOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
