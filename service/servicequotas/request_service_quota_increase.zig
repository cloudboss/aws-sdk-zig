...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RequestServiceQuotaIncreaseInput, options: Options) !RequestServiceQuotaIncreaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RequestServiceQuotaIncreaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.RequestServiceQuotaIncrease");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RequestServiceQuotaIncreaseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RequestServiceQuotaIncreaseOutput, body, alloc);
}
