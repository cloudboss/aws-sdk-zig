...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAWSDefaultServiceQuotaInput, options: Options) !GetAWSDefaultServiceQuotaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAWSDefaultServiceQuotaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.GetAWSDefaultServiceQuota");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAWSDefaultServiceQuotaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAWSDefaultServiceQuotaOutput, body, alloc);
}
