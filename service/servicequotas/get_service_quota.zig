...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceQuotaInput, options: Options) !GetServiceQuotaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceQuotaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.GetServiceQuota");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceQuotaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetServiceQuotaOutput, body, alloc);
}
