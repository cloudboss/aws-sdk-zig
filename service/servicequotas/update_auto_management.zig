...
pub const UpdateAutoManagementInput = struct {
    ...
    /// The [User
    /// Notifications](https://docs.aws.amazon.com/notifications/latest/userguide/resource-level-permissions.html#rlp-table) Amazon Resource Name (ARN) for Automatic Management notifications you want
    /// to update.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAutoManagementInput, options: Options) !UpdateAutoManagementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAutoManagementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ServiceQuotasV20190624.UpdateAutoManagement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAutoManagementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
