...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteServiceQuotaIncreaseRequestFromTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.DeleteServiceQuotaIncreaseRequestFromTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteServiceQuotaIncreaseRequestFromTemplateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
