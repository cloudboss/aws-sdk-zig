...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAutoManagementInput, options: Options) !StartAutoManagementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartAutoManagementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ServiceQuotasV20190624.StartAutoManagement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartAutoManagementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
