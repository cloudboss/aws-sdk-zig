...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteActionInput, options: Options) !ExecuteActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"actionDefinitionId\":");
    try aws.json.writeValue(@TypeOf(input.action_definition_id), input.action_definition_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"actionPayload\":");
    try aws.json.writeValue(@TypeOf(input.action_payload), input.action_payload, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.resolve_to) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resolveTo\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"targetResource\":");
    try aws.json.writeValue(@TypeOf(input.target_resource), input.target_resource, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteActionOutput {
    var result: ExecuteActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ExecuteActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
