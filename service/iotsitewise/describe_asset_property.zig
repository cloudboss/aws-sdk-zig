...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssetPropertyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssetPropertyOutput {
    var result: DescribeAssetPropertyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAssetPropertyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
