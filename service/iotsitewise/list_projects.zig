...
pub const ListProjectsOutput = struct {
    ...
    project_summaries: ?[]const ProjectSummary = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProjectsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "portalId=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.portal_id);
    query_has_prev = true;
