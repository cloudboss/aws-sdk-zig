...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAssetModelCompositeModelInput, options: Options) !UpdateAssetModelCompositeModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssetModelCompositeModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.asset_model_composite_model_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssetModelCompositeModelOutput {
    var result: UpdateAssetModelCompositeModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAssetModelCompositeModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
