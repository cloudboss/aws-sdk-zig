...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetAssetPropertyValueInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetAssetPropertyValueOutput {
    var result: BatchGetAssetPropertyValueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetAssetPropertyValueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
