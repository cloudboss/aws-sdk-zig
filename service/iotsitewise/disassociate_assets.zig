...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateAssetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"childAssetId\":");
    try aws.json.writeValue(@TypeOf(input.child_asset_id), input.child_asset_id, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"hierarchyId\":");
    try aws.json.writeValue(@TypeOf(input.hierarchy_id), input.hierarchy_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateAssetsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
