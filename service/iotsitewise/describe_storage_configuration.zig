...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStorageConfigurationInput, options: Options) !DescribeStorageConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStorageConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStorageConfigurationOutput {
    var result: DescribeStorageConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeStorageConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
