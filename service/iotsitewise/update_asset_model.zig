...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssetModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssetModelOutput {
    var result: UpdateAssetModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAssetModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
