...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessPoliciesInput, options: Options) !ListAccessPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.iam_arn) |v| {
        ...
        try query_buf.appendSlice(alloc, "iamArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.identity_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "identityId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.identity_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "identityType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessPoliciesOutput {
    var result: ListAccessPoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccessPoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
