...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListActionsInput, options: Options) !ListActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "targetResourceId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.target_resource_id);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "targetResourceType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.target_resource_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListActionsOutput {
    var result: ListActionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListActionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
