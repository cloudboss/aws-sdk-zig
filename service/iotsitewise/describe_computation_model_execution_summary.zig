...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeComputationModelExecutionSummaryInput, options: Options) !DescribeComputationModelExecutionSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeComputationModelExecutionSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/computation-models/");
    try path_buf.appendSlice(alloc, input.computation_model_id);
    try path_buf.appendSlice(alloc, "/execution-summary");
    ...
    if (input.resolve_to_resource_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "resolveToResourceId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.resolve_to_resource_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "resolveToResourceType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeComputationModelExecutionSummaryOutput {
    var result: DescribeComputationModelExecutionSummaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeComputationModelExecutionSummaryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
