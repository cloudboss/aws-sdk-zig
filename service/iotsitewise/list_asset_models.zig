...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssetModelsInput, options: Options) !ListAssetModelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssetModelsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.asset_model_types) |v| {
        ...
        try query_buf.appendSlice(alloc, "assetModelTypes=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssetModelsOutput {
    var result: ListAssetModelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetModelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
