...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDashboardInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"dashboardDefinition\":");
    try aws.json.writeValue(@TypeOf(input.dashboard_definition), input.dashboard_definition, alloc, &body_buf);
    has_prev = true;
    if (input.dashboard_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"dashboardDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"dashboardName\":");
    try aws.json.writeValue(@TypeOf(input.dashboard_name), input.dashboard_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDashboardOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateDashboardOutput = .{};
