...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssetModelCompositeModelInput, options: Options) !CreateAssetModelCompositeModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssetModelCompositeModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/asset-models/");
    try path_buf.appendSlice(alloc, input.asset_model_id);
    ...
    if (input.asset_model_composite_model_description) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"assetModelCompositeModelDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.asset_model_composite_model_external_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetModelCompositeModelExternalId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.asset_model_composite_model_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetModelCompositeModelId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"assetModelCompositeModelName\":");
    try aws.json.writeValue(@TypeOf(input.asset_model_composite_model_name), input.asset_model_composite_model_name, alloc, &body_buf);
    has_prev = true;
    if (input.asset_model_composite_model_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetModelCompositeModelProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"assetModelCompositeModelType\":");
    try aws.json.writeValue(@TypeOf(input.asset_model_composite_model_type), input.asset_model_composite_model_type, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.composed_asset_model_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"composedAssetModelId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.parent_asset_model_composite_model_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"parentAssetModelCompositeModelId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.match_for_version_type) |v| {
        try request.headers.put(alloc, "Match-For-Version-Type", @tagName(v));
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssetModelCompositeModelOutput {
    var result: CreateAssetModelCompositeModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssetModelCompositeModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
