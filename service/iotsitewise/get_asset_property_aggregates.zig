...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssetPropertyAggregatesInput, options: Options) !GetAssetPropertyAggregatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssetPropertyAggregatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "aggregateTypes=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.aggregate_types);
    query_has_prev = true;
    if (input.asset_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.resolution);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssetPropertyAggregatesOutput {
    var result: GetAssetPropertyAggregatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssetPropertyAggregatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
