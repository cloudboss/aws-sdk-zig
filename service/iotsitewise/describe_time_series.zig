...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTimeSeriesInput, options: Options) !DescribeTimeSeriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTimeSeriesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTimeSeriesOutput {
    var result: DescribeTimeSeriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTimeSeriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
