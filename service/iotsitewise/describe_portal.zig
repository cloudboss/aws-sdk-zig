...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePortalInput, options: Options) !DescribePortalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePortalInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePortalOutput {
    var result: DescribePortalOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePortalOutput, body, alloc);
    }
    _ = status;
    _ = headers;
