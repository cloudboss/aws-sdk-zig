...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.asset_description) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"assetDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.asset_external_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetExternalId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"assetName\":");
    try aws.json.writeValue(@TypeOf(input.asset_name), input.asset_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssetOutput {
    var result: UpdateAssetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAssetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
