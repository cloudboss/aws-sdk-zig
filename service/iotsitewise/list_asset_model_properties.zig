...
const ListAssetModelPropertiesFilter = @import("list_asset_model_properties_filter.zig").ListAssetModelPropertiesFilter;
const AssetModelPropertySummary = @import("asset_model_property_summary.zig").AssetModelPropertySummary;

pub const ListAssetModelPropertiesInput = struct {
    ...
    ///
    /// Default: `BASE`
    filter: ?ListAssetModelPropertiesFilter = null,
...
pub const ListAssetModelPropertiesOutput = struct {
    /// A list that summarizes the properties associated with the specified asset
    /// model.
    asset_model_property_summaries: ?[]const AssetModelPropertySummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssetModelPropertiesInput, options: Options) !ListAssetModelPropertiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssetModelPropertiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssetModelPropertiesOutput {
    var result: ListAssetModelPropertiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetModelPropertiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
