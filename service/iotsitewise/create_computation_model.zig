...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateComputationModelInput, options: CallOptions) !CreateComputationModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateComputationModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateComputationModelOutput {
    var result: CreateComputationModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateComputationModelOutput, body, allocator);
    }
    _ = status;
    _ = headers;
