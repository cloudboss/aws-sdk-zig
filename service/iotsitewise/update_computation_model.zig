...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateComputationModelInput, options: Options) !UpdateComputationModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateComputationModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateComputationModelOutput {
    var result: UpdateComputationModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateComputationModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
