...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssetModelInput, options: Options) !CreateAssetModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssetModelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.asset_model_composite_models) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"assetModelCompositeModels\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.asset_model_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetModelDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.asset_model_external_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetModelExternalId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.asset_model_hierarchies) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetModelHierarchies\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.asset_model_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetModelId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"assetModelName\":");
    try aws.json.writeValue(@TypeOf(input.asset_model_name), input.asset_model_name, alloc, &body_buf);
    has_prev = true;
    if (input.asset_model_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetModelProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.asset_model_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"assetModelType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssetModelOutput {
    var result: CreateAssetModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssetModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
