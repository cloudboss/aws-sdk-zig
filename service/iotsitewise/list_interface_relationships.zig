...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInterfaceRelationshipsInput, options: Options) !ListInterfaceRelationshipsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInterfaceRelationshipsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInterfaceRelationshipsOutput {
    var result: ListInterfaceRelationshipsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInterfaceRelationshipsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
