...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssetModelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.asset_model_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "assetModelVersion=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.exclude_properties) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "excludeProperties=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssetModelOutput {
    var result: DescribeAssetModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAssetModelOutput, body, alloc);
    }
    _ = status;
