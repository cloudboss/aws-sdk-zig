...
const ListAssetsFilter = @import("list_assets_filter.zig").ListAssetsFilter;
...
pub const ListAssetsInput = struct {
    ...
    /// parameter is
    /// required if you choose `ALL` for `filter`. This can be either the actual ID
...
pub const ListAssetsOutput = struct {
    ...
    asset_summaries: ?[]const AssetSummary = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssetsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.asset_model_id) |v| {
        ...
        try query_buf.appendSlice(allocator, "assetModelId=");
