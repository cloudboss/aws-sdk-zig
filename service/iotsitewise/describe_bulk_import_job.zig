...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBulkImportJobInput, options: Options) !DescribeBulkImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBulkImportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBulkImportJobOutput {
    var result: DescribeBulkImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBulkImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
