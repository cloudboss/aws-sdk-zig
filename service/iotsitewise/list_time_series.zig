...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTimeSeriesInput, options: Options) !ListTimeSeriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTimeSeriesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.alias_prefix) |v| {
        ...
        try query_buf.appendSlice(alloc, "aliasPrefix=");
    ...
    if (input.time_series_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "timeSeriesType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTimeSeriesOutput {
    var result: ListTimeSeriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTimeSeriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
