...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssetPropertyValueHistoryInput, options: Options) !GetAssetPropertyValueHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssetPropertyValueHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.end_date) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "endDate=");
        {
    ...
    if (input.qualities) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "qualities=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.start_date) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "startDate=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.time_ordering) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "timeOrdering=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssetPropertyValueHistoryOutput {
    var result: GetAssetPropertyValueHistoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssetPropertyValueHistoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
