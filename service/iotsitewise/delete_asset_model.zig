...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAssetModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAssetModelOutput {
    var result: DeleteAssetModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAssetModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
