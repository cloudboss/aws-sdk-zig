...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssetPropertyValueInput, options: Options) !GetAssetPropertyValueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssetPropertyValueInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssetPropertyValueOutput {
    var result: GetAssetPropertyValueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssetPropertyValueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
