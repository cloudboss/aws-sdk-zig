...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssetPropertiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssetPropertiesOutput {
    var result: ListAssetPropertiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetPropertiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
