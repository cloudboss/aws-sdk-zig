...
pub const ListAssetPropertiesInput = struct {
...
pub const ListAssetPropertiesOutput = struct {
    /// A list that summarizes the properties associated with the specified asset.
    asset_property_summaries: ?[]const AssetPropertySummary = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssetPropertiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssetPropertiesOutput {
    var result: ListAssetPropertiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetPropertiesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
