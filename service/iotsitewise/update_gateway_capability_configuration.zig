...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGatewayCapabilityConfigurationInput, options: Options) !UpdateGatewayCapabilityConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGatewayCapabilityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/capability");
    ...
    try body_buf.appendSlice(alloc, "\"capabilityConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.capability_configuration), input.capability_configuration, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"capabilityNamespace\":");
    try aws.json.writeValue(@TypeOf(input.capability_namespace), input.capability_namespace, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGatewayCapabilityConfigurationOutput {
    var result: UpdateGatewayCapabilityConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGatewayCapabilityConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
