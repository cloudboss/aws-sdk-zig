...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePortalInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePortalOutput {
    var result: DeletePortalOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePortalOutput, body, allocator);
    }
    _ = status;
    _ = headers;
