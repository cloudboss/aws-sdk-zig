...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeComputationModelInput, options: Options) !DescribeComputationModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeComputationModelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.computation_model_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "computationModelVersion=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeComputationModelOutput {
    var result: DescribeComputationModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeComputationModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
