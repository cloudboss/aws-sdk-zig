...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccessPolicyInput, options: Options) !CreateAccessPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccessPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"accessPolicyIdentity\":");
    try aws.json.writeValue(@TypeOf(input.access_policy_identity), input.access_policy_identity, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"accessPolicyPermission\":");
    try aws.json.writeValue(@TypeOf(input.access_policy_permission), input.access_policy_permission, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"accessPolicyResource\":");
    try aws.json.writeValue(@TypeOf(input.access_policy_resource), input.access_policy_resource, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccessPolicyOutput {
    var result: CreateAccessPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAccessPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
