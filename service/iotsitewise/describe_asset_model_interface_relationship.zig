...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAssetModelInterfaceRelationshipInput, options: Options) !DescribeAssetModelInterfaceRelationshipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssetModelInterfaceRelationshipInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/interface/");
    try path_buf.appendSlice(alloc, input.interface_asset_model_id);
    try path_buf.appendSlice(alloc, "/asset-model-interface-relationship");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssetModelInterfaceRelationshipOutput {
    var result: DescribeAssetModelInterfaceRelationshipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAssetModelInterfaceRelationshipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
