...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBulkImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.adaptive_ingestion) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"adaptiveIngestion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.delete_files_after_import) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"deleteFilesAfterImport\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"errorReportLocation\":");
    try aws.json.writeValue(@TypeOf(input.error_report_location), input.error_report_location, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"jobConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.job_configuration), input.job_configuration, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"jobRoleArn\":");
    try aws.json.writeValue(@TypeOf(input.job_role_arn), input.job_role_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBulkImportJobOutput {
    var result: CreateBulkImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBulkImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
