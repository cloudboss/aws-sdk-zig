...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssetRelationshipsInput, options: CallOptions) !ListAssetRelationshipsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssetRelationshipsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/assetRelationships");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssetRelationshipsOutput {
    var result: ListAssetRelationshipsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetRelationshipsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
