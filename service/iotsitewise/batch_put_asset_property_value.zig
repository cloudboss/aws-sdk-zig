...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchPutAssetPropertyValueInput, options: Options) !BatchPutAssetPropertyValueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchPutAssetPropertyValueInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.enable_partial_entry_processing) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"enablePartialEntryProcessing\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchPutAssetPropertyValueOutput {
    var result: BatchPutAssetPropertyValueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchPutAssetPropertyValueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
