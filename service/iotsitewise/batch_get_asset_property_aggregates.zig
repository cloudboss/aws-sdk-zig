...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetAssetPropertyAggregatesInput, options: Options) !BatchGetAssetPropertyAggregatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetAssetPropertyAggregatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetAssetPropertyAggregatesOutput {
    var result: BatchGetAssetPropertyAggregatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetAssetPropertyAggregatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
