...
fn serializeRequest(alloc: std.mem.Allocator, input: ListComputationModelDataBindingUsagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"dataBindingValueFilter\":");
    try aws.json.writeValue(@TypeOf(input.data_binding_value_filter), input.data_binding_value_filter, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListComputationModelDataBindingUsagesOutput {
    var result: ListComputationModelDataBindingUsagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListComputationModelDataBindingUsagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
