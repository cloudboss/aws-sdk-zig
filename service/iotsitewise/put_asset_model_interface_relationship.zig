...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAssetModelInterfaceRelationshipInput, options: Options) !PutAssetModelInterfaceRelationshipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAssetModelInterfaceRelationshipInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"propertyMappingConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.property_mapping_configuration), input.property_mapping_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAssetModelInterfaceRelationshipOutput {
    var result: PutAssetModelInterfaceRelationshipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutAssetModelInterfaceRelationshipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
