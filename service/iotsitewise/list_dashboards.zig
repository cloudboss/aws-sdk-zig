...
pub const ListDashboardsOutput = struct {
    ...
    dashboard_summaries: ?[]const DashboardSummary = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDashboardsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "projectId=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.project_id);
    query_has_prev = true;
