...
pub const DisassociateTimeSeriesFromAssetPropertyOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateTimeSeriesFromAssetPropertyInput, options: CallOptions) !DisassociateTimeSeriesFromAssetPropertyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateTimeSeriesFromAssetPropertyInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.asset_id);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "propertyId=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.property_id);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateTimeSeriesFromAssetPropertyOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
