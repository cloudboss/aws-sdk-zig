...
pub const DisassociateTimeSeriesFromAssetPropertyInput = struct {
...
const DisassociateTimeSeriesFromAssetPropertyOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateTimeSeriesFromAssetPropertyInput, options: Options) !DisassociateTimeSeriesFromAssetPropertyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateTimeSeriesFromAssetPropertyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/timeseries/disassociate";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "alias=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.alias);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "assetId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.asset_id);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "propertyId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.property_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateTimeSeriesFromAssetPropertyOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateTimeSeriesFromAssetPropertyOutput = .{};
