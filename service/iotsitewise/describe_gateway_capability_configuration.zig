...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGatewayCapabilityConfigurationInput, options: Options) !DescribeGatewayCapabilityConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGatewayCapabilityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.capability_namespace);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGatewayCapabilityConfigurationOutput {
    var result: DescribeGatewayCapabilityConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeGatewayCapabilityConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
