...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"gatewayName\":");
    try aws.json.writeValue(@TypeOf(input.gateway_name), input.gateway_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"gatewayPlatform\":");
    try aws.json.writeValue(@TypeOf(input.gateway_platform), input.gateway_platform, alloc, &body_buf);
    has_prev = true;
    if (input.gateway_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"gatewayVersion\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGatewayOutput {
    var result: CreateGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
