...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccessPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/access-policies/");
    try path_buf.appendSlice(alloc, input.access_policy_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccessPolicyOutput {
    var result: DescribeAccessPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccessPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
