...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteComputationModelInput, options: Options) !DeleteComputationModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteComputationModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteComputationModelOutput {
    var result: DeleteComputationModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteComputationModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
