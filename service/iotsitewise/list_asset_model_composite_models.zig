...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssetModelCompositeModelsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssetModelCompositeModelsOutput {
    var result: ListAssetModelCompositeModelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetModelCompositeModelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
