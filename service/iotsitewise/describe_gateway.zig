...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGatewayInput, options: Options) !DescribeGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/20200301/gateways/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGatewayOutput {
    var result: DescribeGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
