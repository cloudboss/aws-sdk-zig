...
const ListBulkImportJobsFilter = @import("list_bulk_import_jobs_filter.zig").ListBulkImportJobsFilter;
const JobSummary = @import("job_summary.zig").JobSummary;

pub const ListBulkImportJobsInput = struct {
    /// You can use a filter to select the bulk import jobs that you want to
    /// retrieve.
    filter: ?ListBulkImportJobsFilter = null,
...
pub const ListBulkImportJobsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBulkImportJobsInput, options: Options) !ListBulkImportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBulkImportJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBulkImportJobsOutput {
    var result: ListBulkImportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBulkImportJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
