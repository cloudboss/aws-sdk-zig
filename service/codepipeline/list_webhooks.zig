...
pub const ListWebhooksOutput = struct {
    ...
    /// is also
    /// returned and can be used in a subsequent ListWebhooks call to return the
    /// next set of
    ...
    /// ListWebhooks
    /// call.
    webhooks: ?[]const ListWebhookItem = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListWebhooksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodePipeline_20150709.ListWebhooks");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListWebhooksOutput {
    ...
    return aws.json.parseJsonObject(ListWebhooksOutput, body, allocator);
}
