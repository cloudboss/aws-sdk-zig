...
const CurrentRevision = @import("current_revision.zig").CurrentRevision;
const ExecutionDetails = @import("execution_details.zig").ExecutionDetails;

pub const PutThirdPartyJobSuccessResultInput = struct {
    ...
    /// a
    /// successful job provides to identify a partner action in progress. Future
    /// jobs use this
    /// token to identify the running instance of the action. It can be reused to
    /// return more
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutThirdPartyJobSuccessResultInput, options: Options) !PutThirdPartyJobSuccessResultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutThirdPartyJobSuccessResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.PutThirdPartyJobSuccessResult");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutThirdPartyJobSuccessResultOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
