...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutThirdPartyJobSuccessResultInput, options: CallOptions) !PutThirdPartyJobSuccessResultOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutThirdPartyJobSuccessResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodePipeline_20150709.PutThirdPartyJobSuccessResult");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutThirdPartyJobSuccessResultOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
