...
pub const DeleteCustomActionTypeInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCustomActionTypeInput, options: Options) !DeleteCustomActionTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCustomActionTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.DeleteCustomActionType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCustomActionTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
