...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCustomActionTypeInput, options: CallOptions) !DeleteCustomActionTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCustomActionTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodePipeline_20150709.DeleteCustomActionType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCustomActionTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
