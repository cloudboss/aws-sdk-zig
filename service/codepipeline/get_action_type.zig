...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetActionTypeInput, options: Options) !GetActionTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetActionTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.GetActionType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetActionTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetActionTypeOutput, body, alloc);
}
