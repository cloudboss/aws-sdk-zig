...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableStageTransitionInput, options: Options) !EnableStageTransitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableStageTransitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.EnableStageTransition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableStageTransitionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
