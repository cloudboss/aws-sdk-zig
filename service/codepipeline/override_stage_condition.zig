...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: OverrideStageConditionInput, options: Options) !OverrideStageConditionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: OverrideStageConditionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.OverrideStageCondition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !OverrideStageConditionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
