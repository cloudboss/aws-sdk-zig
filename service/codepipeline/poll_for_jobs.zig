...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PollForJobsInput, options: Options) !PollForJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PollForJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.PollForJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PollForJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PollForJobsOutput, body, alloc);
}
