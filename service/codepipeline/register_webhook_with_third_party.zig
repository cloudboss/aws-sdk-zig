...
pub const RegisterWebhookWithThirdPartyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterWebhookWithThirdPartyInput, options: Options) !RegisterWebhookWithThirdPartyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterWebhookWithThirdPartyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodePipeline_20150709.RegisterWebhookWithThirdParty");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterWebhookWithThirdPartyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
