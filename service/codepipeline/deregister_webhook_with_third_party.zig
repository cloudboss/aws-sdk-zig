...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterWebhookWithThirdPartyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.DeregisterWebhookWithThirdParty");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterWebhookWithThirdPartyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
