...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RetryStageExecutionInput, options: Options) !RetryStageExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RetryStageExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.RetryStageExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RetryStageExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RetryStageExecutionOutput, body, alloc);
}
