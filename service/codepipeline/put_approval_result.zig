...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutApprovalResultInput, options: Options) !PutApprovalResultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutApprovalResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.PutApprovalResult");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutApprovalResultOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutApprovalResultOutput, body, alloc);
}
