...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RollbackStageInput, options: Options) !RollbackStageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RollbackStageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.RollbackStage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RollbackStageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RollbackStageOutput, body, alloc);
}
