...
pub const PutJobFailureResultInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutJobFailureResultInput, options: CallOptions) !PutJobFailureResultOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutJobFailureResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodePipeline_20150709.PutJobFailureResult");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutJobFailureResultOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
