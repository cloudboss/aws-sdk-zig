...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListActionExecutionsInput, options: Options) !ListActionExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListActionExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.ListActionExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListActionExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListActionExecutionsOutput, body, alloc);
}
