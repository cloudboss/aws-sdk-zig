...
pub const GetJobDetailsInput = struct {
...
pub const GetJobDetailsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobDetailsInput, options: CallOptions) !GetJobDetailsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetJobDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodePipeline_20150709.GetJobDetails");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetJobDetailsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetJobDetailsOutput, body, allocator);
}
