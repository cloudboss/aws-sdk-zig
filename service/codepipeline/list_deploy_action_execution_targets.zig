...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeployActionExecutionTargetsInput, options: Options) !ListDeployActionExecutionTargetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDeployActionExecutionTargetsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("codepipeline", "CodePipeline", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.ListDeployActionExecutionTargets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDeployActionExecutionTargetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDeployActionExecutionTargetsOutput, body, alloc);
}
