...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPipelinesInput, options: Options) !ListPipelinesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPipelinesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.ListPipelines");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPipelinesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPipelinesOutput, body, alloc);
}
