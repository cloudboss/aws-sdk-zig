...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListActionTypesInput, options: Options) !ListActionTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListActionTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.ListActionTypes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListActionTypesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListActionTypesOutput, body, alloc);
}
