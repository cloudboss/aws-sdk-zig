...
pub const PutActionRevisionInput = struct {
...
pub const PutActionRevisionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutActionRevisionInput, options: Options) !PutActionRevisionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutActionRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.PutActionRevision");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutActionRevisionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutActionRevisionOutput, body, alloc);
}
