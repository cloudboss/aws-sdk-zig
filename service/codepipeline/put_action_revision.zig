...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutActionRevisionInput, options: CallOptions) !PutActionRevisionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutActionRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodePipeline_20150709.PutActionRevision");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutActionRevisionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutActionRevisionOutput, body, allocator);
}
