...
pub const PutJobSuccessResultInput = struct {
    /// A token generated by a job worker, such as a CodeDeploy deployment ID, that
    /// a
    /// successful job provides to identify a custom action in progress. Future jobs
    /// use this
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutJobSuccessResultInput, options: CallOptions) !PutJobSuccessResultOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutJobSuccessResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodePipeline_20150709.PutJobSuccessResult");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutJobSuccessResultOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
