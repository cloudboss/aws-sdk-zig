...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableStageTransitionInput, options: Options) !DisableStageTransitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableStageTransitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodePipeline_20150709.DisableStageTransition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableStageTransitionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
