...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.subscription_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelSubscriptionOutput {
    var result: CancelSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelSubscriptionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
