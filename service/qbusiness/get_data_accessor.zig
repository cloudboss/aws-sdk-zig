...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataAccessorInput, options: Options) !GetDataAccessorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataAccessorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.data_accessor_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataAccessorOutput {
    var result: GetDataAccessorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataAccessorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
