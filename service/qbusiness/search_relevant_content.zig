...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchRelevantContentInput, options: Options) !SearchRelevantContentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchRelevantContentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/relevant-content");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"contentSource\":");
    try aws.json.writeValue(@TypeOf(input.content_source), input.content_source, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchRelevantContentOutput {
    var result: SearchRelevantContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchRelevantContentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
