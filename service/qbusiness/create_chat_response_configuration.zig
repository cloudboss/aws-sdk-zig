...
pub const CreateChatResponseConfigurationInput = struct {
    ...
    /// This helps prevent the same configuration from being created multiple times
    /// if retries occur.
...
pub const CreateChatResponseConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChatResponseConfigurationInput, options: Options) !CreateChatResponseConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateChatResponseConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/chatresponseconfigurations");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"responseConfigurations\":");
    try aws.json.writeValue(@TypeOf(input.response_configurations), input.response_configurations, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateChatResponseConfigurationOutput {
    var result: CreateChatResponseConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChatResponseConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
