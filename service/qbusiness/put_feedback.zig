...
const MessageUsefulnessFeedback = @import("message_usefulness_feedback.zig").MessageUsefulnessFeedback;
...
pub const PutFeedbackInput = struct {
    ...
    message_copied_at: ?i64 = null,

    /// The identifier of the chat message that the feedback was given for.
    message_id: []const u8,

    /// The feedback usefulness value given by the user to the chat message.
    message_usefulness: ?MessageUsefulnessFeedback = null,
    ...
    pub const json_field_names = .{
        ...
        .message_copied_at = "messageCopiedAt",
        .message_id = "messageId",
        .message_usefulness = "messageUsefulness",
        .user_id = "userId",
    };
};

const PutFeedbackOutput = struct {};
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutFeedbackInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.message_copied_at) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"messageCopiedAt\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.message_usefulness) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"messageUsefulness\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
