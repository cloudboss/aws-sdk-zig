...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRetrieversInput, options: Options) !ListRetrieversOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRetrieversInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRetrieversOutput {
    var result: ListRetrieversOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRetrieversOutput, body, alloc);
    }
    _ = status;
    _ = headers;
