...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPluginInput, options: Options) !GetPluginOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPluginInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPluginOutput {
    var result: GetPluginOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPluginOutput, body, alloc);
    }
    _ = status;
    _ = headers;
