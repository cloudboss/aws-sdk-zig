...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChatControlsConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChatControlsConfigurationOutput {
    var result: GetChatControlsConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChatControlsConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
