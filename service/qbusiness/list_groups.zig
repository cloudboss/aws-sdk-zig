...
const ListGroupsInput = @import("list_groups_request.zig").ListGroupsRequest;

const ListGroupsOutput = @import("list_groups_response.zig").ListGroupsResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "updatedEarlierThan=");
    {
        const num_str = std.fmt.allocPrint(alloc, "{d}", .{input.updated_earlier_than}) catch "";
        try query_buf.appendSlice(alloc, num_str);
    }
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
