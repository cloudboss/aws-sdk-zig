...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataAccessorInput, options: Options) !CreateDataAccessorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataAccessorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dataaccessors");
    ...
    try body_buf.appendSlice(alloc, "\"actionConfigurations\":");
    try aws.json.writeValue(@TypeOf(input.action_configurations), input.action_configurations, alloc, &body_buf);
    has_prev = true;
    if (input.authentication_detail) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"authenticationDetail\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataAccessorOutput {
    var result: CreateDataAccessorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataAccessorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
