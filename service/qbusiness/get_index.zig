...
const GetIndexInput = @import("get_index_request.zig").GetIndexRequest;

const GetIndexOutput = @import("get_index_response.zig").GetIndexResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIndexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIndexOutput {
    var result: GetIndexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIndexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
