...
pub const ListConversationsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConversationsInput, options: Options) !ListConversationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConversationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConversationsOutput {
    var result: ListConversationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConversationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
