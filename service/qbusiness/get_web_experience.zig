...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWebExperienceInput, options: Options) !GetWebExperienceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWebExperienceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWebExperienceOutput {
    var result: GetWebExperienceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWebExperienceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
