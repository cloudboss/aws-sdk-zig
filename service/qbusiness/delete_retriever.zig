...
pub const DeleteRetrieverInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRetrieverInput, options: Options) !DeleteRetrieverOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRetrieverInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRetrieverOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteRetrieverOutput = .{};
