...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMediaInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.media_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMediaOutput {
    var result: GetMediaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMediaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
