...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRetrieverInput, options: Options) !GetRetrieverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRetrieverInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/retrievers/");
    try path_buf.appendSlice(alloc, input.retriever_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRetrieverOutput {
    var result: GetRetrieverOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRetrieverOutput, body, alloc);
    }
    _ = status;
    _ = headers;
