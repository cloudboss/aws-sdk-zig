...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataAccessorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataAccessorsOutput {
    var result: ListDataAccessorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataAccessorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
