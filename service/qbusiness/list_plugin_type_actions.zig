...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPluginTypeActionsInput, options: Options) !ListPluginTypeActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPluginTypeActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/pluginTypes/");
    try path_buf.appendSlice(alloc, input.plugin_type);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPluginTypeActionsOutput {
    var result: ListPluginTypeActionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPluginTypeActionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
