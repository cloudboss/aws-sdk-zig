...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSubscriptionInput, options: Options) !CreateSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSubscriptionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSubscriptionOutput {
    var result: CreateSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSubscriptionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
