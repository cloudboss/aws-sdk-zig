...
pub const ListPluginActionsInput = struct {
    ...
    /// If the number of plugin actions returned exceeds `maxResults`, Amazon Q
...
pub const ListPluginActionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPluginActionsInput, options: Options) !ListPluginActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPluginActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.plugin_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPluginActionsOutput {
    var result: ListPluginActionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPluginActionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
