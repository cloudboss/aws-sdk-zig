...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CheckDocumentAccessInput, options: Options) !CheckDocumentAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CheckDocumentAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/check-document-access");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CheckDocumentAccessOutput {
    var result: CheckDocumentAccessOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CheckDocumentAccessOutput, body, alloc);
    }
    _ = status;
    _ = headers;
