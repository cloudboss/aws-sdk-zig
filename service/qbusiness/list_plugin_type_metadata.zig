...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPluginTypeMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPluginTypeMetadataOutput {
    var result: ListPluginTypeMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPluginTypeMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
