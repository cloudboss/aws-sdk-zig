...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutGroupInput, options: Options) !PutGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.data_source_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"dataSourceId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"groupMembers\":");
    try aws.json.writeValue(@TypeOf(input.group_members), input.group_members, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutGroupOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
