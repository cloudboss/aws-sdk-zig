...
const WebExperience = @import("web_experience.zig").WebExperience;

pub const ListWebExperiencesInput = struct {
...
pub const ListWebExperiencesOutput = struct {
    ...
    /// experiences.
    web_experiences: ?[]const WebExperience = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .web_experiences = "webExperiences",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWebExperiencesInput, options: CallOptions) !ListWebExperiencesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListWebExperiencesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListWebExperiencesOutput {
    var result: ListWebExperiencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWebExperiencesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
