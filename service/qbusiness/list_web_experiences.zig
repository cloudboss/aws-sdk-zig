...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWebExperiencesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWebExperiencesOutput {
    var result: ListWebExperiencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWebExperiencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
