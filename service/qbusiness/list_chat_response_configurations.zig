...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChatResponseConfigurationsInput, options: Options) !ListChatResponseConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChatResponseConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChatResponseConfigurationsOutput {
    var result: ListChatResponseConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChatResponseConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
