...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetChatResponseConfigurationInput, options: Options) !GetChatResponseConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChatResponseConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.chat_response_configuration_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChatResponseConfigurationOutput {
    var result: GetChatResponseConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChatResponseConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
