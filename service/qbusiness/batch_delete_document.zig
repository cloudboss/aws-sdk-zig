...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteDocumentInput, options: Options) !BatchDeleteDocumentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteDocumentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/documents/delete");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteDocumentOutput {
    var result: BatchDeleteDocumentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteDocumentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
