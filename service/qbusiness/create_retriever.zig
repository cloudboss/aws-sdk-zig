...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRetrieverInput, options: Options) !CreateRetrieverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRetrieverInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRetrieverOutput {
    var result: CreateRetrieverOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRetrieverOutput, body, alloc);
    }
    _ = status;
    _ = headers;
