...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWebExperienceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWebExperienceOutput {
    var result: CreateWebExperienceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWebExperienceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
