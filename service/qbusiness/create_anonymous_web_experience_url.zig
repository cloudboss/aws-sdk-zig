...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAnonymousWebExperienceUrlInput, options: Options) !CreateAnonymousWebExperienceUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAnonymousWebExperienceUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/anonymous-url");
    ...
    if (input.session_duration_in_minutes) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"sessionDurationInMinutes\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAnonymousWebExperienceUrlOutput {
    var result: CreateAnonymousWebExperienceUrlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAnonymousWebExperienceUrlOutput, body, alloc);
    }
    _ = status;
    _ = headers;
