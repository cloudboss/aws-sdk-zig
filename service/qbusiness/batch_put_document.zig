...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchPutDocumentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.data_source_sync_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"dataSourceSyncId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"documents\":");
    try aws.json.writeValue(@TypeOf(input.documents), input.documents, alloc, &body_buf);
    has_prev = true;
    if (input.role_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchPutDocumentOutput {
    var result: BatchPutDocumentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchPutDocumentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
