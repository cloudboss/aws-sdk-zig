...
pub const UpdateRetrieverInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRetrieverInput, options: Options) !UpdateRetrieverOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRetrieverInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRetrieverOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateRetrieverOutput = .{};
