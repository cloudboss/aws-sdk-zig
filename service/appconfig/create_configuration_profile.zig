...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConfigurationProfileInput, options: Options) !CreateConfigurationProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConfigurationProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"LocationUri\":");
    try aws.json.writeValue(@TypeOf(input.location_uri), input.location_uri, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConfigurationProfileOutput {
    var result: CreateConfigurationProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConfigurationProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
