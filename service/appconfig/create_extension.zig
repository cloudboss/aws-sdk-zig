...
pub const CreateExtensionInput = struct {
    /// The actions defined in the extension.
    actions: []const aws.map.MapEntry([]const Action),
    ...
    /// version,
    /// specify the most recent current version number. For example, you create
    /// version 3, enter 2
    /// for this field.
    ...
    /// help you
    /// categorize resources in different ways, for example, by purpose, owner, or
    /// environment.
...
pub const CreateExtensionOutput = struct {
    /// The actions defined in the extension.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateExtensionInput, options: Options) !CreateExtensionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateExtensionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateExtensionOutput {
    var result: CreateExtensionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateExtensionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
