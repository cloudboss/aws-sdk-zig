...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHostedConfigurationVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.version_label) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "version_label=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHostedConfigurationVersionsOutput {
    var result: ListHostedConfigurationVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListHostedConfigurationVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
