...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.environment);
    try path_buf.appendSlice(alloc, "/configurations/");
    try path_buf.appendSlice(alloc, input.configuration);
    ...
    if (input.client_configuration_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "client_configuration_version=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "client_id=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.client_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfigurationOutput {
    ...
    if (headers.get("configuration-version")) |value| {
        result.configuration_version = try alloc.dupe(u8, value);
    }
