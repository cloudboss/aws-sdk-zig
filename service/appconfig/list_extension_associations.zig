...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExtensionAssociationsInput, options: Options) !ListExtensionAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExtensionAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.extension_identifier) |v| {
        ...
        try query_buf.appendSlice(alloc, "extension_identifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.extension_version_number) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "extension_version_number=");
        {
    ...
    if (input.resource_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "resource_identifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExtensionAssociationsOutput {
    var result: ListExtensionAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListExtensionAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
