...
const CreateDeploymentStrategyOutput = @import("deployment_strategy.zig").DeploymentStrategy;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDeploymentStrategyInput, options: Options) !CreateDeploymentStrategyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeploymentStrategyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"DeploymentDurationInMinutes\":");
    try aws.json.writeValue(@TypeOf(input.deployment_duration_in_minutes), input.deployment_duration_in_minutes, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.final_bake_time_in_minutes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FinalBakeTimeInMinutes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"GrowthFactor\":");
    try aws.json.writeValue(@TypeOf(input.growth_factor), input.growth_factor, alloc, &body_buf);
    has_prev = true;
    if (input.growth_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"GrowthType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.replicate_to) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ReplicateTo\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDeploymentStrategyOutput {
    var result: CreateDeploymentStrategyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDeploymentStrategyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
