...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExtensionInput, options: Options) !GetExtensionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExtensionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/extensions/");
    try path_buf.appendSlice(alloc, input.extension_identifier);
    ...
    if (input.version_number) |v| {
        ...
        try query_buf.appendSlice(alloc, "version_number=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExtensionOutput {
    var result: GetExtensionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetExtensionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
