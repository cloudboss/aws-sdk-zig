...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfigurationProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfigurationProfileOutput {
    var result: GetConfigurationProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfigurationProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
