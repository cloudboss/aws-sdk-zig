...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateExtensionAssociationInput, options: CallOptions) !CreateExtensionAssociationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateExtensionAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"ExtensionIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.extension_identifier), input.extension_identifier, allocator, &body_buf);
    has_prev = true;
    if (input.extension_version_number) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ExtensionVersionNumber\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateExtensionAssociationOutput {
    var result: CreateExtensionAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateExtensionAssociationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
