...
const UpdateDeploymentStrategyOutput = @import("deployment_strategy.zig").DeploymentStrategy;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDeploymentStrategyInput, options: Options) !UpdateDeploymentStrategyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDeploymentStrategyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/deploymentstrategies/");
    try path_buf.appendSlice(alloc, input.deployment_strategy_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDeploymentStrategyOutput {
    var result: UpdateDeploymentStrategyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDeploymentStrategyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
