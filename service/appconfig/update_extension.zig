...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateExtensionInput, options: Options) !UpdateExtensionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateExtensionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateExtensionOutput {
    var result: UpdateExtensionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateExtensionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
