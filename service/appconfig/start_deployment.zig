...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDeploymentInput, options: CallOptions) !StartDeploymentOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "appconfig");
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"ConfigurationProfileId\":");
    try aws.json.writeValue(@TypeOf(input.configuration_profile_id), input.configuration_profile_id, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"ConfigurationVersion\":");
    try aws.json.writeValue(@TypeOf(input.configuration_version), input.configuration_version, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"DeploymentStrategyId\":");
    try aws.json.writeValue(@TypeOf(input.deployment_strategy_id), input.deployment_strategy_id, allocator, &body_buf);
    has_prev = true;
    ...
    if (input.dynamic_extension_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"DynamicExtensionParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
