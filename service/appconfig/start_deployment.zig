...
pub const StartDeploymentInput = struct {
    /// The application ID.
...
pub const StartDeploymentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDeploymentInput, options: Options) !StartDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ConfigurationProfileId\":");
    try aws.json.writeValue(@TypeOf(input.configuration_profile_id), input.configuration_profile_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ConfigurationVersion\":");
    try aws.json.writeValue(@TypeOf(input.configuration_version), input.configuration_version, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DeploymentStrategyId\":");
    try aws.json.writeValue(@TypeOf(input.deployment_strategy_id), input.deployment_strategy_id, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.dynamic_extension_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DynamicExtensionParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDeploymentOutput {
    var result: StartDeploymentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDeploymentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
