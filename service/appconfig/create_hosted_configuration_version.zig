...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHostedConfigurationVersionInput, options: CallOptions) !CreateHostedConfigurationVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateHostedConfigurationVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.description) |v| {
        try request.headers.put(allocator, "Description", v);
    }
    if (input.latest_version_number) |v| {
        {
            const num_str = std.fmt.allocPrint(allocator, "{d}", .{v}) catch "";
            try request.headers.put(allocator, "Latest-Version-Number", num_str);
        }
    }
    if (input.version_label) |v| {
        try request.headers.put(allocator, "VersionLabel", v);
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateHostedConfigurationVersionOutput {
    ...
    if (headers.get("application-id")) |value| {
        result.application_id = try allocator.dupe(u8, value);
    }
    if (headers.get("configuration-profile-id")) |value| {
        result.configuration_profile_id = try allocator.dupe(u8, value);
    }
    ...
    if (headers.get("kmskeyarn")) |value| {
        result.kms_key_arn = try allocator.dupe(u8, value);
    }
    if (headers.get("versionlabel")) |value| {
        result.version_label = try allocator.dupe(u8, value);
    }
