...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExtensionAssociationInput, options: Options) !GetExtensionAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExtensionAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExtensionAssociationOutput {
    var result: GetExtensionAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetExtensionAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
