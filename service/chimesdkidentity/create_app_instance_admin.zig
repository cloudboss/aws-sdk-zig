...
pub const CreateAppInstanceAdminInput = struct {
...
pub const CreateAppInstanceAdminOutput = struct {
    ...
    app_instance_admin: ?Identity = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAppInstanceAdminInput, options: CallOptions) !CreateAppInstanceAdminOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAppInstanceAdminInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"AppInstanceAdminArn\":");
    try aws.json.writeValue(@TypeOf(input.app_instance_admin_arn), input.app_instance_admin_arn, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAppInstanceAdminOutput {
    var result: CreateAppInstanceAdminOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppInstanceAdminOutput, body, allocator);
    }
    _ = status;
    _ = headers;
