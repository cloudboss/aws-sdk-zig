...
pub const CreateAppInstanceBotInput = struct {
...
pub const CreateAppInstanceBotOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAppInstanceBotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAppInstanceBotOutput {
    var result: CreateAppInstanceBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppInstanceBotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
