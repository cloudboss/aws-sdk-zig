...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppInstanceInput, options: Options) !DescribeAppInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAppInstanceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAppInstanceOutput {
    var result: DescribeAppInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAppInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
