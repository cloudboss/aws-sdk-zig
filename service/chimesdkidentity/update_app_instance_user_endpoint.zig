...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppInstanceUserEndpointInput, options: Options) !UpdateAppInstanceUserEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAppInstanceUserEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAppInstanceUserEndpointOutput {
    var result: UpdateAppInstanceUserEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAppInstanceUserEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
