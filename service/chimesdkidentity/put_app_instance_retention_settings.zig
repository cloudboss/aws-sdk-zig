...
fn serializeRequest(allocator: std.mem.Allocator, input: PutAppInstanceRetentionSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"AppInstanceRetentionSettings\":");
    try aws.json.writeValue(@TypeOf(input.app_instance_retention_settings), input.app_instance_retention_settings, allocator, &body_buf);
    has_prev = true;
