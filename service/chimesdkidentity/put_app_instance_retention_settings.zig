...
pub const PutAppInstanceRetentionSettingsInput = struct {
...
pub const PutAppInstanceRetentionSettingsOutput = struct {
    /// The time in days to retain data. Data type: number.
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAppInstanceRetentionSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/retention-settings");
    ...
    try body_buf.appendSlice(alloc, "\"AppInstanceRetentionSettings\":");
    try aws.json.writeValue(@TypeOf(input.app_instance_retention_settings), input.app_instance_retention_settings, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAppInstanceRetentionSettingsOutput {
    var result: PutAppInstanceRetentionSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutAppInstanceRetentionSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
