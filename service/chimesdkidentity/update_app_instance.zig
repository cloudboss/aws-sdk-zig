...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppInstanceInput, options: CallOptions) !UpdateAppInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAppInstanceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAppInstanceOutput {
    var result: UpdateAppInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAppInstanceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
