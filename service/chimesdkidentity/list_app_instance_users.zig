...
const AppInstanceUserSummary = @import("app_instance_user_summary.zig").AppInstanceUserSummary;

pub const ListAppInstanceUsersInput = struct {
...
pub const ListAppInstanceUsersOutput = struct {
    /// The ARN of the `AppInstance`.
    app_instance_arn: ?[]const u8 = null,

    /// The information for each requested `AppInstanceUser`.
    app_instance_users: ?[]const AppInstanceUserSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppInstanceUsersInput, options: CallOptions) !ListAppInstanceUsersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAppInstanceUsersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAppInstanceUsersOutput {
    var result: ListAppInstanceUsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppInstanceUsersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
