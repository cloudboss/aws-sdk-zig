...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppInstanceUsersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppInstanceUsersOutput {
    var result: ListAppInstanceUsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppInstanceUsersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
