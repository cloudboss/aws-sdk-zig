...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppInstanceUserInput, options: Options) !UpdateAppInstanceUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAppInstanceUserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAppInstanceUserOutput {
    var result: UpdateAppInstanceUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAppInstanceUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
