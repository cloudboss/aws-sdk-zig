...
pub const CreateAppInstanceInput = struct {
...
pub const CreateAppInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAppInstanceInput, options: CallOptions) !CreateAppInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAppInstanceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAppInstanceOutput {
    var result: CreateAppInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppInstanceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
