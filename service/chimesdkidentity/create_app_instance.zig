...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAppInstanceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAppInstanceOutput {
    var result: CreateAppInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
