...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppInstanceAdminsInput, options: Options) !ListAppInstanceAdminsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppInstanceAdminsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppInstanceAdminsOutput {
    var result: ListAppInstanceAdminsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppInstanceAdminsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
