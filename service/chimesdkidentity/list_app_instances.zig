...
const AppInstanceSummary = @import("app_instance_summary.zig").AppInstanceSummary;

pub const ListAppInstancesInput = struct {
...
pub const ListAppInstancesOutput = struct {
    /// The information for each `AppInstance`.
    app_instances: ?[]const AppInstanceSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppInstancesInput, options: Options) !ListAppInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppInstancesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppInstancesOutput {
    var result: ListAppInstancesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppInstancesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
