...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAppInstanceRetentionSettingsInput, options: Options) !GetAppInstanceRetentionSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAppInstanceRetentionSettingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAppInstanceRetentionSettingsOutput {
    var result: GetAppInstanceRetentionSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAppInstanceRetentionSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
