...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppInstanceBotInput, options: CallOptions) !UpdateAppInstanceBotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAppInstanceBotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAppInstanceBotOutput {
    var result: UpdateAppInstanceBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAppInstanceBotOutput, body, allocator);
    }
    _ = status;
    _ = headers;
