...
pub const UpdateAppInstanceBotInput = struct {
...
pub const UpdateAppInstanceBotOutput = struct {
    /// The ARN of the `AppInstanceBot`.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppInstanceBotInput, options: Options) !UpdateAppInstanceBotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAppInstanceBotInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.metadata), input.metadata, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAppInstanceBotOutput {
    var result: UpdateAppInstanceBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAppInstanceBotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
