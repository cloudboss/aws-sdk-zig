...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAppInstanceUserExpirationSettingsInput, options: Options) !PutAppInstanceUserExpirationSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAppInstanceUserExpirationSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/expiration-settings");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAppInstanceUserExpirationSettingsOutput {
    var result: PutAppInstanceUserExpirationSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutAppInstanceUserExpirationSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
