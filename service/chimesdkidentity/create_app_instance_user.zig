...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAppInstanceUserInput, options: Options) !CreateAppInstanceUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAppInstanceUserInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"AppInstanceUserId\":");
    try aws.json.writeValue(@TypeOf(input.app_instance_user_id), input.app_instance_user_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.expiration_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ExpirationSettings\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAppInstanceUserOutput {
    var result: CreateAppInstanceUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppInstanceUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
