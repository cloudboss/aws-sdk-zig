...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppInstanceUserEndpointInput, options: Options) !DescribeAppInstanceUserEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAppInstanceUserEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAppInstanceUserEndpointOutput {
    var result: DescribeAppInstanceUserEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAppInstanceUserEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
