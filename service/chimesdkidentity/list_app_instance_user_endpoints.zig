...
const AppInstanceUserEndpointSummary = @import("app_instance_user_endpoint_summary.zig").AppInstanceUserEndpointSummary;

pub const ListAppInstanceUserEndpointsInput = struct {
...
pub const ListAppInstanceUserEndpointsOutput = struct {
    /// The information for each requested `AppInstanceUserEndpoint`.
    app_instance_user_endpoints: ?[]const AppInstanceUserEndpointSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppInstanceUserEndpointsInput, options: CallOptions) !ListAppInstanceUserEndpointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAppInstanceUserEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAppInstanceUserEndpointsOutput {
    var result: ListAppInstanceUserEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppInstanceUserEndpointsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
