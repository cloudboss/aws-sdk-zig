...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppInstanceUserEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppInstanceUserEndpointsOutput {
    var result: ListAppInstanceUserEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppInstanceUserEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
