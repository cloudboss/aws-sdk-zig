...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppInstanceBotsInput, options: Options) !ListAppInstanceBotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppInstanceBotsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppInstanceBotsOutput {
    var result: ListAppInstanceBotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppInstanceBotsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
