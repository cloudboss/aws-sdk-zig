...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOrderableDBInstanceOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeOrderableDBInstanceOptions&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOrderableDBInstanceOptionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.orderable_db_instance_options = try serde.deserializeOrderableDBInstanceOptionsList(&reader, alloc, "OrderableDBInstanceOption");
                } else {
