...
pub const StartDBClusterInput = struct {
    /// The identifier of the cluster to restart. Example:
    /// `docdb-2019-05-28-15-24-52`
    db_cluster_identifier: []const u8,
};

pub const StartDBClusterOutput = struct {
    db_cluster: ?DBCluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDBClusterInput, options: CallOptions) !StartDBClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartDBClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=StartDBCluster&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartDBClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StartDBClusterResult")) break;
            },
            else => {},
        }
    }

    var result: StartDBClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
