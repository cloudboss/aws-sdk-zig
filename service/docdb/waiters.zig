...
pub const DBInstanceDeletedWaiter = struct {
    client: *Client,
    ...
    fn poll(self: *Self) aws.waiter.AcceptorState {
        ...
        _ = self.client.describeDbInstances(arena.allocator(), self.params, .{ .diagnostic = &diagnostic }) catch |err| {
            if (err == error.ServiceError) {
                defer diagnostic.deinit();
                if (std.mem.eql(u8, diagnostic.code(), "DBInstanceNotFound")) {
                    return .success;
                }
            }
            return .retry;
