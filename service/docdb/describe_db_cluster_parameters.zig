...
pub const DescribeDBClusterParametersInput = struct {
...
pub const DescribeDBClusterParametersOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is specified, the response
    /// includes only records beyond the marker, up to the value specified by
    /// `MaxRecords`.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBClusterParametersInput, options: Options) !DescribeDBClusterParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBClusterParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBClusterParameters&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBClusterParametersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBClusterParametersResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBClusterParametersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.parameters = try serde.deserializeParametersList(&reader, alloc, "Parameter");
                } else {
