...
fn serializeRequest(alloc: std.mem.Allocator, input: FailoverDBClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=FailoverDBCluster&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !FailoverDBClusterOutput {
    _ = status;
    _ = headers;
