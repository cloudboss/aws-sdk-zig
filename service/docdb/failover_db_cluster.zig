...
pub const FailoverDBClusterInput = struct {
    ...
    target_db_instance_identifier: ?[]const u8 = null,
};

pub const FailoverDBClusterOutput = struct {
    db_cluster: ?DBCluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: FailoverDBClusterInput, options: CallOptions) !FailoverDBClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: FailoverDBClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=FailoverDBCluster&Version=2014-10-31");
    if (input.db_cluster_identifier) |v| {
        try body_buf.appendSlice(allocator, "&DBClusterIdentifier=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.target_db_instance_identifier) |v| {
        try body_buf.appendSlice(allocator, "&TargetDBInstanceIdentifier=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !FailoverDBClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "FailoverDBClusterResult")) break;
            },
            else => {},
        }
    }

    var result: FailoverDBClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
