...
fn serializeRequest(alloc: std.mem.Allocator, input: AddSourceIdentifierToSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AddSourceIdentifierToSubscription&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&SourceIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddSourceIdentifierToSubscriptionOutput {
    _ = status;
    _ = headers;
