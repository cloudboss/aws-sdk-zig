...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGlobalClustersInput, options: Options) !DescribeGlobalClustersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGlobalClustersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeGlobalClusters&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGlobalClustersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GlobalClusters")) {
                    result.global_clusters = try serde.deserializeGlobalClusterList(&reader, alloc, "GlobalClusterMember");
