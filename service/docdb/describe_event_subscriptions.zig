...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventSubscriptionsInput, options: CallOptions) !DescribeEventSubscriptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEventSubscriptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeEventSubscriptions&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEventSubscriptionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EventSubscriptionsList")) {
                    result.event_subscriptions_list = try serde.deserializeEventSubscriptionsList(allocator, &reader, "EventSubscription");
