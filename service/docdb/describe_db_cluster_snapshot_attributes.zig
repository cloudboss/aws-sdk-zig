...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBClusterSnapshotAttributesInput, options: Options) !DescribeDBClusterSnapshotAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBClusterSnapshotAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBClusterSnapshotAttributes&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBClusterSnapshotAttributesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterSnapshotAttributesResult")) {
                    result.db_cluster_snapshot_attributes_result = try serde.deserializeDBClusterSnapshotAttributesResult(&reader, alloc);
                } else {
