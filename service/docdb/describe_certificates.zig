...
pub const DescribeCertificatesInput = struct {
    ...
    /// parameter is omitted, a list of up to `MaxRecords` certificates is returned.
...
pub const DescribeCertificatesOutput = struct {
    ...
    /// greater than `MaxRecords`. If this parameter is specified, the marker
    /// specifies the next record in the list. Including the value of `Marker` in
    /// the next call to `DescribeCertificates` results in the next page of
    /// certificates.
    marker: ?[]const u8 = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCertificatesInput, options: Options) !DescribeCertificatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCertificatesInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("docdb", "DocDB", alloc);
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCertificates&Version=2014-10-31");
    if (input.certificate_identifier) |v| {
        try body_buf.appendSlice(alloc, "&CertificateIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCertificatesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeCertificatesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeCertificatesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Certificates")) {
                    result.certificates = try serde.deserializeCertificateList(&reader, alloc, "Certificate");
