...
pub const DescribeEngineDefaultClusterParametersInput = struct {
...
pub const DescribeEngineDefaultClusterParametersOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEngineDefaultClusterParametersInput, options: Options) !DescribeEngineDefaultClusterParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEngineDefaultClusterParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEngineDefaultClusterParameters&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&DBParameterGroupFamily=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_parameter_group_family);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEngineDefaultClusterParametersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeEngineDefaultClusterParametersResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeEngineDefaultClusterParametersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EngineDefaults")) {
                    result.engine_defaults = try serde.deserializeEngineDefaults(&reader, alloc);
                } else {
