...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEngineDefaultClusterParametersInput, options: CallOptions) !DescribeEngineDefaultClusterParametersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEngineDefaultClusterParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeEngineDefaultClusterParameters&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEngineDefaultClusterParametersOutput {
    _ = status;
    _ = headers;
