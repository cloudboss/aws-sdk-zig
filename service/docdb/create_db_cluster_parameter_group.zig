...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDBClusterParameterGroupInput, options: Options) !CreateDBClusterParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDBClusterParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateDBClusterParameterGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDBClusterParameterGroupOutput {
    _ = status;
    _ = headers;
