...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDataIntegrationAssociationInput, options: Options) !UpdateDataIntegrationAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataIntegrationAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.data_integration_association_identifier);
    ...
    try aws.json.writeValue(@TypeOf(input.execution_configuration), input.execution_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataIntegrationAssociationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
