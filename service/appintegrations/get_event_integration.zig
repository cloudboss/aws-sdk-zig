...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEventIntegrationInput, options: Options) !GetEventIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEventIntegrationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEventIntegrationOutput {
    var result: GetEventIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEventIntegrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
