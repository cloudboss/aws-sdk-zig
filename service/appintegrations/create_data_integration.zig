...
pub const CreateDataIntegrationInput = struct {
    ...
    schedule_config: ?ScheduleConfiguration = null,
...
pub const CreateDataIntegrationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataIntegrationInput, options: Options) !CreateDataIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.file_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FileConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"KmsKey\":");
    try aws.json.writeValue(@TypeOf(input.kms_key), input.kms_key, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.object_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ObjectConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.schedule_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ScheduleConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_uri) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceURI\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataIntegrationOutput {
    var result: CreateDataIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataIntegrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
