...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataIntegrationInput, options: Options) !GetDataIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataIntegrationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataIntegrationOutput {
    var result: GetDataIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataIntegrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
