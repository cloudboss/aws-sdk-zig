...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataIntegrationAssociationInput, options: Options) !CreateDataIntegrationAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataIntegrationAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dataIntegrations/");
    try path_buf.appendSlice(alloc, input.data_integration_identifier);
    ...
    if (input.client_association_metadata) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ClientAssociationMetadata\":");
    ...
    if (input.destination_uri) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DestinationURI\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.execution_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ExecutionConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataIntegrationAssociationOutput {
    var result: CreateDataIntegrationAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataIntegrationAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
