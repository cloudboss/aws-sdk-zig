...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePracticeRunConfigurationInput, options: Options) !UpdatePracticeRunConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePracticeRunConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allowed_windows) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"allowedWindows\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.blocked_dates) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"blockedDates\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.blocked_windows) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"blockedWindows\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.blocking_alarms) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"blockingAlarms\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.outcome_alarms) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"outcomeAlarms\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePracticeRunConfigurationOutput {
    var result: UpdatePracticeRunConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePracticeRunConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
