...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePracticeRunConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.outcome_alarms), input.outcome_alarms, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePracticeRunConfigurationOutput {
    var result: CreatePracticeRunConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePracticeRunConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
