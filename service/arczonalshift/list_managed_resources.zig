...
pub const ListManagedResourcesInput = struct {
...
pub const ListManagedResourcesOutput = struct {
    /// The items in the response list.
    items: ?[]const ManagedResourceSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedResourcesInput, options: Options) !ListManagedResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedResourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedResourcesOutput {
    var result: ListManagedResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
