...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartPracticeRunInput, options: Options) !StartPracticeRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartPracticeRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"awayFrom\":");
    try aws.json.writeValue(@TypeOf(input.away_from), input.away_from, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"comment\":");
    try aws.json.writeValue(@TypeOf(input.comment), input.comment, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartPracticeRunOutput {
    var result: StartPracticeRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartPracticeRunOutput, body, alloc);
    }
    _ = status;
    _ = headers;
