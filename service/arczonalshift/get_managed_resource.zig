...
fn serializeRequest(alloc: std.mem.Allocator, input: GetManagedResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/managedresources/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetManagedResourceOutput {
    var result: GetManagedResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetManagedResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
