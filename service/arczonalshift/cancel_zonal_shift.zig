...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelZonalShiftInput, options: Options) !CancelZonalShiftOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelZonalShiftInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelZonalShiftOutput {
    var result: CancelZonalShiftOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelZonalShiftOutput, body, alloc);
    }
    _ = status;
    _ = headers;
