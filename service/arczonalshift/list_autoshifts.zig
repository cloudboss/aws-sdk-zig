...
const AutoshiftSummary = @import("autoshift_summary.zig").AutoshiftSummary;

pub const ListAutoshiftsInput = struct {
    ...
    status: ?AutoshiftExecutionStatus = null,
...
pub const ListAutoshiftsOutput = struct {
    /// The items in the response list.
    items: ?[]const AutoshiftSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutoshiftsInput, options: Options) !ListAutoshiftsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutoshiftsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutoshiftsOutput {
    var result: ListAutoshiftsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutoshiftsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
