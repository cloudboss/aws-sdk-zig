...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateZonalAutoshiftConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"zonalAutoshiftStatus\":");
    try aws.json.writeValue(@TypeOf(input.zonal_autoshift_status), input.zonal_autoshift_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateZonalAutoshiftConfigurationOutput {
    var result: UpdateZonalAutoshiftConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateZonalAutoshiftConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
