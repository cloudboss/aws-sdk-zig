...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListZonalShiftsInput, options: Options) !ListZonalShiftsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListZonalShiftsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListZonalShiftsOutput {
    var result: ListZonalShiftsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListZonalShiftsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
