...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelPracticeRunInput, options: Options) !CancelPracticeRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelPracticeRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/practiceruns/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelPracticeRunOutput {
    var result: CancelPracticeRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelPracticeRunOutput, body, alloc);
    }
    _ = status;
    _ = headers;
