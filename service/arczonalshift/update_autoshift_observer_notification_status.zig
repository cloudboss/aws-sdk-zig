...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAutoshiftObserverNotificationStatusInput, options: Options) !UpdateAutoshiftObserverNotificationStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAutoshiftObserverNotificationStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAutoshiftObserverNotificationStatusOutput {
    var result: UpdateAutoshiftObserverNotificationStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAutoshiftObserverNotificationStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
