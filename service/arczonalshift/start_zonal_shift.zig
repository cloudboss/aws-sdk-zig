...
fn serializeRequest(alloc: std.mem.Allocator, input: StartZonalShiftInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.expires_in), input.expires_in, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartZonalShiftOutput {
    var result: StartZonalShiftOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartZonalShiftOutput, body, alloc);
    }
    _ = status;
    _ = headers;
