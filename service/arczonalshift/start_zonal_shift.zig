...
pub const StartZonalShiftInput = struct {
...
pub const StartZonalShiftOutput = struct {
    /// The Availability Zone (for example, `use1-az1`) that traffic is moved away
    /// from for a resource when you start a zonal shift. Until the zonal shift
    /// expires or you cancel it, traffic for the resource is instead moved to other
    /// Availability Zones in the Amazon Web Services Region.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartZonalShiftInput, options: CallOptions) !StartZonalShiftOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartZonalShiftInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"expiresIn\":");
    try aws.json.writeValue(@TypeOf(input.expires_in), input.expires_in, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartZonalShiftOutput {
    var result: StartZonalShiftOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartZonalShiftOutput, body, allocator);
    }
    _ = status;
    _ = headers;
