...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRelationshipsInput, options: Options) !ListRelationshipsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRelationshipsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralChannel.ListRelationships");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRelationshipsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRelationshipsOutput, body, alloc);
}
