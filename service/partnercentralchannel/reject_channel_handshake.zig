...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectChannelHandshakeInput, options: Options) !RejectChannelHandshakeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectChannelHandshakeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralChannel.RejectChannelHandshake");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectChannelHandshakeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RejectChannelHandshakeOutput, body, alloc);
}
