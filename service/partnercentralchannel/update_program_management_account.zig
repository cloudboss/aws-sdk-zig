...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProgramManagementAccountInput, options: Options) !UpdateProgramManagementAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProgramManagementAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralChannel.UpdateProgramManagementAccount");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProgramManagementAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateProgramManagementAccountOutput, body, alloc);
}
