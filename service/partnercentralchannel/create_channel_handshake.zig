...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChannelHandshakeInput, options: Options) !CreateChannelHandshakeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateChannelHandshakeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralChannel.CreateChannelHandshake");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateChannelHandshakeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateChannelHandshakeOutput, body, alloc);
}
