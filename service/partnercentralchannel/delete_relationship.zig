...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRelationshipInput, options: Options) !DeleteRelationshipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRelationshipInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralChannel.DeleteRelationship");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRelationshipOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
