...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRelationshipInput, options: Options) !GetRelationshipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRelationshipInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralChannel.GetRelationship");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRelationshipOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRelationshipOutput, body, alloc);
}
