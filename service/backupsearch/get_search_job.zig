...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSearchJobInput, options: Options) !GetSearchJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSearchJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSearchJobOutput {
    var result: GetSearchJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSearchJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
