...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSearchResultExportJobInput, options: Options) !StartSearchResultExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSearchResultExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ExportSpecification\":");
    try aws.json.writeValue(@TypeOf(input.export_specification), input.export_specification, alloc, &body_buf);
    has_prev = true;
    if (input.role_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SearchJobIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.search_job_identifier), input.search_job_identifier, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSearchResultExportJobOutput {
    var result: StartSearchResultExportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartSearchResultExportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
