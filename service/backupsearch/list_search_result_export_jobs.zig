...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSearchResultExportJobsInput, options: Options) !ListSearchResultExportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSearchResultExportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.search_job_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "SearchJobIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.status) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSearchResultExportJobsOutput {
    var result: ListSearchResultExportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSearchResultExportJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
