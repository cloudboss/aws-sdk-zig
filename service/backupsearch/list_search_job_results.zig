...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSearchJobResultsInput, options: Options) !ListSearchJobResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSearchJobResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.search_job_identifier);
    try path_buf.appendSlice(alloc, "/search-results");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSearchJobResultsOutput {
    var result: ListSearchJobResultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSearchJobResultsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
