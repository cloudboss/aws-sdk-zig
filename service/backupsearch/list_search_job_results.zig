...
pub const ListSearchJobResultsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSearchJobResultsInput, options: CallOptions) !ListSearchJobResultsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "backup-search");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSearchJobResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/search-results");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSearchJobResultsOutput {
    var result: ListSearchJobResultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSearchJobResultsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
