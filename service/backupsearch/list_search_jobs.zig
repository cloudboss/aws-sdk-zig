...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSearchJobsInput, options: Options) !ListSearchJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSearchJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSearchJobsOutput {
    var result: ListSearchJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSearchJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
