...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSearchJobInput, options: Options) !StartSearchJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSearchJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.item_filters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ItemFilters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSearchJobOutput {
    var result: StartSearchJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartSearchJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
