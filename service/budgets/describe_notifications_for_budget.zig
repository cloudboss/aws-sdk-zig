...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNotificationsForBudgetInput, options: Options) !DescribeNotificationsForBudgetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNotificationsForBudgetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBudgetServiceGateway.DescribeNotificationsForBudget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNotificationsForBudgetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeNotificationsForBudgetOutput, body, alloc);
}
