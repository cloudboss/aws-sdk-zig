...
pub const DescribeSubscribersForNotificationInput = struct {
...
pub const DescribeSubscribersForNotificationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSubscribersForNotificationInput, options: CallOptions) !DescribeSubscribersForNotificationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSubscribersForNotificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSBudgetServiceGateway.DescribeSubscribersForNotification");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSubscribersForNotificationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSubscribersForNotificationOutput, body, allocator);
}
