...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBudgetsInput, options: Options) !DescribeBudgetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBudgetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBudgetServiceGateway.DescribeBudgets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBudgetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeBudgetsOutput, body, alloc);
}
