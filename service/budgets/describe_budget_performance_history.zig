...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBudgetPerformanceHistoryInput, options: Options) !DescribeBudgetPerformanceHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBudgetPerformanceHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBudgetServiceGateway.DescribeBudgetPerformanceHistory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBudgetPerformanceHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeBudgetPerformanceHistoryOutput, body, alloc);
}
