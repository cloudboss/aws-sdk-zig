...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBudgetInput, options: Options) !DescribeBudgetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBudgetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBudgetServiceGateway.DescribeBudget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBudgetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeBudgetOutput, body, alloc);
}
