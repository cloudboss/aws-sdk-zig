...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBudgetActionInput, options: Options) !UpdateBudgetActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBudgetActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBudgetServiceGateway.UpdateBudgetAction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBudgetActionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateBudgetActionOutput, body, alloc);
}
