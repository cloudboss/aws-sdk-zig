...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateNotificationInput, options: Options) !CreateNotificationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNotificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBudgetServiceGateway.CreateNotification");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateNotificationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
