...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBudgetActionInput, options: Options) !CreateBudgetActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBudgetActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSBudgetServiceGateway.CreateBudgetAction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBudgetActionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateBudgetActionOutput, body, alloc);
}
