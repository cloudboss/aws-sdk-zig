...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCasesInput, options: Options) !DescribeCasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSupport_20130415.DescribeCases");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCasesOutput, body, alloc);
}
