...
pub const DescribeTrustedAdvisorCheckRefreshStatusesInput = struct {
...
pub const DescribeTrustedAdvisorCheckRefreshStatusesOutput = struct {
    /// The refresh status of the specified Trusted Advisor checks.
    statuses: ?[]const TrustedAdvisorCheckRefreshStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustedAdvisorCheckRefreshStatusesInput, options: CallOptions) !DescribeTrustedAdvisorCheckRefreshStatusesOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "support");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTrustedAdvisorCheckRefreshStatusesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSupport_20130415.DescribeTrustedAdvisorCheckRefreshStatuses");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTrustedAdvisorCheckRefreshStatusesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeTrustedAdvisorCheckRefreshStatusesOutput, body, allocator);
}
