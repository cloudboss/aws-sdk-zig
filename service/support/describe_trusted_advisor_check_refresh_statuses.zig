...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrustedAdvisorCheckRefreshStatusesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSupport_20130415.DescribeTrustedAdvisorCheckRefreshStatuses");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrustedAdvisorCheckRefreshStatusesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTrustedAdvisorCheckRefreshStatusesOutput, body, alloc);
}
