...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustedAdvisorCheckSummariesInput, options: Options) !DescribeTrustedAdvisorCheckSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrustedAdvisorCheckSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSupport_20130415.DescribeTrustedAdvisorCheckSummaries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrustedAdvisorCheckSummariesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeTrustedAdvisorCheckSummariesOutput, body, alloc);
}
