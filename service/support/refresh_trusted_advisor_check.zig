...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RefreshTrustedAdvisorCheckInput, options: Options) !RefreshTrustedAdvisorCheckOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RefreshTrustedAdvisorCheckInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSupport_20130415.RefreshTrustedAdvisorCheck");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RefreshTrustedAdvisorCheckOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RefreshTrustedAdvisorCheckOutput, body, alloc);
}
