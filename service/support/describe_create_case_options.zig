...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCreateCaseOptionsInput, options: Options) !DescribeCreateCaseOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCreateCaseOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSupport_20130415.DescribeCreateCaseOptions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCreateCaseOptionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCreateCaseOptionsOutput, body, alloc);
}
