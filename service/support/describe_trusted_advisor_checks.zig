...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrustedAdvisorChecksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSupport_20130415.DescribeTrustedAdvisorChecks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrustedAdvisorChecksOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeTrustedAdvisorChecksOutput, body, alloc);
}
