...
const TrustedAdvisorCheckDescription = @import("trusted_advisor_check_description.zig").TrustedAdvisorCheckDescription;

pub const DescribeTrustedAdvisorChecksInput = struct {
...
pub const DescribeTrustedAdvisorChecksOutput = struct {
    /// Information about all available Trusted Advisor checks.
    checks: ?[]const TrustedAdvisorCheckDescription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrustedAdvisorChecksInput, options: CallOptions) !DescribeTrustedAdvisorChecksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTrustedAdvisorChecksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSupport_20130415.DescribeTrustedAdvisorChecks");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTrustedAdvisorChecksOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeTrustedAdvisorChecksOutput, body, allocator);
}
