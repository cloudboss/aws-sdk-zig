...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResolveCaseInput, options: Options) !ResolveCaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResolveCaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSupport_20130415.ResolveCase");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResolveCaseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResolveCaseOutput, body, alloc);
}
