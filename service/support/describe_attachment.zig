...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAttachmentInput, options: Options) !DescribeAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSupport_20130415.DescribeAttachment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAttachmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAttachmentOutput, body, alloc);
}
