...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSeverityLevelsInput, options: Options) !DescribeSeverityLevelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSeverityLevelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSupport_20130415.DescribeSeverityLevels");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSeverityLevelsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSeverityLevelsOutput, body, alloc);
}
