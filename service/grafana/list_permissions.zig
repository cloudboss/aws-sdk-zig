...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPermissionsInput, options: Options) !ListPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.user_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "userType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPermissionsOutput {
    var result: ListPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
