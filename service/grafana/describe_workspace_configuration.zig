...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspaceConfigurationInput, options: Options) !DescribeWorkspaceConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkspaceConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkspaceConfigurationOutput {
    var result: DescribeWorkspaceConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeWorkspaceConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
