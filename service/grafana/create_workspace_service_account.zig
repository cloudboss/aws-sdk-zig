...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkspaceServiceAccountInput, options: Options) !CreateWorkspaceServiceAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkspaceServiceAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"grafanaRole\":");
    try aws.json.writeValue(@TypeOf(input.grafana_role), input.grafana_role, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkspaceServiceAccountOutput {
    var result: CreateWorkspaceServiceAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWorkspaceServiceAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
