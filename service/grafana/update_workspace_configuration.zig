...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkspaceConfigurationInput, options: Options) !UpdateWorkspaceConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspaceConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.grafana_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"grafanaVersion\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspaceConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
