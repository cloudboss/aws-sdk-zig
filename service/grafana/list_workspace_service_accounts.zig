...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkspaceServiceAccountsInput, options: Options) !ListWorkspaceServiceAccountsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkspaceServiceAccountsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkspaceServiceAccountsOutput {
    var result: ListWorkspaceServiceAccountsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkspaceServiceAccountsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
