...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkspaceServiceAccountTokenInput, options: Options) !CreateWorkspaceServiceAccountTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkspaceServiceAccountTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"secondsToLive\":");
    try aws.json.writeValue(@TypeOf(input.seconds_to_live), input.seconds_to_live, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkspaceServiceAccountTokenOutput {
    var result: CreateWorkspaceServiceAccountTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWorkspaceServiceAccountTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
