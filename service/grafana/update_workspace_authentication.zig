...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspaceAuthenticationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.saml_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"samlConfiguration\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspaceAuthenticationOutput {
    var result: UpdateWorkspaceAuthenticationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateWorkspaceAuthenticationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
