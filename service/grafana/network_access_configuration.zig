...
/// can pass in an empty array of strings for either parameter if you do not
/// want to allow any
/// of that type.
///
...
pub const NetworkAccessConfiguration = struct {
    ...
    /// addresses.
    /// The IP addresses specified are allowed to access your workspace. If the list
    /// is not
    /// included in the configuration (passed an empty array) then no IP addresses
    /// are
    /// allowed to access the workspace. You create a prefix list using the Amazon
    /// VPC
    /// console.
    ///
    ...
    /// managed
    /// prefix
    /// lists](https://docs.aws.amazon.com/vpc/latest/userguide/managed-prefix-lists.html)in the *Amazon Virtual Private Cloud User
    /// Guide*.
