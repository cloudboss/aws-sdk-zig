...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateLicenseInput, options: Options) !AssociateLicenseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateLicenseInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.grafana_token) |v| {
        try request.headers.put(alloc, "Grafana-Token", v);
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateLicenseOutput {
    var result: AssociateLicenseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateLicenseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
