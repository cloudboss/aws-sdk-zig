...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTokenWithIAMInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "aws_iam=t");
    query_has_prev = true;
    ...
    if (input.assertion) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"assertion\":");
    ...
    if (input.code_verifier) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"codeVerifier\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"grantType\":");
    try aws.json.writeValue(@TypeOf(input.grant_type), input.grant_type, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.refresh_token) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"refreshToken\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.requested_token_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"requestedTokenType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.scope) |v| {
    ...
    if (input.subject_token) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"subjectToken\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.subject_token_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"subjectTokenType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTokenWithIAMOutput {
    var result: CreateTokenWithIAMOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTokenWithIAMOutput, body, alloc);
    }
    _ = status;
    _ = headers;
