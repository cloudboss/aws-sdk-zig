...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterClientInput, options: Options) !RegisterClientOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterClientInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"clientName\":");
    try aws.json.writeValue(@TypeOf(input.client_name), input.client_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"clientType\":");
    try aws.json.writeValue(@TypeOf(input.client_type), input.client_type, alloc, &body_buf);
    has_prev = true;
    if (input.entitled_application_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"entitledApplicationArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.grant_types) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"grantTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.issuer_url) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"issuerUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.redirect_uris) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"redirectUris\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.scopes) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterClientOutput {
    var result: RegisterClientOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterClientOutput, body, alloc);
    }
    _ = status;
    _ = headers;
