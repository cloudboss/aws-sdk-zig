...
const AgentSummary = @import("agent_summary.zig").AgentSummary;
...
pub const ListAgentsOutput = struct {
    ...
    agent_summaries: ?[]const AgentSummary = null,
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAgentsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
