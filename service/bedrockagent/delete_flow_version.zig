...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteFlowVersionInput, options: Options) !DeleteFlowVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteFlowVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.flow_version);
    ...
    if (input.skip_resource_in_use_check) |v| {
        ...
        try query_buf.appendSlice(alloc, "skipResourceInUseCheck=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteFlowVersionOutput {
    var result: DeleteFlowVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteFlowVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
