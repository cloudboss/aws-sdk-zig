...
pub const GetIngestionJobInput = struct {
    /// The unique identifier of the data source for the data ingestion job you want
    /// to get information on.
    data_source_id: []const u8,

    /// The unique identifier of the data ingestion job you want to get information
    /// on.
...
pub const GetIngestionJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIngestionJobInput, options: Options) !GetIngestionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIngestionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ingestionjobs/");
    try path_buf.appendSlice(alloc, input.ingestion_job_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIngestionJobOutput {
    var result: GetIngestionJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIngestionJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
