...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIngestionJobInput, options: CallOptions) !GetIngestionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIngestionJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIngestionJobOutput {
    var result: GetIngestionJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIngestionJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
