...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAgentCollaboratorInput, options: Options) !GetAgentCollaboratorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAgentCollaboratorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAgentCollaboratorOutput {
    var result: GetAgentCollaboratorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAgentCollaboratorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
