...
const AgentKnowledgeBaseSummary = @import("agent_knowledge_base_summary.zig").AgentKnowledgeBaseSummary;

pub const ListAgentKnowledgeBasesInput = struct {
...
pub const ListAgentKnowledgeBasesOutput = struct {
    /// A list of objects, each of which contains information about a knowledge base
    /// associated with the agent.
    agent_knowledge_base_summaries: ?[]const AgentKnowledgeBaseSummary = null,
    ...
    pub const json_field_names = .{
        .agent_knowledge_base_summaries = "agentKnowledgeBaseSummaries",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAgentKnowledgeBasesInput, options: CallOptions) !ListAgentKnowledgeBasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAgentKnowledgeBasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAgentKnowledgeBasesOutput {
    var result: ListAgentKnowledgeBasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentKnowledgeBasesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
