...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAgentKnowledgeBasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAgentKnowledgeBasesOutput {
    var result: ListAgentKnowledgeBasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentKnowledgeBasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
