...
const FlowVersionSummary = @import("flow_version_summary.zig").FlowVersionSummary;

pub const ListFlowVersionsInput = struct {
...
pub const ListFlowVersionsOutput = struct {
    /// A list, each member of which contains information about a flow.
    flow_version_summaries: ?[]const FlowVersionSummary = null,
    ...
    pub const json_field_names = .{
        .flow_version_summaries = "flowVersionSummaries",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFlowVersionsInput, options: Options) !ListFlowVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFlowVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFlowVersionsOutput {
    var result: ListFlowVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFlowVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
