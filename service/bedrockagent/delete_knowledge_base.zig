...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteKnowledgeBaseInput, options: Options) !DeleteKnowledgeBaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteKnowledgeBaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteKnowledgeBaseOutput {
    var result: DeleteKnowledgeBaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteKnowledgeBaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
