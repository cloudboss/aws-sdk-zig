...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PrepareFlowInput, options: Options) !PrepareFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PrepareFlowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PrepareFlowOutput {
    var result: PrepareFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PrepareFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
