...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAgentCollaboratorInput, options: Options) !UpdateAgentCollaboratorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAgentCollaboratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.collaborator_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAgentCollaboratorOutput {
    var result: UpdateAgentCollaboratorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAgentCollaboratorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
