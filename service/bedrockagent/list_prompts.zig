...
pub const ListPromptsInput = struct {
    ...
    /// account.
    prompt_identifier: ?[]const u8 = null,
...
pub const ListPromptsOutput = struct {
    ...
    /// Prompt management.
    prompt_summaries: ?[]const PromptSummary = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .prompt_summaries = "promptSummaries",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPromptsInput, options: Options) !ListPromptsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPromptsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.prompt_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "promptIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPromptsOutput {
    var result: ListPromptsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPromptsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
