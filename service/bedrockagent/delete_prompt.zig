...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePromptInput, options: Options) !DeletePromptOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePromptInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.prompt_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "promptVersion=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePromptOutput {
    var result: DeletePromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePromptOutput, body, alloc);
    }
    _ = status;
    _ = headers;
