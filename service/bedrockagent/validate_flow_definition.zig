...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ValidateFlowDefinitionInput, options: Options) !ValidateFlowDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ValidateFlowDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ValidateFlowDefinitionOutput {
    var result: ValidateFlowDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ValidateFlowDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
