...
const FlowAliasSummary = @import("flow_alias_summary.zig").FlowAliasSummary;

pub const ListFlowAliasesInput = struct {
...
pub const ListFlowAliasesOutput = struct {
    /// A list, each member of which contains information about an alias.
    flow_alias_summaries: ?[]const FlowAliasSummary = null,
    ...
    pub const json_field_names = .{
        .flow_alias_summaries = "flowAliasSummaries",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFlowAliasesInput, options: Options) !ListFlowAliasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFlowAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFlowAliasesOutput {
    var result: ListFlowAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFlowAliasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
