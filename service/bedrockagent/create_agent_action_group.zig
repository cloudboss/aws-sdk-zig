...
pub const CreateAgentActionGroupInput = struct {
...
pub const CreateAgentActionGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAgentActionGroupInput, options: Options) !CreateAgentActionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAgentActionGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAgentActionGroupOutput {
    var result: CreateAgentActionGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAgentActionGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
