...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFlowAliasInput, options: Options) !CreateFlowAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFlowAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFlowAliasOutput {
    var result: CreateFlowAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFlowAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
