...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAgentActionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/actiongroups/");
    try path_buf.appendSlice(alloc, input.action_group_id);
    ...
    if (input.action_group_executor) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"actionGroupExecutor\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"actionGroupName\":");
    try aws.json.writeValue(@TypeOf(input.action_group_name), input.action_group_name, alloc, &body_buf);
    has_prev = true;
    if (input.action_group_state) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"actionGroupState\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.api_schema) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"apiSchema\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.function_schema) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"functionSchema\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.parent_action_group_signature) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"parentActionGroupSignature\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.parent_action_group_signature_params) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"parentActionGroupSignatureParams\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAgentActionGroupOutput {
    var result: UpdateAgentActionGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAgentActionGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
