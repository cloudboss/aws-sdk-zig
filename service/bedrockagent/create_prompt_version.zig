...
pub const CreatePromptVersionInput = struct {
...
pub const CreatePromptVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePromptVersionInput, options: Options) !CreatePromptVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePromptVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.prompt_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePromptVersionOutput {
    var result: CreatePromptVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePromptVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
