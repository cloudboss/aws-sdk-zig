...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAgentVersionInput, options: Options) !DeleteAgentVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAgentVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAgentVersionOutput {
    var result: DeleteAgentVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAgentVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
