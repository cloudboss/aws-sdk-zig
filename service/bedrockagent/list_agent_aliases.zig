...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAgentAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAgentAliasesOutput {
    var result: ListAgentAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentAliasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
