...
const AgentAliasSummary = @import("agent_alias_summary.zig").AgentAliasSummary;

pub const ListAgentAliasesInput = struct {
...
pub const ListAgentAliasesOutput = struct {
    /// A list of objects, each of which contains information about an alias of the
    /// agent.
    agent_alias_summaries: ?[]const AgentAliasSummary = null,
    ...
    pub const json_field_names = .{
        .agent_alias_summaries = "agentAliasSummaries",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAgentAliasesInput, options: CallOptions) !ListAgentAliasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAgentAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAgentAliasesOutput {
    var result: ListAgentAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentAliasesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
