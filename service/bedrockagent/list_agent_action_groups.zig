...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAgentActionGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAgentActionGroupsOutput {
    var result: ListAgentActionGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentActionGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
