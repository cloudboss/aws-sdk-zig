...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFlowAliasInput, options: Options) !UpdateFlowAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFlowAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.alias_identifier);
    ...
    if (input.concurrency_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"concurrencyConfiguration\":");
    ...
    try aws.json.writeValue(@TypeOf(input.routing_configuration), input.routing_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFlowAliasOutput {
    var result: UpdateFlowAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFlowAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
