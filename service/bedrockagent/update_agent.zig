...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAgentInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.agent_resource_role_arn), input.agent_resource_role_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAgentOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAgentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
