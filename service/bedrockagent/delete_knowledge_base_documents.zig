...
pub const DeleteKnowledgeBaseDocumentsInput = struct {
...
pub const DeleteKnowledgeBaseDocumentsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteKnowledgeBaseDocumentsInput, options: Options) !DeleteKnowledgeBaseDocumentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteKnowledgeBaseDocumentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/documents/deleteDocuments");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"documentIdentifiers\":");
    try aws.json.writeValue(@TypeOf(input.document_identifiers), input.document_identifiers, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteKnowledgeBaseDocumentsOutput {
    var result: DeleteKnowledgeBaseDocumentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteKnowledgeBaseDocumentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
