...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAgentKnowledgeBaseInput, options: CallOptions) !UpdateAgentKnowledgeBaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAgentKnowledgeBaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAgentKnowledgeBaseOutput {
    var result: UpdateAgentKnowledgeBaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAgentKnowledgeBaseOutput, body, allocator);
    }
    _ = status;
    _ = headers;
