...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIngestionJobsInput, options: Options) !ListIngestionJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIngestionJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIngestionJobsOutput {
    var result: ListIngestionJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIngestionJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
