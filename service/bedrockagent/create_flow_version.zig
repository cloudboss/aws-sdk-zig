...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFlowVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFlowVersionOutput {
    var result: CreateFlowVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFlowVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
