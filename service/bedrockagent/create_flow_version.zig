...
pub const CreateFlowVersionInput = struct {
...
pub const CreateFlowVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFlowVersionInput, options: CallOptions) !CreateFlowVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateFlowVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateFlowVersionOutput {
    var result: CreateFlowVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFlowVersionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
