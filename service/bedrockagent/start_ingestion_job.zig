...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartIngestionJobInput, options: CallOptions) !StartIngestionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartIngestionJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartIngestionJobOutput {
    var result: StartIngestionJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartIngestionJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
