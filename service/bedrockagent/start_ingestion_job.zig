...
pub const StartIngestionJobInput = struct {
...
pub const StartIngestionJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartIngestionJobInput, options: Options) !StartIngestionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartIngestionJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartIngestionJobOutput {
    var result: StartIngestionJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartIngestionJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
