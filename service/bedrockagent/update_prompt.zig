...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePromptInput, options: Options) !UpdatePromptOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePromptInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.default_variant) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"defaultVariant\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.variants) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"variants\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePromptOutput {
    var result: UpdatePromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePromptOutput, body, alloc);
    }
    _ = status;
    _ = headers;
