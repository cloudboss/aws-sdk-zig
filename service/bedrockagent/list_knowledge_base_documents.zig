...
pub const ListKnowledgeBaseDocumentsInput = struct {
...
pub const ListKnowledgeBaseDocumentsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListKnowledgeBaseDocumentsInput, options: CallOptions) !ListKnowledgeBaseDocumentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListKnowledgeBaseDocumentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListKnowledgeBaseDocumentsOutput {
    var result: ListKnowledgeBaseDocumentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListKnowledgeBaseDocumentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
