...
fn serializeRequest(alloc: std.mem.Allocator, input: ListKnowledgeBaseDocumentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListKnowledgeBaseDocumentsOutput {
    var result: ListKnowledgeBaseDocumentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListKnowledgeBaseDocumentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
