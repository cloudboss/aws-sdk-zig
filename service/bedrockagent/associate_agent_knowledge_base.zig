...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateAgentKnowledgeBaseInput, options: Options) !AssociateAgentKnowledgeBaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateAgentKnowledgeBaseInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"knowledgeBaseId\":");
    try aws.json.writeValue(@TypeOf(input.knowledge_base_id), input.knowledge_base_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateAgentKnowledgeBaseOutput {
    var result: AssociateAgentKnowledgeBaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateAgentKnowledgeBaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
