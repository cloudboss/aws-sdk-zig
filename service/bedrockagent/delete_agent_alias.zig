...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAgentAliasInput, options: Options) !DeleteAgentAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAgentAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAgentAliasOutput {
    var result: DeleteAgentAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAgentAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
