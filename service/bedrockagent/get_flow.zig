...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFlowInput, options: Options) !GetFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFlowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFlowOutput {
    var result: GetFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
