...
pub const DeleteAgentInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAgentInput, options: Options) !DeleteAgentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAgentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAgentOutput {
    var result: DeleteAgentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAgentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
