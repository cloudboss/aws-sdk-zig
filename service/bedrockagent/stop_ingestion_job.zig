...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopIngestionJobInput, options: Options) !StopIngestionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopIngestionJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopIngestionJobOutput {
    var result: StopIngestionJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopIngestionJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
