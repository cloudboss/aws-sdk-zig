...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PrepareAgentInput, options: Options) !PrepareAgentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PrepareAgentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PrepareAgentOutput {
    var result: PrepareAgentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PrepareAgentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
