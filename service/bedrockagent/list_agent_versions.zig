...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAgentVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAgentVersionsOutput {
    var result: ListAgentVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
