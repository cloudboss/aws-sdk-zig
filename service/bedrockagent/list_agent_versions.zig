...
const AgentVersionSummary = @import("agent_version_summary.zig").AgentVersionSummary;

pub const ListAgentVersionsInput = struct {
...
pub const ListAgentVersionsOutput = struct {
    /// A list of objects, each of which contains information about a version of the
    /// agent.
    agent_version_summaries: ?[]const AgentVersionSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAgentVersionsInput, options: CallOptions) !ListAgentVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAgentVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAgentVersionsOutput {
    var result: ListAgentVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAgentVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
