...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAgentAliasInput, options: Options) !UpdateAgentAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAgentAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"agentAliasName\":");
    try aws.json.writeValue(@TypeOf(input.agent_alias_name), input.agent_alias_name, alloc, &body_buf);
    has_prev = true;
    if (input.alias_invocation_state) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"aliasInvocationState\":");
    ...
    if (input.routing_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"routingConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAgentAliasOutput {
    var result: UpdateAgentAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAgentAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
