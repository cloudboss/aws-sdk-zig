...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFlowVersionInput, options: Options) !GetFlowVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFlowVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFlowVersionOutput {
    var result: GetFlowVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
