...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAgentKnowledgeBaseInput, options: Options) !GetAgentKnowledgeBaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAgentKnowledgeBaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAgentKnowledgeBaseOutput {
    var result: GetAgentKnowledgeBaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAgentKnowledgeBaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
