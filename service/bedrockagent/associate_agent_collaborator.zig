...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateAgentCollaboratorInput, options: CallOptions) !AssociateAgentCollaboratorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateAgentCollaboratorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateAgentCollaboratorOutput {
    var result: AssociateAgentCollaboratorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateAgentCollaboratorOutput, body, allocator);
    }
    _ = status;
    _ = headers;
