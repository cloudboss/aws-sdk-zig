...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPromptInput, options: Options) !GetPromptOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPromptInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPromptOutput {
    var result: GetPromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPromptOutput, body, alloc);
    }
    _ = status;
    _ = headers;
