...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: IngestKnowledgeBaseDocumentsInput, options: Options) !IngestKnowledgeBaseDocumentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: IngestKnowledgeBaseDocumentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !IngestKnowledgeBaseDocumentsOutput {
    var result: IngestKnowledgeBaseDocumentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(IngestKnowledgeBaseDocumentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
