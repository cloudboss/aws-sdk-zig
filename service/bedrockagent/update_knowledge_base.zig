...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKnowledgeBaseInput, options: Options) !UpdateKnowledgeBaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKnowledgeBaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKnowledgeBaseOutput {
    var result: UpdateKnowledgeBaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKnowledgeBaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
