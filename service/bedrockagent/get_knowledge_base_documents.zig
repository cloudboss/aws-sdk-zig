...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetKnowledgeBaseDocumentsInput, options: Options) !GetKnowledgeBaseDocumentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKnowledgeBaseDocumentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/documents/getDocuments");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKnowledgeBaseDocumentsOutput {
    var result: GetKnowledgeBaseDocumentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetKnowledgeBaseDocumentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
