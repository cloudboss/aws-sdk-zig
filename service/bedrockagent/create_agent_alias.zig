...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAgentAliasInput, options: Options) !CreateAgentAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAgentAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAgentAliasOutput {
    var result: CreateAgentAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAgentAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
