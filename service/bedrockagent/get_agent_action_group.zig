...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAgentActionGroupInput, options: Options) !GetAgentActionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAgentActionGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAgentActionGroupOutput {
    var result: GetAgentActionGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAgentActionGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
