...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAgentAliasInput, options: Options) !GetAgentAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAgentAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAgentAliasOutput {
    var result: GetAgentAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAgentAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
