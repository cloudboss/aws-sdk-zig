...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFlowAliasInput, options: Options) !GetFlowAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFlowAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFlowAliasOutput {
    var result: GetFlowAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
