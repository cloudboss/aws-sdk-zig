...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMetricWidgetImageInput, options: Options) !GetMetricWidgetImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetricWidgetImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetMetricWidgetImage&Version=2010-08-01");
    try body_buf.appendSlice(alloc, "&MetricWidget=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.metric_widget);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetricWidgetImageOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "MetricWidgetImage")) {
                    result.metric_widget_image = try alloc.dupe(u8, try reader.readElementText());
                } else {
