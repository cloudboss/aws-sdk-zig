...
pub const GetMetricWidgetImageInput = struct {
    ...
    /// most use cases, and all actions using an Amazon Web Services SDK, you should
    /// use
...
pub const GetMetricWidgetImageOutput = struct {
    /// The image of the graph, in the output format specified. The output is
    /// base64-encoded.
    metric_widget_image: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMetricWidgetImageInput, options: CallOptions) !GetMetricWidgetImageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMetricWidgetImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetMetricWidgetImage&Version=2010-08-01");
    try body_buf.appendSlice(allocator, "&MetricWidget=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.metric_widget);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMetricWidgetImageOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetMetricWidgetImageResult")) break;
            },
            else => {},
        }
    }

    var result: GetMetricWidgetImageOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "MetricWidgetImage")) {
                    result.metric_widget_image = try allocator.dupe(u8, try reader.readElementText());
                } else {
