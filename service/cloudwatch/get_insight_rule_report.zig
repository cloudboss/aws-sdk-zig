...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInsightRuleReportInput, options: Options) !GetInsightRuleReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInsightRuleReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetInsightRuleReport&Version=2010-08-01");
    ...
    if (input.max_contributor_count) |v| {
        try body_buf.appendSlice(alloc, "&MaxContributorCount=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInsightRuleReportOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AggregateValue")) {
                    ...
                    result.aggregation_statistic = try alloc.dupe(u8, try reader.readElementText());
                    ...
                    result.contributors = try serde.deserializeInsightRuleContributors(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "KeyLabels")) {
                    result.key_labels = try serde.deserializeInsightRuleContributorKeyLabels(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "MetricDatapoints")) {
                    result.metric_datapoints = try serde.deserializeInsightRuleMetricDatapoints(&reader, alloc, "member");
                } else {
