...
pub const PutDashboardInput = struct {
    ...
    /// this call
    /// modifies that dashboard, replacing its current contents. Otherwise, a new
    /// dashboard is
    /// created. The maximum length is 255, and valid characters are A-Z, a-z, 0-9,
    /// "-", and
...
pub const PutDashboardOutput = struct {
    ...
    /// enough for
    /// the dashboard to be created or modified, but some elements of the dashboard
    /// might not
    /// render.
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDashboardInput, options: Options) !PutDashboardOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDashboardInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PutDashboard&Version=2010-08-01");
    try body_buf.appendSlice(alloc, "&DashboardBody=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.dashboard_body);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDashboardOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PutDashboardResult")) break;
            },
            else => {},
        }
    }

    var result: PutDashboardOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DashboardValidationMessages")) {
                    result.dashboard_validation_messages = try serde.deserializeDashboardValidationMessages(&reader, alloc, "member");
                } else {
