...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDashboardInput, options: CallOptions) !PutDashboardOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutDashboardInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=PutDashboard&Version=2010-08-01");
    try body_buf.appendSlice(allocator, "&DashboardBody=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutDashboardOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DashboardValidationMessages")) {
                    result.dashboard_validation_messages = try serde.deserializeDashboardValidationMessages(allocator, &reader, "member");
                } else {
