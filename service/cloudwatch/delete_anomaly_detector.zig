...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAnomalyDetectorInput, options: Options) !DeleteAnomalyDetectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAnomalyDetectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteAnomalyDetector&Version=2010-08-01");
            ...
            for (list_d0, 0..) |item, idx| {
                ...
                if (item.metric_stat) |sv_1| {
                    ...
                    }
                    {
                        ...
                        try body_buf.appendSlice(alloc, field_prefix);
                        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{sv_1.period}) catch "");
                    }
                    {
                        var prefix_buf: [256]u8 = undefined;
                        ...
                        try body_buf.appendSlice(alloc, field_prefix);
                        try aws.url.appendUrlEncoded(alloc, &body_buf, sv_1.stat);
                    }
                    {
                        var prefix_buf: [256]u8 = undefined;
    ...
    if (input.single_metric_anomaly_detector) |v| {
        if (v.account_id) |sv| {
            try body_buf.appendSlice(alloc, "&SingleMetricAnomalyDetector.AccountId=");
        ...
        if (v.metric_name) |sv| {
            try body_buf.appendSlice(alloc, "&SingleMetricAnomalyDetector.MetricName=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
        }
        if (v.namespace) |sv| {
            try body_buf.appendSlice(alloc, "&SingleMetricAnomalyDetector.Namespace=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
        }
        if (v.stat) |sv| {
            try body_buf.appendSlice(alloc, "&SingleMetricAnomalyDetector.Stat=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
        }
    }
    if (input.stat) |v| {
        try body_buf.appendSlice(alloc, "&Stat=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAnomalyDetectorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: DeleteAnomalyDetectorOutput = .{};
