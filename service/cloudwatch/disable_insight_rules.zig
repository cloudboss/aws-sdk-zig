...
pub const DisableInsightRulesInput = struct {
...
pub const DisableInsightRulesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableInsightRulesInput, options: Options) !DisableInsightRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableInsightRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisableInsightRules&Version=2010-08-01");
    for (input.rule_names, 0..) |item, idx| {
        const n = idx + 1;
        var prefix_buf: [256]u8 = undefined;
        const field_prefix = std.fmt.bufPrint(&prefix_buf, "&RuleNames.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableInsightRulesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DisableInsightRulesResult")) break;
            },
            else => {},
        }
    }

    var result: DisableInsightRulesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Failures")) {
                    result.failures = try serde.deserializeBatchFailures(&reader, alloc, "member");
                } else {
