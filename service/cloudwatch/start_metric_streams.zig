...
pub const StartMetricStreamsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMetricStreamsInput, options: Options) !StartMetricStreamsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartMetricStreamsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=StartMetricStreams&Version=2010-08-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartMetricStreamsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: StartMetricStreamsOutput = .{};
