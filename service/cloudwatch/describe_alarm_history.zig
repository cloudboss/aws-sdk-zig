...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAlarmHistoryInput, options: Options) !DescribeAlarmHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAlarmHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAlarmHistory&Version=2010-08-01");
    if (input.alarm_contributor_id) |v| {
        try body_buf.appendSlice(alloc, "&AlarmContributorId=");
    ...
    if (input.history_item_type) |v| {
        try body_buf.appendSlice(alloc, "&HistoryItemType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAlarmHistoryOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AlarmHistoryItems")) {
                    result.alarm_history_items = try serde.deserializeAlarmHistoryItems(&reader, alloc, "member");
