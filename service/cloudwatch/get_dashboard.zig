...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDashboardInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetDashboard&Version=2010-08-01");
    try body_buf.appendSlice(alloc, "&DashboardName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.dashboard_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDashboardOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetDashboardResult")) break;
            },
            else => {},
        }
    }

    var result: GetDashboardOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DashboardArn")) {
                    result.dashboard_arn = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "DashboardBody")) {
                    result.dashboard_body = try alloc.dupe(u8, try reader.readElementText());
