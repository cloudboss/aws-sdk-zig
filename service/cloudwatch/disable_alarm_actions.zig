...
pub const DisableAlarmActionsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableAlarmActionsInput, options: CallOptions) !DisableAlarmActionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableAlarmActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DisableAlarmActions&Version=2010-08-01");
    for (input.alarm_names, 0..) |item, idx| {
        const n = idx + 1;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableAlarmActionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DisableAlarmActionsOutput = .{};
