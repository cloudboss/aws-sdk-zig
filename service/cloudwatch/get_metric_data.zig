...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetricDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetMetricData&Version=2010-08-01");
    ...
    if (input.label_options) |v| {
        if (v.timezone) |sv| {
            try body_buf.appendSlice(alloc, "&LabelOptions.Timezone=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
        }
    }
    if (input.max_datapoints) |v| {
        try body_buf.appendSlice(alloc, "&MaxDatapoints=");
    ...
    if (input.scan_by) |v| {
        try body_buf.appendSlice(alloc, "&ScanBy=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetricDataOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Messages")) {
                    result.messages = try serde.deserializeMetricDataResultMessages(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "MetricDataResults")) {
                    result.metric_data_results = try serde.deserializeMetricDataResults(&reader, alloc, "member");
