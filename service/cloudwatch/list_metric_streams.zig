...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMetricStreamsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListMetricStreams&Version=2010-08-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMetricStreamsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Entries")) {
                    result.entries = try serde.deserializeMetricStreamEntries(&reader, alloc, "member");
