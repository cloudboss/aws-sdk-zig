...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedInsightRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListManagedInsightRules&Version=2010-08-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedInsightRulesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ManagedRules")) {
                    result.managed_rules = try serde.deserializeManagedRuleDescriptions(&reader, alloc, "member");
