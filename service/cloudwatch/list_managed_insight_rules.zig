...
pub const ListManagedInsightRulesInput = struct {
...
pub const ListManagedInsightRulesOutput = struct {
    /// The managed rules that are available for the specified Amazon Web Services
    /// resource.
    managed_rules: ?[]const ManagedRuleDescription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedInsightRulesInput, options: CallOptions) !ListManagedInsightRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListManagedInsightRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListManagedInsightRules&Version=2010-08-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListManagedInsightRulesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListManagedInsightRulesResult")) break;
            },
            else => {},
        }
    }

    var result: ListManagedInsightRulesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ManagedRules")) {
                    result.managed_rules = try serde.deserializeManagedRuleDescriptions(allocator, &reader, "member");
