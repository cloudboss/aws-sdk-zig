...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAlarmsInput, options: Options) !DescribeAlarmsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAlarmsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAlarms&Version=2010-08-01");
    if (input.action_prefix) |v| {
        try body_buf.appendSlice(alloc, "&ActionPrefix=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.alarm_name_prefix) |v| {
        try body_buf.appendSlice(alloc, "&AlarmNamePrefix=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    ...
    if (input.children_of_alarm_name) |v| {
        try body_buf.appendSlice(alloc, "&ChildrenOfAlarmName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    ...
    if (input.parents_of_alarm_name) |v| {
        try body_buf.appendSlice(alloc, "&ParentsOfAlarmName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAlarmsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CompositeAlarms")) {
                    result.composite_alarms = try serde.deserializeCompositeAlarms(&reader, alloc, "member");
