...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAnomalyDetectorsInput, options: CallOptions) !DescribeAnomalyDetectorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAnomalyDetectorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAnomalyDetectorsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AnomalyDetectors")) {
                    result.anomaly_detectors = try serde.deserializeAnomalyDetectors(allocator, &reader, "member");
