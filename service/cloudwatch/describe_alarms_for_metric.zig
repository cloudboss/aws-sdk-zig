...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAlarmsForMetricInput, options: Options) !DescribeAlarmsForMetricOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAlarmsForMetricInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAlarmsForMetric&Version=2010-08-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAlarmsForMetricOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "MetricAlarms")) {
                    result.metric_alarms = try serde.deserializeMetricAlarms(&reader, alloc, "member");
                } else {
