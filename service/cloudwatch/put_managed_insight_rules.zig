...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutManagedInsightRulesInput, options: Options) !PutManagedInsightRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutManagedInsightRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PutManagedInsightRules&Version=2010-08-01");
    for (input.managed_rules, 0..) |item, idx| {
        const n = idx + 1;
        {
            ...
            try aws.url.appendUrlEncoded(alloc, &body_buf, item.resource_arn);
        }
        if (item.tags) |lst_1| {
            for (lst_1, 0..) |item_1, idx_1| {
                const n_1 = idx_1 + 1;
                {
                    ...
                    try body_buf.appendSlice(alloc, field_prefix);
                    try aws.url.appendUrlEncoded(alloc, &body_buf, item_1.key);
                }
                {
                    var prefix_buf: [256]u8 = undefined;
        ...
        }
        {
            ...
            try body_buf.appendSlice(alloc, field_prefix);
            try aws.url.appendUrlEncoded(alloc, &body_buf, item.template_name);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutManagedInsightRulesOutput {
    _ = status;
    _ = headers;
