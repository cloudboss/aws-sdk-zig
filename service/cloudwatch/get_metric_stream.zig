...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMetricStreamInput, options: Options) !GetMetricStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetricStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetMetricStream&Version=2010-08-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetricStreamOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Arn")) {
                    ...
                    result.exclude_filters = try serde.deserializeMetricStreamFilters(&reader, alloc, "member");
                    ...
                    result.include_filters = try serde.deserializeMetricStreamFilters(&reader, alloc, "member");
                    ...
                    result.statistics_configurations = try serde.deserializeMetricStreamStatisticsConfigurations(&reader, alloc, "member");
                } else {
