...
/// time series. For more information, see [Metric Math Syntax and
...
pub const MetricDataQuery = struct {
    ...
    /// expression to
    /// be performed on the returned data. For more information about Metrics
    /// Insights queries,
    /// see [Metrics Insights query components and
    /// syntax](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/cloudwatch-metrics-insights-querylanguage) in the *Amazon
    ...
    /// serve as a
    /// variable in the mathematical expression. The valid characters are letters,
    /// numbers, and
    ...
    /// useful if
    /// this is an expression, so that you know what the value represents. If the
    /// metric or
    /// expression is shown in a CloudWatch dashboard widget, the label is shown. If
    /// Label is
    ...
    /// this
    /// parameter only if this object is retrieving a metric and not performing a
    /// math
    ...
    /// call just to
    /// do math expressions and do not also need the raw data returned, you can
    /// specify
