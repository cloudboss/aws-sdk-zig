...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutMetricDataInput, options: CallOptions) !PutMetricDataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutMetricDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=PutMetricData&Version=2010-08-01");
    ...
    if (input.metric_data) |list| {
        for (list, 0..) |item, idx| {
            ...
            }
            {
                ...
                try body_buf.appendSlice(allocator, field_prefix);
                try aws.url.appendUrlEncoded(allocator, &body_buf, item.metric_name);
            }
    ...
    if (input.strict_entity_validation) |v| {
        try body_buf.appendSlice(allocator, "&StrictEntityValidation=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutMetricDataOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
