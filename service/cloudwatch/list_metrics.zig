...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListMetrics&Version=2010-08-01");
    ...
    if (input.owning_account) |v| {
        try body_buf.appendSlice(alloc, "&OwningAccount=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.recently_active) |v| {
        try body_buf.appendSlice(alloc, "&RecentlyActive=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMetricsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Metrics")) {
                    result.metrics = try serde.deserializeMetrics(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "NextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "OwningAccounts")) {
                    result.owning_accounts = try serde.deserializeOwningAccounts(&reader, alloc, "member");
                } else {
