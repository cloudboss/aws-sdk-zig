...
pub const EnableInsightRulesInput = struct {
...
pub const EnableInsightRulesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableInsightRulesInput, options: Options) !EnableInsightRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableInsightRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableInsightRules&Version=2010-08-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableInsightRulesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EnableInsightRulesResult")) break;
            },
            else => {},
        }
    }

    var result: EnableInsightRulesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
