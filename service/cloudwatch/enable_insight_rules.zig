...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableInsightRulesInput, options: CallOptions) !EnableInsightRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableInsightRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=EnableInsightRules&Version=2010-08-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableInsightRulesOutput {
    _ = status;
    _ = headers;
