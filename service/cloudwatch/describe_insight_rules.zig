...
pub const DescribeInsightRulesInput = struct {
...
pub const DescribeInsightRulesOutput = struct {
    /// The rules returned by the operation.
    insight_rules: ?[]const InsightRule = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInsightRulesInput, options: CallOptions) !DescribeInsightRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInsightRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInsightRules&Version=2010-08-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInsightRulesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeInsightRulesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeInsightRulesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InsightRules")) {
                    result.insight_rules = try serde.deserializeInsightRules(allocator, &reader, "member");
