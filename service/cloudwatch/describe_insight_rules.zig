...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInsightRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInsightRules&Version=2010-08-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInsightRulesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InsightRules")) {
                    result.insight_rules = try serde.deserializeInsightRules(&reader, alloc, "member");
