...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDashboardsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListDashboards&Version=2010-08-01");
    if (input.dashboard_name_prefix) |v| {
        try body_buf.appendSlice(alloc, "&DashboardNamePrefix=");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DashboardEntries")) {
                    result.dashboard_entries = try serde.deserializeDashboardEntries(&reader, alloc, "member");
