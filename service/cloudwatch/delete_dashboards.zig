...
pub const DeleteDashboardsInput = struct {
    /// The dashboards to be deleted. This parameter is required.
    dashboard_names: []const []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDashboardsInput, options: Options) !DeleteDashboardsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDashboardsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteDashboards&Version=2010-08-01");
    for (input.dashboard_names, 0..) |item, idx| {
        const n = idx + 1;
        var prefix_buf: [256]u8 = undefined;
        const field_prefix = std.fmt.bufPrint(&prefix_buf, "&DashboardNames.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDashboardsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteDashboardsOutput = .{};
