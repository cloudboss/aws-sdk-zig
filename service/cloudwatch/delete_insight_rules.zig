...
pub const DeleteInsightRulesInput = struct {
...
pub const DeleteInsightRulesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteInsightRulesInput, options: Options) !DeleteInsightRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteInsightRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteInsightRules&Version=2010-08-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteInsightRulesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteInsightRulesResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteInsightRulesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
