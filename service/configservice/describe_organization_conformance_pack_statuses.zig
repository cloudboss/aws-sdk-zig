...
const OrganizationConformancePackStatus = @import("organization_conformance_pack_status.zig").OrganizationConformancePackStatus;

pub const DescribeOrganizationConformancePackStatusesInput = struct {
...
pub const DescribeOrganizationConformancePackStatusesOutput = struct {
    /// The nextToken string returned on a previous page that you use to get the
    /// next page of results in a paginated response.
    next_token: ?[]const u8 = null,

    /// A list of `OrganizationConformancePackStatus` objects.
    organization_conformance_pack_statuses: ?[]const OrganizationConformancePackStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrganizationConformancePackStatusesInput, options: CallOptions) !DescribeOrganizationConformancePackStatusesOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "config");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeOrganizationConformancePackStatusesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DescribeOrganizationConformancePackStatuses");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeOrganizationConformancePackStatusesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeOrganizationConformancePackStatusesOutput, body, allocator);
}
