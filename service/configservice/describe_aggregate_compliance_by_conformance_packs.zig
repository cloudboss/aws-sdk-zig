...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAggregateComplianceByConformancePacksInput, options: Options) !DescribeAggregateComplianceByConformancePacksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAggregateComplianceByConformancePacksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeAggregateComplianceByConformancePacks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAggregateComplianceByConformancePacksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAggregateComplianceByConformancePacksOutput, body, alloc);
}
