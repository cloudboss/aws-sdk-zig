...
fn serializeRequest(alloc: std.mem.Allocator, input: PutConfigRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutConfigRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutConfigRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
