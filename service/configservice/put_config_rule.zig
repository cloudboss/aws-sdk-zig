...
pub const PutConfigRuleInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutConfigRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.PutConfigRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutConfigRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
