...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAggregateComplianceDetailsByConfigRuleInput, options: Options) !GetAggregateComplianceDetailsByConfigRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAggregateComplianceDetailsByConfigRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetAggregateComplianceDetailsByConfigRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAggregateComplianceDetailsByConfigRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAggregateComplianceDetailsByConfigRuleOutput, body, alloc);
}
