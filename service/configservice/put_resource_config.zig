...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutResourceConfigInput, options: Options) !PutResourceConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutResourceConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutResourceConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutResourceConfigOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
