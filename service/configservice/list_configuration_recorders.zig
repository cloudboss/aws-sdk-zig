...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConfigurationRecordersInput, options: Options) !ListConfigurationRecordersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConfigurationRecordersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.ListConfigurationRecorders");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConfigurationRecordersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListConfigurationRecordersOutput, body, alloc);
}
