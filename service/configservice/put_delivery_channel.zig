...
pub const PutDeliveryChannelInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDeliveryChannelInput, options: CallOptions) !PutDeliveryChannelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutDeliveryChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.PutDeliveryChannel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutDeliveryChannelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
