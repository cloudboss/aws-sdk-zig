...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDeliveryChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutDeliveryChannel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDeliveryChannelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
