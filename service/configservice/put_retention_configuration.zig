...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRetentionConfigurationInput, options: Options) !PutRetentionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRetentionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutRetentionConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRetentionConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutRetentionConfigurationOutput, body, alloc);
}
