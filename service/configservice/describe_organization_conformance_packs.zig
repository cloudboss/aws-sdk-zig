...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrganizationConformancePacksInput, options: Options) !DescribeOrganizationConformancePacksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOrganizationConformancePacksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeOrganizationConformancePacks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOrganizationConformancePacksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeOrganizationConformancePacksOutput, body, alloc);
}
