...
pub const BatchGetAggregateResourceConfigInput = struct {
    /// The name of the configuration aggregator.
    configuration_aggregator_name: []const u8,

    /// A list of aggregate ResourceIdentifiers objects.
    resource_identifiers: []const AggregateResourceIdentifier,
...
pub const BatchGetAggregateResourceConfigOutput = struct {
    ...
    unprocessed_resource_identifiers: ?[]const AggregateResourceIdentifier = null,

    pub const json_field_names = .{
        .base_configuration_items = "BaseConfigurationItems",
        .unprocessed_resource_identifiers = "UnprocessedResourceIdentifiers",
    };
};
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchGetAggregateResourceConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.BatchGetAggregateResourceConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchGetAggregateResourceConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetAggregateResourceConfigOutput, body, allocator);
}
