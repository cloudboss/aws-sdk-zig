...
const ConfigurationRecorder = @import("configuration_recorder.zig").ConfigurationRecorder;

pub const DescribeConfigurationRecordersInput = struct {
...
pub const DescribeConfigurationRecordersOutput = struct {
    /// A list that contains the descriptions of the specified
    /// configuration recorders.
    configuration_recorders: ?[]const ConfigurationRecorder = null,

    pub const json_field_names = .{
        .configuration_recorders = "ConfigurationRecorders",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConfigurationRecordersInput, options: CallOptions) !DescribeConfigurationRecordersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeConfigurationRecordersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DescribeConfigurationRecorders");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeConfigurationRecordersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConfigurationRecordersOutput, body, allocator);
}
