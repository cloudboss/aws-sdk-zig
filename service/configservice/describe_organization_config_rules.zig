...
const OrganizationConfigRule = @import("organization_config_rule.zig").OrganizationConfigRule;
...
pub const DescribeOrganizationConfigRulesInput = struct {
    ...
    /// not specify any names, Config returns details for all your organization
    /// Config rules.
...
pub const DescribeOrganizationConfigRulesOutput = struct {
    /// The `nextToken` string returned on a previous page that you use to get the
    /// next page of results in a paginated response.
    next_token: ?[]const u8 = null,

    /// Returns a list of `OrganizationConfigRule` objects.
    organization_config_rules: ?[]const OrganizationConfigRule = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrganizationConfigRulesInput, options: CallOptions) !DescribeOrganizationConfigRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeOrganizationConfigRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DescribeOrganizationConfigRules");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeOrganizationConfigRulesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeOrganizationConfigRulesOutput, body, allocator);
}
