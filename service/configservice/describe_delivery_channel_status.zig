...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDeliveryChannelStatusInput, options: Options) !DescribeDeliveryChannelStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDeliveryChannelStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeDeliveryChannelStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDeliveryChannelStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDeliveryChannelStatusOutput, body, alloc);
}
