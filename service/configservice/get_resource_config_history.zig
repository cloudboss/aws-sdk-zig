...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceConfigHistoryInput, options: Options) !GetResourceConfigHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceConfigHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetResourceConfigHistory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceConfigHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResourceConfigHistoryOutput, body, alloc);
}
