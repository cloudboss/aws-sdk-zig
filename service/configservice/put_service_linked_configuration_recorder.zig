...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutServiceLinkedConfigurationRecorderInput, options: Options) !PutServiceLinkedConfigurationRecorderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutServiceLinkedConfigurationRecorderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutServiceLinkedConfigurationRecorder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutServiceLinkedConfigurationRecorderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutServiceLinkedConfigurationRecorderOutput, body, alloc);
}
