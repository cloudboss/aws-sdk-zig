...
fn serializeRequest(alloc: std.mem.Allocator, input: StartConfigurationRecorderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.StartConfigurationRecorder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartConfigurationRecorderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
