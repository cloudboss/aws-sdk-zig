...
pub const DeleteConfigurationAggregatorInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConfigurationAggregatorInput, options: CallOptions) !DeleteConfigurationAggregatorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteConfigurationAggregatorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteConfigurationAggregator");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteConfigurationAggregatorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
