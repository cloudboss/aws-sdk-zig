...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRemediationExceptionsInput, options: Options) !DescribeRemediationExceptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRemediationExceptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeRemediationExceptions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRemediationExceptionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRemediationExceptionsOutput, body, alloc);
}
