...
const StoredQuery = @import("stored_query.zig").StoredQuery;

pub const GetStoredQueryInput = struct {
...
pub const GetStoredQueryOutput = struct {
    /// Returns a `StoredQuery` object.
    stored_query: ?StoredQuery = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStoredQueryInput, options: CallOptions) !GetStoredQueryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetStoredQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.GetStoredQuery");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetStoredQueryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetStoredQueryOutput, body, allocator);
}
