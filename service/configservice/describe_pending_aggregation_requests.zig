...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePendingAggregationRequestsInput, options: Options) !DescribePendingAggregationRequestsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePendingAggregationRequestsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribePendingAggregationRequests");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePendingAggregationRequestsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePendingAggregationRequestsOutput, body, alloc);
}
