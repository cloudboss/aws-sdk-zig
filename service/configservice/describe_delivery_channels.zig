...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDeliveryChannelsInput, options: Options) !DescribeDeliveryChannelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDeliveryChannelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeDeliveryChannels");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDeliveryChannelsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDeliveryChannelsOutput, body, alloc);
}
