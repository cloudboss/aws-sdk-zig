...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStoredQueriesInput, options: Options) !ListStoredQueriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStoredQueriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.ListStoredQueries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStoredQueriesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListStoredQueriesOutput, body, alloc);
}
