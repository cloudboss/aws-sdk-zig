...
pub const DeleteConfigRuleInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConfigRuleInput, options: CallOptions) !DeleteConfigRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteConfigRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteConfigRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteConfigRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
