...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteConfigRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DeleteConfigRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteConfigRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
