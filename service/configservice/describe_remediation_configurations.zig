...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRemediationConfigurationsInput, options: Options) !DescribeRemediationConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRemediationConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeRemediationConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRemediationConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRemediationConfigurationsOutput, body, alloc);
}
