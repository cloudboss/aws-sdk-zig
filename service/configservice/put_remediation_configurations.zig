...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRemediationConfigurationsInput, options: Options) !PutRemediationConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRemediationConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutRemediationConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRemediationConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutRemediationConfigurationsOutput, body, alloc);
}
