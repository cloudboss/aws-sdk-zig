...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConfigurationRecorderStatusInput, options: Options) !DescribeConfigurationRecorderStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConfigurationRecorderStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeConfigurationRecorderStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConfigurationRecorderStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConfigurationRecorderStatusOutput, body, alloc);
}
