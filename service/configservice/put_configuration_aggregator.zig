...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutConfigurationAggregatorInput, options: Options) !PutConfigurationAggregatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutConfigurationAggregatorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutConfigurationAggregator");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutConfigurationAggregatorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutConfigurationAggregatorOutput, body, alloc);
}
