...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConformancePackComplianceDetailsInput, options: Options) !GetConformancePackComplianceDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConformancePackComplianceDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetConformancePackComplianceDetails");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConformancePackComplianceDetailsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetConformancePackComplianceDetailsOutput, body, alloc);
}
