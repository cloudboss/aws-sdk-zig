...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRemediationExecutionStatusInput, options: Options) !DescribeRemediationExecutionStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRemediationExecutionStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeRemediationExecutionStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRemediationExecutionStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRemediationExecutionStatusOutput, body, alloc);
}
