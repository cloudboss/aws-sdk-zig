...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceEvaluationSummaryInput, options: Options) !GetResourceEvaluationSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceEvaluationSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetResourceEvaluationSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceEvaluationSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResourceEvaluationSummaryOutput, body, alloc);
}
