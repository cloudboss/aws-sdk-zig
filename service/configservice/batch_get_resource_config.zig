...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetResourceConfigInput, options: Options) !BatchGetResourceConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetResourceConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.BatchGetResourceConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetResourceConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetResourceConfigOutput, body, alloc);
}
