...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutEvaluationsInput, options: Options) !PutEvaluationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEvaluationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutEvaluations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEvaluationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutEvaluationsOutput, body, alloc);
}
