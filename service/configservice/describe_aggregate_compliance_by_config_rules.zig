...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAggregateComplianceByConfigRulesInput, options: Options) !DescribeAggregateComplianceByConfigRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAggregateComplianceByConfigRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeAggregateComplianceByConfigRules");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAggregateComplianceByConfigRulesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAggregateComplianceByConfigRulesOutput, body, alloc);
}
