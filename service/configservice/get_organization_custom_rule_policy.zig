...
pub const GetOrganizationCustomRulePolicyInput = struct {
...
pub const GetOrganizationCustomRulePolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOrganizationCustomRulePolicyInput, options: CallOptions) !GetOrganizationCustomRulePolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetOrganizationCustomRulePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.GetOrganizationCustomRulePolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetOrganizationCustomRulePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOrganizationCustomRulePolicyOutput, body, allocator);
}
