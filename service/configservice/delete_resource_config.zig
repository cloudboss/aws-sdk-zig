...
pub const DeleteResourceConfigInput = struct {
...
pub const DeleteResourceConfigOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResourceConfigInput, options: CallOptions) !DeleteResourceConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteResourceConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteResourceConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteResourceConfigOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
