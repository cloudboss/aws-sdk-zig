...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAggregateConformancePackComplianceSummaryInput, options: Options) !GetAggregateConformancePackComplianceSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAggregateConformancePackComplianceSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetAggregateConformancePackComplianceSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAggregateConformancePackComplianceSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAggregateConformancePackComplianceSummaryOutput, body, alloc);
}
