...
pub const DeleteConformancePackInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConformancePackInput, options: CallOptions) !DeleteConformancePackOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteConformancePackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteConformancePack");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteConformancePackOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
