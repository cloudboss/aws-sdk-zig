...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteConformancePackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DeleteConformancePack");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteConformancePackOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
