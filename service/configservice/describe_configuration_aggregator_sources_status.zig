...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConfigurationAggregatorSourcesStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeConfigurationAggregatorSourcesStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConfigurationAggregatorSourcesStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConfigurationAggregatorSourcesStatusOutput, body, alloc);
}
