...
const AggregatedSourceStatus = @import("aggregated_source_status.zig").AggregatedSourceStatus;

pub const DescribeConfigurationAggregatorSourcesStatusInput = struct {
    ...
    /// recent.
    update_status: ?[]const AggregatedSourceStatusType = null,
...
pub const DescribeConfigurationAggregatorSourcesStatusOutput = struct {
    /// Returns an AggregatedSourceStatus object.
    aggregated_source_status_list: ?[]const AggregatedSourceStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConfigurationAggregatorSourcesStatusInput, options: CallOptions) !DescribeConfigurationAggregatorSourcesStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeConfigurationAggregatorSourcesStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DescribeConfigurationAggregatorSourcesStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeConfigurationAggregatorSourcesStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConfigurationAggregatorSourcesStatusOutput, body, allocator);
}
