...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SelectResourceConfigInput, options: Options) !SelectResourceConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SelectResourceConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.SelectResourceConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SelectResourceConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SelectResourceConfigOutput, body, alloc);
}
