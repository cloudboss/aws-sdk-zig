...
pub const GetAggregateResourceConfigInput = struct {
...
pub const GetAggregateResourceConfigOutput = struct {
    /// Returns a `ConfigurationItem` object.
    configuration_item: ?ConfigurationItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAggregateResourceConfigInput, options: CallOptions) !GetAggregateResourceConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAggregateResourceConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.GetAggregateResourceConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAggregateResourceConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAggregateResourceConfigOutput, body, allocator);
}
