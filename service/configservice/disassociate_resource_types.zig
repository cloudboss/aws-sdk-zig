...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateResourceTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DisassociateResourceTypes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateResourceTypesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DisassociateResourceTypesOutput, body, alloc);
}
