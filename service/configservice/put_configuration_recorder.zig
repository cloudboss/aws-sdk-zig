...
fn serializeRequest(alloc: std.mem.Allocator, input: PutConfigurationRecorderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutConfigurationRecorder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutConfigurationRecorderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
