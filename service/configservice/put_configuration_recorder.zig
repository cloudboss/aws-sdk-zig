...
pub const PutConfigurationRecorderInput = struct {
    ...
    /// The tags for the customer managed configuration recorder. Each tag consists
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutConfigurationRecorderInput, options: CallOptions) !PutConfigurationRecorderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutConfigurationRecorderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.PutConfigurationRecorder");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutConfigurationRecorderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
