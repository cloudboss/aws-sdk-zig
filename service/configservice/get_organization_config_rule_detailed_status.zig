...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOrganizationConfigRuleDetailedStatusInput, options: Options) !GetOrganizationConfigRuleDetailedStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOrganizationConfigRuleDetailedStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetOrganizationConfigRuleDetailedStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOrganizationConfigRuleDetailedStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOrganizationConfigRuleDetailedStatusOutput, body, alloc);
}
