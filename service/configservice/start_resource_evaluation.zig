...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartResourceEvaluationInput, options: Options) !StartResourceEvaluationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartResourceEvaluationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.StartResourceEvaluation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartResourceEvaluationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartResourceEvaluationOutput, body, alloc);
}
