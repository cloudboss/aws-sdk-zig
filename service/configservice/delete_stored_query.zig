...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteStoredQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DeleteStoredQuery");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteStoredQueryOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
