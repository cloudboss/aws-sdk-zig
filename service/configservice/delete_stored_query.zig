...
pub const DeleteStoredQueryInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStoredQueryInput, options: Options) !DeleteStoredQueryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteStoredQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteStoredQuery");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteStoredQueryOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
