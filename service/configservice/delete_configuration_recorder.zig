...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConfigurationRecorderInput, options: Options) !DeleteConfigurationRecorderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteConfigurationRecorderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DeleteConfigurationRecorder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteConfigurationRecorderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
