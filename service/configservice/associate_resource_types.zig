...
pub const AssociateResourceTypesInput = struct {
...
pub const AssociateResourceTypesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateResourceTypesInput, options: CallOptions) !AssociateResourceTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateResourceTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.AssociateResourceTypes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateResourceTypesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(AssociateResourceTypesOutput, body, allocator);
}
