...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeComplianceByResourceInput, options: Options) !DescribeComplianceByResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeComplianceByResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeComplianceByResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeComplianceByResourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeComplianceByResourceOutput, body, alloc);
}
