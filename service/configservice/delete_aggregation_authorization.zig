...
pub const DeleteAggregationAuthorizationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAggregationAuthorizationInput, options: CallOptions) !DeleteAggregationAuthorizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAggregationAuthorizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteAggregationAuthorization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAggregationAuthorizationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
