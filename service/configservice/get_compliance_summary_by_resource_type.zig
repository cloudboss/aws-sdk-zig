...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetComplianceSummaryByResourceTypeInput, options: Options) !GetComplianceSummaryByResourceTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetComplianceSummaryByResourceTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetComplianceSummaryByResourceType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetComplianceSummaryByResourceTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetComplianceSummaryByResourceTypeOutput, body, alloc);
}
