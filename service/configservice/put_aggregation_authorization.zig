...
pub const PutAggregationAuthorizationInput = struct {
...
pub const PutAggregationAuthorizationOutput = struct {
    /// Returns an AggregationAuthorization object.
    aggregation_authorization: ?AggregationAuthorization = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAggregationAuthorizationInput, options: CallOptions) !PutAggregationAuthorizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutAggregationAuthorizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.PutAggregationAuthorization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutAggregationAuthorizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutAggregationAuthorizationOutput, body, allocator);
}
