...
pub const DeleteOrganizationConfigRuleInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOrganizationConfigRuleInput, options: CallOptions) !DeleteOrganizationConfigRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteOrganizationConfigRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteOrganizationConfigRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteOrganizationConfigRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
