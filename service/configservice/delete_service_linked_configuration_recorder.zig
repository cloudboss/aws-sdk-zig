...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteServiceLinkedConfigurationRecorderInput, options: Options) !DeleteServiceLinkedConfigurationRecorderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteServiceLinkedConfigurationRecorderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DeleteServiceLinkedConfigurationRecorder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteServiceLinkedConfigurationRecorderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteServiceLinkedConfigurationRecorderOutput, body, alloc);
}
