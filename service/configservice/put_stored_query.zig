...
pub const PutStoredQueryInput = struct {
...
pub const PutStoredQueryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutStoredQueryInput, options: CallOptions) !PutStoredQueryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutStoredQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.PutStoredQuery");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutStoredQueryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutStoredQueryOutput, body, allocator);
}
