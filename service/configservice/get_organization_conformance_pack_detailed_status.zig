...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOrganizationConformancePackDetailedStatusInput, options: Options) !GetOrganizationConformancePackDetailedStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOrganizationConformancePackDetailedStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetOrganizationConformancePackDetailedStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOrganizationConformancePackDetailedStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOrganizationConformancePackDetailedStatusOutput, body, alloc);
}
