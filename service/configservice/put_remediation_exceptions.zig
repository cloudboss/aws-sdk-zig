...
const FailedRemediationExceptionBatch = @import("failed_remediation_exception_batch.zig").FailedRemediationExceptionBatch;

pub const PutRemediationExceptionsInput = struct {
...
pub const PutRemediationExceptionsOutput = struct {
    ...
    failed_batches: ?[]const FailedRemediationExceptionBatch = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRemediationExceptionsInput, options: CallOptions) !PutRemediationExceptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutRemediationExceptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.PutRemediationExceptions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutRemediationExceptionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutRemediationExceptionsOutput, body, allocator);
}
