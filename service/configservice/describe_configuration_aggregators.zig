...
pub const DescribeConfigurationAggregatorsInput = struct {
    /// The name of the configuration aggregators.
    configuration_aggregator_names: ?[]const []const u8 = null,
...
pub const DescribeConfigurationAggregatorsOutput = struct {
    /// Returns a ConfigurationAggregators object.
    configuration_aggregators: ?[]const ConfigurationAggregator = null,
    ...
    pub const json_field_names = .{
        .configuration_aggregators = "ConfigurationAggregators",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConfigurationAggregatorsInput, options: CallOptions) !DescribeConfigurationAggregatorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeConfigurationAggregatorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DescribeConfigurationAggregators");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeConfigurationAggregatorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConfigurationAggregatorsOutput, body, allocator);
}
