...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAggregationAuthorizationsInput, options: Options) !DescribeAggregationAuthorizationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAggregationAuthorizationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeAggregationAuthorizations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAggregationAuthorizationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAggregationAuthorizationsOutput, body, alloc);
}
