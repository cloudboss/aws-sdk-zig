...
pub const DeleteRemediationConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRemediationConfigurationInput, options: Options) !DeleteRemediationConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRemediationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteRemediationConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRemediationConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
