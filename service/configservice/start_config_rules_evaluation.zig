...
pub const StartConfigRulesEvaluationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartConfigRulesEvaluationInput, options: Options) !StartConfigRulesEvaluationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartConfigRulesEvaluationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.StartConfigRulesEvaluation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartConfigRulesEvaluationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
