...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRemediationExecutionInput, options: Options) !StartRemediationExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRemediationExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.StartRemediationExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRemediationExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartRemediationExecutionOutput, body, alloc);
}
