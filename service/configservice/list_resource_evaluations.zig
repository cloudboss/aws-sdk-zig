...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceEvaluationsInput, options: Options) !ListResourceEvaluationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceEvaluationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.ListResourceEvaluations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceEvaluationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceEvaluationsOutput, body, alloc);
}
