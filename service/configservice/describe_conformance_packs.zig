...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConformancePacksInput, options: Options) !DescribeConformancePacksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConformancePacksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeConformancePacks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConformancePacksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConformancePacksOutput, body, alloc);
}
