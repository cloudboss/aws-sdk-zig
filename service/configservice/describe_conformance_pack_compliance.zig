...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConformancePackComplianceInput, options: Options) !DescribeConformancePackComplianceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConformancePackComplianceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeConformancePackCompliance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConformancePackComplianceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeConformancePackComplianceOutput, body, alloc);
}
