...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetComplianceDetailsByConfigRuleInput, options: Options) !GetComplianceDetailsByConfigRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetComplianceDetailsByConfigRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetComplianceDetailsByConfigRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetComplianceDetailsByConfigRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetComplianceDetailsByConfigRuleOutput, body, alloc);
}
