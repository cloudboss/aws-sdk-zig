...
pub const DeleteEvaluationResultsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEvaluationResultsInput, options: Options) !DeleteEvaluationResultsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteEvaluationResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteEvaluationResults");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteEvaluationResultsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
