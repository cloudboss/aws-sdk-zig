...
pub const DeleteDeliveryChannelInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDeliveryChannelInput, options: CallOptions) !DeleteDeliveryChannelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDeliveryChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeleteDeliveryChannel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDeliveryChannelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
