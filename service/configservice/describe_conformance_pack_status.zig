...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConformancePackStatusInput, options: Options) !DescribeConformancePackStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConformancePackStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeConformancePackStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConformancePackStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConformancePackStatusOutput, body, alloc);
}
