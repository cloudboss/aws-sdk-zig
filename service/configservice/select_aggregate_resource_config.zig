...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SelectAggregateResourceConfigInput, options: Options) !SelectAggregateResourceConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SelectAggregateResourceConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.SelectAggregateResourceConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SelectAggregateResourceConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SelectAggregateResourceConfigOutput, body, alloc);
}
