...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAggregateDiscoveredResourcesInput, options: Options) !ListAggregateDiscoveredResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAggregateDiscoveredResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.ListAggregateDiscoveredResources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAggregateDiscoveredResourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAggregateDiscoveredResourcesOutput, body, alloc);
}
