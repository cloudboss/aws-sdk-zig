...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDiscoveredResourceCountsInput, options: Options) !GetDiscoveredResourceCountsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDiscoveredResourceCountsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetDiscoveredResourceCounts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDiscoveredResourceCountsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDiscoveredResourceCountsOutput, body, alloc);
}
