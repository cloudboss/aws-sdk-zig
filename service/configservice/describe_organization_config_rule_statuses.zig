...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOrganizationConfigRuleStatusesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeOrganizationConfigRuleStatuses");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOrganizationConfigRuleStatusesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeOrganizationConfigRuleStatusesOutput, body, alloc);
}
