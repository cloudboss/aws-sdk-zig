...
const OrganizationConfigRuleStatus = @import("organization_config_rule_status.zig").OrganizationConfigRuleStatus;

pub const DescribeOrganizationConfigRuleStatusesInput = struct {
    ...
    organization_config_rule_names: ?[]const []const u8 = null,
...
pub const DescribeOrganizationConfigRuleStatusesOutput = struct {
    /// The `nextToken` string returned on a previous page that you use to get the
    /// next page of results in a paginated response.
    next_token: ?[]const u8 = null,

    /// A list of `OrganizationConfigRuleStatus` objects.
    organization_config_rule_statuses: ?[]const OrganizationConfigRuleStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrganizationConfigRuleStatusesInput, options: CallOptions) !DescribeOrganizationConfigRuleStatusesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeOrganizationConfigRuleStatusesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DescribeOrganizationConfigRuleStatuses");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeOrganizationConfigRuleStatusesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeOrganizationConfigRuleStatusesOutput, body, allocator);
}
