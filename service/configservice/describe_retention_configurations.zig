...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRetentionConfigurationsInput, options: Options) !DescribeRetentionConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRetentionConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeRetentionConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRetentionConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRetentionConfigurationsOutput, body, alloc);
}
