...
const ExternalEvaluation = @import("external_evaluation.zig").ExternalEvaluation;

pub const PutExternalEvaluationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutExternalEvaluationInput, options: Options) !PutExternalEvaluationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutExternalEvaluationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.PutExternalEvaluation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutExternalEvaluationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
