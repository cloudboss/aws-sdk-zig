...
fn serializeRequest(alloc: std.mem.Allocator, input: PutExternalEvaluationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutExternalEvaluation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutExternalEvaluationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
