...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeliverConfigSnapshotInput, options: Options) !DeliverConfigSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeliverConfigSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DeliverConfigSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeliverConfigSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeliverConfigSnapshotOutput, body, alloc);
}
