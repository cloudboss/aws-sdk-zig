...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAggregateDiscoveredResourceCountsInput, options: Options) !GetAggregateDiscoveredResourceCountsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAggregateDiscoveredResourceCountsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetAggregateDiscoveredResourceCounts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAggregateDiscoveredResourceCountsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetAggregateDiscoveredResourceCountsOutput, body, alloc);
}
