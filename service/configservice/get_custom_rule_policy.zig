...
pub const GetCustomRulePolicyInput = struct {
...
pub const GetCustomRulePolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCustomRulePolicyInput, options: CallOptions) !GetCustomRulePolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCustomRulePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.GetCustomRulePolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCustomRulePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCustomRulePolicyOutput, body, allocator);
}
