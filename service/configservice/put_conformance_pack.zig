...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutConformancePackInput, options: Options) !PutConformancePackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutConformancePackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.PutConformancePack");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutConformancePackOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutConformancePackOutput, body, alloc);
}
