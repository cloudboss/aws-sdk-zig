...
pub const GetComplianceDetailsByResourceInput = struct {
    ...
    ///
    /// You need to only provide either a `ResourceEvaluationID` or a `ResourceID
    /// `and `ResourceType`.
...
pub const GetComplianceDetailsByResourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetComplianceDetailsByResourceInput, options: CallOptions) !GetComplianceDetailsByResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetComplianceDetailsByResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.GetComplianceDetailsByResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetComplianceDetailsByResourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetComplianceDetailsByResourceOutput, body, allocator);
}
