...
pub const DeletePendingAggregationRequestInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePendingAggregationRequestInput, options: CallOptions) !DeletePendingAggregationRequestOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePendingAggregationRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StarlingDoveService.DeletePendingAggregationRequest");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePendingAggregationRequestOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
