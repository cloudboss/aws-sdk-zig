...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConfigRulesInput, options: Options) !DescribeConfigRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConfigRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.DescribeConfigRules");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConfigRulesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConfigRulesOutput, body, alloc);
}
