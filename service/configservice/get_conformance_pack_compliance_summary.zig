...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConformancePackComplianceSummaryInput, options: Options) !GetConformancePackComplianceSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConformancePackComplianceSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StarlingDoveService.GetConformancePackComplianceSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConformancePackComplianceSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetConformancePackComplianceSummaryOutput, body, alloc);
}
