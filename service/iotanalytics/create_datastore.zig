...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDatastoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"datastoreName\":");
    try aws.json.writeValue(@TypeOf(input.datastore_name), input.datastore_name, alloc, &body_buf);
    has_prev = true;
    if (input.datastore_partitions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"datastorePartitions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.datastore_storage) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"datastoreStorage\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.file_format_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"fileFormatConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDatastoreOutput {
    var result: CreateDatastoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDatastoreOutput, body, alloc);
    }
    _ = status;
    _ = headers;
