...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartPipelineReprocessingInput, options: Options) !StartPipelineReprocessingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartPipelineReprocessingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.channel_messages) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"channelMessages\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartPipelineReprocessingOutput {
    var result: StartPipelineReprocessingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartPipelineReprocessingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
