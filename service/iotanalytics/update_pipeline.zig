...
pub const UpdatePipelineInput = struct {
    ...
    /// filtering messages
    /// based on attribute values; invoking your Lambda functions on messages for
    /// advanced processing;
    ...
    /// ]`
    pipeline_activities: []const PipelineActivity,
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"pipelineActivities\":");
    try aws.json.writeValue(@TypeOf(input.pipeline_activities), input.pipeline_activities, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePipelineOutput {
    ...
    _ = status;
    _ = headers;
    const result: UpdatePipelineOutput = .{};
