...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"pipelineActivities\":");
    try aws.json.writeValue(@TypeOf(input.pipeline_activities), input.pipeline_activities, allocator, &body_buf);
    has_prev = true;
