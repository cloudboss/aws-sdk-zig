...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIntentInput, options: Options) !DescribeIntentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIntentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIntentOutput {
    var result: DescribeIntentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeIntentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
