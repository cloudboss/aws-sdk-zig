...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCustomVocabularyInput, options: Options) !DeleteCustomVocabularyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCustomVocabularyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/customvocabulary");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCustomVocabularyOutput {
    var result: DeleteCustomVocabularyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteCustomVocabularyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
