...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBotReplicaInput, options: CallOptions) !DeleteBotReplicaOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBotReplicaInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBotReplicaOutput {
    var result: DeleteBotReplicaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBotReplicaOutput, body, allocator);
    }
    _ = status;
    _ = headers;
