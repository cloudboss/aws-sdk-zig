...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSlotTypesInput, options: Options) !ListSlotTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSlotTypesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSlotTypesOutput {
    var result: ListSlotTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSlotTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
