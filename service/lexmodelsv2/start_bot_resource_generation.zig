...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartBotResourceGenerationInput, options: Options) !StartBotResourceGenerationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartBotResourceGenerationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/startgeneration");
    ...
    try body_buf.appendSlice(alloc, "\"generationInputPrompt\":");
    try aws.json.writeValue(@TypeOf(input.generation_input_prompt), input.generation_input_prompt, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartBotResourceGenerationOutput {
    var result: StartBotResourceGenerationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartBotResourceGenerationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
