...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResourcePolicyStatementInput, options: Options) !CreateResourcePolicyStatementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourcePolicyStatementInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourcePolicyStatementOutput {
    var result: CreateResourcePolicyStatementOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateResourcePolicyStatementOutput, body, alloc);
    }
    _ = status;
    _ = headers;
