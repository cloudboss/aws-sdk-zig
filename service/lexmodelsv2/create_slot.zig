...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSlotInput, options: Options) !CreateSlotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSlotInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.intent_id);
    try path_buf.appendSlice(alloc, "/slots");
    ...
    if (input.multiple_values_setting) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"multipleValuesSetting\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.obfuscation_setting) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"obfuscationSetting\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"slotName\":");
    try aws.json.writeValue(@TypeOf(input.slot_name), input.slot_name, alloc, &body_buf);
    has_prev = true;
    if (input.slot_type_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"slotTypeId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.sub_slot_setting) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"subSlotSetting\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"valueElicitationSetting\":");
    try aws.json.writeValue(@TypeOf(input.value_elicitation_setting), input.value_elicitation_setting, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSlotOutput {
    var result: CreateSlotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSlotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
