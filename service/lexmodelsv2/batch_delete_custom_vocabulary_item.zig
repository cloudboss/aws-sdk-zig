...
pub const BatchDeleteCustomVocabularyItemInput = struct {
    ...
    custom_vocabulary_item_list: []const CustomVocabularyEntryId,
...
pub const BatchDeleteCustomVocabularyItemOutput = struct {
    /// The identifier of the bot associated with this custom vocabulary.
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteCustomVocabularyItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/customvocabulary/DEFAULT/batchdelete");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteCustomVocabularyItemOutput {
    var result: BatchDeleteCustomVocabularyItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteCustomVocabularyItemOutput, body, alloc);
    }
    _ = status;
    _ = headers;
