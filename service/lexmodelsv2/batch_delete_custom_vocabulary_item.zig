...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDeleteCustomVocabularyItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/customvocabulary/DEFAULT/batchdelete");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDeleteCustomVocabularyItemOutput {
    var result: BatchDeleteCustomVocabularyItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteCustomVocabularyItemOutput, body, allocator);
    }
    _ = status;
    _ = headers;
