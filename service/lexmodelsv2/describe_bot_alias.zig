...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBotAliasInput, options: Options) !DescribeBotAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBotAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBotAliasOutput {
    var result: DescribeBotAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBotAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
