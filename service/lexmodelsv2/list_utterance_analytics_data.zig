...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUtteranceAnalyticsDataInput, options: Options) !ListUtteranceAnalyticsDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUtteranceAnalyticsDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/analytics/utterances");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUtteranceAnalyticsDataOutput {
    var result: ListUtteranceAnalyticsDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUtteranceAnalyticsDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
