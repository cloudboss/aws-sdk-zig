...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBotLocaleInput, options: Options) !CreateBotLocaleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBotLocaleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.generative_ai_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"generativeAISettings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"nluIntentConfidenceThreshold\":");
    try aws.json.writeValue(@TypeOf(input.nlu_intent_confidence_threshold), input.nlu_intent_confidence_threshold, alloc, &body_buf);
    has_prev = true;
    if (input.speech_detection_sensitivity) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"speechDetectionSensitivity\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.speech_recognition_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"speechRecognitionSettings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.unified_speech_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"unifiedSpeechSettings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.voice_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"voiceSettings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBotLocaleOutput {
    var result: CreateBotLocaleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBotLocaleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
