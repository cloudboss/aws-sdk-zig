...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreateCustomVocabularyItemInput, options: CallOptions) !BatchCreateCustomVocabularyItemOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchCreateCustomVocabularyItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/customvocabulary/DEFAULT/batchcreate");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchCreateCustomVocabularyItemOutput {
    var result: BatchCreateCustomVocabularyItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchCreateCustomVocabularyItemOutput, body, allocator);
    }
    _ = status;
    _ = headers;
