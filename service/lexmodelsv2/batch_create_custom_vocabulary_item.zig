...
const NewCustomVocabularyItem = @import("new_custom_vocabulary_item.zig").NewCustomVocabularyItem;
...
pub const BatchCreateCustomVocabularyItemInput = struct {
    ...
    custom_vocabulary_item_list: []const NewCustomVocabularyItem,
...
pub const BatchCreateCustomVocabularyItemOutput = struct {
    /// The identifier of the bot associated with this custom vocabulary.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreateCustomVocabularyItemInput, options: Options) !BatchCreateCustomVocabularyItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateCustomVocabularyItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/customvocabulary/DEFAULT/batchcreate");
    ...
    try body_buf.appendSlice(alloc, "\"customVocabularyItemList\":");
    try aws.json.writeValue(@TypeOf(input.custom_vocabulary_item_list), input.custom_vocabulary_item_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateCustomVocabularyItemOutput {
    var result: BatchCreateCustomVocabularyItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchCreateCustomVocabularyItemOutput, body, alloc);
    }
    _ = status;
    _ = headers;
