...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResourcePolicyInput, options: Options) !UpdateResourcePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResourcePolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResourcePolicyOutput {
    var result: UpdateResourcePolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResourcePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
