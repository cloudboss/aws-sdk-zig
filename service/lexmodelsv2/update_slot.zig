...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSlotInput, options: Options) !UpdateSlotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSlotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSlotOutput {
    var result: UpdateSlotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSlotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
