...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSessionMetricsInput, options: Options) !ListSessionMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSessionMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/analytics/sessionmetrics");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSessionMetricsOutput {
    var result: ListSessionMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSessionMetricsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
