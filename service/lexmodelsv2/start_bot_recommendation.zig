...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartBotRecommendationInput, options: CallOptions) !StartBotRecommendationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartBotRecommendationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.encryption_setting) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"encryptionSetting\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"transcriptSourceSetting\":");
    try aws.json.writeValue(@TypeOf(input.transcript_source_setting), input.transcript_source_setting, allocator, &body_buf);
    has_prev = true;
