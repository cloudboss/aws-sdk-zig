...
pub const StartBotRecommendationInput = struct {
...
pub const StartBotRecommendationOutput = struct {
    /// The unique identifier of the bot containing the bot
    /// recommendation.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartBotRecommendationInput, options: Options) !StartBotRecommendationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartBotRecommendationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"transcriptSourceSetting\":");
    try aws.json.writeValue(@TypeOf(input.transcript_source_setting), input.transcript_source_setting, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartBotRecommendationOutput {
    var result: StartBotRecommendationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartBotRecommendationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
