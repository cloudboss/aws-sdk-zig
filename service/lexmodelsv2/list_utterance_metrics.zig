...
const AnalyticsUtteranceAttribute = @import("analytics_utterance_attribute.zig").AnalyticsUtteranceAttribute;
...
const AnalyticsUtteranceGroupBySpecification = @import("analytics_utterance_group_by_specification.zig").AnalyticsUtteranceGroupBySpecification;
const AnalyticsUtteranceMetric = @import("analytics_utterance_metric.zig").AnalyticsUtteranceMetric;
const AnalyticsUtteranceResult = @import("analytics_utterance_result.zig").AnalyticsUtteranceResult;

pub const ListUtteranceMetricsInput = struct {
    ...
    attributes: ?[]const AnalyticsUtteranceAttribute = null,
    ...
    ///   detailed in [Key
    ///   definitions](https://docs.aws.amazon.com/analytics-key-definitions-utterances) in the user guide.
    group_by: ?[]const AnalyticsUtteranceGroupBySpecification = null,
    ...
    metrics: []const AnalyticsUtteranceMetric,
...
pub const ListUtteranceMetricsOutput = struct {
    ...
    results: ?[]const AnalyticsUtteranceResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUtteranceMetricsInput, options: Options) !ListUtteranceMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUtteranceMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/analytics/utterancemetrics");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUtteranceMetricsOutput {
    var result: ListUtteranceMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUtteranceMetricsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
