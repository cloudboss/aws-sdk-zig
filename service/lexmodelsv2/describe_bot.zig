...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBotInput, options: Options) !DescribeBotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBotOutput {
    var result: DescribeBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
