...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCustomVocabularyMetadataInput, options: Options) !DescribeCustomVocabularyMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCustomVocabularyMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/customvocabulary/DEFAULT/metadata");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCustomVocabularyMetadataOutput {
    var result: DescribeCustomVocabularyMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCustomVocabularyMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
