...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchAssociatedTranscriptsInput, options: Options) !SearchAssociatedTranscriptsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchAssociatedTranscriptsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/associatedtranscripts");
    ...
    if (input.next_index) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"nextIndex\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.search_order) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"searchOrder\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchAssociatedTranscriptsOutput {
    var result: SearchAssociatedTranscriptsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchAssociatedTranscriptsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
