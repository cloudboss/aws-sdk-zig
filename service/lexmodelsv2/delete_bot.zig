...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBotInput, options: Options) !DeleteBotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBotOutput {
    var result: DeleteBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
