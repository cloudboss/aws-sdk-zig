...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSlotTypeInput, options: Options) !UpdateSlotTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSlotTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/slottypes/");
    try path_buf.appendSlice(alloc, input.slot_type_id);
    ...
    if (input.composite_slot_type_setting) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"compositeSlotTypeSetting\":");
    ...
    if (input.external_source_setting) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"externalSourceSetting\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.parent_slot_type_signature) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"parentSlotTypeSignature\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"slotTypeName\":");
    try aws.json.writeValue(@TypeOf(input.slot_type_name), input.slot_type_name, alloc, &body_buf);
    has_prev = true;
    if (input.slot_type_values) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"slotTypeValues\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.value_selection_setting) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"valueSelectionSetting\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSlotTypeOutput {
    var result: UpdateSlotTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSlotTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
