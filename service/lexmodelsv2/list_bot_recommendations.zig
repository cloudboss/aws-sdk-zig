...
const BotRecommendationSummary = @import("bot_recommendation_summary.zig").BotRecommendationSummary;

pub const ListBotRecommendationsInput = struct {
...
pub const ListBotRecommendationsOutput = struct {
    /// The unique identifier of the bot that contains the bot
    /// recommendation list.
    ...
    bot_recommendation_summaries: ?[]const BotRecommendationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBotRecommendationsInput, options: CallOptions) !ListBotRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBotRecommendationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBotRecommendationsOutput {
    var result: ListBotRecommendationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBotRecommendationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
