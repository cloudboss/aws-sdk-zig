...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBotRecommendationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBotRecommendationsOutput {
    var result: ListBotRecommendationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBotRecommendationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
