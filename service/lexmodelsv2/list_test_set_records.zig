...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTestSetRecordsInput, options: Options) !ListTestSetRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTestSetRecordsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTestSetRecordsOutput {
    var result: ListTestSetRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTestSetRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
