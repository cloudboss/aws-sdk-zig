...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTestSetsInput, options: Options) !ListTestSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTestSetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTestSetsOutput {
    var result: ListTestSetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTestSetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
