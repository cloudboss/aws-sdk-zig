...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTestSetGenerationInput, options: Options) !DescribeTestSetGenerationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTestSetGenerationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/testsetgenerations/");
    try path_buf.appendSlice(alloc, input.test_set_generation_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTestSetGenerationOutput {
    var result: DescribeTestSetGenerationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTestSetGenerationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
