...
pub const CreateResourcePolicyInput = struct {
...
pub const CreateResourcePolicyOutput = struct {
    /// The Amazon Resource Name (ARN) of the bot or bot alias that the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResourcePolicyInput, options: Options) !CreateResourcePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourcePolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourcePolicyOutput {
    var result: CreateResourcePolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateResourcePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
