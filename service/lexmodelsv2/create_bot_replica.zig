...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBotReplicaInput, options: Options) !CreateBotReplicaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBotReplicaInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBotReplicaOutput {
    var result: CreateBotReplicaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBotReplicaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
