...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"fileFormat\":");
    try aws.json.writeValue(@TypeOf(input.file_format), input.file_format, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateExportOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateExportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
