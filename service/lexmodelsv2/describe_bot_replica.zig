...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBotReplicaInput, options: Options) !DescribeBotReplicaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBotReplicaInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBotReplicaOutput {
    var result: DescribeBotReplicaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBotReplicaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
