...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBotAliasReplicasInput, options: Options) !ListBotAliasReplicasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBotAliasReplicasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBotAliasReplicasOutput {
    var result: ListBotAliasReplicasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBotAliasReplicasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
