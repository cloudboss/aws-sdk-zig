...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBotAliasInput, options: Options) !DeleteBotAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBotAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBotAliasOutput {
    var result: DeleteBotAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBotAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
