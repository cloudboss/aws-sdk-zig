...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBuiltInSlotTypesInput, options: Options) !ListBuiltInSlotTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBuiltInSlotTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/builtins/locales/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBuiltInSlotTypesOutput {
    var result: ListBuiltInSlotTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBuiltInSlotTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
