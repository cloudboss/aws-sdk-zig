...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateBotElementInput, options: Options) !GenerateBotElementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateBotElementInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/generate");
    ...
    try body_buf.appendSlice(alloc, "\"intentId\":");
    try aws.json.writeValue(@TypeOf(input.intent_id), input.intent_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateBotElementOutput {
    var result: GenerateBotElementOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateBotElementOutput, body, alloc);
    }
    _ = status;
    _ = headers;
