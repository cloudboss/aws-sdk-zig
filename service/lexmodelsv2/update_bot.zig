...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBotInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.bot_members) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"botMembers\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"botName\":");
    try aws.json.writeValue(@TypeOf(input.bot_name), input.bot_name, alloc, &body_buf);
    has_prev = true;
    if (input.bot_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"botType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"dataPrivacy\":");
    try aws.json.writeValue(@TypeOf(input.data_privacy), input.data_privacy, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.error_log_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"errorLogSettings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"idleSessionTTLInSeconds\":");
    try aws.json.writeValue(@TypeOf(input.idle_session_ttl_in_seconds), input.idle_session_ttl_in_seconds, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
