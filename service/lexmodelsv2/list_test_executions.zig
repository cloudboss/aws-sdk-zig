...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTestExecutionsInput, options: Options) !ListTestExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTestExecutionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTestExecutionsOutput {
    var result: ListTestExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTestExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
