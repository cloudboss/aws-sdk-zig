...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBotVersionsInput, options: Options) !ListBotVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBotVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBotVersionsOutput {
    var result: ListBotVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBotVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
