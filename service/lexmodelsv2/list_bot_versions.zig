...
const BotVersionSortBy = @import("bot_version_sort_by.zig").BotVersionSortBy;
const BotVersionSummary = @import("bot_version_summary.zig").BotVersionSummary;

pub const ListBotVersionsInput = struct {
...
pub const ListBotVersionsOutput = struct {
    /// The identifier of the bot to list versions for.
    ...
    bot_version_summaries: ?[]const BotVersionSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBotVersionsInput, options: CallOptions) !ListBotVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBotVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBotVersionsOutput {
    var result: ListBotVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBotVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
