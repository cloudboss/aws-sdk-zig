...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSlotTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSlotTypeOutput {
    var result: CreateSlotTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSlotTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
