...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBotLocaleInput, options: Options) !UpdateBotLocaleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBotLocaleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBotLocaleOutput {
    var result: UpdateBotLocaleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBotLocaleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
