...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTestExecutionInput, options: Options) !DescribeTestExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTestExecutionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTestExecutionOutput {
    var result: DescribeTestExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTestExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
