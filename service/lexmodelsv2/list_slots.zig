...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSlotsInput, options: CallOptions) !ListSlotsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSlotsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSlotsOutput {
    var result: ListSlotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSlotsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
