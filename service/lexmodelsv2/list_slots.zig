...
const SlotFilter = @import("slot_filter.zig").SlotFilter;
const SlotSortBy = @import("slot_sort_by.zig").SlotSortBy;
const SlotSummary = @import("slot_summary.zig").SlotSummary;

pub const ListSlotsInput = struct {
...
pub const ListSlotsOutput = struct {
    ...
    slot_summaries: ?[]const SlotSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSlotsInput, options: Options) !ListSlotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSlotsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSlotsOutput {
    var result: ListSlotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSlotsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
