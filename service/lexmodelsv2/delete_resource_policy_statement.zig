...
pub const DeleteResourcePolicyStatementInput = struct {
...
pub const DeleteResourcePolicyStatementOutput = struct {
    /// The Amazon Resource Name (ARN) of the bot or bot alias that the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResourcePolicyStatementInput, options: Options) !DeleteResourcePolicyStatementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteResourcePolicyStatementInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/statements/");
    try path_buf.appendSlice(alloc, input.statement_id);
    ...
    if (input.expected_revision_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "expectedRevisionId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteResourcePolicyStatementOutput {
    var result: DeleteResourcePolicyStatementOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteResourcePolicyStatementOutput, body, alloc);
    }
    _ = status;
    _ = headers;
