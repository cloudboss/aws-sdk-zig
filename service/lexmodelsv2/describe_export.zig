...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeExportOutput {
    var result: DescribeExportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeExportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
