...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSlotTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSlotTypeOutput {
    var result: DescribeSlotTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSlotTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
