...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTestExecutionResultItemsInput, options: CallOptions) !ListTestExecutionResultItemsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTestExecutionResultItemsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"resultFilterBy\":");
    try aws.json.writeValue(@TypeOf(input.result_filter_by), input.result_filter_by, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTestExecutionResultItemsOutput {
    var result: ListTestExecutionResultItemsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTestExecutionResultItemsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
