...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecommendedIntentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecommendedIntentsOutput {
    var result: ListRecommendedIntentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRecommendedIntentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
