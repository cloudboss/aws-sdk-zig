...
const RecommendedIntentSummary = @import("recommended_intent_summary.zig").RecommendedIntentSummary;

pub const ListRecommendedIntentsInput = struct {
...
pub const ListRecommendedIntentsOutput = struct {
    /// The unique identifier of the bot associated with the recommended
    /// intent.
    ...
    /// results.
    summary_list: ?[]const RecommendedIntentSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecommendedIntentsInput, options: CallOptions) !ListRecommendedIntentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRecommendedIntentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRecommendedIntentsOutput {
    var result: ListRecommendedIntentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRecommendedIntentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
