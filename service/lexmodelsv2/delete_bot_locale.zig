...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBotLocaleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBotLocaleOutput {
    var result: DeleteBotLocaleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBotLocaleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
