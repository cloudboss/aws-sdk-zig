...
const BotLocaleFilter = @import("bot_locale_filter.zig").BotLocaleFilter;
const BotLocaleSortBy = @import("bot_locale_sort_by.zig").BotLocaleSortBy;
const BotLocaleSummary = @import("bot_locale_summary.zig").BotLocaleSummary;

pub const ListBotLocalesInput = struct {
    ...
    filters: ?[]const BotLocaleFilter = null,
...
pub const ListBotLocalesOutput = struct {
    /// The identifier of the bot to list locales for.
    ...
    bot_locale_summaries: ?[]const BotLocaleSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBotLocalesInput, options: Options) !ListBotLocalesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBotLocalesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBotLocalesOutput {
    var result: ListBotLocalesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBotLocalesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
