...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTestSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTestSetOutput {
    var result: UpdateTestSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTestSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
