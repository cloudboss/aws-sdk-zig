...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTestSetDiscrepancyReportInput, options: Options) !CreateTestSetDiscrepancyReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTestSetDiscrepancyReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/testsetdiscrepancy");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTestSetDiscrepancyReportOutput {
    var result: CreateTestSetDiscrepancyReportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTestSetDiscrepancyReportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
