...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBotVersionInput, options: Options) !CreateBotVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBotVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"botVersionLocaleSpecification\":");
    try aws.json.writeValue(@TypeOf(input.bot_version_locale_specification), input.bot_version_locale_specification, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBotVersionOutput {
    var result: CreateBotVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBotVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
