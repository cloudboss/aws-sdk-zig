...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopBotRecommendationInput, options: Options) !StopBotRecommendationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopBotRecommendationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/stopbotrecommendation");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopBotRecommendationOutput {
    var result: StopBotRecommendationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopBotRecommendationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
