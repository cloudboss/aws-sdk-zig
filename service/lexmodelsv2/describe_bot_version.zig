...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBotVersionInput, options: Options) !DescribeBotVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBotVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBotVersionOutput {
    var result: DescribeBotVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBotVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
