...
pub const ListCustomVocabularyItemsInput = struct {
...
pub const ListCustomVocabularyItemsOutput = struct {
    ...
    /// response.
    custom_vocabulary_items: ?[]const CustomVocabularyItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomVocabularyItemsInput, options: CallOptions) !ListCustomVocabularyItemsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomVocabularyItemsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/customvocabulary/DEFAULT/list");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomVocabularyItemsOutput {
    var result: ListCustomVocabularyItemsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomVocabularyItemsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
