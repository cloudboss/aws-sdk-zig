...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCustomVocabularyItemsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/customvocabulary/DEFAULT/list");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCustomVocabularyItemsOutput {
    var result: ListCustomVocabularyItemsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomVocabularyItemsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
