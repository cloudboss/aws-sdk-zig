...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUploadUrlInput, options: Options) !CreateUploadUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUploadUrlInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUploadUrlOutput {
    var result: CreateUploadUrlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateUploadUrlOutput, body, alloc);
    }
    _ = status;
    _ = headers;
