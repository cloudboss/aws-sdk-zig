...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBotLocaleInput, options: Options) !DescribeBotLocaleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBotLocaleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBotLocaleOutput {
    var result: DescribeBotLocaleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBotLocaleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
