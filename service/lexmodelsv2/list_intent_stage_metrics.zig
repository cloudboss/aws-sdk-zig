...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIntentStageMetricsInput, options: CallOptions) !ListIntentStageMetricsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListIntentStageMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/analytics/intentstagemetrics");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListIntentStageMetricsOutput {
    var result: ListIntentStageMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIntentStageMetricsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
