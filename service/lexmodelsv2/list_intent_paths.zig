...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIntentPathsInput, options: Options) !ListIntentPathsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIntentPathsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/analytics/intentpaths");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"intentPath\":");
    try aws.json.writeValue(@TypeOf(input.intent_path), input.intent_path, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIntentPathsOutput {
    var result: ListIntentPathsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIntentPathsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
