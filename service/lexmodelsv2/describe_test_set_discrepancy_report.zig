...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTestSetDiscrepancyReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.test_set_discrepancy_report_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTestSetDiscrepancyReportOutput {
    var result: DescribeTestSetDiscrepancyReportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTestSetDiscrepancyReportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
