...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSlotInput, options: Options) !DescribeSlotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSlotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSlotOutput {
    var result: DescribeSlotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSlotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
