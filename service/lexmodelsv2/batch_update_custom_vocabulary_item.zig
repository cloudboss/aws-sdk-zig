...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateCustomVocabularyItemInput, options: Options) !BatchUpdateCustomVocabularyItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateCustomVocabularyItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/customvocabulary/DEFAULT/batchupdate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateCustomVocabularyItemOutput {
    var result: BatchUpdateCustomVocabularyItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateCustomVocabularyItemOutput, body, alloc);
    }
    _ = status;
    _ = headers;
