...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIntentInput, options: CallOptions) !CreateIntentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateIntentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateIntentOutput {
    var result: CreateIntentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIntentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
