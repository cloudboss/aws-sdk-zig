...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBotAliasesInput, options: Options) !ListBotAliasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBotAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBotAliasesOutput {
    var result: ListBotAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBotAliasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
