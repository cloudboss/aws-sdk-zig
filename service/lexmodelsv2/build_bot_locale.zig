...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BuildBotLocaleInput, options: Options) !BuildBotLocaleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BuildBotLocaleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BuildBotLocaleOutput {
    var result: BuildBotLocaleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BuildBotLocaleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
