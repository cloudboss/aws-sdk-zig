...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTestSetInput, options: Options) !DescribeTestSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTestSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTestSetOutput {
    var result: DescribeTestSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTestSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
