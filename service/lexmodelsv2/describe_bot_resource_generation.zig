...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBotResourceGenerationInput, options: Options) !DescribeBotResourceGenerationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBotResourceGenerationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.generation_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBotResourceGenerationOutput {
    var result: DescribeBotResourceGenerationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBotResourceGenerationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
