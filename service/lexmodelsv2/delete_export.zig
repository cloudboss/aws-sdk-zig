...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteExportInput, options: Options) !DeleteExportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.export_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteExportOutput {
    var result: DeleteExportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteExportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
