...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIntentMetricsInput, options: Options) !ListIntentMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIntentMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/analytics/intentmetrics");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIntentMetricsOutput {
    var result: ListIntentMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIntentMetricsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
