...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSessionAnalyticsDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/analytics/sessions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSessionAnalyticsDataOutput {
    var result: ListSessionAnalyticsDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSessionAnalyticsDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
