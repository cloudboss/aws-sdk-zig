...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBotRecommendationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBotRecommendationOutput {
    var result: DescribeBotRecommendationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBotRecommendationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
