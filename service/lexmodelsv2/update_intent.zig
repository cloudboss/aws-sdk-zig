...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIntentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.slot_priorities) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"slotPriorities\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIntentOutput {
    var result: UpdateIntentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIntentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
