...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBuiltInIntentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBuiltInIntentsOutput {
    var result: ListBuiltInIntentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBuiltInIntentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
