...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUtterancesInput, options: Options) !DeleteUtterancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteUtterancesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.locale_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "localeId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteUtterancesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
