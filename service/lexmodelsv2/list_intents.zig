...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIntentsInput, options: Options) !ListIntentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIntentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIntentsOutput {
    var result: ListIntentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIntentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
