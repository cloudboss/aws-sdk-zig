...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ScanSbomInput, options: Options) !ScanSbomOutput {
    ...
    try aws.signing.signRequest(alloc, &request, creds, client.config.region, "inspectorscan");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ScanSbomInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("inspectorscan", "Inspector Scan", allocator);
