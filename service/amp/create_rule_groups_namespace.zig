...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRuleGroupsNamespaceInput, options: Options) !CreateRuleGroupsNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRuleGroupsNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRuleGroupsNamespaceOutput {
    var result: CreateRuleGroupsNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRuleGroupsNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
