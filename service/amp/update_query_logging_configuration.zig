...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQueryLoggingConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQueryLoggingConfigurationOutput {
    var result: UpdateQueryLoggingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQueryLoggingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
