...
pub const UpdateQueryLoggingConfigurationInput = struct {
...
pub const UpdateQueryLoggingConfigurationOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateQueryLoggingConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateQueryLoggingConfigurationOutput {
    var result: UpdateQueryLoggingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQueryLoggingConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
