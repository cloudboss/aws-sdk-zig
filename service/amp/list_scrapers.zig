...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListScrapersInput, options: Options) !ListScrapersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListScrapersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListScrapersOutput {
    var result: ListScrapersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListScrapersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
