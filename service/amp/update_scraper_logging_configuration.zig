...
pub const UpdateScraperLoggingConfigurationInput = struct {
    /// The destination where scraper logs will be sent.
    logging_destination: ScraperLoggingDestination,
...
pub const UpdateScraperLoggingConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateScraperLoggingConfigurationInput, options: CallOptions) !UpdateScraperLoggingConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateScraperLoggingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"loggingDestination\":");
    try aws.json.writeValue(@TypeOf(input.logging_destination), input.logging_destination, allocator, &body_buf);
    has_prev = true;
    if (input.scraper_components) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"scraperComponents\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateScraperLoggingConfigurationOutput {
    var result: UpdateScraperLoggingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateScraperLoggingConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
