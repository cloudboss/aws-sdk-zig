...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAlertManagerDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAlertManagerDefinitionOutput {
    var result: CreateAlertManagerDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAlertManagerDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
