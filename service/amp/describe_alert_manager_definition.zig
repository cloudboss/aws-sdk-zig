...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAlertManagerDefinitionInput, options: Options) !DescribeAlertManagerDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAlertManagerDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/alertmanager/definition");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAlertManagerDefinitionOutput {
    var result: DescribeAlertManagerDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAlertManagerDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
