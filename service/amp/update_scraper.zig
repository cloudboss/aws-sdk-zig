...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateScraperInput, options: Options) !UpdateScraperOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateScraperInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateScraperOutput {
    var result: UpdateScraperOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateScraperOutput, body, alloc);
    }
    _ = status;
    _ = headers;
