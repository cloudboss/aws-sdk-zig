...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspaceConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.limits_per_label_set) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"limitsPerLabelSet\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.retention_period_in_days) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"retentionPeriodInDays\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspaceConfigurationOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateWorkspaceConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
