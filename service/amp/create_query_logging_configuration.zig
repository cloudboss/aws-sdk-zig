...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateQueryLoggingConfigurationInput, options: Options) !CreateQueryLoggingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateQueryLoggingConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateQueryLoggingConfigurationOutput {
    var result: CreateQueryLoggingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateQueryLoggingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
