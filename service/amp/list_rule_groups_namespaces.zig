...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRuleGroupsNamespacesInput, options: Options) !ListRuleGroupsNamespacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRuleGroupsNamespacesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRuleGroupsNamespacesOutput {
    var result: ListRuleGroupsNamespacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRuleGroupsNamespacesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
