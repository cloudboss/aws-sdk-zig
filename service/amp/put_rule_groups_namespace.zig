...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRuleGroupsNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRuleGroupsNamespaceOutput {
    var result: PutRuleGroupsNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutRuleGroupsNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
