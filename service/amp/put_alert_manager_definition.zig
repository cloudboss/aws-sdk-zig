...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAlertManagerDefinitionInput, options: Options) !PutAlertManagerDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAlertManagerDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAlertManagerDefinitionOutput {
    var result: PutAlertManagerDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutAlertManagerDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
