...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnomalyDetectorsInput, options: Options) !ListAnomalyDetectorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAnomalyDetectorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAnomalyDetectorsOutput {
    var result: ListAnomalyDetectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAnomalyDetectorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
