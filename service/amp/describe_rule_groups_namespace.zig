...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRuleGroupsNamespaceInput, options: Options) !DescribeRuleGroupsNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRuleGroupsNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRuleGroupsNamespaceOutput {
    var result: DescribeRuleGroupsNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRuleGroupsNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
