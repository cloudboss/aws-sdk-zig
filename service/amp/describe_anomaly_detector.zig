...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAnomalyDetectorInput, options: Options) !DescribeAnomalyDetectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAnomalyDetectorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAnomalyDetectorOutput {
    var result: DescribeAnomalyDetectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAnomalyDetectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
