...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAnomalyDetectorInput, options: Options) !CreateAnomalyDetectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAnomalyDetectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.alias), input.alias, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAnomalyDetectorOutput {
    var result: CreateAnomalyDetectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAnomalyDetectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
