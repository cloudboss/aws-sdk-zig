...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetAutomationRulesInput, options: Options) !BatchGetAutomationRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetAutomationRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetAutomationRulesOutput {
    var result: BatchGetAutomationRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetAutomationRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
