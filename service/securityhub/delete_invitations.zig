...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteInvitationsInput, options: Options) !DeleteInvitationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteInvitationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteInvitationsOutput {
    var result: DeleteInvitationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteInvitationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
