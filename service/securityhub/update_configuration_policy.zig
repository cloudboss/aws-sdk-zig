...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfigurationPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/configurationPolicy/");
    ...
    if (input.configuration_policy) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ConfigurationPolicy\":");
    ...
    if (input.updated_reason) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UpdatedReason\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfigurationPolicyOutput {
    var result: UpdateConfigurationPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConfigurationPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
