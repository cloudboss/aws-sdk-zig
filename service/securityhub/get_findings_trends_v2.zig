...
const FindingsTrendsFilters = @import("findings_trends_filters.zig").FindingsTrendsFilters;
const GranularityField = @import("granularity_field.zig").GranularityField;
const TrendsMetricsResult = @import("trends_metrics_result.zig").TrendsMetricsResult;

pub const GetFindingsTrendsV2Input = struct {
    ...
    filters: ?FindingsTrendsFilters = null,
    ...
    /// The starting timestamp for the time period to analyze findings trends, in
    /// ISO 8601 format.
    start_time: i64,
...
pub const GetFindingsTrendsV2Output = struct {
    ...
    trends_metrics: ?[]const TrendsMetricsResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingsTrendsV2Input, options: CallOptions) !GetFindingsTrendsV2Output {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFindingsTrendsV2Input, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("securityhub", "SecurityHub", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/findingsTrendsv2";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFindingsTrendsV2Output {
    var result: GetFindingsTrendsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingsTrendsV2Output, body, allocator);
    }
    _ = status;
    _ = headers;
