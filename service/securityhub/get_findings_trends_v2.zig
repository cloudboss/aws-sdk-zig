...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingsTrendsV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingsTrendsV2Output {
    var result: GetFindingsTrendsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingsTrendsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
