...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeclineInvitationsInput, options: Options) !DeclineInvitationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeclineInvitationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeclineInvitationsOutput {
    var result: DeclineInvitationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeclineInvitationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
