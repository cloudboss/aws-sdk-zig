...
pub const DeclineInvitationsOutput = struct {
    ...
    /// account, the list includes
    /// the account ID and the email address.
    unprocessed_accounts: ?[]const Result = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeclineInvitationsInput, options: CallOptions) !DeclineInvitationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeclineInvitationsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/invitations/decline";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeclineInvitationsOutput {
    var result: DeclineInvitationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeclineInvitationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
