...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourcesTrendsV2Input, options: Options) !GetResourcesTrendsV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourcesTrendsV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourcesTrendsV2Output {
    var result: GetResourcesTrendsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourcesTrendsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
