...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetConfigurationPolicyAssociationsInput, options: Options) !BatchGetConfigurationPolicyAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetConfigurationPolicyAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ConfigurationPolicyAssociationIdentifiers\":");
    try aws.json.writeValue(@TypeOf(input.configuration_policy_association_identifiers), input.configuration_policy_association_identifiers, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetConfigurationPolicyAssociationsOutput {
    var result: BatchGetConfigurationPolicyAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetConfigurationPolicyAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
