...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMasterAccountInput, options: CallOptions) !GetMasterAccountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMasterAccountInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMasterAccountOutput {
    var result: GetMasterAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMasterAccountOutput, body, allocator);
    }
    _ = status;
    _ = headers;
