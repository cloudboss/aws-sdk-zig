...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingHistoryInput, options: Options) !GetFindingHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FindingIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.finding_identifier), input.finding_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingHistoryOutput {
    var result: GetFindingHistoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingHistoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
