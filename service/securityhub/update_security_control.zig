...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSecurityControlInput, options: Options) !UpdateSecurityControlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSecurityControlInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.last_update_reason) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"LastUpdateReason\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SecurityControlId\":");
    try aws.json.writeValue(@TypeOf(input.security_control_id), input.security_control_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSecurityControlOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
