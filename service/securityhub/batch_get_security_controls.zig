...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetSecurityControlsInput, options: Options) !BatchGetSecurityControlsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetSecurityControlsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"SecurityControlIds\":");
    try aws.json.writeValue(@TypeOf(input.security_control_ids), input.security_control_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetSecurityControlsOutput {
    var result: BatchGetSecurityControlsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetSecurityControlsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
