...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableOrganizationAdminAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AdminAccountId\":");
    try aws.json.writeValue(@TypeOf(input.admin_account_id), input.admin_account_id, alloc, &body_buf);
    has_prev = true;
    if (input.feature) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Feature\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableOrganizationAdminAccountOutput {
    var result: EnableOrganizationAdminAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EnableOrganizationAdminAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
