...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStandardsInput, options: Options) !DescribeStandardsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStandardsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStandardsOutput {
    var result: DescribeStandardsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeStandardsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
