...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInsightResultsInput, options: Options) !GetInsightResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInsightResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/insights/results/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInsightResultsOutput {
    var result: GetInsightResultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInsightResultsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
