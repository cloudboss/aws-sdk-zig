...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateStandardsControlAssociationsInput, options: Options) !BatchUpdateStandardsControlAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateStandardsControlAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"StandardsControlAssociationUpdates\":");
    try aws.json.writeValue(@TypeOf(input.standards_control_association_updates), input.standards_control_association_updates, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateStandardsControlAssociationsOutput {
    var result: BatchUpdateStandardsControlAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateStandardsControlAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
