...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConnectorV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConnectorV2Output {
    var result: GetConnectorV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConnectorV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
