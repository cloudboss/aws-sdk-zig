...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableSecurityHubInput, options: Options) !EnableSecurityHubOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableSecurityHubInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.control_finding_generator) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ControlFindingGenerator\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.enable_default_standards) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EnableDefaultStandards\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableSecurityHubOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
