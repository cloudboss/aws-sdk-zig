...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConfigurationPoliciesInput, options: Options) !ListConfigurationPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConfigurationPoliciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConfigurationPoliciesOutput {
    var result: ListConfigurationPoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConfigurationPoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
