...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInsightsInput, options: Options) !GetInsightsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInsightsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.insight_arns) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"InsightArns\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInsightsOutput {
    var result: GetInsightsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInsightsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
