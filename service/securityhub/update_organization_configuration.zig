...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOrganizationConfigurationInput, options: Options) !UpdateOrganizationConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateOrganizationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AutoEnable\":");
    try aws.json.writeValue(@TypeOf(input.auto_enable), input.auto_enable, alloc, &body_buf);
    has_prev = true;
    if (input.auto_enable_standards) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"AutoEnableStandards\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.organization_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OrganizationConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateOrganizationConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
