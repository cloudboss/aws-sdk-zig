...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetStandardsControlAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"StandardsControlAssociationIds\":");
    try aws.json.writeValue(@TypeOf(input.standards_control_association_ids), input.standards_control_association_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetStandardsControlAssociationsOutput {
    var result: BatchGetStandardsControlAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetStandardsControlAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
