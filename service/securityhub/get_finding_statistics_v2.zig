...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingStatisticsV2Input, options: Options) !GetFindingStatisticsV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingStatisticsV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingStatisticsV2Output {
    var result: GetFindingStatisticsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingStatisticsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
