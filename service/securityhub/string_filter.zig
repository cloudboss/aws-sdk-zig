...
pub const StringFilter = struct {
    ...
    ///   For example,
    /// the filter `AwsAccountId EQUALS 123456789012` only matches findings that
    /// have an account ID of
    /// `123456789012`.
    ///
    /// * To search for values that start with the filter value, use `PREFIX`. For
    ///   example, the
    /// filter `ResourceRegion PREFIX us` matches findings that have a
    /// `ResourceRegion` that starts
    /// with `us`. A `ResourceRegion` that starts with a different value, such as
    /// `af`,
    /// `ap`, or `ca`, doesn't match.
    ///
    /// `CONTAINS`, `EQUALS`, and `PREFIX` filters on the same field are joined by
    /// `OR`. A finding matches if it matches any one of those filters. For example,
    /// the filters `Title CONTAINS CloudFront OR
    ...
    ///   For example, the
    /// filter `Title NOT_CONTAINS CloudFront` matches findings that have a `Title`
    /// that
    ...
    /// findings that have an account
    /// ID other than `123456789012`.
    ///
    /// * To search for values that don't start with the filter value, use
    ///   `PREFIX_NOT_EQUALS`. For
    /// example, the filter `ResourceRegion PREFIX_NOT_EQUALS us` matches findings
    /// with a
    /// `ResourceRegion` that starts with a value other than `us`.
    ///
    /// `NOT_CONTAINS`, `NOT_EQUALS`, and `PREFIX_NOT_EQUALS` filters on the same
    /// field
    /// are joined by `AND`. A finding matches only if it matches all of those
    /// filters. For example, the filters `Title NOT_CONTAINS CloudFront AND
    /// Title NOT_CONTAINS CloudWatch` match a finding that excludes both
    /// `CloudFront` and
    /// `CloudWatch` in the title.
    ///
    /// You can’t have both a `CONTAINS` filter and a `NOT_CONTAINS` filter on the
    /// same field. Similarly,
    ...
    /// returns an error. `CONTAINS` filters
    /// can only be used with other `CONTAINS` filters. `NOT_CONTAINS` filters can
    /// only be used with
