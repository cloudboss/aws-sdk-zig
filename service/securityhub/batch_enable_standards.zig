...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchEnableStandardsInput, options: Options) !BatchEnableStandardsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchEnableStandardsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"StandardsSubscriptionRequests\":");
    try aws.json.writeValue(@TypeOf(input.standards_subscription_requests), input.standards_subscription_requests, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchEnableStandardsOutput {
    var result: BatchEnableStandardsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchEnableStandardsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
