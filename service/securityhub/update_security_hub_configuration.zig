...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSecurityHubConfigurationInput, options: Options) !UpdateSecurityHubConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSecurityHubConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.auto_enable_controls) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AutoEnableControls\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSecurityHubConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
