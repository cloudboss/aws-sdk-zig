...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InviteMembersInput, options: Options) !InviteMembersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InviteMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InviteMembersOutput {
    var result: InviteMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(InviteMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
