...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTicketV2Input, options: Options) !CreateTicketV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTicketV2Input, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FindingMetadataUid\":");
    try aws.json.writeValue(@TypeOf(input.finding_metadata_uid), input.finding_metadata_uid, alloc, &body_buf);
    has_prev = true;
    if (input.mode) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTicketV2Output {
    var result: CreateTicketV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTicketV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
