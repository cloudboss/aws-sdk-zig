...
pub const GetEnabledStandardsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEnabledStandardsInput, options: Options) !GetEnabledStandardsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEnabledStandardsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.standards_subscription_arns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"StandardsSubscriptionArns\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEnabledStandardsOutput {
    var result: GetEnabledStandardsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEnabledStandardsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
