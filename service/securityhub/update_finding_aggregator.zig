...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFindingAggregatorInput, options: Options) !UpdateFindingAggregatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFindingAggregatorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"FindingAggregatorArn\":");
    try aws.json.writeValue(@TypeOf(input.finding_aggregator_arn), input.finding_aggregator_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RegionLinkingMode\":");
    try aws.json.writeValue(@TypeOf(input.region_linking_mode), input.region_linking_mode, alloc, &body_buf);
    has_prev = true;
    if (input.regions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFindingAggregatorOutput {
    var result: UpdateFindingAggregatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFindingAggregatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
