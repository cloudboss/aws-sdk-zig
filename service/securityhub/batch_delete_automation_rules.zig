...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteAutomationRulesInput, options: Options) !BatchDeleteAutomationRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteAutomationRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AutomationRulesArns\":");
    try aws.json.writeValue(@TypeOf(input.automation_rules_arns), input.automation_rules_arns, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteAutomationRulesOutput {
    var result: BatchDeleteAutomationRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteAutomationRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
