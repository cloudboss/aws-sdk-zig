...
pub const FirewallPolicyDetails = struct {
    ...
    stateful_rule_group_references: ?[]const FirewallPolicyStatefulRuleGroupReferencesDetails,

    /// The custom action definitions that are available to use in the firewall
    /// policy's `StatelessDefaultActions` setting.
    stateless_custom_actions: ?[]const FirewallPolicyStatelessCustomActionsDetails,
    ...
    stateless_rule_group_references: ?[]const FirewallPolicyStatelessRuleGroupReferencesDetails,
