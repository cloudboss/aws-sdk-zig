...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFindingsInput, options: Options) !UpdateFindingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFindingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.note) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Note\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.record_state) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecordState\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFindingsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
