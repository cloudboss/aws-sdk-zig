...
pub const MapFilter = struct {
    ...
    ///   example, for the
    /// `ResourceTags` field, the filter `Department CONTAINS Security` matches
    /// findings that
    /// include the value `Security` for the `Department` tag. In the same example,
    /// a finding with a value of
    ...
    /// finding matches if it matches any one of those filters. For example, the
    /// filters `Department CONTAINS Security OR
    /// Department CONTAINS Finance` match a finding that includes either
    /// `Security`,
    ...
    ///   example, for the
    /// `ResourceTags` field, the filter `Department NOT_EQUALS Finance` matches
    /// findings that
    ...
    /// filters `Department NOT_CONTAINS Security AND
    ...
    /// same field.
    /// Similarly, you can’t have both an `EQUALS` filter and a `NOT_EQUALS` filter
    /// on the same field.
