...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAggregatorsV2Input, options: Options) !ListAggregatorsV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAggregatorsV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAggregatorsV2Output {
    var result: ListAggregatorsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAggregatorsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
