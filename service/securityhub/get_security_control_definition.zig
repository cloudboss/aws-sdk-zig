...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSecurityControlDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSecurityControlDefinitionOutput {
    var result: GetSecurityControlDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSecurityControlDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
