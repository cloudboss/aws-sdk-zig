...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConnectorsV2Input, options: Options) !ListConnectorsV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConnectorsV2Input, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connector_status) |v| {
        ...
        try query_buf.appendSlice(alloc, "ConnectorStatus=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConnectorsV2Output {
    var result: ListConnectorsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConnectorsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
