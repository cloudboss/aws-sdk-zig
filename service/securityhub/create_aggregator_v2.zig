...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAggregatorV2Input, options: Options) !CreateAggregatorV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAggregatorV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAggregatorV2Output {
    var result: CreateAggregatorV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAggregatorV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
