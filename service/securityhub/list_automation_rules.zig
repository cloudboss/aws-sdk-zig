...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutomationRulesInput, options: Options) !ListAutomationRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutomationRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutomationRulesOutput {
    var result: ListAutomationRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutomationRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
