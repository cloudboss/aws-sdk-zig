...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStandardsControlInput, options: Options) !UpdateStandardsControlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStandardsControlInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/standards/control/");
    try path_buf.appendSlice(alloc, input.standards_control_arn);
    ...
    if (input.disabled_reason) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DisabledReason\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStandardsControlOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
