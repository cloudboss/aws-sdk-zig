...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateFindingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.confidence) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"Confidence\":");
    ...
    try aws.json.writeValue(@TypeOf(input.finding_identifiers), input.finding_identifiers, alloc, &body_buf);
    has_prev = true;
    if (input.note) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.related_findings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RelatedFindings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.severity) |v| {
    ...
    if (input.user_defined_fields) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UserDefinedFields\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.workflow) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Workflow\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateFindingsOutput {
    var result: BatchUpdateFindingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateFindingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
