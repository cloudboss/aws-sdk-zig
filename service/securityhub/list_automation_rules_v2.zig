...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutomationRulesV2Input, options: Options) !ListAutomationRulesV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutomationRulesV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutomationRulesV2Output {
    var result: ListAutomationRulesV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutomationRulesV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
