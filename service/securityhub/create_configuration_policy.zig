...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConfigurationPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.configuration_policy), input.configuration_policy, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConfigurationPolicyOutput {
    var result: CreateConfigurationPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConfigurationPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
