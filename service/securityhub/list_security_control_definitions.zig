...
pub const ListSecurityControlDefinitionsInput = struct {
    ...
    /// results include the
    /// first 25 security controls that apply to the specified standard. The results
    /// also include a
    /// `NextToken` parameter that you can use in a subsequent API call to get the
    /// next 25 controls. This repeats until all controls for the standard are
    /// returned.
...
pub const ListSecurityControlDefinitionsOutput = struct {
    ...
    security_control_definitions: ?[]const SecurityControlDefinition = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .security_control_definitions = "SecurityControlDefinitions",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecurityControlDefinitionsInput, options: Options) !ListSecurityControlDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityControlDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/securityControls/definitions";
    ...
    if (input.standards_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "StandardsArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecurityControlDefinitionsOutput {
    var result: ListSecurityControlDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityControlDefinitionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
