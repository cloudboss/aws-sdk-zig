...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMembersInput, options: Options) !GetMembersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMembersOutput {
    var result: GetMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
