...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSecurityHubV2Input, options: Options) !DescribeSecurityHubV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSecurityHubV2Input, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSecurityHubV2Output {
    var result: DescribeSecurityHubV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSecurityHubV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
