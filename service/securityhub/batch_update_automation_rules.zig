...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateAutomationRulesInput, options: Options) !BatchUpdateAutomationRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateAutomationRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"UpdateAutomationRulesRequestItems\":");
    try aws.json.writeValue(@TypeOf(input.update_automation_rules_request_items), input.update_automation_rules_request_items, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateAutomationRulesOutput {
    var result: BatchUpdateAutomationRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateAutomationRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
