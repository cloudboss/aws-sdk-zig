...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingAggregatorInput, options: Options) !GetFindingAggregatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingAggregatorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/findingAggregator/get/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingAggregatorOutput {
    var result: GetFindingAggregatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingAggregatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
