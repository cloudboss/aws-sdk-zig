...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingsV2Input, options: Options) !GetFindingsV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingsV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingsV2Output {
    var result: GetFindingsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
