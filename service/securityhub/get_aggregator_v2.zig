...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAggregatorV2Input, options: Options) !GetAggregatorV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAggregatorV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/aggregatorv2/get/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAggregatorV2Output {
    var result: GetAggregatorV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAggregatorV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
