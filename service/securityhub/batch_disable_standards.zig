...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDisableStandardsInput, options: Options) !BatchDisableStandardsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDisableStandardsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.standards_subscription_arns), input.standards_subscription_arns, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDisableStandardsOutput {
    var result: BatchDisableStandardsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDisableStandardsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
