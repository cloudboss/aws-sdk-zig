...
pub const DescribeActionTargetsInput = struct {
    /// A list of custom action target ARNs for the custom action targets to
    /// retrieve.
    action_target_arns: ?[]const []const u8 = null,
    ...
    pub const json_field_names = .{
        .action_target_arns = "ActionTargetArns",
        .max_results = "MaxResults",
        .next_token = "NextToken",
    };
};

pub const DescribeActionTargetsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeActionTargetsInput, options: Options) !DescribeActionTargetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeActionTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/actionTargets/get";

    var body_buf: std.ArrayList(u8) = .{};
    var has_prev = false;
    try body_buf.appendSlice(alloc, "{");

    if (input.action_target_arns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ActionTargetArns\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeActionTargetsOutput {
    var result: DescribeActionTargetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeActionTargetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
