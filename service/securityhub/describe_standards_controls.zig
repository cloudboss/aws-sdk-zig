...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStandardsControlsInput, options: Options) !DescribeStandardsControlsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStandardsControlsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/standards/controls/");
    try path_buf.appendSlice(alloc, input.standards_subscription_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStandardsControlsOutput {
    var result: DescribeStandardsControlsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeStandardsControlsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
