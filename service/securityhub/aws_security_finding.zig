...
pub const AwsSecurityFinding = struct {
    ...
    ///
    /// Security Hub CSPM populates this attribute automatically for each finding.
    ...
    ///
    /// When you use the Security Hub CSPM console or API to filter findings by
    ...
    /// A timestamp that indicates when Security Hub CSPM received a finding and
    ...
    /// The ARN generated by Security Hub CSPM that uniquely identifies a product
    /// that generates findings.
    /// This can be the ARN for a third-party product that is integrated with
    /// Security Hub CSPM, or the ARN for
