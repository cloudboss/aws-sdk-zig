...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAggregatorV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/aggregatorv2/update/");
    ...
    if (input.linked_regions) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"LinkedRegions\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAggregatorV2Output {
    var result: UpdateAggregatorV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAggregatorV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
