...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInsightInput, options: Options) !CreateInsightOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInsightInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"GroupByAttribute\":");
    try aws.json.writeValue(@TypeOf(input.group_by_attribute), input.group_by_attribute, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInsightOutput {
    var result: CreateInsightOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateInsightOutput, body, alloc);
    }
    _ = status;
    _ = headers;
