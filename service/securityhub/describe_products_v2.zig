...
const ProductV2 = @import("product_v2.zig").ProductV2;

pub const DescribeProductsV2Input = struct {
...
pub const DescribeProductsV2Output = struct {
    /// The pagination token to use to request the next page of results.
    /// Otherwise, this parameter is null.
    next_token: ?[]const u8 = null,

    /// Gets information about the product integration.
    products_v2: ?[]const ProductV2 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProductsV2Input, options: Options) !DescribeProductsV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProductsV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProductsV2Output {
    var result: DescribeProductsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeProductsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
