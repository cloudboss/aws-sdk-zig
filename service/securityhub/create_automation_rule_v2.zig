...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAutomationRuleV2Input, options: Options) !CreateAutomationRuleV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAutomationRuleV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAutomationRuleV2Output {
    var result: CreateAutomationRuleV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAutomationRuleV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
