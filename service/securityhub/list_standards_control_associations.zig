...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStandardsControlAssociationsInput, options: Options) !ListStandardsControlAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStandardsControlAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "SecurityControlId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.security_control_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStandardsControlAssociationsOutput {
    var result: ListStandardsControlAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStandardsControlAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
