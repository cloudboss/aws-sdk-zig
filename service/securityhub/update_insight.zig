...
pub const UpdateInsightInput = struct {
    /// The updated filters that define this insight.
    filters: ?AwsSecurityFindingFilters = null,

    /// The updated `GroupBy` attribute that defines this insight.
    group_by_attribute: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateInsightInput, options: Options) !UpdateInsightOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateInsightInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/insights/");
    try path_buf.appendSlice(alloc, input.insight_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateInsightOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateInsightOutput = .{};
