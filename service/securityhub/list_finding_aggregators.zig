...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFindingAggregatorsInput, options: Options) !ListFindingAggregatorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFindingAggregatorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFindingAggregatorsOutput {
    var result: ListFindingAggregatorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFindingAggregatorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
