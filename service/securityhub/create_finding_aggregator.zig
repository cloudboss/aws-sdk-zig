...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFindingAggregatorInput, options: Options) !CreateFindingAggregatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFindingAggregatorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFindingAggregatorOutput {
    var result: CreateFindingAggregatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFindingAggregatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
