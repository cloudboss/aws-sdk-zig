...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConnectorV2Input, options: Options) !CreateConnectorV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConnectorV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.provider), input.provider, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConnectorV2Output {
    var result: CreateConnectorV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConnectorV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
