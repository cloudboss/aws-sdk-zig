...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConnectorV2Input, options: Options) !DeleteConnectorV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteConnectorV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/connectorsv2/");
    try path_buf.appendSlice(alloc, input.connector_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteConnectorV2Output {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
