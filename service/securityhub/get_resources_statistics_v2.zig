...
pub const GetResourcesStatisticsV2Input = struct {
    ...
    /// see only statistics for their own resources.
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourcesStatisticsV2Input, options: CallOptions) !GetResourcesStatisticsV2Output {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResourcesStatisticsV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResourcesStatisticsV2Output {
    var result: GetResourcesStatisticsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourcesStatisticsV2Output, body, allocator);
    }
    _ = status;
    _ = headers;
