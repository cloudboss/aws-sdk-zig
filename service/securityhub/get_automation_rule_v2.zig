...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutomationRuleV2Input, options: Options) !GetAutomationRuleV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAutomationRuleV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/automationrulesv2/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAutomationRuleV2Output {
    var result: GetAutomationRuleV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAutomationRuleV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
