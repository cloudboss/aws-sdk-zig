...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfigurationPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/configurationPolicy/get/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfigurationPolicyOutput {
    var result: GetConfigurationPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfigurationPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
