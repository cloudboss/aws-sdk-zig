...
pub const RejectAssignmentInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectAssignmentInput, options: Options) !RejectAssignmentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RejectAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MTurkRequesterServiceV20170117.RejectAssignment");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RejectAssignmentOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
