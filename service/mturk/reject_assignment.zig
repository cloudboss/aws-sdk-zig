...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.RejectAssignment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectAssignmentOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
