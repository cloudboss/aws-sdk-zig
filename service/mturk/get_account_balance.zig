...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountBalanceInput, options: CallOptions) !GetAccountBalanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAccountBalanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MTurkRequesterServiceV20170117.GetAccountBalance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAccountBalanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAccountBalanceOutput, body, allocator);
}
