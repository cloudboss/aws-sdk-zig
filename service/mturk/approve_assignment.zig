...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ApproveAssignmentInput, options: Options) !ApproveAssignmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ApproveAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.ApproveAssignment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ApproveAssignmentOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
