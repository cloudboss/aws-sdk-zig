...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptQualificationRequestInput, options: Options) !AcceptQualificationRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptQualificationRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.AcceptQualificationRequest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptQualificationRequestOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
