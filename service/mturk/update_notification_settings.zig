...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNotificationSettingsInput, options: Options) !UpdateNotificationSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNotificationSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.UpdateNotificationSettings");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNotificationSettingsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
