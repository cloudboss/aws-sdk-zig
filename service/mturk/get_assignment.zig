...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssignmentInput, options: Options) !GetAssignmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.GetAssignment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssignmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAssignmentOutput, body, alloc);
}
