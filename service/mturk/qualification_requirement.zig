const HITAccessActions = @import("hit_access_actions.zig").HITAccessActions;
...
/// A requirement may optionally state that a Worker must have the Qualification
/// in order to preview the HIT, or
/// see the HIT in search results.
pub const QualificationRequirement = struct {
    ...
    /// arguments include
    /// "Accept" (Worker cannot accept the HIT, but can preview the HIT and see it
    /// in their search results),
    /// "PreviewAndAccept" (Worker cannot accept or preview the HIT, but can see the
    /// HIT in their search results),
    /// and "DiscoverPreviewAndAccept" (Worker cannot accept, preview, or see the
    /// HIT in their search results). It's possible for you to create a HIT with
    /// multiple
    /// QualificationRequirements (which can have different values for the
    /// ActionGuarded attribute). In this case,
    /// the Worker is only permitted to perform an action when they have met all
    /// QualificationRequirements guarding
    /// the action. The actions in the order of least restrictive to most
    /// restrictive are Discover, Preview and Accept.
    /// For example, if a Worker meets all QualificationRequirements that are set to
    /// DiscoverPreviewAndAccept, but do
    /// not meet all requirements that are set with PreviewAndAccept, then the
    /// Worker will be able to Discover, i.e. see the
    ...
    /// If RequiredToPreview is true, the question data for the HIT will not be
    /// shown
