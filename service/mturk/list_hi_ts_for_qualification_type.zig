...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHITsForQualificationTypeInput, options: Options) !ListHITsForQualificationTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHITsForQualificationTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.ListHITsForQualificationType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHITsForQualificationTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListHITsForQualificationTypeOutput, body, alloc);
}
