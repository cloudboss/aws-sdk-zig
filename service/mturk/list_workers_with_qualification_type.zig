...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkersWithQualificationTypeInput, options: Options) !ListWorkersWithQualificationTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkersWithQualificationTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.ListWorkersWithQualificationType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkersWithQualificationTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListWorkersWithQualificationTypeOutput, body, alloc);
}
