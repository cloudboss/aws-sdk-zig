...
pub const ListQualificationTypesInput = struct {
    ...
    /// Qualification types.
    must_be_owned_by_caller: ?bool = null,
...
pub const ListQualificationTypesOutput = struct {
    next_token: ?[]const u8 = null,
    ...
    /// query.
    qualification_types: ?[]const QualificationType = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQualificationTypesInput, options: CallOptions) !ListQualificationTypesOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "mturk-requester");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListQualificationTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MTurkRequesterServiceV20170117.ListQualificationTypes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListQualificationTypesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListQualificationTypesOutput, body, allocator);
}
