...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectQualificationRequestInput, options: Options) !RejectQualificationRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectQualificationRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.RejectQualificationRequest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectQualificationRequestOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
