...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendTestEventNotificationInput, options: Options) !SendTestEventNotificationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendTestEventNotificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.SendTestEventNotification");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendTestEventNotificationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
