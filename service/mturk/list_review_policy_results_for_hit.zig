...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReviewPolicyResultsForHITInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.ListReviewPolicyResultsForHIT");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReviewPolicyResultsForHITOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListReviewPolicyResultsForHITOutput, body, alloc);
}
