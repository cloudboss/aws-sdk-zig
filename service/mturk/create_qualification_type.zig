...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateQualificationTypeInput, options: Options) !CreateQualificationTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateQualificationTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.CreateQualificationType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateQualificationTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateQualificationTypeOutput, body, alloc);
}
