...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQualificationScoreInput, options: Options) !GetQualificationScoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQualificationScoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.GetQualificationScore");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQualificationScoreOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetQualificationScoreOutput, body, alloc);
}
