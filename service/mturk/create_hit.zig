...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHITInput, options: Options) !CreateHITOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHITInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.CreateHIT");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHITOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateHITOutput, body, alloc);
}
