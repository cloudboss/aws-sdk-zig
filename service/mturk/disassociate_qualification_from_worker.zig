...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateQualificationFromWorkerInput, options: Options) !DisassociateQualificationFromWorkerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateQualificationFromWorkerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.DisassociateQualificationFromWorker");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateQualificationFromWorkerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
