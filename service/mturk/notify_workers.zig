...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: NotifyWorkersInput, options: Options) !NotifyWorkersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: NotifyWorkersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.NotifyWorkers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !NotifyWorkersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(NotifyWorkersOutput, body, alloc);
}
