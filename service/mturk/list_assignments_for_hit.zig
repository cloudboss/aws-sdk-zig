...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssignmentsForHITInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.ListAssignmentsForHIT");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssignmentsForHITOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssignmentsForHITOutput, body, alloc);
}
