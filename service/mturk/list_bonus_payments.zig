...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBonusPaymentsInput, options: Options) !ListBonusPaymentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBonusPaymentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.ListBonusPayments");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBonusPaymentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBonusPaymentsOutput, body, alloc);
}
