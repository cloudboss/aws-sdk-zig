...
pub const CreateAdditionalAssignmentsForHITInput = struct {
    ...
    /// or not
    /// the call succeeded on the server. If the extend HIT already exists in the
    /// system
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAdditionalAssignmentsForHITInput, options: Options) !CreateAdditionalAssignmentsForHITOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAdditionalAssignmentsForHITInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MTurkRequesterServiceV20170117.CreateAdditionalAssignmentsForHIT");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAdditionalAssignmentsForHITOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
