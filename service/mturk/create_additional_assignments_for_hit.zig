...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAdditionalAssignmentsForHITInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MTurkRequesterServiceV20170117.CreateAdditionalAssignmentsForHIT");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAdditionalAssignmentsForHITOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
