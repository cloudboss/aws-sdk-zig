...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateNotificationRuleInput, options: Options) !CreateNotificationRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNotificationRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.detail_type), input.detail_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EventTypeIds\":");
    try aws.json.writeValue(@TypeOf(input.event_type_ids), input.event_type_ids, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateNotificationRuleOutput {
    var result: CreateNotificationRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateNotificationRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
