...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListNotificationRulesInput, options: Options) !ListNotificationRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListNotificationRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListNotificationRulesOutput {
    var result: ListNotificationRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListNotificationRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
