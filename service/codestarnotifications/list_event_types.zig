...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEventTypesInput, options: Options) !ListEventTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventTypesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEventTypesOutput {
    var result: ListEventTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEventTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
