...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNotificationRuleInput, options: Options) !DescribeNotificationRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNotificationRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNotificationRuleOutput {
    var result: DescribeNotificationRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeNotificationRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
