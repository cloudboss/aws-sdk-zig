...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPersonalizedRankingInput, options: CallOptions) !GetPersonalizedRankingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPersonalizedRankingInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.campaign_arn), input.campaign_arn, allocator, &body_buf);
    has_prev = true;
    if (input.context) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"inputList\":");
    try aws.json.writeValue(@TypeOf(input.input_list), input.input_list, allocator, &body_buf);
    has_prev = true;
