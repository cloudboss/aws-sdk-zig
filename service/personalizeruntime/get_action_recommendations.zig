...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetActionRecommendationsInput, options: CallOptions) !GetActionRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetActionRecommendationsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("personalize-runtime", "Personalize Runtime", allocator);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetActionRecommendationsOutput {
    var result: GetActionRecommendationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetActionRecommendationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
