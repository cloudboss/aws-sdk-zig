...
pub const ProtocolDetails = struct {
    ...
    /// behind a firewall or NAT with Transfer
    /// Family](http://aws.amazon.com/blogs/storage/configuring-your-ftps-server-behind-a-firewall-or-nat-with-aws-transfer-family/).
    ///
    ...
    /// `TlsSessionResumptionMode = ENFORCED`. For optimal performance, migrate to
    /// VPC endpoints with Elastic IP addresses instead of using NLBs. For more
    /// details, see [ Avoid placing NLBs and NATs in front of Transfer
    /// Family](https://docs.aws.amazon.com/transfer/latest/userguide/infrastructure-security.html#nlb-considerations).
    ///
    /// *Special values*
    ///
    /// The `AUTO` and `0.0.0.0` are special values for the `PassiveIp` parameter.
    /// The value `PassiveIp=AUTO` is assigned by default to FTP and FTPS type
    ...
    /// Network Load Balancer (NLB) environment, where you have 3 subnets, you can
    /// only specify a single IP address using the `PassiveIp` parameter. This
    /// reduces the effectiveness of having High Availability. In this case, you can
    /// specify `PassiveIp=0.0.0.0`. This tells the client to use the same IP
    ...
    /// `SETSTAT` when uploading the file. However, these commands are not
    ...
    /// to your SFTP client. While the `SetStatOption` `ENABLE_NO_OP` setting
    /// ignores the error, it does generate a log entry in Amazon CloudWatch Logs,
    /// so you can determine when the client is making a `SETSTAT` call.
    ///
    ...
    /// `TlsSessionResumptionMode` determines whether or not the server resumes
    /// recent, negotiated sessions through a unique session ID. This property is
    /// available during `CreateServer` and `UpdateServer` calls. If a
    /// `TlsSessionResumptionMode` value is not specified during `CreateServer`, it
    ...
    ///   TLS session resumption. The server rejects client data connections that do
    ///   not perform the TLS session resumption client processing. Before you set
