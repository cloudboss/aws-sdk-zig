...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHostKeyInput, options: Options) !DescribeHostKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHostKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.DescribeHostKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHostKeyOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeHostKeyOutput, body, alloc);
}
