...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWebAppCustomizationInput, options: Options) !UpdateWebAppCustomizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWebAppCustomizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.UpdateWebAppCustomization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWebAppCustomizationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateWebAppCustomizationOutput, body, alloc);
}
