...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWebAppsInput, options: Options) !ListWebAppsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWebAppsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.ListWebApps");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWebAppsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListWebAppsOutput, body, alloc);
}
