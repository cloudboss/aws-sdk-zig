...
pub const UpdateServerInput = struct {
    ...
    /// certificate](https://docs.aws.amazon.com/acm/latest/userguide/gs-acm-request-public.html) in the * Amazon Web ServicesCertificate Manager User Guide*.
    ///
    /// To import an existing certificate into ACM, see [Importing certificates into
    /// ACM](https://docs.aws.amazon.com/acm/latest/userguide/import-certificate.html) in the * Amazon Web ServicesCertificate Manager User Guide*.
    ///
    /// To request a private certificate to use FTPS through private IP addresses,
    /// see [Request a private
    /// certificate](https://docs.aws.amazon.com/acm/latest/userguide/gs-acm-request-private.html) in the * Amazon Web ServicesCertificate Manager User Guide*.
    ///
...
pub const UpdateServerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServerInput, options: CallOptions) !UpdateServerOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "transfer");
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TransferService.UpdateServer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateServerOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateServerOutput, body, allocator);
}
