...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateServerInput, options: CallOptions) !CreateServerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TransferService.CreateServer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateServerOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateServerOutput, body, allocator);
}
