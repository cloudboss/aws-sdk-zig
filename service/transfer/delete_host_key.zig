...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteHostKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.DeleteHostKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteHostKeyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
