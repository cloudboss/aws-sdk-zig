...
pub const ListSecurityPoliciesOutput = struct {
    ...
    /// operation, a `NextToken` parameter is returned in the output. In a following
    /// command, you can pass in the `NextToken` parameter to continue listing
    /// security policies.
    next_token: ?[]const u8 = null,

    /// An array of security policies that were listed.
    security_policy_names: ?[]const []const u8 = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSecurityPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TransferService.ListSecurityPolicies");
