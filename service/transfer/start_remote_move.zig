...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRemoteMoveInput, options: Options) !StartRemoteMoveOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRemoteMoveInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.StartRemoteMove");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRemoteMoveOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartRemoteMoveOutput, body, alloc);
}
