...
fn serializeRequest(alloc: std.mem.Allocator, input: StartFileTransferInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.StartFileTransfer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartFileTransferOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StartFileTransferOutput, body, alloc);
}
