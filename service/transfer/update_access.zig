...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccessInput, options: CallOptions) !UpdateAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TransferService.UpdateAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAccessOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateAccessOutput, body, allocator);
}
