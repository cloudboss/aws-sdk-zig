...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSecurityPolicyInput, options: Options) !DescribeSecurityPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSecurityPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.DescribeSecurityPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSecurityPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSecurityPolicyOutput, body, alloc);
}
