...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAgreementInput, options: CallOptions) !UpdateAgreementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAgreementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TransferService.UpdateAgreement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAgreementOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateAgreementOutput, body, allocator);
}
