...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportSshPublicKeyInput, options: Options) !ImportSshPublicKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportSshPublicKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.ImportSshPublicKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportSshPublicKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ImportSshPublicKeyOutput, body, alloc);
}
