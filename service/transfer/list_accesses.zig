...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessesInput, options: Options) !ListAccessesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.ListAccesses");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAccessesOutput, body, alloc);
}
