...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateHostKeyInput, options: Options) !UpdateHostKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateHostKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.UpdateHostKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateHostKeyOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateHostKeyOutput, body, alloc);
}
