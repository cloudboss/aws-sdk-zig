...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestIdentityProviderInput, options: Options) !TestIdentityProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestIdentityProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.TestIdentityProvider");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestIdentityProviderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TestIdentityProviderOutput, body, alloc);
}
