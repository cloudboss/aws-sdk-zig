...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportHostKeyInput, options: Options) !ImportHostKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportHostKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.ImportHostKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportHostKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ImportHostKeyOutput, body, alloc);
}
