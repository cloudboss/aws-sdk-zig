...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHostKeysInput, options: Options) !ListHostKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHostKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.ListHostKeys");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHostKeysOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListHostKeysOutput, body, alloc);
}
