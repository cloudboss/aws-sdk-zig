...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWebAppInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.DescribeWebApp");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWebAppOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeWebAppOutput, body, alloc);
}
