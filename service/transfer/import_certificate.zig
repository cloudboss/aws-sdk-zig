...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportCertificateInput, options: Options) !ImportCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.ImportCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ImportCertificateOutput, body, alloc);
}
