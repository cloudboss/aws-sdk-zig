...
pub const CreateAccessInput = struct {
    ...
    /// [AssumeRole](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html) in the *Security Token Service API Reference*.
...
pub const CreateAccessOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TransferService.CreateAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAccessOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateAccessOutput, body, allocator);
}
