...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendWorkflowStepStateInput, options: Options) !SendWorkflowStepStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendWorkflowStepStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.SendWorkflowStepState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendWorkflowStepStateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
