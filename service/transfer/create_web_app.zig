...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWebAppInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.CreateWebApp");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWebAppOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateWebAppOutput, body, alloc);
}
