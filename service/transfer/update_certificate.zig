...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCertificateInput, options: Options) !UpdateCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.UpdateCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCertificateOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateCertificateOutput, body, alloc);
}
