...
pub const CreateAgreementInput = struct {
...
pub const CreateAgreementOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAgreementInput, options: CallOptions) !CreateAgreementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAgreementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TransferService.CreateAgreement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAgreementOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateAgreementOutput, body, allocator);
}
