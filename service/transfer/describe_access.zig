...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccessInput, options: Options) !DescribeAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TransferService.DescribeAccess");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccessOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAccessOutput, body, alloc);
}
