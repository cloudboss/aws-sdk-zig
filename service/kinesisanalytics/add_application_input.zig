...
pub const AddApplicationInputInput = struct {
    ...
    /// The
    /// [Input](https://docs.aws.amazon.com/kinesisanalytics/latest/dev/API_Input.html) to add.
    input: Input,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddApplicationInputInput, options: Options) !AddApplicationInputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddApplicationInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20150814.AddApplicationInput");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddApplicationInputOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
