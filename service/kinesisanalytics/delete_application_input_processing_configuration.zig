...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteApplicationInputProcessingConfigurationInput, options: Options) !DeleteApplicationInputProcessingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteApplicationInputProcessingConfigurationInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("kinesisanalytics", "Kinesis Analytics", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20150814.DeleteApplicationInputProcessingConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteApplicationInputProcessingConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
