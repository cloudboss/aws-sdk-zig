...
const DeleteApplicationOutput = @import("delete_application_response.zig").DeleteApplicationResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteApplicationInput, options: Options) !DeleteApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20150814.DeleteApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteApplicationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
