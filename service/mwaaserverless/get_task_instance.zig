...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTaskInstanceInput, options: Options) !GetTaskInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTaskInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMWAAServerless.GetTaskInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTaskInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTaskInstanceOutput, body, alloc);
}
