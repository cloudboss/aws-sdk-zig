...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopWorkflowRunInput, options: Options) !StopWorkflowRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopWorkflowRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMWAAServerless.StopWorkflowRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopWorkflowRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopWorkflowRunOutput, body, alloc);
}
