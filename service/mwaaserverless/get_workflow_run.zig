...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkflowRunInput, options: Options) !GetWorkflowRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkflowRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMWAAServerless.GetWorkflowRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkflowRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetWorkflowRunOutput, body, alloc);
}
