...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTaskInstancesInput, options: Options) !ListTaskInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTaskInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMWAAServerless.ListTaskInstances");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTaskInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTaskInstancesOutput, body, alloc);
}
