...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExperimentResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.base_stat) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"baseStat\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"metricNames\":");
    try aws.json.writeValue(@TypeOf(input.metric_names), input.metric_names, alloc, &body_buf);
    has_prev = true;
    if (input.period) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.report_names) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"reportNames\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.result_stats) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resultStats\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"treatmentNames\":");
    try aws.json.writeValue(@TypeOf(input.treatment_names), input.treatment_names, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExperimentResultsOutput {
    var result: GetExperimentResultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetExperimentResultsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
