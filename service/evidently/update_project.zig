...
pub const UpdateProjectInput = struct {
    ...
    /// AppConfig. Client-side evaluation allows your application to assign
    /// variations to user sessions locally instead of by calling the
    /// [EvaluateFeature](https://docs.aws.amazon.com/cloudwatchevidently/latest/APIReference/API_EvaluateFeature.html) operation. This mitigates the latency and availability risks that come with an API call. allows you to
    ///
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProjectOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProjectOutput, body, alloc);
    }
    _ = status;
    _ = headers;
