...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLaunchInput, options: CallOptions) !CreateLaunchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLaunchInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.groups), input.groups, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLaunchOutput {
    var result: CreateLaunchOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLaunchOutput, body, allocator);
    }
    _ = status;
    _ = headers;
