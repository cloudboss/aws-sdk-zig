...
pub const UpdateProjectDataDeliveryInput = struct {
...
pub const UpdateProjectDataDeliveryOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateProjectDataDeliveryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/data-delivery");
    ...
    if (input.cloud_watch_logs) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"cloudWatchLogs\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.s_3_destination) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateProjectDataDeliveryOutput {
    var result: UpdateProjectDataDeliveryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProjectDataDeliveryOutput, body, allocator);
    }
    _ = status;
    _ = headers;
