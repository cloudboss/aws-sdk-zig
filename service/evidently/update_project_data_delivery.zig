...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProjectDataDeliveryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/data-delivery");
    ...
    if (input.cloud_watch_logs) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"cloudWatchLogs\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProjectDataDeliveryOutput {
    var result: UpdateProjectDataDeliveryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProjectDataDeliveryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
