...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSegmentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSegmentsOutput {
    var result: ListSegmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSegmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
