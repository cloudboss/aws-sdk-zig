...
pub const ListSegmentsInput = struct {
...
pub const ListSegmentsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSegmentsInput, options: CallOptions) !ListSegmentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSegmentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSegmentsOutput {
    var result: ListSegmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSegmentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
