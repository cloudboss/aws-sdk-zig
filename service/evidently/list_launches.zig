...
pub const ListLaunchesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLaunchesInput, options: CallOptions) !ListLaunchesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLaunchesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLaunchesOutput {
    var result: ListLaunchesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLaunchesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
