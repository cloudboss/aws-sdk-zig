...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLaunchesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLaunchesOutput {
    var result: ListLaunchesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLaunchesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
