...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFeaturesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFeaturesOutput {
    var result: ListFeaturesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFeaturesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
