...
fn serializeRequest(alloc: std.mem.Allocator, input: StartExperimentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"analysisCompleteTime\":");
    try aws.json.writeValue(@TypeOf(input.analysis_complete_time), input.analysis_complete_time, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartExperimentOutput {
    var result: StartExperimentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartExperimentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
