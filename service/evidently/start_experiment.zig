...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartExperimentInput, options: Options) !StartExperimentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartExperimentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"analysisCompleteTime\":");
    try aws.json.writeValue(@TypeOf(input.analysis_complete_time), input.analysis_complete_time, allocator, &body_buf);
    has_prev = true;
