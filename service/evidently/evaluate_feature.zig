...
fn serializeRequest(alloc: std.mem.Allocator, input: EvaluateFeatureInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.evaluation_context) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"evaluationContext\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EvaluateFeatureOutput {
    var result: EvaluateFeatureOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EvaluateFeatureOutput, body, alloc);
    }
    _ = status;
    _ = headers;
