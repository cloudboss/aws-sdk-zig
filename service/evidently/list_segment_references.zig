...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSegmentReferencesInput, options: CallOptions) !ListSegmentReferencesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSegmentReferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.segment);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSegmentReferencesOutput {
    var result: ListSegmentReferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSegmentReferencesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
