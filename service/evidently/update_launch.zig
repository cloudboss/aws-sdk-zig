...
pub const UpdateLaunchOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLaunchInput, options: CallOptions) !UpdateLaunchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateLaunchInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/launches/");
    try path_buf.appendSlice(allocator, input.launch);
    ...
    if (input.metric_monitors) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"metricMonitors\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    ...
    if (input.scheduled_splits_config) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"scheduledSplitsConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateLaunchOutput {
    var result: UpdateLaunchOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateLaunchOutput, body, allocator);
    }
    _ = status;
    _ = headers;
