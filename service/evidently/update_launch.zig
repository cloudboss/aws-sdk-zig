...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLaunchInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.launch);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLaunchOutput {
    var result: UpdateLaunchOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateLaunchOutput, body, alloc);
    }
    _ = status;
    _ = headers;
