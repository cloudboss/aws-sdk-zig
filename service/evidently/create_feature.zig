...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFeatureInput, options: CallOptions) !CreateFeatureOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateFeatureInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"variations\":");
    try aws.json.writeValue(@TypeOf(input.variations), input.variations, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateFeatureOutput {
    var result: CreateFeatureOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFeatureOutput, body, allocator);
    }
    _ = status;
    _ = headers;
