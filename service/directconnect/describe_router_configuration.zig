...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRouterConfigurationInput, options: CallOptions) !DescribeRouterConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRouterConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.DescribeRouterConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeRouterConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRouterConfigurationOutput, body, allocator);
}
