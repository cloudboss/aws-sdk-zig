...
pub const CreatePrivateVirtualInterfaceInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreatePrivateVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.CreatePrivateVirtualInterface");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreatePrivateVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePrivateVirtualInterfaceOutput, body, allocator);
}
