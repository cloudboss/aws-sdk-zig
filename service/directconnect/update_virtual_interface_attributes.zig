...
const UpdateVirtualInterfaceAttributesOutput = @import("virtual_interface.zig").VirtualInterface;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVirtualInterfaceAttributesInput, options: Options) !UpdateVirtualInterfaceAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVirtualInterfaceAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.UpdateVirtualInterfaceAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVirtualInterfaceAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateVirtualInterfaceAttributesOutput, body, alloc);
}
