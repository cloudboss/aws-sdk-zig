...
pub const ConfirmPublicVirtualInterfaceInput = struct {
...
pub const ConfirmPublicVirtualInterfaceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfirmPublicVirtualInterfaceInput, options: CallOptions) !ConfirmPublicVirtualInterfaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ConfirmPublicVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.ConfirmPublicVirtualInterface");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ConfirmPublicVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ConfirmPublicVirtualInterfaceOutput, body, allocator);
}
