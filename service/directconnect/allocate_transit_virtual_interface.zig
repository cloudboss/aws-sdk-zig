...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AllocateTransitVirtualInterfaceInput, options: Options) !AllocateTransitVirtualInterfaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AllocateTransitVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.AllocateTransitVirtualInterface");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AllocateTransitVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AllocateTransitVirtualInterfaceOutput, body, alloc);
}
