...
const DisassociateConnectionFromLagOutput = @import("connection.zig").Connection;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateConnectionFromLagInput, options: Options) !DisassociateConnectionFromLagOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateConnectionFromLagInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.DisassociateConnectionFromLag");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateConnectionFromLagOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateConnectionFromLagOutput, body, alloc);
}
