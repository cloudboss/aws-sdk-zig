...
pub const AllocateConnectionOnInterconnectInput = struct {
    /// The bandwidth of the connection. The possible values are 50Mbps, 100Mbps,
    /// 200Mbps,
    /// 300Mbps, 400Mbps, 500Mbps, 1Gbps, 2Gbps, 5Gbps, and 10Gbps. Note that only
    /// those Direct Connect Partners
...
pub const AllocateConnectionOnInterconnectOutput = @import("connection.zig").Connection;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AllocateConnectionOnInterconnectInput, options: CallOptions) !AllocateConnectionOnInterconnectOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AllocateConnectionOnInterconnectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.AllocateConnectionOnInterconnect");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AllocateConnectionOnInterconnectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AllocateConnectionOnInterconnectOutput, body, allocator);
}
