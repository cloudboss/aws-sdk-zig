...
const NewPrivateVirtualInterfaceAllocation = @import("new_private_virtual_interface_allocation.zig").NewPrivateVirtualInterfaceAllocation;
...
pub const AllocatePrivateVirtualInterfaceInput = struct {
...
pub const AllocatePrivateVirtualInterfaceOutput = @import("virtual_interface.zig").VirtualInterface;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AllocatePrivateVirtualInterfaceInput, options: CallOptions) !AllocatePrivateVirtualInterfaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AllocatePrivateVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.AllocatePrivateVirtualInterface");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AllocatePrivateVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AllocatePrivateVirtualInterfaceOutput, body, allocator);
}
