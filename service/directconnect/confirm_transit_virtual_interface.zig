...
pub const ConfirmTransitVirtualInterfaceInput = struct {
...
pub const ConfirmTransitVirtualInterfaceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfirmTransitVirtualInterfaceInput, options: CallOptions) !ConfirmTransitVirtualInterfaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ConfirmTransitVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.ConfirmTransitVirtualInterface");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ConfirmTransitVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ConfirmTransitVirtualInterfaceOutput, body, allocator);
}
