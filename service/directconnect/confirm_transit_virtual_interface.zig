...
fn serializeRequest(alloc: std.mem.Allocator, input: ConfirmTransitVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.ConfirmTransitVirtualInterface");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConfirmTransitVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ConfirmTransitVirtualInterfaceOutput, body, alloc);
}
