...
const AllocateHostedConnectionOutput = @import("connection.zig").Connection;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AllocateHostedConnectionInput, options: Options) !AllocateHostedConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AllocateHostedConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.AllocateHostedConnection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AllocateHostedConnectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AllocateHostedConnectionOutput, body, alloc);
}
