...
pub const DescribeVirtualGatewaysOutput = struct {
    ...
    virtual_gateways: ?[]const VirtualGateway = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVirtualGatewaysInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.DescribeVirtualGateways");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVirtualGatewaysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeVirtualGatewaysOutput, body, allocator);
}
