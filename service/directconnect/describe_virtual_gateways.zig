...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVirtualGatewaysInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.DescribeVirtualGateways");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVirtualGatewaysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeVirtualGatewaysOutput, body, alloc);
}
