...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDirectConnectGatewayAttachmentsInput, options: Options) !DescribeDirectConnectGatewayAttachmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDirectConnectGatewayAttachmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.DescribeDirectConnectGatewayAttachments");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDirectConnectGatewayAttachmentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDirectConnectGatewayAttachmentsOutput, body, alloc);
}
