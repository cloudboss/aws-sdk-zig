...
pub const AssociateHostedConnectionInput = struct {
    /// The ID of the hosted connection.
    connection_id: []const u8,

    /// The ID of the interconnect or the LAG.
    parent_connection_id: []const u8,
...
pub const AssociateHostedConnectionOutput = @import("connection.zig").Connection;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateHostedConnectionInput, options: CallOptions) !AssociateHostedConnectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateHostedConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.AssociateHostedConnection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateHostedConnectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateHostedConnectionOutput, body, allocator);
}
