...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteInterconnectInput, options: Options) !DeleteInterconnectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteInterconnectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.DeleteInterconnect");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteInterconnectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteInterconnectOutput, body, alloc);
}
