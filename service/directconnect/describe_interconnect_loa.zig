...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInterconnectLoaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.DescribeInterconnectLoa");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInterconnectLoaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInterconnectLoaOutput, body, alloc);
}
