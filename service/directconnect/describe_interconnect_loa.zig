...
pub const DescribeInterconnectLoaInput = struct {
...
pub const DescribeInterconnectLoaOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInterconnectLoaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.DescribeInterconnectLoa");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInterconnectLoaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInterconnectLoaOutput, body, allocator);
}
