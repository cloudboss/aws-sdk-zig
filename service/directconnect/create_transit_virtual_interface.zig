...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTransitVirtualInterfaceInput, options: Options) !CreateTransitVirtualInterfaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTransitVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.CreateTransitVirtualInterface");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTransitVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTransitVirtualInterfaceOutput, body, alloc);
}
