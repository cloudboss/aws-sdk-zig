...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDirectConnectGatewayAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.UpdateDirectConnectGatewayAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDirectConnectGatewayAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateDirectConnectGatewayAssociationOutput, body, alloc);
}
