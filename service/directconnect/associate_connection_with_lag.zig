...
pub const AssociateConnectionWithLagInput = struct {
...
pub const AssociateConnectionWithLagOutput = @import("connection.zig").Connection;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateConnectionWithLagInput, options: CallOptions) !AssociateConnectionWithLagOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateConnectionWithLagInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.AssociateConnectionWithLag");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateConnectionWithLagOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateConnectionWithLagOutput, body, allocator);
}
