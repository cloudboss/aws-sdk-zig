...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateConnectionWithLagInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.AssociateConnectionWithLag");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateConnectionWithLagOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateConnectionWithLagOutput, body, alloc);
}
