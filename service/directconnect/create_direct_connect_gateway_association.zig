...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDirectConnectGatewayAssociationInput, options: Options) !CreateDirectConnectGatewayAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDirectConnectGatewayAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.CreateDirectConnectGatewayAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDirectConnectGatewayAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDirectConnectGatewayAssociationOutput, body, alloc);
}
