...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePublicVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.CreatePublicVirtualInterface");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePublicVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePublicVirtualInterfaceOutput, body, alloc);
}
