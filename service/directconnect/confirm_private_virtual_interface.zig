...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfirmPrivateVirtualInterfaceInput, options: Options) !ConfirmPrivateVirtualInterfaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ConfirmPrivateVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.ConfirmPrivateVirtualInterface");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConfirmPrivateVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ConfirmPrivateVirtualInterfaceOutput, body, alloc);
}
