...
const DescribeCustomerMetadataInput = struct {};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCustomerMetadataInput, options: Options) !DescribeCustomerMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCustomerMetadataInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.DescribeCustomerMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCustomerMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCustomerMetadataOutput, body, alloc);
}
