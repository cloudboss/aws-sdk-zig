...
pub const DisassociateMacSecKeyInput = struct {
...
pub const DisassociateMacSecKeyOutput = struct {
    /// The ID of the dedicated connection (dxcon-xxxx), interconnect (dxcon-xxxx),
    /// or LAG (dxlag-xxxx).
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateMacSecKeyInput, options: CallOptions) !DisassociateMacSecKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateMacSecKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.DisassociateMacSecKey");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateMacSecKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateMacSecKeyOutput, body, allocator);
}
