...
const DeleteLagOutput = @import("lag.zig").Lag;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLagInput, options: Options) !DeleteLagOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteLagInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.DeleteLag");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteLagOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteLagOutput, body, alloc);
}
