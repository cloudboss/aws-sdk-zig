...
pub const DescribeConnectionLoaInput = struct {
    ...
    /// on your behalf. If you specify this parameter,
...
pub const DescribeConnectionLoaOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConnectionLoaInput, options: CallOptions) !DescribeConnectionLoaOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeConnectionLoaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.DescribeConnectionLoa");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeConnectionLoaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConnectionLoaOutput, body, allocator);
}
