...
pub const DeleteVirtualInterfaceInput = struct {
...
pub const DeleteVirtualInterfaceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVirtualInterfaceInput, options: CallOptions) !DeleteVirtualInterfaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.DeleteVirtualInterface");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteVirtualInterfaceOutput, body, allocator);
}
