...
pub const ConfirmConnectionInput = struct {
...
pub const ConfirmConnectionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfirmConnectionInput, options: CallOptions) !ConfirmConnectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ConfirmConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.ConfirmConnection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ConfirmConnectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ConfirmConnectionOutput, body, allocator);
}
