...
fn serializeRequest(alloc: std.mem.Allocator, input: AllocatePublicVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OvertureService.AllocatePublicVirtualInterface");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AllocatePublicVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AllocatePublicVirtualInterfaceOutput, body, alloc);
}
