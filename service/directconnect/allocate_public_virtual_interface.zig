...
const NewPublicVirtualInterfaceAllocation = @import("new_public_virtual_interface_allocation.zig").NewPublicVirtualInterfaceAllocation;
...
pub const AllocatePublicVirtualInterfaceInput = struct {
...
pub const AllocatePublicVirtualInterfaceOutput = @import("virtual_interface.zig").VirtualInterface;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AllocatePublicVirtualInterfaceInput, options: CallOptions) !AllocatePublicVirtualInterfaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AllocatePublicVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.AllocatePublicVirtualInterface");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AllocatePublicVirtualInterfaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AllocatePublicVirtualInterfaceOutput, body, allocator);
}
