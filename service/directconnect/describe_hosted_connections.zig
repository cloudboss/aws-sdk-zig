...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHostedConnectionsInput, options: CallOptions) !DescribeHostedConnectionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHostedConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.DescribeHostedConnections");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHostedConnectionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeHostedConnectionsOutput, body, allocator);
}
