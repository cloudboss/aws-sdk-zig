...
pub const CreateInterconnectInput = struct {
...
pub const CreateInterconnectOutput = @import("interconnect.zig").Interconnect;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInterconnectInput, options: CallOptions) !CreateInterconnectOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateInterconnectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OvertureService.CreateInterconnect");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateInterconnectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateInterconnectOutput, body, allocator);
}
