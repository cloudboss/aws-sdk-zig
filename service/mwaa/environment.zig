...
pub const Environment = struct {
    ...
    /// options attached to your environment. For more information, refer to [Apache
    ...
    /// Executor](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/executor/celery.html). Amazon MWAA uses a Celery Executor to distribute tasks across multiple workers. When you create an environment in a shared VPC, you must provide access to the Celery Executor queue from your VPC.
    ...
    /// `s3://mwaa-environment/dags`. For more information, refer to [Adding or
    /// updating
    ...
    /// more information, refer to [About networking on Amazon
    ...
    ///
    /// For more information, refer to [Installing custom
    ...
    /// environment. For more information, refer to [Amazon MWAA Service-linked
    ...
    /// * `CREATING_SNAPSHOT` - The request to update environment details, or
    ///   upgrade the environment version, is in progress and Amazon MWAA is
    ...
    ///   at a specific point in time. Amazon MWAA uses snapshots to recover
    ///   environment metadata if the process to update or upgrade an environment
    ///   fails.
    /// * `CREATE_FAILED` - The request to create the environment failed and the
    ///   environment was not created.
    /// * `AVAILABLE` - The request was successful and the environment is ready to
    ///   use.
    /// * `PENDING` - The request was successful, but the process to create the
    ///   environment is paused until you create the required VPC endpoints in your
    ...
    ///   environment version failed and Amazon MWAA is restoring the environment
    ...
    /// * `UNAVAILABLE` - The request failed, but the environment did not return to
    ///   its previous state and is not stable.
    /// * `UPDATE_FAILED` - The request to update the environment failed and the
    ///   environment was restored to its previous state successfully and is ready
    ///   to use.
    /// * `MAINTENANCE` - The environment is undergoing maintenance. Depending on
    ///   the type of work Amazon MWAA is performing, your environment might be
    ///   unavailable during this process. Note that as part of the maintenance
    ///   work, Amazon MWAA performs with a `GRACEFUL` [ `workerReplacementStrategy`
    ///   ](https://docs.aws.amazon.com/mwaa/latest/API/API_UpdateEnvironment.html#mwaa-UpdateEnvironment-request-WorkerReplacementStrategy).
    ///
    /// You can review our troubleshooting guide for a list of common errors and
    /// their solutions. For more information, refer to [Amazon MWAA
    ...
    /// `"Environment": "Staging"`. For more information, refer to [Tagging Amazon
    /// Web Services
    ...
    /// For more information, refer to [Accessing the Apache Airflow
