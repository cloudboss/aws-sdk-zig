...
pub const Environment = struct {
    ...
    airflow_configuration_options: ?[]const aws.map.StringMapEntry,
    ...
    /// your VPC.
    celery_executor_queue: ?[]const u8,
    ...
    database_vpc_endpoint_service: ?[]const u8,
    ...
    plugins_s3_object_version: ?[]const u8,
    ...
    requirements_s3_object_version: ?[]const u8,
    ...
    startup_script_s3_object_version: ?[]const u8,
    ...
    /// [Using a startup
    /// script](https://docs.aws.amazon.com/mwaa/latest/userguide/using-startup-script.html).
    startup_script_s3_path: ?[]const u8,
    ...
    webserver_url: ?[]const u8,

    /// The VPC endpoint for the environment's web server.
    webserver_vpc_endpoint_service: ?[]const u8,

    /// The day and time of the week in Coordinated Universal Time (UTC) 24-hour
    /// standard time that weekly maintenance updates are scheduled. For example:
    /// `TUE:03:30`.
    weekly_maintenance_window_start: ?[]const u8,
