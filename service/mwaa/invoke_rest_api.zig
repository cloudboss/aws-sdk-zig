...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InvokeRestApiInput, options: Options) !InvokeRestApiOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InvokeRestApiInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/restapi/");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Method\":");
    try aws.json.writeValue(@TypeOf(input.method), input.method, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Path\":");
    try aws.json.writeValue(@TypeOf(input.path), input.path, alloc, &body_buf);
    has_prev = true;
    if (input.query_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"QueryParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InvokeRestApiOutput {
    var result: InvokeRestApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(InvokeRestApiOutput, body, alloc);
    }
    _ = status;
    _ = headers;
