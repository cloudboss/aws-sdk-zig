...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWebLoginTokenInput, options: Options) !CreateWebLoginTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWebLoginTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/webtoken/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWebLoginTokenOutput {
    var result: CreateWebLoginTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWebLoginTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
