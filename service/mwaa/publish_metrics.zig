...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PublishMetricsInput, options: Options) !PublishMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PublishMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/metrics/environments/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PublishMetricsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
