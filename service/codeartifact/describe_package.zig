...
const PackageDescription = @import("package_description.zig").PackageDescription;
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePackageInput, options: Options) !DescribePackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePackageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePackageOutput {
    var result: DescribePackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
