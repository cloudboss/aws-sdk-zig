...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSubPackageGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSubPackageGroupsOutput {
    var result: ListSubPackageGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSubPackageGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
