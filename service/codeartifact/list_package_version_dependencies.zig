...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPackageVersionDependenciesInput, options: Options) !ListPackageVersionDependenciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPackageVersionDependenciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPackageVersionDependenciesOutput {
    var result: ListPackageVersionDependenciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPackageVersionDependenciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
