...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePackageVersionsStatusInput, options: Options) !UpdatePackageVersionsStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePackageVersionsStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"targetStatus\":");
    try aws.json.writeValue(@TypeOf(input.target_status), input.target_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePackageVersionsStatusOutput {
    var result: UpdatePackageVersionsStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePackageVersionsStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
