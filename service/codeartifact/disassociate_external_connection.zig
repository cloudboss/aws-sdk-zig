...
pub const DisassociateExternalConnectionInput = struct {
...
pub const DisassociateExternalConnectionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateExternalConnectionInput, options: Options) !DisassociateExternalConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateExternalConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "external-connection=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.external_connection);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateExternalConnectionOutput {
    var result: DisassociateExternalConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateExternalConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
