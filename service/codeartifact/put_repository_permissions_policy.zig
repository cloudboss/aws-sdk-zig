...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRepositoryPermissionsPolicyInput, options: Options) !PutRepositoryPermissionsPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRepositoryPermissionsPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRepositoryPermissionsPolicyOutput {
    var result: PutRepositoryPermissionsPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutRepositoryPermissionsPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
