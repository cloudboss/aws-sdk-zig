...
pub const DeletePackageVersionsInput = struct {
...
pub const DeletePackageVersionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePackageVersionsInput, options: Options) !DeletePackageVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePackageVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/package/versions/delete";
    ...
    if (input.expected_status) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"expectedStatus\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"versions\":");
    try aws.json.writeValue(@TypeOf(input.versions), input.versions, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePackageVersionsOutput {
    var result: DeletePackageVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePackageVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
