...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePackageVersionsInput, options: CallOptions) !DeletePackageVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePackageVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePackageVersionsOutput {
    var result: DeletePackageVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePackageVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
