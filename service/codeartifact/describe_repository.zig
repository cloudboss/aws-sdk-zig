...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRepositoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRepositoryOutput {
    var result: DescribeRepositoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRepositoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
