...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDomainPermissionsPolicyInput, options: Options) !PutDomainPermissionsPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDomainPermissionsPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.domain_owner) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"domainOwner\":");
    ...
    if (input.policy_revision) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"policyRevision\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDomainPermissionsPolicyOutput {
    var result: PutDomainPermissionsPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutDomainPermissionsPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
