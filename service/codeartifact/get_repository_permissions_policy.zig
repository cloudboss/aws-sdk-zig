...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRepositoryPermissionsPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRepositoryPermissionsPolicyOutput {
    var result: GetRepositoryPermissionsPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRepositoryPermissionsPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
