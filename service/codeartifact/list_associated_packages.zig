...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssociatedPackagesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.preview) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "preview=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociatedPackagesOutput {
    var result: ListAssociatedPackagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssociatedPackagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
