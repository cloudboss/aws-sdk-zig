...
fn serializeRequest(alloc: std.mem.Allocator, input: DisposePackageVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisposePackageVersionsOutput {
    var result: DisposePackageVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisposePackageVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
