...
pub const ListAllowedRepositoriesForGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAllowedRepositoriesForGroupInput, options: Options) !ListAllowedRepositoriesForGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAllowedRepositoriesForGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/package-group-allowed-repositories";
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "originRestrictionType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.origin_restriction_type));
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAllowedRepositoriesForGroupOutput {
    var result: ListAllowedRepositoriesForGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAllowedRepositoriesForGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
