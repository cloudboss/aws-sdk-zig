...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRepositoryPermissionsPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.policy_revision) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "policy-revision=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRepositoryPermissionsPolicyOutput {
    var result: DeleteRepositoryPermissionsPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRepositoryPermissionsPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
