...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePackageGroupInput, options: Options) !UpdatePackageGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePackageGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePackageGroupOutput {
    var result: UpdatePackageGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePackageGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
