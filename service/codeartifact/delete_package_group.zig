...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePackageGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePackageGroupOutput {
    var result: DeletePackageGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePackageGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
