...
pub const DeletePackageGroupInput = struct {
...
pub const DeletePackageGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePackageGroupInput, options: CallOptions) !DeletePackageGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePackageGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePackageGroupOutput {
    var result: DeletePackageGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePackageGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
