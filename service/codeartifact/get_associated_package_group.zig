...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssociatedPackageGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssociatedPackageGroupOutput {
    var result: GetAssociatedPackageGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssociatedPackageGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
