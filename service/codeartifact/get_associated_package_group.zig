...
pub const GetAssociatedPackageGroupInput = struct {
...
pub const GetAssociatedPackageGroupOutput = struct {
    /// Describes the strength of the association between the package and package
    /// group. A strong match is also known as an
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssociatedPackageGroupInput, options: CallOptions) !GetAssociatedPackageGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAssociatedPackageGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/get-associated-package-group";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAssociatedPackageGroupOutput {
    var result: GetAssociatedPackageGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssociatedPackageGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
