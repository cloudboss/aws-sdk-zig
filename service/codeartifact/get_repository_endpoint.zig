...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRepositoryEndpointInput, options: Options) !GetRepositoryEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRepositoryEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.endpoint_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "endpointType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRepositoryEndpointOutput {
    var result: GetRepositoryEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRepositoryEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
