...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRepositoryInput, options: Options) !UpdateRepositoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRepositoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRepositoryOutput {
    var result: UpdateRepositoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRepositoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
