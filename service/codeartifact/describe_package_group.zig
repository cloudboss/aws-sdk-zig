...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePackageGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePackageGroupOutput {
    var result: DescribePackageGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePackageGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
