...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRepositoriesInDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.administrator_account) |v| {
        ...
        try query_buf.appendSlice(alloc, "administrator-account=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRepositoriesInDomainOutput {
    var result: ListRepositoriesInDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRepositoriesInDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
