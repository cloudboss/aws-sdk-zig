...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PublishPackageVersionInput, options: Options) !PublishPackageVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PublishPackageVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.asset_name);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    ...
    if (input.unfinished) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "unfinished=");
    ...
    try request.headers.put(alloc, "x-amz-content-sha256", input.asset_sha256);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PublishPackageVersionOutput {
    var result: PublishPackageVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PublishPackageVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
