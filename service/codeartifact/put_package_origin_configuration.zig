...
pub const PutPackageOriginConfigurationInput = struct {
    ...
    /// origin restrictions.
    /// The `upstream` restriction determines if new package versions can be
    /// ingested or retained from external connections or upstream repositories.
    /// The `publish` restriction determines if new package versions can be
...
pub const PutPackageOriginConfigurationOutput = struct {
    /// A
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutPackageOriginConfigurationInput, options: CallOptions) !PutPackageOriginConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutPackageOriginConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.restrictions), input.restrictions, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutPackageOriginConfigurationOutput {
    var result: PutPackageOriginConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutPackageOriginConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
