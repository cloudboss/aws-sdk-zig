...
fn serializeRequest(alloc: std.mem.Allocator, input: PutPackageOriginConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutPackageOriginConfigurationOutput {
    var result: PutPackageOriginConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutPackageOriginConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
