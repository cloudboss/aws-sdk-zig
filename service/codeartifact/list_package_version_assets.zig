...
pub const ListPackageVersionAssetsInput = struct {
    ...
    /// version assets. The package component that specifies its
...
pub const ListPackageVersionAssetsOutput = struct {
    /// The returned list of
    /// [AssetSummary](https://docs.aws.amazon.com/codeartifact/latest/APIReference/API_AssetSummary.html) objects.
    assets: ?[]const AssetSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPackageVersionAssetsInput, options: Options) !ListPackageVersionAssetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPackageVersionAssetsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/package/version/assets";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPackageVersionAssetsOutput {
    var result: ListPackageVersionAssetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPackageVersionAssetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
