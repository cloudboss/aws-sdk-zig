...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePackageGroupInput, options: Options) !CreatePackageGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePackageGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.contact_info) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"contactInfo\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"packageGroup\":");
    try aws.json.writeValue(@TypeOf(input.package_group), input.package_group, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePackageGroupOutput {
    var result: CreatePackageGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePackageGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
