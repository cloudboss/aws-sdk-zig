...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateExternalConnectionInput, options: Options) !AssociateExternalConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateExternalConnectionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateExternalConnectionOutput {
    var result: AssociateExternalConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateExternalConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
