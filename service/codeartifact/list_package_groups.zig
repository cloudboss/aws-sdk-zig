...
const PackageGroupSummary = @import("package_group_summary.zig").PackageGroupSummary;

pub const ListPackageGroupsInput = struct {
    ...
    /// `ListPackageGroups` will return only
...
pub const ListPackageGroupsOutput = struct {
    /// The token for the next set of results. Use the value returned in the
    /// previous response in the next request to retrieve the next set of results.
    next_token: ?[]const u8 = null,

    /// The list of package groups in the requested domain.
    package_groups: ?[]const PackageGroupSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPackageGroupsInput, options: CallOptions) !ListPackageGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPackageGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPackageGroupsOutput {
    var result: ListPackageGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPackageGroupsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
