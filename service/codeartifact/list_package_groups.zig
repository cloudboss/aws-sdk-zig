...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPackageGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPackageGroupsOutput {
    var result: ListPackageGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPackageGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
