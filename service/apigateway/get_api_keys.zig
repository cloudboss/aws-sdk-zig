...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApiKeysInput, options: Options) !GetApiKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApiKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.customer_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "customerId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.include_values) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "includeValues=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApiKeysOutput {
    var result: GetApiKeysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetApiKeysOutput, body, alloc);
    }
    _ = status;
    _ = headers;
