...
pub const UpdateVpcLinkInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVpcLinkInput, options: Options) !UpdateVpcLinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVpcLinkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVpcLinkOutput {
    var result: UpdateVpcLinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVpcLinkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
