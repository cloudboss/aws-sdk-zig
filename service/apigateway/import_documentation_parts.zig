...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportDocumentationPartsInput, options: Options) !ImportDocumentationPartsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportDocumentationPartsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportDocumentationPartsOutput {
    var result: ImportDocumentationPartsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportDocumentationPartsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
