...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUsagePlanKeyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUsagePlanKeyOutput {
    var result: GetUsagePlanKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUsagePlanKeyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
