...
fn serializeRequest(allocator: std.mem.Allocator, input: GetUsagePlanKeyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetUsagePlanKeyOutput {
    var result: GetUsagePlanKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUsagePlanKeyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
