...
pub const CreateVpcLinkInput = struct {
...
const CreateVpcLinkOutput = @import("vpc_link.zig").CreateVpcLinkOutput;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVpcLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"targetArns\":");
    try aws.json.writeValue(@TypeOf(input.target_arns), input.target_arns, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVpcLinkOutput {
    var result: CreateVpcLinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVpcLinkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
