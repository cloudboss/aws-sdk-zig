...
pub const UpdateBasePathMappingOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBasePathMappingInput, options: Options) !UpdateBasePathMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBasePathMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.base_path);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBasePathMappingOutput {
    var result: UpdateBasePathMappingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBasePathMappingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
