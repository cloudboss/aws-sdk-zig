...
pub const CreateBasePathMappingOutput = @import("base_path_mapping.zig").BasePathMapping;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBasePathMappingInput, options: CallOptions) !CreateBasePathMappingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateBasePathMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.base_path) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"basePath\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"restApiId\":");
    try aws.json.writeValue(@TypeOf(input.rest_api_id), input.rest_api_id, allocator, &body_buf);
    has_prev = true;
    if (input.stage) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
