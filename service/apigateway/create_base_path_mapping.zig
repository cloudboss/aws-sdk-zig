...
pub const CreateBasePathMappingInput = struct {
...
const CreateBasePathMappingOutput = @import("base_path_mapping.zig").CreateBasePathMappingOutput;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBasePathMappingInput, options: Options) !CreateBasePathMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBasePathMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.base_path) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"basePath\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"restApiId\":");
    try aws.json.writeValue(@TypeOf(input.rest_api_id), input.rest_api_id, alloc, &body_buf);
    has_prev = true;
    if (input.stage) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBasePathMappingOutput {
    var result: CreateBasePathMappingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBasePathMappingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
