...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUsageInput, options: Options) !UpdateUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUsageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUsageOutput {
    var result: UpdateUsageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateUsageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
