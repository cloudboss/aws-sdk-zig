...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUsagePlansInput, options: Options) !GetUsagePlansOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUsagePlansInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUsagePlansOutput {
    var result: GetUsagePlansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUsagePlansOutput, body, alloc);
    }
    _ = status;
    _ = headers;
