...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainNameInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainNameOutput {
    var result: GetDomainNameOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainNameOutput, body, alloc);
    }
    _ = status;
    _ = headers;
