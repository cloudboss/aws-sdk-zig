...
pub const GetDomainNameAccessAssociationsInput = struct {
    ...
    /// domain name access associations owned by your own account.
    /// Use `OTHER_ACCOUNTS` to list the domain name access associations with your
    /// private custom domain names that are owned by other AWS
    /// accounts.
...
pub const GetDomainNameAccessAssociationsOutput = struct {
    /// The current page of elements from this collection.
    items: ?[]const DomainNameAccessAssociation = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainNameAccessAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.position) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "position=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.resource_owner) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "resourceOwner=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainNameAccessAssociationsOutput {
    var result: GetDomainNameAccessAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainNameAccessAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
