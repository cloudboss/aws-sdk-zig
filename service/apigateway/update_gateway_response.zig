...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGatewayResponseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGatewayResponseOutput {
    var result: UpdateGatewayResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGatewayResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
