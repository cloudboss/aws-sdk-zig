...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBasePathMappingInput, options: Options) !GetBasePathMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBasePathMappingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBasePathMappingOutput {
    var result: GetBasePathMappingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBasePathMappingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
