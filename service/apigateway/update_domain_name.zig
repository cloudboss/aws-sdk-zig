...
pub const UpdateDomainNameOutput = @import("domain_name.zig").DomainName;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDomainNameInput, options: CallOptions) !UpdateDomainNameOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDomainNameInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDomainNameOutput {
    var result: UpdateDomainNameOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDomainNameOutput, body, allocator);
    }
    _ = status;
    _ = headers;
