...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportApiKeysInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportApiKeysOutput {
    var result: ImportApiKeysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportApiKeysOutput, body, alloc);
    }
    _ = status;
    _ = headers;
