...
const GetResourceOutput = @import("resource.zig").Resource;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceInput, options: Options) !GetResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.embed) |v| {
        ...
        try query_buf.appendSlice(alloc, "embed=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceOutput {
    var result: GetResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
