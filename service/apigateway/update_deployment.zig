...
pub const UpdateDeploymentInput = struct {
...
const UpdateDeploymentOutput = @import("deployment.zig").UpdateDeploymentOutput;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDeploymentInput, options: Options) !UpdateDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.deployment_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDeploymentOutput {
    var result: UpdateDeploymentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDeploymentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
