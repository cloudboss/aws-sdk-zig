...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutGatewayResponseInput, options: Options) !PutGatewayResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutGatewayResponseInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.status_code) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"statusCode\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutGatewayResponseOutput {
    var result: PutGatewayResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutGatewayResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
