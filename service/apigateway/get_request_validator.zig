...
pub const GetRequestValidatorInput = struct {
...
pub const GetRequestValidatorOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRequestValidatorInput, options: Options) !GetRequestValidatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRequestValidatorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRequestValidatorOutput {
    var result: GetRequestValidatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRequestValidatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
