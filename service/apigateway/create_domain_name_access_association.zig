...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDomainNameAccessAssociationInput, options: Options) !CreateDomainNameAccessAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDomainNameAccessAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"accessAssociationSource\":");
    try aws.json.writeValue(@TypeOf(input.access_association_source), input.access_association_source, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"accessAssociationSourceType\":");
    try aws.json.writeValue(@TypeOf(input.access_association_source_type), input.access_association_source_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"domainNameArn\":");
    try aws.json.writeValue(@TypeOf(input.domain_name_arn), input.domain_name_arn, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDomainNameAccessAssociationOutput {
    var result: CreateDomainNameAccessAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDomainNameAccessAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
