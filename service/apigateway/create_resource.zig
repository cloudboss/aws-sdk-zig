...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.parent_id);
    ...
    try body_buf.appendSlice(allocator, "\"pathPart\":");
    try aws.json.writeValue(@TypeOf(input.path_part), input.path_part, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateResourceOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateResourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
