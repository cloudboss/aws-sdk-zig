...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetModelInput, options: Options) !GetModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetModelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.flatten) |v| {
        ...
        try query_buf.appendSlice(alloc, "flatten=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetModelOutput {
    var result: GetModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
