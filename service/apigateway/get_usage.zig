...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUsageInput, options: Options) !GetUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/usage");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.end_date);
    query_has_prev = true;
    if (input.key_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "keyId=");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.start_date);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUsageOutput {
    var result: GetUsageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUsageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
