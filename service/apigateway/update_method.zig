...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMethodInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMethodOutput {
    var result: UpdateMethodOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMethodOutput, body, alloc);
    }
    _ = status;
    _ = headers;
