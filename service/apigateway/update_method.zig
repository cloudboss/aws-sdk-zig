...
pub const UpdateMethodInput = struct {
...
pub const UpdateMethodOutput = @import("method.zig").Method;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMethodInput, options: CallOptions) !UpdateMethodOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateMethodInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateMethodOutput {
    var result: UpdateMethodOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMethodOutput, body, allocator);
    }
    _ = status;
    _ = headers;
