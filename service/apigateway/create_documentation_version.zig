...
pub const CreateDocumentationVersionOutput = @import("documentation_version.zig").DocumentationVersion;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDocumentationVersionInput, options: CallOptions) !CreateDocumentationVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDocumentationVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.documentation_version), input.documentation_version, allocator, &body_buf);
    has_prev = true;
