...
pub const UpdateRestApiOutput = @import("rest_api.zig").RestApi;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRestApiInput, options: CallOptions) !UpdateRestApiOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRestApiInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRestApiOutput {
    var result: UpdateRestApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRestApiOutput, body, allocator);
    }
    _ = status;
    _ = headers;
