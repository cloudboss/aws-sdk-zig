...
const GetMethodResponseOutput = @import("method_response.zig").GetMethodResponseOutput;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMethodResponseInput, options: Options) !GetMethodResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMethodResponseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMethodResponseOutput {
    var result: GetMethodResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMethodResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
