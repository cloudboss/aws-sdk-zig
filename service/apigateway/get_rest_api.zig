...
const GetRestApiOutput = @import("rest_api.zig").GetRestApiOutput;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRestApiInput, options: Options) !GetRestApiOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRestApiInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRestApiOutput {
    var result: GetRestApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRestApiOutput, body, alloc);
    }
    _ = status;
    _ = headers;
