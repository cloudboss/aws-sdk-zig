...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRequestValidatorsInput, options: Options) !GetRequestValidatorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRequestValidatorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRequestValidatorsOutput {
    var result: GetRequestValidatorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRequestValidatorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
