...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDocumentationPartsInput, options: Options) !GetDocumentationPartsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDocumentationPartsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/documentation/parts");
    ...
    if (input.location_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "locationStatus=");
    ...
    if (input.path) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "path=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.position) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDocumentationPartsOutput {
    var result: GetDocumentationPartsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDocumentationPartsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
