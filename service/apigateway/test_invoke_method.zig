...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestInvokeMethodInput, options: Options) !TestInvokeMethodOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestInvokeMethodInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.client_certificate_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"clientCertificateId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.headers) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"headers\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.multi_value_headers) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"multiValueHeaders\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.path_with_query_string) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"pathWithQueryString\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.stage_variables) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"stageVariables\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestInvokeMethodOutput {
    var result: TestInvokeMethodOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TestInvokeMethodOutput, body, alloc);
    }
    _ = status;
    _ = headers;
