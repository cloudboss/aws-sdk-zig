...
const UpdateMethodResponseOutput = @import("method_response.zig").UpdateMethodResponseOutput;
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMethodResponseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMethodResponseOutput {
    var result: UpdateMethodResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMethodResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
