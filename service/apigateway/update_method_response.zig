...
pub const UpdateMethodResponseInput = struct {
...
pub const UpdateMethodResponseOutput = @import("method_response.zig").MethodResponse;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMethodResponseInput, options: CallOptions) !UpdateMethodResponseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateMethodResponseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateMethodResponseOutput {
    var result: UpdateMethodResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMethodResponseOutput, body, allocator);
    }
    _ = status;
    _ = headers;
