...
const UpdateModelOutput = @import("model.zig").UpdateModelOutput;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateModelInput, options: Options) !UpdateModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.model_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateModelOutput {
    var result: UpdateModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
