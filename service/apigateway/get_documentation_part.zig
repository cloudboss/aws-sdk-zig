...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDocumentationPartInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.documentation_part_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDocumentationPartOutput {
    var result: GetDocumentationPartOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDocumentationPartOutput, body, allocator);
    }
    _ = status;
    _ = headers;
