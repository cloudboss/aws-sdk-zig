...
pub const UpdateUsagePlanOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUsagePlanInput, options: Options) !UpdateUsagePlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUsagePlanInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUsagePlanOutput {
    var result: UpdateUsagePlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateUsagePlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
