...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBasePathMappingsInput, options: Options) !GetBasePathMappingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBasePathMappingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/basepathmappings");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBasePathMappingsOutput {
    var result: GetBasePathMappingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBasePathMappingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
