...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSdkInput, options: Options) !GetSdkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSdkInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sdks/");
    try path_buf.appendSlice(alloc, input.sdk_type);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSdkOutput {
