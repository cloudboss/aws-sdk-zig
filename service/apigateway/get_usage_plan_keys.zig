...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUsagePlanKeysInput, options: Options) !GetUsagePlanKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUsagePlanKeysInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUsagePlanKeysOutput {
    var result: GetUsagePlanKeysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUsagePlanKeysOutput, body, alloc);
    }
    _ = status;
    _ = headers;
