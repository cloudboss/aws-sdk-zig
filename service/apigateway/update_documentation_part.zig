...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDocumentationPartInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDocumentationPartOutput {
    var result: UpdateDocumentationPartOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDocumentationPartOutput, body, alloc);
    }
    _ = status;
    _ = headers;
