...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateReviewTemplateLensReviewInput, options: Options) !UpdateReviewTemplateLensReviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateReviewTemplateLensReviewInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.lens_notes) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"LensNotes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.pillar_notes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PillarNotes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateReviewTemplateLensReviewOutput {
    var result: UpdateReviewTemplateLensReviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateReviewTemplateLensReviewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
