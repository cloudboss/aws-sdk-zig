...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProfileNotificationsInput, options: Options) !ListProfileNotificationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProfileNotificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.workload_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "WorkloadId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProfileNotificationsOutput {
    var result: ListProfileNotificationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProfileNotificationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
