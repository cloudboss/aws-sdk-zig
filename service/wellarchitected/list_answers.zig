...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnswersInput, options: Options) !ListAnswersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAnswersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAnswersOutput {
    var result: ListAnswersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAnswersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
