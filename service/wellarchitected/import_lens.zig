...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportLensInput, options: Options) !ImportLensOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportLensInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"JSONString\":");
    try aws.json.writeValue(@TypeOf(input.json_string), input.json_string, alloc, &body_buf);
    has_prev = true;
    if (input.lens_alias) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LensAlias\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportLensOutput {
    var result: ImportLensOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportLensOutput, body, alloc);
    }
    _ = status;
    _ = headers;
