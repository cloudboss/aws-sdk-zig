...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLensReviewInput, options: CallOptions) !GetLensReviewOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLensReviewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLensReviewOutput {
    var result: GetLensReviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLensReviewOutput, body, allocator);
    }
    _ = status;
    _ = headers;
