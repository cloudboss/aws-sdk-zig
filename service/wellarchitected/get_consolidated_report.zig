...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConsolidatedReportInput, options: Options) !GetConsolidatedReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConsolidatedReportInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_shared_resources) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "IncludeSharedResources=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConsolidatedReportOutput {
    var result: GetConsolidatedReportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConsolidatedReportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
