...
const IntegratingService = @import("integrating_service.zig").IntegratingService;

pub const UpdateIntegrationInput = struct {
    client_request_token: []const u8,

    /// Which integrated service to update.
    integrating_service: IntegratingService,
...
const UpdateIntegrationOutput = struct {};
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/updateIntegration");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"IntegratingService\":");
    try aws.json.writeValue(@TypeOf(input.integrating_service), input.integrating_service, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIntegrationOutput {
    ...
    _ = status;
    _ = headers;
    const result: UpdateIntegrationOutput = .{};
