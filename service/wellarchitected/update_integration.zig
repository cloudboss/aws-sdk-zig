...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/updateIntegration");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"IntegratingService\":");
    try aws.json.writeValue(@TypeOf(input.integrating_service), input.integrating_service, allocator, &body_buf);
    has_prev = true;
