...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkloadShareInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkloadShareOutput {
    var result: UpdateWorkloadShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateWorkloadShareOutput, body, alloc);
    }
    _ = status;
    _ = headers;
