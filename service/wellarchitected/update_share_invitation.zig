...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateShareInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/shareInvitations/");
    try path_buf.appendSlice(alloc, input.share_invitation_id);
    ...
    try body_buf.appendSlice(alloc, "\"ShareInvitationAction\":");
    try aws.json.writeValue(@TypeOf(input.share_invitation_action), input.share_invitation_action, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateShareInvitationOutput {
    var result: UpdateShareInvitationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateShareInvitationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
