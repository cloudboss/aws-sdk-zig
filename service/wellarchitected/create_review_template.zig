...
pub const CreateReviewTemplateInput = struct {
...
pub const CreateReviewTemplateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReviewTemplateInput, options: Options) !CreateReviewTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReviewTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReviewTemplateOutput {
    var result: CreateReviewTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateReviewTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
