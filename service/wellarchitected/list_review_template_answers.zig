...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReviewTemplateAnswersInput, options: Options) !ListReviewTemplateAnswersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReviewTemplateAnswersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReviewTemplateAnswersOutput {
    var result: ListReviewTemplateAnswersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReviewTemplateAnswersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
