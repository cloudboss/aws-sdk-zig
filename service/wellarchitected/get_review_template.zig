...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReviewTemplateInput, options: Options) !GetReviewTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReviewTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReviewTemplateOutput {
    var result: GetReviewTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReviewTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
