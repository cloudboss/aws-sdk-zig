...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAnswerInput, options: Options) !UpdateAnswerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAnswerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAnswerOutput {
    var result: UpdateAnswerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAnswerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
