...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLensReviewImprovementsInput, options: Options) !ListLensReviewImprovementsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLensReviewImprovementsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/improvements");
    ...
    if (input.pillar_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "PillarId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.question_priority) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "QuestionPriority=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLensReviewImprovementsOutput {
    var result: ListLensReviewImprovementsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLensReviewImprovementsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
