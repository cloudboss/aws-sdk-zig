...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLensReviewInput, options: Options) !UpdateLensReviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLensReviewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLensReviewOutput {
    var result: UpdateLensReviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateLensReviewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
