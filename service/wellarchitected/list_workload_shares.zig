...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkloadSharesInput, options: Options) !ListWorkloadSharesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkloadSharesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkloadSharesOutput {
    var result: ListWorkloadSharesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkloadSharesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
