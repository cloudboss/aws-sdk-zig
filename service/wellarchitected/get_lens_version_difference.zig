...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLensVersionDifferenceInput, options: Options) !GetLensVersionDifferenceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLensVersionDifferenceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/lenses/");
    try path_buf.appendSlice(alloc, input.lens_alias);
    try path_buf.appendSlice(alloc, "/versionDifference");
    ...
    if (input.base_lens_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "BaseLensVersion=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.target_lens_version) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "TargetLensVersion=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLensVersionDifferenceOutput {
    var result: GetLensVersionDifferenceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLensVersionDifferenceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
