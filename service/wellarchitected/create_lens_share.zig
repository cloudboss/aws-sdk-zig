...
pub const CreateLensShareInput = struct {
...
pub const CreateLensShareOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLensShareInput, options: CallOptions) !CreateLensShareOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLensShareInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLensShareOutput {
    var result: CreateLensShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLensShareOutput, body, allocator);
    }
    _ = status;
    _ = headers;
