...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMilestoneInput, options: CallOptions) !CreateMilestoneOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateMilestoneInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"MilestoneName\":");
    try aws.json.writeValue(@TypeOf(input.milestone_name), input.milestone_name, allocator, &body_buf);
    has_prev = true;
