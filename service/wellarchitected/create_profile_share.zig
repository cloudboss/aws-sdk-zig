...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProfileShareInput, options: CallOptions) !CreateProfileShareOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateProfileShareInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateProfileShareOutput {
    var result: CreateProfileShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProfileShareOutput, body, allocator);
    }
    _ = status;
    _ = headers;
