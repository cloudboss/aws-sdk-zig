...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListShareInvitationsInput, options: Options) !ListShareInvitationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListShareInvitationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.lens_name_prefix) |v| {
        ...
        try query_buf.appendSlice(alloc, "LensNamePrefix=");
    ...
    if (input.share_resource_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ShareResourceType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.template_name_prefix) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "TemplateNamePrefix=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.workload_name_prefix) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "WorkloadNamePrefix=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListShareInvitationsOutput {
    var result: ListShareInvitationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListShareInvitationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
