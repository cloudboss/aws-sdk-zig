...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLensVersionInput, options: Options) !CreateLensVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLensVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.is_major_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IsMajorVersion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"LensVersion\":");
    try aws.json.writeValue(@TypeOf(input.lens_version), input.lens_version, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLensVersionOutput {
    var result: CreateLensVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLensVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
