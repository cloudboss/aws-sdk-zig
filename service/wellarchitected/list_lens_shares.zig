...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLensSharesInput, options: Options) !ListLensSharesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLensSharesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLensSharesOutput {
    var result: ListLensSharesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLensSharesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
