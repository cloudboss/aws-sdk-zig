...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateReviewTemplateAnswerInput, options: CallOptions) !UpdateReviewTemplateAnswerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateReviewTemplateAnswerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateReviewTemplateAnswerOutput {
    var result: UpdateReviewTemplateAnswerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateReviewTemplateAnswerOutput, body, allocator);
    }
    _ = status;
    _ = headers;
