...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutProfileOutboundRequestBatchInput, options: Options) !PutProfileOutboundRequestBatchOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutProfileOutboundRequestBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/profile-outbound-requests");
    ...
    try body_buf.appendSlice(alloc, "\"profileOutboundRequests\":");
    try aws.json.writeValue(@TypeOf(input.profile_outbound_requests), input.profile_outbound_requests, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutProfileOutboundRequestBatchOutput {
    var result: PutProfileOutboundRequestBatchOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutProfileOutboundRequestBatchOutput, body, alloc);
    }
    _ = status;
    _ = headers;
