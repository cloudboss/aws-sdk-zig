...
fn serializeRequest(alloc: std.mem.Allocator, input: PutOutboundRequestBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/outbound-requests");
    ...
    try body_buf.appendSlice(alloc, "\"outboundRequests\":");
    try aws.json.writeValue(@TypeOf(input.outbound_requests), input.outbound_requests, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutOutboundRequestBatchOutput {
    var result: PutOutboundRequestBatchOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutOutboundRequestBatchOutput, body, alloc);
    }
    _ = status;
    _ = headers;
