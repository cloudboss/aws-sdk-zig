...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInstanceOnboardingJobStatusInput, options: Options) !GetInstanceOnboardingJobStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInstanceOnboardingJobStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInstanceOnboardingJobStatusOutput {
    var result: GetInstanceOnboardingJobStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInstanceOnboardingJobStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
