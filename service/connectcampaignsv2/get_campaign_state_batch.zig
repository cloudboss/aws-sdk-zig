...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCampaignStateBatchInput, options: Options) !GetCampaignStateBatchOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCampaignStateBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"campaignIds\":");
    try aws.json.writeValue(@TypeOf(input.campaign_ids), input.campaign_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCampaignStateBatchOutput {
    var result: GetCampaignStateBatchOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCampaignStateBatchOutput, body, alloc);
    }
    _ = status;
    _ = headers;
