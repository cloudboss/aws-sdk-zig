...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUserAccessLoggingSettingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUserAccessLoggingSettingsOutput {
    var result: GetUserAccessLoggingSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUserAccessLoggingSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
