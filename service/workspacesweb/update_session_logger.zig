...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSessionLoggerInput, options: Options) !UpdateSessionLoggerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSessionLoggerInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.event_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"eventFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSessionLoggerOutput {
    var result: UpdateSessionLoggerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSessionLoggerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
