...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataLakeOrganizationConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataLakeOrganizationConfigurationOutput {
    var result: GetDataLakeOrganizationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataLakeOrganizationConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
