...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSubscriberInput, options: Options) !GetSubscriberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSubscriberInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSubscriberOutput {
    var result: GetSubscriberOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSubscriberOutput, body, alloc);
    }
    _ = status;
    _ = headers;
