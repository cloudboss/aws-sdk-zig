...
pub const CreateCustomLogSourceInput = struct {
    ...
    /// supported event classes, see the [Amazon Security Lake User
...
pub const CreateCustomLogSourceOutput = struct {
    /// The third-party custom source that was created.
    source: ?CustomLogSourceResource = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomLogSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/datalake/logsources/custom";
    ...
    has_prev = true;
    if (input.event_classes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"eventClasses\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sourceName\":");
    try aws.json.writeValue(@TypeOf(input.source_name), input.source_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomLogSourceOutput {
    var result: CreateCustomLogSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCustomLogSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
