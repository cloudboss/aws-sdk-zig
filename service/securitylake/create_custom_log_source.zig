...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateCustomLogSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.event_classes) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"eventClasses\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"sourceName\":");
    try aws.json.writeValue(@TypeOf(input.source_name), input.source_name, allocator, &body_buf);
    has_prev = true;
