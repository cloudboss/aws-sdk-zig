...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSubscriberNotificationInput, options: Options) !CreateSubscriberNotificationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSubscriberNotificationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSubscriberNotificationOutput {
    var result: CreateSubscriberNotificationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSubscriberNotificationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
