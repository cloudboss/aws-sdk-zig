...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataLakeExceptionSubscriptionInput, options: Options) !GetDataLakeExceptionSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataLakeExceptionSubscriptionInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataLakeExceptionSubscriptionOutput {
    var result: GetDataLakeExceptionSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataLakeExceptionSubscriptionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
