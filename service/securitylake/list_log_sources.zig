...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLogSourcesInput, options: Options) !ListLogSourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLogSourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLogSourcesOutput {
    var result: ListLogSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLogSourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
