...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAwsLogSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAwsLogSourceOutput {
    var result: DeleteAwsLogSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAwsLogSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
