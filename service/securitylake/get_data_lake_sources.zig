...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataLakeSourcesInput, options: Options) !GetDataLakeSourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataLakeSourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataLakeSourcesOutput {
    var result: GetDataLakeSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataLakeSourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
