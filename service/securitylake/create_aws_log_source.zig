...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAwsLogSourceInput, options: Options) !CreateAwsLogSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAwsLogSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAwsLogSourceOutput {
    var result: CreateAwsLogSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAwsLogSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
