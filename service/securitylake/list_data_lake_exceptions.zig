...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataLakeExceptionsInput, options: Options) !ListDataLakeExceptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataLakeExceptionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataLakeExceptionsOutput {
    var result: ListDataLakeExceptionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataLakeExceptionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
