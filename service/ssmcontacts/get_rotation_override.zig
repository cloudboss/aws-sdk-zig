...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRotationOverrideInput, options: Options) !GetRotationOverrideOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRotationOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.GetRotationOverride");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRotationOverrideOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRotationOverrideOutput, body, alloc);
}
