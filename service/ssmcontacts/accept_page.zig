...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptPageInput, options: Options) !AcceptPageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptPageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.AcceptPage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptPageOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
