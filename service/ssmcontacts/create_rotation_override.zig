...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRotationOverrideInput, options: Options) !CreateRotationOverrideOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRotationOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.CreateRotationOverride");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRotationOverrideOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateRotationOverrideOutput, body, alloc);
}
