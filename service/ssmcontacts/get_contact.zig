...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContactInput, options: Options) !GetContactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.GetContact");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContactOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetContactOutput, body, alloc);
}
