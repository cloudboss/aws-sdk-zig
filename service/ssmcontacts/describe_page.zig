...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePageInput, options: Options) !DescribePageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.DescribePage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePageOutput, body, alloc);
}
