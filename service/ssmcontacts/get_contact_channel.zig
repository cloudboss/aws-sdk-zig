...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContactChannelInput, options: Options) !GetContactChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContactChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.GetContactChannel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContactChannelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetContactChannelOutput, body, alloc);
}
