...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRotationInput, options: Options) !CreateRotationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRotationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.CreateRotation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRotationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateRotationOutput, body, alloc);
}
