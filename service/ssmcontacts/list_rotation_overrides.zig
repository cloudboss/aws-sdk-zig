...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRotationOverridesInput, options: Options) !ListRotationOverridesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRotationOverridesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.ListRotationOverrides");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRotationOverridesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRotationOverridesOutput, body, alloc);
}
