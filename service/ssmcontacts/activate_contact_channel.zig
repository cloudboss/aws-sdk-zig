...
fn serializeRequest(allocator: std.mem.Allocator, input: ActivateContactChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SSMContacts.ActivateContactChannel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ActivateContactChannelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
