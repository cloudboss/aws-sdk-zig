...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContactChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.UpdateContactChannel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContactChannelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
