...
pub const UpdateContactChannelInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContactChannelInput, options: Options) !UpdateContactChannelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateContactChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SSMContacts.UpdateContactChannel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateContactChannelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
