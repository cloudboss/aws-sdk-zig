...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartEngagementInput, options: Options) !StartEngagementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartEngagementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.StartEngagement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartEngagementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartEngagementOutput, body, alloc);
}
