...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEngagementInput, options: Options) !DescribeEngagementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEngagementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.DescribeEngagement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEngagementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEngagementOutput, body, alloc);
}
