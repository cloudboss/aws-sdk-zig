...
pub const ListRotationShiftsInput = struct {
...
pub const ListRotationShiftsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRotationShiftsInput, options: CallOptions) !ListRotationShiftsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRotationShiftsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SSMContacts.ListRotationShifts");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRotationShiftsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRotationShiftsOutput, body, allocator);
}
