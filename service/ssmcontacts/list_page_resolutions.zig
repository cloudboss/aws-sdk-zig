...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPageResolutionsInput, options: Options) !ListPageResolutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPageResolutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SSMContacts.ListPageResolutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPageResolutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPageResolutionsOutput, body, alloc);
}
