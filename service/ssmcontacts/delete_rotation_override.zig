...
pub const DeleteRotationOverrideInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRotationOverrideInput, options: Options) !DeleteRotationOverrideOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRotationOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SSMContacts.DeleteRotationOverride");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRotationOverrideOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
