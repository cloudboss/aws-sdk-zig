...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePortalInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.authorization), input.authorization, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"EndpointConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.endpoint_configuration), input.endpoint_configuration, alloc, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.portal_content), input.portal_content, alloc, &body_buf);
    has_prev = true;
