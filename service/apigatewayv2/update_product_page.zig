...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProductPageInput, options: Options) !UpdateProductPageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProductPageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.product_page_id);
    ...
    if (input.display_content) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DisplayContent\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProductPageOutput {
    var result: UpdateProductPageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProductPageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
