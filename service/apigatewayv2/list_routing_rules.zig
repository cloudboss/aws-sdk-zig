...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoutingRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoutingRulesOutput {
    var result: ListRoutingRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoutingRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
