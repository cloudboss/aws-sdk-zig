...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRoutingRuleInput, options: Options) !GetRoutingRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRoutingRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRoutingRuleOutput {
    var result: GetRoutingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRoutingRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
