...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProductPageInput, options: Options) !GetProductPageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProductPageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProductPageOutput {
    var result: GetProductPageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProductPageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
