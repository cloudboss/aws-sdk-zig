...
pub const CreateRouteResponseInput = struct {
    /// The API identifier.
    api_id: []const u8,
...
pub const CreateRouteResponseOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRouteResponseInput, options: Options) !CreateRouteResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRouteResponseInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RouteResponseKey\":");
    try aws.json.writeValue(@TypeOf(input.route_response_key), input.route_response_key, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRouteResponseOutput {
    var result: CreateRouteResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRouteResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
