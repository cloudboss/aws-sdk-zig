...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPortalInput, options: Options) !GetPortalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPortalInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPortalOutput {
    var result: GetPortalOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPortalOutput, body, alloc);
    }
    _ = status;
    _ = headers;
