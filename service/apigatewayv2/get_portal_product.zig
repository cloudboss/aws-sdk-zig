...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPortalProductInput, options: Options) !GetPortalProductOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPortalProductInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPortalProductOutput {
    var result: GetPortalProductOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPortalProductOutput, body, alloc);
    }
    _ = status;
    _ = headers;
