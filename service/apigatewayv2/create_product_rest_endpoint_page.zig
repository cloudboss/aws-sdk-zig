...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProductRestEndpointPageInput, options: CallOptions) !CreateProductRestEndpointPageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateProductRestEndpointPageInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"RestEndpointIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.rest_endpoint_identifier), input.rest_endpoint_identifier, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateProductRestEndpointPageOutput {
    var result: CreateProductRestEndpointPageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProductRestEndpointPageOutput, body, allocator);
    }
    _ = status;
    _ = headers;
