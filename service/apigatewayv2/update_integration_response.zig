...
pub const UpdateIntegrationResponseInput = struct {
    /// The API identifier.
    api_id: []const u8,
...
pub const UpdateIntegrationResponseOutput = struct {
    /// Supported only for WebSocket APIs. Specifies how to handle response payload
    /// content type conversions. Supported values are CONVERT_TO_BINARY and
    /// CONVERT_TO_TEXT, with the following behaviors:
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIntegrationResponseInput, options: Options) !UpdateIntegrationResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIntegrationResponseInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/integrations/");
    try path_buf.appendSlice(alloc, input.integration_id);
    try path_buf.appendSlice(alloc, "/integrationresponses/");
    try path_buf.appendSlice(alloc, input.integration_response_id);
    ...
    if (input.content_handling_strategy) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ContentHandlingStrategy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.integration_response_key) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IntegrationResponseKey\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.template_selection_expression) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TemplateSelectionExpression\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIntegrationResponseOutput {
    var result: UpdateIntegrationResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIntegrationResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
