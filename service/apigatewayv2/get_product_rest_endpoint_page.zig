...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProductRestEndpointPageInput, options: Options) !GetProductRestEndpointPageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProductRestEndpointPageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.product_rest_endpoint_page_id);
    ...
    if (input.include_raw_display_content) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeRawDisplayContent=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProductRestEndpointPageOutput {
    var result: GetProductRestEndpointPageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProductRestEndpointPageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
