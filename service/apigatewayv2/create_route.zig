...
pub const CreateRouteInput = @import("create_route_request.zig").CreateRouteRequest;

pub const CreateRouteOutput = @import("create_route_result.zig").CreateRouteResult;
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRouteInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.route_key), input.route_key, allocator, &body_buf);
    has_prev = true;
