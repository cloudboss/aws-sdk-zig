...
pub const ReimportApiInput = struct {
    /// The API identifier.
    api_id: []const u8,
...
pub const ReimportApiOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReimportApiInput, options: Options) !ReimportApiOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReimportApiInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.basepath) |v| {
        ...
        try query_buf.appendSlice(alloc, "basepath=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReimportApiOutput {
    var result: ReimportApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ReimportApiOutput, body, alloc);
    }
    _ = status;
    _ = headers;
