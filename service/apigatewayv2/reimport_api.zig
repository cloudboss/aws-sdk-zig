...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReimportApiInput, options: CallOptions) !ReimportApiOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ReimportApiInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ReimportApiOutput {
    var result: ReimportApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ReimportApiOutput, body, allocator);
    }
    _ = status;
    _ = headers;
