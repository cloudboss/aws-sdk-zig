...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePortalProductInput, options: CallOptions) !UpdatePortalProductOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePortalProductInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePortalProductOutput {
    var result: UpdatePortalProductOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePortalProductOutput, body, allocator);
    }
    _ = status;
    _ = headers;
