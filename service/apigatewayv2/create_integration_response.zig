...
pub const CreateIntegrationResponseInput = struct {
    /// The API identifier.
    api_id: []const u8,
...
pub const CreateIntegrationResponseOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIntegrationResponseInput, options: Options) !CreateIntegrationResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIntegrationResponseInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.integration_response_key), input.integration_response_key, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIntegrationResponseOutput {
    var result: CreateIntegrationResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIntegrationResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
