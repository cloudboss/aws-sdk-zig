...
pub const CreateProductPageInput = struct {
...
pub const CreateProductPageOutput = struct {
    /// The content of the product page.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProductPageInput, options: CallOptions) !CreateProductPageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateProductPageInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.display_content), input.display_content, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateProductPageOutput {
    var result: CreateProductPageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProductPageOutput, body, allocator);
    }
    _ = status;
    _ = headers;
