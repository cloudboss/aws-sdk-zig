...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIntegrationResponseInput, options: Options) !GetIntegrationResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIntegrationResponseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIntegrationResponseOutput {
    var result: GetIntegrationResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIntegrationResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
