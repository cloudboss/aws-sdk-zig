...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouteResponseInput, options: Options) !GetRouteResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRouteResponseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRouteResponseOutput {
    var result: GetRouteResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouteResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
