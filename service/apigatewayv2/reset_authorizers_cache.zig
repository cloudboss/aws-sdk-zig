...
pub const ResetAuthorizersCacheInput = struct {
    /// The API identifier.
    api_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetAuthorizersCacheInput, options: CallOptions) !ResetAuthorizersCacheOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResetAuthorizersCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/cache/authorizers");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResetAuthorizersCacheOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ResetAuthorizersCacheOutput = .{};
