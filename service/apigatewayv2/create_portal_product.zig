...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePortalProductInput, options: Options) !CreatePortalProductOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePortalProductInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePortalProductOutput {
    var result: CreatePortalProductOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePortalProductOutput, body, alloc);
    }
    _ = status;
    _ = headers;
