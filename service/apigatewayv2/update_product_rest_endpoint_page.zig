...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProductRestEndpointPageInput, options: Options) !UpdateProductRestEndpointPageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProductRestEndpointPageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProductRestEndpointPageOutput {
    var result: UpdateProductRestEndpointPageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProductRestEndpointPageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
