...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRoutingRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRoutingRuleOutput {
    var result: PutRoutingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutRoutingRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
