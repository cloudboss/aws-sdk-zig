...
pub const PublishPortalInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PublishPortalInput, options: Options) !PublishPortalOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PublishPortalInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PublishPortalOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PublishPortalOutput = .{};
