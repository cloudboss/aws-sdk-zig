...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRouteResponseInput, options: Options) !UpdateRouteResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRouteResponseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRouteResponseOutput {
    var result: UpdateRouteResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRouteResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
