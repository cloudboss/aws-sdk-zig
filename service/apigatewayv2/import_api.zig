...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportApiInput, options: Options) !ImportApiOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportApiInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportApiOutput {
    var result: ImportApiOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportApiOutput, body, alloc);
    }
    _ = status;
    _ = headers;
