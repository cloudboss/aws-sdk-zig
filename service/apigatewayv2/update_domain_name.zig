...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDomainNameInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.domain_name_configurations) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DomainNameConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.mutual_tls_authentication) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MutualTlsAuthentication\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.routing_mode) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RoutingMode\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
