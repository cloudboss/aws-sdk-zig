...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkflowDefinitionInput, options: Options) !CreateWorkflowDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkflowDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.export_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"exportConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkflowDefinitionOutput {
    var result: CreateWorkflowDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWorkflowDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
