...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelsInput, options: Options) !ListModelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "clientCompatibilityVersion=");
    {
        const num_str = std.fmt.allocPrint(alloc, "{d}", .{input.client_compatibility_version}) catch "";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelsOutput {
    var result: ListModelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListModelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
