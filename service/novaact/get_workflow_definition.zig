...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkflowDefinitionInput, options: Options) !GetWorkflowDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkflowDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkflowDefinitionOutput {
    var result: GetWorkflowDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkflowDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
