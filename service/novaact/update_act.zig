...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateActInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("novaact", "Nova Act", allocator);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateActOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
