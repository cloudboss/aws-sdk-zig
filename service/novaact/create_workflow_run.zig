...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkflowRunInput, options: Options) !CreateWorkflowRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkflowRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"clientInfo\":");
    try aws.json.writeValue(@TypeOf(input.client_info), input.client_info, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkflowRunOutput {
    var result: CreateWorkflowRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWorkflowRunOutput, body, alloc);
    }
    _ = status;
    _ = headers;
