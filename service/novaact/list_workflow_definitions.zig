...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkflowDefinitionsInput, options: Options) !ListWorkflowDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkflowDefinitionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkflowDefinitionsOutput {
    var result: ListWorkflowDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkflowDefinitionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
