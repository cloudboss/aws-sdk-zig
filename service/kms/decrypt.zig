...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DecryptInput, options: Options) !DecryptOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DecryptInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.Decrypt");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DecryptOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DecryptOutput, body, alloc);
}
