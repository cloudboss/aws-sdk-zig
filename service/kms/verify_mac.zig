...
pub const VerifyMacInput = struct {
    ...
    /// algorithm as the values
    /// specified in this request.
    mac: []const u8,

    /// The MAC algorithm that will be used in the verification. Enter the same MAC
    /// algorithm that
    /// was used to compute the HMAC. This algorithm must be supported by the HMAC
    /// KMS key identified
    ...
    /// for message digests. If you generated an HMAC for a hash digest of a
    /// message, you must verify
...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyMacInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.VerifyMac");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyMacOutput {
    ...
    return aws.json.parseJsonObject(VerifyMacOutput, body, alloc);
}
