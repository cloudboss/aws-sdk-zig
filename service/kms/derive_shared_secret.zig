...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeriveSharedSecretInput, options: Options) !DeriveSharedSecretOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeriveSharedSecretInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.DeriveSharedSecret");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeriveSharedSecretOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeriveSharedSecretOutput, body, alloc);
}
