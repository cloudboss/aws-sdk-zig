...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePrimaryRegionInput, options: Options) !UpdatePrimaryRegionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePrimaryRegionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.UpdatePrimaryRegion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePrimaryRegionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
