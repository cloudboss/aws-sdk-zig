...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteImportedKeyMaterialInput, options: Options) !DeleteImportedKeyMaterialOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteImportedKeyMaterialInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.DeleteImportedKeyMaterial");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteImportedKeyMaterialOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteImportedKeyMaterialOutput, body, alloc);
}
