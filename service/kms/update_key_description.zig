...
pub const UpdateKeyDescriptionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKeyDescriptionInput, options: CallOptions) !UpdateKeyDescriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateKeyDescriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.UpdateKeyDescription");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateKeyDescriptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
