...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKeyDescriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.UpdateKeyDescription");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKeyDescriptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
