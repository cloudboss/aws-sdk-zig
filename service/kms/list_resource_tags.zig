...
pub const ListResourceTagsInput = struct {
...
pub const ListResourceTagsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceTagsInput, options: CallOptions) !ListResourceTagsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListResourceTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.ListResourceTags");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListResourceTagsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceTagsOutput, body, allocator);
}
