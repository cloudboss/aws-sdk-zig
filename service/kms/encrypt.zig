...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EncryptInput, options: Options) !EncryptOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EncryptInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.Encrypt");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EncryptOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(EncryptOutput, body, alloc);
}
