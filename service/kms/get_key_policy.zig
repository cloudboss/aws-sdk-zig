...
pub const GetKeyPolicyInput = struct {
...
pub const GetKeyPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetKeyPolicyInput, options: CallOptions) !GetKeyPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetKeyPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.GetKeyPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetKeyPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetKeyPolicyOutput, body, allocator);
}
