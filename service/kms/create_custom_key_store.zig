...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomKeyStoreInput, options: Options) !CreateCustomKeyStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomKeyStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.CreateCustomKeyStore");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomKeyStoreOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCustomKeyStoreOutput, body, alloc);
}
