...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RotateKeyOnDemandInput, options: Options) !RotateKeyOnDemandOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RotateKeyOnDemandInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.RotateKeyOnDemand");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RotateKeyOnDemandOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RotateKeyOnDemandOutput, body, alloc);
}
