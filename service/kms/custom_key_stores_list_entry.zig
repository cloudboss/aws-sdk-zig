const ConnectionErrorCodeType = @import("connection_error_code_type.zig").ConnectionErrorCodeType;
const ConnectionStateType = @import("connection_state_type.zig").ConnectionStateType;
const CustomKeyStoreType = @import("custom_key_store_type.zig").CustomKeyStoreType;
const XksProxyConfigurationType = @import("xks_proxy_configuration_type.zig").XksProxyConfigurationType;

/// Contains information about each custom key store in the custom key store
/// list.
pub const CustomKeyStoresListEntry = struct {
    ...
    /// the errors (UpdateCustomKeyStore), and try to connect again
    /// (ConnectCustomKeyStore). For additional help resolving these errors, see
    /// [How to Fix a
    /// Connection
    /// Failure](https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-failed) in *Key Management Service Developer Guide*.
    ///
    ...
    /// addresses. A CloudHSM
    /// key store connection requires one free IP address in each of the associated
    /// private
    /// subnets, although two are preferable. For details, see [How to Fix a
    /// Connection
    /// Failure](https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-failed) in the *Key Management Service Developer Guide*.
    ///
    ...
    /// user in the CloudHSM cluster. Before you can connect your custom key store
    /// to its CloudHSM
    ...
    /// attempts to
    /// connect the custom key store to the CloudHSM cluster fail. To fix this
    /// error, create a
    /// cluster from a recent backup and associate it with your custom key store.
    /// (This process
    ...
    /// connect your custom key store to its CloudHSM cluster, you must log the
    /// `kmsuser`
    ...
    /// Reconnect](https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#login-kmsuser-2) in the *Key Management Service Developer Guide*.
    ///
    /// * `USER_NOT_FOUND` — KMS cannot find a `kmsuser` CU
    /// account in the associated CloudHSM cluster. Before you can connect your
    /// custom key store to
    /// its CloudHSM cluster, you must create a `kmsuser` CU account in the cluster,
    /// and
    ...
    /// that the XKS
    /// proxy supports TLS 1.2 or 1.3. Also, verify that the TLS certificate is not
    /// expired, and
    /// that it matches the hostname in the `XksProxyUriEndpoint` value, and that it
    /// is
    /// signed by a certificate authority included in the [Trusted Certificate
    /// Authorities](https://github.com/aws/aws-kms-xksproxy-api-spec/blob/main/TrustedCertificateAuthorities) list.
    ///
    ...
    /// proxy to verify that the proxy is active and available on its network. Also,
    /// verify that
    /// your external key manager instances are operating properly. Connection
    /// attempts fail with
    /// this connection error code if the proxy reports that all external key
    /// manager instances
    /// are unavailable.
    ///
    ...
    /// see this
    /// connection error code repeatedly, notify your external key store proxy
    /// vendor.
    ///
    ...
    ///
    /// * It must *not* require
    ///   [acceptance](https://docs.aws.amazon.com/vpc/latest/privatelink/create-endpoint-service.html) of connection requests.
    ///
    /// * It must have a private DNS name. The private DNS name for an external key
    ///   store with `VPC_ENDPOINT_SERVICE` connectivity
    /// must be unique in its Amazon Web Services Region.
    ///
    /// * The domain of the private DNS name must have a [verification
    ///   status](https://docs.aws.amazon.com/vpc/latest/privatelink/verify-domains.html) of
    /// `verified`.
    ///
    /// * The [TLS
    ///   certificate](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/create-tls-listener.html) specifies the private DNS hostname at which the endpoint is reachable.
    ///
    ...
    /// key store, the `ConnectionState` indicates whether it is connected to its
    /// CloudHSM
    /// cluster. For an external key store, the `ConnectionState` indicates whether
    /// it is
    /// connected to the external key store proxy that communicates with your
    /// external key
    /// manager.
    ///
    ...
    /// using the
    /// custom key store, make sure that the backing key store is reachable and
    /// active. For an CloudHSM
    /// key store, verify that its associated CloudHSM cluster is active and
    ...
    /// The
    /// `ConnectionErrorCode` field in the response indicates the cause of the
    /// failure.
    ...
    /// custom
    /// key store backed by an CloudHSM cluster. `EXTERNAL_KEY_STORE` indicates a
    /// custom key
    ...
    /// you [initialize
    /// the
    /// cluster](https://docs.aws.amazon.com/cloudhsm/latest/userguide/initialize-cluster.html#sign-csr), you create this certificate and save it in the
    /// `customerCA.crt` file.
    ///
