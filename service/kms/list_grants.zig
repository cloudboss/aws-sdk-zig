...
pub const ListGrantsInput = struct {
...
pub const ListGrantsOutput = struct {
    /// A list of grants.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGrantsInput, options: CallOptions) !ListGrantsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListGrantsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.ListGrants");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListGrantsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListGrantsOutput, body, allocator);
}
