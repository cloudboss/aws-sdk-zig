...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutKeyPolicyInput, options: Options) !PutKeyPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutKeyPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.PutKeyPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutKeyPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
