...
pub const SignInput = struct {
    ...
    /// `SIGN_VERIFY`. To find the `KeyUsage` of a KMS key, use the DescribeKey
    /// operation.
    ///
    ...
    /// verification failures when verifying with a system that assumes a single
    /// hash.
    ///
    ...
    /// KMS key. When signing with RSA key pairs, RSASSA-PSS algorithms are
    /// preferred. We include
...
pub const SignOutput = struct {
    /// The Amazon Resource Name ([key
    /// ARN](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-key-ARN)) of the asymmetric KMS key that was used to sign the message.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SignInput, options: CallOptions) !SignOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "kms");
...
fn serializeRequest(allocator: std.mem.Allocator, input: SignInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.Sign");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SignOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SignOutput, body, allocator);
}
