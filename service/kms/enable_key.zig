...
pub const EnableKeyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableKeyInput, options: CallOptions) !EnableKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.EnableKey");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableKeyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
