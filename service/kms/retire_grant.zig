...
pub const RetireGrantInput = struct {
    ...
    /// and [Eventual
    /// consistency](https://docs.aws.amazon.com/kms/latest/developerguide/grants.html#terms-eventual-consistency) in the *Key Management Service Developer Guide*.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RetireGrantInput, options: CallOptions) !RetireGrantOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RetireGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.RetireGrant");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RetireGrantOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
