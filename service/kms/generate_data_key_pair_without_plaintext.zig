...
pub const GenerateDataKeyPairWithoutPlaintextInput = struct {
...
pub const GenerateDataKeyPairWithoutPlaintextOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateDataKeyPairWithoutPlaintextInput, options: CallOptions) !GenerateDataKeyPairWithoutPlaintextOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GenerateDataKeyPairWithoutPlaintextInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.GenerateDataKeyPairWithoutPlaintext");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GenerateDataKeyPairWithoutPlaintextOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GenerateDataKeyPairWithoutPlaintextOutput, body, allocator);
}
