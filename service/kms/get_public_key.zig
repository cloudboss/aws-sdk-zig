...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPublicKeyInput, options: Options) !GetPublicKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPublicKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.GetPublicKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPublicKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPublicKeyOutput, body, alloc);
}
