...
pub const DisableKeyInput = struct {
...
pub const DisableKeyOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableKeyInput, options: CallOptions) !DisableKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.DisableKey");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableKeyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
