...
pub const DisconnectCustomKeyStoreInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisconnectCustomKeyStoreInput, options: Options) !DisconnectCustomKeyStoreOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisconnectCustomKeyStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.DisconnectCustomKeyStore");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisconnectCustomKeyStoreOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
