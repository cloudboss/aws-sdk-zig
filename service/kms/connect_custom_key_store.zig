...
pub const ConnectCustomKeyStoreInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConnectCustomKeyStoreInput, options: Options) !ConnectCustomKeyStoreOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ConnectCustomKeyStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.ConnectCustomKeyStore");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ConnectCustomKeyStoreOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
