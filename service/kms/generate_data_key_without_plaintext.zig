...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateDataKeyWithoutPlaintextInput, options: Options) !GenerateDataKeyWithoutPlaintextOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateDataKeyWithoutPlaintextInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.GenerateDataKeyWithoutPlaintext");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateDataKeyWithoutPlaintextOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GenerateDataKeyWithoutPlaintextOutput, body, alloc);
}
