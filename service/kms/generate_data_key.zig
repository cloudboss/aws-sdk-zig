...
pub const GenerateDataKeyInput = struct {
    ...
    /// to generate a
    /// 512-bit data key (64 bytes is 512 bits). For 128-bit (16-byte) and 256-bit
    /// (32-byte) data
    ...
    /// KMS encrypts
    /// the plaintext data key under the public key in the attestation document, and
    /// returns the
...
pub const GenerateDataKeyOutput = struct {
    ...
    /// this data key to encrypt your data outside of
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateDataKeyInput, options: CallOptions) !GenerateDataKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GenerateDataKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.GenerateDataKey");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GenerateDataKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GenerateDataKeyOutput, body, allocator);
}
