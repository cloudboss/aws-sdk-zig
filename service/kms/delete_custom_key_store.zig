...
pub const DeleteCustomKeyStoreInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCustomKeyStoreInput, options: Options) !DeleteCustomKeyStoreOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCustomKeyStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.DeleteCustomKeyStore");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCustomKeyStoreOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
