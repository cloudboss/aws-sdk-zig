...
pub const DescribeKeyInput = struct {
    ...
    /// Services alias with no key ID), KMS associates
    /// the alias with an [Amazon Web Services managed
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-managed-key) and returns its `KeyId` and `Arn` in the
    /// response.
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeKeyInput, options: Options) !DescribeKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.DescribeKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeKeyOutput, body, alloc);
}
