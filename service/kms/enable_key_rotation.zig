...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableKeyRotationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.EnableKeyRotation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableKeyRotationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
