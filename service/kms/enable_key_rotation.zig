...
pub const EnableKeyRotationInput = struct {
    ...
    /// key rotation
    /// that KMS will rotate your key material, and the number of days between each
    /// automatic
    /// rotation thereafter.
    ///
    /// You can use the [
    /// `kms:RotationPeriodInDays`
    /// ](https://docs.aws.amazon.com/kms/latest/developerguide/conditions-kms.html#conditions-kms-rotation-period-in-days) condition key to further constrain the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableKeyRotationInput, options: CallOptions) !EnableKeyRotationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableKeyRotationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.EnableKeyRotation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableKeyRotationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
