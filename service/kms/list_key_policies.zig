...
pub const ListKeyPoliciesInput = struct {
...
pub const ListKeyPoliciesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListKeyPoliciesInput, options: CallOptions) !ListKeyPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListKeyPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.ListKeyPolicies");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListKeyPoliciesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListKeyPoliciesOutput, body, allocator);
}
