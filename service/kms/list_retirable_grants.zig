...
const GrantListEntry = @import("grant_list_entry.zig").GrantListEntry;

pub const ListRetirableGrantsInput = struct {
...
pub const ListRetirableGrantsOutput = struct {
    /// A list of grants.
    grants: ?[]const GrantListEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRetirableGrantsInput, options: CallOptions) !ListRetirableGrantsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRetirableGrantsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.ListRetirableGrants");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRetirableGrantsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRetirableGrantsOutput, body, allocator);
}
