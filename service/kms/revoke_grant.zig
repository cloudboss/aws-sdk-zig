...
pub const RevokeGrantInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeGrantInput, options: CallOptions) !RevokeGrantOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RevokeGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.RevokeGrant");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RevokeGrantOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
