...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeGrantInput, options: Options) !RevokeGrantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RevokeGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.RevokeGrant");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RevokeGrantOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
