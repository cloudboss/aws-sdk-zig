...
pub const DisableKeyRotationInput = struct {
    /// Identifies a symmetric encryption KMS key. You cannot enable or disable
    /// automatic rotation
    /// of [asymmetric KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html#asymmetric-cmks), [HMAC
    /// KMS keys](https://docs.aws.amazon.com/kms/latest/developerguide/hmac.html),
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableKeyRotationInput, options: CallOptions) !DisableKeyRotationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableKeyRotationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "TrentService.DisableKeyRotation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableKeyRotationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
