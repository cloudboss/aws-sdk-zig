...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableKeyRotationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.DisableKeyRotation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableKeyRotationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
