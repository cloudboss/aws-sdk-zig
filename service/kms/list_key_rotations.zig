...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListKeyRotationsInput, options: Options) !ListKeyRotationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListKeyRotationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.ListKeyRotations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListKeyRotationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListKeyRotationsOutput, body, alloc);
}
