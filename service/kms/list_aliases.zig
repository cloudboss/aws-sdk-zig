...
const AliasListEntry = @import("alias_list_entry.zig").AliasListEntry;
...
pub const ListAliasesOutput = struct {
    /// A list of aliases.
    aliases: ?[]const AliasListEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAliasesInput, options: Options) !ListAliasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.ListAliases");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAliasesOutput {
    ...
    return aws.json.parseJsonObject(ListAliasesOutput, body, alloc);
}
