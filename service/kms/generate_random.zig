...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateRandomInput, options: Options) !GenerateRandomOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateRandomInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.GenerateRandom");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateRandomOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GenerateRandomOutput, body, alloc);
}
