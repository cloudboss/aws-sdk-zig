...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelKeyDeletionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.CancelKeyDeletion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelKeyDeletionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelKeyDeletionOutput, body, alloc);
}
