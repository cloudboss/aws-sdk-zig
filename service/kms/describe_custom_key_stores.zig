...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCustomKeyStoresInput, options: Options) !DescribeCustomKeyStoresOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCustomKeyStoresInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.DescribeCustomKeyStores");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCustomKeyStoresOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCustomKeyStoresOutput, body, alloc);
}
