...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateDataKeyPairInput, options: Options) !GenerateDataKeyPairOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateDataKeyPairInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.GenerateDataKeyPair");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateDataKeyPairOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GenerateDataKeyPairOutput, body, alloc);
}
