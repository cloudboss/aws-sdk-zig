...
const cancel_key_deletion = @import("cancel_key_deletion.zig");
const connect_custom_key_store = @import("connect_custom_key_store.zig");
const create_alias = @import("create_alias.zig");
const create_custom_key_store = @import("create_custom_key_store.zig");
const create_grant = @import("create_grant.zig");
const create_key = @import("create_key.zig");
const decrypt_ = @import("decrypt.zig");
const delete_alias = @import("delete_alias.zig");
const delete_custom_key_store = @import("delete_custom_key_store.zig");
const delete_imported_key_material = @import("delete_imported_key_material.zig");
const derive_shared_secret = @import("derive_shared_secret.zig");
const describe_custom_key_stores = @import("describe_custom_key_stores.zig");
const describe_key = @import("describe_key.zig");
const disable_key = @import("disable_key.zig");
const disable_key_rotation = @import("disable_key_rotation.zig");
const disconnect_custom_key_store = @import("disconnect_custom_key_store.zig");
const enable_key = @import("enable_key.zig");
const enable_key_rotation = @import("enable_key_rotation.zig");
const encrypt_ = @import("encrypt.zig");
const generate_data_key = @import("generate_data_key.zig");
const generate_data_key_pair = @import("generate_data_key_pair.zig");
const generate_data_key_pair_without_plaintext = @import("generate_data_key_pair_without_plaintext.zig");
const generate_data_key_without_plaintext = @import("generate_data_key_without_plaintext.zig");
const generate_mac = @import("generate_mac.zig");
const generate_random = @import("generate_random.zig");
const get_key_last_usage = @import("get_key_last_usage.zig");
const get_key_policy = @import("get_key_policy.zig");
const get_key_rotation_status = @import("get_key_rotation_status.zig");
const get_parameters_for_import = @import("get_parameters_for_import.zig");
const get_public_key = @import("get_public_key.zig");
const import_key_material = @import("import_key_material.zig");
...
const put_key_policy = @import("put_key_policy.zig");
const re_encrypt = @import("re_encrypt.zig");
const replicate_key = @import("replicate_key.zig");
const retire_grant = @import("retire_grant.zig");
const revoke_grant = @import("revoke_grant.zig");
const rotate_key_on_demand = @import("rotate_key_on_demand.zig");
const schedule_key_deletion = @import("schedule_key_deletion.zig");
const sign_ = @import("sign.zig");
const tag_resource = @import("tag_resource.zig");
const untag_resource = @import("untag_resource.zig");
const update_alias = @import("update_alias.zig");
const update_custom_key_store = @import("update_custom_key_store.zig");
const update_key_description = @import("update_key_description.zig");
...
pub const Client = struct {
    ...
    /// see [Deleting KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html) in the
    ...
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html) to its backing key store. For an CloudHSM key
    /// store, `ConnectCustomKeyStore` connects the key store to its associated
    /// CloudHSM
    ...
    /// time to
    /// complete. This operation starts the connection process, but it does not wait
    /// for it to
    ...
    /// combines the convenience and extensive integration of KMS with the isolation
    /// and control of a
    ...
    /// the cluster, logs into the CloudHSM client as the `kmsuser` CU, and rotates
    /// its
    /// password.
    ///
    ...
    /// [CreateHsm](https://docs.aws.amazon.com/cloudhsm/latest/APIReference/API_CreateHsm.html) operation. Also, the [
    /// `kmsuser` crypto
    /// user](https://docs.aws.amazon.com/kms/latest/developerguide/keystore-cloudhsm.html#concept-kmsuser) (CU) must not be logged into the cluster. This prevents KMS from using this
    /// account to log in.
    ///
    /// If you are having trouble connecting or disconnecting a CloudHSM key store,
    /// see [Troubleshooting an CloudHSM key
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html) in the *Key Management Service Developer Guide*.
    ///
    /// **External key store**
    ///
    ...
    /// see [Troubleshooting an external
    /// key
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/xks-troubleshooting.html) in the *Key Management Service Developer Guide*.
    ///
    ...
    /// KMS key. For details, see [ABAC for
    ...
    /// DescribeKey operation and in [cryptographic
    /// operations](https://docs.aws.amazon.com/kms/latest/developerguide/kms-cryptography.html#cryptographic-operations), such as Encrypt and
    ...
    /// [Aliases in
    /// KMS](https://docs.aws.amazon.com/kms/latest/developerguide/kms-alias.html)
    /// in the
    /// *Key Management Service Developer Guide*.
    ///
    ...
    /// in the *Key Management Service Developer Guide*.
    ///
    /// **Cross-account use**: No. You cannot perform this operation on an alias in
    /// a different Amazon Web Services account.
    ///
    /// **Required permissions**
    ///
    ...
    /// aliases](https://docs.aws.amazon.com/kms/latest/developerguide/alias-access.html) in the *Key Management Service Developer Guide*.
    ///
    /// **Related operations:**
    ///
    /// * DeleteAlias
    ...
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html) backed by a key store that you own and manage. When you use a
    ...
    /// and [external key
    /// stores](https://docs.aws.amazon.com/kms/latest/developerguide/keystore-external.html) backed by an external key store proxy and external key manager outside of
    /// Amazon Web Services.
    ///
    /// This operation is part of the custom key stores feature in KMS, which
    /// combines the convenience and extensive integration of KMS with the isolation
    ...
    /// CloudHSM key
    /// stores)](https://docs.aws.amazon.com/kms/latest/developerguide/create-keystore.html#before-keystore) or [Assemble the prerequisites (for
    /// external key
    /// stores)](https://docs.aws.amazon.com/kms/latest/developerguide/create-xks-keystore.html#xks-requirements) in the *Key Management Service Developer Guide*.
    ///
    ...
    /// token](https://docs.aws.amazon.com/kms/latest/developerguide/using-grant-token.html) in the *
    /// Key Management Service Developer Guide*
    /// .
    ///
    ...
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#kms-keys) in your Amazon Web Services account and Region.
    ...
    /// the key
    /// material used in cryptographic operations, a KMS key includes metadata, such
    /// as the key ID,
    ...
    /// material that KMS generates. This is the basic and most widely used type of
    /// KMS key, and
    ...
    /// to determine whether the KMS key will be used to encrypt and decrypt or sign
    /// and verify.
    ...
    /// ReplicateKey operation. For instructions, see [Importing key material step
    /// 1](https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-create-cmk.html ). For more information about multi-Region keys, see [Multi-Region keys in KMS](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html) in the *Key Management Service Developer Guide*.
    ///
    /// **Custom key store**
    ///
    /// A [custom key
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html) lets you protect your Amazon Web Services resources using keys in a backing key
    ...
    /// stores](https://docs.aws.amazon.com/kms/latest/developerguide/keystore-cloudhsm.html) backed by an CloudHSM cluster and [external key stores](https://docs.aws.amazon.com/kms/latest/developerguide/keystore-external.html) backed by an
    ...
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/create-cmk-keystore.html), use the
    ...
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/create-xks-keys.html), use the
    ...
    /// `Tags` parameter,
    /// [kms:TagResource](https://docs.aws.amazon.com/kms/latest/developerguide/kms-api-permissions-reference.html) (IAM policy). For examples and information about related
    /// permissions, see [Allow a user
    /// to create KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/customer-managed-policies.html#iam-policy-example-create-key) in the *Key Management Service Developer Guide*.
    ///
    /// **Related operations:**
    ///
    /// * DescribeKey
    ...
    /// was encrypted, even
    /// if they've lost track of the key ID. However, specifying the KMS key is
    ...
    /// different KMS key,
    /// the `Decrypt` operation fails. This practice ensures that you use the KMS
    /// key that
    /// you intend.
    ///
    ...
    /// it. If you must use
    /// an IAM policy for `Decrypt` permissions, limit the user to particular KMS
    /// keys or
    /// particular trusted accounts. For details, see [Best practices for IAM
    /// policies](https://docs.aws.amazon.com/kms/latest/developerguide/iam-policies.html#iam-policies-best-practices) in the *Key Management Service Developer Guide*.
    ///
    /// `Decrypt` also supports [Amazon Web Services Nitro
    /// Enclaves](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/nitro-enclave.html) and NitroTPM, which
    /// provide attested environments in Amazon EC2. To call `Decrypt` for a Nitro
    /// enclave or
    /// NitroTPM, use the [Amazon Web Services Nitro Enclaves
    /// SDK](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications.html#sdk) or any Amazon Web Services SDK. Use the `Recipient`
    ...
    /// the interaction between KMS and Amazon Web Services Nitro Enclaves or Amazon
    ...
    /// in a different Amazon Web Services account, use the [key
    /// ARN](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-key-ARN) or [alias
    /// ARN](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-alias-ARN). A short [key ID](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-key-id) is also acceptable
    ...
    ///
    /// For details, see [Controlling access to
    /// aliases](https://docs.aws.amazon.com/kms/latest/developerguide/kms-alias.html#alias-access) in the
    /// *Key Management Service Developer Guide*.
    ///
    /// **Related operations:**
    ///
    ...
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html). This operation does not affect any backing elements of the
    ...
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#kms_keys). Before deleting the key store,
    /// verify that you will never need to use any of the KMS keys in the key store
    /// for any
    /// [cryptographic
    /// operations](https://docs.aws.amazon.com/kms/latest/developerguide/kms-cryptography.html#cryptographic-operations). Then, use ScheduleKeyDeletion to delete the KMS keys from the
    ...
    /// a
    /// best effort to delete the key material from the associated cluster. However,
    /// you might need to
    /// manually [delete the orphaned key
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-orphaned-key) from the cluster and its backups. KMS never creates, manages, or deletes
    ...
    /// DisconnectCustomKeyStore operation to disconnect the custom key store from
    /// its
    /// backing key store. While the key store is disconnected, you cannot create or
    /// use the KMS keys
    ...
    /// specified KMS
    /// key temporarily unusable. To restore the usability of the KMS key, reimport
    /// the same key
    ...
    /// Diffie-Hellman
    /// Primitive](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Ar3.pdf#page=60) (ECDH) to establish a
    ...
    /// derivation](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Cr2.pdf) when using the raw shared secret to derive a
    /// symmetric key.
    ///
    ...
    /// Diffie-Hellman Primitive to calculate the same raw secret using his private
    /// key and
    /// Alice's public key.
    ///
    /// To derive a shared secret you must provide a key agreement algorithm, the
    /// private key of
    /// the caller's asymmetric NIST-standard elliptic curve or SM2 (China Regions
    /// only) KMS key
    /// pair, and the public key from your peer's NIST-standard elliptic curve or
    /// SM2 (China
    ...
    /// stores](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html) in the account and Region.
    ///
    /// This operation is part of the custom key stores feature in KMS, which
    /// combines the convenience and extensive integration of KMS with the isolation
    /// and control of a
    ...
    /// key store
    /// connection state is `CONNECTED` but you are having trouble using it, verify
    /// that
    ...
    /// any. For an external key store, verify that the external key store proxy and
    /// its associated
    /// external key manager are reachable and enabled.
    ///
    /// For help repairing your CloudHSM key store, see the [Troubleshooting
    /// CloudHSM key
    /// stores](https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html). For help
    ...
    /// [customer
    /// managed
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#customer-mgn-key) or an [Amazon Web Services managed key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-managed-key).
    ///
    ...
    /// the key material.
    /// It includes fields, like `KeySpec`, that help you distinguish different
    /// types of
    /// KMS keys. It also displays the key usage (encryption, signing, or generating
    /// and verifying
    /// MACs) and the algorithms that the KMS key supports.
    ///
    /// For [multi-Region
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html), `DescribeKey` displays the primary key and all
    /// related replica keys. For KMS keys in [CloudHSM key
    /// stores](https://docs.aws.amazon.com/kms/latest/developerguide/keystore-cloudhsm.html), it includes information
    /// about the key store, such as the key store ID and the CloudHSM cluster ID.
    /// For KMS keys in [external key
    ...
    /// works](https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html#rotate-keys-how-it-works) in the *Key Management Service Developer Guide*.
    ///
    ...
    /// managed
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-managed-key) from a *predefined Amazon Web Services alias* with no key
    /// ID.
    ///
    /// **Cross-account use**: Yes. To perform this operation with a KMS key in a
    /// different Amazon Web Services account, specify
    ...
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/rotating-keys-enable-disable.html) of the specified symmetric encryption KMS
    /// key.
    ///
    ...
    /// key material in [customer managed KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#customer-mgn-key). Key material rotation of [Amazon Web Services managed KMS keys](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-managed-key) is not
    /// configurable. KMS always rotates the key material for every year. Rotation
    /// of [Amazon Web Services owned KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-owned-key) varies.
    ///
    ...
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html) from its backing key store. This operation disconnects an
    ...
    /// the custom key store or to use existing KMS keys in [cryptographic
    /// operations](https://docs.aws.amazon.com/kms/latest/developerguide/kms-cryptography.html#cryptographic-operations) will
    ...
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#customer-mgn-key), KMS
    ...
    /// managed KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-managed-key). KMS
    /// always rotates the key material of Amazon Web Services managed keys every
    /// year. Rotation of [Amazon Web Services owned KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-owned-key) is managed by the Amazon Web Services service that owns the key.
    ///
    /// In May 2022, KMS changed the rotation schedule for Amazon Web Services
    /// managed keys from every three
    ...
    /// Context](https://docs.aws.amazon.com/kms/latest/developerguide/encrypt_context.html) in the
    /// *Key Management Service Developer Guide*.
    ///
    /// `GenerateDataKey` also supports [Amazon Web Services Nitro
    /// Enclaves](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/nitro-enclave.html), which provide an
    ...
    /// SDK](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications.html#sdk) or any Amazon Web Services SDK. Use the
    /// `Recipient` parameter to provide the attestation document for the attested
    /// environment. `GenerateDataKey` returns a copy of the data key encrypted
    /// under the
    ...
    /// (`CiphertextForRecipient`). For information about the interaction between
    /// KMS and Amazon Web Services Nitro Enclaves or Amazon Web Services NitroTPM,
    ...
    /// perform asymmetric cryptography and implement digital signatures outside of
    /// KMS. The bytes
    ...
    /// to encrypt
    /// the private key in a data key pair. You cannot use an asymmetric KMS key or
    /// a KMS key in a
    ...
    /// Nitro enclave or NitroTPM, use the [Amazon Web Services Nitro Enclaves
    /// SDK](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications.html#sdk) or any Amazon Web Services SDK. Use the
    /// `Recipient` parameter to provide the attestation document for the attested
    /// environment. `GenerateDataKeyPair` returns the public data key and a copy of
    /// the
    /// private data key encrypted under the specified KMS key, as usual. But
    /// instead of a plaintext
    /// copy of the private data key (`PrivateKeyPlaintext`), the response includes
    /// a copy
    ...
    /// Returns a unique asymmetric data key pair for use outside of KMS. This
    /// operation returns
    /// a plaintext public key and a copy of the private key that is encrypted under
    /// the symmetric
    ...
    /// they are not
    /// related to the caller or to the KMS key that is used to encrypt the private
    /// key.
    ///
    ...
    /// use the Decrypt operation to decrypt the encrypted private key.
    ///
    /// To generate a data key pair, you must specify a symmetric encryption KMS key
    /// to encrypt
    /// the private key in a data key pair. You cannot use an asymmetric KMS key or
    ...
    /// is used to encrypt
    /// the private key. The public key is a DER-encoded X.509 SubjectPublicKeyInfo,
    /// as specified in
    ...
    /// using an HMAC KMS
    /// key and a MAC algorithm that the key supports. HMAC KMS keys and the HMAC
    /// algorithms that
    /// KMS uses conform to industry standards defined in [RFC
    ...
    /// the required secret
    /// key. You can also use the raw result to implement HMAC-based algorithms such
    /// as key derivation
    ...
    /// mechanism,
    /// including an HMAC, is effective. This deters an attack where the actor uses
    /// a signed message
    /// to establish validity repeatedly or long after the message is superseded.
    /// HMAC tags do not
    /// include a timestamp, but you can include a timestamp in the token or message
    /// to help you
    ...
    /// Enclaves](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/nitro-enclave.html), which provide an
    /// isolated compute environment in Amazon EC2. To call `GenerateRandom` for a
    /// Nitro enclave
    /// or NitroTPM, use the [Amazon Web Services Nitro Enclaves
    /// SDK](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications.html#sdk) or any Amazon Web Services SDK. Use the `Recipient`
    /// parameter to provide the attestation document for the attested environment.
    /// Instead of
    ...
    /// generation](https://docs.aws.amazon.com/kms/latest/developerguide/kms-cryptography.html#entropy-and-random-numbers) in the *Key Management Service Developer Guide*.
    ///
    ...
    /// `TrackingStartDate`. The response includes the operation type, timestamp,
    /// and
    ...
    /// deletion. Instead, first [disable the
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/enabling-keys.html) and monitor CloudTrail for
    ...
    /// For more information, see [KMS eventual
    /// consistency](https://docs.aws.amazon.com/kms/latest/developerguide/accessing-kms.html#programming-eventual-consistency).
    pub fn getKeyLastUsage(self: *Self, allocator: std.mem.Allocator, input: get_key_last_usage.GetKeyLastUsageInput, options: CallOptions) !get_key_last_usage.GetKeyLastUsageOutput {
        return get_key_last_usage.execute(self, allocator, input, options);
    }
    ...
    /// [automatic
    /// rotation of the key
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/rotating-keys-enable-disable.html) is enabled for the specified KMS key, the [rotation
    /// period](https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html#rotation-period), and the next scheduled rotation date.
    ///
    /// Automatic key rotation is supported only on symmetric encryption KMS keys.
    /// You cannot enable automatic rotation of [asymmetric KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html), [HMAC KMS keys](https://docs.aws.amazon.com/kms/latest/developerguide/hmac.html), KMS keys with [imported key material](https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys.html), or KMS keys in a [custom key store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html). To enable or disable automatic rotation of a set of related [multi-Region keys](https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html#multi-region-rotate), set the property on the primary key.
    ...
    /// material rotation of [Amazon Web Services managed KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-managed-key) is not
    ...
    /// enabled. You can use GetKeyRotationStatus to identify the date and time that
    /// an in progress
    ...
    /// hasn't been
    /// rotated in one year, KMS rotates it immediately, and every year thereafter.
    /// If it's been
    ...
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys.html), an advanced feature that lets you generate and import the cryptographic
    ...
    /// no key material. You can import key material for a symmetric encryption KMS
    /// key, HMAC KMS key,
    /// asymmetric encryption KMS key, or asymmetric signing KMS key. You can also
    /// import key material
    /// into a [multi-Region
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html) of any supported type. However, you can't import key material into
    /// a KMS key in a [custom
    /// key
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html). You can also use `GetParametersForImport` to get a public key
    /// and import token to [reimport
    /// the original key
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-import-key-material.html#reimport-key-material) into a KMS key whose key material expired or was
    /// deleted.
    ///
    ...
    ///
    /// *
    ///   [KeyUsage](https://docs.aws.amazon.com/kms/latest/APIReference/API_GetPublicKey.html#KMS-GetPublicKey-response-KeyUsage): Whether the key is used for encryption, signing, or deriving a shared
    /// secret.
    ///
    /// *
    ///   [EncryptionAlgorithms](https://docs.aws.amazon.com/kms/latest/APIReference/API_GetPublicKey.html#KMS-GetPublicKey-response-EncryptionAlgorithms), [KeyAgreementAlgorithms](https://docs.aws.amazon.com/kms/latest/APIReference/API_GetPublicKey.html#KMS-GetPublicKey-response-KeyAgreementAlgorithms), or [SigningAlgorithms](https://docs.aws.amazon.com/kms/latest/APIReference/API_GetPublicKey.html#KMS-GetPublicKey-response-SigningAlgorithms): A list of the encryption algorithms, key agreement
    ...
    /// improperly. For
    /// example, you can prevent a public signing key from being used encrypt data,
    /// or prevent a
    ...
    /// without key
    /// material. You can also use this operation to set or update the expiration
    ...
    /// initial import.
    /// You can import multiple key materials into symmetric encryption keys and
    /// rotate the key
    /// material on demand using `RotateKeyOnDemand`.
    ///
    /// You can import new key materials into multi-Region symmetric encryption
    /// keys. To do so, you must
    /// import the new key material into the primary Region key. Then you can import
    /// the same key
    ...
    /// material imported to the primary Region key. You must do this for every
    /// replica
    ...
    /// the same key
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-import-key-material.html#reimport-key-material) into that KMS key or, if the key supports on-demand rotation,
    ...
    /// change the expiration of your key material, you must import it again, either
    /// by calling
    /// `ImportKeyMaterial` or using the [import
    /// features](https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-import-key-material.html#importing-keys-import-key-material-console) of the KMS console.
    ///
    ...
    /// material into a
    /// [multi-Region
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html) of any supported type. However, you can't import key material
    /// into a KMS key in a [custom key
    ...
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html), or on a KMS key in a different Amazon Web Services account. To get the
    ...
    /// need to import all of the key materials associated with the KMS key to
    /// change its state to
    ...
    /// imported key
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-conceptual.html) in the *Key Management Service Developer Guide*.
    ///
    /// The KMS key that you use for this operation must be in a compatible key
    /// state. For
    /// details, see [Key states of KMS
    ...
    /// [Rotate KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html) in the
    /// *Key Management Service Developer Guide*.
    ///
    /// **Cross-account use**: No. You cannot perform this operation on a KMS key in
    /// a different Amazon Web Services account.
    ...
    /// `kms:ListRetirableGrants` permission (or any other additional permission) in
    /// any
    ...
    /// Policies](https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html) in the *Key Management Service Developer Guide*.
    ...
    /// see [Use
    /// PutKeyPolicy with an Amazon Web Services SDK or
    /// CLI](https://docs.aws.amazon.com/kms/latest/developerguide/example_kms_PutKeyPolicy_section.html) in the *Key Management Service Developer Guide*.
    ///
    /// **Cross-account use**: No. You cannot perform this operation on a KMS key in
    /// a different Amazon Web Services account.
    ///
    ...
    /// you [manually
    /// rotate](https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys-manually.html) a
    ...
    /// context](https://docs.aws.amazon.com/kms/latest/developerguide/encrypt_context.html) of a ciphertext.
    ///
    ...
    /// public key of an [asymmetric KMS
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html) outside of KMS. However, it cannot decrypt ciphertext produced
    ...
    /// or [alias
    /// ARN](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-alias-ARN). A short [key ID](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-key-id)
    ...
    /// policy](https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html). This permission is
    /// automatically included in the key policy when you use the console to create
    /// a KMS key. But you
    /// must include it manually when you create a KMS key programmatically or when
    ...
    /// replica keys share
    /// properties that make them interoperable. They have the same [key
    /// ID](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-key-id) and key material. They also
    ...
    /// replica
    /// key is complete. While the key state is `Creating`, you can manage key, but
    /// you
    /// cannot yet use it in cryptographic operations. If you are creating and using
    /// the replica key
    /// programmatically, retry on `KMSInvalidStateException` or call
    /// `DescribeKey` to check its `KeyState` value before using it. For
    /// details about the `Creating` key state, see [Key states of KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html)
    /// in the
    /// *Key Management Service Developer Guide*.
    ///
    ...
    /// properties](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html#mrk-sync-properties) as the original replica key.
    ///
    ...
    /// and `Tags` parameters than those used in the KMS console. For details, see
    /// the
    ...
    /// permissions. To
    /// identify the grant to retire, use a [grant
    /// token](https://docs.aws.amazon.com/kms/latest/developerguide/grants.html#grant_token), or both the grant ID and a
    ...
    /// grants](https://docs.aws.amazon.com/kms/latest/developerguide/grant-delete.html) in the
    /// *Key Management Service Developer Guide*.
    ///
    /// **Related operations:**
    ///
    ...
    /// that the
    /// grant allows. For more information, see [Retiring and revoking
    /// grants](https://docs.aws.amazon.com/kms/latest/developerguide/grant-delete.html) in the
    /// *
    /// Key Management Service Developer Guide*
    /// .
    ...
    /// This state is known as *eventual consistency*. For details, see [Eventual
    /// consistency](https://docs.aws.amazon.com/kms/latest/developerguide/grants.html#terms-eventual-consistency) in
    /// the *
    /// Key Management Service Developer Guide*
    /// .
    ///
    ...
    /// rotation](https://docs.aws.amazon.com/kms/latest/developerguide/rotating-keys-on-demand.html) of the key
    /// material in customer managed KMS keys, regardless of whether or not
    /// [automatic key
    /// rotation](https://docs.aws.amazon.com/kms/latest/developerguide/rotating-keys-enable-disable.html) is enabled. On-demand rotations do not change existing automatic rotation
    /// schedules. For example, consider a KMS key that has automatic key rotation
    /// enabled with a
    /// rotation period of 730 days. If the key is scheduled to automatically rotate
    /// on April 14,
    ...
    /// You cannot
    /// perform on-demand rotation of [asymmetric KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html), [HMAC KMS keys](https://docs.aws.amazon.com/kms/latest/developerguide/hmac.html), or KMS keys in a
    /// [custom key
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html). When you initiate on-demand key rotation on a symmetric encryption KMS key
    /// with imported key material, you must have already imported [new key
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-import-key-material.html) and that
    ...
    /// [multi-Region
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html#multi-region-rotate), import
    /// new key material in the primary Region key, import the same key material in
    /// each replica
    ...
    /// period of 30
    /// days, but you can specify a waiting period of 7-30 days. When this operation
    /// is successful,
    /// the key state of the KMS key changes to `PendingDeletion` and the key can't
    /// be used
    ...
    /// When a KMS key
    /// is deleted, all data that was encrypted under the KMS key is unrecoverable.
    /// (The only
    /// exception is a [multi-Region replica
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-delete.html), or an [asymmetric or HMAC KMS key with
    /// imported key
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html#import-delete-key).) To prevent the use of a KMS key without deleting it, use
    /// DisableKey.
    ///
    ...
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html#deleting-mrks) in
    /// the *Key Management Service Developer Guide*.
    ///
    /// When KMS [deletes a KMS key from an CloudHSM
    /// key
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html#delete-cmk-keystore), it makes a best effort to delete the associated key material from the
    /// associated CloudHSM cluster. However, you might need to manually [delete the
    /// orphaned key
    /// material](https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-orphaned-key) from the cluster and its backups. [Deleting a KMS key from an external key
    /// store](https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html#delete-xks-key) has no effect on the associated external key. However, for both types of
    ...
    /// Creates a [digital
    /// signature](https://en.wikipedia.org/wiki/Digital_signature) for a message or
    ...
    /// public key can
    /// verify that the message was signed with that particular private key and that
    /// the message
    ...
    /// The tag value can be an empty (null) string. To add a tag, specify a new tag
    /// key and a tag
    /// value. To edit a tag, specify an existing tag key and a new tag value.
    ///
    /// You can use this operation to tag a [customer managed
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#customer-mgn-key), but you
    /// cannot tag an [Amazon Web Services managed
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-managed-key), an [Amazon Web Services owned key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-owned-key), a [custom key store](https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html),
    /// or an
    ...
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/tagging-keys.html). For general information about
    ...
    /// key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#customer-mgn-key). To delete a
    ...
    /// proxy configuration
    /// file to the `UpdateCustomKeyStore` operation. However, you can use the file
    /// to
    ...
    /// (`CloudHsmClusterId`). To update most properties of an CloudHSM key store,
    /// the
    /// `ConnectionState` of the CloudHSM key store must be `DISCONNECTED`.
    /// However, you can update the `CustomKeyStoreName` of an AWS CloudHSM key
    /// store
    ...
    /// key store
    /// friendly name (`NewCustomKeyStoreName`), or to tell KMS about a change to
    /// the
    ...
    /// primary key in a
    /// set of multi-Region
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-update.html) in the *Key Management Service Developer Guide*.
    ///
    /// This operation supports *multi-Region keys*, an KMS feature that lets you
    /// create multiple
    /// interoperable KMS keys in different Amazon Web Services Regions. Because
    ...
    /// material, [key
    /// ID](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-key-id), [key spec](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-spec), [key usage](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-usage), [key material
    /// origin](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-origin), and [automatic
    /// key
    /// rotation](https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html). It's the only key that can be replicated. You cannot [delete the primary
    /// key](https://docs.aws.amazon.com/kms/latest/APIReference/API_ScheduleKeyDeletion.html) until all replica keys are deleted.
    ///
    /// The key ID and primary Region that you specify uniquely identify the replica
    /// key that will
    /// become the primary key. The primary Region must already have a replica key.
    /// This operation
    /// does not create a KMS key in the specified Region. To find the replica keys,
    /// use the DescribeKey operation on the primary key or any replica key. To
    /// create a replica
    ...
    /// primary keys have a
    /// transient key state of `Updating`. The original key state is restored when
    /// the
    /// update is complete. While the key state is `Updating`, you can use the keys
    /// in
    ...
    /// about the
    /// `Updating` key state, see [Key states of KMS
    /// keys](https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html)
    /// in the *Key Management Service Developer Guide*.
    ///
    ...
    /// outside of KMS. The
    /// advantage of using the `Verify` operation is that it is performed within
    /// KMS. As
    ...
    /// message, HMAC
    /// KMS key, and MAC algorithm. To verify the HMAC, `VerifyMac` computes an HMAC
    /// using
    /// the message, HMAC KMS key, and MAC algorithm that you specify, and compares
    /// the computed HMAC
    ...
    pub fn describeCustomKeyStoresPaginator(self: *Self, params: describe_custom_key_stores.DescribeCustomKeyStoresInput) paginator.DescribeCustomKeyStoresPaginator {
        return .{
            .client = self,
            .params = params,
        };
    ...
    pub fn listGrantsPaginator(self: *Self, params: list_grants.ListGrantsInput) paginator.ListGrantsPaginator {
        return .{
            .client = self,
            .params = params,
        };
    ...
    pub fn listKeyPoliciesPaginator(self: *Self, params: list_key_policies.ListKeyPoliciesInput) paginator.ListKeyPoliciesPaginator {
        return .{
            .client = self,
            .params = params,
        };
    ...
    pub fn listKeyRotationsPaginator(self: *Self, params: list_key_rotations.ListKeyRotationsInput) paginator.ListKeyRotationsPaginator {
        return .{
            .client = self,
            .params = params,
        };
    ...
    pub fn listResourceTagsPaginator(self: *Self, params: list_resource_tags.ListResourceTagsInput) paginator.ListResourceTagsPaginator {
        return .{
            .client = self,
            .params = params,
        };
    ...
    pub fn listRetirableGrantsPaginator(self: *Self, params: list_retirable_grants.ListRetirableGrantsInput) paginator.ListRetirableGrantsPaginator {
        return .{
            .client = self,
            .params = params,
        };
