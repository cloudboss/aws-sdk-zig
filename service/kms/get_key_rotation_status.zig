...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetKeyRotationStatusInput, options: Options) !GetKeyRotationStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKeyRotationStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "TrentService.GetKeyRotationStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKeyRotationStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetKeyRotationStatusOutput, body, alloc);
}
