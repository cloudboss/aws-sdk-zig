...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteStatementInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.continue_after_timeout) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"continueAfterTimeout\":");
    ...
    if (input.format_records_as) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"formatRecordsAs\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.include_result_metadata) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"includeResultMetadata\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.result_set_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resultSetOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.schema) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteStatementOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ExecuteStatementOutput, body, alloc);
    }
    _ = status;
    _ = headers;
