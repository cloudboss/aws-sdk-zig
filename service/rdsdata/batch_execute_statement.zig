...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchExecuteStatementInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.parameter_sets) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"parameterSets\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sql\":");
    try aws.json.writeValue(@TypeOf(input.sql), input.sql, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchExecuteStatementOutput {
    var result: BatchExecuteStatementOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchExecuteStatementOutput, body, alloc);
    }
    _ = status;
    _ = headers;
