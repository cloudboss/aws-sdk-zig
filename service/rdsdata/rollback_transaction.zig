...
pub const RollbackTransactionInput = struct {
...
pub const RollbackTransactionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RollbackTransactionInput, options: CallOptions) !RollbackTransactionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RollbackTransactionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/RollbackTransaction";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RollbackTransactionOutput {
    var result: RollbackTransactionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RollbackTransactionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
