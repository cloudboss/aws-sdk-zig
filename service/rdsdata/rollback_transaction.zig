...
fn serializeRequest(alloc: std.mem.Allocator, input: RollbackTransactionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RollbackTransactionOutput {
    var result: RollbackTransactionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RollbackTransactionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
