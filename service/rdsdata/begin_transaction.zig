...
pub const BeginTransactionInput = struct {
...
pub const BeginTransactionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BeginTransactionInput, options: Options) !BeginTransactionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BeginTransactionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BeginTransactionOutput {
    var result: BeginTransactionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BeginTransactionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
