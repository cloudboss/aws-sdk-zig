...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteSqlInput, options: CallOptions) !ExecuteSqlOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExecuteSqlInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("rdsdata", "RDS Data", allocator);
    ...
    try body_buf.appendSlice(allocator, "\"awsSecretStoreArn\":");
    try aws.json.writeValue(@TypeOf(input.aws_secret_store_arn), input.aws_secret_store_arn, allocator, &body_buf);
    has_prev = true;
    if (input.database) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"dbClusterOrInstanceArn\":");
    try aws.json.writeValue(@TypeOf(input.db_cluster_or_instance_arn), input.db_cluster_or_instance_arn, allocator, &body_buf);
    has_prev = true;
    if (input.schema) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"sqlStatements\":");
    try aws.json.writeValue(@TypeOf(input.sql_statements), input.sql_statements, allocator, &body_buf);
    has_prev = true;
