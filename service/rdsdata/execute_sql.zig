...
const SqlStatementResult = @import("sql_statement_result.zig").SqlStatementResult;
...
pub const ExecuteSqlInput = struct {
    ...
    db_cluster_or_instance_arn: []const u8,
    ...
    pub const json_field_names = .{
        ...
        .database = "database",
        .db_cluster_or_instance_arn = "dbClusterOrInstanceArn",
        .schema = "schema",
        .sql_statements = "sqlStatements",
    };
};

pub const ExecuteSqlOutput = struct {
    /// The results of the SQL statement or statements.
    sql_statement_results: ?[]const SqlStatementResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteSqlInput, options: Options) !ExecuteSqlOutput {
    ...
    try aws.signing.signRequest(alloc, &request, creds, client.config.region, "rdsdata");
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteSqlInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("rdsdata", "RDS Data", alloc);

    const host = aws.url.parseHost(endpoint);
    const tls = !std.mem.startsWith(u8, endpoint, "http://");
    const port = aws.url.parsePort(endpoint);

    const path = "/ExecuteSql";
    ...
    try body_buf.appendSlice(alloc, "\"awsSecretStoreArn\":");
    try aws.json.writeValue(@TypeOf(input.aws_secret_store_arn), input.aws_secret_store_arn, alloc, &body_buf);
    has_prev = true;
    if (input.database) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"database\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"dbClusterOrInstanceArn\":");
    try aws.json.writeValue(@TypeOf(input.db_cluster_or_instance_arn), input.db_cluster_or_instance_arn, alloc, &body_buf);
    has_prev = true;
    if (input.schema) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"schema\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sqlStatements\":");
    try aws.json.writeValue(@TypeOf(input.sql_statements), input.sql_statements, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteSqlOutput {
    var result: ExecuteSqlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ExecuteSqlOutput, body, alloc);
    }
    _ = status;
    _ = headers;
