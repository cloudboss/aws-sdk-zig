...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetClusterInput, options: Options) !GetClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetClusterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetClusterOutput {
    var result: GetClusterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetClusterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
