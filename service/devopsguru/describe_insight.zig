...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInsightInput, options: Options) !DescribeInsightOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInsightInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInsightOutput {
    var result: DescribeInsightOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeInsightOutput, body, alloc);
    }
    _ = status;
    _ = headers;
