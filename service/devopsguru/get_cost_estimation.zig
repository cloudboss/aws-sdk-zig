...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCostEstimationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCostEstimationOutput {
    var result: GetCostEstimationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCostEstimationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
