...
pub const DescribeOrganizationHealthInput = struct {
...
pub const DescribeOrganizationHealthOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrganizationHealthInput, options: CallOptions) !DescribeOrganizationHealthOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeOrganizationHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/organization/health";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeOrganizationHealthOutput {
    var result: DescribeOrganizationHealthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeOrganizationHealthOutput, body, allocator);
    }
    _ = status;
    _ = headers;
