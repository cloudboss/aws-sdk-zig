...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOrganizationHealthInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOrganizationHealthOutput {
    var result: DescribeOrganizationHealthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeOrganizationHealthOutput, body, alloc);
    }
    _ = status;
    _ = headers;
