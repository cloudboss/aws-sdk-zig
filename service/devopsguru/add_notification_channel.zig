...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddNotificationChannelInput, options: Options) !AddNotificationChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddNotificationChannelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddNotificationChannelOutput {
    var result: AddNotificationChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddNotificationChannelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
