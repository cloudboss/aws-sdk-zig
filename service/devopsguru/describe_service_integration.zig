...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeServiceIntegrationInput, options: Options) !DescribeServiceIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeServiceIntegrationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeServiceIntegrationOutput {
    var result: DescribeServiceIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeServiceIntegrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
