...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeResourceCollectionHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/accounts/health/resource-collection/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeResourceCollectionHealthOutput {
    var result: DescribeResourceCollectionHealthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeResourceCollectionHealthOutput, body, alloc);
    }
    _ = status;
    _ = headers;
