...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOrganizationInsightsInput, options: Options) !ListOrganizationInsightsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOrganizationInsightsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"StatusFilter\":");
    try aws.json.writeValue(@TypeOf(input.status_filter), input.status_filter, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOrganizationInsightsOutput {
    var result: ListOrganizationInsightsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOrganizationInsightsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
