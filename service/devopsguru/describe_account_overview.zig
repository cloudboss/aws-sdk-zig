...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountOverviewInput, options: Options) !DescribeAccountOverviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountOverviewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountOverviewOutput {
    var result: DescribeAccountOverviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccountOverviewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
