...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInsightsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInsightsOutput {
    var result: ListInsightsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInsightsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
