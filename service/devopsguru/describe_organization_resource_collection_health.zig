...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrganizationResourceCollectionHealthInput, options: Options) !DescribeOrganizationResourceCollectionHealthOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOrganizationResourceCollectionHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.organizational_unit_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OrganizationalUnitIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OrganizationResourceCollectionType\":");
    try aws.json.writeValue(@TypeOf(input.organization_resource_collection_type), input.organization_resource_collection_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOrganizationResourceCollectionHealthOutput {
    var result: DescribeOrganizationResourceCollectionHealthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeOrganizationResourceCollectionHealthOutput, body, alloc);
    }
    _ = status;
    _ = headers;
