...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceCollectionInput, options: Options) !GetResourceCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/resource-collections/");
    try path_buf.appendSlice(alloc, input.resource_collection_type);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceCollectionOutput {
    var result: GetResourceCollectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceCollectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
