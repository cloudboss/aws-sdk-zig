...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountHealthInput, options: Options) !DescribeAccountHealthOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountHealthInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountHealthOutput {
    var result: DescribeAccountHealthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccountHealthOutput, body, alloc);
    }
    _ = status;
    _ = headers;
