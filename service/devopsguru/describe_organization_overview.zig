...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrganizationOverviewInput, options: Options) !DescribeOrganizationOverviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOrganizationOverviewInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FromTime\":");
    try aws.json.writeValue(@TypeOf(input.from_time), input.from_time, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.to_time) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ToTime\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOrganizationOverviewOutput {
    var result: DescribeOrganizationOverviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeOrganizationOverviewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
