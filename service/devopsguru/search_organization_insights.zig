...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchOrganizationInsightsInput, options: Options) !SearchOrganizationInsightsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchOrganizationInsightsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchOrganizationInsightsOutput {
    var result: SearchOrganizationInsightsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchOrganizationInsightsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
