...
const SearchInsightsFilters = @import("search_insights_filters.zig").SearchInsightsFilters;
...
pub const SearchInsightsInput = struct {
...
pub const SearchInsightsOutput = struct {
    /// The pagination token to use to retrieve
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchInsightsInput, options: CallOptions) !SearchInsightsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchInsightsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchInsightsOutput {
    var result: SearchInsightsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchInsightsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
