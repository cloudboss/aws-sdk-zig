...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchInsightsInput, options: Options) !SearchInsightsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchInsightsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.start_time_range), input.start_time_range, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchInsightsOutput {
    var result: SearchInsightsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchInsightsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
