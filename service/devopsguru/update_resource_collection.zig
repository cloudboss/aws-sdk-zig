...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResourceCollectionInput, options: Options) !UpdateResourceCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResourceCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ResourceCollection\":");
    try aws.json.writeValue(@TypeOf(input.resource_collection), input.resource_collection, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResourceCollectionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
