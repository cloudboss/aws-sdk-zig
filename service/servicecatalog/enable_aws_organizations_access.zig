...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableAWSOrganizationsAccessInput, options: CallOptions) !EnableAWSOrganizationsAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableAWSOrganizationsAccessInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.EnableAWSOrganizationsAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableAWSOrganizationsAccessOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
