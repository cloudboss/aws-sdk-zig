...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceActionInput, options: Options) !UpdateServiceActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.UpdateServiceAction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceActionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateServiceActionOutput, body, alloc);
}
