...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePortfolioInput, options: Options) !UpdatePortfolioOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePortfolioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.UpdatePortfolio");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePortfolioOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdatePortfolioOutput, body, alloc);
}
