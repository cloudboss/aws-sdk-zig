...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeServiceActionInput, options: Options) !DescribeServiceActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeServiceActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeServiceAction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeServiceActionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeServiceActionOutput, body, alloc);
}
