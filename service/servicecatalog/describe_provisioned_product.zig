...
const CloudWatchDashboard = @import("cloud_watch_dashboard.zig").CloudWatchDashboard;
const ProvisionedProductDetail = @import("provisioned_product_detail.zig").ProvisionedProductDetail;

pub const DescribeProvisionedProductInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub const DescribeProvisionedProductOutput = struct {
    /// Any CloudWatch dashboards that were created when provisioning the product.
    cloud_watch_dashboards: ?[]const CloudWatchDashboard = null,

    /// Information about the provisioned product.
    provisioned_product_detail: ?ProvisionedProductDetail = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProvisionedProductInput, options: CallOptions) !DescribeProvisionedProductOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "servicecatalog");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeProvisionedProductInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeProvisionedProduct");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeProvisionedProductOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProvisionedProductOutput, body, allocator);
}
