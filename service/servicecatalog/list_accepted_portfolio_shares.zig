...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAcceptedPortfolioSharesInput, options: Options) !ListAcceptedPortfolioSharesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAcceptedPortfolioSharesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListAcceptedPortfolioShares");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAcceptedPortfolioSharesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAcceptedPortfolioSharesOutput, body, alloc);
}
