...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteServiceActionInput, options: Options) !DeleteServiceActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteServiceActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DeleteServiceAction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteServiceActionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
