...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteProvisioningArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DeleteProvisioningArtifact");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteProvisioningArtifactOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
