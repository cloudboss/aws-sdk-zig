...
pub const DeleteProvisioningArtifactInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteProvisioningArtifactInput, options: CallOptions) !DeleteProvisioningArtifactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteProvisioningArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.DeleteProvisioningArtifact");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteProvisioningArtifactOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
