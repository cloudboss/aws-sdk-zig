...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPortfolioAccessInput, options: Options) !ListPortfolioAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPortfolioAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListPortfolioAccess");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPortfolioAccessOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPortfolioAccessOutput, body, alloc);
}
