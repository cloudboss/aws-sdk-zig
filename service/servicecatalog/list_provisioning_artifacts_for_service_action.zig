...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProvisioningArtifactsForServiceActionInput, options: Options) !ListProvisioningArtifactsForServiceActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProvisioningArtifactsForServiceActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListProvisioningArtifactsForServiceAction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProvisioningArtifactsForServiceActionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProvisioningArtifactsForServiceActionOutput, body, alloc);
}
