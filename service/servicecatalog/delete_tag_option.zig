...
pub const DeleteTagOptionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTagOptionInput, options: Options) !DeleteTagOptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTagOptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.DeleteTagOption");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTagOptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
