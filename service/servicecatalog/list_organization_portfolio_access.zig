...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOrganizationPortfolioAccessInput, options: Options) !ListOrganizationPortfolioAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOrganizationPortfolioAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListOrganizationPortfolioAccess");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOrganizationPortfolioAccessOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOrganizationPortfolioAccessOutput, body, alloc);
}
