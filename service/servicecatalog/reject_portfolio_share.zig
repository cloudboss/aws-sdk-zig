...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectPortfolioShareInput, options: Options) !RejectPortfolioShareOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectPortfolioShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.RejectPortfolioShare");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectPortfolioShareOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
