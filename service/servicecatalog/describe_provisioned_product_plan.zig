...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProvisionedProductPlanInput, options: Options) !DescribeProvisionedProductPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProvisionedProductPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeProvisionedProductPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProvisionedProductPlanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProvisionedProductPlanOutput, body, alloc);
}
