...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteProvisionedProductPlanInput, options: Options) !ExecuteProvisionedProductPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteProvisionedProductPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ExecuteProvisionedProductPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteProvisionedProductPlanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExecuteProvisionedProductPlanOutput, body, alloc);
}
