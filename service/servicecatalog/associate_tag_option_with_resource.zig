...
pub const AssociateTagOptionWithResourceInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateTagOptionWithResourceInput, options: Options) !AssociateTagOptionWithResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateTagOptionWithResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.AssociateTagOptionWithResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateTagOptionWithResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
