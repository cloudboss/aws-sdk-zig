...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyProductInput, options: Options) !CopyProductOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyProductInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.CopyProduct");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyProductOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CopyProductOutput, body, alloc);
}
