...
pub const DeleteConstraintInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConstraintInput, options: Options) !DeleteConstraintOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteConstraintInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.DeleteConstraint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteConstraintOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
