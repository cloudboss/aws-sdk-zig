...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProductInput, options: Options) !UpdateProductOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProductInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.UpdateProduct");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProductOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateProductOutput, body, alloc);
}
