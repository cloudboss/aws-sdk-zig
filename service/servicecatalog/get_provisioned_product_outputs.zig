...
pub const GetProvisionedProductOutputsInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub const GetProvisionedProductOutputsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProvisionedProductOutputsInput, options: CallOptions) !GetProvisionedProductOutputsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetProvisionedProductOutputsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.GetProvisionedProductOutputs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetProvisionedProductOutputsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetProvisionedProductOutputsOutput, body, allocator);
}
