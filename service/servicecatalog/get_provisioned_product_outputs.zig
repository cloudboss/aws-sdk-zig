...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProvisionedProductOutputsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.GetProvisionedProductOutputs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProvisionedProductOutputsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetProvisionedProductOutputsOutput, body, alloc);
}
