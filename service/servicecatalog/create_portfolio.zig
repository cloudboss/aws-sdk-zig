...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePortfolioInput, options: Options) !CreatePortfolioOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePortfolioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.CreatePortfolio");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePortfolioOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePortfolioOutput, body, alloc);
}
