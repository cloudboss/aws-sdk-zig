...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProductViewInput, options: Options) !DescribeProductViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProductViewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeProductView");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProductViewOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProductViewOutput, body, alloc);
}
