...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAWSOrganizationsAccessStatusInput, options: CallOptions) !GetAWSOrganizationsAccessStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAWSOrganizationsAccessStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.GetAWSOrganizationsAccessStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAWSOrganizationsAccessStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAWSOrganizationsAccessStatusOutput, body, allocator);
}
