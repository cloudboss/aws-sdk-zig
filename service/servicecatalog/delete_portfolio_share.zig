...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePortfolioShareInput, options: Options) !DeletePortfolioShareOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePortfolioShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DeletePortfolioShare");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePortfolioShareOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeletePortfolioShareOutput, body, alloc);
}
