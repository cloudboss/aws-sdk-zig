...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProductInput, options: Options) !CreateProductOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProductInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.CreateProduct");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProductOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateProductOutput, body, alloc);
}
