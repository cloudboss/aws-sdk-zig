...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProvisioningParametersInput, options: Options) !DescribeProvisioningParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProvisioningParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeProvisioningParameters");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProvisioningParametersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProvisioningParametersOutput, body, alloc);
}
