...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceActionsForProvisioningArtifactInput, options: Options) !ListServiceActionsForProvisioningArtifactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServiceActionsForProvisioningArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListServiceActionsForProvisioningArtifact");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServiceActionsForProvisioningArtifactOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListServiceActionsForProvisioningArtifactOutput, body, alloc);
}
