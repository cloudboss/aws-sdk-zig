...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchProductsAsAdminInput, options: Options) !SearchProductsAsAdminOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchProductsAsAdminInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.SearchProductsAsAdmin");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchProductsAsAdminOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchProductsAsAdminOutput, body, alloc);
}
