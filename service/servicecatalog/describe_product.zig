...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProductInput, options: Options) !DescribeProductOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProductInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeProduct");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProductOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProductOutput, body, alloc);
}
