...
pub const CreateTagOptionInput = struct {
    /// The TagOption key.
    key: []const u8,
...
pub const CreateTagOptionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTagOptionInput, options: CallOptions) !CreateTagOptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTagOptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.CreateTagOption");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTagOptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTagOptionOutput, body, allocator);
}
