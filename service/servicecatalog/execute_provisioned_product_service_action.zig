...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteProvisionedProductServiceActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ExecuteProvisionedProductServiceAction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteProvisionedProductServiceActionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExecuteProvisionedProductServiceActionOutput, body, alloc);
}
