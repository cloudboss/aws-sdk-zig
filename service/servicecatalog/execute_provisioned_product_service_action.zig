...
pub const ExecuteProvisionedProductServiceActionInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
    ...
    /// override the default value generated by Service Catalog. If the parameters
    /// field is not provided, no additional parameters are passed and default
...
pub const ExecuteProvisionedProductServiceActionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteProvisionedProductServiceActionInput, options: CallOptions) !ExecuteProvisionedProductServiceActionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExecuteProvisionedProductServiceActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.ExecuteProvisionedProductServiceAction");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExecuteProvisionedProductServiceActionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExecuteProvisionedProductServiceActionOutput, body, allocator);
}
