...
pub const ListBudgetsForResourceInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub const ListBudgetsForResourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBudgetsForResourceInput, options: CallOptions) !ListBudgetsForResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBudgetsForResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.ListBudgetsForResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBudgetsForResourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBudgetsForResourceOutput, body, allocator);
}
