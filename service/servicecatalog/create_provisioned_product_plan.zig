...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProvisionedProductPlanInput, options: Options) !CreateProvisionedProductPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProvisionedProductPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.CreateProvisionedProductPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProvisionedProductPlanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateProvisionedProductPlanOutput, body, alloc);
}
