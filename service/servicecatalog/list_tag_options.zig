...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTagOptionsInput, options: Options) !ListTagOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTagOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListTagOptions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTagOptionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTagOptionsOutput, body, alloc);
}
