...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourcesForTagOptionInput, options: Options) !ListResourcesForTagOptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourcesForTagOptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListResourcesForTagOption");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourcesForTagOptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourcesForTagOptionOutput, body, alloc);
}
