...
pub const ListPortfoliosInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub const ListPortfoliosOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPortfoliosInput, options: CallOptions) !ListPortfoliosOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPortfoliosInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.ListPortfolios");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPortfoliosOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPortfoliosOutput, body, allocator);
}
