...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecordHistoryInput, options: Options) !ListRecordHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecordHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListRecordHistory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecordHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRecordHistoryOutput, body, alloc);
}
