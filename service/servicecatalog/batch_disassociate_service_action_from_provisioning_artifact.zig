...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDisassociateServiceActionFromProvisioningArtifactInput, options: Options) !BatchDisassociateServiceActionFromProvisioningArtifactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDisassociateServiceActionFromProvisioningArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.BatchDisassociateServiceActionFromProvisioningArtifact");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDisassociateServiceActionFromProvisioningArtifactOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDisassociateServiceActionFromProvisioningArtifactOutput, body, alloc);
}
