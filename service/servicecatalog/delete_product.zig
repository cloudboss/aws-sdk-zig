...
pub const DeleteProductInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteProductInput, options: Options) !DeleteProductOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteProductInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.DeleteProduct");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteProductOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
