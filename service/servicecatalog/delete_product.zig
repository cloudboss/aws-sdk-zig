...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteProductInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DeleteProduct");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteProductOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
