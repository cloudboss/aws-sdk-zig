...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchProvisionedProductsInput, options: Options) !SearchProvisionedProductsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchProvisionedProductsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.SearchProvisionedProducts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchProvisionedProductsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchProvisionedProductsOutput, body, alloc);
}
