...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchProductsInput, options: Options) !SearchProductsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchProductsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.SearchProducts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchProductsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchProductsOutput, body, alloc);
}
