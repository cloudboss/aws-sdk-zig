...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ProvisionProductInput, options: Options) !ProvisionProductOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ProvisionProductInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ProvisionProduct");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ProvisionProductOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ProvisionProductOutput, body, alloc);
}
