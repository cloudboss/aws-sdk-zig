...
pub const DescribeConstraintInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub const DescribeConstraintOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConstraintInput, options: CallOptions) !DescribeConstraintOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeConstraintInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeConstraint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeConstraintOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConstraintOutput, body, allocator);
}
