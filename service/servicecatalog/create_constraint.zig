...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConstraintInput, options: CallOptions) !CreateConstraintOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateConstraintInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.CreateConstraint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateConstraintOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateConstraintOutput, body, allocator);
}
