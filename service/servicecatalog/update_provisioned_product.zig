...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProvisionedProductInput, options: Options) !UpdateProvisionedProductOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProvisionedProductInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.UpdateProvisionedProduct");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProvisionedProductOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateProvisionedProductOutput, body, alloc);
}
