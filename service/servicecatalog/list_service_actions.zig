...
pub const ListServiceActionsInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub const ListServiceActionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceActionsInput, options: CallOptions) !ListServiceActionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListServiceActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.ListServiceActions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListServiceActionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListServiceActionsOutput, body, allocator);
}
