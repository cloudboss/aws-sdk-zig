...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePortfolioInput, options: Options) !DescribePortfolioOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePortfolioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribePortfolio");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePortfolioOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePortfolioOutput, body, alloc);
}
