...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePortfolioShareStatusInput, options: Options) !DescribePortfolioShareStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePortfolioShareStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribePortfolioShareStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePortfolioShareStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePortfolioShareStatusOutput, body, alloc);
}
