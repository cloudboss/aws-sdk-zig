...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPrincipalsForPortfolioInput, options: Options) !ListPrincipalsForPortfolioOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPrincipalsForPortfolioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListPrincipalsForPortfolio");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPrincipalsForPortfolioOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPrincipalsForPortfolioOutput, body, alloc);
}
