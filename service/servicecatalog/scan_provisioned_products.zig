...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ScanProvisionedProductsInput, options: Options) !ScanProvisionedProductsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ScanProvisionedProductsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ScanProvisionedProducts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ScanProvisionedProductsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ScanProvisionedProductsOutput, body, alloc);
}
