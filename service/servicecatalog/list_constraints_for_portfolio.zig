...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConstraintsForPortfolioInput, options: Options) !ListConstraintsForPortfolioOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConstraintsForPortfolioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListConstraintsForPortfolio");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConstraintsForPortfolioOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListConstraintsForPortfolioOutput, body, alloc);
}
