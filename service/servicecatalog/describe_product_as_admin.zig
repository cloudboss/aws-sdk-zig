...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProductAsAdminInput, options: Options) !DescribeProductAsAdminOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProductAsAdminInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeProductAsAdmin");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProductAsAdminOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProductAsAdminOutput, body, alloc);
}
