...
pub const UpdateConstraintInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub const UpdateConstraintOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConstraintInput, options: CallOptions) !UpdateConstraintOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateConstraintInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.UpdateConstraint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateConstraintOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateConstraintOutput, body, allocator);
}
