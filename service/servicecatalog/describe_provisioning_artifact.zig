...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProvisioningArtifactInput, options: Options) !DescribeProvisioningArtifactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProvisioningArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeProvisioningArtifact");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProvisioningArtifactOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProvisioningArtifactOutput, body, alloc);
}
