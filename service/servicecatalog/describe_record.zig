...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRecordInput, options: Options) !DescribeRecordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRecordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeRecord");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRecordOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRecordOutput, body, alloc);
}
