...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociatePrincipalFromPortfolioInput, options: Options) !DisassociatePrincipalFromPortfolioOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociatePrincipalFromPortfolioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DisassociatePrincipalFromPortfolio");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociatePrincipalFromPortfolioOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
