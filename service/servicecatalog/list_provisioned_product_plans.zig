...
const ProvisionedProductPlanSummary = @import("provisioned_product_plan_summary.zig").ProvisionedProductPlanSummary;

pub const ListProvisionedProductPlansInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub const ListProvisionedProductPlansOutput = struct {
    /// The page token to use to retrieve the next set of results. If there are no
    /// additional results, this value is null.
    next_page_token: ?[]const u8 = null,

    /// Information about the plans.
    provisioned_product_plans: ?[]const ProvisionedProductPlanSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProvisionedProductPlansInput, options: CallOptions) !ListProvisionedProductPlansOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProvisionedProductPlansInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.ListProvisionedProductPlans");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListProvisionedProductPlansOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProvisionedProductPlansOutput, body, allocator);
}
