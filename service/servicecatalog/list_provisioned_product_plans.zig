...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProvisionedProductPlansInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.ListProvisionedProductPlans");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProvisionedProductPlansOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProvisionedProductPlansOutput, body, alloc);
}
