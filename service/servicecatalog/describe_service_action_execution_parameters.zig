...
pub const DescribeServiceActionExecutionParametersInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub const DescribeServiceActionExecutionParametersOutput = struct {
    /// The parameters of the self-service action.
    service_action_parameters: ?[]const ExecutionParameter = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeServiceActionExecutionParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.DescribeServiceActionExecutionParameters");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeServiceActionExecutionParametersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeServiceActionExecutionParametersOutput, body, allocator);
}
