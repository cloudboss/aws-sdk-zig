...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePortfolioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.DeletePortfolio");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePortfolioOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
