...
pub const DeletePortfolioInput = struct {
    /// The language code.
    ///
    /// * `jp` - Japanese
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePortfolioInput, options: Options) !DeletePortfolioOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePortfolioInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.DeletePortfolio");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePortfolioOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
