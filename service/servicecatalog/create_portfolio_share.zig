...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePortfolioShareInput, options: Options) !CreatePortfolioShareOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePortfolioShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWS242ServiceCatalogService.CreatePortfolioShare");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePortfolioShareOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePortfolioShareOutput, body, alloc);
}
