...
pub const AssociateBudgetWithResourceInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateBudgetWithResourceInput, options: Options) !AssociateBudgetWithResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateBudgetWithResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWS242ServiceCatalogService.AssociateBudgetWithResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateBudgetWithResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
