...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelChangeSetInput, options: Options) !CancelChangeSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelChangeSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelChangeSetOutput {
    var result: CancelChangeSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelChangeSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
