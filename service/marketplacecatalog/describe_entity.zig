...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEntityInput, config: *aws.Config) !aws.http.Request {
    ...
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "entityId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.entity_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEntityOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeEntityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
