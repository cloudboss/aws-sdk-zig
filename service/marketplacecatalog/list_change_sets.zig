...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChangeSetsInput, options: Options) !ListChangeSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChangeSetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChangeSetsOutput {
    var result: ListChangeSetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChangeSetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
