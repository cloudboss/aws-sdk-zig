...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDescribeEntitiesInput, options: Options) !BatchDescribeEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDescribeEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"EntityRequestList\":");
    try aws.json.writeValue(@TypeOf(input.entity_request_list), input.entity_request_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDescribeEntitiesOutput {
    var result: BatchDescribeEntitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDescribeEntitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
