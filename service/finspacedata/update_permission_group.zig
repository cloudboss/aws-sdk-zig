...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePermissionGroupInput, options: Options) !UpdatePermissionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePermissionGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePermissionGroupOutput {
    var result: UpdatePermissionGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePermissionGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
