...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExternalDataViewAccessDetailsInput, options: Options) !GetExternalDataViewAccessDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExternalDataViewAccessDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/external-access-details");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExternalDataViewAccessDetailsOutput {
    var result: GetExternalDataViewAccessDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetExternalDataViewAccessDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
