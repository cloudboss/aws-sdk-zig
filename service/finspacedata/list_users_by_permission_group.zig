...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUsersByPermissionGroupInput, options: Options) !ListUsersByPermissionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUsersByPermissionGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUsersByPermissionGroupOutput {
    var result: ListUsersByPermissionGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUsersByPermissionGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
