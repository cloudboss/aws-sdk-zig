...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateChangesetInput, options: Options) !UpdateChangesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChangesetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChangesetOutput {
    var result: UpdateChangesetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChangesetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
