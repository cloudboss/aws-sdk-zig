...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePermissionGroupInput, options: Options) !CreatePermissionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePermissionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"applicationPermissions\":");
    try aws.json.writeValue(@TypeOf(input.application_permissions), input.application_permissions, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePermissionGroupOutput {
    var result: CreatePermissionGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePermissionGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
