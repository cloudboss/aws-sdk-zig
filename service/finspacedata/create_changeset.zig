...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateChangesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"changeType\":");
    try aws.json.writeValue(@TypeOf(input.change_type), input.change_type, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateChangesetOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChangesetOutput, body, allocator);
    }
    _ = status;
    _ = headers;
