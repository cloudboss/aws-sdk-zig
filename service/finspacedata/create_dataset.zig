...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDatasetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dataset_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"datasetDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"datasetTitle\":");
    try aws.json.writeValue(@TypeOf(input.dataset_title), input.dataset_title, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"kind\":");
    try aws.json.writeValue(@TypeOf(input.kind), input.kind, alloc, &body_buf);
    has_prev = true;
    if (input.owner_info) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ownerInfo\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"permissionGroupParams\":");
    try aws.json.writeValue(@TypeOf(input.permission_group_params), input.permission_group_params, alloc, &body_buf);
    has_prev = true;
    if (input.schema_definition) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"schemaDefinition\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDatasetOutput {
    var result: CreateDatasetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDatasetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
