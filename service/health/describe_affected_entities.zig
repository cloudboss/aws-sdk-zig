...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAffectedEntitiesInput, options: Options) !DescribeAffectedEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAffectedEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHealth_20160804.DescribeAffectedEntities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAffectedEntitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAffectedEntitiesOutput, body, alloc);
}
