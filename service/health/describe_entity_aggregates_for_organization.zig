...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEntityAggregatesForOrganizationInput, options: Options) !DescribeEntityAggregatesForOrganizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEntityAggregatesForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHealth_20160804.DescribeEntityAggregatesForOrganization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEntityAggregatesForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEntityAggregatesForOrganizationOutput, body, alloc);
}
