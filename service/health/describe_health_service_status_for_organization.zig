...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHealthServiceStatusForOrganizationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHealth_20160804.DescribeHealthServiceStatusForOrganization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHealthServiceStatusForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeHealthServiceStatusForOrganizationOutput, body, alloc);
}
