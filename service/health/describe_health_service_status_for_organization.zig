...
pub const DescribeHealthServiceStatusForOrganizationOutput = struct {
    ...
    health_service_access_status_for_organization: ?[]const u8 = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHealthServiceStatusForOrganizationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSHealth_20160804.DescribeHealthServiceStatusForOrganization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHealthServiceStatusForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeHealthServiceStatusForOrganizationOutput, body, allocator);
}
