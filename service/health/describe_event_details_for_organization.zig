...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEventDetailsForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHealth_20160804.DescribeEventDetailsForOrganization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEventDetailsForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEventDetailsForOrganizationOutput, body, alloc);
}
