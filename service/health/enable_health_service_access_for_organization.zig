...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableHealthServiceAccessForOrganizationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSHealth_20160804.EnableHealthServiceAccessForOrganization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableHealthServiceAccessForOrganizationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
