...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableHealthServiceAccessForOrganizationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHealth_20160804.EnableHealthServiceAccessForOrganization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableHealthServiceAccessForOrganizationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
