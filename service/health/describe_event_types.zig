...
const EventTypeFilter = @import("event_type_filter.zig").EventTypeFilter;
const EventType = @import("event_type.zig").EventType;

pub const DescribeEventTypesInput = struct {
...
pub const DescribeEventTypesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventTypesInput, options: CallOptions) !DescribeEventTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEventTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSHealth_20160804.DescribeEventTypes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEventTypesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEventTypesOutput, body, allocator);
}
