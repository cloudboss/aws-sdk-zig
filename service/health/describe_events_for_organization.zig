...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventsForOrganizationInput, options: CallOptions) !DescribeEventsForOrganizationOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "health");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEventsForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSHealth_20160804.DescribeEventsForOrganization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEventsForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEventsForOrganizationOutput, body, allocator);
}
