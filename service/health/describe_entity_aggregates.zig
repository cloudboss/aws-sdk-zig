...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEntityAggregatesInput, options: Options) !DescribeEntityAggregatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEntityAggregatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHealth_20160804.DescribeEntityAggregates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEntityAggregatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEntityAggregatesOutput, body, alloc);
}
