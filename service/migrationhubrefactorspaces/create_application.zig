...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.api_gateway_proxy) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ApiGatewayProxy\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ProxyType\":");
    try aws.json.writeValue(@TypeOf(input.proxy_type), input.proxy_type, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
