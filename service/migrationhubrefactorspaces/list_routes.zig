...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoutesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoutesOutput {
    var result: ListRoutesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoutesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
