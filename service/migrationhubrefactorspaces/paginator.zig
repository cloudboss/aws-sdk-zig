...
pub const ListEnvironmentVpcsPaginator = struct {
    client: *Client,
    params: list_environment_vpcs.ListEnvironmentVpcsInput,
    next_token: ?[]const u8 = null,
    done: bool = false,
    allocator: std.mem.Allocator,
    ...
    pub fn next(self: *Self, allocator: std.mem.Allocator, options: list_environment_vpcs.Options) !list_environment_vpcs.ListEnvironmentVpcsOutput {
        if (self.done) {
            return error.EndOfPagination;
        }
        ...
        const output = try list_environment_vpcs.execute(self.client, allocator, self.params, options);
