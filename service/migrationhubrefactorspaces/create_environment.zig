...
const CreateEnvironmentInput = @import("create_environment_request.zig").CreateEnvironmentRequest;

const CreateEnvironmentOutput = @import("create_environment_response.zig").CreateEnvironmentResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEnvironmentInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"NetworkFabricType\":");
    try aws.json.writeValue(@TypeOf(input.network_fabric_type), input.network_fabric_type, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
