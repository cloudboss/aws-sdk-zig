...
pub const ListContentsInput = struct {
...
pub const ListContentsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContentsInput, options: Options) !ListContentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContentsOutput {
    var result: ListContentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
