...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContentsInput, options: CallOptions) !ListContentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListContentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListContentsOutput {
    var result: ListContentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
