...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchContentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search");
    ...
    try body_buf.appendSlice(alloc, "\"searchExpression\":");
    try aws.json.writeValue(@TypeOf(input.search_expression), input.search_expression, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchContentOutput {
    var result: SearchContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchContentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
