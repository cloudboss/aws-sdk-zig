...
pub const CreateKnowledgeBaseInput = struct {
    ...
    /// your own content.
    /// EXTERNAL knowledge bases support integrations with third-party systems whose
    /// content is
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKnowledgeBaseInput, options: Options) !CreateKnowledgeBaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKnowledgeBaseInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"knowledgeBaseType\":");
    try aws.json.writeValue(@TypeOf(input.knowledge_base_type), input.knowledge_base_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.rendering_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"renderingConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.source_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sourceConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKnowledgeBaseOutput {
    var result: CreateKnowledgeBaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKnowledgeBaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
