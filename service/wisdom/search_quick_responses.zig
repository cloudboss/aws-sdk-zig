...
const QuickResponseSearchExpression = @import("quick_response_search_expression.zig").QuickResponseSearchExpression;
...
pub const SearchQuickResponsesOutput = struct {
    /// The token for the next set of results. Use the value returned in the
    /// previous
    /// response in the next request to retrieve the next set of results.
    next_token: ?[]const u8 = null,
    ...
    results: ?[]const QuickResponseSearchResultData = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchQuickResponsesInput, options: Options) !SearchQuickResponsesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchQuickResponsesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search/quickResponses");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchQuickResponsesOutput {
    var result: SearchQuickResponsesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchQuickResponsesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
