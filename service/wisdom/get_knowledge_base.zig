...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKnowledgeBaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKnowledgeBaseOutput {
    var result: GetKnowledgeBaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetKnowledgeBaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
