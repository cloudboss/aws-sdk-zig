...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQuickResponseInput, options: Options) !GetQuickResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQuickResponseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQuickResponseOutput {
    var result: GetQuickResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQuickResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
