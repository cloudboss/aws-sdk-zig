...
pub const ListImportJobsOutput = struct {
    ...
    import_job_summaries: ?[]const ImportJobSummary = null,
    ...
    pub const json_field_names = .{
        .import_job_summaries = "importJobSummaries",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImportJobsInput, options: Options) !ListImportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImportJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImportJobsOutput {
    var result: ListImportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImportJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
