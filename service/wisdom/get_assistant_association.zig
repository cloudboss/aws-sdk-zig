...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssistantAssociationInput, options: Options) !GetAssistantAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssistantAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.assistant_association_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssistantAssociationOutput {
    var result: GetAssistantAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssistantAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
