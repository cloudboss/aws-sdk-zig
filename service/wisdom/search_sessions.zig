...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchSessionsInput, options: CallOptions) !SearchSessionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/searchSessions");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchSessionsOutput {
    var result: SearchSessionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchSessionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
