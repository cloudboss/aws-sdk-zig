...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContentSummaryInput, options: Options) !GetContentSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContentSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/summary");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContentSummaryOutput {
    var result: GetContentSummaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContentSummaryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
