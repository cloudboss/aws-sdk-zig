...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContentInput, options: CallOptions) !CreateContentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateContentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateContentOutput {
    var result: CreateContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateContentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
