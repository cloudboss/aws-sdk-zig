...
pub const CreateContentInput = struct {
    ...
    /// a unique
    /// name. You can retrieve a piece of content using only its knowledge base and
    /// its name with the
    /// [SearchContent](https://docs.aws.amazon.com/wisdom/latest/APIReference/API_SearchContent.html) API.
    name: []const u8,
...
pub const CreateContentOutput = struct {
    /// The content.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContentInput, options: Options) !CreateContentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/contents");
    ...
    if (input.override_link_out_uri) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"overrideLinkOutUri\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"uploadId\":");
    try aws.json.writeValue(@TypeOf(input.upload_id), input.upload_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContentOutput {
    var result: CreateContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateContentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
