...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssistantInput, options: Options) !GetAssistantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssistantInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssistantOutput {
    var result: GetAssistantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssistantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
