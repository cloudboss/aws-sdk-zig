...
pub const UpdateContentInput = struct {
    ...
    /// recommendations. For
    /// example, when synchronizing data between an external system and Wisdom, you
    /// can store an
    ...
    /// this argument
    /// overrides it for this piece of content. To remove an existing
    /// `overrideLinkOurUri`,
    ...
    /// ensure
    /// content was not modified since it was last read. If it has been modified,
    /// this API throws a
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContentInput, options: Options) !UpdateContentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.remove_override_link_out_uri) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"removeOverrideLinkOutUri\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContentOutput {
    var result: UpdateContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateContentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
