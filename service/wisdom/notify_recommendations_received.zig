...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: NotifyRecommendationsReceivedInput, options: Options) !NotifyRecommendationsReceivedOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: NotifyRecommendationsReceivedInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/recommendations/notify");
    ...
    try aws.json.writeValue(@TypeOf(input.recommendation_ids), input.recommendation_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !NotifyRecommendationsReceivedOutput {
    var result: NotifyRecommendationsReceivedOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(NotifyRecommendationsReceivedOutput, body, alloc);
    }
    _ = status;
    _ = headers;
