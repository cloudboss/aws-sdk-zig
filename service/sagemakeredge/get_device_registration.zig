...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeviceRegistrationInput, options: Options) !GetDeviceRegistrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeviceRegistrationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeviceRegistrationOutput {
    var result: GetDeviceRegistrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDeviceRegistrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
