...
const GetDomainInput = @import("get_domain_request.zig").GetDomainRequest;

const GetDomainOutput = @import("get_domain_response.zig").GetDomainResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainOutput {
    var result: GetDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
