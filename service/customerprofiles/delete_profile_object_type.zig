...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteProfileObjectTypeInput, options: Options) !DeleteProfileObjectTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteProfileObjectTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteProfileObjectTypeOutput {
    var result: DeleteProfileObjectTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteProfileObjectTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
