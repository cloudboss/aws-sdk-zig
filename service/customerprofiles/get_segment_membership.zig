...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSegmentMembershipInput, options: Options) !GetSegmentMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSegmentMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/membership");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSegmentMembershipOutput {
    var result: GetSegmentMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSegmentMembershipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
