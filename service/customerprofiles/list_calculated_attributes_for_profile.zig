...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCalculatedAttributesForProfileInput, options: Options) !ListCalculatedAttributesForProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCalculatedAttributesForProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCalculatedAttributesForProfileOutput {
    var result: ListCalculatedAttributesForProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCalculatedAttributesForProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
