...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddProfileKeyInput, options: Options) !AddProfileKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddProfileKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/profiles/keys");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddProfileKeyOutput {
    var result: AddProfileKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddProfileKeyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
