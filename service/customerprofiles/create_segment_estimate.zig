...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSegmentEstimateInput, options: Options) !CreateSegmentEstimateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSegmentEstimateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/segment-estimates");
    ...
    if (input.segment_query) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"SegmentQuery\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.segment_sql_query) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SegmentSqlQuery\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSegmentEstimateOutput {
    var result: CreateSegmentEstimateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSegmentEstimateOutput, body, alloc);
    }
