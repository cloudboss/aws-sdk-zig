...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRecommenderInput, options: Options) !UpdateRecommenderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRecommenderInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/recommenders/");
    try path_buf.appendSlice(alloc, input.recommender_name);
    ...
    if (input.recommender_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecommenderConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRecommenderOutput {
    var result: UpdateRecommenderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRecommenderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
