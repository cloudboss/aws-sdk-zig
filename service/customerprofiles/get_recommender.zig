...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRecommenderInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.training_metrics_count) |v| {
        ...
        try query_buf.appendSlice(alloc, "training-metrics-count=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRecommenderOutput {
    var result: GetRecommenderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRecommenderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
