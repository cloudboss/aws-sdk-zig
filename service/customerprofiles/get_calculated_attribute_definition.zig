...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCalculatedAttributeDefinitionInput, options: Options) !GetCalculatedAttributeDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCalculatedAttributeDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCalculatedAttributeDefinitionOutput {
    var result: GetCalculatedAttributeDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCalculatedAttributeDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
