...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDomainLayoutInput, options: Options) !GetDomainLayoutOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainLayoutInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainLayoutOutput {
    var result: GetDomainLayoutOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainLayoutOutput, body, alloc);
    }
    _ = status;
    _ = headers;
