...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEventTriggerInput, options: CallOptions) !UpdateEventTriggerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateEventTriggerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateEventTriggerOutput {
    var result: UpdateEventTriggerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEventTriggerOutput, body, allocator);
    }
    _ = status;
    _ = headers;
