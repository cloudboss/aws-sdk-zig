...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUploadJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.data_expiry) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DataExpiry\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UniqueKey\":");
    try aws.json.writeValue(@TypeOf(input.unique_key), input.unique_key, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUploadJobOutput {
    var result: CreateUploadJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateUploadJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
