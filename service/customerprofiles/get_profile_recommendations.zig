...
pub const GetProfileRecommendationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProfileRecommendationsInput, options: Options) !GetProfileRecommendationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProfileRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RecommenderName\":");
    try aws.json.writeValue(@TypeOf(input.recommender_name), input.recommender_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProfileRecommendationsOutput {
    var result: GetProfileRecommendationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProfileRecommendationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
