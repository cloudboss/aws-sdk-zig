...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSegmentDefinitionInput, options: Options) !GetSegmentDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSegmentDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSegmentDefinitionOutput {
    var result: GetSegmentDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSegmentDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
