...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityResolutionJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityResolutionJobOutput {
    var result: GetIdentityResolutionJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIdentityResolutionJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
