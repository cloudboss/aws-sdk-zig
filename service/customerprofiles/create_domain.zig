...
const CreateDomainInput = @import("create_domain_request.zig").CreateDomainRequest;

const CreateDomainOutput = @import("create_domain_response.zig").CreateDomainResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.data_store) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DataStore\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.dead_letter_queue_url) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DeadLetterQueueUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.default_encryption_key) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DefaultEncryptionKey\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DefaultExpirationDays\":");
    try aws.json.writeValue(@TypeOf(input.default_expiration_days), input.default_expiration_days, alloc, &body_buf);
    has_prev = true;
    if (input.matching) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Matching\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.rule_based_matching) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RuleBasedMatching\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDomainOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
