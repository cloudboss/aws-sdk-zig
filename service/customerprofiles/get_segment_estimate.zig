...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSegmentEstimateInput, options: Options) !GetSegmentEstimateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSegmentEstimateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.estimate_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSegmentEstimateOutput {
    var result: GetSegmentEstimateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSegmentEstimateOutput, body, alloc);
    }
