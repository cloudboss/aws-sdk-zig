...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSegmentSnapshotInput, options: Options) !CreateSegmentSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSegmentSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.segment_definition_name);
    ...
    try body_buf.appendSlice(alloc, "\"DataFormat\":");
    try aws.json.writeValue(@TypeOf(input.data_format), input.data_format, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.encryption_key) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EncryptionKey\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.role_arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSegmentSnapshotOutput {
    var result: CreateSegmentSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSegmentSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
