...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountIntegrationsInput, options: Options) !ListAccountIntegrationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountIntegrationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountIntegrationsOutput {
    var result: ListAccountIntegrationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccountIntegrationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
