...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIntegrationsInput, options: Options) !ListIntegrationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIntegrationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_hidden) |v| {
        ...
        try query_buf.appendSlice(alloc, "include-hidden=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIntegrationsOutput {
    var result: ListIntegrationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIntegrationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
