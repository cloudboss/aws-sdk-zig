...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCalculatedAttributeDefinitionInput, options: CallOptions) !UpdateCalculatedAttributeDefinitionOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateCalculatedAttributeDefinitionOutput {
    var result: UpdateCalculatedAttributeDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCalculatedAttributeDefinitionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
