...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDomainLayoutInput, options: Options) !UpdateDomainLayoutOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDomainLayoutInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDomainLayoutOutput {
    var result: UpdateDomainLayoutOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDomainLayoutOutput, body, alloc);
    }
    _ = status;
    _ = headers;
