...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSimilarProfilesInput, options: Options) !GetSimilarProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSimilarProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"MatchType\":");
    try aws.json.writeValue(@TypeOf(input.match_type), input.match_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SearchKey\":");
    try aws.json.writeValue(@TypeOf(input.search_key), input.search_key, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SearchValue\":");
    try aws.json.writeValue(@TypeOf(input.search_value), input.search_value, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSimilarProfilesOutput {
    var result: GetSimilarProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSimilarProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
