...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProfileObjectTypeTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProfileObjectTypeTemplateOutput {
    var result: GetProfileObjectTypeTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProfileObjectTypeTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
