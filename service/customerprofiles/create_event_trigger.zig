...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEventTriggerInput, options: CallOptions) !CreateEventTriggerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateEventTriggerInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"EventTriggerConditions\":");
    try aws.json.writeValue(@TypeOf(input.event_trigger_conditions), input.event_trigger_conditions, allocator, &body_buf);
    has_prev = true;
    if (input.event_trigger_limits) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"EventTriggerLimits\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    if (input.segment_filter) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"SegmentFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
