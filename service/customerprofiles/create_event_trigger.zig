...
pub const CreateEventTriggerInput = struct {
    ...
    /// destination.
    event_trigger_conditions: []const EventTriggerCondition,
...
pub const CreateEventTriggerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEventTriggerInput, options: Options) !CreateEventTriggerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEventTriggerInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.event_trigger_conditions), input.event_trigger_conditions, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEventTriggerOutput {
    var result: CreateEventTriggerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEventTriggerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
