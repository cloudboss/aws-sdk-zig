...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDomainLayoutInput, options: Options) !DeleteDomainLayoutOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDomainLayoutInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDomainLayoutOutput {
    var result: DeleteDomainLayoutOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDomainLayoutOutput, body, alloc);
    }
    _ = status;
    _ = headers;
