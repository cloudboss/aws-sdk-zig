...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUploadJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUploadJobsOutput {
    var result: ListUploadJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUploadJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
