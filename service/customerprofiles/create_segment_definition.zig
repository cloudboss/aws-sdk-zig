...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSegmentDefinitionInput, options: Options) !CreateSegmentDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSegmentDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.segment_groups) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SegmentGroups\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSegmentDefinitionOutput {
    var result: CreateSegmentDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSegmentDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
