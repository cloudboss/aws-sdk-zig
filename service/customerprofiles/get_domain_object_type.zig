...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainObjectTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainObjectTypeOutput {
    var result: GetDomainObjectTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainObjectTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
