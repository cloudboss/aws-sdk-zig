...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetCalculatedAttributeForProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batch-get-for-profiles");
    ...
    if (input.condition_overrides) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ConditionOverrides\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetCalculatedAttributeForProfileOutput {
    var result: BatchGetCalculatedAttributeForProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetCalculatedAttributeForProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
