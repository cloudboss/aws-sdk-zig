...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDomainObjectTypeInput, options: Options) !PutDomainObjectTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDomainObjectTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDomainObjectTypeOutput {
    var result: PutDomainObjectTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutDomainObjectTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
