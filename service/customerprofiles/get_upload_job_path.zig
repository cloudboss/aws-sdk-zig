...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUploadJobPathInput, options: Options) !GetUploadJobPathOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUploadJobPathInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUploadJobPathOutput {
    var result: GetUploadJobPathOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUploadJobPathOutput, body, alloc);
    }
    _ = status;
    _ = headers;
