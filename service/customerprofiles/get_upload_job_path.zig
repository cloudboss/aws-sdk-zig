...
pub const GetUploadJobPathInput = struct {
...
pub const GetUploadJobPathOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUploadJobPathInput, options: CallOptions) !GetUploadJobPathOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetUploadJobPathInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetUploadJobPathOutput {
    var result: GetUploadJobPathOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUploadJobPathOutput, body, allocator);
    }
    _ = status;
    _ = headers;
