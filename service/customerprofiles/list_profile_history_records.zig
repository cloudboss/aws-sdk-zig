...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProfileHistoryRecordsInput, options: Options) !ListProfileHistoryRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProfileHistoryRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/profiles/history-records");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProfileHistoryRecordsOutput {
    var result: ListProfileHistoryRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProfileHistoryRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
