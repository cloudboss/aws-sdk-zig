...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRecommenderInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RecommenderRecipeName\":");
    try aws.json.writeValue(@TypeOf(input.recommender_recipe_name), input.recommender_recipe_name, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRecommenderOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRecommenderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
