...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/profiles/search");
    ...
    if (input.additional_search_keys) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AdditionalSearchKeys\":");
    ...
    if (input.logical_operator) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LogicalOperator\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchProfilesOutput {
    var result: SearchProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
