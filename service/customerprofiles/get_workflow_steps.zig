...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkflowStepsInput, options: Options) !GetWorkflowStepsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkflowStepsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkflowStepsOutput {
    var result: GetWorkflowStepsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkflowStepsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
