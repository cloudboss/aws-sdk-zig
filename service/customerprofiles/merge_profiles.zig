...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MergeProfilesInput, options: Options) !MergeProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: MergeProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/profiles/objects/merge");
    ...
    if (input.field_source_profile_ids) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"FieldSourceProfileIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MainProfileId\":");
    try aws.json.writeValue(@TypeOf(input.main_profile_id), input.main_profile_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ProfileIdsToBeMerged\":");
    try aws.json.writeValue(@TypeOf(input.profile_ids_to_be_merged), input.profile_ids_to_be_merged, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MergeProfilesOutput {
    var result: MergeProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(MergeProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
