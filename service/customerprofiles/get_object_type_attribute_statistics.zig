...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetObjectTypeAttributeStatisticsInput, options: Options) !GetObjectTypeAttributeStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetObjectTypeAttributeStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetObjectTypeAttributeStatisticsOutput {
    var result: GetObjectTypeAttributeStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetObjectTypeAttributeStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
