...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEventTriggerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEventTriggerOutput {
    var result: GetEventTriggerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEventTriggerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
