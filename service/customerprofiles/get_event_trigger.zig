...
pub const GetEventTriggerInput = struct {
...
pub const GetEventTriggerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEventTriggerInput, options: CallOptions) !GetEventTriggerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetEventTriggerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetEventTriggerOutput {
    var result: GetEventTriggerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEventTriggerOutput, body, allocator);
    }
    _ = status;
    _ = headers;
