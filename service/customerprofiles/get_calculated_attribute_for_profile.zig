...
pub const GetCalculatedAttributeForProfileInput = struct {
...
pub const GetCalculatedAttributeForProfileOutput = struct {
    /// The unique name of the calculated attribute.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCalculatedAttributeForProfileInput, options: CallOptions) !GetCalculatedAttributeForProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCalculatedAttributeForProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCalculatedAttributeForProfileOutput {
    var result: GetCalculatedAttributeForProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCalculatedAttributeForProfileOutput, body, allocator);
    }
    _ = status;
    _ = headers;
