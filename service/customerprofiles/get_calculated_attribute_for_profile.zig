...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCalculatedAttributeForProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCalculatedAttributeForProfileOutput {
    var result: GetCalculatedAttributeForProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCalculatedAttributeForProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
