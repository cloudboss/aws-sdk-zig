...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutProfileObjectTypeInput, options: Options) !PutProfileObjectTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutProfileObjectTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allow_profile_creation) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AllowProfileCreation\":");
    ...
    if (input.expiration_days) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ExpirationDays\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.fields) |v| {
    ...
    if (input.max_profile_object_count) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MaxProfileObjectCount\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_last_updated_timestamp_format) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceLastUpdatedTimestampFormat\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutProfileObjectTypeOutput {
    var result: PutProfileObjectTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutProfileObjectTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
