...
pub const DescribeKeyPhrasesDetectionJobInput = struct {
...
pub const DescribeKeyPhrasesDetectionJobOutput = struct {
    /// An object that contains the properties associated with a key phrases
    /// detection job.
    key_phrases_detection_job_properties: ?KeyPhrasesDetectionJobProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeKeyPhrasesDetectionJobInput, options: CallOptions) !DescribeKeyPhrasesDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeKeyPhrasesDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.DescribeKeyPhrasesDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeKeyPhrasesDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeKeyPhrasesDetectionJobOutput, body, allocator);
}
