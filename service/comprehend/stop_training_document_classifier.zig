...
fn serializeRequest(alloc: std.mem.Allocator, input: StopTrainingDocumentClassifierInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.StopTrainingDocumentClassifier");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopTrainingDocumentClassifierOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
