...
pub const StopEntitiesDetectionJobInput = struct {
...
pub const StopEntitiesDetectionJobOutput = struct {
    /// The identifier of the entities detection job to stop.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopEntitiesDetectionJobInput, options: CallOptions) !StopEntitiesDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopEntitiesDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StopEntitiesDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopEntitiesDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopEntitiesDetectionJobOutput, body, allocator);
}
