...
const DatasetFilter = @import("dataset_filter.zig").DatasetFilter;
const DatasetProperties = @import("dataset_properties.zig").DatasetProperties;
...
pub const ListDatasetsOutput = struct {
    ...
    dataset_properties_list: ?[]const DatasetProperties = null,

    /// Identifies the next page of results to return.
    next_token: ?[]const u8 = null,

    pub const json_field_names = .{
        .dataset_properties_list = "DatasetPropertiesList",
        .next_token = "NextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDatasetsInput, options: Options) !ListDatasetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDatasetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.ListDatasets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDatasetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDatasetsOutput, body, alloc);
}
