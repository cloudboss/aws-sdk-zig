...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDetectKeyPhrasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.BatchDetectKeyPhrases");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDetectKeyPhrasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDetectKeyPhrasesOutput, body, alloc);
}
