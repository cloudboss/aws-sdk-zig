...
const BatchDetectKeyPhrasesItemResult = @import("batch_detect_key_phrases_item_result.zig").BatchDetectKeyPhrasesItemResult;

pub const BatchDetectKeyPhrasesInput = struct {
...
pub const BatchDetectKeyPhrasesOutput = struct {
    ...
    result_list: ?[]const BatchDetectKeyPhrasesItemResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDetectKeyPhrasesInput, options: CallOptions) !BatchDetectKeyPhrasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDetectKeyPhrasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.BatchDetectKeyPhrases");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDetectKeyPhrasesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDetectKeyPhrasesOutput, body, allocator);
}
