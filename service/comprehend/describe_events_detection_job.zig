...
pub const DescribeEventsDetectionJobInput = struct {
...
pub const DescribeEventsDetectionJobOutput = struct {
    /// An object that contains the properties associated with an event detection
    /// job.
    events_detection_job_properties: ?EventsDetectionJobProperties = null,

    pub const json_field_names = .{
        .events_detection_job_properties = "EventsDetectionJobProperties",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventsDetectionJobInput, options: CallOptions) !DescribeEventsDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEventsDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.DescribeEventsDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEventsDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEventsDetectionJobOutput, body, allocator);
}
