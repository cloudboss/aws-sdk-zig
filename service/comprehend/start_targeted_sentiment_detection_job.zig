...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTargetedSentimentDetectionJobInput, options: Options) !StartTargetedSentimentDetectionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartTargetedSentimentDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.StartTargetedSentimentDetectionJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartTargetedSentimentDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartTargetedSentimentDetectionJobOutput, body, alloc);
}
