...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectKeyPhrasesInput, options: Options) !DetectKeyPhrasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectKeyPhrasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.DetectKeyPhrases");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectKeyPhrasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetectKeyPhrasesOutput, body, alloc);
}
