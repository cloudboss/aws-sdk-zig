...
pub const DetectKeyPhrasesInput = struct {
...
pub const DetectKeyPhrasesOutput = struct {
    /// A collection of key phrases that Amazon Comprehend identified in the input
    /// text. For
    /// each key phrase, the response provides the text of the key phrase, where the
    /// key phrase begins
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectKeyPhrasesInput, options: CallOptions) !DetectKeyPhrasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DetectKeyPhrasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.DetectKeyPhrases");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DetectKeyPhrasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetectKeyPhrasesOutput, body, allocator);
}
