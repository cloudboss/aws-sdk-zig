...
pub const DeleteFlywheelInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteFlywheelInput, options: Options) !DeleteFlywheelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteFlywheelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.DeleteFlywheel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteFlywheelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
