...
const DominantLanguageDetectionJobFilter = @import("dominant_language_detection_job_filter.zig").DominantLanguageDetectionJobFilter;
...
pub const ListDominantLanguageDetectionJobsInput = struct {
...
pub const ListDominantLanguageDetectionJobsOutput = struct {
    /// A list containing the properties of each job that is returned.
    dominant_language_detection_job_properties_list: ?[]const DominantLanguageDetectionJobProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDominantLanguageDetectionJobsInput, options: CallOptions) !ListDominantLanguageDetectionJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDominantLanguageDetectionJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.ListDominantLanguageDetectionJobs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDominantLanguageDetectionJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDominantLanguageDetectionJobsOutput, body, allocator);
}
