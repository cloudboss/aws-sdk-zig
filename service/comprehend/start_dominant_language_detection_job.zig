...
pub const StartDominantLanguageDetectionJobInput = struct {
...
pub const StartDominantLanguageDetectionJobOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartDominantLanguageDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StartDominantLanguageDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartDominantLanguageDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartDominantLanguageDetectionJobOutput, body, allocator);
}
