...
const FlywheelFilter = @import("flywheel_filter.zig").FlywheelFilter;
const FlywheelSummary = @import("flywheel_summary.zig").FlywheelSummary;

pub const ListFlywheelsInput = struct {
...
pub const ListFlywheelsOutput = struct {
    /// A list of flywheel properties retrieved by the service in response to the
    /// request.
    flywheel_summary_list: ?[]const FlywheelSummary = null,

    /// Identifies the next page of results to return.
    next_token: ?[]const u8 = null,

    pub const json_field_names = .{
        .flywheel_summary_list = "FlywheelSummaryList",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFlywheelsInput, options: CallOptions) !ListFlywheelsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFlywheelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.ListFlywheels");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFlywheelsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFlywheelsOutput, body, allocator);
}
