...
pub const StartTopicsDetectionJobInput = struct {
    ...
    /// with two
    /// files, `topic-terms.csv` that lists the terms associated with each topic,
    /// and
    ...
    /// containing
    /// the resources you are using for your topic detection job. For more
...
pub const StartTopicsDetectionJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTopicsDetectionJobInput, options: CallOptions) !StartTopicsDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartTopicsDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StartTopicsDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartTopicsDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartTopicsDetectionJobOutput, body, allocator);
}
