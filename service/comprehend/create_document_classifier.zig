...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDocumentClassifierInput, options: Options) !CreateDocumentClassifierOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDocumentClassifierInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.CreateDocumentClassifier");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDocumentClassifierOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDocumentClassifierOutput, body, alloc);
}
