...
pub const DescribeFlywheelInput = struct {
...
pub const DescribeFlywheelOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFlywheelInput, options: CallOptions) !DescribeFlywheelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFlywheelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.DescribeFlywheel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFlywheelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFlywheelOutput, body, allocator);
}
