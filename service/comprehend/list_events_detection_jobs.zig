...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEventsDetectionJobsInput, options: Options) !ListEventsDetectionJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventsDetectionJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.ListEventsDetectionJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEventsDetectionJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEventsDetectionJobsOutput, body, alloc);
}
