...
const UpdateDataSecurityConfig = @import("update_data_security_config.zig").UpdateDataSecurityConfig;
const FlywheelProperties = @import("flywheel_properties.zig").FlywheelProperties;

pub const UpdateFlywheelInput = struct {
    ...
    data_security_config: ?UpdateDataSecurityConfig = null,
...
pub const UpdateFlywheelOutput = struct {
    /// The flywheel properties.
    flywheel_properties: ?FlywheelProperties = null,

    pub const json_field_names = .{
        .flywheel_properties = "FlywheelProperties",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFlywheelInput, options: CallOptions) !UpdateFlywheelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFlywheelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.UpdateFlywheel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFlywheelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFlywheelOutput, body, allocator);
}
