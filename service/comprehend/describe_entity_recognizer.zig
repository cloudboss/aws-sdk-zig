...
pub const DescribeEntityRecognizerInput = struct {
...
pub const DescribeEntityRecognizerOutput = struct {
    /// Describes information associated with an entity recognizer.
    entity_recognizer_properties: ?EntityRecognizerProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEntityRecognizerInput, options: CallOptions) !DescribeEntityRecognizerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEntityRecognizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.DescribeEntityRecognizer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEntityRecognizerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEntityRecognizerOutput, body, allocator);
}
