...
const BatchDetectTargetedSentimentItemResult = @import("batch_detect_targeted_sentiment_item_result.zig").BatchDetectTargetedSentimentItemResult;

pub const BatchDetectTargetedSentimentInput = struct {
    /// The language of the input documents. Currently, English is the only
    /// supported language.
    language_code: LanguageCode,

    /// A list containing the UTF-8 encoded text of the input documents.
    /// The list can contain a maximum of 25 documents. The maximum size of each
    /// document is 5 KB.
...
pub const BatchDetectTargetedSentimentOutput = struct {
    /// List of errors that the operation can return.
    error_list: ?[]const BatchItemError = null,

    /// A list of objects containing the results of the operation.
    /// The results are sorted in ascending order by the `Index` field and match the
    /// order of the documents in the input list.
    /// If all of the documents contain an error, the `ResultList` is empty.
    result_list: ?[]const BatchDetectTargetedSentimentItemResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDetectTargetedSentimentInput, options: CallOptions) !BatchDetectTargetedSentimentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDetectTargetedSentimentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.BatchDetectTargetedSentiment");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDetectTargetedSentimentOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDetectTargetedSentimentOutput, body, allocator);
}
