...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDetectTargetedSentimentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.BatchDetectTargetedSentiment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDetectTargetedSentimentOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDetectTargetedSentimentOutput, body, alloc);
}
