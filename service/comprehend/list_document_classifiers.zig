...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDocumentClassifiersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.ListDocumentClassifiers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDocumentClassifiersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDocumentClassifiersOutput, body, alloc);
}
