...
const DocumentClassifierFilter = @import("document_classifier_filter.zig").DocumentClassifierFilter;
const DocumentClassifierProperties = @import("document_classifier_properties.zig").DocumentClassifierProperties;

pub const ListDocumentClassifiersInput = struct {
...
pub const ListDocumentClassifiersOutput = struct {
    /// A list containing the properties of each job returned.
    document_classifier_properties_list: ?[]const DocumentClassifierProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDocumentClassifiersInput, options: CallOptions) !ListDocumentClassifiersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDocumentClassifiersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.ListDocumentClassifiers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDocumentClassifiersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDocumentClassifiersOutput, body, allocator);
}
