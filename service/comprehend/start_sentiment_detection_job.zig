...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSentimentDetectionJobInput, options: Options) !StartSentimentDetectionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSentimentDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.StartSentimentDetectionJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSentimentDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartSentimentDetectionJobOutput, body, alloc);
}
