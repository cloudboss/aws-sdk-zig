...
pub const StartFlywheelIterationInput = struct {
...
pub const StartFlywheelIterationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFlywheelIterationInput, options: CallOptions) !StartFlywheelIterationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartFlywheelIterationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StartFlywheelIteration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartFlywheelIterationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartFlywheelIterationOutput, body, allocator);
}
