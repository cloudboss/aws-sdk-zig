...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectPiiEntitiesInput, options: Options) !DetectPiiEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectPiiEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.DetectPiiEntities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectPiiEntitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetectPiiEntitiesOutput, body, alloc);
}
