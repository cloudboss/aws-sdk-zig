...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ContainsPiiEntitiesInput, options: Options) !ContainsPiiEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ContainsPiiEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.ContainsPiiEntities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ContainsPiiEntitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ContainsPiiEntitiesOutput, body, alloc);
}
