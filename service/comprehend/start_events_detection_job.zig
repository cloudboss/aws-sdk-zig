...
pub const StartEventsDetectionJobInput = struct {
    ...
    /// that adds
    /// metadata to a resource used by Amazon Comprehend. For example, a tag with
    /// "Sales" as the key
...
pub const StartEventsDetectionJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartEventsDetectionJobInput, options: CallOptions) !StartEventsDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartEventsDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StartEventsDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartEventsDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartEventsDetectionJobOutput, body, allocator);
}
