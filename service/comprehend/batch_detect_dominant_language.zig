...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDetectDominantLanguageInput, options: Options) !BatchDetectDominantLanguageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDetectDominantLanguageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.BatchDetectDominantLanguage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDetectDominantLanguageOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDetectDominantLanguageOutput, body, alloc);
}
