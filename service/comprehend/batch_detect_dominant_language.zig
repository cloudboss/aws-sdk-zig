...
const BatchDetectDominantLanguageItemResult = @import("batch_detect_dominant_language_item_result.zig").BatchDetectDominantLanguageItemResult;

pub const BatchDetectDominantLanguageInput = struct {
    /// A list containing the UTF-8 encoded text of the input documents. The list
    /// can contain a maximum of 25
    /// documents. Each document should contain at least 20 characters. The maximum
...
pub const BatchDetectDominantLanguageOutput = struct {
    ...
    /// A list of objects
    /// containing the results of the operation. The results are sorted in ascending
    /// order by the
    /// `Index` field and match the order of the documents in the input list. If all
    /// of
    /// the documents contain an error, the `ResultList` is empty.
    result_list: ?[]const BatchDetectDominantLanguageItemResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDetectDominantLanguageInput, options: CallOptions) !BatchDetectDominantLanguageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDetectDominantLanguageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.BatchDetectDominantLanguage");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDetectDominantLanguageOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDetectDominantLanguageOutput, body, allocator);
}
