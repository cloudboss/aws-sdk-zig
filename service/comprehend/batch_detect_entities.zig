...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDetectEntitiesInput, options: Options) !BatchDetectEntitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDetectEntitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.BatchDetectEntities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDetectEntitiesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDetectEntitiesOutput, body, alloc);
}
