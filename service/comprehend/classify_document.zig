...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ClassifyDocumentInput, options: Options) !ClassifyDocumentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ClassifyDocumentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.ClassifyDocument");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ClassifyDocumentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ClassifyDocumentOutput, body, alloc);
}
