...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEntityRecognizerInput, options: Options) !CreateEntityRecognizerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEntityRecognizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.CreateEntityRecognizer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEntityRecognizerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEntityRecognizerOutput, body, alloc);
}
