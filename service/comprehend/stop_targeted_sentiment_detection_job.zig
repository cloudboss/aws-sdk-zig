...
pub const StopTargetedSentimentDetectionJobInput = struct {
...
pub const StopTargetedSentimentDetectionJobOutput = struct {
    /// The identifier of the targeted sentiment detection job to stop.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopTargetedSentimentDetectionJobInput, options: CallOptions) !StopTargetedSentimentDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopTargetedSentimentDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StopTargetedSentimentDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopTargetedSentimentDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopTargetedSentimentDetectionJobOutput, body, allocator);
}
