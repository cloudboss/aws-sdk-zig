...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDetectSyntaxInput, options: Options) !BatchDetectSyntaxOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDetectSyntaxInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.BatchDetectSyntax");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDetectSyntaxOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDetectSyntaxOutput, body, alloc);
}
