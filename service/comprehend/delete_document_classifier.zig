...
pub const DeleteDocumentClassifierInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDocumentClassifierInput, options: Options) !DeleteDocumentClassifierOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDocumentClassifierInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.DeleteDocumentClassifier");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDocumentClassifierOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
