...
const EndpointFilter = @import("endpoint_filter.zig").EndpointFilter;
...
pub const ListEndpointsInput = struct {
    ...
    /// name, model,
    /// status, or the date and time that they were created. You can only set one
    /// filter at a time.
...
pub const ListEndpointsOutput = struct {
    ...
    /// response to the
    /// request.
    endpoint_properties_list: ?[]const EndpointProperties = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.ListEndpoints");
