...
const SentimentDetectionJobFilter = @import("sentiment_detection_job_filter.zig").SentimentDetectionJobFilter;
const SentimentDetectionJobProperties = @import("sentiment_detection_job_properties.zig").SentimentDetectionJobProperties;

pub const ListSentimentDetectionJobsInput = struct {
...
pub const ListSentimentDetectionJobsOutput = struct {
    /// Identifies the next page of results to return.
    next_token: ?[]const u8 = null,

    /// A list containing the properties of each job that is returned.
    sentiment_detection_job_properties_list: ?[]const SentimentDetectionJobProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSentimentDetectionJobsInput, options: CallOptions) !ListSentimentDetectionJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSentimentDetectionJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.ListSentimentDetectionJobs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSentimentDetectionJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSentimentDetectionJobsOutput, body, allocator);
}
