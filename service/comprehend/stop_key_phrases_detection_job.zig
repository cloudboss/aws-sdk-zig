...
pub const StopKeyPhrasesDetectionJobInput = struct {
...
pub const StopKeyPhrasesDetectionJobOutput = struct {
    /// The identifier of the key phrases detection job to stop.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopKeyPhrasesDetectionJobInput, options: CallOptions) !StopKeyPhrasesDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopKeyPhrasesDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StopKeyPhrasesDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopKeyPhrasesDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopKeyPhrasesDetectionJobOutput, body, allocator);
}
