...
pub const StopSentimentDetectionJobInput = struct {
...
pub const StopSentimentDetectionJobOutput = struct {
    /// The identifier of the sentiment detection job to stop.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopSentimentDetectionJobInput, options: CallOptions) !StopSentimentDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopSentimentDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StopSentimentDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopSentimentDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopSentimentDetectionJobOutput, body, allocator);
}
