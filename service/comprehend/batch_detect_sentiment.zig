...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDetectSentimentInput, options: Options) !BatchDetectSentimentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDetectSentimentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.BatchDetectSentiment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDetectSentimentOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDetectSentimentOutput, body, alloc);
}
