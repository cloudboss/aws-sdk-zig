...
pub const StopEventsDetectionJobInput = struct {
...
pub const StopEventsDetectionJobOutput = struct {
    /// The identifier of the events detection job to stop.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopEventsDetectionJobInput, options: CallOptions) !StopEventsDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopEventsDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StopEventsDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopEventsDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopEventsDetectionJobOutput, body, allocator);
}
