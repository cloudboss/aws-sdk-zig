...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopEventsDetectionJobInput, options: Options) !StopEventsDetectionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopEventsDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Comprehend_20171127.StopEventsDetectionJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopEventsDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopEventsDetectionJobOutput, body, alloc);
}
