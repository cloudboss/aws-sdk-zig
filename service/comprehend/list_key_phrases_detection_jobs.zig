...
pub const ListKeyPhrasesDetectionJobsInput = struct {
...
pub const ListKeyPhrasesDetectionJobsOutput = struct {
    /// A list containing the properties of each job that is returned.
    key_phrases_detection_job_properties_list: ?[]const KeyPhrasesDetectionJobProperties = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListKeyPhrasesDetectionJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.ListKeyPhrasesDetectionJobs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListKeyPhrasesDetectionJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListKeyPhrasesDetectionJobsOutput, body, allocator);
}
