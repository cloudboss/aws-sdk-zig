...
pub const StartEntitiesDetectionJobInput = struct {
    ...
    /// recognizer to be used
    /// by the `StartEntitiesDetectionJob`. This ARN is optional and is only used
    /// for a
    ...
    /// entities recognition is
    /// used, this parameter is ignored and the language used for training the model
    /// is used
    /// instead.
    ...
    /// containing
    /// the resources you are using for your entity detection job. For more
...
pub const StartEntitiesDetectionJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartEntitiesDetectionJobInput, options: CallOptions) !StartEntitiesDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartEntitiesDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.StartEntitiesDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartEntitiesDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartEntitiesDetectionJobOutput, body, allocator);
}
