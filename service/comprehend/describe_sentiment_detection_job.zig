...
pub const DescribeSentimentDetectionJobInput = struct {
    /// The identifier that Amazon Comprehend generated for the job. The operation
    /// returns this identifier in its
    /// response.
    job_id: []const u8,
...
pub const DescribeSentimentDetectionJobOutput = struct {
    /// An object that contains the properties associated with a sentiment detection
    /// job.
    sentiment_detection_job_properties: ?SentimentDetectionJobProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSentimentDetectionJobInput, options: CallOptions) !DescribeSentimentDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSentimentDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Comprehend_20171127.DescribeSentimentDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSentimentDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSentimentDetectionJobOutput, body, allocator);
}
