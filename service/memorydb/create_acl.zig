...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateACLInput, options: Options) !CreateACLOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateACLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.CreateACL");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateACLOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateACLOutput, body, alloc);
}
