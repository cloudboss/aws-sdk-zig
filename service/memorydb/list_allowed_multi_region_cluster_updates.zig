...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAllowedMultiRegionClusterUpdatesInput, options: Options) !ListAllowedMultiRegionClusterUpdatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAllowedMultiRegionClusterUpdatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.ListAllowedMultiRegionClusterUpdates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAllowedMultiRegionClusterUpdatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAllowedMultiRegionClusterUpdatesOutput, body, alloc);
}
