...
const MultiRegionParameterGroup = @import("multi_region_parameter_group.zig").MultiRegionParameterGroup;

pub const DescribeMultiRegionParameterGroupsInput = struct {
...
pub const DescribeMultiRegionParameterGroupsOutput = struct {
    /// A list of multi-region parameter groups. Each element in the list contains
    /// detailed information about one parameter group.
    multi_region_parameter_groups: ?[]const MultiRegionParameterGroup = null,
    ...
    pub const json_field_names = .{
        .multi_region_parameter_groups = "MultiRegionParameterGroups",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMultiRegionParameterGroupsInput, options: CallOptions) !DescribeMultiRegionParameterGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMultiRegionParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonMemoryDB.DescribeMultiRegionParameterGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMultiRegionParameterGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMultiRegionParameterGroupsOutput, body, allocator);
}
