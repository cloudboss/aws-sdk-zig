...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMultiRegionParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.DescribeMultiRegionParameterGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMultiRegionParameterGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMultiRegionParameterGroupsOutput, body, alloc);
}
