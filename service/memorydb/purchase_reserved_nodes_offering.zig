...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurchaseReservedNodesOfferingInput, options: Options) !PurchaseReservedNodesOfferingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseReservedNodesOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.PurchaseReservedNodesOffering");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PurchaseReservedNodesOfferingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PurchaseReservedNodesOfferingOutput, body, alloc);
}
