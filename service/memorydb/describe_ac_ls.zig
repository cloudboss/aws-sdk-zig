...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeACLsInput, options: Options) !DescribeACLsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeACLsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.DescribeACLs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeACLsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeACLsOutput, body, alloc);
}
