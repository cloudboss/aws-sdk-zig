...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMultiRegionClustersInput, options: Options) !DescribeMultiRegionClustersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMultiRegionClustersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.DescribeMultiRegionClusters");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMultiRegionClustersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMultiRegionClustersOutput, body, alloc);
}
