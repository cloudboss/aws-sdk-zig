...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateClusterInput, options: Options) !BatchUpdateClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.BatchUpdateCluster");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateClusterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchUpdateClusterOutput, body, alloc);
}
