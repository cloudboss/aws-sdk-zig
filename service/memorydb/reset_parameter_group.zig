...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetParameterGroupInput, options: Options) !ResetParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.ResetParameterGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetParameterGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResetParameterGroupOutput, body, alloc);
}
