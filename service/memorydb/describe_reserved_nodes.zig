...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReservedNodesInput, options: Options) !DescribeReservedNodesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.DescribeReservedNodes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReservedNodesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReservedNodesOutput, body, alloc);
}
