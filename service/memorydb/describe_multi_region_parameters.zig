...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMultiRegionParametersInput, options: Options) !DescribeMultiRegionParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMultiRegionParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.DescribeMultiRegionParameters");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMultiRegionParametersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMultiRegionParametersOutput, body, alloc);
}
