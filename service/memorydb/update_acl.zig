...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateACLInput, options: Options) !UpdateACLOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateACLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.UpdateACL");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateACLOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateACLOutput, body, alloc);
}
