...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAllowedNodeTypeUpdatesInput, options: Options) !ListAllowedNodeTypeUpdatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAllowedNodeTypeUpdatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.ListAllowedNodeTypeUpdates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAllowedNodeTypeUpdatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAllowedNodeTypeUpdatesOutput, body, alloc);
}
