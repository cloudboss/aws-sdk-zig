...
const EngineVersionInfo = @import("engine_version_info.zig").EngineVersionInfo;
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEngineVersionsInput, options: CallOptions) !DescribeEngineVersionsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "memorydb");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEngineVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonMemoryDB.DescribeEngineVersions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEngineVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEngineVersionsOutput, body, allocator);
}
