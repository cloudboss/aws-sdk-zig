...
pub const DeleteMultiRegionClusterInput = struct {
...
pub const DeleteMultiRegionClusterOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMultiRegionClusterInput, options: CallOptions) !DeleteMultiRegionClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMultiRegionClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonMemoryDB.DeleteMultiRegionCluster");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMultiRegionClusterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteMultiRegionClusterOutput, body, allocator);
}
