...
const ReservedNodesOffering = @import("reserved_nodes_offering.zig").ReservedNodesOffering;

pub const DescribeReservedNodesOfferingsInput = struct {
...
pub const DescribeReservedNodesOfferingsOutput = struct {
    /// An optional marker returned from a prior request. Use this marker for
    /// pagination of results from this operation. If this parameter is specified,
    /// the response includes only records beyond the marker, up to the value
    /// specified by MaxRecords.
    ...
    reserved_nodes_offerings: ?[]const ReservedNodesOffering = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReservedNodesOfferingsInput, options: CallOptions) !DescribeReservedNodesOfferingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReservedNodesOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonMemoryDB.DescribeReservedNodesOfferings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReservedNodesOfferingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReservedNodesOfferingsOutput, body, allocator);
}
