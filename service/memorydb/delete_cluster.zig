...
pub const DeleteClusterInput = struct {
    ...
    /// that identifies the snapshot. MemoryDB creates the snapshot, and then
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.DeleteCluster");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteClusterOutput {
    ...
    return aws.json.parseJsonObject(DeleteClusterOutput, body, alloc);
}
