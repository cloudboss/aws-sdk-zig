...
pub const DeleteACLInput = struct {
...
pub const DeleteACLOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteACLInput, options: CallOptions) !DeleteACLOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteACLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonMemoryDB.DeleteACL");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteACLOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteACLOutput, body, allocator);
}
