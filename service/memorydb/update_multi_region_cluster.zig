...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMultiRegionClusterInput, options: Options) !UpdateMultiRegionClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMultiRegionClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonMemoryDB.UpdateMultiRegionCluster");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMultiRegionClusterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateMultiRegionClusterOutput, body, alloc);
}
