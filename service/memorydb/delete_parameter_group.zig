...
pub const DeleteParameterGroupInput = struct {
...
pub const DeleteParameterGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteParameterGroupInput, options: CallOptions) !DeleteParameterGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonMemoryDB.DeleteParameterGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteParameterGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteParameterGroupOutput, body, allocator);
}
