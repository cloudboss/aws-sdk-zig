...
pub const FailoverShardInput = struct {
...
pub const FailoverShardOutput = struct {
    /// The cluster being failed over.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: FailoverShardInput, options: CallOptions) !FailoverShardOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: FailoverShardInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonMemoryDB.FailoverShard");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !FailoverShardOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(FailoverShardOutput, body, allocator);
}
