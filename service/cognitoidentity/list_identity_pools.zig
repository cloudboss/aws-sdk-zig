...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIdentityPoolsInput, options: Options) !ListIdentityPoolsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdentityPoolsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.ListIdentityPools");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdentityPoolsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListIdentityPoolsOutput, body, alloc);
}
