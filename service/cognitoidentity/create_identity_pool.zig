...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIdentityPoolInput, options: Options) !CreateIdentityPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIdentityPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.CreateIdentityPool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIdentityPoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateIdentityPoolOutput, body, alloc);
}
