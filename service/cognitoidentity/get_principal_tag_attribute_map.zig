...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPrincipalTagAttributeMapInput, options: Options) !GetPrincipalTagAttributeMapOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPrincipalTagAttributeMapInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.GetPrincipalTagAttributeMap");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPrincipalTagAttributeMapOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPrincipalTagAttributeMapOutput, body, alloc);
}
