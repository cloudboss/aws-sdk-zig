...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIdentityPoolInput, options: Options) !DescribeIdentityPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIdentityPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.DescribeIdentityPool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIdentityPoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeIdentityPoolOutput, body, alloc);
}
