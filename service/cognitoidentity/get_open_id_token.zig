...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOpenIdTokenInput, options: Options) !GetOpenIdTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOpenIdTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.GetOpenIdToken");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOpenIdTokenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOpenIdTokenOutput, body, alloc);
}
