...
const UnlinkDeveloperIdentityOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UnlinkDeveloperIdentityInput, options: Options) !UnlinkDeveloperIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UnlinkDeveloperIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.UnlinkDeveloperIdentity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UnlinkDeveloperIdentityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
