...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIdentityPoolInput, options: Options) !UpdateIdentityPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIdentityPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.UpdateIdentityPool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIdentityPoolOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateIdentityPoolOutput, body, alloc);
}
