...
const UnlinkIdentityOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UnlinkIdentityInput, options: Options) !UnlinkIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UnlinkIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.UnlinkIdentity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UnlinkIdentityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
