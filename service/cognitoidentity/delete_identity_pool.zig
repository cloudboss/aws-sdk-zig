...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteIdentityPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.DeleteIdentityPool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteIdentityPoolOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
