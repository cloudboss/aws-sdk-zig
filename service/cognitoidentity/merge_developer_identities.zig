...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MergeDeveloperIdentitiesInput, options: Options) !MergeDeveloperIdentitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: MergeDeveloperIdentitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.MergeDeveloperIdentities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MergeDeveloperIdentitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(MergeDeveloperIdentitiesOutput, body, alloc);
}
