...
pub const DescribeIdentityInput = struct {
...
pub const DescribeIdentityOutput = @import("identity_description.zig").IdentityDescription;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIdentityInput, options: CallOptions) !DescribeIdentityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSCognitoIdentityService.DescribeIdentity");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeIdentityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeIdentityOutput, body, allocator);
}
