...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteIdentitiesInput, options: Options) !DeleteIdentitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteIdentitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.DeleteIdentities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteIdentitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteIdentitiesOutput, body, alloc);
}
