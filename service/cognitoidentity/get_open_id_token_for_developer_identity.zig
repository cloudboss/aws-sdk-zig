...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOpenIdTokenForDeveloperIdentityInput, options: Options) !GetOpenIdTokenForDeveloperIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOpenIdTokenForDeveloperIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.GetOpenIdTokenForDeveloperIdentity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOpenIdTokenForDeveloperIdentityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOpenIdTokenForDeveloperIdentityOutput, body, alloc);
}
