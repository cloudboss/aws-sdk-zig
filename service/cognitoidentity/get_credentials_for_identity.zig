...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCredentialsForIdentityInput, options: Options) !GetCredentialsForIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCredentialsForIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.GetCredentialsForIdentity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCredentialsForIdentityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCredentialsForIdentityOutput, body, alloc);
}
