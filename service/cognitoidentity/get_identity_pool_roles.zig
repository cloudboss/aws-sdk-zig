...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentityPoolRolesInput, options: Options) !GetIdentityPoolRolesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityPoolRolesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.GetIdentityPoolRoles");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityPoolRolesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetIdentityPoolRolesOutput, body, alloc);
}
