...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetPrincipalTagAttributeMapInput, options: Options) !SetPrincipalTagAttributeMapOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetPrincipalTagAttributeMapInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSCognitoIdentityService.SetPrincipalTagAttributeMap");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetPrincipalTagAttributeMapOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SetPrincipalTagAttributeMapOutput, body, alloc);
}
