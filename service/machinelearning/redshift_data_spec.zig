const RedshiftDatabaseCredentials = @import("redshift_database_credentials.zig").RedshiftDatabaseCredentials;
const RedshiftDatabase = @import("redshift_database.zig").RedshiftDatabase;

/// Describes the data specification of an Amazon Redshift `DataSource`.
pub const RedshiftDataSpec = struct {
    ...
    /// is `sequential`, meaning that Amazon ML takes all of the data records
    /// between the `percentBegin`
    /// and `percentEnd` parameters for the datasource, in the order that the
    /// records appear in the input data.
    ///
    ...
    /// Amazon ML assigns each row of data a pseudo-random number between 0 and 100,
    /// and then selects the rows that have an assigned
    /// number between `percentBegin` and `percentEnd`. Pseudo-random numbers are
    /// assigned
    /// using both the input seed string value and the byte offset as a seed, so
    /// changing the data results in a
    /// different split. Any existing ordering is preserved.
    /// The random splitting strategy ensures that variables in the training and
    /// evaluation data are distributed similarly.
    /// It is useful in the cases where the input data may have an implicit sort
    /// order, which would otherwise result in
