...
const MLModelFilterVariable = @import("ml_model_filter_variable.zig").MLModelFilterVariable;
const SortOrder = @import("sort_order.zig").SortOrder;
const MLModel = @import("ml_model.zig").MLModel;

pub const DescribeMLModelsInput = struct {
    ...
    ///   training a `MLModel`. The URL can identify either a file or an Amazon
    ///   Simple Storage Service (Amazon S3) bucket or directory.
    filter_variable: ?MLModelFilterVariable = null,
    ...
    /// this `MLModel`, select `Name` for the `FilterVariable` and any of the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMLModelsInput, options: CallOptions) !DescribeMLModelsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMLModelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonML_20141212.DescribeMLModels");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMLModelsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMLModelsOutput, body, allocator);
}
