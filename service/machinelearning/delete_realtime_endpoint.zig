...
pub const DeleteRealtimeEndpointInput = struct {
...
pub const DeleteRealtimeEndpointOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRealtimeEndpointInput, options: CallOptions) !DeleteRealtimeEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRealtimeEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonML_20141212.DeleteRealtimeEndpoint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRealtimeEndpointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRealtimeEndpointOutput, body, allocator);
}
