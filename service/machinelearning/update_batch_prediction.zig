...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBatchPredictionInput, options: Options) !UpdateBatchPredictionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBatchPredictionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonML_20141212.UpdateBatchPrediction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBatchPredictionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateBatchPredictionOutput, body, alloc);
}
