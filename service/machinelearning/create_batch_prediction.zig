...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBatchPredictionInput, options: Options) !CreateBatchPredictionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBatchPredictionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonML_20141212.CreateBatchPrediction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBatchPredictionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateBatchPredictionOutput, body, alloc);
}
