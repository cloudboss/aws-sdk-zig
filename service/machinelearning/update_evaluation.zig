...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEvaluationInput, options: Options) !UpdateEvaluationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEvaluationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonML_20141212.UpdateEvaluation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEvaluationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateEvaluationOutput, body, alloc);
}
