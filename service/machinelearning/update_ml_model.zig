...
pub const UpdateMLModelInput = struct {
    ...
    /// the `ScoreThreshold` receive a negative response from the `MLModel`, such as
    /// `false`.
...
pub const UpdateMLModelOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMLModelInput, options: CallOptions) !UpdateMLModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateMLModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonML_20141212.UpdateMLModel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateMLModelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateMLModelOutput, body, allocator);
}
