...
pub const DeleteMLModelInput = struct {
...
pub const DeleteMLModelOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMLModelInput, options: CallOptions) !DeleteMLModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMLModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonML_20141212.DeleteMLModel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMLModelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteMLModelOutput, body, allocator);
}
