...
pub const DeleteEvaluationInput = struct {
...
pub const DeleteEvaluationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEvaluationInput, options: CallOptions) !DeleteEvaluationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteEvaluationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonML_20141212.DeleteEvaluation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteEvaluationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteEvaluationOutput, body, allocator);
}
