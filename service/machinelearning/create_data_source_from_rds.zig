...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataSourceFromRDSInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonML_20141212.CreateDataSourceFromRDS");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataSourceFromRDSOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDataSourceFromRDSOutput, body, alloc);
}
