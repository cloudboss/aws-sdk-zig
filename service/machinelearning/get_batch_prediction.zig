...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBatchPredictionInput, options: Options) !GetBatchPredictionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBatchPredictionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonML_20141212.GetBatchPrediction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBatchPredictionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetBatchPredictionOutput, body, alloc);
}
