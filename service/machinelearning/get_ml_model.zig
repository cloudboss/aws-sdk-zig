...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMLModelInput, options: CallOptions) !GetMLModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMLModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonML_20141212.GetMLModel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMLModelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMLModelOutput, body, allocator);
}
