...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMLModelInput, options: CallOptions) !CreateMLModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateMLModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonML_20141212.CreateMLModel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateMLModelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateMLModelOutput, body, allocator);
}
