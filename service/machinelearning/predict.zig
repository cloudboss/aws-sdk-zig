...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PredictInput, options: Options) !PredictOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PredictInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonML_20141212.Predict");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PredictOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PredictOutput, body, alloc);
}
