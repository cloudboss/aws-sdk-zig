...
const DataSourceFilterVariable = @import("data_source_filter_variable.zig").DataSourceFilterVariable;
const SortOrder = @import("sort_order.zig").SortOrder;
const DataSource = @import("data_source.zig").DataSource;

pub const DescribeDataSourcesInput = struct {
    ...
    ///   `DataSource` creation.
    filter_variable: ?DataSourceFilterVariable = null,
    ...
    /// this `DataSource`, select `Name` for the `FilterVariable` and any of the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataSourcesInput, options: CallOptions) !DescribeDataSourcesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDataSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonML_20141212.DescribeDataSources");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDataSourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDataSourcesOutput, body, allocator);
}
