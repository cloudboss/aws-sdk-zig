...
pub const CreateRealtimeEndpointInput = struct {
...
pub const CreateRealtimeEndpointOutput = struct {
    ...
    realtime_endpoint_info: ?RealtimeEndpointInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRealtimeEndpointInput, options: CallOptions) !CreateRealtimeEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRealtimeEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonML_20141212.CreateRealtimeEndpoint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateRealtimeEndpointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateRealtimeEndpointOutput, body, allocator);
}
