...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataSourceFromRedshiftInput, options: Options) !CreateDataSourceFromRedshiftOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataSourceFromRedshiftInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonML_20141212.CreateDataSourceFromRedshift");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataSourceFromRedshiftOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDataSourceFromRedshiftOutput, body, alloc);
}
