...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UntagInput, options: CallOptions) !UntagOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UntagInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UntagOutput {
    var result: UntagOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UntagOutput, body, allocator);
    }
    _ = status;
    _ = headers;
