...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTagSyncTaskInput, options: Options) !GetTagSyncTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTagSyncTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"TaskArn\":");
    try aws.json.writeValue(@TypeOf(input.task_arn), input.task_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTagSyncTaskOutput {
    var result: GetTagSyncTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTagSyncTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
