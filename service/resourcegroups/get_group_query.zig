...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGroupQueryInput, options: Options) !GetGroupQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGroupQueryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGroupQueryOutput {
    var result: GetGroupQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGroupQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
