...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGroupingStatusesInput, options: Options) !ListGroupingStatusesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupingStatusesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupingStatusesOutput {
    var result: ListGroupingStatusesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGroupingStatusesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
