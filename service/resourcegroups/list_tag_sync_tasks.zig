...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTagSyncTasksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTagSyncTasksOutput {
    var result: ListTagSyncTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTagSyncTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
