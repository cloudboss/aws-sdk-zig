...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GroupResourcesInput, options: Options) !GroupResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GroupResourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GroupResourcesOutput {
    var result: GroupResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GroupResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
