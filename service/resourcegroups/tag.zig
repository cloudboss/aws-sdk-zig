...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TagInput, options: CallOptions) !TagOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TagInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TagOutput {
    var result: TagOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TagOutput, body, allocator);
    }
    _ = status;
    _ = headers;
