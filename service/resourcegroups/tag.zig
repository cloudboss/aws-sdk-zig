...
fn serializeRequest(alloc: std.mem.Allocator, input: TagInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TagOutput {
    var result: TagOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TagOutput, body, alloc);
    }
    _ = status;
    _ = headers;
