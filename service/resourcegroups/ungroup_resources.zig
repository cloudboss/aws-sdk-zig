...
fn serializeRequest(alloc: std.mem.Allocator, input: UngroupResourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UngroupResourcesOutput {
    var result: UngroupResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UngroupResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
