...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGroupConfigurationInput, options: Options) !GetGroupConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGroupConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGroupConfigurationOutput {
    var result: GetGroupConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGroupConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
