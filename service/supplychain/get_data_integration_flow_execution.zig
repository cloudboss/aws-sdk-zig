...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataIntegrationFlowExecutionInput, options: Options) !GetDataIntegrationFlowExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataIntegrationFlowExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.flow_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataIntegrationFlowExecutionOutput {
    var result: GetDataIntegrationFlowExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataIntegrationFlowExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
