...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDataLakeDatasetInput, options: Options) !DeleteDataLakeDatasetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDataLakeDatasetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDataLakeDatasetOutput {
    var result: DeleteDataLakeDatasetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDataLakeDatasetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
