...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataLakeDatasetInput, options: CallOptions) !GetDataLakeDatasetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDataLakeDatasetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDataLakeDatasetOutput {
    var result: GetDataLakeDatasetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataLakeDatasetOutput, body, allocator);
    }
    _ = status;
    _ = headers;
