...
pub const GetDataLakeDatasetInput = struct {
...
pub const GetDataLakeDatasetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataLakeDatasetInput, options: Options) !GetDataLakeDatasetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataLakeDatasetInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/api/datalake/instance/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataLakeDatasetOutput {
    var result: GetDataLakeDatasetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataLakeDatasetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
