...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDataLakeNamespaceInput, options: Options) !UpdateDataLakeNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataLakeNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataLakeNamespaceOutput {
    var result: UpdateDataLakeNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDataLakeNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
