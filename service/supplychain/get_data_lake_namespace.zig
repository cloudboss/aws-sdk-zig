...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataLakeNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataLakeNamespaceOutput {
    var result: GetDataLakeNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataLakeNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
