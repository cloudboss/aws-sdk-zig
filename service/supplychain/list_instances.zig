...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.instance_name_filter) |v| {
        ...
        try query_buf.appendSlice(alloc, "instanceNameFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.instance_state_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "instanceStateFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstancesOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInstancesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
