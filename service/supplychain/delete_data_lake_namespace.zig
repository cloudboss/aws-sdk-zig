...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDataLakeNamespaceInput, options: Options) !DeleteDataLakeNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDataLakeNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDataLakeNamespaceOutput {
    var result: DeleteDataLakeNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDataLakeNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
