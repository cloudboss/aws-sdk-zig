...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataIntegrationEventInput, options: Options) !GetDataIntegrationEventOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataIntegrationEventInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataIntegrationEventOutput {
    var result: GetDataIntegrationEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataIntegrationEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
