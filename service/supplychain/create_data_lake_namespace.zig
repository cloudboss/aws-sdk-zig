...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataLakeNamespaceInput, options: Options) !CreateDataLakeNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataLakeNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataLakeNamespaceOutput {
    var result: CreateDataLakeNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataLakeNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
