...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDataLakeDatasetInput, options: Options) !UpdateDataLakeDatasetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataLakeDatasetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataLakeDatasetOutput {
    var result: UpdateDataLakeDatasetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDataLakeDatasetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
