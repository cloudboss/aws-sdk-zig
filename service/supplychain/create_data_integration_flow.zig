...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataIntegrationFlowInput, options: Options) !CreateDataIntegrationFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataIntegrationFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.transformation), input.transformation, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataIntegrationFlowOutput {
    var result: CreateDataIntegrationFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataIntegrationFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
