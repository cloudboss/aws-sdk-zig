...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataLakeDatasetsInput, options: Options) !ListDataLakeDatasetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataLakeDatasetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataLakeDatasetsOutput {
    var result: ListDataLakeDatasetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataLakeDatasetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
