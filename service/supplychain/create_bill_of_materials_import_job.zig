...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBillOfMaterialsImportJobInput, options: Options) !CreateBillOfMaterialsImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBillOfMaterialsImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/api/configuration/instances/");
    try path_buf.appendSlice(alloc, input.instance_id);
    try path_buf.appendSlice(alloc, "/bill-of-materials-import-jobs");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"s3uri\":");
    try aws.json.writeValue(@TypeOf(input.s_3_uri), input.s_3_uri, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBillOfMaterialsImportJobOutput {
    var result: CreateBillOfMaterialsImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBillOfMaterialsImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
