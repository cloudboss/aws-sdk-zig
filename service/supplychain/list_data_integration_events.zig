...
pub const ListDataIntegrationEventsInput = struct {
    /// List data integration events for the specified eventType.
    event_type: ?DataIntegrationEventType = null,
...
pub const ListDataIntegrationEventsOutput = struct {
    /// The list of data integration events.
    events: ?[]const DataIntegrationEvent = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataIntegrationEventsInput, options: Options) !ListDataIntegrationEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataIntegrationEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataIntegrationEventsOutput {
    var result: ListDataIntegrationEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataIntegrationEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
