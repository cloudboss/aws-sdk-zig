...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataLakeDatasetInput, options: Options) !CreateDataLakeDatasetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataLakeDatasetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.partition_spec) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"partitionSpec\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.schema) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataLakeDatasetOutput {
    var result: CreateDataLakeDatasetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataLakeDatasetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
