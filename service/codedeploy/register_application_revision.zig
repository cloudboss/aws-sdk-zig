...
const RegisterApplicationRevisionOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterApplicationRevisionInput, options: Options) !RegisterApplicationRevisionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterApplicationRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.RegisterApplicationRevision");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterApplicationRevisionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
