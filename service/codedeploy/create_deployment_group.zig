...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDeploymentGroupInput, options: Options) !CreateDeploymentGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeploymentGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.CreateDeploymentGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDeploymentGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDeploymentGroupOutput, body, alloc);
}
