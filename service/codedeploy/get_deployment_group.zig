...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeploymentGroupInput, options: Options) !GetDeploymentGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeploymentGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.GetDeploymentGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeploymentGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeploymentGroupOutput, body, alloc);
}
