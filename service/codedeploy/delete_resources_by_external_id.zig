...
pub const DeleteResourcesByExternalIdInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResourcesByExternalIdInput, options: Options) !DeleteResourcesByExternalIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteResourcesByExternalIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.DeleteResourcesByExternalId");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteResourcesByExternalIdOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
