...
pub const BatchGetOnPremisesInstancesInput = struct {
...
pub const BatchGetOnPremisesInstancesOutput = struct {
    /// Information about the on-premises instances.
    instance_infos: ?[]const InstanceInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetOnPremisesInstancesInput, options: CallOptions) !BatchGetOnPremisesInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchGetOnPremisesInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeDeploy_20141006.BatchGetOnPremisesInstances");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchGetOnPremisesInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetOnPremisesInstancesOutput, body, allocator);
}
