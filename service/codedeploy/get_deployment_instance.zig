...
pub const GetDeploymentInstanceInput = struct {
...
pub const GetDeploymentInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeploymentInstanceInput, options: CallOptions) !GetDeploymentInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDeploymentInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeDeploy_20141006.GetDeploymentInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDeploymentInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeploymentInstanceOutput, body, allocator);
}
