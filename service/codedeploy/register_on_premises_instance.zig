...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterOnPremisesInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.RegisterOnPremisesInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterOnPremisesInstanceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
