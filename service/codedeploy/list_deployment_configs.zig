...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeploymentConfigsInput, options: Options) !ListDeploymentConfigsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDeploymentConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.ListDeploymentConfigs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDeploymentConfigsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDeploymentConfigsOutput, body, alloc);
}
