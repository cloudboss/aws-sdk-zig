...
pub const ListApplicationsOutput = struct {
    ...
    /// returned. It can
    /// be used in a subsequent list applications call to return the next set of
    /// applications in
    /// the list.
    next_token: ?[]const u8 = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeDeploy_20141006.ListApplications");
