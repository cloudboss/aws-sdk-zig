...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApplicationRevisionsInput, options: Options) !ListApplicationRevisionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApplicationRevisionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.ListApplicationRevisions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApplicationRevisionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListApplicationRevisionsOutput, body, alloc);
}
