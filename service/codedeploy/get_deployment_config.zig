...
const DeploymentConfigInfo = @import("deployment_config_info.zig").DeploymentConfigInfo;

pub const GetDeploymentConfigInput = struct {
...
pub const GetDeploymentConfigOutput = struct {
    /// Information about the deployment configuration.
    deployment_config_info: ?DeploymentConfigInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeploymentConfigInput, options: CallOptions) !GetDeploymentConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDeploymentConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeDeploy_20141006.GetDeploymentConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDeploymentConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeploymentConfigOutput, body, allocator);
}
