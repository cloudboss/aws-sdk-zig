...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeploymentConfigInput, options: Options) !GetDeploymentConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeploymentConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.GetDeploymentConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeploymentConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeploymentConfigOutput, body, alloc);
}
