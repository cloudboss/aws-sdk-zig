...
pub const GetDeploymentTargetInput = struct {
...
pub const GetDeploymentTargetOutput = struct {
    /// A deployment target that contains information about a deployment such as its
    /// status,
    /// lifecycle events, and when it was last updated. It also contains metadata
    /// about the
    /// deployment target. The deployment target metadata depends on the deployment
    /// target's
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeploymentTargetInput, options: CallOptions) !GetDeploymentTargetOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "codedeploy");
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDeploymentTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeDeploy_20141006.GetDeploymentTarget");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDeploymentTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeploymentTargetOutput, body, allocator);
}
