...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeploymentTargetInput, options: Options) !GetDeploymentTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeploymentTargetInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("codedeploy", "CodeDeploy", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.GetDeploymentTarget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeploymentTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeploymentTargetOutput, body, alloc);
}
