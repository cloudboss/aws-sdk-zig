...
pub const ListDeploymentGroupsInput = struct {
...
pub const ListDeploymentGroupsOutput = struct {
    ...
    /// returned. It can
    /// be used in a subsequent list deployment groups call to return the next set
    /// of deployment
    /// groups in the list.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeploymentGroupsInput, options: CallOptions) !ListDeploymentGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDeploymentGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeDeploy_20141006.ListDeploymentGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDeploymentGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDeploymentGroupsOutput, body, allocator);
}
