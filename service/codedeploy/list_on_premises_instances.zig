...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOnPremisesInstancesInput, options: Options) !ListOnPremisesInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOnPremisesInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.ListOnPremisesInstances");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOnPremisesInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOnPremisesInstancesOutput, body, alloc);
}
