...
pub const GetApplicationRevisionInput = struct {
...
pub const GetApplicationRevisionOutput = struct {
    /// The name of the application that corresponds to the revision.
    ...
    revision_info: ?GenericRevisionInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApplicationRevisionInput, options: CallOptions) !GetApplicationRevisionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetApplicationRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeDeploy_20141006.GetApplicationRevision");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetApplicationRevisionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetApplicationRevisionOutput, body, allocator);
}
