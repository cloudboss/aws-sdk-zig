...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApplicationRevisionInput, options: Options) !GetApplicationRevisionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApplicationRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeDeploy_20141006.GetApplicationRevision");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApplicationRevisionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetApplicationRevisionOutput, body, alloc);
}
