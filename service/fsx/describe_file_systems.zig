...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFileSystemsInput, options: Options) !DescribeFileSystemsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFileSystemsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSimbaAPIService_v20180301.DescribeFileSystems");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFileSystemsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFileSystemsOutput, body, alloc);
}
