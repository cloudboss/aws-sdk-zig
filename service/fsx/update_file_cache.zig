...
const UpdateFileCacheLustreConfiguration = @import("update_file_cache_lustre_configuration.zig").UpdateFileCacheLustreConfiguration;
const FileCache = @import("file_cache.zig").FileCache;

pub const UpdateFileCacheInput = struct {
    ...
    lustre_configuration: ?UpdateFileCacheLustreConfiguration = null,
...
pub const UpdateFileCacheOutput = struct {
    /// A description of the cache that was updated.
    file_cache: ?FileCache = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFileCacheInput, options: CallOptions) !UpdateFileCacheOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFileCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSimbaAPIService_v20180301.UpdateFileCache");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFileCacheOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFileCacheOutput, body, allocator);
}
