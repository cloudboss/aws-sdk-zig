...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDataRepositoryAssociationInput, options: Options) !UpdateDataRepositoryAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataRepositoryAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSimbaAPIService_v20180301.UpdateDataRepositoryAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataRepositoryAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateDataRepositoryAssociationOutput, body, alloc);
}
