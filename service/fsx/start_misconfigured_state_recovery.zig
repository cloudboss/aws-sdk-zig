...
pub const StartMisconfiguredStateRecoveryInput = struct {
...
pub const StartMisconfiguredStateRecoveryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMisconfiguredStateRecoveryInput, options: CallOptions) !StartMisconfiguredStateRecoveryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartMisconfiguredStateRecoveryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSimbaAPIService_v20180301.StartMisconfiguredStateRecovery");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartMisconfiguredStateRecoveryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMisconfiguredStateRecoveryOutput, body, allocator);
}
