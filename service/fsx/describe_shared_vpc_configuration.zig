...
pub const DescribeSharedVpcConfigurationOutput = struct {
    ...
    enable_fsx_route_table_updates_from_participant_accounts: ?[]const u8 = null,

    pub const json_field_names = .{
        .enable_fsx_route_table_updates_from_participant_accounts = "EnableFsxRouteTableUpdatesFromParticipantAccounts",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSharedVpcConfigurationInput, options: CallOptions) !DescribeSharedVpcConfigurationOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "fsx");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSharedVpcConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSimbaAPIService_v20180301.DescribeSharedVpcConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSharedVpcConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSharedVpcConfigurationOutput, body, allocator);
}
