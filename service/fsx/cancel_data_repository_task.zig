...
pub const CancelDataRepositoryTaskInput = struct {
...
pub const CancelDataRepositoryTaskOutput = struct {
    ...
    ///   there may be files the task failed to process.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelDataRepositoryTaskInput, options: CallOptions) !CancelDataRepositoryTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelDataRepositoryTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSimbaAPIService_v20180301.CancelDataRepositoryTask");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelDataRepositoryTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelDataRepositoryTaskOutput, body, allocator);
}
