...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAndAttachS3AccessPointInput, options: Options) !CreateAndAttachS3AccessPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAndAttachS3AccessPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSimbaAPIService_v20180301.CreateAndAttachS3AccessPoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAndAttachS3AccessPointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAndAttachS3AccessPointOutput, body, alloc);
}
