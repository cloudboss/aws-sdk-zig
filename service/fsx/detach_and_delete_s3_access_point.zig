...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachAndDeleteS3AccessPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSimbaAPIService_v20180301.DetachAndDeleteS3AccessPoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachAndDeleteS3AccessPointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetachAndDeleteS3AccessPointOutput, body, alloc);
}
