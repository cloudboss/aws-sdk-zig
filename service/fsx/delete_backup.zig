...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBackupInput, options: CallOptions) !DeleteBackupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBackupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSimbaAPIService_v20180301.DeleteBackup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBackupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteBackupOutput, body, allocator);
}
