...
pub const ReleaseFileSystemNfsV3LocksInput = struct {
...
pub const ReleaseFileSystemNfsV3LocksOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReleaseFileSystemNfsV3LocksInput, options: CallOptions) !ReleaseFileSystemNfsV3LocksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ReleaseFileSystemNfsV3LocksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSimbaAPIService_v20180301.ReleaseFileSystemNfsV3Locks");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ReleaseFileSystemNfsV3LocksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ReleaseFileSystemNfsV3LocksOutput, body, allocator);
}
