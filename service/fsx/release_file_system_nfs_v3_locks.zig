...
fn serializeRequest(alloc: std.mem.Allocator, input: ReleaseFileSystemNfsV3LocksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSimbaAPIService_v20180301.ReleaseFileSystemNfsV3Locks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReleaseFileSystemNfsV3LocksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ReleaseFileSystemNfsV3LocksOutput, body, alloc);
}
