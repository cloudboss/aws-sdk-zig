...
pub const DeleteSnapshotInput = struct {
...
pub const DeleteSnapshotOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSnapshotInput, options: CallOptions) !DeleteSnapshotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSimbaAPIService_v20180301.DeleteSnapshot");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteSnapshotOutput, body, allocator);
}
