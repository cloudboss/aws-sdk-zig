...
const VolumeFilter = @import("volume_filter.zig").VolumeFilter;
...
pub const DescribeVolumesInput = struct {
    ...
    /// volumes.
    filters: ?[]const VolumeFilter = null,
...
pub const DescribeVolumesOutput = struct {
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVolumesInput, options: CallOptions) !DescribeVolumesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVolumesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSimbaAPIService_v20180301.DescribeVolumes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVolumesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeVolumesOutput, body, allocator);
}
