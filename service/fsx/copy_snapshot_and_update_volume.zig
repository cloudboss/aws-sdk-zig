...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopySnapshotAndUpdateVolumeInput, options: Options) !CopySnapshotAndUpdateVolumeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopySnapshotAndUpdateVolumeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSimbaAPIService_v20180301.CopySnapshotAndUpdateVolume");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopySnapshotAndUpdateVolumeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CopySnapshotAndUpdateVolumeOutput, body, alloc);
}
