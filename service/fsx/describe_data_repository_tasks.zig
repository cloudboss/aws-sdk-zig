...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataRepositoryTasksInput, options: Options) !DescribeDataRepositoryTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataRepositoryTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSimbaAPIService_v20180301.DescribeDataRepositoryTasks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataRepositoryTasksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDataRepositoryTasksOutput, body, alloc);
}
