...
pub const DeleteFileCacheInput = struct {
...
pub const DeleteFileCacheOutput = struct {
    /// The ID of the cache that's being deleted.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteFileCacheInput, options: CallOptions) !DeleteFileCacheOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteFileCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSSimbaAPIService_v20180301.DeleteFileCache");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteFileCacheOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteFileCacheOutput, body, allocator);
}
