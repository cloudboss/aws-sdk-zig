...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStorageVirtualMachineInput, options: Options) !UpdateStorageVirtualMachineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStorageVirtualMachineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSimbaAPIService_v20180301.UpdateStorageVirtualMachine");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStorageVirtualMachineOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateStorageVirtualMachineOutput, body, alloc);
}
