...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreVolumeFromSnapshotInput, options: Options) !RestoreVolumeFromSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreVolumeFromSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSSimbaAPIService_v20180301.RestoreVolumeFromSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreVolumeFromSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RestoreVolumeFromSnapshotOutput, body, alloc);
}
