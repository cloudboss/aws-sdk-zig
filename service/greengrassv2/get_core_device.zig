...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCoreDeviceInput, options: Options) !GetCoreDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCoreDeviceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCoreDeviceOutput {
    var result: GetCoreDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCoreDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
