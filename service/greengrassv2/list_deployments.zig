...
const DeploymentHistoryFilter = @import("deployment_history_filter.zig").DeploymentHistoryFilter;
...
pub const ListDeploymentsInput = struct {
    ...
    ///
    /// Default: `LATEST_ONLY`
    history_filter: ?DeploymentHistoryFilter = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeploymentsInput, options: Options) !ListDeploymentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDeploymentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.history_filter) |v| {
        ...
        try query_buf.appendSlice(alloc, "historyFilter=");
    ...
    if (input.parent_target_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "parentTargetArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.target_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "targetArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDeploymentsOutput {
    var result: ListDeploymentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDeploymentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
