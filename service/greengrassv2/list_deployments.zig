...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDeploymentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.history_filter) |v| {
        ...
        try query_buf.appendSlice(allocator, "historyFilter=");
    ...
    if (input.parent_target_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "parentTargetArn=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDeploymentsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDeploymentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
