...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResolveComponentCandidatesInput, options: Options) !ResolveComponentCandidatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResolveComponentCandidatesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.component_candidates) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"componentCandidates\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResolveComponentCandidatesOutput {
    var result: ResolveComponentCandidatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ResolveComponentCandidatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
