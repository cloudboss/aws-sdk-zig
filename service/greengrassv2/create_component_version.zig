...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateComponentVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.inline_recipe) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"inlineRecipe\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.lambda_function) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"lambdaFunction\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateComponentVersionOutput {
    var result: CreateComponentVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateComponentVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
