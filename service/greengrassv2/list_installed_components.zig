...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstalledComponentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/installedComponents");
    ...
    if (input.topology_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "topologyFilter=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstalledComponentsOutput {
    var result: ListInstalledComponentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInstalledComponentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
