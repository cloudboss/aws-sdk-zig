...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchAssociateClientDeviceWithCoreDeviceInput, options: Options) !BatchAssociateClientDeviceWithCoreDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchAssociateClientDeviceWithCoreDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/associateClientDevices");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchAssociateClientDeviceWithCoreDeviceOutput {
    var result: BatchAssociateClientDeviceWithCoreDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchAssociateClientDeviceWithCoreDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
