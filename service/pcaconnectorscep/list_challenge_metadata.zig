...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChallengeMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChallengeMetadataOutput {
    var result: ListChallengeMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChallengeMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
