...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChallengeInput, options: CallOptions) !CreateChallengeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateChallengeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateChallengeOutput {
    var result: CreateChallengeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChallengeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
