...
pub const GetRegexMatchSetInput = struct {
...
pub const GetRegexMatchSetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegexMatchSetInput, options: CallOptions) !GetRegexMatchSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRegexMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20150824.GetRegexMatchSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRegexMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRegexMatchSetOutput, body, allocator);
}
