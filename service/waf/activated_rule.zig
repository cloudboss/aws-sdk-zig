...
const WafRuleType = @import("waf_rule_type.zig").WafRuleType;
...
pub const ActivatedRule = struct {
    ...
    ///   conditions in the rule and then continues to
    ...
    /// the specific
    /// rule within the rule group that is blocking the legitimate traffic and then
    /// disable
    /// (exclude) that particular rule. You can exclude rules from both your own
    /// rule groups and
    ...
    /// * The first action
    /// deletes
    /// the existing rule group from the web ACL. That is, in the UpdateWebACL
    ...
    /// * The second action
    /// inserts
    /// the same rule group back in, but specifying the rules to exclude. That is,
    /// the
    ...
    /// `Priority` are evaluated before `Rules` with a higher value. The value must
    /// be a unique integer. If you add multiple
    ...
    /// is REGULAR. Although this field is optional, be aware that if you try to add
