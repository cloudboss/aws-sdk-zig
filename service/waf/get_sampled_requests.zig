...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSampledRequestsInput, options: Options) !GetSampledRequestsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSampledRequestsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20150824.GetSampledRequests");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSampledRequestsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSampledRequestsOutput, body, alloc);
}
