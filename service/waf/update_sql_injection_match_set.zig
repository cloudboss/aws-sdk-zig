...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSqlInjectionMatchSetInput, options: Options) !UpdateSqlInjectionMatchSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSqlInjectionMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20150824.UpdateSqlInjectionMatchSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSqlInjectionMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSqlInjectionMatchSetOutput, body, alloc);
}
