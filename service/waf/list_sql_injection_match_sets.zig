...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSqlInjectionMatchSetsInput, options: Options) !ListSqlInjectionMatchSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSqlInjectionMatchSetsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("waf", "WAF", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20150824.ListSqlInjectionMatchSets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSqlInjectionMatchSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSqlInjectionMatchSetsOutput, body, alloc);
}
