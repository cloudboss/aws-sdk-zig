...
const SqlInjectionMatchSetSummary = @import("sql_injection_match_set_summary.zig").SqlInjectionMatchSetSummary;

pub const ListSqlInjectionMatchSetsInput = struct {
    /// Specifies the number of SqlInjectionMatchSet objects that you want AWS WAF
    /// to return for this request. If you have more
    ...
    /// `Limit`, AWS WAF returns a `NextMarker` value in the response that allows
    /// you to list another group of
    /// `SqlInjectionMatchSets`. For the second and subsequent
    /// `ListSqlInjectionMatchSets` requests, specify the
...
pub const ListSqlInjectionMatchSetsOutput = struct {
    /// If you have more SqlInjectionMatchSet objects than the number that you
    /// specified for `Limit` in the request,
    /// the response includes a `NextMarker` value. To list more
    /// `SqlInjectionMatchSet` objects, submit another
    /// `ListSqlInjectionMatchSets` request, and specify the `NextMarker` value from
    /// the response in the
    /// `NextMarker` value in the next request.
    next_marker: ?[]const u8 = null,

    /// An array of SqlInjectionMatchSetSummary objects.
    sql_injection_match_sets: ?[]const SqlInjectionMatchSetSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSqlInjectionMatchSetsInput, options: CallOptions) !ListSqlInjectionMatchSetsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "waf");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSqlInjectionMatchSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20150824.ListSqlInjectionMatchSets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSqlInjectionMatchSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSqlInjectionMatchSetsOutput, body, allocator);
}
