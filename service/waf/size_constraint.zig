...
pub const SizeConstraint = struct {
    ...
    /// combination with the provided `Size` and `FieldToMatch`
    ...
    /// with `ComparisonOperator` and `FieldToMatch` to build an expression in the
    /// form of "`Size`
    /// `ComparisonOperator` size
    /// in bytes of `FieldToMatch`". If that expression is true, the
    /// `SizeConstraint` is considered to match.
    ///
