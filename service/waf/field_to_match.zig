const MatchFieldType = @import("match_field_type.zig").MatchFieldType;
...
pub const FieldToMatch = struct {
    ...
    /// want AWS WAF to search,
    /// for example, `User-Agent` or `Referer`. The name of the header is not case
    /// sensitive.
    ///
    /// When the value of `Type` is `SINGLE_QUERY_ARG`, enter the name of the
    /// parameter that you want AWS WAF to search,
    /// for example, `UserName` or `SalesRegion`. The parameter name is not case
    /// sensitive.
    ///
    ...
    /// If you choose `HEADER` for the type, specify the name of the header in
    /// `Data`.
    ///
    /// * `METHOD`: The HTTP method, which indicated the type of operation that the
    ///   request is asking the origin to perform.
    /// Amazon CloudFront supports the following methods: `DELETE`, `GET`, `HEAD`,
    /// `OPTIONS`, `PATCH`,
    /// `POST`, and `PUT`.
    ///
