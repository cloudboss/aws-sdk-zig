...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetChangeTokenInput, options: Options) !GetChangeTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChangeTokenInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20150824.GetChangeToken");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChangeTokenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetChangeTokenOutput, body, alloc);
}
