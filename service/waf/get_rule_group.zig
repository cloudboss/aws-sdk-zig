...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRuleGroupInput, options: Options) !GetRuleGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRuleGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20150824.GetRuleGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRuleGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRuleGroupOutput, body, alloc);
}
