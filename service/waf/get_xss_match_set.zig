...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetXssMatchSetInput, options: Options) !GetXssMatchSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetXssMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20150824.GetXssMatchSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetXssMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetXssMatchSetOutput, body, alloc);
}
