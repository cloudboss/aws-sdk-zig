...
pub const DeleteXssMatchSetInput = struct {
...
pub const DeleteXssMatchSetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteXssMatchSetInput, options: CallOptions) !DeleteXssMatchSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteXssMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20150824.DeleteXssMatchSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteXssMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteXssMatchSetOutput, body, allocator);
}
