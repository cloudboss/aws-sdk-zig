...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRateBasedRuleManagedKeysInput, options: Options) !GetRateBasedRuleManagedKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRateBasedRuleManagedKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20150824.GetRateBasedRuleManagedKeys");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRateBasedRuleManagedKeysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRateBasedRuleManagedKeysOutput, body, alloc);
}
