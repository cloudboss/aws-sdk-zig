...
pub const DeleteWebACLInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteWebACLInput, options: Options) !DeleteWebACLOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteWebACLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20150824.DeleteWebACL");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteWebACLOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteWebACLOutput, body, alloc);
}
