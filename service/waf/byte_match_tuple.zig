...
pub const ByteMatchTuple = struct {
    ...
    /// The specified part of the web request must include the value of
    /// `TargetString`, and
    /// `TargetString` must contain only alphanumeric characters or underscore (A-Z,
    ...
    ///   request, such as the value of a
    /// header.
    ///
    /// * `TargetString` is at the beginning of the specified part of the web
    ///   request and is followed by a character
    /// other than an alphanumeric character or underscore (_), for example,
    /// `BadBot;`.
    ///
    ...
    /// by characters other than alphanumeric characters or underscore (_), for
    /// example, `-BadBot;`.
    ///
    /// **EXACTLY**
    ///
    ...
    /// specified string in the part of web requests that you
    ...
    ///   header that you specified in
    /// FieldToMatch, for example, the value of the `User-Agent` or `Referer`
    /// header.
    ///
    /// * `METHOD`: The HTTP method, which indicates the type of operation specified
    ///   in the request.
    /// CloudFront supports the following methods: `DELETE`, `GET`, `HEAD`,
    /// `OPTIONS`,
    /// `PATCH`, `POST`, and `PUT`.
    ///
    /// * `QUERY_STRING`: The value that you want AWS WAF to search for in the query
    ///   string, which is the part
    ...
    ///   want to send to your web server
    /// as the HTTP request body, such as data from a form. The request body
    /// immediately follows the request headers.
    ...
    /// use in web requests in an effort to bypass AWS WAF.
    ...
    /// characters. `HTML_ENTITY_DECODE` performs
    ...
    ///   `(ampersand)#xhhhh;`, with the corresponding
    /// characters
    ///
    /// * Replaces characters that are represented in decimal format,
    ///   `(ampersand)#nnnn;`, with the corresponding
    /// characters
    ///
    /// **LOWERCASE**
    ///
