...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRateBasedRuleInput, options: Options) !DeleteRateBasedRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRateBasedRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20150824.DeleteRateBasedRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRateBasedRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRateBasedRuleOutput, body, alloc);
}
