...
const DelegatedService = @import("delegated_service.zig").DelegatedService;

pub const ListDelegatedServicesForAccountInput = struct {
...
pub const ListDelegatedServicesForAccountOutput = struct {
    /// The services for which the account is a delegated administrator.
    delegated_services: ?[]const DelegatedService = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDelegatedServicesForAccountInput, options: CallOptions) !ListDelegatedServicesForAccountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDelegatedServicesForAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListDelegatedServicesForAccount");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDelegatedServicesForAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDelegatedServicesForAccountOutput, body, allocator);
}
