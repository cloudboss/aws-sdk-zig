...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterDelegatedAdministratorInput, options: Options) !DeregisterDelegatedAdministratorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterDelegatedAdministratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.DeregisterDelegatedAdministrator");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterDelegatedAdministratorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
