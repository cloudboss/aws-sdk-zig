...
fn serializeRequest(alloc: std.mem.Allocator, input: LeaveOrganizationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.LeaveOrganization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !LeaveOrganizationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
