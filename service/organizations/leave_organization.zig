...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: LeaveOrganizationInput, options: CallOptions) !LeaveOrganizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: LeaveOrganizationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.LeaveOrganization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !LeaveOrganizationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
