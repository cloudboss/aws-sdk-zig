...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InviteAccountToOrganizationInput, options: Options) !InviteAccountToOrganizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InviteAccountToOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.InviteAccountToOrganization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InviteAccountToOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(InviteAccountToOrganizationOutput, body, alloc);
}
