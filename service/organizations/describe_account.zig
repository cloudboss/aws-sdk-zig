...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountInput, options: Options) !DescribeAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.DescribeAccount");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAccountOutput, body, alloc);
}
