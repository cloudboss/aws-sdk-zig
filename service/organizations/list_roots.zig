...
pub const ListRootsInput = struct {
...
pub const ListRootsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRootsInput, options: CallOptions) !ListRootsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRootsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListRoots");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRootsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRootsOutput, body, allocator);
}
