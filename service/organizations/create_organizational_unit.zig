...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOrganizationalUnitInput, options: Options) !CreateOrganizationalUnitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOrganizationalUnitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.CreateOrganizationalUnit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOrganizationalUnitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateOrganizationalUnitOutput, body, alloc);
}
