...
const DelegatedAdministrator = @import("delegated_administrator.zig").DelegatedAdministrator;

pub const ListDelegatedAdministratorsInput = struct {
...
pub const ListDelegatedAdministratorsOutput = struct {
    /// The list of delegated administrators in your organization.
    delegated_administrators: ?[]const DelegatedAdministrator = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDelegatedAdministratorsInput, options: CallOptions) !ListDelegatedAdministratorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDelegatedAdministratorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListDelegatedAdministrators");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDelegatedAdministratorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDelegatedAdministratorsOutput, body, allocator);
}
