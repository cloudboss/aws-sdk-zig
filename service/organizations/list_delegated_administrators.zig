...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDelegatedAdministratorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.ListDelegatedAdministrators");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDelegatedAdministratorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDelegatedAdministratorsOutput, body, alloc);
}
