...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveAccountFromOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.RemoveAccountFromOrganization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveAccountFromOrganizationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
