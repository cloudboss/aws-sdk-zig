...
pub const RemoveAccountFromOrganizationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveAccountFromOrganizationInput, options: CallOptions) !RemoveAccountFromOrganizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveAccountFromOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.RemoveAccountFromOrganization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveAccountFromOrganizationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
