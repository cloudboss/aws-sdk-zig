...
pub const AttachPolicyInput = struct {
    /// ID for the policy that you want to attach to the target. You can get the ID
    /// for the
    ...
    /// can get the
    /// ID by calling the ListRoots, ListOrganizationalUnitsForParent, or
    /// ListAccounts
    /// operations.
    ///
...
fn serializeRequest(allocator: std.mem.Allocator, input: AttachPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.AttachPolicy");
