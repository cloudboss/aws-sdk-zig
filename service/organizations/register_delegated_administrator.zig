...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterDelegatedAdministratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.RegisterDelegatedAdministrator");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterDelegatedAdministratorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
