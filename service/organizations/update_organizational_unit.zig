...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOrganizationalUnitInput, options: Options) !UpdateOrganizationalUnitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateOrganizationalUnitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.UpdateOrganizationalUnit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateOrganizationalUnitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateOrganizationalUnitOutput, body, alloc);
}
