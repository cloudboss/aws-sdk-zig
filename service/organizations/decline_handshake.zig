...
pub const DeclineHandshakeInput = struct {
...
pub const DeclineHandshakeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeclineHandshakeInput, options: CallOptions) !DeclineHandshakeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeclineHandshakeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.DeclineHandshake");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeclineHandshakeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeclineHandshakeOutput, body, allocator);
}
