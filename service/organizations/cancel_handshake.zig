...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelHandshakeInput, options: Options) !CancelHandshakeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelHandshakeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.CancelHandshake");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelHandshakeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelHandshakeOutput, body, alloc);
}
