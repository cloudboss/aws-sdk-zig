...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOrganizationalUnitsForParentInput, options: Options) !ListOrganizationalUnitsForParentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOrganizationalUnitsForParentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.ListOrganizationalUnitsForParent");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOrganizationalUnitsForParentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOrganizationalUnitsForParentOutput, body, alloc);
}
