...
pub const AcceptHandshakeInput = struct {
...
pub const AcceptHandshakeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptHandshakeInput, options: CallOptions) !AcceptHandshakeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptHandshakeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.AcceptHandshake");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptHandshakeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AcceptHandshakeOutput, body, allocator);
}
