...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOutboundResponsibilityTransfersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.ListOutboundResponsibilityTransfers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOutboundResponsibilityTransfersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOutboundResponsibilityTransfersOutput, body, alloc);
}
