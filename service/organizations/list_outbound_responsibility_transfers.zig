...
const ResponsibilityTransfer = @import("responsibility_transfer.zig").ResponsibilityTransfer;

pub const ListOutboundResponsibilityTransfersInput = struct {
...
pub const ListOutboundResponsibilityTransfersOutput = struct {
    ...
    /// transfer.
    responsibility_transfers: ?[]const ResponsibilityTransfer = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .responsibility_transfers = "ResponsibilityTransfers",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOutboundResponsibilityTransfersInput, options: CallOptions) !ListOutboundResponsibilityTransfersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOutboundResponsibilityTransfersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListOutboundResponsibilityTransfers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOutboundResponsibilityTransfersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOutboundResponsibilityTransfersOutput, body, allocator);
}
