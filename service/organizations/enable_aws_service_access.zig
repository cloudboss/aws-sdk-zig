...
pub const EnableAWSServiceAccessInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableAWSServiceAccessInput, options: CallOptions) !EnableAWSServiceAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableAWSServiceAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.EnableAWSServiceAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableAWSServiceAccessOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
