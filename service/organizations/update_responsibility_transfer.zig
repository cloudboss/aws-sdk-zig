...
pub const UpdateResponsibilityTransferInput = struct {
    /// ID for the transfer.
    id: []const u8,
...
pub const UpdateResponsibilityTransferOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResponsibilityTransferInput, options: CallOptions) !UpdateResponsibilityTransferOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateResponsibilityTransferInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.UpdateResponsibilityTransfer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateResponsibilityTransferOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateResponsibilityTransferOutput, body, allocator);
}
