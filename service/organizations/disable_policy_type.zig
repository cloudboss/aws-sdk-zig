...
pub const DisablePolicyTypeInput = struct {
...
pub const DisablePolicyTypeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisablePolicyTypeInput, options: CallOptions) !DisablePolicyTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisablePolicyTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.DisablePolicyType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisablePolicyTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisablePolicyTypeOutput, body, allocator);
}
