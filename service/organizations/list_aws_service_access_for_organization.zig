...
const EnabledServicePrincipal = @import("enabled_service_principal.zig").EnabledServicePrincipal;

pub const ListAWSServiceAccessForOrganizationInput = struct {
...
pub const ListAWSServiceAccessForOrganizationOutput = struct {
    /// A list of the service principals for the services that are enabled to
    /// integrate with
    /// your organization. Each principal is a structure that includes the name and
    /// the date
    /// that it was enabled for integration with Organizations.
    enabled_service_principals: ?[]const EnabledServicePrincipal = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAWSServiceAccessForOrganizationInput, options: CallOptions) !ListAWSServiceAccessForOrganizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAWSServiceAccessForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListAWSServiceAccessForOrganization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAWSServiceAccessForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAWSServiceAccessForOrganizationOutput, body, allocator);
}
