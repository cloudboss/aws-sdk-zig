...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePolicyInput, options: Options) !DescribePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.DescribePolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePolicyOutput, body, alloc);
}
