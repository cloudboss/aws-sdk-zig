...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHandshakesForOrganizationInput, options: Options) !ListHandshakesForOrganizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHandshakesForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.ListHandshakesForOrganization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHandshakesForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListHandshakesForOrganizationOutput, body, alloc);
}
