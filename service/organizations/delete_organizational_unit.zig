...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOrganizationalUnitInput, options: Options) !DeleteOrganizationalUnitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteOrganizationalUnitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.DeleteOrganizationalUnit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteOrganizationalUnitOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
