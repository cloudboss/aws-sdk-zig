...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEffectivePolicyInput, options: Options) !DescribeEffectivePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEffectivePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.DescribeEffectivePolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEffectivePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEffectivePolicyOutput, body, alloc);
}
