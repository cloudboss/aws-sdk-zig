...
pub const ListParentsInput = struct {
...
pub const ListParentsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListParentsInput, options: CallOptions) !ListParentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListParentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListParents");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListParentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListParentsOutput, body, allocator);
}
