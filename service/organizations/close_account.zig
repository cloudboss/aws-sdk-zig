...
pub const CloseAccountInput = struct {
...
pub const CloseAccountOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CloseAccountInput, options: CallOptions) !CloseAccountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CloseAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.CloseAccount");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CloseAccountOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
