...
pub const TerminateResponsibilityTransferInput = struct {
...
pub const TerminateResponsibilityTransferOutput = struct {
    /// A `ResponsibilityTransfer` object. Contains details for a transfer.
    responsibility_transfer: ?ResponsibilityTransfer = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TerminateResponsibilityTransferInput, options: CallOptions) !TerminateResponsibilityTransferOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TerminateResponsibilityTransferInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.TerminateResponsibilityTransfer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TerminateResponsibilityTransferOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TerminateResponsibilityTransferOutput, body, allocator);
}
