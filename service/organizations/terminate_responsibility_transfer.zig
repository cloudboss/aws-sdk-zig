...
fn serializeRequest(alloc: std.mem.Allocator, input: TerminateResponsibilityTransferInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.TerminateResponsibilityTransfer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TerminateResponsibilityTransferOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TerminateResponsibilityTransferOutput, body, alloc);
}
