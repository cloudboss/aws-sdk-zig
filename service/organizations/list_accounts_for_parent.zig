...
pub const ListAccountsForParentInput = struct {
...
pub const ListAccountsForParentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountsForParentInput, options: CallOptions) !ListAccountsForParentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAccountsForParentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListAccountsForParent");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAccountsForParentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAccountsForParentOutput, body, allocator);
}
