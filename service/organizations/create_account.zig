...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccountInput, options: CallOptions) !CreateAccountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.CreateAccount");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAccountOutput, body, allocator);
}
