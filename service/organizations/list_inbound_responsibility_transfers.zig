...
pub const ListInboundResponsibilityTransfersInput = struct {
    /// ID for the transfer.
    id: ?[]const u8 = null,
...
pub const ListInboundResponsibilityTransfersOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInboundResponsibilityTransfersInput, options: CallOptions) !ListInboundResponsibilityTransfersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListInboundResponsibilityTransfersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListInboundResponsibilityTransfers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListInboundResponsibilityTransfersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInboundResponsibilityTransfersOutput, body, allocator);
}
