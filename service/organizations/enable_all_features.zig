...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableAllFeaturesInput, options: CallOptions) !EnableAllFeaturesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableAllFeaturesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.EnableAllFeatures");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableAllFeaturesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(EnableAllFeaturesOutput, body, allocator);
}
