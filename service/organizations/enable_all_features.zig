...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableAllFeaturesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.EnableAllFeatures");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableAllFeaturesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(EnableAllFeaturesOutput, body, alloc);
}
