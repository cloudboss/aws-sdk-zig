...
pub const DescribeOrganizationalUnitInput = struct {
    /// ID for the organizational unit that you want details about. You can get the
    /// ID from
...
pub const DescribeOrganizationalUnitOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrganizationalUnitInput, options: CallOptions) !DescribeOrganizationalUnitOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeOrganizationalUnitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.DescribeOrganizationalUnit");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeOrganizationalUnitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeOrganizationalUnitOutput, body, allocator);
}
