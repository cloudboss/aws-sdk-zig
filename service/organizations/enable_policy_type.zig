...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnablePolicyTypeInput, options: Options) !EnablePolicyTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnablePolicyTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.EnablePolicyType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnablePolicyTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(EnablePolicyTypeOutput, body, alloc);
}
