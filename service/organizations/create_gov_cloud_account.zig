...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGovCloudAccountInput, options: Options) !CreateGovCloudAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGovCloudAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.CreateGovCloudAccount");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGovCloudAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateGovCloudAccountOutput, body, alloc);
}
