...
pub const DescribeResponsibilityTransferInput = struct {
    /// ID for the transfer.
    id: []const u8,
...
pub const DescribeResponsibilityTransferOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeResponsibilityTransferInput, options: CallOptions) !DescribeResponsibilityTransferOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeResponsibilityTransferInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.DescribeResponsibilityTransfer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeResponsibilityTransferOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeResponsibilityTransferOutput, body, allocator);
}
