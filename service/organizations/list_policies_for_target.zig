...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPoliciesForTargetInput, options: Options) !ListPoliciesForTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPoliciesForTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.ListPoliciesForTarget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPoliciesForTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPoliciesForTargetOutput, body, alloc);
}
