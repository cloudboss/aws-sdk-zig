...
pub const ListCreateAccountStatusInput = struct {
    ...
    states: ?[]const CreateAccountState = null,
...
pub const ListCreateAccountStatusOutput = struct {
    ...
    /// successfully
    /// created.
    create_account_statuses: ?[]const CreateAccountStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCreateAccountStatusInput, options: CallOptions) !ListCreateAccountStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCreateAccountStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListCreateAccountStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCreateAccountStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCreateAccountStatusOutput, body, allocator);
}
