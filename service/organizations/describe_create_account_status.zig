...
pub const DescribeCreateAccountStatusInput = struct {
    /// Specifies the `Id` value that uniquely identifies the
    /// `CreateAccount` request. You can get the value from the
    /// `CreateAccountStatus.Id` response in an earlier CreateAccount request, or
    ...
    create_account_request_id: []const u8,
...
pub const DescribeCreateAccountStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCreateAccountStatusInput, options: CallOptions) !DescribeCreateAccountStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeCreateAccountStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.DescribeCreateAccountStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeCreateAccountStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCreateAccountStatusOutput, body, allocator);
}
