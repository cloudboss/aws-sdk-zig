...
pub const ListChildrenInput = struct {
...
pub const ListChildrenOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChildrenInput, options: CallOptions) !ListChildrenOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListChildrenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListChildren");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListChildrenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListChildrenOutput, body, allocator);
}
