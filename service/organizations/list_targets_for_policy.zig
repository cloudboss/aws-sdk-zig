...
const PolicyTargetSummary = @import("policy_target_summary.zig").PolicyTargetSummary;

pub const ListTargetsForPolicyInput = struct {
...
pub const ListTargetsForPolicyOutput = struct {
    ...
    targets: ?[]const PolicyTargetSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTargetsForPolicyInput, options: CallOptions) !ListTargetsForPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTargetsForPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListTargetsForPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTargetsForPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTargetsForPolicyOutput, body, allocator);
}
