...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountsWithInvalidEffectivePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.ListAccountsWithInvalidEffectivePolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountsWithInvalidEffectivePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAccountsWithInvalidEffectivePolicyOutput, body, alloc);
}
