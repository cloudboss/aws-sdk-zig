...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InviteOrganizationToTransferResponsibilityInput, options: Options) !InviteOrganizationToTransferResponsibilityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InviteOrganizationToTransferResponsibilityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.InviteOrganizationToTransferResponsibility");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InviteOrganizationToTransferResponsibilityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(InviteOrganizationToTransferResponsibilityOutput, body, alloc);
}
