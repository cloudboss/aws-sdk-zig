...
pub const DescribeHandshakeInput = struct {
...
pub const DescribeHandshakeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHandshakeInput, options: CallOptions) !DescribeHandshakeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHandshakeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.DescribeHandshake");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHandshakeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeHandshakeOutput, body, allocator);
}
