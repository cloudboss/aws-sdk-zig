...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MoveAccountInput, options: Options) !MoveAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: MoveAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSOrganizationsV20161128.MoveAccount");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MoveAccountOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
