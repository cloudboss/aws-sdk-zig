...
pub const ListHandshakesForAccountInput = struct {
...
pub const ListHandshakesForAccountOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHandshakesForAccountInput, options: CallOptions) !ListHandshakesForAccountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListHandshakesForAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.ListHandshakesForAccount");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListHandshakesForAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListHandshakesForAccountOutput, body, allocator);
}
