...
pub const DisableAWSServiceAccessInput = struct {
...
pub const DisableAWSServiceAccessOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableAWSServiceAccessInput, options: CallOptions) !DisableAWSServiceAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableAWSServiceAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSOrganizationsV20161128.DisableAWSServiceAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableAWSServiceAccessOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
