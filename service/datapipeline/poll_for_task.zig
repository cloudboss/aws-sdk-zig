...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PollForTaskInput, options: Options) !PollForTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PollForTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DataPipeline.PollForTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PollForTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PollForTaskOutput, body, alloc);
}
