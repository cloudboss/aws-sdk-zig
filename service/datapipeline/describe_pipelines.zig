...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePipelinesInput, options: Options) !DescribePipelinesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePipelinesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DataPipeline.DescribePipelines");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePipelinesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribePipelinesOutput, body, alloc);
}
