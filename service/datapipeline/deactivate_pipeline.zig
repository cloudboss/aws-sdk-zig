...
pub const DeactivatePipelineInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeactivatePipelineInput, options: Options) !DeactivatePipelineOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeactivatePipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DataPipeline.DeactivatePipeline");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeactivatePipelineOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
