...
fn serializeRequest(alloc: std.mem.Allocator, input: DeactivatePipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DataPipeline.DeactivatePipeline");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeactivatePipelineOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
