...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: QueryObjectsInput, options: Options) !QueryObjectsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: QueryObjectsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DataPipeline.QueryObjects");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !QueryObjectsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(QueryObjectsOutput, body, alloc);
}
