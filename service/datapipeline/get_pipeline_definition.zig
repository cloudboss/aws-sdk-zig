...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPipelineDefinitionInput, options: Options) !GetPipelineDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPipelineDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DataPipeline.GetPipelineDefinition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPipelineDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPipelineDefinitionOutput, body, alloc);
}
