...
pub const SetStatusInput = struct {
    ...
    /// components, use `PAUSE` or `RESUME`.
...
pub const SetStatusOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetStatusInput, options: CallOptions) !SetStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DataPipeline.SetStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetStatusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
