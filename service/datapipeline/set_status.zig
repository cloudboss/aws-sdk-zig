...
fn serializeRequest(alloc: std.mem.Allocator, input: SetStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DataPipeline.SetStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetStatusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
