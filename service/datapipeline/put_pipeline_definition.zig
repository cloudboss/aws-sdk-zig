...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutPipelineDefinitionInput, options: Options) !PutPipelineDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutPipelineDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DataPipeline.PutPipelineDefinition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutPipelineDefinitionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutPipelineDefinitionOutput, body, alloc);
}
