...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReportTaskProgressInput, options: Options) !ReportTaskProgressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReportTaskProgressInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DataPipeline.ReportTaskProgress");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReportTaskProgressOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ReportTaskProgressOutput, body, alloc);
}
