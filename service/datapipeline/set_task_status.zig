...
fn serializeRequest(alloc: std.mem.Allocator, input: SetTaskStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DataPipeline.SetTaskStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetTaskStatusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
