...
pub const ListTrustedEntitySetsInput = struct {
...
pub const ListTrustedEntitySetsOutput = struct {
    /// The pagination parameter to be used on the next list operation to retrieve
    /// more items.
    next_token: ?[]const u8 = null,

    /// The IDs of the trusted entity set resources.
    trusted_entity_set_ids: ?[]const []const u8 = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .trusted_entity_set_ids = "TrustedEntitySetIds",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrustedEntitySetsInput, options: Options) !ListTrustedEntitySetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrustedEntitySetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/trustedentityset");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrustedEntitySetsOutput {
    var result: ListTrustedEntitySetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTrustedEntitySetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
