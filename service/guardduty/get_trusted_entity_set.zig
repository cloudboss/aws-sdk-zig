...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTrustedEntitySetInput, options: Options) !GetTrustedEntitySetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTrustedEntitySetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTrustedEntitySetOutput {
    var result: GetTrustedEntitySetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTrustedEntitySetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
