...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMasterAccountInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMasterAccountOutput {
    var result: GetMasterAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMasterAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
