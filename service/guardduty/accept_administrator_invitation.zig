...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptAdministratorInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/administrator");
    ...
    try body_buf.appendSlice(allocator, "\"AdministratorId\":");
    try aws.json.writeValue(@TypeOf(input.administrator_id), input.administrator_id, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptAdministratorInvitationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
