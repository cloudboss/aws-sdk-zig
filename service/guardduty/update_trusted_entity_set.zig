...
pub const UpdateTrustedEntitySetInput = struct {
    /// A boolean value that indicates whether GuardDuty is to start using this
    /// updated trusted entity set. After you
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTrustedEntitySetInput, options: CallOptions) !UpdateTrustedEntitySetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTrustedEntitySetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTrustedEntitySetOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateTrustedEntitySetOutput = .{};
