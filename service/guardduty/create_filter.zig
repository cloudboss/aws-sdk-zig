...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFilterInput, options: Options) !CreateFilterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/filter");
    ...
    try aws.json.writeValue(@TypeOf(input.finding_criteria), input.finding_criteria, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.rank) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Rank\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFilterOutput {
    var result: CreateFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFilterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
