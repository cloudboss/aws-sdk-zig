...
pub const ListFindingsInput = struct {
    ...
    /// When it's set
    /// to 'false', only unarchived findings are listed. When this attribute is not
    /// set, all
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFindingsInput, options: Options) !ListFindingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFindingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/findings");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFindingsOutput {
    var result: ListFindingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFindingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
