...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDetectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.enable), input.enable, alloc, &body_buf);
    has_prev = true;
    if (input.features) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDetectorOutput {
    var result: CreateDetectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDetectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
