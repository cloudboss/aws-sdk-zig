...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThreatIntelSetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThreatIntelSetsOutput {
    var result: ListThreatIntelSetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThreatIntelSetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
