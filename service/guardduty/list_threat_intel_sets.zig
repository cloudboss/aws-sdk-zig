...
pub const ListThreatIntelSetsInput = struct {
...
pub const ListThreatIntelSetsOutput = struct {
    ...
    threat_intel_set_ids: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThreatIntelSetsInput, options: CallOptions) !ListThreatIntelSetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListThreatIntelSetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListThreatIntelSetsOutput {
    var result: ListThreatIntelSetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThreatIntelSetsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
