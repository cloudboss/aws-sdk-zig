...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePublishingDestinationInput, options: Options) !DescribePublishingDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePublishingDestinationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePublishingDestinationOutput {
    var result: DescribePublishingDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePublishingDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
