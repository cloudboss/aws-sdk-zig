...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPublishingDestinationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPublishingDestinationsOutput {
    var result: ListPublishingDestinationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPublishingDestinationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
