...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRemainingFreeTrialDaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/freeTrial/daysRemaining");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRemainingFreeTrialDaysOutput {
    var result: GetRemainingFreeTrialDaysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRemainingFreeTrialDaysOutput, body, alloc);
    }
    _ = status;
    _ = headers;
