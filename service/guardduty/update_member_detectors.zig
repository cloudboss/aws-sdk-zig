...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMemberDetectorsInput, options: Options) !UpdateMemberDetectorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMemberDetectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/member/detector/update");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMemberDetectorsOutput {
    var result: UpdateMemberDetectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMemberDetectorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
