...
pub const GetThreatIntelSetInput = struct {
...
pub const GetThreatIntelSetOutput = struct {
    /// The Amazon Web Services account ID that owns the Amazon S3 bucket specified
    /// in the **location** parameter.
    /// This field appears in the response only if it was provided during
    /// ThreatIntelSet creation or update.
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetThreatIntelSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/threatintelset/");
    try path_buf.appendSlice(alloc, input.threat_intel_set_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetThreatIntelSetOutput {
    var result: GetThreatIntelSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetThreatIntelSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
