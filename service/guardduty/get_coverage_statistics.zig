...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCoverageStatisticsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/coverage/statistics");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"StatisticsType\":");
    try aws.json.writeValue(@TypeOf(input.statistics_type), input.statistics_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCoverageStatisticsOutput {
    var result: GetCoverageStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCoverageStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
