...
const CoverageStatisticsType = @import("coverage_statistics_type.zig").CoverageStatisticsType;
const CoverageStatistics = @import("coverage_statistics.zig").CoverageStatistics;
...
pub const GetCoverageStatisticsInput = struct {
    ...
    statistics_type: []const CoverageStatisticsType,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCoverageStatisticsInput, options: CallOptions) !GetCoverageStatisticsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCoverageStatisticsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/coverage/statistics");
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"StatisticsType\":");
    try aws.json.writeValue(@TypeOf(input.statistics_type), input.statistics_type, allocator, &body_buf);
    has_prev = true;
