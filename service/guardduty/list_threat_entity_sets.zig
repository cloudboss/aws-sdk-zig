...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThreatEntitySetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThreatEntitySetsOutput {
    var result: ListThreatEntitySetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThreatEntitySetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
