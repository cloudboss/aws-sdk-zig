...
pub const ListThreatEntitySetsInput = struct {
...
pub const ListThreatEntitySetsOutput = struct {
    /// The pagination parameter to be used on the next list operation to retrieve
    /// more items.
    next_token: ?[]const u8 = null,

    /// The IDs of the threat entity set resources.
    threat_entity_set_ids: ?[]const []const u8 = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .threat_entity_set_ids = "ThreatEntitySetIds",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThreatEntitySetsInput, options: CallOptions) !ListThreatEntitySetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListThreatEntitySetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListThreatEntitySetsOutput {
    var result: ListThreatEntitySetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThreatEntitySetsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
