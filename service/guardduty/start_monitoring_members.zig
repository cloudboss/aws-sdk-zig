...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMonitoringMembersInput, options: CallOptions) !StartMonitoringMembersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartMonitoringMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/member/start");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartMonitoringMembersOutput {
    var result: StartMonitoringMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMonitoringMembersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
