...
pub const StartMonitoringMembersInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMonitoringMembersInput, options: Options) !StartMonitoringMembersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMonitoringMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/member/start");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMonitoringMembersOutput {
    var result: StartMonitoringMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMonitoringMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
