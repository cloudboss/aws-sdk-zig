...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateMembersInput, options: Options) !DisassociateMembersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/member/disassociate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateMembersOutput {
    var result: DisassociateMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
