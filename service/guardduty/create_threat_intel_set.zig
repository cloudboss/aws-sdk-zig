...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateThreatIntelSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateThreatIntelSetOutput {
    var result: CreateThreatIntelSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateThreatIntelSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
