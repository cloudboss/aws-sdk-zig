...
pub const CreateThreatIntelSetInput = struct {
...
pub const CreateThreatIntelSetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateThreatIntelSetInput, options: CallOptions) !CreateThreatIntelSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateThreatIntelSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateThreatIntelSetOutput {
    var result: CreateThreatIntelSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateThreatIntelSetOutput, body, allocator);
    }
    _ = status;
    _ = headers;
