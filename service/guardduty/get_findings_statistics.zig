...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingsStatisticsInput, options: Options) !GetFindingsStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingsStatisticsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/findings/statistics");
    ...
    if (input.finding_statistic_types) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FindingStatisticTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.group_by) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingsStatisticsOutput {
    var result: GetFindingsStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingsStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
