...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.filter_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteFilterOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteFilterOutput = .{};
