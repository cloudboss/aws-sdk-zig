...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDetectorInput, options: Options) !GetDetectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDetectorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDetectorOutput {
    var result: GetDetectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDetectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
