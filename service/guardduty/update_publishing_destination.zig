...
pub const UpdatePublishingDestinationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePublishingDestinationInput, options: Options) !UpdatePublishingDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePublishingDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/publishingDestination/");
    try path_buf.appendSlice(alloc, input.destination_id);
    ...
    if (input.destination_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DestinationProperties\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePublishingDestinationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdatePublishingDestinationOutput = .{};
