...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateThreatEntitySetInput, options: Options) !CreateThreatEntitySetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateThreatEntitySetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateThreatEntitySetOutput {
    var result: CreateThreatEntitySetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateThreatEntitySetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
