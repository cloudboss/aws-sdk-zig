...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUsageStatisticsInput, options: Options) !GetUsageStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUsageStatisticsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/usage/statistics");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UsageCriteria\":");
    try aws.json.writeValue(@TypeOf(input.usage_criteria), input.usage_criteria, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UsageStatisticType\":");
    try aws.json.writeValue(@TypeOf(input.usage_statistic_type), input.usage_statistic_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUsageStatisticsOutput {
    var result: GetUsageStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUsageStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
