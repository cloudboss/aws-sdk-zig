...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptInvitationInput, options: Options) !AcceptInvitationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/master");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MasterId\":");
    try aws.json.writeValue(@TypeOf(input.master_id), input.master_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptInvitationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
