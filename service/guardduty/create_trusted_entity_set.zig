...
pub const CreateTrustedEntitySetInput = struct {
...
pub const CreateTrustedEntitySetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrustedEntitySetInput, options: Options) !CreateTrustedEntitySetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrustedEntitySetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrustedEntitySetOutput {
    var result: CreateTrustedEntitySetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTrustedEntitySetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
