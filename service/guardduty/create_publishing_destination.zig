...
pub const CreatePublishingDestinationInput = struct {
...
pub const CreatePublishingDestinationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePublishingDestinationInput, options: CallOptions) !CreatePublishingDestinationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreatePublishingDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.destination_properties), input.destination_properties, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreatePublishingDestinationOutput {
    var result: CreatePublishingDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePublishingDestinationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
