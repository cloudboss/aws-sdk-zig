...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFilterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFilterOutput {
    var result: GetFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFilterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
