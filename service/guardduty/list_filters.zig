...
pub const ListFiltersOutput = struct {
    ...
    filter_names: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFiltersInput, options: Options) !ListFiltersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFiltersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFiltersOutput {
    var result: ListFiltersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFiltersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
