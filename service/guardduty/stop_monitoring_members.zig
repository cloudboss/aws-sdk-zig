...
fn serializeRequest(alloc: std.mem.Allocator, input: StopMonitoringMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/member/stop");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopMonitoringMembersOutput {
    var result: StopMonitoringMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopMonitoringMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
