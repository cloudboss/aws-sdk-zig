...
pub const GetMemberDetectorsInput = struct {
...
pub const GetMemberDetectorsOutput = struct {
    ...
    pub const json_field_names = .{
        .member_data_source_configurations = "MemberDataSourceConfigurations",
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMemberDetectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/member/detector/get");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMemberDetectorsOutput {
    var result: GetMemberDetectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMemberDetectorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
