...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFraudstersInput, options: Options) !ListFraudstersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFraudstersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VoiceID.ListFraudsters");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFraudstersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFraudstersOutput, body, alloc);
}
