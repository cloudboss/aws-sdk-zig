...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFraudsterRegistrationJobInput, options: Options) !StartFraudsterRegistrationJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartFraudsterRegistrationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "VoiceID.StartFraudsterRegistrationJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartFraudsterRegistrationJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartFraudsterRegistrationJobOutput, body, alloc);
}
