...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveRoleFromDBClusterInput, options: CallOptions) !RemoveRoleFromDBClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveRoleFromDBClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RemoveRoleFromDBCluster&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveRoleFromDBClusterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
