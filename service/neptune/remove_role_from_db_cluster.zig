...
pub const RemoveRoleFromDBClusterInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveRoleFromDBClusterInput, options: Options) !RemoveRoleFromDBClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveRoleFromDBClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RemoveRoleFromDBCluster&Version=2014-10-31");
    ...
    if (input.feature_name) |v| {
        try body_buf.appendSlice(alloc, "&FeatureName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveRoleFromDBClusterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: RemoveRoleFromDBClusterOutput = .{};
