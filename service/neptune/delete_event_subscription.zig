...
pub const DeleteEventSubscriptionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEventSubscriptionInput, options: Options) !DeleteEventSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteEventSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteEventSubscription&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&SubscriptionName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.subscription_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteEventSubscriptionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteEventSubscriptionResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteEventSubscriptionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EventSubscription")) {
                    result.event_subscription = try serde.deserializeEventSubscription(&reader, alloc);
                } else {
