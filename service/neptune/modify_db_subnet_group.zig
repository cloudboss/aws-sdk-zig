...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBSubnetGroupInput, options: Options) !ModifyDBSubnetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBSubnetGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBSubnetGroupOutput {
    _ = status;
    _ = headers;
