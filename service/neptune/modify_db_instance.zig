...
pub const ModifyDBInstanceInput = struct {
    ...
    /// instance during
    /// the maintenance window. Changing this parameter doesn't result in an outage
    /// except in the
    /// following case and the change is asynchronously applied as soon as possible.
    /// An outage will
    /// result if this parameter is set to `true` during the maintenance window, and
    /// a
    /// newer minor version is available, and Neptune has enabled auto patching for
    /// that engine
    /// version.
    ...
    /// this setting
    /// doesn't result in an outage. The parameter group name itself is changed
    /// immediately, but the
    /// actual parameter changes are not applied until you reboot the instance
    /// without failover. The
    /// db instance will NOT be rebooted automatically and the parameter changes
    /// will NOT be applied
    ...
    /// instance. When you
    /// change the DB instance identifier, an instance reboot will occur immediately
    /// if you set
    /// `Apply Immediately` to true, or will occur during the next maintenance
    /// window if
    ...
    /// possible. If there
    /// are pending actions that cause a reboot, and the maintenance window is
    /// changed to include the
    /// current time, then changing this parameter will cause a reboot of the DB
    /// instance. If moving
    /// this window to the current time, there must be at least 30 minutes between
    /// the current time
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBInstanceInput, options: Options) !ModifyDBInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBInstance&Version=2014-10-31");
    ...
    if (input.db_port_number) |v| {
        try body_buf.appendSlice(alloc, "&DBPortNumber=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    ...
    if (input.new_db_instance_identifier) |v| {
        try body_buf.appendSlice(alloc, "&NewDBInstanceIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBInstanceOutput {
    _ = status;
    _ = headers;
