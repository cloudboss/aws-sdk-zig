...
pub const DescribeDBParameterGroupsInput = struct {
...
pub const DescribeDBParameterGroupsOutput = struct {
    /// A list of DBParameterGroup instances.
    db_parameter_groups: ?[]const DBParameterGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBParameterGroupsInput, options: Options) !DescribeDBParameterGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBParameterGroups&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBParameterGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBParameterGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBParameterGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBParameterGroups")) {
                    result.db_parameter_groups = try serde.deserializeDBParameterGroupList(&reader, alloc, "DBParameterGroup");
