...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBParameterGroupsInput, options: CallOptions) !DescribeDBParameterGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDBParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDBParameterGroups&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDBParameterGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBParameterGroups")) {
                    result.db_parameter_groups = try serde.deserializeDBParameterGroupList(allocator, &reader, "DBParameterGroup");
