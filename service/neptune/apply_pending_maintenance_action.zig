...
fn serializeRequest(alloc: std.mem.Allocator, input: ApplyPendingMaintenanceActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ApplyPendingMaintenanceAction&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&ApplyAction=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.apply_action);
    try body_buf.appendSlice(alloc, "&OptInType=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.opt_in_type);
    try body_buf.appendSlice(alloc, "&ResourceIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.resource_identifier);
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ResourcePendingMaintenanceActions")) {
                    result.resource_pending_maintenance_actions = try serde.deserializeResourcePendingMaintenanceActions(&reader, alloc);
                } else {
