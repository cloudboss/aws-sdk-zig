...
pub const StopDBClusterInput = struct {
    /// The DB cluster identifier of the Neptune DB cluster to be stopped.
    /// This parameter is stored as a lowercase string.
    db_cluster_identifier: []const u8,
};

pub const StopDBClusterOutput = struct {
    db_cluster: ?DBCluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopDBClusterInput, options: CallOptions) !StopDBClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopDBClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=StopDBCluster&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopDBClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StopDBClusterResult")) break;
            },
            else => {},
        }
    }

    var result: StopDBClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
