...
pub const DeleteDBParameterGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBParameterGroupInput, options: Options) !DeleteDBParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDBParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteDBParameterGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDBParameterGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: DeleteDBParameterGroupOutput = .{};
