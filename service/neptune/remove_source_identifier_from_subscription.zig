...
pub const RemoveSourceIdentifierFromSubscriptionInput = struct {
    /// The source identifier to be removed from the subscription, such as the **DB
    /// instance identifier** for a DB instance or the name of a security
    /// group.
...
pub const RemoveSourceIdentifierFromSubscriptionOutput = struct {
    event_subscription: ?EventSubscription = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveSourceIdentifierFromSubscriptionInput, options: CallOptions) !RemoveSourceIdentifierFromSubscriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveSourceIdentifierFromSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RemoveSourceIdentifierFromSubscription&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveSourceIdentifierFromSubscriptionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RemoveSourceIdentifierFromSubscriptionResult")) break;
            },
            else => {},
        }
    }

    var result: RemoveSourceIdentifierFromSubscriptionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
