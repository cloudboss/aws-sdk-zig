...
pub const DescribeDBClusterParameterGroupsInput = struct {
...
pub const DescribeDBClusterParameterGroupsOutput = struct {
    /// A list of DB cluster parameter groups.
    db_cluster_parameter_groups: ?[]const DBClusterParameterGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBClusterParameterGroupsInput, options: Options) !DescribeDBClusterParameterGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBClusterParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBClusterParameterGroups&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBClusterParameterGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBClusterParameterGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBClusterParameterGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterParameterGroups")) {
                    result.db_cluster_parameter_groups = try serde.deserializeDBClusterParameterGroupList(&reader, alloc, "DBClusterParameterGroup");
