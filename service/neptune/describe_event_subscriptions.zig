...
pub const DescribeEventSubscriptionsInput = struct {
...
pub const DescribeEventSubscriptionsOutput = struct {
    /// A list of EventSubscriptions data types.
    event_subscriptions_list: ?[]const EventSubscription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventSubscriptionsInput, options: Options) !DescribeEventSubscriptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEventSubscriptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEventSubscriptions&Version=2014-10-31");
    ...
    if (input.subscription_name) |v| {
        try body_buf.appendSlice(alloc, "&SubscriptionName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEventSubscriptionsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeEventSubscriptionsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeEventSubscriptionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EventSubscriptionsList")) {
                    result.event_subscriptions_list = try serde.deserializeEventSubscriptionsList(&reader, alloc, "EventSubscription");
