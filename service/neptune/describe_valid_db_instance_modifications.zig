...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeValidDBInstanceModificationsInput, options: Options) !DescribeValidDBInstanceModificationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeValidDBInstanceModificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeValidDBInstanceModifications&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeValidDBInstanceModificationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ValidDBInstanceModificationsMessage")) {
                    result.valid_db_instance_modifications_message = try serde.deserializeValidDBInstanceModificationsMessage(&reader, alloc);
                } else {
