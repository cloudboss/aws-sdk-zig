...
pub const DeleteGlobalClusterInput = struct {
    /// The cluster identifier of the global database cluster being deleted.
    global_cluster_identifier: []const u8,
};

pub const DeleteGlobalClusterOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGlobalClusterInput, options: Options) !DeleteGlobalClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGlobalClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteGlobalCluster&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGlobalClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteGlobalClusterResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteGlobalClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
