...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddTagsToResourceInput, options: Options) !AddTagsToResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddTagsToResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AddTagsToResource&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&ResourceName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.resource_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddTagsToResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: AddTagsToResourceOutput = .{};
