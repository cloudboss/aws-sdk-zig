...
pub const DescribeDBParametersInput = struct {
...
pub const DescribeDBParametersOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBParametersInput, options: Options) !DescribeDBParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBParameters&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&DBParameterGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_parameter_group_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBParametersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBParametersResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBParametersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
