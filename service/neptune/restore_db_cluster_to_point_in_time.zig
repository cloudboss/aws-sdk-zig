...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreDBClusterToPointInTimeInput, options: Options) !RestoreDBClusterToPointInTimeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreDBClusterToPointInTimeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RestoreDBClusterToPointInTime&Version=2014-10-31");
    ...
    if (input.restore_to_time) |v| {
        try body_buf.appendSlice(alloc, "&RestoreToTime=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.restore_type) |v| {
        try body_buf.appendSlice(alloc, "&RestoreType=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_db_cluster_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreDBClusterToPointInTimeOutput {
    _ = status;
    _ = headers;
