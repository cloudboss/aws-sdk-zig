...
pub const DeleteDBSubnetGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBSubnetGroupInput, options: Options) !DeleteDBSubnetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDBSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteDBSubnetGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDBSubnetGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: DeleteDBSubnetGroupOutput = .{};
