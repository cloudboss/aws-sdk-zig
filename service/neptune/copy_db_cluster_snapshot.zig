...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyDBClusterSnapshotInput, options: Options) !CopyDBClusterSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyDBClusterSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CopyDBClusterSnapshot&Version=2014-10-31");
    ...
    try body_buf.appendSlice(alloc, "&SourceDBClusterSnapshotIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_db_cluster_snapshot_identifier);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    try body_buf.appendSlice(alloc, "&TargetDBClusterSnapshotIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_db_cluster_snapshot_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyDBClusterSnapshotOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterSnapshot")) {
                    result.db_cluster_snapshot = try serde.deserializeDBClusterSnapshot(&reader, alloc);
                } else {
