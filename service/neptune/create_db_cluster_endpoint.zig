...
pub const CreateDBClusterEndpointOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDBClusterEndpointInput, options: Options) !CreateDBClusterEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDBClusterEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateDBClusterEndpoint&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&DBClusterEndpointIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_cluster_endpoint_identifier);
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.endpoint_type);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDBClusterEndpointOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CustomEndpointType")) {
                    ...
                    result.excluded_members = try serde.deserializeStringList(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "StaticMembers")) {
                    result.static_members = try serde.deserializeStringList(&reader, alloc, "member");
