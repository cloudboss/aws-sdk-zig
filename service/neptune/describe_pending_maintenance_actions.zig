...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePendingMaintenanceActionsInput, options: Options) !DescribePendingMaintenanceActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePendingMaintenanceActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribePendingMaintenanceActions&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePendingMaintenanceActionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.pending_maintenance_actions = try serde.deserializePendingMaintenanceActions(&reader, alloc, "ResourcePendingMaintenanceActions");
                } else {
