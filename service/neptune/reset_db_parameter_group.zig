...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetDBParameterGroupInput, options: Options) !ResetDBParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetDBParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ResetDBParameterGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetDBParameterGroupOutput {
    _ = status;
    _ = headers;
