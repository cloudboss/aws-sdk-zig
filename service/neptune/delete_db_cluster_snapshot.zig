...
pub const DeleteDBClusterSnapshotInput = struct {
...
pub const DeleteDBClusterSnapshotOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBClusterSnapshotInput, options: Options) !DeleteDBClusterSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDBClusterSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteDBClusterSnapshot&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&DBClusterSnapshotIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_cluster_snapshot_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDBClusterSnapshotOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteDBClusterSnapshotResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteDBClusterSnapshotOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
