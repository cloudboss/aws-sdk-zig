...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyGlobalClusterInput, options: Options) !ModifyGlobalClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyGlobalClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyGlobalCluster&Version=2014-10-31");
    ...
    if (input.new_global_cluster_identifier) |v| {
        try body_buf.appendSlice(alloc, "&NewGlobalClusterIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyGlobalClusterOutput {
    _ = status;
    _ = headers;
