...
pub const RemoveFromGlobalClusterInput = struct {
...
pub const RemoveFromGlobalClusterOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveFromGlobalClusterInput, options: Options) !RemoveFromGlobalClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveFromGlobalClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RemoveFromGlobalCluster&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveFromGlobalClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RemoveFromGlobalClusterResult")) break;
            },
            else => {},
        }
    }

    var result: RemoveFromGlobalClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
