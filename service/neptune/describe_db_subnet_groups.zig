...
pub const DescribeDBSubnetGroupsInput = struct {
...
pub const DescribeDBSubnetGroupsOutput = struct {
    /// A list of DBSubnetGroup instances.
    db_subnet_groups: ?[]const DBSubnetGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBSubnetGroupsInput, options: Options) !DescribeDBSubnetGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBSubnetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBSubnetGroups&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBSubnetGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBSubnetGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBSubnetGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBSubnetGroups")) {
                    result.db_subnet_groups = try serde.deserializeDBSubnetGroups(&reader, alloc, "DBSubnetGroup");
