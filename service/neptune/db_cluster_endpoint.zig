...
pub const DBClusterEndpoint = struct {
    ...
    /// `WRITER`, `ANY`.
    custom_endpoint_type: ?[]const u8,

    /// The Amazon Resource Name (ARN) for the endpoint.
    db_cluster_endpoint_arn: ?[]const u8,
    ...
    db_cluster_endpoint_resource_identifier: ?[]const u8,
    ...
    excluded_members: ?[]const []const u8,

    /// List of DB instance identifiers that are part of the custom endpoint group.
    static_members: ?[]const []const u8,
