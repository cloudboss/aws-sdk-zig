...
pub const ModifyDBClusterSnapshotAttributeInput = struct {
    ...
    /// snapshot, set this
    /// list to include one or more Amazon account IDs, or `all` to make the manual
    /// DB cluster
    /// snapshot restorable by any Amazon account. Do not add the `all` value for
    /// any manual
    ...
    /// manual DB cluster
    /// snapshot, set this list to include one or more Amazon account identifiers,
    /// or `all` to
    /// remove authorization for any Amazon account to copy or restore the DB
    /// cluster snapshot. If you
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBClusterSnapshotAttributeInput, options: Options) !ModifyDBClusterSnapshotAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBClusterSnapshotAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBClusterSnapshotAttribute&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBClusterSnapshotAttributeOutput {
    _ = status;
    _ = headers;
