...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBClustersInput, options: Options) !DescribeDBClustersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBClustersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBClusters&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBClustersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusters")) {
                    result.db_clusters = try serde.deserializeDBClusterList(&reader, alloc, "DBCluster");
