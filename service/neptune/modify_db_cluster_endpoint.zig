...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBClusterEndpointInput, options: Options) !ModifyDBClusterEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBClusterEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBClusterEndpoint&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBClusterEndpointOutput {
    _ = status;
    _ = headers;
