...
pub const DescribeDBInstancesInput = struct {
...
pub const DescribeDBInstancesOutput = struct {
    /// A list of DBInstance instances.
    db_instances: ?[]const DBInstance = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBInstancesInput, options: Options) !DescribeDBInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBInstances&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBInstancesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBInstancesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBInstancesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBInstances")) {
                    result.db_instances = try serde.deserializeDBInstanceList(&reader, alloc, "DBInstance");
