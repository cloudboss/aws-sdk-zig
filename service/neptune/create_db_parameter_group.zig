...
pub const CreateDBParameterGroupInput = struct {
    ...
    /// with one and
    /// only one DB parameter group family, and can be applied only to a DB instance
    /// running a
    /// database engine and engine version compatible with that DB parameter group
    /// family.
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDBParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateDBParameterGroup&Version=2014-10-31");
