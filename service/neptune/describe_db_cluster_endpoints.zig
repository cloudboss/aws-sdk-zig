...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBClusterEndpointsInput, options: Options) !DescribeDBClusterEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBClusterEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBClusterEndpoints&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBClusterEndpointsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterEndpoints")) {
                    result.db_cluster_endpoints = try serde.deserializeDBClusterEndpointList(&reader, alloc, "DBClusterEndpointList");
