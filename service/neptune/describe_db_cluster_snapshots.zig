...
pub const DescribeDBClusterSnapshotsInput = struct {
    ...
    /// be used in
    /// conjunction with the `DBClusterIdentifier` parameter. This value is stored
    /// as a
    /// lowercase string.
    ///
    /// Constraints:
    ...
    /// copied or restored
    /// by any Amazon account, and otherwise false. The default is `false`. The
    /// default is
    /// false.
    ///
    ...
    /// DB
    /// cluster snapshots are returned. You can include shared DB cluster snapshots
    /// with these results
