...
pub const PromoteReadReplicaDBClusterInput = struct {
    /// Not supported.
    db_cluster_identifier: []const u8,
};

pub const PromoteReadReplicaDBClusterOutput = struct {
    db_cluster: ?DBCluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PromoteReadReplicaDBClusterInput, options: CallOptions) !PromoteReadReplicaDBClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PromoteReadReplicaDBClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=PromoteReadReplicaDBCluster&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PromoteReadReplicaDBClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PromoteReadReplicaDBClusterResult")) break;
            },
            else => {},
        }
    }

    var result: PromoteReadReplicaDBClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
