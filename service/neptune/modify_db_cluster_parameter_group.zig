...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBClusterParameterGroupInput, options: Options) !ModifyDBClusterParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBClusterParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBClusterParameterGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBClusterParameterGroupOutput {
    _ = status;
    _ = headers;
