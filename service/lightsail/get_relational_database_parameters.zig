...
const RelationalDatabaseParameter = @import("relational_database_parameter.zig").RelationalDatabaseParameter;

pub const GetRelationalDatabaseParametersInput = struct {
...
pub const GetRelationalDatabaseParametersOutput = struct {
    /// The token to advance to the next page of results from your request.
    ///
    ...
    /// request.
    parameters: ?[]const RelationalDatabaseParameter = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRelationalDatabaseParametersInput, options: CallOptions) !GetRelationalDatabaseParametersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRelationalDatabaseParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetRelationalDatabaseParameters");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRelationalDatabaseParametersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRelationalDatabaseParametersOutput, body, allocator);
}
