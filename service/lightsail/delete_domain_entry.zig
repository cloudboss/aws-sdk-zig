...
pub const DeleteDomainEntryInput = struct {
...
pub const DeleteDomainEntryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDomainEntryInput, options: CallOptions) !DeleteDomainEntryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDomainEntryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.DeleteDomainEntry");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDomainEntryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteDomainEntryOutput, body, allocator);
}
