...
pub const GetContainerImagesInput = struct {
...
pub const GetContainerImagesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContainerImagesInput, options: CallOptions) !GetContainerImagesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetContainerImagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetContainerImages");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetContainerImagesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetContainerImagesOutput, body, allocator);
}
