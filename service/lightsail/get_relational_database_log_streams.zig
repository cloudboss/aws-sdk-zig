...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRelationalDatabaseLogStreamsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.GetRelationalDatabaseLogStreams");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRelationalDatabaseLogStreamsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRelationalDatabaseLogStreamsOutput, body, alloc);
}
