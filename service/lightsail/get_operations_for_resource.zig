...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOperationsForResourceInput, options: Options) !GetOperationsForResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOperationsForResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.GetOperationsForResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOperationsForResourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOperationsForResourceOutput, body, alloc);
}
