...
const RelationalDatabase = @import("relational_database.zig").RelationalDatabase;

pub const GetRelationalDatabasesInput = struct {
...
pub const GetRelationalDatabasesOutput = struct {
    /// The token to advance to the next page of results from your request.
    ///
    ...
    relational_databases: ?[]const RelationalDatabase = null,

    pub const json_field_names = .{
        .next_page_token = "nextPageToken",
        .relational_databases = "relationalDatabases",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRelationalDatabasesInput, options: CallOptions) !GetRelationalDatabasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRelationalDatabasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetRelationalDatabases");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRelationalDatabasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRelationalDatabasesOutput, body, allocator);
}
