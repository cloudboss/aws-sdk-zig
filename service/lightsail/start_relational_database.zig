...
pub const StartRelationalDatabaseInput = struct {
...
pub const StartRelationalDatabaseOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRelationalDatabaseInput, options: CallOptions) !StartRelationalDatabaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartRelationalDatabaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.StartRelationalDatabase");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartRelationalDatabaseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartRelationalDatabaseOutput, body, allocator);
}
