...
pub const GetLoadBalancerInput = struct {
...
pub const GetLoadBalancerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLoadBalancerInput, options: CallOptions) !GetLoadBalancerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLoadBalancerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetLoadBalancer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLoadBalancerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLoadBalancerOutput, body, allocator);
}
