...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestAlarmInput, options: Options) !TestAlarmOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestAlarmInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.TestAlarm");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestAlarmOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TestAlarmOutput, body, alloc);
}
