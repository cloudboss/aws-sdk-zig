...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInstanceSnapshotInput, options: Options) !CreateInstanceSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInstanceSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.CreateInstanceSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInstanceSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateInstanceSnapshotOutput, body, alloc);
}
