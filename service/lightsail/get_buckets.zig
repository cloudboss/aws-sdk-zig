...
pub const GetBucketsInput = struct {
    ...
    /// action.
    include_connected_resources: ?bool = null,
    ...
    pub const json_field_names = .{
        ...
        .include_cors = "includeCors",
        .page_token = "pageToken",
    };
};

pub const GetBucketsOutput = struct {
    ...
    /// Lightsail](https://docs.aws.amazon.com/lightsail/latest/userguide/amazon-lightsail-block-public-access-for-buckets).
    account_level_bpa_sync: ?AccountLevelBpaSync = null,
    ...
    pub const json_field_names = .{
        .account_level_bpa_sync = "accountLevelBpaSync",
        .buckets = "buckets",
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBucketsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetBuckets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBucketsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetBucketsOutput, body, allocator);
}
