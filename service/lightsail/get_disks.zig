...
pub const GetDisksInput = struct {
...
pub const GetDisksOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDisksInput, options: CallOptions) !GetDisksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDisksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetDisks");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDisksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDisksOutput, body, allocator);
}
