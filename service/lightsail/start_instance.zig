...
pub const StartInstanceInput = struct {
...
pub const StartInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartInstanceInput, options: CallOptions) !StartInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.StartInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartInstanceOutput, body, allocator);
}
