...
pub const ExportSnapshotInput = struct {
...
pub const ExportSnapshotOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportSnapshotInput, options: CallOptions) !ExportSnapshotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExportSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.ExportSnapshot");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExportSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportSnapshotOutput, body, allocator);
}
