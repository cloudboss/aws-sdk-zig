...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachInstancesToLoadBalancerInput, options: Options) !AttachInstancesToLoadBalancerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachInstancesToLoadBalancerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.AttachInstancesToLoadBalancer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachInstancesToLoadBalancerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AttachInstancesToLoadBalancerOutput, body, alloc);
}
