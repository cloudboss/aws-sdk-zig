...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: IsVpcPeeredInput, options: Options) !IsVpcPeeredOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: IsVpcPeeredInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.IsVpcPeered");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !IsVpcPeeredOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(IsVpcPeeredOutput, body, alloc);
}
