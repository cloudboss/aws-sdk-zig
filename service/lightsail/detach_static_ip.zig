...
pub const DetachStaticIpInput = struct {
...
pub const DetachStaticIpOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachStaticIpInput, options: CallOptions) !DetachStaticIpOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DetachStaticIpInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.DetachStaticIp");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DetachStaticIpOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetachStaticIpOutput, body, allocator);
}
