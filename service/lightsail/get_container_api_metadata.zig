...
pub const GetContainerAPIMetadataOutput = struct {
    ...
    metadata: ?[]const []const aws.map.StringMapEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContainerAPIMetadataInput, options: CallOptions) !GetContainerAPIMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetContainerAPIMetadataInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetContainerAPIMetadata");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetContainerAPIMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetContainerAPIMetadataOutput, body, allocator);
}
