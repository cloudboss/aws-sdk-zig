...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCertificateInput, options: Options) !CreateCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.CreateCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCertificateOutput, body, alloc);
}
