...
pub const DeleteDistributionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDistributionInput, options: CallOptions) !DeleteDistributionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDistributionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.DeleteDistribution");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDistributionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteDistributionOutput, body, allocator);
}
