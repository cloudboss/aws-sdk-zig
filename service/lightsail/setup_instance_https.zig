...
fn serializeRequest(alloc: std.mem.Allocator, input: SetupInstanceHttpsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.SetupInstanceHttps");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetupInstanceHttpsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SetupInstanceHttpsOutput, body, alloc);
}
