...
pub const SetupInstanceHttpsInput = struct {
...
pub const SetupInstanceHttpsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetupInstanceHttpsInput, options: CallOptions) !SetupInstanceHttpsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetupInstanceHttpsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.SetupInstanceHttps");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetupInstanceHttpsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SetupInstanceHttpsOutput, body, allocator);
}
