...
const AutoSnapshotDetails = @import("auto_snapshot_details.zig").AutoSnapshotDetails;
const ResourceType = @import("resource_type.zig").ResourceType;

pub const GetAutoSnapshotsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutoSnapshotsInput, options: CallOptions) !GetAutoSnapshotsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAutoSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetAutoSnapshots");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAutoSnapshotsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAutoSnapshotsOutput, body, allocator);
}
