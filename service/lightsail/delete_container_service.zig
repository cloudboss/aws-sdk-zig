...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteContainerServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.DeleteContainerService");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteContainerServiceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
