...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContactMethodInput, options: Options) !CreateContactMethodOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContactMethodInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.CreateContactMethod");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContactMethodOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateContactMethodOutput, body, alloc);
}
