...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInstanceSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.GetInstanceSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInstanceSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInstanceSnapshotOutput, body, alloc);
}
