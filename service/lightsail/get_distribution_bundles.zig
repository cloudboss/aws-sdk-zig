...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDistributionBundlesInput, options: Options) !GetDistributionBundlesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDistributionBundlesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.GetDistributionBundles");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDistributionBundlesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDistributionBundlesOutput, body, alloc);
}
