...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContainerServiceInput, options: CallOptions) !UpdateContainerServiceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateContainerServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.UpdateContainerService");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateContainerServiceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateContainerServiceOutput, body, allocator);
}
