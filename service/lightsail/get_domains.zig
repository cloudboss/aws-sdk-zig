...
pub const GetDomainsInput = struct {
...
pub const GetDomainsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDomainsInput, options: CallOptions) !GetDomainsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDomainsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetDomains");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDomainsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDomainsOutput, body, allocator);
}
