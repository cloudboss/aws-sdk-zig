...
pub const DeleteAutoSnapshotInput = struct {
...
pub const DeleteAutoSnapshotOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAutoSnapshotInput, options: CallOptions) !DeleteAutoSnapshotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAutoSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.DeleteAutoSnapshot");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAutoSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteAutoSnapshotOutput, body, allocator);
}
