...
pub const CreateDiskSnapshotInput = struct {
...
pub const CreateDiskSnapshotOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDiskSnapshotInput, options: CallOptions) !CreateDiskSnapshotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDiskSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.CreateDiskSnapshot");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateDiskSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDiskSnapshotOutput, body, allocator);
}
