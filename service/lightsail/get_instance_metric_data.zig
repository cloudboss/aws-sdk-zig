...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInstanceMetricDataInput, options: CallOptions) !GetInstanceMetricDataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetInstanceMetricDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetInstanceMetricData");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetInstanceMetricDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInstanceMetricDataOutput, body, allocator);
}
