...
pub const GetLoadBalancersInput = struct {
...
pub const GetLoadBalancersOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLoadBalancersInput, options: CallOptions) !GetLoadBalancersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLoadBalancersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetLoadBalancers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLoadBalancersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLoadBalancersOutput, body, allocator);
}
