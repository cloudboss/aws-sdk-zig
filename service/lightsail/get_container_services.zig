...
pub const GetContainerServicesInput = struct {
...
pub const GetContainerServicesOutput = struct {
    /// An array of objects that describe one or more container services.
    container_services: ?[]const ContainerService = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContainerServicesInput, options: CallOptions) !GetContainerServicesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetContainerServicesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetContainerServices");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetContainerServicesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetContainerServicesOutput, body, allocator);
}
