...
pub const DetachDiskInput = struct {
...
pub const DetachDiskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachDiskInput, options: CallOptions) !DetachDiskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DetachDiskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.DetachDisk");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DetachDiskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetachDiskOutput, body, allocator);
}
