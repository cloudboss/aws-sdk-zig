...
pub const GetActiveNamesInput = struct {
...
pub const GetActiveNamesOutput = struct {
    /// The list of active names returned by the get active names request.
    active_names: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetActiveNamesInput, options: CallOptions) !GetActiveNamesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetActiveNamesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetActiveNames");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetActiveNamesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetActiveNamesOutput, body, allocator);
}
