...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRelationalDatabaseInput, options: CallOptions) !DeleteRelationalDatabaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRelationalDatabaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.DeleteRelationalDatabase");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRelationalDatabaseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRelationalDatabaseOutput, body, allocator);
}
