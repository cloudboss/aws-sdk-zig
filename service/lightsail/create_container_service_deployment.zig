...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContainerServiceDeploymentInput, options: Options) !CreateContainerServiceDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContainerServiceDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.CreateContainerServiceDeployment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContainerServiceDeploymentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateContainerServiceDeploymentOutput, body, alloc);
}
