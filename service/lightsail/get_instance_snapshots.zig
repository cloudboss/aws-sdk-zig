...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInstanceSnapshotsInput, options: Options) !GetInstanceSnapshotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInstanceSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.GetInstanceSnapshots");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInstanceSnapshotsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInstanceSnapshotsOutput, body, alloc);
}
