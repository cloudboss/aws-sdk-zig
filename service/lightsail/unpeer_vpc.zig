...
fn serializeRequest(alloc: std.mem.Allocator, input: UnpeerVpcInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.UnpeerVpc");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UnpeerVpcOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UnpeerVpcOutput, body, alloc);
}
