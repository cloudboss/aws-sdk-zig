...
pub const UnpeerVpcInput = struct {
};

pub const UnpeerVpcOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UnpeerVpcInput, options: CallOptions) !UnpeerVpcOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UnpeerVpcInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.UnpeerVpc");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UnpeerVpcOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UnpeerVpcOutput, body, allocator);
}
