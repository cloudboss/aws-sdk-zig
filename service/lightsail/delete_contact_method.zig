...
pub const DeleteContactMethodInput = struct {
...
pub const DeleteContactMethodOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteContactMethodInput, options: CallOptions) !DeleteContactMethodOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteContactMethodInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.DeleteContactMethod");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteContactMethodOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteContactMethodOutput, body, allocator);
}
