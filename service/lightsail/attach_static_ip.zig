...
pub const AttachStaticIpInput = struct {
...
pub const AttachStaticIpOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachStaticIpInput, options: CallOptions) !AttachStaticIpOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AttachStaticIpInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.AttachStaticIp");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AttachStaticIpOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AttachStaticIpOutput, body, allocator);
}
