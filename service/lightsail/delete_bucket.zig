...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBucketInput, options: Options) !DeleteBucketOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBucketInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.DeleteBucket");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBucketOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteBucketOutput, body, alloc);
}
