...
pub const GetInstancesInput = struct {
...
pub const GetInstancesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInstancesInput, options: CallOptions) !GetInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetInstances");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInstancesOutput, body, allocator);
}
