...
pub const RebootInstanceInput = struct {
...
pub const RebootInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebootInstanceInput, options: CallOptions) !RebootInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RebootInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.RebootInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RebootInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RebootInstanceOutput, body, allocator);
}
