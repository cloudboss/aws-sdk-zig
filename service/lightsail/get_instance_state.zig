...
pub const GetInstanceStateInput = struct {
...
pub const GetInstanceStateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInstanceStateInput, options: CallOptions) !GetInstanceStateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetInstanceStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetInstanceState");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetInstanceStateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInstanceStateOutput, body, allocator);
}
