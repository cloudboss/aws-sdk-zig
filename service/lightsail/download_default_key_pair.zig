...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DownloadDefaultKeyPairInput, options: Options) !DownloadDefaultKeyPairOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DownloadDefaultKeyPairInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.DownloadDefaultKeyPair");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DownloadDefaultKeyPairOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DownloadDefaultKeyPairOutput, body, alloc);
}
