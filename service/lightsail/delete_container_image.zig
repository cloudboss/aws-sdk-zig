...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteContainerImageInput, options: Options) !DeleteContainerImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteContainerImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.DeleteContainerImage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteContainerImageOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
