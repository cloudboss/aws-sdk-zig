...
pub const EnableAddOnInput = struct {
...
pub const EnableAddOnOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableAddOnInput, options: CallOptions) !EnableAddOnOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableAddOnInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.EnableAddOn");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableAddOnOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(EnableAddOnOutput, body, allocator);
}
