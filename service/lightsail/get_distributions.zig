...
pub const GetDistributionsInput = struct {
...
pub const GetDistributionsOutput = struct {
    /// An array of objects that describe your distributions.
    distributions: ?[]const LightsailDistribution = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDistributionsInput, options: CallOptions) !GetDistributionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDistributionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetDistributions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDistributionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDistributionsOutput, body, allocator);
}
