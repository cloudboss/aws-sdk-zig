...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLoadBalancerTlsCertificateInput, options: Options) !CreateLoadBalancerTlsCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLoadBalancerTlsCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.CreateLoadBalancerTlsCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLoadBalancerTlsCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLoadBalancerTlsCertificateOutput, body, alloc);
}
