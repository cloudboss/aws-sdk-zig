...
pub const StopRelationalDatabaseInput = struct {
...
pub const StopRelationalDatabaseOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopRelationalDatabaseInput, options: CallOptions) !StopRelationalDatabaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopRelationalDatabaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.StopRelationalDatabase");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopRelationalDatabaseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopRelationalDatabaseOutput, body, allocator);
}
