...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDiskFromSnapshotInput, options: Options) !CreateDiskFromSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDiskFromSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.CreateDiskFromSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDiskFromSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDiskFromSnapshotOutput, body, alloc);
}
