...
pub const DeleteDiskSnapshotInput = struct {
...
pub const DeleteDiskSnapshotOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDiskSnapshotInput, options: CallOptions) !DeleteDiskSnapshotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDiskSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.DeleteDiskSnapshot");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDiskSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteDiskSnapshotOutput, body, allocator);
}
