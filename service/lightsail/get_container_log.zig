...
const ContainerServiceLogEvent = @import("container_service_log_event.zig").ContainerServiceLogEvent;

pub const GetContainerLogInput = struct {
...
pub const GetContainerLogOutput = struct {
    /// An array of objects that describe the log events of a container.
    log_events: ?[]const ContainerServiceLogEvent = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContainerLogInput, options: CallOptions) !GetContainerLogOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetContainerLogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetContainerLog");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetContainerLogOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetContainerLogOutput, body, allocator);
}
