...
pub const GetKeyPairsInput = struct {
    ...
    pub const json_field_names = .{
        .include_default_key_pair = "includeDefaultKeyPair",
        .page_token = "pageToken",
    };
};

pub const GetKeyPairsOutput = struct {
    /// An array of key-value pairs containing information about the key pairs.
    key_pairs: ?[]const KeyPair = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetKeyPairsInput, options: CallOptions) !GetKeyPairsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetKeyPairsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetKeyPairs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetKeyPairsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetKeyPairsOutput, body, allocator);
}
