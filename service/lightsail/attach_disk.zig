...
pub const AttachDiskInput = struct {
...
pub const AttachDiskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachDiskInput, options: CallOptions) !AttachDiskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AttachDiskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.AttachDisk");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AttachDiskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AttachDiskOutput, body, allocator);
}
