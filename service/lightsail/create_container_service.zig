...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateContainerServiceInput, options: Options) !CreateContainerServiceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContainerServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.CreateContainerService");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContainerServiceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateContainerServiceOutput, body, alloc);
}
