...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInstancesInput, options: Options) !CreateInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.CreateInstances");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateInstancesOutput, body, alloc);
}
