...
pub const DisableAddOnInput = struct {
...
pub const DisableAddOnOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableAddOnInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.DisableAddOn");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableAddOnOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisableAddOnOutput, body, allocator);
}
