...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PeerVpcInput, options: CallOptions) !PeerVpcOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PeerVpcInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.PeerVpc");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PeerVpcOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PeerVpcOutput, body, allocator);
}
