...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAlarmsInput, options: Options) !GetAlarmsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAlarmsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.GetAlarms");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAlarmsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAlarmsOutput, body, alloc);
}
