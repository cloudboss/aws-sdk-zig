...
pub const GetOperationsInput = struct {
...
pub const GetOperationsOutput = struct {
    /// The token to advance to the next page of results from your request.
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOperationsInput, options: CallOptions) !GetOperationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetOperations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetOperationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOperationsOutput, body, allocator);
}
