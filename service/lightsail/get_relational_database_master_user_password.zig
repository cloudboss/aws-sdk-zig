...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRelationalDatabaseMasterUserPasswordInput, options: Options) !GetRelationalDatabaseMasterUserPasswordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRelationalDatabaseMasterUserPasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.GetRelationalDatabaseMasterUserPassword");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRelationalDatabaseMasterUserPasswordOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRelationalDatabaseMasterUserPasswordOutput, body, alloc);
}
