...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCertificatesInput, options: Options) !GetCertificatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.GetCertificates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCertificatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCertificatesOutput, body, alloc);
}
