...
pub const StopInstanceInput = struct {
...
pub const StopInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopInstanceInput, options: CallOptions) !StopInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.StopInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopInstanceOutput, body, allocator);
}
