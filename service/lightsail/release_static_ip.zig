...
pub const ReleaseStaticIpInput = struct {
...
pub const ReleaseStaticIpOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReleaseStaticIpInput, options: CallOptions) !ReleaseStaticIpOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ReleaseStaticIpInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.ReleaseStaticIp");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ReleaseStaticIpOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ReleaseStaticIpOutput, body, allocator);
}
