...
pub const GetContainerServicePowersOutput = struct {
    ...
    /// container
    /// service.
    powers: ?[]const ContainerServicePower = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContainerServicePowersInput, options: CallOptions) !GetContainerServicePowersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetContainerServicePowersInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.GetContainerServicePowers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetContainerServicePowersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetContainerServicePowersOutput, body, allocator);
}
