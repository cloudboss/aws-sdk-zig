...
pub const UpdateDistributionBundleInput = struct {
...
pub const UpdateDistributionBundleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDistributionBundleInput, options: CallOptions) !UpdateDistributionBundleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDistributionBundleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Lightsail_20161128.UpdateDistributionBundle");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDistributionBundleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateDistributionBundleOutput, body, allocator);
}
