...
pub const PortInfo = struct {
    ...
    /// `fromPort` (ICMP type), and `-1` as the `toPort` (ICMP
    /// code), to enable ICMP Ping. For more information, see [Control
    /// Messages](https://en.wikipedia.org/wiki/Internet_Control_Message_Protocol#Control_messages) on *Wikipedia*.
    ///
    /// * ICMPv6 - The ICMP type for IPv6 addresses. For example, specify `128` as
    /// the `fromPort` (ICMPv6 type), and `0` as `toPort` (ICMPv6
    /// code). For more information, see [Internet
    /// Control Message Protocol for
    /// IPv6](https://en.wikipedia.org/wiki/Internet_Control_Message_Protocol_for_IPv6).
    from_port: i32 = 0,

    /// The IPv6 address, or range of IPv6 addresses (in CIDR notation) that are
    /// allowed to
    /// connect to an instance through the ports, and the protocol. Only devices
    ...
    ///   and
    /// error-checked delivery of streamed data between applications running on
    /// hosts
    /// communicating by an IP network. If you have an application that doesn't
    /// require reliable
    ...
    /// Prior
    /// communications are not required to set up transmission channels or data
    /// paths.
    /// Applications that don't require reliable data stream service can use UDP,
    /// which provides a
    ...
    /// communicating with
    /// an instance. For example, an error is indicated when an instance could not
    /// be reached.
    ...
    /// Messages](https://en.wikipedia.org/wiki/Internet_Control_Message_Protocol#Control_messages) on *Wikipedia*.
    ///
    /// * ICMPv6 - The ICMP code for IPv6 addresses. For example, specify `128` as
    /// the `fromPort` (ICMPv6 type), and `0` as `toPort` (ICMPv6
    /// code). For more information, see [Internet
    /// Control Message Protocol for
    /// IPv6](https://en.wikipedia.org/wiki/Internet_Control_Message_Protocol_for_IPv6).
