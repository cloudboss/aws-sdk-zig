const AccessDirection = @import("access_direction.zig").AccessDirection;
const PortAccessType = @import("port_access_type.zig").PortAccessType;
const NetworkProtocol = @import("network_protocol.zig").NetworkProtocol;

/// Describes information about ports for an Amazon Lightsail instance.
pub const InstancePortInfo = struct {
    /// The access direction (`inbound` or `outbound`).
    ///
    /// Lightsail currently supports only `inbound` access direction.
    access_direction: ?AccessDirection = null,
    ...
    access_from: ?[]const u8 = null,

    /// The type of access (`Public` or `Private`).
    access_type: ?PortAccessType = null,
