...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOperationInput, options: Options) !GetOperationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOperationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Lightsail_20161128.GetOperation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOperationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOperationOutput, body, alloc);
}
