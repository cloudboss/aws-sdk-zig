const BatchResourceRequirementType = @import("batch_resource_requirement_type.zig").BatchResourceRequirementType;

/// The type and amount of a resource to assign to a container. The supported
/// resources
/// include `GPU`, `MEMORY`, and `VCPU`.
pub const BatchResourceRequirement = struct {
    ...
    /// API](https://docs.docker.com/engine/api/v1.23/) and
    ...
    /// management](https://docs.aws.amazon.com/batch/latest/userguide/memory-management.html) in the *Batch User
    /// Guide*.
    ///
    ...
    /// at
    /// least one vCPU. This is required but can be specified in several places; it
    /// must
    /// be specified for each node at least once.
    ///
    /// The default for the Fargate On-Demand vCPU resource count quota
    /// is 6 vCPUs. For more information about Fargate quotas, see [Fargate
    /// quotas](https://docs.aws.amazon.com/general/latest/gr/ecs-service.html#service-quotas-fargate) in the *Amazon Web Services
    /// General Reference*.
    ///
    /// For jobs that are running on Fargate resources, then
