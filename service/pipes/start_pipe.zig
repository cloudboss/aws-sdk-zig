...
pub const StartPipeInput = struct {
...
pub const StartPipeOutput = struct {
    /// The ARN of the pipe.
    arn: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartPipeInput, options: CallOptions) !StartPipeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartPipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartPipeOutput {
    var result: StartPipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartPipeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
