...
fn serializeRequest(alloc: std.mem.Allocator, input: StartPipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartPipeOutput {
    var result: StartPipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartPipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
