...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePipeInput, options: Options) !DescribePipeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePipeOutput {
    var result: DescribePipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
