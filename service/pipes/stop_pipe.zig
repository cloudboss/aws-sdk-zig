...
pub const StopPipeInput = struct {
...
pub const StopPipeOutput = struct {
    /// The ARN of the pipe.
    arn: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopPipeInput, options: Options) !StopPipeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopPipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopPipeOutput {
    var result: StopPipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopPipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
