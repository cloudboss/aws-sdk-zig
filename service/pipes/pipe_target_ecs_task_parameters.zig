...
const EcsTaskOverride = @import("ecs_task_override.zig").EcsTaskOverride;
...
pub const PipeTargetEcsTaskParameters = struct {
    ...
    /// that you
    /// specify here must match one of the launch type (compatibilities) of the
    /// target task. The
    /// `FARGATE` value is supported only in the Regions where Fargate with Amazon
    /// ECS is supported. For more information, see
    /// [Fargate on Amazon
    /// ECS](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/AWS-Fargate.html) in the *Amazon Elastic Container Service Developer Guide*.
    ...
    /// task. If no
    /// value is specified, the tags are not propagated. Tags can only be propagated
    /// to the task
    ...
    /// them. Each
    /// tag consists of a key and an optional value, both of which you define. To
    /// learn more, see
