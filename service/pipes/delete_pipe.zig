...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePipeOutput {
    var result: DeletePipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
