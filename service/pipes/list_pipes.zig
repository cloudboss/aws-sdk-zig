...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPipesInput, options: CallOptions) !ListPipesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPipesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.current_state) |v| {
        ...
        try query_buf.appendSlice(allocator, "CurrentState=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v.wireName());
        query_has_prev = true;
    }
    if (input.desired_state) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "DesiredState=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v.wireName());
        query_has_prev = true;
    }
    if (input.limit) |v| {
    ...
    if (input.source_prefix) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "SourcePrefix=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
    if (input.target_prefix) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "TargetPrefix=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPipesOutput {
    var result: ListPipesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPipesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
