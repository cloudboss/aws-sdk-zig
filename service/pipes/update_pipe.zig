...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePipeOutput {
    var result: UpdatePipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
