...
const UpdatePipeSourceParameters = @import("update_pipe_source_parameters.zig").UpdatePipeSourceParameters;
...
pub const UpdatePipeInput = struct {
    ...
    ///
    /// To update a pipe that is using the default Amazon Web Services owned key to
    /// use a customer managed key instead, or update a pipe that is using a
    /// customer managed key to use a
    ...
    source_parameters: ?UpdatePipeSourceParameters = null,
...
pub const UpdatePipeOutput = struct {
    /// The ARN of the pipe.
    arn: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePipeInput, options: CallOptions) !UpdatePipeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePipeOutput {
    var result: UpdatePipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePipeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
