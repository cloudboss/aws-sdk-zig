...
/// [CapacityProviderStrategyItem](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_CapacityProviderStrategyItem.html) in the Amazon ECS API Reference.
pub const CapacityProviderStrategyItem = struct {
    /// The base value designates how many tasks, at a minimum, to run on the
    /// specified capacity
    /// provider. Only one capacity provider in a capacity provider strategy can
    /// have a base
    ...
    /// tasks
    /// launched that should use the specified capacity provider. The weight value
    /// is taken into
