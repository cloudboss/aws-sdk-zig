...
const PutAccessPointConfigurationForObjectLambdaOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccessPointConfigurationForObjectLambdaInput, options: Options) !PutAccessPointConfigurationForObjectLambdaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccessPointConfigurationForObjectLambdaInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<PutAccessPointConfigurationForObjectLambdaRequest>");
    try body_buf.appendSlice(alloc, "<Configuration>");
    try serde.serializeObjectLambdaConfiguration(alloc, &body_buf, input.configuration);
    try body_buf.appendSlice(alloc, "</Configuration>");
    try body_buf.appendSlice(alloc, "</PutAccessPointConfigurationForObjectLambdaRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccessPointConfigurationForObjectLambdaOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
