...
const PutAccessPointPolicyForObjectLambdaOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccessPointPolicyForObjectLambdaInput, options: Options) !PutAccessPointPolicyForObjectLambdaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccessPointPolicyForObjectLambdaInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v20180820/accesspointforobjectlambda/");
    try path_buf.appendSlice(alloc, input.name);
    try path_buf.appendSlice(alloc, "/policy");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<PutAccessPointPolicyForObjectLambdaRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccessPointPolicyForObjectLambdaOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
