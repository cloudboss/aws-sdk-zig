...
pub const GetJobTaggingInput = struct {
...
pub const GetJobTaggingOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobTaggingInput, options: Options) !GetJobTaggingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetJobTaggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v20180820/jobs/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetJobTaggingOutput {
    var result: GetJobTaggingOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Tags")) {
                    result.tags = try serde.deserializeS3TagSet(&reader, alloc, "member");
                } else {
