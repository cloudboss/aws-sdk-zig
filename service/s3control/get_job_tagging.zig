...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobTaggingInput, options: CallOptions) !GetJobTaggingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetJobTaggingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetJobTaggingOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Tags")) {
                    result.tags = try serde.deserializeS3TagSet(allocator, &reader, "member");
                } else {
