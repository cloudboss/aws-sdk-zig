...
pub const UpdateJobPriorityInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateJobPriorityInput, options: Options) !UpdateJobPriorityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateJobPriorityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/priority");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "priority=");
    {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateJobPriorityOutput {
