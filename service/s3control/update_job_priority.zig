...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateJobPriorityInput, options: CallOptions) !UpdateJobPriorityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateJobPriorityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/priority");
    const path = try path_buf.toOwnedSlice(allocator);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "priority=");
    {
