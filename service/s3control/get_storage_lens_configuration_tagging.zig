...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStorageLensConfigurationTaggingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStorageLensConfigurationTaggingOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Tags")) {
                    result.tags = try serde.deserializeStorageLensTags(&reader, alloc, "Tag");
                } else {
