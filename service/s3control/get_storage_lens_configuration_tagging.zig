...
pub const GetStorageLensConfigurationTaggingInput = struct {
...
pub const GetStorageLensConfigurationTaggingOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetStorageLensConfigurationTaggingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetStorageLensConfigurationTaggingOutput {
    var result: GetStorageLensConfigurationTaggingOutput = .{};
    _ = status;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Tags")) {
                    result.tags = try serde.deserializeStorageLensTags(allocator, &reader, "Tag");
                } else {
