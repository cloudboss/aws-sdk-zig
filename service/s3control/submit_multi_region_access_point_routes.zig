...
pub const SubmitMultiRegionAccessPointRoutesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SubmitMultiRegionAccessPointRoutesInput, options: Options) !SubmitMultiRegionAccessPointRoutesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SubmitMultiRegionAccessPointRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<SubmitMultiRegionAccessPointRoutesRequest>");
    try body_buf.appendSlice(alloc, "<RouteUpdates>");
    try serde.serializeRouteList(alloc, &body_buf, input.route_updates, "Route");
    try body_buf.appendSlice(alloc, "</RouteUpdates>");
    try body_buf.appendSlice(alloc, "</SubmitMultiRegionAccessPointRoutesRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SubmitMultiRegionAccessPointRoutesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: SubmitMultiRegionAccessPointRoutesOutput = .{};
