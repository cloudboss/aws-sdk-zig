...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutStorageLensConfigurationTaggingInput, options: Options) !PutStorageLensConfigurationTaggingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutStorageLensConfigurationTaggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v20180820/storagelens/");
    try path_buf.appendSlice(alloc, input.config_id);
    try path_buf.appendSlice(alloc, "/tagging");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<PutStorageLensConfigurationTaggingRequest>");
    try body_buf.appendSlice(alloc, "<Tags>");
    try serde.serializeStorageLensTags(alloc, &body_buf, input.tags, "Tag");
    try body_buf.appendSlice(alloc, "</Tags>");
    try body_buf.appendSlice(alloc, "</PutStorageLensConfigurationTaggingRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutStorageLensConfigurationTaggingOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
