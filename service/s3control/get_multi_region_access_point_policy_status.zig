...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMultiRegionAccessPointPolicyStatusInput, options: Options) !GetMultiRegionAccessPointPolicyStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMultiRegionAccessPointPolicyStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMultiRegionAccessPointPolicyStatusOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Established")) {
                    result.established = try serde.deserializePolicyStatus(&reader, alloc);
                } else {
