...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMultiRegionAccessPointPolicyInput, options: CallOptions) !GetMultiRegionAccessPointPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMultiRegionAccessPointPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMultiRegionAccessPointPolicyOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Policy")) {
                    result.policy = try serde.deserializeMultiRegionAccessPointPolicyDocument(allocator, &reader);
                } else {
