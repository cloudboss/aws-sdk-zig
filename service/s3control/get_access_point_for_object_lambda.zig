...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccessPointForObjectLambdaInput, options: Options) !GetAccessPointForObjectLambdaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessPointForObjectLambdaInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessPointForObjectLambdaOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Alias")) {
                    result.alias = try serde.deserializeObjectLambdaAccessPointAlias(&reader, alloc);
                    ...
                    result.public_access_block_configuration = try serde.deserializePublicAccessBlockConfiguration(&reader, alloc);
                } else {
