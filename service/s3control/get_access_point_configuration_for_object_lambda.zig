...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessPointConfigurationForObjectLambdaInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessPointConfigurationForObjectLambdaOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Configuration")) {
                    result.configuration = try serde.deserializeObjectLambdaConfiguration(&reader, alloc);
                } else {
