...
const PutBucketReplicationOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBucketReplicationInput, options: Options) !PutBucketReplicationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutBucketReplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "<ReplicationConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://awss3control.amazonaws.com/doc/2018-08-20/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeReplicationConfiguration(allocator, &body_buf, input.replication_configuration);
    try body_buf.appendSlice(allocator, "</ReplicationConfiguration>");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutBucketReplicationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutBucketReplicationOutput = .{};
