...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessPointsForDirectoryBucketsInput, options: Options) !ListAccessPointsForDirectoryBucketsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessPointsForDirectoryBucketsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.directory_bucket) |v| {
        ...
        try query_buf.appendSlice(alloc, "directoryBucket=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessPointsForDirectoryBucketsOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccessPointList")) {
                    result.access_point_list = try serde.deserializeAccessPointList(&reader, alloc, "AccessPoint");
