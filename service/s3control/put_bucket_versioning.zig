...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBucketVersioningInput, options: Options) !PutBucketVersioningOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBucketVersioningInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/versioning");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<VersioningConfiguration xmlns=" ++ &[_]u8{0x22} ++ "http://awss3control.amazonaws.com/doc/2018-08-20/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeVersioningConfiguration(alloc, &body_buf, input.versioning_configuration);
