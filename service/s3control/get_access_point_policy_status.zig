...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccessPointPolicyStatusInput, options: CallOptions) !GetAccessPointPolicyStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAccessPointPolicyStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAccessPointPolicyStatusOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PolicyStatus")) {
                    result.policy_status = try serde.deserializePolicyStatus(allocator, &reader);
                } else {
