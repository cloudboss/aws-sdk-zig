...
pub const GetAccessPointPolicyStatusInput = struct {
...
pub const GetAccessPointPolicyStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccessPointPolicyStatusInput, options: Options) !GetAccessPointPolicyStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessPointPolicyStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/policyStatus");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessPointPolicyStatusOutput {
    var result: GetAccessPointPolicyStatusOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PolicyStatus")) {
                    result.policy_status = try serde.deserializePolicyStatus(&reader, alloc);
                } else {
