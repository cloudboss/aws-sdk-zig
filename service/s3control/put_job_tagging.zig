...
pub const PutJobTaggingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutJobTaggingInput, options: Options) !PutJobTaggingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutJobTaggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "<PutJobTaggingRequest>");
    try body_buf.appendSlice(allocator, "<Tags>");
    try serde.serializeS3TagSet(allocator, &body_buf, input.tags, "member");
    try body_buf.appendSlice(allocator, "</Tags>");
    try body_buf.appendSlice(allocator, "</PutJobTaggingRequest>");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutJobTaggingOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutJobTaggingOutput = .{};
