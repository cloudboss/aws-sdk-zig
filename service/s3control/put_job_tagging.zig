...
fn serializeRequest(alloc: std.mem.Allocator, input: PutJobTaggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<PutJobTaggingRequest>");
    try body_buf.appendSlice(alloc, "<Tags>");
    try serde.serializeS3TagSet(alloc, &body_buf, input.tags, "member");
    try body_buf.appendSlice(alloc, "</Tags>");
    try body_buf.appendSlice(alloc, "</PutJobTaggingRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutJobTaggingOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
