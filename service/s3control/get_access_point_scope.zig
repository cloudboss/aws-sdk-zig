...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessPointScopeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessPointScopeOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Scope")) {
                    result.scope = try serde.deserializeScope(&reader, alloc);
                } else {
