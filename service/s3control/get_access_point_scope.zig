...
pub const GetAccessPointScopeInput = struct {
...
pub const GetAccessPointScopeOutput = struct {
    /// The contents of the access point scope.
    scope: ?Scope = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccessPointScopeInput, options: CallOptions) !GetAccessPointScopeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAccessPointScopeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAccessPointScopeOutput {
    var result: GetAccessPointScopeOutput = .{};
    _ = status;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Scope")) {
                    result.scope = try serde.deserializeScope(allocator, &reader);
                } else {
