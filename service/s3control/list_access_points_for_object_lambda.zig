...
pub const ListAccessPointsForObjectLambdaInput = struct {
    ...
    /// The response may
    /// contain fewer access points but will never contain more. If there are more
    /// than this number of
    ...
    /// API, this field
    /// contains a continuation token that you can provide in subsequent calls to
    /// this API to
    /// retrieve additional access points.
    next_token: ?[]const u8 = null,
};

pub const ListAccessPointsForObjectLambdaOutput = struct {
    /// If the list has more access points than can be returned in one call to this
    /// API, this field
    /// contains a continuation token that you can provide in subsequent calls to
    /// this API to
    ...
    object_lambda_access_point_list: ?[]const ObjectLambdaAccessPoint = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessPointsForObjectLambdaInput, options: Options) !ListAccessPointsForObjectLambdaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessPointsForObjectLambdaInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessPointsForObjectLambdaOutput {
    var result: ListAccessPointsForObjectLambdaOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                ...
                } else if (std.mem.eql(u8, e.local, "ObjectLambdaAccessPointList")) {
                    result.object_lambda_access_point_list = try serde.deserializeObjectLambdaAccessPointList(&reader, alloc, "ObjectLambdaAccessPoint");
                } else {
