...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccessGrantsInstanceInput, options: Options) !CreateAccessGrantsInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccessGrantsInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<CreateAccessGrantsInstanceRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccessGrantsInstanceOutput {
