...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateJobStatusInput, options: Options) !UpdateJobStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateJobStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "requestedJobStatus=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.requested_job_status));
    query_has_prev = true;
    if (input.status_update_reason) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "statusUpdateReason=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateJobStatusOutput {
