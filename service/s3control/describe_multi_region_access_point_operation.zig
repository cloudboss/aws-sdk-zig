...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMultiRegionAccessPointOperationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v20180820/async-requests/mrap/");
    try path_buf.appendSlice(alloc, input.request_token_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMultiRegionAccessPointOperationOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AsyncOperation")) {
                    result.async_operation = try serde.deserializeAsyncOperation(&reader, alloc);
                } else {
