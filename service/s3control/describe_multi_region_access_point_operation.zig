...
pub const DescribeMultiRegionAccessPointOperationInput = struct {
    ...
    /// request token
    /// is returned as part of the response when you make an asynchronous request.
    /// You provide this
    /// token to query about the status of the asynchronous action.
    request_token_arn: []const u8,
};

pub const DescribeMultiRegionAccessPointOperationOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMultiRegionAccessPointOperationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/v20180820/async-requests/mrap/");
    try path_buf.appendSlice(allocator, input.request_token_arn);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMultiRegionAccessPointOperationOutput {
    var result: DescribeMultiRegionAccessPointOperationOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AsyncOperation")) {
                    result.async_operation = try serde.deserializeAsyncOperation(allocator, &reader);
                } else {
