...
const PutStorageLensConfigurationOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutStorageLensConfigurationInput, options: Options) !PutStorageLensConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutStorageLensConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<PutStorageLensConfigurationRequest>");
    try body_buf.appendSlice(alloc, "<StorageLensConfiguration>");
    try serde.serializeStorageLensConfiguration(alloc, &body_buf, input.storage_lens_configuration);
    try body_buf.appendSlice(alloc, "</StorageLensConfiguration>");
    if (input.tags) |v| {
        try body_buf.appendSlice(alloc, "<Tags>");
        try serde.serializeStorageLensTags(alloc, &body_buf, v, "Tag");
        try body_buf.appendSlice(alloc, "</Tags>");
    }
    try body_buf.appendSlice(alloc, "</PutStorageLensConfigurationRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutStorageLensConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
