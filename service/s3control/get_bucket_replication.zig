...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBucketReplicationOutput {
    var result: GetBucketReplicationOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ReplicationConfiguration")) {
                    result.replication_configuration = try serde.deserializeReplicationConfiguration(allocator, &reader);
                } else {
