...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStorageLensGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStorageLensGroupsOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    try storage_lens_group_list_list.append(alloc, try serde.deserializeListStorageLensGroupEntry(&reader, alloc));
                } else {
    ...
    result.storage_lens_group_list = if (storage_lens_group_list_list.items.len > 0) try storage_lens_group_list_list.toOwnedSlice(alloc) else null;
    _ = headers;
