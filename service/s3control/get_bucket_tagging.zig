...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBucketTaggingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBucketTaggingOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TagSet")) {
                    result.tag_set = try serde.deserializeS3TagSet(allocator, &reader, "member");
                } else {
