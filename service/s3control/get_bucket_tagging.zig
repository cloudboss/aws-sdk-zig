...
pub const GetBucketTaggingInput = struct {
...
const GetBucketTaggingOutput = @import("get_bucket_tagging_result.zig").GetBucketTaggingResult;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBucketTaggingInput, options: Options) !GetBucketTaggingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBucketTaggingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBucketTaggingOutput {
    var result: GetBucketTaggingOutput = .{};
    _ = status;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TagSet")) {
                    result.tag_set = try serde.deserializeS3TagSet(&reader, alloc, "member");
                } else {
