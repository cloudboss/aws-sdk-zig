...
pub const GetMultiRegionAccessPointRoutesInput = struct {
...
pub const GetMultiRegionAccessPointRoutesOutput = struct {
    /// The Multi-Region Access Point ARN.
    mrap: ?[]const u8 = null,
    ...
    routes: ?[]const MultiRegionAccessPointRoute = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMultiRegionAccessPointRoutesInput, options: CallOptions) !GetMultiRegionAccessPointRoutesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMultiRegionAccessPointRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.mrap);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMultiRegionAccessPointRoutesOutput {
    var result: GetMultiRegionAccessPointRoutesOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Mrap")) {
                    result.mrap = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "Routes")) {
                    result.routes = try serde.deserializeRouteList(allocator, &reader, "Route");
                } else {
