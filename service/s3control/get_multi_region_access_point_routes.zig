...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMultiRegionAccessPointRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.mrap);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMultiRegionAccessPointRoutesOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Mrap")) {
                    result.mrap = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "Routes")) {
                    result.routes = try serde.deserializeRouteList(&reader, alloc, "Route");
                } else {
