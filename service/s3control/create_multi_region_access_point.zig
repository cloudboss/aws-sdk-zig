...
const CreateMultiRegionAccessPointInput = @import("create_multi_region_access_point_request.zig").CreateMultiRegionAccessPointRequest;

pub const CreateMultiRegionAccessPointOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMultiRegionAccessPointInput, options: Options) !CreateMultiRegionAccessPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMultiRegionAccessPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<CreateMultiRegionAccessPointRequest>");
    try body_buf.appendSlice(alloc, "<ClientToken>");
    try aws.xml.appendXmlEscaped(alloc, &body_buf, input.client_token);
    try body_buf.appendSlice(alloc, "</ClientToken>");
    try body_buf.appendSlice(alloc, "<Details>");
    try serde.serializeCreateMultiRegionAccessPointInput(alloc, &body_buf, input.details);
    try body_buf.appendSlice(alloc, "</Details>");
    try body_buf.appendSlice(alloc, "</CreateMultiRegionAccessPointRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMultiRegionAccessPointOutput {
    var result: CreateMultiRegionAccessPointOutput = .{};
    _ = status;
