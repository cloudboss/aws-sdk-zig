...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMultiRegionAccessPointInput, options: Options) !GetMultiRegionAccessPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMultiRegionAccessPointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMultiRegionAccessPointOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccessPoint")) {
                    result.access_point = try serde.deserializeMultiRegionAccessPointReport(&reader, alloc);
                } else {
