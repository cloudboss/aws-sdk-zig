...
pub const ListAccessPointsInput = struct {
    ...
    /// the specified
    /// bucket has more than this number of access points, then the response will
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessPointsInput, options: Options) !ListAccessPointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessPointsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v20180820/accesspoint";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.bucket) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "bucket=");
    ...
    if (input.data_source_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "dataSourceType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessPointsOutput {
    var result: ListAccessPointsOutput = .{};
    _ = status;
