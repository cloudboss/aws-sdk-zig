...
const DeleteMultiRegionAccessPointInput = @import("delete_multi_region_access_point_request.zig").DeleteMultiRegionAccessPointRequest;
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMultiRegionAccessPointInput, options: Options) !DeleteMultiRegionAccessPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMultiRegionAccessPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<DeleteMultiRegionAccessPointRequest>");
    try body_buf.appendSlice(alloc, "<ClientToken>");
    try aws.xml.appendXmlEscaped(alloc, &body_buf, input.client_token);
    try body_buf.appendSlice(alloc, "</ClientToken>");
    try body_buf.appendSlice(alloc, "<Details>");
    try serde.serializeDeleteMultiRegionAccessPointInput(alloc, &body_buf, input.details);
    try body_buf.appendSlice(alloc, "</Details>");
    try body_buf.appendSlice(alloc, "</DeleteMultiRegionAccessPointRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMultiRegionAccessPointOutput {
