...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessGrantsInstancesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessGrantsInstancesOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccessGrantsInstancesList")) {
                    result.access_grants_instances_list = try serde.deserializeAccessGrantsInstancesList(&reader, alloc, "AccessGrantsInstance");
