...
pub const ListAccessGrantsInstancesInput = struct {
...
pub const ListAccessGrantsInstancesOutput = struct {
    /// A container for a list of S3 Access Grants instances.
    access_grants_instances_list: ?[]const ListAccessGrantsInstanceEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessGrantsInstancesInput, options: CallOptions) !ListAccessGrantsInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAccessGrantsInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v20180820/accessgrantsinstances";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAccessGrantsInstancesOutput {
    var result: ListAccessGrantsInstancesOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccessGrantsInstancesList")) {
                    result.access_grants_instances_list = try serde.deserializeAccessGrantsInstancesList(allocator, &reader, "AccessGrantsInstance");
