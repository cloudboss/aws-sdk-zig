...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccessPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<CreateAccessPointRequest>");
    try body_buf.appendSlice(alloc, "<Bucket>");
    try aws.xml.appendXmlEscaped(alloc, &body_buf, input.bucket);
    try body_buf.appendSlice(alloc, "</Bucket>");
    if (input.bucket_account_id) |v| {
        try body_buf.appendSlice(alloc, "<BucketAccountId>");
    ...
    if (input.public_access_block_configuration) |v| {
        ...
        try serde.serializePublicAccessBlockConfiguration(alloc, &body_buf, v);
        try body_buf.appendSlice(alloc, "</PublicAccessBlockConfiguration>");
    }
    if (input.scope) |v| {
        try body_buf.appendSlice(alloc, "<Scope>");
        try serde.serializeScope(alloc, &body_buf, v);
        try body_buf.appendSlice(alloc, "</Scope>");
    }
    if (input.tags) |v| {
    ...
    if (input.vpc_configuration) |v| {
        ...
        try serde.serializeVpcConfiguration(alloc, &body_buf, v);
        try body_buf.appendSlice(alloc, "</VpcConfiguration>");
    }
    try body_buf.appendSlice(alloc, "</CreateAccessPointRequest>");
