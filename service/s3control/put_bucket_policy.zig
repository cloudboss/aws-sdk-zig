...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBucketPolicyInput, options: Options) !PutBucketPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBucketPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<PutBucketPolicyRequest>");
    ...
    if (input.confirm_remove_self_bucket_access) |v| {
        try request.headers.put(alloc, "x-amz-confirm-remove-self-bucket-access", if (v) "true" else "false");
    }
