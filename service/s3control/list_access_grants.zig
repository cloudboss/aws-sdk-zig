...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessGrantsInput, options: Options) !ListAccessGrantsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessGrantsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.application_arn) |v| {
        ...
        try query_buf.appendSlice(alloc, "application_arn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.grantee_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "granteeidentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.grantee_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "granteetype=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.grant_scope) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "grantscope=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessGrantsOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccessGrantsList")) {
                    result.access_grants_list = try serde.deserializeAccessGrantsList(&reader, alloc, "AccessGrant");
