...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessGrantsInstanceForPrefixInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "s3prefix=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.s3_prefix);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessGrantsInstanceForPrefixOutput {
