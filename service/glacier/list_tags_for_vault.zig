...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTagsForVaultInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTagsForVaultOutput {
    var result: ListTagsForVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTagsForVaultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
