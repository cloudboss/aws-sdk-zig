...
pub const ListTagsForVaultInput = struct {
...
pub const ListTagsForVaultOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTagsForVaultInput, options: CallOptions) !ListTagsForVaultOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTagsForVaultInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTagsForVaultOutput {
    var result: ListTagsForVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTagsForVaultOutput, body, allocator);
    }
    _ = status;
    _ = headers;
