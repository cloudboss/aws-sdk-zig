...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UploadMultipartPartInput, options: Options) !UploadMultipartPartOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UploadMultipartPartInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.range) |v| {
        try request.headers.put(alloc, "Content-Range", v);
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UploadMultipartPartOutput {
