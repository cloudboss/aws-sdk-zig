...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMultipartUploadsInput, options: Options) !ListMultipartUploadsOutput {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMultipartUploadsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMultipartUploadsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
