...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobOutputInput, options: Options) !GetJobOutputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetJobOutputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/output");
...
fn deserializeStreamingResponse(stream_resp: *aws.http.StreamingResponse, alloc: std.mem.Allocator) !GetJobOutputOutput {
    ...
    if (stream_resp.headers.get("x-amz-archive-description")) |value| {
        result.archive_description = try alloc.dupe(u8, value);
    }
