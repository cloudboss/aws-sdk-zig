...
pub const SetVaultNotificationsInput = struct {
...
pub const SetVaultNotificationsOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetVaultNotificationsInput, options: CallOptions) !SetVaultNotificationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetVaultNotificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    const body: ?[]const u8 = if (input.vault_notification_config) |v| try aws.json.jsonStringify(v, allocator) else null;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetVaultNotificationsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: SetVaultNotificationsOutput = .{};
