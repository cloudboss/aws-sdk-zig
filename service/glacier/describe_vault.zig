...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVaultInput, options: Options) !DescribeVaultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVaultInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVaultOutput {
    var result: DescribeVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeVaultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
