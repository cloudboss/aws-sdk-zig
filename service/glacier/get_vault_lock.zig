...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVaultLockInput, options: Options) !GetVaultLockOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVaultLockInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVaultLockOutput {
    var result: GetVaultLockOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVaultLockOutput, body, alloc);
    }
    _ = status;
    _ = headers;
