...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurchaseProvisionedCapacityInput, options: Options) !PurchaseProvisionedCapacityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseProvisionedCapacityInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PurchaseProvisionedCapacityOutput {
    var result: PurchaseProvisionedCapacityOutput = .{};
    _ = body;
    _ = status;
    if (headers.get("x-amz-capacity-id")) |value| {
        result.capacity_id = try alloc.dupe(u8, value);
    }
