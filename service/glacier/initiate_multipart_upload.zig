...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InitiateMultipartUploadInput, options: Options) !InitiateMultipartUploadOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InitiateMultipartUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.part_size) |v| {
        try request.headers.put(alloc, "x-amz-part-size", v);
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InitiateMultipartUploadOutput {
    ...
    if (headers.get("x-amz-multipart-upload-id")) |value| {
        result.upload_id = try alloc.dupe(u8, value);
    }
