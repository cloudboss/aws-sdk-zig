...
const abort_vault_lock = @import("abort_vault_lock.zig");
const add_tags_to_vault = @import("add_tags_to_vault.zig");
const complete_multipart_upload = @import("complete_multipart_upload.zig");
const complete_vault_lock = @import("complete_vault_lock.zig");
const create_vault = @import("create_vault.zig");
const delete_archive = @import("delete_archive.zig");
const delete_vault = @import("delete_vault.zig");
const delete_vault_access_policy = @import("delete_vault_access_policy.zig");
const delete_vault_notifications = @import("delete_vault_notifications.zig");
const describe_job = @import("describe_job.zig");
const describe_vault = @import("describe_vault.zig");
const get_data_retrieval_policy = @import("get_data_retrieval_policy.zig");
const get_job_output = @import("get_job_output.zig");
const get_vault_access_policy = @import("get_vault_access_policy.zig");
const get_vault_lock = @import("get_vault_lock.zig");
const get_vault_notifications = @import("get_vault_notifications.zig");
const initiate_job = @import("initiate_job.zig");
const initiate_multipart_upload = @import("initiate_multipart_upload.zig");
const initiate_vault_lock = @import("initiate_vault_lock.zig");
...
const list_provisioned_capacity = @import("list_provisioned_capacity.zig");
const list_tags_for_vault = @import("list_tags_for_vault.zig");
const list_vaults = @import("list_vaults.zig");
const purchase_provisioned_capacity = @import("purchase_provisioned_capacity.zig");
const remove_tags_from_vault = @import("remove_tags_from_vault.zig");
const set_data_retrieval_policy = @import("set_data_retrieval_policy.zig");
const set_vault_access_policy = @import("set_vault_access_policy.zig");
...
pub const Client = struct {
    ...
    /// key and
    /// a value. Each vault can have up to 10 tags. If your request would cause the
    /// tag limit for
    ...
    /// [Computing
    /// Checksums](https://docs.aws.amazon.com/amazonglacier/latest/dev/checksum-calculations.html). On the server side, Glacier also constructs the SHA256 tree
    ...
    /// assembling the archive, if missing content ranges are found, Glacier returns
    /// an
    ...
    /// 500 server error,
    /// in which case you can repeat your Complete Multipart Upload request and get
    /// the same
    ...
    /// error. If an invalid lock ID is passed in the request when the vault lock is
    /// in the
    ...
    /// initiate a
    /// retrieval of this archive will fail. Archive retrievals that are in progress
    /// for this
    ...
    /// jobs)](https://docs.aws.amazon.com/amazonglacier/latest/dev/api-initiate-job-post.html) to initiate a new inventory retrieval for a vault. The inventory contains
    ...
    /// disable the notifications and you might still receive some notifications for
    /// a short time
    ...
    /// code/message and
    /// the Amazon SNS topic to notify after Amazon Glacier (Glacier) completes the
    ...
    /// it contains,
    /// and the total size of all the archives in the vault. The number of archives
    /// and their total
    ...
    /// region
    /// specified in the GET request. For more information about data retrieval
    /// policies, see
    ...
    /// range you
    /// specify, Amazon Glacier (Glacier) returns the checksum for the portion of
    /// the data. You can compute the
    ...
    /// Policy
    /// (PUT
    /// access-policy)](https://docs.aws.amazon.com/amazonglacier/latest/dev/api-SetVaultAccessPolicy.html). If there is no access policy set on the vault, the
    ...
    /// multipart
    /// upload resource and returns its ID in the response. The multipart upload ID
    /// is used in
    ...
    /// resource if
    /// you cancel the multipart upload or it may be removed if there is no activity
    /// for a period
    /// of 24 hours.
    ///
    /// An AWS account has full permission to perform all operations (actions).
    ...
    /// to 20 KB
    /// in size. For more information about vault lock policies, see [Amazon Glacier
    /// Access Control with
    /// Vault Lock
    /// Policies](https://docs.aws.amazon.com/amazonglacier/latest/dev/vault-lock-policy.html).
    ///
    ...
    /// and jobs
    /// that have recently finished. The List Job operation returns a list of these
    /// jobs sorted by job initiation
    /// time.
    ///
    /// Amazon Glacier retains recently completed jobs for a period before deleting
    /// them;
    /// however, it eventually removes completed jobs. The output of completed jobs
    /// can be
    ...
    /// If there are no more jobs to list, the `Marker` field is set to `null`. If
    /// there are more jobs to list,
    /// the `Marker` field is set to a non-null value, which you can use to continue
    ...
    /// by
    /// specifying the `limit` parameter in the request. The default limit is 50.
    /// The
    ...
    /// Using the `completed` parameter, you can specify to return only jobs that
    /// were
    ...
    /// operation
    /// returns up to 50 multipart uploads in the response. You should always check
    /// the response
    ...
    /// begins at a specific upload, set the `marker` request parameter to the value
    /// you
    ...
    /// The List Multipart Uploads operation lists all multipart uploads
    ...
    /// specific
    /// multipart upload. You can make this request at any time during an
    ...
    /// to
    /// list, the response `marker` field contains the vault Amazon Resource Name
    /// (ARN)
    ...
    /// vault. For
    /// more information about tags, see [Tagging Amazon Glacier
    /// Resources](https://docs.aws.amazon.com/amazonglacier/latest/dev/tagging.html).
    ...
    /// specified
    /// in the PUT request. You can set one policy per region for an AWS account.
    /// The policy is
    ...
    /// progress before
    /// the policy was enacted. For more information about data retrieval policies,
    /// see [Amazon
    ...
    /// and for a
    /// successful upload, your data is durably persisted. Amazon Glacier returns
    /// the archive ID in
    /// the `x-amz-archive-id` header of the response.
    ///
    /// You must use the archive ID to access your data in Amazon Glacier. After you
    /// upload
    /// an archive, you should save the archive ID returned so that you can retrieve
    /// or delete the
    /// archive later. Besides saving the archive ID, you can also index it and give
    /// it a friendly
    /// name to allow for better searching. You can also use the optional archive
    /// description field
    /// to specify how the archive is referred to in an external index of archives,
    /// such as you
    ...
    pub fn listMultipartUploadsPaginator(self: *Self, params: list_multipart_uploads.ListMultipartUploadsInput) paginator.ListMultipartUploadsPaginator {
        return .{
            .client = self,
            .params = params,
        };
    ...
    pub fn listVaultsPaginator(self: *Self, params: list_vaults.ListVaultsInput) paginator.ListVaultsPaginator {
        return .{
            .client = self,
            .params = params,
        };
    ...
    pub fn waitUntilVaultExists(self: *Self, params: describe_vault.DescribeVaultInput) aws.waiter.WaiterError!void {
        var w = waiters.VaultExistsWaiter{ .client = self, .params = params };
        return w.wait();
    }

    pub fn waitUntilVaultNotExists(self: *Self, params: describe_vault.DescribeVaultInput) aws.waiter.WaiterError!void {
        var w = waiters.VaultNotExistsWaiter{ .client = self, .params = params };
        return w.wait();
    }
};
