...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InitiateVaultLockInput, options: Options) !InitiateVaultLockOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InitiateVaultLockInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InitiateVaultLockOutput {
    var result: InitiateVaultLockOutput = .{};
    _ = body;
    _ = status;
    if (headers.get("x-amz-lock-id")) |value| {
        result.lock_id = try alloc.dupe(u8, value);
    }
