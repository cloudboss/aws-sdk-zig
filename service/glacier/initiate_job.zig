...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InitiateJobInput, options: Options) !InitiateJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InitiateJobInput, config: *aws.Config) !aws.http.Request {
    ...
    const body: ?[]const u8 = if (input.job_parameters) |v| try aws.json.jsonStringify(v, alloc) else null;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InitiateJobOutput {
    var result: InitiateJobOutput = .{};
    _ = body;
    _ = status;
    if (headers.get("x-amz-job-id")) |value| {
        result.job_id = try alloc.dupe(u8, value);
    }
    if (headers.get("x-amz-job-output-path")) |value| {
        result.job_output_path = try alloc.dupe(u8, value);
    }
