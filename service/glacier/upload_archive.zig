...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UploadArchiveInput, options: Options) !UploadArchiveOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UploadArchiveInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.archive_description) |v| {
        try request.headers.put(alloc, "x-amz-archive-description", v);
    }
    if (input.checksum) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UploadArchiveOutput {
