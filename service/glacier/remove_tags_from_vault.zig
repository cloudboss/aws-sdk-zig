...
const RemoveTagsFromVaultOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveTagsFromVaultInput, options: Options) !RemoveTagsFromVaultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveTagsFromVaultInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/tags");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    try query_buf.appendSlice(alloc, "operation=remove");
    query_has_prev = true;
    ...
    if (input.tag_keys) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"TagKeys\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveTagsFromVaultOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
