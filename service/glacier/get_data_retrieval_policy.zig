...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataRetrievalPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataRetrievalPolicyOutput {
    var result: GetDataRetrievalPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataRetrievalPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
