...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProvisionedCapacityInput, options: Options) !ListProvisionedCapacityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProvisionedCapacityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/provisioned-capacity");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProvisionedCapacityOutput {
    var result: ListProvisionedCapacityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProvisionedCapacityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
