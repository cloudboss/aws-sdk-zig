...
const DescribeVaultOutput = @import("describe_vault_output.zig").DescribeVaultOutput;

pub const ListVaultsInput = struct {
    ...
    /// number of
    /// vaults returned might be fewer than the specified limit, but the number of
    /// returned vaults
...
pub const ListVaultsOutput = struct {
    ...
    /// List of vaults.
    vault_list: ?[]const DescribeVaultOutput = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVaultsInput, options: CallOptions) !ListVaultsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "glacier");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVaultsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVaultsOutput {
    var result: ListVaultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVaultsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
