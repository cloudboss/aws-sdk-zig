...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVaultsInput, options: Options) !ListVaultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVaultsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVaultsOutput {
    var result: ListVaultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVaultsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
