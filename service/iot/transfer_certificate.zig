...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TransferCertificateInput, options: Options) !TransferCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TransferCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/transfer-certificate/");
    try path_buf.appendSlice(alloc, input.certificate_id);
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "targetAwsAccount=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.target_aws_account);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TransferCertificateOutput {
    var result: TransferCertificateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TransferCertificateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
