...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDefaultAuthorizerInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDefaultAuthorizerOutput {
    var result: DescribeDefaultAuthorizerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDefaultAuthorizerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
