...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDefaultAuthorizerInput, options: CallOptions) !DescribeDefaultAuthorizerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDefaultAuthorizerInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDefaultAuthorizerOutput {
    var result: DescribeDefaultAuthorizerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDefaultAuthorizerOutput, body, allocator);
    }
    _ = status;
    _ = headers;
