...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMitigationActionInput, options: Options) !DescribeMitigationActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMitigationActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMitigationActionOutput {
    var result: DescribeMitigationActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeMitigationActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
