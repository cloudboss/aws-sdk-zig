...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDimensionInput, options: Options) !DescribeDimensionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDimensionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDimensionOutput {
    var result: DescribeDimensionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDimensionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
