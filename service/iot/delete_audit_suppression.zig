...
pub const DeleteAuditSuppressionInput = struct {
    check_name: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAuditSuppressionInput, options: Options) !DeleteAuditSuppressionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAuditSuppressionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/audit/suppressions/delete";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAuditSuppressionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteAuditSuppressionOutput = .{};
