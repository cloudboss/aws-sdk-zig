...
pub const ListBillingGroupsOutput = struct {
    ...
    billing_groups: ?[]const GroupNameAndArn = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBillingGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.name_prefix_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "namePrefixFilter=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBillingGroupsOutput {
    var result: ListBillingGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBillingGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
