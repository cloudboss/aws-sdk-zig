...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDomainConfigurationInput, options: CallOptions) !CreateDomainConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDomainConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.server_certificate_arns) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"serverCertificateArns\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    ...
    if (input.service_type) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"serviceType\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
    ...
    if (input.validation_certificate_arn) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"validationCertificateArn\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateDomainConfigurationOutput {
    var result: CreateDomainConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDomainConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
