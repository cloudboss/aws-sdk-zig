...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProvisioningTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.default_version_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"defaultVersionId\":");
    ...
    if (input.remove_pre_provisioning_hook) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"removePreProvisioningHook\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProvisioningTemplateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
