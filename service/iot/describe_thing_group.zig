...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeThingGroupInput, options: Options) !DescribeThingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeThingGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeThingGroupOutput {
    var result: DescribeThingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeThingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
