...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAuthorizerInput, options: Options) !DescribeAuthorizerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAuthorizerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAuthorizerOutput {
    var result: DescribeAuthorizerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAuthorizerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
