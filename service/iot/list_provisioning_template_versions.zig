...
const ProvisioningTemplateVersionSummary = @import("provisioning_template_version_summary.zig").ProvisioningTemplateVersionSummary;

pub const ListProvisioningTemplateVersionsInput = struct {
...
pub const ListProvisioningTemplateVersionsOutput = struct {
    /// A token to retrieve the next set of results.
    next_token: ?[]const u8 = null,

    /// The list of provisioning template versions.
    versions: ?[]const ProvisioningTemplateVersionSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProvisioningTemplateVersionsInput, options: Options) !ListProvisioningTemplateVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProvisioningTemplateVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/provisioning-templates/");
    try path_buf.appendSlice(alloc, input.template_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProvisioningTemplateVersionsOutput {
    var result: ListProvisioningTemplateVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProvisioningTemplateVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
