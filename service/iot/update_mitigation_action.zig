...
pub const UpdateMitigationActionInput = struct {
...
pub const UpdateMitigationActionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMitigationActionInput, options: Options) !UpdateMitigationActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMitigationActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/mitigationactions/actions/");
    try path_buf.appendSlice(alloc, input.action_name);
    ...
    if (input.action_params) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"actionParams\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMitigationActionOutput {
    var result: UpdateMitigationActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMitigationActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
