...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMitigationActionInput, options: CallOptions) !UpdateMitigationActionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateMitigationActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateMitigationActionOutput {
    var result: UpdateMitigationActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMitigationActionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
