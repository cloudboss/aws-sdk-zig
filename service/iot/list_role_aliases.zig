...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoleAliasesInput, options: CallOptions) !ListRoleAliasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRoleAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRoleAliasesOutput {
    var result: ListRoleAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoleAliasesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
