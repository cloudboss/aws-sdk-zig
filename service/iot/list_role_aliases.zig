...
pub const ListRoleAliasesOutput = struct {
    /// A marker used to get the next set of results.
    next_marker: ?[]const u8 = null,

    /// The role aliases.
    role_aliases: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoleAliasesInput, options: Options) !ListRoleAliasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoleAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoleAliasesOutput {
    var result: ListRoleAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoleAliasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
