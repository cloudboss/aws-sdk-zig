...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartThingRegistrationTaskInput, options: Options) !StartThingRegistrationTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartThingRegistrationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"inputFileBucket\":");
    try aws.json.writeValue(@TypeOf(input.input_file_bucket), input.input_file_bucket, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"inputFileKey\":");
    try aws.json.writeValue(@TypeOf(input.input_file_key), input.input_file_key, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.template_body), input.template_body, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartThingRegistrationTaskOutput {
    var result: StartThingRegistrationTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartThingRegistrationTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
