...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeJobExecutionInput, options: Options) !DescribeJobExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeJobExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.execution_number) |v| {
        ...
        try query_buf.appendSlice(alloc, "executionNumber=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeJobExecutionOutput {
    var result: DescribeJobExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeJobExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
