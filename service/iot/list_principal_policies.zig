...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPrincipalPoliciesInput, options: Options) !ListPrincipalPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPrincipalPoliciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPrincipalPoliciesOutput {
    var result: ListPrincipalPoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrincipalPoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
