...
pub const UpdateCustomMetricInput = struct {
    /// Field
...
pub const UpdateCustomMetricOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCustomMetricInput, options: Options) !UpdateCustomMetricOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCustomMetricInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCustomMetricOutput {
    var result: UpdateCustomMetricOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCustomMetricOutput, body, alloc);
    }
    _ = status;
    _ = headers;
