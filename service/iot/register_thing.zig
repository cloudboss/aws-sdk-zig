...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterThingInput, options: Options) !RegisterThingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterThingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterThingOutput {
    var result: RegisterThingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterThingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
