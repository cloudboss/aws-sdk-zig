...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePolicyVersionInput, options: Options) !CreatePolicyVersionOutput {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePolicyVersionOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePolicyVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
