...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCommandExecutionInput, options: Options) !GetCommandExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCommandExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/command-executions/");
    try path_buf.appendSlice(alloc, input.execution_id);
    ...
    if (input.include_result) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeResult=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "targetArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.target_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCommandExecutionOutput {
    var result: GetCommandExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCommandExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
