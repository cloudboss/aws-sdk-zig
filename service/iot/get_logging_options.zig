...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLoggingOptionsInput, options: Options) !GetLoggingOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLoggingOptionsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLoggingOptionsOutput {
    var result: GetLoggingOptionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLoggingOptionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
