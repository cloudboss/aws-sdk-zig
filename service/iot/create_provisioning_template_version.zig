...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProvisioningTemplateVersionInput, options: Options) !CreateProvisioningTemplateVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProvisioningTemplateVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.set_as_default) |v| {
        ...
        try query_buf.appendSlice(alloc, "setAsDefault=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProvisioningTemplateVersionOutput {
    var result: CreateProvisioningTemplateVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProvisioningTemplateVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
