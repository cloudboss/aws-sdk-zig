...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProvisioningTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProvisioningTemplatesOutput {
    var result: ListProvisioningTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProvisioningTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
