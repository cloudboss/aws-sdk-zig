...
const ProvisioningTemplateSummary = @import("provisioning_template_summary.zig").ProvisioningTemplateSummary;

pub const ListProvisioningTemplatesInput = struct {
...
pub const ListProvisioningTemplatesOutput = struct {
    /// A token to retrieve the next set of results.
    next_token: ?[]const u8 = null,

    /// A list of provisioning templates
    templates: ?[]const ProvisioningTemplateSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProvisioningTemplatesInput, options: CallOptions) !ListProvisioningTemplatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProvisioningTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListProvisioningTemplatesOutput {
    var result: ListProvisioningTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProvisioningTemplatesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
