...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStreamInput, options: Options) !UpdateStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/streams/");
    try path_buf.appendSlice(alloc, input.stream_id);
    ...
    if (input.files) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"files\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.role_arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStreamOutput {
    var result: UpdateStreamOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateStreamOutput, body, alloc);
    }
    _ = status;
    _ = headers;
