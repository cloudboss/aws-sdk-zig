...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFleetMetricInput, options: Options) !CreateFleetMetricOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFleetMetricInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/fleet-metric/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFleetMetricOutput {
    var result: CreateFleetMetricOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFleetMetricOutput, body, alloc);
    }
    _ = status;
    _ = headers;
