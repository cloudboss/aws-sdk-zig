...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSbomValidationResultsInput, options: Options) !ListSbomValidationResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSbomValidationResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sbom-validation-results");
    ...
    if (input.validation_result) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "validationResult=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSbomValidationResultsOutput {
    var result: ListSbomValidationResultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSbomValidationResultsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
