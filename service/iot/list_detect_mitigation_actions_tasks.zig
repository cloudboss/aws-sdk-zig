...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDetectMitigationActionsTasksInput, options: Options) !ListDetectMitigationActionsTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDetectMitigationActionsTasksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDetectMitigationActionsTasksOutput {
    var result: ListDetectMitigationActionsTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDetectMitigationActionsTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
