...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateThingGroupsForThingInput, options: Options) !UpdateThingGroupsForThingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateThingGroupsForThingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.thing_groups_to_add) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"thingGroupsToAdd\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.thing_groups_to_remove) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"thingGroupsToRemove\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateThingGroupsForThingOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
