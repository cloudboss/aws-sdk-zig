...
fn serializeRequest(alloc: std.mem.Allocator, input: TestInvokeAuthorizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/test");
    ...
    if (input.http_context) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"httpContext\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.mqtt_context) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"mqttContext\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tls_context) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"tlsContext\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.token) |v| {
    ...
    if (input.token_signature) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"tokenSignature\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestInvokeAuthorizerOutput {
    var result: TestInvokeAuthorizerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TestInvokeAuthorizerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
