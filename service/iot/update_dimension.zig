...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDimensionInput, options: Options) !UpdateDimensionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDimensionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDimensionOutput {
    var result: UpdateDimensionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDimensionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
