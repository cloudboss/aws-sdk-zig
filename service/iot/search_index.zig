...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchIndexInput, options: Options) !SearchIndexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchIndexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchIndexOutput {
    var result: SearchIndexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchIndexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
