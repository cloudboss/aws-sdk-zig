...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCustomMetricInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCustomMetricOutput {
    var result: DescribeCustomMetricOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCustomMetricOutput, body, alloc);
    }
    _ = status;
    _ = headers;
