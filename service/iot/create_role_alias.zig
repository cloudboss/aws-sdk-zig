...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRoleAliasInput, options: Options) !CreateRoleAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRoleAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRoleAliasOutput {
    var result: CreateRoleAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRoleAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
