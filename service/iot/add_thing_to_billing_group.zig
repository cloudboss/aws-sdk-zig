...
pub const AddThingToBillingGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddThingToBillingGroupInput, options: Options) !AddThingToBillingGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AddThingToBillingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/billing-groups/addThingToBillingGroup";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AddThingToBillingGroupOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: AddThingToBillingGroupOutput = .{};
