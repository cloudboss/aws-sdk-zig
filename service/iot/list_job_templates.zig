...
const JobTemplateSummary = @import("job_template_summary.zig").JobTemplateSummary;
...
pub const ListJobTemplatesOutput = struct {
    ...
    job_templates: ?[]const JobTemplateSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListJobTemplatesInput, options: Options) !ListJobTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListJobTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListJobTemplatesOutput {
    var result: ListJobTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListJobTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
