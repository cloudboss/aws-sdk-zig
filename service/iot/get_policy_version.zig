...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPolicyVersionInput, options: Options) !GetPolicyVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPolicyVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPolicyVersionOutput {
    var result: GetPolicyVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPolicyVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
