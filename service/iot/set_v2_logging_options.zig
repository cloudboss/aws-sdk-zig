...
pub const SetV2LoggingOptionsInput = struct {
...
const SetV2LoggingOptionsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetV2LoggingOptionsInput, options: Options) !SetV2LoggingOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetV2LoggingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.default_log_level) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"defaultLogLevel\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.disable_all_logs) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"disableAllLogs\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.event_configurations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"eventConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.role_arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetV2LoggingOptionsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: SetV2LoggingOptionsOutput = .{};
