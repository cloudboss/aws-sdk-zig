...
pub const SetV2LoggingOptionsOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetV2LoggingOptionsInput, options: CallOptions) !SetV2LoggingOptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetV2LoggingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.disable_all_logs) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"disableAllLogs\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.event_configurations) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"eventConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.role_arn) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetV2LoggingOptionsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
