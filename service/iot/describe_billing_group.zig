...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBillingGroupInput, options: Options) !DescribeBillingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBillingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/billing-groups/");
    try path_buf.appendSlice(alloc, input.billing_group_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBillingGroupOutput {
    var result: DescribeBillingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBillingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
