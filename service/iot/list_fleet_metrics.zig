...
const FleetMetricNameAndArn = @import("fleet_metric_name_and_arn.zig").FleetMetricNameAndArn;

pub const ListFleetMetricsInput = struct {
...
pub const ListFleetMetricsOutput = struct {
    /// The list of fleet metrics objects.
    fleet_metrics: ?[]const FleetMetricNameAndArn = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFleetMetricsInput, options: Options) !ListFleetMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFleetMetricsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFleetMetricsOutput {
    var result: ListFleetMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFleetMetricsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
