...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPrincipalThingsInput, options: Options) !ListPrincipalThingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPrincipalThingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPrincipalThingsOutput {
    var result: ListPrincipalThingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrincipalThingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
