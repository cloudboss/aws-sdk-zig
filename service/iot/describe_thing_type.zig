...
pub const DescribeThingTypeInput = struct {
...
pub const DescribeThingTypeOutput = struct {
    /// The thing type ARN.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeThingTypeInput, options: Options) !DescribeThingTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeThingTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeThingTypeOutput {
    var result: DescribeThingTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeThingTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
