...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeManagedJobTemplateInput, options: Options) !DescribeManagedJobTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeManagedJobTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/managed-job-templates/");
    ...
    if (input.template_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "templateVersion=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeManagedJobTemplateOutput {
    var result: DescribeManagedJobTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeManagedJobTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
