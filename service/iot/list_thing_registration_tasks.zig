...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThingRegistrationTasksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThingRegistrationTasksOutput {
    var result: ListThingRegistrationTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingRegistrationTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
