...
pub const ListThingRegistrationTasksInput = struct {
...
pub const ListThingRegistrationTasksOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingRegistrationTasksInput, options: CallOptions) !ListThingRegistrationTasksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListThingRegistrationTasksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListThingRegistrationTasksOutput {
    var result: ListThingRegistrationTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingRegistrationTasksOutput, body, allocator);
    }
    _ = status;
    _ = headers;
