...
pub const ListCustomMetricsInput = struct {
...
pub const ListCustomMetricsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomMetricsInput, options: CallOptions) !ListCustomMetricsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomMetricsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomMetricsOutput {
    var result: ListCustomMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomMetricsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
