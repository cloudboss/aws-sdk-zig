...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCustomMetricsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCustomMetricsOutput {
    var result: ListCustomMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomMetricsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
