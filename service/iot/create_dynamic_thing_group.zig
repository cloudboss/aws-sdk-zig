...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDynamicThingGroupInput, options: Options) !CreateDynamicThingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDynamicThingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dynamic-thing-groups/");
    try path_buf.appendSlice(alloc, input.thing_group_name);
    ...
    if (input.thing_group_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"thingGroupProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDynamicThingGroupOutput {
    var result: CreateDynamicThingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDynamicThingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
