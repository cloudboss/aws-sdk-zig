...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStatisticsInput, options: Options) !GetStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStatisticsOutput {
    var result: GetStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
