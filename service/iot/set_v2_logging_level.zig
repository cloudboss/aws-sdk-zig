...
fn serializeRequest(alloc: std.mem.Allocator, input: SetV2LoggingLevelInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"logTarget\":");
    try aws.json.writeValue(@TypeOf(input.log_target), input.log_target, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetV2LoggingLevelOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
