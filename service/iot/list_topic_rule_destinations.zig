...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTopicRuleDestinationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTopicRuleDestinationsOutput {
    var result: ListTopicRuleDestinationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTopicRuleDestinationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
