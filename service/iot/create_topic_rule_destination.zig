...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTopicRuleDestinationInput, options: Options) !CreateTopicRuleDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTopicRuleDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.destination_configuration), input.destination_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTopicRuleDestinationOutput {
    var result: CreateTopicRuleDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTopicRuleDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
