...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAuditMitigationActionsTaskInput, options: Options) !DescribeAuditMitigationActionsTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAuditMitigationActionsTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAuditMitigationActionsTaskOutput {
    var result: DescribeAuditMitigationActionsTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAuditMitigationActionsTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
