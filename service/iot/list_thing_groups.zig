...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingGroupsInput, options: Options) !ListThingGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThingGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.parent_group) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "parentGroup=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThingGroupsOutput {
    var result: ListThingGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
