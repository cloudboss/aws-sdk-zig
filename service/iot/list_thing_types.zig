...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThingTypesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThingTypesOutput {
    var result: ListThingTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
