...
const ThingTypeDefinition = @import("thing_type_definition.zig").ThingTypeDefinition;

pub const ListThingTypesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingTypesInput, options: CallOptions) !ListThingTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListThingTypesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListThingTypesOutput {
    var result: ListThingTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingTypesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
