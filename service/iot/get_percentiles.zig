...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPercentilesInput, options: Options) !GetPercentilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPercentilesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.percents) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"percents\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPercentilesOutput {
    var result: GetPercentilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPercentilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
