...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCertificateProviderInput, options: Options) !UpdateCertificateProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCertificateProviderInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCertificateProviderOutput {
    var result: UpdateCertificateProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCertificateProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
