...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListV2LoggingLevelsInput, options: Options) !ListV2LoggingLevelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListV2LoggingLevelsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListV2LoggingLevelsOutput {
    var result: ListV2LoggingLevelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListV2LoggingLevelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
