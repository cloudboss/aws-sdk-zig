...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetDefaultAuthorizerInput, options: Options) !SetDefaultAuthorizerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetDefaultAuthorizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"authorizerName\":");
    try aws.json.writeValue(@TypeOf(input.authorizer_name), input.authorizer_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetDefaultAuthorizerOutput {
    var result: SetDefaultAuthorizerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SetDefaultAuthorizerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
