...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeScheduledAuditInput, options: Options) !DescribeScheduledAuditOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScheduledAuditInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeScheduledAuditOutput {
    var result: DescribeScheduledAuditOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeScheduledAuditOutput, body, alloc);
    }
    _ = status;
    _ = headers;
