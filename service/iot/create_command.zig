...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCommandInput, options: Options) !CreateCommandOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCommandInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/commands/");
    try path_buf.appendSlice(alloc, input.command_id);
    ...
    if (input.mandatory_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"mandatoryParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.payload_template) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"payloadTemplate\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.preprocessor) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"preprocessor\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.role_arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCommandOutput {
    var result: CreateCommandOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCommandOutput, body, alloc);
    }
    _ = status;
    _ = headers;
