...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveThingFromBillingGroupInput, options: Options) !RemoveThingFromBillingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveThingFromBillingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.billing_group_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"billingGroupArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.billing_group_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"billingGroupName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.thing_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"thingName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveThingFromBillingGroupOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
