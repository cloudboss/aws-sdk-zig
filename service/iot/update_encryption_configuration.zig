...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEncryptionConfigurationInput, options: Options) !UpdateEncryptionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEncryptionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"encryptionType\":");
    try aws.json.writeValue(@TypeOf(input.encryption_type), input.encryption_type, alloc, &body_buf);
    has_prev = true;
    if (input.kms_access_role_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"kmsAccessRoleArn\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEncryptionConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
