...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAuditTasksInput, options: Options) !ListAuditTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAuditTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.task_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "taskType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAuditTasksOutput {
    var result: ListAuditTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAuditTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
