...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachThingPrincipalInput, options: Options) !AttachThingPrincipalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachThingPrincipalInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/principals");
    ...
    if (input.thing_principal_type) |v| {
        ...
        try query_buf.appendSlice(alloc, "thingPrincipalType=");
    ...
    try request.headers.put(alloc, "x-amzn-principal", input.principal);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachThingPrincipalOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
