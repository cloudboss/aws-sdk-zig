...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTopicRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTopicRuleOutput {
    var result: GetTopicRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTopicRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
