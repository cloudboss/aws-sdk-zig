...
const TopicRule = @import("topic_rule.zig").TopicRule;

pub const GetTopicRuleInput = struct {
...
pub const GetTopicRuleOutput = struct {
    /// The rule.
    rule: ?TopicRule = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTopicRuleInput, options: CallOptions) !GetTopicRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTopicRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTopicRuleOutput {
    var result: GetTopicRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTopicRuleOutput, body, allocator);
    }
    _ = status;
    _ = headers;
