...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetMetricInput, options: Options) !DescribeFleetMetricOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetMetricInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetMetricOutput {
    var result: DescribeFleetMetricOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFleetMetricOutput, body, alloc);
    }
    _ = status;
    _ = headers;
