...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccountAuditConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.audit_check_configurations) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"auditCheckConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.audit_notification_target_configurations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"auditNotificationTargetConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.role_arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccountAuditConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
