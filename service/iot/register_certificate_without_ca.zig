...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterCertificateWithoutCAInput, options: Options) !RegisterCertificateWithoutCAOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterCertificateWithoutCAInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"certificatePem\":");
    try aws.json.writeValue(@TypeOf(input.certificate_pem), input.certificate_pem, alloc, &body_buf);
    has_prev = true;
    if (input.status) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterCertificateWithoutCAOutput {
    var result: RegisterCertificateWithoutCAOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterCertificateWithoutCAOutput, body, alloc);
    }
    _ = status;
    _ = headers;
