...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoleAliasInput, options: Options) !UpdateRoleAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoleAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/role-aliases/");
    try path_buf.appendSlice(alloc, input.role_alias);
    ...
    if (input.credential_duration_seconds) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"credentialDurationSeconds\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoleAliasOutput {
    var result: UpdateRoleAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRoleAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
