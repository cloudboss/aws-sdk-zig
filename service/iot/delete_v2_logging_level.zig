...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteV2LoggingLevelInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.target_name);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "targetType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.target_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteV2LoggingLevelOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
