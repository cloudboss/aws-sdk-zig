...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSecurityProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.delete_additional_metrics_to_retain) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"deleteAdditionalMetricsToRetain\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.delete_alert_targets) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"deleteAlertTargets\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.delete_behaviors) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"deleteBehaviors\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.delete_metrics_export_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"deleteMetricsExportConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSecurityProfileOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSecurityProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
