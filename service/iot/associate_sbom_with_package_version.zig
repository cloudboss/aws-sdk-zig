...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateSbomWithPackageVersionInput, options: Options) !AssociateSbomWithPackageVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateSbomWithPackageVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateSbomWithPackageVersionOutput {
    var result: AssociateSbomWithPackageVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateSbomWithPackageVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
