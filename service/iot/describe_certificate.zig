...
const CertificateDescription = @import("certificate_description.zig").CertificateDescription;
...
pub const DescribeCertificateOutput = struct {
    ...
    certificate_description: ?CertificateDescription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCertificateInput, options: Options) !DescribeCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCertificateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCertificateOutput {
    var result: DescribeCertificateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCertificateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
