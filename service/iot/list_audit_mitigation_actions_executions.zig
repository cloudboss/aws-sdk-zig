...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAuditMitigationActionsExecutionsInput, options: Options) !ListAuditMitigationActionsExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAuditMitigationActionsExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.action_status) |v| {
        ...
        try query_buf.appendSlice(alloc, "actionStatus=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "findingId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.finding_id);
    query_has_prev = true;
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "taskId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.task_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAuditMitigationActionsExecutionsOutput {
    var result: ListAuditMitigationActionsExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAuditMitigationActionsExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
