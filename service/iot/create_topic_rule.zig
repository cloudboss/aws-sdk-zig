...
pub const CreateTopicRuleInput = struct {
...
const CreateTopicRuleOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTopicRuleInput, options: Options) !CreateTopicRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTopicRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.topic_rule_payload, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTopicRuleOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateTopicRuleOutput = .{};
