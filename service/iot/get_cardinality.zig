...
pub const GetCardinalityInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCardinalityInput, options: CallOptions) !GetCardinalityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCardinalityInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/indices/cardinality";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCardinalityOutput {
    var result: GetCardinalityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCardinalityOutput, body, allocator);
    }
    _ = status;
    _ = headers;
