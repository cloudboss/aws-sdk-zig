...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCardinalityInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCardinalityOutput {
    var result: GetCardinalityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCardinalityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
