...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventConfigurationsInput, options: Options) !DescribeEventConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEventConfigurationsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEventConfigurationsOutput {
    var result: DescribeEventConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeEventConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
