...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMitigationActionInput, options: Options) !CreateMitigationActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMitigationActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.action_params), input.action_params, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMitigationActionOutput {
    var result: CreateMitigationActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMitigationActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
