...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProvisioningTemplateInput, options: Options) !DescribeProvisioningTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProvisioningTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProvisioningTemplateOutput {
    var result: DescribeProvisioningTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeProvisioningTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
