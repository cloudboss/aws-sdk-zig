...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListActiveViolationsInput, options: Options) !ListActiveViolationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListActiveViolationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.behavior_criteria_type) |v| {
        ...
        try query_buf.appendSlice(alloc, "behaviorCriteriaType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.list_suppressed_alerts) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "listSuppressedAlerts=");
    ...
    if (input.thing_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "thingName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.verification_state) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "verificationState=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListActiveViolationsOutput {
    var result: ListActiveViolationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListActiveViolationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
