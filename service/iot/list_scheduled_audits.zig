...
const ScheduledAuditMetadata = @import("scheduled_audit_metadata.zig").ScheduledAuditMetadata;

pub const ListScheduledAuditsInput = struct {
...
pub const ListScheduledAuditsOutput = struct {
    /// A token that can be used to retrieve the next set of results,
    /// or `null` if there are no additional results.
    next_token: ?[]const u8 = null,

    /// The list of scheduled audits.
    scheduled_audits: ?[]const ScheduledAuditMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListScheduledAuditsInput, options: CallOptions) !ListScheduledAuditsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListScheduledAuditsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/audit/scheduledaudits";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListScheduledAuditsOutput {
    var result: ListScheduledAuditsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListScheduledAuditsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
