...
fn serializeRequest(alloc: std.mem.Allocator, input: ListScheduledAuditsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListScheduledAuditsOutput {
    var result: ListScheduledAuditsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListScheduledAuditsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
