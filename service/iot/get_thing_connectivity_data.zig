...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetThingConnectivityDataInput, options: Options) !GetThingConnectivityDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetThingConnectivityDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/connectivity-data");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetThingConnectivityDataOutput {
    var result: GetThingConnectivityDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetThingConnectivityDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
