...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomMetricInput, options: Options) !CreateCustomMetricOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomMetricInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/custom-metric/");
    try path_buf.appendSlice(alloc, input.metric_name);
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"metricType\":");
    try aws.json.writeValue(@TypeOf(input.metric_type), input.metric_type, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomMetricOutput {
    var result: CreateCustomMetricOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCustomMetricOutput, body, alloc);
    }
    _ = status;
    _ = headers;
