...
const OTAUpdateInfo = @import("ota_update_info.zig").OTAUpdateInfo;

pub const GetOTAUpdateInput = struct {
    /// The OTA update ID.
...
pub const GetOTAUpdateOutput = struct {
    /// The OTA update info.
    ota_update_info: ?OTAUpdateInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOTAUpdateInput, options: Options) !GetOTAUpdateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOTAUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/otaUpdates/");
    try path_buf.appendSlice(alloc, input.ota_update_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOTAUpdateOutput {
    var result: GetOTAUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOTAUpdateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
