...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOTAUpdateInput, options: CallOptions) !GetOTAUpdateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetOTAUpdateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetOTAUpdateOutput {
    var result: GetOTAUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOTAUpdateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
