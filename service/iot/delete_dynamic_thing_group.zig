...
pub const DeleteDynamicThingGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDynamicThingGroupInput, options: Options) !DeleteDynamicThingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDynamicThingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.expected_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "expectedVersion=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDynamicThingGroupOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteDynamicThingGroupOutput = .{};
