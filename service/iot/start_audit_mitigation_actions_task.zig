...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAuditMitigationActionsTaskInput, options: Options) !StartAuditMitigationActionsTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartAuditMitigationActionsTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/audit/mitigationactions/tasks/");
    ...
    try body_buf.appendSlice(alloc, "\"auditCheckToActionsMapping\":");
    try aws.json.writeValue(@TypeOf(input.audit_check_to_actions_mapping), input.audit_check_to_actions_mapping, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartAuditMitigationActionsTaskOutput {
    var result: StartAuditMitigationActionsTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartAuditMitigationActionsTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
