...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTopicRulesInput, options: CallOptions) !ListTopicRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTopicRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.rule_disabled) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "ruleDisabled=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTopicRulesOutput {
    var result: ListTopicRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTopicRulesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
