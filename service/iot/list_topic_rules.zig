...
const TopicRuleListItem = @import("topic_rule_list_item.zig").TopicRuleListItem;

pub const ListTopicRulesInput = struct {
...
pub const ListTopicRulesOutput = struct {
    /// The token to use to get the next set of results, or **null** if there are no
    /// additional results.
    next_token: ?[]const u8 = null,

    /// The rules.
    rules: ?[]const TopicRuleListItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTopicRulesInput, options: Options) !ListTopicRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTopicRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.rule_disabled) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ruleDisabled=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.topic) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "topic=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTopicRulesOutput {
    var result: ListTopicRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTopicRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
