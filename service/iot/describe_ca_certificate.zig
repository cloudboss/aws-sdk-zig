...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCACertificateInput, options: Options) !DescribeCACertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCACertificateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCACertificateOutput {
    var result: DescribeCACertificateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCACertificateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
