...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKeysAndCertificateInput, options: Options) !CreateKeysAndCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKeysAndCertificateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKeysAndCertificateOutput {
    var result: CreateKeysAndCertificateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKeysAndCertificateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
