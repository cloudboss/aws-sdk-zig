...
pub const EnableTopicRuleInput = struct {
...
pub const EnableTopicRuleOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableTopicRuleInput, options: CallOptions) !EnableTopicRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableTopicRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableTopicRuleOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: EnableTopicRuleOutput = .{};
