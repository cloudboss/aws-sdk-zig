...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListJobExecutionsForThingInput, options: Options) !ListJobExecutionsForThingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListJobExecutionsForThingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.job_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "jobId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListJobExecutionsForThingOutput {
    var result: ListJobExecutionsForThingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListJobExecutionsForThingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
