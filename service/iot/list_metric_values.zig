...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMetricValuesInput, options: Options) !ListMetricValuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMetricValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dimension_value_operator) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "dimensionValueOperator=");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.metric_name);
    query_has_prev = true;
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.thing_name);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMetricValuesOutput {
    var result: ListMetricValuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMetricValuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
