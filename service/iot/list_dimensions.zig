...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDimensionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDimensionsOutput {
    var result: ListDimensionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDimensionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
