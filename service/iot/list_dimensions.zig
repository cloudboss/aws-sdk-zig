...
pub const ListDimensionsInput = struct {
...
pub const ListDimensionsOutput = struct {
    /// A list of the names of the defined dimensions. Use `DescribeDimension` to
    /// get details for a dimension.
    dimension_names: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDimensionsInput, options: CallOptions) !ListDimensionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDimensionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDimensionsOutput {
    var result: ListDimensionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDimensionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
