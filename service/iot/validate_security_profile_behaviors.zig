...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ValidateSecurityProfileBehaviorsInput, options: Options) !ValidateSecurityProfileBehaviorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ValidateSecurityProfileBehaviorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.behaviors), input.behaviors, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ValidateSecurityProfileBehaviorsOutput {
    var result: ValidateSecurityProfileBehaviorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ValidateSecurityProfileBehaviorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
