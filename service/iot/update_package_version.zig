...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePackageVersionInput, options: Options) !UpdatePackageVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePackageVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.artifact) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"artifact\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.recipe) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"recipe\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePackageVersionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
