...
const ViolationEvent = @import("violation_event.zig").ViolationEvent;

pub const ListViolationEventsInput = struct {
...
pub const ListViolationEventsOutput = struct {
    ...
    /// given time period,
    /// potentially filtered by security profile, behavior violated, or thing
    /// (device) violating.
    violation_events: ?[]const ViolationEvent = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListViolationEventsInput, options: CallOptions) !ListViolationEventsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListViolationEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListViolationEventsOutput {
    var result: ListViolationEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListViolationEventsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
