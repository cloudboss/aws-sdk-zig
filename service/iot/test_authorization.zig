...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestAuthorizationInput, options: Options) !TestAuthorizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestAuthorizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"authInfos\":");
    try aws.json.writeValue(@TypeOf(input.auth_infos), input.auth_infos, alloc, &body_buf);
    has_prev = true;
    if (input.cognito_identity_pool_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"cognitoIdentityPoolId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.policy_names_to_add) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"policyNamesToAdd\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.policy_names_to_skip) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"policyNamesToSkip\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestAuthorizationOutput {
    var result: TestAuthorizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TestAuthorizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
