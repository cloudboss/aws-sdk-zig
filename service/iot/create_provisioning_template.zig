...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProvisioningTemplateInput, options: Options) !CreateProvisioningTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProvisioningTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.pre_provisioning_hook) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"preProvisioningHook\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"provisioningRoleArn\":");
    try aws.json.writeValue(@TypeOf(input.provisioning_role_arn), input.provisioning_role_arn, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProvisioningTemplateOutput {
    var result: CreateProvisioningTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProvisioningTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
