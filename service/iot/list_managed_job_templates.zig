...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedJobTemplatesInput, options: CallOptions) !ListManagedJobTemplatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListManagedJobTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListManagedJobTemplatesOutput {
    var result: ListManagedJobTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedJobTemplatesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
