...
const ManagedJobTemplateSummary = @import("managed_job_template_summary.zig").ManagedJobTemplateSummary;

pub const ListManagedJobTemplatesInput = struct {
    ...
    /// the
    /// managed job templates that have the specified template name will be
    /// returned.
...
pub const ListManagedJobTemplatesOutput = struct {
    /// A list of managed job templates that are returned.
    managed_job_templates: ?[]const ManagedJobTemplateSummary = null,

    /// The token to retrieve the next set of results.
    next_token: ?[]const u8 = null,

    pub const json_field_names = .{
        .managed_job_templates = "managedJobTemplates",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedJobTemplatesInput, options: Options) !ListManagedJobTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedJobTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/managed-job-templates";
    ...
    if (input.template_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "templateName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedJobTemplatesOutput {
    var result: ListManagedJobTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedJobTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
