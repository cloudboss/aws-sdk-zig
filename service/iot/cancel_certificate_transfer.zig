...
pub const CancelCertificateTransferInput = struct {
...
pub const CancelCertificateTransferOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelCertificateTransferInput, options: CallOptions) !CancelCertificateTransferOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelCertificateTransferInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/cancel-certificate-transfer/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelCertificateTransferOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: CancelCertificateTransferOutput = .{};
