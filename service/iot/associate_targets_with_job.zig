...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateTargetsWithJobInput, options: Options) !AssociateTargetsWithJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateTargetsWithJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateTargetsWithJobOutput {
    var result: AssociateTargetsWithJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateTargetsWithJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
