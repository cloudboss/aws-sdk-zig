...
const DomainConfigurationSummary = @import("domain_configuration_summary.zig").DomainConfigurationSummary;

pub const ListDomainConfigurationsInput = struct {
...
pub const ListDomainConfigurationsOutput = struct {
    /// A list of objects that contain summary information about the user's domain
    /// configurations.
    domain_configurations: ?[]const DomainConfigurationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDomainConfigurationsInput, options: Options) !ListDomainConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDomainConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/domainConfigurations";
    ...
    if (input.service_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "serviceType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDomainConfigurationsOutput {
    var result: ListDomainConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDomainConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
