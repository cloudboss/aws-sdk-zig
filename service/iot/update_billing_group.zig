...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBillingGroupInput, options: Options) !UpdateBillingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBillingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"billingGroupProperties\":");
    try aws.json.writeValue(@TypeOf(input.billing_group_properties), input.billing_group_properties, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBillingGroupOutput {
    var result: UpdateBillingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBillingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
