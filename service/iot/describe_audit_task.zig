...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAuditTaskInput, options: Options) !DescribeAuditTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAuditTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAuditTaskOutput {
    var result: DescribeAuditTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAuditTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
