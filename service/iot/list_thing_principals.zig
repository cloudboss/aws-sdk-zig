...
pub const ListThingPrincipalsInput = struct {
...
pub const ListThingPrincipalsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingPrincipalsInput, options: CallOptions) !ListThingPrincipalsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListThingPrincipalsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListThingPrincipalsOutput {
    var result: ListThingPrincipalsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingPrincipalsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
