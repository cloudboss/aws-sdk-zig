...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThingPrincipalsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThingPrincipalsOutput {
    var result: ListThingPrincipalsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingPrincipalsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
