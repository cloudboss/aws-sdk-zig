...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAuditSuppressionInput, options: Options) !UpdateAuditSuppressionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAuditSuppressionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.suppress_indefinitely) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"suppressIndefinitely\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAuditSuppressionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
