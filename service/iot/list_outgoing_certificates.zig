...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOutgoingCertificatesInput, options: CallOptions) !ListOutgoingCertificatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOutgoingCertificatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOutgoingCertificatesOutput {
    var result: ListOutgoingCertificatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOutgoingCertificatesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
