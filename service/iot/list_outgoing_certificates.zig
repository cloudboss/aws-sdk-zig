...
const OutgoingCertificate = @import("outgoing_certificate.zig").OutgoingCertificate;

pub const ListOutgoingCertificatesInput = struct {
...
pub const ListOutgoingCertificatesOutput = struct {
    /// The marker for the next set of results.
    next_marker: ?[]const u8 = null,

    /// The certificates that are being transferred but not yet accepted.
    outgoing_certificates: ?[]const OutgoingCertificate = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOutgoingCertificatesInput, options: Options) !ListOutgoingCertificatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOutgoingCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/certificates-out-going";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.ascending_order) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "isAscendingOrder=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOutgoingCertificatesOutput {
    var result: ListOutgoingCertificatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOutgoingCertificatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
