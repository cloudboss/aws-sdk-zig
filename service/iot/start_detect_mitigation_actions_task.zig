...
pub const StartDetectMitigationActionsTaskInput = struct {
    ...
    include_only_active_violations: ?bool = null,

    /// Specifies to include suppressed alerts.
    include_suppressed_alerts: ?bool = null,
    ...
    pub const json_field_names = .{
        ...
        .include_only_active_violations = "includeOnlyActiveViolations",
        .include_suppressed_alerts = "includeSuppressedAlerts",
        .target = "target",
        .task_id = "taskId",
...
pub const StartDetectMitigationActionsTaskOutput = struct {
    /// The unique identifier of the task.
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDetectMitigationActionsTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/detect/mitigationactions/tasks/");
    try path_buf.appendSlice(alloc, input.task_id);
    ...
    has_prev = true;
    if (input.include_only_active_violations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"includeOnlyActiveViolations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.include_suppressed_alerts) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"includeSuppressedAlerts\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (input.violation_event_occurrence_range) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"violationEventOccurrenceRange\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDetectMitigationActionsTaskOutput {
    var result: StartDetectMitigationActionsTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDetectMitigationActionsTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
