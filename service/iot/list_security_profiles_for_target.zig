...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecurityProfilesForTargetInput, options: Options) !ListSecurityProfilesForTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityProfilesForTargetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecurityProfilesForTargetOutput {
    var result: ListSecurityProfilesForTargetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityProfilesForTargetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
