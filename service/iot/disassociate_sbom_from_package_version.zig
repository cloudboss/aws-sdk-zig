...
pub const DisassociateSbomFromPackageVersionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateSbomFromPackageVersionInput, options: Options) !DisassociateSbomFromPackageVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateSbomFromPackageVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/packages/");
    try path_buf.appendSlice(alloc, input.package_name);
    try path_buf.appendSlice(alloc, "/versions/");
    try path_buf.appendSlice(alloc, input.version_name);
    try path_buf.appendSlice(alloc, "/sbom");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateSbomFromPackageVersionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateSbomFromPackageVersionOutput = .{};
