...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCertificateProviderInput, options: Options) !DescribeCertificateProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCertificateProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/certificate-providers/");
    try path_buf.appendSlice(alloc, input.certificate_provider_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCertificateProviderOutput {
    var result: DescribeCertificateProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCertificateProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
