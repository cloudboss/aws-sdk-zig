...
pub const DescribeAuditFindingInput = struct {
...
pub const DescribeAuditFindingOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAuditFindingInput, options: Options) !DescribeAuditFindingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAuditFindingInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/audit/findings/");
    try path_buf.appendSlice(alloc, input.finding_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAuditFindingOutput {
    var result: DescribeAuditFindingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAuditFindingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
