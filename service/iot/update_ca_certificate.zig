...
const UpdateCACertificateOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCACertificateInput, options: Options) !UpdateCACertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCACertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/cacertificate/");
    ...
    if (input.new_auto_registration_status) |v| {
        ...
        try query_buf.appendSlice(alloc, "newAutoRegistrationStatus=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.new_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "newStatus=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    ...
    if (input.remove_auto_registration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"removeAutoRegistration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCACertificateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
