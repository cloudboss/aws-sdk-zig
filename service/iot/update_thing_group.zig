...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateThingGroupInput, options: Options) !UpdateThingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateThingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/thing-groups/");
    ...
    try aws.json.writeValue(@TypeOf(input.thing_group_properties), input.thing_group_properties, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateThingGroupOutput {
    var result: UpdateThingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateThingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
