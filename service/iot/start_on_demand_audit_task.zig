...
pub const StartOnDemandAuditTaskInput = struct {
    /// Which checks are performed during the audit. The checks you specify must be
    /// enabled
    /// for your account or an exception occurs. Use
...
pub const StartOnDemandAuditTaskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartOnDemandAuditTaskInput, options: Options) !StartOnDemandAuditTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartOnDemandAuditTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"targetCheckNames\":");
    try aws.json.writeValue(@TypeOf(input.target_check_names), input.target_check_names, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartOnDemandAuditTaskOutput {
    var result: StartOnDemandAuditTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartOnDemandAuditTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
