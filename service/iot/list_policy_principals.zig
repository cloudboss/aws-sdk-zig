...
pub const ListPolicyPrincipalsInput = struct {
...
pub const ListPolicyPrincipalsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPolicyPrincipalsInput, options: Options) !ListPolicyPrincipalsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPolicyPrincipalsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "x-amzn-iot-policy", input.policy_name);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPolicyPrincipalsOutput {
    var result: ListPolicyPrincipalsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPolicyPrincipalsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
