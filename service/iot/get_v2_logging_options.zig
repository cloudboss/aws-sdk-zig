...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetV2LoggingOptionsInput, options: Options) !GetV2LoggingOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetV2LoggingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.verbose) |v| {
        ...
        try query_buf.appendSlice(alloc, "verbose=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetV2LoggingOptionsOutput {
    var result: GetV2LoggingOptionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetV2LoggingOptionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
