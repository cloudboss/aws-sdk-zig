...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePackageVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePackageVersionOutput {
    var result: CreatePackageVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePackageVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
