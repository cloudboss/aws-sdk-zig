...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCertificateProviderInput, options: Options) !CreateCertificateProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCertificateProviderInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"accountDefaultForOperations\":");
    try aws.json.writeValue(@TypeOf(input.account_default_for_operations), input.account_default_for_operations, alloc, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.lambda_function_arn), input.lambda_function_arn, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCertificateProviderOutput {
    var result: CreateCertificateProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCertificateProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
