...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeJobTemplateInput, options: Options) !DescribeJobTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeJobTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeJobTemplateOutput {
    var result: DescribeJobTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeJobTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
