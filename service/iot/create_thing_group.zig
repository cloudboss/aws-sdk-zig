...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateThingGroupInput, options: Options) !CreateThingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateThingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.parent_group_name) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"parentGroupName\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateThingGroupOutput {
    var result: CreateThingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateThingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
