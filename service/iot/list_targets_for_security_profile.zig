...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTargetsForSecurityProfileInput, options: CallOptions) !ListTargetsForSecurityProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTargetsForSecurityProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTargetsForSecurityProfileOutput {
    var result: ListTargetsForSecurityProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTargetsForSecurityProfileOutput, body, allocator);
    }
    _ = status;
    _ = headers;
