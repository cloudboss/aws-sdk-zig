...
pub const ListTargetsForSecurityProfileInput = struct {
...
pub const ListTargetsForSecurityProfileOutput = struct {
    ...
    security_profile_targets: ?[]const SecurityProfileTarget = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTargetsForSecurityProfileInput, options: Options) !ListTargetsForSecurityProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTargetsForSecurityProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.security_profile_name);
    try path_buf.appendSlice(alloc, "/targets");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTargetsForSecurityProfileOutput {
    var result: ListTargetsForSecurityProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTargetsForSecurityProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
