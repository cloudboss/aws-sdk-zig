...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeThingRegistrationTaskInput, options: Options) !DescribeThingRegistrationTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeThingRegistrationTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeThingRegistrationTaskOutput {
    var result: DescribeThingRegistrationTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeThingRegistrationTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
