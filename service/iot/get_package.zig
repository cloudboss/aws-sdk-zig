...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPackageInput, options: Options) !GetPackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPackageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPackageOutput {
    var result: GetPackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
