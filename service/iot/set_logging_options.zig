...
const LoggingOptionsPayload = @import("logging_options_payload.zig").LoggingOptionsPayload;

pub const SetLoggingOptionsInput = struct {
...
pub const SetLoggingOptionsOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetLoggingOptionsInput, options: CallOptions) !SetLoggingOptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetLoggingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/loggingOptions";

    const body = try aws.json.jsonStringify(input.logging_options_payload, allocator);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetLoggingOptionsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: SetLoggingOptionsOutput = .{};
