...
fn serializeRequest(alloc: std.mem.Allocator, input: SetLoggingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.logging_options_payload, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetLoggingOptionsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
