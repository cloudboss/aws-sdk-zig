...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIndexingConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIndexingConfigurationOutput {
    var result: GetIndexingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIndexingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
