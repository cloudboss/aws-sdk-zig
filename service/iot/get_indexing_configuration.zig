...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIndexingConfigurationInput, options: CallOptions) !GetIndexingConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIndexingConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIndexingConfigurationOutput {
    var result: GetIndexingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIndexingConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
