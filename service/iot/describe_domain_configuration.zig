...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDomainConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDomainConfigurationOutput {
    var result: DescribeDomainConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDomainConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
