...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteOTAUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.delete_stream) |v| {
        ...
        try query_buf.appendSlice(alloc, "deleteStream=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.force_delete_aws_job) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "forceDeleteAWSJob=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteOTAUpdateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
