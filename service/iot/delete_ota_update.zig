...
pub const DeleteOTAUpdateInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOTAUpdateInput, options: CallOptions) !DeleteOTAUpdateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteOTAUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.delete_stream) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "deleteStream=");
        try query_buf.appendSlice(allocator, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.force_delete_aws_job) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "forceDeleteAWSJob=");
        try query_buf.appendSlice(allocator, if (v) "true" else "false");
        query_has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteOTAUpdateOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteOTAUpdateOutput = .{};
