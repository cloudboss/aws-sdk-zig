...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeThingInput, options: Options) !DescribeThingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeThingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeThingOutput {
    var result: DescribeThingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeThingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
