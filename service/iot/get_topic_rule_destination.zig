...
pub const GetTopicRuleDestinationInput = struct {
...
pub const GetTopicRuleDestinationOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTopicRuleDestinationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTopicRuleDestinationOutput {
    var result: GetTopicRuleDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTopicRuleDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
