...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAuditMitigationActionsTasksInput, options: Options) !ListAuditMitigationActionsTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAuditMitigationActionsTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.audit_task_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "auditTaskId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAuditMitigationActionsTasksOutput {
    var result: ListAuditMitigationActionsTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAuditMitigationActionsTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
