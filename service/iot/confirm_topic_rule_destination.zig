...
pub const ConfirmTopicRuleDestinationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConfirmTopicRuleDestinationInput, options: Options) !ConfirmTopicRuleDestinationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ConfirmTopicRuleDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/confirmdestination/");
    try path_buf.appendSlice(allocator, input.confirmation_token);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ConfirmTopicRuleDestinationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ConfirmTopicRuleDestinationOutput = .{};
