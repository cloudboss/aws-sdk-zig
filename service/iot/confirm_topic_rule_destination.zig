...
fn serializeRequest(alloc: std.mem.Allocator, input: ConfirmTopicRuleDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/confirmdestination/");
    try path_buf.appendSlice(alloc, input.confirmation_token);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConfirmTopicRuleDestinationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
