...
pub const ListPackageVersionsOutput = struct {
    ...
    package_version_summaries: ?[]const PackageVersionSummary = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .package_version_summaries = "packageVersionSummaries",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPackageVersionsInput, options: Options) !ListPackageVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPackageVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPackageVersionsOutput {
    var result: ListPackageVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPackageVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
