...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingsInput, options: Options) !ListThingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.attribute_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "attributeName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.attribute_value) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "attributeValue=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.thing_type_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "thingTypeName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.use_prefix_attribute_value) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "usePrefixAttributeValue=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThingsOutput {
    var result: ListThingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
