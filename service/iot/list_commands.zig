...
pub const ListCommandsInput = struct {
    ...
    /// can override this default value if you want to return all commands that have
    /// been
    ...
    /// Specify whether to list the commands that you have created in the ascending
    /// or
...
pub const ListCommandsOutput = struct {
    ...
    commands: ?[]const CommandSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCommandsInput, options: Options) !ListCommandsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCommandsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.command_parameter_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "commandParameterName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCommandsOutput {
    var result: ListCommandsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCommandsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
