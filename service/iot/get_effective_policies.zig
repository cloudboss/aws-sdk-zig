...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEffectivePoliciesInput, options: Options) !GetEffectivePoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEffectivePoliciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEffectivePoliciesOutput {
    var result: GetEffectivePoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEffectivePoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
