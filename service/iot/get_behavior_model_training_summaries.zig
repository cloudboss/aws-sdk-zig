...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBehaviorModelTrainingSummariesInput, options: CallOptions) !GetBehaviorModelTrainingSummariesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBehaviorModelTrainingSummariesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBehaviorModelTrainingSummariesOutput {
    var result: GetBehaviorModelTrainingSummariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBehaviorModelTrainingSummariesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
