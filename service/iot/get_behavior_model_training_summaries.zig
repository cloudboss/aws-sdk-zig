...
const BehaviorModelTrainingSummary = @import("behavior_model_training_summary.zig").BehaviorModelTrainingSummary;

pub const GetBehaviorModelTrainingSummariesInput = struct {
...
pub const GetBehaviorModelTrainingSummariesOutput = struct {
    ...
    /// Security Profile.
    summaries: ?[]const BehaviorModelTrainingSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBehaviorModelTrainingSummariesInput, options: Options) !GetBehaviorModelTrainingSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBehaviorModelTrainingSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/behavior-model-training/summaries";
    ...
    if (input.security_profile_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "securityProfileName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBehaviorModelTrainingSummariesOutput {
    var result: GetBehaviorModelTrainingSummariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBehaviorModelTrainingSummariesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
