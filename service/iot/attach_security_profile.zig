...
pub const AttachSecurityProfileInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachSecurityProfileInput, options: Options) !AttachSecurityProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachSecurityProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "securityProfileTargetArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.security_profile_target_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachSecurityProfileOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AttachSecurityProfileOutput = .{};
