...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateThingInput, options: Options) !CreateThingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateThingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.attribute_payload) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"attributePayload\":");
    ...
    if (input.thing_type_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"thingTypeName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateThingOutput {
    var result: CreateThingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateThingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
