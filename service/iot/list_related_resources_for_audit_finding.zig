...
pub const ListRelatedResourcesForAuditFindingInput = struct {
    /// The finding Id.
    finding_id: []const u8,
...
pub const ListRelatedResourcesForAuditFindingOutput = struct {
    /// A token that can be used to retrieve the next set of results,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRelatedResourcesForAuditFindingInput, options: Options) !ListRelatedResourcesForAuditFindingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRelatedResourcesForAuditFindingInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/audit/relatedResources";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRelatedResourcesForAuditFindingOutput {
    var result: ListRelatedResourcesForAuditFindingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRelatedResourcesForAuditFindingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
