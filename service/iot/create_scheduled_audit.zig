...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateScheduledAuditInput, options: Options) !CreateScheduledAuditOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateScheduledAuditInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/audit/scheduledaudits/");
    try path_buf.appendSlice(alloc, input.scheduled_audit_name);
    ...
    if (input.day_of_month) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"dayOfMonth\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.day_of_week) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"dayOfWeek\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"frequency\":");
    try aws.json.writeValue(@TypeOf(input.frequency), input.frequency, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateScheduledAuditOutput {
    var result: CreateScheduledAuditOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateScheduledAuditOutput, body, alloc);
    }
    _ = status;
    _ = headers;
