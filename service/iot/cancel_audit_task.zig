...
pub const CancelAuditTaskInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelAuditTaskInput, options: Options) !CancelAuditTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelAuditTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/audit/tasks/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelAuditTaskOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CancelAuditTaskOutput = .{};
