...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProvisioningTemplateVersionInput, options: Options) !DescribeProvisioningTemplateVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProvisioningTemplateVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProvisioningTemplateVersionOutput {
    var result: DescribeProvisioningTemplateVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeProvisioningTemplateVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
