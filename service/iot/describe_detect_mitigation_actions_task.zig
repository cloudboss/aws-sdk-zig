...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDetectMitigationActionsTaskInput, options: Options) !DescribeDetectMitigationActionsTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDetectMitigationActionsTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDetectMitigationActionsTaskOutput {
    var result: DescribeDetectMitigationActionsTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDetectMitigationActionsTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
