...
const OTAUpdateSummary = @import("ota_update_summary.zig").OTAUpdateSummary;

pub const ListOTAUpdatesInput = struct {
...
pub const ListOTAUpdatesOutput = struct {
    /// A token to use to get the next set of results.
    next_token: ?[]const u8 = null,

    /// A list of OTA update jobs.
    ota_updates: ?[]const OTAUpdateSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOTAUpdatesInput, options: Options) !ListOTAUpdatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOTAUpdatesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ota_update_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "otaUpdateStatus=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOTAUpdatesOutput {
    var result: ListOTAUpdatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOTAUpdatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
