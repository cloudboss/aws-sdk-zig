...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOTAUpdatesInput, options: CallOptions) !ListOTAUpdatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOTAUpdatesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ota_update_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "otaUpdateStatus=");
