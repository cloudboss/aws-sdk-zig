...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPackageVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPackageVersionOutput {
    var result: GetPackageVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPackageVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
