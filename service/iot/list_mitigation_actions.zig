...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMitigationActionsInput, options: Options) !ListMitigationActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMitigationActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.action_type) |v| {
        ...
        try query_buf.appendSlice(alloc, "actionType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMitigationActionsOutput {
    var result: ListMitigationActionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMitigationActionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
