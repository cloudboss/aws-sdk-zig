...
pub const AddThingToThingGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddThingToThingGroupInput, options: Options) !AddThingToThingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddThingToThingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/thing-groups/addThingToThingGroup";
    ...
    if (input.override_dynamic_groups) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"overrideDynamicGroups\":");
    ...
    if (input.thing_group_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"thingGroupArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.thing_group_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"thingGroupName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddThingToThingGroupOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AddThingToThingGroupOutput = .{};
