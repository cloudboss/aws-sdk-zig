...
pub const ListSecurityProfilesOutput = struct {
    ...
    security_profile_identifiers: ?[]const SecurityProfileIdentifier = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dimension_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "dimensionName=");
    ...
    if (input.metric_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "metricName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
