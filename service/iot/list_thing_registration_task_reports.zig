...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingRegistrationTaskReportsInput, options: Options) !ListThingRegistrationTaskReportsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThingRegistrationTaskReportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/thing-registration-tasks/");
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "reportType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.report_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThingRegistrationTaskReportsOutput {
    var result: ListThingRegistrationTaskReportsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingRegistrationTaskReportsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
