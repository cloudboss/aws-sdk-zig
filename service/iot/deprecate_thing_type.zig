...
pub const DeprecateThingTypeInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeprecateThingTypeInput, options: Options) !DeprecateThingTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeprecateThingTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/deprecate");
    ...
    if (input.undo_deprecate) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"undoDeprecate\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeprecateThingTypeOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeprecateThingTypeOutput = .{};
