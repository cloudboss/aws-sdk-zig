...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutVerificationStateOnViolationInput, options: Options) !PutVerificationStateOnViolationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutVerificationStateOnViolationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/violations/verification-state/");
    try path_buf.appendSlice(alloc, input.violation_id);
    ...
    try body_buf.appendSlice(alloc, "\"verificationState\":");
    try aws.json.writeValue(@TypeOf(input.verification_state), input.verification_state, alloc, &body_buf);
    has_prev = true;
    if (input.verification_state_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"verificationStateDescription\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutVerificationStateOnViolationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
