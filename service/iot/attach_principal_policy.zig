...
pub const AttachPrincipalPolicyOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachPrincipalPolicyInput, options: CallOptions) !AttachPrincipalPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AttachPrincipalPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/principal-policies/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AttachPrincipalPolicyOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
