...
pub const AttachPrincipalPolicyInput = struct {
    /// The policy name.
...
const AttachPrincipalPolicyOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachPrincipalPolicyInput, options: Options) !AttachPrincipalPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachPrincipalPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/principal-policies/");
    try path_buf.appendSlice(alloc, input.policy_name);
    ...
    try request.headers.put(alloc, "x-amzn-iot-principal", input.principal);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachPrincipalPolicyOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AttachPrincipalPolicyOutput = .{};
