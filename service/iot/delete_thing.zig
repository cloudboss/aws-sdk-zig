...
pub const DeleteThingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteThingInput, options: Options) !DeleteThingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteThingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteThingOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteThingOutput = .{};
