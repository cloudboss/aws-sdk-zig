...
const JobExecutionSummaryForJob = @import("job_execution_summary_for_job.zig").JobExecutionSummaryForJob;

pub const ListJobExecutionsForJobInput = struct {
...
pub const ListJobExecutionsForJobOutput = struct {
    /// A list of job execution summaries.
    execution_summaries: ?[]const JobExecutionSummaryForJob = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListJobExecutionsForJobInput, options: CallOptions) !ListJobExecutionsForJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListJobExecutionsForJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListJobExecutionsForJobOutput {
    var result: ListJobExecutionsForJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListJobExecutionsForJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
