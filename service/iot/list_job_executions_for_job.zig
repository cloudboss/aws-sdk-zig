...
fn serializeRequest(alloc: std.mem.Allocator, input: ListJobExecutionsForJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListJobExecutionsForJobOutput {
    var result: ListJobExecutionsForJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListJobExecutionsForJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
