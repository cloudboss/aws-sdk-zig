...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingPrincipalsV2Input, options: Options) !ListThingPrincipalsV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThingPrincipalsV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/principals-v2");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThingPrincipalsV2Output {
    var result: ListThingPrincipalsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingPrincipalsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
