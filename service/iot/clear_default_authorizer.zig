...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ClearDefaultAuthorizerInput, options: Options) !ClearDefaultAuthorizerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ClearDefaultAuthorizerInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ClearDefaultAuthorizerOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ClearDefaultAuthorizerOutput = .{};
