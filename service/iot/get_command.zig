...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCommandInput, options: Options) !GetCommandOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCommandInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCommandOutput {
    var result: GetCommandOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCommandOutput, body, alloc);
    }
    _ = status;
    _ = headers;
