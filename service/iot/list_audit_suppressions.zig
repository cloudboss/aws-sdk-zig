...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAuditSuppressionsInput, options: Options) !ListAuditSuppressionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAuditSuppressionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ascending_order) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ascendingOrder\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAuditSuppressionsOutput {
    var result: ListAuditSuppressionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAuditSuppressionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
