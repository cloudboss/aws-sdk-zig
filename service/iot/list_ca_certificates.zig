...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCACertificatesInput, options: Options) !ListCACertificatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCACertificatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCACertificatesOutput {
    var result: ListCACertificatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCACertificatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
