...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegistrationCodeInput, options: Options) !GetRegistrationCodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRegistrationCodeInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRegistrationCodeOutput {
    var result: GetRegistrationCodeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRegistrationCodeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
