...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCommandExecutionsInput, options: Options) !ListCommandExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCommandExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.command_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"commandArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.completed_time_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"completedTimeFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.started_time_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"startedTimeFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.status) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCommandExecutionsOutput {
    var result: ListCommandExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCommandExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
