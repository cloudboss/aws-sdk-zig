...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProvisioningClaimInput, options: CallOptions) !CreateProvisioningClaimOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateProvisioningClaimInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/provisioning-claim");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateProvisioningClaimOutput {
    var result: CreateProvisioningClaimOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProvisioningClaimOutput, body, allocator);
    }
    _ = status;
    _ = headers;
