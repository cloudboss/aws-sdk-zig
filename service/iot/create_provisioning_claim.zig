...
pub const CreateProvisioningClaimInput = struct {
...
pub const CreateProvisioningClaimOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProvisioningClaimInput, options: Options) !CreateProvisioningClaimOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProvisioningClaimInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/provisioning-claim");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProvisioningClaimOutput {
    var result: CreateProvisioningClaimOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProvisioningClaimOutput, body, alloc);
    }
    _ = status;
    _ = headers;
