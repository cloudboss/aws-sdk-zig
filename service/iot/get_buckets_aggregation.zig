...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBucketsAggregationInput, options: Options) !GetBucketsAggregationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBucketsAggregationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"aggregationField\":");
    try aws.json.writeValue(@TypeOf(input.aggregation_field), input.aggregation_field, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"bucketsAggregationType\":");
    try aws.json.writeValue(@TypeOf(input.buckets_aggregation_type), input.buckets_aggregation_type, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"queryString\":");
    try aws.json.writeValue(@TypeOf(input.query_string), input.query_string, alloc, &body_buf);
    has_prev = true;
    if (input.query_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"queryVersion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBucketsAggregationOutput {
    var result: GetBucketsAggregationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBucketsAggregationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
