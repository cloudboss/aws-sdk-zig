...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDimensionInput, options: Options) !CreateDimensionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDimensionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dimensions/");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"stringValues\":");
    try aws.json.writeValue(@TypeOf(input.string_values), input.string_values, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDimensionOutput {
    var result: CreateDimensionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDimensionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
