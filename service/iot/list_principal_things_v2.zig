...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPrincipalThingsV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPrincipalThingsV2Output {
    var result: ListPrincipalThingsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrincipalThingsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
