...
const PrincipalThingObject = @import("principal_thing_object.zig").PrincipalThingObject;

pub const ListPrincipalThingsV2Input = struct {
    ...
    /// The principal. A principal can be an X.509 certificate or an Amazon Cognito
    /// ID.
    principal: []const u8,
...
pub const ListPrincipalThingsV2Output = struct {
    /// The token to use to get the next set of results, or **null** if there are no
    /// additional results.
    next_token: ?[]const u8 = null,

    /// A list of `thingPrincipalObject` that represents the principal and the type
    /// of relation it has with the thing.
    principal_thing_objects: ?[]const PrincipalThingObject = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .principal_thing_objects = "principalThingObjects",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPrincipalThingsV2Input, options: CallOptions) !ListPrincipalThingsV2Output {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPrincipalThingsV2Input, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/principals/things-v2";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPrincipalThingsV2Output {
    var result: ListPrincipalThingsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrincipalThingsV2Output, body, allocator);
    }
    _ = status;
    _ = headers;
