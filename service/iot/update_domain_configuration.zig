...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDomainConfigurationInput, options: Options) !UpdateDomainConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDomainConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.domain_configuration_status) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"domainConfigurationStatus\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.remove_authorizer_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"removeAuthorizerConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDomainConfigurationOutput {
    var result: UpdateDomainConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDomainConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
