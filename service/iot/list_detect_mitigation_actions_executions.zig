...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDetectMitigationActionsExecutionsInput, options: Options) !ListDetectMitigationActionsExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDetectMitigationActionsExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.violation_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "violationId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDetectMitigationActionsExecutionsOutput {
    var result: ListDetectMitigationActionsExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDetectMitigationActionsExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
