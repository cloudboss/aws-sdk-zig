...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAuditSuppressionInput, options: CallOptions) !DescribeAuditSuppressionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAuditSuppressionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAuditSuppressionOutput {
    var result: DescribeAuditSuppressionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAuditSuppressionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
