...
pub const DescribeAuditSuppressionInput = struct {
    check_name: []const u8,
...
pub const DescribeAuditSuppressionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAuditSuppressionInput, options: Options) !DescribeAuditSuppressionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAuditSuppressionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/audit/suppressions/describe";
    ...
    try body_buf.appendSlice(alloc, "\"checkName\":");
    try aws.json.writeValue(@TypeOf(input.check_name), input.check_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAuditSuppressionOutput {
    var result: DescribeAuditSuppressionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAuditSuppressionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
