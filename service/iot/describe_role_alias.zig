...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRoleAliasInput, options: Options) !DescribeRoleAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRoleAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRoleAliasOutput {
    var result: DescribeRoleAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRoleAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
