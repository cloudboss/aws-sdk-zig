...
pub const UpdatePackageConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePackageConfigurationInput, options: Options) !UpdatePackageConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePackageConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.version_update_by_jobs_config) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"versionUpdateByJobsConfig\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePackageConfigurationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdatePackageConfigurationOutput = .{};
