...
pub const ListThingsInBillingGroupInput = struct {
...
pub const ListThingsInBillingGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingsInBillingGroupInput, options: Options) !ListThingsInBillingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThingsInBillingGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThingsInBillingGroupOutput {
    var result: ListThingsInBillingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingsInBillingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
