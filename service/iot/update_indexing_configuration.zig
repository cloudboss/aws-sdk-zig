...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIndexingConfigurationInput, options: Options) !UpdateIndexingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIndexingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.thing_group_indexing_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"thingGroupIndexingConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.thing_indexing_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"thingIndexingConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIndexingConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
