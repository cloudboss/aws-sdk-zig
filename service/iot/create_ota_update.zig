...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOTAUpdateInput, options: Options) !CreateOTAUpdateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOTAUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.additional_parameters) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"additionalParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.aws_job_abort_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"awsJobAbortConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.aws_job_executions_rollout_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"awsJobExecutionsRolloutConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.aws_job_presigned_url_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"awsJobPresignedUrlConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.aws_job_timeout_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"awsJobTimeoutConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.protocols) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"protocols\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOTAUpdateOutput {
    var result: CreateOTAUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateOTAUpdateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
