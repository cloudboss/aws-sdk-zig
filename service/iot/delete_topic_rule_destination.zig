...
pub const DeleteTopicRuleDestinationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTopicRuleDestinationInput, options: Options) !DeleteTopicRuleDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTopicRuleDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/destinations/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTopicRuleDestinationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteTopicRuleDestinationOutput = .{};
