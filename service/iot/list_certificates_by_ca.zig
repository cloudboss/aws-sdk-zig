...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCertificatesByCAInput, options: Options) !ListCertificatesByCAOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCertificatesByCAInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/certificates-by-ca/");
    try path_buf.appendSlice(alloc, input.ca_certificate_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCertificatesByCAOutput {
    var result: ListCertificatesByCAOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCertificatesByCAOutput, body, alloc);
    }
    _ = status;
    _ = headers;
