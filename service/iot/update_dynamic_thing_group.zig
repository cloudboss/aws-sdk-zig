...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDynamicThingGroupInput, options: Options) !UpdateDynamicThingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDynamicThingGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDynamicThingGroupOutput {
    var result: UpdateDynamicThingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDynamicThingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
