...
pub const RemoveThingFromThingGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveThingFromThingGroupInput, options: Options) !RemoveThingFromThingGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveThingFromThingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/thing-groups/removeThingFromThingGroup";
    ...
    if (input.thing_group_arn) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"thingGroupArn\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.thing_group_name) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"thingGroupName\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveThingFromThingGroupOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: RemoveThingFromThingGroupOutput = .{};
