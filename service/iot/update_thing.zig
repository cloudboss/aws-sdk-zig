...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateThingInput, options: Options) !UpdateThingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateThingInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.remove_thing_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"removeThingType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateThingOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
