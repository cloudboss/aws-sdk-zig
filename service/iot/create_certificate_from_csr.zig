...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCertificateFromCsrInput, options: Options) !CreateCertificateFromCsrOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCertificateFromCsrInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"certificateSigningRequest\":");
    try aws.json.writeValue(@TypeOf(input.certificate_signing_request), input.certificate_signing_request, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCertificateFromCsrOutput {
    var result: CreateCertificateFromCsrOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCertificateFromCsrOutput, body, alloc);
    }
    _ = status;
    _ = headers;
