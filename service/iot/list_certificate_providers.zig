...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCertificateProvidersInput, options: Options) !ListCertificateProvidersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCertificateProvidersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCertificateProvidersOutput {
    var result: ListCertificateProvidersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCertificateProvidersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
