...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateThingTypeInput, options: Options) !CreateThingTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateThingTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/thing-types/");
    try path_buf.appendSlice(alloc, input.thing_type_name);
    ...
    if (input.thing_type_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"thingTypeProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateThingTypeOutput {
    var result: CreateThingTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateThingTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
