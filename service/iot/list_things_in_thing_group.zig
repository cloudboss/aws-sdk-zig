...
pub const ListThingsInThingGroupInput = struct {
...
pub const ListThingsInThingGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingsInThingGroupInput, options: Options) !ListThingsInThingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThingsInThingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.recursive) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "recursive=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThingsInThingGroupOutput {
    var result: ListThingsInThingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingsInThingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
