...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThingsInThingGroupInput, options: CallOptions) !ListThingsInThingGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListThingsInThingGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListThingsInThingGroupOutput {
    var result: ListThingsInThingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThingsInThingGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
