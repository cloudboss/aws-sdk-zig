...
const AuthorizerSummary = @import("authorizer_summary.zig").AuthorizerSummary;
...
pub const ListAuthorizersOutput = struct {
    /// The authorizers.
    authorizers: ?[]const AuthorizerSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAuthorizersInput, options: Options) !ListAuthorizersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAuthorizersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAuthorizersOutput {
    var result: ListAuthorizersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAuthorizersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
