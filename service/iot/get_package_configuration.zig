...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPackageConfigurationInput, options: Options) !GetPackageConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPackageConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPackageConfigurationOutput {
    var result: GetPackageConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPackageConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
