...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobDocumentInput, options: Options) !GetJobDocumentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetJobDocumentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/job-document");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetJobDocumentOutput {
    var result: GetJobDocumentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetJobDocumentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
