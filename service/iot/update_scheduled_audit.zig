...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateScheduledAuditInput, options: Options) !UpdateScheduledAuditOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateScheduledAuditInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateScheduledAuditOutput {
    var result: UpdateScheduledAuditOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateScheduledAuditOutput, body, alloc);
    }
    _ = status;
    _ = headers;
