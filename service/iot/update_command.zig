...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCommandInput, options: Options) !UpdateCommandOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCommandInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.deprecated) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"deprecated\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCommandOutput {
    var result: UpdateCommandOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCommandOutput, body, alloc);
    }
    _ = status;
    _ = headers;
