...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAttachedPoliciesInput, options: Options) !ListAttachedPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAttachedPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/attached-policies/");
    try path_buf.appendSlice(alloc, input.target);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAttachedPoliciesOutput {
    var result: ListAttachedPoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAttachedPoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
