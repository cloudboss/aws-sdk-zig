...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAuthorizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.authorizer_function_arn), input.authorizer_function_arn, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.signing_disabled) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"signingDisabled\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.status) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAuthorizerOutput {
    var result: CreateAuthorizerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAuthorizerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
