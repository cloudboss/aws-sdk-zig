...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterCACertificateInput, options: Options) !RegisterCACertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterCACertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allow_auto_registration) |v| {
        ...
        try query_buf.appendSlice(alloc, "allowAutoRegistration=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.set_as_active) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "setAsActive=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    ...
    try body_buf.appendSlice(alloc, "\"caCertificate\":");
    try aws.json.writeValue(@TypeOf(input.ca_certificate), input.ca_certificate, alloc, &body_buf);
    has_prev = true;
    if (input.certificate_mode) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"certificateMode\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.registration_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"registrationConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
    ...
    if (input.verification_certificate) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"verificationCertificate\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterCACertificateOutput {
    var result: RegisterCACertificateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterCACertificateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
