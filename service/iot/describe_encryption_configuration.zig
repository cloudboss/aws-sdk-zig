...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEncryptionConfigurationInput, options: Options) !DescribeEncryptionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEncryptionConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEncryptionConfigurationOutput {
    var result: DescribeEncryptionConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeEncryptionConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
