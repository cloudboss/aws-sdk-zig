...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InvokeEndpointInput, options: Options) !InvokeEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InvokeEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.enable_explanations) |v| {
        try request.headers.put(alloc, "X-Amzn-SageMaker-Enable-Explanations", v);
    }
    ...
    if (input.target_model) |v| {
        try request.headers.put(alloc, "X-Amzn-SageMaker-Target-Model", v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InvokeEndpointOutput {
    var result: InvokeEndpointOutput = .{};
    result.body = try alloc.dupe(u8, body);
    _ = status;
    if (headers.get("x-amzn-sagemaker-closed-session-id")) |value| {
        result.closed_session_id = try alloc.dupe(u8, value);
    }
    ...
    if (headers.get("x-amzn-sagemaker-custom-attributes")) |value| {
        result.custom_attributes = try alloc.dupe(u8, value);
    }
    if (headers.get("x-amzn-invoked-production-variant")) |value| {
        result.invoked_production_variant = try alloc.dupe(u8, value);
    }
    if (headers.get("x-amzn-sagemaker-new-session-id")) |value| {
        result.new_session_id = try alloc.dupe(u8, value);
    }
