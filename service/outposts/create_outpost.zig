...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOutpostInput, options: Options) !CreateOutpostOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOutpostInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.site_id), input.site_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOutpostOutput {
    var result: CreateOutpostOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateOutpostOutput, body, alloc);
    }
    _ = status;
    _ = headers;
