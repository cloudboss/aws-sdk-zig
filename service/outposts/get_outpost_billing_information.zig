...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOutpostBillingInformationInput, options: Options) !GetOutpostBillingInformationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOutpostBillingInformationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/outpost/");
    try path_buf.appendSlice(alloc, input.outpost_identifier);
    try path_buf.appendSlice(alloc, "/billing-information");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOutpostBillingInformationOutput {
    var result: GetOutpostBillingInformationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOutpostBillingInformationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
