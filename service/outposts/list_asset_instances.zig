...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssetInstancesInput, options: Options) !ListAssetInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssetInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/assetInstances");
    ...
    if (input.account_id_filter) |v| {
        ...
        try query_buf.appendSlice(alloc, "AccountIdFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.asset_id_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "AssetIdFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.aws_service_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "AwsServiceFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.instance_type_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "InstanceTypeFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssetInstancesOutput {
    var result: ListAssetInstancesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetInstancesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
