...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCatalogItemsInput, options: Options) !ListCatalogItemsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCatalogItemsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ec2_family_filter) |v| {
        ...
        try query_buf.appendSlice(alloc, "EC2FamilyFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.item_class_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ItemClassFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.supported_storage_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "SupportedStorageFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCatalogItemsOutput {
    var result: ListCatalogItemsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCatalogItemsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
