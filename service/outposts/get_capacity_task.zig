...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCapacityTaskInput, options: Options) !GetCapacityTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCapacityTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCapacityTaskOutput {
    var result: GetCapacityTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCapacityTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
