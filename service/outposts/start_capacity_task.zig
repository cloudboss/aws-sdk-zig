...
fn serializeRequest(alloc: std.mem.Allocator, input: StartCapacityTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/capacity");
    ...
    if (input.asset_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AssetId\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"InstancePools\":");
    try aws.json.writeValue(@TypeOf(input.instance_pools), input.instance_pools, alloc, &body_buf);
    has_prev = true;
    if (input.instances_to_exclude) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InstancesToExclude\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.order_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OrderId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.task_action_on_blocking_instances) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TaskActionOnBlockingInstances\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartCapacityTaskOutput {
    var result: StartCapacityTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartCapacityTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
