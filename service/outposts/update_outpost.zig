...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOutpostInput, options: CallOptions) !UpdateOutpostOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateOutpostInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateOutpostOutput {
    var result: UpdateOutpostOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateOutpostOutput, body, allocator);
    }
    _ = status;
    _ = headers;
