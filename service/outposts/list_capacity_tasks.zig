...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCapacityTasksInput, options: Options) !ListCapacityTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCapacityTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.capacity_task_status_filter) |v| {
        ...
        try query_buf.appendSlice(alloc, "CapacityTaskStatusFilter=");
    ...
    if (input.outpost_identifier_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "OutpostIdentifierFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCapacityTasksOutput {
    var result: ListCapacityTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCapacityTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
