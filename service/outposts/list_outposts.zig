...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOutpostsInput, options: Options) !ListOutpostsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOutpostsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.availability_zone_filter) |v| {
        ...
        try query_buf.appendSlice(alloc, "AvailabilityZoneFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.availability_zone_id_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "AvailabilityZoneIdFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.life_cycle_status_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "LifeCycleStatusFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOutpostsOutput {
    var result: ListOutpostsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOutpostsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
