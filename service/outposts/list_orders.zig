...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOrdersInput, options: Options) !ListOrdersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOrdersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOrdersOutput {
    var result: ListOrdersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOrdersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
