...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSitesInput, options: Options) !ListSitesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSitesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.operating_address_city_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "OperatingAddressCityFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.operating_address_country_code_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "OperatingAddressCountryCodeFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.operating_address_state_or_region_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "OperatingAddressStateOrRegionFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSitesOutput {
    var result: ListSitesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSitesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
