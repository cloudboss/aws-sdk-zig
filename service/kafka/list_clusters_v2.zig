...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListClustersV2Input, options: Options) !ListClustersV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListClustersV2Input, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cluster_name_filter) |v| {
        ...
        try query_buf.appendSlice(alloc, "clusterNameFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.cluster_type_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "clusterTypeFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListClustersV2Output {
    var result: ListClustersV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListClustersV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
