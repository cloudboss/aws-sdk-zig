...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateClusterV2Input, options: Options) !CreateClusterV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateClusterV2Input, config: *aws.Config) !aws.http.Request {
    ...
    if (input.provisioned) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Provisioned\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.serverless) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Serverless\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateClusterV2Output {
    var result: CreateClusterV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateClusterV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
