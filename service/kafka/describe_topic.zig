...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTopicInput, options: Options) !DescribeTopicOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTopicInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.topic_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTopicOutput {
    var result: DescribeTopicOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTopicOutput, body, alloc);
    }
    _ = status;
    _ = headers;
