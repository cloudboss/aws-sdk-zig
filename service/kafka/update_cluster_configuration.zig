...
pub const UpdateClusterConfigurationInput = struct {
...
pub const UpdateClusterConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateClusterConfigurationInput, options: CallOptions) !UpdateClusterConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateClusterConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.configuration_info), input.configuration_info, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateClusterConfigurationOutput {
    var result: UpdateClusterConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateClusterConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
