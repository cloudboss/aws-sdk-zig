...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReplicatorsInput, options: Options) !ListReplicatorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReplicatorsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.replicator_name_filter) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "replicatorNameFilter=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReplicatorsOutput {
    var result: ListReplicatorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReplicatorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
