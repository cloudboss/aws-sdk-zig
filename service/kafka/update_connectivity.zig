...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConnectivityInput, options: CallOptions) !UpdateConnectivityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateConnectivityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/connectivity");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateConnectivityOutput {
    var result: UpdateConnectivityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConnectivityOutput, body, allocator);
    }
    _ = status;
    _ = headers;
