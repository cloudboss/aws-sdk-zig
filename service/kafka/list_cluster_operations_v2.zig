...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListClusterOperationsV2Input, options: Options) !ListClusterOperationsV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListClusterOperationsV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListClusterOperationsV2Output {
    var result: ListClusterOperationsV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListClusterOperationsV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
