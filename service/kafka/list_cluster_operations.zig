...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListClusterOperationsInput, options: Options) !ListClusterOperationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListClusterOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/operations");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListClusterOperationsOutput {
    var result: ListClusterOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListClusterOperationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
