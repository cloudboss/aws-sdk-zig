...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMonitoringInput, options: Options) !UpdateMonitoringOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMonitoringInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/monitoring");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMonitoringOutput {
    var result: UpdateMonitoringOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMonitoringOutput, body, alloc);
    }
    _ = status;
    _ = headers;
