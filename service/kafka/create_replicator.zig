...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReplicatorInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"KafkaClusters\":");
    try aws.json.writeValue(@TypeOf(input.kafka_clusters), input.kafka_clusters, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReplicationInfoList\":");
    try aws.json.writeValue(@TypeOf(input.replication_info_list), input.replication_info_list, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReplicatorName\":");
    try aws.json.writeValue(@TypeOf(input.replicator_name), input.replicator_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReplicatorOutput {
    var result: CreateReplicatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateReplicatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
