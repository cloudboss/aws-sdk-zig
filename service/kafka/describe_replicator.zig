...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicatorInput, options: Options) !DescribeReplicatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReplicatorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReplicatorOutput {
    var result: DescribeReplicatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeReplicatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
