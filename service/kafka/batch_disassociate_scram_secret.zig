...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDisassociateScramSecretInput, options: CallOptions) !BatchDisassociateScramSecretOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDisassociateScramSecretInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDisassociateScramSecretOutput {
    var result: BatchDisassociateScramSecretOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDisassociateScramSecretOutput, body, allocator);
    }
    _ = status;
    _ = headers;
