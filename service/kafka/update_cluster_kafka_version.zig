...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateClusterKafkaVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"TargetKafkaVersion\":");
    try aws.json.writeValue(@TypeOf(input.target_kafka_version), input.target_kafka_version, allocator, &body_buf);
    has_prev = true;
