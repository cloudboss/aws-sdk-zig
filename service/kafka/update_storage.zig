...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStorageInput, options: CallOptions) !UpdateStorageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateStorageInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.provisioned_throughput) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ProvisionedThroughput\":");
    ...
    if (input.volume_size_gb) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"VolumeSizeGB\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
