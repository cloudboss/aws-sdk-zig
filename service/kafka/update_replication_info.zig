...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateReplicationInfoInput, options: Options) !UpdateReplicationInfoOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateReplicationInfoInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/replication/v1/replicators/");
    try path_buf.appendSlice(alloc, input.replicator_arn);
    try path_buf.appendSlice(alloc, "/replication-info");
    ...
    if (input.consumer_group_replication) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ConsumerGroupReplication\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceKafkaClusterArn\":");
    try aws.json.writeValue(@TypeOf(input.source_kafka_cluster_arn), input.source_kafka_cluster_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TargetKafkaClusterArn\":");
    try aws.json.writeValue(@TypeOf(input.target_kafka_cluster_arn), input.target_kafka_cluster_arn, alloc, &body_buf);
    has_prev = true;
    if (input.topic_replication) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TopicReplication\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateReplicationInfoOutput {
    var result: UpdateReplicationInfoOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateReplicationInfoOutput, body, alloc);
    }
    _ = status;
    _ = headers;
