...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSecurityInput, options: Options) !UpdateSecurityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSecurityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/security");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSecurityOutput {
    var result: UpdateSecurityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSecurityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
