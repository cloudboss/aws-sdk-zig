...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBootstrapBrokersInput, options: Options) !GetBootstrapBrokersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBootstrapBrokersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/bootstrap-brokers");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBootstrapBrokersOutput {
    var result: GetBootstrapBrokersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBootstrapBrokersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
