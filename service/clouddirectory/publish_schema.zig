...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PublishSchemaInput, options: Options) !PublishSchemaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PublishSchemaInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.minor_version) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"MinorVersion\":");
    ...
    try aws.json.writeValue(@TypeOf(input.version), input.version, alloc, &body_buf);
    has_prev = true;
    ...
    try request.headers.put(alloc, "x-amz-data-partition", input.development_schema_arn);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PublishSchemaOutput {
    var result: PublishSchemaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PublishSchemaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
