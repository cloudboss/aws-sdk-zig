...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTypedLinkFacetAttributesInput, options: Options) !ListTypedLinkFacetAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTypedLinkFacetAttributesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTypedLinkFacetAttributesOutput {
    var result: ListTypedLinkFacetAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTypedLinkFacetAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
