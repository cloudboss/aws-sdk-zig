...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPolicyAttachmentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPolicyAttachmentsOutput {
    var result: ListPolicyAttachmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPolicyAttachmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
