...
pub const ListPolicyAttachmentsInput = struct {
...
pub const ListPolicyAttachmentsOutput = struct {
    /// The pagination token.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPolicyAttachmentsInput, options: CallOptions) !ListPolicyAttachmentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPolicyAttachmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/policy/attachment";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPolicyAttachmentsOutput {
    var result: ListPolicyAttachmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPolicyAttachmentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
