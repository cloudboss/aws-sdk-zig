...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIncomingTypedLinksInput, options: Options) !ListIncomingTypedLinksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIncomingTypedLinksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIncomingTypedLinksOutput {
    var result: ListIncomingTypedLinksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIncomingTypedLinksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
