...
pub const RemoveFacetFromObjectInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveFacetFromObjectInput, options: Options) !RemoveFacetFromObjectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveFacetFromObjectInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/object/facets/delete";
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SchemaFacet\":");
    try aws.json.writeValue(@TypeOf(input.schema_facet), input.schema_facet, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveFacetFromObjectOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: RemoveFacetFromObjectOutput = .{};
