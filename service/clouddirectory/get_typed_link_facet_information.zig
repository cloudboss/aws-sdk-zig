...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTypedLinkFacetInformationInput, options: Options) !GetTypedLinkFacetInformationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTypedLinkFacetInformationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTypedLinkFacetInformationOutput {
    var result: GetTypedLinkFacetInformationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTypedLinkFacetInformationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
