...
pub const DetachFromIndexInput = struct {
...
pub const DetachFromIndexOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachFromIndexInput, options: Options) !DetachFromIndexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachFromIndexInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/index/detach";
    ...
    try body_buf.appendSlice(alloc, "\"IndexReference\":");
    try aws.json.writeValue(@TypeOf(input.index_reference), input.index_reference, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachFromIndexOutput {
    var result: DetachFromIndexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DetachFromIndexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
