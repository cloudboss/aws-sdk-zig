...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSchemaFromJsonInput, options: Options) !PutSchemaFromJsonOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutSchemaFromJsonInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.document), input.document, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutSchemaFromJsonOutput {
    var result: PutSchemaFromJsonOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutSchemaFromJsonOutput, body, alloc);
    }
    _ = status;
    _ = headers;
