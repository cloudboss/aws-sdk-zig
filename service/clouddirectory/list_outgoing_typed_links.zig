...
pub const ListOutgoingTypedLinksInput = struct {
...
pub const ListOutgoingTypedLinksOutput = struct {
    /// The pagination token.
    next_token: ?[]const u8 = null,

    /// Returns a typed link specifier as output.
    typed_link_specifiers: ?[]const TypedLinkSpecifier = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOutgoingTypedLinksInput, options: Options) !ListOutgoingTypedLinksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOutgoingTypedLinksInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("clouddirectory", "CloudDirectory", alloc);

    const host = aws.url.parseHost(endpoint);
    const tls = !std.mem.startsWith(u8, endpoint, "http://");
    const port = aws.url.parsePort(endpoint);

    const path = "/amazonclouddirectory/2017-01-11/typedlink/outgoing";
    ...
    if (input.consistency_level) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ConsistencyLevel\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.filter_attribute_ranges) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FilterAttributeRanges\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.filter_typed_link) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FilterTypedLink\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ObjectReference\":");
    try aws.json.writeValue(@TypeOf(input.object_reference), input.object_reference, alloc, &body_buf);
    has_prev = true;
    ...
    try request.headers.put(alloc, "x-amz-data-partition", input.directory_arn);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOutgoingTypedLinksOutput {
    var result: ListOutgoingTypedLinksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOutgoingTypedLinksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
