...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchReadInput, options: Options) !BatchReadOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchReadInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchReadOutput {
    var result: BatchReadOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchReadOutput, body, alloc);
    }
    _ = status;
    _ = headers;
