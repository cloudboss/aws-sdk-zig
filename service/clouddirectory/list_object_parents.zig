...
pub const ListObjectParentsInput = struct {
...
pub const ListObjectParentsOutput = struct {
    /// The pagination token.
    next_token: ?[]const u8 = null,
    ...
    parents: ?[]const aws.map.StringMapEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObjectParentsInput, options: Options) !ListObjectParentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListObjectParentsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/object/parent";

    var body_buf: std.ArrayList(u8) = .{};
    var has_prev = false;
    try body_buf.appendSlice(alloc, "{");

    if (input.include_all_links_to_each_parent) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IncludeAllLinksToEachParent\":");
    ...
    if (input.consistency_level) |v| {
        try request.headers.put(alloc, "x-amz-consistency-level", @tagName(v));
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListObjectParentsOutput {
    var result: ListObjectParentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListObjectParentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
