...
fn serializeRequest(alloc: std.mem.Allocator, input: GetObjectAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AttributeNames\":");
    try aws.json.writeValue(@TypeOf(input.attribute_names), input.attribute_names, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetObjectAttributesOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetObjectAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
