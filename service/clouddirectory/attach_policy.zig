...
pub const AttachPolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachPolicyInput, options: Options) !AttachPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/policy/attach";
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PolicyReference\":");
    try aws.json.writeValue(@TypeOf(input.policy_reference), input.policy_reference, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachPolicyOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AttachPolicyOutput = .{};
