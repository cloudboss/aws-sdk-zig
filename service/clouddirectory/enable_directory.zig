...
pub const EnableDirectoryInput = struct {
...
pub const EnableDirectoryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableDirectoryInput, options: CallOptions) !EnableDirectoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableDirectoryInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/directory/enable";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableDirectoryOutput {
    var result: EnableDirectoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EnableDirectoryOutput, body, allocator);
    }
    _ = status;
    _ = headers;
