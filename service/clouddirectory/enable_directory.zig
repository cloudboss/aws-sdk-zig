...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableDirectoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableDirectoryOutput {
    var result: EnableDirectoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EnableDirectoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
