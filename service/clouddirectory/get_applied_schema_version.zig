...
pub const GetAppliedSchemaVersionInput = struct {
...
pub const GetAppliedSchemaVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAppliedSchemaVersionInput, options: CallOptions) !GetAppliedSchemaVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAppliedSchemaVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/schema/getappliedschema";
    ...
    try aws.json.writeValue(@TypeOf(input.schema_arn), input.schema_arn, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAppliedSchemaVersionOutput {
    var result: GetAppliedSchemaVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAppliedSchemaVersionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
