...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAppliedSchemaVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.schema_arn), input.schema_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAppliedSchemaVersionOutput {
    var result: GetAppliedSchemaVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAppliedSchemaVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
