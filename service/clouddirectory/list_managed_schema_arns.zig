...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedSchemaArnsInput, options: CallOptions) !ListManagedSchemaArnsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListManagedSchemaArnsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListManagedSchemaArnsOutput {
    var result: ListManagedSchemaArnsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedSchemaArnsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
