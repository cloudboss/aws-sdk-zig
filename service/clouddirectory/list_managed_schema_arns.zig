...
pub const ListManagedSchemaArnsInput = struct {
...
pub const ListManagedSchemaArnsOutput = struct {
    /// The pagination token.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedSchemaArnsInput, options: Options) !ListManagedSchemaArnsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedSchemaArnsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/schema/managed";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedSchemaArnsOutput {
    var result: ListManagedSchemaArnsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedSchemaArnsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
