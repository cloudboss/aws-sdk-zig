...
pub const ListAttachedIndicesInput = struct {
...
pub const ListAttachedIndicesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAttachedIndicesInput, options: Options) !ListAttachedIndicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAttachedIndicesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/object/indices";
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TargetReference\":");
    try aws.json.writeValue(@TypeOf(input.target_reference), input.target_reference, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAttachedIndicesOutput {
    var result: ListAttachedIndicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAttachedIndicesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
