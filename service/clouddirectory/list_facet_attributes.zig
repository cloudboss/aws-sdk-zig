...
pub const ListFacetAttributesInput = struct {
...
pub const ListFacetAttributesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFacetAttributesInput, options: CallOptions) !ListFacetAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFacetAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/facet/attributes";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFacetAttributesOutput {
    var result: ListFacetAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFacetAttributesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
