...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFacetAttributesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFacetAttributesOutput {
    var result: ListFacetAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFacetAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
