...
pub const ListFacetNamesInput = struct {
...
pub const ListFacetNamesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFacetNamesInput, options: Options) !ListFacetNamesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFacetNamesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/facet/list";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFacetNamesOutput {
    var result: ListFacetNamesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFacetNamesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
