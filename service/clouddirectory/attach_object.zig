...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachObjectInput, options: Options) !AttachObjectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachObjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ChildReference\":");
    try aws.json.writeValue(@TypeOf(input.child_reference), input.child_reference, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"LinkName\":");
    try aws.json.writeValue(@TypeOf(input.link_name), input.link_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ParentReference\":");
    try aws.json.writeValue(@TypeOf(input.parent_reference), input.parent_reference, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachObjectOutput {
    var result: AttachObjectOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AttachObjectOutput, body, alloc);
    }
    _ = status;
    _ = headers;
