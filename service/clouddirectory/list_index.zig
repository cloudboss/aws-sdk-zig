...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIndexInput, options: Options) !ListIndexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIndexInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ranges_on_indexed_values) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RangesOnIndexedValues\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIndexOutput {
    var result: ListIndexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIndexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
