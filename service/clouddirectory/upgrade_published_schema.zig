...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpgradePublishedSchemaInput, options: Options) !UpgradePublishedSchemaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpgradePublishedSchemaInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"DevelopmentSchemaArn\":");
    try aws.json.writeValue(@TypeOf(input.development_schema_arn), input.development_schema_arn, alloc, &body_buf);
    has_prev = true;
    if (input.dry_run) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.minor_version), input.minor_version, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpgradePublishedSchemaOutput {
    var result: UpgradePublishedSchemaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpgradePublishedSchemaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
