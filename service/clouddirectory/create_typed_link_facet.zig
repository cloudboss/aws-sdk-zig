...
const TypedLinkFacet = @import("typed_link_facet.zig").TypedLinkFacet;

pub const CreateTypedLinkFacetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTypedLinkFacetInput, options: Options) !CreateTypedLinkFacetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTypedLinkFacetInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/typedlink/facet/create";
    ...
    try body_buf.appendSlice(alloc, "\"Facet\":");
    try aws.json.writeValue(@TypeOf(input.facet), input.facet, alloc, &body_buf);
    has_prev = true;
    ...
    try request.headers.put(alloc, "x-amz-data-partition", input.schema_arn);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTypedLinkFacetOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateTypedLinkFacetOutput = .{};
