...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTypedLinkFacetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"Facet\":");
    try aws.json.writeValue(@TypeOf(input.facet), input.facet, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTypedLinkFacetOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
