...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSchemaAsJsonInput, options: Options) !GetSchemaAsJsonOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSchemaAsJsonInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSchemaAsJsonOutput {
    var result: GetSchemaAsJsonOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSchemaAsJsonOutput, body, alloc);
    }
    _ = status;
    _ = headers;
