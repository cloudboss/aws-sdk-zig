...
pub const ListDevelopmentSchemaArnsInput = struct {
...
pub const ListDevelopmentSchemaArnsOutput = struct {
    /// The pagination token.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDevelopmentSchemaArnsInput, options: Options) !ListDevelopmentSchemaArnsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDevelopmentSchemaArnsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/schema/development";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDevelopmentSchemaArnsOutput {
    var result: ListDevelopmentSchemaArnsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevelopmentSchemaArnsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
