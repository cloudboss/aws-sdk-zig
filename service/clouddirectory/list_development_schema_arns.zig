...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDevelopmentSchemaArnsInput, options: CallOptions) !ListDevelopmentSchemaArnsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDevelopmentSchemaArnsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDevelopmentSchemaArnsOutput {
    var result: ListDevelopmentSchemaArnsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevelopmentSchemaArnsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
