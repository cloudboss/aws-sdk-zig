...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFacetInput, options: Options) !CreateFacetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFacetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.facet_style) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FacetStyle\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.object_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ObjectType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFacetOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
