...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ApplySchemaInput, options: Options) !ApplySchemaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ApplySchemaInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ApplySchemaOutput {
    var result: ApplySchemaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ApplySchemaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
