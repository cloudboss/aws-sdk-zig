...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateObjectInput, options: Options) !CreateObjectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateObjectInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.link_name) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"LinkName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.object_attribute_list) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ObjectAttributeList\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.parent_reference) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ParentReference\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SchemaFacets\":");
    try aws.json.writeValue(@TypeOf(input.schema_facets), input.schema_facets, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateObjectOutput {
    var result: CreateObjectOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateObjectOutput, body, alloc);
    }
    _ = status;
    _ = headers;
