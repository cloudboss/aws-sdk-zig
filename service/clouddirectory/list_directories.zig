...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDirectoriesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDirectoriesOutput {
    var result: ListDirectoriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDirectoriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
