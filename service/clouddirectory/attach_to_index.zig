...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachToIndexInput, options: Options) !AttachToIndexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachToIndexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachToIndexOutput {
    var result: AttachToIndexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AttachToIndexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
