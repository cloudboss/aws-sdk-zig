...
pub const ListObjectChildrenInput = struct {
...
pub const ListObjectChildrenOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObjectChildrenInput, options: Options) !ListObjectChildrenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListObjectChildrenInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/object/children";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListObjectChildrenOutput {
    var result: ListObjectChildrenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListObjectChildrenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
