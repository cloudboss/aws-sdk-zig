...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObjectChildrenInput, options: CallOptions) !ListObjectChildrenOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListObjectChildrenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListObjectChildrenOutput {
    var result: ListObjectChildrenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListObjectChildrenOutput, body, allocator);
    }
    _ = status;
    _ = headers;
