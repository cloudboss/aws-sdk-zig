...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateObjectAttributesInput, options: Options) !UpdateObjectAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateObjectAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.attribute_updates), input.attribute_updates, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateObjectAttributesOutput {
    var result: UpdateObjectAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateObjectAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
