...
pub const ListAppliedSchemaArnsInput = struct {
...
pub const ListAppliedSchemaArnsOutput = struct {
    /// The pagination token.
    next_token: ?[]const u8 = null,

    /// The ARNs of schemas that are applied to the directory.
    schema_arns: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppliedSchemaArnsInput, options: Options) !ListAppliedSchemaArnsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppliedSchemaArnsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/schema/applied";
    ...
    try body_buf.appendSlice(alloc, "\"DirectoryArn\":");
    try aws.json.writeValue(@TypeOf(input.directory_arn), input.directory_arn, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.schema_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SchemaArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppliedSchemaArnsOutput {
    var result: ListAppliedSchemaArnsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppliedSchemaArnsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
