...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppliedSchemaArnsInput, options: CallOptions) !ListAppliedSchemaArnsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAppliedSchemaArnsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAppliedSchemaArnsOutput {
    var result: ListAppliedSchemaArnsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppliedSchemaArnsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
