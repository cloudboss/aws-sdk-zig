...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDirectoryInput, options: CallOptions) !DeleteDirectoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDirectoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDirectoryOutput {
    var result: DeleteDirectoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDirectoryOutput, body, allocator);
    }
    _ = status;
    _ = headers;
