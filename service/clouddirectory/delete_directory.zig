...
pub const DeleteDirectoryInput = struct {
...
pub const DeleteDirectoryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDirectoryInput, options: Options) !DeleteDirectoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDirectoryInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/directory";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDirectoryOutput {
    var result: DeleteDirectoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDirectoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
