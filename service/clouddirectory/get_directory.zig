...
pub const GetDirectoryInput = struct {
...
pub const GetDirectoryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDirectoryInput, options: Options) !GetDirectoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDirectoryInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/directory/get";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDirectoryOutput {
    var result: GetDirectoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDirectoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
