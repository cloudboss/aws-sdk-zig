...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFacetInput, options: Options) !GetFacetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFacetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFacetOutput {
    var result: GetFacetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFacetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
