...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLinkAttributesInput, options: Options) !GetLinkAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLinkAttributesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLinkAttributesOutput {
    var result: GetLinkAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLinkAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
