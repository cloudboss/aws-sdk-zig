...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObjectPoliciesInput, options: Options) !ListObjectPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListObjectPoliciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListObjectPoliciesOutput {
    var result: ListObjectPoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListObjectPoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
