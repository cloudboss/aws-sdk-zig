...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: LookupPolicyInput, options: Options) !LookupPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: LookupPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !LookupPolicyOutput {
    var result: LookupPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(LookupPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
