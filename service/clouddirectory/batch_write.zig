...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchWriteInput, options: Options) !BatchWriteOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchWriteInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.operations), input.operations, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchWriteOutput {
    var result: BatchWriteOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchWriteOutput, body, alloc);
    }
    _ = status;
    _ = headers;
