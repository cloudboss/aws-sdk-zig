...
pub const DisableDirectoryInput = struct {
...
pub const DisableDirectoryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableDirectoryInput, options: Options) !DisableDirectoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableDirectoryInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/directory/disable";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableDirectoryOutput {
    var result: DisableDirectoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisableDirectoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
