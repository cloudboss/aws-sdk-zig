...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetObjectInformationInput, options: Options) !GetObjectInformationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetObjectInformationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetObjectInformationOutput {
    var result: GetObjectInformationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetObjectInformationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
