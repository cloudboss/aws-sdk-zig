...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObjectAttributesInput, options: CallOptions) !ListObjectAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListObjectAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.facet_filter) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"FacetFilter\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListObjectAttributesOutput {
    var result: ListObjectAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListObjectAttributesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
