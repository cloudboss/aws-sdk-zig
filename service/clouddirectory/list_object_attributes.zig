...
pub const ListObjectAttributesInput = struct {
...
pub const ListObjectAttributesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObjectAttributesInput, options: Options) !ListObjectAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListObjectAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/object/attributes";

    var body_buf: std.ArrayList(u8) = .{};
    var has_prev = false;
    try body_buf.appendSlice(alloc, "{");

    if (input.facet_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FacetFilter\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListObjectAttributesOutput {
    var result: ListObjectAttributesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListObjectAttributesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
