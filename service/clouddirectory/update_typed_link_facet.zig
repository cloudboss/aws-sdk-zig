...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTypedLinkFacetInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"IdentityAttributeOrder\":");
    try aws.json.writeValue(@TypeOf(input.identity_attribute_order), input.identity_attribute_order, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTypedLinkFacetOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
