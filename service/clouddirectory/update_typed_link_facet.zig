...
const TypedLinkFacetAttributeUpdate = @import("typed_link_facet_attribute_update.zig").TypedLinkFacetAttributeUpdate;

pub const UpdateTypedLinkFacetInput = struct {
    /// Attributes update structure.
    attribute_updates: []const TypedLinkFacetAttributeUpdate,
    ...
    /// facet, not the order
    /// in which they are supplied to any API calls. For more information about
    /// identity attributes, see [Typed
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTypedLinkFacetInput, options: Options) !UpdateTypedLinkFacetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTypedLinkFacetInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/typedlink/facet";
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"IdentityAttributeOrder\":");
    try aws.json.writeValue(@TypeOf(input.identity_attribute_order), input.identity_attribute_order, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTypedLinkFacetOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateTypedLinkFacetOutput = .{};
