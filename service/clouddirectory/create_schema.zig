...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSchemaInput, options: Options) !CreateSchemaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSchemaInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/schema/create";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSchemaOutput {
    var result: CreateSchemaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSchemaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
