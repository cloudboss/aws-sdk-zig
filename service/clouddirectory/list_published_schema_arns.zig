...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPublishedSchemaArnsInput, options: CallOptions) !ListPublishedSchemaArnsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPublishedSchemaArnsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.schema_arn) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"SchemaArn\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPublishedSchemaArnsOutput {
    var result: ListPublishedSchemaArnsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPublishedSchemaArnsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
