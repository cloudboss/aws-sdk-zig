...
pub const ListPublishedSchemaArnsInput = struct {
...
pub const ListPublishedSchemaArnsOutput = struct {
    /// The pagination token.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPublishedSchemaArnsInput, options: Options) !ListPublishedSchemaArnsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPublishedSchemaArnsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/schema/published";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPublishedSchemaArnsOutput {
    var result: ListPublishedSchemaArnsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPublishedSchemaArnsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
