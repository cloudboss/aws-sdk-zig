...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObjectParentPathsInput, options: Options) !ListObjectParentPathsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListObjectParentPathsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListObjectParentPathsOutput {
    var result: ListObjectParentPathsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListObjectParentPathsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
