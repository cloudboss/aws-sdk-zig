...
pub const DetachTypedLinkInput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachTypedLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/amazonclouddirectory/2017-01-11/typedlink/detach";
    ...
    try body_buf.appendSlice(alloc, "\"TypedLinkSpecifier\":");
    try aws.json.writeValue(@TypeOf(input.typed_link_specifier), input.typed_link_specifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachTypedLinkOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DetachTypedLinkOutput = .{};
