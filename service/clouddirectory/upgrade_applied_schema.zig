...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpgradeAppliedSchemaInput, options: Options) !UpgradeAppliedSchemaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpgradeAppliedSchemaInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PublishedSchemaArn\":");
    try aws.json.writeValue(@TypeOf(input.published_schema_arn), input.published_schema_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpgradeAppliedSchemaOutput {
    var result: UpgradeAppliedSchemaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpgradeAppliedSchemaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
