...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachObjectInput, options: Options) !DetachObjectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachObjectInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachObjectOutput {
    var result: DetachObjectOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DetachObjectOutput, body, alloc);
    }
    _ = status;
    _ = headers;
