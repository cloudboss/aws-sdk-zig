...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTypedLinkFacetNamesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTypedLinkFacetNamesOutput {
    var result: ListTypedLinkFacetNamesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTypedLinkFacetNamesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
