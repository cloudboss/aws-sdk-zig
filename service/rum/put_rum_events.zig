...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRumEventsInput, options: Options) !PutRumEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRumEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/appmonitors/");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"AppMonitorDetails\":");
    try aws.json.writeValue(@TypeOf(input.app_monitor_details), input.app_monitor_details, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"BatchId\":");
    try aws.json.writeValue(@TypeOf(input.batch_id), input.batch_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RumEvents\":");
    try aws.json.writeValue(@TypeOf(input.rum_events), input.rum_events, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UserDetails\":");
    try aws.json.writeValue(@TypeOf(input.user_details), input.user_details, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRumEventsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
