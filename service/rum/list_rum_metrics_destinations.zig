...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRumMetricsDestinationsInput, options: Options) !ListRumMetricsDestinationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRumMetricsDestinationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRumMetricsDestinationsOutput {
    var result: ListRumMetricsDestinationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRumMetricsDestinationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
