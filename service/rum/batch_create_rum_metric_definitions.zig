...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateRumMetricDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MetricDefinitions\":");
    try aws.json.writeValue(@TypeOf(input.metric_definitions), input.metric_definitions, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateRumMetricDefinitionsOutput {
    var result: BatchCreateRumMetricDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchCreateRumMetricDefinitionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
