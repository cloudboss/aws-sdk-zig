...
const AppMonitor = @import("app_monitor.zig").AppMonitor;

pub const GetAppMonitorInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAppMonitorInput, options: CallOptions) !GetAppMonitorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAppMonitorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAppMonitorOutput {
    var result: GetAppMonitorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAppMonitorOutput, body, allocator);
    }
    _ = status;
    _ = headers;
