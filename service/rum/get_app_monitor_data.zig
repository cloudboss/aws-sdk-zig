...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAppMonitorDataInput, options: Options) !GetAppMonitorDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAppMonitorDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/appmonitor/");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TimeRange\":");
    try aws.json.writeValue(@TypeOf(input.time_range), input.time_range, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAppMonitorDataOutput {
    var result: GetAppMonitorDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAppMonitorDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
