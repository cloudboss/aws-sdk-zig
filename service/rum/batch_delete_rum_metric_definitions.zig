...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteRumMetricDefinitionsInput, options: Options) !BatchDeleteRumMetricDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteRumMetricDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "metricDefinitionIds=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.metric_definition_ids);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteRumMetricDefinitionsOutput {
    var result: BatchDeleteRumMetricDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteRumMetricDefinitionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
