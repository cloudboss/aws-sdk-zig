...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetRumMetricDefinitionsInput, options: CallOptions) !BatchGetRumMetricDefinitionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchGetRumMetricDefinitionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchGetRumMetricDefinitionsOutput {
    var result: BatchGetRumMetricDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetRumMetricDefinitionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
