...
pub const BatchGetRumMetricDefinitionsInput = struct {
...
pub const BatchGetRumMetricDefinitionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetRumMetricDefinitionsInput, options: Options) !BatchGetRumMetricDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetRumMetricDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "destination=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.destination));
    query_has_prev = true;
    if (input.destination_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "destinationArn=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetRumMetricDefinitionsOutput {
    var result: BatchGetRumMetricDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetRumMetricDefinitionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
