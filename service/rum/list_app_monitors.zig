...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppMonitorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppMonitorsOutput {
    var result: ListAppMonitorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppMonitorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
