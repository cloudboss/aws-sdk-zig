...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteConnectionInput, options: Options) !BatchDeleteConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchDeleteConnection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteConnectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDeleteConnectionOutput, body, alloc);
}
