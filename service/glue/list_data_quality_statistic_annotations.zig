...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataQualityStatisticAnnotationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListDataQualityStatisticAnnotations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataQualityStatisticAnnotationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDataQualityStatisticAnnotationsOutput, body, alloc);
}
