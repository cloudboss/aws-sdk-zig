...
const TimestampFilter = @import("timestamp_filter.zig").TimestampFilter;
const StatisticAnnotation = @import("statistic_annotation.zig").StatisticAnnotation;

pub const ListDataQualityStatisticAnnotationsInput = struct {
    ...
    /// A timestamp filter.
    timestamp_filter: ?TimestampFilter = null,
...
pub const ListDataQualityStatisticAnnotationsOutput = struct {
    /// A list of `StatisticAnnotation` applied to the Statistic
    annotations: ?[]const StatisticAnnotation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataQualityStatisticAnnotationsInput, options: CallOptions) !ListDataQualityStatisticAnnotationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDataQualityStatisticAnnotationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListDataQualityStatisticAnnotations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDataQualityStatisticAnnotationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDataQualityStatisticAnnotationsOutput, body, allocator);
}
