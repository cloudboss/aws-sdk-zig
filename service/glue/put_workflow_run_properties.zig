...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutWorkflowRunPropertiesInput, options: Options) !PutWorkflowRunPropertiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutWorkflowRunPropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.PutWorkflowRunProperties");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutWorkflowRunPropertiesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
