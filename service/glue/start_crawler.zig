...
pub const StartCrawlerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartCrawlerInput, options: Options) !StartCrawlerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartCrawlerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartCrawler");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartCrawlerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
