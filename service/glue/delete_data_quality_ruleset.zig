...
pub const DeleteDataQualityRulesetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDataQualityRulesetInput, options: Options) !DeleteDataQualityRulesetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDataQualityRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteDataQualityRuleset");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDataQualityRulesetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
