...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCatalogInput, options: Options) !CreateCatalogOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CreateCatalog");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCatalogOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
