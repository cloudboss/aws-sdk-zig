...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateClassifierInput, options: Options) !UpdateClassifierOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateClassifierInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateClassifier");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateClassifierOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
