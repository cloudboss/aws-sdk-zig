...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteWorkflowInput, options: Options) !DeleteWorkflowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteWorkflowInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.DeleteWorkflow");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteWorkflowOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteWorkflowOutput, body, alloc);
}
