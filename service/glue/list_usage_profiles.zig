...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUsageProfilesInput, options: Options) !ListUsageProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUsageProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListUsageProfiles");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUsageProfilesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUsageProfilesOutput, body, alloc);
}
