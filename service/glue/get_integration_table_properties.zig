...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIntegrationTablePropertiesInput, options: Options) !GetIntegrationTablePropertiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIntegrationTablePropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetIntegrationTableProperties");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIntegrationTablePropertiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetIntegrationTablePropertiesOutput, body, alloc);
}
