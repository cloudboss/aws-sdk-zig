...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBlueprintsInput, options: Options) !ListBlueprintsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBlueprintsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListBlueprints");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBlueprintsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBlueprintsOutput, body, alloc);
}
