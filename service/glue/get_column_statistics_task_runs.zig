...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetColumnStatisticsTaskRunsInput, options: Options) !GetColumnStatisticsTaskRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetColumnStatisticsTaskRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetColumnStatisticsTaskRuns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetColumnStatisticsTaskRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetColumnStatisticsTaskRunsOutput, body, alloc);
}
