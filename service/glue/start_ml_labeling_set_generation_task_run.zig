...
pub const StartMLLabelingSetGenerationTaskRunInput = struct {
...
pub const StartMLLabelingSetGenerationTaskRunOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMLLabelingSetGenerationTaskRunInput, options: CallOptions) !StartMLLabelingSetGenerationTaskRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartMLLabelingSetGenerationTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartMLLabelingSetGenerationTaskRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartMLLabelingSetGenerationTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMLLabelingSetGenerationTaskRunOutput, body, allocator);
}
