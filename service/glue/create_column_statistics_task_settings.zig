...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateColumnStatisticsTaskSettingsInput, options: Options) !CreateColumnStatisticsTaskSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateColumnStatisticsTaskSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CreateColumnStatisticsTaskSettings");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateColumnStatisticsTaskSettingsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
