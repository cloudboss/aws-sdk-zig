...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDataQualityRuleRecommendationRunInput, options: Options) !StartDataQualityRuleRecommendationRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDataQualityRuleRecommendationRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.StartDataQualityRuleRecommendationRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDataQualityRuleRecommendationRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartDataQualityRuleRecommendationRunOutput, body, alloc);
}
