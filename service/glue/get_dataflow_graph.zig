...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataflowGraphInput, options: Options) !GetDataflowGraphOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataflowGraphInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetDataflowGraph");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataflowGraphOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDataflowGraphOutput, body, alloc);
}
