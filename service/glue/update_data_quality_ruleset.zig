...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDataQualityRulesetInput, options: Options) !UpdateDataQualityRulesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataQualityRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateDataQualityRuleset");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataQualityRulesetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateDataQualityRulesetOutput, body, alloc);
}
