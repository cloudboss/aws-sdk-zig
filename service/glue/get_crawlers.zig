...
pub const GetCrawlersInput = struct {
...
pub const GetCrawlersOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCrawlersInput, options: CallOptions) !GetCrawlersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCrawlersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetCrawlers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCrawlersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCrawlersOutput, body, allocator);
}
