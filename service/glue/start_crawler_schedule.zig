...
pub const StartCrawlerScheduleInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartCrawlerScheduleInput, options: Options) !StartCrawlerScheduleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartCrawlerScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartCrawlerSchedule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartCrawlerScheduleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
