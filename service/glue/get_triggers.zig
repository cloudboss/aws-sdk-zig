...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTriggersInput, options: Options) !GetTriggersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTriggersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetTriggers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTriggersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTriggersOutput, body, alloc);
}
