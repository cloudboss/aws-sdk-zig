...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUserDefinedFunctionsInput, options: Options) !GetUserDefinedFunctionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUserDefinedFunctionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetUserDefinedFunctions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUserDefinedFunctionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUserDefinedFunctionsOutput, body, alloc);
}
