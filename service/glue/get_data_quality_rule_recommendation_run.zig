...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataQualityRuleRecommendationRunInput, options: Options) !GetDataQualityRuleRecommendationRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataQualityRuleRecommendationRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetDataQualityRuleRecommendationRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataQualityRuleRecommendationRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDataQualityRuleRecommendationRunOutput, body, alloc);
}
