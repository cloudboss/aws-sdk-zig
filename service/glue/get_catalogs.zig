...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCatalogsInput, options: Options) !GetCatalogsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCatalogsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetCatalogs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCatalogsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCatalogsOutput, body, alloc);
}
