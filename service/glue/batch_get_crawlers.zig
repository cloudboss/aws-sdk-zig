...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetCrawlersInput, options: Options) !BatchGetCrawlersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetCrawlersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchGetCrawlers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetCrawlersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetCrawlersOutput, body, alloc);
}
