...
pub const DeleteTableOptimizerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTableOptimizerInput, options: Options) !DeleteTableOptimizerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTableOptimizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteTableOptimizer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTableOptimizerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
