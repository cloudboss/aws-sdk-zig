...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBlueprintInput, options: Options) !GetBlueprintOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBlueprintInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetBlueprint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBlueprintOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetBlueprintOutput, body, alloc);
}
