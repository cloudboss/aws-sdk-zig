...
pub const ListMLTransformsInput = struct {
...
pub const ListMLTransformsOutput = struct {
    ...
    transform_ids: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMLTransformsInput, options: CallOptions) !ListMLTransformsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMLTransformsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListMLTransforms");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMLTransformsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListMLTransformsOutput, body, allocator);
}
