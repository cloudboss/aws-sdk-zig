...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBlueprintRunsInput, options: Options) !GetBlueprintRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBlueprintRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetBlueprintRuns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBlueprintRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetBlueprintRunsOutput, body, alloc);
}
