...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResourcePoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetResourcePolicies");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResourcePoliciesOutput {
    ...
    return aws.json.parseJsonObject(GetResourcePoliciesOutput, body, allocator);
}
