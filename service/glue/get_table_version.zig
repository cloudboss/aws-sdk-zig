...
pub const GetTableVersionInput = struct {
...
pub const GetTableVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableVersionInput, options: CallOptions) !GetTableVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTableVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetTableVersion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTableVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTableVersionOutput, body, allocator);
}
