...
pub const DeletePartitionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePartitionInput, options: Options) !DeletePartitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeletePartition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePartitionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
