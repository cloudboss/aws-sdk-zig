...
const TriggerUpdate = @import("trigger_update.zig").TriggerUpdate;
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTriggerInput, options: Options) !UpdateTriggerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTriggerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateTrigger");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTriggerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateTriggerOutput, body, alloc);
}
