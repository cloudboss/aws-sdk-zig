...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePartitionInput, options: Options) !UpdatePartitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdatePartition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePartitionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
