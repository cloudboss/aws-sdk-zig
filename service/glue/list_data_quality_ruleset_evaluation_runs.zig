...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataQualityRulesetEvaluationRunsInput, options: Options) !ListDataQualityRulesetEvaluationRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataQualityRulesetEvaluationRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListDataQualityRulesetEvaluationRuns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataQualityRulesetEvaluationRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDataQualityRulesetEvaluationRunsOutput, body, alloc);
}
