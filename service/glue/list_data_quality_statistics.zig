...
const StatisticSummary = @import("statistic_summary.zig").StatisticSummary;

pub const ListDataQualityStatisticsInput = struct {
...
pub const ListDataQualityStatisticsOutput = struct {
    /// A pagination token to request the next page of results.
    next_token: ?[]const u8 = null,

    /// A `StatisticSummaryList`.
    statistics: ?[]const StatisticSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataQualityStatisticsInput, options: CallOptions) !ListDataQualityStatisticsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDataQualityStatisticsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListDataQualityStatistics");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDataQualityStatisticsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDataQualityStatisticsOutput, body, allocator);
}
