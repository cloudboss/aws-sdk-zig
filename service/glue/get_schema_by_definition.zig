...
pub const GetSchemaByDefinitionInput = struct {
...
pub const GetSchemaByDefinitionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSchemaByDefinitionInput, options: CallOptions) !GetSchemaByDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSchemaByDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetSchemaByDefinition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSchemaByDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSchemaByDefinitionOutput, body, allocator);
}
