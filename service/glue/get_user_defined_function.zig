...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUserDefinedFunctionInput, options: Options) !GetUserDefinedFunctionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUserDefinedFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetUserDefinedFunction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUserDefinedFunctionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUserDefinedFunctionOutput, body, alloc);
}
