...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdatePartitionInput, options: Options) !BatchUpdatePartitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdatePartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchUpdatePartition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdatePartitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchUpdatePartitionOutput, body, alloc);
}
