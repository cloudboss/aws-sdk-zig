...
pub const ResumeWorkflowRunInput = struct {
...
pub const ResumeWorkflowRunOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResumeWorkflowRunInput, options: CallOptions) !ResumeWorkflowRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResumeWorkflowRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ResumeWorkflowRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResumeWorkflowRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResumeWorkflowRunOutput, body, allocator);
}
