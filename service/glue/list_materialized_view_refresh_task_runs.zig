...
const MaterializedViewRefreshTaskRun = @import("materialized_view_refresh_task_run.zig").MaterializedViewRefreshTaskRun;

pub const ListMaterializedViewRefreshTaskRunsInput = struct {
...
pub const ListMaterializedViewRefreshTaskRunsOutput = struct {
    /// The results of the ListMaterializedViewRefreshTaskRuns action.
    materialized_view_refresh_task_runs: ?[]const MaterializedViewRefreshTaskRun = null,

    /// A continuation token, if not all task run IDs have yet been returned.
    next_token: ?[]const u8 = null,

    pub const json_field_names = .{
        .materialized_view_refresh_task_runs = "MaterializedViewRefreshTaskRuns",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMaterializedViewRefreshTaskRunsInput, options: CallOptions) !ListMaterializedViewRefreshTaskRunsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMaterializedViewRefreshTaskRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListMaterializedViewRefreshTaskRuns");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMaterializedViewRefreshTaskRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMaterializedViewRefreshTaskRunsOutput, body, allocator);
}
