...
pub const ResetJobBookmarkInput = struct {
...
pub const ResetJobBookmarkOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetJobBookmarkInput, options: CallOptions) !ResetJobBookmarkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResetJobBookmarkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ResetJobBookmark");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResetJobBookmarkOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResetJobBookmarkOutput, body, allocator);
}
