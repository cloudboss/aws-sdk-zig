...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUnfilteredPartitionMetadataInput, options: Options) !GetUnfilteredPartitionMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUnfilteredPartitionMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetUnfilteredPartitionMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUnfilteredPartitionMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUnfilteredPartitionMetadataOutput, body, alloc);
}
