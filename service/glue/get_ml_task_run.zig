...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMLTaskRunInput, options: Options) !GetMLTaskRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMLTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetMLTaskRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMLTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMLTaskRunOutput, body, alloc);
}
