...
pub const StartImportLabelsTaskRunInput = struct {
    ...
    pub const json_field_names = .{
        ...
        .replace_all_labels = "ReplaceAllLabels",
        .transform_id = "TransformId",
    };
};

pub const StartImportLabelsTaskRunOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartImportLabelsTaskRunInput, options: CallOptions) !StartImportLabelsTaskRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartImportLabelsTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartImportLabelsTaskRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartImportLabelsTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartImportLabelsTaskRunOutput, body, allocator);
}
