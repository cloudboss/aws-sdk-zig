...
pub const GetMaterializedViewRefreshTaskRunInput = struct {
...
pub const GetMaterializedViewRefreshTaskRunOutput = struct {
    /// A MaterializedViewRefreshTaskRun object representing the details of the task
    /// run.
    materialized_view_refresh_task_run: ?MaterializedViewRefreshTaskRun = null,

    pub const json_field_names = .{
        .materialized_view_refresh_task_run = "MaterializedViewRefreshTaskRun",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMaterializedViewRefreshTaskRunInput, options: CallOptions) !GetMaterializedViewRefreshTaskRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMaterializedViewRefreshTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetMaterializedViewRefreshTaskRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMaterializedViewRefreshTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMaterializedViewRefreshTaskRunOutput, body, allocator);
}
