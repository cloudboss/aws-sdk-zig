...
pub const UpdateSourceControlFromJobInput = struct {
...
pub const UpdateSourceControlFromJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSourceControlFromJobInput, options: CallOptions) !UpdateSourceControlFromJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSourceControlFromJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.UpdateSourceControlFromJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateSourceControlFromJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSourceControlFromJobOutput, body, allocator);
}
