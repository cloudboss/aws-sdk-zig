...
pub const GetTriggerInput = struct {
...
pub const GetTriggerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTriggerInput, options: CallOptions) !GetTriggerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTriggerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetTrigger");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTriggerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTriggerOutput, body, allocator);
}
