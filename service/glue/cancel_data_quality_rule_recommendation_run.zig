...
pub const CancelDataQualityRuleRecommendationRunInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelDataQualityRuleRecommendationRunInput, options: Options) !CancelDataQualityRuleRecommendationRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelDataQualityRuleRecommendationRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.CancelDataQualityRuleRecommendationRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelDataQualityRuleRecommendationRunOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
