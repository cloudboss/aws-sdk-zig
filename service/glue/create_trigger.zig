...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTriggerInput, options: Options) !CreateTriggerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTriggerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CreateTrigger");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTriggerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTriggerOutput, body, alloc);
}
