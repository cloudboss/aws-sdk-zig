...
const PartitionError = @import("partition_error.zig").PartitionError;

pub const BatchCreatePartitionInput = struct {
    ...
    partition_input_list: []const PartitionInput,
...
pub const BatchCreatePartitionOutput = struct {
    /// The errors encountered when trying to create the requested partitions.
    errors: ?[]const PartitionError = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreatePartitionInput, options: CallOptions) !BatchCreatePartitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchCreatePartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.BatchCreatePartition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchCreatePartitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchCreatePartitionOutput, body, allocator);
}
