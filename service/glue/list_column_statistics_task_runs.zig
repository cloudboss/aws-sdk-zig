...
pub const ListColumnStatisticsTaskRunsInput = struct {
...
pub const ListColumnStatisticsTaskRunsOutput = struct {
    /// A list of column statistics task run IDs.
    column_statistics_task_run_ids: ?[]const []const u8 = null,

    /// A continuation token, if not all task run IDs have yet been returned.
    next_token: ?[]const u8 = null,

    pub const json_field_names = .{
        .column_statistics_task_run_ids = "ColumnStatisticsTaskRunIds",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListColumnStatisticsTaskRunsInput, options: CallOptions) !ListColumnStatisticsTaskRunsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListColumnStatisticsTaskRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListColumnStatisticsTaskRuns");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListColumnStatisticsTaskRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListColumnStatisticsTaskRunsOutput, body, allocator);
}
