...
pub const GetClassifierInput = struct {
...
pub const GetClassifierOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetClassifierInput, options: CallOptions) !GetClassifierOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetClassifierInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetClassifier");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetClassifierOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetClassifierOutput, body, allocator);
}
