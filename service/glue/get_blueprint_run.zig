...
pub const GetBlueprintRunInput = struct {
...
pub const GetBlueprintRunOutput = struct {
    /// Returns a `BlueprintRun` object.
    blueprint_run: ?BlueprintRun = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBlueprintRunInput, options: CallOptions) !GetBlueprintRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBlueprintRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetBlueprintRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBlueprintRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetBlueprintRunOutput, body, allocator);
}
