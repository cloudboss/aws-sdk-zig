...
pub const StartExportLabelsTaskRunInput = struct {
...
pub const StartExportLabelsTaskRunOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartExportLabelsTaskRunInput, options: CallOptions) !StartExportLabelsTaskRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartExportLabelsTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartExportLabelsTaskRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartExportLabelsTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartExportLabelsTaskRunOutput, body, allocator);
}
