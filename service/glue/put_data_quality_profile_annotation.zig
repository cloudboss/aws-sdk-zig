...
pub const PutDataQualityProfileAnnotationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDataQualityProfileAnnotationInput, options: Options) !PutDataQualityProfileAnnotationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutDataQualityProfileAnnotationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.PutDataQualityProfileAnnotation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutDataQualityProfileAnnotationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
