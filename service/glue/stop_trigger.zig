...
pub const StopTriggerInput = struct {
...
pub const StopTriggerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopTriggerInput, options: CallOptions) !StopTriggerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopTriggerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StopTrigger");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopTriggerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopTriggerOutput, body, allocator);
}
