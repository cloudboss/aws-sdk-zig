...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIntegrationResourcePropertyInput, options: Options) !CreateIntegrationResourcePropertyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIntegrationResourcePropertyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CreateIntegrationResourceProperty");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIntegrationResourcePropertyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateIntegrationResourcePropertyOutput, body, alloc);
}
