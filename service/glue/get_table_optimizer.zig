...
pub const GetTableOptimizerInput = struct {
...
pub const GetTableOptimizerOutput = struct {
    /// The Catalog ID of the table.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableOptimizerInput, options: CallOptions) !GetTableOptimizerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTableOptimizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetTableOptimizer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTableOptimizerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTableOptimizerOutput, body, allocator);
}
