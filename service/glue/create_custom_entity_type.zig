...
pub const CreateCustomEntityTypeInput = struct {
...
pub const CreateCustomEntityTypeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomEntityTypeInput, options: CallOptions) !CreateCustomEntityTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateCustomEntityTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.CreateCustomEntityType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateCustomEntityTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCustomEntityTypeOutput, body, allocator);
}
