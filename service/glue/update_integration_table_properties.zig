...
pub const UpdateIntegrationTablePropertiesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIntegrationTablePropertiesInput, options: Options) !UpdateIntegrationTablePropertiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateIntegrationTablePropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.UpdateIntegrationTableProperties");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateIntegrationTablePropertiesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
