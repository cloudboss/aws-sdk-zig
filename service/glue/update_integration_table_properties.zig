...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIntegrationTablePropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateIntegrationTableProperties");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIntegrationTablePropertiesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
