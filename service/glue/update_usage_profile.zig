...
pub const UpdateUsageProfileInput = struct {
    /// A `ProfileConfiguration` object specifying the job and session values for
    /// the profile.
...
pub const UpdateUsageProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUsageProfileInput, options: CallOptions) !UpdateUsageProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateUsageProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.UpdateUsageProfile");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateUsageProfileOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateUsageProfileOutput, body, allocator);
}
