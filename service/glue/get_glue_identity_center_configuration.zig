...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGlueIdentityCenterConfigurationInput, options: CallOptions) !GetGlueIdentityCenterConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetGlueIdentityCenterConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetGlueIdentityCenterConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetGlueIdentityCenterConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetGlueIdentityCenterConfigurationOutput, body, allocator);
}
