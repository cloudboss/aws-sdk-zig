...
pub const GetDevEndpointsInput = struct {
...
pub const GetDevEndpointsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDevEndpointsInput, options: CallOptions) !GetDevEndpointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDevEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetDevEndpoints");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDevEndpointsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDevEndpointsOutput, body, allocator);
}
