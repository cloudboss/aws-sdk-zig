...
const DatabaseInput = @import("database_input.zig").DatabaseInput;
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDatabaseInput, options: Options) !UpdateDatabaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDatabaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateDatabase");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDatabaseOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
