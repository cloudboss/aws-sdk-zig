...
const CatalogImportStatus = @import("catalog_import_status.zig").CatalogImportStatus;

pub const GetCatalogImportStatusInput = struct {
...
pub const GetCatalogImportStatusOutput = struct {
    /// The status of the specified catalog migration.
    import_status: ?CatalogImportStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCatalogImportStatusInput, options: CallOptions) !GetCatalogImportStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCatalogImportStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetCatalogImportStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCatalogImportStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCatalogImportStatusOutput, body, allocator);
}
