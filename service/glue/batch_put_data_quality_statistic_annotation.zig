...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchPutDataQualityStatisticAnnotationInput, options: Options) !BatchPutDataQualityStatisticAnnotationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchPutDataQualityStatisticAnnotationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchPutDataQualityStatisticAnnotation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchPutDataQualityStatisticAnnotationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchPutDataQualityStatisticAnnotationOutput, body, alloc);
}
