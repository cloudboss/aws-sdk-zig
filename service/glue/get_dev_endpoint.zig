...
pub const GetDevEndpointInput = struct {
...
pub const GetDevEndpointOutput = struct {
    /// A `DevEndpoint` definition.
    dev_endpoint: ?DevEndpoint = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDevEndpointInput, options: CallOptions) !GetDevEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDevEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetDevEndpoint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDevEndpointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDevEndpointOutput, body, allocator);
}
