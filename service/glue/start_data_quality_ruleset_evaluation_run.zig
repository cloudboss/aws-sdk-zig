...
pub const StartDataQualityRulesetEvaluationRunInput = struct {
...
pub const StartDataQualityRulesetEvaluationRunOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDataQualityRulesetEvaluationRunInput, options: CallOptions) !StartDataQualityRulesetEvaluationRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartDataQualityRulesetEvaluationRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartDataQualityRulesetEvaluationRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartDataQualityRulesetEvaluationRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartDataQualityRulesetEvaluationRunOutput, body, allocator);
}
