...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIntegrationTablePropertiesInput, options: Options) !CreateIntegrationTablePropertiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIntegrationTablePropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CreateIntegrationTableProperties");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIntegrationTablePropertiesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
