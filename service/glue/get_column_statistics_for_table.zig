...
pub const GetColumnStatisticsForTableInput = struct {
...
pub const GetColumnStatisticsForTableOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetColumnStatisticsForTableInput, options: CallOptions) !GetColumnStatisticsForTableOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetColumnStatisticsForTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetColumnStatisticsForTable");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetColumnStatisticsForTableOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetColumnStatisticsForTableOutput, body, allocator);
}
