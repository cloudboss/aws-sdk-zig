...
pub const GetJobRunsInput = struct {
...
pub const GetJobRunsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobRunsInput, options: CallOptions) !GetJobRunsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetJobRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetJobRuns");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetJobRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetJobRunsOutput, body, allocator);
}
