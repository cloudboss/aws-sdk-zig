...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSchemaInput, options: Options) !DeleteSchemaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSchemaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.DeleteSchema");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSchemaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteSchemaOutput, body, alloc);
}
