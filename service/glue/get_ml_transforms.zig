...
const MLTransform = @import("ml_transform.zig").MLTransform;

pub const GetMLTransformsInput = struct {
...
pub const GetMLTransformsOutput = struct {
    /// A pagination token, if more results are available.
    next_token: ?[]const u8 = null,

    /// A list of machine learning transforms.
    transforms: ?[]const MLTransform = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMLTransformsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetMLTransforms");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMLTransformsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetMLTransformsOutput, body, allocator);
}
