...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMLTransformsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetMLTransforms");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMLTransformsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetMLTransformsOutput, body, alloc);
}
