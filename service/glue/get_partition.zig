...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPartitionInput, options: Options) !GetPartitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetPartition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPartitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPartitionOutput, body, alloc);
}
