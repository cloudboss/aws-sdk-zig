...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetDevEndpointsInput, options: Options) !BatchGetDevEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetDevEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchGetDevEndpoints");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetDevEndpointsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetDevEndpointsOutput, body, alloc);
}
