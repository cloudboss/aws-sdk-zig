...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRegistryInput, options: Options) !UpdateRegistryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRegistryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateRegistry");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRegistryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateRegistryOutput, body, alloc);
}
