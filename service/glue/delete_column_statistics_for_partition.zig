...
pub const DeleteColumnStatisticsForPartitionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteColumnStatisticsForPartitionInput, options: Options) !DeleteColumnStatisticsForPartitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteColumnStatisticsForPartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteColumnStatisticsForPartition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteColumnStatisticsForPartitionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
