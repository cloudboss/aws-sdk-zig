...
pub const GetCatalogInput = struct {
...
pub const GetCatalogOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCatalogInput, options: CallOptions) !GetCatalogOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetCatalog");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCatalogOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCatalogOutput, body, allocator);
}
