...
pub const StartMLEvaluationTaskRunInput = struct {
...
pub const StartMLEvaluationTaskRunOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMLEvaluationTaskRunInput, options: CallOptions) !StartMLEvaluationTaskRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartMLEvaluationTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartMLEvaluationTaskRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartMLEvaluationTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMLEvaluationTaskRunOutput, body, allocator);
}
