...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobInput, options: Options) !GetJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetJobOutput, body, alloc);
}
