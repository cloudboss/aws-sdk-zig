...
pub const BatchGetBlueprintsInput = struct {
...
pub const BatchGetBlueprintsOutput = struct {
    /// Returns a list of blueprint as a `Blueprints` object.
    blueprints: ?[]const Blueprint = null,

    /// Returns a list of `BlueprintNames` that were not found.
    missing_blueprints: ?[]const []const u8 = null,

    pub const json_field_names = .{
        .blueprints = "Blueprints",
        .missing_blueprints = "MissingBlueprints",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetBlueprintsInput, options: CallOptions) !BatchGetBlueprintsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchGetBlueprintsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.BatchGetBlueprints");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchGetBlueprintsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetBlueprintsOutput, body, allocator);
}
