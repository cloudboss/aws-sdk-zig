...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMaterializedViewRefreshTaskRunInput, options: Options) !StartMaterializedViewRefreshTaskRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMaterializedViewRefreshTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.StartMaterializedViewRefreshTaskRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMaterializedViewRefreshTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMaterializedViewRefreshTaskRunOutput, body, alloc);
}
