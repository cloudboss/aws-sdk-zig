...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCrawlerInput, options: Options) !UpdateCrawlerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCrawlerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateCrawler");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCrawlerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
