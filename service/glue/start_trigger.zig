...
pub const StartTriggerInput = struct {
...
pub const StartTriggerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTriggerInput, options: CallOptions) !StartTriggerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartTriggerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartTrigger");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartTriggerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartTriggerOutput, body, allocator);
}
