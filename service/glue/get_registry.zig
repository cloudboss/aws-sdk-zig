...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegistryInput, options: Options) !GetRegistryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRegistryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetRegistry");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRegistryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRegistryOutput, body, alloc);
}
