...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkflowRunsInput, options: Options) !GetWorkflowRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkflowRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetWorkflowRuns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkflowRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetWorkflowRunsOutput, body, alloc);
}
