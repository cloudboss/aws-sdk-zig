...
pub const UpdateCatalogInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCatalogInput, options: Options) !UpdateCatalogOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.UpdateCatalog");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateCatalogOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
