...
pub const DeleteCatalogInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCatalogInput, options: Options) !DeleteCatalogOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteCatalog");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCatalogOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
