...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataQualityRulesetInput, options: Options) !CreateDataQualityRulesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataQualityRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CreateDataQualityRuleset");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataQualityRulesetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDataQualityRulesetOutput, body, alloc);
}
