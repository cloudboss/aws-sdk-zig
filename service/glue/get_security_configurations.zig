...
pub const GetSecurityConfigurationsInput = struct {
...
pub const GetSecurityConfigurationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSecurityConfigurationsInput, options: CallOptions) !GetSecurityConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSecurityConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetSecurityConfigurations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSecurityConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSecurityConfigurationsOutput, body, allocator);
}
