...
pub const DeleteMLTransformInput = struct {
...
pub const DeleteMLTransformOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMLTransformInput, options: CallOptions) !DeleteMLTransformOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMLTransformInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteMLTransform");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMLTransformOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteMLTransformOutput, body, allocator);
}
