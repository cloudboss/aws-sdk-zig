...
const TableVersion = @import("table_version.zig").TableVersion;

pub const GetTableVersionsInput = struct {
...
pub const GetTableVersionsOutput = struct {
    ...
    /// specified table.
    table_versions: ?[]const TableVersion = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableVersionsInput, options: CallOptions) !GetTableVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTableVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetTableVersions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTableVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTableVersionsOutput, body, allocator);
}
