...
pub const GetSecurityConfigurationInput = struct {
...
pub const GetSecurityConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSecurityConfigurationInput, options: CallOptions) !GetSecurityConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSecurityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetSecurityConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSecurityConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSecurityConfigurationOutput, body, allocator);
}
