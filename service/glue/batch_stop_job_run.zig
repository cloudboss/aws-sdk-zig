...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchStopJobRunInput, options: Options) !BatchStopJobRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchStopJobRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchStopJobRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchStopJobRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchStopJobRunOutput, body, alloc);
}
