...
pub const ListCrawlersInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCrawlersInput, options: CallOptions) !ListCrawlersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCrawlersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListCrawlers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCrawlersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCrawlersOutput, body, allocator);
}
