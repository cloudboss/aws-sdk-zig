...
pub const CreateCrawlerInput = struct {
    ...
    /// to the
    /// crawler. For more information about tags in Glue, see [Amazon Web Services
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCrawlerInput, options: Options) !CreateCrawlerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateCrawlerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.CreateCrawler");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateCrawlerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
