...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataQualityModelResultInput, options: Options) !GetDataQualityModelResultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataQualityModelResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetDataQualityModelResult");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataQualityModelResultOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDataQualityModelResultOutput, body, alloc);
}
