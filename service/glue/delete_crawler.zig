...
pub const DeleteCrawlerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCrawlerInput, options: Options) !DeleteCrawlerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCrawlerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteCrawler");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCrawlerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
