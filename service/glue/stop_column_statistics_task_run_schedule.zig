...
fn serializeRequest(alloc: std.mem.Allocator, input: StopColumnStatisticsTaskRunScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.StopColumnStatisticsTaskRunSchedule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopColumnStatisticsTaskRunScheduleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
