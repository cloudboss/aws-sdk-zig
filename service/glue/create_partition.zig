...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePartitionInput, options: Options) !CreatePartitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CreatePartition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePartitionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
