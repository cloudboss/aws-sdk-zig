...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetJobsInput, options: Options) !BatchGetJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchGetJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetJobsOutput, body, alloc);
}
