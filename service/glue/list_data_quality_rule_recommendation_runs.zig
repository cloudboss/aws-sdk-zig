...
const DataQualityRuleRecommendationRunFilter = @import("data_quality_rule_recommendation_run_filter.zig").DataQualityRuleRecommendationRunFilter;
const DataQualityRuleRecommendationRunDescription = @import("data_quality_rule_recommendation_run_description.zig").DataQualityRuleRecommendationRunDescription;

pub const ListDataQualityRuleRecommendationRunsInput = struct {
    /// The filter criteria.
    filter: ?DataQualityRuleRecommendationRunFilter = null,
...
pub const ListDataQualityRuleRecommendationRunsOutput = struct {
    /// A pagination token, if more results are available.
    next_token: ?[]const u8 = null,

    /// A list of `DataQualityRuleRecommendationRunDescription` objects.
    runs: ?[]const DataQualityRuleRecommendationRunDescription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataQualityRuleRecommendationRunsInput, options: CallOptions) !ListDataQualityRuleRecommendationRunsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "glue");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDataQualityRuleRecommendationRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListDataQualityRuleRecommendationRuns");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDataQualityRuleRecommendationRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDataQualityRuleRecommendationRunsOutput, body, allocator);
}
