...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSchemaVersionMetadataInput, options: Options) !PutSchemaVersionMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutSchemaVersionMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.PutSchemaVersionMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutSchemaVersionMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutSchemaVersionMetadataOutput, body, alloc);
}
