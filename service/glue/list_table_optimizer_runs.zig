...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTableOptimizerRunsInput, options: Options) !ListTableOptimizerRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTableOptimizerRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListTableOptimizerRuns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTableOptimizerRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTableOptimizerRunsOutput, body, alloc);
}
