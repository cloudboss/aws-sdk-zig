...
pub const BatchGetWorkflowsInput = struct {
...
pub const BatchGetWorkflowsOutput = struct {
    /// A list of names of workflows not found.
    missing_workflows: ?[]const []const u8 = null,

    /// A list of workflow resource metadata.
    workflows: ?[]const Workflow = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetWorkflowsInput, options: CallOptions) !BatchGetWorkflowsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchGetWorkflowsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.BatchGetWorkflows");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchGetWorkflowsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetWorkflowsOutput, body, allocator);
}
