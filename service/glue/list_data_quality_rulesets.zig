...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataQualityRulesetsInput, options: Options) !ListDataQualityRulesetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataQualityRulesetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListDataQualityRulesets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataQualityRulesetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDataQualityRulesetsOutput, body, alloc);
}
