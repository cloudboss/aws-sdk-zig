...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateClassifierInput, options: Options) !CreateClassifierOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateClassifierInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CreateClassifier");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateClassifierOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
