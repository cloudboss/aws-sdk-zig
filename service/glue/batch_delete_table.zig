...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteTableInput, options: Options) !BatchDeleteTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchDeleteTable");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteTableOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDeleteTableOutput, body, alloc);
}
