...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPartitionIndexesInput, options: Options) !GetPartitionIndexesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPartitionIndexesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetPartitionIndexes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPartitionIndexesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPartitionIndexesOutput, body, alloc);
}
