...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateColumnStatisticsForPartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateColumnStatisticsForPartition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateColumnStatisticsForPartitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateColumnStatisticsForPartitionOutput, body, alloc);
}
