...
const ColumnStatisticsError = @import("column_statistics_error.zig").ColumnStatisticsError;

pub const UpdateColumnStatisticsForPartitionInput = struct {
    /// The ID of the Data Catalog where the partitions in question reside.
    /// If none is supplied, the Amazon Web Services account ID is used by default.
    catalog_id: ?[]const u8 = null,

    /// A list of the column statistics.
    column_statistics_list: []const ColumnStatistics,
    ...
    pub const json_field_names = .{
        ...
        .column_statistics_list = "ColumnStatisticsList",
...
pub const UpdateColumnStatisticsForPartitionOutput = struct {
    /// Error occurred during updating column statistics data.
    errors: ?[]const ColumnStatisticsError = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateColumnStatisticsForPartitionInput, options: CallOptions) !UpdateColumnStatisticsForPartitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateColumnStatisticsForPartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.UpdateColumnStatisticsForPartition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateColumnStatisticsForPartitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateColumnStatisticsForPartitionOutput, body, allocator);
}
