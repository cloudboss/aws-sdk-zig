...
pub const RemoveSchemaVersionMetadataInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveSchemaVersionMetadataInput, options: CallOptions) !RemoveSchemaVersionMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveSchemaVersionMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.RemoveSchemaVersionMetadata");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveSchemaVersionMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RemoveSchemaVersionMetadataOutput, body, allocator);
}
