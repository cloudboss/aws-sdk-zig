...
pub const DeletePartitionIndexInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePartitionIndexInput, options: Options) !DeletePartitionIndexOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePartitionIndexInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeletePartitionIndex");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePartitionIndexOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
