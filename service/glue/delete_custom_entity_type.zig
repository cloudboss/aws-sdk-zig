...
pub const DeleteCustomEntityTypeInput = struct {
...
pub const DeleteCustomEntityTypeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCustomEntityTypeInput, options: CallOptions) !DeleteCustomEntityTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCustomEntityTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteCustomEntityType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCustomEntityTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteCustomEntityTypeOutput, body, allocator);
}
