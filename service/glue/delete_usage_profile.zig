...
pub const DeleteUsageProfileInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUsageProfileInput, options: Options) !DeleteUsageProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUsageProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteUsageProfile");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUsageProfileOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
