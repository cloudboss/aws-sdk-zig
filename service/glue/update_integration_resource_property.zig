...
pub const UpdateIntegrationResourcePropertyInput = struct {
...
pub const UpdateIntegrationResourcePropertyOutput = struct {
    /// The connection ARN of the source, or the database ARN of the target.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIntegrationResourcePropertyInput, options: CallOptions) !UpdateIntegrationResourcePropertyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateIntegrationResourcePropertyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.UpdateIntegrationResourceProperty");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateIntegrationResourcePropertyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateIntegrationResourcePropertyOutput, body, allocator);
}
