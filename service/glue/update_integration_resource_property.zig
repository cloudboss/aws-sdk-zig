...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIntegrationResourcePropertyInput, options: Options) !UpdateIntegrationResourcePropertyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIntegrationResourcePropertyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateIntegrationResourceProperty");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIntegrationResourcePropertyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateIntegrationResourcePropertyOutput, body, alloc);
}
