...
pub const DeleteClassifierInput = struct {
...
pub const DeleteClassifierOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteClassifierInput, options: CallOptions) !DeleteClassifierOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteClassifierInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteClassifier");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteClassifierOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
