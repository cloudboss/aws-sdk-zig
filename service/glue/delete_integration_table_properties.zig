...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteIntegrationTablePropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.DeleteIntegrationTableProperties");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteIntegrationTablePropertiesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
