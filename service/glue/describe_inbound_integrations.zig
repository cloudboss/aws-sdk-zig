...
pub const DescribeInboundIntegrationsInput = struct {
...
pub const DescribeInboundIntegrationsOutput = struct {
    /// A list of inbound integrations.
    inbound_integrations: ?[]const InboundIntegration = null,
    ...
    pub const json_field_names = .{
        .inbound_integrations = "InboundIntegrations",
        .marker = "Marker",
    };
};
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInboundIntegrationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DescribeInboundIntegrations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInboundIntegrationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInboundIntegrationsOutput, body, allocator);
}
