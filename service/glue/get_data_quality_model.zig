...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataQualityModelInput, options: Options) !GetDataQualityModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataQualityModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetDataQualityModel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataQualityModelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDataQualityModelOutput, body, alloc);
}
