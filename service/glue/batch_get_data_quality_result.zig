...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetDataQualityResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchGetDataQualityResult");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetDataQualityResultOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchGetDataQualityResultOutput, body, alloc);
}
