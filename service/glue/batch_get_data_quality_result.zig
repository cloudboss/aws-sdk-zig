...
const DataQualityResult = @import("data_quality_result.zig").DataQualityResult;

pub const BatchGetDataQualityResultInput = struct {
...
pub const BatchGetDataQualityResultOutput = struct {
    /// A list of `DataQualityResult` objects representing the data quality results.
    results: ?[]const DataQualityResult = null,

    /// A list of result IDs for which results were not found.
    results_not_found: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetDataQualityResultInput, options: CallOptions) !BatchGetDataQualityResultOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchGetDataQualityResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.BatchGetDataQualityResult");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchGetDataQualityResultOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchGetDataQualityResultOutput, body, allocator);
}
