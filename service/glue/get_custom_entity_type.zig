...
pub const GetCustomEntityTypeInput = struct {
...
pub const GetCustomEntityTypeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCustomEntityTypeInput, options: CallOptions) !GetCustomEntityTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCustomEntityTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetCustomEntityType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCustomEntityTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCustomEntityTypeOutput, body, allocator);
}
