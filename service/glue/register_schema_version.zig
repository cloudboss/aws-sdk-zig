...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterSchemaVersionInput, options: Options) !RegisterSchemaVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterSchemaVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.RegisterSchemaVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterSchemaVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterSchemaVersionOutput, body, alloc);
}
