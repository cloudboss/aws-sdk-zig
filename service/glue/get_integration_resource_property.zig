...
pub const GetIntegrationResourcePropertyInput = struct {
...
pub const GetIntegrationResourcePropertyOutput = struct {
    /// The connection ARN of the source, or the database ARN of the target.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIntegrationResourcePropertyInput, options: CallOptions) !GetIntegrationResourcePropertyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIntegrationResourcePropertyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetIntegrationResourceProperty");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIntegrationResourcePropertyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetIntegrationResourcePropertyOutput, body, allocator);
}
