...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDevEndpointsInput, options: Options) !ListDevEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDevEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListDevEndpoints");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDevEndpointsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDevEndpointsOutput, body, alloc);
}
