...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetCustomEntityTypesInput, options: Options) !BatchGetCustomEntityTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetCustomEntityTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchGetCustomEntityTypes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetCustomEntityTypesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetCustomEntityTypesOutput, body, alloc);
}
