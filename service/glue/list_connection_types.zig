...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConnectionTypesInput, options: Options) !ListConnectionTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConnectionTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListConnectionTypes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConnectionTypesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListConnectionTypesOutput, body, alloc);
}
