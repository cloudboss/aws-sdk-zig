...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataQualityResultInput, options: Options) !GetDataQualityResultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataQualityResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetDataQualityResult");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataQualityResultOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDataQualityResultOutput, body, alloc);
}
