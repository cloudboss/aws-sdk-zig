...
pub const ImportCatalogToGlueInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportCatalogToGlueInput, options: Options) !ImportCatalogToGlueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ImportCatalogToGlueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ImportCatalogToGlue");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ImportCatalogToGlueOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
