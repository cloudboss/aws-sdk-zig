...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIntegrationResourcePropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListIntegrationResourceProperties");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIntegrationResourcePropertiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListIntegrationResourcePropertiesOutput, body, alloc);
}
