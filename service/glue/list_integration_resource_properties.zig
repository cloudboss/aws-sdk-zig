...
const IntegrationResourcePropertyFilter = @import("integration_resource_property_filter.zig").IntegrationResourcePropertyFilter;
const IntegrationResourceProperty = @import("integration_resource_property.zig").IntegrationResourceProperty;

pub const ListIntegrationResourcePropertiesInput = struct {
    /// A list of filters, supported filter Key is `SourceArn` and `TargetArn`.
    filters: ?[]const IntegrationResourcePropertyFilter = null,
...
pub const ListIntegrationResourcePropertiesOutput = struct {
    /// A list of integration resource property meeting the filter criteria.
    integration_resource_property_list: ?[]const IntegrationResourceProperty = null,

    /// This is the pagination token for the next page.
    marker: ?[]const u8 = null,

    pub const json_field_names = .{
        .integration_resource_property_list = "IntegrationResourcePropertyList",
        .marker = "Marker",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIntegrationResourcePropertiesInput, options: CallOptions) !ListIntegrationResourcePropertiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListIntegrationResourcePropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListIntegrationResourceProperties");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListIntegrationResourcePropertiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListIntegrationResourcePropertiesOutput, body, allocator);
}
