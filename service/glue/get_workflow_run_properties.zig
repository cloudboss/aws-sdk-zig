...
pub const GetWorkflowRunPropertiesInput = struct {
...
pub const GetWorkflowRunPropertiesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkflowRunPropertiesInput, options: CallOptions) !GetWorkflowRunPropertiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetWorkflowRunPropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetWorkflowRunProperties");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetWorkflowRunPropertiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetWorkflowRunPropertiesOutput, body, allocator);
}
