...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConnectionTypeInput, options: Options) !DescribeConnectionTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConnectionTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.DescribeConnectionType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConnectionTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConnectionTypeOutput, body, alloc);
}
