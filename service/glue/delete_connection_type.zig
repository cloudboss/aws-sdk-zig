...
pub const DeleteConnectionTypeInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConnectionTypeInput, options: Options) !DeleteConnectionTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteConnectionTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteConnectionType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteConnectionTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
