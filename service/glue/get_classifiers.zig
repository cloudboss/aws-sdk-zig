...
pub const GetClassifiersInput = struct {
...
pub const GetClassifiersOutput = struct {
    /// The requested list of classifier
    /// objects.
    classifiers: ?[]const Classifier = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetClassifiersInput, options: CallOptions) !GetClassifiersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetClassifiersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetClassifiers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetClassifiersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetClassifiersOutput, body, allocator);
}
