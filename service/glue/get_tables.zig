...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTablesInput, options: Options) !GetTablesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTablesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetTables");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTablesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTablesOutput, body, alloc);
}
