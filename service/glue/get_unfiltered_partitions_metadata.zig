...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUnfilteredPartitionsMetadataInput, options: Options) !GetUnfilteredPartitionsMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUnfilteredPartitionsMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetUnfilteredPartitionsMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUnfilteredPartitionsMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUnfilteredPartitionsMetadataOutput, body, alloc);
}
