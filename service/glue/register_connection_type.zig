...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterConnectionTypeInput, options: Options) !RegisterConnectionTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterConnectionTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.RegisterConnectionType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterConnectionTypeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterConnectionTypeOutput, body, alloc);
}
