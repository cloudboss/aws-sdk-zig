...
pub const CheckSchemaVersionValidityInput = struct {
...
pub const CheckSchemaVersionValidityOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CheckSchemaVersionValidityInput, options: CallOptions) !CheckSchemaVersionValidityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CheckSchemaVersionValidityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.CheckSchemaVersionValidity");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CheckSchemaVersionValidityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CheckSchemaVersionValidityOutput, body, allocator);
}
