...
const ColumnError = @import("column_error.zig").ColumnError;

pub const GetColumnStatisticsForPartitionInput = struct {
...
pub const GetColumnStatisticsForPartitionOutput = struct {
    /// List of ColumnStatistics that failed to be retrieved.
    column_statistics_list: ?[]const ColumnStatistics = null,

    /// Error occurred during retrieving column statistics data.
    errors: ?[]const ColumnError = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetColumnStatisticsForPartitionInput, options: CallOptions) !GetColumnStatisticsForPartitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetColumnStatisticsForPartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetColumnStatisticsForPartition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetColumnStatisticsForPartitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetColumnStatisticsForPartitionOutput, body, allocator);
}
