...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTriggersInput, options: Options) !ListTriggersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTriggersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListTriggers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTriggersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTriggersOutput, body, alloc);
}
