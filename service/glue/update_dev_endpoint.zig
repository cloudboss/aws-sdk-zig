...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDevEndpointInput, options: Options) !UpdateDevEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDevEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateDevEndpoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDevEndpointOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
