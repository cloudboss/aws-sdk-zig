...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartColumnStatisticsTaskRunScheduleInput, options: Options) !StartColumnStatisticsTaskRunScheduleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartColumnStatisticsTaskRunScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartColumnStatisticsTaskRunSchedule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartColumnStatisticsTaskRunScheduleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
