...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPartitionsInput, options: Options) !GetPartitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPartitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetPartitions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPartitionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPartitionsOutput, body, alloc);
}
