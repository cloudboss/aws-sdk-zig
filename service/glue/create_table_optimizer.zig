...
pub const CreateTableOptimizerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTableOptimizerInput, options: Options) !CreateTableOptimizerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTableOptimizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.CreateTableOptimizer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTableOptimizerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
