...
pub const StartBlueprintRunInput = struct {
...
pub const StartBlueprintRunOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartBlueprintRunInput, options: CallOptions) !StartBlueprintRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartBlueprintRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StartBlueprintRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartBlueprintRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartBlueprintRunOutput, body, allocator);
}
