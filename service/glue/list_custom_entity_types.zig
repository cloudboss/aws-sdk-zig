...
pub const ListCustomEntityTypesInput = struct {
...
pub const ListCustomEntityTypesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomEntityTypesInput, options: CallOptions) !ListCustomEntityTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomEntityTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListCustomEntityTypes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomEntityTypesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCustomEntityTypesOutput, body, allocator);
}
