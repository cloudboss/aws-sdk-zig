...
const SchemaVersionErrorItem = @import("schema_version_error_item.zig").SchemaVersionErrorItem;

pub const DeleteSchemaVersionsInput = struct {
...
pub const DeleteSchemaVersionsOutput = struct {
    /// A list of `SchemaVersionErrorItem` objects, each containing an error and
    /// schema version.
    schema_version_errors: ?[]const SchemaVersionErrorItem = null,

    pub const json_field_names = .{
        .schema_version_errors = "SchemaVersionErrors",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSchemaVersionsInput, options: CallOptions) !DeleteSchemaVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSchemaVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteSchemaVersions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSchemaVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteSchemaVersionsOutput, body, allocator);
}
