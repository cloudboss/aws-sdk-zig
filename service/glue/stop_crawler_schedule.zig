...
pub const StopCrawlerScheduleInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopCrawlerScheduleInput, options: Options) !StopCrawlerScheduleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopCrawlerScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StopCrawlerSchedule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopCrawlerScheduleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
