...
pub const DeleteRegistryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRegistryInput, options: CallOptions) !DeleteRegistryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRegistryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteRegistry");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRegistryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRegistryOutput, body, allocator);
}
