...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMLTransformInput, options: Options) !GetMLTransformOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMLTransformInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetMLTransform");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMLTransformOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMLTransformOutput, body, alloc);
}
