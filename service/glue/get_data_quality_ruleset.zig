...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataQualityRulesetInput, options: Options) !GetDataQualityRulesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataQualityRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetDataQualityRuleset");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataQualityRulesetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDataQualityRulesetOutput, body, alloc);
}
