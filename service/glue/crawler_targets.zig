...
pub const CrawlerTargets = struct {
    ...
    catalog_targets: ?[]const CatalogTarget,

    /// Specifies Delta data store targets.
    delta_targets: ?[]const DeltaTarget,

    /// Specifies Amazon DynamoDB targets.
    dynamo_db_targets: ?[]const DynamoDBTarget,

    /// Specifies Apache Hudi data store targets.
    hudi_targets: ?[]const HudiTarget,

    /// Specifies Apache Iceberg data store targets.
    iceberg_targets: ?[]const IcebergTarget,

    /// Specifies JDBC targets.
    jdbc_targets: ?[]const JdbcTarget,

    /// Specifies Amazon DocumentDB or MongoDB targets.
    mongo_db_targets: ?[]const MongoDBTarget,

    /// Specifies Amazon Simple Storage Service (Amazon S3) targets.
    s3_targets: ?[]const S3Target,
