...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataQualityResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListDataQualityResults");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataQualityResultsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListDataQualityResultsOutput, body, alloc);
}
