...
const DataQualityResultFilterCriteria = @import("data_quality_result_filter_criteria.zig").DataQualityResultFilterCriteria;
const DataQualityResultDescription = @import("data_quality_result_description.zig").DataQualityResultDescription;

pub const ListDataQualityResultsInput = struct {
    /// The filter criteria.
    filter: ?DataQualityResultFilterCriteria = null,
...
pub const ListDataQualityResultsOutput = struct {
    /// A pagination token, if more results are available.
    next_token: ?[]const u8 = null,

    /// A list of `DataQualityResultDescription` objects.
    results: ?[]const DataQualityResultDescription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataQualityResultsInput, options: CallOptions) !ListDataQualityResultsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDataQualityResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListDataQualityResults");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDataQualityResultsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListDataQualityResultsOutput, body, allocator);
}
