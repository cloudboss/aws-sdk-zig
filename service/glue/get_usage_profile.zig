...
pub const GetUsageProfileInput = struct {
...
pub const GetUsageProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUsageProfileInput, options: CallOptions) !GetUsageProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetUsageProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetUsageProfile");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetUsageProfileOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUsageProfileOutput, body, allocator);
}
