...
const SchemaListItem = @import("schema_list_item.zig").SchemaListItem;
...
pub const ListSchemasOutput = struct {
    ...
    schemas: ?[]const SchemaListItem = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSchemasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.ListSchemas");
