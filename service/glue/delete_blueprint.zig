...
pub const DeleteBlueprintInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBlueprintInput, options: Options) !DeleteBlueprintOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBlueprintInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteBlueprint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBlueprintOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteBlueprintOutput, body, allocator);
}
