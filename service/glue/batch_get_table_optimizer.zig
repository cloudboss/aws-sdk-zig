...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetTableOptimizerInput, options: Options) !BatchGetTableOptimizerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetTableOptimizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchGetTableOptimizer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetTableOptimizerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetTableOptimizerOutput, body, alloc);
}
