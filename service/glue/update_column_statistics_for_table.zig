...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateColumnStatisticsForTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateColumnStatisticsForTable");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateColumnStatisticsForTableOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateColumnStatisticsForTableOutput, body, alloc);
}
