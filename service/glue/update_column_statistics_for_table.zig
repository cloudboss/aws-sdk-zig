...
pub const UpdateColumnStatisticsForTableInput = struct {
...
pub const UpdateColumnStatisticsForTableOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateColumnStatisticsForTableInput, options: CallOptions) !UpdateColumnStatisticsForTableOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateColumnStatisticsForTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.UpdateColumnStatisticsForTable");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateColumnStatisticsForTableOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateColumnStatisticsForTableOutput, body, allocator);
}
