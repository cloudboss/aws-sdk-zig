...
pub const StopSessionInput = struct {
...
pub const StopSessionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopSessionInput, options: CallOptions) !StopSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StopSession");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopSessionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopSessionOutput, body, allocator);
}
