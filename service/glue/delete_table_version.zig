...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTableVersionInput, options: Options) !DeleteTableVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTableVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.DeleteTableVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTableVersionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
