...
pub const AuthConfiguration = struct {
    ...
    /// `Property` object.
    basic_authentication_properties: ?[]const aws.map.MapEntry(Property),

    /// A map of key-value pairs for the custom authentication properties. Each
    /// value is a a `Property` object.
    custom_authentication_properties: ?[]const aws.map.MapEntry(Property),

    /// A map of key-value pairs for the OAuth2 properties. Each value is a a
    /// `Property` object.
    o_auth_2_properties: ?[]const aws.map.MapEntry(Property),
