...
pub const DeleteTriggerInput = struct {
...
pub const DeleteTriggerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTriggerInput, options: CallOptions) !DeleteTriggerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTriggerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteTrigger");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTriggerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteTriggerOutput, body, allocator);
}
