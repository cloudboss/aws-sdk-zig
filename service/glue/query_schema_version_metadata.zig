...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: QuerySchemaVersionMetadataInput, options: Options) !QuerySchemaVersionMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: QuerySchemaVersionMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.QuerySchemaVersionMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !QuerySchemaVersionMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(QuerySchemaVersionMetadataOutput, body, alloc);
}
