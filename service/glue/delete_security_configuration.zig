...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSecurityConfigurationInput, options: Options) !DeleteSecurityConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSecurityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteSecurityConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSecurityConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
