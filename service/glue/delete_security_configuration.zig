...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSecurityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.DeleteSecurityConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSecurityConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
