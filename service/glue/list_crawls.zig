...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCrawlsInput, options: Options) !ListCrawlsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCrawlsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListCrawls");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCrawlsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCrawlsOutput, body, alloc);
}
