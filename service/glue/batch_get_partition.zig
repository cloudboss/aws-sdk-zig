...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetPartitionInput, options: Options) !BatchGetPartitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetPartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchGetPartition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetPartitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetPartitionOutput, body, alloc);
}
