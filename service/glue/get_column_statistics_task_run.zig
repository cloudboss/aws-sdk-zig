...
pub const GetColumnStatisticsTaskRunInput = struct {
    /// The identifier for the particular column statistics task run.
    column_statistics_task_run_id: []const u8,
...
pub const GetColumnStatisticsTaskRunOutput = struct {
    /// A `ColumnStatisticsTaskRun` object representing the details of the column
    /// stats run.
    column_statistics_task_run: ?ColumnStatisticsTaskRun = null,

    pub const json_field_names = .{
        .column_statistics_task_run = "ColumnStatisticsTaskRun",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetColumnStatisticsTaskRunInput, options: CallOptions) !GetColumnStatisticsTaskRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetColumnStatisticsTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetColumnStatisticsTaskRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetColumnStatisticsTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetColumnStatisticsTaskRunOutput, body, allocator);
}
