...
pub const CreateUsageProfileInput = struct {
...
pub const CreateUsageProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUsageProfileInput, options: CallOptions) !CreateUsageProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateUsageProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.CreateUsageProfile");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateUsageProfileOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateUsageProfileOutput, body, allocator);
}
