...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMappingInput, options: Options) !GetMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetMapping");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMappingOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetMappingOutput, body, alloc);
}
