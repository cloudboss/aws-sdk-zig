...
pub const DeleteColumnStatisticsForTableInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteColumnStatisticsForTableInput, options: Options) !DeleteColumnStatisticsForTableOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteColumnStatisticsForTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteColumnStatisticsForTable");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteColumnStatisticsForTableOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
