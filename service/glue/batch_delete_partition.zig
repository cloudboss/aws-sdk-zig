...
pub const BatchDeletePartitionInput = struct {
    ...
    partitions_to_delete: []const PartitionValueList,
    ...
    pub const json_field_names = .{
        ...
        .partitions_to_delete = "PartitionsToDelete",
        .table_name = "TableName",
    };
};

pub const BatchDeletePartitionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeletePartitionInput, options: CallOptions) !BatchDeletePartitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDeletePartitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.BatchDeletePartition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDeletePartitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDeletePartitionOutput, body, allocator);
}
