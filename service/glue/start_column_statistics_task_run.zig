...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartColumnStatisticsTaskRunInput, options: Options) !StartColumnStatisticsTaskRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartColumnStatisticsTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.StartColumnStatisticsTaskRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartColumnStatisticsTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartColumnStatisticsTaskRunOutput, body, alloc);
}
