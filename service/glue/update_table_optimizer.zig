...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTableOptimizerInput, options: Options) !UpdateTableOptimizerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTableOptimizerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.UpdateTableOptimizer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTableOptimizerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
