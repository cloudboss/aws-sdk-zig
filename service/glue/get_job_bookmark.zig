...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobBookmarkInput, options: Options) !GetJobBookmarkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetJobBookmarkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetJobBookmark");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetJobBookmarkOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetJobBookmarkOutput, body, alloc);
}
