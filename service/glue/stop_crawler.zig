...
pub const StopCrawlerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopCrawlerInput, options: Options) !StopCrawlerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopCrawlerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StopCrawler");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopCrawlerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
