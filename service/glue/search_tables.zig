...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchTablesInput, options: Options) !SearchTablesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchTablesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.SearchTables");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchTablesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchTablesOutput, body, alloc);
}
