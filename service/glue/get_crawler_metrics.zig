...
const CrawlerMetrics = @import("crawler_metrics.zig").CrawlerMetrics;

pub const GetCrawlerMetricsInput = struct {
    /// A list of the names of crawlers about which to retrieve metrics.
    crawler_name_list: ?[]const []const u8 = null,
...
pub const GetCrawlerMetricsOutput = struct {
    /// A list of metrics for the specified crawler.
    crawler_metrics_list: ?[]const CrawlerMetrics = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCrawlerMetricsInput, options: CallOptions) !GetCrawlerMetricsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCrawlerMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetCrawlerMetrics");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCrawlerMetricsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCrawlerMetricsOutput, body, allocator);
}
