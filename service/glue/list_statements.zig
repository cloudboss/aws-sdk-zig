...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStatementsInput, options: Options) !ListStatementsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStatementsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.ListStatements");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStatementsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListStatementsOutput, body, alloc);
}
