...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetTriggersInput, options: Options) !BatchGetTriggersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetTriggersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.BatchGetTriggers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetTriggersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetTriggersOutput, body, alloc);
}
