...
pub const UpdateUserDefinedFunctionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserDefinedFunctionInput, options: CallOptions) !UpdateUserDefinedFunctionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateUserDefinedFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.UpdateUserDefinedFunction");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateUserDefinedFunctionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
