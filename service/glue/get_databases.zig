...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDatabasesInput, options: Options) !GetDatabasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDatabasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetDatabases");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDatabasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDatabasesOutput, body, alloc);
}
