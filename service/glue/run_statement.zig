...
pub const RunStatementInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RunStatementInput, options: CallOptions) !RunStatementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RunStatementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.RunStatement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RunStatementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RunStatementOutput, body, allocator);
}
