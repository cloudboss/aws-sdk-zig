...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePartitionIndexInput, options: Options) !CreatePartitionIndexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePartitionIndexInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CreatePartitionIndex");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePartitionIndexOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
