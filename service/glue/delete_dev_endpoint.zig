...
pub const DeleteDevEndpointInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDevEndpointInput, options: Options) !DeleteDevEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDevEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.DeleteDevEndpoint");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDevEndpointOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
