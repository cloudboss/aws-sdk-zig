...
pub const GetCrawlerInput = struct {
...
pub const GetCrawlerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCrawlerInput, options: CallOptions) !GetCrawlerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCrawlerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetCrawler");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCrawlerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCrawlerOutput, body, allocator);
}
