...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMLTaskRunInput, options: Options) !CancelMLTaskRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelMLTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.CancelMLTaskRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelMLTaskRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelMLTaskRunOutput, body, alloc);
}
