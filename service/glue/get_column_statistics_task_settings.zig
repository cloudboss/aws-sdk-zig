...
const ColumnStatisticsTaskSettings = @import("column_statistics_task_settings.zig").ColumnStatisticsTaskSettings;

pub const GetColumnStatisticsTaskSettingsInput = struct {
...
pub const GetColumnStatisticsTaskSettingsOutput = struct {
    /// A `ColumnStatisticsTaskSettings` object representing the settings for the
    /// column statistics task.
    column_statistics_task_settings: ?ColumnStatisticsTaskSettings = null,

    pub const json_field_names = .{
        .column_statistics_task_settings = "ColumnStatisticsTaskSettings",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetColumnStatisticsTaskSettingsInput, options: CallOptions) !GetColumnStatisticsTaskSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetColumnStatisticsTaskSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.GetColumnStatisticsTaskSettings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetColumnStatisticsTaskSettingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetColumnStatisticsTaskSettingsOutput, body, allocator);
}
