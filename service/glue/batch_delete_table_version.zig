...
const TableVersionError = @import("table_version_error.zig").TableVersionError;

pub const BatchDeleteTableVersionInput = struct {
...
pub const BatchDeleteTableVersionOutput = struct {
    /// A list of errors encountered while trying to delete
    /// the specified table versions.
    errors: ?[]const TableVersionError = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteTableVersionInput, options: CallOptions) !BatchDeleteTableVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDeleteTableVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.BatchDeleteTableVersion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDeleteTableVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDeleteTableVersionOutput, body, allocator);
}
