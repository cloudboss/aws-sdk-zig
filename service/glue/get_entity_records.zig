...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEntityRecordsInput, options: Options) !GetEntityRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEntityRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetEntityRecords");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEntityRecordsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEntityRecordsOutput, body, alloc);
}
