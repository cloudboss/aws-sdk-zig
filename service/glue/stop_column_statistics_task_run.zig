...
pub const StopColumnStatisticsTaskRunInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopColumnStatisticsTaskRunInput, options: Options) !StopColumnStatisticsTaskRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopColumnStatisticsTaskRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSGlue.StopColumnStatisticsTaskRun");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopColumnStatisticsTaskRunOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
