...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUnfilteredTableMetadataInput, options: Options) !GetUnfilteredTableMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUnfilteredTableMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSGlue.GetUnfilteredTableMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUnfilteredTableMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUnfilteredTableMetadataOutput, body, alloc);
}
