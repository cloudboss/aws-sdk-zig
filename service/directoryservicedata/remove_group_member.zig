...
pub const RemoveGroupMemberInput = struct {
    ...
    /// The `SAMAccountName` of the user, group, or computer to remove from the
    /// group.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveGroupMemberInput, options: Options) !RemoveGroupMemberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveGroupMemberInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GroupMemberships/RemoveGroupMember";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveGroupMemberOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: RemoveGroupMemberOutput = .{};
