...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGroupsForMemberInput, options: Options) !ListGroupsForMemberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupsForMemberInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupsForMemberOutput {
    var result: ListGroupsForMemberOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGroupsForMemberOutput, body, alloc);
    }
    _ = status;
    _ = headers;
