...
fn serializeRequest(allocator: std.mem.Allocator, input: AddGroupMemberInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"MemberName\":");
    try aws.json.writeValue(@TypeOf(input.member_name), input.member_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AddGroupMemberOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
