...
pub const AddGroupMemberInput = struct {
    ...
    /// required only
    /// when adding a member outside of your Managed Microsoft AD domain to a group
    /// inside of your
    /// Managed Microsoft AD domain. This parameter defaults to the Managed
    /// Microsoft AD domain.
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddGroupMemberInput, options: Options) !AddGroupMemberOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddGroupMemberInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GroupMemberships/AddGroupMember";
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MemberName\":");
    try aws.json.writeValue(@TypeOf(input.member_name), input.member_name, alloc, &body_buf);
    has_prev = true;
    if (input.member_realm) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MemberRealm\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddGroupMemberOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
