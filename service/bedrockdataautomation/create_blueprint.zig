...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBlueprintInput, options: Options) !CreateBlueprintOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBlueprintInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"blueprintName\":");
    try aws.json.writeValue(@TypeOf(input.blueprint_name), input.blueprint_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBlueprintOutput {
    var result: CreateBlueprintOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBlueprintOutput, body, alloc);
    }
    _ = status;
    _ = headers;
