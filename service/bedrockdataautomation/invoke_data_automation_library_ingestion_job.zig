...
pub const InvokeDataAutomationLibraryIngestionJobInput = struct {
    /// Idempotency token
...
pub const InvokeDataAutomationLibraryIngestionJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InvokeDataAutomationLibraryIngestionJobInput, options: CallOptions) !InvokeDataAutomationLibraryIngestionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: InvokeDataAutomationLibraryIngestionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"inputConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.input_configuration), input.input_configuration, allocator, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.operation_type), input.operation_type, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !InvokeDataAutomationLibraryIngestionJobOutput {
    var result: InvokeDataAutomationLibraryIngestionJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(InvokeDataAutomationLibraryIngestionJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
