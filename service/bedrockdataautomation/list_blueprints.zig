...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBlueprintsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.blueprint_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"blueprintArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.blueprint_stage_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"blueprintStageFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.project_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"projectFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBlueprintsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBlueprintsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
