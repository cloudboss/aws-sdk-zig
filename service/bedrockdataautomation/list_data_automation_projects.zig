...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataAutomationProjectsInput, options: Options) !ListDataAutomationProjectsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataAutomationProjectsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.blueprint_filter) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"blueprintFilter\":");
    ...
    if (input.project_stage_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"projectStageFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataAutomationProjectsOutput {
    var result: ListDataAutomationProjectsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataAutomationProjectsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
