...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataAutomationProjectInput, options: Options) !GetDataAutomationProjectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataAutomationProjectInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataAutomationProjectOutput {
    var result: GetDataAutomationProjectOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataAutomationProjectOutput, body, alloc);
    }
    _ = status;
    _ = headers;
