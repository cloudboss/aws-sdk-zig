...
pub const GetBlueprintOptimizationStatusInput = struct {
    /// Invocation arn.
...
pub const GetBlueprintOptimizationStatusOutput = struct {
    /// Error Message.
    ...
    output_configuration: ?BlueprintOptimizationOutputConfiguration = null,

    /// Job Status.
    status: ?BlueprintOptimizationJobStatus = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBlueprintOptimizationStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/getBlueprintOptimizationStatus/");
    try path_buf.appendSlice(alloc, input.invocation_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBlueprintOptimizationStatusOutput {
    var result: GetBlueprintOptimizationStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBlueprintOptimizationStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
