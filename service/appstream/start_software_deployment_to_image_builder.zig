...
pub const StartSoftwareDeploymentToImageBuilderInput = struct {
    ...
    retry_failed_deployments: ?bool = null,

    pub const json_field_names = .{
        .image_builder_name = "ImageBuilderName",
        .retry_failed_deployments = "RetryFailedDeployments",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSoftwareDeploymentToImageBuilderInput, options: Options) !StartSoftwareDeploymentToImageBuilderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartSoftwareDeploymentToImageBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.StartSoftwareDeploymentToImageBuilder");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartSoftwareDeploymentToImageBuilderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
