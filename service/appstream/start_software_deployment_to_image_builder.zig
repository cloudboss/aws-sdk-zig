...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSoftwareDeploymentToImageBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.StartSoftwareDeploymentToImageBuilder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSoftwareDeploymentToImageBuilderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
