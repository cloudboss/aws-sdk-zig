...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStackInput, options: CallOptions) !DeleteStackOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DeleteStack");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteStackOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
