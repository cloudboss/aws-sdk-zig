...
pub const DrainSessionInstanceInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DrainSessionInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DrainSessionInstance");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DrainSessionInstanceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
