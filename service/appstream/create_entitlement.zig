...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEntitlementInput, options: Options) !CreateEntitlementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEntitlementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.CreateEntitlement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEntitlementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEntitlementOutput, body, alloc);
}
