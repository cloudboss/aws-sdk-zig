...
pub const DisassociateApplicationFleetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateApplicationFleetInput, options: Options) !DisassociateApplicationFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateApplicationFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DisassociateApplicationFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateApplicationFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
