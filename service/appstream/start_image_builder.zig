...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartImageBuilderInput, options: Options) !StartImageBuilderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartImageBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.StartImageBuilder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartImageBuilderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartImageBuilderOutput, body, alloc);
}
