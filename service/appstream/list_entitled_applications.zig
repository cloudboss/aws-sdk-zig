...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEntitledApplicationsInput, options: Options) !ListEntitledApplicationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEntitledApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.ListEntitledApplications");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEntitledApplicationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEntitledApplicationsOutput, body, alloc);
}
