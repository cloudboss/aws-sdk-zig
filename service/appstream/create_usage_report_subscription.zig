...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUsageReportSubscriptionInput, options: Options) !CreateUsageReportSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUsageReportSubscriptionInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.CreateUsageReportSubscription");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUsageReportSubscriptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateUsageReportSubscriptionOutput, body, alloc);
}
