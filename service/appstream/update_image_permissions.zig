...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateImagePermissionsInput, options: Options) !UpdateImagePermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateImagePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.UpdateImagePermissions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateImagePermissionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
