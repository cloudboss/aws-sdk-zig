...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteImagePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DeleteImagePermissions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteImagePermissionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
