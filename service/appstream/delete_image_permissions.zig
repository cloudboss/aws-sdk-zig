...
pub const DeleteImagePermissionsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteImagePermissionsInput, options: Options) !DeleteImagePermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteImagePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DeleteImagePermissions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteImagePermissionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
