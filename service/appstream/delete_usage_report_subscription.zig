...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUsageReportSubscriptionInput, options: CallOptions) !DeleteUsageReportSubscriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUsageReportSubscriptionInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DeleteUsageReportSubscription");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUsageReportSubscriptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
