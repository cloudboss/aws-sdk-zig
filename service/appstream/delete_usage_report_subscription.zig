...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteUsageReportSubscriptionInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DeleteUsageReportSubscription");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteUsageReportSubscriptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
