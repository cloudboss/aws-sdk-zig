...
pub const DescribeEntitlementsInput = struct {
...
pub const DescribeEntitlementsOutput = struct {
    /// The entitlements.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEntitlementsInput, options: CallOptions) !DescribeEntitlementsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEntitlementsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DescribeEntitlements");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEntitlementsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEntitlementsOutput, body, allocator);
}
