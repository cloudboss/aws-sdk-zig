...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateSoftwareToImageBuilderInput, options: Options) !AssociateSoftwareToImageBuilderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateSoftwareToImageBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.AssociateSoftwareToImageBuilder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateSoftwareToImageBuilderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
