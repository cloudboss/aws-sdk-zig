...
pub const DeleteImageBuilderInput = struct {
...
pub const DeleteImageBuilderOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteImageBuilderInput, options: CallOptions) !DeleteImageBuilderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteImageBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DeleteImageBuilder");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteImageBuilderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteImageBuilderOutput, body, allocator);
}
