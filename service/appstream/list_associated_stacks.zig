...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssociatedStacksInput, options: Options) !ListAssociatedStacksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssociatedStacksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.ListAssociatedStacks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociatedStacksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssociatedStacksOutput, body, alloc);
}
