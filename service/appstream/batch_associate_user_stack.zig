...
pub const BatchAssociateUserStackInput = struct {
...
pub const BatchAssociateUserStackOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchAssociateUserStackInput, options: CallOptions) !BatchAssociateUserStackOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchAssociateUserStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.BatchAssociateUserStack");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchAssociateUserStackOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchAssociateUserStackOutput, body, allocator);
}
