...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEntitlementInput, options: Options) !DeleteEntitlementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteEntitlementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DeleteEntitlement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteEntitlementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
