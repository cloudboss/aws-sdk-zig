...
const UserStackAssociationError = @import("user_stack_association_error.zig").UserStackAssociationError;

pub const BatchDisassociateUserStackInput = struct {
    /// The list of UserStackAssociation objects.
    user_stack_associations: []const UserStackAssociation,
...
pub const BatchDisassociateUserStackOutput = struct {
    /// The list of UserStackAssociationError objects.
    errors: ?[]const UserStackAssociationError = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDisassociateUserStackInput, options: CallOptions) !BatchDisassociateUserStackOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDisassociateUserStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.BatchDisassociateUserStack");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDisassociateUserStackOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDisassociateUserStackOutput, body, allocator);
}
