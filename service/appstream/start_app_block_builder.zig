...
pub const StartAppBlockBuilderInput = struct {
...
pub const StartAppBlockBuilderOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAppBlockBuilderInput, options: CallOptions) !StartAppBlockBuilderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartAppBlockBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.StartAppBlockBuilder");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartAppBlockBuilderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartAppBlockBuilderOutput, body, allocator);
}
