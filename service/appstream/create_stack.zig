...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStackInput, options: Options) !CreateStackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.CreateStack");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStackOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateStackOutput, body, alloc);
}
