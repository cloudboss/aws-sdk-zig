...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeApplicationFleetAssociationsInput, options: Options) !DescribeApplicationFleetAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeApplicationFleetAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DescribeApplicationFleetAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeApplicationFleetAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeApplicationFleetAssociationsOutput, body, alloc);
}
