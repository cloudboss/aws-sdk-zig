...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppBlockBuilderInput, options: Options) !UpdateAppBlockBuilderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAppBlockBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.UpdateAppBlockBuilder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAppBlockBuilderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateAppBlockBuilderOutput, body, alloc);
}
