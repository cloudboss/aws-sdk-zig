...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUserStackAssociationsInput, options: Options) !DescribeUserStackAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUserStackAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DescribeUserStackAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUserStackAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeUserStackAssociationsOutput, body, alloc);
}
