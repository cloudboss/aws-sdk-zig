...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopAppBlockBuilderInput, options: Options) !StopAppBlockBuilderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopAppBlockBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.StopAppBlockBuilder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopAppBlockBuilderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopAppBlockBuilderOutput, body, alloc);
}
