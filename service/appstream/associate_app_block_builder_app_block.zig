...
const AppBlockBuilderAppBlockAssociation = @import("app_block_builder_app_block_association.zig").AppBlockBuilderAppBlockAssociation;

pub const AssociateAppBlockBuilderAppBlockInput = struct {
...
pub const AssociateAppBlockBuilderAppBlockOutput = struct {
    /// The list of app block builders associated with app blocks.
    app_block_builder_app_block_association: ?AppBlockBuilderAppBlockAssociation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateAppBlockBuilderAppBlockInput, options: CallOptions) !AssociateAppBlockBuilderAppBlockOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "appstream");
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateAppBlockBuilderAppBlockInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.AssociateAppBlockBuilderAppBlock");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateAppBlockBuilderAppBlockOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateAppBlockBuilderAppBlockOutput, body, allocator);
}
