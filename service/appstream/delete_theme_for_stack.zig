...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteThemeForStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DeleteThemeForStack");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteThemeForStackOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
