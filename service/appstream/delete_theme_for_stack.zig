...
pub const DeleteThemeForStackInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteThemeForStackInput, options: Options) !DeleteThemeForStackOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteThemeForStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DeleteThemeForStack");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteThemeForStackOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
