...
pub const AssociateApplicationFleetInput = struct {
...
pub const AssociateApplicationFleetOutput = struct {
    /// If fleet name is specified, this returns the list of applications that are
    /// associated
    /// to it. If application ARN is specified, this returns the list of fleets to
    /// which it is
    /// associated.
    application_fleet_association: ?ApplicationFleetAssociation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateApplicationFleetInput, options: CallOptions) !AssociateApplicationFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateApplicationFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.AssociateApplicationFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateApplicationFleetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateApplicationFleetOutput, body, allocator);
}
