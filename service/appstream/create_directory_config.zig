...
pub const CreateDirectoryConfigInput = struct {
...
pub const CreateDirectoryConfigOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDirectoryConfigInput, options: CallOptions) !CreateDirectoryConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDirectoryConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.CreateDirectoryConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateDirectoryConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDirectoryConfigOutput, body, allocator);
}
