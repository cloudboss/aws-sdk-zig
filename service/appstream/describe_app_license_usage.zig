...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppLicenseUsageInput, options: Options) !DescribeAppLicenseUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAppLicenseUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DescribeAppLicenseUsage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAppLicenseUsageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAppLicenseUsageOutput, body, alloc);
}
