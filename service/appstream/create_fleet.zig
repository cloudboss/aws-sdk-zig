...
pub const CreateFleetInput = struct {
    ...
    /// inactivity. If
    /// they try to reconnect to the streaming session before the time interval
    /// specified in
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.CreateFleet");
