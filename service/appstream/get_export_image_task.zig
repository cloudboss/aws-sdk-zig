...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExportImageTaskInput, options: Options) !GetExportImageTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExportImageTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.GetExportImageTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExportImageTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetExportImageTaskOutput, body, alloc);
}
