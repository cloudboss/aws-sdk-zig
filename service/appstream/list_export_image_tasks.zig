...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExportImageTasksInput, options: Options) !ListExportImageTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExportImageTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.ListExportImageTasks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExportImageTasksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListExportImageTasksOutput, body, alloc);
}
