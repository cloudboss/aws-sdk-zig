...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateApplicationToEntitlementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.AssociateApplicationToEntitlement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateApplicationToEntitlementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
