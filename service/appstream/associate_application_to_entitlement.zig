...
pub const AssociateApplicationToEntitlementInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateApplicationToEntitlementInput, options: Options) !AssociateApplicationToEntitlementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateApplicationToEntitlementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.AssociateApplicationToEntitlement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateApplicationToEntitlementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
