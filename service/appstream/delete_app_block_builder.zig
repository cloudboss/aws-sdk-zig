...
pub const DeleteAppBlockBuilderInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAppBlockBuilderInput, options: Options) !DeleteAppBlockBuilderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAppBlockBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DeleteAppBlockBuilder");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAppBlockBuilderOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
