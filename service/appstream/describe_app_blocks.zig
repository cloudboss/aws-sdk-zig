...
pub const DescribeAppBlocksInput = struct {
...
pub const DescribeAppBlocksOutput = struct {
    /// The app blocks in the list.
    app_blocks: ?[]const AppBlock = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppBlocksInput, options: CallOptions) !DescribeAppBlocksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAppBlocksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DescribeAppBlocks");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAppBlocksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAppBlocksOutput, body, allocator);
}
