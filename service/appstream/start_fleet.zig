...
pub const StartFleetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFleetInput, options: Options) !StartFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.StartFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
