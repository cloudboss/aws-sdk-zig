...
pub const DeleteDirectoryConfigInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDirectoryConfigInput, options: CallOptions) !DeleteDirectoryConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDirectoryConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DeleteDirectoryConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDirectoryConfigOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
