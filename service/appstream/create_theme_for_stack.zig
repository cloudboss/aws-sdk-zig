...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateThemeForStackInput, options: Options) !CreateThemeForStackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateThemeForStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.CreateThemeForStack");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateThemeForStackOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateThemeForStackOutput, body, alloc);
}
