...
pub const DisassociateFleetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateFleetInput, options: Options) !DisassociateFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DisassociateFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
