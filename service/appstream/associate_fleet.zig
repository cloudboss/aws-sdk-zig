...
pub const AssociateFleetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateFleetInput, options: Options) !AssociateFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.AssociateFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
