...
fn serializeRequest(alloc: std.mem.Allocator, input: StopFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.StopFleet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
