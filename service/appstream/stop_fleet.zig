...
pub const StopFleetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopFleetInput, options: Options) !StopFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.StopFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
