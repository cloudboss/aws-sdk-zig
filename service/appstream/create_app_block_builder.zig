...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAppBlockBuilderInput, options: CallOptions) !CreateAppBlockBuilderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAppBlockBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.CreateAppBlockBuilder");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAppBlockBuilderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAppBlockBuilderOutput, body, allocator);
}
