...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateThemeForStackInput, options: Options) !UpdateThemeForStackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateThemeForStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.UpdateThemeForStack");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateThemeForStackOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateThemeForStackOutput, body, alloc);
}
