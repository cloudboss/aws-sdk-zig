...
pub const StopImageBuilderInput = struct {
...
pub const StopImageBuilderOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopImageBuilderInput, options: CallOptions) !StopImageBuilderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopImageBuilderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.StopImageBuilder");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopImageBuilderOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopImageBuilderOutput, body, allocator);
}
