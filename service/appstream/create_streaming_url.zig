...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStreamingURLInput, options: Options) !CreateStreamingURLOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStreamingURLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.CreateStreamingURL");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStreamingURLOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateStreamingURLOutput, body, alloc);
}
