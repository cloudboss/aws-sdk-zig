...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUsageReportSubscriptionsInput, options: Options) !DescribeUsageReportSubscriptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUsageReportSubscriptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DescribeUsageReportSubscriptions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUsageReportSubscriptionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeUsageReportSubscriptionsOutput, body, alloc);
}
