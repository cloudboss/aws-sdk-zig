...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateApplicationFromEntitlementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DisassociateApplicationFromEntitlement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateApplicationFromEntitlementOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
