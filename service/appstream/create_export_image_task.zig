...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateExportImageTaskInput, options: Options) !CreateExportImageTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateExportImageTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.CreateExportImageTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateExportImageTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateExportImageTaskOutput, body, alloc);
}
