...
pub const UpdateEntitlementInput = struct {
    /// Specifies whether all or only selected apps are entitled.
    app_visibility: ?AppVisibility = null,

    /// The attributes of the entitlement.
    attributes: ?[]const EntitlementAttribute = null,
...
pub const UpdateEntitlementOutput = struct {
    /// The entitlement.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEntitlementInput, options: CallOptions) !UpdateEntitlementOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateEntitlementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.UpdateEntitlement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateEntitlementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateEntitlementOutput, body, allocator);
}
