...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUpdatedImageInput, options: Options) !CreateUpdatedImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUpdatedImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.CreateUpdatedImage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUpdatedImageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateUpdatedImageOutput, body, alloc);
}
