...
pub const ListAssociatedFleetsInput = struct {
...
pub const ListAssociatedFleetsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssociatedFleetsInput, options: CallOptions) !ListAssociatedFleetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssociatedFleetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.ListAssociatedFleets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssociatedFleetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssociatedFleetsOutput, body, allocator);
}
