...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDirectoryConfigInput, options: Options) !UpdateDirectoryConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDirectoryConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.UpdateDirectoryConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDirectoryConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateDirectoryConfigOutput, body, alloc);
}
