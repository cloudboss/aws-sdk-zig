...
pub const DescribeThemeForStackInput = struct {
...
pub const DescribeThemeForStackOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeThemeForStackInput, options: CallOptions) !DescribeThemeForStackOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeThemeForStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DescribeThemeForStack");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeThemeForStackOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeThemeForStackOutput, body, allocator);
}
