...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImagePermissionsInput, options: Options) !DescribeImagePermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeImagePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DescribeImagePermissions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeImagePermissionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeImagePermissionsOutput, body, alloc);
}
