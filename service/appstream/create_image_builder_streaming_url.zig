...
pub const CreateImageBuilderStreamingURLInput = struct {
...
pub const CreateImageBuilderStreamingURLOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateImageBuilderStreamingURLInput, options: CallOptions) !CreateImageBuilderStreamingURLOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateImageBuilderStreamingURLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.CreateImageBuilderStreamingURL");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateImageBuilderStreamingURLOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateImageBuilderStreamingURLOutput, body, allocator);
}
