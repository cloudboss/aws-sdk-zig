...
pub const DescribeDirectoryConfigsInput = struct {
    /// The directory names.
    directory_names: ?[]const []const u8 = null,
...
pub const DescribeDirectoryConfigsOutput = struct {
    ...
    directory_configs: ?[]const DirectoryConfig = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDirectoryConfigsInput, options: CallOptions) !DescribeDirectoryConfigsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDirectoryConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DescribeDirectoryConfigs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDirectoryConfigsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDirectoryConfigsOutput, body, allocator);
}
