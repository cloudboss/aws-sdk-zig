...
pub const DescribeImageBuildersInput = struct {
...
pub const DescribeImageBuildersOutput = struct {
    /// Information about the image builders.
    image_builders: ?[]const ImageBuilder = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageBuildersInput, options: CallOptions) !DescribeImageBuildersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeImageBuildersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DescribeImageBuilders");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeImageBuildersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeImageBuildersOutput, body, allocator);
}
