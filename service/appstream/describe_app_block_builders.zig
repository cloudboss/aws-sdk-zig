...
pub const DescribeAppBlockBuildersInput = struct {
...
pub const DescribeAppBlockBuildersOutput = struct {
    /// The list that describes one or more app block builders.
    app_block_builders: ?[]const AppBlockBuilder = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppBlockBuildersInput, options: CallOptions) !DescribeAppBlockBuildersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAppBlockBuildersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DescribeAppBlockBuilders");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAppBlockBuildersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAppBlockBuildersOutput, body, allocator);
}
