...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSoftwareAssociationsInput, options: Options) !DescribeSoftwareAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSoftwareAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DescribeSoftwareAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSoftwareAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSoftwareAssociationsOutput, body, alloc);
}
