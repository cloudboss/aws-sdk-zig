...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateAppBlockBuilderAppBlockInput, options: Options) !DisassociateAppBlockBuilderAppBlockOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateAppBlockBuilderAppBlockInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PhotonAdminProxyService.DisassociateAppBlockBuilderAppBlock");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateAppBlockBuilderAppBlockOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
