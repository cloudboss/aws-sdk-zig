...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.DeleteImage");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteImageOutput {
    ...
    return aws.json.parseJsonObject(DeleteImageOutput, body, allocator);
}
