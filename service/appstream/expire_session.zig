...
pub const ExpireSessionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExpireSessionInput, options: CallOptions) !ExpireSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExpireSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PhotonAdminProxyService.ExpireSession");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExpireSessionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
