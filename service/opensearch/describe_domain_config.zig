...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDomainConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDomainConfigOutput {
    var result: DescribeDomainConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDomainConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
