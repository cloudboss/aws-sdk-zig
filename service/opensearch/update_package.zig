...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePackageInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PackageID\":");
    try aws.json.writeValue(@TypeOf(input.package_id), input.package_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePackageOutput {
    var result: UpdatePackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
