...
const OutboundConnection = @import("outbound_connection.zig").OutboundConnection;

pub const DescribeOutboundConnectionsInput = struct {
    ...
    /// subsequent `DescribeOutboundConnections` operations, which returns results
    /// in
    /// the next page.
...
pub const DescribeOutboundConnectionsOutput = struct {
    /// List of outbound connections that match the filter criteria.
    connections: ?[]const OutboundConnection = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOutboundConnectionsInput, options: CallOptions) !DescribeOutboundConnectionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeOutboundConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2021-01-01/opensearch/cc/outboundConnection/search";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeOutboundConnectionsOutput {
    var result: DescribeOutboundConnectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeOutboundConnectionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
