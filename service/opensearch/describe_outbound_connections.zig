...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOutboundConnectionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOutboundConnectionsOutput {
    var result: DescribeOutboundConnectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeOutboundConnectionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
