...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDomainHealthInput, options: Options) !DescribeDomainHealthOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDomainHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/health");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDomainHealthOutput {
    var result: DescribeDomainHealthOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDomainHealthOutput, body, alloc);
    }
    _ = status;
    _ = headers;
