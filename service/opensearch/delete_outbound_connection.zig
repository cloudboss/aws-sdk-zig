...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOutboundConnectionInput, options: CallOptions) !DeleteOutboundConnectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteOutboundConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/2021-01-01/opensearch/cc/outboundConnection/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteOutboundConnectionOutput {
    var result: DeleteOutboundConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteOutboundConnectionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
