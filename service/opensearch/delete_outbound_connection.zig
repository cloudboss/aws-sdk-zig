...
pub const DeleteOutboundConnectionInput = struct {
...
pub const DeleteOutboundConnectionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteOutboundConnectionInput, options: Options) !DeleteOutboundConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteOutboundConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2021-01-01/opensearch/cc/outboundConnection/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteOutboundConnectionOutput {
    var result: DeleteOutboundConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteOutboundConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
