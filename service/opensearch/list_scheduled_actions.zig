...
pub const ListScheduledActionsInput = struct {
    ...
    /// subsequent `ListScheduledActions` operations, which returns results in the
    /// next page.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListScheduledActionsInput, options: Options) !ListScheduledActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListScheduledActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/scheduledActions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListScheduledActionsOutput {
    var result: ListScheduledActionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListScheduledActionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
