...
const VpcEndpointError = @import("vpc_endpoint_error.zig").VpcEndpointError;
...
pub const DescribeVpcEndpointsOutput = struct {
    ...
    vpc_endpoint_errors: ?[]const VpcEndpointError = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpcEndpointsInput, options: CallOptions) !DescribeVpcEndpointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpcEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2021-01-01/opensearch/vpcEndpoints/describe";
    ...
    try body_buf.appendSlice(allocator, "\"VpcEndpointIds\":");
    try aws.json.writeValue(@TypeOf(input.vpc_endpoint_ids), input.vpc_endpoint_ids, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVpcEndpointsOutput {
    var result: DescribeVpcEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeVpcEndpointsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
