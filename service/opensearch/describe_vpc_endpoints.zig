...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpcEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"VpcEndpointIds\":");
    try aws.json.writeValue(@TypeOf(input.vpc_endpoint_ids), input.vpc_endpoint_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVpcEndpointsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeVpcEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
