...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDefaultApplicationSettingInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDefaultApplicationSettingOutput {
    var result: GetDefaultApplicationSettingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDefaultApplicationSettingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
