...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDefaultApplicationSettingInput, options: CallOptions) !GetDefaultApplicationSettingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDefaultApplicationSettingInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDefaultApplicationSettingOutput {
    var result: GetDefaultApplicationSettingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDefaultApplicationSettingOutput, body, allocator);
    }
    _ = status;
    _ = headers;
