...
pub const DeleteVpcEndpointOutput = struct {
    ...
    vpc_endpoint_summary: ?VpcEndpointSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVpcEndpointInput, options: Options) !DeleteVpcEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVpcEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2021-01-01/opensearch/vpcEndpoints/");
    try path_buf.appendSlice(alloc, input.vpc_endpoint_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVpcEndpointOutput {
    var result: DeleteVpcEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteVpcEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
