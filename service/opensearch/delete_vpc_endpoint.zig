...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteVpcEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/2021-01-01/opensearch/vpcEndpoints/");
    try path_buf.appendSlice(allocator, input.vpc_endpoint_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteVpcEndpointOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteVpcEndpointOutput, body, allocator);
    }
    _ = status;
    _ = headers;
