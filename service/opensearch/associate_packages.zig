...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociatePackagesInput, options: Options) !AssociatePackagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociatePackagesInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PackageList\":");
    try aws.json.writeValue(@TypeOf(input.package_list), input.package_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociatePackagesOutput {
    var result: AssociatePackagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociatePackagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
