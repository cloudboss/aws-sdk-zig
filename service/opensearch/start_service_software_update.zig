...
fn serializeRequest(alloc: std.mem.Allocator, input: StartServiceSoftwareUpdateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartServiceSoftwareUpdateOutput {
    var result: StartServiceSoftwareUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartServiceSoftwareUpdateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
