...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDryRunProgressInput, options: Options) !DescribeDryRunProgressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDryRunProgressInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dryRun");
    ...
    if (input.dry_run_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "dryRunId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.load_dry_run_config) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "loadDryRunConfig=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDryRunProgressOutput {
    var result: DescribeDryRunProgressOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDryRunProgressOutput, body, alloc);
    }
    _ = status;
    _ = headers;
