...
pub const ListVersionsInput = struct {
    ...
    /// If your initial `ListVersions` operation returns a `nextToken`,
    /// you can include the returned `nextToken` in subsequent
    /// `ListVersions` operations, which returns results in the next page.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVersionsInput, options: Options) !ListVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2021-01-01/opensearch/versions";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVersionsOutput {
    var result: ListVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
