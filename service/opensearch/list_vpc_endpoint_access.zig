...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVpcEndpointAccessInput, options: Options) !ListVpcEndpointAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVpcEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/listVpcEndpointAccess");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVpcEndpointAccessOutput {
    var result: ListVpcEndpointAccessOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVpcEndpointAccessOutput, body, alloc);
    }
    _ = status;
    _ = headers;
