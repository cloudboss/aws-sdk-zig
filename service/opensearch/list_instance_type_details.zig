...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInstanceTypeDetailsInput, options: Options) !ListInstanceTypeDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInstanceTypeDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2021-01-01/opensearch/instanceTypeDetails/");
    try path_buf.appendSlice(alloc, input.engine_version);
    ...
    if (input.instance_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "instanceType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.retrieve_a_zs) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "retrieveAZs=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInstanceTypeDetailsOutput {
    var result: ListInstanceTypeDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInstanceTypeDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
