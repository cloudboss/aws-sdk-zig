...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseReservedInstanceOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReservedInstanceOfferingId\":");
    try aws.json.writeValue(@TypeOf(input.reserved_instance_offering_id), input.reserved_instance_offering_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PurchaseReservedInstanceOfferingOutput {
    var result: PurchaseReservedInstanceOfferingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PurchaseReservedInstanceOfferingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
