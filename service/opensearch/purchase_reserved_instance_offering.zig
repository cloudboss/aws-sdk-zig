...
pub const PurchaseReservedInstanceOfferingInput = struct {
...
pub const PurchaseReservedInstanceOfferingOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurchaseReservedInstanceOfferingInput, options: CallOptions) !PurchaseReservedInstanceOfferingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PurchaseReservedInstanceOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2021-01-01/opensearch/purchaseReservedInstanceOffering";
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"ReservedInstanceOfferingId\":");
    try aws.json.writeValue(@TypeOf(input.reserved_instance_offering_id), input.reserved_instance_offering_id, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PurchaseReservedInstanceOfferingOutput {
    var result: PurchaseReservedInstanceOfferingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PurchaseReservedInstanceOfferingOutput, body, allocator);
    }
    _ = status;
    _ = headers;
