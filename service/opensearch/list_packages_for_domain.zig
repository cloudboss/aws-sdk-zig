...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPackagesForDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2021-01-01/domain/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPackagesForDomainOutput {
    var result: ListPackagesForDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPackagesForDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
