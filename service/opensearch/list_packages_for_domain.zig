...
pub const ListPackagesForDomainInput = struct {
    ...
    /// subsequent `ListPackagesForDomain` operations, which returns results in the
    /// next page.
...
pub const ListPackagesForDomainOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPackagesForDomainInput, options: CallOptions) !ListPackagesForDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPackagesForDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/2021-01-01/domain/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPackagesForDomainOutput {
    var result: ListPackagesForDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPackagesForDomainOutput, body, allocator);
    }
    _ = status;
    _ = headers;
