...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDomainMaintenanceStatusInput, options: Options) !GetDomainMaintenanceStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainMaintenanceStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "maintenanceId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.maintenance_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainMaintenanceStatusOutput {
    var result: GetDomainMaintenanceStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainMaintenanceStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
