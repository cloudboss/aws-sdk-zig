...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptInboundConnectionInput, options: CallOptions) !AcceptInboundConnectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptInboundConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/2021-01-01/opensearch/cc/inboundConnection/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptInboundConnectionOutput {
    var result: AcceptInboundConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptInboundConnectionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
