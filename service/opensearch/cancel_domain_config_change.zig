...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelDomainConfigChangeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/config/cancel");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelDomainConfigChangeOutput {
    var result: CancelDomainConfigChangeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelDomainConfigChangeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
