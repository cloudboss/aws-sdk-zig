...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPackageVersionHistoryInput, options: Options) !GetPackageVersionHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPackageVersionHistoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPackageVersionHistoryOutput {
    var result: GetPackageVersionHistoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPackageVersionHistoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
