...
pub const PackageDetails = struct {
    ...
    allow_listed_user_list: ?[]const []const u8,

    /// This represents the available configuration parameters for the package.
    available_package_configuration: ?PackageConfiguration,

    /// The package version.
    available_package_version: ?[]const u8,

    /// If the package is a `ZIP-PLUGIN` package, additional information about
    /// plugin properties.
    available_plugin_properties: ?PluginProperties,
