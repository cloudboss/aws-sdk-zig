...
pub const AddDataSourceInput = struct {
...
pub const AddDataSourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddDataSourceInput, options: Options) !AddDataSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddDataSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dataSource");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddDataSourceOutput {
    var result: AddDataSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddDataSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
