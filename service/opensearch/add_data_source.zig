...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddDataSourceInput, options: CallOptions) !AddDataSourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AddDataSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AddDataSourceOutput {
    var result: AddDataSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddDataSourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
