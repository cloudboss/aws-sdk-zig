...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDomainMaintenanceInput, options: CallOptions) !StartDomainMaintenanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartDomainMaintenanceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.node_id) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"NodeId\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartDomainMaintenanceOutput {
    var result: StartDomainMaintenanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDomainMaintenanceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
