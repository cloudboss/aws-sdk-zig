...
pub const StartDomainMaintenanceInput = struct {
...
pub const StartDomainMaintenanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDomainMaintenanceInput, options: Options) !StartDomainMaintenanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDomainMaintenanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/domainMaintenance");
    ...
    if (input.node_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"NodeId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDomainMaintenanceOutput {
    var result: StartDomainMaintenanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDomainMaintenanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
