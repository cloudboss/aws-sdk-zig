...
const DirectQueryDataSource = @import("direct_query_data_source.zig").DirectQueryDataSource;

pub const ListDirectQueryDataSourcesInput = struct {
...
pub const ListDirectQueryDataSourcesOutput = struct {
    /// A list of the direct query data sources that are returned by the
    /// `ListDirectQueryDataSources` API operation.
    direct_query_data_sources: ?[]const DirectQueryDataSource = null,

    next_token: ?[]const u8 = null,

    pub const json_field_names = .{
        .direct_query_data_sources = "DirectQueryDataSources",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDirectQueryDataSourcesInput, options: CallOptions) !ListDirectQueryDataSourcesOutput {
