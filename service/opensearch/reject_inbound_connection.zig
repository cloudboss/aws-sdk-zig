...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectInboundConnectionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectInboundConnectionOutput {
    var result: RejectInboundConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RejectInboundConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
