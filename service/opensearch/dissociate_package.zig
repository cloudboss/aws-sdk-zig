...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DissociatePackageInput, options: Options) !DissociatePackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DissociatePackageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2021-01-01/packages/dissociate/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DissociatePackageOutput {
    var result: DissociatePackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DissociatePackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
