...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDirectQueryDataSourceInput, options: Options) !GetDirectQueryDataSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDirectQueryDataSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDirectQueryDataSourceOutput {
    var result: GetDirectQueryDataSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDirectQueryDataSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
