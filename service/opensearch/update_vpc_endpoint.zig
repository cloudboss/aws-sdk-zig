...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVpcEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.vpc_endpoint_id), input.vpc_endpoint_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VpcOptions\":");
    try aws.json.writeValue(@TypeOf(input.vpc_options), input.vpc_options, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVpcEndpointOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVpcEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
