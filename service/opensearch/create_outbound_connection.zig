...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOutboundConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connection_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ConnectionProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"LocalDomainInfo\":");
    try aws.json.writeValue(@TypeOf(input.local_domain_info), input.local_domain_info, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RemoteDomainInfo\":");
    try aws.json.writeValue(@TypeOf(input.remote_domain_info), input.remote_domain_info, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOutboundConnectionOutput {
    var result: CreateOutboundConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateOutboundConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
