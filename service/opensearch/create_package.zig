...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePackageInput, options: Options) !CreatePackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePackageInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.package_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PackageConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.package_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PackageDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.package_encryption_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PackageEncryptionOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PackageName\":");
    try aws.json.writeValue(@TypeOf(input.package_name), input.package_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PackageSource\":");
    try aws.json.writeValue(@TypeOf(input.package_source), input.package_source, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PackageType\":");
    try aws.json.writeValue(@TypeOf(input.package_type), input.package_type, alloc, &body_buf);
    has_prev = true;
    if (input.package_vending_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PackageVendingOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePackageOutput {
    var result: CreatePackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePackageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
