...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePackageScopeInput, options: Options) !UpdatePackageScopeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePackageScopeInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PackageUserList\":");
    try aws.json.writeValue(@TypeOf(input.package_user_list), input.package_user_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePackageScopeOutput {
    var result: UpdatePackageScopeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePackageScopeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
