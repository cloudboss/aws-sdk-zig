...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelServiceSoftwareUpdateInput, options: Options) !CancelServiceSoftwareUpdateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelServiceSoftwareUpdateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelServiceSoftwareUpdateOutput {
    var result: CancelServiceSoftwareUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelServiceSoftwareUpdateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
