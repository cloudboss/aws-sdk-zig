...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddDirectQueryDataSourceInput, options: Options) !AddDirectQueryDataSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddDirectQueryDataSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DataSourceType\":");
    try aws.json.writeValue(@TypeOf(input.data_source_type), input.data_source_type, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OpenSearchArns\":");
    try aws.json.writeValue(@TypeOf(input.open_search_arns), input.open_search_arns, alloc, &body_buf);
    has_prev = true;
    if (input.tag_list) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TagList\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddDirectQueryDataSourceOutput {
    var result: AddDirectQueryDataSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddDirectQueryDataSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
