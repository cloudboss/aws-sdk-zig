...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInboundConnectionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInboundConnectionsOutput {
    var result: DescribeInboundConnectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeInboundConnectionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
