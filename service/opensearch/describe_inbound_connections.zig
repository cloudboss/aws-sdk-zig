...
pub const DescribeInboundConnectionsInput = struct {
    ...
    /// subsequent `DescribeInboundConnections` operations, which returns results in
    /// the next page.
...
pub const DescribeInboundConnectionsOutput = struct {
    /// List of inbound connections.
    connections: ?[]const InboundConnection = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInboundConnectionsInput, options: CallOptions) !DescribeInboundConnectionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInboundConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2021-01-01/opensearch/cc/inboundConnection/search";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInboundConnectionsOutput {
    var result: DescribeInboundConnectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeInboundConnectionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
