...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociatePackageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2021-01-01/packages/associate/");
    ...
    if (input.association_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AssociationConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.prerequisite_package_id_list) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PrerequisitePackageIDList\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
