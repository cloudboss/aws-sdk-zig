...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCompatibleVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCompatibleVersionsOutput {
    var result: GetCompatibleVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCompatibleVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
