...
pub const GetCompatibleVersionsInput = struct {
...
pub const GetCompatibleVersionsOutput = struct {
    /// A map of OpenSearch or Elasticsearch versions and the versions you can
    /// upgrade them
    /// to.
    compatible_versions: ?[]const CompatibleVersionsMap = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCompatibleVersionsInput, options: CallOptions) !GetCompatibleVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCompatibleVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2021-01-01/opensearch/compatibleVersions";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCompatibleVersionsOutput {
    var result: GetCompatibleVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCompatibleVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
