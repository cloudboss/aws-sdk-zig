...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateScheduledActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/scheduledAction/update");
    ...
    if (input.desired_start_time) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DesiredStartTime\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ScheduleAt\":");
    try aws.json.writeValue(@TypeOf(input.schedule_at), input.schedule_at, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateScheduledActionOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateScheduledActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
