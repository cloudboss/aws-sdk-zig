...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDomainsForPackageInput, options: CallOptions) !ListDomainsForPackageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDomainsForPackageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDomainsForPackageOutput {
    var result: ListDomainsForPackageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDomainsForPackageOutput, body, allocator);
    }
    _ = status;
    _ = headers;
