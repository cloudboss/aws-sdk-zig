...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDomainMaintenancesInput, options: Options) !ListDomainMaintenancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDomainMaintenancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/domainMaintenances");
    ...
    if (input.action) |v| {
        ...
        try query_buf.appendSlice(alloc, "action=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDomainMaintenancesOutput {
    var result: ListDomainMaintenancesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDomainMaintenancesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
