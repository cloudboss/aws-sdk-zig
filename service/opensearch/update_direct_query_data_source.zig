...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDirectQueryDataSourceInput, options: Options) !UpdateDirectQueryDataSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDirectQueryDataSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDirectQueryDataSourceOutput {
    var result: UpdateDirectQueryDataSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDirectQueryDataSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
