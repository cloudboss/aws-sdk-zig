...
pub const ListVpcEndpointsForDomainInput = struct {
    ...
    /// subsequent `ListEndpointsForDomain` operations, which returns results in the
    /// next page.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVpcEndpointsForDomainInput, options: Options) !ListVpcEndpointsForDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVpcEndpointsForDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/vpcEndpoints");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVpcEndpointsForDomainOutput {
    var result: ListVpcEndpointsForDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVpcEndpointsForDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
