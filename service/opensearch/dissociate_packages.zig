...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DissociatePackagesInput, options: Options) !DissociatePackagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DissociatePackagesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DissociatePackagesOutput {
    var result: DissociatePackagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DissociatePackagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
