...
const DomainNodesStatus = @import("domain_nodes_status.zig").DomainNodesStatus;

pub const DescribeDomainNodesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDomainNodesInput, options: Options) !DescribeDomainNodesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDomainNodesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDomainNodesOutput {
    var result: DescribeDomainNodesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDomainNodesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
