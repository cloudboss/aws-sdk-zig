...
const ReservedInstanceOffering = @import("reserved_instance_offering.zig").ReservedInstanceOffering;

pub const DescribeReservedInstanceOfferingsInput = struct {
...
pub const DescribeReservedInstanceOfferingsOutput = struct {
    ...
    reserved_instance_offerings: ?[]const ReservedInstanceOffering = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReservedInstanceOfferingsInput, options: Options) !DescribeReservedInstanceOfferingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedInstanceOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2021-01-01/opensearch/reservedInstanceOfferings";
    ...
    if (input.reserved_instance_offering_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "offeringId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReservedInstanceOfferingsOutput {
    var result: DescribeReservedInstanceOfferingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeReservedInstanceOfferingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
