...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDefaultApplicationSettingInput, options: Options) !PutDefaultApplicationSettingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDefaultApplicationSettingInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.application_arn), input.application_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"setAsDefault\":");
    try aws.json.writeValue(@TypeOf(input.set_as_default), input.set_as_default, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDefaultApplicationSettingOutput {
    var result: PutDefaultApplicationSettingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutDefaultApplicationSettingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
