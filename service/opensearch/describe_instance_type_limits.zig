...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceTypeLimitsInput, options: Options) !DescribeInstanceTypeLimitsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceTypeLimitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2021-01-01/opensearch/instanceTypeLimits/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceTypeLimitsOutput {
    var result: DescribeInstanceTypeLimitsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeInstanceTypeLimitsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
