...
fn serializeRequest(alloc: std.mem.Allocator, input: UpgradeDomainInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpgradeDomainOutput {
    var result: UpgradeDomainOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpgradeDomainOutput, body, alloc);
    }
    _ = status;
    _ = headers;
