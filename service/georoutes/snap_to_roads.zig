...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SnapToRoadsInput, options: Options) !SnapToRoadsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SnapToRoadsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.snapped_geometry_format) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"SnappedGeometryFormat\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.snap_radius) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SnapRadius\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TracePoints\":");
    try aws.json.writeValue(@TypeOf(input.trace_points), input.trace_points, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SnapToRoadsOutput {
    var result: SnapToRoadsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SnapToRoadsOutput, body, alloc);
    }
    _ = status;
