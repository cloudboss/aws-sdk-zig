...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateJobTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2017-08-29/jobTemplates/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateJobTemplateOutput {
    var result: UpdateJobTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateJobTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
