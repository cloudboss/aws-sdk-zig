...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePresetInput, options: Options) !UpdatePresetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePresetInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2017-08-29/presets/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePresetOutput {
    var result: UpdatePresetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePresetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
