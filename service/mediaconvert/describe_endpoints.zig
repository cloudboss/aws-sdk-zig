...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEndpointsInput, options: Options) !DescribeEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEndpointsOutput {
    var result: DescribeEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
