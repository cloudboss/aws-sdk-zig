...
pub const UpdateQueueInput = struct {
    ...
    /// jobs, per account. For reserved queues, update your reservation plan instead
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateQueueOutput {
    var result: UpdateQueueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQueueOutput, body, allocator);
    }
    _ = status;
    _ = headers;
