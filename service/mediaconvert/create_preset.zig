...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePresetInput, options: Options) !CreatePresetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePresetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePresetOutput {
    var result: CreatePresetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePresetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
