...
const QueueListBy = @import("queue_list_by.zig").QueueListBy;
...
pub const ListQueuesOutput = struct {
    ...
    total_concurrent_jobs: ?i32 = null,

    /// The remaining number of concurrent jobs that are not associated with a queue
    /// and are available to allocate to a queue. You can allocate these jobs when
    /// you create or update a queue.
    unallocated_concurrent_jobs: ?i32 = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .queues = "Queues",
        .total_concurrent_jobs = "TotalConcurrentJobs",
        .unallocated_concurrent_jobs = "UnallocatedConcurrentJobs",
    };
};
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQueuesOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQueuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
