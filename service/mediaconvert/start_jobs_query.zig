...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartJobsQueryInput, options: Options) !StartJobsQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartJobsQueryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartJobsQueryOutput {
    var result: StartJobsQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartJobsQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
