...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPresetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPresetsOutput {
    var result: ListPresetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPresetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
