...
pub const DescribeRxNormInferenceJobInput = struct {
...
pub const DescribeRxNormInferenceJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRxNormInferenceJobInput, options: CallOptions) !DescribeRxNormInferenceJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRxNormInferenceJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.DescribeRxNormInferenceJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeRxNormInferenceJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRxNormInferenceJobOutput, body, allocator);
}
