...
pub const ListPHIDetectionJobsInput = struct {
...
pub const ListPHIDetectionJobsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPHIDetectionJobsInput, options: CallOptions) !ListPHIDetectionJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPHIDetectionJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.ListPHIDetectionJobs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPHIDetectionJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPHIDetectionJobsOutput, body, allocator);
}
