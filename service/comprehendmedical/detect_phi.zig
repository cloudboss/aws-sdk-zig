...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectPHIInput, options: Options) !DetectPHIOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectPHIInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComprehendMedical_20181030.DetectPHI");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectPHIOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DetectPHIOutput, body, alloc);
}
