...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEntitiesDetectionV2JobInput, options: Options) !DescribeEntitiesDetectionV2JobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEntitiesDetectionV2JobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComprehendMedical_20181030.DescribeEntitiesDetectionV2Job");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEntitiesDetectionV2JobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEntitiesDetectionV2JobOutput, body, alloc);
}
