...
pub const ListICD10CMInferenceJobsInput = struct {
...
pub const ListICD10CMInferenceJobsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListICD10CMInferenceJobsInput, options: CallOptions) !ListICD10CMInferenceJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListICD10CMInferenceJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.ListICD10CMInferenceJobs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListICD10CMInferenceJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListICD10CMInferenceJobsOutput, body, allocator);
}
