...
fn serializeRequest(alloc: std.mem.Allocator, input: ListICD10CMInferenceJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComprehendMedical_20181030.ListICD10CMInferenceJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListICD10CMInferenceJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListICD10CMInferenceJobsOutput, body, alloc);
}
