...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePHIDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComprehendMedical_20181030.DescribePHIDetectionJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePHIDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePHIDetectionJobOutput, body, alloc);
}
