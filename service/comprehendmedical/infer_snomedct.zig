...
fn serializeRequest(allocator: std.mem.Allocator, input: InferSNOMEDCTInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.InferSNOMEDCT");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !InferSNOMEDCTOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(InferSNOMEDCTOutput, body, allocator);
}
