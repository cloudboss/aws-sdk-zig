...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEntitiesDetectionV2JobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComprehendMedical_20181030.ListEntitiesDetectionV2Jobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEntitiesDetectionV2JobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEntitiesDetectionV2JobsOutput, body, alloc);
}
