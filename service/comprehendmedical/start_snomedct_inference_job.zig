...
pub const StartSNOMEDCTInferenceJobInput = struct {
...
pub const StartSNOMEDCTInferenceJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSNOMEDCTInferenceJobInput, options: CallOptions) !StartSNOMEDCTInferenceJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartSNOMEDCTInferenceJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.StartSNOMEDCTInferenceJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartSNOMEDCTInferenceJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartSNOMEDCTInferenceJobOutput, body, allocator);
}
