...
pub const StartRxNormInferenceJobInput = struct {
...
pub const StartRxNormInferenceJobOutput = struct {
    /// The identifier of the job.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRxNormInferenceJobInput, options: CallOptions) !StartRxNormInferenceJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartRxNormInferenceJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.StartRxNormInferenceJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartRxNormInferenceJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartRxNormInferenceJobOutput, body, allocator);
}
