...
pub const StartICD10CMInferenceJobInput = struct {
...
pub const StartICD10CMInferenceJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartICD10CMInferenceJobInput, options: CallOptions) !StartICD10CMInferenceJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartICD10CMInferenceJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.StartICD10CMInferenceJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartICD10CMInferenceJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartICD10CMInferenceJobOutput, body, allocator);
}
