...
fn serializeRequest(alloc: std.mem.Allocator, input: StartICD10CMInferenceJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComprehendMedical_20181030.StartICD10CMInferenceJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartICD10CMInferenceJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartICD10CMInferenceJobOutput, body, alloc);
}
