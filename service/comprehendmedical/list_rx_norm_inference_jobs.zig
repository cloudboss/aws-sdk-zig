...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRxNormInferenceJobsInput, options: Options) !ListRxNormInferenceJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRxNormInferenceJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComprehendMedical_20181030.ListRxNormInferenceJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRxNormInferenceJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRxNormInferenceJobsOutput, body, alloc);
}
