...
pub const StartPHIDetectionJobInput = struct {
...
pub const StartPHIDetectionJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartPHIDetectionJobInput, options: CallOptions) !StartPHIDetectionJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartPHIDetectionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.StartPHIDetectionJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartPHIDetectionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartPHIDetectionJobOutput, body, allocator);
}
