...
pub const DescribeICD10CMInferenceJobInput = struct {
...
pub const DescribeICD10CMInferenceJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeICD10CMInferenceJobInput, options: CallOptions) !DescribeICD10CMInferenceJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeICD10CMInferenceJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.DescribeICD10CMInferenceJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeICD10CMInferenceJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeICD10CMInferenceJobOutput, body, allocator);
}
