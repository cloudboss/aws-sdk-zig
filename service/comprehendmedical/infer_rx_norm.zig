...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InferRxNormInput, options: Options) !InferRxNormOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InferRxNormInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComprehendMedical_20181030.InferRxNorm");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InferRxNormOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(InferRxNormOutput, body, alloc);
}
