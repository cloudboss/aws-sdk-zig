...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectEntitiesV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComprehendMedical_20181030.DetectEntitiesV2");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectEntitiesV2Output {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DetectEntitiesV2Output, body, alloc);
}
