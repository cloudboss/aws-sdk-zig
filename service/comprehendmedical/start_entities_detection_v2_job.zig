...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartEntitiesDetectionV2JobInput, options: CallOptions) !StartEntitiesDetectionV2JobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartEntitiesDetectionV2JobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComprehendMedical_20181030.StartEntitiesDetectionV2Job");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartEntitiesDetectionV2JobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartEntitiesDetectionV2JobOutput, body, allocator);
}
