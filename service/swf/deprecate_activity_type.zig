...
fn serializeRequest(alloc: std.mem.Allocator, input: DeprecateActivityTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.DeprecateActivityType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeprecateActivityTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
