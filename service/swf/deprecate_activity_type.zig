...
pub const DeprecateActivityTypeInput = struct {
...
pub const DeprecateActivityTypeOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeprecateActivityTypeInput, options: CallOptions) !DeprecateActivityTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeprecateActivityTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.DeprecateActivityType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeprecateActivityTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
