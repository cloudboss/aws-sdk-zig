...
pub const DeleteWorkflowTypeInput = struct {
...
pub const DeleteWorkflowTypeOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteWorkflowTypeInput, options: CallOptions) !DeleteWorkflowTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteWorkflowTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.DeleteWorkflowType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteWorkflowTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
