...
fn serializeRequest(alloc: std.mem.Allocator, input: CountPendingActivityTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.CountPendingActivityTasks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CountPendingActivityTasksOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CountPendingActivityTasksOutput, body, alloc);
}
