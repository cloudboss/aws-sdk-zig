...
pub const DeleteActivityTypeInput = struct {
...
pub const DeleteActivityTypeOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteActivityTypeInput, options: CallOptions) !DeleteActivityTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteActivityTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.DeleteActivityType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteActivityTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
