...
fn serializeRequest(alloc: std.mem.Allocator, input: UndeprecateActivityTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.UndeprecateActivityType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UndeprecateActivityTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
