...
pub const UndeprecateActivityTypeInput = struct {
...
pub const UndeprecateActivityTypeOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UndeprecateActivityTypeInput, options: CallOptions) !UndeprecateActivityTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UndeprecateActivityTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.UndeprecateActivityType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UndeprecateActivityTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
