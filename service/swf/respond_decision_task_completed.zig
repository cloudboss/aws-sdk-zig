...
fn serializeRequest(alloc: std.mem.Allocator, input: RespondDecisionTaskCompletedInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.RespondDecisionTaskCompleted");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RespondDecisionTaskCompletedOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
