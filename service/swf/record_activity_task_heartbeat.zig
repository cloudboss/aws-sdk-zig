...
pub const RecordActivityTaskHeartbeatInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RecordActivityTaskHeartbeatInput, options: CallOptions) !RecordActivityTaskHeartbeatOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RecordActivityTaskHeartbeatInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.RecordActivityTaskHeartbeat");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RecordActivityTaskHeartbeatOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(RecordActivityTaskHeartbeatOutput, body, allocator);
}
