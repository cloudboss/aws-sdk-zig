...
fn serializeRequest(alloc: std.mem.Allocator, input: RecordActivityTaskHeartbeatInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.RecordActivityTaskHeartbeat");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RecordActivityTaskHeartbeatOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(RecordActivityTaskHeartbeatOutput, body, alloc);
}
