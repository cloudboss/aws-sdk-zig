...
pub const UndeprecateDomainInput = struct {
...
pub const UndeprecateDomainOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UndeprecateDomainInput, options: CallOptions) !UndeprecateDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UndeprecateDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.UndeprecateDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UndeprecateDomainOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
