...
pub const ListWorkflowTypesInput = struct {
...
pub const ListWorkflowTypesOutput = struct {
    ...
    type_infos: ?[]const WorkflowTypeInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkflowTypesInput, options: CallOptions) !ListWorkflowTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListWorkflowTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.ListWorkflowTypes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListWorkflowTypesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListWorkflowTypesOutput, body, allocator);
}
