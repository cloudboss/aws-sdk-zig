...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeActivityTypeInput, options: Options) !DescribeActivityTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeActivityTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.DescribeActivityType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeActivityTypeOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeActivityTypeOutput, body, alloc);
}
