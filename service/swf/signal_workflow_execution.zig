...
fn serializeRequest(alloc: std.mem.Allocator, input: SignalWorkflowExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.SignalWorkflowExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SignalWorkflowExecutionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
