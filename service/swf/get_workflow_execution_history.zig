...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkflowExecutionHistoryInput, options: Options) !GetWorkflowExecutionHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkflowExecutionHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.GetWorkflowExecutionHistory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkflowExecutionHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetWorkflowExecutionHistoryOutput, body, alloc);
}
