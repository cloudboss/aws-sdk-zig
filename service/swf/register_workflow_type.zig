...
const RegisterWorkflowTypeOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterWorkflowTypeInput, options: Options) !RegisterWorkflowTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterWorkflowTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.RegisterWorkflowType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterWorkflowTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
