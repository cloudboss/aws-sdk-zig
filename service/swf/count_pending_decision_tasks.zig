...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CountPendingDecisionTasksInput, options: Options) !CountPendingDecisionTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CountPendingDecisionTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.CountPendingDecisionTasks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CountPendingDecisionTasksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CountPendingDecisionTasksOutput, body, alloc);
}
