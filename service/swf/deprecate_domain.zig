...
fn serializeRequest(alloc: std.mem.Allocator, input: DeprecateDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.DeprecateDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeprecateDomainOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
