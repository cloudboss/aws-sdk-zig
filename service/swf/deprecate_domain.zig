...
fn serializeRequest(allocator: std.mem.Allocator, input: DeprecateDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.DeprecateDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeprecateDomainOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
