...
fn serializeRequest(alloc: std.mem.Allocator, input: UndeprecateWorkflowTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.UndeprecateWorkflowType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UndeprecateWorkflowTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
