...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CountOpenWorkflowExecutionsInput, options: Options) !CountOpenWorkflowExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CountOpenWorkflowExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.CountOpenWorkflowExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CountOpenWorkflowExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CountOpenWorkflowExecutionsOutput, body, alloc);
}
