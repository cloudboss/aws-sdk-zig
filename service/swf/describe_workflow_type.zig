...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkflowTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.DescribeWorkflowType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkflowTypeOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeWorkflowTypeOutput, body, alloc);
}
