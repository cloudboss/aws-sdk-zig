...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListActivityTypesInput, options: Options) !ListActivityTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListActivityTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.ListActivityTypes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListActivityTypesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListActivityTypesOutput, body, alloc);
}
