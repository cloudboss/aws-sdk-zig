...
pub const DeprecateWorkflowTypeOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeprecateWorkflowTypeInput, options: CallOptions) !DeprecateWorkflowTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeprecateWorkflowTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.DeprecateWorkflowType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeprecateWorkflowTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
