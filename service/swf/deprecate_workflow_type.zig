...
fn serializeRequest(alloc: std.mem.Allocator, input: DeprecateWorkflowTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.DeprecateWorkflowType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeprecateWorkflowTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
