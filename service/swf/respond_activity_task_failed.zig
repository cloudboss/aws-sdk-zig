...
pub const RespondActivityTaskFailedInput = struct {
...
pub const RespondActivityTaskFailedOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RespondActivityTaskFailedInput, options: CallOptions) !RespondActivityTaskFailedOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RespondActivityTaskFailedInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.RespondActivityTaskFailed");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RespondActivityTaskFailedOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
