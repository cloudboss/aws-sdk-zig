...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOpenWorkflowExecutionsInput, options: CallOptions) !ListOpenWorkflowExecutionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOpenWorkflowExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.ListOpenWorkflowExecutions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOpenWorkflowExecutionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListOpenWorkflowExecutionsOutput, body, allocator);
}
