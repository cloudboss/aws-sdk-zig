...
fn serializeRequest(alloc: std.mem.Allocator, input: RespondActivityTaskCanceledInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.RespondActivityTaskCanceled");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RespondActivityTaskCanceledOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
