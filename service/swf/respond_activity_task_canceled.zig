...
pub const RespondActivityTaskCanceledInput = struct {
...
pub const RespondActivityTaskCanceledOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RespondActivityTaskCanceledInput, options: CallOptions) !RespondActivityTaskCanceledOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RespondActivityTaskCanceledInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.RespondActivityTaskCanceled");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RespondActivityTaskCanceledOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
