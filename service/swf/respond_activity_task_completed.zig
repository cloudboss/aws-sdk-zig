...
fn serializeRequest(alloc: std.mem.Allocator, input: RespondActivityTaskCompletedInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.RespondActivityTaskCompleted");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RespondActivityTaskCompletedOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
