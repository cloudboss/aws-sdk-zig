...
pub const RespondActivityTaskCompletedInput = struct {
    ...
    /// value.
    /// If the task is passed to another process, its `taskToken` must also be
    /// passed.
...
pub const RespondActivityTaskCompletedOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RespondActivityTaskCompletedInput, options: CallOptions) !RespondActivityTaskCompletedOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RespondActivityTaskCompletedInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SimpleWorkflowService.RespondActivityTaskCompleted");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RespondActivityTaskCompletedOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
