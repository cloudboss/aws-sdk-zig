...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListClosedWorkflowExecutionsInput, options: Options) !ListClosedWorkflowExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListClosedWorkflowExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.ListClosedWorkflowExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListClosedWorkflowExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListClosedWorkflowExecutionsOutput, body, alloc);
}
