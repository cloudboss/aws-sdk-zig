...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartWorkflowExecutionInput, options: Options) !StartWorkflowExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartWorkflowExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SimpleWorkflowService.StartWorkflowExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartWorkflowExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartWorkflowExecutionOutput, body, alloc);
}
