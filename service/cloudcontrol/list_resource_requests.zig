...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceRequestsInput, options: Options) !ListResourceRequestsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceRequestsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudApiService.ListResourceRequests");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceRequestsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceRequestsOutput, body, alloc);
}
