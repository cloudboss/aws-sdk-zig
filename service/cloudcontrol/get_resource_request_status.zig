...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceRequestStatusInput, options: Options) !GetResourceRequestStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceRequestStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudApiService.GetResourceRequestStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceRequestStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetResourceRequestStatusOutput, body, alloc);
}
