...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDevEnvironmentSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.session_configuration), input.session_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDevEnvironmentSessionOutput {
    var result: StartDevEnvironmentSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDevEnvironmentSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
