...
pub const StopDevEnvironmentInput = struct {
...
pub const StopDevEnvironmentOutput = struct {
    /// The system-generated unique ID of the Dev Environment.
    id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopDevEnvironmentInput, options: CallOptions) !StopDevEnvironmentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopDevEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopDevEnvironmentOutput {
    var result: StopDevEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopDevEnvironmentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
