...
fn serializeRequest(alloc: std.mem.Allocator, input: StopDevEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopDevEnvironmentOutput {
    var result: StopDevEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopDevEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
