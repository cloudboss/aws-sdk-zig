...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDevEnvironmentInput, options: Options) !CreateDevEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDevEnvironmentInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"persistentStorage\":");
    try aws.json.writeValue(@TypeOf(input.persistent_storage), input.persistent_storage, alloc, &body_buf);
    has_prev = true;
    if (input.repositories) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"repositories\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.vpc_connection_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"vpcConnectionName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDevEnvironmentOutput {
    var result: CreateDevEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDevEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
