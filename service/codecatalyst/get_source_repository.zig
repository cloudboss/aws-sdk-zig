...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSourceRepositoryInput, options: Options) !GetSourceRepositoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSourceRepositoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSourceRepositoryOutput {
    var result: GetSourceRepositoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSourceRepositoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
