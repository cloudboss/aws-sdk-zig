...
const DevEnvironmentSummary = @import("dev_environment_summary.zig").DevEnvironmentSummary;

pub const ListDevEnvironmentsInput = struct {
...
pub const ListDevEnvironmentsOutput = struct {
    /// Information about the Dev Environments in a project.
    items: ?[]const DevEnvironmentSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDevEnvironmentsInput, options: CallOptions) !ListDevEnvironmentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDevEnvironmentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDevEnvironmentsOutput {
    var result: ListDevEnvironmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevEnvironmentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
