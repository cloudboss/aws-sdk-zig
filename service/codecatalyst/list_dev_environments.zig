...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDevEnvironmentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDevEnvironmentsOutput {
    var result: ListDevEnvironmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevEnvironmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
