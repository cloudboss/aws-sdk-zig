...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSourceRepositoryBranchesInput, options: CallOptions) !ListSourceRepositoryBranchesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListSourceRepositoryBranchesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListSourceRepositoryBranchesOutput {
    var result: ListSourceRepositoryBranchesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSourceRepositoryBranchesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
