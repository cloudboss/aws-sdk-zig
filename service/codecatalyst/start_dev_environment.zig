...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDevEnvironmentInput, options: Options) !StartDevEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDevEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDevEnvironmentOutput {
    var result: StartDevEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDevEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
