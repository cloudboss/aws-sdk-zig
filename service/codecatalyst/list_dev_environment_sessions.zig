...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDevEnvironmentSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.dev_environment_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDevEnvironmentSessionsOutput {
    var result: ListDevEnvironmentSessionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevEnvironmentSessionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
