...
const DevEnvironmentSessionSummary = @import("dev_environment_session_summary.zig").DevEnvironmentSessionSummary;

pub const ListDevEnvironmentSessionsInput = struct {
...
pub const ListDevEnvironmentSessionsOutput = struct {
    /// Information about each session retrieved in the list.
    items: ?[]const DevEnvironmentSessionSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDevEnvironmentSessionsInput, options: CallOptions) !ListDevEnvironmentSessionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDevEnvironmentSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.dev_environment_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDevEnvironmentSessionsOutput {
    var result: ListDevEnvironmentSessionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevEnvironmentSessionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
