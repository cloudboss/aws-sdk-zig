...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSpaceInput, options: Options) !GetSpaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSpaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSpaceOutput {
    var result: GetSpaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSpaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
