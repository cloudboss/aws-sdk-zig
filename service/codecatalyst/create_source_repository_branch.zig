...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSourceRepositoryBranchInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.head_commit_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"headCommitId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSourceRepositoryBranchOutput {
    var result: CreateSourceRepositoryBranchOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSourceRepositoryBranchOutput, body, alloc);
    }
    _ = status;
    _ = headers;
