...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartWorkflowRunInput, options: Options) !StartWorkflowRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartWorkflowRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/workflowRuns");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartWorkflowRunOutput {
    var result: StartWorkflowRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartWorkflowRunOutput, body, alloc);
    }
    _ = status;
    _ = headers;
