...
const ListSourceRepositoriesItem = @import("list_source_repositories_item.zig").ListSourceRepositoriesItem;

pub const ListSourceRepositoriesInput = struct {
...
pub const ListSourceRepositoriesOutput = struct {
    /// Information about the source repositories.
    items: ?[]const ListSourceRepositoriesItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSourceRepositoriesInput, options: Options) !ListSourceRepositoriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSourceRepositoriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sourceRepositories");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSourceRepositoriesOutput {
    var result: ListSourceRepositoriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSourceRepositoriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
