...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDevEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDevEnvironmentOutput {
    var result: DeleteDevEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDevEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
