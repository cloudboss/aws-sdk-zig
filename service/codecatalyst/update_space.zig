...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSpaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSpaceOutput {
    var result: UpdateSpaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSpaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
