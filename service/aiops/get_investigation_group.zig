...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInvestigationGroupInput, options: Options) !GetInvestigationGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInvestigationGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInvestigationGroupOutput {
    var result: GetInvestigationGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInvestigationGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
