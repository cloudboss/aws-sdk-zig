...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInvestigationGroupInput, options: CallOptions) !CreateInvestigationGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateInvestigationGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateInvestigationGroupOutput {
    var result: CreateInvestigationGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateInvestigationGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
