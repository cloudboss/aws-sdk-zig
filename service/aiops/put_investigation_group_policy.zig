...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutInvestigationGroupPolicyInput, options: Options) !PutInvestigationGroupPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutInvestigationGroupPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutInvestigationGroupPolicyOutput {
    var result: PutInvestigationGroupPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutInvestigationGroupPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
