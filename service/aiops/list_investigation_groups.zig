...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInvestigationGroupsInput, options: Options) !ListInvestigationGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInvestigationGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInvestigationGroupsOutput {
    var result: ListInvestigationGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInvestigationGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
