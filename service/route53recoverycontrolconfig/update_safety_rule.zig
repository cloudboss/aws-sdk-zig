...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSafetyRuleInput, options: Options) !UpdateSafetyRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSafetyRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.assertion_rule_update) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AssertionRuleUpdate\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.gating_rule_update) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"GatingRuleUpdate\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSafetyRuleOutput {
    var result: UpdateSafetyRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSafetyRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
