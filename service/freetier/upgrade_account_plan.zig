...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpgradeAccountPlanInput, options: Options) !UpgradeAccountPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpgradeAccountPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFreeTierService.UpgradeAccountPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpgradeAccountPlanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpgradeAccountPlanOutput, body, alloc);
}
