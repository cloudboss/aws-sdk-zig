...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountPlanStateInput, options: Options) !GetAccountPlanStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccountPlanStateInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    const endpoint = try config.getEndpointForService("freetier", "FreeTier", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFreeTierService.GetAccountPlanState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccountPlanStateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAccountPlanStateOutput, body, alloc);
}
