...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFreeTierUsageInput, options: Options) !GetFreeTierUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFreeTierUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFreeTierService.GetFreeTierUsage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFreeTierUsageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetFreeTierUsageOutput, body, alloc);
}
