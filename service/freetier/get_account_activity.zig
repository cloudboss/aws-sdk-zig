...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountActivityInput, options: Options) !GetAccountActivityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccountActivityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFreeTierService.GetAccountActivity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccountActivityOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetAccountActivityOutput, body, alloc);
}
