...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountActivitiesInput, options: Options) !ListAccountActivitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountActivitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFreeTierService.ListAccountActivities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountActivitiesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListAccountActivitiesOutput, body, alloc);
}
