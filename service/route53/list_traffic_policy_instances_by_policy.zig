...
pub const ListTrafficPolicyInstancesByPolicyInput = struct {
    ...
    /// ID of the first traffic policy instance that Amazon Route 53 will return if
    /// you submit
    /// another request.
    ///
    /// If the value of `IsTruncated` in the previous response was
...
pub const ListTrafficPolicyInstancesByPolicyOutput = struct {
    /// If `IsTruncated` is `true`, `HostedZoneIdMarker` is
    /// the ID of the hosted zone of the first traffic policy instance in the next
    /// group of
    ...
    /// sets that are associated with the first traffic policy instance in the next
    /// group of
    /// `MaxItems` traffic policy instances.
    traffic_policy_instance_type_marker: ?RRType = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrafficPolicyInstancesByPolicyInput, options: CallOptions) !ListTrafficPolicyInstancesByPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTrafficPolicyInstancesByPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2013-04-01/trafficpolicyinstances/trafficpolicy";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTrafficPolicyInstancesByPolicyOutput {
    var result: ListTrafficPolicyInstancesByPolicyOutput = undefined;
