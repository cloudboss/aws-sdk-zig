...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetHostedZoneCountInput, options: Options) !GetHostedZoneCountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetHostedZoneCountInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    const path = "/2013-04-01/hostedzonecount";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetHostedZoneCountOutput {
    _ = alloc;
    var result: GetHostedZoneCountOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HostedZoneCount")) {
                    result.hosted_zone_count = std.fmt.parseInt(i64, try reader.readElementText(), 10) catch null;
                } else {
