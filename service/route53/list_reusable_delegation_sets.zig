...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReusableDelegationSetsInput, options: Options) !ListReusableDelegationSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReusableDelegationSetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReusableDelegationSetsOutput {
    var result: ListReusableDelegationSetsOutput = .{};
    _ = status;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DelegationSets")) {
                    result.delegation_sets = try serde.deserializeDelegationSets(&reader, alloc, "DelegationSet");
