...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrafficPolicyVersionInput, options: Options) !CreateTrafficPolicyVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrafficPolicyVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<CreateTrafficPolicyVersionRequest>");
    if (input.comment) |v| {
    ...
    try aws.xml.appendXmlEscaped(alloc, &body_buf, input.document);
    try body_buf.appendSlice(alloc, "</Document>");
    try body_buf.appendSlice(alloc, "</CreateTrafficPolicyVersionRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrafficPolicyVersionOutput {
    var result: CreateTrafficPolicyVersionOutput = .{};
    _ = status;
