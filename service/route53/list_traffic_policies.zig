...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrafficPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.traffic_policy_id_marker) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "trafficpolicyid=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrafficPoliciesOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.traffic_policy_id_marker = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "TrafficPolicySummaries")) {
                    result.traffic_policy_summaries = try serde.deserializeTrafficPolicySummaries(&reader, alloc, "TrafficPolicySummary");
                } else {
