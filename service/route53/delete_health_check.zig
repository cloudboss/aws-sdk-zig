...
pub const DeleteHealthCheckInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteHealthCheckInput, options: Options) !DeleteHealthCheckOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteHealthCheckInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteHealthCheckOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteHealthCheckOutput = .{};
