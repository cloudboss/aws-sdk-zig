...
pub const EnableHostedZoneDNSSECInput = struct {
    /// A unique string used to identify a hosted zone.
    hosted_zone_id: []const u8,
};

pub const EnableHostedZoneDNSSECOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableHostedZoneDNSSECInput, options: Options) !EnableHostedZoneDNSSECOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableHostedZoneDNSSECInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/enable-dnssec");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableHostedZoneDNSSECOutput {
