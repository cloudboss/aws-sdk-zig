...
pub const HealthCheckConfig = struct {
    ...
    /// route traffic
    /// to the corresponding resources. If you want to stop routing traffic to a
    /// resource,
    /// change the value of
    ...
    /// that you want
    /// to associate with a `CALCULATED` health check, use the
    /// [ChildHealthChecks](https://docs.aws.amazon.com/Route53/latest/APIReference/API_UpdateHealthCheck.html#Route53-UpdateHealthCheck-request-ChildHealthChecks) element.
    ///
    /// Note the following:
    ///
    ...
    /// from your
    /// endpoint and the time that it sends the next health check request. Each
    /// Route 53 health
    ...
    /// connection. If successful, Route 53 submits an HTTPS request and waits for
    /// an
    /// HTTP status code of 200 or greater and less than 400.
    ///
    ...
    /// TCP connection. If successful, Route 53 submits an HTTP request and searches
    /// the
    ...
    /// and searches the first 5,120 bytes of the response body for the string that
    /// you
    ...
    /// the status of other health checks, Route 53 adds up the number of health
    /// checks
    /// that Route 53 health checkers consider to be healthy and compares that
    /// number
    /// with the value of `HealthThreshold`.
    ///
    /// * **RECOVERY_CONTROL**: The health check is associated with a
    /// Route53 Application Recovery Controller routing control. If the routing
    /// control
