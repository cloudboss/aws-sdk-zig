...
pub const GetHealthCheckLastFailureReasonInput = struct {
...
pub const GetHealthCheckLastFailureReasonOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetHealthCheckLastFailureReasonInput, options: Options) !GetHealthCheckLastFailureReasonOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetHealthCheckLastFailureReasonInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/lastfailurereason");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetHealthCheckLastFailureReasonOutput {
