...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ChangeResourceRecordSetsInput, options: Options) !ChangeResourceRecordSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ChangeResourceRecordSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/rrset");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<ChangeResourceRecordSetsRequest>");
    try body_buf.appendSlice(alloc, "<ChangeBatch>");
    try serde.serializeChangeBatch(alloc, &body_buf, input.change_batch);
    try body_buf.appendSlice(alloc, "</ChangeBatch>");
    try body_buf.appendSlice(alloc, "</ChangeResourceRecordSetsRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ChangeResourceRecordSetsOutput {
