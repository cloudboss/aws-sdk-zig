...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrafficPolicyVersionsInput, options: Options) !ListTrafficPolicyVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrafficPolicyVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2013-04-01/trafficpolicies/");
    ...
    if (input.traffic_policy_version_marker) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "trafficpolicyversion=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrafficPolicyVersionsOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.traffic_policies = try serde.deserializeTrafficPolicies(&reader, alloc, "TrafficPolicy");
                } else if (std.mem.eql(u8, e.local, "TrafficPolicyVersionMarker")) {
                    result.traffic_policy_version_marker = try alloc.dupe(u8, try reader.readElementText());
                } else {
