...
pub const ResourceRecordSet = struct {
    ...
    /// always
    /// responds to DNS queries with the applicable value from the secondary
    /// resource
    ...
    /// origin of
    /// the query. For example, if you want all queries from Africa to be routed to
    /// a web server
    ...
    /// regions (for
    /// example, one resource record set for a continent and one for a country on
    /// the same
    /// continent), priority goes to the smallest geographic region. This allows you
    /// to route
    /// most queries for a continent to one resource and to route queries for a
    /// country on that
    ...
    /// queries
    /// from locations that it can't identify. We recommend that you create a
    /// resource
    /// record set for which the value of `CountryCode` is `*`. Two
    /// groups of queries are routed to the resource that you specify in this
    /// record:
    /// queries that come from locations for which you haven't created geolocation
    /// resource
    ...
    /// choosing between two or more resource record sets to respond to a DNS query,
    /// and you
    ...
    /// checking the health of a group of non-alias resource record sets that have
    /// the
    /// same routing policy, name, and type (such as multiple weighted records named
    /// www.example.com with a type of A) and you specify health check IDs for all
    /// the
    ...
    /// looks for
    /// a resource record set for the larger, associated geographic region. For
    /// example, suppose
    /// you have resource record sets for a state in the United States, for the
    /// entire United
    ...
    /// healthy
    /// resource record sets in the following order until it finds a resource record
    /// set for
    ...
    /// that you
    /// create a separate health check for each endpoint. For example, create a
    /// health check for
    ...
    ///   set,
    /// Amazon Route 53 responds to DNS queries with the corresponding IP address
    /// only
    ...
    ///   have
    /// eight or fewer healthy records, Route 53 responds to all DNS queries with
    /// all
    /// the healthy records.
    ///
    ...
    /// where you created the resource that this resource record set refers to. The
    /// resource
    /// typically is an Amazon Web Services resource, such as an EC2 instance or an
    /// ELB load
    ...
    /// which you
    /// have created latency resource record sets, Route 53 selects the latency
    /// resource record
    /// set that has the lowest latency between the end user and the associated
    /// Amazon EC2
    ...
    ///   EC2
    /// Regions. Route 53 will choose the region with the best latency from among
    /// the
    ...
    /// sets that have the same combination of name and type, such as multiple
    /// weighted resource
    ...
    /// in [RFC4408] have led to some interoperability issues. Accordingly, its use
    /// is no
    /// longer appropriate for SPF version 1; implementations are not to use it." In
    /// RFC
    /// 7208, see section 14.1, [The SPF DNS Record
    ...
    /// resource
    /// record set. Route 53 calculates the sum of the weights for the resource
    /// record sets that
    /// have the same combination of DNS name and type. Route 53 then responds to
    /// queries based
    ...
    /// information, see [Options for Configuring Route 53 Active-Active and
    /// Active-Passive
