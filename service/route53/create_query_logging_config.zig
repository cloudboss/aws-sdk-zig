...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateQueryLoggingConfigInput, options: Options) !CreateQueryLoggingConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateQueryLoggingConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<CreateQueryLoggingConfigRequest>");
    try body_buf.appendSlice(alloc, "<CloudWatchLogsLogGroupArn>");
    try aws.xml.appendXmlEscaped(alloc, &body_buf, input.cloud_watch_logs_log_group_arn);
    try body_buf.appendSlice(alloc, "</CloudWatchLogsLogGroupArn>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateQueryLoggingConfigOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "QueryLoggingConfig")) {
                    result.query_logging_config = try serde.deserializeQueryLoggingConfig(&reader, alloc);
                } else {
