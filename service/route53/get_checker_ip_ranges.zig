...
pub const GetCheckerIpRangesOutput = struct {
    ...
    /// Amazon Route
    /// 53 health checkers.
    checker_ip_ranges: ?[]const []const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCheckerIpRangesInput, options: CallOptions) !GetCheckerIpRangesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCheckerIpRangesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    const path = "/2013-04-01/checkeripranges";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCheckerIpRangesOutput {
    var result: GetCheckerIpRangesOutput = undefined;
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CheckerIpRanges")) {
                    result.checker_ip_ranges = try serde.deserializeCheckerIpRanges(allocator, &reader, "member");
                } else {
