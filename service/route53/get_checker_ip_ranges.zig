...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCheckerIpRangesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCheckerIpRangesOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CheckerIpRanges")) {
                    result.checker_ip_ranges = try serde.deserializeCheckerIpRanges(&reader, alloc, "member");
                } else {
