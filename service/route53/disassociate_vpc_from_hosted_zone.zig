...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateVPCFromHostedZoneInput, options: Options) !DisassociateVPCFromHostedZoneOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateVPCFromHostedZoneInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/disassociatevpc");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<DisassociateVPCFromHostedZoneRequest>");
    if (input.comment) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateVPCFromHostedZoneOutput {
    var result: DisassociateVPCFromHostedZoneOutput = .{};
    _ = status;
