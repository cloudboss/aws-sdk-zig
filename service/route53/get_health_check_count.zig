...
pub const GetHealthCheckCountOutput = struct {
    ...
    /// account.
    health_check_count: i64,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetHealthCheckCountInput, options: CallOptions) !GetHealthCheckCountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetHealthCheckCountInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    const path = "/2013-04-01/healthcheckcount";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetHealthCheckCountOutput {
    _ = allocator;
    var result: GetHealthCheckCountOutput = undefined;
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HealthCheckCount")) {
                    result.health_check_count = try std.fmt.parseInt(i64, try reader.readElementText(), 10);
                } else {
