...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateHostedZoneCommentInput, options: Options) !UpdateHostedZoneCommentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateHostedZoneCommentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<UpdateHostedZoneCommentRequest>");
    if (input.comment) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateHostedZoneCommentOutput {
    var result: UpdateHostedZoneCommentOutput = .{};
    _ = status;
