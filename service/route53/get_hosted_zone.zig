...
pub const GetHostedZoneInput = struct {
    /// The ID of the hosted zone that you want to get information about.
    id: []const u8,
};

pub const GetHostedZoneOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetHostedZoneInput, options: Options) !GetHostedZoneOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetHostedZoneInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetHostedZoneOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DelegationSet")) {
                    ...
                    result.vp_cs = try serde.deserializeVPCs(&reader, alloc, "VPC");
                } else {
