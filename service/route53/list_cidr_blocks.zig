...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCidrBlocksInput, options: Options) !ListCidrBlocksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCidrBlocksInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/cidrblocks");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCidrBlocksOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CidrBlocks")) {
                    result.cidr_blocks = try serde.deserializeCidrBlockSummaries(&reader, alloc, "member");
