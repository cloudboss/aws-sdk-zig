...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceRecordSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.start_record_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "identifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceRecordSetsOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "IsTruncated")) {
                    ...
                    result.next_record_identifier = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "NextRecordName")) {
                    result.next_record_name = try alloc.dupe(u8, try reader.readElementText());
                    ...
                    result.resource_record_sets = try serde.deserializeResourceRecordSets(&reader, alloc, "ResourceRecordSet");
                } else {
