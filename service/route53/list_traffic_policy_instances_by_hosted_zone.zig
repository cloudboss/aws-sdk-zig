...
pub const ListTrafficPolicyInstancesByHostedZoneInput = struct {
...
pub const ListTrafficPolicyInstancesByHostedZoneOutput = struct {
    /// A flag that indicates whether there are more traffic policy instances to be
    /// listed. If
    /// the response was truncated, you can get the next group of traffic policy
    /// instances by
    ...
    /// the DNS type of the resource record sets that are associated with the first
    /// traffic
    /// policy instance in the next group of traffic policy instances.
    traffic_policy_instance_type_marker: ?RRType = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrafficPolicyInstancesByHostedZoneInput, options: CallOptions) !ListTrafficPolicyInstancesByHostedZoneOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTrafficPolicyInstancesByHostedZoneInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2013-04-01/trafficpolicyinstances/hostedzone";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTrafficPolicyInstancesByHostedZoneOutput {
    var result: ListTrafficPolicyInstancesByHostedZoneOutput = undefined;
