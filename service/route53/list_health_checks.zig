...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHealthChecksInput, options: Options) !ListHealthChecksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHealthChecksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHealthChecksOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HealthChecks")) {
                    result.health_checks = try serde.deserializeHealthChecks(&reader, alloc, "HealthCheck");
