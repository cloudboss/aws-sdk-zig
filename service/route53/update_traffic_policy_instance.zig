...
pub const UpdateTrafficPolicyInstanceInput = struct {
...
pub const UpdateTrafficPolicyInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTrafficPolicyInstanceInput, options: Options) !UpdateTrafficPolicyInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTrafficPolicyInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2013-04-01/trafficpolicyinstance/");
    try path_buf.appendSlice(alloc, input.id);
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<UpdateTrafficPolicyInstanceRequest>");
    try body_buf.appendSlice(alloc, "<TrafficPolicyId>");
    try aws.xml.appendXmlEscaped(alloc, &body_buf, input.traffic_policy_id);
    try body_buf.appendSlice(alloc, "</TrafficPolicyId>");
    try body_buf.appendSlice(alloc, "<TrafficPolicyVersion>");
    {
    ...
    {
        const num_str = std.fmt.allocPrint(alloc, "{d}", .{input.ttl}) catch "";
        try body_buf.appendSlice(alloc, num_str);
    }
    try body_buf.appendSlice(alloc, "</TTL>");
    try body_buf.appendSlice(alloc, "</UpdateTrafficPolicyInstanceRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTrafficPolicyInstanceOutput {
    var result: UpdateTrafficPolicyInstanceOutput = .{};
    _ = status;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TrafficPolicyInstance")) {
                    result.traffic_policy_instance = try serde.deserializeTrafficPolicyInstance(&reader, alloc);
                } else {
