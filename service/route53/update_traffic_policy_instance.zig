...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTrafficPolicyInstanceInput, options: CallOptions) !UpdateTrafficPolicyInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTrafficPolicyInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/2013-04-01/trafficpolicyinstance/");
    try path_buf.appendSlice(allocator, input.id);
    const path = try path_buf.toOwnedSlice(allocator);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(allocator, "<UpdateTrafficPolicyInstanceRequest xmlns=\"https://route53.amazonaws.com/doc/2013-04-01/\">");
    ...
    try body_buf.appendSlice(allocator, "</TTL>");
    try body_buf.appendSlice(allocator, "</UpdateTrafficPolicyInstanceRequest>");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTrafficPolicyInstanceOutput {
    var result: UpdateTrafficPolicyInstanceOutput = undefined;
    _ = status;
