...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQueryLoggingConfigsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQueryLoggingConfigsOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.query_logging_configs = try serde.deserializeQueryLoggingConfigs(&reader, alloc, "QueryLoggingConfig");
                } else {
