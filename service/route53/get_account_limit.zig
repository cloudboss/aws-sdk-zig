...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountLimitInput, options: Options) !GetAccountLimitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccountLimitInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccountLimitOutput {
    var result: GetAccountLimitOutput = .{};
    _ = status;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Count")) {
                    ...
                    result.limit = try serde.deserializeAccountLimit(&reader, alloc);
                } else {
