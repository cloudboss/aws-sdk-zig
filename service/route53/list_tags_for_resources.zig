...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTagsForResourcesInput, options: Options) !ListTagsForResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTagsForResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2013-04-01/tags/");
    try path_buf.appendSlice(alloc, input.resource_type);
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<ListTagsForResourcesRequest>");
    try body_buf.appendSlice(alloc, "<ResourceIds>");
    try serde.serializeTagResourceIdList(alloc, &body_buf, input.resource_ids, "ResourceId");
    try body_buf.appendSlice(alloc, "</ResourceIds>");
    try body_buf.appendSlice(alloc, "</ListTagsForResourcesRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTagsForResourcesOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ResourceTagSets")) {
                    result.resource_tag_sets = try serde.deserializeResourceTagSetList(&reader, alloc, "ResourceTagSet");
                } else {
