...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDNSSECInput, options: Options) !GetDNSSECOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDNSSECInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dnssec");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDNSSECOutput {
    var result: GetDNSSECOutput = .{};
    _ = status;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "KeySigningKeys")) {
                    result.key_signing_keys = try serde.deserializeKeySigningKeys(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "Status")) {
                    result.status = try serde.deserializeDNSSECStatus(&reader, alloc);
                } else {
