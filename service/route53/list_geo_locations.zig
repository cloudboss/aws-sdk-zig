...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGeoLocationsInput, options: Options) !ListGeoLocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGeoLocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.start_continent_code) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "startcontinentcode=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.start_country_code) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "startcountrycode=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.start_subdivision_code) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "startsubdivisioncode=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGeoLocationsOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GeoLocationDetailsList")) {
                    result.geo_location_details_list = try serde.deserializeGeoLocationDetailsList(&reader, alloc, "GeoLocationDetails");
                    ...
                    result.next_continent_code = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "NextCountryCode")) {
                    result.next_country_code = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "NextSubdivisionCode")) {
                    result.next_subdivision_code = try alloc.dupe(u8, try reader.readElementText());
                } else {
