...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCidrLocationsInput, options: CallOptions) !ListCidrLocationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCidrLocationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCidrLocationsOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CidrLocations")) {
                    result.cidr_locations = try serde.deserializeLocationSummaries(allocator, &reader, "member");
