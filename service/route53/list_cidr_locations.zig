...
pub const ListCidrLocationsInput = struct {
...
pub const ListCidrLocationsOutput = struct {
    /// A complex type that contains information about the list of CIDR locations.
    cidr_locations: ?[]const LocationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCidrLocationsInput, options: Options) !ListCidrLocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCidrLocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.collection_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCidrLocationsOutput {
    var result: ListCidrLocationsOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CidrLocations")) {
                    result.cidr_locations = try serde.deserializeLocationSummaries(&reader, alloc, "member");
