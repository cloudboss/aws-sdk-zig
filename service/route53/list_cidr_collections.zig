...
pub const ListCidrCollectionsInput = struct {
...
pub const ListCidrCollectionsOutput = struct {
    /// A complex type with information about the CIDR collection.
    cidr_collections: ?[]const CollectionSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCidrCollectionsInput, options: CallOptions) !ListCidrCollectionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCidrCollectionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCidrCollectionsOutput {
    var result: ListCidrCollectionsOutput = .{};
    _ = status;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CidrCollections")) {
                    result.cidr_collections = try serde.deserializeCollectionSummaries(allocator, &reader, "member");
