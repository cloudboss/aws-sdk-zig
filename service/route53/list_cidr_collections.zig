...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCidrCollectionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCidrCollectionsOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CidrCollections")) {
                    result.cidr_collections = try serde.deserializeCollectionSummaries(&reader, alloc, "member");
