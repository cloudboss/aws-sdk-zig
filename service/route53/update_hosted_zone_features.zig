...
pub const UpdateHostedZoneFeaturesInput = struct {
    ...
    enable_accelerated_recovery: ?bool = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateHostedZoneFeaturesInput, options: Options) !UpdateHostedZoneFeaturesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateHostedZoneFeaturesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<UpdateHostedZoneFeaturesRequest>");
    if (input.enable_accelerated_recovery) |v| {
        try body_buf.appendSlice(alloc, "<EnableAcceleratedRecovery>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateHostedZoneFeaturesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateHostedZoneFeaturesOutput = .{};
