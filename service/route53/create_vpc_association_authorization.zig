...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVPCAssociationAuthorizationInput, options: Options) !CreateVPCAssociationAuthorizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVPCAssociationAuthorizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/authorizevpcassociation");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<CreateVPCAssociationAuthorizationRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVPCAssociationAuthorizationOutput {
