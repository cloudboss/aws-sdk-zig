...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHostedZoneInput, options: Options) !CreateHostedZoneOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHostedZoneInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<CreateHostedZoneRequest>");
    ...
    if (input.delegation_set_id) |v| {
        try body_buf.appendSlice(alloc, "<DelegationSetId>");
        try aws.xml.appendXmlEscaped(alloc, &body_buf, v);
        try body_buf.appendSlice(alloc, "</DelegationSetId>");
    }
    if (input.hosted_zone_config) |v| {
        try body_buf.appendSlice(alloc, "<HostedZoneConfig>");
        try serde.serializeHostedZoneConfig(alloc, &body_buf, v);
        try body_buf.appendSlice(alloc, "</HostedZoneConfig>");
    }
    ...
    if (input.vpc) |v| {
        ...
        try serde.serializeVPC(alloc, &body_buf, v);
        try body_buf.appendSlice(alloc, "</VPC>");
    }
    try body_buf.appendSlice(alloc, "</CreateHostedZoneRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHostedZoneOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ChangeInfo")) {
                    ...
                    result.delegation_set = try serde.deserializeDelegationSet(&reader, alloc);
                } else if (std.mem.eql(u8, e.local, "HostedZone")) {
                    result.hosted_zone = try serde.deserializeHostedZone(&reader, alloc);
