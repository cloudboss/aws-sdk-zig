...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ChangeCidrCollectionInput, options: Options) !ChangeCidrCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ChangeCidrCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2013-04-01/cidrcollection/");
    try path_buf.appendSlice(alloc, input.id);
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<ChangeCidrCollectionRequest>");
    try body_buf.appendSlice(alloc, "<Changes>");
    try serde.serializeCidrCollectionChanges(alloc, &body_buf, input.changes, "member");
    try body_buf.appendSlice(alloc, "</Changes>");
    if (input.collection_version) |v| {
        try body_buf.appendSlice(alloc, "<CollectionVersion>");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ChangeCidrCollectionOutput {
    var result: ChangeCidrCollectionOutput = .{};
    _ = status;
