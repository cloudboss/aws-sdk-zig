...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetReusableDelegationSetLimitInput, options: Options) !GetReusableDelegationSetLimitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReusableDelegationSetLimitInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReusableDelegationSetLimitOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Count")) {
                    ...
                    result.limit = try serde.deserializeReusableDelegationSetLimit(&reader, alloc);
                } else {
