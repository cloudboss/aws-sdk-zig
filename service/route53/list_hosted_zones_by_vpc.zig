...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHostedZonesByVPCInput, options: Options) !ListHostedZonesByVPCOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHostedZonesByVPCInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "vpcid=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.vpc_id);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "vpcregion=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.vpc_region));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHostedZonesByVPCOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HostedZoneSummaries")) {
                    result.hosted_zone_summaries = try serde.deserializeHostedZoneSummaries(&reader, alloc, "HostedZoneSummary");
