...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHostedZonesInput, options: Options) !ListHostedZonesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHostedZonesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.delegation_set_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "delegationsetid=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.hosted_zone_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "hostedzonetype=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHostedZonesOutput {
