...
pub const UpdateTrafficPolicyCommentInput = struct {
...
pub const UpdateTrafficPolicyCommentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTrafficPolicyCommentInput, options: Options) !UpdateTrafficPolicyCommentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTrafficPolicyCommentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2013-04-01/trafficpolicy/");
    try path_buf.appendSlice(alloc, input.id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.version);
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<UpdateTrafficPolicyCommentRequest>");
    try body_buf.appendSlice(alloc, "<Comment>");
    try aws.xml.appendXmlEscaped(alloc, &body_buf, input.comment);
    try body_buf.appendSlice(alloc, "</Comment>");
    try body_buf.appendSlice(alloc, "</UpdateTrafficPolicyCommentRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTrafficPolicyCommentOutput {
    var result: UpdateTrafficPolicyCommentOutput = .{};
    _ = status;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TrafficPolicy")) {
                    result.traffic_policy = try serde.deserializeTrafficPolicy(&reader, alloc);
                } else {
