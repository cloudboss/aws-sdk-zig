...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PostTextInput, options: CallOptions) !PostTextOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PostTextInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.input_text), input.input_text, allocator, &body_buf);
    has_prev = true;
