...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPricingPlansInput, options: Options) !ListPricingPlansOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPricingPlansInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPricingPlansOutput {
    var result: ListPricingPlansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPricingPlansOutput, body, alloc);
    }
    _ = status;
    _ = headers;
