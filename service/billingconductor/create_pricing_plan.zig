...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePricingPlanInput, options: Options) !CreatePricingPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePricingPlanInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePricingPlanOutput {
    var result: CreatePricingPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePricingPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
