...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPricingRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPricingRulesOutput {
    var result: ListPricingRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPricingRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
