...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomLineItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.billing_group_arn), input.billing_group_arn, alloc, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.charge_details), input.charge_details, alloc, &body_buf);
    has_prev = true;
    if (input.computation_rule) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ComputationRule\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.presentation_details) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PresentationDetails\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomLineItemOutput {
    var result: CreateCustomLineItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCustomLineItemOutput, body, alloc);
    }
    _ = status;
    _ = headers;
