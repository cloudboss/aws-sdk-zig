...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBillingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AccountGrouping\":");
    try aws.json.writeValue(@TypeOf(input.account_grouping), input.account_grouping, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ComputationPreference\":");
    try aws.json.writeValue(@TypeOf(input.computation_preference), input.computation_preference, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.primary_account_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PrimaryAccountId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBillingGroupOutput {
    var result: CreateBillingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBillingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
