...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePricingRuleInput, options: CallOptions) !UpdatePricingRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePricingRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePricingRuleOutput {
    var result: UpdatePricingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePricingRuleOutput, body, allocator);
    }
    _ = status;
    _ = headers;
