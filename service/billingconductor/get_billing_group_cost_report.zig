...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBillingGroupCostReportInput, options: Options) !GetBillingGroupCostReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBillingGroupCostReportInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBillingGroupCostReportOutput {
    var result: GetBillingGroupCostReportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBillingGroupCostReportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
