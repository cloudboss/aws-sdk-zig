...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPricingPlansAssociatedWithPricingRuleInput, options: Options) !ListPricingPlansAssociatedWithPricingRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPricingPlansAssociatedWithPricingRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PricingRuleArn\":");
    try aws.json.writeValue(@TypeOf(input.pricing_rule_arn), input.pricing_rule_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPricingPlansAssociatedWithPricingRuleOutput {
    var result: ListPricingPlansAssociatedWithPricingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPricingPlansAssociatedWithPricingRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
