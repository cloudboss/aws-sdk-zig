...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPricingRulesAssociatedToPricingPlanInput, options: CallOptions) !ListPricingRulesAssociatedToPricingPlanOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPricingRulesAssociatedToPricingPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"PricingPlanArn\":");
    try aws.json.writeValue(@TypeOf(input.pricing_plan_arn), input.pricing_plan_arn, allocator, &body_buf);
    has_prev = true;
