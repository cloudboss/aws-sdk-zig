...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCustomLineItemInput, options: Options) !DeleteCustomLineItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCustomLineItemInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCustomLineItemOutput {
    var result: DeleteCustomLineItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteCustomLineItemOutput, body, alloc);
    }
    _ = status;
    _ = headers;
