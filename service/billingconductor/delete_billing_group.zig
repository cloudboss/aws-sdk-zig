...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBillingGroupInput, options: Options) !DeleteBillingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBillingGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBillingGroupOutput {
    var result: DeleteBillingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBillingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
