...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePricingRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.billing_entity) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"BillingEntity\":");
    ...
    if (input.usage_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UsageType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePricingRuleOutput {
    var result: CreatePricingRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePricingRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
