...
pub const DeleteAccountPolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAccountPolicyInput, options: CallOptions) !DeleteAccountPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAccountPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DeleteAccountPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAccountPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
