...
pub const DeleteQueryDefinitionInput = struct {
...
pub const DeleteQueryDefinitionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteQueryDefinitionInput, options: CallOptions) !DeleteQueryDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteQueryDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DeleteQueryDefinition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteQueryDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteQueryDefinitionOutput, body, allocator);
}
