...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDeliveryDestinationPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.PutDeliveryDestinationPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDeliveryDestinationPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutDeliveryDestinationPolicyOutput, body, alloc);
}
