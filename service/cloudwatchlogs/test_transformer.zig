...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestTransformerInput, options: Options) !TestTransformerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestTransformerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.TestTransformer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestTransformerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TestTransformerOutput, body, alloc);
}
