...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutLogEventsInput, options: Options) !PutLogEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutLogEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.PutLogEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutLogEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutLogEventsOutput, body, alloc);
}
