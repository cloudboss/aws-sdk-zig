...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateSourceToS3TableIntegrationInput, options: Options) !AssociateSourceToS3TableIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateSourceToS3TableIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.AssociateSourceToS3TableIntegration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateSourceToS3TableIntegrationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateSourceToS3TableIntegrationOutput, body, alloc);
}
