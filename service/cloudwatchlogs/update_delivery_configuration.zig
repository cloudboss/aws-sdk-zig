...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDeliveryConfigurationInput, options: Options) !UpdateDeliveryConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDeliveryConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.UpdateDeliveryConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDeliveryConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
