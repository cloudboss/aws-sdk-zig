...
pub const UntagLogGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UntagLogGroupInput, options: CallOptions) !UntagLogGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UntagLogGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.UntagLogGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UntagLogGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
