...
pub const GetDeliveryDestinationInput = struct {
...
pub const GetDeliveryDestinationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeliveryDestinationInput, options: CallOptions) !GetDeliveryDestinationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDeliveryDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.GetDeliveryDestination");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDeliveryDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeliveryDestinationOutput, body, allocator);
}
