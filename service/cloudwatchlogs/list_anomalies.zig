...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnomaliesInput, options: Options) !ListAnomaliesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAnomaliesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.ListAnomalies");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAnomaliesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAnomaliesOutput, body, alloc);
}
