...
fn serializeRequest(alloc: std.mem.Allocator, input: PutSubscriptionFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.PutSubscriptionFilter");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutSubscriptionFilterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
