...
pub const DescribeDeliveriesInput = struct {
...
pub const DescribeDeliveriesOutput = struct {
    /// An array of structures. Each structure contains information about one
    /// delivery in the
    /// account.
    deliveries: ?[]const Delivery = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDeliveriesInput, options: CallOptions) !DescribeDeliveriesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDeliveriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DescribeDeliveries");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDeliveriesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDeliveriesOutput, body, allocator);
}
