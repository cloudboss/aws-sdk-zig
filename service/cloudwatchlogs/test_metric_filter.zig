...
fn serializeRequest(alloc: std.mem.Allocator, input: TestMetricFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.TestMetricFilter");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestMetricFilterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TestMetricFilterOutput, body, alloc);
}
