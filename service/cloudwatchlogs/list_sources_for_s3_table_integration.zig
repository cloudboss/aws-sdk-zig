...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSourcesForS3TableIntegrationInput, options: Options) !ListSourcesForS3TableIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSourcesForS3TableIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.ListSourcesForS3TableIntegration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSourcesForS3TableIntegrationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSourcesForS3TableIntegrationOutput, body, alloc);
}
