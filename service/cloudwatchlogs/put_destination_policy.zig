...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDestinationPolicyInput, options: Options) !PutDestinationPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDestinationPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.PutDestinationPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDestinationPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
