...
pub const DeleteMetricFilterInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMetricFilterInput, options: CallOptions) !DeleteMetricFilterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMetricFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DeleteMetricFilter");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMetricFilterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
