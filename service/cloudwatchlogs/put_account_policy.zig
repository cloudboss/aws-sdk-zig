...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccountPolicyInput, options: CallOptions) !PutAccountPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutAccountPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.PutAccountPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutAccountPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutAccountPolicyOutput, body, allocator);
}
