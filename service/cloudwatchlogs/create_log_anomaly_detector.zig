...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLogAnomalyDetectorInput, options: Options) !CreateLogAnomalyDetectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLogAnomalyDetectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.CreateLogAnomalyDetector");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLogAnomalyDetectorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLogAnomalyDetectorOutput, body, alloc);
}
