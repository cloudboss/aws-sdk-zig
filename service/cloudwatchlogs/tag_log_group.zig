...
pub const TagLogGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TagLogGroupInput, options: CallOptions) !TagLogGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TagLogGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.TagLogGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TagLogGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
