...
pub const PutLogGroupDeletionProtectionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutLogGroupDeletionProtectionInput, options: CallOptions) !PutLogGroupDeletionProtectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutLogGroupDeletionProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.PutLogGroupDeletionProtection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutLogGroupDeletionProtectionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
