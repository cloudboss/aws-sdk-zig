...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateScheduledQueryInput, options: Options) !UpdateScheduledQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateScheduledQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.UpdateScheduledQuery");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateScheduledQueryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateScheduledQueryOutput, body, alloc);
}
