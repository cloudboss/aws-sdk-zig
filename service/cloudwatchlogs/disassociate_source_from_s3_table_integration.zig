...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateSourceFromS3TableIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.DisassociateSourceFromS3TableIntegration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateSourceFromS3TableIntegrationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateSourceFromS3TableIntegrationOutput, body, alloc);
}
