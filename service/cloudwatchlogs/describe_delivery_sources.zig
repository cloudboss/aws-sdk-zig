...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDeliverySourcesInput, options: Options) !DescribeDeliverySourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDeliverySourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.DescribeDeliverySources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDeliverySourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDeliverySourcesOutput, body, alloc);
}
