...
pub const DeleteLogGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLogGroupInput, options: CallOptions) !DeleteLogGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteLogGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DeleteLogGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteLogGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
