...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLogEventsInput, options: Options) !GetLogEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLogEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.GetLogEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLogEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLogEventsOutput, body, alloc);
}
