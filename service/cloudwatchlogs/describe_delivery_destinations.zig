...
pub const DescribeDeliveryDestinationsInput = struct {
    /// Optionally specify the maximum number of delivery destinations to return in
    /// the
    /// response.
    limit: ?i32 = null,
...
pub const DescribeDeliveryDestinationsOutput = struct {
    /// An array of structures. Each structure contains information about one
    /// delivery destination
    /// in the account.
    delivery_destinations: ?[]const DeliveryDestination = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDeliveryDestinationsInput, options: CallOptions) !DescribeDeliveryDestinationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDeliveryDestinationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DescribeDeliveryDestinations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDeliveryDestinationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDeliveryDestinationsOutput, body, allocator);
}
