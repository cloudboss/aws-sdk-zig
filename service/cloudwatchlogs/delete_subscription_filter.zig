...
pub const DeleteSubscriptionFilterInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSubscriptionFilterInput, options: CallOptions) !DeleteSubscriptionFilterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSubscriptionFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DeleteSubscriptionFilter");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSubscriptionFilterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
