...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLogGroupsInput, options: Options) !ListLogGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLogGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.ListLogGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLogGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLogGroupsOutput, body, alloc);
}
