...
pub const GetDeliverySourceInput = struct {
...
pub const GetDeliverySourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeliverySourceInput, options: CallOptions) !GetDeliverySourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDeliverySourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.GetDeliverySource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDeliverySourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeliverySourceOutput, body, allocator);
}
