...
pub const DescribeIndexPoliciesInput = struct {
...
pub const DescribeIndexPoliciesOutput = struct {
    /// An array containing the field index policies.
    index_policies: ?[]const IndexPolicy = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIndexPoliciesInput, options: CallOptions) !DescribeIndexPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeIndexPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DescribeIndexPolicies");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeIndexPoliciesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeIndexPoliciesOutput, body, allocator);
}
