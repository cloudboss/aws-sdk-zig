...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: FilterLogEventsInput, options: Options) !FilterLogEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: FilterLogEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.FilterLogEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !FilterLogEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(FilterLogEventsOutput, body, alloc);
}
