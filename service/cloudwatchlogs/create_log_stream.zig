...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLogStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.CreateLogStream");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLogStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
