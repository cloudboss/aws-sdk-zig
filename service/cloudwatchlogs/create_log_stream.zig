...
pub const CreateLogStreamInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLogStreamInput, options: CallOptions) !CreateLogStreamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLogStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.CreateLogStream");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLogStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
