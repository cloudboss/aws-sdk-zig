...
pub const DescribeDestinationsInput = struct {
    /// The prefix to match. If you don't specify a value, no prefix filter is
    /// applied.
    destination_name_prefix: ?[]const u8 = null,
...
pub const DescribeDestinationsOutput = struct {
    /// The destinations.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDestinationsInput, options: CallOptions) !DescribeDestinationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDestinationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DescribeDestinations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDestinationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDestinationsOutput, body, allocator);
}
