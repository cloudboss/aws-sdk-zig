...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLogRecordInput, options: Options) !GetLogRecordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLogRecordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.GetLogRecord");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLogRecordOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLogRecordOutput, body, alloc);
}
