...
pub const DeleteDeliverySourceInput = struct {
...
pub const DeleteDeliverySourceOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDeliverySourceInput, options: CallOptions) !DeleteDeliverySourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDeliverySourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DeleteDeliverySource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDeliverySourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
