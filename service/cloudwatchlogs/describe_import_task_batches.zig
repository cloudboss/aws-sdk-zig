...
const ImportBatch = @import("import_batch.zig").ImportBatch;

pub const DescribeImportTaskBatchesInput = struct {
    ...
    batch_import_status: ?[]const ImportStatus = null,
    ...
    pub const json_field_names = .{
        .batch_import_status = "batchImportStatus",
        .import_id = "importId",
        .limit = "limit",
...
pub const DescribeImportTaskBatchesOutput = struct {
    /// The list of import batches that match the request filters.
    import_batches: ?[]const ImportBatch = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImportTaskBatchesInput, options: CallOptions) !DescribeImportTaskBatchesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeImportTaskBatchesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.DescribeImportTaskBatches");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeImportTaskBatchesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeImportTaskBatchesOutput, body, allocator);
}
