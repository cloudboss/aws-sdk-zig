...
pub const GetLogFieldsInput = struct {
...
pub const GetLogFieldsOutput = struct {
    /// The list of log fields for the specified data source, including field names
    /// and their data
    /// types.
    log_fields: ?[]const LogFieldsListItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLogFieldsInput, options: CallOptions) !GetLogFieldsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLogFieldsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.GetLogFields");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLogFieldsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLogFieldsOutput, body, allocator);
}
