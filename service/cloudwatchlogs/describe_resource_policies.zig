...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeResourcePoliciesInput, options: Options) !DescribeResourcePoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeResourcePoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.DescribeResourcePolicies");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeResourcePoliciesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeResourcePoliciesOutput, body, alloc);
}
