...
pub const GetLookupTableInput = struct {
...
pub const GetLookupTableOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLookupTableInput, options: CallOptions) !GetLookupTableOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLookupTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.GetLookupTable");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLookupTableOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLookupTableOutput, body, allocator);
}
