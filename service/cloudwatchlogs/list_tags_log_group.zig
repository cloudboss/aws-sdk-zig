...
pub const ListTagsLogGroupInput = struct {
...
pub const ListTagsLogGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTagsLogGroupInput, options: CallOptions) !ListTagsLogGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTagsLogGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.ListTagsLogGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTagsLogGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTagsLogGroupOutput, body, allocator);
}
