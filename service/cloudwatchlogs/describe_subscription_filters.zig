...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSubscriptionFiltersInput, options: Options) !DescribeSubscriptionFiltersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSubscriptionFiltersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.DescribeSubscriptionFilters");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSubscriptionFiltersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSubscriptionFiltersOutput, body, alloc);
}
