...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTransformerInput, options: Options) !DeleteTransformerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTransformerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Logs_20140328.DeleteTransformer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTransformerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
