...
pub const GetDeliveryInput = struct {
...
pub const GetDeliveryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeliveryInput, options: CallOptions) !GetDeliveryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDeliveryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Logs_20140328.GetDelivery");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDeliveryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDeliveryOutput, body, allocator);
}
