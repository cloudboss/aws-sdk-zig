...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApplicationVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.semantic_version);
    ...
    if (input.source_code_archive_url) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"SourceCodeArchiveUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_code_url) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceCodeUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateApplicationVersionOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateApplicationVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
