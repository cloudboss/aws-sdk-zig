...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.author) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"Author\":");
    ...
    if (input.home_page_url) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"HomePageUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.labels) |v| {
    ...
    if (input.readme_body) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ReadmeBody\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.readme_url) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ReadmeUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
