...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTelemetryRuleForOrganizationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTelemetryRuleForOrganizationOutput {
    var result: CreateTelemetryRuleForOrganizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTelemetryRuleForOrganizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
