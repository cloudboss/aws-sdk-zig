...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetS3TableIntegrationInput, options: Options) !GetS3TableIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetS3TableIntegrationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetS3TableIntegrationOutput {
    var result: GetS3TableIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetS3TableIntegrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
