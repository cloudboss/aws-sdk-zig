...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTelemetryRulesForOrganizationInput, options: Options) !ListTelemetryRulesForOrganizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTelemetryRulesForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.source_account_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceAccountIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_organization_unit_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceOrganizationUnitIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTelemetryRulesForOrganizationOutput {
    var result: ListTelemetryRulesForOrganizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTelemetryRulesForOrganizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
