...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTelemetryRuleInput, options: CallOptions) !UpdateTelemetryRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTelemetryRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTelemetryRuleOutput {
    var result: UpdateTelemetryRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTelemetryRuleOutput, body, allocator);
    }
    _ = status;
    _ = headers;
