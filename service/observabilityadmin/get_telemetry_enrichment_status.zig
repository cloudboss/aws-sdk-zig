...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTelemetryEnrichmentStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTelemetryEnrichmentStatusOutput {
    var result: GetTelemetryEnrichmentStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTelemetryEnrichmentStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
