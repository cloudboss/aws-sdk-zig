...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTelemetryRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTelemetryRulesOutput {
    var result: ListTelemetryRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTelemetryRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
