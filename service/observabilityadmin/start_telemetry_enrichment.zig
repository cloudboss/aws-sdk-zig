...
const StartTelemetryEnrichmentInput = struct {};

pub const StartTelemetryEnrichmentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTelemetryEnrichmentInput, options: Options) !StartTelemetryEnrichmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartTelemetryEnrichmentInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    const path = "/StartTelemetryEnrichment";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartTelemetryEnrichmentOutput {
    var result: StartTelemetryEnrichmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartTelemetryEnrichmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
