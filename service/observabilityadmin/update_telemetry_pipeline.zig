...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTelemetryPipelineInput, options: Options) !UpdateTelemetryPipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTelemetryPipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PipelineIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.pipeline_identifier), input.pipeline_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTelemetryPipelineOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
