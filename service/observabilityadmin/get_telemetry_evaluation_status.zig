...
pub const GetTelemetryEvaluationStatusInput = struct {};

pub const GetTelemetryEvaluationStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTelemetryEvaluationStatusInput, options: CallOptions) !GetTelemetryEvaluationStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTelemetryEvaluationStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    const path = "/GetTelemetryEvaluationStatus";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTelemetryEvaluationStatusOutput {
    var result: GetTelemetryEvaluationStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTelemetryEvaluationStatusOutput, body, allocator);
    }
    _ = status;
    _ = headers;
