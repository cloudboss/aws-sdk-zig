...
/// * **Processors (optional):** Transform, parse, and enrich log data as it
///   flows through the pipeline. Processors are applied sequentially in the
///   order they are defined.
/// * **Sink:** Defines the destination where processed log data is sent. Each
///   pipeline must have exactly one sink.
/// * **Extensions (optional):** Provide additional functionality such as Amazon
///   Web Services Secrets Manager integration for credential management.
///
/// For more details on each configuration section see [CloudWatch pipelines
/// User
