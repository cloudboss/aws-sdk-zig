...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTelemetryRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTelemetryRuleOutput {
    var result: GetTelemetryRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTelemetryRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
