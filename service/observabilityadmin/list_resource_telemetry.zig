...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceTelemetryInput, options: Options) !ListResourceTelemetryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceTelemetryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceTelemetryOutput {
    var result: ListResourceTelemetryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourceTelemetryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
