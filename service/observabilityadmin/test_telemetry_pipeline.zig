...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestTelemetryPipelineInput, options: Options) !TestTelemetryPipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestTelemetryPipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/TestTelemetryPipeline";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestTelemetryPipelineOutput {
    var result: TestTelemetryPipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TestTelemetryPipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
