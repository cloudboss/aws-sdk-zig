...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTelemetryPipelineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTelemetryPipelineOutput {
    var result: CreateTelemetryPipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTelemetryPipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
