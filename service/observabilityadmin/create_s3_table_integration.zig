...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateS3TableIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.encryption), input.encryption, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateS3TableIntegrationOutput {
    var result: CreateS3TableIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateS3TableIntegrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
