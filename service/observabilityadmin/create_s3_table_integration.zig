...
pub const CreateS3TableIntegrationInput = struct {
...
pub const CreateS3TableIntegrationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateS3TableIntegrationInput, options: CallOptions) !CreateS3TableIntegrationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateS3TableIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.encryption), input.encryption, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateS3TableIntegrationOutput {
    var result: CreateS3TableIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateS3TableIntegrationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
