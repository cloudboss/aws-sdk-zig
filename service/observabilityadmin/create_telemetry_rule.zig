...
pub const CreateTelemetryRuleInput = struct {
...
pub const CreateTelemetryRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTelemetryRuleInput, options: Options) !CreateTelemetryRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTelemetryRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/CreateTelemetryRule";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTelemetryRuleOutput {
    var result: CreateTelemetryRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTelemetryRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
