...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTelemetryEvaluationStatusForOrganizationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTelemetryEvaluationStatusForOrganizationOutput {
    var result: GetTelemetryEvaluationStatusForOrganizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTelemetryEvaluationStatusForOrganizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
