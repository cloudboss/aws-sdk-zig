...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTelemetryRuleForOrganizationInput, options: Options) !GetTelemetryRuleForOrganizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTelemetryRuleForOrganizationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTelemetryRuleForOrganizationOutput {
    var result: GetTelemetryRuleForOrganizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTelemetryRuleForOrganizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
