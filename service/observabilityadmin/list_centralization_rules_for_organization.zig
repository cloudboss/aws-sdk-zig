...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCentralizationRulesForOrganizationInput, options: Options) !ListCentralizationRulesForOrganizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCentralizationRulesForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.all_regions) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AllRegions\":");
    ...
    if (input.rule_name_prefix) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RuleNamePrefix\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCentralizationRulesForOrganizationOutput {
    var result: ListCentralizationRulesForOrganizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCentralizationRulesForOrganizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
