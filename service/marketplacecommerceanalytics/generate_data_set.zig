...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateDataSetInput, options: CallOptions) !GenerateDataSetOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "marketplacecommerceanalytics", client.config.http_client.clock_skew_offset);
...
fn serializeRequest(allocator: std.mem.Allocator, input: GenerateDataSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "MarketplaceCommerceAnalytics20150701.GenerateDataSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GenerateDataSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GenerateDataSetOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "MarketplaceCommerceAnalyticsException")) {
        const parsed_error: ?errors.MarketplaceCommerceAnalyticsException = aws.json.parseJsonObject(errors.MarketplaceCommerceAnalyticsException, body, arena_alloc) catch |err| switch (err) {
        ...
        if (parsed_error) |parsed| {
            ...
            return .{ .arena = arena, .kind = .{ .marketplace_commerce_analytics_exception = typed_error } };
        }
    }
