...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSupportDataExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "MarketplaceCommerceAnalytics20150701.StartSupportDataExport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSupportDataExportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartSupportDataExportOutput, body, alloc);
}
