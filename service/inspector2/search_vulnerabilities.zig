...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchVulnerabilitiesInput, options: Options) !SearchVulnerabilitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchVulnerabilitiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchVulnerabilitiesOutput {
    var result: SearchVulnerabilitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchVulnerabilitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
