...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCodeSecurityIntegrationInput, options: Options) !GetCodeSecurityIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCodeSecurityIntegrationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCodeSecurityIntegrationOutput {
    var result: GetCodeSecurityIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCodeSecurityIntegrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
