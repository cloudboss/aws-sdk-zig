...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCisScansInput, options: Options) !ListCisScansOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCisScansInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCisScansOutput {
    var result: ListCisScansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCisScansOutput, body, alloc);
    }
    _ = status;
    _ = headers;
