...
pub const DisableInput = struct {
    /// An array of account IDs you want to disable Amazon Inspector scans for.
    account_ids: ?[]const []const u8 = null,

    /// The resource scan types you want to disable.
    resource_types: ?[]const ResourceScanType = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableInput, options: Options) !DisableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableOutput {
    var result: DisableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisableOutput, body, alloc);
    }
    _ = status;
    _ = headers;
