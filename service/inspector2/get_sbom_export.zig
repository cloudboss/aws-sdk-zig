...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSbomExportInput, options: Options) !GetSbomExportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSbomExportInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSbomExportOutput {
    var result: GetSbomExportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSbomExportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
