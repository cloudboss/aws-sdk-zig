...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetFindingDetailsInput, options: Options) !BatchGetFindingDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetFindingDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"findingArns\":");
    try aws.json.writeValue(@TypeOf(input.finding_arns), input.finding_arns, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetFindingDetailsOutput {
    var result: BatchGetFindingDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetFindingDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
