...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCodeSecurityScanConfigurationInput, options: Options) !CreateCodeSecurityScanConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCodeSecurityScanConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.scope_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"scopeSettings\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCodeSecurityScanConfigurationOutput {
    var result: CreateCodeSecurityScanConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCodeSecurityScanConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
