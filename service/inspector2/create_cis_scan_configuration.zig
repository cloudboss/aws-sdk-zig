...
const CreateCisTargets = @import("create_cis_targets.zig").CreateCisTargets;

pub const CreateCisScanConfigurationInput = struct {
...
pub const CreateCisScanConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCisScanConfigurationInput, options: CallOptions) !CreateCisScanConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateCisScanConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/cis/scan-configuration/create";
    ...
    try aws.json.writeValue(@TypeOf(input.security_level), input.security_level, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateCisScanConfigurationOutput {
    var result: CreateCisScanConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCisScanConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
