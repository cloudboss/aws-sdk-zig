...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCodeSecurityIntegrationInput, options: Options) !CreateCodeSecurityIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCodeSecurityIntegrationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCodeSecurityIntegrationOutput {
    var result: CreateCodeSecurityIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCodeSecurityIntegrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
