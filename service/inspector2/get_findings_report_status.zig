...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingsReportStatusInput, options: Options) !GetFindingsReportStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingsReportStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingsReportStatusOutput {
    var result: GetFindingsReportStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingsReportStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
