...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCisScanConfigurationInput, options: Options) !UpdateCisScanConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCisScanConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.scan_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"scanName\":");
    ...
    if (input.security_level) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"securityLevel\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.targets) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCisScanConfigurationOutput {
    var result: UpdateCisScanConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCisScanConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
