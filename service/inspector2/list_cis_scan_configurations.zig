...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCisScanConfigurationsInput, options: Options) !ListCisScanConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCisScanConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCisScanConfigurationsOutput {
    var result: ListCisScanConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCisScanConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
