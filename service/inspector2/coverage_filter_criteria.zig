...
pub const CoverageFilterCriteria = struct {
    ...
    /// for.
    account_id: ?[]const CoverageStringFilter,

    /// Filter criteria for code repositories based on project name.
    code_repository_project_name: ?[]const CoverageStringFilter,

    /// Filter criteria for code repositories based on provider type (such as
    /// GitHub, GitLab,
    /// etc.).
    code_repository_provider_type: ?[]const CoverageStringFilter,

    /// Filter criteria for code repositories based on visibility setting (public or
    /// private).
    code_repository_provider_type_visibility: ?[]const CoverageStringFilter,

    /// The Amazon EC2 instance tags to filter on.
    ec_2_instance_tags: ?[]const CoverageMapFilter,

    /// The number of Amazon ECR images in use.
    ecr_image_in_use_count: ?[]const CoverageNumberFilter,

    /// The Amazon ECR image that was last in use.
    ecr_image_last_in_use_at: ?[]const CoverageDateFilter,

    /// The Amazon ECR image tags to filter on.
    ecr_image_tags: ?[]const CoverageStringFilter,

    /// The Amazon ECR repository name to filter on.
    ecr_repository_name: ?[]const CoverageStringFilter,

    /// The date an image was last pulled at.
    image_pulled_at: ?[]const CoverageDateFilter,

    /// Returns coverage statistics for Amazon Web Services Lambda functions
    /// filtered by function
    /// names.
    lambda_function_name: ?[]const CoverageStringFilter,

    /// Returns coverage statistics for Amazon Web Services Lambda functions
    /// filtered by
    /// runtime.
    lambda_function_runtime: ?[]const CoverageStringFilter,

    /// Returns coverage statistics for Amazon Web Services Lambda functions
    /// filtered by
    /// tag.
    lambda_function_tags: ?[]const CoverageMapFilter,

    /// Filters Amazon Web Services resources based on whether Amazon Inspector has
    /// checked them for vulnerabilities
    /// within the specified time range.
    last_scanned_at: ?[]const CoverageDateFilter,

    /// Filter criteria for code repositories based on the ID of the last scanned
    /// commit.
    last_scanned_commit_id: ?[]const CoverageStringFilter,

    /// An array of Amazon Web Services resource IDs to return coverage statistics
    /// for.
    resource_id: ?[]const CoverageStringFilter,
    ...
    scan_status_code: ?[]const CoverageStringFilter,

    /// The scan status reason to filter on.
    scan_status_reason: ?[]const CoverageStringFilter,

    /// An array of Amazon Inspector scan types to return coverage statistics for.
    scan_type: ?[]const CoverageStringFilter,
