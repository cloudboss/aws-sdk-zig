...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetCodeSnippetInput, options: Options) !BatchGetCodeSnippetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetCodeSnippetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetCodeSnippetOutput {
    var result: BatchGetCodeSnippetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetCodeSnippetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
