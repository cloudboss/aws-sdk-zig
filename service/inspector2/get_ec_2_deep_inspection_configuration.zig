...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEc2DeepInspectionConfigurationInput, options: Options) !GetEc2DeepInspectionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEc2DeepInspectionConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEc2DeepInspectionConfigurationOutput {
    var result: GetEc2DeepInspectionConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEc2DeepInspectionConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
