...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFindingAggregationsInput, options: Options) !ListFindingAggregationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFindingAggregationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.aggregation_request) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"aggregationRequest\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"aggregationType\":");
    try aws.json.writeValue(@TypeOf(input.aggregation_type), input.aggregation_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFindingAggregationsOutput {
    var result: ListFindingAggregationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFindingAggregationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
