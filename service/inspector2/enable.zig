...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableInput, options: Options) !EnableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableOutput {
    var result: EnableOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EnableOutput, body, alloc);
    }
    _ = status;
    _ = headers;
