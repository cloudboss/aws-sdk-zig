...
fn serializeRequest(alloc: std.mem.Allocator, input: GetClustersForImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.filter), input.filter, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetClustersForImageOutput {
    var result: GetClustersForImageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetClustersForImageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
