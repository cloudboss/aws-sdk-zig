...
pub const GetClustersForImageInput = struct {
...
pub const GetClustersForImageOutput = struct {
    /// A unit of work inside of a cluster, which can include metadata about the
    /// cluster.
    cluster: ?[]const ClusterInformation = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetClustersForImageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetClustersForImageOutput {
    var result: GetClustersForImageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetClustersForImageOutput, body, allocator);
    }
    _ = status;
    _ = headers;
