...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountPermissionsInput, options: Options) !ListAccountPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountPermissionsOutput {
    var result: ListAccountPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccountPermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
