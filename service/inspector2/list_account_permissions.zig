...
pub const ListAccountPermissionsInput = struct {
...
pub const ListAccountPermissionsOutput = struct {
    /// A token to use for paginating results that are returned in the response. Set
    /// the value
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountPermissionsInput, options: CallOptions) !ListAccountPermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAccountPermissionsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("inspector2", "Inspector2", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/accountpermissions/list";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAccountPermissionsOutput {
    var result: ListAccountPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccountPermissionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
