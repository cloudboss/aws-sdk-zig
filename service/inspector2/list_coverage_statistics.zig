...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCoverageStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCoverageStatisticsOutput {
    var result: ListCoverageStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCoverageStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
