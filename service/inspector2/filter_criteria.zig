...
pub const FilterCriteria = struct {
    ...
    code_vulnerability_detector_tags: ?[]const StringFilter,

    /// The file path to the file in a Lambda function that contains a code
    /// vulnerability used
    /// to filter findings.
    code_vulnerability_file_path: ?[]const StringFilter,

    /// Details of the component IDs used to filter findings.
    component_id: ?[]const StringFilter,

    /// Details of the component types used to filter findings.
    component_type: ?[]const StringFilter,

    /// Details of the Amazon EC2 instance image IDs used to filter findings.
    ec_2_instance_image_id: ?[]const StringFilter,

    /// Details of the Amazon EC2 instance subnet IDs used to filter findings.
    ec_2_instance_subnet_id: ?[]const StringFilter,

    /// Details of the Amazon EC2 instance VPC IDs used to filter findings.
    ec_2_instance_vpc_id: ?[]const StringFilter,

    /// Details of the Amazon ECR image architecture types used to filter findings.
    ecr_image_architecture: ?[]const StringFilter,

    /// Details of the Amazon ECR image hashes used to filter findings.
    ecr_image_hash: ?[]const StringFilter,

    /// Filter criteria indicating when details for an Amazon ECR image include when
    /// an
    /// Amazon ECR image is in use.
    ecr_image_in_use_count: ?[]const NumberFilter,

    /// Filter criteria indicating when an Amazon ECR image was last used in an
    /// Amazon ECS cluster task or Amazon EKS cluster pod.
    ecr_image_last_in_use_at: ?[]const DateFilter,

    /// Details on the Amazon ECR image push date and time used to filter findings.
    ecr_image_pushed_at: ?[]const DateFilter,

    /// Details on the Amazon ECR registry used to filter findings.
    ecr_image_registry: ?[]const StringFilter,

    /// Details on the name of the Amazon ECR repository used to filter findings.
    ecr_image_repository_name: ?[]const StringFilter,

    /// The tags attached to the Amazon ECR container image.
    ecr_image_tags: ?[]const StringFilter,

    /// The EPSS score used to filter findings.
    epss_score: ?[]const NumberFilter,

    /// Filters the list of Amazon Web Services Lambda findings by the availability
    /// of
    /// exploits.
    exploit_available: ?[]const StringFilter,

    /// Details on the finding ARNs used to filter findings.
    finding_arn: ?[]const StringFilter,

    /// Details on the finding status types used to filter findings.
    finding_status: ?[]const StringFilter,

    /// Details on the finding types used to filter findings.
    finding_type: ?[]const StringFilter,
    ...
    fix_available: ?[]const StringFilter,

    /// The Amazon Inspector score to filter on.
    inspector_score: ?[]const NumberFilter,

    /// Filters the list of Amazon Web Services Lambda functions by execution role.
    lambda_function_execution_role_arn: ?[]const StringFilter,
    ...
    lambda_function_last_modified_at: ?[]const DateFilter,
    ...
    port_range: ?[]const PortRangeFilter,

    /// Details on the related vulnerabilities used to filter findings.
    related_vulnerabilities: ?[]const StringFilter,
    ...
    vendor_severity: ?[]const StringFilter,

    /// Details on the vulnerability ID used to filter findings.
    vulnerability_id: ?[]const StringFilter,

    /// Details on the vulnerability type used to filter findings.
    vulnerability_source: ?[]const StringFilter,

    /// Details on the vulnerable packages used to filter findings.
    vulnerable_packages: ?[]const PackageFilter,
