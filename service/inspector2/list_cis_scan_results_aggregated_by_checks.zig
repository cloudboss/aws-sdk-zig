...
const CisCheckAggregation = @import("cis_check_aggregation.zig").CisCheckAggregation;

pub const ListCisScanResultsAggregatedByChecksInput = struct {
    /// The filter criteria.
    filter_criteria: ?CisScanResultsAggregatedByChecksFilterCriteria = null,
    ...
    /// The sort by order.
    sort_by: ?CisScanResultsAggregatedByChecksSortBy = null,
...
pub const ListCisScanResultsAggregatedByChecksOutput = struct {
    /// The check aggregations.
    check_aggregations: ?[]const CisCheckAggregation = null,
    ...
    pub const json_field_names = .{
        .check_aggregations = "checkAggregations",
        .next_token = "nextToken",
    };
};
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCisScanResultsAggregatedByChecksInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/cis/scan-result/check/list";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCisScanResultsAggregatedByChecksOutput {
    var result: ListCisScanResultsAggregatedByChecksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCisScanResultsAggregatedByChecksOutput, body, allocator);
    }
    _ = status;
    _ = headers;
