...
pub const ListCisScanResultsAggregatedByChecksInput = struct {
    ...
    /// The sort by order.
    sort_by: CisScanResultsAggregatedByChecksSortBy = "FAILED_COUNTS",
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCisScanResultsAggregatedByChecksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCisScanResultsAggregatedByChecksOutput {
    var result: ListCisScanResultsAggregatedByChecksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCisScanResultsAggregatedByChecksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
