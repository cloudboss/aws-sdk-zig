...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCisScanResultsAggregatedByTargetResourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCisScanResultsAggregatedByTargetResourceOutput {
    var result: ListCisScanResultsAggregatedByTargetResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCisScanResultsAggregatedByTargetResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
