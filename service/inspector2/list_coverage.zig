...
const CoveredResource = @import("covered_resource.zig").CoveredResource;
...
pub const ListCoverageOutput = struct {
    ...
    /// environment.
    covered_resources: ?[]const CoveredResource = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCoverageInput, options: Options) !ListCoverageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCoverageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCoverageOutput {
    var result: ListCoverageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCoverageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
