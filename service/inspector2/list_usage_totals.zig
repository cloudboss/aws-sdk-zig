...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUsageTotalsInput, options: Options) !ListUsageTotalsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUsageTotalsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUsageTotalsOutput {
    var result: ListUsageTotalsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUsageTotalsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
