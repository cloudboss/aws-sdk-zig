...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCodeSecurityIntegrationInput, options: Options) !UpdateCodeSecurityIntegrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCodeSecurityIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.details), input.details, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCodeSecurityIntegrationOutput {
    var result: UpdateCodeSecurityIntegrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCodeSecurityIntegrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
