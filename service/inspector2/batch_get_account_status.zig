...
pub const BatchGetAccountStatusInput = struct {
...
pub const BatchGetAccountStatusOutput = struct {
    /// An array of objects that provide details on the status of Amazon Inspector
    /// for each of the
    /// requested accounts.
    accounts: ?[]const AccountState = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetAccountStatusInput, options: Options) !BatchGetAccountStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetAccountStatusInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetAccountStatusOutput {
    var result: BatchGetAccountStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetAccountStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
