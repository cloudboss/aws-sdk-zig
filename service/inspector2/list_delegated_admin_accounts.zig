...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDelegatedAdminAccountsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDelegatedAdminAccountsOutput {
    var result: ListDelegatedAdminAccountsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDelegatedAdminAccountsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
