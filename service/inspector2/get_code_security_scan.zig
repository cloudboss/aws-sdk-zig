...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCodeSecurityScanInput, options: Options) !GetCodeSecurityScanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCodeSecurityScanInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"scanId\":");
    try aws.json.writeValue(@TypeOf(input.scan_id), input.scan_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCodeSecurityScanOutput {
    var result: GetCodeSecurityScanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCodeSecurityScanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
