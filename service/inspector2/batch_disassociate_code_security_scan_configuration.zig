...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDisassociateCodeSecurityScanConfigurationInput, options: Options) !BatchDisassociateCodeSecurityScanConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDisassociateCodeSecurityScanConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"disassociateConfigurationRequests\":");
    try aws.json.writeValue(@TypeOf(input.disassociate_configuration_requests), input.disassociate_configuration_requests, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDisassociateCodeSecurityScanConfigurationOutput {
    var result: BatchDisassociateCodeSecurityScanConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDisassociateCodeSecurityScanConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
