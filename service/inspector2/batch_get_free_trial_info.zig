...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetFreeTrialInfoInput, options: Options) !BatchGetFreeTrialInfoOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetFreeTrialInfoInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetFreeTrialInfoOutput {
    var result: BatchGetFreeTrialInfoOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetFreeTrialInfoOutput, body, alloc);
    }
    _ = status;
    _ = headers;
