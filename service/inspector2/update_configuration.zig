...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfigurationInput, options: Options) !UpdateConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ec_2_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ec2Configuration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.ecr_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ecrConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
