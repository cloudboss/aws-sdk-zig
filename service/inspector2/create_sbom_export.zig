...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSbomExportInput, options: Options) !CreateSbomExportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSbomExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"reportFormat\":");
    try aws.json.writeValue(@TypeOf(input.report_format), input.report_format, alloc, &body_buf);
    has_prev = true;
    if (input.resource_filter_criteria) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resourceFilterCriteria\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"s3Destination\":");
    try aws.json.writeValue(@TypeOf(input.s_3_destination), input.s_3_destination, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSbomExportOutput {
    var result: CreateSbomExportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSbomExportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
