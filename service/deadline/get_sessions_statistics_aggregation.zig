...
const SessionsStatisticsAggregationStatus = @import("sessions_statistics_aggregation_status.zig").SessionsStatisticsAggregationStatus;

pub const GetSessionsStatisticsAggregationInput = struct {
...
pub const GetSessionsStatisticsAggregationOutput = struct {
    ...
    statistics: ?[]const Statistics = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSessionsStatisticsAggregationInput, options: CallOptions) !GetSessionsStatisticsAggregationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSessionsStatisticsAggregationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "aggregationId=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.aggregation_id);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSessionsStatisticsAggregationOutput {
    var result: GetSessionsStatisticsAggregationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSessionsStatisticsAggregationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
