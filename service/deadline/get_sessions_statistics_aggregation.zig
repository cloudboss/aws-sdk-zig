...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSessionsStatisticsAggregationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "aggregationId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.aggregation_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSessionsStatisticsAggregationOutput {
    var result: GetSessionsStatisticsAggregationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSessionsStatisticsAggregationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
