...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateQueueLimitAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"limitId\":");
    try aws.json.writeValue(@TypeOf(input.limit_id), input.limit_id, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateQueueLimitAssociationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
