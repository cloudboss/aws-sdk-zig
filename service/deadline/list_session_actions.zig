...
const SessionActionSummary = @import("session_action_summary.zig").SessionActionSummary;

pub const ListSessionActionsInput = struct {
...
pub const ListSessionActionsOutput = struct {
    ...
    /// The session actions.
    session_actions: ?[]const SessionActionSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSessionActionsInput, options: Options) !ListSessionActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSessionActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/session-actions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSessionActionsOutput {
    var result: ListSessionActionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSessionActionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
