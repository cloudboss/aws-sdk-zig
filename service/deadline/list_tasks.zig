...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTasksInput, options: Options) !ListTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTasksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTasksOutput {
    var result: ListTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
