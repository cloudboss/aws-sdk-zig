...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStepInput, options: CallOptions) !GetStepOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetStepInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetStepOutput {
    var result: GetStepOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetStepOutput, body, allocator);
    }
    _ = status;
    _ = headers;
