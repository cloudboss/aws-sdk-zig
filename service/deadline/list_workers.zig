...
const WorkerSummary = @import("worker_summary.zig").WorkerSummary;

pub const ListWorkersInput = struct {
...
pub const ListWorkersOutput = struct {
    ...
    workers: ?[]const WorkerSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkersInput, options: CallOptions) !ListWorkersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListWorkersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListWorkersOutput {
    var result: ListWorkersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
