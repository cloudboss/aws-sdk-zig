...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkersOutput {
    var result: ListWorkersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
