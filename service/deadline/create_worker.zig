...
pub const CreateWorkerInput = struct {
...
pub const CreateWorkerOutput = struct {
    /// The worker ID.
    worker_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkerInput, options: Options) !CreateWorkerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkerInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.host_properties) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"hostProperties\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkerOutput {
    var result: CreateWorkerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWorkerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
