...
const LimitSummary = @import("limit_summary.zig").LimitSummary;

pub const ListLimitsInput = struct {
...
pub const ListLimitsOutput = struct {
    /// A list of limits that the farm contains.
    limits: ?[]const LimitSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLimitsInput, options: CallOptions) !ListLimitsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLimitsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLimitsOutput {
    var result: ListLimitsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLimitsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
