...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLimitsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLimitsOutput {
    var result: ListLimitsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLimitsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
