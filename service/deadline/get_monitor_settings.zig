...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMonitorSettingsInput, options: CallOptions) !GetMonitorSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMonitorSettingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMonitorSettingsOutput {
    var result: GetMonitorSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMonitorSettingsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
