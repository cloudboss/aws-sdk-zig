...
pub const DeleteQueueEnvironmentInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteQueueEnvironmentInput, options: Options) !DeleteQueueEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteQueueEnvironmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.queue_environment_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteQueueEnvironmentOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteQueueEnvironmentOutput = .{};
