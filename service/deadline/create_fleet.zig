...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.host_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"hostConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"maxWorkerCount\":");
    try aws.json.writeValue(@TypeOf(input.max_worker_count), input.max_worker_count, allocator, &body_buf);
    has_prev = true;
    if (input.min_worker_count) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"minWorkerCount\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateFleetOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFleetOutput, body, allocator);
    }
    _ = status;
    _ = headers;
