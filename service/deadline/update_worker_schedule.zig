...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkerScheduleInput, options: CallOptions) !UpdateWorkerScheduleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateWorkerScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.updated_session_actions) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"updatedSessionActions\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateWorkerScheduleOutput {
    var result: UpdateWorkerScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateWorkerScheduleOutput, body, allocator);
    }
    _ = status;
    _ = headers;
