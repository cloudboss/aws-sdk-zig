...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLicenseEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLicenseEndpointsOutput {
    var result: ListLicenseEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLicenseEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
