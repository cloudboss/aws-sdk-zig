...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSessionActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.session_action_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSessionActionOutput {
    var result: GetSessionActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSessionActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
