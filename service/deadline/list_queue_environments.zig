...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQueueEnvironmentsInput, options: CallOptions) !ListQueueEnvironmentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListQueueEnvironmentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListQueueEnvironmentsOutput {
    var result: ListQueueEnvironmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQueueEnvironmentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
