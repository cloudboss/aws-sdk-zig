...
pub const GetStorageProfileInput = struct {
...
pub const GetStorageProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStorageProfileInput, options: Options) !GetStorageProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStorageProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStorageProfileOutput {
    var result: GetStorageProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetStorageProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
