...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStorageProfileInput, options: CallOptions) !GetStorageProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetStorageProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetStorageProfileOutput {
    var result: GetStorageProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetStorageProfileOutput, body, allocator);
    }
    _ = status;
    _ = headers;
