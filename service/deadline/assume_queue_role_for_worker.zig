...
pub const AssumeQueueRoleForWorkerInput = struct {
...
pub const AssumeQueueRoleForWorkerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssumeQueueRoleForWorkerInput, options: Options) !AssumeQueueRoleForWorkerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssumeQueueRoleForWorkerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/queue-roles");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "queueId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.queue_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssumeQueueRoleForWorkerOutput {
    var result: AssumeQueueRoleForWorkerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssumeQueueRoleForWorkerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
