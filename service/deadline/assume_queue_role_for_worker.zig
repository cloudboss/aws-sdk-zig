...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssumeQueueRoleForWorkerInput, options: CallOptions) !AssumeQueueRoleForWorkerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssumeQueueRoleForWorkerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/queue-roles");
    ...
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.queue_id);
    query_has_prev = true;
