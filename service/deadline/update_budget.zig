...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBudgetInput, options: CallOptions) !UpdateBudgetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateBudgetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.actions_to_add) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"actionsToAdd\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.actions_to_remove) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"actionsToRemove\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.approximate_dollar_limit) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"approximateDollarLimit\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateBudgetOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
