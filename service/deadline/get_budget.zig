...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBudgetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBudgetOutput {
    var result: GetBudgetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBudgetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
