...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetJobEntityInput, options: Options) !BatchGetJobEntityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetJobEntityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batchGetJobEntity");
    ...
    try body_buf.appendSlice(alloc, "\"identifiers\":");
    try aws.json.writeValue(@TypeOf(input.identifiers), input.identifiers, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetJobEntityOutput {
    var result: BatchGetJobEntityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetJobEntityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
