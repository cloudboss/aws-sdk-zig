...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQueueLimitAssociationInput, options: Options) !GetQueueLimitAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQueueLimitAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/queue-limit-associations/");
    try path_buf.appendSlice(alloc, input.queue_id);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.limit_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQueueLimitAssociationOutput {
    var result: GetQueueLimitAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQueueLimitAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
