...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQueueFleetAssociationInput, options: Options) !GetQueueFleetAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQueueFleetAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/queue-fleet-associations/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQueueFleetAssociationOutput {
    var result: GetQueueFleetAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQueueFleetAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
