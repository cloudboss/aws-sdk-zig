...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateQueueEnvironmentInput, options: Options) !CreateQueueEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateQueueEnvironmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.template), input.template, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateQueueEnvironmentOutput {
    var result: CreateQueueEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateQueueEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
