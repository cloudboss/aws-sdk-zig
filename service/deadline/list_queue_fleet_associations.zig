...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQueueFleetAssociationsInput, options: Options) !ListQueueFleetAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQueueFleetAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.fleet_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "fleetId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQueueFleetAssociationsOutput {
    var result: ListQueueFleetAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQueueFleetAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
