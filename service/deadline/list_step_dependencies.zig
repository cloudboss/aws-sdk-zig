...
const StepDependency = @import("step_dependency.zig").StepDependency;

pub const ListStepDependenciesInput = struct {
...
pub const ListStepDependenciesOutput = struct {
    /// The dependencies on the list.
    dependencies: ?[]const StepDependency = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStepDependenciesInput, options: Options) !ListStepDependenciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStepDependenciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dependencies");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStepDependenciesOutput {
    var result: ListStepDependenciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStepDependenciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
