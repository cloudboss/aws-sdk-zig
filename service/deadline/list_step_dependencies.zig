...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStepDependenciesInput, options: CallOptions) !ListStepDependenciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListStepDependenciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListStepDependenciesOutput {
    var result: ListStepDependenciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStepDependenciesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
