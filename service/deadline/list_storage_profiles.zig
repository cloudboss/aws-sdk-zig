...
pub const ListStorageProfilesInput = struct {
...
pub const ListStorageProfilesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStorageProfilesInput, options: Options) !ListStorageProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStorageProfilesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStorageProfilesOutput {
    var result: ListStorageProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStorageProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
