...
const FleetMember = @import("fleet_member.zig").FleetMember;

pub const ListFleetMembersInput = struct {
...
pub const ListFleetMembersOutput = struct {
    /// The members on the list.
    members: ?[]const FleetMember = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFleetMembersInput, options: CallOptions) !ListFleetMembersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFleetMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFleetMembersOutput {
    var result: ListFleetMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFleetMembersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
