...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFleetMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFleetMembersOutput {
    var result: ListFleetMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFleetMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
