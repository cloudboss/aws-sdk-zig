...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFarmsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFarmsOutput {
    var result: ListFarmsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFarmsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
