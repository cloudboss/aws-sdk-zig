...
const FarmSummary = @import("farm_summary.zig").FarmSummary;

pub const ListFarmsInput = struct {
...
pub const ListFarmsOutput = struct {
    /// Farms on the list.
    farms: ?[]const FarmSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFarmsInput, options: CallOptions) !ListFarmsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFarmsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFarmsOutput {
    var result: ListFarmsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFarmsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
