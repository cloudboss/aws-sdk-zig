...
/// Cloud worker host.
pub const AcceleratorSelection = struct {
    ...
    ///   `latest` and a new version of the runtime is released, the new version of
    ///   the runtime is used.
    /// * `grid:r580` - [NVIDIA vGPU software
    ///   19](https://docs.nvidia.com/vgpu/19.0/index.html)
    /// * `grid:r570` - [NVIDIA vGPU software
    ///   18](https://docs.nvidia.com/vgpu/18.0/index.html)
    /// * `grid:r535` - [NVIDIA vGPU software
    ///   16](https://docs.nvidia.com/vgpu/16.0/index.html)
    ///
    /// If you don't specify a runtime, Amazon Web Services Deadline Cloud uses
    /// `latest` as the default. However, if you have multiple accelerators and
    /// specify `latest` for some and leave others blank, Amazon Web Services
    /// Deadline Cloud raises an exception.
    ///
    ...
    ///
    /// When you specify `latest`, it resolves to `grid:r580` for all currently
    /// supported accelerators.
    runtime: []const u8 = "latest",
