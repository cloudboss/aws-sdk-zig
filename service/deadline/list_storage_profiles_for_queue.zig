...
const StorageProfileSummary = @import("storage_profile_summary.zig").StorageProfileSummary;

pub const ListStorageProfilesForQueueInput = struct {
...
pub const ListStorageProfilesForQueueOutput = struct {
    ...
    storage_profiles: ?[]const StorageProfileSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStorageProfilesForQueueInput, options: Options) !ListStorageProfilesForQueueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStorageProfilesForQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/storage-profiles");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStorageProfilesForQueueOutput {
    var result: ListStorageProfilesForQueueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStorageProfilesForQueueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
