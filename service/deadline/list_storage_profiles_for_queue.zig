...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStorageProfilesForQueueInput, options: CallOptions) !ListStorageProfilesForQueueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListStorageProfilesForQueueInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListStorageProfilesForQueueOutput {
    var result: ListStorageProfilesForQueueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStorageProfilesForQueueOutput, body, allocator);
    }
    _ = status;
    _ = headers;
