...
pub const CreateStorageProfileInput = struct {
...
pub const CreateStorageProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStorageProfileInput, options: CallOptions) !CreateStorageProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateStorageProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (input.file_system_locations) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"fileSystemLocations\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"osFamily\":");
    try aws.json.writeValue(@TypeOf(input.os_family), input.os_family, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateStorageProfileOutput {
    var result: CreateStorageProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateStorageProfileOutput, body, allocator);
    }
    _ = status;
    _ = headers;
