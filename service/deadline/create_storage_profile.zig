...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStorageProfileInput, options: Options) !CreateStorageProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStorageProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.file_system_locations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"fileSystemLocations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"osFamily\":");
    try aws.json.writeValue(@TypeOf(input.os_family), input.os_family, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStorageProfileOutput {
    var result: CreateStorageProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateStorageProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
