...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStorageProfileForQueueInput, options: Options) !GetStorageProfileForQueueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStorageProfileForQueueInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStorageProfileForQueueOutput {
    var result: GetStorageProfileForQueueOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetStorageProfileForQueueOutput, body, alloc);
    }
    _ = status;
    _ = headers;
