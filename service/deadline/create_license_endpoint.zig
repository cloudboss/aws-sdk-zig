...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLicenseEndpointInput, options: Options) !CreateLicenseEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLicenseEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLicenseEndpointOutput {
    var result: CreateLicenseEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLicenseEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
