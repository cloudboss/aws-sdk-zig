...
const StepConsumer = @import("step_consumer.zig").StepConsumer;

pub const ListStepConsumersInput = struct {
...
pub const ListStepConsumersOutput = struct {
    /// The consumers on the list.
    consumers: ?[]const StepConsumer = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStepConsumersInput, options: Options) !ListStepConsumersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStepConsumersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/consumers");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStepConsumersOutput {
    var result: ListStepConsumersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStepConsumersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
