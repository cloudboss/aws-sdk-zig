...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStepConsumersInput, options: CallOptions) !ListStepConsumersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListStepConsumersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListStepConsumersOutput {
    var result: ListStepConsumersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStepConsumersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
