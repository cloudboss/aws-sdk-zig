...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQueueEnvironmentInput, options: Options) !GetQueueEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQueueEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQueueEnvironmentOutput {
    var result: GetQueueEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQueueEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
