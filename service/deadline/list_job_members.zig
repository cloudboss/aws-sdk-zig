...
fn serializeRequest(alloc: std.mem.Allocator, input: ListJobMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListJobMembersOutput {
    var result: ListJobMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListJobMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
