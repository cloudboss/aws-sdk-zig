...
const JobMember = @import("job_member.zig").JobMember;

pub const ListJobMembersInput = struct {
...
pub const ListJobMembersOutput = struct {
    /// The members on the list.
    members: ?[]const JobMember = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListJobMembersInput, options: CallOptions) !ListJobMembersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListJobMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListJobMembersOutput {
    var result: ListJobMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListJobMembersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
