...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkerInput, options: Options) !GetWorkerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkerOutput {
    var result: GetWorkerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
