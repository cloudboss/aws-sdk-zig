...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBudgetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBudgetsOutput {
    var result: ListBudgetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBudgetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
