...
const BudgetSummary = @import("budget_summary.zig").BudgetSummary;

pub const ListBudgetsInput = struct {
...
pub const ListBudgetsOutput = struct {
    /// The budgets to include on the list.
    budgets: ?[]const BudgetSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBudgetsInput, options: CallOptions) !ListBudgetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBudgetsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBudgetsOutput {
    var result: ListBudgetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBudgetsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
