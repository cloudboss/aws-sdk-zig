...
const QueueLimitAssociationSummary = @import("queue_limit_association_summary.zig").QueueLimitAssociationSummary;

pub const ListQueueLimitAssociationsInput = struct {
    ...
    /// for the specified limit. If you specify both the `queueId` and the
    /// `limitId`, only the specified limit is returned if it exists.
    ...
    /// Specifies that the operation should return only the queue limit associations
    /// for the specified queue. If you specify both the `queueId` and the
    /// `limitId`, only the specified limit is returned if it exists.
...
pub const ListQueueLimitAssociationsOutput = struct {
    ...
    /// the request.
    queue_limit_associations: ?[]const QueueLimitAssociationSummary = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .queue_limit_associations = "queueLimitAssociations",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQueueLimitAssociationsInput, options: CallOptions) !ListQueueLimitAssociationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListQueueLimitAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.limit_id) |v| {
        ...
        try query_buf.appendSlice(allocator, "limitId=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListQueueLimitAssociationsOutput {
    var result: ListQueueLimitAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQueueLimitAssociationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
