...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQueueLimitAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.limit_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "limitId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQueueLimitAssociationsOutput {
    var result: ListQueueLimitAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQueueLimitAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
