...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFarmInput, options: Options) !CreateFarmOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFarmInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFarmOutput {
    var result: CreateFarmOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFarmOutput, body, alloc);
    }
    _ = status;
    _ = headers;
