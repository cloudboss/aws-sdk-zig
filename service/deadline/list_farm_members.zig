...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFarmMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFarmMembersOutput {
    var result: ListFarmMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFarmMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
