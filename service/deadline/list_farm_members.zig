...
const FarmMember = @import("farm_member.zig").FarmMember;

pub const ListFarmMembersInput = struct {
    /// The farm ID.
    farm_id: []const u8,
...
pub const ListFarmMembersOutput = struct {
    /// The members on the list.
    members: ?[]const FarmMember = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFarmMembersInput, options: CallOptions) !ListFarmMembersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFarmMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFarmMembersOutput {
    var result: ListFarmMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFarmMembersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
