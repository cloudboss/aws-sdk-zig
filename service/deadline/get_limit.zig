...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLimitInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLimitOutput {
    var result: GetLimitOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLimitOutput, body, alloc);
    }
    _ = status;
    _ = headers;
