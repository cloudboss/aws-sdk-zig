...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBudgetInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.approximate_dollar_limit), input.approximate_dollar_limit, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"usageTrackingResource\":");
    try aws.json.writeValue(@TypeOf(input.usage_tracking_resource), input.usage_tracking_resource, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBudgetOutput {
    var result: CreateBudgetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBudgetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
