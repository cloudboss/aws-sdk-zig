...
pub const CreateBudgetInput = struct {
    /// The budget actions to specify what happens when the budget runs out.
    actions: []const BudgetActionToAdd,
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateBudgetInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.approximate_dollar_limit), input.approximate_dollar_limit, allocator, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"usageTrackingResource\":");
    try aws.json.writeValue(@TypeOf(input.usage_tracking_resource), input.usage_tracking_resource, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateBudgetOutput {
    var result: CreateBudgetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBudgetOutput, body, allocator);
    }
    _ = status;
    _ = headers;
