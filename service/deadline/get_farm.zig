...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFarmInput, options: CallOptions) !GetFarmOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFarmInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFarmOutput {
    var result: GetFarmOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFarmOutput, body, allocator);
    }
    _ = status;
    _ = headers;
