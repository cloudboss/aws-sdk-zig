...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFarmInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFarmOutput {
    var result: GetFarmOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFarmOutput, body, alloc);
    }
    _ = status;
    _ = headers;
