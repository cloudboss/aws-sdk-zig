...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"targetLifecycleStatus\":");
    try aws.json.writeValue(@TypeOf(input.target_lifecycle_status), input.target_lifecycle_status, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateSessionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
