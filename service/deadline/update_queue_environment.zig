...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateQueueEnvironmentInput, options: Options) !UpdateQueueEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQueueEnvironmentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.template) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"template\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQueueEnvironmentOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
