...
pub const AssumeFleetRoleForWorkerInput = struct {
...
pub const AssumeFleetRoleForWorkerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssumeFleetRoleForWorkerInput, options: Options) !AssumeFleetRoleForWorkerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssumeFleetRoleForWorkerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/fleet-roles");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssumeFleetRoleForWorkerOutput {
    var result: AssumeFleetRoleForWorkerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssumeFleetRoleForWorkerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
