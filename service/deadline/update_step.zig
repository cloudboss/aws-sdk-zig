...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStepInput, options: Options) !UpdateStepOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStepInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"targetTaskRunStatus\":");
    try aws.json.writeValue(@TypeOf(input.target_task_run_status), input.target_task_run_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStepOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
