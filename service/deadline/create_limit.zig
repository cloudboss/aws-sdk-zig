...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLimitInput, options: CallOptions) !CreateLimitOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLimitInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"amountRequirementName\":");
    try aws.json.writeValue(@TypeOf(input.amount_requirement_name), input.amount_requirement_name, allocator, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.max_count), input.max_count, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLimitOutput {
    var result: CreateLimitOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLimitOutput, body, allocator);
    }
    _ = status;
    _ = headers;
