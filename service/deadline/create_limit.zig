...
pub const CreateLimitInput = struct {
    ...
    /// A description of the limit. A description helps you identify the purpose of
    /// the limit.
    ///
...
pub const CreateLimitOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLimitInput, options: Options) !CreateLimitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLimitInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/limits");
    ...
    try body_buf.appendSlice(alloc, "\"amountRequirementName\":");
    try aws.json.writeValue(@TypeOf(input.amount_requirement_name), input.amount_requirement_name, alloc, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.max_count), input.max_count, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLimitOutput {
    var result: CreateLimitOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLimitOutput, body, alloc);
    }
    _ = status;
    _ = headers;
