...
const QueueMember = @import("queue_member.zig").QueueMember;

pub const ListQueueMembersInput = struct {
...
pub const ListQueueMembersOutput = struct {
    /// The members on the list.
    members: ?[]const QueueMember = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQueueMembersInput, options: Options) !ListQueueMembersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQueueMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQueueMembersOutput {
    var result: ListQueueMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQueueMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
