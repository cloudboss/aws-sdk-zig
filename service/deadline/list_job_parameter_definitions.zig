...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListJobParameterDefinitionsInput, options: CallOptions) !ListJobParameterDefinitionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListJobParameterDefinitionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListJobParameterDefinitionsOutput {
    var result: ListJobParameterDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListJobParameterDefinitionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
