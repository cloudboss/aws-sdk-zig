...
pub const ListJobParameterDefinitionsInput = struct {
...
pub const ListJobParameterDefinitionsOutput = struct {
    /// Lists parameter definitions of a job.
    job_parameter_definitions: ?[]const []const u8 = null,
    ...
    pub const json_field_names = .{
        .job_parameter_definitions = "jobParameterDefinitions",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListJobParameterDefinitionsInput, options: Options) !ListJobParameterDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListJobParameterDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/parameter-definitions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListJobParameterDefinitionsOutput {
    var result: ListJobParameterDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListJobParameterDefinitionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
