...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkerInput, options: Options) !UpdateWorkerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkerOutput {
    var result: UpdateWorkerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateWorkerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
