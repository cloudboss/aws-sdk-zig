...
pub const CreateReplicationConfigurationOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReplicationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.source_file_system_id);
    try path_buf.appendSlice(alloc, "/replication-configuration");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReplicationConfigurationOutput {
    var result: CreateReplicationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateReplicationConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
