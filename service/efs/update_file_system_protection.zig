...
pub const UpdateFileSystemProtectionOutput = struct {
    /// The status of the file system's replication overwrite protection.
    ///
    /// * `ENABLED` – The file system cannot be used as the destination file
    /// system in a replication configuration. The file system is writeable.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFileSystemProtectionInput, options: Options) !UpdateFileSystemProtectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFileSystemProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/protection");
    ...
    if (input.replication_overwrite_protection) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ReplicationOverwriteProtection\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFileSystemProtectionOutput {
    var result: UpdateFileSystemProtectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFileSystemProtectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
