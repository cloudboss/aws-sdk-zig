...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBackupPolicyInput, options: Options) !PutBackupPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBackupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BackupPolicy\":");
    try aws.json.writeValue(@TypeOf(input.backup_policy), input.backup_policy, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutBackupPolicyOutput {
    var result: PutBackupPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutBackupPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
