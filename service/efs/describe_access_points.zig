...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccessPointsInput, options: Options) !DescribeAccessPointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccessPointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccessPointsOutput {
    var result: DescribeAccessPointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccessPointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
