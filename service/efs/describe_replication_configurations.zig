...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationConfigurationsInput, options: CallOptions) !DescribeReplicationConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReplicationConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReplicationConfigurationsOutput {
    var result: DescribeReplicationConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeReplicationConfigurationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
