...
pub const PutLifecycleConfigurationInput = struct {
    ...
    /// (Standard storage class) after they are accessed in IA
    /// or Archive storage.
    ///
    ...
    /// `LifecyclePolicy` objects, one object for each storage transition. See the
    /// example
    /// requests in the following section for more information.
    lifecycle_policies: []const LifecyclePolicy,
...
pub const PutLifecycleConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutLifecycleConfigurationInput, options: Options) !PutLifecycleConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutLifecycleConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/lifecycle-configuration");
    ...
    try body_buf.appendSlice(alloc, "\"LifecyclePolicies\":");
    try aws.json.writeValue(@TypeOf(input.lifecycle_policies), input.lifecycle_policies, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutLifecycleConfigurationOutput {
    var result: PutLifecycleConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutLifecycleConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
