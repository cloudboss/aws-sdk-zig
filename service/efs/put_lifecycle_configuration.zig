...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutLifecycleConfigurationInput, options: CallOptions) !PutLifecycleConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutLifecycleConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"LifecyclePolicies\":");
    try aws.json.writeValue(@TypeOf(input.lifecycle_policies), input.lifecycle_policies, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutLifecycleConfigurationOutput {
    var result: PutLifecycleConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutLifecycleConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
