...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountPreferencesInput, options: Options) !DescribeAccountPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountPreferencesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountPreferencesOutput {
    var result: DescribeAccountPreferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccountPreferencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
