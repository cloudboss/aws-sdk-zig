...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccountPreferencesInput, options: Options) !PutAccountPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccountPreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ResourceIdType\":");
    try aws.json.writeValue(@TypeOf(input.resource_id_type), input.resource_id_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccountPreferencesOutput {
    var result: PutAccountPreferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutAccountPreferencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
