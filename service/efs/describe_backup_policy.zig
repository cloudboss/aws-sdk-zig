...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBackupPolicyInput, options: Options) !DescribeBackupPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBackupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/backup-policy");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBackupPolicyOutput {
    var result: DescribeBackupPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBackupPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
