...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFileSystemPolicyInput, options: Options) !DescribeFileSystemPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFileSystemPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFileSystemPolicyOutput {
    var result: DescribeFileSystemPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFileSystemPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
