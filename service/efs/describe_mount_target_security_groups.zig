...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMountTargetSecurityGroupsInput, options: CallOptions) !DescribeMountTargetSecurityGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMountTargetSecurityGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/2015-02-01/mount-targets/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMountTargetSecurityGroupsOutput {
    var result: DescribeMountTargetSecurityGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeMountTargetSecurityGroupsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
