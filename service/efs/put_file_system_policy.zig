...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutFileSystemPolicyInput, options: Options) !PutFileSystemPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutFileSystemPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.bypass_policy_lockout_safety_check) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"BypassPolicyLockoutSafetyCheck\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutFileSystemPolicyOutput {
    var result: PutFileSystemPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutFileSystemPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
