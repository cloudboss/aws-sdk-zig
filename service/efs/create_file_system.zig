...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFileSystemInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.availability_zone_name) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AvailabilityZoneName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.backup) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Backup\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"CreationToken\":");
    try aws.json.writeValue(@TypeOf(input.creation_token), input.creation_token, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.performance_mode) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PerformanceMode\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFileSystemOutput {
    var result: CreateFileSystemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFileSystemOutput, body, alloc);
    }
    _ = status;
    _ = headers;
