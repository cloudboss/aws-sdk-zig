...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLifecycleConfigurationInput, options: Options) !DescribeLifecycleConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLifecycleConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLifecycleConfigurationOutput {
    var result: DescribeLifecycleConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeLifecycleConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
