...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMountTargetsInput, options: Options) !DescribeMountTargetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMountTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.access_point_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "AccessPointId=");
    ...
    if (input.mount_target_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "MountTargetId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMountTargetsOutput {
    var result: DescribeMountTargetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeMountTargetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
