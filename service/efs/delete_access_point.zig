...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAccessPointInput, options: Options) !DeleteAccessPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAccessPointInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2015-02-01/access-points/");
    try path_buf.appendSlice(alloc, input.access_point_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAccessPointOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteAccessPointOutput = .{};
