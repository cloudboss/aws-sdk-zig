...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDiscoverersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.discoverer_id_prefix) |v| {
        ...
        try query_buf.appendSlice(alloc, "discovererIdPrefix=");
    ...
    if (input.source_arn_prefix) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sourceArnPrefix=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDiscoverersOutput {
    var result: ListDiscoverersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDiscoverersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
