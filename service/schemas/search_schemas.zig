...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchSchemasInput, options: CallOptions) !SearchSchemasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchSchemasInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/schemas/search");
    const path = try path_buf.toOwnedSlice(allocator);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "keywords=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.keywords);
    query_has_prev = true;
    if (input.limit) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
