...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRegistriesInput, options: Options) !ListRegistriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRegistriesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.registry_name_prefix) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "registryNamePrefix=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.scope) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRegistriesOutput {
    var result: ListRegistriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRegistriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
