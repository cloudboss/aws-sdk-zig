...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeKeyValueStoreInput, options: Options) !DescribeKeyValueStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeKeyValueStoreInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeKeyValueStoreOutput {
    var result: DescribeKeyValueStoreOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeKeyValueStoreOutput, body, alloc);
    }
    _ = status;
