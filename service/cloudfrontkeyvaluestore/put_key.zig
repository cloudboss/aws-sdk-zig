...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutKeyInput, options: Options) !PutKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutKeyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutKeyOutput {
    var result: PutKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutKeyOutput, body, alloc);
    }
    _ = status;
