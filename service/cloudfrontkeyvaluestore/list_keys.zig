...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListKeysInput, options: Options) !ListKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListKeysInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListKeysOutput {
    var result: ListKeysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListKeysOutput, body, alloc);
    }
    _ = status;
    _ = headers;
