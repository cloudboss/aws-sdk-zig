...
pub const CreateOriginEndpointInput = struct {
    ...
    /// The type of container to attach to this origin endpoint. A container type is
    /// a file format that encapsulates one or more media streams, such as audio and
    /// video, into a single file. You can't change the container type after you
    /// create the endpoint.
    ...
    /// different streaming experiences or to support different client requirements.
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOriginEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dash_manifests) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DashManifests\":");
    ...
    if (input.force_endpoint_error_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ForceEndpointErrorConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.hls_manifests) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"HlsManifests\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.low_latency_hls_manifests) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LowLatencyHlsManifests\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.mss_manifests) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MssManifests\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OriginEndpointName\":");
    try aws.json.writeValue(@TypeOf(input.origin_endpoint_name), input.origin_endpoint_name, alloc, &body_buf);
    has_prev = true;
    if (input.segment) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.startover_window_seconds) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"StartoverWindowSeconds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOriginEndpointOutput {
    var result: CreateOriginEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateOriginEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
