...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOriginEndpointInput, options: CallOptions) !CreateOriginEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateOriginEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"OriginEndpointName\":");
    try aws.json.writeValue(@TypeOf(input.origin_endpoint_name), input.origin_endpoint_name, allocator, &body_buf);
    has_prev = true;
    if (input.segment) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
