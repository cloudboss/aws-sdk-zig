...
fn serializeRequest(alloc: std.mem.Allocator, input: GetHarvestJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetHarvestJobOutput {
    var result: GetHarvestJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetHarvestJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
