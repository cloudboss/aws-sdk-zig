...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOriginEndpointPolicyInput, options: Options) !GetOriginEndpointPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOriginEndpointPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOriginEndpointPolicyOutput {
    var result: GetOriginEndpointPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOriginEndpointPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
