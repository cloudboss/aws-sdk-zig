...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelHarvestJobInput, options: Options) !CancelHarvestJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelHarvestJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.harvest_job_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelHarvestJobOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
