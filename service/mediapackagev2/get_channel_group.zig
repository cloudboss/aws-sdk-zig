...
pub const GetChannelGroupInput = struct {
...
pub const GetChannelGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetChannelGroupInput, options: CallOptions) !GetChannelGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetChannelGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetChannelGroupOutput {
    var result: GetChannelGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChannelGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
