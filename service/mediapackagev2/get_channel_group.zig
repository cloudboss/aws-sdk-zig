...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChannelGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChannelGroupOutput {
    var result: GetChannelGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChannelGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
