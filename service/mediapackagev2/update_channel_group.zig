...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChannelGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChannelGroupOutput {
    var result: UpdateChannelGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChannelGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
