...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetChannelStateInput, options: Options) !ResetChannelStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetChannelStateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetChannelStateOutput {
    var result: ResetChannelStateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ResetChannelStateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
