...
const OriginEndpointListConfiguration = @import("origin_endpoint_list_configuration.zig").OriginEndpointListConfiguration;
...
pub const ListOriginEndpointsOutput = struct {
    ...
    items: ?[]const OriginEndpointListConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOriginEndpointsInput, options: Options) !ListOriginEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOriginEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOriginEndpointsOutput {
    var result: ListOriginEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOriginEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
