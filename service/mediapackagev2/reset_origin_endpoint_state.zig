...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetOriginEndpointStateInput, options: Options) !ResetOriginEndpointStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetOriginEndpointStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/reset");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetOriginEndpointStateOutput {
    var result: ResetOriginEndpointStateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ResetOriginEndpointStateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
