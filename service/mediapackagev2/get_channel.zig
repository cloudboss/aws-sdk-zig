...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetChannelInput, options: Options) !GetChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChannelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChannelOutput {
    var result: GetChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChannelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
