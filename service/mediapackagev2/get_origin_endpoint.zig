...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOriginEndpointInput, options: Options) !GetOriginEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOriginEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOriginEndpointOutput {
    var result: GetOriginEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOriginEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
