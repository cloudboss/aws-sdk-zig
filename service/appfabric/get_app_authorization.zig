...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAppAuthorizationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAppAuthorizationOutput {
    var result: GetAppAuthorizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAppAuthorizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
