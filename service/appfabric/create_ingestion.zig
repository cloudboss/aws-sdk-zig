...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIngestionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ingestionType\":");
    try aws.json.writeValue(@TypeOf(input.ingestion_type), input.ingestion_type, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIngestionOutput {
    var result: CreateIngestionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIngestionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
