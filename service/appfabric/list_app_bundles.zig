...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppBundlesInput, options: Options) !ListAppBundlesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppBundlesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppBundlesOutput {
    var result: ListAppBundlesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppBundlesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
