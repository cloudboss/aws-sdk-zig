...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppAuthorizationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppAuthorizationsOutput {
    var result: ListAppAuthorizationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppAuthorizationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
