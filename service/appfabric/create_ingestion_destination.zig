...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIngestionDestinationInput, options: Options) !CreateIngestionDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIngestionDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"processingConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.processing_configuration), input.processing_configuration, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIngestionDestinationOutput {
    var result: CreateIngestionDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIngestionDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
