...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIngestionDestinationInput, options: Options) !UpdateIngestionDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIngestionDestinationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIngestionDestinationOutput {
    var result: UpdateIngestionDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIngestionDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
