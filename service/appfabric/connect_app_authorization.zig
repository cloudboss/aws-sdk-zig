...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ConnectAppAuthorizationInput, options: Options) !ConnectAppAuthorizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ConnectAppAuthorizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/connect");
    ...
    if (input.auth_request) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"authRequest\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ConnectAppAuthorizationOutput {
    var result: ConnectAppAuthorizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ConnectAppAuthorizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
