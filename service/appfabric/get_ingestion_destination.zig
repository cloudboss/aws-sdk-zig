...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIngestionDestinationInput, options: CallOptions) !GetIngestionDestinationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIngestionDestinationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIngestionDestinationOutput {
    var result: GetIngestionDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIngestionDestinationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
