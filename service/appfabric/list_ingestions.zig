...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIngestionsInput, options: Options) !ListIngestionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIngestionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIngestionsOutput {
    var result: ListIngestionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIngestionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
