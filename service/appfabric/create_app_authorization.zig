...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAppAuthorizationInput, options: Options) !CreateAppAuthorizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAppAuthorizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.credential), input.credential, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.tenant), input.tenant, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAppAuthorizationOutput {
    var result: CreateAppAuthorizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppAuthorizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
