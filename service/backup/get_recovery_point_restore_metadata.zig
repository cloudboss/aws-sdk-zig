...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRecoveryPointRestoreMetadataInput, options: CallOptions) !GetRecoveryPointRestoreMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRecoveryPointRestoreMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/restore-metadata");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRecoveryPointRestoreMetadataOutput {
    var result: GetRecoveryPointRestoreMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRecoveryPointRestoreMetadataOutput, body, allocator);
    }
    _ = status;
    _ = headers;
