...
pub const GetRecoveryPointRestoreMetadataInput = struct {
...
pub const GetRecoveryPointRestoreMetadataOutput = struct {
    ...
    /// restored.
    restore_metadata: ?[]const aws.map.StringMapEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRecoveryPointRestoreMetadataInput, options: Options) !GetRecoveryPointRestoreMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRecoveryPointRestoreMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/backup-vaults/");
    try path_buf.appendSlice(alloc, input.backup_vault_name);
    try path_buf.appendSlice(alloc, "/recovery-points/");
    try path_buf.appendSlice(alloc, input.recovery_point_arn);
    try path_buf.appendSlice(alloc, "/restore-metadata");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRecoveryPointRestoreMetadataOutput {
    var result: GetRecoveryPointRestoreMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRecoveryPointRestoreMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
