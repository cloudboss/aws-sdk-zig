...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRecoveryPointIndexDetailsInput, options: Options) !GetRecoveryPointIndexDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRecoveryPointIndexDetailsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRecoveryPointIndexDetailsOutput {
    var result: GetRecoveryPointIndexDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRecoveryPointIndexDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
