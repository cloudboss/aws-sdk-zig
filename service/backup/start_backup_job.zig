...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartBackupJobInput, options: Options) !StartBackupJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartBackupJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.backup_options) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"BackupOptions\":");
    ...
    if (input.complete_window_minutes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"CompleteWindowMinutes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.index) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Index\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.logically_air_gapped_backup_vault_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LogicallyAirGappedBackupVaultArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.recovery_point_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecoveryPointTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.start_window_minutes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"StartWindowMinutes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartBackupJobOutput {
    var result: StartBackupJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartBackupJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
