...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRestoreTestingSelectionsInput, options: Options) !ListRestoreTestingSelectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRestoreTestingSelectionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRestoreTestingSelectionsOutput {
    var result: ListRestoreTestingSelectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRestoreTestingSelectionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
