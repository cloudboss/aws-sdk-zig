...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRecoveryPointIndexSettingsInput, options: Options) !UpdateRecoveryPointIndexSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRecoveryPointIndexSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.index), input.index, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRecoveryPointIndexSettingsOutput {
    var result: UpdateRecoveryPointIndexSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRecoveryPointIndexSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
