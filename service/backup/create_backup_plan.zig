...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBackupPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.backup_plan_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"BackupPlanTags\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBackupPlanOutput {
    var result: CreateBackupPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBackupPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
