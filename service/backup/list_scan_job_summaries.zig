...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListScanJobSummariesInput, options: Options) !ListScanJobSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListScanJobSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.malware_scanner) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "MalwareScanner=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    ...
    if (input.scan_result_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ScanResultStatus=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.state) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListScanJobSummariesOutput {
    var result: ListScanJobSummariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListScanJobSummariesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
