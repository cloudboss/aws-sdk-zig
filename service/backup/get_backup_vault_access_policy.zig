...
pub const GetBackupVaultAccessPolicyInput = struct {
...
pub const GetBackupVaultAccessPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackupVaultAccessPolicyInput, options: Options) !GetBackupVaultAccessPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBackupVaultAccessPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBackupVaultAccessPolicyOutput {
    var result: GetBackupVaultAccessPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackupVaultAccessPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
