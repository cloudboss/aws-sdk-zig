...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackupVaultAccessPolicyInput, options: CallOptions) !GetBackupVaultAccessPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBackupVaultAccessPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBackupVaultAccessPolicyOutput {
    var result: GetBackupVaultAccessPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackupVaultAccessPolicyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
