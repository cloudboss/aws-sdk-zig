...
const ProtectedResource = @import("protected_resource.zig").ProtectedResource;

pub const ListProtectedResourcesInput = struct {
...
pub const ListProtectedResourcesOutput = struct {
    /// The next item following a partial list of returned items. For example, if a
    /// request is
    /// made to return `MaxResults` number of items, `NextToken` allows you
    /// to return more items in your list starting at the location pointed to by the
    ...
    /// resource
    /// type.
    results: ?[]const ProtectedResource = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProtectedResourcesInput, options: CallOptions) !ListProtectedResourcesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProtectedResourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListProtectedResourcesOutput {
    var result: ListProtectedResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProtectedResourcesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
