...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProtectedResourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProtectedResourcesOutput {
    var result: ListProtectedResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProtectedResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
