...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBackupPlansInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_deleted) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeDeleted=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBackupPlansOutput {
    var result: ListBackupPlansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBackupPlansOutput, body, alloc);
    }
    _ = status;
    _ = headers;
