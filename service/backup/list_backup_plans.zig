...
const BackupPlansListMember = @import("backup_plans_list_member.zig").BackupPlansListMember;

pub const ListBackupPlansInput = struct {
...
pub const ListBackupPlansOutput = struct {
    /// Information about the backup plans.
    backup_plans_list: ?[]const BackupPlansListMember = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBackupPlansInput, options: CallOptions) !ListBackupPlansOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBackupPlansInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_deleted) |v| {
        ...
        try query_buf.appendSlice(allocator, "includeDeleted=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBackupPlansOutput {
    var result: ListBackupPlansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBackupPlansOutput, body, allocator);
    }
    _ = status;
    _ = headers;
