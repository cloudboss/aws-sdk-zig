...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRestoreTestingSelectionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRestoreTestingSelectionOutput {
    var result: CreateRestoreTestingSelectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRestoreTestingSelectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
