...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartScanJobInput, options: Options) !StartScanJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartScanJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BackupVaultName\":");
    try aws.json.writeValue(@TypeOf(input.backup_vault_name), input.backup_vault_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MalwareScanner\":");
    try aws.json.writeValue(@TypeOf(input.malware_scanner), input.malware_scanner, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RecoveryPointArn\":");
    try aws.json.writeValue(@TypeOf(input.recovery_point_arn), input.recovery_point_arn, alloc, &body_buf);
    has_prev = true;
    if (input.scan_base_recovery_point_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ScanBaseRecoveryPointArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ScanMode\":");
    try aws.json.writeValue(@TypeOf(input.scan_mode), input.scan_mode, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ScannerRoleArn\":");
    try aws.json.writeValue(@TypeOf(input.scanner_role_arn), input.scanner_role_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartScanJobOutput {
    var result: StartScanJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartScanJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
