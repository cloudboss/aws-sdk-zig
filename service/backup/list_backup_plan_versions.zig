...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBackupPlanVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBackupPlanVersionsOutput {
    var result: ListBackupPlanVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBackupPlanVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
