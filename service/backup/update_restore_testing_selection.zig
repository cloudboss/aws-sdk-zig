...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRestoreTestingSelectionInput, options: Options) !UpdateRestoreTestingSelectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRestoreTestingSelectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/restore-testing/plans/");
    try path_buf.appendSlice(alloc, input.restore_testing_plan_name);
    try path_buf.appendSlice(alloc, "/selections/");
    try path_buf.appendSlice(alloc, input.restore_testing_selection_name);
    ...
    try body_buf.appendSlice(alloc, "\"RestoreTestingSelection\":");
    try aws.json.writeValue(@TypeOf(input.restore_testing_selection), input.restore_testing_selection, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRestoreTestingSelectionOutput {
    var result: UpdateRestoreTestingSelectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRestoreTestingSelectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
