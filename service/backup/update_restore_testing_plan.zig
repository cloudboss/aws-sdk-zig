...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRestoreTestingPlanInput, options: Options) !UpdateRestoreTestingPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRestoreTestingPlanInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRestoreTestingPlanOutput {
    var result: UpdateRestoreTestingPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRestoreTestingPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
