...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBackupVaultInput, options: Options) !CreateBackupVaultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBackupVaultInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBackupVaultOutput {
    var result: CreateBackupVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBackupVaultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
