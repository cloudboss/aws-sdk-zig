...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRestoreAccessBackupVaultInput, options: Options) !CreateRestoreAccessBackupVaultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRestoreAccessBackupVaultInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceBackupVaultArn\":");
    try aws.json.writeValue(@TypeOf(input.source_backup_vault_arn), input.source_backup_vault_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRestoreAccessBackupVaultOutput {
    var result: CreateRestoreAccessBackupVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRestoreAccessBackupVaultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
