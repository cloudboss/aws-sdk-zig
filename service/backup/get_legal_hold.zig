...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLegalHoldInput, options: Options) !GetLegalHoldOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLegalHoldInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLegalHoldOutput {
    var result: GetLegalHoldOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLegalHoldOutput, body, alloc);
    }
    _ = status;
    _ = headers;
