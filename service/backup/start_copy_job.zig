...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartCopyJobInput, options: Options) !StartCopyJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartCopyJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"DestinationBackupVaultArn\":");
    try aws.json.writeValue(@TypeOf(input.destination_backup_vault_arn), input.destination_backup_vault_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.lifecycle) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Lifecycle\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceBackupVaultName\":");
    try aws.json.writeValue(@TypeOf(input.source_backup_vault_name), input.source_backup_vault_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartCopyJobOutput {
    var result: StartCopyJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartCopyJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
