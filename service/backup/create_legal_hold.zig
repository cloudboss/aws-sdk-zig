...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLegalHoldInput, options: Options) !CreateLegalHoldOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLegalHoldInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.recovery_point_selection) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecoveryPointSelection\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLegalHoldOutput {
    var result: CreateLegalHoldOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLegalHoldOutput, body, alloc);
    }
    _ = status;
    _ = headers;
