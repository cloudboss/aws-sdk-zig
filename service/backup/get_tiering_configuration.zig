...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTieringConfigurationInput, options: Options) !GetTieringConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTieringConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTieringConfigurationOutput {
    var result: GetTieringConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTieringConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
