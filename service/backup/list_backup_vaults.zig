...
const BackupVaultListMember = @import("backup_vault_list_member.zig").BackupVaultListMember;
...
pub const ListBackupVaultsInput = struct {
    ...
    pub const json_field_names = .{
        .by_shared = "ByShared",
        .by_vault_type = "ByVaultType",
        .max_results = "MaxResults",
        .next_token = "NextToken",
    };
};

pub const ListBackupVaultsOutput = struct {
    /// An array of backup vault list members containing vault metadata, including
    /// Amazon
    /// Resource Name (ARN), display name, creation date, number of saved recovery
    /// points, and
    /// encryption information if the resources saved in the backup vault are
    /// encrypted.
    backup_vault_list: ?[]const BackupVaultListMember = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBackupVaultsInput, options: Options) !ListBackupVaultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBackupVaultsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/backup-vaults";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.by_shared) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "shared=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.by_vault_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "vaultType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBackupVaultsOutput {
    var result: ListBackupVaultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBackupVaultsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
