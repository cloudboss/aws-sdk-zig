...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBackupVaultsInput, options: CallOptions) !ListBackupVaultsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBackupVaultsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.by_shared) |v| {
        ...
        try query_buf.appendSlice(allocator, "shared=");
        try query_buf.appendSlice(allocator, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.by_vault_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "vaultType=");
