...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRestoreJobInput, options: CallOptions) !DescribeRestoreJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRestoreJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeRestoreJobOutput {
    var result: DescribeRestoreJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRestoreJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
