...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRestoreTestingPlansInput, options: Options) !ListRestoreTestingPlansOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRestoreTestingPlansInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRestoreTestingPlansOutput {
    var result: ListRestoreTestingPlansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRestoreTestingPlansOutput, body, alloc);
    }
    _ = status;
    _ = headers;
