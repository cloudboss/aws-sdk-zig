...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReportJobInput, options: Options) !DescribeReportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.report_job_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReportJobOutput {
    var result: DescribeReportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeReportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
