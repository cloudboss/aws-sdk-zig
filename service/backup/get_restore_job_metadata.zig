...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRestoreJobMetadataInput, options: Options) !GetRestoreJobMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRestoreJobMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRestoreJobMetadataOutput {
    var result: GetRestoreJobMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRestoreJobMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
