...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLegalHoldsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLegalHoldsOutput {
    var result: ListLegalHoldsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLegalHoldsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
