...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCopyJobSummariesInput, options: CallOptions) !ListCopyJobSummariesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCopyJobSummariesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCopyJobSummariesOutput {
    var result: ListCopyJobSummariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCopyJobSummariesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
