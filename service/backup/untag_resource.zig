...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/untag/");
    ...
    try body_buf.appendSlice(alloc, "\"TagKeyList\":");
    try aws.json.writeValue(@TypeOf(input.tag_key_list), input.tag_key_list, alloc, &body_buf);
    has_prev = true;
