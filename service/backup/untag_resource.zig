...
fn serializeRequest(allocator: std.mem.Allocator, input: UntagResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/untag/");
    ...
    try body_buf.appendSlice(allocator, "\"TagKeyList\":");
    try aws.json.writeValue(@TypeOf(input.tag_key_list), input.tag_key_list, allocator, &body_buf);
    has_prev = true;
