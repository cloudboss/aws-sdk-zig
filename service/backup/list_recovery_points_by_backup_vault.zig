...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecoveryPointsByBackupVaultInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.by_backup_plan_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "backupPlanId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.by_parent_recovery_point_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "parentRecoveryPointArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecoveryPointsByBackupVaultOutput {
    var result: ListRecoveryPointsByBackupVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRecoveryPointsByBackupVaultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
