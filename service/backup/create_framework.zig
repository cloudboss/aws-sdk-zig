...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFrameworkInput, options: CallOptions) !CreateFrameworkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateFrameworkInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.framework_controls), input.framework_controls, allocator, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"FrameworkName\":");
    try aws.json.writeValue(@TypeOf(input.framework_name), input.framework_name, allocator, &body_buf);
    has_prev = true;
    if (input.framework_tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"FrameworkTags\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateFrameworkOutput {
    var result: CreateFrameworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFrameworkOutput, body, allocator);
    }
    _ = status;
    _ = headers;
