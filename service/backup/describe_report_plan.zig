...
pub const DescribeReportPlanInput = struct {
...
pub const DescribeReportPlanOutput = struct {
    /// Returns details about the report plan that is specified by its name. These
    /// details
    /// include the report plan's Amazon Resource Name (ARN), description, settings,
    /// delivery
    /// channel, deployment status, creation time, and last attempted and successful
    /// run
    /// times.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReportPlanInput, options: Options) !DescribeReportPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReportPlanInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReportPlanOutput {
    var result: DescribeReportPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeReportPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
