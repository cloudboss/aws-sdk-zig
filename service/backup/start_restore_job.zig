...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRestoreJobInput, options: Options) !StartRestoreJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRestoreJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.copy_source_tags_to_restored_resource) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"CopySourceTagsToRestoredResource\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRestoreJobOutput {
    var result: StartRestoreJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartRestoreJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
