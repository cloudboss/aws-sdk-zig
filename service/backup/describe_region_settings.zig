...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRegionSettingsInput, options: Options) !DescribeRegionSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRegionSettingsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRegionSettingsOutput {
    var result: DescribeRegionSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRegionSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
