...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackupSelectionInput, options: Options) !GetBackupSelectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBackupSelectionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBackupSelectionOutput {
    var result: GetBackupSelectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackupSelectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
