...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBackupPlanInput, options: Options) !UpdateBackupPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBackupPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BackupPlan\":");
    try aws.json.writeValue(@TypeOf(input.backup_plan), input.backup_plan, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBackupPlanOutput {
    var result: UpdateBackupPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBackupPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
