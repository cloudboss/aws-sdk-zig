...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIndexedRecoveryPointsInput, options: Options) !ListIndexedRecoveryPointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIndexedRecoveryPointsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.index_status) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "indexStatus=");
    ...
    if (input.source_resource_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sourceResourceArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIndexedRecoveryPointsOutput {
    var result: ListIndexedRecoveryPointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIndexedRecoveryPointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
