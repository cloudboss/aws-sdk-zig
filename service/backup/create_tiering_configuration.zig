...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTieringConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.tiering_configuration_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TieringConfigurationTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTieringConfigurationOutput {
    var result: CreateTieringConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTieringConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
