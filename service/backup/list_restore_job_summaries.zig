...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRestoreJobSummariesInput, options: Options) !ListRestoreJobSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRestoreJobSummariesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRestoreJobSummariesOutput {
    var result: ListRestoreJobSummariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRestoreJobSummariesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
