...
const UpdateRegionSettingsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRegionSettingsInput, options: Options) !UpdateRegionSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRegionSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.resource_type_management_preference) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ResourceTypeManagementPreference\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.resource_type_opt_in_preference) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ResourceTypeOptInPreference\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRegionSettingsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
