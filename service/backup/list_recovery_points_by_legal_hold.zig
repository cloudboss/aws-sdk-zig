...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecoveryPointsByLegalHoldInput, options: CallOptions) !ListRecoveryPointsByLegalHoldOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRecoveryPointsByLegalHoldInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRecoveryPointsByLegalHoldOutput {
    var result: ListRecoveryPointsByLegalHoldOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRecoveryPointsByLegalHoldOutput, body, allocator);
    }
    _ = status;
    _ = headers;
