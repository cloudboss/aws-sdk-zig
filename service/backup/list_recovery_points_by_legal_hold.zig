...
const RecoveryPointMember = @import("recovery_point_member.zig").RecoveryPointMember;

pub const ListRecoveryPointsByLegalHoldInput = struct {
...
pub const ListRecoveryPointsByLegalHoldOutput = struct {
    /// The next item following a partial list of returned resources.
    next_token: ?[]const u8 = null,

    /// The recovery points.
    recovery_points: ?[]const RecoveryPointMember = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecoveryPointsByLegalHoldInput, options: Options) !ListRecoveryPointsByLegalHoldOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecoveryPointsByLegalHoldInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecoveryPointsByLegalHoldOutput {
    var result: ListRecoveryPointsByLegalHoldOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRecoveryPointsByLegalHoldOutput, body, alloc);
    }
    _ = status;
    _ = headers;
