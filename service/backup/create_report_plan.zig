...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReportPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.report_delivery_channel), input.report_delivery_channel, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReportPlanName\":");
    try aws.json.writeValue(@TypeOf(input.report_plan_name), input.report_plan_name, alloc, &body_buf);
    has_prev = true;
    if (input.report_plan_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ReportPlanTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReportSetting\":");
    try aws.json.writeValue(@TypeOf(input.report_setting), input.report_setting, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReportPlanOutput {
    var result: CreateReportPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateReportPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
