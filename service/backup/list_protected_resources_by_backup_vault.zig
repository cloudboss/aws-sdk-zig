...
pub const ListProtectedResourcesByBackupVaultInput = struct {
...
pub const ListProtectedResourcesByBackupVaultOutput = struct {
    /// The next item following a partial list of returned items. For example, if a
    /// request is
    /// made to return `MaxResults` number of items, `NextToken` allows you
    /// to return more items in your list starting at the location pointed to by the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProtectedResourcesByBackupVaultInput, options: CallOptions) !ListProtectedResourcesByBackupVaultOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProtectedResourcesByBackupVaultInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListProtectedResourcesByBackupVaultOutput {
    var result: ListProtectedResourcesByBackupVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProtectedResourcesByBackupVaultOutput, body, allocator);
    }
    _ = status;
    _ = headers;
