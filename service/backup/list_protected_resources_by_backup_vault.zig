...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProtectedResourcesByBackupVaultInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProtectedResourcesByBackupVaultOutput {
    var result: ListProtectedResourcesByBackupVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProtectedResourcesByBackupVaultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
