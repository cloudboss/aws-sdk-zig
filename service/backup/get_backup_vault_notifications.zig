...
pub const GetBackupVaultNotificationsInput = struct {
...
pub const GetBackupVaultNotificationsOutput = struct {
    ...
    /// the backup
    /// vault.
    backup_vault_events: ?[]const BackupVaultEvent = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackupVaultNotificationsInput, options: CallOptions) !GetBackupVaultNotificationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBackupVaultNotificationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBackupVaultNotificationsOutput {
    var result: GetBackupVaultNotificationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackupVaultNotificationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
