...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBackupVaultNotificationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBackupVaultNotificationsOutput {
    var result: GetBackupVaultNotificationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackupVaultNotificationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
