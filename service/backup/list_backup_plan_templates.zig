...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBackupPlanTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBackupPlanTemplatesOutput {
    var result: ListBackupPlanTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBackupPlanTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
