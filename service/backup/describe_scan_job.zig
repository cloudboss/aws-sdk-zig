...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeScanJobInput, options: Options) !DescribeScanJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScanJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/scan/jobs/");
    try path_buf.appendSlice(alloc, input.scan_job_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeScanJobOutput {
    var result: DescribeScanJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeScanJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
