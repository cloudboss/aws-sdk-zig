...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBackupJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBackupJobOutput {
    var result: DescribeBackupJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBackupJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
