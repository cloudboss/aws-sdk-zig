...
const RestoreAccessBackupVaultListMember = @import("restore_access_backup_vault_list_member.zig").RestoreAccessBackupVaultListMember;

pub const ListRestoreAccessBackupVaultsInput = struct {
...
pub const ListRestoreAccessBackupVaultsOutput = struct {
    ...
    /// vault.
    restore_access_backup_vaults: ?[]const RestoreAccessBackupVaultListMember = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .restore_access_backup_vaults = "RestoreAccessBackupVaults",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRestoreAccessBackupVaultsInput, options: CallOptions) !ListRestoreAccessBackupVaultsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRestoreAccessBackupVaultsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRestoreAccessBackupVaultsOutput {
    var result: ListRestoreAccessBackupVaultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRestoreAccessBackupVaultsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
