...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReportPlansInput, options: Options) !ListReportPlansOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReportPlansInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReportPlansOutput {
    var result: ListReportPlansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReportPlansOutput, body, alloc);
    }
    _ = status;
    _ = headers;
