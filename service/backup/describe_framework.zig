...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFrameworkInput, options: Options) !DescribeFrameworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFrameworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFrameworkOutput {
    var result: DescribeFrameworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFrameworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
