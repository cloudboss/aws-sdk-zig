...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCopyJobsInput, options: Options) !ListCopyJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCopyJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.by_destination_vault_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "destinationVaultArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.by_source_recovery_point_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sourceRecoveryPointArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.by_state) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCopyJobsOutput {
    var result: ListCopyJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCopyJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
