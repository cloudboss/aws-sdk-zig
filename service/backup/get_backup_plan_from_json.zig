...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackupPlanFromJSONInput, options: CallOptions) !GetBackupPlanFromJSONOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBackupPlanFromJSONInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"BackupPlanTemplateJson\":");
    try aws.json.writeValue(@TypeOf(input.backup_plan_template_json), input.backup_plan_template_json, allocator, &body_buf);
    has_prev = true;
