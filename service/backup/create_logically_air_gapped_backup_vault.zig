...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLogicallyAirGappedBackupVaultInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/logically-air-gapped-backup-vaults/");
    ...
    if (input.backup_vault_tags) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"BackupVaultTags\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MaxRetentionDays\":");
    try aws.json.writeValue(@TypeOf(input.max_retention_days), input.max_retention_days, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MinRetentionDays\":");
    try aws.json.writeValue(@TypeOf(input.min_retention_days), input.min_retention_days, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLogicallyAirGappedBackupVaultOutput {
    var result: CreateLogicallyAirGappedBackupVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLogicallyAirGappedBackupVaultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
