...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackupPlanInput, options: CallOptions) !GetBackupPlanOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBackupPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.max_scheduled_runs_preview) |v| {
        ...
        try query_buf.appendSlice(allocator, "MaxScheduledRunsPreview=");
        {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBackupPlanOutput {
    var result: GetBackupPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackupPlanOutput, body, allocator);
    }
    _ = status;
    _ = headers;
