...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReportJobsInput, options: Options) !ListReportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.by_creation_after) |v| {
        ...
        try query_buf.appendSlice(alloc, "CreationAfter=");
        {
    ...
    if (input.by_creation_before) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "CreationBefore=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.by_report_plan_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ReportPlanName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReportJobsOutput {
    var result: ListReportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReportJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
