...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBackupPlanInput, options: Options) !DeleteBackupPlanOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBackupPlanOutput {
    var result: DeleteBackupPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBackupPlanOutput, body, allocator);
    }
    _ = status;
    _ = headers;
