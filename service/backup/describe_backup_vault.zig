...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBackupVaultInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBackupVaultOutput {
    var result: DescribeBackupVaultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBackupVaultOutput, body, alloc);
    }
    _ = status;
    _ = headers;
