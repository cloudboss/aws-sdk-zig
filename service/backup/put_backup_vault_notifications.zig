...
const PutBackupVaultNotificationsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBackupVaultNotificationsInput, options: Options) !PutBackupVaultNotificationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBackupVaultNotificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BackupVaultEvents\":");
    try aws.json.writeValue(@TypeOf(input.backup_vault_events), input.backup_vault_events, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SNSTopicArn\":");
    try aws.json.writeValue(@TypeOf(input.sns_topic_arn), input.sns_topic_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutBackupVaultNotificationsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
