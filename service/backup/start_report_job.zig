...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartReportJobInput, options: CallOptions) !StartReportJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartReportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartReportJobOutput {
    var result: StartReportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReportJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
