...
pub const StartReportJobInput = struct {
...
pub const StartReportJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartReportJobInput, options: Options) !StartReportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartReportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/audit/report-jobs/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartReportJobOutput {
    var result: StartReportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
