...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRestoreTestingPlanInput, options: Options) !CreateRestoreTestingPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRestoreTestingPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RestoreTestingPlan\":");
    try aws.json.writeValue(@TypeOf(input.restore_testing_plan), input.restore_testing_plan, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRestoreTestingPlanOutput {
    var result: CreateRestoreTestingPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRestoreTestingPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
