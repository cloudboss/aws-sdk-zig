...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBackupSelectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BackupSelection\":");
    try aws.json.writeValue(@TypeOf(input.backup_selection), input.backup_selection, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBackupSelectionOutput {
    var result: CreateBackupSelectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBackupSelectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
