...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFrameworksInput, options: Options) !ListFrameworksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFrameworksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFrameworksOutput {
    var result: ListFrameworksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFrameworksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
