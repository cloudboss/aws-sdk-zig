...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRestoreJobsInput, options: CallOptions) !ListRestoreJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRestoreJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.by_restore_testing_plan_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "restoreTestingPlanArn=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
