...
pub const ListRestoreJobsInput = struct {
    ...
    by_restore_testing_plan_arn: ?[]const u8 = null,
    ...
    pub const json_field_names = .{
        ...
        .by_restore_testing_plan_arn = "ByRestoreTestingPlanArn",
        .by_status = "ByStatus",
...
pub const ListRestoreJobsOutput = struct {
    /// The next item following a partial list of returned items. For example, if a
    /// request is
    /// made to return `MaxResults` number of items, `NextToken` allows you
    /// to return more items in your list starting at the location pointed to by the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRestoreJobsInput, options: Options) !ListRestoreJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRestoreJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.by_restore_testing_plan_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "restoreTestingPlanArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRestoreJobsOutput {
    var result: ListRestoreJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRestoreJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
