...
const AssociateBackupVaultMpaApprovalTeamOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateBackupVaultMpaApprovalTeamInput, options: Options) !AssociateBackupVaultMpaApprovalTeamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateBackupVaultMpaApprovalTeamInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/mpaApprovalTeam");
    ...
    try body_buf.appendSlice(alloc, "\"MpaApprovalTeamArn\":");
    try aws.json.writeValue(@TypeOf(input.mpa_approval_team_arn), input.mpa_approval_team_arn, alloc, &body_buf);
    has_prev = true;
    if (input.requester_comment) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RequesterComment\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateBackupVaultMpaApprovalTeamOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
