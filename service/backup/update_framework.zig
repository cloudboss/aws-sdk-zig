...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFrameworkInput, options: Options) !UpdateFrameworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFrameworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFrameworkOutput {
    var result: UpdateFrameworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFrameworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
