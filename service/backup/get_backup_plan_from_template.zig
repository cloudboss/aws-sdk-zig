...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBackupPlanFromTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/backup/template/plans/");
    try path_buf.appendSlice(alloc, input.backup_plan_template_id);
    try path_buf.appendSlice(alloc, "/toPlan");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBackupPlanFromTemplateOutput {
    var result: GetBackupPlanFromTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackupPlanFromTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
