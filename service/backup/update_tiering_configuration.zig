...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTieringConfigurationInput, options: Options) !UpdateTieringConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTieringConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/tiering-configurations/");
    try path_buf.appendSlice(alloc, input.tiering_configuration_name);
    ...
    try body_buf.appendSlice(alloc, "\"TieringConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.tiering_configuration), input.tiering_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTieringConfigurationOutput {
    var result: UpdateTieringConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTieringConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
