...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCopyJobInput, options: Options) !DescribeCopyJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCopyJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/copy-jobs/");
    try path_buf.appendSlice(alloc, input.copy_job_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCopyJobOutput {
    var result: DescribeCopyJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCopyJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
