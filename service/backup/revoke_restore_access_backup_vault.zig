...
const RevokeRestoreAccessBackupVaultOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeRestoreAccessBackupVaultInput, options: Options) !RevokeRestoreAccessBackupVaultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RevokeRestoreAccessBackupVaultInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/restore-access-backup-vaults/");
    try path_buf.appendSlice(alloc, input.restore_access_backup_vault_arn);
    ...
    if (input.requester_comment) |v| {
        ...
        try query_buf.appendSlice(alloc, "requesterComment=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RevokeRestoreAccessBackupVaultOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
