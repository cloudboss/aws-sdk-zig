...
const BackupSelectionsListMember = @import("backup_selections_list_member.zig").BackupSelectionsListMember;

pub const ListBackupSelectionsInput = struct {
...
pub const ListBackupSelectionsOutput = struct {
    /// An array of backup selection list items containing metadata about each
    /// resource in the
    /// list.
    backup_selections_list: ?[]const BackupSelectionsListMember = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBackupSelectionsInput, options: Options) !ListBackupSelectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBackupSelectionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBackupSelectionsOutput {
    var result: ListBackupSelectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBackupSelectionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
