...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProtectedResourceInput, options: Options) !DescribeProtectedResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProtectedResourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProtectedResourceOutput {
    var result: DescribeProtectedResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeProtectedResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
