...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGlobalSettingsInput, options: Options) !DescribeGlobalSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGlobalSettingsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGlobalSettingsOutput {
    var result: DescribeGlobalSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeGlobalSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
