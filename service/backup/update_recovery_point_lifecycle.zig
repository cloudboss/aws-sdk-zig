...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRecoveryPointLifecycleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRecoveryPointLifecycleOutput {
    var result: UpdateRecoveryPointLifecycleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRecoveryPointLifecycleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
