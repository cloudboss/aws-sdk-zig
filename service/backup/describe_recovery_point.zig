...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRecoveryPointInput, options: Options) !DescribeRecoveryPointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRecoveryPointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRecoveryPointOutput {
    var result: DescribeRecoveryPointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRecoveryPointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
