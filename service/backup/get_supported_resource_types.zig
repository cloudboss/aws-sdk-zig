...
const GetSupportedResourceTypesInput = struct {};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSupportedResourceTypesInput, options: Options) !GetSupportedResourceTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSupportedResourceTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSupportedResourceTypesOutput {
    var result: GetSupportedResourceTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSupportedResourceTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
