...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEndpointGroupInput, options: Options) !UpdateEndpointGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEndpointGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.UpdateEndpointGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEndpointGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateEndpointGroupOutput, body, alloc);
}
