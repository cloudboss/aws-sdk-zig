...
pub const DescribeAcceleratorInput = struct {
...
pub const DescribeAcceleratorOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAcceleratorInput, options: CallOptions) !DescribeAcceleratorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAcceleratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.DescribeAccelerator");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAcceleratorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAcceleratorOutput, body, allocator);
}
