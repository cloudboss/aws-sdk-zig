...
pub const AddEndpointsInput = struct {
    /// The list of endpoint objects.
    endpoint_configurations: []const EndpointConfiguration,
...
pub const AddEndpointsOutput = struct {
    /// The list of endpoint objects.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddEndpointsInput, options: CallOptions) !AddEndpointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AddEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.AddEndpoints");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AddEndpointsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AddEndpointsOutput, body, allocator);
}
