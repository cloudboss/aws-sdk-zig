...
fn serializeRequest(allocator: std.mem.Allocator, input: ListListenersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.ListListeners");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListListenersOutput {
    ...
    return aws.json.parseJsonObject(ListListenersOutput, body, allocator);
}
