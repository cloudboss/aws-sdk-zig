...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCustomRoutingAcceleratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.UpdateCustomRoutingAccelerator");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCustomRoutingAcceleratorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCustomRoutingAcceleratorOutput, body, alloc);
}
