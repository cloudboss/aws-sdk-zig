...
const RemoveEndpointsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveEndpointsInput, options: Options) !RemoveEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.RemoveEndpoints");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveEndpointsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
