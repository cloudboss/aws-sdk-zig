...
const CustomRoutingEndpointConfiguration = @import("custom_routing_endpoint_configuration.zig").CustomRoutingEndpointConfiguration;
const CustomRoutingEndpointDescription = @import("custom_routing_endpoint_description.zig").CustomRoutingEndpointDescription;

pub const AddCustomRoutingEndpointsInput = struct {
    /// The list of endpoint objects to add to a custom routing accelerator.
    endpoint_configurations: []const CustomRoutingEndpointConfiguration,
...
pub const AddCustomRoutingEndpointsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddCustomRoutingEndpointsInput, options: CallOptions) !AddCustomRoutingEndpointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AddCustomRoutingEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.AddCustomRoutingEndpoints");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AddCustomRoutingEndpointsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AddCustomRoutingEndpointsOutput, body, allocator);
}
