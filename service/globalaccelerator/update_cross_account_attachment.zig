...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCrossAccountAttachmentInput, options: Options) !UpdateCrossAccountAttachmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCrossAccountAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.UpdateCrossAccountAttachment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCrossAccountAttachmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCrossAccountAttachmentOutput, body, alloc);
}
