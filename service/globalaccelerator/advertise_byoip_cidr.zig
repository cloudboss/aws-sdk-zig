...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AdvertiseByoipCidrInput, options: CallOptions) !AdvertiseByoipCidrOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AdvertiseByoipCidrInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.AdvertiseByoipCidr");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AdvertiseByoipCidrOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AdvertiseByoipCidrOutput, body, allocator);
}
