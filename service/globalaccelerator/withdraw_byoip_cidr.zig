...
pub const WithdrawByoipCidrOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: WithdrawByoipCidrInput, options: CallOptions) !WithdrawByoipCidrOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: WithdrawByoipCidrInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.WithdrawByoipCidr");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !WithdrawByoipCidrOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(WithdrawByoipCidrOutput, body, allocator);
}
