...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCustomRoutingAcceleratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.DeleteCustomRoutingAccelerator");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCustomRoutingAcceleratorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
