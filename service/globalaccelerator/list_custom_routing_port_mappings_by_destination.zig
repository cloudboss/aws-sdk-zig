...
const DestinationPortMapping = @import("destination_port_mapping.zig").DestinationPortMapping;

pub const ListCustomRoutingPortMappingsByDestinationInput = struct {
    /// The endpoint IP address in a virtual private cloud (VPC) subnet for which
    /// you want to receive back port
    /// mappings.
...
pub const ListCustomRoutingPortMappingsByDestinationOutput = struct {
    /// The port mappings for the endpoint IP address that you specified in the
    /// request.
    destination_port_mappings: ?[]const DestinationPortMapping = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomRoutingPortMappingsByDestinationInput, options: CallOptions) !ListCustomRoutingPortMappingsByDestinationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomRoutingPortMappingsByDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.ListCustomRoutingPortMappingsByDestination");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomRoutingPortMappingsByDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCustomRoutingPortMappingsByDestinationOutput, body, allocator);
}
