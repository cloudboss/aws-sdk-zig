...
pub const DeleteListenerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteListenerInput, options: CallOptions) !DeleteListenerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteListenerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.DeleteListener");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteListenerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
