...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListByoipCidrsInput, options: Options) !ListByoipCidrsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListByoipCidrsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.ListByoipCidrs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListByoipCidrsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListByoipCidrsOutput, body, alloc);
}
