...
pub const DescribeListenerInput = struct {
...
pub const DescribeListenerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeListenerInput, options: CallOptions) !DescribeListenerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeListenerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.DescribeListener");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeListenerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeListenerOutput, body, allocator);
}
