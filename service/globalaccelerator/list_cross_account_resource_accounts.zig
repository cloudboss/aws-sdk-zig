...
pub const ListCrossAccountResourceAccountsOutput = struct {
    ...
    /// attachment who can work
    /// with resources listed in the same attachment.
    resource_owner_aws_account_ids: ?[]const []const u8 = null,

    pub const json_field_names = .{
        .resource_owner_aws_account_ids = "ResourceOwnerAwsAccountIds",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCrossAccountResourceAccountsInput, options: CallOptions) !ListCrossAccountResourceAccountsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCrossAccountResourceAccountsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.ListCrossAccountResourceAccounts");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCrossAccountResourceAccountsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCrossAccountResourceAccountsOutput, body, allocator);
}
