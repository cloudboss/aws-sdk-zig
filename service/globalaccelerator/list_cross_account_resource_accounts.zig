...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCrossAccountResourceAccountsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.ListCrossAccountResourceAccounts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCrossAccountResourceAccountsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCrossAccountResourceAccountsOutput, body, alloc);
}
