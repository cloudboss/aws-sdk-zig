...
pub const ListCustomRoutingAcceleratorsInput = struct {
...
pub const ListCustomRoutingAcceleratorsOutput = struct {
    /// The list of custom routing accelerators for a customer account.
    accelerators: ?[]const CustomRoutingAccelerator = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomRoutingAcceleratorsInput, options: CallOptions) !ListCustomRoutingAcceleratorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomRoutingAcceleratorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.ListCustomRoutingAccelerators");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomRoutingAcceleratorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCustomRoutingAcceleratorsOutput, body, allocator);
}
