...
pub const ListCustomRoutingListenersInput = struct {
...
pub const ListCustomRoutingListenersOutput = struct {
    /// The list of listeners for a custom routing accelerator.
    listeners: ?[]const CustomRoutingListener = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomRoutingListenersInput, options: CallOptions) !ListCustomRoutingListenersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomRoutingListenersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.ListCustomRoutingListeners");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomRoutingListenersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCustomRoutingListenersOutput, body, allocator);
}
