...
const AcceleratorAttributes = @import("accelerator_attributes.zig").AcceleratorAttributes;

pub const UpdateAcceleratorAttributesInput = struct {
    ...
    /// folder structure will include a double slash (//),
    /// like the following:
    ///
...
pub const UpdateAcceleratorAttributesOutput = struct {
    /// Updated attributes for the accelerator.
    accelerator_attributes: ?AcceleratorAttributes = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAcceleratorAttributesInput, options: CallOptions) !UpdateAcceleratorAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAcceleratorAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.UpdateAcceleratorAttributes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAcceleratorAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateAcceleratorAttributesOutput, body, allocator);
}
