...
pub const DeleteAcceleratorInput = struct {
...
pub const DeleteAcceleratorOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAcceleratorInput, options: CallOptions) !DeleteAcceleratorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAcceleratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.DeleteAccelerator");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAcceleratorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
