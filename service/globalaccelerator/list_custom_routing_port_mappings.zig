...
pub const ListCustomRoutingPortMappingsInput = struct {
...
pub const ListCustomRoutingPortMappingsOutput = struct {
    /// The token for the next set of results. You receive this token from a
    /// previous call.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomRoutingPortMappingsInput, options: CallOptions) !ListCustomRoutingPortMappingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomRoutingPortMappingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.ListCustomRoutingPortMappings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomRoutingPortMappingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCustomRoutingPortMappingsOutput, body, allocator);
}
