...
pub const AllowCustomRoutingTrafficInput = struct {
    /// Indicates whether all destination IP addresses and ports for a specified VPC
    /// subnet endpoint can receive traffic
    /// from a custom routing accelerator. The value is TRUE or FALSE.
    ///
    /// When set to TRUE, *all* destinations in the custom routing VPC subnet can
    ...
    /// destination IP addresses that are allowed
    /// to receive traffic. A list of ports is optional. If you don't specify a list
    /// of ports, the ports that can accept traffic is
    /// the same as the ports configured for the endpoint group.
    ///
    /// The default value is FALSE.
    allow_all_traffic_to_endpoint: ?bool = null,

    /// A list of specific Amazon EC2 instance IP addresses (destination addresses)
    /// in a subnet that you want to allow to receive
    /// traffic. The IP addresses must be a subset of the IP addresses that you
    /// specified for the endpoint group.
    ///
    /// `DestinationAddresses` is required if `AllowAllTrafficToEndpoint` is `FALSE`
    /// or is
    /// not specified.
...
pub const AllowCustomRoutingTrafficOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AllowCustomRoutingTrafficInput, options: CallOptions) !AllowCustomRoutingTrafficOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "globalaccelerator");
...
fn serializeRequest(allocator: std.mem.Allocator, input: AllowCustomRoutingTrafficInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.AllowCustomRoutingTraffic");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AllowCustomRoutingTrafficOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
