...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAcceleratorInput, options: Options) !CreateAcceleratorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAcceleratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.CreateAccelerator");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAcceleratorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAcceleratorOutput, body, alloc);
}
