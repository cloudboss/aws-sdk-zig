...
pub const ListAcceleratorsInput = struct {
...
pub const ListAcceleratorsOutput = struct {
    /// The list of accelerators for a customer account.
    accelerators: ?[]const Accelerator = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAcceleratorsInput, options: CallOptions) !ListAcceleratorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAcceleratorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.ListAccelerators");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAcceleratorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAcceleratorsOutput, body, allocator);
}
