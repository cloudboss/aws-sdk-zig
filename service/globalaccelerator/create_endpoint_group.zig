...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEndpointGroupInput, options: CallOptions) !CreateEndpointGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateEndpointGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.CreateEndpointGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateEndpointGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEndpointGroupOutput, body, allocator);
}
