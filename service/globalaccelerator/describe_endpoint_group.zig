...
pub const DescribeEndpointGroupInput = struct {
...
pub const DescribeEndpointGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEndpointGroupInput, options: CallOptions) !DescribeEndpointGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEndpointGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.DescribeEndpointGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeEndpointGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEndpointGroupOutput, body, allocator);
}
