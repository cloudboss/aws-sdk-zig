...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomRoutingEndpointGroupInput, options: Options) !CreateCustomRoutingEndpointGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomRoutingEndpointGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.CreateCustomRoutingEndpointGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomRoutingEndpointGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCustomRoutingEndpointGroupOutput, body, alloc);
}
