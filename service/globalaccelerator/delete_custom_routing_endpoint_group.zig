...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCustomRoutingEndpointGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GlobalAccelerator_V20180706.DeleteCustomRoutingEndpointGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCustomRoutingEndpointGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
