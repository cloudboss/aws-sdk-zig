...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCustomRoutingEndpointGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.DeleteCustomRoutingEndpointGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCustomRoutingEndpointGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
