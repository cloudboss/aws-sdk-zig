...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCustomRoutingListenerInput, options: Options) !UpdateCustomRoutingListenerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCustomRoutingListenerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.UpdateCustomRoutingListener");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCustomRoutingListenerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCustomRoutingListenerOutput, body, alloc);
}
