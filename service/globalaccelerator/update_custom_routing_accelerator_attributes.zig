...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCustomRoutingAcceleratorAttributesInput, options: Options) !UpdateCustomRoutingAcceleratorAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCustomRoutingAcceleratorAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GlobalAccelerator_V20180706.UpdateCustomRoutingAcceleratorAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCustomRoutingAcceleratorAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCustomRoutingAcceleratorAttributesOutput, body, alloc);
}
