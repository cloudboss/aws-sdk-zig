...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProfileResourceAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/profileresourceassociations/profileid/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProfileResourceAssociationsOutput {
    var result: ListProfileResourceAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProfileResourceAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
