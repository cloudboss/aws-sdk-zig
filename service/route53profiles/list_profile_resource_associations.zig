...
pub const ListProfileResourceAssociationsInput = struct {
...
pub const ListProfileResourceAssociationsOutput = struct {
    ...
    /// In the next request, specify the value of `NextToken` from the previous
    /// response.
    next_token: ?[]const u8 = null,

    /// Information about the profile resource association that you specified in a
    /// `GetProfileResourceAssociation` request.
    profile_resource_associations: ?[]const ProfileResourceAssociation = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .profile_resource_associations = "ProfileResourceAssociations",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProfileResourceAssociationsInput, options: CallOptions) !ListProfileResourceAssociationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProfileResourceAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/profileresourceassociations/profileid/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListProfileResourceAssociationsOutput {
    var result: ListProfileResourceAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProfileResourceAssociationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
