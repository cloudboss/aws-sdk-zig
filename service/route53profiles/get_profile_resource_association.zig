...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProfileResourceAssociationInput, options: CallOptions) !GetProfileResourceAssociationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetProfileResourceAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetProfileResourceAssociationOutput {
    var result: GetProfileResourceAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProfileResourceAssociationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
