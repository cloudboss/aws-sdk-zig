...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateResourceToProfileInput, options: Options) !AssociateResourceToProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateResourceToProfileInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("route53profiles", "Route53Profiles", alloc);
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ProfileId\":");
    try aws.json.writeValue(@TypeOf(input.profile_id), input.profile_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.resource_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ResourceProperties\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateResourceToProfileOutput {
    var result: AssociateResourceToProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateResourceToProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
