...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProfileResourceAssociationInput, options: Options) !UpdateProfileResourceAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProfileResourceAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProfileResourceAssociationOutput {
    var result: UpdateProfileResourceAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProfileResourceAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
