...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProfileAssociationsInput, options: Options) !ListProfileAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProfileAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.profile_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "profileId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProfileAssociationsOutput {
    var result: ListProfileAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProfileAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
