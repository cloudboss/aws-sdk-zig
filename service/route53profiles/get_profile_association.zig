...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProfileAssociationInput, options: Options) !GetProfileAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProfileAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/profileassociation/");
    try path_buf.appendSlice(alloc, input.profile_association_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProfileAssociationOutput {
    var result: GetProfileAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProfileAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
