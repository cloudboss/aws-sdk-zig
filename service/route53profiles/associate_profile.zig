...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateProfileInput, options: Options) !AssociateProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.resource_id), input.resource_id, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateProfileOutput {
    var result: AssociateProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
