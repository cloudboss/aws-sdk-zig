...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCodeReviewInput, options: Options) !CreateCodeReviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCodeReviewInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RepositoryAssociationArn\":");
    try aws.json.writeValue(@TypeOf(input.repository_association_arn), input.repository_association_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCodeReviewOutput {
    var result: CreateCodeReviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCodeReviewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
