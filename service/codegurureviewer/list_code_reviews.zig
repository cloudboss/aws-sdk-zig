...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCodeReviewsInput, options: Options) !ListCodeReviewsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCodeReviewsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.provider_types) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ProviderTypes=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.repository_names) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "RepositoryNames=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.states) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCodeReviewsOutput {
    var result: ListCodeReviewsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCodeReviewsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
