...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateRepositoryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.kms_key_details) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"KMSKeyDetails\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Repository\":");
    try aws.json.writeValue(@TypeOf(input.repository), input.repository, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateRepositoryOutput {
    var result: AssociateRepositoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateRepositoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
