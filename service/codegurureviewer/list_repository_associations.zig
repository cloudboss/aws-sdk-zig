...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRepositoryAssociationsInput, options: Options) !ListRepositoryAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRepositoryAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.owners) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "Owner=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.provider_types) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "ProviderType=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.states) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRepositoryAssociationsOutput {
    var result: ListRepositoryAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRepositoryAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
