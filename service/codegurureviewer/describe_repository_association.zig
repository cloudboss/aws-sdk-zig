...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRepositoryAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRepositoryAssociationOutput {
    var result: DescribeRepositoryAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRepositoryAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
