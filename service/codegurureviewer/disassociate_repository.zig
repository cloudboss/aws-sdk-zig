...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateRepositoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.association_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateRepositoryOutput {
    var result: DisassociateRepositoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateRepositoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
