...
pub const StartDeploymentInput = struct {
    ...
    /// `StartDeployment` without `CreateDeployment`. The source URL
    /// can be either an HTTP GET URL that is publicly accessible and downloads a
    /// single .zip
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/deployments/start");
    ...
    if (input.source_url) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sourceUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.source_url_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sourceUrlType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
