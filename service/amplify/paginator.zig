...
pub const ListDomainAssociationsPaginator = struct {
    client: *Client,
    params: list_domain_associations.ListDomainAssociationsInput,
    next_token: ?[]const u8 = null,
    done: bool = false,
    allocator: std.mem.Allocator,
    ...
    pub fn next(self: *Self, allocator: std.mem.Allocator, options: CallOptions) !list_domain_associations.ListDomainAssociationsOutput {
        if (self.done) {
            return error.EndOfPagination;
        }
        ...
        const output = try list_domain_associations.execute(self.client, allocator, self.params, options);
