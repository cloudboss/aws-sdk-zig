...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWebhooksInput, options: Options) !ListWebhooksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWebhooksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWebhooksOutput {
    var result: ListWebhooksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWebhooksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
