...
pub const CreateBackendEnvironmentInput = struct {
...
pub const CreateBackendEnvironmentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBackendEnvironmentInput, options: CallOptions) !CreateBackendEnvironmentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateBackendEnvironmentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.deployment_artifacts) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"deploymentArtifacts\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"environmentName\":");
    try aws.json.writeValue(@TypeOf(input.environment_name), input.environment_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateBackendEnvironmentOutput {
    var result: CreateBackendEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBackendEnvironmentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
