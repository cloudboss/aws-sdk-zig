...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDomainAssociationInput, options: Options) !GetDomainAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainAssociationOutput {
    var result: GetDomainAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
