...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDomainAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDomainAssociationOutput {
    var result: DeleteDomainAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDomainAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
