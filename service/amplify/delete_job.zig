...
pub const DeleteJobInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteJobInput, options: Options) !DeleteJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteJobOutput {
    var result: DeleteJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
