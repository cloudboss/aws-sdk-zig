...
pub const CreateAppInput = struct {
    ...
    /// Bitbucket or CodeCommit. To authorize access to GitHub as your repository
    /// provider, use
    /// `accessToken`.
    ///
    ...
    /// settings. For an example of the build specification settings, see [Amplify
    /// build settings for a Next.js 14 SSG
    /// application](https://docs.aws.amazon.com/amplify/latest/userguide/deploy-nextjs-app.html#build-setting-detection-ssg-14) in the
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAppOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAppOutput, body, allocator);
    }
    _ = status;
    _ = headers;
