...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBackendEnvironmentInput, options: Options) !GetBackendEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBackendEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBackendEnvironmentOutput {
    var result: GetBackendEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBackendEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
