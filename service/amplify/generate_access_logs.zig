...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateAccessLogsInput, options: Options) !GenerateAccessLogsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateAccessLogsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/accesslogs");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateAccessLogsOutput {
    var result: GenerateAccessLogsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateAccessLogsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
