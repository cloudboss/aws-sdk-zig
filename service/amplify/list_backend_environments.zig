...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBackendEnvironmentsInput, options: Options) !ListBackendEnvironmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBackendEnvironmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/backendenvironments");
    ...
    if (input.environment_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "environmentName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBackendEnvironmentsOutput {
    var result: ListBackendEnvironmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBackendEnvironmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
