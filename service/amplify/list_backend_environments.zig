...
pub const ListBackendEnvironmentsInput = struct {
...
pub const ListBackendEnvironmentsOutput = struct {
    /// The list of backend environments for an Amplify app.
    backend_environments: ?[]const BackendEnvironment = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBackendEnvironmentsInput, options: CallOptions) !ListBackendEnvironmentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBackendEnvironmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.environment_name) |v| {
        ...
        try query_buf.appendSlice(allocator, "environmentName=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBackendEnvironmentsOutput {
    var result: ListBackendEnvironmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBackendEnvironmentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
