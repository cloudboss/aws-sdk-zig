...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDomainAssociationInput, options: Options) !CreateDomainAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDomainAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.sub_domain_settings), input.sub_domain_settings, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDomainAssociationOutput {
    var result: CreateDomainAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDomainAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
