...
pub const DeleteBackendEnvironmentInput = struct {
...
pub const DeleteBackendEnvironmentOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBackendEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBackendEnvironmentOutput {
    var result: DeleteBackendEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBackendEnvironmentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
