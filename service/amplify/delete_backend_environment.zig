...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBackendEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBackendEnvironmentOutput {
    var result: DeleteBackendEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBackendEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
