...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWebhookInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWebhookOutput {
    var result: UpdateWebhookOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateWebhookOutput, body, alloc);
    }
    _ = status;
    _ = headers;
