...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSubChannelsInput, options: Options) !ListSubChannelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSubChannelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/subchannels");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSubChannelsOutput {
    var result: ListSubChannelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSubChannelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
