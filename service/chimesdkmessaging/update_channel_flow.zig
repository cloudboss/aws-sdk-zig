...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateChannelFlowInput, options: Options) !UpdateChannelFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChannelFlowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChannelFlowOutput {
    var result: UpdateChannelFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChannelFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
