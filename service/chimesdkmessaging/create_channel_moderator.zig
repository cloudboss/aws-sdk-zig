...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChannelModeratorInput, options: Options) !CreateChannelModeratorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateChannelModeratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/moderators");
    ...
    try body_buf.appendSlice(alloc, "\"ChannelModeratorArn\":");
    try aws.json.writeValue(@TypeOf(input.channel_moderator_arn), input.channel_moderator_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateChannelModeratorOutput {
    var result: CreateChannelModeratorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChannelModeratorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
