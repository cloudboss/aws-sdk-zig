...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchChannelsInput, options: Options) !SearchChannelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchChannelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=search");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchChannelsOutput {
    var result: SearchChannelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchChannelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
