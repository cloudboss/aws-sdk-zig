...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateChannelMessageInput, options: Options) !UpdateChannelMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChannelMessageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChannelMessageOutput {
    var result: UpdateChannelMessageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChannelMessageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
