...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChannelBanInput, options: Options) !CreateChannelBanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateChannelBanInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateChannelBanOutput {
    var result: CreateChannelBanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChannelBanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
