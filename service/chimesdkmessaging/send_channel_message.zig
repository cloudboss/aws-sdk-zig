...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendChannelMessageInput, options: Options) !SendChannelMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendChannelMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.message_attributes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MessageAttributes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.metadata) |v| {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Persistence\":");
    try aws.json.writeValue(@TypeOf(input.persistence), input.persistence, alloc, &body_buf);
    has_prev = true;
    if (input.push_notification) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PushNotification\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendChannelMessageOutput {
    var result: SendChannelMessageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendChannelMessageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
