...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChannelMessageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChannelMessageOutput {
    var result: GetChannelMessageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChannelMessageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
