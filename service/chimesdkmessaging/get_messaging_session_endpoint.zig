...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMessagingSessionEndpointInput, options: Options) !GetMessagingSessionEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMessagingSessionEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.network_type) |v| {
        ...
        try query_buf.appendSlice(alloc, "network-type=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMessagingSessionEndpointOutput {
    var result: GetMessagingSessionEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMessagingSessionEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
