...
pub const CreateChannelFlowInput = struct {
...
pub const CreateChannelFlowOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChannelFlowInput, options: Options) !CreateChannelFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateChannelFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AppInstanceArn\":");
    try aws.json.writeValue(@TypeOf(input.app_instance_arn), input.app_instance_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Processors\":");
    try aws.json.writeValue(@TypeOf(input.processors), input.processors, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateChannelFlowOutput {
    var result: CreateChannelFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChannelFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
