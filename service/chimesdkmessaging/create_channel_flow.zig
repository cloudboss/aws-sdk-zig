...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChannelFlowInput, options: CallOptions) !CreateChannelFlowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateChannelFlowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateChannelFlowOutput {
    var result: CreateChannelFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChannelFlowOutput, body, allocator);
    }
    _ = status;
    _ = headers;
