...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChannelReadMarkerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/readMarker");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChannelReadMarkerOutput {
    var result: UpdateChannelReadMarkerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChannelReadMarkerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
