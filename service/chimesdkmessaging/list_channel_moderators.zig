...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelModeratorsInput, options: Options) !ListChannelModeratorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChannelModeratorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChannelModeratorsOutput {
    var result: ListChannelModeratorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelModeratorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
