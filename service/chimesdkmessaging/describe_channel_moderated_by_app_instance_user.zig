...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeChannelModeratedByAppInstanceUserInput, options: Options) !DescribeChannelModeratedByAppInstanceUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeChannelModeratedByAppInstanceUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "scope=app-instance-user-moderated-channel");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeChannelModeratedByAppInstanceUserOutput {
    var result: DescribeChannelModeratedByAppInstanceUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeChannelModeratedByAppInstanceUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
