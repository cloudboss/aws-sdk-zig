...
const ChannelFlowSummary = @import("channel_flow_summary.zig").ChannelFlowSummary;

pub const ListChannelFlowsInput = struct {
...
pub const ListChannelFlowsOutput = struct {
    /// The information about each channel flow.
    channel_flows: ?[]const ChannelFlowSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelFlowsInput, options: Options) !ListChannelFlowsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChannelFlowsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChannelFlowsOutput {
    var result: ListChannelFlowsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelFlowsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
