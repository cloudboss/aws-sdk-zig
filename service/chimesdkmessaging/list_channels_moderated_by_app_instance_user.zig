...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelsModeratedByAppInstanceUserInput, options: CallOptions) !ListChannelsModeratedByAppInstanceUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListChannelsModeratedByAppInstanceUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "scope=app-instance-user-moderated-channels");
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListChannelsModeratedByAppInstanceUserOutput {
    var result: ListChannelsModeratedByAppInstanceUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelsModeratedByAppInstanceUserOutput, body, allocator);
    }
    _ = status;
    _ = headers;
