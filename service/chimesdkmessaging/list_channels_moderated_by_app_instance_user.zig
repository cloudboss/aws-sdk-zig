...
pub const ListChannelsModeratedByAppInstanceUserInput = struct {
...
pub const ListChannelsModeratedByAppInstanceUserOutput = struct {
    /// The moderated channels in the request.
    channels: ?[]const ChannelModeratedByAppInstanceUserSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelsModeratedByAppInstanceUserInput, options: Options) !ListChannelsModeratedByAppInstanceUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChannelsModeratedByAppInstanceUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "scope=app-instance-user-moderated-channels");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChannelsModeratedByAppInstanceUserOutput {
    var result: ListChannelsModeratedByAppInstanceUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelsModeratedByAppInstanceUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
