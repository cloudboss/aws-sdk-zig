...
pub const DescribeChannelMembershipForAppInstanceUserInput = struct {
...
pub const DescribeChannelMembershipForAppInstanceUserOutput = struct {
    /// The channel to which a user belongs.
    channel_membership: ?ChannelMembershipForAppInstanceUserSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeChannelMembershipForAppInstanceUserInput, options: Options) !DescribeChannelMembershipForAppInstanceUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeChannelMembershipForAppInstanceUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "scope=app-instance-user-membership");
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "app-instance-user-arn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.app_instance_user_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeChannelMembershipForAppInstanceUserOutput {
    var result: DescribeChannelMembershipForAppInstanceUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeChannelMembershipForAppInstanceUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
