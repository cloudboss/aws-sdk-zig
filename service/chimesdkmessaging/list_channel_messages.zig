...
const ChannelMessageSummary = @import("channel_message_summary.zig").ChannelMessageSummary;

pub const ListChannelMessagesInput = struct {
...
pub const ListChannelMessagesOutput = struct {
    /// The ARN of the channel containing the requested messages.
    channel_arn: ?[]const u8 = null,

    /// The information about, and content of, each requested message.
    channel_messages: ?[]const ChannelMessageSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelMessagesInput, options: Options) !ListChannelMessagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChannelMessagesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.not_after) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "not-after=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.not_before) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "not-before=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.sort_order) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sort-order=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.sub_channel_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sub-channel-id=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChannelMessagesOutput {
    var result: ListChannelMessagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelMessagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
