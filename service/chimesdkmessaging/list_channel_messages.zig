...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelMessagesInput, options: CallOptions) !ListChannelMessagesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListChannelMessagesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.not_after) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "not-after=");
        {
            const num_str = std.fmt.allocPrint(allocator, "{d}", .{v}) catch "";
            try query_buf.appendSlice(allocator, num_str);
        }
    ...
    if (input.not_before) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "not-before=");
        {
            const num_str = std.fmt.allocPrint(allocator, "{d}", .{v}) catch "";
            try query_buf.appendSlice(allocator, num_str);
        }
    ...
    if (input.sort_order) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "sort-order=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListChannelMessagesOutput {
    var result: ListChannelMessagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelMessagesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
