...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutChannelExpirationSettingsInput, options: Options) !PutChannelExpirationSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutChannelExpirationSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.chime_bearer) |v| {
        try request.headers.put(alloc, "x-amz-chime-bearer", v);
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutChannelExpirationSettingsOutput {
    var result: PutChannelExpirationSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutChannelExpirationSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
