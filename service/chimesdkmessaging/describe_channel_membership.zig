...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeChannelMembershipInput, options: Options) !DescribeChannelMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeChannelMembershipInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeChannelMembershipOutput {
    var result: DescribeChannelMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeChannelMembershipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
