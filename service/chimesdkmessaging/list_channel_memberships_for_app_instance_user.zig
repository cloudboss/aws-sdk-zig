...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelMembershipsForAppInstanceUserInput, options: Options) !ListChannelMembershipsForAppInstanceUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChannelMembershipsForAppInstanceUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "scope=app-instance-user-memberships");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChannelMembershipsForAppInstanceUserOutput {
    var result: ListChannelMembershipsForAppInstanceUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelMembershipsForAppInstanceUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
