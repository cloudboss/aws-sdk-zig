...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutMessagingStreamingConfigurationsInput, options: Options) !PutMessagingStreamingConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutMessagingStreamingConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/streaming-configurations");
    ...
    try aws.json.writeValue(@TypeOf(input.streaming_configurations), input.streaming_configurations, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutMessagingStreamingConfigurationsOutput {
    var result: PutMessagingStreamingConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutMessagingStreamingConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
