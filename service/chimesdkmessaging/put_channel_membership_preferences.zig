...
const ChannelMembershipPreferences = @import("channel_membership_preferences.zig").ChannelMembershipPreferences;
const Identity = @import("identity.zig").Identity;

pub const PutChannelMembershipPreferencesInput = struct {
...
pub const PutChannelMembershipPreferencesOutput = struct {
    /// The ARN of the channel.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutChannelMembershipPreferencesInput, options: Options) !PutChannelMembershipPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutChannelMembershipPreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.member_arn);
    try path_buf.appendSlice(alloc, "/preferences");
    ...
    try body_buf.appendSlice(alloc, "\"Preferences\":");
    try aws.json.writeValue(@TypeOf(input.preferences), input.preferences, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutChannelMembershipPreferencesOutput {
    var result: PutChannelMembershipPreferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutChannelMembershipPreferencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
