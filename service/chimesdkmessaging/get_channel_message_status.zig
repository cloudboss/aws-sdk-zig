...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetChannelMessageStatusInput, options: Options) !GetChannelMessageStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChannelMessageStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "scope=message-status");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChannelMessageStatusOutput {
    var result: GetChannelMessageStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChannelMessageStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
