...
pub const AssociateChannelFlowInput = struct {
...
pub const AssociateChannelFlowOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateChannelFlowInput, options: CallOptions) !AssociateChannelFlowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateChannelFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"ChannelFlowArn\":");
    try aws.json.writeValue(@TypeOf(input.channel_flow_arn), input.channel_flow_arn, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateChannelFlowOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateChannelFlowOutput = .{};
