...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateChannelFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ChannelFlowArn\":");
    try aws.json.writeValue(@TypeOf(input.channel_flow_arn), input.channel_flow_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateChannelFlowOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
