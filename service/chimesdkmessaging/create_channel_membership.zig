...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChannelMembershipInput, options: CallOptions) !CreateChannelMembershipOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateChannelMembershipInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateChannelMembershipOutput {
    var result: CreateChannelMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChannelMembershipOutput, body, allocator);
    }
    _ = status;
    _ = headers;
