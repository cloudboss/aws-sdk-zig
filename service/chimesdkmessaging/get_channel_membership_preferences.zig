...
fn serializeRequest(alloc: std.mem.Allocator, input: GetChannelMembershipPreferencesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetChannelMembershipPreferencesOutput {
    var result: GetChannelMembershipPreferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetChannelMembershipPreferencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
