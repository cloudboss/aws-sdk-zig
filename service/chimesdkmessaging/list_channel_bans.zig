...
const ChannelBanSummary = @import("channel_ban_summary.zig").ChannelBanSummary;

pub const ListChannelBansInput = struct {
...
pub const ListChannelBansOutput = struct {
    /// The ARN of the channel.
    channel_arn: ?[]const u8 = null,

    /// The information for each requested ban.
    channel_bans: ?[]const ChannelBanSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChannelBansInput, options: Options) !ListChannelBansOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChannelBansInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/bans");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChannelBansOutput {
    var result: ListChannelBansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChannelBansOutput, body, alloc);
    }
    _ = status;
    _ = headers;
