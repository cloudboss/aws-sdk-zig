...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ChannelFlowCallbackInput, options: Options) !ChannelFlowCallbackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ChannelFlowCallbackInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "operation=channel-flow-callback");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "\"CallbackId\":");
    try aws.json.writeValue(@TypeOf(input.callback_id), input.callback_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ChannelMessage\":");
    try aws.json.writeValue(@TypeOf(input.channel_message), input.channel_message, alloc, &body_buf);
    has_prev = true;
    if (input.delete_resource) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DeleteResource\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ChannelFlowCallbackOutput {
    var result: ChannelFlowCallbackOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ChannelFlowCallbackOutput, body, alloc);
    }
    _ = status;
    _ = headers;
