...
fn serializeRequest(alloc: std.mem.Allocator, input: PutActionInteractionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"actionInteractions\":");
    try aws.json.writeValue(@TypeOf(input.action_interactions), input.action_interactions, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"trackingId\":");
    try aws.json.writeValue(@TypeOf(input.tracking_id), input.tracking_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutActionInteractionsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
