...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEntityEventsInput, options: CallOptions) !ListEntityEventsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListEntityEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListEntityEventsOutput {
    var result: ListEntityEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEntityEventsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
