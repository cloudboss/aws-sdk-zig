...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceLevelObjectiveInput, options: Options) !GetServiceLevelObjectiveOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceLevelObjectiveInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceLevelObjectiveOutput {
    var result: GetServiceLevelObjectiveOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetServiceLevelObjectiveOutput, body, alloc);
    }
    _ = status;
    _ = headers;
