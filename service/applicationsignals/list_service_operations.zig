...
const ServiceOperation = @import("service_operation.zig").ServiceOperation;

pub const ListServiceOperationsInput = struct {
...
pub const ListServiceOperationsOutput = struct {
    ...
    /// this service.
    service_operations: ?[]const ServiceOperation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceOperationsInput, options: Options) !ListServiceOperationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServiceOperationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServiceOperationsOutput {
    var result: ListServiceOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListServiceOperationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
