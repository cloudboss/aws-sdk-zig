...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceStatesInput, options: CallOptions) !ListServiceStatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListServiceStatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListServiceStatesOutput {
    var result: ListServiceStatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListServiceStatesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
