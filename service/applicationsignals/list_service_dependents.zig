...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceDependentsInput, options: Options) !ListServiceDependentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServiceDependentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServiceDependentsOutput {
    var result: ListServiceDependentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListServiceDependentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
