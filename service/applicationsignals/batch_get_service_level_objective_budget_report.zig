...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetServiceLevelObjectiveBudgetReportInput, options: Options) !BatchGetServiceLevelObjectiveBudgetReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetServiceLevelObjectiveBudgetReportInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Timestamp\":");
    try aws.json.writeValue(@TypeOf(input.timestamp), input.timestamp, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetServiceLevelObjectiveBudgetReportOutput {
    var result: BatchGetServiceLevelObjectiveBudgetReportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetServiceLevelObjectiveBudgetReportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
