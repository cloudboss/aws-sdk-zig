...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutGroupingConfigurationInput, options: Options) !PutGroupingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutGroupingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"GroupingAttributeDefinitions\":");
    try aws.json.writeValue(@TypeOf(input.grouping_attribute_definitions), input.grouping_attribute_definitions, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutGroupingConfigurationOutput {
    var result: PutGroupingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutGroupingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
