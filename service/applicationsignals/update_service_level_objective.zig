...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceLevelObjectiveInput, options: Options) !UpdateServiceLevelObjectiveOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceLevelObjectiveInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceLevelObjectiveOutput {
    var result: UpdateServiceLevelObjectiveOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateServiceLevelObjectiveOutput, body, alloc);
    }
    _ = status;
    _ = headers;
