...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAuditFindingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.auditors) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"Auditors\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"AuditTargets\":");
    try aws.json.writeValue(@TypeOf(input.audit_targets), input.audit_targets, alloc, &body_buf);
    has_prev = true;
    if (input.detail_level) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DetailLevel\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAuditFindingsOutput {
    var result: ListAuditFindingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAuditFindingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
