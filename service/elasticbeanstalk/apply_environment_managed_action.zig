...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ApplyEnvironmentManagedActionInput, options: Options) !ApplyEnvironmentManagedActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ApplyEnvironmentManagedActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ApplyEnvironmentManagedAction&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&ActionId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.action_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ApplyEnvironmentManagedActionOutput {
    _ = status;
    _ = headers;
