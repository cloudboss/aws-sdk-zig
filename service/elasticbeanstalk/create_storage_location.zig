...
pub const CreateStorageLocationInput = struct {};

pub const CreateStorageLocationOutput = struct {
    /// The name of the Amazon S3 bucket created.
    s3_bucket: ?[]const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStorageLocationInput, options: CallOptions) !CreateStorageLocationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateStorageLocationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=CreateStorageLocation&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateStorageLocationOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateStorageLocationResult")) break;
            },
            else => {},
        }
    }

    var result: CreateStorageLocationOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
