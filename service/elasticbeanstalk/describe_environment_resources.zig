...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEnvironmentResourcesInput, options: Options) !DescribeEnvironmentResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEnvironmentResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEnvironmentResources&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEnvironmentResourcesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EnvironmentResources")) {
                    result.environment_resources = try serde.deserializeEnvironmentResourceDescription(&reader, alloc);
                } else {
