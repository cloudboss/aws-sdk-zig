...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEnvironmentManagedActionsInput, options: Options) !DescribeEnvironmentManagedActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEnvironmentManagedActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEnvironmentManagedActions&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEnvironmentManagedActionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ManagedActions")) {
                    result.managed_actions = try serde.deserializeManagedActions(&reader, alloc, "member");
                } else {
