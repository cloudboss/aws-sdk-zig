...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CheckDNSAvailabilityInput, options: Options) !CheckDNSAvailabilityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CheckDNSAvailabilityInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CheckDNSAvailability&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&CNAMEPrefix=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cname_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CheckDNSAvailabilityOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Available")) {
                    ...
                    result.fully_qualified_cname = try alloc.dupe(u8, try reader.readElementText());
                } else {
