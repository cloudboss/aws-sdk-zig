...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAccountAttributes&Version=2010-12-01");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ResourceQuotas")) {
                    result.resource_quotas = try serde.deserializeResourceQuotas(&reader, alloc);
                } else {
