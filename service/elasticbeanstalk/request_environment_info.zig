...
pub const RequestEnvironmentInfoInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RequestEnvironmentInfoInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RequestEnvironmentInfo&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RequestEnvironmentInfoOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: RequestEnvironmentInfoOutput = .{};
