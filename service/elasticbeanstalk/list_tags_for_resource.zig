...
const ListTagsForResourceOutput = @import("resource_tags_description_message.zig").ResourceTagsDescriptionMessage;
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTagsForResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListTagsForResource&Version=2010-12-01");
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                ...
                } else if (std.mem.eql(u8, e.local, "ResourceTags")) {
                    result.resource_tags = try serde.deserializeTagList(allocator, &reader, "member");
                } else {
