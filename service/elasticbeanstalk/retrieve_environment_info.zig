...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RetrieveEnvironmentInfoInput, options: Options) !RetrieveEnvironmentInfoOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RetrieveEnvironmentInfoInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RetrieveEnvironmentInfo&Version=2010-12-01");
    ...
    try body_buf.appendSlice(alloc, "&InfoType=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.info_type));
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RetrieveEnvironmentInfoOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EnvironmentInfo")) {
                    result.environment_info = try serde.deserializeEnvironmentInfoDescriptionList(&reader, alloc, "member");
                } else {
