...
const CreateConfigurationTemplateOutput = @import("configuration_settings_description.zig").ConfigurationSettingsDescription;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConfigurationTemplateInput, options: Options) !CreateConfigurationTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConfigurationTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateConfigurationTemplate&Version=2010-12-01");
    ...
    if (input.source_configuration) |v| {
        if (v.application_name) |sv| {
            try body_buf.appendSlice(alloc, "&SourceConfiguration.ApplicationName=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
        }
        if (v.template_name) |sv| {
            try body_buf.appendSlice(alloc, "&SourceConfiguration.TemplateName=");
            try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
        }
    }
    if (input.tags) |list| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConfigurationTemplateOutput {
    _ = status;
    _ = headers;
