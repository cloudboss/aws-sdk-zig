...
pub const DescribePlatformVersionInput = struct {
    /// The ARN of the platform version.
    platform_arn: ?[]const u8 = null,
};

pub const DescribePlatformVersionOutput = struct {
    /// Detailed information about the platform version.
    platform_description: ?PlatformDescription = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePlatformVersionInput, options: CallOptions) !DescribePlatformVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribePlatformVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribePlatformVersion&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribePlatformVersionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribePlatformVersionResult")) break;
            },
            else => {},
        }
    }

    var result: DescribePlatformVersionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PlatformDescription")) {
                    result.platform_description = try serde.deserializePlatformDescription(allocator, &reader);
                } else {
