...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePlatformVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribePlatformVersion&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePlatformVersionOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PlatformDescription")) {
                    result.platform_description = try serde.deserializePlatformDescription(&reader, alloc);
                } else {
