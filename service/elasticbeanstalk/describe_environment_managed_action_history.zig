...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEnvironmentManagedActionHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEnvironmentManagedActionHistory&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEnvironmentManagedActionHistoryOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ManagedActionHistoryItems")) {
                    result.managed_action_history_items = try serde.deserializeManagedActionHistoryItems(&reader, alloc, "member");
