...
pub const AbortEnvironmentUpdateOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AbortEnvironmentUpdateInput, options: CallOptions) !AbortEnvironmentUpdateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AbortEnvironmentUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AbortEnvironmentUpdate&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AbortEnvironmentUpdateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: AbortEnvironmentUpdateOutput = .{};
