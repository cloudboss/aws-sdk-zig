...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeApplicationVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeApplicationVersions&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeApplicationVersionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ApplicationVersions")) {
                    result.application_versions = try serde.deserializeApplicationVersionDescriptionList(&reader, alloc, "member");
