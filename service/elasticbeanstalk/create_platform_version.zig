...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePlatformVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreatePlatformVersion&Version=2010-12-01");
    ...
    if (input.platform_definition_bundle.s3_bucket) |sv| {
        try body_buf.appendSlice(alloc, "&PlatformDefinitionBundle.S3Bucket=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
    }
    if (input.platform_definition_bundle.s3_key) |sv| {
        try body_buf.appendSlice(alloc, "&PlatformDefinitionBundle.S3Key=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
    }
    try body_buf.appendSlice(alloc, "&PlatformName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.platform_name);
    try body_buf.appendSlice(alloc, "&PlatformVersion=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.platform_version);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePlatformVersionOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Builder")) {
                    result.builder = try serde.deserializeBuilder(&reader, alloc);
