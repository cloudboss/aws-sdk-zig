...
pub const ListAvailableSolutionStacksInput = struct {};

pub const ListAvailableSolutionStacksOutput = struct {
    /// A list of available solution stacks and their SolutionStackDescription.
    solution_stack_details: ?[]const SolutionStackDescription = null,

    /// A list of available solution stacks.
    solution_stacks: ?[]const []const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAvailableSolutionStacksInput, options: CallOptions) !ListAvailableSolutionStacksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAvailableSolutionStacksInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=ListAvailableSolutionStacks&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAvailableSolutionStacksOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListAvailableSolutionStacksResult")) break;
            },
            else => {},
        }
    }

    var result: ListAvailableSolutionStacksOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SolutionStackDetails")) {
                    result.solution_stack_details = try serde.deserializeAvailableSolutionStackDetailsList(allocator, &reader, "member");
                } else if (std.mem.eql(u8, e.local, "SolutionStacks")) {
                    result.solution_stacks = try serde.deserializeAvailableSolutionStackNamesList(allocator, &reader, "member");
                } else {
