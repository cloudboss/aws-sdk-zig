...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAvailableSolutionStacksInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=ListAvailableSolutionStacks&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAvailableSolutionStacksOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SolutionStackDetails")) {
                    result.solution_stack_details = try serde.deserializeAvailableSolutionStackDetailsList(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "SolutionStacks")) {
                    result.solution_stacks = try serde.deserializeAvailableSolutionStackNamesList(&reader, alloc, "member");
                } else {
