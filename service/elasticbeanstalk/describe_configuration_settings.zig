...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConfigurationSettingsInput, options: Options) !DescribeConfigurationSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConfigurationSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeConfigurationSettings&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConfigurationSettingsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ConfigurationSettings")) {
                    result.configuration_settings = try serde.deserializeConfigurationSettingsDescriptionList(&reader, alloc, "member");
                } else {
