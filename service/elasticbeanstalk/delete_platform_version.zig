...
pub const DeletePlatformVersionInput = struct {
    /// The ARN of the version of the custom platform.
    platform_arn: ?[]const u8 = null,
};

pub const DeletePlatformVersionOutput = struct {
    /// Detailed information about the version of the custom platform.
    platform_summary: ?PlatformSummary = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePlatformVersionInput, options: CallOptions) !DeletePlatformVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePlatformVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeletePlatformVersion&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePlatformVersionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeletePlatformVersionResult")) break;
            },
            else => {},
        }
    }

    var result: DeletePlatformVersionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PlatformSummary")) {
