...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePlatformVersionInput, options: Options) !DeletePlatformVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePlatformVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeletePlatformVersion&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePlatformVersionOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PlatformSummary")) {
                    result.platform_summary = try serde.deserializePlatformSummary(&reader, alloc);
                } else {
