...
pub const DescribeInstancesHealthOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstancesHealthInput, options: CallOptions) !DescribeInstancesHealthOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstancesHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstancesHealth&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstancesHealthOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeInstancesHealthResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeInstancesHealthOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InstanceHealthList")) {
                    result.instance_health_list = try serde.deserializeInstanceHealthList(allocator, &reader, "member");
