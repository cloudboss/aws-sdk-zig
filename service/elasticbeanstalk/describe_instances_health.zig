...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstancesHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInstancesHealth&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstancesHealthOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InstanceHealthList")) {
                    result.instance_health_list = try serde.deserializeInstanceHealthList(&reader, alloc, "member");
