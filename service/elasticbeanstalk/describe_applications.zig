...
pub const DescribeApplicationsOutput = struct {
    ...
    applications: ?[]const ApplicationDescription = null,
};
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeApplications&Version=2010-12-01");
    if (input.application_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ApplicationNames.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeApplicationsOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeApplicationsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeApplicationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Applications")) {
                    result.applications = try serde.deserializeApplicationDescriptionList(allocator, &reader, "member");
                } else {
