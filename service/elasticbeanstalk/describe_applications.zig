...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeApplications&Version=2010-12-01");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Applications")) {
                    result.applications = try serde.deserializeApplicationDescriptionList(&reader, alloc, "member");
                } else {
