...
const AssociateEnvironmentOperationsRoleOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateEnvironmentOperationsRoleInput, options: Options) !AssociateEnvironmentOperationsRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateEnvironmentOperationsRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AssociateEnvironmentOperationsRole&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&EnvironmentName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.environment_name);
    try body_buf.appendSlice(alloc, "&OperationsRole=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.operations_role);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateEnvironmentOperationsRoleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
