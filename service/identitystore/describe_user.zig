...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUserInput, options: Options) !DescribeUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIdentityStore.DescribeUser");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUserOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeUserOutput, body, alloc);
}
