...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUserIdInput, options: Options) !GetUserIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUserIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIdentityStore.GetUserId");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUserIdOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetUserIdOutput, body, alloc);
}
