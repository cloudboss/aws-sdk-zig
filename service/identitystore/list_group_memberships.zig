...
pub const ListGroupMembershipsInput = struct {
    ...
    /// used in all ` List` requests to specify how many results to return in one
    /// page.
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupMembershipsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIdentityStore.ListGroupMemberships");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupMembershipsOutput {
    ...
    return aws.json.parseJsonObject(ListGroupMembershipsOutput, body, alloc);
}
