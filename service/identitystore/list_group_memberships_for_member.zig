...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupMembershipsForMemberInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIdentityStore.ListGroupMembershipsForMember");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupMembershipsForMemberOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListGroupMembershipsForMemberOutput, body, alloc);
}
