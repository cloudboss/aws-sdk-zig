...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: IsMemberInGroupsInput, options: Options) !IsMemberInGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: IsMemberInGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIdentityStore.IsMemberInGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !IsMemberInGroupsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(IsMemberInGroupsOutput, body, alloc);
}
