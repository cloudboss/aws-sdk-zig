...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGroupIdInput, options: Options) !GetGroupIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGroupIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIdentityStore.GetGroupId");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGroupIdOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetGroupIdOutput, body, alloc);
}
