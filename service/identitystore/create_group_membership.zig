...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGroupMembershipInput, options: Options) !CreateGroupMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGroupMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIdentityStore.CreateGroupMembership");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGroupMembershipOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateGroupMembershipOutput, body, alloc);
}
