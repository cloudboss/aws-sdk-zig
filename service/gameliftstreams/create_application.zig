...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.application_log_output_uri) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ApplicationLogOutputUri\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.application_log_paths) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ApplicationLogPaths\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ApplicationSourceUri\":");
    try aws.json.writeValue(@TypeOf(input.application_source_uri), input.application_source_uri, alloc, &body_buf);
    has_prev = true;
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ExecutablePath\":");
    try aws.json.writeValue(@TypeOf(input.executable_path), input.executable_path, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RuntimeEnvironment\":");
    try aws.json.writeValue(@TypeOf(input.runtime_environment), input.runtime_environment, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
