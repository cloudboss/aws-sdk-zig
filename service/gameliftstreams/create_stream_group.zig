...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStreamGroupInput, options: CallOptions) !CreateStreamGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateStreamGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"StreamClass\":");
    try aws.json.writeValue(@TypeOf(input.stream_class), input.stream_class, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
