...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddStreamGroupLocationsInput, options: Options) !AddStreamGroupLocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddStreamGroupLocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.location_configurations), input.location_configurations, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddStreamGroupLocationsOutput {
    var result: AddStreamGroupLocationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddStreamGroupLocationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
