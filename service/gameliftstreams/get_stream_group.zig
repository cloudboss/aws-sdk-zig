...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStreamGroupInput, options: Options) !GetStreamGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStreamGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStreamGroupOutput {
    var result: GetStreamGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetStreamGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
