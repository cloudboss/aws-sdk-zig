...
pub const DisassociateApplicationsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateApplicationsInput, options: Options) !DisassociateApplicationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/disassociations");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateApplicationsOutput {
    var result: DisassociateApplicationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateApplicationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
