...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateApplicationsInput, options: CallOptions) !AssociateApplicationsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "gameliftstreams");
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"ApplicationIdentifiers\":");
    try aws.json.writeValue(@TypeOf(input.application_identifiers), input.application_identifiers, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateApplicationsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateApplicationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
