...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStreamGroupInput, options: Options) !UpdateStreamGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStreamGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStreamGroupOutput {
    var result: UpdateStreamGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateStreamGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
