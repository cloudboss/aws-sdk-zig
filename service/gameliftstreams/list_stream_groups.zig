...
const StreamGroupSummary = @import("stream_group_summary.zig").StreamGroupSummary;

pub const ListStreamGroupsInput = struct {
    ...
    /// you retrieve results as sequential pages. To get the first page of results,
    /// omit a token value. To get the remaining pages, provide the token returned
...
pub const ListStreamGroupsOutput = struct {
    ...
    items: ?[]const StreamGroupSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStreamGroupsInput, options: Options) !ListStreamGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStreamGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStreamGroupsOutput {
    var result: ListStreamGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStreamGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
