...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartStreamSessionInput, options: CallOptions) !StartStreamSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartStreamSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connection_timeout_seconds) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ConnectionTimeoutSeconds\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    ...
    if (input.performance_stats_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"PerformanceStatsConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
