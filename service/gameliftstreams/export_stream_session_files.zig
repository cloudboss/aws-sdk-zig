...
fn serializeRequest(alloc: std.mem.Allocator, input: ExportStreamSessionFilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.stream_session_identifier);
    try path_buf.appendSlice(alloc, "/exportfiles");
    ...
    try body_buf.appendSlice(alloc, "\"OutputUri\":");
    try aws.json.writeValue(@TypeOf(input.output_uri), input.output_uri, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExportStreamSessionFilesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
