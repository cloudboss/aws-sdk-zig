...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStreamSessionsByAccountInput, options: Options) !ListStreamSessionsByAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStreamSessionsByAccountInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStreamSessionsByAccountOutput {
    var result: ListStreamSessionsByAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStreamSessionsByAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
