...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStreamSessionConnectionInput, options: Options) !CreateStreamSessionConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStreamSessionConnectionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStreamSessionConnectionOutput {
    var result: CreateStreamSessionConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateStreamSessionConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
