...
const RemoveStreamGroupLocationsOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveStreamGroupLocationsInput, options: Options) !RemoveStreamGroupLocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveStreamGroupLocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/locations");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "locations=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.locations);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveStreamGroupLocationsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
