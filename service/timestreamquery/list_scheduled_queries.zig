...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListScheduledQueriesInput, options: Options) !ListScheduledQueriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListScheduledQueriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Timestream_20181101.ListScheduledQueries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListScheduledQueriesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListScheduledQueriesOutput, body, alloc);
}
