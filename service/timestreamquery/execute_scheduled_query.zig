...
const ExecuteScheduledQueryOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteScheduledQueryInput, options: Options) !ExecuteScheduledQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteScheduledQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Timestream_20181101.ExecuteScheduledQuery");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecuteScheduledQueryOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
