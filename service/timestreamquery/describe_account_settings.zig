...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountSettingsInput, options: Options) !DescribeAccountSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountSettingsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Timestream_20181101.DescribeAccountSettings");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountSettingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAccountSettingsOutput, body, alloc);
}
