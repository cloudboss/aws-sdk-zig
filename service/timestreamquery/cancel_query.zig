...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelQueryInput, options: Options) !CancelQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Timestream_20181101.CancelQuery");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelQueryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelQueryOutput, body, alloc);
}
