...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccountConfigurationInput, options: Options) !UpdateAccountConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccountConfigurationInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("codegurusecurity", "CodeGuru Security", alloc);
    ...
    try aws.json.writeValue(@TypeOf(input.encryption_config), input.encryption_config, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccountConfigurationOutput {
    var result: UpdateAccountConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccountConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
