...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetFindingsInput, options: Options) !BatchGetFindingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetFindingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetFindingsOutput {
    var result: BatchGetFindingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetFindingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
