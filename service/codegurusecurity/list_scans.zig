...
const ScanSummary = @import("scan_summary.zig").ScanSummary;

pub const ListScansInput = struct {
...
pub const ListScansOutput = struct {
    /// A pagination token. You can use this in future calls to `ListScans` to
    /// continue listing results after the current page.
    next_token: ?[]const u8 = null,

    /// A list of `ScanSummary` objects with information about all scans in an
    /// account.
    summaries: ?[]const ScanSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListScansInput, options: CallOptions) !ListScansOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListScansInput, config: *aws.Config) !aws.http.Request {
