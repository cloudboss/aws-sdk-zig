...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountConfigurationInput, options: Options) !GetAccountConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccountConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccountConfigurationOutput {
    var result: GetAccountConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAccountConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
