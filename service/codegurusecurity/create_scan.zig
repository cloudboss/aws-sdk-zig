...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateScanInput, options: Options) !CreateScanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateScanInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.analysis_type) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"analysisType\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateScanOutput {
    var result: CreateScanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateScanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
