...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFindingsMetricsInput, options: Options) !ListFindingsMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFindingsMetricsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFindingsMetricsOutput {
    var result: ListFindingsMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFindingsMetricsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
