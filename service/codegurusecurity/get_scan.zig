...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetScanInput, options: Options) !GetScanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetScanInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/scans/");
    try path_buf.appendSlice(alloc, input.scan_name);
    ...
    if (input.run_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "runId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetScanOutput {
    var result: GetScanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetScanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
