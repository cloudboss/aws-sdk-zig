...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateOdbNetworkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.UpdateOdbNetwork");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateOdbNetworkOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateOdbNetworkOutput, body, alloc);
}
