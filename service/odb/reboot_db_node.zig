...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebootDbNodeInput, options: Options) !RebootDbNodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RebootDbNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.RebootDbNode");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RebootDbNodeOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(RebootDbNodeOutput, body, alloc);
}
