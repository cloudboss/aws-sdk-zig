...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDbServersInput, options: Options) !ListDbServersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDbServersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.ListDbServers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDbServersOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListDbServersOutput, body, alloc);
}
