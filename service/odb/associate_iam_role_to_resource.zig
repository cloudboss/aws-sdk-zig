...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateIamRoleToResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.AssociateIamRoleToResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateIamRoleToResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
