...
pub const AssociateIamRoleToResourceInput = struct {
    /// The Amazon Web Services integration configuration settings for the Amazon
    /// Web Services Identity and Access Management (IAM) service role association.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateIamRoleToResourceInput, options: Options) !AssociateIamRoleToResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateIamRoleToResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Odb.AssociateIamRoleToResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateIamRoleToResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
