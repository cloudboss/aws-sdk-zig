...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOciOnboardingStatusInput, options: Options) !GetOciOnboardingStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOciOnboardingStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.GetOciOnboardingStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOciOnboardingStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetOciOnboardingStatusOutput, body, alloc);
}
