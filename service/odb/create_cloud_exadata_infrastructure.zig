...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCloudExadataInfrastructureInput, options: Options) !CreateCloudExadataInfrastructureOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCloudExadataInfrastructureInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.CreateCloudExadataInfrastructure");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCloudExadataInfrastructureOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCloudExadataInfrastructureOutput, body, alloc);
}
