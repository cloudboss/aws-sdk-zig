...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOdbNetworkInput, options: CallOptions) !CreateOdbNetworkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateOdbNetworkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Odb.CreateOdbNetwork");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateOdbNetworkOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateOdbNetworkOutput, body, allocator);
}
