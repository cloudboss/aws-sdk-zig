...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopDbNodeInput, options: Options) !StopDbNodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopDbNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.StopDbNode");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopDbNodeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopDbNodeOutput, body, alloc);
}
