...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDbNodeInput, options: Options) !GetDbNodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDbNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.GetDbNode");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDbNodeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDbNodeOutput, body, alloc);
}
