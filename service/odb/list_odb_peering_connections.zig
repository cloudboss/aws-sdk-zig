...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOdbPeeringConnectionsInput, options: Options) !ListOdbPeeringConnectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOdbPeeringConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.ListOdbPeeringConnections");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOdbPeeringConnectionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOdbPeeringConnectionsOutput, body, alloc);
}
