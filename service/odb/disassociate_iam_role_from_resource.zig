...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateIamRoleFromResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Odb.DisassociateIamRoleFromResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateIamRoleFromResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
