...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOdbPeeringConnectionInput, options: Options) !UpdateOdbPeeringConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateOdbPeeringConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.UpdateOdbPeeringConnection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateOdbPeeringConnectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateOdbPeeringConnectionOutput, body, alloc);
}
