...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDbNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Odb.StartDbNode");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDbNodeOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StartDbNodeOutput, body, alloc);
}
