...
const SearchNearbyAdditionalFeature = @import("search_nearby_additional_feature.zig").SearchNearbyAdditionalFeature;
const SearchNearbyFilter = @import("search_nearby_filter.zig").SearchNearbyFilter;
const SearchNearbyIntendedUse = @import("search_nearby_intended_use.zig").SearchNearbyIntendedUse;
const SearchNearbyResultItem = @import("search_nearby_result_item.zig").SearchNearbyResultItem;

pub const SearchNearbyInput = struct {
    /// A list of optional additional parameters, such as time zone, that can be
    /// requested for each result.
    additional_features: ?[]const SearchNearbyAdditionalFeature = null,
    ...
    filter: ?SearchNearbyFilter = null,
    ...
    intended_use: ?SearchNearbyIntendedUse = null,
...
pub const SearchNearbyOutput = struct {
    /// If `nextToken` is returned, there are more results available. The value of
    /// `nextToken` is a unique pagination token for each page.
    next_token: ?[]const u8 = null,
    ...
    result_items: ?[]const SearchNearbyResultItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchNearbyInput, options: Options) !SearchNearbyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchNearbyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"QueryPosition\":");
    try aws.json.writeValue(@TypeOf(input.query_position), input.query_position, alloc, &body_buf);
    has_prev = true;
    if (input.query_radius) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"QueryRadius\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchNearbyOutput {
    var result: SearchNearbyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchNearbyOutput, body, alloc);
    }
    _ = status;
