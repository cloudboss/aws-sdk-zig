...
pub const SearchNearbyFilter = struct {
    ...
    exclude_business_chains: ?[]const []const u8,

    /// Categories of results that results are excluded from.
    exclude_categories: ?[]const []const u8,

    /// Food types that results are excluded from.
    exclude_food_types: ?[]const []const u8,

    /// The Business Chains associated with the place.
    include_business_chains: ?[]const []const u8,
    ...
    include_food_types: ?[]const []const u8,
