...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReverseGeocodeInput, options: Options) !ReverseGeocodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReverseGeocodeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.heading) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Heading\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReverseGeocodeOutput {
    var result: ReverseGeocodeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ReverseGeocodeOutput, body, alloc);
    }
    _ = status;
