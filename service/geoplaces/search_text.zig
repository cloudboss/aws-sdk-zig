...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchTextInput, options: Options) !SearchTextOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchTextInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchTextOutput {
    var result: SearchTextOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchTextOutput, body, alloc);
    }
    _ = status;
