...
fn serializeRequest(alloc: std.mem.Allocator, input: GeocodeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.query_components) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"QueryComponents\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GeocodeOutput {
    var result: GeocodeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GeocodeOutput, body, alloc);
    }
    _ = status;
