...
pub const UpdateModelVersionInput = struct {
...
pub const UpdateModelVersionOutput = struct {
    /// The model ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateModelVersionInput, options: CallOptions) !UpdateModelVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateModelVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSHawksNestServiceFacade.UpdateModelVersion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateModelVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateModelVersionOutput, body, allocator);
}
