...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateModelVersionInput, options: Options) !CreateModelVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateModelVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.CreateModelVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateModelVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateModelVersionOutput, body, alloc);
}
