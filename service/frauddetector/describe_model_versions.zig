...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeModelVersionsInput, options: Options) !DescribeModelVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeModelVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.DescribeModelVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeModelVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeModelVersionsOutput, body, alloc);
}
