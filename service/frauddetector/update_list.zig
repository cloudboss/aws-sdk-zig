...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateListInput, options: Options) !UpdateListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateListInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.UpdateList");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateListOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
