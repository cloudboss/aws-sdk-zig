...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVariableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.CreateVariable");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVariableOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
