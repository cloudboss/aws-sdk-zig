...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEventPredictionsInput, options: Options) !ListEventPredictionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventPredictionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.ListEventPredictions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEventPredictionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEventPredictionsOutput, body, alloc);
}
