...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutExternalModelInput, options: Options) !PutExternalModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutExternalModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.PutExternalModel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutExternalModelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
