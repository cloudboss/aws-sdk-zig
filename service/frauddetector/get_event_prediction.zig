...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEventPredictionInput, options: Options) !GetEventPredictionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEventPredictionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.GetEventPrediction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEventPredictionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEventPredictionOutput, body, alloc);
}
