...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateListInput, options: Options) !CreateListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateListInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.CreateList");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateListOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
