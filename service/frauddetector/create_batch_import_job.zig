...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBatchImportJobInput, options: Options) !CreateBatchImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBatchImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.CreateBatchImportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBatchImportJobOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
