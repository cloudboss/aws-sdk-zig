...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDetectorVersionInput, options: CallOptions) !GetDetectorVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDetectorVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSHawksNestServiceFacade.GetDetectorVersion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDetectorVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDetectorVersionOutput, body, allocator);
}
