...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEventLabelInput, options: Options) !UpdateEventLabelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEventLabelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.UpdateEventLabel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEventLabelOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
