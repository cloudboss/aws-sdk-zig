...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDetectorVersionInput, options: Options) !UpdateDetectorVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDetectorVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.UpdateDetectorVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDetectorVersionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
