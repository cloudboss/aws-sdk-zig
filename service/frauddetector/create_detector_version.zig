...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDetectorVersionInput, options: Options) !CreateDetectorVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDetectorVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.CreateDetectorVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDetectorVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDetectorVersionOutput, body, alloc);
}
