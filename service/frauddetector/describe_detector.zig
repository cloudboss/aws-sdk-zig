...
const DetectorVersionSummary = @import("detector_version_summary.zig").DetectorVersionSummary;
...
pub const DescribeDetectorOutput = struct {
    ...
    detector_version_summaries: ?[]const DetectorVersionSummary = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDetectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSHawksNestServiceFacade.DescribeDetector");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDetectorOutput {
    ...
    return aws.json.parseJsonObject(DescribeDetectorOutput, body, allocator);
}
