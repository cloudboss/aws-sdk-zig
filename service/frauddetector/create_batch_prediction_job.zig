...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBatchPredictionJobInput, options: Options) !CreateBatchPredictionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBatchPredictionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.CreateBatchPredictionJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBatchPredictionJobOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
