...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEventTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSHawksNestServiceFacade.PutEventType");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEventTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
