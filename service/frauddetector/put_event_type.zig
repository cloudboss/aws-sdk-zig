...
pub const PutEventTypeInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutEventTypeInput, options: Options) !PutEventTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutEventTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSHawksNestServiceFacade.PutEventType");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutEventTypeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
