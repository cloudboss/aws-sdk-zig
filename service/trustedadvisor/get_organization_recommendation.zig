...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOrganizationRecommendationInput, options: Options) !GetOrganizationRecommendationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOrganizationRecommendationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOrganizationRecommendationOutput {
    var result: GetOrganizationRecommendationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOrganizationRecommendationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
