...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOrganizationRecommendationAccountsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOrganizationRecommendationAccountsOutput {
    var result: ListOrganizationRecommendationAccountsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOrganizationRecommendationAccountsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
