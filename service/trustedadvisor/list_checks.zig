...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChecksInput, options: Options) !ListChecksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChecksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChecksOutput {
    var result: ListChecksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChecksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
