...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOrganizationRecommendationsInput, options: Options) !ListOrganizationRecommendationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOrganizationRecommendationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOrganizationRecommendationsOutput {
    var result: ListOrganizationRecommendationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOrganizationRecommendationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
