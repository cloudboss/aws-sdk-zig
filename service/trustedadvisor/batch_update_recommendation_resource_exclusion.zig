...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateRecommendationResourceExclusionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"recommendationResourceExclusions\":");
    try aws.json.writeValue(@TypeOf(input.recommendation_resource_exclusions), input.recommendation_resource_exclusions, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateRecommendationResourceExclusionOutput {
    var result: BatchUpdateRecommendationResourceExclusionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateRecommendationResourceExclusionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
