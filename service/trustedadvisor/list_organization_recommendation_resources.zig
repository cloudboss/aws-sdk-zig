...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOrganizationRecommendationResourcesInput, options: Options) !ListOrganizationRecommendationResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOrganizationRecommendationResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.affected_account_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "affectedAccountId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOrganizationRecommendationResourcesOutput {
    var result: ListOrganizationRecommendationResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOrganizationRecommendationResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
