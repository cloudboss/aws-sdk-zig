...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecommendationResourcesInput, options: Options) !ListRecommendationResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecommendationResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/recommendations/");
    try path_buf.appendSlice(alloc, input.recommendation_identifier);
    ...
    if (input.exclusion_status) |v| {
        ...
        try query_buf.appendSlice(alloc, "exclusionStatus=");
    ...
    if (input.region_code) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "regionCode=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.status) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecommendationResourcesOutput {
    var result: ListRecommendationResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRecommendationResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
