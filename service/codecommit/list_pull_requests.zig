...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPullRequestsInput, options: Options) !ListPullRequestsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPullRequestsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.ListPullRequests");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPullRequestsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListPullRequestsOutput, body, alloc);
}
