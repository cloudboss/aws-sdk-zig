...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUnreferencedMergeCommitInput, options: Options) !CreateUnreferencedMergeCommitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUnreferencedMergeCommitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.CreateUnreferencedMergeCommit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateUnreferencedMergeCommitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateUnreferencedMergeCommitOutput, body, alloc);
}
