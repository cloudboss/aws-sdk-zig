...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFileCommitHistoryInput, options: Options) !ListFileCommitHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFileCommitHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.ListFileCommitHistory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFileCommitHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFileCommitHistoryOutput, body, alloc);
}
