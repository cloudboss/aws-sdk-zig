...
pub const DeleteBranchInput = struct {
...
pub const DeleteBranchOutput = struct {
    ...
    deleted_branch: ?BranchInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBranchInput, options: CallOptions) !DeleteBranchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBranchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.DeleteBranch");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBranchOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteBranchOutput, body, allocator);
}
