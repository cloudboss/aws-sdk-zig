...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutCommentReactionInput, options: Options) !PutCommentReactionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutCommentReactionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.PutCommentReaction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutCommentReactionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
