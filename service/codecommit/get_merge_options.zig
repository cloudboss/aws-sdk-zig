...
pub const GetMergeOptionsInput = struct {
...
pub const GetMergeOptionsOutput = struct {
    /// The commit ID of the merge base.
    base_commit_id: []const u8,
    ...
    merge_options: ?[]const MergeOptionTypeEnum = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMergeOptionsInput, options: CallOptions) !GetMergeOptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMergeOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.GetMergeOptions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMergeOptionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetMergeOptionsOutput, body, allocator);
}
