...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFolderInput, options: Options) !GetFolderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFolderInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.GetFolder");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFolderOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetFolderOutput, body, alloc);
}
