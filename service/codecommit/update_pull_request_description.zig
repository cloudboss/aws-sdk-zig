...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePullRequestDescriptionInput, options: CallOptions) !UpdatePullRequestDescriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePullRequestDescriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.UpdatePullRequestDescription");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePullRequestDescriptionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdatePullRequestDescriptionOutput, body, allocator);
}
