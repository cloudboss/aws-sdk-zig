...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDisassociateApprovalRuleTemplateFromRepositoriesInput, options: Options) !BatchDisassociateApprovalRuleTemplateFromRepositoriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDisassociateApprovalRuleTemplateFromRepositoriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.BatchDisassociateApprovalRuleTemplateFromRepositories");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDisassociateApprovalRuleTemplateFromRepositoriesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDisassociateApprovalRuleTemplateFromRepositoriesOutput, body, alloc);
}
