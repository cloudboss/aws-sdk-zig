...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMergeConflictsInput, options: Options) !GetMergeConflictsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMergeConflictsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.GetMergeConflicts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMergeConflictsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMergeConflictsOutput, body, alloc);
}
