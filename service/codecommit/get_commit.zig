...
pub const GetCommitInput = struct {
...
pub const GetCommitOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCommitInput, options: CallOptions) !GetCommitOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCommitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.GetCommit");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCommitOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetCommitOutput, body, allocator);
}
