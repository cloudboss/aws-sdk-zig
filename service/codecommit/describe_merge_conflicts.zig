...
pub const DescribeMergeConflictsInput = struct {
...
pub const DescribeMergeConflictsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMergeConflictsInput, options: CallOptions) !DescribeMergeConflictsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeMergeConflictsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.DescribeMergeConflicts");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeMergeConflictsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeMergeConflictsOutput, body, allocator);
}
