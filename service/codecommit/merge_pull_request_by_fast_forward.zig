...
fn serializeRequest(alloc: std.mem.Allocator, input: MergePullRequestByFastForwardInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.MergePullRequestByFastForward");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MergePullRequestByFastForwardOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(MergePullRequestByFastForwardOutput, body, alloc);
}
