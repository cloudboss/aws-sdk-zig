...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBlobInput, options: Options) !GetBlobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBlobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.GetBlob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBlobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetBlobOutput, body, alloc);
}
