...
pub const UpdatePullRequestApprovalStateInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePullRequestApprovalStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.UpdatePullRequestApprovalState");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePullRequestApprovalStateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
