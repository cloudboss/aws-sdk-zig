...
const Approval = @import("approval.zig").Approval;

pub const GetPullRequestApprovalStatesInput = struct {
...
pub const GetPullRequestApprovalStatesOutput = struct {
    /// Information about users who have approved the pull request.
    approvals: ?[]const Approval = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPullRequestApprovalStatesInput, options: CallOptions) !GetPullRequestApprovalStatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPullRequestApprovalStatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.GetPullRequestApprovalStates");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPullRequestApprovalStatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPullRequestApprovalStatesOutput, body, allocator);
}
