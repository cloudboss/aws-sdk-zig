...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteApprovalRuleTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.DeleteApprovalRuleTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteApprovalRuleTemplateOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteApprovalRuleTemplateOutput, body, alloc);
}
