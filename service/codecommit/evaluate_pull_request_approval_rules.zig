...
fn serializeRequest(alloc: std.mem.Allocator, input: EvaluatePullRequestApprovalRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.EvaluatePullRequestApprovalRules");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EvaluatePullRequestApprovalRulesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(EvaluatePullRequestApprovalRulesOutput, body, alloc);
}
