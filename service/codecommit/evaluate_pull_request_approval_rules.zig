...
pub const EvaluatePullRequestApprovalRulesInput = struct {
...
pub const EvaluatePullRequestApprovalRulesOutput = struct {
    ...
    /// have
    /// not been met (if any), whether the pull request is in the approved state,
    /// and whether the pull request approval rule has been set aside by an
    /// override.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EvaluatePullRequestApprovalRulesInput, options: CallOptions) !EvaluatePullRequestApprovalRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EvaluatePullRequestApprovalRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.EvaluatePullRequestApprovalRules");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EvaluatePullRequestApprovalRulesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(EvaluatePullRequestApprovalRulesOutput, body, allocator);
}
