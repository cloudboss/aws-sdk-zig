...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCommentsForComparedCommitInput, options: Options) !GetCommentsForComparedCommitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCommentsForComparedCommitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.GetCommentsForComparedCommit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCommentsForComparedCommitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCommentsForComparedCommitOutput, body, alloc);
}
