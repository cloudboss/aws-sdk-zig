...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePullRequestEventsInput, options: Options) !DescribePullRequestEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePullRequestEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.DescribePullRequestEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePullRequestEventsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribePullRequestEventsOutput, body, alloc);
}
