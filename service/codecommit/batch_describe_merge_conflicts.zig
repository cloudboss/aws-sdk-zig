...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDescribeMergeConflictsInput, options: Options) !BatchDescribeMergeConflictsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDescribeMergeConflictsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.BatchDescribeMergeConflicts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDescribeMergeConflictsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDescribeMergeConflictsOutput, body, alloc);
}
