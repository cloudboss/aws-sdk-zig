...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPullRequestInput, options: Options) !GetPullRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPullRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.GetPullRequest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPullRequestOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPullRequestOutput, body, alloc);
}
