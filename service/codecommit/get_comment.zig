...
pub const GetCommentInput = struct {
...
pub const GetCommentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCommentInput, options: CallOptions) !GetCommentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCommentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.GetComment");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCommentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCommentOutput, body, allocator);
}
