...
pub const DeletePullRequestApprovalRuleInput = struct {
...
pub const DeletePullRequestApprovalRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePullRequestApprovalRuleInput, options: CallOptions) !DeletePullRequestApprovalRuleOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "codecommit");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePullRequestApprovalRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.DeletePullRequestApprovalRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePullRequestApprovalRuleOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeletePullRequestApprovalRuleOutput, body, allocator);
}
