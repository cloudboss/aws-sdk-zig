...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MergePullRequestBySquashInput, options: Options) !MergePullRequestBySquashOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: MergePullRequestBySquashInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.MergePullRequestBySquash");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MergePullRequestBySquashOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(MergePullRequestBySquashOutput, body, alloc);
}
