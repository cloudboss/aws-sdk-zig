...
pub const MergeBranchesBySquashInput = struct {
...
pub const MergeBranchesBySquashOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MergeBranchesBySquashInput, options: CallOptions) !MergeBranchesBySquashOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: MergeBranchesBySquashInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.MergeBranchesBySquash");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !MergeBranchesBySquashOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(MergeBranchesBySquashOutput, body, allocator);
}
