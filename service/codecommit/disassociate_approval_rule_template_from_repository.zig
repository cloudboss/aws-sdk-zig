...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateApprovalRuleTemplateFromRepositoryInput, options: Options) !DisassociateApprovalRuleTemplateFromRepositoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateApprovalRuleTemplateFromRepositoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.DisassociateApprovalRuleTemplateFromRepository");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateApprovalRuleTemplateFromRepositoryOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
