...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateApprovalRuleTemplateNameInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.UpdateApprovalRuleTemplateName");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateApprovalRuleTemplateNameOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateApprovalRuleTemplateNameOutput, body, alloc);
}
