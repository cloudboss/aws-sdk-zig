...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutFileInput, options: Options) !PutFileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutFileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.PutFile");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutFileOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutFileOutput, body, alloc);
}
