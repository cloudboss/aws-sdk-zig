...
pub const MergeBranchesByThreeWayInput = struct {
...
pub const MergeBranchesByThreeWayOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MergeBranchesByThreeWayInput, options: CallOptions) !MergeBranchesByThreeWayOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: MergeBranchesByThreeWayInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.MergeBranchesByThreeWay");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !MergeBranchesByThreeWayOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(MergeBranchesByThreeWayOutput, body, allocator);
}
