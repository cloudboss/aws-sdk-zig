...
pub const UpdateDefaultBranchInput = struct {
    /// The name of the branch to set as the default branch.
    default_branch_name: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDefaultBranchInput, options: CallOptions) !UpdateDefaultBranchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDefaultBranchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.UpdateDefaultBranch");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDefaultBranchOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
