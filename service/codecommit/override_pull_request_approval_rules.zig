...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: OverridePullRequestApprovalRulesInput, options: Options) !OverridePullRequestApprovalRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: OverridePullRequestApprovalRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.OverridePullRequestApprovalRules");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !OverridePullRequestApprovalRulesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
