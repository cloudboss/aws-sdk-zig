...
pub const UpdatePullRequestTitleInput = struct {
...
pub const UpdatePullRequestTitleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePullRequestTitleInput, options: CallOptions) !UpdatePullRequestTitleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePullRequestTitleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.UpdatePullRequestTitle");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePullRequestTitleOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdatePullRequestTitleOutput, body, allocator);
}
