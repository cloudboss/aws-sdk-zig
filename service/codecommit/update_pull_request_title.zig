...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePullRequestTitleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.UpdatePullRequestTitle");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePullRequestTitleOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdatePullRequestTitleOutput, body, alloc);
}
