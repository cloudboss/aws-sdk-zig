...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePullRequestApprovalRuleInput, options: Options) !CreatePullRequestApprovalRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePullRequestApprovalRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.CreatePullRequestApprovalRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePullRequestApprovalRuleOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreatePullRequestApprovalRuleOutput, body, alloc);
}
