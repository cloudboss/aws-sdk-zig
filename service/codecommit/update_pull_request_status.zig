...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePullRequestStatusInput, options: Options) !UpdatePullRequestStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePullRequestStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.UpdatePullRequestStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePullRequestStatusOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdatePullRequestStatusOutput, body, alloc);
}
