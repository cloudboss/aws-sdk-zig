...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCommentReactionsInput, options: Options) !GetCommentReactionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCommentReactionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.GetCommentReactions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCommentReactionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetCommentReactionsOutput, body, alloc);
}
