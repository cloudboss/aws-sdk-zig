...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRepositoryEncryptionKeyInput, options: Options) !UpdateRepositoryEncryptionKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRepositoryEncryptionKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.UpdateRepositoryEncryptionKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRepositoryEncryptionKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateRepositoryEncryptionKeyOutput, body, alloc);
}
