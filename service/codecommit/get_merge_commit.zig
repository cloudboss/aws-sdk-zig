...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMergeCommitInput, options: Options) !GetMergeCommitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMergeCommitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.GetMergeCommit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMergeCommitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMergeCommitOutput, body, alloc);
}
