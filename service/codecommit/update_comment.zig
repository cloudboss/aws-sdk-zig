...
pub const UpdateCommentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCommentInput, options: CallOptions) !UpdateCommentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateCommentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.UpdateComment");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateCommentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCommentOutput, body, allocator);
}
