...
pub const GetRepositoryTriggersInput = struct {
...
pub const GetRepositoryTriggersOutput = struct {
    /// The system-generated unique ID for the trigger.
    configuration_id: ?[]const u8 = null,

    /// The JSON block of configuration information for each trigger.
    triggers: ?[]const RepositoryTrigger = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRepositoryTriggersInput, options: CallOptions) !GetRepositoryTriggersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRepositoryTriggersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeCommit_20150413.GetRepositoryTriggers");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRepositoryTriggersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRepositoryTriggersOutput, body, allocator);
}
