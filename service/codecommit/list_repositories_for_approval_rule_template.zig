...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRepositoriesForApprovalRuleTemplateInput, options: Options) !ListRepositoriesForApprovalRuleTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRepositoriesForApprovalRuleTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.ListRepositoriesForApprovalRuleTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRepositoriesForApprovalRuleTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRepositoriesForApprovalRuleTemplateOutput, body, alloc);
}
