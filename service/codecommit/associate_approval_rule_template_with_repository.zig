...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateApprovalRuleTemplateWithRepositoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.AssociateApprovalRuleTemplateWithRepository");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateApprovalRuleTemplateWithRepositoryOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
