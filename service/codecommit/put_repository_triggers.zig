...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRepositoryTriggersInput, options: Options) !PutRepositoryTriggersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRepositoryTriggersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeCommit_20150413.PutRepositoryTriggers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRepositoryTriggersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutRepositoryTriggersOutput, body, alloc);
}
