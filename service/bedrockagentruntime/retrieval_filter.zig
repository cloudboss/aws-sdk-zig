...
/// configurations](https://docs.aws.amazon.com/bedrock/latest/userguide/kb-test-config.html). See the examples below to see how to use these filters.
///
/// This data type is used in the following API operations:
///
/// * [Retrieve
///   request](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_agent-runtime_Retrieve.html#API_agent-runtime_Retrieve_RequestSyntax) – in the `filter` field
/// * [RetrieveAndGenerate
...
pub const RetrievalFilter = union(enum) {
    ...
    ///
    /// The following example would return data sources with an `year` attribute
    /// whose value is less than or equal to `1989`.
    ///
    ...
    ///
    /// * It contains a metadata attribute whose name matches the `key` and whose
    ///   value doesn't match the `value` in this object.
    ...
    /// `value` in this object. This filter is currently only supported for Amazon
    /// OpenSearch Serverless vector stores.
    ///
    /// The following example would return data sources with an `animal` attribute
    /// starts with `ca` (for example, `cat` or `camel`).
    ///
