...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFlowExecutionsInput, options: Options) !ListFlowExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFlowExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.flow_alias_identifier) |v| {
        ...
        try query_buf.appendSlice(alloc, "flowAliasIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFlowExecutionsOutput {
    var result: ListFlowExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFlowExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
