...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutInvocationStepInput, options: Options) !PutInvocationStepOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutInvocationStepInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.invocation_step_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"invocationStepId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"invocationStepTime\":");
    try aws.json.writeValue(@TypeOf(input.invocation_step_time), input.invocation_step_time, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutInvocationStepOutput {
    var result: PutInvocationStepOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutInvocationStepOutput, body, alloc);
    }
    _ = status;
    _ = headers;
