...
pub const GetAgentMemoryInput = struct {
    ...
    /// in the request, enter the token returned in the `nextToken` field in the
...
pub const GetAgentMemoryOutput = struct {
    /// Contains details of the sessions stored in the memory
    memory_contents: ?[]const Memory = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAgentMemoryInput, options: Options) !GetAgentMemoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAgentMemoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.memory_id);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "memoryType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.memory_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAgentMemoryOutput {
    var result: GetAgentMemoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAgentMemoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
