...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAgentMemoryInput, options: CallOptions) !GetAgentMemoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAgentMemoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.memory_id);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
