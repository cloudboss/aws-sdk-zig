...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInvocationInput, options: Options) !CreateInvocationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInvocationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.invocation_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"invocationId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInvocationOutput {
    var result: CreateInvocationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateInvocationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
