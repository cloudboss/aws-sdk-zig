...
const InvocationSummary = @import("invocation_summary.zig").InvocationSummary;

pub const ListInvocationsInput = struct {
...
pub const ListInvocationsOutput = struct {
    /// A list of invocation summaries associated with the session.
    invocation_summaries: ?[]const InvocationSummary = null,
    ...
    pub const json_field_names = .{
        .invocation_summaries = "invocationSummaries",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInvocationsInput, options: CallOptions) !ListInvocationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListInvocationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListInvocationsOutput {
    var result: ListInvocationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInvocationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
