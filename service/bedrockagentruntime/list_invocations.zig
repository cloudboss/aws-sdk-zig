...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInvocationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInvocationsOutput {
    var result: ListInvocationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInvocationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
