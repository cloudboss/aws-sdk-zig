...
pub const DeleteAgentMemoryInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAgentMemoryInput, options: Options) !DeleteAgentMemoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAgentMemoryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.memory_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "memoryId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAgentMemoryOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteAgentMemoryOutput = .{};
