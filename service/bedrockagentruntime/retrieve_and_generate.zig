...
const RetrieveAndGenerateInput = @import("retrieve_and_generate_request.zig").RetrieveAndGenerateRequest;

const RetrieveAndGenerateOutput = @import("retrieve_and_generate_response.zig").RetrieveAndGenerateResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RetrieveAndGenerateInput, options: Options) !RetrieveAndGenerateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RetrieveAndGenerateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RetrieveAndGenerateOutput {
    var result: RetrieveAndGenerateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RetrieveAndGenerateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
