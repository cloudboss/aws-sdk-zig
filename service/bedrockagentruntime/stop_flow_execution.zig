...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopFlowExecutionInput, options: Options) !StopFlowExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopFlowExecutionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopFlowExecutionOutput {
    var result: StopFlowExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopFlowExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
