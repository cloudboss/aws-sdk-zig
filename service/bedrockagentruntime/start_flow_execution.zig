...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFlowExecutionInput, options: CallOptions) !StartFlowExecutionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartFlowExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.flow_execution_name) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"flowExecutionName\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartFlowExecutionOutput {
    var result: StartFlowExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartFlowExecutionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
