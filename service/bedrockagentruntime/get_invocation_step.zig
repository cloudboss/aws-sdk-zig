...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInvocationStepInput, options: Options) !GetInvocationStepOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInvocationStepInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.session_identifier);
    try path_buf.appendSlice(alloc, "/invocationSteps/");
    try path_buf.appendSlice(alloc, input.invocation_step_id);
    ...
    try body_buf.appendSlice(alloc, "\"invocationIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.invocation_identifier), input.invocation_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInvocationStepOutput {
    var result: GetInvocationStepOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInvocationStepOutput, body, alloc);
    }
    _ = status;
    _ = headers;
