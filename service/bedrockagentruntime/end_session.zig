...
pub const EndSessionInput = struct {
...
pub const EndSessionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EndSessionInput, options: CallOptions) !EndSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EndSessionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EndSessionOutput {
    var result: EndSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EndSessionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
