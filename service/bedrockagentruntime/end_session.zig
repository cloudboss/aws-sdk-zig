...
fn serializeRequest(alloc: std.mem.Allocator, input: EndSessionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EndSessionOutput {
    var result: EndSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EndSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
