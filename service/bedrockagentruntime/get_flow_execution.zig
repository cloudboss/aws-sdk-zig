...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFlowExecutionInput, options: Options) !GetFlowExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFlowExecutionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFlowExecutionOutput {
    var result: GetFlowExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
