...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExecutionFlowSnapshotInput, options: CallOptions) !GetExecutionFlowSnapshotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetExecutionFlowSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/flowsnapshot");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetExecutionFlowSnapshotOutput {
    var result: GetExecutionFlowSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetExecutionFlowSnapshotOutput, body, allocator);
    }
    _ = status;
    _ = headers;
