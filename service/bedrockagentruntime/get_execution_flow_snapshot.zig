...
pub const GetExecutionFlowSnapshotInput = struct {
...
pub const GetExecutionFlowSnapshotOutput = struct {
    ...
    /// [FlowDefinition](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_agent_FlowDefinition.html) object.
    definition: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExecutionFlowSnapshotInput, options: Options) !GetExecutionFlowSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExecutionFlowSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.execution_identifier);
    try path_buf.appendSlice(alloc, "/flowsnapshot");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExecutionFlowSnapshotOutput {
    var result: GetExecutionFlowSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetExecutionFlowSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
