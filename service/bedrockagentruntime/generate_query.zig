...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateQueryInput, options: Options) !GenerateQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"queryGenerationInput\":");
    try aws.json.writeValue(@TypeOf(input.query_generation_input), input.query_generation_input, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"transformationConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.transformation_configuration), input.transformation_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateQueryOutput {
    var result: GenerateQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateQueryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
