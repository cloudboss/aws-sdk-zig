...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInvocationStepsInput, options: Options) !ListInvocationStepsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInvocationStepsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInvocationStepsOutput {
    var result: ListInvocationStepsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInvocationStepsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
