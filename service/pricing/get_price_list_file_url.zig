...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPriceListFileUrlInput, options: Options) !GetPriceListFileUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPriceListFileUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPriceListService.GetPriceListFileUrl");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPriceListFileUrlOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPriceListFileUrlOutput, body, alloc);
}
