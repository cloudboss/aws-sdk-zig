...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPriceListsInput, options: Options) !ListPriceListsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPriceListsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPriceListService.ListPriceLists");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPriceListsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPriceListsOutput, body, alloc);
}
