...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProductsInput, options: Options) !GetProductsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProductsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPriceListService.GetProducts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProductsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetProductsOutput, body, alloc);
}
