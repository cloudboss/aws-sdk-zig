...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeServicesInput, options: Options) !DescribeServicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeServicesInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("pricing", "Pricing", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPriceListService.DescribeServices");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeServicesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeServicesOutput, body, alloc);
}
