...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAttributeValuesInput, options: Options) !GetAttributeValuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAttributeValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPriceListService.GetAttributeValues");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAttributeValuesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAttributeValuesOutput, body, alloc);
}
