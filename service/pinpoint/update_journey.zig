...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateJourneyInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.write_journey_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateJourneyOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
