...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGcmChannelInput, options: Options) !UpdateGcmChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGcmChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/channels/gcm");
    const path = try path_buf.toOwnedSlice(alloc);

    const body = try aws.json.jsonStringify(input.gcm_channel_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGcmChannelOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
