...
pub const UpdateApnsChannelInput = struct {
...
pub const UpdateApnsChannelOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateApnsChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.apns_channel_request, allocator);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateApnsChannelOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateApnsChannelOutput = .{};
