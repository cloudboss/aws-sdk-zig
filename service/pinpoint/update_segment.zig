...
pub const UpdateSegmentInput = struct {
...
pub const UpdateSegmentOutput = struct {
    segment_response: ?SegmentResponse = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSegmentInput, options: CallOptions) !UpdateSegmentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSegmentInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.write_segment_request, allocator);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateSegmentOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateSegmentOutput = .{};
