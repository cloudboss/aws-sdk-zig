...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSegmentInput, options: Options) !UpdateSegmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSegmentInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.write_segment_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSegmentOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
