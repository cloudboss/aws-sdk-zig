...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEndpointsBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.endpoint_batch_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEndpointsBatchOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
