...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCampaignDateRangeKpiInput, options: Options) !GetCampaignDateRangeKpiOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCampaignDateRangeKpiInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/campaigns/");
    try path_buf.appendSlice(alloc, input.campaign_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCampaignDateRangeKpiOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
