...
const VoiceTemplateRequest = @import("voice_template_request.zig").VoiceTemplateRequest;
const MessageBody = @import("message_body.zig").MessageBody;

pub const UpdateVoiceTemplateInput = struct {
...
pub const UpdateVoiceTemplateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVoiceTemplateInput, options: CallOptions) !UpdateVoiceTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateVoiceTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/voice");
    ...
    const body = try aws.json.jsonStringify(input.voice_template_request, allocator);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateVoiceTemplateOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateVoiceTemplateOutput = .{};
