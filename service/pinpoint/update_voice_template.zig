...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVoiceTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/voice");
    ...
    const body = try aws.json.jsonStringify(input.voice_template_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVoiceTemplateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
