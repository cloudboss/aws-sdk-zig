...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateApnsVoipSandboxChannelInput, options: Options) !UpdateApnsVoipSandboxChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateApnsVoipSandboxChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/channels/apns_voip_sandbox");
    const path = try path_buf.toOwnedSlice(alloc);

    const body = try aws.json.jsonStringify(input.apns_voip_sandbox_channel_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateApnsVoipSandboxChannelOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
