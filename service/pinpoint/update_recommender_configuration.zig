...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRecommenderConfigurationInput, options: Options) !UpdateRecommenderConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRecommenderConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/recommenders/");
    try path_buf.appendSlice(alloc, input.recommender_id);
    const path = try path_buf.toOwnedSlice(alloc);

    const body = try aws.json.jsonStringify(input.update_recommender_configuration, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRecommenderConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
