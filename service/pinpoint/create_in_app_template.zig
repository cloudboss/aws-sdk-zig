...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInAppTemplateInput, options: Options) !CreateInAppTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInAppTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.in_app_template_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInAppTemplateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
