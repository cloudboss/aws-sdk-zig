...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePushTemplateInput, options: Options) !CreatePushTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePushTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/push");
    const path = try path_buf.toOwnedSlice(alloc);

    const body = try aws.json.jsonStringify(input.push_notification_template_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePushTemplateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
