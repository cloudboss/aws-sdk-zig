...
const EventsRequest = @import("events_request.zig").EventsRequest;
const EventsResponse = @import("events_response.zig").EventsResponse;
...
pub const PutEventsOutput = struct {
    events_response: ?EventsResponse = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.events_request, allocator);
