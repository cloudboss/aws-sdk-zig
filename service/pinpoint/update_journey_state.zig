...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateJourneyStateInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.journey_state_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateJourneyStateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
