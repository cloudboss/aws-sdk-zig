...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendOTPMessageInput, options: Options) !SendOTPMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendOTPMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/otp");
    const path = try path_buf.toOwnedSlice(alloc);

    const body = try aws.json.jsonStringify(input.send_otp_message_request_parameters, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendOTPMessageOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
