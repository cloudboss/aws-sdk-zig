...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVoiceChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/channels/voice");
    const path = try path_buf.toOwnedSlice(alloc);

    const body = try aws.json.jsonStringify(input.voice_channel_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVoiceChannelOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
