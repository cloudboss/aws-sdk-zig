...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendUsersMessagesInput, options: Options) !SendUsersMessagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendUsersMessagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/users-messages");
    const path = try path_buf.toOwnedSlice(alloc);

    const body = try aws.json.jsonStringify(input.send_users_message_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendUsersMessagesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
