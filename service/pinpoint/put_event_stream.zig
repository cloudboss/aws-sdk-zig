...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEventStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.write_event_stream, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEventStreamOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
