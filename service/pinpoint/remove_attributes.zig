...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveAttributesInput, options: Options) !RemoveAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.update_attributes_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
