...
fn serializeRequest(alloc: std.mem.Allocator, input: SendMessagesInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.message_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendMessagesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
