...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSegmentImportJobsInput, options: Options) !GetSegmentImportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSegmentImportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/jobs/import");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSegmentImportJobsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
