...
pub const DeleteEventStreamInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEventStreamInput, options: Options) !DeleteEventStreamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteEventStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/eventstream");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteEventStreamOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteEventStreamOutput = .{};
