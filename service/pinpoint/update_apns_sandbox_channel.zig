...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateApnsSandboxChannelInput, options: Options) !UpdateApnsSandboxChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateApnsSandboxChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/channels/apns_sandbox");
    const path = try path_buf.toOwnedSlice(alloc);

    const body = try aws.json.jsonStringify(input.apns_sandbox_channel_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateApnsSandboxChannelOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
