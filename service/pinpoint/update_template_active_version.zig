...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTemplateActiveVersionInput, options: Options) !UpdateTemplateActiveVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTemplateActiveVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/active-version");
    const path = try path_buf.toOwnedSlice(alloc);

    const body = try aws.json.jsonStringify(input.template_active_version_request, alloc);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTemplateActiveVersionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
