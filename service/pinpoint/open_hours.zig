...
pub const OpenHours = struct {
    ...
    custom: ?[]const aws.map.MapEntry([]const OpenHoursRule) = null,

    /// Specifies the schedule settings for the email channel.
    email: ?[]const aws.map.MapEntry([]const OpenHoursRule) = null,

    /// Specifies the schedule settings for the push channel.
    push: ?[]const aws.map.MapEntry([]const OpenHoursRule) = null,

    /// Specifies the schedule settings for the SMS channel.
    sms: ?[]const aws.map.MapEntry([]const OpenHoursRule) = null,

    /// Specifies the schedule settings for the voice channel.
    voice: ?[]const aws.map.MapEntry([]const OpenHoursRule) = null,
