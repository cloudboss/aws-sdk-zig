...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCaseRuleInput, options: Options) !CreateCaseRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCaseRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCaseRuleOutput {
    var result: CreateCaseRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCaseRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
