...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCaseOutput {
    var result: GetCaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
