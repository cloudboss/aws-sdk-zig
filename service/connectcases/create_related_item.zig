...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRelatedItemInput, options: Options) !CreateRelatedItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRelatedItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/related-items/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRelatedItemOutput {
    var result: CreateRelatedItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRelatedItemOutput, body, alloc);
    }
    _ = status;
    _ = headers;
