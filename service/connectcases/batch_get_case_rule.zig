...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetCaseRuleInput, options: Options) !BatchGetCaseRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetCaseRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/rules-batch");
    ...
    try body_buf.appendSlice(alloc, "\"caseRules\":");
    try aws.json.writeValue(@TypeOf(input.case_rules), input.case_rules, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetCaseRuleOutput {
    var result: BatchGetCaseRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetCaseRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
