...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFieldInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/fields");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFieldOutput {
    var result: CreateFieldOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFieldOutput, body, alloc);
    }
    _ = status;
    _ = headers;
