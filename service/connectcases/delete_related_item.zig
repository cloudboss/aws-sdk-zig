...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRelatedItemInput, options: Options) !DeleteRelatedItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRelatedItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.related_item_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRelatedItemOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
