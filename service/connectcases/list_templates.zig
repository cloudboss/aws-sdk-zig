...
pub const ListTemplatesInput = struct {
    ...
    status: ?[]const TemplateStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTemplatesInput, options: Options) !ListTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/templates-list");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTemplatesOutput {
    var result: ListTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
