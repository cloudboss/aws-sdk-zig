...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLayoutInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLayoutOutput {
    var result: GetLayoutOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLayoutOutput, body, alloc);
    }
    _ = status;
    _ = headers;
