...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetFieldInput, options: Options) !BatchGetFieldOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetFieldInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/fields-batch");
    ...
    try aws.json.writeValue(@TypeOf(input.fields), input.fields, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetFieldOutput {
    var result: BatchGetFieldOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetFieldOutput, body, alloc);
    }
    _ = status;
    _ = headers;
