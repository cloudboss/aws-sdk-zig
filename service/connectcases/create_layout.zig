...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLayoutInput, options: Options) !CreateLayoutOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLayoutInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLayoutOutput {
    var result: CreateLayoutOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLayoutOutput, body, alloc);
    }
    _ = status;
    _ = headers;
