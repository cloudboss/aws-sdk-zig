...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLayoutsInput, options: Options) !ListLayoutsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLayoutsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/layouts-list");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLayoutsOutput {
    var result: ListLayoutsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLayoutsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
