...
pub const UpdateContributorInsightsOutput = struct {
    ...
    pub fn deinit(self: *const UpdateContributorInsightsOutput) void {
...
pub fn execute(client: *Client, input: UpdateContributorInsightsInput, options: Options) !UpdateContributorInsightsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContributorInsightsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.UpdateContributorInsights");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContributorInsightsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{ .allocator = alloc };
    return aws.json.parseJsonObject(UpdateContributorInsightsOutput, body, alloc);
}
