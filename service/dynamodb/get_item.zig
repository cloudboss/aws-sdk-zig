...
pub const GetItemOutput = struct {
    ...
    pub fn deinit(self: *const GetItemOutput) void {
        if (self.item) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetItemOutput {
    _ = status;
    var result: GetItemOutput = .{ .allocator = alloc };
    if (findJsonValue(body, "Item")) |content| {
        result.item = try alloc.dupe(u8, content);
    }
