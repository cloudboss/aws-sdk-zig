...
pub const UntagResourceOutput = struct {
    ...
    pub fn deinit(self: *const UntagResourceOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.UntagResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !UntagResourceOutput {
    _ = status;
    _ = body;
