...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ExportArn\":\"");
    try appendJsonEscaped(alloc, &body_buf, input.export_arn);
    try body_buf.appendSlice(alloc, "\"");
    has_prev = true;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.DescribeExport");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeExportOutput {
    ...
    return aws.json.parseJsonObject(DescribeExportOutput, body, alloc);
}
