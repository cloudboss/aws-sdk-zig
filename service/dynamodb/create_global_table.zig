...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGlobalTableInput, options: CallOptions) !CreateGlobalTableOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateGlobalTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.CreateGlobalTable");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateGlobalTableOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateGlobalTableOutput, body, allocator);
}
