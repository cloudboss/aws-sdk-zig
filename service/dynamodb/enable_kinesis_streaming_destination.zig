...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableKinesisStreamingDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.EnableKinesisStreamingDestination");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableKinesisStreamingDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(EnableKinesisStreamingDestinationOutput, body, allocator);
}
