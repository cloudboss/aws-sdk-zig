...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTimeToLiveInput, options: CallOptions) !UpdateTimeToLiveOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTimeToLiveInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.UpdateTimeToLive");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTimeToLiveOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateTimeToLiveOutput, body, allocator);
}
