...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateItemInput, options: CallOptions) !UpdateItemOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.UpdateItem");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateItemOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateItemOutput, body, allocator);
}
