...
pub const UpdateContinuousBackupsOutput = struct {
    ...
    pub fn deinit(self: *UpdateContinuousBackupsOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: UpdateContinuousBackupsInput, options: Options) !UpdateContinuousBackupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContinuousBackupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.UpdateContinuousBackups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContinuousBackupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateContinuousBackupsOutput, body, alloc);
}
