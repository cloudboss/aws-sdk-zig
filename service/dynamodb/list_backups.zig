...
pub const ListBackupsOutput = struct {
    ...
    pub fn deinit(self: *const ListBackupsOutput) void {
        if (self.last_evaluated_backup_arn) |v| {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBackupsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.time_range_lower_bound) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TimeRangeLowerBound\":");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try body_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.time_range_upper_bound) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TimeRangeUpperBound\":");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try body_buf.appendSlice(alloc, num_str);
        }
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.ListBackups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBackupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{ .allocator = alloc };
    return aws.json.parseJsonObject(ListBackupsOutput, body, alloc);
}
