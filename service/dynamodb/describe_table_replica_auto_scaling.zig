...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTableReplicaAutoScalingInput, options: CallOptions) !DescribeTableReplicaAutoScalingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTableReplicaAutoScalingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.DescribeTableReplicaAutoScaling");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTableReplicaAutoScalingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTableReplicaAutoScalingOutput, body, allocator);
}
