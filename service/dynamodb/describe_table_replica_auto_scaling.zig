...
pub fn execute(client: *Client, input: DescribeTableReplicaAutoScalingInput, options: Options) !DescribeTableReplicaAutoScalingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTableReplicaAutoScalingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.DescribeTableReplicaAutoScaling");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeTableReplicaAutoScalingOutput {
    _ = status;
    _ = body;
