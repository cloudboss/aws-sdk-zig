...
pub fn execute(client: *Client, input: DescribeTableInput, options: Options) !DescribeTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.DescribeTable");

    return request;
}

fn deserializeResponse(body: []const u8, alloc: std.mem.Allocator) !DescribeTableOutput {
    _ = body;
