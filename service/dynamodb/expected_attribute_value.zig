...
/// not, the
/// operation fails. You can use `ExpectedAttributeValue` in one of two
/// different
/// ways:
///
...
pub const ExpectedAttributeValue = struct {
    ...
    /// attribute of the comparison is of type String, then the operator checks for
    /// a
    /// substring match. If the target attribute of the comparison is of type
    /// Binary,
    /// then the operator looks for a subsequence of the target that matches the
    /// input.
    ...
    /// any
    /// elements of the input are equal to the item attribute, the expression
    /// evaluates
    /// to true.
    ///
    ...
    /// type). A
    /// target attribute matches if the target value is greater than, or equal to,
    /// the
    ...
    /// see if that attribute value already exists in the table. If it is found,
    /// then
