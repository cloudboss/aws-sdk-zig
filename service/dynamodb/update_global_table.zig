...
pub const UpdateGlobalTableOutput = struct {
    ...
    pub fn deinit(self: *UpdateGlobalTableOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: UpdateGlobalTableInput, options: Options) !UpdateGlobalTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGlobalTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.UpdateGlobalTable");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGlobalTableOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateGlobalTableOutput, body, alloc);
}
