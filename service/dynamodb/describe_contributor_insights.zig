...
pub const DescribeContributorInsightsOutput = struct {
    ...
    pub fn deinit(self: *const DescribeContributorInsightsOutput) void {
        if (self.index_name) |v| {
...
pub fn execute(client: *Client, input: DescribeContributorInsightsInput, options: Options) !DescribeContributorInsightsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeContributorInsightsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.DescribeContributorInsights");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeContributorInsightsOutput {
    _ = status;
