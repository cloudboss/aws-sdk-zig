...
pub fn execute(client: *Client, input: DescribeContinuousBackupsInput, options: Options) !DescribeContinuousBackupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeContinuousBackupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.DescribeContinuousBackups");

    return request;
}

fn deserializeResponse(body: []const u8, alloc: std.mem.Allocator) !DescribeContinuousBackupsOutput {
    _ = body;
