...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContinuousBackupsInput, options: CallOptions) !DescribeContinuousBackupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeContinuousBackupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.DescribeContinuousBackups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeContinuousBackupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeContinuousBackupsOutput, body, allocator);
}
