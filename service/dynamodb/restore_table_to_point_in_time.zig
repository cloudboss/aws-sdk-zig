...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreTableToPointInTimeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.source_table_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SourceTableArn\":\"");
        try appendJsonEscaped(alloc, &body_buf, v);
        try body_buf.appendSlice(alloc, "\"");
        has_prev = true;
    }
    if (input.source_table_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.RestoreTableToPointInTime");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreTableToPointInTimeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{ .allocator = alloc };
    return aws.json.parseJsonObject(RestoreTableToPointInTimeOutput, body, alloc);
}
