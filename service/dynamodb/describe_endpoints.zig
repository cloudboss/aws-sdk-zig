...
pub fn execute(client: *Client, input: DescribeEndpointsInput, options: Options) !DescribeEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.DescribeEndpoints");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeEndpointsOutput {
    _ = status;
    _ = body;
    const result: DescribeEndpointsOutput = .{ .allocator = alloc };
