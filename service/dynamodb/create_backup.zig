...
pub fn execute(client: *Client, input: CreateBackupInput, options: Options) !CreateBackupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBackupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BackupName\":\"");
    try appendJsonEscaped(alloc, &body_buf, input.backup_name);
    try body_buf.appendSlice(alloc, "\"");
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.CreateBackup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBackupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{ .allocator = alloc };
    return aws.json.parseJsonObject(CreateBackupOutput, body, alloc);
}
