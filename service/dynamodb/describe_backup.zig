...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBackupInput, options: CallOptions) !DescribeBackupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeBackupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.DescribeBackup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeBackupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeBackupOutput, body, allocator);
}
