...
pub const UpdateGlobalTableSettingsOutput = struct {
    ...
    pub fn deinit(self: *const UpdateGlobalTableSettingsOutput) void {
        if (self.global_table_name) |v| {
...
pub fn execute(client: *Client, input: UpdateGlobalTableSettingsInput, options: Options) !UpdateGlobalTableSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGlobalTableSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"");
    has_prev = true;
    if (input.global_table_provisioned_write_capacity_units) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"GlobalTableProvisionedWriteCapacityUnits\":");
        {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.UpdateGlobalTableSettings");
