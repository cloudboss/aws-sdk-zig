...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGlobalTableInput, options: CallOptions) !DescribeGlobalTableOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "dynamodb");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeGlobalTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.DescribeGlobalTable");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeGlobalTableOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeGlobalTableOutput, body, allocator);
}
