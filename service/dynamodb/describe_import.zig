...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeImportInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ImportArn\":\"");
    try appendJsonEscaped(alloc, &body_buf, input.import_arn);
    try body_buf.appendSlice(alloc, "\"");
    has_prev = true;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.DescribeImport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeImportOutput {
    _ = status;
    _ = body;
    const result: DescribeImportOutput = .{ .allocator = alloc };
