...
pub const ListExportsOutput = struct {
    ...
    pub fn deinit(self: *const ListExportsOutput) void {
...
pub fn execute(client: *Client, input: ListExportsInput, options: Options) !ListExportsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.ListExports");

    return request;
}

fn deserializeResponse(body: []const u8, alloc: std.mem.Allocator) !ListExportsOutput {
