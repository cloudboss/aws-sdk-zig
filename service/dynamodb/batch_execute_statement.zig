...
pub fn execute(client: *Client, input: BatchExecuteStatementInput, options: Options) !BatchExecuteStatementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchExecuteStatementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.BatchExecuteStatement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !BatchExecuteStatementOutput {
    _ = status;
    _ = body;
