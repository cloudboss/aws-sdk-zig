...
pub const ListContributorInsightsOutput = struct {
    ...
    pub fn deinit(self: *const ListContributorInsightsOutput) void {
...
pub fn execute(client: *Client, input: ListContributorInsightsInput, options: Options) !ListContributorInsightsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContributorInsightsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.ListContributorInsights");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContributorInsightsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{ .allocator = alloc };
    return aws.json.parseJsonObject(ListContributorInsightsOutput, body, alloc);
}
