...
pub const AutoScalingTargetTrackingScalingPolicyConfigurationDescription = struct {
    /// Indicates whether scale in by the target tracking policy is disabled. If the
    /// value is
    /// true, scale in is disabled and the target tracking policy won't remove
    /// capacity from the
    ...
    /// another
    /// scale in activity can start. The cooldown period is used to block subsequent
    /// scale in
    /// requests until it has expired. You should scale in conservatively to protect
    /// your
    ...
    /// another
    /// scale out activity can start. While the cooldown period is in effect, the
    /// capacity that
    /// has been added by the previous scale out event that initiated the cooldown
    /// is calculated
