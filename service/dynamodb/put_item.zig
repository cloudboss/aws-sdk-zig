...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutItemInput, options: CallOptions) !PutItemOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutItemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.PutItem");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutItemOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutItemOutput, body, allocator);
}
