...
pub fn execute(client: *Client, input: DeleteBackupInput, options: Options) !DeleteBackupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBackupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.DeleteBackup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeleteBackupOutput {
    _ = status;
    _ = body;
    const result: DeleteBackupOutput = .{ .allocator = alloc };
