...
pub const UpdateKinesisStreamingDestinationOutput = struct {
    ...
    pub fn deinit(self: *const UpdateKinesisStreamingDestinationOutput) void {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKinesisStreamingDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "DynamoDB_20120810.UpdateKinesisStreamingDestination");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKinesisStreamingDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{ .allocator = alloc };
    return aws.json.parseJsonObject(UpdateKinesisStreamingDestinationOutput, body, alloc);
}
