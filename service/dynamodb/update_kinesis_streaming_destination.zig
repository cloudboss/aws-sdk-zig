...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKinesisStreamingDestinationInput, options: CallOptions) !UpdateKinesisStreamingDestinationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateKinesisStreamingDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "DynamoDB_20120810.UpdateKinesisStreamingDestination");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateKinesisStreamingDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateKinesisStreamingDestinationOutput, body, allocator);
}
