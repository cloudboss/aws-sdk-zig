...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeploymentEventsInput, options: Options) !ListDeploymentEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDeploymentEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDeploymentEventsOutput {
    var result: ListDeploymentEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDeploymentEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
