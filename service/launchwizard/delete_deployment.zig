...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDeploymentInput, options: Options) !DeleteDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDeploymentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDeploymentOutput {
    var result: DeleteDeploymentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDeploymentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
