...
const DeploymentDataSummary = @import("deployment_data_summary.zig").DeploymentDataSummary;
...
pub const UpdateDeploymentOutput = struct {
    /// The deployment.
    deployment: ?DeploymentDataSummary = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (input.deployment_pattern_version_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"specifications\":");
    try aws.json.writeValue(@TypeOf(input.specifications), input.specifications, alloc, &body_buf);
    has_prev = true;
    if (input.workload_version_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"workloadVersionName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
