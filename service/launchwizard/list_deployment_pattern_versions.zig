...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeploymentPatternVersionsInput, options: Options) !ListDeploymentPatternVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDeploymentPatternVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDeploymentPatternVersionsOutput {
    var result: ListDeploymentPatternVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDeploymentPatternVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
