...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkloadDeploymentPatternInput, options: Options) !GetWorkloadDeploymentPatternOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkloadDeploymentPatternInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("launchwizard", "Launch Wizard", alloc);
    ...
    try body_buf.appendSlice(alloc, "\"deploymentPatternName\":");
    try aws.json.writeValue(@TypeOf(input.deployment_pattern_name), input.deployment_pattern_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkloadDeploymentPatternOutput {
    var result: GetWorkloadDeploymentPatternOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkloadDeploymentPatternOutput, body, alloc);
    }
    _ = status;
    _ = headers;
