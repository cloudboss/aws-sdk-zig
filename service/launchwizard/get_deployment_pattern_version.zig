...
pub const GetDeploymentPatternVersionInput = struct {
...
pub const GetDeploymentPatternVersionOutput = struct {
    /// The deployment pattern version.
    deployment_pattern_version: ?DeploymentPatternVersionDataSummary = null,

    pub const json_field_names = .{
        .deployment_pattern_version = "deploymentPatternVersion",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeploymentPatternVersionInput, options: CallOptions) !GetDeploymentPatternVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDeploymentPatternVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/getDeploymentPatternVersion";
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"deploymentPatternVersionName\":");
    try aws.json.writeValue(@TypeOf(input.deployment_pattern_version_name), input.deployment_pattern_version_name, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDeploymentPatternVersionOutput {
    var result: GetDeploymentPatternVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDeploymentPatternVersionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
