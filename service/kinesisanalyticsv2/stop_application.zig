...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopApplicationInput, options: Options) !StopApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.StopApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopApplicationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopApplicationOutput, body, alloc);
}
