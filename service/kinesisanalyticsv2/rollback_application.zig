...
pub const RollbackApplicationInput = struct {
...
pub const RollbackApplicationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RollbackApplicationInput, options: CallOptions) !RollbackApplicationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RollbackApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "KinesisAnalytics_20180523.RollbackApplication");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RollbackApplicationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(RollbackApplicationOutput, body, allocator);
}
