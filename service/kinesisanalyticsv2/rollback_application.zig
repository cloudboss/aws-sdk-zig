...
fn serializeRequest(alloc: std.mem.Allocator, input: RollbackApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.RollbackApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RollbackApplicationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(RollbackApplicationOutput, body, alloc);
}
