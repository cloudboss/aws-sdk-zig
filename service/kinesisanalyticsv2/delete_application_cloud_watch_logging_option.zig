...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteApplicationCloudWatchLoggingOptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.DeleteApplicationCloudWatchLoggingOption");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteApplicationCloudWatchLoggingOptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteApplicationCloudWatchLoggingOptionOutput, body, alloc);
}
