...
pub const DeleteApplicationCloudWatchLoggingOptionInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteApplicationCloudWatchLoggingOptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "KinesisAnalytics_20180523.DeleteApplicationCloudWatchLoggingOption");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteApplicationCloudWatchLoggingOptionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteApplicationCloudWatchLoggingOptionOutput, body, allocator);
}
