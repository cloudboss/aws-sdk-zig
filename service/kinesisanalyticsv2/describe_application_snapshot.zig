...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeApplicationSnapshotInput, options: Options) !DescribeApplicationSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeApplicationSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.DescribeApplicationSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeApplicationSnapshotOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeApplicationSnapshotOutput, body, alloc);
}
