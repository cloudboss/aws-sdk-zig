...
fn serializeRequest(alloc: std.mem.Allocator, input: AddApplicationInputProcessingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.AddApplicationInputProcessingConfiguration");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddApplicationInputProcessingConfigurationOutput {
    ...
    return aws.json.parseJsonObject(AddApplicationInputProcessingConfigurationOutput, body, alloc);
}
