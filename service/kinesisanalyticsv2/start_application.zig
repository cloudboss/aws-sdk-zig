...
const RunConfiguration = @import("run_configuration.zig").RunConfiguration;
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "KinesisAnalytics_20180523.StartApplication");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartApplicationOutput {
    ...
    return aws.json.parseJsonObject(StartApplicationOutput, body, allocator);
}
