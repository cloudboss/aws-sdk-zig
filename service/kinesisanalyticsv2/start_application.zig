...
fn serializeRequest(alloc: std.mem.Allocator, input: StartApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.StartApplication");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartApplicationOutput {
    ...
    return aws.json.parseJsonObject(StartApplicationOutput, body, alloc);
}
