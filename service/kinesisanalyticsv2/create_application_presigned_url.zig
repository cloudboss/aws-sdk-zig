...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApplicationPresignedUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.CreateApplicationPresignedUrl");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateApplicationPresignedUrlOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateApplicationPresignedUrlOutput, body, alloc);
}
