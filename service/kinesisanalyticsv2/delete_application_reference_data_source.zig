...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteApplicationReferenceDataSourceInput, options: Options) !DeleteApplicationReferenceDataSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteApplicationReferenceDataSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.DeleteApplicationReferenceDataSource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteApplicationReferenceDataSourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteApplicationReferenceDataSourceOutput, body, alloc);
}
