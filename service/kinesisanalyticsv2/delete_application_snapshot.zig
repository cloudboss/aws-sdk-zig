...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteApplicationSnapshotInput, options: Options) !DeleteApplicationSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteApplicationSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.DeleteApplicationSnapshot");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteApplicationSnapshotOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
