...
fn serializeRequest(alloc: std.mem.Allocator, input: AddApplicationCloudWatchLoggingOptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "KinesisAnalytics_20180523.AddApplicationCloudWatchLoggingOption");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddApplicationCloudWatchLoggingOptionOutput {
    ...
    return aws.json.parseJsonObject(AddApplicationCloudWatchLoggingOptionOutput, body, alloc);
}
