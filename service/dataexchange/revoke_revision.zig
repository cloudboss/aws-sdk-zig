...
pub const RevokeRevisionInput = struct {
...
pub const RevokeRevisionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeRevisionInput, options: Options) !RevokeRevisionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RevokeRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/data-sets/");
    try path_buf.appendSlice(alloc, input.data_set_id);
    try path_buf.appendSlice(alloc, "/revisions/");
    try path_buf.appendSlice(alloc, input.revision_id);
    ...
    try body_buf.appendSlice(alloc, "\"RevocationComment\":");
    try aws.json.writeValue(@TypeOf(input.revocation_comment), input.revocation_comment, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RevokeRevisionOutput {
    var result: RevokeRevisionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RevokeRevisionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
