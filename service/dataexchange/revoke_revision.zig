...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeRevisionInput, options: CallOptions) !RevokeRevisionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RevokeRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"RevocationComment\":");
    try aws.json.writeValue(@TypeOf(input.revocation_comment), input.revocation_comment, allocator, &body_buf);
    has_prev = true;
