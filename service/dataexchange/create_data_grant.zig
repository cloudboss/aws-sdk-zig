...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataGrantInput, options: Options) !CreateDataGrantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ends_at) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EndsAt\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"GrantDistributionScope\":");
    try aws.json.writeValue(@TypeOf(input.grant_distribution_scope), input.grant_distribution_scope, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ReceiverPrincipal\":");
    try aws.json.writeValue(@TypeOf(input.receiver_principal), input.receiver_principal, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SourceDataSetId\":");
    try aws.json.writeValue(@TypeOf(input.source_data_set_id), input.source_data_set_id, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataGrantOutput {
    var result: CreateDataGrantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataGrantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
