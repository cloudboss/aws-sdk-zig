...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataSetInput, options: Options) !GetDataSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataSetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataSetOutput {
    var result: GetDataSetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataSetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
