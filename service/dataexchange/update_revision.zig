...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRevisionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRevisionOutput {
    var result: UpdateRevisionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRevisionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
