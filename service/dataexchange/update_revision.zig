...
pub const UpdateRevisionInput = struct {
...
pub const UpdateRevisionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRevisionInput, options: CallOptions) !UpdateRevisionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.finalized) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"Finalized\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRevisionOutput {
    var result: UpdateRevisionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRevisionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
