...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEventActionInput, options: Options) !GetEventActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEventActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEventActionOutput {
    var result: GetEventActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEventActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
