...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptDataGrantInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptDataGrantOutput {
    var result: AcceptDataGrantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptDataGrantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
