...
pub const AcceptDataGrantInput = struct {
...
pub const AcceptDataGrantOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptDataGrantInput, options: CallOptions) !AcceptDataGrantOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptDataGrantInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptDataGrantOutput {
    var result: AcceptDataGrantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptDataGrantOutput, body, allocator);
    }
    _ = status;
    _ = headers;
