...
const DataSetEntry = @import("data_set_entry.zig").DataSetEntry;
...
pub const ListDataSetsOutput = struct {
    ...
    data_sets: ?[]const DataSetEntry = null,
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataSetsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataSetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
