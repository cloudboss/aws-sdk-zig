...
pub const CreateRevisionInput = struct {
...
pub const CreateRevisionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRevisionInput, options: CallOptions) !CreateRevisionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRevisionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateRevisionOutput {
    var result: CreateRevisionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRevisionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
