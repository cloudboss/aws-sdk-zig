...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRevisionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRevisionOutput {
    var result: CreateRevisionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRevisionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
