...
fn serializeRequest(alloc: std.mem.Allocator, input: GetReceivedDataGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/received-data-grants/");
    try path_buf.appendSlice(alloc, input.data_grant_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetReceivedDataGrantOutput {
    var result: GetReceivedDataGrantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetReceivedDataGrantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
