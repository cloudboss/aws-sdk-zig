...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRevisionInput, options: Options) !GetRevisionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRevisionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRevisionOutput {
    var result: GetRevisionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRevisionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
