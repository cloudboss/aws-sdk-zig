...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEventActionInput, options: Options) !CreateEventActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEventActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEventActionOutput {
    var result: CreateEventActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEventActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
