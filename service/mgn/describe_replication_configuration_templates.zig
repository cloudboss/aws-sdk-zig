...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplicationConfigurationTemplatesInput, options: Options) !DescribeReplicationConfigurationTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReplicationConfigurationTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.replication_configuration_template_i_ds) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"replicationConfigurationTemplateIDs\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReplicationConfigurationTemplatesOutput {
    var result: DescribeReplicationConfigurationTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeReplicationConfigurationTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
