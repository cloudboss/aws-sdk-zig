...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartExportInput, options: CallOptions) !StartExportOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartExportInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartExportOutput {
    var result: StartExportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartExportOutput, body, allocator);
    }
    _ = status;
    _ = headers;
