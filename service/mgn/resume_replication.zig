...
const ResumeReplicationOutput = @import("source_server.zig").SourceServer;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResumeReplicationInput, options: Options) !ResumeReplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResumeReplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResumeReplicationOutput {
    var result: ResumeReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ResumeReplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
