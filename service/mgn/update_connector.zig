...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"connectorID\":");
    try aws.json.writeValue(@TypeOf(input.connector_id), input.connector_id, alloc, &body_buf);
    has_prev = true;
    if (input.name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.ssm_command_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ssmCommandConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConnectorOutput {
    var result: UpdateConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConnectorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
