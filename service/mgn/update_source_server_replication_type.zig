...
pub const UpdateSourceServerReplicationTypeInput = struct {
...
const UpdateSourceServerReplicationTypeOutput = @import("source_server.zig").SourceServer;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSourceServerReplicationTypeInput, options: Options) !UpdateSourceServerReplicationTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSourceServerReplicationTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/UpdateSourceServerReplicationType";
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"replicationType\":");
    try aws.json.writeValue(@TypeOf(input.replication_type), input.replication_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSourceServerReplicationTypeOutput {
    var result: UpdateSourceServerReplicationTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSourceServerReplicationTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
