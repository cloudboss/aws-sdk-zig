...
pub const UpdateSourceServerReplicationTypeOutput = @import("source_server.zig").SourceServer;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSourceServerReplicationTypeInput, options: CallOptions) !UpdateSourceServerReplicationTypeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSourceServerReplicationTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"replicationType\":");
    try aws.json.writeValue(@TypeOf(input.replication_type), input.replication_type, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
