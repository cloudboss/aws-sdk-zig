...
fn serializeRequest(alloc: std.mem.Allocator, input: UnarchiveApplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UnarchiveApplicationOutput {
    var result: UnarchiveApplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UnarchiveApplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
