...
pub const UnarchiveApplicationInput = struct {
    /// Account ID.
...
fn serializeRequest(allocator: std.mem.Allocator, input: UnarchiveApplicationInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("mgn", "mgn", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/UnarchiveApplication";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UnarchiveApplicationOutput {
    var result: UnarchiveApplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UnarchiveApplicationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
