...
pub const RemoveTemplateActionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveTemplateActionInput, options: Options) !RemoveTemplateActionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveTemplateActionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/RemoveTemplateAction";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveTemplateActionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: RemoveTemplateActionOutput = .{};
