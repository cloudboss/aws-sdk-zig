...
fn serializeRequest(alloc: std.mem.Allocator, input: ChangeServerLifeCycleStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.life_cycle), input.life_cycle, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ChangeServerLifeCycleStateOutput {
    var result: ChangeServerLifeCycleStateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ChangeServerLifeCycleStateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
