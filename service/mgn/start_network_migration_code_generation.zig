...
const CodeGenerationOutputFormatType = @import("code_generation_output_format_type.zig").CodeGenerationOutputFormatType;

pub const StartNetworkMigrationCodeGenerationInput = struct {
    /// The output format types for code generation, such as CloudFormation or
    /// Terraform.
    code_generation_output_format_types: ?[]const CodeGenerationOutputFormatType = null,
    ...
    pub const json_field_names = .{
        .code_generation_output_format_types = "codeGenerationOutputFormatTypes",
        .network_migration_definition_id = "networkMigrationDefinitionID",
        .network_migration_execution_id = "networkMigrationExecutionID",
    };
};

pub const StartNetworkMigrationCodeGenerationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartNetworkMigrationCodeGenerationInput, options: CallOptions) !StartNetworkMigrationCodeGenerationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartNetworkMigrationCodeGenerationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/network-migration/StartNetworkMigrationCodeGeneration";

    var body_buf: std.ArrayList(u8) = .{};
    var has_prev = false;
    try body_buf.appendSlice(allocator, "{");

    if (input.code_generation_output_format_types) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"codeGenerationOutputFormatTypes\":");
