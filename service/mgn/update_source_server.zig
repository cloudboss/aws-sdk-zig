...
const UpdateSourceServerOutput = @import("source_server.zig").SourceServer;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSourceServerInput, options: Options) !UpdateSourceServerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSourceServerInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connector_action) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"connectorAction\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSourceServerOutput {
    var result: UpdateSourceServerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSourceServerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
