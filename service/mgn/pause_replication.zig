...
pub const PauseReplicationInput = struct {
...
pub const PauseReplicationOutput = @import("source_server.zig").SourceServer;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PauseReplicationInput, options: CallOptions) !PauseReplicationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PauseReplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PauseReplicationOutput {
    var result: PauseReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PauseReplicationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
