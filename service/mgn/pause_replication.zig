...
fn serializeRequest(alloc: std.mem.Allocator, input: PauseReplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PauseReplicationOutput {
    var result: PauseReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PauseReplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
