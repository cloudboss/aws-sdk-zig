...
fn serializeRequest(alloc: std.mem.Allocator, input: PutTemplateActionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.operating_system) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"operatingSystem\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutTemplateActionOutput {
    var result: PutTemplateActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutTemplateActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
