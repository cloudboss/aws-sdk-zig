...
pub const MarkAsArchivedInput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: MarkAsArchivedInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MarkAsArchivedOutput {
    var result: MarkAsArchivedOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(MarkAsArchivedOutput, body, alloc);
    }
    _ = status;
    _ = headers;
