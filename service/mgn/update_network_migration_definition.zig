...
const TargetNetworkUpdate = @import("target_network_update.zig").TargetNetworkUpdate;
const TargetS3ConfigurationUpdate = @import("target_s3_configuration_update.zig").TargetS3ConfigurationUpdate;
...
pub const UpdateNetworkMigrationDefinitionInput = struct {
    ...
    target_network: ?TargetNetworkUpdate = null,

    /// The updated S3 configuration for storing the target network artifacts.
    target_s3_configuration: ?TargetS3ConfigurationUpdate = null,
...
pub const UpdateNetworkMigrationDefinitionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNetworkMigrationDefinitionInput, options: CallOptions) !UpdateNetworkMigrationDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateNetworkMigrationDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/network-migration/UpdateNetworkMigrationDefinition";
    ...
    if (input.target_network) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"targetNetwork\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.target_s3_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateNetworkMigrationDefinitionOutput {
    var result: UpdateNetworkMigrationDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateNetworkMigrationDefinitionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
