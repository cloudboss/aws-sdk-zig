...
const ListExportsRequestFilters = @import("list_exports_request_filters.zig").ListExportsRequestFilters;
const ExportTask = @import("export_task.zig").ExportTask;

pub const ListExportsInput = struct {
    filters: ?ListExportsRequestFilters = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExportsInput, options: Options) !ListExportsOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListExportsOutput {
    var result: ListExportsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListExportsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
