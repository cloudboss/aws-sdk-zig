...
pub const DisconnectFromServiceInput = struct {
...
const DisconnectFromServiceOutput = @import("source_server.zig").SourceServer;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisconnectFromServiceInput, options: Options) !DisconnectFromServiceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisconnectFromServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/DisconnectFromService";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisconnectFromServiceOutput {
    var result: DisconnectFromServiceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisconnectFromServiceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
