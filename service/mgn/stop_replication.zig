...
pub const StopReplicationInput = struct {
...
pub const StopReplicationOutput = @import("source_server.zig").SourceServer;
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopReplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopReplicationOutput {
    var result: StopReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopReplicationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
