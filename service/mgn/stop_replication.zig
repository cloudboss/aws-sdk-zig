...
fn serializeRequest(alloc: std.mem.Allocator, input: StopReplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopReplicationOutput {
    var result: StopReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopReplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
