...
pub const DeleteNetworkMigrationDefinitionInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteNetworkMigrationDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/network-migration/DeleteNetworkMigrationDefinition";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteNetworkMigrationDefinitionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteNetworkMigrationDefinitionOutput = .{};
