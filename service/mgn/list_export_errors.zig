...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExportErrorsInput, options: Options) !ListExportErrorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExportErrorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"exportID\":");
    try aws.json.writeValue(@TypeOf(input.export_id), input.export_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExportErrorsOutput {
    var result: ListExportErrorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListExportErrorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
