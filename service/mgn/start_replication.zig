...
pub const StartReplicationOutput = @import("source_server.zig").SourceServer;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartReplicationOutput {
    var result: StartReplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartReplicationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
