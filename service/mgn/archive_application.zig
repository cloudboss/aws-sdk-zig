...
pub const ArchiveApplicationInput = struct {
    /// Account ID.
...
const ArchiveApplicationOutput = @import("application.zig").Application;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ArchiveApplicationInput, options: Options) !ArchiveApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ArchiveApplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ArchiveApplicationOutput {
    var result: ArchiveApplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ArchiveApplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
