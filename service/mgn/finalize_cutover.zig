...
pub const FinalizeCutoverInput = struct {
...
const FinalizeCutoverOutput = @import("source_server.zig").SourceServer;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: FinalizeCutoverInput, options: Options) !FinalizeCutoverOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: FinalizeCutoverInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !FinalizeCutoverOutput {
    var result: FinalizeCutoverOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(FinalizeCutoverOutput, body, alloc);
    }
    _ = status;
    _ = headers;
