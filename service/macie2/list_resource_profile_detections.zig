...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceProfileDetectionsInput, options: Options) !ListResourceProfileDetectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceProfileDetectionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceProfileDetectionsOutput {
    var result: ListResourceProfileDetectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourceProfileDetectionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
