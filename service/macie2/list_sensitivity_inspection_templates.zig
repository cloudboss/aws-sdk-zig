...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSensitivityInspectionTemplatesInput, options: Options) !ListSensitivityInspectionTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSensitivityInspectionTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSensitivityInspectionTemplatesOutput {
    var result: ListSensitivityInspectionTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSensitivityInspectionTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
