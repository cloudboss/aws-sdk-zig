...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUsageTotalsInput, options: Options) !GetUsageTotalsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUsageTotalsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.time_range) |v| {
        ...
        try query_buf.appendSlice(alloc, "timeRange=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUsageTotalsOutput {
    var result: GetUsageTotalsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUsageTotalsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
