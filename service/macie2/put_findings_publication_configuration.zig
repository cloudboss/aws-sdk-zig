...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutFindingsPublicationConfigurationInput, options: Options) !PutFindingsPublicationConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutFindingsPublicationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.security_hub_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"securityHubConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutFindingsPublicationConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
