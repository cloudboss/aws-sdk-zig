...
pub const UpdateMemberSessionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMemberSessionInput, options: Options) !UpdateMemberSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateMemberSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/macie/members/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateMemberSessionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateMemberSessionOutput = .{};
