...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSensitiveDataOccurrencesAvailabilityInput, options: Options) !GetSensitiveDataOccurrencesAvailabilityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSensitiveDataOccurrencesAvailabilityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/reveal/availability");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSensitiveDataOccurrencesAvailabilityOutput {
    var result: GetSensitiveDataOccurrencesAvailabilityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSensitiveDataOccurrencesAvailabilityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
