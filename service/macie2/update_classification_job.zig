...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateClassificationJobInput, options: Options) !UpdateClassificationJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateClassificationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.job_status), input.job_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateClassificationJobOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
