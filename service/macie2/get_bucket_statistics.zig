...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBucketStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBucketStatisticsOutput {
    var result: GetBucketStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBucketStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
