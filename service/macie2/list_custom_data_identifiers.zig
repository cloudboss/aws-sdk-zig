...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCustomDataIdentifiersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCustomDataIdentifiersOutput {
    var result: ListCustomDataIdentifiersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomDataIdentifiersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
