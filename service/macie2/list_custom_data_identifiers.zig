...
pub const ListCustomDataIdentifiersInput = struct {
...
pub const ListCustomDataIdentifiersOutput = struct {
    /// An array of objects, one for each custom data identifier.
    items: ?[]const CustomDataIdentifierSummary = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomDataIdentifiersInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/custom-data-identifiers/list";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomDataIdentifiersOutput {
    var result: ListCustomDataIdentifiersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomDataIdentifiersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
