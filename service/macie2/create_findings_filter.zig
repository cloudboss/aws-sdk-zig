...
pub const CreateFindingsFilterInput = struct {
    ...
    /// filter.
    ///
    /// A findings filter can have a maximum of 50 tags. Each tag consists of a tag
...
pub const CreateFindingsFilterOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFindingsFilterInput, options: Options) !CreateFindingsFilterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFindingsFilterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFindingsFilterOutput {
    var result: CreateFindingsFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFindingsFilterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
