...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFindingsFilterInput, options: CallOptions) !UpdateFindingsFilterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFindingsFilterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFindingsFilterOutput {
    var result: UpdateFindingsFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFindingsFilterOutput, body, allocator);
    }
    _ = status;
    _ = headers;
