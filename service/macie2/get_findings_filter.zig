...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingsFilterInput, options: Options) !GetFindingsFilterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingsFilterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingsFilterOutput {
    var result: GetFindingsFilterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingsFilterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
