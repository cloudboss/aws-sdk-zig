...
const ResourceProfileArtifact = @import("resource_profile_artifact.zig").ResourceProfileArtifact;

pub const ListResourceProfileArtifactsInput = struct {
...
pub const ListResourceProfileArtifactsOutput = struct {
    ...
    artifacts: ?[]const ResourceProfileArtifact = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceProfileArtifactsInput, options: Options) !ListResourceProfileArtifactsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceProfileArtifactsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/resource-profiles/artifacts";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceProfileArtifactsOutput {
    var result: ListResourceProfileArtifactsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourceProfileArtifactsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
