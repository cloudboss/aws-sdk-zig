...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInvitationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInvitationsOutput {
    var result: CreateInvitationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateInvitationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
