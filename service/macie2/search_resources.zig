...
const SearchResourcesBucketCriteria = @import("search_resources_bucket_criteria.zig").SearchResourcesBucketCriteria;
const SearchResourcesSortCriteria = @import("search_resources_sort_criteria.zig").SearchResourcesSortCriteria;
const MatchingResource = @import("matching_resource.zig").MatchingResource;

pub const SearchResourcesInput = struct {
    /// The filter conditions that determine which S3 buckets to include or exclude
    /// from the query results.
    bucket_criteria: ?SearchResourcesBucketCriteria = null,
    ...
    sort_criteria: ?SearchResourcesSortCriteria = null,
...
pub const SearchResourcesOutput = struct {
    ...
    matching_resources: ?[]const MatchingResource = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/datasources/search-resources";

    var body_buf: std.ArrayList(u8) = .{};
    var has_prev = false;
    try body_buf.appendSlice(allocator, "{");

    if (input.bucket_criteria) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"bucketCriteria\":");
