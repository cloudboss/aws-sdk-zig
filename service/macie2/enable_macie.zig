...
pub const EnableMacieInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableMacieInput, options: Options) !EnableMacieOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableMacieInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableMacieOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: EnableMacieOutput = .{};
