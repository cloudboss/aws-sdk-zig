...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResourceProfileInput, options: Options) !UpdateResourceProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResourceProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.sensitivity_score_override) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"sensitivityScoreOverride\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResourceProfileOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
