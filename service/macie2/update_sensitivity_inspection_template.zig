...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSensitivityInspectionTemplateInput, options: Options) !UpdateSensitivityInspectionTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSensitivityInspectionTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/templates/sensitivity-inspections/");
    ...
    if (input.excludes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"excludes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.includes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"includes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSensitivityInspectionTemplateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
