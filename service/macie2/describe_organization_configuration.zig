...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOrganizationConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOrganizationConfigurationOutput {
    var result: DescribeOrganizationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeOrganizationConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
