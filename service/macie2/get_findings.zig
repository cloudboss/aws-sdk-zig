...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingsInput, options: Options) !GetFindingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingsOutput {
    var result: GetFindingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
