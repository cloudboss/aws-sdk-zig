...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetCustomDataIdentifiersInput, options: Options) !BatchGetCustomDataIdentifiersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetCustomDataIdentifiersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetCustomDataIdentifiersOutput {
    var result: BatchGetCustomDataIdentifiersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetCustomDataIdentifiersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
