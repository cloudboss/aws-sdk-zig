...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMacieSessionInput, options: Options) !GetMacieSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMacieSessionInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMacieSessionOutput {
    var result: GetMacieSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMacieSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
