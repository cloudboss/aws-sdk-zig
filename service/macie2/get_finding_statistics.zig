...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingStatisticsOutput {
    var result: GetFindingStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
