...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCustomDataIdentifierInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/custom-data-identifiers/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCustomDataIdentifierOutput {
    var result: GetCustomDataIdentifierOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCustomDataIdentifierOutput, body, alloc);
    }
    _ = status;
    _ = headers;
