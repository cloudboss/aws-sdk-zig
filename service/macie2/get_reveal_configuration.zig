...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRevealConfigurationInput, options: Options) !GetRevealConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRevealConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRevealConfigurationOutput {
    var result: GetRevealConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRevealConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
