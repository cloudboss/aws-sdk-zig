...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClassificationJobInput, options: CallOptions) !DescribeClassificationJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeClassificationJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeClassificationJobOutput {
    var result: DescribeClassificationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeClassificationJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
