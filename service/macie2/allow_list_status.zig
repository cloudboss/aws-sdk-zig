const AllowListStatusCode = @import("allow_list_status_code.zig").AllowListStatusCode;
...
pub const AllowListStatus = struct {
    ...
    ///   object is encrypted with a customer managed KMS key that Macie isn't
    ///   allowed to use. Check the bucket policy and other permissions settings for
    ///   the bucket and the object. If the object is encrypted, also ensure that
    ///   it's encrypted with a key that Macie is allowed to use.
    ...
    ///   allow list. Try breaking the list into multiple files and ensure that each
    ///   file doesn't exceed any quotas. Then configure list settings in Macie for
    ///   each file.
    ...
    ///   object. If the specified object exists, you're not allowed to access it or
    ///   it's encrypted with an KMS key that you're not allowed to use. Work with
    ///   your Amazon Web Services administrator to ensure that the list's criteria
    ///   specify the correct bucket and object names, and you have read access to
    ///   the bucket and the object. If the object is encrypted, also ensure that
    ///   it's encrypted with a key that you're allowed to use.
    /// * UNKNOWN_ERROR - A transient or internal error occurred when Macie
    ///   attempted to retrieve or parse the object. Wait a few minutes and then try
    ///   again. A list can also have this status if it's encrypted with a key that
