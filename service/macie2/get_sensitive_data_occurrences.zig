...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSensitiveDataOccurrencesInput, options: Options) !GetSensitiveDataOccurrencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSensitiveDataOccurrencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/reveal");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSensitiveDataOccurrencesOutput {
    var result: GetSensitiveDataOccurrencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSensitiveDataOccurrencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
