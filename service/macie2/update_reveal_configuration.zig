...
const UpdateRetrievalConfiguration = @import("update_retrieval_configuration.zig").UpdateRetrievalConfiguration;
const RetrievalConfiguration = @import("retrieval_configuration.zig").RetrievalConfiguration;

pub const UpdateRevealConfigurationInput = struct {
    ...
    retrieval_configuration: ?UpdateRetrievalConfiguration = null,
...
pub const UpdateRevealConfigurationOutput = struct {
    /// The KMS key to use to encrypt the sensitive data, and the status of the
    /// configuration for the Amazon Macie account.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRevealConfigurationInput, options: Options) !UpdateRevealConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRevealConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.retrieval_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"retrievalConfiguration\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRevealConfigurationOutput {
    var result: UpdateRevealConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRevealConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
