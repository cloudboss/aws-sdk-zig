...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRevealConfigurationInput, options: CallOptions) !UpdateRevealConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRevealConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRevealConfigurationOutput {
    var result: UpdateRevealConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRevealConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
