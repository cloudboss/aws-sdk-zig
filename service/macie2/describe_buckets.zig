...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBucketsInput, options: Options) !DescribeBucketsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBucketsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBucketsOutput {
    var result: DescribeBucketsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBucketsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
