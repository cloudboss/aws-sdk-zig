...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSensitivityInspectionTemplateInput, options: Options) !GetSensitivityInspectionTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSensitivityInspectionTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSensitivityInspectionTemplateOutput {
    var result: GetSensitivityInspectionTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSensitivityInspectionTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
