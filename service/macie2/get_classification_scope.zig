...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetClassificationScopeInput, options: Options) !GetClassificationScopeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetClassificationScopeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetClassificationScopeOutput {
    var result: GetClassificationScopeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetClassificationScopeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
