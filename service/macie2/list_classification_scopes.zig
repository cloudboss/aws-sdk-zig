...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListClassificationScopesInput, options: Options) !ListClassificationScopesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListClassificationScopesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListClassificationScopesOutput {
    var result: ListClassificationScopesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListClassificationScopesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
