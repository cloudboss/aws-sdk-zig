...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAllowListInput, options: Options) !UpdateAllowListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAllowListInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAllowListOutput {
    var result: UpdateAllowListOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAllowListOutput, body, alloc);
    }
    _ = status;
    _ = headers;
