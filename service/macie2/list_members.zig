...
const ListMembersInput = @import("list_members_request.zig").ListMembersRequest;

const ListMembersOutput = @import("list_members_response.zig").ListMembersResponse;
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.only_associated) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "onlyAssociated=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
