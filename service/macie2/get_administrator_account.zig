...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAdministratorAccountInput, options: Options) !GetAdministratorAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAdministratorAccountInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAdministratorAccountOutput {
    var result: GetAdministratorAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAdministratorAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
