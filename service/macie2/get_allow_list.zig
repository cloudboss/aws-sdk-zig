...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAllowListInput, options: Options) !GetAllowListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAllowListInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAllowListOutput {
    var result: GetAllowListOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAllowListOutput, body, alloc);
    }
    _ = status;
    _ = headers;
