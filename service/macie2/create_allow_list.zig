...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAllowListInput, options: Options) !CreateAllowListOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAllowListInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAllowListOutput {
    var result: CreateAllowListOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAllowListOutput, body, alloc);
    }
    _ = status;
    _ = headers;
