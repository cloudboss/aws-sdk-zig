...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutomatedDiscoveryAccountsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.account_ids) |v| {
        ...
        try query_buf.appendSlice(alloc, "accountIds=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutomatedDiscoveryAccountsOutput {
    var result: ListAutomatedDiscoveryAccountsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutomatedDiscoveryAccountsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
