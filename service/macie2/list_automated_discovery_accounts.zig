...
const AutomatedDiscoveryAccount = @import("automated_discovery_account.zig").AutomatedDiscoveryAccount;

pub const ListAutomatedDiscoveryAccountsInput = struct {
...
pub const ListAutomatedDiscoveryAccountsOutput = struct {
    ...
    items: ?[]const AutomatedDiscoveryAccount = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutomatedDiscoveryAccountsInput, options: CallOptions) !ListAutomatedDiscoveryAccountsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAutomatedDiscoveryAccountsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.account_ids) |v| {
        for (v) |item| {
            ...
            try query_buf.appendSlice(allocator, "accountIds=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAutomatedDiscoveryAccountsOutput {
    var result: ListAutomatedDiscoveryAccountsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAutomatedDiscoveryAccountsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
