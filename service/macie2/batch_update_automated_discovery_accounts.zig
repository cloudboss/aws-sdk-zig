...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateAutomatedDiscoveryAccountsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateAutomatedDiscoveryAccountsOutput {
    var result: BatchUpdateAutomatedDiscoveryAccountsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateAutomatedDiscoveryAccountsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
