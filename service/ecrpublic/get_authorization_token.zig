...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAuthorizationTokenInput, options: Options) !GetAuthorizationTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAuthorizationTokenInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SpencerFrontendService.GetAuthorizationToken");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAuthorizationTokenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAuthorizationTokenOutput, body, alloc);
}
