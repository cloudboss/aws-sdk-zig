...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRepositoryCatalogDataInput, options: Options) !PutRepositoryCatalogDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRepositoryCatalogDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SpencerFrontendService.PutRepositoryCatalogData");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRepositoryCatalogDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutRepositoryCatalogDataOutput, body, alloc);
}
