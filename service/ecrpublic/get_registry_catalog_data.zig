...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegistryCatalogDataInput, options: CallOptions) !GetRegistryCatalogDataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRegistryCatalogDataInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SpencerFrontendService.GetRegistryCatalogData");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRegistryCatalogDataOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetRegistryCatalogDataOutput, body, allocator);
}
