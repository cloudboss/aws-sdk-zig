...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageTagsInput, options: Options) !DescribeImageTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeImageTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SpencerFrontendService.DescribeImageTags");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeImageTagsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeImageTagsOutput, body, alloc);
}
