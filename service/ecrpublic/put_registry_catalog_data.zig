...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRegistryCatalogDataInput, options: Options) !PutRegistryCatalogDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRegistryCatalogDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SpencerFrontendService.PutRegistryCatalogData");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRegistryCatalogDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutRegistryCatalogDataOutput, body, alloc);
}
