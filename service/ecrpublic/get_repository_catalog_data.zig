...
pub const GetRepositoryCatalogDataInput = struct {
...
pub const GetRepositoryCatalogDataOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRepositoryCatalogDataInput, options: CallOptions) !GetRepositoryCatalogDataOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "ecr-public");
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRepositoryCatalogDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SpencerFrontendService.GetRepositoryCatalogData");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRepositoryCatalogDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRepositoryCatalogDataOutput, body, allocator);
}
