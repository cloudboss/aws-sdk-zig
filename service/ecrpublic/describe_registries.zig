...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRegistriesInput, options: Options) !DescribeRegistriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRegistriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SpencerFrontendService.DescribeRegistries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRegistriesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeRegistriesOutput, body, alloc);
}
