...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InitiateLayerUploadInput, options: Options) !InitiateLayerUploadOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InitiateLayerUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SpencerFrontendService.InitiateLayerUpload");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InitiateLayerUploadOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(InitiateLayerUploadOutput, body, alloc);
}
