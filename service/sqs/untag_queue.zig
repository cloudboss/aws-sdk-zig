...
pub const UntagQueueOutput = struct {
    ...
    pub fn deinit(self: *UntagQueueOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UntagQueueInput, options: Options) !UntagQueueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.UntagQueue");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UntagQueueOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
