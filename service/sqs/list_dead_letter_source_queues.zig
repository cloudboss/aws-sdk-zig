...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeadLetterSourceQueuesInput, options: CallOptions) !ListDeadLetterSourceQueuesOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "sqs");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDeadLetterSourceQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSQS.ListDeadLetterSourceQueues");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDeadLetterSourceQueuesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListDeadLetterSourceQueuesOutput, body, allocator);
}
