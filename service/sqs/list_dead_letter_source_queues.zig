...
pub const ListDeadLetterSourceQueuesOutput = struct {
    ...
    pub fn deinit(self: *ListDeadLetterSourceQueuesOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ListDeadLetterSourceQueuesInput, options: Options) !ListDeadLetterSourceQueuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDeadLetterSourceQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.ListDeadLetterSourceQueues");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDeadLetterSourceQueuesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDeadLetterSourceQueuesOutput, body, alloc);
}
