...
pub const DeleteMessageBatchOutput = struct {
    ...
    pub fn deinit(self: *DeleteMessageBatchOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: DeleteMessageBatchInput, options: Options) !DeleteMessageBatchOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMessageBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.DeleteMessageBatch");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMessageBatchOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteMessageBatchOutput, body, alloc);
}
