...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMessageMoveTasksInput, options: CallOptions) !ListMessageMoveTasksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMessageMoveTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSQS.ListMessageMoveTasks");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMessageMoveTasksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMessageMoveTasksOutput, body, allocator);
}
