...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReceiveMessageInput, options: CallOptions) !ReceiveMessageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ReceiveMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSQS.ReceiveMessage");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ReceiveMessageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ReceiveMessageOutput, body, allocator);
}
