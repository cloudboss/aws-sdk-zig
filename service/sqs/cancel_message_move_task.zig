...
pub const CancelMessageMoveTaskOutput = struct {
    ...
    pub fn deinit(self: *CancelMessageMoveTaskOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: CancelMessageMoveTaskInput, options: Options) !CancelMessageMoveTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelMessageMoveTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.CancelMessageMoveTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelMessageMoveTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelMessageMoveTaskOutput, body, alloc);
}
