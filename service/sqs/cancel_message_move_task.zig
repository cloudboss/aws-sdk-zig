...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMessageMoveTaskInput, options: CallOptions) !CancelMessageMoveTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelMessageMoveTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSQS.CancelMessageMoveTask");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelMessageMoveTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelMessageMoveTaskOutput, body, allocator);
}
