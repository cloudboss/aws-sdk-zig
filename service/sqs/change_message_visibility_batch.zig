...
pub const ChangeMessageVisibilityBatchOutput = struct {
    ...
    pub fn deinit(self: *ChangeMessageVisibilityBatchOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: ChangeMessageVisibilityBatchInput, options: Options) !ChangeMessageVisibilityBatchOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ChangeMessageVisibilityBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.ChangeMessageVisibilityBatch");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ChangeMessageVisibilityBatchOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ChangeMessageVisibilityBatchOutput, body, alloc);
}
