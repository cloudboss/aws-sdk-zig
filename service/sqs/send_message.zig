...
pub fn execute(client: *Client, input: SendMessageInput, options: Options) !SendMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.SendMessage");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendMessageOutput {
    ...
    return aws.json.parseJsonObject(SendMessageOutput, body, alloc);
}
