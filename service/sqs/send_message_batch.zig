...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendMessageBatchInput, options: CallOptions) !SendMessageBatchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SendMessageBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSQS.SendMessageBatch");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SendMessageBatchOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(SendMessageBatchOutput, body, allocator);
}
