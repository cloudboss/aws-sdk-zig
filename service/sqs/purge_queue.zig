...
fn serializeRequest(alloc: std.mem.Allocator, input: PurgeQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.PurgeQueue");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PurgeQueueOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
