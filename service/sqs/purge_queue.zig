...
const PurgeQueueOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurgeQueueInput, options: Options) !PurgeQueueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PurgeQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSQS.PurgeQueue");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PurgeQueueOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
