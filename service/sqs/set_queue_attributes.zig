...
pub const SetQueueAttributesOutput = struct {
    ...
    pub fn deinit(self: *SetQueueAttributesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: SetQueueAttributesInput, options: Options) !SetQueueAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetQueueAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.SetQueueAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetQueueAttributesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
