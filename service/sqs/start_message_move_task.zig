...
pub const StartMessageMoveTaskOutput = struct {
    ...
    pub fn deinit(self: *StartMessageMoveTaskOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: StartMessageMoveTaskInput, options: Options) !StartMessageMoveTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMessageMoveTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.StartMessageMoveTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMessageMoveTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMessageMoveTaskOutput, body, alloc);
}
