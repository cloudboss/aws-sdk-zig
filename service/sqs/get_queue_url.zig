...
pub const GetQueueUrlOutput = struct {
    ...
    pub fn deinit(self: *GetQueueUrlOutput) void {
        self._arena.deinit();
    }
...
pub fn execute(client: *Client, input: GetQueueUrlInput, options: Options) !GetQueueUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQueueUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.GetQueueUrl");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQueueUrlOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetQueueUrlOutput, body, alloc);
}
