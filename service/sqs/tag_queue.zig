...
fn serializeRequest(alloc: std.mem.Allocator, input: TagQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonSQS.TagQueue");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TagQueueOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
