...
const TagQueueOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TagQueueInput, options: Options) !TagQueueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TagQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonSQS.TagQueue");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TagQueueOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
