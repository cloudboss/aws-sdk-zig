...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContentModerationInput, options: Options) !GetContentModerationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContentModerationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.GetContentModeration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContentModerationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetContentModerationOutput, body, alloc);
}
