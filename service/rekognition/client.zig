...
const associate_faces = @import("associate_faces.zig");
const compare_faces = @import("compare_faces.zig");
const copy_project_version = @import("copy_project_version.zig");
const create_collection = @import("create_collection.zig");
const create_dataset = @import("create_dataset.zig");
const create_face_liveness_session = @import("create_face_liveness_session.zig");
const create_project = @import("create_project.zig");
const create_project_version = @import("create_project_version.zig");
const create_stream_processor = @import("create_stream_processor.zig");
...
const delete_faces = @import("delete_faces.zig");
const delete_project = @import("delete_project.zig");
const delete_project_policy = @import("delete_project_policy.zig");
const delete_project_version = @import("delete_project_version.zig");
const delete_stream_processor = @import("delete_stream_processor.zig");
const delete_user = @import("delete_user.zig");
const describe_collection = @import("describe_collection.zig");
const describe_dataset = @import("describe_dataset.zig");
const describe_project_versions = @import("describe_project_versions.zig");
const describe_projects = @import("describe_projects.zig");
const describe_stream_processor = @import("describe_stream_processor.zig");
const detect_custom_labels = @import("detect_custom_labels.zig");
const detect_faces = @import("detect_faces.zig");
const detect_labels = @import("detect_labels.zig");
const detect_moderation_labels = @import("detect_moderation_labels.zig");
const detect_protective_equipment = @import("detect_protective_equipment.zig");
const detect_text = @import("detect_text.zig");
const disassociate_faces = @import("disassociate_faces.zig");
const distribute_dataset_entries = @import("distribute_dataset_entries.zig");
const get_celebrity_info = @import("get_celebrity_info.zig");
const get_celebrity_recognition = @import("get_celebrity_recognition.zig");
const get_content_moderation = @import("get_content_moderation.zig");
const get_face_detection = @import("get_face_detection.zig");
const get_face_liveness_session_results = @import("get_face_liveness_session_results.zig");
const get_face_search = @import("get_face_search.zig");
const get_label_detection = @import("get_label_detection.zig");
const get_media_analysis_job = @import("get_media_analysis_job.zig");
const get_person_tracking = @import("get_person_tracking.zig");
const get_segment_detection = @import("get_segment_detection.zig");
const get_text_detection = @import("get_text_detection.zig");
const index_faces = @import("index_faces.zig");
const list_collections = @import("list_collections.zig");
const list_dataset_entries = @import("list_dataset_entries.zig");
const list_dataset_labels = @import("list_dataset_labels.zig");
const list_faces = @import("list_faces.zig");
const list_media_analysis_jobs = @import("list_media_analysis_jobs.zig");
const list_project_policies = @import("list_project_policies.zig");
const list_stream_processors = @import("list_stream_processors.zig");
const list_tags_for_resource = @import("list_tags_for_resource.zig");
const list_users = @import("list_users.zig");
const put_project_policy = @import("put_project_policy.zig");
const recognize_celebrities = @import("recognize_celebrities.zig");
const search_faces = @import("search_faces.zig");
const search_faces_by_image = @import("search_faces_by_image.zig");
const search_users = @import("search_users.zig");
const search_users_by_image = @import("search_users_by_image.zig");
const start_celebrity_recognition = @import("start_celebrity_recognition.zig");
const start_content_moderation = @import("start_content_moderation.zig");
const start_face_detection = @import("start_face_detection.zig");
const start_face_search = @import("start_face_search.zig");
const start_label_detection = @import("start_label_detection.zig");
const start_media_analysis_job = @import("start_media_analysis_job.zig");
const start_person_tracking = @import("start_person_tracking.zig");
const start_project_version = @import("start_project_version.zig");
const start_segment_detection = @import("start_segment_detection.zig");
const start_stream_processor = @import("start_stream_processor.zig");
const start_text_detection = @import("start_text_detection.zig");
const stop_project_version = @import("stop_project_version.zig");
const stop_stream_processor = @import("stop_stream_processor.zig");
...
pub const Client = struct {
    ...
    /// list is associated with the provided UserID. The number of FaceIds that can
    /// be used as input
    ...
    /// associated with it, no additional faces can be added. If more API calls are
    /// made after the
    ...
    /// collection,
    /// doesn’t meet the `UserMatchThreshold`, or there are already 100 faces
    /// associated
    ...
    /// then Custom Labels
    /// is used by default. For adapters, you can also choose whether or not to have
    /// the project
    ...
    /// and begins training. Models and adapters are managed as part of a
    /// Rekognition project. The
    ...
    /// `DELETE_IN_PROGRESS`. If you try to access the dataset after it is deleted,
    /// you get
    ...
    /// provided as
    /// input. This includes objects like flower, tree, and table; events like
    /// wedding, graduation,
    ...
    /// also add the
    /// `MaxLabels` parameter to limit the number of labels returned. The default
    /// and
    ...
    /// contains a
    /// BoundingBox object, describing the location of the label on the input image.
    /// It also
    ...
    /// image. Amazon Rekognition can detect the
    ...
    ///
    /// You pass the input image as base64-encoded image bytes or as a reference to
    /// an image in an Amazon S3 bucket.
    ...
    /// For each body part, an array of detected items of PPE is returned, including
    /// an indicator of whether or not the PPE
    ...
    /// box (BoundingBox) for each detected
    ...
    /// elements, `TextDetections`. Each
    /// `TextDetection` element provides information about a single word or line of
    /// text
    ...
    /// applies to the detected face instance. The other facial attributes listed in
    /// the
    ...
    /// video. You can also sort them by moderated label by specifying `NAME` for
    /// the `SortBy`
    /// input parameter.
    ///
    ...
    /// the Amazon Simple Notification Service topic registered in the initial call
    /// to `StartFaceDetection`. To get the results
    ...
    /// are more results than
    /// specified in `MaxResults`, the value of `NextToken` in the operation
    /// response contains a pagination token for getting the next set
    ...
    /// (`Labels`) sorted by the time the labels were detected. You can also sort by
    /// the
    ...
    /// define a segment. Although the “Timestamp” structure is still returned with
    /// each label,
    ...
    /// When the segment detection operation finishes, Amazon Rekognition publishes
    ...
    /// if so, call `GetSegmentDetection` and pass the job identifier (`JobId`) from
    /// the initial call
    ...
    /// specified in `MaxResults`, the value of `NextToken` in the operation
    /// response contains
    /// a pagination token for getting the next set of results. To get the next page
    /// of results, call `GetSegmentDetection`
    ...
    /// completion status to the Amazon Simple Notification Service
    /// topic registered in the initial call to `StartTextDetection`. To get the
    /// results
    /// of the text detection operation, first check that the status value published
    /// to the Amazon SNS topic is `SUCCEEDED`.
    /// if so, call `GetTextDetection` and pass the job identifier (`JobId`) from
    /// the initial call
    ...
    /// DescribeCollection and supply the collection ID. You can also get the model
    ...
    /// external ID. You can use this
    /// external image ID to create a client-side index to associate the faces with
    /// each image. You
    ...
    /// that don’t meet a required quality bar. The quality bar is based on a
    /// variety of common use
    /// cases. By default, `IndexFaces` chooses the quality bar that's used to
    /// filter
    ...
    /// external ID
    /// in the `IndexFaces` operation, Amazon Rekognition doesn't save duplicate
    /// face
    /// metadata.
    ///
    ...
    /// more information, see [Creating a manifest
    ...
    /// Non terminal errors are reported in `errors` lists within each JSON Line.
    /// The
    ...
    /// celebrity name or
    /// additional information URLs returned by `RecognizeCelebrities`, you will
    /// need the
    ...
    /// face
    /// belongs to. You get a face ID when you add a face to the collection using
    ...
    /// then searches
    /// the specified collection for matching faces. The operation compares the
    /// features of the input
    /// face with faces in the specified collection.
    ///
    /// To search for all faces in an input image, you might first call the
    /// IndexFaces operation, and then use the face IDs returned in subsequent calls
    ...
    /// score with
    /// the highest similarity first. More specifically, it is an array of metadata
    /// for each face
    ...
    /// S3 bucket. Use Video to specify the bucket name
    ...
    /// returns a job identifier (`JobId`) which you use to get the search results
    ...
    /// bucket name
    /// and the filename of the video.
    /// `StartLabelDetection` returns a job identifier (`JobId`) which you use to
    /// get the
    ...
    /// categories. You can
    /// specify inclusive filters, exclusive filters, or a combination of inclusive
    /// and exclusive
    ...
    /// S3 bucket. Use Video to specify the bucket name and
    ...
    /// is updated.
    /// If `source-ref` field doesn't reference an existing image, the image is
    ...
    pub fn describeProjectVersionsPaginator(self: *Self, params: describe_project_versions.DescribeProjectVersionsInput) paginator.DescribeProjectVersionsPaginator {
        return .{
            .client = self,
            .params = params,
    ...
    pub fn describeProjectsPaginator(self: *Self, params: describe_projects.DescribeProjectsInput) paginator.DescribeProjectsPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn getCelebrityRecognitionPaginator(self: *Self, params: get_celebrity_recognition.GetCelebrityRecognitionInput) paginator.GetCelebrityRecognitionPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn getContentModerationPaginator(self: *Self, params: get_content_moderation.GetContentModerationInput) paginator.GetContentModerationPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn getFaceDetectionPaginator(self: *Self, params: get_face_detection.GetFaceDetectionInput) paginator.GetFaceDetectionPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn getFaceSearchPaginator(self: *Self, params: get_face_search.GetFaceSearchInput) paginator.GetFaceSearchPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn getLabelDetectionPaginator(self: *Self, params: get_label_detection.GetLabelDetectionInput) paginator.GetLabelDetectionPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn getPersonTrackingPaginator(self: *Self, params: get_person_tracking.GetPersonTrackingInput) paginator.GetPersonTrackingPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn getSegmentDetectionPaginator(self: *Self, params: get_segment_detection.GetSegmentDetectionInput) paginator.GetSegmentDetectionPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn getTextDetectionPaginator(self: *Self, params: get_text_detection.GetTextDetectionInput) paginator.GetTextDetectionPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn listDatasetEntriesPaginator(self: *Self, params: list_dataset_entries.ListDatasetEntriesInput) paginator.ListDatasetEntriesPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn listDatasetLabelsPaginator(self: *Self, params: list_dataset_labels.ListDatasetLabelsInput) paginator.ListDatasetLabelsPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn listFacesPaginator(self: *Self, params: list_faces.ListFacesInput) paginator.ListFacesPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn listMediaAnalysisJobsPaginator(self: *Self, params: list_media_analysis_jobs.ListMediaAnalysisJobsInput) paginator.ListMediaAnalysisJobsPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn listProjectPoliciesPaginator(self: *Self, params: list_project_policies.ListProjectPoliciesInput) paginator.ListProjectPoliciesPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
    ...
    pub fn listStreamProcessorsPaginator(self: *Self, params: list_stream_processors.ListStreamProcessorsInput) paginator.ListStreamProcessorsPaginator {
        return .{
            .client = self,
            .params = params,
            .allocator = self.allocator,
