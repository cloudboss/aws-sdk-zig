...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFaceLivenessSessionResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.GetFaceLivenessSessionResults");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFaceLivenessSessionResultsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetFaceLivenessSessionResultsOutput, body, alloc);
}
