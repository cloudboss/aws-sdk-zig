...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteProjectPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DeleteProjectPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteProjectPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
