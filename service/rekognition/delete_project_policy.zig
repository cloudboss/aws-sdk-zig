...
pub const DeleteProjectPolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteProjectPolicyInput, options: Options) !DeleteProjectPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteProjectPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.DeleteProjectPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteProjectPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
