...
const ProjectDescription = @import("project_description.zig").ProjectDescription;

pub const DescribeProjectsInput = struct {
    /// Specifies the type of customization to filter projects by. If no value is
    /// specified,
    /// CUSTOM_LABELS is used as a default.
    features: ?[]const CustomizationFeature = null,
    ...
    /// response. You can use this pagination
    ...
    project_names: ?[]const []const u8 = null,
...
pub const DescribeProjectsOutput = struct {
    /// If the previous response was incomplete (because there is more
    ...
    project_descriptions: ?[]const ProjectDescription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProjectsInput, options: CallOptions) !DescribeProjectsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeProjectsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.DescribeProjects");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeProjectsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProjectsOutput, body, allocator);
}
