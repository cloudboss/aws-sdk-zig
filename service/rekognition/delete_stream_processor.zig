...
pub const DeleteStreamProcessorInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStreamProcessorInput, options: Options) !DeleteStreamProcessorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteStreamProcessorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.DeleteStreamProcessor");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteStreamProcessorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
