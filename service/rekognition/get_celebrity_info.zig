...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCelebrityInfoInput, options: Options) !GetCelebrityInfoOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCelebrityInfoInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.GetCelebrityInfo");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCelebrityInfoOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCelebrityInfoOutput, body, alloc);
}
