...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchFacesByImageInput, options: Options) !SearchFacesByImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchFacesByImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.SearchFacesByImage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchFacesByImageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchFacesByImageOutput, body, alloc);
}
