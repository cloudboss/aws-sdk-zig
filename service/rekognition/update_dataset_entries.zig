...
const DatasetChanges = @import("dataset_changes.zig").DatasetChanges;

pub const UpdateDatasetEntriesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDatasetEntriesInput, options: Options) !UpdateDatasetEntriesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDatasetEntriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.UpdateDatasetEntries");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDatasetEntriesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
