...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDatasetEntriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.UpdateDatasetEntries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDatasetEntriesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
