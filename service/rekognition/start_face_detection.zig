...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFaceDetectionInput, options: Options) !StartFaceDetectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartFaceDetectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StartFaceDetection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartFaceDetectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartFaceDetectionOutput, body, alloc);
}
