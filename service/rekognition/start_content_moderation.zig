...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartContentModerationInput, options: Options) !StartContentModerationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartContentModerationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StartContentModeration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartContentModerationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartContentModerationOutput, body, alloc);
}
