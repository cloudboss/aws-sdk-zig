...
const PersonTrackingSortBy = @import("person_tracking_sort_by.zig").PersonTrackingSortBy;
const VideoJobStatus = @import("video_job_status.zig").VideoJobStatus;
const PersonDetection = @import("person_detection.zig").PersonDetection;
const Video = @import("video.zig").Video;
const VideoMetadata = @import("video_metadata.zig").VideoMetadata;

pub const GetPersonTrackingInput = struct {
    ...
    /// array elements
    /// by the time persons are detected. Use `INDEX` to sort by the tracked
    /// persons.
    /// If you sort by `INDEX`, the array elements for each person are sorted by
    /// detection confidence.
    /// The default sort is by `TIMESTAMP`.
    sort_by: ?PersonTrackingSortBy = null,
...
pub const GetPersonTrackingOutput = struct {
    ...
    persons: ?[]const PersonDetection = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPersonTrackingInput, options: CallOptions) !GetPersonTrackingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPersonTrackingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.GetPersonTracking");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPersonTrackingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPersonTrackingOutput, body, allocator);
}
