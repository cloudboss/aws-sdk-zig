...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTextDetectionInput, options: Options) !StartTextDetectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartTextDetectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StartTextDetection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartTextDetectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartTextDetectionOutput, body, alloc);
}
