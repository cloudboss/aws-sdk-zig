...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartPersonTrackingInput, options: Options) !StartPersonTrackingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartPersonTrackingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StartPersonTracking");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartPersonTrackingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartPersonTrackingOutput, body, alloc);
}
