...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectLabelsInput, options: Options) !DetectLabelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectLabelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DetectLabels");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectLabelsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetectLabelsOutput, body, alloc);
}
