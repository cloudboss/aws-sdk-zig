...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectTextInput, options: Options) !DetectTextOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectTextInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DetectText");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectTextOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetectTextOutput, body, alloc);
}
