...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCollectionInput, options: Options) !DescribeCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DescribeCollection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCollectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCollectionOutput, body, alloc);
}
