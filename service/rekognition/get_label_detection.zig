...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLabelDetectionInput, options: Options) !GetLabelDetectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLabelDetectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.GetLabelDetection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLabelDetectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLabelDetectionOutput, body, alloc);
}
