...
/// requested types of person protective equipment (PPE), which persons
/// were detected as not wearing PPE, and the persons in which a determination
/// could not be made. For more information,
/// see ProtectiveEquipmentSummary.
pub const ProtectiveEquipmentSummarizationAttributes = struct {
    ...
    /// equipment detection, and body part coverage.
    /// Amazon Rekognition doesn't return summary information with a confidence than
    /// this specified value. There isn't a
    /// default value.
    ///
    ...
    /// 100%.
    /// If you specify a value that is less than 50%, the results are the same
    /// specifying a value of 50%.
    min_confidence: f32,

    /// An array of personal protective equipment types for which you want summary
    /// information.
    /// If a person is detected wearing a required requipment type, the person's ID
    /// is added to the
    /// `PersonsWithRequiredEquipment` array field returned in
    /// ProtectiveEquipmentSummary
    /// by `DetectProtectiveEquipment`.
    required_equipment_types: []const ProtectiveEquipmentType,
