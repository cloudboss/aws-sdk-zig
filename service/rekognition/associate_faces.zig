...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateFacesInput, options: Options) !AssociateFacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateFacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.AssociateFaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateFacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateFacesOutput, body, alloc);
}
