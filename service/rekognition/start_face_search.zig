...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFaceSearchInput, options: Options) !StartFaceSearchOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartFaceSearchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StartFaceSearch");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartFaceSearchOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartFaceSearchOutput, body, alloc);
}
