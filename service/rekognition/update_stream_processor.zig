...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStreamProcessorInput, options: Options) !UpdateStreamProcessorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStreamProcessorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.UpdateStreamProcessor");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStreamProcessorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
