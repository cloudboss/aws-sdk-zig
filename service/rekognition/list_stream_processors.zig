...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStreamProcessorsInput, options: Options) !ListStreamProcessorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStreamProcessorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.ListStreamProcessors");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStreamProcessorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListStreamProcessorsOutput, body, alloc);
}
