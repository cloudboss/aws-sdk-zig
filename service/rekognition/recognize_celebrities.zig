...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RecognizeCelebritiesInput, options: Options) !RecognizeCelebritiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RecognizeCelebritiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.RecognizeCelebrities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RecognizeCelebritiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RecognizeCelebritiesOutput, body, alloc);
}
