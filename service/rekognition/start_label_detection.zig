...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartLabelDetectionInput, options: Options) !StartLabelDetectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartLabelDetectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StartLabelDetection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartLabelDetectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartLabelDetectionOutput, body, alloc);
}
