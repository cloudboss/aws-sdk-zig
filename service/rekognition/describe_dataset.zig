...
const DatasetDescription = @import("dataset_description.zig").DatasetDescription;
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDatasetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DescribeDataset");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDatasetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDatasetOutput, body, alloc);
}
