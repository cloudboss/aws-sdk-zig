...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyProjectVersionInput, options: Options) !CopyProjectVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyProjectVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.CopyProjectVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyProjectVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CopyProjectVersionOutput, body, alloc);
}
