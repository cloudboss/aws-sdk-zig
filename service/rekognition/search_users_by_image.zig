...
const SearchedFaceDetails = @import("searched_face_details.zig").SearchedFaceDetails;
const UnsearchedFace = @import("unsearched_face.zig").UnsearchedFace;
const UserMatch = @import("user_match.zig").UserMatch;

pub const SearchUsersByImageInput = struct {
    ...
    /// identify faces.
    /// Filtered faces aren't searched for in the collection. The default value is
    /// NONE.
...
pub const SearchUsersByImageOutput = struct {
    ...
    /// in image, as
    /// well as the confidence in the bounding box, that was searched for matches.
    /// If no valid face is
    /// detected in the image the response will contain no SearchedFace object.
    searched_face: ?SearchedFaceDetails = null,

    /// List of UnsearchedFace objects. Contains the face details infered from the
    /// specified image
    /// but not used for search. Contains reasons that describe why a face wasn't
    /// used for Search.
    unsearched_faces: ?[]const UnsearchedFace = null,

    /// An array of UserID objects that matched the input face, along with the
    /// confidence in the
    /// match. The returned structure will be empty if there are no matches.
    /// Returned if the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchUsersByImageInput, options: CallOptions) !SearchUsersByImageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchUsersByImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.SearchUsersByImage");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchUsersByImageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchUsersByImageOutput, body, allocator);
}
