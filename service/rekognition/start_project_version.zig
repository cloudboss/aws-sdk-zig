...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartProjectVersionInput, options: Options) !StartProjectVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartProjectVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StartProjectVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartProjectVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartProjectVersionOutput, body, alloc);
}
