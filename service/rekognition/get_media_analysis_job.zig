...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMediaAnalysisJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.GetMediaAnalysisJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMediaAnalysisJobOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetMediaAnalysisJobOutput, body, alloc);
}
