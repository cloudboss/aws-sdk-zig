...
const FaceDetection = @import("face_detection.zig").FaceDetection;
...
pub const GetFaceDetectionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFaceDetectionInput, options: CallOptions) !GetFaceDetectionOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "rekognition");
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFaceDetectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.GetFaceDetection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFaceDetectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetFaceDetectionOutput, body, allocator);
}
