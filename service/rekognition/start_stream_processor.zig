...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartStreamProcessorInput, options: Options) !StartStreamProcessorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartStreamProcessorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StartStreamProcessor");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartStreamProcessorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartStreamProcessorOutput, body, alloc);
}
