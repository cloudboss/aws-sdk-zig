...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTextDetectionInput, options: Options) !GetTextDetectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTextDetectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.GetTextDetection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTextDetectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTextDetectionOutput, body, alloc);
}
