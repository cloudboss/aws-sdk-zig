...
/// image size. For example, if the input image is 700x200 pixels, and the
...
/// Amazon Rekognition is
/// able to detect a face that is at the image edge and is only partially
/// visible, the service
/// can return coordinates that are outside the image bounds and, depending on
/// the image edge,
