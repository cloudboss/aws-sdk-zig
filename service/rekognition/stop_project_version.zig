...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopProjectVersionInput, options: Options) !StopProjectVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopProjectVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StopProjectVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopProjectVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopProjectVersionOutput, body, alloc);
}
