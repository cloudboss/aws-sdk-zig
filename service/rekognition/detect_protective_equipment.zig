...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectProtectiveEquipmentInput, options: Options) !DetectProtectiveEquipmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectProtectiveEquipmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DetectProtectiveEquipment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectProtectiveEquipmentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetectProtectiveEquipmentOutput, body, alloc);
}
