...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectCustomLabelsInput, options: Options) !DetectCustomLabelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectCustomLabelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DetectCustomLabels");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectCustomLabelsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetectCustomLabelsOutput, body, alloc);
}
