...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProjectVersionsInput, options: Options) !DescribeProjectVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProjectVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DescribeProjectVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProjectVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProjectVersionsOutput, body, alloc);
}
