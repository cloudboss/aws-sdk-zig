...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectFacesInput, options: Options) !DetectFacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectFacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DetectFaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectFacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetectFacesOutput, body, alloc);
}
