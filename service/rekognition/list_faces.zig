...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFacesInput, options: Options) !ListFacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.ListFaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFacesOutput, body, alloc);
}
