...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDatasetLabelsInput, options: Options) !ListDatasetLabelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDatasetLabelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.ListDatasetLabels");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDatasetLabelsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDatasetLabelsOutput, body, alloc);
}
