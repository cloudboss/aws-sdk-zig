...
const DistributeDataset = @import("distribute_dataset.zig").DistributeDataset;

pub const DistributeDatasetEntriesInput = struct {
    /// The ARNS for the training dataset and test dataset that you want to use. The
    /// datasets must belong to
    /// the same project. The test dataset must be empty.
    datasets: []const DistributeDataset,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DistributeDatasetEntriesInput, options: Options) !DistributeDatasetEntriesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DistributeDatasetEntriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.DistributeDatasetEntries");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DistributeDatasetEntriesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
