...
fn serializeRequest(alloc: std.mem.Allocator, input: DistributeDatasetEntriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.DistributeDatasetEntries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DistributeDatasetEntriesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
