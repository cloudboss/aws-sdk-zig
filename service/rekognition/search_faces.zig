...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchFacesInput, options: Options) !SearchFacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchFacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.SearchFaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchFacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchFacesOutput, body, alloc);
}
