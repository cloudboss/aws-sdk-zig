...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProjectPoliciesInput, options: Options) !ListProjectPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProjectPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.ListProjectPolicies");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProjectPoliciesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProjectPoliciesOutput, body, alloc);
}
