...
fn serializeRequest(alloc: std.mem.Allocator, input: StopStreamProcessorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StopStreamProcessor");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopStreamProcessorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
