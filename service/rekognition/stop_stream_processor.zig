...
pub const StopStreamProcessorInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopStreamProcessorInput, options: Options) !StopStreamProcessorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopStreamProcessorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.StopStreamProcessor");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopStreamProcessorOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
