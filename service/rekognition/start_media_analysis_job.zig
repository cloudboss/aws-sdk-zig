...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMediaAnalysisJobInput, options: Options) !StartMediaAnalysisJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMediaAnalysisJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.StartMediaAnalysisJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMediaAnalysisJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMediaAnalysisJobOutput, body, alloc);
}
