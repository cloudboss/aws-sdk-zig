...
pub const PutProjectPolicyInput = struct {
    ...
    /// generates and assigns a new `PolicyRevisionId` and then deletes the previous
    /// version of the
    /// policy.
...
pub const PutProjectPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutProjectPolicyInput, options: CallOptions) !PutProjectPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutProjectPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.PutProjectPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutProjectPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutProjectPolicyOutput, body, allocator);
}
