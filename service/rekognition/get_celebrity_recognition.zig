...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCelebrityRecognitionInput, options: Options) !GetCelebrityRecognitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCelebrityRecognitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.GetCelebrityRecognition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCelebrityRecognitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCelebrityRecognitionOutput, body, alloc);
}
