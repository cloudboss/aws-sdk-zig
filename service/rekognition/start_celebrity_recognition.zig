...
pub const StartCelebrityRecognitionInput = struct {
    ...
    /// celebrity recognition analysis to. The Amazon SNS topic must have a topic
...
pub const StartCelebrityRecognitionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartCelebrityRecognitionInput, options: CallOptions) !StartCelebrityRecognitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartCelebrityRecognitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "RekognitionService.StartCelebrityRecognition");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartCelebrityRecognitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartCelebrityRecognitionOutput, body, allocator);
}
