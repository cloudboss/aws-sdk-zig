...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSegmentDetectionInput, options: Options) !GetSegmentDetectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSegmentDetectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.GetSegmentDetection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSegmentDetectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSegmentDetectionOutput, body, alloc);
}
