...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFaceSearchInput, options: Options) !GetFaceSearchOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFaceSearchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.GetFaceSearch");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFaceSearchOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetFaceSearchOutput, body, alloc);
}
