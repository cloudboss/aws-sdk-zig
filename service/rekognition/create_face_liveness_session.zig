...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFaceLivenessSessionInput, options: Options) !CreateFaceLivenessSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFaceLivenessSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.CreateFaceLivenessSession");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFaceLivenessSessionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateFaceLivenessSessionOutput, body, alloc);
}
