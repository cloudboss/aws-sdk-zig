...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMediaAnalysisJobsInput, options: Options) !ListMediaAnalysisJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMediaAnalysisJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "RekognitionService.ListMediaAnalysisJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMediaAnalysisJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMediaAnalysisJobsOutput, body, alloc);
}
