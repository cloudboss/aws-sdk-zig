...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateDistributionTenantWebACLInput, options: Options) !AssociateDistributionTenantWebACLOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateDistributionTenantWebACLInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/associate-web-acl");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<AssociateDistributionTenantWebACLRequest>");
    try body_buf.appendSlice(alloc, "<WebACLArn>");
    try aws.xml.appendXmlEscaped(alloc, &body_buf, input.web_acl_arn);
    try body_buf.appendSlice(alloc, "</WebACLArn>");
    try body_buf.appendSlice(alloc, "</AssociateDistributionTenantWebACLRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateDistributionTenantWebACLOutput {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Id")) {
                    ...
                    result.web_acl_arn = try alloc.dupe(u8, try reader.readElementText());
                } else {
