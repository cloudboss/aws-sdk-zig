...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDistributionWithTagsInput, options: Options) !CreateDistributionWithTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDistributionWithTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<DistributionConfigWithTags xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeDistributionConfigWithTags(alloc, &body_buf, input.distribution_config_with_tags);
    try body_buf.appendSlice(alloc, "</DistributionConfigWithTags>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDistributionWithTagsOutput {
