...
/// key, use `CookiesConfig` in an origin request policy. See
/// `OriginRequestPolicy`.
///
/// A complex type that specifies whether you want CloudFront to forward cookies
/// to the origin and, if so, which ones. For more information about forwarding
/// cookies to the origin, see [Caching Content Based on
/// Cookies](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Cookies.html) in the *Amazon CloudFront Developer Guide*.
pub const CookiePreference = struct {
    /// This field is deprecated. We recommend that you use a cache policy or an
    /// origin request policy instead of this field.
    ///
    /// If you want to include cookies in the cache key, use a cache policy. For
    /// more information, see [Creating cache
    /// policies](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/controlling-the-cache-key.html#cache-key-create-cache-policy) in the *Amazon CloudFront Developer Guide*.
    ///
    /// If you want to send cookies to the origin but not include them in the cache
    /// key, use origin request policy. For more information, see [Creating origin
    /// request
    /// policies](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/controlling-origin-requests.html#origin-request-create-origin-request-policy) in the *Amazon CloudFront Developer Guide*.
    ///
    ...
    /// `WhitelistedNames`. If you change the value of `Forward` from `whitelist` to
    /// `all` or `none` and you don't delete the `WhitelistedNames` element and its
