...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOriginRequestPolicyInput, options: Options) !CreateOriginRequestPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOriginRequestPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<OriginRequestPolicyConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeOriginRequestPolicyConfig(alloc, &body_buf, input.origin_request_policy_config);
    try body_buf.appendSlice(alloc, "</OriginRequestPolicyConfig>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOriginRequestPolicyOutput {
