...
pub const ListKeyValueStoresInput = struct {
...
pub const ListKeyValueStoresOutput = struct {
    /// The resulting key value stores list.
    key_value_store_list: ?KeyValueStoreList = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListKeyValueStoresInput, options: CallOptions) !ListKeyValueStoresOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListKeyValueStoresInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListKeyValueStoresOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ListKeyValueStoresOutput = .{};
