...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInvalidationForDistributionTenantInput, options: Options) !GetInvalidationForDistributionTenantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInvalidationForDistributionTenantInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.distribution_tenant_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInvalidationForDistributionTenantOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
