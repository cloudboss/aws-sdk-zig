...
pub const UpdateVpcOriginInput = struct {
    /// The VPC origin ID.
    id: []const u8,
...
pub const UpdateVpcOriginOutput = struct {
    /// The VPC origin ETag.
    e_tag: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVpcOriginInput, options: Options) !UpdateVpcOriginOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVpcOriginInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/vpc-origin/");
    try path_buf.appendSlice(alloc, input.id);
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<VpcOriginEndpointConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeVpcOriginEndpointConfig(alloc, &body_buf, input.vpc_origin_endpoint_config);
    try body_buf.appendSlice(alloc, "</VpcOriginEndpointConfig>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVpcOriginOutput {
    var result: UpdateVpcOriginOutput = .{};
    _ = status;
    _ = body;
