...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStreamingDistributionWithTagsInput, options: Options) !CreateStreamingDistributionWithTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStreamingDistributionWithTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "WithTags");
    query_has_prev = true;
    const query = try query_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<StreamingDistributionConfigWithTags xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeStreamingDistributionConfigWithTags(alloc, &body_buf, input.streaming_distribution_config_with_tags);
    try body_buf.appendSlice(alloc, "</StreamingDistributionConfigWithTags>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStreamingDistributionWithTagsOutput {
