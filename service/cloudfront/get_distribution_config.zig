...
pub const GetDistributionConfigInput = struct {
    /// The distribution's ID. If the ID is empty, an empty distribution
    /// configuration is returned.
    id: []const u8,
};

pub const GetDistributionConfigOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDistributionConfigInput, options: Options) !GetDistributionConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDistributionConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDistributionConfigOutput {
    var result: GetDistributionConfigOutput = .{};
    _ = status;
    _ = body;
