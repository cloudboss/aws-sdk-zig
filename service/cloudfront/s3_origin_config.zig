...
pub const S3OriginConfig = struct {
    /// If you're using origin access control (OAC) instead of origin access
    /// identity, specify an empty `OriginAccessIdentity` element. For more
    /// information, see [Restricting access to an Amazon Web
    /// Services](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/private-content-restricting-access-to-origin.html) in the *Amazon CloudFront Developer Guide*.
    ///
    ...
    ///
    /// The ` *ID-of-origin-access-identity* ` is the value that CloudFront returned
