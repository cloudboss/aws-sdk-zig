...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDistributionsByResponseHeadersPolicyIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/distributionsByResponseHeadersPolicyId/");
    try path_buf.appendSlice(alloc, input.response_headers_policy_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDistributionsByResponseHeadersPolicyIdOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
