...
pub const PublishConnectionFunctionInput = struct {
...
pub const PublishConnectionFunctionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PublishConnectionFunctionInput, options: Options) !PublishConnectionFunctionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PublishConnectionFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/connection-function/");
    try path_buf.appendSlice(alloc, input.id);
    try path_buf.appendSlice(alloc, "/publish");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PublishConnectionFunctionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: PublishConnectionFunctionOutput = .{};
