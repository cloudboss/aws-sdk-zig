...
pub const ListKeyGroupsInput = struct {
...
pub const ListKeyGroupsOutput = struct {
    /// A list of key groups.
    key_group_list: ?KeyGroupList = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListKeyGroupsInput, options: CallOptions) !ListKeyGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListKeyGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListKeyGroupsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ListKeyGroupsOutput = .{};
