...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRealtimeLogConfigInput, options: Options) !DeleteRealtimeLogConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRealtimeLogConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<DeleteRealtimeLogConfigRequest>");
    if (input.arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRealtimeLogConfigOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
