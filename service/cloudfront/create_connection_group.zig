...
pub const CreateConnectionGroupInput = struct {
...
pub const CreateConnectionGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConnectionGroupInput, options: Options) !CreateConnectionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConnectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<CreateConnectionGroupRequest>");
    if (input.anycast_ip_list_id) |v| {
        try body_buf.appendSlice(alloc, "<AnycastIpListId>");
    ...
    if (input.ipv_6_enabled) |v| {
        try body_buf.appendSlice(alloc, "<Ipv6Enabled>");
        try body_buf.appendSlice(alloc, if (v) "true" else "false");
        try body_buf.appendSlice(alloc, "</Ipv6Enabled>");
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConnectionGroupOutput {
    var result: CreateConnectionGroupOutput = .{};
    _ = status;
    _ = body;
