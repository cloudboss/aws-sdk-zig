...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDistributionsByOriginRequestPolicyIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/distributionsByOriginRequestPolicyId/");
    try path_buf.appendSlice(alloc, input.origin_request_policy_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDistributionsByOriginRequestPolicyIdOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
