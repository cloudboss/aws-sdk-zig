...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOriginAccessControlInput, options: CallOptions) !UpdateOriginAccessControlOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateOriginAccessControlInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/2020-05-31/origin-access-control/");
    try path_buf.appendSlice(allocator, input.id);
    try path_buf.appendSlice(allocator, "/config");
    const path = try path_buf.toOwnedSlice(allocator);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(allocator, "<OriginAccessControlConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateOriginAccessControlOutput {
