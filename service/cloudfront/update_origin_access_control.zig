...
pub const UpdateOriginAccessControlInput = struct {
...
pub const UpdateOriginAccessControlOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateOriginAccessControlInput, options: Options) !UpdateOriginAccessControlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateOriginAccessControlInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/origin-access-control/");
    try path_buf.appendSlice(alloc, input.id);
    try path_buf.appendSlice(alloc, "/config");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<OriginAccessControlConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeOriginAccessControlConfig(alloc, &body_buf, input.origin_access_control_config);
    try body_buf.appendSlice(alloc, "</OriginAccessControlConfig>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateOriginAccessControlOutput {
    var result: UpdateOriginAccessControlOutput = .{};
    _ = status;
    _ = body;
