...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDistributionsByWebACLIdInput, options: Options) !ListDistributionsByWebACLIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDistributionsByWebACLIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/distributionsByWebACLId/");
    try path_buf.appendSlice(alloc, input.web_acl_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDistributionsByWebACLIdOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
