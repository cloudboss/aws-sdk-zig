...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKeyValueStoreInput, options: Options) !CreateKeyValueStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKeyValueStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<CreateKeyValueStoreRequest>");
    if (input.comment) |v| {
    ...
    if (input.import_source) |v| {
        try body_buf.appendSlice(alloc, "<ImportSource>");
        try serde.serializeImportSource(alloc, &body_buf, v);
        try body_buf.appendSlice(alloc, "</ImportSource>");
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKeyValueStoreOutput {
