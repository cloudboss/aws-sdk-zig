...
pub const GetConnectionGroupInput = struct {
    /// The ID, name, or Amazon Resource Name (ARN) of the connection group.
    identifier: []const u8,
};

pub const GetConnectionGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConnectionGroupInput, options: Options) !GetConnectionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConnectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/connection-group/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConnectionGroupOutput {
    var result: GetConnectionGroupOutput = .{};
    _ = status;
    _ = body;
