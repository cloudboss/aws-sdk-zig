...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMonitoringSubscriptionInput, options: Options) !CreateMonitoringSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMonitoringSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/distributions/");
    try path_buf.appendSlice(alloc, input.distribution_id);
    try path_buf.appendSlice(alloc, "/monitoring-subscription");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<MonitoringSubscription xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeMonitoringSubscription(alloc, &body_buf, input.monitoring_subscription);
    try body_buf.appendSlice(alloc, "</MonitoringSubscription>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMonitoringSubscriptionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
