...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConnectionGroupByRoutingEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "RoutingEndpoint=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.routing_endpoint);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConnectionGroupByRoutingEndpointOutput {
