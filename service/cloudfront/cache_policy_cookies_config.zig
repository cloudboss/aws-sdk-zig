...
/// sends to the origin.
pub const CachePolicyCookiesConfig = struct {
    ...
    ///   requests that CloudFront sends to the origin. Even when this field is set
    ///   to `none`, any cookies that are listed in an `OriginRequestPolicy` *are*
    ...
    /// * `allExcept` – All cookies in viewer requests are included in the cache key
    ///   and in requests that CloudFront sends to the origin, * **except** * for
    ///   those that are listed in the `CookieNames` type, which are not included.
    /// * `all` – All cookies in viewer requests are included in the cache key and
