...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePublicKeyInput, options: Options) !UpdatePublicKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePublicKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/public-key/");
    try path_buf.appendSlice(alloc, input.id);
    try path_buf.appendSlice(alloc, "/config");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<PublicKeyConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializePublicKeyConfig(alloc, &body_buf, input.public_key_config);
    try body_buf.appendSlice(alloc, "</PublicKeyConfig>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePublicKeyOutput {
