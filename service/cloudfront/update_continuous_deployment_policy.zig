...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContinuousDeploymentPolicyInput, options: Options) !UpdateContinuousDeploymentPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContinuousDeploymentPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/continuous-deployment-policy/");
    try path_buf.appendSlice(alloc, input.id);
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<ContinuousDeploymentPolicyConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeContinuousDeploymentPolicyConfig(alloc, &body_buf, input.continuous_deployment_policy_config);
    try body_buf.appendSlice(alloc, "</ContinuousDeploymentPolicyConfig>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContinuousDeploymentPolicyOutput {
