...
pub const DeleteKeyValueStoreInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteKeyValueStoreInput, options: Options) !DeleteKeyValueStoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteKeyValueStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/key-value-store/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteKeyValueStoreOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteKeyValueStoreOutput = .{};
