...
pub const ListFunctionsOutput = struct {
    ...
    function_list: ?FunctionList = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFunctionsInput, options: Options) !ListFunctionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFunctionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.stage) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "Stage=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFunctionsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: ListFunctionsOutput = .{};
