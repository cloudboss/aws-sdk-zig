...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDistributionsByConnectionFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "ConnectionFunctionIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.connection_function_identifier);
    query_has_prev = true;
    if (input.marker) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDistributionsByConnectionFunctionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
