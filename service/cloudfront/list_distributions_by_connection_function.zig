...
pub const ListDistributionsByConnectionFunctionInput = struct {
    /// The distributions by connection function identifier.
    connection_function_identifier: []const u8,
...
pub const ListDistributionsByConnectionFunctionOutput = struct {
    distribution_list: ?DistributionList = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDistributionsByConnectionFunctionInput, options: CallOptions) !ListDistributionsByConnectionFunctionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDistributionsByConnectionFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2020-05-31/distributionsByConnectionFunction";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "ConnectionFunctionIdentifier=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.connection_function_identifier);
    query_has_prev = true;
    if (input.marker) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDistributionsByConnectionFunctionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ListDistributionsByConnectionFunctionOutput = .{};
