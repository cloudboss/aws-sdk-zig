...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDistributionInput, options: Options) !UpdateDistributionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDistributionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<DistributionConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeDistributionConfig(alloc, &body_buf, input.distribution_config);
    try body_buf.appendSlice(alloc, "</DistributionConfig>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDistributionOutput {
