...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKeyGroupInput, options: Options) !UpdateKeyGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKeyGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/key-group/");
    try path_buf.appendSlice(alloc, input.id);
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<KeyGroupConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeKeyGroupConfig(alloc, &body_buf, input.key_group_config);
    try body_buf.appendSlice(alloc, "</KeyGroupConfig>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKeyGroupOutput {
