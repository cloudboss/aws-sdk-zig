...
/// name such as `d111111abcdef8.cloudfront.net`—set
...
///   `SSLSupportMethod` to `sni-only`. This is recommended. Most browsers and
///   clients support SNI.
...
///   Policy](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/distribution-web-values-specify.html#DownloadDistValues-security-policy) in the *Amazon CloudFront Developer Guide*.
...
///   (IAM)](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_server-certs.html). You specify the location by setting a value in one of the following fields (not both):
///
/// * `ACMCertificateArn`
/// * `IAMCertificateId`
///
...
/// `ViewerProtocolPolicy` in the `CacheBehavior` or `DefaultCacheBehavior`. To
/// specify how CloudFront should use SSL/TLS to communicate with your custom
/// origin, use `CustomOriginConfig`.
///
/// For more information, see [Using HTTPS with
/// CloudFront](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/using-https.html) and [ Using Alternate Domain Names and HTTPS](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/using-https-alternate-domain-names.html) in the *Amazon CloudFront Developer Guide*.
pub const ViewerCertificate = struct {
    /// If the distribution uses `Aliases` (alternate domain names or CNAMEs) and
    /// the SSL/TLS certificate is stored in [Certificate Manager
    /// (ACM)](https://docs.aws.amazon.com/acm/latest/userguide/acm-overview.html),
    ...
    ///
    /// If the distribution uses `Aliases` (alternate domain names or CNAMEs) and
    /// the SSL/TLS certificate is stored in [Identity and Access Management
    /// (IAM)](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_server-certs.html), provide the ID of the IAM certificate.
    ///
    ...
    ///   (SNI)](https://en.wikipedia.org/wiki/Server_Name_Indication). This is
    ...
    ///   through the [Amazon Web ServicesSupport
