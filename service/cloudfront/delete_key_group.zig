...
pub const DeleteKeyGroupInput = struct {
    ...
    /// group's `ETag` value. To get the `ETag`, use `GetKeyGroup` or
    /// `GetKeyGroupConfig`.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteKeyGroupInput, options: CallOptions) !DeleteKeyGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteKeyGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteKeyGroupOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteKeyGroupOutput = .{};
