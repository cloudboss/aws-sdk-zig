...
pub const CreateInvalidationForDistributionTenantInput = struct {
...
pub const CreateInvalidationForDistributionTenantOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInvalidationForDistributionTenantInput, options: Options) !CreateInvalidationForDistributionTenantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInvalidationForDistributionTenantInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/invalidation");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<InvalidationBatch xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeInvalidationBatch(alloc, &body_buf, input.invalidation_batch);
    try body_buf.appendSlice(alloc, "</InvalidationBatch>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInvalidationForDistributionTenantOutput {
    var result: CreateInvalidationForDistributionTenantOutput = .{};
    _ = status;
    _ = body;
