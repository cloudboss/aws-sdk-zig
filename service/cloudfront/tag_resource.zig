...
fn serializeRequest(alloc: std.mem.Allocator, input: TagResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "Operation=Tag");
    query_has_prev = true;
    ...
    try body_buf.appendSlice(alloc, "<Tags xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeTags(alloc, &body_buf, input.tags);
