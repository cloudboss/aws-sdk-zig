...
pub const DeleteVpcOriginInput = struct {
    /// The VPC origin ID.
    id: []const u8,
...
pub const DeleteVpcOriginOutput = struct {
    /// The VPC origin ETag.
    e_tag: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVpcOriginInput, options: Options) !DeleteVpcOriginOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVpcOriginInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVpcOriginOutput {
    var result: DeleteVpcOriginOutput = .{};
    _ = status;
    _ = body;
