...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVpcOriginInput, options: Options) !CreateVpcOriginOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVpcOriginInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<CreateVpcOriginRequest>");
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVpcOriginOutput {
