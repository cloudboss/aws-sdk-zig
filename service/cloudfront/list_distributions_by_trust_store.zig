...
pub const ListDistributionsByTrustStoreInput = struct {
    ...
    trust_store_identifier: []const u8,
};

pub const ListDistributionsByTrustStoreOutput = struct {
    distribution_list: ?DistributionList = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDistributionsByTrustStoreInput, options: CallOptions) !ListDistributionsByTrustStoreOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDistributionsByTrustStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/2020-05-31/distributionsByTrustStore";
    ...
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "TrustStoreIdentifier=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.trust_store_identifier);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDistributionsByTrustStoreOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ListDistributionsByTrustStoreOutput = .{};
