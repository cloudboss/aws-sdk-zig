...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDistributionsByTrustStoreInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "TrustStoreIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.trust_store_identifier);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDistributionsByTrustStoreOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
