...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRealtimeLogConfigInput, options: Options) !UpdateRealtimeLogConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRealtimeLogConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<UpdateRealtimeLogConfigRequest>");
    if (input.arn) |v| {
    ...
    if (input.end_points) |v| {
        ...
        try serde.serializeEndPointList(alloc, &body_buf, v, "member");
        try body_buf.appendSlice(alloc, "</EndPoints>");
    }
    if (input.fields) |v| {
        try body_buf.appendSlice(alloc, "<Fields>");
        try serde.serializeFieldList(alloc, &body_buf, v, "Field");
        try body_buf.appendSlice(alloc, "</Fields>");
    }
    if (input.name) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRealtimeLogConfigOutput {
