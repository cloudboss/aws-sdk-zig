...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConnectionFunctionInput, options: Options) !UpdateConnectionFunctionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConnectionFunctionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<UpdateConnectionFunctionRequest>");
    try body_buf.appendSlice(alloc, "<ConnectionFunctionCode>");
    try aws.xml.appendXmlEscaped(alloc, &body_buf, input.connection_function_code);
    try body_buf.appendSlice(alloc, "</ConnectionFunctionCode>");
    try body_buf.appendSlice(alloc, "<ConnectionFunctionConfig>");
    try serde.serializeFunctionConfig(alloc, &body_buf, input.connection_function_config);
    try body_buf.appendSlice(alloc, "</ConnectionFunctionConfig>");
    try body_buf.appendSlice(alloc, "</UpdateConnectionFunctionRequest>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConnectionFunctionOutput {
