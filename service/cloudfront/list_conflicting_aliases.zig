...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConflictingAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "DistributionId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.distribution_id);
    query_has_prev = true;
    if (input.marker) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConflictingAliasesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
