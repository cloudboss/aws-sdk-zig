...
pub const ListDistributionsByCachePolicyIdInput = struct {
...
pub const ListDistributionsByCachePolicyIdOutput = struct {
    /// A list of distribution IDs.
    distribution_id_list: ?DistributionIdList = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDistributionsByCachePolicyIdInput, options: CallOptions) !ListDistributionsByCachePolicyIdOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDistributionsByCachePolicyIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/2020-05-31/distributionsByCachePolicyId/");
    try path_buf.appendSlice(allocator, input.cache_policy_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDistributionsByCachePolicyIdOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ListDistributionsByCachePolicyIdOutput = .{};
