...
pub const ListAnycastIpListsInput = struct {
...
pub const ListAnycastIpListsOutput = struct {
    /// Root level tag for the `AnycastIpLists` parameters.
    anycast_ip_lists: ?AnycastIpListCollection = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnycastIpListsInput, options: CallOptions) !ListAnycastIpListsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAnycastIpListsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAnycastIpListsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: ListAnycastIpListsOutput = .{};
