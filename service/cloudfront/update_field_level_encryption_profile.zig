...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFieldLevelEncryptionProfileInput, options: Options) !UpdateFieldLevelEncryptionProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFieldLevelEncryptionProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/2020-05-31/field-level-encryption-profile/");
    try path_buf.appendSlice(alloc, input.id);
    try path_buf.appendSlice(alloc, "/config");
    const path = try path_buf.toOwnedSlice(alloc);

    var body_buf: std.ArrayList(u8) = .{};
    try body_buf.appendSlice(alloc, "<FieldLevelEncryptionProfileConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeFieldLevelEncryptionProfileConfig(alloc, &body_buf, input.field_level_encryption_profile_config);
    try body_buf.appendSlice(alloc, "</FieldLevelEncryptionProfileConfig>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFieldLevelEncryptionProfileOutput {
