...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDistributionWithStagingConfigInput, options: Options) !UpdateDistributionWithStagingConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDistributionWithStagingConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/promote-staging-config");
    ...
    if (input.staging_distribution_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "StagingDistributionId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDistributionWithStagingConfigOutput {
