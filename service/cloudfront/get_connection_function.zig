...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConnectionFunctionInput, options: Options) !GetConnectionFunctionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConnectionFunctionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConnectionFunctionOutput {
    var result: GetConnectionFunctionOutput = .{};
    _ = status;
    if (body.len > 0) {
        result.connection_function_code = try alloc.dupe(u8, body);
    }
