...
pub const UpdateResponseHeadersPolicyInput = struct {
...
pub const UpdateResponseHeadersPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResponseHeadersPolicyInput, options: Options) !UpdateResponseHeadersPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResponseHeadersPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "<ResponseHeadersPolicyConfig xmlns=" ++ &[_]u8{0x22} ++ "http://cloudfront.amazonaws.com/doc/2020-05-31/" ++ &[_]u8{0x22} ++ ">");
    try serde.serializeResponseHeadersPolicyConfig(alloc, &body_buf, input.response_headers_policy_config);
    try body_buf.appendSlice(alloc, "</ResponseHeadersPolicyConfig>");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResponseHeadersPolicyOutput {
    var result: UpdateResponseHeadersPolicyOutput = .{};
    _ = status;
    _ = body;
