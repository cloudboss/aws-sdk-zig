...
pub const GetVpcOriginInput = struct {
    /// The VPC origin ID.
    id: []const u8,
};

pub const GetVpcOriginOutput = struct {
    /// The VPC origin ETag.
    e_tag: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVpcOriginInput, options: CallOptions) !GetVpcOriginOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetVpcOriginInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetVpcOriginOutput {
    var result: GetVpcOriginOutput = .{};
    _ = status;
    _ = body;
