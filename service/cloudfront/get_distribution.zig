...
pub const GetDistributionInput = struct {
    /// The distribution's ID. If the ID is empty, an empty distribution
    /// configuration is returned.
    id: []const u8,
};

pub const GetDistributionOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDistributionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDistributionOutput {
    var result: GetDistributionOutput = .{};
    _ = status;
    _ = body;
