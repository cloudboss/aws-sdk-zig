...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateApprovalTeamInput, options: Options) !CreateApprovalTeamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApprovalTeamInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.approval_strategy), input.approval_strategy, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Approvers\":");
    try aws.json.writeValue(@TypeOf(input.approvers), input.approvers, alloc, &body_buf);
    has_prev = true;
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Policies\":");
    try aws.json.writeValue(@TypeOf(input.policies), input.policies, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateApprovalTeamOutput {
    var result: CreateApprovalTeamOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateApprovalTeamOutput, body, alloc);
    }
    _ = status;
    _ = headers;
