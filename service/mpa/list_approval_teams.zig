...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApprovalTeamsInput, options: Options) !ListApprovalTeamsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApprovalTeamsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApprovalTeamsOutput {
    var result: ListApprovalTeamsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListApprovalTeamsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
