...
const IdentitySourceForList = @import("identity_source_for_list.zig").IdentitySourceForList;
...
pub const ListIdentitySourcesOutput = struct {
    ...
    identity_sources: ?[]const IdentitySourceForList = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdentitySourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdentitySourcesOutput {
    var result: ListIdentitySourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIdentitySourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
