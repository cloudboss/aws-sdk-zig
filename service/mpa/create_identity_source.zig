...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIdentitySourceInput, options: Options) !CreateIdentitySourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIdentitySourceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"IdentitySourceParameters\":");
    try aws.json.writeValue(@TypeOf(input.identity_source_parameters), input.identity_source_parameters, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIdentitySourceOutput {
    var result: CreateIdentitySourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIdentitySourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
