...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartActiveApprovalTeamDeletionInput, options: Options) !StartActiveApprovalTeamDeletionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartActiveApprovalTeamDeletionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.pending_window_days) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"PendingWindowDays\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartActiveApprovalTeamDeletionOutput {
    var result: StartActiveApprovalTeamDeletionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartActiveApprovalTeamDeletionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
