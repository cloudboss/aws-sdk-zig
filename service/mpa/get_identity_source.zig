...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentitySourceInput, options: Options) !GetIdentitySourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentitySourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/identity-sources/");
    try path_buf.appendSlice(alloc, input.identity_source_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentitySourceOutput {
    var result: GetIdentitySourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIdentitySourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
