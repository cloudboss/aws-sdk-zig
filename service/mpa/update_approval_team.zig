...
pub const UpdateApprovalTeamOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateApprovalTeamInput, options: Options) !UpdateApprovalTeamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateApprovalTeamInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.approval_strategy) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ApprovalStrategy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.approvers) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Approvers\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.update_actions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UpdateActions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateApprovalTeamOutput {
    var result: UpdateApprovalTeamOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateApprovalTeamOutput, body, alloc);
    }
    _ = status;
    _ = headers;
