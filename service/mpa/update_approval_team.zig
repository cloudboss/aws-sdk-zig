...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateApprovalTeamInput, options: CallOptions) !UpdateApprovalTeamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateApprovalTeamInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.update_actions) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"UpdateActions\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
