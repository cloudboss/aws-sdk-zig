...
const ClientPropertiesResult = @import("client_properties_result.zig").ClientPropertiesResult;

pub const DescribeClientPropertiesInput = struct {
...
pub const DescribeClientPropertiesOutput = struct {
    /// Information about the specified Amazon WorkSpaces clients.
    client_properties_list: ?[]const ClientPropertiesResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClientPropertiesInput, options: CallOptions) !DescribeClientPropertiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeClientPropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeClientProperties");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeClientPropertiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeClientPropertiesOutput, body, allocator);
}
