...
pub const RejectAccountLinkInvitationInput = struct {
...
pub const RejectAccountLinkInvitationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectAccountLinkInvitationInput, options: CallOptions) !RejectAccountLinkInvitationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RejectAccountLinkInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.RejectAccountLinkInvitation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RejectAccountLinkInvitationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RejectAccountLinkInvitationOutput, body, allocator);
}
