...
pub const DisassociateConnectionAliasInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateConnectionAliasInput, options: Options) !DisassociateConnectionAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateConnectionAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DisassociateConnectionAlias");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateConnectionAliasOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
