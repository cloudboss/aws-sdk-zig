...
pub const ModifyStreamingPropertiesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyStreamingPropertiesInput, options: Options) !ModifyStreamingPropertiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyStreamingPropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.ModifyStreamingProperties");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyStreamingPropertiesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
