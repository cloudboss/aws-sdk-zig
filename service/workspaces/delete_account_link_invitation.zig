...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAccountLinkInvitationInput, options: Options) !DeleteAccountLinkInvitationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAccountLinkInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DeleteAccountLinkInvitation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAccountLinkInvitationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteAccountLinkInvitationOutput, body, alloc);
}
