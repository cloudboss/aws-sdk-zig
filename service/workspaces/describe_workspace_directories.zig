...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspaceDirectoriesInput, options: Options) !DescribeWorkspaceDirectoriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkspaceDirectoriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeWorkspaceDirectories");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkspaceDirectoriesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspaceDirectoriesOutput, body, alloc);
}
