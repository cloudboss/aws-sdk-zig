...
pub const UpdateConnectionAliasPermissionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConnectionAliasPermissionInput, options: Options) !UpdateConnectionAliasPermissionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateConnectionAliasPermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.UpdateConnectionAliasPermission");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateConnectionAliasPermissionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
