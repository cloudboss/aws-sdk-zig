...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifySelfservicePermissionsInput, options: Options) !ModifySelfservicePermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifySelfservicePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.ModifySelfservicePermissions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifySelfservicePermissionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
