...
pub const AssociateConnectionAliasInput = struct {
...
pub const AssociateConnectionAliasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateConnectionAliasInput, options: CallOptions) !AssociateConnectionAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateConnectionAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.AssociateConnectionAlias");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateConnectionAliasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateConnectionAliasOutput, body, allocator);
}
