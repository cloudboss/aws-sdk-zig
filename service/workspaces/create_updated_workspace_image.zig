...
pub const CreateUpdatedWorkspaceImageInput = struct {
...
pub const CreateUpdatedWorkspaceImageOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateUpdatedWorkspaceImageInput, options: CallOptions) !CreateUpdatedWorkspaceImageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateUpdatedWorkspaceImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.CreateUpdatedWorkspaceImage");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateUpdatedWorkspaceImageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateUpdatedWorkspaceImageOutput, body, allocator);
}
