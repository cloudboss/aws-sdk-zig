...
pub const ModifyWorkspacePropertiesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyWorkspacePropertiesInput, options: Options) !ModifyWorkspacePropertiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyWorkspacePropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.ModifyWorkspaceProperties");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyWorkspacePropertiesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
