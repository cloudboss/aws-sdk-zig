...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyWorkspaceStateInput, options: Options) !ModifyWorkspaceStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyWorkspaceStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.ModifyWorkspaceState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyWorkspaceStateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
