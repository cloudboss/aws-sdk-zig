...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccountLinksInput, options: Options) !ListAccountLinksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccountLinksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.ListAccountLinks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccountLinksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAccountLinksOutput, body, alloc);
}
