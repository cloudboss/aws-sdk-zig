...
pub const AssociateIpGroupsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateIpGroupsInput, options: Options) !AssociateIpGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateIpGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.AssociateIpGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateIpGroupsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
