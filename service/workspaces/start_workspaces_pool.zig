...
fn serializeRequest(alloc: std.mem.Allocator, input: StartWorkspacesPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.StartWorkspacesPool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartWorkspacesPoolOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
