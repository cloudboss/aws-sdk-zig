...
pub const StartWorkspacesPoolInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartWorkspacesPoolInput, options: Options) !StartWorkspacesPoolOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartWorkspacesPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.StartWorkspacesPool");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartWorkspacesPoolOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
