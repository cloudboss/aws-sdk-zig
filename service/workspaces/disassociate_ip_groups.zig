...
pub const DisassociateIpGroupsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateIpGroupsInput, options: Options) !DisassociateIpGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateIpGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DisassociateIpGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateIpGroupsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
