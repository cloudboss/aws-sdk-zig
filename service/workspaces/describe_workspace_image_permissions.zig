...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspaceImagePermissionsInput, options: Options) !DescribeWorkspaceImagePermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkspaceImagePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeWorkspaceImagePermissions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkspaceImagePermissionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspaceImagePermissionsOutput, body, alloc);
}
