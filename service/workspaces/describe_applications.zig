...
const WorkSpaceApplication = @import("work_space_application.zig").WorkSpaceApplication;
...
pub const DescribeApplicationsInput = struct {
    ...
    compute_type_names: ?[]const Compute = null,
    ...
    operating_system_names: ?[]const OperatingSystemName = null,
    ...
    pub const json_field_names = .{
        ...
        .compute_type_names = "ComputeTypeNames",
        .license_type = "LicenseType",
        .max_results = "MaxResults",
        .next_token = "NextToken",
        .operating_system_names = "OperatingSystemNames",
        .owner = "Owner",
    };
};

pub const DescribeApplicationsOutput = struct {
    /// List of information about the specified applications.
    applications: ?[]const WorkSpaceApplication = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeApplicationsInput, options: CallOptions) !DescribeApplicationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeApplications");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeApplicationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeApplicationsOutput, body, allocator);
}
