...
pub const DescribeClientBrandingInput = struct {
...
pub const DescribeClientBrandingOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClientBrandingInput, options: CallOptions) !DescribeClientBrandingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeClientBrandingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeClientBranding");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeClientBrandingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeClientBrandingOutput, body, allocator);
}
