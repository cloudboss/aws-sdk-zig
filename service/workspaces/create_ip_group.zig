...
pub const CreateIpGroupInput = struct {
...
pub const CreateIpGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIpGroupInput, options: CallOptions) !CreateIpGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateIpGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.CreateIpGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateIpGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateIpGroupOutput, body, allocator);
}
