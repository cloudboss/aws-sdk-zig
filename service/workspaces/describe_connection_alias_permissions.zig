...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConnectionAliasPermissionsInput, options: Options) !DescribeConnectionAliasPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConnectionAliasPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeConnectionAliasPermissions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConnectionAliasPermissionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConnectionAliasPermissionsOutput, body, alloc);
}
