...
pub const UpdateRulesOfIpGroupInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRulesOfIpGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.UpdateRulesOfIpGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRulesOfIpGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
