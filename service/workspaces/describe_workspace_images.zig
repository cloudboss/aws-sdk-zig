...
const WorkspaceImage = @import("workspace_image.zig").WorkspaceImage;

pub const DescribeWorkspaceImagesInput = struct {
...
pub const DescribeWorkspaceImagesOutput = struct {
    /// Information about the images.
    images: ?[]const WorkspaceImage = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspaceImagesInput, options: CallOptions) !DescribeWorkspaceImagesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeWorkspaceImagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeWorkspaceImages");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeWorkspaceImagesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspaceImagesOutput, body, allocator);
}
