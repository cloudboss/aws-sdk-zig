...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkspacesPoolInput, options: Options) !CreateWorkspacesPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkspacesPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.CreateWorkspacesPool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkspacesPoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateWorkspacesPoolOutput, body, alloc);
}
