...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyAccountInput, options: Options) !ModifyAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.ModifyAccount");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyAccountOutput, body, alloc);
}
