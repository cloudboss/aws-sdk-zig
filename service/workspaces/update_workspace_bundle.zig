...
pub const UpdateWorkspaceBundleInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkspaceBundleInput, options: Options) !UpdateWorkspaceBundleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateWorkspaceBundleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.UpdateWorkspaceBundle");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateWorkspaceBundleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
