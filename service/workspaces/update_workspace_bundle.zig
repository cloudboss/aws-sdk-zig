...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspaceBundleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.UpdateWorkspaceBundle");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspaceBundleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
