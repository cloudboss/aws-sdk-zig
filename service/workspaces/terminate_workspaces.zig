...
const TerminateRequest = @import("terminate_request.zig").TerminateRequest;
const FailedWorkspaceChangeRequest = @import("failed_workspace_change_request.zig").FailedWorkspaceChangeRequest;

pub const TerminateWorkspacesInput = struct {
    /// The WorkSpaces to terminate. You can specify up to 25 WorkSpaces.
    terminate_workspace_requests: []const TerminateRequest,
...
pub const TerminateWorkspacesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TerminateWorkspacesInput, options: CallOptions) !TerminateWorkspacesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TerminateWorkspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.TerminateWorkspaces");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TerminateWorkspacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TerminateWorkspacesOutput, body, allocator);
}
