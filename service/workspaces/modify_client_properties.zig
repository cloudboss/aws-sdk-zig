...
pub const ModifyClientPropertiesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyClientPropertiesInput, options: Options) !ModifyClientPropertiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyClientPropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.ModifyClientProperties");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyClientPropertiesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
