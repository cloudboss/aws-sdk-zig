...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspacesPoolSessionsInput, options: Options) !DescribeWorkspacesPoolSessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkspacesPoolSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeWorkspacesPoolSessions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkspacesPoolSessionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspacesPoolSessionsOutput, body, alloc);
}
