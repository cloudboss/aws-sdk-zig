...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConnectionAliasesInput, options: Options) !DescribeConnectionAliasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConnectionAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeConnectionAliases");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeConnectionAliasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConnectionAliasesOutput, body, alloc);
}
