...
pub const DescribeWorkspaceBundlesInput = struct {
...
pub const DescribeWorkspaceBundlesOutput = struct {
    /// Information about the bundles.
    bundles: ?[]const WorkspaceBundle = null,

    /// The token to use to retrieve the next page of results. This value is null
    /// when there are no more
    /// results to return. This token is valid for one day and must be used within
    /// that time
    /// frame.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspaceBundlesInput, options: CallOptions) !DescribeWorkspaceBundlesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeWorkspaceBundlesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeWorkspaceBundles");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeWorkspaceBundlesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspaceBundlesOutput, body, allocator);
}
