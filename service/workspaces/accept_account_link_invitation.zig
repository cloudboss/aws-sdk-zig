...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptAccountLinkInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.AcceptAccountLinkInvitation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptAccountLinkInvitationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AcceptAccountLinkInvitationOutput, body, alloc);
}
