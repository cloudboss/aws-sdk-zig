...
pub const DeleteConnectionAliasInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConnectionAliasInput, options: CallOptions) !DeleteConnectionAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteConnectionAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DeleteConnectionAlias");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteConnectionAliasOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
