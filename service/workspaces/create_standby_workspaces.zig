...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStandbyWorkspacesInput, options: Options) !CreateStandbyWorkspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStandbyWorkspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.CreateStandbyWorkspaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStandbyWorkspacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateStandbyWorkspacesOutput, body, alloc);
}
