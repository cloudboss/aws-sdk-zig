...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebuildWorkspacesInput, options: Options) !RebuildWorkspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RebuildWorkspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.RebuildWorkspaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RebuildWorkspacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RebuildWorkspacesOutput, body, alloc);
}
