...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspaceSnapshotsInput, options: Options) !DescribeWorkspaceSnapshotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkspaceSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeWorkspaceSnapshots");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkspaceSnapshotsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspaceSnapshotsOutput, body, alloc);
}
