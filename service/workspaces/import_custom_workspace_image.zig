...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportCustomWorkspaceImageInput, options: Options) !ImportCustomWorkspaceImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportCustomWorkspaceImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.ImportCustomWorkspaceImage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportCustomWorkspaceImageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ImportCustomWorkspaceImageOutput, body, alloc);
}
