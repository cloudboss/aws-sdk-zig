...
pub const MigrateWorkspaceInput = struct {
...
pub const MigrateWorkspaceOutput = struct {
    /// The original identifier of the WorkSpace that is being migrated.
    source_workspace_id: ?[]const u8 = null,

    /// The new identifier of the WorkSpace that is being migrated. If the migration
    /// does not
    /// succeed, the target WorkSpace ID will not be used, and the WorkSpace will
    /// still have the
    /// original WorkSpace ID.
    target_workspace_id: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MigrateWorkspaceInput, options: CallOptions) !MigrateWorkspaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: MigrateWorkspaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.MigrateWorkspace");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !MigrateWorkspaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(MigrateWorkspaceOutput, body, allocator);
}
