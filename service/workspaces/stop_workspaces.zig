...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopWorkspacesInput, options: Options) !StopWorkspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopWorkspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.StopWorkspaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopWorkspacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopWorkspacesOutput, body, alloc);
}
