...
pub const StopWorkspacesPoolInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopWorkspacesPoolInput, options: Options) !StopWorkspacesPoolOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopWorkspacesPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.StopWorkspacesPool");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopWorkspacesPoolOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
