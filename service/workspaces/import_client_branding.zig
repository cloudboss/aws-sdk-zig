...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportClientBrandingInput, options: Options) !ImportClientBrandingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportClientBrandingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.ImportClientBranding");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportClientBrandingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ImportClientBrandingOutput, body, alloc);
}
