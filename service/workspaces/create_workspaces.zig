...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkspacesInput, options: Options) !CreateWorkspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.CreateWorkspaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkspacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateWorkspacesOutput, body, alloc);
}
