...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConnectionAliasInput, options: Options) !CreateConnectionAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConnectionAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.CreateConnectionAlias");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConnectionAliasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateConnectionAliasOutput, body, alloc);
}
