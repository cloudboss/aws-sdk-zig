...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkspacesPoolInput, options: Options) !UpdateWorkspacesPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspacesPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.UpdateWorkspacesPool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspacesPoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateWorkspacesPoolOutput, body, alloc);
}
