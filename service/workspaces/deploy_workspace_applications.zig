...
const WorkSpaceApplicationDeployment = @import("work_space_application_deployment.zig").WorkSpaceApplicationDeployment;

pub const DeployWorkspaceApplicationsInput = struct {
    /// Indicates whether the force flag is applied for the specified WorkSpace.
    /// When the force flag is enabled,
...
pub const DeployWorkspaceApplicationsOutput = struct {
    /// The list of deployed associations and information about them.
    deployment: ?WorkSpaceApplicationDeployment = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeployWorkspaceApplicationsInput, options: CallOptions) !DeployWorkspaceApplicationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeployWorkspaceApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DeployWorkspaceApplications");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeployWorkspaceApplicationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeployWorkspaceApplicationsOutput, body, allocator);
}
