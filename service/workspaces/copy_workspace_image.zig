...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyWorkspaceImageInput, options: Options) !CopyWorkspaceImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyWorkspaceImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.CopyWorkspaceImage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyWorkspaceImageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CopyWorkspaceImageOutput, body, alloc);
}
