...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeApplicationAssociationsInput, options: Options) !DescribeApplicationAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeApplicationAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeApplicationAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeApplicationAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeApplicationAssociationsOutput, body, alloc);
}
