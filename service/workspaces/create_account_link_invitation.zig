...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccountLinkInvitationInput, options: Options) !CreateAccountLinkInvitationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccountLinkInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.CreateAccountLinkInvitation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccountLinkInvitationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAccountLinkInvitationOutput, body, alloc);
}
