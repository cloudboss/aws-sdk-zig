...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspacesPoolsInput, options: Options) !DescribeWorkspacesPoolsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkspacesPoolsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeWorkspacesPools");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkspacesPoolsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspacesPoolsOutput, body, alloc);
}
