...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAvailableManagementCidrRangesInput, options: Options) !ListAvailableManagementCidrRangesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAvailableManagementCidrRangesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.ListAvailableManagementCidrRanges");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAvailableManagementCidrRangesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAvailableManagementCidrRangesOutput, body, alloc);
}
