...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCustomWorkspaceImageImportInput, options: Options) !DescribeCustomWorkspaceImageImportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCustomWorkspaceImageImportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeCustomWorkspaceImageImport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCustomWorkspaceImageImportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCustomWorkspaceImageImportOutput, body, alloc);
}
