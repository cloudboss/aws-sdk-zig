...
const BundleResourceAssociation = @import("bundle_resource_association.zig").BundleResourceAssociation;

pub const DescribeBundleAssociationsInput = struct {
    /// The resource types of the associated resource.
    associated_resource_types: []const BundleAssociatedResourceType,
...
pub const DescribeBundleAssociationsOutput = struct {
    /// List of information about the specified associations.
    associations: ?[]const BundleResourceAssociation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBundleAssociationsInput, options: CallOptions) !DescribeBundleAssociationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeBundleAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeBundleAssociations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeBundleAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeBundleAssociationsOutput, body, allocator);
}
