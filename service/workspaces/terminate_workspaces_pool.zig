...
fn serializeRequest(alloc: std.mem.Allocator, input: TerminateWorkspacesPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.TerminateWorkspacesPool");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TerminateWorkspacesPoolOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
