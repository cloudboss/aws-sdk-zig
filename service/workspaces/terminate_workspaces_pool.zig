...
pub const TerminateWorkspacesPoolInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TerminateWorkspacesPoolInput, options: Options) !TerminateWorkspacesPoolOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TerminateWorkspacesPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.TerminateWorkspacesPool");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TerminateWorkspacesPoolOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
