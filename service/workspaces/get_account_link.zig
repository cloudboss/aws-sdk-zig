...
pub const GetAccountLinkInput = struct {
...
pub const GetAccountLinkOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountLinkInput, options: CallOptions) !GetAccountLinkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAccountLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.GetAccountLink");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAccountLinkOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAccountLinkOutput, body, allocator);
}
