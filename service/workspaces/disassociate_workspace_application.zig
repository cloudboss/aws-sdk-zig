...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateWorkspaceApplicationInput, options: Options) !DisassociateWorkspaceApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateWorkspaceApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DisassociateWorkspaceApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateWorkspaceApplicationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateWorkspaceApplicationOutput, body, alloc);
}
