...
fn serializeRequest(alloc: std.mem.Allocator, input: AuthorizeIpRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.AuthorizeIpRules");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AuthorizeIpRulesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
