...
pub const AuthorizeIpRulesInput = struct {
    /// The identifier of the group.
    group_id: []const u8,

    /// The rules to add to the group.
    user_rules: []const IpRuleItem,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AuthorizeIpRulesInput, options: Options) !AuthorizeIpRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AuthorizeIpRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.AuthorizeIpRules");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AuthorizeIpRulesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
