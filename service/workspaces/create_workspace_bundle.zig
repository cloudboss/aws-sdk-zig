...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkspaceBundleInput, options: Options) !CreateWorkspaceBundleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkspaceBundleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.CreateWorkspaceBundle");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkspaceBundleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateWorkspaceBundleOutput, body, alloc);
}
