...
const AccountModification = @import("account_modification.zig").AccountModification;

pub const DescribeAccountModificationsInput = struct {
...
pub const DescribeAccountModificationsOutput = struct {
    /// The list of modifications to the configuration of BYOL.
    account_modifications: ?[]const AccountModification = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountModificationsInput, options: CallOptions) !DescribeAccountModificationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAccountModificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeAccountModifications");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAccountModificationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAccountModificationsOutput, body, allocator);
}
