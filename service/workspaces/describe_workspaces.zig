...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspacesInput, options: Options) !DescribeWorkspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeWorkspaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkspacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspacesOutput, body, alloc);
}
