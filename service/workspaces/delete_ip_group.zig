...
pub const DeleteIpGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteIpGroupInput, options: Options) !DeleteIpGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteIpGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DeleteIpGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteIpGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
