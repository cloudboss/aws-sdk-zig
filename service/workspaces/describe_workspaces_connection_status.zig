...
const WorkspaceConnectionStatus = @import("workspace_connection_status.zig").WorkspaceConnectionStatus;

pub const DescribeWorkspacesConnectionStatusInput = struct {
...
pub const DescribeWorkspacesConnectionStatusOutput = struct {
    ...
    workspaces_connection_status: ?[]const WorkspaceConnectionStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspacesConnectionStatusInput, options: CallOptions) !DescribeWorkspacesConnectionStatusOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "workspaces");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeWorkspacesConnectionStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeWorkspacesConnectionStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeWorkspacesConnectionStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspacesConnectionStatusOutput, body, allocator);
}
