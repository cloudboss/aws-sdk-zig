...
pub const DeleteWorkspaceBundleInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteWorkspaceBundleInput, options: Options) !DeleteWorkspaceBundleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteWorkspaceBundleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DeleteWorkspaceBundle");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteWorkspaceBundleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
