...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartWorkspacesInput, options: Options) !StartWorkspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartWorkspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.StartWorkspaces");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartWorkspacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartWorkspacesOutput, body, alloc);
}
