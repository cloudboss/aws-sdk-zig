...
pub const RevokeIpRulesInput = struct {
    /// The identifier of the group.
    group_id: []const u8,

    /// The rules to remove from the group.
    user_rules: []const []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeIpRulesInput, options: Options) !RevokeIpRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RevokeIpRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.RevokeIpRules");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RevokeIpRulesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
