...
pub const DeregisterWorkspaceDirectoryInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterWorkspaceDirectoryInput, options: Options) !DeregisterWorkspaceDirectoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterWorkspaceDirectoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DeregisterWorkspaceDirectory");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterWorkspaceDirectoryOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
