...
const ConnectClientAddIn = @import("connect_client_add_in.zig").ConnectClientAddIn;

pub const DescribeConnectClientAddInsInput = struct {
...
pub const DescribeConnectClientAddInsOutput = struct {
    /// Information about client add-ins.
    add_ins: ?[]const ConnectClientAddIn = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeConnectClientAddInsInput, options: CallOptions) !DescribeConnectClientAddInsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeConnectClientAddInsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeConnectClientAddIns");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeConnectClientAddInsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeConnectClientAddInsOutput, body, allocator);
}
