...
pub const ModifyEndpointEncryptionModeInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyEndpointEncryptionModeInput, options: Options) !ModifyEndpointEncryptionModeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyEndpointEncryptionModeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.ModifyEndpointEncryptionMode");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyEndpointEncryptionModeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
