...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkspaceImagePermissionInput, options: Options) !UpdateWorkspaceImagePermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspaceImagePermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.UpdateWorkspaceImagePermission");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspaceImagePermissionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
