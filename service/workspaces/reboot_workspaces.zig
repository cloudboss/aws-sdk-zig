...
const RebootRequest = @import("reboot_request.zig").RebootRequest;
const FailedWorkspaceChangeRequest = @import("failed_workspace_change_request.zig").FailedWorkspaceChangeRequest;

pub const RebootWorkspacesInput = struct {
    /// The WorkSpaces to reboot. You can specify up to 25 WorkSpaces.
    reboot_workspace_requests: []const RebootRequest,
...
pub const RebootWorkspacesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebootWorkspacesInput, options: CallOptions) !RebootWorkspacesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RebootWorkspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.RebootWorkspaces");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RebootWorkspacesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RebootWorkspacesOutput, body, allocator);
}
