...
pub const RestoreWorkspaceInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreWorkspaceInput, options: Options) !RestoreWorkspaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RestoreWorkspaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.RestoreWorkspace");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RestoreWorkspaceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
