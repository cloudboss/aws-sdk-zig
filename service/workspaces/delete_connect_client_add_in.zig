...
pub const DeleteConnectClientAddInInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConnectClientAddInInput, options: Options) !DeleteConnectClientAddInOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteConnectClientAddInInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DeleteConnectClientAddIn");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteConnectClientAddInOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
