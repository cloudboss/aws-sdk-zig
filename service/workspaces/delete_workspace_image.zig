...
pub const DeleteWorkspaceImageInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteWorkspaceImageInput, options: CallOptions) !DeleteWorkspaceImageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteWorkspaceImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DeleteWorkspaceImage");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteWorkspaceImageOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
