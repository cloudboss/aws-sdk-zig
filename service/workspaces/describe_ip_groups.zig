...
const WorkspacesIpGroup = @import("workspaces_ip_group.zig").WorkspacesIpGroup;

pub const DescribeIpGroupsInput = struct {
...
pub const DescribeIpGroupsOutput = struct {
    ...
    result: ?[]const WorkspacesIpGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIpGroupsInput, options: CallOptions) !DescribeIpGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeIpGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.DescribeIpGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeIpGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeIpGroupsOutput, body, allocator);
}
