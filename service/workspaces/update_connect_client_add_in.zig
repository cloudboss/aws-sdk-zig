...
pub const UpdateConnectClientAddInInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConnectClientAddInInput, options: Options) !UpdateConnectClientAddInOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateConnectClientAddInInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkspacesService.UpdateConnectClientAddIn");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateConnectClientAddInOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
