...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageAssociationsInput, options: Options) !DescribeImageAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeImageAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeImageAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeImageAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeImageAssociationsOutput, body, alloc);
}
