...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkspaceAssociationsInput, options: Options) !DescribeWorkspaceAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkspaceAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.DescribeWorkspaceAssociations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkspaceAssociationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkspaceAssociationsOutput, body, alloc);
}
