...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyWorkspaceCreationPropertiesInput, options: Options) !ModifyWorkspaceCreationPropertiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyWorkspaceCreationPropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkspacesService.ModifyWorkspaceCreationProperties");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyWorkspaceCreationPropertiesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
