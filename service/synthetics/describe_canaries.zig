...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCanariesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCanariesOutput {
    var result: DescribeCanariesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCanariesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
