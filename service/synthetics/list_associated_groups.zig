...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssociatedGroupsInput, options: Options) !ListAssociatedGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssociatedGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociatedGroupsOutput {
    var result: ListAssociatedGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssociatedGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
