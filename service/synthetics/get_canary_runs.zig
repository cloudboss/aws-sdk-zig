...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCanaryRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.run_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RunType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCanaryRunsOutput {
    var result: GetCanaryRunsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCanaryRunsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
