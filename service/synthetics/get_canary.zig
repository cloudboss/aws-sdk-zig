...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCanaryInput, options: Options) !GetCanaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCanaryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCanaryOutput {
    var result: GetCanaryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCanaryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
