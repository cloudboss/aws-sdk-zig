...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCanariesLastRunInput, options: Options) !DescribeCanariesLastRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCanariesLastRunInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.browser_type) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"BrowserType\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCanariesLastRunOutput {
    var result: DescribeCanariesLastRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCanariesLastRunOutput, body, alloc);
    }
    _ = status;
    _ = headers;
