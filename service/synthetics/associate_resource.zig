...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateResourceInput, options: Options) !AssociateResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/group/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateResourceOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateResourceOutput = .{};
