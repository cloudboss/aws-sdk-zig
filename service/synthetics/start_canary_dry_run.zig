...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartCanaryDryRunInput, options: CallOptions) !StartCanaryDryRunOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartCanaryDryRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/dry-run/start");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartCanaryDryRunOutput {
    var result: StartCanaryDryRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartCanaryDryRunOutput, body, allocator);
    }
    _ = status;
    _ = headers;
