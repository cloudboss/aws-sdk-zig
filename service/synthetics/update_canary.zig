...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCanaryInput, options: Options) !UpdateCanaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCanaryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dry_run_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DryRunId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCanaryOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
