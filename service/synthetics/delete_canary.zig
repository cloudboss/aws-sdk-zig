...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCanaryInput, options: Options) !DeleteCanaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCanaryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.delete_lambda) |v| {
        ...
        try query_buf.appendSlice(alloc, "deleteLambda=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCanaryOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
