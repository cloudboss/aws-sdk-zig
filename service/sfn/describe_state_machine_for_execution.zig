...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStateMachineForExecutionInput, options: Options) !DescribeStateMachineForExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStateMachineForExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.DescribeStateMachineForExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStateMachineForExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeStateMachineForExecutionOutput, body, alloc);
}
