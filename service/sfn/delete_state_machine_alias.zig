...
pub const DeleteStateMachineAliasInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStateMachineAliasInput, options: Options) !DeleteStateMachineAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteStateMachineAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSStepFunctions.DeleteStateMachineAlias");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteStateMachineAliasOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
