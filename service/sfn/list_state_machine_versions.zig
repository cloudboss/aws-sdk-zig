...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStateMachineVersionsInput, options: Options) !ListStateMachineVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStateMachineVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.ListStateMachineVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStateMachineVersionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListStateMachineVersionsOutput, body, alloc);
}
