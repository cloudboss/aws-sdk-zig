...
const StateMachineListItem = @import("state_machine_list_item.zig").StateMachineListItem;

pub const ListStateMachinesInput = struct {
...
pub const ListStateMachinesOutput = struct {
    /// If `nextToken` is returned, there are more results available. The value of
    /// `nextToken` is a unique pagination token for each page.
    /// Make the call again using the returned token to retrieve the next page. Keep
    /// all other arguments unchanged. Each pagination token expires after 24 hours.
    ...
    state_machines: ?[]const StateMachineListItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStateMachinesInput, options: CallOptions) !ListStateMachinesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListStateMachinesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSStepFunctions.ListStateMachines");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListStateMachinesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListStateMachinesOutput, body, allocator);
}
