...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStateMachineAliasInput, options: Options) !DescribeStateMachineAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStateMachineAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.DescribeStateMachineAlias");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStateMachineAliasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeStateMachineAliasOutput, body, alloc);
}
