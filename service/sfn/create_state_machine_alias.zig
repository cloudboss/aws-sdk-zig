...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStateMachineAliasInput, options: Options) !CreateStateMachineAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStateMachineAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.CreateStateMachineAlias");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStateMachineAliasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateStateMachineAliasOutput, body, alloc);
}
