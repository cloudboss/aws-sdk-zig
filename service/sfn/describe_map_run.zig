...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMapRunInput, options: Options) !DescribeMapRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMapRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.DescribeMapRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMapRunOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMapRunOutput, body, alloc);
}
