...
const ActivityListItem = @import("activity_list_item.zig").ActivityListItem;

pub const ListActivitiesInput = struct {
...
pub const ListActivitiesOutput = struct {
    /// The list of activities.
    activities: ?[]const ActivityListItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListActivitiesInput, options: CallOptions) !ListActivitiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListActivitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSStepFunctions.ListActivities");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListActivitiesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListActivitiesOutput, body, allocator);
}
