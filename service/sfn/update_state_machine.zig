...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStateMachineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.UpdateStateMachine");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStateMachineOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateStateMachineOutput, body, alloc);
}
