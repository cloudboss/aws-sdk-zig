...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartExecutionInput, options: Options) !StartExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.StartExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartExecutionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StartExecutionOutput, body, alloc);
}
