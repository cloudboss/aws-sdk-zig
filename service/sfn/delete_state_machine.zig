...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteStateMachineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.DeleteStateMachine");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteStateMachineOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
