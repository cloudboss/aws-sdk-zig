...
pub const DeleteStateMachineInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStateMachineInput, options: Options) !DeleteStateMachineOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteStateMachineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSStepFunctions.DeleteStateMachine");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteStateMachineOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
