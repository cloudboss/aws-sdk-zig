...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateActivityInput, options: Options) !CreateActivityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateActivityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.CreateActivity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateActivityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateActivityOutput, body, alloc);
}
