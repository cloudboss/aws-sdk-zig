...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStateMachineAliasInput, options: Options) !UpdateStateMachineAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStateMachineAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.UpdateStateMachineAlias");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStateMachineAliasOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateStateMachineAliasOutput, body, alloc);
}
