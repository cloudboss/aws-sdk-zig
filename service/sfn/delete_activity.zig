...
pub const DeleteActivityInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteActivityInput, options: Options) !DeleteActivityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteActivityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSStepFunctions.DeleteActivity");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteActivityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
