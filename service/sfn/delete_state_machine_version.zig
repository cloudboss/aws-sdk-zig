...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteStateMachineVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.DeleteStateMachineVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteStateMachineVersionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
