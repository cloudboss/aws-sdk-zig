...
pub const DeleteStateMachineVersionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStateMachineVersionInput, options: Options) !DeleteStateMachineVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteStateMachineVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSStepFunctions.DeleteStateMachineVersion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteStateMachineVersionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
