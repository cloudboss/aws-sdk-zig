...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetActivityTaskInput, options: Options) !GetActivityTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetActivityTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.GetActivityTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetActivityTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetActivityTaskOutput, body, alloc);
}
