...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStateMachineInput, options: Options) !DescribeStateMachineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStateMachineInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.DescribeStateMachine");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStateMachineOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeStateMachineOutput, body, alloc);
}
