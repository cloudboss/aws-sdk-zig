...
fn serializeRequest(alloc: std.mem.Allocator, input: SendTaskFailureInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.SendTaskFailure");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendTaskFailureOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
