...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMapRunInput, options: Options) !UpdateMapRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMapRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.UpdateMapRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMapRunOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
