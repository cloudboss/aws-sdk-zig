...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeActivityInput, options: Options) !DescribeActivityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeActivityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.DescribeActivity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeActivityOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeActivityOutput, body, alloc);
}
