...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStateMachineAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.ListStateMachineAliases");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStateMachineAliasesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListStateMachineAliasesOutput, body, alloc);
}
