...
const StateMachineAliasListItem = @import("state_machine_alias_list_item.zig").StateMachineAliasListItem;

pub const ListStateMachineAliasesInput = struct {
...
pub const ListStateMachineAliasesOutput = struct {
    /// If `nextToken` is returned, there are more results available. The value of
    /// `nextToken` is a unique pagination token for each page.
    /// Make the call again using the returned token to retrieve the next page. Keep
    /// all other arguments unchanged. Each pagination token expires after 24 hours.
    ...
    state_machine_aliases: ?[]const StateMachineAliasListItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStateMachineAliasesInput, options: CallOptions) !ListStateMachineAliasesOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "states");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListStateMachineAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSStepFunctions.ListStateMachineAliases");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListStateMachineAliasesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListStateMachineAliasesOutput, body, allocator);
}
