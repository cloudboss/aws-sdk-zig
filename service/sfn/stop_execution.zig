...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopExecutionInput, options: Options) !StopExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.StopExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopExecutionOutput, body, alloc);
}
