...
pub const SendTaskSuccessInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendTaskSuccessInput, options: Options) !SendTaskSuccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SendTaskSuccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSStepFunctions.SendTaskSuccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SendTaskSuccessOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
