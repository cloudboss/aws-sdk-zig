...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PublishStateMachineVersionInput, options: Options) !PublishStateMachineVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PublishStateMachineVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.PublishStateMachineVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PublishStateMachineVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PublishStateMachineVersionOutput, body, alloc);
}
