...
pub const SendTaskHeartbeatInput = struct {
...
pub const SendTaskHeartbeatOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendTaskHeartbeatInput, options: CallOptions) !SendTaskHeartbeatOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SendTaskHeartbeatInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSStepFunctions.SendTaskHeartbeat");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SendTaskHeartbeatOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
