...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExecutionHistoryInput, options: Options) !GetExecutionHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExecutionHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.GetExecutionHistory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExecutionHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetExecutionHistoryOutput, body, alloc);
}
