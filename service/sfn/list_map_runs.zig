...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMapRunsInput, options: Options) !ListMapRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMapRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.ListMapRuns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMapRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMapRunsOutput, body, alloc);
}
