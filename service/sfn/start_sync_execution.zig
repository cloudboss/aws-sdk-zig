...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSyncExecutionInput, options: Options) !StartSyncExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSyncExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.StartSyncExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSyncExecutionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StartSyncExecutionOutput, body, alloc);
}
