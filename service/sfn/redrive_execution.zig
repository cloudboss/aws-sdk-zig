...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RedriveExecutionInput, options: Options) !RedriveExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RedriveExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSStepFunctions.RedriveExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RedriveExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RedriveExecutionOutput, body, alloc);
}
