...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLoggerDefinitionVersionInput, options: Options) !GetLoggerDefinitionVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLoggerDefinitionVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.logger_definition_version_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLoggerDefinitionVersionOutput {
    var result: GetLoggerDefinitionVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLoggerDefinitionVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
