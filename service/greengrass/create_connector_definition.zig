...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConnectorDefinitionInput, options: CallOptions) !CreateConnectorDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateConnectorDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateConnectorDefinitionOutput {
    var result: CreateConnectorDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConnectorDefinitionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
