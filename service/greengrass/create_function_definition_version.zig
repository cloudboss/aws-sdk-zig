...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFunctionDefinitionVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.default_config) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DefaultConfig\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFunctionDefinitionVersionOutput {
    var result: CreateFunctionDefinitionVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFunctionDefinitionVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
