...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCoreDefinitionsInput, options: CallOptions) !ListCoreDefinitionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCoreDefinitionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCoreDefinitionsOutput {
    var result: ListCoreDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCoreDefinitionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
