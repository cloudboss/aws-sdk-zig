...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDeviceDefinitionInput, options: Options) !CreateDeviceDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeviceDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDeviceDefinitionOutput {
    var result: CreateDeviceDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDeviceDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
