...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartBulkDeploymentInput, options: Options) !StartBulkDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartBulkDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"InputFileUri\":");
    try aws.json.writeValue(@TypeOf(input.input_file_uri), input.input_file_uri, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartBulkDeploymentOutput {
    var result: StartBulkDeploymentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartBulkDeploymentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
