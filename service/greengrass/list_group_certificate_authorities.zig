...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupCertificateAuthoritiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupCertificateAuthoritiesOutput {
    var result: ListGroupCertificateAuthoritiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGroupCertificateAuthoritiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
