...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCoreDefinitionInput, options: Options) !CreateCoreDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCoreDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCoreDefinitionOutput {
    var result: CreateCoreDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCoreDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
