...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceDefinitionVersionInput, options: Options) !GetResourceDefinitionVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceDefinitionVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.resource_definition_version_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceDefinitionVersionOutput {
    var result: GetResourceDefinitionVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceDefinitionVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
