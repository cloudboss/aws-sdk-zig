...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateThingRuntimeConfigurationInput, options: Options) !UpdateThingRuntimeConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateThingRuntimeConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.telemetry_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"TelemetryConfiguration\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateThingRuntimeConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
