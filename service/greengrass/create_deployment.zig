...
pub const CreateDeploymentInput = struct {
    ...
    group_version_id: ?[]const u8 = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"DeploymentType\":");
    try aws.json.writeValue(@TypeOf(input.deployment_type), input.deployment_type, allocator, &body_buf);
    has_prev = true;
    if (input.group_version_id) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"GroupVersionId\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
