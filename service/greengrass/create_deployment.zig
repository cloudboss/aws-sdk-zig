...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DeploymentType\":");
    try aws.json.writeValue(@TypeOf(input.deployment_type), input.deployment_type, alloc, &body_buf);
    has_prev = true;
    if (input.group_version_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"GroupVersionId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
