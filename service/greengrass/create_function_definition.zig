...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFunctionDefinitionInput, options: Options) !CreateFunctionDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFunctionDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFunctionDefinitionOutput {
    var result: CreateFunctionDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFunctionDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
