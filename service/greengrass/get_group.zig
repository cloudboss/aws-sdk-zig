...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGroupInput, options: Options) !GetGroupOutput {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGroupOutput {
    var result: GetGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
