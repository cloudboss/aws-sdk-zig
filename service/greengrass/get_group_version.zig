...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGroupVersionInput, options: Options) !GetGroupVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGroupVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.group_version_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGroupVersionOutput {
    var result: GetGroupVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGroupVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
