...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAvailabilityOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateAvailabilityOptions&Version=2013-01-01");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, if (input.multi_az) "true" else "false");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAvailabilityOptionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AvailabilityOptions")) {
                    result.availability_options = try serde.deserializeAvailabilityOptionsStatus(&reader, alloc);
                } else {
