...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDomainEndpointOptionsInput, options: Options) !UpdateDomainEndpointOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDomainEndpointOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateDomainEndpointOptions&Version=2013-01-01");
    if (input.domain_endpoint_options.enforce_https) |sv| {
        try body_buf.appendSlice(alloc, "&DomainEndpointOptions.EnforceHTTPS=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (sv) "true" else "false");
    }
    if (input.domain_endpoint_options.tls_security_policy) |sv| {
        try body_buf.appendSlice(alloc, "&DomainEndpointOptions.TLSSecurityPolicy=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDomainEndpointOptionsOutput {
    _ = status;
    _ = headers;
