...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDomainsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDomains&Version=2013-01-01");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DomainStatusList")) {
                    result.domain_status_list = try serde.deserializeDomainStatusList(allocator, &reader, "member");
                } else {
