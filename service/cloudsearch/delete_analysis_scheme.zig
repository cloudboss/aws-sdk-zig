...
pub const DeleteAnalysisSchemeInput = struct {
...
pub const DeleteAnalysisSchemeOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAnalysisSchemeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteAnalysisScheme&Version=2013-01-01");
    try body_buf.appendSlice(alloc, "&AnalysisSchemeName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.analysis_scheme_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAnalysisSchemeOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteAnalysisSchemeResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteAnalysisSchemeOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AnalysisScheme")) {
                    result.analysis_scheme = try serde.deserializeAnalysisSchemeStatus(&reader, alloc);
                } else {
