...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAnalysisSchemeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteAnalysisScheme&Version=2013-01-01");
    try body_buf.appendSlice(allocator, "&AnalysisSchemeName=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.analysis_scheme_name);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAnalysisSchemeOutput {
    _ = status;
    _ = headers;
