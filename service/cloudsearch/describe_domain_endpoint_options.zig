...
pub const DescribeDomainEndpointOptionsInput = struct {
...
pub const DescribeDomainEndpointOptionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDomainEndpointOptionsInput, options: Options) !DescribeDomainEndpointOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDomainEndpointOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDomainEndpointOptions&Version=2013-01-01");
    if (input.deployed) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDomainEndpointOptionsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDomainEndpointOptionsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDomainEndpointOptionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DomainEndpointOptions")) {
                    result.domain_endpoint_options = try serde.deserializeDomainEndpointOptionsStatus(&reader, alloc);
                } else {
