...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSuggesterInput, options: CallOptions) !DeleteSuggesterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSuggesterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteSuggester&Version=2013-01-01");
    try body_buf.appendSlice(allocator, "&DomainName=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.domain_name);
    try body_buf.appendSlice(allocator, "&SuggesterName=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.suggester_name);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSuggesterOutput {
    _ = status;
    _ = headers;
