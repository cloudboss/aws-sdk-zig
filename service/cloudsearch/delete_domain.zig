...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteDomain&Version=2013-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDomainOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteDomainResult")) break;
            },
            else => {},
        }
    }
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DomainStatus")) {
                    result.domain_status = try serde.deserializeDomainStatus(allocator, &reader);
                } else {
