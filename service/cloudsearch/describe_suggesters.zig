...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSuggestersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeSuggesters&Version=2013-01-01");
    if (input.deployed) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSuggestersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Suggesters")) {
                    result.suggesters = try serde.deserializeSuggesterStatusList(&reader, alloc, "member");
                } else {
