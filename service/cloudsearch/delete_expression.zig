...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteExpressionInput, options: Options) !DeleteExpressionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteExpressionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteExpression&Version=2013-01-01");
    try body_buf.appendSlice(alloc, "&DomainName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.domain_name);
    try body_buf.appendSlice(alloc, "&ExpressionName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.expression_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteExpressionOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Expression")) {
                    result.expression = try serde.deserializeExpressionStatus(&reader, alloc);
                } else {
