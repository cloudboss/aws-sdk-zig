...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAnalysisSchemesInput, options: Options) !DescribeAnalysisSchemesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAnalysisSchemesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAnalysisSchemes&Version=2013-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAnalysisSchemesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AnalysisSchemes")) {
                    result.analysis_schemes = try serde.deserializeAnalysisSchemeStatusList(&reader, alloc, "member");
                } else {
