...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DefineSuggesterInput, options: Options) !DefineSuggesterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DefineSuggesterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DefineSuggester&Version=2013-01-01");
    ...
    if (input.suggester.document_suggester_options.fuzzy_matching) |sv2| {
        try body_buf.appendSlice(alloc, "&Suggester.DocumentSuggesterOptions.FuzzyMatching=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(sv2));
    }
    if (input.suggester.document_suggester_options.sort_expression) |sv2| {
        try body_buf.appendSlice(alloc, "&Suggester.DocumentSuggesterOptions.SortExpression=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, sv2);
    }
    try body_buf.appendSlice(alloc, "&Suggester.DocumentSuggesterOptions.SourceField=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.suggester.document_suggester_options.source_field);
    try body_buf.appendSlice(alloc, "&Suggester.SuggesterName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.suggester.suggester_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DefineSuggesterOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Suggester")) {
                    result.suggester = try serde.deserializeSuggesterStatus(&reader, alloc);
                } else {
