...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLinuxSubscriptionInstancesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLinuxSubscriptionInstancesOutput {
    var result: ListLinuxSubscriptionInstancesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLinuxSubscriptionInstancesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
