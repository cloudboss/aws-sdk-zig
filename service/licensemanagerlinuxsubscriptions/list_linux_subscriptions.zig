...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLinuxSubscriptionsInput, options: Options) !ListLinuxSubscriptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLinuxSubscriptionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLinuxSubscriptionsOutput {
    var result: ListLinuxSubscriptionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLinuxSubscriptionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
