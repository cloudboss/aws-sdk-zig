...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRegisteredSubscriptionProvidersInput, options: Options) !ListRegisteredSubscriptionProvidersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRegisteredSubscriptionProvidersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.subscription_provider_sources) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SubscriptionProviderSources\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRegisteredSubscriptionProvidersOutput {
    var result: ListRegisteredSubscriptionProvidersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRegisteredSubscriptionProvidersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
