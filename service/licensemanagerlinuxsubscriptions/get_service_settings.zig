...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceSettingsInput, options: Options) !GetServiceSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceSettingsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceSettingsOutput {
    var result: GetServiceSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetServiceSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
