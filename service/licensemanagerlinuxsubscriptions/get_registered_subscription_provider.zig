...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegisteredSubscriptionProviderInput, options: Options) !GetRegisteredSubscriptionProviderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRegisteredSubscriptionProviderInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRegisteredSubscriptionProviderOutput {
    var result: GetRegisteredSubscriptionProviderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRegisteredSubscriptionProviderOutput, body, alloc);
    }
    _ = status;
    _ = headers;
