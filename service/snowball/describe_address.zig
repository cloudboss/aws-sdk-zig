...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAddressInput, options: Options) !DescribeAddressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.DescribeAddress");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAddressOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAddressOutput, body, alloc);
}
