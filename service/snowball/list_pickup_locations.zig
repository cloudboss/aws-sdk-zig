...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPickupLocationsInput, options: Options) !ListPickupLocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPickupLocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.ListPickupLocations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPickupLocationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPickupLocationsOutput, body, alloc);
}
