...
pub const GetJobManifestInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobManifestInput, options: CallOptions) !GetJobManifestOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetJobManifestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSIESnowballJobManagementService.GetJobManifest");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetJobManifestOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetJobManifestOutput, body, allocator);
}
