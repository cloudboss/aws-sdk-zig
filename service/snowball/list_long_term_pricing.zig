...
const LongTermPricingListEntry = @import("long_term_pricing_list_entry.zig").LongTermPricingListEntry;

pub const ListLongTermPricingInput = struct {
...
pub const ListLongTermPricingOutput = struct {
    /// Each `LongTermPricingEntry` object contains a status, ID, and other
    /// information
    /// about the `LongTermPricing` type.
    long_term_pricing_entries: ?[]const LongTermPricingListEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLongTermPricingInput, options: CallOptions) !ListLongTermPricingOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "snowball");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLongTermPricingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSIESnowballJobManagementService.ListLongTermPricing");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLongTermPricingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLongTermPricingOutput, body, allocator);
}
