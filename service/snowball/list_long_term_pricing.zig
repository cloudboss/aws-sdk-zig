...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLongTermPricingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.ListLongTermPricing");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLongTermPricingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLongTermPricingOutput, body, alloc);
}
