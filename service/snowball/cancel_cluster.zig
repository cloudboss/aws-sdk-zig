...
pub const CancelClusterInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelClusterInput, options: Options) !CancelClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSIESnowballJobManagementService.CancelCluster");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelClusterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
