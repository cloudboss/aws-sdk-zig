...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.CancelCluster");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelClusterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
