...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateJobShipmentStateInput, options: Options) !UpdateJobShipmentStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateJobShipmentStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.UpdateJobShipmentState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateJobShipmentStateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
