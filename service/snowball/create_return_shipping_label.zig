...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReturnShippingLabelInput, options: Options) !CreateReturnShippingLabelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateReturnShippingLabelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.CreateReturnShippingLabel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateReturnShippingLabelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateReturnShippingLabelOutput, body, alloc);
}
