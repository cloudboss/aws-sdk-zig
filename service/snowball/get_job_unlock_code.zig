...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetJobUnlockCodeInput, options: Options) !GetJobUnlockCodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetJobUnlockCodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.GetJobUnlockCode");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetJobUnlockCodeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetJobUnlockCodeOutput, body, alloc);
}
