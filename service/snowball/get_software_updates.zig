...
pub const GetSoftwareUpdatesInput = struct {
...
pub const GetSoftwareUpdatesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSoftwareUpdatesInput, options: CallOptions) !GetSoftwareUpdatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSoftwareUpdatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSIESnowballJobManagementService.GetSoftwareUpdates");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSoftwareUpdatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSoftwareUpdatesOutput, body, allocator);
}
