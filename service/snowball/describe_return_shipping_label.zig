...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReturnShippingLabelInput, options: Options) !DescribeReturnShippingLabelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReturnShippingLabelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.DescribeReturnShippingLabel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReturnShippingLabelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReturnShippingLabelOutput, body, alloc);
}
