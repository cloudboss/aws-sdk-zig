...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListClusterJobsInput, options: Options) !ListClusterJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListClusterJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.ListClusterJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListClusterJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListClusterJobsOutput, body, alloc);
}
