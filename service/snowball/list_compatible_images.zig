...
const CompatibleImage = @import("compatible_image.zig").CompatibleImage;

pub const ListCompatibleImagesInput = struct {
    ...
    /// list of
    /// compatible images, you can specify a value for `NextToken` as the starting
    /// point
...
pub const ListCompatibleImagesOutput = struct {
    /// A JSON-formatted object that describes a compatible AMI, including the ID
    /// and name for
    /// a Snow device AMI.
    compatible_images: ?[]const CompatibleImage = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCompatibleImagesInput, options: CallOptions) !ListCompatibleImagesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCompatibleImagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSIESnowballJobManagementService.ListCompatibleImages");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCompatibleImagesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCompatibleImagesOutput, body, allocator);
}
