...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSnowballUsageInput, options: Options) !GetSnowballUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSnowballUsageInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.GetSnowballUsage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSnowballUsageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSnowballUsageOutput, body, alloc);
}
