...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAddressInput, options: Options) !CreateAddressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.CreateAddress");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAddressOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAddressOutput, body, alloc);
}
