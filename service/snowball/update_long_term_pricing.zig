...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLongTermPricingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSIESnowballJobManagementService.UpdateLongTermPricing");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLongTermPricingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
