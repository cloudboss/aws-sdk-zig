...
pub const UpdateLongTermPricingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLongTermPricingInput, options: Options) !UpdateLongTermPricingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateLongTermPricingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSIESnowballJobManagementService.UpdateLongTermPricing");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateLongTermPricingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
