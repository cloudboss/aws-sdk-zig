...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendSerialConsoleSSHPublicKeyInput, options: Options) !SendSerialConsoleSSHPublicKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendSerialConsoleSSHPublicKeyInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("ec2instanceconnect", "EC2 Instance Connect", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEC2InstanceConnectService.SendSerialConsoleSSHPublicKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendSerialConsoleSSHPublicKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SendSerialConsoleSSHPublicKeyOutput, body, alloc);
}
