...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendSSHPublicKeyInput, options: Options) !SendSSHPublicKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendSSHPublicKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEC2InstanceConnectService.SendSSHPublicKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendSSHPublicKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SendSSHPublicKeyOutput, body, alloc);
}
