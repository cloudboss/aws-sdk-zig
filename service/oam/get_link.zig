...
pub const GetLinkOutput = struct {
    /// The ARN of the link.
    arn: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLinkInput, options: Options) !GetLinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLinkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLinkOutput {
    var result: GetLinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLinkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
