...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLinkInput, options: Options) !UpdateLinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLinkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLinkOutput {
    var result: UpdateLinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateLinkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
