...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSinkInput, options: Options) !CreateSinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSinkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSinkOutput {
    var result: CreateSinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSinkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
