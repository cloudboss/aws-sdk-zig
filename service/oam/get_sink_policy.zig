...
pub const GetSinkPolicyInput = struct {
...
pub const GetSinkPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSinkPolicyInput, options: Options) !GetSinkPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSinkPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSinkPolicyOutput {
    var result: GetSinkPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSinkPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
