...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSinkInput, options: Options) !GetSinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSinkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSinkOutput {
    var result: GetSinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSinkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
