...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAttachedLinksInput, options: Options) !ListAttachedLinksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAttachedLinksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAttachedLinksOutput {
    var result: ListAttachedLinksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAttachedLinksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
