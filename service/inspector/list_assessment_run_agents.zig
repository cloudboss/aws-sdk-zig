...
const AgentFilter = @import("agent_filter.zig").AgentFilter;
const AssessmentRunAgent = @import("assessment_run_agent.zig").AssessmentRunAgent;

pub const ListAssessmentRunAgentsInput = struct {
...
pub const ListAssessmentRunAgentsOutput = struct {
    /// A list of ARNs that specifies the agents returned by the action.
    assessment_run_agents: ?[]const AssessmentRunAgent = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssessmentRunAgentsInput, options: CallOptions) !ListAssessmentRunAgentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssessmentRunAgentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "InspectorService.ListAssessmentRunAgents");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssessmentRunAgentsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListAssessmentRunAgentsOutput, body, allocator);
}
