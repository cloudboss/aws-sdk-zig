...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeResourceGroupsInput, options: Options) !DescribeResourceGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeResourceGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.DescribeResourceGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeResourceGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeResourceGroupsOutput, body, alloc);
}
