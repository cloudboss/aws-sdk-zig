...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeExclusionsInput, options: Options) !DescribeExclusionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeExclusionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.DescribeExclusions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeExclusionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeExclusionsOutput, body, alloc);
}
