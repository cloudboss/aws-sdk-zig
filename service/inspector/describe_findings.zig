...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFindingsInput, options: Options) !DescribeFindingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFindingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.DescribeFindings");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFindingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFindingsOutput, body, alloc);
}
