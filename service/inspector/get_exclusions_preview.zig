...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExclusionsPreviewInput, options: Options) !GetExclusionsPreviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExclusionsPreviewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.GetExclusionsPreview");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExclusionsPreviewOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetExclusionsPreviewOutput, body, alloc);
}
