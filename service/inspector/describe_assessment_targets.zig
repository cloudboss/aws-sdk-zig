...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAssessmentTargetsInput, options: Options) !DescribeAssessmentTargetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssessmentTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.DescribeAssessmentTargets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssessmentTargetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAssessmentTargetsOutput, body, alloc);
}
