...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTelemetryMetadataInput, options: Options) !GetTelemetryMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTelemetryMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.GetTelemetryMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTelemetryMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTelemetryMetadataOutput, body, alloc);
}
