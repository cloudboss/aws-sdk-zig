...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAssessmentRunsInput, options: Options) !DescribeAssessmentRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssessmentRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.DescribeAssessmentRuns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssessmentRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAssessmentRunsOutput, body, alloc);
}
