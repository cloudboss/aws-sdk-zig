...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEventSubscriptionsInput, options: Options) !ListEventSubscriptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventSubscriptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.ListEventSubscriptions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEventSubscriptionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListEventSubscriptionsOutput, body, alloc);
}
