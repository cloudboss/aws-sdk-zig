...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssessmentTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.ListAssessmentTargets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssessmentTargetsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListAssessmentTargetsOutput, body, alloc);
}
