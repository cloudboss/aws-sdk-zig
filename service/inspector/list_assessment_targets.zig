...
pub const ListAssessmentTargetsInput = struct {
...
pub const ListAssessmentTargetsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssessmentTargetsInput, options: CallOptions) !ListAssessmentTargetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssessmentTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "InspectorService.ListAssessmentTargets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssessmentTargetsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListAssessmentTargetsOutput, body, allocator);
}
