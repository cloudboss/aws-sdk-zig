...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRulesPackagesInput, options: Options) !DescribeRulesPackagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRulesPackagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.DescribeRulesPackages");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRulesPackagesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRulesPackagesOutput, body, alloc);
}
