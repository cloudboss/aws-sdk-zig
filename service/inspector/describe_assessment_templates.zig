...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAssessmentTemplatesInput, options: Options) !DescribeAssessmentTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssessmentTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.DescribeAssessmentTemplates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssessmentTemplatesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeAssessmentTemplatesOutput, body, alloc);
}
