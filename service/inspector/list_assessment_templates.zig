...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssessmentTemplatesInput, options: CallOptions) !ListAssessmentTemplatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssessmentTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "InspectorService.ListAssessmentTemplates");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssessmentTemplatesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListAssessmentTemplatesOutput, body, allocator);
}
