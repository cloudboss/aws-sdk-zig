...
const AssessmentTemplateFilter = @import("assessment_template_filter.zig").AssessmentTemplateFilter;

pub const ListAssessmentTemplatesInput = struct {
    /// A list of ARNs that specifies the assessment targets whose assessment
    /// templates you
    /// want to list.
    assessment_target_arns: ?[]const []const u8 = null,
...
pub const ListAssessmentTemplatesOutput = struct {
    /// A list of ARNs that specifies the assessment templates returned by the
    /// action.
    assessment_template_arns: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssessmentTemplatesInput, options: Options) !ListAssessmentTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssessmentTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.ListAssessmentTemplates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssessmentTemplatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssessmentTemplatesOutput, body, alloc);
}
