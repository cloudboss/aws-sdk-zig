...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssessmentReportInput, options: Options) !GetAssessmentReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssessmentReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.GetAssessmentReport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssessmentReportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAssessmentReportOutput, body, alloc);
}
