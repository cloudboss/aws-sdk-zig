...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssessmentTargetInput, options: Options) !CreateAssessmentTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssessmentTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.CreateAssessmentTarget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssessmentTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAssessmentTargetOutput, body, alloc);
}
