...
fn serializeRequest(alloc: std.mem.Allocator, input: StopAssessmentRunInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.StopAssessmentRun");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopAssessmentRunOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
