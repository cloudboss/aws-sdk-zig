...
const AgentPreview = @import("agent_preview.zig").AgentPreview;

pub const PreviewAgentsInput = struct {
...
pub const PreviewAgentsOutput = struct {
    /// The resulting list of agents.
    agent_previews: ?[]const AgentPreview = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PreviewAgentsInput, options: CallOptions) !PreviewAgentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PreviewAgentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "InspectorService.PreviewAgents");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PreviewAgentsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PreviewAgentsOutput, body, allocator);
}
