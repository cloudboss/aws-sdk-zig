...
pub const ListRulesPackagesInput = struct {
...
pub const ListRulesPackagesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRulesPackagesInput, options: CallOptions) !ListRulesPackagesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRulesPackagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "InspectorService.ListRulesPackages");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRulesPackagesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListRulesPackagesOutput, body, allocator);
}
