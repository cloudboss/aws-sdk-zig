...
pub const UnsubscribeFromEventInput = struct {
...
pub const UnsubscribeFromEventOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UnsubscribeFromEventInput, options: CallOptions) !UnsubscribeFromEventOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UnsubscribeFromEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "InspectorService.UnsubscribeFromEvent");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UnsubscribeFromEventOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
