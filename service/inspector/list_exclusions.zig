...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExclusionsInput, options: Options) !ListExclusionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExclusionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.ListExclusions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExclusionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListExclusionsOutput, body, alloc);
}
