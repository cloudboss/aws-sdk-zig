...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResourceGroupInput, options: Options) !CreateResourceGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourceGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.CreateResourceGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourceGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateResourceGroupOutput, body, alloc);
}
