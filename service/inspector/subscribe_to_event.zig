...
pub const SubscribeToEventOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SubscribeToEventInput, options: CallOptions) !SubscribeToEventOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SubscribeToEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "InspectorService.SubscribeToEvent");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SubscribeToEventOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
