...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssessmentRunsInput, options: CallOptions) !ListAssessmentRunsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssessmentRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "InspectorService.ListAssessmentRuns");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssessmentRunsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListAssessmentRunsOutput, body, allocator);
}
