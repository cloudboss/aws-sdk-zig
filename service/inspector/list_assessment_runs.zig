...
const AssessmentRunFilter = @import("assessment_run_filter.zig").AssessmentRunFilter;

pub const ListAssessmentRunsInput = struct {
...
pub const ListAssessmentRunsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssessmentRunsInput, options: Options) !ListAssessmentRunsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssessmentRunsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "InspectorService.ListAssessmentRuns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssessmentRunsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAssessmentRunsOutput, body, alloc);
}
