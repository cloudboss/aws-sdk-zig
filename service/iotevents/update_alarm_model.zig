...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAlarmModelInput, options: CallOptions) !UpdateAlarmModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAlarmModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/alarm-models/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAlarmModelOutput {
    var result: UpdateAlarmModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAlarmModelOutput, body, allocator);
    }
    _ = status;
    _ = headers;
