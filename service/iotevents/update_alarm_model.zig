...
pub const UpdateAlarmModelInput = struct {
...
pub const UpdateAlarmModelOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAlarmModelInput, options: Options) !UpdateAlarmModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAlarmModelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.alarm_capabilities) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"alarmCapabilities\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.alarm_event_actions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"alarmEventActions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.alarm_model_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"alarmModelDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.alarm_notification) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"alarmNotification\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"alarmRule\":");
    try aws.json.writeValue(@TypeOf(input.alarm_rule), input.alarm_rule, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAlarmModelOutput {
    var result: UpdateAlarmModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAlarmModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
