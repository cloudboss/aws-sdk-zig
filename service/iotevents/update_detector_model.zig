...
pub const UpdateDetectorModelInput = struct {
...
pub const UpdateDetectorModelOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDetectorModelInput, options: CallOptions) !UpdateDetectorModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDetectorModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDetectorModelOutput {
    var result: UpdateDetectorModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDetectorModelOutput, body, allocator);
    }
    _ = status;
    _ = headers;
