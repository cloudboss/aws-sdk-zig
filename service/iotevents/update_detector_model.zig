...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDetectorModelInput, options: Options) !UpdateDetectorModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDetectorModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDetectorModelOutput {
    var result: UpdateDetectorModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDetectorModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
