...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDetectorModelAnalysisInput, options: Options) !DescribeDetectorModelAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDetectorModelAnalysisInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDetectorModelAnalysisOutput {
    var result: DescribeDetectorModelAnalysisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDetectorModelAnalysisOutput, body, alloc);
    }
    _ = status;
    _ = headers;
