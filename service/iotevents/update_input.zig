...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/inputs/");
    try path_buf.appendSlice(alloc, input.input_name);
    ...
    try body_buf.appendSlice(alloc, "\"inputDefinition\":");
    try aws.json.writeValue(@TypeOf(input.input_definition), input.input_definition, alloc, &body_buf);
    has_prev = true;
    if (input.input_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"inputDescription\":");
