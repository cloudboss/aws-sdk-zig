...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDetectorModelInput, options: Options) !CreateDetectorModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDetectorModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"detectorModelDefinition\":");
    try aws.json.writeValue(@TypeOf(input.detector_model_definition), input.detector_model_definition, alloc, &body_buf);
    has_prev = true;
    if (input.detector_model_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"detectorModelDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"detectorModelName\":");
    try aws.json.writeValue(@TypeOf(input.detector_model_name), input.detector_model_name, alloc, &body_buf);
    has_prev = true;
    if (input.evaluation_method) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"evaluationMethod\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.key) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDetectorModelOutput {
    var result: CreateDetectorModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDetectorModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
