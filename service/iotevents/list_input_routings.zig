...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInputRoutingsInput, options: Options) !ListInputRoutingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInputRoutingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"inputIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.input_identifier), input.input_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInputRoutingsOutput {
    var result: ListInputRoutingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInputRoutingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
