...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAlarmModelInput, options: CallOptions) !DescribeAlarmModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAlarmModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAlarmModelOutput {
    var result: DescribeAlarmModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAlarmModelOutput, body, allocator);
    }
    _ = status;
    _ = headers;
