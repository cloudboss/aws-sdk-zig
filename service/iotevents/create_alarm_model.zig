...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAlarmModelInput, options: Options) !CreateAlarmModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAlarmModelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"alarmModelName\":");
    try aws.json.writeValue(@TypeOf(input.alarm_model_name), input.alarm_model_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAlarmModelOutput {
    var result: CreateAlarmModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAlarmModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
