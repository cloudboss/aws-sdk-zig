...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDetectorModelInput, options: Options) !DescribeDetectorModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDetectorModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDetectorModelOutput {
    var result: DescribeDetectorModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDetectorModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
