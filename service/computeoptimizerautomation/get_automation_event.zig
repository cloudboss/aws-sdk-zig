...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutomationEventInput, options: Options) !GetAutomationEventOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAutomationEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerAutomationService.GetAutomationEvent");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAutomationEventOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAutomationEventOutput, body, alloc);
}
