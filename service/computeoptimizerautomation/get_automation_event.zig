...
pub const GetAutomationEventInput = struct {
...
pub const GetAutomationEventOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutomationEventInput, options: CallOptions) !GetAutomationEventOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAutomationEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerAutomationService.GetAutomationEvent");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAutomationEventOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAutomationEventOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "IdempotencyTokenInUseException")) {
        const parsed_error: ?errors.IdempotencyTokenInUseException = aws.json.parseJsonObject(errors.IdempotencyTokenInUseException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .idempotency_token_in_use_exception = typed_error } };
        }
    }
    ...
    if (std.mem.eql(u8, error_code, "NotManagementAccountException")) {
        const parsed_error: ?errors.NotManagementAccountException = aws.json.parseJsonObject(errors.NotManagementAccountException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .not_management_account_exception = typed_error } };
        }
    }
