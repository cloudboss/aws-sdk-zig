...
pub const UpdateAutomationRuleInput = struct {
...
pub const UpdateAutomationRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAutomationRuleInput, options: CallOptions) !UpdateAutomationRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAutomationRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerAutomationService.UpdateAutomationRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAutomationRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateAutomationRuleOutput, body, allocator);
}
