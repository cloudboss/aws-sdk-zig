...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecommendedActionSummariesInput, options: Options) !ListRecommendedActionSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecommendedActionSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerAutomationService.ListRecommendedActionSummaries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecommendedActionSummariesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRecommendedActionSummariesOutput, body, alloc);
}
