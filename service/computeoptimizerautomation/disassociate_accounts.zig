...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateAccountsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerAutomationService.DisassociateAccounts");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateAccountsOutput {
    ...
    return aws.json.parseJsonObject(DisassociateAccountsOutput, body, allocator);
}
