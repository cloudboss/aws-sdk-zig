...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutomationRulePreviewSummariesInput, options: Options) !ListAutomationRulePreviewSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutomationRulePreviewSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerAutomationService.ListAutomationRulePreviewSummaries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutomationRulePreviewSummariesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAutomationRulePreviewSummariesOutput, body, alloc);
}
