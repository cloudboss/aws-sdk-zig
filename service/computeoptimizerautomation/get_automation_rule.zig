...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutomationRuleInput, options: Options) !GetAutomationRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAutomationRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerAutomationService.GetAutomationRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAutomationRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAutomationRuleOutput, body, alloc);
}
