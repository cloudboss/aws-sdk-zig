...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutomationEventSummariesInput, options: Options) !ListAutomationEventSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAutomationEventSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerAutomationService.ListAutomationEventSummaries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAutomationEventSummariesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAutomationEventSummariesOutput, body, alloc);
}
