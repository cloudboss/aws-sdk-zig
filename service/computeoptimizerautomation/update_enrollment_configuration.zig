...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEnrollmentConfigurationInput, options: Options) !UpdateEnrollmentConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEnrollmentConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerAutomationService.UpdateEnrollmentConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEnrollmentConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateEnrollmentConfigurationOutput, body, alloc);
}
