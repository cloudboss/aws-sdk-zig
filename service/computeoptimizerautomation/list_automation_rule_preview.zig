...
const PreviewResult = @import("preview_result.zig").PreviewResult;

pub const ListAutomationRulePreviewInput = struct {
...
pub const ListAutomationRulePreviewOutput = struct {
    ...
    preview_results: ?[]const PreviewResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAutomationRulePreviewInput, options: CallOptions) !ListAutomationRulePreviewOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAutomationRulePreviewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerAutomationService.ListAutomationRulePreview");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAutomationRulePreviewOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAutomationRulePreviewOutput, body, allocator);
}
