...
pub const GetSlotTypesInput = struct {
...
pub const GetSlotTypesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSlotTypesInput, options: CallOptions) !GetSlotTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSlotTypesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSlotTypesOutput {
    var result: GetSlotTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSlotTypesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
