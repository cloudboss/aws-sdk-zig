...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSlotTypesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSlotTypesOutput {
    var result: GetSlotTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSlotTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
