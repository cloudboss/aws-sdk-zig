...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIntentVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIntentVersionsOutput {
    var result: GetIntentVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIntentVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
