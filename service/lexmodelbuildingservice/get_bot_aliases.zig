...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBotAliasesInput, options: Options) !GetBotAliasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBotAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBotAliasesOutput {
    var result: GetBotAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBotAliasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
