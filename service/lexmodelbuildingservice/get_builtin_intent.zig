...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBuiltinIntentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/builtins/intents/");
    try path_buf.appendSlice(alloc, input.signature);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBuiltinIntentOutput {
    var result: GetBuiltinIntentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBuiltinIntentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
