...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMigrationInput, options: Options) !GetMigrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/migrations/");
    try path_buf.appendSlice(alloc, input.migration_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMigrationOutput {
    var result: GetMigrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMigrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
