...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBuiltinIntentsInput, options: Options) !GetBuiltinIntentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBuiltinIntentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.signature_contains) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "signatureContains=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBuiltinIntentsOutput {
    var result: GetBuiltinIntentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBuiltinIntentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
