...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBotInput, options: Options) !PutBotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBotInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.abort_statement) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"abortStatement\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"childDirected\":");
    try aws.json.writeValue(@TypeOf(input.child_directed), input.child_directed, alloc, &body_buf);
    has_prev = true;
    if (input.clarification_prompt) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"clarificationPrompt\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.detect_sentiment) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"detectSentiment\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.enable_model_improvements) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"enableModelImprovements\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.intents) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"intents\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"locale\":");
    try aws.json.writeValue(@TypeOf(input.locale), input.locale, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.process_behavior) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"processBehavior\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutBotOutput {
    var result: PutBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutBotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
