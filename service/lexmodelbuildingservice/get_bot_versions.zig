...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBotVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBotVersionsOutput {
    var result: GetBotVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBotVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
