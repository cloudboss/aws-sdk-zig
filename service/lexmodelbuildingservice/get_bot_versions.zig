...
const BotMetadata = @import("bot_metadata.zig").BotMetadata;

pub const GetBotVersionsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBotVersionsInput, options: CallOptions) !GetBotVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetBotVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetBotVersionsOutput {
    var result: GetBotVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBotVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
