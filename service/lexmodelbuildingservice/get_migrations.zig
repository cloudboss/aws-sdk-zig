...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMigrationsInput, options: Options) !GetMigrationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMigrationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.migration_status_equals) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "migrationStatusEquals=");
    ...
    if (input.sort_by_attribute) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sortByAttribute=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.sort_by_order) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sortByOrder=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.v_1_bot_name_contains) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "v1BotNameContains=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMigrationsOutput {
    var result: GetMigrationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMigrationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
