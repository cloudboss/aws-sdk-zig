...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSlotTypeInput, options: Options) !GetSlotTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSlotTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSlotTypeOutput {
    var result: GetSlotTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSlotTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
