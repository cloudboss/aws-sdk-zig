...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBotChannelAssociationsInput, options: Options) !GetBotChannelAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBotChannelAssociationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBotChannelAssociationsOutput {
    var result: GetBotChannelAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBotChannelAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
