...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutBotAliasInput, options: Options) !PutBotAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutBotAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.bot_version), input.bot_version, alloc, &body_buf);
    has_prev = true;
    if (input.checksum) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.conversation_logs) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"conversationLogs\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutBotAliasOutput {
    var result: PutBotAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutBotAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
