...
const IntentMetadata = @import("intent_metadata.zig").IntentMetadata;

pub const GetIntentsInput = struct {
...
pub const GetIntentsOutput = struct {
    /// An array of `Intent` objects. For more information, see
    /// PutBot.
    intents: ?[]const IntentMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIntentsInput, options: Options) !GetIntentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIntentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIntentsOutput {
    var result: GetIntentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIntentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
