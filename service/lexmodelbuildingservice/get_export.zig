...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "exportType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.export_type));
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.version);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExportOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetExportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
