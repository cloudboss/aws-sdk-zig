...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUtterancesViewInput, options: Options) !GetUtterancesViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUtterancesViewInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/utterances");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    try query_buf.appendSlice(alloc, "view=aggregation");
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "bot_versions=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.bot_versions);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "status_type=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.status_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUtterancesViewOutput {
    var result: GetUtterancesViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUtterancesViewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
