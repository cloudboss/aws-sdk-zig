...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIntentVersionInput, options: CallOptions) !CreateIntentVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateIntentVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateIntentVersionOutput {
    var result: CreateIntentVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIntentVersionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
