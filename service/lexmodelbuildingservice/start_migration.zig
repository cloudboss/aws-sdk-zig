...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMigrationInput, options: Options) !StartMigrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMigrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"migrationStrategy\":");
    try aws.json.writeValue(@TypeOf(input.migration_strategy), input.migration_strategy, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"v1BotName\":");
    try aws.json.writeValue(@TypeOf(input.v_1_bot_name), input.v_1_bot_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"v1BotVersion\":");
    try aws.json.writeValue(@TypeOf(input.v_1_bot_version), input.v_1_bot_version, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"v2BotName\":");
    try aws.json.writeValue(@TypeOf(input.v_2_bot_name), input.v_2_bot_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"v2BotRole\":");
    try aws.json.writeValue(@TypeOf(input.v_2_bot_role), input.v_2_bot_role, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMigrationOutput {
    var result: StartMigrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMigrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
