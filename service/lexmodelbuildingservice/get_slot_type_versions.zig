...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSlotTypeVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSlotTypeVersionsOutput {
    var result: GetSlotTypeVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSlotTypeVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
