...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIntentInput, options: Options) !GetIntentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIntentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIntentOutput {
    var result: GetIntentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIntentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
