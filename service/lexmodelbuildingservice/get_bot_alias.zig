...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBotAliasInput, options: Options) !GetBotAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBotAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBotAliasOutput {
    var result: GetBotAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBotAliasOutput, body, alloc);
    }
    _ = status;
    _ = headers;
