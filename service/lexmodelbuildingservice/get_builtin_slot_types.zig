...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBuiltinSlotTypesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBuiltinSlotTypesOutput {
    var result: GetBuiltinSlotTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBuiltinSlotTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
