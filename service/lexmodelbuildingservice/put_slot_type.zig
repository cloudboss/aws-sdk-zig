...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSlotTypeInput, options: Options) !PutSlotTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutSlotTypeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.enumeration_values) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"enumerationValues\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.slot_type_configurations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"slotTypeConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.value_selection_strategy) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"valueSelectionStrategy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutSlotTypeOutput {
    var result: PutSlotTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutSlotTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
