...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSlotTypeVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSlotTypeVersionOutput {
    var result: CreateSlotTypeVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSlotTypeVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
