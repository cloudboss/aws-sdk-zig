...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListObjectivesInput, options: Options) !ListObjectivesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListObjectivesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.objective_filter) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ObjectiveFilter\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListObjectivesOutput {
    var result: ListObjectivesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListObjectivesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
