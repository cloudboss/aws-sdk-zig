...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptLinkInput, options: Options) !AcceptLinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptLinkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptLinkOutput {
    var result: AcceptLinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptLinkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
