...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInboundExternalLinkInput, options: CallOptions) !CreateInboundExternalLinkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateInboundExternalLinkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateInboundExternalLinkOutput {
    var result: CreateInboundExternalLinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateInboundExternalLinkOutput, body, allocator);
    }
    _ = status;
    _ = headers;
