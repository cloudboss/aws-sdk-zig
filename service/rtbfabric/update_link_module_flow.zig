...
pub const UpdateLinkModuleFlowInput = struct {
    ...
    modules: []const ModuleConfiguration,
...
pub const UpdateLinkModuleFlowOutput = struct {
    /// The unique identifier of the gateway.
    gateway_id: []const u8,
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLinkModuleFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/module-flow");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"modules\":");
    try aws.json.writeValue(@TypeOf(input.modules), input.modules, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLinkModuleFlowOutput {
    var result: UpdateLinkModuleFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateLinkModuleFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
