...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateLinkModuleFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/module-flow");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"modules\":");
    try aws.json.writeValue(@TypeOf(input.modules), input.modules, allocator, &body_buf);
    has_prev = true;
