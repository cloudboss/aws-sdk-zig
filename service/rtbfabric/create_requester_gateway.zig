...
pub const CreateRequesterGatewayOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRequesterGatewayInput, options: Options) !CreateRequesterGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRequesterGatewayInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRequesterGatewayOutput {
    var result: CreateRequesterGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRequesterGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
