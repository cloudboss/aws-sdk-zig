...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResponderGatewayInput, options: Options) !UpdateResponderGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResponderGatewayInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResponderGatewayOutput {
    var result: UpdateResponderGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResponderGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
