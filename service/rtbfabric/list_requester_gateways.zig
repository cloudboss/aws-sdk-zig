...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRequesterGatewaysInput, options: Options) !ListRequesterGatewaysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRequesterGatewaysInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRequesterGatewaysOutput {
    var result: ListRequesterGatewaysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRequesterGatewaysOutput, body, alloc);
    }
    _ = status;
    _ = headers;
