...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRequesterGatewayInput, options: Options) !UpdateRequesterGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRequesterGatewayInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRequesterGatewayOutput {
    var result: UpdateRequesterGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRequesterGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
