...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResponderGatewayInput, options: Options) !GetResponderGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResponderGatewayInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResponderGatewayOutput {
    var result: GetResponderGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResponderGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
