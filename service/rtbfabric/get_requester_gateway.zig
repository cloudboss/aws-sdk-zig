...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRequesterGatewayInput, options: Options) !GetRequesterGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRequesterGatewayInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRequesterGatewayOutput {
    var result: GetRequesterGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRequesterGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
