...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInboundExternalLinkInput, options: Options) !GetInboundExternalLinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInboundExternalLinkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInboundExternalLinkOutput {
    var result: GetInboundExternalLinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInboundExternalLinkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
