...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOutboundExternalLinkInput, options: CallOptions) !CreateOutboundExternalLinkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateOutboundExternalLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"publicEndpoint\":");
    try aws.json.writeValue(@TypeOf(input.public_endpoint), input.public_endpoint, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
