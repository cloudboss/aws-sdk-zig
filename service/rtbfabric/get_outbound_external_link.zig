...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetOutboundExternalLinkInput, options: Options) !GetOutboundExternalLinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetOutboundExternalLinkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetOutboundExternalLinkOutput {
    var result: GetOutboundExternalLinkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetOutboundExternalLinkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
