...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSubCheckResultsInput, options: Options) !ListSubCheckResultsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSubCheckResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.operation_id), input.operation_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSubCheckResultsOutput {
    var result: ListSubCheckResultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSubCheckResultsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
