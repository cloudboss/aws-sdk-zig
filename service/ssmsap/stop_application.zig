...
fn serializeRequest(alloc: std.mem.Allocator, input: StopApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_ec_2_instance_shutdown) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IncludeEc2InstanceShutdown\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.stop_connected_entity) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"StopConnectedEntity\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopApplicationOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopApplicationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
