...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConfigurationCheckOperationsInput, options: CallOptions) !ListConfigurationCheckOperationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListConfigurationCheckOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.list_mode) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ListMode\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
