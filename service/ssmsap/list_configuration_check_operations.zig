...
const ConfigurationCheckOperationListingMode = @import("configuration_check_operation_listing_mode.zig").ConfigurationCheckOperationListingMode;
const ConfigurationCheckOperation = @import("configuration_check_operation.zig").ConfigurationCheckOperation;

pub const ListConfigurationCheckOperationsInput = struct {
...
pub const ListConfigurationCheckOperationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConfigurationCheckOperationsInput, options: Options) !ListConfigurationCheckOperationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConfigurationCheckOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/list-configuration-check-operations";
    ...
    if (input.list_mode) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ListMode\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConfigurationCheckOperationsOutput {
    var result: ListConfigurationCheckOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConfigurationCheckOperationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
