...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartConfigurationChecksInput, options: Options) !StartConfigurationChecksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartConfigurationChecksInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.configuration_check_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ConfigurationCheckIds\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartConfigurationChecksOutput {
    var result: StartConfigurationChecksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartConfigurationChecksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
