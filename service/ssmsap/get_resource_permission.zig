...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourcePermissionInput, options: Options) !GetResourcePermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourcePermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourcePermissionOutput {
    var result: GetResourcePermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourcePermissionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
