...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResourcePermissionInput, options: Options) !DeleteResourcePermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteResourcePermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteResourcePermissionOutput {
    var result: DeleteResourcePermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteResourcePermissionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
