...
const DatabaseSummary = @import("database_summary.zig").DatabaseSummary;
...
pub const ListDatabasesOutput = struct {
    ...
    databases: ?[]const DatabaseSummary = null,
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDatabasesOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDatabasesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
