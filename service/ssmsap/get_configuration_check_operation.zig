...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConfigurationCheckOperationInput, options: Options) !GetConfigurationCheckOperationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfigurationCheckOperationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfigurationCheckOperationOutput {
    var result: GetConfigurationCheckOperationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfigurationCheckOperationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
