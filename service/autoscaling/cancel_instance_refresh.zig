...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelInstanceRefreshInput, options: Options) !CancelInstanceRefreshOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelInstanceRefreshInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CancelInstanceRefresh&Version=2011-01-01");
    ...
    if (input.wait_for_transitioning_instances) |v| {
        try body_buf.appendSlice(alloc, "&WaitForTransitioningInstances=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelInstanceRefreshOutput {
    _ = status;
    _ = headers;
