...
pub const AutoScalingGroup = struct {
    ...
    /// only.
    desired_capacity_type: ?[]const u8,

    /// The metrics enabled for the Auto Scaling group.
    enabled_metrics: ?[]const EnabledMetric,
    ...
    suspended_processes: ?[]const SuspendedProcess,
    ...
    termination_policies: ?[]const []const u8,
