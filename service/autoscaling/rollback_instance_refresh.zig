...
pub const RollbackInstanceRefreshInput = struct {
    /// The name of the Auto Scaling group.
    auto_scaling_group_name: []const u8,
};

pub const RollbackInstanceRefreshOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RollbackInstanceRefreshInput, options: CallOptions) !RollbackInstanceRefreshOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RollbackInstanceRefreshInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RollbackInstanceRefresh&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RollbackInstanceRefreshOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RollbackInstanceRefreshResult")) break;
            },
            else => {},
        }
    }

    var result: RollbackInstanceRefreshOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
