...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteScheduledActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=BatchDeleteScheduledAction&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteScheduledActionOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "FailedScheduledActions")) {
                    result.failed_scheduled_actions = try serde.deserializeFailedScheduledUpdateGroupActionRequests(&reader, alloc, "member");
                } else {
