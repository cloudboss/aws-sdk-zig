...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceRefreshesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInstanceRefreshes&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceRefreshesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InstanceRefreshes")) {
                    result.instance_refreshes = try serde.deserializeInstanceRefreshes(&reader, alloc, "member");
