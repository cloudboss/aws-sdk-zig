...
pub const DescribeInstanceRefreshesInput = struct {
    /// The name of the Auto Scaling group.
    auto_scaling_group_name: []const u8,

    /// One or more instance refresh IDs.
    instance_refresh_ids: ?[]const []const u8 = null,
...
pub const DescribeInstanceRefreshesOutput = struct {
    /// The instance refreshes for the specified group, sorted by creation timestamp
    /// in
    /// descending order.
    instance_refreshes: ?[]const InstanceRefresh = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceRefreshesInput, options: CallOptions) !DescribeInstanceRefreshesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstanceRefreshesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstanceRefreshes&Version=2011-01-01");
    ...
    if (input.instance_refresh_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InstanceRefreshIds.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstanceRefreshesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeInstanceRefreshesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeInstanceRefreshesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InstanceRefreshes")) {
                    result.instance_refreshes = try serde.deserializeInstanceRefreshes(allocator, &reader, "member");
