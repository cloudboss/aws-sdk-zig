...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAutoScalingGroupsInput, options: CallOptions) !DescribeAutoScalingGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAutoScalingGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAutoScalingGroups&Version=2011-01-01");
    ...
    if (input.include_instances) |v| {
        try body_buf.appendSlice(allocator, "&IncludeInstances=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAutoScalingGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AutoScalingGroups")) {
                    result.auto_scaling_groups = try serde.deserializeAutoScalingGroups(allocator, &reader, "member");
