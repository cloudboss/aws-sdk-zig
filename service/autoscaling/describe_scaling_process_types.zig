...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeScalingProcessTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeScalingProcessTypes&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeScalingProcessTypesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeScalingProcessTypesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeScalingProcessTypesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Processes")) {
                    result.processes = try serde.deserializeProcesses(allocator, &reader, "member");
                } else {
