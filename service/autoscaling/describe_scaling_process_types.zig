...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScalingProcessTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeScalingProcessTypes&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeScalingProcessTypesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Processes")) {
                    result.processes = try serde.deserializeProcesses(&reader, alloc, "member");
                } else {
