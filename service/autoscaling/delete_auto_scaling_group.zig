...
pub const DeleteAutoScalingGroupInput = struct {
    /// The name of the Auto Scaling group.
    auto_scaling_group_name: []const u8,

    /// Specifies that the group is to be deleted along with all instances
    /// associated with the
    /// group, without waiting for all instances to be terminated. This action also
    /// deletes any
    /// outstanding lifecycle actions associated with the group.
    force_delete: ?bool = null,
};

pub const DeleteAutoScalingGroupOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAutoScalingGroupInput, options: CallOptions) !DeleteAutoScalingGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAutoScalingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteAutoScalingGroup&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAutoScalingGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteAutoScalingGroupOutput = .{};
