...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWarmPoolInput, options: CallOptions) !DescribeWarmPoolOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeWarmPoolInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeWarmPoolOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Instances")) {
                    ...
                    result.warm_pool_configuration = try serde.deserializeWarmPoolConfiguration(allocator, &reader);
                } else {
