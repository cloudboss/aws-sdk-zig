...
pub const DescribeWarmPoolInput = struct {
...
pub const DescribeWarmPoolOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWarmPoolInput, options: Options) !DescribeWarmPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWarmPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeWarmPool&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWarmPoolOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeWarmPoolResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeWarmPoolOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Instances")) {
                    ...
                    result.warm_pool_configuration = try serde.deserializeWarmPoolConfiguration(&reader, alloc);
                } else {
