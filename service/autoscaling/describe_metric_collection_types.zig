...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMetricCollectionTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeMetricCollectionTypes&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMetricCollectionTypesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Granularities")) {
                    result.granularities = try serde.deserializeMetricGranularityTypes(&reader, alloc, "member");
                } else if (std.mem.eql(u8, e.local, "Metrics")) {
                    result.metrics = try serde.deserializeMetricCollectionTypes(&reader, alloc, "member");
                } else {
