...
const SetInstanceHealthOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetInstanceHealthInput, options: Options) !SetInstanceHealthOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetInstanceHealthInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetInstanceHealth&Version=2011-01-01");
    try body_buf.appendSlice(alloc, "&HealthStatus=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.health_status);
    ...
    if (input.should_respect_grace_period) |v| {
        try body_buf.appendSlice(alloc, "&ShouldRespectGracePeriod=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetInstanceHealthOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
