...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScheduledActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeScheduledActions&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeScheduledActionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.scheduled_update_group_actions = try serde.deserializeScheduledUpdateGroupActions(&reader, alloc, "member");
                } else {
