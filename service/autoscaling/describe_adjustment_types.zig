...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAdjustmentTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAdjustmentTypes&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAdjustmentTypesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AdjustmentTypes")) {
                    result.adjustment_types = try serde.deserializeAdjustmentTypes(&reader, alloc, "member");
                } else {
