...
pub const DescribeAdjustmentTypesInput = struct {};

pub const DescribeAdjustmentTypesOutput = struct {
    /// The policy adjustment types.
    adjustment_types: ?[]const AdjustmentType = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAdjustmentTypesInput, options: CallOptions) !DescribeAdjustmentTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAdjustmentTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAdjustmentTypes&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAdjustmentTypesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeAdjustmentTypesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeAdjustmentTypesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AdjustmentTypes")) {
                    result.adjustment_types = try serde.deserializeAdjustmentTypes(allocator, &reader, "member");
                } else {
