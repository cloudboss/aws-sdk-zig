...
/// Auto Scaling.
pub const PredictiveScalingConfiguration = struct {
    ...
    ///   capacity
    /// of the group when the forecast capacity is close to or exceeds the maximum
    /// capacity. The upper limit is determined by the forecasted capacity and the
    /// value
    /// for `MaxCapacityBuffer`.
    ///
    /// Use caution when allowing the maximum capacity to be automatically
    /// increased. This
    /// can lead to more instances being launched than intended if the increased
    /// maximum
    /// capacity is not monitored and managed. The increased maximum capacity then
    /// becomes
    /// the new normal maximum capacity for the Auto Scaling group until you
    /// manually update
    ...
    /// advanced. For
    /// example, the forecast says to add capacity at 10:00 AM, and you choose to
    /// pre-launch
    /// instances by 5 minutes. In that case, the instances will be launched at 9:55
    /// AM. The
    /// intention is to give resources time to be provisioned. It can take a few
    /// minutes to
    /// launch an EC2 instance. The actual amount of time required depends on
    /// several factors,
