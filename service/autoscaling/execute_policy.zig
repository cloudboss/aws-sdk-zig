...
const ExecutePolicyOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecutePolicyInput, options: Options) !ExecutePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecutePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ExecutePolicy&Version=2011-01-01");
    ...
    if (input.breach_threshold) |v| {
        try body_buf.appendSlice(alloc, "&BreachThreshold=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    ...
    if (input.metric_value) |v| {
        try body_buf.appendSlice(alloc, "&MetricValue=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExecutePolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
