...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTagsInput, options: Options) !DescribeTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTags&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTagsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.tags = try serde.deserializeTagDescriptionList(&reader, alloc, "member");
                } else {
