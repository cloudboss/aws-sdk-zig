...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTerminationPolicyTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTerminationPolicyTypes&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTerminationPolicyTypesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TerminationPolicyTypes")) {
                    result.termination_policy_types = try serde.deserializeTerminationPolicies(&reader, alloc, "member");
                } else {
