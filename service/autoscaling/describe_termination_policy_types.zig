...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTerminationPolicyTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTerminationPolicyTypes&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTerminationPolicyTypesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeTerminationPolicyTypesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeTerminationPolicyTypesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TerminationPolicyTypes")) {
                    result.termination_policy_types = try serde.deserializeTerminationPolicies(allocator, &reader, "member");
                } else {
