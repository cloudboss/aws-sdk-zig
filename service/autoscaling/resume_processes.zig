...
pub const ResumeProcessesInput = struct {
...
pub const ResumeProcessesOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResumeProcessesInput, options: CallOptions) !ResumeProcessesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResumeProcessesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ResumeProcesses&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResumeProcessesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: ResumeProcessesOutput = .{};
