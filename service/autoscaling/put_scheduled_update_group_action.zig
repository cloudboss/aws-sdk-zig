...
fn serializeRequest(alloc: std.mem.Allocator, input: PutScheduledUpdateGroupActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PutScheduledUpdateGroupAction&Version=2011-01-01");
    ...
    if (input.recurrence) |v| {
        try body_buf.appendSlice(alloc, "&Recurrence=");
    ...
    if (input.time) |v| {
        try body_buf.appendSlice(alloc, "&Time=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutScheduledUpdateGroupActionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
