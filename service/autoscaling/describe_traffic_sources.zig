...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrafficSourcesInput, options: Options) !DescribeTrafficSourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrafficSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTrafficSources&Version=2011-01-01");
    ...
    if (input.traffic_source_type) |v| {
        try body_buf.appendSlice(alloc, "&TrafficSourceType=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrafficSourcesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.traffic_sources = try serde.deserializeTrafficSourceStates(&reader, alloc, "member");
                } else {
