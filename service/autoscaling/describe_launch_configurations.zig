...
pub const DescribeLaunchConfigurationsInput = struct {
    /// The launch configuration names. If you omit this property, all launch
    /// configurations
    /// are described.
    ///
    /// Array Members: Maximum number of 50 items.
    launch_configuration_names: ?[]const []const u8 = null,
...
pub const DescribeLaunchConfigurationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLaunchConfigurationsInput, options: CallOptions) !DescribeLaunchConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeLaunchConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeLaunchConfigurations&Version=2011-01-01");
    if (input.launch_configuration_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&LaunchConfigurationNames.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeLaunchConfigurationsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeLaunchConfigurationsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeLaunchConfigurationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LaunchConfigurations")) {
                    result.launch_configurations = try serde.deserializeLaunchConfigurations(allocator, &reader, "member");
