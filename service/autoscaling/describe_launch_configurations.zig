...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLaunchConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeLaunchConfigurations&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLaunchConfigurationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LaunchConfigurations")) {
                    result.launch_configurations = try serde.deserializeLaunchConfigurations(&reader, alloc, "member");
