...
pub const DescribeNotificationConfigurationsInput = struct {
...
pub const DescribeNotificationConfigurationsOutput = struct {
    ...
    notification_configurations: ?[]const NotificationConfiguration = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNotificationConfigurationsInput, options: CallOptions) !DescribeNotificationConfigurationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeNotificationConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeNotificationConfigurations&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeNotificationConfigurationsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeNotificationConfigurationsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeNotificationConfigurationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    result.next_token = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "NotificationConfigurations")) {
                    result.notification_configurations = try serde.deserializeNotificationConfigurations(allocator, &reader, "member");
                } else {
