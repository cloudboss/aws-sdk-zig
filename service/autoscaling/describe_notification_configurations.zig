...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNotificationConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeNotificationConfigurations&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNotificationConfigurationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.notification_configurations = try serde.deserializeNotificationConfigurations(&reader, alloc, "member");
                } else {
