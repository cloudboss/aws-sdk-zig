...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePoliciesInput, options: Options) !DescribePoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribePolicies&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePoliciesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.scaling_policies = try serde.deserializeScalingPolicies(&reader, alloc, "member");
                } else {
