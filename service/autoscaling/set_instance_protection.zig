...
pub const SetInstanceProtectionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetInstanceProtectionInput, options: Options) !SetInstanceProtectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetInstanceProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=SetInstanceProtection&Version=2011-01-01");
    ...
    try body_buf.appendSlice(allocator, "&ProtectedFromScaleIn=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, if (input.protected_from_scale_in) "true" else "false");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetInstanceProtectionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: SetInstanceProtectionOutput = .{};
