...
fn serializeRequest(alloc: std.mem.Allocator, input: SetInstanceProtectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetInstanceProtection&Version=2011-01-01");
    ...
    try body_buf.appendSlice(alloc, "&ProtectedFromScaleIn=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, if (input.protected_from_scale_in) "true" else "false");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetInstanceProtectionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
