...
pub const DetachLoadBalancersInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachLoadBalancersInput, options: Options) !DetachLoadBalancersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DetachLoadBalancersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DetachLoadBalancers&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DetachLoadBalancersOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DetachLoadBalancersOutput = .{};
