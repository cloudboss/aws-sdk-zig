...
pub const DescribeLoadBalancersOutput = struct {
    /// The load balancers.
    load_balancers: ?[]const LoadBalancerState = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeLoadBalancersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeLoadBalancers&Version=2011-01-01");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LoadBalancers")) {
                    result.load_balancers = try serde.deserializeLoadBalancerStates(allocator, &reader, "member");
