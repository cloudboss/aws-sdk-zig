...
const SetDesiredCapacityOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetDesiredCapacityInput, options: Options) !SetDesiredCapacityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetDesiredCapacityInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetDesiredCapacity&Version=2011-01-01");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.desired_capacity}) catch "");
    if (input.honor_cooldown) |v| {
        try body_buf.appendSlice(alloc, "&HonorCooldown=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetDesiredCapacityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
