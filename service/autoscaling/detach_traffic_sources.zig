...
pub const DetachTrafficSourcesInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachTrafficSourcesInput, options: Options) !DetachTrafficSourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachTrafficSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DetachTrafficSources&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachTrafficSourcesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: DetachTrafficSourcesOutput = .{};
