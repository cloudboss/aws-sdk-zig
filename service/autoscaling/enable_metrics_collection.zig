...
const EnableMetricsCollectionOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableMetricsCollectionInput, options: Options) !EnableMetricsCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableMetricsCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableMetricsCollection&Version=2011-01-01");
    ...
    try body_buf.appendSlice(alloc, "&Granularity=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.granularity);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableMetricsCollectionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
