...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAutoScalingNotificationTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAutoScalingNotificationTypes&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAutoScalingNotificationTypesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AutoScalingNotificationTypes")) {
                    result.auto_scaling_notification_types = try serde.deserializeAutoScalingNotificationTypes(&reader, alloc, "member");
                } else {
