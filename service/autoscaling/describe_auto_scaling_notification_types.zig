...
pub const DescribeAutoScalingNotificationTypesInput = struct {};

pub const DescribeAutoScalingNotificationTypesOutput = struct {
    /// The notification types.
    auto_scaling_notification_types: ?[]const []const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAutoScalingNotificationTypesInput, options: CallOptions) !DescribeAutoScalingNotificationTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAutoScalingNotificationTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAutoScalingNotificationTypes&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAutoScalingNotificationTypesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeAutoScalingNotificationTypesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeAutoScalingNotificationTypesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AutoScalingNotificationTypes")) {
                    result.auto_scaling_notification_types = try serde.deserializeAutoScalingNotificationTypes(allocator, &reader, "member");
                } else {
