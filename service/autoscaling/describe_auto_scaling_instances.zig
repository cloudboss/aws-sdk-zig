...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAutoScalingInstancesInput, options: Options) !DescribeAutoScalingInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAutoScalingInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAutoScalingInstances&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAutoScalingInstancesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AutoScalingInstances")) {
                    result.auto_scaling_instances = try serde.deserializeAutoScalingInstances(&reader, alloc, "member");
