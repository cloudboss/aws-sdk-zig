...
pub const RecordLifecycleActionHeartbeatInput = struct {
    ...
    /// an
    /// instance. Amazon EC2 Auto Scaling sends this token to the notification
    /// target that you specified when
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RecordLifecycleActionHeartbeatInput, options: Options) !RecordLifecycleActionHeartbeatOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RecordLifecycleActionHeartbeatInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RecordLifecycleActionHeartbeat&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RecordLifecycleActionHeartbeatOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: RecordLifecycleActionHeartbeatOutput = .{};
