...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLifecycleHooksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeLifecycleHooks&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLifecycleHooksOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LifecycleHooks")) {
                    result.lifecycle_hooks = try serde.deserializeLifecycleHooks(&reader, alloc, "member");
                } else {
