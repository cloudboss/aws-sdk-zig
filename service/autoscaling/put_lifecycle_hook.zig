...
fn serializeRequest(alloc: std.mem.Allocator, input: PutLifecycleHookInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PutLifecycleHook&Version=2011-01-01");
    ...
    if (input.default_result) |v| {
        try body_buf.appendSlice(alloc, "&DefaultResult=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.heartbeat_timeout) |v| {
        try body_buf.appendSlice(alloc, "&HeartbeatTimeout=");
    ...
    if (input.lifecycle_transition) |v| {
        try body_buf.appendSlice(alloc, "&LifecycleTransition=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.notification_metadata) |v| {
        try body_buf.appendSlice(alloc, "&NotificationMetadata=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.notification_target_arn) |v| {
        try body_buf.appendSlice(alloc, "&NotificationTargetARN=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.role_arn) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutLifecycleHookOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
