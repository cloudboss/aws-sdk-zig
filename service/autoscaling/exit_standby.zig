...
pub const ExitStandbyInput = struct {
...
pub const ExitStandbyOutput = struct {
    /// The activities related to moving instances out of `Standby` mode.
    activities: ?[]const Activity = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExitStandbyInput, options: CallOptions) !ExitStandbyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExitStandbyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ExitStandby&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExitStandbyOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ExitStandbyResult")) break;
            },
            else => {},
        }
    }

    var result: ExitStandbyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
