...
pub const DetachLoadBalancerTargetGroupsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachLoadBalancerTargetGroupsInput, options: Options) !DetachLoadBalancerTargetGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DetachLoadBalancerTargetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DetachLoadBalancerTargetGroups&Version=2011-01-01");
    ...
    for (input.target_group_ar_ns, 0..) |item, idx| {
        const n = idx + 1;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DetachLoadBalancerTargetGroupsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DetachLoadBalancerTargetGroupsOutput = .{};
