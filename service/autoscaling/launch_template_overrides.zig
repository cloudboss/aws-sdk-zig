...
/// Auto Scaling group has a mixed
/// instances policy:
///
...
/// and let Amazon EC2 Auto Scaling provision the available instance types that
/// meet your requirements.
/// This can provide Amazon EC2 Auto Scaling with a larger selection of instance
/// types to choose from when
...
/// these
/// settings to get new EC2 instance types automatically. Amazon EC2 Auto
/// Scaling uses the instance
/// requirements of the Auto Scaling group to determine whether a new EC2
...
pub const LaunchTemplateOverrides = struct {
    ...
    /// this override. When
    /// using Instance Refresh with `ReplaceRootVolume` strategy, this specifies the
    /// AMI for root volume
    ...
    /// Scaling group. This
    /// is useful for provisioning instances from different Amazon Machine Images
    /// (AMIs) in the
    /// same Auto Scaling group. To do this, create the AMIs and create a new launch
    /// template for each
    /// AMI. Then, create a compatible set of instance requirements for each launch
    /// template.
    ///
    ...
    /// template that's specified in
    /// the `LaunchTemplate` definition. For more information, see [Specifying a
    /// different launch template for an instance
    /// type](https://docs.aws.amazon.com/autoscaling/ec2/userguide/ec2-auto-scaling-mixed-instances-groups-launch-template-overrides.html) in the
    ...
    /// of capacity
    /// units provided by each instance type in terms of virtual CPUs, memory,
    /// storage,
    /// throughput, or other relative performance characteristic. When a Spot or
    /// On-Demand
    /// Instance is launched, the capacity units count toward the desired capacity.
    /// Amazon EC2 Auto Scaling
    /// launches instances until the desired capacity is totally fulfilled, even if
    /// this results
    /// in an overage. For example, if there are two units remaining to fulfill
    /// capacity, and
    /// Amazon EC2 Auto Scaling can only launch an instance with a
    /// `WeightedCapacity` of five units,
    /// the instance is launched, and the desired capacity is exceeded by three
    /// units. For more
    /// information, see [Configure an Auto Scaling group to use instance
    /// weights](https://docs.aws.amazon.com/autoscaling/ec2/userguide/ec2-auto-scaling-mixed-instances-groups-instance-weighting.html) in the
