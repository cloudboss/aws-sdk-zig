...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeScalingActivitiesInput, options: Options) !DescribeScalingActivitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScalingActivitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeScalingActivities&Version=2011-01-01");
    ...
    if (input.include_deleted_groups) |v| {
        try body_buf.appendSlice(alloc, "&IncludeDeletedGroups=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeScalingActivitiesOutput {
    _ = status;
    _ = headers;
