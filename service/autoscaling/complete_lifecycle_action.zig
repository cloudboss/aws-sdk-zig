...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CompleteLifecycleActionInput, options: Options) !CompleteLifecycleActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CompleteLifecycleActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CompleteLifecycleAction&Version=2011-01-01");
    ...
    try body_buf.appendSlice(alloc, "&LifecycleActionResult=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.lifecycle_action_result);
    if (input.lifecycle_action_token) |v| {
        try body_buf.appendSlice(alloc, "&LifecycleActionToken=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&LifecycleHookName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.lifecycle_hook_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CompleteLifecycleActionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
