...
pub const InstancesDistribution = struct {
    ...
    /// priority instance
    /// type, and so on. This is the default value for Auto Scaling groups that
    /// don't
    /// specify
    ...
    /// capacity. If you
    /// change the default unit of measurement (number of instances) by specifying
    /// weighted
    /// capacity values in your launch template overrides list, or by changing the
    /// default
    /// desired capacity type setting of the group, you must specify this number
    /// using the same
    /// unit of measurement.
    ///
    /// Default: 0
    ...
    /// 20 specifies 20% On-Demand Instances, 80% Spot Instances). If set to 100,
    /// only On-Demand
    /// Instances are used.
    ///
    /// Default: 100
    ...
    /// On-Demand capacity. This is not a valid value for Auto Scaling groups that
    /// specify
    /// [InstanceRequirements](https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_InstanceRequirements.html).
    ///
    /// **lowest-price**
    ...
    /// Instance. If
    /// your maximum price is lower than the Spot price for the instance types that
    /// you
    /// selected, your Spot Instances are not launched. We do not recommend
    /// specifying a maximum
    /// price because it can lead to increased interruptions. When Spot Instances
    /// launch, you
    /// pay the current Spot price. To remove a maximum price that you previously
    /// set, include
