...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TerminateInstanceInAutoScalingGroupInput, options: Options) !TerminateInstanceInAutoScalingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TerminateInstanceInAutoScalingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=TerminateInstanceInAutoScalingGroup&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TerminateInstanceInAutoScalingGroupOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Activity")) {
                    result.activity = try serde.deserializeActivity(&reader, alloc);
                } else {
