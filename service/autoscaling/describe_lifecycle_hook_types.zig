...
pub const DescribeLifecycleHookTypesInput = struct {};

pub const DescribeLifecycleHookTypesOutput = struct {
    /// The lifecycle hook types.
    lifecycle_hook_types: ?[]const []const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLifecycleHookTypesInput, options: CallOptions) !DescribeLifecycleHookTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeLifecycleHookTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeLifecycleHookTypes&Version=2011-01-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeLifecycleHookTypesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeLifecycleHookTypesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeLifecycleHookTypesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LifecycleHookTypes")) {
                    result.lifecycle_hook_types = try serde.deserializeAutoScalingNotificationTypes(allocator, &reader, "member");
                } else {
