...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLifecycleHookTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeLifecycleHookTypes&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLifecycleHookTypesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LifecycleHookTypes")) {
                    result.lifecycle_hook_types = try serde.deserializeAutoScalingNotificationTypes(&reader, alloc, "member");
                } else {
