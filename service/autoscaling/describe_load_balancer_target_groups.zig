...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLoadBalancerTargetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeLoadBalancerTargetGroups&Version=2011-01-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLoadBalancerTargetGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "LoadBalancerTargetGroups")) {
                    result.load_balancer_target_groups = try serde.deserializeLoadBalancerTargetGroupStates(&reader, alloc, "member");
