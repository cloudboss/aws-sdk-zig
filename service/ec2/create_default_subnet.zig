...
pub const CreateDefaultSubnetInput = struct {
    ...
    /// default subnet
    /// for this Availability Zone, you must delete it before you can create an IPv6
    /// only subnet.
    ipv_6_native: ?bool = null,
};

pub const CreateDefaultSubnetOutput = struct {
    /// Information about the subnet.
    subnet: ?Subnet = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDefaultSubnetInput, options: CallOptions) !CreateDefaultSubnetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDefaultSubnetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateDefaultSubnet&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateDefaultSubnetOutput {
    _ = status;
    _ = headers;
