...
pub const CreateInstanceEventWindowOutput = struct {
    ...
    pub fn deinit(self: *const CreateInstanceEventWindowOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateInstanceEventWindowOutput {
    _ = status;
    _ = body;
    const result: CreateInstanceEventWindowOutput = .{ .allocator = alloc };
