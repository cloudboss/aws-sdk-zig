...
pub const ModifyInstanceCapacityReservationAttributesOutput = struct {
    ...
    pub fn deinit(self: *const ModifyInstanceCapacityReservationAttributesOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyInstanceCapacityReservationAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyInstanceCapacityReservationAttributes&Version=2016-11-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyInstanceCapacityReservationAttributesOutput {
    _ = status;
    _ = headers;
    var result: ModifyInstanceCapacityReservationAttributesOutput = .{ .allocator = alloc };
