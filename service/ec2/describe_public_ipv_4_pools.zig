...
pub const DescribePublicIpv4PoolsOutput = struct {
    ...
    pub fn deinit(self: *DescribePublicIpv4PoolsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribePublicIpv4PoolsInput, options: Options) !DescribePublicIpv4PoolsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePublicIpv4PoolsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribePublicIpv4Pools&Version=2016-11-15");
    ...
    if (input.pool_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&PoolIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.public_ipv_4_pools = try serde.deserializePublicIpv4PoolSet(&reader, alloc, "item");
                } else {
