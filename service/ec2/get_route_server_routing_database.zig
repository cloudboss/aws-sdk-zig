...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouteServerRoutingDatabaseInput, options: CallOptions) !GetRouteServerRoutingDatabaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRouteServerRoutingDatabaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetRouteServerRoutingDatabase&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRouteServerRoutingDatabaseOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "areRoutesPersisted")) {
                    ...
                    result.routes = try serde.deserializeRouteServerRouteList(allocator, &reader, "item");
                } else {
