...
pub const DisableCapacityManagerOutput = struct {
    /// The current status of Capacity Manager after the disable operation.
    capacity_manager_status: ?CapacityManagerStatus = null,

    /// Indicates whether Organizations access is enabled. This will be `false`
    /// after disabling Capacity Manager.
    organizations_access: ?bool = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableCapacityManagerInput, options: CallOptions) !DisableCapacityManagerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableCapacityManagerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DisableCapacityManager&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableCapacityManagerOutput {
    _ = status;
    _ = headers;
    _ = allocator;
    ...
    var result: DisableCapacityManagerOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
