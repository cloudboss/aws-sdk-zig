...
pub const DisassociateInstanceEventWindowOutput = struct {
    /// Information about the event window.
    instance_event_window: ?InstanceEventWindow = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateInstanceEventWindowInput, options: CallOptions) !DisassociateInstanceEventWindowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateInstanceEventWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DisassociateInstanceEventWindow&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateInstanceEventWindowOutput {
    _ = status;
    _ = headers;
    ...
    var result: DisassociateInstanceEventWindowOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instanceEventWindow")) {
                    result.instance_event_window = try serde.deserializeInstanceEventWindow(allocator, &reader);
                } else {
