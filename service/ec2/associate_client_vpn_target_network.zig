...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateClientVpnTargetNetworkInput, options: CallOptions) !AssociateClientVpnTargetNetworkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateClientVpnTargetNetworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateClientVpnTargetNetworkOutput {
    _ = status;
    _ = headers;
    ...
    var result: AssociateClientVpnTargetNetworkOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
