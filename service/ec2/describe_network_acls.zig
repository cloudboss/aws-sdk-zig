...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNetworkAclsInput, options: CallOptions) !DescribeNetworkAclsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeNetworkAclsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeNetworkAcls&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeNetworkAclsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "networkAclSet")) {
                    result.network_acls = try serde.deserializeNetworkAclList(allocator, &reader, "item");
