...
pub const DescribeNetworkAclsInput = struct {
    ...
    network_acl_ids: ?[]const []const u8 = null,
...
pub const DescribeNetworkAclsOutput = struct {
    /// Information about the network ACLs.
    network_acls: ?[]const NetworkAcl = null,
    ...
    pub fn deinit(self: *DescribeNetworkAclsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeNetworkAclsInput, options: Options) !DescribeNetworkAclsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNetworkAclsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeNetworkAcls&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.network_acl_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&NetworkAclIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNetworkAclsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeNetworkAclsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "networkAclSet")) {
                    result.network_acls = try serde.deserializeNetworkAclList(&reader, alloc, "item");
