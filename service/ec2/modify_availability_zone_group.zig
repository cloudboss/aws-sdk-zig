...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyAvailabilityZoneGroupInput, options: CallOptions) !ModifyAvailabilityZoneGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyAvailabilityZoneGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyAvailabilityZoneGroup&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyAvailabilityZoneGroupOutput {
    _ = status;
    _ = headers;
    _ = allocator;
