...
pub const ModifySecurityGroupRulesInput = struct {
    ...
    security_group_rules: []const SecurityGroupRuleUpdate,
};

pub const ModifySecurityGroupRulesOutput = struct {
    /// Returns `true` if the request succeeds; otherwise, returns an error.
    @"return": ?bool = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifySecurityGroupRulesInput, options: CallOptions) !ModifySecurityGroupRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifySecurityGroupRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifySecurityGroupRules&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifySecurityGroupRulesOutput {
    _ = status;
    _ = headers;
    _ = allocator;
    ...
    var result: ModifySecurityGroupRulesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
