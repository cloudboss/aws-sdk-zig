...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyInstanceCpuOptionsInput, options: Options) !ModifyInstanceCpuOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyInstanceCpuOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.core_count}) catch "");
    if (input.dry_run) |v| {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.threads_per_core}) catch "");
