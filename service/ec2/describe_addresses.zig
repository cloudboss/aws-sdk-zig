...
pub const DescribeAddressesOutput = struct {
    ...
    pub fn deinit(self: *DescribeAddressesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeAddressesInput, options: Options) !DescribeAddressesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAddressesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAddresses&Version=2016-11-15");
    ...
    if (input.public_ips) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&PublicIps.PublicIp.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try appendUrlEncoded(alloc, &body_buf, item);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAddressesOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeAddressesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "addressesSet")) {
                    result.addresses = try serde.deserializeAddressList(&reader, alloc, "item");
                } else {
