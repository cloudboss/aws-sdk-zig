...
fn serializeRequest(allocator: std.mem.Allocator, input: CopySnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CopySnapshot&Version=2016-11-15");
    if (input.completion_duration_minutes) |v| {
        try body_buf.appendSlice(allocator, "&CompletionDurationMinutes=");
    ...
    try body_buf.appendSlice(allocator, "&SourceSnapshotId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.source_snapshot_id);
