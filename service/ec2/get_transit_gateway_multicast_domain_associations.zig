...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTransitGatewayMulticastDomainAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetTransitGatewayMulticastDomainAssociations&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTransitGatewayMulticastDomainAssociationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "multicastDomainAssociations")) {
                    result.multicast_domain_associations = try serde.deserializeTransitGatewayMulticastDomainAssociationList(allocator, &reader, "item");
