...
pub const GetTransitGatewayPrefixListReferencesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTransitGatewayPrefixListReferencesInput, options: CallOptions) !GetTransitGatewayPrefixListReferencesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTransitGatewayPrefixListReferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetTransitGatewayPrefixListReferences&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTransitGatewayPrefixListReferencesOutput {
    _ = status;
    _ = headers;
    ...
    var result: GetTransitGatewayPrefixListReferencesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "transitGatewayPrefixListReferenceSet")) {
                    result.transit_gateway_prefix_list_references = try serde.deserializeTransitGatewayPrefixListReferenceSet(allocator, &reader, "item");
                } else {
