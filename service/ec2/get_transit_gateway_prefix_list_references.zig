...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTransitGatewayPrefixListReferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetTransitGatewayPrefixListReferences&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTransitGatewayPrefixListReferencesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.transit_gateway_prefix_list_references = try serde.deserializeTransitGatewayPrefixListReferenceSet(&reader, alloc, "item");
                } else {
