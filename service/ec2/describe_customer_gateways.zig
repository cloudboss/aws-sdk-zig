...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCustomerGatewaysInput, options: CallOptions) !DescribeCustomerGatewaysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeCustomerGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeCustomerGateways&Version=2016-11-15");
    if (input.customer_gateway_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&CustomerGatewayId.CustomerGatewayId.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeCustomerGatewaysOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "customerGatewaySet")) {
                    result.customer_gateways = try serde.deserializeCustomerGatewayList(allocator, &reader, "item");
                } else {
