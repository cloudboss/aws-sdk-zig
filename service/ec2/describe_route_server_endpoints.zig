...
pub const DescribeRouteServerEndpointsInput = struct {
    ...
    route_server_endpoint_ids: ?[]const []const u8 = null,
};

pub const DescribeRouteServerEndpointsOutput = struct {
    /// The token to use to retrieve the next page of results. This value is `null`
    /// when there are no more results to return.
    next_token: ?[]const u8 = null,

    /// Information about the described route server endpoints.
    route_server_endpoints: ?[]const RouteServerEndpoint = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *DescribeRouteServerEndpointsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeRouteServerEndpointsInput, options: Options) !DescribeRouteServerEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRouteServerEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeRouteServerEndpoints&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.route_server_endpoint_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&RouteServerEndpointIds.member.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRouteServerEndpointsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeRouteServerEndpointsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "routeServerEndpointSet")) {
                    result.route_server_endpoints = try serde.deserializeRouteServerEndpointsList(&reader, alloc, "item");
                } else {
