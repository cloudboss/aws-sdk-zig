...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTransitGatewaysInput, options: CallOptions) !DescribeTransitGatewaysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTransitGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTransitGateways&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTransitGatewaysOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.transit_gateways = try serde.deserializeTransitGatewayList(allocator, &reader, "item");
                } else {
