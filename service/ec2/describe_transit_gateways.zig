...
pub const DescribeTransitGatewaysInput = struct {
    ...
    ///   attachments automatically propagate
    ...
    /// example, to
    /// find all resources that have a tag with the key `Owner` and the value
    /// `TeamA`, specify
    ...
    transit_gateway_ids: ?[]const []const u8 = null,
};

pub const DescribeTransitGatewaysOutput = struct {
    /// The token to use to retrieve the next page of results. This value is `null`
    /// when there are no more results to return.
    next_token: ?[]const u8 = null,

    /// Information about the transit gateways.
    transit_gateways: ?[]const TransitGateway = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *DescribeTransitGatewaysOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeTransitGatewaysInput, options: Options) !DescribeTransitGatewaysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTransitGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTransitGateways&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.transit_gateway_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&TransitGatewayIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTransitGatewaysOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeTransitGatewaysOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "transitGatewaySet")) {
                    result.transit_gateways = try serde.deserializeTransitGatewayList(&reader, alloc, "item");
                } else {
