...
pub const DeleteRouteServerOutput = struct {
    ...
    pub fn deinit(self: *const DeleteRouteServerOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeleteRouteServerOutput {
    _ = status;
    _ = body;
    const result: DeleteRouteServerOutput = .{ .allocator = alloc };
