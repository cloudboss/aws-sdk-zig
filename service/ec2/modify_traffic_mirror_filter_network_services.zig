...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyTrafficMirrorFilterNetworkServicesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyTrafficMirrorFilterNetworkServices&Version=2016-11-15");
    if (input.add_network_services) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&AddNetworkServices.item.{d}=", .{n}) catch continue;
    ...
    if (input.remove_network_services) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&RemoveNetworkServices.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try appendUrlEncoded(alloc, &body_buf, item);
        }
    }
