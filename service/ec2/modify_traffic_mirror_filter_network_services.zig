...
pub const ModifyTrafficMirrorFilterNetworkServicesInput = struct {
    ...
    /// mirror.
    remove_network_services: ?[]const TrafficMirrorNetworkService = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyTrafficMirrorFilterNetworkServicesInput, options: CallOptions) !ModifyTrafficMirrorFilterNetworkServicesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyTrafficMirrorFilterNetworkServicesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyTrafficMirrorFilterNetworkServices&Version=2016-11-15");
    if (input.add_network_services) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
    ...
    if (input.remove_network_services) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyTrafficMirrorFilterNetworkServicesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "trafficMirrorFilter")) {
                    result.traffic_mirror_filter = try serde.deserializeTrafficMirrorFilter(allocator, &reader);
                } else {
