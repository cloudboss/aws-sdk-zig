...
pub const DescribeFleetInstancesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetInstancesInput, options: CallOptions) !DescribeFleetInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFleetInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeFleetInstances&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFleetInstancesOutput {
    _ = status;
    _ = headers;
