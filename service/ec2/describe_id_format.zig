...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIdFormatInput, options: CallOptions) !DescribeIdFormatOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeIdFormatInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeIdFormat&Version=2016-11-15");
    if (input.resource) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeIdFormatOutput {
    _ = status;
    _ = headers;
