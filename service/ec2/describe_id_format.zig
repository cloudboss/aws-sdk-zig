...
pub const DescribeIdFormatOutput = struct {
    ...
    pub fn deinit(self: *const DescribeIdFormatOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeIdFormatOutput {
    _ = status;
    _ = body;
    const result: DescribeIdFormatOutput = .{ .allocator = alloc };
