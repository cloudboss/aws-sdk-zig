...
pub const DescribeRouteServerPeersInput = struct {
    ...
    route_server_peer_ids: ?[]const []const u8 = null,
};

pub const DescribeRouteServerPeersOutput = struct {
    /// The token to use to retrieve the next page of results. This value is `null`
    /// when there are no more results to return.
    next_token: ?[]const u8 = null,

    /// Information about the described route server peers.
    route_server_peers: ?[]const RouteServerPeer = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *DescribeRouteServerPeersOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeRouteServerPeersInput, options: Options) !DescribeRouteServerPeersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRouteServerPeersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeRouteServerPeers&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.route_server_peer_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&RouteServerPeerIds.member.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRouteServerPeersOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeRouteServerPeersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "routeServerPeerSet")) {
                    result.route_server_peers = try serde.deserializeRouteServerPeersList(&reader, alloc, "item");
                } else {
