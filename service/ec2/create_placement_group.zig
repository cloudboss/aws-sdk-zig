...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePlacementGroupInput, options: CallOptions) !CreatePlacementGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreatePlacementGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreatePlacementGroup&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.spread_level) |v| {
        try body_buf.appendSlice(allocator, "&SpreadLevel=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreatePlacementGroupOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "placementGroup")) {
                    result.placement_group = try serde.deserializePlacementGroup(allocator, &reader);
                } else {
