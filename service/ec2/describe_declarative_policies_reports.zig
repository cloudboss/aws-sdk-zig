...
pub fn execute(client: *Client, input: DescribeDeclarativePoliciesReportsInput, options: Options) !DescribeDeclarativePoliciesReportsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDeclarativePoliciesReportsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.report_ids) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ReportIds.item.{d}=", .{n}) catch continue;
