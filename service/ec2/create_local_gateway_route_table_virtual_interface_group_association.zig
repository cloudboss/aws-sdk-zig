...
pub const CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociation&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(allocator, "&LocalGatewayRouteTableId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.local_gateway_route_table_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationOutput {
    _ = status;
    _ = headers;
