...
pub fn execute(client: *Client, input: CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationInput, options: Options) !CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociation&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&LocalGatewayRouteTableId=");
    try appendUrlEncoded(alloc, &body_buf, input.local_gateway_route_table_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationOutput {
    _ = status;
    _ = body;
