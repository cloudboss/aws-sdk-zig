...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVpnConnectionDeviceSampleConfigurationInput, options: CallOptions) !GetVpnConnectionDeviceSampleConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetVpnConnectionDeviceSampleConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetVpnConnectionDeviceSampleConfiguration&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    if (input.internet_key_exchange_version) |v| {
        try body_buf.appendSlice(allocator, "&InternetKeyExchangeVersion=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.sample_type) |v| {
        try body_buf.appendSlice(allocator, "&SampleType=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    try body_buf.appendSlice(allocator, "&VpnConnectionDeviceTypeId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.vpn_connection_device_type_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetVpnConnectionDeviceSampleConfigurationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "vpnConnectionDeviceSampleConfiguration")) {
                    result.vpn_connection_device_sample_configuration = try allocator.dupe(u8, try reader.readElementText());
                } else {
