...
pub fn execute(client: *Client, input: DescribeStoreImageTasksInput, options: Options) !DescribeStoreImageTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStoreImageTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.image_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ImageIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
