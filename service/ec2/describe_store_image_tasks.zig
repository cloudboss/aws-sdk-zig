...
pub const DescribeStoreImageTasksInput = struct {
    ...
    /// The AMI IDs for which to show progress. Up to 20 AMI IDs can be included in
    /// a
    /// request.
...
pub const DescribeStoreImageTasksOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStoreImageTasksInput, options: CallOptions) !DescribeStoreImageTasksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeStoreImageTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeStoreImageTasks&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.image_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ImageId.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeStoreImageTasksOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.store_image_task_results = try serde.deserializeStoreImageTaskResultSet(allocator, &reader, "item");
                } else {
