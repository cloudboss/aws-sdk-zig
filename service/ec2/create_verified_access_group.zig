...
pub const CreateVerifiedAccessGroupOutput = struct {
    /// Details about the Verified Access group.
    verified_access_group: ?VerifiedAccessGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVerifiedAccessGroupInput, options: CallOptions) !CreateVerifiedAccessGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVerifiedAccessGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateVerifiedAccessGroup&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVerifiedAccessGroupOutput {
    _ = status;
    _ = headers;
