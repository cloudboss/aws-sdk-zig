...
pub fn execute(client: *Client, input: CreateLocalGatewayVirtualInterfaceGroupInput, options: Options) !CreateLocalGatewayVirtualInterfaceGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocalGatewayVirtualInterfaceGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateLocalGatewayVirtualInterfaceGroup&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.local_bgp_asn) |v| {
        try body_buf.appendSlice(alloc, "&LocalBgpAsn=");
        try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.local_bgp_asn_extended) |v| {
        try body_buf.appendSlice(alloc, "&LocalBgpAsnExtended=");
        try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    try body_buf.appendSlice(alloc, "&LocalGatewayId=");
    try appendUrlEncoded(alloc, &body_buf, input.local_gateway_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateLocalGatewayVirtualInterfaceGroupOutput {
    _ = status;
    _ = body;
