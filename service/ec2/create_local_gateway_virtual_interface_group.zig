...
pub const CreateLocalGatewayVirtualInterfaceGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocalGatewayVirtualInterfaceGroupInput, options: CallOptions) !CreateLocalGatewayVirtualInterfaceGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLocalGatewayVirtualInterfaceGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateLocalGatewayVirtualInterfaceGroup&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    if (input.local_bgp_asn) |v| {
        try body_buf.appendSlice(allocator, "&LocalBgpAsn=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, std.fmt.allocPrint(allocator, "{d}", .{v}) catch "");
    }
    if (input.local_bgp_asn_extended) |v| {
        try body_buf.appendSlice(allocator, "&LocalBgpAsnExtended=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, std.fmt.allocPrint(allocator, "{d}", .{v}) catch "");
    }
    try body_buf.appendSlice(allocator, "&LocalGatewayId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.local_gateway_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLocalGatewayVirtualInterfaceGroupOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "localGatewayVirtualInterfaceGroup")) {
                    result.local_gateway_virtual_interface_group = try serde.deserializeLocalGatewayVirtualInterfaceGroup(allocator, &reader);
                } else {
