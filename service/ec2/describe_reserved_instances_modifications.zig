...
pub const DescribeReservedInstancesModificationsOutput = struct {
    ...
    pub fn deinit(self: *const DescribeReservedInstancesModificationsOutput) void {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedInstancesModificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.reserved_instances_modification_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ReservedInstancesModificationIds.ReservedInstancesModificationId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeReservedInstancesModificationsOutput {
    _ = status;
    var result: DescribeReservedInstancesModificationsOutput = .{ .allocator = alloc };
