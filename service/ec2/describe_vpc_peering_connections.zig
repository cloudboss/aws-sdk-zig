...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpcPeeringConnectionsInput, options: CallOptions) !DescribeVpcPeeringConnectionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpcPeeringConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVpcPeeringConnections&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVpcPeeringConnectionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.vpc_peering_connections = try serde.deserializeVpcPeeringConnectionList(allocator, &reader, "item");
                } else {
