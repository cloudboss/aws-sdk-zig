...
pub const DescribeVpcPeeringConnectionsInput = struct {
    ...
    vpc_peering_connection_ids: ?[]const []const u8 = null,
};

pub const DescribeVpcPeeringConnectionsOutput = struct {
    /// The token to include in another request to get the next page of items. This
    /// value is `null` when there are no more items to return.
    next_token: ?[]const u8 = null,

    /// Information about the VPC peering connections.
    vpc_peering_connections: ?[]const VpcPeeringConnection = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *DescribeVpcPeeringConnectionsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeVpcPeeringConnectionsInput, options: Options) !DescribeVpcPeeringConnectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpcPeeringConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeVpcPeeringConnections&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.vpc_peering_connection_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&VpcPeeringConnectionIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVpcPeeringConnectionsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeVpcPeeringConnectionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "vpcPeeringConnectionSet")) {
                    result.vpc_peering_connections = try serde.deserializeVpcPeeringConnectionList(&reader, alloc, "item");
                } else {
