...
pub const DisableSerialConsoleAccessOutput = struct {
    ...
    pub fn deinit(self: *const DisableSerialConsoleAccessOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DisableSerialConsoleAccessInput, options: Options) !DisableSerialConsoleAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableSerialConsoleAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisableSerialConsoleAccess&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableSerialConsoleAccessOutput {
    _ = status;
    _ = headers;
    var result: DisableSerialConsoleAccessOutput = .{ .allocator = alloc };
