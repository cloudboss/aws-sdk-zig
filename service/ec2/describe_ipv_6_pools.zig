...
pub const DescribeIpv6PoolsOutput = struct {
    /// Information about the IPv6 address pools.
    ipv_6_pools: ?[]const Ipv6Pool = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIpv6PoolsInput, options: CallOptions) !DescribeIpv6PoolsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeIpv6PoolsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeIpv6Pools&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeIpv6PoolsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipv6PoolSet")) {
                    result.ipv_6_pools = try serde.deserializeIpv6PoolSet(allocator, &reader, "item");
