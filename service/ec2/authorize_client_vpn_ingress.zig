...
pub const AuthorizeClientVpnIngressOutput = struct {
    ...
    pub fn deinit(self: *const AuthorizeClientVpnIngressOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: AuthorizeClientVpnIngressInput, options: Options) !AuthorizeClientVpnIngressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AuthorizeClientVpnIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AuthorizeClientVpnIngress&Version=2016-11-15");
    if (input.access_group_id) |v| {
        try body_buf.appendSlice(alloc, "&AccessGroupId=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.authorize_all_groups) |v| {
        try body_buf.appendSlice(alloc, "&AuthorizeAllGroups=");
    ...
    try appendUrlEncoded(alloc, &body_buf, input.target_network_cidr);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AuthorizeClientVpnIngressOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: AuthorizeClientVpnIngressOutput = .{ .allocator = alloc };
