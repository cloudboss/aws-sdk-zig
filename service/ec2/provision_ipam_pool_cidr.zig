...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ProvisionIpamPoolCidrInput, options: CallOptions) !ProvisionIpamPoolCidrOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ProvisionIpamPoolCidrInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ProvisionIpamPoolCidr&Version=2016-11-15");
    if (input.cidr) |v| {
    ...
    if (input.cidr_authorization_context) |v| {
        if (v.message) |sv| {
            try body_buf.appendSlice(allocator, "&CidrAuthorizationContext.Message=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, sv);
        }
        if (v.signature) |sv| {
            try body_buf.appendSlice(allocator, "&CidrAuthorizationContext.Signature=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, sv);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ProvisionIpamPoolCidrOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamPoolCidr")) {
                    result.ipam_pool_cidr = try serde.deserializeIpamPoolCidr(allocator, &reader);
                } else {
