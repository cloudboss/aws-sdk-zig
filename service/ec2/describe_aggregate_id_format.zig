...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAggregateIdFormatInput, options: CallOptions) !DescribeAggregateIdFormatOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAggregateIdFormatInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAggregateIdFormat&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAggregateIdFormatOutput {
    _ = status;
    _ = headers;
