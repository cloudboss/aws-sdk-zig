...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTagsInput, options: CallOptions) !DeleteTagsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteTags&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTagsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteTagsOutput = .{};
