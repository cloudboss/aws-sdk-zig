...
pub fn execute(client: *Client, input: DescribeCapacityBlockExtensionOfferingsInput, options: Options) !DescribeCapacityBlockExtensionOfferingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCapacityBlockExtensionOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCapacityBlockExtensionOfferings&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&CapacityBlockExtensionDurationHours=");
    try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.capacity_block_extension_duration_hours}) catch "");
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeCapacityBlockExtensionOfferingsOutput {
    _ = status;
