...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCapacityManagerDataExportInput, options: CallOptions) !CreateCapacityManagerDataExportOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateCapacityManagerDataExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateCapacityManagerDataExport&Version=2016-11-15");
    ...
    if (input.s3_bucket_prefix) |v| {
        try body_buf.appendSlice(allocator, "&S3BucketPrefix=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateCapacityManagerDataExportOutput {
    _ = status;
    _ = headers;
    ...
    var result: CreateCapacityManagerDataExportOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "capacityManagerDataExportId")) {
                    result.capacity_manager_data_export_id = try allocator.dupe(u8, try reader.readElementText());
                } else {
