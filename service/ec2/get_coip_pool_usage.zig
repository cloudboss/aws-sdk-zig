...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCoipPoolUsageInput, options: CallOptions) !GetCoipPoolUsageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCoipPoolUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetCoipPoolUsage&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCoipPoolUsageOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "coipAddressUsageSet")) {
                    result.coip_address_usages = try serde.deserializeCoipAddressUsageSet(allocator, &reader, "item");
                } else if (std.mem.eql(u8, e.local, "coipPoolId")) {
                    result.coip_pool_id = try allocator.dupe(u8, try reader.readElementText());
