...
pub const DescribeAddressesAttributeOutput = struct {
    ...
    pub fn deinit(self: *const DescribeAddressesAttributeOutput) void {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAddressesAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allocation_ids) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&AllocationIds.item.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeAddressesAttributeOutput {
    _ = status;
    var result: DescribeAddressesAttributeOutput = .{ .allocator = alloc };
