...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAddressesAttributeInput, options: CallOptions) !DescribeAddressesAttributeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAddressesAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAddressesAttribute&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAddressesAttributeOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "addressSet")) {
                    result.addresses = try serde.deserializeAddressSet(allocator, &reader, "item");
