...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeSecurityGroupIngressInput, options: CallOptions) !RevokeSecurityGroupIngressOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RevokeSecurityGroupIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RevokeSecurityGroupIngress&Version=2016-11-15");
    if (input.cidr_ip) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RevokeSecurityGroupIngressOutput {
    _ = status;
    _ = headers;
