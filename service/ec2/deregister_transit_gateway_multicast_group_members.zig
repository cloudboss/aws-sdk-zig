...
pub fn execute(client: *Client, input: DeregisterTransitGatewayMulticastGroupMembersInput, options: Options) !DeregisterTransitGatewayMulticastGroupMembersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterTransitGatewayMulticastGroupMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeregisterTransitGatewayMulticastGroupMembers&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeregisterTransitGatewayMulticastGroupMembersOutput {
    _ = status;
    _ = body;
