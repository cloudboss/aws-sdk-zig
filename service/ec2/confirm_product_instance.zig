...
fn serializeRequest(alloc: std.mem.Allocator, input: ConfirmProductInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ConfirmProductInstance&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    try body_buf.appendSlice(alloc, "&ProductCode=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.product_code);
