...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNetworkInterfaceAttributeInput, options: CallOptions) !DescribeNetworkInterfaceAttributeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeNetworkInterfaceAttributeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeNetworkInterfaceAttributeOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "associatePublicIpAddress")) {
                    ...
                    result.attachment = try serde.deserializeNetworkInterfaceAttachment(allocator, &reader);
