...
pub const DescribeDhcpOptionsOutput = struct {
    /// Information about the DHCP options sets.
    dhcp_options: ?[]const DhcpOptions = null,
    ...
    pub fn deinit(self: *DescribeDhcpOptionsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeDhcpOptionsInput, options: Options) !DescribeDhcpOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDhcpOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDhcpOptions&Version=2016-11-15");
    if (input.dhcp_options_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&DhcpOptionsIds.DhcpOptionsId.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDhcpOptionsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeDhcpOptionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "dhcpOptionsSet")) {
                    result.dhcp_options = try serde.deserializeDhcpOptionsList(&reader, alloc, "item");
