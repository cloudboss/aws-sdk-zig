...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDhcpOptionsInput, options: CallOptions) !DescribeDhcpOptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDhcpOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDhcpOptions&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDhcpOptionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "dhcpOptionsSet")) {
                    result.dhcp_options = try serde.deserializeDhcpOptionsList(allocator, &reader, "item");
