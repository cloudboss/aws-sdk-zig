...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyIpamResourceCidrInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.current_ipam_scope_id);
    ...
    try appendUrlEncoded(alloc, &body_buf, if (input.monitored) "true" else "false");
    try body_buf.appendSlice(alloc, "&ResourceCidr=");
    try appendUrlEncoded(alloc, &body_buf, input.resource_cidr);
    try body_buf.appendSlice(alloc, "&ResourceId=");
    try appendUrlEncoded(alloc, &body_buf, input.resource_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyIpamResourceCidrOutput {
    _ = status;
    _ = body;
