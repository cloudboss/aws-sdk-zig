...
pub const DescribeIpamPrefixListResolverTargetsOutput = struct {
    ...
    pub fn deinit(self: *DescribeIpamPrefixListResolverTargetsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeIpamPrefixListResolverTargetsInput, options: Options) !DescribeIpamPrefixListResolverTargetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIpamPrefixListResolverTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeIpamPrefixListResolverTargets&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.ipam_prefix_list_resolver_target_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&IpamPrefixListResolverTargetIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamPrefixListResolverTargetSet")) {
                    result.ipam_prefix_list_resolver_targets = try serde.deserializeIpamPrefixListResolverTargetSet(&reader, alloc, "item");
