...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreSnapshotTierInput, options: CallOptions) !RestoreSnapshotTierOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RestoreSnapshotTierInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RestoreSnapshotTier&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    if (input.permanent_restore) |v| {
        try body_buf.appendSlice(allocator, "&PermanentRestore=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    ...
    if (input.temporary_restore_days) |v| {
        try body_buf.appendSlice(allocator, "&TemporaryRestoreDays=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RestoreSnapshotTierOutput {
    _ = status;
    _ = headers;
