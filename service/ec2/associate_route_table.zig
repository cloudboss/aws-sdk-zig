...
pub const AssociateRouteTableInput = struct {
...
pub const AssociateRouteTableOutput = struct {
    ...
    pub fn deinit(self: *AssociateRouteTableOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: AssociateRouteTableInput, options: Options) !AssociateRouteTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateRouteTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AssociateRouteTable&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateRouteTableOutput {
    _ = status;
    _ = headers;
