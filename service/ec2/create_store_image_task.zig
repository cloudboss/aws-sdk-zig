...
pub const CreateStoreImageTaskOutput = struct {
    ...
    pub fn deinit(self: *const CreateStoreImageTaskOutput) void {
        if (self.object_key) |v| {
...
pub fn execute(client: *Client, input: CreateStoreImageTaskInput, options: Options) !CreateStoreImageTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStoreImageTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateStoreImageTask&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&Bucket=");
    try appendUrlEncoded(alloc, &body_buf, input.bucket);
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStoreImageTaskOutput {
    _ = status;
    _ = headers;
    var result: CreateStoreImageTaskOutput = .{ .allocator = alloc };
    if (findElement(body, "objectKey")) |content| {
        result.object_key = try alloc.dupe(u8, content);
    }
