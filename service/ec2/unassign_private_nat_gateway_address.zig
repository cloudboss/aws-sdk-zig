...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UnassignPrivateNatGatewayAddressInput, options: CallOptions) !UnassignPrivateNatGatewayAddressOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UnassignPrivateNatGatewayAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UnassignPrivateNatGatewayAddress&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UnassignPrivateNatGatewayAddressOutput {
    _ = status;
    _ = headers;
