...
pub const DisableFastSnapshotRestoresOutput = struct {
    /// Information about the snapshots for which fast snapshot restores were
    /// successfully disabled.
    successful: ?[]const DisableFastSnapshotRestoreSuccessItem = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableFastSnapshotRestoresInput, options: CallOptions) !DisableFastSnapshotRestoresOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableFastSnapshotRestoresInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DisableFastSnapshotRestores&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableFastSnapshotRestoresOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "successful")) {
                    result.successful = try serde.deserializeDisableFastSnapshotRestoreSuccessSet(allocator, &reader, "item");
                } else if (std.mem.eql(u8, e.local, "unsuccessful")) {
                    result.unsuccessful = try serde.deserializeDisableFastSnapshotRestoreErrorSet(allocator, &reader, "item");
                } else {
