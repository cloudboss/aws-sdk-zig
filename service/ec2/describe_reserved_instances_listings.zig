...
pub const DescribeReservedInstancesListingsOutput = struct {
    ...
    pub fn deinit(self: *const DescribeReservedInstancesListingsOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeReservedInstancesListingsOutput {
    _ = status;
    _ = body;
    const result: DescribeReservedInstancesListingsOutput = .{ .allocator = alloc };
