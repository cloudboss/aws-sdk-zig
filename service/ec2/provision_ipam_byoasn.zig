...
pub fn execute(client: *Client, input: ProvisionIpamByoasnInput, options: Options) !ProvisionIpamByoasnOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ProvisionIpamByoasnInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ProvisionIpamByoasn&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&Asn=");
    try appendUrlEncoded(alloc, &body_buf, input.asn);
    try body_buf.appendSlice(alloc, "&AsnAuthorizationContext.Message=");
    try appendUrlEncoded(alloc, &body_buf, input.asn_authorization_context.message);
    try body_buf.appendSlice(alloc, "&AsnAuthorizationContext.Signature=");
    try appendUrlEncoded(alloc, &body_buf, input.asn_authorization_context.signature);
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ProvisionIpamByoasnOutput {
    _ = status;
    _ = body;
