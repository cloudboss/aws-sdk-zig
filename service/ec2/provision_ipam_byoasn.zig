...
fn serializeRequest(alloc: std.mem.Allocator, input: ProvisionIpamByoasnInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.asn_authorization_context.message);
    try body_buf.appendSlice(alloc, "&AsnAuthorizationContext.Signature=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.asn_authorization_context.signature);
    if (input.dry_run) |v| {
