...
pub const ModifyFleetOutput = struct {
    ...
    pub fn deinit(self: *const ModifyFleetOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyFleetOutput {
    _ = status;
    var result: ModifyFleetOutput = .{ .allocator = alloc };
