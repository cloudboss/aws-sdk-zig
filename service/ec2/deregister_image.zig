...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterImageInput, options: CallOptions) !DeregisterImageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeregisterImage&Version=2016-11-15");
    if (input.delete_associated_snapshots) |v| {
        try body_buf.appendSlice(allocator, "&DeleteAssociatedSnapshots=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterImageOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "deleteSnapshotResultSet")) {
                    result.delete_snapshot_results = try serde.deserializeDeleteSnapshotResultSet(allocator, &reader, "item");
