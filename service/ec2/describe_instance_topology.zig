...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceTopologyInput, options: CallOptions) !DescribeInstanceTopologyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstanceTopologyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstanceTopology&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.group_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&GroupName.member.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
    ...
    if (input.instance_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InstanceId.member.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstanceTopologyOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeInstanceTopologyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instanceSet")) {
                    result.instances = try serde.deserializeInstanceSet(allocator, &reader, "item");
