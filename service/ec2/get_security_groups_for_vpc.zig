...
pub const GetSecurityGroupsForVpcOutput = struct {
    /// The token to include in another request to get the next page of items. This
    /// value is `null` when there are no more items to return.
    next_token: ?[]const u8 = null,

    /// The security group that can be used by interfaces in the VPC.
    security_group_for_vpcs: ?[]const SecurityGroupForVpc = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *GetSecurityGroupsForVpcOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetSecurityGroupsForVpcInput, options: Options) !GetSecurityGroupsForVpcOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSecurityGroupsForVpcInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetSecurityGroupsForVpc&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSecurityGroupsForVpcOutput {
    _ = status;
    _ = headers;
    ...
    var result: GetSecurityGroupsForVpcOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "securityGroupForVpcSet")) {
                    result.security_group_for_vpcs = try serde.deserializeSecurityGroupForVpcList(&reader, alloc, "item");
                } else {
