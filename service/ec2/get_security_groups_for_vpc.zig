...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSecurityGroupsForVpcInput, options: CallOptions) !GetSecurityGroupsForVpcOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSecurityGroupsForVpcInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetSecurityGroupsForVpc&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSecurityGroupsForVpcOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.security_group_for_vpcs = try serde.deserializeSecurityGroupForVpcList(allocator, &reader, "item");
                } else {
