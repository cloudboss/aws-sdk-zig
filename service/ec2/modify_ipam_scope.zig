...
pub const ModifyIpamScopeOutput = struct {
    ...
    pub fn deinit(self: *const ModifyIpamScopeOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyIpamScopeOutput {
    _ = status;
    _ = body;
    const result: ModifyIpamScopeOutput = .{ .allocator = alloc };
