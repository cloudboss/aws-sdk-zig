...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInternetGatewaysInput, options: CallOptions) !DescribeInternetGatewaysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInternetGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInternetGateways&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.internet_gateway_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InternetGatewayId.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInternetGatewaysOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeInternetGatewaysOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "internetGatewaySet")) {
                    result.internet_gateways = try serde.deserializeInternetGatewayList(allocator, &reader, "item");
