...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpcEndpointConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVpcEndpointConnections&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVpcEndpointConnectionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.vpc_endpoint_connections = try serde.deserializeVpcEndpointConnectionSet(allocator, &reader, "item");
                } else {
