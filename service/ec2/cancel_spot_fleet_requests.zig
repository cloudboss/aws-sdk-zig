...
pub const CancelSpotFleetRequestsOutput = struct {
    ...
    pub fn deinit(self: *const CancelSpotFleetRequestsOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CancelSpotFleetRequestsOutput {
    _ = status;
    _ = body;
    const result: CancelSpotFleetRequestsOutput = .{ .allocator = alloc };
