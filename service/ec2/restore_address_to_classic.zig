...
pub const RestoreAddressToClassicOutput = struct {
    ...
    pub fn deinit(self: *RestoreAddressToClassicOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: RestoreAddressToClassicInput, options: Options) !RestoreAddressToClassicOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreAddressToClassicInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RestoreAddressToClassic&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&PublicIp=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.public_ip);
