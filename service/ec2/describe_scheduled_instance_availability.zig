...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScheduledInstanceAvailabilityInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.first_slot_start_time_range.earliest_time}) catch "");
    try body_buf.appendSlice(alloc, "&FirstSlotStartTimeRange.LatestTime=");
    try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.first_slot_start_time_range.latest_time}) catch "");
