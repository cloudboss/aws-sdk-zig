...
pub const DescribeSnapshotAttributeOutput = struct {
    ...
    pub fn deinit(self: *DescribeSnapshotAttributeOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeSnapshotAttributeInput, options: Options) !DescribeSnapshotAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSnapshotAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeSnapshotAttribute&Version=2016-11-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSnapshotAttributeOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "createVolumePermission")) {
                    result.create_volume_permissions = try serde.deserializeCreateVolumePermissionList(&reader, alloc, "item");
