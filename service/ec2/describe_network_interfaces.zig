...
pub const DescribeNetworkInterfacesInput = struct {
    ...
    /// parameter is set
    /// to `true`, the output includes resources that are managed by Amazon Web
    /// Services services,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNetworkInterfacesInput, options: CallOptions) !DescribeNetworkInterfacesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeNetworkInterfacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeNetworkInterfaces&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.network_interface_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&NetworkInterfaceId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeNetworkInterfacesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "networkInterfaceSet")) {
                    result.network_interfaces = try serde.deserializeNetworkInterfaceList(allocator, &reader, "item");
