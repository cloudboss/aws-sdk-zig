...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeExportTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeExportTasks&Version=2016-11-15");
    if (input.export_task_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ExportTaskIds.ExportTaskId.{d}=", .{n}) catch continue;
