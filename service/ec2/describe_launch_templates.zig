...
pub const DescribeLaunchTemplatesOutput = struct {
    ...
    pub fn deinit(self: *const DescribeLaunchTemplatesOutput) void {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLaunchTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.launch_template_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&LaunchTemplateIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
    ...
    if (input.launch_template_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&LaunchTemplateNames.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try appendUrlEncoded(alloc, &body_buf, item);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeLaunchTemplatesOutput {
    _ = status;
    var result: DescribeLaunchTemplatesOutput = .{ .allocator = alloc };
