...
pub const ModifyInstanceConnectEndpointOutput = struct {
    ...
    pub fn deinit(self: *const ModifyInstanceConnectEndpointOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyInstanceConnectEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.instance_connect_endpoint_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyInstanceConnectEndpointOutput {
    _ = status;
    var result: ModifyInstanceConnectEndpointOutput = .{ .allocator = alloc };
