...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyInstanceConnectEndpointInput, options: CallOptions) !ModifyInstanceConnectEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyInstanceConnectEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyInstanceConnectEndpoint&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(allocator, "&InstanceConnectEndpointId=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyInstanceConnectEndpointOutput {
    _ = status;
    _ = headers;
    _ = allocator;
