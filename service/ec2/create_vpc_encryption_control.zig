...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVpcEncryptionControlInput, options: CallOptions) !CreateVpcEncryptionControlOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVpcEncryptionControlInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateVpcEncryptionControl&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVpcEncryptionControlOutput {
    _ = status;
    _ = headers;
    ...
    var result: CreateVpcEncryptionControlOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
