...
pub fn execute(client: *Client, input: DeleteRouteServerPeerInput, options: Options) !DeleteRouteServerPeerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRouteServerPeerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteRouteServerPeer&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&RouteServerPeerId=");
    try appendUrlEncoded(alloc, &body_buf, input.route_server_peer_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeleteRouteServerPeerOutput {
    _ = status;
    _ = body;
