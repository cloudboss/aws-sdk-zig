...
pub const DeleteFleetsOutput = struct {
    ...
    pub fn deinit(self: *DeleteFleetsOutput) void {
        self._arena.deinit();
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteFleetsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "successfulFleetDeletionSet")) {
                    result.successful_fleet_deletions = try serde.deserializeDeleteFleetSuccessSet(&reader, alloc, "item");
                } else if (std.mem.eql(u8, e.local, "unsuccessfulFleetDeletionSet")) {
                    result.unsuccessful_fleet_deletions = try serde.deserializeDeleteFleetErrorSet(&reader, alloc, "item");
                } else {
