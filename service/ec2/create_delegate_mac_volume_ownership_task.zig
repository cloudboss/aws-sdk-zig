...
pub const CreateDelegateMacVolumeOwnershipTaskInput = struct {
    ...
    /// }`
    mac_credentials: []const u8,

    /// The tags to assign to the volume ownership delegation task.
    tag_specifications: ?[]const TagSpecification = null,
};

pub const CreateDelegateMacVolumeOwnershipTaskOutput = struct {
    /// Information about the volume ownership delegation task.
    mac_modification_task: ?MacModificationTask = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDelegateMacVolumeOwnershipTaskInput, options: CallOptions) !CreateDelegateMacVolumeOwnershipTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDelegateMacVolumeOwnershipTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateDelegateMacVolumeOwnershipTask&Version=2016-11-15");
    ...
    try body_buf.appendSlice(allocator, "&MacCredentials=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.mac_credentials);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateDelegateMacVolumeOwnershipTaskOutput {
    _ = status;
    _ = headers;
    ...
    var result: CreateDelegateMacVolumeOwnershipTaskOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "macModificationTask")) {
                    result.mac_modification_task = try serde.deserializeMacModificationTask(allocator, &reader);
                } else {
