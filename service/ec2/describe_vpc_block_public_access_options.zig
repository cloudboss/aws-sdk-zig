...
pub fn execute(client: *Client, input: DescribeVpcBlockPublicAccessOptionsInput, options: Options) !DescribeVpcBlockPublicAccessOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpcBlockPublicAccessOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeVpcBlockPublicAccessOptions&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeVpcBlockPublicAccessOptionsOutput {
    _ = status;
    _ = body;
