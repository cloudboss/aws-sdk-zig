...
pub const EnableCapacityManagerOutput = struct {
    ...
    pub fn deinit(self: *const EnableCapacityManagerOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: EnableCapacityManagerInput, options: Options) !EnableCapacityManagerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableCapacityManagerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableCapacityManager&Version=2016-11-15");
    ...
    if (input.organizations_access) |v| {
        try body_buf.appendSlice(alloc, "&OrganizationsAccess=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableCapacityManagerOutput {
    _ = status;
    _ = headers;
    var result: EnableCapacityManagerOutput = .{ .allocator = alloc };
