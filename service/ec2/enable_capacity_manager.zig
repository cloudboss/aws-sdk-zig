...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableCapacityManagerInput, options: CallOptions) !EnableCapacityManagerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableCapacityManagerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=EnableCapacityManager&Version=2016-11-15");
    ...
    if (input.organizations_access) |v| {
        try body_buf.appendSlice(allocator, "&OrganizationsAccess=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableCapacityManagerOutput {
    _ = status;
    _ = headers;
    _ = allocator;
