...
const InstanceImageMetadata = @import("instance_image_metadata.zig").InstanceImageMetadata;
const serde = @import("serde.zig");

pub const DescribeInstanceImageMetadataInput = struct {
    ...
    /// account alias that can be set
    /// using the IAM console. We recommend that you use the `Owner` request
    /// parameter
...
pub const DescribeInstanceImageMetadataOutput = struct {
    /// Information about the instance and the AMI used to launch the instance.
    instance_image_metadata: ?[]const InstanceImageMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceImageMetadataInput, options: CallOptions) !DescribeInstanceImageMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstanceImageMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstanceImageMetadata&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstanceImageMetadataOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeInstanceImageMetadataOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instanceImageMetadataSet")) {
                    result.instance_image_metadata = try serde.deserializeInstanceImageMetadataList(allocator, &reader, "item");
