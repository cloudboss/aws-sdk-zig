...
pub const ModifyInstanceNetworkPerformanceOptionsOutput = struct {
    ...
    pub fn deinit(self: *ModifyInstanceNetworkPerformanceOptionsOutput) void {
        self._arena.deinit();
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyInstanceNetworkPerformanceOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.bandwidth_weighting));
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyInstanceNetworkPerformanceOptionsOutput {
    ...
    var result: ModifyInstanceNetworkPerformanceOptionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
