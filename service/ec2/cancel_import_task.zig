...
pub fn execute(client: *Client, input: CancelImportTaskInput, options: Options) !CancelImportTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelImportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CancelImportTask&Version=2016-11-15");
    if (input.cancel_reason) |v| {
        try body_buf.appendSlice(alloc, "&CancelReason=");
    ...
    if (input.import_task_id) |v| {
        try body_buf.appendSlice(alloc, "&ImportTaskId=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CancelImportTaskOutput {
    _ = status;
