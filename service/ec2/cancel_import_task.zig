...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelImportTaskInput, options: Options) !CancelImportTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelImportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CancelImportTask&Version=2016-11-15");
    if (input.cancel_reason) |v| {
        try body_buf.appendSlice(allocator, "&CancelReason=");
    ...
    if (input.import_task_id) |v| {
        try body_buf.appendSlice(allocator, "&ImportTaskId=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "importTaskId")) {
                    ...
                    result.previous_state = try allocator.dupe(u8, try reader.readElementText());
