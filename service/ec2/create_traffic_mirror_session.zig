...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrafficMirrorSessionInput, options: CallOptions) !CreateTrafficMirrorSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTrafficMirrorSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateTrafficMirrorSession&Version=2016-11-15");
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, std.fmt.allocPrint(allocator, "{d}", .{input.session_number}) catch "");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTrafficMirrorSessionOutput {
    _ = status;
    _ = headers;
