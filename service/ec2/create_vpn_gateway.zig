...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVpnGatewayInput, options: CallOptions) !CreateVpnGatewayOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVpnGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateVpnGateway&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVpnGatewayOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "vpnGateway")) {
                    result.vpn_gateway = try serde.deserializeVpnGateway(allocator, &reader);
                } else {
