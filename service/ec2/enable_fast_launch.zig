...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableFastLaunchInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=EnableFastLaunch&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.launch_template) |v| {
        ...
        try aws.url.appendUrlEncoded(allocator, &body_buf, v.version);
    }
    if (input.max_parallel_launches) |v| {
        try body_buf.appendSlice(allocator, "&MaxParallelLaunches=");
    ...
    if (input.snapshot_configuration) |v| {
        if (v.target_resource_count) |sv| {
            try body_buf.appendSlice(allocator, "&SnapshotConfiguration.TargetResourceCount=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableFastLaunchOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "imageId")) {
                    ...
                    result.launch_template = try serde.deserializeFastLaunchLaunchTemplateSpecificationResponse(allocator, &reader);
