...
pub const CreateRouteTableOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRouteTableInput, options: CallOptions) !CreateRouteTableOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRouteTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateRouteTable&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateRouteTableOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "clientToken")) {
                    ...
                    result.route_table = try serde.deserializeRouteTable(allocator, &reader);
                } else {
