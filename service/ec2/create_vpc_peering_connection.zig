...
pub const CreateVpcPeeringConnectionOutput = struct {
    ...
    pub fn deinit(self: *const CreateVpcPeeringConnectionOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateVpcPeeringConnectionOutput {
    _ = status;
    _ = body;
    const result: CreateVpcPeeringConnectionOutput = .{ .allocator = alloc };
