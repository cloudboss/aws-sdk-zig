...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInstanceExportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, @tagName(input.target_environment));
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateInstanceExportTaskOutput {
    _ = status;
    _ = body;
