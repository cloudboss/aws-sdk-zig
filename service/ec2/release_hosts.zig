...
pub const ReleaseHostsInput = struct {
    /// The IDs of the Dedicated Hosts to release.
    host_ids: []const []const u8,
};

pub const ReleaseHostsOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ReleaseHostsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ReleaseHosts&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ReleaseHostsOutput {
    _ = status;
    _ = headers;
