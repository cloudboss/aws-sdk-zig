...
pub const DeletePublicIpv4PoolInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePublicIpv4PoolInput, options: CallOptions) !DeletePublicIpv4PoolOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePublicIpv4PoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeletePublicIpv4Pool&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePublicIpv4PoolOutput {
    _ = status;
    _ = headers;
    _ = allocator;
    ...
    var result: DeletePublicIpv4PoolOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
