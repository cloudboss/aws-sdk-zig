...
pub const DeletePublicIpv4PoolOutput = struct {
    ...
    pub fn deinit(self: *const DeletePublicIpv4PoolOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePublicIpv4PoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeletePublicIpv4Pool&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeletePublicIpv4PoolOutput {
    _ = status;
    var result: DeletePublicIpv4PoolOutput = .{ .allocator = alloc };
