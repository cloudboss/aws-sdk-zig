...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVolumeStatusInput, options: CallOptions) !DescribeVolumeStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVolumeStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVolumeStatus&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVolumeStatusOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.volume_statuses = try serde.deserializeVolumeStatusList(allocator, &reader, "item");
                } else {
