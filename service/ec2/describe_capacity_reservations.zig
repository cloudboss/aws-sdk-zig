...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCapacityReservationsInput, options: Options) !DescribeCapacityReservationsOutput {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCapacityReservationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "capacityReservationSet")) {
                    result.capacity_reservations = try serde.deserializeCapacityReservationSet(&reader, alloc, "item");
