...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TerminateInstancesInput, options: CallOptions) !TerminateInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TerminateInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=TerminateInstances&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TerminateInstancesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instancesSet")) {
                    result.terminating_instances = try serde.deserializeInstanceStateChangeList(allocator, &reader, "item");
                } else {
