...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyTransitGatewayInput, options: CallOptions) !ModifyTransitGatewayOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyTransitGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyTransitGateway&Version=2016-11-15");
    ...
    if (input.options) |v| {
        ...
        if (v.association_default_route_table_id) |sv| {
            try body_buf.appendSlice(allocator, "&Options.AssociationDefaultRouteTableId=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyTransitGatewayOutput {
    _ = status;
    _ = headers;
