...
pub const CreateRestoreImageTaskOutput = struct {
    ...
    pub fn deinit(self: *const CreateRestoreImageTaskOutput) void {
        if (self.image_id) |v| {
...
pub fn execute(client: *Client, input: CreateRestoreImageTaskInput, options: Options) !CreateRestoreImageTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRestoreImageTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateRestoreImageTask&Version=2016-11-15");
    ...
    try body_buf.appendSlice(alloc, "&ObjectKey=");
    try appendUrlEncoded(alloc, &body_buf, input.object_key);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRestoreImageTaskOutput {
    _ = status;
    _ = headers;
    var result: CreateRestoreImageTaskOutput = .{ .allocator = alloc };
