...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyInstanceEventWindowInput, options: CallOptions) !ModifyInstanceEventWindowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyInstanceEventWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyInstanceEventWindow&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyInstanceEventWindowOutput {
    _ = status;
    _ = headers;
    ...
    var result: ModifyInstanceEventWindowOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
