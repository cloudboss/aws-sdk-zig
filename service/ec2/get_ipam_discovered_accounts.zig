...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIpamDiscoveredAccountsInput, options: CallOptions) !GetIpamDiscoveredAccountsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIpamDiscoveredAccountsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetIpamDiscoveredAccounts&Version=2016-11-15");
    try body_buf.appendSlice(allocator, "&DiscoveryRegion=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.discovery_region);
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIpamDiscoveredAccountsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamDiscoveredAccountSet")) {
                    result.ipam_discovered_accounts = try serde.deserializeIpamDiscoveredAccountSet(allocator, &reader, "item");
