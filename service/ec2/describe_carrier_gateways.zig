...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCarrierGatewaysInput, options: CallOptions) !DescribeCarrierGatewaysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeCarrierGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeCarrierGateways&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeCarrierGatewaysOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "carrierGatewaySet")) {
                    result.carrier_gateways = try serde.deserializeCarrierGatewaySet(allocator, &reader, "item");
