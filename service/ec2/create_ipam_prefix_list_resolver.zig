...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIpamPrefixListResolverInput, options: CallOptions) !CreateIpamPrefixListResolverOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateIpamPrefixListResolverInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateIpamPrefixListResolver&Version=2016-11-15");
    ...
    if (input.rules) |list| {
        for (list, 0..) |item, idx| {
            ...
            if (item.conditions) |lst_1| {
                for (lst_1, 0..) |item_1, idx_1| {
                    ...
                    }
                    {
                        ...
                        try body_buf.appendSlice(allocator, field_prefix);
                        try aws.url.appendUrlEncoded(allocator, &body_buf, @tagName(item_1.operation));
                    }
                    {
                        var prefix_buf: [256]u8 = undefined;
            ...
            }
            {
                ...
                try body_buf.appendSlice(allocator, field_prefix);
                try aws.url.appendUrlEncoded(allocator, &body_buf, @tagName(item.rule_type));
            }
            {
                var prefix_buf: [256]u8 = undefined;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateIpamPrefixListResolverOutput {
    _ = status;
    _ = headers;
