...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLaunchTemplateInput, options: CallOptions) !CreateLaunchTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLaunchTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateLaunchTemplate&Version=2016-11-15");
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.launch_template_name);
    if (input.operator) |v| {
        if (v.principal) |sv| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLaunchTemplateOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "launchTemplate")) {
                    result.launch_template = try serde.deserializeLaunchTemplate(allocator, &reader);
