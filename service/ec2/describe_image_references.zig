...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageReferencesInput, options: CallOptions) !DescribeImageReferencesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeImageReferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeImageReferences&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.include_all_resource_types) |v| {
        try body_buf.appendSlice(allocator, "&IncludeAllResourceTypes=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeImageReferencesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "imageReferenceSet")) {
                    result.image_references = try serde.deserializeImageReferenceList(allocator, &reader, "item");
