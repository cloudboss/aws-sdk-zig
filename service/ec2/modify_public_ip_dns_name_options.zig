...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyPublicIpDnsNameOptionsInput, options: CallOptions) !ModifyPublicIpDnsNameOptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyPublicIpDnsNameOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyPublicIpDnsNameOptions&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyPublicIpDnsNameOptionsOutput {
    _ = status;
    _ = headers;
    _ = allocator;
