...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIpamDiscoveredPublicAddressesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetIpamDiscoveredPublicAddresses&Version=2016-11-15");
    try body_buf.appendSlice(allocator, "&AddressRegion=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.address_region);
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIpamDiscoveredPublicAddressesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamDiscoveredPublicAddressSet")) {
                    result.ipam_discovered_public_addresses = try serde.deserializeIpamDiscoveredPublicAddressSet(allocator, &reader, "item");
