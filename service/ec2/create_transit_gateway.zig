...
pub const CreateTransitGatewayOutput = struct {
    ...
    pub fn deinit(self: *const CreateTransitGatewayOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateTransitGatewayOutput {
    _ = status;
    _ = body;
    const result: CreateTransitGatewayOutput = .{ .allocator = alloc };
