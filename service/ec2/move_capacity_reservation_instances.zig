...
fn serializeRequest(alloc: std.mem.Allocator, input: MoveCapacityReservationInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.destination_capacity_reservation_id);
    if (input.dry_run) |v| {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_capacity_reservation_id);
