...
pub fn execute(client: *Client, input: MoveCapacityReservationInstancesInput, options: Options) !MoveCapacityReservationInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: MoveCapacityReservationInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=MoveCapacityReservationInstances&Version=2016-11-15");
    if (input.client_token) |v| {
        try body_buf.appendSlice(alloc, "&ClientToken=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&DestinationCapacityReservationId=");
    try appendUrlEncoded(alloc, &body_buf, input.destination_capacity_reservation_id);
    if (input.dry_run) |v| {
    ...
    try body_buf.appendSlice(alloc, "&SourceCapacityReservationId=");
    try appendUrlEncoded(alloc, &body_buf, input.source_capacity_reservation_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !MoveCapacityReservationInstancesOutput {
    _ = status;
