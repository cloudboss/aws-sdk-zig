...
pub const DeleteCapacityManagerDataExportOutput = struct {
    ...
    pub fn deinit(self: *DeleteCapacityManagerDataExportOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteCapacityManagerDataExportInput, options: Options) !DeleteCapacityManagerDataExportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCapacityManagerDataExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteCapacityManagerDataExport&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&CapacityManagerDataExportId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.capacity_manager_data_export_id);
    if (input.dry_run) |v| {
