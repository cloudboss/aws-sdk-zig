...
pub fn execute(client: *Client, input: StartNetworkInsightsAnalysisInput, options: Options) !StartNetworkInsightsAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartNetworkInsightsAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.additional_accounts) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&AdditionalAccounts.item.{d}=", .{n}) catch continue;
    ...
    if (input.filter_in_arns) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&FilterInArns.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try aws.url.appendUrlEncoded(alloc, &body_buf, item);
        }
    }
    if (input.filter_out_arns) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&FilterOutArns.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try aws.url.appendUrlEncoded(alloc, &body_buf, item);
        }
    }
