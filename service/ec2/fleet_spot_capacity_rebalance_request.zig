...
/// rebalancing](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-fleet-capacity-rebalance.html) in the *Amazon EC2 User Guide*.
pub const FleetSpotCapacityRebalanceRequest = struct {
    /// The replacement strategy to use. Only available for fleets of type
    /// `maintain`.
    ///
    /// `launch` - EC2 Fleet launches a replacement Spot Instance when a rebalance
    /// notification is emitted for an existing Spot Instance in the fleet. EC2
    /// Fleet does not
    /// terminate the instances that receive a rebalance notification. You can
    /// terminate the old
    /// instances, or you can leave them running. You are charged for all instances
    /// while they are
    /// running.
    ///
