...
pub const DescribeFlowLogsOutput = struct {
    ...
    pub fn deinit(self: *DescribeFlowLogsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeFlowLogsInput, options: Options) !DescribeFlowLogsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFlowLogsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeFlowLogs&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.flow_log_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&FlowLogIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFlowLogsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "flowLogSet")) {
                    result.flow_logs = try serde.deserializeFlowLogSet(&reader, alloc, "item");
