...
pub const DescribeFlowLogsOutput = struct {
    /// Information about the flow logs.
    flow_logs: ?[]const FlowLog = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFlowLogsInput, options: CallOptions) !DescribeFlowLogsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFlowLogsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeFlowLogs&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.flow_log_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&FlowLogId.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFlowLogsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeFlowLogsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "flowLogSet")) {
                    result.flow_logs = try serde.deserializeFlowLogSet(allocator, &reader, "item");
