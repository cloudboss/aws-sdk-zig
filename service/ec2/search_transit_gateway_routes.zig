...
pub const SearchTransitGatewayRoutesOutput = struct {
    /// Indicates whether there are additional routes available.
    additional_routes_available: ?bool = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchTransitGatewayRoutesInput, options: CallOptions) !SearchTransitGatewayRoutesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchTransitGatewayRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=SearchTransitGatewayRoutes&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    for (input.filters, 0..) |item, idx| {
        const n = idx + 1;
        {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchTransitGatewayRoutesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "additionalRoutesAvailable")) {
                    ...
                    result.routes = try serde.deserializeTransitGatewayRouteList(allocator, &reader, "item");
                } else {
