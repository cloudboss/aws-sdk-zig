...
pub fn execute(client: *Client, input: SearchTransitGatewayRoutesInput, options: Options) !SearchTransitGatewayRoutesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchTransitGatewayRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SearchTransitGatewayRoutes&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !SearchTransitGatewayRoutesOutput {
    _ = status;
