...
pub const DeleteNetworkInterfacePermissionOutput = struct {
    ...
    pub fn deinit(self: *DeleteNetworkInterfacePermissionOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteNetworkInterfacePermissionInput, options: Options) !DeleteNetworkInterfacePermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteNetworkInterfacePermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteNetworkInterfacePermission&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    try body_buf.appendSlice(alloc, "&NetworkInterfacePermissionId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.network_interface_permission_id);
