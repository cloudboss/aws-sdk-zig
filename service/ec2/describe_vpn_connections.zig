...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpnConnectionsInput, options: Options) !DescribeVpnConnectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpnConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.vpn_connection_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&VpnConnectionId.VpnConnectionId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
