...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceEventWindowsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInstanceEventWindows&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.instance_event_window_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InstanceEventWindowIds.InstanceEventWindowId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeInstanceEventWindowsOutput {
    _ = status;
