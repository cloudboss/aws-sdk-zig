...
pub const DescribeInstanceEventWindowsInput = struct {
    ...
    /// event window. Use the tag key in the filter name and the tag value as the
    /// filter
    ...
    instance_event_window_ids: ?[]const []const u8 = null,

    /// The maximum number of results to return in a single call. To retrieve the
    /// remaining
    /// results, make another call with the returned `NextToken` value. This value
    /// can
    /// be between 20 and 500. You cannot specify this parameter and the event
    /// window IDs parameter
    /// in the same call.
...
pub const DescribeInstanceEventWindowsOutput = struct {
    /// Information about the event windows.
    instance_event_windows: ?[]const InstanceEventWindow = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceEventWindowsInput, options: CallOptions) !DescribeInstanceEventWindowsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstanceEventWindowsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstanceEventWindows&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.instance_event_window_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InstanceEventWindowId.InstanceEventWindowId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstanceEventWindowsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instanceEventWindowSet")) {
                    result.instance_event_windows = try serde.deserializeInstanceEventWindowSet(allocator, &reader, "item");
