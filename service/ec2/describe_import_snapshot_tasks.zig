...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeImportSnapshotTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeImportSnapshotTasks&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeImportSnapshotTasksOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "importSnapshotTaskSet")) {
                    result.import_snapshot_tasks = try serde.deserializeImportSnapshotTaskList(allocator, &reader, "item");
