...
pub const DeprovisionIpamPoolCidrOutput = struct {
    ...
    pub fn deinit(self: *const DeprovisionIpamPoolCidrOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeprovisionIpamPoolCidrInput, options: Options) !DeprovisionIpamPoolCidrOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeprovisionIpamPoolCidrInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeprovisionIpamPoolCidr&Version=2016-11-15");
    if (input.cidr) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeprovisionIpamPoolCidrOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeprovisionIpamPoolCidrOutput = .{ .allocator = alloc };
