...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSecurityGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateSecurityGroup&Version=2016-11-15");
    try body_buf.appendSlice(allocator, "&GroupDescription=");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "groupId")) {
                    ...
                    result.security_group_arn = try allocator.dupe(u8, try reader.readElementText());
