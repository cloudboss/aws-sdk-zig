...
pub fn execute(client: *Client, input: ModifyInstanceCreditSpecificationInput, options: Options) !ModifyInstanceCreditSpecificationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyInstanceCreditSpecificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyInstanceCreditSpecification&Version=2016-11-15");
    ...
    for (input.instance_credit_specifications, 0..) |item, idx| {
        ...
        }
        {
            ...
            try body_buf.appendSlice(alloc, field_prefix);
            try appendUrlEncoded(alloc, &body_buf, item.instance_id);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyInstanceCreditSpecificationOutput {
    _ = status;
    _ = body;
