...
pub const ModifyInstanceCreditSpecificationInput = struct {
    ...
    instance_credit_specifications: []const InstanceCreditSpecificationRequest,
};

pub const ModifyInstanceCreditSpecificationOutput = struct {
    /// Information about the instances whose credit option for CPU usage was
    /// successfully
    /// modified.
    successful_instance_credit_specifications: ?[]const SuccessfulInstanceCreditSpecificationItem = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyInstanceCreditSpecificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyInstanceCreditSpecification&Version=2016-11-15");
    ...
    for (input.instance_credit_specifications, 0..) |item, idx| {
        const n = idx + 1;
        {
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InstanceCreditSpecification.{d}.CpuCredits=", .{n}) catch continue;
        ...
        {
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InstanceCreditSpecification.{d}.InstanceId=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item.instance_id);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyInstanceCreditSpecificationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "successfulInstanceCreditSpecificationSet")) {
                    result.successful_instance_credit_specifications = try serde.deserializeSuccessfulInstanceCreditSpecificationSet(allocator, &reader, "item");
                } else if (std.mem.eql(u8, e.local, "unsuccessfulInstanceCreditSpecificationSet")) {
                    result.unsuccessful_instance_credit_specifications = try serde.deserializeUnsuccessfulInstanceCreditSpecificationSet(allocator, &reader, "item");
                } else {
