...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTransitGatewayRouteTableAnnouncementInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateTransitGatewayRouteTableAnnouncement&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&PeeringAttachmentId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.peering_attachment_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTransitGatewayRouteTableAnnouncementOutput {
    _ = status;
    _ = headers;
