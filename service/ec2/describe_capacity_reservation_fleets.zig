...
pub const DescribeCapacityReservationFleetsOutput = struct {
    /// Information about the Capacity Reservation Fleets.
    capacity_reservation_fleets: ?[]const CapacityReservationFleet = null,
    ...
    pub fn deinit(self: *DescribeCapacityReservationFleetsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeCapacityReservationFleetsInput, options: Options) !DescribeCapacityReservationFleetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCapacityReservationFleetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCapacityReservationFleets&Version=2016-11-15");
    if (input.capacity_reservation_fleet_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCapacityReservationFleetsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeCapacityReservationFleetsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "capacityReservationFleetSet")) {
                    result.capacity_reservation_fleets = try serde.deserializeCapacityReservationFleetSet(&reader, alloc, "item");
