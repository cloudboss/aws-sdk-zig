...
pub const ModifyIdFormatOutput = struct {
    ...
    pub fn deinit(self: *const ModifyIdFormatOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyIdFormatOutput {
    _ = status;
    _ = body;
    const result: ModifyIdFormatOutput = .{ .allocator = alloc };
