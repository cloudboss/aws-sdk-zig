...
pub const ResetSnapshotAttributeOutput = struct {
    ...
    pub fn deinit(self: *const ResetSnapshotAttributeOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: ResetSnapshotAttributeInput, options: Options) !ResetSnapshotAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetSnapshotAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ResetSnapshotAttribute&Version=2016-11-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetSnapshotAttributeOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: ResetSnapshotAttributeOutput = .{ .allocator = alloc };
