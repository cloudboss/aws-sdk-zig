...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpnGatewaysInput, options: CallOptions) !DescribeVpnGatewaysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpnGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVpnGateways&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVpnGatewaysOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "vpnGatewaySet")) {
                    result.vpn_gateways = try serde.deserializeVpnGatewayList(allocator, &reader, "item");
                } else {
