...
pub const DescribeVpnGatewaysInput = struct {
    ...
    vpn_gateway_ids: ?[]const []const u8 = null,
};

pub const DescribeVpnGatewaysOutput = struct {
    /// Information about one or more virtual private gateways.
    vpn_gateways: ?[]const VpnGateway = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *DescribeVpnGatewaysOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeVpnGatewaysInput, options: Options) !DescribeVpnGatewaysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpnGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeVpnGateways&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.vpn_gateway_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&VpnGatewayIds.VpnGatewayId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVpnGatewaysOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeVpnGatewaysOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "vpnGatewaySet")) {
                    result.vpn_gateways = try serde.deserializeVpnGatewayList(&reader, alloc, "item");
                } else {
