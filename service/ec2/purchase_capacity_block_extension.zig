...
pub fn execute(client: *Client, input: PurchaseCapacityBlockExtensionInput, options: Options) !PurchaseCapacityBlockExtensionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseCapacityBlockExtensionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PurchaseCapacityBlockExtension&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&CapacityBlockExtensionOfferingId=");
    try appendUrlEncoded(alloc, &body_buf, input.capacity_block_extension_offering_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !PurchaseCapacityBlockExtensionOutput {
    _ = status;
    _ = body;
