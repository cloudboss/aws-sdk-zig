...
pub fn execute(client: *Client, input: GetVpnConnectionDeviceTypesInput, options: Options) !GetVpnConnectionDeviceTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVpnConnectionDeviceTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetVpnConnectionDeviceTypes&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetVpnConnectionDeviceTypesOutput {
    _ = status;
