...
pub const DescribeFleetsOutput = struct {
    ...
    pub fn deinit(self: *DescribeFleetsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeFleetsInput, options: Options) !DescribeFleetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeFleets&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "fleetSet")) {
                    result.fleets = try serde.deserializeFleetSet(&reader, alloc, "item");
