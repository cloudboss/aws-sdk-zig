...
pub const DescribeFleetsOutput = struct {
    /// Information about the EC2 Fleets.
    fleets: ?[]const FleetData = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetsInput, options: CallOptions) !DescribeFleetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFleetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeFleets&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.fleet_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&FleetId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFleetsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeFleetsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "fleetSet")) {
                    result.fleets = try serde.deserializeFleetSet(allocator, &reader, "item");
