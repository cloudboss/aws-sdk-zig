...
pub fn execute(client: *Client, input: CancelCapacityReservationFleetsInput, options: Options) !CancelCapacityReservationFleetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelCapacityReservationFleetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CancelCapacityReservationFleets&Version=2016-11-15");
    for (input.capacity_reservation_fleet_ids, 0..) |item, idx| {
        const n = idx + 1;
        var prefix_buf: [256]u8 = undefined;
        const field_prefix = std.fmt.bufPrint(&prefix_buf, "&CapacityReservationFleetIds.item.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CancelCapacityReservationFleetsOutput {
    _ = status;
    _ = body;
