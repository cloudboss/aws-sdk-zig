...
pub const AssignPrivateNatGatewayAddressOutput = struct {
    ...
    pub fn deinit(self: *AssignPrivateNatGatewayAddressOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: AssignPrivateNatGatewayAddressInput, options: Options) !AssignPrivateNatGatewayAddressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssignPrivateNatGatewayAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AssignPrivateNatGatewayAddress&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.private_ip_address_count) |v| {
        try body_buf.appendSlice(alloc, "&PrivateIpAddressCount=");
