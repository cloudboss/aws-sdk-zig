...
pub const AssignPrivateNatGatewayAddressOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssignPrivateNatGatewayAddressInput, options: CallOptions) !AssignPrivateNatGatewayAddressOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssignPrivateNatGatewayAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AssignPrivateNatGatewayAddress&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.private_ip_address_count) |v| {
        try body_buf.appendSlice(allocator, "&PrivateIpAddressCount=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssignPrivateNatGatewayAddressOutput {
    _ = status;
    _ = headers;
    ...
    var result: AssignPrivateNatGatewayAddressOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
