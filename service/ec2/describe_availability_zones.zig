...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAvailabilityZonesInput, options: CallOptions) !DescribeAvailabilityZonesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAvailabilityZonesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAvailabilityZones&Version=2016-11-15");
    ...
    if (input.zone_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ZoneId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
    ...
    if (input.zone_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ZoneName.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAvailabilityZonesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "availabilityZoneInfo")) {
                    result.availability_zones = try serde.deserializeAvailabilityZoneList(allocator, &reader, "item");
                } else {
