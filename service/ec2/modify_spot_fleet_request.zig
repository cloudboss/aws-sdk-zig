...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifySpotFleetRequestInput, options: CallOptions) !ModifySpotFleetRequestOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifySpotFleetRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifySpotFleetRequest&Version=2016-11-15");
    if (input.context) |v| {
    ...
    if (input.on_demand_target_capacity) |v| {
        try body_buf.appendSlice(allocator, "&OnDemandTargetCapacity=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifySpotFleetRequestOutput {
    _ = status;
    _ = headers;
    _ = allocator;
