...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableFastSnapshotRestoresInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=EnableFastSnapshotRestores&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableFastSnapshotRestoresOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "successful")) {
                    result.successful = try serde.deserializeEnableFastSnapshotRestoreSuccessSet(allocator, &reader, "item");
                } else if (std.mem.eql(u8, e.local, "unsuccessful")) {
                    result.unsuccessful = try serde.deserializeEnableFastSnapshotRestoreErrorSet(allocator, &reader, "item");
                } else {
