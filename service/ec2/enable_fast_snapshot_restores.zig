...
pub const EnableFastSnapshotRestoresOutput = struct {
    ...
    pub fn deinit(self: *const EnableFastSnapshotRestoresOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !EnableFastSnapshotRestoresOutput {
    _ = status;
    _ = body;
    const result: EnableFastSnapshotRestoresOutput = .{ .allocator = alloc };
