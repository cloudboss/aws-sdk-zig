...
pub fn execute(client: *Client, input: CancelReservedInstancesListingInput, options: Options) !CancelReservedInstancesListingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelReservedInstancesListingInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CancelReservedInstancesListing&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&ReservedInstancesListingId=");
    try appendUrlEncoded(alloc, &body_buf, input.reserved_instances_listing_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CancelReservedInstancesListingOutput {
    _ = status;
    _ = body;
