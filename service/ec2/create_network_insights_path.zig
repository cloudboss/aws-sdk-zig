...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNetworkInsightsPathInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, @tagName(input.protocol));
    try body_buf.appendSlice(alloc, "&Source=");
    try appendUrlEncoded(alloc, &body_buf, input.source);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateNetworkInsightsPathOutput {
    _ = status;
    _ = body;
