...
pub const ModifyVpcTenancyOutput = struct {
    ...
    pub fn deinit(self: *const ModifyVpcTenancyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: ModifyVpcTenancyInput, options: Options) !ModifyVpcTenancyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyVpcTenancyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyVpcTenancy&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&InstanceTenancy=");
    try appendUrlEncoded(alloc, &body_buf, @tagName(input.instance_tenancy));
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyVpcTenancyOutput {
    _ = status;
    _ = headers;
    var result: ModifyVpcTenancyOutput = .{ .allocator = alloc };
