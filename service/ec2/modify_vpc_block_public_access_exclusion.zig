...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyVpcBlockPublicAccessExclusionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyVpcBlockPublicAccessExclusion&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyVpcBlockPublicAccessExclusionOutput {
    _ = status;
    _ = headers;
