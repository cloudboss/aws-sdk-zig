...
pub const DeleteVpcInput = struct {
...
pub const DeleteVpcOutput = struct {
    ...
    pub fn deinit(self: *const DeleteVpcOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteVpcInput, options: Options) !DeleteVpcOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVpcInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteVpc&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVpcOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeleteVpcOutput = .{ .allocator = alloc };
