...
pub const DeleteIpamOutput = struct {
    ...
    pub fn deinit(self: *const DeleteIpamOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeleteIpamOutput {
    _ = status;
    _ = body;
    const result: DeleteIpamOutput = .{ .allocator = alloc };
