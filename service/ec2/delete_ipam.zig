...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteIpamInput, options: CallOptions) !DeleteIpamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteIpamInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteIpam&Version=2016-11-15");
    if (input.cascade) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteIpamOutput {
    _ = status;
    _ = headers;
