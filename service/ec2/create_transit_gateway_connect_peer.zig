...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTransitGatewayConnectPeerInput, options: CallOptions) !CreateTransitGatewayConnectPeerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTransitGatewayConnectPeerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateTransitGatewayConnectPeer&Version=2016-11-15");
    ...
    for (input.inside_cidr_blocks, 0..) |item, idx| {
        ...
        const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InsideCidrBlocks.{d}=", .{n}) catch continue;
    ...
    if (input.transit_gateway_address) |v| {
        try body_buf.appendSlice(allocator, "&TransitGatewayAddress=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTransitGatewayConnectPeerOutput {
    _ = status;
    _ = headers;
