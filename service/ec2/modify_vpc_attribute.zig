...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyVpcAttributeInput, options: CallOptions) !ModifyVpcAttributeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyVpcAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyVpcAttribute&Version=2016-11-15");
    if (input.enable_dns_hostnames) |v| {
        if (v.value) |sv| {
            try body_buf.appendSlice(allocator, "&EnableDnsHostnames.Value=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, if (sv) "true" else "false");
        }
    }
    if (input.enable_dns_support) |v| {
        if (v.value) |sv| {
            try body_buf.appendSlice(allocator, "&EnableDnsSupport.Value=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, if (sv) "true" else "false");
        }
    }
    if (input.enable_network_address_usage_metrics) |v| {
        if (v.value) |sv| {
            try body_buf.appendSlice(allocator, "&EnableNetworkAddressUsageMetrics.Value=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, if (sv) "true" else "false");
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyVpcAttributeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
