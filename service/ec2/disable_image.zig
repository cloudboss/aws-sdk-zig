...
pub const DisableImageOutput = struct {
    ...
    pub fn deinit(self: *const DisableImageOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DisableImageInput, options: Options) !DisableImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisableImage&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableImageOutput {
    _ = status;
    _ = headers;
    var result: DisableImageOutput = .{ .allocator = alloc };
