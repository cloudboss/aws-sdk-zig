...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurchaseScheduledInstancesInput, options: CallOptions) !PurchaseScheduledInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PurchaseScheduledInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=PurchaseScheduledInstances&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PurchaseScheduledInstancesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "scheduledInstanceSet")) {
                    result.scheduled_instance_set = try serde.deserializePurchasedScheduledInstanceSet(allocator, &reader, "item");
                } else {
