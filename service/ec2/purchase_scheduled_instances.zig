...
pub const PurchaseScheduledInstancesOutput = struct {
    ...
    pub fn deinit(self: *const PurchaseScheduledInstancesOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseScheduledInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    for (input.purchase_requests, 0..) |item, idx| {
        const n = idx + 1;
        {
            ...
            try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{item.instance_count}) catch "");
        }
        {
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&PurchaseRequests.PurchaseRequest.{d}.PurchaseToken=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try appendUrlEncoded(alloc, &body_buf, item.purchase_token);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !PurchaseScheduledInstancesOutput {
    _ = status;
    _ = body;
    const result: PurchaseScheduledInstancesOutput = .{ .allocator = alloc };
