...
pub const ModifyTrafficMirrorFilterRuleOutput = struct {
    ...
    pub fn deinit(self: *const ModifyTrafficMirrorFilterRuleOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyTrafficMirrorFilterRuleOutput {
    _ = status;
    _ = body;
    const result: ModifyTrafficMirrorFilterRuleOutput = .{ .allocator = alloc };
