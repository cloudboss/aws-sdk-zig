...
pub const ResetInstanceAttributeOutput = struct {
    ...
    pub fn deinit(self: *const ResetInstanceAttributeOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: ResetInstanceAttributeInput, options: Options) !ResetInstanceAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetInstanceAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ResetInstanceAttribute&Version=2016-11-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetInstanceAttributeOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: ResetInstanceAttributeOutput = .{ .allocator = alloc };
