...
pub const CreateVpnConnectionRouteOutput = struct {
    ...
    pub fn deinit(self: *const CreateVpnConnectionRouteOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateVpnConnectionRouteOutput {
    _ = status;
    _ = body;
    const result: CreateVpnConnectionRouteOutput = .{ .allocator = alloc };
