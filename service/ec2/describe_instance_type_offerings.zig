...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceTypeOfferingsInput, options: CallOptions) !DescribeInstanceTypeOfferingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstanceTypeOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstanceTypeOfferings&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstanceTypeOfferingsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instanceTypeOfferingSet")) {
                    result.instance_type_offerings = try serde.deserializeInstanceTypeOfferingsList(allocator, &reader, "item");
