...
pub const DescribeInstanceTypeOfferingsOutput = struct {
    /// The instance types offered in the location.
    instance_type_offerings: ?[]const InstanceTypeOffering = null,
    ...
    pub fn deinit(self: *DescribeInstanceTypeOfferingsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeInstanceTypeOfferingsInput, options: Options) !DescribeInstanceTypeOfferingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceTypeOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInstanceTypeOfferings&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.location_type) |v| {
        try body_buf.appendSlice(alloc, "&LocationType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceTypeOfferingsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeInstanceTypeOfferingsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instanceTypeOfferingSet")) {
                    result.instance_type_offerings = try serde.deserializeInstanceTypeOfferingsList(&reader, alloc, "item");
