...
pub const DeleteVpcPeeringConnectionOutput = struct {
    ...
    pub fn deinit(self: *const DeleteVpcPeeringConnectionOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteVpcPeeringConnectionInput, options: Options) !DeleteVpcPeeringConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVpcPeeringConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteVpcPeeringConnection&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVpcPeeringConnectionOutput {
    _ = status;
    _ = headers;
    var result: DeleteVpcPeeringConnectionOutput = .{ .allocator = alloc };
