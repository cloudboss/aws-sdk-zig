...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyVpnConnectionOptionsInput, options: CallOptions) !ModifyVpnConnectionOptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyVpnConnectionOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyVpnConnectionOptions&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    if (input.local_ipv_4_network_cidr) |v| {
        try body_buf.appendSlice(allocator, "&LocalIpv4NetworkCidr=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.local_ipv_6_network_cidr) |v| {
        try body_buf.appendSlice(allocator, "&LocalIpv6NetworkCidr=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.remote_ipv_4_network_cidr) |v| {
        try body_buf.appendSlice(allocator, "&RemoteIpv4NetworkCidr=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyVpnConnectionOptionsOutput {
    _ = status;
    _ = headers;
