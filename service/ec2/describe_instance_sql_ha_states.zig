...
pub fn execute(client: *Client, input: DescribeInstanceSqlHaStatesInput, options: Options) !DescribeInstanceSqlHaStatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceSqlHaStatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInstanceSqlHaStates&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeInstanceSqlHaStatesOutput {
    _ = status;
