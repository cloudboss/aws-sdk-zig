...
pub const DescribeInstanceSqlHaStatesInput = struct {
    ...
    /// SQL Server High Availability states for all SQL Server High Availability
    /// instances.
...
pub const DescribeInstanceSqlHaStatesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceSqlHaStatesInput, options: CallOptions) !DescribeInstanceSqlHaStatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstanceSqlHaStatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstanceSqlHaStates&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstanceSqlHaStatesOutput {
    _ = status;
    _ = headers;
