...
const ModifyIdentityIdFormatOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyIdentityIdFormatInput, options: Options) !ModifyIdentityIdFormatOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyIdentityIdFormatInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyIdentityIdFormat&Version=2016-11-15");
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.resource);
    try body_buf.appendSlice(allocator, "&UseLongIds=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, if (input.use_long_ids) "true" else "false");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyIdentityIdFormatOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
