...
pub const CreateLocalGatewayVirtualInterfaceOutput = struct {
    ...
    pub fn deinit(self: *CreateLocalGatewayVirtualInterfaceOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: CreateLocalGatewayVirtualInterfaceInput, options: Options) !CreateLocalGatewayVirtualInterfaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocalGatewayVirtualInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateLocalGatewayVirtualInterface&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&LocalAddress=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.local_address);
    try body_buf.appendSlice(alloc, "&LocalGatewayVirtualInterfaceGroupId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.local_gateway_virtual_interface_group_id);
    try body_buf.appendSlice(alloc, "&OutpostLagId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.outpost_lag_id);
    ...
    if (input.peer_bgp_asn) |v| {
        try body_buf.appendSlice(alloc, "&PeerBgpAsn=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.peer_bgp_asn_extended) |v| {
        try body_buf.appendSlice(alloc, "&PeerBgpAsnExtended=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    ...
    try body_buf.appendSlice(alloc, "&Vlan=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.vlan}) catch "");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "localGatewayVirtualInterface")) {
                    result.local_gateway_virtual_interface = try serde.deserializeLocalGatewayVirtualInterface(&reader, alloc);
                } else {
