...
pub fn execute(client: *Client, input: ModifyVpcBlockPublicAccessOptionsInput, options: Options) !ModifyVpcBlockPublicAccessOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyVpcBlockPublicAccessOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyVpcBlockPublicAccessOptions&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&InternetGatewayBlockMode=");
    try appendUrlEncoded(alloc, &body_buf, @tagName(input.internet_gateway_block_mode));
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyVpcBlockPublicAccessOptionsOutput {
    _ = status;
    _ = body;
