...
pub const EnableSnapshotBlockPublicAccessInput = struct {
    ...
    /// public
    /// sharing. However, snapshots that are already publicly shared, remain
    /// publicly
    /// available.
    ///
...
pub const EnableSnapshotBlockPublicAccessOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableSnapshotBlockPublicAccessInput, options: CallOptions) !EnableSnapshotBlockPublicAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableSnapshotBlockPublicAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=EnableSnapshotBlockPublicAccess&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(allocator, "&State=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.state.wireName());
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableSnapshotBlockPublicAccessOutput {
    _ = status;
    _ = headers;
    _ = allocator;
