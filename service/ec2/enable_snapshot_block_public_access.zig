...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableSnapshotBlockPublicAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableSnapshotBlockPublicAccess&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&State=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.state));
