...
pub const DescribeCapacityReservationTopologyInput = struct {
    ...
    ///
    /// You can't specify this parameter and the Capacity Reservation IDs parameter
    /// in the
    /// same request.
    ///
    /// Default: `10`
...
pub const DescribeCapacityReservationTopologyOutput = struct {
    /// Information about the topology of each Capacity Reservation.
    capacity_reservations: ?[]const CapacityReservationTopology = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCapacityReservationTopologyInput, options: CallOptions) !DescribeCapacityReservationTopologyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeCapacityReservationTopologyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeCapacityReservationTopology&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeCapacityReservationTopologyOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeCapacityReservationTopologyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "capacityReservationSet")) {
                    result.capacity_reservations = try serde.deserializeCapacityReservationTopologySet(allocator, &reader, "item");
