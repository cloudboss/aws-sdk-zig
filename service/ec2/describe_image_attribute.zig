...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageAttributeInput, options: CallOptions) !DescribeImageAttributeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeImageAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeImageAttribute&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeImageAttributeOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "blockDeviceMapping")) {
                    result.block_device_mappings = try serde.deserializeBlockDeviceMappingList(allocator, &reader, "item");
                } else if (std.mem.eql(u8, e.local, "bootMode")) {
                    result.boot_mode = try serde.deserializeAttributeValue(allocator, &reader);
                } else if (std.mem.eql(u8, e.local, "deregistrationProtection")) {
                    result.deregistration_protection = try serde.deserializeAttributeValue(allocator, &reader);
                } else if (std.mem.eql(u8, e.local, "description")) {
                    result.description = try serde.deserializeAttributeValue(allocator, &reader);
                    ...
                    result.imds_support = try serde.deserializeAttributeValue(allocator, &reader);
                    ...
                    result.last_launched_time = try serde.deserializeAttributeValue(allocator, &reader);
                } else if (std.mem.eql(u8, e.local, "launchPermission")) {
                    result.launch_permissions = try serde.deserializeLaunchPermissionList(allocator, &reader, "item");
                    ...
                    result.tpm_support = try serde.deserializeAttributeValue(allocator, &reader);
                } else if (std.mem.eql(u8, e.local, "uefiData")) {
                    result.uefi_data = try serde.deserializeAttributeValue(allocator, &reader);
                } else {
