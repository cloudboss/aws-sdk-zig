...
pub const DeleteNatGatewayOutput = struct {
    ...
    pub fn deinit(self: *const DeleteNatGatewayOutput) void {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteNatGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteNatGateway&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteNatGatewayOutput {
    _ = status;
    _ = headers;
    var result: DeleteNatGatewayOutput = .{ .allocator = alloc };
