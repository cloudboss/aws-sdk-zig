...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSpotFleetRequestHistoryInput, options: CallOptions) !DescribeSpotFleetRequestHistoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSpotFleetRequestHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeSpotFleetRequestHistory&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSpotFleetRequestHistoryOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "historyRecordSet")) {
                    result.history_records = try serde.deserializeHistoryRecords(allocator, &reader, "item");
