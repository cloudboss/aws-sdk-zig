...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIpamAddressHistoryInput, options: CallOptions) !GetIpamAddressHistoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIpamAddressHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetIpamAddressHistory&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIpamAddressHistoryOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "historyRecordSet")) {
                    result.history_records = try serde.deserializeIpamAddressHistoryRecordSet(allocator, &reader, "item");
