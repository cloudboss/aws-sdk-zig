...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMacSystemIntegrityProtectionModificationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, @tagName(input.mac_system_integrity_protection_status));
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateMacSystemIntegrityProtectionModificationTaskOutput {
    _ = status;
    _ = body;
