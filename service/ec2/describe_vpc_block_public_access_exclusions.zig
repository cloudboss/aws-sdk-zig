...
pub const DescribeVpcBlockPublicAccessExclusionsOutput = struct {
    ...
    pub fn deinit(self: *DescribeVpcBlockPublicAccessExclusionsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeVpcBlockPublicAccessExclusionsInput, options: Options) !DescribeVpcBlockPublicAccessExclusionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpcBlockPublicAccessExclusionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeVpcBlockPublicAccessExclusions&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.exclusion_ids) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ExclusionIds.item.{d}=", .{n}) catch continue;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.vpc_block_public_access_exclusions = try serde.deserializeVpcBlockPublicAccessExclusionList(&reader, alloc, "item");
                } else {
