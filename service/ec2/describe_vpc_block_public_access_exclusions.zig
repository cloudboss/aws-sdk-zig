...
pub const DescribeVpcBlockPublicAccessExclusionsInput = struct {
    ...
    /// IDs of exclusions.
    exclusion_ids: ?[]const []const u8 = null,
    ...
    ///   to find all resources assigned a tag with a specific value, regardless of
    ///   the tag key.
...
pub const DescribeVpcBlockPublicAccessExclusionsOutput = struct {
    /// The token to include in another request to get the next page of items. This
    /// value is `null` when there are no more items to return.
    next_token: ?[]const u8 = null,

    /// Details related to the exclusions.
    vpc_block_public_access_exclusions: ?[]const VpcBlockPublicAccessExclusion = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpcBlockPublicAccessExclusionsInput, options: CallOptions) !DescribeVpcBlockPublicAccessExclusionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpcBlockPublicAccessExclusionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVpcBlockPublicAccessExclusions&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    if (input.exclusion_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ExclusionId.item.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVpcBlockPublicAccessExclusionsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeVpcBlockPublicAccessExclusionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "vpcBlockPublicAccessExclusionSet")) {
                    result.vpc_block_public_access_exclusions = try serde.deserializeVpcBlockPublicAccessExclusionList(allocator, &reader, "item");
                } else {
