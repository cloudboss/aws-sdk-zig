...
pub const GetTransitGatewayMeteringPolicyEntriesInput = struct {
...
pub const GetTransitGatewayMeteringPolicyEntriesOutput = struct {
    /// The token to use to retrieve the next page of results. This value is `null`
    /// when there are no more results to return.
    next_token: ?[]const u8 = null,

    /// Information about the transit gateway metering policy entries.
    transit_gateway_metering_policy_entries: ?[]const TransitGatewayMeteringPolicyEntry = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *GetTransitGatewayMeteringPolicyEntriesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetTransitGatewayMeteringPolicyEntriesInput, options: Options) !GetTransitGatewayMeteringPolicyEntriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTransitGatewayMeteringPolicyEntriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetTransitGatewayMeteringPolicyEntries&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTransitGatewayMeteringPolicyEntriesOutput {
    _ = status;
    _ = headers;
    ...
    var result: GetTransitGatewayMeteringPolicyEntriesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "transitGatewayMeteringPolicyEntries")) {
                    result.transit_gateway_metering_policy_entries = try serde.deserializeTransitGatewayMeteringPolicyEntryList(&reader, alloc, "item");
                } else {
