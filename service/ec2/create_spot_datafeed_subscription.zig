...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSpotDatafeedSubscriptionInput, options: CallOptions) !CreateSpotDatafeedSubscriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSpotDatafeedSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateSpotDatafeedSubscription&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSpotDatafeedSubscriptionOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "spotDatafeedSubscription")) {
                    result.spot_datafeed_subscription = try serde.deserializeSpotDatafeedSubscription(allocator, &reader);
                } else {
