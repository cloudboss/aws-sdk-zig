...
pub const DisassociateSecurityGroupVpcOutput = struct {
    /// The state of the disassociation.
    state: ?SecurityGroupVpcAssociationState = null,

    allocator: std.mem.Allocator,

    pub fn deinit(self: *const DisassociateSecurityGroupVpcOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DisassociateSecurityGroupVpcInput, options: Options) !DisassociateSecurityGroupVpcOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateSecurityGroupVpcInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisassociateSecurityGroupVpc&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateSecurityGroupVpcOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DisassociateSecurityGroupVpcOutput = .{ .allocator = alloc };
