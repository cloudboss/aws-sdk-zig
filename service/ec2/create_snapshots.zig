...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSnapshotsInput, options: CallOptions) !CreateSnapshotsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.instance_specification.exclude_boot_volume) |sv| {
        try body_buf.appendSlice(allocator, "&InstanceSpecification.ExcludeBootVolume=");
    ...
    if (input.instance_specification.exclude_data_volume_ids) |list_d0| {
        for (list_d0, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InstanceSpecification.ExcludeDataVolumeId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item);
        }
    }
    try body_buf.appendSlice(allocator, "&InstanceSpecification.InstanceId=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSnapshotsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "snapshotSet")) {
                    result.snapshots = try serde.deserializeSnapshotSet(allocator, &reader, "item");
                } else {
