...
pub const EnableIpamPolicyInput = struct {
...
pub const EnableIpamPolicyOutput = struct {
    /// The ID of the IPAM policy that was enabled.
    ipam_policy_id: ?[]const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableIpamPolicyInput, options: CallOptions) !EnableIpamPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableIpamPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=EnableIpamPolicy&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableIpamPolicyOutput {
    _ = status;
    _ = headers;
