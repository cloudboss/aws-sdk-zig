...
pub const CreateTransitGatewayRouteOutput = struct {
    /// Information about the route.
    route: ?TransitGatewayRoute = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTransitGatewayRouteInput, options: CallOptions) !CreateTransitGatewayRouteOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTransitGatewayRouteInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateTransitGatewayRoute&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTransitGatewayRouteOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "route")) {
                    result.route = try serde.deserializeTransitGatewayRoute(allocator, &reader);
                } else {
