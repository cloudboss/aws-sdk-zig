...
pub fn execute(client: *Client, input: DescribeCapacityBlockOfferingsInput, options: Options) !DescribeCapacityBlockOfferingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCapacityBlockOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCapacityBlockOfferings&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&CapacityDurationHours=");
    try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.capacity_duration_hours}) catch "");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.end_date_range) |v| {
        try body_buf.appendSlice(alloc, "&EndDateRange=");
    ...
    if (input.start_date_range) |v| {
        try body_buf.appendSlice(alloc, "&StartDateRange=");
        try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.ultraserver_count) |v| {
        try body_buf.appendSlice(alloc, "&UltraserverCount=");
        try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.ultraserver_type) |v| {
        try body_buf.appendSlice(alloc, "&UltraserverType=");
        try appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeCapacityBlockOfferingsOutput {
    _ = status;
