...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCapacityBlockOfferingsInput, options: Options) !DescribeCapacityBlockOfferingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCapacityBlockOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.capacity_duration_hours}) catch "");
    if (input.dry_run) |v| {
