...
pub const ModifyVpnTunnelOptionsOutput = struct {
    ...
    pub fn deinit(self: *const ModifyVpnTunnelOptionsOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyVpnTunnelOptionsOutput {
    _ = status;
    _ = body;
    const result: ModifyVpnTunnelOptionsOutput = .{ .allocator = alloc };
