...
pub const GetPasswordDataOutput = struct {
    ...
    pub fn deinit(self: *const GetPasswordDataOutput) void {
        if (self.instance_id) |v| {
            self.allocator.free(v);
        }
        if (self.password_data) |v| {
            self.allocator.free(v);
        }
    }
};
...
pub fn execute(client: *Client, input: GetPasswordDataInput, options: Options) !GetPasswordDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPasswordDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetPasswordData&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetPasswordDataOutput {
    ...
    if (findElement(body, "passwordData")) |content| {
        result.password_data = try alloc.dupe(u8, content);
    }
