...
pub const DeleteSnapshotOutput = struct {
    ...
    pub fn deinit(self: *const DeleteSnapshotOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteSnapshotInput, options: Options) !DeleteSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteSnapshot&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSnapshotOutput {
    ...
    _ = headers;
    _ = body;
    const result: DeleteSnapshotOutput = .{ .allocator = alloc };
