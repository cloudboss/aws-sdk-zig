...
pub const DescribeCapacityReservationBillingRequestsOutput = struct {
    ...
    pub fn deinit(self: *DescribeCapacityReservationBillingRequestsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeCapacityReservationBillingRequestsInput, options: Options) !DescribeCapacityReservationBillingRequestsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCapacityReservationBillingRequestsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCapacityReservationBillingRequests&Version=2016-11-15");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.role));
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCapacityReservationBillingRequestsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "capacityReservationBillingRequestSet")) {
                    result.capacity_reservation_billing_requests = try serde.deserializeCapacityReservationBillingRequestSet(&reader, alloc, "item");
