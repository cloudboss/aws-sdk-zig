...
pub const DescribeTagsOutput = struct {
    ...
    pub fn deinit(self: *const DescribeTagsOutput) void {
...
pub fn execute(client: *Client, input: DescribeTagsInput, options: Options) !DescribeTagsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTags&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeTagsOutput {
    _ = status;
    var result: DescribeTagsOutput = .{ .allocator = alloc };
