...
fn serializeRequest(allocator: std.mem.Allocator, input: LockSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=LockSnapshot&Version=2016-11-15");
    if (input.cool_off_period) |v| {
        try body_buf.appendSlice(allocator, "&CoolOffPeriod=");
    ...
    if (input.lock_duration) |v| {
        try body_buf.appendSlice(allocator, "&LockDuration=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, std.fmt.allocPrint(allocator, "{d}", .{v}) catch "");
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !LockSnapshotOutput {
    _ = status;
    _ = headers;
