...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSubnetCidrReservationInput, options: CallOptions) !CreateSubnetCidrReservationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSubnetCidrReservationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateSubnetCidrReservation&Version=2016-11-15");
    ...
    try body_buf.appendSlice(allocator, "&ReservationType=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.reservation_type.wireName());
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSubnetCidrReservationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "subnetCidrReservation")) {
                    result.subnet_cidr_reservation = try serde.deserializeSubnetCidrReservation(allocator, &reader);
                } else {
