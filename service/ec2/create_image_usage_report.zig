...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateImageUsageReportInput, options: CallOptions) !CreateImageUsageReportOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateImageUsageReportInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.account_ids) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&AccountId.{d}=", .{n}) catch continue;
