...
pub const EnableRouteServerPropagationOutput = struct {
    /// Information about the enabled route server propagation.
    route_server_propagation: ?RouteServerPropagation = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableRouteServerPropagationInput, options: CallOptions) !EnableRouteServerPropagationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableRouteServerPropagationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=EnableRouteServerPropagation&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableRouteServerPropagationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "routeServerPropagation")) {
                    result.route_server_propagation = try serde.deserializeRouteServerPropagation(allocator, &reader);
                } else {
