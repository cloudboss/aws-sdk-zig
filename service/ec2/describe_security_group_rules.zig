...
pub const DescribeSecurityGroupRulesInput = struct {
    ...
    /// of
    /// items, make another request with the token returned in the output. This
    /// value
...
pub const DescribeSecurityGroupRulesOutput = struct {
    ...
    pub fn deinit(self: *const DescribeSecurityGroupRulesOutput) void {
...
pub fn execute(client: *Client, input: DescribeSecurityGroupRulesInput, options: Options) !DescribeSecurityGroupRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSecurityGroupRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeSecurityGroupRules&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSecurityGroupRulesOutput {
    _ = status;
    _ = headers;
    var result: DescribeSecurityGroupRulesOutput = .{ .allocator = alloc };
