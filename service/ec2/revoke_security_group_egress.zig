...
pub const RevokeSecurityGroupEgressInput = struct {
...
pub const RevokeSecurityGroupEgressOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeSecurityGroupEgressInput, options: CallOptions) !RevokeSecurityGroupEgressOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RevokeSecurityGroupEgressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RevokeSecurityGroupEgress&Version=2016-11-15");
    if (input.cidr_ip) |v| {
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.group_id);
    ...
    if (input.security_group_rule_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&SecurityGroupRuleId.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RevokeSecurityGroupEgressOutput {
    _ = status;
    _ = headers;
    ...
    var result: RevokeSecurityGroupEgressOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "return")) {
                    ...
                    result.revoked_security_group_rules = try serde.deserializeRevokedSecurityGroupRuleList(allocator, &reader, "item");
                } else if (std.mem.eql(u8, e.local, "unknownIpPermissionSet")) {
                    result.unknown_ip_permissions = try serde.deserializeIpPermissionList(allocator, &reader, "item");
                } else {
