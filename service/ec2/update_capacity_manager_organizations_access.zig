...
pub const UpdateCapacityManagerOrganizationsAccessOutput = struct {
    ...
    pub fn deinit(self: *UpdateCapacityManagerOrganizationsAccessOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: UpdateCapacityManagerOrganizationsAccessInput, options: Options) !UpdateCapacityManagerOrganizationsAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCapacityManagerOrganizationsAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateCapacityManagerOrganizationsAccess&Version=2016-11-15");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, if (input.organizations_access) "true" else "false");
