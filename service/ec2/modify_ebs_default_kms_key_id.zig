...
pub const ModifyEbsDefaultKmsKeyIdOutput = struct {
    ...
    pub fn deinit(self: *const ModifyEbsDefaultKmsKeyIdOutput) void {
...
pub fn execute(client: *Client, input: ModifyEbsDefaultKmsKeyIdInput, options: Options) !ModifyEbsDefaultKmsKeyIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyEbsDefaultKmsKeyIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyEbsDefaultKmsKeyId&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&KmsKeyId=");
    try appendUrlEncoded(alloc, &body_buf, input.kms_key_id);
