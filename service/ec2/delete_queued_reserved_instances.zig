...
pub fn execute(client: *Client, input: DeleteQueuedReservedInstancesInput, options: Options) !DeleteQueuedReservedInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteQueuedReservedInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteQueuedReservedInstances&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    for (input.reserved_instances_ids, 0..) |item, idx| {
        ...
        const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ReservedInstancesIds.item.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeleteQueuedReservedInstancesOutput {
    _ = status;
    _ = body;
