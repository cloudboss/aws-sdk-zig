...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIpamPoliciesInput, options: CallOptions) !DescribeIpamPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeIpamPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeIpamPolicies&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeIpamPoliciesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamPolicySet")) {
                    result.ipam_policies = try serde.deserializeIpamPolicySet(allocator, &reader, "item");
