...
pub fn execute(client: *Client, input: CreateTransitGatewayMeteringPolicyInput, options: Options) !CreateTransitGatewayMeteringPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTransitGatewayMeteringPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateTransitGatewayMeteringPolicy&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.middlebox_attachment_ids) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&MiddleboxAttachmentIds.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateTransitGatewayMeteringPolicyOutput {
    _ = status;
    _ = body;
