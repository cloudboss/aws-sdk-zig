...
pub const CreateTransitGatewayMeteringPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTransitGatewayMeteringPolicyInput, options: CallOptions) !CreateTransitGatewayMeteringPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTransitGatewayMeteringPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateTransitGatewayMeteringPolicy&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    if (input.middlebox_attachment_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&MiddleboxAttachmentId.member.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTransitGatewayMeteringPolicyOutput {
    _ = status;
    _ = headers;
