...
pub const CreateRouteServerOutput = struct {
    ...
    pub fn deinit(self: *const CreateRouteServerOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRouteServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.amazon_side_asn}) catch "");
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateRouteServerOutput {
    _ = status;
    _ = body;
    const result: CreateRouteServerOutput = .{ .allocator = alloc };
