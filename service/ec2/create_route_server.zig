...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRouteServerInput, options: CallOptions) !CreateRouteServerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRouteServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateRouteServer&Version=2016-11-15");
    try body_buf.appendSlice(allocator, "&AmazonSideAsn=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, std.fmt.allocPrint(allocator, "{d}", .{input.amazon_side_asn}) catch "");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateRouteServerOutput {
    _ = status;
    _ = headers;
