...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeClientVpnIngressInput, options: CallOptions) !RevokeClientVpnIngressOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RevokeClientVpnIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RevokeClientVpnIngress&Version=2016-11-15");
    ...
    if (input.revoke_all_groups) |v| {
        try body_buf.appendSlice(allocator, "&RevokeAllGroups=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RevokeClientVpnIngressOutput {
    _ = status;
    _ = headers;
