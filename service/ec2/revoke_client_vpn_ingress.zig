...
pub const RevokeClientVpnIngressOutput = struct {
    ...
    pub fn deinit(self: *const RevokeClientVpnIngressOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !RevokeClientVpnIngressOutput {
    _ = status;
    _ = body;
    const result: RevokeClientVpnIngressOutput = .{ .allocator = alloc };
