...
pub const ResetAddressAttributeOutput = struct {
    /// Information about the IP address.
    address: ?AddressAttribute = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetAddressAttributeInput, options: CallOptions) !ResetAddressAttributeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResetAddressAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ResetAddressAttribute&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResetAddressAttributeOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "address")) {
                    result.address = try serde.deserializeAddressAttribute(allocator, &reader);
                } else {
