...
pub const CreateVpcBlockPublicAccessExclusionOutput = struct {
    ...
    pub fn deinit(self: *const CreateVpcBlockPublicAccessExclusionOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: CreateVpcBlockPublicAccessExclusionInput, options: Options) !CreateVpcBlockPublicAccessExclusionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVpcBlockPublicAccessExclusionInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, @tagName(input.internet_gateway_exclusion_mode));
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateVpcBlockPublicAccessExclusionOutput {
    _ = status;
    _ = body;
    const result: CreateVpcBlockPublicAccessExclusionOutput = .{ .allocator = alloc };
