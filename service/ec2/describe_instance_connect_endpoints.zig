...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstanceConnectEndpointsInput, options: CallOptions) !DescribeInstanceConnectEndpointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstanceConnectEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstanceConnectEndpoints&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstanceConnectEndpointsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instanceConnectEndpointSet")) {
                    result.instance_connect_endpoints = try serde.deserializeInstanceConnectEndpointSet(allocator, &reader, "item");
