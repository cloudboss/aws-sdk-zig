...
pub const DescribeInstanceConnectEndpointsInput = struct {
    ...
    instance_connect_endpoint_ids: ?[]const []const u8 = null,
...
pub const DescribeInstanceConnectEndpointsOutput = struct {
    /// Information about the EC2 Instance Connect Endpoints.
    instance_connect_endpoints: ?[]const Ec2InstanceConnectEndpoint = null,
    ...
    pub fn deinit(self: *DescribeInstanceConnectEndpointsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeInstanceConnectEndpointsInput, options: Options) !DescribeInstanceConnectEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceConnectEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInstanceConnectEndpoints&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.instance_connect_endpoint_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&InstanceConnectEndpointIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceConnectEndpointsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeInstanceConnectEndpointsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instanceConnectEndpointSet")) {
                    result.instance_connect_endpoints = try serde.deserializeInstanceConnectEndpointSet(&reader, alloc, "item");
