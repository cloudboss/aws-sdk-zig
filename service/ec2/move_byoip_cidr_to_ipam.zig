...
pub const MoveByoipCidrToIpamOutput = struct {
    ...
    pub fn deinit(self: *const MoveByoipCidrToIpamOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: MoveByoipCidrToIpamInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=MoveByoipCidrToIpam&Version=2016-11-15");
    ...
    try body_buf.appendSlice(alloc, "&IpamPoolOwner=");
    try appendUrlEncoded(alloc, &body_buf, input.ipam_pool_owner);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MoveByoipCidrToIpamOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: MoveByoipCidrToIpamOutput = .{ .allocator = alloc };
