...
pub const DescribeInstanceCreditSpecificationsInput = struct {
...
pub const DescribeInstanceCreditSpecificationsOutput = struct {
    /// Information about the credit option for CPU usage of an instance.
    instance_credit_specifications: ?[]const InstanceCreditSpecification = null,
    ...
    pub fn deinit(self: *DescribeInstanceCreditSpecificationsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeInstanceCreditSpecificationsInput, options: Options) !DescribeInstanceCreditSpecificationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInstanceCreditSpecificationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInstanceCreditSpecifications&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInstanceCreditSpecificationsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeInstanceCreditSpecificationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instanceCreditSpecificationSet")) {
                    result.instance_credit_specifications = try serde.deserializeInstanceCreditSpecificationList(&reader, alloc, "item");
