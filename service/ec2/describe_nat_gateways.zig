...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNatGatewaysInput, options: CallOptions) !DescribeNatGatewaysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeNatGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeNatGateways&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.nat_gateway_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&NatGatewayId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeNatGatewaysOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "natGatewaySet")) {
                    result.nat_gateways = try serde.deserializeNatGatewayList(allocator, &reader, "item");
