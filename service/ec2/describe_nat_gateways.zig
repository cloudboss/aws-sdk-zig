...
pub const DescribeNatGatewaysOutput = struct {
    ...
    pub fn deinit(self: *const DescribeNatGatewaysOutput) void {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNatGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.nat_gateway_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&NatGatewayIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeNatGatewaysOutput {
    _ = status;
    var result: DescribeNatGatewaysOutput = .{ .allocator = alloc };
