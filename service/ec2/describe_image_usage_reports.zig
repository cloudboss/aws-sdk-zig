...
pub const DescribeImageUsageReportsOutput = struct {
    /// The image usage reports.
    image_usage_reports: ?[]const ImageUsageReport = null,
    ...
    pub fn deinit(self: *DescribeImageUsageReportsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeImageUsageReportsInput, options: Options) !DescribeImageUsageReportsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeImageUsageReportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeImageUsageReports&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.report_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ReportIds.member.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try appendUrlEncoded(alloc, &body_buf, item);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeImageUsageReportsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeImageUsageReportsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "imageUsageReportSet")) {
                    result.image_usage_reports = try serde.deserializeImageUsageReportList(&reader, alloc, "item");
