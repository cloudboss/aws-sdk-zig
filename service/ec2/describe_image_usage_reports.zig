...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageUsageReportsInput, options: CallOptions) !DescribeImageUsageReportsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeImageUsageReportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeImageUsageReports&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeImageUsageReportsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "imageUsageReportSet")) {
                    result.image_usage_reports = try serde.deserializeImageUsageReportList(allocator, &reader, "item");
