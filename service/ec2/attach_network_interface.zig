...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.device_index}) catch "");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !AttachNetworkInterfaceOutput {
    _ = status;
