...
pub fn execute(client: *Client, input: DescribeRegionsInput, options: Options) !DescribeRegionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRegionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeRegions&Version=2016-11-15");
    ...
    if (input.region_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&RegionNames.RegionName.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeRegionsOutput {
    _ = status;
    _ = body;
