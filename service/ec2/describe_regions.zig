...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRegionsInput, options: Options) !DescribeRegionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRegionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeRegions&Version=2016-11-15");
    ...
    if (input.region_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&RegionName.RegionName.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "regionInfo")) {
                    result.regions = try serde.deserializeRegionList(allocator, &reader, "item");
                } else {
