...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteIpamPolicyInput, options: CallOptions) !DeleteIpamPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteIpamPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteIpamPolicy&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteIpamPolicyOutput {
    _ = status;
    _ = headers;
