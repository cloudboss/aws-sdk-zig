...
pub const DeleteIpamPolicyInput = struct {
...
pub const DeleteIpamPolicyOutput = struct {
    ...
    pub fn deinit(self: *const DeleteIpamPolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeleteIpamPolicyInput, options: Options) !DeleteIpamPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteIpamPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteIpamPolicy&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteIpamPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeleteIpamPolicyOutput = .{ .allocator = alloc };
