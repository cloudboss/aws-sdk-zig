...
pub fn execute(client: *Client, input: DisableSnapshotBlockPublicAccessInput, options: Options) !DisableSnapshotBlockPublicAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableSnapshotBlockPublicAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisableSnapshotBlockPublicAccess&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DisableSnapshotBlockPublicAccessOutput {
    _ = status;
    _ = body;
