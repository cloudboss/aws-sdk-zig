...
pub const EnableImageBlockPublicAccessInput = struct {
    ...
    /// account level in the specified Region. This will block any attempt to
    /// publicly share your AMIs
    /// in the specified Region.
    image_block_public_access_state: ImageBlockPublicAccessEnabledState,
};

pub const EnableImageBlockPublicAccessOutput = struct {
    /// Returns `block-new-sharing` if the request succeeds; otherwise, it returns
    /// an
    /// error.
    image_block_public_access_state: ?ImageBlockPublicAccessEnabledState = null,

    allocator: std.mem.Allocator,

    pub fn deinit(self: *const EnableImageBlockPublicAccessOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: EnableImageBlockPublicAccessInput, options: Options) !EnableImageBlockPublicAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableImageBlockPublicAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableImageBlockPublicAccess&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&ImageBlockPublicAccessState=");
    try appendUrlEncoded(alloc, &body_buf, @tagName(input.image_block_public_access_state));
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableImageBlockPublicAccessOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: EnableImageBlockPublicAccessOutput = .{ .allocator = alloc };
