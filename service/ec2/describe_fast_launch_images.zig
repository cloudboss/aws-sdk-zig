...
pub const DescribeFastLaunchImagesOutput = struct {
    ...
    pub fn deinit(self: *DescribeFastLaunchImagesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeFastLaunchImagesInput, options: Options) !DescribeFastLaunchImagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFastLaunchImagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeFastLaunchImages&Version=2016-11-15");
    if (input.dry_run) |v| {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "fastLaunchImageSet")) {
                    result.fast_launch_images = try serde.deserializeDescribeFastLaunchImagesSuccessSet(&reader, alloc, "item");
