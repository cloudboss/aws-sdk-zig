...
pub const DescribeClassicLinkInstancesOutput = struct {
    /// Information about one or more linked EC2-Classic instances.
    instances: ?[]const ClassicLinkInstance = null,
    ...
    pub fn deinit(self: *DescribeClassicLinkInstancesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeClassicLinkInstancesInput, options: Options) !DescribeClassicLinkInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClassicLinkInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeClassicLinkInstances&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClassicLinkInstancesOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeClassicLinkInstancesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instancesSet")) {
                    result.instances = try serde.deserializeClassicLinkInstanceList(&reader, alloc, "item");
