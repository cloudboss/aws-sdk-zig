...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClassicLinkInstancesInput, options: CallOptions) !DescribeClassicLinkInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeClassicLinkInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeClassicLinkInstances&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeClassicLinkInstancesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instancesSet")) {
                    result.instances = try serde.deserializeClassicLinkInstanceList(allocator, &reader, "item");
