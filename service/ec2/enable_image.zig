...
pub const EnableImageInput = struct {
...
pub const EnableImageOutput = struct {
    /// Returns `true` if the request succeeds; otherwise, it returns an error.
    @"return": ?bool = null,

    allocator: std.mem.Allocator,

    pub fn deinit(self: *const EnableImageOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: EnableImageInput, options: Options) !EnableImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableImage&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableImageOutput {
    _ = status;
    _ = headers;
    var result: EnableImageOutput = .{ .allocator = alloc };
