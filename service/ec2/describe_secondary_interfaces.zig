...
pub const DescribeSecondaryInterfacesInput = struct {
    ...
    ///
    /// * `attachment.status` - The attachment status (`attaching` | `attached` |
    ///   `detaching` | `detached`).
    ///
    ...
    secondary_interface_ids: ?[]const []const u8 = null,
};

pub const DescribeSecondaryInterfacesOutput = struct {
    /// The token to use to retrieve the next page of results. This value is `null`
    /// when there are no more results to return.
    next_token: ?[]const u8 = null,

    /// Information about the secondary interfaces.
    secondary_interfaces: ?[]const SecondaryInterface = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSecondaryInterfacesInput, options: CallOptions) !DescribeSecondaryInterfacesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSecondaryInterfacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeSecondaryInterfaces&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.secondary_interface_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&SecondaryInterfaceId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSecondaryInterfacesOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeSecondaryInterfacesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.secondary_interfaces = try serde.deserializeSecondaryInterfaceList(allocator, &reader, "item");
                } else {
