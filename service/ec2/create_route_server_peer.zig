...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRouteServerPeerInput, options: CallOptions) !CreateRouteServerPeerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRouteServerPeerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateRouteServerPeer&Version=2016-11-15");
    try body_buf.appendSlice(allocator, "&BgpOptions.PeerAsn=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateRouteServerPeerOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "routeServerPeer")) {
                    result.route_server_peer = try serde.deserializeRouteServerPeer(allocator, &reader);
                } else {
