...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTransitGatewayMulticastDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateTransitGatewayMulticastDomain&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.options) |v| {
        if (v.auto_accept_shared_associations) |sv| {
            try body_buf.appendSlice(alloc, "&Options.AutoAcceptSharedAssociations=");
            try appendUrlEncoded(alloc, &body_buf, @tagName(sv));
        }
        if (v.igmpv_2_support) |sv| {
            try body_buf.appendSlice(alloc, "&Options.Igmpv2Support=");
            try appendUrlEncoded(alloc, &body_buf, @tagName(sv));
        }
        if (v.static_sources_support) |sv| {
            try body_buf.appendSlice(alloc, "&Options.StaticSourcesSupport=");
            try appendUrlEncoded(alloc, &body_buf, @tagName(sv));
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateTransitGatewayMulticastDomainOutput {
    _ = status;
    _ = body;
