...
pub const ImportKeyPairInput = struct {
...
pub const ImportKeyPairOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportKeyPairInput, options: CallOptions) !ImportKeyPairOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ImportKeyPairInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ImportKeyPair&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.public_key_material);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ImportKeyPairOutput {
    _ = status;
    _ = headers;
