...
pub const DescribeVpcsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpcsInput, options: CallOptions) !DescribeVpcsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpcsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVpcs&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVpcsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeVpcsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "vpcSet")) {
                    result.vpcs = try serde.deserializeVpcList(allocator, &reader, "item");
                } else {
