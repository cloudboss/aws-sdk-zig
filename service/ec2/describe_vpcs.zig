...
pub const DescribeVpcsOutput = struct {
    ...
    pub fn deinit(self: *DescribeVpcsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeVpcsInput, options: Options) !DescribeVpcsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpcsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeVpcs&Version=2016-11-15");
    if (input.dry_run) |v| {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.vpcs = try serde.deserializeVpcList(&reader, alloc, "item");
                } else {
