...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrafficMirrorFilterRulesInput, options: CallOptions) !DescribeTrafficMirrorFilterRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTrafficMirrorFilterRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTrafficMirrorFilterRules&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.traffic_mirror_filter_rule_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&TrafficMirrorFilterRuleId.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTrafficMirrorFilterRulesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.traffic_mirror_filter_rules = try serde.deserializeTrafficMirrorFilterRuleSet(allocator, &reader, "item");
                } else {
