...
pub const CreateIpamResourceDiscoveryOutput = struct {
    /// An IPAM resource discovery.
    ipam_resource_discovery: ?IpamResourceDiscovery = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIpamResourceDiscoveryInput, options: CallOptions) !CreateIpamResourceDiscoveryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateIpamResourceDiscoveryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateIpamResourceDiscovery&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateIpamResourceDiscoveryOutput {
    _ = status;
    _ = headers;
