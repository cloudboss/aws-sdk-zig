...
pub const GetIpamResourceCidrsOutput = struct {
    /// The resource CIDRs.
    ipam_resource_cidrs: ?[]const IpamResourceCidr = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIpamResourceCidrsInput, options: CallOptions) !GetIpamResourceCidrsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIpamResourceCidrsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetIpamResourceCidrs&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.resource_tag) |v| {
        if (v.key) |sv| {
            try body_buf.appendSlice(allocator, "&ResourceTag.Key=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, sv);
        }
        if (v.value) |sv| {
            try body_buf.appendSlice(allocator, "&ResourceTag.Value=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, sv);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIpamResourceCidrsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamResourceCidrSet")) {
                    result.ipam_resource_cidrs = try serde.deserializeIpamResourceCidrSet(allocator, &reader, "item");
