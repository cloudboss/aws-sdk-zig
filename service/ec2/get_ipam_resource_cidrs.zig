...
pub fn execute(client: *Client, input: GetIpamResourceCidrsInput, options: Options) !GetIpamResourceCidrsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIpamResourceCidrsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetIpamResourceCidrs&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.resource_tag) |v| {
        if (v.key) |sv| {
            try body_buf.appendSlice(alloc, "&ResourceTag.Key=");
            try appendUrlEncoded(alloc, &body_buf, sv);
        }
        if (v.value) |sv| {
            try body_buf.appendSlice(alloc, "&ResourceTag.Value=");
            try appendUrlEncoded(alloc, &body_buf, sv);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetIpamResourceCidrsOutput {
    _ = status;
