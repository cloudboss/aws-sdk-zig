...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInstanceTypesFromInstanceRequirementsInput, options: Options) !GetInstanceTypesFromInstanceRequirementsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInstanceTypesFromInstanceRequirementsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.instance_requirements.memory_mi_b.min}) catch "");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.instance_requirements.v_cpu_count.min}) catch "");
