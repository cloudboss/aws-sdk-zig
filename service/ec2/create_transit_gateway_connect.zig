...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTransitGatewayConnectInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.options.protocol));
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.transport_transit_gateway_attachment_id);
