...
pub const CreateTransitGatewayConnectOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTransitGatewayConnectInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateTransitGatewayConnect&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    try body_buf.appendSlice(allocator, "&TransportTransitGatewayAttachmentId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.transport_transit_gateway_attachment_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTransitGatewayConnectOutput {
    _ = status;
    _ = headers;
