...
pub fn execute(client: *Client, input: CreateTransitGatewayConnectInput, options: Options) !CreateTransitGatewayConnectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTransitGatewayConnectInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateTransitGatewayConnect&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&Options.Protocol=");
    try appendUrlEncoded(alloc, &body_buf, @tagName(input.options.protocol));
    ...
    try body_buf.appendSlice(alloc, "&TransportTransitGatewayAttachmentId=");
    try appendUrlEncoded(alloc, &body_buf, input.transport_transit_gateway_attachment_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateTransitGatewayConnectOutput {
    _ = status;
    _ = body;
