...
pub fn execute(client: *Client, input: DescribeNetworkInsightsPathsInput, options: Options) !DescribeNetworkInsightsPathsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNetworkInsightsPathsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeNetworkInsightsPaths&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.network_insights_path_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&NetworkInsightsPathIds.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeNetworkInsightsPathsOutput {
    _ = status;
