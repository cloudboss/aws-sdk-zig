...
pub const DescribeNetworkInsightsPathsInput = struct {
    ...
    network_insights_path_ids: ?[]const []const u8 = null,

    /// The token for the next page of results.
    next_token: ?[]const u8 = null,
};

pub const DescribeNetworkInsightsPathsOutput = struct {
    /// Information about the paths.
    network_insights_paths: ?[]const NetworkInsightsPath = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNetworkInsightsPathsInput, options: CallOptions) !DescribeNetworkInsightsPathsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeNetworkInsightsPathsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeNetworkInsightsPaths&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.network_insights_path_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&NetworkInsightsPathId.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeNetworkInsightsPathsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "networkInsightsPathSet")) {
                    result.network_insights_paths = try serde.deserializeNetworkInsightsPathList(allocator, &reader, "item");
