...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateClientVpnRouteInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.target_vpc_subnet_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateClientVpnRouteOutput {
    _ = status;
    _ = body;
