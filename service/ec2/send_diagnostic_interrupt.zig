...
pub const SendDiagnosticInterruptOutput = struct {
    ...
    pub fn deinit(self: *const SendDiagnosticInterruptOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !SendDiagnosticInterruptOutput {
    _ = status;
    _ = body;
    const result: SendDiagnosticInterruptOutput = .{ .allocator = alloc };
