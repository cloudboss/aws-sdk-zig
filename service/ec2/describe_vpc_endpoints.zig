...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpcEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVpcEndpoints&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.vpc_endpoint_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&VpcEndpointId.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                ...
                } else if (std.mem.eql(u8, e.local, "vpcEndpointSet")) {
                    result.vpc_endpoints = try serde.deserializeVpcEndpointSet(allocator, &reader, "item");
                } else {
