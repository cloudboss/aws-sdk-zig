...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIpamInput, options: CallOptions) !CreateIpamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateIpamInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateIpam&Version=2016-11-15");
    ...
    if (input.operating_regions) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
                ...
                const field_prefix = std.fmt.bufPrint(&prefix_buf, "&OperatingRegion.member.{d}.RegionName=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateIpamOutput {
    _ = status;
    _ = headers;
