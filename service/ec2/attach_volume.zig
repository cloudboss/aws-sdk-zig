...
pub const AttachVolumeOutput = struct {
    ...
    pub fn deinit(self: *const AttachVolumeOutput) void {
...
pub fn execute(client: *Client, input: AttachVolumeInput, options: Options) !AttachVolumeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AttachVolumeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AttachVolume&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&Device=");
    try appendUrlEncoded(alloc, &body_buf, input.device);
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.ebs_card_index) |v| {
        try body_buf.appendSlice(alloc, "&EbsCardIndex=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AttachVolumeOutput {
    _ = status;
    _ = headers;
    var result: AttachVolumeOutput = .{ .allocator = alloc };
