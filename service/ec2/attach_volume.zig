...
pub const AttachVolumeOutput = @import("volume_attachment.zig").VolumeAttachment;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AttachVolumeInput, options: CallOptions) !AttachVolumeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AttachVolumeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AttachVolume&Version=2016-11-15");
    try body_buf.appendSlice(allocator, "&Device=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.device);
    if (input.dry_run) |v| {
        try body_buf.appendSlice(allocator, "&DryRun=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    if (input.ebs_card_index) |v| {
        try body_buf.appendSlice(allocator, "&EbsCardIndex=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AttachVolumeOutput {
    _ = status;
    _ = headers;
    ...
    var result: AttachVolumeOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "associatedResource")) {
                    result.associated_resource = try allocator.dupe(u8, try reader.readElementText());
                    ...
                    result.device = try allocator.dupe(u8, try reader.readElementText());
