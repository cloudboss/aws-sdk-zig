...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpcEndpointServicePermissionsInput, options: CallOptions) !DescribeVpcEndpointServicePermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpcEndpointServicePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVpcEndpointServicePermissions&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVpcEndpointServicePermissionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "allowedPrincipals")) {
                    result.allowed_principals = try serde.deserializeAllowedPrincipalSet(allocator, &reader, "item");
