...
pub fn execute(client: *Client, input: GetLaunchTemplateDataInput, options: Options) !GetLaunchTemplateDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLaunchTemplateDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetLaunchTemplateData&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetLaunchTemplateDataOutput {
    _ = status;
    _ = body;
