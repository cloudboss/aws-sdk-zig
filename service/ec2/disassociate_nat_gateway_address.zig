...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateNatGatewayAddressInput, options: CallOptions) !DisassociateNatGatewayAddressOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateNatGatewayAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DisassociateNatGatewayAddress&Version=2016-11-15");
    for (input.association_ids, 0..) |item, idx| {
        const n = idx + 1;
        var prefix_buf: [256]u8 = undefined;
        const field_prefix = std.fmt.bufPrint(&prefix_buf, "&AssociationId.item.{d}=", .{n}) catch continue;
    ...
    if (input.max_drain_duration_seconds) |v| {
        try body_buf.appendSlice(allocator, "&MaxDrainDurationSeconds=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateNatGatewayAddressOutput {
    _ = status;
    _ = headers;
