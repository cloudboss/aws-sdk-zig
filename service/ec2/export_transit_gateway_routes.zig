...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportTransitGatewayRoutesInput, options: CallOptions) !ExportTransitGatewayRoutesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExportTransitGatewayRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ExportTransitGatewayRoutes&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExportTransitGatewayRoutesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "s3Location")) {
                    result.s3_location = try allocator.dupe(u8, try reader.readElementText());
                } else {
