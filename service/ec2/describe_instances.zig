...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInstances&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                ...
                } else if (std.mem.eql(u8, e.local, "reservationSet")) {
                    result.reservations = try serde.deserializeReservationList(allocator, &reader, "item");
                } else {
