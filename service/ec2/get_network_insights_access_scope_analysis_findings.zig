...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNetworkInsightsAccessScopeAnalysisFindingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetNetworkInsightsAccessScopeAnalysisFindings&Version=2016-11-15");
    if (input.dry_run) |v| {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "analysisFindingSet")) {
                    result.analysis_findings = try serde.deserializeAccessScopeAnalysisFindingList(&reader, alloc, "item");
