...
pub const DeprovisionIpamByoasnOutput = struct {
    ...
    pub fn deinit(self: *const DeprovisionIpamByoasnOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DeprovisionIpamByoasnInput, options: Options) !DeprovisionIpamByoasnOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeprovisionIpamByoasnInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeprovisionIpamByoasn&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&Asn=");
    try appendUrlEncoded(alloc, &body_buf, input.asn);
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeprovisionIpamByoasnOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: DeprovisionIpamByoasnOutput = .{ .allocator = alloc };
