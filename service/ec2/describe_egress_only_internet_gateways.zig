...
pub const DescribeEgressOnlyInternetGatewaysInput = struct {
    ...
    egress_only_internet_gateway_ids: ?[]const []const u8 = null,
...
pub const DescribeEgressOnlyInternetGatewaysOutput = struct {
    /// Information about the egress-only internet gateways.
    egress_only_internet_gateways: ?[]const EgressOnlyInternetGateway = null,
    ...
    pub fn deinit(self: *DescribeEgressOnlyInternetGatewaysOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeEgressOnlyInternetGatewaysInput, options: Options) !DescribeEgressOnlyInternetGatewaysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEgressOnlyInternetGatewaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEgressOnlyInternetGateways&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.egress_only_internet_gateway_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&EgressOnlyInternetGatewayIds.item.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEgressOnlyInternetGatewaysOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeEgressOnlyInternetGatewaysOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "egressOnlyInternetGatewaySet")) {
                    result.egress_only_internet_gateways = try serde.deserializeEgressOnlyInternetGatewayList(&reader, alloc, "item");
