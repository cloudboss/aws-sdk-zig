...
pub const DeleteNetworkInsightsAnalysisOutput = struct {
    ...
    pub fn deinit(self: *DeleteNetworkInsightsAnalysisOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DeleteNetworkInsightsAnalysisInput, options: Options) !DeleteNetworkInsightsAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteNetworkInsightsAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteNetworkInsightsAnalysis&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&NetworkInsightsAnalysisId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.network_insights_analysis_id);
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "networkInsightsAnalysisId")) {
                    result.network_insights_analysis_id = try alloc.dupe(u8, try reader.readElementText());
                } else {
