...
pub const DescribeSecondarySubnetsInput = struct {
    ...
    /// The filters.
    ///
    /// * `ipv4-cidr-block-association.association-id` - The association ID for an
    ///   IPv4 CIDR block associated with the secondary subnet.
    ///
    /// * `ipv4-cidr-block-association.cidr-block` - An IPv4 CIDR block associated
    ///   with the secondary subnet.
    ///
    ...
    secondary_subnet_ids: ?[]const []const u8 = null,
};

pub const DescribeSecondarySubnetsOutput = struct {
    /// The token to use to retrieve the next page of results. This value is `null`
    /// when there are no more results to return.
    next_token: ?[]const u8 = null,

    /// Information about the secondary subnets.
    secondary_subnets: ?[]const SecondarySubnet = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSecondarySubnetsInput, options: CallOptions) !DescribeSecondarySubnetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSecondarySubnetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeSecondarySubnets&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.secondary_subnet_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&SecondarySubnetId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSecondarySubnetsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeSecondarySubnetsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "secondarySubnetSet")) {
                    result.secondary_subnets = try serde.deserializeSecondarySubnetList(allocator, &reader, "item");
                } else {
