...
pub const CreateTransitGatewayPeeringAttachmentInput = struct {
    ...
    options: ?CreateTransitGatewayPeeringAttachmentRequestOptions = null,

    /// The ID of the Amazon Web Services account that owns the peer transit
    /// gateway.
    peer_account_id: []const u8,

    /// The Region where the peer transit gateway is located.
    peer_region: []const u8,
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTransitGatewayPeeringAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateTransitGatewayPeeringAttachment&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.peer_account_id);
    try body_buf.appendSlice(allocator, "&PeerRegion=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.peer_region);
    try body_buf.appendSlice(allocator, "&PeerTransitGatewayId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.peer_transit_gateway_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTransitGatewayPeeringAttachmentOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "transitGatewayPeeringAttachment")) {
                    result.transit_gateway_peering_attachment = try serde.deserializeTransitGatewayPeeringAttachment(allocator, &reader);
                } else {
