...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTransitGatewayPeeringAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateTransitGatewayPeeringAttachment&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.options) |v| {
        if (v.dynamic_routing) |sv| {
            try body_buf.appendSlice(alloc, "&Options.DynamicRouting=");
            try appendUrlEncoded(alloc, &body_buf, @tagName(sv));
        }
    }
    try body_buf.appendSlice(alloc, "&PeerAccountId=");
    try appendUrlEncoded(alloc, &body_buf, input.peer_account_id);
    try body_buf.appendSlice(alloc, "&PeerRegion=");
    try appendUrlEncoded(alloc, &body_buf, input.peer_region);
    try body_buf.appendSlice(alloc, "&PeerTransitGatewayId=");
    try appendUrlEncoded(alloc, &body_buf, input.peer_transit_gateway_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateTransitGatewayPeeringAttachmentOutput {
    _ = status;
    _ = body;
