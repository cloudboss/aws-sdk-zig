...
pub const DescribeVolumesOutput = struct {
    ...
    pub fn deinit(self: *DescribeVolumesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeVolumesInput, options: Options) !DescribeVolumesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVolumesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeVolumes&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVolumesOutput {
    ...
    var result: DescribeVolumesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
