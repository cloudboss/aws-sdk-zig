...
pub const DescribeLockedSnapshotsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLockedSnapshotsInput, options: CallOptions) !DescribeLockedSnapshotsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeLockedSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeLockedSnapshots&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.snapshot_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&SnapshotId.SnapshotId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeLockedSnapshotsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.snapshots = try serde.deserializeLockedSnapshotsInfoList(allocator, &reader, "item");
                } else {
