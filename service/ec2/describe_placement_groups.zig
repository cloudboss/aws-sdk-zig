...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePlacementGroupsInput, options: CallOptions) !DescribePlacementGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribePlacementGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribePlacementGroups&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribePlacementGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "placementGroupSet")) {
                    result.placement_groups = try serde.deserializePlacementGroupList(allocator, &reader, "item");
                } else {
