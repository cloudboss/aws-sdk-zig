...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableAllowedImagesSettingsInput, options: CallOptions) !EnableAllowedImagesSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableAllowedImagesSettingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableAllowedImagesSettingsOutput {
    _ = status;
    _ = headers;
    _ = allocator;
    ...
    var result: EnableAllowedImagesSettingsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
