...
pub fn execute(client: *Client, input: DeleteVpcEncryptionControlInput, options: Options) !DeleteVpcEncryptionControlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVpcEncryptionControlInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteVpcEncryptionControl&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeleteVpcEncryptionControlOutput {
    _ = status;
    _ = body;
