...
pub const GetTransitGatewayAttachmentPropagationsOutput = struct {
    /// The token to use to retrieve the next page of results. This value is `null`
    /// when there are no more results to return.
    next_token: ?[]const u8 = null,

    /// Information about the propagation route tables.
    transit_gateway_attachment_propagations: ?[]const TransitGatewayAttachmentPropagation = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *GetTransitGatewayAttachmentPropagationsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetTransitGatewayAttachmentPropagationsInput, options: Options) !GetTransitGatewayAttachmentPropagationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTransitGatewayAttachmentPropagationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetTransitGatewayAttachmentPropagations&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTransitGatewayAttachmentPropagationsOutput {
    _ = status;
    _ = headers;
    ...
    var result: GetTransitGatewayAttachmentPropagationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    result.next_token = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "transitGatewayAttachmentPropagations")) {
                    result.transit_gateway_attachment_propagations = try serde.deserializeTransitGatewayAttachmentPropagationList(&reader, alloc, "item");
                } else {
