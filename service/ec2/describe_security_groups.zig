...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSecurityGroupsInput, options: CallOptions) !DescribeSecurityGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSecurityGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeSecurityGroups&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.group_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&GroupName.GroupName.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSecurityGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.security_groups = try serde.deserializeSecurityGroupList(allocator, &reader, "item");
                } else {
