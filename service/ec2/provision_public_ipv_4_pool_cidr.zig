...
fn serializeRequest(allocator: std.mem.Allocator, input: ProvisionPublicIpv4PoolCidrInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ProvisionPublicIpv4PoolCidr&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, std.fmt.allocPrint(allocator, "{d}", .{input.netmask_length}) catch "");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ProvisionPublicIpv4PoolCidrOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "poolAddressRange")) {
                    result.pool_address_range = try serde.deserializePublicIpv4PoolRange(allocator, &reader);
