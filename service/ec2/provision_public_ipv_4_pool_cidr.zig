...
fn serializeRequest(alloc: std.mem.Allocator, input: ProvisionPublicIpv4PoolCidrInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.netmask_length}) catch "");
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ProvisionPublicIpv4PoolCidrOutput {
    _ = status;
