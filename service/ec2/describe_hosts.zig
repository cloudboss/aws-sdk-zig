...
pub const DescribeHostsOutput = struct {
    ...
    pub fn deinit(self: *DescribeHostsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeHostsInput, options: Options) !DescribeHostsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHostsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeHosts&Version=2016-11-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHostsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "hostSet")) {
                    result.hosts = try serde.deserializeHostList(&reader, alloc, "item");
