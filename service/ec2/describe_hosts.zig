...
pub const DescribeHostsInput = struct {
    /// The filters.
    ///
...
pub const DescribeHostsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHostsInput, options: CallOptions) !DescribeHostsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHostsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeHosts&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHostsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeHostsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "hostSet")) {
                    result.hosts = try serde.deserializeHostList(allocator, &reader, "item");
