...
pub fn execute(client: *Client, input: DisassociateIpamResourceDiscoveryInput, options: Options) !DisassociateIpamResourceDiscoveryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateIpamResourceDiscoveryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisassociateIpamResourceDiscovery&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&IpamResourceDiscoveryAssociationId=");
    try appendUrlEncoded(alloc, &body_buf, input.ipam_resource_discovery_association_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DisassociateIpamResourceDiscoveryOutput {
    _ = status;
    _ = body;
