...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSnapshotBlockPublicAccessStateInput, options: CallOptions) !GetSnapshotBlockPublicAccessStateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSnapshotBlockPublicAccessStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetSnapshotBlockPublicAccessState&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSnapshotBlockPublicAccessStateOutput {
    _ = status;
    _ = headers;
    _ = allocator;
