...
pub const GetSnapshotBlockPublicAccessStateOutput = struct {
    ...
    /// the account can't request new public sharing. Additionally, snapshots that
    /// were already
    ...
    pub fn deinit(self: *GetSnapshotBlockPublicAccessStateOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetSnapshotBlockPublicAccessStateInput, options: Options) !GetSnapshotBlockPublicAccessStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSnapshotBlockPublicAccessStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetSnapshotBlockPublicAccessState&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSnapshotBlockPublicAccessStateOutput {
    _ = status;
    _ = headers;
    _ = alloc;
    ...
    var result: GetSnapshotBlockPublicAccessStateOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "managedBy")) {
