...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVpnConcentratorInput, options: CallOptions) !CreateVpnConcentratorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVpnConcentratorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateVpnConcentrator&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVpnConcentratorOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "vpnConcentrator")) {
                    result.vpn_concentrator = try serde.deserializeVpnConcentrator(allocator, &reader);
                } else {
