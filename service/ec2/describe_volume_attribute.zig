...
pub const DescribeVolumeAttributeOutput = struct {
    /// The state of `autoEnableIO` attribute.
    auto_enable_io: ?AttributeBooleanValue = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVolumeAttributeInput, options: CallOptions) !DescribeVolumeAttributeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVolumeAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVolumeAttribute&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVolumeAttributeOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "autoEnableIO")) {
                    result.auto_enable_io = try serde.deserializeAttributeBooleanValue(allocator, &reader);
