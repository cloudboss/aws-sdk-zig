...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyNetworkInterfaceAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.associated_subnet_ids) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&AssociatedSubnetIds.AssociatedSubnetId.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyNetworkInterfaceAttributeOutput {
    _ = status;
    _ = body;
