...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyNetworkInterfaceAttributeInput, options: CallOptions) !ModifyNetworkInterfaceAttributeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyNetworkInterfaceAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyNetworkInterfaceAttribute&Version=2016-11-15");
    if (input.associated_subnet_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&AssociatedSubnetId.{d}=", .{n}) catch continue;
    ...
    if (input.attachment) |v| {
        if (v.attachment_id) |sv| {
            try body_buf.appendSlice(allocator, "&Attachment.AttachmentId=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, sv);
        }
        if (v.default_ena_queue_count) |sv| {
            try body_buf.appendSlice(allocator, "&Attachment.DefaultEnaQueueCount=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, if (sv) "true" else "false");
        }
        if (v.delete_on_termination) |sv| {
            try body_buf.appendSlice(allocator, "&Attachment.DeleteOnTermination=");
            try aws.url.appendUrlEncoded(allocator, &body_buf, if (sv) "true" else "false");
        }
        if (v.ena_queue_count) |sv| {
            try body_buf.appendSlice(allocator, "&Attachment.EnaQueueCount=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyNetworkInterfaceAttributeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
