...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHostReservationsInput, options: CallOptions) !DescribeHostReservationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHostReservationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeHostReservations&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHostReservationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "hostReservationSet")) {
                    result.host_reservation_set = try serde.deserializeHostReservationSet(allocator, &reader, "item");
