...
pub const DescribeHostReservationsInput = struct {
    ...
    host_reservation_id_set: ?[]const []const u8 = null,
...
pub const DescribeHostReservationsOutput = struct {
    /// Details about the reservation's configuration.
    host_reservation_set: ?[]const HostReservation = null,
    ...
    pub fn deinit(self: *DescribeHostReservationsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeHostReservationsInput, options: Options) !DescribeHostReservationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHostReservationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeHostReservations&Version=2016-11-15");
    ...
    if (input.host_reservation_id_set) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&HostReservationIdSet.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHostReservationsOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeHostReservationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "hostReservationSet")) {
                    result.host_reservation_set = try serde.deserializeHostReservationSet(&reader, alloc, "item");
