...
pub fn execute(client: *Client, input: GetEbsEncryptionByDefaultInput, options: Options) !GetEbsEncryptionByDefaultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEbsEncryptionByDefaultInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetEbsEncryptionByDefault&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetEbsEncryptionByDefaultOutput {
    _ = status;
