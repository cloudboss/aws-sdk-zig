...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIpamDiscoveredResourceCidrsInput, options: CallOptions) !GetIpamDiscoveredResourceCidrsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIpamDiscoveredResourceCidrsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetIpamDiscoveredResourceCidrs&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIpamDiscoveredResourceCidrsOutput {
    _ = status;
    _ = headers;
    ...
    var result: GetIpamDiscoveredResourceCidrsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamDiscoveredResourceCidrSet")) {
                    result.ipam_discovered_resource_cidrs = try serde.deserializeIpamDiscoveredResourceCidrSet(allocator, &reader, "item");
