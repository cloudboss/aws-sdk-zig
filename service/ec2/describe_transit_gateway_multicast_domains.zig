...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTransitGatewayMulticastDomainsInput, options: CallOptions) !DescribeTransitGatewayMulticastDomainsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTransitGatewayMulticastDomainsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTransitGatewayMulticastDomains&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.transit_gateway_multicast_domain_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&TransitGatewayMulticastDomainIds.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTransitGatewayMulticastDomainsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.transit_gateway_multicast_domains = try serde.deserializeTransitGatewayMulticastDomainList(allocator, &reader, "item");
                } else {
