...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartInstancesInput, options: CallOptions) !StartInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=StartInstances&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartInstancesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instancesSet")) {
                    result.starting_instances = try serde.deserializeInstanceStateChangeList(allocator, &reader, "item");
                } else {
