...
pub const ModifyIpamPrefixListResolverTargetInput = struct {
    ...
    ipam_prefix_list_resolver_target_id: []const u8,
...
pub const ModifyIpamPrefixListResolverTargetOutput = struct {
    ...
    pub fn deinit(self: *ModifyIpamPrefixListResolverTargetOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: ModifyIpamPrefixListResolverTargetInput, options: Options) !ModifyIpamPrefixListResolverTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyIpamPrefixListResolverTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyIpamPrefixListResolverTarget&Version=2016-11-15");
    if (input.client_token) |v| {
        try body_buf.appendSlice(alloc, "&ClientToken=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.desired_version) |v| {
        try body_buf.appendSlice(alloc, "&DesiredVersion=");
    ...
    try body_buf.appendSlice(alloc, "&IpamPrefixListResolverTargetId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.ipam_prefix_list_resolver_target_id);
    if (input.track_latest_version) |v| {
        try body_buf.appendSlice(alloc, "&TrackLatestVersion=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyIpamPrefixListResolverTargetOutput {
    _ = status;
    _ = headers;
    ...
    var result: ModifyIpamPrefixListResolverTargetOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamPrefixListResolverTarget")) {
                    result.ipam_prefix_list_resolver_target = try serde.deserializeIpamPrefixListResolverTarget(&reader, alloc);
                } else {
