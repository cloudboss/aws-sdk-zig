...
pub const ModifySnapshotTierInput = struct {
    ...
    storage_tier: ?TargetStorageTier = null,
};

pub const ModifySnapshotTierOutput = struct {
    /// The ID of the snapshot.
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifySnapshotTierInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifySnapshotTier&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifySnapshotTierOutput {
    _ = status;
    _ = headers;
