...
pub fn execute(client: *Client, input: CreateLocalGatewayRouteTableInput, options: Options) !CreateLocalGatewayRouteTableOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocalGatewayRouteTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateLocalGatewayRouteTable&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !CreateLocalGatewayRouteTableOutput {
    _ = status;
    _ = body;
