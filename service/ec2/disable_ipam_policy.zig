...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableIpamPolicyInput, options: CallOptions) !DisableIpamPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableIpamPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DisableIpamPolicy&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.organization_target_id) |v| {
        try body_buf.appendSlice(allocator, "&OrganizationTargetId=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableIpamPolicyOutput {
    _ = status;
    _ = headers;
    _ = allocator;
    ...
    var result: DisableIpamPolicyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
