...
pub const DisableIpamPolicyInput = struct {
    ...
    /// The ID of the Amazon Web Services Organizations target for which to disable
    ...
    /// be disabled for the current account.
    ///
...
pub const DisableIpamPolicyOutput = struct {
    /// Returns true if the IPAM policy was successfully disabled.
    @"return": ?bool = null,

    allocator: std.mem.Allocator,

    pub fn deinit(self: *const DisableIpamPolicyOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: DisableIpamPolicyInput, options: Options) !DisableIpamPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableIpamPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisableIpamPolicy&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.organization_target_id) |v| {
        try body_buf.appendSlice(alloc, "&OrganizationTargetId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableIpamPolicyOutput {
    _ = status;
    _ = headers;
    var result: DisableIpamPolicyOutput = .{ .allocator = alloc };
