...
pub fn execute(client: *Client, input: DescribeConversionTasksInput, options: Options) !DescribeConversionTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeConversionTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeConversionTasks&Version=2016-11-15");
    if (input.conversion_task_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ConversionTaskIds.item.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DescribeConversionTasksOutput {
    _ = status;
    _ = body;
