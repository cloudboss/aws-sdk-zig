...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSecurityGroupRuleDescriptionsIngressInput, options: CallOptions) !UpdateSecurityGroupRuleDescriptionsIngressOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSecurityGroupRuleDescriptionsIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UpdateSecurityGroupRuleDescriptionsIngress&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateSecurityGroupRuleDescriptionsIngressOutput {
    _ = status;
    _ = headers;
    _ = allocator;
