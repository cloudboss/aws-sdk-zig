...
pub const UpdateSecurityGroupRuleDescriptionsIngressOutput = struct {
    /// Returns `true` if the request succeeds; otherwise, returns an error.
    @"return": ?bool = null,

    allocator: std.mem.Allocator,

    pub fn deinit(self: *const UpdateSecurityGroupRuleDescriptionsIngressOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: UpdateSecurityGroupRuleDescriptionsIngressInput, options: Options) !UpdateSecurityGroupRuleDescriptionsIngressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSecurityGroupRuleDescriptionsIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateSecurityGroupRuleDescriptionsIngress&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.security_group_rule_descriptions) |list| {
        for (list, 0..) |item, idx| {
            ...
            {
                var prefix_buf: [256]u8 = undefined;
                const field_prefix = std.fmt.bufPrint(&prefix_buf, "&SecurityGroupRuleDescriptions.item.{d}.Description=", .{n}) catch continue;
                try body_buf.appendSlice(alloc, field_prefix);
            ...
            {
                var prefix_buf: [256]u8 = undefined;
                const field_prefix = std.fmt.bufPrint(&prefix_buf, "&SecurityGroupRuleDescriptions.item.{d}.SecurityGroupRuleId=", .{n}) catch continue;
                try body_buf.appendSlice(alloc, field_prefix);
                if (item.security_group_rule_id) |v| {
                    try appendUrlEncoded(alloc, &body_buf, v);
                }
            }
        }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSecurityGroupRuleDescriptionsIngressOutput {
    _ = status;
    _ = headers;
    var result: UpdateSecurityGroupRuleDescriptionsIngressOutput = .{ .allocator = alloc };
