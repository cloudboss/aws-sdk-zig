...
pub fn execute(client: *Client, input: DeleteIpamExternalResourceVerificationTokenInput, options: Options) !DeleteIpamExternalResourceVerificationTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteIpamExternalResourceVerificationTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteIpamExternalResourceVerificationToken&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&IpamExternalResourceVerificationTokenId=");
    try appendUrlEncoded(alloc, &body_buf, input.ipam_external_resource_verification_token_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !DeleteIpamExternalResourceVerificationTokenOutput {
    _ = status;
    _ = body;
