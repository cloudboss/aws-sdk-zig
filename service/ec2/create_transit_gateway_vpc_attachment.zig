...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTransitGatewayVpcAttachmentInput, options: CallOptions) !CreateTransitGatewayVpcAttachmentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTransitGatewayVpcAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateTransitGatewayVpcAttachment&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    for (input.subnet_ids, 0..) |item, idx| {
        ...
        const field_prefix = std.fmt.bufPrint(&prefix_buf, "&SubnetIds.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTransitGatewayVpcAttachmentOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "transitGatewayVpcAttachment")) {
                    result.transit_gateway_vpc_attachment = try serde.deserializeTransitGatewayVpcAttachment(allocator, &reader);
                } else {
