...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRouteTablesInput, options: CallOptions) !DescribeRouteTablesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRouteTablesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeRouteTables&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeRouteTablesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.route_tables = try serde.deserializeRouteTableList(allocator, &reader, "item");
                } else {
