...
pub const ModifyLaunchTemplateOutput = struct {
    ...
    pub fn deinit(self: *const ModifyLaunchTemplateOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ModifyLaunchTemplateOutput {
    _ = status;
    _ = body;
    const result: ModifyLaunchTemplateOutput = .{ .allocator = alloc };
