...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyLaunchTemplateInput, options: CallOptions) !ModifyLaunchTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyLaunchTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyLaunchTemplate&Version=2016-11-15");
    if (input.client_token) |v| {
        try body_buf.appendSlice(allocator, "&ClientToken=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.default_version) |v| {
        try body_buf.appendSlice(allocator, "&SetDefaultVersion=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyLaunchTemplateOutput {
    _ = status;
    _ = headers;
