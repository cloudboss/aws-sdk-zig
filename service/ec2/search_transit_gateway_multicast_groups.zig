...
pub fn execute(client: *Client, input: SearchTransitGatewayMulticastGroupsInput, options: Options) !SearchTransitGatewayMulticastGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchTransitGatewayMulticastGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SearchTransitGatewayMulticastGroups&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !SearchTransitGatewayMulticastGroupsOutput {
    _ = status;
