...
pub const SearchTransitGatewayMulticastGroupsOutput = struct {
    /// Information about the transit gateway multicast group.
    multicast_groups: ?[]const TransitGatewayMulticastGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchTransitGatewayMulticastGroupsInput, options: CallOptions) !SearchTransitGatewayMulticastGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchTransitGatewayMulticastGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=SearchTransitGatewayMulticastGroups&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchTransitGatewayMulticastGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "multicastGroups")) {
                    result.multicast_groups = try serde.deserializeTransitGatewayMulticastGroupList(allocator, &reader, "item");
