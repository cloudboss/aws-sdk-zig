...
pub const GetFlowLogsIntegrationTemplateOutput = struct {
    ...
    pub fn deinit(self: *const GetFlowLogsIntegrationTemplateOutput) void {
        if (self.result) |v| {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFlowLogsIntegrationTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try appendUrlEncoded(alloc, &body_buf, input.config_delivery_s_3_destination_arn);
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&FlowLogId=");
    try appendUrlEncoded(alloc, &body_buf, input.flow_log_id);
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !GetFlowLogsIntegrationTemplateOutput {
    _ = status;
    var result: GetFlowLogsIntegrationTemplateOutput = .{ .allocator = alloc };
    if (findElement(body, "result")) |content| {
