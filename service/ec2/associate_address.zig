...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AssociateAddress&Version=2016-11-15");
    if (input.allocation_id) |v| {
        try body_buf.appendSlice(allocator, "&AllocationId=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.allow_reassociation) |v| {
        try body_buf.appendSlice(allocator, "&AllowReassociation=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateAddressOutput {
    _ = status;
    _ = headers;
