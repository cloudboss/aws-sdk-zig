...
pub const CreateIpamPolicyOutput = struct {
    ...
    pub fn deinit(self: *CreateIpamPolicyOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: CreateIpamPolicyInput, options: Options) !CreateIpamPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIpamPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateIpamPolicy&Version=2016-11-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIpamPolicyOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamPolicy")) {
                    result.ipam_policy = try serde.deserializeIpamPolicy(&reader, alloc);
                } else {
