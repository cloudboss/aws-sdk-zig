...
pub const CreateIpamPolicyInput = struct {
...
pub const CreateIpamPolicyOutput = struct {
    ...
    ipam_policy: ?IpamPolicy = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIpamPolicyInput, options: CallOptions) !CreateIpamPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateIpamPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateIpamPolicy&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateIpamPolicyOutput {
    _ = status;
    _ = headers;
    ...
    var result: CreateIpamPolicyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ipamPolicy")) {
                    result.ipam_policy = try serde.deserializeIpamPolicy(allocator, &reader);
                } else {
