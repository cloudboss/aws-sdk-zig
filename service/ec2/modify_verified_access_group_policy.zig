...
pub const ModifyVerifiedAccessGroupPolicyOutput = struct {
    /// The Verified Access policy document.
    policy_document: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyVerifiedAccessGroupPolicyInput, options: CallOptions) !ModifyVerifiedAccessGroupPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyVerifiedAccessGroupPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyVerifiedAccessGroupPolicy&Version=2016-11-15");
    ...
    if (input.policy_enabled) |v| {
        try body_buf.appendSlice(allocator, "&PolicyEnabled=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyVerifiedAccessGroupPolicyOutput {
    _ = status;
    _ = headers;
    ...
    var result: ModifyVerifiedAccessGroupPolicyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "policyDocument")) {
                    ...
                    result.sse_specification = try serde.deserializeVerifiedAccessSseSpecificationResponse(allocator, &reader);
                } else {
