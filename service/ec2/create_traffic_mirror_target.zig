...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrafficMirrorTargetInput, options: CallOptions) !CreateTrafficMirrorTargetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTrafficMirrorTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateTrafficMirrorTarget&Version=2016-11-15");
    ...
    if (input.gateway_load_balancer_endpoint_id) |v| {
        try body_buf.appendSlice(allocator, "&GatewayLoadBalancerEndpointId=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    ...
    if (input.network_load_balancer_arn) |v| {
        try body_buf.appendSlice(allocator, "&NetworkLoadBalancerArn=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTrafficMirrorTargetOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "clientToken")) {
                    ...
                    result.traffic_mirror_target = try serde.deserializeTrafficMirrorTarget(allocator, &reader);
                } else {
