...
pub const DescribeSubnetsOutput = struct {
    ...
    pub fn deinit(self: *DescribeSubnetsOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeSubnetsInput, options: Options) !DescribeSubnetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSubnetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeSubnets&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSubnetsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.subnets = try serde.deserializeSubnetList(&reader, alloc, "item");
                } else {
