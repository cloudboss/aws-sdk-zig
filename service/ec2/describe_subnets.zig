...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSubnetsInput, options: CallOptions) !DescribeSubnetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSubnetsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.subnet_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&SubnetId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
