...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DetachNetworkInterface&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&AttachmentId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.attachment_id);
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachNetworkInterfaceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
