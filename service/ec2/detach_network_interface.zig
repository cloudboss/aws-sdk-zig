...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachNetworkInterfaceInput, options: CallOptions) !DetachNetworkInterfaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DetachNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DetachNetworkInterface&Version=2016-11-15");
    try body_buf.appendSlice(allocator, "&AttachmentId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.attachment_id);
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DetachNetworkInterfaceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
