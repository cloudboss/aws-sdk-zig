...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportVolumeInput, options: CallOptions) !ImportVolumeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ImportVolumeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ImportVolume&Version=2016-11-15");
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, std.fmt.allocPrint(allocator, "{d}", .{input.volume.size}) catch "");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ImportVolumeOutput {
    _ = status;
    _ = headers;
