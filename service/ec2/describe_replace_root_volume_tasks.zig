...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplaceRootVolumeTasksInput, options: CallOptions) !DescribeReplaceRootVolumeTasksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReplaceRootVolumeTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeReplaceRootVolumeTasks&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.replace_root_volume_task_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ReplaceRootVolumeTaskId.ReplaceRootVolumeTaskId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReplaceRootVolumeTasksOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "nextToken")) {
                    ...
                    result.replace_root_volume_tasks = try serde.deserializeReplaceRootVolumeTasks(allocator, &reader, "item");
                } else {
