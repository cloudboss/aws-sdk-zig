...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UnassignPrivateIpAddressesInput, options: CallOptions) !UnassignPrivateIpAddressesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UnassignPrivateIpAddressesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UnassignPrivateIpAddresses&Version=2016-11-15");
    if (input.ipv_4_prefixes) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&Ipv4Prefix.{d}=", .{n}) catch continue;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UnassignPrivateIpAddressesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
