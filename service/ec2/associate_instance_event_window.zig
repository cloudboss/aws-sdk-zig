...
pub fn execute(client: *Client, input: AssociateInstanceEventWindowInput, options: Options) !AssociateInstanceEventWindowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateInstanceEventWindowInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AssociateInstanceEventWindow&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !AssociateInstanceEventWindowOutput {
    _ = status;
    _ = body;
