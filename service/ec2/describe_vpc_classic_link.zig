...
pub const DescribeVpcClassicLinkOutput = struct {
    /// The ClassicLink status of the VPCs.
    vpcs: ?[]const VpcClassicLink = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpcClassicLinkInput, options: CallOptions) !DescribeVpcClassicLinkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpcClassicLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeVpcClassicLink&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVpcClassicLinkOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeVpcClassicLinkOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "vpcSet")) {
                    result.vpcs = try serde.deserializeVpcClassicLinkList(allocator, &reader, "item");
                } else {
