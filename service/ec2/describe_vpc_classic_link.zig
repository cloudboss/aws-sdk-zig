...
pub const DescribeVpcClassicLinkOutput = struct {
    ...
    pub fn deinit(self: *DescribeVpcClassicLinkOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeVpcClassicLinkInput, options: Options) !DescribeVpcClassicLinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVpcClassicLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeVpcClassicLink&Version=2016-11-15");
    if (input.dry_run) |v| {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "vpcSet")) {
                    result.vpcs = try serde.deserializeVpcClassicLinkList(&reader, alloc, "item");
                } else {
