...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopInstancesInput, options: CallOptions) !StopInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=StopInstances&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.hibernate) |v| {
        try body_buf.appendSlice(allocator, "&Hibernate=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    ...
    if (input.skip_os_shutdown) |v| {
        try body_buf.appendSlice(allocator, "&SkipOsShutdown=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopInstancesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "instancesSet")) {
                    result.stopping_instances = try serde.deserializeInstanceStateChangeList(allocator, &reader, "item");
                } else {
