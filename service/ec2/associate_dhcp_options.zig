...
pub const AssociateDhcpOptionsInput = struct {
...
pub const AssociateDhcpOptionsOutput = struct {
    ...
    pub fn deinit(self: *const AssociateDhcpOptionsOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: AssociateDhcpOptionsInput, options: Options) !AssociateDhcpOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateDhcpOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AssociateDhcpOptions&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&DhcpOptionsId=");
    try appendUrlEncoded(alloc, &body_buf, input.dhcp_options_id);
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateDhcpOptionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: AssociateDhcpOptionsOutput = .{ .allocator = alloc };
