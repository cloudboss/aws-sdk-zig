...
pub const DescribeFpgaImagesInput = struct {
    ...
    /// The AFI IDs.
    fpga_image_ids: ?[]const []const u8 = null,
...
pub const DescribeFpgaImagesOutput = struct {
    /// Information about the FPGA images.
    fpga_images: ?[]const FpgaImage = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFpgaImagesInput, options: CallOptions) !DescribeFpgaImagesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFpgaImagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeFpgaImages&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.fpga_image_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&FpgaImageId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
    ...
    if (input.owners) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&Owner.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFpgaImagesOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeFpgaImagesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "fpgaImageSet")) {
                    result.fpga_images = try serde.deserializeFpgaImageList(allocator, &reader, "item");
