...
pub const DescribeFpgaImagesOutput = struct {
    ...
    pub fn deinit(self: *DescribeFpgaImagesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeFpgaImagesInput, options: Options) !DescribeFpgaImagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFpgaImagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeFpgaImages&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFpgaImagesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "fpgaImageSet")) {
                    result.fpga_images = try serde.deserializeFpgaImageList(&reader, alloc, "item");
