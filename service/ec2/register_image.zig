...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterImageInput, options: CallOptions) !RegisterImageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterImageInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.billing_products) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&BillingProduct.{d}=", .{n}) catch continue;
    ...
    if (input.ena_support) |v| {
        try body_buf.appendSlice(allocator, "&EnaSupport=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, if (v) "true" else "false");
    }
    if (input.image_location) |v| {
        try body_buf.appendSlice(allocator, "&ImageLocation=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    ...
    if (input.root_device_name) |v| {
        try body_buf.appendSlice(allocator, "&RootDeviceName=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.sriov_net_support) |v| {
        try body_buf.appendSlice(allocator, "&SriovNetSupport=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    ...
    if (input.virtualization_type) |v| {
        try body_buf.appendSlice(allocator, "&VirtualizationType=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterImageOutput {
    _ = status;
    _ = headers;
