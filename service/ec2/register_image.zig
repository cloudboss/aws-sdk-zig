...
pub const RegisterImageOutput = struct {
    ...
    pub fn deinit(self: *const RegisterImageOutput) void {
        if (self.image_id) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !RegisterImageOutput {
    _ = status;
    var result: RegisterImageOutput = .{ .allocator = alloc };
