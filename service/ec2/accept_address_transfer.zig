...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptAddressTransferInput, options: CallOptions) !AcceptAddressTransferOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptAddressTransferInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AcceptAddressTransfer&Version=2016-11-15");
    try body_buf.appendSlice(allocator, "&Address=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.address);
    if (input.dry_run) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptAddressTransferOutput {
    _ = status;
    _ = headers;
