...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptAddressTransferInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AcceptAddressTransfer&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&Address=");
    try appendUrlEncoded(alloc, &body_buf, input.address);
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !AcceptAddressTransferOutput {
    _ = status;
    _ = body;
