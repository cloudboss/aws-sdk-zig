...
pub const AssociateIpamResourceDiscoveryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateIpamResourceDiscoveryInput, options: CallOptions) !AssociateIpamResourceDiscoveryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateIpamResourceDiscoveryInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AssociateIpamResourceDiscovery&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateIpamResourceDiscoveryOutput {
    _ = status;
    _ = headers;
    ...
    var result: AssociateIpamResourceDiscoveryOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
