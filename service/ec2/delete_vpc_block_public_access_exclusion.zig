...
pub fn execute(client: *Client, input: DeleteVpcBlockPublicAccessExclusionInput, options: Options) !DeleteVpcBlockPublicAccessExclusionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVpcBlockPublicAccessExclusionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteVpcBlockPublicAccessExclusion&Version=2016-11-15");
    if (input.dry_run) |v| {
        try body_buf.appendSlice(alloc, "&DryRun=");
        try appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&ExclusionId=");
    try appendUrlEncoded(alloc, &body_buf, input.exclusion_id);
