...
pub const CancelExportTaskOutput = struct {
    ...
    pub fn deinit(self: *const CancelExportTaskOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: CancelExportTaskInput, options: Options) !CancelExportTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelExportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CancelExportTask&Version=2016-11-15");
    try body_buf.appendSlice(alloc, "&ExportTaskId=");
    try appendUrlEncoded(alloc, &body_buf, input.export_task_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelExportTaskOutput {
    ...
    _ = headers;
    _ = body;
    const result: CancelExportTaskOutput = .{ .allocator = alloc };
