...
pub const CreateLocalGatewayRouteOutput = struct {
    /// Information about the route.
    route: ?LocalGatewayRoute = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocalGatewayRouteInput, options: CallOptions) !CreateLocalGatewayRouteOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLocalGatewayRouteInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateLocalGatewayRoute&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLocalGatewayRouteOutput {
    _ = status;
    _ = headers;
    ...
    var result: CreateLocalGatewayRouteOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "route")) {
                    result.route = try serde.deserializeLocalGatewayRoute(allocator, &reader);
                } else {
