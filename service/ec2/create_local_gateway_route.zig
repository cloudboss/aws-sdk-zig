...
pub const CreateLocalGatewayRouteOutput = struct {
    ...
    pub fn deinit(self: *const CreateLocalGatewayRouteOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: CreateLocalGatewayRouteInput, options: Options) !CreateLocalGatewayRouteOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocalGatewayRouteInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateLocalGatewayRoute&Version=2016-11-15");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLocalGatewayRouteOutput {
    _ = status;
    _ = headers;
    _ = body;
    const result: CreateLocalGatewayRouteOutput = .{ .allocator = alloc };
