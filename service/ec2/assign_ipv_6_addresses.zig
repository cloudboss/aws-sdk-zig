...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssignIpv6AddressesInput, options: Options) !AssignIpv6AddressesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssignIpv6AddressesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AssignIpv6Addresses&Version=2016-11-15");
    ...
    if (input.ipv_6_addresses) |list| {
        for (list, 0..) |item, idx| {
            ...
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&Ipv6Addresses.item.{d}=", .{n}) catch continue;
    ...
    if (input.ipv_6_prefixes) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&Ipv6Prefix.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try aws.url.appendUrlEncoded(alloc, &body_buf, item);
        }
    }
