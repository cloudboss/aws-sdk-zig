...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReplaceRootVolumeTaskInput, options: CallOptions) !CreateReplaceRootVolumeTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateReplaceRootVolumeTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateReplaceRootVolumeTask&Version=2016-11-15");
    if (input.client_token) |v| {
        try body_buf.appendSlice(allocator, "&ClientToken=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.delete_replaced_root_volume) |v| {
        try body_buf.appendSlice(allocator, "&DeleteReplacedRootVolume=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateReplaceRootVolumeTaskOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "replaceRootVolumeTask")) {
                    result.replace_root_volume_task = try serde.deserializeReplaceRootVolumeTask(allocator, &reader);
                } else {
