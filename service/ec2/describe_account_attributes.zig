...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAccountAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAccountAttributes&Version=2016-11-15");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "accountAttributeSet")) {
                    result.account_attributes = try serde.deserializeAccountAttributeList(allocator, &reader, "item");
                } else {
