...
pub const DescribeAccountAttributesOutput = struct {
    ...
    account_attributes: ?[]const AccountAttribute = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *DescribeAccountAttributesOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: DescribeAccountAttributesInput, options: Options) !DescribeAccountAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAccountAttributes&Version=2016-11-15");
    if (input.attribute_names) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&AttributeNames.attributeName.{d}=", .{n}) catch continue;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountAttributesOutput {
    _ = status;
    _ = headers;
    ...
    var result: DescribeAccountAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "accountAttributeSet")) {
                    result.account_attributes = try serde.deserializeAccountAttributeList(&reader, alloc, "item");
                } else {
