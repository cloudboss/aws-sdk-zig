...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReservedInstancesOfferingsInput, options: CallOptions) !DescribeReservedInstancesOfferingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReservedInstancesOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.reserved_instances_offering_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&ReservedInstancesOfferingId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
