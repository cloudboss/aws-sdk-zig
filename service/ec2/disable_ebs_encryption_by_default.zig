...
pub const DisableEbsEncryptionByDefaultOutput = struct {
    ...
    pub fn deinit(self: *const DisableEbsEncryptionByDefaultOutput) void {
        _ = self;
    }
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableEbsEncryptionByDefaultInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisableEbsEncryptionByDefault&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableEbsEncryptionByDefaultOutput {
    _ = status;
    _ = headers;
    var result: DisableEbsEncryptionByDefaultOutput = .{ .allocator = alloc };
