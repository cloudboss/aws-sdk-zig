...
fn serializeRequest(allocator: std.mem.Allocator, input: ProvisionByoipCidrInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ProvisionByoipCidr&Version=2016-11-15");
    ...
    if (input.cidr_authorization_context) |v| {
        ...
        try aws.url.appendUrlEncoded(allocator, &body_buf, v.message);
        try body_buf.appendSlice(allocator, "&CidrAuthorizationContext.Signature=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v.signature);
    }
