...
pub const ProvisionByoipCidrOutput = struct {
    ...
    pub fn deinit(self: *const ProvisionByoipCidrOutput) void {
        _ = self;
    }
};
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ProvisionByoipCidrOutput {
    _ = status;
    _ = body;
    const result: ProvisionByoipCidrOutput = .{ .allocator = alloc };
