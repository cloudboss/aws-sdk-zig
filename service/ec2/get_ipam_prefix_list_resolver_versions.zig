...
pub fn execute(client: *Client, input: GetIpamPrefixListResolverVersionsInput, options: Options) !GetIpamPrefixListResolverVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIpamPrefixListResolverVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetIpamPrefixListResolverVersions&Version=2016-11-15");
    if (input.dry_run) |v| {
    ...
    if (input.ipam_prefix_list_resolver_versions) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&IpamPrefixListResolverVersions.item.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
