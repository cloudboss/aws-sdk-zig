...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateNatGatewayInput, options: CallOptions) !CreateNatGatewayOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateNatGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateNatGateway&Version=2016-11-15");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateNatGatewayOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "clientToken")) {
                    ...
                    result.nat_gateway = try serde.deserializeNatGateway(allocator, &reader);
                } else {
