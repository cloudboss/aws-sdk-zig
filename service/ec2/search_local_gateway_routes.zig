...
pub fn execute(client: *Client, input: SearchLocalGatewayRoutesInput, options: Options) !SearchLocalGatewayRoutesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchLocalGatewayRoutesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SearchLocalGatewayRoutes&Version=2016-11-15");
    if (input.dry_run) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !SearchLocalGatewayRoutesOutput {
    _ = status;
