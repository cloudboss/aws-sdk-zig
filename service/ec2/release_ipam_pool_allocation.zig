...
pub const ReleaseIpamPoolAllocationInput = struct {
    ...
    ipam_pool_allocation_id: []const u8,

    /// The ID of the IPAM pool which contains the allocation you want to release.
    ipam_pool_id: []const u8,
};

pub const ReleaseIpamPoolAllocationOutput = struct {
    /// Indicates if the release was successful.
    success: ?bool = null,

    allocator: std.mem.Allocator,

    pub fn deinit(self: *const ReleaseIpamPoolAllocationOutput) void {
        _ = self;
    }
};
...
pub fn execute(client: *Client, input: ReleaseIpamPoolAllocationInput, options: Options) !ReleaseIpamPoolAllocationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReleaseIpamPoolAllocationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ReleaseIpamPoolAllocation&Version=2016-11-15");
    ...
    try body_buf.appendSlice(alloc, "&IpamPoolAllocationId=");
    try appendUrlEncoded(alloc, &body_buf, input.ipam_pool_allocation_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReleaseIpamPoolAllocationOutput {
    _ = status;
    _ = headers;
    var result: ReleaseIpamPoolAllocationOutput = .{ .allocator = alloc };
