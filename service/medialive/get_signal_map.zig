...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSignalMapInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSignalMapOutput {
    var result: GetSignalMapOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSignalMapOutput, body, alloc);
    }
    _ = status;
    _ = headers;
