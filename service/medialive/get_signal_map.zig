...
pub const GetSignalMapInput = struct {
...
pub const GetSignalMapOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSignalMapInput, options: CallOptions) !GetSignalMapOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSignalMapInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSignalMapOutput {
    var result: GetSignalMapOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSignalMapOutput, body, allocator);
    }
    _ = status;
    _ = headers;
