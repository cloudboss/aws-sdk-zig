...
pub const DescribeMultiplexInput = struct {
...
pub const DescribeMultiplexOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMultiplexInput, options: Options) !DescribeMultiplexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMultiplexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMultiplexOutput {
    var result: DescribeMultiplexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeMultiplexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
