...
fn serializeRequest(alloc: std.mem.Allocator, input: StopChannelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopChannelOutput {
    var result: StopChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopChannelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
