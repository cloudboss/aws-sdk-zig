...
pub const StopChannelInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopChannelInput, options: Options) !StopChannelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopChannelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopChannelOutput {
    var result: StopChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopChannelOutput, body, allocator);
    }
    _ = status;
    _ = headers;
