...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMultiplexInput, options: Options) !UpdateMultiplexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMultiplexInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.multiplex_settings) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"MultiplexSettings\":");
    ...
    if (input.packet_identifiers_mapping) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PacketIdentifiersMapping\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMultiplexOutput {
    var result: UpdateMultiplexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMultiplexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
