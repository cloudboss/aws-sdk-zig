...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEventBridgeRuleTemplateInput, options: Options) !CreateEventBridgeRuleTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEventBridgeRuleTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.event_targets) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EventTargets\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.group_identifier), input.group_identifier, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEventBridgeRuleTemplateOutput {
    var result: CreateEventBridgeRuleTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEventBridgeRuleTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
