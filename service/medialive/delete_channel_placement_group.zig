...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteChannelPlacementGroupInput, options: Options) !DeleteChannelPlacementGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteChannelPlacementGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteChannelPlacementGroupOutput {
    var result: DeleteChannelPlacementGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteChannelPlacementGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
