...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInputDeviceTransfersInput, options: Options) !ListInputDeviceTransfersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInputDeviceTransfersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "transferType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.transfer_type);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInputDeviceTransfersOutput {
    var result: ListInputDeviceTransfersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInputDeviceTransfersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
