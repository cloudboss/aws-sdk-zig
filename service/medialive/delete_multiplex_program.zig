...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMultiplexProgramInput, options: Options) !DeleteMultiplexProgramOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMultiplexProgramInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/programs/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMultiplexProgramOutput {
    var result: DeleteMultiplexProgramOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteMultiplexProgramOutput, body, alloc);
    }
    _ = status;
    _ = headers;
