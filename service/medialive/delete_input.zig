...
pub const DeleteInputInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteInputInput, options: Options) !DeleteInputOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteInputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteInputOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteInputOutput = .{};
