...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateChannelClassInput, options: Options) !UpdateChannelClassOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChannelClassInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/channelClass");
    ...
    try aws.json.writeValue(@TypeOf(input.channel_class), input.channel_class, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChannelClassOutput {
    var result: UpdateChannelClassOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChannelClassOutput, body, alloc);
    }
    _ = status;
    _ = headers;
