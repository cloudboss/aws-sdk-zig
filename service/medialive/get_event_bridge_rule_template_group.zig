...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEventBridgeRuleTemplateGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/prod/eventbridge-rule-template-groups/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEventBridgeRuleTemplateGroupOutput {
    var result: GetEventBridgeRuleTemplateGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEventBridgeRuleTemplateGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
