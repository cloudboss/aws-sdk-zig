...
const ListNetworksInput = @import("list_networks_request.zig").ListNetworksRequest;

const ListNetworksOutput = @import("list_networks_response.zig").ListNetworksResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListNetworksInput, options: Options) !ListNetworksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListNetworksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListNetworksOutput {
    var result: ListNetworksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListNetworksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
