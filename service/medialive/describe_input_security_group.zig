...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInputSecurityGroupInput, options: Options) !DescribeInputSecurityGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInputSecurityGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInputSecurityGroupOutput {
    var result: DescribeInputSecurityGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeInputSecurityGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
