...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSignalMapsInput, options: Options) !ListSignalMapsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSignalMapsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cloud_watch_alarm_template_group_identifier) |v| {
        ...
        try query_buf.appendSlice(alloc, "cloudWatchAlarmTemplateGroupIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.event_bridge_rule_template_group_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "eventBridgeRuleTemplateGroupIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSignalMapsOutput {
    var result: ListSignalMapsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSignalMapsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
