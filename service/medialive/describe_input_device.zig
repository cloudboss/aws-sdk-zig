...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInputDeviceInput, options: Options) !DescribeInputDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInputDeviceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInputDeviceOutput {
    var result: DescribeInputDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeInputDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
