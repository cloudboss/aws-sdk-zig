...
pub const CreateSdiSourceInput = struct {
...
pub const CreateSdiSourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSdiSourceInput, options: CallOptions) !CreateSdiSourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSdiSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSdiSourceOutput {
    var result: CreateSdiSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSdiSourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
