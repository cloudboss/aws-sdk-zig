...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSdiSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSdiSourceOutput {
    var result: CreateSdiSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSdiSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
