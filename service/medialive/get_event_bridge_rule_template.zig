...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEventBridgeRuleTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEventBridgeRuleTemplateOutput {
    var result: GetEventBridgeRuleTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEventBridgeRuleTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
