...
pub const BatchStartInput = struct {
    /// List of channel IDs
...
pub const BatchStartOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchStartInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchStartOutput {
    var result: BatchStartOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchStartOutput, body, alloc);
    }
    _ = status;
    _ = headers;
