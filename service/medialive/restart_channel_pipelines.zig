...
pub const RestartChannelPipelinesInput = struct {
    /// ID of channel
    channel_id: []const u8,

    /// An array of pipelines to restart in this channel. Format PIPELINE_0 or
    /// PIPELINE_1.
    pipeline_ids: ?[]const ChannelPipelineIdToRestart = null,
...
pub const RestartChannelPipelinesOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestartChannelPipelinesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/restartChannelPipelines");
    ...
    if (input.pipeline_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PipelineIds\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestartChannelPipelinesOutput {
    var result: RestartChannelPipelinesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RestartChannelPipelinesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
