...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMultiplexInput, options: Options) !StartMultiplexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMultiplexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMultiplexOutput {
    var result: StartMultiplexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMultiplexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
