...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMultiplexProgramInput, options: Options) !CreateMultiplexProgramOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMultiplexProgramInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"MultiplexProgramSettings\":");
    try aws.json.writeValue(@TypeOf(input.multiplex_program_settings), input.multiplex_program_settings, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ProgramName\":");
    try aws.json.writeValue(@TypeOf(input.program_name), input.program_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMultiplexProgramOutput {
    var result: CreateMultiplexProgramOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMultiplexProgramOutput, body, alloc);
    }
    _ = status;
    _ = headers;
