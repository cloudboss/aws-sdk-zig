...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountConfigurationInput, options: Options) !DescribeAccountConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountConfigurationOutput {
    var result: DescribeAccountConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccountConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
