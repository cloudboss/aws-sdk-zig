...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateInputSecurityGroupInput, options: CallOptions) !UpdateInputSecurityGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateInputSecurityGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.whitelist_rules) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"WhitelistRules\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
