...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSignalMapInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cloud_watch_alarm_template_group_identifiers) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"CloudWatchAlarmTemplateGroupIdentifiers\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DiscoveryEntryPointArn\":");
    try aws.json.writeValue(@TypeOf(input.discovery_entry_point_arn), input.discovery_entry_point_arn, alloc, &body_buf);
    has_prev = true;
    if (input.event_bridge_rule_template_group_identifiers) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EventBridgeRuleTemplateGroupIdentifiers\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSignalMapOutput {
    var result: CreateSignalMapOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSignalMapOutput, body, alloc);
    }
    _ = status;
    _ = headers;
