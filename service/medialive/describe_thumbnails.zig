...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeThumbnailsInput, options: CallOptions) !DescribeThumbnailsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeThumbnailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/thumbnails");
    const path = try path_buf.toOwnedSlice(allocator);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "pipelineId=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.pipeline_id);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "thumbnailType=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.thumbnail_type);
    query_has_prev = true;
