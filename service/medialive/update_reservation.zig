...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateReservationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateReservationOutput {
    var result: UpdateReservationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateReservationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
