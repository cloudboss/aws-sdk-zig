...
pub const UpdateReservationInput = struct {
...
pub const UpdateReservationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateReservationInput, options: CallOptions) !UpdateReservationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateReservationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateReservationOutput {
    var result: UpdateReservationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateReservationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
