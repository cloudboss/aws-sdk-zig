...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEventBridgeRuleTemplateInput, options: Options) !UpdateEventBridgeRuleTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEventBridgeRuleTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEventBridgeRuleTemplateOutput {
    var result: UpdateEventBridgeRuleTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEventBridgeRuleTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
