...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCloudWatchAlarmTemplatesInput, options: Options) !ListCloudWatchAlarmTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCloudWatchAlarmTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.group_identifier) |v| {
        ...
        try query_buf.appendSlice(alloc, "groupIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCloudWatchAlarmTemplatesOutput {
    var result: ListCloudWatchAlarmTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCloudWatchAlarmTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
