...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMultiplexInput, options: Options) !CreateMultiplexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMultiplexInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AvailabilityZones\":");
    try aws.json.writeValue(@TypeOf(input.availability_zones), input.availability_zones, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"MultiplexSettings\":");
    try aws.json.writeValue(@TypeOf(input.multiplex_settings), input.multiplex_settings, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMultiplexOutput {
    var result: CreateMultiplexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMultiplexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
