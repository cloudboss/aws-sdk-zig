...
const CreateNodeInput = @import("create_node_request.zig").CreateNodeRequest;

const CreateNodeOutput = @import("create_node_response.zig").CreateNodeResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateNodeInput, options: Options) !CreateNodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNodeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateNodeOutput {
    var result: CreateNodeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateNodeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
