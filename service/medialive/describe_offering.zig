...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOfferingInput, options: Options) !DescribeOfferingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOfferingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOfferingOutput {
    var result: DescribeOfferingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeOfferingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
