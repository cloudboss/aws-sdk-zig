...
pub const UpdateSdiSourceInput = struct {
...
pub const UpdateSdiSourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSdiSourceInput, options: Options) !UpdateSdiSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSdiSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSdiSourceOutput {
    var result: UpdateSdiSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSdiSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
