...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchStopInput, options: Options) !BatchStopOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchStopInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchStopOutput {
    var result: BatchStopOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchStopOutput, body, alloc);
    }
    _ = status;
    _ = headers;
