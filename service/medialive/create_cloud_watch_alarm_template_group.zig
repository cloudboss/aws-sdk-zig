...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCloudWatchAlarmTemplateGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCloudWatchAlarmTemplateGroupOutput {
    var result: CreateCloudWatchAlarmTemplateGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCloudWatchAlarmTemplateGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
