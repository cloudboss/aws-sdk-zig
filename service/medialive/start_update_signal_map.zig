...
pub const StartUpdateSignalMapInput = struct {
...
pub const StartUpdateSignalMapOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartUpdateSignalMapInput, options: CallOptions) !StartUpdateSignalMapOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartUpdateSignalMapInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.discovery_entry_point_arn) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    if (input.force_rediscovery) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ForceRediscovery\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartUpdateSignalMapOutput {
    var result: StartUpdateSignalMapOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartUpdateSignalMapOutput, body, allocator);
    }
    _ = status;
    _ = headers;
