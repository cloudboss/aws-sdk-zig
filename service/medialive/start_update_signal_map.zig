...
fn serializeRequest(alloc: std.mem.Allocator, input: StartUpdateSignalMapInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.force_rediscovery) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ForceRediscovery\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartUpdateSignalMapOutput {
    var result: StartUpdateSignalMapOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartUpdateSignalMapOutput, body, alloc);
    }
    _ = status;
    _ = headers;
