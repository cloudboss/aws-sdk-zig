...
const ClusterNetworkSettingsCreateRequest = @import("cluster_network_settings_create_request.zig").ClusterNetworkSettingsCreateRequest;
...
pub const CreateClusterInput = struct {
    /// Specify a type. All the Nodes that you later add to this Cluster must be
    /// this type of hardware. One Cluster instance can't contain different hardware
    /// types. You won't be able to change this parameter after you create the
    /// Cluster.
    cluster_type: ?ClusterType = null,

    /// The ARN of the IAM role for the Node in this Cluster. The role must include
    /// all the operations that you expect these Node to perform. If necessary,
    /// create a role in IAM, then attach it here.
    instance_role_arn: ?[]const u8 = null,

    /// Specify a name that is unique in the AWS account. We recommend that you
    /// assign a name that hints at the types of Nodes in the Cluster. Names are
    /// case-sensitive.
    name: ?[]const u8 = null,

    /// Network settings that connect the Nodes in the Cluster to one or more of the
    /// Networks that the Cluster is associated with.
    network_settings: ?ClusterNetworkSettingsCreateRequest = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.instance_role_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InstanceRoleArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
    ...
    if (input.network_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"NetworkSettings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
