...
const RebootInputDeviceForce = @import("reboot_input_device_force.zig").RebootInputDeviceForce;

pub const RebootInputDeviceInput = struct {
    /// Force a reboot of an input device. If the device is streaming, it will stop
    /// streaming and begin rebooting within a few seconds of sending the command.
    /// If the device was streaming prior to the reboot, the device will resume
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebootInputDeviceInput, options: Options) !RebootInputDeviceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RebootInputDeviceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RebootInputDeviceOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: RebootInputDeviceOutput = .{};
