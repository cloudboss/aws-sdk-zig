...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteInput, options: CallOptions) !BatchDeleteOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchDeleteInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.input_ids) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"InputIds\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.input_security_group_ids) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"InputSecurityGroupIds\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchDeleteOutput {
    var result: BatchDeleteOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteOutput, body, allocator);
    }
    _ = status;
    _ = headers;
