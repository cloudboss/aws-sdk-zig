...
pub const BatchDeleteInput = struct {
    /// List of channel IDs
    channel_ids: ?[]const []const u8 = null,

    /// List of input IDs
    input_ids: ?[]const []const u8 = null,

    /// List of input security group IDs
    input_security_group_ids: ?[]const []const u8 = null,
...
pub const BatchDeleteOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteInput, options: Options) !BatchDeleteOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.channel_ids) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ChannelIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.input_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InputIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.input_security_group_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"InputSecurityGroupIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.multiplex_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MultiplexIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteOutput {
    var result: BatchDeleteOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteOutput, body, alloc);
    }
    _ = status;
    _ = headers;
