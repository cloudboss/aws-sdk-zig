...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePartnerInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/partners");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePartnerInputOutput {
    var result: CreatePartnerInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePartnerInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
