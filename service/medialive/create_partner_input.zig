...
pub const CreatePartnerInputInput = struct {
...
pub const CreatePartnerInputOutput = struct {
    input: ?Input = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePartnerInputInput, options: CallOptions) !CreatePartnerInputOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreatePartnerInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/partners");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreatePartnerInputOutput {
    var result: CreatePartnerInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePartnerInputOutput, body, allocator);
    }
    _ = status;
    _ = headers;
