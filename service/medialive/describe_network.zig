...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNetworkInput, options: Options) !DescribeNetworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNetworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNetworkOutput {
    var result: DescribeNetworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeNetworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
