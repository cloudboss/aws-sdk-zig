...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOfferingsInput, options: CallOptions) !ListOfferingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.channel_configuration) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "channelConfiguration=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
    if (input.codec) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOfferingsOutput {
    var result: ListOfferingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOfferingsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
