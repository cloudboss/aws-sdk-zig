...
pub const ListOfferingsInput = struct {
    ...
    channel_configuration: ?[]const u8 = null,
...
pub const ListOfferingsOutput = struct {
    /// Token to retrieve the next page of results
    next_token: ?[]const u8 = null,

    /// List of offerings
    offerings: ?[]const Offering = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOfferingsInput, options: Options) !ListOfferingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/prod/offerings";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (input.channel_class) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "channelClass=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.channel_configuration) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "channelConfiguration=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.codec) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "codec=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.duration) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "duration=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.maximum_bitrate) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "maximumBitrate=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.maximum_framerate) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "maximumFramerate=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.resolution) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "resolution=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.special_feature) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "specialFeature=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.video_quality) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "videoQuality=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOfferingsOutput {
    var result: ListOfferingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOfferingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
