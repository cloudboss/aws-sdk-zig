...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDeleteMonitorDeploymentInput, options: Options) !StartDeleteMonitorDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDeleteMonitorDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/prod/signal-maps/");
    try path_buf.appendSlice(alloc, input.identifier);
    try path_buf.appendSlice(alloc, "/monitor-deployment");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDeleteMonitorDeploymentOutput {
    var result: StartDeleteMonitorDeploymentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDeleteMonitorDeploymentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
