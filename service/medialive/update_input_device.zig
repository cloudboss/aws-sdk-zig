...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateInputDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.hd_device_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"HdDeviceSettings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
    ...
    if (input.uhd_device_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"UhdDeviceSettings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateInputDeviceOutput {
    var result: UpdateInputDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateInputDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
