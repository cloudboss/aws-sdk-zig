...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReservationsInput, options: Options) !ListReservationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReservationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReservationsOutput {
    var result: ListReservationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReservationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
