...
const DescribeClusterSummary = @import("describe_cluster_summary.zig").DescribeClusterSummary;
...
pub const ListClustersOutput = struct {
    ...
    clusters: ?[]const DescribeClusterSummary = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListClustersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListClustersOutput {
    var result: ListClustersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListClustersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
