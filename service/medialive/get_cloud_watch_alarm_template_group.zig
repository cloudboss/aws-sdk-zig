...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCloudWatchAlarmTemplateGroupInput, options: Options) !GetCloudWatchAlarmTemplateGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCloudWatchAlarmTemplateGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/prod/cloudwatch-alarm-template-groups/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCloudWatchAlarmTemplateGroupOutput {
    var result: GetCloudWatchAlarmTemplateGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCloudWatchAlarmTemplateGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
