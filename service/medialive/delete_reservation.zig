...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteReservationInput, options: Options) !DeleteReservationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteReservationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteReservationOutput {
    var result: DeleteReservationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteReservationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
