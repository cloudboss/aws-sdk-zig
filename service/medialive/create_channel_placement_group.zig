...
pub const CreateChannelPlacementGroupInput = struct {
    /// The ID of the cluster.
    cluster_id: []const u8,

    /// Specify a name that is unique in the Cluster. You can't change the name.
    ...
    /// ChannelPlacementGroup.) The Node and the ChannelPlacementGroup must be in
    /// the same Cluster.
...
pub const CreateChannelPlacementGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateChannelPlacementGroupInput, options: Options) !CreateChannelPlacementGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateChannelPlacementGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.nodes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Nodes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateChannelPlacementGroupOutput {
    var result: CreateChannelPlacementGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChannelPlacementGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
