...
pub const StopMultiplexInput = struct {
...
pub const StopMultiplexOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopMultiplexInput, options: Options) !StopMultiplexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopMultiplexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopMultiplexOutput {
    var result: StopMultiplexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopMultiplexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
