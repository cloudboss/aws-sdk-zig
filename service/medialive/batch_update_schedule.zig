...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.creates) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"Creates\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateScheduleOutput {
    var result: BatchUpdateScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateScheduleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
