...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCloudWatchAlarmTemplateInput, options: Options) !GetCloudWatchAlarmTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCloudWatchAlarmTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCloudWatchAlarmTemplateOutput {
    var result: GetCloudWatchAlarmTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCloudWatchAlarmTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
