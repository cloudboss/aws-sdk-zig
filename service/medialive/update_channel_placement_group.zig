...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateChannelPlacementGroupInput, options: Options) !UpdateChannelPlacementGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChannelPlacementGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChannelPlacementGroupOutput {
    var result: UpdateChannelPlacementGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChannelPlacementGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
