...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMonitorDeploymentInput, options: Options) !StartMonitorDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMonitorDeploymentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMonitorDeploymentOutput {
    var result: StartMonitorDeploymentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMonitorDeploymentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
