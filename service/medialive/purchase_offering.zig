...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurchaseOfferingInput, options: Options) !PurchaseOfferingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/prod/offerings/");
    try path_buf.appendSlice(alloc, input.offering_id);
    try path_buf.appendSlice(alloc, "/purchase");
    ...
    try body_buf.appendSlice(alloc, "\"Count\":");
    try aws.json.writeValue(@TypeOf(input.count), input.count, alloc, &body_buf);
    has_prev = true;
    if (input.name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.renewal_settings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RenewalSettings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.start) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Start\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PurchaseOfferingOutput {
    var result: PurchaseOfferingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PurchaseOfferingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
