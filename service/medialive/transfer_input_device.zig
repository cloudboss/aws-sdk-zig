...
fn serializeRequest(allocator: std.mem.Allocator, input: TransferInputDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/transfer");
    ...
    if (input.target_customer_id) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"TargetCustomerId\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.target_region) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"TargetRegion\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TransferInputDeviceOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
