...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMultiplexProgramInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMultiplexProgramOutput {
    var result: DescribeMultiplexProgramOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeMultiplexProgramOutput, body, alloc);
    }
    _ = status;
    _ = headers;
