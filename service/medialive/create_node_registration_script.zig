...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateNodeRegistrationScriptInput, options: CallOptions) !CreateNodeRegistrationScriptOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateNodeRegistrationScriptInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/nodeRegistrationScript");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateNodeRegistrationScriptOutput {
    var result: CreateNodeRegistrationScriptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateNodeRegistrationScriptOutput, body, allocator);
    }
    _ = status;
    _ = headers;
