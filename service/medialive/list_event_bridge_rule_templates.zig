...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEventBridgeRuleTemplatesInput, options: Options) !ListEventBridgeRuleTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventBridgeRuleTemplatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEventBridgeRuleTemplatesOutput {
    var result: ListEventBridgeRuleTemplatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEventBridgeRuleTemplatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
