...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMultiplexProgramInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMultiplexProgramOutput {
    var result: UpdateMultiplexProgramOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMultiplexProgramOutput, body, alloc);
    }
    _ = status;
    _ = headers;
