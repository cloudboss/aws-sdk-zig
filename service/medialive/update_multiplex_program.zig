...
pub const UpdateMultiplexProgramInput = struct {
...
pub const UpdateMultiplexProgramOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMultiplexProgramInput, options: CallOptions) !UpdateMultiplexProgramOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateMultiplexProgramInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.multiplex_program_settings) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateMultiplexProgramOutput {
    var result: UpdateMultiplexProgramOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMultiplexProgramOutput, body, allocator);
    }
    _ = status;
    _ = headers;
