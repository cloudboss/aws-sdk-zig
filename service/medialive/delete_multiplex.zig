...
pub const DeleteMultiplexInput = struct {
...
pub const DeleteMultiplexOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMultiplexInput, options: Options) !DeleteMultiplexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMultiplexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMultiplexOutput {
    var result: DeleteMultiplexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteMultiplexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
