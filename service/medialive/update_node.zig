...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.sdi_source_mappings) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"SdiSourceMappings\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
