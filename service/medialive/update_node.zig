...
const UpdateNodeInput = @import("update_node_request.zig").UpdateNodeRequest;

const UpdateNodeOutput = @import("update_node_response.zig").UpdateNodeResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNodeInput, options: Options) !UpdateNodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/nodes/");
    try path_buf.appendSlice(alloc, input.node_id);
    ...
    if (input.sdi_source_mappings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SdiSourceMappings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNodeOutput {
    var result: UpdateNodeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateNodeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
