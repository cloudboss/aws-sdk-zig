...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNodeStateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNodeStateOutput {
    var result: UpdateNodeStateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateNodeStateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
