...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTaskInput, options: Options) !CreateTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.CreateTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTaskOutput, body, alloc);
}
