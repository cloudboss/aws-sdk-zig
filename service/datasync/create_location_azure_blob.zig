...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationAzureBlobInput, options: Options) !CreateLocationAzureBlobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocationAzureBlobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.CreateLocationAzureBlob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLocationAzureBlobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationAzureBlobOutput, body, alloc);
}
