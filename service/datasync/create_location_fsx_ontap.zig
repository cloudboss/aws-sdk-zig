...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationFsxOntapInput, options: Options) !CreateLocationFsxOntapOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocationFsxOntapInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.CreateLocationFsxOntap");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLocationFsxOntapOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationFsxOntapOutput, body, alloc);
}
