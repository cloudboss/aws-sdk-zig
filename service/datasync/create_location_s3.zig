...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationS3Input, options: CallOptions) !CreateLocationS3Output {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLocationS3Input, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "FmrsService.CreateLocationS3");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLocationS3Output {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationS3Output, body, allocator);
}
