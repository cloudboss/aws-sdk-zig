...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLocationFsxLustreInput, options: Options) !UpdateLocationFsxLustreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLocationFsxLustreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.UpdateLocationFsxLustre");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLocationFsxLustreOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
