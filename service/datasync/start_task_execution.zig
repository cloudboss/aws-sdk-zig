...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartTaskExecutionInput, options: Options) !StartTaskExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartTaskExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.StartTaskExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartTaskExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartTaskExecutionOutput, body, alloc);
}
