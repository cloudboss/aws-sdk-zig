...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLocationHdfsInput, options: Options) !DescribeLocationHdfsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLocationHdfsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.DescribeLocationHdfs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLocationHdfsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeLocationHdfsOutput, body, alloc);
}
