...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateLocationS3Input, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "FmrsService.UpdateLocationS3");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateLocationS3Output {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
