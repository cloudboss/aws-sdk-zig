...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTaskExecutionInput, options: Options) !DescribeTaskExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTaskExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.DescribeTaskExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTaskExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTaskExecutionOutput, body, alloc);
}
