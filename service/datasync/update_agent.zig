...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAgentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.UpdateAgent");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAgentOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
