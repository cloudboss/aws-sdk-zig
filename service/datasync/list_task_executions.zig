...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTaskExecutionsInput, options: Options) !ListTaskExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTaskExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.ListTaskExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTaskExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTaskExecutionsOutput, body, alloc);
}
