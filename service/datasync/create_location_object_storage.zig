...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationObjectStorageInput, options: Options) !CreateLocationObjectStorageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocationObjectStorageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.CreateLocationObjectStorage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLocationObjectStorageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationObjectStorageOutput, body, alloc);
}
