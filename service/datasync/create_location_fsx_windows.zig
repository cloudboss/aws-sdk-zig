...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationFsxWindowsInput, options: Options) !CreateLocationFsxWindowsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocationFsxWindowsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.CreateLocationFsxWindows");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLocationFsxWindowsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationFsxWindowsOutput, body, alloc);
}
