...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLocationObjectStorageInput, options: Options) !DescribeLocationObjectStorageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLocationObjectStorageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.DescribeLocationObjectStorage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLocationObjectStorageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeLocationObjectStorageOutput, body, alloc);
}
