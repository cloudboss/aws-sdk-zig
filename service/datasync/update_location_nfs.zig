...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLocationNfsInput, options: Options) !UpdateLocationNfsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLocationNfsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.UpdateLocationNfs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLocationNfsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
