...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationFsxOpenZfsInput, options: Options) !CreateLocationFsxOpenZfsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocationFsxOpenZfsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.CreateLocationFsxOpenZfs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLocationFsxOpenZfsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationFsxOpenZfsOutput, body, alloc);
}
