...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLocationSmbInput, options: Options) !DescribeLocationSmbOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLocationSmbInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.DescribeLocationSmb");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLocationSmbOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeLocationSmbOutput, body, alloc);
}
