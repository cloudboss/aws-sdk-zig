...
pub const CreateLocationFsxLustreInput = struct {
    ...
    /// system must
    /// also allow access from the security groups. For information about file
    /// system access, see the
    /// [
...
pub const CreateLocationFsxLustreOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationFsxLustreInput, options: CallOptions) !CreateLocationFsxLustreOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "datasync");
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLocationFsxLustreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "FmrsService.CreateLocationFsxLustre");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLocationFsxLustreOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationFsxLustreOutput, body, allocator);
}
