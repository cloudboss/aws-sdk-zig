...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationSmbInput, options: Options) !CreateLocationSmbOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocationSmbInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.CreateLocationSmb");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLocationSmbOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationSmbOutput, body, alloc);
}
