...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLocationAzureBlobInput, options: Options) !DescribeLocationAzureBlobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLocationAzureBlobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.DescribeLocationAzureBlob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLocationAzureBlobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeLocationAzureBlobOutput, body, alloc);
}
