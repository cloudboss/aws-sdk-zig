...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLocationFsxWindowsInput, options: Options) !UpdateLocationFsxWindowsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLocationFsxWindowsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.UpdateLocationFsxWindows");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLocationFsxWindowsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
