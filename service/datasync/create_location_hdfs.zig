...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationHdfsInput, options: Options) !CreateLocationHdfsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLocationHdfsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.CreateLocationHdfs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLocationHdfsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationHdfsOutput, body, alloc);
}
