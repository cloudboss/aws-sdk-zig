...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLocationFsxOpenZfsInput, options: Options) !DescribeLocationFsxOpenZfsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLocationFsxOpenZfsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.DescribeLocationFsxOpenZfs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLocationFsxOpenZfsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeLocationFsxOpenZfsOutput, body, alloc);
}
