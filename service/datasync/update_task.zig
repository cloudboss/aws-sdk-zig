...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTaskInput, options: Options) !UpdateTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "FmrsService.UpdateTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTaskOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
