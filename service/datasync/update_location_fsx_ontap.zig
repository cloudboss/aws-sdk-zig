...
const FsxUpdateProtocol = @import("fsx_update_protocol.zig").FsxUpdateProtocol;

pub const UpdateLocationFsxOntapInput = struct {
    ...
    protocol: ?FsxUpdateProtocol = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLocationFsxOntapInput, options: Options) !UpdateLocationFsxOntapOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateLocationFsxOntapInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "FmrsService.UpdateLocationFsxOntap");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateLocationFsxOntapOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
