...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetadataTransferJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetadataTransferJobOutput {
    var result: GetMetadataTransferJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMetadataTransferJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
