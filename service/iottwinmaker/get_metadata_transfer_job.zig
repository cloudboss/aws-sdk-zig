...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMetadataTransferJobInput, options: Options) !GetMetadataTransferJobOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMetadataTransferJobOutput {
    var result: GetMetadataTransferJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMetadataTransferJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
