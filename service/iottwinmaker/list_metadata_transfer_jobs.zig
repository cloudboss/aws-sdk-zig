...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMetadataTransferJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"destinationType\":");
    try aws.json.writeValue(@TypeOf(input.destination_type), input.destination_type, alloc, &body_buf);
    has_prev = true;
    if (input.filters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMetadataTransferJobsOutput {
    var result: ListMetadataTransferJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMetadataTransferJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
