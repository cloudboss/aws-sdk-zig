...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSyncJobInput, options: Options) !GetSyncJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSyncJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.workspace_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "workspace=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSyncJobOutput {
    var result: GetSyncJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSyncJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
