...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSceneInput, options: Options) !UpdateSceneOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSceneInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSceneOutput {
    var result: UpdateSceneOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSceneOutput, body, alloc);
    }
    _ = status;
    _ = headers;
