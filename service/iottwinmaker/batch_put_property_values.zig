...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchPutPropertyValuesInput, options: Options) !BatchPutPropertyValuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchPutPropertyValuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/entity-properties");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchPutPropertyValuesOutput {
    var result: BatchPutPropertyValuesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchPutPropertyValuesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
