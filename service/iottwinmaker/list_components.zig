...
fn serializeRequest(alloc: std.mem.Allocator, input: ListComponentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/components-list");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListComponentsOutput {
    var result: ListComponentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListComponentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
