...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPricingPlanInput, options: Options) !GetPricingPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPricingPlanInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPricingPlanOutput {
    var result: GetPricingPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPricingPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
