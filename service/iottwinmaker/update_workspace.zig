...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkspaceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.s_3_location) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"s3Location\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkspaceOutput {
    var result: UpdateWorkspaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateWorkspaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
