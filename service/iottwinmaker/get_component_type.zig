...
fn serializeRequest(alloc: std.mem.Allocator, input: GetComponentTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetComponentTypeOutput {
    var result: GetComponentTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetComponentTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
