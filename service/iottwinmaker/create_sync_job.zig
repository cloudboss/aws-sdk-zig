...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSyncJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"syncRole\":");
    try aws.json.writeValue(@TypeOf(input.sync_role), input.sync_role, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSyncJobOutput {
    var result: CreateSyncJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSyncJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
