...
pub const CreateSyncJobInput = struct {
...
pub const CreateSyncJobOutput = struct {
    /// The SyncJob ARN.
    arn: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSyncJobInput, options: CallOptions) !CreateSyncJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSyncJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"syncRole\":");
    try aws.json.writeValue(@TypeOf(input.sync_role), input.sync_role, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSyncJobOutput {
    var result: CreateSyncJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSyncJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
