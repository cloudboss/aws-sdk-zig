...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEntityInput, options: Options) !DeleteEntityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteEntityInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.is_recursive) |v| {
        ...
        try query_buf.appendSlice(alloc, "isRecursive=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteEntityOutput {
    var result: DeleteEntityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteEntityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
