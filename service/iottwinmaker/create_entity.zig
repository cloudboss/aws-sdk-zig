...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEntityInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.composite_components) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"compositeComponents\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    try aws.json.writeValue(@TypeOf(input.entity_name), input.entity_name, alloc, &body_buf);
    has_prev = true;
    if (input.parent_entity_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"parentEntityId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEntityOutput {
    var result: CreateEntityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEntityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
