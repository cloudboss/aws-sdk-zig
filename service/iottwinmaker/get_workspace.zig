...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkspaceInput, options: Options) !GetWorkspaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkspaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkspaceOutput {
    var result: GetWorkspaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkspaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
