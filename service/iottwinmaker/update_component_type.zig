...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateComponentTypeInput, options: Options) !UpdateComponentTypeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateComponentTypeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateComponentTypeOutput {
    var result: UpdateComponentTypeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateComponentTypeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
