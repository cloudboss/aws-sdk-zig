...
const SceneError = @import("scene_error.zig").SceneError;

pub const GetSceneInput = struct {
    /// The ID of the scene.
    scene_id: []const u8,
...
pub const GetSceneOutput = struct {
    ...
    generated_scene_metadata: ?[]const aws.map.StringMapEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSceneInput, options: Options) !GetSceneOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSceneInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.scene_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSceneOutput {
    var result: GetSceneOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSceneOutput, body, alloc);
    }
    _ = status;
    _ = headers;
