...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSceneInput, options: CallOptions) !GetSceneOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSceneInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSceneOutput {
    var result: GetSceneOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSceneOutput, body, allocator);
    }
    _ = status;
    _ = headers;
