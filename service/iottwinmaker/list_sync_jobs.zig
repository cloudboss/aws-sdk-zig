...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSyncJobsInput, options: Options) !ListSyncJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSyncJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sync-jobs-list");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSyncJobsOutput {
    var result: ListSyncJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSyncJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
