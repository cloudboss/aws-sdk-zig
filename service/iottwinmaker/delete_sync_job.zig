...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSyncJobInput, options: Options) !DeleteSyncJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSyncJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSyncJobOutput {
    var result: DeleteSyncJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteSyncJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
