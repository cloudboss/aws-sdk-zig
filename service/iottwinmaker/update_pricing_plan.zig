...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePricingPlanInput, options: Options) !UpdatePricingPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePricingPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.bundle_names) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"bundleNames\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"pricingMode\":");
    try aws.json.writeValue(@TypeOf(input.pricing_mode), input.pricing_mode, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePricingPlanOutput {
    var result: UpdatePricingPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePricingPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
