...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSceneInput, options: Options) !CreateSceneOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSceneInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/scenes");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"contentLocation\":");
    try aws.json.writeValue(@TypeOf(input.content_location), input.content_location, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sceneId\":");
    try aws.json.writeValue(@TypeOf(input.scene_id), input.scene_id, alloc, &body_buf);
    has_prev = true;
    if (input.scene_metadata) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sceneMetadata\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSceneOutput {
    var result: CreateSceneOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSceneOutput, body, alloc);
    }
    _ = status;
    _ = headers;
