...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMetadataTransferJobInput, options: Options) !CreateMetadataTransferJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMetadataTransferJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.metadata_transfer_job_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"metadataTransferJobId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMetadataTransferJobOutput {
    var result: CreateMetadataTransferJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMetadataTransferJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
