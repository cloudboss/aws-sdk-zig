...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPropertiesInput, options: Options) !ListPropertiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/properties-list");
    ...
    try aws.json.writeValue(@TypeOf(input.entity_id), input.entity_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPropertiesOutput {
    var result: ListPropertiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPropertiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
