...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetTokenBalanceInput, options: Options) !BatchGetTokenBalanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetTokenBalanceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.get_token_balance_inputs) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"getTokenBalanceInputs\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetTokenBalanceOutput {
    var result: BatchGetTokenBalanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetTokenBalanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
