...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTokenBalancesInput, options: Options) !ListTokenBalancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTokenBalancesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.owner_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ownerFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"tokenFilter\":");
    try aws.json.writeValue(@TypeOf(input.token_filter), input.token_filter, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTokenBalancesOutput {
    var result: ListTokenBalancesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTokenBalancesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
