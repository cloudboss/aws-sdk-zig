...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAssetContractInput, options: Options) !GetAssetContractOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAssetContractInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"contractIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.contract_identifier), input.contract_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAssetContractOutput {
    var result: GetAssetContractOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAssetContractOutput, body, alloc);
    }
    _ = status;
    _ = headers;
