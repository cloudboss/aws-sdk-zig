...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFilteredTransactionEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"addressIdentifierFilter\":");
    try aws.json.writeValue(@TypeOf(input.address_identifier_filter), input.address_identifier_filter, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.time_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"timeFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.vout_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"voutFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFilteredTransactionEventsOutput {
    var result: ListFilteredTransactionEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFilteredTransactionEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
