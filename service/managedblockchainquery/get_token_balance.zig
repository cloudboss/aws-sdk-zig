...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTokenBalanceInput, options: Options) !GetTokenBalanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTokenBalanceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.at_blockchain_instant) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"atBlockchainInstant\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ownerIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.owner_identifier), input.owner_identifier, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"tokenIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.token_identifier), input.token_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTokenBalanceOutput {
    var result: GetTokenBalanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTokenBalanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
