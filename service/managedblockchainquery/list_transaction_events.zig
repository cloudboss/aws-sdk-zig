...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTransactionEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.transaction_hash) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"transactionHash\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTransactionEventsOutput {
    var result: ListTransactionEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTransactionEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
