...
pub const ListTransactionEventsInput = struct {
...
pub const ListTransactionEventsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTransactionEventsInput, options: CallOptions) !ListTransactionEventsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTransactionEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/list-transaction-events";
    ...
    if (input.transaction_hash) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"transactionHash\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTransactionEventsOutput {
    var result: ListTransactionEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTransactionEventsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
