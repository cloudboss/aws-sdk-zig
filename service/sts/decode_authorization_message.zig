...
pub const DecodeAuthorizationMessageInput = struct {
    /// The encoded message that was returned with the response.
    encoded_message: []const u8,
};

pub const DecodeAuthorizationMessageOutput = struct {
    /// The API returns a response with the decoded message.
    decoded_message: ?[]const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DecodeAuthorizationMessageInput, options: CallOptions) !DecodeAuthorizationMessageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DecodeAuthorizationMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DecodeAuthorizationMessage&Version=2011-06-15");
    try body_buf.appendSlice(allocator, "&EncodedMessage=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.encoded_message);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DecodeAuthorizationMessageOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DecodedMessage")) {
                    result.decoded_message = try allocator.dupe(u8, try reader.readElementText());
                } else {
