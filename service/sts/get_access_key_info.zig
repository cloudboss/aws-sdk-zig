...
/// exist.
pub const GetAccessKeyInfoInput = struct {
...
pub const GetAccessKeyInfoOutput = struct {
    /// The number used to identify the Amazon Web Services account.
    account: ?[]const u8 = null,

    _arena: std.heap.ArenaAllocator = undefined,

    pub fn deinit(self: *GetAccessKeyInfoOutput) void {
        self._arena.deinit();
    }
};
...
pub fn execute(client: *Client, input: GetAccessKeyInfoInput, options: Options) !GetAccessKeyInfoOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessKeyInfoInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetAccessKeyInfo&Version=2011-06-15");
    try body_buf.appendSlice(alloc, "&AccessKeyId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.access_key_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessKeyInfoOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetAccessKeyInfoResult")) break;
            },
            else => {},
        }
    }

    var result: GetAccessKeyInfoOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
