...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutPartnerEventsInput, options: Options) !PutPartnerEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutPartnerEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.PutPartnerEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutPartnerEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutPartnerEventsOutput, body, alloc);
}
