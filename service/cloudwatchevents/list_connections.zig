...
const ListConnectionsInput = @import("list_connections_request.zig").ListConnectionsRequest;

const ListConnectionsOutput = @import("list_connections_response.zig").ListConnectionsResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConnectionsInput, options: Options) !ListConnectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListConnections");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConnectionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListConnectionsOutput, body, alloc);
}
