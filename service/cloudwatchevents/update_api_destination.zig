...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateApiDestinationInput, options: CallOptions) !UpdateApiDestinationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateApiDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSEvents.UpdateApiDestination");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateApiDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateApiDestinationOutput, body, allocator);
}
