...
pub const DeleteApiDestinationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteApiDestinationInput, options: Options) !DeleteApiDestinationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteApiDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSEvents.DeleteApiDestination");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteApiDestinationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
