...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEventSourcesInput, options: Options) !ListEventSourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListEventSources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEventSourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEventSourcesOutput, body, alloc);
}
