...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateArchiveInput, options: Options) !CreateArchiveOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateArchiveInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.CreateArchive");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateArchiveOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateArchiveOutput, body, alloc);
}
