...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPartnerEventSourceAccountsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListPartnerEventSourceAccounts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPartnerEventSourceAccountsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPartnerEventSourceAccountsOutput, body, alloc);
}
