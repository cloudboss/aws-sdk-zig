...
pub const DisableRuleInput = struct {
...
pub const DisableRuleOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableRuleInput, options: CallOptions) !DisableRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSEvents.DisableRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
