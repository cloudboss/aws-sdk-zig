...
pub const TestEventPatternInput = struct {
    ...
    /// Events](https://docs.aws.amazon.com/eventbridge/latest/userguide/aws-events.html), and the following
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestEventPatternInput, options: Options) !TestEventPatternOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestEventPatternInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.TestEventPattern");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestEventPatternOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TestEventPatternOutput, body, alloc);
}
