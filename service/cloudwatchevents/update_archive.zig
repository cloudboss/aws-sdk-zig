...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateArchiveInput, options: Options) !UpdateArchiveOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateArchiveInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.UpdateArchive");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateArchiveOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateArchiveOutput, body, alloc);
}
