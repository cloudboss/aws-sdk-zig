...
fn serializeRequest(alloc: std.mem.Allocator, input: DeactivateEventSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.DeactivateEventSource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeactivateEventSourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
