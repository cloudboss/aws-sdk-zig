...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReplaysInput, options: Options) !ListReplaysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReplaysInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListReplays");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReplaysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListReplaysOutput, body, alloc);
}
