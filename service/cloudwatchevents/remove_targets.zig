...
const RemoveTargetsResultEntry = @import("remove_targets_result_entry.zig").RemoveTargetsResultEntry;

pub const RemoveTargetsInput = struct {
...
pub const RemoveTargetsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveTargetsInput, options: CallOptions) !RemoveTargetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSEvents.RemoveTargets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveTargetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RemoveTargetsOutput, body, allocator);
}
