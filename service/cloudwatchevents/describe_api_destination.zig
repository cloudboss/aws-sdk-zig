...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeApiDestinationInput, options: Options) !DescribeApiDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeApiDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.DescribeApiDestination");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeApiDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeApiDestinationOutput, body, alloc);
}
