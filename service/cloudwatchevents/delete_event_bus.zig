...
pub const DeleteEventBusInput = struct {
...
pub const DeleteEventBusOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEventBusInput, options: CallOptions) !DeleteEventBusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteEventBusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSEvents.DeleteEventBus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteEventBusOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
