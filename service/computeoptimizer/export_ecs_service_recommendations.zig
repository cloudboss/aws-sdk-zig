...
const ExportableECSServiceField = @import("exportable_ecs_service_field.zig").ExportableECSServiceField;
...
pub const ExportECSServiceRecommendationsInput = struct {
    ...
    /// Guide*.
    fields_to_export: ?[]const ExportableECSServiceField = null,
...
pub const ExportECSServiceRecommendationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportECSServiceRecommendationsInput, options: CallOptions) !ExportECSServiceRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExportECSServiceRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerService.ExportECSServiceRecommendations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExportECSServiceRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportECSServiceRecommendationsOutput, body, allocator);
}
