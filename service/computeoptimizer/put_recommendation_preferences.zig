...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRecommendationPreferencesInput, options: Options) !PutRecommendationPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRecommendationPreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.PutRecommendationPreferences");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRecommendationPreferencesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
