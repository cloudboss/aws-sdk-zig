...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRecommendationPreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.DeleteRecommendationPreferences");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRecommendationPreferencesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
