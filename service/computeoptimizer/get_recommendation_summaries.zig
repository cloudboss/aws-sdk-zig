...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRecommendationSummariesInput, options: Options) !GetRecommendationSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRecommendationSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.GetRecommendationSummaries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRecommendationSummariesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRecommendationSummariesOutput, body, alloc);
}
