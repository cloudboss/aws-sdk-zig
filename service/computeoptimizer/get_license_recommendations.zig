...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLicenseRecommendationsInput, options: Options) !GetLicenseRecommendationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLicenseRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.GetLicenseRecommendations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLicenseRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLicenseRecommendationsOutput, body, alloc);
}
