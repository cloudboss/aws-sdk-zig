...
const ExportableInstanceField = @import("exportable_instance_field.zig").ExportableInstanceField;
...
pub const ExportEC2InstanceRecommendationsInput = struct {
    ...
    /// Guide*.
    fields_to_export: ?[]const ExportableInstanceField = null,
    ...
    /// If you plan to
    /// specify an object prefix when you create the export job, you must include
    /// the object
    /// prefix in the policy that you add to the S3 bucket. For more information,
    /// see [Amazon S3 Bucket Policy for Compute
...
pub const ExportEC2InstanceRecommendationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportEC2InstanceRecommendationsInput, options: CallOptions) !ExportEC2InstanceRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExportEC2InstanceRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerService.ExportEC2InstanceRecommendations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExportEC2InstanceRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportEC2InstanceRecommendationsOutput, body, allocator);
}
