...
fn serializeRequest(alloc: std.mem.Allocator, input: ExportEC2InstanceRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.ExportEC2InstanceRecommendations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExportEC2InstanceRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportEC2InstanceRecommendationsOutput, body, alloc);
}
