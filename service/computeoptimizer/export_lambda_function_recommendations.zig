...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportLambdaFunctionRecommendationsInput, options: Options) !ExportLambdaFunctionRecommendationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExportLambdaFunctionRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.ExportLambdaFunctionRecommendations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExportLambdaFunctionRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportLambdaFunctionRecommendationsOutput, body, alloc);
}
