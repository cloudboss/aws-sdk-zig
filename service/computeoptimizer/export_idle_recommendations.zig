...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportIdleRecommendationsInput, options: CallOptions) !ExportIdleRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExportIdleRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerService.ExportIdleRecommendations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExportIdleRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportIdleRecommendationsOutput, body, allocator);
}
