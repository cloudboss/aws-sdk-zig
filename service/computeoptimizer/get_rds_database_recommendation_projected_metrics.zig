...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRDSDatabaseRecommendationProjectedMetricsInput, options: Options) !GetRDSDatabaseRecommendationProjectedMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRDSDatabaseRecommendationProjectedMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.GetRDSDatabaseRecommendationProjectedMetrics");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRDSDatabaseRecommendationProjectedMetricsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRDSDatabaseRecommendationProjectedMetricsOutput, body, alloc);
}
