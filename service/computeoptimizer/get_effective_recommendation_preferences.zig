...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEffectiveRecommendationPreferencesInput, options: Options) !GetEffectiveRecommendationPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEffectiveRecommendationPreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.GetEffectiveRecommendationPreferences");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEffectiveRecommendationPreferencesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEffectiveRecommendationPreferencesOutput, body, alloc);
}
