...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportLicenseRecommendationsInput, options: Options) !ExportLicenseRecommendationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExportLicenseRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.ExportLicenseRecommendations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExportLicenseRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportLicenseRecommendationsOutput, body, alloc);
}
