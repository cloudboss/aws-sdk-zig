...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRecommendationExportJobsInput, options: Options) !DescribeRecommendationExportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRecommendationExportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.DescribeRecommendationExportJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRecommendationExportJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRecommendationExportJobsOutput, body, alloc);
}
