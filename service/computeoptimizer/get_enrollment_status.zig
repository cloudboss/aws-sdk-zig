...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEnrollmentStatusInput, options: Options) !GetEnrollmentStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEnrollmentStatusInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.GetEnrollmentStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEnrollmentStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEnrollmentStatusOutput, body, alloc);
}
