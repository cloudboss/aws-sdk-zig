...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportRDSDatabaseRecommendationsInput, options: Options) !ExportRDSDatabaseRecommendationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExportRDSDatabaseRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.ExportRDSDatabaseRecommendations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExportRDSDatabaseRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportRDSDatabaseRecommendationsOutput, body, alloc);
}
