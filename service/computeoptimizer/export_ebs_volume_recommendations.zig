...
const ExportableVolumeField = @import("exportable_volume_field.zig").ExportableVolumeField;
...
pub const ExportEBSVolumeRecommendationsInput = struct {
    ...
    /// Guide*.
    fields_to_export: ?[]const ExportableVolumeField = null,
...
pub const ExportEBSVolumeRecommendationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportEBSVolumeRecommendationsInput, options: CallOptions) !ExportEBSVolumeRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExportEBSVolumeRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerService.ExportEBSVolumeRecommendations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExportEBSVolumeRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportEBSVolumeRecommendationsOutput, body, allocator);
}
