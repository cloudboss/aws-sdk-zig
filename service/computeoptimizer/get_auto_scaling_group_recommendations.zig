...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAutoScalingGroupRecommendationsInput, options: CallOptions) !GetAutoScalingGroupRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAutoScalingGroupRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerService.GetAutoScalingGroupRecommendations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAutoScalingGroupRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAutoScalingGroupRecommendationsOutput, body, allocator);
}
