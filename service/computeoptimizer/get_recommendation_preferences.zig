...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRecommendationPreferencesInput, options: Options) !GetRecommendationPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRecommendationPreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ComputeOptimizerService.GetRecommendationPreferences");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRecommendationPreferencesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRecommendationPreferencesOutput, body, alloc);
}
