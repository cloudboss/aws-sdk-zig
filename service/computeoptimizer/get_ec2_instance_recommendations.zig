...
const InstanceRecommendation = @import("instance_recommendation.zig").InstanceRecommendation;

pub const GetEC2InstanceRecommendationsInput = struct {
    ...
    /// recommendations.
    instance_arns: ?[]const []const u8 = null,
...
pub const GetEC2InstanceRecommendationsOutput = struct {
    ...
    instance_recommendations: ?[]const InstanceRecommendation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEC2InstanceRecommendationsInput, options: CallOptions) !GetEC2InstanceRecommendationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetEC2InstanceRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ComputeOptimizerService.GetEC2InstanceRecommendations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetEC2InstanceRecommendationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetEC2InstanceRecommendationsOutput, body, allocator);
}
