...
pub const GetNetworkInput = struct {
...
pub const GetNetworkOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNetworkInput, options: Options) !GetNetworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNetworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNetworkOutput {
    var result: GetNetworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNetworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
