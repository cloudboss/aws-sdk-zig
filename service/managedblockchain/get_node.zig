...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNodeInput, options: Options) !GetNodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.member_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "memberId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNodeOutput {
    var result: GetNodeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNodeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
