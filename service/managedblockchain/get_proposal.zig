...
pub const GetProposalInput = struct {
...
pub const GetProposalOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProposalInput, options: Options) !GetProposalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProposalInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProposalOutput {
    var result: GetProposalOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProposalOutput, body, alloc);
    }
    _ = status;
    _ = headers;
