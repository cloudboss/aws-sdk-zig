...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetProposalInput, options: CallOptions) !GetProposalOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetProposalInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetProposalOutput {
    var result: GetProposalOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProposalOutput, body, allocator);
    }
    _ = status;
    _ = headers;
