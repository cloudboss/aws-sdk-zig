...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProposalsInput, options: CallOptions) !ListProposalsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListProposalsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListProposalsOutput {
    var result: ListProposalsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProposalsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
