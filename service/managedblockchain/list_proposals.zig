...
pub const ListProposalsInput = struct {
...
pub const ListProposalsOutput = struct {
    /// The pagination token that indicates the next set of results to retrieve.
    next_token: ?[]const u8 = null,

    /// The summary of each proposal made on the network.
    proposals: ?[]const ProposalSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProposalsInput, options: Options) !ListProposalsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProposalsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProposalsOutput {
    var result: ListProposalsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProposalsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
