...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VoteOnProposalInput, options: Options) !VoteOnProposalOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: VoteOnProposalInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"Vote\":");
    try aws.json.writeValue(@TypeOf(input.vote), input.vote, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VoterMemberId\":");
    try aws.json.writeValue(@TypeOf(input.voter_member_id), input.voter_member_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VoteOnProposalOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
