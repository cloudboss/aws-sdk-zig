...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessorsInput, options: Options) !ListAccessorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessorsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.network_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "networkType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessorsOutput {
    var result: ListAccessorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccessorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
