...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProposalInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProposalOutput {
    var result: CreateProposalOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProposalOutput, body, alloc);
    }
    _ = status;
    _ = headers;
