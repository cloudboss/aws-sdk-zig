...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccessorInput, options: Options) !CreateAccessorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccessorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AccessorType\":");
    try aws.json.writeValue(@TypeOf(input.accessor_type), input.accessor_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccessorOutput {
    var result: CreateAccessorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAccessorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
