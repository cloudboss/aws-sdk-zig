...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.access_type) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AccessType\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OutpostId\":");
    try aws.json.writeValue(@TypeOf(input.outpost_id), input.outpost_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SecurityGroupId\":");
    try aws.json.writeValue(@TypeOf(input.security_group_id), input.security_group_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEndpointOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
