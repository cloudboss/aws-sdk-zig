...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOutpostsWithS3Input, options: Options) !ListOutpostsWithS3Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOutpostsWithS3Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOutpostsWithS3Output {
    var result: ListOutpostsWithS3Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOutpostsWithS3Output, body, alloc);
    }
    _ = status;
    _ = headers;
