...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSavingsPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"commitment\":");
    try aws.json.writeValue(@TypeOf(input.commitment), input.commitment, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"savingsPlanOfferingId\":");
    try aws.json.writeValue(@TypeOf(input.savings_plan_offering_id), input.savings_plan_offering_id, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.upfront_payment_amount) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"upfrontPaymentAmount\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSavingsPlanOutput {
    var result: CreateSavingsPlanOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSavingsPlanOutput, body, alloc);
    }
    _ = status;
    _ = headers;
