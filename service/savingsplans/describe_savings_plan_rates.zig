...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSavingsPlanRatesInput, options: Options) !DescribeSavingsPlanRatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSavingsPlanRatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSavingsPlanRatesOutput {
    var result: DescribeSavingsPlanRatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSavingsPlanRatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
