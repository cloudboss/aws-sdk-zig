...
pub const DeleteTapeInput = struct {
...
pub const DeleteTapeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTapeInput, options: CallOptions) !DeleteTapeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTapeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.DeleteTape");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTapeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteTapeOutput, body, allocator);
}
