...
pub const DescribeCacheReportInput = struct {
...
pub const DescribeCacheReportOutput = struct {
    ...
    cache_report_info: ?CacheReportInfo = null,

    pub const json_field_names = .{
        .cache_report_info = "CacheReportInfo",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCacheReportInput, options: CallOptions) !DescribeCacheReportOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeCacheReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.DescribeCacheReport");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeCacheReportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCacheReportOutput, body, allocator);
}
