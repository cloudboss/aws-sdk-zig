...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSMBSecurityStrategyInput, options: CallOptions) !UpdateSMBSecurityStrategyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSMBSecurityStrategyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.UpdateSMBSecurityStrategy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateSMBSecurityStrategyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSMBSecurityStrategyOutput, body, allocator);
}
