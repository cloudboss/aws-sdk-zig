...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTapeWithBarcodeInput, options: CallOptions) !CreateTapeWithBarcodeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTapeWithBarcodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.CreateTapeWithBarcode");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTapeWithBarcodeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTapeWithBarcodeOutput, body, allocator);
}
