...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTapeArchiveInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StorageGateway_20130630.DeleteTapeArchive");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTapeArchiveOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteTapeArchiveOutput, body, alloc);
}
