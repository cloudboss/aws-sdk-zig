...
pub const DeleteTapeArchiveInput = struct {
...
pub const DeleteTapeArchiveOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTapeArchiveInput, options: CallOptions) !DeleteTapeArchiveOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTapeArchiveInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.DeleteTapeArchive");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTapeArchiveOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteTapeArchiveOutput, body, allocator);
}
