...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAvailabilityMonitorTestInput, options: Options) !DescribeAvailabilityMonitorTestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAvailabilityMonitorTestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StorageGateway_20130630.DescribeAvailabilityMonitorTest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAvailabilityMonitorTestOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAvailabilityMonitorTestOutput, body, alloc);
}
