...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCacheInput, options: CallOptions) !DescribeCacheOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeCacheInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.DescribeCache");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeCacheOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeCacheOutput, body, allocator);
}
