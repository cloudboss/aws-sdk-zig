...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTapeArchivesInput, options: CallOptions) !DescribeTapeArchivesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTapeArchivesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.DescribeTapeArchives");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTapeArchivesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTapeArchivesOutput, body, allocator);
}
