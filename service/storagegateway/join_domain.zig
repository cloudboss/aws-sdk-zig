...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: JoinDomainInput, options: CallOptions) !JoinDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: JoinDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.JoinDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !JoinDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(JoinDomainOutput, body, allocator);
}
