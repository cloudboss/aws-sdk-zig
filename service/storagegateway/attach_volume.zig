...
pub const AttachVolumeInput = struct {
    ...
    /// disk used to
    /// create the volume. This value is only required when you are attaching a
    /// stored
    /// volume.
...
fn serializeRequest(allocator: std.mem.Allocator, input: AttachVolumeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.AttachVolume");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AttachVolumeOutput {
    ...
    return aws.json.parseJsonObject(AttachVolumeOutput, body, allocator);
}
