...
pub const CreateCachediSCSIVolumeInput = struct {
    ...
    /// `VolumeSizeInBytes` value for this new volume must be equal to or larger
    /// than
    /// the size of the existing volume, in bytes.
    source_volume_arn: ?[]const u8 = null,
...
pub const CreateCachediSCSIVolumeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCachediSCSIVolumeInput, options: CallOptions) !CreateCachediSCSIVolumeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateCachediSCSIVolumeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.CreateCachediSCSIVolume");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateCachediSCSIVolumeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCachediSCSIVolumeOutput, body, allocator);
}
