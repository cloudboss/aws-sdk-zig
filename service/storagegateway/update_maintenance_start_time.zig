...
pub const UpdateMaintenanceStartTimeInput = struct {
    ...
    /// as an
    /// ordinal number from 0 to 6, where 0 represents Sunday and 6 represents
    /// Saturday.
...
pub const UpdateMaintenanceStartTimeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMaintenanceStartTimeInput, options: CallOptions) !UpdateMaintenanceStartTimeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateMaintenanceStartTimeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.UpdateMaintenanceStartTime");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateMaintenanceStartTimeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateMaintenanceStartTimeOutput, body, allocator);
}
