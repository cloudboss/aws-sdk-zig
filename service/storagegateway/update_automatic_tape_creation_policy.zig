...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAutomaticTapeCreationPolicyInput, options: Options) !UpdateAutomaticTapeCreationPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAutomaticTapeCreationPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StorageGateway_20130630.UpdateAutomaticTapeCreationPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAutomaticTapeCreationPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateAutomaticTapeCreationPolicyOutput, body, alloc);
}
