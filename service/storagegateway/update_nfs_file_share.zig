...
pub const UpdateNFSFileShareInput = struct {
...
pub const UpdateNFSFileShareOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNFSFileShareInput, options: CallOptions) !UpdateNFSFileShareOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateNFSFileShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.UpdateNFSFileShare");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateNFSFileShareOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateNFSFileShareOutput, body, allocator);
}
