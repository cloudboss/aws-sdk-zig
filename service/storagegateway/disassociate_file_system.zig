...
pub const DisassociateFileSystemInput = struct {
...
pub const DisassociateFileSystemOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateFileSystemInput, options: CallOptions) !DisassociateFileSystemOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateFileSystemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.DisassociateFileSystem");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateFileSystemOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateFileSystemOutput, body, allocator);
}
