...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBandwidthRateLimitInput, options: Options) !DescribeBandwidthRateLimitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBandwidthRateLimitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StorageGateway_20130630.DescribeBandwidthRateLimit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBandwidthRateLimitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeBandwidthRateLimitOutput, body, alloc);
}
