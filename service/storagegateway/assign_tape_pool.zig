...
pub const AssignTapePoolInput = struct {
    ...
    /// the retention lock. This parameter is set to true by default for calls from
    /// the
    /// console.
    ///
...
pub const AssignTapePoolOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssignTapePoolInput, options: CallOptions) !AssignTapePoolOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssignTapePoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.AssignTapePool");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssignTapePoolOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssignTapePoolOutput, body, allocator);
}
