...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGatewayInformationInput, options: CallOptions) !DescribeGatewayInformationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeGatewayInformationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.DescribeGatewayInformation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeGatewayInformationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeGatewayInformationOutput, body, allocator);
}
