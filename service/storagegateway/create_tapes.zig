...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTapesInput, options: CallOptions) !CreateTapesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTapesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.CreateTapes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTapesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTapesOutput, body, allocator);
}
