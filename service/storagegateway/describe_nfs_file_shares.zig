...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNFSFileSharesInput, options: CallOptions) !DescribeNFSFileSharesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeNFSFileSharesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "StorageGateway_20130630.DescribeNFSFileShares");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeNFSFileSharesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeNFSFileSharesOutput, body, allocator);
}
