...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ActivateGatewayInput, options: Options) !ActivateGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ActivateGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StorageGateway_20130630.ActivateGateway");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ActivateGatewayOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ActivateGatewayOutput, body, alloc);
}
