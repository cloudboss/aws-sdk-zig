...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChapCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StorageGateway_20130630.UpdateChapCredentials");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChapCredentialsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateChapCredentialsOutput, body, alloc);
}
