...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteFileShareInput, options: Options) !DeleteFileShareOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteFileShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "StorageGateway_20130630.DeleteFileShare");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteFileShareOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteFileShareOutput, body, alloc);
}
