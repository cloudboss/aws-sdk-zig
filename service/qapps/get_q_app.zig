...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQAppInput, options: Options) !GetQAppOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQAppInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.app_version) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "appVersion=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQAppOutput {
    var result: GetQAppOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQAppOutput, body, alloc);
    }
    _ = status;
    _ = headers;
