...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLibraryItemInput, options: Options) !GetLibraryItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLibraryItemInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "libraryItemId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.library_item_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLibraryItemOutput {
    var result: GetLibraryItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLibraryItemOutput, body, alloc);
    }
    _ = status;
    _ = headers;
