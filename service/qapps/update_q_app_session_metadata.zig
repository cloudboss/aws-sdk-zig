...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateQAppSessionMetadataInput, options: Options) !UpdateQAppSessionMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQAppSessionMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.session_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sessionName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sharingConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.sharing_configuration), input.sharing_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQAppSessionMetadataOutput {
    var result: UpdateQAppSessionMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQAppSessionMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
