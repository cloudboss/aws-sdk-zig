...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLibraryItemInput, options: Options) !CreateLibraryItemOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLibraryItemInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLibraryItemOutput {
    var result: CreateLibraryItemOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLibraryItemOutput, body, alloc);
    }
    _ = status;
    _ = headers;
