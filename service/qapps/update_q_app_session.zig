...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateQAppSessionInput, options: Options) !UpdateQAppSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQAppSessionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQAppSessionOutput {
    var result: UpdateQAppSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQAppSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
