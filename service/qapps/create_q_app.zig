...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateQAppInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.app_definition), input.app_definition, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateQAppOutput {
    var result: CreateQAppOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateQAppOutput, body, alloc);
    }
    _ = status;
    _ = headers;
