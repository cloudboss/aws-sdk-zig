...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartQAppSessionInput, options: Options) !StartQAppSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartQAppSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.initial_values) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"initialValues\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartQAppSessionOutput {
    var result: StartQAppSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartQAppSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
