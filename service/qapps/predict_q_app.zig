...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PredictQAppInput, options: Options) !PredictQAppOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PredictQAppInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PredictQAppOutput {
    var result: PredictQAppOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PredictQAppOutput, body, alloc);
    }
    _ = status;
    _ = headers;
