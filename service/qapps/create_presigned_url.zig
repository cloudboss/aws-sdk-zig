...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePresignedUrlInput, options: Options) !CreatePresignedUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePresignedUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"fileContentsSha256\":");
    try aws.json.writeValue(@TypeOf(input.file_contents_sha_256), input.file_contents_sha_256, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePresignedUrlOutput {
    var result: CreatePresignedUrlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePresignedUrlOutput, body, alloc);
    }
    _ = status;
    _ = headers;
