...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQAppPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQAppPermissionsOutput {
    var result: UpdateQAppPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQAppPermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
