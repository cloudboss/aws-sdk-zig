...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQAppSessionMetadataInput, options: Options) !GetQAppSessionMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetQAppSessionMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetQAppSessionMetadataOutput {
    var result: GetQAppSessionMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQAppSessionMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
