...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeQAppPermissionsInput, options: Options) !DescribeQAppPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeQAppPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "appId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.app_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeQAppPermissionsOutput {
    var result: DescribeQAppPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeQAppPermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
