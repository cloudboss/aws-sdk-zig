...
pub const BatchCreateCategoryInput = struct {
    /// The list of category objects to be created
    categories: []const BatchCreateCategoryInputCategory,
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchCreateCategoryInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/catalog.createCategories";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchCreateCategoryOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: BatchCreateCategoryOutput = .{};
