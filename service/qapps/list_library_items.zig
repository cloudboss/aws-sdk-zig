...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLibraryItemsInput, options: Options) !ListLibraryItemsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLibraryItemsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.category_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "categoryId=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLibraryItemsOutput {
    var result: ListLibraryItemsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLibraryItemsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
