...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQAppSessionInput, options: Options) !GetQAppSessionOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetQAppSessionOutput {
    var result: GetQAppSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQAppSessionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
