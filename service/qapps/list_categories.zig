...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCategoriesInput, options: Options) !ListCategoriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCategoriesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCategoriesOutput {
    var result: ListCategoriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCategoriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
