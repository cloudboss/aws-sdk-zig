...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQAppInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.app_definition) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"appDefinition\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQAppOutput {
    var result: UpdateQAppOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQAppOutput, body, alloc);
    }
    _ = status;
    _ = headers;
