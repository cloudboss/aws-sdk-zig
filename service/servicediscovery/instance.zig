...
pub const Instance = struct {
    ...
    /// to an Elastic Load Balancing
    /// load balancer, specify the DNS name that's associated with the load
    /// balancer. For information
    /// about how to get the DNS name, see
    /// [AliasTarget->DNSName](https://docs.aws.amazon.com/Route53/latest/APIReference/API_AliasTarget.html#Route53-Type-AliasTarget-DNSName) in the *Route 53 API Reference*.
    ///
    /// Note the following:
    ///
    ...
    /// `HealthCheckConfig` settings, Cloud Map creates the health check, but it
    /// won't
    ...
    /// updates the existing DNS records. If there's also an existing health check,
    /// Cloud Map deletes
