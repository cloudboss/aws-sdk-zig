...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceAttributesInput, options: Options) !UpdateServiceAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53AutoNaming_v20170314.UpdateServiceAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceAttributesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
