...
const ListOperationsInput = @import("list_operations_request.zig").ListOperationsRequest;

const ListOperationsOutput = @import("list_operations_response.zig").ListOperationsResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOperationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53AutoNaming_v20170314.ListOperations");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOperationsOutput {
    ...
    return aws.json.parseJsonObject(ListOperationsOutput, body, alloc);
}
