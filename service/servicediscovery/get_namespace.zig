...
pub const GetNamespaceInput = struct {
    ...
    /// information about. For namespaces
    /// shared with your Amazon Web Services account, specify the namespace ARN. For
    /// more information about shared
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNamespaceInput, options: Options) !GetNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53AutoNaming_v20170314.GetNamespace");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNamespaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetNamespaceOutput, body, alloc);
}
