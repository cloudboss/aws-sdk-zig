...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHttpNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53AutoNaming_v20170314.CreateHttpNamespace");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHttpNamespaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateHttpNamespaceOutput, body, alloc);
}
