...
pub const CreateHttpNamespaceInput = struct {
...
pub const CreateHttpNamespaceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHttpNamespaceInput, options: CallOptions) !CreateHttpNamespaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateHttpNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53AutoNaming_v20170314.CreateHttpNamespace");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateHttpNamespaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateHttpNamespaceOutput, body, allocator);
}
