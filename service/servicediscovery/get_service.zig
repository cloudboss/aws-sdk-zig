...
pub const GetServiceInput = struct {
    ...
    /// settings for. For services created
    /// by consumers in a shared namespace, specify the service ARN. For more
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53AutoNaming_v20170314.GetService");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetServiceOutput {
    ...
    return aws.json.parseJsonObject(GetServiceOutput, body, allocator);
}
