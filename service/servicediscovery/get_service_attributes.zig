...
const ServiceAttributes = @import("service_attributes.zig").ServiceAttributes;

pub const GetServiceAttributesInput = struct {
    /// The ID or Amazon Resource Name (ARN) of the service that you want to get
    /// attributes for. For services created
    /// in a namespace shared with your Amazon Web Services account, specify the
...
pub const GetServiceAttributesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceAttributesInput, options: CallOptions) !GetServiceAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetServiceAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53AutoNaming_v20170314.GetServiceAttributes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetServiceAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetServiceAttributesOutput, body, allocator);
}
