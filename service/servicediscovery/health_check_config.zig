...
/// Services endpoint. For
/// information about pricing for health checks, see [Amazon Route 53
...
/// `AAAA` records, Cloud Map creates a health check that uses the IPv4 address
/// to
/// check the health of the resource. If the endpoint tthat's specified by the
/// IPv4 address is
...
/// to an endpoint every 30 seconds. On average, your endpoint receives a health
/// check request
...
///   records.
/// When `EvaluateTargetHealth` is true, the alias record inherits the health of
/// the
/// referenced Amazon Web Services resource. such as an ELB load balancer. For
...
pub const HealthCheckConfig = struct {
    ...
    /// for Route 53 to
    /// change the current status of the endpoint from unhealthy to healthy or the
    ...
    /// The path can be any
    /// value that your endpoint returns an HTTP status code of a 2xx or 3xx format
    /// for when the endpoint
    /// is healthy. An example file is `/docs/route53-health-check.html`. Route 53
    /// automatically
