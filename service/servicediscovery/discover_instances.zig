...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DiscoverInstancesInput, options: Options) !DiscoverInstancesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DiscoverInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53AutoNaming_v20170314.DiscoverInstances");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DiscoverInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DiscoverInstancesOutput, body, alloc);
}
