...
pub const DiscoverInstancesRevisionInput = struct {
    ...
    /// must specify an `OwnerAccount`. For more information about
...
pub const DiscoverInstancesRevisionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DiscoverInstancesRevisionInput, options: CallOptions) !DiscoverInstancesRevisionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DiscoverInstancesRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53AutoNaming_v20170314.DiscoverInstancesRevision");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DiscoverInstancesRevisionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DiscoverInstancesRevisionOutput, body, allocator);
}
