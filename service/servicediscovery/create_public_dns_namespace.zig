...
const PublicDnsNamespaceProperties = @import("public_dns_namespace_properties.zig").PublicDnsNamespaceProperties;
const Tag = @import("tag.zig").Tag;

pub const CreatePublicDnsNamespaceInput = struct {
...
pub const CreatePublicDnsNamespaceOutput = struct {
    /// A value that you can use to determine whether the request completed
    /// successfully. To get the
    /// status of the operation, see
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePublicDnsNamespaceInput, options: CallOptions) !CreatePublicDnsNamespaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreatePublicDnsNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53AutoNaming_v20170314.CreatePublicDnsNamespace");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreatePublicDnsNamespaceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePublicDnsNamespaceOutput, body, allocator);
}
