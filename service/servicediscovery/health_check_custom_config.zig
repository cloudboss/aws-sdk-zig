...
/// check. A custom
/// health check, which requires that you use a third-party health checker to
...
///   during
/// that time to change the status back to healthy, Cloud Map stops routing
/// traffic to the
/// resource.
pub const HealthCheckCustomConfig = struct {
    ...
    /// receiving an
    /// `UpdateInstanceCustomHealthStatus` request before it changes the health
    /// status of a
    /// service instance.
    ///
