...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableSnapshotCopyInput, options: Options) !EnableSnapshotCopyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableSnapshotCopyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableSnapshotCopy&Version=2012-12-01");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.destination_region);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableSnapshotCopyOutput {
    _ = status;
    _ = headers;
