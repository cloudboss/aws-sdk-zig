...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInboundIntegrationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeInboundIntegrations&Version=2012-12-01");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InboundIntegrations")) {
                    result.inbound_integrations = try serde.deserializeInboundIntegrationList(&reader, alloc, "InboundIntegration");
