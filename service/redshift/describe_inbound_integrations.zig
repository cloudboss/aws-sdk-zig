...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInboundIntegrationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeInboundIntegrations&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInboundIntegrationsOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeInboundIntegrationsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeInboundIntegrationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "InboundIntegrations")) {
                    result.inbound_integrations = try serde.deserializeInboundIntegrationList(allocator, &reader, "InboundIntegration");
