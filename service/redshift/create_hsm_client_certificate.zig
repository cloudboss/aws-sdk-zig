...
pub const CreateHsmClientCertificateInput = struct {
...
pub const CreateHsmClientCertificateOutput = struct {
    hsm_client_certificate: ?HsmClientCertificate = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHsmClientCertificateInput, options: CallOptions) !CreateHsmClientCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateHsmClientCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateHsmClientCertificate&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateHsmClientCertificateOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateHsmClientCertificateResult")) break;
            },
            else => {},
        }
    }

    var result: CreateHsmClientCertificateOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HsmClientCertificate")) {
                    result.hsm_client_certificate = try serde.deserializeHsmClientCertificate(allocator, &reader);
                } else {
