...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHsmClientCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateHsmClientCertificate&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHsmClientCertificateOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HsmClientCertificate")) {
                    result.hsm_client_certificate = try serde.deserializeHsmClientCertificate(&reader, alloc);
                } else {
