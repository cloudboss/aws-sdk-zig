...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyClusterParameterGroupInput, options: Options) !ModifyClusterParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyClusterParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyClusterParameterGroup&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyClusterParameterGroupOutput {
    _ = status;
    _ = headers;
