...
const CreateScheduledActionOutput = @import("scheduled_action.zig").ScheduledAction;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateScheduledActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateScheduledAction&Version=2012-12-01");
    if (input.enable) |v| {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.iam_role);
    try body_buf.appendSlice(alloc, "&Schedule=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.schedule);
