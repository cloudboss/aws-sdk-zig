...
const ModifyEndpointAccessOutput = @import("endpoint_access.zig").EndpointAccess;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyEndpointAccessInput, options: Options) !ModifyEndpointAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyEndpointAccess&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyEndpointAccessOutput {
    _ = status;
    _ = headers;
