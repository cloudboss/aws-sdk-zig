...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyClusterSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CopyClusterSnapshot&Version=2012-12-01");
    ...
    if (input.source_snapshot_cluster_identifier) |v| {
        try body_buf.appendSlice(alloc, "&SourceSnapshotClusterIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&SourceSnapshotIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_snapshot_identifier);
    try body_buf.appendSlice(alloc, "&TargetSnapshotIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_snapshot_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyClusterSnapshotOutput {
    _ = status;
    _ = headers;
