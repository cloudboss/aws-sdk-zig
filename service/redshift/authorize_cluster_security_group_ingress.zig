...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AuthorizeClusterSecurityGroupIngressInput, options: Options) !AuthorizeClusterSecurityGroupIngressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AuthorizeClusterSecurityGroupIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AuthorizeClusterSecurityGroupIngress&Version=2012-12-01");
    if (input.cidrip) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AuthorizeClusterSecurityGroupIngressOutput {
    _ = status;
    _ = headers;
