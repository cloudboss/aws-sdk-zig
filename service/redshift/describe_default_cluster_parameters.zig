...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDefaultClusterParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDefaultClusterParameters&Version=2012-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDefaultClusterParametersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DefaultClusterParameters")) {
                    result.default_cluster_parameters = try serde.deserializeDefaultClusterParameters(&reader, alloc);
                } else {
