...
pub const DescribeDefaultClusterParametersInput = struct {
...
pub const DescribeDefaultClusterParametersOutput = struct {
    default_cluster_parameters: ?DefaultClusterParameters = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDefaultClusterParametersInput, options: CallOptions) !DescribeDefaultClusterParametersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDefaultClusterParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDefaultClusterParameters&Version=2012-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDefaultClusterParametersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDefaultClusterParametersResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDefaultClusterParametersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DefaultClusterParameters")) {
                    result.default_cluster_parameters = try serde.deserializeDefaultClusterParameters(allocator, &reader);
                } else {
