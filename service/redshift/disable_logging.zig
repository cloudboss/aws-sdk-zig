...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableLoggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisableLogging&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableLoggingOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DisableLoggingResult")) break;
            },
            else => {},
        }
    }

    var result: DisableLoggingOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
