...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyClusterSnapshotScheduleInput, options: Options) !ModifyClusterSnapshotScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyClusterSnapshotScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyClusterSnapshotSchedule&Version=2012-12-01");
    ...
    if (input.disassociate_schedule) |v| {
        try body_buf.appendSlice(alloc, "&DisassociateSchedule=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyClusterSnapshotScheduleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
