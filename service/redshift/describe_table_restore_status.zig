...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTableRestoreStatusInput, options: Options) !DescribeTableRestoreStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTableRestoreStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTableRestoreStatus&Version=2012-12-01");
    ...
    if (input.table_restore_request_id) |v| {
        try body_buf.appendSlice(alloc, "&TableRestoreRequestId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTableRestoreStatusOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.table_restore_status_details = try serde.deserializeTableRestoreStatusList(&reader, alloc, "TableRestoreStatus");
                } else {
