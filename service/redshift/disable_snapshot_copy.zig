...
pub const DisableSnapshotCopyInput = struct {
...
pub const DisableSnapshotCopyOutput = struct {
    cluster: ?Cluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableSnapshotCopyInput, options: CallOptions) !DisableSnapshotCopyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableSnapshotCopyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DisableSnapshotCopy&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableSnapshotCopyOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DisableSnapshotCopyResult")) break;
            },
            else => {},
        }
    }

    var result: DisableSnapshotCopyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
