...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateClusterSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateClusterSubnetGroup&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&ClusterSubnetGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cluster_subnet_group_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateClusterSubnetGroupOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterSubnetGroup")) {
                    result.cluster_subnet_group = try serde.deserializeClusterSubnetGroup(&reader, alloc);
                } else {
