...
pub const CreateClusterSubnetGroupInput = struct {
...
pub const CreateClusterSubnetGroupOutput = struct {
    cluster_subnet_group: ?ClusterSubnetGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateClusterSubnetGroupInput, options: CallOptions) !CreateClusterSubnetGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateClusterSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateClusterSubnetGroup&Version=2012-12-01");
    try body_buf.appendSlice(allocator, "&ClusterSubnetGroupName=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.cluster_subnet_group_name);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateClusterSubnetGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateClusterSubnetGroupResult")) break;
            },
            else => {},
        }
    }

    var result: CreateClusterSubnetGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterSubnetGroup")) {
                    result.cluster_subnet_group = try serde.deserializeClusterSubnetGroup(allocator, &reader);
                } else {
