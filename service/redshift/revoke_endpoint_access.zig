...
pub const RevokeEndpointAccessOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RevokeEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RevokeEndpointAccess&Version=2012-12-01");
    if (input.account) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RevokeEndpointAccessOutput {
    _ = status;
    _ = headers;
