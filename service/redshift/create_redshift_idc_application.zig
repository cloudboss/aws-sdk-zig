...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRedshiftIdcApplicationInput, options: Options) !CreateRedshiftIdcApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRedshiftIdcApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateRedshiftIdcApplication&Version=2012-12-01");
    ...
    try body_buf.appendSlice(alloc, "&IdcDisplayName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.idc_display_name);
    try body_buf.appendSlice(alloc, "&IdcInstanceArn=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.idc_instance_arn);
    if (input.identity_namespace) |v| {
        try body_buf.appendSlice(alloc, "&IdentityNamespace=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&RedshiftIdcApplicationName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.redshift_idc_application_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRedshiftIdcApplicationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RedshiftIdcApplication")) {
                    result.redshift_idc_application = try serde.deserializeRedshiftIdcApplication(&reader, alloc);
                } else {
