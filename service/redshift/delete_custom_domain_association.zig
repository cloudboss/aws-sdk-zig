...
pub const DeleteCustomDomainAssociationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCustomDomainAssociationInput, options: Options) !DeleteCustomDomainAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCustomDomainAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteCustomDomainAssociation&Version=2012-12-01");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.custom_domain_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCustomDomainAssociationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: DeleteCustomDomainAssociationOutput = .{};
