...
pub const DescribeAuthenticationProfilesInput = struct {
...
pub const DescribeAuthenticationProfilesOutput = struct {
    /// The list of authentication profiles.
    authentication_profiles: ?[]const AuthenticationProfile = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAuthenticationProfilesInput, options: Options) !DescribeAuthenticationProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAuthenticationProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAuthenticationProfiles&Version=2012-12-01");
    if (input.authentication_profile_name) |v| {
        try body_buf.appendSlice(alloc, "&AuthenticationProfileName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAuthenticationProfilesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeAuthenticationProfilesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeAuthenticationProfilesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AuthenticationProfiles")) {
                    result.authentication_profiles = try serde.deserializeAuthenticationProfileList(&reader, alloc, "member");
                } else {
