...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAuthenticationProfilesInput, options: CallOptions) !DescribeAuthenticationProfilesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAuthenticationProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAuthenticationProfiles&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAuthenticationProfilesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AuthenticationProfiles")) {
                    result.authentication_profiles = try serde.deserializeAuthenticationProfileList(allocator, &reader, "member");
                } else {
