...
pub const ResizeClusterInput = @import("resize_cluster_message.zig").ResizeClusterMessage;

pub const ResizeClusterOutput = struct {
    cluster: ?Cluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResizeClusterInput, options: CallOptions) !ResizeClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResizeClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ResizeCluster&Version=2012-12-01");
    if (input.classic) |v| {
        try body_buf.appendSlice(allocator, "&Classic=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResizeClusterOutput {
    _ = status;
    _ = headers;
