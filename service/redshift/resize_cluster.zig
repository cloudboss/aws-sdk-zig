...
const ResizeClusterInput = @import("resize_cluster_message.zig").ResizeClusterMessage;

pub const ResizeClusterOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResizeClusterInput, options: Options) !ResizeClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResizeClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ResizeCluster&Version=2012-12-01");
    if (input.classic) |v| {
        try body_buf.appendSlice(alloc, "&Classic=");
    ...
    if (input.target_reserved_node_offering_id) |v| {
        try body_buf.appendSlice(alloc, "&TargetReservedNodeOfferingId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResizeClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ResizeClusterResult")) break;
            },
            else => {},
        }
    }

    var result: ResizeClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
