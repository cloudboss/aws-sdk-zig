...
const DataShareStatusForProducer = @import("data_share_status_for_producer.zig").DataShareStatusForProducer;
const DataShare = @import("data_share.zig").DataShare;
const serde = @import("serde.zig");

pub const DescribeDataSharesForProducerInput = struct {
    ...
    /// field is specified, Amazon
    /// Redshift returns the list of datashares that have the specified status.
    status: ?DataShareStatusForProducer = null,
};

pub const DescribeDataSharesForProducerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataSharesForProducerInput, options: Options) !DescribeDataSharesForProducerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataSharesForProducerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDataSharesForProducer&Version=2012-12-01");
    if (input.marker) |v| {
    ...
    if (input.producer_arn) |v| {
        try body_buf.appendSlice(alloc, "&ProducerArn=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.status) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataSharesForProducerOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDataSharesForProducerResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDataSharesForProducerOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DataShares")) {
                    result.data_shares = try serde.deserializeDataShareList(&reader, alloc, "member");
