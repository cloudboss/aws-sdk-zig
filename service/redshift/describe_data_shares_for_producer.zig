...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataSharesForProducerInput, options: CallOptions) !DescribeDataSharesForProducerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDataSharesForProducerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDataSharesForProducer&Version=2012-12-01");
    if (input.marker) |v| {
    ...
    if (input.producer_arn) |v| {
        try body_buf.appendSlice(allocator, "&ProducerArn=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    if (input.status) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDataSharesForProducerOutput {
    _ = status;
    _ = headers;
