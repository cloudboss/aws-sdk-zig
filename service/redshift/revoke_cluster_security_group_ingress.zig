...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeClusterSecurityGroupIngressInput, options: Options) !RevokeClusterSecurityGroupIngressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RevokeClusterSecurityGroupIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RevokeClusterSecurityGroupIngress&Version=2012-12-01");
    if (input.cidrip) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RevokeClusterSecurityGroupIngressOutput {
    _ = status;
    _ = headers;
