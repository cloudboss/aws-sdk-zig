...
pub const DescribeClusterVersionsOutput = struct {
    ...
    cluster_versions: ?[]const ClusterVersion = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClusterVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeClusterVersions&Version=2012-12-01");
    if (input.cluster_parameter_group_family) |v| {
        try body_buf.appendSlice(alloc, "&ClusterParameterGroupFamily=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClusterVersionsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeClusterVersionsResult")) break;
            },
            else => {},
        }
    }
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterVersions")) {
                    result.cluster_versions = try serde.deserializeClusterVersionList(&reader, alloc, "ClusterVersion");
