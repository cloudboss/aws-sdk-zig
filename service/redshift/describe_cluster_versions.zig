...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeClusterVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeClusterVersions&Version=2012-12-01");
    if (input.cluster_parameter_group_family) |v| {
        try body_buf.appendSlice(allocator, "&ClusterParameterGroupFamily=");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterVersions")) {
                    result.cluster_versions = try serde.deserializeClusterVersionList(allocator, &reader, "ClusterVersion");
