...
pub const DescribeEventSubscriptionsInput = struct {
    ...
    /// response
    /// records. When the results of a DescribeEventSubscriptions request exceed the
    /// value
    ...
    /// notification
    /// subscriptions that are associated with the specified key or keys. For
    /// example, suppose
    ...
    /// the request, Amazon Redshift returns a response with the subscriptions that
    /// have either or
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeEventSubscriptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeEventSubscriptions&Version=2012-12-01");
    if (input.marker) |v| {
