...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateClusterParameterGroupInput, options: Options) !CreateClusterParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateClusterParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateClusterParameterGroup&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&Description=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.description);
    try body_buf.appendSlice(alloc, "&ParameterGroupFamily=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.parameter_group_family);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateClusterParameterGroupOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterParameterGroup")) {
                    result.cluster_parameter_group = try serde.deserializeClusterParameterGroup(&reader, alloc);
                } else {
