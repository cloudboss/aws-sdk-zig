...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteClusterSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=BatchDeleteClusterSnapshots&Version=2012-12-01");
    for (input.identifiers, 0..) |item, idx| {
        ...
        }
        {
            ...
            try body_buf.appendSlice(alloc, field_prefix);
            try aws.url.appendUrlEncoded(alloc, &body_buf, item.snapshot_identifier);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteClusterSnapshotsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Errors")) {
                    result.errors = try serde.deserializeBatchSnapshotOperationErrorList(&reader, alloc, "SnapshotErrorMessage");
