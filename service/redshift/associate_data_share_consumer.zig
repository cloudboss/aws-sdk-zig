...
const AssociateDataShareConsumerOutput = @import("data_share.zig").DataShare;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateDataShareConsumerInput, options: Options) !AssociateDataShareConsumerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateDataShareConsumerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AssociateDataShareConsumer&Version=2012-12-01");
    if (input.allow_writes) |v| {
        try body_buf.appendSlice(alloc, "&AllowWrites=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.associate_entire_account) |v| {
        try body_buf.appendSlice(alloc, "&AssociateEntireAccount=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateDataShareConsumerOutput {
    _ = status;
    _ = headers;
