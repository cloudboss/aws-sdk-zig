...
pub const DescribeClusterParametersInput = struct {
...
pub const DescribeClusterParametersOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClusterParametersInput, options: Options) !DescribeClusterParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClusterParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeClusterParameters&Version=2012-12-01");
    if (input.marker) |v| {
    ...
    try body_buf.appendSlice(alloc, "&ParameterGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.parameter_group_name);
    if (input.source) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClusterParametersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeClusterParametersResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeClusterParametersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
