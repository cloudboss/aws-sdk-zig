...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClusterParametersInput, options: CallOptions) !DescribeClusterParametersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeClusterParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeClusterParameters&Version=2012-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeClusterParametersOutput {
    _ = status;
    _ = headers;
