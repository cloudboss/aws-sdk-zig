...
pub const RejectDataShareInput = struct {
    /// The Amazon Resource Name (ARN) of the datashare to reject.
    data_share_arn: []const u8,
};

pub const RejectDataShareOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectDataShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RejectDataShare&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectDataShareOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RejectDataShareResult")) break;
            },
            else => {},
        }
    }

    var result: RejectDataShareOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
