...
fn serializeRequest(allocator: std.mem.Allocator, input: RejectDataShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RejectDataShare&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RejectDataShareOutput {
    _ = status;
    _ = headers;
