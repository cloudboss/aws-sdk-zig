...
pub const ModifyClusterDbRevisionInput = struct {
    ...
    revision_target: []const u8,
};

pub const ModifyClusterDbRevisionOutput = struct {
    cluster: ?Cluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyClusterDbRevisionInput, options: CallOptions) !ModifyClusterDbRevisionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyClusterDbRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyClusterDbRevision&Version=2012-12-01");
    ...
    try body_buf.appendSlice(allocator, "&RevisionTarget=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.revision_target);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyClusterDbRevisionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ModifyClusterDbRevisionResult")) break;
            },
            else => {},
        }
    }

    var result: ModifyClusterDbRevisionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
