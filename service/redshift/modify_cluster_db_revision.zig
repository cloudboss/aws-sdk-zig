...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyClusterDbRevisionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyClusterDbRevision&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&ClusterIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cluster_identifier);
    try body_buf.appendSlice(alloc, "&RevisionTarget=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.revision_target);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyClusterDbRevisionOutput {
    _ = status;
    _ = headers;
