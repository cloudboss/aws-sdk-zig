...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEndpointAuthorizationInput, options: Options) !DescribeEndpointAuthorizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEndpointAuthorizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEndpointAuthorization&Version=2012-12-01");
    if (input.account) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEndpointAuthorizationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EndpointAuthorizationList")) {
                    result.endpoint_authorization_list = try serde.deserializeEndpointAuthorizations(&reader, alloc, "member");
