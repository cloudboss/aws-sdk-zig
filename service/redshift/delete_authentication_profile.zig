...
pub const DeleteAuthenticationProfileInput = struct {
    /// The name of the authentication profile to delete.
    authentication_profile_name: []const u8,
};

pub const DeleteAuthenticationProfileOutput = struct {
    /// The name of the authentication profile that was deleted.
    authentication_profile_name: ?[]const u8 = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAuthenticationProfileInput, options: CallOptions) !DeleteAuthenticationProfileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAuthenticationProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteAuthenticationProfile&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAuthenticationProfileOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteAuthenticationProfileResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteAuthenticationProfileOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AuthenticationProfileName")) {
