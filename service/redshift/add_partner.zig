...
pub const AddPartnerInput = struct {
...
pub const AddPartnerOutput = struct {
    /// The name of the database that receives data from the partner.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddPartnerInput, options: Options) !AddPartnerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddPartnerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AddPartner&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddPartnerOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AddPartnerResult")) break;
            },
            else => {},
        }
    }

    var result: AddPartnerOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
