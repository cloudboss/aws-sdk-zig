...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTags&Version=2012-12-01");
    if (input.marker) |v| {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.tagged_resources = try serde.deserializeTaggedResourceList(&reader, alloc, "TaggedResource");
                } else {
