...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHsmClientCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeHsmClientCertificates&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHsmClientCertificatesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HsmClientCertificates")) {
                    result.hsm_client_certificates = try serde.deserializeHsmClientCertificateList(&reader, alloc, "HsmClientCertificate");
