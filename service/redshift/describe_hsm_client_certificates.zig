...
pub const DescribeHsmClientCertificatesInput = struct {
    ...
    /// Amazon Redshift returns a response with the HSM client certificates that
    /// have either or both
    ...
    /// example,
    /// suppose that you have HSM client certificates that are tagged with values
    /// called
...
pub const DescribeHsmClientCertificatesOutput = struct {
    ...
    hsm_client_certificates: ?[]const HsmClientCertificate = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHsmClientCertificatesInput, options: CallOptions) !DescribeHsmClientCertificatesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHsmClientCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeHsmClientCertificates&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHsmClientCertificatesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeHsmClientCertificatesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeHsmClientCertificatesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HsmClientCertificates")) {
                    result.hsm_client_certificates = try serde.deserializeHsmClientCertificateList(allocator, &reader, "HsmClientCertificate");
