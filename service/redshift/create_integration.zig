...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateIntegration&Version=2012-12-01");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.integration_name);
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_arn);
