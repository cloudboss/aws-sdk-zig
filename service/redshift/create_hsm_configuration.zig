...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHsmConfigurationInput, options: Options) !CreateHsmConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHsmConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateHsmConfiguration&Version=2012-12-01");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.hsm_configuration_identifier);
    try body_buf.appendSlice(alloc, "&HsmIpAddress=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.hsm_ip_address);
    try body_buf.appendSlice(alloc, "&HsmPartitionName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.hsm_partition_name);
    try body_buf.appendSlice(alloc, "&HsmPartitionPassword=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.hsm_partition_password);
    try body_buf.appendSlice(alloc, "&HsmServerPublicCertificate=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.hsm_server_public_certificate);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHsmConfigurationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HsmConfiguration")) {
                    result.hsm_configuration = try serde.deserializeHsmConfiguration(&reader, alloc);
                } else {
