...
pub const RotateEncryptionKeyInput = struct {
...
pub const RotateEncryptionKeyOutput = struct {
    cluster: ?Cluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RotateEncryptionKeyInput, options: CallOptions) !RotateEncryptionKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RotateEncryptionKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RotateEncryptionKey&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RotateEncryptionKeyOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RotateEncryptionKeyResult")) break;
            },
            else => {},
        }
    }

    var result: RotateEncryptionKeyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
