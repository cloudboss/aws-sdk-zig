...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteCluster&Version=2012-12-01");
    ...
    if (input.final_cluster_snapshot_identifier) |v| {
        try body_buf.appendSlice(alloc, "&FinalClusterSnapshotIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.final_cluster_snapshot_retention_period) |v| {
        try body_buf.appendSlice(alloc, "&FinalClusterSnapshotRetentionPeriod=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.skip_final_cluster_snapshot) |v| {
        try body_buf.appendSlice(alloc, "&SkipFinalClusterSnapshot=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteClusterOutput {
    _ = status;
    _ = headers;
