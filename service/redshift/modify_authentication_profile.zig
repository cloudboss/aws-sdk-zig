...
pub const ModifyAuthenticationProfileInput = struct {
...
pub const ModifyAuthenticationProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyAuthenticationProfileInput, options: Options) !ModifyAuthenticationProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyAuthenticationProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyAuthenticationProfile&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&AuthenticationProfileContent=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.authentication_profile_content);
    try body_buf.appendSlice(alloc, "&AuthenticationProfileName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.authentication_profile_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyAuthenticationProfileOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ModifyAuthenticationProfileResult")) break;
            },
            else => {},
        }
    }

    var result: ModifyAuthenticationProfileOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
