...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReservedNodeOfferingsInput, options: Options) !DescribeReservedNodeOfferingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedNodeOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeReservedNodeOfferings&Version=2012-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReservedNodeOfferingsOutput {
    _ = status;
    _ = headers;
