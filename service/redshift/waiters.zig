...
pub const ClusterAvailableWaiter = struct {
    client: *Client,
    ...
    fn poll(self: *Self) aws.waiter.AcceptorState {
        ...
        _ = self.client.describeClusters(arena.allocator(), self.params, .{ .diagnostic = &diagnostic }) catch |err| {
            if (err == error.ServiceError) {
                defer diagnostic.deinit();
                if (std.mem.eql(u8, diagnostic.code(), "ClusterNotFound")) {
                    return .retry;
                }
            }
            return .retry;
