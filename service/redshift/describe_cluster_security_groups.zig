...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClusterSecurityGroupsInput, options: Options) !DescribeClusterSecurityGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClusterSecurityGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeClusterSecurityGroups&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClusterSecurityGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterSecurityGroups")) {
                    result.cluster_security_groups = try serde.deserializeClusterSecurityGroups(&reader, alloc, "ClusterSecurityGroup");
