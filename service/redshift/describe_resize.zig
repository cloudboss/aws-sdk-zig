...
pub const DescribeResizeInput = struct {
    ...
    /// Services account are
    /// returned.
    cluster_identifier: []const u8,
};

pub const DescribeResizeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeResizeInput, options: CallOptions) !DescribeResizeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeResizeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeResize&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeResizeOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeResizeResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeResizeOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
