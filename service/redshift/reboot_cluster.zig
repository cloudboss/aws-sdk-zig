...
pub const RebootClusterInput = struct {
    /// The cluster identifier.
    cluster_identifier: []const u8,
};

pub const RebootClusterOutput = struct {
    cluster: ?Cluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebootClusterInput, options: CallOptions) !RebootClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RebootClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RebootCluster&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RebootClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RebootClusterResult")) break;
            },
            else => {},
        }
    }

    var result: RebootClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
