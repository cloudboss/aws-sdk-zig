...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIntegrationsInput, options: Options) !DescribeIntegrationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIntegrationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeIntegrations&Version=2012-12-01");
    if (input.filters) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
                ...
                try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(item.name));
            }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIntegrationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Integrations")) {
                    result.integrations = try serde.deserializeIntegrationList(&reader, alloc, "Integration");
