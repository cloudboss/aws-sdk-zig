...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptReservedNodeExchangeInput, options: Options) !AcceptReservedNodeExchangeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptReservedNodeExchangeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AcceptReservedNodeExchange&Version=2012-12-01");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_reserved_node_offering_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptReservedNodeExchangeOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ExchangedReservedNode")) {
                    result.exchanged_reserved_node = try serde.deserializeReservedNode(&reader, alloc);
                } else {
