...
pub const ModifyEventSubscriptionInput = struct {
    ...
    /// the objects
    /// must be of the same type as was specified in the source type parameter. The
    /// event
    /// subscription will return only events generated by the specified objects. If
    /// not
    ...
    /// want to
    /// be notified of events generated by a cluster, you would set this parameter
    /// to cluster.
    /// If this value is not specified, events are returned for all Amazon Redshift
    /// objects in your
    /// Amazon Web Services account. You must specify a source type in order to
    /// specify source IDs.
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyEventSubscriptionInput, options: Options) !ModifyEventSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyEventSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyEventSubscription&Version=2012-12-01");
    if (input.enabled) |v| {
    ...
    if (input.source_ids) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&SourceIds.SourceId.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            try aws.url.appendUrlEncoded(alloc, &body_buf, item);
        }
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyEventSubscriptionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ModifyEventSubscriptionResult")) break;
            },
            else => {},
        }
    }

    var result: ModifyEventSubscriptionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
