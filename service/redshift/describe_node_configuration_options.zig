...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNodeConfigurationOptionsInput, options: Options) !DescribeNodeConfigurationOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNodeConfigurationOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeNodeConfigurationOptions&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&ActionType=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.action_type));
        ...
        for (list, 0..) |item, idx| {
            ...
            if (item.values) |lst_1| {
                for (lst_1, 0..) |item_1, idx_1| {
                    ...
                    {
                        var prefix_buf: [256]u8 = undefined;
                        const field_prefix = std.fmt.bufPrint(&prefix_buf, "&Filter.NodeConfigurationOptionsFilter.{d}.Values.item.{d}=", .{n, n_1}) catch continue;
                        try body_buf.appendSlice(alloc, field_prefix);
    ...
    if (input.owner_account) |v| {
        try body_buf.appendSlice(alloc, "&OwnerAccount=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.snapshot_arn) |v| {
        try body_buf.appendSlice(alloc, "&SnapshotArn=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNodeConfigurationOptionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.node_configuration_option_list = try serde.deserializeNodeConfigurationOptionList(&reader, alloc, "NodeConfigurationOption");
                } else {
