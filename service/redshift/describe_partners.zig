...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePartnersInput, options: Options) !DescribePartnersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePartnersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribePartners&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePartnersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PartnerIntegrationInfoList")) {
                    result.partner_integration_info_list = try serde.deserializePartnerIntegrationInfoList(&reader, alloc, "PartnerIntegrationInfo");
                } else {
