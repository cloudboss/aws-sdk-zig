...
pub const DescribeHsmConfigurationsInput = struct {
    ...
    /// Amazon Redshift returns a response with the HSM configurations that have
    /// either or both of
    ...
    /// Redshift
    /// returns a response with the HSM configurations that have either or both of
    /// these tag
    /// values associated with them.
    tag_values: ?[]const []const u8 = null,
};

pub const DescribeHsmConfigurationsOutput = struct {
    /// A list of `HsmConfiguration` objects.
    hsm_configurations: ?[]const HsmConfiguration = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHsmConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeHsmConfigurations&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHsmConfigurationsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeHsmConfigurationsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeHsmConfigurationsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HsmConfigurations")) {
                    result.hsm_configurations = try serde.deserializeHsmConfigurationList(allocator, &reader, "HsmConfiguration");
