...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHsmConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeHsmConfigurations&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHsmConfigurationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "HsmConfigurations")) {
                    result.hsm_configurations = try serde.deserializeHsmConfigurationList(&reader, alloc, "HsmConfiguration");
