...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUsageLimitsInput, options: CallOptions) !DescribeUsageLimitsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeUsageLimitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeUsageLimits&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeUsageLimitsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.usage_limits = try serde.deserializeUsageLimits(allocator, &reader, "member");
                } else {
