...
pub const DescribeUsageLimitsInput = struct {
    ...
    /// Amazon Redshift returns a response with the usage limit objects have either
    /// or both of these
    ...
    /// Redshift
    /// returns a response with the usage limit objects that have either or both of
    /// these tag
...
pub const DescribeUsageLimitsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUsageLimitsInput, options: Options) !DescribeUsageLimitsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUsageLimitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeUsageLimits&Version=2012-12-01");
    if (input.cluster_identifier) |v| {
        try body_buf.appendSlice(alloc, "&ClusterIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.feature_type) |v| {
        try body_buf.appendSlice(alloc, "&FeatureType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUsageLimitsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeUsageLimitsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeUsageLimitsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "UsageLimits")) {
                    result.usage_limits = try serde.deserializeUsageLimits(&reader, alloc, "member");
                } else {
