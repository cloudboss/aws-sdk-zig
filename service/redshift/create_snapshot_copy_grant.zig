...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSnapshotCopyGrantInput, options: Options) !CreateSnapshotCopyGrantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSnapshotCopyGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateSnapshotCopyGrant&Version=2012-12-01");
    if (input.kms_key_id) |v| {
        try body_buf.appendSlice(alloc, "&KmsKeyId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&SnapshotCopyGrantName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.snapshot_copy_grant_name);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSnapshotCopyGrantOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SnapshotCopyGrant")) {
                    result.snapshot_copy_grant = try serde.deserializeSnapshotCopyGrant(&reader, alloc);
                } else {
