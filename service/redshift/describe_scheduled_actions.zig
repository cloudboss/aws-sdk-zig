...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScheduledActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeScheduledActions&Version=2012-12-01");
    if (input.active) |v| {
    ...
    if (input.target_action_type) |v| {
        try body_buf.appendSlice(alloc, "&TargetActionType=");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.scheduled_actions = try serde.deserializeScheduledActionList(&reader, alloc, "ScheduledAction");
                } else {
