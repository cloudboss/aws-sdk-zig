...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateCluster&Version=2012-12-01");
    ...
    if (input.load_sample_data) |v| {
        try body_buf.appendSlice(allocator, "&LoadSampleData=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.node_type);
