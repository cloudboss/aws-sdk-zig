...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyCustomDomainAssociationInput, options: Options) !ModifyCustomDomainAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyCustomDomainAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyCustomDomainAssociation&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&ClusterIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cluster_identifier);
    try body_buf.appendSlice(alloc, "&CustomDomainCertificateArn=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.custom_domain_certificate_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyCustomDomainAssociationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterIdentifier")) {
                    ...
                    result.custom_domain_cert_expiry_time = try alloc.dupe(u8, try reader.readElementText());
