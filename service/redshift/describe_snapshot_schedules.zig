...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSnapshotSchedulesInput, options: Options) !DescribeSnapshotSchedulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSnapshotSchedulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeSnapshotSchedules&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSnapshotSchedulesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.snapshot_schedules = try serde.deserializeSnapshotScheduleList(&reader, alloc, "SnapshotSchedule");
                } else {
