...
pub const DescribeClusterSubnetGroupsInput = struct {
    ...
    /// Amazon Redshift returns a response with the subnet groups that have either
    /// or both of these
    ...
    /// Redshift
    /// returns a response with the subnet groups that have either or both of these
    /// tag values
    /// associated with them.
    tag_values: ?[]const []const u8 = null,
};

pub const DescribeClusterSubnetGroupsOutput = struct {
    /// A list of ClusterSubnetGroup instances.
    cluster_subnet_groups: ?[]const ClusterSubnetGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClusterSubnetGroupsInput, options: CallOptions) !DescribeClusterSubnetGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeClusterSubnetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeClusterSubnetGroups&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeClusterSubnetGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeClusterSubnetGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeClusterSubnetGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterSubnetGroups")) {
                    result.cluster_subnet_groups = try serde.deserializeClusterSubnetGroups(allocator, &reader, "ClusterSubnetGroup");
