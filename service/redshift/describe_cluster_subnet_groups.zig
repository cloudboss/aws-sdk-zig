...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClusterSubnetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeClusterSubnetGroups&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClusterSubnetGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterSubnetGroups")) {
                    result.cluster_subnet_groups = try serde.deserializeClusterSubnetGroups(&reader, alloc, "ClusterSubnetGroup");
