...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyAquaConfigurationInput, options: Options) !ModifyAquaConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyAquaConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyAquaConfiguration&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyAquaConfigurationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AquaConfiguration")) {
                    result.aqua_configuration = try serde.deserializeAquaConfiguration(&reader, alloc);
                } else {
