...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUsageLimitInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateUsageLimit&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&Amount=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.amount}) catch "");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.feature_type));
    try body_buf.appendSlice(alloc, "&LimitType=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.limit_type));
    if (input.period) |v| {
