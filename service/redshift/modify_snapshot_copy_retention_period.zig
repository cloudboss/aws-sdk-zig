...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifySnapshotCopyRetentionPeriodInput, options: Options) !ModifySnapshotCopyRetentionPeriodOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifySnapshotCopyRetentionPeriodInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifySnapshotCopyRetentionPeriod&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&ClusterIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cluster_identifier);
    if (input.manual) |v| {
        try body_buf.appendSlice(alloc, "&Manual=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&RetentionPeriod=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.retention_period}) catch "");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifySnapshotCopyRetentionPeriodOutput {
    _ = status;
    _ = headers;
