...
pub const RegisterNamespaceInput = struct {
...
pub const RegisterNamespaceOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RegisterNamespace&Version=2012-12-01");
    ...
    try body_buf.appendSlice(alloc, "&NamespaceIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.namespace_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterNamespaceOutput {
    _ = status;
    _ = headers;
    _ = alloc;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RegisterNamespaceResult")) break;
            },
            else => {},
        }
    }

    var result: RegisterNamespaceOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
