...
const CreateEndpointAccessOutput = @import("endpoint_access.zig").EndpointAccess;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEndpointAccessInput, options: Options) !CreateEndpointAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateEndpointAccess&Version=2012-12-01");
    ...
    try body_buf.appendSlice(alloc, "&SubnetGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.subnet_group_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEndpointAccessOutput {
    _ = status;
    _ = headers;
