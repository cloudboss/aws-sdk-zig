...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClusterDbRevisionsInput, options: Options) !DescribeClusterDbRevisionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClusterDbRevisionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeClusterDbRevisions&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClusterDbRevisionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterDbRevisions")) {
                    result.cluster_db_revisions = try serde.deserializeClusterDbRevisionsList(&reader, alloc, "ClusterDbRevision");
