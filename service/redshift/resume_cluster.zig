...
pub const ResumeClusterInput = @import("resume_cluster_message.zig").ResumeClusterMessage;

pub const ResumeClusterOutput = struct {
    cluster: ?Cluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResumeClusterInput, options: CallOptions) !ResumeClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResumeClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ResumeCluster&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResumeClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ResumeClusterResult")) break;
            },
            else => {},
        }
    }

    var result: ResumeClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
