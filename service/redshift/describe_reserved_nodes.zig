...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeReservedNodes&Version=2012-12-01");
    if (input.marker) |v| {
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.reserved_nodes = try serde.deserializeReservedNodeList(&reader, alloc, "ReservedNode");
                } else {
