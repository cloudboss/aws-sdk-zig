...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReservedNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeReservedNodes&Version=2012-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReservedNodesOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeReservedNodesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeReservedNodesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "ReservedNodes")) {
                    result.reserved_nodes = try serde.deserializeReservedNodeList(allocator, &reader, "ReservedNode");
                } else {
