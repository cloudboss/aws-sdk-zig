...
pub const ModifyClusterSubnetGroupInput = struct {
...
pub const ModifyClusterSubnetGroupOutput = struct {
    cluster_subnet_group: ?ClusterSubnetGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyClusterSubnetGroupInput, options: CallOptions) !ModifyClusterSubnetGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyClusterSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ModifyClusterSubnetGroup&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyClusterSubnetGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ModifyClusterSubnetGroupResult")) break;
            },
            else => {},
        }
    }

    var result: ModifyClusterSubnetGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
