...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSnapshotCopyGrantsInput, options: CallOptions) !DescribeSnapshotCopyGrantsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSnapshotCopyGrantsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeSnapshotCopyGrants&Version=2012-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSnapshotCopyGrantsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.snapshot_copy_grants = try serde.deserializeSnapshotCopyGrantList(allocator, &reader, "SnapshotCopyGrant");
                } else {
