...
pub const DescribeSnapshotCopyGrantsInput = struct {
    /// An optional parameter that specifies the starting point to return a set of
    /// response
    /// records. When the results of a `DescribeSnapshotCopyGrant` request exceed
    /// the
...
pub const DescribeSnapshotCopyGrantsOutput = struct {
    /// An optional parameter that specifies the starting point to return a set of
    /// response
    /// records. When the results of a `DescribeSnapshotCopyGrant` request exceed
    /// the
    ...
    snapshot_copy_grants: ?[]const SnapshotCopyGrant = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSnapshotCopyGrantsInput, options: Options) !DescribeSnapshotCopyGrantsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSnapshotCopyGrantsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeSnapshotCopyGrants&Version=2012-12-01");
    if (input.marker) |v| {
    ...
    if (input.snapshot_copy_grant_name) |v| {
        try body_buf.appendSlice(alloc, "&SnapshotCopyGrantName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSnapshotCopyGrantsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeSnapshotCopyGrantsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeSnapshotCopyGrantsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "SnapshotCopyGrants")) {
                    result.snapshot_copy_grants = try serde.deserializeSnapshotCopyGrantList(&reader, alloc, "SnapshotCopyGrant");
                } else {
