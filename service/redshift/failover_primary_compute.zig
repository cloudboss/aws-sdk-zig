...
pub const FailoverPrimaryComputeInput = struct {
...
pub const FailoverPrimaryComputeOutput = struct {
    cluster: ?Cluster = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: FailoverPrimaryComputeInput, options: CallOptions) !FailoverPrimaryComputeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: FailoverPrimaryComputeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=FailoverPrimaryCompute&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !FailoverPrimaryComputeOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "FailoverPrimaryComputeResult")) break;
            },
            else => {},
        }
    }

    var result: FailoverPrimaryComputeOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
