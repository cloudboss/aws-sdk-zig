...
pub const DescribeLoggingStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLoggingStatusInput, options: Options) !DescribeLoggingStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLoggingStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeLoggingStatus&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLoggingStatusOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeLoggingStatusResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeLoggingStatusOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "BucketName")) {
                    ...
                    result.last_failure_message = try alloc.dupe(u8, try reader.readElementText());
                    ...
                    result.log_exports = try serde.deserializeLogTypeList(&reader, alloc, "member");
                    ...
                    result.s3_key_prefix = try alloc.dupe(u8, try reader.readElementText());
                } else {
