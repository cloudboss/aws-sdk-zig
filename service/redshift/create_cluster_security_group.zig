...
pub const CreateClusterSecurityGroupInput = struct {
...
pub const CreateClusterSecurityGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateClusterSecurityGroupInput, options: Options) !CreateClusterSecurityGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateClusterSecurityGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateClusterSecurityGroup&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&ClusterSecurityGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.cluster_security_group_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateClusterSecurityGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateClusterSecurityGroupResult")) break;
            },
            else => {},
        }
    }

    var result: CreateClusterSecurityGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ClusterSecurityGroup")) {
                    result.cluster_security_group = try serde.deserializeClusterSecurityGroup(&reader, alloc);
                } else {
