...
pub const DeleteUsageLimitInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUsageLimitInput, options: CallOptions) !DeleteUsageLimitOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteUsageLimitInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteUsageLimit&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteUsageLimitOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteUsageLimitOutput = .{};
