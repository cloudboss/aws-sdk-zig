...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRedshiftIdcApplicationsInput, options: Options) !DescribeRedshiftIdcApplicationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRedshiftIdcApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeRedshiftIdcApplications&Version=2012-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRedshiftIdcApplicationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.redshift_idc_applications = try serde.deserializeRedshiftIdcApplicationList(&reader, alloc, "member");
                } else {
