...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePartnerInput, options: CallOptions) !DeletePartnerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePartnerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeletePartner&Version=2012-12-01");
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.database_name);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePartnerOutput {
    _ = status;
    _ = headers;
