...
pub const DeletePartnerInput = struct {
...
pub const DeletePartnerOutput = struct {
    /// The name of the database that receives data from the partner.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePartnerInput, options: Options) !DeletePartnerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePartnerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeletePartner&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&AccountId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.account_id);
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.database_name);
    try body_buf.appendSlice(alloc, "&PartnerName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.partner_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePartnerOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeletePartnerResult")) break;
            },
            else => {},
        }
    }

    var result: DeletePartnerOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
