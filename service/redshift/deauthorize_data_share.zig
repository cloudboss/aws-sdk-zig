...
pub const DeauthorizeDataShareInput = struct {
...
pub const DeauthorizeDataShareOutput = @import("data_share.zig").DataShare;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeauthorizeDataShareInput, options: CallOptions) !DeauthorizeDataShareOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeauthorizeDataShareInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeauthorizeDataShare&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeauthorizeDataShareOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeauthorizeDataShareResult")) break;
            },
            else => {},
        }
    }

    var result: DeauthorizeDataShareOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
