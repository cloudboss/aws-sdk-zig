...
pub const UpdatePartnerStatusInput = struct {
...
pub const UpdatePartnerStatusOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePartnerStatusInput, options: CallOptions) !UpdatePartnerStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePartnerStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UpdatePartnerStatus&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePartnerStatusOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "UpdatePartnerStatusResult")) break;
            },
            else => {},
        }
    }

    var result: UpdatePartnerStatusOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
