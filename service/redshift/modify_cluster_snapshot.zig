...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyClusterSnapshotInput, options: Options) !ModifyClusterSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyClusterSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyClusterSnapshot&Version=2012-12-01");
    if (input.force) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyClusterSnapshotOutput {
    _ = status;
    _ = headers;
