...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableLoggingInput, options: Options) !EnableLoggingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableLoggingInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=EnableLogging&Version=2012-12-01");
    ...
    if (input.s3_key_prefix) |v| {
        try body_buf.appendSlice(alloc, "&S3KeyPrefix=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableLoggingOutput {
    _ = status;
    _ = headers;
