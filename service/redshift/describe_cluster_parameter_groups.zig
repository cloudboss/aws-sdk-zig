...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClusterParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeClusterParameterGroups&Version=2012-12-01");
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClusterParameterGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.parameter_groups = try serde.deserializeParameterGroupList(&reader, alloc, "ClusterParameterGroup");
                } else {
