...
pub const DescribeClusterParameterGroupsInput = struct {
    ...
    /// Amazon Redshift returns a response with the parameter groups that have
...
pub const DescribeClusterParameterGroupsOutput = struct {
    ...
    parameter_groups: ?[]const ClusterParameterGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClusterParameterGroupsInput, options: CallOptions) !DescribeClusterParameterGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeClusterParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeClusterParameterGroups&Version=2012-12-01");
    if (input.marker) |v| {
    ...
    if (input.parameter_group_name) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeClusterParameterGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeClusterParameterGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeClusterParameterGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "ParameterGroups")) {
                    result.parameter_groups = try serde.deserializeParameterGroupList(allocator, &reader, "ClusterParameterGroup");
                } else {
