...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListRecommendations&Version=2012-12-01");
    ...
    if (input.namespace_arn) |v| {
        try body_buf.appendSlice(alloc, "&NamespaceArn=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.recommendations = try serde.deserializeRecommendationList(&reader, alloc, "Recommendation");
                } else {
