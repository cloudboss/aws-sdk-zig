...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCustomDomainAssociationsInput, options: Options) !DescribeCustomDomainAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCustomDomainAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeCustomDomainAssociations&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCustomDomainAssociationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Associations")) {
                    result.associations = try serde.deserializeAssociationList(&reader, alloc, "Association");
