...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedNodeExchangeStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeReservedNodeExchangeStatus&Version=2012-12-01");
    if (input.marker) |v| {
    ...
    if (input.reserved_node_exchange_request_id) |v| {
        try body_buf.appendSlice(alloc, "&ReservedNodeExchangeRequestId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReservedNodeExchangeStatusOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.reserved_node_exchange_status_details = try serde.deserializeReservedNodeExchangeStatusList(&reader, alloc, "ReservedNodeExchangeStatus");
                } else {
