...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchModifyClusterSnapshotsInput, options: Options) !BatchModifyClusterSnapshotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchModifyClusterSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=BatchModifyClusterSnapshots&Version=2012-12-01");
    if (input.force) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchModifyClusterSnapshotsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Errors")) {
                    result.errors = try serde.deserializeBatchSnapshotOperationErrors(&reader, alloc, "SnapshotErrorMessage");
                } else if (std.mem.eql(u8, e.local, "Resources")) {
                    result.resources = try serde.deserializeSnapshotIdentifierList(&reader, alloc, "String");
                } else {
