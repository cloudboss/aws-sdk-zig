...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteHsmClientCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteHsmClientCertificate&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&HsmClientCertificateIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.hsm_client_certificate_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteHsmClientCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
