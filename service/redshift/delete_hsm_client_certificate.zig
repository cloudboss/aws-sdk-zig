...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteHsmClientCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteHsmClientCertificate&Version=2012-12-01");
    try body_buf.appendSlice(allocator, "&HsmClientCertificateIdentifier=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.hsm_client_certificate_identifier);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteHsmClientCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteHsmClientCertificateOutput = .{};
