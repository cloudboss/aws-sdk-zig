...
pub const DeleteSnapshotCopyGrantInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSnapshotCopyGrantInput, options: CallOptions) !DeleteSnapshotCopyGrantOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSnapshotCopyGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteSnapshotCopyGrant&Version=2012-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSnapshotCopyGrantOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteSnapshotCopyGrantOutput = .{};
