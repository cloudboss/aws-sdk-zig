...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClusterTracksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeClusterTracks&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClusterTracksOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "MaintenanceTracks")) {
                    result.maintenance_tracks = try serde.deserializeTrackList(&reader, alloc, "MaintenanceTrack");
