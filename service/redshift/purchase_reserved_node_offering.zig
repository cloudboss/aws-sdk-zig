...
pub const PurchaseReservedNodeOfferingInput = struct {
...
pub const PurchaseReservedNodeOfferingOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurchaseReservedNodeOfferingInput, options: Options) !PurchaseReservedNodeOfferingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseReservedNodeOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PurchaseReservedNodeOffering&Version=2012-12-01");
    if (input.node_count) |v| {
        try body_buf.appendSlice(alloc, "&NodeCount=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    try body_buf.appendSlice(alloc, "&ReservedNodeOfferingId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.reserved_node_offering_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PurchaseReservedNodeOfferingOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PurchaseReservedNodeOfferingResult")) break;
            },
            else => {},
        }
    }

    var result: PurchaseReservedNodeOfferingOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ReservedNode")) {
                    result.reserved_node = try serde.deserializeReservedNode(&reader, alloc);
                } else {
