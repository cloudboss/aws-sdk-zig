...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventCategoriesInput, options: Options) !DescribeEventCategoriesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEventCategoriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeEventCategories&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEventCategoriesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EventCategoriesMapList")) {
                    result.event_categories_map_list = try serde.deserializeEventCategoriesMapList(&reader, alloc, "EventCategoriesMap");
                } else {
