...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteClusterSnapshotInput, options: Options) !DeleteClusterSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteClusterSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteClusterSnapshot&Version=2012-12-01");
    if (input.snapshot_cluster_identifier) |v| {
        try body_buf.appendSlice(alloc, "&SnapshotClusterIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteClusterSnapshotOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteClusterSnapshotResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteClusterSnapshotOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Snapshot")) {
                    result.snapshot = try serde.deserializeSnapshot(&reader, alloc);
                } else {
