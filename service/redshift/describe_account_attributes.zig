...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeAccountAttributes&Version=2012-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountAttributesOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeAccountAttributesResult")) break;
            },
            else => {},
        }
    }
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccountAttributes")) {
                    result.account_attributes = try serde.deserializeAttributeList(&reader, alloc, "AccountAttribute");
                } else {
