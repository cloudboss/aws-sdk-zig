...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAccountAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAccountAttributes&Version=2012-12-01");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccountAttributes")) {
                    result.account_attributes = try serde.deserializeAttributeList(allocator, &reader, "AccountAttribute");
                } else {
