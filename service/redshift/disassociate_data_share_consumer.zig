...
pub const DisassociateDataShareConsumerOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateDataShareConsumerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DisassociateDataShareConsumer&Version=2012-12-01");
    if (input.consumer_arn) |v| {
        try body_buf.appendSlice(alloc, "&ConsumerArn=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.consumer_region) |v| {
        try body_buf.appendSlice(alloc, "&ConsumerRegion=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    ...
    if (input.disassociate_entire_account) |v| {
        try body_buf.appendSlice(alloc, "&DisassociateEntireAccount=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateDataShareConsumerOutput {
    _ = status;
    _ = headers;
