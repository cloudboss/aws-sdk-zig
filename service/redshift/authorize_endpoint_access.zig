...
pub const AuthorizeEndpointAccessOutput = @import("endpoint_authorization.zig").EndpointAuthorization;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AuthorizeEndpointAccessInput, options: CallOptions) !AuthorizeEndpointAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AuthorizeEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AuthorizeEndpointAccess&Version=2012-12-01");
    try body_buf.appendSlice(allocator, "&Account=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.account);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AuthorizeEndpointAccessOutput {
    _ = status;
    _ = headers;
