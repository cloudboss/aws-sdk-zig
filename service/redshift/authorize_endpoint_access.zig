...
pub const AuthorizeEndpointAccessInput = struct {
...
const AuthorizeEndpointAccessOutput = @import("endpoint_authorization.zig").EndpointAuthorization;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AuthorizeEndpointAccessInput, options: Options) !AuthorizeEndpointAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AuthorizeEndpointAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AuthorizeEndpointAccess&Version=2012-12-01");
    try body_buf.appendSlice(alloc, "&Account=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.account);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AuthorizeEndpointAccessOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AuthorizeEndpointAccessResult")) break;
            },
            else => {},
        }
    }

    var result: AuthorizeEndpointAccessOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AllowedAllVPCs")) {
                    ...
                    result.allowed_vp_cs = try serde.deserializeVpcIdentifierList(&reader, alloc, "VpcIdentifier");
