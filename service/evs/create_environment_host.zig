...
pub const CreateEnvironmentHostInput = struct {
...
pub const CreateEnvironmentHostOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateEnvironmentHostInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonElasticVMwareService.CreateEnvironmentHost");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateEnvironmentHostOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEnvironmentHostOutput, body, allocator);
}
