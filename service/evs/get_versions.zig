...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVersionsInput, options: Options) !GetVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVersionsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonElasticVMwareService.GetVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetVersionsOutput, body, alloc);
}
