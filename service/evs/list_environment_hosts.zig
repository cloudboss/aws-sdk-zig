...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentHostsInput, options: Options) !ListEnvironmentHostsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnvironmentHostsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonElasticVMwareService.ListEnvironmentHosts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnvironmentHostsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEnvironmentHostsOutput, body, alloc);
}
