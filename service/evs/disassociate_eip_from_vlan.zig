...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateEipFromVlanInput, options: Options) !DisassociateEipFromVlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateEipFromVlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonElasticVMwareService.DisassociateEipFromVlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateEipFromVlanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateEipFromVlanOutput, body, alloc);
}
