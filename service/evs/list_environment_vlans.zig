...
pub const ListEnvironmentVlansInput = struct {
...
pub const ListEnvironmentVlansOutput = struct {
    /// A list of VLANs that are associated with the specified environment.
    environment_vlans: ?[]const Vlan = null,
    ...
    pub const json_field_names = .{
        .environment_vlans = "environmentVlans",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentVlansInput, options: CallOptions) !ListEnvironmentVlansOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "evs");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListEnvironmentVlansInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonElasticVMwareService.ListEnvironmentVlans");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListEnvironmentVlansOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEnvironmentVlansOutput, body, allocator);
}
