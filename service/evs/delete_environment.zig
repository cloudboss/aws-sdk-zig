...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteEnvironmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonElasticVMwareService.DeleteEnvironment");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteEnvironmentOutput {
    ...
    return aws.json.parseJsonObject(DeleteEnvironmentOutput, body, alloc);
}
