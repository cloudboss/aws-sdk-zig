...
pub const DeleteEnvironmentConnectorInput = struct {
...
pub const DeleteEnvironmentConnectorOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEnvironmentConnectorInput, options: CallOptions) !DeleteEnvironmentConnectorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteEnvironmentConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonElasticVMwareService.DeleteEnvironmentConnector");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteEnvironmentConnectorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteEnvironmentConnectorOutput, body, allocator);
}
