...
pub const ListEnvironmentConnectorsInput = struct {
...
pub const ListEnvironmentConnectorsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentConnectorsInput, options: CallOptions) !ListEnvironmentConnectorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListEnvironmentConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonElasticVMwareService.ListEnvironmentConnectors");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListEnvironmentConnectorsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEnvironmentConnectorsOutput, body, allocator);
}
