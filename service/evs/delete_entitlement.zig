...
pub const DeleteEntitlementInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteEntitlementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonElasticVMwareService.DeleteEntitlement");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteEntitlementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteEntitlementOutput, body, allocator);
}
