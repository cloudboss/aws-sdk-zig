...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEnvironmentHostInput, options: Options) !DeleteEnvironmentHostOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteEnvironmentHostInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonElasticVMwareService.DeleteEnvironmentHost");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteEnvironmentHostOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteEnvironmentHostOutput, body, alloc);
}
