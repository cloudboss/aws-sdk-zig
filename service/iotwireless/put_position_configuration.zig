...
const PositionSolverConfigurations = @import("position_solver_configurations.zig").PositionSolverConfigurations;

pub const PutPositionConfigurationInput = struct {
    ...
    /// resource.
    solvers: ?PositionSolverConfigurations = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutPositionConfigurationInput, options: Options) !PutPositionConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutPositionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.solvers) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"Solvers\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutPositionConfigurationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutPositionConfigurationOutput = .{};
