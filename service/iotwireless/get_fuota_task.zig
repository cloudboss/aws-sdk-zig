...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFuotaTaskInput, options: Options) !GetFuotaTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFuotaTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFuotaTaskOutput {
    var result: GetFuotaTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFuotaTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
