...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMetricConfigurationInput, options: Options) !GetMetricConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetricConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetricConfigurationOutput {
    var result: GetMetricConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMetricConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
