...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMetricsInput, options: Options) !GetMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.summary_metric_queries) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"SummaryMetricQueries\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMetricsOutput {
    var result: GetMetricsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMetricsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
