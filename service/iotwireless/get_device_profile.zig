...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeviceProfileInput, options: Options) !GetDeviceProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeviceProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeviceProfileOutput {
    var result: GetDeviceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDeviceProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
