...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPartnerAccountInput, options: Options) !GetPartnerAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPartnerAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/partner-accounts/");
    try path_buf.appendSlice(alloc, input.partner_account_id);
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.partner_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPartnerAccountOutput {
    var result: GetPartnerAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPartnerAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
