...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWirelessGatewayTaskDefinitionInput, options: Options) !GetWirelessGatewayTaskDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWirelessGatewayTaskDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/wireless-gateway-task-definitions/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWirelessGatewayTaskDefinitionOutput {
    var result: GetWirelessGatewayTaskDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWirelessGatewayTaskDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
