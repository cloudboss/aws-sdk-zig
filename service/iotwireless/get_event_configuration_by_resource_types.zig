...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEventConfigurationByResourceTypesInput, options: Options) !GetEventConfigurationByResourceTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEventConfigurationByResourceTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEventConfigurationByResourceTypesOutput {
    var result: GetEventConfigurationByResourceTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEventConfigurationByResourceTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
