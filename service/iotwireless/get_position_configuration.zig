...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPositionConfigurationInput, options: Options) !GetPositionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPositionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/position-configurations/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPositionConfigurationOutput {
    var result: GetPositionConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPositionConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
