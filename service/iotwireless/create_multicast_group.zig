...
pub const CreateMulticastGroupInput = struct {
...
pub const CreateMulticastGroupOutput = struct {
    arn: ?[]const u8 = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMulticastGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.lo_ra_wan), input.lo_ra_wan, alloc, &body_buf);
    has_prev = true;
    if (input.name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMulticastGroupOutput {
    var result: CreateMulticastGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMulticastGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
