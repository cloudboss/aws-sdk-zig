...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWirelessDeviceStatisticsInput, options: Options) !GetWirelessDeviceStatisticsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWirelessDeviceStatisticsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWirelessDeviceStatisticsOutput {
    var result: GetWirelessDeviceStatisticsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWirelessDeviceStatisticsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
