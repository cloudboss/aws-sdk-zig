...
pub const DeregisterWirelessDeviceInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterWirelessDeviceInput, options: Options) !DeregisterWirelessDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterWirelessDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/deregister");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterWirelessDeviceOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeregisterWirelessDeviceOutput = .{};
