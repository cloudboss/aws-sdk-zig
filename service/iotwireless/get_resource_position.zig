...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourcePositionInput, options: Options) !GetResourcePositionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourcePositionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/resource-positions/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourcePositionOutput {
    var result: GetResourcePositionOutput = .{};
    if (body.len > 0) {
        result.geo_json_payload = try alloc.dupe(u8, body);
    }
    _ = status;
    _ = headers;
