...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLogLevelsByResourceTypesInput, options: Options) !UpdateLogLevelsByResourceTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLogLevelsByResourceTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.fuota_task_log_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FuotaTaskLogOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.wireless_device_log_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"WirelessDeviceLogOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.wireless_gateway_log_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"WirelessGatewayLogOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLogLevelsByResourceTypesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
