...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateServiceProfileOutput {
    var result: CreateServiceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateServiceProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
