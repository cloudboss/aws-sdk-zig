...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLogLevelsByResourceTypesInput, options: CallOptions) !GetLogLevelsByResourceTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLogLevelsByResourceTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLogLevelsByResourceTypesOutput {
    var result: GetLogLevelsByResourceTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLogLevelsByResourceTypesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
