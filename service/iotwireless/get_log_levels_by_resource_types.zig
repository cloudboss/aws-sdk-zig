...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLogLevelsByResourceTypesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLogLevelsByResourceTypesOutput {
    var result: GetLogLevelsByResourceTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLogLevelsByResourceTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
