...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWirelessDeviceInput, options: Options) !CreateWirelessDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWirelessDeviceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWirelessDeviceOutput {
    var result: CreateWirelessDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWirelessDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
