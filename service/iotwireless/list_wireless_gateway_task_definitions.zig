...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWirelessGatewayTaskDefinitionsInput, options: Options) !ListWirelessGatewayTaskDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWirelessGatewayTaskDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.task_definition_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "taskDefinitionType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWirelessGatewayTaskDefinitionsOutput {
    var result: ListWirelessGatewayTaskDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWirelessGatewayTaskDefinitionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
