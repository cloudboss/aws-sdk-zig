...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDestinationInput, options: CallOptions) !CreateDestinationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.expression_type), input.expression_type, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateDestinationOutput {
    var result: CreateDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDestinationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
