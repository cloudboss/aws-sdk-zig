...
pub const CreateDestinationInput = struct {
...
pub const CreateDestinationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDestinationInput, options: Options) !CreateDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.expression), input.expression, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ExpressionType\":");
    try aws.json.writeValue(@TypeOf(input.expression_type), input.expression_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDestinationOutput {
    var result: CreateDestinationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDestinationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
