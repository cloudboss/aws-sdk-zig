...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWirelessGatewayTaskDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AutoCreateTasks\":");
    try aws.json.writeValue(@TypeOf(input.auto_create_tasks), input.auto_create_tasks, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.update) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Update\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWirelessGatewayTaskDefinitionOutput {
    var result: CreateWirelessGatewayTaskDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWirelessGatewayTaskDefinitionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
