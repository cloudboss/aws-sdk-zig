...
pub const CreateWirelessGatewayTaskDefinitionInput = struct {
...
pub const CreateWirelessGatewayTaskDefinitionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWirelessGatewayTaskDefinitionInput, options: CallOptions) !CreateWirelessGatewayTaskDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateWirelessGatewayTaskDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"AutoCreateTasks\":");
    try aws.json.writeValue(@TypeOf(input.auto_create_tasks), input.auto_create_tasks, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateWirelessGatewayTaskDefinitionOutput {
    var result: CreateWirelessGatewayTaskDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWirelessGatewayTaskDefinitionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
