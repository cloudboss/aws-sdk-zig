...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFuotaTaskInput, options: Options) !CreateFuotaTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFuotaTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.descriptor) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Descriptor\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FirmwareUpdateImage\":");
    try aws.json.writeValue(@TypeOf(input.firmware_update_image), input.firmware_update_image, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"FirmwareUpdateRole\":");
    try aws.json.writeValue(@TypeOf(input.firmware_update_role), input.firmware_update_role, alloc, &body_buf);
    has_prev = true;
    if (input.fragment_interval_ms) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FragmentIntervalMS\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.fragment_size_bytes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FragmentSizeBytes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.redundancy_percent) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RedundancyPercent\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFuotaTaskOutput {
    var result: CreateFuotaTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFuotaTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
