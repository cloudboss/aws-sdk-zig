...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWirelessGatewayInput, options: Options) !CreateWirelessGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWirelessGatewayInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWirelessGatewayOutput {
    var result: CreateWirelessGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWirelessGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
