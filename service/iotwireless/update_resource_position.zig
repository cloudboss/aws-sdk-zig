...
pub const UpdateResourcePositionInput = struct {
    ...
    /// The type of resource for which position information is updated, which can be
    /// a
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResourcePositionInput, options: Options) !UpdateResourcePositionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateResourcePositionInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = input.geo_json_payload orelse "";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateResourcePositionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateResourcePositionOutput = .{};
