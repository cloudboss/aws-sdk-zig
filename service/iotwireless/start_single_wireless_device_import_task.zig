...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSingleWirelessDeviceImportTaskInput, options: Options) !StartSingleWirelessDeviceImportTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSingleWirelessDeviceImportTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSingleWirelessDeviceImportTaskOutput {
    var result: StartSingleWirelessDeviceImportTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartSingleWirelessDeviceImportTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
