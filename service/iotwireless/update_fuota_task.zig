...
pub const UpdateFuotaTaskInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFuotaTaskInput, options: Options) !UpdateFuotaTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFuotaTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.firmware_update_image) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FirmwareUpdateImage\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.firmware_update_role) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFuotaTaskOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateFuotaTaskOutput = .{};
