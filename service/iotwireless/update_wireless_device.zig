...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWirelessDeviceInput, options: Options) !UpdateWirelessDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWirelessDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.positioning) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Positioning\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.sidewalk) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWirelessDeviceOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
