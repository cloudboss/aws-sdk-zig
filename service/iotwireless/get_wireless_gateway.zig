...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWirelessGatewayInput, options: Options) !GetWirelessGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWirelessGatewayInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWirelessGatewayOutput {
    var result: GetWirelessGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWirelessGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
