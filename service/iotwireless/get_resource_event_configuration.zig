...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceEventConfigurationInput, options: Options) !GetResourceEventConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceEventConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceEventConfigurationOutput {
    var result: GetResourceEventConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceEventConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
