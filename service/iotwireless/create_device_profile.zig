...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeviceProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDeviceProfileOutput {
    var result: CreateDeviceProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDeviceProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
