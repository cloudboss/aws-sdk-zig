...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteQueuedMessagesInput, options: Options) !DeleteQueuedMessagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteQueuedMessagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "messageId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.message_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteQueuedMessagesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
