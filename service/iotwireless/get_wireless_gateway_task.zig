...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWirelessGatewayTaskInput, options: Options) !GetWirelessGatewayTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWirelessGatewayTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/tasks");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWirelessGatewayTaskOutput {
    var result: GetWirelessGatewayTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWirelessGatewayTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
