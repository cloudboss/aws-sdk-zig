...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWirelessDeviceInput, options: Options) !GetWirelessDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWirelessDeviceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWirelessDeviceOutput {
    var result: GetWirelessDeviceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWirelessDeviceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
