...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPositionInput, options: Options) !GetPositionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPositionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPositionOutput {
    var result: GetPositionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPositionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
