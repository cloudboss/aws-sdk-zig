...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWirelessDevicesInput, options: Options) !ListWirelessDevicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWirelessDevicesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.destination_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "destinationName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.device_profile_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "deviceProfileId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.fuota_task_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "fuotaTaskId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.multicast_group_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "multicastGroupId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.service_profile_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "serviceProfileId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWirelessDevicesOutput {
    var result: ListWirelessDevicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWirelessDevicesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
