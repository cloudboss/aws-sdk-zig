...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWirelessDeviceImportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/wireless_device_import_task/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWirelessDeviceImportTaskOutput {
    var result: GetWirelessDeviceImportTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWirelessDeviceImportTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
