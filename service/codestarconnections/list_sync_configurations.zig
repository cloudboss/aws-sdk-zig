...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSyncConfigurationsInput, options: Options) !ListSyncConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSyncConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeStar_connections_20191201.ListSyncConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSyncConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSyncConfigurationsOutput, body, alloc);
}
