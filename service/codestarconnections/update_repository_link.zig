...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRepositoryLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeStar_connections_20191201.UpdateRepositoryLink");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRepositoryLinkOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateRepositoryLinkOutput, body, alloc);
}
