...
pub const UpdateRepositoryLinkInput = struct {
    /// The Amazon Resource Name (ARN) of the connection for the repository link to
    /// be updated. The updated connection ARN must have the same providerType (such
...
pub const UpdateRepositoryLinkOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRepositoryLinkInput, options: CallOptions) !UpdateRepositoryLinkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRepositoryLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeStar_connections_20191201.UpdateRepositoryLink");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRepositoryLinkOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateRepositoryLinkOutput, body, allocator);
}
