...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateHostInput, options: Options) !UpdateHostOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateHostInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeStar_connections_20191201.UpdateHost");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateHostOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
