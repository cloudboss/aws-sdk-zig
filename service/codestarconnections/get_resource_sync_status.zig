...
const ResourceSyncAttempt = @import("resource_sync_attempt.zig").ResourceSyncAttempt;

pub const GetResourceSyncStatusInput = struct {
...
pub const GetResourceSyncStatusOutput = struct {
    /// The desired state of the Amazon Web Services resource for the sync status
    /// with the Git
    /// repository.
    desired_state: ?Revision = null,

    /// The latest successful sync for the sync status with the Git repository.
    latest_successful_sync: ?ResourceSyncAttempt = null,

    /// The latest sync for the sync status with the Git repository, whether
    /// successful or not.
    latest_sync: ?ResourceSyncAttempt = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceSyncStatusInput, options: CallOptions) !GetResourceSyncStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetResourceSyncStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeStar_connections_20191201.GetResourceSyncStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetResourceSyncStatusOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetResourceSyncStatusOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "RetryLatestCommitFailedException")) {
        const parsed_error: ?errors.RetryLatestCommitFailedException = aws.json.parseJsonObject(errors.RetryLatestCommitFailedException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .retry_latest_commit_failed_exception = typed_error } };
        }
    }
    if (std.mem.eql(u8, error_code, "SyncBlockerDoesNotExistException")) {
        const parsed_error: ?errors.SyncBlockerDoesNotExistException = aws.json.parseJsonObject(errors.SyncBlockerDoesNotExistException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .sync_blocker_does_not_exist_exception = typed_error } };
        }
    }
    if (std.mem.eql(u8, error_code, "SyncConfigurationStillExistsException")) {
        const parsed_error: ?errors.SyncConfigurationStillExistsException = aws.json.parseJsonObject(errors.SyncConfigurationStillExistsException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .sync_configuration_still_exists_exception = typed_error } };
        }
    }
    ...
    if (std.mem.eql(u8, error_code, "UnsupportedProviderTypeException")) {
        const parsed_error: ?errors.UnsupportedProviderTypeException = aws.json.parseJsonObject(errors.UnsupportedProviderTypeException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .unsupported_provider_type_exception = typed_error } };
        }
    }
    if (std.mem.eql(u8, error_code, "UpdateOutOfSyncException")) {
        const parsed_error: ?errors.UpdateOutOfSyncException = aws.json.parseJsonObject(errors.UpdateOutOfSyncException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .update_out_of_sync_exception = typed_error } };
        }
    }
