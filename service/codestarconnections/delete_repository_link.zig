...
pub const DeleteRepositoryLinkInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRepositoryLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeStar_connections_20191201.DeleteRepositoryLink");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRepositoryLinkOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
