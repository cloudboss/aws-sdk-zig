...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRepositoryLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeStar_connections_20191201.DeleteRepositoryLink");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRepositoryLinkOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
