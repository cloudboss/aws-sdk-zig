...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBenefitApplicationInput, options: Options) !CreateBenefitApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBenefitApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralBenefitsService.CreateBenefitApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBenefitApplicationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateBenefitApplicationOutput, body, alloc);
}
