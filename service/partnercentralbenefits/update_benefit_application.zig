...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBenefitApplicationInput, options: Options) !UpdateBenefitApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBenefitApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralBenefitsService.UpdateBenefitApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBenefitApplicationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateBenefitApplicationOutput, body, alloc);
}
