...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateBenefitApplicationResourceInput, options: Options) !DisassociateBenefitApplicationResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateBenefitApplicationResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralBenefitsService.DisassociateBenefitApplicationResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateBenefitApplicationResourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateBenefitApplicationResourceOutput, body, alloc);
}
