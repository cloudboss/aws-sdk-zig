...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBenefitAllocationsInput, options: Options) !ListBenefitAllocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBenefitAllocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralBenefitsService.ListBenefitAllocations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBenefitAllocationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBenefitAllocationsOutput, body, alloc);
}
