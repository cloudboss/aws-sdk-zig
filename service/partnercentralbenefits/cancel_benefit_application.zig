...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelBenefitApplicationInput, options: Options) !CancelBenefitApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelBenefitApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralBenefitsService.CancelBenefitApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelBenefitApplicationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
