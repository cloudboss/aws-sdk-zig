...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBenefitApplicationInput, options: Options) !GetBenefitApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBenefitApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralBenefitsService.GetBenefitApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBenefitApplicationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetBenefitApplicationOutput, body, alloc);
}
