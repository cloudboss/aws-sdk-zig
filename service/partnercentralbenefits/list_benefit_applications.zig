...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBenefitApplicationsInput, options: Options) !ListBenefitApplicationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBenefitApplicationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralBenefitsService.ListBenefitApplications");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBenefitApplicationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBenefitApplicationsOutput, body, alloc);
}
