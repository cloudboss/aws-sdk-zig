...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AmendBenefitApplicationInput, options: Options) !AmendBenefitApplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AmendBenefitApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralBenefitsService.AmendBenefitApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AmendBenefitApplicationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
