...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBenefitsInput, options: Options) !ListBenefitsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBenefitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PartnerCentralBenefitsService.ListBenefits");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBenefitsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBenefitsOutput, body, alloc);
}
