...
fn serializeRequest(alloc: std.mem.Allocator, input: PutConfigurationSetTrackingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/tracking-options");
    ...
    if (input.custom_redirect_domain) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"CustomRedirectDomain\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutConfigurationSetTrackingOptionsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
