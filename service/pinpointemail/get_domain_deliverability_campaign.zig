...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDomainDeliverabilityCampaignInput, options: Options) !GetDomainDeliverabilityCampaignOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainDeliverabilityCampaignInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/email/deliverability-dashboard/campaigns/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainDeliverabilityCampaignOutput {
    var result: GetDomainDeliverabilityCampaignOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainDeliverabilityCampaignOutput, body, alloc);
    }
    _ = status;
    _ = headers;
