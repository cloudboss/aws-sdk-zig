...
pub const PutDeliverabilityDashboardOptionInput = struct {
    ...
    /// Pinpoint account. To enable the
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDeliverabilityDashboardOptionInput, options: Options) !PutDeliverabilityDashboardOptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDeliverabilityDashboardOptionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/email/deliverability-dashboard";
    ...
    try body_buf.appendSlice(alloc, "\"DashboardEnabled\":");
    try aws.json.writeValue(@TypeOf(input.dashboard_enabled), input.dashboard_enabled, alloc, &body_buf);
    has_prev = true;
    if (input.subscribed_domains) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SubscribedDomains\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDeliverabilityDashboardOptionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
