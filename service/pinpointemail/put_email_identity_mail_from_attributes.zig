...
fn serializeRequest(allocator: std.mem.Allocator, input: PutEmailIdentityMailFromAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/mail-from");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutEmailIdentityMailFromAttributesOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
