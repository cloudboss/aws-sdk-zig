...
pub const PutEmailIdentityMailFromAttributesInput = struct {
    /// The action that you want Amazon Pinpoint to take if it can't read the
    /// required MX record when
    /// you send an email. When you set this value to `UseDefaultValue`, Amazon
    /// Pinpoint uses
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutEmailIdentityMailFromAttributesInput, options: Options) !PutEmailIdentityMailFromAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEmailIdentityMailFromAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/mail-from");
    ...
    if (input.behavior_on_mx_failure) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"BehaviorOnMxFailure\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.mail_from_domain) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MailFromDomain\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEmailIdentityMailFromAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: PutEmailIdentityMailFromAttributesOutput = .{};
