...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDedicatedIpPoolInput, options: Options) !CreateDedicatedIpPoolOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDedicatedIpPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"PoolName\":");
    try aws.json.writeValue(@TypeOf(input.pool_name), input.pool_name, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDedicatedIpPoolOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
