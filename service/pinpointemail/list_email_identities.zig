...
pub const ListEmailIdentitiesOutput = struct {
    /// An array that includes all of the identities associated with your Amazon
    /// Pinpoint
    /// account.
    email_identities: ?[]const IdentityInfo = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEmailIdentitiesInput, options: Options) !ListEmailIdentitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEmailIdentitiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEmailIdentitiesOutput {
    var result: ListEmailIdentitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEmailIdentitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
