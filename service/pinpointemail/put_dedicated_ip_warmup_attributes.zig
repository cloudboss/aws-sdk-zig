...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDedicatedIpWarmupAttributesInput, options: Options) !PutDedicatedIpWarmupAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDedicatedIpWarmupAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/email/dedicated-ips/");
    try path_buf.appendSlice(alloc, input.ip);
    try path_buf.appendSlice(alloc, "/warmup");
    ...
    try body_buf.appendSlice(alloc, "\"WarmupPercentage\":");
    try aws.json.writeValue(@TypeOf(input.warmup_percentage), input.warmup_percentage, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDedicatedIpWarmupAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
