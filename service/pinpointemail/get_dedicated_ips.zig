...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDedicatedIpsInput, options: Options) !GetDedicatedIpsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDedicatedIpsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/email/dedicated-ips";
    ...
    if (input.pool_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "PoolName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDedicatedIpsOutput {
    var result: GetDedicatedIpsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDedicatedIpsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
