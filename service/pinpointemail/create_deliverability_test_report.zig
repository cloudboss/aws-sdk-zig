...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDeliverabilityTestReportInput, options: Options) !CreateDeliverabilityTestReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeliverabilityTestReportInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/email/deliverability-dashboard/test";
    ...
    if (input.report_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ReportName\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDeliverabilityTestReportOutput {
    var result: CreateDeliverabilityTestReportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDeliverabilityTestReportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
