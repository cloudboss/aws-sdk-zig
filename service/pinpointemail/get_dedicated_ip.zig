...
pub const GetDedicatedIpInput = struct {
    ...
    /// specify
    /// has to be a dedicated IP address that's assocaited with your Amazon Pinpoint
    /// account.
    ip: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDedicatedIpInput, options: Options) !GetDedicatedIpOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDedicatedIpInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDedicatedIpOutput {
    var result: GetDedicatedIpOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDedicatedIpOutput, body, alloc);
    }
    _ = status;
    _ = headers;
