...
fn serializeRequest(allocator: std.mem.Allocator, input: PutConfigurationSetReputationOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/reputation-options");
    ...
    if (input.reputation_metrics_enabled) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"ReputationMetricsEnabled\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutConfigurationSetReputationOptionsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
