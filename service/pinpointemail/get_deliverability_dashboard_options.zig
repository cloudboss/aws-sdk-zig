...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeliverabilityDashboardOptionsInput, options: Options) !GetDeliverabilityDashboardOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeliverabilityDashboardOptionsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeliverabilityDashboardOptionsOutput {
    var result: GetDeliverabilityDashboardOptionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDeliverabilityDashboardOptionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
