...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutEmailIdentityFeedbackAttributesInput, options: Options) !PutEmailIdentityFeedbackAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEmailIdentityFeedbackAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/email/identities/");
    ...
    if (input.email_forwarding_enabled) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"EmailForwardingEnabled\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEmailIdentityFeedbackAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
