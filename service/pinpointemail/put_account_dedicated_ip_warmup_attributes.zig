...
pub const PutAccountDedicatedIpWarmupAttributesInput = struct {
    ...
    /// that are
    /// associated with your Amazon Pinpoint account in the current AWS Region. Set
    /// to `true`
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccountDedicatedIpWarmupAttributesInput, options: Options) !PutAccountDedicatedIpWarmupAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccountDedicatedIpWarmupAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/email/account/dedicated-ips/warmup";
    ...
    if (input.auto_warmup_enabled) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AutoWarmupEnabled\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccountDedicatedIpWarmupAttributesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
