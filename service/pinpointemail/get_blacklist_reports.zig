...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBlacklistReportsInput, options: Options) !GetBlacklistReportsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBlacklistReportsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/email/deliverability-dashboard/blacklist-report";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "BlacklistItemNames=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.blacklist_item_names);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBlacklistReportsOutput {
    var result: GetBlacklistReportsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBlacklistReportsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
