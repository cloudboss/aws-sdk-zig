...
pub const SendEmailOutput = struct {
    ...
    /// This can happen
    /// when the message you're trying to send has an attachment doesn't pass a
    /// virus check,
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/email/outbound-emails";
    ...
    if (input.email_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EmailTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendEmailOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendEmailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
