...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDeliverabilityTestReportInput, options: Options) !GetDeliverabilityTestReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDeliverabilityTestReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/email/deliverability-dashboard/test-reports/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDeliverabilityTestReportOutput {
    var result: GetDeliverabilityTestReportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDeliverabilityTestReportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
