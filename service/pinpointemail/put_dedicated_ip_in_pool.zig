...
fn serializeRequest(allocator: std.mem.Allocator, input: PutDedicatedIpInPoolInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/pool");
    ...
    try body_buf.appendSlice(allocator, "\"DestinationPoolName\":");
    try aws.json.writeValue(@TypeOf(input.destination_pool_name), input.destination_pool_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutDedicatedIpInPoolOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
