...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainStatisticsReportInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/email/deliverability-dashboard/statistics-report/");
    try path_buf.appendSlice(alloc, input.domain);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainStatisticsReportOutput {
    var result: GetDomainStatisticsReportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainStatisticsReportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
