...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "EndpointTemporarilyUnavailableException")) {
        const parsed_error: ?errors.EndpointTemporarilyUnavailableException = aws.json.parseJsonObject(errors.EndpointTemporarilyUnavailableException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
