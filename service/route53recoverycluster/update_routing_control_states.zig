...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoutingControlStatesInput, options: Options) !UpdateRoutingControlStatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoutingControlStatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ToggleCustomerAPI.UpdateRoutingControlStates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoutingControlStatesOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
