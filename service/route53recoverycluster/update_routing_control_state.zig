...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoutingControlStateInput, options: Options) !UpdateRoutingControlStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoutingControlStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ToggleCustomerAPI.UpdateRoutingControlState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoutingControlStateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
