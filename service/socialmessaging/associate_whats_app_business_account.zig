...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateWhatsAppBusinessAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.setup_finalization) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"setupFinalization\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.signup_callback) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"signupCallback\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateWhatsAppBusinessAccountOutput {
    var result: AssociateWhatsAppBusinessAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateWhatsAppBusinessAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
