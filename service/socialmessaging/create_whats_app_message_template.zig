...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWhatsAppMessageTemplateInput, options: Options) !CreateWhatsAppMessageTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWhatsAppMessageTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"templateDefinition\":");
    try aws.json.writeValue(@TypeOf(input.template_definition), input.template_definition, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWhatsAppMessageTemplateOutput {
    var result: CreateWhatsAppMessageTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWhatsAppMessageTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
