...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutWhatsAppBusinessAccountEventDestinationsInput, options: Options) !PutWhatsAppBusinessAccountEventDestinationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutWhatsAppBusinessAccountEventDestinationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"eventDestinations\":");
    try aws.json.writeValue(@TypeOf(input.event_destinations), input.event_destinations, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutWhatsAppBusinessAccountEventDestinationsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
