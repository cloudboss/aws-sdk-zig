...
pub const GetWhatsAppMessageMediaInput = struct {
    /// The `bucketName` and `key` of the S3 media file.
    destination_s3_file: ?S3File = null,

    /// The presign url of the media file.
    destination_s3_presigned_url: ?S3PresignedUrl = null,
    ...
    pub const json_field_names = .{
        .destination_s3_file = "destinationS3File",
        .destination_s3_presigned_url = "destinationS3PresignedUrl",
        .media_id = "mediaId",
...
pub const GetWhatsAppMessageMediaOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWhatsAppMessageMediaInput, options: CallOptions) !GetWhatsAppMessageMediaOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetWhatsAppMessageMediaInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/v1/whatsapp/media/get";

    var body_buf: std.ArrayList(u8) = .{};
    var has_prev = false;
    try body_buf.appendSlice(allocator, "{");

    if (input.destination_s3_file) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"destinationS3File\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.destination_s3_presigned_url) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"destinationS3PresignedUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"mediaId\":");
    try aws.json.writeValue(@TypeOf(input.media_id), input.media_id, allocator, &body_buf);
    has_prev = true;
    if (input.metadata_only) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"metadataOnly\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetWhatsAppMessageMediaOutput {
    var result: GetWhatsAppMessageMediaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWhatsAppMessageMediaOutput, body, allocator);
    }
    _ = status;
    _ = headers;
