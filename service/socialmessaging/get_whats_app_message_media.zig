...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWhatsAppMessageMediaInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.destination_s3_file) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"destinationS3File\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.destination_s3_presigned_url) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"destinationS3PresignedUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"mediaId\":");
    try aws.json.writeValue(@TypeOf(input.media_id), input.media_id, alloc, &body_buf);
    has_prev = true;
    if (input.metadata_only) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"metadataOnly\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWhatsAppMessageMediaOutput {
    var result: GetWhatsAppMessageMediaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWhatsAppMessageMediaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
