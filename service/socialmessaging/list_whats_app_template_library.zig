...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWhatsAppTemplateLibraryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWhatsAppTemplateLibraryOutput {
    var result: ListWhatsAppTemplateLibraryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWhatsAppTemplateLibraryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
