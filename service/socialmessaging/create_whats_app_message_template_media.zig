...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWhatsAppMessageTemplateMediaInput, options: Options) !CreateWhatsAppMessageTemplateMediaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWhatsAppMessageTemplateMediaInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWhatsAppMessageTemplateMediaOutput {
    var result: CreateWhatsAppMessageTemplateMediaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWhatsAppMessageTemplateMediaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
