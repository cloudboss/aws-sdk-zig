...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteWhatsAppMessageMediaInput, options: Options) !DeleteWhatsAppMessageMediaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteWhatsAppMessageMediaInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "mediaId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.media_id);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "originationPhoneNumberId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.origination_phone_number_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteWhatsAppMessageMediaOutput {
    var result: DeleteWhatsAppMessageMediaOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteWhatsAppMessageMediaOutput, body, alloc);
    }
    _ = status;
    _ = headers;
