...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLinkedWhatsAppBusinessAccountInput, options: Options) !GetLinkedWhatsAppBusinessAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLinkedWhatsAppBusinessAccountInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLinkedWhatsAppBusinessAccountOutput {
    var result: GetLinkedWhatsAppBusinessAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLinkedWhatsAppBusinessAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
