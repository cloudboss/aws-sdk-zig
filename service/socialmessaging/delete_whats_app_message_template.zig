...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteWhatsAppMessageTemplateInput, options: Options) !DeleteWhatsAppMessageTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteWhatsAppMessageTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.delete_all_languages) |v| {
        ...
        try query_buf.appendSlice(alloc, "deleteAllTemplates=");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.template_name);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteWhatsAppMessageTemplateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
