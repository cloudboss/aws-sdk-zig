...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendWhatsAppMessageInput, options: Options) !SendWhatsAppMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendWhatsAppMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"metaApiVersion\":");
    try aws.json.writeValue(@TypeOf(input.meta_api_version), input.meta_api_version, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendWhatsAppMessageOutput {
    var result: SendWhatsAppMessageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendWhatsAppMessageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
