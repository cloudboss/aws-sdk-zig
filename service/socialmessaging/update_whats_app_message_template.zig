...
pub const UpdateWhatsAppMessageTemplateInput = struct {
    /// When true, disables click tracking for call-to-action URL buttons in the
    /// template.
    cta_url_link_tracking_opted_out: ?bool = null,
    ...
    /// 'named' or 'positional'.
    parameter_format: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWhatsAppMessageTemplateInput, options: Options) !UpdateWhatsAppMessageTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWhatsAppMessageTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cta_url_link_tracking_opted_out) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ctaUrlLinkTrackingOptedOut\":");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"metaTemplateId\":");
    try aws.json.writeValue(@TypeOf(input.meta_template_id), input.meta_template_id, alloc, &body_buf);
    has_prev = true;
    if (input.parameter_format) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"parameterFormat\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.template_category) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"templateCategory\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.template_components) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"templateComponents\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWhatsAppMessageTemplateOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateWhatsAppMessageTemplateOutput = .{};
