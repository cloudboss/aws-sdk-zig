...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateWhatsAppMessageTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cta_url_link_tracking_opted_out) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"ctaUrlLinkTrackingOptedOut\":");
    ...
    if (input.template_category) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"templateCategory\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateWhatsAppMessageTemplateOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
