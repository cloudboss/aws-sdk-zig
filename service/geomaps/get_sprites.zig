...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSpritesInput, options: Options) !GetSpritesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSpritesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.color_scheme);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.variant);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSpritesOutput {
