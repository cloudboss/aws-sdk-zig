...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStyleDescriptorInput, options: Options) !GetStyleDescriptorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetStyleDescriptorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/styles/");
    try path_buf.appendSlice(alloc, input.style);
    try path_buf.appendSlice(alloc, "/descriptor");
    ...
    if (input.buildings) |v| {
        ...
        try query_buf.appendSlice(alloc, "buildings=");
    ...
    if (input.contour_density) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "contour-density=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.key) |v| {
    ...
    if (input.terrain) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "terrain=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.traffic) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "traffic=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.travel_modes) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "travel-modes=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetStyleDescriptorOutput {
