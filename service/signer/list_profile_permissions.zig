...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProfilePermissionsInput, options: Options) !ListProfilePermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProfilePermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProfilePermissionsOutput {
    var result: ListProfilePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListProfilePermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
