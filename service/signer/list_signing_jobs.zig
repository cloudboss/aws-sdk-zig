...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSigningJobsInput, options: Options) !ListSigningJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSigningJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.is_revoked) |v| {
        ...
        try query_buf.appendSlice(alloc, "isRevoked=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.job_invoker) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "jobInvoker=");
    ...
    if (input.requested_by) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "requestedBy=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.signature_expires_after) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "signatureExpiresAfter=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
    ...
    if (input.signature_expires_before) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "signatureExpiresBefore=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSigningJobsOutput {
    var result: ListSigningJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSigningJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
