...
pub const RevokeSignatureInput = struct {
...
pub const RevokeSignatureOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeSignatureInput, options: CallOptions) !RevokeSignatureOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RevokeSignatureInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.job_owner) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"jobOwner\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RevokeSignatureOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: RevokeSignatureOutput = .{};
