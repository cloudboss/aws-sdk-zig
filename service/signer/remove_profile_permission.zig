...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveProfilePermissionInput, options: Options) !RemoveProfilePermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveProfilePermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "revisionId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.revision_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveProfilePermissionOutput {
    var result: RemoveProfilePermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveProfilePermissionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
