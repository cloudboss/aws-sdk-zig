...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSigningProfilesInput, options: Options) !ListSigningProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSigningProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_canceled) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeCanceled=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSigningProfilesOutput {
    var result: ListSigningProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSigningProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
