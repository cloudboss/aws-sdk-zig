...
pub const AddProfilePermissionInput = struct {
    /// For cross-account signing. Grant a designated account permission to perform
    /// one or more of the following actions. Each action is associated with a
    /// specific API's operations. For more information about cross-account signing,
    /// see [Using cross-account signing with signing
    /// profiles](http://docs.aws.amazon.com/signer/latest/developerguide/signing-profile-cross-account.html) in the *AWS Signer Developer Guide*.
    ///
...
pub const AddProfilePermissionOutput = struct {
    /// A unique identifier for the current profile revision.
    revision_id: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddProfilePermissionInput, options: Options) !AddProfilePermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddProfilePermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddProfilePermissionOutput {
    var result: AddProfilePermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddProfilePermissionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
