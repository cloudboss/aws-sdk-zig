...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSigningPlatformsInput, options: Options) !ListSigningPlatformsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSigningPlatformsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.category) |v| {
        ...
        try query_buf.appendSlice(alloc, "category=");
    ...
    if (input.partner) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "partner=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.target) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSigningPlatformsOutput {
    var result: ListSigningPlatformsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSigningPlatformsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
