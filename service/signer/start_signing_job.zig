...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartSigningJobInput, options: Options) !StartSigningJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSigningJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSigningJobOutput {
    var result: StartSigningJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartSigningJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
