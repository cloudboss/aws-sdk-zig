...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSigningProfileInput, options: Options) !GetSigningProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSigningProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.profile_owner) |v| {
        ...
        try query_buf.appendSlice(alloc, "profileOwner=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSigningProfileOutput {
    var result: GetSigningProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSigningProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
