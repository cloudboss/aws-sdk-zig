...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSigningJobInput, options: Options) !DescribeSigningJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSigningJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSigningJobOutput {
    var result: DescribeSigningJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSigningJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
