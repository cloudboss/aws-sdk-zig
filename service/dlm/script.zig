const ExecutionHandlerServiceValues = @import("execution_handler_service_values.zig").ExecutionHandlerServiceValues;
const StageValues = @import("stage_values.zig").StageValues;

/// **[Custom snapshot policies that target instances only]** Information about
/// pre and/or post scripts for a
...
pub const Script = struct {
    ...
    /// crash-consistent snapshots if the
    /// pre script fails.
    ///
    ...
    /// run attempt if it has not completed. If a script does not complete within
    /// its
    /// timeout period, Amazon Data Lifecycle Manager fails the attempt. The timeout
    /// period applies to the pre
    ...
    /// run before Amazon Data Lifecycle Manager initiates snapshot creation. Post
    /// scripts run after Amazon Data Lifecycle Manager
    ...
    /// this case, Amazon Data Lifecycle Manager calls the SSM document with the
    /// `pre-script`
