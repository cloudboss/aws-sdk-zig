...
pub const ListItemsInput = struct {
    /// The maximum number of results to return per API request. For example, you
    /// submit a
    /// `ListItems` request with `MaxResults` set at 500. Although 2,000
    /// items match your request, the service returns no more than the first 500
    /// items. (The
    /// service also returns a `NextToken` value that you can use to fetch the next
    /// batch of results.) The service might return fewer results than the
    /// `MaxResults`
    /// value.
    ///
    ...
    /// example,
    /// you submit a `ListItems` request with `MaxResults` set at 500. The
    /// service returns the first batch of results (up to 500) and a `NextToken`
    /// value.
...
pub const ListItemsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListItemsInput, options: Options) !ListItemsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListItemsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListItemsOutput {
    var result: ListItemsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListItemsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
