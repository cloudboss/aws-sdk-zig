...
pub const CreateStreamKeyInput = struct {
...
pub const CreateStreamKeyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStreamKeyInput, options: Options) !CreateStreamKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStreamKeyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStreamKeyOutput {
    var result: CreateStreamKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateStreamKeyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
