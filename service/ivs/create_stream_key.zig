...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStreamKeyInput, options: CallOptions) !CreateStreamKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateStreamKeyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateStreamKeyOutput {
    var result: CreateStreamKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateStreamKeyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
