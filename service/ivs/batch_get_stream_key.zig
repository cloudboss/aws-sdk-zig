...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetStreamKeyInput, options: Options) !BatchGetStreamKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetStreamKeyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetStreamKeyOutput {
    var result: BatchGetStreamKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetStreamKeyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
