...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStreamKeysInput, options: Options) !ListStreamKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStreamKeysInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStreamKeysOutput {
    var result: ListStreamKeysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListStreamKeysOutput, body, alloc);
    }
    _ = status;
    _ = headers;
