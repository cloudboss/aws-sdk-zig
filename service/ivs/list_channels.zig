...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChannelsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.filter_by_name) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"filterByName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.filter_by_playback_restriction_policy_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"filterByPlaybackRestrictionPolicyArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.filter_by_recording_configuration_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"filterByRecordingConfigurationArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
