...
pub const GetAdConfigurationInput = struct {
...
pub const GetAdConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAdConfigurationInput, options: CallOptions) !GetAdConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAdConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAdConfigurationOutput {
    var result: GetAdConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAdConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
