...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportPlaybackKeyPairInput, options: Options) !ImportPlaybackKeyPairOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportPlaybackKeyPairInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportPlaybackKeyPairOutput {
    var result: ImportPlaybackKeyPairOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportPlaybackKeyPairOutput, body, alloc);
    }
    _ = status;
    _ = headers;
