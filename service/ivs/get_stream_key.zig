...
pub const GetStreamKeyInput = struct {
...
pub const GetStreamKeyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetStreamKeyInput, options: CallOptions) !GetStreamKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetStreamKeyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetStreamKeyOutput {
    var result: GetStreamKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetStreamKeyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
