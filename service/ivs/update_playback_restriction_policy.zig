...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePlaybackRestrictionPolicyInput, options: Options) !UpdatePlaybackRestrictionPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePlaybackRestrictionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allowed_countries) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"allowedCountries\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.allowed_origins) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"allowedOrigins\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.enable_strict_origin_enforcement) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"enableStrictOriginEnforcement\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePlaybackRestrictionPolicyOutput {
    var result: UpdatePlaybackRestrictionPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePlaybackRestrictionPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
