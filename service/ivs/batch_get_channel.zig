...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetChannelInput, options: Options) !BatchGetChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"arns\":");
    try aws.json.writeValue(@TypeOf(input.arns), input.arns, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetChannelOutput {
    var result: BatchGetChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetChannelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
