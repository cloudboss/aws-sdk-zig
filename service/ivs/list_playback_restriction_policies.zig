...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPlaybackRestrictionPoliciesInput, options: Options) !ListPlaybackRestrictionPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPlaybackRestrictionPoliciesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPlaybackRestrictionPoliciesOutput {
    var result: ListPlaybackRestrictionPoliciesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPlaybackRestrictionPoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
