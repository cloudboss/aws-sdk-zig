...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePlaybackRestrictionPolicyInput, options: Options) !CreatePlaybackRestrictionPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePlaybackRestrictionPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePlaybackRestrictionPolicyOutput {
    var result: CreatePlaybackRestrictionPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePlaybackRestrictionPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
