...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartViewerSessionRevocationInput, options: Options) !StartViewerSessionRevocationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartViewerSessionRevocationInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"viewerId\":");
    try aws.json.writeValue(@TypeOf(input.viewer_id), input.viewer_id, alloc, &body_buf);
    has_prev = true;
    if (input.viewer_session_versions_less_than_or_equal_to) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"viewerSessionVersionsLessThanOrEqualTo\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartViewerSessionRevocationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
