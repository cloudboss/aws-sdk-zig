...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchStartViewerSessionRevocationInput, options: Options) !BatchStartViewerSessionRevocationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchStartViewerSessionRevocationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"viewerSessions\":");
    try aws.json.writeValue(@TypeOf(input.viewer_sessions), input.viewer_sessions, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchStartViewerSessionRevocationOutput {
    var result: BatchStartViewerSessionRevocationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchStartViewerSessionRevocationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
