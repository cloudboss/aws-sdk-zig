...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIdMappingWorkflowInput, options: CallOptions) !CreateIdMappingWorkflowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateIdMappingWorkflowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateIdMappingWorkflowOutput {
    var result: CreateIdMappingWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIdMappingWorkflowOutput, body, allocator);
    }
    _ = status;
    _ = headers;
