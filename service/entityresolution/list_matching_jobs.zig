...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMatchingJobsInput, options: Options) !ListMatchingJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMatchingJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMatchingJobsOutput {
    var result: ListMatchingJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMatchingJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
