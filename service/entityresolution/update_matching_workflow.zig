...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMatchingWorkflowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMatchingWorkflowOutput {
    var result: UpdateMatchingWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMatchingWorkflowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
