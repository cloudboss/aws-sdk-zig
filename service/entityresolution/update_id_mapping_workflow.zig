...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIdMappingWorkflowInput, options: Options) !UpdateIdMappingWorkflowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIdMappingWorkflowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIdMappingWorkflowOutput {
    var result: UpdateIdMappingWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIdMappingWorkflowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
