...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMatchIdInput, options: Options) !GetMatchIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMatchIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/matches");
    ...
    if (input.apply_normalization) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"applyNormalization\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"record\":");
    try aws.json.writeValue(@TypeOf(input.record), input.record, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMatchIdOutput {
    var result: GetMatchIdOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMatchIdOutput, body, alloc);
    }
    _ = status;
    _ = headers;
