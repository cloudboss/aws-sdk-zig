...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSchemaMappingInput, options: Options) !GetSchemaMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSchemaMappingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSchemaMappingOutput {
    var result: GetSchemaMappingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSchemaMappingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
