...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIdNamespaceInput, options: Options) !UpdateIdNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIdNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIdNamespaceOutput {
    var result: UpdateIdNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIdNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
