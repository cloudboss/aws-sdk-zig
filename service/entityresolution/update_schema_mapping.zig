...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSchemaMappingInput, options: Options) !UpdateSchemaMappingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSchemaMappingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSchemaMappingOutput {
    var result: UpdateSchemaMappingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSchemaMappingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
