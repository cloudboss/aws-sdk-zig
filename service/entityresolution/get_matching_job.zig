...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMatchingJobInput, options: Options) !GetMatchingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMatchingJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMatchingJobOutput {
    var result: GetMatchingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMatchingJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
