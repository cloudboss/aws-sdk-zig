...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddPolicyStatementInput, options: Options) !AddPolicyStatementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddPolicyStatementInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"effect\":");
    try aws.json.writeValue(@TypeOf(input.effect), input.effect, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddPolicyStatementOutput {
    var result: AddPolicyStatementOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddPolicyStatementOutput, body, alloc);
    }
    _ = status;
    _ = headers;
