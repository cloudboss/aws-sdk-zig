...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdNamespaceInput, options: Options) !GetIdNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdNamespaceOutput {
    var result: GetIdNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIdNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
