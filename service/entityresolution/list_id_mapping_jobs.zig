...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdMappingJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdMappingJobsOutput {
    var result: ListIdMappingJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIdMappingJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
