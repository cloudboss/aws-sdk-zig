...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMatchingJobInput, options: Options) !StartMatchingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMatchingJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMatchingJobOutput {
    var result: StartMatchingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMatchingJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
