...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdMappingJobInput, options: Options) !GetIdMappingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdMappingJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdMappingJobOutput {
    var result: GetIdMappingJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIdMappingJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
