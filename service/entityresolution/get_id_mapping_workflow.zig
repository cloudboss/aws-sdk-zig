...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdMappingWorkflowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdMappingWorkflowOutput {
    var result: GetIdMappingWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIdMappingWorkflowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
