...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSchemaMappingInput, options: CallOptions) !CreateSchemaMappingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSchemaMappingInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"schemaName\":");
    try aws.json.writeValue(@TypeOf(input.schema_name), input.schema_name, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSchemaMappingOutput {
    var result: CreateSchemaMappingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSchemaMappingOutput, body, allocator);
    }
    _ = status;
    _ = headers;
