...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMatchingWorkflowInput, options: Options) !GetMatchingWorkflowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMatchingWorkflowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMatchingWorkflowOutput {
    var result: GetMatchingWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMatchingWorkflowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
