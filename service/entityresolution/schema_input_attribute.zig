...
/// The `SchemaInputAttribute` specifies how individual fields in your input
/// data should be processed and matched.
pub const SchemaInputAttribute = struct {
    ...
    ///
    /// Normalization is only supported for `NAME`, `ADDRESS`, `PHONE`, and
    /// `EMAIL_ADDRESS`.
    ///
    /// If you want to normalize `NAME_FIRST`, `NAME_MIDDLE`, and `NAME_LAST`, you
    /// must group them by assigning them to the `NAME` `groupName`.
    ///
    /// If you want to normalize `ADDRESS_STREET1`, `ADDRESS_STREET2`,
    /// `ADDRESS_STREET3`, `ADDRESS_CITY`, `ADDRESS_STATE`, `ADDRESS_COUNTRY`, and
    /// `ADDRESS_POSTALCODE`, you must group them by assigning them to the `ADDRESS`
    /// `groupName`.
    ///
