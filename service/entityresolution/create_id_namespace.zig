...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIdNamespaceInput, options: Options) !CreateIdNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIdNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.id_mapping_workflow_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"idMappingWorkflowProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"idNamespaceName\":");
    try aws.json.writeValue(@TypeOf(input.id_namespace_name), input.id_namespace_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIdNamespaceOutput {
    var result: CreateIdNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIdNamespaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
