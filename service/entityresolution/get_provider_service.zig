...
fn serializeRequest(alloc: std.mem.Allocator, input: GetProviderServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/providerservices/");
    try path_buf.appendSlice(alloc, input.provider_name);
    try path_buf.appendSlice(alloc, "/");
    try path_buf.appendSlice(alloc, input.provider_service_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetProviderServiceOutput {
    var result: GetProviderServiceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetProviderServiceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
