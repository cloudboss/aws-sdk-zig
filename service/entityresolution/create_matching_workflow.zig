...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMatchingWorkflowInput, options: CallOptions) !CreateMatchingWorkflowOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateMatchingWorkflowInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"workflowName\":");
    try aws.json.writeValue(@TypeOf(input.workflow_name), input.workflow_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateMatchingWorkflowOutput {
    var result: CreateMatchingWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMatchingWorkflowOutput, body, allocator);
    }
    _ = status;
    _ = headers;
