...
pub const CreateMatchingWorkflowInput = struct {
...
pub const CreateMatchingWorkflowOutput = struct {
    /// A description of the workflow.
    description: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMatchingWorkflowInput, options: Options) !CreateMatchingWorkflowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMatchingWorkflowInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.output_source_config), input.output_source_config, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"resolutionTechniques\":");
    try aws.json.writeValue(@TypeOf(input.resolution_techniques), input.resolution_techniques, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMatchingWorkflowOutput {
    var result: CreateMatchingWorkflowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMatchingWorkflowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
