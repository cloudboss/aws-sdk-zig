...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteUniqueIdInput, options: Options) !BatchDeleteUniqueIdOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteUniqueIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/uniqueids");
    ...
    if (input.input_source) |v| {
        try request.headers.put(alloc, "inputSource", v);
    }
    {
        var header_buf: std.ArrayList(u8) = .{};
        for (input.unique_ids) |item| {
            if (header_buf.items.len > 0) try header_buf.appendSlice(alloc, ", ");
            try header_buf.appendSlice(alloc, item);
        }
        try request.headers.put(alloc, "uniqueIds", header_buf.items);
    }

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteUniqueIdOutput {
    var result: BatchDeleteUniqueIdOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteUniqueIdOutput, body, alloc);
    }
    _ = status;
    _ = headers;
