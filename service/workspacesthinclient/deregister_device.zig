...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterDeviceInput, options: Options) !DeregisterDeviceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterDeviceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/deregister-device/");
    ...
    if (input.target_device_status) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"targetDeviceStatus\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterDeviceOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
