...
pub const RenewDomainInput = struct {
    ...
    /// number of years
    /// depends on the top-level domain. For the range of valid values for your
    /// domain, see
...
pub const RenewDomainOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RenewDomainInput, options: CallOptions) !RenewDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RenewDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Domains_v20140515.RenewDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RenewDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RenewDomainOutput, body, allocator);
}
