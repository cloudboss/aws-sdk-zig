...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateDelegationSignerFromDomainInput, options: Options) !DisassociateDelegationSignerFromDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateDelegationSignerFromDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Domains_v20140515.DisassociateDelegationSignerFromDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateDelegationSignerFromDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisassociateDelegationSignerFromDomainOutput, body, alloc);
}
