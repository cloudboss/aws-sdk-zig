...
pub const CancelDomainTransferToAnotherAwsAccountOutput = struct {
    /// The identifier that `TransferDomainToAnotherAwsAccount` returned to track
    /// the progress of the request. Because the transfer request was canceled, the
    /// value is no
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelDomainTransferToAnotherAwsAccountInput, options: Options) !CancelDomainTransferToAnotherAwsAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelDomainTransferToAnotherAwsAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Domains_v20140515.CancelDomainTransferToAnotherAwsAccount");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelDomainTransferToAnotherAwsAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelDomainTransferToAnotherAwsAccountOutput, body, alloc);
}
