...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelDomainTransferToAnotherAwsAccountInput, options: CallOptions) !CancelDomainTransferToAnotherAwsAccountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelDomainTransferToAnotherAwsAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Domains_v20140515.CancelDomainTransferToAnotherAwsAccount");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelDomainTransferToAnotherAwsAccountOutput {
    _ = status;
    _ = headers;
