...
fn serializeRequest(alloc: std.mem.Allocator, input: ResendContactReachabilityEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Domains_v20140515.ResendContactReachabilityEmail");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResendContactReachabilityEmailOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResendContactReachabilityEmailOutput, body, alloc);
}
