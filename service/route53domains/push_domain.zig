...
fn serializeRequest(alloc: std.mem.Allocator, input: PushDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Domains_v20140515.PushDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PushDomainOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
