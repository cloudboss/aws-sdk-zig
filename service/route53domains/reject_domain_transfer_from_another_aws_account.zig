...
pub const RejectDomainTransferFromAnotherAwsAccountInput = struct {
    /// The name of the domain that was specified when another Amazon Web Services
    /// account
    /// submitted a
    /// [TransferDomainToAnotherAwsAccount](https://docs.aws.amazon.com/Route53/latest/APIReference/API_domains_TransferDomainToAnotherAwsAccount.html) request.
...
pub const RejectDomainTransferFromAnotherAwsAccountOutput = struct {
    /// The identifier that `TransferDomainToAnotherAwsAccount` returned to track
    /// the progress of the request. Because the transfer request was rejected, the
    /// value is no
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectDomainTransferFromAnotherAwsAccountInput, options: CallOptions) !RejectDomainTransferFromAnotherAwsAccountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RejectDomainTransferFromAnotherAwsAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Domains_v20140515.RejectDomainTransferFromAnotherAwsAccount");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RejectDomainTransferFromAnotherAwsAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RejectDomainTransferFromAnotherAwsAccountOutput, body, allocator);
}
