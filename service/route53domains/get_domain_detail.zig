...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDomainDetailInput, options: CallOptions) !GetDomainDetailOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDomainDetailInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Domains_v20140515.GetDomainDetail");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDomainDetailOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDomainDetailOutput, body, allocator);
}
