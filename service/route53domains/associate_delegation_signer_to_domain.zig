...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateDelegationSignerToDomainInput, options: CallOptions) !AssociateDelegationSignerToDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateDelegationSignerToDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Domains_v20140515.AssociateDelegationSignerToDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateDelegationSignerToDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssociateDelegationSignerToDomainOutput, body, allocator);
}
