...
pub const DisableDomainAutoRenewInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableDomainAutoRenewInput, options: Options) !DisableDomainAutoRenewOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableDomainAutoRenewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Domains_v20140515.DisableDomainAutoRenew");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableDomainAutoRenewOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
