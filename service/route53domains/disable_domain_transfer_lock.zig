...
pub const DisableDomainTransferLockInput = struct {
...
pub const DisableDomainTransferLockOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableDomainTransferLockInput, options: CallOptions) !DisableDomainTransferLockOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisableDomainTransferLockInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Domains_v20140515.DisableDomainTransferLock");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisableDomainTransferLockOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisableDomainTransferLockOutput, body, allocator);
}
