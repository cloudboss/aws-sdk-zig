...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableDomainTransferLockInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Domains_v20140515.DisableDomainTransferLock");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableDomainTransferLockOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DisableDomainTransferLockOutput, body, alloc);
}
