...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterDomainInput, options: Options) !RegisterDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Route53Domains_v20140515.RegisterDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterDomainOutput, body, alloc);
}
