...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTagsForDomainInput, options: CallOptions) !ListTagsForDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTagsForDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Domains_v20140515.ListTagsForDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTagsForDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTagsForDomainOutput, body, allocator);
}
