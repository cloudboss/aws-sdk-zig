...
pub const EnableDomainAutoRenewInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableDomainAutoRenewInput, options: Options) !EnableDomainAutoRenewOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableDomainAutoRenewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Route53Domains_v20140515.EnableDomainAutoRenew");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableDomainAutoRenewOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
