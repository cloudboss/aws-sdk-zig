...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRateBasedStatementManagedKeysInput, options: Options) !GetRateBasedStatementManagedKeysOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRateBasedStatementManagedKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20190729.GetRateBasedStatementManagedKeys");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRateBasedStatementManagedKeysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRateBasedStatementManagedKeysOutput, body, alloc);
}
