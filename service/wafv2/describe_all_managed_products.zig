...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAllManagedProductsInput, options: CallOptions) !DescribeAllManagedProductsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAllManagedProductsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20190729.DescribeAllManagedProducts");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAllManagedProductsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAllManagedProductsOutput, body, allocator);
}
