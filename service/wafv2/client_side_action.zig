...
const UsageOfAction = @import("usage_of_action.zig").UsageOfAction;

/// This is part of the `AWSManagedRulesAntiDDoSRuleSet`
/// `ClientSideActionConfig` configuration in `ManagedRuleGroupConfig`.
pub const ClientSideAction = struct {
    /// The regular expression to match against the web request URI, used to
    /// identify requests
    /// that can't handle a silent browser challenge. When the `ClientSideAction`
    /// setting `UsageOfAction` is enabled,
    ...
    /// has no effect and the managed rule group doesn't add the label to any
    /// requests.
    ///
    /// The anti-DDoS managed rule group doesn't
    /// evaluate the rules `ChallengeDDoSRequests` or `ChallengeAllDuringEvent` for
    /// web requests whose URIs match this regex. This
    /// is true regardless of whether you override the rule action for either of the
    /// rules in your web ACL configuration.
    ///
    ...
    /// evaluation and the related label
    /// `awswaf:managed:aws:anti-ddos:challengeable-request`.
    ///
    /// * If usage is enabled:
    ///
    /// * The managed rule group adds the label
    ///   `awswaf:managed:aws:anti-ddos:challengeable-request` to any web request
    ///   whose URL does *NOT* match the regular expressions provided in the
    ...
    ///   that are experiencing a DDoS attack.
    /// The two rules only apply their action to matching requests that have the
    /// label `awswaf:managed:aws:anti-ddos:challengeable-request`.
    ///
    ...
    /// `ChallengeAllDuringEvent` and `ChallengeDDoSRequests` in the anti-DDoS
    /// managed rule group.
    ///
