...
const ManagedRuleGroupSummary = @import("managed_rule_group_summary.zig").ManagedRuleGroupSummary;

pub const ListAvailableManagedRuleGroupsInput = struct {
...
pub const ListAvailableManagedRuleGroupsOutput = struct {
    ...
    pub const json_field_names = .{
        .managed_rule_groups = "ManagedRuleGroups",
        .next_marker = "NextMarker",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAvailableManagedRuleGroupsInput, options: CallOptions) !ListAvailableManagedRuleGroupsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "wafv2");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAvailableManagedRuleGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20190729.ListAvailableManagedRuleGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAvailableManagedRuleGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAvailableManagedRuleGroupsOutput, body, allocator);
}
