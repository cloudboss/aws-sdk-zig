const RateBasedStatementAggregateKeyType = @import("rate_based_statement_aggregate_key_type.zig").RateBasedStatementAggregateKeyType;
const RateBasedStatementCustomKey = @import("rate_based_statement_custom_key.zig").RateBasedStatementCustomKey;
...
pub const RateBasedStatement = struct {
    ...
    ///   scope-down
    /// statement. With this option, the counted requests aren't further aggregated.
    /// The scope-down statement
    /// is the only specification used. When the count of all requests that satisfy
    /// the scope-down statement
    /// goes over the limit, WAF applies the rule action to all requests that
    ...
    /// use custom keys. Instead, set the aggregate
    ...
    /// For example,
    /// for a setting of 120, when WAF checks the rate, it counts the requests for
    /// the 2 minutes immediately preceding
    ...
    /// If the rate-based statement includes a `ScopeDownStatement`, this limit is
    /// applied only to the
