...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateMobileSdkReleaseUrlInput, options: Options) !GenerateMobileSdkReleaseUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateMobileSdkReleaseUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20190729.GenerateMobileSdkReleaseUrl");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateMobileSdkReleaseUrlOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GenerateMobileSdkReleaseUrlOutput, body, alloc);
}
