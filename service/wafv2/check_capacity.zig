...
pub const CheckCapacityInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CheckCapacityInput, options: CallOptions) !CheckCapacityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CheckCapacityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20190729.CheckCapacity");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CheckCapacityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CheckCapacityOutput, body, allocator);
}
