...
const APIKeySummary = @import("api_key_summary.zig").APIKeySummary;

pub const ListAPIKeysInput = struct {
...
pub const ListAPIKeysOutput = struct {
    ...
    pub const json_field_names = .{
        .api_key_summaries = "APIKeySummaries",
        .application_integration_url = "ApplicationIntegrationURL",
        .next_marker = "NextMarker",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAPIKeysInput, options: CallOptions) !ListAPIKeysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAPIKeysInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20190729.ListAPIKeys");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAPIKeysOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAPIKeysOutput, body, allocator);
}
