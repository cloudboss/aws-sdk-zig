...
const MobileSdkRelease = @import("mobile_sdk_release.zig").MobileSdkRelease;

pub const GetMobileSdkReleaseInput = struct {
    /// The device platform.
    platform: Platform,
...
pub const GetMobileSdkReleaseOutput = struct {
    /// Information for a specified SDK release, including release notes and tags.
    mobile_sdk_release: ?MobileSdkRelease = null,

    pub const json_field_names = .{
        .mobile_sdk_release = "MobileSdkRelease",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMobileSdkReleaseInput, options: CallOptions) !GetMobileSdkReleaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMobileSdkReleaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20190729.GetMobileSdkRelease");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMobileSdkReleaseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMobileSdkReleaseOutput, body, allocator);
}
