...
const VersionToPublish = @import("version_to_publish.zig").VersionToPublish;

pub const PutManagedRuleSetVersionsInput = struct {
    ...
    /// customers.
    versions_to_publish: ?[]const aws.map.MapEntry(VersionToPublish) = null,
...
pub const PutManagedRuleSetVersionsOutput = struct {
    /// A token used for optimistic locking. WAF returns a token to your `get` and
    /// `list` requests, to mark the state of the entity at the time of the request.
    /// To make changes to the entity associated with the token, you provide the
    /// token to operations like `update` and `delete`. WAF uses the token to ensure
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutManagedRuleSetVersionsInput, options: CallOptions) !PutManagedRuleSetVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutManagedRuleSetVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20190729.PutManagedRuleSetVersions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutManagedRuleSetVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutManagedRuleSetVersionsOutput, body, allocator);
}
