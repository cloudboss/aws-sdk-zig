...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteFirewallManagerRuleGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20190729.DeleteFirewallManagerRuleGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteFirewallManagerRuleGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteFirewallManagerRuleGroupsOutput, body, alloc);
}
