...
const ForwardedIPPosition = @import("forwarded_ip_position.zig").ForwardedIPPosition;
...
pub const IPSetForwardedIPConfig = struct {
    ...
    /// contain IP
    /// addresses of the original client and also of proxies. For example, the
    /// header value could
    /// be `10.1.1.1, 127.0.0.0, 10.10.10.10` where the first IP address identifies
    /// the
