...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedRuleSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_20190729.ListManagedRuleSets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedRuleSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListManagedRuleSetsOutput, body, alloc);
}
