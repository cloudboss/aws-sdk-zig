...
const ManagedRuleSetSummary = @import("managed_rule_set_summary.zig").ManagedRuleSetSummary;

pub const ListManagedRuleSetsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedRuleSetsInput, options: CallOptions) !ListManagedRuleSetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListManagedRuleSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_20190729.ListManagedRuleSets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListManagedRuleSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListManagedRuleSetsOutput, body, allocator);
}
