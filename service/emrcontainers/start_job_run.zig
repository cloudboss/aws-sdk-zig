...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartJobRunInput, options: Options) !StartJobRunOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartJobRunInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.job_driver) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"jobDriver\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.job_template_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"jobTemplateId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.job_template_parameters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"jobTemplateParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
    ...
    if (input.retry_policy_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"retryPolicyConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartJobRunOutput {
    var result: StartJobRunOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartJobRunOutput, body, alloc);
    }
    _ = status;
    _ = headers;
