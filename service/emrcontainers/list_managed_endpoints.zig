...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedEndpointsOutput {
    var result: ListManagedEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
