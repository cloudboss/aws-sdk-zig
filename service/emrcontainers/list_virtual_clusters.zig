...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVirtualClustersInput, options: Options) !ListVirtualClustersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVirtualClustersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.container_provider_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "containerProviderId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.container_provider_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "containerProviderType=");
    ...
    if (input.eks_access_entry_integrated) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "eksAccessEntryIntegrated=");
    ...
    if (input.states) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "states=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVirtualClustersOutput {
    var result: ListVirtualClustersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVirtualClustersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
