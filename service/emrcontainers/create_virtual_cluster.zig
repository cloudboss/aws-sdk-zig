...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVirtualClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.container_provider), input.container_provider, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.security_configuration_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"securityConfigurationId\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVirtualClusterOutput {
    var result: CreateVirtualClusterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVirtualClusterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
