...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetManagedEndpointSessionCredentialsInput, options: Options) !GetManagedEndpointSessionCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetManagedEndpointSessionCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.virtual_cluster_identifier);
    try path_buf.appendSlice(alloc, "/endpoints/");
    try path_buf.appendSlice(alloc, input.endpoint_identifier);
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"credentialType\":");
    try aws.json.writeValue(@TypeOf(input.credential_type), input.credential_type, alloc, &body_buf);
    has_prev = true;
    if (input.duration_in_seconds) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"durationInSeconds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.log_context) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"logContext\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetManagedEndpointSessionCredentialsOutput {
    var result: GetManagedEndpointSessionCredentialsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetManagedEndpointSessionCredentialsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
