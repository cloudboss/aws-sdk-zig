...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeManagedEndpointInput, options: Options) !DescribeManagedEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeManagedEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeManagedEndpointOutput {
    var result: DescribeManagedEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeManagedEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
