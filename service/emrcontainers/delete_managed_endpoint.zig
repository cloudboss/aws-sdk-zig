...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteManagedEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteManagedEndpointOutput {
    var result: DeleteManagedEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteManagedEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
