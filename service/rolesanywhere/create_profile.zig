...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (input.require_instance_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"requireInstanceProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"roleArns\":");
    try aws.json.writeValue(@TypeOf(input.role_arns), input.role_arns, alloc, &body_buf);
    has_prev = true;
