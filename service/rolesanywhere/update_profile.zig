...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProfileInput, options: Options) !UpdateProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.accept_role_session_name) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"acceptRoleSessionName\":");
    ...
    if (input.managed_policy_arns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"managedPolicyArns\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
    ...
    if (input.role_arns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"roleArns\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.session_policy) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sessionPolicy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProfileOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
