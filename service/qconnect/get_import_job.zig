...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetImportJobInput, options: Options) !GetImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImportJobInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImportJobOutput {
    var result: GetImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
