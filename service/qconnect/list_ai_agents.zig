...
pub const ListAIAgentsInput = struct {
...
pub const ListAIAgentsOutput = struct {
    /// The summaries of AI Agents.
    ai_agent_summaries: ?[]const AIAgentSummary = null,
    ...
    pub const json_field_names = .{
        .ai_agent_summaries = "aiAgentSummaries",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAIAgentsInput, options: CallOptions) !ListAIAgentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAIAgentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAIAgentsOutput {
    var result: ListAIAgentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAIAgentsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
