...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAIAgentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAIAgentsOutput {
    var result: ListAIAgentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAIAgentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
