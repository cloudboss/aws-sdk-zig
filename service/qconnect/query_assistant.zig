...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: QueryAssistantInput, options: Options) !QueryAssistantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: QueryAssistantInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/query");
    ...
    if (input.override_knowledge_base_search_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"overrideKnowledgeBaseSearchType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.query_condition) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"queryCondition\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.query_input_data) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"queryInputData\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !QueryAssistantOutput {
    var result: QueryAssistantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(QueryAssistantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
