...
pub const CreateAIAgentInput = struct {
...
pub const CreateAIAgentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAIAgentInput, options: Options) !CreateAIAgentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAIAgentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAIAgentOutput {
    var result: CreateAIAgentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAIAgentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
