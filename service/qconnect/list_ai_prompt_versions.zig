...
const AIPromptVersionSummary = @import("ai_prompt_version_summary.zig").AIPromptVersionSummary;

pub const ListAIPromptVersionsInput = struct {
...
pub const ListAIPromptVersionsOutput = struct {
    /// The summaries of the AI Prompt versions.
    ai_prompt_version_summaries: ?[]const AIPromptVersionSummary = null,
    ...
    pub const json_field_names = .{
        .ai_prompt_version_summaries = "aiPromptVersionSummaries",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAIPromptVersionsInput, options: Options) !ListAIPromptVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAIPromptVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAIPromptVersionsOutput {
    var result: ListAIPromptVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAIPromptVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
