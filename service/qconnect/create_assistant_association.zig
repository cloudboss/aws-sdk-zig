...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssistantAssociationInput, options: CallOptions) !CreateAssistantAssociationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAssistantAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"association\":");
    try aws.json.writeValue(@TypeOf(input.association), input.association, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAssistantAssociationOutput {
    var result: CreateAssistantAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssistantAssociationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
