...
pub const ListAIGuardrailsInput = struct {
...
pub const ListAIGuardrailsOutput = struct {
    /// The summaries of the AI Guardrails.
    ai_guardrail_summaries: ?[]const AIGuardrailSummary = null,
    ...
    pub const json_field_names = .{
        .ai_guardrail_summaries = "aiGuardrailSummaries",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAIGuardrailsInput, options: CallOptions) !ListAIGuardrailsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAIGuardrailsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAIGuardrailsOutput {
    var result: ListAIGuardrailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAIGuardrailsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
