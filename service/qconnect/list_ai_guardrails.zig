...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAIGuardrailsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAIGuardrailsOutput {
    var result: ListAIGuardrailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAIGuardrailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
