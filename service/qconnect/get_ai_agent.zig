...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAIAgentInput, options: Options) !GetAIAgentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAIAgentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAIAgentOutput {
    var result: GetAIAgentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAIAgentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
