...
pub const DeleteAIPromptVersionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAIPromptVersionInput, options: Options) !DeleteAIPromptVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAIPromptVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.ai_prompt_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAIPromptVersionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteAIPromptVersionOutput = .{};
