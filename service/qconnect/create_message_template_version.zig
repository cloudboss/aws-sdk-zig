...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMessageTemplateVersionInput, options: Options) !CreateMessageTemplateVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMessageTemplateVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.message_template_content_sha_256) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"messageTemplateContentSha256\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMessageTemplateVersionOutput {
    var result: CreateMessageTemplateVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMessageTemplateVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
