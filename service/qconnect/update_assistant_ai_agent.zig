...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAssistantAIAgentInput, options: Options) !UpdateAssistantAIAgentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssistantAIAgentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"aiAgentType\":");
    try aws.json.writeValue(@TypeOf(input.ai_agent_type), input.ai_agent_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssistantAIAgentOutput {
    var result: UpdateAssistantAIAgentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAssistantAIAgentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
