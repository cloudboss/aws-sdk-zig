...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContentAssociationInput, options: Options) !GetContentAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContentAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContentAssociationOutput {
    var result: GetContentAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContentAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
