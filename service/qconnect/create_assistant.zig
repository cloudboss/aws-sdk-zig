...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssistantInput, options: Options) !CreateAssistantOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssistantInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssistantOutput {
    var result: CreateAssistantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssistantOutput, body, alloc);
    }
    _ = status;
    _ = headers;
