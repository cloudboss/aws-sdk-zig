...
pub const DeleteImportJobInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteImportJobInput, options: Options) !DeleteImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.import_job_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteImportJobOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteImportJobOutput = .{};
