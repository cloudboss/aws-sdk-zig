...
pub const CreateMessageTemplateAttachmentInput = struct {
...
pub const CreateMessageTemplateAttachmentOutput = struct {
    /// The message template attachment.
    attachment: ?MessageTemplateAttachment = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMessageTemplateAttachmentInput, options: CallOptions) !CreateMessageTemplateAttachmentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateMessageTemplateAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"contentDisposition\":");
    try aws.json.writeValue(@TypeOf(input.content_disposition), input.content_disposition, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
