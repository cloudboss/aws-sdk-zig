...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMessageTemplateAttachmentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"contentDisposition\":");
    try aws.json.writeValue(@TypeOf(input.content_disposition), input.content_disposition, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMessageTemplateAttachmentOutput {
    var result: CreateMessageTemplateAttachmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMessageTemplateAttachmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
