...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAIPromptVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAIPromptVersionOutput {
    var result: CreateAIPromptVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAIPromptVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
