...
const CreateSessionOutput = @import("create_session_response.zig").CreateSessionResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ai_agent_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"aiAgentConfiguration\":");
    ...
    if (input.orchestrator_configuration_list) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"orchestratorConfigurationList\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.remove_orchestrator_configuration_list) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"removeOrchestratorConfigurationList\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tag_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"tagFilter\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
