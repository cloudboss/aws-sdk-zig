...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAIGuardrailVersionInput, options: Options) !CreateAIGuardrailVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAIGuardrailVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.modified_time) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"modifiedTime\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAIGuardrailVersionOutput {
    var result: CreateAIGuardrailVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAIGuardrailVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
