...
const AIGuardrailVersionSummary = @import("ai_guardrail_version_summary.zig").AIGuardrailVersionSummary;

pub const ListAIGuardrailVersionsInput = struct {
...
pub const ListAIGuardrailVersionsOutput = struct {
    /// The summaries of the AI Guardrail versions.
    ai_guardrail_version_summaries: ?[]const AIGuardrailVersionSummary = null,
    ...
    pub const json_field_names = .{
        .ai_guardrail_version_summaries = "aiGuardrailVersionSummaries",
        .next_token = "nextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAIGuardrailVersionsInput, options: CallOptions) !ListAIGuardrailVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAIGuardrailVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAIGuardrailVersionsOutput {
    var result: ListAIGuardrailVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAIGuardrailVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
