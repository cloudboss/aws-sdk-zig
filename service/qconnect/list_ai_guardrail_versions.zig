...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAIGuardrailVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAIGuardrailVersionsOutput {
    var result: ListAIGuardrailVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAIGuardrailVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
