...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateQuickResponseInput, options: CallOptions) !UpdateQuickResponseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateQuickResponseInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.remove_description) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"removeDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.remove_grouping_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"removeGroupingConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.remove_shortcut_key) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"removeShortcutKey\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
