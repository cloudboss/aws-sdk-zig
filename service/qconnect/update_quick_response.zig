...
pub const UpdateQuickResponseInput = struct {
    ...
    content: ?QuickResponseDataProvider = null,
    ...
    remove_description: ?bool = null,

    /// Whether to remove the grouping configuration of the quick response.
    remove_grouping_configuration: ?bool = null,

    /// Whether to remove the shortcut key of the quick response.
    remove_shortcut_key: ?bool = null,
    ...
    pub const json_field_names = .{
        ...
        .remove_grouping_configuration = "removeGroupingConfiguration",
        .remove_shortcut_key = "removeShortcutKey",
        .shortcut_key = "shortcutKey",
    };
};

pub const UpdateQuickResponseOutput = struct {
    /// The quick response.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateQuickResponseInput, options: Options) !UpdateQuickResponseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQuickResponseInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/quickResponses/");
    try path_buf.appendSlice(alloc, input.quick_response_id);
    ...
    if (input.channels) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"channels\":");
    ...
    if (input.content_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"contentType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.grouping_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"groupingConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.is_active) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"isActive\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.language) |v| {
    ...
    if (input.remove_description) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"removeDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.remove_grouping_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"removeGroupingConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.remove_shortcut_key) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"removeShortcutKey\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.shortcut_key) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"shortcutKey\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQuickResponseOutput {
    var result: UpdateQuickResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQuickResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
