...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartImportJobInput, options: Options) !StartImportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.external_source_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"externalSourceConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"importJobType\":");
    try aws.json.writeValue(@TypeOf(input.import_job_type), input.import_job_type, alloc, &body_buf);
    has_prev = true;
    if (input.metadata) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartImportJobOutput {
    var result: StartImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartImportJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
