...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveAssistantAIAgentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.orchestrator_use_case) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "orchestratorUseCase=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveAssistantAIAgentOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
