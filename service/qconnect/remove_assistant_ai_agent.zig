...
pub const RemoveAssistantAIAgentInput = struct {
    /// The type of the AI Agent being removed for use by default from the Amazon Q
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveAssistantAIAgentInput, options: Options) !RemoveAssistantAIAgentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveAssistantAIAgentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/aiagentConfiguration");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "aiAgentType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.ai_agent_type));
    query_has_prev = true;
    if (input.orchestrator_use_case) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "orchestratorUseCase=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveAssistantAIAgentOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: RemoveAssistantAIAgentOutput = .{};
