...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAIAgentVersionInput, options: Options) !CreateAIAgentVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAIAgentVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAIAgentVersionOutput {
    var result: CreateAIAgentVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAIAgentVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
