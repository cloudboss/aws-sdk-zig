...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAIGuardrailInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAIGuardrailOutput {
    var result: UpdateAIGuardrailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAIGuardrailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
