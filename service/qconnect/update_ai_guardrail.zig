...
pub const UpdateAIGuardrailInput = struct {
...
pub const UpdateAIGuardrailOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAIGuardrailInput, options: CallOptions) !UpdateAIGuardrailOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAIGuardrailInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAIGuardrailOutput {
    var result: UpdateAIGuardrailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAIGuardrailOutput, body, allocator);
    }
    _ = status;
    _ = headers;
