...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAIPromptInput, options: Options) !GetAIPromptOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAIPromptInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAIPromptOutput {
    var result: GetAIPromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAIPromptOutput, body, alloc);
    }
    _ = status;
    _ = headers;
