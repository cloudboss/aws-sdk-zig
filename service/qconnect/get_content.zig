...
pub const GetContentInput = struct {
...
pub const GetContentOutput = struct {
    /// The content.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContentInput, options: Options) !GetContentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.content_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContentOutput {
    var result: GetContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
