...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContentInput, options: CallOptions) !GetContentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetContentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetContentOutput {
    var result: GetContentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
