...
pub const UpdateAIPromptInput = struct {
    ...
    template_configuration: ?AIPromptTemplateConfiguration = null,
...
pub const UpdateAIPromptOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAIPromptInput, options: CallOptions) !UpdateAIPromptOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAIPromptInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAIPromptOutput {
    var result: UpdateAIPromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAIPromptOutput, body, allocator);
    }
    _ = status;
    _ = headers;
