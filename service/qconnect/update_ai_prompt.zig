...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAIPromptInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAIPromptOutput {
    var result: UpdateAIPromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAIPromptOutput, body, alloc);
    }
    _ = status;
    _ = headers;
