...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSessionDataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSessionDataOutput {
    var result: UpdateSessionDataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSessionDataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
