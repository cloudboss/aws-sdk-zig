...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RenderMessageTemplateInput, options: Options) !RenderMessageTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RenderMessageTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.attributes), input.attributes, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RenderMessageTemplateOutput {
    var result: RenderMessageTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RenderMessageTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
