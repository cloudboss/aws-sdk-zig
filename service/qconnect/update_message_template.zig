...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMessageTemplateInput, options: Options) !UpdateMessageTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMessageTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.default_attributes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"defaultAttributes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.language) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMessageTemplateOutput {
    var result: UpdateMessageTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMessageTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
