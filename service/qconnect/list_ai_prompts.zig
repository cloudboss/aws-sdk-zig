...
pub const ListAIPromptsInput = struct {
...
pub const ListAIPromptsOutput = struct {
    /// The summaries of the AI Prompts.
    ai_prompt_summaries: ?[]const AIPromptSummary = null,
    ...
    pub const json_field_names = .{
        .ai_prompt_summaries = "aiPromptSummaries",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAIPromptsInput, options: Options) !ListAIPromptsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAIPromptsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAIPromptsOutput {
    var result: ListAIPromptsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAIPromptsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
