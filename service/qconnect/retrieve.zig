...
fn serializeRequest(alloc: std.mem.Allocator, input: RetrieveInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.retrieval_configuration), input.retrieval_configuration, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"retrievalQuery\":");
    try aws.json.writeValue(@TypeOf(input.retrieval_query), input.retrieval_query, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RetrieveOutput {
    var result: RetrieveOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RetrieveOutput, body, alloc);
    }
    _ = status;
    _ = headers;
