...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKnowledgeBaseTemplateUriInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"templateUri\":");
    try aws.json.writeValue(@TypeOf(input.template_uri), input.template_uri, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKnowledgeBaseTemplateUriOutput {
    var result: UpdateKnowledgeBaseTemplateUriOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKnowledgeBaseTemplateUriOutput, body, alloc);
    }
    _ = status;
    _ = headers;
