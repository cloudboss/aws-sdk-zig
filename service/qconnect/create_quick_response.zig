...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateQuickResponseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateQuickResponseOutput {
    var result: CreateQuickResponseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateQuickResponseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
