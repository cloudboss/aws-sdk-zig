...
pub const GetNextMessageOutput = struct {
    ...
    pub const json_field_names = .{
        ...
        .conversation_session_data = "conversationSessionData",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNextMessageInput, options: Options) !GetNextMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNextMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/messages/next");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "nextMessageToken=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.next_message_token);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNextMessageOutput {
    var result: GetNextMessageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetNextMessageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
