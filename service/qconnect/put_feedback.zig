...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutFeedbackInput, options: Options) !PutFeedbackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutFeedbackInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"contentFeedback\":");
    try aws.json.writeValue(@TypeOf(input.content_feedback), input.content_feedback, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"targetId\":");
    try aws.json.writeValue(@TypeOf(input.target_id), input.target_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"targetType\":");
    try aws.json.writeValue(@TypeOf(input.target_type), input.target_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutFeedbackOutput {
    var result: PutFeedbackOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutFeedbackOutput, body, alloc);
    }
    _ = status;
    _ = headers;
