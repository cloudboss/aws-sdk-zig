...
pub const DeleteAIGuardrailInput = struct {
    /// The identifier of the Amazon Q in Connect AI Guardrail. Can be either the ID
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAIGuardrailInput, options: Options) !DeleteAIGuardrailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAIGuardrailInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/aiguardrails/");
    try path_buf.appendSlice(alloc, input.ai_guardrail_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAIGuardrailOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteAIGuardrailOutput = .{};
