...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAIGuardrailInput, options: Options) !CreateAIGuardrailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAIGuardrailInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAIGuardrailOutput {
    var result: CreateAIGuardrailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAIGuardrailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
