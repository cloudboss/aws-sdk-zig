...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateContentAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateContentAssociationOutput {
    var result: CreateContentAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateContentAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
