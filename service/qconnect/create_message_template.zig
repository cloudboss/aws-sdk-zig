...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMessageTemplateInput, options: Options) !CreateMessageTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMessageTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"channelSubtype\":");
    try aws.json.writeValue(@TypeOf(input.channel_subtype), input.channel_subtype, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMessageTemplateOutput {
    var result: CreateMessageTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMessageTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
