...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAIPromptInput, options: Options) !CreateAIPromptOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAIPromptInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/aiprompts");
    ...
    try body_buf.appendSlice(alloc, "\"apiFormat\":");
    try aws.json.writeValue(@TypeOf(input.api_format), input.api_format, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.inference_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"inferenceConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"templateConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.template_configuration), input.template_configuration, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"templateType\":");
    try aws.json.writeValue(@TypeOf(input.template_type), input.template_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.visibility_status), input.visibility_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAIPromptOutput {
    var result: CreateAIPromptOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAIPromptOutput, body, alloc);
    }
    _ = status;
    _ = headers;
