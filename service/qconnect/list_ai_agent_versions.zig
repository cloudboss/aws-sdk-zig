...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAIAgentVersionsInput, options: Options) !ListAIAgentVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAIAgentVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.origin) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "origin=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAIAgentVersionsOutput {
    var result: ListAIAgentVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAIAgentVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
