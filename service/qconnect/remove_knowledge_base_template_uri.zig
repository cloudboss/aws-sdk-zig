...
pub const RemoveKnowledgeBaseTemplateUriInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveKnowledgeBaseTemplateUriInput, options: Options) !RemoveKnowledgeBaseTemplateUriOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveKnowledgeBaseTemplateUriInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/templateUri");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveKnowledgeBaseTemplateUriOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: RemoveKnowledgeBaseTemplateUriOutput = .{};
