...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeactivateMessageTemplateInput, options: Options) !DeactivateMessageTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeactivateMessageTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/deactivate");
    ...
    try body_buf.appendSlice(alloc, "\"versionNumber\":");
    try aws.json.writeValue(@TypeOf(input.version_number), input.version_number, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeactivateMessageTemplateOutput {
    var result: DeactivateMessageTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeactivateMessageTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
