...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAIGuardrailInput, options: Options) !GetAIGuardrailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAIGuardrailInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAIGuardrailOutput {
    var result: GetAIGuardrailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAIGuardrailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
