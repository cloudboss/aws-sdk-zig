...
pub const UpdateAIAgentInput = struct {
...
pub const UpdateAIAgentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAIAgentInput, options: Options) !UpdateAIAgentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAIAgentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/aiagents/");
    try path_buf.appendSlice(alloc, input.ai_agent_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAIAgentOutput {
    var result: UpdateAIAgentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAIAgentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
