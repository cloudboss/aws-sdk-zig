...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ActivateMessageTemplateInput, options: Options) !ActivateMessageTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ActivateMessageTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ActivateMessageTemplateOutput {
    var result: ActivateMessageTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ActivateMessageTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
