...
const QuickResponseSummary = @import("quick_response_summary.zig").QuickResponseSummary;

pub const ListQuickResponsesInput = struct {
...
pub const ListQuickResponsesOutput = struct {
    /// The token for the next set of results. Use the value returned in the
    /// previous response in the next request to retrieve the next set of results.
    next_token: ?[]const u8 = null,

    /// Summary information about the quick responses.
    quick_response_summaries: ?[]const QuickResponseSummary = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .quick_response_summaries = "quickResponseSummaries",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQuickResponsesInput, options: Options) !ListQuickResponsesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQuickResponsesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQuickResponsesOutput {
    var result: ListQuickResponsesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListQuickResponsesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
