...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMessageTemplateInput, options: Options) !GetMessageTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMessageTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMessageTemplateOutput {
    var result: GetMessageTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMessageTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
