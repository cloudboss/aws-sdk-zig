...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSpansInput, options: Options) !ListSpansOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSpansInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/spans");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSpansOutput {
    var result: ListSpansOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSpansOutput, body, alloc);
    }
    _ = status;
    _ = headers;
