...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"EndTimestamp\":");
    try aws.json.writeValue(@TypeOf(input.end_timestamp), input.end_timestamp, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.format_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FormatConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.height_pixels) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"HeightPixels\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ImageSelectorType\":");
    try aws.json.writeValue(@TypeOf(input.image_selector_type), input.image_selector_type, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.sampling_interval) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SamplingInterval\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"StartTimestamp\":");
    try aws.json.writeValue(@TypeOf(input.start_timestamp), input.start_timestamp, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.width_pixels) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"WidthPixels\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImagesOutput {
    var result: GetImagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
