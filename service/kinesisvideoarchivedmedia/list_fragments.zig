...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFragmentsInput, options: Options) !ListFragmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFragmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.fragment_selector) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"FragmentSelector\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFragmentsOutput {
    var result: ListFragmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFragmentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
