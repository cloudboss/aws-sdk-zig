...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDASHStreamingSessionURLInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dash_fragment_selector) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DASHFragmentSelector\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.display_fragment_number) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DisplayFragmentNumber\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.max_manifest_fragment_results) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MaxManifestFragmentResults\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDASHStreamingSessionURLOutput {
    var result: GetDASHStreamingSessionURLOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDASHStreamingSessionURLOutput, body, alloc);
    }
    _ = status;
    _ = headers;
