...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceShareAssociationsInput, options: Options) !GetResourceShareAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceShareAssociationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceShareAssociationsOutput {
    var result: GetResourceShareAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceShareAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
