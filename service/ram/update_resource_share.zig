...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResourceShareInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResourceShareOutput {
    var result: UpdateResourceShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResourceShareOutput, body, alloc);
    }
    _ = status;
    _ = headers;
