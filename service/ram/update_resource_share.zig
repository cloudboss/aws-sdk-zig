...
pub const UpdateResourceShareInput = struct {
    ...
    /// (ARN)](https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) of the resource share that you want to modify.
...
pub const UpdateResourceShareOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResourceShareInput, options: CallOptions) !UpdateResourceShareOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateResourceShareInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/updateresourceshare";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateResourceShareOutput {
    var result: UpdateResourceShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResourceShareOutput, body, allocator);
    }
    _ = status;
    _ = headers;
