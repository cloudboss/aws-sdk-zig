...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceShareInvitationsInput, options: Options) !GetResourceShareInvitationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceShareInvitationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.resource_share_arns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resourceShareArns\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.resource_share_invitation_arns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resourceShareInvitationArns\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceShareInvitationsOutput {
    var result: GetResourceShareInvitationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceShareInvitationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
