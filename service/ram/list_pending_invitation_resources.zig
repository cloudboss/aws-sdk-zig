...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPendingInvitationResourcesInput, options: CallOptions) !ListPendingInvitationResourcesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPendingInvitationResourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPendingInvitationResourcesOutput {
    var result: ListPendingInvitationResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPendingInvitationResourcesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
