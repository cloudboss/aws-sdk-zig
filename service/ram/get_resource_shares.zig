...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceSharesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.resource_share_status) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resourceShareStatus\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tag_filters) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"tagFilters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceSharesOutput {
    var result: GetResourceSharesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceSharesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
