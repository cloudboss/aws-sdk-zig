...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPermissionInput, options: Options) !GetPermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPermissionOutput {
    var result: GetPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPermissionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
