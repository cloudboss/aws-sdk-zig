...
pub const DeleteResourceShareInput = struct {
    ...
    /// (ARN)](https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) of the resource share to delete.
...
pub const DeleteResourceShareOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResourceShareInput, options: Options) !DeleteResourceShareOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteResourceShareInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "resourceShareArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.resource_share_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteResourceShareOutput {
    var result: DeleteResourceShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteResourceShareOutput, body, alloc);
    }
    _ = status;
    _ = headers;
