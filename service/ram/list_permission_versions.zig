...
pub const ListPermissionVersionsInput = struct {
    ...
    /// (ARN)](https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) of the RAM permission whose versions you want to list. You
    /// can use the `permissionVersion` parameter on the
    /// AssociateResourceSharePermission operation to specify a non-default
    /// version to attach.
...
pub const ListPermissionVersionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPermissionVersionsInput, options: CallOptions) !ListPermissionVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPermissionVersionsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("ram", "RAM", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/listpermissionversions";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPermissionVersionsOutput {
    var result: ListPermissionVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPermissionVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
