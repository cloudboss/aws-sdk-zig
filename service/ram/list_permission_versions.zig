...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPermissionVersionsInput, options: Options) !ListPermissionVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPermissionVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPermissionVersionsOutput {
    var result: ListPermissionVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPermissionVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
