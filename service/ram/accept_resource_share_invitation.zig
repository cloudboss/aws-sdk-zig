...
pub const AcceptResourceShareInvitationInput = struct {
    ...
    /// (ARN)](https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) of the invitation that you want to accept.
...
pub const AcceptResourceShareInvitationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptResourceShareInvitationInput, options: Options) !AcceptResourceShareInvitationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptResourceShareInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/acceptresourceshareinvitation";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptResourceShareInvitationOutput {
    var result: AcceptResourceShareInvitationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AcceptResourceShareInvitationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
