...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReplacePermissionAssociationsInput, options: Options) !ReplacePermissionAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReplacePermissionAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"fromPermissionArn\":");
    try aws.json.writeValue(@TypeOf(input.from_permission_arn), input.from_permission_arn, alloc, &body_buf);
    has_prev = true;
    if (input.from_permission_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"fromPermissionVersion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"toPermissionArn\":");
    try aws.json.writeValue(@TypeOf(input.to_permission_arn), input.to_permission_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReplacePermissionAssociationsOutput {
    var result: ReplacePermissionAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ReplacePermissionAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
