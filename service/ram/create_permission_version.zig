...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePermissionVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"policyTemplate\":");
    try aws.json.writeValue(@TypeOf(input.policy_template), input.policy_template, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePermissionVersionOutput {
    var result: CreatePermissionVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePermissionVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
