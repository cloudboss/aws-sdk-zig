...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPrincipalsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPrincipalsOutput {
    var result: ListPrincipalsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrincipalsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
