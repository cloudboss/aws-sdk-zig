...
pub const SetDefaultPermissionVersionInput = struct {
    ...
    /// (ARN)](https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) of the customer managed permission whose default version you want to change.
    permission_arn: []const u8,

    /// Specifies the version number that you want to designate as the default for
    /// customer managed permission. To
    /// see a list of all available version numbers, use ListPermissionVersions.
...
pub const SetDefaultPermissionVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetDefaultPermissionVersionInput, options: CallOptions) !SetDefaultPermissionVersionOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "ram");
...
fn serializeRequest(allocator: std.mem.Allocator, input: SetDefaultPermissionVersionInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("ram", "RAM", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/setdefaultpermissionversion";
    ...
    try aws.json.writeValue(@TypeOf(input.permission_version), input.permission_version, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SetDefaultPermissionVersionOutput {
    var result: SetDefaultPermissionVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SetDefaultPermissionVersionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
