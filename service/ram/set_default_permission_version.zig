...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetDefaultPermissionVersionInput, options: Options) !SetDefaultPermissionVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetDefaultPermissionVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.permission_arn), input.permission_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"permissionVersion\":");
    try aws.json.writeValue(@TypeOf(input.permission_version), input.permission_version, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetDefaultPermissionVersionOutput {
    var result: SetDefaultPermissionVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SetDefaultPermissionVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
