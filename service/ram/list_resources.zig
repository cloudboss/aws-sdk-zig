...
const ListResourcesInput = @import("list_resources_request.zig").ListResourcesRequest;

const ListResourcesOutput = @import("list_resources_response.zig").ListResourcesResponse;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourcesOutput {
    var result: ListResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
