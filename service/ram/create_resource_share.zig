...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourceShareInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.allow_external_principals) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"allowExternalPrincipals\":");
    ...
    if (input.permission_arns) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"permissionArns\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourceShareOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateResourceShareOutput, body, alloc);
    }
    _ = status;
    _ = headers;
