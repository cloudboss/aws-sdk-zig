...
const PermissionTypeFilter = @import("permission_type_filter.zig").PermissionTypeFilter;
...
pub const ListPermissionsInput = struct {
    ...
    permission_type: ?PermissionTypeFilter = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.permission_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"permissionType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
