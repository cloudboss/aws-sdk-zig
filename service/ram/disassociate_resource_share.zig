...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateResourceShareInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateResourceShareOutput {
    var result: DisassociateResourceShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisassociateResourceShareOutput, body, alloc);
    }
    _ = status;
    _ = headers;
