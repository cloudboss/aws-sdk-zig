...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePermissionVersionInput, options: Options) !DeletePermissionVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePermissionVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "permissionArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.permission_arn);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "permissionVersion=");
    {
        const num_str = std.fmt.allocPrint(alloc, "{d}", .{input.permission_version}) catch "";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePermissionVersionOutput {
    var result: DeletePermissionVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePermissionVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
