...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableSharingWithAwsOrganizationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableSharingWithAwsOrganizationOutput {
    var result: EnableSharingWithAwsOrganizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EnableSharingWithAwsOrganizationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
