...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableSharingWithAwsOrganizationInput, options: CallOptions) !EnableSharingWithAwsOrganizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableSharingWithAwsOrganizationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    const path = "/enablesharingwithawsorganization";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableSharingWithAwsOrganizationOutput {
    var result: EnableSharingWithAwsOrganizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EnableSharingWithAwsOrganizationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
