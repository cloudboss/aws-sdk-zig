...
const ServiceNameAndResourceType = @import("service_name_and_resource_type.zig").ServiceNameAndResourceType;

pub const ListResourceTypesInput = struct {
...
pub const ListResourceTypesOutput = struct {
    ...
    /// can be
    /// shared using RAM.
    resource_types: ?[]const ServiceNameAndResourceType = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceTypesInput, options: CallOptions) !ListResourceTypesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListResourceTypesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListResourceTypesOutput {
    var result: ListResourceTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourceTypesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
