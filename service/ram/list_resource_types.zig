...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceTypesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceTypesOutput {
    var result: ListResourceTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourceTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
