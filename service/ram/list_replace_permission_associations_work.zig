...
pub const ListReplacePermissionAssociationsWorkInput = struct {
...
pub const ListReplacePermissionAssociationsWorkOutput = struct {
    ...
    replace_permission_associations_works: ?[]const ReplacePermissionAssociationsWork = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListReplacePermissionAssociationsWorkInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/listreplacepermissionassociationswork";
    ...
    if (input.work_ids) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"workIds\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListReplacePermissionAssociationsWorkOutput {
    var result: ListReplacePermissionAssociationsWorkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReplacePermissionAssociationsWorkOutput, body, allocator);
    }
    _ = status;
    _ = headers;
