...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReplacePermissionAssociationsWorkInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.work_ids) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"workIds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReplacePermissionAssociationsWorkOutput {
    var result: ListReplacePermissionAssociationsWorkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListReplacePermissionAssociationsWorkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
