...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSourceAssociationsInput, options: Options) !ListSourceAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSourceAssociationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSourceAssociationsOutput {
    var result: ListSourceAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSourceAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
