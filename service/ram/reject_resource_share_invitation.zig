...
pub const RejectResourceShareInvitationInput = struct {
    ...
    /// (ARN)](https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) of the invitation that you want to reject.
...
pub const RejectResourceShareInvitationOutput = struct {
    ...
    resource_share_invitation: ?ResourceShareInvitation = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RejectResourceShareInvitationInput, options: CallOptions) !RejectResourceShareInvitationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RejectResourceShareInvitationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/rejectresourceshareinvitation";
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"resourceShareInvitationArn\":");
    try aws.json.writeValue(@TypeOf(input.resource_share_invitation_arn), input.resource_share_invitation_arn, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RejectResourceShareInvitationOutput {
    var result: RejectResourceShareInvitationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RejectResourceShareInvitationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
