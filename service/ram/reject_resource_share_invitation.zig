...
fn serializeRequest(alloc: std.mem.Allocator, input: RejectResourceShareInvitationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RejectResourceShareInvitationOutput {
    var result: RejectResourceShareInvitationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RejectResourceShareInvitationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
