...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceSharePermissionsInput, options: Options) !ListResourceSharePermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceSharePermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceSharePermissionsOutput {
    var result: ListResourceSharePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourceSharePermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
