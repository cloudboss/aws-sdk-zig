...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PromotePermissionCreatedFromPolicyInput, options: Options) !PromotePermissionCreatedFromPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PromotePermissionCreatedFromPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PromotePermissionCreatedFromPolicyOutput {
    var result: PromotePermissionCreatedFromPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PromotePermissionCreatedFromPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
