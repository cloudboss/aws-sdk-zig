...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTargetAccountConfigurationInput, options: Options) !CreateTargetAccountConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTargetAccountConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTargetAccountConfigurationOutput {
    var result: CreateTargetAccountConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTargetAccountConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
