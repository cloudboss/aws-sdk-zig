...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSafetyLeverStateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSafetyLeverStateOutput {
    var result: UpdateSafetyLeverStateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSafetyLeverStateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
