...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTargetAccountConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTargetAccountConfigurationOutput {
    var result: UpdateTargetAccountConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTargetAccountConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
