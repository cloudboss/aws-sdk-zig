...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateExperimentTemplateInput, options: CallOptions) !CreateExperimentTemplateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateExperimentTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.stop_conditions), input.stop_conditions, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateExperimentTemplateOutput {
    var result: CreateExperimentTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateExperimentTemplateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
