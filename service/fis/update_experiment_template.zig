...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateExperimentTemplateInput, options: Options) !UpdateExperimentTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateExperimentTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateExperimentTemplateOutput {
    var result: UpdateExperimentTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateExperimentTemplateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
