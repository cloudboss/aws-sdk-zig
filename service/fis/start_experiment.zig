...
fn serializeRequest(alloc: std.mem.Allocator, input: StartExperimentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"experimentTemplateId\":");
    try aws.json.writeValue(@TypeOf(input.experiment_template_id), input.experiment_template_id, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
