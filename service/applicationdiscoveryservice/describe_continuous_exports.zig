...
const ContinuousExportDescription = @import("continuous_export_description.zig").ContinuousExportDescription;

pub const DescribeContinuousExportsInput = struct {
...
pub const DescribeContinuousExportsOutput = struct {
    /// A list of continuous export descriptions.
    descriptions: ?[]const ContinuousExportDescription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContinuousExportsInput, options: CallOptions) !DescribeContinuousExportsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeContinuousExportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSPoseidonService_V2015_11_01.DescribeContinuousExports");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeContinuousExportsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeContinuousExportsOutput, body, allocator);
}
