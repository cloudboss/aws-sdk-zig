...
const ConfigurationTag = @import("configuration_tag.zig").ConfigurationTag;

pub const DescribeTagsInput = struct {
    /// You can filter the list using a *key*-*value*
    /// format. You can separate these items by using logical operators. Allowed
    /// filters include
    /// `tagKey`, `tagValue`, and `configurationId`.
    filters: ?[]const TagFilter = null,
...
pub const DescribeTagsOutput = struct {
    ...
    tags: ?[]const ConfigurationTag = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTagsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSPoseidonService_V2015_11_01.DescribeTags");
