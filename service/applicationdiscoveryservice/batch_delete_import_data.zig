...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteImportDataInput, options: Options) !BatchDeleteImportDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteImportDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPoseidonService_V2015_11_01.BatchDeleteImportData");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteImportDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDeleteImportDataOutput, body, alloc);
}
