...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopDataCollectionByAgentIdsInput, options: Options) !StopDataCollectionByAgentIdsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopDataCollectionByAgentIdsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPoseidonService_V2015_11_01.StopDataCollectionByAgentIds");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopDataCollectionByAgentIdsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopDataCollectionByAgentIdsOutput, body, alloc);
}
