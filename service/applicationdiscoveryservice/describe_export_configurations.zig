...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeExportConfigurationsInput, options: Options) !DescribeExportConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeExportConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPoseidonService_V2015_11_01.DescribeExportConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeExportConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeExportConfigurationsOutput, body, alloc);
}
