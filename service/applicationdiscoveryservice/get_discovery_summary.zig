...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDiscoverySummaryInput, options: Options) !GetDiscoverySummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDiscoverySummaryInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPoseidonService_V2015_11_01.GetDiscoverySummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDiscoverySummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDiscoverySummaryOutput, body, alloc);
}
