...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateConfigurationItemsToApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPoseidonService_V2015_11_01.AssociateConfigurationItemsToApplication");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateConfigurationItemsToApplicationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
