...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBatchDeleteConfigurationTaskInput, options: Options) !DescribeBatchDeleteConfigurationTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBatchDeleteConfigurationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSPoseidonService_V2015_11_01.DescribeBatchDeleteConfigurationTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBatchDeleteConfigurationTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeBatchDeleteConfigurationTaskOutput, body, alloc);
}
