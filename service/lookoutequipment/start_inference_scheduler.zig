...
pub const StartInferenceSchedulerInput = struct {
...
pub const StartInferenceSchedulerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartInferenceSchedulerInput, options: CallOptions) !StartInferenceSchedulerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartInferenceSchedulerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.StartInferenceScheduler");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartInferenceSchedulerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartInferenceSchedulerOutput, body, allocator);
}
