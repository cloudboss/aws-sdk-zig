...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLabelsInput, options: Options) !ListLabelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLabelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.ListLabels");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLabelsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLabelsOutput, body, alloc);
}
