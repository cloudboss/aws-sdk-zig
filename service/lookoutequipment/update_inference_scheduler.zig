...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateInferenceSchedulerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.UpdateInferenceScheduler");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateInferenceSchedulerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
