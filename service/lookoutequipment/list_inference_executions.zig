...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInferenceExecutionsInput, options: Options) !ListInferenceExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInferenceExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.ListInferenceExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInferenceExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInferenceExecutionsOutput, body, alloc);
}
