...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportDatasetInput, options: Options) !ImportDatasetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportDatasetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.ImportDataset");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportDatasetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ImportDatasetOutput, body, alloc);
}
