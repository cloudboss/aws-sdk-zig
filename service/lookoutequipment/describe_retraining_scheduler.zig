...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRetrainingSchedulerInput, options: Options) !DescribeRetrainingSchedulerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRetrainingSchedulerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.DescribeRetrainingScheduler");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRetrainingSchedulerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRetrainingSchedulerOutput, body, alloc);
}
