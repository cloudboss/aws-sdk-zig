...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRetrainingSchedulersInput, options: Options) !ListRetrainingSchedulersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRetrainingSchedulersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.ListRetrainingSchedulers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRetrainingSchedulersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRetrainingSchedulersOutput, body, alloc);
}
