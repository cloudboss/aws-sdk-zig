...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRetrainingSchedulerInput, options: Options) !StartRetrainingSchedulerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRetrainingSchedulerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.StartRetrainingScheduler");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRetrainingSchedulerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartRetrainingSchedulerOutput, body, alloc);
}
