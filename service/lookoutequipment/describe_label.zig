...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLabelInput, options: Options) !DescribeLabelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLabelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.DescribeLabel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLabelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeLabelOutput, body, alloc);
}
