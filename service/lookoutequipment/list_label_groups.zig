...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLabelGroupsInput, options: Options) !ListLabelGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLabelGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.ListLabelGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLabelGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLabelGroupsOutput, body, alloc);
}
