...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataIngestionJobsInput, options: Options) !ListDataIngestionJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataIngestionJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.ListDataIngestionJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataIngestionJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDataIngestionJobsOutput, body, alloc);
}
