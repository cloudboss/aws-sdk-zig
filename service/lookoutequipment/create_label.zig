...
pub const CreateLabelInput = struct {
...
pub const CreateLabelOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLabelInput, options: CallOptions) !CreateLabelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLabelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.CreateLabel");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLabelOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLabelOutput, body, allocator);
}
