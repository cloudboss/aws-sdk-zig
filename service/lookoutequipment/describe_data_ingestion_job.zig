...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataIngestionJobInput, options: Options) !DescribeDataIngestionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataIngestionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.DescribeDataIngestionJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataIngestionJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDataIngestionJobOutput, body, alloc);
}
