...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInferenceSchedulersInput, options: Options) !ListInferenceSchedulersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInferenceSchedulersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.ListInferenceSchedulers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInferenceSchedulersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInferenceSchedulersOutput, body, alloc);
}
