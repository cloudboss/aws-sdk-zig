...
pub const StopInferenceSchedulerInput = struct {
...
pub const StopInferenceSchedulerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopInferenceSchedulerInput, options: CallOptions) !StopInferenceSchedulerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopInferenceSchedulerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.StopInferenceScheduler");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopInferenceSchedulerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopInferenceSchedulerOutput, body, allocator);
}
