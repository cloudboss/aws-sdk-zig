...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRetrainingSchedulerInput, options: Options) !CreateRetrainingSchedulerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRetrainingSchedulerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.CreateRetrainingScheduler");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRetrainingSchedulerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateRetrainingSchedulerOutput, body, alloc);
}
