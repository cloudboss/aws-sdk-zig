...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeModelVersionInput, options: Options) !DescribeModelVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeModelVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.DescribeModelVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeModelVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeModelVersionOutput, body, alloc);
}
