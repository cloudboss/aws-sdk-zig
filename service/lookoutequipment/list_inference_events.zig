...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInferenceEventsInput, options: Options) !ListInferenceEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInferenceEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.ListInferenceEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInferenceEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInferenceEventsOutput, body, alloc);
}
