...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateActiveModelVersionInput, options: Options) !UpdateActiveModelVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateActiveModelVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSLookoutEquipmentFrontendService.UpdateActiveModelVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateActiveModelVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateActiveModelVersionOutput, body, alloc);
}
