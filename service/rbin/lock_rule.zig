...
pub const LockRuleInput = struct {
...
pub const LockRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: LockRuleInput, options: Options) !LockRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: LockRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/lock");
    ...
    try aws.json.writeValue(@TypeOf(input.lock_configuration), input.lock_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !LockRuleOutput {
    var result: LockRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(LockRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
