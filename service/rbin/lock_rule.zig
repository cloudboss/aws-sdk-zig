...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: LockRuleInput, options: CallOptions) !LockRuleOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "rbin");
...
fn serializeRequest(allocator: std.mem.Allocator, input: LockRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/lock");
    ...
    try aws.json.writeValue(@TypeOf(input.lock_configuration), input.lock_configuration, allocator, &body_buf);
    has_prev = true;
