...
fn serializeRequest(alloc: std.mem.Allocator, input: UnlockRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/unlock");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UnlockRuleOutput {
    var result: UnlockRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UnlockRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
