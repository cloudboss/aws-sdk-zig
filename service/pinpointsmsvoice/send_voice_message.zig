...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendVoiceMessageInput, options: Options) !SendVoiceMessageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendVoiceMessageInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.caller_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"CallerId\":");
    ...
    if (input.origination_phone_number) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OriginationPhoneNumber\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendVoiceMessageOutput {
    var result: SendVoiceMessageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendVoiceMessageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
