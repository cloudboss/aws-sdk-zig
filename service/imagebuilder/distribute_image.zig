...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DistributeImageInput, options: Options) !DistributeImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DistributeImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.distribution_configuration_arn), input.distribution_configuration_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"executionRole\":");
    try aws.json.writeValue(@TypeOf(input.execution_role), input.execution_role, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"sourceImage\":");
    try aws.json.writeValue(@TypeOf(input.source_image), input.source_image, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DistributeImageOutput {
    var result: DistributeImageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DistributeImageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
