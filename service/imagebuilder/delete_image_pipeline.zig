...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteImagePipelineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteImagePipelineOutput {
    var result: DeleteImagePipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteImagePipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
