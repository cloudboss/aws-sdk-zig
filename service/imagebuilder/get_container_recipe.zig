...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContainerRecipeInput, options: Options) !GetContainerRecipeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContainerRecipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContainerRecipeOutput {
    var result: GetContainerRecipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContainerRecipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
