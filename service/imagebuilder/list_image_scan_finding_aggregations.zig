...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImageScanFindingAggregationsInput, options: Options) !ListImageScanFindingAggregationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImageScanFindingAggregationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImageScanFindingAggregationsOutput {
    var result: ListImageScanFindingAggregationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImageScanFindingAggregationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
