...
fn serializeRequest(alloc: std.mem.Allocator, input: StartImagePipelineExecutionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartImagePipelineExecutionOutput {
    var result: StartImagePipelineExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartImagePipelineExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
