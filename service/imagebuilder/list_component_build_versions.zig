...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListComponentBuildVersionsInput, options: Options) !ListComponentBuildVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListComponentBuildVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/ListComponentBuildVersions";

    var body_buf: std.ArrayList(u8) = .{};
    var has_prev = false;
    try body_buf.appendSlice(alloc, "{");

    if (input.component_version_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"componentVersionArn\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListComponentBuildVersionsOutput {
    var result: ListComponentBuildVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListComponentBuildVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
