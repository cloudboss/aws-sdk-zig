...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkflowExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "workflowExecutionId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.workflow_execution_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkflowExecutionOutput {
    var result: GetWorkflowExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkflowExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
