...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendWorkflowStepActionInput, options: Options) !SendWorkflowStepActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendWorkflowStepActionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"stepExecutionId\":");
    try aws.json.writeValue(@TypeOf(input.step_execution_id), input.step_execution_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendWorkflowStepActionOutput {
    var result: SendWorkflowStepActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendWorkflowStepActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
