...
pub const UpdateDistributionConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDistributionConfigurationInput, options: Options) !UpdateDistributionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDistributionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/UpdateDistributionConfiguration";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDistributionConfigurationOutput {
    var result: UpdateDistributionConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDistributionConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
