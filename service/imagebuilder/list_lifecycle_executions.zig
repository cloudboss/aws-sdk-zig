...
pub const ListLifecycleExecutionsOutput = struct {
    /// A list of lifecycle runtime instances for the specified resource.
    lifecycle_executions: ?[]const LifecycleExecution = null,
    ...
    pub const json_field_names = .{
        .lifecycle_executions = "lifecycleExecutions",
        .next_token = "nextToken",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLifecycleExecutionsInput, options: Options) !ListLifecycleExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLifecycleExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/ListLifecycleExecutions";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLifecycleExecutionsOutput {
    var result: ListLifecycleExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLifecycleExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
