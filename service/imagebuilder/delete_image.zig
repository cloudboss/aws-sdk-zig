...
pub const DeleteImageInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteImageInput, options: Options) !DeleteImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "imageBuildVersionArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.image_build_version_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteImageOutput {
    var result: DeleteImageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteImageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
