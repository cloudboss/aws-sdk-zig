...
pub const PutContainerRecipePolicyInput = struct {
...
pub const PutContainerRecipePolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutContainerRecipePolicyInput, options: Options) !PutContainerRecipePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutContainerRecipePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/PutContainerRecipePolicy";
    ...
    try aws.json.writeValue(@TypeOf(input.container_recipe_arn), input.container_recipe_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutContainerRecipePolicyOutput {
    var result: PutContainerRecipePolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutContainerRecipePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
