...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutContainerRecipePolicyInput, options: CallOptions) !PutContainerRecipePolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutContainerRecipePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.container_recipe_arn), input.container_recipe_arn, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
