...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateImagePipelineInput, options: Options) !CreateImagePipelineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateImagePipelineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateImagePipelineOutput {
    var result: CreateImagePipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateImagePipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
