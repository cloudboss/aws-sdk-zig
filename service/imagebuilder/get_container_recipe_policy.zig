...
pub const GetContainerRecipePolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetContainerRecipePolicyInput, options: Options) !GetContainerRecipePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetContainerRecipePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetContainerRecipePolicy";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetContainerRecipePolicyOutput {
    var result: GetContainerRecipePolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetContainerRecipePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
