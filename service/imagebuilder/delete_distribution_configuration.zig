...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDistributionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "distributionConfigurationArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.distribution_configuration_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDistributionConfigurationOutput {
    var result: DeleteDistributionConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDistributionConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
