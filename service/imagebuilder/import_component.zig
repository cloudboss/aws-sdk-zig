...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportComponentInput, options: Options) !ImportComponentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportComponentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportComponentOutput {
    var result: ImportComponentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportComponentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
