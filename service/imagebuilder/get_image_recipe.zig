...
pub const GetImageRecipeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetImageRecipeInput, options: Options) !GetImageRecipeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImageRecipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImageRecipeOutput {
    var result: GetImageRecipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImageRecipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
