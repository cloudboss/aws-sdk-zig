...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelLifecycleExecutionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelLifecycleExecutionOutput {
    var result: CancelLifecycleExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelLifecycleExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
