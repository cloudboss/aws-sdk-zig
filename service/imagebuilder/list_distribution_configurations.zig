...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDistributionConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDistributionConfigurationsOutput {
    var result: ListDistributionConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDistributionConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
