...
pub const PutImagePolicyInput = struct {
...
pub const PutImagePolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutImagePolicyInput, options: Options) !PutImagePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutImagePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"imageArn\":");
    try aws.json.writeValue(@TypeOf(input.image_arn), input.image_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutImagePolicyOutput {
    var result: PutImagePolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutImagePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
