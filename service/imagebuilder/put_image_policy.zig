...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutImagePolicyInput, options: CallOptions) !PutImagePolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutImagePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"imageArn\":");
    try aws.json.writeValue(@TypeOf(input.image_arn), input.image_arn, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
