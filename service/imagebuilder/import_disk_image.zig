...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportDiskImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.os_version), input.os_version, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportDiskImageOutput {
    var result: ImportDiskImageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportDiskImageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
