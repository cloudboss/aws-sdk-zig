...
pub const GetImagePolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetImagePolicyInput, options: CallOptions) !GetImagePolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetImagePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetImagePolicy";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "imageArn=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.image_arn);
    query_has_prev = true;
