...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImagePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "imageArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.image_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImagePolicyOutput {
    var result: GetImagePolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImagePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
