...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteLifecyclePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/DeleteLifecyclePolicy";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "lifecyclePolicyArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.lifecycle_policy_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteLifecyclePolicyOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteLifecyclePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
