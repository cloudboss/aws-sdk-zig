...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetImageInput, options: Options) !GetImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImageOutput {
    var result: GetImageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
