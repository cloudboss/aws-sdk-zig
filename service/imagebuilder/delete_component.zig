...
pub const DeleteComponentInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteComponentInput, options: Options) !DeleteComponentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteComponentInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/DeleteComponent";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "componentBuildVersionArn=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.component_build_version_arn);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteComponentOutput {
    var result: DeleteComponentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteComponentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
