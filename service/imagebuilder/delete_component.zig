...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteComponentInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "componentBuildVersionArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.component_build_version_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteComponentOutput {
    var result: DeleteComponentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteComponentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
