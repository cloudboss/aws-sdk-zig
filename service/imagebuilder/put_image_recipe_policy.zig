...
fn serializeRequest(alloc: std.mem.Allocator, input: PutImageRecipePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.image_recipe_arn), input.image_recipe_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutImageRecipePolicyOutput {
    var result: PutImageRecipePolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutImageRecipePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
