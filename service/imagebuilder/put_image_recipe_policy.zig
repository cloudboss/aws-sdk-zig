...
pub const PutImageRecipePolicyInput = struct {
...
pub const PutImageRecipePolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutImageRecipePolicyInput, options: CallOptions) !PutImageRecipePolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutImageRecipePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/PutImageRecipePolicy";
    ...
    try aws.json.writeValue(@TypeOf(input.image_recipe_arn), input.image_recipe_arn, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutImageRecipePolicyOutput {
    var result: PutImageRecipePolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutImageRecipePolicyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
