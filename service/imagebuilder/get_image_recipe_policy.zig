...
pub const GetImageRecipePolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetImageRecipePolicyInput, options: Options) !GetImageRecipePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImageRecipePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetImageRecipePolicy";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImageRecipePolicyOutput {
    var result: GetImageRecipePolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImageRecipePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
