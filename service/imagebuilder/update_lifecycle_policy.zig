...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLifecyclePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"lifecyclePolicyArn\":");
    try aws.json.writeValue(@TypeOf(input.lifecycle_policy_arn), input.lifecycle_policy_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"policyDetails\":");
    try aws.json.writeValue(@TypeOf(input.policy_details), input.policy_details, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"resourceSelection\":");
    try aws.json.writeValue(@TypeOf(input.resource_selection), input.resource_selection, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLifecyclePolicyOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateLifecyclePolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
