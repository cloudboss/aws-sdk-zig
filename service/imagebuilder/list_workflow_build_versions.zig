...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkflowBuildVersionsInput, options: Options) !ListWorkflowBuildVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkflowBuildVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.workflow_version_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"workflowVersionArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkflowBuildVersionsOutput {
    var result: ListWorkflowBuildVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkflowBuildVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
