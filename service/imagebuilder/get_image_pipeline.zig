...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImagePipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "imagePipelineArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.image_pipeline_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImagePipelineOutput {
    var result: GetImagePipelineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImagePipelineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
