...
const WorkflowExecutionMetadata = @import("workflow_execution_metadata.zig").WorkflowExecutionMetadata;

pub const ListWorkflowExecutionsInput = struct {
...
pub const ListWorkflowExecutionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkflowExecutionsInput, options: Options) !ListWorkflowExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkflowExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/ListWorkflowExecutions";
    ...
    try body_buf.appendSlice(alloc, "\"imageBuildVersionArn\":");
    try aws.json.writeValue(@TypeOf(input.image_build_version_arn), input.image_build_version_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkflowExecutionsOutput {
    var result: ListWorkflowExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkflowExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
