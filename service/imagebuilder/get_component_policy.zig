...
pub const GetComponentPolicyInput = struct {
...
pub const GetComponentPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetComponentPolicyInput, options: CallOptions) !GetComponentPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetComponentPolicyInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("imagebuilder", "imagebuilder", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/GetComponentPolicy";

    var query_buf: std.ArrayList(u8) = .empty;
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "componentArn=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.component_arn);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetComponentPolicyOutput {
    var result: GetComponentPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetComponentPolicyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
