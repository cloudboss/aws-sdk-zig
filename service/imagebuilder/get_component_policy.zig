...
fn serializeRequest(alloc: std.mem.Allocator, input: GetComponentPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "componentArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.component_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetComponentPolicyOutput {
    var result: GetComponentPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetComponentPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
