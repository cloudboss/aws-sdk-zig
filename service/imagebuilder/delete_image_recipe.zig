...
pub const DeleteImageRecipeInput = struct {
...
pub const DeleteImageRecipeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteImageRecipeInput, options: CallOptions) !DeleteImageRecipeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteImageRecipeInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/DeleteImageRecipe";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "imageRecipeArn=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.image_recipe_arn);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteImageRecipeOutput {
    var result: DeleteImageRecipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteImageRecipeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
