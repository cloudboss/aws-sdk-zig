...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteImageRecipeInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "imageRecipeArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.image_recipe_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteImageRecipeOutput {
    var result: DeleteImageRecipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteImageRecipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
