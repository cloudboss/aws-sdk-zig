...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkflowStepExecutionsInput, options: Options) !ListWorkflowStepExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkflowStepExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"workflowExecutionId\":");
    try aws.json.writeValue(@TypeOf(input.workflow_execution_id), input.workflow_execution_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkflowStepExecutionsOutput {
    var result: ListWorkflowStepExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWorkflowStepExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
