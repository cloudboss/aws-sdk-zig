...
pub const RetryImageInput = struct {
...
pub const RetryImageOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RetryImageInput, options: CallOptions) !RetryImageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RetryImageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RetryImageOutput {
    var result: RetryImageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RetryImageOutput, body, allocator);
    }
    _ = status;
    _ = headers;
