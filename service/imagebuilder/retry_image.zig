...
fn serializeRequest(alloc: std.mem.Allocator, input: RetryImageInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RetryImageOutput {
    var result: RetryImageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RetryImageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
