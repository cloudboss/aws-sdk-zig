...
pub const CreateDistributionConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDistributionConfigurationInput, options: Options) !CreateDistributionConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDistributionConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/CreateDistributionConfiguration";
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"distributions\":");
    try aws.json.writeValue(@TypeOf(input.distributions), input.distributions, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDistributionConfigurationOutput {
    var result: CreateDistributionConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDistributionConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
