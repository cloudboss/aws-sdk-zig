...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInfrastructureConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "infrastructureConfigurationArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.infrastructure_configuration_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInfrastructureConfigurationOutput {
    var result: GetInfrastructureConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInfrastructureConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
