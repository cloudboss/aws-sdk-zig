...
pub const GetInfrastructureConfigurationInput = struct {
...
pub const GetInfrastructureConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInfrastructureConfigurationInput, options: CallOptions) !GetInfrastructureConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetInfrastructureConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetInfrastructureConfiguration";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(allocator, "&");
    try query_buf.appendSlice(allocator, "infrastructureConfigurationArn=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.infrastructure_configuration_arn);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetInfrastructureConfigurationOutput {
    var result: GetInfrastructureConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetInfrastructureConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
