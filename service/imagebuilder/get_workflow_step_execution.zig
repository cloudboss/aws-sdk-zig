...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetWorkflowStepExecutionInput, options: Options) !GetWorkflowStepExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkflowStepExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "stepExecutionId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.step_execution_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkflowStepExecutionOutput {
    var result: GetWorkflowStepExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkflowStepExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
