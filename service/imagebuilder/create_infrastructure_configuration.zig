...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInfrastructureConfigurationInput, options: Options) !CreateInfrastructureConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInfrastructureConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.instance_metadata_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"instanceMetadataOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"instanceProfileName\":");
    try aws.json.writeValue(@TypeOf(input.instance_profile_name), input.instance_profile_name, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.key_pair) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"keyPair\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.logging) |v| {
    ...
    if (input.placement) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"placement\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.terminate_instance_on_failure) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"terminateInstanceOnFailure\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInfrastructureConfigurationOutput {
    var result: CreateInfrastructureConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateInfrastructureConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
