...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutComponentPolicyInput, options: Options) !PutComponentPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutComponentPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"componentArn\":");
    try aws.json.writeValue(@TypeOf(input.component_arn), input.component_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"policy\":");
    try aws.json.writeValue(@TypeOf(input.policy), input.policy, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutComponentPolicyOutput {
    var result: PutComponentPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutComponentPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
