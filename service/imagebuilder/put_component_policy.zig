...
pub const PutComponentPolicyInput = struct {
...
pub const PutComponentPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutComponentPolicyInput, options: CallOptions) !PutComponentPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutComponentPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"componentArn\":");
    try aws.json.writeValue(@TypeOf(input.component_arn), input.component_arn, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutComponentPolicyOutput {
    var result: PutComponentPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutComponentPolicyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
