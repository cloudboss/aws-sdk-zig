...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateImageRecipeInput, options: Options) !CreateImageRecipeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateImageRecipeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.additional_instance_configuration) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"additionalInstanceConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.ami_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"amiTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.block_device_mappings) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"blockDeviceMappings\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateImageRecipeOutput {
    var result: CreateImageRecipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateImageRecipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
