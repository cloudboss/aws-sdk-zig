...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateImagePipelineInput, options: CallOptions) !UpdateImagePipelineOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateImagePipelineInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"imagePipelineArn\":");
    try aws.json.writeValue(@TypeOf(input.image_pipeline_arn), input.image_pipeline_arn, allocator, &body_buf);
    has_prev = true;
