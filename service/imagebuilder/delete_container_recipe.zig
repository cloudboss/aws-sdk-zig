...
pub const DeleteContainerRecipeInput = struct {
...
pub const DeleteContainerRecipeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteContainerRecipeInput, options: Options) !DeleteContainerRecipeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteContainerRecipeInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/DeleteContainerRecipe";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "containerRecipeArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.container_recipe_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteContainerRecipeOutput {
    var result: DeleteContainerRecipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteContainerRecipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
