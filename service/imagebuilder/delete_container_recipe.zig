...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteContainerRecipeInput, options: CallOptions) !DeleteContainerRecipeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteContainerRecipeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteContainerRecipeOutput {
    var result: DeleteContainerRecipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteContainerRecipeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
