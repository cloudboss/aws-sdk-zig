...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDistributionConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDistributionConfigurationOutput {
    var result: GetDistributionConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDistributionConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
