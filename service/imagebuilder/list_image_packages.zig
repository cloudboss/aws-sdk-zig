...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImagePackagesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImagePackagesOutput {
    var result: ListImagePackagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImagePackagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
