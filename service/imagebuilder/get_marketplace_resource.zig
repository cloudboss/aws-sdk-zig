...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMarketplaceResourceInput, options: CallOptions) !GetMarketplaceResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMarketplaceResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.resource_location) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"resourceLocation\":");
