...
const MarketplaceResourceType = @import("marketplace_resource_type.zig").MarketplaceResourceType;

pub const GetMarketplaceResourceInput = struct {
...
pub const GetMarketplaceResourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMarketplaceResourceInput, options: Options) !GetMarketplaceResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMarketplaceResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetMarketplaceResource";
    ...
    has_prev = true;
    if (input.resource_location) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resourceLocation\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMarketplaceResourceOutput {
    var result: GetMarketplaceResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMarketplaceResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
