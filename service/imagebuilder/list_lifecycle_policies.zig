...
pub const ListLifecyclePoliciesOutput = struct {
    ...
    lifecycle_policy_summary_list: ?[]const LifecyclePolicySummary = null,
    ...
    pub const json_field_names = .{
        .lifecycle_policy_summary_list = "lifecyclePolicySummaryList",
        .next_token = "nextToken",
    };
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLifecyclePoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/ListLifecyclePolicies";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLifecyclePoliciesOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLifecyclePoliciesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
