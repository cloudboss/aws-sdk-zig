...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImageScanFindingsInput, options: Options) !ListImageScanFindingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImageScanFindingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImageScanFindingsOutput {
    var result: ListImageScanFindingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImageScanFindingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
