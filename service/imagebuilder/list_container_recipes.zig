...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContainerRecipesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContainerRecipesOutput {
    var result: ListContainerRecipesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListContainerRecipesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
