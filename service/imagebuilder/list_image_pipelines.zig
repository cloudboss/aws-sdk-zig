...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImagePipelinesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImagePipelinesOutput {
    var result: ListImagePipelinesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImagePipelinesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
