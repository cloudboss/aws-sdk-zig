...
pub const ListImagePipelinesInput = struct {
...
pub const ListImagePipelinesOutput = struct {
    /// The list of image pipelines.
    image_pipeline_list: ?[]const ImagePipeline = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImagePipelinesInput, options: CallOptions) !ListImagePipelinesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListImagePipelinesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListImagePipelinesOutput {
    var result: ListImagePipelinesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImagePipelinesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
