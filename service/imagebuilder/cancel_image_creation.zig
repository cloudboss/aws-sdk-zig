...
pub const CancelImageCreationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelImageCreationInput, options: Options) !CancelImageCreationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelImageCreationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/CancelImageCreation";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelImageCreationOutput {
    var result: CancelImageCreationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelImageCreationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
