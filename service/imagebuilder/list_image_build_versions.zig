...
const ImageSummary = @import("image_summary.zig").ImageSummary;

pub const ListImageBuildVersionsInput = struct {
...
pub const ListImageBuildVersionsOutput = struct {
    /// The list of image build versions.
    image_summary_list: ?[]const ImageSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImageBuildVersionsInput, options: Options) !ListImageBuildVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImageBuildVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/ListImageBuildVersions";
    ...
    if (input.image_version_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"imageVersionArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImageBuildVersionsOutput {
    var result: ListImageBuildVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImageBuildVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
