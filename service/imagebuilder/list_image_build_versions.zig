...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImageBuildVersionsInput, options: CallOptions) !ListImageBuildVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListImageBuildVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.image_version_arn) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"imageVersionArn\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
