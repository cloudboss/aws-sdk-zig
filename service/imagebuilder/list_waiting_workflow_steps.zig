...
pub const ListWaitingWorkflowStepsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWaitingWorkflowStepsInput, options: Options) !ListWaitingWorkflowStepsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWaitingWorkflowStepsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/ListWaitingWorkflowSteps";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWaitingWorkflowStepsOutput {
    var result: ListWaitingWorkflowStepsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListWaitingWorkflowStepsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
