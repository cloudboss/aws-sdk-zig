...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInfrastructureConfigurationsInput, options: Options) !ListInfrastructureConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInfrastructureConfigurationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInfrastructureConfigurationsOutput {
    var result: ListInfrastructureConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListInfrastructureConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
