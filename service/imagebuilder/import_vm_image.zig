...
pub const ImportVmImageInput = struct {
    ...
    pub const json_field_names = .{
        ...
        .tags = "tags",
        .vm_import_task_id = "vmImportTaskId",
    };
};

pub const ImportVmImageOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportVmImageInput, options: Options) !ImportVmImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportVmImageInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (input.os_version) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"osVersion\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"platform\":");
    try aws.json.writeValue(@TypeOf(input.platform), input.platform, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"vmImportTaskId\":");
    try aws.json.writeValue(@TypeOf(input.vm_import_task_id), input.vm_import_task_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportVmImageOutput {
    var result: ImportVmImageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ImportVmImageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
