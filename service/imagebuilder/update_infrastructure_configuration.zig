...
pub const UpdateInfrastructureConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateInfrastructureConfigurationInput, options: Options) !UpdateInfrastructureConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateInfrastructureConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/UpdateInfrastructureConfiguration";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateInfrastructureConfigurationOutput {
    var result: UpdateInfrastructureConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateInfrastructureConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
