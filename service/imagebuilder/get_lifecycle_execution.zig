...
pub const GetLifecycleExecutionOutput = struct {
    /// Runtime details for the specified runtime instance of the lifecycle policy.
    lifecycle_execution: ?LifecycleExecution = null,

    pub const json_field_names = .{
        .lifecycle_execution = "lifecycleExecution",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLifecycleExecutionInput, options: Options) !GetLifecycleExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLifecycleExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetLifecycleExecution";

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "lifecycleExecutionId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.lifecycle_execution_id);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLifecycleExecutionOutput {
    var result: GetLifecycleExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLifecycleExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
