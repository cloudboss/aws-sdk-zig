...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVoicesInput, options: Options) !DescribeVoicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVoicesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVoicesOutput {
    var result: DescribeVoicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeVoicesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
