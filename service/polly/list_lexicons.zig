...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLexiconsInput, options: Options) !ListLexiconsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLexiconsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLexiconsOutput {
    var result: ListLexiconsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLexiconsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
