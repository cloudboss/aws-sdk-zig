...
pub const GetSpeechSynthesisTaskInput = struct {
...
pub const GetSpeechSynthesisTaskOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSpeechSynthesisTaskInput, options: Options) !GetSpeechSynthesisTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSpeechSynthesisTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/synthesisTasks/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSpeechSynthesisTaskOutput {
    var result: GetSpeechSynthesisTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSpeechSynthesisTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
