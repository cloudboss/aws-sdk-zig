...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSpeechSynthesisTasksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSpeechSynthesisTasksOutput {
    var result: ListSpeechSynthesisTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSpeechSynthesisTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
