...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLexiconInput, options: Options) !GetLexiconOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLexiconInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/lexicons/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLexiconOutput {
    var result: GetLexiconOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetLexiconOutput, body, alloc);
    }
    _ = status;
    _ = headers;
