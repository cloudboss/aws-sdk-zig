...
pub const ResumeBatchLoadTaskInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResumeBatchLoadTaskInput, options: CallOptions) !ResumeBatchLoadTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResumeBatchLoadTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Timestream_20181101.ResumeBatchLoadTask");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResumeBatchLoadTaskOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
