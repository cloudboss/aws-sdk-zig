...
pub const DeleteTableInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTableInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Timestream_20181101.DeleteTable");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTableOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
