...
pub const DescribeDatabaseInput = struct {
...
pub const DescribeDatabaseOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDatabaseInput, options: CallOptions) !DescribeDatabaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDatabaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Timestream_20181101.DescribeDatabase");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDatabaseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDatabaseOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "RejectedRecordsException")) {
        const parsed_error: ?errors.RejectedRecordsException = aws.json.parseJsonObject(errors.RejectedRecordsException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .rejected_records_exception = typed_error } };
        }
    }
