...
pub const DeleteComputeNodeGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteComputeNodeGroupInput, options: Options) !DeleteComputeNodeGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteComputeNodeGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSParallelComputingService.DeleteComputeNodeGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteComputeNodeGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
