...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteComputeNodeGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSParallelComputingService.DeleteComputeNodeGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteComputeNodeGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
