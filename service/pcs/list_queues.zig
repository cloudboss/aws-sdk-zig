...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListQueuesInput, options: Options) !ListQueuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSParallelComputingService.ListQueues");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListQueuesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListQueuesOutput, body, alloc);
}
