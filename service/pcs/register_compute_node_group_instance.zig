...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterComputeNodeGroupInstanceInput, options: Options) !RegisterComputeNodeGroupInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterComputeNodeGroupInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSParallelComputingService.RegisterComputeNodeGroupInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterComputeNodeGroupInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterComputeNodeGroupInstanceOutput, body, alloc);
}
