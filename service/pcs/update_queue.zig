...
pub const UpdateQueueInput = struct {
    ...
    slurm_configuration: ?UpdateQueueSlurmConfigurationRequest = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSParallelComputingService.UpdateQueue");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQueueOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateQueueOutput, body, alloc);
}
