...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteQueueInput, options: Options) !DeleteQueueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSParallelComputingService.DeleteQueue");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteQueueOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
