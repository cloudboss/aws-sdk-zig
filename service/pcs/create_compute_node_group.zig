...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateComputeNodeGroupInput, options: Options) !CreateComputeNodeGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateComputeNodeGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSParallelComputingService.CreateComputeNodeGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateComputeNodeGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateComputeNodeGroupOutput, body, alloc);
}
