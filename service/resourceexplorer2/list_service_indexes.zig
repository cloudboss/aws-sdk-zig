...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceIndexesInput, options: Options) !ListServiceIndexesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServiceIndexesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServiceIndexesOutput {
    var result: ListServiceIndexesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListServiceIndexesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
