...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateViewInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.included_properties) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IncludedProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateViewOutput {
    var result: UpdateViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateViewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
