...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedViewsInput, options: CallOptions) !ListManagedViewsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListManagedViewsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.service_principal) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ServicePrincipal\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
