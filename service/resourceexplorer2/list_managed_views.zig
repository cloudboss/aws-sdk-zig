...
pub const ListManagedViewsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedViewsInput, options: Options) !ListManagedViewsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedViewsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.service_principal) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ServicePrincipal\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedViewsOutput {
    var result: ListManagedViewsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedViewsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
