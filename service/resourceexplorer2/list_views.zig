...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListViewsInput, options: Options) !ListViewsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListViewsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListViewsOutput {
    var result: ListViewsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListViewsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
