...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateResourceExplorerSetupInput, options: Options) !CreateResourceExplorerSetupOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateResourceExplorerSetupOutput {
    var result: CreateResourceExplorerSetupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateResourceExplorerSetupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
