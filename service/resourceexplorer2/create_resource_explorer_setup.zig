...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourceExplorerSetupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.aggregator_regions) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AggregatorRegions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RegionList\":");
    try aws.json.writeValue(@TypeOf(input.region_list), input.region_list, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ViewName\":");
    try aws.json.writeValue(@TypeOf(input.view_name), input.view_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourceExplorerSetupOutput {
    var result: CreateResourceExplorerSetupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateResourceExplorerSetupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
