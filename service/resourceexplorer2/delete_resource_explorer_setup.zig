...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResourceExplorerSetupInput, options: Options) !DeleteResourceExplorerSetupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteResourceExplorerSetupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.delete_in_all_regions) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DeleteInAllRegions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.region_list) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RegionList\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteResourceExplorerSetupOutput {
    var result: DeleteResourceExplorerSetupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteResourceExplorerSetupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
