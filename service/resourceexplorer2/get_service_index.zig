...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceIndexInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceIndexOutput {
    var result: GetServiceIndexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetServiceIndexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
