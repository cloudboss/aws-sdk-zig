...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIndexesInput, options: Options) !ListIndexesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIndexesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIndexesOutput {
    var result: ListIndexesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIndexesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
