...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIndexesForMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AccountIdList\":");
    try aws.json.writeValue(@TypeOf(input.account_id_list), input.account_id_list, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIndexesForMembersOutput {
    var result: ListIndexesForMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIndexesForMembersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
