...
const MemberIndex = @import("member_index.zig").MemberIndex;

pub const ListIndexesForMembersInput = struct {
    /// The account IDs will limit the output to only indexes from these accounts.
    account_id_list: []const []const u8,
...
pub const ListIndexesForMembersOutput = struct {
    /// A structure that contains the details and status of each index.
    indexes: ?[]const MemberIndex = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIndexesForMembersInput, options: CallOptions) !ListIndexesForMembersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListIndexesForMembersInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/ListIndexesForMembers";
    ...
    try body_buf.appendSlice(allocator, "\"AccountIdList\":");
    try aws.json.writeValue(@TypeOf(input.account_id_list), input.account_id_list, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListIndexesForMembersOutput {
    var result: ListIndexesForMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIndexesForMembersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
