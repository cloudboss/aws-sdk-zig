...
const SupportedResourceType = @import("supported_resource_type.zig").SupportedResourceType;

pub const ListSupportedResourceTypesInput = struct {
...
pub const ListSupportedResourceTypesOutput = struct {
    ...
    resource_types: ?[]const SupportedResourceType = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSupportedResourceTypesInput, options: Options) !ListSupportedResourceTypesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSupportedResourceTypesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/ListSupportedResourceTypes";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSupportedResourceTypesOutput {
    var result: ListSupportedResourceTypesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSupportedResourceTypesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
