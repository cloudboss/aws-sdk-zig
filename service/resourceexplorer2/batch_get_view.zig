...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetViewInput, options: Options) !BatchGetViewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetViewInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.view_arns) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ViewArns\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetViewOutput {
    var result: BatchGetViewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetViewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
