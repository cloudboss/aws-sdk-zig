...
pub const DescribeFargateProfileInput = struct {
...
pub const DescribeFargateProfileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFargateProfileInput, options: Options) !DescribeFargateProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFargateProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFargateProfileOutput {
    var result: DescribeFargateProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFargateProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
