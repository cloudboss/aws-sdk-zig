...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterClusterInput, options: Options) !DeregisterClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/cluster-registrations/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterClusterOutput {
    var result: DeregisterClusterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeregisterClusterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
