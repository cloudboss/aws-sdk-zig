...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccessEntryInput, options: Options) !UpdateAccessEntryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccessEntryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccessEntryOutput {
    var result: UpdateAccessEntryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccessEntryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
