...
pub const UpdateEksAnywhereSubscriptionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEksAnywhereSubscriptionInput, options: Options) !UpdateEksAnywhereSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEksAnywhereSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.auto_renew), input.auto_renew, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEksAnywhereSubscriptionOutput {
    var result: UpdateEksAnywhereSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEksAnywhereSubscriptionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
