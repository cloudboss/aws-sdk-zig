...
pub const KubernetesNetworkConfigRequest = struct {
    ...
    /// family when you create a cluster and can't change this value once the
    /// cluster is
    ...
    /// services](https://docs.aws.amazon.com/eks/latest/userguide/cni-ipv6.html) in
    /// the *Amazon EKS User Guide*. Kubernetes assigns services `IPv6`
    ...
    /// that you specify a block that does not overlap with resources in other
    /// networks that are
    /// peered or connected to your VPC. The block must meet the following
    /// requirements:
    ///
