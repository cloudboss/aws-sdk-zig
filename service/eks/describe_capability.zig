...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCapabilityInput, options: Options) !DescribeCapabilityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCapabilityInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCapabilityOutput {
    var result: DescribeCapabilityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCapabilityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
