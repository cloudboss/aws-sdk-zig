...
const ConnectorConfigRequest = @import("connector_config_request.zig").ConnectorConfigRequest;
const Cluster = @import("cluster.zig").Cluster;

pub const RegisterClusterInput = struct {
...
pub const RegisterClusterOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterClusterInput, options: Options) !RegisterClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/cluster-registrations";
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"connectorConfig\":");
    try aws.json.writeValue(@TypeOf(input.connector_config), input.connector_config, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterClusterOutput {
    var result: RegisterClusterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterClusterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
