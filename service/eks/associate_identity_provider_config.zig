...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateIdentityProviderConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/identity-provider-configs/associate");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"oidc\":");
    try aws.json.writeValue(@TypeOf(input.oidc), input.oidc, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateIdentityProviderConfigOutput {
    var result: AssociateIdentityProviderConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateIdentityProviderConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
