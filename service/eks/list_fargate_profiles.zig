...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFargateProfilesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFargateProfilesOutput {
    var result: ListFargateProfilesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFargateProfilesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
