...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEksAnywhereSubscriptionsInput, options: Options) !ListEksAnywhereSubscriptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEksAnywhereSubscriptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_status) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeStatus=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEksAnywhereSubscriptionsOutput {
    var result: ListEksAnywhereSubscriptionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEksAnywhereSubscriptionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
