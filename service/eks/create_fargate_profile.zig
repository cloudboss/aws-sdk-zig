...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFargateProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"fargateProfileName\":");
    try aws.json.writeValue(@TypeOf(input.fargate_profile_name), input.fargate_profile_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"podExecutionRoleArn\":");
    try aws.json.writeValue(@TypeOf(input.pod_execution_role_arn), input.pod_execution_role_arn, alloc, &body_buf);
    has_prev = true;
    if (input.selectors) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"selectors\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.subnets) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFargateProfileOutput {
    var result: CreateFargateProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFargateProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
