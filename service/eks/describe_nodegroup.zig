...
pub const DescribeNodegroupInput = struct {
...
pub const DescribeNodegroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNodegroupInput, options: Options) !DescribeNodegroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNodegroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNodegroupOutput {
    var result: DescribeNodegroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeNodegroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
