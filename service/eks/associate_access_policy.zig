...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateAccessPolicyInput, options: Options) !AssociateAccessPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateAccessPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"accessScope\":");
    try aws.json.writeValue(@TypeOf(input.access_scope), input.access_scope, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateAccessPolicyOutput {
    var result: AssociateAccessPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateAccessPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
