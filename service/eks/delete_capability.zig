...
pub const DeleteCapabilityInput = struct {
...
pub const DeleteCapabilityOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCapabilityInput, options: CallOptions) !DeleteCapabilityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCapabilityInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCapabilityOutput {
    var result: DeleteCapabilityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteCapabilityOutput, body, allocator);
    }
    _ = status;
    _ = headers;
