...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCapabilityInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCapabilityOutput {
    var result: DeleteCapabilityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteCapabilityOutput, body, alloc);
    }
    _ = status;
    _ = headers;
