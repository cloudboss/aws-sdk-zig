...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAddonInput, options: Options) !UpdateAddonOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAddonInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAddonOutput {
    var result: UpdateAddonOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAddonOutput, body, alloc);
    }
    _ = status;
    _ = headers;
