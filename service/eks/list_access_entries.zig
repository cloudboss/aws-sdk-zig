...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessEntriesInput, options: Options) !ListAccessEntriesOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAccessEntriesOutput {
    var result: ListAccessEntriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccessEntriesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
