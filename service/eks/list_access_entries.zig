...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessEntriesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.associated_policy_arn) |v| {
        ...
        try query_buf.appendSlice(alloc, "associatedPolicyArn=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessEntriesOutput {
    var result: ListAccessEntriesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccessEntriesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
