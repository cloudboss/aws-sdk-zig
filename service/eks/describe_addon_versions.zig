...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAddonVersionsInput, options: CallOptions) !DescribeAddonVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAddonVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.kubernetes_version) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "kubernetesVersion=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.owners) |v| {
        for (v) |item| {
            if (query_has_prev) try query_buf.appendSlice(allocator, "&");
            try query_buf.appendSlice(allocator, "owners=");
    ...
    if (input.publishers) |v| {
        for (v) |item| {
            if (query_has_prev) try query_buf.appendSlice(allocator, "&");
            try query_buf.appendSlice(allocator, "publishers=");
            try aws.url.appendUrlEncoded(allocator, &query_buf, item);
            query_has_prev = true;
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAddonVersionsOutput {
    var result: DescribeAddonVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAddonVersionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
