...
const AddonInfo = @import("addon_info.zig").AddonInfo;

pub const DescribeAddonVersionsInput = struct {
    ...
    /// for this property.
    publishers: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAddonVersionsInput, options: Options) !DescribeAddonVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAddonVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/addons/supported-versions";
    ...
    if (input.kubernetes_version) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "kubernetesVersion=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    ...
    if (input.owners) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "owners=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.publishers) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "publishers=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.types) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "types=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAddonVersionsOutput {
    var result: DescribeAddonVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAddonVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
