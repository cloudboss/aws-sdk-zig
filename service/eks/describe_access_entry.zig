...
pub const DescribeAccessEntryInput = struct {
...
pub const DescribeAccessEntryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccessEntryInput, options: CallOptions) !DescribeAccessEntryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAccessEntryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAccessEntryOutput {
    var result: DescribeAccessEntryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccessEntryOutput, body, allocator);
    }
    _ = status;
    _ = headers;
