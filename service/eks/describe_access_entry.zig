...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccessEntryInput, options: Options) !DescribeAccessEntryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccessEntryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.principal_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccessEntryOutput {
    var result: DescribeAccessEntryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccessEntryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
