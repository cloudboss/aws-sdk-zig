...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePodIdentityAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePodIdentityAssociationOutput {
    var result: DescribePodIdentityAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePodIdentityAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
