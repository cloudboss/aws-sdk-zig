...
fn serializeRequest(alloc: std.mem.Allocator, input: ListNodegroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListNodegroupsOutput {
    var result: ListNodegroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListNodegroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
