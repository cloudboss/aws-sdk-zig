...
pub const ListNodegroupsInput = struct {
...
pub const ListNodegroupsOutput = struct {
    /// The `nextToken` value returned from a previous paginated request, where
    /// `maxResults` was used and
    /// the results exceeded the value of that parameter. Pagination continues from
    /// the end of
    ...
    nodegroups: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListNodegroupsInput, options: CallOptions) !ListNodegroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListNodegroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListNodegroupsOutput {
    var result: ListNodegroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListNodegroupsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
