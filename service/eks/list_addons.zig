...
pub const ListAddonsInput = struct {
...
pub const ListAddonsOutput = struct {
    /// A list of installed add-ons.
    addons: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAddonsInput, options: Options) !ListAddonsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAddonsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAddonsOutput {
    var result: ListAddonsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAddonsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
