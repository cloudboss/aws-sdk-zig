...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAddonInput, options: Options) !DescribeAddonOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAddonInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAddonOutput {
    var result: DescribeAddonOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAddonOutput, body, alloc);
    }
    _ = status;
    _ = headers;
