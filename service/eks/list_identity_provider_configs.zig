...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdentityProviderConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/identity-provider-configs");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdentityProviderConfigsOutput {
    var result: ListIdentityProviderConfigsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIdentityProviderConfigsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
