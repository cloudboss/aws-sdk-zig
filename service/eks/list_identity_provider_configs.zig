...
pub const ListIdentityProviderConfigsInput = struct {
...
pub const ListIdentityProviderConfigsOutput = struct {
    /// The identity provider configurations for the cluster.
    identity_provider_configs: ?[]const IdentityProviderConfig = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIdentityProviderConfigsInput, options: CallOptions) !ListIdentityProviderConfigsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListIdentityProviderConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/identity-provider-configs");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListIdentityProviderConfigsOutput {
    var result: ListIdentityProviderConfigsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIdentityProviderConfigsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
