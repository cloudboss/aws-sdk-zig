...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateClusterConfigInput, options: Options) !UpdateClusterConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateClusterConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/update-config");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateClusterConfigOutput {
    var result: UpdateClusterConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateClusterConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
