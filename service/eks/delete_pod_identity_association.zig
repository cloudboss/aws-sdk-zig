...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePodIdentityAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePodIdentityAssociationOutput {
    var result: DeletePodIdentityAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePodIdentityAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
