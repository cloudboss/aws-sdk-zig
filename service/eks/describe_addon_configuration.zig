...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAddonConfigurationInput, options: Options) !DescribeAddonConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAddonConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "addonName=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.addon_name);
    query_has_prev = true;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "addonVersion=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.addon_version);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAddonConfigurationOutput {
    var result: DescribeAddonConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAddonConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
