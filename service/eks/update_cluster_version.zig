...
pub const UpdateClusterVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateClusterVersionInput, options: Options) !UpdateClusterVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateClusterVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateClusterVersionOutput {
    var result: UpdateClusterVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateClusterVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
