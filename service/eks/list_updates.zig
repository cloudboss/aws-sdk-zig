...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUpdatesInput, options: Options) !ListUpdatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUpdatesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUpdatesOutput {
    var result: ListUpdatesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUpdatesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
