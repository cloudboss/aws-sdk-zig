...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAddonInput, options: Options) !DeleteAddonOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAddonInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.addon_name);
    ...
    if (input.preserve) |v| {
        ...
        try query_buf.appendSlice(alloc, "preserve=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAddonOutput {
    var result: DeleteAddonOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAddonOutput, body, alloc);
    }
    _ = status;
    _ = headers;
