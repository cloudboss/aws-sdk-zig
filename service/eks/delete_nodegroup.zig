...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteNodegroupInput, options: Options) !DeleteNodegroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteNodegroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteNodegroupOutput {
    var result: DeleteNodegroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteNodegroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
