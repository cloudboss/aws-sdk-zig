...
pub const DescribeInsightsRefreshInput = struct {
...
pub const DescribeInsightsRefreshOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInsightsRefreshInput, options: Options) !DescribeInsightsRefreshOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInsightsRefreshInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/insights-refresh");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInsightsRefreshOutput {
    var result: DescribeInsightsRefreshOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeInsightsRefreshOutput, body, alloc);
    }
    _ = status;
    _ = headers;
