...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPodIdentityAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.service_account) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "serviceAccount=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPodIdentityAssociationsOutput {
    var result: ListPodIdentityAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPodIdentityAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
