...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUpdateInput, options: Options) !DescribeUpdateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUpdateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/updates/");
    try path_buf.appendSlice(alloc, input.update_id);
    ...
    if (input.capability_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "capabilityName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.nodegroup_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "nodegroupName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUpdateOutput {
    var result: DescribeUpdateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeUpdateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
