...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartInsightsRefreshInput, options: Options) !StartInsightsRefreshOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartInsightsRefreshInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartInsightsRefreshOutput {
    var result: StartInsightsRefreshOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartInsightsRefreshOutput, body, alloc);
    }
    _ = status;
    _ = headers;
