...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNodegroupConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNodegroupConfigOutput {
    var result: UpdateNodegroupConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateNodegroupConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
