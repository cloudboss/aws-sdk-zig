...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateEncryptionConfigInput, options: Options) !AssociateEncryptionConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateEncryptionConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/encryption-config/associate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateEncryptionConfigOutput {
    var result: AssociateEncryptionConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AssociateEncryptionConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
