...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMonitoringScheduleInput, options: Options) !CreateMonitoringScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMonitoringScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateMonitoringSchedule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMonitoringScheduleOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateMonitoringScheduleOutput, body, alloc);
}
