...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFeatureGroupsInput, options: Options) !ListFeatureGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFeatureGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListFeatureGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFeatureGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFeatureGroupsOutput, body, alloc);
}
