...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateComputeQuotaInput, options: Options) !CreateComputeQuotaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateComputeQuotaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateComputeQuota");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateComputeQuotaOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateComputeQuotaOutput, body, alloc);
}
