...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListClusterEventsInput, options: Options) !ListClusterEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListClusterEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListClusterEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListClusterEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListClusterEventsOutput, body, alloc);
}
