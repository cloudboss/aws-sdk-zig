...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelCardsInput, options: Options) !ListModelCardsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelCardsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListModelCards");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelCardsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListModelCardsOutput, body, alloc);
}
