...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLineageGroupsInput, options: Options) !ListLineageGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLineageGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListLineageGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLineageGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLineageGroupsOutput, body, alloc);
}
