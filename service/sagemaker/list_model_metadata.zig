...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelMetadataInput, options: Options) !ListModelMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListModelMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelMetadataOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListModelMetadataOutput, body, alloc);
}
