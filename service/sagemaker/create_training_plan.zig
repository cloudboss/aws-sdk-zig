...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrainingPlanInput, options: Options) !CreateTrainingPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrainingPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateTrainingPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrainingPlanOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateTrainingPlanOutput, body, alloc);
}
