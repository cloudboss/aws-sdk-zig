...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddAssociationInput, options: Options) !AddAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.AddAssociation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddAssociationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AddAssociationOutput, body, alloc);
}
