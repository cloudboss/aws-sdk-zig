...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEdgeDeploymentPlanInput, options: Options) !DescribeEdgeDeploymentPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEdgeDeploymentPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeEdgeDeploymentPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEdgeDeploymentPlanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEdgeDeploymentPlanOutput, body, alloc);
}
