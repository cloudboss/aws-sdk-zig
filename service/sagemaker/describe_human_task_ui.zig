...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHumanTaskUiInput, options: Options) !DescribeHumanTaskUiOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHumanTaskUiInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeHumanTaskUi");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHumanTaskUiOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeHumanTaskUiOutput, body, alloc);
}
