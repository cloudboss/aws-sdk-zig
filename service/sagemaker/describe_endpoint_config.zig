...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEndpointConfigInput, options: Options) !DescribeEndpointConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEndpointConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeEndpointConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEndpointConfigOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeEndpointConfigOutput, body, alloc);
}
