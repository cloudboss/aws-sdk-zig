...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePipelineExecutionInput, options: Options) !UpdatePipelineExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePipelineExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdatePipelineExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePipelineExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdatePipelineExecutionOutput, body, alloc);
}
