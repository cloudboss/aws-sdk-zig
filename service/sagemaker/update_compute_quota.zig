...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateComputeQuotaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateComputeQuota");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateComputeQuotaOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateComputeQuotaOutput, body, alloc);
}
