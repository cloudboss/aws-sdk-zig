...
pub const UpdateComputeQuotaInput = struct {
...
pub const UpdateComputeQuotaOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateComputeQuotaInput, options: CallOptions) !UpdateComputeQuotaOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateComputeQuotaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.UpdateComputeQuota");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateComputeQuotaOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateComputeQuotaOutput, body, allocator);
}
