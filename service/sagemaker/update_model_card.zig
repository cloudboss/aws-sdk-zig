...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateModelCardInput, options: Options) !UpdateModelCardOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateModelCardInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateModelCard");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateModelCardOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateModelCardOutput, body, alloc);
}
