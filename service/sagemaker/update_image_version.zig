...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateImageVersionInput, options: Options) !UpdateImageVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateImageVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateImageVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateImageVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateImageVersionOutput, body, alloc);
}
