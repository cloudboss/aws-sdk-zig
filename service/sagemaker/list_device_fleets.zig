...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDeviceFleetsInput, options: Options) !ListDeviceFleetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDeviceFleetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListDeviceFleets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDeviceFleetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDeviceFleetsOutput, body, alloc);
}
