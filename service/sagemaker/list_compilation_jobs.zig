...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCompilationJobsInput, options: Options) !ListCompilationJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCompilationJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListCompilationJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCompilationJobsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListCompilationJobsOutput, body, alloc);
}
