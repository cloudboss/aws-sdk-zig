...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrainingJobsForHyperParameterTuningJobInput, options: Options) !ListTrainingJobsForHyperParameterTuningJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrainingJobsForHyperParameterTuningJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListTrainingJobsForHyperParameterTuningJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrainingJobsForHyperParameterTuningJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTrainingJobsForHyperParameterTuningJobOutput, body, alloc);
}
