...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePartnerAppInput, options: CallOptions) !DescribePartnerAppOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribePartnerAppInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.DescribePartnerApp");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribePartnerAppOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePartnerAppOutput, body, allocator);
}
