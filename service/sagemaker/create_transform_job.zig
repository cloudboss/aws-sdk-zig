...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTransformJobInput, options: Options) !CreateTransformJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTransformJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateTransformJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTransformJobOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateTransformJobOutput, body, alloc);
}
