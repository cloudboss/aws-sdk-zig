...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteHubContentReferenceInput, options: Options) !DeleteHubContentReferenceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteHubContentReferenceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DeleteHubContentReference");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteHubContentReferenceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
