...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReservedCapacityInput, options: CallOptions) !DescribeReservedCapacityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReservedCapacityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.DescribeReservedCapacity");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReservedCapacityOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeReservedCapacityOutput, body, allocator);
}
