...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeComputeQuotaInput, options: Options) !DescribeComputeQuotaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeComputeQuotaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeComputeQuota");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeComputeQuotaOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeComputeQuotaOutput, body, alloc);
}
