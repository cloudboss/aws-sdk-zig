...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeProject");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProjectOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeProjectOutput, body, alloc);
}
