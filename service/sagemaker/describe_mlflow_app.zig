...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMlflowAppInput, options: Options) !DescribeMlflowAppOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMlflowAppInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeMlflowApp");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMlflowAppOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMlflowAppOutput, body, alloc);
}
