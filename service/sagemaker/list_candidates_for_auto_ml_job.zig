...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCandidatesForAutoMLJobInput, options: Options) !ListCandidatesForAutoMLJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCandidatesForAutoMLJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListCandidatesForAutoMLJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCandidatesForAutoMLJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCandidatesForAutoMLJobOutput, body, alloc);
}
