...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppImageConfigsInput, options: Options) !ListAppImageConfigsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppImageConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListAppImageConfigs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppImageConfigsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAppImageConfigsOutput, body, alloc);
}
