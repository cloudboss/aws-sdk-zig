...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHubContentReferenceInput, options: Options) !CreateHubContentReferenceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHubContentReferenceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateHubContentReference");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHubContentReferenceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateHubContentReferenceOutput, body, alloc);
}
