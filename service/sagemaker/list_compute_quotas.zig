...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListComputeQuotasInput, options: Options) !ListComputeQuotasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListComputeQuotasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListComputeQuotas");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListComputeQuotasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListComputeQuotasOutput, body, alloc);
}
