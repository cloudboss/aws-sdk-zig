...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateModelPackageInput, options: CallOptions) !CreateModelPackageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateModelPackageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.CreateModelPackage");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateModelPackageOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateModelPackageOutput, body, allocator);
}
