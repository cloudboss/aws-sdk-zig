...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateClusterSoftwareInput, options: Options) !UpdateClusterSoftwareOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateClusterSoftwareInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateClusterSoftware");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateClusterSoftwareOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateClusterSoftwareOutput, body, alloc);
}
