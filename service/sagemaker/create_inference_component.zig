...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInferenceComponentInput, options: Options) !CreateInferenceComponentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInferenceComponentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateInferenceComponent");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInferenceComponentOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateInferenceComponentOutput, body, alloc);
}
