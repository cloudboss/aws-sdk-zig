...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLabelingJobsForWorkteamInput, options: Options) !ListLabelingJobsForWorkteamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLabelingJobsForWorkteamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListLabelingJobsForWorkteam");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLabelingJobsForWorkteamOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListLabelingJobsForWorkteamOutput, body, alloc);
}
