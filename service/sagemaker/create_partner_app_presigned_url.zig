...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePartnerAppPresignedUrlInput, options: Options) !CreatePartnerAppPresignedUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePartnerAppPresignedUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreatePartnerAppPresignedUrl");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePartnerAppPresignedUrlOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePartnerAppPresignedUrlOutput, body, alloc);
}
