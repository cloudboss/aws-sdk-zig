...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFeatureMetadataInput, options: Options) !UpdateFeatureMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFeatureMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateFeatureMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFeatureMetadataOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
