...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDescribeModelPackageInput, options: Options) !BatchDescribeModelPackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDescribeModelPackageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.BatchDescribeModelPackage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDescribeModelPackageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDescribeModelPackageOutput, body, alloc);
}
