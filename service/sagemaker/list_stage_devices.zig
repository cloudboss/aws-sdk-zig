...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStageDevicesInput, options: Options) !ListStageDevicesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStageDevicesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListStageDevices");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStageDevicesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListStageDevicesOutput, body, alloc);
}
