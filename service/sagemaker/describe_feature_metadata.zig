...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFeatureMetadataInput, options: Options) !DescribeFeatureMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFeatureMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeFeatureMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFeatureMetadataOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeFeatureMetadataOutput, body, alloc);
}
