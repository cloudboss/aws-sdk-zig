...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEdgePackagingJobInput, options: Options) !CreateEdgePackagingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEdgePackagingJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateEdgePackagingJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEdgePackagingJobOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
