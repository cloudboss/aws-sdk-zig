...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClusterSchedulerConfigInput, options: Options) !DescribeClusterSchedulerConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClusterSchedulerConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeClusterSchedulerConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClusterSchedulerConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeClusterSchedulerConfigOutput, body, alloc);
}
