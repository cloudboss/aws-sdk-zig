...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageVersionInput, options: Options) !DescribeImageVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeImageVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeImageVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeImageVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeImageVersionOutput, body, alloc);
}
