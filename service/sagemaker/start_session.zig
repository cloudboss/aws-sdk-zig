...
pub const StartSessionInput = struct {
    ...
    /// connection will be established. For example, this identifies the specific
    /// ARN space application you want to connect to from your local IDE.
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.StartSession");
