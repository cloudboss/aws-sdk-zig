...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHyperParameterTuningJobsInput, options: CallOptions) !ListHyperParameterTuningJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListHyperParameterTuningJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.ListHyperParameterTuningJobs");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListHyperParameterTuningJobsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListHyperParameterTuningJobsOutput, body, allocator);
}
