...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMlflowTrackingServersInput, options: Options) !ListMlflowTrackingServersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMlflowTrackingServersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListMlflowTrackingServers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMlflowTrackingServersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMlflowTrackingServersOutput, body, alloc);
}
