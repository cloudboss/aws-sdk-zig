...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOptimizationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeOptimizationJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOptimizationJobOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeOptimizationJobOutput, body, alloc);
}
