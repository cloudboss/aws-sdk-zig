...
const ClusterNodeSummary = @import("cluster_node_summary.zig").ClusterNodeSummary;

pub const ListClusterNodesInput = struct {
    ...
    /// formats for `CreationTimeAfter`. For more information about the timestamp
    /// format, see
    /// [Timestamp](https://docs.aws.amazon.com/cli/latest/userguide/cli-usage-parameters-types.html#parameter-type-timestamp) in the *Amazon Web Services Command Line Interface User Guide*.
    creation_time_before: ?i64 = null,

    /// Specifies whether to include nodes that are still being provisioned in the
    /// response. When set to true, the response includes all nodes regardless of
    /// their provisioning status. When set to `False` (default), only nodes with
    /// assigned `InstanceIds` are returned.
    include_node_logical_ids: ?bool = null,

    /// A filter that returns the instance groups whose name contain a specified
    /// string.
    instance_group_name_contains: ?[]const u8 = null,
    ...
    pub const json_field_names = .{
        ...
        .include_node_logical_ids = "IncludeNodeLogicalIds",
        .instance_group_name_contains = "InstanceGroupNameContains",
...
pub const ListClusterNodesOutput = struct {
    /// The summaries of listed instances in a SageMaker HyperPod cluster
    cluster_node_summaries: ?[]const ClusterNodeSummary = null,
    ...
    pub const json_field_names = .{
        .cluster_node_summaries = "ClusterNodeSummaries",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListClusterNodesInput, options: CallOptions) !ListClusterNodesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListClusterNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.ListClusterNodes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListClusterNodesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListClusterNodesOutput, body, allocator);
}
