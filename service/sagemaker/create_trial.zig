...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrialInput, options: Options) !CreateTrialOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrialInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateTrial");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrialOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTrialOutput, body, alloc);
}
