...
pub const CreateModelCardInput = struct {
...
pub const CreateModelCardOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateModelCardInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.CreateModelCard");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateModelCardOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateModelCardOutput, body, allocator);
}
