...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateModelCardInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateModelCard");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateModelCardOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateModelCardOutput, body, alloc);
}
