...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNotebookInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateNotebookInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNotebookInstanceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
