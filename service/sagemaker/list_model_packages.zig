...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelPackagesInput, options: Options) !ListModelPackagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelPackagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListModelPackages");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelPackagesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListModelPackagesOutput, body, alloc);
}
