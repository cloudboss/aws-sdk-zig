...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFeatureGroupInput, options: CallOptions) !DescribeFeatureGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFeatureGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.DescribeFeatureGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFeatureGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeFeatureGroupOutput, body, allocator);
}
