...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataQualityJobDefinitionsInput, options: Options) !ListDataQualityJobDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataQualityJobDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListDataQualityJobDefinitions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataQualityJobDefinitionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListDataQualityJobDefinitionsOutput, body, alloc);
}
