...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateActionInput, options: Options) !CreateActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateAction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateActionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateActionOutput, body, alloc);
}
