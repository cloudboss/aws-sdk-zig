...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUserProfilesInput, options: Options) !ListUserProfilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUserProfilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListUserProfiles");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUserProfilesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUserProfilesOutput, body, alloc);
}
