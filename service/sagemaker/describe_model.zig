...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeModel");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeModelOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeModelOutput, body, alloc);
}
