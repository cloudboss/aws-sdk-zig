...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFeatureGroupInput, options: Options) !UpdateFeatureGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFeatureGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateFeatureGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFeatureGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateFeatureGroupOutput, body, alloc);
}
