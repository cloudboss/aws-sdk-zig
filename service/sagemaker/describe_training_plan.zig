...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrainingPlanInput, options: CallOptions) !DescribeTrainingPlanOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTrainingPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.DescribeTrainingPlan");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTrainingPlanOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeTrainingPlanOutput, body, allocator);
}
