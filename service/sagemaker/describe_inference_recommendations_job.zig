...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInferenceRecommendationsJobInput, options: Options) !DescribeInferenceRecommendationsJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeInferenceRecommendationsJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeInferenceRecommendationsJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeInferenceRecommendationsJobOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeInferenceRecommendationsJobOutput, body, alloc);
}
