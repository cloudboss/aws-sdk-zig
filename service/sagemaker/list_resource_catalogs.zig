...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceCatalogsInput, options: Options) !ListResourceCatalogsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceCatalogsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListResourceCatalogs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceCatalogsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceCatalogsOutput, body, alloc);
}
