...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHumanTaskUiInput, options: Options) !CreateHumanTaskUiOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHumanTaskUiInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateHumanTaskUi");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHumanTaskUiOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateHumanTaskUiOutput, body, alloc);
}
