...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHubContentPresignedUrlsInput, options: Options) !CreateHubContentPresignedUrlsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHubContentPresignedUrlsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateHubContentPresignedUrls");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHubContentPresignedUrlsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateHubContentPresignedUrlsOutput, body, alloc);
}
