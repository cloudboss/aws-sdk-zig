...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUltraServersByReservedCapacityInput, options: Options) !ListUltraServersByReservedCapacityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUltraServersByReservedCapacityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListUltraServersByReservedCapacity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUltraServersByReservedCapacityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListUltraServersByReservedCapacityOutput, body, alloc);
}
