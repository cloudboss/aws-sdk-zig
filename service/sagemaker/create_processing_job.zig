...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProcessingJobInput, options: Options) !CreateProcessingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProcessingJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateProcessingJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProcessingJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateProcessingJobOutput, body, alloc);
}
