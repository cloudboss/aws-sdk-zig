...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEdgeDeploymentPlanInput, options: Options) !CreateEdgeDeploymentPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEdgeDeploymentPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateEdgeDeploymentPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEdgeDeploymentPlanOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateEdgeDeploymentPlanOutput, body, alloc);
}
