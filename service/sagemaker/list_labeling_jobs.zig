...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLabelingJobsInput, options: Options) !ListLabelingJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLabelingJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListLabelingJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLabelingJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLabelingJobsOutput, body, alloc);
}
