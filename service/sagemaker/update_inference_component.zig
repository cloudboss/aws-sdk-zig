...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateInferenceComponentInput, options: Options) !UpdateInferenceComponentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateInferenceComponentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateInferenceComponent");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateInferenceComponentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateInferenceComponentOutput, body, alloc);
}
