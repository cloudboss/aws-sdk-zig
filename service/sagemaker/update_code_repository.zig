...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCodeRepositoryInput, options: Options) !UpdateCodeRepositoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCodeRepositoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateCodeRepository");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCodeRepositoryOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateCodeRepositoryOutput, body, alloc);
}
