...
pub const CreateNotebookInstanceLifecycleConfigInput = struct {
...
pub const CreateNotebookInstanceLifecycleConfigOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateNotebookInstanceLifecycleConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.CreateNotebookInstanceLifecycleConfig");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateNotebookInstanceLifecycleConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateNotebookInstanceLifecycleConfigOutput, body, allocator);
}
