...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeModelCardExportJobInput, options: Options) !DescribeModelCardExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeModelCardExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeModelCardExportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeModelCardExportJobOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeModelCardExportJobOutput, body, alloc);
}
