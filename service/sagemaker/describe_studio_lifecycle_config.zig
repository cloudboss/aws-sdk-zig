...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStudioLifecycleConfigInput, options: Options) !DescribeStudioLifecycleConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStudioLifecycleConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeStudioLifecycleConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStudioLifecycleConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeStudioLifecycleConfigOutput, body, alloc);
}
