...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeModelExplainabilityJobDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeModelExplainabilityJobDefinition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeModelExplainabilityJobDefinitionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeModelExplainabilityJobDefinitionOutput, body, alloc);
}
