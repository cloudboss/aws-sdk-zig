...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContextsInput, options: Options) !ListContextsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContextsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListContexts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContextsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListContextsOutput, body, alloc);
}
