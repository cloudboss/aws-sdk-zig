...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFeatureGroupInput, options: CallOptions) !CreateFeatureGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateFeatureGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.CreateFeatureGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateFeatureGroupOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateFeatureGroupOutput, body, allocator);
}
