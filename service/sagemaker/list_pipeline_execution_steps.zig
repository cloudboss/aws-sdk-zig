...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPipelineExecutionStepsInput, options: Options) !ListPipelineExecutionStepsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPipelineExecutionStepsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListPipelineExecutionSteps");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPipelineExecutionStepsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPipelineExecutionStepsOutput, body, alloc);
}
