...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHubContentInput, options: Options) !DescribeHubContentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHubContentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeHubContent");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHubContentOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeHubContentOutput, body, alloc);
}
