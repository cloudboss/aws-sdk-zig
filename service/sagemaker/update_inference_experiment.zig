...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateInferenceExperimentInput, options: Options) !UpdateInferenceExperimentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateInferenceExperimentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateInferenceExperiment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateInferenceExperimentOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateInferenceExperimentOutput, body, alloc);
}
