...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkteamsInput, options: Options) !ListWorkteamsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkteamsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListWorkteams");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkteamsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListWorkteamsOutput, body, alloc);
}
