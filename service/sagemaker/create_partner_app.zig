...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePartnerAppInput, options: Options) !CreatePartnerAppOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePartnerAppInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreatePartnerApp");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePartnerAppOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePartnerAppOutput, body, alloc);
}
