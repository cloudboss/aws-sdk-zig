...
pub const DescribeWorkforceInput = struct {
...
pub const DescribeWorkforceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkforceInput, options: CallOptions) !DescribeWorkforceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeWorkforceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.DescribeWorkforce");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeWorkforceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeWorkforceOutput, body, allocator);
}
