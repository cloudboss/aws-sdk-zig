...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkforceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeWorkforce");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkforceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeWorkforceOutput, body, alloc);
}
