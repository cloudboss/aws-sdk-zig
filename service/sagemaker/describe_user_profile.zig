...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUserProfileInput, options: Options) !DescribeUserProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUserProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeUserProfile");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUserProfileOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeUserProfileOutput, body, alloc);
}
