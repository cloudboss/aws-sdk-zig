...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOptimizationJobsInput, options: Options) !ListOptimizationJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOptimizationJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListOptimizationJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOptimizationJobsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListOptimizationJobsOutput, body, alloc);
}
