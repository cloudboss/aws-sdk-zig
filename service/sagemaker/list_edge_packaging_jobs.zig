...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEdgePackagingJobsInput, options: Options) !ListEdgePackagingJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEdgePackagingJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListEdgePackagingJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEdgePackagingJobsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListEdgePackagingJobsOutput, body, alloc);
}
