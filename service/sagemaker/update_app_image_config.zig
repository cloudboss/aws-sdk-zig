...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAppImageConfigInput, options: Options) !UpdateAppImageConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAppImageConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateAppImageConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAppImageConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateAppImageConfigOutput, body, alloc);
}
