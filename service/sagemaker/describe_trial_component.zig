...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrialComponentInput, options: Options) !DescribeTrialComponentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrialComponentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeTrialComponent");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrialComponentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTrialComponentOutput, body, alloc);
}
