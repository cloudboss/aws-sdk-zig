...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelPackageGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListModelPackageGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelPackageGroupsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListModelPackageGroupsOutput, body, alloc);
}
