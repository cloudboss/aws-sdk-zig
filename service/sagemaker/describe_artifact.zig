...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeArtifactInput, options: Options) !DescribeArtifactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeArtifact");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeArtifactOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeArtifactOutput, body, alloc);
}
