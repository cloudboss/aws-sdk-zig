...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTrainingJobInput, options: Options) !UpdateTrainingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTrainingJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateTrainingJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTrainingJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateTrainingJobOutput, body, alloc);
}
