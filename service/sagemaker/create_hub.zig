...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHubInput, options: Options) !CreateHubOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHubInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateHub");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHubOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateHubOutput, body, alloc);
}
