...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RetryPipelineExecutionInput, options: Options) !RetryPipelineExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RetryPipelineExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.RetryPipelineExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RetryPipelineExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RetryPipelineExecutionOutput, body, alloc);
}
