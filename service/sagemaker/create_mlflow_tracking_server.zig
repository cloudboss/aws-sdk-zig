...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMlflowTrackingServerInput, options: Options) !CreateMlflowTrackingServerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMlflowTrackingServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateMlflowTrackingServer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMlflowTrackingServerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateMlflowTrackingServerOutput, body, alloc);
}
