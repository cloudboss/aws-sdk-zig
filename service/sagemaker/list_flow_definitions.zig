...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFlowDefinitionsInput, options: Options) !ListFlowDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFlowDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListFlowDefinitions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFlowDefinitionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListFlowDefinitionsOutput, body, alloc);
}
