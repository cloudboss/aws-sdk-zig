...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHyperParameterTuningJobInput, options: Options) !CreateHyperParameterTuningJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHyperParameterTuningJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateHyperParameterTuningJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHyperParameterTuningJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateHyperParameterTuningJobOutput, body, alloc);
}
