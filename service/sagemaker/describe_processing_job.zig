...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProcessingJobInput, options: Options) !DescribeProcessingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeProcessingJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeProcessingJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeProcessingJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProcessingJobOutput, body, alloc);
}
