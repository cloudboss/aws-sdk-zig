...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDeviceFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateDeviceFleet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDeviceFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
