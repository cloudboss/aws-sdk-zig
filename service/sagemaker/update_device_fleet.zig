...
pub const UpdateDeviceFleetInput = struct {
...
pub const UpdateDeviceFleetOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDeviceFleetInput, options: CallOptions) !UpdateDeviceFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDeviceFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.UpdateDeviceFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDeviceFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
