...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteClusterNodesInput, options: Options) !BatchDeleteClusterNodesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteClusterNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.BatchDeleteClusterNodes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteClusterNodesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchDeleteClusterNodesOutput, body, alloc);
}
