...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopInferenceExperimentInput, options: Options) !StopInferenceExperimentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopInferenceExperimentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.StopInferenceExperiment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopInferenceExperimentOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(StopInferenceExperimentOutput, body, alloc);
}
