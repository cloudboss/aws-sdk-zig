...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProcessingJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListProcessingJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProcessingJobsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListProcessingJobsOutput, body, alloc);
}
