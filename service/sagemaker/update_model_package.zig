...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateModelPackageInput, options: Options) !UpdateModelPackageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateModelPackageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateModelPackage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateModelPackageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateModelPackageOutput, body, alloc);
}
