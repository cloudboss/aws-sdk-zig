...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateNotebookInstanceInput, options: Options) !CreateNotebookInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNotebookInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateNotebookInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateNotebookInstanceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateNotebookInstanceOutput, body, alloc);
}
