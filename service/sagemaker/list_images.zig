...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImagesInput, options: Options) !ListImagesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImagesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListImages");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImagesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListImagesOutput, body, alloc);
}
