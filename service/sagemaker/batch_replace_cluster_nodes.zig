...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchReplaceClusterNodesInput, options: CallOptions) !BatchReplaceClusterNodesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchReplaceClusterNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.BatchReplaceClusterNodes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchReplaceClusterNodesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchReplaceClusterNodesOutput, body, allocator);
}
