...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListModelBiasJobDefinitionsInput, options: Options) !ListModelBiasJobDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListModelBiasJobDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListModelBiasJobDefinitions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListModelBiasJobDefinitionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListModelBiasJobDefinitionsOutput, body, alloc);
}
