...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInferenceRecommendationsJobsInput, options: Options) !ListInferenceRecommendationsJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInferenceRecommendationsJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListInferenceRecommendationsJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInferenceRecommendationsJobsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListInferenceRecommendationsJobsOutput, body, alloc);
}
