...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPartnerAppsInput, options: Options) !ListPartnerAppsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPartnerAppsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListPartnerApps");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPartnerAppsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPartnerAppsOutput, body, alloc);
}
