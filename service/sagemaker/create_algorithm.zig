...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAlgorithmInput, options: Options) !CreateAlgorithmOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAlgorithmInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateAlgorithm");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAlgorithmOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateAlgorithmOutput, body, alloc);
}
