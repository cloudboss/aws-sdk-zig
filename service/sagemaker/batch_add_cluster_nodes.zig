...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchAddClusterNodesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.BatchAddClusterNodes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchAddClusterNodesOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchAddClusterNodesOutput, body, allocator);
}
