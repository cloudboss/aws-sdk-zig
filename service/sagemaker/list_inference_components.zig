...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInferenceComponentsInput, options: Options) !ListInferenceComponentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInferenceComponentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListInferenceComponents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInferenceComponentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInferenceComponentsOutput, body, alloc);
}
