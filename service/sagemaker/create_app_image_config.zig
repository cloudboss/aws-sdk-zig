...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAppImageConfigInput, options: Options) !CreateAppImageConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAppImageConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateAppImageConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAppImageConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAppImageConfigOutput, body, alloc);
}
