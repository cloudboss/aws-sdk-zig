...
/// baseline](https://docs.aws.amazon.com/sagemaker/latest/dg/clarify-feature-attribute-shap-baselines.html) (also called the background or reference dataset) of the Kernal SHAP algorithm.
///
...
///   requests. For more information, see the **Synthetic data** of [Configure
///   and create an
...
pub const ClarifyShapBaselineConfig = struct {
    ...
    /// four features, and all features are numerical, then the format of the
    /// baseline data should also share these characteristics. For natural language
    /// processing (NLP) of text columns, the baseline value should be the value
    /// used to replace the unit of text specified by the `Granularity` of the
