...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAlgorithmInput, options: Options) !DescribeAlgorithmOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAlgorithmInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeAlgorithm");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAlgorithmOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAlgorithmOutput, body, alloc);
}
