...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEndpointConfigsInput, options: Options) !ListEndpointConfigsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEndpointConfigsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListEndpointConfigs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEndpointConfigsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEndpointConfigsOutput, body, alloc);
}
