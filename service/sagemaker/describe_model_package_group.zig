...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeModelPackageGroupInput, options: Options) !DescribeModelPackageGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeModelPackageGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeModelPackageGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeModelPackageGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeModelPackageGroupOutput, body, alloc);
}
