...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMlflowAppInput, options: Options) !CreateMlflowAppOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMlflowAppInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateMlflowApp");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMlflowAppOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateMlflowAppOutput, body, alloc);
}
