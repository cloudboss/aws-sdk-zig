...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPipelineVersionsInput, options: Options) !ListPipelineVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPipelineVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListPipelineVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPipelineVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPipelineVersionsOutput, body, alloc);
}
