...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInferenceExperimentInput, options: Options) !CreateInferenceExperimentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInferenceExperimentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateInferenceExperiment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInferenceExperimentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateInferenceExperimentOutput, body, alloc);
}
