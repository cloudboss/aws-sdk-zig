...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWorkteamInput, options: Options) !UpdateWorkteamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWorkteamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateWorkteam");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWorkteamOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateWorkteamOutput, body, alloc);
}
