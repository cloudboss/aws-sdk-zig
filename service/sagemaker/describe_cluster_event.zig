...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeClusterEventInput, options: Options) !DescribeClusterEventOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeClusterEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeClusterEvent");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeClusterEventOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeClusterEventOutput, body, alloc);
}
