...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchTrainingPlanOfferingsInput, options: Options) !SearchTrainingPlanOfferingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchTrainingPlanOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.SearchTrainingPlanOfferings");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchTrainingPlanOfferingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchTrainingPlanOfferingsOutput, body, alloc);
}
