...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrainingJobInput, options: Options) !CreateTrainingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrainingJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateTrainingJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrainingJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateTrainingJobOutput, body, alloc);
}
