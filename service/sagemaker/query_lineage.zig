...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: QueryLineageInput, options: Options) !QueryLineageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: QueryLineageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.QueryLineage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !QueryLineageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(QueryLineageOutput, body, alloc);
}
