...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAutoMLJobV2Input, options: CallOptions) !CreateAutoMLJobV2Output {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAutoMLJobV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "SageMaker.CreateAutoMLJobV2");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAutoMLJobV2Output {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateAutoMLJobV2Output, body, allocator);
}
