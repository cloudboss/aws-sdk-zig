...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateArtifactInput, options: Options) !CreateArtifactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateArtifact");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateArtifactOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateArtifactOutput, body, alloc);
}
