...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDataQualityJobDefinitionInput, options: Options) !DescribeDataQualityJobDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataQualityJobDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeDataQualityJobDefinition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataQualityJobDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDataQualityJobDefinitionOutput, body, alloc);
}
