...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAppImageConfigInput, options: Options) !DescribeAppImageConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAppImageConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeAppImageConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAppImageConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAppImageConfigOutput, body, alloc);
}
