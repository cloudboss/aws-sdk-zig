...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrialsInput, options: Options) !ListTrialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListTrials");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrialsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTrialsOutput, body, alloc);
}
