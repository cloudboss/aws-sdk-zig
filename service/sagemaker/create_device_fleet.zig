...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDeviceFleetInput, options: Options) !CreateDeviceFleetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeviceFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateDeviceFleet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDeviceFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
