...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLabelingJobInput, options: Options) !CreateLabelingJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLabelingJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateLabelingJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLabelingJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLabelingJobOutput, body, alloc);
}
