...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMonitoringAlertHistoryInput, options: Options) !ListMonitoringAlertHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMonitoringAlertHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.ListMonitoringAlertHistory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMonitoringAlertHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMonitoringAlertHistoryOutput, body, alloc);
}
