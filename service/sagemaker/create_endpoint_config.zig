...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEndpointConfigInput, options: Options) !CreateEndpointConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEndpointConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateEndpointConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEndpointConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEndpointConfigOutput, body, alloc);
}
