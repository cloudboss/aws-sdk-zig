...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartPipelineExecutionInput, options: Options) !StartPipelineExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartPipelineExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.StartPipelineExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartPipelineExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartPipelineExecutionOutput, body, alloc);
}
