...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateStudioLifecycleConfigInput, options: Options) !CreateStudioLifecycleConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateStudioLifecycleConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateStudioLifecycleConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateStudioLifecycleConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateStudioLifecycleConfigOutput, body, alloc);
}
