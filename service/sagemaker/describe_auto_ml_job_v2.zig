...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAutoMLJobV2Input, options: Options) !DescribeAutoMLJobV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAutoMLJobV2Input, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeAutoMLJobV2");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAutoMLJobV2Output {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeAutoMLJobV2Output, body, alloc);
}
