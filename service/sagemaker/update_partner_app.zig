...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePartnerAppInput, options: Options) !UpdatePartnerAppOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePartnerAppInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdatePartnerApp");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePartnerAppOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdatePartnerAppOutput, body, alloc);
}
