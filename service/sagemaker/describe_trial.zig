...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrialInput, options: Options) !DescribeTrialOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrialInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeTrial");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrialOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTrialOutput, body, alloc);
}
