...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetachClusterNodeVolumeInput, options: Options) !DetachClusterNodeVolumeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetachClusterNodeVolumeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DetachClusterNodeVolume");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetachClusterNodeVolumeOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DetachClusterNodeVolumeOutput, body, alloc);
}
