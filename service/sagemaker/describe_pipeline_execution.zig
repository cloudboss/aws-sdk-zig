...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePipelineExecutionInput, options: Options) !DescribePipelineExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePipelineExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribePipelineExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePipelineExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePipelineExecutionOutput, body, alloc);
}
