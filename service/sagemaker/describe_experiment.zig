...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeExperimentInput, options: Options) !DescribeExperimentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeExperimentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.DescribeExperiment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeExperimentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeExperimentOutput, body, alloc);
}
