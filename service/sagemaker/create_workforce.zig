...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkforceInput, options: Options) !CreateWorkforceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkforceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.CreateWorkforce");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkforceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateWorkforceOutput, body, alloc);
}
