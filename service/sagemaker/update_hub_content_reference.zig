...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateHubContentReferenceInput, options: Options) !UpdateHubContentReferenceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateHubContentReferenceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "SageMaker.UpdateHubContentReference");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateHubContentReferenceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateHubContentReferenceOutput, body, alloc);
}
