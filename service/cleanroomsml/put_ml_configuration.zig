...
const PutMLConfigurationOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutMLConfigurationInput, options: Options) !PutMLConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutMLConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"defaultOutputLocation\":");
    try aws.json.writeValue(@TypeOf(input.default_output_location), input.default_output_location, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutMLConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
