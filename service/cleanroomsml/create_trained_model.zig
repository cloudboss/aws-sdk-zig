...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrainedModelInput, options: Options) !CreateTrainedModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrainedModelInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.configured_model_algorithm_association_arn), input.configured_model_algorithm_association_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"dataChannels\":");
    try aws.json.writeValue(@TypeOf(input.data_channels), input.data_channels, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.incremental_training_data_channels) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"incrementalTrainingDataChannels\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.stopping_condition) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"stoppingCondition\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
    ...
    if (input.training_input_mode) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"trainingInputMode\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrainedModelOutput {
    var result: CreateTrainedModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTrainedModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
