...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTrainedModelInferenceJobInput, options: Options) !GetTrainedModelInferenceJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTrainedModelInferenceJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.trained_model_inference_job_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTrainedModelInferenceJobOutput {
    var result: GetTrainedModelInferenceJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTrainedModelInferenceJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
