...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMLInputChannelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMLInputChannelOutput {
    var result: GetMLInputChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMLInputChannelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
