...
pub const CreateTrainingDatasetInput = struct {
...
pub const CreateTrainingDatasetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTrainingDatasetInput, options: CallOptions) !CreateTrainingDatasetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateTrainingDatasetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"trainingData\":");
    try aws.json.writeValue(@TypeOf(input.training_data), input.training_data, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateTrainingDatasetOutput {
    var result: CreateTrainingDatasetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTrainingDatasetOutput, body, allocator);
    }
    _ = status;
    _ = headers;
