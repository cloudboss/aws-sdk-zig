...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrainingDatasetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"trainingData\":");
    try aws.json.writeValue(@TypeOf(input.training_data), input.training_data, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrainingDatasetOutput {
    var result: CreateTrainingDatasetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTrainingDatasetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
