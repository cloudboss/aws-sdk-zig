...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTrainedModelInput, options: Options) !GetTrainedModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTrainedModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTrainedModelOutput {
    var result: GetTrainedModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTrainedModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
