...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCollaborationTrainedModelsInput, options: Options) !ListCollaborationTrainedModelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCollaborationTrainedModelsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCollaborationTrainedModelsOutput {
    var result: ListCollaborationTrainedModelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCollaborationTrainedModelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
