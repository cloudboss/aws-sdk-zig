...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMLInputChannelInput, options: CallOptions) !CreateMLInputChannelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateMLInputChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"configuredModelAlgorithmAssociations\":");
    try aws.json.writeValue(@TypeOf(input.configured_model_algorithm_associations), input.configured_model_algorithm_associations, allocator, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"inputChannel\":");
    try aws.json.writeValue(@TypeOf(input.input_channel), input.input_channel, allocator, &body_buf);
    has_prev = true;
