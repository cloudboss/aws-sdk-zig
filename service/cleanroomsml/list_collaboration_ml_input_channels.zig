...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCollaborationMLInputChannelsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCollaborationMLInputChannelsOutput {
    var result: ListCollaborationMLInputChannelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCollaborationMLInputChannelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
