...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConfiguredAudienceModelInput, options: Options) !GetConfiguredAudienceModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfiguredAudienceModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfiguredAudienceModelOutput {
    var result: GetConfiguredAudienceModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfiguredAudienceModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
