...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAudienceGenerationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/audience-generation-job/");
    try path_buf.appendSlice(alloc, input.audience_generation_job_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAudienceGenerationJobOutput {
    var result: GetAudienceGenerationJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAudienceGenerationJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
