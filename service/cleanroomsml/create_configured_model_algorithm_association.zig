...
pub const CreateConfiguredModelAlgorithmAssociationInput = struct {
    ...
    /// algorithm association. This information includes the maximum data size that
    /// can be exported.
...
pub const CreateConfiguredModelAlgorithmAssociationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConfiguredModelAlgorithmAssociationInput, options: Options) !CreateConfiguredModelAlgorithmAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConfiguredModelAlgorithmAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"configuredModelAlgorithmArn\":");
    try aws.json.writeValue(@TypeOf(input.configured_model_algorithm_arn), input.configured_model_algorithm_arn, alloc, &body_buf);
    has_prev = true;
    ...
    has_prev = true;
    if (input.privacy_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"privacyConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConfiguredModelAlgorithmAssociationOutput {
    var result: CreateConfiguredModelAlgorithmAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConfiguredModelAlgorithmAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
