...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrainedModelInferenceJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrainedModelInferenceJobsOutput {
    var result: ListTrainedModelInferenceJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTrainedModelInferenceJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
