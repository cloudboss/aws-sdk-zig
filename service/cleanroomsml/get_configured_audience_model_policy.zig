...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConfiguredAudienceModelPolicyInput, options: Options) !GetConfiguredAudienceModelPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfiguredAudienceModelPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfiguredAudienceModelPolicyOutput {
    var result: GetConfiguredAudienceModelPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfiguredAudienceModelPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
