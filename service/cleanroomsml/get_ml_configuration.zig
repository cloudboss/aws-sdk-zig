...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMLConfigurationInput, options: CallOptions) !GetMLConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMLConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMLConfigurationOutput {
    var result: GetMLConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMLConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
