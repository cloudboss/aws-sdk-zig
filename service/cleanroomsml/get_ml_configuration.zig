...
pub const GetMLConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMLConfigurationInput, options: Options) !GetMLConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMLConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ml-configurations");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMLConfigurationOutput {
    var result: GetMLConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMLConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
