...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAudienceGenerationJobsInput, options: Options) !ListAudienceGenerationJobsOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAudienceGenerationJobsOutput {
    var result: ListAudienceGenerationJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAudienceGenerationJobsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
