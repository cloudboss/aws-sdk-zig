...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAudienceGenerationJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.collaboration_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "collaborationId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.configured_audience_model_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "configuredAudienceModelArn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAudienceGenerationJobsOutput {
    var result: ListAudienceGenerationJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAudienceGenerationJobsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
