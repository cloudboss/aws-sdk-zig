...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateConfiguredModelAlgorithmInput, options: Options) !CreateConfiguredModelAlgorithmOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConfiguredModelAlgorithmInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.inference_container_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"inferenceContainerConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.training_container_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"trainingContainerConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateConfiguredModelAlgorithmOutput {
    var result: CreateConfiguredModelAlgorithmOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateConfiguredModelAlgorithmOutput, body, alloc);
    }
    _ = status;
    _ = headers;
