...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCollaborationConfiguredModelAlgorithmAssociationInput, options: Options) !GetCollaborationConfiguredModelAlgorithmAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCollaborationConfiguredModelAlgorithmAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.configured_model_algorithm_association_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCollaborationConfiguredModelAlgorithmAssociationOutput {
    var result: GetCollaborationConfiguredModelAlgorithmAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCollaborationConfiguredModelAlgorithmAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
