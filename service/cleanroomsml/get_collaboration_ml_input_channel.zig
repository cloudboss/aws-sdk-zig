...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCollaborationMLInputChannelInput, options: Options) !GetCollaborationMLInputChannelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCollaborationMLInputChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.ml_input_channel_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCollaborationMLInputChannelOutput {
    var result: GetCollaborationMLInputChannelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCollaborationMLInputChannelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
