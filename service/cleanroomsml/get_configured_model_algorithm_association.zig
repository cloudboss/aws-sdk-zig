...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfiguredModelAlgorithmAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfiguredModelAlgorithmAssociationOutput {
    var result: GetConfiguredModelAlgorithmAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfiguredModelAlgorithmAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
