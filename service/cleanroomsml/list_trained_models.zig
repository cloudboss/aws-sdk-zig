...
pub const ListTrainedModelsInput = struct {
...
pub const ListTrainedModelsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTrainedModelsInput, options: CallOptions) !ListTrainedModelsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTrainedModelsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTrainedModelsOutput {
    var result: ListTrainedModelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTrainedModelsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
