...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTrainedModelsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTrainedModelsOutput {
    var result: ListTrainedModelsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTrainedModelsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
