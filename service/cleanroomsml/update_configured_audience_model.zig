...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfiguredAudienceModelInput, options: CallOptions) !UpdateConfiguredAudienceModelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateConfiguredAudienceModelInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateConfiguredAudienceModelOutput {
    var result: UpdateConfiguredAudienceModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConfiguredAudienceModelOutput, body, allocator);
    }
    _ = status;
    _ = headers;
