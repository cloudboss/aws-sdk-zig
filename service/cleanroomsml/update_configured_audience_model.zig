...
pub const UpdateConfiguredAudienceModelInput = struct {
    /// The Amazon Resource Name (ARN) of the new audience model that you want to
    /// use.
    audience_model_arn: ?[]const u8 = null,
...
pub const UpdateConfiguredAudienceModelOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfiguredAudienceModelInput, options: Options) !UpdateConfiguredAudienceModelOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfiguredAudienceModelInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.audience_model_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"audienceModelArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.audience_size_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"audienceSizeConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.min_matching_seed_size) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"minMatchingSeedSize\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.shared_audience_metrics) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sharedAudienceMetrics\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfiguredAudienceModelOutput {
    var result: UpdateConfiguredAudienceModelOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateConfiguredAudienceModelOutput, body, alloc);
    }
    _ = status;
    _ = headers;
