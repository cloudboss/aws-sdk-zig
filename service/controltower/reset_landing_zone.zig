...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetLandingZoneInput, options: CallOptions) !ResetLandingZoneOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResetLandingZoneInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResetLandingZoneOutput {
    var result: ResetLandingZoneOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ResetLandingZoneOutput, body, allocator);
    }
    _ = status;
    _ = headers;
