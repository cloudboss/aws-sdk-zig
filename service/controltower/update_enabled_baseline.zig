...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEnabledBaselineInput, options: CallOptions) !UpdateEnabledBaselineOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateEnabledBaselineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateEnabledBaselineOutput {
    var result: UpdateEnabledBaselineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEnabledBaselineOutput, body, allocator);
    }
    _ = status;
    _ = headers;
