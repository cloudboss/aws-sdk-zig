...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnabledControlsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnabledControlsOutput {
    var result: ListEnabledControlsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEnabledControlsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
