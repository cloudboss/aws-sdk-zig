...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnabledBaselinesInput, options: Options) !ListEnabledBaselinesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnabledBaselinesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_children) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"includeChildren\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnabledBaselinesOutput {
    var result: ListEnabledBaselinesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEnabledBaselinesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
