...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisableControlInput, options: Options) !DisableControlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisableControlInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.control_identifier) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"controlIdentifier\":");
    ...
    if (input.target_identifier) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"targetIdentifier\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisableControlOutput {
    var result: DisableControlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DisableControlOutput, body, alloc);
    }
    _ = status;
    _ = headers;
