...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateLandingZoneInput, options: Options) !UpdateLandingZoneOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateLandingZoneInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.remediation_types) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"remediationTypes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateLandingZoneOutput {
    var result: UpdateLandingZoneOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateLandingZoneOutput, body, alloc);
    }
    _ = status;
    _ = headers;
