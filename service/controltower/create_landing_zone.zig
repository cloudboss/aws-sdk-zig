...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLandingZoneInput, options: Options) !CreateLandingZoneOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLandingZoneInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLandingZoneOutput {
    var result: CreateLandingZoneOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateLandingZoneOutput, body, alloc);
    }
    _ = status;
    _ = headers;
