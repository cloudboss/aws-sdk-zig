...
pub const ListLandingZoneOperationsOutput = struct {
    /// Lists landing zone operations.
    landing_zone_operations: ?[]const LandingZoneOperationSummary = null,
    ...
    pub const json_field_names = .{
        .landing_zone_operations = "landingZoneOperations",
        .next_token = "nextToken",
    };
};
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLandingZoneOperationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLandingZoneOperationsOutput {
    var result: ListLandingZoneOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListLandingZoneOperationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
