...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableBaselineInput, options: Options) !EnableBaselineOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableBaselineInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableBaselineOutput {
    var result: EnableBaselineOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EnableBaselineOutput, body, alloc);
    }
    _ = status;
    _ = headers;
