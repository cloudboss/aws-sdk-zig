...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableControlInput, options: CallOptions) !EnableControlOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableControlInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.control_identifier), input.control_identifier, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableControlOutput {
    var result: EnableControlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(EnableControlOutput, body, allocator);
    }
    _ = status;
    _ = headers;
