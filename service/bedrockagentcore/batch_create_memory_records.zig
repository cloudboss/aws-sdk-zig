...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreateMemoryRecordsInput, options: Options) !BatchCreateMemoryRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateMemoryRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/memoryRecords/batchCreate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateMemoryRecordsOutput {
    var result: BatchCreateMemoryRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchCreateMemoryRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
