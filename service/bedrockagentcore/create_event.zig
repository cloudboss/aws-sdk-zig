...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEventInput, options: Options) !CreateEventOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/memories/");
    try path_buf.appendSlice(alloc, input.memory_id);
    try path_buf.appendSlice(alloc, "/events");
    ...
    try body_buf.appendSlice(alloc, "\"actorId\":");
    try aws.json.writeValue(@TypeOf(input.actor_id), input.actor_id, alloc, &body_buf);
    has_prev = true;
    if (input.branch) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"branch\":");
    ...
    try aws.json.writeValue(@TypeOf(input.event_timestamp), input.event_timestamp, alloc, &body_buf);
    has_prev = true;
    if (input.metadata) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"payload\":");
    try aws.json.writeValue(@TypeOf(input.payload), input.payload, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEventOutput {
    var result: CreateEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
