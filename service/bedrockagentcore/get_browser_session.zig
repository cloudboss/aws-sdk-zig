...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBrowserSessionInput, options: Options) !GetBrowserSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBrowserSessionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBrowserSessionOutput {
    var result: GetBrowserSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBrowserSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
