...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateMemoryRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/memoryRecords/batchUpdate");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateMemoryRecordsOutput {
    var result: BatchUpdateMemoryRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateMemoryRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
