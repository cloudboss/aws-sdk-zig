...
const MemoryRecordUpdateInput = @import("memory_record_update_input.zig").MemoryRecordUpdateInput;
const MemoryRecordOutput = @import("memory_record_output.zig").MemoryRecordOutput;

pub const BatchUpdateMemoryRecordsInput = struct {
    ...
    /// operation.
    records: []const MemoryRecordUpdateInput,
...
pub const BatchUpdateMemoryRecordsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateMemoryRecordsInput, options: CallOptions) !BatchUpdateMemoryRecordsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: BatchUpdateMemoryRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/memoryRecords/batchUpdate");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchUpdateMemoryRecordsOutput {
    var result: BatchUpdateMemoryRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateMemoryRecordsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
