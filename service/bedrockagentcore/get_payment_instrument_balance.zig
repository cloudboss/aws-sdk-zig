...
pub const GetPaymentInstrumentBalanceInput = struct {
...
pub const GetPaymentInstrumentBalanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPaymentInstrumentBalanceInput, options: CallOptions) !GetPaymentInstrumentBalanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPaymentInstrumentBalanceInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("bedrock-agentcore", "Bedrock AgentCore", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/payments/getPaymentInstrumentBalance";
    ...
    try body_buf.appendSlice(allocator, "\"chain\":");
    try aws.json.writeValue(@TypeOf(input.chain), input.chain, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
