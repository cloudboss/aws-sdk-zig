...
const MemoryRecord = @import("memory_record.zig").MemoryRecord;

pub const GetMemoryRecordInput = struct {
...
pub const GetMemoryRecordOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMemoryRecordInput, options: Options) !GetMemoryRecordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMemoryRecordInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/memoryRecord/");
    try path_buf.appendSlice(alloc, input.memory_record_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMemoryRecordOutput {
    var result: GetMemoryRecordOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMemoryRecordOutput, body, alloc);
    }
    _ = status;
    _ = headers;
