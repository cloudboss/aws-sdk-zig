...
pub const DeleteMemoryRecordInput = struct {
...
pub const DeleteMemoryRecordOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMemoryRecordInput, options: Options) !DeleteMemoryRecordOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMemoryRecordInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/memoryRecords/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMemoryRecordOutput {
    var result: DeleteMemoryRecordOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteMemoryRecordOutput, body, alloc);
    }
    _ = status;
    _ = headers;
