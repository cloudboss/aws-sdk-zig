...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RetrieveMemoryRecordsInput, options: Options) !RetrieveMemoryRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RetrieveMemoryRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/retrieve");
    ...
    try aws.json.writeValue(@TypeOf(input.search_criteria), input.search_criteria, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RetrieveMemoryRecordsOutput {
    var result: RetrieveMemoryRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RetrieveMemoryRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
