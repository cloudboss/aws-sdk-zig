...
fn serializeRequest(alloc: std.mem.Allocator, input: ListActorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/actors");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListActorsOutput {
    var result: ListActorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListActorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
