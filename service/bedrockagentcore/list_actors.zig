...
const ActorSummary = @import("actor_summary.zig").ActorSummary;
...
pub const ListActorsOutput = struct {
    /// The list of actor summaries.
    actor_summaries: ?[]const ActorSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListActorsInput, options: CallOptions) !ListActorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListActorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/actors");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListActorsOutput {
    var result: ListActorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListActorsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
