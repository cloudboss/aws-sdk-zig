...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SaveBrowserSessionProfileInput, options: Options) !SaveBrowserSessionProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SaveBrowserSessionProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/browser-profiles/");
    try path_buf.appendSlice(alloc, input.profile_identifier);
    try path_buf.appendSlice(alloc, "/save");
    ...
    try body_buf.appendSlice(alloc, "\"browserIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.browser_identifier), input.browser_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SaveBrowserSessionProfileOutput {
    var result: SaveBrowserSessionProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SaveBrowserSessionProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
