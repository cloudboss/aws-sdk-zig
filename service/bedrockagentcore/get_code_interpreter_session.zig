...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCodeInterpreterSessionInput, options: Options) !GetCodeInterpreterSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCodeInterpreterSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sessions/get");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCodeInterpreterSessionOutput {
    var result: GetCodeInterpreterSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCodeInterpreterSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
