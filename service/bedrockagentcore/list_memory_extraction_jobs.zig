...
const ExtractionJobFilterInput = @import("extraction_job_filter_input.zig").ExtractionJobFilterInput;
const ExtractionJobMetadata = @import("extraction_job_metadata.zig").ExtractionJobMetadata;

pub const ListMemoryExtractionJobsInput = struct {
    /// Filter criteria to apply when listing extraction jobs.
    filter: ?ExtractionJobFilterInput = null,
...
pub const ListMemoryExtractionJobsOutput = struct {
    /// List of extraction job metadata matching the specified criteria.
    jobs: ?[]const ExtractionJobMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMemoryExtractionJobsInput, options: CallOptions) !ListMemoryExtractionJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMemoryExtractionJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/extractionJobs");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMemoryExtractionJobsOutput {
    var result: ListMemoryExtractionJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMemoryExtractionJobsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
