...
const CodeInterpreterSessionSummary = @import("code_interpreter_session_summary.zig").CodeInterpreterSessionSummary;

pub const ListCodeInterpreterSessionsInput = struct {
    ...
    /// is 10. Valid values range from 1 to 100. To retrieve the remaining results,
...
pub const ListCodeInterpreterSessionsOutput = struct {
    /// The list of code interpreter sessions that match the specified criteria.
    items: ?[]const CodeInterpreterSessionSummary = null,

    /// The token to use in a subsequent `ListCodeInterpreterSessions` request to
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCodeInterpreterSessionsInput, options: Options) !ListCodeInterpreterSessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCodeInterpreterSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sessions/list");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCodeInterpreterSessionsOutput {
    var result: ListCodeInterpreterSessionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCodeInterpreterSessionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
