...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteMemoryRecordsInput, options: Options) !BatchDeleteMemoryRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteMemoryRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/memoryRecords/batchDelete");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteMemoryRecordsOutput {
    var result: BatchDeleteMemoryRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteMemoryRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
