...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceApiKeyInput, options: Options) !GetResourceApiKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceApiKeyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceApiKeyOutput {
    var result: GetResourceApiKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceApiKeyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
