...
pub const StopRuntimeSessionInput = struct {
...
pub const StopRuntimeSessionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopRuntimeSessionInput, options: Options) !StopRuntimeSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopRuntimeSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/stopruntimesession");
    ...
    try request.headers.put(alloc, "X-Amzn-Bedrock-AgentCore-Runtime-Session-Id", input.runtime_session_id);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopRuntimeSessionOutput {
