...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBrowserStreamInput, options: Options) !UpdateBrowserStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBrowserStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/browsers/");
    try path_buf.appendSlice(alloc, input.browser_identifier);
    try path_buf.appendSlice(alloc, "/sessions/streams/update");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "sessionId=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.session_id);
    query_has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"streamUpdate\":");
    try aws.json.writeValue(@TypeOf(input.stream_update), input.stream_update, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBrowserStreamOutput {
    var result: UpdateBrowserStreamOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBrowserStreamOutput, body, alloc);
    }
    _ = status;
    _ = headers;
