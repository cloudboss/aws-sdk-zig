...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMemoryExtractionJobInput, options: Options) !StartMemoryExtractionJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMemoryExtractionJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/extractionJobs/start");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"extractionJob\":");
    try aws.json.writeValue(@TypeOf(input.extraction_job), input.extraction_job, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMemoryExtractionJobOutput {
    var result: StartMemoryExtractionJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartMemoryExtractionJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
