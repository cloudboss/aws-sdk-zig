...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteEventInput, options: Options) !DeleteEventOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/actor/");
    try path_buf.appendSlice(alloc, input.actor_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteEventOutput {
    var result: DeleteEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
