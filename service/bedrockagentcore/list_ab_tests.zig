...
const ABTestSummary = @import("ab_test_summary.zig").ABTestSummary;

pub const ListABTestsInput = struct {
...
pub const ListABTestsOutput = struct {
    /// The list of A/B test summaries.
    ab_tests: ?[]const ABTestSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListABTestsInput, options: CallOptions) !ListABTestsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListABTestsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListABTestsOutput {
    var result: ListABTestsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListABTestsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
