...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CompleteResourceTokenAuthInput, options: Options) !CompleteResourceTokenAuthOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CompleteResourceTokenAuthInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.session_uri), input.session_uri, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CompleteResourceTokenAuthOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
