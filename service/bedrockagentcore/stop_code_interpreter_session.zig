...
fn serializeRequest(alloc: std.mem.Allocator, input: StopCodeInterpreterSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sessions/stop");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopCodeInterpreterSessionOutput {
    var result: StopCodeInterpreterSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopCodeInterpreterSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
