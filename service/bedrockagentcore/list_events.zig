...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEventsInput, options: Options) !ListEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_payloads) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"includePayloads\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEventsOutput {
    var result: ListEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
