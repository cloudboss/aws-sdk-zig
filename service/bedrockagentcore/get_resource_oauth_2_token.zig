...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceOauth2TokenInput, options: Options) !GetResourceOauth2TokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceOauth2TokenInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.custom_parameters) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"customParameters\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.custom_state) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"customState\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.force_authentication) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"forceAuthentication\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"oauth2Flow\":");
    try aws.json.writeValue(@TypeOf(input.oauth_2_flow), input.oauth_2_flow, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"resourceCredentialProviderName\":");
    try aws.json.writeValue(@TypeOf(input.resource_credential_provider_name), input.resource_credential_provider_name, alloc, &body_buf);
    has_prev = true;
    if (input.resource_oauth_2_return_url) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"resourceOauth2ReturnUrl\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.session_uri) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sessionUri\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"workloadIdentityToken\":");
    try aws.json.writeValue(@TypeOf(input.workload_identity_token), input.workload_identity_token, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceOauth2TokenOutput {
    var result: GetResourceOauth2TokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceOauth2TokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
