...
pub const GetPaymentInstrumentInput = struct {
...
pub const GetPaymentInstrumentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPaymentInstrumentInput, options: CallOptions) !GetPaymentInstrumentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPaymentInstrumentInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("bedrock-agentcore", "Bedrock AgentCore", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/payments/getPaymentInstrument";

    var body_buf: std.ArrayList(u8) = .empty;
    var has_prev = false;
    try body_buf.appendSlice(allocator, "{");

    if (input.payment_connector_id) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPaymentInstrumentOutput {
    var result: GetPaymentInstrumentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPaymentInstrumentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
