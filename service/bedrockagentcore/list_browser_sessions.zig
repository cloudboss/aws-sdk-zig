...
const BrowserSessionSummary = @import("browser_session_summary.zig").BrowserSessionSummary;

pub const ListBrowserSessionsInput = struct {
    /// The unique identifier of the browser to list sessions for. If specified,
    /// only sessions for this browser are returned. If not specified, sessions for
    /// all browsers are returned.
...
pub const ListBrowserSessionsOutput = struct {
    /// The list of browser sessions that match the specified criteria.
    items: ?[]const BrowserSessionSummary = null,

    /// The token to use in a subsequent `ListBrowserSessions` request to get the
    /// next set of results.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBrowserSessionsInput, options: CallOptions) !ListBrowserSessionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBrowserSessionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBrowserSessionsOutput {
    var result: ListBrowserSessionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBrowserSessionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
