...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBrowserSessionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBrowserSessionsOutput {
    var result: ListBrowserSessionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBrowserSessionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
