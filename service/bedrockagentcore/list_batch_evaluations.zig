...
const BatchEvaluationSummary = @import("batch_evaluation_summary.zig").BatchEvaluationSummary;

pub const ListBatchEvaluationsInput = struct {
...
pub const ListBatchEvaluationsOutput = struct {
    /// The list of batch evaluation summaries.
    batch_evaluations: ?[]const BatchEvaluationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBatchEvaluationsInput, options: CallOptions) !ListBatchEvaluationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBatchEvaluationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBatchEvaluationsOutput {
    var result: ListBatchEvaluationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBatchEvaluationsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
