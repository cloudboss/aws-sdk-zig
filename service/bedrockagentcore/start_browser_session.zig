...
pub const StartBrowserSessionInput = struct {
    ...
    /// seconds (8 hours).
    session_timeout_seconds: i32 = 3600,
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartBrowserSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.extensions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"extensions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.name) |v| {
    ...
    if (input.profile_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"profileConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.view_port) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"viewPort\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartBrowserSessionOutput {
    var result: StartBrowserSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartBrowserSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
