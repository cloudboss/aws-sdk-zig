...
fn serializeRequest(alloc: std.mem.Allocator, input: StopBrowserSessionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopBrowserSessionOutput {
    var result: StopBrowserSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopBrowserSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
