...
pub const GetPaymentSessionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPaymentSessionInput, options: CallOptions) !GetPaymentSessionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPaymentSessionInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("bedrock-agentcore", "Bedrock AgentCore", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/payments/getPaymentSession";
