...
fn serializeRequest(alloc: std.mem.Allocator, input: GetWorkloadAccessTokenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetWorkloadAccessTokenOutput {
    var result: GetWorkloadAccessTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetWorkloadAccessTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
