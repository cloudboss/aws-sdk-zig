...
pub const DeleteABTestInput = struct {
...
pub const DeleteABTestOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteABTestInput, options: CallOptions) !DeleteABTestOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteABTestInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteABTestOutput {
    var result: DeleteABTestOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteABTestOutput, body, allocator);
    }
    _ = status;
    _ = headers;
