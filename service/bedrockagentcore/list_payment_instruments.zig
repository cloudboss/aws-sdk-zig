...
const PaymentInstrumentSummary = @import("payment_instrument_summary.zig").PaymentInstrumentSummary;

pub const ListPaymentInstrumentsInput = struct {
...
pub const ListPaymentInstrumentsOutput = struct {
    /// Token for pagination to retrieve the next set of results.
    next_token: ?[]const u8 = null,

    /// List of payment instrument summaries matching the request criteria.
    payment_instruments: ?[]const PaymentInstrumentSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPaymentInstrumentsInput, options: CallOptions) !ListPaymentInstrumentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPaymentInstrumentsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("bedrock-agentcore", "Bedrock AgentCore", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/payments/listPaymentInstruments";
