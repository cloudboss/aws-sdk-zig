...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMemoryRecordsInput, options: Options) !ListMemoryRecordsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMemoryRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.memory_strategy_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"memoryStrategyId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMemoryRecordsOutput {
    var result: ListMemoryRecordsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMemoryRecordsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
