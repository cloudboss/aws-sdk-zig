...
fn serializeRequest(alloc: std.mem.Allocator, input: StartTextTranslationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShineFrontendService_20170701.StartTextTranslationJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartTextTranslationJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartTextTranslationJobOutput, body, alloc);
}
