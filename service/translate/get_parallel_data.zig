...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetParallelDataInput, options: Options) !GetParallelDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetParallelDataInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("translate", "Translate", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShineFrontendService_20170701.GetParallelData");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetParallelDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetParallelDataOutput, body, alloc);
}
