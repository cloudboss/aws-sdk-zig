...
const ParallelDataDataLocation = @import("parallel_data_data_location.zig").ParallelDataDataLocation;
const ParallelDataProperties = @import("parallel_data_properties.zig").ParallelDataProperties;

pub const GetParallelDataInput = struct {
...
pub const GetParallelDataOutput = struct {
    ...
    /// create a parallel
    /// data resource. The location is returned as a presigned URL to that has a
    /// 30-minute
    /// expiration.
    auxiliary_data_location: ?ParallelDataDataLocation = null,
    ...
    data_location: ?ParallelDataDataLocation = null,
    ...
    /// 30-minute
    /// expiration.
    latest_update_attempt_auxiliary_data_location: ?ParallelDataDataLocation = null,

    /// The properties of the parallel data resource that is being retrieved.
    parallel_data_properties: ?ParallelDataProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetParallelDataInput, options: CallOptions) !GetParallelDataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetParallelDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShineFrontendService_20170701.GetParallelData");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetParallelDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetParallelDataOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "DetectedLanguageLowConfidenceException")) {
        const parsed_error: ?errors.DetectedLanguageLowConfidenceException = aws.json.parseJsonObject(errors.DetectedLanguageLowConfidenceException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
    ...
    if (std.mem.eql(u8, error_code, "UnsupportedDisplayLanguageCodeException")) {
        const parsed_error: ?errors.UnsupportedDisplayLanguageCodeException = aws.json.parseJsonObject(errors.UnsupportedDisplayLanguageCodeException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
    ...
    if (std.mem.eql(u8, error_code, "UnsupportedLanguagePairException")) {
        const parsed_error: ?errors.UnsupportedLanguagePairException = aws.json.parseJsonObject(errors.UnsupportedLanguagePairException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
