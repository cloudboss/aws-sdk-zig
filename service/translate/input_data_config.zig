...
pub const InputDataConfig = struct {
    ...
    /// input. You can
    /// specify one of the following multipurpose internet mail extension (MIME)
    /// types:
    ///
    ...
    /// translates all the
    /// files in the folder and all its sub-folders. The folder must be in the same
    /// Region as the API endpoint you are
    /// calling.
    s3_uri: []const u8,
