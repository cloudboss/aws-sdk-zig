...
pub const StopTextTranslationJobInput = struct {
...
pub const StopTextTranslationJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopTextTranslationJobInput, options: CallOptions) !StopTextTranslationJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopTextTranslationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShineFrontendService_20170701.StopTextTranslationJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopTextTranslationJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopTextTranslationJobOutput, body, allocator);
}
