...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateParallelDataInput, options: Options) !CreateParallelDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateParallelDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShineFrontendService_20170701.CreateParallelData");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateParallelDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateParallelDataOutput, body, alloc);
}
