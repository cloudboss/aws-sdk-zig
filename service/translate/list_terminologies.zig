...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTerminologiesInput, options: Options) !ListTerminologiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTerminologiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShineFrontendService_20170701.ListTerminologies");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTerminologiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTerminologiesOutput, body, alloc);
}
