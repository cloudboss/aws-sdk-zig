...
pub const ListTerminologiesInput = struct {
...
pub const ListTerminologiesOutput = struct {
    ...
    /// request.
    terminology_properties_list: ?[]const TerminologyProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTerminologiesInput, options: CallOptions) !ListTerminologiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTerminologiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShineFrontendService_20170701.ListTerminologies");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTerminologiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTerminologiesOutput, body, allocator);
}
