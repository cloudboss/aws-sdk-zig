...
pub const DeleteTerminologyInput = struct {
...
pub const DeleteTerminologyOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTerminologyInput, options: CallOptions) !DeleteTerminologyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTerminologyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShineFrontendService_20170701.DeleteTerminology");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTerminologyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
