...
pub const DescribeTextTranslationJobInput = struct {
...
pub const DescribeTextTranslationJobOutput = struct {
    /// An object that contains the properties associated with an asynchronous batch
    /// translation
    /// job.
    text_translation_job_properties: ?TextTranslationJobProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTextTranslationJobInput, options: CallOptions) !DescribeTextTranslationJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTextTranslationJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShineFrontendService_20170701.DescribeTextTranslationJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTextTranslationJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeTextTranslationJobOutput, body, allocator);
}
