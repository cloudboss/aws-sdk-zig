...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TranslateTextInput, options: Options) !TranslateTextOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TranslateTextInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShineFrontendService_20170701.TranslateText");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TranslateTextOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(TranslateTextOutput, body, alloc);
}
