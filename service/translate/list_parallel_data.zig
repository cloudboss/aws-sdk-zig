...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListParallelDataInput, options: Options) !ListParallelDataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListParallelDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSShineFrontendService_20170701.ListParallelData");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListParallelDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListParallelDataOutput, body, alloc);
}
