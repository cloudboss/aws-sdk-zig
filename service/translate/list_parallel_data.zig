...
pub const ListParallelDataInput = struct {
...
pub const ListParallelDataOutput = struct {
    ...
    parallel_data_properties_list: ?[]const ParallelDataProperties = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListParallelDataInput, options: CallOptions) !ListParallelDataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListParallelDataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSShineFrontendService_20170701.ListParallelData");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListParallelDataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListParallelDataOutput, body, allocator);
}
