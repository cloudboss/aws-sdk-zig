...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDefaultKeyReplicationRegionsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PaymentCryptographyControlPlane.GetDefaultKeyReplicationRegions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDefaultKeyReplicationRegionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDefaultKeyReplicationRegionsOutput, body, alloc);
}
