...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportKeyInput, options: Options) !ExportKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExportKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PaymentCryptographyControlPlane.ExportKey");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExportKeyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportKeyOutput, body, alloc);
}
