...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPublicKeyCertificateInput, options: Options) !GetPublicKeyCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPublicKeyCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PaymentCryptographyControlPlane.GetPublicKeyCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPublicKeyCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPublicKeyCertificateOutput, body, alloc);
}
