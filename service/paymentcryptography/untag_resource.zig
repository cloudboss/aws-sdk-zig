...
pub const UntagResourceInput = struct {
    ...
    /// specified tag key, Amazon Web Services Payment Cryptography doesn't throw an
    /// exception or return a response. To confirm that the operation succeeded, use
    /// the
    /// [ListTagsForResource](https://docs.aws.amazon.com/payment-cryptography/latest/APIReference/API_ListTagsForResource.html) operation.
...
fn serializeRequest(allocator: std.mem.Allocator, input: UntagResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PaymentCryptographyControlPlane.UntagResource");
