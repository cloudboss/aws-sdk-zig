...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCertificateSigningRequestInput, options: Options) !GetCertificateSigningRequestOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCertificateSigningRequestInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PaymentCryptographyControlPlane.GetCertificateSigningRequest");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCertificateSigningRequestOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCertificateSigningRequestOutput, body, alloc);
}
