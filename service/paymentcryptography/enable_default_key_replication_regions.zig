...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableDefaultKeyReplicationRegionsInput, options: Options) !EnableDefaultKeyReplicationRegionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableDefaultKeyReplicationRegionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PaymentCryptographyControlPlane.EnableDefaultKeyReplicationRegions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableDefaultKeyReplicationRegionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(EnableDefaultKeyReplicationRegionsOutput, body, alloc);
}
