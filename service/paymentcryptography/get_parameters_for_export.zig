...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetParametersForExportInput, options: Options) !GetParametersForExportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetParametersForExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PaymentCryptographyControlPlane.GetParametersForExport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetParametersForExportOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetParametersForExportOutput, body, alloc);
}
