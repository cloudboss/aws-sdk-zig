...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddKeyReplicationRegionsInput, options: Options) !AddKeyReplicationRegionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddKeyReplicationRegionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PaymentCryptographyControlPlane.AddKeyReplicationRegions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddKeyReplicationRegionsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(AddKeyReplicationRegionsOutput, body, alloc);
}
