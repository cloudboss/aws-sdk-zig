...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveKeyReplicationRegionsInput, options: Options) !RemoveKeyReplicationRegionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveKeyReplicationRegionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "PaymentCryptographyControlPlane.RemoveKeyReplicationRegions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveKeyReplicationRegionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RemoveKeyReplicationRegionsOutput, body, alloc);
}
