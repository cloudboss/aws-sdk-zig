...
pub const GetMpaTeamAssociationInput = struct {
...
pub const GetMpaTeamAssociationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMpaTeamAssociationInput, options: CallOptions) !GetMpaTeamAssociationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMpaTeamAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PaymentCryptographyControlPlane.GetMpaTeamAssociation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMpaTeamAssociationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetMpaTeamAssociationOutput, body, allocator);
}
