...
pub const DisassociateMpaTeamInput = struct {
...
pub const DisassociateMpaTeamOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateMpaTeamInput, options: CallOptions) !DisassociateMpaTeamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateMpaTeamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "PaymentCryptographyControlPlane.DisassociateMpaTeam");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateMpaTeamOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DisassociateMpaTeamOutput, body, allocator);
}
