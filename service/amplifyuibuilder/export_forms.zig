...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportFormsInput, options: CallOptions) !ExportFormsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ExportFormsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ExportFormsOutput {
    var result: ExportFormsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ExportFormsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
