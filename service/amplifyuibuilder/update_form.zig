...
const UpdateFormData = @import("update_form_data.zig").UpdateFormData;
const Form = @import("form.zig").Form;

pub const UpdateFormInput = struct {
...
pub const UpdateFormOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFormInput, options: CallOptions) !UpdateFormOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFormInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.updated_form, allocator);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFormOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateFormOutput = .{};
