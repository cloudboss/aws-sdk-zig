...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExchangeCodeForTokenInput, options: Options) !ExchangeCodeForTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExchangeCodeForTokenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExchangeCodeForTokenOutput {
    var result: ExchangeCodeForTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ExchangeCodeForTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
