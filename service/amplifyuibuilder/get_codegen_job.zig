...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCodegenJobInput, options: CallOptions) !GetCodegenJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCodegenJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/codegen-jobs/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCodegenJobOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
