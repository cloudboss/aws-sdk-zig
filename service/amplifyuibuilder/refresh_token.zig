...
fn serializeRequest(alloc: std.mem.Allocator, input: RefreshTokenInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RefreshTokenOutput {
    var result: RefreshTokenOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RefreshTokenOutput, body, alloc);
    }
    _ = status;
    _ = headers;
