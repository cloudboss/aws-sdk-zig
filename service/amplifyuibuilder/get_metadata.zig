...
pub const GetMetadataInput = struct {
...
pub const GetMetadataOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMetadataInput, options: CallOptions) !GetMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMetadataOutput {
    var result: GetMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMetadataOutput, body, allocator);
    }
    _ = status;
    _ = headers;
