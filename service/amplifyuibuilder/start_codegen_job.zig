...
pub const StartCodegenJobInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartCodegenJobInput, config: *aws.Config) !aws.http.Request {
    ...
    const body = try aws.json.jsonStringify(input.codegen_job_to_create, allocator);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartCodegenJobOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: StartCodegenJobOutput = .{};
