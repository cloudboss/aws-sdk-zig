...
pub const GetThingShadowInput = struct {
...
pub const GetThingShadowOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetThingShadowInput, options: CallOptions) !GetThingShadowOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "iotdata");
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetThingShadowInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetThingShadowOutput {
    var result: GetThingShadowOutput = .{};
    if (body.len > 0) {
