...
const DeleteConnectionOutput = struct {};
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.client_id);
    ...
    if (input.clean_session) |v| {
        ...
        try query_buf.appendSlice(alloc, "cleanSession=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.prevent_will_message) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "preventWillMessage=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
