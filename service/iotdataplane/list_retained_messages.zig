...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRetainedMessagesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRetainedMessagesOutput {
    var result: ListRetainedMessagesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRetainedMessagesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
