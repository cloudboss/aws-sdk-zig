...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVirtualNodesInput, options: CallOptions) !ListVirtualNodesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVirtualNodesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVirtualNodesOutput {
    var result: ListVirtualNodesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVirtualNodesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
