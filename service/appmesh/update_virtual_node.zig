...
pub const UpdateVirtualNodeInput = struct {
...
pub const UpdateVirtualNodeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVirtualNodeInput, options: Options) !UpdateVirtualNodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVirtualNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.virtual_node_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVirtualNodeOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateVirtualNodeOutput = .{};
