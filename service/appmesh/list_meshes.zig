...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMeshesInput, options: Options) !ListMeshesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMeshesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMeshesOutput {
    var result: ListMeshesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMeshesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
