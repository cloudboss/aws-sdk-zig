...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVirtualRouterInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"virtualRouterName\":");
    try aws.json.writeValue(@TypeOf(input.virtual_router_name), input.virtual_router_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVirtualRouterOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
