...
pub const CreateVirtualRouterInput = struct {
...
pub const CreateVirtualRouterOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVirtualRouterInput, options: CallOptions) !CreateVirtualRouterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVirtualRouterInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"virtualRouterName\":");
    try aws.json.writeValue(@TypeOf(input.virtual_router_name), input.virtual_router_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVirtualRouterOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateVirtualRouterOutput = .{};
