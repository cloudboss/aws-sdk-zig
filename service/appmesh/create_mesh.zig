...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMeshInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"meshName\":");
    try aws.json.writeValue(@TypeOf(input.mesh_name), input.mesh_name, alloc, &body_buf);
    has_prev = true;
    if (input.spec) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMeshOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
