...
pub const UpdateGatewayRouteInput = struct {
...
pub const UpdateGatewayRouteOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGatewayRouteInput, options: Options) !UpdateGatewayRouteOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGatewayRouteInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.gateway_route_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGatewayRouteOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: UpdateGatewayRouteOutput = .{};
