...
const VirtualGatewayRef = @import("virtual_gateway_ref.zig").VirtualGatewayRef;

pub const ListVirtualGatewaysInput = struct {
...
pub const ListVirtualGatewaysOutput = struct {
    ...
    virtual_gateways: ?[]const VirtualGatewayRef = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVirtualGatewaysInput, options: CallOptions) !ListVirtualGatewaysOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVirtualGatewaysInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVirtualGatewaysOutput {
    var result: ListVirtualGatewaysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVirtualGatewaysOutput, body, allocator);
    }
    _ = status;
    _ = headers;
