...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVirtualGatewaysInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVirtualGatewaysOutput {
    var result: ListVirtualGatewaysOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVirtualGatewaysOutput, body, alloc);
    }
    _ = status;
    _ = headers;
