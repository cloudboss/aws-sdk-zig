...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVirtualRoutersInput, options: Options) !ListVirtualRoutersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVirtualRoutersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVirtualRoutersOutput {
    var result: ListVirtualRoutersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVirtualRoutersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
