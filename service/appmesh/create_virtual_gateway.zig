...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVirtualGatewayInput, options: CallOptions) !CreateVirtualGatewayOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVirtualGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"virtualGatewayName\":");
    try aws.json.writeValue(@TypeOf(input.virtual_gateway_name), input.virtual_gateway_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVirtualGatewayOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
