...
pub const CreateVirtualGatewayInput = struct {
...
pub const CreateVirtualGatewayOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVirtualGatewayInput, options: Options) !CreateVirtualGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVirtualGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/virtualGateways");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"virtualGatewayName\":");
    try aws.json.writeValue(@TypeOf(input.virtual_gateway_name), input.virtual_gateway_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVirtualGatewayOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateVirtualGatewayOutput = .{};
