...
const GatewayRouteRef = @import("gateway_route_ref.zig").GatewayRouteRef;

pub const ListGatewayRoutesInput = struct {
...
pub const ListGatewayRoutesOutput = struct {
    /// The list of existing gateway routes for the specified service mesh and
    /// virtual
    /// gateway.
    gateway_routes: ?[]const GatewayRouteRef = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGatewayRoutesInput, options: Options) !ListGatewayRoutesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGatewayRoutesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGatewayRoutesOutput {
    var result: ListGatewayRoutesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGatewayRoutesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
