...
const VirtualServiceRef = @import("virtual_service_ref.zig").VirtualServiceRef;

pub const ListVirtualServicesInput = struct {
...
pub const ListVirtualServicesOutput = struct {
    ...
    virtual_services: ?[]const VirtualServiceRef = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVirtualServicesInput, options: CallOptions) !ListVirtualServicesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVirtualServicesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVirtualServicesOutput {
    var result: ListVirtualServicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVirtualServicesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
