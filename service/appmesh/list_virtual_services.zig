...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVirtualServicesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVirtualServicesOutput {
    var result: ListVirtualServicesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVirtualServicesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
