...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVirtualNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"virtualNodeName\":");
    try aws.json.writeValue(@TypeOf(input.virtual_node_name), input.virtual_node_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVirtualNodeOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
