...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGatewayRouteInput, options: CallOptions) !CreateGatewayRouteOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateGatewayRouteInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"gatewayRouteName\":");
    try aws.json.writeValue(@TypeOf(input.gateway_route_name), input.gateway_route_name, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateGatewayRouteOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
