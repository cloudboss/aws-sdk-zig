...
pub const CreateGatewayRouteInput = struct {
    ...
    /// virtual
    /// gateway is in a shared mesh, then you must be the owner of the virtual
    /// gateway
    /// resource.
...
pub const CreateGatewayRouteOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGatewayRouteInput, options: Options) !CreateGatewayRouteOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGatewayRouteInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/virtualGateway/");
    try path_buf.appendSlice(alloc, input.virtual_gateway_name);
    try path_buf.appendSlice(alloc, "/gatewayRoutes");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"gatewayRouteName\":");
    try aws.json.writeValue(@TypeOf(input.gateway_route_name), input.gateway_route_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"spec\":");
    try aws.json.writeValue(@TypeOf(input.spec), input.spec, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGatewayRouteOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateGatewayRouteOutput = .{};
