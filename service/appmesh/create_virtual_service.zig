...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVirtualServiceInput, options: CallOptions) !CreateVirtualServiceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVirtualServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"virtualServiceName\":");
    try aws.json.writeValue(@TypeOf(input.virtual_service_name), input.virtual_service_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVirtualServiceOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
