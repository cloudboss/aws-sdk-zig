...
pub const CreateVirtualServiceInput = struct {
...
pub const CreateVirtualServiceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVirtualServiceInput, options: Options) !CreateVirtualServiceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVirtualServiceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/virtualServices");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"virtualServiceName\":");
    try aws.json.writeValue(@TypeOf(input.virtual_service_name), input.virtual_service_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVirtualServiceOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateVirtualServiceOutput = .{};
