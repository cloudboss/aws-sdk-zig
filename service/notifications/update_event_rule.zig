...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEventRuleInput, options: Options) !UpdateEventRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEventRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/event-rules/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEventRuleOutput {
    var result: UpdateEventRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEventRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
