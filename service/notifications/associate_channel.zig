...
pub const AssociateChannelInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateChannelInput, options: Options) !AssociateChannelOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateChannelInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/channels/associate/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateChannelOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateChannelOutput = .{};
