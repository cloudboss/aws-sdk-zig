...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventRulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEventRulesOutput {
    var result: ListEventRulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEventRulesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
