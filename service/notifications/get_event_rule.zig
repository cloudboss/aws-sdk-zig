...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEventRuleInput, options: Options) !GetEventRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEventRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEventRuleOutput {
    var result: GetEventRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEventRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
