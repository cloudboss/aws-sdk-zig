...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetManagedNotificationChildEventInput, options: Options) !GetManagedNotificationChildEventOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetManagedNotificationChildEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/managed-notification-child-events/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetManagedNotificationChildEventOutput {
    var result: GetManagedNotificationChildEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetManagedNotificationChildEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
