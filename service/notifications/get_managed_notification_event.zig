...
const ManagedNotificationEvent = @import("managed_notification_event.zig").ManagedNotificationEvent;

pub const GetManagedNotificationEventInput = struct {
...
pub const GetManagedNotificationEventOutput = struct {
    /// The ARN of the resource.
    arn: []const u8,

    /// The content of the `ManagedNotificationEvent`.
    content: ?ManagedNotificationEvent = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetManagedNotificationEventInput, options: Options) !GetManagedNotificationEventOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetManagedNotificationEventInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/managed-notification-events/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetManagedNotificationEventOutput {
    var result: GetManagedNotificationEventOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetManagedNotificationEventOutput, body, alloc);
    }
    _ = status;
    _ = headers;
