...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedNotificationConfigurationsInput, options: Options) !ListManagedNotificationConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedNotificationConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.channel_identifier) |v| {
        ...
        try query_buf.appendSlice(alloc, "channelIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedNotificationConfigurationsOutput {
    var result: ListManagedNotificationConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedNotificationConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
