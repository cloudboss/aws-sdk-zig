...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedNotificationEventsInput, options: Options) !ListManagedNotificationEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedNotificationEventsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedNotificationEventsOutput {
    var result: ListManagedNotificationEventsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedNotificationEventsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
