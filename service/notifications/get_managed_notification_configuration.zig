...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetManagedNotificationConfigurationInput, options: Options) !GetManagedNotificationConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetManagedNotificationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/managed-notification-configurations/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetManagedNotificationConfigurationOutput {
    var result: GetManagedNotificationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetManagedNotificationConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
