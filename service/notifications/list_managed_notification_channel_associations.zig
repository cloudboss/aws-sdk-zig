...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListManagedNotificationChannelAssociationsInput, options: Options) !ListManagedNotificationChannelAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListManagedNotificationChannelAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "managedNotificationConfigurationArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.managed_notification_configuration_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListManagedNotificationChannelAssociationsOutput {
    var result: ListManagedNotificationChannelAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListManagedNotificationChannelAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
