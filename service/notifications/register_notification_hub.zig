...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterNotificationHubInput, options: CallOptions) !RegisterNotificationHubOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterNotificationHubInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"notificationHubRegion\":");
    try aws.json.writeValue(@TypeOf(input.notification_hub_region), input.notification_hub_region, allocator, &body_buf);
    has_prev = true;
