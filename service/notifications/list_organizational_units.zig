...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOrganizationalUnitsInput, options: Options) !ListOrganizationalUnitsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListOrganizationalUnitsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListOrganizationalUnitsOutput {
    var result: ListOrganizationalUnitsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListOrganizationalUnitsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
