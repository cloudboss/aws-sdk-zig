...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterNotificationHubInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/notification-hubs/");
    try path_buf.appendSlice(alloc, input.notification_hub_region);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterNotificationHubOutput {
    var result: DeregisterNotificationHubOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeregisterNotificationHubOutput, body, alloc);
    }
    _ = status;
    _ = headers;
