...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKxDatabaseInput, options: CallOptions) !CreateKxDatabaseOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateKxDatabaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateKxDatabaseOutput {
    var result: CreateKxDatabaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKxDatabaseOutput, body, allocator);
    }
    _ = status;
    _ = headers;
