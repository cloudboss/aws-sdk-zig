...
pub const CreateKxDatabaseInput = struct {
...
pub const CreateKxDatabaseOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKxDatabaseInput, options: Options) !CreateKxDatabaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKxDatabaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKxDatabaseOutput {
    var result: CreateKxDatabaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKxDatabaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
