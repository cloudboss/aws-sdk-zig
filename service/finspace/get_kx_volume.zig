...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetKxVolumeInput, options: Options) !GetKxVolumeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKxVolumeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKxVolumeOutput {
    var result: GetKxVolumeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetKxVolumeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
