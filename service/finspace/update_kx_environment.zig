...
pub const UpdateKxEnvironmentInput = struct {
...
pub const UpdateKxEnvironmentOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKxEnvironmentInput, options: Options) !UpdateKxEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKxEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKxEnvironmentOutput {
    var result: UpdateKxEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKxEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
