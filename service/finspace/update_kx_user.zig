...
pub const UpdateKxUserInput = struct {
...
pub const UpdateKxUserOutput = struct {
    /// A unique identifier for the kdb environment.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKxUserInput, options: CallOptions) !UpdateKxUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateKxUserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateKxUserOutput {
    var result: UpdateKxUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKxUserOutput, body, allocator);
    }
    _ = status;
    _ = headers;
