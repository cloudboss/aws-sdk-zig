...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKxClusterInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKxClusterOutput {
    var result: GetKxClusterOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetKxClusterOutput, body, alloc);
    }
    _ = status;
    _ = headers;
