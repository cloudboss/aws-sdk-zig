...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetKxUserInput, options: Options) !GetKxUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKxUserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKxUserOutput {
    var result: GetKxUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetKxUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
