...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKxEnvironmentNetworkInput, options: CallOptions) !UpdateKxEnvironmentNetworkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateKxEnvironmentNetworkInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/network");
    ...
    if (input.custom_dns_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"customDNSConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.transit_gateway_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"transitGatewayConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
