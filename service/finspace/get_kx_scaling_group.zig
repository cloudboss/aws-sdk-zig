...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetKxScalingGroupInput, options: Options) !GetKxScalingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKxScalingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.scaling_group_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKxScalingGroupOutput {
    var result: GetKxScalingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetKxScalingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
