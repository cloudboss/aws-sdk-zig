...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKxClusterDatabasesInput, options: Options) !UpdateKxClusterDatabasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKxClusterDatabasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/configuration/databases");
    ...
    try aws.json.writeValue(@TypeOf(input.databases), input.databases, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKxClusterDatabasesOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
