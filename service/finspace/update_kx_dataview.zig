...
pub const UpdateKxDataviewInput = struct {
...
pub const UpdateKxDataviewOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKxDataviewInput, options: Options) !UpdateKxDataviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKxDataviewInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dataviews/");
    try path_buf.appendSlice(alloc, input.dataview_name);
    ...
    if (input.changeset_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"changesetId\":");
    ...
    if (input.segment_configurations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"segmentConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKxDataviewOutput {
    var result: UpdateKxDataviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKxDataviewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
