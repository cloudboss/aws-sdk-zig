...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKxDataviewInput, options: Options) !CreateKxDataviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKxDataviewInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"dataviewName\":");
    try aws.json.writeValue(@TypeOf(input.dataview_name), input.dataview_name, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.read_write) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"readWrite\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKxDataviewOutput {
    var result: CreateKxDataviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKxDataviewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
