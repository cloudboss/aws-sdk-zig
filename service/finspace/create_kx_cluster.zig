...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKxClusterInput, options: CallOptions) !CreateKxClusterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateKxClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.auto_scaling_configuration) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"autoScalingConfiguration\":");
    ...
    if (input.cache_storage_configurations) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"cacheStorageConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    ...
    if (input.cluster_description) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"clusterDescription\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.cluster_type), input.cluster_type, allocator, &body_buf);
    has_prev = true;
    if (input.code) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    if (input.savedown_storage_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"savedownStorageConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.scaling_group_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"scalingGroupConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
    ...
    if (input.tickerplant_log_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"tickerplantLogConfiguration\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"vpcConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.vpc_configuration), input.vpc_configuration, allocator, &body_buf);
    has_prev = true;
