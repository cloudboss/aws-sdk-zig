...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetKxDatabaseInput, options: Options) !GetKxDatabaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKxDatabaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKxDatabaseOutput {
    var result: GetKxDatabaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetKxDatabaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
