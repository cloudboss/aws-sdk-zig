...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKxDatabaseInput, options: Options) !UpdateKxDatabaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKxDatabaseInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKxDatabaseOutput {
    var result: UpdateKxDatabaseOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKxDatabaseOutput, body, alloc);
    }
    _ = status;
    _ = headers;
