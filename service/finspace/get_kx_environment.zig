...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetKxEnvironmentInput, options: Options) !GetKxEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKxEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKxEnvironmentOutput {
    var result: GetKxEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetKxEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
