...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKxScalingGroupInput, options: Options) !CreateKxScalingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKxScalingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.availability_zone_id), input.availability_zone_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"hostType\":");
    try aws.json.writeValue(@TypeOf(input.host_type), input.host_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"scalingGroupName\":");
    try aws.json.writeValue(@TypeOf(input.scaling_group_name), input.scaling_group_name, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKxScalingGroupOutput {
    var result: CreateKxScalingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKxScalingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
