...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListKxScalingGroupsInput, options: Options) !ListKxScalingGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListKxScalingGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/scalingGroups");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListKxScalingGroupsOutput {
    var result: ListKxScalingGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListKxScalingGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
