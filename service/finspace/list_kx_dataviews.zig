...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListKxDataviewsInput, options: Options) !ListKxDataviewsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListKxDataviewsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListKxDataviewsOutput {
    var result: ListKxDataviewsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListKxDataviewsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
