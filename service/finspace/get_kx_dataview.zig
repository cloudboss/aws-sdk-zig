...
pub const GetKxDataviewInput = struct {
...
pub const GetKxDataviewOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetKxDataviewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetKxDataviewOutput {
    var result: GetKxDataviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetKxDataviewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
