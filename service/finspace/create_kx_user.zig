...
pub const CreateKxUserInput = struct {
...
pub const CreateKxUserOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKxUserInput, options: CallOptions) !CreateKxUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateKxUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.iam_role), input.iam_role, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.user_name), input.user_name, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateKxUserOutput {
    var result: CreateKxUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKxUserOutput, body, allocator);
    }
    _ = status;
    _ = headers;
