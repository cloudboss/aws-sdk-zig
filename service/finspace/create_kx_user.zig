...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKxUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.iam_role), input.iam_role, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.user_name), input.user_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKxUserOutput {
    var result: CreateKxUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKxUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
