...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKxChangesetInput, options: Options) !CreateKxChangesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKxChangesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"changeRequests\":");
    try aws.json.writeValue(@TypeOf(input.change_requests), input.change_requests, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKxChangesetOutput {
    var result: CreateKxChangesetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKxChangesetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
