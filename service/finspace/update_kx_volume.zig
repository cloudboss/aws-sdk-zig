...
pub const UpdateKxVolumeInput = struct {
...
pub const UpdateKxVolumeOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKxVolumeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.volume_name);
    ...
    if (input.nas_1_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"nas1Configuration\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKxVolumeOutput {
    var result: UpdateKxVolumeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKxVolumeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
