...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateKxVolumeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateKxVolumeOutput {
    var result: UpdateKxVolumeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKxVolumeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
