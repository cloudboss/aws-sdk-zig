...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateKxEnvironmentInput, options: Options) !CreateKxEnvironmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateKxEnvironmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateKxEnvironmentOutput {
    var result: CreateKxEnvironmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateKxEnvironmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
