...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartNextPendingJobExecutionInput, options: Options) !StartNextPendingJobExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartNextPendingJobExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/jobs/$next");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartNextPendingJobExecutionOutput {
    var result: StartNextPendingJobExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartNextPendingJobExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
