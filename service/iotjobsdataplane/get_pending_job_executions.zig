...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPendingJobExecutionsInput, options: Options) !GetPendingJobExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPendingJobExecutionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPendingJobExecutionsOutput {
    var result: GetPendingJobExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPendingJobExecutionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
