...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartCommandExecutionInput, options: Options) !StartCommandExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartCommandExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.command_arn), input.command_arn, alloc, &body_buf);
    has_prev = true;
    if (input.execution_timeout_seconds) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"executionTimeoutSeconds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartCommandExecutionOutput {
    var result: StartCommandExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartCommandExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
