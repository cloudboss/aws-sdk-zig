...
pub const DescribeChimeWebhookConfigurationsInput = struct {
...
pub const DescribeChimeWebhookConfigurationsOutput = struct {
    /// An optional token returned from a prior request. Use this token for
    /// pagination of results from this action. If this parameter is specified, the
    /// response includes only results beyond the token, up to the value specified
    /// by MaxResults.
    ...
    webhook_configurations: ?[]const ChimeWebhookConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeChimeWebhookConfigurationsInput, options: Options) !DescribeChimeWebhookConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeChimeWebhookConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/describe-chime-webhook-configurations";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeChimeWebhookConfigurationsOutput {
    var result: DescribeChimeWebhookConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeChimeWebhookConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
