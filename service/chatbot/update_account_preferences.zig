...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccountPreferencesInput, options: Options) !UpdateAccountPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccountPreferencesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.training_data_collection_enabled) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"TrainingDataCollectionEnabled\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccountPreferencesOutput {
    var result: UpdateAccountPreferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccountPreferencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
