...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccountPreferencesInput, options: Options) !GetAccountPreferencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccountPreferencesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccountPreferencesOutput {
    var result: GetAccountPreferencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAccountPreferencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
