...
pub const AssociateToConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateToConfigurationInput, options: Options) !AssociateToConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateToConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/associate-to-configuration";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateToConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateToConfigurationOutput = .{};
