...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateToConfigurationInput, options: CallOptions) !AssociateToConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateToConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"ChatConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.chat_configuration), input.chat_configuration, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateToConfigurationOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
