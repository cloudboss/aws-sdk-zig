...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSlackUserIdentitiesInput, options: Options) !DescribeSlackUserIdentitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSlackUserIdentitiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSlackUserIdentitiesOutput {
    var result: DescribeSlackUserIdentitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSlackUserIdentitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
