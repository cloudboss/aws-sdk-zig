...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSlackChannelConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSlackChannelConfigurationOutput {
    var result: UpdateSlackChannelConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSlackChannelConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
