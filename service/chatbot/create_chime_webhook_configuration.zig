...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateChimeWebhookConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.sns_topic_arns), input.sns_topic_arns, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"WebhookDescription\":");
    try aws.json.writeValue(@TypeOf(input.webhook_description), input.webhook_description, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"WebhookUrl\":");
    try aws.json.writeValue(@TypeOf(input.webhook_url), input.webhook_url, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateChimeWebhookConfigurationOutput {
    var result: CreateChimeWebhookConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateChimeWebhookConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
