...
pub const UpdateChimeWebhookConfigurationInput = struct {
    ...
    webhook_description: ?[]const u8 = null,
...
pub const UpdateChimeWebhookConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateChimeWebhookConfigurationInput, options: CallOptions) !UpdateChimeWebhookConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateChimeWebhookConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/update-chime-webhook-configuration";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateChimeWebhookConfigurationOutput {
    var result: UpdateChimeWebhookConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChimeWebhookConfigurationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
