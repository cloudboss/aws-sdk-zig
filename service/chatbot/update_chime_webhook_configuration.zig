...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateChimeWebhookConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateChimeWebhookConfigurationOutput {
    var result: UpdateChimeWebhookConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateChimeWebhookConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
