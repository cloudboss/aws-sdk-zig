...
const SlackWorkspace = @import("slack_workspace.zig").SlackWorkspace;

pub const DescribeSlackWorkspacesInput = struct {
...
pub const DescribeSlackWorkspacesOutput = struct {
    /// An optional token returned from a prior request. Use this token for
    /// pagination of results from this action. If this parameter is specified, the
    /// response includes only results beyond the token, up to the value specified
    /// by MaxResults.
    ...
    slack_workspaces: ?[]const SlackWorkspace = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSlackWorkspacesInput, options: CallOptions) !DescribeSlackWorkspacesOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "chatbot");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSlackWorkspacesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/describe-slack-workspaces";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSlackWorkspacesOutput {
    var result: DescribeSlackWorkspacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSlackWorkspacesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
