...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSlackWorkspacesInput, options: Options) !DescribeSlackWorkspacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSlackWorkspacesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSlackWorkspacesOutput {
    var result: DescribeSlackWorkspacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSlackWorkspacesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
