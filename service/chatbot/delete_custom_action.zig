...
pub const DeleteCustomActionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCustomActionInput, options: Options) !DeleteCustomActionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteCustomActionInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/delete-custom-action";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteCustomActionOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: DeleteCustomActionOutput = .{};
