...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCustomActionInput, options: Options) !UpdateCustomActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCustomActionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.alias_name) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AliasName\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"CustomActionArn\":");
    try aws.json.writeValue(@TypeOf(input.custom_action_arn), input.custom_action_arn, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCustomActionOutput {
    var result: UpdateCustomActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCustomActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
