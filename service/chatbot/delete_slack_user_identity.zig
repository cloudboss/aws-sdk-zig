...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSlackUserIdentityInput, options: Options) !DeleteSlackUserIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSlackUserIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"SlackUserId\":");
    try aws.json.writeValue(@TypeOf(input.slack_user_id), input.slack_user_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSlackUserIdentityOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
