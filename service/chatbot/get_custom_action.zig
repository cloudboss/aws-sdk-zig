...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCustomActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCustomActionOutput {
    var result: GetCustomActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCustomActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
