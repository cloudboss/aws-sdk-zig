...
pub const GetCustomActionInput = struct {
...
pub const GetCustomActionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCustomActionInput, options: CallOptions) !GetCustomActionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCustomActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCustomActionOutput {
    var result: GetCustomActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCustomActionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
