...
pub const DisassociateFromConfigurationInput = struct {
    /// The channel configuration the resource is being disassociated from.
    chat_configuration: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateFromConfigurationInput, options: Options) !DisassociateFromConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateFromConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/disassociate-from-configuration";
    ...
    try body_buf.appendSlice(alloc, "\"ChatConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.chat_configuration), input.chat_configuration, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Resource\":");
    try aws.json.writeValue(@TypeOf(input.resource), input.resource, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateFromConfigurationOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: DisassociateFromConfigurationOutput = .{};
