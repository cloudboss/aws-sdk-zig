...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomActionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomActionOutput {
    var result: CreateCustomActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCustomActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
