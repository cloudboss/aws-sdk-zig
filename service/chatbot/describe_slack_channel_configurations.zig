...
pub const DescribeSlackChannelConfigurationsInput = struct {
...
pub const DescribeSlackChannelConfigurationsOutput = struct {
    /// An optional token returned from a prior request. Use this token for
    /// pagination of results from this action. If this parameter is specified, the
    /// response includes only results beyond the token, up to the value specified
    /// by MaxResults.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSlackChannelConfigurationsInput, options: Options) !DescribeSlackChannelConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSlackChannelConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/describe-slack-channel-configurations";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSlackChannelConfigurationsOutput {
    var result: DescribeSlackChannelConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSlackChannelConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
