...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMicrosoftTeamsChannelConfigurationInput, options: Options) !UpdateMicrosoftTeamsChannelConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMicrosoftTeamsChannelConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ChatConfigurationArn\":");
    try aws.json.writeValue(@TypeOf(input.chat_configuration_arn), input.chat_configuration_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMicrosoftTeamsChannelConfigurationOutput {
    var result: UpdateMicrosoftTeamsChannelConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateMicrosoftTeamsChannelConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
