...
const TeamsUserIdentity = @import("teams_user_identity.zig").TeamsUserIdentity;

pub const ListMicrosoftTeamsUserIdentitiesInput = struct {
...
pub const ListMicrosoftTeamsUserIdentitiesOutput = struct {
    /// An optional token returned from a prior request. Use this token for
    /// pagination of results from this action. If this parameter is specified, the
    /// response includes only results beyond the token, up to the value specified
    /// by MaxResults.
    ...
    teams_user_identities: ?[]const TeamsUserIdentity = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .teams_user_identities = "TeamsUserIdentities",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMicrosoftTeamsUserIdentitiesInput, options: Options) !ListMicrosoftTeamsUserIdentitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMicrosoftTeamsUserIdentitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/list-ms-teams-user-identities";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMicrosoftTeamsUserIdentitiesOutput {
    var result: ListMicrosoftTeamsUserIdentitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMicrosoftTeamsUserIdentitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
