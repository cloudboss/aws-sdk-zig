...
const ConfiguredTeam = @import("configured_team.zig").ConfiguredTeam;

pub const ListMicrosoftTeamsConfiguredTeamsInput = struct {
...
pub const ListMicrosoftTeamsConfiguredTeamsOutput = struct {
    /// A list of teams in Microsoft Teams that are configured with AWS Chatbot.
    configured_teams: ?[]const ConfiguredTeam = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMicrosoftTeamsConfiguredTeamsInput, options: CallOptions) !ListMicrosoftTeamsConfiguredTeamsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMicrosoftTeamsConfiguredTeamsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/list-ms-teams-configured-teams";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMicrosoftTeamsConfiguredTeamsOutput {
    var result: ListMicrosoftTeamsConfiguredTeamsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMicrosoftTeamsConfiguredTeamsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
