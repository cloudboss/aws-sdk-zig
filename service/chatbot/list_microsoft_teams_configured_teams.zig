...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMicrosoftTeamsConfiguredTeamsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMicrosoftTeamsConfiguredTeamsOutput {
    var result: ListMicrosoftTeamsConfiguredTeamsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListMicrosoftTeamsConfiguredTeamsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
