...
pub const ListCustomActionsInput = struct {
...
pub const ListCustomActionsOutput = struct {
    /// A list of custom actions.
    custom_actions: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomActionsInput, options: CallOptions) !ListCustomActionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/list-custom-actions";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomActionsOutput {
    var result: ListCustomActionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomActionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
