...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomActionsInput, options: Options) !ListCustomActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCustomActionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCustomActionsOutput {
    var result: ListCustomActionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomActionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
