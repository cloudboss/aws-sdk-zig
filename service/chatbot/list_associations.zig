...
const AssociationListing = @import("association_listing.zig").AssociationListing;
...
pub const ListAssociationsOutput = struct {
    ...
    associations: ?[]const AssociationListing = null,
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssociationsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
