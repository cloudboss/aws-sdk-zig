...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMeetingInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMeetingOutput {
    var result: CreateMeetingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMeetingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
