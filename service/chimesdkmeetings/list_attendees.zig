...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAttendeesInput, options: CallOptions) !ListAttendeesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAttendeesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAttendeesOutput {
    var result: ListAttendeesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAttendeesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
