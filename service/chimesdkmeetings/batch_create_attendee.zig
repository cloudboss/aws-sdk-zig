...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateAttendeeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateAttendeeOutput {
    var result: BatchCreateAttendeeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchCreateAttendeeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
