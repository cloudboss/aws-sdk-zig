...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAttendeeInput, options: CallOptions) !CreateAttendeeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAttendeeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"ExternalUserId\":");
    try aws.json.writeValue(@TypeOf(input.external_user_id), input.external_user_id, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAttendeeOutput {
    var result: CreateAttendeeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAttendeeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
