...
pub const CreateAttendeeInput = struct {
...
pub const CreateAttendeeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAttendeeInput, options: Options) !CreateAttendeeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAttendeeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/meetings/");
    try path_buf.appendSlice(alloc, input.meeting_id);
    try path_buf.appendSlice(alloc, "/attendees");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ExternalUserId\":");
    try aws.json.writeValue(@TypeOf(input.external_user_id), input.external_user_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAttendeeOutput {
    var result: CreateAttendeeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAttendeeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
