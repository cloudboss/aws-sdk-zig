...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAttendeeCapabilitiesInput, options: Options) !UpdateAttendeeCapabilitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAttendeeCapabilitiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAttendeeCapabilitiesOutput {
    var result: UpdateAttendeeCapabilitiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAttendeeCapabilitiesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
