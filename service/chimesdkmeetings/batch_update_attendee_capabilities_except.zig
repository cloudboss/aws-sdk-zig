...
const BatchUpdateAttendeeCapabilitiesExceptOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateAttendeeCapabilitiesExceptInput, options: Options) !BatchUpdateAttendeeCapabilitiesExceptOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateAttendeeCapabilitiesExceptInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/attendees/capabilities");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    try query_buf.appendSlice(alloc, "operation=batch-update-except");
    query_has_prev = true;
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ExcludedAttendeeIds\":");
    try aws.json.writeValue(@TypeOf(input.excluded_attendee_ids), input.excluded_attendee_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateAttendeeCapabilitiesExceptOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
