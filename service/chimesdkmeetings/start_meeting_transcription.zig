...
const StartMeetingTranscriptionOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMeetingTranscriptionInput, options: Options) !StartMeetingTranscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMeetingTranscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"TranscriptionConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.transcription_configuration), input.transcription_configuration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMeetingTranscriptionOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
