...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkerConfigurationInput, options: Options) !DescribeWorkerConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWorkerConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWorkerConfigurationOutput {
    var result: DescribeWorkerConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeWorkerConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
