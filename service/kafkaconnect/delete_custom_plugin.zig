...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCustomPluginInput, options: Options) !DeleteCustomPluginOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCustomPluginInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/custom-plugins/");
    try path_buf.appendSlice(alloc, input.custom_plugin_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCustomPluginOutput {
    var result: DeleteCustomPluginOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteCustomPluginOutput, body, alloc);
    }
    _ = status;
    _ = headers;
