...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConnectorOperationsInput, options: Options) !ListConnectorOperationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConnectorOperationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConnectorOperationsOutput {
    var result: ListConnectorOperationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConnectorOperationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
