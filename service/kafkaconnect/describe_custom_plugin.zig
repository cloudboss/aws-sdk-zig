...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeCustomPluginInput, options: Options) !DescribeCustomPluginOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCustomPluginInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCustomPluginOutput {
    var result: DescribeCustomPluginOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCustomPluginOutput, body, alloc);
    }
    _ = status;
    _ = headers;
