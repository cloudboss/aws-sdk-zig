...
pub const ListConnectorsInput = struct {
    ...
    connector_name_prefix: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConnectorsInput, options: Options) !ListConnectorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.connector_name_prefix) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "connectorNamePrefix=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConnectorsOutput {
    var result: ListConnectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListConnectorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
