...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomPluginInput, options: Options) !CreateCustomPluginOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomPluginInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.content_type), input.content_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomPluginOutput {
    var result: CreateCustomPluginOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCustomPluginOutput, body, alloc);
    }
    _ = status;
    _ = headers;
