...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomPluginsInput, options: Options) !ListCustomPluginsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCustomPluginsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCustomPluginsOutput {
    var result: ListCustomPluginsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomPluginsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
