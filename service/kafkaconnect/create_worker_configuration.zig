...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkerConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"propertiesFileContent\":");
    try aws.json.writeValue(@TypeOf(input.properties_file_content), input.properties_file_content, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkerConfigurationOutput {
    var result: CreateWorkerConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateWorkerConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
