...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLatestConfigurationInput, options: CallOptions) !GetLatestConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetLatestConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(allocator, "configuration_token=");
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.configuration_token);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetLatestConfigurationOutput {
    ...
    if (headers.get("next-poll-configuration-token")) |value| {
        result.next_poll_configuration_token = try allocator.dupe(u8, value);
    }
