...
pub const GetSyncConfigurationInput = struct {
...
pub const GetSyncConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSyncConfigurationInput, options: CallOptions) !GetSyncConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSyncConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeConnections_20231201.GetSyncConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSyncConfigurationOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetSyncConfigurationOutput, body, allocator);
}
