...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSyncConfigurationInput, options: Options) !GetSyncConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSyncConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.GetSyncConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSyncConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSyncConfigurationOutput, body, alloc);
}
