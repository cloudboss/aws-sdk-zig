...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRepositoryLinkInput, options: Options) !CreateRepositoryLinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRepositoryLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.CreateRepositoryLink");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRepositoryLinkOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateRepositoryLinkOutput, body, alloc);
}
