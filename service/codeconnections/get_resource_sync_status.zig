...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceSyncStatusInput, options: Options) !GetResourceSyncStatusOutput {
    ...
    try aws.signing.signRequest(alloc, &request, creds, client.config.region, "codeconnections");
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceSyncStatusInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("codeconnections", "CodeConnections", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.GetResourceSyncStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceSyncStatusOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetResourceSyncStatusOutput, body, alloc);
}
