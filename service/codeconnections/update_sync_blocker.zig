...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSyncBlockerInput, options: Options) !UpdateSyncBlockerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSyncBlockerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.UpdateSyncBlocker");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSyncBlockerOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateSyncBlockerOutput, body, alloc);
}
