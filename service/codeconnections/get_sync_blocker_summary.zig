...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSyncBlockerSummaryInput, options: Options) !GetSyncBlockerSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSyncBlockerSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.GetSyncBlockerSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSyncBlockerSummaryOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetSyncBlockerSummaryOutput, body, alloc);
}
