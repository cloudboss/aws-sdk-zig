...
pub const GetRepositoryLinkInput = struct {
...
pub const GetRepositoryLinkOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRepositoryLinkInput, options: CallOptions) !GetRepositoryLinkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRepositoryLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeConnections_20231201.GetRepositoryLink");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRepositoryLinkOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetRepositoryLinkOutput, body, allocator);
}
