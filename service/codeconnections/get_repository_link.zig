...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRepositoryLinkInput, options: Options) !GetRepositoryLinkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRepositoryLinkInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.GetRepositoryLink");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRepositoryLinkOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetRepositoryLinkOutput, body, alloc);
}
