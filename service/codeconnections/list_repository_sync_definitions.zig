...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRepositorySyncDefinitionsInput, options: Options) !ListRepositorySyncDefinitionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRepositorySyncDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.ListRepositorySyncDefinitions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRepositorySyncDefinitionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRepositorySyncDefinitionsOutput, body, alloc);
}
