...
pub const DeleteHostInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteHostInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CodeConnections_20231201.DeleteHost");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteHostOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
