...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteHostInput, options: Options) !DeleteHostOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteHostInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.DeleteHost");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteHostOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
