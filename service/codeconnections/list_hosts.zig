...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHostsInput, options: Options) !ListHostsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHostsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.ListHosts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHostsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListHostsOutput, body, alloc);
}
