...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRepositoryLinksInput, options: Options) !ListRepositoryLinksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRepositoryLinksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.ListRepositoryLinks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRepositoryLinksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRepositoryLinksOutput, body, alloc);
}
