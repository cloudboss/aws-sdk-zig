...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRepositorySyncStatusInput, options: Options) !GetRepositorySyncStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRepositorySyncStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.GetRepositorySyncStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRepositorySyncStatusOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetRepositorySyncStatusOutput, body, alloc);
}
