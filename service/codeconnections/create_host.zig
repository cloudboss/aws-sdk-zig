...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateHostInput, options: Options) !CreateHostOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHostInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.CreateHost");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateHostOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateHostOutput, body, alloc);
}
