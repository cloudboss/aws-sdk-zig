...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSyncConfigurationInput, options: Options) !DeleteSyncConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSyncConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CodeConnections_20231201.DeleteSyncConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSyncConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
