...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRuleInput, options: Options) !UpdateRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/listeners/");
    try path_buf.appendSlice(alloc, input.listener_identifier);
    try path_buf.appendSlice(alloc, "/rules/");
    try path_buf.appendSlice(alloc, input.rule_identifier);
    ...
    if (input.match) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"match\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.priority) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRuleOutput {
    var result: UpdateRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
