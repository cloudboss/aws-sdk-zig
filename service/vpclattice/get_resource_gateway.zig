...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceGatewayInput, options: Options) !GetResourceGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceGatewayInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceGatewayOutput {
    var result: GetResourceGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetResourceGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
