...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccessLogSubscriptionInput, options: Options) !GetAccessLogSubscriptionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessLogSubscriptionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessLogSubscriptionOutput {
    var result: GetAccessLogSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAccessLogSubscriptionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
