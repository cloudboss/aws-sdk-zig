...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateResourceGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ipv_4_addresses_per_eni) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ipv4AddressesPerEni\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateResourceGatewayOutput {
    var result: CreateResourceGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateResourceGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
