...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceNetworkInput, options: CallOptions) !UpdateServiceNetworkOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateServiceNetworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateServiceNetworkOutput {
    var result: UpdateServiceNetworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateServiceNetworkOutput, body, allocator);
    }
    _ = status;
    _ = headers;
