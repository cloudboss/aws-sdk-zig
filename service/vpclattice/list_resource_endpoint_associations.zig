...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceEndpointAssociationsInput, options: Options) !ListResourceEndpointAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceEndpointAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "resourceConfigurationIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.resource_configuration_identifier);
    query_has_prev = true;
    if (input.resource_endpoint_association_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "resourceEndpointAssociationIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.vpc_endpoint_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "vpcEndpointId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.vpc_endpoint_owner) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "vpcEndpointOwner=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceEndpointAssociationsOutput {
    var result: ListResourceEndpointAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourceEndpointAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
