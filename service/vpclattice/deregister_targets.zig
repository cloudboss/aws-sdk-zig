...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterTargetsInput, options: Options) !DeregisterTargetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/deregistertargets");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterTargetsOutput {
    var result: DeregisterTargetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeregisterTargetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
