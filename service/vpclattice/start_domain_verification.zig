...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDomainVerificationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDomainVerificationOutput {
    var result: StartDomainVerificationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDomainVerificationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
