...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceNetworkResourceAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceNetworkResourceAssociationOutput {
    var result: GetServiceNetworkResourceAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetServiceNetworkResourceAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
