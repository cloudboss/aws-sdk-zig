...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResourceConfigurationInput, options: Options) !UpdateResourceConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResourceConfigurationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResourceConfigurationOutput {
    var result: UpdateResourceConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResourceConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
