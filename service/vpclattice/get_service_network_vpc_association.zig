...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceNetworkVpcAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceNetworkVpcAssociationOutput {
    var result: GetServiceNetworkVpcAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetServiceNetworkVpcAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
