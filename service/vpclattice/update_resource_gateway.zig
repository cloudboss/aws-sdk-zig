...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateResourceGatewayInput, options: Options) !UpdateResourceGatewayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateResourceGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/resourcegateways/");
    try path_buf.appendSlice(alloc, input.resource_gateway_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateResourceGatewayOutput {
    var result: UpdateResourceGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateResourceGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
