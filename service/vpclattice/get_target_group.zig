...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTargetGroupInput, options: Options) !GetTargetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetTargetGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetTargetGroupOutput {
    var result: GetTargetGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetTargetGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
