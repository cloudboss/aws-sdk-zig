...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTargetGroupsInput, options: Options) !ListTargetGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTargetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.target_group_type) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "targetGroupType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTargetGroupsOutput {
    var result: ListTargetGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTargetGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
