...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.domain_verification_identifier) |v| {
        ...
        try query_buf.appendSlice(alloc, "domainVerificationIdentifier=");
    ...
    if (input.resource_configuration_group_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "resourceConfigurationGroupIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.resource_gateway_identifier) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "resourceGatewayIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceConfigurationsOutput {
    var result: ListResourceConfigurationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListResourceConfigurationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
