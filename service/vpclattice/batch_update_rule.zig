...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchUpdateRuleInput, options: Options) !BatchUpdateRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchUpdateRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchUpdateRuleOutput {
    var result: BatchUpdateRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchUpdateRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
