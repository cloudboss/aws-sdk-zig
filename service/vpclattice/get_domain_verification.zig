...
pub const GetDomainVerificationInput = struct {
    /// The ID or ARN of the domain verification to retrieve.
    domain_verification_identifier: []const u8,
...
pub const GetDomainVerificationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDomainVerificationInput, options: CallOptions) !GetDomainVerificationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDomainVerificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/domainverifications/");
    try path_buf.appendSlice(allocator, input.domain_verification_identifier);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDomainVerificationOutput {
    var result: GetDomainVerificationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainVerificationOutput, body, allocator);
    }
    _ = status;
    _ = headers;
