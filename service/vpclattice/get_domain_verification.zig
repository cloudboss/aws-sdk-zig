...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDomainVerificationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/domainverifications/");
    try path_buf.appendSlice(alloc, input.domain_verification_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDomainVerificationOutput {
    var result: GetDomainVerificationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDomainVerificationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
