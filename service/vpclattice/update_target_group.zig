...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTargetGroupInput, options: Options) !UpdateTargetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTargetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"healthCheck\":");
    try aws.json.writeValue(@TypeOf(input.health_check), input.health_check, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTargetGroupOutput {
    var result: UpdateTargetGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTargetGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
