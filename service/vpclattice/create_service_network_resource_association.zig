...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateServiceNetworkResourceAssociationInput, options: Options) !CreateServiceNetworkResourceAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceNetworkResourceAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"resourceConfigurationIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.resource_configuration_identifier), input.resource_configuration_identifier, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateServiceNetworkResourceAssociationOutput {
    var result: CreateServiceNetworkResourceAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateServiceNetworkResourceAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
