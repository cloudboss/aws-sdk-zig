...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAuthPolicyInput, options: Options) !PutAuthPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAuthPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/authpolicy/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAuthPolicyOutput {
    var result: PutAuthPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutAuthPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
