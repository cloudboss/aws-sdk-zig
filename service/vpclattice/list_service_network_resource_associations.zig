...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceNetworkResourceAssociationsInput, options: Options) !ListServiceNetworkResourceAssociationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServiceNetworkResourceAssociationsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include_children) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeChildren=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServiceNetworkResourceAssociationsOutput {
    var result: ListServiceNetworkResourceAssociationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListServiceNetworkResourceAssociationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
