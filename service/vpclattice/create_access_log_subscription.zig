...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccessLogSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.service_network_log_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"serviceNetworkLogType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccessLogSubscriptionOutput {
    var result: CreateAccessLogSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAccessLogSubscriptionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
