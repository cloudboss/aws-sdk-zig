...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRuleInput, options: Options) !GetRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRuleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRuleOutput {
    var result: GetRuleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRuleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
