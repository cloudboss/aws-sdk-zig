...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteServiceNetworkResourceAssociationInput, options: Options) !DeleteServiceNetworkResourceAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteServiceNetworkResourceAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/servicenetworkresourceassociations/");
    try path_buf.appendSlice(alloc, input.service_network_resource_association_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteServiceNetworkResourceAssociationOutput {
    var result: DeleteServiceNetworkResourceAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteServiceNetworkResourceAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
