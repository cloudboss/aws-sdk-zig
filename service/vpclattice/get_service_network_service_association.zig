...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceNetworkServiceAssociationInput, options: Options) !GetServiceNetworkServiceAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceNetworkServiceAssociationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/servicenetworkserviceassociations/");
    try path_buf.appendSlice(alloc, input.service_network_service_association_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceNetworkServiceAssociationOutput {
    var result: GetServiceNetworkServiceAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetServiceNetworkServiceAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
