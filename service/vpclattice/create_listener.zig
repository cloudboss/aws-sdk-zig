...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateListenerInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"defaultAction\":");
    try aws.json.writeValue(@TypeOf(input.default_action), input.default_action, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateListenerOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateListenerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
