...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateServiceNetworkInput, options: Options) !CreateServiceNetworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceNetworkInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.sharing_config) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"sharingConfig\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateServiceNetworkOutput {
    var result: CreateServiceNetworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateServiceNetworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
