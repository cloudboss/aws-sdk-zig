...
pub const GetListenerInput = struct {
...
pub const GetListenerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetListenerInput, options: Options) !GetListenerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetListenerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetListenerOutput {
    var result: GetListenerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetListenerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
