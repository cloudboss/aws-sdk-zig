...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAuthPolicyInput, options: Options) !GetAuthPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAuthPolicyInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAuthPolicyOutput {
    var result: GetAuthPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetAuthPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
