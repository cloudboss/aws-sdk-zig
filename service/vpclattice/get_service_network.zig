...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceNetworkInput, options: Options) !GetServiceNetworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceNetworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceNetworkOutput {
    var result: GetServiceNetworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetServiceNetworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
