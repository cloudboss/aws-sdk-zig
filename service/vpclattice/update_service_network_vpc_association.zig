...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceNetworkVpcAssociationInput, options: Options) !UpdateServiceNetworkVpcAssociationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceNetworkVpcAssociationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceNetworkVpcAssociationOutput {
    var result: UpdateServiceNetworkVpcAssociationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateServiceNetworkVpcAssociationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
