...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLunaClientInput, options: Options) !DescribeLunaClientOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeLunaClientInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudHsmFrontendService.DescribeLunaClient");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeLunaClientOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeLunaClientOutput, body, alloc);
}
