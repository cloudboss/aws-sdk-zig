...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLunaClientInput, options: Options) !CreateLunaClientOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLunaClientInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudHsmFrontendService.CreateLunaClient");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLunaClientOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLunaClientOutput, body, alloc);
}
