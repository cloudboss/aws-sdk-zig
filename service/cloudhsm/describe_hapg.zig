...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHapgInput, options: Options) !DescribeHapgOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHapgInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudHsmFrontendService.DescribeHapg");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHapgOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeHapgOutput, body, alloc);
}
