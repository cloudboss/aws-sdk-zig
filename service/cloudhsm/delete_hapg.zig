...
pub const DeleteHapgInput = struct {
...
pub const DeleteHapgOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteHapgInput, options: CallOptions) !DeleteHapgOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteHapgInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudHsmFrontendService.DeleteHapg");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteHapgOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DeleteHapgOutput, body, allocator);
}
