...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyHsmInput, options: Options) !ModifyHsmOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyHsmInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudHsmFrontendService.ModifyHsm");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyHsmOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyHsmOutput, body, alloc);
}
