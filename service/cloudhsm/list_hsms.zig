...
pub const ListHsmsInput = struct {
...
pub const ListHsmsOutput = struct {
    /// The list of ARNs that identify the HSMs.
    hsm_list: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListHsmsInput, options: CallOptions) !ListHsmsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListHsmsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudHsmFrontendService.ListHsms");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListHsmsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListHsmsOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "CloudHsmInternalException")) {
        const parsed_error: ?errors.CloudHsmInternalException = aws.json.parseJsonObject(errors.CloudHsmInternalException, body, arena_alloc) catch |err| switch (err) {
