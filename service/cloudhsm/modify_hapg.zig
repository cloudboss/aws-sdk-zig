...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyHapgInput, options: Options) !ModifyHapgOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyHapgInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CloudHsmFrontendService.ModifyHapg");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyHapgOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyHapgOutput, body, alloc);
}
