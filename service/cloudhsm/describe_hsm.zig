...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHsmInput, options: CallOptions) !DescribeHsmOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeHsmInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CloudHsmFrontendService.DescribeHsm");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeHsmOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeHsmOutput, body, allocator);
}
