...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMailboxPermissionsInput, options: Options) !ListMailboxPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMailboxPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.ListMailboxPermissions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMailboxPermissionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMailboxPermissionsOutput, body, alloc);
}
