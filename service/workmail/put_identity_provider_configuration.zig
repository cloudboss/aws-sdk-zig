...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutIdentityProviderConfigurationInput, options: Options) !PutIdentityProviderConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutIdentityProviderConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.PutIdentityProviderConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutIdentityProviderConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
