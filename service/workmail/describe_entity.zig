...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEntityInput, options: Options) !DescribeEntityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEntityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DescribeEntity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEntityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEntityOutput, body, alloc);
}
