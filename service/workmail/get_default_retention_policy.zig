...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDefaultRetentionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.GetDefaultRetentionPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDefaultRetentionPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDefaultRetentionPolicyOutput, body, alloc);
}
