...
pub const GetDefaultRetentionPolicyInput = struct {
    /// The organization ID.
...
pub const GetDefaultRetentionPolicyOutput = struct {
    /// The retention policy description.
    description: ?[]const u8 = null,

    /// The retention policy folder configurations.
    folder_configurations: ?[]const FolderConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDefaultRetentionPolicyInput, options: CallOptions) !GetDefaultRetentionPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDefaultRetentionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.GetDefaultRetentionPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetDefaultRetentionPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDefaultRetentionPolicyOutput, body, allocator);
}
