...
const MailDomainSummary = @import("mail_domain_summary.zig").MailDomainSummary;

pub const ListMailDomainsInput = struct {
...
pub const ListMailDomainsOutput = struct {
    ...
    mail_domains: ?[]const MailDomainSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMailDomainsInput, options: CallOptions) !ListMailDomainsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMailDomainsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.ListMailDomains");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMailDomainsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMailDomainsOutput, body, allocator);
}
