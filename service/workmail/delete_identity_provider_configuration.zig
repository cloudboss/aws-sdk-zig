...
pub const DeleteIdentityProviderConfigurationInput = struct {
    /// The Organization ID.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteIdentityProviderConfigurationInput, options: Options) !DeleteIdentityProviderConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteIdentityProviderConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeleteIdentityProviderConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteIdentityProviderConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
