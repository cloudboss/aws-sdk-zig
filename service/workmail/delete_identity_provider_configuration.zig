...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteIdentityProviderConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DeleteIdentityProviderConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteIdentityProviderConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
