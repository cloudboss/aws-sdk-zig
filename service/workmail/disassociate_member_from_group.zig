...
pub const DisassociateMemberFromGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateMemberFromGroupInput, options: Options) !DisassociateMemberFromGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateMemberFromGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DisassociateMemberFromGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateMemberFromGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
