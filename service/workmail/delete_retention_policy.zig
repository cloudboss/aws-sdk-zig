...
pub const DeleteRetentionPolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRetentionPolicyInput, options: Options) !DeleteRetentionPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRetentionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeleteRetentionPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRetentionPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
