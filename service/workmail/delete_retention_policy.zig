...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRetentionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DeleteRetentionPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRetentionPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
