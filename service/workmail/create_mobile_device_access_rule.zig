...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMobileDeviceAccessRuleInput, options: Options) !CreateMobileDeviceAccessRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMobileDeviceAccessRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.CreateMobileDeviceAccessRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMobileDeviceAccessRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateMobileDeviceAccessRuleOutput, body, alloc);
}
