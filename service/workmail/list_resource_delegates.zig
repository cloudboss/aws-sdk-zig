...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourceDelegatesInput, options: Options) !ListResourceDelegatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourceDelegatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.ListResourceDelegates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourceDelegatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourceDelegatesOutput, body, alloc);
}
