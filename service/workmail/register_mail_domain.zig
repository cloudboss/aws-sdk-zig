...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterMailDomainInput, options: Options) !RegisterMailDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterMailDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.RegisterMailDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterMailDomainOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
