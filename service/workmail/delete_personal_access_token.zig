...
pub const DeletePersonalAccessTokenInput = struct {
    /// The Organization ID.
...
pub const DeletePersonalAccessTokenOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePersonalAccessTokenInput, options: CallOptions) !DeletePersonalAccessTokenOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePersonalAccessTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeletePersonalAccessToken");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePersonalAccessTokenOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
