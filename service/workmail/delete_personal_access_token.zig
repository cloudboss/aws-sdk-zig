...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePersonalAccessTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DeletePersonalAccessToken");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePersonalAccessTokenOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
