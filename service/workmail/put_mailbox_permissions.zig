...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutMailboxPermissionsInput, options: Options) !PutMailboxPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutMailboxPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.PutMailboxPermissions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutMailboxPermissionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
