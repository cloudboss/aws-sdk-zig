...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMobileDeviceAccessRulesInput, options: Options) !ListMobileDeviceAccessRulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMobileDeviceAccessRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.ListMobileDeviceAccessRules");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMobileDeviceAccessRulesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMobileDeviceAccessRulesOutput, body, alloc);
}
