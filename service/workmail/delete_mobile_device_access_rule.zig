...
pub const DeleteMobileDeviceAccessRuleInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMobileDeviceAccessRuleInput, options: Options) !DeleteMobileDeviceAccessRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMobileDeviceAccessRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeleteMobileDeviceAccessRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMobileDeviceAccessRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
