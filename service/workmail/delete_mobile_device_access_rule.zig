...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMobileDeviceAccessRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DeleteMobileDeviceAccessRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMobileDeviceAccessRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
