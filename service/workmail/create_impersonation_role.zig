...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateImpersonationRoleInput, options: Options) !CreateImpersonationRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateImpersonationRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.CreateImpersonationRole");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateImpersonationRoleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateImpersonationRoleOutput, body, alloc);
}
