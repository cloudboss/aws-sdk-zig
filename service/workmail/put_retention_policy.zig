...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRetentionPolicyInput, options: Options) !PutRetentionPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRetentionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.PutRetentionPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRetentionPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
