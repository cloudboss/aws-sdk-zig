...
pub const DeleteIdentityCenterApplicationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteIdentityCenterApplicationInput, options: Options) !DeleteIdentityCenterApplicationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteIdentityCenterApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeleteIdentityCenterApplication");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteIdentityCenterApplicationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
