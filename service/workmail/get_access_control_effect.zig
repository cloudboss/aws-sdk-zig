...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccessControlEffectInput, options: Options) !GetAccessControlEffectOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessControlEffectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.GetAccessControlEffect");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessControlEffectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAccessControlEffectOutput, body, alloc);
}
