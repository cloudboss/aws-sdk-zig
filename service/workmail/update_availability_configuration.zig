...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAvailabilityConfigurationInput, options: Options) !UpdateAvailabilityConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAvailabilityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.UpdateAvailabilityConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAvailabilityConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
