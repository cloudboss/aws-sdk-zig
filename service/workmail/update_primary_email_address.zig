...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePrimaryEmailAddressInput, options: Options) !UpdatePrimaryEmailAddressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePrimaryEmailAddressInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.UpdatePrimaryEmailAddress");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePrimaryEmailAddressOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
