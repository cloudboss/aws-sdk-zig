...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteEmailMonitoringConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DeleteEmailMonitoringConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteEmailMonitoringConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
