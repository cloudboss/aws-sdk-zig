...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIdentityProviderConfigurationInput, options: Options) !DescribeIdentityProviderConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIdentityProviderConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DescribeIdentityProviderConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIdentityProviderConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeIdentityProviderConfigurationOutput, body, alloc);
}
