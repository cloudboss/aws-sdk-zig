...
pub const AccessControlRule = struct {
    ...
    impersonation_role_ids: ?[]const []const u8,
    ...
    /// `ActiveSync`, `AutoDiscover`, `EWS`, `IMAP`,
    /// `SMTP`, `WindowsOutlook`, and `WebMail`.
    not_actions: ?[]const []const u8,

    /// Impersonation role IDs to exclude from the rule.
    not_impersonation_role_ids: ?[]const []const u8,

    /// IPv4 CIDR ranges to exclude from the rule.
    not_ip_ranges: ?[]const []const u8,

    /// User IDs to exclude from the rule.
    not_user_ids: ?[]const []const u8,
