...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssumeImpersonationRoleInput, options: Options) !AssumeImpersonationRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssumeImpersonationRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.AssumeImpersonationRole");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssumeImpersonationRoleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AssumeImpersonationRoleOutput, body, alloc);
}
