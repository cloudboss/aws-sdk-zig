...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMobileDeviceAccessRuleInput, options: Options) !UpdateMobileDeviceAccessRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMobileDeviceAccessRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.UpdateMobileDeviceAccessRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMobileDeviceAccessRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
