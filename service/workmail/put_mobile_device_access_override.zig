...
pub const PutMobileDeviceAccessOverrideInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutMobileDeviceAccessOverrideInput, options: Options) !PutMobileDeviceAccessOverrideOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutMobileDeviceAccessOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.PutMobileDeviceAccessOverride");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutMobileDeviceAccessOverrideOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
