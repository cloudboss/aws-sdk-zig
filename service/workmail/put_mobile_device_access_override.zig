...
fn serializeRequest(alloc: std.mem.Allocator, input: PutMobileDeviceAccessOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.PutMobileDeviceAccessOverride");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutMobileDeviceAccessOverrideOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
