...
pub const DeregisterMailDomainInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterMailDomainInput, options: Options) !DeregisterMailDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterMailDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeregisterMailDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterMailDomainOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
