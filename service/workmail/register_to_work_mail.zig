...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterToWorkMailInput, options: Options) !RegisterToWorkMailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterToWorkMailInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.RegisterToWorkMail");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterToWorkMailOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
