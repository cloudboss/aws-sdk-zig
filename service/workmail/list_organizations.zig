...
const OrganizationSummary = @import("organization_summary.zig").OrganizationSummary;

pub const ListOrganizationsInput = struct {
...
pub const ListOrganizationsOutput = struct {
    ...
    /// summaries.
    organization_summaries: ?[]const OrganizationSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListOrganizationsInput, options: CallOptions) !ListOrganizationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListOrganizationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.ListOrganizations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListOrganizationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListOrganizationsOutput, body, allocator);
}
