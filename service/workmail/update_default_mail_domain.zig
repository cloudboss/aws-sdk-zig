...
pub const UpdateDefaultMailDomainInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDefaultMailDomainInput, options: Options) !UpdateDefaultMailDomainOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateDefaultMailDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.UpdateDefaultMailDomain");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateDefaultMailDomainOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
