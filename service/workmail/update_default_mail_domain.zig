...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDefaultMailDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.UpdateDefaultMailDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDefaultMailDomainOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
