...
const MobileDeviceAccessMatchedRule = @import("mobile_device_access_matched_rule.zig").MobileDeviceAccessMatchedRule;

pub const GetMobileDeviceAccessEffectInput = struct {
    /// Device model the simulated user will report.
    device_model: ?[]const u8 = null,

    /// Device operating system the simulated user will report.
    device_operating_system: ?[]const u8 = null,

    /// Device type the simulated user will report.
    device_type: ?[]const u8 = null,

    /// Device user agent the simulated user will report.
    device_user_agent: ?[]const u8 = null,
...
pub const GetMobileDeviceAccessEffectOutput = struct {
    ...
    /// effect.
    matched_rules: ?[]const MobileDeviceAccessMatchedRule = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMobileDeviceAccessEffectInput, options: CallOptions) !GetMobileDeviceAccessEffectOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "workmail");
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMobileDeviceAccessEffectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.GetMobileDeviceAccessEffect");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMobileDeviceAccessEffectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMobileDeviceAccessEffectOutput, body, allocator);
}
