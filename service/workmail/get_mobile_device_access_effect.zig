...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMobileDeviceAccessEffectInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.GetMobileDeviceAccessEffect");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMobileDeviceAccessEffectOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMobileDeviceAccessEffectOutput, body, alloc);
}
