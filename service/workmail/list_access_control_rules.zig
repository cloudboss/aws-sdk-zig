...
const AccessControlRule = @import("access_control_rule.zig").AccessControlRule;

pub const ListAccessControlRulesInput = struct {
...
pub const ListAccessControlRulesOutput = struct {
    /// The access control rules.
    rules: ?[]const AccessControlRule = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessControlRulesInput, options: CallOptions) !ListAccessControlRulesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAccessControlRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.ListAccessControlRules");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAccessControlRulesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAccessControlRulesOutput, body, allocator);
}
