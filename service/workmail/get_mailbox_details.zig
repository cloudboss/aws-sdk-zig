...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMailboxDetailsInput, options: Options) !GetMailboxDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMailboxDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.GetMailboxDetails");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMailboxDetailsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMailboxDetailsOutput, body, alloc);
}
