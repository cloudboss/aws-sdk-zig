...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMailboxQuotaInput, options: Options) !UpdateMailboxQuotaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMailboxQuotaInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.UpdateMailboxQuota");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMailboxQuotaOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
