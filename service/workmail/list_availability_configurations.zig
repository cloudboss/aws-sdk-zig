...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAvailabilityConfigurationsInput, options: Options) !ListAvailabilityConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAvailabilityConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.ListAvailabilityConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAvailabilityConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAvailabilityConfigurationsOutput, body, alloc);
}
