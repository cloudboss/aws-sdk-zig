...
pub const DeleteAccessControlRuleInput = struct {
...
pub const DeleteAccessControlRuleOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAccessControlRuleInput, options: CallOptions) !DeleteAccessControlRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAccessControlRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeleteAccessControlRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAccessControlRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
