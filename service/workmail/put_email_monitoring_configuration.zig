...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutEmailMonitoringConfigurationInput, options: Options) !PutEmailMonitoringConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutEmailMonitoringConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.PutEmailMonitoringConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutEmailMonitoringConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
