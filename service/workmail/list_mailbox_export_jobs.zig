...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMailboxExportJobsInput, options: Options) !ListMailboxExportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMailboxExportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.ListMailboxExportJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMailboxExportJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMailboxExportJobsOutput, body, alloc);
}
