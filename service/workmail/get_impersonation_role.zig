...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetImpersonationRoleInput, options: Options) !GetImpersonationRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImpersonationRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.GetImpersonationRole");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImpersonationRoleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetImpersonationRoleOutput, body, alloc);
}
