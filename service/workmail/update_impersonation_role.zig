...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateImpersonationRoleInput, options: Options) !UpdateImpersonationRoleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateImpersonationRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.UpdateImpersonationRole");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateImpersonationRoleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
