...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateMemberToGroupInput, options: Options) !AssociateMemberToGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateMemberToGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.AssociateMemberToGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateMemberToGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
