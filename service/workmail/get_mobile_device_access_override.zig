...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMobileDeviceAccessOverrideInput, options: Options) !GetMobileDeviceAccessOverrideOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMobileDeviceAccessOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.GetMobileDeviceAccessOverride");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMobileDeviceAccessOverrideOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMobileDeviceAccessOverrideOutput, body, alloc);
}
