...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMailboxExportJobInput, options: Options) !StartMailboxExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMailboxExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.StartMailboxExportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMailboxExportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMailboxExportJobOutput, body, alloc);
}
