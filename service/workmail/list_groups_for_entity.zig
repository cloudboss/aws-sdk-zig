...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGroupsForEntityInput, options: Options) !ListGroupsForEntityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupsForEntityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.ListGroupsForEntity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupsForEntityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListGroupsForEntityOutput, body, alloc);
}
