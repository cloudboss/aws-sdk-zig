...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMobileDeviceAccessOverrideInput, options: Options) !DeleteMobileDeviceAccessOverrideOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMobileDeviceAccessOverrideInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DeleteMobileDeviceAccessOverride");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMobileDeviceAccessOverrideOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
