...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPersonalAccessTokensInput, options: Options) !ListPersonalAccessTokensOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPersonalAccessTokensInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.ListPersonalAccessTokens");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPersonalAccessTokensOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPersonalAccessTokensOutput, body, alloc);
}
