...
pub const DeleteAvailabilityConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAvailabilityConfigurationInput, options: Options) !DeleteAvailabilityConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAvailabilityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeleteAvailabilityConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAvailabilityConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
