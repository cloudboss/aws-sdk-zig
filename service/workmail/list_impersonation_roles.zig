...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImpersonationRolesInput, options: Options) !ListImpersonationRolesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImpersonationRolesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.ListImpersonationRoles");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImpersonationRolesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListImpersonationRolesOutput, body, alloc);
}
