...
pub const CreateAvailabilityConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAvailabilityConfigurationInput, options: Options) !CreateAvailabilityConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAvailabilityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.CreateAvailabilityConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAvailabilityConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
