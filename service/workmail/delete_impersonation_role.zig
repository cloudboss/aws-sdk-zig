...
pub const DeleteImpersonationRoleInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteImpersonationRoleInput, options: Options) !DeleteImpersonationRoleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteImpersonationRoleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeleteImpersonationRole");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteImpersonationRoleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
