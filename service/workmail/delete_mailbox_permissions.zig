...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteMailboxPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DeleteMailboxPermissions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteMailboxPermissionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
