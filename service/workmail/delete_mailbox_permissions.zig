...
pub const DeleteMailboxPermissionsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMailboxPermissionsInput, options: Options) !DeleteMailboxPermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMailboxPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DeleteMailboxPermissions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMailboxPermissionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
