...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPersonalAccessTokenMetadataInput, options: Options) !GetPersonalAccessTokenMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPersonalAccessTokenMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.GetPersonalAccessTokenMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPersonalAccessTokenMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPersonalAccessTokenMetadataOutput, body, alloc);
}
