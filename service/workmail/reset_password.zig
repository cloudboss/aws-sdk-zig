...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetPasswordInput, options: Options) !ResetPasswordOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResetPasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.ResetPassword");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResetPasswordOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
