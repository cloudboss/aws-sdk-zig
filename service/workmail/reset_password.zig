...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetPasswordInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.ResetPassword");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetPasswordOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
