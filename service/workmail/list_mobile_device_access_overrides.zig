...
pub const ListMobileDeviceAccessOverridesInput = struct {
...
pub const ListMobileDeviceAccessOverridesOutput = struct {
    ...
    overrides: ?[]const MobileDeviceAccessOverride = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMobileDeviceAccessOverridesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.ListMobileDeviceAccessOverrides");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMobileDeviceAccessOverridesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMobileDeviceAccessOverridesOutput, body, allocator);
}
