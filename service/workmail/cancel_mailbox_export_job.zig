...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelMailboxExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.CancelMailboxExportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelMailboxExportJobOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
