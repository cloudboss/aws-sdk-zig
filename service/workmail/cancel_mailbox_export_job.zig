...
pub const CancelMailboxExportJobInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelMailboxExportJobInput, options: CallOptions) !CancelMailboxExportJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelMailboxExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.CancelMailboxExportJob");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelMailboxExportJobOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
