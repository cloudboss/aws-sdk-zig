...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMailboxExportJobInput, options: Options) !DescribeMailboxExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMailboxExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.DescribeMailboxExportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMailboxExportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMailboxExportJobOutput, body, alloc);
}
