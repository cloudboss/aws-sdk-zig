...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMailDomainInput, options: Options) !GetMailDomainOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMailDomainInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.GetMailDomain");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMailDomainOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetMailDomainOutput, body, alloc);
}
