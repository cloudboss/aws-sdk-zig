...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TestAvailabilityConfigurationInput, options: Options) !TestAvailabilityConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TestAvailabilityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.TestAvailabilityConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TestAvailabilityConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TestAvailabilityConfigurationOutput, body, alloc);
}
