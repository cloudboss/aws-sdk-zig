...
pub const DescribeInboundDmarcSettingsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInboundDmarcSettingsInput, options: CallOptions) !DescribeInboundDmarcSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInboundDmarcSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.DescribeInboundDmarcSettings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInboundDmarcSettingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInboundDmarcSettingsOutput, body, allocator);
}
