...
pub const PutInboundDmarcSettingsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutInboundDmarcSettingsInput, options: Options) !PutInboundDmarcSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutInboundDmarcSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.PutInboundDmarcSettings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutInboundDmarcSettingsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
