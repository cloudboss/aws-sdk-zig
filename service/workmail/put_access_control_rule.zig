...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccessControlRuleInput, options: Options) !PutAccessControlRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccessControlRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "WorkMailService.PutAccessControlRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccessControlRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
