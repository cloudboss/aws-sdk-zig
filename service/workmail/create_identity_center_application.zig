...
pub const CreateIdentityCenterApplicationInput = struct {
...
pub const CreateIdentityCenterApplicationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIdentityCenterApplicationInput, options: CallOptions) !CreateIdentityCenterApplicationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateIdentityCenterApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "WorkMailService.CreateIdentityCenterApplication");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateIdentityCenterApplicationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateIdentityCenterApplicationOutput, body, allocator);
}
