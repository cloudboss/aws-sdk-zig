...
pub const ListSimulationsPaginator = struct {
    client: *Client,
    params: list_simulations.ListSimulationsInput,
    next_token: ?[]const u8 = null,
    done: bool = false,
    allocator: std.mem.Allocator,
    ...
    pub fn next(self: *Self, allocator: std.mem.Allocator, options: CallOptions) !list_simulations.ListSimulationsOutput {
        if (self.done) {
            return error.EndOfPagination;
        }
        ...
        const output = try list_simulations.execute(self.client, allocator, self.params, options);
