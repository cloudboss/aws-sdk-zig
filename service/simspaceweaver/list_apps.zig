...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAppsInput, options: Options) !ListAppsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAppsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAppsOutput {
    var result: ListAppsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAppsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
