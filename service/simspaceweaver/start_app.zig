...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartAppInput, options: Options) !StartAppOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartAppInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.launch_overrides) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"LaunchOverrides\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartAppOutput {
    var result: StartAppOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartAppOutput, body, alloc);
    }
    _ = status;
    _ = headers;
