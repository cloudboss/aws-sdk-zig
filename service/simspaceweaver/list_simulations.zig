...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSimulationsInput, options: Options) !ListSimulationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSimulationsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSimulationsOutput {
    var result: ListSimulationsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSimulationsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
