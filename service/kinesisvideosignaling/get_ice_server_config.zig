...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIceServerConfigInput, options: Options) !GetIceServerConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIceServerConfigInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("kinesisvideosignaling", "Kinesis Video Signaling", alloc);
    ...
    try body_buf.appendSlice(alloc, "\"ChannelARN\":");
    try aws.json.writeValue(@TypeOf(input.channel_arn), input.channel_arn, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.service) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Service\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.username) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIceServerConfigOutput {
    var result: GetIceServerConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIceServerConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
