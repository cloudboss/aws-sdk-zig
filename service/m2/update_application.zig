...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"currentApplicationVersion\":");
    try aws.json.writeValue(@TypeOf(input.current_application_version), input.current_application_version, alloc, &body_buf);
    has_prev = true;
