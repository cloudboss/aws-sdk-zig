...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataSetExportHistoryInput, options: Options) !ListDataSetExportHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataSetExportHistoryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataSetExportHistoryOutput {
    var result: ListDataSetExportHistoryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataSetExportHistoryOutput, body, alloc);
    }
    _ = status;
    _ = headers;
