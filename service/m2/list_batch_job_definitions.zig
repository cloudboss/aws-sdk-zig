...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBatchJobDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batch-job-definitions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBatchJobDefinitionsOutput {
    var result: ListBatchJobDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBatchJobDefinitionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
