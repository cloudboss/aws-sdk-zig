...
pub const ListBatchJobDefinitionsInput = struct {
...
pub const ListBatchJobDefinitionsOutput = struct {
    /// The list of batch job definitions.
    batch_job_definitions: ?[]const BatchJobDefinition = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBatchJobDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/batch-job-definitions");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBatchJobDefinitionsOutput {
    var result: ListBatchJobDefinitionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBatchJobDefinitionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
