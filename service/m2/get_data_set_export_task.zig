...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataSetExportTaskInput, options: Options) !GetDataSetExportTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataSetExportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dataset-export-tasks/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataSetExportTaskOutput {
    var result: GetDataSetExportTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataSetExportTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
