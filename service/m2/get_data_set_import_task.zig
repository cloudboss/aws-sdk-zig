...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataSetImportTaskInput, options: Options) !GetDataSetImportTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataSetImportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dataset-import-tasks/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataSetImportTaskOutput {
    var result: GetDataSetImportTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataSetImportTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
