...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBatchJobExecutionsInput, options: CallOptions) !ListBatchJobExecutionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBatchJobExecutionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBatchJobExecutionsOutput {
    var result: ListBatchJobExecutionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBatchJobExecutionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
