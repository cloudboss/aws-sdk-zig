...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetApplicationVersionInput, options: Options) !GetApplicationVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetApplicationVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.application_version);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetApplicationVersionOutput {
    var result: GetApplicationVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetApplicationVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
