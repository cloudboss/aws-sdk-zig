...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBatchJobExecutionInput, options: Options) !GetBatchJobExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBatchJobExecutionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBatchJobExecutionOutput {
    var result: GetBatchJobExecutionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBatchJobExecutionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
