...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBatchJobRestartPointsInput, options: Options) !ListBatchJobRestartPointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBatchJobRestartPointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batch-job-executions/");
    ...
    if (input.auth_secrets_manager_arn) |v| {
        ...
        try query_buf.appendSlice(alloc, "authSecretsManagerArn=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBatchJobRestartPointsOutput {
    var result: ListBatchJobRestartPointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBatchJobRestartPointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
