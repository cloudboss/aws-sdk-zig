...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataSetImportTaskInput, options: CallOptions) !CreateDataSetImportTaskOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "m2");
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDataSetImportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/dataset-import-task");
    ...
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"importConfig\":");
    try aws.json.writeValue(@TypeOf(input.import_config), input.import_config, allocator, &body_buf);
    has_prev = true;
