...
const CreateDeploymentInput = @import("create_deployment_request.zig").CreateDeploymentRequest;

const CreateDeploymentOutput = @import("create_deployment_response.zig").CreateDeploymentResponse;
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"applicationVersion\":");
    try aws.json.writeValue(@TypeOf(input.application_version), input.application_version, allocator, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.environment_id), input.environment_id, allocator, &body_buf);
    has_prev = true;
