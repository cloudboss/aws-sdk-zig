...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"applicationVersion\":");
    try aws.json.writeValue(@TypeOf(input.application_version), input.application_version, alloc, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.environment_id), input.environment_id, alloc, &body_buf);
    has_prev = true;
