...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartBatchJobInput, options: Options) !StartBatchJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartBatchJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batch-job");
    ...
    if (input.auth_secrets_manager_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"authSecretsManagerArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"batchJobIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.batch_job_identifier), input.batch_job_identifier, alloc, &body_buf);
    has_prev = true;
    if (input.job_params) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"jobParams\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartBatchJobOutput {
    var result: StartBatchJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartBatchJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
