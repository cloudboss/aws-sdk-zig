...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataSetDetailsInput, options: Options) !GetDataSetDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataSetDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.data_set_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataSetDetailsOutput {
    var result: GetDataSetDetailsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataSetDetailsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
