...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataSetExportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/dataset-export-task");
    ...
    try aws.json.writeValue(@TypeOf(input.export_config), input.export_config, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataSetExportTaskOutput {
    var result: CreateDataSetExportTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataSetExportTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
