...
const DataSetExportConfig = @import("data_set_export_config.zig").DataSetExportConfig;

pub const CreateDataSetExportTaskInput = struct {
...
pub const CreateDataSetExportTaskOutput = struct {
    /// The task identifier. This operation is asynchronous. Use this identifier
    /// with the GetDataSetExportTask operation to obtain the status of this task.
    task_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataSetExportTaskInput, options: CallOptions) !CreateDataSetExportTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDataSetExportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/dataset-export-task");
    ...
    try aws.json.writeValue(@TypeOf(input.export_config), input.export_config, allocator, &body_buf);
    has_prev = true;
