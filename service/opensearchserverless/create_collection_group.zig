...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCollectionGroupInput, options: Options) !CreateCollectionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCollectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.CreateCollectionGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCollectionGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCollectionGroupOutput, body, alloc);
}
