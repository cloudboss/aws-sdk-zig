...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetVpcEndpointInput, options: Options) !BatchGetVpcEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetVpcEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.BatchGetVpcEndpoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetVpcEndpointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetVpcEndpointOutput, body, alloc);
}
