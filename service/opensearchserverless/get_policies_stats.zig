...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPoliciesStatsInput, options: CallOptions) !GetPoliciesStatsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPoliciesStatsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OpenSearchServerless.GetPoliciesStats");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPoliciesStatsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPoliciesStatsOutput, body, allocator);
}
