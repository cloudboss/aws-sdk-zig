...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetEffectiveLifecyclePolicyInput, options: Options) !BatchGetEffectiveLifecyclePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetEffectiveLifecyclePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.BatchGetEffectiveLifecyclePolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetEffectiveLifecyclePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetEffectiveLifecyclePolicyOutput, body, alloc);
}
