...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccessPolicyInput, options: Options) !GetAccessPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccessPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.GetAccessPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccessPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAccessPolicyOutput, body, alloc);
}
