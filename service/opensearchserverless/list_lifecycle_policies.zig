...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLifecyclePoliciesInput, options: Options) !ListLifecyclePoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLifecyclePoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.ListLifecyclePolicies");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLifecyclePoliciesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLifecyclePoliciesOutput, body, alloc);
}
