...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccessPolicyInput, options: Options) !UpdateAccessPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccessPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.UpdateAccessPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccessPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateAccessPolicyOutput, body, alloc);
}
