...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteCollectionInput, options: Options) !DeleteCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.DeleteCollection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteCollectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteCollectionOutput, body, alloc);
}
