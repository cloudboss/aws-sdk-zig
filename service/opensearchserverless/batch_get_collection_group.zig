...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetCollectionGroupInput, options: Options) !BatchGetCollectionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetCollectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.BatchGetCollectionGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetCollectionGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetCollectionGroupOutput, body, alloc);
}
