...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCollectionInput, options: Options) !UpdateCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.UpdateCollection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCollectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCollectionOutput, body, alloc);
}
