...
pub const UpdateSecurityPolicyInput = struct {
...
pub const UpdateSecurityPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSecurityPolicyInput, options: CallOptions) !UpdateSecurityPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateSecurityPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OpenSearchServerless.UpdateSecurityPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateSecurityPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSecurityPolicyOutput, body, allocator);
}
