...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCollectionInput, options: Options) !CreateCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.CreateCollection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCollectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateCollectionOutput, body, alloc);
}
