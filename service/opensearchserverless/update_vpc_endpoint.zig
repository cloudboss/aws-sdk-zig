...
const UpdateVpcEndpointDetail = @import("update_vpc_endpoint_detail.zig").UpdateVpcEndpointDetail;
...
pub const UpdateVpcEndpointOutput = struct {
    ...
    update_vpc_endpoint_detail: ?UpdateVpcEndpointDetail = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateVpcEndpointInput, options: Options) !UpdateVpcEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVpcEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.UpdateVpcEndpoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVpcEndpointOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateVpcEndpointOutput, body, alloc);
}
