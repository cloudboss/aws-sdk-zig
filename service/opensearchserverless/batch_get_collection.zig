...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetCollectionInput, options: Options) !BatchGetCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.BatchGetCollection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetCollectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetCollectionOutput, body, alloc);
}
