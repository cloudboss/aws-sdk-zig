...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSecurityPolicyInput, options: Options) !GetSecurityPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSecurityPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.GetSecurityPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSecurityPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSecurityPolicyOutput, body, alloc);
}
