...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSecurityConfigInput, options: Options) !UpdateSecurityConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSecurityConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.UpdateSecurityConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSecurityConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSecurityConfigOutput, body, alloc);
}
