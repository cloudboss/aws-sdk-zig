...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSecurityConfigInput, options: Options) !GetSecurityConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSecurityConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.GetSecurityConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSecurityConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSecurityConfigOutput, body, alloc);
}
