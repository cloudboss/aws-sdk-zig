...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSecurityConfigInput, options: Options) !DeleteSecurityConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSecurityConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.DeleteSecurityConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSecurityConfigOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
