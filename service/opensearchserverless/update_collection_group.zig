...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCollectionGroupInput, options: Options) !UpdateCollectionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCollectionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "OpenSearchServerless.UpdateCollectionGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCollectionGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateCollectionGroupOutput, body, alloc);
}
