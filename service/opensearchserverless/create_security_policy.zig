...
pub const CreateSecurityPolicyInput = struct {
...
pub const CreateSecurityPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSecurityPolicyInput, options: CallOptions) !CreateSecurityPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSecurityPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "OpenSearchServerless.CreateSecurityPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateSecurityPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateSecurityPolicyOutput, body, allocator);
}
