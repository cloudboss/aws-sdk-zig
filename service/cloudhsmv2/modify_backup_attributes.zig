...
pub const ModifyBackupAttributesInput = struct {
...
pub const ModifyBackupAttributesOutput = struct {
    backup: ?Backup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyBackupAttributesInput, options: CallOptions) !ModifyBackupAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyBackupAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "BaldrApiService.ModifyBackupAttributes");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyBackupAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ModifyBackupAttributesOutput, body, allocator);
}
