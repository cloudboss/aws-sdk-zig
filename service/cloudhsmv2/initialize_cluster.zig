...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: InitializeClusterInput, options: Options) !InitializeClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: InitializeClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "BaldrApiService.InitializeCluster");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InitializeClusterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(InitializeClusterOutput, body, alloc);
}
