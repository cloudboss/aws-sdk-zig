...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyBackupToRegionInput, options: Options) !CopyBackupToRegionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyBackupToRegionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "BaldrApiService.CopyBackupToRegion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyBackupToRegionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CopyBackupToRegionOutput, body, alloc);
}
