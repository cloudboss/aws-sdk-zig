...
pub const RestoreBackupInput = struct {
...
pub const RestoreBackupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreBackupInput, options: CallOptions) !RestoreBackupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RestoreBackupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "BaldrApiService.RestoreBackup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RestoreBackupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RestoreBackupOutput, body, allocator);
}
