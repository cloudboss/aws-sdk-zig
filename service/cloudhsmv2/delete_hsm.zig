...
pub const DeleteHsmInput = struct {
...
pub const DeleteHsmOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteHsmInput, options: CallOptions) !DeleteHsmOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteHsmInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "BaldrApiService.DeleteHsm");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteHsmOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteHsmOutput, body, allocator);
}
