...
pub const Certificates = struct {
    ...
    aws_hardware_certificate: ?[]const u8 = null,

    /// The cluster certificate issued (signed) by the issuing certificate authority
    /// (CA) of
    /// the cluster's owner.
    cluster_certificate: ?[]const u8 = null,

    /// The cluster's certificate signing request (CSR). The CSR exists only when
    /// the cluster's
    /// state is `UNINITIALIZED`.
    cluster_csr: ?[]const u8 = null,

    /// The HSM certificate issued (signed) by the HSM hardware.
    hsm_certificate: ?[]const u8 = null,

    /// The HSM hardware certificate issued (signed) by the hardware manufacturer.
    manufacturer_hardware_certificate: ?[]const u8 = null,
