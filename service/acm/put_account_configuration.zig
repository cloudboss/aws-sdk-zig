...
pub const PutAccountConfigurationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAccountConfigurationInput, options: CallOptions) !PutAccountConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutAccountConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CertificateManager.PutAccountConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutAccountConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
