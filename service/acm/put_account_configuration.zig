...
fn serializeRequest(alloc: std.mem.Allocator, input: PutAccountConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CertificateManager.PutAccountConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutAccountConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
