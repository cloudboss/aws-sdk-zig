...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccountConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CertificateManager.GetAccountConfiguration");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccountConfigurationOutput {
    ...
    return aws.json.parseJsonObject(GetAccountConfigurationOutput, body, alloc);
}
