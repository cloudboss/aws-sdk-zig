...
fn serializeRequest(alloc: std.mem.Allocator, input: ResendValidationEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CertificateManager.ResendValidationEmail");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResendValidationEmailOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
