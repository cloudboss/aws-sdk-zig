...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCertificateInput, options: Options) !GetCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CertificateManager.GetCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCertificateOutput, body, alloc);
}
