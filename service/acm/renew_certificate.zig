...
pub const RenewCertificateInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RenewCertificateInput, options: CallOptions) !RenewCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RenewCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CertificateManager.RenewCertificate");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RenewCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
