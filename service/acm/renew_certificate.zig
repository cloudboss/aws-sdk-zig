...
fn serializeRequest(alloc: std.mem.Allocator, input: RenewCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CertificateManager.RenewCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RenewCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
