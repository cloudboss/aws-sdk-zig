...
fn serializeRequest(alloc: std.mem.Allocator, input: AddTagsToCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CertificateManager.AddTagsToCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddTagsToCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
