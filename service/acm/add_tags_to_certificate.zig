...
pub const AddTagsToCertificateInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddTagsToCertificateInput, options: CallOptions) !AddTagsToCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AddTagsToCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CertificateManager.AddTagsToCertificate");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AddTagsToCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
