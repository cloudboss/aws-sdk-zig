...
pub const ListTagsForCertificateInput = struct {
...
pub const ListTagsForCertificateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTagsForCertificateInput, options: CallOptions) !ListTagsForCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTagsForCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CertificateManager.ListTagsForCertificate");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTagsForCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTagsForCertificateOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "InvalidDomainValidationOptionsException")) {
        const parsed_error: ?errors.InvalidDomainValidationOptionsException = aws.json.parseJsonObject(errors.InvalidDomainValidationOptionsException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .invalid_domain_validation_options_exception = typed_error } };
        }
    }
