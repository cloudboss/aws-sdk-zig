...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCertificateOptionsInput, options: Options) !UpdateCertificateOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCertificateOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CertificateManager.UpdateCertificateOptions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCertificateOptionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
