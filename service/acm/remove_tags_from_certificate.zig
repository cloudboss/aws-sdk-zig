...
pub const RemoveTagsFromCertificateInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveTagsFromCertificateInput, options: CallOptions) !RemoveTagsFromCertificateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveTagsFromCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "CertificateManager.RemoveTagsFromCertificate");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveTagsFromCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
