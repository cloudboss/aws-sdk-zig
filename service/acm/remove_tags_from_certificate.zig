...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveTagsFromCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CertificateManager.RemoveTagsFromCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveTagsFromCertificateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
