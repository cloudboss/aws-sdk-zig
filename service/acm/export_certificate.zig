...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExportCertificateInput, options: Options) !ExportCertificateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExportCertificateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "CertificateManager.ExportCertificate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ExportCertificateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ExportCertificateOutput, body, alloc);
}
