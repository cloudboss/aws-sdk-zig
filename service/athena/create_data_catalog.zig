...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataCatalogInput, options: CallOptions) !CreateDataCatalogOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDataCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonAthena.CreateDataCatalog");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateDataCatalogOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDataCatalogOutput, body, allocator);
}
