...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePresignedNotebookUrlInput, options: Options) !CreatePresignedNotebookUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePresignedNotebookUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.CreatePresignedNotebookUrl");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePresignedNotebookUrlOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreatePresignedNotebookUrlOutput, body, alloc);
}
