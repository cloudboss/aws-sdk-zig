...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetNamedQueryInput, options: Options) !GetNamedQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetNamedQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.GetNamedQuery");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetNamedQueryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetNamedQueryOutput, body, alloc);
}
