...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelCapacityReservationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.CancelCapacityReservation");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelCapacityReservationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
