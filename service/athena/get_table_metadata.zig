...
pub const GetTableMetadataInput = struct {
...
pub const GetTableMetadataOutput = struct {
    /// An object that contains table metadata.
    table_metadata: ?TableMetadata = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetTableMetadataInput, options: CallOptions) !GetTableMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetTableMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonAthena.GetTableMetadata");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetTableMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetTableMetadataOutput, body, allocator);
}
