...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDataCatalogInput, options: Options) !DeleteDataCatalogOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDataCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.DeleteDataCatalog");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDataCatalogOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteDataCatalogOutput, body, alloc);
}
