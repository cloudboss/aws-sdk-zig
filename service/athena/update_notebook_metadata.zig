...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNotebookMetadataInput, options: Options) !UpdateNotebookMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNotebookMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.UpdateNotebookMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNotebookMetadataOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
