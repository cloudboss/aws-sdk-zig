...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetResourceDashboardInput, options: Options) !GetResourceDashboardOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetResourceDashboardInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.GetResourceDashboard");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetResourceDashboardOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetResourceDashboardOutput, body, alloc);
}
