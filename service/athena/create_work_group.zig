...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWorkGroupInput, options: Options) !CreateWorkGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWorkGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.CreateWorkGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWorkGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
