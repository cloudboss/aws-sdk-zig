...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportNotebookInput, options: Options) !ImportNotebookOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ImportNotebookInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.ImportNotebook");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ImportNotebookOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ImportNotebookOutput, body, alloc);
}
