...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartQueryExecutionInput, options: Options) !StartQueryExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartQueryExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.StartQueryExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartQueryExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartQueryExecutionOutput, body, alloc);
}
