...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDataCatalogInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.UpdateDataCatalog");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDataCatalogOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
