...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExecutorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.ListExecutors");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExecutorsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListExecutorsOutput, body, alloc);
}
