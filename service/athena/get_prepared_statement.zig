...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPreparedStatementInput, options: Options) !GetPreparedStatementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPreparedStatementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.GetPreparedStatement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPreparedStatementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPreparedStatementOutput, body, alloc);
}
