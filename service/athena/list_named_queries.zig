...
pub const ListNamedQueriesInput = struct {
...
pub const ListNamedQueriesOutput = struct {
    /// The list of unique query IDs.
    named_query_ids: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListNamedQueriesInput, options: CallOptions) !ListNamedQueriesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListNamedQueriesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonAthena.ListNamedQueries");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListNamedQueriesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListNamedQueriesOutput, body, allocator);
}
