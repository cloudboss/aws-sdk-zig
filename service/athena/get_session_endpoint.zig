...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSessionEndpointInput, options: Options) !GetSessionEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSessionEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.GetSessionEndpoint");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSessionEndpointOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetSessionEndpointOutput, body, alloc);
}
