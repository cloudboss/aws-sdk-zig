...
pub const ListNotebookMetadataInput = struct {
...
pub const ListNotebookMetadataOutput = struct {
    /// A token generated by the Athena service that specifies where to continue
    ...
    notebook_metadata_list: ?[]const NotebookMetadata = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .notebook_metadata_list = "NotebookMetadataList",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListNotebookMetadataInput, options: Options) !ListNotebookMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListNotebookMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.ListNotebookMetadata");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListNotebookMetadataOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListNotebookMetadataOutput, body, alloc);
}
