...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCapacityReservationsInput, options: CallOptions) !ListCapacityReservationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCapacityReservationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonAthena.ListCapacityReservations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCapacityReservationsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListCapacityReservationsOutput, body, allocator);
}
