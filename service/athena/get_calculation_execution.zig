...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCalculationExecutionInput, options: Options) !GetCalculationExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCalculationExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.GetCalculationExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCalculationExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCalculationExecutionOutput, body, alloc);
}
