...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetNamedQueryInput, options: Options) !BatchGetNamedQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetNamedQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.BatchGetNamedQuery");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetNamedQueryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetNamedQueryOutput, body, alloc);
}
