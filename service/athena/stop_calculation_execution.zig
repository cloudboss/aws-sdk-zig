...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopCalculationExecutionInput, options: Options) !StopCalculationExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopCalculationExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.StopCalculationExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopCalculationExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopCalculationExecutionOutput, body, alloc);
}
