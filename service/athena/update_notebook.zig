...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNotebookInput, options: Options) !UpdateNotebookOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNotebookInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.UpdateNotebook");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNotebookOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
