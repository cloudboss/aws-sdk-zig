...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateNotebookInput, options: Options) !CreateNotebookOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNotebookInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.CreateNotebook");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateNotebookOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateNotebookOutput, body, alloc);
}
