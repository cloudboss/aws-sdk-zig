...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopQueryExecutionInput, options: Options) !StopQueryExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopQueryExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.StopQueryExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopQueryExecutionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
