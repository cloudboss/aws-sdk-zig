...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCalculationExecutionStatusInput, options: Options) !GetCalculationExecutionStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCalculationExecutionStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonAthena.GetCalculationExecutionStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCalculationExecutionStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetCalculationExecutionStatusOutput, body, alloc);
}
