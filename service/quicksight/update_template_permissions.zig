...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTemplatePermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTemplatePermissionsOutput {
    var result: UpdateTemplatePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTemplatePermissionsOutput, body, alloc);
    }
