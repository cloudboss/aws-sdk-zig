...
pub const DescribeFolderResolvedPermissionsInput = struct {
...
pub const DescribeFolderResolvedPermissionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFolderResolvedPermissionsInput, options: CallOptions) !DescribeFolderResolvedPermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFolderResolvedPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/resolved-permissions");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFolderResolvedPermissionsOutput {
    var result: DescribeFolderResolvedPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFolderResolvedPermissionsOutput, body, allocator);
    }
