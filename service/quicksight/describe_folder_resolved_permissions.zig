...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFolderResolvedPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/resolved-permissions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFolderResolvedPermissionsOutput {
    var result: DescribeFolderResolvedPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFolderResolvedPermissionsOutput, body, alloc);
    }
