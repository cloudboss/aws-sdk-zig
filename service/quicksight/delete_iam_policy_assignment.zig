...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteIAMPolicyAssignmentInput, options: CallOptions) !DeleteIAMPolicyAssignmentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteIAMPolicyAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/namespace/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteIAMPolicyAssignmentOutput {
    var result: DeleteIAMPolicyAssignmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteIAMPolicyAssignmentOutput, body, allocator);
    }
