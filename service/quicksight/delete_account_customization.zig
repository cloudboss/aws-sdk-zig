...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAccountCustomizationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAccountCustomizationOutput {
    var result: DeleteAccountCustomizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAccountCustomizationOutput, body, alloc);
    }
