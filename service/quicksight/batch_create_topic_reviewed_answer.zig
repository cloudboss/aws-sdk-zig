...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchCreateTopicReviewedAnswerInput, options: Options) !BatchCreateTopicReviewedAnswerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchCreateTopicReviewedAnswerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.topic_id);
    try path_buf.appendSlice(alloc, "/batch-create-reviewed-answers");
    ...
    try body_buf.appendSlice(alloc, "\"Answers\":");
    try aws.json.writeValue(@TypeOf(input.answers), input.answers, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchCreateTopicReviewedAnswerOutput {
    var result: BatchCreateTopicReviewedAnswerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchCreateTopicReviewedAnswerOutput, body, alloc);
    }
