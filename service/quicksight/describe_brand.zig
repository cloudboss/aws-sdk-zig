...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBrandInput, options: Options) !DescribeBrandOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBrandInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBrandOutput {
    var result: DescribeBrandOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBrandOutput, body, alloc);
    }
    _ = status;
    _ = headers;
