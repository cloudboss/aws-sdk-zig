...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIAMPolicyAssignmentInput, options: Options) !DescribeIAMPolicyAssignmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIAMPolicyAssignmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIAMPolicyAssignmentOutput {
    var result: DescribeIAMPolicyAssignmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeIAMPolicyAssignmentOutput, body, alloc);
    }
