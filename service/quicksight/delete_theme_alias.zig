...
pub const DeleteThemeAliasInput = struct {
...
pub const DeleteThemeAliasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteThemeAliasInput, options: Options) !DeleteThemeAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteThemeAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteThemeAliasOutput {
    var result: DeleteThemeAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteThemeAliasOutput, body, alloc);
    }
