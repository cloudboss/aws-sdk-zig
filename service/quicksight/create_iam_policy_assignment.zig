...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateIAMPolicyAssignmentInput, options: Options) !CreateIAMPolicyAssignmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIAMPolicyAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"AssignmentName\":");
    try aws.json.writeValue(@TypeOf(input.assignment_name), input.assignment_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"AssignmentStatus\":");
    try aws.json.writeValue(@TypeOf(input.assignment_status), input.assignment_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateIAMPolicyAssignmentOutput {
    var result: CreateIAMPolicyAssignmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateIAMPolicyAssignmentOutput, body, alloc);
    }
