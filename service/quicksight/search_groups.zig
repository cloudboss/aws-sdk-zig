...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchGroupsInput, options: Options) !SearchGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/groups-search");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchGroupsOutput {
    var result: SearchGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchGroupsOutput, body, alloc);
    }
