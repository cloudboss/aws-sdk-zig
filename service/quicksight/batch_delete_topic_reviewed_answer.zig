...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteTopicReviewedAnswerInput, options: Options) !BatchDeleteTopicReviewedAnswerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteTopicReviewedAnswerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/batch-delete-reviewed-answers");
    ...
    if (input.answer_ids) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AnswerIds\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteTopicReviewedAnswerOutput {
    var result: BatchDeleteTopicReviewedAnswerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteTopicReviewedAnswerOutput, body, alloc);
    }
