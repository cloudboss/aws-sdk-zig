...
pub const DescribeNamespaceOutput = struct {
    ...
    /// includes
    /// the namespace ARN, name, Amazon Web Services Region, creation status, and
    /// identity store. `DescribeNamespace` also
    /// works for namespaces that are in the process of being created. For
    /// incomplete namespaces,
    /// this API operation lists the namespace error types and messages associated
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNamespaceInput, options: Options) !DescribeNamespaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNamespaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNamespaceOutput {
    var result: DescribeNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeNamespaceOutput, body, alloc);
    }
