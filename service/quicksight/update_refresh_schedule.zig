...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRefreshScheduleInput, options: Options) !UpdateRefreshScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRefreshScheduleOutput {
    var result: UpdateRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRefreshScheduleOutput, body, alloc);
    }
