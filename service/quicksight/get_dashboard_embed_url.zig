...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDashboardEmbedUrlInput, options: Options) !GetDashboardEmbedUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDashboardEmbedUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/embed-url");
    ...
    if (input.additional_dashboard_ids) |v| {
        ...
        try query_buf.appendSlice(alloc, "additional-dashboard-ids=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "creds-type=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.identity_type));
    query_has_prev = true;
    ...
    if (input.reset_disabled) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "reset-disabled=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.session_lifetime_in_minutes) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "session-lifetime=");
        {
    ...
    if (input.state_persistence_enabled) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "state-persistence-enabled=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.undo_redo_disabled) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "undo-redo-disabled=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.user_arn) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "user-arn=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDashboardEmbedUrlOutput {
    var result: GetDashboardEmbedUrlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDashboardEmbedUrlOutput, body, alloc);
    }
