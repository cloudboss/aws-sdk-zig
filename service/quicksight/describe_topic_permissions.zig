...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTopicPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTopicPermissionsOutput {
    var result: DescribeTopicPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTopicPermissionsOutput, body, alloc);
    }
