...
const QSearchStatus = @import("q_search_status.zig").QSearchStatus;

pub const UpdateQuickSightQSearchConfigurationInput = struct {
...
pub const UpdateQuickSightQSearchConfigurationOutput = struct {
    /// The status of the Quick Sight Q Search configuration.
    q_search_status: ?QSearchStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateQuickSightQSearchConfigurationInput, options: Options) !UpdateQuickSightQSearchConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQuickSightQSearchConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/quicksight-q-search-configuration");
    ...
    try body_buf.appendSlice(alloc, "\"QSearchStatus\":");
    try aws.json.writeValue(@TypeOf(input.q_search_status), input.q_search_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQuickSightQSearchConfigurationOutput {
    var result: UpdateQuickSightQSearchConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQuickSightQSearchConfigurationOutput, body, alloc);
    }
