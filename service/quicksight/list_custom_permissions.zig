...
const CustomPermissions = @import("custom_permissions.zig").CustomPermissions;

pub const ListCustomPermissionsInput = struct {
...
pub const ListCustomPermissionsOutput = struct {
    /// A list of custom permissions profiles.
    custom_permissions_list: ?[]const CustomPermissions = null,
    ...
    pub const json_field_names = .{
        .custom_permissions_list = "CustomPermissionsList",
        .next_token = "NextToken",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCustomPermissionsInput, options: CallOptions) !ListCustomPermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/custom-permissions");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomPermissionsOutput {
    var result: ListCustomPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomPermissionsOutput, body, allocator);
    }
