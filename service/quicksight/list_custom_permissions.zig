...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCustomPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/custom-permissions");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCustomPermissionsOutput {
    var result: ListCustomPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListCustomPermissionsOutput, body, alloc);
    }
