...
pub const CreateCustomPermissionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomPermissionsInput, options: Options) !CreateCustomPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomPermissionsOutput {
    var result: CreateCustomPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCustomPermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
