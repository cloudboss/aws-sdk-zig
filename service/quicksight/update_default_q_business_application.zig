...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDefaultQBusinessApplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDefaultQBusinessApplicationOutput {
    var result: UpdateDefaultQBusinessApplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDefaultQBusinessApplicationOutput, body, alloc);
    }
