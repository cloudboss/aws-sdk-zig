...
const VPCConnectionSummary = @import("vpc_connection_summary.zig").VPCConnectionSummary;

pub const ListVPCConnectionsInput = struct {
...
pub const ListVPCConnectionsOutput = struct {
    ...
    /// objects.
    vpc_connection_summaries: ?[]const VPCConnectionSummary = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .request_id = "RequestId",
        .status = "Status",
        .vpc_connection_summaries = "VPCConnectionSummaries",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVPCConnectionsInput, options: Options) !ListVPCConnectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVPCConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/vpc-connections");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVPCConnectionsOutput {
    var result: ListVPCConnectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVPCConnectionsOutput, body, alloc);
    }
