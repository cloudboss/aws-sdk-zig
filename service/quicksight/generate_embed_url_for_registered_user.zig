...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateEmbedUrlForRegisteredUserInput, options: Options) !GenerateEmbedUrlForRegisteredUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateEmbedUrlForRegisteredUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/embed-url/registered-user");
    ...
    if (input.allowed_domains) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AllowedDomains\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"ExperienceConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.experience_configuration), input.experience_configuration, alloc, &body_buf);
    has_prev = true;
    if (input.session_lifetime_in_minutes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SessionLifetimeInMinutes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UserArn\":");
    try aws.json.writeValue(@TypeOf(input.user_arn), input.user_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateEmbedUrlForRegisteredUserOutput {
    var result: GenerateEmbedUrlForRegisteredUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateEmbedUrlForRegisteredUserOutput, body, alloc);
    }
