...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeThemeInput, options: Options) !DescribeThemeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeThemeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeThemeOutput {
    var result: DescribeThemeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeThemeOutput, body, alloc);
    }
