...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDashboardSnapshotJobScheduleInput, options: Options) !StartDashboardSnapshotJobScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDashboardSnapshotJobScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDashboardSnapshotJobScheduleOutput {
    var result: StartDashboardSnapshotJobScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDashboardSnapshotJobScheduleOutput, body, alloc);
    }
