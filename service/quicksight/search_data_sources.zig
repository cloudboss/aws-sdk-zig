...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchDataSourcesInput, options: CallOptions) !SearchDataSourcesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchDataSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/search/data-sources");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchDataSourcesOutput {
    var result: SearchDataSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchDataSourcesOutput, body, allocator);
    }
