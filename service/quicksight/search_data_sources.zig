...
const DataSourceSearchFilter = @import("data_source_search_filter.zig").DataSourceSearchFilter;
const DataSourceSummary = @import("data_source_summary.zig").DataSourceSummary;

pub const SearchDataSourcesInput = struct {
    /// The Amazon Web Services account ID.
    aws_account_id: []const u8,

    /// The filters to apply to the search.
    filters: []const DataSourceSearchFilter,
...
pub const SearchDataSourcesOutput = struct {
    /// A `DataSourceSummaries` object that returns a summary of a data
    /// source.
    data_source_summaries: ?[]const DataSourceSummary = null,
    ...
    pub const json_field_names = .{
        .data_source_summaries = "DataSourceSummaries",
        .next_token = "NextToken",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchDataSourcesInput, options: Options) !SearchDataSourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchDataSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search/data-sources");
    ...
    try aws.json.writeValue(@TypeOf(input.filters), input.filters, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchDataSourcesOutput {
    var result: SearchDataSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchDataSourcesOutput, body, alloc);
    }
