...
fn serializeRequest(alloc: std.mem.Allocator, input: PredictQAResultsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/qa/predict");
    ...
    if (input.include_generated_answer) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"IncludeGeneratedAnswer\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.include_quick_sight_q_index) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IncludeQuickSightQIndex\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.max_topics_to_consider) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MaxTopicsToConsider\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PredictQAResultsOutput {
    var result: PredictQAResultsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PredictQAResultsOutput, body, alloc);
    }
