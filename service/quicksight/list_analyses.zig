...
pub const ListAnalysesInput = struct {
...
pub const ListAnalysesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnalysesInput, options: Options) !ListAnalysesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAnalysesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAnalysesOutput {
    var result: ListAnalysesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAnalysesOutput, body, alloc);
    }
