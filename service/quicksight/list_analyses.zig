...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAnalysesInput, options: CallOptions) !ListAnalysesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAnalysesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAnalysesOutput {
    var result: ListAnalysesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAnalysesOutput, body, allocator);
    }
