...
pub const UpdateCustomPermissionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateCustomPermissionsInput, options: Options) !UpdateCustomPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateCustomPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.custom_permissions_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateCustomPermissionsOutput {
    var result: UpdateCustomPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateCustomPermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
