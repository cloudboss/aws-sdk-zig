...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAnalysisPermissionsInput, options: Options) !DescribeAnalysisPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAnalysisPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/analyses/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAnalysisPermissionsOutput {
    var result: DescribeAnalysisPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAnalysisPermissionsOutput, body, alloc);
    }
