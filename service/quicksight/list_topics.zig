...
pub const ListTopicsOutput = struct {
    ...
    topics_summaries: ?[]const TopicSummary = null,

    pub const json_field_names = .{
        .next_token = "NextToken",
        .request_id = "RequestId",
        .status = "Status",
        .topics_summaries = "TopicsSummaries",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTopicsInput, options: Options) !ListTopicsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTopicsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTopicsOutput {
    var result: ListTopicsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTopicsOutput, body, alloc);
    }
