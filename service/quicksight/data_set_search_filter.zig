const DataSetFilterAttribute = @import("data_set_filter_attribute.zig").DataSetFilterAttribute;
const FilterOperator = @import("filter_operator.zig").FilterOperator;

/// A filter that you apply when searching for datasets.
pub const DataSetSearchFilter = struct {
    ...
    ///   datasets with that ARN listed as the only owner of the dataset are
    ...
    ///   and any datasets with that ARN listed as one of the owners or viewers of
    ///   the dataset are returned. Implicit permissions from folders or groups are
    ///   not considered.
    ///
    ...
    /// group whose datasets you want to search in the `"Value"` field. For example,
    ...
    /// datasets you are searching for. For example, `"Name":"DATASET_NAME",
    /// "Operator": "StringLike", "Value": "Test"`. The `"StringLike"` operator only
    /// supports the `NAME` value `DATASET_NAME`.
