...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTopicRefreshScheduleInput, options: Options) !CreateTopicRefreshScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTopicRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTopicRefreshScheduleOutput {
    var result: CreateTopicRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTopicRefreshScheduleOutput, body, alloc);
    }
