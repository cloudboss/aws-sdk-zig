...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeThemeAliasInput, options: Options) !DescribeThemeAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeThemeAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeThemeAliasOutput {
    var result: DescribeThemeAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeThemeAliasOutput, body, alloc);
    }
