...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterUserInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.custom_federation_provider_url) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"CustomFederationProviderUrl\":");
    ...
    try aws.json.writeValue(@TypeOf(input.email), input.email, alloc, &body_buf);
    has_prev = true;
    if (input.external_login_federation_provider_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ExternalLoginFederationProviderType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.external_login_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ExternalLoginId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.iam_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IamArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"IdentityType\":");
    try aws.json.writeValue(@TypeOf(input.identity_type), input.identity_type, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UserRole\":");
    try aws.json.writeValue(@TypeOf(input.user_role), input.user_role, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterUserOutput {
    var result: RegisterUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterUserOutput, body, alloc);
    }
