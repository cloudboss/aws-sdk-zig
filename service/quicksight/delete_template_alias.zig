...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTemplateAliasInput, options: Options) !DeleteTemplateAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTemplateAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTemplateAliasOutput {
    var result: DeleteTemplateAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteTemplateAliasOutput, body, alloc);
    }
