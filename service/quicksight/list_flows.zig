...
pub const ListFlowsInput = struct {
    /// The ID of the Amazon Web Services account that contains the flow list that
    /// you are getting.
...
pub const ListFlowsOutput = struct {
    /// A structure that contains all of the flows in your Amazon Web Services
    /// account. This structure
    /// provides basic information about the flows.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFlowsInput, options: CallOptions) !ListFlowsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFlowsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFlowsOutput {
    var result: ListFlowsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFlowsOutput, body, allocator);
    }
