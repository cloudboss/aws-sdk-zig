...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFlowsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFlowsOutput {
    var result: ListFlowsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFlowsOutput, body, alloc);
    }
