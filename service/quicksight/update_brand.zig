...
pub const UpdateBrandInput = struct {
...
pub const UpdateBrandOutput = struct {
    /// The definition of the brand.
    brand_definition: ?BrandDefinition = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBrandInput, options: Options) !UpdateBrandOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBrandInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.brand_definition) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"BrandDefinition\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBrandOutput {
    var result: UpdateBrandOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBrandOutput, body, alloc);
    }
    _ = status;
    _ = headers;
