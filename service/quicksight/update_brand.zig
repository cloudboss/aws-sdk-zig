...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBrandInput, options: CallOptions) !UpdateBrandOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateBrandInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateBrandOutput {
    var result: UpdateBrandOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBrandOutput, body, allocator);
    }
    _ = status;
    _ = headers;
