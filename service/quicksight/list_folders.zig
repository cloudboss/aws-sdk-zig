...
pub const ListFoldersOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFoldersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFoldersOutput {
    var result: ListFoldersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFoldersOutput, body, alloc);
    }
