...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDataSetRefreshPropertiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDataSetRefreshPropertiesOutput {
    var result: DeleteDataSetRefreshPropertiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDataSetRefreshPropertiesOutput, body, alloc);
    }
