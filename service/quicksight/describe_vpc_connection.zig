...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVPCConnectionInput, options: Options) !DescribeVPCConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeVPCConnectionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeVPCConnectionOutput {
    var result: DescribeVPCConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeVPCConnectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
