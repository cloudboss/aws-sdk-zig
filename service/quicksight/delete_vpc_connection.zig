...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVPCConnectionInput, options: Options) !DeleteVPCConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVPCConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.vpc_connection_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVPCConnectionOutput {
    var result: DeleteVPCConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteVPCConnectionOutput, body, alloc);
    }
