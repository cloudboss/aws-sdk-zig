...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDashboardSnapshotJobInput, options: CallOptions) !StartDashboardSnapshotJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartDashboardSnapshotJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"SnapshotConfiguration\":");
    try aws.json.writeValue(@TypeOf(input.snapshot_configuration), input.snapshot_configuration, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"SnapshotJobId\":");
    try aws.json.writeValue(@TypeOf(input.snapshot_job_id), input.snapshot_job_id, allocator, &body_buf);
    has_prev = true;
    if (input.user_configuration) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"UserConfiguration\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartDashboardSnapshotJobOutput {
    var result: StartDashboardSnapshotJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartDashboardSnapshotJobOutput, body, allocator);
    }
