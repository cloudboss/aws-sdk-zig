...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTopicRefreshScheduleInput, options: Options) !DescribeTopicRefreshScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTopicRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTopicRefreshScheduleOutput {
    var result: DescribeTopicRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTopicRefreshScheduleOutput, body, alloc);
    }
