...
pub const DescribeFolderPermissionsInput = struct {
...
pub const DescribeFolderPermissionsOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFolderPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFolderPermissionsOutput {
    var result: DescribeFolderPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFolderPermissionsOutput, body, alloc);
    }
