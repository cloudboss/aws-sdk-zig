...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataSetPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataSetPermissionsOutput {
    var result: DescribeDataSetPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDataSetPermissionsOutput, body, alloc);
    }
