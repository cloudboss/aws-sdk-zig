...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSessionEmbedUrlInput, options: Options) !GetSessionEmbedUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSessionEmbedUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/session-embed-url");
    ...
    if (input.entry_point) |v| {
        ...
        try query_buf.appendSlice(alloc, "entry-point=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSessionEmbedUrlOutput {
    var result: GetSessionEmbedUrlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSessionEmbedUrlOutput, body, alloc);
    }
