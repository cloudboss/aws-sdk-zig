...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountCustomPermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountCustomPermissionOutput {
    var result: DescribeAccountCustomPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccountCustomPermissionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
