...
const UpdateAccountSettingsInput = @import("update_account_settings_request.zig").UpdateAccountSettingsRequest;

const UpdateAccountSettingsOutput = @import("update_account_settings_response.zig").UpdateAccountSettingsResponse;

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccountSettingsInput, options: Options) !UpdateAccountSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccountSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/settings");
    ...
    try body_buf.appendSlice(alloc, "\"DefaultNamespace\":");
    try aws.json.writeValue(@TypeOf(input.default_namespace), input.default_namespace, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.termination_protection_enabled) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TerminationProtectionEnabled\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccountSettingsOutput {
    var result: UpdateAccountSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccountSettingsOutput, body, alloc);
    }
