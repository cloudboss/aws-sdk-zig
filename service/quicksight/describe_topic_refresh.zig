...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTopicRefreshInput, options: Options) !DescribeTopicRefreshOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTopicRefreshInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.refresh_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTopicRefreshOutput {
    var result: DescribeTopicRefreshOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTopicRefreshOutput, body, alloc);
    }
