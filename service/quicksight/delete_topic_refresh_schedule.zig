...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTopicRefreshScheduleInput, options: CallOptions) !DeleteTopicRefreshScheduleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteTopicRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteTopicRefreshScheduleOutput {
    var result: DeleteTopicRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteTopicRefreshScheduleOutput, body, allocator);
    }
