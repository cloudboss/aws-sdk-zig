...
pub const DescribeFolderInput = struct {
...
pub const DescribeFolderOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFolderInput, options: CallOptions) !DescribeFolderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFolderInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFolderOutput {
    var result: DescribeFolderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFolderOutput, body, allocator);
    }
