...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFolderInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFolderOutput {
    var result: DescribeFolderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFolderOutput, body, alloc);
    }
