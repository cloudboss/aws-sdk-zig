...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssetBundleExportJobsInput, options: CallOptions) !ListAssetBundleExportJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssetBundleExportJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssetBundleExportJobsOutput {
    var result: ListAssetBundleExportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetBundleExportJobsOutput, body, allocator);
    }
