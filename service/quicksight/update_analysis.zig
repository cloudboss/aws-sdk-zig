...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAnalysisInput, options: Options) !UpdateAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAnalysisInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAnalysisOutput {
    var result: UpdateAnalysisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAnalysisOutput, body, alloc);
    }
