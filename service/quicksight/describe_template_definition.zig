...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTemplateDefinitionInput, options: CallOptions) !DescribeTemplateDefinitionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTemplateDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTemplateDefinitionOutput {
    var result: DescribeTemplateDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTemplateDefinitionOutput, body, allocator);
    }
