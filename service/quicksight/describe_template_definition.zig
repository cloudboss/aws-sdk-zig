...
pub const DescribeTemplateDefinitionInput = struct {
...
pub const DescribeTemplateDefinitionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTemplateDefinitionInput, options: Options) !DescribeTemplateDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTemplateDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/definition");
    ...
    if (input.alias_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "alias-name=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.version_number) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "version-number=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTemplateDefinitionOutput {
    var result: DescribeTemplateDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTemplateDefinitionOutput, body, alloc);
    }
