...
pub const DeleteRefreshScheduleInput = struct {
...
pub const DeleteRefreshScheduleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRefreshScheduleInput, options: Options) !DeleteRefreshScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.schedule_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRefreshScheduleOutput {
    var result: DeleteRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRefreshScheduleOutput, body, alloc);
    }
