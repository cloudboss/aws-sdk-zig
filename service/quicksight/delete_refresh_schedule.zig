...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRefreshScheduleInput, options: CallOptions) !DeleteRefreshScheduleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRefreshScheduleOutput {
    var result: DeleteRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRefreshScheduleOutput, body, allocator);
    }
