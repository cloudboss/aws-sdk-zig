...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDashboardSnapshotJobResultInput, options: Options) !DescribeDashboardSnapshotJobResultOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDashboardSnapshotJobResultInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/result");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDashboardSnapshotJobResultOutput {
    var result: DescribeDashboardSnapshotJobResultOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDashboardSnapshotJobResultOutput, body, alloc);
    }
