...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAssetBundleExportJobInput, options: Options) !DescribeAssetBundleExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssetBundleExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.asset_bundle_export_job_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssetBundleExportJobOutput {
    var result: DescribeAssetBundleExportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAssetBundleExportJobOutput, body, alloc);
    }
