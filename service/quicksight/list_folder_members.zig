...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFolderMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFolderMembersOutput {
    var result: ListFolderMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFolderMembersOutput, body, alloc);
    }
