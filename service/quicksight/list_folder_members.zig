...
const MemberIdArnPair = @import("member_id_arn_pair.zig").MemberIdArnPair;

pub const ListFolderMembersInput = struct {
...
pub const ListFolderMembersOutput = struct {
    ...
    pub const json_field_names = .{
        .folder_member_list = "FolderMemberList",
        .next_token = "NextToken",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFolderMembersInput, options: CallOptions) !ListFolderMembersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListFolderMembersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListFolderMembersOutput {
    var result: ListFolderMembersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFolderMembersOutput, body, allocator);
    }
