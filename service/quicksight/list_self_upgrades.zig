...
pub const ListSelfUpgradesInput = struct {
...
pub const ListSelfUpgradesOutput = struct {
    /// The token for the next set of results, or null if there are no more results.
    next_token: ?[]const u8 = null,
    ...
    pub const json_field_names = .{
        ...
        .self_upgrade_request_details = "SelfUpgradeRequestDetails",
        .status = "Status",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSelfUpgradesInput, options: Options) !ListSelfUpgradesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSelfUpgradesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/self-upgrade-requests");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSelfUpgradesOutput {
    var result: ListSelfUpgradesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSelfUpgradesOutput, body, alloc);
    }
