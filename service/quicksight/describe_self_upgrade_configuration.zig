...
pub const DescribeSelfUpgradeConfigurationOutput = struct {
    ...
    pub const json_field_names = .{
        ...
        .self_upgrade_configuration = "SelfUpgradeConfiguration",
        .status = "Status",
    };
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSelfUpgradeConfigurationInput, options: Options) !DescribeSelfUpgradeConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSelfUpgradeConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/self-upgrade-configuration");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSelfUpgradeConfigurationOutput {
    var result: DescribeSelfUpgradeConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeSelfUpgradeConfigurationOutput, body, alloc);
    }
