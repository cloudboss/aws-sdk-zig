...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateActionConnectorInput, options: Options) !UpdateActionConnectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateActionConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.action_connector_id);
    ...
    try body_buf.appendSlice(alloc, "\"AuthenticationConfig\":");
    try aws.json.writeValue(@TypeOf(input.authentication_config), input.authentication_config, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.vpc_connection_arn) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"VpcConnectionArn\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateActionConnectorOutput {
    var result: UpdateActionConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateActionConnectorOutput, body, alloc);
    }
