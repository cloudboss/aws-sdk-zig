...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchAnalysesInput, options: Options) !SearchAnalysesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchAnalysesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search/analyses");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchAnalysesOutput {
    var result: SearchAnalysesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchAnalysesOutput, body, alloc);
    }
