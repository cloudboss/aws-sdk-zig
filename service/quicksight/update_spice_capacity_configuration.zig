...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSPICECapacityConfigurationInput, options: Options) !UpdateSPICECapacityConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSPICECapacityConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/spice-capacity-configuration");
    ...
    try body_buf.appendSlice(alloc, "\"PurchaseMode\":");
    try aws.json.writeValue(@TypeOf(input.purchase_mode), input.purchase_mode, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSPICECapacityConfigurationOutput {
    var result: UpdateSPICECapacityConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSPICECapacityConfigurationOutput, body, alloc);
    }
