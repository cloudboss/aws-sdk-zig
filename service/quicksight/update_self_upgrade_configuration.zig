...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSelfUpgradeConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"SelfUpgradeStatus\":");
    try aws.json.writeValue(@TypeOf(input.self_upgrade_status), input.self_upgrade_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSelfUpgradeConfigurationOutput {
    var result: UpdateSelfUpgradeConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSelfUpgradeConfigurationOutput, body, alloc);
    }
