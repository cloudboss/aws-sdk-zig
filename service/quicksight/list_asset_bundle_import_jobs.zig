...
const AssetBundleImportJobSummary = @import("asset_bundle_import_job_summary.zig").AssetBundleImportJobSummary;

pub const ListAssetBundleImportJobsInput = struct {
...
pub const ListAssetBundleImportJobsOutput = struct {
    /// A list of import job summaries.
    asset_bundle_import_job_summary_list: ?[]const AssetBundleImportJobSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssetBundleImportJobsInput, options: Options) !ListAssetBundleImportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssetBundleImportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/asset-bundle-import-jobs");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssetBundleImportJobsOutput {
    var result: ListAssetBundleImportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetBundleImportJobsOutput, body, alloc);
    }
