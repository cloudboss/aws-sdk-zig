...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssetBundleImportJobsInput, options: CallOptions) !ListAssetBundleImportJobsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAssetBundleImportJobsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAssetBundleImportJobsOutput {
    var result: ListAssetBundleImportJobsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssetBundleImportJobsOutput, body, allocator);
    }
