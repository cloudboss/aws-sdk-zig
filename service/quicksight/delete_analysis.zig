...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAnalysisInput, options: Options) !DeleteAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.force_delete_without_recovery) |v| {
        ...
        try query_buf.appendSlice(alloc, "force-delete-without-recovery=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.recovery_window_in_days) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "recovery-window-in-days=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAnalysisOutput {
    var result: DeleteAnalysisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAnalysisOutput, body, alloc);
    }
