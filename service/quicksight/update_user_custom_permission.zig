...
pub const UpdateUserCustomPermissionInput = struct {
...
pub const UpdateUserCustomPermissionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserCustomPermissionInput, options: Options) !UpdateUserCustomPermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserCustomPermissionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/custom-permission");
    ...
    try body_buf.appendSlice(alloc, "\"CustomPermissionsName\":");
    try aws.json.writeValue(@TypeOf(input.custom_permissions_name), input.custom_permissions_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateUserCustomPermissionOutput {
    var result: UpdateUserCustomPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateUserCustomPermissionOutput, body, alloc);
    }
