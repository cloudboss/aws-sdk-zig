...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateUserCustomPermissionInput, options: CallOptions) !UpdateUserCustomPermissionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateUserCustomPermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateUserCustomPermissionOutput {
    var result: UpdateUserCustomPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateUserCustomPermissionOutput, body, allocator);
    }
