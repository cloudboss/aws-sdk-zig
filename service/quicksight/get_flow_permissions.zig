...
pub const GetFlowPermissionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFlowPermissionsInput, options: CallOptions) !GetFlowPermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFlowPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFlowPermissionsOutput {
    var result: GetFlowPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowPermissionsOutput, body, allocator);
    }
