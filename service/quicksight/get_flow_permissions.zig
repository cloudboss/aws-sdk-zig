...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFlowPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFlowPermissionsOutput {
    var result: GetFlowPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowPermissionsOutput, body, alloc);
    }
