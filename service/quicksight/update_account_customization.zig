...
pub const UpdateAccountCustomizationInput = struct {
    /// The Quick Sight customizations you're updating.
    account_customization: AccountCustomization,

    /// The ID for the Amazon Web Services account that you want to update Quick
    /// Sight customizations
    /// for.
...
pub const UpdateAccountCustomizationOutput = struct {
    /// The Quick Sight customizations you're updating.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccountCustomizationInput, options: Options) !UpdateAccountCustomizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccountCustomizationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccountCustomizationOutput {
    var result: UpdateAccountCustomizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccountCustomizationOutput, body, alloc);
    }
