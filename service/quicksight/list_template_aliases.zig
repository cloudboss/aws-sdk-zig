...
pub const ListTemplateAliasesInput = struct {
...
pub const ListTemplateAliasesOutput = struct {
    /// The token for the next set of results, or null if there are no more results.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTemplateAliasesInput, options: CallOptions) !ListTemplateAliasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListTemplateAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.max_results) |v| {
        ...
        try query_buf.appendSlice(allocator, "max-result=");
        {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTemplateAliasesOutput {
    var result: ListTemplateAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTemplateAliasesOutput, body, allocator);
    }
