...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTemplateAliasesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.max_results) |v| {
        ...
        try query_buf.appendSlice(alloc, "max-result=");
        {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTemplateAliasesOutput {
    var result: ListTemplateAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTemplateAliasesOutput, body, alloc);
    }
