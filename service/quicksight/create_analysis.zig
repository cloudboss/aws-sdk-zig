...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAnalysisInput, options: Options) !CreateAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAnalysisInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAnalysisOutput {
    var result: CreateAnalysisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAnalysisOutput, body, alloc);
    }
