...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentityContextInput, options: Options) !GetIdentityContextOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityContextInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/identity-context");
    ...
    if (input.session_expires_at) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SessionExpiresAt\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UserIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.user_identifier), input.user_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityContextOutput {
    var result: GetIdentityContextOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetIdentityContextOutput, body, alloc);
    }
