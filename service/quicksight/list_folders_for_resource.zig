...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFoldersForResourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFoldersForResourceOutput {
    var result: ListFoldersForResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListFoldersForResourceOutput, body, alloc);
    }
