...
pub const UpdateApplicationWithTokenExchangeGrantInput = struct {
...
pub const UpdateApplicationWithTokenExchangeGrantOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateApplicationWithTokenExchangeGrantInput, options: CallOptions) !UpdateApplicationWithTokenExchangeGrantOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateApplicationWithTokenExchangeGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/application-with-token-exchange-grant");
    ...
    try aws.url.appendUrlEncoded(allocator, &query_buf, input.namespace);
    query_has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateApplicationWithTokenExchangeGrantOutput {
    var result: UpdateApplicationWithTokenExchangeGrantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateApplicationWithTokenExchangeGrantOutput, body, allocator);
    }
