...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateApplicationWithTokenExchangeGrantInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/application-with-token-exchange-grant");
    ...
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.namespace);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateApplicationWithTokenExchangeGrantOutput {
    var result: UpdateApplicationWithTokenExchangeGrantOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateApplicationWithTokenExchangeGrantOutput, body, alloc);
    }
