...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDefaultQBusinessApplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/default-qbusiness-application");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDefaultQBusinessApplicationOutput {
    var result: DeleteDefaultQBusinessApplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDefaultQBusinessApplicationOutput, body, alloc);
    }
