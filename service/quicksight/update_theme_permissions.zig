...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateThemePermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateThemePermissionsOutput {
    var result: UpdateThemePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateThemePermissionsOutput, body, alloc);
    }
