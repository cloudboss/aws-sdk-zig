...
const NamespaceInfoV2 = @import("namespace_info_v2.zig").NamespaceInfoV2;
...
pub const ListNamespacesInput = struct {
    ...
    /// receive the data, make another `ListNamespaces` API call with the returned
    /// token to retrieve the next page of data. Each token is valid for 24 hours.
    /// If you try to make a `ListNamespaces` API call with an expired token, you
...
pub const ListNamespacesOutput = struct {
    ...
    /// the namespace ARN, name, Amazon Web Services Region, notification email
    /// address, creation status, and
    /// identity store.
    namespaces: ?[]const NamespaceInfoV2 = null,
    ...
    /// that can be returned. To receive the data, make another `ListNamespaces` API
    /// call with the returned token to retrieve the next page of data. Each token
    /// is valid for 24 hours. If you try to make a `ListNamespaces` API call with
    /// an expired token, you will receive a `HTTP 400 InvalidNextTokenException`
    /// error.
    next_token: ?[]const u8 = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListNamespacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/namespaces");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListNamespacesOutput {
    var result: ListNamespacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListNamespacesOutput, body, alloc);
    }
