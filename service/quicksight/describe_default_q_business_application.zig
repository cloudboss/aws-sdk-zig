...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDefaultQBusinessApplicationInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDefaultQBusinessApplicationOutput {
    var result: DescribeDefaultQBusinessApplicationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDefaultQBusinessApplicationOutput, body, alloc);
    }
