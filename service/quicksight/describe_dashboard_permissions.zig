...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDashboardPermissionsInput, options: Options) !DescribeDashboardPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDashboardPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDashboardPermissionsOutput {
    var result: DescribeDashboardPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDashboardPermissionsOutput, body, alloc);
    }
