...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTopicInput, options: Options) !DeleteTopicOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTopicInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTopicOutput {
    var result: DeleteTopicOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteTopicOutput, body, alloc);
    }
