...
pub const PutDataSetRefreshPropertiesInput = struct {
...
pub const PutDataSetRefreshPropertiesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutDataSetRefreshPropertiesInput, options: CallOptions) !PutDataSetRefreshPropertiesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutDataSetRefreshPropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/refresh-properties");
    ...
    try body_buf.appendSlice(allocator, "\"DataSetRefreshProperties\":");
    try aws.json.writeValue(@TypeOf(input.data_set_refresh_properties), input.data_set_refresh_properties, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutDataSetRefreshPropertiesOutput {
    var result: PutDataSetRefreshPropertiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutDataSetRefreshPropertiesOutput, body, allocator);
    }
