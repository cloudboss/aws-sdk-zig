...
fn serializeRequest(alloc: std.mem.Allocator, input: PutDataSetRefreshPropertiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/refresh-properties");
    ...
    try body_buf.appendSlice(alloc, "\"DataSetRefreshProperties\":");
    try aws.json.writeValue(@TypeOf(input.data_set_refresh_properties), input.data_set_refresh_properties, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutDataSetRefreshPropertiesOutput {
    var result: PutDataSetRefreshPropertiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutDataSetRefreshPropertiesOutput, body, alloc);
    }
