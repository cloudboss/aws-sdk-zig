...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBrandPublishedVersionInput, options: CallOptions) !DescribeBrandPublishedVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeBrandPublishedVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeBrandPublishedVersionOutput {
    var result: DescribeBrandPublishedVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBrandPublishedVersionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
