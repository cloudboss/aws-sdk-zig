...
pub const DescribeBrandPublishedVersionInput = struct {
...
pub const DescribeBrandPublishedVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBrandPublishedVersionInput, options: Options) !DescribeBrandPublishedVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBrandPublishedVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/brands/");
    try path_buf.appendSlice(alloc, input.brand_id);
    try path_buf.appendSlice(alloc, "/publishedversion");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBrandPublishedVersionOutput {
    var result: DescribeBrandPublishedVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBrandPublishedVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
