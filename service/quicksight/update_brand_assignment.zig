...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBrandAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BrandArn\":");
    try aws.json.writeValue(@TypeOf(input.brand_arn), input.brand_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBrandAssignmentOutput {
    var result: UpdateBrandAssignmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBrandAssignmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
