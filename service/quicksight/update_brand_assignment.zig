...
pub const UpdateBrandAssignmentInput = struct {
...
pub const UpdateBrandAssignmentOutput = struct {
    /// The Amazon Resource Name (ARN) of the brand.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBrandAssignmentInput, options: CallOptions) !UpdateBrandAssignmentOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateBrandAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"BrandArn\":");
    try aws.json.writeValue(@TypeOf(input.brand_arn), input.brand_arn, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateBrandAssignmentOutput {
    var result: UpdateBrandAssignmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBrandAssignmentOutput, body, allocator);
    }
    _ = status;
    _ = headers;
