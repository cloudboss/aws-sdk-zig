...
const TemplateVersionSummary = @import("template_version_summary.zig").TemplateVersionSummary;
...
pub const ListTemplateVersionsOutput = struct {
    ...
    template_version_summary_list: ?[]const TemplateVersionSummary = null,
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListTemplateVersionsOutput {
    var result: ListTemplateVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTemplateVersionsOutput, body, allocator);
    }
