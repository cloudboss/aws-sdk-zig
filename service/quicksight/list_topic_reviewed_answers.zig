...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTopicReviewedAnswersInput, options: Options) !ListTopicReviewedAnswersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTopicReviewedAnswersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/reviewed-answers");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTopicReviewedAnswersOutput {
    var result: ListTopicReviewedAnswersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTopicReviewedAnswersOutput, body, alloc);
    }
