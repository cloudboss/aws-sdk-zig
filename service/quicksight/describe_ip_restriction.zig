...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIpRestrictionInput, options: Options) !DescribeIpRestrictionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIpRestrictionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIpRestrictionOutput {
    var result: DescribeIpRestrictionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeIpRestrictionOutput, body, alloc);
    }
