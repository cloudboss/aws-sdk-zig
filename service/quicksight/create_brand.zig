...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBrandInput, options: Options) !CreateBrandOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBrandInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBrandOutput {
    var result: CreateBrandOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBrandOutput, body, alloc);
    }
    _ = status;
    _ = headers;
