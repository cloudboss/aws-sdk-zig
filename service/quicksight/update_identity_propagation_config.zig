...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIdentityPropagationConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.authorized_targets) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AuthorizedTargets\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIdentityPropagationConfigOutput {
    var result: UpdateIdentityPropagationConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIdentityPropagationConfigOutput, body, alloc);
    }
