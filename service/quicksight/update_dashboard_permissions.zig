...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDashboardPermissionsInput, options: Options) !UpdateDashboardPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDashboardPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.grant_link_permissions) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"GrantLinkPermissions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.grant_permissions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"GrantPermissions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.revoke_link_permissions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RevokeLinkPermissions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.revoke_permissions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RevokePermissions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDashboardPermissionsOutput {
    var result: UpdateDashboardPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDashboardPermissionsOutput, body, alloc);
    }
