...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAccountCustomPermissionInput, options: CallOptions) !DeleteAccountCustomPermissionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAccountCustomPermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAccountCustomPermissionOutput {
    var result: DeleteAccountCustomPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAccountCustomPermissionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
