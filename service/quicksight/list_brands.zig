...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBrandsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBrandsOutput {
    var result: ListBrandsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBrandsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
