...
const BrandSummary = @import("brand_summary.zig").BrandSummary;

pub const ListBrandsInput = struct {
    /// The ID of the Amazon Web Services account that owns the brands that you want
    /// to list.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBrandsInput, options: CallOptions) !ListBrandsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBrandsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBrandsOutput {
    var result: ListBrandsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBrandsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
