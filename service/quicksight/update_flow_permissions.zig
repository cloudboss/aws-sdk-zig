...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFlowPermissionsInput, options: Options) !UpdateFlowPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFlowPermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.flow_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFlowPermissionsOutput {
    var result: UpdateFlowPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFlowPermissionsOutput, body, alloc);
    }
