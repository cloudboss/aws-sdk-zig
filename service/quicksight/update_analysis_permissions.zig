...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAnalysisPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAnalysisPermissionsOutput {
    var result: UpdateAnalysisPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAnalysisPermissionsOutput, body, alloc);
    }
