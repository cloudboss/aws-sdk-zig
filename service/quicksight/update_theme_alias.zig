...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateThemeAliasInput, options: CallOptions) !UpdateThemeAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateThemeAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateThemeAliasOutput {
    var result: UpdateThemeAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateThemeAliasOutput, body, allocator);
    }
