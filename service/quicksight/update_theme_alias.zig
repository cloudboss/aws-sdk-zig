...
pub const UpdateThemeAliasInput = struct {
...
pub const UpdateThemeAliasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateThemeAliasInput, options: Options) !UpdateThemeAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateThemeAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/themes/");
    try path_buf.appendSlice(alloc, input.theme_id);
    try path_buf.appendSlice(alloc, "/aliases/");
    try path_buf.appendSlice(alloc, input.alias_name);
    ...
    try body_buf.appendSlice(alloc, "\"ThemeVersionNumber\":");
    try aws.json.writeValue(@TypeOf(input.theme_version_number), input.theme_version_number, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateThemeAliasOutput {
    var result: UpdateThemeAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateThemeAliasOutput, body, alloc);
    }
