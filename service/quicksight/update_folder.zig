...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFolderInput, options: Options) !UpdateFolderOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFolderInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFolderOutput {
    var result: UpdateFolderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFolderOutput, body, alloc);
    }
