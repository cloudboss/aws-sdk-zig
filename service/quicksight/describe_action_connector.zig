...
const ActionConnector = @import("action_connector.zig").ActionConnector;

pub const DescribeActionConnectorInput = struct {
...
pub const DescribeActionConnectorOutput = struct {
    /// The detailed information about the action connector, including its
    /// configuration and current state.
    action_connector: ?ActionConnector = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeActionConnectorInput, options: Options) !DescribeActionConnectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeActionConnectorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeActionConnectorOutput {
    var result: DescribeActionConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeActionConnectorOutput, body, alloc);
    }
