...
pub const UpdatePublicSharingSettingsInput = struct {
...
pub const UpdatePublicSharingSettingsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePublicSharingSettingsInput, options: CallOptions) !UpdatePublicSharingSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePublicSharingSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/public-sharing-settings");
    ...
    if (input.public_sharing_enabled) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"PublicSharingEnabled\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePublicSharingSettingsOutput {
    var result: UpdatePublicSharingSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePublicSharingSettingsOutput, body, allocator);
    }
