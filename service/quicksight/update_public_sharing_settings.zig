...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePublicSharingSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/public-sharing-settings");
    ...
    if (input.public_sharing_enabled) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"PublicSharingEnabled\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePublicSharingSettingsOutput {
    var result: UpdatePublicSharingSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePublicSharingSettingsOutput, body, alloc);
    }
