...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDashboardDefinitionInput, options: Options) !DescribeDashboardDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDashboardDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDashboardDefinitionOutput {
    var result: DescribeDashboardDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDashboardDefinitionOutput, body, alloc);
    }
