...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFolderMembershipInput, options: Options) !CreateFolderMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateFolderMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.member_type);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateFolderMembershipOutput {
    var result: CreateFolderMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateFolderMembershipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
