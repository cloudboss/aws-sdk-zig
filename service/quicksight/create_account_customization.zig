...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccountCustomizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/customizations");
    ...
    try body_buf.appendSlice(alloc, "\"AccountCustomization\":");
    try aws.json.writeValue(@TypeOf(input.account_customization), input.account_customization, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccountCustomizationOutput {
    var result: CreateAccountCustomizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAccountCustomizationOutput, body, alloc);
    }
