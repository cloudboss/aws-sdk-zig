...
const AccountCustomization = @import("account_customization.zig").AccountCustomization;
const Tag = @import("tag.zig").Tag;

pub const CreateAccountCustomizationInput = struct {
    /// The Quick Sight customizations you're adding. You can add
    ...
    /// "arn:aws:quicksight::aws:theme/MIDNIGHT" }`. Or, you can add a custom theme
    /// by
...
pub const CreateAccountCustomizationOutput = struct {
    /// The Quick Sight customizations you're adding.
    account_customization: ?AccountCustomization = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAccountCustomizationInput, options: CallOptions) !CreateAccountCustomizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateAccountCustomizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/customizations");
    ...
    try body_buf.appendSlice(allocator, "\"AccountCustomization\":");
    try aws.json.writeValue(@TypeOf(input.account_customization), input.account_customization, allocator, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateAccountCustomizationOutput {
    var result: CreateAccountCustomizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAccountCustomizationOutput, body, allocator);
    }
