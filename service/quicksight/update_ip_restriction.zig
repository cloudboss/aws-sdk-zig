...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIpRestrictionInput, options: Options) !UpdateIpRestrictionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIpRestrictionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ip-restriction");
    ...
    if (input.ip_restriction_rule_map) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IpRestrictionRuleMap\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.vpc_endpoint_id_restriction_rule_map) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"VpcEndpointIdRestrictionRuleMap\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.vpc_id_restriction_rule_map) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"VpcIdRestrictionRuleMap\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIpRestrictionOutput {
    var result: UpdateIpRestrictionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIpRestrictionOutput, body, alloc);
    }
