...
pub const DeleteBrandInput = struct {
...
pub const DeleteBrandOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBrandInput, options: CallOptions) !DeleteBrandOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBrandInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBrandOutput {
    var result: DeleteBrandOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBrandOutput, body, allocator);
    }
    _ = status;
    _ = headers;
