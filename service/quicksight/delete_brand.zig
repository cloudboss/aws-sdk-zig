...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBrandInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBrandOutput {
    var result: DeleteBrandOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBrandOutput, body, alloc);
    }
    _ = status;
    _ = headers;
