...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDashboardInput, options: Options) !DescribeDashboardOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDashboardInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDashboardOutput {
    var result: DescribeDashboardOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDashboardOutput, body, alloc);
    }
