...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeActionConnectorPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeActionConnectorPermissionsOutput {
    var result: DescribeActionConnectorPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeActionConnectorPermissionsOutput, body, alloc);
    }
