...
pub const ListRoleMembershipsInput = struct {
...
pub const ListRoleMembershipsOutput = struct {
    /// The list of groups associated with a role
    members_list: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoleMembershipsInput, options: Options) !ListRoleMembershipsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoleMembershipsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.role);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoleMembershipsOutput {
    var result: ListRoleMembershipsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRoleMembershipsOutput, body, alloc);
    }
