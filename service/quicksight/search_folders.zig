...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchFoldersInput, options: Options) !SearchFoldersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchFoldersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search/folders");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchFoldersOutput {
    var result: SearchFoldersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchFoldersOutput, body, alloc);
    }
