...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteActionConnectorInput, options: Options) !DeleteActionConnectorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteActionConnectorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteActionConnectorOutput {
    var result: DeleteActionConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteActionConnectorOutput, body, alloc);
    }
