...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRefreshScheduleOutput {
    var result: DescribeRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRefreshScheduleOutput, body, alloc);
    }
