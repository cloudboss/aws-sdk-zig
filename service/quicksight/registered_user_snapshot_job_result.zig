...
pub const RegisteredUserSnapshotJobResult = struct {
    /// A list of `SnapshotJobResultFileGroup` objects that contain information on
    /// the files that are requested for registered user during a
    /// `StartDashboardSnapshotJob` API call. If the job succeeds, these objects
    /// contain the location where the snapshot artifacts are stored. If the job
    /// fails, the objects contain information about the error that caused the job
    /// to fail.
