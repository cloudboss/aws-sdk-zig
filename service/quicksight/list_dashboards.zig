...
const DashboardSummary = @import("dashboard_summary.zig").DashboardSummary;
...
pub const ListDashboardsOutput = struct {
    ...
    dashboard_summary_list: ?[]const DashboardSummary = null,
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDashboardsOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDashboardsOutput, body, alloc);
    }
