...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateIAMPolicyAssignmentInput, options: Options) !UpdateIAMPolicyAssignmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateIAMPolicyAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.assignment_status) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AssignmentStatus\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.identities) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Identities\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateIAMPolicyAssignmentOutput {
    var result: UpdateIAMPolicyAssignmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateIAMPolicyAssignmentOutput, body, alloc);
    }
