...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteUserCustomPermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteUserCustomPermissionOutput {
    var result: DeleteUserCustomPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteUserCustomPermissionOutput, body, alloc);
    }
