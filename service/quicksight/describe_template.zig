...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTemplateInput, options: Options) !DescribeTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTemplateInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTemplateOutput {
    var result: DescribeTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTemplateOutput, body, alloc);
    }
