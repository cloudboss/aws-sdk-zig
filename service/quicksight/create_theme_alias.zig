...
pub const CreateThemeAliasInput = struct {
...
pub const CreateThemeAliasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateThemeAliasInput, options: Options) !CreateThemeAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateThemeAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateThemeAliasOutput {
    var result: CreateThemeAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateThemeAliasOutput, body, alloc);
    }
