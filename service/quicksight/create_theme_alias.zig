...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateThemeAliasInput, options: CallOptions) !CreateThemeAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateThemeAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"ThemeVersionNumber\":");
    try aws.json.writeValue(@TypeOf(input.theme_version_number), input.theme_version_number, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateThemeAliasOutput {
    var result: CreateThemeAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateThemeAliasOutput, body, allocator);
    }
