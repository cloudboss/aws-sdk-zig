...
pub const UpdateTemplateAliasInput = struct {
...
pub const UpdateTemplateAliasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTemplateAliasInput, options: Options) !UpdateTemplateAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTemplateAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"TemplateVersionNumber\":");
    try aws.json.writeValue(@TypeOf(input.template_version_number), input.template_version_number, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTemplateAliasOutput {
    var result: UpdateTemplateAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTemplateAliasOutput, body, alloc);
    }
