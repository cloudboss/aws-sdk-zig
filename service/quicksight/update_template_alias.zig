...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTemplateAliasInput, options: CallOptions) !UpdateTemplateAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTemplateAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTemplateAliasOutput {
    var result: UpdateTemplateAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTemplateAliasOutput, body, allocator);
    }
