...
const CreateGroupInput = @import("create_group_request.zig").CreateGroupRequest;

const CreateGroupOutput = @import("create_group_response.zig").CreateGroupResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.group_name), input.group_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGroupOutput {
    var result: CreateGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGroupOutput, body, alloc);
    }
