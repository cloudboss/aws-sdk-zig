...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountCustomizationInput, options: CallOptions) !DescribeAccountCustomizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAccountCustomizationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.resolved) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "resolved=");
