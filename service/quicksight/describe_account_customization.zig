...
pub const DescribeAccountCustomizationInput = struct {
    ...
    /// of Quick Sight customizations is returned. You can add this flag to your
    /// command to use
    /// the same view that Quick Sight uses to identify which customizations to
    /// apply to the
...
pub const DescribeAccountCustomizationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAccountCustomizationInput, options: Options) !DescribeAccountCustomizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAccountCustomizationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.resolved) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "resolved=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAccountCustomizationOutput {
    var result: DescribeAccountCustomizationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAccountCustomizationOutput, body, alloc);
    }
