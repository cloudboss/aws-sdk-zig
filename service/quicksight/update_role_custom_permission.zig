...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRoleCustomPermissionInput, options: Options) !UpdateRoleCustomPermissionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRoleCustomPermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRoleCustomPermissionOutput {
    var result: UpdateRoleCustomPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRoleCustomPermissionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
