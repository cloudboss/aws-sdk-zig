...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSelfUpgradeInput, options: Options) !UpdateSelfUpgradeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSelfUpgradeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/update-self-upgrade-request");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"UpgradeRequestId\":");
    try aws.json.writeValue(@TypeOf(input.upgrade_request_id), input.upgrade_request_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSelfUpgradeOutput {
    var result: UpdateSelfUpgradeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSelfUpgradeOutput, body, alloc);
    }
