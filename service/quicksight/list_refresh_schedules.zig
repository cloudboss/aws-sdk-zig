...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRefreshSchedulesInput, options: Options) !ListRefreshSchedulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRefreshSchedulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRefreshSchedulesOutput {
    var result: ListRefreshSchedulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRefreshSchedulesOutput, body, alloc);
    }
