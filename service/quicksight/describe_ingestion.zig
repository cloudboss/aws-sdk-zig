...
pub const DescribeIngestionInput = struct {
...
pub const DescribeIngestionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIngestionInput, options: Options) !DescribeIngestionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeIngestionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.ingestion_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeIngestionOutput {
    var result: DescribeIngestionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeIngestionOutput, body, alloc);
    }
