...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeIngestionInput, options: CallOptions) !DescribeIngestionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeIngestionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.ingestion_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeIngestionOutput {
    var result: DescribeIngestionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeIngestionOutput, body, allocator);
    }
