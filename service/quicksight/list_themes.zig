...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThemesInput, options: Options) !ListThemesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThemesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThemesOutput {
    var result: ListThemesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThemesOutput, body, alloc);
    }
