...
pub const CancelIngestionInput = struct {
...
pub const CancelIngestionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelIngestionInput, options: Options) !CancelIngestionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelIngestionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelIngestionOutput {
    var result: CancelIngestionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CancelIngestionOutput, body, alloc);
    }
