...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGroupMembershipInput, options: Options) !DeleteGroupMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGroupMembershipInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGroupMembershipOutput {
    var result: DeleteGroupMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteGroupMembershipOutput, body, alloc);
    }
