...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDashboardLinksInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/linked-entities");
    ...
    try aws.json.writeValue(@TypeOf(input.link_entities), input.link_entities, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDashboardLinksOutput {
    var result: UpdateDashboardLinksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDashboardLinksOutput, body, alloc);
    }
