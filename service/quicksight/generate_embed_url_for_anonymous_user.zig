...
pub const GenerateEmbedUrlForAnonymousUserInput = struct {
    ...
    ///
    /// When using session tags, you must call `GenerateEmbedUrlForAnonymousUser`
    /// from a secure, trusted environment.
    /// The API call passes session tags that enable server-side data redaction by
    /// using the row-level security (RLS) rules configured in your datasets.
    /// A secure, trusted environment has access controls that you implement. These
    /// controls ensure that only your server or authorized users can add or modify
    /// session tags.
    ///
...
pub const GenerateEmbedUrlForAnonymousUserOutput = struct {
    /// The Amazon Resource Name (ARN) to use for the anonymous Amazon Quick Suite
    /// user.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateEmbedUrlForAnonymousUserInput, options: Options) !GenerateEmbedUrlForAnonymousUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateEmbedUrlForAnonymousUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/embed-url/anonymous-user");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"AuthorizedResourceArns\":");
    try aws.json.writeValue(@TypeOf(input.authorized_resource_arns), input.authorized_resource_arns, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.session_tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"SessionTags\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateEmbedUrlForAnonymousUserOutput {
    var result: GenerateEmbedUrlForAnonymousUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateEmbedUrlForAnonymousUserOutput, body, alloc);
    }
