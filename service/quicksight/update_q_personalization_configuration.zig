...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateQPersonalizationConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"PersonalizationMode\":");
    try aws.json.writeValue(@TypeOf(input.personalization_mode), input.personalization_mode, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateQPersonalizationConfigurationOutput {
    var result: UpdateQPersonalizationConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateQPersonalizationConfigurationOutput, body, alloc);
    }
