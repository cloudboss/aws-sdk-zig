...
const DataSetSearchFilter = @import("data_set_search_filter.zig").DataSetSearchFilter;
const DataSetSummary = @import("data_set_summary.zig").DataSetSummary;

pub const SearchDataSetsInput = struct {
    /// The Amazon Web Services account ID.
    aws_account_id: []const u8,

    /// The filters to apply to the search.
    filters: []const DataSetSearchFilter,
...
pub const SearchDataSetsOutput = struct {
    /// A `DataSetSummaries` object that returns a summary of a dataset.
    data_set_summaries: ?[]const DataSetSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchDataSetsInput, options: Options) !SearchDataSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchDataSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search/data-sets");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchDataSetsOutput {
    var result: SearchDataSetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchDataSetsOutput, body, alloc);
    }
