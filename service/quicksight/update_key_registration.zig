...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKeyRegistrationInput, options: Options) !UpdateKeyRegistrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKeyRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"KeyRegistration\":");
    try aws.json.writeValue(@TypeOf(input.key_registration), input.key_registration, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKeyRegistrationOutput {
    var result: UpdateKeyRegistrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKeyRegistrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
