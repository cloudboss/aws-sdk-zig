...
pub const CreateRefreshScheduleInput = struct {
...
pub const CreateRefreshScheduleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRefreshScheduleInput, options: Options) !CreateRefreshScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/data-sets/");
    try path_buf.appendSlice(alloc, input.data_set_id);
    try path_buf.appendSlice(alloc, "/refresh-schedules");
    ...
    try aws.json.writeValue(@TypeOf(input.schedule), input.schedule, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRefreshScheduleOutput {
    var result: CreateRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRefreshScheduleOutput, body, alloc);
    }
