...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRefreshScheduleInput, options: CallOptions) !CreateRefreshScheduleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateRefreshScheduleOutput {
    var result: CreateRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRefreshScheduleOutput, body, allocator);
    }
