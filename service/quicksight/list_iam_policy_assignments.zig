...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIAMPolicyAssignmentsInput, options: CallOptions) !ListIAMPolicyAssignmentsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListIAMPolicyAssignmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/v2/iam-policy-assignments");
    ...
    if (input.assignment_status) |v| {
        ...
        try query_buf.appendSlice(allocator, "assignment-status=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListIAMPolicyAssignmentsOutput {
    var result: ListIAMPolicyAssignmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIAMPolicyAssignmentsOutput, body, allocator);
    }
