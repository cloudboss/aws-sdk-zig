...
const IAMPolicyAssignmentSummary = @import("iam_policy_assignment_summary.zig").IAMPolicyAssignmentSummary;

pub const ListIAMPolicyAssignmentsInput = struct {
...
pub const ListIAMPolicyAssignmentsOutput = struct {
    /// Information describing the IAM policy assignments.
    iam_policy_assignments: ?[]const IAMPolicyAssignmentSummary = null,
    ...
    pub const json_field_names = .{
        .iam_policy_assignments = "IAMPolicyAssignments",
        .next_token = "NextToken",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIAMPolicyAssignmentsInput, options: Options) !ListIAMPolicyAssignmentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIAMPolicyAssignmentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.namespace);
    try path_buf.appendSlice(alloc, "/v2/iam-policy-assignments");
    ...
    if (input.assignment_status) |v| {
        ...
        try query_buf.appendSlice(alloc, "assignment-status=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIAMPolicyAssignmentsOutput {
    var result: ListIAMPolicyAssignmentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIAMPolicyAssignmentsOutput, body, alloc);
    }
