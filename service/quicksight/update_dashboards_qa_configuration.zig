...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDashboardsQAConfigurationInput, options: Options) !UpdateDashboardsQAConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDashboardsQAConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"DashboardsQAStatus\":");
    try aws.json.writeValue(@TypeOf(input.dashboards_qa_status), input.dashboards_qa_status, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDashboardsQAConfigurationOutput {
    var result: UpdateDashboardsQAConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDashboardsQAConfigurationOutput, body, alloc);
    }
