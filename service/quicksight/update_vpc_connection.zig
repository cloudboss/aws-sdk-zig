...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateVPCConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dns_resolvers) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"DnsResolvers\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateVPCConnectionOutput {
    var result: UpdateVPCConnectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateVPCConnectionOutput, body, alloc);
    }
