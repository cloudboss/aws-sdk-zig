...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteUserInput, options: Options) !DeleteUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteUserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteUserOutput {
    var result: DeleteUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteUserOutput, body, alloc);
    }
