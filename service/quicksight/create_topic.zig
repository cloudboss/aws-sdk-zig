...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTopicInput, options: Options) !CreateTopicOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTopicInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TopicId\":");
    try aws.json.writeValue(@TypeOf(input.topic_id), input.topic_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTopicOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTopicOutput, body, alloc);
    }
