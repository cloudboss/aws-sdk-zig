...
const ActionConnectorSummary = @import("action_connector_summary.zig").ActionConnectorSummary;

pub const ListActionConnectorsInput = struct {
...
pub const ListActionConnectorsOutput = struct {
    /// A list of action connector summaries containing basic information about each
    /// connector.
    action_connector_summaries: ?[]const ActionConnectorSummary = null,

    /// A pagination token to retrieve the next set of results. If null, there are
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListActionConnectorsInput, options: Options) !ListActionConnectorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListActionConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/action-connectors");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListActionConnectorsOutput {
    var result: ListActionConnectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListActionConnectorsOutput, body, alloc);
    }
