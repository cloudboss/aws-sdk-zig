...
pub const UpdateThemeInput = struct {
...
pub const UpdateThemeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateThemeInput, options: Options) !UpdateThemeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateThemeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"BaseThemeId\":");
    try aws.json.writeValue(@TypeOf(input.base_theme_id), input.base_theme_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateThemeOutput {
    var result: UpdateThemeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateThemeOutput, body, alloc);
    }
