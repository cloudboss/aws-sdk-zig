...
pub const DeleteTemplateInput = struct {
    ...
    /// If you don't provide a version number, `DeleteTemplate` deletes all versions
    /// of the template.
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTemplateOutput {
    var result: DeleteTemplateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteTemplateOutput, body, alloc);
    }
