...
pub const DescribeTemplateAliasInput = struct {
...
pub const DescribeTemplateAliasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTemplateAliasInput, options: Options) !DescribeTemplateAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTemplateAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTemplateAliasOutput {
    var result: DescribeTemplateAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTemplateAliasOutput, body, alloc);
    }
