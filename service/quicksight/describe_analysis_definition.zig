...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAnalysisDefinitionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAnalysisDefinitionOutput {
    var result: DescribeAnalysisDefinitionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAnalysisDefinitionOutput, body, alloc);
    }
