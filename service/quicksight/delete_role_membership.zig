...
pub const DeleteRoleMembershipInput = struct {
...
pub const DeleteRoleMembershipOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRoleMembershipInput, options: Options) !DeleteRoleMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRoleMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.member_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRoleMembershipOutput {
    var result: DeleteRoleMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRoleMembershipOutput, body, alloc);
    }
