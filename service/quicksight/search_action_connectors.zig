...
const ActionConnectorSearchFilter = @import("action_connector_search_filter.zig").ActionConnectorSearchFilter;
const ActionConnectorSummary = @import("action_connector_summary.zig").ActionConnectorSummary;

pub const SearchActionConnectorsInput = struct {
    ...
    filters: []const ActionConnectorSearchFilter,
...
pub const SearchActionConnectorsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchActionConnectorsInput, options: CallOptions) !SearchActionConnectorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchActionConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/search/action-connectors");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchActionConnectorsOutput {
    var result: SearchActionConnectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchActionConnectorsOutput, body, allocator);
    }
