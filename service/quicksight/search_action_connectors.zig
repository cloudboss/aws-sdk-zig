...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchActionConnectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search/action-connectors");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchActionConnectorsOutput {
    var result: SearchActionConnectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchActionConnectorsOutput, body, alloc);
    }
