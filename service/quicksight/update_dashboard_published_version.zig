...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDashboardPublishedVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDashboardPublishedVersionOutput {
    var result: UpdateDashboardPublishedVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDashboardPublishedVersionOutput, body, alloc);
    }
