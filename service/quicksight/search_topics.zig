...
const TopicSearchFilter = @import("topic_search_filter.zig").TopicSearchFilter;
const TopicSummary = @import("topic_summary.zig").TopicSummary;

pub const SearchTopicsInput = struct {
    ...
    filters: []const TopicSearchFilter,
...
pub const SearchTopicsOutput = struct {
    /// The token for the next set of results, or null if there are no more results.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchTopicsInput, options: Options) !SearchTopicsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchTopicsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search/topics");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchTopicsOutput {
    var result: SearchTopicsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchTopicsOutput, body, alloc);
    }
