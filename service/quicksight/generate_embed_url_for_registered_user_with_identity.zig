...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GenerateEmbedUrlForRegisteredUserWithIdentityInput, options: Options) !GenerateEmbedUrlForRegisteredUserWithIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GenerateEmbedUrlForRegisteredUserWithIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/embed-url/registered-user-with-identity");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GenerateEmbedUrlForRegisteredUserWithIdentityOutput {
    var result: GenerateEmbedUrlForRegisteredUserWithIdentityOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GenerateEmbedUrlForRegisteredUserWithIdentityOutput, body, alloc);
    }
