...
const ActiveIAMPolicyAssignment = @import("active_iam_policy_assignment.zig").ActiveIAMPolicyAssignment;

pub const ListIAMPolicyAssignmentsForUserInput = struct {
...
pub const ListIAMPolicyAssignmentsForUserOutput = struct {
    /// The active assignments for this user.
    active_assignments: ?[]const ActiveIAMPolicyAssignment = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIAMPolicyAssignmentsForUserInput, options: Options) !ListIAMPolicyAssignmentsForUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIAMPolicyAssignmentsForUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.user_name);
    try path_buf.appendSlice(alloc, "/iam-policy-assignments");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIAMPolicyAssignmentsForUserOutput {
    var result: ListIAMPolicyAssignmentsForUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIAMPolicyAssignmentsForUserOutput, body, alloc);
    }
