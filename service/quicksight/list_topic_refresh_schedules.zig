...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTopicRefreshSchedulesInput, options: Options) !ListTopicRefreshSchedulesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTopicRefreshSchedulesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTopicRefreshSchedulesOutput {
    var result: ListTopicRefreshSchedulesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTopicRefreshSchedulesOutput, body, alloc);
    }
