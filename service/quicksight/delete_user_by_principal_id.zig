...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteUserByPrincipalIdInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/user-principals/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteUserByPrincipalIdOutput {
    var result: DeleteUserByPrincipalIdOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteUserByPrincipalIdOutput, body, alloc);
    }
