...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeThemePermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeThemePermissionsOutput {
    var result: DescribeThemePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeThemePermissionsOutput, body, alloc);
    }
