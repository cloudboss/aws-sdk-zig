...
pub const ListThemeAliasesInput = struct {
...
pub const ListThemeAliasesOutput = struct {
    /// The token for the next set of results, or null if there are no more results.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThemeAliasesInput, options: CallOptions) !ListThemeAliasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListThemeAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListThemeAliasesOutput {
    var result: ListThemeAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThemeAliasesOutput, body, allocator);
    }
