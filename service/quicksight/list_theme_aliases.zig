...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThemeAliasesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThemeAliasesOutput {
    var result: ListThemeAliasesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThemeAliasesOutput, body, alloc);
    }
