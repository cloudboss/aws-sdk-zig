...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFolderPermissionsInput, options: Options) !UpdateFolderPermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFolderPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFolderPermissionsOutput {
    var result: UpdateFolderPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFolderPermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
