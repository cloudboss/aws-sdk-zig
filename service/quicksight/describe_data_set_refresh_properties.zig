...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDataSetRefreshPropertiesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDataSetRefreshPropertiesOutput {
    var result: DescribeDataSetRefreshPropertiesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDataSetRefreshPropertiesOutput, body, alloc);
    }
