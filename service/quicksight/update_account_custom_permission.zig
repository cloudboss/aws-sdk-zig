...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAccountCustomPermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAccountCustomPermissionOutput {
    var result: UpdateAccountCustomPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccountCustomPermissionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
