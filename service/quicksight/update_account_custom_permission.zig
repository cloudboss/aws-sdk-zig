...
pub const UpdateAccountCustomPermissionInput = struct {
    /// The ID of the Amazon Web Services account for which you want to apply a
    /// custom permissions profile.
    aws_account_id: []const u8,

    /// The name of the custom permissions profile that you want to apply to an
    /// account.
...
pub const UpdateAccountCustomPermissionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAccountCustomPermissionInput, options: CallOptions) !UpdateAccountCustomPermissionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateAccountCustomPermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateAccountCustomPermissionOutput {
    var result: UpdateAccountCustomPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAccountCustomPermissionOutput, body, allocator);
    }
    _ = status;
    _ = headers;
