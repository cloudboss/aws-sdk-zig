...
const ThemeVersionSummary = @import("theme_version_summary.zig").ThemeVersionSummary;

pub const ListThemeVersionsInput = struct {
...
pub const ListThemeVersionsOutput = struct {
    /// The token for the next set of results, or null if there are no more results.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThemeVersionsInput, options: CallOptions) !ListThemeVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListThemeVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListThemeVersionsOutput {
    var result: ListThemeVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThemeVersionsOutput, body, allocator);
    }
