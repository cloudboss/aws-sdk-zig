...
fn serializeRequest(alloc: std.mem.Allocator, input: ListThemeVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListThemeVersionsOutput {
    var result: ListThemeVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListThemeVersionsOutput, body, alloc);
    }
