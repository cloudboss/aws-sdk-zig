...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreAnalysisInput, options: Options) !RestoreAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/restore/analyses/");
    ...
    if (input.restore_to_folders) |v| {
        ...
        try query_buf.appendSlice(alloc, "restore-to-folders=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreAnalysisOutput {
    var result: RestoreAnalysisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RestoreAnalysisOutput, body, alloc);
    }
