...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBrandPublishedVersionInput, options: Options) !UpdateBrandPublishedVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBrandPublishedVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.version_id), input.version_id, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBrandPublishedVersionOutput {
    var result: UpdateBrandPublishedVersionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBrandPublishedVersionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
