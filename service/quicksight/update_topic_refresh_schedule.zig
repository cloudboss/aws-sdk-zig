...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTopicRefreshScheduleInput, options: CallOptions) !UpdateTopicRefreshScheduleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTopicRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateTopicRefreshScheduleOutput {
    var result: UpdateTopicRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTopicRefreshScheduleOutput, body, allocator);
    }
