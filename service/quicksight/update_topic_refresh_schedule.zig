...
pub const UpdateTopicRefreshScheduleInput = struct {
...
pub const UpdateTopicRefreshScheduleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTopicRefreshScheduleInput, options: Options) !UpdateTopicRefreshScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTopicRefreshScheduleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"RefreshSchedule\":");
    try aws.json.writeValue(@TypeOf(input.refresh_schedule), input.refresh_schedule, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTopicRefreshScheduleOutput {
    var result: UpdateTopicRefreshScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTopicRefreshScheduleOutput, body, alloc);
    }
