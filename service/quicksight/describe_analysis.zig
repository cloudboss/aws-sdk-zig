...
pub const DescribeAnalysisInput = struct {
...
pub const DescribeAnalysisOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAnalysisInput, options: Options) !DescribeAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAnalysisInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAnalysisOutput {
    var result: DescribeAnalysisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAnalysisOutput, body, alloc);
    }
