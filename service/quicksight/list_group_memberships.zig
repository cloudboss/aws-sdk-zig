...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGroupMembershipsInput, options: Options) !ListGroupMembershipsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGroupMembershipsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGroupMembershipsOutput {
    var result: ListGroupMembershipsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGroupMembershipsOutput, body, alloc);
    }
