...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssetBundleImportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.asset_bundle_import_job_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssetBundleImportJobOutput {
    var result: DescribeAssetBundleImportJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAssetBundleImportJobOutput, body, alloc);
    }
