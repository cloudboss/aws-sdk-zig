...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBrandAssignmentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBrandAssignmentOutput {
    var result: DeleteBrandAssignmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBrandAssignmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
