...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFlowMetadataInput, options: CallOptions) !GetFlowMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFlowMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFlowMetadataOutput {
    var result: GetFlowMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowMetadataOutput, body, allocator);
    }
