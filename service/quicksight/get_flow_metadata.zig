...
pub const GetFlowMetadataInput = struct {
...
pub const GetFlowMetadataOutput = struct {
    ...
    /// The publish state for the flow. Valid values are `DRAFT`, `PUBLISHED`,
    /// or `PENDING_APPROVAL`.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFlowMetadataInput, options: Options) !GetFlowMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFlowMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFlowMetadataOutput {
    var result: GetFlowMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFlowMetadataOutput, body, alloc);
    }
