...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeKeyRegistrationInput, options: Options) !DescribeKeyRegistrationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeKeyRegistrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/key-registration");
    ...
    if (input.default_key_only) |v| {
        ...
        try query_buf.appendSlice(alloc, "default-key-only=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeKeyRegistrationOutput {
    var result: DescribeKeyRegistrationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeKeyRegistrationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
