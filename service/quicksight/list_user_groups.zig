...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUserGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUserGroupsOutput {
    var result: ListUserGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUserGroupsOutput, body, alloc);
    }
