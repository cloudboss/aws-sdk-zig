...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRoleCustomPermissionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRoleCustomPermissionOutput {
    var result: DeleteRoleCustomPermissionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRoleCustomPermissionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
