...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateActionConnectorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"ActionConnectorId\":");
    try aws.json.writeValue(@TypeOf(input.action_connector_id), input.action_connector_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateActionConnectorOutput {
    var result: CreateActionConnectorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateActionConnectorOutput, body, alloc);
    }
