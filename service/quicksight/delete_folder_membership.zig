...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteFolderMembershipInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteFolderMembershipOutput {
    var result: DeleteFolderMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteFolderMembershipOutput, body, alloc);
    }
    _ = status;
    _ = headers;
