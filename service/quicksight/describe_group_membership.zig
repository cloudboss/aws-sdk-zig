...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGroupMembershipInput, options: Options) !DescribeGroupMembershipOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGroupMembershipInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/groups/");
    try path_buf.appendSlice(alloc, input.group_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGroupMembershipOutput {
    var result: DescribeGroupMembershipOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeGroupMembershipOutput, body, alloc);
    }
