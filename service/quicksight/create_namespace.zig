...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNamespaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"IdentityStore\":");
    try aws.json.writeValue(@TypeOf(input.identity_store), input.identity_store, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateNamespaceOutput {
    var result: CreateNamespaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateNamespaceOutput, body, alloc);
    }
