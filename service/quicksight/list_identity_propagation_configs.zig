...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdentityPropagationConfigsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdentityPropagationConfigsOutput {
    var result: ListIdentityPropagationConfigsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIdentityPropagationConfigsOutput, body, alloc);
    }
