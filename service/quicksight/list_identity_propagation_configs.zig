...
const AuthorizedTargetsByService = @import("authorized_targets_by_service.zig").AuthorizedTargetsByService;

pub const ListIdentityPropagationConfigsInput = struct {
...
pub const ListIdentityPropagationConfigsOutput = struct {
    /// The token for the next set of results, or null if there are no more results.
    next_token: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIdentityPropagationConfigsInput, options: CallOptions) !ListIdentityPropagationConfigsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListIdentityPropagationConfigsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListIdentityPropagationConfigsOutput {
    var result: ListIdentityPropagationConfigsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListIdentityPropagationConfigsOutput, body, allocator);
    }
