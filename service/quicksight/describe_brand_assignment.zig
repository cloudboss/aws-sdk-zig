...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBrandAssignmentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/brandassignments");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBrandAssignmentOutput {
    var result: DescribeBrandAssignmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBrandAssignmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
