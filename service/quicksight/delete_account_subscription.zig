...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAccountSubscriptionInput, options: CallOptions) !DeleteAccountSubscriptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAccountSubscriptionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAccountSubscriptionOutput {
    var result: DeleteAccountSubscriptionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteAccountSubscriptionOutput, body, allocator);
    }
