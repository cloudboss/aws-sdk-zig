...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTopicPermissionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTopicPermissionsOutput {
    var result: UpdateTopicPermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTopicPermissionsOutput, body, alloc);
    }
