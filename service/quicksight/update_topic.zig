...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTopicInput, options: Options) !UpdateTopicOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTopicInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.custom_instructions) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"CustomInstructions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Topic\":");
    try aws.json.writeValue(@TypeOf(input.topic), input.topic, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTopicOutput {
    var result: UpdateTopicOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTopicOutput, body, alloc);
    }
