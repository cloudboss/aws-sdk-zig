...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateTemplateAliasInput, options: Options) !CreateTemplateAliasOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTemplateAliasInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTemplateAliasOutput {
    var result: CreateTemplateAliasOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTemplateAliasOutput, body, alloc);
    }
