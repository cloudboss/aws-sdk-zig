...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchFlowsInput, options: Options) !SearchFlowsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchFlowsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/flows/searchFlows");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchFlowsOutput {
    var result: SearchFlowsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchFlowsOutput, body, alloc);
    }
