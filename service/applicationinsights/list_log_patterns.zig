...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLogPatternsInput, options: Options) !ListLogPatternsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLogPatternsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EC2WindowsBarleyService.ListLogPatterns");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLogPatternsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLogPatternsOutput, body, alloc);
}
