...
pub const CreateLogPatternInput = struct {
...
pub const CreateLogPatternOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLogPatternInput, options: CallOptions) !CreateLogPatternOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLogPatternInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "EC2WindowsBarleyService.CreateLogPattern");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLogPatternOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLogPatternOutput, body, allocator);
}
