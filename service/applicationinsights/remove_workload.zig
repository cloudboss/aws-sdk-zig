...
pub const RemoveWorkloadInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveWorkloadInput, options: CallOptions) !RemoveWorkloadOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveWorkloadInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "EC2WindowsBarleyService.RemoveWorkload");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveWorkloadOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
