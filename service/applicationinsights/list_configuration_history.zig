...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConfigurationHistoryInput, options: Options) !ListConfigurationHistoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConfigurationHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EC2WindowsBarleyService.ListConfigurationHistory");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConfigurationHistoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListConfigurationHistoryOutput, body, alloc);
}
