...
pub const ListLogPatternSetsInput = struct {
...
pub const ListLogPatternSetsOutput = struct {
    /// The Amazon Web Services account ID for the resource group owner.
    account_id: ?[]const u8 = null,

    /// The list of log pattern sets.
    log_pattern_sets: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLogPatternSetsInput, options: CallOptions) !ListLogPatternSetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListLogPatternSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "EC2WindowsBarleyService.ListLogPatternSets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListLogPatternSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLogPatternSetsOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "TagsAlreadyExistException")) {
        const parsed_error: ?errors.TagsAlreadyExistException = aws.json.parseJsonObject(errors.TagsAlreadyExistException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
