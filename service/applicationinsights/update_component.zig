...
pub const UpdateComponentInput = struct {
    ...
    new_component_name: ?[]const u8 = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateComponentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EC2WindowsBarleyService.UpdateComponent");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateComponentOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
