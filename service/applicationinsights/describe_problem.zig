...
pub const DescribeProblemInput = struct {
...
pub const DescribeProblemOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeProblemInput, options: CallOptions) !DescribeProblemOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeProblemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "EC2WindowsBarleyService.DescribeProblem");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeProblemOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeProblemOutput, body, allocator);
}
