...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateComponentConfigurationInput, options: Options) !UpdateComponentConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateComponentConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EC2WindowsBarleyService.UpdateComponentConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateComponentConfigurationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
