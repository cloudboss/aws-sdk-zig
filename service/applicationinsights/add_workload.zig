...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddWorkloadInput, options: Options) !AddWorkloadOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddWorkloadInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EC2WindowsBarleyService.AddWorkload");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddWorkloadOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AddWorkloadOutput, body, alloc);
}
