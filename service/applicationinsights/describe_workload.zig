...
pub const DescribeWorkloadInput = struct {
...
pub const DescribeWorkloadOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWorkloadInput, options: CallOptions) !DescribeWorkloadOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeWorkloadInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "EC2WindowsBarleyService.DescribeWorkload");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeWorkloadOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWorkloadOutput, body, allocator);
}
