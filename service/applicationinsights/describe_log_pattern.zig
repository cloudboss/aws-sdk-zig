...
pub const DescribeLogPatternInput = struct {
...
pub const DescribeLogPatternOutput = struct {
    /// The Amazon Web Services account ID for the resource group owner.
    account_id: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeLogPatternInput, options: CallOptions) !DescribeLogPatternOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeLogPatternInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "EC2WindowsBarleyService.DescribeLogPattern");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeLogPatternOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeLogPatternOutput, body, allocator);
}
