...
pub const UpdateProblemInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProblemInput, options: Options) !UpdateProblemOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateProblemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "EC2WindowsBarleyService.UpdateProblem");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateProblemOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
