...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProblemInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EC2WindowsBarleyService.UpdateProblem");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProblemOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
