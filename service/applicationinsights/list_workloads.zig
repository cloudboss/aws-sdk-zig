...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWorkloadsInput, options: Options) !ListWorkloadsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWorkloadsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EC2WindowsBarleyService.ListWorkloads");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWorkloadsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListWorkloadsOutput, body, alloc);
}
