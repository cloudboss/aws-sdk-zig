...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProblemsInput, options: Options) !ListProblemsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProblemsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "EC2WindowsBarleyService.ListProblems");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProblemsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProblemsOutput, body, alloc);
}
