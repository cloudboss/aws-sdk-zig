...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGraphSnapshotInput, options: Options) !DeleteGraphSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGraphSnapshotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGraphSnapshotOutput {
    var result: DeleteGraphSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteGraphSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
