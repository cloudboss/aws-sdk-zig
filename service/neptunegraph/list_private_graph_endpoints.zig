...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPrivateGraphEndpointsInput, options: Options) !ListPrivateGraphEndpointsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPrivateGraphEndpointsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPrivateGraphEndpointsOutput {
    var result: ListPrivateGraphEndpointsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPrivateGraphEndpointsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
