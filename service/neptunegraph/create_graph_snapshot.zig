...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGraphSnapshotInput, options: Options) !CreateGraphSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGraphSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"graphIdentifier\":");
    try aws.json.writeValue(@TypeOf(input.graph_identifier), input.graph_identifier, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGraphSnapshotOutput {
    var result: CreateGraphSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGraphSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
