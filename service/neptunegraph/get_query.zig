...
pub const GetQueryInput = struct {
...
pub const GetQueryOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetQueryInput, options: CallOptions) !GetQueryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetQueryInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetQueryOutput {
    var result: GetQueryOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetQueryOutput, body, allocator);
    }
    _ = status;
    _ = headers;
