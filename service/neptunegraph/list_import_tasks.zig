...
pub const ListImportTasksOutput = struct {
    /// Pagination token used to paginate output.
    ///
    /// When this value is provided as input, the service returns results from where
    /// the previous response left off. When this value is present in output, it
    ...
    tasks: ?[]const ImportTaskSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListImportTasksInput, options: Options) !ListImportTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListImportTasksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListImportTasksOutput {
    var result: ListImportTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListImportTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
