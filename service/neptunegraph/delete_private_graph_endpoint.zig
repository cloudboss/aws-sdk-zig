...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePrivateGraphEndpointInput, options: Options) !DeletePrivateGraphEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeletePrivateGraphEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeletePrivateGraphEndpointOutput {
    var result: DeletePrivateGraphEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeletePrivateGraphEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
