...
pub const GetGraphInput = struct {
...
pub const GetGraphOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGraphInput, options: CallOptions) !GetGraphOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetGraphInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetGraphOutput {
    var result: GetGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGraphOutput, body, allocator);
    }
    _ = status;
    _ = headers;
