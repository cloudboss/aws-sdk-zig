...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGraphInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGraphOutput {
    var result: GetGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGraphOutput, body, alloc);
    }
    _ = status;
    _ = headers;
