...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGraphsInput, options: Options) !ListGraphsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGraphsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGraphsOutput {
    var result: ListGraphsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGraphsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
