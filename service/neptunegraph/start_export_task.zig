...
fn serializeRequest(alloc: std.mem.Allocator, input: StartExportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.export_filter) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"exportFilter\":");
    ...
    try aws.json.writeValue(@TypeOf(input.kms_key_identifier), input.kms_key_identifier, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartExportTaskOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartExportTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
