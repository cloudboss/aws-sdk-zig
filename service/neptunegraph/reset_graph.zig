...
pub const ResetGraphInput = struct {
    ...
    /// is deleted. If set to `true`, no graph snapshot is created. If set to
    /// `false`, a graph snapshot is created before the data is deleted.
    skip_snapshot: bool,
...
pub const ResetGraphOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetGraphInput, options: Options) !ResetGraphOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResetGraphInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"skipSnapshot\":");
    try aws.json.writeValue(@TypeOf(input.skip_snapshot), input.skip_snapshot, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResetGraphOutput {
    var result: ResetGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ResetGraphOutput, body, alloc);
    }
    _ = status;
    _ = headers;
