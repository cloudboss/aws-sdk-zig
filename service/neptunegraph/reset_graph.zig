...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResetGraphInput, options: CallOptions) !ResetGraphOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResetGraphInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "\"skipSnapshot\":");
    try aws.json.writeValue(@TypeOf(input.skip_snapshot), input.skip_snapshot, allocator, &body_buf);
    has_prev = true;
