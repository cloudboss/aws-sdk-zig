...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ExecuteQueryInput, options: Options) !ExecuteQueryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ExecuteQueryInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.explain_mode) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"explainMode\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"language\":");
    try aws.json.writeValue(@TypeOf(input.language), input.language, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.plan_cache) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"planCache\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.query_timeout_milliseconds) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"queryTimeoutMilliseconds\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeStreamingResponse(stream_resp: *aws.http.StreamingResponse, alloc: std.mem.Allocator) !ExecuteQueryOutput {
    _ = alloc;
