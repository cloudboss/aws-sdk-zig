...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGraphInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGraphOutput {
    var result: UpdateGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGraphOutput, body, alloc);
    }
    _ = status;
    _ = headers;
