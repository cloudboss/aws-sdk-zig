...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPrivateGraphEndpointInput, options: Options) !GetPrivateGraphEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPrivateGraphEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.vpc_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPrivateGraphEndpointOutput {
    var result: GetPrivateGraphEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetPrivateGraphEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
