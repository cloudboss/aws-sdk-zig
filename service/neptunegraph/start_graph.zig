...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartGraphInput, options: Options) !StartGraphOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartGraphInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartGraphOutput {
    var result: StartGraphOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartGraphOutput, body, alloc);
    }
    _ = status;
    _ = headers;
