...
const GraphSnapshotSummary = @import("graph_snapshot_summary.zig").GraphSnapshotSummary;

pub const ListGraphSnapshotsInput = struct {
...
pub const ListGraphSnapshotsOutput = struct {
    /// The requested list of snapshots.
    graph_snapshots: ?[]const GraphSnapshotSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGraphSnapshotsInput, options: CallOptions) !ListGraphSnapshotsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListGraphSnapshotsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListGraphSnapshotsOutput {
    var result: ListGraphSnapshotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGraphSnapshotsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
