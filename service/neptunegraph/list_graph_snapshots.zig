...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGraphSnapshotsInput, options: Options) !ListGraphSnapshotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGraphSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.graph_identifier) |v| {
        ...
        try query_buf.appendSlice(alloc, "graphIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGraphSnapshotsOutput {
    var result: ListGraphSnapshotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGraphSnapshotsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
