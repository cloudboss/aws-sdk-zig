...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExportTasksInput, options: Options) !ListExportTasksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExportTasksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExportTasksOutput {
    var result: ListExportTasksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListExportTasksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
