...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetImportTaskInput, options: Options) !GetImportTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetImportTaskInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetImportTaskOutput {
    var result: GetImportTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetImportTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
