...
pub const TagResourceInput = struct {
    ...
    ///
    /// **Key** (string)   –   A key is the required name of the tag. The string
    ...
    /// specified by this Java regular expression:
    /// `"^([\p{L}\p{Z}\p{N}_.:/=+\-]*)$")`.
    ///
    /// **Value** (string)   –   A value is the optional value of the tag. The
    /// string value can be from 1 to 256 Unicode characters in length. It can't be
    /// prefixed with `aws:` and can only contain the set of Unicode characters
    /// specified by this Java regular expression:
    /// `"^([\p{L}\p{Z}\p{N}_.:/=+\-]*)$")`.
