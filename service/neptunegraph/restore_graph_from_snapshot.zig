...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreGraphFromSnapshotInput, options: Options) !RestoreGraphFromSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestoreGraphFromSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.snapshot_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestoreGraphFromSnapshotOutput {
    var result: RestoreGraphFromSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RestoreGraphFromSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
