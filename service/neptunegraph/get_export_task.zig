...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExportTaskInput, options: Options) !GetExportTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExportTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/exporttasks/");
    try path_buf.appendSlice(alloc, input.task_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExportTaskOutput {
    var result: GetExportTaskOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetExportTaskOutput, body, alloc);
    }
    _ = status;
    _ = headers;
