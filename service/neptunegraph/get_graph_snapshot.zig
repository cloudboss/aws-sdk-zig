...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGraphSnapshotInput, options: Options) !GetGraphSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGraphSnapshotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGraphSnapshotOutput {
    var result: GetGraphSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGraphSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
