...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGraphInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "skipSnapshot=");
    try query_buf.appendSlice(alloc, if (input.skip_snapshot) "true" else "false");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGraphOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteGraphOutput, body, alloc);
    }
    _ = status;
    _ = headers;
