...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResolveCustomerInput, options: Options) !ResolveCustomerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ResolveCustomerInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("marketplacemetering", "Marketplace Metering", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMPMeteringService.ResolveCustomer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ResolveCustomerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResolveCustomerOutput, body, alloc);
}
