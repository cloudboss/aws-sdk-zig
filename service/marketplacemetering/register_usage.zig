...
pub const RegisterUsageInput = struct {
...
pub const RegisterUsageOutput = struct {
    /// (Optional) Only included when public key version has expired
    public_key_rotation_timestamp: ?i64 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterUsageInput, options: CallOptions) !RegisterUsageOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMPMeteringService.RegisterUsage");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterUsageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterUsageOutput, body, allocator);
}
