...
const batch_meter_usage = @import("batch_meter_usage.zig");
...
pub const Client = struct {
    ...
    const Self = @This();
    pub const sdk_id = "Marketplace Metering";
    ...
    /// uses this API action to emit metering records directly to Amazon Web
    /// Services Marketplace.
    /// You must use the following buyer Amazon Web Services account credentials to
    ...
    /// [EKS Pod
    /// Identity](https://docs.aws.amazon.com/eks/latest/userguide/pod-identities.html), the node role, or long-term access keys is not supported.
    ///
    ...
    ///
    /// * For **Amazon Bedrock AgentCore Runtime** deployments, your software must
    ///   use the
    ...
    /// to sign the API call for the `MeterUsage` API operation. Long-term access
    ...
    ///
    /// * For **non-Amazon Bedrock AgentCore Runtime** deployments, you can only
    ///   report usage once per hour for each dimension.
    ...
    /// can't modify values
    /// after they're recorded. If you report usage before a current hour ends, you
    /// will be unable to report additional usage until the next hour begins.
    /// The `Timestamp` request parameter is rounded down to the hour and used to
    /// enforce this once-per-hour rule for idempotency.
    /// For requests that are identical after the `Timestamp` is rounded down, the
    ...
    /// You do not need to aggregate metering records by the hour. You must include
    /// an idempotency token in the `ClientToken` request parameter. If using an
    /// Amazon
    ...
    /// won't be accepted. The timestamp in your request determines when an event is
    /// recorded.
    ///
    ...
    /// entitlement and metering. Free and BYOL products for Amazon ECS or Amazon
    /// EKS aren't required to call `RegisterUsage`, but you may choose to
    ...
    /// cluster and a service configured as a Daemon Set, then Amazon ECS or Amazon
    /// EKS will launch a task on all 10 cluster nodes
    ...
    /// immediately at launch. If you don’t register the container within the first
    /// 6 hours
    ...
    /// example](https://docs.aws.amazon.com/marketplace/latest/userguide/saas-code-examples.html#saas-resolvecustomer-example) in the *Amazon Web Services Marketplace Seller
    /// Guide*.
    ///
    ...
    /// for Amazon Web Services Marketplace Metering
    /// Service](https://docs.aws.amazon.com/service-authorization/latest/reference/list_awsmarketplacemeteringservice.html) in
