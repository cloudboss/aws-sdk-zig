...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchMeterUsageInput, options: Options) !BatchMeterUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchMeterUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMPMeteringService.BatchMeterUsage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchMeterUsageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchMeterUsageOutput, body, alloc);
}
