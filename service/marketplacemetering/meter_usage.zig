...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MeterUsageInput, options: Options) !MeterUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: MeterUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMPMeteringService.MeterUsage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MeterUsageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(MeterUsageOutput, body, alloc);
}
