...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePredictorBacktestExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.CreatePredictorBacktestExportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePredictorBacktestExportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePredictorBacktestExportJobOutput, body, alloc);
}
