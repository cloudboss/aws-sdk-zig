...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePredictorBacktestExportJobInput, options: Options) !DescribePredictorBacktestExportJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePredictorBacktestExportJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.DescribePredictorBacktestExportJob");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePredictorBacktestExportJobOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePredictorBacktestExportJobOutput, body, alloc);
}
