...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAccuracyMetricsInput, options: Options) !GetAccuracyMetricsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAccuracyMetricsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.GetAccuracyMetrics");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAccuracyMetricsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAccuracyMetricsOutput, body, alloc);
}
