...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWhatIfForecastInput, options: Options) !DescribeWhatIfForecastOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWhatIfForecastInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.DescribeWhatIfForecast");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWhatIfForecastOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWhatIfForecastOutput, body, alloc);
}
