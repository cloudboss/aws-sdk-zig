...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeExplainabilityInput, options: Options) !DescribeExplainabilityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeExplainabilityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.DescribeExplainability");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeExplainabilityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeExplainabilityOutput, body, alloc);
}
