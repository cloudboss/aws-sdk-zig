...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePredictorInput, options: CallOptions) !CreatePredictorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreatePredictorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonForecast.CreatePredictor");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreatePredictorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePredictorOutput, body, allocator);
}
