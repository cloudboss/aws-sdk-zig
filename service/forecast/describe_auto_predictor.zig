...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAutoPredictorInput, options: Options) !DescribeAutoPredictorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAutoPredictorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.DescribeAutoPredictor");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAutoPredictorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAutoPredictorOutput, body, alloc);
}
