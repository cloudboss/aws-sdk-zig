...
pub const CreateExplainabilityExportInput = struct {
...
pub const CreateExplainabilityExportOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateExplainabilityExportInput, options: CallOptions) !CreateExplainabilityExportOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateExplainabilityExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonForecast.CreateExplainabilityExport");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateExplainabilityExportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateExplainabilityExportOutput, body, allocator);
}
