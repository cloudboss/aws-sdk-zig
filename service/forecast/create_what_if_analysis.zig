...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWhatIfAnalysisInput, options: Options) !CreateWhatIfAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWhatIfAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.CreateWhatIfAnalysis");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWhatIfAnalysisOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateWhatIfAnalysisOutput, body, alloc);
}
