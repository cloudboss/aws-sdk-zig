...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPredictorBacktestExportJobsInput, options: Options) !ListPredictorBacktestExportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPredictorBacktestExportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.ListPredictorBacktestExportJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPredictorBacktestExportJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPredictorBacktestExportJobsOutput, body, alloc);
}
