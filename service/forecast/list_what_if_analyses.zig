...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWhatIfAnalysesInput, options: CallOptions) !ListWhatIfAnalysesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListWhatIfAnalysesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonForecast.ListWhatIfAnalyses");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListWhatIfAnalysesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListWhatIfAnalysesOutput, body, allocator);
}
