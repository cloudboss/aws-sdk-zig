...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWhatIfForecastExportsInput, options: Options) !ListWhatIfForecastExportsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWhatIfForecastExportsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.ListWhatIfForecastExports");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWhatIfForecastExportsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListWhatIfForecastExportsOutput, body, alloc);
}
