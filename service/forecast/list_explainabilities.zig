...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListExplainabilitiesInput, options: Options) !ListExplainabilitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListExplainabilitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.ListExplainabilities");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListExplainabilitiesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListExplainabilitiesOutput, body, alloc);
}
