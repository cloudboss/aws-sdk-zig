...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMonitorEvaluationsInput, options: Options) !ListMonitorEvaluationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMonitorEvaluationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.ListMonitorEvaluations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMonitorEvaluationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMonitorEvaluationsOutput, body, alloc);
}
