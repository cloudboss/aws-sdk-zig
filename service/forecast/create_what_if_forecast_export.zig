...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWhatIfForecastExportInput, options: Options) !CreateWhatIfForecastExportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWhatIfForecastExportInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.CreateWhatIfForecastExport");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWhatIfForecastExportOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateWhatIfForecastExportOutput, body, alloc);
}
