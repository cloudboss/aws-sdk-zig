...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDatasetGroupInput, options: CallOptions) !CreateDatasetGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateDatasetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonForecast.CreateDatasetGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateDatasetGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDatasetGroupOutput, body, allocator);
}
