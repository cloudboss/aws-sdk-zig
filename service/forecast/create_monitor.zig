...
const CreateMonitorInput = @import("create_monitor_request.zig").CreateMonitorRequest;

const CreateMonitorOutput = @import("create_monitor_response.zig").CreateMonitorResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMonitorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.CreateMonitor");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMonitorOutput {
    ...
    return aws.json.parseJsonObject(CreateMonitorOutput, body, alloc);
}
