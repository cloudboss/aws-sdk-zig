...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeWhatIfAnalysisInput, options: Options) !DescribeWhatIfAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeWhatIfAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.DescribeWhatIfAnalysis");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeWhatIfAnalysisOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeWhatIfAnalysisOutput, body, alloc);
}
