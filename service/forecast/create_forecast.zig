...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateForecastInput, options: Options) !CreateForecastOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateForecastInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.CreateForecast");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateForecastOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateForecastOutput, body, alloc);
}
