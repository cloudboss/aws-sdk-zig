...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListForecastExportJobsInput, options: Options) !ListForecastExportJobsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListForecastExportJobsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonForecast.ListForecastExportJobs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListForecastExportJobsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListForecastExportJobsOutput, body, alloc);
}
