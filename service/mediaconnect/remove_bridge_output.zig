...
pub const RemoveBridgeOutputInput = struct {
...
pub const RemoveBridgeOutputOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveBridgeOutputInput, options: CallOptions) !RemoveBridgeOutputOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveBridgeOutputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveBridgeOutputOutput {
    var result: RemoveBridgeOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveBridgeOutputOutput, body, allocator);
    }
    _ = status;
    _ = headers;
