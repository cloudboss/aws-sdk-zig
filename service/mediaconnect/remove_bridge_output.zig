...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveBridgeOutputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveBridgeOutputOutput {
    var result: RemoveBridgeOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveBridgeOutputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
