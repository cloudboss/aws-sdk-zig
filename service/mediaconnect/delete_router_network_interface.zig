...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRouterNetworkInterfaceInput, options: Options) !DeleteRouterNetworkInterfaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRouterNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRouterNetworkInterfaceOutput {
    var result: DeleteRouterNetworkInterfaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRouterNetworkInterfaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
