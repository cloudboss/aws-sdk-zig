...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBridgeInput, options: Options) !CreateBridgeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBridgeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.egress_gateway_bridge) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"EgressGatewayBridge\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.ingress_gateway_bridge) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IngressGatewayBridge\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PlacementArn\":");
    try aws.json.writeValue(@TypeOf(input.placement_arn), input.placement_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBridgeOutput {
    var result: CreateBridgeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateBridgeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
