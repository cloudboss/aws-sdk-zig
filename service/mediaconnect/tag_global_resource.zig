...
const TagGlobalResourceOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TagGlobalResourceInput, options: Options) !TagGlobalResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TagGlobalResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.tags), input.tags, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TagGlobalResourceOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
