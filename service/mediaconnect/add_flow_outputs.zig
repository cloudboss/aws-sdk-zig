...
fn serializeRequest(alloc: std.mem.Allocator, input: AddFlowOutputsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddFlowOutputsOutput {
    var result: AddFlowOutputsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddFlowOutputsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
