...
pub const AddFlowOutputsInput = struct {
    ...
    outputs: []const AddOutputRequest,
...
pub const AddFlowOutputsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddFlowOutputsInput, options: CallOptions) !AddFlowOutputsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AddFlowOutputsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AddFlowOutputsOutput {
    var result: AddFlowOutputsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddFlowOutputsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
