...
pub const StartRouterOutputInput = struct {
...
pub const StartRouterOutputOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRouterOutputInput, options: CallOptions) !StartRouterOutputOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartRouterOutputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/v1/routerOutput/start/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartRouterOutputOutput {
    var result: StartRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartRouterOutputOutput, body, allocator);
    }
    _ = status;
    _ = headers;
