...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRouterOutputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerOutput/start/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRouterOutputOutput {
    var result: StartRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartRouterOutputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
