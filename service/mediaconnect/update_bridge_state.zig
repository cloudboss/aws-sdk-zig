...
pub const UpdateBridgeStateInput = struct {
...
pub const UpdateBridgeStateOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBridgeStateInput, options: Options) !UpdateBridgeStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBridgeStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/state");
    ...
    try aws.json.writeValue(@TypeOf(input.desired_state), input.desired_state, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBridgeStateOutput {
    var result: UpdateBridgeStateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBridgeStateOutput, body, alloc);
    }
    _ = status;
    _ = headers;
