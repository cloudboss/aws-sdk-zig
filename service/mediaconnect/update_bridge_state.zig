...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBridgeStateInput, options: CallOptions) !UpdateBridgeStateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateBridgeStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.desired_state), input.desired_state, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateBridgeStateOutput {
    var result: UpdateBridgeStateOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBridgeStateOutput, body, allocator);
    }
    _ = status;
    _ = headers;
