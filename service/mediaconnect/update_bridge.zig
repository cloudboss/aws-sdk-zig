...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBridgeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBridgeOutput {
    var result: UpdateBridgeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBridgeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
