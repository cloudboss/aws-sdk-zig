...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRouterInputSourceMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/source-metadata");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRouterInputSourceMetadataOutput {
    var result: GetRouterInputSourceMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouterInputSourceMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
