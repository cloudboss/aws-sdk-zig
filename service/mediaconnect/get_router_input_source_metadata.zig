...
const RouterInputSourceMetadataDetails = @import("router_input_source_metadata_details.zig").RouterInputSourceMetadataDetails;

pub const GetRouterInputSourceMetadataInput = struct {
...
pub const GetRouterInputSourceMetadataOutput = struct {
    ...
    source_metadata_details: ?RouterInputSourceMetadataDetails = null,

    pub const json_field_names = .{
        .arn = "Arn",
        .name = "Name",
        .source_metadata_details = "SourceMetadataDetails",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouterInputSourceMetadataInput, options: CallOptions) !GetRouterInputSourceMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRouterInputSourceMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/source-metadata");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRouterInputSourceMetadataOutput {
    var result: GetRouterInputSourceMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouterInputSourceMetadataOutput, body, allocator);
    }
    _ = status;
    _ = headers;
