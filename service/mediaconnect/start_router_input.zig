...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartRouterInputInput, options: Options) !StartRouterInputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartRouterInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerInput/start/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartRouterInputOutput {
    var result: StartRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartRouterInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
