...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRouterNetworkInterfaceInput, options: Options) !UpdateRouterNetworkInterfaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRouterNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRouterNetworkInterfaceOutput {
    var result: UpdateRouterNetworkInterfaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRouterNetworkInterfaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
