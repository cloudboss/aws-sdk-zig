...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveFlowSourceInput, options: Options) !RemoveFlowSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveFlowSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveFlowSourceOutput {
    var result: RemoveFlowSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveFlowSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
