...
pub const RemoveFlowSourceInput = struct {
...
pub const RemoveFlowSourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveFlowSourceInput, options: CallOptions) !RemoveFlowSourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveFlowSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveFlowSourceOutput {
    var result: RemoveFlowSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveFlowSourceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
