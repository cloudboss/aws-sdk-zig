...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetRouterNetworkInterfaceInput, options: Options) !BatchGetRouterNetworkInterfaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetRouterNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "arns=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.arns);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetRouterNetworkInterfaceOutput {
    var result: BatchGetRouterNetworkInterfaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetRouterNetworkInterfaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
