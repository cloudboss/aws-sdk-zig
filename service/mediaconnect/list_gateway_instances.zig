...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGatewayInstancesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGatewayInstancesOutput {
    var result: ListGatewayInstancesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGatewayInstancesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
