...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRouterOutputInput, options: Options) !UpdateRouterOutputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRouterOutputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRouterOutputOutput {
    var result: UpdateRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRouterOutputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
