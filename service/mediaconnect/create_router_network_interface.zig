...
pub const CreateRouterNetworkInterfaceInput = struct {
...
pub const CreateRouterNetworkInterfaceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRouterNetworkInterfaceInput, options: CallOptions) !CreateRouterNetworkInterfaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateRouterNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("mediaconnect", "MediaConnect", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    const path = "/v1/routerNetworkInterface";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateRouterNetworkInterfaceOutput {
    var result: CreateRouterNetworkInterfaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRouterNetworkInterfaceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
