...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRouterNetworkInterfaceInput, options: Options) !CreateRouterNetworkInterfaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRouterNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRouterNetworkInterfaceOutput {
    var result: CreateRouterNetworkInterfaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRouterNetworkInterfaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
