...
pub const RestartRouterInputInput = struct {
...
pub const RestartRouterInputOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestartRouterInputInput, options: Options) !RestartRouterInputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RestartRouterInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerInput/restart/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestartRouterInputOutput {
    var result: RestartRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RestartRouterInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
