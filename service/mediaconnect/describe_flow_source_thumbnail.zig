...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFlowSourceThumbnailInput, options: Options) !DescribeFlowSourceThumbnailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFlowSourceThumbnailInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/source-thumbnail");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFlowSourceThumbnailOutput {
    var result: DescribeFlowSourceThumbnailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFlowSourceThumbnailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
