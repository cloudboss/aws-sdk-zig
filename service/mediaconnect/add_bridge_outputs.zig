...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddBridgeOutputsInput, options: Options) !AddBridgeOutputsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddBridgeOutputsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddBridgeOutputsOutput {
    var result: AddBridgeOutputsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddBridgeOutputsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
