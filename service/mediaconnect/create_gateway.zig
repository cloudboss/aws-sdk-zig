...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"EgressCidrBlocks\":");
    try aws.json.writeValue(@TypeOf(input.egress_cidr_blocks), input.egress_cidr_blocks, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Networks\":");
    try aws.json.writeValue(@TypeOf(input.networks), input.networks, alloc, &body_buf);
    has_prev = true;
