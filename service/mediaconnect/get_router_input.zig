...
pub const GetRouterInputInput = struct {
...
pub const GetRouterInputOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouterInputInput, options: Options) !GetRouterInputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRouterInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerInput/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRouterInputOutput {
    var result: GetRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouterInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
