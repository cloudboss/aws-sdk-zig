...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetRouterOutputInput, options: Options) !BatchGetRouterOutputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetRouterOutputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetRouterOutputOutput {
    var result: BatchGetRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetRouterOutputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
