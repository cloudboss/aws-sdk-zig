...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFlowMediaStreamInput, options: Options) !UpdateFlowMediaStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFlowMediaStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/mediaStreams/");
    try path_buf.appendSlice(alloc, input.media_stream_name);
    ...
    if (input.clock_rate) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ClockRate\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.media_stream_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"MediaStreamType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.video_format) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"VideoFormat\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFlowMediaStreamOutput {
    var result: UpdateFlowMediaStreamOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateFlowMediaStreamOutput, body, alloc);
    }
    _ = status;
    _ = headers;
