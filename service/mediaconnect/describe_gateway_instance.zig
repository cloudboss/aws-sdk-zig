...
const GatewayInstance = @import("gateway_instance.zig").GatewayInstance;

pub const DescribeGatewayInstanceInput = struct {
...
pub const DescribeGatewayInstanceOutput = struct {
    /// The gateway instance that you requested a description of.
    gateway_instance: ?GatewayInstance = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGatewayInstanceInput, options: Options) !DescribeGatewayInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGatewayInstanceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGatewayInstanceOutput {
    var result: DescribeGatewayInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeGatewayInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
