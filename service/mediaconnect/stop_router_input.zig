...
pub const StopRouterInputInput = struct {
...
pub const StopRouterInputOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopRouterInputInput, options: CallOptions) !StopRouterInputOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopRouterInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/v1/routerInput/stop/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopRouterInputOutput {
    var result: StopRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopRouterInputOutput, body, allocator);
    }
    _ = status;
    _ = headers;
