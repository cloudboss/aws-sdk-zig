...
fn serializeRequest(alloc: std.mem.Allocator, input: StopRouterInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerInput/stop/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopRouterInputOutput {
    var result: StopRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopRouterInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
