...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetRouterInputInput, options: Options) !BatchGetRouterInputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetRouterInputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetRouterInputOutput {
    var result: BatchGetRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchGetRouterInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
