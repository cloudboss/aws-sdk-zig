...
pub const GetRouterNetworkInterfaceInput = struct {
...
pub const GetRouterNetworkInterfaceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouterNetworkInterfaceInput, options: CallOptions) !GetRouterNetworkInterfaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRouterNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRouterNetworkInterfaceOutput {
    var result: GetRouterNetworkInterfaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouterNetworkInterfaceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
