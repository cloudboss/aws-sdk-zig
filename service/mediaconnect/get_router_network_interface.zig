...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouterNetworkInterfaceInput, options: Options) !GetRouterNetworkInterfaceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRouterNetworkInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerNetworkInterface/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRouterNetworkInterfaceOutput {
    var result: GetRouterNetworkInterfaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouterNetworkInterfaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
