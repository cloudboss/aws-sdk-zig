...
const NdiSourceMetadataInfo = @import("ndi_source_metadata_info.zig").NdiSourceMetadataInfo;
const TransportMediaInfo = @import("transport_media_info.zig").TransportMediaInfo;

pub const DescribeFlowSourceMetadataInput = struct {
...
pub const DescribeFlowSourceMetadataOutput = struct {
    ...
    /// messages.
    ndi_info: ?NdiSourceMetadataInfo = null,

    /// The timestamp of the most recent change in metadata for this flow’s source.
    timestamp: ?i64 = null,

    /// Information about the flow's transport media.
    transport_media_info: ?TransportMediaInfo = null,
    ...
    pub const json_field_names = .{
        ...
        .ndi_info = "NdiInfo",
        .timestamp = "Timestamp",
        .transport_media_info = "TransportMediaInfo",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFlowSourceMetadataInput, options: CallOptions) !DescribeFlowSourceMetadataOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFlowSourceMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFlowSourceMetadataOutput {
    var result: DescribeFlowSourceMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFlowSourceMetadataOutput, body, allocator);
    }
    _ = status;
    _ = headers;
