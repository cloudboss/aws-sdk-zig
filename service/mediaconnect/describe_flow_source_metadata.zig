...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFlowSourceMetadataInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFlowSourceMetadataOutput {
    var result: DescribeFlowSourceMetadataOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFlowSourceMetadataOutput, body, alloc);
    }
    _ = status;
    _ = headers;
