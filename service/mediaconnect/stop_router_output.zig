...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopRouterOutputInput, options: Options) !StopRouterOutputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopRouterOutputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerOutput/stop/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopRouterOutputOutput {
    var result: StopRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StopRouterOutputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
