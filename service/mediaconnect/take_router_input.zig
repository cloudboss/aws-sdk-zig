...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TakeRouterInputInput, options: Options) !TakeRouterInputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TakeRouterInputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerOutput/takeRouterInput/");
    try path_buf.appendSlice(alloc, input.router_output_arn);
    ...
    if (input.router_input_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"RouterInputArn\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TakeRouterInputOutput {
    var result: TakeRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(TakeRouterInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
