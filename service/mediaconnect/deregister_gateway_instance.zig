...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterGatewayInstanceInput, options: Options) !DeregisterGatewayInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterGatewayInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/gateway-instances/");
    try path_buf.appendSlice(alloc, input.gateway_instance_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterGatewayInstanceOutput {
    var result: DeregisterGatewayInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeregisterGatewayInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
