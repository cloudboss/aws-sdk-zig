...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBridgeSourceInput, options: Options) !UpdateBridgeSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBridgeSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sources/");
    try path_buf.appendSlice(alloc, input.source_name);
    ...
    if (input.flow_source) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"FlowSource\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.network_source) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"NetworkSource\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBridgeSourceOutput {
    var result: UpdateBridgeSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBridgeSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
