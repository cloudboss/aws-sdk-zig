...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTagsForGlobalResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/tags/global/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTagsForGlobalResourceOutput {
    var result: ListTagsForGlobalResourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListTagsForGlobalResourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
