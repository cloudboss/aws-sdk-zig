...
fn serializeRequest(alloc: std.mem.Allocator, input: AddFlowMediaStreamsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.media_streams), input.media_streams, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddFlowMediaStreamsOutput {
    var result: AddFlowMediaStreamsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddFlowMediaStreamsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
