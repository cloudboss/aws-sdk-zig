...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRouterInputsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRouterInputsOutput {
    var result: ListRouterInputsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRouterInputsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
