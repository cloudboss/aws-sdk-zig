...
const RouterInputFilter = @import("router_input_filter.zig").RouterInputFilter;
const ListedRouterInput = @import("listed_router_input.zig").ListedRouterInput;

pub const ListRouterInputsInput = struct {
    /// The filters to apply when retrieving the list of router inputs.
    filters: ?[]const RouterInputFilter = null,
...
pub const ListRouterInputsOutput = struct {
    /// The token to use to retrieve the next page of results.
    next_token: ?[]const u8 = null,

    /// The summary information for the retrieved router inputs.
    router_inputs: ?[]const ListedRouterInput = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRouterInputsInput, options: CallOptions) !ListRouterInputsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRouterInputsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRouterInputsOutput {
    var result: ListRouterInputsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRouterInputsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
