...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeFlowEntitlementInput, options: Options) !RevokeFlowEntitlementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RevokeFlowEntitlementInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RevokeFlowEntitlementOutput {
    var result: RevokeFlowEntitlementOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RevokeFlowEntitlementOutput, body, alloc);
    }
    _ = status;
    _ = headers;
