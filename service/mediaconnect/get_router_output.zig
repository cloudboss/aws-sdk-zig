...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouterOutputInput, options: Options) !GetRouterOutputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRouterOutputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRouterOutputOutput {
    var result: GetRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouterOutputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
