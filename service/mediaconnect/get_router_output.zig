...
pub const GetRouterOutputInput = struct {
...
pub const GetRouterOutputOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouterOutputInput, options: CallOptions) !GetRouterOutputOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRouterOutputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRouterOutputOutput {
    var result: GetRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouterOutputOutput, body, allocator);
    }
    _ = status;
    _ = headers;
