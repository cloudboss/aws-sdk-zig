...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGatewayInstanceInput, options: Options) !UpdateGatewayInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGatewayInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.bridge_placement) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"BridgePlacement\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGatewayInstanceOutput {
    var result: UpdateGatewayInstanceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGatewayInstanceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
