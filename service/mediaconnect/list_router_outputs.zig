...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRouterOutputsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRouterOutputsOutput {
    var result: ListRouterOutputsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRouterOutputsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
