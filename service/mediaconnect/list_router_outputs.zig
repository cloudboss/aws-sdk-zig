...
const RouterOutputFilter = @import("router_output_filter.zig").RouterOutputFilter;
const ListedRouterOutput = @import("listed_router_output.zig").ListedRouterOutput;

pub const ListRouterOutputsInput = struct {
    /// The filters to apply when retrieving the list of router outputs.
    filters: ?[]const RouterOutputFilter = null,
...
pub const ListRouterOutputsOutput = struct {
    /// The token to use to retrieve the next page of results.
    next_token: ?[]const u8 = null,

    /// The summary information for the retrieved router outputs.
    router_outputs: ?[]const ListedRouterOutput = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRouterOutputsInput, options: CallOptions) !ListRouterOutputsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRouterOutputsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRouterOutputsOutput {
    var result: ListRouterOutputsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRouterOutputsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
