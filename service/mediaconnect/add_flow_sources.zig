...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddFlowSourcesInput, options: Options) !AddFlowSourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddFlowSourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddFlowSourcesOutput {
    var result: AddFlowSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddFlowSourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
