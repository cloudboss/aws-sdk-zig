...
pub const DescribeBridgeInput = struct {
...
pub const DescribeBridgeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBridgeInput, options: CallOptions) !DescribeBridgeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeBridgeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeBridgeOutput {
    var result: DescribeBridgeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBridgeOutput, body, allocator);
    }
    _ = status;
    _ = headers;
