...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBridgeInput, options: Options) !DescribeBridgeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBridgeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBridgeOutput {
    var result: DescribeBridgeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeBridgeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
