...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRouterInputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRouterInputOutput {
    var result: UpdateRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRouterInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
