...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRouterInputInput, options: Options) !CreateRouterInputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRouterInputInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.transit_encryption) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TransitEncryption\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRouterInputOutput {
    var result: CreateRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRouterInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
