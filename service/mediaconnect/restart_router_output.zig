...
fn serializeRequest(alloc: std.mem.Allocator, input: RestartRouterOutputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerOutput/restart/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RestartRouterOutputOutput {
    var result: RestartRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RestartRouterOutputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
