...
pub const RestartRouterOutputInput = struct {
...
pub const RestartRouterOutputOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestartRouterOutputInput, options: CallOptions) !RestartRouterOutputOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RestartRouterOutputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/v1/routerOutput/restart/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RestartRouterOutputOutput {
    var result: RestartRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RestartRouterOutputOutput, body, allocator);
    }
    _ = status;
    _ = headers;
