...
fn serializeRequest(alloc: std.mem.Allocator, input: AddBridgeSourcesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddBridgeSourcesOutput {
    var result: AddBridgeSourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddBridgeSourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
