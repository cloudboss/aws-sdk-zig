...
const ListedEntitlement = @import("listed_entitlement.zig").ListedEntitlement;
...
pub const ListEntitlementsOutput = struct {
    ...
    /// Services accounts.
    entitlements: ?[]const ListedEntitlement = null,

    /// The token that identifies the batch of results that you want to see.
    ///
    /// For example, you submit a ListEntitlements request with `MaxResults` set at
    /// 5. The service returns the first batch of results (up to 5) and a NextToken
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEntitlementsInput, options: Options) !ListEntitlementsOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListEntitlementsOutput {
    var result: ListEntitlementsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEntitlementsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
