...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEntitlementsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEntitlementsOutput {
    var result: ListEntitlementsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEntitlementsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
