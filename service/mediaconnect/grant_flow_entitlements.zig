...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GrantFlowEntitlementsInput, options: Options) !GrantFlowEntitlementsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GrantFlowEntitlementsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.entitlements), input.entitlements, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GrantFlowEntitlementsOutput {
    var result: GrantFlowEntitlementsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GrantFlowEntitlementsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
