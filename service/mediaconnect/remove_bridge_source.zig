...
pub const RemoveBridgeSourceInput = struct {
...
pub const RemoveBridgeSourceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveBridgeSourceInput, options: Options) !RemoveBridgeSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveBridgeSourceInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveBridgeSourceOutput {
    var result: RemoveBridgeSourceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveBridgeSourceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
