...
pub const AddFlowVpcInterfacesInput = struct {
    /// The Amazon Resource Name (ARN) of the flow that you want to update.
    flow_arn: []const u8,

    /// A list of VPC interfaces that you want to add to the flow.
    vpc_interfaces: []const VpcInterfaceRequest,
...
pub const AddFlowVpcInterfacesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddFlowVpcInterfacesInput, options: Options) !AddFlowVpcInterfacesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddFlowVpcInterfacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/vpcInterfaces");
    ...
    try body_buf.appendSlice(alloc, "\"VpcInterfaces\":");
    try aws.json.writeValue(@TypeOf(input.vpc_interfaces), input.vpc_interfaces, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddFlowVpcInterfacesOutput {
    var result: AddFlowVpcInterfacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddFlowVpcInterfacesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
