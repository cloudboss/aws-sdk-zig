...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddFlowVpcInterfacesInput, options: CallOptions) !AddFlowVpcInterfacesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AddFlowVpcInterfacesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/vpcInterfaces");
    ...
    try aws.json.writeValue(@TypeOf(input.vpc_interfaces), input.vpc_interfaces, allocator, &body_buf);
    has_prev = true;
