...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRouterOutputInput, options: Options) !DeleteRouterOutputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRouterOutputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/routerOutput/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRouterOutputOutput {
    var result: DeleteRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRouterOutputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
