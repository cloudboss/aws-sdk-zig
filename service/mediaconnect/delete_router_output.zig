...
pub const DeleteRouterOutputInput = struct {
...
pub const DeleteRouterOutputOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRouterOutputInput, options: CallOptions) !DeleteRouterOutputOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRouterOutputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRouterOutputOutput {
    var result: DeleteRouterOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRouterOutputOutput, body, allocator);
    }
    _ = status;
    _ = headers;
