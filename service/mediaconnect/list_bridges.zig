...
const ListedBridge = @import("listed_bridge.zig").ListedBridge;

pub const ListBridgesInput = struct {
...
pub const ListBridgesOutput = struct {
    /// A list of bridge summaries.
    bridges: ?[]const ListedBridge = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBridgesInput, options: CallOptions) !ListBridgesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBridgesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.filter_arn) |v| {
        ...
        try query_buf.appendSlice(allocator, "filterArn=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBridgesOutput {
    var result: ListBridgesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBridgesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
