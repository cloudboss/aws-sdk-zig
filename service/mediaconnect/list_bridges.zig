...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBridgesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.filter_arn) |v| {
        ...
        try query_buf.appendSlice(alloc, "filterArn=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBridgesOutput {
    var result: ListBridgesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBridgesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
