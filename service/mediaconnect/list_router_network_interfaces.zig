...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRouterNetworkInterfacesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRouterNetworkInterfacesOutput {
    var result: ListRouterNetworkInterfacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRouterNetworkInterfacesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
