...
const RouterNetworkInterfaceFilter = @import("router_network_interface_filter.zig").RouterNetworkInterfaceFilter;
const ListedRouterNetworkInterface = @import("listed_router_network_interface.zig").ListedRouterNetworkInterface;

pub const ListRouterNetworkInterfacesInput = struct {
    /// The filters to apply when retrieving the list of router network interfaces.
    filters: ?[]const RouterNetworkInterfaceFilter = null,
...
pub const ListRouterNetworkInterfacesOutput = struct {
    /// The token to use to retrieve the next page of results.
    next_token: ?[]const u8 = null,

    /// The summary information for the retrieved router network interfaces.
    router_network_interfaces: ?[]const ListedRouterNetworkInterface = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRouterNetworkInterfacesInput, options: CallOptions) !ListRouterNetworkInterfacesOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "mediaconnect");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRouterNetworkInterfacesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRouterNetworkInterfacesOutput {
    var result: ListRouterNetworkInterfacesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRouterNetworkInterfacesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
