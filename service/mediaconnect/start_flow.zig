...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFlowInput, options: Options) !StartFlowOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartFlowInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/flows/start/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartFlowOutput {
    var result: StartFlowOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartFlowOutput, body, alloc);
    }
    _ = status;
    _ = headers;
