...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRouterInputInput, options: Options) !DeleteRouterInputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRouterInputInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRouterInputOutput {
    var result: DeleteRouterInputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteRouterInputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
