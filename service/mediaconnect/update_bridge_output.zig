...
const UpdateBridgeNetworkOutputRequest = @import("update_bridge_network_output_request.zig").UpdateBridgeNetworkOutputRequest;
const BridgeOutput = @import("bridge_output.zig").BridgeOutput;

pub const UpdateBridgeOutputInput = struct {
    /// The Amazon Resource Name (ARN) of the bridge that you want to update.
    bridge_arn: []const u8,

    /// The network of the bridge output.
    network_output: ?UpdateBridgeNetworkOutputRequest = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBridgeOutputInput, options: Options) !UpdateBridgeOutputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBridgeOutputInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/bridges/");
    try path_buf.appendSlice(alloc, input.bridge_arn);
    try path_buf.appendSlice(alloc, "/outputs/");
    try path_buf.appendSlice(alloc, input.output_name);
    ...
    if (input.network_output) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"NetworkOutput\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBridgeOutputOutput {
    var result: UpdateBridgeOutputOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateBridgeOutputOutput, body, alloc);
    }
    _ = status;
    _ = headers;
