...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveFlowVpcInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.vpc_interface_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveFlowVpcInterfaceOutput {
    var result: RemoveFlowVpcInterfaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveFlowVpcInterfaceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
