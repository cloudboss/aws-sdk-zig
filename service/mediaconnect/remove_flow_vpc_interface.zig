...
pub const RemoveFlowVpcInterfaceInput = struct {
...
pub const RemoveFlowVpcInterfaceOutput = struct {
    ...
    non_deleted_network_interface_ids: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveFlowVpcInterfaceInput, options: CallOptions) !RemoveFlowVpcInterfaceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveFlowVpcInterfaceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, input.vpc_interface_name);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveFlowVpcInterfaceOutput {
    var result: RemoveFlowVpcInterfaceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RemoveFlowVpcInterfaceOutput, body, allocator);
    }
    _ = status;
    _ = headers;
