...
pub const DeleteBridgeInput = struct {
...
pub const DeleteBridgeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBridgeInput, options: Options) !DeleteBridgeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBridgeInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBridgeOutput {
    var result: DeleteBridgeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteBridgeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
