...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRouterInputThumbnailInput, options: Options) !GetRouterInputThumbnailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetRouterInputThumbnailInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/thumbnail");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetRouterInputThumbnailOutput {
    var result: GetRouterInputThumbnailOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetRouterInputThumbnailOutput, body, alloc);
    }
    _ = status;
    _ = headers;
