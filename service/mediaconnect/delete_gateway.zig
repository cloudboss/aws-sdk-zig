...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGatewayInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/v1/gateways/");
    try path_buf.appendSlice(alloc, input.gateway_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGatewayOutput {
    var result: DeleteGatewayOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteGatewayOutput, body, alloc);
    }
    _ = status;
    _ = headers;
