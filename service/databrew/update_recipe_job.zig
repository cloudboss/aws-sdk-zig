...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRecipeJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/recipeJobs/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRecipeJobOutput {
    var result: UpdateRecipeJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRecipeJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
