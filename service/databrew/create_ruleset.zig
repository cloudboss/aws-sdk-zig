...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRulesetInput, options: Options) !CreateRulesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Rules\":");
    try aws.json.writeValue(@TypeOf(input.rules), input.rules, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRulesetOutput {
    var result: CreateRulesetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRulesetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
