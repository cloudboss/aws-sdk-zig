...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProfileJobInput, options: Options) !CreateProfileJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProfileJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DatasetName\":");
    try aws.json.writeValue(@TypeOf(input.dataset_name), input.dataset_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateProfileJobOutput {
    var result: CreateProfileJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateProfileJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
