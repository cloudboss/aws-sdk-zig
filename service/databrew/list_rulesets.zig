...
const RulesetItem = @import("ruleset_item.zig").RulesetItem;
...
pub const ListRulesetsInput = struct {
    ...
    /// indicates to return only those rulesets that are associated with the
    /// specified resource.
...
pub const ListRulesetsOutput = struct {
    ...
    rulesets: ?[]const RulesetItem = null,
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRulesetsOutput {
    var result: ListRulesetsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRulesetsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
