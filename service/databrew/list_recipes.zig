...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRecipesInput, options: Options) !ListRecipesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRecipesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRecipesOutput {
    var result: ListRecipesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListRecipesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
