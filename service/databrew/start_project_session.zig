...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartProjectSessionInput, options: Options) !StartProjectSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartProjectSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/startProjectSession");
    ...
    if (input.assume_control) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AssumeControl\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartProjectSessionOutput {
    var result: StartProjectSessionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartProjectSessionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
