...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeScheduleInput, options: Options) !DescribeScheduleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScheduleInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeScheduleOutput {
    var result: DescribeScheduleOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeScheduleOutput, body, alloc);
    }
    _ = status;
    _ = headers;
