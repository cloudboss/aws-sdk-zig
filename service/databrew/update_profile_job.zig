...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProfileJobInput, options: CallOptions) !UpdateProfileJobOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateProfileJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/profileJobs/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateProfileJobOutput {
    var result: UpdateProfileJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProfileJobOutput, body, allocator);
    }
    _ = status;
    _ = headers;
