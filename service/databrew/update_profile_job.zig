...
pub const UpdateProfileJobInput = struct {
    ...
    /// which the
    /// Profile job will be executed. If a JobSample value is not provided for
    /// profile jobs, the
    /// default value will be used. The default value is CUSTOM_ROWS for the mode
    /// parameter and
...
pub const UpdateProfileJobOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProfileJobInput, options: Options) !UpdateProfileJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProfileJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/profileJobs/");
    ...
    if (input.job_sample) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"JobSample\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OutputLocation\":");
    try aws.json.writeValue(@TypeOf(input.output_location), input.output_location, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.validation_configurations) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ValidationConfigurations\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProfileJobOutput {
    var result: UpdateProfileJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateProfileJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
