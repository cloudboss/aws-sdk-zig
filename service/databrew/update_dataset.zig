...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateDatasetInput, options: Options) !UpdateDatasetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateDatasetInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.format_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FormatOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.path_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PathOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateDatasetOutput {
    var result: UpdateDatasetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateDatasetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
