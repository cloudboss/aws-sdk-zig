...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRecipeInput, options: Options) !DescribeRecipeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRecipeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.recipe_version) |v| {
        ...
        try query_buf.appendSlice(alloc, "recipeVersion=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRecipeOutput {
    var result: DescribeRecipeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRecipeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
