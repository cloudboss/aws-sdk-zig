...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendProjectSessionActionInput, options: Options) !SendProjectSessionActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendProjectSessionActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/sendProjectSessionAction");
    ...
    if (input.client_session_id) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ClientSessionId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.preview) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Preview\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.recipe_step) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecipeStep\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.step_index) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"StepIndex\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.view_frame) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ViewFrame\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendProjectSessionActionOutput {
    var result: SendProjectSessionActionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SendProjectSessionActionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
