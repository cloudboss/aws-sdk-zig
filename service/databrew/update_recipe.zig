...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRecipeInput, options: CallOptions) !UpdateRecipeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRecipeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.steps) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"Steps\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
