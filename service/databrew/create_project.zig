...
const CreateProjectInput = @import("create_project_request.zig").CreateProjectRequest;

const CreateProjectOutput = @import("create_project_response.zig").CreateProjectResponse;
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateProjectInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"RecipeName\":");
    try aws.json.writeValue(@TypeOf(input.recipe_name), input.recipe_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.sample) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Sample\":");
