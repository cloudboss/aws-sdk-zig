...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRecipeJobInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.recipe_reference) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"RecipeReference\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRecipeJobOutput {
    var result: CreateRecipeJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRecipeJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
