...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRulesetInput, options: Options) !DescribeRulesetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRulesetInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/rulesets/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRulesetOutput {
    var result: DescribeRulesetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRulesetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
