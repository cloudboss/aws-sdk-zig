...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutResourceSetInput, options: Options) !PutResourceSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutResourceSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFMS_20180101.PutResourceSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutResourceSetOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutResourceSetOutput, body, alloc);
}
