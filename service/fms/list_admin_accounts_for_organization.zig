...
const AdminAccountSummary = @import("admin_account_summary.zig").AdminAccountSummary;

pub const ListAdminAccountsForOrganizationInput = struct {
...
pub const ListAdminAccountsForOrganizationOutput = struct {
    ...
    /// PutAdminAccount.
    admin_accounts: ?[]const AdminAccountSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAdminAccountsForOrganizationInput, options: CallOptions) !ListAdminAccountsForOrganizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAdminAccountsForOrganizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSFMS_20180101.ListAdminAccountsForOrganization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAdminAccountsForOrganizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAdminAccountsForOrganizationOutput, body, allocator);
}
