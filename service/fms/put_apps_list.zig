...
const AppsListData = @import("apps_list_data.zig").AppsListData;
const Tag = @import("tag.zig").Tag;

pub const PutAppsListInput = struct {
...
pub const PutAppsListOutput = struct {
    /// The details of the Firewall Manager applications list.
    apps_list: ?AppsListData = null,

    /// The Amazon Resource Name (ARN) of the applications list.
    apps_list_arn: ?[]const u8 = null,

    pub const json_field_names = .{
        .apps_list = "AppsList",
        .apps_list_arn = "AppsListArn",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutAppsListInput, options: CallOptions) !PutAppsListOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutAppsListInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSFMS_20180101.PutAppsList");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutAppsListOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutAppsListOutput, body, allocator);
}
