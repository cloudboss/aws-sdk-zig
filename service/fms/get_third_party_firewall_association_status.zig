...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetThirdPartyFirewallAssociationStatusInput, options: Options) !GetThirdPartyFirewallAssociationStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetThirdPartyFirewallAssociationStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFMS_20180101.GetThirdPartyFirewallAssociationStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetThirdPartyFirewallAssociationStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetThirdPartyFirewallAssociationStatusOutput, body, alloc);
}
