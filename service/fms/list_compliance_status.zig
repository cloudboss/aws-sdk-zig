...
const PolicyComplianceStatus = @import("policy_compliance_status.zig").PolicyComplianceStatus;

pub const ListComplianceStatusInput = struct {
    ...
    /// objects.
    /// For the second and subsequent `ListComplianceStatus` requests, specify the
    /// value of
...
pub const ListComplianceStatusOutput = struct {
    ...
    policy_compliance_status_list: ?[]const PolicyComplianceStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListComplianceStatusInput, options: CallOptions) !ListComplianceStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListComplianceStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSFMS_20180101.ListComplianceStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListComplianceStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListComplianceStatusOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "InvalidTypeException")) {
        const parsed_error: ?errors.InvalidTypeException = aws.json.parseJsonObject(errors.InvalidTypeException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .invalid_type_exception = typed_error } };
        }
    }
