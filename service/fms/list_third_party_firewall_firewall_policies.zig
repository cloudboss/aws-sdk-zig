...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListThirdPartyFirewallFirewallPoliciesInput, options: CallOptions) !ListThirdPartyFirewallFirewallPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListThirdPartyFirewallFirewallPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSFMS_20180101.ListThirdPartyFirewallFirewallPolicies");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListThirdPartyFirewallFirewallPoliciesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListThirdPartyFirewallFirewallPoliciesOutput, body, allocator);
}
