...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchAssociateResourceInput, options: Options) !BatchAssociateResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchAssociateResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFMS_20180101.BatchAssociateResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchAssociateResourceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchAssociateResourceOutput, body, alloc);
}
