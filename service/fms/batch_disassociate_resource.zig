...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDisassociateResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFMS_20180101.BatchDisassociateResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDisassociateResourceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(BatchDisassociateResourceOutput, body, alloc);
}
