...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAdminAccountInput, options: Options) !GetAdminAccountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAdminAccountInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFMS_20180101.GetAdminAccount");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAdminAccountOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAdminAccountOutput, body, alloc);
}
