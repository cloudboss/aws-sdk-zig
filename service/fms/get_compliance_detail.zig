...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetComplianceDetailInput, options: Options) !GetComplianceDetailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetComplianceDetailInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSFMS_20180101.GetComplianceDetail");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetComplianceDetailOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetComplianceDetailOutput, body, alloc);
}
