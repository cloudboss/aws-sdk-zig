...
pub const DeleteResourceSetInput = struct {
...
pub const DeleteResourceSetOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteResourceSetInput, options: CallOptions) !DeleteResourceSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteResourceSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSFMS_20180101.DeleteResourceSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteResourceSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
