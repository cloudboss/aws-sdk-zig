...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRootFoldersInput, options: Options) !DescribeRootFoldersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRootFoldersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "Authentication", input.authentication_token);

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRootFoldersOutput {
    var result: DescribeRootFoldersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeRootFoldersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
