...
pub const RestoreDocumentVersionsInput = struct {
...
pub const RestoreDocumentVersionsOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RestoreDocumentVersionsInput, options: CallOptions) !RestoreDocumentVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RestoreDocumentVersionsInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("workdocs", "WorkDocs", allocator);

    const ep = try aws.url.parseEndpoint(endpoint);

    var path_buf: std.ArrayList(u8) = .empty;
    try path_buf.appendSlice(allocator, "/api/v1/documentVersions/restore/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RestoreDocumentVersionsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: RestoreDocumentVersionsOutput = .{};
