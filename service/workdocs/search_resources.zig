...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchResourcesInput, options: Options) !SearchResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.additional_response_fields) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"AdditionalResponseFields\":");
    ...
    if (input.order_by) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OrderBy\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.organization_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OrganizationId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.query_scopes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"QueryScopes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchResourcesOutput {
    var result: SearchResourcesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchResourcesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
