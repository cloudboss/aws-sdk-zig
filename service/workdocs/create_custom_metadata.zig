...
pub const CreateCustomMetadataInput = struct {
    /// Amazon WorkDocs authentication token. Not required when using Amazon Web
    /// Services administrator credentials to access the API.
    authentication_token: ?[]const u8 = null,

    /// Custom metadata in the form of name-value pairs.
    custom_metadata: []const aws.map.StringMapEntry,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomMetadataInput, options: Options) !CreateCustomMetadataOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomMetadataInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/customMetadata");
    ...
    try body_buf.appendSlice(alloc, "\"CustomMetadata\":");
    try aws.json.writeValue(@TypeOf(input.custom_metadata), input.custom_metadata, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomMetadataOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateCustomMetadataOutput = .{};
