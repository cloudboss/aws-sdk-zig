...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFolderContentsInput, options: Options) !DescribeFolderContentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFolderContentsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFolderContentsOutput {
    var result: DescribeFolderContentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeFolderContentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
