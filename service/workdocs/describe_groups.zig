...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGroupsInput, options: Options) !DescribeGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "searchQuery=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.search_query);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGroupsOutput {
    var result: DescribeGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
