...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDocumentInput, options: Options) !GetDocumentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDocumentInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDocumentOutput {
    var result: GetDocumentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDocumentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
