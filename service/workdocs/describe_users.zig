...
const UserFilterType = @import("user_filter_type.zig").UserFilterType;
const OrderType = @import("order_type.zig").OrderType;
const UserSortType = @import("user_sort_type.zig").UserSortType;
...
pub const DescribeUsersInput = struct {
    ...
    ///   `SurName`, or `UserName` fields included in a
    /// [CreateUser](https://docs.aws.amazon.com/workdocs/latest/APIReference/API_CreateUser.html) API call. For example, querying on
    /// `Ma` returns Márcia Oliveira, María García, and Mateo Jackson. If you use
    /// multiple characters, the API only returns data that matches all characters.
    /// For example, querying on `Ma J` only
...
pub const DescribeUsersOutput = struct {
    ...
    total_number_of_users: ?i64 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeUsersInput, options: Options) !DescribeUsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeUsersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.include) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "include=");
    ...
    if (input.order) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "order=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    ...
    if (input.query) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "query=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.sort) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sort=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.user_ids) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeUsersOutput {
    var result: DescribeUsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeUsersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
