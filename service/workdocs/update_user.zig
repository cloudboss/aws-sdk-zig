...
const BooleanEnumType = @import("boolean_enum_type.zig").BooleanEnumType;
...
pub const UpdateUserInput = struct {
    ...
    /// privileges.
    grant_poweruser_privileges: ?BooleanEnumType = null,
    ...
    pub const json_field_names = .{
        ...
        .grant_poweruser_privileges = "GrantPoweruserPrivileges",
        .locale = "Locale",
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/api/v1/users/");
    ...
    if (input.given_name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"GivenName\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.grant_poweruser_privileges) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"GrantPoweruserPrivileges\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.locale) |v| {
    ...
    if (input.storage_rule) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"StorageRule\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.surname) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"Surname\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.time_zone_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TimeZoneId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.type) |v| {
