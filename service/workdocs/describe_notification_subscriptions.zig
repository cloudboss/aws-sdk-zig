...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeNotificationSubscriptionsInput, options: Options) !DescribeNotificationSubscriptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeNotificationSubscriptionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeNotificationSubscriptionsOutput {
    var result: DescribeNotificationSubscriptionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeNotificationSubscriptionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
