...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeCommentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/comments");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeCommentsOutput {
    var result: DescribeCommentsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeCommentsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
