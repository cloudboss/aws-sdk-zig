...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCommentInput, options: Options) !CreateCommentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCommentInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/comment");
    ...
    if (input.notify_collaborators) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"NotifyCollaborators\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.parent_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ParentId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    if (input.thread_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ThreadId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCommentOutput {
    var result: CreateCommentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateCommentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
