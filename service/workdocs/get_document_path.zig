...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDocumentPathInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDocumentPathOutput {
    var result: GetDocumentPathOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDocumentPathOutput, body, alloc);
    }
    _ = status;
    _ = headers;
