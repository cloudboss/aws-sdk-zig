...
const NotificationOptions = @import("notification_options.zig").NotificationOptions;
const SharePrincipal = @import("share_principal.zig").SharePrincipal;
const ShareResult = @import("share_result.zig").ShareResult;

pub const AddResourcePermissionsInput = struct {
    /// Amazon WorkDocs authentication token. Not required when using Amazon Web
    /// Services administrator credentials to access the API.
    authentication_token: ?[]const u8 = null,

    /// The notification options.
    notification_options: ?NotificationOptions = null,

    /// The users, groups, or organization being granted permission.
    principals: []const SharePrincipal,
...
pub const AddResourcePermissionsOutput = struct {
    /// The share results.
    share_results: ?[]const ShareResult = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddResourcePermissionsInput, options: Options) !AddResourcePermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AddResourcePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/api/v1/resources/");
    try path_buf.appendSlice(alloc, input.resource_id);
    try path_buf.appendSlice(alloc, "/permissions");
    ...
    if (input.notification_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"NotificationOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Principals\":");
    try aws.json.writeValue(@TypeOf(input.principals), input.principals, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AddResourcePermissionsOutput {
    var result: AddResourcePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(AddResourcePermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
