...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddResourcePermissionsInput, options: CallOptions) !AddResourcePermissionsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "workdocs");
...
fn serializeRequest(allocator: std.mem.Allocator, input: AddResourcePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.notification_options) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"NotificationOptions\":");
