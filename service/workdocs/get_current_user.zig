...
pub const GetCurrentUserInput = struct {
...
pub const GetCurrentUserOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCurrentUserInput, options: CallOptions) !GetCurrentUserOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetCurrentUserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetCurrentUserOutput {
    var result: GetCurrentUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCurrentUserOutput, body, allocator);
    }
    _ = status;
    _ = headers;
