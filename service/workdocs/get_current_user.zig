...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCurrentUserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCurrentUserOutput {
    var result: GetCurrentUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetCurrentUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
