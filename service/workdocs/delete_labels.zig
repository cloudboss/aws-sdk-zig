...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLabelsInput, options: Options) !DeleteLabelsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteLabelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/labels");
    ...
    if (input.delete_all) |v| {
        ...
        try query_buf.appendSlice(alloc, "deleteAll=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.labels) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "labels=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteLabelsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
