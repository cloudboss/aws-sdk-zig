...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ActivateUserInput, options: Options) !ActivateUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ActivateUserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ActivateUserOutput {
    var result: ActivateUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ActivateUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
