...
pub const CreateLabelsInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLabelsInput, options: Options) !CreateLabelsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLabelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.labels), input.labels, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLabelsOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: CreateLabelsOutput = .{};
