...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateLabelsInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.labels), input.labels, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateLabelsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
