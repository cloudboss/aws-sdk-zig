...
fn serializeRequest(alloc: std.mem.Allocator, input: InitiateDocumentVersionUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.content_created_timestamp) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"ContentCreatedTimestamp\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.content_modified_timestamp) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ContentModifiedTimestamp\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.document_size_in_bytes) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"DocumentSizeInBytes\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.id) |v| {
    ...
    if (input.parent_folder_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ParentFolderId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !InitiateDocumentVersionUploadOutput {
    var result: InitiateDocumentVersionUploadOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(InitiateDocumentVersionUploadOutput, body, alloc);
    }
    _ = status;
    _ = headers;
