...
const ResourcePath = @import("resource_path.zig").ResourcePath;

pub const GetFolderPathInput = struct {
...
pub const GetFolderPathOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFolderPathInput, options: CallOptions) !GetFolderPathOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFolderPathInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFolderPathOutput {
    var result: GetFolderPathOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFolderPathOutput, body, allocator);
    }
    _ = status;
    _ = headers;
