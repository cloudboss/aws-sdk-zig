...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFolderPathInput, options: Options) !GetFolderPathOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFolderPathInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/api/v1/folders/");
    try path_buf.appendSlice(alloc, input.folder_id);
    try path_buf.appendSlice(alloc, "/path");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFolderPathOutput {
    var result: GetFolderPathOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFolderPathOutput, body, alloc);
    }
    _ = status;
    _ = headers;
