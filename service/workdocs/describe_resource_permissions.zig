...
pub const DescribeResourcePermissionsInput = struct {
...
pub const DescribeResourcePermissionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeResourcePermissionsInput, options: Options) !DescribeResourcePermissionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeResourcePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.principal_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "principalId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeResourcePermissionsOutput {
    var result: DescribeResourcePermissionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeResourcePermissionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
