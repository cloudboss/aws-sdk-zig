...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeResourcePermissionsInput, options: CallOptions) !DescribeResourcePermissionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeResourcePermissionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.principal_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "principalId=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
