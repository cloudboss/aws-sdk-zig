...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDocumentVersionsInput, options: Options) !DescribeDocumentVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDocumentVersionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDocumentVersionsOutput {
    var result: DescribeDocumentVersionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeDocumentVersionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
