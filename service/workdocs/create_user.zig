...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.given_name), input.given_name, alloc, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.password), input.password, alloc, &body_buf);
    has_prev = true;
    ...
    try aws.json.writeValue(@TypeOf(input.surname), input.surname, alloc, &body_buf);
    has_prev = true;
