...
fn serializeRequest(alloc: std.mem.Allocator, input: PutVectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"vectors\":");
    try aws.json.writeValue(@TypeOf(input.vectors), input.vectors, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutVectorsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
