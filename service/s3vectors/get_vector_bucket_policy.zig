...
pub const GetVectorBucketPolicyInput = struct {
...
pub const GetVectorBucketPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVectorBucketPolicyInput, options: Options) !GetVectorBucketPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVectorBucketPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/GetVectorBucketPolicy";
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVectorBucketPolicyOutput {
    var result: GetVectorBucketPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVectorBucketPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
