...
const VectorBucket = @import("vector_bucket.zig").VectorBucket;

pub const GetVectorBucketInput = struct {
...
pub const GetVectorBucketOutput = struct {
    /// The attributes of the vector bucket.
    vector_bucket: ?VectorBucket = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetVectorBucketInput, options: Options) !GetVectorBucketOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVectorBucketInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.vector_bucket_arn) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"vectorBucketArn\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVectorBucketOutput {
    var result: GetVectorBucketOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVectorBucketOutput, body, alloc);
    }
    _ = status;
    _ = headers;
