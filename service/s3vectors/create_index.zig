...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIndexInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"dataType\":");
    try aws.json.writeValue(@TypeOf(input.data_type), input.data_type, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"dimension\":");
    try aws.json.writeValue(@TypeOf(input.dimension), input.dimension, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"distanceMetric\":");
    try aws.json.writeValue(@TypeOf(input.distance_metric), input.distance_metric, alloc, &body_buf);
    has_prev = true;
