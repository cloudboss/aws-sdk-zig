...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVectorsInput, options: CallOptions) !ListVectorsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.segment_count) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"segmentCount\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    if (input.segment_index) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"segmentIndex\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
