...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVectorBucketsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVectorBucketsOutput {
    var result: ListVectorBucketsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVectorBucketsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
