...
const VectorBucketSummary = @import("vector_bucket_summary.zig").VectorBucketSummary;

pub const ListVectorBucketsInput = struct {
...
pub const ListVectorBucketsOutput = struct {
    ...
    vector_buckets: ?[]const VectorBucketSummary = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .vector_buckets = "vectorBuckets",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListVectorBucketsInput, options: CallOptions) !ListVectorBucketsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "s3vectors");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVectorBucketsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVectorBucketsOutput {
    var result: ListVectorBucketsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListVectorBucketsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
