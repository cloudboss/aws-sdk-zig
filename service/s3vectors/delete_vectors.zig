...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVectorsInput, options: Options) !DeleteVectorsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVectorsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"keys\":");
    try aws.json.writeValue(@TypeOf(input.keys), input.keys, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVectorsOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
