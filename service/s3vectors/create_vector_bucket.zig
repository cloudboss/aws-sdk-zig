...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVectorBucketInput, options: Options) !CreateVectorBucketOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateVectorBucketInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.vector_bucket_name), input.vector_bucket_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateVectorBucketOutput {
    var result: CreateVectorBucketOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateVectorBucketOutput, body, alloc);
    }
    _ = status;
    _ = headers;
