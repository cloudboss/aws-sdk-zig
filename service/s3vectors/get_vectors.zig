...
fn serializeRequest(alloc: std.mem.Allocator, input: GetVectorsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetVectorsOutput {
    var result: GetVectorsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetVectorsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
