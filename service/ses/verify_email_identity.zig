...
pub const VerifyEmailIdentityInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VerifyEmailIdentityInput, options: Options) !VerifyEmailIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyEmailIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=VerifyEmailIdentity&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&EmailAddress=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.email_address);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyEmailIdentityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: VerifyEmailIdentityOutput = .{};
