...
pub const ReorderReceiptRuleSetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReorderReceiptRuleSetInput, options: Options) !ReorderReceiptRuleSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ReorderReceiptRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ReorderReceiptRuleSet&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ReorderReceiptRuleSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: ReorderReceiptRuleSetOutput = .{};
