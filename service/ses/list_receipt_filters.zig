...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReceiptFiltersInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=ListReceiptFilters&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReceiptFiltersOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Filters")) {
                    result.filters = try serde.deserializeReceiptFilterList(&reader, alloc, "member");
                } else {
