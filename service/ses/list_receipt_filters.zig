...
pub const ListReceiptFiltersOutput = struct {
    ...
    filters: ?[]const ReceiptFilter = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListReceiptFiltersInput, options: CallOptions) !ListReceiptFiltersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListReceiptFiltersInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=ListReceiptFilters&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListReceiptFiltersOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListReceiptFiltersResult")) break;
            },
            else => {},
        }
    }

    var result: ListReceiptFiltersOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Filters")) {
                    result.filters = try serde.deserializeReceiptFilterList(allocator, &reader, "member");
                } else {
