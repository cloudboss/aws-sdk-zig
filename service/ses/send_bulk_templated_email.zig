...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendBulkTemplatedEmailInput, options: CallOptions) !SendBulkTemplatedEmailOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SendBulkTemplatedEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=SendBulkTemplatedEmail&Version=2010-12-01");
    ...
    try body_buf.appendSlice(allocator, "&DefaultTemplateData=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.default_template_data);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SendBulkTemplatedEmailOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Status")) {
                    result.status = try serde.deserializeBulkEmailDestinationStatusList(allocator, &reader, "member");
                } else {
