...
pub const SendBulkTemplatedEmailInput = struct {
    ...
    default_tags: ?[]const MessageTag = null,

    /// A list of replacement values to apply to the template when replacement data
    /// is not
    /// specified in a Destination object. These values act as a default or fallback
    /// option when
    ...
    default_template_data: []const u8,
    ...
    destinations: []const BulkEmailDestination,
...
pub const SendBulkTemplatedEmailOutput = struct {
    /// One object per intended recipient. Check each response object and retry any
    /// messages
    /// with a failure status. (Note that order of responses will be respective to
    /// order of
    /// destinations in the request.)Receipt rules enable you to specify which
    /// actions
    status: ?[]const BulkEmailDestinationStatus = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendBulkTemplatedEmailInput, options: Options) !SendBulkTemplatedEmailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendBulkTemplatedEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SendBulkTemplatedEmail&Version=2010-12-01");
    if (input.configuration_set_name) |v| {
        try body_buf.appendSlice(alloc, "&ConfigurationSetName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.default_tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
                var prefix_buf: [256]u8 = undefined;
                const field_prefix = std.fmt.bufPrint(&prefix_buf, "&DefaultTags.member.{d}.Name=", .{n}) catch continue;
                try body_buf.appendSlice(alloc, field_prefix);
                try aws.url.appendUrlEncoded(alloc, &body_buf, item.name);
            }
            {
                var prefix_buf: [256]u8 = undefined;
                const field_prefix = std.fmt.bufPrint(&prefix_buf, "&DefaultTags.member.{d}.Value=", .{n}) catch continue;
                try body_buf.appendSlice(alloc, field_prefix);
                try aws.url.appendUrlEncoded(alloc, &body_buf, item.value);
            }
        }
    }
    try body_buf.appendSlice(alloc, "&DefaultTemplateData=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.default_template_data);
    for (input.destinations, 0..) |item, idx| {
        const n = idx + 1;
        if (item.destination.bcc_addresses) |lst_2| {
        ...
        if (item.destination.cc_addresses) |lst_2| {
            for (lst_2, 0..) |item_2, idx_2| {
                const n_2 = idx_2 + 1;
                {
                    var prefix_buf: [256]u8 = undefined;
        ...
        if (item.destination.to_addresses) |lst_2| {
            for (lst_2, 0..) |item_2, idx_2| {
                const n_2 = idx_2 + 1;
                {
                    var prefix_buf: [256]u8 = undefined;
        ...
        if (item.replacement_tags) |lst_1| {
        ...
        {
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&Destinations.member.{d}.ReplacementTemplateData=", .{n}) catch continue;
            try body_buf.appendSlice(alloc, field_prefix);
            if (item.replacement_template_data) |fv_1| {
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.template);
    if (input.template_arn) |v| {
        try body_buf.appendSlice(alloc, "&TemplateArn=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendBulkTemplatedEmailOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SendBulkTemplatedEmailResult")) break;
            },
            else => {},
        }
    }

    var result: SendBulkTemplatedEmailOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Status")) {
                    result.status = try serde.deserializeBulkEmailDestinationStatusList(&reader, alloc, "member");
                } else {
