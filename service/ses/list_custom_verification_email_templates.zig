...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCustomVerificationEmailTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListCustomVerificationEmailTemplates&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCustomVerificationEmailTemplatesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CustomVerificationEmailTemplates")) {
                    result.custom_verification_email_templates = try serde.deserializeCustomVerificationEmailTemplates(&reader, alloc, "member");
