...
pub const ListCustomVerificationEmailTemplatesInput = struct {
    /// The maximum number of custom verification email templates to return. This
    /// value must
    /// be at least 1 and less than or equal to 50. If you do not specify a value,
    /// or if you
    /// specify a value less than 1 or greater than 50, the operation returns up to
    /// 50
    /// results.
...
pub const ListCustomVerificationEmailTemplatesOutput = struct {
    /// A list of the custom verification email templates that exist in your
    /// account.
    custom_verification_email_templates: ?[]const CustomVerificationEmailTemplate = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListCustomVerificationEmailTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListCustomVerificationEmailTemplates&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListCustomVerificationEmailTemplatesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListCustomVerificationEmailTemplatesResult")) break;
            },
            else => {},
        }
    }

    var result: ListCustomVerificationEmailTemplatesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CustomVerificationEmailTemplates")) {
                    result.custom_verification_email_templates = try serde.deserializeCustomVerificationEmailTemplates(allocator, &reader, "member");
