...
pub const ListConfigurationSetsOutput = struct {
    ...
    configuration_sets: ?[]const ConfigurationSet = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListConfigurationSetsInput, options: Options) !ListConfigurationSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListConfigurationSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListConfigurationSets&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListConfigurationSetsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListConfigurationSetsResult")) break;
            },
            else => {},
        }
    }
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ConfigurationSets")) {
                    result.configuration_sets = try serde.deserializeConfigurationSets(&reader, alloc, "member");
