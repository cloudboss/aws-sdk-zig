...
pub const ListIdentitiesOutput = struct {
    ...
    identities: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIdentitiesInput, options: Options) !ListIdentitiesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdentitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListIdentities&Version=2010-12-01");
    if (input.identity_type) |v| {
        try body_buf.appendSlice(alloc, "&IdentityType=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdentitiesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListIdentitiesResult")) break;
            },
            else => {},
        }
    }

    var result: ListIdentitiesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Identities")) {
                    result.identities = try serde.deserializeIdentityList(&reader, alloc, "member");
