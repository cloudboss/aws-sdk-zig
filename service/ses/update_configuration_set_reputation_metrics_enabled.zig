...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateConfigurationSetReputationMetricsEnabledInput, options: Options) !UpdateConfigurationSetReputationMetricsEnabledOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfigurationSetReputationMetricsEnabledInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateConfigurationSetReputationMetricsEnabled&Version=2010-12-01");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, if (input.enabled) "true" else "false");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfigurationSetReputationMetricsEnabledOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
