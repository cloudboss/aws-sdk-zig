...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSendQuotaInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=GetSendQuota&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSendQuotaOutput {
    _ = status;
    _ = headers;
    _ = alloc;
