...
const CreateCustomVerificationEmailTemplateOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateCustomVerificationEmailTemplateInput, options: Options) !CreateCustomVerificationEmailTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateCustomVerificationEmailTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateCustomVerificationEmailTemplate&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&FailureRedirectionURL=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.failure_redirection_url);
    try body_buf.appendSlice(alloc, "&FromEmailAddress=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.from_email_address);
    try body_buf.appendSlice(alloc, "&SuccessRedirectionURL=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.success_redirection_url);
    try body_buf.appendSlice(alloc, "&TemplateContent=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.template_content);
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.template_subject);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateCustomVerificationEmailTemplateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
