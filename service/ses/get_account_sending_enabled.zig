...
fn serializeRequest(allocator: std.mem.Allocator, input: GetAccountSendingEnabledInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=GetAccountSendingEnabled&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetAccountSendingEnabledOutput {
    _ = status;
    _ = headers;
    _ = allocator;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetAccountSendingEnabledResult")) break;
            },
            else => {},
        }
    }

    var result: GetAccountSendingEnabledOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
