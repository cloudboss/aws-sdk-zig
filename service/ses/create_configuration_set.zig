...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateConfigurationSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateConfigurationSet&Version=2010-12-01");
    try body_buf.appendSlice(allocator, "&ConfigurationSet.Name=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.configuration_set.name);
