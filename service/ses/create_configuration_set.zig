...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateConfigurationSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateConfigurationSet&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&ConfigurationSet.Name=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.configuration_set.name);
