...
pub const GetIdentityMailFromDomainAttributesInput = struct {
    /// A list of one or more identities.
    identities: []const []const u8,
};

pub const GetIdentityMailFromDomainAttributesOutput = struct {
    /// A map of identities to custom MAIL FROM attributes.
    mail_from_domain_attributes: ?[]const aws.map.MapEntry(IdentityMailFromDomainAttributes) = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentityMailFromDomainAttributesInput, options: CallOptions) !GetIdentityMailFromDomainAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIdentityMailFromDomainAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetIdentityMailFromDomainAttributes&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIdentityMailFromDomainAttributesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetIdentityMailFromDomainAttributesResult")) break;
            },
            else => {},
        }
    }

    var result: GetIdentityMailFromDomainAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "MailFromDomainAttributes")) {
                    result.mail_from_domain_attributes = try serde.deserializeMailFromDomainAttributes(allocator, &reader, "entry");
                } else {
