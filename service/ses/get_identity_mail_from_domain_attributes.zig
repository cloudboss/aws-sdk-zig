...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityMailFromDomainAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetIdentityMailFromDomainAttributes&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityMailFromDomainAttributesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "MailFromDomainAttributes")) {
                    result.mail_from_domain_attributes = try serde.deserializeMailFromDomainAttributes(&reader, alloc, "entry");
                } else {
