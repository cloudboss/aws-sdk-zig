...
fn serializeRequest(alloc: std.mem.Allocator, input: CloneReceiptRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CloneReceiptRuleSet&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&OriginalRuleSetName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.original_rule_set_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CloneReceiptRuleSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
