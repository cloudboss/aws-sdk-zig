...
pub const CloneReceiptRuleSetInput = struct {
    /// The name of the rule set to clone.
    original_rule_set_name: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CloneReceiptRuleSetInput, options: Options) !CloneReceiptRuleSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CloneReceiptRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CloneReceiptRuleSet&Version=2010-12-01");
    try body_buf.appendSlice(allocator, "&OriginalRuleSetName=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.original_rule_set_name);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CloneReceiptRuleSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: CloneReceiptRuleSetOutput = .{};
