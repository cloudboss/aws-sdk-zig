...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VerifyDomainIdentityInput, options: Options) !VerifyDomainIdentityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyDomainIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=VerifyDomainIdentity&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&Domain=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.domain);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyDomainIdentityOutput {
    _ = status;
    _ = headers;
