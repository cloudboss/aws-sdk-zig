...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetCustomVerificationEmailTemplateInput, options: Options) !GetCustomVerificationEmailTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetCustomVerificationEmailTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetCustomVerificationEmailTemplate&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetCustomVerificationEmailTemplateOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "FailureRedirectionURL")) {
                    ...
                    result.template_content = try alloc.dupe(u8, try reader.readElementText());
