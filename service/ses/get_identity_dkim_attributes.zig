...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityDkimAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetIdentityDkimAttributes&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityDkimAttributesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DkimAttributes")) {
                    result.dkim_attributes = try serde.deserializeDkimAttributes(&reader, alloc, "entry");
                } else {
