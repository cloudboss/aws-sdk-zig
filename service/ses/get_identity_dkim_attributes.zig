...
pub const GetIdentityDkimAttributesInput = struct {
    /// A list of one or more verified identities - email addresses, domains, or
    /// both.
    identities: []const []const u8,
};

pub const GetIdentityDkimAttributesOutput = struct {
    /// The DKIM attributes for an email address or a domain.
    dkim_attributes: ?[]const aws.map.MapEntry(IdentityDkimAttributes) = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentityDkimAttributesInput, options: CallOptions) !GetIdentityDkimAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIdentityDkimAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetIdentityDkimAttributes&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIdentityDkimAttributesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetIdentityDkimAttributesResult")) break;
            },
            else => {},
        }
    }

    var result: GetIdentityDkimAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DkimAttributes")) {
                    result.dkim_attributes = try serde.deserializeDkimAttributes(allocator, &reader, "entry");
                } else {
