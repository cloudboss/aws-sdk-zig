...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=UpdateTemplate&Version=2010-12-01");
    if (input.template.html_part) |sv| {
        try body_buf.appendSlice(allocator, "&Template.HtmlPart=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, sv);
    }
    if (input.template.subject_part) |sv| {
        try body_buf.appendSlice(allocator, "&Template.SubjectPart=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, sv);
    }
    try body_buf.appendSlice(allocator, "&Template.TemplateName=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.template.template_name);
    if (input.template.text_part) |sv| {
        try body_buf.appendSlice(allocator, "&Template.TextPart=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, sv);
    }
