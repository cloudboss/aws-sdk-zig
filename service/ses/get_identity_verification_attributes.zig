...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentityVerificationAttributesInput, options: Options) !GetIdentityVerificationAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityVerificationAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetIdentityVerificationAttributes&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityVerificationAttributesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "VerificationAttributes")) {
                    result.verification_attributes = try serde.deserializeVerificationAttributes(&reader, alloc, "entry");
                } else {
