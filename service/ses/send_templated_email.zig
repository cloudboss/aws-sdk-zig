...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendTemplatedEmailInput, options: Options) !SendTemplatedEmailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendTemplatedEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SendTemplatedEmail&Version=2010-12-01");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.template_data);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendTemplatedEmailOutput {
    _ = status;
    _ = headers;
