...
pub const CreateReceiptRuleSetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateReceiptRuleSetInput, options: Options) !CreateReceiptRuleSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateReceiptRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateReceiptRuleSet&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateReceiptRuleSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: CreateReceiptRuleSetOutput = .{};
