...
fn serializeRequest(alloc: std.mem.Allocator, input: ListReceiptRuleSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListReceiptRuleSets&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListReceiptRuleSetsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NextToken")) {
                    ...
                    result.rule_sets = try serde.deserializeReceiptRuleSetsLists(&reader, alloc, "member");
                } else {
