...
fn serializeRequest(alloc: std.mem.Allocator, input: ListVerifiedEmailAddressesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=ListVerifiedEmailAddresses&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListVerifiedEmailAddressesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "VerifiedEmailAddresses")) {
                    result.verified_email_addresses = try serde.deserializeAddressList(&reader, alloc, "member");
                } else {
