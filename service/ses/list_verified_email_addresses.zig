...
fn serializeRequest(allocator: std.mem.Allocator, input: ListVerifiedEmailAddressesInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=ListVerifiedEmailAddresses&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListVerifiedEmailAddressesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListVerifiedEmailAddressesResult")) break;
            },
            else => {},
        }
    }

    var result: ListVerifiedEmailAddressesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "VerifiedEmailAddresses")) {
                    result.verified_email_addresses = try serde.deserializeAddressList(allocator, &reader, "member");
                } else {
