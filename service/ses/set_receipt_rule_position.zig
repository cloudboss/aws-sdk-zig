...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetReceiptRulePositionInput, options: Options) !SetReceiptRulePositionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetReceiptRulePositionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetReceiptRulePosition&Version=2010-12-01");
    if (input.after) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetReceiptRulePositionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
