...
pub const DescribeReceiptRuleInput = struct {
...
pub const DescribeReceiptRuleOutput = struct {
    /// A data structure that contains the specified receipt rule's name, actions,
    /// recipients,
    /// domains, enabled status, scan status, and Transport Layer Security (TLS)
    /// policy.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReceiptRuleInput, options: Options) !DescribeReceiptRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReceiptRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeReceiptRule&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&RuleName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.rule_name);
    try body_buf.appendSlice(alloc, "&RuleSetName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.rule_set_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReceiptRuleOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeReceiptRuleResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeReceiptRuleOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Rule")) {
                    result.rule = try serde.deserializeReceiptRule(&reader, alloc);
                } else {
