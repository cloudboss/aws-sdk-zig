...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReceiptRuleInput, options: CallOptions) !DescribeReceiptRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeReceiptRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeReceiptRule&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeReceiptRuleOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Rule")) {
                    result.rule = try serde.deserializeReceiptRule(allocator, &reader);
                } else {
