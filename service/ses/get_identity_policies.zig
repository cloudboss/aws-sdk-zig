...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentityPoliciesInput, options: Options) !GetIdentityPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetIdentityPolicies&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityPoliciesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Policies")) {
                    result.policies = try serde.deserializePolicyMap(&reader, alloc, "entry");
                } else {
