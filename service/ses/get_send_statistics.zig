...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSendStatisticsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(allocator, "Action=GetSendStatistics&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSendStatisticsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetSendStatisticsResult")) break;
            },
            else => {},
        }
    }

    var result: GetSendStatisticsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SendDataPoints")) {
                    result.send_data_points = try serde.deserializeSendDataPointList(allocator, &reader, "member");
                } else {
