...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSendStatisticsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=GetSendStatistics&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSendStatisticsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SendDataPoints")) {
                    result.send_data_points = try serde.deserializeSendDataPointList(&reader, alloc, "member");
                } else {
