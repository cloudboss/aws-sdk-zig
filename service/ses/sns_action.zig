...
/// (Amazon SNS). This action includes a complete copy of the email content in
/// the Amazon SNS
/// notifications. Amazon SNS notifications for all other actions simply provide
/// information
...
/// Amazon SES permission to
/// publish emails to it. However, if you don't own the Amazon SNS topic, you
/// need to attach a
/// policy to the topic to give Amazon SES permissions to access it. For
...
pub const SNSAction = struct {
    ...
    /// is easier to
    /// use, but may not preserve all special characters when a message was encoded
    /// with a
