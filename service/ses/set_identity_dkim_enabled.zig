...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetIdentityDkimEnabledInput, options: Options) !SetIdentityDkimEnabledOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetIdentityDkimEnabledInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetIdentityDkimEnabled&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&DkimEnabled=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, if (input.dkim_enabled) "true" else "false");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetIdentityDkimEnabledOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
