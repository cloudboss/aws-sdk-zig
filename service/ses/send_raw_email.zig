...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SendRawEmailInput, options: Options) !SendRawEmailOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SendRawEmailInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SendRawEmail&Version=2010-12-01");
    ...
    if (input.from_arn) |v| {
        try body_buf.appendSlice(alloc, "&FromArn=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&RawMessage.Data=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.raw_message.data);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SendRawEmailOutput {
    _ = status;
    _ = headers;
