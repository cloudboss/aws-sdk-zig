...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateTemplate&Version=2010-12-01");
    if (input.template.html_part) |sv| {
        try body_buf.appendSlice(alloc, "&Template.HtmlPart=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
    }
    if (input.template.subject_part) |sv| {
        try body_buf.appendSlice(alloc, "&Template.SubjectPart=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
    }
    try body_buf.appendSlice(alloc, "&Template.TemplateName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.template.template_name);
    if (input.template.text_part) |sv| {
        try body_buf.appendSlice(alloc, "&Template.TextPart=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, sv);
    }
