...
pub const DeleteIdentityInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteIdentityInput, options: Options) !DeleteIdentityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteIdentityInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteIdentity&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteIdentityOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteIdentityOutput = .{};
