...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: VerifyDomainDkimInput, options: Options) !VerifyDomainDkimOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyDomainDkimInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=VerifyDomainDkim&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyDomainDkimOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DkimTokens")) {
                    result.dkim_tokens = try serde.deserializeVerificationTokenList(&reader, alloc, "member");
                } else {
