...
pub const SetActiveReceiptRuleSetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetActiveReceiptRuleSetInput, options: Options) !SetActiveReceiptRuleSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetActiveReceiptRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetActiveReceiptRuleSet&Version=2010-12-01");
    if (input.rule_set_name) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetActiveReceiptRuleSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: SetActiveReceiptRuleSetOutput = .{};
