...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SetIdentityFeedbackForwardingEnabledInput, options: Options) !SetIdentityFeedbackForwardingEnabledOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SetIdentityFeedbackForwardingEnabledInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetIdentityFeedbackForwardingEnabled&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&ForwardingEnabled=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, if (input.forwarding_enabled) "true" else "false");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetIdentityFeedbackForwardingEnabledOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
