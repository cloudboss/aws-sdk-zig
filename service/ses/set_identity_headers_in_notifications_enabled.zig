...
fn serializeRequest(alloc: std.mem.Allocator, input: SetIdentityHeadersInNotificationsEnabledInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SetIdentityHeadersInNotificationsEnabled&Version=2010-12-01");
    ...
    try body_buf.appendSlice(alloc, "&NotificationType=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.notification_type));
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SetIdentityHeadersInNotificationsEnabledOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
