...
fn serializeRequest(alloc: std.mem.Allocator, input: GetIdentityNotificationAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=GetIdentityNotificationAttributes&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetIdentityNotificationAttributesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NotificationAttributes")) {
                    result.notification_attributes = try serde.deserializeNotificationAttributes(&reader, alloc, "entry");
                } else {
