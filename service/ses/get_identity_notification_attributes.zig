...
pub const GetIdentityNotificationAttributesInput = struct {
...
pub const GetIdentityNotificationAttributesOutput = struct {
    /// A map of Identity to IdentityNotificationAttributes.
    notification_attributes: ?[]const aws.map.MapEntry(IdentityNotificationAttributes) = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetIdentityNotificationAttributesInput, options: CallOptions) !GetIdentityNotificationAttributesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetIdentityNotificationAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=GetIdentityNotificationAttributes&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetIdentityNotificationAttributesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "GetIdentityNotificationAttributesResult")) break;
            },
            else => {},
        }
    }

    var result: GetIdentityNotificationAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "NotificationAttributes")) {
                    result.notification_attributes = try serde.deserializeNotificationAttributes(allocator, &reader, "entry");
                } else {
