...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateConfigurationSetTrackingOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=UpdateConfigurationSetTrackingOptions&Version=2010-12-01");
    ...
    if (input.tracking_options.custom_redirect_domain) |sv| {
        try body_buf.appendSlice(alloc, "&TrackingOptions.CustomRedirectDomain=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateConfigurationSetTrackingOptionsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
