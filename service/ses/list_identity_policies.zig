...
pub const ListIdentityPoliciesInput = struct {
...
pub const ListIdentityPoliciesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIdentityPoliciesInput, options: Options) !ListIdentityPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListIdentityPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ListIdentityPolicies&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&Identity=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.identity);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListIdentityPoliciesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ListIdentityPoliciesResult")) break;
            },
            else => {},
        }
    }

    var result: ListIdentityPoliciesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PolicyNames")) {
                    result.policy_names = try serde.deserializePolicyNameList(&reader, alloc, "member");
                } else {
