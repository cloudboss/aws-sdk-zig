...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListIdentityPoliciesInput, options: CallOptions) !ListIdentityPoliciesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListIdentityPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=ListIdentityPolicies&Version=2010-12-01");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListIdentityPoliciesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PolicyNames")) {
                    result.policy_names = try serde.deserializePolicyNameList(allocator, &reader, "member");
                } else {
