...
pub const DeleteReceiptFilterInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteReceiptFilterInput, options: Options) !DeleteReceiptFilterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteReceiptFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteReceiptFilter&Version=2010-12-01");
    try body_buf.appendSlice(allocator, "&FilterName=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.filter_name);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteReceiptFilterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeleteReceiptFilterOutput = .{};
