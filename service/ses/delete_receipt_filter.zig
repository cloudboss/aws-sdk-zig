...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteReceiptFilterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteReceiptFilter&Version=2010-12-01");
    try body_buf.appendSlice(alloc, "&FilterName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.filter_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteReceiptFilterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
