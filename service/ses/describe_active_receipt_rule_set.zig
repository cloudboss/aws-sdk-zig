...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeActiveReceiptRuleSetInput, options: Options) !DescribeActiveReceiptRuleSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeActiveReceiptRuleSetInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeActiveReceiptRuleSet&Version=2010-12-01");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeActiveReceiptRuleSetOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Metadata")) {
                    result.metadata = try serde.deserializeReceiptRuleSetMetadata(&reader, alloc);
                } else if (std.mem.eql(u8, e.local, "Rules")) {
                    result.rules = try serde.deserializeReceiptRulesList(&reader, alloc, "member");
                } else {
