...
fn serializeRequest(alloc: std.mem.Allocator, input: RotateIngestEndpointCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ingest_endpoints/");
    try path_buf.appendSlice(alloc, input.ingest_endpoint_id);
    try path_buf.appendSlice(alloc, "/credentials");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RotateIngestEndpointCredentialsOutput {
    var result: RotateIngestEndpointCredentialsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RotateIngestEndpointCredentialsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
