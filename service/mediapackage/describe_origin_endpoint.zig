...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOriginEndpointInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOriginEndpointOutput {
    var result: DescribeOriginEndpointOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeOriginEndpointOutput, body, alloc);
    }
    _ = status;
    _ = headers;
