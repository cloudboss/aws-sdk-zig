...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHarvestJobInput, options: Options) !DescribeHarvestJobOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHarvestJobInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/harvest_jobs/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHarvestJobOutput {
    var result: DescribeHarvestJobOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeHarvestJobOutput, body, alloc);
    }
    _ = status;
    _ = headers;
