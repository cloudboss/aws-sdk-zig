...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RotateChannelCredentialsInput, options: Options) !RotateChannelCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RotateChannelCredentialsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RotateChannelCredentialsOutput {
    var result: RotateChannelCredentialsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RotateChannelCredentialsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
