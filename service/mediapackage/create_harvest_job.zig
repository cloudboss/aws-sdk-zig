...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateHarvestJobInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"OriginEndpointId\":");
    try aws.json.writeValue(@TypeOf(input.origin_endpoint_id), input.origin_endpoint_id, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"S3Destination\":");
    try aws.json.writeValue(@TypeOf(input.s3_destination), input.s3_destination, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
