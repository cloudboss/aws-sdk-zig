...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateControlInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateControlOutput {
    var result: CreateControlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateControlOutput, body, allocator);
    }
    _ = status;
    _ = headers;
