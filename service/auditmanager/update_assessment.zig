...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAssessmentInput, options: Options) !UpdateAssessmentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssessmentInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.assessment_description) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"assessmentDescription\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssessmentOutput {
    var result: UpdateAssessmentOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAssessmentOutput, body, alloc);
    }
    _ = status;
    _ = headers;
