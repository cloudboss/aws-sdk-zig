...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssessmentFrameworkShareInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"requestType\":");
    try aws.json.writeValue(@TypeOf(input.request_type), input.request_type, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssessmentFrameworkShareOutput {
    var result: UpdateAssessmentFrameworkShareOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAssessmentFrameworkShareOutput, body, alloc);
    }
    _ = status;
    _ = headers;
