...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterAccountInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.delegated_admin_account) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"delegatedAdminAccount\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterAccountOutput {
    var result: RegisterAccountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterAccountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
