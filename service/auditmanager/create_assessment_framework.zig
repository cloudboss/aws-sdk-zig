...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssessmentFrameworkInput, options: Options) !CreateAssessmentFrameworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssessmentFrameworkInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.compliance_type) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"complianceType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"controlSets\":");
    try aws.json.writeValue(@TypeOf(input.control_sets), input.control_sets, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssessmentFrameworkOutput {
    var result: CreateAssessmentFrameworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssessmentFrameworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
