...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEvidenceFileUploadUrlInput, options: Options) !GetEvidenceFileUploadUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEvidenceFileUploadUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try query_buf.appendSlice(alloc, "fileName=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.file_name);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEvidenceFileUploadUrlOutput {
    var result: GetEvidenceFileUploadUrlOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEvidenceFileUploadUrlOutput, body, alloc);
    }
    _ = status;
    _ = headers;
