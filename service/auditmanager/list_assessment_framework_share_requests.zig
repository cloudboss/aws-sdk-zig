...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAssessmentFrameworkShareRequestsInput, options: Options) !ListAssessmentFrameworkShareRequestsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAssessmentFrameworkShareRequestsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAssessmentFrameworkShareRequestsOutput {
    var result: ListAssessmentFrameworkShareRequestsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAssessmentFrameworkShareRequestsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
