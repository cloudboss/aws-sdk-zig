...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateAssessmentFrameworkInput, options: Options) !UpdateAssessmentFrameworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateAssessmentFrameworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateAssessmentFrameworkOutput {
    var result: UpdateAssessmentFrameworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateAssessmentFrameworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
