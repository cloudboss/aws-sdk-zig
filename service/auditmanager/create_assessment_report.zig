...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAssessmentReportInput, options: Options) !CreateAssessmentReportOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAssessmentReportInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAssessmentReportOutput {
    var result: CreateAssessmentReportOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAssessmentReportOutput, body, alloc);
    }
    _ = status;
    _ = headers;
