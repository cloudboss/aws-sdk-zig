...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListControlsInput, options: Options) !ListControlsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListControlsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.control_catalog_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "controlCatalogId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "controlType=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(input.control_type));
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListControlsOutput {
    var result: ListControlsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListControlsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
