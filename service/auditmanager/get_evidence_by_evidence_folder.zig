...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEvidenceByEvidenceFolderInput, options: CallOptions) !GetEvidenceByEvidenceFolderOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetEvidenceByEvidenceFolderInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetEvidenceByEvidenceFolderOutput {
    var result: GetEvidenceByEvidenceFolderOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetEvidenceByEvidenceFolderOutput, body, allocator);
    }
    _ = status;
    _ = headers;
