...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteTenantDatabaseInput, options: Options) !DeleteTenantDatabaseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteTenantDatabaseInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteTenantDatabase&Version=2014-10-31");
    ...
    if (input.final_db_snapshot_identifier) |v| {
        try body_buf.appendSlice(alloc, "&FinalDBSnapshotIdentifier=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.skip_final_snapshot) |v| {
        try body_buf.appendSlice(alloc, "&SkipFinalSnapshot=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&TenantDBName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.tenant_db_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteTenantDatabaseOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "TenantDatabase")) {
                    result.tenant_database = try serde.deserializeTenantDatabase(&reader, alloc);
                } else {
