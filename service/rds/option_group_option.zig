...
pub const OptionGroupOption = struct {
    ...
    minimum_required_minor_engine_version: ?[]const u8,
    ...
    option_group_option_settings: ?[]const OptionGroupOptionSetting,

    /// The versions that are available for the option.
    option_group_option_versions: ?[]const OptionVersion,

    /// The options that conflict with this option.
    options_conflicts_with: ?[]const []const u8,

    /// The options that are prerequisites for this option.
    options_depended_on: ?[]const []const u8,
    ...
    supports_option_version_downgrade: ?bool,
