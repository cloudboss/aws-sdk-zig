...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebootDBClusterInput, options: Options) !RebootDBClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RebootDBClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RebootDBCluster&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RebootDBClusterOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RebootDBClusterResult")) break;
            },
            else => {},
        }
    }

    var result: RebootDBClusterOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
