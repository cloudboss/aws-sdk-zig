...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RevokeDBSecurityGroupIngressInput, options: Options) !RevokeDBSecurityGroupIngressOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RevokeDBSecurityGroupIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RevokeDBSecurityGroupIngress&Version=2014-10-31");
    if (input.cidrip) |v| {
        try body_buf.appendSlice(alloc, "&CIDRIP=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&DBSecurityGroupName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_security_group_name);
    if (input.ec2_security_group_id) |v| {
        try body_buf.appendSlice(alloc, "&EC2SecurityGroupId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RevokeDBSecurityGroupIngressOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBSecurityGroup")) {
                    result.db_security_group = try serde.deserializeDBSecurityGroup(&reader, alloc);
                } else {
