...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyActivityStreamInput, options: Options) !ModifyActivityStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyActivityStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyActivityStream&Version=2014-10-31");
    if (input.audit_policy_state) |v| {
        try body_buf.appendSlice(alloc, "&AuditPolicyState=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyActivityStreamOutput {
    _ = status;
    _ = headers;
