...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBProxyTargetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBProxyTargetGroups&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBProxyTargetGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.target_groups = try serde.deserializeTargetGroupList(&reader, alloc, "member");
                } else {
