...
pub const DescribeDBProxyTargetGroupsInput = struct {
...
pub const DescribeDBProxyTargetGroupsOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is specified, the response includes only records beyond the
    /// marker, up to the value specified by `MaxRecords`.
    marker: ?[]const u8 = null,
    ...
    target_groups: ?[]const DBProxyTargetGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBProxyTargetGroupsInput, options: CallOptions) !DescribeDBProxyTargetGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDBProxyTargetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDBProxyTargetGroups&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDBProxyTargetGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBProxyTargetGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBProxyTargetGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "TargetGroups")) {
                    result.target_groups = try serde.deserializeTargetGroupList(allocator, &reader, "member");
                } else {
