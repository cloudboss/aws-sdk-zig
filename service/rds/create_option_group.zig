...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateOptionGroupInput, options: Options) !CreateOptionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateOptionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateOptionGroup&Version=2014-10-31");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.major_engine_version);
    try body_buf.appendSlice(alloc, "&OptionGroupDescription=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.option_group_description);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateOptionGroupOutput {
    _ = status;
    _ = headers;
