...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDBProxyInput, options: Options) !CreateDBProxyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDBProxyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateDBProxy&Version=2014-10-31");
    if (input.auth) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    if (input.debug_logging) |v| {
        try body_buf.appendSlice(alloc, "&DebugLogging=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.default_auth_scheme) |v| {
        try body_buf.appendSlice(alloc, "&DefaultAuthScheme=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
    if (input.endpoint_network_type) |v| {
        try body_buf.appendSlice(alloc, "&EndpointNetworkType=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
    try body_buf.appendSlice(alloc, "&EngineFamily=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.engine_family));
    if (input.idle_client_timeout) |v| {
        try body_buf.appendSlice(alloc, "&IdleClientTimeout=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.require_tls) |v| {
        try body_buf.appendSlice(alloc, "&RequireTLS=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    ...
    if (input.target_connection_network_type) |v| {
        try body_buf.appendSlice(alloc, "&TargetConnectionNetworkType=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDBProxyOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBProxy")) {
                    result.db_proxy = try serde.deserializeDBProxy(&reader, alloc);
                } else {
