...
pub const DescribeDBProxyTargetsInput = struct {
...
pub const DescribeDBProxyTargetsOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is specified, the response includes only records beyond the
    /// marker, up to the value specified by `MaxRecords`.
    marker: ?[]const u8 = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDBProxyTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDBProxyTargets&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDBProxyTargetsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBProxyTargetsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBProxyTargetsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "Targets")) {
                    result.targets = try serde.deserializeTargetList(allocator, &reader, "member");
                } else {
