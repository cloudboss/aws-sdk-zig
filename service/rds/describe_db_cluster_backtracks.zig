...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBClusterBacktracksInput, options: Options) !DescribeDBClusterBacktracksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBClusterBacktracksInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBClusterBacktracks&Version=2014-10-31");
    if (input.backtrack_identifier) |v| {
        try body_buf.appendSlice(alloc, "&BacktrackIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBClusterBacktracksOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterBacktracks")) {
                    result.db_cluster_backtracks = try serde.deserializeDBClusterBacktrackList(&reader, alloc, "DBClusterBacktrack");
