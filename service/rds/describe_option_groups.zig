...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOptionGroupsInput, options: Options) !DescribeOptionGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOptionGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeOptionGroups&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOptionGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.option_groups_list = try serde.deserializeOptionGroupsList(&reader, alloc, "OptionGroup");
                } else {
