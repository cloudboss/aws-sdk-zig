...
pub const RestoreDBClusterToPointInTimeInput = struct {
    ...
    /// cluster.
    ///
    /// If the `DBClusterParameterGroupName` parameter is omitted, the default DB
    ...
    /// depending on the type of the cluster you are creating. You can create an
    /// Aurora Serverless v1 clone from a provisioned cluster, or a provisioned
    /// clone from an Aurora Serverless v1 cluster. To create a clone that is an
    /// Aurora Serverless v1 cluster, the original cluster must be an Aurora
    /// Serverless v1 cluster or an encrypted provisioned cluster. To create a full
    /// copy that is an Aurora Serverless v1 cluster, specify the engine mode
    /// `serverless`.
    ///
    /// Valid for: Aurora DB clusters only
    ...
    source_db_cluster_resource_id: ?[]const u8 = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: RestoreDBClusterToPointInTimeInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.source_db_cluster_resource_id) |v| {
        try body_buf.appendSlice(allocator, "&SourceDbClusterResourceId=");
        try aws.url.appendUrlEncoded(allocator, &body_buf, v);
    }
