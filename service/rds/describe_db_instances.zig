...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBInstancesInput, options: CallOptions) !DescribeDBInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDBInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDBInstances&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDBInstancesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBInstances")) {
                    result.db_instances = try serde.deserializeDBInstanceList(allocator, &reader, "DBInstance");
