...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBClusterSnapshotsInput, options: Options) !DescribeDBClusterSnapshotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBClusterSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBClusterSnapshots&Version=2014-10-31");
    ...
    if (input.db_cluster_resource_id) |v| {
        try body_buf.appendSlice(alloc, "&DbClusterResourceId=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    ...
    if (input.include_public) |v| {
        try body_buf.appendSlice(alloc, "&IncludePublic=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.include_shared) |v| {
        try body_buf.appendSlice(alloc, "&IncludeShared=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.marker) |v| {
    ...
    if (input.snapshot_type) |v| {
        try body_buf.appendSlice(alloc, "&SnapshotType=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBClusterSnapshotsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterSnapshots")) {
                    result.db_cluster_snapshots = try serde.deserializeDBClusterSnapshotList(&reader, alloc, "DBClusterSnapshot");
