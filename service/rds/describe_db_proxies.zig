...
pub const DescribeDBProxiesInput = struct {
...
pub const DescribeDBProxiesOutput = struct {
    /// A return value representing an arbitrary number of `DBProxy` data
    /// structures.
    db_proxies: ?[]const DBProxy = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBProxiesInput, options: CallOptions) !DescribeDBProxiesOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDBProxiesOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBProxiesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBProxiesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
