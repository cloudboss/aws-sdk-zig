...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBRecommendationsInput, options: Options) !DescribeDBRecommendationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBRecommendationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBRecommendations&Version=2014-10-31");
    ...
    if (input.last_updated_after) |v| {
        try body_buf.appendSlice(alloc, "&LastUpdatedAfter=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.last_updated_before) |v| {
        try body_buf.appendSlice(alloc, "&LastUpdatedBefore=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.locale) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBRecommendationsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBRecommendations")) {
                    result.db_recommendations = try serde.deserializeDBRecommendationList(&reader, alloc, "member");
