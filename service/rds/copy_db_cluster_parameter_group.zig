...
pub const CopyDBClusterParameterGroupInput = struct {
    ...
    /// an ARN for Amazon
    /// RDS](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_Tagging.ARN.html#USER_Tagging.ARN.Constructing) in the *Amazon Aurora User Guide*.
    ///
    /// Constraints:
    ///
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyDBClusterParameterGroupInput, options: Options) !CopyDBClusterParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyDBClusterParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CopyDBClusterParameterGroup&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&SourceDBClusterParameterGroupIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_db_cluster_parameter_group_identifier);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    try body_buf.appendSlice(alloc, "&TargetDBClusterParameterGroupDescription=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_db_cluster_parameter_group_description);
    try body_buf.appendSlice(alloc, "&TargetDBClusterParameterGroupIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_db_cluster_parameter_group_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyDBClusterParameterGroupOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterParameterGroup")) {
                    result.db_cluster_parameter_group = try serde.deserializeDBClusterParameterGroup(&reader, alloc);
                } else {
