...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBProxyEndpointInput, options: Options) !ModifyDBProxyEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBProxyEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBProxyEndpoint&Version=2014-10-31");
    ...
    if (input.new_db_proxy_endpoint_name) |v| {
        try body_buf.appendSlice(alloc, "&NewDBProxyEndpointName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBProxyEndpointOutput {
    _ = status;
    _ = headers;
