...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTenantDatabasesInput, options: CallOptions) !DescribeTenantDatabasesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeTenantDatabasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeTenantDatabases&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeTenantDatabasesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.tenant_databases = try serde.deserializeTenantDatabasesList(allocator, &reader, "TenantDatabase");
                } else {
