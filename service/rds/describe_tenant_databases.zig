...
pub const DescribeTenantDatabasesInput = struct {
    /// The user-supplied DB instance identifier, which must match the identifier of
    /// an existing instance owned by the Amazon Web Services account. This
...
pub const DescribeTenantDatabasesOutput = struct {
    /// An optional pagination token provided by a previous
    /// `DescribeTenantDatabases` request. If this parameter is specified, the
    /// response includes only records beyond the marker, up to the value specified
    /// by `MaxRecords`.
    ...
    /// operation.
    tenant_databases: ?[]const TenantDatabase = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTenantDatabasesInput, options: Options) !DescribeTenantDatabasesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTenantDatabasesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeTenantDatabases&Version=2014-10-31");
    if (input.db_instance_identifier) |v| {
        try body_buf.appendSlice(alloc, "&DBInstanceIdentifier=");
    ...
    if (input.tenant_db_name) |v| {
        try body_buf.appendSlice(alloc, "&TenantDBName=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTenantDatabasesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeTenantDatabasesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeTenantDatabasesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "TenantDatabases")) {
                    result.tenant_databases = try serde.deserializeTenantDatabasesList(&reader, alloc, "TenantDatabase");
                } else {
