...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDBClusterSnapshotInput, options: Options) !CreateDBClusterSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDBClusterSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateDBClusterSnapshot&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDBClusterSnapshotOutput {
    _ = status;
    _ = headers;
