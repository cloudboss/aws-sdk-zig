...
pub const CopyDBParameterGroupInput = struct {
    ...
    target_db_parameter_group_description: []const u8,
    ...
    target_db_parameter_group_identifier: []const u8,
};

pub const CopyDBParameterGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyDBParameterGroupInput, options: Options) !CopyDBParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyDBParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CopyDBParameterGroup&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&SourceDBParameterGroupIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_db_parameter_group_identifier);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    try body_buf.appendSlice(alloc, "&TargetDBParameterGroupDescription=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_db_parameter_group_description);
    try body_buf.appendSlice(alloc, "&TargetDBParameterGroupIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_db_parameter_group_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyDBParameterGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CopyDBParameterGroupResult")) break;
            },
            else => {},
        }
    }

    var result: CopyDBParameterGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBParameterGroup")) {
                    result.db_parameter_group = try serde.deserializeDBParameterGroup(&reader, alloc);
                } else {
