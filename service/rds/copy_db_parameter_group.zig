...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyDBParameterGroupInput, options: CallOptions) !CopyDBParameterGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CopyDBParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CopyDBParameterGroup&Version=2014-10-31");
    try body_buf.appendSlice(allocator, "&SourceDBParameterGroupIdentifier=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.source_db_parameter_group_identifier);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    try body_buf.appendSlice(allocator, "&TargetDBParameterGroupDescription=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.target_db_parameter_group_description);
    try body_buf.appendSlice(allocator, "&TargetDBParameterGroupIdentifier=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.target_db_parameter_group_identifier);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CopyDBParameterGroupOutput {
    _ = status;
    _ = headers;
