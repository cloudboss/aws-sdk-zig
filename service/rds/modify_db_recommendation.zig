...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBRecommendationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBRecommendation&Version=2014-10-31");
    if (input.locale) |v| {
        try body_buf.appendSlice(alloc, "&Locale=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&RecommendationId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.recommendation_id);
    if (input.recommended_action_updates) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
                ...
                try aws.url.appendUrlEncoded(alloc, &body_buf, item.action_id);
            }
            {
                var prefix_buf: [256]u8 = undefined;
                const field_prefix = std.fmt.bufPrint(&prefix_buf, "&RecommendedActionUpdates.member.{d}.Status=", .{n}) catch continue;
                try body_buf.appendSlice(alloc, field_prefix);
                try aws.url.appendUrlEncoded(alloc, &body_buf, item.status);
            }
        }
    }
    if (input.status) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBRecommendationOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBRecommendation")) {
                    result.db_recommendation = try serde.deserializeDBRecommendation(&reader, alloc);
                } else {
