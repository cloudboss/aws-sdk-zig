...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReservedDBInstancesOfferingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeReservedDBInstancesOfferings&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReservedDBInstancesOfferingsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.reserved_db_instances_offerings = try serde.deserializeReservedDBInstancesOfferingList(&reader, alloc, "ReservedDBInstancesOffering");
                } else {
