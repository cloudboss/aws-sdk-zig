...
pub const DeleteDBProxyEndpointInput = struct {
    /// The name of the DB proxy endpoint to delete.
    db_proxy_endpoint_name: []const u8,
};

pub const DeleteDBProxyEndpointOutput = struct {
    /// The data structure representing the details of the DB proxy endpoint that
    /// you delete.
    db_proxy_endpoint: ?DBProxyEndpoint = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBProxyEndpointInput, options: CallOptions) !DeleteDBProxyEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDBProxyEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteDBProxyEndpoint&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDBProxyEndpointOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteDBProxyEndpointResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteDBProxyEndpointOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
