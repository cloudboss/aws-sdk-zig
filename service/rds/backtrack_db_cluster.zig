...
pub const BacktrackDBClusterOutput = struct {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BacktrackDBClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=BacktrackDBCluster&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&BacktrackTo=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{input.backtrack_to}) catch "");
    ...
    if (input.use_earliest_time_on_point_in_time_unavailable) |v| {
        try body_buf.appendSlice(alloc, "&UseEarliestTimeOnPointInTimeUnavailable=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BacktrackDBClusterOutput {
    _ = status;
    _ = headers;
