...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveRoleFromDBInstanceInput, options: Options) !RemoveRoleFromDBInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RemoveRoleFromDBInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RemoveRoleFromDBInstance&Version=2014-10-31");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.feature_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RemoveRoleFromDBInstanceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
