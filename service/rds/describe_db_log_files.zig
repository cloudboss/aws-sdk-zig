...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBLogFilesInput, options: Options) !DescribeDBLogFilesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBLogFilesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBLogFiles&Version=2014-10-31");
    ...
    if (input.file_last_written) |v| {
        try body_buf.appendSlice(alloc, "&FileLastWritten=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.filename_contains) |v| {
        try body_buf.appendSlice(alloc, "&FilenameContains=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.file_size) |v| {
        try body_buf.appendSlice(alloc, "&FileSize=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBLogFilesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBLogFiles")) {
                    result.describe_db_log_files = try serde.deserializeDescribeDBLogFilesList(&reader, alloc, "DescribeDBLogFilesDetails");
