...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyOptionGroupInput, options: Options) !ModifyOptionGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyOptionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyOptionGroup&Version=2014-10-31");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.option_group_name);
    if (input.options_to_include) |list| {
        for (list, 0..) |item, idx| {
            ...
            }
            {
                ...
                try body_buf.appendSlice(alloc, field_prefix);
                try aws.url.appendUrlEncoded(alloc, &body_buf, item.option_name);
            }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyOptionGroupOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "OptionGroup")) {
                    result.option_group = try serde.deserializeOptionGroup(&reader, alloc);
                } else {
