...
pub const RegisterDBProxyTargetsInput = struct {
...
pub const RegisterDBProxyTargetsOutput = struct {
    /// One or more `DBProxyTarget` objects that are created when you register
    /// targets with a target group.
    db_proxy_targets: ?[]const DBProxyTarget = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterDBProxyTargetsInput, options: CallOptions) !RegisterDBProxyTargetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterDBProxyTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=RegisterDBProxyTargets&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterDBProxyTargetsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RegisterDBProxyTargetsResult")) break;
            },
            else => {},
        }
    }

    var result: RegisterDBProxyTargetsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBProxyTargets")) {
                    result.db_proxy_targets = try serde.deserializeTargetList(allocator, &reader, "member");
                } else {
