...
pub const CreateIntegrationInput = struct {
    ...
    /// Redshift](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/zero-etl.filtering.html) or [Data filtering for Amazon RDS zero-ETL integrations with Amazon Redshift](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/zero-etl.filtering.html).
    ...
    /// identifier for the key to use to encrypt the integration. If you don't
    /// specify an encryption key, RDS uses a default Amazon Web Services owned key.
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateIntegrationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateIntegration&Version=2014-10-31");
    ...
    if (input.data_filter) |v| {
        try body_buf.appendSlice(alloc, "&DataFilter=");
