...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBSnapshotAttributeInput, options: Options) !ModifyDBSnapshotAttributeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBSnapshotAttributeInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBSnapshotAttribute&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBSnapshotAttributeOutput {
    _ = status;
    _ = headers;
