...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyDBSnapshotInput, options: Options) !CopyDBSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyDBSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CopyDBSnapshot&Version=2014-10-31");
    if (input.copy_option_group) |v| {
        try body_buf.appendSlice(alloc, "&CopyOptionGroup=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.copy_tags) |v| {
        try body_buf.appendSlice(alloc, "&CopyTags=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    ...
    if (input.snapshot_availability_zone) |v| {
        try body_buf.appendSlice(alloc, "&SnapshotAvailabilityZone=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.snapshot_target) |v| {
        try body_buf.appendSlice(alloc, "&SnapshotTarget=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&SourceDBSnapshotIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_db_snapshot_identifier);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    if (input.target_custom_availability_zone) |v| {
        try body_buf.appendSlice(alloc, "&TargetCustomAvailabilityZone=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    try body_buf.appendSlice(alloc, "&TargetDBSnapshotIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_db_snapshot_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyDBSnapshotOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBSnapshot")) {
                    result.db_snapshot = try serde.deserializeDBSnapshot(&reader, alloc);
                } else {
