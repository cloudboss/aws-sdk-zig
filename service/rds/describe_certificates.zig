...
pub const DescribeCertificatesOutput = struct {
    ...
    default_certificate_for_new_launches: ?[]const u8 = null,
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                ...
                } else if (std.mem.eql(u8, e.local, "DefaultCertificateForNewLaunches")) {
                    result.default_certificate_for_new_launches = try alloc.dupe(u8, try reader.readElementText());
