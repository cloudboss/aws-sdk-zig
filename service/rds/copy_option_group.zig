...
fn serializeRequest(alloc: std.mem.Allocator, input: CopyOptionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CopyOptionGroup&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&SourceOptionGroupIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_option_group_identifier);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    try body_buf.appendSlice(alloc, "&TargetOptionGroupDescription=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_option_group_description);
    try body_buf.appendSlice(alloc, "&TargetOptionGroupIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_option_group_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CopyOptionGroupOutput {
    _ = status;
    _ = headers;
