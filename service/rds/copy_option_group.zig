...
pub const CopyOptionGroupInput = struct {
    /// The identifier for the source option group.
    ///
    /// Constraints:
    ///
    /// * Must specify a valid option group.
    source_option_group_identifier: []const u8,

    tags: ?[]const Tag = null,

    /// The description for the copied option group.
    target_option_group_description: []const u8,
    ...
    target_option_group_identifier: []const u8,
};

pub const CopyOptionGroupOutput = struct {
    option_group: ?OptionGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CopyOptionGroupInput, options: CallOptions) !CopyOptionGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CopyOptionGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CopyOptionGroup&Version=2014-10-31");
    try body_buf.appendSlice(allocator, "&SourceOptionGroupIdentifier=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.source_option_group_identifier);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    try body_buf.appendSlice(allocator, "&TargetOptionGroupDescription=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.target_option_group_description);
    try body_buf.appendSlice(allocator, "&TargetOptionGroupIdentifier=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.target_option_group_identifier);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CopyOptionGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CopyOptionGroupResult")) break;
            },
            else => {},
        }
    }

    var result: CopyOptionGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
