...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDBProxyEndpointInput, options: Options) !CreateDBProxyEndpointOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDBProxyEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateDBProxyEndpoint&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&DBProxyEndpointName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_proxy_endpoint_name);
    ...
    if (input.target_role) |v| {
        try body_buf.appendSlice(alloc, "&TargetRole=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(v));
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDBProxyEndpointOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBProxyEndpoint")) {
                    result.db_proxy_endpoint = try serde.deserializeDBProxyEndpoint(&reader, alloc);
                } else {
