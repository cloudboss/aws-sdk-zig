...
pub const StopDBInstanceInput = struct {
...
pub const StopDBInstanceOutput = struct {
    db_instance: ?DBInstance = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopDBInstanceInput, options: CallOptions) !StopDBInstanceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopDBInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=StopDBInstance&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopDBInstanceOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StopDBInstanceResult")) break;
            },
            else => {},
        }
    }

    var result: StopDBInstanceOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
