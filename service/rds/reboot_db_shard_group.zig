...
pub const RebootDBShardGroupInput = struct {
    /// The name of the DB shard group to reboot.
    db_shard_group_identifier: []const u8,
};

pub const RebootDBShardGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebootDBShardGroupInput, options: Options) !RebootDBShardGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RebootDBShardGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=RebootDBShardGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RebootDBShardGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "RebootDBShardGroupResult")) break;
            },
            else => {},
        }
    }

    var result: RebootDBShardGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
