...
pub const StopActivityStreamInput = struct {
...
pub const StopActivityStreamOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopActivityStreamInput, options: Options) !StopActivityStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopActivityStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=StopActivityStream&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopActivityStreamOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StopActivityStreamResult")) break;
            },
            else => {},
        }
    }

    var result: StopActivityStreamOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "KinesisStreamName")) {
                    result.kinesis_stream_name = try alloc.dupe(u8, try reader.readElementText());
