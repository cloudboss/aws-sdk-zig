...
pub const AuthorizeDBSecurityGroupIngressInput = struct {
...
pub const AuthorizeDBSecurityGroupIngressOutput = struct {
    db_security_group: ?DBSecurityGroup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AuthorizeDBSecurityGroupIngressInput, options: CallOptions) !AuthorizeDBSecurityGroupIngressOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AuthorizeDBSecurityGroupIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=AuthorizeDBSecurityGroupIngress&Version=2014-10-31");
    if (input.cidrip) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AuthorizeDBSecurityGroupIngressOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AuthorizeDBSecurityGroupIngressResult")) break;
            },
            else => {},
        }
    }

    var result: AuthorizeDBSecurityGroupIngressOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
