...
fn serializeRequest(alloc: std.mem.Allocator, input: AuthorizeDBSecurityGroupIngressInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=AuthorizeDBSecurityGroupIngress&Version=2014-10-31");
    if (input.cidrip) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AuthorizeDBSecurityGroupIngressOutput {
    _ = status;
    _ = headers;
