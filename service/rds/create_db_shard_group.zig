...
pub const CreateDBShardGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDBShardGroupInput, options: Options) !CreateDBShardGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDBShardGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateDBShardGroup&Version=2014-10-31");
    if (input.compute_redundancy) |v| {
        try body_buf.appendSlice(alloc, "&ComputeRedundancy=");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_shard_group_identifier);
    try body_buf.appendSlice(alloc, "&MaxACU=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.max_acu);
    if (input.min_acu) |v| {
        try body_buf.appendSlice(alloc, "&MinACU=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDBShardGroupOutput {
    _ = status;
    _ = headers;
