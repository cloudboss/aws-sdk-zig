...
pub const DeleteDBProxyInput = struct {
    /// The name of the DB proxy to delete.
    db_proxy_name: []const u8,
};

pub const DeleteDBProxyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBProxyInput, options: Options) !DeleteDBProxyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDBProxyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteDBProxy&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDBProxyOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteDBProxyResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteDBProxyOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
