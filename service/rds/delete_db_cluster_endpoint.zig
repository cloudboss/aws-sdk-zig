...
pub const DeleteDBClusterEndpointInput = struct {
    /// The identifier associated with the custom endpoint. This parameter is stored
    /// as a lowercase string.
    db_cluster_endpoint_identifier: []const u8,
};

pub const DeleteDBClusterEndpointOutput = @import("db_cluster_endpoint.zig").DBClusterEndpoint;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBClusterEndpointInput, options: CallOptions) !DeleteDBClusterEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDBClusterEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteDBClusterEndpoint&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDBClusterEndpointOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteDBClusterEndpointResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteDBClusterEndpointOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
