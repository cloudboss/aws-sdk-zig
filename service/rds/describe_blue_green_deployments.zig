...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeBlueGreenDeploymentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeBlueGreenDeployments&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeBlueGreenDeploymentsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "BlueGreenDeployments")) {
                    result.blue_green_deployments = try serde.deserializeBlueGreenDeploymentList(allocator, &reader, "member");
