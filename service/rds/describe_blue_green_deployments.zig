...
pub const DescribeBlueGreenDeploymentsInput = struct {
    ...
    ///   deployments. The results list only includes information about the
    ...
    /// `DescribeBlueGreenDeployments` request. If you specify this parameter, the
    /// response only includes records beyond the marker, up to the value specified
    /// by `MaxRecords`.
...
pub const DescribeBlueGreenDeploymentsOutput = struct {
    /// A list of blue/green deployments in the current account and Amazon Web
    /// Services Region.
    blue_green_deployments: ?[]const BlueGreenDeployment = null,
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeBlueGreenDeploymentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeBlueGreenDeployments&Version=2014-10-31");
    if (input.blue_green_deployment_identifier) |v| {
        try body_buf.appendSlice(alloc, "&BlueGreenDeploymentIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeBlueGreenDeploymentsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeBlueGreenDeploymentsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeBlueGreenDeploymentsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "BlueGreenDeployments")) {
                    result.blue_green_deployments = try serde.deserializeBlueGreenDeploymentList(&reader, alloc, "member");
