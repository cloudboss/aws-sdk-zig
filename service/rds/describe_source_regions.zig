...
pub const DescribeSourceRegionsInput = struct {
...
pub const DescribeSourceRegionsOutput = struct {
    ...
    source_regions: ?[]const SourceRegion = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSourceRegionsInput, options: CallOptions) !DescribeSourceRegionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeSourceRegionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeSourceRegions&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeSourceRegionsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeSourceRegionsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeSourceRegionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "SourceRegions")) {
                    result.source_regions = try serde.deserializeSourceRegionList(allocator, &reader, "SourceRegion");
                } else {
