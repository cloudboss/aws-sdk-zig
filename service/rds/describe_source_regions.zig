...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSourceRegionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeSourceRegions&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSourceRegionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    ...
                    result.source_regions = try serde.deserializeSourceRegionList(&reader, alloc, "SourceRegion");
                } else {
