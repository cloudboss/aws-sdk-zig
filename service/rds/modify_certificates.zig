...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyCertificatesInput, options: Options) !ModifyCertificatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyCertificatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyCertificates&Version=2014-10-31");
    ...
    if (input.remove_customer_override) |v| {
        try body_buf.appendSlice(alloc, "&RemoveCustomerOverride=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyCertificatesOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Certificate")) {
                    result.certificate = try serde.deserializeCertificate(&reader, alloc);
                } else {
