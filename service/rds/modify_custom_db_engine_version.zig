...
const CustomEngineVersionStatus = @import("custom_engine_version_status.zig").CustomEngineVersionStatus;
...
pub const ModifyCustomDBEngineVersionInput = struct {
    ...
    /// eligible for use again. For example, you might change the status of your CEV
    /// from `available` to `inactive`, and from `inactive` back to `available`. To
    /// change the availability status of the CEV, it must not currently be in use
    /// by an RDS Custom instance, snapshot, or automated backup.
    status: ?CustomEngineVersionStatus = null,
};

pub const ModifyCustomDBEngineVersionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyCustomDBEngineVersionInput, options: Options) !ModifyCustomDBEngineVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyCustomDBEngineVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyCustomDBEngineVersion&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyCustomDBEngineVersionOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ModifyCustomDBEngineVersionResult")) break;
            },
            else => {},
        }
    }

    var result: ModifyCustomDBEngineVersionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
