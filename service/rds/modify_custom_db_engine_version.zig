...
pub const ModifyCustomDBEngineVersionOutput = @import("db_engine_version.zig").DBEngineVersion;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyCustomDBEngineVersionInput, options: CallOptions) !ModifyCustomDBEngineVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ModifyCustomDBEngineVersionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ModifyCustomDBEngineVersionOutput {
    _ = status;
    _ = headers;
