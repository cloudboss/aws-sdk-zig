...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SwitchoverBlueGreenDeploymentInput, options: Options) !SwitchoverBlueGreenDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SwitchoverBlueGreenDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SwitchoverBlueGreenDeployment&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&BlueGreenDeploymentIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.blue_green_deployment_identifier);
    if (input.switchover_timeout) |v| {
        try body_buf.appendSlice(alloc, "&SwitchoverTimeout=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SwitchoverBlueGreenDeploymentOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "BlueGreenDeployment")) {
                    result.blue_green_deployment = try serde.deserializeBlueGreenDeployment(&reader, alloc);
                } else {
