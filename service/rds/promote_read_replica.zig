...
pub const PromoteReadReplicaInput = struct {
    ...
    /// time for each Amazon Web Services Region. To see the time blocks available,
    /// see [ Adjusting the Preferred Maintenance
    /// Window](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/AdjustingTheMaintenanceWindow.html) in the *Amazon RDS User Guide.*
    ///
    /// Constraints:
    ///
...
pub const PromoteReadReplicaOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PromoteReadReplicaInput, options: Options) !PromoteReadReplicaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PromoteReadReplicaInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PromoteReadReplica&Version=2014-10-31");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_instance_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PromoteReadReplicaOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "PromoteReadReplicaResult")) break;
            },
            else => {},
        }
    }

    var result: PromoteReadReplicaOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
