...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartActivityStreamInput, options: Options) !StartActivityStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartActivityStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=StartActivityStream&Version=2014-10-31");
    ...
    if (input.engine_native_audit_fields_included) |v| {
        try body_buf.appendSlice(alloc, "&EngineNativeAuditFieldsIncluded=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, @tagName(input.mode));
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartActivityStreamOutput {
    _ = status;
    _ = headers;
