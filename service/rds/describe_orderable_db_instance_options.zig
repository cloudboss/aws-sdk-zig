...
pub const DescribeOrderableDBInstanceOptionsInput = struct {
...
pub const DescribeOrderableDBInstanceOptionsOutput = struct {
    /// An optional pagination token provided by a previous
    ...
    orderable_db_instance_options: ?[]const OrderableDBInstanceOption = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOrderableDBInstanceOptionsInput, options: CallOptions) !DescribeOrderableDBInstanceOptionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeOrderableDBInstanceOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeOrderableDBInstanceOptions&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeOrderableDBInstanceOptionsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeOrderableDBInstanceOptionsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeOrderableDBInstanceOptionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try allocator.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "OrderableDBInstanceOptions")) {
                    result.orderable_db_instance_options = try serde.deserializeOrderableDBInstanceOptionsList(allocator, &reader, "OrderableDBInstanceOption");
                } else {
