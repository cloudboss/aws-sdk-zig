...
pub const CreateEventSubscriptionInput = struct {
    ...
    /// ](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_Events.Messages.html) or the [ *Amazon Aurora User Guide* ](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_Events.Messages.html). You can also see this list by using the `DescribeEventCategories` operation.
    ...
    /// see [Message ordering and deduplication (FIFO
    /// topics)](https://docs.aws.amazon.com/sns/latest/dg/sns-fifo-topics.html) in
    ...
    /// to `db-instance`. For RDS Proxy events, specify `db-proxy`. If this value
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateEventSubscriptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=CreateEventSubscription&Version=2014-10-31");
    if (input.enabled) |v| {
    ...
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.sns_topic_arn);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateEventSubscriptionOutput {
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateEventSubscriptionResult")) break;
            },
            else => {},
        }
    }

    var result: CreateEventSubscriptionOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
