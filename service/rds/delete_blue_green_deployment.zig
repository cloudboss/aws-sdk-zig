...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBlueGreenDeploymentInput, options: Options) !DeleteBlueGreenDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteBlueGreenDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteBlueGreenDeployment&Version=2014-10-31");
    ...
    if (input.delete_target) |v| {
        try body_buf.appendSlice(alloc, "&DeleteTarget=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteBlueGreenDeploymentOutput {
    _ = status;
    _ = headers;
