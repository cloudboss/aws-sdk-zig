...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyCurrentDBClusterCapacityInput, options: Options) !ModifyCurrentDBClusterCapacityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyCurrentDBClusterCapacityInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyCurrentDBClusterCapacity&Version=2014-10-31");
    if (input.capacity) |v| {
    ...
    if (input.seconds_before_timeout) |v| {
        try body_buf.appendSlice(alloc, "&SecondsBeforeTimeout=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, std.fmt.allocPrint(alloc, "{d}", .{v}) catch "");
    }
    if (input.timeout_action) |v| {
        try body_buf.appendSlice(alloc, "&TimeoutAction=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyCurrentDBClusterCapacityOutput {
    _ = status;
    _ = headers;
