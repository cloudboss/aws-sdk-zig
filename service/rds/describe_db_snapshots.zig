...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBSnapshotsInput, options: Options) !DescribeDBSnapshotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBSnapshotsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBSnapshots&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBSnapshotsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBSnapshots")) {
                    result.db_snapshots = try serde.deserializeDBSnapshotList(&reader, alloc, "DBSnapshot");
