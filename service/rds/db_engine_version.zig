...
pub const DBEngineVersion = struct {
    ...
    custom_db_engine_version_manifest: ?[]const u8,
    ...
    db_engine_media_type: ?[]const u8,

    /// The ARN of the custom engine version.
    db_engine_version_arn: ?[]const u8,
    ...
    /// output.
    supported_feature_names: ?[]const []const u8,

    /// A list of the character sets supported by the Oracle DB engine for the
    /// `NcharCharacterSetName` parameter of the `CreateDBInstance` operation.
    supported_nchar_character_sets: ?[]const CharacterSet,
