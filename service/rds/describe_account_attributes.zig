...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAccountAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeAccountAttributes&Version=2014-10-31");
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AccountQuotas")) {
                    result.account_quotas = try serde.deserializeAccountQuotaList(allocator, &reader, "AccountQuota");
                } else {
