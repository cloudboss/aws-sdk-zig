...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBMajorEngineVersionsInput, options: Options) !DescribeDBMajorEngineVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBMajorEngineVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBMajorEngineVersions&Version=2014-10-31");
    if (input.engine) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBMajorEngineVersionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBMajorEngineVersions")) {
                    result.db_major_engine_versions = try serde.deserializeDBMajorEngineVersionsList(&reader, alloc, "DBMajorEngineVersion");
