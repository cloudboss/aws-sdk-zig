...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBParameterGroupInput, options: Options) !ModifyDBParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBParameterGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBParameterGroupOutput {
    _ = status;
    _ = headers;
