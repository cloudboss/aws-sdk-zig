...
pub const StopDBInstanceAutomatedBackupsReplicationInput = struct {
...
pub const StopDBInstanceAutomatedBackupsReplicationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopDBInstanceAutomatedBackupsReplicationInput, options: Options) !StopDBInstanceAutomatedBackupsReplicationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopDBInstanceAutomatedBackupsReplicationInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=StopDBInstanceAutomatedBackupsReplication&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&SourceDBInstanceArn=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.source_db_instance_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopDBInstanceAutomatedBackupsReplicationOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StopDBInstanceAutomatedBackupsReplicationResult")) break;
            },
            else => {},
        }
    }

    var result: StopDBInstanceAutomatedBackupsReplicationOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBInstanceAutomatedBackup")) {
                    result.db_instance_automated_backup = try serde.deserializeDBInstanceAutomatedBackup(&reader, alloc);
                } else {
