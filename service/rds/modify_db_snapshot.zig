...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBSnapshotInput, options: Options) !ModifyDBSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBSnapshot&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBSnapshotOutput {
    _ = status;
    _ = headers;
