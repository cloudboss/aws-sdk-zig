...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: FailoverGlobalClusterInput, options: Options) !FailoverGlobalClusterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: FailoverGlobalClusterInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=FailoverGlobalCluster&Version=2014-10-31");
    if (input.allow_data_loss) |v| {
        try body_buf.appendSlice(alloc, "&AllowDataLoss=");
    ...
    if (input.switchover) |v| {
        try body_buf.appendSlice(alloc, "&Switchover=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    try body_buf.appendSlice(alloc, "&TargetDbClusterIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.target_db_cluster_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !FailoverGlobalClusterOutput {
    _ = status;
    _ = headers;
