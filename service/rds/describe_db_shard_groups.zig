...
pub const DescribeDBShardGroupsInput = struct {
...
pub const DescribeDBShardGroupsOutput = struct {
    /// Contains a list of DB shard groups for the user.
    db_shard_groups: ?[]const DBShardGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBShardGroupsInput, options: Options) !DescribeDBShardGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBShardGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBShardGroups&Version=2014-10-31");
    if (input.db_shard_group_identifier) |v| {
        try body_buf.appendSlice(alloc, "&DBShardGroupIdentifier=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBShardGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBShardGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBShardGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBShardGroups")) {
                    result.db_shard_groups = try serde.deserializeDBShardGroupsList(&reader, alloc, "DBShardGroup");
