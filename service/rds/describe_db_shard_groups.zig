...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBShardGroupsInput, options: CallOptions) !DescribeDBShardGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDBShardGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDBShardGroups&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDBShardGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBShardGroups")) {
                    result.db_shard_groups = try serde.deserializeDBShardGroupsList(allocator, &reader, "DBShardGroup");
