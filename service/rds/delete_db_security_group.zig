...
pub const DeleteDBSecurityGroupInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBSecurityGroupInput, options: Options) !DeleteDBSecurityGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDBSecurityGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteDBSecurityGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDBSecurityGroupOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
    const result: DeleteDBSecurityGroupOutput = .{};
