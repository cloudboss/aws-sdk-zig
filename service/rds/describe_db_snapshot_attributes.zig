...
pub const DescribeDBSnapshotAttributesInput = struct {
    /// The identifier for the DB snapshot to describe the attributes for.
    db_snapshot_identifier: []const u8,
};

pub const DescribeDBSnapshotAttributesOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBSnapshotAttributesInput, options: Options) !DescribeDBSnapshotAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBSnapshotAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBSnapshotAttributes&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&DBSnapshotIdentifier=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_snapshot_identifier);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBSnapshotAttributesOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBSnapshotAttributesResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBSnapshotAttributesOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBSnapshotAttributesResult")) {
                    result.db_snapshot_attributes_result = try serde.deserializeDBSnapshotAttributesResult(&reader, alloc);
                } else {
