...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBEngineVersionsInput, options: Options) !DescribeDBEngineVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBEngineVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBEngineVersions&Version=2014-10-31");
    ...
    if (input.include_all) |v| {
        try body_buf.appendSlice(alloc, "&IncludeAll=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.list_supported_character_sets) |v| {
        try body_buf.appendSlice(alloc, "&ListSupportedCharacterSets=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.list_supported_timezones) |v| {
        try body_buf.appendSlice(alloc, "&ListSupportedTimezones=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, if (v) "true" else "false");
    }
    if (input.marker) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBEngineVersionsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBEngineVersions")) {
                    result.db_engine_versions = try serde.deserializeDBEngineVersionList(&reader, alloc, "DBEngineVersion");
