...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PurchaseReservedDBInstancesOfferingInput, options: Options) !PurchaseReservedDBInstancesOfferingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PurchaseReservedDBInstancesOfferingInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=PurchaseReservedDBInstancesOffering&Version=2014-10-31");
    if (input.db_instance_count) |v| {
        try body_buf.appendSlice(alloc, "&DBInstanceCount=");
    ...
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.reserved_db_instances_offering_id);
    if (input.tags) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PurchaseReservedDBInstancesOfferingOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "ReservedDBInstance")) {
                    result.reserved_db_instance = try serde.deserializeReservedDBInstance(&reader, alloc);
                } else {
