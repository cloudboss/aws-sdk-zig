...
pub const DeleteDBSnapshotInput = struct {
...
pub const DeleteDBSnapshotOutput = struct {
    db_snapshot: ?DBSnapshot = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBSnapshotInput, options: CallOptions) !DeleteDBSnapshotOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDBSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteDBSnapshot&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDBSnapshotOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteDBSnapshotResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteDBSnapshotOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
