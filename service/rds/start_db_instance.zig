...
pub const StartDBInstanceInput = struct {
    /// The user-supplied instance identifier.
    db_instance_identifier: []const u8,
};

pub const StartDBInstanceOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDBInstanceInput, options: Options) !StartDBInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDBInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=StartDBInstance&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDBInstanceOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "StartDBInstanceResult")) break;
            },
            else => {},
        }
    }

    var result: StartDBInstanceOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
