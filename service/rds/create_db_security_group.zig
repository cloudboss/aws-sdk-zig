...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDBSecurityGroupInput, options: Options) !CreateDBSecurityGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDBSecurityGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateDBSecurityGroup&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&DBSecurityGroupDescription=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_security_group_description);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDBSecurityGroupOutput {
    _ = status;
    _ = headers;
