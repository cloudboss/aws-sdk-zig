...
pub const DescribeDBSecurityGroupsInput = struct {
...
pub const DescribeDBSecurityGroupsOutput = struct {
    /// A list of `DBSecurityGroup` instances.
    db_security_groups: ?[]const DBSecurityGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBSecurityGroupsInput, options: CallOptions) !DescribeDBSecurityGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDBSecurityGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDBSecurityGroups&Version=2014-10-31");
    if (input.db_security_group_name) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDBSecurityGroupsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBSecurityGroupsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBSecurityGroupsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBSecurityGroups")) {
                    result.db_security_groups = try serde.deserializeDBSecurityGroups(allocator, &reader, "DBSecurityGroup");
