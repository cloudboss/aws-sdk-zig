...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBSecurityGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBSecurityGroups&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBSecurityGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBSecurityGroups")) {
                    result.db_security_groups = try serde.deserializeDBSecurityGroups(&reader, alloc, "DBSecurityGroup");
