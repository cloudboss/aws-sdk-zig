...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBProxyEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBProxyEndpoints&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBProxyEndpointsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBProxyEndpoints")) {
                    result.db_proxy_endpoints = try serde.deserializeDBProxyEndpointList(&reader, alloc, "member");
