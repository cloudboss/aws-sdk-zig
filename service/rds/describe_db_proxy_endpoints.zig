...
pub const DescribeDBProxyEndpointsInput = struct {
...
pub const DescribeDBProxyEndpointsOutput = struct {
    /// The list of `ProxyEndpoint` objects returned by the API operation.
    db_proxy_endpoints: ?[]const DBProxyEndpoint = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBProxyEndpointsInput, options: CallOptions) !DescribeDBProxyEndpointsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDBProxyEndpointsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDBProxyEndpoints&Version=2014-10-31");
    if (input.db_proxy_endpoint_name) |v| {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDBProxyEndpointsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeDBProxyEndpointsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeDBProxyEndpointsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBProxyEndpoints")) {
                    result.db_proxy_endpoints = try serde.deserializeDBProxyEndpointList(allocator, &reader, "member");
