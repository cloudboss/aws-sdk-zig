...
pub const ModifyDBShardGroupOutput = struct {
    /// Specifies whether to create standby DB shard groups for the DB shard group.
    /// Valid values are the following:
    ///
    /// * 0 - Creates a DB shard group without a standby DB shard group. This is the
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBShardGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBShardGroup&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBShardGroupOutput {
    _ = status;
    _ = headers;
