...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DownloadDBLogFilePortionInput, options: Options) !DownloadDBLogFilePortionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DownloadDBLogFilePortionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DownloadDBLogFilePortion&Version=2014-10-31");
    ...
    try body_buf.appendSlice(alloc, "&LogFileName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.log_file_name);
    if (input.marker) |v| {
        try body_buf.appendSlice(alloc, "&Marker=");
        try aws.url.appendUrlEncoded(alloc, &body_buf, v);
    }
    if (input.number_of_lines) |v| {
        try body_buf.appendSlice(alloc, "&NumberOfLines=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DownloadDBLogFilePortionOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "AdditionalDataPending")) {
                    ...
                    result.log_file_data = try alloc.dupe(u8, try reader.readElementText());
