...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ModifyDBProxyInput, options: Options) !ModifyDBProxyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ModifyDBProxyInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=ModifyDBProxy&Version=2014-10-31");
    if (input.auth) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            {
    ...
    if (input.new_db_proxy_name) |v| {
        try body_buf.appendSlice(alloc, "&NewDBProxyName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ModifyDBProxyOutput {
    _ = status;
    _ = headers;
