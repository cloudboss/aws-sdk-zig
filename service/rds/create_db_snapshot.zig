...
pub const CreateDBSnapshotInput = struct {
...
pub const CreateDBSnapshotOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDBSnapshotInput, options: Options) !CreateDBSnapshotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDBSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=CreateDBSnapshot&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDBSnapshotOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "CreateDBSnapshotResult")) break;
            },
            else => {},
        }
    }

    var result: CreateDBSnapshotOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
