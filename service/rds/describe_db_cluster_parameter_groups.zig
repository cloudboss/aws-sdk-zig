...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBClusterParameterGroupsInput, options: CallOptions) !DescribeDBClusterParameterGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDBClusterParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DescribeDBClusterParameterGroups&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDBClusterParameterGroupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterParameterGroups")) {
                    result.db_cluster_parameter_groups = try serde.deserializeDBClusterParameterGroupList(allocator, &reader, "DBClusterParameterGroup");
