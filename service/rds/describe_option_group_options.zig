...
pub const DescribeOptionGroupOptionsInput = struct {
...
pub const DescribeOptionGroupOptionsOutput = struct {
    /// An optional pagination token provided by a previous request. If this
    /// parameter is specified, the response includes only records beyond the
    /// marker, up to the value specified by `MaxRecords`.
    marker: ?[]const u8 = null,

    option_group_options: ?[]const OptionGroupOption = null,
};
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeOptionGroupOptionsInput, options: Options) !DescribeOptionGroupOptionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeOptionGroupOptionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeOptionGroupOptions&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&EngineName=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.engine_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeOptionGroupOptionsOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DescribeOptionGroupOptionsResult")) break;
            },
            else => {},
        }
    }

    var result: DescribeOptionGroupOptionsOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "Marker")) {
                    result.marker = try alloc.dupe(u8, try reader.readElementText());
                } else if (std.mem.eql(u8, e.local, "OptionGroupOptions")) {
                    result.option_group_options = try serde.deserializeOptionGroupOptionsList(&reader, alloc, "OptionGroupOption");
                } else {
