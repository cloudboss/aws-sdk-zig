...
pub const SwitchoverReadReplicaInput = struct {
...
pub const SwitchoverReadReplicaOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SwitchoverReadReplicaInput, options: Options) !SwitchoverReadReplicaOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SwitchoverReadReplicaInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=SwitchoverReadReplica&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SwitchoverReadReplicaOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "SwitchoverReadReplicaResult")) break;
            },
            else => {},
        }
    }

    var result: SwitchoverReadReplicaOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
