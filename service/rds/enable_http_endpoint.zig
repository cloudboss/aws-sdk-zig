...
pub const EnableHttpEndpointInput = struct {
    /// The Amazon Resource Name (ARN) of the DB cluster.
    resource_arn: []const u8,
};

pub const EnableHttpEndpointOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableHttpEndpointInput, options: CallOptions) !EnableHttpEndpointOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableHttpEndpointInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=EnableHttpEndpoint&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableHttpEndpointOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "EnableHttpEndpointResult")) break;
            },
            else => {},
        }
    }

    var result: EnableHttpEndpointOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
