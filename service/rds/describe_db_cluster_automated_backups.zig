...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDBClusterAutomatedBackupsInput, options: Options) !DescribeDBClusterAutomatedBackupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDBClusterAutomatedBackupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DescribeDBClusterAutomatedBackups&Version=2014-10-31");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDBClusterAutomatedBackupsOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterAutomatedBackups")) {
                    result.db_cluster_automated_backups = try serde.deserializeDBClusterAutomatedBackupList(&reader, alloc, "DBClusterAutomatedBackup");
