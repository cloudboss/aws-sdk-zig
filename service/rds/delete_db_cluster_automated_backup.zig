...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDBClusterAutomatedBackupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "Action=DeleteDBClusterAutomatedBackup&Version=2014-10-31");
    try body_buf.appendSlice(alloc, "&DbClusterResourceId=");
    try aws.url.appendUrlEncoded(alloc, &body_buf, input.db_cluster_resource_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDBClusterAutomatedBackupOutput {
    _ = status;
    _ = headers;
        ...
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterAutomatedBackup")) {
                    result.db_cluster_automated_backup = try serde.deserializeDBClusterAutomatedBackup(&reader, alloc);
                } else {
