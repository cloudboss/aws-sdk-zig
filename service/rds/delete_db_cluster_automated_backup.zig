...
pub const DeleteDBClusterAutomatedBackupInput = struct {
...
pub const DeleteDBClusterAutomatedBackupOutput = struct {
    db_cluster_automated_backup: ?DBClusterAutomatedBackup = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBClusterAutomatedBackupInput, options: CallOptions) !DeleteDBClusterAutomatedBackupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDBClusterAutomatedBackupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteDBClusterAutomatedBackup&Version=2014-10-31");
    try body_buf.appendSlice(allocator, "&DbClusterResourceId=");
    try aws.url.appendUrlEncoded(allocator, &body_buf, input.db_cluster_resource_id);
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDBClusterAutomatedBackupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteDBClusterAutomatedBackupResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteDBClusterAutomatedBackupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DBClusterAutomatedBackup")) {
                    result.db_cluster_automated_backup = try serde.deserializeDBClusterAutomatedBackup(allocator, &reader);
                } else {
