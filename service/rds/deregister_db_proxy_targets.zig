...
pub const DeregisterDBProxyTargetsInput = struct {
    /// One or more DB cluster identifiers.
    db_cluster_identifiers: ?[]const []const u8 = null,

    /// One or more DB instance identifiers.
    db_instance_identifiers: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterDBProxyTargetsInput, options: Options) !DeregisterDBProxyTargetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterDBProxyTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeregisterDBProxyTargets&Version=2014-10-31");
    if (input.db_cluster_identifiers) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&DBClusterIdentifiers.member.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item);
        }
    }
    if (input.db_instance_identifiers) |list| {
        for (list, 0..) |item, idx| {
            const n = idx + 1;
            var prefix_buf: [256]u8 = undefined;
            const field_prefix = std.fmt.bufPrint(&prefix_buf, "&DBInstanceIdentifiers.member.{d}=", .{n}) catch continue;
            try body_buf.appendSlice(allocator, field_prefix);
            try aws.url.appendUrlEncoded(allocator, &body_buf, item);
        }
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterDBProxyTargetsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
    const result: DeregisterDBProxyTargetsOutput = .{};
