...
pub const DeleteDBShardGroupInput = struct {
    /// The name of the DB shard group to delete.
    db_shard_group_identifier: []const u8,
};

pub const DeleteDBShardGroupOutput = @import("db_shard_group.zig").DBShardGroup;

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDBShardGroupInput, options: CallOptions) !DeleteDBShardGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteDBShardGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(allocator, "Action=DeleteDBShardGroup&Version=2014-10-31");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDBShardGroupOutput {
    _ = status;
    _ = headers;
    ...
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
                if (std.mem.eql(u8, e.local, "DeleteDBShardGroupResult")) break;
            },
            else => {},
        }
    }

    var result: DeleteDBShardGroupOutput = .{};
    while (try reader.next()) |event| {
        switch (event) {
            .element_start => |e| {
