...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterScalableTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AnyScaleFrontendService.DeregisterScalableTarget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterScalableTargetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
