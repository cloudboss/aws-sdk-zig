...
pub const DeregisterScalableTargetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterScalableTargetInput, options: Options) !DeregisterScalableTargetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterScalableTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AnyScaleFrontendService.DeregisterScalableTarget");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterScalableTargetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
