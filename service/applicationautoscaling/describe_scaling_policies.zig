...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeScalingPoliciesInput, options: Options) !DescribeScalingPoliciesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScalingPoliciesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AnyScaleFrontendService.DescribeScalingPolicies");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeScalingPoliciesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeScalingPoliciesOutput, body, alloc);
}
