...
const ScalableTarget = @import("scalable_target.zig").ScalableTarget;

pub const DescribeScalableTargetsInput = struct {
...
pub const DescribeScalableTargetsOutput = struct {
    /// The token required to get the next set of results. This value is `null` if
    /// there are no more results to return.
    next_token: ?[]const u8 = null,

    /// The scalable targets that match the request parameters.
    scalable_targets: ?[]const ScalableTarget = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeScalableTargetsInput, options: CallOptions) !DescribeScalableTargetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeScalableTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AnyScaleFrontendService.DescribeScalableTargets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeScalableTargetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeScalableTargetsOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "FailedResourceAccessException")) {
        const parsed_error: ?errors.FailedResourceAccessException = aws.json.parseJsonObject(errors.FailedResourceAccessException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
