...
/// for one
/// metric only, or whether it is performing a math expression on the values of
/// returned metric
/// statistics to create a new time series. A time series is a series of data
/// points, each of
...
/// math](https://docs.aws.amazon.com/autoscaling/application/userguide/application-auto-scaling-target-tracking-metric-math.html) in the
/// *Application Auto Scaling User Guide*.
pub const TargetTrackingMetricDataQuery = struct {
    /// The math expression to perform on the returned data, if this object is
    /// performing a math
    /// expression. This expression can use the `Id` of the other metrics to refer
    /// to
    /// those metrics, and can also use the `Id` of other expressions to use the
    /// result
