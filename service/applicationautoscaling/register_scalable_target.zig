...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterScalableTargetInput, options: Options) !RegisterScalableTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterScalableTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AnyScaleFrontendService.RegisterScalableTarget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterScalableTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterScalableTargetOutput, body, alloc);
}
