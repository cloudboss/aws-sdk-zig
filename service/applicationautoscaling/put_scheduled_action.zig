...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutScheduledActionInput, options: Options) !PutScheduledActionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutScheduledActionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AnyScaleFrontendService.PutScheduledAction");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutScheduledActionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
