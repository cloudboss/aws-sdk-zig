...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeScalingActivitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AnyScaleFrontendService.DescribeScalingActivities");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeScalingActivitiesOutput {
    ...
    return aws.json.parseJsonObject(DescribeScalingActivitiesOutput, body, alloc);
}
