...
const ScalingActivity = @import("scaling_activity.zig").ScalingActivity;
...
pub const DescribeScalingActivitiesInput = struct {
    ...
    /// aren't
    /// completed or started for various reasons, such as preventing infinite
    /// scaling loops. For
    /// help interpreting the not scaled reason details in the response, see
    /// [Scaling activities for Application Auto
    /// Scaling](https://docs.aws.amazon.com/autoscaling/application/userguide/application-auto-scaling-scaling-activities.html).
    include_not_scaled_activities: ?bool = null,
...
pub const DescribeScalingActivitiesOutput = struct {
    ...
    scaling_activities: ?[]const ScalingActivity = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeScalingActivitiesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AnyScaleFrontendService.DescribeScalingActivities");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeScalingActivitiesOutput {
    ...
    return aws.json.parseJsonObject(DescribeScalingActivitiesOutput, body, allocator);
}
