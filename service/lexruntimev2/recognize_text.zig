...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RecognizeTextInput, options: Options) !RecognizeTextOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RecognizeTextInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RecognizeTextOutput {
    var result: RecognizeTextOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RecognizeTextOutput, body, alloc);
    }
    _ = status;
    _ = headers;
