...
fn serializeRequest(allocator: std.mem.Allocator, input: RecognizeUtteranceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/utterance");
    ...
    try request.headers.put(allocator, "Content-Type", input.request_content_type);
    if (input.response_content_type) |v| {
        try request.headers.put(allocator, "Response-Content-Type", v);
    }
    if (input.session_state) |v| {
        try request.headers.put(allocator, "x-amz-lex-session-state", v);
    }
...
fn deserializeStreamingResponse(allocator: std.mem.Allocator, stream_resp: *aws.http.StreamingResponse) !RecognizeUtteranceOutput {
    ...
    if (stream_resp.headers.get("x-amz-lex-input-mode")) |value| {
        result.input_mode = try allocator.dupe(u8, value);
    }
    ...
    if (stream_resp.headers.get("x-amz-lex-interpretations")) |value| {
        result.interpretations = try allocator.dupe(u8, value);
    }
    ...
    if (stream_resp.headers.get("x-amz-lex-recognized-bot-member")) |value| {
        result.recognized_bot_member = try allocator.dupe(u8, value);
    }
