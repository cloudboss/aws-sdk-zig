...
pub const CreateParameterGroupInput = struct {
...
pub const CreateParameterGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateParameterGroupInput, options: Options) !CreateParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.CreateParameterGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateParameterGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateParameterGroupOutput, body, alloc);
}
