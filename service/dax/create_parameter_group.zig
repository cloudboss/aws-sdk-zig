...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateParameterGroupInput, options: CallOptions) !CreateParameterGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDAXV3.CreateParameterGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateParameterGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateParameterGroupOutput, body, allocator);
}
