...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSubnetGroupInput, options: CallOptions) !DeleteSubnetGroupOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "dax");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonDAXV3.DeleteSubnetGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSubnetGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteSubnetGroupOutput, body, allocator);
}
