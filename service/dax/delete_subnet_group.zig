...
pub const DeleteSubnetGroupInput = struct {
...
pub const DeleteSubnetGroupOutput = struct {
    /// A user-specified message for this action (i.e., a reason for deleting the
    /// subnet
    /// group).
    deletion_message: ?[]const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSubnetGroupInput, options: Options) !DeleteSubnetGroupOutput {
    ...
    try aws.signing.signRequest(alloc, &request, creds, client.config.region, "dax");
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("dax", "DAX", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.DeleteSubnetGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSubnetGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteSubnetGroupOutput, body, alloc);
}
...
fn parseErrorResponse(body: []const u8, status: u16, alloc: std.mem.Allocator) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "NodeNotFoundFault")) {
        return .{ .arena = arena, .kind = .{ .node_not_found_fault = .{
            .message = owned_message,
            .request_id = owned_request_id,
        } } };
    }
