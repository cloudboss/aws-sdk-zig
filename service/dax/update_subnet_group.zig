...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSubnetGroupInput, options: Options) !UpdateSubnetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.UpdateSubnetGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSubnetGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateSubnetGroupOutput, body, alloc);
}
