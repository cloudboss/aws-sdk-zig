...
pub const UpdateParameterGroupInput = struct {
    ...
    /// supported parameter names. For more details, see [Configuring TTL
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateParameterGroupInput, options: Options) !UpdateParameterGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateParameterGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.UpdateParameterGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateParameterGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateParameterGroupOutput, body, alloc);
}
