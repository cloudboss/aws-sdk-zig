...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSubnetGroupInput, options: Options) !CreateSubnetGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSubnetGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.CreateSubnetGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSubnetGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateSubnetGroupOutput, body, alloc);
}
