...
pub const DescribeSubnetGroupsInput = struct {
    ...
    subnet_group_names: ?[]const []const u8 = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeSubnetGroupsInput, options: Options) !DescribeSubnetGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeSubnetGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.DescribeSubnetGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeSubnetGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeSubnetGroupsOutput, body, alloc);
}
