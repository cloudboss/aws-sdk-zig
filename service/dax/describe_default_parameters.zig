...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDefaultParametersInput, options: Options) !DescribeDefaultParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeDefaultParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.DescribeDefaultParameters");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeDefaultParametersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeDefaultParametersOutput, body, alloc);
}
