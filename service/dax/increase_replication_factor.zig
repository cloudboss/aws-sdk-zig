...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: IncreaseReplicationFactorInput, options: Options) !IncreaseReplicationFactorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: IncreaseReplicationFactorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.IncreaseReplicationFactor");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !IncreaseReplicationFactorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(IncreaseReplicationFactorOutput, body, alloc);
}
