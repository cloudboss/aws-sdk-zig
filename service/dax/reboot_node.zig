...
pub const RebootNodeInput = struct {
...
pub const RebootNodeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RebootNodeInput, options: Options) !RebootNodeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RebootNodeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.RebootNode");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RebootNodeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RebootNodeOutput, body, alloc);
}
