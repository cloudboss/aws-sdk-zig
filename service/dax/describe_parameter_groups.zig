...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeParameterGroupsInput, options: Options) !DescribeParameterGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeParameterGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.DescribeParameterGroups");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeParameterGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeParameterGroupsOutput, body, alloc);
}
