...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DecreaseReplicationFactorInput, options: Options) !DecreaseReplicationFactorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DecreaseReplicationFactorInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.DecreaseReplicationFactor");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DecreaseReplicationFactorOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DecreaseReplicationFactorOutput, body, alloc);
}
