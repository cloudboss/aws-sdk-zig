...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeParametersInput, options: Options) !DescribeParametersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeParametersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonDAXV3.DescribeParameters");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeParametersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeParametersOutput, body, alloc);
}
