...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentTemplatesInput, options: Options) !ListEnvironmentTemplatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnvironmentTemplatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.ListEnvironmentTemplates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnvironmentTemplatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEnvironmentTemplatesOutput, body, alloc);
}
