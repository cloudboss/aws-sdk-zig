...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceInstanceInput, options: Options) !UpdateServiceInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.UpdateServiceInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceInstanceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateServiceInstanceOutput, body, alloc);
}
