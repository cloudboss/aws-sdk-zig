...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateServiceTemplateVersionInput, options: Options) !CreateServiceTemplateVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceTemplateVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.CreateServiceTemplateVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateServiceTemplateVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateServiceTemplateVersionOutput, body, alloc);
}
