...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTemplateSyncConfigInput, options: Options) !UpdateTemplateSyncConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTemplateSyncConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.UpdateTemplateSyncConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTemplateSyncConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateTemplateSyncConfigOutput, body, alloc);
}
