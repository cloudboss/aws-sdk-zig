...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateServiceInstanceInput, options: Options) !CreateServiceInstanceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceInstanceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.CreateServiceInstance");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateServiceInstanceOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateServiceInstanceOutput, body, alloc);
}
