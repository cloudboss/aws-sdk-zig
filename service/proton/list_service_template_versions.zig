...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceTemplateVersionsInput, options: Options) !ListServiceTemplateVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServiceTemplateVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.ListServiceTemplateVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServiceTemplateVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListServiceTemplateVersionsOutput, body, alloc);
}
