...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEnvironmentTemplateVersionInput, options: Options) !CreateEnvironmentTemplateVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEnvironmentTemplateVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.CreateEnvironmentTemplateVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEnvironmentTemplateVersionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateEnvironmentTemplateVersionOutput, body, alloc);
}
