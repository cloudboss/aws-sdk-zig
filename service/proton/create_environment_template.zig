...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEnvironmentTemplateInput, options: Options) !CreateEnvironmentTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEnvironmentTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.CreateEnvironmentTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEnvironmentTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEnvironmentTemplateOutput, body, alloc);
}
