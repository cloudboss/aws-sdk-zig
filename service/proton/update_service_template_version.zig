...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceTemplateVersionInput, options: Options) !UpdateServiceTemplateVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceTemplateVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.UpdateServiceTemplateVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceTemplateVersionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(UpdateServiceTemplateVersionOutput, body, alloc);
}
