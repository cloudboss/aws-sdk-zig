...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceTemplateVersionInput, options: Options) !GetServiceTemplateVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceTemplateVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.GetServiceTemplateVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceTemplateVersionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetServiceTemplateVersionOutput, body, alloc);
}
