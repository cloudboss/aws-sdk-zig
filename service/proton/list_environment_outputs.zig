...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentOutputsInput, options: Options) !ListEnvironmentOutputsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnvironmentOutputsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.ListEnvironmentOutputs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnvironmentOutputsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEnvironmentOutputsOutput, body, alloc);
}
