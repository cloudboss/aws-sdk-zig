...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceSyncBlockerInput, options: Options) !UpdateServiceSyncBlockerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceSyncBlockerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.UpdateServiceSyncBlocker");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceSyncBlockerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateServiceSyncBlockerOutput, body, alloc);
}
