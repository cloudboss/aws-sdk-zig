...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEnvironmentAccountConnectionInput, options: CallOptions) !CreateEnvironmentAccountConnectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateEnvironmentAccountConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AwsProton20200720.CreateEnvironmentAccountConnection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateEnvironmentAccountConnectionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateEnvironmentAccountConnectionOutput, body, allocator);
}
