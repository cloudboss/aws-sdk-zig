...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetServiceSyncBlockerSummaryInput, options: Options) !GetServiceSyncBlockerSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetServiceSyncBlockerSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.GetServiceSyncBlockerSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetServiceSyncBlockerSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetServiceSyncBlockerSummaryOutput, body, alloc);
}
