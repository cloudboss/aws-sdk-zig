...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetEnvironmentTemplateVersionInput, options: Options) !GetEnvironmentTemplateVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetEnvironmentTemplateVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.GetEnvironmentTemplateVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetEnvironmentTemplateVersionOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(GetEnvironmentTemplateVersionOutput, body, alloc);
}
