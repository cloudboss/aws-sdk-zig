...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateServiceTemplateInput, options: Options) !CreateServiceTemplateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateServiceTemplateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.CreateServiceTemplate");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateServiceTemplateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateServiceTemplateOutput, body, alloc);
}
