...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListServiceInstanceProvisionedResourcesInput, options: Options) !ListServiceInstanceProvisionedResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListServiceInstanceProvisionedResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.ListServiceInstanceProvisionedResources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListServiceInstanceProvisionedResourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListServiceInstanceProvisionedResourcesOutput, body, alloc);
}
