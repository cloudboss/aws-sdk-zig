...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateServiceSyncConfigInput, options: Options) !UpdateServiceSyncConfigOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateServiceSyncConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.UpdateServiceSyncConfig");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateServiceSyncConfigOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateServiceSyncConfigOutput, body, alloc);
}
