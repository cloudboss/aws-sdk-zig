...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEnvironmentAccountConnectionsInput, options: Options) !ListEnvironmentAccountConnectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEnvironmentAccountConnectionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AwsProton20200720.ListEnvironmentAccountConnections");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEnvironmentAccountConnectionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEnvironmentAccountConnectionsOutput, body, alloc);
}
