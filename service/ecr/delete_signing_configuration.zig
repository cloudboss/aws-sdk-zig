...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSigningConfigurationInput, options: Options) !DeleteSigningConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteSigningConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DeleteSigningConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteSigningConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteSigningConfigurationOutput, body, alloc);
}
