...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSigningConfigurationInput, options: Options) !GetSigningConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSigningConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.GetSigningConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSigningConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSigningConfigurationOutput, body, alloc);
}
