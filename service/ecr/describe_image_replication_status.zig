...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageReplicationStatusInput, options: CallOptions) !DescribeImageReplicationStatusOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeImageReplicationStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DescribeImageReplicationStatus");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeImageReplicationStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeImageReplicationStatusOutput, body, allocator);
}
