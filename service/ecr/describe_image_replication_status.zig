...
const ImageReplicationStatus = @import("image_replication_status.zig").ImageReplicationStatus;

pub const DescribeImageReplicationStatusInput = struct {
...
pub const DescribeImageReplicationStatusOutput = struct {
    image_id: ?ImageIdentifier = null,

    /// The replication status details for the images in the specified repository.
    replication_statuses: ?[]const ImageReplicationStatus = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeImageReplicationStatusInput, options: Options) !DescribeImageReplicationStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeImageReplicationStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DescribeImageReplicationStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeImageReplicationStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeImageReplicationStatusOutput, body, alloc);
}
