...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutImageTagMutabilityInput, options: Options) !PutImageTagMutabilityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutImageTagMutabilityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.PutImageTagMutability");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutImageTagMutabilityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutImageTagMutabilityOutput, body, alloc);
}
