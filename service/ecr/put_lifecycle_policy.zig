...
pub const PutLifecyclePolicyInput = struct {
    ...
    /// contains the repository. If you
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutLifecyclePolicyInput, options: Options) !PutLifecyclePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutLifecyclePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.PutLifecyclePolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutLifecyclePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutLifecyclePolicyOutput, body, alloc);
}
