...
/// encryption type is used, Amazon ECR uses server-side encryption with Amazon
/// S3-managed encryption
/// keys which encrypts your data at rest using an AES256 encryption algorithm.
/// This does
/// not require any action on your part.
///
/// For more control over the encryption of the contents of your repository, you
/// can use
/// server-side encryption with Key Management Service key stored in Key
/// Management Service (KMS) to encrypt your
...
pub const EncryptionConfiguration = struct {
    ...
    /// will be encrypted with two layers of encryption using server-side encryption
    /// with the
    /// KMS Management Service key stored in KMS. Similar to the `KMS` encryption
    /// type, you can either use the default Amazon Web Services managed KMS key for
    /// Amazon ECR, or specify your
    ...
    /// encryption. The alias, key ID, or full ARN of the KMS key can be specified.
    /// The key
