...
pub const PutRegistryPolicyInput = struct {
...
pub const PutRegistryPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRegistryPolicyInput, options: CallOptions) !PutRegistryPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutRegistryPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.PutRegistryPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutRegistryPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutRegistryPolicyOutput, body, allocator);
}
