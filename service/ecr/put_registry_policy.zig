...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRegistryPolicyInput, options: Options) !PutRegistryPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRegistryPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.PutRegistryPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRegistryPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutRegistryPolicyOutput, body, alloc);
}
