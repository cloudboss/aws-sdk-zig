...
pub const GetRegistryScanningConfigurationOutput = struct {
    ...
    scanning_configuration: ?RegistryScanningConfiguration = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegistryScanningConfigurationInput, options: CallOptions) !GetRegistryScanningConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRegistryScanningConfigurationInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.GetRegistryScanningConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRegistryScanningConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRegistryScanningConfigurationOutput, body, allocator);
}
