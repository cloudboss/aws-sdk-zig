...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterPullTimeUpdateExclusionInput, options: Options) !DeregisterPullTimeUpdateExclusionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterPullTimeUpdateExclusionInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("ecr", "ECR", alloc);
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DeregisterPullTimeUpdateExclusion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterPullTimeUpdateExclusionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeregisterPullTimeUpdateExclusionOutput, body, alloc);
}
