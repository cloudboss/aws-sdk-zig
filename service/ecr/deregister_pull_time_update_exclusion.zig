...
pub const DeregisterPullTimeUpdateExclusionInput = struct {
...
pub const DeregisterPullTimeUpdateExclusionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterPullTimeUpdateExclusionInput, options: CallOptions) !DeregisterPullTimeUpdateExclusionOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "ecr");
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterPullTimeUpdateExclusionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DeregisterPullTimeUpdateExclusion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterPullTimeUpdateExclusionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeregisterPullTimeUpdateExclusionOutput, body, allocator);
}
