...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRegistryPolicyInput, options: CallOptions) !DeleteRegistryPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRegistryPolicyInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DeleteRegistryPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRegistryPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRegistryPolicyOutput, body, allocator);
}
