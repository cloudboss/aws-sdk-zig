...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRegistryPolicyInput, options: CallOptions) !GetRegistryPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRegistryPolicyInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.GetRegistryPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRegistryPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRegistryPolicyOutput, body, allocator);
}
