...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartImageScanInput, options: Options) !StartImageScanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartImageScanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.StartImageScan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartImageScanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartImageScanOutput, body, alloc);
}
