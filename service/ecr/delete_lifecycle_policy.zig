...
pub const DeleteLifecyclePolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLifecyclePolicyInput, options: Options) !DeleteLifecyclePolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteLifecyclePolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DeleteLifecyclePolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteLifecyclePolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteLifecyclePolicyOutput, body, alloc);
}
