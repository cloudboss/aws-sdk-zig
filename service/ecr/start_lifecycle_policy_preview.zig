...
pub const StartLifecyclePolicyPreviewInput = struct {
...
pub const StartLifecyclePolicyPreviewOutput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartLifecyclePolicyPreviewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.StartLifecyclePolicyPreview");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartLifecyclePolicyPreviewOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartLifecyclePolicyPreviewOutput, body, allocator);
}
