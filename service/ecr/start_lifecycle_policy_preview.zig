...
fn serializeRequest(alloc: std.mem.Allocator, input: StartLifecyclePolicyPreviewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.StartLifecyclePolicyPreview");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartLifecyclePolicyPreviewOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartLifecyclePolicyPreviewOutput, body, alloc);
}
