...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRepositoryInput, options: Options) !DeleteRepositoryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRepositoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DeleteRepository");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRepositoryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRepositoryOutput, body, alloc);
}
