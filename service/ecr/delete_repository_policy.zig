...
pub const DeleteRepositoryPolicyInput = struct {
    /// The Amazon Web Services account ID associated with the registry that
    /// contains the repository policy
...
pub const DeleteRepositoryPolicyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRepositoryPolicyInput, options: CallOptions) !DeleteRepositoryPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRepositoryPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DeleteRepositoryPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRepositoryPolicyOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRepositoryPolicyOutput, body, allocator);
}
