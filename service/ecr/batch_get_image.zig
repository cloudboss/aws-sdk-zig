...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchGetImageInput, options: Options) !BatchGetImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchGetImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.BatchGetImage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchGetImageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(BatchGetImageOutput, body, alloc);
}
