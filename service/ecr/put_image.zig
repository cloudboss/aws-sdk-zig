...
pub const PutImageInput = struct {
    ...
    /// which to put the image. If you do not specify a registry, the default
    /// registry is assumed.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutImageInput, options: Options) !PutImageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutImageInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.PutImage");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutImageOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutImageOutput, body, alloc);
}
