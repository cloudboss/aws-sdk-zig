...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRegistryInput, options: CallOptions) !DescribeRegistryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeRegistryInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.DescribeRegistry");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeRegistryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRegistryOutput, body, allocator);
}
