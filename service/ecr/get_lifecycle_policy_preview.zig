...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLifecyclePolicyPreviewInput, options: Options) !GetLifecyclePolicyPreviewOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLifecyclePolicyPreviewInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.GetLifecyclePolicyPreview");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLifecyclePolicyPreviewOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLifecyclePolicyPreviewOutput, body, alloc);
}
