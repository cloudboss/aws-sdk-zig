...
pub const ListPullTimeUpdateExclusionsInput = struct {
    /// The maximum number of pull time update exclusion results returned by
    /// `ListPullTimeUpdateExclusions` in
    /// paginated output. When this parameter is used,
    /// `ListPullTimeUpdateExclusions` only returns
    /// `maxResults` results in a single page along with a `nextToken`
    /// response element. The remaining results of the initial request can be seen
    /// by sending
    /// another `ListPullTimeUpdateExclusions` request with the returned `nextToken`
    /// value.
    /// This value can be between 1 and 1000. If this parameter is
    /// not used, then `ListPullTimeUpdateExclusions` returns up to 100 results and
    /// a
...
pub const ListPullTimeUpdateExclusionsOutput = struct {
    ...
    /// pull times recorded.
    pull_time_update_exclusions: ?[]const []const u8 = null,

    pub const json_field_names = .{
        .next_token = "nextToken",
        .pull_time_update_exclusions = "pullTimeUpdateExclusions",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPullTimeUpdateExclusionsInput, options: CallOptions) !ListPullTimeUpdateExclusionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPullTimeUpdateExclusionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.ListPullTimeUpdateExclusions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPullTimeUpdateExclusionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPullTimeUpdateExclusionsOutput, body, allocator);
}
