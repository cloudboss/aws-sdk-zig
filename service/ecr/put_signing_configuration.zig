...
pub const PutSigningConfigurationInput = struct {
...
pub const PutSigningConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutSigningConfigurationInput, options: CallOptions) !PutSigningConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutSigningConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.PutSigningConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutSigningConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutSigningConfigurationOutput, body, allocator);
}
