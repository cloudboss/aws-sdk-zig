...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePullThroughCacheRuleInput, options: Options) !CreatePullThroughCacheRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePullThroughCacheRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.CreatePullThroughCacheRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePullThroughCacheRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePullThroughCacheRuleOutput, body, alloc);
}
