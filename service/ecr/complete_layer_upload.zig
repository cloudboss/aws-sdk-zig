...
pub const CompleteLayerUploadInput = struct {
...
pub const CompleteLayerUploadOutput = struct {
    /// The `sha256` digest of the image layer.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CompleteLayerUploadInput, options: CallOptions) !CompleteLayerUploadOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CompleteLayerUploadInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AmazonEC2ContainerRegistry_V20150921.CompleteLayerUpload");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CompleteLayerUploadOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CompleteLayerUploadOutput, body, allocator);
}
