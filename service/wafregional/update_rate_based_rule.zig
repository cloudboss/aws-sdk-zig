...
pub const UpdateRateBasedRuleInput = struct {
    /// The value returned by the most recent call to GetChangeToken.
    change_token: []const u8,

    /// The maximum number of requests, which have an identical value in the field
    /// specified by the `RateKey`, allowed in a
    /// five-minute period. If the number of requests exceeds the `RateLimit` and
    /// the other
...
pub const UpdateRateBasedRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRateBasedRuleInput, options: CallOptions) !UpdateRateBasedRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRateBasedRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.UpdateRateBasedRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRateBasedRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateRateBasedRuleOutput, body, allocator);
}
