...
pub const GetRuleGroupInput = struct {
...
pub const GetRuleGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRuleGroupInput, options: CallOptions) !GetRuleGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRuleGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.GetRuleGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRuleGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRuleGroupOutput, body, allocator);
}
