...
pub const DeleteRateBasedRuleInput = struct {
...
pub const DeleteRateBasedRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRateBasedRuleInput, options: CallOptions) !DeleteRateBasedRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRateBasedRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.DeleteRateBasedRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRateBasedRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRateBasedRuleOutput, body, allocator);
}
