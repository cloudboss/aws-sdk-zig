...
pub const GetRateBasedRuleInput = struct {
...
pub const GetRateBasedRuleOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetRateBasedRuleInput, options: CallOptions) !GetRateBasedRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetRateBasedRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.GetRateBasedRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetRateBasedRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetRateBasedRuleOutput, body, allocator);
}
