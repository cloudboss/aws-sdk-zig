...
pub const DeleteRegexMatchSetInput = struct {
...
pub const DeleteRegexMatchSetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRegexMatchSetInput, options: CallOptions) !DeleteRegexMatchSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteRegexMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.DeleteRegexMatchSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteRegexMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRegexMatchSetOutput, body, allocator);
}
