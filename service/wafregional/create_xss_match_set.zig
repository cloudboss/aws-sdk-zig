...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateXssMatchSetInput, options: Options) !CreateXssMatchSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateXssMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.CreateXssMatchSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateXssMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateXssMatchSetOutput, body, alloc);
}
