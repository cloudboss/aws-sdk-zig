...
const ByteMatchSetSummary = @import("byte_match_set_summary.zig").ByteMatchSetSummary;

pub const ListByteMatchSetsInput = struct {
    /// Specifies the number of `ByteMatchSet` objects that you want AWS WAF to
    /// return for this request. If you have more
    /// `ByteMatchSets` objects than the number you specify for `Limit`, the
    /// response includes a
    ...
    /// For the second and subsequent `ListByteMatchSets` requests, specify the
    /// value of `NextMarker`
...
pub const ListByteMatchSetsOutput = struct {
    /// An array of ByteMatchSetSummary objects.
    byte_match_sets: ?[]const ByteMatchSetSummary = null,

    /// If you have more `ByteMatchSet` objects than the number that you specified
    /// for `Limit` in the request,
    /// the response includes a `NextMarker` value. To list more `ByteMatchSet`
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListByteMatchSetsInput, options: CallOptions) !ListByteMatchSetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListByteMatchSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.ListByteMatchSets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListByteMatchSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListByteMatchSetsOutput, body, allocator);
}
