...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteRuleGroupInput, options: Options) !DeleteRuleGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteRuleGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.DeleteRuleGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteRuleGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteRuleGroupOutput, body, alloc);
}
