...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutPermissionPolicyInput, options: Options) !PutPermissionPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutPermissionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.PutPermissionPolicy");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutPermissionPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
