...
pub const DeletePermissionPolicyInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeletePermissionPolicyInput, options: Options) !DeletePermissionPolicyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePermissionPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.DeletePermissionPolicy");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePermissionPolicyOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
