...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListWebACLsInput, options: Options) !ListWebACLsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListWebACLsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.ListWebACLs");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListWebACLsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListWebACLsOutput, body, alloc);
}
