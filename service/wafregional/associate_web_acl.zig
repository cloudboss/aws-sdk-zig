...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateWebACLInput, options: Options) !AssociateWebACLOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateWebACLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.AssociateWebACL");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateWebACLOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
