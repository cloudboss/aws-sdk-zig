...
pub const ListRegexPatternSetsInput = struct {
    ...
    /// another group of `RegexPatternSet` objects.
...
pub const ListRegexPatternSetsOutput = struct {
    /// If you have more `RegexPatternSet` objects than the number that you
    /// specified for `Limit` in the request,
    /// the response includes a `NextMarker` value. To list more `RegexPatternSet`
    ...
    regex_pattern_sets: ?[]const RegexPatternSetSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRegexPatternSetsInput, options: CallOptions) !ListRegexPatternSetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRegexPatternSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.ListRegexPatternSets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListRegexPatternSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRegexPatternSetsOutput, body, allocator);
}
