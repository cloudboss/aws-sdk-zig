...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateWebACLInput, options: Options) !UpdateWebACLOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateWebACLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.UpdateWebACL");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateWebACLOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateWebACLOutput, body, alloc);
}
