...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetByteMatchSetInput, options: Options) !GetByteMatchSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetByteMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.GetByteMatchSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetByteMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetByteMatchSetOutput, body, alloc);
}
