...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetChangeTokenInput, options: CallOptions) !GetChangeTokenOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetChangeTokenInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.GetChangeToken");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetChangeTokenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetChangeTokenOutput, body, allocator);
}
