...
pub const CreateByteMatchSetInput = struct {
...
pub const CreateByteMatchSetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateByteMatchSetInput, options: CallOptions) !CreateByteMatchSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateByteMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.CreateByteMatchSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateByteMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateByteMatchSetOutput, body, allocator);
}
