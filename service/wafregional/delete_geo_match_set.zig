...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGeoMatchSetInput, options: CallOptions) !DeleteGeoMatchSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteGeoMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.DeleteGeoMatchSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteGeoMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteGeoMatchSetOutput, body, allocator);
}
