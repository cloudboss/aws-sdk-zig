...
const XssMatchSetSummary = @import("xss_match_set_summary.zig").XssMatchSetSummary;

pub const ListXssMatchSetsInput = struct {
    /// Specifies the number of XssMatchSet objects that you want AWS WAF to return
    /// for this request. If you have more
    /// `XssMatchSet` objects than the number you specify for `Limit`, the response
    /// includes a
    ...
    /// `XssMatchSets`. For the second and subsequent `ListXssMatchSets` requests,
    /// specify the
...
pub const ListXssMatchSetsOutput = struct {
    /// If you have more XssMatchSet objects than the number that you specified for
    /// `Limit` in the request,
    /// the response includes a `NextMarker` value. To list more `XssMatchSet`
    ...
    xss_match_sets: ?[]const XssMatchSetSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListXssMatchSetsInput, options: CallOptions) !ListXssMatchSetsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "waf-regional");
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListXssMatchSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.ListXssMatchSets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListXssMatchSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListXssMatchSetsOutput, body, allocator);
}
