...
pub const DeleteSizeConstraintSetInput = struct {
...
pub const DeleteSizeConstraintSetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteSizeConstraintSetInput, options: CallOptions) !DeleteSizeConstraintSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteSizeConstraintSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.DeleteSizeConstraintSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteSizeConstraintSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteSizeConstraintSetOutput, body, allocator);
}
