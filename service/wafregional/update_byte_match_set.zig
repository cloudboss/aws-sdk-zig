...
const ByteMatchSetUpdate = @import("byte_match_set_update.zig").ByteMatchSetUpdate;

pub const UpdateByteMatchSetInput = struct {
    ...
    updates: []const ByteMatchSetUpdate,
...
pub const UpdateByteMatchSetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateByteMatchSetInput, options: CallOptions) !UpdateByteMatchSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateByteMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.UpdateByteMatchSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateByteMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateByteMatchSetOutput, body, allocator);
}
