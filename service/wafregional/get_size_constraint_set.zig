...
pub const GetSizeConstraintSetInput = struct {
...
pub const GetSizeConstraintSetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSizeConstraintSetInput, options: CallOptions) !GetSizeConstraintSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetSizeConstraintSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.GetSizeConstraintSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetSizeConstraintSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetSizeConstraintSetOutput, body, allocator);
}
