...
pub const ListRulesInput = struct {
    ...
    /// For the second and subsequent `ListRules` requests, specify the value of
    /// `NextMarker`
...
pub const ListRulesOutput = struct {
    ...
    /// `Rules`, submit another
    /// `ListRules` request, and specify the `NextMarker` value from the response in
    /// the
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListRulesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSWAF_Regional_20161128.ListRules");
