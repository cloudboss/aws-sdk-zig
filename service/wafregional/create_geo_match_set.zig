...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGeoMatchSetInput, options: Options) !CreateGeoMatchSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGeoMatchSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.CreateGeoMatchSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGeoMatchSetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateGeoMatchSetOutput, body, alloc);
}
