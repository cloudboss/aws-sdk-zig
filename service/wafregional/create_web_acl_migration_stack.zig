...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateWebACLMigrationStackInput, options: Options) !CreateWebACLMigrationStackOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateWebACLMigrationStackInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.CreateWebACLMigrationStack");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateWebACLMigrationStackOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateWebACLMigrationStackOutput, body, alloc);
}
