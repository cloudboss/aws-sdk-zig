...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListResourcesForWebACLInput, options: Options) !ListResourcesForWebACLOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListResourcesForWebACLInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSWAF_Regional_20161128.ListResourcesForWebACL");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListResourcesForWebACLOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListResourcesForWebACLOutput, body, alloc);
}
