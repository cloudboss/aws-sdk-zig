...
fn serializeRequest(alloc: std.mem.Allocator, input: ListHumanLoopsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "FlowDefinitionArn=");
    try aws.url.appendUrlEncoded(alloc, &query_buf, input.flow_definition_arn);
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListHumanLoopsOutput {
    var result: ListHumanLoopsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListHumanLoopsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
