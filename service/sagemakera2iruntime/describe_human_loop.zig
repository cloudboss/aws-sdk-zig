...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeHumanLoopInput, options: Options) !DescribeHumanLoopOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeHumanLoopInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("sagemakera2iruntime", "SageMaker A2I Runtime", alloc);
    ...
    try path_buf.appendSlice(alloc, "/human-loops/");
    try path_buf.appendSlice(alloc, input.human_loop_name);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeHumanLoopOutput {
    var result: DescribeHumanLoopOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeHumanLoopOutput, body, alloc);
    }
    _ = status;
    _ = headers;
