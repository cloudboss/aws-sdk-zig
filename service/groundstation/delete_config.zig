...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteConfigOutput {
    var result: DeleteConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
