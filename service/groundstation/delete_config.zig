...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteConfigInput, options: Options) !DeleteConfigOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteConfigOutput {
    var result: DeleteConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteConfigOutput, body, allocator);
    }
    _ = status;
    _ = headers;
