...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDataflowEndpointGroupsInput, options: Options) !ListDataflowEndpointGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDataflowEndpointGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDataflowEndpointGroupsOutput {
    var result: ListDataflowEndpointGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDataflowEndpointGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
