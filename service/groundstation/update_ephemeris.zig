...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateEphemerisInput, options: Options) !UpdateEphemerisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateEphemerisInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/ephemeris/");
    try path_buf.appendSlice(alloc, input.ephemeris_id);
    ...
    try aws.json.writeValue(@TypeOf(input.enabled), input.enabled, alloc, &body_buf);
    has_prev = true;
    if (input.name) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateEphemerisOutput {
    var result: UpdateEphemerisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateEphemerisOutput, body, alloc);
    }
    _ = status;
    _ = headers;
