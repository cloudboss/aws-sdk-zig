...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEphemeridesInput, options: Options) !ListEphemeridesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEphemeridesInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.ephemeris_type) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ephemerisType\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.satellite_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"satelliteId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEphemeridesOutput {
    var result: ListEphemeridesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListEphemeridesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
