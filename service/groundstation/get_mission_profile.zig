...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMissionProfileInput, options: Options) !GetMissionProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMissionProfileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMissionProfileOutput {
    var result: GetMissionProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMissionProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
