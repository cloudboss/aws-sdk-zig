...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataflowEndpointGroupInput, options: Options) !GetDataflowEndpointGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataflowEndpointGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataflowEndpointGroupOutput {
    var result: GetDataflowEndpointGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataflowEndpointGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
