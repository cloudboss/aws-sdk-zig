...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEphemerisInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEphemerisOutput {
    var result: DescribeEphemerisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeEphemerisOutput, body, alloc);
    }
    _ = status;
    _ = headers;
