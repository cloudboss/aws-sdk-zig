...
fn serializeRequest(alloc: std.mem.Allocator, input: GetConfigInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetConfigOutput {
    var result: GetConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfigOutput, body, alloc);
    }
    _ = status;
    _ = headers;
