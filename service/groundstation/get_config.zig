...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetConfigInput, options: Options) !GetConfigOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetConfigOutput {
    var result: GetConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetConfigOutput, body, allocator);
    }
    _ = status;
    _ = headers;
