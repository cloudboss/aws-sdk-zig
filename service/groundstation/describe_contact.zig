...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContactInput, options: CallOptions) !DescribeContactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeContactInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeContactOutput {
    var result: DescribeContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeContactOutput, body, allocator);
    }
    _ = status;
    _ = headers;
