...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateContactInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.tracking_overrides) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"trackingOverrides\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateContactOutput {
    var result: UpdateContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateContactOutput, body, allocator);
    }
    _ = status;
    _ = headers;
