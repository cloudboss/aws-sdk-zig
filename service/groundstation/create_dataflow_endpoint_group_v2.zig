...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataflowEndpointGroupV2Input, options: Options) !CreateDataflowEndpointGroupV2Output {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataflowEndpointGroupV2Input, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataflowEndpointGroupV2Output {
    var result: CreateDataflowEndpointGroupV2Output = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataflowEndpointGroupV2Output, body, alloc);
    }
    _ = status;
    _ = headers;
