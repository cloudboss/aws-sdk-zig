...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetSatelliteInput, options: Options) !GetSatelliteOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetSatelliteInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/satellite/");
    try path_buf.appendSlice(alloc, input.satellite_id);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetSatelliteOutput {
    var result: GetSatelliteOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetSatelliteOutput, body, alloc);
    }
    _ = status;
    _ = headers;
