...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMinuteUsageInput, options: Options) !GetMinuteUsageOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetMinuteUsageInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"month\":");
    try aws.json.writeValue(@TypeOf(input.month), input.month, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"year\":");
    try aws.json.writeValue(@TypeOf(input.year), input.year, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetMinuteUsageOutput {
    var result: GetMinuteUsageOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetMinuteUsageOutput, body, alloc);
    }
    _ = status;
    _ = headers;
