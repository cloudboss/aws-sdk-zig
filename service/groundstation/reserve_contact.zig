...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReserveContactInput, options: CallOptions) !ReserveContactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ReserveContactInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.ground_station), input.ground_station, allocator, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(allocator, ",");
    try body_buf.appendSlice(allocator, "\"missionProfileArn\":");
    try aws.json.writeValue(@TypeOf(input.mission_profile_arn), input.mission_profile_arn, allocator, &body_buf);
    has_prev = true;
