...
pub const ReserveContactInput = struct {
...
pub const ReserveContactOutput = struct {
    /// UUID of a contact.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ReserveContactInput, options: Options) !ReserveContactOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ReserveContactInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.ground_station), input.ground_station, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"missionProfileArn\":");
    try aws.json.writeValue(@TypeOf(input.mission_profile_arn), input.mission_profile_arn, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.tracking_overrides) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"trackingOverrides\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ReserveContactOutput {
    var result: ReserveContactOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ReserveContactOutput, body, alloc);
    }
    _ = status;
    _ = headers;
