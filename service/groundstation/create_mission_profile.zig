...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMissionProfileInput, options: Options) !CreateMissionProfileOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMissionProfileInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.dataflow_edges), input.dataflow_edges, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"minimumViableContactDurationSeconds\":");
    try aws.json.writeValue(@TypeOf(input.minimum_viable_contact_duration_seconds), input.minimum_viable_contact_duration_seconds, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    ...
    try aws.json.writeValue(@TypeOf(input.tracking_config_arn), input.tracking_config_arn, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMissionProfileOutput {
    var result: CreateMissionProfileOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateMissionProfileOutput, body, alloc);
    }
    _ = status;
    _ = headers;
