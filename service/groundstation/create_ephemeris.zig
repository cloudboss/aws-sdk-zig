...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEphemerisInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.expiration_time) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"expirationTime\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEphemerisOutput {
    var result: CreateEphemerisOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateEphemerisOutput, body, alloc);
    }
    _ = status;
    _ = headers;
