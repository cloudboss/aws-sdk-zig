...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetUtilizationInput, options: Options) !DescribeFleetUtilizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetUtilizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeFleetUtilization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetUtilizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetUtilizationOutput, body, alloc);
}
