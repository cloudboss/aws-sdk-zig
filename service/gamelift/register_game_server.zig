...
pub const RegisterGameServerInput = struct {
...
pub const RegisterGameServerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterGameServerInput, options: CallOptions) !RegisterGameServerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterGameServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.RegisterGameServer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterGameServerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterGameServerOutput, body, allocator);
}
