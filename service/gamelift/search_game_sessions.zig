...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchGameSessionsInput, options: Options) !SearchGameSessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchGameSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.SearchGameSessions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchGameSessionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SearchGameSessionsOutput, body, alloc);
}
