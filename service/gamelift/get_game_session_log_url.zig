...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGameSessionLogUrlInput, options: Options) !GetGameSessionLogUrlOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGameSessionLogUrlInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.GetGameSessionLogUrl");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGameSessionLogUrlOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetGameSessionLogUrlOutput, body, alloc);
}
