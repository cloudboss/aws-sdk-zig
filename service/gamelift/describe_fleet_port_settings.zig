...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetPortSettingsInput, options: Options) !DescribeFleetPortSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetPortSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeFleetPortSettings");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetPortSettingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetPortSettingsOutput, body, alloc);
}
