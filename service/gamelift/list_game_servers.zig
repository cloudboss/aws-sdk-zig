...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGameServersInput, options: Options) !ListGameServersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGameServersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.ListGameServers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGameServersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListGameServersOutput, body, alloc);
}
