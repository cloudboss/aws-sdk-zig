...
pub const DescribeScriptOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeScriptInput, options: CallOptions) !DescribeScriptOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeScriptInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeScript");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeScriptOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeScriptOutput, body, allocator);
}
