...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGameServerGroupInput, options: Options) !DescribeGameServerGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGameServerGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeGameServerGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGameServerGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeGameServerGroupOutput, body, alloc);
}
