...
const GameServerInstance = @import("game_server_instance.zig").GameServerInstance;

pub const DescribeGameServerInstancesInput = struct {
...
pub const DescribeGameServerInstancesOutput = struct {
    /// The collection of requested game server instances.
    game_server_instances: ?[]const GameServerInstance = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGameServerInstancesInput, options: CallOptions) !DescribeGameServerInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeGameServerInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeGameServerInstances");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeGameServerInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeGameServerInstancesOutput, body, allocator);
}
