...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetEventsInput, options: Options) !DescribeFleetEventsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeFleetEvents");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetEventsOutput, body, alloc);
}
