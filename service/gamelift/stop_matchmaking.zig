...
pub const StopMatchmakingInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopMatchmakingInput, options: Options) !StopMatchmakingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StopMatchmakingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.StopMatchmaking");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StopMatchmakingOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
