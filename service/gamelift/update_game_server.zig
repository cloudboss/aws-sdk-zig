...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGameServerInput, options: Options) !UpdateGameServerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGameServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.UpdateGameServer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGameServerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateGameServerOutput, body, alloc);
}
