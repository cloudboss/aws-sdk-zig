...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateBuildInput, options: Options) !CreateBuildOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateBuildInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.CreateBuild");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateBuildOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateBuildOutput, body, alloc);
}
