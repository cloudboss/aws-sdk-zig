...
pub const CreateLocationInput = struct {
    /// A descriptive name for the custom location.
    location_name: []const u8,

    /// A list of labels to assign to the new resource. Tags are developer-defined
    /// key-value
    /// pairs. Tagging Amazon Web Services resources are useful for resource
    /// management, access management,
    /// and cost allocation. For more information, see [ Tagging Amazon Web Services
...
pub const CreateLocationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateLocationInput, options: CallOptions) !CreateLocationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateLocationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.CreateLocation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateLocationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateLocationOutput, body, allocator);
}
