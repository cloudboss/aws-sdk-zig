...
const GameServerGroupDeleteOption = @import("game_server_group_delete_option.zig").GameServerGroupDeleteOption;
const GameServerGroup = @import("game_server_group.zig").GameServerGroup;

pub const DeleteGameServerGroupInput = struct {
    ...
    /// active game servers regardless of their utilization status, and the Amazon
    /// EC2 Auto
    /// Scaling group.
    ///
    /// * `RETAIN` – Does a safe delete of the game server group but retains
    /// the Amazon EC2 Auto Scaling group as is.
    delete_option: ?GameServerGroupDeleteOption = null,
...
pub const DeleteGameServerGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGameServerGroupInput, options: CallOptions) !DeleteGameServerGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteGameServerGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeleteGameServerGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteGameServerGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteGameServerGroupOutput, body, allocator);
}
