...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopFleetActionsInput, options: Options) !StopFleetActionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopFleetActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.StopFleetActions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopFleetActionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopFleetActionsOutput, body, alloc);
}
