...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StopGameSessionPlacementInput, options: Options) !StopGameSessionPlacementOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StopGameSessionPlacementInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.StopGameSessionPlacement");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StopGameSessionPlacementOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StopGameSessionPlacementOutput, body, alloc);
}
