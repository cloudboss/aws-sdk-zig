...
pub const DescribeAliasInput = struct {
...
pub const DescribeAliasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAliasInput, options: CallOptions) !DescribeAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeAlias");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeAliasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeAliasOutput, body, allocator);
}
