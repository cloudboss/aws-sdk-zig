...
pub const ResumeGameServerGroupOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResumeGameServerGroupInput, options: CallOptions) !ResumeGameServerGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResumeGameServerGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.ResumeGameServerGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResumeGameServerGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResumeGameServerGroupOutput, body, allocator);
}
