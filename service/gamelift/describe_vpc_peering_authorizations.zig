...
pub const DescribeVpcPeeringAuthorizationsOutput = struct {
    ...
    vpc_peering_authorizations: ?[]const VpcPeeringAuthorization = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeVpcPeeringAuthorizationsInput, options: CallOptions) !DescribeVpcPeeringAuthorizationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeVpcPeeringAuthorizationsInput, config: *aws.Config) !aws.http.Request {
    _ = input;
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeVpcPeeringAuthorizations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeVpcPeeringAuthorizationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeVpcPeeringAuthorizationsOutput, body, allocator);
}
