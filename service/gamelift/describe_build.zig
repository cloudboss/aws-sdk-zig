...
pub const DescribeBuildInput = struct {
...
pub const DescribeBuildOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeBuildInput, options: CallOptions) !DescribeBuildOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeBuildInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeBuild");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeBuildOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeBuildOutput, body, allocator);
}
