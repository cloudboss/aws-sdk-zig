...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEC2InstanceLimitsInput, options: Options) !DescribeEC2InstanceLimitsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEC2InstanceLimitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeEC2InstanceLimits");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEC2InstanceLimitsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEC2InstanceLimitsOutput, body, alloc);
}
