...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePlayerSessionsInput, options: Options) !CreatePlayerSessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePlayerSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.CreatePlayerSessions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePlayerSessionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePlayerSessionsOutput, body, alloc);
}
