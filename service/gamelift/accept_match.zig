...
fn serializeRequest(alloc: std.mem.Allocator, input: AcceptMatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.AcceptMatch");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AcceptMatchOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
