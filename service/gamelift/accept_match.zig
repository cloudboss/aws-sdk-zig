...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AcceptMatchInput, options: Options) !AcceptMatchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AcceptMatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.AcceptMatch");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AcceptMatchOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
