...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetComputeAuthTokenInput, options: Options) !GetComputeAuthTokenOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetComputeAuthTokenInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.GetComputeAuthToken");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetComputeAuthTokenOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetComputeAuthTokenOutput, body, alloc);
}
