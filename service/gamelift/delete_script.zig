...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteScriptInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DeleteScript");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteScriptOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
