...
pub const DeleteScriptInput = struct {
...
pub const DeleteScriptOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteScriptInput, options: CallOptions) !DeleteScriptOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteScriptInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeleteScript");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteScriptOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
