...
pub const DeregisterGameServerInput = struct {
...
pub const DeregisterGameServerOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterGameServerInput, options: CallOptions) !DeregisterGameServerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterGameServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeregisterGameServer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterGameServerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
