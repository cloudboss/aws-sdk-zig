...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterGameServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DeregisterGameServer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterGameServerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
