...
pub const StartFleetActionsInput = struct {
...
pub const StartFleetActionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartFleetActionsInput, options: CallOptions) !StartFleetActionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartFleetActionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.StartFleetActions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartFleetActionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartFleetActionsOutput, body, allocator);
}
