...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetAttributesInput, options: Options) !DescribeFleetAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeFleetAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetAttributesOutput, body, alloc);
}
