...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGameSessionsInput, options: Options) !DescribeGameSessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGameSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeGameSessions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGameSessionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeGameSessionsOutput, body, alloc);
}
