...
pub const DeleteContainerFleetInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteContainerFleetInput, options: Options) !DeleteContainerFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteContainerFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeleteContainerFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteContainerFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
