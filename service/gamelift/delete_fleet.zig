...
const DeleteFleetOutput = struct {};
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DeleteFleet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteFleetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
