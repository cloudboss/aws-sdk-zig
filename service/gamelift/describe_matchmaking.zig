...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMatchmakingInput, options: Options) !DescribeMatchmakingOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMatchmakingInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeMatchmaking");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMatchmakingOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMatchmakingOutput, body, alloc);
}
