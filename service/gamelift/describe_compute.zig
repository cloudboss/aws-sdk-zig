...
pub const DescribeComputeInput = struct {
    /// The unique identifier of the compute resource to retrieve properties for.
    /// For a
    /// managed container fleet or Anywhere fleet, use a compute name. For an EC2
    /// fleet, use an
    /// instance ID. To retrieve a fleet's compute identifiers, call
...
pub const DescribeComputeOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeComputeInput, options: CallOptions) !DescribeComputeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeComputeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeCompute");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeComputeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeComputeOutput, body, allocator);
}
