...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInstanceAccessInput, options: Options) !GetInstanceAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInstanceAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.GetInstanceAccess");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInstanceAccessOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInstanceAccessOutput, body, alloc);
}
