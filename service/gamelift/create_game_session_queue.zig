...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGameSessionQueueInput, options: Options) !CreateGameSessionQueueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGameSessionQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.CreateGameSessionQueue");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGameSessionQueueOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateGameSessionQueueOutput, body, alloc);
}
