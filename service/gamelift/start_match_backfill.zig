...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartMatchBackfillInput, options: Options) !StartMatchBackfillOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartMatchBackfillInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.StartMatchBackfill");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartMatchBackfillOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartMatchBackfillOutput, body, alloc);
}
