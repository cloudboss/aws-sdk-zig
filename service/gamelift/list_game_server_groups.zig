...
pub const ListGameServerGroupsInput = struct {
...
pub const ListGameServerGroupsOutput = struct {
    /// The game server groups' game server groups.
    game_server_groups: ?[]const GameServerGroup = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGameServerGroupsInput, options: CallOptions) !ListGameServerGroupsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListGameServerGroupsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.ListGameServerGroups");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListGameServerGroupsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListGameServerGroupsOutput, body, allocator);
}
