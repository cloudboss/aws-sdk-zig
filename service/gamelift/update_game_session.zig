...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGameSessionInput, options: Options) !UpdateGameSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGameSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.UpdateGameSession");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGameSessionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateGameSessionOutput, body, alloc);
}
