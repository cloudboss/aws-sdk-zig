...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RequestUploadCredentialsInput, options: Options) !RequestUploadCredentialsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RequestUploadCredentialsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.RequestUploadCredentials");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RequestUploadCredentialsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RequestUploadCredentialsOutput, body, alloc);
}
