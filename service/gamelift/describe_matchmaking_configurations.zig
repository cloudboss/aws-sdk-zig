...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMatchmakingConfigurationsInput, options: Options) !DescribeMatchmakingConfigurationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMatchmakingConfigurationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeMatchmakingConfigurations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMatchmakingConfigurationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMatchmakingConfigurationsOutput, body, alloc);
}
