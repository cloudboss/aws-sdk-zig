...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFleetCapacityInput, options: Options) !UpdateFleetCapacityOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFleetCapacityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.UpdateFleetCapacity");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFleetCapacityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFleetCapacityOutput, body, alloc);
}
