...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGameServerInput, options: Options) !DescribeGameServerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGameServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeGameServer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGameServerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeGameServerOutput, body, alloc);
}
