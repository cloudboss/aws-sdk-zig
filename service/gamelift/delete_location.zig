...
pub const DeleteLocationInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteLocationInput, options: Options) !DeleteLocationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteLocationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeleteLocation");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteLocationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
