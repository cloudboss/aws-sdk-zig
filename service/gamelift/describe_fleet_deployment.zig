...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetDeploymentInput, options: Options) !DescribeFleetDeploymentOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetDeploymentInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeFleetDeployment");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetDeploymentOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetDeploymentOutput, body, alloc);
}
