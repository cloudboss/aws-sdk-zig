...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGameServerGroupInput, options: CallOptions) !UpdateGameServerGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateGameServerGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.UpdateGameServerGroup");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateGameServerGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateGameServerGroupOutput, body, allocator);
}
