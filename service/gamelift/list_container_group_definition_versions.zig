...
pub const ListContainerGroupDefinitionVersionsInput = struct {
...
pub const ListContainerGroupDefinitionVersionsOutput = struct {
    /// A result set of container group definitions that match the request.
    container_group_definitions: ?[]const ContainerGroupDefinition = null,
    ...
    pub const json_field_names = .{
        .container_group_definitions = "ContainerGroupDefinitions",
        .next_token = "NextToken",
    };
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContainerGroupDefinitionVersionsInput, options: CallOptions) !ListContainerGroupDefinitionVersionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListContainerGroupDefinitionVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.ListContainerGroupDefinitionVersions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListContainerGroupDefinitionVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListContainerGroupDefinitionVersionsOutput, body, allocator);
}
