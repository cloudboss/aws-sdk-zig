...
pub const CreateVpcPeeringAuthorizationInput = struct {
...
pub const CreateVpcPeeringAuthorizationOutput = struct {
    /// Details on the requested VPC peering authorization, including expiration.
    vpc_peering_authorization: ?VpcPeeringAuthorization = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateVpcPeeringAuthorizationInput, options: CallOptions) !CreateVpcPeeringAuthorizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateVpcPeeringAuthorizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.CreateVpcPeeringAuthorization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateVpcPeeringAuthorizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateVpcPeeringAuthorizationOutput, body, allocator);
}
