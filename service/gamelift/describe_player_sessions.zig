...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePlayerSessionsInput, options: Options) !DescribePlayerSessionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePlayerSessionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribePlayerSessions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePlayerSessionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribePlayerSessionsOutput, body, alloc);
}
