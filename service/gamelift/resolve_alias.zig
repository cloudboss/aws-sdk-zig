...
pub const ResolveAliasInput = struct {
...
pub const ResolveAliasOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ResolveAliasInput, options: CallOptions) !ResolveAliasOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ResolveAliasInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.ResolveAlias");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ResolveAliasOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ResolveAliasOutput, body, allocator);
}
