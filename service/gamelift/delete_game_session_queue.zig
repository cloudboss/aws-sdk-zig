...
pub const DeleteGameSessionQueueInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteGameSessionQueueInput, options: CallOptions) !DeleteGameSessionQueueOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteGameSessionQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeleteGameSessionQueue");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteGameSessionQueueOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
