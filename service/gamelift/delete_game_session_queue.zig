...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteGameSessionQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DeleteGameSessionQueue");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteGameSessionQueueOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
