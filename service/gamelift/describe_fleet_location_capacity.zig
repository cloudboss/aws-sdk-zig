...
const FleetCapacity = @import("fleet_capacity.zig").FleetCapacity;

pub const DescribeFleetLocationCapacityInput = struct {
...
pub const DescribeFleetLocationCapacityOutput = struct {
    /// Resource capacity information for the requested fleet location. Capacity
    /// objects are
    /// returned only for fleets and locations that currently exist. Changes in
    /// desired instance
    /// value can take up to 1 minute to be reflected.
    fleet_capacity: ?FleetCapacity = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetLocationCapacityInput, options: CallOptions) !DescribeFleetLocationCapacityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFleetLocationCapacityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeFleetLocationCapacity");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFleetLocationCapacityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetLocationCapacityOutput, body, allocator);
}
