...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMatchmakingRuleSetsInput, options: Options) !DescribeMatchmakingRuleSetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMatchmakingRuleSetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeMatchmakingRuleSets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMatchmakingRuleSetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMatchmakingRuleSetsOutput, body, alloc);
}
