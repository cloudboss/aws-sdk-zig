...
pub const UpdateMatchmakingConfigurationInput = struct {
    /// A flag that indicates whether a match that was created with this
    /// configuration must be
    /// accepted by the matched players. To require acceptance, set to TRUE. With
    /// this option
    ...
    /// manually or does
    /// not use the match backfill feature. Specify AUTOMATIC to have GameLift
    /// create a match backfill
    ...
    /// session](https://docs.aws.amazon.com/gamelift/latest/developerguide/gamelift-sdk-server-api.html#gamelift-sdk-server-startsession). This information is added to the game session
...
pub const UpdateMatchmakingConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMatchmakingConfigurationInput, options: CallOptions) !UpdateMatchmakingConfigurationOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "gamelift");
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateMatchmakingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.UpdateMatchmakingConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateMatchmakingConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateMatchmakingConfigurationOutput, body, allocator);
}
