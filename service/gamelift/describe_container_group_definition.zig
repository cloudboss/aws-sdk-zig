...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContainerGroupDefinitionInput, options: Options) !DescribeContainerGroupDefinitionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeContainerGroupDefinitionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeContainerGroupDefinition");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeContainerGroupDefinitionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeContainerGroupDefinitionOutput, body, alloc);
}
