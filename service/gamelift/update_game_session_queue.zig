...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGameSessionQueueInput, options: Options) !UpdateGameSessionQueueOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGameSessionQueueInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.UpdateGameSessionQueue");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGameSessionQueueOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateGameSessionQueueOutput, body, alloc);
}
