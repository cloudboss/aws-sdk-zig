...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGameSessionDetailsInput, options: Options) !DescribeGameSessionDetailsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGameSessionDetailsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeGameSessionDetails");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGameSessionDetailsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeGameSessionDetailsOutput, body, alloc);
}
