...
pub const DescribeInstancesInput = struct {
    ...
    /// The name of a location to retrieve instance information for, in the form of
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeInstancesInput, options: CallOptions) !DescribeInstancesOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeInstancesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeInstances");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeInstancesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeInstancesOutput, body, allocator);
}
