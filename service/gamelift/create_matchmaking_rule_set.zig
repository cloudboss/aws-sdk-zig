...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMatchmakingRuleSetInput, options: Options) !CreateMatchmakingRuleSetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMatchmakingRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.CreateMatchmakingRuleSet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMatchmakingRuleSetOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(CreateMatchmakingRuleSetOutput, body, alloc);
}
