...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteVpcPeeringAuthorizationInput, options: Options) !DeleteVpcPeeringAuthorizationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteVpcPeeringAuthorizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DeleteVpcPeeringAuthorization");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteVpcPeeringAuthorizationOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
