...
pub const CreateFleetLocationsInput = struct {
    ...
    /// the fleet.
    /// You can add any Amazon GameLift Servers-supported Amazon Web Services Region
    /// as a remote location, in the form of an
    /// Amazon Web Services Region code such as `us-west-2`.
    locations: []const LocationConfiguration,
...
pub const CreateFleetLocationsOutput = struct {
    ...
    /// status of
    /// each location. For new locations, the status is set to `NEW`. During
    /// location
    /// creation, Amazon GameLift Servers updates each location's status as
    /// instances are deployed there and
    /// prepared for game hosting. This list does not include the fleet home Region
    /// or any
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateFleetLocationsInput, options: CallOptions) !CreateFleetLocationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateFleetLocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.CreateFleetLocations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateFleetLocationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateFleetLocationsOutput, body, allocator);
}
