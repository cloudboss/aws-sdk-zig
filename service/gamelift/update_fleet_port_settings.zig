...
pub const UpdateFleetPortSettingsInput = struct {
    ...
    inbound_permission_authorizations: ?[]const IpPermission = null,

    /// A collection of port settings to be removed from the fleet resource.
    inbound_permission_revocations: ?[]const IpPermission = null,
...
pub const UpdateFleetPortSettingsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFleetPortSettingsInput, options: CallOptions) !UpdateFleetPortSettingsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateFleetPortSettingsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.UpdateFleetPortSettings");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateFleetPortSettingsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFleetPortSettingsOutput, body, allocator);
}
