...
pub const DeleteMatchmakingRuleSetInput = struct {
    /// A unique identifier for the matchmaking rule set to be deleted. (Note: The
    /// rule set name is different from the optional "name"
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteMatchmakingRuleSetInput, options: Options) !DeleteMatchmakingRuleSetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteMatchmakingRuleSetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeleteMatchmakingRuleSet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteMatchmakingRuleSetOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
