...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListLocationsInput, options: Options) !ListLocationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListLocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.ListLocations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListLocationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListLocationsOutput, body, alloc);
}
