...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateContainerFleetInput, options: Options) !UpdateContainerFleetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateContainerFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.UpdateContainerFleet");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateContainerFleetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateContainerFleetOutput, body, alloc);
}
