...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRuntimeConfigurationInput, options: Options) !DescribeRuntimeConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRuntimeConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeRuntimeConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRuntimeConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRuntimeConfigurationOutput, body, alloc);
}
