...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListComputeInput, options: Options) !ListComputeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListComputeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.ListCompute");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListComputeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListComputeOutput, body, alloc);
}
