...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListFleetDeploymentsInput, options: Options) !ListFleetDeploymentsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListFleetDeploymentsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.ListFleetDeployments");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListFleetDeploymentsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListFleetDeploymentsOutput, body, alloc);
}
