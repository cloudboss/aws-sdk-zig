...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ClaimGameServerInput, options: Options) !ClaimGameServerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ClaimGameServerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.ClaimGameServer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ClaimGameServerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ClaimGameServerOutput, body, alloc);
}
