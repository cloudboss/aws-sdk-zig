...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterComputeInput, options: Options) !RegisterComputeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RegisterComputeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.RegisterCompute");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RegisterComputeOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RegisterComputeOutput, body, alloc);
}
