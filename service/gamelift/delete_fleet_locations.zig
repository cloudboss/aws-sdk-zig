...
pub const DeleteFleetLocationsInput = struct {
...
pub const DeleteFleetLocationsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteFleetLocationsInput, options: CallOptions) !DeleteFleetLocationsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteFleetLocationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeleteFleetLocations");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteFleetLocationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeleteFleetLocationsOutput, body, allocator);
}
