...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TerminateGameSessionInput, options: Options) !TerminateGameSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: TerminateGameSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.TerminateGameSession");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !TerminateGameSessionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(TerminateGameSessionOutput, body, alloc);
}
