...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateBuildInput, options: Options) !UpdateBuildOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateBuildInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.UpdateBuild");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateBuildOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateBuildOutput, body, alloc);
}
