...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateFleetAttributesInput, options: Options) !UpdateFleetAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateFleetAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.UpdateFleetAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateFleetAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateFleetAttributesOutput, body, alloc);
}
