...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePlayerSessionInput, options: Options) !CreatePlayerSessionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePlayerSessionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.CreatePlayerSession");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePlayerSessionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePlayerSessionOutput, body, alloc);
}
