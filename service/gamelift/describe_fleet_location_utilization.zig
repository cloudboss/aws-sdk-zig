...
pub const DescribeFleetLocationUtilizationInput = struct {
...
pub const DescribeFleetLocationUtilizationOutput = struct {
    ...
    fleet_utilization: ?FleetUtilization = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetLocationUtilizationInput, options: CallOptions) !DescribeFleetLocationUtilizationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFleetLocationUtilizationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeFleetLocationUtilization");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFleetLocationUtilizationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetLocationUtilizationOutput, body, allocator);
}
