...
pub const DeleteBuildInput = struct {
    /// A unique identifier for the build to delete. You can use either the build ID
    /// or ARN value.
    build_id: []const u8,
...
pub const DeleteBuildOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteBuildInput, options: CallOptions) !DeleteBuildOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteBuildInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeleteBuild");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteBuildOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
