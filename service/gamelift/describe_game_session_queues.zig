...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGameSessionQueuesInput, options: Options) !DescribeGameSessionQueuesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGameSessionQueuesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeGameSessionQueues");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGameSessionQueuesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeGameSessionQueuesOutput, body, alloc);
}
