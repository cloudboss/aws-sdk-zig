...
pub const DescribeContainerFleetInput = struct {
    /// A unique identifier for the container fleet to retrieve. You can use either
    /// the fleet
    /// ID or ARN value.
    fleet_id: []const u8,
...
pub const DescribeContainerFleetOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeContainerFleetInput, options: CallOptions) !DescribeContainerFleetOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeContainerFleetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeContainerFleet");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeContainerFleetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeContainerFleetOutput, body, allocator);
}
