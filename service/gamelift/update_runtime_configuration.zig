...
pub const UpdateRuntimeConfigurationInput = struct {
    ...
    /// processes run
    /// either a custom game build executable or a Amazon GameLift Servers Realtime
    /// script. The runtime configuration lists
    /// the types of server processes to run, how to launch them, and the number of
    /// processes to
    /// run concurrently.
...
pub const UpdateRuntimeConfigurationOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRuntimeConfigurationInput, options: CallOptions) !UpdateRuntimeConfigurationOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateRuntimeConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.UpdateRuntimeConfiguration");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateRuntimeConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateRuntimeConfigurationOutput, body, allocator);
}
