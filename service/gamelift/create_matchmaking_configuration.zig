...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateMatchmakingConfigurationInput, options: Options) !CreateMatchmakingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateMatchmakingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.CreateMatchmakingConfiguration");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateMatchmakingConfigurationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateMatchmakingConfigurationOutput, body, alloc);
}
