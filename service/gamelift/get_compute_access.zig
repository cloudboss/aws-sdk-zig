...
const ContainerIdentifier = @import("container_identifier.zig").ContainerIdentifier;
const AwsCredentials = @import("aws_credentials.zig").AwsCredentials;

pub const GetComputeAccessInput = struct {
    /// A unique identifier for the compute resource that you want to connect to.
    /// For an EC2
    /// fleet, use an instance ID. For a managed container fleet, use a compute
    /// name. You can
    ...
    /// want to connect to. You can use either the
...
pub const GetComputeAccessOutput = struct {
    ...
    container_identifiers: ?[]const ContainerIdentifier = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetComputeAccessInput, options: CallOptions) !GetComputeAccessOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetComputeAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.GetComputeAccess");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetComputeAccessOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetComputeAccessOutput, body, allocator);
}
