...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetLocationAttributesInput, options: Options) !DescribeFleetLocationAttributesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeFleetLocationAttributesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DescribeFleetLocationAttributes");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeFleetLocationAttributesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetLocationAttributesOutput, body, alloc);
}
