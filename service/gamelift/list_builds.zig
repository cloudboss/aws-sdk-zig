...
pub const ListBuildsInput = struct {
...
pub const ListBuildsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBuildsInput, options: CallOptions) !ListBuildsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListBuildsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.ListBuilds");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListBuildsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListBuildsOutput, body, allocator);
}
