...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SuspendGameServerGroupInput, options: Options) !SuspendGameServerGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SuspendGameServerGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.SuspendGameServerGroup");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SuspendGameServerGroupOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(SuspendGameServerGroupOutput, body, alloc);
}
