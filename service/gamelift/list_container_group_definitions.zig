...
pub const ListContainerGroupDefinitionsInput = struct {
...
pub const ListContainerGroupDefinitionsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContainerGroupDefinitionsInput, options: CallOptions) !ListContainerGroupDefinitionsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListContainerGroupDefinitionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.ListContainerGroupDefinitions");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListContainerGroupDefinitionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListContainerGroupDefinitionsOutput, body, allocator);
}
