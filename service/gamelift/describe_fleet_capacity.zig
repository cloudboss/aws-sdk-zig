...
pub const DescribeFleetCapacityInput = struct {
...
pub const DescribeFleetCapacityOutput = struct {
    ...
    fleet_capacity: ?[]const FleetCapacity = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeFleetCapacityInput, options: CallOptions) !DescribeFleetCapacityOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeFleetCapacityInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DescribeFleetCapacity");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeFleetCapacityOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeFleetCapacityOutput, body, allocator);
}
