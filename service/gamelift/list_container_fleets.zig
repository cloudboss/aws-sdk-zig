...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListContainerFleetsInput, options: Options) !ListContainerFleetsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListContainerFleetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.ListContainerFleets");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListContainerFleetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListContainerFleetsOutput, body, alloc);
}
