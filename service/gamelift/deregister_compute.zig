...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterComputeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "GameLift.DeregisterCompute");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterComputeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
