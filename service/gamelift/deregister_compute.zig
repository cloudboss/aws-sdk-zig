...
pub const DeregisterComputeInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterComputeInput, options: Options) !DeregisterComputeOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeregisterComputeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "GameLift.DeregisterCompute");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeregisterComputeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
