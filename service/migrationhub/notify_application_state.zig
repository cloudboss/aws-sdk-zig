...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: NotifyApplicationStateInput, options: Options) !NotifyApplicationStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: NotifyApplicationStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.NotifyApplicationState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !NotifyApplicationStateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
