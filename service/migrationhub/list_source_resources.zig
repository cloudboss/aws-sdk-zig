...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSourceResourcesInput, options: Options) !ListSourceResourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSourceResourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.ListSourceResources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSourceResourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListSourceResourcesOutput, body, alloc);
}
