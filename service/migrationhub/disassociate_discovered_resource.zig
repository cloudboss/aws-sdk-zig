...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateDiscoveredResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.DisassociateDiscoveredResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateDiscoveredResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
