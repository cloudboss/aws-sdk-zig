...
pub const DisassociateDiscoveredResourceInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateDiscoveredResourceInput, options: Options) !DisassociateDiscoveredResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateDiscoveredResourceInput, config: *aws.Config) !aws.http.Request {
    const endpoint = try config.getEndpointForService("migrationhub", "Migration Hub", allocator);
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMigrationHub.DisassociateDiscoveredResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateDiscoveredResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
