...
pub const AssociateDiscoveredResourceInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateDiscoveredResourceInput, options: Options) !AssociateDiscoveredResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateDiscoveredResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMigrationHub.AssociateDiscoveredResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateDiscoveredResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
