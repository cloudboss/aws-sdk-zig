...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateDiscoveredResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.AssociateDiscoveredResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateDiscoveredResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
