...
const MigrationTaskSummary = @import("migration_task_summary.zig").MigrationTaskSummary;

pub const ListMigrationTasksInput = struct {
...
pub const ListMigrationTasksOutput = struct {
    ...
    migration_task_summary_list: ?[]const MigrationTaskSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMigrationTasksInput, options: CallOptions) !ListMigrationTasksOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListMigrationTasksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMigrationHub.ListMigrationTasks");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListMigrationTasksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMigrationTasksOutput, body, allocator);
}
