...
pub const DisassociateCreatedArtifactInput = struct {
    /// An ARN of the AWS resource related to the migration (e.g., AMI, EC2
    /// instance, RDS
    /// instance, etc.)
    created_artifact_name: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateCreatedArtifactInput, options: Options) !DisassociateCreatedArtifactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DisassociateCreatedArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMigrationHub.DisassociateCreatedArtifact");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DisassociateCreatedArtifactOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
