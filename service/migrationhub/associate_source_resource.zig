...
pub const AssociateSourceResourceInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateSourceResourceInput, options: Options) !AssociateSourceResourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateSourceResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMigrationHub.AssociateSourceResource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateSourceResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
