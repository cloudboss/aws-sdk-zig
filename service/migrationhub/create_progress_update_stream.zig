...
pub const CreateProgressUpdateStreamInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateProgressUpdateStreamInput, options: CallOptions) !CreateProgressUpdateStreamOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "mgh");
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateProgressUpdateStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMigrationHub.CreateProgressUpdateStream");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreateProgressUpdateStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
