...
pub const AssociateCreatedArtifactInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateCreatedArtifactInput, options: Options) !AssociateCreatedArtifactOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateCreatedArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMigrationHub.AssociateCreatedArtifact");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateCreatedArtifactOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
