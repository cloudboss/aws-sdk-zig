...
fn serializeRequest(alloc: std.mem.Allocator, input: AssociateCreatedArtifactInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.AssociateCreatedArtifact");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AssociateCreatedArtifactOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
