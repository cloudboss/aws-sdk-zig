...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApplicationStatesInput, options: Options) !ListApplicationStatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApplicationStatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.ListApplicationStates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApplicationStatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListApplicationStatesOutput, body, alloc);
}
