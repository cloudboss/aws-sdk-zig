...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListProgressUpdateStreamsInput, options: Options) !ListProgressUpdateStreamsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListProgressUpdateStreamsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.ListProgressUpdateStreams");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListProgressUpdateStreamsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListProgressUpdateStreamsOutput, body, alloc);
}
