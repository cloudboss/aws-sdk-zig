...
pub const ImportMigrationTaskInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ImportMigrationTaskInput, options: Options) !ImportMigrationTaskOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ImportMigrationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMigrationHub.ImportMigrationTask");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ImportMigrationTaskOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
