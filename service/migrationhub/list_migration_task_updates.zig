...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListMigrationTaskUpdatesInput, options: Options) !ListMigrationTaskUpdatesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListMigrationTaskUpdatesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.ListMigrationTaskUpdates");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListMigrationTaskUpdatesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListMigrationTaskUpdatesOutput, body, alloc);
}
