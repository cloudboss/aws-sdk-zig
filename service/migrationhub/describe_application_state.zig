...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeApplicationStateInput, options: Options) !DescribeApplicationStateOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeApplicationStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.DescribeApplicationState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeApplicationStateOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeApplicationStateOutput, body, alloc);
}
