...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeMigrationTaskInput, options: Options) !DescribeMigrationTaskOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMigrationTaskInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.DescribeMigrationTask");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMigrationTaskOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeMigrationTaskOutput, body, alloc);
}
