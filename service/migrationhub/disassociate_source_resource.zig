...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateSourceResourceInput, options: Options) !DisassociateSourceResourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateSourceResourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.DisassociateSourceResource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateSourceResourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
