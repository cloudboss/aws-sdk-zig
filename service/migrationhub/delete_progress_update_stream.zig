...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteProgressUpdateStreamInput, options: Options) !DeleteProgressUpdateStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteProgressUpdateStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.DeleteProgressUpdateStream");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteProgressUpdateStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
