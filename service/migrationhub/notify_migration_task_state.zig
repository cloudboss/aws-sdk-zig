...
pub const NotifyMigrationTaskStateInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: NotifyMigrationTaskStateInput, options: Options) !NotifyMigrationTaskStateOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: NotifyMigrationTaskStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSMigrationHub.NotifyMigrationTaskState");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !NotifyMigrationTaskStateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
