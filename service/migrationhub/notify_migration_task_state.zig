...
fn serializeRequest(alloc: std.mem.Allocator, input: NotifyMigrationTaskStateInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.NotifyMigrationTaskState");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !NotifyMigrationTaskStateOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
