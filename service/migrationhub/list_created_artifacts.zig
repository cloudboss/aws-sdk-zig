...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListCreatedArtifactsInput, options: Options) !ListCreatedArtifactsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListCreatedArtifactsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSMigrationHub.ListCreatedArtifacts");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListCreatedArtifactsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListCreatedArtifactsOutput, body, alloc);
}
