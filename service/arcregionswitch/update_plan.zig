...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePlanInput, options: Options) !UpdatePlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ArcRegionSwitch.UpdatePlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePlanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdatePlanOutput, body, alloc);
}
