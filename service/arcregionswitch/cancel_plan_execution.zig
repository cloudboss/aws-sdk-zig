...
fn serializeRequest(alloc: std.mem.Allocator, input: CancelPlanExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ArcRegionSwitch.CancelPlanExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CancelPlanExecutionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
