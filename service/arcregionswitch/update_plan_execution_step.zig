...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePlanExecutionStepInput, options: Options) !UpdatePlanExecutionStepOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePlanExecutionStepInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ArcRegionSwitch.UpdatePlanExecutionStep");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePlanExecutionStepOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
