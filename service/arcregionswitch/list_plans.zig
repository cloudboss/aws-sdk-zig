...
const AbbreviatedPlan = @import("abbreviated_plan.zig").AbbreviatedPlan;

pub const ListPlansInput = struct {
...
pub const ListPlansOutput = struct {
    /// Specifies that you want to receive the next page of results. Valid only if
    /// you received a `nextToken` response in the previous request. If you did, it
    /// indicates that more output is available. Set this parameter to the value
    /// provided by the previous call's `nextToken` response to request the next
    ...
    plans: ?[]const AbbreviatedPlan = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPlansInput, options: CallOptions) !ListPlansOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPlansInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ArcRegionSwitch.ListPlans");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPlansOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPlansOutput, body, allocator);
}
