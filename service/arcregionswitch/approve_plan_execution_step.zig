...
pub const ApprovePlanExecutionStepInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ApprovePlanExecutionStepInput, options: Options) !ApprovePlanExecutionStepOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ApprovePlanExecutionStepInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ArcRegionSwitch.ApprovePlanExecutionStep");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ApprovePlanExecutionStepOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
