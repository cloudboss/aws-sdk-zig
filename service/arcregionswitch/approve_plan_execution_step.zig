...
fn serializeRequest(alloc: std.mem.Allocator, input: ApprovePlanExecutionStepInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ArcRegionSwitch.ApprovePlanExecutionStep");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ApprovePlanExecutionStepOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
