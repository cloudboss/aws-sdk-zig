...
pub const CreatePlanInput = struct {
...
pub const CreatePlanOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePlanInput, options: CallOptions) !CreatePlanOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreatePlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ArcRegionSwitch.CreatePlan");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CreatePlanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreatePlanOutput, body, allocator);
}
