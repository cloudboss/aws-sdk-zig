...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPlanExecutionsInput, options: Options) !ListPlanExecutionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPlanExecutionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ArcRegionSwitch.ListPlanExecutions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPlanExecutionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPlanExecutionsOutput, body, alloc);
}
