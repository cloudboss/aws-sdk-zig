...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoute53HealthChecksInput, options: Options) !ListRoute53HealthChecksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoute53HealthChecksInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ArcRegionSwitch.ListRoute53HealthChecks");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoute53HealthChecksOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRoute53HealthChecksOutput, body, alloc);
}
