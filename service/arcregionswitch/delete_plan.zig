...
pub const DeletePlanInput = struct {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeletePlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ArcRegionSwitch.DeletePlan");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeletePlanOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
