...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPlanInRegionInput, options: CallOptions) !GetPlanInRegionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetPlanInRegionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ArcRegionSwitch.GetPlanInRegion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetPlanInRegionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPlanInRegionOutput, body, allocator);
}
