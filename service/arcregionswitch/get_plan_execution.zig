...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetPlanExecutionInput, options: Options) !GetPlanExecutionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetPlanExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ArcRegionSwitch.GetPlanExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetPlanExecutionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetPlanExecutionOutput, body, alloc);
}
