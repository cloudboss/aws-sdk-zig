...
const ExecutionEvent = @import("execution_event.zig").ExecutionEvent;

pub const ListPlanExecutionEventsInput = struct {
...
pub const ListPlanExecutionEventsOutput = struct {
    /// The items in the plan execution event.
    items: ?[]const ExecutionEvent = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPlanExecutionEventsInput, options: CallOptions) !ListPlanExecutionEventsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPlanExecutionEventsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ArcRegionSwitch.ListPlanExecutionEvents");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPlanExecutionEventsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPlanExecutionEventsOutput, body, allocator);
}
