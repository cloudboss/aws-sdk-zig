...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePlanExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ArcRegionSwitch.UpdatePlanExecution");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePlanExecutionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
