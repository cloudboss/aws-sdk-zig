...
const UpdatePlanExecutionAction = @import("update_plan_execution_action.zig").UpdatePlanExecutionAction;

pub const UpdatePlanExecutionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePlanExecutionInput, options: Options) !UpdatePlanExecutionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdatePlanExecutionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "ArcRegionSwitch.UpdatePlanExecution");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePlanExecutionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
