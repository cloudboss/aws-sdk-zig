...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRoute53HealthChecksInRegionInput, options: Options) !ListRoute53HealthChecksInRegionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRoute53HealthChecksInRegionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "ArcRegionSwitch.ListRoute53HealthChecksInRegion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRoute53HealthChecksInRegionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRoute53HealthChecksInRegionOutput, body, alloc);
}
