...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePackagingGroupInput, options: CallOptions) !UpdatePackagingGroupOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "mediapackage-vod");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdatePackagingGroupOutput {
    var result: UpdatePackagingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePackagingGroupOutput, body, allocator);
    }
    _ = status;
    _ = headers;
