...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePackagingConfigurationInput, options: Options) !DescribePackagingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePackagingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/packaging_configurations/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePackagingConfigurationOutput {
    var result: DescribePackagingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePackagingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
