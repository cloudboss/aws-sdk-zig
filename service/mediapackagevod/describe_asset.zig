...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeAssetInput, options: Options) !DescribeAssetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeAssetInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeAssetOutput {
    var result: DescribeAssetOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeAssetOutput, body, alloc);
    }
    _ = status;
    _ = headers;
