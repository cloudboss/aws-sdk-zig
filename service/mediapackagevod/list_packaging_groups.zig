...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPackagingGroupsInput, options: Options) !ListPackagingGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPackagingGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPackagingGroupsOutput {
    var result: ListPackagingGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPackagingGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
