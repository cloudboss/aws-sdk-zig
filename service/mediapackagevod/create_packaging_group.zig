...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePackagingGroupInput, options: Options) !CreatePackagingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePackagingGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.egress_access_logs) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EgressAccessLogs\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePackagingGroupOutput {
    var result: CreatePackagingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePackagingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
