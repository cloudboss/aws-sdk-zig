...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePackagingGroupInput, options: Options) !DescribePackagingGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePackagingGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePackagingGroupOutput {
    var result: DescribePackagingGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePackagingGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
