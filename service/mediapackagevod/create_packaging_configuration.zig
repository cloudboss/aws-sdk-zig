...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePackagingConfigurationInput, options: Options) !CreatePackagingConfigurationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePackagingConfigurationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"PackagingGroupId\":");
    try aws.json.writeValue(@TypeOf(input.packaging_group_id), input.packaging_group_id, alloc, &body_buf);
    has_prev = true;
    if (input.tags) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePackagingConfigurationOutput {
    var result: CreatePackagingConfigurationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePackagingConfigurationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
