...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribePlaceIndexInput, options: Options) !DescribePlaceIndexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribePlaceIndexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribePlaceIndexOutput {
    var result: DescribePlaceIndexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribePlaceIndexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
