...
pub const ListDevicePositionsInput = struct {
    /// The geometry used to filter device positions.
    filter_geometry: ?TrackingFilterGeometry = null,
    ...
    pub const json_field_names = .{
        .filter_geometry = "FilterGeometry",
...
pub const ListDevicePositionsOutput = struct {
    /// Contains details about each device's last known position.
    entries: ?[]const ListDevicePositionsResponseEntry = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListDevicePositionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.filter_geometry) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"FilterGeometry\":");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListDevicePositionsOutput {
    var result: ListDevicePositionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevicePositionsOutput, body, allocator);
    }
    _ = status;
    _ = headers;
