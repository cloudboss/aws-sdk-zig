...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDevicePositionsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.filter_geometry) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"FilterGeometry\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDevicePositionsOutput {
    var result: ListDevicePositionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevicePositionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
