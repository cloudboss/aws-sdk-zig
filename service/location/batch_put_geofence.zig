...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchPutGeofenceInput, options: Options) !BatchPutGeofenceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchPutGeofenceInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/put-geofences");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchPutGeofenceOutput {
    var result: BatchPutGeofenceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchPutGeofenceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
