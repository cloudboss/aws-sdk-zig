...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchPlaceIndexForSuggestionsInput, options: Options) !SearchPlaceIndexForSuggestionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchPlaceIndexForSuggestionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/places/v0/indexes/");
    try path_buf.appendSlice(alloc, input.index_name);
    try path_buf.appendSlice(alloc, "/search/suggestions");
    ...
    if (input.filter_b_box) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FilterBBox\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.filter_categories) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FilterCategories\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.filter_countries) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"FilterCountries\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.language) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchPlaceIndexForSuggestionsOutput {
    var result: SearchPlaceIndexForSuggestionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchPlaceIndexForSuggestionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
