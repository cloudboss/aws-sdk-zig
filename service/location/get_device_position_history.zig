...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDevicePositionHistoryInput, options: CallOptions) !GetDevicePositionHistoryOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetDevicePositionHistoryInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/list-positions");
    ...
    if (input.end_time_exclusive) |v| {
        ...
        try body_buf.appendSlice(allocator, "\"EndTimeExclusive\":");
    ...
    if (input.start_time_inclusive) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"StartTimeInclusive\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
