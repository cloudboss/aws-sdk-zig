...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGeofenceCollectionInput, options: Options) !UpdateGeofenceCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGeofenceCollectionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGeofenceCollectionOutput {
    var result: UpdateGeofenceCollectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGeofenceCollectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
