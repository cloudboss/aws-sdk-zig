...
pub const AssociateTrackerConsumerInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AssociateTrackerConsumerInput, options: Options) !AssociateTrackerConsumerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AssociateTrackerConsumerInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.consumer_arn), input.consumer_arn, allocator, &body_buf);
    has_prev = true;
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AssociateTrackerConsumerOutput {
    _ = allocator;
    _ = body;
    _ = status;
    _ = headers;
    const result: AssociateTrackerConsumerOutput = .{};
