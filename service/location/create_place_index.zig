...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreatePlaceIndexInput, options: Options) !CreatePlaceIndexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreatePlaceIndexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreatePlaceIndexOutput {
    var result: CreatePlaceIndexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreatePlaceIndexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
