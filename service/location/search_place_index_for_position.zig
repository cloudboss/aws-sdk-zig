...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchPlaceIndexForPositionInput, options: Options) !SearchPlaceIndexForPositionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchPlaceIndexForPositionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search/position");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchPlaceIndexForPositionOutput {
    var result: SearchPlaceIndexForPositionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchPlaceIndexForPositionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
