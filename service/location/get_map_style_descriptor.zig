...
pub const GetMapStyleDescriptorInput = struct {
    /// The optional [API
...
pub const GetMapStyleDescriptorOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMapStyleDescriptorInput, options: CallOptions) !GetMapStyleDescriptorOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMapStyleDescriptorInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/style-descriptor");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMapStyleDescriptorOutput {
    var result: GetMapStyleDescriptorOutput = .{};
    if (body.len > 0) {
