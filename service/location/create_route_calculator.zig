...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRouteCalculatorInput, options: Options) !CreateRouteCalculatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRouteCalculatorInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"CalculatorName\":");
    try aws.json.writeValue(@TypeOf(input.calculator_name), input.calculator_name, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRouteCalculatorOutput {
    var result: CreateRouteCalculatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateRouteCalculatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
