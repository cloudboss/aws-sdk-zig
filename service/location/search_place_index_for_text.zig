...
const SearchForTextResult = @import("search_for_text_result.zig").SearchForTextResult;
const SearchPlaceIndexForTextSummary = @import("search_place_index_for_text_summary.zig").SearchPlaceIndexForTextSummary;

pub const SearchPlaceIndexForTextInput = struct {
...
pub const SearchPlaceIndexForTextOutput = struct {
    ...
    results: ?[]const SearchForTextResult = null,
    ...
    summary: ?SearchPlaceIndexForTextSummary = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SearchPlaceIndexForTextInput, options: CallOptions) !SearchPlaceIndexForTextOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SearchPlaceIndexForTextInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/search/text");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SearchPlaceIndexForTextOutput {
    var result: SearchPlaceIndexForTextOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchPlaceIndexForTextOutput, body, allocator);
    }
    _ = status;
    _ = headers;
