...
fn serializeRequest(alloc: std.mem.Allocator, input: SearchPlaceIndexForTextInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/search/text");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SearchPlaceIndexForTextOutput {
    var result: SearchPlaceIndexForTextOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(SearchPlaceIndexForTextOutput, body, alloc);
    }
    _ = status;
    _ = headers;
