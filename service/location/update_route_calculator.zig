...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateRouteCalculatorInput, options: Options) !UpdateRouteCalculatorOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateRouteCalculatorInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateRouteCalculatorOutput {
    var result: UpdateRouteCalculatorOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateRouteCalculatorOutput, body, alloc);
    }
    _ = status;
    _ = headers;
