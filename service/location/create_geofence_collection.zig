...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateGeofenceCollectionInput, options: Options) !CreateGeofenceCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateGeofenceCollectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"CollectionName\":");
    try aws.json.writeValue(@TypeOf(input.collection_name), input.collection_name, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.pricing_plan_data_source) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PricingPlanDataSource\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateGeofenceCollectionOutput {
    var result: CreateGeofenceCollectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateGeofenceCollectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
