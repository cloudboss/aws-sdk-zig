...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DisassociateTrackerConsumerInput, options: Options) !DisassociateTrackerConsumerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DisassociateTrackerConsumerInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.consumer_arn);
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DisassociateTrackerConsumerOutput {
    _ = alloc;
    _ = body;
    _ = status;
    _ = headers;
