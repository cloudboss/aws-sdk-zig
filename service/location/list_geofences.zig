...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGeofencesInput, options: Options) !ListGeofencesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGeofencesInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/list-geofences");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGeofencesOutput {
    var result: ListGeofencesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGeofencesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
