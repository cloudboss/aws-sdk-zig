...
pub const GetMapTileInput = struct {
    /// The optional [API
...
pub const GetMapTileOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetMapTileInput, options: CallOptions) !GetMapTileOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetMapTileInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetMapTileOutput {
    var result: GetMapTileOutput = .{};
    if (body.len > 0) {
