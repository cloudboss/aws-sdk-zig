...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPlaceIndexesInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPlaceIndexesOutput {
    var result: ListPlaceIndexesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPlaceIndexesOutput, body, alloc);
    }
    _ = status;
    _ = headers;
