...
pub const ListPlaceIndexesInput = struct {
...
pub const ListPlaceIndexesOutput = struct {
    /// Lists the place index resources that exist in your Amazon Web Services
    /// account
    entries: ?[]const ListPlaceIndexesResponseEntry = null,
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListPlaceIndexesInput, config: *aws.Config) !aws.http.Request {
    ...
    const path = "/places/v0/list-indexes";
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListPlaceIndexesOutput {
    var result: ListPlaceIndexesOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListPlaceIndexesOutput, body, allocator);
    }
    _ = status;
    _ = headers;
