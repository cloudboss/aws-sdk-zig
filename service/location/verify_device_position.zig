...
fn serializeRequest(alloc: std.mem.Allocator, input: VerifyDevicePositionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/positions/verify");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !VerifyDevicePositionOutput {
    var result: VerifyDevicePositionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(VerifyDevicePositionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
