...
fn serializeRequest(alloc: std.mem.Allocator, input: CalculateRouteMatrixInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/calculate/route-matrix");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DeparturePositions\":");
    try aws.json.writeValue(@TypeOf(input.departure_positions), input.departure_positions, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DestinationPositions\":");
    try aws.json.writeValue(@TypeOf(input.destination_positions), input.destination_positions, alloc, &body_buf);
    has_prev = true;
