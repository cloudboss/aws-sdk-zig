...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeGeofenceCollectionInput, options: Options) !DescribeGeofenceCollectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeGeofenceCollectionInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeGeofenceCollectionOutput {
    var result: DescribeGeofenceCollectionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeGeofenceCollectionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
