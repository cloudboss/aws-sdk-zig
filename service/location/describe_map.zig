...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeMapInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeMapOutput {
    var result: DescribeMapOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeMapOutput, body, alloc);
    }
    _ = status;
    _ = headers;
