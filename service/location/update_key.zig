...
pub const UpdateKeyInput = struct {
...
pub const UpdateKeyOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKeyInput, options: Options) !UpdateKeyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/metadata/v0/keys/");
    ...
    if (input.force_update) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ForceUpdate\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateKeyOutput {
    var result: UpdateKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKeyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
