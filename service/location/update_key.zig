...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateKeyInput, options: CallOptions) !UpdateKeyOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: UpdateKeyInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.expire_time) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"ExpireTime\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
    ...
    if (input.no_expiry) |v| {
        if (has_prev) try body_buf.appendSlice(allocator, ",");
        try body_buf.appendSlice(allocator, "\"NoExpiry\":");
        try aws.json.writeValue(@TypeOf(v), v, allocator, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !UpdateKeyOutput {
    var result: UpdateKeyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateKeyOutput, body, allocator);
    }
    _ = status;
    _ = headers;
