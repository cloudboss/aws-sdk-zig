...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateTrackerInput, options: Options) !UpdateTrackerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateTrackerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateTrackerOutput {
    var result: UpdateTrackerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateTrackerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
