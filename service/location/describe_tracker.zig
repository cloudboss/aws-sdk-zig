...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeTrackerInput, options: Options) !DescribeTrackerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeTrackerInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeTrackerOutput {
    var result: DescribeTrackerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DescribeTrackerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
