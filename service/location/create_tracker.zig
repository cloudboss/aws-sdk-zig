...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateTrackerInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.event_bridge_enabled) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"EventBridgeEnabled\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.kms_key_enable_geospatial_queries) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"KmsKeyEnableGeospatialQueries\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.position_filtering) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"PositionFiltering\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"TrackerName\":");
    try aws.json.writeValue(@TypeOf(input.tracker_name), input.tracker_name, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateTrackerOutput {
    var result: CreateTrackerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateTrackerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
