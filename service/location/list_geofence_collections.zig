...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGeofenceCollectionsInput, options: Options) !ListGeofenceCollectionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGeofenceCollectionsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGeofenceCollectionsOutput {
    var result: ListGeofenceCollectionsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGeofenceCollectionsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
