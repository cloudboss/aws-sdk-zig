...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutGeofenceInput, options: Options) !PutGeofenceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: PutGeofenceInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.geofence_properties) |v| {
        ...
        try body_buf.appendSlice(alloc, "\"GeofenceProperties\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"Geometry\":");
    try aws.json.writeValue(@TypeOf(input.geometry), input.geometry, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutGeofenceOutput {
    var result: PutGeofenceOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(PutGeofenceOutput, body, alloc);
    }
    _ = status;
    _ = headers;
