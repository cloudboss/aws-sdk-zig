...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDevicePositionInput, options: Options) !GetDevicePositionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDevicePositionInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/positions/latest");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDevicePositionOutput {
    var result: GetDevicePositionOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDevicePositionOutput, body, alloc);
    }
    _ = status;
    _ = headers;
