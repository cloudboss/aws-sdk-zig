...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CalculateRouteInput, options: Options) !CalculateRouteOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CalculateRouteInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/routes/v0/calculators/");
    try path_buf.appendSlice(alloc, input.calculator_name);
    try path_buf.appendSlice(alloc, "/calculate/route");
    ...
    if (input.car_mode_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"CarModeOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DeparturePosition\":");
    try aws.json.writeValue(@TypeOf(input.departure_position), input.departure_position, alloc, &body_buf);
    has_prev = true;
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"DestinationPosition\":");
    try aws.json.writeValue(@TypeOf(input.destination_position), input.destination_position, alloc, &body_buf);
    has_prev = true;
    ...
    if (input.include_leg_geometry) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"IncludeLegGeometry\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.optimize_for) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"OptimizeFor\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    ...
    if (input.truck_mode_options) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"TruckModeOptions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.waypoint_positions) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"WaypointPositions\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CalculateRouteOutput {
    var result: CalculateRouteOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CalculateRouteOutput, body, alloc);
    }
    _ = status;
    _ = headers;
