...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdatePlaceIndexInput, options: Options) !UpdatePlaceIndexOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdatePlaceIndexInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdatePlaceIndexOutput {
    var result: UpdatePlaceIndexOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdatePlaceIndexOutput, body, alloc);
    }
    _ = status;
    _ = headers;
