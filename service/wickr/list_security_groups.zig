...
pub const ListSecurityGroupsInput = struct {
...
pub const ListSecurityGroupsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecurityGroupsInput, options: Options) !ListSecurityGroupsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityGroupsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecurityGroupsOutput {
    var result: ListSecurityGroupsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityGroupsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
