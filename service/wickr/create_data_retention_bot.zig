...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataRetentionBotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataRetentionBotOutput {
    var result: CreateDataRetentionBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataRetentionBotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
