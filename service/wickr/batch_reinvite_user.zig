...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchReinviteUserInput, options: Options) !BatchReinviteUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchReinviteUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/users/re-invite");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchReinviteUserOutput {
    var result: BatchReinviteUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchReinviteUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
