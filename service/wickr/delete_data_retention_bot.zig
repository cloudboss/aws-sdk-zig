...
pub const DeleteDataRetentionBotInput = struct {
...
pub const DeleteDataRetentionBotOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDataRetentionBotInput, options: CallOptions) !DeleteDataRetentionBotOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteDataRetentionBotOutput {
    var result: DeleteDataRetentionBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDataRetentionBotOutput, body, allocator);
    }
    _ = status;
    _ = headers;
