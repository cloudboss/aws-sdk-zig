...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDataRetentionBotInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDataRetentionBotOutput {
    var result: DeleteDataRetentionBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteDataRetentionBotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
