...
fn serializeRequest(allocator: std.mem.Allocator, input: ListUsersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.first_name) |v| {
        ...
        try query_buf.appendSlice(allocator, "firstName=");
    ...
    if (input.last_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(allocator, "&");
        try query_buf.appendSlice(allocator, "lastName=");
        try aws.url.appendUrlEncoded(allocator, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListUsersOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUsersOutput, body, allocator);
    }
    _ = status;
    _ = headers;
