...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListUsersInput, options: Options) !ListUsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListUsersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.first_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "firstName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.group_id) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "groupId=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.last_name) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "lastName=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListUsersOutput {
    var result: ListUsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListUsersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
