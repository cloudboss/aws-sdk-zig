...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchLookupUserUnameInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/users/uname-lookup");
    ...
    try body_buf.appendSlice(alloc, "\"unames\":");
    try aws.json.writeValue(@TypeOf(input.unames), input.unames, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchLookupUserUnameOutput {
    var result: BatchLookupUserUnameOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchLookupUserUnameOutput, body, alloc);
    }
    _ = status;
    _ = headers;
