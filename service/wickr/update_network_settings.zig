...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateNetworkSettingsInput, options: Options) !UpdateNetworkSettingsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateNetworkSettingsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateNetworkSettingsOutput {
    var result: UpdateNetworkSettingsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateNetworkSettingsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
