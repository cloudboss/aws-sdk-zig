...
pub const BatchDeleteUserInput = struct {
...
pub const BatchDeleteUserOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchDeleteUserInput, options: Options) !BatchDeleteUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchDeleteUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/users/batch-delete");
    ...
    try body_buf.appendSlice(alloc, "\"userIds\":");
    try aws.json.writeValue(@TypeOf(input.user_ids), input.user_ids, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchDeleteUserOutput {
    var result: BatchDeleteUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchDeleteUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
