...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListGuestUsersInput, options: Options) !ListGuestUsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListGuestUsersInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.billing_period) |v| {
        ...
        try query_buf.appendSlice(alloc, "billingPeriod=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListGuestUsersOutput {
    var result: ListGuestUsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListGuestUsersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
