...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateGuestUserInput, options: Options) !UpdateGuestUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateGuestUserInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/guest-users/");
    try path_buf.appendSlice(alloc, input.username_hash);
    ...
    try body_buf.appendSlice(alloc, "\"block\":");
    try aws.json.writeValue(@TypeOf(input.block), input.block, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateGuestUserOutput {
    var result: UpdateGuestUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateGuestUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
