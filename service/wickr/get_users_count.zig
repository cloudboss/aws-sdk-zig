...
fn serializeRequest(alloc: std.mem.Allocator, input: GetUsersCountInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/users/count");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetUsersCountOutput {
    var result: GetUsersCountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUsersCountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
