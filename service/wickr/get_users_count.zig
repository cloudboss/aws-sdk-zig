...
pub const GetUsersCountInput = struct {
...
pub const GetUsersCountOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetUsersCountInput, options: CallOptions) !GetUsersCountOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetUsersCountInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/users/count");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetUsersCountOutput {
    var result: GetUsersCountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetUsersCountOutput, body, allocator);
    }
    _ = status;
    _ = headers;
