...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSecurityGroupInput, options: Options) !CreateSecurityGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateSecurityGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"securityGroupSettings\":");
    try aws.json.writeValue(@TypeOf(input.security_group_settings), input.security_group_settings, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateSecurityGroupOutput {
    ...
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateSecurityGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
