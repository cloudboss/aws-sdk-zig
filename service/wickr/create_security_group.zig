...
const SecurityGroupSettingsRequest = @import("security_group_settings_request.zig").SecurityGroupSettingsRequest;
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateSecurityGroupInput, options: CallOptions) !CreateSecurityGroupOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CreateSecurityGroupInput, config: *aws.Config) !aws.http.Request {
    ...
    try aws.json.writeValue(@TypeOf(input.security_group_settings), input.security_group_settings, allocator, &body_buf);
    has_prev = true;
