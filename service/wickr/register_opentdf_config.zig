...
pub const RegisterOpentdfConfigInput = struct {
...
pub const RegisterOpentdfConfigOutput = struct {
    /// The OIDC client ID used for authenticating with the OpenTDF provider.
    client_id: []const u8,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterOpentdfConfigInput, options: CallOptions) !RegisterOpentdfConfigOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterOpentdfConfigInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.dry_run) |v| {
        ...
        try query_buf.appendSlice(allocator, "dryRun=");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterOpentdfConfigOutput {
    var result: RegisterOpentdfConfigOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(RegisterOpentdfConfigOutput, body, allocator);
    }
    _ = status;
    _ = headers;
