...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateSecurityGroupInput, options: Options) !UpdateSecurityGroupOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateSecurityGroupInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateSecurityGroupOutput {
    var result: UpdateSecurityGroupOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(UpdateSecurityGroupOutput, body, alloc);
    }
    _ = status;
    _ = headers;
