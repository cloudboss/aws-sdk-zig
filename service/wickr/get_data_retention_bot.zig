...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDataRetentionBotInput, options: Options) !GetDataRetentionBotOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDataRetentionBotInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/data-retention-bots");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDataRetentionBotOutput {
    var result: GetDataRetentionBotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetDataRetentionBotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
