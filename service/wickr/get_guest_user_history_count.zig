...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGuestUserHistoryCountInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/guest-users/count");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGuestUserHistoryCountOutput {
    var result: GetGuestUserHistoryCountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGuestUserHistoryCountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
