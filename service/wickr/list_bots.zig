...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBotsInput, options: Options) !ListBotsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBotsInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.display_name) |v| {
        ...
        try query_buf.appendSlice(alloc, "displayName=");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBotsOutput {
    var result: ListBotsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBotsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
