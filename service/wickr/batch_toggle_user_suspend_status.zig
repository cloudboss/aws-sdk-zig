...
fn serializeRequest(alloc: std.mem.Allocator, input: BatchToggleUserSuspendStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/users/toggleSuspend");
    const path = try path_buf.toOwnedSlice(alloc);

    var query_buf: std.ArrayList(u8) = .{};
    var query_has_prev = false;
    if (query_has_prev) try query_buf.appendSlice(alloc, "&");
    try query_buf.appendSlice(alloc, "suspend=");
    try query_buf.appendSlice(alloc, if (input.@"suspend") "true" else "false");
    query_has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !BatchToggleUserSuspendStatusOutput {
    var result: BatchToggleUserSuspendStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchToggleUserSuspendStatusOutput, body, alloc);
    }
    _ = status;
    _ = headers;
