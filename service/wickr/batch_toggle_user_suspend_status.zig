...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: BatchToggleUserSuspendStatusInput, options: Options) !BatchToggleUserSuspendStatusOutput {
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !BatchToggleUserSuspendStatusOutput {
    var result: BatchToggleUserSuspendStatusOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(BatchToggleUserSuspendStatusOutput, body, allocator);
    }
    _ = status;
    _ = headers;
