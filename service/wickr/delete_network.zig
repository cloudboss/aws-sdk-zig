...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteNetworkInput, options: Options) !DeleteNetworkOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteNetworkInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteNetworkOutput {
    var result: DeleteNetworkOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(DeleteNetworkOutput, body, alloc);
    }
    _ = status;
    _ = headers;
