...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateNetworkInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"accessLevel\":");
    try aws.json.writeValue(@TypeOf(input.access_level), input.access_level, alloc, &body_buf);
    has_prev = true;
    if (input.enable_premium_free_trial) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"enablePremiumFreeTrial\":");
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"networkName\":");
    try aws.json.writeValue(@TypeOf(input.network_name), input.network_name, alloc, &body_buf);
    has_prev = true;
