...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDataRetentionBotChallengeInput, options: Options) !CreateDataRetentionBotChallengeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDataRetentionBotChallengeInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/data-retention-bots/challenge");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDataRetentionBotChallengeOutput {
    var result: CreateDataRetentionBotChallengeOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateDataRetentionBotChallengeOutput, body, alloc);
    }
    _ = status;
    _ = headers;
