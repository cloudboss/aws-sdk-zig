...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSecurityGroupUsersInput, options: Options) !ListSecurityGroupUsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSecurityGroupUsersInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSecurityGroupUsersOutput {
    var result: ListSecurityGroupUsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSecurityGroupUsersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
