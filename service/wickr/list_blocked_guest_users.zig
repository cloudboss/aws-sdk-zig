...
const BlockedGuestUser = @import("blocked_guest_user.zig").BlockedGuestUser;

pub const ListBlockedGuestUsersInput = struct {
...
pub const ListBlockedGuestUsersOutput = struct {
    /// A list of blocked guest user objects within the current page.
    blocklist: ?[]const BlockedGuestUser = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListBlockedGuestUsersInput, options: Options) !ListBlockedGuestUsersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListBlockedGuestUsersInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/guest-users/blocklist");
    ...
    if (input.admin) |v| {
        ...
        try query_buf.appendSlice(alloc, "admin=");
    ...
    if (input.sort_direction) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sortDirection=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, @tagName(v));
        query_has_prev = true;
    }
    if (input.sort_fields) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "sortFields=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.username) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "username=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListBlockedGuestUsersOutput {
    var result: ListBlockedGuestUsersOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListBlockedGuestUsersOutput, body, alloc);
    }
    _ = status;
    _ = headers;
