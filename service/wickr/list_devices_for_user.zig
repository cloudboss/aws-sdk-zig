...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListDevicesForUserInput, options: Options) !ListDevicesForUserOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListDevicesForUserInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListDevicesForUserOutput {
    var result: ListDevicesForUserOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListDevicesForUserOutput, body, alloc);
    }
    _ = status;
    _ = headers;
