...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetBotsCountInput, options: Options) !GetBotsCountOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetBotsCountInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/bots/count");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetBotsCountOutput {
    var result: GetBotsCountOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetBotsCountOutput, body, alloc);
    }
    _ = status;
    _ = headers;
