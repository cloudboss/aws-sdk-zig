...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeregisterStreamConsumerInput, options: Options) !DeregisterStreamConsumerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeregisterStreamConsumerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.DeregisterStreamConsumer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeregisterStreamConsumerOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
