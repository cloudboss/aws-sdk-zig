...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: IncreaseStreamRetentionPeriodInput, options: Options) !IncreaseStreamRetentionPeriodOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: IncreaseStreamRetentionPeriodInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.IncreaseStreamRetentionPeriod");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !IncreaseStreamRetentionPeriodOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
