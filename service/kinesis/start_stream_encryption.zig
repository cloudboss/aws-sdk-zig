...
fn serializeRequest(alloc: std.mem.Allocator, input: StartStreamEncryptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.StartStreamEncryption");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartStreamEncryptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
