...
pub const StartStreamEncryptionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartStreamEncryptionInput, options: CallOptions) !StartStreamEncryptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartStreamEncryptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Kinesis_20131202.StartStreamEncryption");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartStreamEncryptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
