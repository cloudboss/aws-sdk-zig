...
pub const ListStreamConsumersPaginator = struct {
    client: *Client,
    params: list_stream_consumers.ListStreamConsumersInput,
    ...
    pub fn next(self: *Self, allocator: std.mem.Allocator, options: list_stream_consumers.Options) !list_stream_consumers.ListStreamConsumersOutput {
        if (self.done) {
        ...
        const output = try list_stream_consumers.execute(self.client, allocator, self.params, options);
