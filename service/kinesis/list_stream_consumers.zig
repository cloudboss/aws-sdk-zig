...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListStreamConsumersInput, options: Options) !ListStreamConsumersOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListStreamConsumersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.ListStreamConsumers");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListStreamConsumersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListStreamConsumersOutput, body, alloc);
}
