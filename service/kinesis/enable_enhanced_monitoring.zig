...
pub const EnableEnhancedMonitoringInput = struct {
...
pub const EnableEnhancedMonitoringOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableEnhancedMonitoringInput, options: CallOptions) !EnableEnhancedMonitoringOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableEnhancedMonitoringInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Kinesis_20131202.EnableEnhancedMonitoring");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableEnhancedMonitoringOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(EnableEnhancedMonitoringOutput, body, allocator);
}
