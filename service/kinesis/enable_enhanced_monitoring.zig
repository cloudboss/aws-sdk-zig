...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableEnhancedMonitoringInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.EnableEnhancedMonitoring");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableEnhancedMonitoringOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(EnableEnhancedMonitoringOutput, body, alloc);
}
