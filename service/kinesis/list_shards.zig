...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListShardsInput, options: CallOptions) !ListShardsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListShardsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Kinesis_20131202.ListShards");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListShardsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListShardsOutput, body, allocator);
}
