...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStreamSummaryInput, options: Options) !DescribeStreamSummaryOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStreamSummaryInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.DescribeStreamSummary");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStreamSummaryOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeStreamSummaryOutput, body, alloc);
}
