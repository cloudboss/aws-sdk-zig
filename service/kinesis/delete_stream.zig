...
const DeleteStreamOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteStreamInput, options: Options) !DeleteStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.DeleteStream");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
