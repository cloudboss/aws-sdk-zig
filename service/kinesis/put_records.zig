...
const PutRecordsRequestEntry = @import("put_records_request_entry.zig").PutRecordsRequestEntry;
...
pub const PutRecordsOutput = struct {
    ...
    records: ?[]const PutRecordsResultEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRecordsInput, options: CallOptions) !PutRecordsOutput {
    ...
    try aws.signing.signRequest(alloc, client.config.io, &request, creds, client.config.region, "kinesis");
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Kinesis_20131202.PutRecords");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutRecordsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutRecordsOutput, body, allocator);
}
