...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRecordsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.PutRecords");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRecordsOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutRecordsOutput, body, alloc);
}
