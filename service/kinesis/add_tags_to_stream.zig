...
pub const AddTagsToStreamInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AddTagsToStreamInput, options: CallOptions) !AddTagsToStreamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: AddTagsToStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Kinesis_20131202.AddTagsToStream");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !AddTagsToStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
