...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateMaxRecordSizeInput, options: Options) !UpdateMaxRecordSizeOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateMaxRecordSizeInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.UpdateMaxRecordSize");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateMaxRecordSizeOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
