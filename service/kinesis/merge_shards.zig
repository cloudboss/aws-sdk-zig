...
fn serializeRequest(alloc: std.mem.Allocator, input: MergeShardsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.MergeShards");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !MergeShardsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
