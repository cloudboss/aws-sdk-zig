...
pub const MergeShardsInput = struct {
    /// The shard ID of the adjacent shard for the merge.
    adjacent_shard_to_merge: []const u8,

    /// The shard ID of the shard to combine with the adjacent shard for the merge.
    shard_to_merge: []const u8,
    ...
    pub const json_field_names = .{
        .adjacent_shard_to_merge = "AdjacentShardToMerge",
        .shard_to_merge = "ShardToMerge",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: MergeShardsInput, options: CallOptions) !MergeShardsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: MergeShardsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Kinesis_20131202.MergeShards");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !MergeShardsOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
