...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateStreamWarmThroughputInput, options: Options) !UpdateStreamWarmThroughputOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateStreamWarmThroughputInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.UpdateStreamWarmThroughput");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateStreamWarmThroughputOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateStreamWarmThroughputOutput, body, alloc);
}
