...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTagsForStreamInput, options: Options) !ListTagsForStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTagsForStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.ListTagsForStream");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTagsForStreamOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(ListTagsForStreamOutput, body, alloc);
}
