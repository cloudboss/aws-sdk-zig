...
pub const RemoveTagsFromStreamInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RemoveTagsFromStreamInput, options: CallOptions) !RemoveTagsFromStreamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RemoveTagsFromStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Kinesis_20131202.RemoveTagsFromStream");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RemoveTagsFromStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
