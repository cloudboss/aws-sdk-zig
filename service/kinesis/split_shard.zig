...
fn serializeRequest(alloc: std.mem.Allocator, input: SplitShardInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.SplitShard");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !SplitShardOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
