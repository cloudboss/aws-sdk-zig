...
pub const SplitShardInput = struct {
    /// A hash key value for the starting hash key of one of the child shards
    /// created by the
    /// split. The hash key range for a given shard constitutes a set of ordered
    /// contiguous
    /// positive integers. The value for `NewStartingHashKey` must be in the range
    /// of
    /// hash keys being mapped into the shard. The `NewStartingHashKey` hash key
    /// value and all higher hash key values in hash key range are distributed to
    /// one of the
    /// child shards. All the lower hash key values in the range are distributed to
    /// the other
    /// child shard.
    new_starting_hash_key: []const u8,

    /// The shard ID of the shard to split.
    shard_to_split: []const u8,
    ...
    pub const json_field_names = .{
        .new_starting_hash_key = "NewStartingHashKey",
        .shard_to_split = "ShardToSplit",
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: SplitShardInput, options: CallOptions) !SplitShardOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: SplitShardInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Kinesis_20131202.SplitShard");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !SplitShardOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
