...
pub const RegisterStreamConsumerInput = struct {
...
pub const RegisterStreamConsumerOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RegisterStreamConsumerInput, options: CallOptions) !RegisterStreamConsumerOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: RegisterStreamConsumerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Kinesis_20131202.RegisterStreamConsumer");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !RegisterStreamConsumerOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(RegisterStreamConsumerOutput, body, allocator);
}
