...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeStreamConsumerInput, options: Options) !DescribeStreamConsumerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeStreamConsumerInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Kinesis_20131202.DescribeStreamConsumer");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeStreamConsumerOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeStreamConsumerOutput, body, alloc);
}
