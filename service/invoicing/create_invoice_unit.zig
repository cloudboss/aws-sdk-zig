...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateInvoiceUnitInput, options: Options) !CreateInvoiceUnitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateInvoiceUnitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Invoicing.CreateInvoiceUnit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateInvoiceUnitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateInvoiceUnitOutput, body, alloc);
}
