...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInvoiceSummariesInput, options: Options) !ListInvoiceSummariesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInvoiceSummariesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Invoicing.ListInvoiceSummaries");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInvoiceSummariesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInvoiceSummariesOutput, body, alloc);
}
