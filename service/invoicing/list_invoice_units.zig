...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListInvoiceUnitsInput, options: Options) !ListInvoiceUnitsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListInvoiceUnitsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Invoicing.ListInvoiceUnits");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListInvoiceUnitsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListInvoiceUnitsOutput, body, alloc);
}
