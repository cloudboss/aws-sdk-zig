...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateProcurementPortalPreferenceStatusInput, options: Options) !UpdateProcurementPortalPreferenceStatusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateProcurementPortalPreferenceStatusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Invoicing.UpdateProcurementPortalPreferenceStatus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateProcurementPortalPreferenceStatusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateProcurementPortalPreferenceStatusOutput, body, alloc);
}
