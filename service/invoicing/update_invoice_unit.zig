...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UpdateInvoiceUnitInput, options: Options) !UpdateInvoiceUnitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UpdateInvoiceUnitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Invoicing.UpdateInvoiceUnit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UpdateInvoiceUnitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(UpdateInvoiceUnitOutput, body, alloc);
}
