...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetInvoiceUnitInput, options: Options) !GetInvoiceUnitOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetInvoiceUnitInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Invoicing.GetInvoiceUnit");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetInvoiceUnitOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetInvoiceUnitOutput, body, alloc);
}
