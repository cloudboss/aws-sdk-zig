...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CheckAccessNotGrantedInput, options: Options) !CheckAccessNotGrantedOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CheckAccessNotGrantedInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"access\":");
    try aws.json.writeValue(@TypeOf(input.access), input.access, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CheckAccessNotGrantedOutput {
    var result: CheckAccessNotGrantedOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CheckAccessNotGrantedOutput, body, alloc);
    }
    _ = status;
    _ = headers;
