...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAccessPreviewsInput, options: Options) !ListAccessPreviewsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAccessPreviewsInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAccessPreviewsOutput {
    var result: ListAccessPreviewsOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListAccessPreviewsOutput, body, alloc);
    }
    _ = status;
    _ = headers;
