...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartPolicyGenerationInput, options: Options) !StartPolicyGenerationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartPolicyGenerationInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.cloud_trail_details) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"cloudTrailDetails\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"policyGenerationDetails\":");
    try aws.json.writeValue(@TypeOf(input.policy_generation_details), input.policy_generation_details, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartPolicyGenerationOutput {
    var result: StartPolicyGenerationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartPolicyGenerationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
