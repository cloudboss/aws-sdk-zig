...
fn serializeRequest(alloc: std.mem.Allocator, input: CheckNoNewAccessInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"existingPolicyDocument\":");
    try aws.json.writeValue(@TypeOf(input.existing_policy_document), input.existing_policy_document, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"newPolicyDocument\":");
    try aws.json.writeValue(@TypeOf(input.new_policy_document), input.new_policy_document, alloc, &body_buf);
    has_prev = true;
    if (has_prev) try body_buf.appendSlice(alloc, ",");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CheckNoNewAccessOutput {
    var result: CheckNoNewAccessOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CheckNoNewAccessOutput, body, alloc);
    }
    _ = status;
    _ = headers;
