...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAnalyzerInput, options: Options) !CreateAnalyzerOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAnalyzerInput, config: *aws.Config) !aws.http.Request {
    ...
    try body_buf.appendSlice(alloc, "\"analyzerName\":");
    try aws.json.writeValue(@TypeOf(input.analyzer_name), input.analyzer_name, alloc, &body_buf);
    has_prev = true;
    if (input.archive_rules) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"archiveRules\":");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAnalyzerOutput {
    var result: CreateAnalyzerOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAnalyzerOutput, body, alloc);
    }
    _ = status;
    _ = headers;
