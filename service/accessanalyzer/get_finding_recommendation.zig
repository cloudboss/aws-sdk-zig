...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingRecommendationInput, options: Options) !GetFindingRecommendationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingRecommendationInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/recommendation/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingRecommendationOutput {
    var result: GetFindingRecommendationOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingRecommendationOutput, body, alloc);
    }
    _ = status;
    _ = headers;
