...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CheckNoPublicAccessInput, options: Options) !CheckNoPublicAccessOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CheckNoPublicAccessInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CheckNoPublicAccessOutput {
    var result: CheckNoPublicAccessOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CheckNoPublicAccessOutput, body, alloc);
    }
    _ = status;
    _ = headers;
