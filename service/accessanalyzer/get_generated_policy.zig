...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetGeneratedPolicyInput, options: Options) !GetGeneratedPolicyOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetGeneratedPolicyInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/policy/generation/");
    ...
    if (input.include_resource_placeholders) |v| {
        ...
        try query_buf.appendSlice(alloc, "includeResourcePlaceholders=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
    if (input.include_service_level_template) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "includeServiceLevelTemplate=");
        try query_buf.appendSlice(alloc, if (v) "true" else "false");
        query_has_prev = true;
    }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetGeneratedPolicyOutput {
    var result: GetGeneratedPolicyOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetGeneratedPolicyOutput, body, alloc);
    }
    _ = status;
    _ = headers;
