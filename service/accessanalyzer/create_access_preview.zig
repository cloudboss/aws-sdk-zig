...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAccessPreviewInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAccessPreviewOutput {
    var result: CreateAccessPreviewOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(CreateAccessPreviewOutput, body, alloc);
    }
    _ = status;
    _ = headers;
