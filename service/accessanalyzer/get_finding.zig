...
fn serializeRequest(alloc: std.mem.Allocator, input: GetFindingInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, "/finding/");
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetFindingOutput {
    var result: GetFindingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingOutput, body, alloc);
    }
    _ = status;
    _ = headers;
