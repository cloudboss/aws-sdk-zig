...
pub const GetFindingInput = struct {
    /// The [ARN of the
...
pub const GetFindingOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetFindingInput, options: CallOptions) !GetFindingOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: GetFindingInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(allocator, "/finding/");
...
fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !GetFindingOutput {
    var result: GetFindingOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(GetFindingOutput, body, allocator);
    }
    _ = status;
    _ = headers;
