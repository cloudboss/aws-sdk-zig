...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventSourceInput, options: Options) !DescribeEventSourceOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEventSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.DescribeEventSource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEventSourceOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEventSourceOutput, body, alloc);
}
