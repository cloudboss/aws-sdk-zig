...
pub const CancelReplayInput = struct {
...
pub const CancelReplayOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CancelReplayInput, options: CallOptions) !CancelReplayOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: CancelReplayInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSEvents.CancelReplay");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !CancelReplayOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CancelReplayOutput, body, allocator);
}
