...
fn serializeRequest(alloc: std.mem.Allocator, input: ActivateEventSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ActivateEventSource");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ActivateEventSourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
