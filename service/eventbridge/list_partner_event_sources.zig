...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListPartnerEventSourcesInput, options: Options) !ListPartnerEventSourcesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListPartnerEventSourcesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListPartnerEventSources");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListPartnerEventSourcesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListPartnerEventSourcesOutput, body, alloc);
}
