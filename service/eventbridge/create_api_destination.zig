...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateApiDestinationInput, options: Options) !CreateApiDestinationOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateApiDestinationInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.CreateApiDestination");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateApiDestinationOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateApiDestinationOutput, body, alloc);
}
