...
pub const DescribeRuleOutput = struct {
    ...
    /// behalf, this
    /// field displays the principal name of the Amazon Web Services service that
    /// created the
    /// rule.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeRuleInput, options: Options) !DescribeRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.DescribeRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeRuleOutput, body, alloc);
}
