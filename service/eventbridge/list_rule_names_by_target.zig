...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListRuleNamesByTargetInput, options: Options) !ListRuleNamesByTargetOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListRuleNamesByTargetInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListRuleNamesByTarget");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListRuleNamesByTargetOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListRuleNamesByTargetOutput, body, alloc);
}
