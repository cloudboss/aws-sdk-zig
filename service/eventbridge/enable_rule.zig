...
pub const EnableRuleInput = struct {
...
pub const EnableRuleOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableRuleInput, options: CallOptions) !EnableRuleOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: EnableRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSEvents.EnableRule");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !EnableRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
