...
const EnableRuleOutput = struct {};

pub const Options = struct {
    diagnostic: ?*ServiceError = null,
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: EnableRuleInput, options: Options) !EnableRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: EnableRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.EnableRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !EnableRuleOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
