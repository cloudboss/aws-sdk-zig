...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartReplayInput, options: Options) !StartReplayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartReplayInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.StartReplay");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartReplayOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartReplayOutput, body, alloc);
}
