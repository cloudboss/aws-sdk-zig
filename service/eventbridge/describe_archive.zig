...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeArchiveInput, options: Options) !DescribeArchiveOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeArchiveInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.DescribeArchive");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeArchiveOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeArchiveOutput, body, alloc);
}
