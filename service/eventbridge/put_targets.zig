...
const PutTargetsResultEntry = @import("put_targets_result_entry.zig").PutTargetsResultEntry;

pub const PutTargetsInput = struct {
...
pub const PutTargetsOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutTargetsInput, options: CallOptions) !PutTargetsOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutTargetsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSEvents.PutTargets");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutTargetsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(PutTargetsOutput, body, allocator);
}
