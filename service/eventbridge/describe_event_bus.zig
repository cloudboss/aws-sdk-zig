...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeEventBusInput, options: Options) !DescribeEventBusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeEventBusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.DescribeEventBus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeEventBusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeEventBusOutput, body, alloc);
}
