...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeReplayInput, options: Options) !DescribeReplayOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DescribeReplayInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.DescribeReplay");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DescribeReplayOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DescribeReplayOutput, body, alloc);
}
