...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListEventBusesInput, options: Options) !ListEventBusesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListEventBusesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListEventBuses");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListEventBusesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListEventBusesOutput, body, alloc);
}
