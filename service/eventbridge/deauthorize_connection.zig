...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeauthorizeConnectionInput, options: Options) !DeauthorizeConnectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeauthorizeConnectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.DeauthorizeConnection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeauthorizeConnectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DeauthorizeConnectionOutput, body, alloc);
}
