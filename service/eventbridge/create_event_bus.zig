...
pub const CreateEventBusInput = struct {
    ...
    /// `/` character in partner event bus names. In addition, for partner event
    /// buses,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateEventBusInput, options: Options) !CreateEventBusOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateEventBusInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.CreateEventBus");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateEventBusOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateEventBusOutput, body, alloc);
}
