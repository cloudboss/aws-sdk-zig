...
pub const DeactivateEventSourceInput = struct {
...
pub const DeactivateEventSourceOutput = struct {};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeactivateEventSourceInput, options: CallOptions) !DeactivateEventSourceOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeactivateEventSourceInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "AWSEvents.DeactivateEventSource");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeactivateEventSourceOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
