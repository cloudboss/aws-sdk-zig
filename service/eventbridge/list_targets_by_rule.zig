...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListTargetsByRuleInput, options: Options) !ListTargetsByRuleOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListTargetsByRuleInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListTargetsByRule");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListTargetsByRuleOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListTargetsByRuleOutput, body, alloc);
}
