...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListArchivesInput, options: Options) !ListArchivesOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListArchivesInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListArchives");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListArchivesOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListArchivesOutput, body, alloc);
}
