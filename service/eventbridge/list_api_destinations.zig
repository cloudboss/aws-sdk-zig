...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListApiDestinationsInput, options: Options) !ListApiDestinationsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListApiDestinationsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSEvents.ListApiDestinations");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListApiDestinationsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListApiDestinationsOutput, body, alloc);
}
