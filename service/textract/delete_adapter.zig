...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteAdapterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.DeleteAdapter");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteAdapterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
