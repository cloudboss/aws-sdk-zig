...
pub const DeleteAdapterInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAdapterInput, options: Options) !DeleteAdapterOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAdapterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Textract.DeleteAdapter");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAdapterOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
