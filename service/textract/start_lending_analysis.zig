...
pub const StartLendingAnalysisInput = struct {
    /// The idempotent token that you use to identify the start request. If you use
    /// the same token
    /// with multiple `StartLendingAnalysis` requests, the same `JobId` is
    /// returned. Use `ClientRequestToken` to prevent the same job from being
    /// accidentally
    /// started more than once. For more information, see [Calling Amazon Textract
    /// Asynchronous
    /// Operations](https://docs.aws.amazon.com/textract/latest/dg/api-sync.html).
    ...
    /// published to
    /// the Amazon SNS topic. For example, you can use `JobTag` to identify the type
    /// of
    ...
    /// server-side encryption of
    /// the objects in the customer bucket. When this parameter is not enabled, the
    /// result will be
...
pub const StartLendingAnalysisOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartLendingAnalysisInput, options: CallOptions) !StartLendingAnalysisOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartLendingAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Textract.StartLendingAnalysis");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartLendingAnalysisOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartLendingAnalysisOutput, body, allocator);
}
