...
pub const StartDocumentTextDetectionInput = struct {
    ...
    /// Textract will
    /// save the results internally to be accessed with the GetDocumentTextDetection
    /// operation.
...
pub const StartDocumentTextDetectionOutput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDocumentTextDetectionInput, options: CallOptions) !StartDocumentTextDetectionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartDocumentTextDetectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Textract.StartDocumentTextDetection");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartDocumentTextDetectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartDocumentTextDetectionOutput, body, allocator);
}
