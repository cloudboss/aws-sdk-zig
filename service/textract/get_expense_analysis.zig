...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetExpenseAnalysisInput, options: Options) !GetExpenseAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetExpenseAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.GetExpenseAnalysis");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetExpenseAnalysisOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetExpenseAnalysisOutput, body, alloc);
}
