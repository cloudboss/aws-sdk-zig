...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDocumentAnalysisInput, options: Options) !GetDocumentAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDocumentAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.GetDocumentAnalysis");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDocumentAnalysisOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDocumentAnalysisOutput, body, alloc);
}
