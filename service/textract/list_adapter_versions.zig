...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAdapterVersionsInput, options: Options) !ListAdapterVersionsOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListAdapterVersionsInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.ListAdapterVersions");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListAdapterVersionsOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAdapterVersionsOutput, body, alloc);
}
