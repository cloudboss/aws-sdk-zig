...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAdapterInput, options: Options) !GetAdapterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAdapterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.GetAdapter");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAdapterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAdapterOutput, body, alloc);
}
