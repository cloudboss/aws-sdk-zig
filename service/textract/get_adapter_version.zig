...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetAdapterVersionInput, options: Options) !GetAdapterVersionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetAdapterVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.GetAdapterVersion");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetAdapterVersionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetAdapterVersionOutput, body, alloc);
}
