...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetLendingAnalysisInput, options: Options) !GetLendingAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetLendingAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.GetLendingAnalysis");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetLendingAnalysisOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetLendingAnalysisOutput, body, alloc);
}
