...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DetectDocumentTextInput, options: Options) !DetectDocumentTextOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DetectDocumentTextInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.DetectDocumentText");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DetectDocumentTextOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(DetectDocumentTextOutput, body, alloc);
}
