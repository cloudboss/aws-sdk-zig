...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateAdapterInput, options: Options) !CreateAdapterOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateAdapterInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.CreateAdapter");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateAdapterOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateAdapterOutput, body, alloc);
}
