...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AnalyzeExpenseInput, options: Options) !AnalyzeExpenseOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AnalyzeExpenseInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.AnalyzeExpense");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AnalyzeExpenseOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AnalyzeExpenseOutput, body, alloc);
}
