...
const AdapterOverview = @import("adapter_overview.zig").AdapterOverview;

pub const ListAdaptersInput = struct {
    /// Specifies the lower bound for the ListAdapters operation.
    /// Ensures ListAdapters returns only adapters created after the specified
    /// creation time.
    after_creation_time: ?i64 = null,

    /// Specifies the upper bound for the ListAdapters operation.
    /// Ensures ListAdapters returns only adapters created before the specified
    /// creation time.
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListAdaptersInput, options: CallOptions) !ListAdaptersOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: ListAdaptersInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Textract.ListAdapters");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !ListAdaptersOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(ListAdaptersOutput, body, allocator);
}
