...
pub const DeleteAdapterVersionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteAdapterVersionInput, options: Options) !DeleteAdapterVersionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DeleteAdapterVersionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Textract.DeleteAdapterVersion");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DeleteAdapterVersionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
