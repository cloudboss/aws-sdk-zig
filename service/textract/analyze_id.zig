...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: AnalyzeIDInput, options: Options) !AnalyzeIDOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: AnalyzeIDInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.AnalyzeID");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !AnalyzeIDOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(AnalyzeIDOutput, body, alloc);
}
