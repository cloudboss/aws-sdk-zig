...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDocumentAnalysisInput, options: Options) !StartDocumentAnalysisOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: StartDocumentAnalysisInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.StartDocumentAnalysis");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartDocumentAnalysisOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(StartDocumentAnalysisOutput, body, alloc);
}
