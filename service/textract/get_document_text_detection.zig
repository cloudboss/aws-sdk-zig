...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: GetDocumentTextDetectionInput, options: Options) !GetDocumentTextDetectionOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: GetDocumentTextDetectionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Textract.GetDocumentTextDetection");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !GetDocumentTextDetectionOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(GetDocumentTextDetectionOutput, body, alloc);
}
