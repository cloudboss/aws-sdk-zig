...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateDeliveryStreamInput, options: Options) !CreateDeliveryStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateDeliveryStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Firehose_20150804.CreateDeliveryStream");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateDeliveryStreamOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateDeliveryStreamOutput, body, alloc);
}
