...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DeleteDeliveryStreamInput, options: Options) !DeleteDeliveryStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: DeleteDeliveryStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Firehose_20150804.DeleteDeliveryStream");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !DeleteDeliveryStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
