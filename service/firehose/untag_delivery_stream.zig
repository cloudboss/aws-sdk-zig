...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: UntagDeliveryStreamInput, options: Options) !UntagDeliveryStreamOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: UntagDeliveryStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Firehose_20150804.UntagDeliveryStream");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !UntagDeliveryStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = alloc;
