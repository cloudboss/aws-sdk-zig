...
pub const TagDeliveryStreamInput = struct {
...
pub const TagDeliveryStreamOutput = struct {
};

pub fn execute(client: *Client, allocator: std.mem.Allocator, input: TagDeliveryStreamInput, options: CallOptions) !TagDeliveryStreamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: TagDeliveryStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Firehose_20150804.TagDeliveryStream");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !TagDeliveryStreamOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
