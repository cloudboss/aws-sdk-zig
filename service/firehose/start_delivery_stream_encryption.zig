...
pub const StartDeliveryStreamEncryptionInput = struct {
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: StartDeliveryStreamEncryptionInput, options: Options) !StartDeliveryStreamEncryptionOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: StartDeliveryStreamEncryptionInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Firehose_20150804.StartDeliveryStreamEncryption");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !StartDeliveryStreamEncryptionOutput {
    _ = status;
    _ = headers;
    _ = body;
    _ = allocator;
