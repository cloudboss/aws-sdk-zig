...
const DeliveryStreamDescription = @import("delivery_stream_description.zig").DeliveryStreamDescription;

pub const DescribeDeliveryStreamInput = struct {
    ...
    exclusive_start_destination_id: ?[]const u8 = null,
...
pub const DescribeDeliveryStreamOutput = struct {
    /// Information about the Firehose stream.
    delivery_stream_description: ?DeliveryStreamDescription = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: DescribeDeliveryStreamInput, options: CallOptions) !DescribeDeliveryStreamOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: DescribeDeliveryStreamInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Firehose_20150804.DescribeDeliveryStream");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !DescribeDeliveryStreamOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(DescribeDeliveryStreamOutput, body, allocator);
}
...
fn parseErrorResponse(allocator: std.mem.Allocator, body: []const u8, status: u16) !ServiceError {
    ...
    if (std.mem.eql(u8, error_code, "InvalidKMSResourceException")) {
        const parsed_error: ?errors.InvalidKMSResourceException = aws.json.parseJsonObject(errors.InvalidKMSResourceException, body, arena_alloc) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            else => null,
        };
        if (parsed_error) |parsed| {
            ...
            typed_error.message = owned_message;
            typed_error.request_id = owned_request_id;
            return .{ .arena = arena, .kind = .{ .invalid_kms_resource_exception = typed_error } };
        }
    }
