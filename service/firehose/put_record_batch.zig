...
fn serializeRequest(alloc: std.mem.Allocator, input: PutRecordBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "Firehose_20150804.PutRecordBatch");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !PutRecordBatchOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutRecordBatchOutput, body, alloc);
}
