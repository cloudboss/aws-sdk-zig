...
const PutRecordBatchResponseEntry = @import("put_record_batch_response_entry.zig").PutRecordBatchResponseEntry;

pub const PutRecordBatchInput = struct {
...
pub const PutRecordBatchOutput = struct {
    ...
    request_responses: ?[]const PutRecordBatchResponseEntry = null,
...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: PutRecordBatchInput, options: CallOptions) !PutRecordBatchOutput {
...
fn serializeRequest(allocator: std.mem.Allocator, input: PutRecordBatchInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(allocator, "X-Amz-Target", "Firehose_20150804.PutRecordBatch");

    return request;
}

fn deserializeResponse(allocator: std.mem.Allocator, body: []const u8, status: u16, headers: anytype) !PutRecordBatchOutput {
    _ = status;
    _ = headers;
    return aws.json.parseJsonObject(PutRecordBatchOutput, body, allocator);
}
