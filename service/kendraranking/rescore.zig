...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: RescoreInput, options: Options) !RescoreOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: RescoreInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraRerankingFrontendService.Rescore");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !RescoreOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(RescoreOutput, body, alloc);
}
