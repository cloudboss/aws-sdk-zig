...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: CreateRescoreExecutionPlanInput, options: Options) !CreateRescoreExecutionPlanOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: CreateRescoreExecutionPlanInput, config: *aws.Config) !aws.http.Request {
    ...
    try request.headers.put(alloc, "X-Amz-Target", "AWSKendraRerankingFrontendService.CreateRescoreExecutionPlan");

    return request;
}

fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !CreateRescoreExecutionPlanOutput {
    _ = status;
    _ = headers;
    if (body.len == 0) return .{};
    return aws.json.parseJsonObject(CreateRescoreExecutionPlanOutput, body, alloc);
}
