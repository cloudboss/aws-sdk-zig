...
fn serializeRequest(alloc: std.mem.Allocator, input: StartSnapshotInput, config: *aws.Config) !aws.http.Request {
    ...
    if (input.parent_snapshot_id) |v| {
        if (has_prev) try body_buf.appendSlice(alloc, ",");
        try body_buf.appendSlice(alloc, "\"ParentSnapshotId\":");
        try aws.json.writeValue(@TypeOf(v), v, alloc, &body_buf);
        has_prev = true;
    }
    if (input.tags) |v| {
    ...
    if (has_prev) try body_buf.appendSlice(alloc, ",");
    try body_buf.appendSlice(alloc, "\"VolumeSize\":");
    try aws.json.writeValue(@TypeOf(input.volume_size), input.volume_size, alloc, &body_buf);
    has_prev = true;
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !StartSnapshotOutput {
    var result: StartSnapshotOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(StartSnapshotOutput, body, alloc);
    }
    _ = status;
    _ = headers;
