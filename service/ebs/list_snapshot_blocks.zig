...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListSnapshotBlocksInput, options: Options) !ListSnapshotBlocksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListSnapshotBlocksInput, config: *aws.Config) !aws.http.Request {
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListSnapshotBlocksOutput {
    var result: ListSnapshotBlocksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListSnapshotBlocksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
