...
pub fn execute(client: *Client, allocator: std.mem.Allocator, input: ListChangedBlocksInput, options: Options) !ListChangedBlocksOutput {
...
fn serializeRequest(alloc: std.mem.Allocator, input: ListChangedBlocksInput, config: *aws.Config) !aws.http.Request {
    ...
    try path_buf.appendSlice(alloc, input.second_snapshot_id);
    try path_buf.appendSlice(alloc, "/changedblocks");
    ...
    if (input.first_snapshot_id) |v| {
        ...
        try query_buf.appendSlice(alloc, "firstSnapshotId=");
    ...
    if (input.next_token) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "pageToken=");
        try aws.url.appendUrlEncoded(alloc, &query_buf, v);
        query_has_prev = true;
    }
    if (input.starting_block_index) |v| {
        if (query_has_prev) try query_buf.appendSlice(alloc, "&");
        try query_buf.appendSlice(alloc, "startingBlockIndex=");
        {
            const num_str = std.fmt.allocPrint(alloc, "{d}", .{v}) catch "";
            try query_buf.appendSlice(alloc, num_str);
        }
...
fn deserializeResponse(body: []const u8, status: u16, headers: anytype, alloc: std.mem.Allocator) !ListChangedBlocksOutput {
    var result: ListChangedBlocksOutput = .{};
    if (body.len > 0) {
        result = try aws.json.parseJsonObject(ListChangedBlocksOutput, body, alloc);
    }
    _ = status;
    _ = headers;
